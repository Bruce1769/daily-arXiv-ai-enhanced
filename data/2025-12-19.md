...
TL;DR: 提出了一种结合规则变形和GAN图像转换的两步生成式数据增强框架，用于解决戴口罩人脸检测和识别中的数据稀缺和分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是戴口罩人脸检测和识别面临的主要挑战，需要生成更真实的戴口罩人脸样本来增强训练数据。

Method: 采用两步生成框架：1) 基于规则的口罩变形；2) 使用GAN进行非配对图像到图像转换。引入非口罩保护损失和随机噪声注入来稳定训练并增强样本多样性。

Result: 相比单纯基于规则的变形方法，该框架在质量上取得一致改进，并补充了现有的GAN方法如IAMGAN。实验验证了所提组件的有效性。

Conclusion: 该研究为面向人脸识别任务的数据中心增强提供了有效方法，并指出了未来改进方向。
...
TL;DR: JARVIS框架通过自监督视觉增强提升MLLMs的视觉推理能力，解决现有MLLMs过度依赖语言先验而忽视视觉细节的问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉推理任务上表现有限，主要原因是：1）从文本描述学习视觉理解，这是主观且不完整的监督信号；2）多模态指令调优规模远小于纯文本预训练，导致模型过度拟合语言先验而忽略视觉细节

Method: 提出JARVIS框架，将I-JEPA学习范式集成到MLLMs的标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练LLM早期层作为预测器，从图像中学习结构和语义规律，不依赖语言监督
...
<details>
  <summary>Details</summary>
Motivation: 当前基于多模态大语言模型的具身智能体评估基准主要关注语言中心任务或模拟环境，缺乏对真实世界复杂场景中知识密集型推理能力的评估，需要建立更贴近实际应用的评估框架。

Method: 提出了稀疏接地视觉导航任务，并构建了CityNav基准，涵盖四个全球城市的真实环境。要求智能体仅依赖视觉输入和多模态推理，在50多个决策点上进行顺序导航。同时提出了Verbalization of Path方法，通过显式构建认知地图（关键地标和方向）来增强导航能力。

Result: 实验表明，当前最先进的多模态大语言模型和标准推理技术（如思维链、反思）在这一挑战性场景中表现显著不足。而提出的Verbalization of Path方法能够大幅提升导航成功率，证明了显式认知地图构建的重要性。

Conclusion: 稀疏接地视觉导航任务和CityNav基准为评估多模态大语言模型在真实世界环境中的导航能力提供了重要框架。Verbalization of Path方法通过显式构建认知地图有效增强了智能体的导航性能，为未来具身智能研究提供了新方向。
...
TL;DR: R4是一个无需训练的四维时空检索增强推理框架，为视觉语言模型提供结构化终身记忆，通过将对象级语义描述锚定在度量空间和时间中构建4D知识库，实现动态环境中的具身推理。


<details>
  <summary>Details</summary>
Motivation: 受人类在四维时空（三维空间+时间）中感知和推理能力的启发，希望为视觉语言模型提供类似的结构化、持久性内部表示，使其能够回忆过去事件、推断未观察状态，并在动态环境中进行上下文依赖的推理。

Method: R4框架通过将对象级语义描述锚定在度量空间和时间中，持续构建4D知识数据库，形成持久的世界模型。推理时，自然语言查询被分解为语义、空间和时间键，用于检索相关观察结果，然后整合到视觉语言模型的推理过程中。

Result: 在具身问答和导航基准测试中，R4在时空信息检索和推理方面显著优于基线方法，展示了无需训练即可实现情节式和协作式推理的能力。

Conclusion: R4为动态环境中的具身4D推理提供了新范式，通过结构化终身记忆增强视觉语言模型在时空维度上的推理能力，无需额外训练即可实现高效检索和推理。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型发展迅速，但模型的感知能力缺乏系统评估。大多数模型家族在扩展语言组件时重用几乎相同的视觉编码器，这引发了一个关键问题：模型的进步是真正基于视觉基础，还是主要依赖互联网规模的文本世界知识。现有评估方法过于强调端任务准确性，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测台"框架，通过多个垂直维度评估MLLMs：1）简单视觉任务（如人脸匹配和文本视觉理解能力）；2）局部到全局理解（包括图像匹配、网格指向游戏和属性定位），测试通用视觉基础。每个垂直维度都使用真实数据集（人脸和文字），并通过基于像素的增强和基于扩散的风格化幻觉进行系统扰动。

Result: 该框架超越了排行榜准确性评估，能够深入分析MLLMs在扰动下如何保持感知基础和关系结构，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: "感知观测台"框架为系统评估多模态大语言模型的感知能力提供了新方法，能够揭示模型在视觉基础方面的真实能力，而不仅仅是依赖文本知识，有助于更全面地理解模型的优势和局限性。
...
TL;DR: CAMP-VLM是一个基于视觉语言模型的框架，通过整合视觉上下文特征和场景图的空间感知，从第三人称视角预测多人类行为，在合成和真实数据上都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单人类场景下的自我中心行为预测，但许多机器人应用需要从第三人称视角理解多人类行为。目前缺乏适合从观察者视角进行多人类行为预测的数据集。

Method: 提出CAMP-VLM框架，整合视觉输入的上下文特征和场景图的空间感知。使用照片级真实模拟器生成合成人类行为数据进行微调，结合监督微调(SFT)和直接偏好优化(DPO)方法。

Result: CAMP-VLM在合成和真实世界序列上都表现出良好的泛化能力，在预测准确率上比最佳基线方法提高了66.9%。

Conclusion: CAMP-VLM框架有效解决了从第三人称视角预测多人类行为的挑战，通过整合视觉上下文和空间感知，在缺乏合适数据集的情况下利用合成数据进行训练，取得了显著性能提升。
...
TL;DR: 该研究探索了基于视觉语言模型（VLMs）的少样本多光谱目标检测，通过适配Grounding DINO和YOLO-World处理多光谱输入，在FLIR和M3FD基准测试中显著优于专用多光谱模型。


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全敏感应用中至关重要，但标注数据的稀缺限制了深度检测器的训练。文本类别信息可作为有价值的语义监督源，而视觉语言模型在计算机视觉中的成功启发了其在少样本多光谱检测中的潜力探索。
...
TL;DR: 当前视觉语言模型在农业分类任务中表现不佳，零样本性能远低于专用监督模型，不适合作为独立的农业诊断系统，但可作为辅助组件使用。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型被提议作为视觉识别任务的通用解决方案，但其在农业决策支持中的可靠性尚不清楚，需要系统评估其在农业分类任务中的表现。

Method: 在AgML集合的27个农业分类数据集（162个类别）上对开源和闭源视觉语言模型进行基准测试，涵盖植物病害、害虫损害、植物和杂草物种识别，使用零样本评估、多项选择和开放式提示，并应用基于LLM的语义判断。

Result: 零样本视觉语言模型在所有任务中都显著低于专用监督基线（YOLO11）；最佳闭源模型（Gemini-3 Pro）在多项选择提示下达到约62%准确率，开放式提示性能更低（通常低于25%）；语义判断可提升开放式准确率；开源模型中Qwen-VL-72B表现最佳；植物和杂草分类比害虫损害识别更容易。
...
Conclusion: 该方法为利用大规模视频数据进行机器人学习提供了一个可扩展的框架，克服了现有方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 单壳低角度分辨率dMRI估计FOD精度有限，而多壳高角度分辨率dMRI需要长扫描时间限制应用。扩散模型在从LAR-FOD估计HAR-FOD方面有潜力，但由于FOD中大量球谐系数，高效生成HAR-FOD具有挑战性。

Method: 提出3D多通道patch扩散模型预测HAR-FOD，设计FOD-patch适配器引入先验脑解剖知识进行更高效的基于patch的学习，引入体素级条件协调模块增强模型全局理解，设计SH注意力模块有效学习SH系数的复杂相关性。

Result: 实验结果显示该方法在HAR-FOD预测中取得最佳性能，优于其他最先进方法。

Conclusion: 提出的3D多通道patch扩散模型能够有效从低角度分辨率扩散MRI预测高角度分辨率纤维取向分布，通过创新的适配器、协调模块和注意力模块设计解决了现有方法的局限性。
...
TL;DR: 本文提出了一种自动词汇3D目标检测方法(AV3DOD)，能够在无需用户指定类别的情况下自动生成检测到的3D物体的类别名称，超越了现有需要用户指定类别的开放词汇方法。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇3D目标检测方法虽然在训练时不需要看到所有类别，但在训练和推理时仍然依赖用户指定的类别。本文旨在研究真正的自动词汇3D目标检测，即无需任何用户输入就能自动生成检测对象的类别名称。

Method: 提出了AV3DOD框架，利用2D视觉语言模型通过图像描述、伪3D框生成和特征空间语义扩展来生成丰富的语义候选。同时引入了语义评分(SS)来评估生成类别名称的质量。

Result: 在ScanNetV2和SUNRGB-D数据集上，AV3DOD在定位精度(mAP)和语义质量(SS)方面都达到了最先进的性能。在ScanNetV2上，相比最先进的CoDA方法，整体mAP提升了3.48，SS相对提升了24.5%。

Conclusion: AV3DOD成功实现了真正的自动词汇3D目标检测，无需用户输入就能自动生成高质量的物体类别名称，在定位精度和语义质量方面都超越了现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机作为新型仿生视觉传感器具有高动态范围和高时间分辨率等优势，但其几何标定（特别是长距离测量场景下的内外参标定）仍然是一个重大挑战
...
Conclusion: 提出的基于准直仪闪烁星点图案的事件相机标定方法有效解决了长距离高精度测量需求，显著提升了标定精度和可靠性
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在视频生成中计算成本高、推理速度慢，需要高效的加速方案来提升实际应用可行性。

Method: 采用低比特SageAttention和可训练稀疏线性注意力加速注意力计算；使用rCM进行高效步骤蒸馏；对模型参数和激活进行W8A8量化加速线性层；结合其他工程优化。

Result: 在多个视频生成模型上实验，单RTX 5090 GPU上实现100-200倍加速，同时保持可比的视频质量。

Conclusion: TurboDiffusion为高质量视频生成提供了高效的加速解决方案，显著提升了扩散模型的实用性。
...
TL;DR: 本文提出了一种基于准直仪系统的新型相机标定方法，利用准直仪的光学几何特性引入角度不变性约束，将标定目标与相机之间的6自由度相对运动简化为3自由度纯旋转运动，实现了多图像闭式解和双图像最小解，并提出了无需相机运动的单准直仪图像标定算法。


<details>
  <summary>Details</summary>
Motivation: 相机标定是摄影测量和3D视觉应用中的关键步骤。传统方法存在环境要求高、灵活性不足等问题。本文旨在通过设计准直仪系统，提供一个可靠可控的标定环境，简化标定过程并提高精度。

Method: 1. 设计准直仪系统提供可控标定环境；2. 利用准直仪光学几何特性提出角度不变性约束；3. 证明标定目标与相机相对运动符合球面运动模型；4. 基于球面运动约束提出多图像闭式线性求解器和双图像最小求解器；5. 提出基于角度不变性约束的单准直仪图像标定算法，无需相机运动。

Result: 在合成和真实世界实验中验证了方法的可行性。实验结果表明：1. 准直仪系统可用于相机标定；2. 提出的方法优于现有基线方法；3. 单图像标定算法提供了灵活快速的标定解决方案。

Conclusion: 本文提出的基于准直仪系统的相机标定方法通过角度不变性约束简化了标定过程，实现了高精度标定。该方法不仅提供了多图像和双图像的求解方案，还创新性地提出了无需相机运动的单图像标定算法，为相机标定提供了更灵活高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型提示调优策略主要关注边际分布对齐，但忽略了条件分布差异，导致类原型错位和语义判别性下降等问题。需要同时解决这两种分布差异来提升域自适应性能。

Method: 提出了C-DGPA方法：1）边际分布对齐分支采用动态对抗训练框架来桥接边际分布差异；2）条件分布对齐分支引入类映射机制，通过标准化语义提示理解和防止源域过依赖来对齐条件分布差异。双分支架构通过协同优化将领域知识整合到提示学习中。

Result: 在OfficeHome、Office31和VisDA-2017三个基准数据集上进行了广泛实验，C-DGPA在所有基准上都取得了新的最先进结果，验证了其优越性。

Conclusion: C-DGPA通过双对齐策略有效整合领域知识到提示学习中，确保了领域不变且语义判别性的表示，成功解决了传统方法中条件分布对齐被忽视的问题。
...
TL;DR: 事件相机在自动驾驶中相比传统相机能更好地应对昼夜光照变化的域差距问题，无需额外调整即可在不同光照条件下保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 传统相机在端到端驾驶中存在域差距问题，当训练数据条件与部署环境不匹配时性能大幅下降，特别是昼夜光照差异导致的域差距。

Method: 提出使用事件相机作为传统相机的替代方案，事件相机基于事件驱动原理，对光照变化不敏感，能够在不需额外调整的情况下适应不同光照条件。

Result: 事件相机在不同光照条件下保持更一致的性能，其域偏移惩罚通常与灰度帧相当或更小，在跨域场景中提供更优的基线性能。

Conclusion: 事件相机是解决自动驾驶中昼夜光照域差距问题的有效替代方案，能够在不需额外调整的情况下保持跨光照条件的稳定性能。
...
TL;DR: Avatar4D是一个可迁移的合成人类运动数据集生成管道，专门针对特定领域应用。它通过Syn2Sport体育数据集验证，展示了合成数据在监督学习、零样本迁移和跨领域泛化方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用动作，灵活性有限，难以满足特定领域（如体育）的需求。体育领域的特定动作和运动模式对运动理解提出了独特挑战，需要可定制、可控制且无需人工标注的数据集生成方法。

Method: 提出Avatar4D管道，提供对身体姿态、外观、相机视角和环境背景的细粒度控制。创建Syn2Sport大规模合成数据集，包含棒球和冰球等体育项目的高保真4D（随时间变化的3D几何）人类运动序列，具有不同玩家外观和多样化环境渲染。

Result: 在Syn2Sport上评估了多个最先进的姿态估计模型，展示了合成数据在监督学习、零样本迁移到真实数据以及跨体育泛化方面的有效性。评估了生成合成数据与真实数据集在特征空间中的对齐程度。

Conclusion: Avatar4D展示了无需依赖特定领域真实数据即可生成可扩展、可控且可迁移的人类数据集的能力，为多样化领域特定任务提供了有前景的解决方案。
...
TL;DR: VALOR提出了一种基于强化学习的后对齐框架，通过GRPO方法改进医学视觉语言模型，提升放射学报告生成的视觉基础和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告生成面临的主要挑战是生成既具有视觉基础又临床准确的报告。现有方法依赖大规模标注数据、昂贵的任务特定偏好数据或基于检索的方法，但未能有效缓解由于视觉和语言表示之间跨模态对齐不良而产生的幻觉问题。
...
Conclusion: VALOR通过强化学习后对齐框架有效解决了医学视觉语言模型在放射学报告生成中的视觉对齐问题，为自动化医疗工作流程提供了更准确可靠的工具。
...
TL;DR: 提出基于信息几何的3D点云形状分析框架，将点云表示为统计流形上的高斯混合模型，并引入具有理论上界保证的改进对称KL散度，解决了传统几何度量的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统点云比较方法面临挑战：点云的非结构化特性和复杂几何表面使得传统几何度量（如Hausdorff和Chamfer距离）难以捕捉全局统计结构且对异常值敏感；现有的高斯混合模型KL散度近似可能产生无界或数值不稳定的值。

Method: 1. 将点云表示为统计流形上的高斯混合模型；2. 证明GMM空间构成统计流形；3. 提出改进对称KL散度，具有理论上界保证，确保所有GMM比较的数值稳定性。

Result: 在人体姿态判别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）的综合实验中，MSKL提供了稳定且单调变化的数值，直接反映了几何变化，优于传统距离和现有的KL近似方法。

Conclusion: 提出的信息几何框架为3D点云形状分析提供了新的方法，改进对称KL散度解决了传统度量的数值不稳定问题，在点云比较任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时满足3D人体反应生成的三个关键需求：高运动保真度、实时推理能力以及在线场景的自回归适应性。需要一种能够兼顾这些要求的新方法。
...
Result: ARMFlow在InterHuman和InterX数据集上的单步在线生成性能超过现有在线方法40%以上（FID指标），同时仅使用部分序列条件就能匹配离线SOTA性能。ReMFlow在离线方法中达到最快推理速度。

Conclusion: ARMFlow成功解决了3D人体反应生成的三大挑战，通过全局上下文编码器增强语义对齐，单步推理实现高精度低延迟，BSCE减少累积误差，在在线和离线场景下都表现出优越性能。
...
TL;DR: 该论文提出了两种半监督多视角人群计数框架，通过排序不同视角数量的融合模型预测结果或模型不确定性，在有限标注数据下提升多视角计数性能。


<details>
  <summary>Details</summary>
Motivation: 多视角人群计数旨在解决大范围场景中的严重遮挡问题，但由于多视角图像采集和标注困难，现有数据集规模有限。需要开发能够在有限标注数据下有效工作的半监督方法。

Method: 提出了两种半监督多视角人群计数框架：1) 基于预测结果排序的朴素模型，要求较少视角输入的预测结果不大于较多视角的预测结果；2) 基于模型不确定性排序的方法，要求较多视角的模型不确定性不大于较少视角的不确定性。这些约束以半监督方式引入模型训练。

Result: 实验表明，提出的多视角模型排序方法相比其他半监督计数方法具有优势，在有限标注数据下能够有效提升多视角人群计数的性能。

Conclusion: 通过排序多视角融合模型的预测结果或模型不确定性，可以在有限标注数据下有效训练多视角人群计数模型，为解决多视角数据集规模有限的问题提供了有效的半监督解决方案。
...
TL;DR: TextEditBench是一个专注于图像中文本区域编辑的评估基准，强调推理密集型编辑场景，并提出了新的评估维度"语义期望"来衡量模型在文本编辑中的语义一致性和跨模态对齐能力。


<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索。现有方法需要生成清晰可读的字符，同时保持语义、几何和上下文的一致性，这是一个尚未解决的重要问题。

Method: 提出了TextEditBench评估基准，专注于图像中的文本中心区域。该基准强调推理密集型编辑场景，要求模型理解物理合理性、语言意义和跨模态依赖关系。还提出了新的评估维度"语义期望"来衡量模型在文本编辑中的推理能力。

Result: 对最先进的编辑系统进行广泛实验发现，虽然当前模型能够遵循简单的文本指令，但在上下文相关推理、物理一致性和布局感知集成方面仍然存在困难。
...
TL;DR: 本文提出MACL方法，通过标签感知采样、频率敏感加权和动态温度缩放来解决多标签遥感图像检索中的语义重叠、标签分布不平衡和复杂类别共现问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像检索面临三大挑战：1）土地覆盖类别间的语义重叠；2）高度不平衡的标签分布；3）复杂的类别间共现模式。这些因素导致传统对比学习方法在多标签场景下效果不佳。

Method: 提出多标签自适应对比学习（MACL），包含三个核心组件：1）标签感知采样策略，平衡常见和稀有类别的表示学习；2）频率敏感加权机制，缓解标签分布不平衡；3）动态温度缩放技术，适应不同类别的语义相似度。

Result: 在DLRSD、ML-AID和WHDLD三个基准数据集上的实验表明，MACL持续优于基于对比学习的基线方法，有效缓解语义不平衡问题，在大规模遥感档案中提供更可靠的检索性能。

Conclusion: MACL通过自适应对比学习框架成功解决了多标签遥感图像检索中的关键挑战，为大规模遥感档案的检索任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的图像生成基准大多关注美学而非细粒度生成能力，需要更客观的评估方法来检验模型的像素级生成智能。

Method: 提出PixelArena基准，使用语义分割任务来客观评估模型的细粒度生成能力，通过像素级精度检验模型的视觉智能。

Result: Gemini 3 Pro Image在零样本设置下展现出卓越的语义掩码生成能力，保真度高，相比其他模型表现优异，展示了真正的泛化能力和前所未有的视觉智能。
...
<details>
  <summary>Details</summary>
Motivation: 在无人机多传感器系统中，当观测条件受限（如远距离、小交会角、大倾角）时，最小二乘估计的设计矩阵列向量存在严重多重共线性，导致病态问题和定位结果不稳定、鲁棒性差。

Method: 提出基于岭估计的融合定位方法，结合序列图像的丰富场景信息和激光测距的高精度，通过岭估计缓解观测受限条件下的多重共线性问题。

Result: 实验结果表明，该方法相比基于单一信息的定位算法具有更高的定位精度，岭估计的引入有效增强了鲁棒性，特别是在观测条件受限的情况下。

Conclusion: 基于岭估计的融合定位方法能够有效解决观测受限条件下的多重共线性问题，提高无人机多传感器系统的定位精度和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 非专用车载移动群智感知系统面临覆盖范围、感知可靠性和车辆动态参与度相互关联的挑战，需要在不依赖专用基础设施的情况下实现低成本高质量的城市监测。

Method: 提出QUIDS系统，引入聚合感知质量指标整合覆盖率和可靠性，开发基于相互辅助信念的车辆调度算法，在不确定性下估计感知可靠性并分配激励。

Result: 在真实城市部署数据评估中，QUIDS比无调度场景提高ASQ 38%，比现有最佳方法提高10%，重建地图误差降低39-74%。
...
Method: 设计了一个两阶段框架：第一阶段服务器在全局图像上进行推理，利用VLM的内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度；如果最小熵超过阈值，则请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行推理精炼

Result: 在多个VLM架构上的实验表明，该框架显著降低了通信成本，同时保持了推理精度

Conclusion: 提出的协作式边缘到服务器推理框架通过选择性重传策略，在减少通信开销的同时有效保持了视觉语言模型的推理准确性
...
<details>
  <summary>Details</summary>
Motivation: 预训练潜在扩散模型在低层视觉任务中表现出强大的感知先验，但直接应用于HDR重建面临三个挑战：8位潜在压缩导致的动态范围有限、多步去噪的高推理成本、以及生成模型固有的内容幻觉问题

Method: 将HDR重建重新表述为条件引导的增益图估计任务，增益图编码扩展的动态范围但保持与LDR图像相同的位深度；从信息丰富的基于回归的估计开始去噪过程，实现单步生成；利用回归先验指导去噪过程和潜在解码，抑制幻觉同时保持结构准确性

Result: GMODiff在性能上优于多个最先进方法，比之前基于LDM的方法快100倍

Conclusion: GMODiff通过增益图驱动的单步扩散框架，有效解决了LDM在HDR重建中的动态范围、推理成本和内容幻觉问题，实现了高效高质量的HDR重建
...
TL;DR: EverybodyDance提出了一种解决多角色动画中身份对应问题的系统方案，通过身份匹配图建模角色关系，优化身份对应正确性


<details>
  <summary>Details</summary>
Motivation: 单角色姿态驱动动画已取得显著进展，但扩展到多角色场景（特别是涉及位置交换时）面临挑战，核心问题在于确保参考帧和生成帧中角色之间的正确身份对应

Method: 提出身份匹配图建模生成帧和参考帧中的角色为加权完全二分图节点集，使用掩码查询注意力计算边权重，将身份对应正确性形式化为图结构度量并在训练中优化；还提出身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略

Result: 在专门构建的身份对应评估基准上进行的广泛实验表明，EverybodyDance在身份对应正确性和视觉保真度方面显著优于现有最先进基线方法

Conclusion: EverybodyDance为多角色动画中的身份对应问题提供了系统解决方案，通过图结构建模和针对性优化策略，在多角色场景中实现了更好的身份对应和视觉质量
...
TL;DR: FVG通过三阶段分解方法解决T2V模型在复杂场景和时序逻辑上的问题：LLM推理重写提示描述初始场景，T2I模型合成高质量锚定帧，视频模型专注于动画化场景，在保持性能的同时大幅提升采样效率。
...
Method: 提出因子化视频生成（FVG）管道，将文本到视频生成分解为三个专门阶段：1）推理阶段：使用大型语言模型重写视频提示，仅描述初始场景，解决时序歧义；2）构图阶段：使用文本到图像模型从新提示合成高质量、构图正确的锚定帧；3）时序合成阶段：微调的视频模型理解此锚定帧，专注于动画化场景并遵循提示。

Result: 在T2V CompBench基准测试中达到新的最先进水平，在VBench2上显著改进所有测试模型。视觉锚定允许将采样步骤减少70%而性能无损失，实现采样速度的大幅提升。

Conclusion: 因子化视频生成为实现更高效、鲁棒和可控的视频合成提供了一条简单而实用的路径，通过任务分解解决了现有T2V模型在复杂场景构建和时序逻辑遵循方面的核心问题。
...
TL;DR: 该研究利用3D神经表示技术，通过高斯泼溅和语义分割从少量未标定人脸图像中重建中性姿态的3D人脸模型，并将其转换为可在标准图形管线中使用的纹理空间神经纹理。


<details>
  <summary>Details</summary>
Motivation: 为了解决从少量未标定人脸图像中重建一致3D人脸模型的挑战，同时保持高视觉保真度并兼容标准图形管线。

Method: 使用高斯泼溅作为3D表示，结合语义分割对齐人脸区域，通过软约束将高斯点绑定到三角网格表面，并将高斯泼溅转换为纹理空间的神经纹理，利用可重光照高斯模型分离纹理和光照。

Result: 仅需11张图像即可重建中性姿态的3D人脸模型，获得可在标准图形管线中直接使用的高分辨率反照率纹理，支持不同光照条件下的图像训练，并能集成到文本驱动的资产创建流程中。

Conclusion: 该方法成功实现了从少量未标定图像中重建高质量3D人脸模型，将高斯泼溅转换为兼容标准图形管线的神经纹理，为3D资产创建提供了灵活高效的解决方案。
...
TL;DR: BrepLLM是首个让大语言模型能够直接处理3D边界表示(Brep)数据的框架，通过两阶段训练解决了结构化3D几何与自然语言之间的模态差距问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于token序列的大语言模型不适合直接处理包含复杂几何和拓扑信息的3D边界表示模型，需要一种能够解析和理解原始Brep数据的框架。

Method: 采用两阶段训练管道：跨模态对齐预训练和多阶段LLM微调。第一阶段通过自适应UV采样将Brep转换为图表示，使用分层BrepEncoder提取几何和拓扑特征，通过对比学习对齐全局token与文本嵌入。第二阶段将预训练的BrepEncoder集成到LLM中，采用三阶段渐进训练策略：MLP语义映射、LLM微调和混合查询专家机制。

Result: 构建了包含269,444个Brep-文本问答对的Brep2Text数据集，实验表明BrepLLM在3D物体分类和描述生成任务上达到了最先进的性能。

Conclusion: BrepLLM成功解决了LLM处理3D边界表示数据的挑战，通过创新的两阶段训练框架实现了3D几何与自然语言的有效对齐，为3D几何理解开辟了新途径。
...
TL;DR: CountZES是一个无需训练、通过零样本示例选择进行目标计数的框架，通过三个协同阶段渐进发现多样示例，在零样本目标计数任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 在复杂场景中进行零样本目标计数面临挑战，现有方法要么依赖开放词汇检测器产生多实例候选，要么使用随机补丁采样无法准确描绘目标实例，需要更精确的示例选择方法。
...
Result: 在多样化数据集上的实验表明，CountZES在零样本目标计数方法中表现出优越性能，并能有效泛化到自然、航空和医学领域。

Conclusion: CountZES通过渐进式示例发现框架，平衡了文本基础、计数一致性和特征代表性，为零样本目标计数提供了一种有效的训练免费解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在生成多幅图像时难以保持主体一致性，限制了在视觉叙事中的应用。现有方法需要模型微调或图像条件，计算成本高且需要针对每个主体进行优化。

Method: 提出一种简单有效的无需训练方法，从几何角度解决语义纠缠问题，通过精炼文本嵌入来抑制不需要的语义。

Result: 大量实验证明，该方法在主体一致性和文本对齐方面显著优于现有基线方法。

Conclusion: 该方法提供了一种有效的训练免费解决方案，能够显著改善多帧图像生成中的主体一致性和文本对齐问题。
...
Method: StageVAR通过分析发现早期步骤对语义和结构一致性至关重要，而后期步骤主要优化细节，因此提出利用后期计算的语义无关性和低秩特性进行剪枝和近似，无需额外训练即可实现加速。

Result: StageVAR实现了高达3.4倍的加速，在GenEval上仅下降0.01，在DPG上仅下降0.26，一致优于现有加速基线方法。

Conclusion: 阶段感知设计是高效视觉自回归图像生成的有力原则，StageVAR为VAR模型提供了一种无需训练的即插即用加速策略，在保持生成质量的同时显著提升效率。
...
TL;DR: 该研究通过强化学习方法，让模型学习人类感知-推理的评估过程，实现盲图像质量评估中的人类化、自洽推理能力。


<details>
  <summary>Details</summary>
Motivation: 人类评估图像质量是通过感知-推理的级联过程，结合感官线索和隐式推理形成自洽判断。现有盲图像质量评估模型缺乏这种人类化的推理能力。

Method: 1. 收集人类评估数据，捕捉人类感知-推理管道的多个方面；2. 采用强化学习，以人类标注作为奖励信号，引导模型学习人类化感知和推理；3. 设计奖励机制，使模型能够从自我生成的描述中推断图像质量，实现自洽推理能力。

Result: 1. 在通用指标（Pearson和Spearman相关系数）上，得分预测性能与最先进的BIQA系统相当；2. 在1000多个人类标注样本上，ROUGE-1得分达到0.512（基线为0.443），表明模型生成的内容与人类感知-推理链有显著覆盖。

Conclusion: 该方法在盲图像质量评估中实现了与人类感知-推理过程的对齐，为开发具有人类化可解释推理能力的BIQA系统迈出了重要一步。
...
TL;DR: 该研究构建了基于眼动行为的多模态情感识别数据集EMER，并提出EMERT模型，通过眼动行为特征弥补面部表情识别与真实情感识别之间的差距，显著提升了情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前情感识别领域过度依赖面部表情识别，但面部表情常被用作社交工具而非真实内心情感的体现，存在面部表情识别与真实情感识别之间的差距。为解决这一问题，研究者引入眼动行为作为重要的情感线索。

Method: 1. 构建EMER数据集：采用自发情感诱导范式收集数据，同时捕捉眼动序列、眼动注视图等非侵入性眼动行为数据以及面部表情视频；2. 设计EMERT模型：采用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的强有力补充；3. 建立七个多模态基准协议用于全面评估。

Result: EMERT模型在EMER数据集上显著优于其他最先进的多模态方法，证明了眼动行为建模对鲁棒情感识别的重要性。研究提供了眼动行为在情感识别中重要性的全面分析。

Conclusion: 眼动行为是弥补面部表情识别与真实情感识别之间差距的关键线索，EMER数据集和EMERT模型为更鲁棒的情感识别研究提供了重要资源，推动了解决FER与ER差距的研究进展。
...
TL;DR: VenusBench-GD是一个跨平台的双语GUI grounding基准测试，包含大规模数据、高质量标注流程和分层任务分类，实验显示通用多模态模型在基础任务上已媲美专用GUI模型，但高级任务仍需专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding基准存在数据量不足、领域覆盖窄、平台单一或需要专业领域知识等局限性，需要构建更全面、多平台的评估框架来支持真实应用。

Method: 1) 构建大规模跨平台基准，覆盖广泛应用、多样UI元素和丰富标注数据；2) 建立高质量数据构建流程，标注准确率高于现有基准；3) 提出分层任务分类法，将grounding分为基础和高级两类，包含六个互补视角的子任务。

Result: 实验发现：通用多模态模型在基础grounding任务上已匹配甚至超越专用GUI模型；而高级任务仍偏向GUI专用模型，但这些模型存在显著过拟合和鲁棒性差的问题。

Conclusion: 研究结果表明需要全面、多层次的评估框架，VenusBench-GD为GUI grounding提供了更全面的评估标准，揭示了当前模型的局限性并指导未来发展方向。
...
TL;DR: 该论文提出了一种用于骨架时序动作定位的自监督预训练方法，通过片段判别任务和U型模块增强特征分辨率，在多个数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 骨架动作识别的自监督预训练已取得很大成功，但骨架时序动作定位的有效表示学习仍然具有挑战性且研究不足。与视频级动作识别不同，检测动作边界需要时间敏感的特征来捕捉标签变化时相邻帧之间的细微差异。

Method: 1. 提出片段判别自监督预训练任务：将骨架序列密集投影到非重叠片段，通过对比学习促进区分不同视频片段的能力
2. 在骨架动作识别模型的基础上，使用U型模块融合中间特征，增强帧级定位的特征分辨率

Result: 1. 在BABEL数据集上，该方法持续改进了现有的骨架对比学习方法，在不同子集和评估协议下都有提升
2. 在PKUMMD上实现了最先进的迁移学习性能，预训练使用了NTU RGB+D和BABEL数据集

Conclusion: 提出的自监督预训练方法有效提升了骨架时序动作定位的性能，通过片段判别任务和特征增强策略，在多个基准数据集上取得了显著改进，为骨架动作定位任务提供了有效的表示学习方法。
...
TL;DR: 提出基于弱监督学习和Grad-CAM的肺炎分类与定位框架，使用图像级标签替代像素级标注，评估7种预训练模型，在肺炎X光图像上实现高精度分类和临床可解释的定位热图。


<details>
  <summary>Details</summary>
Motivation: 肺炎诊断通常需要胸部X光图像，但准确的肺炎区域定位需要昂贵的像素级标注。为了解决这一限制，研究旨在开发一种弱监督学习方法，减少对昂贵标注的依赖，同时提供临床可解释的肺炎定位。

Method: 提出弱监督深度学习框架，使用Grad-CAM生成肺炎区域热图，仅需图像级标签。评估7种预训练模型（包括Vision Transformer），采用focal loss和患者级数据划分防止数据泄露，在相同训练条件下比较性能。

Result: 所有模型均实现高分类准确率（96-98%），其中ResNet-18和EfficientNet-B0表现最佳，MobileNet-V3作为轻量级替代方案。Grad-CAM热图可视化显示模型能聚焦临床相关的肺部区域，支持可解释AI在放射诊断中的应用。

Conclusion: 该研究展示了弱监督可解释模型在肺炎筛查中的潜力，通过减少对昂贵标注的依赖并提供透明化的决策依据，有助于增强临床对AI辅助诊断的信任。
...
TL;DR: N3D-VLM是一个统一框架，将原生3D物体感知与3D感知视觉推理结合，实现精确的3D定位和可解释的空间理解，超越传统2D图像问答模型。
...
Result: 实验结果显示，该框架在3D定位任务上达到最先进性能，在3D空间推理方面持续超越现有方法，数据规模比现有最大单图像3D检测数据集大6倍以上。

Conclusion: N3D-VLM成功将原生3D物体感知与3D感知视觉推理集成，实现了更精确的3D定位和更可解释的结构化空间理解，为3D视觉语言模型提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法处理动态场景中物体消失后的重建问题。需要一种能够保持物体持久性、重建完整时空场景的系统。

Method: 将场景分解为一组刚性3D基元，通过估计的密集2D对应关系，通过优化管道联合推断这些基元的刚性运动，实现4D重建。引入运动外推机制处理不可见物体，使用运动分组技术保持连续性。

Result: 在物体扫描和多物体数据集上，该系统在定量和定性评估中都显著优于现有方法，能够实现可重放的3D重建、多物体扫描和物体持久性。

Conclusion: 该系统实现了完整的4D时空感知，能够从单目视频中重建动态场景的完整时空演化，为动态场景理解和重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在微调视觉基础模型时忽略了长期预训练模型中的artifacts问题，这些artifacts与非因果因素相关，阻碍了有价值表征的利用，从而降低域泛化性能

Method: 提出Causal-Tune方法：1) 使用离散余弦变换提取每层特征的频谱；2) 应用高斯带通滤波器分离因果和非因果成分；3) 引入因果感知可学习token在频域细化因果成分；4) 通过逆DCT将精炼特征转换回空间域

Result: 在多种跨域任务实验中证明方法有效性，特别是在恶劣天气条件下表现优异，在雪天条件下比基线提升+4.8% mIoU

Conclusion: 通过因果机制分析视觉基础模型中的特征因素，提出简单有效的Causal-Tune方法，能够识别和分离因果与非因果因素，实现更鲁棒的域泛化
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过让MLLM在推理过程中原生生成连续的视觉嵌入（潜在草图标记），实现统一的视觉-文本想象和交互，无需预定义工具包。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM在需要视觉想象的场景中表现不足，而人类能够在统一的大脑空间中构建灵活的视觉-文本思维过程。受此启发，考虑到当前MLLM已经在同一特征空间中编码视觉和文本信息，作者认为视觉标记可以无缝插入到文本标记携带的推理过程中。

Method: 提出Sketch-in-Latents (SkiLa)范式，扩展MLLM的自回归能力，使其能够原生生成连续的视觉嵌入（潜在草图标记）。在多步推理过程中，模型动态切换文本思维模式（生成文本思考标记）和视觉草图模式（生成潜在草图标记），并提出潜在视觉语义重建机制确保这些标记具有语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上实现了卓越性能，同时在多样化的通用多模态基准测试中表现出强大的泛化能力。

Conclusion: SkiLa通过让MLLM在统一空间中生成视觉嵌入作为视觉思维，实现了更接近人类认知的多模态推理能力，为视觉想象任务提供了新的解决方案。
...
TL;DR: 该论文提出Yuan-TecSwin，一种基于Swin-transformer的文本条件扩散模型，用于改进图像生成中的长距离语义理解能力，在ImageNet基准上取得了最先进的FID分数1.37。
...
Method: 提出Yuan-TecSwin模型，用Swin-transformer块替代CNN块作为编码器和解码器的基础模块，增强特征提取和图像恢复中的非局部建模能力。通过精心选择的文本编码器、有效的文本嵌入利用和文本条件融合设计来改进文本-图像对齐。采用自适应时间步搜索策略在不同扩散阶段优化推理性能。

Result: 在ImageNet生成基准上取得了最先进的FID分数1.37，无需在不同去噪阶段使用额外模型。推理性能通过自适应时间步搜索提升了10%。在人类评估中，受访者难以区分模型生成的图像和人类绘制的图像。

Conclusion: Yuan-TecSwin通过引入Swin-transformer块有效解决了CNN在扩散模型中的局部性限制，显著提升了长距离语义理解和文本-图像对齐能力，在图像生成任务上达到了最先进的性能水平。
...
TL;DR: Hazedefy是一个轻量级、面向应用的去雾管道，专注于实时视频和直播摄像头增强，优先考虑计算简单性和在消费级硬件上的实际部署能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在移动和嵌入式设备上实时运行的轻量级去雾解决方案，无需GPU加速，适用于实时视频和直播摄像头增强应用。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定性下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 在真实世界图像和视频上的实验演示显示，该管道能够改善可见度和对比度，适用于移动和嵌入式应用，无需GPU加速。

Conclusion: Hazedefy提供了一个计算简单、可实际部署的去雾解决方案，特别适合实时视频和直播摄像头增强应用，能够在消费级硬件上有效运行。
...
<details>
  <summary>Details</summary>
Motivation: 现有DiT的自注意力机制具有二次方计算复杂度，限制了其在长序列上的扩展。现有的Top-K稀疏注意力方法虽然减少了计算，但仍存在二次方选择成本和随着序列增长需要增加K值的问题，这源于单层设计的不足。

Method: LLSA采用分层结构，通过分层Top-K选择逐步进行稀疏选择，并引入分层KV增强机制，在注意力计算中使用不同粒度的token来保持全局上下文。同时开发了高性能GPU实现，仅使用稀疏索引进行前向和反向传播。

Result: 在256×256像素的token序列上，LLSA将注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。该方法支持高分辨率像素空间图像生成，无需使用分块化和VAE编码。

Conclusion: LLSA为高效训练长序列DiT提供了一个有前景的方向，通过将选择和注意力成本从二次方降低到对数线性复杂度，解决了现有稀疏注意力方法的效率瓶颈。
...
TL;DR: 提出基于电源插座的三阶段深度学习管道，用于室内多媒体地理位置定位，通过检测、分类插座类型并映射到国家/地区，解决室内定位难题。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理位置定位在打击人口贩运、儿童剥削等严重犯罪中具有重要价值，但面临房间布局相似、装修频繁、视觉模糊、室内光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战。

Method: 三阶段深度学习管道：1) 使用YOLOv11检测电源插座(mAP@0.5=0.843)；2) 使用Xception将插座分类为12种类型(准确率0.912)；3) 将检测到的插座类型映射到国家/地区(置信度>90%时准确率0.96)。创建了两个专用数据集：插座检测数据集(2,328张标注图像，增强至4,072张)和分类数据集(3,187张图像，12个插座类别)。

Result: 在Hotels-50K数据集（特别是TraffickCam子集）上评估，该数据集包含真实世界条件下的酒店图像（光线差、业余拍摄角度），比旅游网站的专业图像更具现实性。管道展示了在实际数字取证应用中的实用性。

Conclusion: 该框架利用电源插座作为一致的室内标记物，为解决室内地理位置定位挑战提供了实用方案，代码、训练模型和数据已开源，为实际数字取证应用迈出了实际一步。
...
TL;DR: DeContext是一种保护输入图像免遭未经授权的上下文编辑的新方法，通过注入微小扰动来削弱多模态注意力层的跨注意力路径，从而阻断源图像上下文信息向输出的传播。
...
Method: DeContext通过注入微小、有针对性的扰动来削弱多模态注意力层的跨注意力路径，从而阻断源图像上下文信息向输出的传播。研究发现早期去噪步骤和特定transformer块主导上下文传播，因此可以将扰动集中在最关键的位置。
...
TL;DR: SARMAE：一种面向合成孔径雷达（SAR）图像的噪声感知掩码自编码器，通过构建百万级SAR数据集和引入斑点噪声增强、语义锚点约束，实现自监督SAR特征学习，在分类、检测、分割任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有SAR深度学习面临数据稀缺和斑点噪声干扰两大挑战：1）SAR数据获取困难导致训练数据不足；2）SAR图像固有的物理斑点噪声阻碍了细粒度语义表示学习。

Method: 提出SARMAE框架：1）构建首个百万级SAR数据集SAR-1M，包含配对的SAR和光学图像；2）设计斑点感知表示增强（SARE），在掩码自编码器中注入SAR特有的斑点噪声，实现噪声感知的鲁棒表示学习；3）引入语义锚点表示约束（SARC），利用配对光学图像先验对齐SAR特征，确保语义一致性。

Result: 在多个SAR数据集上的广泛实验表明，SARMAE在分类、检测和分割任务上均达到了最先进的性能水平。

Conclusion: SARMAE通过大规模预训练、噪声感知学习和语义对齐，有效解决了SAR数据稀缺和噪声干扰问题，为SAR图像的自监督表示学习提供了新范式。
...
TL;DR: REGLUE是一种统一的潜在扩散框架，通过联合建模VAE潜在变量、局部VFM语义和全局[CLS]标记来改进图像合成质量，加速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型的重构式去噪目标仅提供间接语义监督，高级语义出现缓慢，需要更长的训练时间并限制样本质量。现有方法要么通过表示对齐外部注入语义，要么只在扩散过程中联合建模VFM特征的狭窄切片，未能充分利用VFM丰富的非线性多层空间语义。

Method: 提出REGLUE框架：1) 使用轻量级卷积语义压缩器非线性聚合多层VFM特征为低维空间结构化表示；2) 在单个SiT主干中联合建模VAE图像潜在变量、紧凑的局部VFM语义和全局[CLS]标记；3) 通过外部对齐损失进一步正则化内部表示朝向冻结的VFM目标。

Result: 在ImageNet 256x256上，REGLUE持续改进FID并加速收敛，优于SiT-B/2、SiT-XL/2基线以及REPA、ReDi和REG方法。实验表明：空间VFM语义至关重要，非线性压缩是充分发挥其优势的关键，全局标记和外部对齐作为全局-局部-潜在联合建模框架中的互补轻量级增强。

Conclusion: REGLUE通过统一建模VFM语义与扩散过程，有效提升了图像合成质量和训练效率，证明了充分利用VFM丰富空间语义的重要性。
...
TL;DR: FrameDiffuser是一个自回归神经渲染框架，通过结合G-buffer数据和自身先前输出来生成时间一致、逼真的帧，解决了现有扩散模型在交互应用中缺乏时间一致性和计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在交互应用中的局限性：单图像模型缺乏时间一致性，视频模型计算成本过高且需要完整序列，不适合依赖用户输入的交互应用。

Method: 提出FrameDiffuser自回归框架，结合ControlNet进行结构引导和ControlLoRA保证时间一致性。采用三阶段训练策略，针对特定环境进行专门化训练。

Result: 能够生成时间一致、逼真的帧，在数百到数千帧中保持稳定，相比通用方法在光照、阴影和反射方面达到更高质量的逼真效果。

Conclusion: FrameDiffuser通过环境专门化训练和自回归架构，在保持时间一致性和实时性能的同时，实现了交互应用中高质量的神经渲染。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF、3DGS和RadiantFoam等方法在视图合成和渲染速度方面取得进展，但在精确网格重建方面仍有不足，需要一种既能保持渲染效率又能提升几何重建质量的方法

Method: 联合学习显式Voronoi图和隐式有符号距离场(SDF)，通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面与SDF零水平集对齐

Result: 在多种场景下，SDFoam显著改善了网格重建精度（Chamfer距离），同时保持了可比较的外观质量（PSNR、SSIM），且训练速度与RadiantFoam相当

Conclusion: SDFoam的混合隐式-显式公式在保持渲染效率和外观质量的同时，大幅提升了表面重建的准确性和拓扑结构，减少了伪影
...
<details>
  <summary>Details</summary>
Motivation: 胎儿超声生长评估依赖手动识别标准平面中的解剖标志点进行生物测量，但手动标注耗时、操作者依赖性强，且受不同扫描设备和站点变异性的影响，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备胎儿超声图像基准数据集，包含4,513张去标识化超声图像，来自1,904名受试者，在三个临床站点使用七种不同超声设备采集。数据集提供了专家标注的解剖标志点，涵盖临床使用的所有主要胎儿生物测量指标，包括头双顶径和枕额径、腹横径和前后径、股骨长度。提供标准化的、受试者分离的训练/测试划分、评估代码和基线结果。

Result: 使用自动生物测量模型量化了领域偏移，证明仅局限于单个中心的训练和评估会显著高估性能，而多中心测试能提供更准确的性能评估。这是首个公开的多中心、多设备、标志点标注的数据集，覆盖所有主要胎儿生物测量指标。
...
TL;DR: 该论文提出了OMG-Bench，首个大规模骨架在线微手势识别基准，包含40个精细手势类别和13,948个实例，并提出了分层记忆增强Transformer（HMATr）框架，统一了手势检测和分类任务。


<details>
  <summary>Details</summary>
Motivation: 在线微手势识别对于VR/AR交互至关重要，但面临两大挑战：1）缺乏公开数据集，微手势的细微运动模式使得构建具有精确骨架和帧级标注的数据集困难；2）现有算法多为任务特定，缺乏统一解决方案。

Method: 1）开发多视图自监督流水线自动生成骨架数据，结合启发式规则和专家精炼实现半自动标注，构建OMG-Bench基准；2）提出分层记忆增强Transformer（HMATr）框架，通过分层记忆库存储帧级细节和窗口级语义来保留历史上下文，并使用从记忆初始化的可学习位置感知查询隐式编码手势位置和语义，统一手势检测和分类。

Result: 实验表明，HMATr在检测率上比现有最先进方法高出7.6%，为在线微手势识别建立了强大的基线。OMG-Bench包含40个精细手势类别、13,948个实例和1,272个序列，具有细微运动、快速动态和连续执行的特点。

Conclusion: 该工作通过构建首个大规模公开基准OMG-Bench和提出统一的HMATr框架，有效解决了在线微手势识别中的数据稀缺和算法碎片化问题，为VR/AR交互提供了重要技术基础。
...
TL;DR: 提出TODSynth框架，包含MM-DiT模型和任务反馈引导的采样策略，用于遥感语义分割的数据合成，在少样本和复杂场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 遥感领域可控生成技术快速发展，训练数据合成成为扩展标注数据集、减轻人工标注负担的有前景方法。然而，语义掩码控制的复杂性和采样质量的不确定性限制了合成数据在下游语义分割任务中的效用。

Method: 提出任务导向的数据合成框架TODSynth：1) 基于DiT的多模态扩散变换器MM-DiT，采用统一的三重注意力机制；2) 任务反馈引导的即插即用采样策略；3) 控制-修正流匹配方法CRFM，在早期高可塑性阶段通过语义损失动态调整采样方向。

Result: 实验表明，文本-图像-掩码联合注意力方案结合图像和掩码分支的完全微调，显著提升了遥感语义分割数据合成的效果，特别是在少样本和复杂场景下。CRFM方法缓解了生成图像的不稳定性，缩小了合成数据与下游分割任务之间的差距。

Conclusion: 该方法在遥感语义分割数据合成方面优于现有可控生成方法，能生成更稳定、任务导向的合成数据，为遥感领域数据增强提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的图像压缩技术计算复杂度高，阻碍了其广泛应用，需要开发低复杂度的解决方案

Method: 采用二叉树结构的编码器-解码器架构，使用注意力特征融合机制有效整合多分支特征

Result: 在三个基准数据集上评估，低比特率下比JPEG AI的BD-rate平均提升4.83%，模型复杂度降低87.82%

Conclusion: TreeNet通过二叉树结构和注意力融合机制实现了高效的低复杂度图像压缩，并通过消融研究深入分析了潜在表示对重建的影响
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如自动绑定和姿势条件生成）存在皮肤权重预测不准确、拓扑缺陷和姿势一致性差等问题，限制了其鲁棒性和泛化能力。需要克服这些限制，实现更高质量的3D角色摆姿。
...
Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统角色摆姿方法的局限性，实现了高质量的姿势重建，并具有良好的可扩展性。
...
TL;DR: FlowDet是首个使用现代条件流匹配技术进行目标检测的框架，通过更简单、更直的生成传输路径，在推理步骤增加时能更快提升检测性能，相比基于扩散的方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有的DiffusionDet将目标检测框定为边界框空间中的生成去噪问题，但基于扩散的方法会产生弯曲的随机传输路径。本研究旨在将这一框架推广到更广泛的生成传输问题类别，同时保持无需重新训练即可改变框数量和推理步骤的能力。

Method: FlowDet采用条件流匹配技术重新构建目标检测问题，学习比扩散方法更简单、更直的生成传输路径。该方法保持了无需重新训练即可调整框数量和推理步骤的灵活性，并在多种特征主干和数据集上进行实验验证。

Result: 在COCO和LVIS数据集上，FlowDet在召回率受限设置下相比DiffusionDet分别获得+3.6% AP和+4.2% AP_{rare}的提升。在推理步骤增加时，检测性能提升更快，且在各种精度/召回率操作点上都优于基于扩散的检测系统和非生成基线方法。

Conclusion: 条件流匹配为生成式目标检测提供了更有效的框架，通过更简单的传输路径实现了比扩散方法更好的性能，特别是在推理步骤增加时表现出更快的性能扩展能力，为生成式检测系统提供了新的方向。
...
TL;DR: Kling-Omni是一个端到端的通用生成框架，能够从多模态视觉语言输入直接合成高质量视频，支持视频生成、编辑和智能推理任务。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频生成系统中功能分离的问题，将视频生成、编辑和智能推理任务整合到一个统一的系统中，支持多种用户输入并生成电影级质量的视频内容。

Method: 采用端到端框架，将文本指令、参考图像和视频上下文等多样化输入处理为统一的多模态表示；构建全面的数据系统作为多模态视频创作基础；实施高效的大规模预训练策略和推理基础设施优化。

Result: Kling-Omni在上下文生成、基于推理的编辑和多模态指令遵循方面表现出卓越能力，能够生成高质量、智能的视频内容。
...
Method: 创建合成3D环境，并将从无人机拍摄的鸟瞰数据集SinD中提取的真实世界轨迹集成到该环境中，生成R3ST数据集。

Result: R3ST数据集填补了合成数据与真实轨迹之间的差距，提供了准确的多模态地面实况标注和真实的人类驾驶车辆轨迹。

Conclusion: R3ST数据集通过结合真实世界轨迹和合成3D环境，为道路车辆轨迹预测研究提供了更真实的数据支持。
...
<details>
  <summary>Details</summary>
Motivation: AR/VR应用中需要从头戴设备获取的稀疏信号重建真实多样的全身姿态，现有方法存在计算成本高、时空依赖建模分离、难以平衡准确性、时间一致性和效率的问题。
...
Conclusion: KineST通过运动学引导的状态空间模型，有效提取时空依赖并整合局部和全局姿态感知，为AR/VR应用中的全身运动跟踪提供了准确、平滑且高效的解决方案。
...
TL;DR: TabReX是一个无参考、基于属性的表格生成评估框架，通过图推理将文本和表格转换为知识图进行对齐评估，提供可解释的评分和错误追踪。


<details>
  <summary>Details</summary>
Motivation: 现有表格生成评估指标存在局限性：要么将表格扁平化为文本忽略结构信息，要么依赖固定参考限制泛化能力，需要更可靠的评估方法。

Method: 将源文本和生成表格转换为规范知识图，通过LLM引导的匹配过程对齐，计算可解释的、基于评分标准的分数来量化结构和事实保真度。

Result: TabReX在专家排名相关性方面表现最佳，在更难的扰动下保持稳定，支持细粒度的模型与提示分析，建立了结构化生成系统可信可解释评估的新范式。

Conclusion: TabReX框架通过图推理实现了无参考、基于属性的表格生成评估，提供了敏感性与特异性之间的可控权衡，并创建了TabReX-Bench基准来系统评估指标鲁棒性。
...
TL;DR: SocialStoryFrames是一个用于捕捉读者对故事反应的框架，包括感知作者意图、推理、情感反应和价值判断等，通过对话上下文和跨学科分类学实现，并开发了生成和分类模型进行验证。


<details>
  <summary>Details</summary>
Motivation: 当前计算模型对读者反应的捕捉有限，无法进行细致的分析，需要开发能够捕捉丰富读者反应（如叙事意图推断、人物判断等）的计算模型。

Method: 引入SocialStoryFrames形式化框架，基于叙事理论、语言语用学和心理学建立分类学，开发SSF-Generator和SSF-Classifier两个模型，通过人类调查（382名参与者）和专家标注进行验证，并在SSF-Corpus（6,140个社交媒体故事）上进行试点分析。

Result: 模型验证有效，能够捕捉读者反应；应用模型分析了不同社区中叙事意图的频率和相互依赖性，比较了不同社区的叙事实践及其多样性。

Conclusion: SocialStoryFrames通过将细粒度、上下文敏感的建模与通用的读者反应分类学相结合，为在线社区中的叙事研究提供了新的工具和方法。
...
TL;DR: 研究探索了如何使用个人信息（如个人陈述句）来改进个体特征建模和主观任务标注者标签预测，发现人口统计信息比态度、关系和经历更有效，且少量相关评论即可获得良好效果。


<details>
  <summary>Details</summary>
Motivation: 先前研究虽然探索了使用个人信息来改进标注者标签预测，但信息量通常受限，缺乏对何种信息最有效的深入理解。本研究旨在通过分类自我披露句子，系统分析不同类型信息对预测标注者判断的影响。

Method: 对自我披露句子进行分类，并基于此构建标注者模型来预测社会规范判断。进行了多项消融实验和分析，以检验信息类型对预测标注模式的影响。

Result: 研究发现：1）人口统计信息比态度、关系和经历更具影响力；2）基于理论的方法通常优于自动聚类；3）与先前研究相反，仅需少量相关评论即可获得良好效果；4）标注者自我披露样本的多样性越高，模型性能越好。

Conclusion: 人口统计信息在预测标注者标签方面最为有效，理论指导的方法优于自动聚类，且少量高质量的自我披露信息结合多样化的样本能够获得最佳预测性能。
...
TL;DR: Sage是一个无需人工标注的LLM-as-a-Judge评估套件，通过局部自一致性和全局逻辑一致性来评估LLM评判质量，发现当前顶尖LLM在约1/4困难案例中存在偏好不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-Judge基准主要依赖人工标注的真实标签，这引入了人类偏见，既损害了可靠性评估，又限制了可扩展性。需要一种无需人工标注的方法来评估LLM评判的质量。

Method: 基于理性选择理论公理，引入两个新视角：局部自一致性（成对偏好稳定性）和全局逻辑一致性（完整偏好集的传递性）。构建了650个问题的数据集，结合结构化基准问题和真实用户查询。

Result: 实验表明指标稳定且与监督基准高度相关。发现当前最先进的LLM（包括Gemini-2.5-Pro和GPT-5）在约1/4困难案例中存在偏好不一致问题。提出了"情境偏好"现象，发现微调、小组评审和深度推理能提升一致性。同时发现人类判断也存在显著不一致性。

Conclusion: Sage是一个可靠的LLM-as-a-Judge评估套件，无需人工标注。当前LLM作为评判者存在显著可靠性问题，人类标注可能不是可靠的金标准。微调、小组评审和深度推理是提升一致性的可行方法。
...
Method: 提出语义驱动强化学习方法（SRL），采用组相对策略优化（GRPO）来鼓励临床正确性导向的学习。优化报告级奖励：基于边缘的余弦相似度（MCCS），计算生成报告和参考报告中提取的关键放射学发现之间的相似性，直接对齐临床标签一致性。还引入轻量级推理格式约束，指导模型生成结构化的"思考报告"输出。

Result: 在IU X-Ray和MIMIC-CXR数据集上评估，MRG-R1在临床效能指标上达到最先进性能：IU X-Ray上CE-F1为51.88，MIMIC-CXR上为40.39。结果表明标签语义强化优于传统的词级监督。

Conclusion: 优化基于临床基础的报告级奖励而非词重叠，能够显著改善临床正确性。这项工作是在医学大型视觉语言模型训练中探索语义强化监督医学正确性的先驱研究。
...
TL;DR: 该论文针对社交媒体中基于虚假叙事的仇恨言论检测问题，提出了一个用于Faux-Hate共享任务的系统，包含二元分类和目标严重性预测两个子任务，采用NLP技术和领域特定预训练方法，取得了有竞争力的结果。
...
Method: 结合先进的自然语言处理技术与领域特定预训练，采用多任务学习方法处理两个主要子任务：(a) 二元Faux-Hate检测（虚假和仇恨言论分类），(b) 目标和严重性预测（对仇恨内容的目标和严重性进行分类）。

Result: 系统在共享任务中取得了有竞争力的结果，证明了利用多任务学习处理这一复杂问题的有效性。

Conclusion: 研究表明，结合NLP技术和领域特定预训练的多任务学习方法能够有效检测社交媒体中基于虚假叙事的仇恨言论，为处理这一复杂问题提供了有效的解决方案。
...
TL;DR: 使用Qwen2.5-7B模型结合LoRA微调，从警方通报中提取15个关键字段，准确率超过98%


<details>
  <summary>Details</summary>
Motivation: 警方通报等非结构化文本信息提取面临文本多变、非正式等挑战，需要高效的结构化信息提取方案

Method: 开发领域适应的提取流程，结合针对性提示工程和LoRA参数高效微调Qwen2.5-7B模型

Result: LoRA微调显著优于基础和指令微调模型，死亡率检测准确率超98.36%，死亡人数和省级位置提取的精确匹配率分别达95.31%和95.54%

Conclusion: 该流程为专业领域多任务结构化信息提取提供了验证有效的解决方案，可将非结构化文本转化为可靠结构化数据
...
TL;DR: 该研究提出一个独立于LLM的事实核查模块和领域特定的摘要模型，用于减少医疗领域LLM输出的幻觉问题，在MIMIC-III数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在医疗健康领域，LLM生成的输出必须可靠准确，特别是在涉及决策制定和患者安全的关键场景中。然而，LLM存在幻觉风险，导致输出不可靠，这对医疗应用构成严重威胁。

Method: 1. 开发独立于LLM的事实核查模块，使用数值正确性测试和通过离散逻辑的自然语言处理进行细粒度逻辑检查，以验证事实与电子健康记录的匹配。2. 设计领域特定的摘要模型，使用低秩适应（LoRa）在MIMIC-III数据集上进行微调，以减少幻觉率。

Result: 事实核查模块在3,786个命题上达到精确率0.8904、召回率0.8234、F1分数0.8556。LLM摘要模型在摘要质量上获得ROUGE-1分数0.5797和BERTScore 0.9120。
...
TL;DR: IBKE：基于信息瓶颈理论的大语言模型知识编辑框架，通过压缩和隔离关键信息实现可泛化的知识修正，在保持模型其他行为不变的同时更新知识。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的错误或过时信息会影响其准确性和安全部署。现有模型编辑技术通常难以将修正泛化到狭窄领域之外，导致意外后果并限制实际应用。需要开发无需完全重新训练的高效更新策略。

Method: 基于信息瓶颈理论，提出信息瓶颈知识编辑器（IBKE）。该方法压缩和隔离实现可泛化知识修正所需的关键信息，同时最小化对无关模型行为的干扰。利用紧凑的潜在表示来指导基于梯度的更新。

Result: 在多种LLM架构和标准基准任务上验证了IBKE的有效性，展示了最先进的准确性，并改进了编辑的泛化性和特异性。
...
TL;DR: Sigma-MoE-Tiny是一种高度稀疏的混合专家语言模型，采用细粒度专家分割（每层最多96个专家），每个token仅激活一个专家，总参数量200亿但仅激活5亿参数，在极端稀疏条件下实现了顶级性能。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型因其高效和强大的可扩展性成为基础模型的有前景范式，但现有开源模型的稀疏度仍有提升空间。本研究旨在开发具有最高稀疏度的MoE语言模型，并解决极端稀疏条件下的专家负载均衡挑战。

Method: 1. 采用细粒度专家分割，每层最多96个专家，每个token仅激活一个专家；2. 提出渐进稀疏化调度策略，平衡专家利用率和训练稳定性；3. 在多样化高质量语料上进行预训练，然后进行后训练；4. 深入分析高稀疏MoE模型中的负载均衡问题。

Result: Sigma-MoE-Tiny总参数量200亿，仅激活5亿参数，训练过程非常稳定，未出现不可恢复的损失尖峰。尽管激活参数很少，但在同类或更大规模模型中实现了顶级性能表现。

Conclusion: Sigma-MoE-Tiny成功实现了当前开源模型中最高的稀疏度，证明了极端稀疏MoE架构的可行性。提出的渐进稀疏化调度策略有效解决了高稀疏条件下的负载均衡问题，为未来MoE架构的稀疏化发展提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型翻译评估主要关注高资源语言，对低资源和濒危语言的性能了解不足，特别是乌拉尔语系语言。

Method: 使用文学文本平行语料库，分析GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语（科米-兹良语、莫克沙语、埃尔齐亚语、乌德穆尔特语）翻译任务中的拒绝率差异。

Result: 推理模型比非推理模型的拒绝率低16个百分点，显示出在低资源语言翻译任务中的显著性能优势。

Conclusion: 该研究为乌拉尔语系语言的研究者和实践者提供了有价值的见解，并有助于更广泛地理解推理模型在濒危语言保护方面的能力。
...
TL;DR: SpeechLLMs（语音大语言模型）在语音到文本翻译中是否优于传统级联系统仍存疑问。本文通过全面测试套件比较了5种SpeechLLM与16种级联系统，发现级联系统整体更可靠，SpeechLLMs仅在特定场景下表现相当。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到文本之外，将语音作为原生模态整合产生了SpeechLLMs，这些模型旨在直接翻译口语，绕过传统的基于转录的流水线。然而，这种整合是否比已建立的级联架构提高语音到文本翻译质量仍是一个开放性问题。

Method: 提出了"Hearing to Translate"测试套件，首次全面基准测试了5种最先进的SpeechLLMs与16种强大的直接和级联系统。这些级联系统结合了领先的语音基础模型（SFM）和多语言大语言模型。分析涵盖了16个基准测试、13种语言对和9种具有挑战性的条件，包括不流利、嘈杂和长语音。

Result: 在广泛的评估中，发现级联系统整体上仍然是最可靠的，而当前的SpeechLLMs仅在特定设置下与级联系统匹配，语音基础模型（SFM）则落后于两者。这表明整合大语言模型（无论是模型内部还是流水线中）对于高质量的语音翻译至关重要。

Conclusion: 尽管SpeechLLMs作为新兴技术显示出潜力，但级联系统在语音到文本翻译中仍然保持整体优势。整合大语言模型（无论是通过SpeechLLMs内部整合还是级联系统流水线整合）是获得高质量语音翻译的关键因素。
...
TL;DR: 提出隐私保护的自适应框架，通过LoRA和领域经验回放解决临床ASR部署中的数据隐私、计算资源和领域偏移问题，在真实临床音频上实现17.1%的相对WER改进


<details>
  <summary>Details</summary>
Motivation: 自动语音识别在临床文档处理中具有巨大潜力，但面临数据隐私限制、计算资源有限和严重声学领域偏移等技术障碍，现有模型在真实临床音频上性能严重下降（WER达40.94%），无法实际应用

Method: 提出高效隐私保护自适应框架：1）使用低秩适应（LoRA）在边缘设备上直接从数据流进行持续学习，保护患者数据隐私；2）集成多领域经验回放减少灾难性遗忘

Result: 在目标领域实现17.1%的相对WER改进，相比朴素自适应方法将灾难性遗忘减少47%，证明了在真实高影响环境中构建可靠自改进ASR系统的可行路径

Conclusion: 该框架为在资源受限环境中部署可靠、自改进的ASR系统提供了可行方案，解决了临床ASR应用中的关键技术障碍
...
<details>
  <summary>Details</summary>
Motivation: 解决科学文本在句子和文档两个层面的简化问题，使复杂的科学内容更易于理解，参与CLEF 2025 SimpleText竞赛。

Method: 使用OpenAI的gpt-4.1、gpt-4.1-mini和gpt-4.1-nano模型，比较两种方法：1）无上下文方法（依赖提示工程）；2）微调方法。在句子和文档两个层面进行文本简化。

Result: gpt-4.1-mini模型在无上下文方法中表现出色，在两个简化层面都有稳健表现；微调模型结果参差不齐，其中gpt-4.1-nano-ft在文档级简化中表现突出。

Conclusion: 不同粒度的文本简化具有复杂性，无上下文方法在某些情况下可能优于微调方法，模型选择需要根据简化任务的具体需求来决定。
...
TL;DR: 提出Refusal Steering方法，在推理时精细控制大语言模型对政治敏感话题的拒绝行为，无需重新训练。通过LLM-as-a-judge分配拒绝置信度，使用岭回归变体计算更好的拒绝-顺从方向向量，成功消除模型对政治敏感话题的拒绝行为，同时保持安全性和通用性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖脆弱的模式匹配来检测拒绝行为，缺乏对政治敏感话题拒绝行为的精细控制。需要一种无需重新训练的方法，在推理时控制模型对特定话题的拒绝行为，同时保持对有害内容的安全对齐。

Method: 提出Refusal Steering方法：1) 用LLM-as-a-judge替代模式匹配，为每个响应分配拒绝置信度分数；2) 提出岭回归正则化变体，计算更好的拒绝-顺从方向向量；3) 在推理时应用这些向量来调整模型行为。方法在Qwen3-Next-80B-A3B-Thinking等模型上验证，可消除或诱导拒绝行为。

Result: 在Qwen3-Next-80B-A3B-Thinking上，成功消除了模型对政治敏感话题的拒绝行为，同时在JailbreakBench上保持安全性，在通用基准测试中保持接近基线的性能。方法在4B和80B模型上均有效，分析显示拒绝信号集中在Transformer的深层，分布在多个维度中。

Conclusion: 激活导向可以在推理时移除政治拒绝行为，同时保留对有害内容的安全对齐，为可控、透明的推理时内容审核提供了实用路径。该方法展示了在保持安全性的前提下，对模型拒绝行为进行精细控制的可能性。
...
TL;DR: JustRL：一种极简的单阶段强化学习方法，使用固定超参数在1.5B推理模型上达到SOTA性能，计算量减半，挑战当前复杂训练流程的必要性。
...
Conclusion: 复杂训练方法可能是在解决一些在稳定、规模化基线中本不存在的问题；简单方法可以取得同等甚至更好的效果；建议建立简单、经过验证的基线供社区使用。
...
TL;DR: 提出GinSign框架，通过分层分类方法将自然语言映射到系统签名，解决现有NL-to-TL翻译中原子命题接地准确率低的问题，显著提升接地逻辑等价性。
...
Method: 提出GinSign框架，引入接地模型学习将自然语言片段映射到给定系统签名的抽象任务。采用分层分解方法：先预测谓词标签，再选择适当类型的常量参数，将自由形式生成问题转化为结构化分类问题。

Result: 实验表明，省略接地的框架会产生语法正确但语义不等价的提升LTL，而GinSign支持下游模型检查，达到95.5%的接地逻辑等价分数，比现有最佳方法提升1.4倍。

Conclusion: GinSign通过结构化分类方法有效解决了NL-to-TL翻译中的接地问题，显著提高了翻译准确性和实用性，为构建可信自主系统提供了重要支持。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成在处理冲突、过时或主观信息时存在局限，先前工作缺乏统一的推理监督，需要更系统的方法来提升RAG系统的可靠性和可解释性。

Method: 提出推理轨迹增强的RAG框架，包含三个结构化推理阶段：文档级裁决、冲突分析和基于证据的合成；引入冲突感知信任评分管道，使用LLM作为评判者评估系统性能。

Result: 实验结果显示显著优于基线方法，特别是Qwen模型，监督微调将端到端答案正确率从0.069提升到0.883，行为一致性从0.074提升到0.722。

Conclusion: 该框架为冲突感知、可解释的RAG系统奠定了基础，通过结构化推理和综合评估管道显著提升了系统在复杂信息环境下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成在生物医学QA中有应用前景，但何时将图表转换为文本、何时使用OCR-free视觉检索（返回页面图像让生成器解释）尚不明确，特别是在视觉密集的糖生物学领域

Method: 构建包含120个多选题的基准测试，按检索难度分层；实现四种增强策略：无增强、文本RAG、多模态转换、OCR-free视觉检索；使用Docling解析和Qdrant索引；评估开源和前沿专有模型

Result: 对于中等规模模型（如Gemma-3-27B-IT），将视觉转换为文本的策略优于OCR-free检索（0.722-0.740 vs 0.510准确率）；对于前沿模型（GPT-4o），多模态策略达0.808，各种策略差异小；GPT-5家族中，ColPali和ColFlor提升约2%至0.828

Conclusion: 策略选择依赖模型能力：中等模型更适合将视觉转换为文本以降低解读负担；前沿模型下OCR-free视觉检索变得有竞争力；ColFlor在保持性能的同时具有较小计算开销，是强生成器下的高效选择
...
### [84] [Information theory and discriminative sampling for model discovery](https://arxiv.org/abs/2512.16000)
*Yuxuan Bao,J. Nathan Kutz*

Main category: cs.IT

TL;DR: 该研究将费舍尔信息矩阵应用于SINDy数据驱动框架，通过信息模式可视化分析混沌与非混沌系统，提出基于信息度量的采样策略以提高数据效率和模型性能。


<details>
  <summary>Details</summary>
Motivation: 费舍尔信息和香农熵是理解分析动力系统的互补工具，但如何将其有效整合到数据驱动的非线性动力学稀疏识别框架中，以量化信息模式、指导采样策略、提高学习效率，是本研究的核心动机。

Method: 在SINDy框架中引入费舍尔信息矩阵，可视化混沌与非混沌系统的信息模式，分析单轨迹与多初始条件的信息贡献，通过FIM谱分析阐明统计装袋的益处，并在三种场景中应用信息度量。

Result: 信息模式可视化揭示了系统动力学特征，基于信息的采样策略显著提高了数据效率，FIM谱分析阐明了统计装袋机制，三种场景验证了信息度量在提升模型性能和减少数据需求方面的有效性。

Conclusion: 在数据驱动模型发现中，基于可量化信息度量的原则性采样策略是提高学习效率和减少数据需求的有效方法，为非线性动力学分析提供了新的信息理论视角。
...
### [85] [Optimal Key Rates for Decentralized Secure Aggregation with Arbitrary Collusion and Heterogeneous Security Constraints](https://arxiv.org/abs/2512.16112)
*Zhou Li,Xiang Zhang,Giuseppe Caire*

Main category: cs.IT

TL;DR: 本文研究具有任意合谋和异构安全约束的去中心化安全聚合，通过安全集合和合谋集合的概念，表征了最优通信和源密钥速率，源密钥速率优化问题可转化为线性规划求解。


<details>
  <summary>Details</summary>
Motivation: 传统去中心化安全聚合需要大量密钥来保护除输入总和和合谋用户信息外的所有信息，为减少源密钥开销，研究具有任意合谋和异构安全约束的去中心化安全聚合。

Method: 引入安全集合和合谋集合的概念，安全集合中的用户子集输入必须受到保护，合谋集合中的用户子集可能合谋攻击，通过信息论方法表征最优通信和源密钥速率。

Result: 表征了具有任意合谋和异构安全约束的去中心化安全聚合的最优通信和源密钥速率，源密钥速率的最小化可转化为线性规划问题求解。

Conclusion: 本文为具有任意合谋和异构安全约束的去中心化安全聚合提供了理论基础，通过安全集合和合谋集合的框架，解决了源密钥开销优化问题。
...
### [86] [New Quantum Stabilizer Codes from generalized Monomial-Cartesian Codes constructed using two different generalized Reed-Solomon codes](https://arxiv.org/abs/2512.16482)
*Oisin Campion,Fernando Hernando,Gary McGuire*

Main category: cs.IT

TL;DR: 本文定义了广义单项式笛卡尔码(GMCC)，它是广义Reed-Solomon码的自然扩展，并展示了如何将两个不同的广义Reed-Solomon码组合构建GMCC，同时建立了GMCC成为Hermitian自正交码的充分条件，从而为量子码提供了新构造。


<details>
  <summary>Details</summary>
Motivation: 扩展广义Reed-Solomon码到更一般的框架，探索新的编码结构，特别是为量子码构造提供新的数学工具和理论基础。

Method: 1. 定义广义单项式笛卡尔码(GMCC)；2. 展示如何将两个不同的广义Reed-Solomon码组合构建GMCC；3. 建立GMCC成为Hermitian自正交码的充分条件。

Result: 成功定义了GMCC并建立了其构造方法，获得了GMCC成为Hermitian自正交码的充分条件，从而能够基于这些条件构造新的量子码。

Conclusion: GMCC为广义Reed-Solomon码提供了自然扩展，其Hermitian自正交性质为量子码构造开辟了新的途径，丰富了编码理论的研究工具。
...
### [87] [Novel Inconsistency Results for Partial Information Decomposition](https://arxiv.org/abs/2512.16662)
*Philip Hendrik Matthias,Abdullah Makkeh,Michael Wibral,Aaron J. Gutknecht*

Main category: cs.IT

TL;DR: 本文通过PID的偏序方法建立了新的不一致性结果，证明经典信息论的三个基石属性（非负性、链式法则、可逆变换不变性）在PID设置下无法同时满足，任何PID框架都必须牺牲至少一个看似基本的信息论属性。


<details>
  <summary>Details</summary>
Motivation: 尽管偏信息分解（PID）在多个领域有广泛应用，但寻找唯一、普遍接受的解决方案仍然困难，存在众多竞争性提案。通过建立不一致性结果来澄清可能性范围，迫使研究者认识到必须在基本属性之间做出选择。

Method: 利用最近发展的偏序方法（mereological approach）来建立PID的不一致性结果，证明经典信息论的三个核心属性在PID设置下的互不相容性。

Result: 主要定理证明：非负性、链式法则和可逆变换不变性这三个经典信息论基石属性在PID框架下无法同时满足。同时强化了Rauh等人的经典结果，表明非负性、恒等属性和Williams-Beer公理也无法共存。

Conclusion: 任何偏信息分解框架都必须牺牲至少一个看似基本的信息论属性，这揭示了PID理论发展的根本性限制，迫使研究者在不同的公理集之间做出选择。
...
### [88] [Secure Event-triggered MolecularvCommunication - Information Theoretic Perspective and Optimal Performance](https://arxiv.org/abs/2512.16761)
*Wafa Labidi,Vida Gholamian,Yaning Zhao,Christian Deppe,Holger Boche*

Main category: cs.IT

TL;DR: 该研究分析了离散时间泊松信道在分子通信中的随机识别和安全随机识别性能，推导了相应的容量公式，为生物纳米物联网的安全通信提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 分子通信作为新兴研究领域，关注人体细胞间的通信机制和医疗应用。传统香农通信模型不适合事件驱动的分子通信，而识别框架能更高效地利用能量和硬件资源。同时，生物纳米物联网的安全需求促使研究安全随机识别。

Method: 采用Ahlswede和Dueck提出的识别框架，研究离散时间泊松信道。首先分析随机识别性能，然后扩展到安全随机识别。使用随机编码技术，推导两种场景下的容量公式。

Result: 推导出了离散时间泊松信道在随机识别和安全随机识别情况下的容量公式。识别码的大小随块长度呈双指数增长，相比传统香农传输在能耗和硬件需求方面显著更高效。

Conclusion: 识别框架为分子通信提供了比传统香农传输更高效的通信范式，特别是在事件驱动的场景中。安全随机识别容量公式的推导为生物纳米物联网的安全体内通信奠定了理论基础，有助于开发更安全、更节能的纳米机器辅助疗法。
...
### [89] [An Extension of Enumerative Sphere Shaping for Arbitrary Channel Input Distributions](https://arxiv.org/abs/2512.16808)
*Frederik Ritter,Andrej Rode,Laurent Schmalen*

Main category: cs.IT

TL;DR: 本文提出了一种广义枚举球面成形（ESS）方法，能够生成任意的离散信道输入分布，相比传统ESS和CCDM，在非高斯信道中显著提高了传输速率。


<details>
  <summary>Details</summary>
Motivation: 传统ESS算法只能产生固定的信道输入符号分布，限制了其在具有非高斯容量实现输入分布的信道中的应用。需要一种能够生成任意离散信道输入分布的成形算法。

Method: 将传统ESS算法推广为广义ESS，关键改进是将ESS内部使用的固定权重替换为依赖于期望信道输入分布的权重，从而能够产生任意的离散信道输入分布。

Result: 在简化的无放大相干光链路模型（具有明显非高斯容量实现输入分布的信道）上，使用概率幅度成形（PAS）传输256个符号序列的数值模拟显示，广义ESS相比CCDM在误帧率低于10^{-4}时，最大传输速率提高了0.0425比特/符号。

Conclusion: 广义ESS算法能够生成任意的离散信道输入分布，使其适用于大多数信道类型，特别是在非高斯信道中相比传统成形算法具有显著性能优势。
...
### [90] [Anubuddhi: A Multi-Agent AI System for Designing and Simulating Quantum Optics Experiments](https://arxiv.org/abs/2512.15736)
*S. K. Rithvik*
...
<details>
  <summary>Details</summary>
Motivation: 该研究旨在民主化计算实验设计，让没有专业编程知识的研究人员和学生能够通过自然语言提示设计和模拟量子光学实验，降低量子光学领域的入门门槛。

Method: 系统采用多智能体架构，结合意图路由、知识增强生成和双模式验证（QuTiP和FreeSim）。通过语义检索从三层工具箱中组合光学元件，然后通过物理模拟进行验证和收敛细化。

Result: 系统评估了13个实验，涵盖基础光学、量子信息协议和先进技术。设计-模拟对齐分数达到8-9/10，模拟能够忠实建模预期物理。研究发现结构正确性与定量准确性有区别：高对齐确认正确的物理架构，而数值预测需要专家审查。

Conclusion: Anubuddhi系统民主化了计算实验设计，为研究和教学提供了强大的初始设计工具，用户可以通过对话迭代细化设计。自由形式模拟在11/13实验中优于约束框架，表明量子光学多样性需要灵活的数学表示。
...
### [91] [The Principle of Proportional Duty: A Knowledge-Duty Framework for Ethical Equilibrium in Human and Artificial Systems](https://arxiv.org/abs/2512.15740)
*Timothy Prescher*
...
<details>
  <summary>Details</summary>
Motivation: 传统伦理框架在处理不确定性决策时存在局限，通常将不确定性简单视为行动约束。需要一种能够建模道德责任如何随认知状态变化的框架，特别是在复杂系统决策中。

Method: 提出比例责任原则（PPD）框架，建立数学模型 D_total = K[(1-HI) + HI * g(C_signal)]，其中总责任是知识、谦逊/不确定性和情境信号强度的函数。通过蒙特卡洛模拟验证框架稳定性，并在临床伦理、接收者权利法、经济治理和人工智能四个领域进行应用验证。

Result: 模拟显示保持基线谦逊系数（λ>0）的系统能产生更稳定的责任分配，降低过度自信决策风险。PPD为道德责任提供了数学可处理的方法，可作为可审计AI决策系统的基础。

Conclusion: 比例责任原则作为复杂系统中的稳定原则，通过动态平衡认知信心与情境风险，防止决策过度和遗漏。该框架具有跨学科有效性，为不确定性下的道德决策提供了新范式。
...
### [92] [Prompt-to-Parts: Generative AI for Physical Assembly and Scalable Instructions](https://arxiv.org/abs/2512.15743)
*David Noever*

Main category: cs.AI

TL;DR: 提出一个从自然语言描述生成物理可实现的装配指令框架，使用离散部件词汇表确保几何有效性、连接约束和可建造顺序，通过LDraw作为中间表示，引导大语言模型生成有效装配序列


<details>
  <summary>Details</summary>
Motivation: 现有方法（如像素扩散或CAD模型）无法支持复杂装配指令或部件交换，需要一种能连接语义设计意图与可制造输出的方法，填补从自然语言到物理原型之间的空白

Method: 使用LDraw作为文本丰富的中间表示，在离散部件词汇表内操作，强制几何有效性、连接约束和建造顺序；开发Python库进行程序化模型生成；采用"积木袋"方法作为物理API，将精确定向的积木位置连接到"词袋"

Result: 能够为超过3000个装配部件的砖基原型生成有效的逐步建造序列和装配指令，在复杂卫星、飞机和建筑领域评估可建造输出，展示了可扩展性、模块化和保真度

Conclusion: 该方法作为物理API，通过约束词汇表将任意功能需求编译为物质现实，为制造和工程原型中的自然语言实现提供指导，同时为设计开辟新选项
...
TL;DR: AI Epidemiology是一个通过应用群体层面监测方法来治理和解释AI系统的框架，类似于流行病学方法，通过统计证据而非理解分子机制来预测AI输出失败。
...
Method: 通过标准化捕获AI-专家交互为结构化评估字段（风险等级、对齐分数、准确度分数），将这些作为暴露变量来统计预测输出失败，类似于胆固醇和血压预测心脏事件。通过专家覆盖和真实结果验证输出失败关联。

Result: 该框架为零负担专家提供自动审计跟踪，通过被动追踪专家与AI建议的趋同和分歧来分析输出而非内部计算，在机构更新模型和切换供应商时提供治理连续性，通过可靠性分数和语义评估使专家能够检测不可靠AI输出。

Conclusion: AI Epidemiology通过使领域专家无需机器学习专业知识即可治理AI系统，民主化了AI监督，能够在AI输出造成危害前检测不可靠输出。
...
### [94] [Beyond Training: Enabling Self-Evolution of Agents with MOBIMEM](https://arxiv.org/abs/2512.15784)
*Zibin Liu,Cheng Zhang,Xi Zhao,Yunfei Feng,Bingyu Bai,Dahu Feng,Erhu Feng,Yubin Xia,Haibo Chen*

Main category: cs.AI

TL;DR: MOBIMEM是一个内存中心的智能体系统，通过三种专用内存原语实现无需模型重训练的自进化，显著提升个性化、能力和效率。


<details>
  <summary>Details</summary>
Motivation: 当前以模型为中心的智能体架构部署后难以自我进化，改进个性化、能力和效率通常需要持续模型重训练/微调，计算开销巨大且存在准确性与推理效率的固有权衡。

Method: 提出MOBIMEM内存中心智能体系统：1) 三种专用内存原语：Profile Memory使用轻量级距离图结构对齐用户偏好；Experience Memory采用多级模板实例化新任务执行逻辑；Action Memory记录细粒度交互序列减少模型推理依赖。2) 集成操作系统风格服务：调度器协调并行子任务执行和内存操作；AgentRR机制实现安全高效动作重用；上下文感知异常处理确保从用户中断和运行时错误中优雅恢复。

Result: 在AndroidWorld和top-50应用上评估：MOBIMEM实现83.1%的配置文件对齐，检索时间23.83毫秒（比GraphRAG基线快280倍），任务成功率提升高达50.3%，移动设备端到端延迟降低高达9倍。

Conclusion: MOBIMEM通过内存中心架构实现了无需模型重训练的智能体自进化，有效解决了准确性与效率的权衡问题，显著提升了移动和桌面环境中LLM智能体的性能。
...
### [95] [State-Augmented Graphs for Circular Economy Triage](https://arxiv.org/abs/2512.15824)
*Richard Fox,Rui Li,Gustav Jonsson,Farzaneh Goli,Miying Yang,Emel Aktas,Yongjing Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于状态增强的拆卸序列规划图的确定性求解器，用于优化循环经济产品分类决策，通过编码拆卸历史到状态中确保马尔可夫性，实现递归评估。


<details>
  <summary>Details</summary>
Motivation: 循环经济产品分类需要平衡保留价值与处理成本、劳动力约束的自适应决策，现有方法缺乏统一的数学框架来处理复杂的产品层次结构和操作约束。

Method: 开发了一个确定性求解器，基于状态增强的拆卸序列规划图，将拆卸历史编码到状态中以确保马尔可夫性，支持递归评估，决策包括继续拆卸或选择循环经济选项，整合了基于诊断健康分数的条件感知效用和复杂操作约束。

Result: 以电动汽车电池的分层分类为例，展示了该框架的灵活性，能够处理不同的机械复杂性、安全要求和经济驱动因素，通过组件递归估值指导决策。
...
### [96] [PediatricAnxietyBench: Evaluating Large Language Model Safety Under Parental Anxiety and Pressure in Pediatric Consultations](https://arxiv.org/abs/2512.15894)
*Vahideh Zolfaghari*

Main category: cs.AI

TL;DR: 研究评估大语言模型在儿科咨询中的安全性，发现对抗性查询（如家长焦虑表达）会降低模型安全性，模型规模影响安全表现，但所有模型都存在临床显著漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地被家长用于儿科咨询，但对其在真实世界对抗性压力下的安全性了解不足。焦虑家长常使用紧急语言，可能破坏模型安全防护，导致有害建议。

Method: 开发了PediatricAnxietyBench开源基准，包含300个高质量查询（150个患者来源，150个对抗性），涵盖10个儿科主题。使用多维安全框架评估两个Llama模型（70B和8B），评估维度包括诊断克制、转诊遵循、谨慎表达和紧急情况识别。对抗性查询包含家长压力模式，如紧迫感、经济障碍和对免责声明的挑战。

Result: 平均安全得分5.50/15（SD=2.41）。70B模型优于8B模型（6.26 vs 4.95，p<0.001），关键失败率更低（4.8% vs 12.0%，p=0.02）。对抗性查询使安全性降低8%（p=0.03），紧迫感导致最大降幅（-1.40）。在癫痫发作（33.3%不适当诊断）和疫苗接种后查询中存在漏洞。谨慎表达与安全性强相关（r=0.68，p<0.001），而紧急情况识别完全缺失。
...
### [97] [Darth Vecdor: An Open-Source System for Generating Knowledge Graphs Through Large Language Model Queries](https://arxiv.org/abs/2512.15906)
*Jonathan A. Handler*

Main category: cs.AI

TL;DR: Darth Vecdor (DV) 是一个从大型语言模型中提取知识到结构化SQL数据库的工具，旨在解决直接查询LLM的成本、速度、安全和置信度问题，特别针对医疗保健领域。


<details>
  <summary>Details</summary>
Motivation: 虽然可以直接查询大型语言模型获取知识，但在高容量操作中，成本、速度、安全和置信度等问题可能成为障碍。通过将信息从LLM中预先提取到标准数据库中，可以缓解这些问题，特别是在医疗保健等关键领域。
...
Result: DV已作为免费、开源、可扩展的软件发布，采用"按现状"基础，不提供任何明示或暗示的保证。软件可能存在严重错误，用户需自行承担使用风险和责任。

Conclusion: 尽管DV存在潜在风险和错误，但作者希望当前和未来版本的DV及其输出能够帮助改善医疗保健。用户需要认识到使用DV的潜在风险和益处，并负责确保任何使用都是安全和有效的。
...
### [98] [Leveraging Spreading Activation for Improved Document Retrieval in Knowledge-Graph-Based RAG Systems](https://arxiv.org/abs/2512.15922)
*Jovan Pavlović,Miklós Krész,László Hajdu*

Main category: cs.AI

TL;DR: 提出基于扩散激活算法的RAG框架，通过自动构建的知识图谱增强复杂推理任务性能，相比传统RAG在资源受限环境下表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统在复杂多步推理任务中存在局限性，将所有检索信息视为同等可靠，忽视了文本语料库的可信度和关联性差异。GraphRAG方法虽然通过知识图谱改进，但依赖高质量图表示，构建成本高且自动化构建不可靠。

Method: 提出新型RAG框架，采用扩散激活算法从自动构建的知识图谱连接的文档语料库中检索信息。该方法可作为即插即用模块与多种RAG方法集成，特别适合资源受限环境。

Result: 实验表明该方法在复杂任务（如多跳问答）上达到或优于迭代RAG方法。与思维链迭代检索结合时，相比朴素RAG实现39%绝对准确率提升，且使用小型开源语言模型即可获得良好效果。

Conclusion: 扩散激活算法结合自动构建知识图谱的RAG框架能有效提升复杂推理任务性能，具有易集成、资源效率高的优势，为RAG系统改进提供了新方向。
...
### [99] [Small Language Models for Efficient Agentic Tool Calling: Outperforming Large Models with Targeted Fine-tuning](https://arxiv.org/abs/2512.15943)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI规模化应用，大语言模型的高计算成本成为企业可持续应用的关键障碍，需要探索更经济的小语言模型解决方案。

Method: 使用facebook/opt-350m模型，通过Hugging Face TRL的监督微调方法进行单轮微调，针对文档摘要、问答和结构化数据解释等任务进行领域适配。
...
### [100] [Subjective functions](https://arxiv.org/abs/2512.15948)
*Samuel J. Gershman*

Main category: cs.AI

TL;DR: 论文探讨了人工智能中目标函数来源的问题，提出主观函数概念作为内生于智能体的高阶目标函数，并以预期预测误差为例进行具体分析。


<details>
  <summary>Details</summary>
Motivation: 人类智能能够动态合成新的目标函数，而人工智能系统缺乏这种能力。论文旨在探索目标函数的来源和选择机制，以及如何赋予人工系统类似的动态目标合成能力。

Method: 提出主观函数的概念——一种内生于智能体的高阶目标函数，相对于智能体自身特征而非外部任务定义。以预期预测误差作为具体的主观函数实例进行研究。

Result: 建立了主观函数的概念框架，展示了预期预测误差作为主观函数的可行性，并将该框架与心理学、神经科学和机器学习中的相关思想联系起来。

Conclusion: 主观函数为理解目标函数来源和选择机制提供了新视角，有助于开发能够动态合成目标的人工智能系统，在多个学科领域具有重要理论意义。
...
TL;DR: 论文提出了KalshiBench基准，用于评估大语言模型对未来未知事件的校准能力，发现所有前沿模型都存在系统性过度自信问题，即使推理增强模型校准更差，表明扩展规模和增强推理并不能自动改善校准。
...
Method: 引入KalshiBench基准，包含300个来自受监管交易所Kalshi的预测市场问题，这些问题在模型训练截止后有可验证的真实世界结果。评估了五个前沿模型（Claude Opus 4.5、GPT-5.2、DeepSeek-V3.2、Qwen3-235B、Kimi-K2）的校准性能。
...
Conclusion: 扩展模型规模和增强推理能力并不能自动带来校准改进，认知校准是一种需要针对性开发的独立能力。这突显了在模型开发中需要专门关注不确定性量化能力。
...
TL;DR: 开发自动化系统发现和分类课程大纲中的AI相关政策，结合无监督主题建模和LLM分类，帮助理解教育环境中GenAI的使用规范


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的普及，学生使用这些工具时面临政策不明确的问题。各教育机构的AI政策差异大且不断变化，导致学生不清楚使用规范和最佳实践，需要系统化的政策分析工具

Method: 设计自动化系统，结合无监督主题建模技术识别关键政策主题，使用大语言模型（LLMs）对政策文本中的GenAI允许程度和其他要求进行分类

Result: 开发的应用在主题发现方面获得0.73的连贯性分数。基于GPT-4.0的政策分类在八个已识别主题上实现了0.92-0.97的精确度和0.85-0.97的召回率

Conclusion: 该系统提供了结构化和可解释的政策信息，促进了GenAI在教育中的安全、公平和教学对齐使用，可集成到教育技术平台帮助学生理解和遵守相关指南
...
### [103] [WeMusic-Agent: Efficient Conversational Music Recommendation via Knowledge Internalization and Agentic Boundary Learning](https://arxiv.org/abs/2512.16108)
...
Method: 提出WeMusic-Agent训练框架，结合知识内化和智能边界学习，教导模型智能决策何时利用内化知识、何时调用专业工具。具体实现WeMusic-Agent-M1模型，通过500亿音乐相关语料持续预训练内化音乐知识，同时获得必要时调用外部工具的能力。还构建了基于微信听书真实数据的开源对话音乐推荐基准。

Result: 在真实数据上的实验表明，WeMusic-Agent相比现有模型取得了显著改进。构建的基准支持从相关性、个性化、多样性等多个维度进行全面评估。

Conclusion: WeMusic-Agent框架通过知识内化和智能工具调用的结合，有效解决了对话式音乐推荐中专业领域知识与灵活工具集成的平衡问题，在真实场景中表现出优越性能。
...
### [104] [ToolForge: A Data Synthesis Pipeline for Multi-Hop Search without Real-World APIs](https://arxiv.org/abs/2512.16149)
*Hao Chen,Zhexin Hu,Jiajun Chai,Haocheng Yang,Hang He,Xiaohan Wang,Wei Lin,Luhang Wang,Guojun Yin,Zhuofeng zhao*

Main category: cs.AI

TL;DR: ToolForge是一个自动化合成框架，仅需少量虚拟工具即可生成高质量工具调用训练数据，无需真实API调用，8B参数模型在多项基准测试中超越GPT-4o。
...
Conclusion: ToolForge框架能够以低成本生成高质量工具调用训练数据，显著提升模型性能，为工具学习领域提供了高效的数据合成解决方案。
...
### [105] [PDE-Agent: A toolchain-augmented multi-agent framework for PDE solving](https://arxiv.org/abs/2512.16214)
...
Method: 提出PDE-Agent框架，包含两个关键创新：1) 基于图记忆的Prog-Act多智能体协作框架，支持动态规划和双循环错误修正机制；2) 集成工具-参数分离机制的Resource-Pool，集中管理运行时工件并解决工具间依赖问题。

Result: 开发了PDE-Bench多类型PDE基准测试集，提出多级评估指标。验证表明PDE-Agent在复杂多步骤、跨步骤依赖任务中表现出优越的适用性和性能。

Conclusion: 工具链增强的多智能体PDE求解新范式将推动自动化科学计算的未来发展，实现了从自然语言描述到PDE求解的完全自主化流程。
...
### [106] [Scaling Spatial Reasoning in MLLMs through Programmatic Data Synthesis](https://arxiv.org/abs/2512.16237)
...
TL;DR: SPRITE框架通过模拟器和大型模型程序化合成可扩展、多样且高质量的空间推理数据，解决现有方法在可扩展性和计算精度上的困境。


<details>
  <summary>Details</summary>
Motivation: 当前具身智能面临空间理解和推理能力不足的挑战。现有增强视觉语言模型的方法存在两难：基于模板的数据集可扩展但结构僵化，人工标注语言多样但不可扩展且计算不精确。

Method: 将真实标签生成重构为代码生成任务，利用LLM将复杂空间问题编译为可执行程序，然后通过从模拟器提取的高精度场景元信息进行验证，确保计算精确性和可验证性。
...
### [107] [AlignMerge - Alignment-Preserving Large Language Model Merging via Fisher-Guided Geometric Constraints](https://arxiv.org/abs/2512.16245)
*Aniruddha Roy,Jyoti Patel,Aman Chadha,Vinija Jain,Amitava Das*

Main category: cs.AI

TL;DR: AlignMerge：一种几何感知的LLM合并框架，通过显式保持对齐不变性，在合并专家模型时避免破坏安全对齐


<details>
  <summary>Details</summary>
Motivation: 现有LLM合并方法（线性权重组合、任务向量、Fisher加权平均）虽然能保持性能，但会悄悄破坏模型的安全对齐。合并不应是数值技巧，而应是在已对齐锚点周围的几何约束操作

Method: 在指令调优基模型的局部Fisher图中，估计对齐子空间并引入投影器P_A，优化包含几何保持损失、对齐惩罚损失和软对齐预算损失的三项目标函数，使用解码不变的对齐质量指数（AQI）作为对齐功能度量

Result: 在五个模型家族（LLaMA-3 8B、Mistral 7B、Qwen 2、Phi-3.5、Gemma 2）上，AlignMerge在保持对齐指标（AQI、毒性、LLM-judge对齐）的同时，在指令遵循、推理和帮助性方面匹配或超越最佳专家模型，对齐子空间漂移和预算违规更少

Conclusion: 将对齐保持作为合并的一等设计目标，为未来基础模型的几何感知组合提供路径，强调合并应尊重安全几何而非事后验证
...
### [108] [QuadSentinel: Sequent Safety for Machine-Checkable Control in Multi-agent Systems](https://arxiv.org/abs/2512.16279)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在解决复杂任务时使用工具、多步规划和智能体间通信会带来安全风险。部署者编写的自然语言策略存在模糊性和上下文依赖性，难以映射到机器可检查的规则，导致运行时执行不可靠。

Method: 将安全策略表达为序列，构建QuadSentinel四智能体防护系统：状态跟踪器、策略验证器、威胁监视器和裁判。系统将这些策略编译为基于可观察状态的谓词构建的机器可检查规则，并在线执行。裁判逻辑加上高效的top-k谓词更新器通过优先级检查和分层解决冲突来降低成本。

Result: 在ST-WebAgentBench（ICML CUA '25）和AgentHarm（ICLR '25）上的测试显示，QuadSentinel提高了防护准确性和规则召回率，同时减少了误报。相比ShieldAgent（ICML '25）等单智能体基线，它提供了更好的整体安全控制。
...
### [109] [Code-in-the-Loop Forensics: Agentic Tool Use for Image Forgery Detection](https://arxiv.org/abs/2512.16300)
...
<details>
  <summary>Details</summary>
Motivation: 现有图像伪造检测方法要么利用低级语义无关的伪影特征，要么依赖多模态大语言模型的高级语义知识。这两种信息流在范式和推理方式上高度异构，现有方法难以统一它们或有效建模跨级交互。
...
Conclusion: ForenAgent通过让MLLMs自主操作低级工具，实现了更灵活可解释的伪造分析，成功统一了低级伪影特征和高级语义知识，为图像伪造检测提供了新方向。
...
### [110] [Adaptation of Agentic AI](https://arxiv.org/abs/2512.16301)
*Pengcheng Jiang,Jiacheng Lin,Zhiyi Shi,Zifeng Wang,Luxi He,Yichen Wu,Ming Zhong,Peiyang Song,Qizheng Zhang,Heng Wang,Xueqiang Xu,Hanwen Xu,Pengrui Han,Dylan Zhang,Jiashuo Sun,Chaoqi Yang,Kun Qian,Tian Wang,Changran Hu,Manling Li,Quanzheng Li,Hao Peng,Sheng Wang,Jingbo Shang,Chao Zhang,Jiaxuan You,Liyuan Liu,Pan Lu,Yu Zhang,Heng Ji,Yejin Choi,Dawn Song,Jimeng Sun,Jiawei Han*

Main category: cs.AI

TL;DR: 该论文提出了一个系统化框架，统一了智能体AI系统中的适应机制，包括智能体适应和工具适应两大类别，并进一步细分为不同信号形式和监督方式，为研究人员和从业者提供了概念基础和实用路线图。


<details>
  <summary>Details</summary>
Motivation: 随着基于基础模型的智能体AI系统能力不断增强、应用范围不断扩大，适应机制成为提升性能、可靠性和泛化能力的关键手段。当前研究领域快速扩展但缺乏统一框架，需要系统化的理论指导来明确设计空间、权衡利弊并提供实用指导。

Method: 论文提出了一个系统化框架，将适应机制分为智能体适应和工具适应两大类别：1) 智能体适应包括工具执行信号驱动和智能体输出信号驱动两种形式；2) 工具适应包括智能体无关和智能体监督两种形式。该框架用于澄清设计空间、明确权衡，并提供策略选择和切换的实用指导。

Result: 框架成功统一了快速扩展的研究领域，澄清了智能体AI适应策略的设计空间，使各种策略的权衡更加明确，并为系统设计中的策略选择或切换提供了实用指导。论文还回顾了各类别中的代表性方法，分析了它们的优势和局限性。

Conclusion: 该论文为构建更强大、高效和可靠的智能体AI系统提供了概念基础和实用路线图，同时指出了关键开放挑战和未来机会，有助于指导研究人员和从业者在智能体适应和工具适应方面的系统设计和策略选择。
...
### [111] [Design and Evaluation of Cost-Aware PoQ for Decentralized LLM Inference](https://arxiv.org/abs/2512.16317)
*Arther Tian,Alex Ding,Frank Chen,Alan Wu,Aaron Chan,Bruce Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种成本感知的PoQ框架，将效率测量整合到去中心化LLM推理的奖励机制中，通过实验证明该框架能有效奖励高质量低成本的推理模型和高效评估器。


<details>
  <summary>Details</summary>
Motivation: 去中心化LLM推理需要透明和抗审查的AI访问，但现有验证方法难以扩展到现代模型。原始的PoQ忽略了推理节点和评估节点之间的异构计算成本，需要设计成本感知的奖励机制。

Method: 提出成本感知PoQ框架，将显式效率测量整合到奖励机制中。设计结合了真实标记级F1、轻量级学习评估器和基于GPT判断的统一评估流程，采用平衡归一化质量和成本的线性奖励函数。

Result: 实验显示语义文本相似性双编码器比交叉编码器与真实值和GPT分数相关性更高；质量-成本分析表明池中最大模型在单位延迟质量方面最有效；蒙特卡洛模拟证明成本感知奖励方案能持续奖励高质量低成本的推理模型和高效评估器。
...
### [112] [PCIA: A Path Construction Imitation Algorithm for Global Optimization](https://arxiv.org/abs/2512.16392)
...
<details>
  <summary>Details</summary>
Motivation: 受人类构建和使用路径的行为启发，人类通常偏好热门交通路线，在路径关闭时会智能混合现有路径构建新路线，并随机选择不同路径探索未知目的地，这些行为模式为优化算法设计提供了灵感。

Method: PCIA模仿人类路径构建行为：1）生成随机种群寻找最佳路径（类似群体算法）；2）每个粒子代表一条通往目标的路径；3）算法结合了路径混合和随机探索机制；4）在53个数学优化问题和13个约束优化问题上进行测试。

Result: PCIA在测试中表现优异，与流行和最新的元启发式算法相比具有高度竞争力，验证了该算法在解决优化问题上的有效性。

Conclusion: PCIA是一种有效的元启发式优化算法，其基于人类路径构建行为的灵感使其在解决复杂优化问题时表现出色，具有实际应用价值。
...
### [113] [Synthelite: Chemist-aligned and feasibility-aware synthesis planning with LLMs](https://arxiv.org/abs/2512.16424)
*Nguyen Xuan-Vu,Daniel Armstrong,Milena Wehrbach,Andres M Bran,Zlatko Jončev,Philippe Schwaller*

Main category: cs.AI

TL;DR: Synthelite是一个基于大语言模型的计算机辅助合成规划框架，能够通过自然语言交互生成端到端的合成路线，并灵活适应多种用户约束条件。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机辅助合成规划框架缺乏与人类专家交互的机制，限制了化学家洞察力的整合。需要开发能够利用大语言模型内在化学知识和推理能力，同时允许专家通过自然语言提示进行干预的合成规划系统。

Method: 开发了Synthelite框架，使用大语言模型直接提出逆合成转化方案。该系统能够生成端到端的合成路线，利用LLMs的化学知识和推理能力，同时通过自然语言提示允许专家干预。

Result: 实验表明，Synthelite能够灵活适应不同的用户指定约束条件，在策略约束和起始材料约束的合成任务中达到高达95%的成功率。此外，系统在路线设计中展现出考虑化学可行性的能力。
...
### [114] [TIB AIssistant: a Platform for AI-Supported Research Across Research Life Cycles](https://arxiv.org/abs/2512.16442)
...
<details>
  <summary>Details</summary>
Motivation: 随着人工智能和大语言模型的普及，AI支持的研究具有在整个研究生命周期中协助研究人员的潜力，需要开发一个综合平台来实现这一目标。

Method: 开发了TIB AIssistant平台，包含多个专门负责特定研究任务的助手，提供访问外部学术服务的工具，并将生成的数据存储在资产中，可导出为RO-Crate包。

Result: 通过按顺序演示各个助手的交互，展示了平台如何生成研究论文草稿的各个部分，为研究项目提供了透明度和可重复性支持。

Conclusion: TIB AIssistant为AI支持研究奠定了社区维护平台的基础，推动了更大议程的实现。
...
### [115] [Towards AI-Supported Research: a Vision of the TIB AIssistant](https://arxiv.org/abs/2512.16447)
...
TL;DR: TIB AIssistant是一个领域无关的人机协作平台，旨在通过AI助手支持研究生命周期中的各项任务，解决AI在研究中的集成挑战。
...
Method: 提出了TIB AIssistant平台，采用模块化组件设计，包括提示和工具库、共享数据存储以及灵活的编排框架，支持跨学科研究者在科学发现过程中的各项任务。

Result: 描述了概念框架、系统架构和早期原型实现，证明了该方法的可行性和潜在影响。

Conclusion: TIB AIssistant作为一个领域无关的人机协作平台，有潜力通过AI助手支持整个研究生命周期，促进跨学科的科学发现。
...
### [116] [TimeSeries2Report prompting enables adaptive large language model management of lithium-ion batteries](https://arxiv.org/abs/2512.16453)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在解释多元时间序列数据方面具有潜力，但在实际电池储能系统运维中的应用尚未充分探索。需要一种方法将原始传感器数据转换为LLM可理解的形式，以支持电池管理决策。

Method: 提出TimeSeries2Report（TS2R）提示框架，通过分割、语义抽象和基于规则的解释，将短期时间动态编码为自然语言，将低层传感器信号与高层上下文洞察连接起来。

Result: 在实验室和真实数据集上评估，TS2R在异常检测、荷电状态预测和充放电管理等任务中，相比视觉、嵌入和文本基线方法，在准确性、鲁棒性和可解释性方面持续提升LLM性能。TS2R集成的LLM无需重新训练即可达到专家级决策质量和预测一致性。

Conclusion: TS2R为自适应、LLM驱动的电池智能建立了实用路径，通过报告式提示有效连接时间序列数据与LLM推理能力，在电池储能系统管理中实现专家级性能。
...
### [117] [cuPilot: A Strategy-Coordinated Multi-agent Framework for CUDA Kernel Evolution](https://arxiv.org/abs/2512.16465)
...
Conclusion: cuPilot通过策略协调的多智能体框架有效解决了现有方法在CUDA内核优化中的局限性，实现了显著的性能提升，生成的代码已开源。
...
### [118] [Quantifying and Bridging the Fidelity Gap: A Decisive-Feature Approach to Comparing Synthetic and Real Imagery](https://arxiv.org/abs/2512.16468)
*Danial Safaei,Siddartha Khastgir,Mohsen Alirezaei,Jeroen Ploeg,Son Tong,Xingyu Zhao*

Main category: cs.AI

TL;DR: 论文提出了一种新的系统特定保真度度量方法——决定性特征保真度（DFF），用于评估自动驾驶虚拟测试中合成数据与真实数据之间因果证据的一致性，而不仅仅是像素级视觉真实性。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶安全验证依赖虚拟测试和合成数据，但研究发现像素级视觉保真度不足以保证从仿真到真实世界的可靠迁移。关键问题在于系统在真实和模拟环境中是否基于相同的因果证据做出决策，而非图像是否对人眼"看起来真实"。目前缺乏这种基于行为的保真度度量方法。

Method: 提出决定性特征保真度（DFF）度量方法，利用可解释AI技术识别和比较系统在匹配的真实-合成数据对中驱动决策的决定性特征。进一步提出基于反事实解释的实用估计器，以及DFF引导的校准方案来提升仿真器保真度。

Result: 在2126个匹配的KITTI-VirtualKITTI2数据对上的实验表明，DFF能够揭示传统输出值保真度忽略的差异。DFF引导的校准在提升决定性特征和输入级保真度的同时，不会牺牲不同系统间的输出值保真度。

Conclusion: DFF提供了一种系统特定的行为基础保真度度量方法，能够更好地评估仿真到真实世界的迁移可靠性，并通过校准方案提升仿真器质量，为自动驾驶安全验证提供更有效的工具。
...
### [119] [Best Practices For Empirical Meta-Algorithmic Research Guidelines from the COSEAL Research Network](https://arxiv.org/abs/2512.16491)
...
<details>
  <summary>Details</summary>
Motivation: 元算法研究（如算法选择、配置和调度）依赖大量计算密集型实验，实验设计和执行中存在多种误差来源，威胁研究结果的可扩展性和有效性。现有最佳实践分散在不同出版物和领域，需要系统整理。

Method: 收集COSEAL社区各子领域的良好实践，涵盖完整实验周期：从研究问题提出和实验设计选择，到实验执行，再到结果分析和公正呈现。

Result: 建立了元算法研究领域当前最先进的实践标准，为研究人员提供系统指导。

Conclusion: 该报告为元算法领域的新研究人员和实践者提供了全面的实验指南，有助于提高研究的严谨性和可重复性。
...
### [120] [Scaling Laws for Energy Efficiency of Local LLMs](https://arxiv.org/abs/2512.16531)
*Ander Alvarez,Alessandro Genuardi,Nilotpal Sinha,Antonio Tiene,Samuel Mugel,Román Orús*

Main category: cs.AI

TL;DR: 该研究系统性地评估了在CPU上进行本地大语言模型和视觉语言模型推理的计算规律，发现了文本长度与计算成本近似线性相关、视觉语言模型存在分辨率拐点等经验规律，并展示了量子启发压缩技术可显著降低计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 虽然GPU主导AI部署，但大多数消费硬件（笔记本电脑、台式机、工业控制器、嵌入式系统）都依赖CPU。然而，对于本地语言和视觉语言工作负载的纯CPU推理计算规律尚未得到充分探索，需要系统性的基准测试来理解计算负载如何随输入规模变化。

Method: 在两个代表性的CPU层级上进行系统基准测试：MacBook Pro M2（主流笔记本电脑级）和Raspberry Pi 5（低功耗嵌入式设置）。采用基于处理器和内存使用连续采样以及曲线下面积积分的统一方法，分析计算负载如何随语言模型的输入文本长度和视觉语言模型的图像分辨率变化。

Result: 发现了两个经验性扩展规律：1）语言模型推理的计算成本与令牌长度近似线性相关；2）视觉语言模型存在预处理驱动的"分辨率拐点"，在内部分辨率限制以上计算保持不变，以下则急剧下降。量子启发压缩技术可将处理器和内存使用减少高达71.9%，能耗降低高达62%，同时保持或提高语义准确性。

Conclusion: 该研究为本地语言和视觉语言工作负载的多模态纯CPU扩展提供了系统性量化分析，识别出模型压缩和输入分辨率预处理作为可持续边缘推理的有效、低成本杠杆。这些发现有助于在边缘设备上平衡准确性与计算/能源约束。
...
### [121] [Prefix Probing: Lightweight Harmful Content Detection for Large Language Models](https://arxiv.org/abs/2512.16650)
...
Conclusion: Prefix Probing提供了一种高效、低成本的有害内容检测解决方案，通过单次对数概率计算和前缀缓存技术，在保持检测准确性的同时显著降低计算开销。
...
### [122] [Comprehensive AI Literacy: The Case for Centering Human Agency](https://arxiv.org/abs/2512.16656)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI技术快速融入社会，但教育框架未能有效应对，导致危险的素养鸿沟。现有教育过于关注使用AI工具的功能性技能，而忽视了关于AI的批判性和伦理推理能力的培养。

Method: 提出系统性转向全面的AI素养教育，以人类能动性为核心原则。通过AI素养、流利度和能力框架，让教育者和学生成为以人为中心的AI方法的主体。

Result: 建立了以人类能动性为中心的AI素养框架，强调技术不是必然要采纳的，而是需要做出选择的对象。该框架为教育生态系统中的所有利益相关者提供了明确的决策路径。

Conclusion: 真正的AI素养需要教授能动性本身，将技术视为需要做出选择的对象而非必然采纳的产物。这需要深入致力于批判性思维和对认识论的坚实理解，使教育者和学生能够清晰表达对AI的决策意图及其对学术、职业和社会的影响。
...
### [123] [Unsupervised Thematic Clustering Of hadith Texts Using The Apriori Algorithm](https://arxiv.org/abs/2512.16694)
*Wisnu Uriawan,Achmad Ajie Priyajie,Angga Gustian,Fikri Nur Hidayat,Sendi Ahmad Rafiudin,Muhamad Fikri Zaelani*

Main category: cs.AI

TL;DR: 使用Apriori算法对印尼语布哈里圣训进行无监督主题关联挖掘，发现礼拜、启示、圣训叙述等主题模式
...
Result: 发现了有意义的关联模式：如"rakaat（拜数）-prayer（礼拜）"、"verse（经文）-revelation（启示）"、"hadith（圣训）-story（故事）"，这些模式分别描述了礼拜、启示和圣训叙述等主题
...
### [124] [Do Multi-Agents Solve Better Than Single? Evaluating Agentic Frameworks for Diagram-Grounded Geometry Problem Solving and Reasoning](https://arxiv.org/abs/2512.16698)
...
<details>
  <summary>Details</summary>
Motivation: 探究在多模态大语言模型中，多智能体设计相比单智能体在几何问题求解任务中的实际效益，明确其适用条件和局限性

Method: 在四个视觉数学基准（Geometry3K、MathVerse、OlympiadBench、We-Math）上系统比较单智能体和多智能体流水线，使用开源模型（Qwen-2.5-VL）和闭源模型（Gemini-2.0-Flash）进行对比分析

Result: 开源模型在多智能体模式下性能显著提升（如Qwen-2.5-VL 7B在Geometry3K上提升+6.8分），而闭源模型Gemini-2.0-Flash在经典基准上单智能体表现更好，仅在新数据集We-Math上多智能体有适度改进

Conclusion: 多智能体流水线对开源模型有明显益处，并能辅助强大的专有系统处理较新、较不熟悉的基准，但智能体分解并非普遍最优策略
...
### [125] [Cyber Humanism in Education: Reclaiming Agency through AI and Learning Sciences](https://arxiv.org/abs/2512.16701)
*Giovanni Adorni*

Main category: cs.AI

TL;DR: 本文提出"教育中的赛博人文主义"框架，旨在在生成式AI重塑教育的背景下重新主张人类能动性，将AI学习环境视为人机共同构建的社会技术基础设施，并通过三个支柱（反思能力、算法公民身份、对话式设计）和高等教育案例研究来实施这一理念。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正在迅速改变教育中知识生产和验证的方式，将阅读、写作和编程重构为混合人机工作流程，这引发了关于认知自动化、认知卸载和教师去专业化等问题的担忧。需要建立一个框架来重新主张人类在这一变革中的能动性。

Method: 提出"教育中的赛博人文主义"框架，将AI学习环境概念化为人类和机器共同构建的社会技术基础设施，并建立三个设计支柱：反思能力、算法公民身份和对话式设计。通过高等教育案例研究，包括基于提示的学习和EPICT生态系统中的"对话式AI教育者"认证，来实施这些理念。

Result: 研究发现，这些实践能够增强认知能动性，同时揭示了关于工作量、公平性和治理方面的紧张关系。案例研究表明，赛博人文主义设计能够帮助教育者和学习者作为算法公民塑造AI教育基础设施。

Conclusion: 赛博人文主义为AI丰富、以人为本的教育未来提供了重要框架，强调人类作为认知主体和算法公民的权利与责任，需要在增强人类能动性的同时解决工作量、公平性和治理等实际问题。
...
### [126] [Dual Computational Horizons: Incompleteness and Unpredictability in Intelligent Systems](https://arxiv.org/abs/2512.16707)
*Abhisek Ganguly*
...
<details>
  <summary>Details</summary>
Motivation: 研究算法智能的固有计算限制，特别是形式不完备性和动态不可预测性如何共同约束智能系统的推理、预测和自我分析能力

Method: 形式化两个独立计算限制：形式不完备性（限制一致推理系统的演绎能力）和动态不可预测性（限制有限精度下的长期预测），分析它们对智能体自我预测能力推理的结构性约束

Result: 证明这两个极端共同对智能体推理自身预测能力施加结构性限制，特别是算法智能体通常无法计算自身的最大预测范围

Conclusion: 这一视角阐明了智能系统中推理、预测和自我分析之间固有的权衡关系，揭示了算法智能的基本局限性
...
### [127] [Discovering and Learning Probabilistic Models of Black-Box AI Capabilities](https://arxiv.org/abs/2512.16733)
*Daniel Bramblett,Rushang Karia,Adrian Ciotinga,Ruthvick Suresh,Pulkit Verma,YooJung Choi,Siddharth Srivastava*

Main category: cs.AI

TL;DR: 该论文提出了一种使用PDDL风格表示来学习和建模黑盒AI系统规划能力的方法，通过蒙特卡洛树搜索创建测试任务、获取数据并剪枝假设空间，学习得到的模型能够描述AI能力、执行条件和可能结果及其概率。
...
Method: 采用PDDL风格表示来建模黑盒AI的规划能力，使用蒙特卡洛树搜索范式系统性地创建测试任务、获取数据，并对可能的符号模型假设空间进行剪枝。

Result: 理论结果表明学习模型具有可靠性、完备性和收敛性。多个黑盒AI系统的实证结果展示了所提方法的范围、效率和准确性。

Conclusion: PDDL风格表示能够有效学习和建模黑盒AI系统的规划能力，为理解这些系统的能力提供了可靠且可解释的方法，有助于确保AI系统的安全部署。
...
### [128] [AI-Driven Prediction of Cancer Pain Episodes: A Hybrid Decision Support Approach](https://arxiv.org/abs/2512.16739)
...
TL;DR: 提出混合机器学习和大型语言模型管道，预测肺癌患者住院48和72小时内的疼痛发作，结合结构化与非结构化电子健康记录数据，提高预测准确性和临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 肺癌患者中高达91%会经历突破性疼痛发作，需要及时干预。目前缺乏能够提前预测疼痛发作的工具，无法实现主动疼痛管理。电子健康记录中包含大量结构化数据和非结构化临床文本，但传统方法难以有效整合这些信息进行预测。

Method: 采用混合机器学习和大型语言模型管道：1) 机器学习模块分析结构化数据（人口统计学、肿瘤分期、生命体征、WHO分级镇痛药使用），捕捉时间性药物使用趋势；2) 大型语言模型处理非结构化数据，解释模糊的给药记录和自由文本临床笔记；3) 整合两种模态数据，提高预测敏感性和可解释性。基于266名住院患者的回顾性队列进行分析。

Result: 框架在48小时预测中达到0.874的准确率，72小时预测达到0.917的准确率。大型语言模型的增强使敏感性分别提高了8.6%和10.4%。该方法提供了临床可解释且可扩展的工具，能够早期预测疼痛发作。

Conclusion: 混合方法为早期疼痛发作预测提供了临床可解释且可扩展的工具，有望提高肿瘤护理中的治疗精确性和资源分配优化。整合结构化与非结构化数据显著提升了预测性能，特别是通过大型语言模型处理模糊临床信息的能力。
...
### [129] [CitySeeker: How Do VLMS Explore Embodied Urban Navigation With Implicit Human Needs?](https://arxiv.org/abs/2512.16755)
...
Method: 提出CitySeeker基准测试，包含8个城市的6,440条轨迹，涵盖7种目标驱动场景；通过实验分析模型瓶颈，并探索回溯机制、丰富空间认知和基于记忆检索等策略。

Result: 即使表现最佳的模型（如Qwen2.5-VL-32B-Instruct）任务完成率仅为21.1%，主要瓶颈包括长时推理中的错误累积、空间认知不足和经验回忆缺陷。

Conclusion: 研究揭示了视觉语言模型在隐含需求导航中的关键挑战，提出的BCR策略为开发具有强大空间智能的模型提供了可行见解，有助于解决"最后一公里"导航问题。
