...
Motivation: AI生成图像的真实性越来越难以区分，需要开发能够验证视觉真实性的系统。

Method: 使用轻量级卷积分类器"Faster-Than-Lies"与视觉语言模型Qwen2-VL-7B结合，通过自编码器重建误差图生成伪影定位热力图，将70种视觉伪影分为8个语义组。

Result: 在增强的CiFAKE数据集上达到96.5%准确率，在8核CPU上推理时间为175ms，能够生成可解释的文本描述检测到的异常。

Conclusion: 证明了在低分辨率图像中结合视觉和语言推理进行可解释真实性检测的可行性，在取证、工业检测和社交媒体审核等领域有应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有计数模型在物体具有复杂形状、内部对称性或重叠组件时经常计数错误，无法像人类一样通过感知视觉重复和结构关系来计数。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮的人为垃圾日益增多，对生物多样性、水质以及航运和娱乐等人类活动产生不利影响，需要有效的监测方法。

Method: 使用固定原位摄像头采集数据，应用深度学习模型进行漂浮垃圾检测和量化，测试了多种环境条件和学习配置，包括数据泄露偏差实验，并实现了几何模型从2D图像估计物体实际尺寸。

Result: 研究强调了数据集构建协议的重要性，特别是负样本图像的整合和时间泄露的考虑，证明了使用投影几何结合回归修正进行度量物体估计的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像超分辨率在分布外条件下（如罕见地貌特征和不同传感器数据）往往会产生视觉上合理但物理上不准确的结果，需要开发能够保持物理一致性的鲁棒方法。

Method: 采用双条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示提供复杂特征的语义指导；引入多面损失函数确保光谱和辐射测量一致性；使用随机前向传播方法量化预测不确定性。

Result: 在盲评估中，地球物理专家认为模型输出接近地面真实图像的保真度，显著优于最先进基线；在感知指标上取得定量增益，FID降低近40%。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D场景生成方法通常局限于单对象生成、需要领域特定训练或缺乏完整的360度视角支持。需要一种更通用、无需专门训练且支持大规模场景生成的方法。

Method: 将文本到3D对象扩散模型重新用作模块化瓦片生成器，将场景生成重新表述为多瓦片去噪问题。独立生成重叠的3D区域，通过加权平均进行无缝混合。

Result: 该方法支持多样化的场景布局、高效生成和灵活编辑，能够合成大规模连贯场景，同时保持局部语义控制。

Conclusion: 该方法为通用语言驱动的3D场景构建提供了一个简单而强大的基础，无需场景级数据集或重新训练，继承了对象级先验的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 教学视频中的场景级字幕需要理解视觉线索和时间结构，但现有字幕缺乏这种结构会导致不连贯和质量问题，影响教育效果。

Method: 使用YouCookII数据集，通过自适应帧采样和多模态窗口技术捕捉场景转换，采用多模态思维链生成动作-对象对，并通过动态步长窗口选择算法进行精炼和融合。

Result: 在BLEU、METEOR、BERTScore和CLIPScore等评估指标上均优于VLLaMA3和GPT-4o等强基线模型，产生的时间连贯性和信息量更好的字幕。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人像生成图像到3D模型存在视觉伪影、细节缺失和身份保持不足的问题，而图像扩散模型虽然质量高但计算昂贵且缺乏3D一致性。

Method: 采用前馈图像到化身生成流程获取初始3D表示和噪声渲染，然后通过单步扩散模型进行多视角一致的细化，结合合成多视图数据预训练和高质量真实图像微调。

Result: 该方法在质量和数量上均优于当前最先进的人像新视角合成方法，同时保持时间效率。

Conclusion: TurboPortrait3D成功展示了如何利用图像空间扩散模型提升图像到化身方法的生成质量，同时保持3D感知和低延迟运行。
...
<details>
  <summary>Details</summary>
Motivation: 在室内场景中，大面积的低纹理区域使得基于光度损失的3D高斯泼溅方法难以恢复准确的3D几何表面，导致重建质量下降。

Method: 提出LP3管道，使用预训练视觉语言分割模型生成区域建议，并通过跨视图融合和几何先验检查进行优化；在3DGS优化中加入平面先验监督和几何先验监督项。

Result: 在标准室内基准测试中，PlanarGS重建出准确且详细的3D表面，大幅优于现有最先进方法。

Conclusion: PlanarGS通过结合语言提示平面先验和几何监督，显著提升了室内场景的3D重建质量，解决了3DGS在低纹理区域的局限性。
...
Method: 受计算机图形学中通用场景描述符(USD)标准的启发，引入Neural USD框架，采用结构化分层表示场景和对象，应用微调方法确保控制信号解耦。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像护栏模型受限于预定义类别，缺乏语义推理能力，难以适应新兴威胁且需要昂贵的重新训练。需要开发更高效、透明的安全内容防护系统。

Method: 提出SafeVision系统，包含有效的数据收集和生成框架、策略遵循训练流程、定制化损失函数，以及多样化的问答生成和训练策略来增强学习效果。

Result: SafeVision在不同基准测试中实现最先进性能，在VisionHarm-T上比GPT-4o高出8.6%，在VisionHarm-C上高出15.5%，同时速度提升超过16倍。

Conclusion: SafeVision建立了全面、策略遵循且可解释的图像护栏系统，能够动态适应新兴威胁，为数字媒体安全提供高效透明的保护方案。
...
Method: 采用两阶段训练方法：先进行推理风格的监督微调，然后使用基于X光异常列表的可验证奖励进行强化学习，结合高保真视觉编码生成系统化推理过程。

Result: 在分布外评估中实现竞争性多标签分类性能，同时提升可解释性；专家放射科医生研究表明完整推理轨迹能增加信心、支持错误审计并减少报告完成时间。

Conclusion: 该框架在保持准确性的同时显著提高了医学AI系统的透明度和可审计性，为胸部放射学及其他医学影像任务中可信赖、可解释AI的发展提供了支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常孤立评估AI助手的能力，缺乏真实的流式场景或仅支持短期任务，无法满足现实世界中自我中心AI助手需要处理多模态输入、实时响应并保持长期记忆的需求。

Method: 构建包含工作学习、生活方式、社交活动和外出文化四个领域的同步多模态数据集，所有数据在统一时间线上对齐，并通过人工精炼获得高质量视觉叙述和语音转录。定义了12个诊断子任务，涵盖记忆、理解和跨记忆推理三大核心能力。

Result: TeleEgo包含3,291个人工验证的QA项目，支持多种问题格式（单选、二元、多选和开放式），在严格流式设置下进行评估。提出了实时准确率和记忆持久时间两个关键指标来联合评估正确性、时间响应性和长期保持能力。

Conclusion: TeleEgo提供了一个现实且全面的评估框架，能够推动实用AI助手的发展，解决了现有基准在长时、流式和全模态评估方面的不足。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是导致视力丧失的主要原因，现有深度学习模型由于采集设备、人口统计差异和成像条件等因素导致的分布变化而面临鲁棒性挑战。

Method: AdvBlur方法集成对抗性模糊图像到数据集中，并采用双损失函数框架来处理领域泛化问题。

Result: 在多个数据集上的综合评估表明，该方法有效减轻了未见分布变化的影响，在未见外部数据集上取得了与最先进领域泛化DR模型相竞争的性能。

Conclusion: AdvBlur方法通过对抗性模糊图像和双损失函数框架，成功提高了糖尿病视网膜病变分类模型在领域泛化方面的性能。
...
Method: 挑战赛在隐藏测试集上对自动算法进行基准测试，包括可选的计算模拟表面网格生成任务。主要采用3D U-Net架构的深度学习方法，并探索模型集成策略。

Result: 顶级提交算法显示出向深度学习方法的明显趋同，3D U-Net架构占据主导地位。排名最高算法的集成模型显著优于单个模型。性能与算法设计（特别是定制后处理步骤）和训练数据特征密切相关。
...
<details>
  <summary>Details</summary>
Motivation: 火星科学领域缺乏标准化的基准测试和评估框架，这限制了火星基础模型的发展。作者希望填补这一空白，推动火星科学中机器学习模型的系统发展。

Method: 构建了包含20个数据集的Mars-Bench基准测试，涵盖轨道和表面图像，专注于关键地质特征如陨石坑、锥体、巨石和霜冻。提供了标准化的数据集，并使用在自然图像、地球卫星数据和最先进视觉语言模型上预训练的模型进行基线评估。

Result: 所有分析结果表明，火星特定的基础模型可能比通用领域模型具有优势，这激励了对领域自适应预训练的进一步探索。

Conclusion: Mars-Bench旨在为火星科学中的机器学习模型开发和比较建立标准化基础，其数据、模型和代码已公开提供。
...
Method: 采用交替优化-微调管道，在对抗性后缀优化和LLM微调之间交替进行；在优化阶段集成双重规避策略，通过辅助LLM困惑度评分和禁止令牌惩罚来绕过基于困惑度的过滤器和黑名单词过滤器。

Result: 实验证明该方法在红队测试中表现优异，生成的人类可读、抗过滤的对抗性提示具有出色的零样本可迁移性，能够即时适应未见过的提示，并在商业API中暴露关键漏洞。

Conclusion: APT框架有效解决了现有方法的局限性，为文本到图像模型的安全评估提供了更高效、实用的黑盒测试方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前稀疏调优方法存在两个问题：1）两阶段方法在定位任务相关权重时忽略了微调过程中的参数调整，限制了性能；2）稀疏梯度掩码导致高内存使用。

Method: 提出单阶段方法SNELLA：1）通过两个低秩可学习矩阵合并的稀疏矩阵选择性更新权重矩阵，并引入非线性核函数增加合并矩阵的秩；2）提出自适应双层稀疏分配机制，基于重要性分数在层间和层内进行端到端的权重竞争。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注对抗微调或提示优化，但忽视了CLIP编码特征中的文本和图像特征距离过远的问题，这种错位在对抗扰动下会被显著放大，导致分类性能严重下降

Method: 1) 将对抗图像嵌入投影到类别文本特征张成的子空间，过滤非语义失真；2) 将图像和文本建模为多个增强视图上的离散分布，通过最优传输优化对齐，并将子空间投影无缝集成到成本计算中

Result: 在14个零样本分类基准测试中验证了COLA的有效性，在ImageNet及其变体上平均提升6.7%的准确率，同时在干净样本上保持高精度

Conclusion: COLA是一个无需训练、与现有微调模型兼容的框架，能有效恢复对抗条件下的全局图像-文本对齐和局部结构一致性，显著提升CLIP模型的对抗鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的文本修复方法在文本插入和编辑方面表现出色，但存在三个关键限制：无法删除文本、缺乏文本样式控制以及容易生成重复字母，这限制了它们在更广泛的文本图像操作任务中的应用。

Method: 提出OmniText框架，利用交叉注意力和自注意力机制的特性：通过自注意力反转实现文本删除，通过交叉注意力重分布减少文本幻觉。在潜在优化框架中引入交叉注意力内容损失和自注意力样式损失，以实现可控的文本修复和样式定制。

Result: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，在多个任务和指标上达到了最先进的性能，与其他文本修复方法相比表现出色，并与专门方法相媲美。

Conclusion: OmniText框架成功解决了文本图像操作中的关键限制，通过创新的注意力机制分析和损失函数设计，实现了文本删除、样式控制和内容准确性的全面提升，为广泛的文本图像操作任务提供了有效的解决方案。
...
Method: 提出固有可解释性评分（IIS），通过衡量解释过程中信息损失来量化表示的可解释性，并评估不同分类性能下表示的可解释性。

Result: 发现可解释性与分类性能呈正相关，高分类性能的表示包含更多可被解释捕获的可解释语义；通过可解释性最大化微调可进一步提升分类性能。

Conclusion: 预训练视觉模型的可解释性和分类性能可以统一提升，为实践者提供了同时改进这两个方面的途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要针对同构模型设计，在异构场景下性能下降，特别是涉及中间特征时。架构多样性引入语义差异，阻碍中间表示的有效利用。

Method: 使用傅里叶变换捕捉全局特征信息，通过特征转换模块（FTM）生成紧凑的频域表示，特征对齐模块（FAM）进行多级匹配对齐，结合中间特征的均方误差和logits的KL散度联合训练。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但受限于昂贵的迭代采样过程。现有的训练免费加速方法在重用模型输出时忽略了去噪趋势，缺乏对模型特定容忍度的误差控制，导致轨迹偏差和生成结果不一致。

Method: 1. 引入一致性趋势预测器，利用扩散轨迹的平滑连续性，将历史去噪模式投影到稳定的未来方向，并在多个近似步骤中渐进分布以实现无偏差加速；2. 提出模型特定误差容忍度搜索机制，通过识别从波动语义规划到稳定质量优化的过渡点来推导校正阈值。

Result: 实验表明，ETC在FLUX基础上实现了2.65倍的加速，同时一致性退化极小（-0.074 SSIM分数）。

Conclusion: ETC框架有效解决了扩散模型加速采样中的轨迹偏差问题，在显著提升采样效率的同时保持了生成质量的一致性。
...
Method: 利用大语言模型从输入提示中合成显式布局，将这些布局注入图像生成过程，并使用目标中心视觉语言模型对多个候选结果进行重新排序，迭代选择最符合提示的结果。

Result: 通过统一显式布局基础与基于自优化的推理时扩展，该框架在场景与提示对齐方面比最近的文本到图像模型表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 目前文本到视频生成领域缺乏专门针对训练优化的视频字幕策略，高质量的视频-文本对对于生成连贯且符合指令的视频至关重要。

Method: 提出了VC4VG框架，从T2V角度分析字幕内容，将视频重建所需要素分解为多个维度，并建立了VC4VG-Bench基准，包含细粒度、多维度和必要性分级的评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解任务中的性能问题，特别是在感知、预测、规划和异常检测等多样化任务上的表现优化。

Method: 采用四核心组件：混合提示路由分类问题并分发到专家提示；任务特定提示嵌入坐标系统、空间推理规则等；视觉组装模块组合多视图图像；针对任务配置推理参数。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为SAM的继任者，在视频分割中表现出强大的泛化能力，但其对抗鲁棒性尚未被探索。现有针对SAM的攻击方法是否能直接迁移到SAM2存在疑问，且SAM2的架构差异带来了新的挑战。

Method: 提出UAP-SAM2方法：1）采用目标扫描策略将每帧分为k个区域并随机分配提示，减少优化过程中的提示依赖；2）设计双语义偏差框架，通过扭曲当前帧语义和破坏连续帧间语义一致性来优化通用对抗扰动。

Result: 在6个数据集上的两个分割任务实验表明，UAP-SAM2显著优于现有最先进的攻击方法，证明了其有效性。

Conclusion: UAP-SAM2成功解决了SAM2模型面临的跨提示可迁移性和语义纠缠挑战，为评估分割基础模型的鲁棒性提供了有效工具。
...
Method: 提出CLFSeg框架，集成模糊卷积(FC)模块，结合卷积层和模糊逻辑来识别局部和全局特征，同时减少边界区域的不确定性、噪声和模糊性。采用二元交叉熵(BCE)与dice损失结合来处理类别不平衡问题。

Result: 在CVC-ColonDB、CVC-ClinicDB、EtisLaribPolypDB和ACDC四个公开数据集上表现出色，超越了现有SOTA性能，能够专注于解剖结构的相关感兴趣区域。
...
Result: 相比标准自回归解码，在图像生成中实现约4.2倍加速，在视频生成中实现约13.3倍加速，且输出质量无损失。
...
Conclusion: ID²Face通过训练中心的方法有效解决了面部匿名化中的身份与非身份属性解耦问题，实现了高质量的匿名化效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉token修剪方法主要基于注意力分数选择最显著的token，导致所选token语义不完整，存在冗余计算开销问题。

Method: 提出SCOPE评分策略，结合token的显著性和覆盖度增益，迭代选择评分最高的token，其中覆盖度基于token关系计算。

Result: 在LLaVA-1.5和LLaVA-Next模型上的多模态理解基准测试中，该方法持续优于现有方法。

Conclusion: SCOPE方法通过平衡显著性和覆盖度，有效解决了视觉token修剪中的语义不完整问题，提升了多模态大语言模型的效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于注意力的深度模型在图像阴影去除中，固定的注意力模式容易混合不相关区域的照明线索，导致结构扭曲和颜色不一致。需要探索新的序列建模方法来解决这些问题。

Method: 采用Mamba选择性状态空间模型，提出CrossGate方向调制机制将阴影感知相似性注入输入门，选择性整合相关上下文；引入ColorShift正则化对比学习目标，通过全局颜色统计指导模型抑制颜色污染。

Result: 在公开基准测试上的广泛实验表明，DeshadowMamba实现了最先进的视觉质量和强大的量化性能。

Conclusion: 通过将序列建模与阴影去除所需的结构完整性和色彩一致性要求相结合，所提出的组件能够有效提升阴影去除效果。
...
Conclusion: 该方法在数据稀缺条件下表现出色，新数据集为未来少样本和零样本方法的发展提供了有价值的基准。
...
Method: 开发ViPER自举框架，通过自批判和自预测实现迭代进化，协同整合图像级和实例级重建与两阶段强化学习策略，建立闭环训练范式。

Result: 应用于Qwen2.5-VL系列产生Qwen-Viper系列，在七个综合基准测试中平均提升1.7%，在细粒度感知任务上最高提升6.0%，在不同视觉语言场景中均表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 遥感应用中深度学习性能受限于标记数据稀缺和跨域标注成本高，现有视觉语言模型如CLIP直接应用于遥感领域存在显著领域差距和任务特定语义适应需求。

Method: 评估了多种提示学习方法：上下文优化、条件上下文优化、多模态提示学习、以及带自调节约束的提示学习，并与零样本CLIP和线性探测基线进行比较。

Result: 在多个遥感基准数据集上的实验表明，提示学习方法在少样本场景中始终优于两个基线，其中带自调节约束的提示学习实现了最稳健的跨域性能。

Conclusion: 提示学习为卫星和航空影像中的领域差距提供了可扩展且高效的解决方案，为未来研究奠定了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架如nnU-Net在脑卒中病灶分割中性能优异但难以临床部署，存在依赖重、设计臃肿的问题，需要开发轻量可部署的解决方案。

Method: 采用模块化设计，预处理使用Anima工具箱生成BIDS兼容输出，推理使用ONNX Runtime配合Float16量化，提供图形界面和命令行界面，支持Python脚本和Windows独立可执行文件分发。

Result: 在300名亚急性和慢性卒中患者的测试集上，分割性能与原始PyTorch管道相当（Dice差异<10^-3），模型大小减少约50%。

Conclusion: 高性能研究管道可以成功转化为便携、临床可用的工具，StrokeSeg框架实现了从研究到临床部署的有效转化。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像训练通常伴随包含丰富专家注释的放射学报告，但依赖这些报告需要放射科医生的及时手动工作。研究旨在确定何时以及如何在训练中利用放射学报告来改进仅基于图像的分类。

Method: 系统研究放射学报告在预训练和微调阶段的使用，涵盖诊断和预后任务，并在不同训练集规模下进行评估。

Result: 研究发现：(1) 当标签在文本中表现良好时，在预训练阶段利用报告有益，但当标签与文本关联较弱时，通过显式图像-文本对齐进行预训练可能有害；(2) 在微调阶段使用报告可以带来显著改进，在某些设置下甚至比预训练方法影响更大。
...
Method: 使用基于流的生成模型REFLECT进行无监督异常检测，在ATLAS数据上使用双专家中心切片标注，通过Free-Response ROC分析评估异常图的对象级性能，比较了在卒中患者无病变切片和健康对照数据上训练的两种模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中目标数量未知和时变的问题，特别是在保持目标身份一致性和处理非线性动态方面的挑战，以及应对弱噪声检测器的情况。

Method: 提出混合跟踪方法，结合随机和确定性方式；利用粒子群优化和提出的适应度度量引导粒子；集成目标间的社会交互；建立基于空间一致性、外观、检测置信度、轨迹惩罚和社会分数的综合状态观测模型。

Result: 在标准基准测试和实际场景中，GenTrack相比最先进的跟踪器表现出优越性能，特别是在减少ID切换和轨迹丢失方面。

Conclusion: GenTrack提供了一种有效的多目标跟踪解决方案，通过混合方法和PSO优化显著提升了跟踪性能，并提供了开源实现供进一步研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中未知和时变目标数量、非线性动态以及标识符一致性的挑战，特别是在目标交互和长时间遮挡情况下。

Method: 使用随机粒子滤波处理非线性动态和非高斯噪声，结合粒子群优化引导粒子；采用确定性关联确保标识符一致性；提出平滑更新目标状态的方案；利用历史状态进行速度回归。

Result: 实验结果表明，与最先进的跟踪器相比，该方法具有优越的性能。

Conclusion: 所提出的跟踪器在保持目标身份一致性和处理复杂场景方面表现优异，适用于多种应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型在安全关键和高风险领域的应用增加，确保透明度和信任至关重要。虽然可解释AI方法在分类任务中的评估已有广泛进展，但针对语义分割的评估策略相对较少探索。

Method: 引入一个全面系统的评估框架，专门用于评估语义分割中的可解释AI方法，明确考虑空间和上下文任务复杂性。该框架采用像素级评估策略和精心设计的指标。

Result: 使用最近改编的基于类激活映射的可解释AI方案的仿真结果证明了所提出方法的高效性、鲁棒性和可靠性。
...
TL;DR: 提出基于功能分解的深度条件图像压缩框架DCIC-sgp，通过自生成先验来分离图像结构骨架和纹理细节，有效缓解低码率下的几何变形问题。


<details>
  <summary>Details</summary>
Motivation: 当前学习图像压缩方法难以建模自然图像中复杂的相关性结构，特别是全局不变结构与局部瞬态纹理的纠缠，导致低码率下出现严重几何变形。

Method: 使用功能分解框架，首先编码自生成先验来捕获图像结构骨架，然后用该先验全面调制整个压缩流程，特别是分析变换，使其专注于残差的高熵细节。

Result: 视觉分析显示显著缓解了传统编解码器在低码率下的几何变形伪影，在Kodak、CLIC和Tecnick数据集上相比VTM-12.1分别实现了14.4%、15.7%和15.1%的BD-rate降低。

Conclusion: 提出的深度条件图像压缩框架通过层次化、依赖驱动的方法有效分离信息流，在保持竞争性性能的同时显著改善了低码率下的视觉质量。
...
TL;DR: 视频扩散模型（VDMs）在视觉领域表现出比语言模型更强的数据效率和组合理解能力，通过时空数据预训练获得的结构和动态归纳偏置有助于构建视觉基础模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言领域表现出强大的少样本适应能力，但这种成功尚未有效转化到视觉领域。视觉模型在组合理解、样本效率和通用问题解决方面仍然存在困难。

Method: 使用预训练的视频扩散模型（VDMs）和语言模型（LLMs），为它们配备轻量级适配器，在自然模态中执行任务。通过ARC-AGI、ConceptARC、视觉游戏、路径规划和元胞自动机等基准测试进行对比评估。

Result: 在多个基准测试中，视频扩散模型显示出比语言模型更高的数据效率，表明视频预训练为视觉基础模型提供了有益的归纳偏置。

Conclusion: 视频预训练提供的结构和动态归纳偏置是推进视觉基础模型发展的有前景方向，能够支持广泛的任务适应性。
...
TL;DR: 该论文提出使用深度学习技术检测茶叶病害，包括红锈病、Helopeltis和红蜘蛛螨三种病害，并评估了SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，其中Faster R-CNN表现更优，mAP达到25%。还使用Mask R-CNN进行实例分割以计算叶片受损区域。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别茶叶病害并量化叶片受损区域的深度学习系统，以帮助茶农及时检测和处理病害问题。

Method: 使用SSD MobileNet V2和Faster R-CNN ResNet50 V1进行目标检测，并使用Mask R-CNN进行实例分割，同时开发了自定义方法来计算叶片受损部分。

Result: Faster R-CNN ResNet50 V1在IOU 0.50:0.95范围内获得25%的mAP，优于SSD MobileNet V2的20.9% mAP。

Conclusion: Faster R-CNN ResNet50 V1在茶叶病害检测中表现更好，结合Mask R-CNN的实例分割可以准确量化叶片受损区域，为茶叶病害管理提供了有效的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的无标定运动捕捉方法虽然减少了对精确相机标定的需求，但存在计算成本高和重建精度低的问题，限制了非专家用户和在野外环境中的使用。

Method: 利用现成的2D关键点检测器同时标定相机（包括Brown-Conrady畸变系数）并重建3D关键点和密集场景点云；采用置信度驱动的时空关键点采样策略和基于图的全局优化，确保鲁棒的标定；引入成对重投影一致性评分来量化3D重建可靠性。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比现有最先进方法，Kineo将相机平移误差降低约83-85%，相机角度误差降低86-92%，世界平均每关节误差降低83-91%。

Conclusion: Kineo提供了一种高效、准确的无标定运动捕捉解决方案，在实际场景中处理速度快于视频时长，代码开源以促进可重复性和实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型和流模型）需要大量去噪步骤，存在离散化误差问题。流映射可以缓解此误差并加速采样，但传统方法需要架构修改，与预训练模型不兼容。
...
Conclusion: 训练流模型后转换为流映射比从头训练更高效有效，该方法实现了高质量快速生成，在保持性能的同时显著提升推理效率。
...
TL;DR: Latent Sketchpad框架为多模态大语言模型添加内部视觉草稿本，使其能够在推理过程中生成视觉潜在表示，从而支持视觉规划和想象能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在需要视觉规划和想象的复杂场景中表现不佳，而人类使用草图作为视觉思维工具来发展和交流想法，因此需要为模型提供类似的视觉思考能力。

Method: 在现有MLLMs基础上集成视觉生成到其自回归推理过程中，包含两个组件：上下文感知视觉头自回归生成视觉表示，预训练草图解码器将这些表示渲染为可解释的图像。

Result: 在MazePlanning数据集上的实验表明，Latent Sketchpad在保持推理能力的同时，提供了与骨干模型相当甚至更优的推理性能，并能泛化到不同的前沿MLLMs。
...
<details>
  <summary>Details</summary>
Motivation: 病理学家对千兆像素全切片图像（WSI）的诊断准确率平均约为70%，增加第二名病理学家并不能显著提高决策一致性。该领域缺乏足够的行为数据来解释诊断错误和不一致性。

Method: 通过名为PTAH的应用驱动测试平台收集数据，强调生态有效性。数据集包含19名病理学家解读397张WSI时产生的18.69小时眼动追踪、鼠标交互、刺激追踪、视口导航和诊断决策数据。

Result: 总共记录了171,909次注视、263,320次扫视和1,867,362次鼠标交互事件。

Conclusion: 这些数据可用于改进病理学家和AI系统的训练，以支持人类专家。所有实验都进行了预注册，完整数据集和分析代码已公开。
...
TL;DR: 本文提出了一种名为Group Relative Attention Guidance（GRAG）的方法，通过重新加权DiT模型中不同token的delta值来调节模型对输入图像相对于编辑指令的关注度，实现无需调参的连续细粒度编辑强度控制。
...
Method: 通过分析DiT模型中的MM-Attention机制，发现Query和Key token共享一个仅与层相关的偏置向量，将此偏置解释为模型固有的编辑行为，而每个token与其对应偏置之间的delta编码了内容特定的编辑信号。基于此提出GRAG方法，重新加权不同token的delta值来调节编辑强度。

Result: 在现有图像编辑框架上的大量实验表明，GRAG只需四行代码即可集成，持续提升编辑质量。与常用的Classifier-Free Guidance相比，GRAG实现了更平滑和精确的编辑程度控制。

Conclusion: GRAG是一种简单有效的方法，能够实现对图像编辑强度的连续细粒度控制，无需任何调参即可提升编辑质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法（如线性混合、交叉淡入淡出、变形等）在处理具有大时间间隔或显著语义差异的多样化视频片段时，难以保持视觉连贯性和内容一致性，存在质量限制。

Method: SAGE借鉴艺术工作流程，通过线条图和运动流提供结构引导，结合生成合成技术，实现无需微调的结构感知视频过渡。

Result: 与现有方法（FILM、TVG、DiffMorpher、VACE、GI）相比，SAGE在定量指标和用户研究中均表现出更优的性能，能够为多样化视频片段生成更平滑、语义一致的过渡效果。

Conclusion: SAGE通过结构感知的生成方法有效解决了视频过渡中的语义一致性和视觉连贯性问题，为专业视频编辑提供了高质量的过渡解决方案。
...
TL;DR: MIC-BEV是一个基于Transformer的鸟瞰图感知框架，用于基础设施多摄像头3D物体检测，支持异构摄像头配置并在传感器退化下保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于摄像头的检测模型在基础设施场景中表现不佳，面临多视角设置、不同摄像头配置、视觉输入退化以及各种道路布局等挑战。

Method: 提出图增强融合模块，利用摄像头与BEV单元之间的几何关系和潜在视觉线索，将多视角图像特征集成到BEV空间中。

Result: 在M2I合成数据集和真实世界数据集RoScenes上的广泛实验表明，MIC-BEV在3D物体检测方面达到了最先进的性能，并在极端天气和传感器退化等挑战条件下保持鲁棒性。

Conclusion: MIC-BEV展示了在实际部署中的潜力，其数据集和源代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 探索视觉变换器是否能够自然涌现出物体绑定能力，即识别哪些图像块属于同一物体的能力，以及这种能力如何影响模型性能。

Method: 使用相似性探针从ViT各层的图像块嵌入中解码IsSameObject属性，比较不同预训练目标（自监督vs监督）下该能力的表现，并通过消融实验验证其功能。

Result: 自监督ViTs（DINO、MAE、CLIP）能够可靠地涌现物体绑定能力，准确率超过90%，而ImageNet监督模型表现明显较弱；IsSameObject信号编码在物体特征的低维子空间中，并主动指导注意力。

Conclusion: 物体绑定能力不是ViT架构的简单产物，而是通过特定预训练目标获得的能力，这种涌现的符号知识自然服务于预训练目标，挑战了ViTs缺乏物体绑定的观点。
...
TL;DR: ProMoE是一个专门为视觉任务设计的MoE框架，通过两步路由机制（条件路由和原型路由）解决视觉token的空间冗余和功能异质性问题，在图像生成任务中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE方法在语言模型中表现优异，但在视觉任务（特别是Diffusion Transformers）中效果有限。作者认为这是因为语言token语义密集且变化明显，而视觉token存在空间冗余和功能异质性，阻碍了专家专业化。

Method: 提出ProMoE框架，包含两步路由器：1）条件路由根据功能角色将图像token划分为条件集和无条件集；2）原型路由基于可学习原型通过语义内容精炼条件token的分配。还提出路由对比损失来增强原型路由过程。

Result: 在ImageNet基准测试上的广泛实验表明，ProMoE在Rectified Flow和DDPM训练目标下均超越了最先进的方法。

Conclusion: ProMoE通过明确的路由指导和语义引导机制，有效解决了视觉MoE中的专家专业化问题，为视觉任务的MoE应用提供了新的解决方案。
...
Conclusion: URSA是一个简单而强大的框架，成功弥合了离散方法与连续方法在可扩展视频生成方面的差距，同时需要更少的推理步骤。
...
TL;DR: 提出生成式视图缝合（GVS）方法，通过并行采样整个序列来解决自回归视频扩散模型在相机引导视频生成中的碰撞问题，实现与预定义相机轨迹的忠实对应。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能够进行长序列生成且保持历史一致性，但无法利用未来信息指导当前生成，在预定义相机轨迹的场景中容易导致碰撞和生成崩溃。

Method: 基于扩散缝合技术开发采样算法，兼容任何使用扩散强制训练的现成视频模型；引入全向引导技术增强时间一致性，并提出环闭合机制实现长程连贯性。

Result: GVS实现了稳定、无碰撞、帧间一致且能闭合循环的相机引导视频生成，适用于各种预定义相机路径，包括奥斯卡·路透斯瓦德的不可能阶梯。

Conclusion: GVS方法有效解决了自回归模型在相机引导视频生成中的局限性，实现了高质量、连贯的视频生成效果。
...
TL;DR: 本文系统评估了不同类型记忆增强方法在长上下文对话任务中的效果，发现记忆增强方法能减少90%以上的token使用同时保持竞争力准确率，且记忆架构复杂度应与模型能力相匹配。


<details>
  <summary>Details</summary>
Motivation: 为了让大语言模型实现真正的对话连续性和从经验学习中受益，需要记忆系统。目前研究聚焦于复杂记忆系统的开发，但尚不清楚哪种记忆类型对长上下文对话任务最有效。

Method: 使用LoCoMo基准（包含标注问答任务的合成长上下文对话）系统评估了四种记忆增强方法：全上下文提示、通过检索增强生成的语义记忆和智能体记忆、通过上下文学习的片段记忆、通过提示优化的程序记忆。

Result: 记忆增强方法减少了90%以上的token使用，同时保持竞争力准确率。小型基础模型从RAG中获益最多，而强大的指令调优推理模型则从通过反思的片段学习和更复杂的智能体语义记忆中获益。片段记忆能帮助LLM认识到自身知识的局限性。

Conclusion: 记忆架构的复杂度应与模型能力相匹配，不同类型的记忆系统适用于不同能力的模型，记忆增强方法能显著提高效率同时保持性能。
...
TL;DR: BitSkip是一个混合架构框架，研究发现简单的8位量化模型（BitSkip-V1）在性能上优于更复杂的4位和Hadamard增强模型，并能与全精度基线竞争。Hadamard变换在8位精度下会导致性能灾难性下降。
...
Conclusion: 简单的8位量化模型比复杂方法表现更好，Hadamard变换在量化设置中会导致训练不稳定性，BitSkip-V1展示了优越的提前退出特性。
...
TL;DR: 该研究评估了大语言模型处理文化相关语言的能力，特别是理解和使用体现文化细微差别的比喻表达。结果显示模型在阿拉伯语谚语和埃及阿拉伯语习语上的表现明显低于英语谚语，且在语用使用和内涵理解方面存在显著挑战。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型处理文化根基语言的能力，特别是理解和使用体现当地知识和文化细微差别的比喻表达，以比喻语言作为文化推理能力的诊断工具。

Method: 设计评估任务包括语境理解、语用使用和内涵解释，在阿拉伯语和英语中评估22个开源和闭源大语言模型，使用埃及阿拉伯语习语、多方言阿拉伯语谚语和英语谚语作为测试材料。

Result: 结果显示：阿拉伯语谚语平均准确率比英语谚语低4.29%，埃及习语比阿拉伯语谚语低10.28%；语用使用任务准确率比理解任务下降14.07%，但提供语境习语句子可提高准确率10.66%；模型在内涵理解方面最多达到85.58%与人类标注者的一致性。
...
TL;DR: 本文研究了自然对话中美国手语(ASL)的变异性，特别是STEM教育场景中的动态适应特征，发现对话中的手势比孤立手势短24.6%-44.6%，并开发了评估手语嵌入模型的方法。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型主要基于翻译或孤立词汇数据，忽略了自然对话中的变异性。人类交流通过时空变化和发音风格动态适应语境和对话者，这在教育场景中尤其明显。

Method: 收集ASL STEM对话的运动捕捉数据集，使用连续运动学特征分析对话特有的趋同现象与个体努力减少，比较互动对话、单独讲座和翻译文章的差异。

Result: 对话手势平均比孤立手势短24.6%-44.6%，在独白语境中不存在的显著减少在对话中出现，STEM术语的重复提及显示时空变化。

Conclusion: 研究连接了语言分析和计算建模，理解语用学如何塑造手语发音及其在手语技术中的表示，为开发更自然的手语技术提供了基础。
...
TL;DR: CRADLE BENCH是一个用于心理健康危机检测的基准测试，涵盖7种临床标准定义的危机类型，包含600个临床医生标注的评估样本和420个开发样本，以及约4K个通过多语言模型多数投票自动标注的训练样本。


<details>
  <summary>Details</summary>
Motivation: 检测心理健康危机情况（如自杀意念、强奸、家庭暴力、儿童虐待和性骚扰）是语言模型面临的关键但未被充分探索的挑战，因为未能可靠地标记这些情况可能带来严重后果。

Method: 构建多维度危机检测基准，包含临床医生标注的评估样本和开发样本，以及通过多语言模型多数投票自动标注的训练样本。进一步在基于共识和一致同意定义的子集上微调六个危机检测模型。

Result: 多模型多数投票标注显著优于单模型标注。提供了在不同一致性标准下训练的互补模型。

Conclusion: CRADLE BENCH是首个纳入时间标签的危机检测基准，为可靠的心理健康危机检测提供了全面的评估框架和训练资源。
...
TL;DR: 提出了一种轻量级框架，无需微调即可将冻结的大型语言模型应用于纵向临床数据分析，在神经心理学评估中表现出色，有望用于阿尔茨海默病早期监测。


<details>
  <summary>Details</summary>
Motivation: 探索如何在不进行模型微调的情况下，利用冻结的大型语言模型来分析纵向临床数据，特别是针对阿尔茨海默病等神经退行性疾病的早期监测需求。

Method: 开发了一个轻量级框架，在语言模型空间中整合患者历史数据和上下文信息，生成准确的预测结果，无需对预训练模型进行参数调整。

Result: 在神经心理学评估任务中，该方法实现了准确可靠的性能表现，即使在训练数据量极少的情况下也能保持良好效果。

Conclusion: 该框架展示了在临床数据分析中有效利用冻结语言模型的潜力，为阿尔茨海默病等疾病的早期监测提供了有前景的技术方案。
...
TL;DR: 本文介绍了AfriMTEB基准测试和AfriE5模型，旨在解决非洲语言在文本嵌入任务中的代表性不足问题。AfriMTEB扩展了MMTEB，涵盖59种非洲语言、14个任务和38个数据集；AfriE5通过跨语言对比蒸馏适配mE5模型，在非洲语言上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决非洲语言在文本嵌入任务中的代表性不足问题，现有基准测试如MMTEB中非洲语言代表性不足，任务多从翻译基准转换而来，缺乏针对非洲语言特点的专门任务。

Method: 1) 构建AfriMTEB基准测试：扩展MMTEB，新增6个数据集，涵盖14-56种非洲语言，引入仇恨言论检测、意图检测、情感分类等新任务；2) 开发AfriE5模型：通过跨语言对比蒸馏方法适配mE5模型到非洲语言。

Result: AfriE5模型在评估中表现出色，超越了Gemini-Embeddings和mE5等强基线模型，达到了最先进的性能水平。

Conclusion: AfriMTEB基准测试填补了非洲语言在文本嵌入评估中的空白，AfriE5模型为非洲语言提供了高质量的文本嵌入解决方案，对防止LLM幻觉等NLP任务具有重要意义。
...
TL;DR: 本文提出了一个新颖的增强框架，通过三个即插即用步骤来评估大型语言模型的公平性，发现LLMs对输入扰动很敏感，更容易表现出刻板印象偏见，特别是对研究较少的社区。


<details>
  <summary>Details</summary>
Motivation: 尽管已有方法尝试在决策中避免使用刻板印象信息，但最近研究表明偏见对齐方法很脆弱。大型语言模型因其训练数据的判别性质而表现出刻板印象偏见。

Method: 提出了一个通用的增强框架，包含三个即插即用步骤，适用于多个公平性评估基准。将该框架应用于BBQ公平性评估数据集，测试LLMs对输入扰动的敏感性。

Result: 发现包括最先进的开源和闭源模型在内的LLMs都对输入扰动很敏感，更可能表现出刻板印象行为。模型对研究较少的社区更容易产生偏见行为。

Conclusion: 需要将公平性和安全性研究扩展到更多样化的社区，以解决LLMs对输入扰动的脆弱性和对不同社区的偏见差异问题。
...
TL;DR: 提出了一种基于智能体的自动化声明匹配方法，使用指令遵循的LLMs构建两步流水线：首先生成提示，然后进行二元分类的声明匹配。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在声明匹配任务中的潜力，特别是通过自动生成提示来超越人工设计的提示，并研究不同规模LLMs在生成过程中的表现差异。

Method: 采用两步流水线方法：第一步使用LLM生成提示，第二步使用LLM进行声明匹配的二元分类任务。支持使用不同LLM分别处理两个步骤。

Result: LLM生成的提示可以超越最先进的人工生成提示；较小LLM在生成过程中表现与较大LLM相当，可节省计算资源；使用不同LLM分别处理两个步骤有效。

Conclusion: 该方法展示了LLMs在声明匹配任务中的有效性，为理解LLMs对声明匹配任务的理解提供了新见解，并提供了计算资源优化的可能性。
...
TL;DR: 提出了一种无需训练的自适应提示级联方法，用于评估电商产品质量，显著减少专家工作量并提升性能


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在复杂工业目录中难以将通用语言理解与领域特定知识结合的问题，减少对训练数据和模型微调的依赖

Method: 从人工编写的种子提示开始，通过级联方式自动生成和优化提示，逐步满足目录特定需求，无需训练标签或模型微调

Result: 相比传统思维链提示，精确度和召回率提升8-10%，专家工作量从每属性5.1小时减少到3分钟（减少99%），在5种语言和多个质量评估任务中保持性能增益

Conclusion: 该自适应提示级联方法有效弥合了通用语言理解与领域特定知识之间的差距，在工业规模应用中实现了高效且可扩展的产品质量评估
...
TL;DR: 提出了一种基于连接组信息的LLM框架，将动态fMRI连接性编码为时间序列，应用稳健归一化，并将这些数据映射到适合冻结预训练LLM的表示形式，用于临床预测。在早期阿尔茨海默病检测中，该方法实现了敏感预测，错误率远低于临床认可范围。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够利用动态fMRI连接性数据进行临床预测的框架，特别是针对早期阿尔茨海默病的检测，以实现及时干预。

Method: 使用连接组信息的LLM框架，将动态fMRI连接性编码为时间序列，应用稳健归一化处理，并将数据映射到适合冻结预训练LLM的表示形式。

Result: 在早期阿尔茨海默病检测中实现了敏感预测，错误率远低于临床认可范围。

Conclusion: 该方法在早期阿尔茨海默病检测方面表现出色，具有重要的临床意义，为及时干预提供了可能性。
...
TL;DR: 本文研究了多语言大语言模型在遗忘学习过程中出现的语言混淆问题，指出仅使用英语数据进行知识擦除在多语言LLMs中不足，并提出N-Mix评分来量化这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示仅用英语数据擦除多语言知识在多语言LLMs中不够充分，但分析过于性能导向。本文从评估角度出发，发现在多语言LLMs使用并行多语言数据集进行全微调后遗忘学习时会出现语言混淆现象，这导致基于参考的评估指标失效。

Method: 采用三步方法：(1) 引入基于N-gram的语言混合(N-Mix)评分来量化语言混淆现象；(2) 证明当N-Mix评分高时，基于参考的指标会产生假阴性；(3) 提出需要能够直接评估生成句子内容的新型遗忘学习评估方法，称为基于语义的指标。

Result: 研究发现语言混淆在多语言LLMs中普遍且一致存在，N-Mix评分能够有效量化这一现象，并且当N-Mix评分高时，传统的基于参考的评估指标会失效。

Conclusion: 多语言LLMs的遗忘学习评估需要新的基于语义的指标，因为语言混淆现象使得标准参考指标不可靠，直接评估生成内容语义的方法更为有效。
...
TL;DR: 提出了M-Eval方法，通过异质性分析检测RAG医疗问答系统中的事实错误，提高回答准确性


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成的医疗问答系统存在生成错误信息和无法正确使用外部知识的问题，需要提高系统可靠性

Method: 从外部知识库提取额外医学文献，结合RAG系统生成的证据文档，使用基于循证医学的异质性分析方法验证证据支持度

Result: 在各种LLM上实现了高达23.31%的准确率提升

Conclusion: 该方法能有效检测RAG医疗系统中的错误，提高LLM应用的可靠性并减少诊断错误
...
TL;DR: 本文提出了PICOs-RAG方法，通过将用户查询扩展和规范化成专业格式，使用EBM中的PICO格式提取关键信息，显著提高了证据检索的效率和相关性，相比基线方法提升了8.8%。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成(RAG)的方法在处理真实临床场景中的复杂查询时存在困难，当查询缺乏某些信息或使用不精确语言时，模型可能检索到无关证据并生成无用的回答。

Method: 提出PICOs-RAG方法，将用户查询扩展和规范化成专业格式，使用EBM中的PICO格式提取最重要的信息用于检索。
...
TL;DR: 提出了一种基于元分析思想的新方法，通过可靠性分析、异质性分析和外推分析来重新排序和过滤医学证据，以提高基于检索增强生成（RAG）的循证医学任务中证据质量，实验显示准确率提升达11.4%。


<details>
  <summary>Details</summary>
Motivation: 循证医学对证据质量有严格要求，但现有RAG方法在EBM任务中难以有效区分高质量证据，导致误诊风险增加。

Method: 结合多种EBM方法模拟元分析过程，包括可靠性分析、异质性分析和外推分析，通过这些流程筛选最佳医学证据供LLMs诊断使用。

Result: 实验结果显示准确率提升达11.4%，成功从PubMed数据集中提取更高质量和更可靠的证据。

Conclusion: 该方法能够减少错误知识注入到回答中，帮助用户获得更有效的回复，提高了RAG在循证医学应用中的效果。
...
TL;DR: 提出了一种基于细粒度语义置信度奖励的强化学习框架，通过语义聚类引导LLMs在超出知识范围时拒绝回答，显著提升了模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用粗粒度信号（如整体置信度）指导LLMs拒绝回答超出知识范围的问题，但这种方法可能导致模型对自身知识边界认知不精确。

Method: 基于细粒度语义置信度奖励的强化学习框架，通过采样多个候选答案并进行语义聚类，训练LLM保留高置信度聚类中的答案，丢弃低置信度聚类中的答案。

Result: 该方法在领域内和领域外基准测试中显著提升了可靠性。

Conclusion: 提出的细粒度语义置信度奖励方法能够有效提升LLMs的可靠性，特别是在处理超出知识范围的问题时表现出色。
...
TL;DR: 提出Speculative Knowledge Distillation (SpecKD)，一种基于推测解码思想的知识蒸馏框架，通过动态令牌级门控机制选择性应用蒸馏损失，避免学生学习教师不确定的预测。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法对所有令牌统一应用蒸馏损失，不考虑教师模型的置信度，这可能导致学生模型学习教师的不确定或高熵预测，从而引入噪声并损害学生性能。

Method: 引入动态令牌级门控机制，基于推测解码的'提议-验证'范式：在每个步骤中验证学生的令牌提议与教师分布的一致性，仅对'接受'的令牌应用蒸馏损失，'拒绝'的令牌被屏蔽。

Result: 在多种文本生成任务上的广泛实验表明，SpecKD持续显著优于强基线方法，带来更稳定的训练和更强大的学生模型，并达到最先进的结果。

Conclusion: SpecKD是一种即插即用的知识蒸馏框架，通过选择性蒸馏机制有效提升学生模型性能，在文本生成任务中表现出色。
...
TL;DR: 该论文提出了一种训练大型多模态模型形成语言约定的方法，通过在模型间进行模拟参考游戏，无需额外人工数据，使模型能够与人类进行高效沟通，减少消息长度达41%同时提高成功率15%。


<details>
  <summary>Details</summary>
Motivation: 人类通过共享对话上下文形成临时语言约定来实现高效沟通，作者希望让大型多模态模型也能具备这种能力，实现与人类更有效的沟通。

Method: 使用模拟参考游戏在模型间进行训练，无需额外人工数据，通过重复的参考游戏涉及照片和七巧板图像来训练模型形成约定。

Result: 模型与人类沟通时消息长度减少41%，成功率提高15%，人类听众与形成约定的模型交互时响应更快。实验还表明仅基于成功或成本训练都不足以引发约定形成。

Conclusion: 通过结合成功和成本的双重训练目标，可以有效地训练大型多模态模型形成语言约定，实现与人类的高效沟通。
...
Method: 将传统生成循环分解为细粒度服务处理器，通过API暴露；使用用户提供的inferlets程序控制生成过程；采用WebAssembly执行inferlets以利用其轻量级沙盒特性。

Result: 在标准任务上性能与最先进系统相当（延迟开销3-12%），在智能体工作流程上显著改善延迟和吞吐量（提高1.3x-3.4x）。

Conclusion: Pie系统通过可编程性实现了LLM服务的灵活性和效率，支持应用特定的优化，为复杂LLM应用提供了更好的服务支持。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译方面取得进展，但仍容易产生幻觉。现有的机器翻译基准测试无法充分暴露多语言LLM的失败情况，因此需要一个新的诊断框架来揭示这些幻觉问题。

Method: 提出了一个诊断框架，将幻觉分为指令脱离和源文脱离两类。通过使用4个前沿LLM生成候选翻译，然后使用LLM评判器集合和专家验证来筛选，构建了包含11个英语到其他语言方向的HalloMTBench基准测试。

Result: 评估了17个LLM在HalloMTBench上的表现，发现了独特的"幻觉触发器"，包括模型规模、源文长度敏感性、语言偏见以及强化学习放大的语言混合等失败模式。
...
TL;DR: Global PIQA是一个覆盖100多种语言的文化特定常识推理基准，由全球335名研究人员手工构建，发现当前LLMs在低资源语言上表现较弱，存在高达37%的准确率差距。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏覆盖大量语言和文化的文化特定评估基准，需要构建一个能够反映全球语言和文化多样性的常识推理基准。

Method: 通过全球335名研究人员来自65个国家的参与，手工构建了覆盖116种语言变体的常识推理基准，其中超过50%的示例涉及当地食物、习俗、传统等文化特定元素。

Result: 最先进的LLMs在总体上表现良好，但在低资源语言上表现较弱（准确率差距高达37%，随机概率为50%），开源模型通常表现不如专有模型。

Conclusion: Global PIQA显示在许多语言和文化中，日常知识仍然是需要改进的领域，同时也展示了人类语言所嵌入的广泛文化多样性。
...
TL;DR: 该研究旨在记录和分析孟加拉语方言的语音和形态特征，并探索为区域变体构建计算模型（特别是自动语音识别系统）的可行性，以促进方言多样性的保护和孟加拉语社区包容性数字工具的发展。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语虽然在南亚和侨民社区广泛使用，具有丰富的方言多样性，但针对其方言的计算处理研究仍然有限。本研究旨在填补这一空白，通过系统研究促进语言技术的包容性发展。

Method: 研究通过记录和分析孟加拉语主要方言群（东部孟加拉语、Manbhumi、Rangpuri、Varendri和Rarhi）的语音和形态特性，构建专门的数据集，并探索为区域变体开发自动语音识别系统的可行性。

Result: 研究创建了一个公开可用的数据集，为后续计算模型开发提供了基础资源，并验证了为孟加拉语方言构建专门ASR系统的技术可行性。

Conclusion: 该研究为孟加拉语方言的计算处理开辟了新途径，有助于保护语言多样性并推动包容性语言技术的发展，所创建的数据集将为未来研究提供重要支持。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Text-to-SQL技术发展迅速，但由于集成工具有限，在实际系统中部署仍面临挑战。需要将研究进展与实际应用相结合。

Method: 首先建立通用执行范式来标准化调用接口，然后基于七个抽象的有效原子角色组件提出多角色协作机制。

Result: 在广泛采用的基准测试中，协作工作流持续优于原始个体方法。

Conclusion: Squrve为处理复杂现实世界查询开辟了新的有效途径。
...
TL;DR: 本文提出了两种改进的文本过滤方法：模式感知的行级去重（PLD）和模式感知的尾随标点过滤（PTF），通过考虑行级信号的序列分布来保留结构重要内容，相比传统方法在语言模型训练中取得了更好的性能。
...
Method: 提出了模式感知的行级去重（PLD）和模式感知的尾随标点过滤（PTF），不仅考虑行级信号，还考虑它们在文档中的序列分布。

Result: 在英语和韩语的小型语言模型（10亿参数）训练中，这些方法在多项选择基准上持续提升性能，并在SQuAD v1和KorQuAD v1的生成式问答准确性上显著提高。

Conclusion: 通过增强传统过滤技术，考虑行级信号的序列分布，能够保留结构重要内容，从而改善语言模型的训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 构建首个专门评估韩语长篇叙事中多步软推理能力的基准，最小化数据污染，为韩语NLP发展提供系统评估基础。

Method: 遵循MuSR框架构建，包含完全韩语的叙事、推理链和多选题，由人工标注者验证逻辑一致性和可回答性，评估了四种大语言模型并设计了结合少量示例、推理轨迹和任务特定提示的提示策略。
...
TL;DR: 本文介绍了MuSaG，这是首个德语多模态讽刺检测数据集，包含来自德国电视节目的33分钟手动选择和人工标注的语句，涵盖文本、音频和视频模态。作者评估了9个开源和商业模型，发现人类在对话环境中主要依赖音频线索，而模型在文本上表现最佳，揭示了当前多模态模型在现实场景中的差距。


<details>
  <summary>Details</summary>
Motivation: 讽刺是一种复杂的比喻语言形式，其字面意义与真实意图相矛盾。随着多模态大语言模型的出现，讽刺检测需要整合音频和视觉线索，而现有的德语多模态讽刺检测数据集缺乏，因此需要创建专门的数据集来支持相关研究。

Method: 构建MuSaG数据集，包含33分钟的德语电视节目语句，提供对齐的文本、音频和视频模态，分别由人工标注。评估9个开源和商业模型，涵盖文本、音频、视觉和多模态架构，并与人类标注进行比较。

Result: 人类在对话环境中主要依赖音频线索进行讽刺检测，而模型在文本模态上表现最佳。这揭示了当前多模态模型在整合音频和视觉信息方面的不足，与现实场景存在差距。

Conclusion: MuSaG数据集支持多模态讽刺检测和人类-模型对齐的未来研究。当前多模态模型需要改进以更好地适应现实场景，特别是在整合音频线索方面。
...
TL;DR: 该研究探讨了外部知识整合对自然语言生成的影响，通过创建KITGI基准测试，比较了在完整外部知识和过滤知识条件下使用T5-Large模型生成句子的效果。
...
Method: 采用三阶段方法：(1)识别并移除关键知识，(2)重新生成句子，(3)手动评估输出的常识合理性和概念覆盖度。

Result: 使用完整知识生成的句子在正确性方面达到91%，而过滤知识后性能急剧下降至6%。

Conclusion: 相关的外部知识对于保持自然语言生成中的连贯性和概念覆盖至关重要，强调了设计可解释的知识增强NLG系统的重要性。
...
TL;DR: 该论文提出了一个基于知识和确定性两个维度的LLM幻觉分类框架，通过模型特定的数据集构建过程来区分不同类型的幻觉，并验证了基于知识轴的分类有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常基于幻觉的外部特性而非LLM内部特性进行分类，这种外部焦点忽视了不同幻觉可能需要基于其底层机制采取针对性缓解策略。

Method: 提出基于知识和确定性两个轴的幻觉分类框架，包括模型特定的数据集构建过程；使用转向缓解方法验证知识轴分类；引入新评估指标衡量缓解方法在关键幻觉子集上的效果。

Result: 验证了知识轴分类的有效性，显示两种幻觉类型存在显著差异；发现不同模型具有不同的知识和幻觉模式；揭示某些缓解方法在平均表现良好但在关键幻觉子集上表现不佳。
...
TL;DR: 该研究探讨了如何通过推理和训练选择来提高大语言模型在医疗等敏感领域解释的忠实性，发现少样本示例的数量和质量、提示设计以及指令调优阶段都对模型忠实性有显著影响。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生不能忠实反映其预测驱动因素的解释，在医疗等敏感领域，这种不忠实性尤为危险，可能削弱临床医生的信任并导致不安全的决策支持。
...
Conclusion: 这些发现为在敏感领域增强LLM的可解释性和可信度提供了策略见解。
...
TL;DR: 本文提出了一种结合文本和语音信息的多模态方法来解决阿拉伯语方言的变音符号恢复任务，使用CATT预训练模型的编码器处理文本，OpenAI Whisper基础模型处理语音，通过两种融合策略实现，在测试集上取得了0.55的WER和0.13的CER。
...
Method: 使用CATT预训练模型的编码器处理文本模态，OpenAI Whisper基础模型的编码器处理语音模态。采用两种融合策略：早期融合（将语音token与文本token合并）和交叉注意力融合。在训练中随机禁用语音输入以增强模型鲁棒性。

Result: 在开发集上获得0.25的WER和0.9的CER；在测试集上获得0.55的WER和0.13的CER。

Conclusion: 提出的多模态方法在阿拉伯语方言变音符号恢复任务中表现良好，证明了结合语音信息能够有效提升模型性能，且模型在有无语音输入的情况下都能稳定工作。
...
TL;DR: 本研究评估了5种大语言模型生成挪威语儿童对话的能力，发现这些主要基于成人对话训练的模型在生成真实、适合儿童年龄的对话方面存在挑战，特别是在低资源语言环境中。


<details>
  <summary>Details</summary>
Motivation: 大语言模型主要基于成人对话数据训练，在生成真实儿童对话方面面临挑战，特别是在低资源语言如挪威语中，缺乏适合儿童年龄的词汇资源。

Method: 通过盲评研究，让11位教育专业人士评估5种LLM（GPT-4、RUTER-LLAMA-2-13b、GPTSW、NorMistral-7b、NorBloom-7b）生成的5岁和9岁挪威儿童对话样本，与真实儿童访谈数据对比。

Result: 评估者表现出强评分者间信度（ICC=0.75），对5岁儿童的年龄预测准确率高于9岁儿童。GPT-4和NorBloom-7b表现相对较好，但大多数模型生成的语言被认为比目标年龄组更复杂。

Conclusion: 研究强调了为涉及儿童的专业应用开发LLM系统时面临的关键数据相关挑战，特别是在低资源语言环境中，全面适合年龄的词汇资源稀缺。
...
TL;DR: 提出MERGE方法自动生成自然语言推理问题的变体，通过替换开放类词汇但保留推理逻辑来评估模型的泛化能力，发现模型在变体上的性能下降4-20%。


<details>
  <summary>Details</summary>
Motivation: 现有方法手动创建泛化基准成本高昂，自动生成高质量变体困难，需要一种能自动生成保留推理逻辑的NLI问题变体的方法。

Method: 提出MERGE方法，通过替换开放类词汇但保持底层推理不变，自动生成NLI问题的变体来测试模型泛化能力。

Result: NLI模型在变体上的性能比原始问题下降4-20%，表明即使在最小化修改的问题上泛化能力也较低。

Conclusion: MERGE方法能有效评估NLI模型的泛化能力，揭示了模型在推理保留变体上的脆弱性，为模型评估提供了新工具。
...
TL;DR: 提出了一种名为LATR的新颖rollout策略，通过在高不确定性生成步骤进行分支、前瞻模拟和剪枝来显式促进轨迹级多样性，解决了RLVR中轨迹多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习与可验证奖励（RLVR）流水线中的关键瓶颈在于组rollout期间采样轨迹的多样性有限。同质化轨迹及其相关奖励会削弱策略更新的回报信号，从而阻碍有效的策略学习。

Method: LATR迭代地在三个阶段操作：(1)在高不确定性生成步骤进行分支，(2)对每个新分支执行前瞻模拟，(3)在模拟过程中剪枝表现出持续相似性的分支。

Result: 与随机采样相比，LATR平均加速策略学习131%，并在不同推理任务上将GRPO和DAPO算法的最终pass@1性能提高4.2%。
...
TL;DR: Critique-RL是一种无需强监督的在线强化学习方法，通过两阶段优化策略训练批判语言模型，在提升批判模型判别能力的同时增强其帮助性，从而显著提高复杂推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的批判语言模型方法通常依赖强监督者来标注批判数据，这限制了其应用范围。为了解决这个问题，作者提出了无需强监督的Critique-RL方法。

Method: 采用两玩家范式：演员生成响应，批判者提供反馈，演员根据反馈优化响应。使用两阶段优化策略：第一阶段通过基于规则的直接奖励信号增强批判者的判别能力；第二阶段引入基于演员优化的间接奖励来提升批判者的帮助性，同时通过正则化保持判别能力。

Result: 在各种任务和模型上的广泛实验表明，Critique-RL带来了显著的性能提升。例如，Qwen2.5-7B模型在领域内任务上获得9.02%的提升，在领域外任务上获得5.70%的提升。

Conclusion: Critique-RL展示了无需强监督开发批判语言模型的潜力，通过两阶段优化策略有效平衡了批判者的判别能力和帮助性，为复杂推理任务的改进提供了有前景的解决方案。
...
TL;DR: 该研究提出了一种综合方法，将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，并转换为开放式问题，用于评估LLM在文化基础和方言内容上的表现。研究发现模型在阿拉伯语方言上表现不佳，阿拉伯语中心模型在开放式问题上表现较差，而思维链方法能提高判断正确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在日常问题回答中应用日益广泛，但在文化基础和方言内容上的表现存在语言间差异，需要系统评估模型在多元语言变体上的能力。

Method: 将现代标准阿拉伯语多选题翻译成英语和多种阿拉伯语方言，转换为开放式问题，在零样本和微调设置下对多种LLM进行基准测试，并生成思维链推理来微调模型进行逐步推理。

Result: 模型在阿拉伯语方言上表现不佳，显示在文化基础和方言特定知识方面存在持续差距；阿拉伯语中心模型在多选题上表现良好但在开放式问题上表现困难；思维链方法提高了判断正确性但n-gram指标结果不一。

Conclusion: 开发的数据集将公开发布，支持文化和语言包容性评估的进一步研究，揭示了LLM在方言和文化内容理解方面的局限性。
...
TL;DR: LongWeave是一个新的长文本生成基准，通过约束验证评估（CoV-Eval）平衡真实性和可验证性，评估显示即使最先进的LLM在长文本生成中仍面临显著挑战。


<details>
  <summary>Details</summary>
Motivation: 现有长文本生成基准要么使用难以验证的真实世界查询，要么使用忽略现实复杂性的合成设置，需要一种平衡真实性和可验证性的评估方法。

Method: 提出LongWeave基准和CoV-Eval方法，通过定义可验证目标，系统生成相应查询、文本材料和约束，支持自定义输入/输出长度（最高64K/8K token）和七种任务类型。

Result: 对23个LLM的评估表明，随着真实世界复杂性和输出长度的增加，即使最先进的模型在长文本生成中也面临显著挑战。

Conclusion: LongWeave为长文本生成提供了更现实的评估基准，揭示了当前LLM在复杂长文本生成任务中的局限性。
...
TL;DR: COMPEFFDIST是一个用于情感分析的综合高效蒸馏框架，通过属性自动指令构建和难度数据过滤解决现有方法中指令多样性不足和数据成本高的问题，使3B学生模型在大多数任务上达到20倍大教师模型的性能水平，且仅需10%数据即可达到相同性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识蒸馏的情感分析方法面临两个关键挑战：(1)人工编写指令在多样性和数量上有限，无法确保蒸馏知识的全面覆盖；(2)大规模用户文本计算成本高，影响方法的实用性。

Method: 提出COMPEFFDIST框架，包含两个关键模块：基于属性的自动指令构建和基于难度的数据过滤，分别解决指令多样性不足和数据成本高的问题。

Result: 在多个模型系列（Llama-3、Qwen-3和Gemma-3）上应用该方法，使3B学生模型在大多数任务上匹配20倍大教师模型的性能，且在数据效率上大幅优于基线方法，仅需10%数据即可达到相同性能水平。
...
TL;DR: SynthWorlds框架通过构建结构相同但知识内容不同的真实映射世界和合成映射世界，来区分语言模型的推理能力和事实记忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以清晰区分语言模型的推理能力与事实记忆能力，因为基准测试表现往往反映的是事实回忆而非真正的推理。

Method: 构建平行语料库，包含真实映射世界（可利用参数知识）和合成映射世界（参数知识无意义），并在其上设计多跳问答和页面导航两个镜像任务。

Result: 实验发现在仅参数和知识增强的LM设置中，存在持续的知识优势差距，即模型从记忆的参数世界知识中获得的性能提升。知识获取和整合机制可以减少但无法消除这一差距。

Conclusion: SynthWorlds提供了一个受控环境，能够以前难以实现的方式评估语言模型，实现推理和记忆能力的精确可测试比较。
...
TL;DR: 本文介绍了LuxIT——一个针对卢森堡语的新型单语指令调优数据集，旨在解决低资源语言环境下高质量训练数据不足的问题。通过使用DeepSeek-R1-0528从原生卢森堡语文本合成数据集，并采用LLM作为评判的质量保证流程。实验结果显示，在不同模型上的性能表现存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言环境中，指令调优大语言模型的有效性常常受到高质量训练数据缺乏的限制。卢森堡语作为低资源语言，缺乏专门的指令调优数据集，这限制了相关自然语言处理应用的发展。

Method: 1. 从原生卢森堡语文本语料库合成LuxIT数据集
2. 选择在卢森堡语方面表现出色的DeepSeek-R1-0528进行数据生成
3. 采用LLM作为评判的质量保证流程
4. 在多个小规模LLM上对LuxIT进行微调
5. 在卢森堡语能力考试上对微调后的模型进行基准测试

Result: 在卢森堡语能力考试上的基准测试结果好坏参半，不同模型的性能表现存在显著差异。虽然LuxIT数据集为卢森堡语自然语言处理做出了重要贡献，但其应用效果因模型而异。

Conclusion: LuxIT为卢森堡语自然语言处理提供了关键贡献，并提供了一个可复制的单语方法学。然而，研究结果强调了需要进一步研究来优化其应用，特别是在不同模型上的性能差异表明需要更深入的分析和改进。
...
TL;DR: 本文提出了一种新的对抗性改写任务，通过生成语义等价但语法正确的文本改写来降低多模态大语言模型的推理分割性能，并开发了SPARTA方法在文本自编码器的语义潜在空间中进行优化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注图像输入的扰动，而语义等价的文本改写在现实应用中至关重要，但尚未得到充分探索。本文旨在填补这一空白，评估推理分割模型对对抗性文本改写的鲁棒性。

Method: 提出了SPARTA方法——一种黑盒、句子级优化方法，在文本自编码器的低维语义潜在空间中运行，通过强化学习进行指导，生成对抗性改写。

Result: SPARTA在ReasonSeg和LLMSeg-40k数据集上取得了显著更高的成功率，比先前方法高出最多2倍，成功揭示了先进推理分割模型在严格语义和语法约束下仍然容易受到对抗性改写攻击。

Conclusion: 多模态大语言模型在推理分割任务中对对抗性文本改写表现出脆弱性，即使语义和语法约束严格，模型性能仍会显著下降，这为未来研究提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力的不断提升，现有的基准测试主要针对英语，而在非英语语言方面的评估仍然较少被探索。本文旨在填补这一空白，特别是针对欧洲语言。

Method: 首先对近期LLM基准测试的发展进行了简要概述，然后提出了一个专门针对多语言或非英语使用场景的基准测试分类法。

Result: 提出了一套最佳实践和质量标准，建议提高评估方法的语言和文化敏感性。

Conclusion: 通过协调开发基准测试并提高语言和文化敏感性，可以更好地评估和推动多语言LLM的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法如LaMP和PGraphRAG在生成个性化文本时，往往在语调、主题或风格上产生漂移，需要更有效的个性化对齐机制。

Method: PerFine采用训练免费的批判-精炼框架，包含LLM生成器（基于检索配置文件生成草稿）和批判LLM（基于相同配置文件提供结构化反馈），通过迭代反馈和淘汰策略不断优化输出。

Result: 在Yelp、Goodreads和Amazon数据集上，PerFine相比PGraphRAG在GEval指标上提升7-13%，在3-5次精炼迭代中持续改进，且随着批判模型规模增大而扩展性良好。
...
<details>
  <summary>Details</summary>
Motivation: 在低资源语言、专业领域和受限部署环境中，大语言模型在有限数据下的微调面临实际挑战。虽然预训练模型提供了坚实基础，但在数据稀缺情况下需要专注且高效的微调技术。

Method: 系统回顾参数高效微调技术以降低训练和部署成本，领域和跨语言适应方法（适用于编码器和解码器模型），以及模型专业化策略。进一步研究使用有限人类或合成反馈指导模型行为的偏好对齐方法。

Result: 强调了经验权衡、选择标准和最佳实践，包括模型缩放、数据缩放以及减轻灾难性遗忘。
...
TL;DR: 该调查论文系统分析了RAG和推理增强如何协同缓解LLM幻觉问题，提出了基于知识和逻辑的幻觉分类，并展示了统一框架和实际应用。


<details>
  <summary>Details</summary>
Motivation: 幻觉是LLM可靠部署的主要障碍，RAG和推理增强是最有效的缓解方法，但它们的协同潜力和机制尚未得到系统研究。

Method: 采用应用导向的能力增强视角，提出区分知识型和逻辑型幻觉的分类法，系统分析RAG和推理如何解决各类幻觉，并建立统一框架。

Result: 展示了RAG、推理增强及其在智能体系统中的整合如何缓解幻觉，提供了实际应用案例、评估方法和基准测试。

Conclusion: RAG和推理增强的协同作用为平衡LLM创造性和可靠性提供了有效途径，系统研究揭示了它们缓解幻觉的机制和潜力。
...
<details>
  <summary>Details</summary>
Motivation: 科学演讲正成为传播研究的重要媒介，自动识别与演讲相关的基础性或丰富性文献对研究人员和学生具有重要价值。
...
Method: 提出自然语言批评作为解决方案，包括：分析批评对象（不确定性vs置信度）、提出自我批评方法和CritiCal批评校准训练方法，利用自然语言批评改进置信度校准。

Result: 实验表明CritiCal显著优于自我批评和其他竞争基线，在复杂推理任务中甚至超越了其教师模型GPT-4o，并在分布外设置中表现出稳健的泛化能力。

Conclusion: 自然语言批评能有效增强大语言模型的置信度校准，CritiCal方法在复杂任务中表现出色，提升了LLM的可靠性。
...
TL;DR: 本文提出了一种词性消歧方法，旨在实现零静默率目标，即在词性标注中从不丢弃单词的正确标签。该方法在INTEX系统中实现，强调需要验证转换器路径的交互作用，而不仅仅是单独考虑它们。


<details>
  <summary>Details</summary>
Motivation: 词性消歧是自然语言处理中的关键挑战，许多现有系统在文本存在词汇歧义时会产生多个解决方案。本文旨在实现零静默率目标，确保单词的正确词性标签永远不会被丢弃。

Method: 在Silberztein的INTEX系统中实现的词性消歧方法。该方法使用局部消歧语法和转换器，强调需要验证转换器路径之间的交互作用，而不仅仅是单独考虑它们。

Result: 研究发现，要验证局部消歧语法，不能单独考虑转换器路径，必须验证它们的交互作用。同样，当使用多个转换器组合时，结果不能通过单独考虑它们来预测。
...
TL;DR: 该论文介绍了开放韩语历史语料库，这是一个大规模、开放许可的数据集，涵盖1300年历史、6种语言和多种书写系统，包含1800万份文档和50亿个标记，用于量化分析韩语的主要语言变迁。


<details>
  <summary>Details</summary>
Motivation: 韩语历史存在口语和书面语形式之间的差异，以及从汉字到韩文的关键转变，但由于缺乏可访问的历史语料库，这种语言演变在NLP领域尚未得到充分探索。

Method: 构建开放韩语历史语料库，收集从7世纪到2025年的19个来源的数据，涵盖韩式汉字（Idu）和韩汉混合文字等书写系统。

Result: 量化分析显示：(1) Idu使用在1860年代达到顶峰后急剧下降；(2) 从汉字到韩文的转变从1890年左右开始快速发生；(3) 朝鲜的词汇分化导致现代分词器产生高达51倍的未登录词率。

Conclusion: 该工作为定量历时分析提供了基础资源，捕捉了韩语的历史，并可作为大型语言模型的预训练语料库，提高其对现代韩文中的汉韩词汇和古代书写系统的理解。
...
TL;DR: BEARD框架通过结合BEST-RQ目标和知识蒸馏，使用未标注数据对Whisper编码器进行领域自适应，在航空交通管制语音识别任务中显著提升性能。
...
Method: 提出BEARD框架，结合BEST-RQ目标和冻结教师编码器的知识蒸馏，使用未标注数据对Whisper编码器进行自适应，确保编码器与预训练解码器的互补性。

Result: 在ATCO2航空交通管制语料库上，使用约5000小时未标注语音和2小时标注语音，相比基线模型和微调模型，相对改进达到12%。

Conclusion: 这是首个使用自监督学习目标对Whisper进行领域自适应的工作，在挑战性的航空交通管制领域取得了显著性能提升。
...
TL;DR: 提出ReForm方法，通过将语义一致性评估整合到自动形式化过程中，实现迭代生成、评估和自修正，显著提升自然语言数学到机器可验证形式语句的转换质量。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在自动形式化任务中虽然能生成语法正确的形式语句，但经常无法保持原始问题的语义意图，缺乏人类专家自然采用的自反思和迭代优化机制。

Method: 提出ReForm反射式自动形式化方法，集成语义一致性评估，通过迭代生成形式语句、评估语义保真度并自修正错误。引入前瞻有界序列优化(PBSO)训练方法，在不同序列位置使用不同奖励，确保模型既准确自动形式化又正确进行语义验证。

Result: 在四个自动形式化基准测试中，ReForm比最强基线平均提升17.2个百分点。引入ConsistencyCheck基准(859个专家标注项)，验证LLMs作为评判者的可靠性，并揭示自动形式化固有难度：人类专家在高达38.5%的情况下也会产生语义错误。

Conclusion: ReForm通过反射式方法显著提升自动形式化性能，同时揭示了自动形式化任务的固有挑战性，即使人类专家也难以完全避免语义错误。
...
Method: 训练扩散LLM原生支持可变生成长度，通过准确预测[EOS]标记，使模型能够以块扩散方式推理，同时保持全局双向注意力和高并行性。

Result: 在标准基准测试中，相比传统dLLM推理范式实现30.1倍加速，相比Qwen和Llama等自回归模型实现2.4倍加速，同时达到更高准确率。

Conclusion: 该方法实现了更快的推理速度和更高的准确性，使扩散大语言模型超越学术新奇性，支持在实际应用中的实用化。
...
TL;DR: DHSA是一种动态分层稀疏注意力框架，通过数据驱动的方式在线预测注意力稀疏性，无需重新训练，在保持与密集注意力相同精度的同时，显著降低了长上下文LLM的预填充延迟和内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有静态稀疏方法（如滑动窗口或全局标记）由于静态性无法适应注意力中内容相关的变化，而先前动态方法依赖预定义模板或启发式机制，降低了通用性并可能修剪掉上下文重要的标记。

Method: DHSA自适应地将序列分割为变长块，通过长度归一化聚合计算块表示，然后将块级相似性分数上采样到标记级相似性，以计算重要性分数来确定应保留哪些标记级交互。

Result: 在Gemma2上的实验显示，DHSA在精度上与密集注意力相当，同时将预填充延迟降低20-60%，峰值内存使用降低35%。相比其他基线方法，DHSA实现了更高的准确率（6-18%相对增益）且成本相当或更低。
...
TL;DR: 本文对基于前缀的方法在零样本跨语言迁移中的应用进行了全面研究，发现这些方法在35+种语言上优于LoRA基准，特别是在低资源多语言环境中具有显著优势。


<details>
  <summary>Details</summary>
Motivation: 随着Llama和Mistral等大型语言模型的发布，零样本跨语言迁移变得越来越可行，但将这些仅解码器LLM适应到跨语言新任务仍然具有挑战性。基于前缀的参数高效微调技术在仅解码器模型中的零样本迁移应用研究较少。

Method: 使用三种基于前缀的方法（软提示调优、前缀调优和Llama Adapter）进行从英语到35+种高资源和低资源语言的零样本跨语言迁移研究，分析了跨语言家族和文字系统的迁移效果，以及模型规模从1B到24B的扩展影响。

Result: 使用Llama 3.1 8B时，前缀方法在Belebele基准上比LoRA基准高出6%；Mistral v0.3 7B也观察到类似改进。尽管前缀调优仅使用1.23M学习参数，但在多样化基准测试中实现了持续改进。
...
TL;DR: 本文介绍了相对缩放定律，通过分析255个在相同计算预算下训练的Transformer模型，发现缩放虽然提升整体性能，但不同测试分布之间的性能差距变化轨迹各异，缩放并非普遍均衡器。


<details>
  <summary>Details</summary>
Motivation: 传统的缩放定律基于聚合测试集评估，平均了异质子群体的表现，掩盖了性能差异。本文旨在研究不同测试分布之间的性能差距如何随模型规模变化。

Method: 使用255个解码器Transformer模型，在匹配计算预算（10^18-10^20 FLOPs）下训练，分析不同测试分布之间的相对性能差距随规模的变化轨迹。

Result: 发现多样化的变化轨迹：学术领域趋于均衡；地区英语方言变化取决于人口规模；AI风险行为集群分化，能力和影响力相关风险在预训练期间增加，而对抗性风险不变。

Conclusion: 缩放虽然改善整体性能，但不是普遍均衡器。作者发布所有模型检查点以支持进一步研究相对缩放定律与传统缩放定律，更好地优先处理鲁棒性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的奖励模型在知识密集型和长文本任务上表现不佳，因为评估正确性需要超出模型内部知识的基础，这限制了它们可靠区分细微质量差异的能力，特别是在需要外部证据时。
...
Result: 在三个新收集的数据集和两个广泛使用的基准测试上的广泛实验表明，OpenRM显著优于现有的奖励建模方法。将其集成到推理时响应选择和训练时数据选择中，在下游LLM对齐任务中带来一致增益。
...
TL;DR: 本研究通过眼动追踪分析发现，阅读障碍读者对词汇长度、频率和可预测性三个特征都表现出更强的敏感性，其中可预测性影响最大。通过反事实模拟，这些词汇特征可以解释约三分之一的阅读障碍-正常读者差距。


<details>
  <summary>Details</summary>
Motivation: 探究在自然阅读情境下，阅读障碍读者的额外阅读成本在何时、何种条件下产生，以及词汇特征如何影响这些成本。

Method: 使用眼动追踪技术，结合词汇层面的特征（词汇长度、频率和可预测性），建模这些特征对阅读障碍读者阅读时间的影响。

Result: 所有三个词汇特征都显著影响典型读者和阅读障碍读者的阅读时间，但阅读障碍读者对每个特征都表现出更强的敏感性，特别是对可预测性。反事实操作显示这些特征可以缩小约三分之一的阅读障碍-控制组差距。
...
TL;DR: R3是一个专为RAG优化的检索框架，通过试错反馈强化对比学习，无需标注数据即可动态优化检索器在RAG环境中的相关性。
...
Method: R3采用试错反馈强化对比学习，让检索器在RAG环境中动态探索和优化相关性，通过检索结果与环境的交互产生对比信号来自动指导检索器的自我改进。

Result: 在多样化任务上的广泛实验表明，R3将RAG性能比原始检索器提升5.2%，比最先进的检索器高出4.9%，同时达到与基于后训练或指令调优LLM的LLM增强检索和RAG系统相当的结果。
...
TL;DR: 本文提出了一种名为MQM重新标注的两阶段机器翻译评估方法，通过让标注者审查和编辑现有的MQM标注来提高评估质量。
...
Result: 重新标注行为符合预期目标，并且产生了更高质量的标注，主要原因是发现了第一轮标注中遗漏的错误。

Conclusion: MQM重新标注是一种有效的机器翻译评估方法改进，能够提高标注质量并减少评估噪声。
...
TL;DR: 本研究通过RPNA框架评估角色提示对大型语言模型医学推理能力的影响，发现角色提示主要影响语言风格而非推理能力，未能复制真实医学实践中的认知复杂性。


<details>
  <summary>Details</summary>
Motivation: 当前医学决策支持系统中普遍使用基于提示的角色扮演方法，但角色提示对模型推理能力的影响尚不明确，需要评估其是否能诱导出角色特定的认知过程。

Method: 引入RPNA评估框架，在三个医学问答数据集上测试，采用神经元消融和表征分析技术来评估推理路径的变化。

Result: 角色提示并未显著增强LLMs的医学推理能力，主要影响表面语言特征，没有证据显示不同临床角色之间存在不同的推理路径或认知分化。

Conclusion: 当前基于提示的角色扮演方法无法复制真实医学实践中的认知复杂性，需要开发能模拟真实认知过程而非语言模仿的模型。
...
TL;DR: SPICE是一个强化学习框架，通过单一模型在挑战者和推理者两个角色中的自博弈，从大型语料库中挖掘文档生成多样化推理任务，实现持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有无基础的自博弈方法改进效果有限，需要环境交互来实现持续自适应。SPICE旨在通过语料库基础提供丰富、近乎无限的外部信号，实现持续自我改进。
...
TL;DR: WebLeaper是一个解决LLM智能体信息搜索效率低下的框架，通过构建高覆盖率任务和高效解决方案轨迹来提升搜索性能。
...
Result: 在五个信息搜索基准测试（BrowserComp、GAIA、xbench-DeepSearch、WideSearch、Seal-0）上的实验表明，该方法在基本和全面设置下均持续优于强基线，在效果和效率方面都有提升。

Conclusion: WebLeaper框架通过系统性地增加信息搜索任务的覆盖率和效率，有效解决了LLM智能体搜索效率低的问题，在多个基准测试中表现出色。
...
Method: 引入AgentFold范式，采用主动上下文管理策略，执行多尺度折叠操作，包括细粒度压缩和深度整合，通过监督微调实现。

Result: AgentFold-30B-A3B在BrowseComp上达到36.2%，在BrowseComp-ZH上达到47.3%，超越了更大规模的开源模型和领先的专有智能体。

Conclusion: AgentFold通过主动上下文管理有效解决了长视野任务中的上下文饱和问题，展示了在信息寻求任务中的显著优势。
...
TL;DR: 该论文提出了ComboBench基准，评估大型语言模型将语义动作转换为VR设备操作序列的能力，在4款流行VR游戏的262个场景中测试了7个LLM，发现顶级模型在任务分解方面表现良好，但在程序推理和空间理解方面仍落后于人类。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能够像人类一样基于常识和具身理解，将高级语义动作精确转换为VR设备操作序列，这一能力在VR游戏领域尚未得到充分研究。

Method: 创建ComboBench基准，包含来自Half-Life: Alyx、Into the Radius、Moss: Book II和Vivecraft四款VR游戏的262个场景，评估GPT-3.5、GPT-4、GPT-4o、Gemini-1.5-Pro、LLaMA-3-8B、Mixtral-8x7B和GLM-4-Flash七个LLM，并与人工标注的真实数据和人类表现进行比较。

Result: Gemini-1.5-Pro等顶级模型在任务分解方面表现出色，但在程序推理和空间理解方面仍不如人类；不同游戏间的性能差异显著，表明对交互复杂性敏感；少量示例能显著提升性能。

Conclusion: LLM在VR操作能力方面有潜力，但需要针对程序推理和空间理解进行专门增强，few-shot学习是提升性能的有效方法。
...
TL;DR: 本文介绍了为WMT25翻译评估共享任务开发的两个系统：MetricX-25用于质量评分预测，GemSpanEval用于错误跨度检测，两者都基于Gemma 3模型进行微调。
...
Method: MetricX-25采用编码器架构加回归头，GemSpanEval采用解码器架构将错误检测作为生成任务，两者都基于Gemma 3模型在公开WMT数据上微调。

Result: MetricX-25在MQM和ESA质量评分预测上显著优于前代系统，GemSpanEval在错误跨度检测上与强基线xCOMET具有竞争力。

Conclusion: 基于Gemma 3的两种方法在翻译质量评估任务中表现出色，MetricX-25在评分预测上有显著改进，GemSpanEval在生成式错误检测方面具有竞争力。
...
### [121] [Game-TARS: Pretrained Foundation Models for Scalable Generalist Multimodal Game Agents](https://arxiv.org/abs/2510.23691)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够跨异构领域（操作系统、网页、模拟游戏）工作的通用游戏智能体，通过统一的动作空间实现大规模持续预训练，避免API或GUI方法的限制。

Method: 使用基于人类对齐的键盘鼠标输入的统一动作空间，采用衰减持续损失减少因果混淆，以及高效的稀疏思维策略平衡推理深度和推理成本，在500B+标记的多模态数据上进行预训练。
...
### [122] [AI and the Decentering of Disciplinary Creativity](https://arxiv.org/abs/2510.23734)
*Eamon Duede*
...
<details>
  <summary>Details</summary>
Motivation: 研究AI在科学问题解决中的角色，特别是其对学科创造力的潜在影响，探讨AI是否真正增强还是可能削弱科学追求的创造性价值。

Method: 基于创造力哲学理论，区分创造性方法和创造性产品，提出学科创造力的概念，并通过两个数学案例进行实证分析。

Result: 研究发现计算可以扩展学科创造力，但某些涉及AI的方法可能取代学科创造力，这种取代有可能改变甚至削弱科学追求的价值。

Conclusion: AI在科学问题解决中具有双重作用：既能扩展学科创造力，也可能通过某些方法取代它，这提醒我们需要审慎评估AI对科学创造性的长期影响。
...
### [123] [Multi-Environment POMDPs: Discrete Model Uncertainty Under Partial Observability](https://arxiv.org/abs/2510.23744)
...
Method: 将ME-POMDPs推广为具有初始信念集合的AB-POMDPs，证明ME-POMDPs可以简化为仅变化转移和奖励函数或仅变化观测和奖励函数的形式，同时开发了精确和近似（基于点）的算法来计算鲁棒策略。

Result: 成功将标准POMDP基准扩展到多环境设置，并能够计算相应的策略，验证了所提出方法的有效性。

Conclusion: ME-POMDPs和AB-POMDPs为处理POMDP中的模型不确定性提供了有效框架，所提出的算法能够计算在多环境设置下的鲁棒策略，具有实际应用价值。
...
### [124] [Test-Time Tuned Language Models Enable End-to-end De Novo Molecular Structure Generation from MS/MS Spectra](https://arxiv.org/abs/2510.23746)
*Laura Mismetti,Marvin Alberts,Andreas Krause,Mara Graziani*

Main category: cs.AI

TL;DR: 该论文提出了一种基于测试时调优的框架，利用预训练Transformer模型直接从串联质谱和分子式进行端到端的从头分子结构生成，无需数据库匹配或中间步骤，在两个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前串联质谱分析方法依赖于数据库匹配或需要中间片段预测的多步骤流程，难以识别数据库中未包含的未知化合物。

Method: 采用测试时调优框架，增强预训练Transformer模型的学习能力，直接从串联质谱和分子式进行端到端的分子结构生成。

Result: 在NPLIB1和MassSpecGym两个基准测试中分别比现有最佳方法DiffMS提高了100%和20%，测试时调优相比传统微调在MassSpecGym上性能提升62%。

Conclusion: 该方法能够动态适应新质谱数据，即使预测与真实值有偏差，生成的分子候选结构仍然准确，为人工解释和可靠识别提供了有价值的指导。
...
### [125] [Evaluating In Silico Creativity: An Expert Review of AI Chess Compositions](https://arxiv.org/abs/2510.23772)
...
TL;DR: 该研究探讨生成式AI在象棋谜题创作中的创造力，开发了一个能生成具有美学吸引力、新颖性和反直觉解决方案的象棋谜题AI系统，并邀请三位世界级象棋专家评估其创造性。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，需要验证其是否能够产生真正具有创造性和新颖性的输出，特别是在象棋谜题这一需要高度创造力的领域。

Method: 开发了一个专门用于生成象棋谜题的AI系统，该系统能够产生具有美学吸引力、新颖性、反直觉和独特解决方案的谜题。通过邀请三位世界知名象棋专家（国际象棋排局大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）对AI生成的谜题进行评价，评估其创造力、挑战性和美学设计。

Result: 三位象棋专家对AI生成的谜题进行了评估，选择了他们最喜欢的谜题，并解释了这些谜题在创造力、挑战水平和美学设计方面的吸引力。

Conclusion: 研究表明生成式AI在象棋谜题创作领域能够产生具有创造性和美学价值的输出，得到了专业象棋专家的认可。
...
### [126] [Why Foundation Models in Pathology Are Failing](https://arxiv.org/abs/2510.23807)
*Hamid R. Tizhoosh*

Main category: cs.AI

TL;DR: 病理学基础模型在癌症诊断和预测方面表现不佳，存在诊断准确率低、鲁棒性差、计算需求大等根本性弱点，源于通用AI基础模型假设与人体组织内在复杂性之间的概念不匹配。
...
Method: 通过系统评估分析病理学基础模型的局限性，识别出七个相互关联的根本原因：生物复杂性、无效的自监督、过度泛化、过度架构复杂性、缺乏领域特定创新、数据不足以及与组织切片尺寸相关的根本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性、计算效率和安全漏洞方面存在显著不足，无法有效处理人体组织的复杂形态特征。

Conclusion: 当前病理学基础模型在概念上与组织形态学的本质存在根本性错配，需要对整个范式进行根本性的重新思考，开发更适合病理学领域特点的新方法。
...
### [127] [ReCAP: Recursive Context-Aware Reasoning and Planning for Large Language Model Agents](https://arxiv.org/abs/2510.23822)
*Zhenyu Zhang,Tianyi Chen,Weiran Xu,Alex Pentland,Jiaxin Pei*

Main category: cs.AI

TL;DR: ReCAP是一个用于大语言模型的递归上下文感知推理与规划框架，通过计划分解、结构化父计划重注入和内存高效执行机制，显著提升长时程任务的成功率和子目标对齐。
...
Method: 结合三种关键机制：(1) 计划提前分解：生成完整子任务列表，执行第一项并优化剩余任务；(2) 结构化父计划重注入：在递归返回时保持一致的多级上下文；(3) 内存高效执行：限制活动提示，使成本随任务深度线性扩展。

Result: 在各种长时程推理基准测试中显著改善子目标对齐和成功率，在严格pass@1协议下，同步Robotouille提升32%，异步Robotouille提升29%。

Conclusion: ReCAP框架通过将高层目标与低层动作对齐、减少冗余提示和保持连贯的上下文更新，有效解决了长时程任务中的推理和规划挑战。
...
### [128] [Decentralized Multi-Agent Goal Assignment for Path Planning using Large Language Models](https://arxiv.org/abs/2510.23824)
*Murad Ismayilov,Edwin Meriaux,Shuo Wen,Gregory Dudek*

Main category: cs.AI

TL;DR: 该研究比较了贪婪启发式、最优分配和基于大语言模型（LLM）的智能体在去中心化多智能体路径规划中的表现，发现LLM智能体在良好设计的提示和相关信息支持下能达到接近最优的性能。


<details>
  <summary>Details</summary>
Motivation: 解决去中心化条件下多智能体在共享环境中的协调问题，特别是目标分配这一长期挑战。

Method: 智能体基于环境结构化表示（如网格可视化）独立生成目标偏好排序，然后交换排序信息，通过固定确定性冲突解决规则进行目标分配，无需协商或迭代协调。

Result: LLM智能体在提供良好设计的提示和相关定量信息时，能实现接近最优的完成时间，并持续优于传统启发式方法。
...
### [129] [Generating Creative Chess Puzzles](https://arxiv.org/abs/2510.23881)
...
TL;DR: 本文提出了一种基于强化学习的国际象棋谜题生成方法，通过设计基于象棋引擎搜索统计的新型奖励机制，显著提升了谜题的反直觉性、多样性和真实性。
...
Method: 首先对生成式AI架构进行基准测试，然后引入基于象棋引擎搜索统计的新型奖励机制的强化学习框架，以增强谜题的独特性、反直觉性、多样性和真实性。

Result: 强化学习方法将反直觉谜题的生成率从0.22%（监督学习）大幅提升至2.5%，超过了现有数据集（2.1%）和最佳Lichess训练模型（0.4%）。生成的谜题符合新颖性和多样性基准，保留了美学主题，并被人类专家评为比传统书籍谜题更具创造性、趣味性和反直觉性。
...
### [130] [Hybrid Modeling, Sim-to-Real Reinforcement Learning, and Large Language Model Driven Control for Digital Twins](https://arxiv.org/abs/2510.23882)
*Adil Rasheed,Oscar Ravik,Omer San*

Main category: cs.AI

TL;DR: 本文研究了数字孪生在动态系统建模与控制中的应用，比较了物理模型、数据驱动和混合方法，以及传统与AI驱动的控制器。在温室测试平台上，评估了四种预测模型和三种控制策略的性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索数字孪生技术在动态系统建模与控制中的综合应用，比较不同建模方法和控制策略在精度、适应性和实现复杂度方面的表现，为实际应用提供指导。

Method: 使用微型温室作为测试平台，开发了四种预测模型（线性、物理建模PBM、LSTM、混合分析建模HAM）和三种控制策略（MPC、强化学习RL、基于大语言模型LLM的控制），在插值和外推场景下进行比较评估。

Result: 在建模方面，HAM在准确性、泛化能力和计算效率方面表现最均衡，LSTM精度高但资源消耗大。在控制方面，MPC性能稳健可预测，RL适应性强，LLM控制器结合预测工具可实现灵活的人机交互。

Conclusion: HAM模型在建模中提供了最佳平衡性能，而不同控制器各有优势：MPC适合稳健控制，RL适合自适应场景，LLM控制器适合需要人机交互的应用场景。
...
TL;DR: 本文提出了一种基于摊销变分推理的可扩展训练算法，将LVLMs中的推理重新表述为后验推断，通过多样性寻求强化学习算法引入稀疏奖励函数，并采用贝叶斯推理缩放策略来高效排名最优推理路径和答案。


<details>
  <summary>Details</summary>
Motivation: 现有的训练算法（如SFT、PPO、GRPO）在未见推理任务上泛化能力不足，且严重依赖有偏见的奖励模型，这限制了大型视觉语言模型的推理能力和可靠性。

Method: 将LVLMs中的推理重新表述为后验推断问题，提出基于摊销变分推理的可扩展训练算法，使用多样性寻求强化学习算法引入稀疏奖励函数，并实现贝叶斯推理缩放策略以高效排名最优推理路径。

Result: 在七个推理基准测试中，所提方法在有效性、泛化性和可解释性方面提升了最先进的LVLMs性能。

Conclusion: 通过将推理重新表述为后验推断并采用变分推理方法，可以有效提升LVLMs的推理能力，克服现有训练算法的局限性，实现更好的泛化性能和可解释性。
...
TL;DR: 提出直觉主义去中心化因果发现框架judo calculus，使用j-稳定因果推断和j-do-calculus在层拓扑中形式化定义，解决现实应用中因果效应依赖情境的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界应用（从生物学到医学和社会科学）中因果效应依赖于情境（年龄、国家、剂量、基因型或实验室协议），需要形式化这种情境依赖性。

Method: 使用judo calculus，将情境依赖性形式化为局部真值：因果声明在情境覆盖上被证明为真。结合标准得分型、约束型和梯度型因果发现方法，实现算法和实现框架。

Result: 在从合成到真实世界数据集（生物学和经济学）的一系列领域进行实验，结果显示层拓扑因果发现的去中心化性质带来的计算效率提升，以及相对于经典因果发现方法的性能改进。

Conclusion: judo calculus为情境依赖的因果发现提供了形式化框架，通过去中心化方法提高了计算效率并在多个领域展现出优于传统方法的性能。
...
TL;DR: 提出了一种名为sign estimator的新方法，通过用二元分类损失替换交叉熵损失，解决了传统LLM对齐方法在人类偏好异质性下的不一致性问题，实现了可证明一致且高效的估计。


<details>
  <summary>Details</summary>
Motivation: 传统LLM对齐方法对人类偏好的异质性很敏感，基于成对比较数据的朴素概率模型会产生不一致的总体平均效用估计，这是社会福利的规范衡量标准。

Method: 提出sign estimator方法，在聚合步骤中用二元分类损失替换交叉熵损失，在温和假设下恢复一致的序数对齐，并实现了该设置中首个多项式有限样本误差界。

Result: 在基于数字孪生的LLM对齐现实模拟中，sign estimator显著减少了模拟人物面板的偏好扭曲，将（角度）估计误差降低了近35%，与真实总体偏好的不一致性从12%降至8%，优于标准RLHF。
...
<details>
  <summary>Details</summary>
Motivation: 预测破坏性事件前个体移动模式的变化具有挑战性，因为缺乏衡量个体异质性社会基础设施韧性的方法，空间上下文与移动模式的复杂关系未被充分捕捉，且个体级移动数据稀疏不适合传统预测方法。

Method: 构建条件深度学习模型，整合个体的社会基础设施韧性(SIR)和局部空间上下文，使用大规模稀疏的个体级数据来捕捉个体移动模式与空间环境的复杂关系。

Result: 实验表明，整合个体的SIR和空间上下文能增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前移动模式但SIR不同的个体在移动模式上的差异变化。

Conclusion: 该研究证明了将个体社会基础设施韧性和空间上下文纳入深度学习模型可以有效预测破坏性事件后的个体移动模式变化，为解决这一挑战性问题提供了新方法。
...
TL;DR: OneCast是一个结构化、模块化的跨域时间序列预测框架，通过将时间序列分解为季节性和趋势分量，分别使用定制化生成路径建模，在多个领域实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决跨域时间序列预测中面临的领域特定趋势变化和周期性模式不一致的挑战，现有方法将时间序列视为无差别的序列，未能显式解耦其固有结构组件。

Method: 提出OneCast框架：1）将时间序列分解为季节性和趋势分量；2）季节性分量通过轻量级投影模块使用可解释基函数重建周期性模式；3）趋势分量通过语义感知分词器编码为分段级离散标记，通过掩码离散扩散机制推断；4）两个分支输出结合生成最终预测。

Result: 在八个领域的广泛实验中，OneCast大多优于最先进的基线方法。

Conclusion: OneCast通过结构化分解和组件特定建模，有效解决了跨域时间序列预测中的泛化挑战，能够同时捕捉季节性模式和跟踪领域特定趋势。
...
### [136] [Modeling Electric Vehicle Car-Following Behavior: Classical vs Machine Learning Approach](https://arxiv.org/abs/2510.24085)
*Md. Shihab Uddin,Md Nazmus Shakib,Rahul Bhadani*

Main category: cs.AI

TL;DR: 该研究比较了经典模型和机器学习模型在电动汽车跟驰行为建模中的表现，发现随机森林模型在所有场景下都优于基于物理的经典模型。


<details>
  <summary>Details</summary>
Motivation: 随着电动汽车的普及，需要了解其驾驶行为以提高交通安全和开发智能驾驶系统，特别是在混合交通环境中。

Method: 使用真实世界数据集，比较了IDM、OVM、OVRV、简化CACC等经典模型与随机森林回归器，通过最小化RMSE来校准模型参数。

Result: 随机森林模型表现最佳，RMSE分别为0.0046（中等间距）、0.0016（长间距）和0.0025（超长间距）；经典模型中CACC表现最好，长间距RMSE为2.67。

Conclusion: 机器学习模型在电动汽车跟驰行为建模中具有优越性能，对模拟EV行为和混合自主交通动态分析具有重要价值。
...
### [137] [HistoLens: An Interactive XAI Toolkit for Verifying and Mitigating Flaws in Vision-Language Models for Histopathology](https://arxiv.org/abs/2510.24115)
...
Method: 开发了HistoLens系统，能够将病理学家的自然语言问题智能翻译为AI引擎的精确查询，提供结构化报告和热力图可视化证据，并训练AI专注于患者组织而忽略背景噪声。

Result: 创建了一个工作流程，使病理学家保持专家主导地位，使用可信赖的AI助手验证他们的见解，实现更快、更自信的诊断。

Conclusion: HistoLens成功实现了透明、协作的AI病理学助手，通过提供可解释的推理过程和可视化证据，建立了医生对AI的信任，同时保持医生在诊断过程中的主导地位。
...
### [138] [BMGQ: A Bottom-up Method for Generating Complex Multi-hop Reasoning Questions from Semi-structured Data](https://arxiv.org/abs/2510.24151)
*Bingsen Qiu,Zijian Liu,Xiao Liu,Haoshen Yang,Zeren Gao,Bingjie Wang,Feier Zhang,Yixuan Qin,Chunyan Li*

Main category: cs.AI

TL;DR: 提出了一个自动化框架，用于从半结构化知识源生成高难度的、可用于训练的多跳问答问题，解决现有数据集稀缺且不适合监督微调或强化学习的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多跳问答数据集稀缺，且大多仅用于评估而非训练，人工构建成本高昂且难以扩展，这限制了检索和推理能力强的智能体的训练。

Method: 使用自然语言推理进行关系类型标注和多样性扩展来构建证据簇，应用逆向问题构建来组合模糊线索，并通过多模型共识过滤和结构化约束分解的两步评估流程确保质量。

Result: 开发了一个可扩展的流程，能够生成复杂、检索困难但可验证的问题，适用于监督微调和强化学习训练，同时大幅减少了人工标注工作量。

Conclusion: 该自动化框架有效解决了多跳问答训练数据稀缺的问题，能够生成具有挑战性的训练数据，同时保持与强评估基准相当的难度水平。
...
### [139] [BLM$_1$: A Boundless Large Model for Cross-Space, Cross-Task, and Cross-Embodiment Learning](https://arxiv.org/abs/2510.24161)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在数字-物理空间和具身化之间泛化能力差，VLAs缺乏高级具身推理能力，ELLMs局限于数字空间，因此需要统一的跨空间和跨具身模型。

Method: 采用两阶段训练：阶段I通过精选数字语料向MLLM注入具身知识并保持语言能力；阶段II通过意图桥接接口训练策略模块，提取MLLM的高级语义指导控制，无需微调MLLM主干。
...
### [140] [Investigating Intra-Abstraction Policies For Non-exact Abstraction Algorithms](https://arxiv.org/abs/2510.24297)
*Robin Schmöcker,Alexander Dockhorn,Bodo Rosenhahn*

Main category: cs.AI

TL;DR: 本文针对MCTS中抽象算法的样本效率问题，提出了多种内部抽象策略来替代随机平局决胜规则，并在多个环境中验证了这些策略的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决MCTS中抽象算法（如pruned OGA）在多个动作属于同一抽象节点时使用随机平局决胜规则的问题，这可能导致性能不佳。

Method: 提出并实证评估了多种替代的内部抽象策略，用于处理同一抽象节点中多个动作的UCB值相同的情况。

Result: 多个提出的内部抽象策略在大多数环境和参数设置下都优于随机策略。

Conclusion: 通过改进内部抽象策略可以显著提升MCTS抽象算法的性能，随机平局决胜规则不是最优选择。
...
### [141] [Verifying Large Language Models' Reasoning Paths via Correlation Matrix Rank](https://arxiv.org/abs/2510.24299)
*Jiayu Liu,Wei Dai,Zhenya Huang,Ning Miao,Enhong Chen*

Main category: cs.AI

TL;DR: 本文提出了一种名为Self-Indicator的方法，通过分析LLM内部行为来评估其推理路径的可信度，无需外部资源即可有效检测推理错误。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备强大的推理能力，但容易产生错误和幻觉。现有检查方法依赖外部资源，计算开销大且仅适用于特定领域。

Method: 研究发现输入问题与输出推理路径之间的相关性矩阵的秩是推理正确性的稳健指标，基于此设计了简单即插即用的Self-Indicator方法对候选推理路径进行重加权。

Result: 该方法在区分正确与错误推理路径方面达到超过75%的准确率，并在三个推理基准上将准确率提高了8%以上，且计算开销极小。

Conclusion: LLM的内部行为已经隐含了其推理路径的可信度信息，Self-Indicator方法能够有效提升推理准确性且计算效率高。
...
### [142] [Retrieval and Argumentation Enhanced Multi-Agent LLMs for Judgmental Forecasting](https://arxiv.org/abs/2510.24303)
*Deniz Gorur,Antoni Rago,Francesca Toni*

Main category: cs.AI

TL;DR: 本文提出了一个用于判断预测的多智能体框架，通过不同智能体对声明真实性产生分歧并收集正反证据，构建量化双极论证框架(QBAFs)，使用多种基于大语言模型的智能体进行声明验证，实验表明多智能体组合能提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 将判断预测视为声明验证任务，通过多智能体框架整合不同证据来源，提高预测准确性并提供可解释的验证过程。

Method: 提出多智能体声明验证框架，使用三种基于LLM的智能体：ArgLLM、RbAM和RAG-ArgLLM，分别采用不同方法生成和评估QBAFs，在标准判断预测数据集上进行实验。

Result: 实验结果表明，多智能体组合（特别是三个智能体）能够提高预测准确性，同时提供可解释的证据组合。

Conclusion: 多智能体框架通过整合不同证据来源有效提高了判断预测的准确性，并为声明验证提供了可解释的证据组合方法。
...
### [143] [Generative Large Language Models (gLLMs) in Content Analysis: A Practical Guide for Communication Research](https://arxiv.org/abs/2510.24337)
*Daria Kravets-Meinke,Hannah Schmid-Petri,Sonja Niemann,Ute Schmid*

Main category: cs.AI

TL;DR: 本文探讨了生成式大语言模型在传播研究内容分析中的应用，分析了其优势（如优于人工编码、成本效益高）和面临的七大挑战，并提出最佳实践指南。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式大语言模型在传播研究内容分析中展现出巨大潜力，但其在方法论工具包中的整合仍不充分，需要解决影响结果质量的关键挑战。

Method: 综合新兴研究，提出包含代码本开发、提示工程、模型选择、参数调优、迭代优化、可靠性验证和性能提升的七大挑战应对框架。

Result: 生成式大语言模型在传播科学相关编码任务中能够超越众包工作者和训练有素的编码员，且成本和时间大幅降低，并能解码隐含意义和上下文信息。

Conclusion: 本文旨在使基于生成式大语言模型的内容分析对更广泛的传播研究者更加可及，并确保符合有效性、可靠性、可重复性和研究伦理等学科质量标准。
...
### [144] [VDSAgents: A PCS-Guided Multi-Agent System for Veridical Data Science Automation](https://arxiv.org/abs/2510.24339)
*Yunxuan Jiang,Silan Hu,Xiaoning Wang,Yuanyuan Zhang,Xiangyu Chang*

Main category: cs.AI

TL;DR: VDSAgents是一个基于可预测性-可计算性-稳定性（PCS）原则的多智能体系统，用于改进LLM驱动的数据科学自动化，在九个数据集上表现优于AutoKaggle和DataInterpreter。
...
Result: 在九个不同特征的数据集上评估，使用DeepSeek-V3和GPT-4o作为后端，VDSAgents在性能上持续优于AutoKaggle和DataInterpreter。
...
### [145] [An N-of-1 Artificial Intelligence Ecosystem for Precision Medicine](https://arxiv.org/abs/2510.24359)
*Pedram Fard,Alaleh Azhir,Neguine Rezaii,Jiazi Tian,Hossein Estiri*

Main category: cs.AI

TL;DR: 提出一种多智能体生态系统，用于N-of-1决策支持，通过协调多个专业智能体来解决传统AI医疗系统在罕见病例、多病共存和代表性不足人群中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统AI医疗系统服务于平均患者，但在罕见变异、多病共存和代表性不足人群等边缘情况下表现不佳，这损害了公平性和信任度。

Method: 构建多智能体生态系统，智能体按器官系统、患者群体和分析模式聚类，共享模型库和证据合成工具，通过协调层整合结果并评估可靠性、不确定性和数据密度。

Result: 系统为临床医生提供决策支持包，包括带置信区间的风险估计、异常值标记和相关证据链接，验证重点从群体平均转向个体可靠性。
...
### [146] [Improving LLM Reasoning via Dependency-Aware Query Decomposition and Logic-Parallel Content Expansion](https://arxiv.org/abs/2510.24390)
...
Method: 将查询推理分解为两个协同阶段：关键点生成（通过检索增强的少样本提示提炼逻辑结构化的关键点）和内容并行扩展（基于依赖图并发扩展内容以确保逻辑一致性），并引入管道调度机制实现跨查询并行。

Result: 在多样化基准测试中，Orion相比基线实现了最高4.33倍的token生成速度和3.42倍的答案延迟降低，同时通过显式建模点间依赖关系将推理质量提升高达18.75%。

Conclusion: Orion框架成功解决了LLM推理在Web应用中的效率-质量权衡问题，通过创新的分解和并行化方法显著提升了推理性能和效果。
...
### [147] [APTBench: Benchmarking Agentic Potential of Base LLMs During Pre-Training](https://arxiv.org/abs/2510.24397)
*Jiarui Qin,Yunjia Xi,Junjie Huang,Renting Rui,Di Yin,Weiwen Liu,Yong Yu,Weinan Zhang,Xing Sun*

Main category: cs.AI

TL;DR: APTBench是一个用于评估LLM预训练阶段智能体潜力的基准框架，将真实世界智能体任务转换为适合基础模型的多选题或文本补全问题，比现有基准更能预测下游智能体性能。
...
Result: 相比现有通用基准，APTBench能更准确地预测模型作为智能体的下游性能，同时比后训练的全规模端到端智能体评估更轻量、成本更低。
...
### [148] [Human-Level Reasoning: A Comparative Study of Large Language Models on Logical and Abstract Reasoning](https://arxiv.org/abs/2510.24435)
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的推理能力对于推进人工智能至关重要，因为这超越了单纯的语言任务表现，涉及理解这些模型是否真正理解信息、进行推理并以逻辑有效的方式得出结论。

Method: 使用8个定制设计的推理问题，比较了GPT、Claude、DeepSeek、Gemini、Grok、Llama、Mistral、Perplexity和Sabi'a等LLMs的逻辑和抽象推理技能，并将结果与人类在相同任务上的表现进行基准测试。

Result: 研究揭示了LLMs与人类表现之间的显著差异，表明LLMs在演绎推理方面存在困难。

Conclusion: 大型语言模型在逻辑和抽象推理能力方面仍存在明显不足，特别是在演绎推理方面与人类表现有显著差距。
...
### [149] [Law in Silico: Simulating Legal Society with LLM-Based Agents](https://arxiv.org/abs/2510.24442)
*Yiding Wang,Yuxuan Chen,Fanxu Meng,Xifan Chen,Xiaolei Yang,Muhan Zhang*

Main category: cs.AI

TL;DR: 本文介绍了一个基于大语言模型的法律社会模拟框架Law in Silico，通过模拟立法、司法和执法机制，验证了LLM能够复现宏观犯罪趋势，并发现透明、适应性强的法律系统能更好保护弱势群体权利。
...
Result: 实验结果表明，基于LLM的智能体能够很大程度上复现宏观层面的犯罪趋势，并提供与现实世界观察相符的见解。微观层面模拟显示，功能良好、透明且适应性强的法律系统能更好地保护弱势个体的权利。

Conclusion: LLM能够有效模拟法律系统，不仅能够复现宏观犯罪模式，还能揭示法律系统设计对弱势群体保护的重要性，为法律理论验证和系统优化提供了新途径。
...
### [150] [From Cross-Task Examples to In-Task Prompts: A Graph-Based Pseudo-Labeling Framework for In-context Learning](https://arxiv.org/abs/2510.24528)
*Zihan Chen,Song Wang,Xingbo Fu,Chengshuai Shi,Zhenyu Lei,Cong Shen,Jundong Li*

Main category: cs.AI

TL;DR: 提出了一种成本效益高的两阶段流水线，通过交叉任务示例和基于图的标签传播方法，减少对LLM数据标注的依赖，构建用于上下文学习的演示示例。


<details>
  <summary>Details</summary>
Motivation: 为新的或具有挑战性的任务收集高质量的上下文学习示例成本高昂且劳动密集，需要减少对大型语言模型数据标注的依赖。

Method: 两阶段流水线：首先利用现成的交叉任务示例提示LLM对少量目标任务实例进行伪标注，然后引入基于图的标签传播方法将标签信息传播到剩余目标示例中，无需额外LLM查询。

Result: 在五个任务上的实验表明，该方法在降低标注成本的同时实现了强大的性能。

Conclusion: 该流水线结合了交叉任务监督的灵活性和无需LLM传播的可扩展性，为上下文学习提供了成本效益高的解决方案。
...
### [151] [Generative AI for Healthcare: Fundamentals, Challenges, and Perspectives](https://arxiv.org/abs/2510.24551)
...
Method: 重新定位数据生命周期，构建医疗数据生态系统作为生成式医疗系统的基础设施，支持多模态医疗数据的集成、表示和检索。

Result: 该生态系统能够为基座模型提供高质量多模态数据进行预训练和领域微调，同时作为知识检索后端支持任务特定推理。

Conclusion: 以数据为中心的范式能够实现高质量、有效的生成式AI医疗应用部署，改善医疗服务交付。
...
TL;DR: FunReason-MT是一个用于多轮工具使用的数据合成框架，通过环境-API图交互、高级工具查询合成和引导迭代链来解决现有方法在真实世界环境中的不足，显著提升了函数调用能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法（如随机环境采样或多智能体角色扮演）在真实世界环境中无法生成高质量的多轮函数调用训练数据，存在目标模型训练、工具架构隔离和多轮逻辑依赖三大挑战。

Method: FunReason-MT采用三种核心技术：1）环境-API图交互收集多样化高质量轨迹；2）高级工具查询合成简化困难查询构建；3）引导迭代链生成复杂思维链。

Result: 在Berkeley函数调用排行榜上，基于FunReason-MT生成数据训练的4B模型在同类规模模型中达到最先进性能，超越了大多数闭源模型，在BFCLv4上进一步验证了其可靠性。

Conclusion: FunReason-MT为智能体学习提供了可靠且鲁棒的数据源，有效解决了多轮函数调用数据合成的复杂性问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习在实时计算机视觉中的快速发展，作物病害管理从手工特征提取发展到大规模自动特征学习。基础模型的出现为处理作物病害数据集提供了全新的方法，能够整合视觉和文本数据，解释症状文本，推理症状与防治关系，并为种植者和教育者提供交互式问答支持。

Method: 通过筛选约40篇关于基础模型在定点病害管理中应用的文献，重点分析了大语言模型和视觉语言模型的作用，并讨论了它们在自适应学习、强化学习以及数字孪生框架中的具体应用。

Result: 主要发现包括：基础模型在2023-24年文献激增；视觉语言模型发展快于大语言模型，出版物数量增加5-10倍；强化学习和自适应学习在智能喷洒中仍处于起步阶段；数字孪生结合强化学习可虚拟模拟靶向喷洒；解决模拟到现实的差距对实际部署至关重要；人机协作仍然有限；多模态基础模型与实时反馈将推动下一代定点病害管理发展。

Conclusion: 基础模型在作物定点病害管理中展现出巨大潜力，特别是在视觉语言模型领域。然而，强化学习和自适应学习的应用仍处于早期阶段，解决模拟到现实的差距以及加强人机协作是未来发展的关键方向。多模态基础模型结合实时反馈技术将推动下一代智能病害管理系统的创新。
...
Result: 实验表明该数据集提供了具有挑战性但可解决的基准，提出的奖励与GRPO风格算法结合时效果显著。
...
TL;DR: 提出一个通过构建工具和文档知识图谱来增强示例工件生成的框架，通过深度-稀疏集成策略将工具依赖与程序知识对齐，实验证明该统一框架能有效建模工具交互并改进计划生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用工具与文档之间的依赖关系，限制了示例工件生成的质量和效率。通过构建统一的知识图谱框架，可以更好地建模工具交互并提升计划生成能力。

Method: 从工具模式构建工具知识图谱，从内部文档和SOP构建补充知识图谱，然后将两者融合。采用深度-稀疏集成策略将结构工具依赖与程序知识对齐来生成示例计划。

Result: 实验证明该统一框架能有效建模工具交互，显著改进计划生成质量，展示了将工具图谱与领域知识图谱链接的益处。

Conclusion: 通过将工具知识图谱与领域知识图谱融合，可以显著增强工具增强推理和规划能力，为示例工件生成提供了更有效的解决方案。
...
### [156] [Flexible Intelligent Layered Metasurfaces for Downlink Multi-user MISO Communications](https://arxiv.org/abs/2510.24190)
*Hong Niu,Jiancheng An,Chau Yuen*

Main category: cs.IT

TL;DR: 提出柔性智能分层超表面（FILM）架构，通过两个形状可控的柔性超表面层替代传统刚性超表面，显著减少所需层数同时保持信号处理性能，在MU-MISO系统中实现比传统七层SIMs超过200%的和速率提升和7dB BER增益。


<details>
  <summary>Details</summary>
Motivation: 传统堆叠智能超表面（SIMs）依赖均匀层间距且需要深度堆叠来确保处理能力，导致实际应用中严重的功率衰减。为解决这一问题，提出柔性架构以减少层数并降低功率损耗。

Method: 开发两层FILM辅助的多用户多输入单输出系统，通过交替优化方法解决信道拟合问题，包括闭式相位偏移更新和基于梯度下降的形状优化。

Result: 仿真结果显示，所提出的透射式FILM架构相比传统七层SIMs实现超过200%的和速率提升和超过7dB的误码率增益。

Conclusion: 柔性智能分层超表面架构通过动态调整传输系数矩阵，在减少层数的同时保持性能，为波域信号处理提供了更实用的解决方案。
...
### [157] [What Can Be Recovered Under Sparse Adversarial Corruption? Assumption-Free Theory for Linear Measurements](https://arxiv.org/abs/2510.24215)
*Vishal Halder,Alexandre Reiffers-Masson,Abdeldjalil Aïssa-El-Bey,Gugan Thoppe*

Main category: cs.IT

TL;DR: 该论文研究了在存在稀疏对抗性噪声的情况下，从观测数据中恢复未知信号的问题。主要结果表明，在不知道噪声的情况下，能够恢复的最佳信息是x*加上核空间ker(U)，其中U是投影矩阵。


<details>
  <summary>Details</summary>
Motivation: 研究在任意矩阵A和任意信号x*的情况下，当存在q-稀疏对抗性噪声时，如何从观测数据y中恢复关于x*的最大信息量。现有方法通常需要强结构假设（如受限等距性、稀疏性），而本文关注更一般的情况。

Method: 通过分析所有可能删除2q行得到的子矩阵的行空间的交集，定义投影矩阵U。证明最小化||y - Ax||_0范数的所有x都位于x* + ker(U)中，从而给出了恢复这个集合的构造性方法。

Result: 证明了在不知道稀疏对抗性噪声e的情况下，能够恢复的最佳信息是x* + ker(U)，其中U是投影到所有删除2q行得到的子矩阵行空间交集上的唯一投影矩阵。

Conclusion: 对于任意矩阵A和任意信号x*，在存在q-稀疏对抗性噪声的情况下，能够恢复的最佳信息是x* + ker(U)，这给出了一个构造性的恢复方法，而不需要对A或x*施加强结构假设。
...
### [158] [Joint Active and Passive Beamforming with Sensing-Assisted Discrete Phase Shifts for Dual-RIS ISAC Systems](https://arxiv.org/abs/2510.24480)
...
<details>
  <summary>Details</summary>
Motivation: 针对6G需求，研究双RIS辅助的ISAC系统，通过联合波束成形优化来增强系统性能和用户公平性。

Method: 首先利用双RIS进行用户角度估计以简化问题求解过程，然后开发了一种高效的交替优化算法，包括使用半定松弛和二分法解决发射波束成形优化子问题，以及针对RIS离散相位偏移采用感知辅助方法约束优化搜索空间，并为不同RIS尺寸引入了两种低复杂度搜索策略。

Result: 数值仿真结果表明，所提算法性能接近理想连续相位偏移基准，优于传统离散相位偏移优化算法，且相比单RIS系统有显著改进。

Conclusion: 所提出的双RIS辅助ISAC系统和优化算法在6G场景下能够有效提升系统性能和用户公平性，具有实际应用价值。
...
### [159] [Feedback Lunch: Deep Feedback Codes for Wiretap Channels](https://arxiv.org/abs/2510.16620)
*Yingyao Zhou,Natasha Devroye,Onur Günlü*

Main category: cs.IT

TL;DR: 本文研究了反向退化窃听信道，提出了一种结合通用哈希函数和基于反馈的学习码的模块化编码设计，在存在信道输出反馈的情况下实现正保密速率。


<details>
  <summary>Details</summary>
Motivation: 针对反向退化窃听信道在没有信道反馈时保密容量为零的问题，研究如何利用信道输出反馈来克服窃听者的安全优势，实现合法方之间的安全通信。

Method: 采用种子模块化编码设计，结合通用哈希函数提供安全性，使用基于反馈的学习码确保可靠性，研究通信可靠性与信息泄露之间的权衡。

Result: 研究表明反馈机制使合法方能够协商共享密钥，从而在反向退化窃听信道中实现正保密速率，克服了窃听者的安全优势。

Conclusion: 该研究为下一代集成感知与通信方法中的感知辅助安全通信提供了编码设计动机，证明了反馈在提升窃听信道安全性方面的重要作用。
