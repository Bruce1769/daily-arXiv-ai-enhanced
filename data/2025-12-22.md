...
TL;DR: V-Agent是一个用于高级视频搜索和交互式对话的多智能体平台，通过微调视觉语言模型并集成检索向量，在共享多模态表示空间中嵌入视频帧和音频转录，实现上下文感知的视频搜索。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的检索系统在多模态场景中存在局限性，需要能够同时理解视觉和语音内容进行上下文感知视频搜索的系统。

Method: 1) 使用小型视频偏好数据集微调视觉语言模型；2) 集成图像-文本检索模型的检索向量；3) 构建三个协作智能体：路由智能体、搜索智能体和聊天智能体；4) 搜索智能体使用VLM检索模型和重排序模块；5) 在共享多模态表示空间中嵌入视频帧和ASR转录。

Result: 在MultiVENT 2.0基准测试中实现了最先进的零样本性能，展示了在学术研究和实际应用中的潜力。

Conclusion: V-Agent通过多智能体协作和先进的视觉语言模型，有效解决了多模态视频搜索的挑战，为上下文感知的视频检索和交互提供了创新解决方案。
...
TL;DR: 本文提出了一种新方法，将Finer-CAM解释与TLS投影中的结构树特征片段联系起来，系统评估哪些特征驱动树种分类。使用7个欧洲树种的2445棵树TLS数据训练YOLOv8模型，平均准确率达96%，分析显示模型主要依赖树冠特征进行分类。


<details>
  <summary>Details</summary>
Motivation: 尽管新型传感器和深度学习方法在树种分类中取得了先进精度，但其决策过程仍不清晰。现有方法如Finer-CAM可以突出显示TLS投影中贡献于目标树种分类的特征，但在外观相似的对比树种中应用不常见。需要更好地理解树种分类模型的决策过程，以揭示数据集和模型的局限性、偏见，并建立对模型预测的信心。

Method: 提出一种新颖方法，将Finer-CAM解释与TLS投影中代表结构树特征的片段联系起来，系统评估哪些特征驱动树种区分。使用来自7个欧洲树种的2445棵树的TLS数据，通过交叉验证训练和验证五个YOLOv8模型。分析了630个显著性图来理解模型的决策依据。

Result: 模型达到平均96%的准确率（标准差0.24%）。显著性图分析显示模型主要依赖TLS投影中的树冠特征进行树种分类。这一结果在银桦、欧洲山毛榉、英国栎和挪威云杉中尤为明显，而欧洲白蜡、苏格兰松和花旗松的分类中茎干特征贡献更频繁。特别是较细枝条的表征对模型决策有贡献。模型认为相似的树种与人类专家的判断一致。

Conclusion: 研究结果强调了需要改进对树种分类模型决策过程的理解，以帮助揭示数据集和模型的局限性、偏见，并建立对模型预测的信心。提出的方法能够系统评估驱动树种区分的特征，为理解深度学习模型在森林遥感中的应用提供了重要见解。
...
TL;DR: 提出了一种分阶段的电影制作式视频生成方法，通过LLM生成详细剧本，文本到图像模型创建一致角色视觉锚点，再合成各场景视频，显著提升角色一致性


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频AI在生成长篇、连贯且角色一致的视频故事方面面临重大挑战，需要更系统的方法来保持角色身份一致性

Method: 采用多阶段分解方法：1）使用大语言模型生成详细制作剧本；2）文本到图像模型为每个角色创建一致的视觉锚点；3）视频生成模型基于视觉锚点单独合成每个场景

Result: 视觉锚定机制至关重要，移除后角色一致性分数从7.99急剧下降到0.55；同时发现当前模型存在文化差异，印度与西方主题生成在主体一致性和动态程度方面表现出明显偏见

Conclusion: 多阶段分解方法能有效提升视频生成的角色一致性，视觉先验对于身份保持至关重要，同时需要关注模型中的文化偏见问题
...
<details>
  <summary>Details</summary>
Motivation: 当前视频tokenization方法对所有内容采用固定压缩率，忽略了视频固有的复杂性和可变信息密度，导致冗余或信息丢失。需要一种能够根据信息丰富度自适应分配token的方法。

Method: 基于香农信息理论，提出了InfoTok框架，证明了现有数据无关训练方法在表示长度上的次优性，并提出了基于证据下界（ELBO）的新算法来逼近理论最优性。开发了基于transformer的自适应压缩器实现自适应tokenization。

Result: 实现了最先进的压缩性能：在不影响性能的情况下节省20%的token，达到2.3倍压缩率，同时优于先前启发式自适应方法。通过根据信息丰富度分配token，实现了更压缩且准确的视频表示。

Conclusion: InfoTok为自适应视频tokenization提供了一个理论严谨的框架，通过信息理论指导的token分配实现了更高效的视频压缩，为未来研究提供了有价值的见解。
...
TL;DR: LongShOTBench是一个诊断性基准测试，包含开放式意图驱动问题、多轮对话和多模态推理任务，用于评估长视频理解能力。现有MLLMs表现不佳，Gemini-2.5-Flash仅52.95%，开源模型低于30%，而作者提出的LongShOTAgent达到44.66%。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解基准测试要么关注时间长度，要么关注多模态丰富度，但很少同时兼顾两者。大多数基准测试依赖单一准确率分数，掩盖了失败模式，需要更全面的评估框架。

Method: 提出LongShOTBench基准测试，包含开放式意图驱动问题、单轮和多轮对话、需要多模态推理和工具使用的任务。每个项目都有参考答案和分级评分标准。采用可扩展、人工验证的流水线确保覆盖范围和可重复性。

Result: 在LongShOTBench上，最先进的MLLMs表现不佳：Gemini-2.5-Flash达到52.95%，开源模型低于30%。作者提出的LongShOTAgent系统达到44.66%，显示长视频理解的挑战性。

Conclusion: LongShOTBench为评估和改进MLLMs提供了实用、可重复的基础。长视频理解仍然是一个具有挑战性的任务，需要更先进的模型和方法。
...
TL;DR: 4D-RGPT：一种专门的多模态大语言模型，通过增强的时间感知从视频输入中捕获4D表示，结合感知4D蒸馏训练框架和R4D-Bench基准测试，显著提升了4D视频问答性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D结构和时间动态推理方面能力有限，受限于弱4D感知和时间理解。现有的3D和4D视频问答基准测试强调静态场景且缺乏区域级提示。

Method: 提出4D-RGPT专门模型捕获视频输入的4D表示；设计感知4D蒸馏训练框架，从冻结专家模型转移4D表示到4D-RGPT；构建R4D-Bench基准测试，通过混合自动化和人工验证流程创建深度感知动态场景的区域级提示数据集。

Result: 4D-RGPT在现有4D VQA基准测试和提出的R4D-Bench基准测试上都取得了显著改进。

Conclusion: 该研究通过专门的模型架构、训练框架和基准测试，有效解决了多模态大语言模型在4D感知和时间理解方面的局限性，为动态场景的区域级推理提供了新解决方案。
...
TL;DR: FORMSpoT提供法国10年（2014-2024）全国森林冠层高度1.5米分辨率制图及年度扰动多边形，显著优于现有产品，能检测单个树木尺度变化
...
Conclusion: FORMSpoT-Δ能实现全国尺度的树木级森林动态监测，为分析管理实践、检测森林衰退早期信号、量化细微扰动碳损失提供独特工具；强调维持SPOT等高分辨率卫星任务和DINAMIS等开放数据计划对气候变化下森林监测的重要性
...
TL;DR: InfCam是一个免深度的相机控制视频生成框架，通过无限单应性扭曲和增强数据训练，实现高相机姿态保真度的视频生成。
...
Conclusion: InfCam通过免深度的方法解决了相机控制视频生成中的关键挑战，实现了高相机姿态保真度的视频生成，为动态场景的相机控制提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前评估合成医学图像与真实图像相似性的方法主要依赖用户评估、基于Inception的度量和分类性能，缺乏可解释性。需要一种能够定量评估合成数据集在开发深度学习临床决策支持系统方面效用的方法。

Method: 受广义神经加法模型启发，提出可解释的效用相似性（IUS）度量方法。该方法通过临床相关图像特征来解释为什么一个合成数据集比另一个在临床决策支持系统中更有用，不同于基于Inception的不可解释度量。

Result: 在多种彩色医学成像模态（内窥镜、皮肤镜、眼底成像）的公开数据集上，使用IUS选择高效用相似性的合成图像，分类性能相对提升最高达54.6%。在灰度X光和超声成像模态上也证明了IUS的通用性。

Conclusion: IUS提供了一种可解释的方法来评估合成医学图像数据集在开发深度学习临床决策支持系统方面的效用相似性，能够显著提升分类性能，并适用于多种医学成像模态。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态捕捉和物理模型，需要手动参数调整且计算量大，难以处理多样发型和运动

Method: 1) 粗到细模型学习时间连贯的头发动态；2) 发丝引导优化模块学习动态3D高斯表示，支持可微分渲染

Result: DGH在几何和外观方面取得良好效果，能无缝集成到3D高斯头像框架，实现逼真可动画的头发

Conclusion: DGH提供了可扩展的数据驱动替代方案，能泛化到多种发型和头部运动序列
...
TL;DR: 该综述论文系统回顾了海事雷达帧预测的相关方法，重点分析了transformer架构在时空序列预测中的应用，发现目前尚无transformer用于海事雷达帧预测的研究，明确了这一研究空白。


<details>
  <summary>Details</summary>
Motivation: 海事自主系统需要强大的预测能力来预判船舶运动和动态环境。虽然transformer架构已成功应用于AIS轨迹预测并展示了声纳帧预测的可行性，但在海事雷达帧预测方面仍属空白，而雷达在导航中具有全天候可靠性优势。

Method: 系统综述了与海事雷达预测相关的建模方法，重点分析transformer架构在时空序列预测中的应用。按照数据类型、架构和预测时间范围对现有代表性方法进行分类分析。

Result: 文献综述表明，虽然已有transformer用于声纳传感帧预测的研究，但尚无任何工作涉及基于transformer的海事雷达帧预测，这明确了一个重要的研究空白。

Conclusion: 该综述明确了transformer架构在海事雷达帧预测领域的研究空白，为未来研究指明了具体方向，强调了开发这一技术对提升海事自主系统预测能力的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 专业摄影师拍摄的人物照片通常具有优美的光线、有趣的姿态和高质量效果，而普通人自拍的照片往往达不到这种水平。目前缺乏大规模成对的"野外"照片和专业照片数据集，这给训练带来了挑战

Method: 1) 将输入照片和人物面部转换到规范的UV空间，结合重姿态建模来处理遮挡和新视角合成；2) 通过多图像微调实现个性化输出；3) 利用现有的非配对数据集进行训练

Result: 该方法能够生成高质量的重姿态肖像照，在真实世界图像上取得了良好的定性和定量性能

Conclusion: 提出的方法通过UV空间转换和多图像微调，成功解决了在缺乏配对数据的情况下将普通人物照片转换为专业肖像照的问题，同时保持了人物的身份特征
...
TL;DR: 提出一个文档中心化背景生成框架，支持多页面编辑和主题连续性，通过潜在掩码保持文本可读性，自动优化可读性，并通过摘要-指导过程维持多页面一致性。


<details>
  <summary>Details</summary>
Motivation: 现有文档背景生成方法难以同时保证文本可读性、多页面主题一致性和设计美学，需要一种能自动平衡这些要素的框架。

Method: 1) 潜在掩码公式：在扩散空间中软衰减更新以保护文本区域；2) 自动可读性优化(ARO)：自动放置半透明圆角背景形状以满足WCAG 2.2对比度标准；3) 摘要-指导过程：将每页压缩为紧凑表示以递归指导后续生成；4) 分层处理：将文档视为结构化组合，分别处理文本、图形和背景层。

Result: 该免训练框架能生成视觉连贯、文本保护良好、主题对齐的文档，在保持可读性的同时实现美学和谐，并支持用户通过提示进行风格调整。

Conclusion: 该框架成功地将生成建模与自然设计工作流程相结合，实现了文档背景的自动化生成，同时确保了可读性、多页面一致性和设计灵活性。
...
TL;DR: 提出PhysFire-WM物理信息世界模型，通过编码物理模拟器先验和跨任务协同训练策略，提升细粒度火灾蔓延预测的准确性和物理一致性


<details>
  <summary>Details</summary>
Motivation: 当前火灾预测方法主要局限于二元掩模建模，信号稀疏且无法捕捉火灾复杂动态；世界模型在视频生成中表现良好但存在物理不一致性问题，需要结合物理先验来改进火灾蔓延预测

Method: 提出PhysFire-WM物理信息世界模型，包含：1）从物理模拟器编码结构化先验以修正物理不一致性；2）跨任务协同训练策略（CC-Train），通过参数共享和梯度协调整合热辐射动态和空间边界划分，缓解掩模建模信息有限问题

Result: 在细粒度多模态火灾数据集上的大量实验表明，PhysFire-WM在火灾蔓延预测方面具有优越的准确性；验证强调了物理先验和跨任务协作的重要性

Conclusion: PhysFire-WM为应用物理信息世界模型进行灾害预测提供了新见解，通过整合物理先验和跨任务协作，显著提升了火灾蔓延预测的物理真实性和几何准确性
...
TL;DR: LGCLIP提出了一种无需人工标注图像-文本对的轻量级视觉语言模型框架，利用大语言模型生成类别提示指导扩散模型合成参考图像，仅需类别标签即可实现零样本图像分类。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）依赖人工标注的图像-文本对进行模态对齐，这带来了高昂的数据准备成本和准确性要求。同时，双塔编码器结构也阻碍了模型的轻量化部署。

Method: LGCLIP框架：1）使用大语言模型生成类别特定的文本提示；2）利用扩散模型根据提示合成参考图像作为视觉原型；3）仅使用视觉编码器提取真实图像特征并与原型特征进行比较预测；4）通过LLM优化提示生成，实现轻量化架构。

Result: 实验验证了LGCLIP的可行性和效率，在零样本分类任务中表现出色，仅需类别标签作为输入，无需人工标注的图像-文本对和额外预处理。

Conclusion: LGCLIP建立了一种新颖的分类范式，通过大语言模型生成和扩散模型合成，实现了无需人工标注的轻量级视觉语言模型，为分类任务提供了新的解决方案。
...
Conclusion: ABE-CLIP是一种无需训练的有效方法，能够增强CLIP类模型中的属性绑定能力，解决了现有方法在组合概念泛化和全局表示缺陷方面的局限性。
...
TL;DR: 研究量化了城市绿化客观测量与主观感知之间的差异，发现这种差异在全球范围内具有可比性，人口统计和个性特征对感知影响不大，而居住地是解释感知差异的最重要因素。
...
Method: 使用街景图像提取上下文信息，结合来自5个国家1000人的城市视觉感知调查数据，收集了广泛的人口统计和个性信息。分析客观指标（如绿视率GVI）与主观评分之间的差异，检验这些差异是否可以通过人类和视觉因素（如年龄组、场景中绿化的空间变化）来解释。

Result: 研究发现：1）客观测量与主观感知之间的差异在全球范围内具有可比性；2）人口统计和个性特征在感知中不起显著作用；3）感知绿化与测量绿化在地理上具有一致性，但人们的居住地是解释感知差异的最重要因素；4）在七个影响因素中，居住地是影响感知绿化的最主要因素。

Conclusion: 文化、环境和经验因素显著影响个体对城市绿化的观察方式，居住地是解释客观测量与主观感知差异的关键因素，这对考虑居民感知的城市绿化规划具有重要意义。
...
TL;DR: 提出了一种基于仿射对应的全局最优求解器，用于估计已知垂直方向的多相机系统广义相对位姿，通过解耦旋转矩阵和平移向量，将全局优化问题转化为多项式求解问题。


<details>
  <summary>Details</summary>
Motivation: 移动设备（如自动驾驶汽车）通常配备多相机系统和惯性测量单元（IMU），视觉和惯性信息的相对位姿估计在许多领域有重要应用。为提高多相机系统相对位姿估计的精度，需要开发更准确的求解方法。

Method: 1. 在解耦旋转矩阵和平移向量后，建立关于相对旋转角度的成本函数，最小化仿射对应几何约束的代数误差；2. 基于特征方程及其一阶导数为零，将全局优化问题转化为两个未知数的两个多项式；3. 使用多项式特征值求解器求解相对旋转角度，从特征向量获得平移向量；4. 当相对旋转较小时，提出新的线性解。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在临床应用中存在幻觉问题，模型往往依赖文本先验而非视觉证据。现有缓解策略各有局限：基于训练的方法依赖昂贵专家标注，可扩展性差；训练免费干预如对比解码虽然数据高效，但采用全局非针对性校正，在复杂临床环境中效果不可靠。
...
Conclusion: ARCD提供了一种针对性的区域特定指导策略，能够可靠地缓解医学视觉语言模型的幻觉问题，增强其在临床环境中的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在推理时采样冗余，缺乏高层多样性，限制了推理性能提升和强化学习训练效果

Method: 使用变分自编码器从问答对嵌入中推断潜在变量，解码为可学习的前缀标记，在生成前调制模型的内部推理轨迹，通过监督微调适应潜在条件

Result: 在多个推理基准测试中，该方法实现了对模型策略行为的可解释和可控控制，相比标准强化学习方法获得了一致的性能提升

Conclusion: Reasoning Palette框架通过潜在调制实现了结构化探索，显著提升了探索效率和持续学习能力，为语言模型的推理控制提供了新方法
...
TL;DR: CheXPO-v2是一个针对医学视觉语言模型幻觉问题的新对齐框架，通过过程监督和知识图谱一致性奖励机制，显著提升了临床可靠性和推理可验证性。
...
Method: 提出CheXPO-v2对齐框架，从结果监督转向过程监督。核心创新是知识图谱一致性奖励机制，通过实体关系匹配将推理步骤解析为结构化的"疾病、关系、解剖"三元组，在原子级别惩罚不连贯逻辑和幻觉。结合困难样本挖掘策略，提供细粒度监督。

Result: 在MIMIC-CXR-VQA等基准测试中显著优于GRPO和最先进模型。仅使用5k样本就实现了新的最先进准确率，展示了卓越的数据效率，同时产生临床可靠且可验证的推理。

Conclusion: CheXPO-v2通过过程监督和结构化知识图谱一致性奖励，有效解决了医学视觉语言模型的幻觉问题，在保持数据效率的同时显著提升了临床可靠性和推理可验证性。
...
TL;DR: AOM是一种通用遥感基础模型，能够处理任意波段组合、传感器类型和分辨率尺度，解决了现有遥感基础模型在波段缺失、跨传感器融合和分辨率变化时的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基础模型通常在固定的波段配置和分辨率下预训练，导致在实际应用中面临波段缺失、跨传感器融合和未见分辨率尺度等挑战，限制了模型的泛化能力和实际部署。

Method: 提出AOM模型，包含三个关键技术：1) 光谱独立分词器，为每个通道分配专用波段嵌入以保持光谱特征；2) 多尺度自适应补丁嵌入机制，动态调整感受野；3) 多尺度语义对齐机制和通道级自监督掩码重建预训练策略，联合建模光谱-空间关系。

Result: 在超过10个公开数据集（包括Sentinel-2、Landsat和HLS）上的实验表明，AOM在波段缺失、跨传感器和跨分辨率设置等挑战性条件下均实现了最先进的性能。

Conclusion: AOM是一个通用的遥感基础模型，能够有效处理任意波段组合、传感器类型和分辨率尺度，显著提升了遥感基础模型在实际复杂场景中的泛化能力和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的视觉定位方法仅从几何线索（如共视性图）推导全局描述符，限制了其判别能力，且在存在噪声几何约束时鲁棒性降低

Method: 提出聚合模块学习同时符合几何结构和视觉相似性的全局描述符；使用基于重叠分数的批量挖掘策略和修改后的对比损失，无需手动位置标签即可训练

Result: 在具有挑战性的基准测试中，在大规模环境中实现了显著的定位性能提升，同时保持了计算和内存效率

Conclusion: 该方法通过学习同时考虑几何和视觉一致性的全局描述符，有效纠正了由不可靠重叠分数引起的错误关联，提高了视觉定位的鲁棒性和准确性
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂长链视觉推理任务中表现出"视觉遗忘"现象，即随着推理链延长，模型逐渐失去视觉基础。作者认为这源于当前训练范式过早地将两种认知技能纠缠在一起：抽象逻辑推理（"如何思考"）和策略性视觉感知（"何时观看"）。

Method: 提出两阶段课程学习框架：1）解耦的监督微调课程，先在纯文本数据上构建鲁棒的抽象推理骨干，然后通过感知基础思维链范式将其锚定到视觉；2）将时机选择问题形式化为强化学习问题，设计关键感知奖励，通过将感知动作与认知不确定性语言标记（如"等待"、"验证"）耦合，教会模型何时观看。

Result: 该方法将模型从启发式驱动的观察者转变为策略性、有基础推理能力的推理器，解决了视觉遗忘问题，提升了复杂视觉推理任务的性能。

Conclusion: 通过解耦抽象推理和策略性视觉感知，并采用课程学习和强化学习方法，可以显著改善多模态大语言模型在长链视觉推理任务中的表现，使其成为更具战略性的基础推理器。
...
TL;DR: VideoDetective：一种用于长视频问答的高效问题感知记忆机制，通过迭代处理视频子片段和引入特殊记忆令牌来压缩关键信息，使MLLM能够在有限上下文长度下处理长达1小时的视频。


<details>
  <summary>Details</summary>
Motivation: 长视频问答面临巨大上下文和过载信息的挑战，导致内存消耗过高。现有方法通过减少视觉令牌或扩展上下文长度，可能丢失有用信息或计算量大。实际上，回答问题时只需要少量关键信息。

Method: 提出VideoDetective方法：1）迭代处理视频子片段；2）为每个子片段使用问题感知压缩策略，引入特殊记忆令牌进行有目的压缩；3）递归聚合和存储这些记忆令牌以更新历史上下文，供后续子片段重用。

Result: 实验表明，该方法使上下文长度仅32K的MLLM能够高效处理100K令牌（3600帧，1小时视频，1fps采样），仅需2分钟和37GB GPU内存。在多个长视频基准测试中，能更有效地从海量信息中寻找关键线索。
...
TL;DR: Mitty是一个基于扩散Transformer的视频上下文学习系统，能够直接从人类演示视频生成机器人执行视频，无需动作标签或中间表示，实现了端到端的人类到机器人视频生成。
...
Method: 基于预训练视频扩散模型构建Mitty，利用强大的视觉-时间先验知识。将演示视频压缩为条件标记，通过双向注意力与机器人去噪标记融合。开发自动合成流水线从大型第一人称数据集生成高质量人-机器人配对数据。

Result: 在Human2Robot和EPIC-Kitchens数据集上取得最先进的结果，展示了对未见环境的强大泛化能力，为从人类观察进行可扩展机器人学习提供了新见解。

Conclusion: Mitty通过视频上下文学习实现了端到端的人类到机器人视频生成，避免了中间表示的信息损失，为直接从人类演示视频进行机器人学习提供了有效框架。
...
TL;DR: AnyCXR是一个仅使用合成监督就能在任意胸部X光投影角度实现多器官分割的统一框架，通过多阶段域随机化和条件联合标注正则化策略，在合成数据上训练后能在真实世界数据上实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 胸部X光解剖分割面临两大挑战：1）全面标注数据稀缺；2）真实世界采集条件变化大。需要一种能够减少标注负担同时提高跨不同成像条件鲁棒性的解决方案。

Method: 提出AnyCXR框架，包含两个核心组件：1）多阶段域随机化引擎，从3D CT体积生成超过10万张解剖学忠实且高度多样化的合成X光片；2）条件联合标注正则化学习策略，通过在潜在空间中强制解剖一致性来利用部分和不完美的标签。

Result: 仅使用合成数据训练的AnyCXR在多个真实世界数据集上实现了强大的零样本泛化，能够准确分割54个解剖结构（包括PA、侧位和斜位视图）。分割结果支持下游临床任务，如自动心胸比估计、脊柱曲率评估和疾病分类，其中解剖先验的整合提高了诊断性能。
...
Method: 提出两阶段VLMIR框架：1）VLM特征提取阶段，通过LoRA微调对齐低质量/高质量图像描述的嵌入，使用退化预测器分解退化与干净图像内容嵌入；2）扩散模型修复阶段，通过交叉注意力机制整合视觉和文本嵌入进行增强修复。

Result: 广泛的实验和消融研究表明，VLMIR在通用和退化特定的图像修复任务中都取得了优越性能，证明了VLM中视觉和语言知识整合对提升图像修复能力的关键作用。

Conclusion: VLMIR框架通过有效利用视觉语言模型的视觉-语言先验知识，显著提升了图像修复性能，强调了视觉和语言知识整合在图像修复中的重要性。
...
TL;DR: RadImageNet-VQA是一个大规模放射学视觉问答数据集，包含75万张CT/MRI图像和750万个问答对，覆盖8个解剖区域和97种病理类别，旨在解决现有医学VQA数据集规模小、模态单一和存在文本捷径的问题。
...
Method: 构建了RadImageNet-VQA数据集，包含专家标注的750K张CT和MRI图像，配以7.5M个问答样本。数据集涵盖三个核心任务：异常检测、解剖结构识别和病理识别，支持开放式、封闭式和多项选择题型。

Result: 实验表明，最先进的视觉语言模型在细粒度病理识别任务上表现不佳，特别是在开放式设置中，即使经过微调也难以达到理想效果。纯文本分析显示，没有图像输入时模型性能接近随机水平，证实数据集不存在语言捷径问题。

Conclusion: RadImageNet-VQA填补了放射学VQA领域的大规模数据集空白，为评估和改进医学视觉语言模型提供了重要基准。数据集公开可用，将促进放射学AI研究的发展。
...
TL;DR: 提出HiSIR-Net框架，通过选择性输入重建门和区域优化补丁选择机制，实现高分辨率PCBA缺陷的像素级定位，在自建SIPCBA-500数据集上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: PCB组装板的自动缺陷检测面临三大挑战：标注数据不足、微缺陷只有几个像素、高分辨率图像视觉复杂。现有方法难以在真实4K分辨率板上实用化。

Method: 提出HiSIR-Net框架，包含两个轻量模块：1) 选择性输入重建门(SIR-Gate)，让模型决定在何处信任重建结果而非原始输入，减少无关重建伪影和误报；2) 区域级优化补丁选择(ROPS)方案，利用位置线索选择重叠补丁重建，在任意分辨率下保持一致性。

Result: 在自建的SIPCBA-500数据集和公共基准测试上进行广泛实验，展示了优越的定位性能，同时保持实用运行速度。异常图干净、高分辨率且误报率低。

Conclusion: HiSIR-Net通过有机整合选择性重建和区域优化机制，有效解决了高分辨率PCBA缺陷检测的挑战，为实际工业应用提供了实用解决方案。
...
TL;DR: ProCache：一种无需训练的动态特征缓存框架，通过非均匀缓存模式和选择性计算来解决DiT推理加速中的误差累积问题，实现近2-3倍加速且质量损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在生成建模中表现优异，但计算成本高阻碍实时部署。现有特征缓存方法存在两个关键问题：1）均匀缓存间隔无法匹配DiT的非均匀时间动态；2）简单特征重用会导致严重误差累积。

Method: 提出ProCache框架，包含两个核心组件：1）约束感知缓存模式搜索模块，通过离线约束采样生成非均匀激活计划；2）选择性计算模块，在缓存段内对深层块和高重要性token进行选择性计算，以最小开销缓解误差累积。

Result: 在PixArt-alpha和DiT上的实验表明，ProCache实现了高达1.96倍和2.90倍的加速，质量下降可忽略不计，显著优于现有基于缓存的方法。

Conclusion: ProCache通过分析DiT特征演化的时间深度变化特性，设计了动态特征缓存框架，有效解决了现有缓存方法的局限性，为DiT实时部署提供了高效解决方案。
...
Method: 构建RSHR-Bench基准，包含5,329张超高分辨率遥感图像（长边≥4000像素），设计四种任务家族：多项选择VQA、开放式VQA、图像描述和单图像评估。采用对抗性过滤结合强LLM，然后进行严格人工验证，减少语言先验依赖。

Result: 构建了3,864个VQA任务、3,913个图像描述任务和500个人工编写或验证的单图像评估VQA对。评估开源、闭源和遥感专用VLM，发现在超高分辨率场景下存在持续的性能差距。

Conclusion: RSHR-Bench为遥感视觉理解与推理提供了可靠的超高分辨率评估基准，揭示了现有模型在处理超高分辨率图像时的局限性，为未来模型发展提供了重要参考。
...
TL;DR: EMAG是一种无需训练的方法，通过修改扩散变换器中的注意力机制，生成更精细的负样本，提升生成质量
...
Result: EMAG相比CFG提升人类偏好分数(HPS)0.46分，能自然与APG、CADS等先进引导技术结合，进一步提升HPS

Conclusion: EMAG通过生成更精细的负样本，有效提升扩散和流匹配生成模型的质量和人类偏好
...
### [34] [InsertAnywhere: Bridging 4D Scene Geometry and Diffusion Models for Realistic Video Object Insertion](https://arxiv.org/abs/2512.17504)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成方法在可控视频编辑方面取得了进展，但真实的视频对象插入仍然具有挑战性，主要问题包括有限的4D场景理解以及对遮挡和光照效果处理不足。
...
Conclusion: InsertAnywhere通过结合4D场景理解和扩散模型联合合成，成功解决了视频对象插入中的几何一致性和外观真实性问题，为视频编辑提供了新的可能性。
...
TL;DR: DRIM模型通过深度可靠的多轮推理，在视觉语言模型中实现了自我反思和修正能力，解决了现有模型在推理轨迹错误时难以自我纠正的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在复杂视觉任务中通过思维链展现强大推理能力，但往往在推理轨迹错误时缺乏自我反思和修正能力，导致错误累积。

Method: 采用三阶段流程：1) 基于高分辨率图像构建高难度可验证的视觉问答对；2) 冷启动监督微调阶段收集工具轨迹数据；3) 强化学习阶段引入冗余惩罚策略优化，激励模型发展自我反思推理模式。

Result: 大量实验表明DRIM在视觉理解基准测试中取得了优越性能，验证了其深度可靠多轮推理的有效性。

Conclusion: DRIM通过结合数据构造、冷启动SFT和冗余惩罚RL，成功实现了视觉语言模型在复杂视觉任务中的自我反思和可靠多轮推理能力。
...
### [36] [Robust-R1: Degradation-Aware Reasoning for Robust Visual Understanding](https://arxiv.org/abs/2512.17532)
...
Result: 在真实世界退化基准R-Bench上超越所有通用和鲁棒基线，在MMMB、MMStar和RealWorldQA上对多强度对抗退化保持卓越的抗退化性能
...
TL;DR: CodeDance：使用可执行代码作为视觉推理通用求解器的新方法，通过代码定义、组合和执行来协调多个工具，实现透明、可自检的推理，在多项基准测试中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有开源方法主要依赖纯文本链、固定视觉模式或单步流程，在复杂任务中缺乏灵活性、可解释性和可迁移性。虽然最新模型如o3展示了类似人类的"图像思维"推理能力，但大多数方法仍存在局限性。

Method: 引入CodeDance框架，将可执行代码作为视觉推理的通用求解器。通过代码定义、组合和执行来协调多个工具，计算中间结果并渲染视觉工件（如方框、线条、图表）。引入平衡自适应工具调用的奖励机制，平衡探索与效率，避免工具过度使用。

Result: 在强化学习训练中观察到新颖的涌现行为：CodeDance展示了未见过的工具调用、组合方式和跨任务迁移能力。在视觉搜索、数学、图表问答等推理基准测试中，不仅持续优于模式驱动和纯文本基线，还超越了GPT-4o等先进闭源模型和更大的开源模型。

Conclusion: CodeDance通过可执行代码实现了通用、可扩展的视觉推理机制，无需任务特定微调即可获得跨任务迁移能力，为视觉推理提供了透明、可自检的新范式。
...
TL;DR: ClothHMR：一种用于穿着多样服装的3D人体网格恢复方法，通过服装裁剪和基础视觉模型对齐来准确估计人体形状和姿态


<details>
  <summary>Details</summary>
Motivation: 当前3D人体网格恢复方法主要针对紧身服装，在宽松服装下表现不佳。需要解决穿着多样服装时的人体形状和姿态估计问题。

Method: ClothHMR包含两个模块：1) 服装裁剪(CT)模块，通过身体语义估计和边缘预测裁剪服装使其贴合身体轮廓；2) 基于FHVM的网格恢复(MR)模块，通过将3D网格中间表示与基础人体视觉模型(FHVM)推断结果对齐来优化初始参数。

Result: ClothHMR在基准数据集和真实场景图像中显著优于现有最先进方法，能准确恢复穿着多样服装的3D人体网格，并已开发用于在线时尚购物的Web应用。

Conclusion: ClothHMR通过服装裁剪和基础视觉模型对齐，有效解决了穿着多样服装时的3D人体网格恢复问题，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在分布偏移的下游任务中表现不佳，现有的参数高效微调方法受限于固定的人工提示，无法充分理解类别语义。虽然已有研究提出图像诱导提示，但会带来过高的推理计算开销。

Method: ADK框架利用大语言模型离线为每个类别生成丰富的描述性提示，预计算特征后以两种方式部署：1) 组合知识（平均表示）提供丰富语义；2) 实例特定知识（轻量非参数注意力机制）动态选择最相关描述。该框架作为即插即用的参数无关组件增强现有PEFT方法。

Result: ADK持续提升多种PEFT基线的性能，在各种场景下创造了新的最先进水平。

Conclusion: ADK框架通过高效丰富文本表示而不损害效率，有效解决了视觉语言模型在少样本适应中的语义理解不足问题，为下游任务提供了有效的辅助知识增强方案。
...
Result: 统一模型在不同肿瘤类型上获得良好Dice分数：术前脑膜瘤88.65%、术前转移瘤80.08%、术前胶质瘤90.92%、术后胶质瘤84.60%（BraTS）、术前低级别胶质瘤84.47%、术后低级别胶质瘤61.27%。性能与数据集特定模型相当，具有跨肿瘤类型和采集时间点的泛化能力。

Conclusion: 统一模型在多种脑肿瘤类型和采集时间点上实现了可比较的分割性能，具有良好的泛化能力，便于临床部署，已集成到开源软件Raidionics中用于中枢神经系统肿瘤分析。
...
### [41] [MGRegBench: A Novel Benchmark Dataset with Anatomical Landmarks for Mammography Image Registration](https://arxiv.org/abs/2512.17605)
*Svetlana Krasnova,Emiliya Starikova,Ilia Naletov,Andrey Krylov,Dmitry Sorokin*

Main category: cs.CV

TL;DR: 提出了MGRegBench，首个大规模公开的乳腺X光配准基准数据集，包含5000+图像对和100个手动标注的解剖标志点，用于标准化评估和公平比较不同配准方法。


<details>
  <summary>Details</summary>
Motivation: 乳腺X光配准对疾病进展追踪和纵向变化监测至关重要，但缺乏公开数据集和标准化基准限制了该领域的发展。现有研究使用私有数据和不一致的评估框架，导致结果不可比较。

Method: 创建了MGRegBench公开基准数据集，包含5000多个图像对，其中100个包含手动解剖标志点和分割掩码。使用该数据集对多种配准方法进行基准测试，包括经典方法（ANTs）、学习基方法（VoxelMorph、TransMorph）、隐式神经表示（IDIR）、经典乳腺X光特定方法和最新的深度学习方法MammoRegNet。

Result: MGRegBench成为最大的公开2D配准数据集之一，提供了首个同类方法在乳腺X光配准上的公平比较，并对基于深度学习的配准方法进行了深入分析。

Conclusion: 通过发布代码和数据，为乳腺X光配准研究建立了基础资源，促进了公平比较和未来研究发展。该基准将有助于标准化评估和推动该领域的技术进步。
...
TL;DR: EMMA是一个评估概念擦除技术的新基准，测试5个关键维度和12个指标，发现现有方法在处理间接提示和视觉相似概念时存在困难，某些方法甚至加剧了性别和种族偏见。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成的广泛应用引发了隐私、偏见和版权侵权等担忧。概念擦除技术提供了有前景的解决方案，但现有评估通常局限于有限概念集，依赖过于简单直接的提示，无法真正测试这些技术是否从模型表示中完全移除了目标概念。

Method: 提出了EMMA基准，评估概念擦除技术的五个关键维度（包括12个指标）：超越标准指标如图像质量和时间效率，测试在挑战性条件下的鲁棒性，包括间接描述、视觉相似的非目标概念、潜在的性别和种族偏见，提供方法行为的社会意识分析。使用EMMA分析了五个概念擦除方法在五个领域（物体、名人、艺术风格、NSFW和版权）。

Result: 现有方法在处理间接提示（即当被擦除概念被间接引用时仍能生成该概念）和视觉相似的非目标概念（即无法生成与被擦除概念相似的非目标概念）方面存在困难。某些方法相比原始模型甚至放大了性别和种族偏见。

Conclusion: 概念擦除技术需要更全面的评估框架，现有方法在鲁棒性和社会意识方面存在显著不足，特别是在处理间接提示和避免偏见放大方面需要改进。EMMA基准为未来概念擦除技术的发展提供了重要的评估工具。
...
Method: 从纵向鹿特丹研究中采样彩色眼底图像，使用定制标注界面让标注员在初始血管分割掩码基础上，分别在独立图层上标注动脉、静脉和未知血管，并通过连通分量可视化工具验证和校正连通性。

Result: 数据集包含1024×1024像素PNG图像，提供三种模态：原始RGB眼底图像、对比度增强版本和RGB编码的动静脉掩码。图像质量差异大，包含通常被自动质量评估系统排除但具有重要血管信息的挑战性样本。

Conclusion: 该数据集提供了丰富且异质性的彩色眼底图像资源，包含高质量分割标注，支持在真实世界图像质量和采集条件变化下进行机器学习模型的稳健基准测试和训练。
...
### [44] [An Empirical Study of Sampling Hyperparameters in Diffusion-Based Super-Resolution](https://arxiv.org/abs/2512.17675)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在解决逆问题（如单图像超分辨率）方面表现出强大潜力，但现有的条件化方法如DPS和MCG引入了需要仔细调优的超参数。本文旨在识别影响预训练扩散模型条件化性能的主要因素。

Method: 在FFHQ超分辨率任务上进行实证消融研究，分析条件化步长和扩散步数对性能的影响，确定主导因素。

Result: 研究发现条件化步长对性能的影响显著大于扩散步数，步长在[2.0, 3.0]范围内能获得最佳整体性能。

Conclusion: 在应用条件化到预训练扩散模型时，条件化步长是比扩散步数更关键的超参数，最优步长范围为[2.0, 3.0]，这为超分辨率任务中的参数调优提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频帧预测在动态场景中因缺乏下一帧信息而产生预测误差，事件相机通过高时间分辨率捕获像素亮度变化，但现有事件驱动方法通过光流预测和像素扭曲会产生空洞和模糊问题

Method: 提出DESSERT框架：1）ER-VAE（事件到残差对齐变分自编码器）对齐事件帧与残差；2）扩散模型基于事件数据去噪残差潜在表示；3）引入DLT（多样长度时间）增强提高鲁棒性

Result: 实验结果表明，该方法在事件重建、图像视频帧预测、事件视频帧预测和单边事件视频帧插值等任务中均优于现有方法，生成更清晰、时间一致的帧合成
...
TL;DR: 研究者开发了Polysome工具生成合成指令，创建了HISTAI-Instruct数据集，并训练了ANTONI-α视觉语言模型，在病理学WSI视觉问答任务上超越了MedGemma。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在病理学应用中存在局限：要么只关注WSI中的小区域，要么只提供静态的切片级输出，或者依赖非公开数据限制了可复现性。同时，包含详细临床报告的WSI配对训练数据稀缺，阻碍了透明且可泛化的VLM发展。

Method: 1. 开发Polysome标准化工具用于合成指令生成；2. 将Polysome应用于公开的HISTAI数据集，生成HISTAI-Instruct指令调优数据集（24,259张切片，110万+指令-响应对）；3. 使用该数据集训练ANTONI-α视觉语言模型，支持视觉问答。

Result: ANTONI-α在组织识别、肿瘤检测和鉴别诊断等WSI级视觉问答任务上超越了MedGemma。同时研究了不同数据量训练的ANTONI-α变体性能。所有方法、数据和代码均已公开。

Conclusion: 通过开发标准化合成指令生成工具、创建大规模公开指令调优数据集，成功训练出在病理学视觉问答任务上表现优异的视觉语言模型，为病理学AI助手的发展提供了透明、可复现的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统显式变形方法在准确运动传递和恢复缺失区域方面存在困难，而基于注意力的变形方法虽然有效，但复杂度高且几何基础薄弱。需要一种既能保持几何准确性又能处理遮挡和变形区域的方法。

Method: 提出三阶段渐进式框架：1）显式变形模块使用3D密集光流进行源图像和驱动图像的粗空间对齐；2）参考增强校正模块利用多个参考图像的3D关键点和纹理特征的交叉注意力来语义补全遮挡或变形区域；3）置信度引导融合模块通过学习的置信度图进行空间自适应融合，平衡结构对齐和视觉一致性。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测器存在泛化能力不足的问题，它们往往过度拟合特定生成模型的语义特征，而无法有效检测来自未见生成模型的图像。这主要是因为检测器依赖了脆弱的、非本质的语义模式作为捷径，而非学习通用的生成痕迹。

Method: 提出一种简单但有效的像素级映射预处理步骤，通过破坏图像的像素值分布来打破检测器通常利用的语义捷径。这种方法迫使检测器关注图像生成过程中更基础、更可泛化的高频痕迹。

Result: 在GAN和扩散模型生成器上的综合实验表明，该方法显著提升了最先进检测器的跨生成器性能。进一步分析验证了破坏语义线索是提升泛化能力的关键这一假设。

Conclusion: 通过像素级映射预处理破坏语义捷径，能够有效提升AI生成图像检测器的跨生成器泛化能力，使检测器关注更本质的生成痕迹而非特定模型的语义特征。
...
TL;DR: 提出情感图像过滤（AIF）任务，将文本中的抽象情感转化为视觉具象图像，创建情感共鸣结果。开发了AIF-B和AIF-D模型，后者利用预训练扩散模型的生成先验，在内容一致性和情感保真度上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户通过文本和图像表达情感，但现有方法难以将文本中的抽象情感有效转化为视觉具象的图像表达。需要开发能够从文本中提取情感并反映到图像中的技术。

Method: 1. 提出AIF任务并构建相应数据集；2. 开发AIF-B模型，基于多模态transformer架构；3. 提出AIF-D模型，作为AIF-B的扩展，利用预训练大规模扩散模型的生成先验实现更深层的情感反映。

Result: 定量和定性实验表明，AIF模型在内容一致性和情感保真度方面优于最先进方法。广泛的用户研究实验证明，AIF模型在唤起特定情感方面显著更有效。

Conclusion: AIF模型在将文本情感转化为视觉表达方面具有重要价值和潜力，为情感驱动的图像生成提供了新的技术途径。
...
### [50] [Interpretable Plant Leaf Disease Detection Using Attention-Enhanced CNN](https://arxiv.org/abs/2512.17864)
*Balram Singh,Ram Prakash Sharma,Somnath Dey*

Main category: cs.CV

TL;DR: 提出了一种基于注意力机制的CBAM-VGG16模型，用于植物叶片病害检测，通过集成卷积块注意力模块提升特征提取和病害定位能力，在多个数据集上达到高精度并具有良好的可解释性。
...
Result: 模型在多个数据集上优于现有技术，达到最高98.87%的准确率，表现出强大的泛化能力。可解释性分析验证了模型能够有效定位病害区域，为农业诊断提供透明可靠的解决方案。
...
### [51] [RadarGen: Automotive Radar Point Cloud Generation from Cameras](https://arxiv.org/abs/2512.17897)
*Tomer Borreda,Fangqiang Ding,Sanja Fidler,Shengyu Huang,Or Litany*

Main category: cs.CV

TL;DR: RadarGen是一种扩散模型，可从多视角相机图像合成逼真的汽车雷达点云，通过BEV表示雷达测量值，并利用预训练基础模型提取的深度、语义和运动线索指导生成过程。


<details>
  <summary>Details</summary>
Motivation: 当前需要一种能够从视觉数据生成逼真雷达点云的方法，以实现跨传感模态的统一生成式仿真，并利用现有视觉数据集和仿真框架的兼容性。

Method: 采用高效的图像潜在扩散模型，将雷达测量表示为鸟瞰图形式，编码空间结构、雷达横截面和速度属性；通过轻量级恢复步骤从生成的地图重建点云；利用预训练基础模型提取的BEV对齐深度、语义和运动线索指导生成过程。

Result: 在大规模驾驶数据上的评估表明，RadarGen能够捕捉特征性雷达测量分布，并缩小与在真实数据上训练的感知模型之间的差距。

Conclusion: RadarGen标志着向跨传感模态统一生成式仿真迈出了一步，为多模态生成式仿真提供了可扩展的方向。
...
<details>
  <summary>Details</summary>
Motivation: 洪水检测对于灾害响应和风险评估至关重要，但现有数据集在地理范围和标注细节方面有限，阻碍了鲁棒、通用计算机视觉方法的发展。

Method: 创建AIFloodSense数据集，包含470张高分辨率航拍图像，来自64个国家230次洪水事件（2022-2024），支持三个任务：图像分类（环境类型、相机角度、大陆识别）、语义分割（洪水、天空、建筑物像素级掩码）和视觉问答。
...
### [53] [Adversarial Robustness of Vision in Open Foundation Models](https://arxiv.org/abs/2512.17902)
*Jonathon Fox,William J Buchanan,Pavlos Papadopoulos*

Main category: cs.CV

TL;DR: 该研究评估了LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2在视觉对抗攻击下的鲁棒性，发现尽管Llama 3.2 Vision基准准确率较低，但在对抗攻击下性能下降更小，表明对抗鲁棒性与标准性能不一定相关。
...
Method: 研究对LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2进行无目标PGD（投影梯度下降）攻击，针对视觉输入模态，在VQA v2数据集子集上进行实证评估，使用标准VQA准确率指标量化攻击效果。

Result: Llama 3.2 Vision虽然在此设置下的基准准确率较低，但在攻击下表现出更小的性能下降，特别是在较高扰动水平下。视觉模态被证实是降低当代开放权重视觉语言模型性能的可行攻击向量。

Conclusion: 对抗鲁棒性不一定与标准基准性能直接相关，可能受底层架构和训练因素的影响。视觉模态是当代视觉语言模型的有效攻击向量，需要进一步研究提升模型对抗鲁棒性的方法。
...
### [54] [Re-Depth Anything: Test-Time Depth Refinement via Self-Supervised Re-lighting](https://arxiv.org/abs/2512.17908)
*Ananta R. Bhattarai,Helge Rhodin*

Main category: cs.CV

TL;DR: Re-Depth Anything是一个测试时自监督框架，通过融合Depth Anything V2与大规模2D扩散模型的先验知识，解决单目深度估计在真实世界图像中的域适应问题。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型如Depth Anything V2在处理远离训练分布的图像时表现不佳，存在显著的域适应差距。需要一种方法能够利用生成模型的强大先验来弥合这一差距。

Method: 提出测试时自监督框架，通过重新光照预测的深度图并增强输入图像，在生成上下文中利用形状从阴影(SfS)线索和分数蒸馏采样(SDS)替代传统光度重建。采用针对性优化策略：冻结编码器，仅更新中间嵌入并微调解码器。

Result: 在多个基准测试中，Re-Depth Anything相比Depth Anything V2在深度准确性和真实性方面取得显著提升，展示了通过增强几何推理实现自监督的新途径。

Conclusion: 该方法成功弥合了单目深度估计中的域适应差距，通过融合几何模型与生成先验，为自监督深度估计开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的驾驶视频编辑方法难以实现精细的自然语言控制和真实感，特别是在多对象行为编辑方面存在局限性。需要一种能够理解复杂用户指令并生成逼真交通场景的系统。

Method: 1) 使用显式3D场景分解将驾驶视频表示为包含静态背景和动态对象的场景图；2) 采用智能体管道，其中Orchestrator将用户指令转换为执行图，协调专业智能体和工具；3) Object Grounding Agent建立文本描述与场景图中目标对象的对应关系；4) Behavior Editing Agent从语言指令生成多对象轨迹；5) Behavior Reviewer Agent迭代审查和优化生成的轨迹；6) 使用视频扩散工具渲染和精炼编辑后的场景图。

Result: LangDriveCTRL支持对象节点编辑（移除、插入和替换）和从单一自然语言指令进行多对象行为编辑。在定量评估中，其指令对齐度比先前最先进方法高出近2倍，具有更优的结构保持性、照片真实感和交通真实感。

Conclusion: LangDriveCTRL通过结合3D场景表示和智能体协作，实现了对真实世界驾驶视频的高质量自然语言控制编辑，为合成多样化交通场景提供了有效的解决方案。
...
TL;DR: 3D-RE-GEN是一个从单张图像重建可编辑3D纹理网格场景的框架，解决了现有方法在物体分解、空间关系和背景缺失方面的问题，满足视觉特效和游戏开发的艺术工作流程需求。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然视觉效果出色，但生成的表示形式不便于艺术家在视觉特效和游戏开发工作流程中进行修改。现有纹理网格场景重建方法存在物体分解错误、空间关系不准确和背景缺失等问题，无法满足艺术家的实际需求。

Method: 采用组合式框架，集成资产检测、重建和放置模型，将特定领域的最先进模型结合使用。将遮挡物体获取视为图像编辑任务，使用生成模型在一致的照明和几何条件下进行场景级推理。引入4自由度可微分优化来对齐重建物体与估计的地平面，生成完整的背景以在优化过程中空间约束物体。

Result: 3D-RE-GEN在单图像3D场景重建中实现了最先进的性能，通过精确相机恢复和空间优化指导的组合生成，产生了连贯、可修改的场景。

Conclusion: 该框架通过组合特定领域的最先进模型，解决了艺术家工作流程中的关键问题，能够生成满足视觉特效和游戏开发需求的物理真实、可编辑的3D场景。
...
TL;DR: TwinSegNet：一种隐私保护的联邦学习框架，结合混合ViT-UNet模型和个性化数字孪生，用于准确、实时的脑肿瘤分割，在保持数据隐私的同时实现高性能。


<details>
  <summary>Details</summary>
Motivation: 当前脑肿瘤分割的深度学习方法依赖于集中式数据收集，存在隐私问题且在不同机构间泛化能力有限。需要一种既能保护隐私又能实现高性能分割的解决方案。

Method: 提出TwinSegNet框架，整合混合ViT-UNet模型（卷积编码器+Vision Transformer瓶颈）和个性化数字孪生。每个机构在私有数据上微调全局模型形成自己的数字孪生，通过联邦学习实现隐私保护。

Result: 在9个异构MRI数据集（包括BraTS 2019-2021和自定义肿瘤数据集）上评估，TwinSegNet达到高达0.90的Dice分数，敏感性和特异性超过90%，在非独立同分布客户端分布中表现出鲁棒性。

Conclusion: TwinSegNet在保持数据隐私的同时不牺牲性能，相比集中式模型（如TumorVisNet）表现优异，为多机构临床环境提供了可扩展、个性化的分割解决方案。
...
Method: LumiCtrl包含三个基本组件：(a) 基于物理的沿普朗克轨迹的光照增强，在标准光照下创建微调变体；(b) 使用冻结ControlNet的边缘引导提示解耦，确保提示专注于光照而非结构；(c) 掩码重建损失，专注于前景物体学习，同时允许背景进行上下文适应（上下文光适应）。

Result: 与现有T2I定制方法相比，LumiCtrl在光照保真度、美学质量和场景一致性方面表现显著更好。人类偏好研究进一步证实用户对LumiCtrl输出有强烈偏好。

Conclusion: LumiCtrl提供了一种有效的单图像光照个性化方法，能够精确控制生成图像的照明效果，为内容设计师提供了更好的工具来操纵图像的情绪和视觉美感。
...
Method: 构建了MMLANDMARKS数据集，包含197k高分辨率航拍图像、329k地面视图图像、文本信息和地理坐标，涵盖18,557个美国地标。所有模态之间存在一一对应关系。采用简单的CLIP启发式基线模型进行验证。

Result: 该数据集支持多种地理空间任务，包括跨视图地面到卫星检索、地面和卫星地理定位、文本到图像检索、文本到GPS检索。基线模型展现出广泛的泛化能力和与现成基础模型及专业SOTA模型竞争的性能。
...
TL;DR: GroundingME是一个评估多模态大语言模型视觉定位能力的新基准，通过四个维度系统性地挑战模型，揭示了当前模型在真实世界复杂场景下的显著能力差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法捕捉真实世界的复杂性，无法评估MLLMs是否真正具备人类水平的视觉定位能力，还是仅仅在简化数据集上进行模式匹配。需要创建能够系统评估模型在复杂、模糊和不可定位场景下表现的基准。

Method: 通过自动生成与人工验证相结合的方式，创建了包含1,005个挑战性示例的GroundingME基准，涵盖四个关键维度：区分性（区分高度相似对象）、空间性（理解复杂关系描述）、有限性（处理遮挡或微小对象）和拒绝性（识别不可定位查询）。评估了25个最先进的MLLMs，并探索了两种改进策略：测试时缩放和混合数据训练。

Result: 评估结果显示显著的能力差距：最佳模型准确率仅为45.1%，大多数模型在拒绝任务上得分为0%，倾向于幻觉对象而非承认其不存在。改进策略使复杂定位任务准确率提升2.9%，拒绝准确率从0%提升至27.9%。

Conclusion: GroundingME揭示了当前MLLMs在视觉定位方面的严重局限性，特别是在处理模糊参考和识别不可定位查询方面存在重大安全风险。该基准既是诊断当前模型局限性的工具，也为实现人类水平视觉定位提供了路线图。
...
TL;DR: 该研究首次在中东地区（伊拉克库尔德斯坦）验证了AI在前列腺癌诊断和Gleason分级中的性能，结果显示AI与病理学家的一致性达到病理学家间一致性水平，且在不同扫描仪（包括低成本紧凑型扫描仪）上表现稳定。


<details>
  <summary>Details</summary>
Motivation: 目前病理AI系统的性能评估主要基于欧美大型医疗中心的数据，缺乏对代表性不足人群（如中东地区）的验证研究。这些地区可能从AI支持中获益最大，因此需要进行全球范围内的AI验证研究以确保公平采用。

Method: 收集了伊拉克库尔德斯坦地区2013-2024年间的339个前列腺活检标本（来自185名连续患者），数字化后使用三种扫描仪模型（Hamamatsu、Leica、Grundium）进行评估。测试了任务特定的端到端AI模型和两个基础模型，评估它们与病理学家的一致性以及在不同扫描仪上的一致性。

Result: AI与病理学家的分级一致性（Cohen's二次加权kappa 0.801）与病理学家间的一致性（0.799）相似（p=0.9824）。所有AI模型和扫描仪对之间的跨扫描仪一致性都很高（二次加权kappa > 0.90），包括低成本紧凑型扫描仪。

Conclusion: AI模型在前列腺组织病理学评估中达到了病理学家水平的性能。紧凑型扫描仪为非数字化环境下的验证研究提供了途径，使样本量有限的实验室能够经济有效地采用AI。这是中东地区首个公开可用的数字病理数据集，支持进一步研究全球公平的AI病理学。
...
TL;DR: PathBench-MIL是一个用于组织病理学中多实例学习的开源AutoML和基准测试框架，自动化端到端MIL流程构建，提供可复现的模型和特征提取器基准测试。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中的多实例学习缺乏标准化的自动化工具和基准测试框架，导致实验重复性差、模型比较困难，需要统一的平台来加速研究和标准化流程。

Method: 开发了一个开源AutoML框架，自动化MIL流程的预处理、特征提取和MIL聚合，集成了可视化工具、统一配置系统和模块化扩展架构，支持多种MIL模型和特征提取器的基准测试。

Result: 创建了PathBench-MIL框架，实现了端到端MIL流程的自动化，提供了数十种MIL模型和特征提取器的可复现基准测试，支持快速实验和跨数据集任务的标准化。

Conclusion: PathBench-MIL为组织病理学中的多实例学习研究提供了标准化的AutoML和基准测试平台，通过自动化流程和可扩展架构促进了该领域的快速发展和标准化。
...
TL;DR: G3Splat通过引入几何先验来解决仅使用视图合成损失训练3D高斯场景表示时的几何模糊性问题，在几何一致性重建、相对姿态估计和新视角合成方面达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视图合成损失来预测3D高斯参数，但这种自监督方式不足以恢复几何上有意义的splat表示，存在几何模糊性问题。

Method: 提出G3Splat方法，在自监督的姿势无关泛化splatting中，通过强制几何先验来获得几何一致的3D场景表示，解决3D高斯splat学习的模糊性问题。

Result: 在RE10K数据集上训练，在几何一致性重建、相对姿态估计和新视角合成方面达到最先进性能，并在ScanNet上表现出强大的零样本泛化能力。

Conclusion: 仅使用视图合成损失不足以学习几何有意义的3D高斯splat，引入几何先验的G3Splat方法能有效解决这一问题，获得几何一致的场景表示。
...
TL;DR: 本文提出了一种针对视频快照压缩成像中一热调制的算法，将重建任务转化为生成式视频修复问题，结合一步回归初始化和一步扩散精炼，并采用双光路硬件设计来缓解空间退化。


<details>
  <summary>Details</summary>
Motivation: 视频快照压缩成像中的随机二进制调制会导致时间混叠问题，而一热调制虽然能实现完美的时间解耦，但目前缺乏充分利用其潜力的算法。

Method: 1) 利用一热调制的解耦特性，将重建任务转化为生成式视频修复问题，设计与硬件压缩过程对齐的随机微分方程前向过程；2) 提出结合一步回归初始化和一步扩散精炼的新框架；3) 在硬件层面实现双光路设计，利用互补信息增强修复视频质量。

Result: 在合成数据集和真实场景上的实验证明了该方法的有效性，这是首次将扩散模型集成到视频SCI重建中的工作。

Conclusion: 本文提出的针对一热调制的算法框架成功解决了视频SCI中的时间混叠问题，通过结合回归初始化、扩散精炼和双光路硬件设计，实现了高质量的视频重建。
...
TL;DR: 该研究系统评估了241个生物医学图像分析竞赛的公平性，发现现有基准数据集存在代表性不足、访问受限等问题，揭示了排行榜成功与临床相关性之间的脱节。


<details>
  <summary>Details</summary>
Motivation: 尽管AI医学影像基准竞赛对定义性能标准和推动方法进步至关重要，但目前尚不清楚这些基准是否提供足够代表性、可访问和可重用的数据来支持具有临床意义的AI。研究旨在评估基准竞赛在公平性方面的两个维度：数据集是否代表真实临床多样性，以及是否符合FAIR原则（可查找、可访问、可互操作、可重用）。

Method: 研究者对241个生物医学图像分析竞赛（包含458个任务，涵盖19种成像模态）进行了大规模系统研究，评估数据集的地理位置分布、模态类型、问题类型等方面的代表性，以及数据访问条件、许可实践和文档完整性等可访问性和可重用性指标。

Result: 研究发现数据集组成存在显著偏见，包括地理位置偏见、模态相关偏见和问题类型偏见，表明当前基准不能充分反映真实世界的临床多样性。尽管这些竞赛数据集影响广泛，但经常受到限制性或模糊的访问条件、不一致或不合规的许可实践以及不完整文档的约束，限制了可重复性和长期重用。

Conclusion: 这些缺陷暴露了基准生态系统中的基本公平性限制，并突显了排行榜成功与临床相关性之间的脱节。研究强调需要更公平、更具代表性的基准来推动临床有意义的AI发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法依赖大量标注数据且计算成本高。基于多模态大语言模型的免调优方法虽然前景广阔，但依赖文本输出会导致信息损失、正常性偏差和提示敏感性，难以捕捉细微异常线索。

Method: 提出HeadHunt-VAD范式，通过鲁棒头部识别模块系统评估所有注意力头的显著性和稳定性，识别出稀疏的专家头部子集。将这些头部的特征输入轻量级异常评分器和时序定位器，实现高效准确的异常检测。

Result: 在两个主要VAD基准测试中，HeadHunt-VAD在免调优方法中实现了最先进的性能，同时保持高效率，验证了在MLLM中进行头部级探测作为实际异常检测解决方案的有效性。

Conclusion: HeadHunt-VAD通过直接定位MLLM中异常敏感的注意力头，避免了文本生成的信息损失问题，为现实世界异常检测提供了强大而实用的解决方案，证明了头部级探测在MLLM中的有效性。
...
TL;DR: 本文提出了一种用于多输出模型的半监督学习方法，通过额外的旋转和翻转增强，不依赖模型响应的概率特性，适用于医学图像分割任务。


<details>
  <summary>Details</summary>
Motivation: 医学图像多类分割中，多输出CNN模型能分别预测不同分割类别，提高准确性，但需要大量高质量标注数据。标注3D数据成本高且耗时，而现有的半监督学习方法对多输出模型研究不足。

Method: 提出基于额外旋转和翻转的半监督学习方法，不假设模型响应的概率特性，适用于具有共同编码器但输出层分支的多输出架构。

Result: 该方法在ImageTBDA数据集上用于B型主动脉夹层诊断，能够准确分割真腔、假腔和假腔血栓三个关键类别，解决了数据标注不足的问题。

Conclusion: 提出的半监督学习方法为多输出模型提供了一种通用解决方案，特别适用于需要单独分割的医学图像分析架构，有助于克服标注数据稀缺的挑战。
...
TL;DR: StereoMV2D是一个多视角3D目标检测框架，通过集成时序立体建模来增强2D检测引导的3D检测器，利用相邻帧间的跨时间视差提升深度感知能力，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 多视角3D目标检测需要在检测精度和计算效率之间取得平衡。现有的MV2D方法利用2D检测结果初始化3D查询，但单帧2D检测存在深度模糊问题，限制了3D查询生成的准确性。

Method: 提出StereoMV2D框架，将时序立体建模集成到2D检测引导的多视角3D检测器中：1）利用相邻帧间同一目标的跨时间视差增强深度感知；2）在2D感兴趣区域内高效计算；3）引入动态置信度门控机制，通过学习帧间匹配矩阵的统计模式和外观一致性来自适应评估时序立体线索的可靠性。

Result: 在nuScenes和Argoverse 2数据集上的大量实验表明，StereoMV2D在检测性能上表现优异，且没有显著增加计算开销。

Conclusion: StereoMV2D通过时序立体建模有效解决了单帧2D检测的深度模糊问题，提升了多视角3D目标检测的精度，同时保持了计算效率，为自动驾驶感知任务提供了更可靠的解决方案。
...
TL;DR: PathFLIP是一个用于全切片图像（WSI）解释的病理学细粒度语言-图像预训练框架，通过将幻灯片级描述分解为区域级子标题，生成文本条件区域嵌入，实现精确的视觉-语言对齐，在多个下游任务中表现优异且需要较少训练数据。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在计算病理学中取得了进展，但全切片图像的千兆像素尺度和空间异质性仍然给多模态理解带来挑战。现有的对齐方法难以在数千个图像块中捕捉文本描述与视觉线索之间的细粒度对应关系，影响了下游任务的性能。

Method: PathFLIP框架将幻灯片级描述分解为区域级子标题，生成文本条件区域嵌入以促进精确的视觉-语言对齐。利用大型语言模型（LLMs）来遵循多样化的临床指令并适应不同的诊断场景。该框架支持多种范式，包括幻灯片级分类和检索、细粒度病变定位以及指令跟随。

Result: 在四个代表性基准测试中，PathFLIP优于现有的大规模病理学VLMs，同时需要显著更少的训练数据。该框架在幻灯片级分类和检索、细粒度病变定位以及指令跟随等多个任务中展现出卓越性能。

Conclusion: PathFLIP为临床实践中的细粒度、指令感知的全切片图像解释铺平了道路，通过改进视觉-语言对齐和减少训练数据需求，在计算病理学中实现了更精确的多模态理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI检测方法基于封闭世界假设，只能识别预定义的小规模动词集，难以泛化到现实世界中未见或模糊的交互。多模态大语言模型具有丰富的世界知识，但直接微调计算成本过高，无法与现有HOI检测器结合。

Method: 提出GRASP-HO框架：1) 提取混合交互表示；2) 设计轻量级可学习的认知引导模块，将细粒度视觉证据注入冻结的MLLM进行推理；3) 引入混合指导策略，结合语言建模损失和辅助分类损失，解决分类数据集与生成模型的监督不匹配问题。

Result: 实验表明，该方法在封闭集性能上达到最先进水平，并展现出强大的零样本泛化能力，实现了判别式感知与生成式推理的无缝桥接。

Conclusion: GRASP-HO成功将HOI检测从封闭集分类任务转变为开放词汇生成问题，通过轻量级引导模块有效结合视觉感知与语言推理，为开放世界HOI检测提供了统一的范式。
...
TL;DR: ReCo提出了一种新的教学视频编辑范式，通过联合去噪和正则化约束建模，解决了基于指令的视频编辑中编辑区域不准确和token干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然上下文生成范式在图像编辑中表现出色，但在视频编辑中面临挑战：未指定编辑区域会导致编辑不准确，以及在去噪过程中编辑区域和非编辑区域之间的token干扰问题。

Method: 1) 宽度方向拼接源视频和目标视频进行联合去噪；2) 引入潜在正则化和注意力正则化：潜在正则化增加编辑区域的潜在差异，减少非编辑区域的差异；注意力正则化抑制编辑区域token对源视频对应token的注意力，减少干扰；3) 构建了包含50万对指令-视频的大规模高质量数据集ReCo-Data。

Result: 在四个主要的基于指令的视频编辑任务上进行的广泛实验证明了该方法的优越性。

Conclusion: ReCo通过约束建模解决了教学视频编辑中的关键问题，为上下文视频编辑提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI视频分析技术已能精确量化面部表情、头部动作等行为特征，但现有工具主要面向工程人员，软件栈复杂，难以提供直接支持假设驱动研究的行为测量，阻碍了行为科学家和临床研究人员采用这些现代工具。

Method: Bitbox遵循可重复性、模块化和可解释性原则，提供标准化接口，利用多个面部、头部和身体处理器从视频中提取高级行为测量。核心模块已在临床样本上测试验证，设计上支持轻松添加新测量指标。

Result: Bitbox作为开源工具包，旨在弥合转化鸿沟：为行为研究人员提供无需工程专业知识的高质量行为指标，同时为计算机科学家提供向实际应用领域传播方法的机制。

Conclusion: Bitbox将加速计算行为测量在行为、临床和心理健康研究中的整合，并设计为社区驱动项目，通过方法开发者和领域科学家的贡献持续发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D头像重建方法通常需要相机姿态、表情标签或多视图输入，限制了实际应用。需要一种更灵活的方法，能从任意数量、任意姿态的图像重建高质量3D头像，并支持详细的动态变形。

Method: 1. 基于Transformer的重建模型，使用结构化头部查询标记作为规范锚点，聚合灵活数量的无相机姿态、无表情标签的输入；2. 轻量级UNet解码器，基于UV空间位置图生成详细的表情相关变形；3. 数据分布调整策略平衡训练集中关键表情（如皱纹、露齿）的分布；4. 10秒轻量级细化增强特定身份细节。

Result: 实验表明FlexAvatar在3D一致性、动态真实感方面优于先前方法，提供实用的可动画3D头像创建解决方案，支持实时详细变形。

Conclusion: FlexAvatar为从单张或稀疏图像创建高质量可动画3D头像提供了灵活实用的解决方案，无需相机姿态或表情标签，支持实时动态变形，具有优越的3D一致性和真实感。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要基于模拟环境或人类驾驶车辆数据，缺乏真实ADAS车辆在风险条件下的行为数据，特别是边缘案例如险情事件和系统失效。

Method: 从公开社交媒体内容收集2,119个第一人称视角视频，进行视频帧级标注（碰撞、规避动作、系统失效），并提出整合语义分割和单目深度估计的实时碰撞时间计算框架，使用广义极值分布建模极端风险。

Result: 数据集包含多样化的地理位置、光照和天气条件；提出的框架能有效计算动态物体的实时碰撞时间；GEV分布能准确量化不同道路类型的极端风险；VLLM基准测试显示领域适应能显著提升模型在复杂险情场景中的性能。

Conclusion: SAVeD填补了ADAS安全研究的数据空白，为分析感知和决策失效提供了重要资源，提出的分析方法为ADAS安全评估提供了新工具。
...
TL;DR: MambaMIL+：一种新的多实例学习框架，通过重叠扫描、选择性条纹位置编码和上下文标记选择机制，在计算病理学中有效建模全切片图像的超长序列和空间上下文，在20个基准测试中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSIs）具有千兆像素分辨率且缺乏细粒度标注，传统深度学习模型难以处理。多实例学习（MIL）将WSI视为补丁级实例的包，但有效建模超长序列和丰富空间上下文仍然困难。Mamba虽然对长序列学习有线性扩展优势，但在空间上下文建模和记忆衰减方面存在局限。

Method: 提出MambaMIL+框架：1）重叠扫描：重构补丁序列以嵌入空间连续性和实例相关性；2）选择性条纹位置编码（S2PE）：编码位置信息同时减轻固定扫描顺序的偏差；3）上下文标记选择（CTS）机制：利用监督知识动态扩大上下文记忆以实现稳定的长程建模。

Result: 在20个基准测试（诊断分类、分子预测和生存分析）上进行了广泛实验，使用三种特征提取器（ResNet-50、PLIP和CONCH），MambaMIL+始终实现最先进的性能，展示了其在计算病理学中的有效性和鲁棒性。

Conclusion: MambaMIL+成功解决了Mamba在空间上下文建模和记忆衰减方面的限制，通过显式集成空间上下文同时保持长程依赖建模而不遗忘记忆，为大规模计算病理学提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着图像生成技术的进步，高度逼真的合成媒体广泛传播，使得深度伪造检测变得更加困难。现有检测器在训练数据有限的情况下，面对未见过的生成模型时泛化能力不足，需要开发能够跨多种生成技术识别合成内容的通用检测方法。

Method: 1. 构建Diff-Gen大规模基准数据集，包含10万张扩散生成图像，捕捉广泛的光谱伪影；2. 提出AdaptPrompt参数高效迁移学习框架，联合学习任务特定的文本提示和视觉适配器，同时冻结CLIP主干；3. 通过层剪枝技术，移除视觉编码器的最后一个Transformer块，增强高频生成伪影的保留。

Result: 在25个具有挑战性的测试集（涵盖GAN、扩散模型和商业工具生成的合成内容）上评估，在标准和跨域场景中均达到最先进水平。仅使用320张图像即可实现少样本泛化，并在闭集设置中实现生成器架构的精确溯源。

Conclusion: 通过结合Diff-Gen数据集和AdaptPrompt框架，成功开发出能够跨多种生成技术识别合成内容的通用深度伪造检测系统，显著提升了检测器的泛化能力和准确性，为应对日益复杂的合成媒体挑战提供了有效解决方案。
...
Method: 提出UrbanDIFF，一种纯粹基于空间信息的去噪扩散模型，用于重建云污染的LST图像。模型以静态城市结构信息（建成区表面数据和数字高程模型）为条件，在推理过程中通过监督像素引导细化步骤强制与无云像素保持一致。

Result: 在20-85%云覆盖的合成云掩模实验中，UrbanDIFF始终优于插值基线，特别是在密集云遮挡下，在85%云覆盖时达到SSIM 0.89、RMSE 1.2K、R² 0.84，且随着云密度增加性能下降更慢。

Conclusion: UrbanDIFF证明了去噪扩散模型在空间LST重建中的有效性，特别是在高云覆盖情况下，为连续城市热岛监测提供了可靠的云污染填补解决方案。
...
TL;DR: Chorus是一个多教师预训练框架，通过从2D基础模型蒸馏互补信号，学习整体的前馈3D高斯泼溅场景编码器，实现从高层语义到细粒度结构的特征学习。


<details>
  <summary>Details</summary>
Motivation: 虽然3DGS已成为高保真场景表示方法，但从其基元直接编码丰富、通用的特征仍然未被充分探索。现有方法缺乏从3DGS原语中学习通用特征的能力。

Method: 提出Chorus框架，使用共享的3D编码器和教师特定投影器，从语言对齐、通用和对象感知的教师模型中蒸馏学习，构建共享嵌入空间。还提出了渲染-蒸馏适应方法以促进域外微调。

Result: 在开放词汇语义和实例分割、线性和解码器探测以及数据高效监督等任务上表现优异。仅使用高斯中心、颜色和估计法线作为输入的变体在点云基准测试中表现优于基线，同时使用训练场景数减少39.9倍。

Conclusion: Chorus成功地从3DGS原语中学习通用特征表示，通过多教师蒸馏实现了从高层语义到细粒度结构的全面特征捕获，展示了强大的迁移能力和数据效率。
...
TL;DR: InSPECT是一种新颖的扩散模型，通过在正向和反向过程中保持不变的谱特征，解决了传统扩散模型将数据完全扩散为白噪声导致的预测任务困难问题。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型将数据完全扩散为白噪声，然后重建图像，这导致了极其困难且计算不可行的预测任务。为了克服这一限制，需要一种能够保持关键特征同时维持生成多样性的方法。

Method: 提出InSPECT模型，在正向和反向过程中保持不变的谱特征。在正向过程结束时，傅里叶系数平滑收敛到指定的随机噪声，既保持特征又维持多样性和随机性。

Result: 在CIFAR-10、Celeb-A和LSUN数据集上的实验表明，InSPECT相比DDPM在10K次迭代下平均降低FID 39.23%，提高IS 45.80%，显示出更好的生成质量和多样性，同时计算效率更高，收敛速度更快。
...
TL;DR: 研究发现视觉语言模型在视觉提示基准测试中对看似无关的细节（如标记颜色、大小等）异常敏感，这些细节会显著影响模型性能排名，甚至能让较弱模型超越较强模型。


<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型时，需要测试模型独立分析视觉内容的能力，而不受文本先验的影响。现有视觉提示基准测试（如BLINK）存在脆弱性问题，模型对视觉标记的微小变化反应过度，影响评估的可靠性。

Method: 评估了9个常用开源和闭源VLM在两个视觉提示任务上的表现，分析了基准测试设置细节（视觉标记设计、数据集大小）对模型性能和排名的影响，并创建了VPBench基准测试，包含16种视觉标记变体。

Result: 视觉标记的微小变化（如颜色从红变蓝）能完全改变模型排名；增加视觉标记大小可使开源模型InternVL3-8B达到或超越大型专有模型如Gemini 2.5 Pro的性能；JPEG压缩级别等低级推理选择也会影响模型排名。

Conclusion: 视觉提示基准测试对实现细节异常敏感，这些细节的影响远大于传统语义VLM评估。为缓解这种不稳定性，创建了VPBench基准测试和相应分析工具，以提供更稳健的评估框架。
...
Method: 提出关键点计数分类器（KCCs），利用ViT自动识别图像间匹配关键点的能力，将任何训练好的ViT模型转换为自解释模型，无需重新训练，创建易于可视化在输入中的解释性决策过程。

Result: 广泛评估显示，KCCs相比现有基线方法改善了人机通信效果，为ViT基础模型提供了更好的透明度和可靠性。

Conclusion: KCCs是使ViT基础模型更加透明和可靠的重要一步，提供了一种实用且有效的自解释模型构建方法。
...
TL;DR: MAGNet是一个统一的自回归扩散框架，用于多智能体运动生成，支持灵活的条件设置和采样，能够处理从双人互动到多人互动的各种任务，并能生成超长序列。


<details>
  <summary>Details</summary>
Motivation: 理解和生成多人互动是机器人和社交计算领域的基础挑战。现有方法大多是任务特定的，无法泛化到灵活的多智能体生成，且难以处理长时间跨度、强智能体间依赖和可变群体规模等问题。

Method: MAGNet基于Diffusion Forcing框架，引入了关键修改来显式建模自回归去噪过程中的智能体间耦合，实现跨智能体的协调。该框架支持双人预测、伙伴修复和完整多智能体运动生成，采用可扩展架构，对智能体数量不敏感。

Result: MAGNet在双人基准测试中与专门方法表现相当，同时自然地扩展到涉及三个或更多人的多智能体场景。能够捕捉紧密同步活动（如舞蹈、拳击）和松散结构化的社交互动，并能自回归生成数百帧的超长序列。

Conclusion: MAGNet提供了一个统一的框架，能够灵活处理各种多智能体互动任务，解决了现有方法在泛化性和可扩展性方面的限制，为机器人和社交计算中的多人互动建模提供了有效解决方案。
...
Method: 提出Dexterous World Model (DWM)，这是一个场景-动作条件化的视频扩散框架。该方法通过两个条件进行视频生成：(1) 遵循指定相机轨迹的静态场景渲染，确保空间一致性；(2) 第一人称手部网格渲染，编码几何和运动线索以直接建模动作条件化的动态。使用混合交互视频数据集进行训练，合成数据提供完全对齐的监督，真实世界视频贡献多样且逼真的物体动态。

Result: 实验表明，DWM能够实现逼真且物理合理的人类-场景交互，如抓取、打开和移动物体，同时保持相机和场景的一致性。该框架代表了基于视频扩散的交互式数字孪生的第一步。

Conclusion: DWM框架为从第一人称动作进行具身模拟提供了新途径，是构建交互式数字孪生的重要进展，能够生成空间一致、动作驱动的动态场景变化。
...
TL;DR: 本文提出了一种系统框架，将面向理解的编码器特征适配到生成任务中，通过语义-像素重建目标来正则化潜在空间，实现了语义信息和细粒度细节的高度紧凑表示，在文本到图像和图像编辑任务中取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现代潜在扩散模型通常在低层次VAE潜在空间中运行，主要优化像素级重建。为了统一视觉生成和理解，当前趋势是采用表示编码器的高维特征作为生成潜在表示。然而，作者发现两个基本障碍：1）判别性特征空间缺乏紧凑正则化，导致扩散模型容易产生偏离流形的潜在表示，造成不准确的对象结构；2）编码器固有的弱像素级重建能力阻碍生成器学习准确的细粒度几何和纹理。

Method: 提出了一个系统框架来适配面向理解的编码器特征用于生成任务。引入了语义-像素重建目标来正则化潜在空间，能够将语义信息和细粒度细节压缩到高度紧凑的表示中（96个通道，16×16空间下采样）。基于这种表示，设计了统一的文本到图像和图像编辑模型。

Result: 与各种特征空间相比，该方法实现了最先进的重建效果、更快的收敛速度，在文本到图像和编辑任务中取得了显著的性能提升。验证了表示编码器可以有效地适配为强大的生成组件。

Conclusion: 通过语义-像素重建目标正则化潜在空间，能够创建既语义丰富又具有出色重建能力的紧凑表示，成功地将面向理解的编码器特征适配到生成任务中，实现了视觉生成和理解的有效统一。
...
TL;DR: 该研究创建了首个女性健康基准测试WHB，评估了13个先进大语言模型在女性健康领域的表现，发现模型失败率约60%，在紧急情况识别等方面存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型成为数百万人获取健康信息的主要来源，其在女性健康领域的准确性尚未得到充分检验。需要建立专门的基准来评估LLM在女性健康方面的表现，以揭示潜在风险和改进空间。

Method: 研究者开发了女性健康基准测试WHB，包含96个经过严格验证的模型测试用例，涵盖5个医学专业（妇产科、急诊医学、初级保健、肿瘤学和神经学）、3种查询类型（患者查询、临床医生查询、证据/政策查询）和8种错误类型（剂量/药物错误、缺失关键信息、过时指南/治疗建议、错误治疗建议、错误事实信息、缺失/错误鉴别诊断、错过紧急情况、不适当建议）。评估了13个最先进的大语言模型。

Result: 评估结果显示当前模型在女性健康基准测试中失败率约为60%，性能在不同专业和错误类型间差异显著。所有模型在"错过紧急情况"指标上普遍表现不佳，而GPT-5等较新模型在避免不适当建议方面有显著改进。

Conclusion: AI聊天机器人目前尚不能提供可靠的女性健康建议，需要进一步改进模型在女性健康领域的准确性和安全性。研究强调了建立专门基准的重要性，并为未来模型改进提供了方向。
...
TL;DR: Struct-SQL框架通过结构化推理蒸馏训练小型语言模型，在Text-to-SQL任务上比非结构化CoT蒸馏提升8.1%，显著减少语法错误


<details>
  <summary>Details</summary>
Motivation: 企业级Text-to-SQL系统面临成本、安全和性能的三难困境：需要在昂贵专有的大语言模型和性能低下的小语言模型之间做出选择。现有方法依赖非结构化思维链蒸馏，存在固有模糊性。

Method: 提出Struct-SQL知识蒸馏框架，使用查询执行计划作为结构化推理蓝图，训练小型语言模型模仿强大LLM的推理过程，采用结构化思维链进行蒸馏。

Result: 使用结构化CoT蒸馏的SLM相比非结构化CoT蒸馏基线实现了8.1%的绝对提升，详细错误分析显示关键改进因素是语法错误的显著减少。

Conclusion: 结构化逻辑蓝图有助于SLM生成更可靠的SQL，证明了结构化推理表示能提供更清晰、可靠的教学信号，特别适合需要明确精确逻辑步骤的Text-to-SQL任务。
...
TL;DR: XLM是一个用于快速实现小型非自回归语言模型的Python包，旨在解决该领域缺乏标准化工具的问题，并提供预训练模型供研究社区使用。


<details>
  <summary>Details</summary>
Motivation: 非自回归文本生成领域缺乏标准化的训练和推理库，现有实现多为定制化方案，难以进行系统比较，且每个模型都需要独立的数据整理、损失函数和预测逻辑，组件复用困难。

Method: 开发了XLM Python包，专注于简化小型非自回归语言模型的实现过程，同时提供配套的xlm-models包包含预训练模型，代码开源在GitHub上。

Result: 成功创建了一个标准化工具包，使研究人员能够更快地实现非自回归语言模型，并提供了可供社区使用的小型预训练模型集合。

Conclusion: XLM包解决了非自回归语言建模领域工具标准化不足的问题，通过提供统一的实现框架和预训练模型，促进了该领域研究的可重复性和比较性。
...
TL;DR: SPECTRA是一种水印技术，能在训练数据仅占训练语料库不到0.001%的情况下可靠检测出LLM是否使用了特定数据，通过文本重述和评分模型匹配实现，检测时比较嫌疑模型和评分模型的token概率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在从互联网抓取的大规模文本语料上进行训练，训练数据检测对于执行版权和数据许可变得至关重要，需要一种能在数据占比极低时仍能可靠检测的方法。

Method: SPECTRA通过使用LLM对文本进行重述，并根据单独的评分模型为每个重述版本分配分数。选择分数与原始文本最接近的重述版本，以避免引入分布偏移。检测时，比较嫌疑模型的token概率与评分模型的token概率。
...
TL;DR: 论文提出对话主题分割的新评估框架，强调边界密度和片段连贯性比传统精确匹配F1更重要，发现现有基准的性能差异主要源于标注粒度不匹配而非模型质量改进。


<details>
  <summary>Details</summary>
Motivation: 对话主题分割对摘要、检索、记忆管理和对话连续性至关重要，但现有评估仍依赖严格的边界匹配和F1指标，而现代LLM对话系统依赖分割管理超出固定上下文窗口的对话历史，不恰当的评估方法导致效率低下和连贯性下降。

Method: 引入新的评估目标，将边界密度和片段连贯性作为主要标准，同时使用窗口容忍F1（W-F1）；在八个对话数据集上评估多种结构不同的分割策略，涵盖任务导向、开放域、会议风格和合成交互。

Result: 研究发现：1）跨数据集报告的性能差异主要由标注粒度不匹配和稀疏边界标签驱动，而非模型质量；2）观察到高片段连贯性与相对于稀疏标签的极端过分割相结合，导致误导性的低精确匹配F1分数；3）许多报告的改进源于评估伪影而非边界检测改进。
...
TL;DR: 提出了一种基于高亮引导的生成方法，利用句子级信息作为内容规划来提升长文本摘要的可追溯性和忠实度。
...
Method: 采用高亮引导生成框架，通过自规划方法识别重要内容，然后基于规划生成摘要。探索了端到端和两阶段两种变体。

Result: 在长文本摘要数据集上，该方法持续改善事实一致性，同时保持相关性和整体质量。在GovReport数据集上，ROUGE-L提升了4.1分，SummaC分数提高了约35%。

Conclusion: 高亮引导的摘要方法有助于保留重要细节，在不同领域产生更准确和深入的摘要，两阶段管道在长且信息密集的文档上表现更好。
...
TL;DR: MiA-RAG为RAG系统引入全局语义表征（mindscape），通过分层摘要构建全局上下文，提升长文本任务中的检索和生成能力。


<details>
  <summary>Details</summary>
Motivation: 人类理解长文本时依赖全局语义表征，而现有RAG系统缺乏这种全局上下文指导，导致在长文本任务中表现不佳。

Method: 通过分层摘要构建mindscape（全局语义表征），并以此指导检索和生成过程：检索器形成增强的查询嵌入，生成器在连贯的全局上下文中对检索证据进行推理。

Result: 在多样化的长文本和双语基准测试中，MiA-RAG在基于证据的理解和全局意义构建方面持续超越基线方法。

Conclusion: MiA-RAG通过引入全局语义表征，使RAG系统能够将局部细节与连贯的全局表示对齐，实现更接近人类的长文本检索和推理能力。
...
TL;DR: 提出一个针对波斯语等低资源语言的噪声敏感ASR纠错框架，通过多假设生成和噪声感知建模，显著降低噪声环境下的词错误率。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在噪声环境下性能显著下降，特别是对于波斯语等低资源语言。即使是Whisper等最先进模型在不同信噪比下也难以保持准确性，需要更鲁棒的纠错方法。

Method: 使用噪声波斯语音生成5个最佳假设，引入错误级别噪声（ELN）表示语义和标记级别的假设差异，量化噪声引起的语言失真。评估三种模型：基础LLaMA-2-7B、仅文本微调变体、以及集成句子和单词级别ELN嵌入的噪声条件模型。

Result: ELN条件模型显著降低词错误率。在混合噪声测试集上，提出的微调+ELN模型将WER从原始Whisper的31.10%降至24.84%，显著优于仅文本微调基线的30.79%。原始LLaMA-2-7B模型将WER增至64.58%，表明其无法独立纠正波斯语错误。

Conclusion: 结合多假设和噪声感知嵌入的方法对于噪声现实场景中的鲁棒波斯语ASR是有效的，ELN提供了噪声引起不确定性的直接度量，使LLM能够在纠错过程中推理每个假设的可靠性。
...
TL;DR: 该研究旨在改进或创建新的NLP模型，用于从文本输入中自动生成主观问题并评估答案，以帮助教师评估学生作业并增强学生的学习体验。


<details>
  <summary>Details</summary>
Motivation: 虽然客观问题生成已有较多研究进展，但自动主观问题生成和答案评估仍处于发展阶段。自动化系统可以帮助教师评估学生作业，并让学生通过自我评估来增强学习体验。

Method: 研究计划改进现有NLP模型或创建新的模型，用于从文本输入中自动生成主观问题并评估答案。

Result: 摘要中未提供具体实验结果，但描述了研究目标：开发自动化主观问题生成和答案评估系统。

Conclusion: 该研究旨在通过改进或创建新的NLP模型，实现自动主观问题生成和答案评估，以支持教育领域的评估和学习过程。
...
<details>
  <summary>Details</summary>
Motivation: 针对大语言模型在多语言低资源环境下适应时面临的计算资源限制、数据质量问题和跨语言性能不平衡等挑战，需要一种既能保持模型性能又能控制计算成本的微调方法。

Method: 核心算法结合梯度对齐的低秩更新与结构化正交变换，通过层间混合和引入选定子层的酉约束来稳定深度优化，同时配合轻量级无标签数据治理步骤（语言识别、近重复去除、质量过滤）。

Result: 在XNLI和FLORES数据集上，混合方法相比强PEFT基线获得一致提升，保持方向平衡并改善概率校准；对轻量级正字法变体更具韧性；简单治理步骤带来叠加效益；训练足迹测量显示适度开销和有利的成本-质量边界。
...
TL;DR: 论文提出CANON LAYERS架构组件，通过受控合成预训练任务评估模型能力，发现这些轻量级组件能促进相邻token间的水平信息流，提升推理深度、知识操作等能力，使弱架构达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 在学术规模预训练（如13亿参数，1000亿token）中，语言模型的架构差异分析面临挑战，结果常被噪声和随机性主导。需要一种方法来隔离和评估核心模型能力。
...
Conclusion: 合成预训练框架为隔离常被学术规模噪声掩盖的核心模型能力提供了经济、有原则的路径。通过无限高质量数据，甚至能预测未来架构在训练流程改进后的行为，解锁更深层次的推理和分层推断能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在代码生成任务中严重依赖有监督训练，需要大量标注数据或未标注代码片段，这些数据获取成本高且难以大规模获得。为了解决这一限制，需要开发不依赖外部语料的无监督方法。

Method: IPC框架包含四个核心组件：问题空间探测、测试理解探测、解决方案空间探测、知识整合与强化。通过探测LLMs内部知识和置信度模式，结合自一致性机制和基于表示的质量估计来识别可靠的代码候选，训练UCoder（无监督学习的编码器）。

Result: 在多个代码基准测试中验证了该方法，无监督方法能够达到与有监督方法竞争的性能，同时显著减少对标注数据和计算资源的依赖。分析实验表明模型内部状态包含丰富的代码质量和正确性信号。
...
TL;DR: 提出了CulturalToM-VQA评估基准，包含5095个问题，用于通过视觉问答测试跨文化心理理论推理能力，涵盖不同文化背景下的仪式、服饰、手势等文化线索。
...
Method: 采用VLM辅助的人机协作流程：人类专家首先策划包含不同文化传统、仪式和社会互动的图像；VLM协助生成结构化的心理理论场景描述；然后精炼成涵盖六种心理理论任务和四个复杂度等级的问题-答案对。

Result: 创建了CulturalToM-VQA数据集，包含5095个问题，涵盖心理状态归因、错误信念推理、非字面沟通、社会规范违反、视角协调和多智能体推理等六个心理理论方面，以及四个复杂度等级。

Conclusion: 该工作为评估AI系统在多样化文化背景下的心理理论推理能力提供了系统化基准，超越了以西方为中心的评估标准，有助于推动更具文化包容性的人工智能发展。
...
TL;DR: 本文提出了一种基于置信度加权和可信度感知的集成框架用于文本情感检测，受孔多塞陪审团定理启发。该方法结合了架构多样的小型Transformer模型，通过双权重投票机制（全局可信度和局部置信度）动态加权模型贡献，在DAIR-AI数据集上取得了93.5%的宏F1分数，超越了包括7B参数大模型在内的现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统集成方法通常依赖同质架构，而本文旨在通过结合架构多样的小型Transformer模型来提升情感检测性能。受孔多塞陪审团定理启发，作者希望利用不同模型的独特偏置，同时保持误差多样性，以构建更稳健、参数效率更高的情感检测系统。

Method: 1. 结合五种架构多样的小型Transformer模型：BERT、RoBERTa、DistilBERT、DeBERTa和ELECTRA，每个模型都针对情感分类进行了完全微调。
2. 最小化参数收敛以保持误差多样性，同时利用每个模型的独特偏置。
3. 采用双权重投票机制：结合全局可信度（验证集F1分数）和局部置信度（实例级概率）来动态加权模型贡献。
4. 在DAIR-AI数据集上进行实验验证。

Result: 1. 在DAIR-AI数据集上取得了93.5%的宏F1分数，超越了现有最佳基准。
2. 显著优于包括Falcon、Mistral、Qwen和Phi在内的大规模语言模型，即使这些模型经过任务特定的低秩适配（LoRA）。
3. 总参数仅为595M，比参数高达7B的模型更参数高效且更稳健。
4. 证明了精心设计的小型微调模型集成可以在专门的自然语言处理任务（如情感检测）中超越更大的语言模型。

Conclusion: 本文提出的置信度加权、可信度感知集成框架通过结合架构多样的小型Transformer模型，在文本情感检测任务中取得了卓越性能。该方法不仅超越了现有最佳方法，还证明了在专门的自然语言处理任务中，精心设计的小型模型集成可以比更大的语言模型更有效、更参数高效。这为资源受限环境下的高性能情感检测系统提供了有前景的解决方案。
...
TL;DR: 研究发现大语言模型存在可探测和引导的人格特质线性方向，这些方向在人格检测中有效，但在引导模型行为时效果受上下文限制


<details>
  <summary>Details</summary>
Motivation: 大语言模型展现出的不同人格特质显著影响用户信任和参与度，但现有的人格特征化和控制方法要么成本高昂（后训练），要么脆弱（提示工程）。线性方向探测和引导作为一种廉价高效的方法值得探索。

Method: 使用Llama 3.3 70B模型生成406个虚构角色及其大五人格特质分数描述，通过Alpaca问卷问题采样隐藏激活，利用线性回归学习激活空间中的每层方向，测试这些方向在人格探测和行为引导中的有效性。

Result: 与特质分数对齐的线性方向在人格检测中是有效的探测器，但其引导能力强烈依赖于上下文：在强制选择任务中产生可靠效果，但在开放式生成或提示中存在额外上下文时影响有限。

Conclusion: 线性方向可以作为大语言模型人格特质探测的有效工具，但在行为引导方面需要更复杂的策略来应对不同上下文情境的挑战。
...
TL;DR: 论文介绍了simulstream，这是首个专门用于流式语音到文本翻译系统统一评估和演示的开源框架，解决了现有工具SimulEval的局限性。
...
Method: 开发了simulstream开源框架，专门用于长语音处理，支持增量解码和重翻译方法，提供统一的质量和延迟比较框架，并包含交互式Web界面用于系统演示。

Result: simulstream成为首个专门用于流式语音到文本翻译系统评估和演示的统一框架，支持多种解码方法比较，解决了现有工具的局限性。

Conclusion: simulstream框架为流式语音到文本翻译研究提供了统一的评估和演示平台，支持长音频处理、多种解码方法比较，并具备交互演示功能，填补了该领域工具空白。
...
TL;DR: 该论文提出了一种用于注意力编码器-解码器ASR模型的上下文偏置方法，通过多令牌预测来提升罕见命名实体的识别准确率
...
Method: 提出基于候选命名实体列表的上下文偏置方法，通过同时预测多个未来令牌，让模型能够"窥视未来"并对实体列表中的候选实体进行评分。该方法直接利用多令牌预测逻辑，无需额外的实体编码器或交叉注意力层

Result: 在Librispeech数据集上的实验表明，与基线AED模型相比，该方法在命名实体词错误率方面实现了最高50.34%的相对改进

Conclusion: 该方法通过简单的多令牌预测机制有效提升了ASR模型对罕见命名实体的识别能力，同时保持了较低的结构复杂度
...
TL;DR: 论文研究了用户生成内容（UGC）翻译的评估挑战，提出了非标准现象分类和翻译行动框架，并通过LLM案例研究展示了翻译指南对评估结果的重要影响。


<details>
  <summary>Details</summary>
Motivation: UGC中普遍存在非标准语言现象（如拼写错误、俚语、表情符号等），这使得评估UGC翻译变得特别困难，因为"好"翻译的标准取决于输出所需的标准化程度。

Method: 分析了四个UGC数据集的人工翻译指南，推导出12种非标准现象分类和5种翻译行动（标准化、复制、转移、省略、审查）。通过大型语言模型（LLMs）的案例研究，探索不同提示指令对翻译评分的影响。

Result: 研究发现不同数据集对UGC的处理方式存在显著差异，导致参考翻译的标准化程度形成连续谱系。LLM翻译评分对包含明确UGC翻译指令的提示高度敏感，当提示与数据集指南一致时评分会提高。

Conclusion: 当需要保留UGC风格时，公平的评估要求模型和指标都能理解翻译指南。作者呼吁在数据集创建时提供清晰指南，并开发可控的、指南感知的UGC翻译评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 计算情感科学和计算社会科学研究通常需要标注语言数据，但现有资源和算法难以被发现、访问和使用，特别是对于计算机科学领域外的研究者来说存在显著障碍

Method: 构建ABCDE数据集，从社交媒体、博客、书籍和AI生成来源收集超过4亿条文本话语，并标注与计算情感和社会科学相关的广泛特征

Result: 创建了一个大规模、多源、多特征标注的数据集，包含情感、身体、认知、人口统计和情绪等维度，为跨学科研究提供资源

Conclusion: ABCDE数据集能够促进情感科学、认知科学、数字人文、社会学、政治学和计算语言学等多个领域的跨学科研究，解决了现有资源访问困难的问题
...
TL;DR: AncientBench是一个专门评估大语言模型对古文字理解能力的基准测试，特别关注出土文献场景，包含四个维度和十个任务，旨在推动大语言模型在考古学和古汉语领域的应用。


<details>
  <summary>Details</summary>
Motivation: 现有中文基准测试主要针对现代汉语和传世文献的古汉语，缺乏对出土文献古文字理解的评估。随着大语言模型的快速发展，需要专门的基准来评估其对古文字的理解能力，以促进大语言模型在考古学和古汉语领域的应用。

Method: 提出AncientBench基准，分为四个维度对应古文字理解的四种能力：字形理解、发音理解、意义理解和上下文理解。包含十个具体任务，如部首、声旁、同音字、完形填空、翻译等。召集考古研究人员进行实验评估，提出古文字模型作为基线，并对当前表现最佳的大语言模型进行广泛实验。

Result: 实验结果显示大语言模型在古文字场景中具有巨大潜力，但与人类专家仍存在差距。基准测试为评估大语言模型对古文字的理解提供了全面框架。

Conclusion: AncientBench填补了现有基准测试在出土文献古文字理解评估方面的空白，有助于推动大语言模型在考古学和古汉语领域的发展和应用，揭示了模型在该领域的潜力和改进方向。
...
TL;DR: 本文针对孟加拉语医疗实体识别任务，提出了一种多BERT集成方法，在缺乏标注数据的低资源语言环境下取得了89.58%的最高准确率，比单层BERT提升了11.80%。


<details>
  <summary>Details</summary>
Motivation: 医疗实体识别在医学领域自动化系统开发中至关重要，但目前研究主要集中在英语等资源丰富的语言，孟加拉语等低资源语言的研究相对匮乏。本文旨在填补这一空白，推动低资源语言的医疗NLP发展。

Method: 1. 比较了多种Transformer模型（BERT、DistilBERT、ELECTRA、RoBERTa）在孟加拉语医疗实体识别任务上的表现；2. 提出了一种新颖的多BERT集成方法；3. 为孟加拉语医疗实体识别任务构建了高质量标注数据集。

Result: 多BERT集成方法在所有基线模型中表现最佳，达到89.58%的最高准确率，比单层BERT模型提升了11.80%的准确率。通过多种性能指标验证了模型的鲁棒性和适用性。

Conclusion: 多BERT集成模型在孟加拉语医疗实体识别任务中展现出显著优势，为低资源语言的医疗NLP研究奠定了基础，证明了集成方法在解决低资源语言NLP任务中的有效性。
...
TL;DR: DEER是一个评估专家级深度研究报告的基准，包含50个跨13个领域的报告写作任务，提供专家评估框架和文档级事实核查架构。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型能够生成专家级报告，但现有评估基准缺乏系统性专家报告标准，LLM评估难以捕捉需要专家判断的问题，且源验证通常只覆盖有限引用而非整个报告的事实可靠性。

Method: DEER包含50个跨13个领域的报告写作任务，建立专家评估分类法（7个维度，25个子维度，130个细粒度评分项），提供任务特定专家指导，并提出文档级事实核查架构，提取并验证报告中所有声明（包括引用和未引用的）。

Result: DEER与人类专家判断高度相关，能够产生可解释的系统优势和弱点诊断，量化外部证据质量，提供更全面的报告评估。

Conclusion: DEER为评估专家级深度研究报告提供了系统、全面的基准，通过专家评估框架和文档级事实核查，解决了现有评估方法的局限性，为LLM生成报告的质量评估提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有公共数据集将大语言模型视为通用文本生成器，剥离了界面上下文，而界面设计实际上塑造了用户交互。需要保留原生平台特性（如推理痕迹、来源链接、代码工件）的数据集来理解真实的用户-LLM聊天机器人交互。

Method: 从五个主要平台（ChatGPT、Claude、Gemini、Perplexity、Grok）的公开共享URL中收集对话数据，构建包含142,808个对话和超过660,000轮交互的大规模跨平台语料库，保留原生平台特性，覆盖101种语言，时间跨度为2023年4月至2025年10月。

Result: 创建了ShareChat数据集，具有更长的上下文窗口和更大的交互深度，通过三个代表性分析展示了其多面性用途：1)分析对话完整性以衡量用户意图满意度；2)评估内容生成中的来源引用行为；3)进行时间分析以跟踪演化使用模式。
...
TL;DR: 本文提出了一种高效的实体集扩展图推理方法，支持局部增量导航而无需构建完整图结构
...
Method: 形式化扩展图推理任务，检查两个元组是否属于可比较、不可比较或相同的节点，在限制输入或实体描述的现实假设下实现高效算法

Result: 在现实假设下（如限制输入或实体描述），这些推理任务可以高效实现，支持扩展图的局部增量导航

Conclusion: 通过高效的推理任务实现，可以在不需要构建完整扩展图的情况下支持实际应用，实现实体集扩展图的局部增量导航
...
TL;DR: 该论文提出了科学通用智能（SGI）的操作性定义，基于实践探究模型（PIM），并创建了包含1000多个跨学科样本的SGI-Bench基准，用于评估大语言模型在科学研究任务中的表现，同时引入了测试时强化学习（TTRL）来提升假设新颖性。


<details>
  <summary>Details</summary>
Motivation: 尽管科学AI有所进展，但缺乏一个连贯的科学通用智能（SGI）框架，即能够自主构思、研究和跨科学领域推理的能力。需要建立操作性定义和系统评估方法来推动AI真正参与科学发现。

Method: 1. 基于实践探究模型（PIM：审议、构思、行动、感知）建立SGI操作性定义；2. 通过四个科学家对齐任务（深度研究、想法生成、干/湿实验、实验推理）实现操作化；3. 创建SGI-Bench基准，包含1000多个专家策划的跨学科样本，灵感来自《科学》杂志的125个重大问题；4. 引入测试时强化学习（TTRL），在推理时优化检索增强的新颖性奖励。

Result: 评估结果显示多个差距：深度研究的精确匹配率低（10-20%），尽管步骤级对齐较好；想法缺乏可行性和细节；干实验代码可执行性高但执行结果准确性低；湿实验协议序列保真度低；多模态比较推理存在持续挑战。TTRL方法能够在不依赖参考答案的情况下增强假设新颖性。

Conclusion: 基于PIM的定义、以工作流为中心的基准和实证见解为真正参与科学发现的AI系统奠定了基础，揭示了当前模型的局限性，并提出了改进方向，特别是通过TTRL方法提升科学假设的新颖性。
...
TL;DR: PAACE是一个用于优化LLM智能体上下文管理的统一框架，通过计划感知的自动上下文工程来压缩多步工作流中的上下文，提高智能体准确性同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂多步工作流中会产生快速扩展的上下文，现有压缩方法忽略了多步骤、计划感知的特性，导致注意力稀释和推理成本增加。

Method: PAACE框架包含PAACE-Syn（生成带压缩监督的合成工作流）和PAACE-FT（蒸馏训练的计划感知压缩器），通过任务相关性建模、计划结构分析、指令协同优化和函数保持压缩来优化上下文。

Result: 在AppWorld、OfficeBench和8-Objective QA等长视野基准测试中，PAACE在提高智能体正确性的同时显著降低上下文负载，蒸馏模型PAACE-FT保持97%教师性能同时降低超过一个数量级的推理成本。

Conclusion: PAACE为LLM智能体提供了一种有效的计划感知上下文压缩框架，能够在保持性能的同时大幅降低计算开销，实现实际部署。
...
TL;DR: 本文提出关系中心知识图谱问答新范式，通过子图选择和多阶段图剪枝结合强化学习微调LLM，解决传统实体中心问答的局限性，生成更紧凑、信息丰富的语义连接子图作为答案。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱问答主要关注实体中心查询，返回单个答案实体。然而现实世界查询通常是关系性的，需要理解实体间的关联。现有方法在处理关系查询时面临候选子图过多、平凡或过度常见连接掩盖独特信息答案的挑战。

Method: 提出UniRel-R1统一框架：1) 子图选择；2) 多阶段图剪枝；3) 使用强化学习微调的大型语言模型。奖励函数设计鼓励生成紧凑、具体的子图，包含更多信息丰富的关系和低度中间实体。

Result: 大量实验表明，UniRel-R1在连通性和奖励方面相比Vanilla基线取得显著提升，并能有效泛化到未见过的实体和关系。

Conclusion: 关系中心知识图谱问答是实体中心问答的重要补充，UniRel-R1框架通过集成子图选择、图剪枝和强化学习微调，能够有效识别独特且信息丰富的语义连接作为答案。
...
### [112] [Solomonoff-Inspired Hypothesis Ranking with LLMs for Prediction Under Uncertainty](https://arxiv.org/abs/2512.17145)
*Josh Barber,Rourke Young,Cameron Coombe,Will Browne*

Main category: cs.AI

TL;DR: 提出一种受所罗门诺夫启发的LLM假设加权方法，在不确定性下进行多假设推理，平衡准确性与简洁性


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理稀疏数据的现实任务时，难以在评估多个候选解决方案时平衡准确性和简洁性，特别是在不确定性下的推理挑战

Method: 受所罗门诺夫启发的加权方法，根据简洁性和预测拟合度对LLM生成的假设进行加权，应用于Mini-ARC基准任务，生成每个单元的所罗门诺夫加权混合预测

Result: 与贝叶斯模型平均相比，所罗门诺夫评分将概率更均匀地分布在竞争假设之间，产生保守、不确定性感知的输出，即使在假设有噪声或不完全正确时也能工作
...
TL;DR: 使用LLM驱动的智能体在虚拟社会中模拟威胁与冲突关系，发现物质威胁直接增加敌意，而象征性威胁通过内群体偏见间接影响，且只在物质威胁不存在时增加敌意


<details>
  <summary>Details</summary>
Motivation: 人类冲突常归因于物质条件和象征价值的威胁，但两者如何互动、哪个占主导尚不清楚。研究受限于弱因果控制、伦理约束和时间数据稀缺

Method: 使用大型语言模型驱动的智能体在虚拟社会中模拟，独立变化现实威胁和象征威胁，追踪行动、语言和态度。通过表征分析验证LLM编码的威胁状态与操纵的对应关系

Result: LLM编码现实威胁、象征威胁和敌意为不同内部状态；现实威胁直接增加敌意；象征威胁效应较弱，完全通过内群体偏见中介，且只在现实威胁不存在时增加敌意；非敌对性群体间接触缓冲冲突升级；结构不对称使敌意集中在多数群体

Conclusion: 通过LLM智能体模拟提供了威胁驱动冲突的因果解释，区分了现实威胁和象征威胁的不同作用机制，为理解人类冲突提供了新视角
...
TL;DR: 将AIXI强化学习智能体推广到更广泛的效用函数类别，通过处理信念分布中只能预测有限历史前缀的假设，探讨了半测度损失的死亡解释与不精确概率解释，并研究了使用Choquet积分计算期望效用的可计算性。


<details>
  <summary>Details</summary>
Motivation: AIXI智能体通常假设效用函数定义在完整交互历史上，但实际中许多假设只能预测有限历史前缀。这引入了半测度损失的概念，传统上被解释为"死亡概率"。本文旨在探索这种解释的替代方案，将信念分布视为不精确概率分布，并研究相应的期望效用计算方法。

Method: 1. 将AIXI智能体推广到更广泛的效用函数类别；2. 处理信念分布中只能预测有限历史前缀的假设；3. 探讨半测度损失的两种解释：死亡概率解释和不精确概率解释；4. 使用不精确概率理论中的Choquet积分计算期望效用；5. 分析所得期望效用的可计算性水平。

Result: 1. 证明了标准递归值函数可以作为Choquet积分的特例恢复；2. 发现死亡解释下的最一般期望效用不能表征为Choquet积分；3. 分析了这些期望效用计算方法的可计算性水平。

Conclusion: 通过将AIXI智能体推广到更广泛的效用函数类别，并采用不精确概率框架，为处理只能预测有限历史前缀的假设提供了新的理论视角。虽然死亡解释下的最一般期望效用不能完全用Choquet积分表征，但这种方法为强化学习中的信念建模和决策理论提供了有价值的扩展。
...
TL;DR: 本文提出了一种ASP求解器在环的方法，通过求解器引导的指令微调来提升LLMs生成答案集编程代码的能力，仅需自然语言问题描述及其解决方案即可训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用编程语言支持良好，但在领域特定语言（如答案集编程ASP）的代码生成方面仍面临挑战。ASP是一种解决组合搜索问题的有效方法，但LLMs在ASP代码生成中的有效性受到预训练阶段示例数量有限的限制。

Method: 提出ASP求解器在环方法，通过求解器引导的指令微调LLMs。方法仅需自然语言问题描述及其解决方案：1) 从LLMs采样ASP语句作为程序延续；2) 利用ASP声明式编程特性（部分编码逐步缩小解空间），基于求解器反馈将样本分为选择和拒绝实例；3) 对筛选数据进行监督微调；4) 使用包含最佳N采样的求解器引导搜索进一步提高鲁棒性。

Result: 实验在两个数据集上的两种不同提示设置中展示了一致的改进效果。

Conclusion: ASP求解器在环方法有效提升了LLMs生成ASP代码的能力，通过求解器引导的指令微调和搜索策略，解决了ASP代码生成中复杂的语义解析问题。
...
TL;DR: SAGE：基于强化学习的技能库框架，通过顺序部署和技能集成奖励，显著提升LLM智能体在新环境中的自我改进能力


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体在新环境中部署时缺乏持续改进和适应的能力，现有的技能库方法主要依赖LLM提示，难以实现一致的技能库实施

Method: 提出SAGE框架，采用强化学习方法增强智能体的自我改进能力。核心组件包括：顺序部署（Sequential Rollout）在相似任务链上迭代部署智能体，让先前任务生成的技能积累到库中供后续任务使用；技能集成奖励（Skill-integrated Reward）补充原有的结果奖励，增强技能生成和利用

Result: 在AppWorld上的实验显示，SAGE应用于有专家经验的监督微调模型时，场景目标完成率提高8.9%，交互步骤减少26%，生成token减少59%，在准确性和效率上都显著优于现有方法

Conclusion: SAGE框架通过强化学习结合技能库的方法，有效解决了LLM智能体在新环境中持续改进的挑战，在性能和效率方面都取得了显著提升
...
<details>
  <summary>Details</summary>
Motivation: 现有MMRAG方法无法阐明检索和响应生成背后的推理逻辑，限制了结果的可解释性。需要增强多模态大语言模型的推理能力来实现可解释的多模态检索增强生成。
...
Conclusion: 提出的强化学习方法成功增强了多模态检索增强生成的可解释性，通过两阶段强化微调框架有效提升了多模态大语言模型的推理能力，为可解释的MMRAG提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前对统一多模态模型的评估是分离的，分别用不同的数据集评估其理解和生成能力，缺乏一个综合的评估框架。需要开发一个能够全面评估UMMs能力的基准。

Method: UmniBench利用人类审查的提示和问答对，让UMM自身通过其理解能力来评估其生成和编辑能力。这种简单有效的范式允许对UMMs进行综合评估，同时也能解耦并单独评估各项能力。

Result: 基于UmniBench对24个流行模型进行了基准测试，包括UMMs和单能力大模型。该基准为统一模型提供了更全面客观的评估视角，并为社区模型性能改进提供了支持。

Conclusion: UmniBench是一个全面的多模态模型评估基准，能够同时评估理解、生成和编辑能力，为统一多模态模型的评估提供了更全面和客观的视角。
...
Method: 构建基于GPT风格自回归变换器的球员条件化价值感知下一事件预测模型，将比赛处理为离散令牌序列，联合预测下一持球动作的类型、位置、时间及其估计的剩余持球价值(rOBV)，通过替换学习到的球员嵌入进行反事实模拟

Result: 在五个赛季的英超联赛事件数据评估中，EventGPT在下一事件预测准确性和空间精度方面优于现有基于序列的基线模型，并通过案例研究展示了转会分析的实用价值

Conclusion: EventGPT提供了一个原则性方法来评估转会适应性，能够模拟球员在不同球队或战术结构中的行为分布和价值变化，为转会决策提供数据驱动的分析框架
...
### [120] [Dialectics for Artificial Intelligence](https://arxiv.org/abs/2512.17373)
*Zhengmian Hu*

Main category: cs.AI

TL;DR: 论文提出从算法信息论角度定义"概念"：概念是信息对象，通过其与智能体整体经验的结构关系来定义，核心约束是确定性（可逆一致性关系），并引入过剩信息衡量分解的自然性，在此基础上形式化辩证优化动态和低成本概念传输。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能能否在没有人类监督的情况下从原始经验中发现人类已发现的概念。挑战在于人类概念本身是流动的（如冥王星不再被视为行星），需要一种不是字典标签而是可修订、可比较、可在智能体间对齐的概念定义。

Method: 采用算法信息论视角，将概念定义为信息对象，核心约束是确定性（可逆一致性关系）。定义过剩信息衡量分解的自然性，在此基础上形式化辩证优化动态：新信息出现时，竞争概念通过更短的条件描述来解释它们，驱动系统性扩展、收缩、分裂和合并。形式化低成本概念传输和多智能体对齐。

Result: 提出了一个形式化框架：概念作为信息对象通过结构关系定义，确定性约束防止概念脱离经验，过剩信息衡量分解质量，辩证动态驱动概念演化，低成本传输通过共享协议实现概念重建。

Conclusion: 该框架为概念发现、演化和跨智能体对齐提供了形式化基础，将概念存在性转化为可检查的结构性主张，使通信成为具体的计算-比特权衡，为无监督概念学习开辟了新途径。
...
### [121] [Translating the Rashomon Effect to Sequential Decision-Making Tasks](https://arxiv.org/abs/2512.17470)
*Dennis Gross,Jørn Eirik Betten,Helge Spieker*

Main category: cs.AI

TL;DR: 该论文将Rashomon效应从分类任务扩展到序列决策领域，定义了在相同行为下内部结构不同的多个策略，并使用形式化验证方法进行验证，发现Rashomon集合构建的集成策略对分布偏移具有更强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Rashomon效应在分类任务中已被广泛研究，但在序列决策领域尚未被探索。序列决策中的随机性使得验证策略行为一致性比分类任务更复杂，需要新的验证方法。

Method: 将Rashomon效应定义扩展到序列决策，使用形式化验证方法构建和比较每个策略在环境中的完整概率行为，通过验证方法确保策略行为的一致性。

Result: 实验证明Rashomon效应在序列决策中存在；从Rashomon集合构建的集成策略对分布偏移表现出比单个策略更强的鲁棒性；从Rashomon集合派生的宽松策略在保持最优性能的同时减少了验证计算需求。

Conclusion: Rashomon效应在序列决策中确实存在，这一发现为构建更鲁棒的策略提供了新途径，形式化验证方法为解决序列决策中的策略比较问题提供了有效工具。
...
### [122] [Towards Explainable Conversational AI for Early Diagnosis with Large Language Models](https://arxiv.org/abs/2512.17559)
*Maliha Tabassum,M Shamim Kaiser*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型（GPT-4o）的医疗诊断聊天机器人，结合检索增强生成和可解释AI技术，在动态对话中提取症状并优先诊断，相比传统机器学习模型取得了90%准确率和100%Top-3准确率的优异表现。
...
### [123] [About Time: Model-free Reinforcement Learning with Timed Reward Machines](https://arxiv.org/abs/2512.17637)
*Anirban Majumdar,Ritam Raha,Rajarshi Roy,David Parker,Marta Kwiatkowska*

Main category: cs.AI

TL;DR: 提出定时奖励机(TRMs)扩展传统奖励机，加入时间约束，用于时间敏感应用中的强化学习
...
Method: 提出定时奖励机(TRMs)，将时间约束整合到奖励结构中；研究基于表格Q学习的模型无关RL框架，通过定时自动机抽象将TRM整合到学习中，并采用反事实想象启发式方法利用TRM结构改进搜索

Result: 算法学习到的策略在流行RL基准测试中实现了高奖励并满足TRM指定的时间约束；比较研究显示不同TRM语义下的性能表现，消融实验突显了反事实想象的好处

Conclusion: 定时奖励机为强化学习提供了更丰富的奖励规范能力，能够表达时间约束，实验证明提出的算法能有效学习满足时间约束的最优策略
...
### [124] [Humanlike AI Design Increases Anthropomorphism but Yields Divergent Outcomes on Engagement and Trust Globally](https://arxiv.org/abs/2512.17898)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益模仿人类特征，引发了关于拟人化可能导致不当信任或情感依赖的担忧。然而，现有研究缺乏在真实人机交互中测试拟人化设计与用户行为之间的因果关系，且主要基于西方人群的理论假设，忽视了全球用户的多样性。

Method: 采用两个大规模跨国实验，涵盖10个不同国家的3,500名参与者，涉及与AI系统的实时开放式交互。通过实验方法测试拟人化设计杠杆对拟人化感知的因果影响，并测量用户参与度和信任的行为指标。

Result: 研究发现：1）用户评估AI拟人化程度时更关注交互性线索而非理论特征；2）拟人化设计能因果性地增加用户的拟人化感知；3）拟人化设计并不普遍增加用户参与度和信任的行为指标；4）拟人化与行为结果之间的联系受到文化因素的调节，某些设计在某些文化中促进信任，在其他文化中可能产生相反效果。
...
### [125] [When Reasoning Meets Its Laws](https://arxiv.org/abs/2512.17901)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型(LRMs)性能优越，但其推理行为常常违反直觉，导致推理能力未达最优。为了从理论上形式化理想的推理行为，需要建立一个统一的框架来刻画LRMs的内在推理模式。

Method: 1. 提出推理定律(LoRe)框架，包括计算定律（假设推理计算量应与问题复杂度线性缩放）和补充的准确率定律；2. 由于问题复杂度难以量化，通过单调性和组合性这两个可处理的属性来检验定律假设；3. 开发LoRe-Bench基准系统测量大型推理模型的这两个属性；4. 提出有效的微调方法，强制模型遵循计算定律的组合性。

Result: 评估显示大多数推理模型表现出合理的单调性但缺乏组合性。通过微调方法强制遵循计算定律后，模型在多个基准测试中的推理性能得到一致提升，并揭示了不同属性和定律之间的协同效应。

Conclusion: 推理定律(LoRe)提供了一个理论框架来形式化大型推理模型的理想推理行为。通过LoRe-Bench评估和针对性的微调，可以提升模型对计算定律的遵循度，从而改善推理性能，这为理解和优化大型推理模型提供了系统的方法。
...
### [126] [Hermitian Hulls of Rational Algebraic Geometry Codes and Applications in Quantum Codes](https://arxiv.org/abs/2512.17128)
*Lin Sok,Martianus Frederic Ezerman,Ling San*

Main category: cs.IT

TL;DR: 本文研究线性码的Hermitian hull维度，利用代数函数域工具为广义有理代数几何码提供有效的下界估计，并基于此构造具有设计Hermitian hull维度的MDS码，最终得到两类新的MDS纠缠辅助量子纠错码。


<details>
  <summary>Details</summary>
Motivation: 线性码的hull研究日益重要，但Hermitian内积下的研究相对较少。本文旨在系统确定码的Hermitian hull维度和对偶距离，为设计纠缠辅助量子纠错码（EAQECCs）提供基础。

Method: 使用单变量代数函数域工具，为广义有理代数几何码的Hermitian hull维度提供有效的下界估计。通过选择特定的评估点集，使得与Hermitian对偶码相关的Weil微分留数具有易验证性质，从而构造具有设计Hermitian hull维度的码。

Result: 成功构造了基于Reed-Solomon码及其推广的具有设计Hermitian hull维度的MDS码。应用Hermitian方法得到两类新的MDS纠缠辅助量子纠错码家族，这些量子码具有优异的参数且都是新的。

Conclusion: 本文提出的方法能够有效估计和设计线性码的Hermitian hull维度，为构造新的纠缠辅助量子纠错码提供了系统框架，所得量子码参数优异且具有创新性。
...
### [127] [Quasi-recursive MDS Matrices over Galois Rings](https://arxiv.org/abs/2512.17256)
*Shakir Ali,Atif Ahmad Khan,Abhishek Kesarwani,Susanta Samanta*

Main category: cs.IT

TL;DR: 本文研究了Galois环上的拟递归MDS矩阵，提出了基于斜多项式环的直接构造方法，扩展了现有构造到非交换设置，显著扩大了可用矩阵族，在密码学扩散层中有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的递归MDS矩阵构造主要局限于交换环环境，在非交换设置下的构造方法有限。本文旨在扩展递归MDS矩阵的构造到Galois环上的斜多项式环，利用其丰富的分解性质和扩大的多项式类来生成拟递归MDS矩阵。

Method: 基于斜多项式环GR(p^s, p^{sm})[X;σ]构造，首先建立了两个判别多项式是否产生递归MDS矩阵的准则，推广了现有结果；然后从相关Wedderburn多项式的右根角度推导了附加准则。利用这些准则，开发了构造产生拟递归MDS矩阵的斜多项式的方法。

Result: 提出了在Galois环上构造拟递归MDS矩阵的直接方法，将已知构造扩展到非交换设置，显著扩大了可用矩阵族。特别地，当s=1且p=2（即有限域F_{2^m}）时，结果对实际密码学应用有重要价值。

Conclusion: 本文建立了一个在Galois环上构造拟递归MDS矩阵的框架，利用斜多项式环的代数结构扩展了现有构造，为密码学扩散层设计提供了更丰富的矩阵选择，特别是在有限域F_{2^m}上的应用具有实际意义。
...
### [128] [A distance-free approach to generalized weights](https://arxiv.org/abs/2512.17542)
*Andrea Di Giusto,Elisa Gorla,Alberto Ravagnani*

Main category: cs.IT

TL;DR: 该论文提出了一个统一的广义权重理论，适用于具有任意距离的线性码。通过选择测试族来定义权重，而不是依赖支撑集或反码，从而统一了汉明权重、秩度量权重和和-秩度量权重的广义权重理论。


<details>
  <summary>Details</summary>
Motivation: 现有广义权重理论通常针对特定度量（如汉明度量、秩度量）分别发展，缺乏统一框架。本文旨在建立一个统一的理论框架，能够涵盖各种度量下的广义权重，并揭示它们之间的内在联系。

Method: 提出"测试族"概念，通过码与测试族中空间的交集来定义广义权重。选择不同的测试族可以得到不同性质的广义权重。在该框架下证明了广义权重的单调性、严格递增性以及类似Wei对偶定理的对偶结果。

Result: 1. 建立了统一的广义权重理论框架；2. 证明了广义权重的弱递增性和某些子序列的严格递增性；3. 证明了类似Wei对偶定理的对偶结果；4. 对于和-秩度量码，提出了新的测试族，扩展了对偶结果到具有非零汉明分量的码；5. 探索了通过与MDS或MRD码相交得到的广义权重族。

Conclusion: 该论文提供了一个统一的框架来研究各种度量下的广义权重，通过选择适当的测试族可以恢复已知的广义权重性质，同时为研究新的广义权重类型提供了理论基础，特别是在和-秩度量码领域扩展了现有结果。
...
### [129] [Locally-APN Binomials with Low Boomerang Uniformity in Odd Characteristic](https://arxiv.org/abs/2512.17603)
*Namhun Koo,Soonhak Kwon,Minwoo Ko,Byunguk Kim*

Main category: cs.IT

TL;DR: 该论文研究了有限域上函数F_r(x)=x^r+x^{r+(q-1)/2}的局部APN性质和回旋均匀度，在特定条件下证明了其回旋均匀度至多为2，并进一步研究了F_3和F_{(2q-1)/3}的差分谱以及p=3时F_2的回旋谱。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明当q≡3(mod4)时，函数F_r(x)具有局部APN性质且回旋均匀度至多为2。本文旨在扩展这些结果，在更一般的条件下研究该函数的密码学性质，特别是其差分谱和回旋谱，这对于设计和分析密码算法具有重要意义。

Method: 通过数学分析和证明，在特定条件（存在至多一个x满足特定方程且gcd(r,q-1)整除2）下，证明了F_r的局部APN性质和回旋均匀度上界。进一步采用代数方法研究了F_3和F_{(2q-1)/3}的差分谱，以及在特征p=3时F_2的回旋谱。

Result: 证明了在给定条件下，函数F_r是局部APN的且回旋均匀度至多为2。获得了F_3和F_{(2q-1)/3}的差分谱具体形式，以及在p=3时F_2的回旋谱特性。

Conclusion: 本文成功扩展了有限域上函数F_r的密码学性质研究，在更一般的条件下证明了其良好的密码学特性，为密码函数设计提供了理论支持。对特定参数下函数的差分谱和回旋谱分析进一步丰富了该领域的研究成果。
...
### [130] [Iterative Gaussian Approximation for Random Spreading Unsourced Random Access](https://arxiv.org/abs/2512.17628)
*Liandong Hu,Jian Dang,Zaichen Zhang*

Main category: cs.IT

TL;DR: 提出了一种适用于随机扩频类无源随机接入的迭代高斯近似解码器，通过迭代外在与内在软信息提升解码性能，只需少量迭代即可收敛。


<details>
  <summary>Details</summary>
Motivation: 大规模机器类型通信需要支持大规模连接的高效解决方案，无源随机接入因其高频谱和能量效率成为有前景的方法。随机扩频类无源随机接入在少数活跃用户场景下已接近理论性能极限，但需要更有效的解码器。

Method: 提出了一种通用的迭代高斯近似解码器，适用于随机扩频类无源随机接入。该解码器通过迭代外在与内在软信息来增强解码性能，只需要少量迭代即可收敛。

Result: 数值结果验证了该解码器在性能和鲁棒性方面的有效性。

Conclusion: 提出的迭代高斯近似解码器为随机扩频类无源随机接入提供了一种高效且鲁棒的解决方案，能够在大规模机器类型通信中实现高性能连接。
