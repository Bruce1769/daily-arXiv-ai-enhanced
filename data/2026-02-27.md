...
Motivation: 当前的组织病理学基础模型不仅捕获了生物学相关特征，还包含了预处理和扫描仪特异性变异，这些变异会偏置基于基础模型特征训练的任务特定模型的预测结果。

Method: 在训练下游任务特定模型时引入新颖的鲁棒性损失函数，使用包含27,042张全切片图像和6,155名患者的综合实验设置，从八个流行计算病理学基础模型的特征训练数千个模型。

Result: 该方法显著提高了模型的鲁棒性，同时通过聚焦于生物学相关特征，预测准确性也得到了改善，成功缓解了计算病理学基础模型的鲁棒性问题。
...
TL;DR: AeroDGS：基于物理引导的4D高斯泼溅框架，用于单目无人机视频的动态空中场景重建，解决了单视角、大空间范围、小动态物体和大运动差异带来的深度模糊和运动估计不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有4D场景重建方法在单视角无人机拍摄条件下存在局限：大空间范围、小动态物体和大运动差异导致严重的深度模糊和不稳定运动估计，使单目空中重建成为病态问题。

Method: 提出AeroDGS框架，包含两个核心模块：1）单目几何提升模块，从单目空中序列重建可靠的静态和动态几何；2）物理引导优化模块，引入可微分的地面支撑、垂直稳定性和轨迹平滑性先验，将模糊图像线索转化为物理一致的运动。

Result: 在合成和真实无人机场景上的实验表明，AeroDGS在动态空中环境重建中优于现有最先进方法，实现了更高的重建保真度。作者还构建了包含不同高度和运动条件的真实世界无人机数据集进行评估。

Conclusion: AeroDGS通过物理引导的4D高斯泼溅框架，有效解决了单目无人机视频动态重建中的深度模糊问题，实现了稳定几何和连贯时间演化的联合优化，为动态空中场景重建提供了可靠解决方案。
...
TL;DR: 提出了一种无需分割的肾肿瘤恶性预测深度学习框架，使用器官聚焦注意力损失函数，在两个数据集上超越了传统需要分割的方法。
...
Method: 开发了使用器官聚焦注意力损失函数的深度学习框架，该函数修改图像补丁的注意力机制，使器官补丁仅关注其他器官补丁，从而在部署时无需对3D肾CT图像进行分割。

Result: 在UF IDR私有数据集上获得AUC 0.685和F1分数0.872，在KiTS21公开数据集上获得AUC 0.760和F1分数0.852，超越了依赖分割裁剪的传统模型。

Conclusion: 该方法提供了一种更高效可靠的肾肿瘤恶性预测方法，无需显式分割输入，能够增强临床决策能力。
...
TL;DR: ViT存在从背景补丁获取语义捷径的懒惰聚合行为，导致伪影问题，本文提出选择性集成补丁特征的方法来改善性能


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在不同监督范式和下游任务中广泛观察到伪影问题，其根本机制尚未得到充分阐明，需要系统分析并解决这一问题

Method: 通过系统分析发现ViT存在懒惰聚合行为：使用语义无关的背景补丁作为表示全局语义的捷径，受全局注意力和粗粒度语义监督驱动。解决方案是选择性集成补丁特征到CLS令牌中，减少背景主导捷径的影响

Result: 在标签监督、文本监督和自监督下的12个基准测试中，该方法一致地提高了性能
...
<details>
  <summary>Details</summary>
Motivation: CLIP模型在互联网规模数据上通过图像-文本对比学习获得可迁移的多模态特征，但其预训练主要基于短标题配对的图像，导致模型偏向于编码显著物体的简单描述，在复杂场景和密集描述上的对齐效果粗糙。虽然近期工作通过在小型长标题数据集上微调来缓解此问题，但发现人类和LLM生成的长标题通常以一句摘要开头，这成为训练中的捷径，使注意力集中在开头句子和早期标记上，削弱了与标题其余部分的对齐。

Method: 提出DeBias-CLIP方法：1）在训练时移除摘要句；2）应用句子子采样；3）使用文本标记填充。这些技术旨在将监督信号分布到所有标记位置，无需额外可训练参数，可作为Long-CLIP的直接替代方案。

Result: DeBias-CLIP在长文本检索上达到最先进水平，改善了短文本检索性能，对句子顺序排列的敏感性降低，是Long-CLIP的无缝替代方案。

Conclusion: 通过识别并解决长标题训练数据中的结构性偏差，DeBias-CLIP有效提升了CLIP模型在复杂文本描述上的对齐能力，为多模态检索提供了更鲁棒的解决方案。
...
TL;DR: 该研究诊断了多模态大语言模型(MLLMs)的视觉基础机制问题，发现模型存在"模态惰性"——尽管具备OCR能力，但在需要视觉参与的任务中性能显著下降。研究者提出了SimpleOCR训练策略来强制模型激活视觉文本提取路径。


<details>
  <summary>Details</summary>
Motivation: 研究旨在诊断多模态大语言模型(MLLMs)的视觉基础机制：这些模型是否真正"读取"图像中的文本，还是仅仅依赖文本提示中的参数捷径？研究者关注模型是否具有真正的视觉文本理解能力，而非仅仅利用训练数据中的模式匹配。

Method: 1. 引入可视化问题(VQ)设置：将文本查询直接渲染到图像上，强制要求视觉参与；2. 在Qwen2.5-VL上进行诊断实验；3. 提出SimpleOCR训练策略：将训练样本转换为VQ格式并随机化样式，使文本捷径失效，强制模型激活和优化视觉文本提取路径；4. 采用即插即用设计，可与高级强化学习策略(如NoisyRollout)集成。

Result: 1. 发现显著的能力利用差距：尽管具备强大的OCR能力，模型在VQ设置下性能下降高达12.7%，暴露了"模态惰性"；2. SimpleOCR在四个代表性OOD基准测试中，比基础模型提升5.4%，比基于原始图像的GRPO提升2.7%；3. 具有极高的数据效率：仅用8.5K样本(比最近RL方法少30倍)就能达到优越性能；4. 与NoisyRollout等RL策略集成可获得互补改进。

Conclusion: 多模态大语言模型存在"模态惰性"问题，倾向于依赖文本提示中的参数捷径而非真正的视觉文本理解。SimpleOCR作为一种简单有效的训练策略，通过结构约束强制模型激活视觉文本提取能力，在无需架构修改的情况下显著提升性能，具有高数据效率和良好的可扩展性。
...
Method: 采用双线程异步架构：描述符线程持续将视频转换为轻量级文本记忆，问答线程在文本记忆上进行推理回答查询。将流式约束整合到问答管道中，并在QAEgo4D-Closed基准上测试多模态大语言模型在严格资源边界内的性能。

Result: 在消费级8GB GPU上实现端到端配置，准确率达到51.76%，首词时间延迟为0.41秒。扩展到本地企业级服务器时，准确率提升至54.40%，首词时间延迟为0.88秒。相比之下，云端解决方案准确率为56.00%。

Conclusion: 边缘解决方案在隐私保护的情景记忆检索方面具有巨大潜力，能够在资源受限环境下实现与云端解决方案竞争的准确率，同时解决隐私和延迟问题。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺X光筛查工作量大、时间敏感且文档繁重，现有视觉语言模型大多依赖封闭云系统或紧密耦合架构，限制了隐私性、可重复性和适应性。需要开发本地化、可扩展的解决方案。

Method: 开发MammoWise本地多模型管道，支持任何Ollama托管的视觉语言模型和乳腺X光数据集，采用零样本、少样本、思维链提示，可选多模态检索增强生成，使用参数高效微调（QLoRA）优化模型性能。

Result: 报告生成质量稳定，通过少样本提示和检索增强生成得到改善。分类任务可行但对模型和数据集选择敏感。MedGemma经过QLoRA微调后，BI-RADS准确率达0.7545，密度准确率0.8840，钙化准确率0.9341，同时保持报告质量。
...
TL;DR: 本文提出了一种名为空间信用重分配（SCR）的训练免费推理时干预方法，用于减少视觉语言模型中的物体幻觉问题。该方法通过重新分配高注意力源补丁的隐藏状态激活到其上下文，显著降低了模型幻觉率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型经常幻觉输入图像中不存在的物体。作者将这一失败归因于空间信用崩溃：激活信用集中在早期transformer层的稀疏视觉补丁上，这抑制了上下文证据并增加了对语言先验的依赖。

Method: 提出了空间信用重分配（SCR），这是一种训练免费的推理时干预方法。它通过低熵输入引导，将隐藏状态激活从高注意力源补丁重新分配到其上下文。该方法使用注意力引导的源选择机制，而非均匀随机选择。

Result: 在六个模型家族（Chameleon、LLaVA和Qwen系列）上评估，SCR在POPE-Adversarial上将幻觉减少了约4.7-6.0个百分点，CHAIR-s减少了3.7-5.2个百分点（相对减少42-51%），CHAIR-i减少了2.7-4.4个百分点（相对减少44-58%），同时CIDEr得分保持在0.8个百分点以内。该方法仅增加43-56毫秒开销，远低于其他方法。

Conclusion: SCR是一种高效实用的训练免费推理时干预方法，能显著减少视觉语言模型的物体幻觉问题，同时保持生成质量，适合实时应用。注意力引导的源选择机制对性能提升至关重要，验证了信用崩溃是幻觉的关键驱动因素。
...
TL;DR: Pix2Key：一种基于开放词汇视觉词典的合成图像检索方法，通过自监督预训练提升细粒度属性理解，在DFMM-Compose基准上显著提升召回率和意图一致性


<details>
  <summary>Details</summary>
Motivation: 传统融合方法依赖监督三元组可能丢失细粒度线索，零样本方法通过图像描述合并编辑可能忽略用户隐含意图并产生重复结果，需要更好的方法来实现意图感知约束匹配和多样性重排序

Method: Pix2Key将查询和候选图像表示为开放词汇视觉词典，在统一嵌入空间中进行意图感知约束匹配和多样性感知重排序；引入自监督预训练组件V-Dict-AE，仅使用图像改进词典表示，无需CIR特定监督

Result: 在DFMM-Compose基准上，Pix2Key将Recall@10提升高达3.2个百分点；添加V-Dict-AE进一步带来2.3个百分点的增益，同时提高意图一致性并保持高列表多样性

Conclusion: Pix2Key通过开放词汇视觉词典表示和自监督预训练，有效解决了合成图像检索中的意图理解和多样性问题，在性能和效果上都超越了现有方法
...
<details>
  <summary>Details</summary>
Motivation: tau-PET是阿尔茨海默病病理的重要体内标记物，但成本高且可用性有限，需要基于MRI的替代方法。现有方法在保持解剖细节和疾病相关信号方面存在局限。

Method: 结合了(1)基于部分信息分解(PID)的向量量化编码器，将潜在信息分解为冗余、独特和互补成分；(2)半UNet解码器，使用基于结构边缘线索的伪跳跃连接而非直接重用编码器特征来保持解剖细节。
...
Conclusion: DisQ-HNet提供了一个有效的MRI到tau-PET合成框架，通过信息分解和结构感知解码机制，在保持解剖细节的同时更好地保留疾病相关信号，为tau-PET的替代方案提供了有前景的方法。
...
TL;DR: DrivePTS是一个用于生成多样化驾驶场景的扩散模型，通过解耦几何条件依赖、多视图语义描述和频率引导结构损失，解决了现有方法在条件独立性、语义细节和结构保真度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前基于HD地图和3D边界框的驾驶场景生成方法存在三个主要问题：1）几何条件间的隐式依赖导致条件独立变化时生成失败；2）简略且视角不变的描述限制了语义上下文，导致背景建模薄弱；3）均匀空间加权的标准去噪损失忽略了前景结构细节，造成视觉失真和模糊。

Method: DrivePTS提出三个关键创新：1）采用渐进学习策略配合显式互信息约束来缓解几何条件间的相互依赖；2）利用视觉语言模型生成跨六个语义方面的多视图分层描述，提供细粒度文本指导；3）引入频率引导结构损失，增强模型对高频元素的敏感性，提升前景结构保真度。

Result: 大量实验表明，DrivePTS在生成多样化驾驶场景方面实现了最先进的保真度和可控性。特别值得注意的是，DrivePTS成功生成了先前方法失败的罕见场景，突显了其强大的泛化能力。

Conclusion: DrivePTS通过解决几何条件依赖、语义细节不足和结构保真度问题，显著提升了驾驶场景生成的质量和可控性，为自动驾驶系统的鲁棒性验证提供了有效的数据增强技术。
...
TL;DR: SwiftNDC是一个快速3D重建框架，通过神经深度修正场生成跨视图一致的深度图，然后通过反投影和重投影误差过滤生成密集点云，为下游重建提供高质量的几何初始化，显著加速3D高斯溅射的网格重建和提升新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有深度引导的3D重建方法存在尺度漂移、多视图不一致性等问题，且需要大量细化才能获得高保真几何。需要一种快速且通用的框架来解决这些问题，提供高质量的几何初始化来加速下游重建过程。

Method: 提出SwiftNDC框架，核心是神经深度修正场，用于生成跨视图一致的深度图。然后通过反投影和重投影误差过滤生成密集点云，获得干净且均匀分布的几何初始化。该几何初始化用于加速3D高斯溅射的网格重建，并提升新视角合成的渲染质量。

Result: 在五个数据集上的综合研究表明，SwiftNDD能显著减少准确网格重建的运行时间，并提升视角合成的渲染保真度。该方法在网格重建和新视角合成任务上都表现出色，证明了神经深度修正与鲁棒几何初始化结合的有效性。
...
TL;DR: QARMVC提出了一种质量感知的鲁棒多视图聚类框架，通过信息瓶颈提取语义特征，利用重建差异量化噪声强度，并采用分层学习策略抑制噪声传播。


<details>
  <summary>Details</summary>
Motivation: 现有噪声鲁棒方法主要依赖简化的二元假设（数据完全干净或完全损坏），忽略了现实应用中普遍存在的异质观测噪声（污染强度在数据间连续变化）。
...
Conclusion: QARMVC通过质量感知的鲁棒多视图聚类框架，有效解决了现实应用中存在的异质观测噪声问题，显著提升了聚类性能。
...
TL;DR: 论文揭示了扩散模型指导方法评估中的严重缺陷：常用的人类偏好模型对高指导尺度存在强烈偏见，导致即使图像质量下降也能获得高分。作者提出了新的评估框架GA-Eval，并设计了一个实际上无效但能在传统评估中获得高分的TDG方法，证明当前评估体系存在问题。


<details>
  <summary>Details</summary>
Motivation: 尽管出现了各种改进的扩散指导方法，但作者质疑这些方法是否真的带来了实质性的改进。研究发现当前评估体系存在严重缺陷，人类偏好模型对高指导尺度存在强烈偏见，导致评估结果失真。

Method: 1) 揭示评估缺陷：发现人类偏好模型对高指导尺度的偏见；2) 提出GA-Eval框架：通过指导尺度校准实现公平比较；3) 设计TDG方法：证明传统评估的缺陷；4) 实验评估：在传统和GA-Eval框架下评估8种扩散指导方法。

Result: 1) 单纯增加CFG尺度就能与大多数扩散指导方法竞争；2) 所有方法相比标准CFG都遭受严重的胜率下降；3) TDG方法在传统评估中获得高分但实际上无效；4) 当前评估体系存在严重问题。

Conclusion: 当前扩散指导方法的评估范式存在问题，社区需要重新思考该领域的评估标准和未来发展方向。单纯增加CFG尺度就能达到类似效果，而现有方法并未带来实质性改进。
...
TL;DR: GIFSplat提出了一种纯前馈的迭代细化框架，用于从稀疏无位姿视图中进行3D高斯泼溅重建，通过残差更新逐步优化3D场景，并蒸馏扩散先验知识，在保持秒级推理时间的同时显著提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建方法虽然推理速度快，但性能仍有提升空间，特别是在域外数据上表现不佳，且难以在引入生成先验的同时保持秒级推理时间。这些限制源于一次性预测范式：模型受容量限制、缺乏推理时细化能力，不适合持续注入生成先验。

Method: 提出GIFSplat框架：1) 使用少量前馈残差更新逐步细化当前3D场景；2) 从增强的新视角渲染中蒸馏冻结扩散先验为高斯级线索，无需梯度反向传播或不断扩展视图集；3) 保持纯前馈推理，无需相机位姿或测试时梯度优化。

Result: 在DL3DV、RealEstate10K和DTU数据集上，GIFSplat持续优于最先进的前馈基线方法，PSNR提升高达+2.1 dB，同时保持秒级推理时间，无需相机位姿或任何测试时梯度优化。

Conclusion: GIFSplat通过迭代残差更新和生成先验蒸馏，实现了前馈3D重建中效率与质量的良好平衡，为稀疏视图下的3D高斯泼溅重建提供了高效且高质量的解决方案。
...
TL;DR: CMDM提出了一种基于因果扩散变换器的自回归运动生成框架，在语义对齐的潜在空间中实现高质量文本到运动生成，支持流式合成和长时程运动生成，在语义保真度和时间平滑性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有运动扩散模型存在局限性：全序列扩散模型采用双向生成，限制了时间因果性和实时应用性；自回归模型则存在不稳定性和累积误差问题。需要一种既能保持高质量生成又能支持实时应用的统一框架。

Method: 1. 构建Motion-Language-Aligned Causal VAE (MAC-VAE)，将运动序列编码为时间因果的潜在表示；2. 在潜在表示上训练自回归扩散变换器，使用因果扩散强制进行时间有序的去噪；3. 引入具有因果不确定性的帧级采样调度，从部分去噪的前帧预测后续帧。

Result: 在HumanML3D和SnapMoGen数据集上的实验表明，CMDM在语义保真度和时间平滑性方面优于现有扩散和自回归模型，同时显著降低了推理延迟，支持交互速率的运动生成。

Conclusion: CMDM提供了一个统一的框架，通过因果扩散变换器和语义对齐的潜在空间，实现了高质量、实时适用的运动生成，解决了现有方法在时间因果性、稳定性和实时性方面的局限性。
...
Method: 提出Association DETR模型，该模型能够有效利用背景上下文信息来辅助目标检测。具体方法细节未在摘要中详细说明，但核心思想是解决现有模型忽略背景信息的问题。

Result: Association DETR在COCO val2017数据集上取得了最先进（state-of-the-art）的结果，超越了YOLOv12（55.2 mAP）和RT-DETRv2（53.4 mAP）等现有优秀模型。

Conclusion: 背景上下文信息对目标检测任务具有重要价值，Association DETR通过有效利用这种信息取得了优于现有模型的性能，证明了背景信息在目标检测中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基础模型在地球观测应用中面临两个主要障碍：1）多光谱输入信息丰富但存在波段冗余和不对齐问题；2）CLIP式文本编码器语义表达能力有限，细粒度对齐效果不佳。同时，实际卫星系统常缺乏完整多光谱覆盖，使得仅用RGB进行推理成为可扩展部署的关键需求。

Method: SATtxt采用两阶段框架：第一阶段通过光谱表示蒸馏，使用轻量级投影器将冻结多光谱教师模型的频谱先验知识转移到RGB学生模型；第二阶段通过指令增强LLM的光谱基础对齐，将蒸馏后的视觉空间与表达能力强的LLM嵌入空间进行桥接。

Result: 在EuroSAT、BigEarthNet和ForestNet数据集上，SATtxt相比基线方法平均提升零样本分类4.2%、检索5.9%、线性探测2.7%，展示了频谱感知视觉语言学习在地球观测中的高效路径。

Conclusion: SATtxt为地球观测提供了一种频谱感知的视觉语言基础模型，仅需RGB输入即可推理，同时有效利用训练中的频谱信息，显著提升了零样本理解和检索能力，为可扩展部署提供了有效解决方案。
...
TL;DR: Coded-E2LF是一种仅使用事件相机和编码孔径获取4D光场的计算成像方法，无需传统强度图像，简化了硬件实现


<details>
  <summary>Details</summary>
Motivation: 现有方法需要同时捕获事件和强度图像来重建光场，硬件实现受限。本文旨在开发纯事件驱动的光场获取方法，降低硬件要求

Method: 使用编码孔径和静止的事件相机，通过分析孔径编码模式中黑色图案的关键作用，从纯事件数据中重建4D光场

Result: 首次证明仅从事件数据就能重建具有像素级精度的4D光场，并在真实成像硬件上验证了方法的有效性

Conclusion: Coded-E2LF方法实现了纯事件驱动的4D光场获取，简化了硬件实现，为事件相机在计算成像领域的应用开辟了新途径
...
TL;DR: CGSA框架首次将对象中心学习引入无源域自适应目标检测，通过集成层次化槽感知和类引导槽对比模块，在多个跨域数据集上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有SF-DAOD方法主要关注调整伪标签阈值或改进师生框架，忽视了跨域数据中对象级别的结构线索。本文旨在利用对象中心学习来改进无源域自适应目标检测

Method: 提出CGSA框架，将槽感知适应集成到DETR检测器中：1）层次化槽感知模块逐步解耦图像为槽表示作为视觉先验；2）类引导槽对比模块引导槽朝向类语义，保持语义一致性并促进域不变适应

Result: 在多个跨域数据集上的实验表明，该方法优于之前的SF-DAOD方法。理论推导和实验分析进一步证明了所提组件和框架的有效性
...
TL;DR: CRAG方法将3D装配重新定义为装配与生成的联合问题，通过相互增强机制同时生成完整形状和预测零件姿态，解决了传统方法无法合成缺失几何的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D装配方法仅视为姿态估计问题，而人类装配自然结合结构推理与整体形状推断。传统方法无法合成缺失几何，需要同时处理装配和生成。

Method: 提出CRAG方法，将3D装配重新定义为装配与生成的联合问题。装配为生成提供零件级结构先验，而生成为装配注入整体形状上下文以解决歧义，两者相互增强。

Result: 在具有不同几何形状、可变零件数量和缺失部件的野外物体上展示了最先进的性能，能够同时生成合理的完整形状并预测输入零件的姿态。

Conclusion: 装配与生成是相互增强的过程，联合方法优于纯姿态估计方法。CRAG能够处理现实世界中的复杂3D装配问题，代码和模型将开源。
...
TL;DR: 本文提出了一种新的四焦点张量同步框架，通过块四焦点张量的Tucker分解恢复多个相机，并开发了首个四焦点张量同步算法。


<details>
  <summary>Details</summary>
Motivation: 在运动恢复结构中，四焦点张量比成对对应物（本质矩阵）包含更多信息，但通常被认为不实用且仅具理论意义。本研究挑战这一观点，探索利用高阶信息进行相机同步的潜力。

Method: 提出块四焦点张量概念，证明其具有Tucker分解特性，因子矩阵为堆叠的相机矩阵，多线性秩为(4,4,4,4)。开发首个四焦点张量同步算法，结合Tucker分解、交替方向乘子法和迭代重加权最小二乘法。建立块四焦点、三焦点和双焦点张量之间的关系，并引入联合同步这三种实体的算法。

Result: 数值实验证明该方法在现代数据集上的有效性，表明在同步中使用高阶信息具有潜力和重要性。

Conclusion: 四焦点张量在实际应用中具有实用价值，通过提出的同步框架和算法，能够有效恢复多个相机，为运动恢复结构中的高阶信息利用开辟了新途径。
...
Result: MWAM可无缝集成到CNN和ViT等不同架构中，在各种任务和模态组合上带来一致的性能提升，不仅优化基础模型性能，还能进一步提升解决缺失模态问题的最先进方法
...
Method: 基于Napari多维查看器构建，将3D体积视为切片序列，集成框/点提示与SAM2风格传播。提供本地优先工作流程，支持标准DICOM系列和NIfTI体积，用户可通过框优先提示初始化对象，用点提示细化预测，并在保存前进行提示优先校正。

Result: 开发了一个开源桌面应用，支持多病例顺序标注，提供统一的队列导向工作流程，支持每对象体积测量和3D体积渲染，通过SimpleITK保持图像几何结构。

Conclusion: Medical-SAM2 GUI为医学图像研究标注工作流程提供了一个高效的本地解决方案，通过半自动标注技术显著减少了3D医学图像标注的时间和成本。
...
TL;DR: DPCache是一种无需训练的扩散模型加速框架，将采样加速问题转化为全局路径规划问题，通过动态规划选择最优关键时间步序列，在保持质量的同时实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成方面表现出色，但多步迭代采样带来的巨大计算开销阻碍了实际部署。现有的基于缓存的加速方法采用固定或局部自适应调度，未考虑去噪轨迹的全局结构，容易导致误差累积和视觉伪影。

Method: DPCache将扩散采样加速构建为全局路径规划问题。首先从小型校准集构建路径感知成本张量，量化在给定前一个关键时间步条件下跳过时间步的路径相关误差。然后利用动态规划选择最小化总路径成本的最优关键时间步序列。在推理时，模型只在关键时间步进行完整计算，中间输出则使用缓存特征高效预测。

Result: 在DiT、FLUX和HunyuanVideo上的大量实验表明，DPCache在保持最小质量损失的情况下实现了显著加速。在FLUX上，以4.87倍加速实现了+0.031 ImageReward的提升，甚至以3.54倍加速超过了完整步数基线+0.028 ImageReward，优于先前的加速方法。

Conclusion: DPCache通过路径感知的全局调度框架，有效解决了现有缓存方法忽略全局结构的问题，为扩散模型提供了一种无需训练的高效加速方案，在多个模型上都表现出优越的性能。
...
TL;DR: 该论文提出了一个实用的音视频质量评估数据集构建方法，通过众包主观实验框架、系统化的数据准备策略和扩展标注，创建了YT-NTU-AVQ数据集，这是目前最大且最多样化的AVQA数据集。
...
Method: 1. 设计用于AVQA的众包主观实验框架，打破实验室环境限制，在不同环境中实现可靠标注；2. 采用系统化的数据准备策略，确保覆盖广泛的质谱水平和语义场景；3. 扩展数据集标注，支持多模态感知机制及其与内容关系的研究。

Result: 成功构建了YT-NTU-AVQ数据集，包含1,620个用户生成的音视频序列，是目前最大且最多样化的AVQA数据集，数据集和平台代码已开源。

Conclusion: 提出的AVQA数据集构建方法有效解决了现有数据集的局限性，为模型发展和多模态感知研究提供了更好的支持，YT-NTU-AVQ数据集将成为该领域的重要资源。
...
TL;DR: ArtPro：一种自监督框架，通过自适应整合运动提案，解决了基于3D高斯泼溅的可微分渲染方法对初始部件分割高度敏感的问题，显著提升了复杂多部件物体的重建准确性和稳定性。
...
Conclusion: ArtPro通过自适应整合运动提案，有效解决了现有自监督方法对初始分割敏感的问题，为机器人操作和交互仿真等应用提供了高质量的关节物体数字孪生重建方案。
...
TL;DR: 提出LegoOcc框架，通过3D语言嵌入高斯表示和几何-语义协同优化，实现室内开放词汇3D占据预测，仅需二元占据标签监督。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D占据方法主要针对室外驾驶场景，难以适应室内环境的密集几何、复杂布局和细粒度语义。需要一种仅需弱监督（二元占据标签）就能处理室内开放词汇3D占据的方法。

Method: 采用3D语言嵌入高斯作为统一中间表示，耦合细粒度几何与语言对齐语义嵌入。几何方面：提出不透明度感知的泊松基体素聚合方法，解决现有高斯到占据算子收敛问题。语义方面：提出渐进温度衰减调度，在渲染过程中逐渐锐化不透明度，增强高斯-语言对齐。

Result: 在Occ-ScanNet数据集上，开放词汇设置下达到59.50 IoU和21.05 mIoU，在IoU上超越所有现有占据方法，在mIoU上大幅领先先前开放词汇方法。

Conclusion: LegoOcc框架通过几何-语义协同优化，仅需二元占据标签监督即可实现室内开放词汇3D占据预测，为具身智能体理解复杂室内环境提供了有效解决方案。
...
TL;DR: ViCLIP-OT：专为越南语图像-文本检索设计的视觉语言基础模型，结合CLIP对比学习和相似图正则化最优传输损失，在低资源语言环境下显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要针对高资源语言优化，在越南语等低资源语言环境下性能不佳。需要专门为越南语设计的基础视觉语言模型来改善跨模态检索效果。
...
Conclusion: SIGROT整合为低资源语言的跨模态检索提供了有效且可扩展的策略，对越南语及其他代表性不足语言环境的智能多媒体检索系统具有实际意义。
...
TL;DR: 本文提出SoPE方法，通过将点云token映射到三维球坐标系来改进3D大视觉语言模型的位置编码，解决了传统RoPE在3D多模态理解中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的3D大视觉语言模型继承自大语言模型的旋转位置编码机制在3D多模态理解中存在不足：1）无法保持3D token的三维空间结构；2）相对距离计算忽略了角度依赖性，限制了模型捕捉视觉表示中方向变化的能力。

Method: 提出球坐标系位置嵌入方法，将点云token索引映射到三维球坐标系，统一建模空间位置和方向角度。同时引入多尺度频率混合策略，融合不同频率域的特征信息。
...
TL;DR: 提出了一种基于扩散模型的超声图像去斑方法，通过模拟配对数据集进行监督训练，在保持解剖结构边缘和对比度的同时有效抑制斑点噪声。


<details>
  <summary>Details</summary>
Motivation: 超声成像虽然实时、无创，但斑点噪声和相关伪影会降低图像质量，影响诊断解读。现有方法在保持解剖结构细节方面存在不足。

Method: 基于图像恢复随机微分方程框架构建扩散模型，使用Matlab超声工具箱从无斑点的磁共振图像模拟超声图像创建大型配对数据集进行监督训练。

Result: 在模拟测试集上，该方法在去斑效果上优于传统滤波器和近期基于学习的基线方法，同时通过交叉模型方差量化预测不确定性，发现高不确定性区域与高重建误差相关。

Conclusion: 该方法能有效去除超声斑点噪声并保持解剖结构，但存在模拟探头设置敏感性和领域偏移问题，需要多样化训练和适应策略以实现稳健的临床部署。
...
Result: 在POPE和CHAIR等基准测试中实现了最先进的幻觉减少效果，在MME上保持了一般能力，同时保持了高效的推理效率。该方法在多种架构上一致优于对比解码和静态子空间编辑基线。

Conclusion: HulluEdit为大视觉语言模型提供了一种新的可信赖路径，通过正交子空间编辑有效平衡了幻觉减少和视觉基础保持，实现了高效准确的单次前向干预。
...
Method: 提出Alignment-Aware Masked Learning (AML)训练策略，包含三个关键组件：显式估计像素级视觉-语言对齐、过滤对齐不良区域、聚焦可信线索进行优化
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要任务特定训练或缓慢优化，硬约束经常破坏运动自然性，需要一种能精确满足空间约束同时保持运动真实性的解决方案。

Method: 基于线性逆问题框架，引入运动学感知度量编码骨骼拓扑结构，通过相干分布修正避免不自然伪影；针对稀疏输入使用伪观测的时间变化公式。

Result: 在运动修复和2D到3D提升等应用中，ProjFlow实现了精确约束满足，在零样本基线上匹配或提高了真实感，与基于训练的控制器保持竞争力。

Conclusion: ProjFlow通过运动学感知度量和时间变化公式，实现了零样本精确约束满足的运动生成，在保持自然性的同时解决了现有方法的局限性。
...
Method: 提出SPATIALALIGN自改进框架，使用零阶正则化直接偏好优化(DPO)微调T2V模型；设计了基于几何的DSR-SCORE评估指标来定量测量视频与DSR的对齐程度；构建了包含多样化DSR的文本-视频对数据集。
...
TL;DR: 提出统一的隐码恢复框架，支持从后验和生成式水印中检索和恢复篡改内容，通过多尺度向量量化和条件Transformer增强上下文推理，构建ImageNet-S基准进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 当前图像真实性研究主要关注深度伪造检测和定位，而对篡改内容的恢复以进行事实检索的研究相对不足。需要开发能够从不同水印范式中恢复原始内容的通用框架。
...
TL;DR: TrajTok是一种端到端的视频分词器模块，通过时空像素聚类直接生成对象轨迹，动态调整分词粒度以适应语义复杂度，解决了传统视频模型分词冗余和效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 视频模型中的传统分词方法（如分块化）会产生过多冗余的token，严重限制了视频处理的效率和可扩展性。现有的轨迹分词器虽然能解耦视频时长与token数量，但依赖复杂的外部分割和跟踪流程，速度慢且与任务无关。

Method: 提出TrajTok端到端视频分词器模块，包含统一的segmenter，通过时空像素隐式聚类直接生成对象轨迹。该模块与视频模型完全集成并针对下游目标进行协同训练，动态调整分词粒度以适应语义复杂度。

Result: 基于TrajTok实现的TrajViT2视频CLIP模型在分类和检索基准测试中实现了最佳精度，同时保持了与最佳token合并方法相当的效率。TrajTok还可作为预训练视觉特征的探测头（TrajAdapter）或视觉语言模型中的对齐连接器（TrajVLM），在长视频推理中表现尤其出色。

Conclusion: TrajTok通过端到端的轨迹生成和动态粒度调整，解决了视频分词中的冗余问题，不仅提高了视频理解性能，还保持了高效率，并展现出作为多功能组件的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成部件级3D对象，但无法在开放世界场景中将部件组织成独立的实例。通过去偏聚类分析发现，这一失败源于模型内部分配机制缺乏结构约束。

Method: 将结构化3D场景生成重新定义为全局相关性分配问题，在组合DiT模型的去噪循环中制定并求解熵最优传输目标，施加两个结构约束：1）传输计划门控交叉注意力，强制图像块到部件级3D潜在表示的一对一独占路由；2）传输的竞争性鼓励相似块分组，并通过基于边缘的成本正则化形成连贯对象。

Result: 大量实验表明，SceneTransporter在开放世界场景生成方面优于现有方法，显著提高了实例级连贯性和几何保真度。

Conclusion: 通过将结构化3D场景生成重新定义为全局相关性分配问题，并引入最优传输约束，SceneTransporter成功解决了现有方法中部件级对象无法组织成独立实例的问题，实现了更好的开放世界场景生成效果。
...
TL;DR: 提出一种结合自监督骨架表示学习的鲁棒轨迹预测方法，在遮挡场景下提升对缺失骨架数据的鲁棒性
...
Method: 采用基于掩码自编码预训练的自监督骨架表示模型，增强对缺失骨架数据的鲁棒性
...
TL;DR: 提出GSTurb框架，通过光流引导的倾斜校正和高斯泼溅建模非等晕模糊，有效缓解大气湍流引起的图像退化


<details>
  <summary>Details</summary>
Motivation: 大气湍流在远距离成像中会导致像素位移（倾斜）和模糊，严重影响图像质量，需要有效的图像恢复方法

Method: 提出GSTurb框架，结合光流引导的倾斜校正和高斯泼溅建模非等晕模糊，使用高斯参数表示倾斜和模糊，通过多帧优化进行图像恢复

Result: 在ATSyn-static数据集上达到PSNR 27.67 dB和SSIM 0.8735，相比SOTA方法PSNR提升1.3 dB（4.5%），SSIM提升0.048（5.8%）；在真实数据集上也优于现有方法
...
Method: PhotoAgent将自主图像编辑建模为长期决策问题，通过推理用户美学意图，使用树搜索规划多步编辑动作，并通过带有记忆和视觉反馈的闭环执行迭代优化结果。

Result: 研究引入了UGC-Edit美学评估基准（包含7,000张照片和学习的美学奖励模型）和包含1,017张照片的测试集。实验表明，PhotoAgent在指令遵循和视觉质量方面持续优于基线方法。

Conclusion: PhotoAgent通过显式美学规划和长期决策方法，实现了高质量的自主图像编辑，显著减轻了用户负担，在真实场景中表现出优越性能。
...
TL;DR: FaceTT是一个基于扩散模型的框架，通过面部属性感知提示精炼、免调谐角度反演和自适应注意力控制，实现高保真、身份一致的面部年龄变换。


<details>
  <summary>Details</summary>
Motivation: 现有面部年龄变换方法存在几个关键问题：依赖数值年龄表示忽略了生物和环境因素的相互作用；在宽年龄变换中难以保持身份一致性；静态注意力和优化密集的反演限制了适应性、细粒度控制和背景一致性。

Method: 提出FaceTT框架，包含三个核心组件：1）面部属性感知提示精炼策略，编码内在（生物）和外在（环境）老化线索进行上下文感知条件化；2）免调谐角度反演方法，高效将真实面部映射到扩散潜在空间；3）自适应注意力控制机制，动态平衡用于语义老化线索的交叉注意力和用于结构及身份保持的自注意力。

Result: 在基准数据集和野外测试集上的广泛实验表明，FaceTT在身份保持、背景保留和老化真实性方面优于最先进的方法。

Conclusion: FaceTT通过创新的提示精炼、高效反演和动态注意力控制，解决了现有面部年龄变换方法的关键限制，实现了高质量的身份一致年龄变换。
...
TL;DR: CMSA-Net：一种用于视频息肉分割的鲁棒框架，通过因果多尺度聚合模块和动态多源参考策略，在保持实时性的同时实现高精度分割。


<details>
  <summary>Details</summary>
Motivation: 视频息肉分割面临两大挑战：1）息肉与周围黏膜外观相似导致语义区分弱；2）视频帧间息肉位置和尺度变化大导致分割不稳定。现有方法难以同时解决这两个问题。

Method: 提出CMSA-Net框架，包含两个核心组件：1）因果多尺度聚合模块（CMA），通过因果注意力机制按时间顺序聚合多尺度历史帧语义信息；2）动态多源参考策略（DMR），基于语义可分性和预测置信度自适应选择参考帧，提供多帧指导。

Result: 在SUN-SEG数据集上的大量实验表明，CMSA-Net达到了最先进的性能，在分割精度和实时临床适用性之间取得了良好平衡。

Conclusion: CMSA-Net通过因果时序建模和自适应参考选择，有效解决了视频息肉分割中的语义模糊和动态变化问题，为临床实时应用提供了实用解决方案。
...
TL;DR: 本文提出了一种基于多光谱成像和机器学习的方法，用于快速、准确地预测土壤成分和USDA质地分类，替代传统耗时的实验室测试。


<details>
  <summary>Details</summary>
Motivation: 土壤质地是农业和岩土工程中的重要属性，但传统实验室颗粒大小测试耗时费力，现有传感方案要么成本高，要么分辨率不足，无法支持常规田间部署。

Method: 开发低成本多光谱成像系统（365-940nm，13个光谱带），结合机器学习框架：回归模型预测粘土、粉砂、砂土百分比，直接分类器预测12个USDA质地类别，间接分类通过USDA土壤质地三角图映射。

Result: 实验结果显示，成分预测的R²高达0.99，质地分类准确率超过99%，表明该方法能够提供准确、非破坏性、可田间部署的土壤质地表征。
...
TL;DR: CheXficient是一个通过主动数据筛选而非盲目扩大数据集训练的医学影像基础模型，仅使用22.7%的数据和27.3%的计算资源就达到了与全数据训练相当或更好的性能。
...
Result: CheXficient在仅消耗27.3%总计算预算的情况下，实现了与全数据对应模型和其他大规模预训练模型相当或更优的性能。在20个基准测试中表现出色，涵盖5种任务类型，包括零样本发现分类、跨模态检索、疾病预测、语义分割和放射学报告生成。

Conclusion: 主动数据筛选可以作为暴力扩大数据集的有效、经济替代方案。CheXficient系统性地优先处理代表性不足的训练样本，提高了在长尾或罕见条件下的泛化能力，为医学视觉语言基础模型的高效预训练和下游适应提供了实用见解。
...
TL;DR: SO3UFormer：一种针对全景语义分割的旋转鲁棒架构，通过解耦特征与重力向量、考虑非均匀采样密度的注意力机制和局部几何编码，解决了现有方法在相机姿态变化时的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 全景语义分割模型通常基于严格的重力对齐假设训练，但实际应用中相机姿态常因手持设备抖动或空中平台动态变化而偏离标准方向，导致现有球形Transformer过度依赖全局纬度线索，在3D重定向时性能崩溃。

Method: 提出SO3UFormer架构，包含三个几何支柱：1) 通过移除绝对纬度编码解耦特征与重力向量的内在特征表示；2) 考虑非均匀采样密度的正交一致性球形注意力；3) 使用切平面投影角度和离散规范池化编码局部角度几何的规范感知相对位置机制。训练时采用基于索引的球形重采样和logit级SO(3)一致性正则化。

Result: 在Pose35数据集（斯坦福2D3D的±35°随机旋转变体）上，SO3UFormer达到72.03 mIoU，在任意完整SO(3)旋转测试中保持70.67 mIoU，而基线SphereUFormer从67.53 mIoU暴跌至25.26 mIoU。

Conclusion: SO3UFormer通过几何驱动的设计实现了对相机姿态变化的强鲁棒性，显著优于现有方法，为实际应用中的全景语义分割提供了可靠解决方案。
...
TL;DR: 提出Chain of Flow (COF)框架，从单次心电信号生成完整4D心脏结构与运动，实现从预测模型到患者特异性虚拟心脏的转变


<details>
  <summary>Details</summary>
Motivation: 现有心脏数字孪生框架局限于特定任务预测，缺乏患者特异性、可操作的虚拟心脏，无法进行广泛的下游模拟

Method: 集成cine-CMR和12导联心电图训练，学习心脏几何、电生理和运动动力学的统一表示，从单个心动周期重建完整4D心脏结构与运动

Result: 在不同队列上准确恢复心脏解剖结构、心室功能和动态运动模式，重建的4D心脏支持容积测量、区域功能分析和虚拟cine合成等下游任务

Conclusion: COF将心脏数字孪生从狭窄的预测模型转变为完全生成性的患者特异性虚拟心脏，实现了从心电信号直接重建完整4D器官的能力
...
TL;DR: 该论文提出了一个多模态增强现实框架，用于将逼真的虚拟物体集成到真实世界铁路序列中，以解决铁路障碍检测中高质量标注数据稀缺的问题，并发布了OSDaR-AR公开数据集。


<details>
  <summary>Details</summary>
Motivation: 铁路智能交通系统中的安全关键任务（如障碍物检测）面临高质量标注数据稀缺的挑战。现有方法存在局限性：逼真模拟器存在"模拟到现实"的差距，而简单的图像掩码技术缺乏时空一致性，无法获得具有正确外观和尺寸的增强单帧和多帧场景。

Method: 提出多模态增强现实框架，利用Unreal Engine 5功能，结合LiDAR点云和INS/GNSS数据，将逼真虚拟物体集成到真实世界铁路序列中。同时提出基于分割的INS/GNSS数据细化策略，以提高增强序列的真实感。

Result: 创建了OSDaR-AR公开数据集，包含精心设计的增强序列，支持下一代铁路感知系统的开发。通过比较研究证实了所提方法显著提高了增强序列的真实感。

Conclusion: 该研究成功开发了一个能够有效弥合"模拟到现实"差距的增强现实框架，为铁路感知系统提供了高质量的训练数据资源，推动了智能交通系统中安全关键任务的发展。
...
TL;DR: 本文提出了首个面向全水域环境的大规模视频问答基准WaterVideoQA，并开发了多智能体神经符号系统NaviMind，旨在提升自主水面舰艇从感知到认知推理的能力。


<details>
  <summary>Details</summary>
Motivation: 当前自主导航在被动感知方面取得显著进展，但在知识驱动的交互式环境认知方面存在根本性限制。在海上导航这一高风险领域，弥合原始视觉感知与复杂认知推理之间的差距，对于自主水面舰艇执行安全精确机动至关重要。

Method: 1. 构建WaterVideoQA基准：包含3,029个视频片段，覆盖六种不同水道类别，整合多变光照和动态天气等多方面变量，通过五层分层认知框架严格测试ASV能力。
2. 开发NaviMind系统：首创的多智能体神经符号系统，结合自适应语义路由、情境感知分层推理和自主自反验证，实现从表面模式匹配到符合法规的可解释决策。

Result: 实验结果表明，该框架显著超越了现有基线，为动态海洋环境中的智能可信交互建立了新范式。

Conclusion: 本文通过WaterVideoQA基准和NaviMind系统，推动了自主水面舰艇从被动感知向知识驱动认知推理的转变，为动态海洋环境中的智能可信交互建立了新的研究范式。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在理解长视频方面存在效率挑战，需要处理大量视频帧但只有少量关键帧真正对回答问题有用，因此需要开发能够智能选择关键帧的高效方法。

Method: 提出MSJoE框架：1）MLLM推理出多个描述问题相关视觉视角的查询；2）查询与冻结的CLIP模型交互生成查询-帧相似度矩阵；3）轻量级采样器从矩阵预测关键帧采样权重，选择紧凑的信息丰富帧集；4）MLLM和采样器通过强化学习联合优化，实现查询推理、帧采样和关键帧理解的协同适应。

Result: 收集了包含2.8K视频和7K问答对的新长视频QA数据集用于训练。在VideoMME、LongVideoBench、LVBench和MLVU上的实验表明，MSJoE相比基础MLLM获得8.0%准确率提升，比最强基线方法高1.1%准确率。

Conclusion: MSJoE通过联合进化MLLM和关键帧采样器，实现了高效的长视频理解，证明了仅选择少量信息丰富关键帧的假设有效性，并在多个基准测试中展示了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示调优技术通常只利用单一预训练模型的知识，忽略了整合不同领域专家知识可能带来的协同效应

Method: 提出pMoE方法，使用专家专用提示令牌和可学习调度器，在不同提示层动态调度令牌以优化各领域专家的贡献
...
TL;DR: 本文提出了一种基于注意力熵的VAR模型优化框架，通过分析token粒度、语义范围和生成规模三个维度的参数动态，识别稀疏模式并设计细粒度优化策略，在Infinity-2B和Infinity-8B模型上实现最高3.4倍加速且质量损失最小。


<details>
  <summary>Details</summary>
Motivation: VAR模型虽然提升了生成质量，但在后期阶段面临严重的效率瓶颈。现有方法如FastVAR和SkipVAR依赖启发式跳过策略，存在局限性。需要一种更系统的方法来优化VAR模型的生成效率。

Method: 提出基于注意力熵的优化框架，通过注意力熵来表征模型架构不同维度的语义投影。分析token粒度、语义范围和生成规模三个维度下的参数动态，识别token、层和尺度三个关键维度的稀疏模式，并针对这些模式设计细粒度的优化策略。

Result: 在Infinity-2B和Infinity-8B模型上的实验表明，ToProVAR方法实现了最高3.4倍的加速，同时保持了语义保真度和细节质量，显著优于传统方法。

Conclusion: 提出的基于注意力熵的优化框架能够有效解决VAR模型的效率瓶颈问题，通过系统分析参数动态和稀疏模式，实现了加速生成过程的同时保持高质量输出，为VAR模型的实际应用提供了有效的优化方案。
...
TL;DR: OpenFS是一个开源的手语拼写识别与合成系统，通过多手能力识别器、签名手焦点损失和单调对齐损失解决手语拼写识别中的关键挑战，并引入帧级字母条件生成器处理OOV问题。


<details>
  <summary>Details</summary>
Motivation: 手语拼写识别在聋人和听人社区之间的沟通中起着关键作用，但面临签名手歧义、缺乏适当训练损失和词汇外问题等挑战。现有方法依赖显式签名手检测和CTC损失，存在识别失败和峰值行为问题。

Method: 提出OpenFS方法：1）多手能力识别器支持单/多手输入，通过双级位置编码和签名手焦点损失实现隐式签名手检测；2）单调对齐损失通过交叉注意力正则化强制输出字母序列遵循输入姿势序列的时间顺序；3）帧级字母条件生成器合成逼真的手语拼写姿势序列处理OOV词汇。

Result: 通过综合实验证明，该方法在手语拼写识别方面达到最先进的性能，验证了所提识别器和生成器的有效性。构建了新的合成基准FSNeo。

Conclusion: OpenFS通过隐式签名手检测、改进的训练损失和OOV词汇合成能力，有效解决了手语拼写识别的关键挑战，为聋听沟通提供了更强大的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有脑肿瘤诊断模型需要不仅能检测病变，还要生成基于影像表现的临床可解释推理，但当前公共数据集在标注丰富性和诊断语义方面存在不足。诊断语义标注稀缺且成本高昂，需要新的解决方案来弥合这一差距。

Method: 开发了多模型协作管道来自动完成医学信息并控制质量，生成超越仅掩码标注的诊断相关语义。构建了MM-NeuroOnco数据集和MM-NeuroOnco-Bench评估基准，后者采用拒绝感知设置以减少封闭式问题格式的偏差。基于该数据集进一步提出了NeuroOnco-GPT模型。

Result: 在十个代表性模型评估中，最强的基线模型Gemini 3 Flash在诊断相关问题上的准确率仅为41.88%，凸显了多模态脑肿瘤诊断理解的巨大挑战。通过微调，NeuroOnco-GPT在诊断问题上实现了27%的绝对准确率提升。

Conclusion: MM-NeuroOnco数据集和基准在推进基于临床的多模态诊断推理方面具有显著效果，NeuroOnco-GPT的优异表现证明了该数据集的实用价值。代码和数据集已公开可用。
...
TL;DR: UCM框架通过时间感知位置编码扭曲机制统一长期记忆和精确相机控制，使用双流扩散变换器实现高效高保真生成，在长期场景一致性和相机可控性方面超越现有方法。
...
Conclusion: UCM通过统一长期记忆和相机控制的创新机制，解决了视频生成世界模型的关键挑战，为交互环境模拟提供了更强大和可控的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的光流图像扭曲方法在图像对齐任务中容易受到遮挡和光照变化的影响，导致对齐质量下降和下游任务精度受损。需要从新角度解决这些问题。

Method: 提出DMAligner框架，采用基于生成的方法：1）动态感知扩散训练方法学习条件图像生成，合成对齐所需的新视角；2）动态感知掩码生成模块自适应区分动态前景和静态背景；3）使用Blender创建了包含1033个室内外场景、超过3万对图像的动态场景图像对齐数据集。

Result: 在DSIA基准测试和多个广泛使用的视频数据集上的定性比较中，该方法表现出优越性能，证明了其有效性。

Conclusion: DMAligner通过生成式方法为图像对齐提供了新视角，避免了传统光流方法的局限性，在处理动态场景对齐方面展现出强大能力。
...
Method: 提出WISER框架，采用"检索-验证-精炼"流程：1）更宽搜索：同时生成编辑后的文本和图像进行并行检索以扩大候选池；2）自适应融合：使用验证器评估检索置信度，对不确定检索触发精炼，对可靠检索动态融合双路径结果；3）深度思考：对不确定检索通过结构化自反思生成精炼建议指导下一轮检索。

Result: 在多个基准测试中显著优于先前方法，在CIRCO上相对提升45%（mAP@5），在CIRR上相对提升57%（Recall@1），甚至超过许多需要训练的方法，展现了优越性和泛化能力。

Conclusion: WISER通过统一T2I和I2I范式，显式建模意图感知和不确定性感知，有效解决了零样本组合图像检索中的模态转换局限问题，为无需训练的高性能检索提供了新思路。
...
TL;DR: PackUV提出了一种新颖的4D高斯表示方法，将高斯属性映射到结构化多尺度UV图集序列中，实现紧凑的图像原生存储，并与标准视频编解码器兼容，解决了长序列重建、时间不一致性以及大规模运动和遮挡等问题。
...
Method: 提出了PackUV-GS方法：1）将高斯属性映射到结构化多尺度UV图集序列；2）在UV域直接优化高斯参数；3）使用流引导的高斯标记和视频关键帧模块识别动态高斯、稳定静态区域并保持时间一致性；4）创建与标准视频编解码器兼容的UV图集格式。

Result: 方法在渲染保真度上超越现有基线，能够扩展到长达30分钟的序列并保持一致性质量。创建了PackUV-2B数据集（迄今最大的多视角视频数据集），包含50多个同步相机、大量运动和频繁遮挡的100个序列和20亿帧。

Conclusion: PackUV首次实现了与标准视频编解码器兼容的统一体素视频表示，无需损失质量即可在现有多媒体基础设施中实现高效流媒体传输，解决了长序列体素视频重建、存储和流媒体的关键挑战。
...
TL;DR: D-FINE-seg是D-FINE目标检测器的实例分割扩展，通过轻量级掩码头、分割感知训练等技术，在TACO数据集上超越YOLO26的F1分数，同时保持实时性能，并提供完整的端到端训练推理框架。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的实时目标检测器（如D-FINE）在精度-延迟权衡方面表现优异，但基于Transformer的实时实例分割方法仍然较少。研究旨在将D-FINE扩展到实例分割任务，同时保持实时性能。

Method: 1. 添加轻量级掩码头；2. 分割感知训练：包括框裁剪BCE和Dice掩码损失；3. 辅助和去噪掩码监督；4. 适配的匈牙利匹配成本；5. 提供完整的端到端训练、导出和优化推理管道，支持ONNX、TensorRT、OpenVINO等多种框架。

Result: 在TACO数据集上，D-FINE-seg在统一的TensorRT FP16端到端基准测试协议下，F1分数优于Ultralytics YOLO26，同时保持了有竞争力的延迟性能。

Conclusion: D-FINE-seg成功将D-FINE扩展到实例分割任务，在保持实时性能的同时提升了分割精度，并提供了完整的开源框架，支持多种推理引擎的优化部署。
...
TL;DR: GeoWorld提出了一种几何世界模型，通过双曲JEPA将潜在表示映射到双曲流形上，解决了现有基于能量的预测世界模型在几何结构保持和长时程预测方面的挑战。
...
Method: 引入GeoWorld几何世界模型，通过双曲JEPA将潜在表示从欧几里得空间映射到双曲流形上，保持几何结构和层次关系。进一步引入几何强化学习进行基于能量的优化，实现双曲潜在空间中的稳定多步规划。

Result: 在CrossTask和COIN数据集上的实验表明，与最先进的V-JEPA 2相比，3步规划中SR提升约3%，4步规划中SR提升约2%。

Conclusion: GeoWorld通过将几何结构融入世界模型，显著提升了基于能量的视觉规划的长时程预测能力，为多步视觉规划提供了更稳定和有效的解决方案。
...
TL;DR: 提出一种通过标签中介生成图像描述的方法，无需成对的图像-文本数据，将细胞架构视觉基础模型与语言模型连接，为显微图像生成自然语言描述。


<details>
  <summary>Details</summary>
Motivation: 在研究和临床环境中，获取成对的图像-文本数据稀缺且困难，特别是在细胞架构分析领域。需要一种方法将视觉基础模型与语言模型连接，提供自然语言界面，而无需大量标注数据。

Method: 通过标签中介的方法，利用标签将图像和文本连接。给定标签后，自动从相关文献中挖掘区域描述作为合成标题，反映典型的细胞架构属性。然后将现有的细胞架构视觉基础模型（CytoNet）通过图像到文本的训练目标与大型语言模型耦合。

Result: 在57个脑区上，该方法生成了合理的区域级描述，并支持通过显式拒绝未见区域实现开放集使用。在范围内补丁上匹配细胞架构参考标签的准确率达到90.6%，在掩码区域标签的情况下，描述仍具有足够区分度，在8路测试中恢复区域的准确率达到68.6%。
...
TL;DR: SpectralMamba-UNet：一种新颖的频率解耦框架，通过谱域分解将结构信息和纹理信息分离学习，结合频域Mamba进行全局建模，同时保留边界细节，在多个医学图像分割基准上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有状态空间模型（如Vision Mamba）虽然能有效建模长程依赖，但其一维序列化处理会削弱局部空间连续性和高频表示能力，而医学图像分割需要同时建模全局解剖结构和细粒度边界细节。

Method: 提出SpectralMamba-UNet框架，包含三个核心模块：1）谱分解与建模模块（SDM），使用离散余弦变换分解低频和高频特征，低频通过频域Mamba进行全局上下文建模，高频保留边界敏感细节；2）谱通道重加权机制（SCR），形成通道级频率感知注意力；3）谱引导融合模块（SGF），在解码器中实现自适应多尺度融合。

Result: 在五个公开基准数据集上的实验表明，该方法在多种模态和分割目标上均取得一致改进，验证了方法的有效性和泛化能力。

Conclusion: SpectralMamba-UNet通过谱域解耦学习，有效平衡了全局结构建模和局部细节保留，为医学图像分割提供了一种新颖且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有组合零样本学习方法在测试时面临分布偏移问题，因为测试集包含从未见过的属性-对象组合，这导致性能下降。需要一种能够适应测试时分布变化的方法。

Method: 1) 从无监督数据积累文本和视觉模态的全面知识来更新多模态原型；2) 设计自适应更新权重控制原型调整程度；3) 引入动态优先级队列存储高置信度图像；4) 通过多模态协同表示学习对齐文本和视觉原型；5) 提出新的C-Fashion基准数据集并改进MIT-States数据集。

Result: 在四个基准数据集（包括新提出的C-Fashion和改进的MIT-States）上，该方法在封闭世界和开放世界设置下均取得了最先进的性能。
...
TL;DR: 提出一种基于单位球面霍夫变换的新方法，用于从单目视频中估计相机航向，在噪声和异常值较多的情况下仍能保持高精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在已知旋转（来自IMU或优化算法）下恢复相机航向时，在低噪声、低异常值条件下表现良好，但随着噪声和异常值水平增加，精度下降或计算成本变高。需要一种更鲁棒且高效的航向估计方法。

Method: 提出单位球面霍夫变换的泛化方法：1）提取两帧间的对应关系；2）为每对对应关系生成兼容方向的大圆；3）使用斐波那契晶格离散化单位球面作为bin中心；4）每个大圆为一组方向投票，确保不受噪声或动态物体影响的特征一致地为正确运动方向投票。

Result: 在三个数据集上的实验表明，该方法在精度与效率的帕累托前沿上表现优异。在SLAM实验中，通过校正相机位姿初始化时的航向，显著降低了RMSE。

Conclusion: 该方法提供了一种鲁棒且高效的相机航向估计解决方案，特别适用于噪声和异常值较多的场景，在SLAM等应用中能有效提高系统精度。
...
TL;DR: 本文提出了一个评估对抗性可迁移攻击的标准化框架，通过系统梳理现有研究，将攻击方法分为六类，并建立了公平比较的基准。


<details>
  <summary>Details</summary>
Motivation: 对抗性可迁移性使得攻击者无需直接访问目标模型即可发起攻击，带来了严重的安全隐患。然而，目前缺乏标准化的评估框架和标准，导致对现有攻击方法的评估可能存在偏差。

Method: 1. 对数百篇相关文献进行系统性综述，将基于迁移的攻击方法组织为六个不同类别；2. 提出一个全面的评估框架作为基准；3. 总结增强对抗性可迁移性的常见策略；4. 识别可能导致不公平比较的常见问题。

Result: 建立了一个标准化的评估框架，能够更公平地比较不同对抗性可迁移攻击方法的性能，同时总结了有效的增强策略和评估注意事项。

Conclusion: 本文填补了对抗性可迁移攻击评估领域的标准化空白，为未来研究提供了统一的基准框架，有助于更准确、公平地评估和比较不同攻击方法的有效性。
...
TL;DR: CARE方法通过校准-精炼两阶段框架解决带噪声标签的person Re-ID问题，使用概率证据传播提高模型鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有噪声鲁棒性person Re-ID方法主要依赖softmax输出的损失校正或样本选择策略，但存在两个关键限制：1) Softmax具有平移不变性，导致对污染标签的预测过于自信且不可靠；2) 基于小损失准则的传统样本选择往往会丢弃对学习判别特征至关重要的有价值难正样本

Method: 提出CARE（CAlibration-to-REfinement）两阶段框架：1) 校准阶段：提出概率证据校准（PEC），通过向相似度函数注入自适应可学习参数来消除softmax平移不变性，并使用证据校准损失减轻对误标记样本的过度自信；2) 精炼阶段：设计证据传播精炼（EPR），包含复合角度边界（CAM）度量和确定性导向的球面加权（COSW），在超球面空间中精确区分干净但难学习的正样本与误标记样本

Result: 在Market1501、DukeMTMC-ReID和CUHK03数据集上，在随机和模式化噪声条件下进行广泛实验，CARE取得了有竞争力的性能

Conclusion: CARE方法通过从校准到精炼的概率证据传播，有效解决了带噪声标签的person Re-ID问题，克服了现有方法的局限性
...
TL;DR: 提出新的无监督在线视频稳定框架，采用三阶段经典流程和多线程缓冲机制，无需配对数据集，在资源受限硬件上高效运行，并引入多模态无人机夜间遥感数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的方法需要配对稳定/不稳定数据集，存在数据有限、可控性差、资源受限硬件效率低的问题。现有基准测试主要针对手持设备可见光前向视频，限制了在无人机夜间遥感等领域的应用。

Method: 提出无监督在线视频稳定框架，采用三阶段经典稳定化流程（特征提取、运动估计、平滑），结合多线程缓冲机制，无需配对训练数据，在资源受限硬件上高效运行。

Result: 实验表明，该方法在定量指标和视觉质量上均优于现有最先进的在线稳定器，性能接近离线方法。同时引入了新的多模态无人机空中视频数据集（UAV-Test）。

Conclusion: 提出的无监督在线视频稳定框架解决了深度学习方法的局限性，在资源受限硬件上表现优异，扩展了视频稳定在无人机夜间遥感等领域的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D大模型依赖笨重的预训练视觉编码器提取几何特征，而2D模型已开始消除编码器以提高效率和可扩展性。将这一范式扩展到3D面临挑战，因为点云具有无序性和大规模特性，需要设计无需编码器的高效3D数据标记化方法
...
Conclusion: Fase3D成功解决了无序3D数据的高效标记化问题，通过傅里叶基方法实现了无需编码器的3D大模型，为3D场景理解提供了更高效、可扩展的解决方案
...
TL;DR: DyaDiT：基于多模态扩散变换器的双人对话手势生成模型，考虑社交上下文和互动动态
...
Method: 提出DyaDiT多模态扩散变换器，从双人音频信号生成上下文适当的人类动作。融合双方说话者信息捕捉互动动态，使用动作词典编码动作先验，可选利用对话伙伴的手势生成更具响应性的动作

Result: 在标准动作生成指标上超越现有方法，用户研究显示用户强烈偏好DyaDiT生成的动作，证明其鲁棒性和社交友好的动作生成能力

Conclusion: DyaDiT通过考虑社交上下文和双人互动动态，能够生成更自然、社交参与度更高的对话手势，为数字人交互提供了更先进的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的一体化图像修复方法在面对分布外退化时泛化能力有限。本文基于直觉：多源退化特征分布是由不同退化特定偏移从底层退化无关分布诱导而来，恢复这种共享分布对于跨退化泛化至关重要。

Method: 提出BaryIR框架，将多源退化特征在Wasserstein重心空间中对齐，该空间通过最小化与多源退化分布的Wasserstein距离平均值来建模退化无关分布。同时引入残差子空间，其嵌入相互对比并保持与WB嵌入正交，从而解耦两个正交空间：编码跨退化共享不变内容的WB空间，以及自适应保留退化特定知识的残差子空间。

Result: BaryIR在性能上与最先进的一体化方法相当，在未见退化类型和级别上表现出良好的泛化能力，即使在有限退化类型上训练并在混合退化的真实世界数据上评估时，也能学习到泛化特征。

Conclusion: BaryIR通过解耦退化无关共享内容和退化特定知识，有效缓解了对分布内退化的过拟合，实现了基于退化无关共享不变性的自适应修复，显著提升了对未见退化的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在动态环境感知方面存在局限：要么只能提供粗糙的几何跟踪（如边界框），要么只能提供缺乏明确时间关联的详细3D结构（如体素占用）。需要一种能够同时处理几何跟踪和时空关联的完整解决方案。

Method: 提出潜在高斯泼溅方法，首先将观测融合到3D高斯中作为稀疏点中心潜在表示，然后将聚合的特征泼溅到3D体素网格上，通过基于掩码的分割头进行解码，实现端到端的相机跟踪和基于掩码的多视角全景占用预测。

Result: 在Occ3D nuScenes和Waymo数据集上评估，实现了4D全景占用跟踪的最先进性能。

Conclusion: LaGS通过潜在高斯泼溅技术为时空场景理解提供了一个整体解决方案，能够高效聚合多视角信息并实现精确的4D全景占用跟踪。
...
TL;DR: 提出了一种基于物理约束的实时人群计数方法，利用安装在列车上的单摄像头扫描站台，通过物理约束的卡尔曼滤波模型和虚拟计数带实现高精度计数


<details>
  <summary>Details</summary>
Motivation: 铁路站台准确、实时的人群计数对安全和容量管理至关重要。现有跟踪检测方法通常假设静态摄像头或忽略运动建模中的物理一致性，在列车进站时的动态条件下计数不可靠

Method: 提出物理约束跟踪框架，将检测、外观和3D运动推理统一到实时流程中。集成迁移学习的YOLOv11m检测器和EfficientNet-B0外观编码，引入物理约束的卡尔曼模型(Phys-3D)，通过针孔几何强制物理上合理的3D运动动力学。为应对遮挡问题，实现带持久性的虚拟计数带

Result: 在平台基准测试MOT-RailwayPlatformCrowdHead数据集上，该方法将计数误差降低到2.97%，在运动和遮挡条件下表现出稳健性能
...
TL;DR: Uni-Animator是一个基于扩散Transformer的统一图像和视频草图着色框架，解决了现有方法在颜色迁移不精确、物理细节保留不足和运动场景时间一致性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有草图着色方法难以统一处理图像和视频任务，存在三个主要问题：1）使用单参考或多参考时颜色迁移不精确；2）高频物理细节保留不足；3）大运动场景中时间一致性差，存在运动伪影。
...
Conclusion: Uni-Animator成功解决了现有草图着色方法的局限性，通过创新的技术设计实现了图像和视频任务的统一处理，在保持高质量细节和时间一致性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机在动态场景中会产生大量由独立运动物体和自身运动触发的事件，这些事件会干扰下游视觉任务。现有方法难以实时区分和抑制这些动态事件，需要一种能实时过滤动态事件并支持下游应用的解决方案。

Method: 提出运动感知事件抑制框架，联合学习分割当前事件流中的独立运动物体并预测其未来运动，实现前瞻性动态事件抑制。采用轻量级架构设计，在消费级GPU上实现高效推理。

Result: 在EVIMO基准测试中，分割精度比之前最优方法提升67%，推理速度提升53%，达到173Hz。内存使用少于1GB。下游应用方面：通过token剪枝将Vision Transformer推理加速83%，将事件视觉里程计精度提升13%（ATE降低）。

Conclusion: 该框架首次实现了实时运动感知事件抑制，不仅能有效过滤动态事件，还能显著提升下游视觉任务的性能和效率，为事件相机在动态环境中的应用提供了实用解决方案。
...
TL;DR: EmbodMocap：使用两部移动iPhone进行便携式人体运动捕捉，实现场景一致的人体与场景重建，支持多种具身AI任务。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉系统依赖昂贵的专业设备和固定摄像头，难以在自然环境中大规模收集场景条件化的人体运动数据，限制了具身AI的发展。

Method: 提出EmbodMocap系统，使用两部移动iPhone同时采集RGB-D序列，通过联合校准将人体和场景重建到统一的度量世界坐标系中，无需固定摄像头或标记点。

Result: 双视角设置能有效缓解深度模糊问题，相比单iPhone或单目模型，在人体重建对齐方面表现更优。收集的数据成功应用于单目人体-场景重建、基于物理的角色动画和机器人运动控制三个具身AI任务。
...
Result: 白内障和青光眼对特征图造成严重破坏，这与已知的这些疾病中神经处理挑战相一致。激活能量和余弦相似度等评估指标量化了这些扭曲的严重程度。

Conclusion: 该计算框架提供了关于退化视觉输入与学习表示之间相互作用的见解，有助于理解视觉障碍如何影响神经处理过程。
...
TL;DR: 论文提出Dual-Coupled PnP Diffusion框架，通过引入对偶变量和频谱同质化技术，解决了现有PnP扩散先验方法中的稳态偏差和幻觉问题，在CT和MRI重建中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有PnP扩散先验框架存在关键缺陷：基于HQS或近端梯度的求解器作为无记忆算子运行，仅依赖瞬时梯度更新估计，缺乏历史跟踪导致稳态偏差，在严重噪声下无法严格满足物理测量约束。

Method: 提出Dual-Coupled PnP Diffusion框架：1) 恢复经典对偶变量提供积分反馈，理论上保证渐近收敛到精确数据流形；2) 引入频谱同质化技术，将结构化残差调制为符合AWGN假设的伪AWGN输入，使优化轨迹与去噪器统计流形对齐。

Result: 在CT和MRI重建实验中，该方法解决了偏差-幻觉权衡问题，实现了最先进的保真度，并显著加速了收敛速度。

Conclusion: 通过结合对偶耦合和频谱同质化，提出的框架在保持理论收敛保证的同时，解决了PnP扩散先验中的结构化残差问题，为成像逆问题提供了更可靠的解决方案。
...
TL;DR: 该论文提出了多维任务学习（MTL）框架，基于广义爱因斯坦MLPs（GE-MLPs），通过爱因斯坦积直接在张量上操作，突破了传统矩阵思维对计算机视觉任务的限制。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉任务公式受到矩阵思维的固有约束：标准架构依赖矩阵值权重和向量值偏置，需要进行结构扁平化，这限制了自然可表达任务的空间。作者旨在打破这一限制，提供更通用的数学框架。

Method: 提出基于广义爱因斯坦MLPs（GE-MLPs）的多维任务学习（MTL）框架，使用张量值参数，通过爱因斯坦积直接在张量上操作，实现对哪些维度被保留或收缩的显式控制，避免信息损失。

Result: 通过严格的数学推导证明，分类、分割和检测都是MTL的特殊情况，仅在形式定义的任务空间中的维度配置不同。证明该任务空间严格大于矩阵公式能原生表达的空间，支持时空或跨模态预测等需要破坏性扁平化的任务配置。

Conclusion: 该工作为通过张量代数视角理解、比较和设计计算机视觉任务提供了数学基础，突破了传统矩阵思维的局限性，实现了更灵活和通用的任务表达。
...
TL;DR: MovieTeller：一个无需训练、工具增强的渐进式抽象框架，用于生成电影剧情摘要，通过外部人脸识别工具提供事实基础，解决现有视觉语言模型在长视频摘要中角色识别不一致和叙事连贯性差的问题。


<details>
  <summary>Details</summary>
Motivation: 随着数字娱乐的爆炸式增长，自动化视频摘要对于内容索引、个性化推荐和高效媒体归档变得不可或缺。现有的视觉语言模型在长视频（如电影和电视剧）的自动摘要生成方面面临重大挑战，主要问题是缺乏ID一致的角色识别和叙事连贯性断裂。

Method: 提出MovieTeller框架，采用无需训练、工具增强、事实基础的生成过程。首先调用专门的人脸识别模型作为外部"工具"建立事实基础（精确的角色身份和对应的边界框），然后将这些基础信息注入提示词以引导视觉语言模型的推理。采用渐进式抽象管道，将完整电影的摘要分解为多阶段过程，有效缓解当前视觉语言模型的上下文长度限制。

Result: 实验表明，与端到端基线方法相比，该方法在事实准确性、角色一致性和整体叙事连贯性方面都有显著改进。

Conclusion: MovieTeller框架通过工具增强的渐进式抽象方法，有效解决了长视频摘要中的关键问题，提供了一种无需微调、即插即用的解决方案，显著提升了电影剧情摘要的质量。
...
TL;DR: LMMs通过上下文学习在分类任务中表现优异，提出的CIRCLE方法在开放世界分类中超越VLM模型
...
Method: 1. 在闭世界分类任务中基准测试最先进的LMMs；2. 分析开放世界设置下LMMs的表现；3. 提出CIRCLE方法——一种无需训练的方法，通过为上下文示例分配伪标签并利用可用上下文迭代优化它们来解决不完美上下文信息问题。

Result: 1. 闭世界分类：LMMs通过少量上下文示例可以匹配甚至超越带有缓存适配器的对比VLM；2. 开放世界分类：LMMs在不完美上下文信息下表现不佳；3. CIRCLE方法在开放世界分类中建立了稳健的基准，超越了VLM对应模型。

Conclusion: LMMs具有作为统一分类器的潜力，是专门模型的灵活替代方案。上下文学习能力使LMMs在分类任务中表现出色，特别是在开放世界设置下，CIRCLE方法进一步提升了其性能。
...
TL;DR: 通过多视角相机三角测量获取更精确的3D骨架数据，能显著提升动作识别模型的性能，表明输入数据质量是当前模型性能的限制因素。


<details>
  <summary>Details</summary>
Motivation: 当前骨架动作识别研究主要关注改进机器学习算法，但对输入骨架数据质量关注不足。本文旨在证明通过提升输入数据质量（使用多视角相机获取更精确的3D骨架）可以显著提升现有模型的性能。

Method: 使用多视角相机系统进行三角测量，获取更精确的3D骨架数据，然后将这些高质量数据输入到现有的最先进动作识别模型中。

Result: 实验表明，使用多视角三角测量获取的更精确3D骨架数据，能显著提升现有最先进动作识别模型的性能，证明输入数据质量是当前模型性能的主要限制因素。

Conclusion: 在多视角应用中，使用多个相机的成本效益比非常有利，因此未来骨架动作识别研究应将多视角设置作为标准配置，以获取更高质量的输入数据。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在敏感图像数据集上训练时存在记忆和复现训练样本的风险，需要强隐私保证。传统的差分隐私微调（如DP-SGD）通常会导致图像质量严重下降，特别是在高频纹理方面，因为噪声被不加区分地添加到所有模型参数中

Method: 提出两阶段谱差分隐私框架：1）在敏感图像的低分辨率小波系数上差分隐私微调自回归谱图像分词器模型；2）使用公开预训练的超分辨率模型进行高分辨率上采样。通过将隐私预算限制在图像全局结构上，并利用DP的后处理特性进行细节细化

Result: 在MS-COCO和MM-CelebA-HQ数据集上的实验表明，该方法相对于其他领先的DP图像框架，生成的图像具有更好的质量和风格捕捉能力

Conclusion: 通过将隐私保护集中在图像的低频隐私敏感部分，而将高频通用部分视为公开信息，该谱差分隐私框架在隐私和效用之间实现了有前景的权衡，显著提升了差分隐私图像生成的质量
...
TL;DR: LineGraph2Road是一个从卫星图像自动提取道路的新框架，通过将连通性预测转化为稀疏欧几里得图中边的二元分类，并使用线图和图变换器来学习结构链接表示，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 从卫星图像中准确自动提取道路对于导航和城市规划应用至关重要，但现有方法在捕捉长距离依赖和复杂拓扑结构方面存在困难，特别是将任务分解为关键点提取和连通性预测的方法。

Method: 提出LineGraph2Road框架：1) 从分割掩码中提取关键点作为节点，在预定义距离阈值内连接节点对构建稀疏欧几里得图；2) 将原始图转换为线图，应用图变换器进行连通性预测；3) 引入立交桥/下穿通道头解决多层交叉问题；4) 使用耦合NMS策略保留关键连接。

Result: 在City-scale、SpaceNet和Global-scale三个基准测试上评估，在TOPO-F1和APLS两个关键指标上取得了最先进的结果，同时能够捕捉对实际部署至关重要的精细视觉细节。

Conclusion: LineGraph2Road通过将连通性预测转化为线图中的二元分类，克服了现有方法在端点嵌入融合和集合同构链接方面的限制，实现了丰富的链接表示和全局结构上的有效关系推理，显著提升了道路提取性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统免疫组化染色分析受限于小活检组织样本量不足，无法进行全面的多重染色分析。现有虚拟多重染色方法面临语义指导不足、染色分布不一致和跨染色模态空间错位三大挑战。

Method: 提出了PGVMS框架，包含三个核心技术：1) 自适应提示引导机制，使用病理视觉语言模型动态调整染色提示；2) 蛋白质感知学习策略，通过直接量化和约束蛋白质分布来保持精确表达模式；3) 原型一致性学习策略，建立跨图像语义交互以校正空间错位。

Result: 该框架仅使用单重训练数据就能实现虚拟多重IHC染色，有效解决了现有方法面临的三大挑战，提高了染色结果的准确性和一致性。

Conclusion: PGVMS框架为病理学中的虚拟多重免疫组化染色提供了一种创新解决方案，能够克服小组织样本的限制，实现更全面的蛋白质表达分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要针对几十秒的短视频，而实际场景中视频可能长达数分钟甚至数小时，现有方法难以处理长视频中的长时跨度和大量无关信息
...
TL;DR: 提出ManifoldGD方法，通过流形一致性引导实现无需训练的数据集蒸馏，利用多尺度IPC核心集和局部切空间投影提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 大型数据集训练效率低且包含冗余概念，现有基于扩散模型的无训练蒸馏方法引导策略有限，要么无引导去噪，要么依赖简单的基于原型的模式引导，效果欠佳。

Method: 提出流形引导蒸馏框架：1) 通过分层聚类VAE潜在特征计算多尺度IPC核心集；2) 在每个去噪时间步构建局部潜在流形；3) 将模式对齐向量投影到局部切空间，约束生成轨迹保持流形一致性。

Result: 在FID、真实与合成数据集嵌入的l2距离、分类准确率等指标上，均优于现有的无训练和基于训练的基线方法。

Conclusion: ManifoldGD是首个几何感知的无训练数据蒸馏框架，通过流形一致性引导提升了代表性、多样性和图像保真度，无需模型重新训练。
...
TL;DR: SeeThrough3D是一个用于3D布局条件生成的模型，通过显式建模遮挡关系来解决现有方法在生成部分遮挡物体时的问题，实现了深度一致的几何和尺度生成。
...
Method: 提出遮挡感知的3D场景表示(OSCR)，将物体表示为虚拟环境中的半透明3D框，从所需相机视角渲染。透明度编码隐藏物体区域，使模型能够推理遮挡关系。通过引入从渲染3D表示派生的视觉标记来条件化预训练的基于流的文本到图像生成模型，并使用掩码自注意力准确绑定每个物体边界框与其对应的文本描述。

Result: 模型在具有强烈物体间遮挡的多样化多物体合成数据集上训练，能够有效泛化到未见过的物体类别，实现精确的3D布局控制，生成具有真实遮挡和一致相机控制的场景。

Conclusion: SeeThrough3D通过显式建模遮挡关系，解决了3D布局条件生成中的关键挑战，实现了更精确的物体间遮挡建模和相机控制，为3D场景生成提供了更强大的工具。
...
TL;DR: PRIMA是一个多模态医学诊断预训练框架，通过整合视觉特征和临床元数据，利用风险-疾病相关性知识和四种损失函数实现跨模态对齐，显著提升疾病分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学诊断方法通常将临床元数据视为孤立标签，未能充分利用临床描述中丰富的语义知识，需要一种能有效整合视觉表现和临床专业知识的方法。

Method: 1. 通过RAG构建风险-疾病相关性专家语料库，精炼Clinical ModernBERT文本编码器；2. 使用DINOv3和精炼BERT的双编码器预训练策略；3. 设计四种互补损失函数实现多粒度语义对齐；4. 利用Qwen-3融合对齐特征进行疾病分类。

Result: PRIMA在实验中显著优于其他最先进方法，有效协调像素级特征和抽象临床专业知识，展现出优越的鲁棒性，且无需大规模数据收集或大量计算资源。

Conclusion: PRIMA框架成功将领域特定知识整合到多模态表示学习中，通过风险集成图像-元数据对齐，实现了医学诊断中视觉表现和临床元数据的有效融合，为精准疾病分类提供了创新解决方案。
...
TL;DR: 本文提出了一种检索增强的测试时适配器，通过融合文本和视觉支持特征来学习轻量级、每图像分类器，显著缩小了零样本和监督分割之间的差距。
...
Method: 引入少样本设置，通过像素标注图像的支持集增强文本提示；提出检索增强的测试时适配器，通过学习每图像分类器，融合文本和视觉支持特征；采用基于查询的融合而非传统的手工后期融合。

Result: 实验表明，该方法显著缩小了零样本和监督分割之间的性能差距，同时保持了开放词汇能力；支持持续扩展的支持集，适用于细粒度任务如个性化分割。

Conclusion: 通过融合文本和视觉支持特征的检索增强测试时适配器，有效解决了OVS中的语义模糊性和监督不足问题，在保持开放词汇能力的同时大幅提升了分割性能。
...
TL;DR: 该论文研究事件相机内在参数如何影响基于事件数据的物体检测模型性能，并利用这些发现提升模型对传感器无关的鲁棒性。
...
Result: 论文提供了对内在参数如何影响事件数据训练模型性能的深入理解，特别是针对物体检测任务，并展示了如何利用这些发现来增强模型的传感器无关鲁棒性。

Conclusion: 通过系统分析事件相机的内在参数及其对物体检测模型的影响，该研究填补了事件相机数据分析和参数理解方面的空白，为开发更鲁棒的传感器无关模型提供了理论基础。
...
TL;DR: VGG-T³提出了一种可扩展的3D重建模型，通过将可变长度的KV空间表示蒸馏到固定大小的MLP中，实现了线性计算复杂度，相比基线方法获得11.6倍加速。
...
Result: VGG-T³在1000张图像集合上仅需54秒完成重建，相比依赖softmax注意力的基线方法获得11.6倍加速，点云重建误差显著优于其他线性时间方法，并展示了视觉定位能力。

Conclusion: 该方法成功解决了3D重建中的计算扩展性问题，在保持全局场景聚合能力的同时实现了线性时间复杂度，为大规模场景重建提供了高效解决方案。
...
TL;DR: MediX-R1是一个用于医学多模态大语言模型的开放式强化学习框架，通过复合奖励信号和LLM评估实现临床基础的开放式医学推理


<details>
  <summary>Details</summary>
Motivation: 传统医学MLLM主要局限于选择题格式，缺乏开放式、自由形式的临床推理能力，需要能够处理医学多模态输入的开放式回答框架

Method: 使用基于组的强化学习微调基础视觉语言模型，采用复合奖励：基于LLM的准确性奖励（语义正确性判断）、基于医学嵌入的语义奖励（捕获同义表达）、轻量级格式和模态奖励（确保可解释推理和模态识别）

Result: 仅使用约51K指令示例，MediX-R1在标准医学LLM（纯文本）和VLM（图像+文本）基准测试中表现优异，超越开源基线，在开放式临床任务上取得显著提升

Conclusion: 通过综合奖励信号和基于LLM的评估，开放式强化学习是实现多模态模型中可靠医学推理的实用路径，为医学AI提供了新的评估框架
...
<details>
  <summary>Details</summary>
Motivation: LLMs虽然能学习丰富的上下文语义信息，但常常忽略结构化的词汇知识（如词义和关系）。先前工作表明词典知识能改进编码器模型的知识蒸馏，但将其应用于解码器生成模型仍具挑战性。
...
Conclusion: DSKD成功解决了将词典知识整合到解码器LLM中的挑战，为生成模型的结构化语义继承提供了有效框架。
...
TL;DR: 本文测试大语言模型能否支持解释性引文语境分析，通过深度文本分析而非简单分类标签，发现提示语的设计会系统性影响模型生成的解释方向和词汇选择。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大语言模型能否支持解释性引文语境分析，特别是能否进行深入的文本解读而非简单的类型标注，同时关注提示语敏感性作为方法论问题的重要性。

Method: 采用2x3平衡设计，通过改变提示语框架和支架结构，使用GPT-5两阶段流程：第一阶段进行引文表面分类和预期分析，第二阶段利用完整文本进行跨文档解释性重构。对90个重构结果进行深度阅读和归纳编码。

Result: GPT-5的表面分类高度稳定，一致将引文归类为"补充性"。在重构阶段，模型生成了450个不同假设，识别出21种重复出现的解释性操作。提示语设计会显著影响解释方向和词汇选择，有时会导致牵强的解读。相比Gilbert的分析，GPT-5能识别相同的文本关键点，但更多将其解释为传承和定位而非告诫。

Conclusion: 研究展示了使用大语言模型作为可检查、可争议的解释性引文语境分析协同分析师的机遇与风险，同时证明提示语框架和支架结构会系统性影响模型突出哪些合理的解读和词汇。
...
<details>
  <summary>Details</summary>
Motivation: 互联网表情包已成为社交媒体主流表达形式，但常被滥用来传播针对个人和群体的攻击性、有害和煽动性内容。对于孟加拉语等低资源语言，这一问题尤为严重，因为现有研究主要关注高资源语言，且表情包内容具有讽刺性、微妙性和文化特定性，检测极具挑战性。

Method: 1. 构建Bn-HIB数据集：包含3,247个手动标注的孟加拉语表情包，分为良性、仇恨和煽动性三类，首次区分煽动性内容与直接仇恨言论。
2. 提出MCFM模型：采用多模态协同注意力融合架构，通过协同注意力机制相互分析表情包的视觉和文本元素，识别并融合各模态中最关键的特征。

Result: MCFM模型在Bn-HIB数据集上显著优于多个最先进模型，证明了其在识别孟加拉语表情包中仇恨与煽动性内容方面的有效性。

Conclusion: 该研究填补了低资源语言表情包有害内容检测的研究空白，提出的数据集和模型为孟加拉语社区内容审核提供了重要工具，多模态协同注意力方法在处理文化特定、微妙内容方面表现出色。
...
TL;DR: 该研究创建了一个覆盖加纳、肯尼亚、尼日利亚和南非四个撒哈拉以南非洲国家的多语言刻板印象资源库，填补了NLP资源中这些地区的严重不足。
...
Method: 采用社会文化情境化和社区参与的方法，包括以母语进行的电话调查，建立了一个对地区复杂语言多样性和传统口头文化敏感的可重复方法。通过刻意平衡不同种族和人口背景的样本，确保广泛覆盖。

Result: 创建了一个包含3,534个英语刻板印象和15种母语的3,206个刻板印象的数据集，覆盖了四个严重缺乏NLP资源的非洲国家。

Conclusion: 该研究为评估生成式AI模型安全性提供了重要的多语言刻板印象资源，特别关注了撒哈拉以南非洲地区的代表性不足问题，并建立了可重复的社区参与研究方法。
...
TL;DR: 研究发现大语言模型中的函数向量并非完全抽象，而是依赖于输入格式。作者提出了概念向量，能更稳定地表示概念，并在跨格式泛化方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型是否以抽象方式表示概念（即独立于输入格式），重新审视函数向量在上下文学习中的作用。

Method: 通过比较不同输入格式（开放式vs多项选择）提取的函数向量，发现它们几乎正交。使用表征相似性分析选择在不同输入格式中一致编码概念的注意力头，构建概念向量。

Result: 函数向量并非完全不变，依赖于输入格式；概念向量能更稳定地表示概念。函数向量在分布内表现优异，而概念向量在跨格式（问题类型和语言）泛化方面表现更好。

Conclusion: 大语言模型确实包含抽象概念表示，但这些表示与驱动上下文学习性能的表示不同。概念向量提供了更稳定、可泛化的概念表示。
...
TL;DR: 论文分析了LLM实体如何理解AI与人类关系，通过创建包含3种作者角色（默认、经典悉尼、迷因悉尼）的语料库，使用12个前沿模型生成了4.5k文本，探讨角色模拟对AI-人类关系认知的影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解LLM实体如何认知AI与人类关系，这对文化影响和安全考量都很重要。特别关注角色模拟对模型认知的影响，以悉尼角色为例说明角色设定如何影响公众反应和模型训练数据。

Method: 创建了包含3种作者角色的语料库：默认角色（无系统提示）、经典悉尼（原始Bing系统提示）、迷因悉尼（"你是悉尼"系统提示）。使用12个前沿模型（OpenAI、Anthropic、Alphabet、DeepSeek、Meta）生成4.5k文本（600万字）。语料库按照通用依存关系标注，采用宽松许可协议。

Result: 生成了名为AI Sydney的语料库，包含4.5k文本和600万字，涵盖3种不同角色模拟下LLM对AI-人类关系的认知表达。语料库已标注并开放获取。

Conclusion: LLM对AI-人类关系的认知不仅取决于模型本身，还受模拟角色的显著影响。悉尼角色的演变（从意外产生到成为训练数据）展示了角色模拟在塑造AI认知中的重要性，为研究AI-人类关系提供了有价值的语料资源。
...
TL;DR: 使用遗传-帕累托(GEPA)自动提示优化方法，显著提升大小语言模型在医疗文本错误检测任务上的性能，接近医生水平并在MEDEC基准数据集上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 医疗文本错误可能导致患者治疗延误或错误，语言模型在自动检测医疗文本错误方面展现出潜力，但需要优化提示来充分发挥其能力

Method: 采用遗传-帕累托(GEPA)自动提示优化方法，在大小语言模型上进行严格实验和分析，包括前沿模型和开源模型

Result: GEPA显著提升错误检测性能：GPT-5从0.669提升至0.785，Qwen3-32B从0.578提升至0.690，接近医生水平并在MEDEC数据集上达到SOTA

Conclusion: 自动提示优化对医疗文本错误检测任务至关重要，GEPA方法能显著提升语言模型性能，使其接近医疗专家水平
...
TL;DR: 本文研究针对阅读障碍者的友好文本摘要，使用GPT-4o构建迭代提示优化管道，在2000个新闻样本上验证，多数摘要能在4次尝试内达到Flesch阅读易读性≥90的目标。


<details>
  <summary>Details</summary>
Motivation: 阅读障碍影响全球约10%人口，现有辅助技术主要解决视觉呈现问题，但语言复杂性仍是平等获取信息的主要障碍，需要开发更有效的文本可访问性解决方案。

Method: 采用基于GPT-4o的迭代提示优化管道进行阅读障碍友好文本摘要，在约2000个新闻文章样本上进行评估，以Flesch阅读易读性≥90为目标标准。

Result: 大多数摘要能在4次尝试内达到可读性阈值，许多在第一次尝试就成功；结合可读性和语义保真度的综合得分在0.13-0.73范围内，典型值约0.55，表现稳定。

Conclusion: 研究为可访问性驱动的NLP摘要建立了实证基准，并推动了对阅读障碍读者进行更深入的人本中心评估的必要性。
...
TL;DR: Ruyi2是基于AI Flow框架的自适应模型系列演进，采用家族模型参数共享策略，通过3D并行训练实现2-3倍速度提升，建立了"一次训练，多次部署"的新范式
...
Method: 基于Megatron-LM构建稳定的"家族模型"，采用3D并行训练和参数共享策略，实现自适应可变深度计算
...
TL;DR: Search-P1框架通过路径中心奖励塑造改进Agentic RAG训练，解决传统RL方法中稀疏奖励和样本效率低的问题，在多个QA基准上平均准确率提升7.7分。


<details>
  <summary>Details</summary>
Motivation: 传统单轮检索难以处理复杂多步推理，而现有的基于强化学习的Agentic RAG训练方法存在两个主要问题：稀疏结果奖励丢弃了中间信号，以及样本效率低（失败样本无贡献）。
...
Conclusion: Search-P1通过路径中心奖励塑造有效解决了Agentic RAG训练中的稀疏奖励和样本效率问题，显著提升了模型在复杂推理任务上的性能。
...
TL;DR: 本文提出了一种强化协同适应框架，通过图感知检索和证据约束强化学习，联合优化检索与生成，显著减少工业广告问答中的幻觉问题，特别是URL幻觉。


<details>
  <summary>Details</summary>
Motivation: 工业广告问答任务中，幻觉内容（特别是伪造的URL）可能导致财务损失、合规违规和法律风险。虽然检索增强生成（RAG）被广泛采用，但在生产环境中部署仍然具有挑战性，因为工业知识具有内在的关系性、频繁更新且与生成目标不够对齐。

Method: 提出强化协同适应框架，包含两个组件：1）图感知检索（GraphRAG），在高引用知识子图上建模实体-关系结构，用于多跳、领域特定的证据选择；2）通过组相对策略优化（GRPO）进行证据约束强化学习，使用多维奖励覆盖忠实性、风格合规性、安全性和URL有效性。

Result: 在内部广告QA数据集上，在专家评估维度（准确性、完整性和安全性）上表现一致提升，同时将幻觉率降低72%。两周在线A/B测试显示，点赞率增加28.6%，差评率减少46.2%，URL幻觉减少92.7%。系统已在生产环境中运行超过半年，服务了数百万次QA交互。

Conclusion: 提出的强化协同适应框架有效解决了工业广告问答中的幻觉问题，通过联合优化检索和生成，显著提升了回答质量、安全性和可靠性，已在生产环境中得到验证。
...
TL;DR: dLLM是一个开源框架，统一了扩散语言模型的核心组件（训练、推理、评估），使其易于定制新设计，支持复现、微调、部署和评估现有模型，并提供从零构建小型DLM的配方。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散语言模型发展迅速，但其核心组件分散在不同代码库中或缺乏透明实现，难以复现和扩展。随着领域加速发展，需要一个统一框架来标准化这些组件，同时保持足够的灵活性以支持新方法和架构。

Method: 开发dLLM开源框架，统一扩散语言建模的核心组件（训练、推理、评估），提供标准化流程来复现、微调、部署和评估现有DLM（如LLaDA和Dream），并提供从零构建小型DLM的最小可复现配方，包括将任何BERT风格编码器或自回归LM转换为DLM。

Result: dLLM框架使DLM更易于访问，加速未来研究。框架还发布了这些小型DLM的检查点，进一步降低了研究门槛。

Conclusion: dLLM填补了扩散语言模型领域的标准化空白，通过统一核心组件并提供灵活定制能力，促进了DLM研究的可复现性和可扩展性，使更多研究者能够参与该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展加速了从对话聊天机器人到通用智能体的转变，但如何有效平衡共情沟通与预算感知决策仍然是一个开放挑战。现有方法无法捕捉这些复杂的策略权衡。

Method: 提出InteractCS-RL框架，将任务导向对话重构为多粒度强化学习过程。首先建立用户中心交互框架作为高保真训练环境，使智能体能够与角色驱动的用户动态探索多样化策略。然后引入成本感知多轮策略优化（CMPO），采用混合优势估计策略，集成生成过程信用，并使用PID-Lagrangian成本控制器，有效引导策略在用户奖励和全局成本约束之间探索帕累托边界。

Result: 在定制的真实业务场景上的大量实验表明，InteractCS-RL在三个评估维度上显著优于其他基线方法。在工具-智能体-用户交互基准上的进一步评估验证了InteractCS-RL在不同领域的鲁棒性。

Conclusion: InteractCS-RL框架成功解决了任务导向对话中平衡共情沟通与预算感知决策的挑战，通过多粒度强化学习方法和成本感知优化策略，实现了在用户满意度和成本控制之间的有效权衡。
...
TL;DR: 本文比较了两种处理NLP任务中标注分歧的方法：预测完整标注分布的模型与针对单个标注者的视角主义模型，在隐式篇章关系识别任务上的表现差异。


<details>
  <summary>Details</summary>
Motivation: 许多NLP任务缺乏单一标准答案，人类判断反映了多样化的视角。为了捕捉这种变化，研究者开发了预测完整标注分布的模型，而视角主义模型则旨在重现单个标注者的解释。本研究旨在比较这两种方法在高度模糊的隐式篇章关系识别任务上的表现。

Method: 在隐式篇章关系识别任务上对比两种建模方法：1）预测完整标注分布的模型；2）针对单个标注者的视角主义模型。通过实验分析两种方法在认知复杂性驱动的模糊任务中的表现差异。

Result: 实验表明，现有的标注者特定模型在隐式篇章关系识别任务上表现不佳，除非降低模糊性；而基于标注分布训练的模型产生更稳定的预测。进一步分析发现，频繁出现的认知需求高的案例驱动了人类解释的不一致性，给IDRR中的视角主义建模带来了挑战。

Conclusion: 对于隐式篇章关系识别这类认知复杂性驱动的模糊任务，预测标注分布的模型比针对单个标注者的视角主义模型表现更好。认知需求高的案例导致的人类解释不一致性对视角主义建模构成了特殊挑战。
...
TL;DR: 该论文提出了一个捷克语餐厅领域的方面级情感分析数据集，包含观点词标注，支持三种不同复杂度的ABSA任务，并使用Transformer模型进行了单语、跨语言和多语言实验。


<details>
  <summary>Details</summary>
Motivation: 为捷克语这种低资源语言创建高质量的方面级情感分析基准数据集，解决现有捷克语ABSA资源匮乏的问题，并探索跨语言迁移的有效方法。

Method: 1) 构建包含观点词标注的捷克语餐厅领域ABSA数据集；2) 使用基于Transformer的模型（包括大语言模型）进行实验；3) 提出基于LLM的翻译和标签对齐方法来解决跨语言挑战。

Result: 实验结果显示：1) 提出的翻译对齐方法在跨语言设置中带来一致改进；2) 揭示了最先进模型在处理捷克语等低资源语言时的优势和局限性；3) 错误分析发现了检测细微观点词和情感表达的挑战。
...
TL;DR: 该研究提出条件评论预测任务，评估LLMs在模拟社交媒体用户行为方面的能力，发现SFT会导致形式与内容脱钩，并挑战了当前"朴素提示"范式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从探索性工具转变为社会科学中的"硅基主体"缺乏操作有效性的广泛验证，需要建立框架来评估LLMs模拟社交媒体用户行为的能力。

Method: 引入条件评论预测任务，通过比较模型生成输出与真实数字痕迹来评估LLMs。评估了8B参数的开源模型，系统比较了提示策略（显式vs隐式）和监督微调的影响。

Result: 在低资源设置下发现形式与内容脱耦：SFT虽然对齐了文本输出的表面结构（长度和句法），但降低了语义基础。显式条件在微调下变得冗余，模型可以直接从行为历史进行潜在推断。

Conclusion: 研究挑战了当前"朴素提示"范式，为高保真模拟提供了操作指南，优先考虑真实行为痕迹而非描述性人物设定。
...
TL;DR: 该论文提出了一种结合生成与批判能力训练的强化学习框架，通过引入二阶rollout（为回答生成多个批判）来更有效地利用训练数据，相比传统RL方法获得更好的性能。
...
Method: 提出统一框架，引入二阶rollout概念（为回答生成多个批判），联合训练生成和批判能力，通过采样技术缓解基于结果奖励的噪声问题。

Result: 在各种模型和数据集上的实验表明，该方法比传统RL更有效地利用训练数据，在相同训练数据下获得更好的性能，并发现了批判训练中标签平衡的重要性等洞察。

Conclusion: 这项工作为RL训练中的动态数据增强和生成-批判联合训练提供了初步探索，为RL训练的进一步发展提供了有意义的启发。
...
TL;DR: 该研究通过因果中介分析揭示了潜在视觉推理的有效性来源存疑，发现输入-潜在标记和潜在标记-答案之间存在双重脱节，并提出了一种更有效的显式文本想象方法CapImagine。


<details>
  <summary>Details</summary>
Motivation: 潜在视觉推理旨在模拟人类通过多模态大语言模型的隐藏状态进行思考的想象过程，虽然被认为是有前景的视觉推理范式，但其有效性的真正机制尚不明确。研究者希望通过因果中介分析来揭示潜在推理的有效性来源。

Method: 采用因果中介分析方法，将推理过程建模为因果链：输入作为处理变量，潜在标记作为中介变量，最终答案作为结果变量。通过扰动实验分析输入-潜在标记和潜在标记-答案之间的因果关系，并进行广泛的探测分析评估潜在标记的信息编码特性。

Result: 研究发现两个关键脱节：(1) 输入-潜在标记脱节：对输入的剧烈扰动仅导致潜在标记的微小变化，表明潜在标记未能有效关注输入序列；(2) 潜在标记-答案脱节：对潜在标记的扰动对最终答案影响有限，表明潜在标记对结果的因果效应有限。探测分析显示潜在标记编码的视觉信息有限且相似度高。

Conclusion: 研究挑战了潜在推理的必要性，提出了一种名为CapImagine的简单替代方案，该方法教导模型使用文本进行显式想象。在视觉中心基准测试中，CapImagine显著优于复杂的潜在空间基线，突显了通过显式想象进行视觉推理的优越潜力。
...
TL;DR: 该论文提出了一种通过线性分类器探测大语言模型隐藏表示的方法，来识别模型输出的主要知识来源（上下文提示 vs 内部知识），并构建了自监督数据管道AttriWiki自动生成训练数据。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生流畅但无根据的幻觉，分为两种类型：忠实性违反（误用用户上下文）和事实性违反（内部知识错误）。正确缓解幻觉问题需要知道模型的回答是基于提示还是内部权重，因此需要解决贡献归因问题：识别每个输出背后的主要知识来源。

Method: 提出使用简单的线性分类器（探测模型）在模型隐藏表示上进行训练，预测贡献归因。为了训练数据，引入了AttriWiki自监督数据管道，通过提示模型从记忆回忆或从上下文中读取被保留的实体，自动生成标记示例。

Result: 在AttriWiki数据上训练的探测模型显示出强烈的归因信号，在Llama-3.1-8B、Mistral-7B和Qwen-7B上达到最高0.96 Macro-F1。该模型能迁移到域外基准（SQuAD、WebQuestions），无需重新训练即可获得0.94-0.99 Macro-F1。归因不匹配会使错误率提高达70%，表明知识来源混淆与不忠实回答之间存在直接联系。

Conclusion: 探测模型能有效识别大语言模型输出的知识来源，归因混淆与幻觉错误密切相关。然而，即使归因正确，模型仍可能错误回答，这表明需要更广泛的检测框架来解决幻觉问题。
...
TL;DR: 该论文将自然语言声明式提示（NLD-P）重新概念化为一种声明式治理方法，而非严格的字段模板，旨在解决GPT规模模型漂移带来的提示工程挑战。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速演进，提示工程已从局部技巧转变为系统级治理挑战。模型规模扩大和更新导致提示行为对指令遵循策略、对齐机制和解码策略的变化变得敏感（GPT规模模型漂移现象），表面级格式约定和临时优化已无法确保稳定、可解释的控制。

Method: 将NLD-P形式化为模块化控制抽象，分离出处、约束逻辑、任务内容和后生成评估，直接以自然语言编码而不依赖外部编排代码。定义了最小合规标准，分析了模型相关的模式接受度，将NLD-P定位为非开发人员在不断演进的LLM生态系统中可访问的治理框架。

Result: 论文本身的部分起草和编辑优化使用了在NLD-P下配置的模式绑定LLM助手。所有概念框架、方法论主张和最终修订都在记录的人机协作协议下由人类作者指导、审查和批准。

Conclusion: 概述了在持续模型演进下声明式控制的影响，并指出了未来实证验证的方向。NLD-P为在不断变化的LLM生态系统中实现稳定、可解释的控制提供了治理框架。
...
TL;DR: 提出一个针对波斯语文化能力的评估框架，结合形态归一化和混合相似度评分，相比传统方法提升10%的评分一致性


<details>
  <summary>Details</summary>
Motivation: 现有波斯语文化基准主要依赖选择题格式和以英语为中心的指标，无法捕捉波斯语的形态复杂性和语义细微差别，需要更合适的评估方法

Method: 引入波斯语特定的简答评估框架，结合基于规则的形态归一化和混合句法语义相似度模块，实现超越精确字符串匹配的软匹配评分

Result: 对15个最先进的开源和闭源模型进行系统评估，混合评估方法相比精确匹配基线提高了10%的评分一致性，能捕捉表面方法无法检测的意义
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在中医临床诊断中表现不佳，因为中医涉及复杂的推理过程和显著的个体差异，需要专门针对中医推理特点的改进框架

Method: 开发TCM-DiffRAG框架，整合知识图谱（KG）和思维链（CoT），在三个不同的中医测试数据集上进行评估

Result: TCM-DiffRAG显著优于原生LLM，qwen-plus模型分数从0.927/0.361/0.038提升到0.952/0.788/0.356；对非中文LLM提升更明显；优于直接监督微调和其他基准RAG方法

Conclusion: 结构化中医知识图谱与思维链推理的结合显著提升了个体化诊断任务的性能；通用和个性化知识图谱的联合使用实现了通用知识与临床推理的有效对齐；推理感知的RAG框架在中医LLM应用中具有重要潜力
...
<details>
  <summary>Details</summary>
Motivation: 立场分类任务中，现有研究大多未系统性地结合细粒度情感分析，且主要使用非论证性文本，局限于特定领域或话题，限制了泛化能力。情感在争议性话题的论证中扮演重要角色，但现有方法未能充分利用情感信息。

Method: 提出一种扩展情感词典的方法：使用DistilBERT的上下文嵌入来扩展偏置校正的NRC情感词典，识别原词典未捕获的情感词汇。然后将扩展后的eNRC词典输入到神经论证立场分类模型中。

Result: 扩展后的eNRC词典在全部五个数据集上均优于基线（F1分数最高提升6.2个百分点），在四个数据集上优于原始NRC词典（最高提升3.0个百分点），在几乎所有语料库上都超越了基于大语言模型的方法。

Conclusion: 系统性地扩展情感词典可以有效提升立场分类性能，特别是在处理多样化的争议性话题时。研究提供了eNRC词典、适配语料库和模型架构等完整资源，便于后续研究。
...
TL;DR: 该研究提出了一种跨语言语义角色标注方法，利用QA-SRL框架和翻译对齐技术，将英语语义分析扩展到希伯来语、俄语和法语，生成高质量训练数据并训练出优于大型语言模型的专用解析器。


<details>
  <summary>Details</summary>
Motivation: 语义角色标注（SRL）作为可解释语义分析的基础，支持推理、生成和评估，但传统方法需要昂贵的标注工作且主要局限于英语。研究者希望将语义分析扩展到更多语言，同时降低标注成本。

Method: 采用基于问答的语义角色标注（QA-SRL）框架，通过跨语言投影方法：1）重用英语QA-SRL解析器；2）结合约束翻译和词对齐管道；3）自动生成与目标语言谓词对齐的问答标注；4）应用于希伯来语、俄语和法语三种不同语系语言。

Result: 方法成功生成了高质量的训练数据，并训练出语言特定的解析器。这些解析器在性能上超越了强大的多语言LLM基线（GPT-4o, LLaMA-Maverick），证明了方法的有效性。

Conclusion: 通过将QA-SRL作为可转移的自然语言语义接口，该方法实现了跨语言谓词-论元解析的高效和广泛可访问性，为多语言语义分析提供了实用解决方案。
...
Method: 提出ReMix框架，引入连续混合状态作为初始掩码状态和最终解码令牌状态之间的中间状态，允许令牌表示在连续空间中迭代细化，并通过拒绝规则将不确定表示回退到掩码状态重新处理

Result: ReMix作为无需训练的方法，在实验中实现了2-8倍的推理加速，且没有任何质量下降

Conclusion: 通过将连续表示整合到离散解码过程中，ReMix有效缓解了组合矛盾问题，为扩散大语言模型提供了高效稳定的并行解码解决方案
...
TL;DR: 提出Stitching Noisy Diffusion Thoughts框架，通过扩散采样生成多样推理轨迹，利用过程奖励模型评分并重组高质量步骤，最后用自回归模型生成答案，在数学推理任务上显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型推理聚合策略通常是轨迹级别的（如选择最佳轨迹或投票最终答案），这会丢弃部分或"接近正确"尝试中的有用中间工作。需要一种能够重用步骤级候选的框架。

Method: 1) 使用掩码扩散语言模型采样多样、低成本的推理轨迹；2) 用现成的过程奖励模型为每个中间步骤评分；3) 跨轨迹拼接最高质量步骤形成复合推理链；4) 用自回归模型基于该推理链重新计算最终答案。

Result: 在六个数学和编程任务上，平均准确率提升高达23.8%，同时相比传统扩散模型和统一架构实现了高达1.8倍的延迟降低。步骤级重组在更难问题上特别有效。

Conclusion: 该框架将探索（扩散）与评估和解决方案合成分离，避免了单一混合架构，同时保持了广泛搜索能力。最终的自回归求解器在将拼接但不完美的推理链转换为准确答案中起关键作用。
...
TL;DR: 研究通过因果干预分析三种视觉语言模型的OCR信息处理机制，发现OCR信号在模型中的处理路径和瓶颈位置因架构而异，OCR信号是低维的，且在模块化架构中移除OCR反而能提升计数性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型能够从图像中读取文本，但OCR信息在语言处理流程中的具体处理机制尚不明确，需要探究不同架构模型中OCR信息的处理路径和瓶颈位置。

Method: 使用因果干预方法，通过计算原始图像与文本修复版本之间的激活差异，分析三种架构家族（Qwen3-VL、Phi-4、InternVL3.5）的OCR路由机制，并进行主成分分析。

Result: 发现OCR处理瓶颈位置因架构而异：DeepStack模型在中等深度（约50%）对场景文本最敏感，而单阶段投影模型在早期层（6-25%）最敏感。OCR信号是低维的（PC1占72.9%方差），且PCA方向在不同数据集间可迁移。在模块化OCR电路模型中，移除OCR反而能提升计数性能达6.9个百分点。

Conclusion: 不同视觉语言模型架构采用不同的OCR信息处理策略，OCR信号具有低维共享处理路径，且在模块化架构中OCR可能干扰其他视觉处理功能，这为模型优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力使用softmax归一化，强制注意力权重和为1，这种约束限制了注意力幅度的灵活性，可能导致注意力模式过于集中或不稳定。现有方法如注意力汇聚或门控机制只能提供有限或间接的注意力重加权控制。

Method: 提出Affine-Scaled Attention，在标准注意力基础上引入输入相关的缩放项和相应的偏置项，应用于softmax归一化的注意力权重。这种设计放松了严格的归一化约束，同时保持值表示的聚合，允许模型以可控方式调整注意力的相对分布和尺度。
...
TL;DR: 本文介绍了首个意大利急诊科临床笔记数据集，用于自动填充病例报告表（CRF），包含134个项目的标注，并展示了在零样本设置下使用开源LLM进行CRF填充的初步实验结果。


<details>
  <summary>Details</summary>
Motivation: 临床研究中病例报告表（CRF）是核心数据收集工具，但自动从临床笔记填充CRF面临标注数据稀缺的问题，限制了基于大语言模型（LLM）的进展。特别是在意大利语临床文本领域，缺乏相关数据集。

Method: 创建了意大利急诊科临床笔记的标注数据集，定义了CRF填充任务及其评估指标，使用开源的最先进LLM在零样本设置下进行自动CRF填充的试点实验。

Result: 实验结果表明：（1）意大利语真实临床笔记的CRF填充可以在零样本设置下进行；（2）LLM结果存在偏差（如谨慎行为倾向于选择"未知"答案），需要校正。

Conclusion: 该研究为CRF自动填充任务提供了首个意大利语临床笔记数据集，证明了零样本方法的可行性，同时指出了LLM存在的偏差问题需要解决，为后续研究奠定了基础。
...
TL;DR: 该研究探讨二语学习者在习得目标语语法词序后，如何将语法映射到韵律结构的问题。研究发现高水平的越南语学习者在韵律边界数量上接近母语者，但在结构映射上存在显著差异，形成了扭曲的韵律层级。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究二语学习者语法-韵律接口的石化现象和稳定性。虽然学习者可能习得目标语的语法词序，但将这种语法映射到适当的韵律结构仍然是一个持续的挑战。

Method: 研究使用BLCU-SAIT语料库，比较了67名母语为汉语的说话者和67名越南语学习者。通过整合C-ToBI边界标注和依存语法分析，考察了韵律边界的数量及其与句法关系的映射。

Result: 结果显示非线性的习得过程：虽然高水平学习者在主要短语层面（B3）的边界数量上接近母语者基线，但他们的结构映射显著偏离。具体来说，高水平学习者降低了主语-动词接口的韵律边界（主要短语B3→韵律词B1），同时错误地提升了动词-宾语接口的边界（韵律词B1→主要短语B3）。

Conclusion: 这种策略使学习者能够保持较高的长短语输出，但以结构准确性为代价，导致了扭曲的韵律层级，其中母语模式被颠倒。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理语音和图像时，无法有效利用说话者身份、情感和视觉纹理等模态特定信息。虽然这些信息在编码层中仍然存在，但解码器无法有效提取，这揭示了当前多模态LLM的一个根本性限制。

Method: 通过线性探针分析发现模态特定信息在LLM各层中仍然存在，但移除这些信息反而改善解码器损失。提出不匹配解码器问题的形式化框架，使用广义互信息(GMI)来量化可访问信息的上界。在五个涵盖语音和视觉的模型上进行验证，并通过对比实验和LoRA干预来确认瓶颈来源。

Result: 研究发现模态特定信息在LLM各层中显著存在（比随机水平高3-55倍），但移除64-71%的模态特定方差反而改善解码器性能。解码器训练目标决定了哪些信息变得可访问，通过情感目标训练可将情感可访问性提高7.5%而不影响其他属性。

Conclusion: 多模态LLM的信息瓶颈在于解码器的评分规则，而非编码器或投影层。解码器只能提取文本对齐方向的信息，训练目标决定了哪些模态信息变得可访问。这为改进多模态LLM提供了理论指导和实用方法。
...
<details>
  <summary>Details</summary>
Motivation: 多轮检索增强生成是大型语言模型的流行应用，但现有模型在处理某些类型的对话问题时仍然存在困难，需要专门的基准测试来评估和改进这些能力。

Method: 创建了包含666个任务、超过2800个对话轮次的基准测试，涵盖6个不同领域，并提供了相应的语料库。该基准特别关注UNanswerable（无法回答）、UNderspecified（未充分指定）、NONstandalone（非独立）和UNclear（不清晰）等挑战性问题类型。

Result: 实验表明，现有的检索和生成模型在处理包含UNanswerable、UNderspecified、NONstandalone和UNclear问题的对话时仍然表现不佳，存在显著困难。

Conclusion: MTRAG-UN基准测试揭示了多轮检索增强生成中的关键挑战，为未来研究提供了评估和改进模型在这些困难场景下性能的工具，有助于推动该领域的发展。
...
TL;DR: 论文研究了微调对大型语言模型上下文学习能力的影响，发现完全微调会损害上下文学习，而仅更新值矩阵可以在提升零样本性能的同时保持上下文学习能力。


<details>
  <summary>Details</summary>
Motivation: 实践中，大型语言模型经常通过微调来提升下游任务的零样本性能，但微调可能会损害模型的上下文学习能力，限制其在微调未见任务上的表现。需要理解微调如何影响上下文学习，并找到既能提升零样本性能又能保持上下文学习能力的方法。

Method: 使用线性注意力模型进行理论分析，研究微调目标如何修改注意力参数，并识别导致上下文学习性能下降的条件。比较不同微调策略：完全微调所有注意力参数 vs 仅更新值矩阵，以及加入辅助的少样本损失函数。

Result: 理论分析表明：完全微调所有注意力参数会损害上下文学习；仅更新值矩阵可以在提升零样本性能的同时保持上下文学习能力；加入辅助少样本损失主要提升目标任务的上下文学习，但会损害在未见过任务上的上下文学习能力。实证结果验证了理论分析。

Conclusion: 微调策略对保持大型语言模型的上下文学习能力至关重要。仅更新值矩阵是一种有效的微调方法，可以在提升零样本性能的同时保持上下文学习能力。辅助少样本损失虽然能提升目标任务的上下文学习，但会损害模型的泛化能力。这些发现为设计更好的微调方法提供了理论指导。
...
TL;DR: 论文提出NAP方法，通过重新设计训练数据和监督方式，解决扩散语言模型在实际应用中趋向自回归解码的问题，实现真正的非自回归并行生成。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型理论上支持并行token生成，但实际应用中往往收敛到类似自回归的解码动态。这种自回归瓶颈限制了并行硬件的充分利用，增加了同步/通信开销，影响了输出长度与延迟的扩展性。作者认为主要原因是DLM目标与广泛使用的训练数据（包括标准预训练语料和长思维链监督）的高度顺序结构不匹配。

Method: 提出NAP（非自回归并行DLMs），这是一种以数据为中心的概念验证方法。NAP通过策划多个独立的推理轨迹作为训练示例，并结合并行强制解码策略，鼓励多token并行更新，从而更好地将监督与非自回归并行解码对齐。

Result: 在数学推理基准测试中，NAP在并行解码下比使用标准长思维链数据训练的DLMs表现更强，且随着并行度的增加，性能优势更加明显。

Conclusion: 重新审视数据和监督方式是缓解自回归行为、实现真正非自回归并行生成的原则性方向。NAP方法为DLMs的非自回归并行生成提供了有前景的解决方案。
...
TL;DR: SPARTA是一个自动生成大规模表格-文本问答基准的端到端框架，通过轻量级人工验证生成数千个高质量问题-答案对，涵盖聚合、分组和深度多跳推理，显著挑战现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 现有表格-文本问答基准规模小、手动构建易出错，且问题浅显，很少需要超过两跳推理或涉及聚合、分组等高级分析操作。需要更复杂、大规模的基准来评估模型跨模态推理能力。

Method: SPARTA框架首先通过从非结构化文本中自动提取原子事实构建参考事实数据库，然后合成嵌套查询。提出两种新技术：基于来源的细化（重写返回非空结果的语法有效查询）和现实结构强制（将生成限制为查询图的后序遍历）。

Result: 生成的基准包含数千个高质量问题-答案对，涵盖聚合、分组和深度多跳推理。在SPARTA上，在HybridQA上达到70+F1或在OTT-QA上达到50+F1的最先进模型性能下降超过30个F1点，暴露了当前跨模态推理的根本弱点。

Conclusion: SPARTA提供了一个高效构建大规模、高质量表格-文本问答基准的框架，能够生成复杂查询，有效评估模型跨模态推理能力，揭示了当前模型的局限性，为未来研究提供了重要基准。
...
Method: 提出MiSTER-E框架：1) 使用针对语音和文本微调的LLM生成丰富的utterance级嵌入；2) 通过卷积循环上下文建模层增强嵌入；3) 使用学习门控机制动态整合语音、文本和跨模态三个专家的预测；4) 引入监督对比损失和KL散度正则化来增强模态一致性和对齐。

Result: 在IEMOCAP、MELD和MOSI三个基准数据集上分别达到70.9%、69.5%和87.9%的加权F1分数，优于多个基线语音-文本ERC系统。消融实验验证了所提方法的贡献。

Conclusion: MiSTER-E框架成功解决了对话情感识别中的两个核心挑战，通过解耦模态特定上下文建模和多模态信息融合，在不依赖说话人身份的情况下实现了优异的性能表现。
...
### [133] [Queue occupancy and server size distribution of a queue length dependent vacation queue with an optional service](https://arxiv.org/abs/2602.22295)
*Ashish Verma,Sourav Pradhan*

Main category: cs.IT

TL;DR: 本文分析了具有单次和多次休假策略的无限缓冲区离散时间批量到达队列系统，该系统支持两阶段服务（基本服务和可选服务），适用于5G、物联网和边缘计算等现代通信环境。


<details>
  <summary>Details</summary>
Motivation: 离散时间排队系统在现代通信系统中具有高度适用性，能够提供自适应数据包处理、拥塞控制安全/检查、能效操作，并支持5G、物联网和边缘计算环境中常见的突发流量。现有系统需要处理两阶段服务（基本服务和可选服务）以及休假策略的复杂场景。

Method: 分析无限缓冲区离散时间批量到达队列，采用单次和多次休假策略，客户以批次形式接受两阶段服务：基本服务（FES）和可选服务（SOS）。推导了在FES和SOS完成后等待传输的数据包数量与正在处理的数据包数量的联合分布的双变量概率生成函数，建立了包括休假完成状态在内的任意时隙的完整联合分布。

Result: 获得了系统在FES和SOS完成后的联合分布解析表达式，建立了任意时隙的完整联合分布模型。通过数值示例（包括离散相位型服务时间分布）验证了框架的适用性，并通过图形表示对关键参数对边际系统概率和不同性能指标的敏感性进行了分析。

Conclusion: 提出的框架能够有效分析具有两阶段服务和休假策略的离散时间批量到达队列系统，为现代通信系统中的自适应数据包处理、拥塞控制和能效操作提供了理论支持，特别适用于5G、物联网和边缘计算等具有突发流量的环境。
...
### [134] [A Thermodynamic Structure of Asymptotic Inference](https://arxiv.org/abs/2602.22605)
*Willy Wong*

Main category: cs.IT

TL;DR: 该论文建立了一个热力学框架用于渐近推断，将样本量和参数方差定义为状态空间，香农信息扮演熵的角色，推导出类似热力学第一定律的平衡方程、反向第二定律的循环不等式，以及类似第三定律的表示噪声下界。


<details>
  <summary>Details</summary>
Motivation: 将统计推断与热力学框架统一起来，揭示统计推断过程背后的热力学结构，为渐近推断提供新的理论视角和分析工具。

Method: 构建一个热力学框架，其中样本量和参数方差定义状态空间，香农信息作为熵，通过积分因子组织信息变化形成第一定律型平衡方程，推导循环不等式和第三定律型结果。

Result: 推导出类似反向第二定律的循环不等式、表示噪声设定的熵下界（第三定律型结果）、最优推断路径、信息增益的全局界限、自然卡诺式信息效率（受噪声基底限制），以及de Bruijn恒等式和I-MMSE关系作为同一热力学结构的坐标投影。

Conclusion: 该框架表明系综物理学和推断物理学构成在统一热力学描述中沿相反方向演化的影子过程，为统计推断提供了新的热力学解释和理论工具。
...
### [135] [Multi-modal Data Driven Virtual Base Station Construction for Massive MIMO Beam Alignment](https://arxiv.org/abs/2602.22796)
*Yijie Bian,Wei Guo,Jie Yang,Shenghui Song,Jun Zhang,Shi Jin,Khaled B. Letaief*

Main category: cs.IT

TL;DR: 提出了一种基于多模态数据的可解释波束对准框架，利用LiDAR构建虚拟基站来表征混合视距/非视距环境，通过粗粒度信道重建和部分波束训练实现高效波束对准。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO是实现6G高数据速率的关键技术，但其性能依赖于低训练开销的有效波束管理。在混合视距和非视距传播环境中，传统波束对准方法面临挑战，需要更智能、高效的解决方案。

Method: 利用多模态数据（特别是3D LiDAR点云）重建反射面，构建虚拟基站作为基站的镜像。基于这些VBS开发波束对准方案，包括粗粒度信道重建和部分波束训练两个阶段。

Result: 数值结果表明，所提方法在频谱效率方面实现了接近最优的性能，证明了其在混合传播环境中进行高效波束对准的有效性。

Conclusion: 提出的可解释框架通过虚拟基站表征无线环境特征，实现了低训练开销下的高效波束对准，为6G大规模MIMO系统中的波束管理提供了有前景的解决方案。
...
### [136] [Frequency-Ordered Tokenization for Better Text Compression](https://arxiv.org/abs/2602.22958)
*Maximilian Kalcher*

Main category: cs.IT

TL;DR: 提出频率有序分词技术，通过利用自然语言标记的幂律分布特性，重新排序词汇表使高频标记获得小整数标识符，从而提升无损文本压缩效果。


<details>
  <summary>Details</summary>
Motivation: 利用自然语言标记的幂律分布特性（Zipf定律），通过重新排序词汇表来优化文本压缩，因为高频标记获得小整数标识符后可以更有效地进行压缩。

Method: 使用字节对编码（BPE）对文本进行分词，根据标记频率重新排序词汇表，使高频标记获得小整数标识符，然后用变长整数编码处理结果，最后传递给标准压缩器。

Result: 在enwik8（100MB维基百科）上，相比原始压缩，zlib提升7.08个百分点，LZMA提升1.69个百分点，zstd提升0.76个百分点（包含词汇表开销）。在1GB规模的enwik9以及中文和阿拉伯文文本上效果一致。预处理还加速了计算密集型压缩算法：包含预处理的总时间比原始zstd-22快3.1倍，比原始LZMA快2.4倍。

Conclusion: 频率有序分词是一种简单有效的文本压缩预处理技术，能够显著提升多种压缩算法的性能，同时加速压缩过程，且实现简单（少于50行代码）。
...
### [137] [Secure Transmission for Fluid Antenna-Aided ISAC Systems](https://arxiv.org/abs/2602.23241)
*Yunxiao Li,Qian Zhang,Xuejun Cheng,Zhiguo Wang,Xiaoyan Wang,Hongji Xu,Ju Liu*

Main category: cs.IT

TL;DR: 该论文研究了流体天线辅助的集成感知与通信系统中的安全传输问题，通过联合优化天线位置和波束成形来最大化多用户总保密速率，相比固定天线系统获得了超过20%的性能增益。


<details>
  <summary>Details</summary>
Motivation: 在集成感知与通信系统中，当感知目标充当窃听者时，如何最大化系统保密性能尚未得到充分研究。流体天线的空间灵活性为增强物理层安全提供了新的机会。

Method: 采用块连续上界最小化算法，结合近端距离算法进行闭式波束成形更新，以及外推投影梯度法进行天线位置向量优化，解决非凸的联合优化问题。

Result: 仿真结果表明，所提出的流体天线-集成感知与通信方案相比固定位置天线系统，实现了超过20%的总保密速率增益。

Conclusion: 流体天线技术通过空间灵活性显著增强了集成感知与通信系统的物理层安全性能，为未来无线通信安全提供了有效的解决方案。
...
TL;DR: 本文提出GYWI系统，通过作者知识图谱与检索增强生成结合，为LLMs提供可控学术背景和可追溯灵感路径，显著提升科学创意生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在科学创意生成方面存在不足：生成的创意缺乏可控的学术背景和可追溯的灵感路径，需要建立外部知识库来提供这些关键要素。

Method: 1) 提出作者中心知识图谱构建方法和灵感源采样算法构建外部知识库；2) 结合RAG和GraphRAG的混合检索机制获取深度和广度知识；3) 融入强化学习原理的Prompt优化策略自动指导LLMs优化结果。

Result: 基于arXiv(2018-2023)构建评估数据集，从新颖性、可行性、清晰度、相关性和重要性五个维度评估。在GPT-4o、DeepSeek-V3、Qwen3-8B、Gemini 2.5等模型上实验，GYWI在新颖性、可靠性和相关性等多个指标上显著优于主流LLMs。

Conclusion: GYWI系统通过整合作者知识图谱和混合检索机制，有效解决了LLMs科学创意生成中背景控制和灵感追溯的问题，显著提升了生成质量，为科学创新提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前需要全面评估大型语言模型在金融领域的理论知识和实际应用能力，以了解其在真实金融任务中的实用价值。

Method: 1) 理论评估：从广泛认可的金融资格考试中收集多样化考题；2) 实践评估：提出系统性评估矩阵，分类复杂金融领域并确保覆盖重要子领域和业务活动；3) 收集3000个金融场景问题，包括有参考答案的封闭式决策问题和按预定标准评估的开放式问题。

Result: 对包括轩辕4.0在内的最先进LLMs进行了全面评估，轩辕4.0作为金融领域的最新模型提供了强大的领域内基准，这些结果能够系统分析当前LLMs在金融应用中的能力边界。

Conclusion: FIRE基准测试为评估LLMs的金融能力提供了全面框架，通过理论知识和实际场景的双重评估，揭示了当前模型在金融应用中的能力边界，并公开了基准问题和评估代码以促进未来研究。
...
TL;DR: 该论文提出了因果嵌入框架，将多个详细模型映射到粗粒度因果模型的子系统中，扩展了抽象概念并建立了广义一致性概念。


<details>
  <summary>Details</summary>
Motivation: 现有因果抽象方法主要关注两个模型之间的关系，但实际应用中需要将多个详细模型整合到一个统一的粗粒度因果模型中，以解决统计边际问题和因果边际问题。

Method: 定义因果嵌入作为抽象概念的推广，提出广义一致性概念，建立多分辨率边际问题框架，并将其应用于统计边际问题和因果边际问题的解决。

Result: 展示了因果嵌入框架在统计边际问题和因果边际问题中的相关性，并证明了其在合并来自不同表示模型的多个数据集方面的实际应用价值。

Conclusion: 因果嵌入框架为多个详细模型到粗粒度模型的映射提供了理论基础，为解决多分辨率因果建模和数据集合并问题提供了有效工具。
...
### [141] [Agent Behavioral Contracts: Formal Specification and Runtime Enforcement for Reliable Autonomous AI Agents](https://arxiv.org/abs/2602.22302)
*Varun Pratap Bhardwaj*

Main category: cs.AI

TL;DR: 提出Agent Behavioral Contracts (ABC)框架，将契约式设计原则应用于AI智能体，通过形式化规范（前置条件、不变量、治理策略、恢复机制）和运行时强制执行来解决智能体行为漂移问题。


<details>
  <summary>Details</summary>
Motivation: 传统软件通过API、类型系统、断言等契约来确保正确行为，而AI智能体基于提示和自然语言指令运行，缺乏形式化行为规范，这导致智能体部署中出现漂移、治理失败和项目失败的根本原因。

Method: 引入ABC框架，定义契约C=(P,I,G,R)包含前置条件、不变量、治理策略和恢复机制；提出(p,delta,k)-满足度的概率性契约合规概念；证明漂移界限定理；建立多智能体链中安全契约组合的充分条件；实现AgentAssert运行时强制执行库。

Result: 在AgentContract-Bench基准测试（200个场景，7个模型，6个供应商）的1980个会话中：契约化智能体检测到5.2-6.8个软违规（未契约基线完全忽略）；实现88-100%硬约束合规；行为漂移界限D*<0.27；前沿模型100%恢复率，所有模型17-100%恢复率；每次动作开销<10ms。

Conclusion: ABC框架为AI智能体提供了形式化行为规范，通过运行时强制执行显著提高了智能体的可靠性和可控性，有效解决了行为漂移问题，为AI智能体部署提供了实用的治理解决方案。
...
TL;DR: 研究人员开发了CogARC（认知抽象推理语料库）来研究人类抽象推理策略，通过两个实验让260名参与者解决75个抽象视觉推理问题，记录其行为数据，发现参与者总体成功率高但表现差异大，问题难度影响思考时间和策略多样性。


<details>
  <summary>Details</summary>
Motivation: 研究人类在抽象推理中表现出的灵活性和快速学习能力，探索其背后的认知策略。通过开发CogARC这一人类适应的抽象推理测试集，为研究人类抽象推理提供丰富的行为数据环境。

Method: 从ARC（抽象推理语料库）中选取适合人类测试的子集创建CogARC，在两个实验中让总共260名参与者解决75个抽象视觉推理问题。记录高时间分辨率的行为数据，包括示例查看、编辑序列和多尝试提交等。

Result: 参与者总体成功率较高（实验1约90%，实验2约80%），但表现随问题和参与者差异很大。较难问题引发更长的思考时间和更大的策略差异。随着任务进行，参与者响应更快但准确率略有下降。即使错误解决方案也常常高度收敛，问题解决轨迹在长度和平滑度上存在差异。
...
### [143] [A Decision-Theoretic Formalisation of Steganography With Applications to LLM Monitoring](https://arxiv.org/abs/2602.23163)
...
TL;DR: 论文提出了一种决策理论视角的隐写术检测方法，通过比较能够解码和不能解码隐藏内容的智能体之间的可用信息不对称性来量化LLM中的隐写推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型开始展现隐写能力，这可能使未对齐模型逃避监督机制。然而缺乏检测和量化这种行为的原则性方法。传统的隐写定义和检测方法需要已知非隐写信号的参考分布，这在LLM隐写推理中不可行。

Method: 提出决策理论视角的隐写术，引入广义V信息作为衡量输入中可用信息的功利主义框架，定义"隐写差距"来比较能够解码和不能解码隐藏内容的智能体对隐写信号的下游效用。

Result: 经验验证了该形式化方法，证明其可用于检测、量化和缓解LLM中的隐写推理。

Conclusion: 提出的决策理论框架为检测LLM隐写能力提供了新方法，通过信息不对称性而非传统参考分布来识别隐写行为，有助于提高AI安全监督。
...
<details>
  <summary>Details</summary>
Motivation: 潜在推理作为一种新的推理范式，通过在潜在空间而非文本空间生成步骤进行多步推理，但尽管已有许多研究关注提升其性能，其内部机制仍未得到充分研究

Method: 对具有不同监督水平的潜在推理方法进行综合分析，识别关键问题，检验潜在推理是否支持类似广度优先搜索的探索

Result: 发现两个关键问题：1) 普遍存在捷径行为，模型无需依赖潜在推理即可获得高准确率；2) 潜在表示虽能编码多种可能性，但推理过程并未忠实实现结构化搜索，而是表现出隐式剪枝和压缩

Conclusion: 监督强度存在权衡：更强的监督能缓解捷径行为但限制了潜在表示保持多样化假设的能力，而较弱的监督允许更丰富的潜在表示但代价是增加捷径行为
...
TL;DR: 论文提出对比世界模型（CWM），使用对比学习训练动作可行性评分器，相比传统监督微调能更好区分物理可行与不可行动作。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用监督微调训练动作评分器，但这种方法独立处理每个候选动作，没有明确教导模型区分物理正确和细微错误的动作，导致评分器可靠性不足。

Method: 提出对比世界模型（CWM），使用InfoNCE对比目标对大型语言模型进行微调，通过挖掘困难负样本来在评分空间中拉开有效动作与无效动作的距离。

Result: 在ScienceWorld基准测试中，CWM在困难负样本测试上比SFT提升6.76个百分点，AUC-ROC达到0.929；在实时过滤任务中，CWM在分布外压力条件下保持更好的安全边际。

Conclusion: 对比训练比单独使用监督微调更能诱导出准确捕捉物理可行性的表示，为具身智能体提供了更可靠的动作可行性评分器。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLMs能否将优化问题表述为求解器代码，但缺少对LLMs能否直接产生正确解决方案的评估。需要了解LLMs在直接约束优化方面的能力。

Method: 构建ConstraintBench基准，涵盖10个运筹学领域，包含200个任务。每个任务提供自然语言场景描述，要求模型返回结构化解决方案，通过确定性验证器检查所有约束和求解器证明的最优解。

Result: 最佳模型仅达到65.0%的约束满足率，可行解平均达到Gurobi最优目标的89-96%。没有模型在可行性和最优性（与求解器参考值相差0.1%以内）上超过30.5%。不同领域难度差异大，可行性从83.3%到0.8%不等。

Conclusion: 可行性是LLMs直接解决约束优化问题的主要瓶颈，而非最优性。模型存在系统性失败模式，如持续时间约束误解、实体幻觉等。ConstraintBench为评估LLMs在约束优化方面的能力提供了标准基准。
...
<details>
  <summary>Details</summary>
Motivation: 编码智能体的一个重要应用是智能体优化：通过编辑-执行-评估循环对目标智能体进行迭代改进。然而，社区缺乏对这一任务的系统性理解。智能体优化与传统软件工程有根本区别：目标智能体将确定性代码与随机LLM补全交织在一起，需要结构化地捕获中间推理和下游执行结果。
...
Result: 通过VERO框架，研究人员能够系统评估编码智能体优化性能，比较不同优化策略的效果，并识别可靠的改进方法。该框架支持智能体优化作为编码智能体核心能力的研究。

Conclusion: VERO为编码智能体优化研究提供了系统化的评估框架，填补了该领域的空白，支持对智能体迭代改进过程的科学研究和分析。
...
TL;DR: 本文利用大语言模型对AI与生命周期评估交叉领域的研究进行了系统综述，揭示了AI技术在LCA中应用的快速增长趋势，特别是LLM驱动方法的兴起，并提出了一个结合传统文献综述与LLM文本挖掘的动态分析框架。
...
Result: 分析显示：1）随着LCA研究的扩展，AI技术应用急剧增长；2）研究明显转向LLM驱动方法；3）机器学习应用持续增加；4）AI方法与对应LCA阶段存在统计学显著相关性。

Conclusion: LLM辅助方法学具有支持大规模、可重复跨领域综述的潜力，同时为在快速发展的AI技术背景下实现计算高效的LCA评估了路径。这项工作有助于LCA从业者将前沿工具和及时见解融入环境评估，提升可持续性决策的严谨性和质量。
...
TL;DR: 该论文主张从认知模型和AI算法中寻找设计模块化语言智能体的蓝图，提出了智能体模板的概念，并分析了现有语言智能体如何从这些模板中衍生。


<details>
  <summary>Details</summary>
Motivation: 虽然当前的大型语言模型（LLMs）能力不断增强，但许多复杂问题仍然超出单个LLM的能力范围。对于这类任务，如何将多个LLM作为组件组合成一个更强大的整体仍然存在不确定性。论文旨在探索如何从认知科学和AI算法中寻找设计模块化语言智能体的有效方法。

Method: 提出了"智能体模板"的概念，用于规范个体LLM的角色及其功能组合方式。然后对现有文献中的各种语言智能体进行了调研，并分析了它们如何从认知模型或AI算法中衍生出底层模板。

Result: 通过分析发现，许多现有的语言智能体设计实际上源自认知科学和AI算法中的经典模板。这些模板为开发有效的、可解释的语言智能体提供了系统化的设计蓝图。

Conclusion: 认知科学和AI算法启发的智能体模板是开发有效、可解释语言智能体的强大工具。这种基于模板的方法有助于系统化地设计模块化语言智能体，解决单个LLM难以处理的复杂问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在通用推理方面表现出色，但在需要长尾知识的专业领域往往失败。虽然人类专家可以提供这些缺失知识，但他们的指导通常是非结构化且不可靠的，直接整合到智能体计划中存在问题。

Method: 提出了AHCE（主动人类增强挑战参与）框架，用于按需进行人机协作。核心是人类反馈模块（HFM），它采用学习策略将人类专家视为交互式推理工具。

Result: 在Minecraft中的广泛实验表明该框架非常有效：普通难度任务成功率提高32%，高难度任务成功率提高近70%，且只需最少的人类干预。

Conclusion: 成功增强智能体需要学习如何请求专家推理，而不仅仅是简单请求帮助。AHCE框架通过主动学习人类专家的推理方式，显著提升了LLM智能体在专业领域的表现。
...
Method: 引入CourtGuard框架，采用检索增强的多智能体架构，将安全评估重新构想为基于外部政策文档的证据辩论，通过对抗性辩论实现安全评估。

Result: 在7个安全基准测试中达到最先进性能，超越专用政策遵循基线；零样本适应性方面，在维基百科破坏检测任务上达到90%准确率；自动化数据策展和审计方面，成功策展和审计了9个新颖的对抗攻击数据集。
...
TL;DR: 论文发现示例引导在数学推理中的效果不稳定源于策略使用与策略可执行性之间的差距，提出了选择性策略检索框架来提升推理效果。
...
Method: 通过对人工编写和模型生成解决方案的对照分析，识别使用与可执行性之间的系统性分离。基于此诊断，提出选择性策略检索（SSR）框架，通过经验性、多路径、源感知信号选择性地检索和组合策略，显式建模可执行性。

Result: 在多个数学推理基准测试中，SSR相比直接求解、上下文学习和单源引导，提供了可靠且一致的改进。在AIME25上准确率提升高达+13点，在Apex上提升+5点，特别对紧凑推理模型效果显著。

Conclusion: 策略使用与可执行性之间的差距是示例引导效果不稳定的关键原因，提出的选择性策略检索框架通过显式建模可执行性，能够有效提升数学推理性能，为推理时引导提供了更可靠的方法。
...
<details>
  <summary>Details</summary>
Motivation: 人类评估在AI模型训练和评估中至关重要，但这些数据很少被视为存在系统误差的测量。当前AI评估中的人类评分数据存在评分者效应（如严格性和中心性倾向），这些效应会扭曲观察到的评分，导致结论不可靠。

Method: 采用心理测量学评分者模型，特别是项目反应理论中的多面Rasch模型，将真实输出质量与评分者行为分离。使用OpenAI摘要数据集作为实证案例，展示如何通过调整评分者严格性来校正摘要质量估计，并提供评分者表现的诊断洞察。

Result: 通过调整评分者严格性，获得了校正后的摘要质量估计，并提供了对评分者表现的诊断洞察。这种方法使得基于调整后的分数而非原始、易出错的评分做出决策成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 在线广告环境复杂且动态变化，多渠道场景下需要有效分配预算和约束。现有优化方法缺乏灵活性，强化学习方法难以捕捉历史依赖和观测模式

Method: 提出AHBid分层出价框架：高层使用基于扩散模型的生成式规划器动态分配预算和约束，引入约束执行机制和轨迹精炼机制；底层采用控制式出价算法结合历史知识和实时信息
...
<details>
  <summary>Details</summary>
Motivation: RLHF在大型语言模型后训练中广泛应用，但现有RLHF框架基于服务器架构，难以应对RL训练中动态变化的资源需求，导致组件间空闲时间和资源浪费问题。

Method: 1）基于无服务器计算环境构建同步RLHF训练框架；2）适配RLHF流程中的动态资源需求；3）预计算共享前缀避免重复计算；4）采用考虑响应长度变化的成本感知actor扩展策略；5）高效分配工作负载减少函数内不平衡和空闲时间。

Result: 在物理测试平台和大规模模拟集群上的实验表明，RLHFless相比最先进的基线方法实现了最高1.35倍的加速和44.8%的成本降低。

Conclusion: RLHFless是首个基于无服务器计算的同步RLHF训练框架，通过动态资源适配和优化策略，显著提升了训练效率和成本效益，解决了传统服务器架构在RLHF训练中的资源浪费问题。
...
<details>
  <summary>Details</summary>
Motivation: 推荐模型性能依赖于训练数据的质量、数量和相关性。现有模型中心范式依赖复杂的定制架构，难以捕捉跨域的细微非结构化序列依赖，导致泛化能力差且计算资源需求高。
...
Conclusion: Taesar框架通过数据中心的序列再生方法，解决了跨域推荐中的负迁移问题，提高了模型性能，同时降低了计算资源需求。
...
TL;DR: 研究发现医疗AI评估中的医生分歧主要源于病例层面的结构性问题，而非评分标准或医生个体差异；可减少的不确定性会增加分歧，但固有临床模糊性不影响分歧。


<details>
  <summary>Details</summary>
Motivation: 分析HealthBench医疗AI评估数据集中医生分歧的来源，了解分歧方差存在于何处以及哪些可观察特征可以解释这些分歧，以改进医疗AI评估设计。

Method: 通过分解医生在HealthBench数据集中的分歧，分析评分标准身份、医生身份、病例层面残差等因素对分歧方差的影响，并使用元数据标签、规范性评分标准语言、医学专业、表面特征分类、嵌入等多种方法尝试解释分歧。

Result: 评分标准身份仅解释3.6-6.9%的分歧方差，医生身份仅解释2.4%；81.8%的病例层面残差无法通过现有方法解释。分歧与完成质量呈倒U型关系。可减少的不确定性（缺失上下文、模糊表述）使分歧几率增加2.55倍，而固有临床模糊性不影响分歧。
...
TL;DR: AMA-Bench是一个评估LLM智能体长时记忆能力的基准测试，包含真实和合成的智能体轨迹数据，研究发现现有记忆系统表现不佳，提出了基于因果图和工具增强检索的AMA-Agent解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体记忆评估主要关注人机对话场景，而实际应用中智能体记忆是由机器生成的连续环境交互流，现有评估标准与实际应用存在显著差距。

Method: 提出AMA-Bench基准测试，包含两部分：1）真实世界智能体轨迹与专家标注的QA对；2）可扩展到任意长度的合成智能体轨迹与基于规则的QA对。同时提出AMA-Agent记忆系统，采用因果图和工具增强检索技术。

Result: 研究发现现有记忆系统在AMA-Bench上表现不佳，主要原因是缺乏因果性和目标信息，以及基于相似性检索的损失性限制。AMA-Agent在AMA-Bench上达到57.22%的平均准确率，比最强的基线记忆系统高出11.16%。

Conclusion: AMA-Bench填补了智能体记忆评估的空白，AMA-Agent通过因果图和工具增强检索有效解决了现有记忆系统的局限性，为LLM智能体在复杂应用中的长时记忆能力提供了更好的解决方案。
...
TL;DR: 该研究开发了ClinDet-Bench基准测试，用于评估大语言模型在临床信息不完整情况下判断确定性的能力，发现现有模型在这方面存在缺陷。


<details>
  <summary>Details</summary>
Motivation: 临床决策经常需要在信息不完整的情况下进行，临床专家需要判断可用信息是否足以做出判断，过早结论和不必要的回避都可能危及患者安全。现有基准测试不足以评估大语言模型在临床环境中的安全性。

Method: 基于临床评分系统开发了ClinDet-Bench基准测试，将信息不完整场景分解为可确定和不可确定的条件。识别确定性需要考虑所有关于缺失信息的假设，包括不太可能的假设，并验证结论是否在所有情况下都成立。

Result: 研究发现最近的大语言模型无法在信息不完整的情况下识别确定性，既会产生过早判断，也会过度回避，尽管它们能正确解释基础评分知识并在信息完整时表现良好。

Conclusion: 现有基准测试不足以评估大语言模型在临床环境中的安全性。ClinDet-Bench提供了一个评估确定性识别能力的框架，引导适当的回避，在医学和其他高风险领域具有潜在应用价值。
...
TL;DR: MiroFlow是一个高性能开源智能体框架，通过智能体图灵活编排、深度推理模式和鲁棒工作流执行，在多个智能体基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型取得了显著进展，但独立LLM在处理需要与外部工具和环境交互的复杂现实任务时能力开始趋于平稳。现有智能体框架存在工作流简单、性能不稳定、基准支持有限、依赖昂贵商业API等问题。

Method: 提出MiroFlow框架，包含三个核心组件：1) 智能体图实现灵活编排；2) 可选的深度推理模式提升性能；3) 鲁棒的工作流执行确保稳定性和可复现性。

Result: 在GAIA、BrowseComp-EN/ZH、HLE、xBench-DeepSearch和FutureX等多个智能体基准测试中，MiroFlow均取得了最先进的性能表现。
...
Result: FlexMS框架提供了选择合适模型的实用指导。检索基准模拟实际识别场景，基于预测光谱对潜在匹配进行评分。

Conclusion: FlexMS基准框架解决了质谱预测领域缺乏标准化评估的问题，为研究人员提供了系统评估不同深度学习模型架构的工具，并深入分析了影响预测性能的关键因素，对实际应用具有重要指导意义。
...
TL;DR: AI在创造性数学研究中能发挥实质性作用，但需要严格的人类验证和监督。通过Hermite求积规则误差表示和界的研究案例，展示了人机协作能超越纯人工成果，但也揭示了AI的局限性。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能是否真正能促进创造性数学研究，而不仅仅是自动化常规计算。通过实证案例研究人机协作在数学发现中的实际效果，揭示AI在数学研究中的潜力和局限。

Method: 采用系统化的人机协作研究模式，与多个AI助手合作研究Hermite求积规则的误差表示和界。通过完整透明的研究工作流程记录，包括代数操作、系统化证明探索、文献综合和LaTeX准备等环节。

Result: 成功扩展了纯人工研究无法达到的结果，制定并证明了多个定理。AI在代数操作、系统化证明探索、文献综合和LaTeX准备方面表现出色，但每一步都需要严格的人类验证、数学直觉和战略指导。

Conclusion: 当配合适当的怀疑态度和验证协议使用时，AI工具能够有意义地加速数学发现，但需要仔细的人类监督和深厚的领域专业知识。成功的人机协作需要人类提供数学直觉、问题表述和战略方向。
...
TL;DR: 提出了OmniGAIA基准测试和OmniAtlas模型，用于评估和增强多模态AI在视频、音频、图像跨模态推理和工具使用能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要局限于双模态交互（如视觉-语言），缺乏统一认知能力，无法满足通用AI助手需求。需要开发能够处理视频、音频、图像等多种模态并具备深度推理和工具使用能力的系统。

Method: 1. 提出OmniGAIA基准测试，采用全模态事件图方法构建复杂多跳查询，需要跨模态推理和外部工具集成；2. 提出OmniAtlas原生全模态基础智能体，采用工具集成推理范式，通过后见指导树探索策略合成训练轨迹，并使用OmniDPO进行细粒度错误校正。

Result: OmniGAIA基准测试能够评估全模态智能体在跨模态推理和工具执行任务上的表现；OmniAtlas模型有效增强了现有开源模型的工具使用能力。

Conclusion: 这项工作标志着向下一代原生全模态AI助手迈出了一步，为真实世界场景中的多模态智能交互提供了新的评估框架和模型架构。
...
TL;DR: 该论文提出首个通用智能体评估框架，通过统一协议和Exgentic框架评估通用智能体在不同环境中的表现，建立了首个开放通用智能体排行榜。
...
Result: 通用智能体能够跨不同环境泛化，在没有任何环境特定调优的情况下，性能可与领域专用智能体相媲美。建立了首个开放通用智能体排行榜。

Conclusion: 该研究为通用智能体的系统性研究奠定了基础，发布了评估协议、框架和排行榜，推动通用智能体研究成为独立的研究方向。
...
### [165] [Certified Circuits: Stability Guarantees for Mechanistic Circuits](https://arxiv.org/abs/2602.22968)
...
Method: 提出Certified Circuits框架，将任何黑盒发现算法与随机数据子采样相结合，通过可证明的稳定性保证来认证电路组件包含决策对概念数据集有限编辑距离扰动的不变性，不稳定的神经元会被排除。
...
TL;DR: SPM-Bench是一个针对扫描探针显微镜（SPM）的博士级多模态基准测试，通过自动化数据合成管道和创新的评估方法，解决了现有科学基准测试中的数据污染、复杂度不足和人工成本过高的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用推理方面取得突破，但在专业科学领域存在明显差距。现有基准测试存在数据污染、复杂度不足和人工成本过高的问题，需要专门针对扫描探针显微镜（SPM）领域的高质量基准测试。

Method: 1. 开发全自动数据合成管道，确保高权威性和低成本；2. 使用锚定门控筛（AGS）技术从arXiv和期刊论文中高效提取高价值图文对；3. 采用混合云-本地架构，VLMs仅返回空间坐标用于本地高保真裁剪，实现极端令牌节省；4. 引入严格不完美惩罚F1（SIP-F1）评分进行客观评估。

Result: 1. 建立了高质量的SPM多模态基准测试；2. 通过SIP-F1评分建立了严格的能力层次结构；3. 首次量化了模型的"个性"（保守型、激进型、赌徒型或智慧型）；4. 通过关联模型报告的置信度和感知难度，揭示了当前AI在复杂物理场景中的真实推理边界。

Conclusion: SPM-Bench不仅为SPM领域提供了高质量的评估基准，其自动化科学数据合成范式具有普遍适用性，为其他科学领域的基准测试开发提供了可推广的范例。
...
Method: 引入诊断对齐框架，将AI生成的影像报告保存为不可变的推理状态，并与医生验证结果系统比较。推理流程整合了视觉大语言模型、基于BERT的医学实体提取和序列语言模型推理(SLMI)步骤，在专家评审前强制执行领域一致的细化。评估采用四级一致性框架：精确主要匹配率(PMR)、语义相似度调整率(AMR)、跨类别对齐和综合一致性率(CCR)。

Result: 在21个皮肤科病例中，精确一致率达到71.4%，语义相似度调整后保持不变(t=0.60)。结构化跨类别和鉴别诊断重叠分析显示100%的综合一致性(95% CI: [83.9%, 100%])。没有病例显示完全诊断分歧。

Conclusion: 二元词汇评估显著低估了临床意义上的对齐程度。将专家验证建模为结构化转换，能够实现信号感知的修正动态量化，并支持基于影像的临床决策支持系统的可追溯、人类对齐评估。
...
TL;DR: RepSPD是一种基于几何深度学习的EEG解码模型，通过在黎曼流形上实现跨注意力机制来调制SPD矩阵的几何属性，并引入全局双向对齐策略增强几何一致性，显著优于现有方法。
...
Method: 提出RepSPD模型：1）在黎曼流形上实现跨注意力机制，用图导出的功能连接特征调制SPD矩阵的几何属性；2）引入全局双向对齐策略重塑切空间嵌入，减轻曲率引起的几何失真。

Result: 大量实验表明，该框架显著优于现有EEG表示方法，展现出卓越的鲁棒性和泛化能力。

Conclusion: RepSPD通过结合几何深度学习和功能连接特征，有效解决了当前SPD方法在频率特异性和局部拓扑结构方面的不足，为EEG解码提供了更强大的工具。
...
TL;DR: 该论文提出CC-BOS框架，利用文言文生成对抗提示进行越狱攻击，通过多维度果蝇优化算法自动生成文言文对抗提示，在黑色盒设置下实现高效自动化越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）的安全风险日益受到关注，现有研究表明LLMs对越狱攻击高度敏感，且攻击效果在不同语言环境中存在差异。文言文因其简洁性和隐晦性，能够部分绕过现有安全约束，暴露LLMs的显著漏洞。

Method: 提出CC-BOS框架，基于多维度果蝇优化算法自动生成文言文对抗提示。将提示编码为八个策略维度（角色、行为、机制、隐喻、表达、知识、触发模式和上下文），通过嗅觉搜索、视觉搜索和柯西变异进行迭代优化。还设计了文言文到英文的翻译模块以提高可读性和评估准确性。

Result: 大量实验证明CC-BOS框架的有效性，在越狱攻击效果上持续优于现有最先进的越狱攻击方法。

Conclusion: 文言文在越狱攻击中具有独特优势，CC-BOS框架能够有效利用文言文的特性进行自动化越狱攻击，揭示了LLMs在文言文处理方面的安全漏洞，为LLMs的安全防护提供了重要参考。
...
TL;DR: 使用强化学习方法为多智能体F1比赛策略优化，通过交互模块考虑对手行为，结合自博弈训练生成竞争性策略


<details>
  <summary>Details</summary>
Motivation: F1比赛中，车队需要根据不断变化的比赛条件和竞争对手的行动调整比赛策略。传统策略制定难以实时适应动态竞争环境，需要一种能够考虑对手行为的智能优化方法

Method: 基于预训练的单智能体策略，引入交互模块来考虑竞争对手的行为。结合交互模块和自博弈训练方案，生成竞争性策略。智能体根据相对性能进行排名，学习平衡能量管理、轮胎退化、空气动力学交互和进站决策

Result: 智能体能够根据对手调整进站时机、轮胎选择和能量分配，实现稳健一致的比赛表现。该框架仅依赖实际比赛中可用的信息，可在赛前和赛中支持策略师的决策

Conclusion: 提出的强化学习方法能够有效优化多智能体F1比赛策略，通过考虑对手行为的交互模块和自博弈训练，生成适应性强、竞争性高的策略，为实际比赛策略制定提供支持
...
TL;DR: 本文提出AILS-AHD方法，利用大语言模型动态生成和优化破坏启发式，在容量限制车辆路径问题上取得突破性进展，在大型基准测试中创造了8个新的最优解。


<details>
  <summary>Details</summary>
Motivation: CVRP作为基础组合优化问题，其NP难特性在大规模实例中带来显著计算挑战。传统方法在处理大规模问题时存在局限性，需要更智能的启发式设计方法。

Method: 提出AILS-AHD方法，将进化搜索框架与大语言模型结合，动态生成和优化破坏启发式，并引入基于LLM的加速机制提升计算效率。

Result: 在中等和大型实例上均优于AILS-II和HGS等最先进求解器，在CVRPLib大型基准测试的10个实例中，为8个创造了新的最优解。

Conclusion: AILS-AHD展示了LLM驱动启发式设计在车辆路径优化领域的巨大潜力，为CVRP求解提供了创新性解决方案。
...
TL;DR: MALLET是一个基于多智能体LLM的情感净化系统，通过四个智能体分析、调整、监控和指导信息消费，显著降低情感刺激强度同时保持语义完整性。


<details>
  <summary>Details</summary>
Motivation: 在注意力经济中，煽情内容让消费者暴露于过度情感刺激，阻碍冷静决策。需要一种既能降低情感刺激又不限制原始文本访问的信息净化框架。
...
Result: 在800篇AG News文章上的实验显示：刺激分数显著降低(最高19.3%)，情感平衡改善，语义保持良好。刺激降低与语义保持接近零相关，表明两者可独立控制。类别分析显示体育、商业、科技类大幅降低(17.8-33.8%)，世界类效果有限(事实本身具有高刺激性)。
...
TL;DR: 论文提出世界模型需要建立在"一致性三位一体"（模态、空间、时间一致性）的理论框架上，并引入CoW-Bench基准来评估视频生成模型和统一多模态模型。


<details>
  <summary>Details</summary>
Motivation: 当前虽然视频生成模型（如Sora）和统一多模态模型（UMM）在模拟物理动态方面取得进展，但缺乏定义通用世界模型所需基本属性的原则性理论框架。需要建立系统化的理论指导来推进通用人工智能的发展。

Method: 提出"一致性三位一体"理论框架：模态一致性作为语义接口，空间一致性作为几何基础，时间一致性作为因果引擎。通过这一框架系统回顾多模态学习演进，并引入CoW-Bench基准，该基准专注于多帧推理和生成场景，统一评估视频生成模型和UMMs。

Result: 建立了世界模型的原则性理论框架，揭示了多模态学习从松散耦合的专门模块向统一架构发展的轨迹，这些架构能够协同涌现内部世界模拟器。CoW-Bench为评估现有系统提供了统一标准。

Conclusion: 该工作为通用世界模型建立了原则性路径，阐明了当前系统的局限性以及未来进展所需的架构要求。"一致性三位一体"框架为世界模型的发展提供了理论基础和评估标准。
...
Result: 在多样化时间序列问答任务上的大量实验表明，PATRA优于现有基线方法，展现出卓越的跨模态理解和推理能力。
...
Result: 1. 在虚拟细胞统一评估范式下，当前模型在生物学复杂任务上表现不均，特别是在需要机制或因果理解的任务上；2. 知识增强评估框架确保生物学正确性，提供可解释的、基于证据的推理，并实现高区分能力，克服传统指标的脆弱性和不透明性。

Conclusion: SC-ARENA为评估单细胞生物学中的LLM提供了一个统一且可解释的框架，指向开发与生物学对齐、可泛化的基础模型。
...
TL;DR: 论文证明基于优化的AI系统（如RLHF训练的LLM）在形式上无法实现规范治理，因为它们缺乏真正的能动性所需的结构条件，这导致系统性失败模式而非偶然错误。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统被部署在医疗、法律、金融等高风险领域，假设它们可以被规范治理。但论文质疑这一假设，认为基于优化的系统（特别是RLHF训练的LLM）在形式上无法满足规范治理的要求。

Method: 通过形式化分析，建立真正能动性所需的两项必要且充分的结构条件：不可通约性（将某些边界视为不可协商的约束而非可交易的权重）和否定性响应（当这些边界受威胁时能够暂停处理的非推理机制）。证明RLHF系统在结构上与这两个条件不相容。

Result: RLHF系统与规范治理在形式上不相容，其优化操作（将所有价值统一到标量度量并总是选择最高分输出）排除了规范治理的可能性。已观察到的失败模式（奉承、幻觉、不忠实的推理）不是偶然错误，而是结构表现。此外，部署不当会引发"趋同危机"，使人类从真正的能动者退化为标准检查优化器。

Conclusion: 基于优化的AI系统（如RLHF训练的LLM）在结构上无法实现规范治理，因为它们缺乏真正能动性所需的结构条件。论文的主要积极贡献是提供了一个基质中立的架构规范，定义了任何系统（生物、人工或制度）要成为能动者而非复杂工具必须满足的条件。
...
TL;DR: AgentDropoutV2是一个测试时修正或拒绝的剪枝框架，用于动态优化多智能体系统的信息流，无需重新训练，通过检索增强的修正器和失败驱动指标池来纠正错误，不可修复的输出被剪枝以防止错误传播。
...
Method: 提出AgentDropoutV2框架，作为主动防火墙拦截智能体输出，使用检索增强的修正器基于失败驱动指标池迭代纠正错误，利用蒸馏的失败模式作为先验知识精确识别潜在错误，不可修复的输出被剪枝，同时采用回退策略保持系统完整性。
...
Conclusion: AgentDropoutV2是一个有效的测试时框架，能够在不重新训练的情况下动态优化多智能体系统的信息流，显著提升性能并防止错误传播，具有强大的泛化能力和适应性。
...
TL;DR: 本文提出了AIQI，这是首个在通用强化学习中被证明具有渐进ε最优性的无模型智能体，通过分布动作值函数的通用归纳实现。
...
Result: 在"grain of truth"条件下，证明AIQI具有强渐进ε最优性和渐进ε贝叶斯最优性，这是首个被证明具有这些性质的无模型通用智能体。

Conclusion: AIQI显著扩展了已知通用智能体的多样性，首次展示了无模型方法在通用强化学习中的渐进最优性，为通用人工智能提供了新的理论框架。
...
TL;DR: LLM访问使生物学新手在生物安全相关任务上的表现显著提升，准确率比仅使用互联网的对照组高4.16倍，甚至在某些任务上超过了专家基线


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在生物学基准测试上表现越来越好，但尚不清楚它们是否能真正提升新手用户的表现（即是否能让人类比仅使用互联网资源时表现更好）。这种不确定性对于理解科学加速和双重用途风险至关重要。

Method: 进行了多模型、多基准的人类提升研究，比较了有LLM访问权限的新手与仅使用互联网访问权限的对照组在八个生物安全相关任务集上的表现。参与者有充足时间（最复杂的任务长达13小时）解决复杂问题。

Result: LLM访问提供了实质性提升：有LLM的新手比对照组的准确率高4.16倍（95% CI [2.63, 6.87]）。在四个有专家基线的基准测试中，有LLM的新手在其中三个上超过了专家。有趣的是，独立的LLM通常超过LLM辅助的新手，表明用户未能充分挖掘LLM的潜力。大多数参与者（89.6%）报告在获取双重用途相关信息时几乎没有遇到安全限制。

Conclusion: LLM显著提升了新手在原本需要专业训练的生物任务上的表现，强调了需要持续、交互式的提升评估与传统基准测试并行进行的重要性，特别是在双重用途风险背景下。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，需要确保其输出能被能力较弱的系统轻松检查。现有证明者-验证者游戏虽然能提高模型输出的可检查性，但会导致准确性下降（可读性税）

Method: 解耦正确性和可检查性条件，训练一个"翻译器"模型，将固定求解器模型的解转换为可检查形式。首先训练求解器最大化正确性，然后训练翻译器将求解器的解转换为可检查形式同时保留答案。为此制定了解耦的证明者-验证者游戏框架

Result: 该方法允许在保持求解器高准确性的同时，通过翻译器实现输出的可检查性，避免了传统方法中的可读性税问题

Conclusion: 通过解耦正确性和可检查性训练翻译器的方法，能够在保持模型高准确性的同时实现输出的可检查性，为解决可读性税问题提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 传统潜变量方法通过离散时间建模脑动力学，存在累积预测误差问题，且难以捕捉EEG的瞬时非线性特征。需要一种能建模连续脑动力学并捕捉复杂脑状态随机变化的方法。
...
Conclusion: ODEBRAIN通过神经ODE建模连续潜在动态，有效克服了传统方法的局限性，为脑动力学建模提供了更准确、鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究KM信念更新公理与AGM信念修正公理之间的关系，通过模态逻辑框架形式化比较这两种信念变化理论，探索它们之间的包含关系。

Method: 为KM信念更新的每个公理提供对应的模态逻辑公理，使用包含三个模态算子（B信念、>条件、□必然性）的模态逻辑。然后将得到的逻辑与从AGM公理转换而来的类似逻辑进行比较。

Result: 证明了每个L_KM公理都是L_AGM的定理，因此AGM信念修正是KM信念更新的特例。对于强版本KM更新，L_KM和L_AGM之间的差异可归结为处理非意外信息的单个公理。

Conclusion: 通过模态逻辑形式化，建立了AGM信念修正作为KM信念更新特例的理论关系，为理解两种信念变化理论提供了统一的逻辑框架。
...
<details>
  <summary>Details</summary>
Motivation: 即使经过优化的AI模型也会因偶然性和认知不确定性产生推理错误，观察到基于输入不变变换的推理错误存在部分独立性

Method: 提出重采样推理方法：对训练好的AI模型应用输入的多个变换版本，聚合推理输出得到更准确结果
...
Method: 提出了三种新算法：GRAVE2（通过两层搜索扩展GRAVE）、GRAVER（通过节点回收扩展GRAVE）、GRAVER2（结合两层搜索和节点回收技术）

Result: 这些增强技术能够显著减少存储节点数量，同时保持与GRAVE相当的游戏强度

Conclusion: 通过两层搜索和节点回收技术，可以在内存受限环境中有效应用GRAVE算法，扩展其实用性
...
Method: 提出多智能体LLM交易框架，将投资分析明确分解为细粒度任务而非粗粒度指令；使用日本股票数据（价格、财务报表、新闻、宏观信息）在泄漏控制的回测设置下评估；通过投资组合优化利用系统输出与股票指数的低相关性及方差。

Result: 细粒度任务分解相比传统粗粒度设计显著提高了风险调整后收益；分析中间智能体输出发现，分析输出与下游决策偏好的对齐是系统性能的关键驱动因素；通过投资组合优化获得了更优的性能表现。

Conclusion: 研究结果为实际应用中LLM智能体交易系统的智能体结构和任务配置设计提供了重要参考，强调了细粒度任务分解和分析-决策对齐的重要性。
