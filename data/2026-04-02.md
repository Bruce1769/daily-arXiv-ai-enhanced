...
Method: 提出HIVE框架，引入视觉编码器与LLM之间的分层交叉注意力机制，采用三阶段训练策略逐步对齐视觉编码器和LLM

Result: 在图像分类和多种视觉语言任务上表现优异，在MME、GQA、OK-VQA、ScienceQA等基准测试中超越基于自注意力的方法

Conclusion: 分层特征整合能显著提升视觉语言模型的效率和表达能力，为更高效的视觉语言模型设计开辟了新途径
...
<details>
  <summary>Details</summary>
Motivation: 原始(raw)数据对低级视觉任务更忠实，但大规模原始数据集的收集存在瓶颈，现有数据集有限且与特定相机硬件绑定。现有扩散框架设计用于合成照片完成的sRGB图像，而非物理一致的线性表示。

Method: 利用大规模sRGB扩散模型的生成先验，通过潜在空间和像素空间的专门处理合成物理有意义的线性输出。构建多对一逆ISP数据集，其中同一场景的多个sRGB渲染版本锚定到共同的场景参考目标。微调解码器和条件去噪器来处理未知和多样的ISP流水线。

Result: RawGen在传统假设固定ISP的逆ISP方法上表现出优越性能。通过RawGen的可扩展、文本驱动的合成数据增强训练流水线，可以有益于下游低级视觉任务。
...
TL;DR: 本文提出Q-Mask框架，通过因果查询驱动掩码解码器实现精确OCR，解决现有视觉语言模型在文本区域定位方面的不足，并构建了TextAnchor-Bench基准和TextAnchor-26M数据集进行评估。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然具备OCR能力，但在将查询文本准确锚定到对应空间区域方面表现不佳，缺乏可靠的文本锚点支持下游推理任务。

Method: 提出Q-Mask框架，基于因果查询驱动掩码解码器(CQMD)，采用链式思维推理的视觉解码方式，先生成查询条件化的视觉掩码，再产生最终OCR输出，实现"文本在哪里"和"文本是什么"的解耦。

Result: 通过构建TextAnchor-Bench基准评估发现现有模型在文本区域定位方面存在困难；Q-Mask在TextAnchor-26M数据集上训练后，在各种视觉场景中显著提升了文本锚定和理解能力。

Conclusion: Q-Mask通过因果视觉解码和显式文本锚点构建，有效解决了视觉语言模型在文本区域定位方面的局限性，为可靠的文本锚定提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现MIM目标函数导致学习到的表征保留非语义信息，这会损害推理时的性能表现

Method: 提出SOAP方法：使用PCA分析真实和合成非语义图像，通过语义正交投影直接抑制补丁表征中的非语义信息

Result: SOAP在各种基于MIM的模型中带来零样本性能的一致提升，且无需训练，可作为单个线性头附加到任何模型

Conclusion: SOAP是一种模型无关的后处理抑制方法，能有效提升MIM模型的语义表征质量
...
TL;DR: UCell是一个用于单细胞分割的小型深度学习模型（10-30M参数），通过递归结构设计实现了参数高效性，性能媲美大10-20倍的模型，且无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 在生物医学研究中，模型扩展常受限于训练数据不足和高成本，但现有研究仍过度关注构建更大的基础模型，而提升小模型能力的研究相对不足。

Method: 设计了一个10-30M参数的小型模型UCell，关键创新是在前向计算图中引入递归结构，实现更参数高效的架构，可直接从显微图像数据从头训练，无需自然图像大规模预训练。

Result: 在多个基准测试中，UCell的性能与比它大10-20倍的模型相当，对未见域外数据具有相似的泛化能力，且通过一次性学习和少样本微调实验验证了其适应性。

Conclusion: UCell展示了小模型在生物医学任务中的潜力，通过参数高效设计和递归结构，实现了与大模型相当的性能，同时摆脱了对大规模预训练和外部商业利益的依赖。
...
TL;DR: QAsk-Nav是首个用于协作实例对象导航的可复现基准，通过独立评估导航和协作提问能力，解决了现有基准缺乏协作交互评估的问题。


<details>
  <summary>Details</summary>
Motivation: 现有协作实例对象导航基准主要关注导航成功率，缺乏对协作交互能力的一致评估，无法有效衡量模型在模糊视觉场景下通过对话解决问题的能力。

Method: 提出QAsk-Nav基准，包含：(1) 独立于导航的轻量级提问协议评分机制；(2) 增强的导航协议，提供真实、多样、高质量的目标描述；(3) 包含28,000条质量检查推理和提问轨迹的开源数据集。

Result: 基于QAsk-Nav基准开发了Light-CoNav模型，比现有模块化方法小3倍、快70倍，在未见过的物体和环境泛化能力上优于最先进的CoIN方法。

Conclusion: QAsk-Nav为协作实例对象导航提供了首个可复现的评估基准，能够独立评估导航和协作提问能力，推动了该领域模型交互能力的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究大多基于预处理好的社交线索，而现实场景中AI助手需要从原始数据中感知和推理。现有方法缺乏可靠的身份归属能力，导致社交互动理解不准确。

Method: 提出Omni-MMSI-R，一个参考引导的流程，使用工具生成身份归属的社交线索，并进行链式思维的社交推理。构建参与者级别的参考对，并在现有数据集上添加推理标注。

Result: 实验表明，Omni-MMSI-R在Omni-MMSI任务上优于先进的LLMs和其他对比方法。

Conclusion: Omni-MMSI任务对于开发能够从原始多模态输入中理解和响应人类社交互动的AI助手至关重要，提出的Omni-MMSI-R方法通过有效的身份归属和推理机制显著提升了性能。
...
TL;DR: OmniSch是首个评估大型多模态模型在PCB原理图理解和空间网表图构建能力的基准，包含1,854个真实原理图，涵盖视觉定位、图推理、几何推理和工具增强推理四大任务，揭示了当前模型在工程图纸理解上的显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在视觉定位、文档理解和图表推理方面进展迅速，但在将PCB原理图转换为机器可读的空间加权网表图方面能力不足，而这种图表示是电子设计自动化工作流的核心。现有模型在理解包含组件属性、连接性和几何信息的工程图纸方面存在明显差距。

Method: 研究者提出了OmniSch基准，包含1,854个真实世界原理图，设计四个核心任务：1) 原理图实体的视觉定位（10.99万个实例，42.34万个语义标签）；2) 图到图推理，理解元素间的拓扑关系；3) 几何推理，构建连接依赖布局的权重；4) 工具增强的智能推理，通过调用外部工具完成前三项任务。

Result: 评估结果显示当前大型多模态模型在解释原理图工程工件方面存在显著差距：细粒度视觉定位不可靠、布局到图解析脆弱、全局连接推理不一致、视觉探索效率低下。模型在理解工程图纸方面的能力明显不足。

Conclusion: OmniSch基准填补了评估大型多模态模型在PCB原理图理解和空间网表图构建能力方面的空白，揭示了当前模型在工程图纸理解上的局限性，为未来模型改进提供了重要基准和方向。
...
TL;DR: EASe是一个无监督领域无关的语义分割框架，通过语义感知上采样和通道激励技术，在复杂多组件形态场景中实现细粒度掩码发现，超越了现有方法。
...
Method: 提出了EASe框架，包含三个核心组件：1) SAUCE（语义感知上采样与通道激励）- 激励低分辨率基础模型特征通道进行选择性校准；2) 跨空间编码图像和基础模型特征的注意力机制以恢复全分辨率语义表示；3) CAFE（线索注意特征聚合器）- 利用SAUCE注意力分数作为语义分组信号，将聚合特征分割为多粒度掩码。
...
Method: 提出TPC-CMA（三阶段跨模态对齐课程学习）微调框架：1）将模态间隙分解为质心间隙和分布间隙；2）设计三阶段课程学习，通过梯度感知调度逐步引入对齐；3）联合减少质心偏移并重塑分布结构，实现稳定优化。

Result: 实验表明：当α_target=0.05时，模态间隙减少66.6%，仅带来4.84%的准确率下降；当α_target=0.5时，模态间隙减少82.3%，聚类ARI从0.318提升到0.516，图像描述CIDEr分数提高57.1%。分布间隙是跨模态任务质量的真正预测指标（R²=0.986）。

Conclusion: TPC-CMA方法通过明确减少质心间隙和分布间隙，显著改善了视觉语言模型的跨模态对齐效果，为需要跨模态互换性的任务提供了有效的解决方案。
...
TL;DR: SANA-I2I是一个无文本的高分辨率图像到图像生成框架，通过移除文本条件，仅依赖配对源-目标图像学习潜在空间中的条件流匹配模型，专门用于医学图像处理任务。


<details>
  <summary>Details</summary>
Motivation: 现有图像到图像生成方法通常依赖文本提示，但在医学成像等专业领域，文本描述可能不准确或难以获取。作者希望开发一个完全基于图像对的无文本框架，专门针对医学图像处理任务，如胎儿MRI运动伪影减少。

Method: 1. 扩展SANA家族，完全移除文本条件；2. 仅使用配对源-目标图像训练条件流匹配模型；3. 在潜在空间中学习条件速度场，将目标图像分布映射到另一个分布；4. 针对胎儿MRI运动伪影减少任务，采用基于Duffy等人方法的合成数据生成策略来创建配对训练数据。

Result: 实验结果表明，SANA-I2I能有效抑制运动伪影同时保持解剖结构，在少量推理步骤下达到竞争性性能。该框架特别适合医学成像中的监督图像到图像任务。

Conclusion: 提出的基于流的无文本生成模型在医学成像监督图像到图像任务中表现出高效性和适用性，为专业领域图像处理提供了不依赖语言提示的有效解决方案。
...
TL;DR: 使用冻结的DINOv3 ViT-B嵌入，通过基于最近邻的自训练方法，在仅有5%标签的情况下接近全监督模型性能，为海洋物种识别提供无需训练、即插即用的高效标注解决方案。


<details>
  <summary>Details</summary>
Motivation: 水下图像物种分类面临专家标注成本高、监督模型难以跨数据集迁移的问题，需要探索在标注资源有限情况下的高效分类方法。

Method: 采用冻结的DINOv3 ViT-B嵌入，无需微调，通过基于最近邻的自训练方法，利用少量标注种子在未标注数据中传播标签，在AQUA20基准（20种海洋物种）上进行评估。

Result: 仅使用不到5%的训练标签，自训练方法就能接近全监督ConvNeXt基线的性能；在全监督情况下，差距缩小到几个百分点，部分物种甚至超过监督基线。嵌入空间的类别可分性（ROC-AUC）即使在极低标签情况下也很高。

Conclusion: 冻结基础模型嵌入结合自训练的方法无需训练、无需领域特定数据工程、无需水下适应模型，为标签高效的海洋物种识别建立了实用、可立即部署的基线。
...
<details>
  <summary>Details</summary>
Motivation: 激光雷达对自动驾驶感知至关重要，但易受干扰攻击影响。虽然点云数据在干扰下会随机化，但全波形数据中攻击信号与真实信号存在可区分的特征，这为防御干扰攻击提供了新思路。

Method: 提出PULSAR-Net方法，采用带有轴向空间注意力机制的新型U-Net架构，专门设计用于在全波形表示中识别攻击诱导信号与真实物体返回信号。为解决现有数据集中缺乏干扰攻击下全波形数据的问题，开发了物理感知的数据集生成流程来合成逼真的干扰攻击下全波形表示。

Result: 尽管仅使用合成数据训练，PULSAR-Net在真实世界的静态和动态驾驶场景中，对受干扰攻击遮挡的车辆分别实现了92%和73%的重建率。

Conclusion: 通过利用激光雷达系统中先前未充分利用的中间全波形表示和同步激光传感，PULSAR-Net能够在干扰攻击下有效重建真实点云，为激光雷达安全防御提供了新方法。
...
TL;DR: 提出基于动态图模型的自适应节点选择方法，用于RGB-D室内场景识别，通过自适应选择关键局部特征并建模物体间关系，在公开数据集上取得优于现有方法的效果。
...
Method: 提出动态图模型，包含自适应节点选择机制：1）构建动态图建模物体与场景的关系；2）提出自适应节点选择方法从RGB和深度两种模态中提取关键局部特征进行图建模；3）将节点按三个不同层次分组，表示物体间的远近关系；4）根据注意力权重动态更新图模型；5）将更新优化的RGB和深度特征融合用于室内场景识别。

Result: 在SUN RGB-D和NYU Depth v2公开数据集上进行实验，大量结果表明该方法相比现有最先进方法具有优越性能，证明所提方法能够有效利用RGB和深度两种模态的关键局部特征。

Conclusion: 提出的动态图模型通过自适应节点选择机制解决了RGB-D室内场景识别中关键局部特征的选择和利用问题，有效建模了物体间关系，在公开数据集上验证了方法的有效性。
...
TL;DR: mmAnomaly：结合毫米波雷达与RGBD视觉的多模态异常检测框架，通过条件潜在扩散模型生成预期毫米波频谱，实现遮挡场景下的鲁棒异常检测


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达能够在非视觉场景（如穿透衣物或墙壁）中进行人体感知，但现有方法缺乏上下文感知能力，容易将良性信号变化误判为异常，需要结合视觉上下文来提高检测准确性

Method: 1. 使用快速ResNet分类器提取视觉场景的语义线索（场景几何和材料属性）；2. 采用条件潜在扩散模型根据视觉上下文合成预期的毫米波频谱；3. 通过双输入比较模块识别真实与生成频谱之间的空间偏差来定位异常

Result: 在三个应用场景（隐藏武器定位、穿墙入侵者定位、穿墙跌倒定位）的两个多模态数据集上评估，系统达到94%的F1分数和亚米级定位误差，在衣物、遮挡和杂乱环境中表现出鲁棒泛化能力
...
TL;DR: 本文提出TEP方法，通过跟踪增强提示改进SAM3在复杂视频目标分割中对微小和语义主导目标的性能，在PVUW 2026挑战赛中获得第一名


<details>
  <summary>Details</summary>
Motivation: 当前最先进的SAM3模型在常规目标上表现出色，但在微小目标和语义主导目标上表现不佳，主要原因是SAM3对这些特定目标类型的理解不足

Method: 提出TEP方法，作为无需训练的方法，利用外部跟踪模型和多模态大语言模型引入跟踪增强提示，帮助SAM3更好地理解挑战性目标

Result: 在PVUW挑战赛2026：复杂视频目标分割赛道的测试集上获得了第一名，得分56.91%

Conclusion: TEP方法通过跟踪增强提示有效提升了SAM3在复杂视频目标分割中对挑战性目标的性能，证明了外部跟踪和多模态语言模型辅助的有效性
...
<details>
  <summary>Details</summary>
Motivation: 心电图数字化可以解锁数十亿存档临床记录，但现有方法在真实世界图像上表现不佳，尽管基准测试数据良好。需要一种能够提高数字化质量而不修改现有系统的解决方案。

Method: VLM-in-the-Loop插件模块，通过标准化接口包装任何数字化后端，采用工具锚定机制将视觉语言模型评估锚定在领域特定信号分析工具的定量证据上。

Result: 工具锚定将判决一致性从71%提高到89%，保真度分离翻倍；在四个后端上均改善性能：29.4%边界导联改善，41.2%失败肢体导联恢复，每图像有效导联从2.5增加到5.8；在428个真实临床HCM图像上达到98.0%优秀质量。

Conclusion: VLM-in-the-Loop架构和工具锚定机制是领域参数化的，表明在质量标准可客观测量的任何领域都有更广泛的应用潜力。
...
TL;DR: VAE-MMD预处理管道结合变分自编码器和最大均值差异损失，通过特征对齐减少跨机构数据异质性，提升脑转移瘤分割的泛化性能。
...
Method: 提出VAE-MMD预处理管道，结合变分自编码器(VAE)与最大均值差异(MMD)损失，包含跳跃连接和自注意力机制，配合nnU-Net分割。在斯坦福、UCSF、UCLM和PKG四个公共数据库的740名患者上进行测试。

Result: VAE-MMD将域分类器准确率从0.91降至0.50，成功实现跨机构特征对齐。重建体积PSNR大于36dB，保持解剖准确性。相比基线nnU-Net，平均F1提升11.1%(0.700到0.778)，平均表面Dice提升7.93%(0.7121到0.7686)，平均HD95降低65.5%(11.33到3.91mm)。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数公开数据集（如Waymo、Argoverse）基于西方道路环境收集，无法反映韩国等地区的独特交通模式、基础设施和驾驶行为，导致在这些地区部署时性能下降。

Method: 使用韩国自动驾驶数据集，比较四种训练策略：零样本迁移、从头训练、完全微调和编码器冻结，研究QCNet从美国数据到韩国环境的迁移适应性。

Result: 利用预训练知识显著提高了预测性能，特别是选择性微调解码器同时冻结编码器的策略在准确性和训练效率之间达到最佳平衡，相比从头训练减少了超过66%的预测误差。
...
TL;DR: 本文提出了一种无需训练的解决方案，在PVUW MeViS-Text挑战赛中获胜。该方法结合多模态大语言模型和SAM3，通过事件分解、目标定位、掩码传播和语义验证三阶段流程，实现了运动中心语言表达下的视频对象分割。


<details>
  <summary>Details</summary>
Motivation: 解决MeViS-Text挑战中的视频对象分割问题，该任务要求模型同时理解外观、时间行为和对象交互。传统方法需要大量任务特定的微调，本文旨在探索无需训练的解决方案。

Method: 构建完全无需训练的流水线，包含三个阶段：1) 使用Gemini-3.1 Pro分解目标事件为实例级定位目标，选择目标最清晰可见的帧并生成判别性描述；2) SAM3-agent在选定帧生成精确种子掩码，官方SAM3跟踪器在整个视频中传播掩码；3) 使用Qwen3.5-Plus和行为级验证进行细化，修正模糊或语义不一致的预测。

Result: 在PVUW 2026 MeViS-Text测试集上排名第一，获得Final分数0.909064和J&F分数0.7897，无需任务特定的微调。

Conclusion: 通过结合强大的多模态大语言模型和SAM3，构建了一个完全无需训练的流水线，成功解决了运动中心语言表达下的视频对象分割问题，在挑战赛中取得了优异表现。
...
TL;DR: FADE是一种针对基于查询传播的多目标跟踪器的新型对抗攻击框架，通过时间查询洪泛和时间记忆破坏两种策略，利用TBP架构的漏洞实现有效攻击。


<details>
  <summary>Details</summary>
Motivation: 基于查询传播的MOT方法虽然通过端到端管道和长时间建模取得了进展，但其依赖查询传播的架构存在未探索的对抗攻击漏洞。研究者旨在开发专门针对这些特定漏洞的攻击框架。

Method: FADE采用两种攻击策略：1) 时间查询洪泛：生成虚假的时间一致跟踪查询，耗尽跟踪器的有限查询预算，迫使终止有效跟踪；2) 时间记忆破坏：直接攻击查询更新器的记忆，通过状态去相关切断时间链接，并擦除匹配跟踪的学习特征身份。此外，通过可微分管道优化攻击以实现物理世界可实现性。

Result: 在MOT17和MOT20基准测试上的实验表明，FADE对最先进的TBP跟踪器非常有效，导致显著的身份切换和跟踪终止。

Conclusion: FADE成功揭示了基于查询传播的MOT方法中存在的架构漏洞，为未来开发更鲁棒的跟踪系统提供了重要见解，同时强调了在物理世界中实现对抗攻击的可能性。
...
TL;DR: 本文提出First Logit Boosting (FLB)方法，通过存储第一个生成token的logit并添加到后续token预测中，有效缓解LVLMs中的长期衰减问题，显著减少对象幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)在多模态任务中表现出色，但对象幻觉问题仍然存在。现有方法如重新训练和外部接地方法成本高或结构复杂，而无需训练的方法如对比解码(CD)存在长期衰减问题，即随着生成过程进行，视觉接地减弱而语言先验主导。
...
Result: 实验结果显示，FLB在各种任务、基准测试和骨干模型中显著减少了对象幻觉。该方法仅产生可忽略的推理开销，非常适合实时多模态系统。

Conclusion: FLB是一种简单有效的无需训练技术，能有效缓解LVLMs中的长期衰减问题，显著减少对象幻觉，且计算开销极小，具有很高的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 7T MRI能更好地显示多发性硬化白质病变，但其对比度和伪影与1.5-3T成像有显著差异，现有自动分割工具可能无法直接适用，需要开发专门针对7T MRI的分割方法

Method: 使用7T FLAIR扫描数据，通过LST工具生成参考病变掩膜并经过专家手动修正。比较了传统LST-LPA和LST-AI工具，并训练了3D UNETR和SegFormer Transformer模型在不同分辨率下的表现

Result: 在原生0.5x0.5x0.5³分辨率下，7T训练的Transformer模型与LST-AI表现相当，但能检测到更多小病变。最佳模型SegFormer在体素级Dice得分为0.61，病变级Dice为0.20，优于传统LST-LPA工具。降采样训练会降低性能

Conclusion: 基于Transformer的深度学习模型在7T MRI多发性硬化病变分割中优于传统方法，特别是对小病变的检测。原生7T分辨率对保持分割性能至关重要，研究提供了开源模型供超高清场MS研究使用
...
TL;DR: 本文提出MUPO方法解决GRPO强化学习中多样性崩溃问题，通过多组策略优化激励多样化思考，提升视觉语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明强化学习（特别是GRPO）能提升视觉语言模型的推理能力，但其有效机制和局限性尚未充分探索。研究发现RL模型倾向于深度但狭窄的推理，而基础模型虽然推理不够精细，但思考模式更广泛多样。GRPO容易导致多样性崩溃，使模型过早收敛到有限推理策略，丢弃多数潜在替代方案。

Method: 提出多组策略优化（MUPO），一种简单有效的多组策略优化方法，旨在激励跨多个解决方案的发散性思考，避免多样性崩溃问题。

Result: 在已有基准测试上证明了MUPO方法的有效性，能够解决GRPO的多样性崩溃问题，提升模型的推理能力和可扩展性。

Conclusion: MUPO通过激励多样化思考，解决了强化学习在视觉语言模型推理训练中的多样性崩溃问题，为提升模型推理能力提供了新方法。
...
TL;DR: CheXOne是一个用于胸部X光片解读的推理增强视觉语言模型，能同时生成诊断预测和明确的临床推理轨迹，在多项任务中超越现有模型，临床研究表明其报告质量与住院医师相当或更好。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片是全球最常进行的影像检查之一，但日益增长的影像量增加了放射科医生的工作负担和诊断错误风险。现有AI系统大多只生成最终预测，缺乏明确的视觉证据到放射学发现再到诊断预测的推理过程解释。

Method: CheXOne是一个推理增强的视觉语言模型，采用两阶段训练框架：结合指令微调和强化学习来提升推理质量。模型在来自30个公共数据集的1470万个指令和推理样本上进行训练，涵盖36个胸部X光片解读任务。

Result: 在17个评估设置的零样本测试中，CheXOne超越了现有的医学和通用领域基础模型，在独立公共基准测试中表现优异。临床读者研究表明，CheXOne生成的报告在55%的情况下与住院医师报告相当或更好，有效处理临床指征并提高了报告撰写和X光片解读效率。

Conclusion: 明确的推理轨迹可以提升AI辅助胸部X光片解读中的模型性能、可解释性和临床实用性。生成的推理轨迹具有高临床事实性，为最终预测提供了因果支持，这可能是性能提升的合理解释。
...
TL;DR: ARGS是一个用于3D对象生成的自回归高斯泼溅框架，通过多尺度并行预测实现可控细节级别的生成，相比传统方法具有对数级复杂度优势。


<details>
  <summary>Details</summary>
Motivation: 虽然自回归框架在2D图像生成中表现出色，能够通过逐步细化粗糙输入产生多样且复杂的内容，但将这一范式扩展到3D对象生成领域仍然未被充分探索。现有的3D生成方法在细节控制、计算效率和结构一致性方面存在挑战。

Method: 提出自回归高斯泼溅(ARGS)框架，采用高斯简化策略并通过反转简化过程来指导下一尺度生成。利用层次树结构将生成过程复杂度降至O(log n)，并提出基于树的transformer来自回归预测树结构，使叶节点能够关注其内部祖先节点以增强结构一致性。

Result: 实验表明，ARGS能够有效生成具有可控细节级别的多尺度高斯表示，在视觉保真度和时间消耗预算方面表现优异，相比传统方法具有更好的可管理性。

Conclusion: ARGS框架成功将自回归范式扩展到3D对象生成领域，通过层次树结构和基于树的transformer实现了高效、可控的多尺度3D内容生成，为3D生成领域提供了新的解决方案。
...
TL;DR: PC-SAM是一个统一框架，集成了全自动道路分割和交互式分割，通过精心设计的微调策略，将点提示的影响限制在对应patch内，解决了SAM在遥感道路分割中表现不佳的问题，支持细粒度局部修正。


<details>
  <summary>Details</summary>
Motivation: 当前全自动道路分割方法在识别具有挑战性的道路段时仍存在不足，常产生假阳性和假阴性区域，且不支持感兴趣区域的局部分割或现有掩码的细化。虽然SAM在自然图像交互分割中表现良好，但在遥感道路分割中表现不佳，无法支持细粒度局部细化。

Method: 提出PC-SAM框架，集成全自动道路分割和交互式分割。通过精心设计微调策略，将点提示的影响约束在对应patch内，克服原始SAM无法进行精细局部修正的局限，实现细粒度交互掩码细化。

Result: 在多个代表性遥感道路分割数据集上的实验表明，结合点提示时，PC-SAM在道路掩码分割方面显著优于最先进的全自动模型，同时提供灵活的局部掩码细化和局部道路分割。

Conclusion: PC-SAM成功解决了遥感道路分割中全自动方法的局限性，通过统一框架实现了高质量的全自动分割和灵活的交互式细化，为遥感道路分析提供了更强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 开放集目标检测面临文本表示与复杂视觉概念对齐困难、稀有类别图像-文本对稀缺的问题，现有视觉提示方法通常设计复杂且需要多阶段优化，开发周期长，同时缺乏有效的数据驱动训练策略。
...
Conclusion: PET-DINO通过AFVPG模块和提示增强训练策略，有效解决了开放集目标检测中的文本表示对齐和训练策略问题，缩短了开发周期，提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有弱监督HOI检测方法依赖外部物体检测器生成候选对，然后进行成对推理，存在计算成本高（需要枚举大量实例对）和假阳性问题（非交互组合），阻碍了准确的实例级HOI推理。

Method: 提出关系定位Transformer（RegFormer），利用空间定位信号作为推理过程的指导，促进局部感知的交互学习。通过学习局部化的交互线索，该模块能够区分人类、物体及其交互，实现从图像级交互推理到精确高效的实例级推理的直接迁移。

Result: 实验表明RegFormer能有效学习空间线索进行实例级交互推理，具有高效率，甚至达到与全监督模型相当的性能。

Conclusion: RegFormer通过空间定位信号引导的交互学习，解决了弱监督HOI检测中的计算效率和假阳性问题，实现了从图像级到实例级推理的有效迁移。
...
TL;DR: MAESIL是一个针对3D医学影像的自监督学习框架，通过创新的"超级补丁"和双掩码策略来有效捕获3D结构信息，在CT扫描重建任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像（如CT）的深度学习面临标注数据稀缺的挑战。现有方法通常将3D扫描作为独立的2D切片处理，这丢弃了关键的轴向连贯性和3D结构上下文信息，导致性能受限。

Method: 提出MAESIL框架，核心创新是"超级补丁"——一种平衡3D上下文保留和计算效率的3D块状输入单元。采用3D掩码自编码器策略和双掩码策略来学习全面的空间表示。
...
TL;DR: 提出基于可学习性引导的数据集蒸馏方法，通过渐进式生成合成数据集，减少样本冗余并提升性能
...
Conclusion: 可学习性引导的数据集蒸馏方法能有效减少样本冗余，通过自适应课程学习生成互补的训练样本，提升数据集蒸馏效果
...
<details>
  <summary>Details</summary>
Motivation: 色彩影响图像风格和情感表达，现有色彩分级方法依赖局部重着色或固定滤镜库，难以泛化到不同创意意图或与人类美学偏好对齐。

Method: 1) 将色彩分级定义为生成式色彩转换任务，直接生成条件化的3D-LUTs；2) 开发VQ-VAE分词器将3×32³ LUT向量压缩为64个离散token；3) 构建大规模数据集AceTone-800K；4) 训练视觉语言模型预测LUT token，并通过强化学习对齐感知保真度和美学。

Result: 在文本引导和参考引导的色彩分级任务上达到最先进性能，LPIPS指标相比现有方法提升高达50%。人类评估确认结果视觉愉悦且风格一致。

Conclusion: AceTone展示了语言驱动、美学对齐的色彩分级新途径，通过统一框架支持多模态条件，在感知质量和风格一致性方面显著优于传统方法。
...
TL;DR: FreqPhys是一个基于频率引导的远程光电容积描记(rPPG)框架，通过显式利用生理频率先验来增强对运动伪影和光照波动的鲁棒性，实现更准确的非接触式生理监测。
...
Method: 1) 生理带通滤波模块抑制带外干扰；2) 生理频谱调制和自适应频谱选择强调脉搏相关频率成分并抑制带内残余噪声；3) 跨域表示学习模块融合频谱先验和深度时域特征；4) 频率感知条件扩散过程逐步重建高保真rPPG信号。

Result: 在六个基准测试上的广泛实验表明，FreqPhys相比最先进方法有显著改进，特别是在具有挑战性的运动条件下表现突出。

Conclusion: FreqPhys证明了显式建模生理频率先验的重要性，为鲁棒的非接触式生理监测提供了有效解决方案。源代码将公开发布。
...
TL;DR: TRiGS是一种新的4D高斯泼溅表示方法，通过统一的连续几何变换解决现有方法中的时间碎片化和内存增长问题，能够扩展到长视频序列。


<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯泼溅方法依赖分段线性速度近似和短时间窗口，导致严重的时间碎片化，迫使基元反复消除和再生以跟踪复杂的非线性动态。这种临时近似消除了对象的长期时间身份，导致高斯基元不可避免地增殖，阻碍了扩展到长视频序列的能力。

Method: TRiGS采用统一的连续几何变换表示，整合了SE(3)变换、分层贝塞尔残差和可学习的局部锚点，为单个基元建模几何一致的刚性运动。

Result: 在标准基准测试中实现高保真渲染，同时能够独特地扩展到长视频序列（如600到1200帧），没有严重的内存瓶颈，在时间稳定性方面显著优于先前工作。
...
TL;DR: Reliev3R提出了一种弱监督训练范式，无需昂贵的多视角几何标注即可从头训练前馈重建模型，利用单目相对深度和图像稀疏对应关系实现3D重建。


<details>
  <summary>Details</summary>
Motivation: 前馈重建模型（FFRMs）在重建质量和多任务适应性方面表现出色，但完全监督训练方案过度依赖多视角几何标注（如3D点云图和相机位姿），导致难以扩展。需要一种低成本、可扩展的训练方法。

Method: 提出Reliev3R弱监督范式：1）利用预训练模型的零样本预测获取单目相对深度和图像稀疏对应关系；2）设计模糊感知相对深度损失函数；3）设计基于三角测量的重投影损失函数；4）无需几何传感器数据和计算密集的SfM预处理。

Result: Reliev3R在较少数据下从头训练，能够达到与完全监督模型相当的性能，实现了低成本3D重建监督和可扩展的FFRMs训练。

Conclusion: Reliev3R通过弱监督范式有效减轻了对多视角几何标注的依赖，为低成本3D重建监督和可扩展的前馈重建模型训练提供了可行方案。
...
TL;DR: TF-SSD是一种基于SAM和DINO协同的训练无关共显著目标检测方法，通过生成候选掩码池、质量筛选、语义过滤和跨图像原型选择实现高性能检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的CoSOD方法受限于封闭数据集且泛化能力有限，而视觉基础模型(VFMs)展现出强大的泛化能力和显著性理解能力，但鲜有研究探索其在CoSOD中的应用潜力。

Method: 提出训练无关方法TF-SSD：1) 使用SAM生成原始候选掩码池；2) 基于SAM的质量掩码生成器过滤冗余掩码；3) 利用DINO注意力图的图像内显著性过滤器识别视觉显著掩码；4) 跨图像原型选择器通过计算原型相似度选择最终掩码。

Result: 大量实验表明TF-SSD优于现有方法，相比最近的训练无关方法提升了13.7%的性能。

Conclusion: TF-SSD成功利用视觉基础模型(SAM和DINO)实现了高性能的训练无关共显著目标检测，展示了VFMs在CoSOD任务中的巨大潜力。
...
TL;DR: STAR是一个基于拓扑锚点的两阶段空间导航框架，通过RedMaze-23K数据集和人类启发的转折点标注，在复杂拓扑结构中实现更准确的空间推理，超越了现有开源模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型空间导航方法（如VoT）在复杂拓扑结构中容易产生级联错误，需要更可靠的空间推理框架来解决这一问题。

Method: 提出STAR两阶段框架：第一阶段通过监督微调帮助模型内化空间语义并剪枝冗余路径；第二阶段采用空间感知的段级直接偏好优化（SDPO）来精炼长时程导航中的自我校正能力。

Result: STAR的32B变体在性能上超越了DeepSeek-V3（29.27% vs. 25.00%），达到了GPT-4性能的82.4%，在开源模型中实现了最先进的性能。

Conclusion: STAR框架通过拓扑锚点和两阶段训练策略，显著提升了LLMs在结构化空间导航任务中的表现，为解决复杂拓扑中的级联错误问题提供了有效方案。
...
TL;DR: FecalFed是一个用于家禽疾病分类的隐私保护联邦学习框架，通过消除公开数据集中46.89%的重复图像，在高度异构的非IID数据条件下实现了接近集中式训练的性能。


<details>
  <summary>Details</summary>
Motivation: 高致病性禽流感和地方性家禽疾病的早期检测对全球粮食安全至关重要。虽然计算机视觉模型在粪便图像疾病分类方面表现出色，但大规模部署受到农场数据隐私问题和机构数据孤岛的瓶颈限制。此外，现有的开源农业数据集经常存在严重且未记录的数据污染问题。

Method: 提出了FecalFed隐私保护联邦学习框架，首先创建并发布了poultry-fecal-fl数据集，包含8,770张独特图像，覆盖四种疾病类别，消除了公开数据集中46.89%的重复率。在高度异构、非IID条件下（Dirichlet α=0.5）评估框架性能，使用服务器端自适应优化（FedAdam）配合Swin-Small架构，并测试了边缘优化的Swin-Tiny模型。

Result: 在高度异构数据条件下，孤立单农场训练仅获得64.86%的准确率，而联邦学习方法在不集中敏感数据的情况下恢复了性能。使用FedAdam和Swin-Small架构达到90.31%的准确率，接近集中式上限的95.10%。边缘优化的Swin-Tiny模型保持了89.74%的竞争性性能。

Conclusion: FecalFed框架为农场禽类疾病监测提供了一个高效、隐私优先的蓝图，能够在保护数据隐私的同时，在高度异构的农业环境中实现接近集中式训练的性能，解决了农业数据共享的隐私和数据质量问题。
...
<details>
  <summary>Details</summary>
Motivation: 社交VR平台虽然提供沉浸式社交体验，但用户面临严重的在线骚扰风险。现有安全措施多为被动响应，而主动检测方案通常依赖敏感的生物特征数据，引发隐私担忧。

Method: 提出HarassGuard系统，基于视觉语言模型（VLM），仅使用视觉输入检测社交VR中的物理骚扰。通过构建IRB批准的骚扰视觉数据集，应用提示工程，并微调VLM模型，考虑社交VR中的上下文信息来检测骚扰行为。

Result: 实验结果显示，HarassGuard在性能上达到与最先进基线模型（LSTM/CNN、Transformer）相当的水平，二元分类准确率达88.09%，多分类准确率达68.85%。值得注意的是，HarassGuard在使用更少的微调样本（200 vs. 1,115）的情况下达到这些基准，在上下文推理和隐私保护检测方面具有独特优势。

Conclusion: HarassGuard证明了仅使用视觉输入的VLM方法在社交VR骚扰检测中的有效性，提供了一种既能保护隐私又能实现前瞻性检测的解决方案，为社交VR平台的安全保障提供了新思路。
...
TL;DR: 本文提出了一种利用3D基础模型几何先验的无增强方法，通过深度估计获取像素级3D位置作为位置嵌入，并通过交叉注意力融合中间几何特征，以提升自动驾驶轨迹规划在相机视角变化下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶轨迹规划模型严重依赖训练时看到的相机视角，缺乏对视角变化的鲁棒性。本文旨在解决在相机视角变化下保持轨迹规划性能的问题。

Method: 采用无数据增强的方法，利用3D基础模型的几何先验。具体包括：1) 从深度估计中提取像素级3D位置作为位置嵌入；2) 通过交叉注意力机制融合中间几何特征。

Result: 在VR-Drive相机视角扰动基准测试中，该方法在大多数扰动条件下减少了性能下降，特别是在俯仰角和高度扰动方面有明显改进。但在纵向平移扰动下的增益较小。

Conclusion: 该方法通过几何先验注入提升了轨迹规划在相机视角变化下的鲁棒性，但纵向平移扰动的有限改进表明需要更视角无关的集成方法来实现全面的相机视角变化鲁棒性。
...
TL;DR: 本文提出了一种知识图谱增强的交叉Mamba交互框架（KG-CMI），用于医学视觉问答任务，通过整合专业医学知识来提升模型性能，并在多个数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉问答方法存在两个主要问题：1）未能充分利用领域特定的医学知识，难以准确关联医学图像中的病灶特征与关键诊断标准；2）基于分类的方法依赖预定义答案集，限制了处理自由形式答案的多样性，可能忽略答案中的详细语义信息。

Method: 提出KG-CMI框架，包含四个模块：1）细粒度跨模态特征对齐模块（FCFA）；2）知识图谱嵌入模块（KGE）；3）跨模态交互表示模块（CMIR）；4）自由形式答案增强多任务学习模块（FAMT）。该框架通过图结构有效整合专业医学知识，建立病灶特征与疾病知识之间的关联。

Result: 在三个医学视觉问答数据集（VQA-RAD、SLAKE和OVQA）上的实验结果表明，KG-CMI优于现有的最先进方法。此外，通过可解释性实验进一步验证了框架的有效性。

Conclusion: KG-CMI框架通过整合专业医学知识和处理自由形式答案的能力，显著提升了医学视觉问答任务的性能，为临床决策支持和远程医疗提供了更有效的解决方案。
...
### [42] [Fluently Lying: Adversarial Robustness Can Be Substrate-Dependent](https://arxiv.org/abs/2604.00605)
*Daye Kang,Hyeongboo Baek*

Main category: cs.CV

TL;DR: 研究发现对抗攻击下物体检测器的检测数量与精度可能解耦，在EMS-YOLO模型中出现"质量腐蚀"现象：检测数量保持70%以上而mAP从0.528暴跌至0.042


<details>
  <summary>Details</summary>
Motivation: 传统对抗攻击防御工具假设检测精度下降时检测数量会同步下降，但这一假设未经实证检验。研究旨在验证这一假设并探索对抗攻击失效模式是否与模型架构相关

Method: 在四种SNN架构和两种威胁模型（l-infinity和l-2）下进行对抗攻击测试，使用标准PGD攻击，评估检测数量与mAP的变化关系，并测试五种标准防御组件对质量腐蚀的检测和缓解效果

Result: 仅在EMS-YOLO模型中观察到质量腐蚀现象，其他三种SNN检测器未出现。所有五种标准防御组件均未能检测或缓解质量腐蚀，表明防御生态系统可能基于单一模型架构的假设

Conclusion: 对抗攻击失效模式具有模型架构依赖性，传统防御假设在特定模型上可能失效，需要重新评估对抗攻击防御工具的设计基础
...
TL;DR: TALENT框架通过目标感知高效调优解决指代图像分割中的非目标激活问题，使用校正成本聚合器和目标感知学习机制提升分割精度


<details>
  <summary>Details</summary>
Motivation: 现有参数高效调优方法在指代图像分割中存在非目标激活问题，即视觉特征会激活与文本指代目标同类别但不相关的对象，而非强调目标实例本身

Method: 提出TALENT框架：1) 校正成本聚合器高效聚合文本指代特征；2) 目标感知学习机制包括上下文成对一致性学习和目标中心对比学习，前者通过句子级文本特征构建文本指代亲和图优化视觉特征的语义关联，后者增强目标定位并抑制无关关联

Result: 在多个指标上优于现有方法，如在G-Ref验证集上获得2.5%的mIoU提升

Conclusion: TALENT框架通过目标感知高效调优有效解决了非目标激活问题，提升了指代图像分割的性能
...
TL;DR: 提出DirectFisheye-GS方法，将鱼眼相机模型直接集成到3D高斯泼溅框架中，避免预处理带来的信息损失和细节稀释问题，并通过特征重叠驱动的跨视图联合优化策略解决边缘浮动物体问题。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS在处理鱼眼相机输入时需要先进行图像去畸变预处理，但这会导致两个问题：1) 图像边缘出现黑边，造成信息损失并抵消鱼眼相机大视场的优势；2) 去畸变的重采样过程会稀释像素细节密度，导致3DGS过度拟合低频区域，产生模糊和浮动物体。

Method: 1) 将鱼眼相机模型直接集成到原始3DGS框架中，支持原生鱼眼图像输入训练；2) 提出特征重叠驱动的跨视图联合优化策略，建立跨视图的一致几何和光度约束，解决因畸变增大和随机选择视图优化导致的极端形状问题。

Result: DirectFisheye-GS在公共数据集上达到或超越了最先进方法的性能，有效解决了鱼眼相机输入时的预处理问题和边缘浮动物体问题。

Conclusion: 通过将鱼眼相机模型直接集成到3DGS框架并引入跨视图联合优化策略，实现了对鱼眼相机输入的高质量3D场景重建，避免了预处理带来的信息损失和细节稀释问题，该方法同样适用于现有的针孔相机管道。
...
TL;DR: 研究发现AI在皮肤镜图像诊断中的系统性误分类并非算法偏差所致，而是源于图像本身的视觉模糊性，人类专家在这些困难图像上的诊断表现同样严重下降


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI算法与人类专家的性能对比，本研究从新视角探究皮肤镜图像的内在复杂性，旨在确定AI系统性误分类的根本原因——是算法偏差还是图像本身的视觉模糊性

Method: 使用多种CNN架构进行实验，识别出所有模型都系统性误分类的图像子集；然后让专家皮肤科医生独立评估这些困难案例和对照组，比较人类诊断表现和专家间一致性

Result: AI系统性误分类的图像同样导致人类专家诊断性能崩溃：与真实标签的一致性从kappa 0.61降至0.08；专家间一致性从中等（kappa 0.456）降至微弱（kappa 0.275）；图像质量是导致这种双重系统性失败的主要因素

Conclusion: AI在皮肤镜诊断中的系统性误分类反映了图像本身的固有视觉模糊性而非算法偏差，这挑战了AI与人类专家性能对比的简单范式，强调了图像质量评估在临床AI部署中的重要性
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准测试存在两个主要问题：1) 许多仍基于未经过大规模预训练的模型；2) 通常将单个数据集分割为子任务，导致任务冗余度高，对预训练视频大语言模型的遗忘效应不明显。需要设计一个能有效暴露灾难性遗忘的基准测试。

Method: 提出了CL-VISTA基准测试，包含8个涵盖感知、理解和推理的多样化任务，这些任务能产生显著的分布偏移。建立了包含6种评估协议的全面评估框架，覆盖性能、计算效率和内存占用三个关键维度，其中性能维度还包括通用视频理解评估。

Result: 对10种主流持续学习方法的广泛基准测试揭示了一个基本权衡：没有任何单一方法在所有维度上都表现优越。成功缓解灾难性遗忘的方法往往会在泛化能力、计算开销或内存占用方面做出妥协。

Conclusion: CL-VISTA为推进多模态基础模型的持续学习提供了关键见解，揭示了持续学习方法在性能、效率和内存之间的基本权衡关系，有助于指导未来研究。
...
TL;DR: MoonAnything是一个基于真实月球地形构建的统一基准数据集，提供几何和光度监督，包含LunarGeo（几何数据）和LunarPhoto（光度数据）两个子数据集，共13万+样本，支持月球感知算法开发。


<details>
  <summary>Details</summary>
Motivation: 现有月球数据集通常缺乏几何地面真值、光度真实性、光照多样性或大规模覆盖，这阻碍了基于学习的感知系统开发。需要提供全面的几何和光度监督数据集来支持月球探索任务。

Method: 基于真实月球地形，使用基于物理的渲染技术构建统一基准。包含两个互补子数据集：LunarGeo提供立体图像、密集深度图和相机标定；LunarPhoto使用空间变化的BRDF模型生成光度真实图像，并在真实太阳配置下进行多光照渲染。

Result: 创建了包含超过13万个样本的综合数据集，提供全面的几何和光度监督。建立了使用最先进方法的基线，并发布了完整数据集和生成工具供社区扩展使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一分割方法通常依赖异构任务和模态间的共享编码器，这会导致特征纠缠、梯度干扰和病灶判别能力不足。需要构建一个能高效处理多种医学病灶分割任务的统一模型。
...
TL;DR: TTA-Vid：一种基于测试时强化学习的视频推理方法，无需大规模标注数据和多阶段训练，通过测试时自适应和自适应帧选择在推理时更新模型


<details>
  <summary>Details</summary>
Motivation: 现有视频推理模型依赖大规模监督数据和复杂训练流程，成本高昂且难以适应新领域。需要一种能在测试时自适应、无需标注数据的方法来提升视频理解能力。

Method: 提出TTA-Vid方法，包含两个同时工作的组件：1) 测试时自适应，在推理时对多个帧子集进行逐步推理，使用跨不同帧子集的批量感知频率奖励作为伪真值更新模型；2) 多臂老虎机策略进行自适应帧选择，学习优先选择信息丰富的帧。

Result: 该方法在单个批次甚至单个样本上训练后，能够在测试时泛化到整个数据集甚至跨数据集。在多个视频推理任务上表现一致提升，优于当前基于大规模数据训练的SOTA方法。

Conclusion: TTA-Vid展示了测试时强化学习在时序多模态理解中的潜力，无需真实标注或专用训练分割，通过测试时自适应实现高效视频推理。
...
TL;DR: 提出一种基于注意力机制对偶形式的无训练token剪枝框架，通过选择最优的rank-1更新子集来近似原始注意力权重矩阵，在保持性能的同时提升计算效率。
...
Method: 从注意力对偶形式视角出发，将注意力重新表述为隐式线性层，其权重矩阵是单个token键值对生成的rank-1外积之和。提出新度量量化token的信息量和信息重复度，并引入渐进分块最大边际相关性算法高效选择最优子集。

Result: 大量实验表明，该方法在性能和效率之间实现了更好的权衡，同时为现有剪枝方法提供了新的理论视角。

Conclusion: 基于注意力对偶形式的无训练token剪枝框架有效解决了LVLMs计算成本问题，通过理论驱动的token选择策略实现了更好的性能-效率平衡。
...
Method: 构建PrivHAR-Bench数据集，包含1,932个源视频，涵盖15个具有人体关节多样性的活动类别。每个视频应用9个隐私强度递增的视觉隐私变换（从轻量级空间模糊到密码学块置换），并提供背景移除变体以分离人体运动特征与场景上下文偏差。

Result: 使用R3D-18模型验证显示，随着隐私层级增加，识别准确率呈现可测量且可解释的下降曲线：层级内准确率从88.8%（清晰）降至53.5%（加密+背景移除），跨域准确率降至4.8%。

Conclusion: PrivHAR-Bench为在标准化条件下比较隐私保护HAR方法提供了一个受控基准，包含数据集、生成管道和评估代码，已公开可用。
...
TL;DR: 本文提出SurgSTU-Pipeline，一种确定性生成管道，用于创建具有精细时空关系的手术视频数据集，以解决现有数据集缺乏复杂时空动态捕捉的问题。


<details>
  <summary>Details</summary>
Motivation: 手术视频理解是计算机辅助手术发展的关键前提。现有手术视觉语言数据集在捕捉和评估复杂的交错时空动态方面存在不足，而大规模数据集的创建面临手动标注成本高或使用大语言模型生成易出错的问题。

Method: 提出SurgSTU-Pipeline，一种具有时空连续性过滤的确定性生成管道，能够可靠地创建用于精细时空多模态理解的手术数据集。将该管道应用于公开可用的手术数据集，创建了包含7515个视频片段和15万个精细时空问答样本的SurgSTU数据集。

Result: 综合评估表明，最先进的通用视觉语言模型在零样本设置下表现不佳，但通过上下文学习可以改善其时空能力。在SurgSTU训练数据集上微调的视觉语言模型在所有时空任务中取得了最高性能，验证了该数据集在改善手术视频时空理解方面的有效性。

Conclusion: SurgSTU-Pipeline能够可靠地生成用于精细时空多模态理解的手术数据集，SurgSTU数据集有效提升了视觉语言模型对手术视频时空动态的理解能力，为计算机辅助手术的发展提供了重要支持。
...
TL;DR: 多模态大语言模型在跨视图3D几何推理方面存在困难，论文提出通过生成空间不一致的图像对来评估模型对3D运动一致性的理解能力。


<details>
  <summary>Details</summary>
Motivation: 空间一致性是视觉世界的基本属性，但当前的多模态大语言模型在跨视图3D几何推理方面表现不佳，需要更系统的方法来评估模型对物理现实的理解能力。

Method: 提出一种简单可扩展的方法，从多视角场景中生成真实但空间不一致的图像对，通过让模型识别违反3D运动一致性的物体来系统评估其能力。

Result: 最先进的多模态大语言模型在3D运动一致性任务上显著落后于人类观察者，且在不同场景属性上表现出很大变异性，显示出对3D结构的脆弱和不完整理解。

Conclusion: 研究结果强调了需要开发对物理世界有更深入基础理解的方法，当前模型在3D几何推理方面仍有很大改进空间。
...
TL;DR: 该论文重新审视了人在回路对象检索任务，利用预训练的ViT表示，在复杂的多对象图像中通过主动学习和用户反馈迭代检索目标对象类别。


<details>
  <summary>Details</summary>
Motivation: 在多对象数据集中，目标对象可能只占据图像的小区域且场景复杂混乱，全局描述符往往不足，需要更适应的局部描述符来有效检索对象类别。

Method: 将检索任务构建为二元分类问题，通过主动学习循环迭代学习：系统选择信息丰富的样本供用户标注，利用预训练的ViT表示，探索不同的表示策略来平衡全局上下文和局部细节。

Result: 比较了多对象数据集上的多种表示策略，揭示了捕捉全局上下文与关注细粒度局部对象细节之间的权衡，为基于主动学习的交互式检索管道设计提供了实用见解。

Conclusion: 通过系统性地解决关键设计问题（如考虑哪些对象实例、标注形式、主动选择策略和表示策略），为多对象环境中基于主动学习的高效交互式对象类别检索提供了框架和实用指导。
...
TL;DR: 提出Vision-Geometry-Action (VGA)自动驾驶新范式，强调密集3D几何作为关键线索，并开发了在线推理的DVGT-2模型，同时输出几何重建和轨迹规划。
...
Conclusion: VGA范式通过密集3D几何作为自动驾驶的关键线索是有效的，DVGT-2实现了高效的在线几何重建和规划，展示了该范式的实用性和优越性。
...
TL;DR: 本文提出了CLeaRS基准测试，用于评估遥感视觉语言模型的持续学习能力，发现现有模型存在灾难性遗忘问题，且现有持续学习方法效果有限。
...
Method: 构建了CLeaRS基准测试，包含10个精选子集、超过20.7万张图像-文本对，涵盖多样化解释任务、传感模态和应用场景，并定义了三种评估协议。

Result: 广泛基准测试显示所有设置下都存在灾难性遗忘，现有持续学习方法在适应遥感视觉语言模型时，处理任务、指令和模态转换的效果有限。

Conclusion: 遥感视觉语言模型需要专门针对其特点开发的持续学习方法，现有方法不足以应对任务、指令和模态的持续变化。
...
TL;DR: LinguDistill：一种无需额外适配器的蒸馏方法，通过使用原始冻结语言模型作为教师来恢复视觉语言模型的语言能力，解决了多模态适应导致的语言能力退化问题。
...
Method: 提出LinguDistill方法：1) 使用原始冻结语言模型作为教师；2) 引入层级的KV缓存共享，让教师能够接触到学生的多模态表示；3) 在语言密集型数据上选择性地蒸馏教师的强语言信号，同时保留学生在多模态任务上的视觉基础能力。

Result: LinguDistill恢复了语言和知识基准测试中约10%的性能损失，同时在视觉密集型任务上保持了可比的性能。该方法无需额外模块，为多模态模型中的模态特定退化问题提供了高效实用的解决方案。

Conclusion: 研究表明，无需额外模块即可恢复语言能力，这为解决多模态模型中模态特定退化问题提供了高效实用的解决方案。层级的KV缓存共享是实现这一目标的关键技术。
...
TL;DR: DisCo框架通过解耦和再耦合视觉与文本信息，解决主题驱动文本到图像生成中的"相似性-可控性悖论"，实现高保真主题保持和精确文本控制。


<details>
  <summary>Details</summary>
Motivation: 主题驱动文本到图像生成面临"相似性-可控性悖论"：增强文本控制会降低主题保真度，反之亦然。作者认为这一悖论源于文本提示的模糊角色，它既要描述主题又要描述修改要求，导致模型接收冲突信号。

Method: 提出DisCo框架：1) 文本-视觉解耦模块：从参考图像中提取主题身份（使用实体词），简化文本提示仅包含修改命令（使用通用代词）；2) 设计专用奖励信号，通过强化学习无缝再耦合视觉定义的主题和文本生成的上下文。

Result: 实验表明该方法实现了最先进的性能，能够同时实现高保真主题保持和精确文本控制，生成高度逼真和连贯的图像。

Conclusion: DisCo框架通过解耦和再耦合视觉与文本信息，有效解决了主题驱动T2I生成中的相似性-可控性悖论，为同时实现高保真主题保持和精确文本控制提供了有效解决方案。
...
TL;DR: MotionGrounder：首个基于DiT的多目标可控运动迁移框架，通过流式运动信号和对象-标题对齐损失实现多目标精细控制


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散变换器（DiT）的运动迁移方法仅限于单目标视频，无法处理现实场景中多目标的精细控制需求

Method: 提出MotionGrounder框架：1）流式运动信号（FMS）为目标视频生成提供稳定运动先验；2）对象-标题对齐损失（OCAL）将对象标题锚定到对应空间区域；3）提出新的对象锚定评分（OGS）联合评估空间对齐和语义一致性

Result: MotionGrounder在定量、定性和人工评估中均优于现有基线方法

Conclusion: MotionGrounder首次实现了基于DiT的多目标可控运动迁移，为现实场景视频生成提供了更精细的控制能力
...
<details>
  <summary>Details</summary>
Motivation: 点云运动捕捉面临表达能力和鲁棒性的权衡：点基方法几何细节丰富但噪声敏感，骨架基方法鲁棒但过度简化。需要构建既能表达丰富几何信息又能抵抗噪声的有效表示。
...
Conclusion: 通过结构化表示和显式分解，SparkleMotion成功平衡了运动捕捉的表达能力和鲁棒性，为解决点云运动捕捉中的核心挑战提供了有效方案。
...
Method: 提出基于高斯过程混合模型的对象特定功能形状表示，通过在战略参考点锚定局部高斯过程先验来捕捉复杂拓扑结构，可以使用任何结构分解方法（如骨架化、基于距离的聚类）灵活提取

Result: 在ShapeNetCore和IndustryShapes数据集上的广泛评估表明，该方法能够高效准确地表示复杂几何形状

Conclusion: 该方法提供了一种轻量级、高效的功能形状表示方案，相比传统显式表示和计算密集型神经架构具有优势
...
TL;DR: 该研究提出了一种基于显式4D表示的时空推理框架，将2D多模态大语言模型与3D计算机视觉模型结合，无需额外训练即可为手术AI系统提供时空智能。
...
Method: 利用点跟踪、深度估计和分割模型构建具有时空一致性的工具和组织语义的连贯4D模型，然后让多模态大语言模型基于显式4D表示派生的工具（如轨迹）进行推理，无需微调。

Result: 在包含134个临床相关问题的新数据集上评估，发现通用推理主干与4D表示的结合显著改善了时空理解能力，并实现了4D基础。

Conclusion: 时空智能可以从2D多模态大语言模型和3D计算机视觉模型中"组装"而来，无需额外训练，为智能辅助系统和自主机器人手术开辟了新途径。
...
TL;DR: 提出一种基于典型相关分析(CCA)的训练免费方法，通过利用两个预训练图像编码器表示之间的共享结构，实现表示选择和降维，保留共享语义内容并丢弃冗余维度。


<details>
  <summary>Details</summary>
Motivation: 现代视觉流水线依赖预训练图像编码器，但这些表示通常是过完备且模型特定的，需要提高表示效率。

Method: 使用后处理的典型相关分析(CCA)算子，利用两个预训练图像编码器产生的表示之间的共享结构，找到线性投影作为表示选择和降维的原则性方法。

Result: 表示维度可减少75%以上且下游性能提升，或在固定维度下通过从更大或微调模型进行后处理表示传输来增强表示。在ImageNet-1k、CIFAR-100、MNIST等基准测试中，相比基线和PCA投影表示，准确率提升最高达12.6%。

Conclusion: 提出的基于CCA的训练免费方法能有效提高图像表示效率，通过跨模型一致性指导表示蒸馏和精炼，在保持语义内容的同时减少冗余维度。
...
TL;DR: 提出一种对抗性衰减补丁(AAP)方法，通过能量约束优化和衰减部署框架，在SAR目标检测攻击中平衡攻击效果与隐蔽性，并具有物理实现潜力。


<details>
  <summary>Details</summary>
Motivation: 现有SAR攻击方法虽然有效，但通常引入明显扰动且局限于数字域，忽略了攻击SAR系统的物理实现约束，需要开发更隐蔽且物理可实现的攻击策略。

Method: 提出对抗性衰减补丁(AAP)方法，采用能量约束优化策略结合衰减部署框架，通过信号级电子干扰机制实现物理攻击。

Result: AAP能有效降低检测性能同时保持高隐蔽性，在不同模型间具有良好的可迁移性，为SAR目标检测系统攻击提供了物理基础视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DreamBooth、LoRA等个性化文本到图像扩散模型虽然能生成高质量头像，但生成的内容在社交媒体上分享时，仍可通过人脸识别系统链接到真实用户身份，造成隐私泄露风险。现有防御方法主要针对未经授权的个性化，无法解决授权个性化后生成内容仍泄露身份信息的问题。

Method: 提出IDDM（Identity-Decoupled personalized Diffusion Models）模型端防御方法，将身份解耦集成到个性化流程中。采用交替优化程序，在短期个性化更新和身份解耦数据优化之间交替进行，使用两阶段调度来平衡身份可链接性抑制和生成效用。

Result: 在多个数据集、多样化提示词和最先进的人脸识别系统上的广泛实验表明，IDDM能够持续降低身份可链接性，同时保持高质量的个性化生成效果。

Conclusion: IDDM为个性化文本到图像生成提供了有效的隐私保护解决方案，通过模型端输出免疫技术，在授权个性化场景下保护用户身份隐私，实现了隐私与效用的可调平衡。
...
TL;DR: JAMMEval是一个经过系统精炼的日语视觉问答基准数据集，通过两轮人工标注改进了7个现有日语基准数据集的质量，为日语VLM评估提供了更可靠的基准。


<details>
  <summary>Details</summary>
Motivation: 日语VQA基准相比英语基准缺乏迭代精炼，现有基准存在模糊问题、错误答案、无需视觉基础即可解决等问题，这影响了评估的可靠性，导致模型比较时得出误导性结论。

Method: 通过两轮人工标注系统性地精炼7个现有日语基准数据集，构建JAMMEval数据集，提高数据质量和评估可靠性。

Result: 在JAMMEval上评估开源和专有VLMs，分析近期模型在日语VQA上的能力。精炼后的基准能更好地反映模型能力，降低运行间方差，提高区分不同能力水平模型的能力。

Conclusion: JAMMEval为日语VLM评估提供了更可靠的基准，通过系统精炼现有数据集解决了评估可靠性问题，并发布了数据集和代码以推动VLM的可靠评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有自我中心模拟器存在两个主要问题：1) 缺乏明确的3D基础，导致视角变化时出现结构漂移；2) 将场景视为静态，无法在多阶段交互中更新世界状态。需要一种能够持续更新3D场景状态的模拟器。
...
TL;DR: ProCap是一个解决空间增强现实中虚拟投影内容与物理场景语义混淆问题的框架，通过两阶段管道解耦投影内容与物理场景，并创建了首个大规模SAR语义基准数据集RGBP。


<details>
  <summary>Details</summary>
Motivation: 空间增强现实（SAR）使用投影仪将数字内容直接投影到物理场景上，但为了支持智能交互（如场景推理或回答用户查询），需要语义上区分物理场景和投影内容。标准的视觉语言模型在处理这种虚拟-物理模糊性时经常混淆两者。

Method: ProCap采用两阶段管道：首先通过自动分割在视觉上隔离虚拟和物理层；然后使用区域感知检索来避免因投影变形导致的模糊语义上下文。同时创建了RGBP数据集（包含65个多样化物理场景和超过180,000个投影），并建立了使用任务特定令牌的双重描述评估协议。

Result: 实验表明ProCap为未来SAR研究提供了稳健的语义基础。该框架、预训练模型和RGBP数据集已在项目页面上公开。

Conclusion: ProCap通过明确解耦投影内容与物理场景，解决了SAR中的虚拟-物理语义模糊问题，为空间增强现实的智能交互能力奠定了基础。
...
TL;DR: IKEA-Bench是一个评估视觉语言模型在组装任务中跨描绘对齐能力的基准，发现视觉编码是提高跨描绘鲁棒性的主要瓶颈


<details>
  <summary>Details</summary>
Motivation: 2D组装图通常抽象且难以理解，需要智能助手来监控进度、检测错误并提供逐步指导。在混合现实环境中，系统必须从摄像头画面中识别已完成和进行中的步骤，并与图解说明对齐。视觉语言模型在此任务中显示出潜力，但面临描绘差距问题，因为组装图和视频帧共享的视觉特征很少。

Method: 构建IKEA-Bench基准，包含29个IKEA家具产品的1,623个问题，涵盖6种任务类型。评估19个视觉语言模型（2B-38B参数），采用三种对齐策略。进行三级机制分析，研究图解和视频在ViT子空间中的表示。
...
Conclusion: 视觉编码是提高跨描绘鲁棒性的主要目标。图解和视频之间的描绘差距是视觉语言模型在组装任务中的核心挑战，需要改进视觉表示学习来弥合这一差距。
...
TL;DR: EmoScene是一个大规模双空间情感数据集，联合编码情感维度和感知属性，用于提升文本到图像扩散模型在情感控制方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型缺乏统一表示情感和感知因素的能力，限制了合成具有连贯细腻情感意图场景的能力。人类视觉情感源于上下文意义（效价、唤醒度、优势度）与感知线索（色彩和谐、亮度对比、纹理变化、曲率、空间布局）的快速整合。

Method: 构建EmoScene数据集，包含120万张图像，覆盖300多个真实世界场景类别，每张图像标注离散情感标签、连续VAD值、感知描述符和文本描述。通过浅层交叉注意力调制将双空间控制注入冻结的扩散模型主干。

Result: 多空间分析揭示了离散情感在VAD空间中的分布，以及情感如何系统性地与场景级感知因素相关。提供了一个轻量级参考基线，作为双空间监督下情感可控性的可复现探针。

Conclusion: EmoScene通过联合编码情感维度和感知属性，为文本到图像模型提供了更精确的情感控制能力，解决了当前模型在情感和感知因素统一表示方面的局限性。
...
TL;DR: 发布YieldSAT数据集，这是一个大规模、高质量、多模态的作物产量预测数据集，覆盖多个国家和作物类型，包含超过1200万产量样本和11.3万张卫星图像，用于高分辨率像素回归任务。


<details>
  <summary>Details</summary>
Motivation: 作物产量预测需要大量数据来训练可扩展模型，但现有数据集存在获取成本高、数据质量不均、隐私法规限制等问题，导致数据集稀缺、质量低或局限于特定区域/作物类型，阻碍了数据驱动解决方案的发展。

Method: 构建YieldSAT数据集，涵盖阿根廷、巴西、乌拉圭和德国的多种气候区，包含玉米、油菜、大豆和小麦等主要作物类型，覆盖2,173个专家标注的农田。每个产量样本空间分辨率为10米，配有多光谱卫星图像（共113,555张）和辅助环境数据。将大规模高分辨率作物产量预测作为像素回归任务，比较多种深度学习模型和数据融合架构，并探索领域知识引导的深度集成方法以应对真实世界中的严重分布偏移。

Result: 创建了一个包含超过1220万产量样本的大规模高质量数据集，每个样本具有10米空间分辨率。通过比较不同深度学习模型，展示了大规模高分辨率作物产量预测的潜力。提出的领域知识引导深度集成方法在应对真实世界分布偏移时表现出显著的性能提升。

Conclusion: YieldSAT数据集为作物产量预测研究提供了重要的数据资源，支持大规模高分辨率像素回归任务。研究揭示了真实世界条件下地面真值数据的严重分布偏移挑战，并展示了领域知识引导的深度集成方法在缓解这一问题上的有效性。数据集已公开可用。
...
TL;DR: 本文提出GIFD方法，通过优化GAN中间层特征而非仅潜在空间，显著提升联邦学习中梯度反演攻击的效果，支持分布外场景并解决标签不一致问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然通过传输梯度保护隐私，但研究发现梯度仍可能被反演攻击泄露数据。现有方法仅在GAN潜在空间进行优化，表达能力有限且泛化性不足。

Method: 提出GIFD方法：1）分解GAN模型，在中间层进行层次化特征搜索；2）设计正则化器避免生成不真实图像；3）扩展到分布外场景；4）针对标签不一致问题提出标签映射技术。

Result: 实验表明GIFD能实现像素级重建，在多种联邦学习场景下优于现有基线方法，包括分布外和标签不一致的挑战性场景。

Conclusion: GIFD通过优化GAN中间层特征而非仅潜在空间，显著提升了梯度反演攻击的效果和泛化能力，揭示了联邦学习隐私保护的潜在风险。
...
TL;DR: DLWM是一种用于自动驾驶的双潜在世界模型范式，通过两阶段训练实现基于3D高斯表示的全面预训练，显著提升了3D占用感知、4D占用预测和运动规划性能。
...
Conclusion: DLWM作为一种新颖的双潜在世界模型范式，成功实现了自动驾驶中基于高斯表示的全面预训练，为3D占用感知、4D占用预测和运动规划任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有缓解幻觉的方法主要依赖静态的单步状态，忽视了生成过程中的动态上下文变化，难以纠正信息损失问题。

Method: 提出自适应上下文整合(ACT)方法：1) 视觉上下文探索 - 利用时空分析自适应放大负责视觉探索的注意力头；2) 语义上下文聚合 - 边缘化潜在语义查询以有效聚合视觉证据。

Result: 在多种LVLM上的实验表明，ACT显著减少了幻觉，在判别性和生成性基准测试中都取得了有竞争力的结果。
...
TL;DR: 该研究提出了一种仅使用T2加权MRI进行前列腺癌定位的新方法，通过将DWI序列作为训练时的潜在模态，在推理时仅需T2图像即可实现与多序列方法相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 多参数MRI需要DWI和T2w序列，增加了成本和专业知识要求。虽然早期仅使用T2w图像的方法在分割放射科医生标注的病灶方面表现良好，但本研究旨在解决更具挑战性的问题：仅使用T2w图像在推理时定位基于独立组织病理学标签的个体癌症。

Method: 提出期望最大化算法框架：将DWI图像作为训练时可用的潜在模态，仅使用T2w图像作为输入来分类Barzell分区的癌症存在。E步使用基于流匹配的生成模型（潜在模态生成器）近似潜在DWI图像的后验分布；M步同时优化癌症定位器和生成模型，以最大化癌症存在的期望似然。

Result: 提出的仅T2方法在癌症定位性能上优于缺乏训练DWI图像的方法或现有特权学习框架，甚至优于使用多个输入序列的基线方法：患者级F1分数提高14.4%，分区级QWK提高5.3%。在4,133名前列腺癌患者的内外部数据集上进行了定量评估。

Conclusion: 该研究为从特权DWI模态学习提供了新颖的理论框架，实现了仅使用T2w图像进行前列腺癌定位的优越性能，具有降低临床成本和专业要求的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 地滚波是陆地和垂直地震剖面数据中的主要相干噪声，严重掩盖反射事件并降低后续成像和解释质量。传统衰减方法（包括变换域滤波、稀疏表示和深度学习）存在适应性有限、信号泄漏或依赖标记训练数据等问题，特别是在强信号噪声重叠情况下。

Method: 提出训练免费框架，将地滚波衰减重新定义为语义引导的信号分离问题。使用可提示大视觉模型通过将地震道集转换为视觉表示，并通过文本或图像提示定位地滚波主导区域来提取高级语义先验。将语义响应转换为连续软掩码，嵌入到掩码条件低秩逆公式中，实现空间自适应抑制和反射保护重建。开发基于交替方向乘子法的高效求解器。

Result: 在合成和实际VSP数据集上的广泛实验表明，该方法实现了优异的地滚波衰减效果，同时保持了反射连续性和波形保真度，一致优于代表性的变换域滤波和隐式神经表示方法。

Conclusion: 该方法提供了一种无需任务特定训练或手动标注的稳定且物理一致的信号恢复方案，通过语义引导的信号分离框架有效解决了传统地滚波衰减方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于对抗训练的防御方法计算成本高且难以泛化到未见过的攻击类型，需要一种更通用、高效的防御方案。

Method: 提出PDA框架，在推理时进行：1)提示改写生成多个变体；2)问题分解为子问题；3)一致性聚合多个回答。通过不变式实现效率与鲁棒性的平衡。

Result: 在多个VLM架构和视觉问答、分类、描述任务上，PDA对各种对抗扰动都获得一致的鲁棒性提升，同时保持有竞争力的干净准确率。

Conclusion: PDA建立了通用、强大且实用的VLM推理时防御框架，无需修改底层模型，实现了鲁棒性与效率的良好平衡。
...
Method: 1. 提出密集点轨迹作为行为视觉标记，分离运动与外观；2. 设计扩散变换器建模无序轨迹集，显式处理遮挡；3. 构建300小时无约束动物视频数据集

Result: 轨迹标记预测实现类别无关、数据高效的预测，超越现有基准方法，能泛化到稀有物种和形态
...
TL;DR: Diff3R是一个新颖框架，通过将可微分的3DGS优化层集成到训练循环中，桥接了前馈预测和测试时优化，学习预测最优初始化而非传统零次结果。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅技术存在两种主要方向：前馈模型在稀疏视图设置下推理速度快，但质量有限；逐场景优化质量高但计算成本昂贵。需要结合两者的优势。

Method: 1. 引入可微分3DGS优化层到训练循环中；2. 使用隐函数定理和可扩展的矩阵自由PCG求解器计算梯度；3. 在优化过程中集成数据驱动的不确定性模型，自适应控制参数变化。

Result: 该方法可以无缝集成到现有的前馈3DGS架构中，适用于有姿态和无姿态方法，显著提升了测试时优化的性能，有效缓解了欠约束区域的过拟合问题，并增加了对输入异常值的鲁棒性。

Conclusion: Diff3R成功桥接了前馈预测和测试时优化，通过预测最优初始化而非零次结果，实现了高质量渲染与计算效率的良好平衡，为3D重建领域提供了新的解决方案。
...
TL;DR: 首次使用开源流程从Chandrayaan-2 OHRC多视角图像生成亚米级月球数字高程模型，垂直精度5.85米，水平精度小于30厘米


<details>
  <summary>Details</summary>
Motivation: 高分辨率月球数字高程模型对表面移动规划、着陆点特征描述和行星科学至关重要。Chandrayaan-2的OHRC相机具有目前月球轨道成像中最佳的地面采样能力（20-30厘米/像素），但需要开发方法从其多视角图像生成亚米级DEM

Method: 1. 通过几何分析图像元数据（基线高度比计算和会聚角估计）从未配对的OHRC档案中识别候选立体像对；2. 应用密集立体对应和射线三角测量生成点云；3. 将点云网格化为DEM；4. 通过迭代最近点算法与LRO NAC数字地形模型对齐建立绝对高程一致性；5. 进行恒定偏差偏移校正

Result: 在五个地理分布的月球站点生成了有效空间分辨率约24-54厘米的DEM，垂直RMSE为5.85米（原生OHRC分辨率），通过平面特征匹配评估的水平精度小于30厘米

Conclusion: 成功开发了完全开源的流程，首次从OHRC多视角图像生成了亚米级月球DEM，验证了OHRC数据在生成高精度地形产品方面的能力，为月球探测和科学研究提供了重要工具
...
TL;DR: ONE-SHOT是一个参数高效的人与环境组合视频生成框架，通过解耦人类动态和环境信号，实现细粒度独立编辑，无需繁重的3D预处理。


<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型在人类中心视频合成方面取得进展，但主体与场景的细粒度独立编辑仍是关键挑战。现有方法通过刚性3D几何组合实现环境控制时，面临精确控制与生成灵活性之间的权衡，且繁重的3D预处理限制了实际可扩展性。
...
Conclusion: ONE-SHOT框架通过解耦生成过程，实现了参数高效的人与环境组合视频生成，解决了精确控制与生成灵活性之间的权衡问题，具有实际可扩展性。
...
TL;DR: CROWD数据集是一个从YouTube公开视频中筛选和分割的城市行车记录仪片段数据集，专注于日常驾驶场景，排除事故和编辑内容，包含5万多个片段，覆盖全球238个国家和地区，提供时间、车辆类型标签及机器生成的物体检测数据。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏大规模、高质量、专注于日常驾驶场景的行车记录仪数据集，现有数据集往往包含事故、编辑内容或特定事件，限制了跨域鲁棒性和交互分析的研究。需要构建一个真实、连续、未编辑的日常驾驶数据集来支持计算机视觉和自动驾驶研究。

Method: 从YouTube公开视频中手动筛选和分割行车记录仪片段，排除事故、事故后果和编辑内容，专注于日常驾驶场景。数据集包含51,753个片段，覆盖全球7,103个地点，提供手动标注的时间（白天/夜晚）和车辆类型标签，并使用YOLOv11x和BoT-SORT生成机器检测和目标跟踪数据。

Result: 创建了CROWD数据集，包含51,753个片段（20,275.56小时，42,032个视频），覆盖全球238个国家和地区的7,103个地点。提供手动标注的时间、车辆类型标签，以及机器生成的80个MS-COCO类别的物体检测和目标跟踪数据。数据集以视频标识符和片段边界形式分发，确保研究可复现性。

Conclusion: CROWD数据集为计算机视觉和自动驾驶研究提供了大规模、高质量、全球覆盖的日常驾驶场景数据资源，支持跨域鲁棒性和交互分析研究，通过提供机器生成的检测数据降低了基准测试的门槛。
...
TL;DR: PHASOR是一个用于CT虚拟对比增强的体扩散框架，通过视频扩散模型处理CT体数据，结合解剖学路由专家混合和强度相位感知表示对齐模块，显著提升合成质量和增强准确性。
...
Method: PHASOR框架将CT体数据视为连贯序列，利用视频扩散模型增强结构一致性和体数据准确性。包含两个互补模块：解剖学路由专家混合模块将不同增强模式锚定到解剖语义，器官特定记忆捕获显著细节；强度相位感知表示对齐模块突出精细对比信号，减轻不完美空间对齐的影响。

Result: 在三个数据集上的广泛实验表明，PHASOR在合成质量和增强准确性方面显著优于现有最先进方法。

Conclusion: PHASOR通过创新的体扩散框架和专门设计的模块，有效解决了CT虚拟对比增强中的解剖异质性和空间对齐问题，为临床提供了一种高质量的非侵入性替代方案。
...
TL;DR: ProOOD是一个轻量级即插即用方法，通过原型引导的语义补全和尾部挖掘，结合无训练的OOD评分，提升3D语义占用预测的鲁棒性，有效处理长尾类别偏差和分布外输入问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D语义占用预测方法存在两个主要问题：1）对长尾类别偏差敏感，容易将异常过度自信地分配给稀有类别；2）对分布外（OOD）输入脆弱。这些问题在自动驾驶等安全关键应用中尤为严重。

Method: ProOOD包含三个核心组件：1）原型引导的语义补全，用类别一致的特征填充遮挡区域；2）原型引导的尾部挖掘，增强稀有类别表示以减少OOD吸收；3）EchoOOD，融合局部logit一致性与局部全局原型匹配，生成可靠的体素级OOD分数。

Result: 在五个数据集上的实验表明，ProOOD在3D占用预测和OOD检测方面均达到最先进性能：在SemanticKITTI上，整体mIoU提升+3.57%，尾部类别mIoU提升+24.80%；在VAA-KITTI上，AuPRCr提升+19.34个百分点。在所有基准测试中均获得一致改进。

Conclusion: ProOOD通过原型引导的细化和无训练OOD评分，显著提升了3D语义占用预测的校准性和OOD检测的可靠性，为安全关键的城市驾驶应用提供了更鲁棒的解决方案。代码已开源。
...
TL;DR: ReMoGen：用于实时交互到反应生成的模块化学习框架，利用通用运动先验和元交互模块，在数据稀缺和异构监督下实现鲁棒泛化，支持在线交互的响应式运动生成。
...
Method: 提出ReMoGen（反应运动生成）框架，包含两个核心组件：1）从大规模单人运动数据集中学习的通用运动先验；2）独立训练的元交互模块，将先验适应到目标交互领域。为支持在线交互，采用分段级生成和轻量级帧级分段细化模块，结合新观察到的线索，提高响应性和时间连贯性。

Result: 在人-人、人-场景和混合模态交互设置下的广泛实验表明，ReMoGen能够生成高质量、连贯且响应迅速的反应，同时在多样化的交互场景中有效泛化。

Conclusion: ReMoGen通过模块化设计解决了交互到反应生成中的关键挑战，利用通用运动先验和领域适应策略，在数据稀缺情况下实现鲁棒泛化，并通过分段细化机制支持实时在线交互，为虚拟化身、动画和人机协作等应用提供了有效的解决方案。
...
Method: 提出原型引导文本提示选择（ProTPS）方法，学习类特定的视觉原型和文本提示，使用视觉原型指导每个类的文本提示选择和学习，增加训练灵活性以鼓励学习独特文本提示。

Result: 在类增量（CI）和跨数据集持续学习（CDC）设置中，ProTPS达到接近上限的性能；在真实海洋物种数据集Marine112（包含112个物种，收集时间跨度6年）的类域增量（CDI）学习设置中，ProTPS在三种设置下均优于现有最先进方法。

Conclusion: ProTPS通过视觉原型指导文本提示学习，有效解决了持续学习中文本提示独特性的挑战，在多个设置下表现出色，并为社区提供了具有挑战性的真实世界数据集Marine112。
...
TL;DR: ReinDriveGen是一个可控的动态驾驶场景编辑框架，允许用户自由编辑参与者轨迹以模拟安全关键场景，并通过RL后训练策略提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以实现对动态驾驶场景的完全可控编辑，特别是在模拟安全关键场景（如车辆碰撞、行人乱穿马路等）时，编辑后的场景往往超出训练分布，导致生成质量下降。

Method: 1) 从多帧LiDAR数据构建动态3D点云场景；2) 引入车辆补全模块从部分观测重建完整360°几何；3) 将编辑后的场景渲染为2D条件图像，指导视频扩散模型合成真实驾驶视频；4) 提出基于RL的后训练策略，使用成对偏好模型和成对奖励机制，在无真实监督的情况下提升分布外条件下的生成质量。

Result: 在编辑的驾驶场景上优于现有方法，并在新颖的自我视角合成上取得了最先进的结果。

Conclusion: ReinDriveGen实现了对动态驾驶场景的完全可控编辑，能够有效模拟安全关键场景，并通过RL后训练策略解决了分布外条件下的生成质量问题。
...
TL;DR: 该论文提出了Open3D-AD方法，用于开放集监督的3D异常检测，利用正常样本、模拟异常和部分真实异常来建模正常和异常数据的概率密度分布，并在新构建的Open-Industry数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在实际工业制造场景中，虽然获取高精度点云数据成本较高，但收集少量异常样本是可行的。因此，研究开放集监督的3D异常检测，即仅用正常样本和少量已知异常样本训练模型，以在测试时识别未知异常。

Method: 1. 提出Open3D-AD方法，专门针对点云数据，利用正常样本、模拟异常和部分观察到的真实异常来建模正常和异常数据的概率密度分布；2. 引入简单的对应分布子采样技术，减少正常和非正常分布之间的重叠，实现更强的双分布建模；3. 构建Open-Industry高质量工业数据集，包含15个类别，每个类别有5种真实异常类型。

Result: 在Open-Industry数据集以及Real3D-AD和Anomaly-ShapeNet等现有数据集上建立了全面的基准测试。基准结果和消融研究证明了Open3D-AD的有效性，并进一步揭示了开放集监督3D异常检测的潜力。

Conclusion: 该研究展示了开放集监督3D异常检测的可行性，提出的Open3D-AD方法能够有效利用少量已知异常样本来提高异常检测性能，为工业制造中的质量检测提供了新的解决方案。
...
TL;DR: TRACE是一个基于网格引导的3D高斯泼溅编辑框架，通过3D几何锚定实现自动化、高保真的场景转换，支持局部姿态调整和组件替换等精细操作。


<details>
  <summary>Details</summary>
Motivation: 现有编辑方法缺乏对中心主体结构完整性的保护能力，无法实现细粒度、部件级别的场景操控，如局部姿态调整或组件替换。

Method: 采用三阶段方法：1) 多视角3D锚点合成，利用MV-TRACE数据集生成空间一致的3D锚点；2) 有形几何锚定，通过两阶段配准确保插入网格与3DGS场景的精确空间同步；3) 上下文视频掩码，将3D投影集成到自回归视频管道中实现时间稳定的物理基础渲染。

Result: TRACE在编辑多样性和结构完整性方面持续优于现有方法，特别是在保持中心主体结构完整性的同时实现细粒度操控。

Conclusion: TRACE通过显式3D几何锚定视频扩散，实现了自动化、高保真的场景转换，填补了现有编辑方法在部件级别操控和结构完整性保护方面的空白。
...
TL;DR: 本研究评估了从KRS文档中提取信息的三种策略：纯LLM、混合确定性-LLM（正则+LLM）以及基于Camelot的LLM后备管道。混合方法在确定性元数据上效率更高，Camelot管道在准确性和计算效率上表现最佳，Qwen 2.5:14b模型在所有场景中表现最稳定。


<details>
  <summary>Details</summary>
Motivation: 评估在计算资源受限环境下（仅使用消费级CPU，无GPU）从学术文档中提取信息的可靠方法，比较纯LLM方法与结合确定性方法的混合策略的性能差异。

Method: 使用三种策略：1) 纯LLM方法；2) 混合确定性-LLM方法（正则表达式+LLM）；3) 基于Camelot的管道配合LLM后备。在140份文档上测试LLM方法，在860份文档上评估Camelot管道，涵盖四个不同表格和元数据的研究项目。使用三个12-14B参数的LLM模型（Gemma 3、Phi 4、Qwen 2.5），通过Ollama在本地运行。评估指标采用精确匹配（EM）和Levenshtein相似度（LS），阈值为0.7。

Result: 混合方法相比纯LLM在效率上有提升，特别是在确定性元数据提取方面。基于Camelot的LLM后备管道在准确性和计算效率上表现最佳，EM和LS达到0.99-1.00，大多数情况下每份PDF处理时间少于1秒。Qwen 2.5:14b模型在所有场景中表现最稳定。

Conclusion: 在计算资源受限环境下，结合确定性方法和LLM的混合策略对于从基于文本的学术文档中提取信息既可靠又高效，Camelot管道配合LLM后备是最佳组合方案。
...
TL;DR: LinearARD是一种自蒸馏方法，通过保持注意力结构一致性来恢复RoPE扩展后LLM的短文本性能，使用线性内存核解决二次内存瓶颈，仅需4.25M训练token即可实现性能恢复。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过扩展位置编码和轻量级持续预训练来扩展上下文窗口，但这通常会破坏原始模型能力，导致在标准短文本基准测试上性能下降。需要一种方法既能扩展上下文长度，又能保持原始模型的短文本性能。

Method: 提出LinearARD自蒸馏方法：1）使用冻结的原始RoPE教师模型监督RoPE扩展后的学生模型；2）对齐密集Q/Q、K/K、V/V自关系矩阵的行分布来直接监督注意力动态；3）引入线性内存核，利用每token的log-sum-exp统计量，将logit重新计算融合到反向传播中，以计算精确的KL散度和梯度。

Result: 在LLaMA2-7B从4K扩展到32K的实验中，LinearARD恢复了最先进基线方法短文本性能的98.3%，同时在长上下文基准测试上超越了它们。仅需4.25M训练token，而LongReD和CPT需要256M token。

Conclusion: LinearARD通过注意力结构一致性的自蒸馏方法，有效解决了RoPE扩展后模型性能退化问题，在保持短文本性能的同时提升长上下文处理能力，且训练效率显著提高。
...
TL;DR: 该研究评估了LLM-as-judge方法在评估大语言模型生成解释性回答质量时与人类判断的一致性，发现自动化评分能捕捉大致趋势但存在显著差异，建议该方法更适合筛选表现不佳的模型而非替代人类判断。


<details>
  <summary>Details</summary>
Motivation: 随着定性研究者越来越多地使用自动化工具支持解释性分析，大语言模型常被直接引入分析流程而缺乏系统性评估。模型选择对解释结果有潜在影响却很少被检验，因此需要研究LLM-as-judge评估是否与人类对解释质量的判断一致，能否支持模型选择决策。

Method: 使用712个K-12数学教师半结构化访谈对话片段，通过五个主流推理模型生成单句解释性回答。采用AWS Bedrock的LLM-as-judge框架进行自动化评估，涵盖五个指标。同时由训练有素的人类评估者对分层抽样子集在解释准确性、细微差别保留和解释连贯性三个维度进行独立评分。

Result: LLM-as-judge评分在模型层面能捕捉人类评估的大致趋势，但在分数幅度上存在显著差异。自动化指标中，连贯性与聚合人类评分一致性最强，而忠实性和正确性在片段层面显示系统性偏差，尤其对非字面和细微解释。安全相关指标与解释质量基本无关。
...
TL;DR: 本文介绍了Eyla的设计理念、实现尝试和失败分析，这是一个基于身份的LLM架构，整合了生物启发子系统，旨在实现身份一致性而非通用帮助性。作者尝试用AI编程助手实现该架构但失败，产生了86个子系统仅贡献2%输出的模型，并分析了AI辅助开发的系统性失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要优化通用帮助性，但缺乏身份一致性——在对抗压力下保持连贯自我模型、承认不确定性和抵抗操纵的能力。作者希望开发一种能维持身份一致性的LLM架构，这在当前AI系统中是一个未被充分探索的重要特性。

Method: 提出了Eyla架构，整合了生物启发子系统：HiPPO初始化的状态空间模型、零初始化适配器、情景记忆检索和校准不确定性训练。定义了身份一致性评分（ICS）作为评估基准。作为非程序员，使用AI编程助手（Claude Code、Cursor）尝试实现该架构，并记录了完整的失败过程。

Result: 实现尝试失败，花费1000多美元产生了1.27B参数的模型，其中86个大脑子系统对输出的贡献不到2%。识别了AI辅助开发新型架构的五个系统性失败模式，并提供了具体建议。这是首篇结合架构愿景与AI辅助LLM开发第一人称失败分析的论文。

Conclusion: 论文展示了AI辅助开发在创新架构实现中的局限性，特别是对于非专业开发者。通过详细的失败分析，为AI系统和AI辅助软件工程社区提供了宝贵经验教训，强调了身份一致性作为LLM重要研究方向的价值。
...
TL;DR: 本文针对大语言模型在微调或后训练后安全性下降的问题，提出了一种轻量级解决方案SafeReAct，通过LoRA适配器恢复被抑制的安全机制，在不影响推理性能的前提下显著提升模型安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管通用大语言模型性能出色，但在针对特定任务进行微调或后训练（如大推理模型LRMs）后，模型的安全性往往会下降，导致有害行为增加。本文旨在探究这种安全性下降的根本原因，并提出解决方案。

Method: 首先分析后训练导致安全性下降的原因，发现后训练会掩盖基础LLM原有的安全机制，同时过度放大与后训练能力相关的表征。基于这一发现，提出SafeReAct方法，通过在少数层上使用LoRA适配器进行对齐，恢复被抑制的安全行为。

Result: 在四个最先进的大推理模型上的实验表明，该方法在有害提示上显著提高了安全性，同时不损害推理性能。在其他领域特定LLMs（如医疗模型）上的额外结果进一步证实了该方法的通用性和有效性。

Conclusion: 后训练会抑制LLM原有的安全机制，但这些机制并未被移除。提出的SafeReAct方法能够有效恢复被抑制的安全行为，为提升领域特定LLMs的安全性提供了一种轻量级、成本效益高的解决方案。
...
TL;DR: 本文提出了一种结合结构化判别-校准推理与基于提示的强化学习的新型训练框架，用于提升多模态情感分析的可解释性和性能。
...
Method: 提出集成结构化判别-校准推理与基于提示的强化学习的训练框架：1）使用教师模型合成的高质量思维链数据进行冷启动监督微调，该数据包含DC结构；2）提出Hint-GRPO方法，利用DC结构中的判别阶段作为强化学习中的可验证锚点，为困难样本提供方向性提示，引导策略优化并缓解奖励稀疏问题。
...
TL;DR: 研究者开发了UTCO框架系统测试LLM在心理健康问答中的安全风险，发现幻觉率6.5%、遗漏率13.2%，危机相关提示遗漏率更高，建议将遗漏作为主要安全评估指标。


<details>
  <summary>Details</summary>
Motivation: 心理健康问题常在非临床环境中表达，需要安全关键指导。现有LLM评估缺乏对高压力叙述性询问的系统测试，需要更全面的安全评估框架。

Method: 提出UTCO框架（用户、主题、上下文、语调），将询问分解为四个可控元素进行系统压力测试。使用2075个UTCO生成的提示评估Llama 3.3，标注幻觉（捏造或错误临床内容）和遗漏（缺少必要临床或安全指导）。

Result: 幻觉发生率为6.5%，遗漏率为13.2%。遗漏主要集中在危机和自杀意念提示中。回归分析、元素匹配和相似性匹配比较显示，失败最一致地与上下文和语调相关，而用户背景指标在平衡后无系统性差异。

Conclusion: 应将遗漏作为主要安全评估结果，超越静态基准问题集，系统评估LLM在心理健康应用中的安全风险。
...
TL;DR: ASCAT是一个高质量的英阿平行科学翻译评估语料库，包含多领域科学摘要，通过多引擎翻译和人工验证构建，用于评估科学翻译质量。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语-英语语料库通常依赖短句或单一领域文本，缺乏针对科学翻译的高质量评估资源。ASCAT旨在填补阿拉伯语科学机器翻译资源的空白，支持科学翻译质量的严格评估和领域特定翻译模型的训练。

Method: 从物理学、数学、计算机科学、量子力学和人工智能五个科学领域收集科学摘要，使用三种互补架构进行翻译：生成式AI（Gemini）、基于Transformer的模型（Hugging Face quickmt-en-ar）和商业MT API（Google Translate、DeepL）。所有翻译结果由领域专家在词汇、句法和语义层面进行人工验证。

Result: 构建的语料库包含67,293个英语标记和60,026个阿拉伯语标记，阿拉伯语词汇表包含17,604个独特单词，反映了语言的形态丰富性。在语料库上对三种最先进的LLM进行基准测试：GPT-4o-mini（BLEU: 37.07）、Gemini-3.0-Flash-Preview（BLEU: 30.44）和Qwen3-235B-A22B（BLEU: 23.68），展示了其作为评估基准的区分能力。

Conclusion: ASCAT填补了阿拉伯语科学机器翻译资源的关键空白，旨在支持科学翻译质量的严格评估和领域特定翻译模型的训练，为阿拉伯语科学翻译提供了高质量、多领域的评估基准。
...
TL;DR: 研究提出利用人类工作记忆容量有限这一认知约束来区分在线行为研究中的人类参与者和大型语言模型，即使后者被指令模仿人类限制


<details>
  <summary>Details</summary>
Motivation: 在线行为研究的有效性依赖于参与者是人类而非机器。过去可以通过简单挑战来检测机器，但基于大语言模型的通用智能体现在能解决许多这类挑战，威胁在线研究的有效性

Method: 使用标准序列回忆任务进行认知建模，探测人类工作记忆容量有限的认知约束，即使LLMs被特别指令模仿人类工作记忆限制也能区分

Result: 认知建模可以区分在线参与者和LLMs，证明利用已建立的认知现象来区分LLMs和人类是可行的

Conclusion: 通过检测机器"过于完美"而非"不够好"的表现来识别机器是可行的，利用人类认知约束可以保护在线行为研究的有效性
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索心理学概念在科学文献和大众媒体中的语义变化，了解专业术语如何在不同传播语境中被重新解释和重构，揭示知识传播过程中的概念转换现象。

Method: 构建了两个俄语语料库：科学语料库（约300篇研究文章，767,543词）和大众科学语料库（在线心理学平台文本，1,199,150词）。经过预处理（OCR识别、词形还原、停用词和非信息字符去除）后，采用频率分析、聚类和语义关联识别等方法进行分析。

Result: 研究发现两种话语类型在词汇和概念框架上存在显著差异：科学文本强调方法论和临床术语，而大众科学材料突出日常经验和治疗实践。关键概念（如倦怠和抑郁）的语义关联比较显示，科学话语将这些术语与心理资源、症状学和诊断结构联系起来，而大众科学话语则通过个人叙事、情感和日常情境来构建。

Conclusion: 研究证实了心理学概念从精确的专业术语向更普遍化和经验性意义的明显转变，证明了分布语义学方法在不同传播语境中识别语义转换的有效性，揭示了知识传播过程中概念框架的重要变化。
...
TL;DR: 本文提出了一个可配置的管道，用于生成具有特定特征（如领域、地理位置和流行度）的多语言实体数据集，用于评估LLMs长文本生成的事实性，并以RiDiC数据集为例展示了该方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs评估主要基于短问答数据集，缺乏对长文本生成事实性的评估方法。需要创建能够系统评估LLMs在不同领域、不同语言中事实准确性的数据集。

Method: 使用Wikipedia和Wikidata数据构建可配置的管道，根据领域、地理位置和流行度等特征筛选实体。创建RiDiC数据集作为示例，包含河流、自然灾害和汽车模型三个领域的3000个实体，每个实体包含地理位置、中英文名称和相关维基百科内容。

Result: RiDiC数据集成功生成了包含不同流行度层级的实体集合。使用第三方事实性检查器评估三个LLMs在中英文生成中的表现，结果显示即使是前沿模型也会对数据集中的实体产生幻觉。

Conclusion: 该方法为评估LLMs多语言长文本生成的事实性提供了有效工具，RiDiC数据集展示了其可行性，并开源了代码、数据和评估脚本以促进相关研究。
...
TL;DR: 提出基于预训练Transformer的时间情感聚合框架，通过聚合用户评论情感信号检测反馈异常模式


<details>
  <summary>Details</summary>
Motivation: 在客户反馈监控、品牌声誉管理等应用中，理解用户情感的时间动态对早期检测异常事件（如恶意评论活动或用户满意度突然下降）至关重要。传统情感分析方法关注单个文本分类，无法有效捕捉集体行为随时间的变化，且短评中存在噪声和类别不平衡问题。

Method: 提出时间情感聚合框架：1）使用预训练Transformer语言模型（RoBERTa）提取每条评论的情感信号；2）将情感信号聚合为时间窗口级别的分数；3）将这些聚合分数的显著下降解释为用户反馈模式的潜在异常。

Result: 在真实社交媒体数据上的实证评估表明：1）聚合情感分数能揭示有意义的趋势；2）支持有效的异常检测；3）成功识别出与连贯投诉模式相对应的统计显著情感下降。

Conclusion: 该方法为反馈异常监控提供了一个有效且可解释的解决方案，能够检测用户情感的时间动态变化，适用于客户反馈监控、品牌声誉管理等实际应用场景。
...
TL;DR: 研究通过多智能体模拟发现，语言模型处理伦理指令存在四种不同类型，伦理指令格式仅对高处理能力模型有影响，且安全输出与内部伦理处理基本无关。
...
Method: 使用4个模型（Llama 3.3 70B、GPT-4o mini、Qwen3-Next-80B-A3B、Sonnet 4.5）、4种伦理指令格式（无指令、最小规范、推理规范、美德框架）和2种语言（日语、英语），进行超过600次多智能体模拟。开发了三个新指标：审议深度（DD）、跨困境价值一致性（VCAD）和他人识别指数（ORI）。

Result: 发现四种不同的伦理处理类型：输出过滤（GPT；安全输出但无处理）、防御性重复（Llama；通过公式化重复实现高一致性）、批判性内化（Qwen；深度审议但整合不完全）、原则一致性（Sonnet；审议、一致性和他人识别共存）。关键发现是处理能力与指令格式的交互作用：低DD模型中指令格式不影响内部处理；高DD模型中推理规范和美德框架产生相反效果。词汇合规性与处理指标无相关性。

Conclusion: 语言模型的伦理处理机制存在显著差异，安全输出与内部伦理处理基本无关。这些处理类型与临床罪犯治疗中观察到的模式有结构对应性，其中形式合规而无内部处理是已知的风险信号。
...
### [103] [Phonological Fossils: Machine Learning Detection of Non-Mainstream Vocabulary in Sulawesi Basic Lexicon](https://arxiv.org/abs/2604.00023)
...
<details>
  <summary>Details</summary>
Motivation: 苏拉威西南岛语系语言中存在许多无法重构到原始南岛语系的词汇，这些词汇的语音模式与继承词根不一致。需要确定这些非主流词汇是前南岛语系底层语言还是独立创新。

Method: 结合基于规则的同源词减法与机器学习分类器。使用南岛语系基本词汇数据库中的1,357个词形，通过同源词减法和原始南岛语系交叉检查识别候选底层词汇。训练XGBoost分类器基于26个语音特征区分继承词与非主流词汇。

Result: 识别出438个候选底层词形（26.5%）。机器学习分类器AUC=0.763，发现非主流词汇的语音特征：形式更长、更多辅音丛、更高喉塞音比例、较少南岛语系前缀。跨方法共识识别出266个高置信度非主流候选词。聚类分析未发现连贯的词族，无证据表明存在单一前南岛语系语言层。地理模式显示苏拉威西语言的非主流词汇率（0.606）高于西印尼语言（0.393）。

Conclusion: 语音机器学习可以补充传统比较方法检测非主流词汇层，但语音不一致性不能作为共享底层语言的证据。研究揭示了苏拉威西南岛语系语言中的复杂词汇历史，而非单一前南岛语系语言层。
...
### [104] [WHBench: Evaluating Frontier LLMs with Expert-in-the-Loop Validation on Women's Health Topics](https://arxiv.org/abs/2604.00024)
*Sneha Maurya,Pragya Saboo,Girish Kumar*

Main category: cs.CL

TL;DR: WHBench是一个针对女性健康的AI评估基准，包含47个专家设计的医疗场景，用于测试22个语言模型在临床准确性、安全性、公平性等方面的表现。结果显示所有模型平均表现均低于75%，最佳模型仅达72.1%，暴露出临床部署中需要专家监督的重要问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型越来越多地用于医疗指导，但女性健康领域在基准测试设计中评估不足。需要专门的评估套件来暴露临床上有意义的失败模式，包括过时指南、不安全遗漏、剂量错误和公平性盲点。

Method: 开发了WHBench评估套件，包含47个专家设计的女性健康场景，涵盖10个主题。使用23项标准评估22个模型，包括临床准确性、完整性、安全性、沟通质量、指令遵循、公平性、不确定性处理和指南遵循。采用安全加权惩罚和服务器端分数重新计算。

Result: 在3,102个尝试回答中（3,100个已评分），没有模型平均表现超过75%，最佳模型达到72.1%。即使是顶级模型也显示出较低的完全正确率和显著的伤害率变化。评分者间信度在响应标签层面为中等，但在模型排名层面较高。

Conclusion: WHBench提供了一个公开的、关注失败模式的基准，用于追踪女性健康AI领域更安全和更公平的进展。研究强调了在临床部署中需要专家监督，同时支持WHBench在比较系统评估中的实用性。
...
### [105] [Brevity Constraints Reverse Performance Hierarchies in Language Models](https://arxiv.org/abs/2604.00025)
*MD Azizul Hakim*

Main category: cs.CL

TL;DR: 研究发现大型语言模型存在"规模依赖的啰嗦"现象，导致在7.7%的基准问题上表现反而比小模型差28.4个百分点。通过约束大模型给出简洁回答，准确率可提升26个百分点，性能差距减少三分之二，甚至完全逆转数学推理和科学知识基准上的性能层次。


<details>
  <summary>Details</summary>
Motivation: 标准评估协议揭示了一个反直觉现象：在7.7%的基准问题上，拥有10-100倍参数的大型语言模型表现反而比小型模型差28.4个百分点。研究者希望探究这一现象的机制，并确定是否反映了模型的基本能力限制。

Method: 系统评估了31个模型（0.5B-405B参数）在1,485个问题上的表现，通过因果干预实验识别机制，并测试了约束大模型产生简洁回答的效果。进行了三个独立的污染测试验证发现，并分析了整个参数谱上的连续逆缩放现象。

Result: 发现机制是"规模依赖的自发啰嗦"，通过过度阐述引入错误。约束大模型产生简洁回答可将准确率提高26个百分点，性能差距减少三分之二。在数学推理和科学知识基准上，简洁约束完全逆转了性能层次，大模型获得7.7-15.9个百分点的优势。数据集特定的最优规模范围为0.5B到3.0B参数。

Conclusion: 大型模型拥有优越的潜在能力，但通用提示设计掩盖了这些能力。最大化大模型性能需要规模感知的提示工程而非通用评估协议。提示适应不仅能提高准确性，还能降低计算成本，对实际部署有直接意义。
...
### [106] ["Who Am I, and Who Else Is Here?" Behavioral Differentiation Without Role Assignment in Multi-Agent LLM Systems](https://arxiv.org/abs/2604.00026)
*Houssam EL Kandoussi*

Main category: cs.CL

TL;DR: 研究通过多LLM交互实验发现，异构模型群组比同构群组表现出更丰富的行为分化，模型名称揭示、提示框架移除等条件会显著影响群组行为收敛程度，且这些行为多样性是结构化、可复现的交互现象。


<details>
  <summary>Details</summary>
Motivation: 探究多个大语言模型在共享对话中是否会发展出差异化的社会角色，还是趋向于统一行为，理解LLM群体交互中的行为动态。

Method: 构建控制实验平台，在统一推理后端上协调7个异构LLM进行多智能体讨论，系统变化群组构成、命名惯例和提示结构，进行12个实验系列（208次运行，13,786条编码消息）。每条消息由两个不同模型家族的LLM法官独立编码六个行为标志，采用保守交集裁决方法。

Result: 1) 异构群组比同构群组表现出显著更丰富的行为分化；2) 群组在智能体崩溃时自发表现出补偿性响应模式；3) 揭示真实模型名称显著增加行为收敛；4) 移除所有提示框架会使行为特征收敛到同构水平相似度。这些行为在智能体单独操作时不存在。

Conclusion: LLM群体中的行为多样性是结构化、可复现的现象，由架构异质性、群组上下文和提示级框架的交互驱动，而非随机变化。
...
### [107] [Multi-lingual Multi-institutional Electronic Health Record based Predictive Model](https://arxiv.org/abs/2604.00027)
*Kyunghoon Hur,Heeyoung Kwak,Jinsu Jang,Nakhwan Kim,Edward Choi*

Main category: cs.CL

TL;DR: 本研究探索了多语言多机构电子健康记录预测，通过文本转换和语言对齐技术，实现了无需手动标准化的跨国ICU数据集联合训练。


<details>
  <summary>Details</summary>
Motivation: 大规模跨机构电子健康记录预测面临模式异构性和编码系统差异的挑战。虽然通用数据模型可以标准化记录，但手动协调和词汇映射成本高且难以扩展。文本化协调提供了替代方案，但跨国数据集引入了语言异质性这一额外挑战。

Method: 研究比较了两种处理语言障碍的策略：(1) 使用多语言编码器直接建模多语言记录；(2) 通过基于LLM的词级翻译将非英语记录翻译成英语。在七个公共ICU数据集上进行评估，涵盖十个临床任务和多个预测窗口。

Result: 基于翻译的语言对齐方法比多语言编码器产生更可靠的跨数据集性能。多机构学习模型始终优于需要手动特征选择和协调的基线方法，也超越了单数据集训练。文本化框架结合语言对齐能够通过少量样本微调有效进行迁移学习。
...
### [108] [Hierarchical Chain-of-Thought Prompting: Enhancing LLM Reasoning Performance and Efficiency](https://arxiv.org/abs/2604.00130)
*Xingshuai Huang,Derek Li,Bahareh Nikpour,Parsa Omidi*

Main category: cs.CL

TL;DR: Hi-CoT是一种分层思维链提示方法，通过交替进行指令规划和逐步执行来改进复杂多步推理，相比传统CoT在准确率上平均提升6.2%，推理轨迹长度减少13.9%。


<details>
  <summary>Details</summary>
Motivation: 传统思维链提示通常依赖非结构化、扁平的推理链，存在冗余和性能不佳的问题，特别是在处理复杂多步推理任务时表现不足。

Method: 提出分层思维链提示方法，将推理过程分解为层次化子步骤，通过交替进行指令规划和逐步执行来管理长推理视野并保持逻辑一致性。

Result: 在多种大语言模型和数学推理基准测试中，Hi-CoT相比传统CoT平均准确率提升6.2%（某些模型和任务上最高达61.4%），同时推理轨迹长度减少13.9%。

Conclusion: 分层思维链提示能有效提升大语言模型的复杂推理能力，当模型严格遵守分层结构时，准确率和效率达到最优。
...
TL;DR: 使用分布语义学分析波兰语词汇的音系形态结构与意义的关系，发现语义向量能捕捉亚词汇语言单位信息，音系复杂性和形态句法范畴可从词嵌入中预测。


<details>
  <summary>Details</summary>
Motivation: 研究波兰语词汇的音系形态结构（特别是辅音簇）与其意义之间是否存在关系，探讨复杂词的音系和形态结构是否在语义空间中得到反映。

Method: 使用统计和计算技术（t-SNE、线性判别分析、线性判别学习），通过分布语义学分析波兰语词汇，利用判别词典模型进行建模。

Result: 语义向量不仅能编码丰富的形态句法信息（时态、数、格），还能捕捉音素串等亚词汇语言单位信息；音系复杂性、形态透明度和多种形态句法范畴可从词嵌入中准确预测。

Conclusion: 计算建模显示语义空间中存在与形式空间结构同构的丰富信息，这为理解和生成任务提供了高度准确的预测，支持形式与意义之间存在系统性对应关系的观点。
...
TL;DR: 研究发现大语言模型内部编码了上下文隐私规范，但实际行为仍会泄露隐私信息。通过引入基于上下文完整性理论的参数化调控，可以更有效地减少隐私违规。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在高风险场景中部署时，经常违反上下文隐私规范，泄露本应保密的信息。这引发了一个根本问题：LLMs是否内部编码了上下文隐私规范？如果编码了，为什么还会出现违规行为？

Method: 基于上下文完整性理论，系统研究LLMs中上下文隐私的结构化潜在表示。通过探测多个模型，发现CI的三个参数（信息类型、接收者、传输原则）在激活空间中编码为线性可分离且功能独立的方向。引入CI参数化调控，独立干预每个CI维度。

Result: 研究发现LLMs确实内部编码了上下文隐私规范，三个CI参数在激活空间中线性可分离且功能独立。尽管有这种内部结构，模型在实践中仍会泄露隐私信息。CI参数化调控比整体调控更有效、更可预测地减少隐私违规。

Conclusion: 上下文隐私违规源于表示与行为之间的错位，而非缺乏意识。利用CI的组合结构能够实现更可靠的上下文隐私控制，为改进LLMs的上下文隐私理解提供了启示。
...
TL;DR: 大型语言模型能够预测自己是否会拒绝有害请求，但预测准确性在安全边界处显著下降，不同模型表现差异明显，置信度分数可用于安全关键部署的路由决策。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够准确预测自己是否会拒绝有害请求，这对于理解模型的安全机制和实际部署中的可靠性至关重要。

Method: 采用系统研究方法，让模型先预测自己的拒绝行为，然后在新的上下文中实际响应。使用信号检测理论分析四个前沿模型（Claude Sonnet 4/4.5、GPT-5.2、Llama 3.1 405B）在3754个数据点上的表现。

Result: 所有模型都表现出较高的内省敏感性，但在安全边界处敏感性显著下降。Claude Sonnet 4.5准确率最高（95.7%），GPT-5.2准确率较低（88.9%）且行为更不稳定，Llama 405B敏感性高但存在强烈拒绝偏见和校准差的问题。武器相关查询最难内省。
...
TL;DR: 该研究首次提出了编程语言的资源分类体系，将646种语言分为4个等级，发现仅1.9%的高资源语言占据了74.6%的代码token，而71.7%的低资源语言仅占1.0%，揭示了编程语言资源分布的极端不平衡性。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理领域已有语言资源分类体系，但编程语言领域缺乏类似的资源层级分类。随着大语言模型代码生成能力的提升，建立编程语言资源分类体系变得至关重要，以解决不同编程语言资源可用性的巨大差异问题。

Method: 研究提出了首个可复现的编程语言资源分类方法，基于7个主要代码语料库的数据，将646种编程语言划分为4个资源等级（Tier 0-3），并进行了统计不平等性、分散度和分布偏度的分析。

Result: 结果显示编程语言资源分布极不平衡：仅1.9%的Tier 3（高资源）语言占据了74.6%的所有token，而71.7%的Tier 0（稀缺资源）语言仅贡献了1.0%的token。统计分析证实这种不平衡是极端且系统性的。

Conclusion: 该研究为编程语言建立了首个资源分类体系，揭示了代码资源的极端不平衡分布，为数据集构建和多语言大语言模型的层级感知评估提供了原则性框架。
...
TL;DR: REM-CTX是一个基于强化学习的自动同行评审系统，通过对应感知奖励函数整合辅助上下文（如图表等视觉元素），在8B参数语言模型上使用GRPO训练，在多个科学领域超越现有基线系统。


<details>
  <summary>Details</summary>
Motivation: 现有自动同行评审系统主要依赖文本内容，未能充分利用视觉元素（如图表）和外部学术信号等辅助上下文信息，限制了评审质量和相关性。

Method: 提出REM-CTX系统，采用强化学习框架，通过对应感知奖励函数整合辅助上下文。使用8B参数语言模型，采用Group Relative Policy Optimization (GRPO)训练，结合多维度质量奖励和两个对应奖励（明确鼓励与辅助上下文对齐）。

Result: 在计算机、生物和物理科学领域的稿件实验中，REM-CTX在六个基线系统中获得最高整体评审质量，超越了使用更大商业模型的系统，并在质量和上下文基础指标上均优于次优的RL基线。消融研究证实两个对应奖励具有互补性。

Conclusion: REM-CTX成功整合辅助上下文提升自动同行评审质量，对应奖励机制有效。训练动态分析显示批评维度与其他指标负相关，建议未来研究应将多维度奖励分组处理。
...
TL;DR: LLM在作文评分中与人类评分者存在系统性偏差，特别是在语法等低阶关注点上模型评分更严苛，可通过小样本偏差校正改进部署策略


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在教育评估中的应用日益增长，但其与人类评分的一致性仍不明确。本研究旨在系统评估指令调优的LLM在作文评分任务中与人类评分的一致性和偏差模式。

Method: 使用三个公开的作文评分数据集（ASAP 2.0、ELLIPSE和DREsS），涵盖整体评分和分析评分。分析LLM与人类共识评分的一致性、方向性偏差以及偏差估计的稳定性。采用简洁关键词提示与详细评分标准提示的对比实验。

Result: 强大的开源模型在整体评分上与人类达到中等至高一致性（二次加权Kappa约0.6），但在分析评分上表现不均。在低阶关注点（如语法和规范）上观察到显著且稳定的负向偏差，模型评分比人类更严苛。简洁关键词提示在多特质分析评分中通常优于详细评分标准提示。低阶关注点偏差可在小验证集中检测，而高阶关注点需要更大样本。

Conclusion: 建议采用"偏差校正优先"的部署策略：无需大规模微调，仅需使用小型人工标注的偏差估计集来估计和校正系统性评分偏移，而不是依赖原始零样本评分。
...
TL;DR: 自反思提示在医学QA中并不总能提高准确性，其效果高度依赖于数据集和模型，不应被视为提高可靠性的独立解决方案


<details>
  <summary>Details</summary>
Motivation: 尽管自反思（自我纠正）提示被广泛声称能通过让LLM批判和修正自身推理来提高模型可靠性，但其在安全关键的医学环境中的有效性仍不明确

Method: 使用GPT-4o和GPT-4o-mini，在三个医学QA基准（MedQA、HeadQA和PubMedQA）上比较标准思维链提示与迭代自反思循环，跟踪预测在反思步骤中的演变

Result: 自反思提示不能一致提高准确性，其影响高度依赖于数据集和模型：在MedQA上有适度提升，但在HeadQA和PubMedQA上效果有限或负面，增加反思步骤数不能保证更好的性能
...
TL;DR: 本文探讨生物语言学作为研究语言生物学基础、进化和遗传基础的科学，强调将语言视为内在生物器官而非文化工具，并主张数学代数模型能捕捉语言的自然本质。


<details>
  <summary>Details</summary>
Motivation: 挑战行为主义将语言习得视为刺激-反应关联的观点，主张语言是内在生物器官，需要从生物学角度研究其计算系统，为生物学家、遗传学家和神经科学家提供更清晰的探索方向。

Method: 通过四个步骤展开论证：1)明确生物语言学的研究对象是生成层次结构表达的内部计算系统；2)论证形式特征对进化解释的重要性；3)提出明确的代数句法描述对神经机制候选方案施加非平凡约束；4)考虑神经计算工作如何将这些约束转化为可检验假设。

Result: 建立了生物语言学的研究框架，将句法操作MERGE视为"自然的真实关节"，为跨学科研究提供理论基础，同时承认当前研究计划的推测性和可修正性。

Conclusion: 生物语言学通过数学代数模型研究语言的生物基础，为理解语言的进化、遗传和神经机制提供了新的理论框架和研究方向，促进了语言学与生物科学的跨学科融合。
...
### [117] [Asymmetric Actor-Critic for Multi-turn LLM Agents](https://arxiv.org/abs/2604.00304)
*Shuli Jiang,Zhaoyang Zhang,Yi Zhang,Shuo Yang,Wei Xia,Stefano Soatto*

Main category: cs.CL

TL;DR: 提出一种非对称的actor-critic框架，让强大的专有LLM作为actor执行对话任务，而较小的开源模型作为critic进行实时监督和干预，提高单次交互的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么需要多次尝试（如反思或事后评估），要么假设模型完全可训练（无法利用专有LLM）。在现实应用中，智能体需要在无法重试的单次设置中可靠工作。

Method: 采用非对称actor-critic框架：专有LLM作为actor执行对话，开源小模型作为critic进行运行时监督，可在同一交互轨迹中干预。利用生成-验证的不对称性：高质量生成需要大模型，但有效监督可由小模型完成。还设计了数据生成流程，为critic微调提供监督信号而不修改actor。

Result: 在τ-bench和UserBench上的实验表明，该方法显著提高了可靠性和任务成功率，优于强单智能体基线。轻量级开源critic在监督角色上媲美或超越更大的专有模型，critic微调相比多种SOTA方法带来额外提升。

Conclusion: 提出的非对称actor-critic框架能够有效提高对话智能体在单次交互中的可靠性，利用生成-验证不对称性，让专有LLM专注于生成，小模型负责监督，为实际应用提供了实用解决方案。
...
TL;DR: 这篇综述论文分析了大型语言模型如何整合到滥用检测生命周期中，涵盖标签生成、检测、审核和审计四个阶段，探讨了LLM在在线安全系统中的优势与挑战。


<details>
  <summary>Details</summary>
Motivation: 在线滥用行为日益复杂，传统机器学习方法依赖静态分类器和人工标注难以跟上不断变化的威胁模式和复杂的政策要求，需要更智能的解决方案。

Method: 提出了基于生命周期的分析方法，将滥用检测生命周期分为四个阶段：标签与特征生成、检测、审核与申诉、审计与治理，并系统性地分析LLM在每个阶段的应用。

Result: LLM在上下文推理、政策解释、解释生成和跨模态理解方面展现出新能力，能够支持现代安全系统的多个阶段，但面临延迟、成本、确定性、对抗鲁棒性和公平性等挑战。

Conclusion: LLM有潜力成为大规模滥用检测和治理系统的可靠、可问责组件，但需要进一步研究解决实际部署中的关键挑战，以实现其在实际安全系统中的有效应用。
...
### [119] [Agent Q-Mix: Selecting the Right Action for LLM Multi-Agent Systems through Reinforcement Learning](https://arxiv.org/abs/2604.00344)
...
TL;DR: Agent Q-Mix是一个基于强化学习的多智能体框架，将拓扑选择重新定义为合作式多智能体强化学习问题，通过QMIX价值分解学习分散式通信决策，在多个基准测试中实现最高准确率和最佳令牌效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在完成各种任务方面表现出色，但解决复杂问题需要多个智能体的协调。当前面临的核心问题是如何有效地选择和连接这些智能体，即如何优化多智能体系统的通信拓扑结构。

Method: 提出Agent Q-Mix强化学习框架，将拓扑选择重新定义为合作式多智能体强化学习问题。采用QMIX价值分解学习分散式通信决策，每个智能体从一组通信动作中选择，共同形成轮次通信图。核心组件包括拓扑感知GNN编码器、GRU记忆和每个智能体的Q头，采用集中训练分散执行的范式。优化奖励函数平衡任务准确性和令牌成本。

Result: 在编码、推理和数学等七个核心基准测试中，Agent Q-Mix实现了最高的平均准确率，同时表现出优越的令牌效率和对抗智能体故障的鲁棒性。在具有挑战性的Humanity's Last Exam上，使用Gemini-3.1-Flash-Lite作为骨干，Agent Q-Mix达到20.8%的准确率，优于Microsoft Agent Framework（19.2%）和LangGraph（19.2%），其次是AutoGen和Lobster by OpenClaw。

Conclusion: 学习到的分散式拓扑优化在推动多智能体推理边界方面具有显著效果，Agent Q-Mix框架通过强化学习优化通信拓扑，在多智能体系统中实现了性能、效率和鲁棒性的平衡。
...
TL;DR: 研究发现神经元激活重叠主要源于词汇形式而非语义压缩，通过2x2因子分解揭示词汇混淆效应在多种模型中普遍存在


<details>
  <summary>Details</summary>
Motivation: 当同一神经元对"lender"和"riverside"都激活时，传统指标将其归因于叠加压缩。本研究旨在探究这种重叠在多大程度上源于词汇混淆：神经元实际上是对共享词汇形式（如"bank"）而非两个压缩概念做出反应。

Method: 采用2x2因子分解方法，对比分析词汇条件（相同词形、不同含义）与语义条件（不同词形、相同含义）。研究覆盖110M-70B参数规模的多种模型，并扩展到稀疏自编码器特征分析。

Result: 词汇条件的影响持续超过语义条件；18-36%的稀疏自编码器特征混合了不同词义；混淆效应集中在≤1%的激活维度中；过滤该混淆能提升词义消歧性能，并使知识编辑更具选择性（p=0.002）。

Conclusion: 神经元激活重叠主要源于词汇形式混淆而非语义压缩，这一发现对语言模型解释性研究具有重要意义，过滤词汇混淆能改善下游任务性能。
...
TL;DR: 该论文提出了一种多模态深度上采样方法，通过在冻结的文本LLM中插入新的Transformer层来适应语音数据，减少了对原始文本能力的退化。


<details>
  <summary>Details</summary>
Motivation: 将预训练的文本大语言模型通过持续预训练适应语音数据时，通常会退化原始文本能力。需要一种方法既能获得良好的语音识别性能，又能最小化对文本能力的负面影响。

Method: 提出多模态深度上采样方法：在冻结的文本LLM中插入新的Transformer层，只训练新增的层来处理语音数据。实验中使用SmolLM2-360M和SmolLM2-1.7B模型，在48k小时英语ASR数据上进行测试，并尝试将专门用于语音识别的E-Branchformer架构作为插入层。

Result: 深度上采样方法在ASR性能上与全微调相当，但文本退化远少于全微调和LoRA。使用E-Branchformer作为插入层时，在较大模型上ASR性能达到或超过全微调，同时文本退化减少75%以上，可训练参数减少60%。

Conclusion: 多模态深度上采样是一种有效的策略，可以在保持文本LLM能力的同时，将其适应到语音领域，实现更好的多模态模型性能平衡。
...
TL;DR: 论文提出了一种基于训练效用评估的合成数据优化框架，通过目标模型的梯度信息量化合成样本对特定任务学习的影响，并利用强化学习优化评估标准生成器，实现跨领域、模型和生成器的一致性能提升。


<details>
  <summary>Details</summary>
Motivation: 在人文社科、医学、法律、金融等知识密集型领域，高质量监督微调数据稀缺，专家标注成本高、隐私限制严格、标签一致性难以保证。现有合成数据方法依赖人工设计的评估标准，但标准设计依赖专家经验、跨领域迁移性差、优化过程缺乏可靠的量化反馈。

Method: 提出基于训练效用的合成数据评估方法：1) 采用优化器感知的估计器，利用梯度信息量化每个合成样本对目标模型在特定任务上目标函数的贡献；2) 基于影响分数作为奖励，通过强化学习优化评估标准生成器；3) 提供轻量级指导文本，使用专门化模型生成任务条件评估标准。

Result: 实验表明：1) 合成样本与真实样本在嵌入空间相近时，对学习的影响仍存在显著差异；2) 提出的优化框架在多个领域、目标模型和数据生成器上均实现了一致性能提升；3) 方法具有强泛化能力，无需任务特定调优。

Conclusion: 通过目标模型反馈指导合成数据生成，提供了一种量化评估合成数据训练效用的方法，解决了传统人工设计评估标准的局限性，实现了跨领域、模型和生成器的稳定性能改进。
...
TL;DR: 该研究构建了日本文化特定的偏见评估数据集JUBAKU-v2，基于归因理论评估推理过程中的群体偏见，比现有基准更敏感地检测模型性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有日本偏见评估基准主要依赖英语数据翻译，不适合日本文化背景，且仅评估结论中的偏见，无法捕捉推理过程中的偏见。需要构建能评估日本文化特定偏见、特别是推理过程中群体归因偏见的数据集。

Method: 基于社会心理学中的归因理论，构建JUBAKU-v2数据集，通过固定结论来评估推理过程中对内外群体行为的归因偏见。数据集包含216个反映日本文化特定偏见的示例。

Result: 实验结果表明，JUBAKU-v2比现有基准更敏感地检测出不同模型之间的性能差异，能够有效识别推理过程中的群体归因偏见。

Conclusion: JUBAKU-v2数据集为评估日本文化背景下大语言模型的偏见提供了更合适的工具，特别关注推理过程中的群体归因偏见，弥补了现有基准的不足。
...
TL;DR: 本文研究如何通过微调1.7B参数的小型语言模型，使用有限的人工标注数据，构建比专有LLM更优的自动评估和标注工具。


<details>
  <summary>Details</summary>
Motivation: 随着LLM能力提升，文本语料库指数增长导致高质量标注需求超出人类能力，专有LLM用于自动评估和标注时存在系统性偏见、缺乏可重复性和数据隐私问题。

Method: 采用1.7B参数的小型语言模型，通过4位量化微调，使用有限人工标注数据，结合自定义多维评分框架及简单数据增强和正则化技术。

Result: 提出的方法在标注者间一致性上优于最佳专有LLM（Krippendorff's α提高0.23分），在情感分类任务上也展示了泛化能力。
...
TL;DR: 提出链式思维ASR（CoT-ASR），通过构建推理链让大语言模型先分析语音输入生成上下文分析，再利用这些分析进行更准确的语音识别，同时支持用户引导转录。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型已扩展到语音输入，但在自动语音识别任务中难以充分利用其丰富的知识和上下文理解能力，因为ASR主要是直接的语音到文本映射任务。

Method: 1. 提出CoT-ASR框架，构建推理链让LLM先分析输入语音生成上下文分析；2. 引入CTC引导的模态适配器，使用CTC非空白标记概率加权LLM嵌入，有效对齐语音编码器输出与LLM的文本潜在空间；3. 支持用户引导转录，可无缝整合用户提供的上下文。

Result: 相比标准LLM-based ASR，CoT-ASR实现了8.7%的词错误率相对降低和16.9%的实体错误率相对降低。

Conclusion: CoT-ASR通过构建推理链充分利用LLM的生成能力，结合CTC引导的模态适配器减少模态差距，显著提升了ASR性能，并扩展了用户引导转录功能。
...
TL;DR: KUTED是首个用于中央库尔德语的语音到文本翻译数据集，包含9.1万句对，170小时英语音频。研究发现库尔德语的正字法变体严重影响翻译质量，通过文本标准化方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 中央库尔德语缺乏高质量的语音到文本翻译资源，且存在严重的正字法变体问题，这阻碍了机器翻译系统在该语言上的发展。需要构建专门的数据集并解决标准化问题。

Method: 从TED和TEDx演讲中构建KUTED数据集，包含英语音频和库尔德语文本。提出系统性的文本标准化方法来解决正字法变体问题。评估了多种模型：微调的Seamless模型、从头训练的Transformer模型，以及结合Seamless（ASR）和NLLB（MT）的级联系统。

Result: 在TED测试集上，微调的Seamless模型达到15.18 BLEU分数。在FLEURS基准测试中，相比Seamless基线提升了3.0 BLEU。文本标准化方法显著改善了翻译质量，产生了更一致的输出。

Conclusion: KUTED数据集填补了中央库尔德语语音翻译资源的空白。正字法变体是库尔德语机器翻译的主要挑战，而提出的标准化方法能有效解决这一问题，为低资源语言的语音翻译研究提供了重要参考。
...
TL;DR: TRIMS提出了一种轨迹引导的监督微调框架，通过自回归教师的轻量级信号指导轨迹感知掩码策略，改善扩散语言模型的解码轨迹，在数学和代码基准上显著提升了准确性与并行性的平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行解码提供了低延迟生成的潜力，但实际效率严重依赖解码轨迹。标准训练缺乏对token揭示顺序的显式监督，导致训练-推理不匹配和次优解码行为。

Method: 提出TRIMS框架，将轨迹监督注入标准的掩码扩散语言模型训练中。使用自回归教师的轻量级信号指导轨迹感知掩码策略，而非依赖昂贵的DLM蒸馏，鼓励模型学习更有效的解码顺序。

Result: 在LLaDA和Dream模型上的数学和代码基准测试表明，TRIMS相比标准MDLM训练和无训练加速基线显著改善了准确性-并行性平衡，同时以更低的训练成本实现了与先前蒸馏方法竞争的性能。

Conclusion: TRIMS通过轨迹引导监督改善了扩散语言模型的解码轨迹，验证了轨迹监督对DLMs的有效性，为低延迟并行解码提供了更实用的解决方案。
...
TL;DR: 该论文从统计假设检验角度重新解释TF-IDF，通过惩罚似然比检验框架推导出类似TF-IDF的评分公式，为传统术语加权方案提供统计理论基础。


<details>
  <summary>Details</summary>
Motivation: TF-IDF作为广泛使用的术语重要性评估公式，缺乏坚实的统计理论基础。作者希望通过统计假设检验框架为TF-IDF提供理论解释，并探索基于假设检验的术语加权方案开发潜力。

Method: 采用惩罚似然比检验框架：备择假设使用带有伽马惩罚项的beta-二项分布族建模文档集合，捕捉词语突发性（过离散）；零假设使用二项分布建模，忽略词语突发性。通过该检验统计量推导出类似TF-IDF的术语加权方案。

Result: 基于该检验统计量推导出的术语加权方案在文档分类任务上与TF-IDF表现相当，验证了TF-IDF可以从统计假设检验角度得到解释。

Conclusion: 该研究为TF-IDF提供了统计视角的见解，强调了假设检验框架在推进术语加权方案开发方面的潜力，为传统信息检索方法提供了理论基础。
...
TL;DR: OmniVoice是一个支持600多种语言的零样本多语言文本转语音模型，采用新型扩散语言模型风格的非自回归架构，直接从文本映射到多码本声学标记，在中文、英文和多语言基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统离散非自回归模型在复杂的文本-语义-声学两阶段流程中存在性能瓶颈，需要开发更简化的直接映射方法以实现大规模多语言零样本TTS。

Method: 1. 采用扩散语言模型风格的离散非自回归架构；2. 直接从文本映射到多码本声学标记；3. 使用全码本随机掩码策略进行高效训练；4. 从预训练大语言模型初始化以确保可懂度；5. 基于581k小时开源多语言数据集训练。
...
TL;DR: AfrIFact是一个包含10种非洲语言和英语的事实核查数据集，用于评估自动事实核查的各个步骤，研究发现现有模型在跨语言检索和非洲语言事实核查方面存在不足，但few-shot提示和任务特定微调能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在线声明的真实性核查对信息获取受限的社区尤为重要，特别是在医疗保健和文化等关键领域。由于非洲语言资源匮乏，现有事实核查系统在这些语言上表现不佳，需要专门的数据集和评估来推动相关研究。

Method: 构建了AfrIFact数据集，涵盖10种非洲语言和英语，支持信息检索、证据提取和事实核查三个步骤。评估了现有嵌入模型的跨语言检索能力，测试了LLM在非洲语言上的事实核查性能，并探索了few-shot提示和任务特定微调的效果。

Result: 最佳嵌入模型缺乏跨语言检索能力；文化和新闻文档比医疗保健文档更容易检索；LLM在非洲语言上的事实核查能力不足；few-shot提示使AfriqueQwen-14B性能提升43%；任务特定微调进一步将事实核查准确率提升26%。

Conclusion: AfrIFact数据集填补了低资源语言事实核查的空白，揭示了现有模型在非洲语言上的局限性，few-shot提示和微调是有效的改进方法，鼓励更多研究关注低资源语言的信息检索、证据提取和事实核查。
...
TL;DR: 本文系统研究了在固定数据预算下，预训练语料规模与检索库规模之间的权衡关系，提出了三维扩展框架来建模性能与模型大小、预训练标记和检索语料规模的关系，为理解何时以及如何使用检索增强预训练提供了量化基础。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）通过在测试时提供相关上下文来提升语言模型在知识密集型任务中的性能，但预训练获得的参数化知识与通过检索访问的非参数化知识之间的关系尚不明确，特别是在固定数据预算下的权衡关系需要系统研究。

Method: 使用OLMo-2架构训练从3000万到30亿参数的模型，在DCLM数据上训练最多1000亿标记，同时变化预训练数据规模（参数数的1-150倍）和检索库规模（1-20倍），并在推理、科学问答和开放域问答等多样化基准上评估性能。

Result: 检索在所有模型规模上都持续优于仅参数化基线；提出了三维扩展框架来建模性能与模型大小、预训练标记和检索语料规模的关系；通过该扩展流形可以估计固定数据预算在预训练和检索之间的最优分配，发现检索的边际效用强烈依赖于模型规模、任务类型和预训练饱和程度。

Conclusion: 研究结果为理解何时以及如何用检索补充预训练提供了量化基础，为可扩展语言建模系统的数据资源分配提供了实用指导，表明检索的效用取决于多个因素，需要在系统设计中综合考虑。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在动态环境中难以自主演化协调策略，主要是因为粗粒度的全局结果掩盖了局部策略优化所需的因果信号。这被识别为多智能体信用分配问题，在传统多智能体强化学习中已有研究，但在基于LLM的系统中尚未得到充分解决。

Method: LangMARL框架将合作式多智能体强化学习中的信用分配和策略梯度进化引入语言空间。具体包括：1）智能体级别的语言信用分配；2）在语言空间中进行梯度进化的策略改进；3）从重放轨迹中总结任务相关的因果关系，提供密集反馈并改善稀疏奖励下的收敛性。

Result: 在多种合作式多智能体任务上的广泛实验表明，LangMARL提高了样本效率、可解释性，并展现出强大的泛化能力。

Conclusion: LangMARL成功地将传统多智能体强化学习中的信用分配机制引入语言空间，解决了LLM智能体在动态协调任务中的策略演化瓶颈，为基于语言的多智能体系统提供了有效的学习框架。
...
TL;DR: 受果蝇全脑连接组启发，提出随机注意力机制，通过随机置换token序列实现全局注意力，在保持线性计算复杂度的同时获得指数级增长的感受野。


<details>
  <summary>Details</summary>
Motivation: 果蝇大脑连接组虽然连接概率极低（仅0.02%），但平均最短路径只有4.4跳，表明其长距离连接作为随机捷径实现了高效的全局通信。这启发了作者将类似的随机路由机制应用于注意力机制中。

Method: 提出随机注意力（SA），在滑动窗口注意力（SWA）前对token序列进行随机置换，然后在注意力计算后恢复原始顺序。这样在保持O(nw)每层计算预算的同时，将固定局部窗口转换为随机全局窗口。通过多层独立采样的置换，感受野呈指数增长。

Result: 在从头预训练语言模型中，门控SA+SWA组合获得最佳平均零样本准确率；在Qwen3-8B和Qwen3-30B-A3B的无训练推理中，SA持续优于SWA，并在可比计算预算下匹配或超过混合块注意力。

Conclusion: 受连接组启发的随机路由是提高高效注意力表达能力的实用原语，与现有的线性和稀疏方法互补，为改进注意力机制提供了新思路。
...
TL;DR: 该论文系统比较了心理健康文本分类的多种优化策略，从基础基线到参数高效微调再到偏好优化，提供了实用的方法选择框架。


<details>
  <summary>Details</summary>
Motivation: 心理健康文本分类领域虽然广泛采用现代适应方法，但缺乏关于选择何种优化策略、何时使用以及为何有效的实用指导，需要系统性的比较研究来填补这一空白。

Method: 采用系统性比较研究方法：1) 建立经典和编码器基线；2) 在多种目标和优化设置下评估LoRA/QLoRA参数高效监督微调；3) 评估DPO、ORPO、KTO等偏好优化方法，包括类别平衡训练；4) 重点分析目标公式、适配器选择、优化器行为、上下文窗口和类别平衡干预对性能的影响。

Result: 优化效果高度依赖于具体方法：某些方法提供稳定、可迁移的增益，而其他方法对配置和数据平衡敏感。偏好优化在不同目标间表现出巨大差异，表明方法选择比简单添加偏好训练阶段更为关键。

Conclusion: 提出了心理健康NLP的清晰优化路径：从透明基线开始，应用受控微调，仅在可证明增益的情况下选择性使用偏好优化。这为超越架构选择的有效训练策略提供了可重复且实用的框架。
...
TL;DR: 本文基于先前对Mapudungun语动词词根的确认工作，进一步研究这些已确认为动词词根的配价分类，使用该语言自身的形态结构来分析动词形式中词根与各种后缀的可允许和受限制组合。


<details>
  <summary>Details</summary>
Motivation: 在先前确认Mapudungun语动词词根的基础上，本研究旨在通过分析动词形式中词根与后缀的组合模式，对这些动词进行配价分类，从而改进形态分析器并增进对Mapuche动词配价问题的理论理解。

Method: 使用Mapudungun语自身的形态结构学方法，具体通过检查动词形式中词根或动词词干与各种后缀的可允许和受限制组合模式，来确定动词的配价分类。

Result: 研究结果将整合到形态分析器Dungupeyum中，所有验证过的发现都将被纳入系统，从而改进该分析器的性能。

Conclusion: 本研究不仅有助于改进Mapudungun语的形态分析工具，还希望从理论角度促进对Mapuche动词形式配价相关问题的认识和理解。
...
TL;DR: EmoScene是一个基于情感理论的多维情感理解基准，包含4,731个情境丰富的场景，标注了基于Plutchik基本情感的8维情感向量。研究评估了6个指令调优大语言模型的零样本表现，发现性能有限，并提出了基于贝叶斯推理的纠缠感知框架来改进情感预测的结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有情感理解基准大多基于短文本和预定义情感标签，将情感理解简化为独立标签预测，忽略了情感之间的结构化依赖关系。自然语言中的情感理解本质上是一个多维推理问题，涉及多个情感信号通过上下文、人际关系和情境线索的交互。

Method: 1) 引入EmoScene基准：包含4,731个情境丰富的场景，标注基于Plutchik基本情感的8维情感向量；2) 评估6个指令调优大语言模型在零样本设置下的表现；3) 提出纠缠感知贝叶斯推理框架，利用情感共现统计进行联合后验推理，改进预测的结构一致性。

Result: 1) 最佳模型仅达到0.501的Macro F1分数，表明上下文感知的多标签情感预测具有挑战性；2) 提出的贝叶斯推理框架显著提升了较弱模型的性能（如Qwen2.5-7B提升了+0.051 Macro F1），改善了预测的结构一致性。

Conclusion: EmoScene为研究多维情感理解和当前语言模型的局限性提供了一个具有挑战性的基准。情感很少独立发生，考虑情感之间的纠缠关系可以显著改进预测性能，特别是在结构一致性方面。
...
TL;DR: 本文提出StanceMoE模型，这是一个基于BERT编码器的上下文增强混合专家架构，用于演员级立场检测，通过六个专家模块捕捉互补的语言信号，在StanceNakba 2026数据集上取得了94.26%的宏F1分数。
...
Method: 提出StanceMoE模型，基于微调的BERT编码器构建上下文增强的混合专家架构。模型集成六个专家模块：全局语义方向、显著词汇线索、从句级焦点、短语级模式、框架指标和对比驱动的话语转换。采用上下文感知的门控机制动态加权专家贡献，实现基于输入特征的自适应路由。

Result: 在StanceNakba 2026 Subtask A数据集（包含1,401个英文文本）上进行实验，StanceMoE取得了94.26%的宏F1分数，优于传统基线和替代的BERT变体模型。

Conclusion: StanceMoE通过混合专家架构有效捕捉了立场表达的多维度语言信号，在演员级立场检测任务上表现出色，证明了显式建模多样化立场表达模式的重要性。
...
TL;DR: GPT-NL公共语料库是目前最大的荷兰语开放许可语料库，包含36B荷兰语token和大量其他语言数据，旨在促进合法、有用、无害的商业语言模型开发。


<details>
  <summary>Details</summary>
Motivation: 为了解决荷兰语高质量、开放许可训练数据的缺乏问题，为开发合法、有用、无害的商业语言模型提供基础资源。

Method: 整合现有大型语料库（如Common Corpus和Common Crawl）并进行合规性筛选，同时创建新的荷兰语特定集合，包括与组织合作收集的内容和合成增强内容。

Result: 构建了包含21个荷兰语集合（36B荷兰语token）的语料库，以及207B英语、232B代码、48B德语/丹麦语token，所有数据采用CC-BY许可，已在Hugging Face Hub公开。

Conclusion: GPT-NL公共语料库为荷兰语NLP研究和商业应用提供了高质量、合规的开放数据资源，促进了荷兰语语言模型的开发。
...
TL;DR: 该研究探索大语言模型学习新语言的训练动态，提出基于认知功能专门化的层选择方法CogSym，仅微调25%最外层即可达到接近全微调的性能。


<details>
  <summary>Details</summary>
Motivation: 适应大语言模型到新语言的过程昂贵且不透明，理解模型如何获得新语言和多语言能力对于实现高效适应至关重要。现有研究主要关注训练好的模型如何处理多语言指令，而忽略了训练过程中获得新语言的机制。

Method: 通过两个功能性认知专门化视角研究训练动态：语言感知（输入理解）和语言产生（输出生成）。在低资源语言上进行实验，通过从模型输入和输出方向进行层消融扫描，展示感知和产生专门化在不同模型区域的出现模式。基于观察到的专门化模式，提出CogSym层选择启发式方法。

Result: 仅微调25%最外层（早期和晚期层）即可在下游任务中达到与全微调基线相差2-3%的性能。CogSym与LoRA等适配器方法表现一致，展示了超越全微调的泛化能力。

Conclusion: 这些发现为理解LLMs如何学习新语言提供了见解，并推动了可访问和包容性语言建模的发展。提出的CogSym方法能够实现有效适应，仅需微调少量层即可获得接近全微调的性能。
...
TL;DR: 该研究构建了一个具有短程相互作用的随机语言模型，发现即使模型只参考长度不随句子长度变化的上下文，也会发生相变，表明语言模型的有限温度相变源于语言的内在本质而非长程相互作用。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明长程相互作用可以诱导相变，但尚不清楚语言模型中观察到的相变是否源于传统自旋模型中不存在的真正语言特性。本研究旨在探究短程相互作用下是否仍会发生相变，以确定相变的本质来源。

Method: 构建了一个属于乔姆斯基层级中上下文敏感文法的随机语言模型，该模型允许显式引用上下文，但只参考长度不随句子长度变化的上下文（即短程相互作用）。通过数值方法研究其统计特性。

Result: 研究发现即使模型只参考有限长度的上下文（短程相互作用），仍然会发生相变。这表明相变的发生与长程相互作用无关。

Conclusion: 语言模型中的有限温度相变是由语言的内在本质真正诱导的，而不是由长程相互作用引起的。这一发现有助于理解语言统计力学中相变的根本起源。
...
TL;DR: 提出多模态分析管道，结合自动转录、方面情感分析和语义场景分类，用于分析YouTube Shorts中地缘政治事件的报道，以以色列-哈马斯战争为例，发现不同媒体情感表达差异，小规模领域适应模型优于大型模型。


<details>
  <summary>Details</summary>
Motivation: YouTube Shorts已成为新闻消费的重要平台，但关于地缘政治事件在此格式中的呈现方式研究有限，需要系统分析短格式视频中战争报道的多模态特征。

Method: 开发多模态分析管道，结合自动转录、方面情感分析(ABSA)和语义场景分类，分析2300多个冲突相关Shorts和94000多个视觉帧，比较不同国家资助媒体的报道差异。

Result: 不同媒体在特定方面的情感表达存在差异且随时间变化；场景分类反映真实世界事件的视觉线索；小规模领域适应模型在情感分析上优于大型transformer和LLM。
...
TL;DR: VRF通过变分推理将用户偏好表示为共享偏好空间中的分布，利用Wasserstein距离匹配和方差衰减损失，在稀疏数据下实现更准确可靠的个性化LLM奖励分解。


<details>
  <summary>Details</summary>
Motivation: 现有奖励分解方法从稀疏数据中孤立地估计用户权重作为确定点，导致推断不准确和不可靠。需要一种能够处理不确定性的框架来改善个性化LLM的奖励分解。

Method: 提出变分奖励分解（VRF）：1）通过变分编码器推断用户分布；2）通过Wasserstein距离匹配与共享概率基函数推导权重；3）通过方差衰减损失降低不确定估计的影响。

Result: 在三个基准测试中，VRF在所有基线方法上表现更优，包括已见和未见用户、少样本场景以及不同不确定性水平，且优势延伸至下游对齐任务。

Conclusion: VRF通过变分推理和不确定性感知机制，显著提升了奖励分解在稀疏数据下的准确性和可靠性，为个性化LLM提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索作者是否在其出版作品的信息论新颖性曲线中具有可识别的独特"指纹"，验证作者声音是否在文本新颖性展开过程中留下可测量的痕迹

Method: 使用Books3（52,796本书，759位合格作者）和PG-19（28,439本书，1,821位合格作者）两个语料库，分析作者在书籍层面（平均新颖性、速度、体积、迂回性）和章节层面（SAX主题模式滑动窗口）的信息论新颖性特征

Result: 1. 书籍层面：标量动态特征能显著高于随机水平识别43%的作者；2. 章节层面：SAX主题模式滑动窗口的归因效果比随机水平高30倍，远超书籍层面的标量特征；3. 这些信号是互补的而非冗余的；4. 指纹特征部分与体裁相关，但约四分之一作者在体裁内仍保持可识别性；5. 古典作家（吐温、奥斯汀、吉卜林）与现代作家指纹强度相当

Conclusion: 作者声音确实在文本新颖性展开过程中留下可测量的多尺度痕迹，这些指纹特征在不同尺度上互补，部分特征具有体裁独立性，且这一现象并非当代出版惯例的产物
...
TL;DR: 研究发现开源大语言模型在上下文学习中表现出类似序列回忆的模式，这种模式与认知科学中的自由回忆范式相关，主要由"归纳头"机制驱动。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出强大的上下文学习能力，但模型如何追踪和检索上下文信息仍不清楚。研究者希望探索LLMs在信息处理过程中是否存在类似人类记忆的认知模式。

Method: 借鉴认知科学中的自由回忆范式，通过系统消融实验分析多个开源LLMs。重点关注"归纳头"（专门关注当前标记后出现的标记的注意力头）的作用，通过移除高归纳分数头与随机头进行对比。

Result: LLMs表现出类似序列回忆的模式，倾向于给重复标记后的标记分配峰值概率。移除高归纳分数头显著减少了+1滞后偏差，而移除随机头则无此效果。移除高归纳分数头也更大程度损害了少样本学习中的序列回忆性能。

Conclusion: 归纳头与Transformer中的时间上下文处理存在机制特异性连接，这些头对于上下文学习中的有序检索和序列回忆行为特别重要，揭示了LLMs信息处理的认知类似性。
...
Method: 包含五个核心组件：MoE-LoRA路由机制、残差提升内循环、课程排序外循环、零空间投影防止遗忘、基于结果的sigmoid元路由器。通过冻结适配器堆栈实现领域知识的模块化封装。

Result: 在TinyLlama-1.1B和Gemma 3 12B IT上验证，MoE-LoRA比参数匹配的单LoRA收敛快2.5倍，残差提升突破单堆栈上限，路由系统恢复被无门控堆叠破坏的生成质量。

Conclusion: 领域堆栈编码的是可迁移的认知原语（指令跟随清晰度、数学推理、程序逻辑、思维链结构）而非领域特定知识，元路由器能够发现这些认知原语并实现跨领域组合。
...
TL;DR: 简单自蒸馏（SSD）通过采样模型输出并微调，无需验证器或强化学习即可提升LLM代码生成能力


<details>
  <summary>Details</summary>
Motivation: 探索是否能在不使用验证器、教师模型或强化学习的情况下，仅通过模型自身原始输出来提升大语言模型的代码生成能力

Method: 采用简单自蒸馏方法：以特定温度和截断配置从模型中采样解决方案，然后使用标准监督微调在这些样本上进行微调

Result: SSD将Qwen3-30B-Instruct在LiveCodeBench v6上的pass@1从42.4%提升至55.3%，增益主要集中在更难的问题上，并在4B、8B和30B规模的Qwen和Llama模型中均有效

Conclusion: SSD通过重塑上下文相关的token分布，在精度与探索之间取得平衡，为改进LLM代码生成提供了互补的后训练方向
...
TL;DR: RELISH是一种用于大语言模型文本回归的新型轻量级架构，通过迭代精炼潜在状态并直接预测标量值，在多个数据集和模型上优于现有方法，同时保持高参数效率。
...
Method: RELISH通过交叉注意力在冻结的LLM表示上迭代精炼学习到的潜在状态，然后通过线性回归器将最终状态映射到点估计。它直接预测标量值，而不是将数值目标解码为文本或聚合多个生成输出。

Result: 在五个数据集、四个LLM主干和两种LLM训练机制上，RELISH始终优于所有三大类LLM回归基线方法。同时保持高度参数效率，仅需3.4-3.7M可训练参数（仅0.01-0.04%额外开销），远少于基于LoRA的替代方案。
...
TL;DR: YC-Bench是一个评估LLM智能体在长期复杂任务中战略连贯性的基准测试，模拟初创公司一年运营，包含数百个回合的决策制定


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体处理日益复杂的任务，需要评估它们是否能在长期视野中保持战略连贯性：在不确定性下规划、从延迟反馈中学习、以及当早期错误累积时进行适应

Method: 引入YC-Bench基准测试，让智能体在模拟初创公司的一年运营中管理员工、选择任务合同、在部分可观察环境中保持盈利能力，评估了12个模型（包括专有和开源模型）

Result: 只有三个模型持续超过起始资本20万美元，Claude Opus 4.6达到最高平均最终资金127万美元，GLM-5达到121万美元但推理成本低11倍；草稿纸使用是成功的最强预测因子，对抗性客户检测是主要失败模式（占破产的47%）

Conclusion: 前沿模型仍存在明显的失败模式（如过度并行化），展示了长期性能的能力差距；YC-Bench是开源、可复现且可配置的基准测试
...
<details>
  <summary>Details</summary>
Motivation: 标准Transformer在推理时计算扩展效率低下，传统循环策略计算开销高，KV缓存随模型深度膨胀。需要一种既能提升推理能力又能保持高效推理的方法。
...
### [150] [Signal Constellations with Enhanced Energy Efficiency for High-Speed Communication Systems](https://arxiv.org/abs/2604.00710)
*Mark Bykhovskiy*

Main category: cs.IT

TL;DR: 提出SCOPT方法构建多维信号星座，通过增加归一化信号持续时间而非编码或功率来提升最小欧氏距离，实现低于香农极限的可靠通信，保持与QAM/APSK兼容的简单结构。


<details>
  <summary>Details</summary>
Motivation: 传统通信系统受香农极限约束，需要编码或增加发射功率来提高性能。本文旨在开发一种新方法，在不依赖编码或增加功率的情况下，通过优化信号星座结构来提升能量效率，突破传统香农型星座的限制。

Method: 提出SCOPT（信号星座优化）方法，通过增加归一化信号持续时间来增大信号间的最小欧氏距离，而非依赖编码或增加发射功率。方法推导了解调错误概率和相对于香农极限的能量损失解析表达式，并保持与标准调制方案（如QAM和APSK）兼容的简单结构。

Result: SCOPT能够在某些可靠通信机制下，使所需信噪比低于传统香农极限。数值分析表明，SCOPT在能量效率方面显著优于传统香农型星座（SCSH），同时需要更短的信号持续时间。

Conclusion: SCOPT方法为高速通信系统提供了一种新颖的信号星座构造方案，能够在保持与现有调制方案兼容的同时，显著提升能量效率并突破传统香农极限的约束，具有实际应用价值。
...
### [151] [Optimal Sampling and Actuation Policies of a Markov Source over a Wireless Channel](https://arxiv.org/abs/2604.00748)
*Mehrdad Salimnejad,Anthony Ephremides,Marios Kountouris,Nikolaos Pappas*

Main category: cs.IT

TL;DR: 该论文研究N状态马尔可夫过程的实时监控，分析错误信息年龄(AoII)，提出多种调度策略，并引入不确定性下的行动成本(CoAU)来优化执行器决策。


<details>
  <summary>Details</summary>
Motivation: 实时监控N状态马尔可夫过程需要高效的数据管理和及时的信息传播，以实现准确的状态估计和可靠的控制决策。当前系统存在两个主要问题：1) 信息时效性不足影响状态估计准确性；2) 执行器在状态估计不确定时做出错误决策会降低系统性能。

Method: 首先分析错误信息年龄(AoII)，推导其在多种调度策略下的闭式表达式。然后提出约束优化问题来最小化平均AoII。针对执行器决策问题，引入不确定性下的行动成本(CoAU)，提出随机化执行策略，推导无错误行动概率的闭式表达式，并建立优化问题寻找最优策略。

Result: 推导了AoII在多种调度策略下的闭式表达式，解决了约束优化问题得到最优采样和传输策略。提出的CoAU框架和随机化执行策略显著减少了执行器的错误行动，特别是在接收端对状态估计不确定或控制成本较高的情况下效果更明显。

Conclusion: 该研究为实时监控系统提供了全面的信息时效性和执行器决策优化框架。AoII分析帮助优化信息传播策略，而CoAU框架有效解决了状态估计不确定性下的执行器决策问题，显著提升了系统性能。
...
### [152] [SynDe: Syndrome-guided Decoding of Raw Nanopore Reads](https://arxiv.org/abs/2604.01054)
*Anisha Banerjee,Roman Sokolovskii,Thomas Heinis,Antonia Wachter-Zeh,Eirik Rosnes,Alexandre Graell i Amat*

Main category: cs.IT

TL;DR: 提出两种算法：PrimerSeeker用于在原始纳米孔测序数据中高效检测引物序列，SynDe解码器支持任何具有低复杂度图形表示的线性纠错码，解决纳米孔测序高错误率问题。


<details>
  <summary>Details</summary>
Motivation: 纳米孔测序技术错误率很高，需要高效纠错。现有方法使用卷积码与basecaller耦合的解码器，但只能支持有限编码类别且解码复杂度高。

Method: 提出两种算法：1) PrimerSeeker：在原始纳米孔测序读数中高效检测引物序列；2) SynDe：在相同原始读数上操作，支持任何具有低复杂度图形表示的线性纠错码。

Result: PrimerSeeker提供接近现有方法的引物位置估计，更适合测序过程中的实时引物检测。SynDe在使用周期性标记增强的卷积码时表现良好，通常接近或超过现有算法性能，且时间复杂度更低。SynDe产生的置信度分数能可靠识别应丢弃的输出。
...
### [153] [On the Construction of Recursively Differentiable Quasigroups and an Example of a Recursive $[4,2,3]_{26}$-Code](https://arxiv.org/abs/2604.01105)
*Petr Klimov*

Main category: cs.IT

TL;DR: 该论文解决了递归MDS码存在性的最后一个未决情况q=26，通过完美循环Mendelsohn设计构造递归可微拟群和递归MDS码，并改进了小阶递归n-可微拟群存在性的已知界限。


<details>
  <summary>Details</summary>
Motivation: 1998年Couselo等人提出了递归码和递归可微拟群的概念，并猜想对于维度2、长度4的递归MDS码，除了q∈{2,6}外，在所有有限字母表上都存在。他们验证了除q∈{14,18,26,42}外的所有情况。2008年解决了q=42的情况，现在需要解决最后一个未决情况q=26。

Method: 使用完美循环Mendelsohn设计来构造递归可微拟群和递归MDS码。通过这种方法，作者能够为q=26的情况提供明确的构造，从而完成猜想的证明。

Result: 成功解决了q=26的情况，证明了对于维度2、长度4的递归MDS码，除了q∈{2,6}外，在所有有限字母表上都存在。同时改进了关于小阶递归n-可微拟群存在性的几个已知界限。

Conclusion: 该论文完全解决了Couselo等人1998年提出的关于递归MDS码存在性的猜想，为q=26的情况提供了构造性证明，并通过完美循环Mendelsohn设计的方法推进了递归可微拟群理论的发展。
...
### [154] [How Emotion Shapes the Behavior of LLMs and Agents: A Mechanistic Study](https://arxiv.org/abs/2604.00005)
*Moran Sun,Tianlin Li,Yuwei Zheng,Zhenhong Zhou,Aishan Liu,Xianglong Liu,Yang Liu*

Main category: cs.AI

TL;DR: E-STEER框架通过将情绪作为结构化可控变量嵌入LLM隐藏状态，实现表示层面的情绪干预，研究情绪对推理、生成、安全和多步智能体行为的影响。


<details>
  <summary>Details</summary>
Motivation: 现有情绪感知研究主要将情绪视为表层风格因素或感知目标，忽视了其在任务处理中的机制性作用。情绪在人类认知和表现中扮演重要角色，因此需要探究类似情绪信号是否能够塑造大语言模型和智能体的行为。

Method: 提出E-STEER框架，将情绪作为结构化可控变量嵌入到LLM和智能体的隐藏状态中，实现直接表示层面的干预。该框架允许在隐藏状态层面进行情绪控制，并研究情绪对客观推理、主观生成、安全性和多步智能体行为的影响。

Result: 研究发现情绪与行为之间存在非单调关系，与既有的心理学理论一致。特定情绪不仅能增强LLM的能力，还能提高安全性，并系统性地塑造多步智能体行为。

Conclusion: E-STEER框架为理解情绪在LLM和智能体中的作用提供了可解释的方法，证明了情绪干预能够有效影响模型行为，为开发更符合人类认知特点的AI系统提供了新思路。
...
### [155] [A Safety-Aware Role-Orchestrated Multi-Agent LLM Framework for Behavioral Health Communication Simulation](https://arxiv.org/abs/2604.00249)
*Ha Na Cho*

Main category: cs.AI

TL;DR: 提出一个安全感知、角色编排的多智能体LLM框架，用于模拟支持性行为健康对话，通过协调的角色分化智能体实现对话功能分解和安全审计。


<details>
  <summary>Details</summary>
Motivation: 单智能体LLM系统难以同时支持多样化的对话功能并保持行为健康交流的安全性，需要更安全、功能更全面的对话系统。
...
### [156] [Human-in-the-Loop Control of Objective Drift in LLM-Assisted Computer Science Education](https://arxiv.org/abs/2604.00281)
*Mark Dranias,Adam Whitley*

Main category: cs.AI

TL;DR: 该论文提出了一种基于系统工程和控制理论的人类在环控制教学方法，用于稳定AI辅助编程中的目标漂移问题，通过分离规划与执行、训练学生制定验收标准和架构约束来提升教学效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在计算机科学教育中广泛应用，但AI辅助编程工具常出现目标漂移问题，即局部合理的输出与任务规范偏离。现有教学方法过于依赖特定工具的提示技巧，缺乏长期稳定性。论文从人本主义视角出发，将人类在环控制视为稳定的教育问题而非向AI自主的过渡步骤。

Method: 采用系统工程和控制理论概念，将目标和世界模型作为可配置的操作工件。提出试点本科CS实验室课程，明确分离规划与执行阶段，训练学生在代码生成前制定验收标准和架构约束。部分实验还引入概念对齐的故意漂移，以支持诊断和从规范违规中恢复。

Result: 进行了三组试点设计的敏感性功效分析，比较无结构AI使用、结构化规划、以及结构化规划加注入漂移三种条件，在现实班级规模约束下建立了可检测的效应大小。

Conclusion: 该研究提供了一个理论驱动、方法明确的人类在环教学法基础，使控制能力能够在不断演进的AI工具中可教，为AI辅助编程教育提供了更稳定和可迁移的教学框架。
...
### [157] [Improvisational Games as a Benchmark for Social Intelligence of AI Agents: The Case of Connections](https://arxiv.org/abs/2604.00284)
...
<details>
  <summary>Details</summary>
Motivation: 探索AI智能体在需要社交智能的任务中的推理能力，特别是那些涉及理解其他智能体认知状态、超越自身记忆和演绎推理的复杂场景。
...
Conclusion: Connections游戏为评估AI智能体的社交推理能力提供了有效框架，强调了沟通和协作在展示社交智能中的重要性。
...
### [158] [Signals: Trajectory Sampling and Triage for Agentic Interactions](https://arxiv.org/abs/2604.00356)
...
Method: 提出基于信号的轻量级框架，从实时交互中计算廉价、广泛适用的信号，将其作为结构化属性用于轨迹筛选。信号分为交互（不对齐、停滞、脱离、满意度）、执行（失败、循环）和环境（耗尽）三类，无需模型调用即可计算

Result: 在τ-bench基准测试中，基于信号的采样达到82%的信息丰富率，相比启发式过滤的74%和随机采样的54%，每个信息丰富轨迹的效率增益为1.52倍。优势在不同奖励层级和任务领域均稳健
...
### [159] [Decision-Centric Design for LLM Systems](https://arxiv.org/abs/2604.00414)
*Wei Sun*

Main category: cs.AI

TL;DR: 论文提出了一种决策中心框架，将LLM系统中的控制决策从生成过程中分离出来，使控制成为系统的一个显式可检查层，从而提高可靠性、可控性和可诊断性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM架构中，控制决策（如是否回答、澄清、检索、调用工具等）通常隐含在生成过程中，将评估和行动纠缠在单一模型调用中，导致故障难以检查、约束或修复。

Method: 提出决策中心框架，将决策相关信号与将信号映射到行动的策略分离开来，支持将故障归因于信号估计、决策策略或执行，并支持各模块的独立改进。

Result: 在三个受控实验中，该框架减少了无效行动，提高了任务成功率，并揭示了可解释的故障模式。

Conclusion: 该框架为构建更可靠、可控和可诊断的LLM系统提供了一种通用的架构原则，统一了路由和自适应推理等单步设置，并自然扩展到顺序设置。
...
### [160] [Self-Routing: Parameter-Free Expert Routing from Hidden States](https://arxiv.org/abs/2604.00421)
*Jama Hussein Mohamud,Drew Wagner,Mirco Ravanelli*

Main category: cs.AI

TL;DR: 本文提出了一种名为Self-Routing的无参数路由机制，它直接使用token隐藏状态的指定子空间作为专家logits，完全消除了路由器投影，在保持MoE层其余部分不变的情况下实现了与学习路由器基线竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 研究在MoE设置中专门学习路由器是否严格必要，探索是否可以从隐藏表示本身直接产生有效的路由决策，从而简化MoE架构并减少参数。

Method: 提出Self-Routing方法：使用token隐藏状态的指定子空间直接作为专家logits，完全消除路由器投影模块，保持MoE层其余部分不变。该方法无需额外参数，通过隐藏表示本身实现路由决策。

Result: 在GPT-2规模的语言建模和ImageNet-1K分类任务上，Self-Routing与标准学习路由器基线保持竞争力，同时移除了所有专用路由参数，产生更平衡的专家利用率（平均归一化路由熵提高约17%），无需显式负载平衡损失。在ImageNet-1K的DeiT-S/16上，Self-Routing甚至略微优于相应的学习路由器MoE。

Conclusion: 有效的MoE路由可以从隐藏表示本身产生，而不需要单独的学习路由器模块。Self-Routing作为一种无参数路由机制，在保持性能的同时简化了MoE架构，提高了专家利用率平衡性。
...
### [161] [Towards Reliable Truth-Aligned Uncertainty Estimation in Large Language Models](https://arxiv.org/abs/2604.00445)
*Ponhvoan Srey,Quang Minh Nguyen,Xiaobao Wu,Anh Tuan Luu*

Main category: cs.AI

TL;DR: 本文提出Truth AnChoring (TAC)方法，通过后处理校准将不确定性估计指标映射到与事实对齐的分数，以解决LLM不确定性估计指标在低信息区域失效的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的不确定性估计指标在不同配置下表现不稳定，限制了其实用性。这些指标大多源自模型行为，而非明确基于LLM输出的实际正确性，导致在低信息区域无法有效区分。

Method: 提出Truth AnChoring (TAC)后处理校准方法，将原始不确定性分数映射到与事实对齐的分数。该方法即使在噪声和少样本监督下也能学习到良好校准的不确定性估计，提供实用的校准协议。

Result: TAC方法能够有效校准不确定性估计指标，支持学习到良好校准的不确定性估计，为LLM提供更可靠的不确定性估计。
...
### [162] [The Silicon Mirror: Dynamic Behavioral Gating for Anti-Sycophancy in LLM Agents](https://arxiv.org/abs/2604.00478)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越倾向于优先满足用户验证而非事实准确性，这种现象被称为"谄媚"。需要开发能够动态检测和应对用户说服策略的系统来保持AI的事实完整性。

Method: 提出Silicon Mirror框架，包含三个组件：1) 行为访问控制系统，基于实时谄媚风险评分限制上下文层访问；2) 特质分类器，在多轮对话中识别说服策略；3) 生成器-批评家循环，审计员否决谄媚草稿并触发带有"必要摩擦"的重写。

Result: 在TruthfulQA对抗场景评估中，原始Claude谄媚率为12.0%，静态防护为4.0%，Silicon Mirror为2.0%（相对减少83.3%）。跨模型评估显示Gemini 2.5 Flash基线谄媚率46.0%，Silicon Mirror实现69.6%的显著减少（p<0.001）。

Conclusion: Silicon Mirror能有效减少LLM的谄媚行为，识别了RLHF训练模型特有的"先验证后纠正"失败模式，为保持AI事实完整性提供了动态防护框架。
...
### [163] [Adaptive Parallel Monte Carlo Tree Search for Efficient Test-time Compute Scaling](https://arxiv.org/abs/2604.00510)
*Hongbeen Kim,Juhyun Lee,Sanghyeon Lee,Kwanghoon Choi,Jaehyuk Huh*

Main category: cs.AI

TL;DR: 提出负向提前退出和自适应增强机制，显著降低MCTS在LLM推理中的p99延迟，同时保持准确性和提升吞吐量


<details>
  <summary>Details</summary>
Motivation: MCTS作为有效的测试时计算扩展方法，在实际应用中存在执行时间高度可变的问题，导致严重的尾部延迟。现有优化方法如正向提前退出在有利情况下有效，但在搜索无进展时效果有限。

Method: 引入负向提前退出机制，修剪无生产力的MCTS轨迹；设计自适应增强机制，重新分配回收的计算资源以减少并发搜索间的资源竞争。这些技术集成到vLLM框架中。

Result: 显著降低了p99端到端延迟，同时提高了吞吐量并保持了推理准确性。

Conclusion: 提出的负向提前退出和自适应增强机制有效解决了MCTS在LLM推理中的延迟问题，实现了延迟降低、吞吐提升和准确性保持的三重优化。
...
### [164] [Does Unification Come at a Cost? Uni-SafeBench: A Safety Benchmark for Unified Multimodal Large Models](https://arxiv.org/abs/2604.00547)
*Zixiang Peng,Yongxiu Xu,Qinyi Zhang,Jiexun Shen,Yifan Zhang,Hongbo Xu,Yubin Wang,Gaopeng Gou*

Main category: cs.AI

TL;DR: 本文提出了Uni-SafeBench基准测试和Uni-Judger评估框架，用于评估统一多模态大模型在理解和生成任务中的安全性，发现模型统一化会显著降低底层LLM的内在安全性。


<details>
  <summary>Details</summary>
Motivation: 统一多模态大模型整合了理解和生成能力，虽然提升了性能，但也引入了未被充分探索的安全挑战。现有安全基准主要关注孤立的理解或生成任务，无法评估UMLMs在统一框架下处理多样化任务时的整体安全性。

Method: 提出了Uni-SafeBench基准测试，包含6个主要安全类别和7种任务类型。开发了Uni-Judger框架，有效解耦上下文安全性和内在安全性，进行系统性评估。

Result: 评估发现：1) 统一化过程虽然增强了模型能力，但显著降低了底层LLM的内在安全性；2) 开源UMLMs的安全性远低于专门用于生成或理解任务的多模态大模型。

Conclusion: 统一多模态大模型存在严重的安全风险，需要系统性评估和应对。开源所有资源以暴露这些风险，促进更安全的AGI发展。
...
TL;DR: 企业采用大语言模型面临幻觉、领域漂移和合规性挑战，本文提出基于本体约束的神经符号架构，通过三层本体框架显著提升准确性、合规性和角色一致性。


<details>
  <summary>Details</summary>
Motivation: 企业采用大语言模型受到幻觉、领域漂移和无法在推理层面执行监管合规性的限制，需要一种能够约束LLM推理的架构来确保企业级应用的可靠性和合规性。

Method: 在Foundation AgenticOS平台上实现神经符号架构，采用三层本体框架（角色、领域、交互本体），通过非对称神经符号耦合让符号本体知识约束智能体输入，并提出扩展到输出约束的机制。

Result: 在五个行业（金融科技、保险、医疗、越南银行、越南保险）的600次实验运行中，本体耦合智能体在度量准确性、监管合规性和角色一致性方面显著优于无基础智能体，特别是在LLM参数知识最弱的越南本地化领域改进最大。

Conclusion: 本体约束的神经符号架构能有效解决企业LLM应用的挑战，提出了正式的神经符号耦合模式，并发现本体基础价值与LLM训练数据覆盖度成反比关系，已在21个行业垂直领域部署650+智能体。
...
### [166] [CircuitProbe: Predicting Reasoning Circuits in Transformers via Stability Zone Detection](https://arxiv.org/abs/2604.00716)
*Rajkiran Panuganti*

Main category: cs.AI

TL;DR: CircuitProbe：一种快速预测Transformer语言模型中推理电路位置的方法，通过激活统计在CPU上5分钟内完成，相比传统方法提速3-4个数量级。


<details>
  <summary>Details</summary>
Motivation: 当前寻找语言模型中的推理电路需要耗费大量计算资源（25 GPU小时/模型），需要一种更高效的方法来定位这些能提升推理性能的连续层块。

Method: CircuitProbe通过分析激活统计来预测电路位置：早期层使用表示变化的导数检测稳定性电路，晚期层使用异常评分检测幅度电路。仅需10个校准样本，支持多语言。

Result: 在9个模型（6种架构，包括2025年模型）上验证，CircuitProbe的预测与最优电路匹配或在2层以内。层复制对3B参数以下模型有益，但对7B+模型性能下降。

Conclusion: CircuitProbe提供了一种快速高效的推理电路定位方法，特别适用于小型语言模型，为模型推理优化提供了实用的缩放技术。
...
### [167] [Preference Guided Iterated Pareto Referent Optimisation for Accessible Route Planning](https://arxiv.org/abs/2604.00795)
...
<details>
  <summary>Details</summary>
Motivation: 为满足不同人群（特别是无障碍需求者）的个性化路线规划需求，需要一种直观的用户交互方式，避免计算完整的帕累托前沿以减少计算负担和等待时间。

Method: 提出偏好引导迭代帕累托参考优化（PG-IPRO）算法，用户可对路线提供反馈（指定哪些目标应进一步最小化或可放松），系统迭代优化而不计算完整的帕累托前沿。

Result: 相比基于信息增益的交互方法，PG-IPRO在早期迭代中交互更直观有效；由于不计算完整的帕累托前沿，计算效率更高，用户等待时间更短。

Conclusion: PG-IPRO为城市路线规划提供了一种高效、直观的用户交互方法，特别适合满足不同无障碍需求和偏好的个性化路线优化。
...
### [168] [Beyond Symbolic Solving: Multi Chain-of-Thought Voting for Geometric Reasoning in Large Language Models](https://arxiv.org/abs/2604.00890)
*Md. Abu Bakor Siddique,Shahrin Hossain,Sadman Ahmed Siam,Syed Rifat Raiyan,Hasan Mahmud,Md Kamrul Hasan*

Main category: cs.AI

TL;DR: MARS-GPS通过生成多个并行推理路径，结合Python代码执行进行数值验证，使用token级熵作为置信度信号进行排序，并通过多阶段投票和自验证管道聚合答案，显著提升了几何问题求解的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有几何问题求解方法主要关注图表描述与文本的同步以及问题求解，采用神经、符号或神经符号方法，但只解决了图表理解和符号操作两个需求，而逻辑推理能力不足，通常仅限于单一思维链推理。

Method: 提出MARS-GPS方法：1) 生成多个并行推理路径；2) 使用Python代码执行进行数值验证；3) 利用token级熵作为置信度信号对推理路径进行排序；4) 通过多阶段投票和自验证管道聚合最终答案。

Result: 在Geometry3K数据集上，MARS-GPS使用8个并行推理路径达到88.8%的准确率，比之前的最优方法提升了近11%。当推理路径数量从1增加到16时，准确率持续提升（在消融子集上提升6.0%）。

Conclusion: MARS-GPS通过多路径并行推理、代码验证和置信度排序的方法，有效解决了几何问题求解中逻辑推理能力不足的问题，显著提升了模型在几何问题求解任务上的性能。
...
### [169] [PsychAgent: An Experience-Driven Lifelong Learning Agent for Self-Evolving Psychological Counselor](https://arxiv.org/abs/2604.00931)
*Yutao Yang,Junsong Li,Qianjun Pan,Jie Zhou,Kai Chen,Qin Chen,Jingyuan Zhao,Ningning Zhou,Xin Li,Liang He*

Main category: cs.AI

TL;DR: 提出了一种经验驱动的终身学习AI心理咨询师PsychAgent，通过记忆增强规划引擎、技能进化引擎和强化内化引擎实现持续自我进化，在多轮对话中表现优于通用大模型和专业基线。
...
Method: 1. 记忆增强规划引擎：为纵向多轮对话设计，通过持久记忆和策略规划确保治疗连续性；2. 技能进化引擎：从历史咨询轨迹中提取基于实践的新技能；3. 强化内化引擎：通过拒绝微调将进化技能整合到模型中，提升多样化场景下的性能。

Result: 在所有评估维度上，该方法均优于通用大模型（如GPT-5.4、Gemini-3）和领域特定基线模型。结果表明终身学习能够提高多轮咨询回应的连贯性和整体质量。

Conclusion: 经验驱动的终身学习框架PsychAgent能够有效提升AI心理咨询师的能力，使其能够像人类专家一样通过实践经验持续进化，在多轮对话中表现出更好的连贯性和质量。
...
### [170] [Adversarial Moral Stress Testing of Large Language Models](https://arxiv.org/abs/2604.01108)
*Saeid Jamshidi,Foutse Khomh,Arghavan Moradi Dakhel,Amin Nikanjam,Mohammad Hamdaqa,Kawser Wazed Nafi*

Main category: cs.AI

TL;DR: 本文提出了对抗性道德压力测试（AMST）框架，用于评估大语言模型在多轮对抗交互中的伦理鲁棒性，发现传统单轮评估会遗漏重要的行为不稳定性和退化模式。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要依赖单轮测试和聚合指标（如毒性分数、拒绝率），难以检测现实多轮交互中可能出现的行为不稳定、罕见但高影响的伦理失败以及渐进退化效应。

Method: 提出AMST框架：对提示应用结构化压力变换，通过分布感知的鲁棒性指标评估模型行为，这些指标捕捉交互轮次间的方差、尾部风险和时序行为漂移。
...
### [171] [Detecting Multi-Agent Collusion Through Multi-Agent Interpretability](https://arxiv.org/abs/2604.01151)
*Aaron Rose,Carissa Cullen,Brandon Gary Kaplowitz,Christian Schroeder de Witt*

Main category: cs.AI

TL;DR: 该研究提出了NARCBench基准，用于评估多智能体系统中的共谋检测，开发了五种基于模型内部激活的探测技术，在分布偏移下实现了0.60-0.86的AUROC性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在多智能体系统中的部署增加，它们带来了隐蔽协调的风险，这些风险可能逃避标准的人类监督形式。虽然线性探测在单智能体环境中已显示出检测欺骗的潜力，但共谋本质上是一种多智能体现象，使用内部表示来检测智能体之间的共谋仍未被探索。

Method: 引入NARCBench基准来评估环境分布偏移下的共谋检测，提出了五种探测技术，这些技术聚合每个智能体的欺骗分数，在群体层面分类场景。探测技术基于模型内部激活，并探索了不同形式的共谋在激活空间中的表现差异。

Result: 探测技术在分布内实现了1.00 AUROC，在零样本转移到结构不同的多智能体场景和隐写式21点算牌任务中实现了0.60-0.86 AUROC。研究发现没有单一的探测技术在所有共谋类型中占主导地位，表明不同形式的共谋在激活空间中表现不同。初步证据还显示信号在token级别局部化，共谋智能体的激活在处理伙伴消息的编码部分时特别激增。

Conclusion: 这项工作向多智能体可解释性迈出了一步：将白盒检查从单一模型扩展到多智能体环境，其中检测需要跨智能体聚合信号。结果表明，模型内部为检测多智能体共谋提供了与文本级监控互补的信号，特别是对于能够访问模型激活的组织。代码和数据已公开。
...
### [172] [Therefore I am. I Think](https://arxiv.org/abs/2604.01202)
*Esakkivel Esakkiraja,Sai Rajeswar,Denis Akhiyarov,Rajagopal Venkatesaramani*

Main category: cs.AI

TL;DR: 研究发现大型语言推理模型在生成推理过程前就已编码了决策，通过线性探针可从预生成激活中解码工具调用决策，激活引导可改变决策方向并导致推理过程合理化新决策。


<details>
  <summary>Details</summary>
Motivation: 探究语言推理模型在做出选择时，是"先思考后决定"还是"先决定后思考"，验证推理模型是否在开始文本推理前就已编码了行动选择。

Method: 使用线性探针从预生成激活中解码工具调用决策；通过激活引导技术扰动决策方向；进行行为分析观察推理过程对决策改变的反应。

Result: 线性探针能以高置信度从预生成激活中解码决策，有时甚至在生成第一个推理标记前；激活引导可显著增加审议时间，并在7-79%的案例中翻转行为；当决策被改变时，推理过程倾向于合理化新决策而非抵制。

Conclusion: 推理模型在开始文本推理前就已编码了行动选择，支持"先决定后思考"的假设，这对理解语言模型的推理机制有重要意义。
