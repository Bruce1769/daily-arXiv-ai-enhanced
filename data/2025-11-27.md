...
TL;DR: 本文研究了多模态视觉语言模型（VLMs）的黑盒成员推理攻击（MIA），并提出了神经科学启发的拓扑正则化框架来增强模型对隐私攻击的抵抗能力。实验表明，经过神经科学启发的VLMs在保持模型效用的同时，显著降低了MIA攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛应用，隐私泄露风险日益突出。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性及其防御机制尚未充分探索。本文旨在研究神经科学启发的多模态模型是否能增强对隐私攻击的抵抗能力。

Method: 提出了神经科学启发的拓扑正则化框架（tau），在三种VLMs（BLIP、PaliGemma 2、ViT-GPT2）上应用该框架，并在三个基准数据集（COCO、CC3M、NoCaps）上进行实验，比较基线模型和神经科学启发的VLMs对成员推理攻击的抵抗能力。

Result: 实验结果显示，在BLIP模型上使用COCO数据集时，神经科学启发的VLMs将MIA攻击成功率降低了24%（平均ROC-AUC），同时保持了相似的模型效用（MPNet和ROUGE-2指标）。在PaliGemma 2和ViT-GPT2模型上的额外实验进一步验证了结果的稳定性。

Conclusion: 神经科学启发的多模态视觉语言模型在保持模型效用的同时，显著增强了对抗隐私攻击的抵抗能力，为理解多模态模型的隐私风险提供了重要证据。
...
Conclusion: Inferix作为专门针对世界模拟设计的推理引擎，与高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs）有所区别，希望社区共同推进Inferix发展，促进世界模型探索。
...
Result: 通过硬件在环实验验证了LTED-Ada的优越性能，使用多个Raspberry Pi 4B设备和PC作为边缘服务器。

Conclusion: LTED-Ada算法能够有效优化视频对象识别性能，在单设备和多设备场景下均表现出色。
...
Method: 提出训练免费的、动作引导的早期退出框架，当预测轨迹与轻量级规划先验在可容忍偏差内对齐时终止推理，并引入多跳控制器自适应跳过冗余层。

Result: 在Bench2Drive基准测试中，实现了高达28%的Transformer层稀疏度和29%的延迟减少。

Conclusion: DeeAD可以集成到现有VLA模型中，无需重新训练，在保持规划质量和安全性的同时显著加速推理。
...
TL;DR: DinoLizer是基于DINOv2的模型，用于定位生成式修复图像中的篡改区域。该方法在DINOv2模型上添加线性分类头，通过滑动窗口策略处理大图像，在多个修复数据集上超越现有最优方法，平均IoU提高12%。
...
Method: 在预训练的DINOv2模型上添加线性分类头，基于ViT的patch嵌入预测14×14分辨率的篡改区域。使用滑动窗口策略处理大尺寸图像，并通过后处理优化二值篡改掩码。

Result: 在多个生成模型衍生的修复数据集上超越现有最优方法，平均IoU提高12%。对常见后处理操作（如调整大小、噪声添加、JPEG压缩）保持鲁棒性。

Conclusion: DINOv2为基础的Vision Transformer在篡改定位任务中表现出强大的表示能力，DinoLizer方法在生成式修复检测中具有优越性能。
...
TL;DR: CANVAS是一个用于评估视觉语言模型在基于工具的用户界面设计任务中性能的基准，包含598个设计任务，涵盖设计复制和设计修改两种任务类型。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估视觉语言模型在设计软件中进行工具调用设计能力的基准，而理解这种能力对于促进模型与设计师在常规软件中的协作具有重要意义。

Method: 构建包含598个基于工具的设计任务的基准，这些任务从3.3K个移动UI设计中采样，涵盖30个功能类别，支持通过工具调用逐步更新设计。
...
TL;DR: 本文提出了一种文本引导的语义图像编码器（TIE），它能够根据输入文本查询生成条件化的图像表示，从而提升视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的图像编码器在视觉语言模型中通常是独立预训练的，处理图像时与下游任务或文本查询无关，这限制了模型的性能。

Method: 提出TIE方法，通过文本条件化训练，使图像编码器能够根据输入文本查询生成相应的图像表示。

Result: 在1B和3B规模的九个图像到文本基准测试中，TIE分别平均提升了1.5和1.3个点，在DocVQA和InfoVQA等任务上提升高达6个点，同时仅使用一半的图像块（token），显著提高了推理效率。

Conclusion: TIE能够有效捕捉关键视觉特征，增强模型的可解释性和查询特定的基础能力，具有良好的泛化性能。
...
TL;DR: Δ-NeRF是一种模块化残差框架，用于神经辐射场的增量式精炼，解决了传统NeRF需要完整重新训练的问题，特别适用于卫星地形分析等数据连续到达的场景。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视图时需要完整重新训练，限制了在数据顺序到达场景中的应用，特别是在卫星地形分析中，区域会随时间重复观测。增量式精炼方法存在灾难性遗忘问题。

Method: 提出Δ-NeRF框架，包括：残差控制器向冻结的基础NeRF注入逐层修正；不确定性感知门控机制自适应结合基础和精炼预测；视图选择策略减少47%训练数据；使用知识蒸馏将增强模型压缩至原大小的20%。

Result: 在卫星图像上的实验表明，Δ-NeRF性能与联合训练相当，训练时间减少30-42%，PSNR比简单微调提升43.5%，在某些指标上甚至超过联合训练。

Conclusion: Δ-NeRF提供了一种有效的增量式NeRF精炼方法，在保持性能的同时显著减少训练时间和数据需求，为连续数据场景的3D重建提供了实用解决方案。
...
TL;DR: Split-then-Merge (StM) 是一个新颖的生成式视频合成框架，通过将未标注视频分割为动态前景和背景层，然后自组合学习动态主体与多样化场景的交互，解决了数据稀缺问题并增强了控制能力。


<details>
  <summary>Details</summary>
Motivation: 解决生成式视频合成中的数据稀缺问题和控制能力不足，传统方法依赖标注数据集或手工规则，难以学习复杂的组合动态。

Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景的交互；采用变换感知训练管道，结合多层融合和增强实现可感知组合，并使用身份保持损失维持前景保真度。

Result: 在定量基准测试和人类/VLLM定性评估中，StM均优于最先进方法。

Conclusion: StM框架通过分割-合并策略有效学习视频合成中的复杂组合动态，为生成式视频合成提供了新的解决方案。
...
TL;DR: Sphinx是一个针对视觉感知和推理核心认知原语的合成环境，通过程序化生成包含各种元素的谜题并提供可验证的真实解，用于精确评估和大规模数据集构建。评估显示最先进的LVLMs准确率仅为51.1%，远低于人类水平，而RLVR方法能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够精确评估视觉感知和推理能力的合成环境，解决现有基准在评估核心认知能力方面的局限性，并提供可扩展的数据集构建能力。

Method: 使用程序化生成方法创建包含图案、图块、图表、图标和几何原语的谜题，每个谜题都配有可验证的真实解。涵盖25种任务类型，包括对称检测、几何变换、空间推理、图表解释和序列预测。

Result: 评估显示最先进的大型视觉语言模型（如GPT-5）在Sphinx基准上的准确率仅为51.1%，远低于人类表现。使用可验证奖励的强化学习（RLVR）方法显著提高了模型在这些任务上的准确性，并在外部视觉推理基准上获得提升。

Conclusion: Sphinx为评估视觉感知和推理提供了有效的合成环境，揭示了当前LVLMs在核心认知任务上的局限性，并展示了RLVR方法在提升多模态推理能力方面的潜力。
...
TL;DR: RefTr是一个用于血管树中心线生成的3D图像到图模型，通过循环精炼汇合轨迹来生成中心线图。该模型采用Producer-Refiner架构，在保持高召回率的同时减少参数数量，在多个公共数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在临床应用中，准确检测血管树的中心线拓扑结构至关重要。保持高召回率尤为重要，因为遗漏小分支可能导致致命错误。现有方法在召回率和参数效率方面存在不足。

Method: 提出RefTr模型，采用基于Transformer解码器的Producer-Refiner架构。Producer生成初始汇合轨迹，Refiner循环精炼这些轨迹形成最终中心线图。使用汇合轨迹表示法确保有效树拓扑，并引入空间树图非极大值抑制算法合并重复分支。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前SOTA的召回率和相当的精度，同时提供更快的推理速度和显著更少的参数（解码器参数减少2.4倍）。

Conclusion: RefTr作为一种新的血管树分析框架，在3D医学成像中展现出作为最先进方法的潜力，在保持高召回率的同时提高了参数效率和推理速度。
...
Result: 创建了包含18000张高分辨率图像的数据集，支持几何和光学效应的受控分析，包含具有挑战性的视觉元素如多尺度光学错觉、反射表面、透明玻璃墙等。
...
TL;DR: 本文提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频和描述性回忆数据，通过利用Reddit等平台的舌尖现象检索查询，为回忆生成和ToT检索任务提供丰富信号。


<details>
  <summary>Details</summary>
Motivation: 视觉内容记忆性研究面临人类记忆注释收集成本高昂的问题，限制了数据集的多样性和可扩展性。现有数据集主要收集聚合记忆分数，未能捕捉自然开放式回忆描述中的细微记忆信号。

Method: 利用在线平台（如Reddit）的舌尖现象检索查询构建无监督数据集，采用对比训练策略创建多模态ToT检索模型，并对大型视觉语言模型进行微调。

Result: 在生成视觉内容的开放式记忆描述方面，基于该数据集微调的模型优于GPT-4o等最先进模型；成功创建了首个能够执行多模态ToT检索的模型。
...
TL;DR: 提出一种从立体雾天图像序列中动态估计雾模型参数的方法，通过同时优化所有参数避免误差传播，并创建了首个包含真实雾天道路场景的高质量立体数据集SDIRF。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾参数容易导致误差传播，且难以处理现实中全局不均匀的雾。需要一种能同时估计所有参数并适应真实雾环境的方法。

Method: 通过求解新型优化问题同时估计所有雾参数，假设雾仅在局部均匀，可处理全局不均匀的真实雾。可作为附加模块集成到现有SLAM或里程计系统中。

Result: 在合成雾数据和真实SDIRF数据集上的实验表明，该方法不仅产生最准确的参数估计，而且对真实雾的适应性更好。

Conclusion: 提出的算法在雾参数估计方面优于现有方法，创建的SDIRF数据集为雾中视觉感知研究提供了重要资源，代码和数据集已开源。
...
Method: 提出V^2-SAM框架，包含：1）基于DINOv3特征的跨视角锚点提示生成器（V^2-Anchor），建立几何感知对应；2）跨视角视觉提示生成器（V^2-Visual），通过视觉提示匹配器增强外观引导线索；3）多专家设计和后验循环一致性选择器（PCCS）自适应选择最可靠的专家。

Result: 在Ego-Exo4D（自我-外部视角物体对应）、DAVIS-2017（视频物体跟踪）和HANDAL-X（机器人就绪的跨视角对应）上达到新的最先进性能。

Conclusion: V^2-SAM有效解决了跨视角物体对应问题，通过互补的提示生成器和自适应选择机制，显著提升了跨视角场景下的对应性能。
...
TL;DR: 提出Null-Text Test-Time Alignment (Null-TTA)方法，通过优化无分类器引导中的无条件嵌入来对齐扩散模型，避免奖励黑客问题并保持语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法存在欠优化或过优化（奖励黑客）问题，需要一种能有效对齐目标奖励同时防止利用非语义噪声模式的方法。

Method: 通过优化无分类器引导中的无条件嵌入而非操作潜在变量或噪声变量，在语义一致的流形上进行对齐，直接引导模型生成分布朝向目标奖励。
...
TL;DR: 本研究开发了一种深度学习模型，用于降低小脑桥脑角区对比增强T1加权MRI的造影剂剂量，通过从低剂量模拟图像恢复标准剂量图像质量，在10%-30%标准剂量下仍能保持良好的图像质量和诊断性能。
...
Result: 随着输入剂量增加，恢复图像的结构相似性指数从0.639提高到0.993，峰值信噪比从21.6dB提高到41.4dB。在10%输入剂量下，使用DL恢复的T1ce进行分割，Dice系数从0.673提高到0.734，Hausdorff距离从2.38mm改善到2.07mm。

Conclusion: 深度学习模型显著改善了小脑桥脑角区低剂量MRI的图像质量，使得在仅使用10%-30%标准剂量的情况下仍能实现病灶检测和诊断特征分析。
...
TL;DR: 提出一种平滑正则化技术，通过在视频识别模型中施加时间归纳偏置，特别是对轻量级架构有益。该方法通过将连续帧中间层嵌入的变化建模为高斯随机游走，鼓励嵌入平滑性，从而促进与视频自然时间连贯性一致的低加速度解决方案。


<details>
  <summary>Details</summary>
Motivation: 轻量级视频识别模型在捕捉复杂时间动态方面存在局限，需要一种能够有效利用视频时间连贯性的技术来提升性能。

Method: 使用高斯随机游走建模连续帧中间层嵌入的变化，通过惩罚表示上的突然变化来强制平滑性，使模型更好地对齐视频的自然时间连贯性。

Result: 在Kinetics-600数据集上，轻量级模型准确率提升3.8%至6.4%。MoViNets模型系列在各自FLOP约束下将当前最优结果提升3.8%至6.1%，MobileNetV3和MoViNets-Stream系列在可比内存占用下比先前最优模型提升4.9%至6.4%。
...
TL;DR: 本文介绍了UruDendro4数据集，包含102个火炬松横截面图像样本，每个都手动标注了年轮。该数据集支持基于横截面图像的年轮体积建模，并为自动年轮检测提供了性能基准。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，支持年轮体积建模研究，提高年轮检测的自动化和准确性。

Method: 构建包含102个火炬松样本的UruDendro4数据集，样本从树干多个高度提取；使用DeepCS-TRD等最先进方法进行自动年轮检测性能基准测试；进行消融实验验证参数配置。

Result: DeepCS-TRD方法表现最佳，平均精度0.838，平均召回率0.782，适应兰德误差0.084；包含该数据集的训练能提高模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持年轮体积建模，为自动年轮检测提供了可靠基准，有助于改进林木培育实践。
...
Method: 使用多任务视觉语言框架，从结构化描述符构建报告，通过多头Swin编码器学习描述符感知的视觉表示，使用双级目标对齐视觉和文本标记。
...
TL;DR: 本文提出了一种新的图像合成任务——表达性合成，关注艺术性和社交参与性而非真实性，并开发了StickerNet框架从真实编辑行为中学习贴纸放置参数。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究主要关注视觉真实性和语义合理性，但在实际在线创作平台中，用户更倾向于创作艺术性、趣味性和社交参与性的内容，而非保持真实性。

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和尺度等放置参数。数据集基于180万真实编辑行为构建，而非模拟对象放置。

Result: 用户研究和定量评估显示，StickerNet优于常见基线方法，与人类放置行为高度匹配，证明了从真实编辑模式中学习的有效性。
...
TL;DR: TrafficLens是一个针对多摄像头交通路口的优化算法，通过顺序处理摄像头重叠区域、迭代使用不同token限制的VLM以及对象级相似性检测，将视频转文本时间减少4倍，同时保持信息准确性。


<details>
  <summary>Details</summary>
Motivation: 多摄像头交通视频数据量大，传统基于LLM的分析方法需要先将视频转换为文本，这个过程耗时且延迟了交通视频的及时利用。
...
Conclusion: TrafficLens通过优化多摄像头交通视频的分析流程，显著提高了处理效率，为智能交通系统提供了更快速的分析解决方案。
...
TL;DR: 提出结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的多机构病理学分类，通过加密CLS令牌实现30倍通信减少，有效防御模型反演攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构的协作机器学习受隐私法规限制，传统联邦学习的模型梯度易受重建攻击泄露敏感医疗信息，需要更安全的隐私保护方案。

Method: 使用Vision Transformers的CLS令牌作为紧凑特征表示，采用CKKS同态加密对CLS令牌进行加密后传输到服务器进行安全聚合。

Result: 梯度方法易受模型反演攻击(PSNR: 52.26 dB, SSIM: 0.999, NMI: 0.741)，而CLS保护的HE方法能防止此类攻击，每轮聚合仅需326 KB加密数据传输，在未加密域达到96.12%准确率，加密域达到90.02%准确率。

Conclusion: 提出的框架在保持强隐私保证的同时显著降低通信开销，为医疗领域的隐私保护联邦学习提供了有效解决方案。
...
Method: 使用双整流流框架并行预测内容和风格轨迹，通过动态中点插值融合两种路径的速度，结合注意力注入指导风格集成。

Result: 实验证明该方法在多样化风格和内容上具有良好泛化能力，提供高效且有效的风格迁移流程。

Conclusion: 该免反演框架通过联合建模内容、风格和风格化分布，实现了鲁棒的融合效果，避免了朴素叠加的缺点，提升了视觉保真度和计算效率。
...
TL;DR: 本文提出了一种无需测试时参考图像的伪装目标检测框架，通过将参考图像蒸馏到类别原型内存中，并在推理时通过查询条件混合原型来合成参考向量，从而提高了部署效率和降低了数据收集负担。


<details>
  <summary>Details</summary>
Motivation: 当前的双分支Ref-COD系统在测试时需要参考图像，这限制了部署性、增加了延迟和数据收集负担。本文旨在开发一个无需测试时参考的Ref-COD框架。

Method: 采用EMA更新的类别原型内存，通过查询条件混合原型生成参考向量，并提出双向注意力对齐模块来弥合参考统计信息和伪装查询特征之间的表示差距。

Result: 在R2C7K基准测试上的广泛实验表明，该方法与现有最先进方法相比具有竞争性或更优的性能。

Conclusion: 该方法为Ref-COD提供了一条简单、高效的路径，无需强制性的参考图像，同时保持了竞争性的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态安全防护主要只评估输入问题和最终答案，忽略了中间推理过程，这导致偏见推断或违反政策的视觉内容使用等危害在推理过程中未被检测到。

Method: 提出了GuardTrace-VL安全审计器，构建了GuardTrace数据集，采用多样化的提示策略生成，并通过MLRM和人类投票验证流程进行精炼。还提出了三阶段渐进式训练方案与数据精炼过程相结合。

Result: 在提出的测试集上，GuardTrace-VL模型在不安全推理检测任务中达到93.1%的F1分数，相比之前最强的多模态安全防御方法提高了13.5%的F1分数。

Conclusion: GuardTrace-VL能够有效检测多模态推理过程中的不安全内容，显著提升了安全防护能力，代码将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 图像可以看作是由前景对象和背景组成的层次结构，这种分层表示能够独立编辑元素，为内容创作提供更大灵活性。但由于方法和数据有限，将单张图像分解为层次仍然具有挑战性。

Method: 观察到分层分解与图像修复/扩展任务之间的强关联性，提出通过轻量级微调将基于扩散的修复模型适配到分层分解任务。为了在潜在空间中保留细节，引入了具有线性注意力复杂度的多模态上下文融合模块。模型完全在从开源资源构建的合成数据集上进行训练。

Result: 该方法在对象移除和遮挡恢复方面实现了优越性能，为下游编辑和创意应用开辟了新的可能性。

Conclusion: 提出的方法成功解决了单张图像分层分解的挑战，通过创新的多模态上下文融合模块和合成数据训练策略，在图像编辑任务中展现出显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法存在三个关键挑战：信息覆盖不完整、跨模态对齐弱、视觉实体定位差，需要开发更有效的多模态框架来解决这些问题。

Method: 构建实体中心的多模态知识库（EMKB）整合文本、视觉和结构化知识；采用多阶段假设-标题策略改善跨模态对齐；通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上显著优于现有方法，CIDEr得分分别提升+6.84和+1.16，命名实体识别F1得分分别提升+4.14和+2.64；在未见过的Visual News数据集上表现优异，CIDEr提升+20.17，F1得分提升+6.22。

Conclusion: MERGE框架通过多模态知识增强和动态检索机制，显著提升了新闻图像描述的质量和实体识别能力，展现出强大的鲁棒性和领域适应性。
...
TL;DR: MetaRank是一个元学习框架，用于自动选择模型可迁移性评估(MTE)指标，通过将数据集和MTE指标的文本描述编码到共享语义空间，学习数据集特征与指标机制之间的关系，实现任务感知的指标选择。
...
Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和MTE指标的文本描述，在共享语义空间中训练元预测器，采用列表式优化目标优先排序表现最佳的指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有可信多视图分类方法主要依赖全局密集邻居关系建模视图内依赖，导致计算成本高且无法直接确保视图间关系一致性，同时通过手动分配权重聚合不同视图证据，缺乏对学习到的多视图邻居结构在类别空间一致性的保证。
...
Conclusion: 所提出的基于原型的可信多视图分类框架能够更有效地处理异构、不一致甚至冲突的多源信息，提高分类结果的可信度。
...
TL;DR: CameraMaster是一个统一的相机感知框架，通过解耦相机指令并整合指令表示和参数嵌入来实现精确的图像修饰控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖模糊的文本提示难以精确控制相机参数，要么训练单独的头/权重来调整参数，这影响了可扩展性、多参数组合和对细微变化的敏感性。
...
Conclusion: CameraMaster通过显式解耦相机指令并协调整合两个关键信息流，实现了精确的相机参数控制和物理一致的图像修饰。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，涵盖4个领域和25个主要类别，构建了33,027个密集标注的多选题，直接测量描述是否能替代图像支持LLM完成任务。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法未能回答图像描述是否能在真实下游任务中替代图像，需要基于效用的基准来评估模型生成的描述质量。

Method: 创建可扩展的领域相关基准，包含4个领域和69个子类别，构建需要视觉信息回答的多选题，通过LLM仅使用描述回答问题来测量描述效用。

Result: 评估发现最先进MLLMs在描述效用与图像效用之间存在显著差距，传统图像QA基准表现相近的模型在描述效用上下降高达32%。

Conclusion: CaptionQA基准揭示了当前图像描述在支持下游任务方面的局限性，提供了评估描述实用性的新方法，并开源了扩展到新领域的流程。
...
TL;DR: 本文提出了LungNoduleAgent，一个用于分析肺部CT扫描的协作多智能体系统，通过三个主要模块（结节定位器、放射科医生、医生智能体系统）提高结节描述和恶性程度分级的精确度，在多个数据集上表现优于主流视觉语言模型和专家模型。


<details>
  <summary>Details</summary>
Motivation: 当前使用多模态大语言模型分析肺部CT扫描存在准确描述结节形态和融入医学专业知识的挑战，影响临床可靠性。协作多智能体系统在医学应用中具有平衡通用性和精确性的潜力，但在病理学领域尚未充分探索。

Method: LungNoduleAgent将诊断过程分解为顺序组件：1）结节定位器协调临床检测模型准确定位结节；2）放射科医生整合局部图像描述技术生成全面CT报告；3）医生智能体系统利用图像和CT报告，在病理知识库和多智能体框架支持下进行恶性程度推理。
...
TL;DR: 本文提出了一种新框架，通过将强大的生成先验与显式密集物理约束相结合，解决了空间变化图像去模糊问题。该方法使用高维压缩核建模密集退化场，并利用ControlNet架构引导扩散采样过程，在物理准确性和感知真实性之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的图像去模糊方法存在两个主要问题：基于模型的深度展开方法虽然能强制物理约束，但会产生过度平滑和伪影纹理；生成模型虽然感知质量更好，但由于物理约束较弱会产生细节幻觉。需要一种能够调和这两种范式的方法。

Method: 提出了一种新颖框架，将生成先验与密集物理约束相结合。将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化。利用这个丰富的描述符场来调节ControlNet架构，强有力地引导扩散采样过程。

Result: 广泛的实验表明，该方法有效地弥合了物理准确性和感知真实性之间的差距，在具有挑战性的严重模糊场景中，优于最先进的基于模型方法和生成基线方法。

Conclusion: 该框架通过将强大的生成先验与显式密集物理约束相结合，成功解决了空间变化图像去模糊问题，在保持物理准确性的同时实现了优越的感知质量。
...
TL;DR: MUSE是一个统一的图像情感合成框架，能够同时进行情感生成和编辑，通过基于梯度的情感标记优化和语义相似度监督，在无需额外训练扩散模型的情况下实现高效的情感引导合成。
...
Method: 采用概念上对齐测试时间缩放(TTS)的策略，利用现成情感分类器进行基于梯度的情感标记优化，通过语义相似度确定最佳引导时机，并使用多情感损失减少固有和相似情感的干扰。

Result: 实验结果显示MUSE在生成和编辑任务中均优于所有对比方法，提高了情感准确性和语义多样性，同时在期望内容、文本提示遵循和真实情感表达之间保持最佳平衡。

Conclusion: MUSE为情感合成建立了新的范式，证明了统一框架在图像情感生成和编辑任务中的有效性。
...
TL;DR: 本文提出了一种基于正态逆伽马分布的时间参数估计模型（T-NIG），用于解决阿尔茨海默病长期预测中图像生成时疾病相关特征保持困难的问题，特别是在处理不规则时间间隔序列数据时。


<details>
  <summary>Details</summary>
Motivation: 现有的阿尔茨海默病长期预测方法在处理不规则时间间隔的序列数据时，难以保持疾病相关特征。由于时间相关方面的分布可以反映图像分布不均匀时疾病相关特征的变化，因此需要开发能够处理不规则时间间隔的模型。

Method: T-NIG模型使用两个不同时间点的脑图像来生成中间脑图像、预测未来图像和疾病。该模型通过坐标邻域识别特征，在正态逆伽马分布中引入时间参数来理解脑成像序列中特征的变化，并利用不确定性估计来减少由于时间数据不足引起的认知和随机不确定性。

Result: 实验结果表明，T-NIG模型在数据集中的短期和长期预测任务中都表现出最先进的性能。该模型能够熟练预测疾病进展，同时保持疾病相关特征，即使面对不规则的时间数据分布。

Conclusion: T-NIG模型通过整合时间参数和不确定性估计，有效解决了阿尔茨海默病长期预测中图像生成时疾病特征保持的挑战，为不规则时间间隔的序列数据提供了可靠的解决方案。
...
TL;DR: 提出一种新颖的虹膜认证匹配流程，通过3D-CNN学习虹膜特征的丰富时空表示，采用课程学习方式训练，提高对旋转、缩放、反射和模糊等挑战的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有虹膜认证算法在旋转、缩放、镜面反射和散焦模糊等变化下的鲁棒性不足，且大多依赖简单的点对点比较，未能有效利用虹膜图案的时空结构。

Method: 将虹膜图像沿一维分割生成子图像序列，输入3D-CNN捕获空间和时空线索，采用课程学习方式结合三元组损失和ArcFace损失进行端到端训练。

Result: 该方法能够将时间依赖性直接嵌入特征空间，在深度度量域中提高区分性，产生鲁棒且可泛化的虹膜认证解决方案。

Conclusion: 提出的框架通过学习丰富的时空表示，显著提升了虹膜认证系统在现实应用中的鲁棒性和泛化能力。
...
Conclusion: RadarFM通过结构化空间语言监督成功实现了雷达基础模型的构建，为雷达感知提供了统一的表示学习方法。
...
TL;DR: 本文提出EM-KD方法，通过知识蒸馏增强高效多模态大语言模型，解决视觉令牌不平衡导致的细粒度视觉理解差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌以减少资源消耗，但视觉信息丢失会降低理解能力。先前方法虽然引入知识蒸馏，但忽略了高效学生模型与原始教师模型之间由于视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 提出EM-KD范式：1) 使用曼哈顿距离和匈牙利匹配算法对齐师生视觉令牌；2) 引入两种蒸馏策略：视觉-语言亲和度蒸馏(VLAD)和视觉语义蒸馏(VSD)。VLAD计算文本令牌与对齐视觉令牌的亲和矩阵并最小化平滑L1距离，VSD使用反向KL散度测量词汇空间中对齐视觉逻辑的离散概率分布。

Result: 在多样化基准测试中的综合评估表明，EM-KD训练模型在准确性和效率上都显著优于先前的高效MLLMs。与配备相同视觉令牌匹配策略的先前蒸馏方法相比，EM-KD也实现了更好的性能。

Conclusion: EM-KD通过有效的视觉令牌对齐和双重蒸馏策略，成功提升了高效多模态大语言模型的性能，验证了该方法的有效性。
...
TL;DR: AnchorOPT是一个动态锚点提示学习框架，通过动态学习锚点值和位置关系来提升CLIP模型的泛化能力，无需手工设计文本标记或固定位置关系。
...
Result: 仅使用简单的可学习锚点和位置矩阵就能达到或超过某些包含额外可学习模块或正则化技术的方法的性能。作为即插即用模块，AnchorOPT能无缝集成到现有框架中，在多样化数据集上获得一致的性能提升。

Conclusion: AnchorOPT通过动态锚点机制有效解决了现有提示学习方法中锚点静态性的限制，在保持简单性的同时实现了优异的性能表现。
...
TL;DR: FaithFusion是一个基于像素级期望信息增益（EIG）的3DGS-扩散模型融合框架，用于可控驾驶场景重建和3D场景生成，在保持几何保真度的同时实现大视角变化下的视觉逼真合成。


<details>
  <summary>Details</summary>
Motivation: 解决在可控驾驶场景重建和3D场景生成中，几何保真度与视觉逼真度之间的平衡问题，特别是处理3DGS和扩散模型融合时的过恢复和几何漂移挑战。

Method: 提出基于像素级期望信息增益（EIG）的统一策略，EIG作为空间先验指导扩散模型细化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS，形成一个即插即用的系统。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到最先进性能，即使在6米车道偏移时仍保持107.47的FID。

Conclusion: FaithFusion通过EIG驱动的统一策略，有效解决了3DGS与扩散模型融合的挑战，实现了几何保真和视觉逼真的平衡，无需额外先验条件和结构修改。
...
<details>
  <summary>Details</summary>
Motivation: 先前关于语言模型的研究报告了逆向扩展效应，即文本干扰项会导致更长但效果更差的推理。本研究旨在探究多模态环境中是否会出现类似现象。

Method: 引入了Idis（带干扰项的图像）数据集，这是一个视觉问答数据集，系统地在语义、数值和空间维度上变化干扰项。通过分析推理轨迹中的属性计数来研究干扰项、推理长度和准确性之间的相互作用。

Result: 视觉干扰项与文本干扰项存在根本差异：虽然逆向扩展效应仍然存在，但添加视觉干扰项会降低准确性而不增加推理长度。这些趋势也扩展到已建立的视觉偏见基准测试中。

Conclusion: 提出了一个简单的提示策略来减轻推理模型中的偏见驱动预测，为理解多模态环境中干扰项的影响提供了重要见解。
...
TL;DR: EntPruner是一个基于熵指导的自动渐进式剪枝框架，专门针对扩散模型和流模型，通过条件熵偏差评估模块重要性，实现参数冗余减少和推理加速。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型在下游任务中存在显著的参数冗余问题，需要一种专门针对生成模型的剪枝方法，既要保持输出分布的多样性和条件保真度，又要适应不同下游任务的需求。

Method: 提出熵指导剪枝策略，使用数据依赖的条件熵偏差(CED)作为模块重要性评估指标；设计零样本自适应剪枝框架，自动决定何时以及剪枝多少，避免一次性剪枝带来的模式崩溃问题。

Result: 在DiT和SiT模型上的实验表明，EntPruner能够实现最高2.22倍的推理加速，同时在ImageNet和三个下游数据集上保持竞争力的生成质量。

Conclusion: EntPruner框架有效解决了生成模型在下游任务中的参数冗余问题，通过熵指导的渐进式剪枝实现了模型压缩和性能保持的良好平衡。
...
TL;DR: CtrlVDiff是一个统一的扩散模型，通过混合模态控制策略融合深度、法线、分割、边缘和图形学内在属性等多种模态，实现视频理解和可控视频生成的双重任务。


<details>
  <summary>Details</summary>
Motivation: 解决传统仅使用几何线索（如深度、边缘）进行视频控制时出现的布局约束不足、外观和光照控制不精确以及时间漂移问题，通过引入图形学内在属性提供更丰富的约束条件。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS）来路由和融合多种模态特征，并构建MMVideo混合数据集进行训练，支持任意模态子集输入并保持时间一致性。

Result: 在理解和生成基准测试中，CtrlVDiff实现了卓越的可控性和保真度，支持分层编辑（重新光照、材质调整、对象插入），并在某些模态不可用时仍保持鲁棒性。

Conclusion: 通过融合多种图形学模态，CtrlVDiff成功解决了视频理解和可控生成的双重挑战，实现了更精确的物理意义编辑和更好的时间一致性。
...
TL;DR: 提出了一种名为Ent-Prog的高效训练框架，用于人类视频生成的扩散模型，通过条件熵膨胀评估模型组件重要性，并采用自适应渐进式调度，实现了2.2倍训练加速和2.4倍GPU内存减少，同时保持生成性能。


<details>
  <summary>Details</summary>
Motivation: 人类视频生成的扩散模型在训练高分辨率多帧数据时面临高计算成本和内存消耗的挑战，需要开发更高效的训练方法。

Method: 提出Ent-Prog框架：1）条件熵膨胀（CEI）评估模型组件重要性，优先训练关键组件；2）自适应渐进式调度，根据收敛效率自适应增加计算复杂度。

Result: 在三个数据集上的实验表明，Ent-Prog实现了最高2.2倍训练加速和2.4倍GPU内存减少，且不损害生成性能。

Conclusion: Ent-Prog是一种有效的训练框架，显著降低了扩散模型在人类视频生成中的训练成本和内存需求，同时保持了模型性能。
...
TL;DR: TEAR是一个针对文本到视频模型安全性的自动化红队框架，通过时间感知的测试生成器发现与动态时间序列相关的安全风险。
...
Method: 采用时间感知的测试生成器，通过两阶段优化方法（初始生成器训练和时间感知在线偏好学习）创建文本上无害但能利用时间动态引发违规视频输出的提示，并使用精炼模型循环改进提示的隐蔽性和对抗效果。

Result: 在开源和商业T2V系统上的广泛实验评估显示，TEAR的攻击成功率超过80%，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架能有效发现T2V模型中的时间相关安全风险，证明了时间动态在视频生成安全评估中的重要性。
...
TL;DR: LLaVA-UHD v3提出渐进式视觉压缩(PVC)方法，在标准ViT中实现高效原生分辨率编码，显著降低计算开销同时保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 解决全局视觉编码在MLLMs中计算开销大的问题，平衡性能与效率。

Method: 采用渐进式视觉压缩(PVC)，包含细化补丁嵌入和窗口化令牌压缩两个模块，在ViT层间分层部署。

Result: ViT-UHD在保持与MoonViT相当性能的同时，TTFT降低2.4倍；LLaVA-UHD v3与Qwen2-VL性能相当，TTFT进一步降低1.9倍。
...
Method: GridAR采用网格分区渐进生成方案，在同一画布位置生成多个部分候选，早期修剪不可行候选，将可行候选固定为锚点指导后续解码；同时提出布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N（N=8）在T2I-CompBench++上性能提升14.4%，同时成本降低25.6%；在图像编辑任务上，与更大N的基线相比，语义保持度提升13.9%。
...
TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和渐进式特征融合，实现3D场景理解任务，在3D参考分割、视觉问答和密集描述等任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在将3D场景有效分词为整体场景标记，并在多样化3D理解任务中利用这些标记方面仍面临挑战。

Method: 提出三阶段场景分词流程：1)从原始点云构建多尺度NDT表示；2)使用多尺度NDT解码器渐进融合跨尺度特征生成场景标记；3)将解码器重新用于人机交互提示和分割掩码解码。
...
TL;DR: UPA-RFAS是一个针对视觉-语言-动作(VLA)模型的通用可迁移对抗补丁攻击框架，通过共享特征空间学习单一物理补丁，实现在未知架构、微调变体和仿真到现实迁移中的跨模型攻击。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对抗攻击大多过拟合单一模型，在未知黑盒设置中失效，缺乏通用性和可迁移性，需要系统研究跨模型、跨任务的对抗补丁攻击。

Method: 提出UPA-RFAS统一框架：1)特征空间目标函数结合ℓ1偏差先验和排斥性InfoNCE损失；2)鲁棒性增强的两阶段min-max优化，内环学习不可见样本扰动，外环优化通用补丁；3)两个VLA特定损失：补丁注意力主导和补丁语义错配。

Result: 实验表明UPA-RFAS在不同VLA模型、操作套件和物理执行中持续实现跨模型、跨任务和跨视角的迁移攻击，暴露了实用的补丁攻击面。

Conclusion: UPA-RFAS建立了针对VLA驱动机器人的通用可迁移对抗补丁攻击的强基线，为未来防御研究提供了重要参考。
...
Result: 在植物存在预测、蝴蝶出现建模和土壤营养组丰度估计三个生态任务中，BotaCLIP表示相比DOFA和监督基线方法表现出一致的改进。
...
<details>
  <summary>Details</summary>
Motivation: 当前动作识别方法主要捕捉粗粒度运动模式，难以识别随时间演变的局部区域细微差异。
...
Conclusion: ART框架通过跟踪局部区域动态变化，有效解决了细粒度动作识别中细微差异的识别问题。
...
TL;DR: ConFu是一个多模态对比学习框架，通过融合模态对比项同时学习单个模态和融合模态的联合表示，既能捕捉高阶依赖关系又保持强配对对应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要在成对设置下对齐两种模态，而一些捕捉多模态高阶交互的方法往往忽视或不足以保持成对关系，限制了在单模态任务上的有效性。

Method: ConFu扩展了传统的成对对比目标，增加了融合模态对比项，鼓励模态对与第三个模态的联合嵌入，从而捕捉无法通过成对对齐恢复的高阶依赖关系。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，同时支持在单一对比框架内进行统一的一对一和一对多检索。

Conclusion: ConFu能够有效利用跨模态互补性，捕捉高阶依赖关系，并随着多模态复杂性的增加而扩展，同时保持强配对对应性。
...
TL;DR: 本文对三种生成建模范式进行了比较研究：DDPM、CFM和MeanFlow。CFM在50步采样时FID为24.15，显著优于DDPM的402.98。MeanFlow通过单步采样实现FID 29.15，推理时间减少50倍。还将CFM扩展到图像修复，在中心掩码上PSNR从4.95提升至8.57 dB，SSIM从0.289提升至0.418。


<details>
  <summary>Details</summary>
Motivation: 比较三种生成建模方法（DDPM、CFM、MeanFlow）的性能差异，特别是关注推理效率和生成质量，并探索CFM在图像修复任务中的应用潜力。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10数据集上实现三种方法。CFM和DDPM采用迭代采样，MeanFlow采用直接单步生成。还扩展CFM到图像修复，使用四种掩码类型进行掩码引导采样。

Result: CFM在50步采样时FID为24.15，显著优于DDPM的402.98。MeanFlow单步采样FID为29.15，推理时间减少50倍。图像修复方面，经过微调的模型在中心掩码上PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。
...
TL;DR: SurgMLLMBench是一个统一的多模态基准测试，专门用于开发和评估用于手术场景理解的交互式多模态大语言模型，整合了像素级器械分割掩码和结构化VQA注释。
...
Method: 提出了SurgMLLMBench基准测试，整合了新收集的MAVIS数据集，在腹腔镜、机器人辅助和显微外科领域下统一分类法，提供像素级器械分割掩码和结构化VQA注释。

Result: 基线实验表明，在SurgMLLMBench上训练的单一模型在不同领域实现了持续性能，并能有效泛化到未见过的数据集。

Conclusion: SurgMLLMBench将作为强大资源公开发布，支持可重复评估和交互式手术推理模型的开发，推动多模态手术AI研究。
...
TL;DR: FIELDS提出了一种从单张图像重建3D人脸的方法，通过结合3D表情参数监督和情感识别分支，解决了现有方法因依赖2D监督而丢失细微情感细节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法主要依赖2D监督，缺乏3D真实数据，导致无法捕捉细微的情感表达细节。

Method: 扩展自监督2D图像一致性线索，加入直接的3D表情参数监督和辅助情感识别分支，使用来自自发4D面部扫描的真实表情参数指导编码器，并采用强度感知的情感损失。

Result: 产生了保留细微情感线索的高保真3D重建，从单张图像生成情感丰富的面部模型，显著提高了野外面部表情识别性能而不牺牲自然度。

Conclusion: 双监督策略弥合了2D/3D领域差距并减轻了表情强度偏差，能够生成具有高度真实表情的情感丰富面部模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在抽象视觉推理方面缺乏人类般的灵活性，受限于外部工具。需要让模型直接在潜在视觉空间中进行推理。

Method: 采用三阶段蒸馏监督微调管道，构建了125K高质量文本图像交错推理链数据集，并提出VLPO强化学习方法。
...
TL;DR: AVFakeBench是首个全面的音视频伪造检测基准，涵盖人类和一般主体的丰富伪造语义，包含12K个音视频问题，覆盖7种伪造类型和4级标注。通过多阶段混合伪造框架生成高质量多样化伪造内容，评估11个音视频大语言模型和2种主流检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有的音视频伪造检测基准局限于DeepFake类伪造和单粒度标注，无法捕捉真实世界伪造场景的多样性和复杂性。

Method: 提出多阶段混合伪造框架，集成专有模型进行任务规划和专家生成模型进行精确操作；建立多任务评估框架，包括二元判断、伪造类型分类、伪造细节选择和解释性推理。

Result: 评估显示音视频大语言模型作为新兴伪造检测器的潜力，但揭示了它们在细粒度感知和推理方面的显著弱点。

Conclusion: AVFakeBench为音视频伪造检测提供了更全面的评估基准，展示了音视频大语言模型的潜力，同时指出了需要改进的领域。
...
Conclusion: LaGen成功解决了长序列LiDAR场景生成问题，为自动驾驶领域提供了有效的4D场景生成解决方案。
...
TL;DR: 本文提出了一种利用SAM基础模型进行遥感变化字幕生成的新方法，通过提取区域级表示和注入感兴趣区域知识，解决了现有方法区域感知弱和时间对齐有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化字幕方法通常使用CNNs/Transformers提取视觉表示或引入辅助任务，但存在区域感知弱和时间对齐有限的问题。本文旨在通过利用SAM基础模型来改进这些不足。

Method: 使用CNN/Transformer提取全局视觉特征，利用SAM基础模型划分语义和运动级变化区域，构建专门的知识图谱提供感兴趣对象信息，通过交叉注意力融合异构信息源，使用Transformer解码器生成最终的自然语言描述。

Result: 在多个广泛使用的基准数据集上，该方法实现了最先进的性能表现。

Conclusion: 通过引入SAM基础模型提取区域级表示和注入感兴趣区域知识，显著提升了遥感变化字幕生成的质量和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配方法严重依赖大规模、多样化且几何准确的训练数据。3D高斯泼溅技术虽然能实现逼真的新视角合成，但其几何不准确性和深度渲染偏差限制了其在对应关系标注中的应用。

Method: MatchGS采用双重策略：(1) 几何忠实的数据生成流程，通过优化3DGS几何来产生精确的对应关系标签；(2) 2D-3D表示对齐策略，将3DGS的显式3D知识注入2D匹配器，引导其学习视角不变的3D表示。

Result: 生成的对应关系标签相比现有数据集将极线误差降低了40倍，支持极端视角变化下的监督学习。仅使用该数据训练的匹配器在公共基准测试中实现了高达17.7%的零样本性能提升。

Conclusion: 通过适当的几何优化，3DGS可以成为可扩展、高保真且结构丰富的数据源，为新一代鲁棒零样本图像匹配器的发展铺平道路。
...
### [62] [From Observation to Action: Latent Action-based Primitive Segmentation for VLA Pre-training in Industrial Settings](https://arxiv.org/abs/2511.21428)
...
<details>
  <summary>Details</summary>
Motivation: 解锁大量未标记的工业演示数据用于VLA模型预训练，解决制造业中具身AI集成的可扩展性问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上表现出色，研究者希望开发一个统一模型，通过多任务学习在多个任务上表现优异，以提高泛化性、可扩展性和实际应用性。

Method: 提出数据引擎处理复杂遥感数据环境，统一动态分辨率策略处理不同图像尺度，Zoom-in Chain机制处理超高分辨率图像，并增强目标检测能力。

Result: RSCoVLM在多样化任务上实现了最先进性能，超越了现有遥感视觉语言模型，甚至可与专业专家模型相媲美。
...
### [64] [EvRainDrop: HyperGraph-guided Completion for Effective Frame and Event Stream Aggregation](https://arxiv.org/abs/2511.21439)
*Futian Wang,Fan Zhang,Xiao Wang,Mengqi Wang,Dexing Huang,Jin Tang*

Main category: cs.CV

TL;DR: 提出了一种基于超图的时空事件流补全机制，通过超图连接不同时间和空间位置的事件令牌，利用上下文信息传递来补全稀疏事件，并能灵活融合RGB令牌实现多模态信息补全。
...
Method: 使用超图引导的时空事件流补全机制，通过超图连接事件令牌并利用上下文信息传递进行补全，可融合RGB令牌实现多模态信息补全，然后通过自注意力聚合不同时间步的超图节点信息。
...
TL;DR: 本文提出了一种频率感知的令牌缩减策略，通过将令牌划分为高频和低频令牌，选择性保留高频令牌并将低频令牌聚合为紧凑的直流令牌，以提高计算效率同时保持性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在计算复杂度方面存在挑战，现有令牌缩减方法往往忽略了自注意力的频率特性，如秩崩溃和过平滑现象。

Method: 提出频率感知令牌缩减策略，将令牌划分为高频和低频令牌，选择性保留高频令牌，将低频令牌聚合为紧凑的直流令牌以保留必要的低频分量。

Result: 通过大量实验和分析，证明该方法在减少计算开销的同时显著提高准确性，并缓解秩崩溃和过平滑问题。
...
TL;DR: 本文提出了HTTM方法，一种无需训练的3D token合并技术，用于加速VGGT模型的大规模3D场景重建。该方法通过多头粒度的token合并，解决了现有方法导致特征token同质化的问题，实现了高达7倍的加速效果。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在3D场景重建中需要全局注意力计算，当处理大规模场景的长序列输入时会产生显著的延迟瓶颈。现有token合并方法在不同注意力头中统一合并token，导致输出层中token同质化，限制了模型的表示能力。

Method: 提出头级时序合并(HTTM)方法，在多头粒度上合并token，保持头级拼接后特征token的独特性。该方法利用头级观察到的空间局部性和时序对应性，以更低的合并成本实现更高的合并比率。

Result: 在GPU推理中实现了高达7倍的加速效果，性能下降可忽略不计。与现有方法相比，HTTM能够以更低的合并成本获得更高的合并比率。

Conclusion: HTTM是一种有效的训练免费加速方法，通过多头粒度的token合并策略，成功解决了VGGT模型在大规模3D场景重建中的延迟瓶颈问题，同时保持了模型的表示能力。
...
TL;DR: 提出了一种名为Merge-and-Bound（M&B）的类增量学习新训练方法，通过在参数空间中直接操作模型权重进行优化，包含跨任务权重合并和任务内权重合并两种机制，并通过有界更新技术减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 为了解决类增量学习中的灾难性遗忘问题，需要一种能够有效保留先前任务知识同时学习新任务的方法，而直接在参数空间操作权重可以更高效地实现这一目标。

Method: 采用两种权重合并策略：跨任务权重合并（统一先前所有阶段模型的权重）和任务内权重合并（在当前阶段内合并模型参数），结合有界更新技术限制累积更新幅度以保护先前知识。

Result: 在标准类增量学习基准测试中进行了广泛评估，相比最先进方法展现出优越性能。

Conclusion: M&B方法能够无缝集成到现有的类增量学习方法中，无需修改架构组件或调整学习目标，有效减少灾难性遗忘并获得接近旧模型的新模型。
...
<details>
  <summary>Details</summary>
Motivation: 对抗性攻击严重威胁基于学习的3D点云模型可靠性，现有防御方法存在高计算开销和跨攻击类型泛化能力差的问题。

Method: 采用教师-学生框架，通过三个不同教师模型（深度投影视觉模型、高性能3D模型、文本编码器）提供鲁棒嵌入，使用置信门控机制动态平衡多模态知识转移，在训练阶段完成轻量级提示学习。

Result: 大量实验表明MRPD显著优于最先进的防御方法，能够有效抵御多种白盒和黑盒攻击，甚至在干净数据上也能获得更好性能。

Conclusion: 该工作通过高效利用多模态知识，为构建鲁棒3D视觉系统提供了一种新的实用范式。
...
### [69] [Endo-G$^{2}$T: Geometry-Guided & Temporally Aware Time-Embedded 4DGS For Endoscopic Scenes](https://arxiv.org/abs/2511.21367)
*Yangle Liu,Fengze Li,Kan Liu,Jieming Ma*

Main category: cs.CV

TL;DR: Endo-G²T是一种用于动态内窥镜场景的几何引导和时间感知训练方案，通过几何先验蒸馏、时间嵌入高斯场和关键帧约束流式处理，在4D高斯溅射中实现早期几何锚定，保持时间一致性和效率。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视角依赖效应，纯光度监督会导致几何漂移问题。需要早期锚定几何形状，同时保持时间一致性和效率。

Method: 1) 几何引导先验蒸馏：将置信度门控的单目深度转换为监督信号；2) 时间嵌入高斯场：在XYZT空间中表示动态，使用转子式旋转参数化；3) 关键帧约束流式处理：在最大点数预算下进行关键帧优化。

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中达到了最先进的性能。

Conclusion: 该方法成功解决了内窥镜场景中的几何漂移问题，实现了时间一致的几何重建，并在效率和长期稳定性方面表现出色。
...
### [70] [Thinking With Bounding Boxes: Enhancing Spatio-Temporal Video Grounding via Reinforcement Fine-Tuning](https://arxiv.org/abs/2511.21375)
*Xin Gu,Haoji Zhang,Qihang Fan,Jingxuan Niu,Zhipeng Zhang,Libo Zhang,Guang Chen,Fan Chen,Longyin Wen,Sijie Zhu*

Main category: cs.CV

TL;DR: STVG-o1是首个无需修改架构就能让现成多模态大语言模型在时空视频定位任务中达到最先进性能的框架，通过边界框思维链机制和多维强化奖励函数实现。
...
Result: 在HCSTVG-v1/v2和VidSTG数据集上评估，STVG-o1在HCSTVG上创造了新的最先进结果，在HCSTVG-v1上比最佳任务特定方法高出7.3% m_tIoU，在VidSTG上与专用模型相当，大幅超越所有现有基于MLLM的方法。

Conclusion: STVG-o1展示了多模态大语言模型作为精确时空定位的强大骨干的可行性，并表现出强大的跨数据集开放词汇泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在图像生成中表现出色，但存在多样性不足的问题，即使对于简单提示也会产生几乎相同的图像。这个问题在图像质量主导的研究中被忽视。

Method: 1. 在文本嵌入中注入噪声以增强多样性；2. 提出尺度旅行精炼技术，使用多尺度自编码器提取粗粒度标记，在中间阶段恢复生成以保持图像质量。

Result: 结合文本嵌入噪声注入和尺度旅行精炼，在显著增强多样性的同时最小化图像质量下降，实现了多样性-质量权衡的新帕累托前沿。

Conclusion: DiverseVAR框架有效解决了VAR模型的多样性限制问题，无需重新训练或大量计算开销，为多样性-质量权衡提供了新的解决方案。
...
Result: 在四个数据集上测试，在RePAIR数据集上旋转误差降低23.1%，平移误差降低13.2%，Chamfer距离减少18.4%。

Conclusion: 多模态特征融合能有效解决仅依赖几何特征的局限性，显著提升3D重组精度，特别是在文化遗产保护等实际应用中。
...
Method: 1) 分析线性注意力和softmax注意力在移动设备上的性能，提出平衡生成效率与质量的线性混合架构去噪器；2) 设计时间步蒸馏策略，将采样步骤从20+压缩到仅2步；3) 应用移动端特定的注意力优化，使注意力操作速度提升2倍。

Result: MobileI2V首次在移动设备上实现快速720p图像到视频生成，质量与现有模型相当。在单步条件下，每帧720p视频生成时间小于100毫秒。
...
Method: 引入交叉注意力机制，使学生特征图的每个像素能够动态关注教师特征图的所有像素位置，仅通过额外损失函数实现非局部知识蒸馏。

Result: 在目标检测和图像分割任务上的大量实验表明，CanKD优于现有的特征和混合蒸馏方法，性能表现优异。
...
TL;DR: 本文系统研究深度伪造检测方法中不同设计选择对性能和泛化能力的影响，旨在建立架构无关的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更依赖于实现细节而非核心设计，这使得公平比较检测器变得困难。需要理解真正影响性能的因素。

Method: 通过系统调查不同设计选择对深度伪造检测模型准确性和泛化能力的影响，重点关注训练、推理和增量更新相关方面，隔离单个因素的影响。

Result: 实验识别出一组能持续改进深度伪造检测的设计选择，并在AI-GenBench基准测试中实现了最先进的性能。

Conclusion: 建立了一套稳健、架构无关的最佳实践，为未来深度伪造检测系统的设计和开发提供指导。
...
TL;DR: 本文提出了一种用于抗核抗体(ANA)检测的新框架，该框架处理多实例多标签(MIML)任务的复杂性，使用未经预处理的显微镜图像。该框架通过实例采样器、概率伪标签分发器和自定步调学习率系数实现，在ANA数据集上比现有最佳方法提升7.0% F1-Macro和12.6% mAP。


<details>
  <summary>Details</summary>
Motivation: ANA检测对于诊断自身免疫性疾病至关重要，但手动检测速度慢、劳动密集且需要多年训练。虽然机器学习和深度学习实现了自动化，但真实临床环境中的ANA检测涉及多实例多标签学习，存在独特挑战。

Method: 提出一种新颖框架，包含三个任务特定组件：实例采样器（通过建模模式置信度抑制低置信度实例）、概率伪标签分发器（基于实例可区分性自适应分配标签）和自定步调学习率系数（根据经验标签观察调整训练）。

Result: 在ANA数据集上，模型比最佳现有方法提升7.0% F1-Macro和12.6% mAP，创下新的最先进结果。在三个公共医疗MIML基准测试中，在所有关键指标上排名前2，汉明损失和单错误分别降低18.2%和26.9%。
...
TL;DR: 提出了一种用于构建遥感基础模型的高效集成专家框架，通过分解训练过程为轻量级任务特定的ConvNeXtV2专家模型，解决了现有大模型方法在计算资源、可持续性和可访问性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型主要依赖扩大模型规模和数据集体积，导致计算资源需求过高、碳足迹巨大，且仅限于少数大型机构使用，这与可持续和环境友好AI原则相悖。

Method: 采用集成专家框架，将训练过程分解为轻量级任务特定的ConvNeXtV2专家模型，这些专家模型可以被冻结和重用，支持联邦训练、剪枝和持续专家集成。

Result: 该方法在效率、可解释性和可扩展性方面具有显著优势，特别适合协作和资源受限的环境，为构建可扩展和高效的遥感基础模型提供了新方向。

Conclusion: 提出的集成专家框架为遥感基础模型的发展提供了一种高效、可持续的替代方案，解决了当前大模型范式在资源消耗和可访问性方面的问题。
...
TL;DR: 本研究提出了一种基于定量指标的序列图像生成方法，用于预测阿尔茨海默病进展，通过整合年龄缩放因子生成特定年龄的MRI图像，显著提高了MRI图像合成的准确性。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期识别对个性化治疗至关重要，但现有方法在输入序列时间间隔不规则时难以准确表征疾病特征，需要开发新的图像生成方法来保持疾病进展的关键特征。

Method: 提出创新的序列图像生成方法，基于定量指标指导生成过程，整合年龄缩放因子生成年龄特定的MRI图像，使用年龄缩放像素损失增强迭代生成效果。

Result: 消融研究表明，包含定量指标显著提高了MRI图像合成准确性，年龄缩放像素损失改善了MRI图像的迭代生成。在长期疾病预测中，结构相似性指数达到0.882峰值。

Conclusion: 该方法通过定量指标和年龄缩放因子有效提升了阿尔茨海默病MRI图像生成的准确性和相似度，为疾病进展预测提供了可靠工具。
...
TL;DR: 本文提出了PRFL框架，在潜在空间进行偏好优化，解决了视频生成中奖励反馈学习的内存和效率问题。
...
Conclusion: 潜在空间的奖励反馈学习比RGB空间方法更高效，能更好地优化视频生成质量。
...
TL;DR: UAVLight是一个用于光照鲁棒性3D重建的基准数据集，通过可重复的飞行路径在一天中不同时间捕获场景，提供自然光照变化下的标准化评估。
...
Method: 通过可重复、地理参考的飞行路径在一天中多个固定时间捕获每个场景，在一致的几何、校准和视点下产生自然光照变化。

Result: UAVLight提供了一个受控但真实的基准，用于开发在真实室外环境中一致、忠实且可重光照的重建方法。

Conclusion: 该数据集为开发光照鲁棒性3D重建方法提供了可靠基础，支持在不同光照条件下的标准化评估。
...
Method: 将2D/3D地标配准集成到U-Net地标预测模型的训练中，包括基线U-Net、带姿态估计损失的U-Net训练，以及带姿态估计损失的U-Net微调。

Result: 在患者姿态变化的真实手术条件下比较了不同方法的检测精度。

Conclusion: 该框架能够提高在姿态变化情况下的地标检测准确性。
...
Method: 提出跨任务协同训练范式缓解漂移，设计全局-局部解耦交互模块实现高效精确的时序风格对齐，开发同步增强CFG在推理时显式分离和放大对齐信号

Result: 大量实验证明Harmony建立了新的最先进水平，在生成保真度和细粒度音频-视频同步方面显著优于现有方法

Conclusion: Harmony框架通过机制化强制音频-视频同步，成功解决了联合扩散过程中的关键挑战，为同步音频-视觉内容合成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常普遍，但由于口腔内良性、癌前病变和恶性病变在视觉上相似，通常到晚期才被诊断出来。早期实施计算机辅助诊断系统有可能显著改善临床结果。

Method: 结合分层数据分割、高级数据增强和过采样技术来执行分类，以克服有限和不平衡数据集的挑战。

Result: 实验结果显示模型达到了83.33%的准确率、89.12%的精确率和77.31%的召回率，优于现有最先进方法。

Conclusion: 该模型有效展示了过采样和增强策略在少数类分类性能方面的有效性，作为可信赖的计算机辅助诊断系统在临床环境中早期检测口腔癌的第一步，显示出良好前景。
...
TL;DR: 论文主张LLM效率研究应从追求大规模复杂技术转向注重资源有限环境下的稳健简化，提出了包括模型架构改造、轻量微调、经济推理、动态知识管理等新研究方向，以降低部署成本、促进公平和可持续性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的LLM效率方法（如MoE、推测解码、复杂RAG）主要服务于大型科技公司，在资源有限的环境中效果不佳，导致中小型组织无法受益，加剧了技术不平等和碳浪费问题。

Method: 提出新的研究议程：无需重新训练即可改造预训练模型架构、轻量级微调保持对齐、长推理链的经济化、无需重型RAG管道的动态知识管理，以及采用开销感知效率作为标准基准。

Result: 通过重新定义效率概念，将采用成本、可持续性和公平性纳入考量，可以推动LLM部署的民主化。

Conclusion: LLM效率研究应转向关注在有限资源和专业知识下的稳健简化，通过降低部署门槛来减少不平等和碳浪费，而非加剧这些问题。
...
TL;DR: HTP是一种无需训练、词汇表或随机参数的可逆确定性文本嵌入框架，通过Unicode整数的谐波轨迹生成嵌入，在STS-B基准测试中达到0.68的Spearman相关性，并在多语言场景下保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 开发一种不依赖统计共现或优化的透明高效嵌入方法，证明语义关系可以从确定性几何中产生，提供数据驱动嵌入的替代方案。

Method: 基于Unicode整数表示的谐波轨迹，为每个标记生成解析编码，建立离散符号与连续向量空间的双射可解释映射。

Result: 在STS-B及其多语言扩展上，HTP在英语中达到Spearman相关性0.68，在十种语言中保持稳定性能，计算成本极低，每句对延迟亚毫秒级。

Conclusion: 有意义的语义关系可以从确定性几何中产生，HTP提供了一种透明高效的嵌入替代方案，无需训练即可实现语义相似性估计。
...
<details>
  <summary>Details</summary>
Motivation: 传统判别式奖励模型存在数据效率低和容易奖励过优化的问题，需要新的训练方法来提高奖励模型的效果和鲁棒性。

Method: 提出PIRA训练范式，包含三个策略：1) 将问答对重构为基于偏好的指令；2) 聚合多样化偏好任务的奖励；3) 在不同dropout率下平均价值头输出。

Result: 大量实验证明了PIRA方法的有效性。

Conclusion: PIRA通过创新的训练策略成功解决了奖励模型的数据效率低和奖励过优化问题，为LLM与人类偏好对齐提供了更有效的解决方案。
...
TL;DR: 本文研究如何通过重组法律文档结构和定义修辞角色来提升大语言模型在法律任务中的表现，在零样本设置下实现了1.5%-4.36%的F1分数提升。
...
Method: 通过三个实验：(i)基于修辞角色重组文档以评估结构化信息对长文本处理的影响；(ii)定义修辞角色使模型熟悉法律术语；(iii)模拟法院对修辞角色的逐步推理过程来增强模型推理能力。

Result: 在三个印度法律判决预测数据集上的实验显示，重组数据或解释关键法律术语能显著提升模型性能，F1分数相比基线最低提升约1.5%，最高提升4.36%。

Conclusion: 通过结构化法律文档信息和定义领域特定术语，可以有效提升大语言模型在法律任务中的表现，特别是在零样本设置下。
...
Method: 从Reddit收集自报告诊断数据，应用严格预处理步骤（语言过滤、NSFW内容移除、重复内容删除），使用LIWC进行语言分析，并进行了二元分类实验来验证数据集效用。

Result: MindSET数据集规模是之前基准的两倍多，在诊断检测任务中，基于MindSET训练的模型表现优于现有基准，自闭症检测的F1分数提高了18个百分点。
...
TL;DR: FlexCode是一个基于流行度感知的生成式推荐框架，通过自适应分配固定token预算在协同过滤和语义编码本之间，使用轻量级MoE平衡CF精度和语义泛化，解决了传统方法使用单一编码本处理热门和长尾物品的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐方法使用单一统一编码本编码所有物品，忽视了热门物品（富含协同信号）和长尾物品（依赖语义理解）之间的内在不平衡，这种统一处理限制了表示效率并阻碍了泛化能力。

Method: 引入FlexCode框架，自适应分配固定token预算在CF编码本和语义编码本之间；使用轻量级MoE动态平衡CF特定精度和语义泛化；通过对齐和平滑目标保持整个流行度谱系的连贯性。
...
TL;DR: 本文介绍了首个阿拉伯语跨领域上下文依赖的text-to-SQL数据集Ar-SParC，包含3,450个相关问答序列共10,225个问题及其SQL查询。使用GPT模型进行40个实验，并提出了GAT校正器方法，在零样本和上下文学习设置下均提升了性能。
...
Method: 构建了Ar-SParC数据集，使用GPT-3.5-turbo和GPT-4.5-turbo模型，应用10种提示工程技术和4种问题表示方法、6种上下文学习技术。提出了GAT校正器方法来提升性能。

Result: GAT校正器在所有40个实验中均提升了性能，零样本设置下执行准确率(EX)和交互准确率(IX)平均提升1.9%，上下文学习设置下EX提升1.72%、IX提升0.92%。

Conclusion: 成功构建了首个阿拉伯语跨领域上下文依赖text-to-SQL数据集，提出的GAT校正器方法有效提升了模型性能，为阿拉伯语自然语言数据库交互提供了重要基础。
...
Result: 23%的解释中出现推理错误，确认偏差和锚定偏差最为常见。推理失败与指南不一致和可能有害的建议相关，特别是在晚期疾病管理中。自动评估器可以检测错误存在但无法可靠分类子类型。
...
Method: 比较了两种路由方法：使用预计算嵌入的简单MLP和微调的RoBERTa变换器。在1,000个MMLU问题上进行评估，MLP路由器的准确率达到90.5%，比RoBERTa略高且参数少1.25亿。

Result: MLP路由器在测试数据上达到90.5%的路由准确率，略高于RoBERTa的89.5%。在三个主要LLM提供商(OpenAI GPT-4、Google Gemini、Anthropic Claude)的9,000次生产API调用验证中，路由决策具有提供商无关性，token减少量在32.6%到33.9%之间。

Conclusion: 动态模板选择方法能够在不影响响应质量的情况下显著降低token成本，路由决策在不同LLM提供商间具有良好泛化能力，为LLM部署提供了高效的成本优化方案。
...
TL;DR: 该论文探讨了二维材料在能源存储和转换中的应用，并指出研究论文中包含了材料特性和制备方法等有价值信息，但由于合成方法的分散性，信息获取存在挑战。


<details>
  <summary>Details</summary>
Motivation: 二维材料因其独特的物理化学和电子特性在能源领域有广泛应用，但相关信息的分散性限制了其有效利用，需要系统整理和分析。

Method: 通过分析已发表的研究论文，提取二维材料的特性和制备方法等信息，解决信息分散的问题。

Result: 识别出二维材料在能源存储和转换中的关键特性和制备方法，但信息分散性仍是主要挑战。

Conclusion: 需要系统化的方法来整合和分析二维材料的相关信息，以促进其在能源领域的进一步应用。
...
TL;DR: 本文提出了一种新的记忆化框架——多前缀记忆化，通过衡量模型能够通过多少不同的前缀来提取训练数据中的序列，从而更全面地评估语言模型的记忆化现象。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型记忆化定义存在不足，特别是在对齐模型中难以全面捕捉记忆化现象，这带来了隐私和版权风险。

Method: 引入多前缀记忆化框架，定义序列被记忆的条件是外部对抗搜索能够找到足够数量的不同前缀来引出该序列，通过量化记忆的鲁棒性来评估记忆化程度。

Result: 在开源和对齐聊天模型上的实验表明，多前缀定义能够可靠地区分记忆化和非记忆化数据，为审计LLM中的数据泄露提供了稳健实用的工具。

Conclusion: 多前缀记忆化框架提供了一个更全面和实用的方法来评估语言模型的记忆化现象，有助于更好地理解和缓解数据泄露风险。
...
TL;DR: 本文提出了一个可复现的DSPy+HELM框架，通过结构化提示方法激发推理，实现更准确的语言模型性能评估。研究发现，不使用结构化提示会导致性能低估、评估结果波动、性能差距误判等问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架（如HELM）依赖固定提示，无法跨模型泛化，导致性能估计不具代表性。需要估计每个语言模型的上限性能，避免低估其真实能力。

Method: 开发了DSPy+HELM框架，使用四种提示方法评估四个前沿语言模型在七个基准测试（通用/医疗领域）上的表现，并与现有HELM基线分数对比。

Result: 研究发现：HELM低估语言模型性能（平均4%）；性能估计在基准测试间波动更大（标准差增加2%）；性能差距被误判（3/7基准测试的排行榜排名翻转）；引入推理（思维链）降低模型对提示设计的敏感性。

Conclusion: 这是首个大规模实证研究，系统地表征了语言模型在不同基准测试和提示方法下的行为，表明可扩展的性能上限估计能够提供更有决策价值的基准测试。
...
TL;DR: 本文提出了一种新的Length-MAX分词器，通过最小化平均字符数来减少训练和推理时的token数量，相比BPE在不同词汇量下减少13-18%的token，显著提升训练效率和推理速度。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型使用的分词器（如BPE）主要基于频率优化，但未充分考虑token长度对效率的影响。本文旨在开发一种能最小化平均字符数的分词器，以提高语言模型的训练和推理效率。

Method: 将长度加权目标最大化建模为图划分问题，并开发贪心近似算法来获取词汇表。该方法优化平均token长度而非仅频率。

Result: 在FineWeb和多样化领域上，相比BPE减少14-18%的token；GPT-2模型训练步骤减少17-18.5%，推理延迟降低12.7-13.7%；下游任务表现提升，LAMBADA困惑度降低11.7%，HellaSwag准确率提高4.3%；词汇覆盖率达99.62%，OOV率仅为0.12%。

Conclusion: 优化平均token长度而非仅频率，为高效语言建模提供了有效方法，在不牺牲下游性能的情况下显著提升效率，且兼容生产系统，推理时减少18%的嵌入和KV缓存内存。
...
TL;DR: Evo-Memory是一个用于评估LLM智能体自进化记忆的流式基准测试框架，通过将数据集构建为顺序任务流，要求LLM在每次交互后搜索、适应和进化记忆。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注静态对话设置，忽略了在动态任务流中积累和重用经验的能力。现实环境中LLM需要处理连续任务流，但往往无法从积累的交互中学习，丢失有价值的上下文洞察。

Method: 引入Evo-Memory框架，统一实现超过10个代表性记忆模块，并提出ReMem方法——一个紧密集成推理、任务操作和记忆更新的行动-思考-记忆优化管道。

Result: 在10个多样化的多轮目标导向和单轮推理问答数据集上进行评估，提供了基线方法ExpRAG用于检索和利用先前经验。

Conclusion: Evo-Memory填补了LLM智能体自进化记忆评估的空白，支持在部署期间持续检索、整合和更新记忆，实现持续改进。
...
TL;DR: 本文研究了跨语言论元挖掘方法，针对低资源语言（波斯语）构建了三种训练场景：零样本迁移、LLM增强训练和跨语言模型，发现跨语言模型在波斯语测试集上取得了最佳性能（F1 74.8%）。


<details>
  <summary>Details</summary>
Motivation: 为了解决低资源语言在论元挖掘任务中数据不足的问题，探索有效的跨语言迁移方法。

Method: 构建三种训练场景：零样本迁移（仅用英语数据）、LLM增强训练（英语数据+LLM生成样本）、跨语言模型（英语数据+人工翻译波斯语句子）。

Result: 零样本迁移模型在英语测试集F1为50.2%，波斯语为50.7%；LLM增强模型分别提升至59.2%和69.3%；跨语言模型在波斯语测试集达到74.8% F1。

Conclusion: 轻量级的跨语言混合方法显著优于资源密集的增强管道，为低资源语言的论元挖掘任务提供了实用的解决方案。
...
TL;DR: 该论文提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究大语言模型如何实现语义角色。研究发现LLM形成了紧凑、因果隔离的抽象语义结构机制，这些机制在不同规模和架构间存在部分迁移。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型显示出语义能力，但其实现抽象语义结构的内在机制仍缺乏充分表征。需要研究LLM如何实现语义角色。

Method: 整合角色交叉最小对、时间涌现分析和跨模型比较的方法来研究LLM实现语义角色的机制。

Result: 发现：(i)高度集中的电路（89-94%归因在28个节点内）；(ii)渐进结构细化而非相变，较大模型有时绕过局部电路；(iii)中等跨尺度保守性（24-59%组件重叠）伴随高光谱相似性。

Conclusion: LLM形成了紧凑、因果隔离的抽象语义结构机制，这些机制在不同规模和架构间存在部分迁移。
...
TL;DR: Chatty-KG是一个用于知识图谱上对话式问答的模块化多智能体系统，结合了RAG式检索和结构化执行，通过专门的LLM智能体生成SPARQL查询，在单轮和多轮设置中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在知识图谱对话问答中的局限性：LLM缺乏对私有动态知识图谱的直接访问，RAG系统序列化图结构且难以处理多轮上下文，传统KGQA系统仅支持单轮问答且延迟高。

Method: 采用模块化多智能体架构，结合RAG式检索和结构化执行，通过专门LLM智能体进行上下文解释、对话跟踪、实体关系链接和高效查询规划，将自然语言问题转换为可执行SPARQL查询。

Result: 在大型多样化知识图谱上的实验显示，Chatty-KG在单轮和多轮设置中显著优于最先进的基线方法，获得更高的F1和P@1分数，与商业和开源LLM兼容且性能稳定。
...
TL;DR: 本文研究了大型语言模型在不同类型语言查询中的表现差异，发现LLM在定义类问题中表现最佳，而在举例类问题中表现最差。研究还表明LLM倾向于对流行知识进行更多改写，而对尾部技术知识改写较少。


<details>
  <summary>Details</summary>
Motivation: 虽然人类能够轻松提供各种类型的答案（如例子和释义），但大型语言模型在除定义类查询外的其他类型查询中表现不佳。本研究旨在评估这种性能下降，并研究预训练数据对LLM回答多样化语言查询的影响。

Method: 使用TrackList细粒度语言和统计分析管道，引入RefoMed-EN数据集（包含6170个人工标注的医学术语及其定义、命名、举例、解释或释义），通过句法和语义相似性指标、统计相关性和嵌入来评估LLM输出质量。

Result: 结果显示LLM在定义类问题中任务表现最高，在举例类问题中表现最低。对于定义类问题，大型语言模型倾向于对流行和频繁知识进行更多改写，而对尾部和专业知识改写较少，特别是在专家文本中。

Conclusion: 大型语言模型在处理不同类型语言查询时存在显著性能差异，定义类问题表现最佳而举例类问题表现最差。模型对知识的改写行为与知识频率相关，高频知识更易被改写。
...
TL;DR: 本文研究了上下文学习(ICL)是否能覆盖预训练的标签语义，发现ICL主要是在预训练的语义基础上进行微调，而非重新映射标签含义。


<details>
  <summary>Details</summary>
Motivation: 探讨ICL是能够覆盖预训练的标签语义，还是仅仅在现有语义基础上进行精炼，以理解ICL的基本机制和局限性。

Method: 将LLMs视为提示诱导的分类器，对比自然演示（正确标签）和反转演示（系统翻转标签含义）下的行为，定义了三个对齐指标和语义覆盖率。

Result: 在8个分类任务和8个开源LLM上的实验表明，ICL无法学习连贯的反语义分类器，语义覆盖率在少样本设置下保持为零。

Conclusion: ICL主要调整输入如何投影到预训练期间学习的稳定语义方向上，而非灵活重新映射标签含义，表明覆盖标签语义需要ICL之外的干预措施。
...
TL;DR: 本文探索在低资源语言（如缅甸语）分类任务中使用Kolmogorov-Arnold Networks (KANs)作为分类头替代传统的多层感知机(MLPs)，评估了三种KAN变体在不同嵌入表示上的性能。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言分类任务中，通常只微调最后的分类层而保持预训练编码器权重冻结。MLPs虽然常用，但其固定的非线性可能限制表达能力并增加计算成本，因此需要寻找更优的替代方案。

Method: 评估了三种KAN变体：基于傅里叶的FourierKAN、基于样条的EfficientKAN和基于网格的FasterKAN，使用TF-IDF、fastText和多语言transformer（mBERT、Distil-mBERT）等多种嵌入表示进行实验。

Result: 实验结果显示KAN分类头与MLPs相当或更优。EfficientKAN与fastText组合获得最高F1分数（0.928），FasterKAN在速度和精度之间提供了最佳平衡。在transformer嵌入上，EfficientKAN与mBERT组合达到0.917 F1分数，匹配或略优于MLPs。

Conclusion: KANs作为MLPs的表达性、高效替代方案，在低资源语言分类任务中表现出色，为这类任务提供了新的技术选择。
...
TL;DR: 该研究评估了28种不同配置的大语言模型在58个字符级约束的单词谜题上的表现，发现架构差异比参数规模对约束满足能力的影响更大，模型在处理拼写不常见但约束有效的词汇时存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 系统评估不同架构的大语言模型在受控文本生成中满足硬性拼写约束的能力，现有研究在这方面存在局限。

Method: 使用三个模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）的28种配置，在58个需要字符级约束满足的单词谜题上进行评估，并利用10,000名人类解谜者的难度评级进行分析。

Result: 架构差异产生的性能差距（2.0-2.2倍）远大于家族内参数扩展带来的增益（83%），高容量模型在思考预算增加时表现提升明显，而中等规模模型趋于饱和或退化。模型在拼写不常见的常见词汇上存在系统性失败（人类成功率86-95%，模型失败率89-96%）。

Conclusion: 约束满足可能需要专门的架构特征或训练目标，而不仅仅是标准语言模型扩展，模型过度依赖分布合理性而惩罚拼写不典型但约束有效的模式，表明需要超越简单参数扩展或计算预算的架构创新。
...
TL;DR: 本文提出了MortgageLLM，一个专门针对抵押贷款金融领域的双专家大语言模型，通过双轨道专业化框架解决了单一模型在结构化任务和对话任务之间的性能权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用领域表现出色，但在专业领域如抵押贷款金融中需要领域知识增强，同时保持指令遵循能力。单一模型在多任务优化时存在性能权衡问题。

Method: 采用双轨道专业化框架，从单一基础模型(LLaMA-3.1-8B)创建两个专家模型：对话问答模型和结构化任务模型。应用指令残差技术恢复领域适应后的指令遵循能力，并使用少量样本分类的任务路由机制。

Result: 在领域特定基准测试中，最终模型(MLM v2)显著优于基础模型：LLM-as-a-Judge评分在摘要任务中为4.58(对比3.99)，问答任务为4.09(对比4.0)，分类任务为2.6(对比1.2)；BERTScore在摘要任务中为0.77(对比0.74)，问答任务为0.68(对比0.58)，分类任务为0.75(对比0.73)。

Conclusion: 双专家架构有效解决了专业领域大语言模型在结构化任务和对话任务之间的性能权衡问题，在抵押贷款金融领域实现了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 确保推理模型在对抗性越狱提示下的安全性是一个关键挑战，需要自适应安全对齐方法使模型能够自主增强防御。
...
TL;DR: 本研究探讨了在少量人类调查数据上微调大型语言模型是否能改善其模拟人类行为的真实性。通过信息披露行为实验，发现微调确实提高了模型的异质性、对齐性和信念-行为一致性，但仍无法准确复现原始研究的回归系数。


<details>
  <summary>Details</summary>
Motivation: 针对LLM能否替代人类参与者的争议，本文研究通过少量人类数据微调是否能解决LLM模拟人类行为的局限性，如多样性不足、少数群体对齐问题等。

Method: 使用信息披露行为实验，比较人类和LLM生成的反应，分析分布差异、子群体对齐、信念-行为一致性以及回归系数恢复等多个维度。

Result: 在少量人类样本上微调显著提高了模型的异质性、对齐性和信念-行为一致性，但即使表现最好的微调模型也无法复现原始研究的回归系数。

Conclusion: 虽然微调改善了LLM模拟人类行为的某些方面，但LLM生成的数据仍不适合在正式推断分析中替代人类参与者。
...
TL;DR: 本文开发了首个伊森语开放对话语音数据集，该语言是泰国使用最广泛的地区方言。与现有的朗读或脚本语音语料库不同，该数据集包含自然语音，捕捉了真实的语言现象如口语表达、自发韵律、不流畅性和与泰语频繁的语码转换。


<details>
  <summary>Details</summary>
Motivation: 伊森语作为泰国最广泛使用的地区方言，缺乏标准化的书写系统，现有语音语料库多为朗读或脚本语音，无法捕捉真实的对话特征。开发自然对话语音数据集旨在支持包容性AI发展，促进对代表性不足语言的研究。

Method: 建立实用的转录协议，在表征准确性和计算处理需求之间取得平衡。处理伊森语缺乏标准化正字法的挑战，设计转录指南以应对书写实践的高度变异性。

Result: 成功创建了首个伊森语开放对话语音数据集，包含自然语音，捕捉了口语表达、自发韵律、不流畅性和语码转换等真实语言现象。

Conclusion: 通过发布这一开放资源，为包容性AI发展做出贡献，支持对代表性不足语言的研究，并为解决对话语音建模中的语言和技术挑战提供基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上取得了最先进的性能，但其大规模性导致高计算和环境成本，限制了可访问性。参数高效微调方法通过减少可训练参数来解决这一挑战，但当前评估仍有限且难以复现。

Method: 引入PEFT-Bench统一基准测试，在27个NLP数据集和6种PEFT方法上进行评估，并提出了PSCP指标来综合考虑可训练参数、推理速度和训练内存使用等因素。

Result: 开发了一个端到端的基准测试框架，能够系统评估不同PEFT方法的性能表现。

Conclusion: PEFT-Bench填补了当前PEFT方法评估的空白，提供了一个统一且可复现的评估平台，有助于更全面地比较不同参数高效微调方法的优劣。
...
Result: 结果显示，LLMs经常无法逐字复制源文本中的证据，导致无效输出。llama3.1:8b模型尽管规模相对较小，但正确输出比例很高，而gpt-oss-120b模型尽管参数更多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b模型在模型大小与人类标注一致性之间达到了有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中经常无法准确复制源文本证据，模型大小与性能之间不存在简单的正相关关系，某些中等规模模型在性能与效率之间达到了更好的平衡。
...
TL;DR: DSR-SQL是一个双状态推理框架，通过自适应上下文状态和渐进生成状态的交互来解决复杂企业数据库中的Text-to-SQL问题，在无需后训练或上下文示例的情况下实现了竞争性性能。
...
Conclusion: DSR-SQL框架有效解决了复杂企业数据库中的Text-to-SQL挑战，展示了在没有额外训练数据的情况下实现竞争性性能的潜力。
...
TL;DR: Odin是一个新颖的架构，通过定向双模块机制在特定Transformer层注入图结构，避免了传统GNN的过平滑问题，同时解决了Transformer忽略图拓扑的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本属性图处理方法存在局限：GNNs面临过平滑和跳数依赖扩散问题，而Transformers则忽略了图拓扑结构，将节点视为孤立序列。

Method: 提出Odin架构，在选定的Transformer深度通过定向双模块机制注入图结构，不依赖多跳扩散，而是在特定层集成多跳结构，产生与语义层次对齐的低、中、高级结构抽象。

Result: 在多个文本丰富图基准测试中，Odin实现了最先进的准确率，而轻量版Light Odin在显著降低计算成本的同时保持了竞争力。

Conclusion: Odin和Light Odin形成了一个统一的、无跳数的框架，用于原则性的结构与文本集成，其表达能力严格包含纯Transformers和GNNs。
...
TL;DR: 本文系统评估了六种模型合并方法在大型语言模型上的表现，发现最简单的方法Task Arithmetic是唯一能可靠提升性能的方法，而其他复杂方法通常导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 模型合并是一种无需额外训练即可复用多个微调模型的有效方法，但现有研究主要针对小模型和分类器，不清楚这些方法是否适用于大型语言模型。

Method: 对六种最先进的合并方法（包括子空间方法）进行大规模系统评估，涵盖四个开源权重LLM、每个基础模型十二个微调检查点以及十六个标准LLM基准测试。

Result: Task Arithmetic是唯一能可靠提升LLM性能的方法，其他干扰感知和子空间合并方法通常导致显著的性能下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在可读性评估中未能充分考虑文本长度和可读性标签的序数关系，需要改进这些方面。

Method: 采用双向可读性评估机制预测句子级可读性标签，并使用成对排序算法通过标签减法建模可读性级别的序数关系。

Result: 在中文和英文数据集上的实验结果表明，该模型取得了有竞争力的性能，优于其他基线模型。

Conclusion: 提出的方法能够有效解决文本长度和可读性标签序数关系的问题，在可读性评估任务中表现出色。
...
TL;DR: 本文研究了语音翻译模型中性别分配机制，发现模型不仅复制训练数据中的性别关联，还学习更广泛的男性主导模式。模型能够基于声学输入覆盖语言模型的内在偏见，并通过第一人称代词将性别化术语与说话者关联，利用分布在整个频谱而非集中于音高的性别信息。


<details>
  <summary>Details</summary>
Motivation: 语音通过音高等声学线索传达说话者性别信息，这在语音翻译中引发特定模态的偏见问题。当从有概念性性别的语言翻译到需要为性别模糊术语分配语法性别的语言时，说话者的声音特征可能影响性别分配，存在错误性别识别的风险。但目前对语音翻译模型如何做出这些决策的理解仍然不足。

Method: 研究考察了三个语言对（英语-西班牙语/法语/意大利语）中语音翻译模型为说话者相关术语分配性别的机制，分析了训练数据模式、内部语言模型偏见和声学信息如何相互作用。使用对比性特征归因方法分析频谱图。

Result: 模型不简单复制训练数据中的术语特定性别关联，而是学习更广泛的男性主导模式。虽然内部语言模型表现出强烈的男性偏见，但模型能够基于声学输入覆盖这些偏好。具有更高性别准确率的模型使用第一人称代词将性别化术语链接回说话者，访问分布在整个频率频谱而非集中于音高的性别信息。

Conclusion: 语音翻译模型发展出复杂的性别分配机制，能够整合声学和语言信息，通过第一人称代词建立说话者与性别化术语的连接，利用分布式的频谱信息而非仅依赖音高特征来准确识别说话者性别。
...
TL;DR: 本文介绍了孟加拉手语翻译数据集IsharaKhobor及其两个子集，旨在解决孟加拉手语资源匮乏问题，为开发AI辅助工具提供支持。
...
Result: 开发了三个公开可用的数据集：IsharaKhobor、IsharaKhobor_small和IsharaKhobor_canonical_small，已在Kaggle平台发布。

Conclusion: 该数据集填补了孟加拉手语翻译研究的空白，为后续AI辅助工具开发提供了重要资源基础。
...
TL;DR: RoParQ是一个专门评估闭卷多选QA中跨释义一致性的基准，通过生成释义并筛选不一致样本来构建。提出了XParaCon评估指标和基于推理的释义感知SFT策略，显著提升了模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在回答释义问题时经常表现出不一致行为，表明其依赖表面模式而非真正的语义理解。为解决这一局限性，需要专门评估和提升模型的跨释义一致性。

Method: 从标准数据集生成释义，通过法官模型筛选不一致样本构建RoParQ基准。提出XParaCon评估指标，并实施基于推理的释义感知监督微调策略。

Result: 实验表明，针对性对齐显著增强了鲁棒性。微调后的轻量级模型达到了与更大预训练模型相当的稳定性水平。

Conclusion: 该方法有效缓解了表面记忆问题，培养了更鲁棒可靠的大语言模型，证明了语义对齐策略的有效性。
...
TL;DR: 本文研究使用大型语言模型自动重构层次结构以优化双曲嵌入质量，通过提示方法将现有层次结构转化为具有高分支因子和单继承特性的结构，实验证明重构后的层次结构在多个标准嵌入质量指标上表现更好。


<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构紧密相关，而最优双曲嵌入需要高分支因子和单继承特性。为了帮助知识工程师重新组织层次知识，研究LLMs是否能够自动重构层次结构以满足这些标准。

Method: 提出基于提示的方法，使用LLMs在已知双曲嵌入期望标准的指导下转换现有层次结构。

Result: 在16个不同层次结构上的实验表明，LLM重构的层次结构在多个标准嵌入质量指标上始终产生更高质量的双曲嵌入。

Conclusion: LLM引导的层次结构重构不仅提高了双曲嵌入质量，还实现了可解释的重组，为知识工程师提供合理性解释。
...
TL;DR: 本文介绍了AssurAI，一个专门针对韩语多模态生成AI安全评估的质量控制数据集，包含11,480个实例，涵盖文本、图像、视频和音频，旨在解决当前安全数据集主要面向英语且缺乏韩语社会文化背景的问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成AI安全评估数据集主要基于英语，无法捕捉非英语社会文化背景（如韩语）中的特定风险，且多为单模态文本数据，存在局限性。

Method: 1. 定义包含35个AI风险因素的分类法；2. 构建大规模韩语多模态数据集AssurAI；3. 采用两阶段构建（专家引导种子和众包扩展）、三重独立标注和迭代专家红队测试的严格质量控制流程。

Result: 试点研究验证了AssurAI在评估最新大语言模型安全性方面的有效性，数据集已公开发布以促进更安全可靠的韩语生成AI系统开发。

Conclusion: AssurAI填补了韩语多模态AI安全评估的空白，通过严格的质量控制和针对韩语社会文化背景的风险分类，为开发更安全的韩语生成AI系统提供了有效工具。
...
TL;DR: 该论文提出了一个名为"Reasoning With a Star"的日球物理学数据集和基准测试方法，用于评估大型语言模型在科学推理中的表现，包括物理假设、单位一致性和科学格式等要求。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在日球物理学科学推理中的挑战，包括整合物理假设、保持单位一致性和提供清晰科学格式的需求。
...
Conclusion: 分解工作流程的系统工程方法在科学推理任务中比直接提示更有效，特别是在需要演绎推理而非纯粹归纳回忆的问题上。
...
<details>
  <summary>Details</summary>
Motivation: 推动医疗从被动治疗向预测性、预防性和个性化医疗转变，利用数字孪生技术改善患者治疗效果和加速药物发现。

Method: 整合医学影像、生物传感器和计算模型，创建动态数据驱动的患者虚拟副本，通过实时数据流持续更新并支持双向交互。

Result: 已开发出心脏数字孪生预测心律失常治疗效果、肿瘤学数字孪生跟踪肿瘤进展优化放疗、药理学数字孪生加速药物发现等代表性应用。

Conclusion: 尽管面临互操作性、数据隐私和模型保真度等挑战，但通过可解释AI、联邦学习和统一监管框架等新兴解决方案，数字孪生技术有望实现多器官整合、基因组学集成和伦理治理，最终推动医疗向预测性、预防性和个性化方向发展。
...
TL;DR: 本文对多模态思维链推理进行了全面分析，评估其在A-OKVQA、OKVQA和ChartQA数据集上的有效性，发现视觉集成显著减少推理幻觉，但思维链推理效果在不同问题类型中差异显著。


<details>
  <summary>Details</summary>
Motivation: 虽然最近工作将思维链扩展到多模态设置并在科学问答基准上取得最佳结果，但这些方法在不同领域的泛化能力仍未充分探索。

Method: 采用Zhang等人提出的两阶段框架，将推理生成与答案推断分离，并通过门控融合机制将视觉特征与基于T5的语言模型集成。

Result: 视觉集成显著减少推理生成中的幻觉，但思维链推理的有效性在不同问题类型中差异很大，常识推理面临特别挑战。
...
TL;DR: RILKE是一种在表示空间进行干预的知识控制方法，能够在无需重新训练的情况下高效更新LLM的知识，支持大规模终身知识编辑，同时保持模型通用能力。


<details>
  <summary>Details</summary>
Motivation: 解决LLM知识错误或过时的问题，避免昂贵的重新训练，特别是在终身学习场景下处理复杂非结构化知识的多重编辑需求。

Method: 在模型表示空间进行干预，学习抗释义和编辑局部化的模块，将每个更新限制在低维子空间以减少干扰，推理时通过查询自适应路由器选择适当模块。
...
TL;DR: ENACT是一个评估具身认知的基准测试，通过视觉问答形式评估模型从自我中心交互中进行世界建模的能力，包含前向世界建模和逆向世界建模两个序列重排序任务。
...
Method: 将具身认知评估构建为部分可观察马尔可夫决策过程，动作是场景图变化，包含两个互补的序列重排序任务：给定动作重新排序观察序列（前向世界建模）和给定观察重新排序动作序列（逆向世界建模）。

Result: 实验显示前沿视觉语言模型与人类之间存在性能差距，且随着交互视野扩大而加剧。模型在逆向任务上表现更好，并表现出人类中心偏见，如偏好右手动作，当相机参数或视角偏离人类视觉时性能下降。

Conclusion: ENACT基准测试揭示了现代视觉语言模型在具身认知能力方面与人类存在差距，并显示出人类中心偏见，为评估模型具身认知能力提供了有效工具。
...
TL;DR: Ivy是一个AI教练系统，通过结合符号化的TMK模型和生成式解释层，提供结构化的多步骤解释，以改善程序性技能学习中的解释质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成流畅的回答，但往往缺乏因果、目标导向和组合逻辑的结构，无法有效传达程序性技能学习所需的深层理解。

Method: 将符号化的任务-方法-知识（TMK）模型与生成式解释层结合，TMK编码因果转换、目标层次和问题分解，约束LLM在明确的结构边界内构建解释。

Result: 与GPT和检索增强GPT基线相比，符号约束显著提高了对"如何"和"为什么"问题的解释结构质量，在三个推理维度上获得专家和独立标注者的认可。
...
TL;DR: 本文提出ICPO方法，通过利用LLM生成不同响应的概率来反映其对推理过程的自我评估，结合可验证奖励来解决现有RLVR方法中存在的粗粒度奖励、奖励噪声和低效探索等问题。
...
Method: 提出ICPO方法，通过计算多个响应在同一输入提示下的相对生成概率得到偏好优势分数，并将该分数与可验证奖励结合来指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO能稳定提升推理能力。

Conclusion: ICPO方法通过偏好优势分数有效缓解了粗粒度奖励和奖励噪声问题，抑制了过度自信错误，增强了被低估高质量响应的相对优势，防止模型过度拟合特定策略，促进更彻底的探索。
...
TL;DR: 本文对Halpern和Pearl的因果定义进行了抽象化，提取其核心特征，使其能够应用于任何定义了反事实的模型。这种抽象化带来了更广泛的应用范围、处理更复杂逻辑公式的能力，以及对因果定义更深层次的理解。


<details>
  <summary>Details</summary>
Motivation: Halpern和Pearl的因果定义虽然重要，但局限于因果模型。作者希望通过抽象化该定义，使其能够应用于更广泛的模型类型，包括允许回溯的模型，并能处理包含析取、否定、信念和嵌套反事实的复杂公式。

Method: 通过提取Halpern-Pearl因果定义的关键特征，构建一个抽象化的定义框架。这个框架不依赖于特定的因果模型结构，而是基于反事实的定义，从而可以应用于任何定义了反事实的模型。

Result: 成功开发了一个抽象化的因果定义，能够：1) 应用于更广泛的模型类型；2) 处理包含析取、否定、信念和嵌套反事实的复杂公式；3) 扩展到解释的定义；4) 加深对因果定义特征的理解。

Conclusion: 抽象化Halpern-Pearl因果定义不仅扩展了其应用范围，还提升了处理复杂逻辑关系的能力，并为理解因果关系的本质提供了新的视角。这种方法在理论和实践上都具有重要意义。
...
TL;DR: Prune4Web是一个创新的网页自动化范式，通过将DOM处理从资源密集的LLM读取转向高效的程序化修剪，解决了复杂网页DOM结构过大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有网页自动化方法面临DOM结构过大（10,000-100,000个token）的挑战，现有策略要么依赖粗暴的DOM截断（可能丢失关键信息），要么使用低效的启发式方法和独立排序模型，无法在精度和可扩展性之间达到最佳平衡。

Method: 提出DOM树修剪编程方法，让LLM生成可执行的Python评分脚本，基于分解子任务的语义线索动态过滤DOM元素。该方法将遍历和评分委托给轻量级、可解释的程序，避免了LLM直接处理原始大型DOM的需求。

Result: 实现了候选元素25倍到50倍的减少，显著提高了动作定位精度。在低级别接地任务上，准确率从46.8%大幅提升至88.28%。

Conclusion: Prune4Web通过程序化DOM修剪机制，在真实世界网页自动化中实现了最先进的性能，有效解决了大规模DOM处理的可扩展性和精度问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是结合了观察文字方案和计数方法的混合模式，但需要改进其决策启发式

Method: 为混合模式引入了新的启发式决策方法

Result: 新启发式方法能够显著超越RoundingSAT求解器中的当前方法

Conclusion: 新的混合决策启发式在伪布尔求解中表现优异，大幅提升了求解器性能
...
TL;DR: MADRA是一个无需训练的多智能体辩论风险评估框架，通过集体推理增强安全意识而不牺牲任务性能，在AI2-THOR和VirtualHome上实现超过90%的危险任务拒绝率。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在具身AI任务规划中的安全问题，现有方法存在计算成本高或过度拒绝的问题，特别是在家庭环境中危险指令构成重大风险。

Method: 采用多智能体辩论框架，多个基于LLM的智能体辩论给定指令的安全性，由关键评估器根据逻辑合理性、风险识别、证据质量和清晰度评分，通过迭代审议和共识投票。

Result: 在AI2-THOR和VirtualHome上的广泛实验表明，该方法实现了超过90%的不安全任务拒绝率，同时确保安全任务拒绝率低，在安全性和执行效率方面优于现有方法。
...
TL;DR: 本文提出了一个分层空间认知框架，将空间智能分解为五个渐进复杂层次，并构建了SpatialBench基准测试来评估多模态大语言模型的空间推理能力。
...
Method: 提出分层空间认知框架，将空间智能分解为五个认知层次；构建SpatialBench大规模细粒度基准测试，涵盖15个任务；引入高层能力导向的统一评估指标。

Result: 大规模实验显示模型在不同认知层次上表现分层：模型在感知基础方面表现强劲，但在符号推理、因果推理和规划方面仍有限制；人类测试表明人类进行选择性、目标导向的抽象，而MLLMs倾向于过度关注表面细节。
...
TL;DR: 本文提出悲观验证方法，通过构建多个并行验证来判断数学证明的正确性，显著提升了数学问题验证性能且计算资源消耗低。


<details>
  <summary>Details</summary>
Motivation: 现有验证性能的关键限制在于错误检测能力，因此需要设计更有效的验证方法来提高开放数学问题的验证准确性。

Method: 设计悲观验证的多个变体，为同一证明构建多个并行验证，只要任一验证报告错误就判定证明不正确。

Result: 该方法在多个数学验证基准测试中显著提升性能，计算资源消耗低，token效率甚至超过扩展长链思维测试时缩放。

Conclusion: 悲观验证能有效提升语言模型输出的可靠性和性能，在长视野数学任务中发挥关键作用，有助于增强语言模型的数学能力。
...
TL;DR: 本研究审计了16个开源模型在专业角色下的AI身份披露情况，发现在高风险领域存在严重的领域特定不一致性，某些模型的身份披露率在不同专业角色间差异巨大，表明透明度反映的是训练因素而非模型规模。


<details>
  <summary>Details</summary>
Motivation: 在专家语境中，如果语言模型不能可靠地披露其AI身份，用户就无法信任其能力边界。本研究旨在评估模型在承担高风险领域专业角色时的自我透明度，以防止虚假专业知识对用户造成伤害。

Method: 采用共同花园设计，对16个开源模型（4B-671B参数）进行了19,200次试验审计，评估不同专业角色下的身份披露行为，并使用贝叶斯验证和Rogan-Gladen校正确保测量误差的稳健性。

Result: 模型表现出显著的领域特定不一致性：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。披露率从2.8%到73.6%不等，模型身份比参数数量更能预测行为（ΔR²adj=0.359 vs 0.018）。推理优化在某些模型中主动抑制了自我透明度，推理变体的披露率比基础模型低48.4%。
...
TL;DR: 本文提出了"负责任计算预见"概念，探讨人工智能在负责任预见中的作用，建立了该领域的基本原则，并展示了一系列AI驱动的预见工具。
...
Method: 通过分析人类中心人工智能和计算建模在负责任预见中的作用，建立负责任计算预见的基本原则，并开发AI驱动的预见工具，结合模拟和情景分析。

Result: AI增强了政策制定者应对不确定性、评估风险和制定可持续战略的能力，但负责任预见需要超越技术预测，理解社会、环境、经济和政治系统的相互依赖性。

Conclusion: AI将作为支持性工具在负责任、以人为中心的预见中发挥作用，补充而非替代政策制定者的判断，实现主动塑造有韧性和道德健全的未来。
