...
Motivation: 联邦学习在非独立同分布（non-IID）客户端数据上面临严重挑战，特别是在多模态感知场景中，传统方法难以解决客户端间的语义差异，导致多媒体系统感知性能不佳

Method: 1. 利用预训练Stable Diffusion模型的多层语义表示（包括VAE编码的潜在空间和U-Net分层特征）创建共享潜在空间；2. 采用高效的客户端-服务器架构，将繁重计算卸载到服务器；3. 使用跨模态对比学习的统一一致性机制稳定收敛
...
TL;DR: 提出基于Qwen3-VL视觉编码器的统一多任务框架，用于超声图像分析，在FMC-UIA挑战中性能从67%提升至85%


<details>
  <summary>Details</summary>
Motivation: 超声图像因扫描设备、操作者和解剖目标不同而存在巨大差异，导致在单一场景训练的模型难以泛化到新的医院和临床条件。FMC-UIA挑战要求单一模型处理多个任务，包括分割、检测、分类和关键点回归。

Method: 使用Qwen3-VL家族的Transformer视觉编码器构建统一多任务框架。将中间token特征投影为空间特征图，通过轻量级多尺度特征金字塔融合，实现像素级预测和全局推理。每个任务使用小型任务特定预测头，训练采用任务感知采样和选择性损失平衡来管理异构监督和减少任务不平衡。

Result: 验证集性能从67%提升至85%，在官方测试集上所有任务平均得分达到81.84%。

Conclusion: 提出的方法简单易优化，能够适应广泛的超声分析任务，有效解决了超声图像分析中的泛化问题。
...
<details>
  <summary>Details</summary>
Motivation: FireANTs框架使用Adam优化器进行图像配准，但Adam需要存储动量估计等状态变量，消耗大量内存，限制了其在大尺寸图像（如大体积医学图像）上的应用。

Method: 提出改进的Levenberg-Marquardt优化器，仅需单个标量阻尼参数作为优化器状态，采用信任区域方法自适应调整该参数。同时研究了使用Metropolis-Hastings拒绝步骤防止损失函数恶化的有效性。

Result: 改进的LM优化器在大体积图像上减少内存达24.6%，在四个数据集上保持性能。单一超参数配置在脑MRI上调整后，无需修改即可应用于肺CT和跨模态腹部配准，在四个基准测试中的三个上匹配或优于Adam。

Conclusion: 提出的改进LM优化器有效解决了FireANTs在大图像配准中的内存瓶颈问题，同时保持了配准性能，展示了良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光中临床相关发现通常空间受限，需要细粒度表示学习。现有对比模型缺乏区域级监督，大型视觉语言模型在外部验证中捕获细粒度表示能力有限，导致这些任务性能不佳。
...
Conclusion: LoFi方法通过位置感知的细粒度表示学习，有效解决了胸部X光图像检索和短语定位中的细粒度表示学习问题，在多个数据集上表现出优越性能。
...
TL;DR: 本文介绍了ProactiveBench基准测试，用于评估多模态大语言模型是否具备主动请求用户干预的能力，发现现有模型普遍缺乏主动性，且主动性不与模型能力正相关，但可以通过强化学习微调来学习主动性。


<details>
  <summary>Details</summary>
Motivation: 人类在协作中知道何时寻求帮助（如请求移除遮挡物来识别物体），但多模态大语言模型是否具备类似的"主动性"行为尚不清楚。研究者希望探究MLLMs能否主动请求简单的用户干预来完成任务。

Method: 构建了ProactiveBench基准测试，包含7个重新设计的数据集，涵盖识别遮挡物体、提升图像质量、解释粗略草图等任务。评估了22个MLLMs，分析了提示策略、对话历史和上下文学习的影响，并探索了基于强化学习的简单微调策略。

Result: 研究发现：(1) MLLMs普遍缺乏主动性；(2) 主动性与模型能力不相关；(3) 提示主动性仅带来边际收益；(4) 对话历史和上下文学习会引入负面偏见，阻碍性能。强化学习微调表明主动性可以学习，并能泛化到未见场景。

Conclusion: 多模态大语言模型目前缺乏主动性，但可以通过训练来学习这种能力。ProactiveBench作为首个主动性基准测试，为构建主动的多模态模型迈出了第一步。
...
TL;DR: 提出了NA-VQA基准测试来评估长视频中的深度时序和叙事推理能力，包含88部完整电影和4.4K个开放性问题，并开发了Video-NaRA框架来提升长距离推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型基准测试大多依赖局部线索，无法捕捉完整的叙事推理能力，包括追踪意图、连接远距离事件和重建因果链。需要专门评估长视频中深度时序和叙事推理的基准。

Method: 1) 创建NA-VQA基准：包含88部完整电影和4.4K个开放性问题-答案对，每个问题基于多个证据跨度（短、中、远距离）标注；2) 提出Video-NaRA框架：构建事件级链条并存储在结构化记忆中，用于推理时的检索。

Result: 实验表明最先进的多模态大语言模型在需要远距离证据的问题上表现不佳，突出了显式叙事建模的必要性。Video-NaRA将长距离推理性能提升了最多3%，证明其在处理复杂叙事结构方面的有效性。

Conclusion: NA-VQA基准测试填补了长视频深度叙事推理评估的空白，Video-NaRA框架通过结构化事件记忆有效提升了长距离推理能力，为多模态大语言模型的叙事理解提供了新的方向。
...
### [7] [FedAgain: A Trust-Based and Robust Federated Learning Strategy for an Automated Kidney Stone Identification in Ureteroscopy](https://arxiv.org/abs/2603.19512)
...
<details>
  <summary>Details</summary>
Motivation: 医学影像AI的可靠性严重依赖于其对不同医院设备采集的异质和损坏图像的鲁棒性，这在现实临床环境中极具挑战性。需要一种能够在保护数据隐私的同时，处理非独立同分布数据和噪声客户端更新的联邦学习方法。

Method: FedAgain采用基于信任的联邦学习策略，整合了基准可靠性和模型差异的双重信任机制。该机制动态加权客户端的贡献，在聚合过程中减轻噪声或对抗性更新的影响。框架支持在多机构间训练协作模型，同时保护数据隐私并在真实条件下促进稳定收敛。

Result: 在五个数据集上的广泛实验表明，FedAgain在非独立同分布数据和损坏客户端场景下，始终优于标准联邦学习基线。这些数据集包括两个经典基准（MNIST和CIFAR-10）、两个私有多机构肾结石数据集和一个公共数据集（MyStone）。
...
### [8] [Gastric-X: A Multimodal Multi-Phase Benchmark Dataset for Advancing Vision-Language Models in Gastric Cancer Analysis](https://arxiv.org/abs/2603.19516)
*Sheng Lu,Hao Chen,Rui Yin,Juyan Ba,Yu Zhang,Yuanzhe Li*

Main category: cs.CV

TL;DR: Gastric-X是一个大规模胃癌多模态基准数据集，包含1.7K病例，集成了CT扫描、内镜图像、生化指标、诊断笔记和肿瘤标注，用于评估视觉语言模型在临床任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学诊断中的应用受到缺乏全面结构化临床数据集的限制，特别是胃癌领域。需要创建反映真实临床工作流程的多模态基准来推动医学VLM的发展。

Method: 构建Gastric-X数据集，包含1.7K病例，每个病例包括静息和动态CT扫描、内镜图像、结构化生化指标、专家诊断笔记和肿瘤区域边界框标注。基于此数据集系统评估VLM在五个核心任务上的表现：视觉问答、报告生成、跨模态检索、疾病分类和病灶定位。

Result: 通过Gastric-X基准，能够评估VLM在模拟临床工作流程任务中的表现，并探究VLM是否能够有意义地关联生化信号与空间肿瘤特征及文本报告。

Conclusion: Gastric-X是推动机器智能与医生认知和证据推理过程对齐的重要一步，旨在激发下一代医学VLM的发展，促进VLM在胃癌临床诊断中的应用。
...
### [9] [ReXInTheWild: A Unified Benchmark for Medical Photograph Understanding](https://arxiv.org/abs/2603.19517)
...
TL;DR: ReXInTheWild是一个包含955个临床验证多选题的基准测试，涵盖7个临床主题的484张生物医学文献照片，用于评估视觉语言模型在医学图像理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 日常医疗照片已在远程医疗中广泛应用，但缺乏评估视觉语言模型解释其医学内容的综合基准。这些图像需要细粒度的自然图像理解和领域特定的医学推理，这对通用和专业模型都是挑战。

Method: 创建了ReXInTheWild基准测试，包含955个临床验证的多选题，涵盖7个临床主题，基于484张来自生物医学文献的照片。对领先的多模态大语言模型进行系统性评估和错误分析。

Result: 主要模型表现差异显著：Gemini-3准确率78%，Claude Opus 4.5为72%，GPT-5为68%，而医学专业模型MedGemma仅37%。错误分析揭示了从低级几何错误到高级推理失败的四种常见错误类型。

Conclusion: ReXInTheWild提供了一个具有挑战性的临床基准，位于自然图像理解和医学推理的交汇点。数据集已在HuggingFace上公开，有助于推动医疗AI的发展。
...
### [10] [Recognising BSL Fingerspelling in Continuous Signing Sequences](https://arxiv.org/abs/2603.19523)
*Alyssa Chan,Taein Kwon,Andrew Zisserman*

Main category: cs.CV

TL;DR: 本文提出了一个大规模英国手语拼写数据集FS23K和一种考虑双手交互和口型线索的拼写识别模型，将字符错误率降低了一半。


<details>
  <summary>Details</summary>
Motivation: 英国手语拼写对于拼写专有名词、技术术语等至关重要，但现有数据集规模小且标注不准确，同时拼写识别面临速度快、字母省略等挑战。

Method: 采用迭代标注框架构建大规模BSL拼写数据集FS23K，并提出显式考虑双手交互和口型线索的拼写识别模型。

Result: 通过精炼标注，该方法将字符错误率相比先前最佳结果降低了一半，证明了方法的有效性。
...
### [11] [SurfaceXR: Fusing Smartwatch IMUs and Egocentric Hand Pose for Seamless Surface Interactions](https://arxiv.org/abs/2603.19529)
*Vasco Xu,Brian Chen,Eric J. Gonzalez,Andrea Colaço,Henry Hoffmann,Mar Gonzalez-Franco,Karan Ahuja*

Main category: cs.CV

TL;DR: SurfaceXR：结合头显手部追踪与智能手表IMU数据的传感器融合方法，实现在日常表面上的稳健XR交互
...
### [12] [dinov3.seg: Open-Vocabulary Semantic Segmentation with DINOv3](https://arxiv.org/abs/2603.19531)
*Saikat Dutta,Biplab Banerjee,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: DINOv3.seg扩展DINOv3.txt用于开放词汇语义分割，通过任务特定架构、全局-局部文本嵌入对齐、早期视觉表示精炼和后期图像-文本相关特征精炼，以及高分辨率局部-全局推理策略，在五个基准测试中优于现有方法。
...
Method: 1. 为DINOv3.txt设计任务特定架构；2. 联合利用与全局[CLS]标记对齐的文本嵌入和ViT编码器的局部补丁级视觉特征；3. 在图像-文本交互前进行早期视觉表示精炼，然后在图像-文本相关特征上进行后期精炼；4. 提出基于滑动窗口聚合的高分辨率局部-全局推理策略。

Result: 在五个广泛采用的开放词汇语义分割基准测试中进行了广泛实验，结果证明了该方法的有效性和鲁棒性，一致优于当前最先进的方法。

Conclusion: DINOv3.seg通过系统性的架构设计、多级精炼策略和高分辨率推理方法，为开放词汇语义分割提供了更准确和鲁棒的性能，特别是在复杂、杂乱场景中表现出色。
...
TL;DR: 提出一种轻量级、社会感知的行人意图预测架构，融合注意力、位置、情境和交互四个行为流，使用紧凑Transformer和不确定性量化机制，在PSI基准上优于现有视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在城市场景中需要准确预测行人意图以确保安全导航。现有方法要么过于复杂（如视觉语言模型），要么缺乏不确定性量化，难以在实际资源受限平台上部署。

Method: 提出轻量级社会感知架构：1) 融合四个行为流（注意力、位置、情境、交互）；2) 使用高速公路编码器、4-token紧凑Transformer和全局自注意力池化；3) 集成两个互补的不确定性量化头：变分瓶颈（捕获认知不确定性）和马氏距离检测器（识别分布偏移）。

Result: 在PSI 1.0基准上：F1=0.9，AUC-ROC=0.94，MCC=0.78，优于现有视觉语言模型。在PSI 2.0上建立首个基线：F1=0.78，AUC-ROC=0.79。基于马氏距离的选择性预测在80%覆盖率下提升测试精度0.4个百分点。注意力热图显示模型在模糊情况下能动态调整跨流关注。

Conclusion: 该方法提供了一种轻量级、不确定性感知的行人意图预测方案，具有模态无关性、易于与视觉语言管道集成、适合资源受限平台部署的特点，为风险感知的意图预测提供了实用解决方案。
...
TL;DR: MoCA3D：一种无需相机内参的单目、类别无关3D模型，直接预测投影的3D边界框角点和深度，在未知相机参数的下游应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D目标理解方法通常需要已知相机内参才能获得图像平面几何信息（如投影的3D框角点），这限制了在野外场景和未知相机参数的下游应用。

Method: MoCA3D将像素空间定位和深度分配建模为密集预测问题，通过角点热力图和深度图直接预测投影的3D边界框角点及其深度，无需推理时的相机内参。

Result: MoCA3D在图像平面几何保真度上取得SOTA性能，图像平面角点PAG指标提升22.8%，同时3D IoU保持可比，参数量减少最多57倍。

Conclusion: MoCA3D解决了未知相机内参下的3D目标理解问题，为之前不切实际的下游任务提供了实用解决方案，超越了标准基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用需要快速场景重建以高效利用大规模驾驶数据集，避免耗时的逐场景优化，实现闭环仿真和其他下游任务

Method: 基于VGGT的交替注意力机制，提出简单有效的时间掩码注意力模块捕捉动态运动信息；使用3D高斯泼溅统一表示静态和动态内容；通过跨帧渲染优化时空一致性；可推断像素级速度

Result: 在Waymo Open Dataset上训练评估，在新视角合成和深度估计方面优于现有方法；在CARLA等数据集上的零样本推理验证了泛化能力

Conclusion: StreetForward实现了无需位姿和跟踪器的动态街道重建，能够生成高保真新视角，具有良好的泛化性能，适用于自动驾驶应用
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉模型需要在捕获图像级上下文的同时不牺牲局部细节，同时保持计算效率。现有方法在这三者之间存在权衡，作者提出解耦全局推理和局部表示角色的原则来解决这一问题。

Method: 提出ConvNeur双分支架构：1）轻量级神经记忆分支在紧凑的token集上聚合全局上下文；2）局部保持分支提取精细结构。通过学习的门控机制让全局线索调制局部特征而不混淆其目标。

Result: 在标准分类、检测和分割基准测试中，ConvNeur在相似或更低的计算成本下匹配或超越了可比替代方案，并在相似预算下提供了有利的准确率与延迟权衡。

Conclusion: 全局-局部解耦是实现效率的关键，ConvNeur架构通过分离全局推理和局部表示的角色，在保持计算效率的同时实现了优秀的性能表现。
...
TL;DR: 本文提出了一种两阶段加速框架，通过多尺度采样策略和单步去噪模型蒸馏，将基于扩散的图像解码器的推理时间减少了一个数量级，同时保持高质量重建。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的图像解码器虽然能实现高保真重建，但其迭代采样过程引入了显著延迟，不适用于实时或大规模应用。现有扩散模型主要用于内容生成而非忠实重建，因此需要专门针对图像标记化任务进行优化。

Method: 提出两阶段加速框架：1）多尺度采样策略：从粗分辨率开始解码，每个阶段分辨率翻倍，实现O(log n)的理论加速；2）单步去噪模型蒸馏：将每个尺度的扩散解码器蒸馏为单步去噪模型，每个尺度只需一次前向传播。

Result: 该方法将解码时间减少了一个数量级，同时输出质量几乎没有下降。为高效而富有表现力的图像标记器提供了实用路径。

Conclusion: 该加速框架为高效的视觉标记化和下游生成任务奠定了基础，有望推动图像标记化技术的实际应用。
...
TL;DR: CurveStream是一个无需训练的曲率感知分层视觉记忆管理框架，用于解决流式视频理解中视觉token线性爆炸导致的内存溢出和灾难性遗忘问题，通过曲率评分和动态阈值自适应管理记忆，在多个基准测试上取得超过10%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在离线视频理解中取得了显著成功，但在流式视频应用中面临视觉token线性爆炸的问题，这会导致内存溢出错误或灾难性遗忘。现有的视觉保留和内存管理方法通常依赖于均匀采样、低级物理指标或被动的缓存淘汰策略，但这些方法缺乏内在的语义感知能力，可能会破坏上下文连贯性并模糊短暂但关键的语义转换。

Method: 提出CurveStream框架，这是一个无需训练的曲率感知分层视觉记忆管理框架。基于连续特征轨迹中高曲率区域与关键全局语义转换密切相关的观察，通过曲率评分评估实时语义强度，并集成在线K-Sigma动态阈值，在严格的token预算下自适应地将帧路由到清晰和模糊记忆状态中。

Result: 在多样化的时间尺度评估中，这个轻量级框架CurveStream在多个基准测试上取得了超过10%的绝对性能提升：在StreamingBench上提升10.69%，在OVOBench上提升13.58%，建立了流式视频感知的新最先进结果。

Conclusion: CurveStream通过曲率感知的视觉记忆管理有效解决了流式视频理解中的关键挑战，在保持轻量级的同时显著提升了性能，为流式视频感知任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界语义分割严重依赖大规模图像-文本对数据集，但这些数据集通常缺乏细粒度像素级标注，且人工标注成本高昂、耗时巨大。需要一种经济高效的数据生成方法。

Method: 1. 从类别标签开始生成高质量文本描述；2. 使用扩散模型根据文本生成图像；3. 同时生成正样本和对应的负样本（反事实样本）；4. 结合开放词汇检测模型和交互式分割模型从图像中提取对象掩码作为伪分割标签；5. 使用伪掩码监督和辅助反事实对比训练进行对比语言-图像预训练。

Result: 在PASCAL VOC、PASCAL Context和COCO数据集上分别达到62.9%、26.7%和40.2%的性能，实现了最先进水平，证明了生成数据集对开放世界语义分割的有效性。

Conclusion: MagicSeg通过扩散模型驱动的自动数据集生成管道，解决了开放世界语义分割中数据标注成本高的问题，生成的包含正负样本对的数据集显著提升了分割模型的性能，为开放世界视觉理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景生成方法存在局限性：语言驱动检索方法缺乏物体级控制和场景级风格一致性；基于图的方法虽然可控性更高，但难以生成高保真纹理结果，限制了实际应用。

Method: 提出FlowScene模型，包含三个分支分别生成场景布局、物体形状和物体纹理。核心是紧密耦合的整流流模型，在生成过程中交换物体信息，实现跨图协同推理。

Result: 实验表明FlowScene在生成真实性、风格一致性和与人类偏好对齐方面优于语言条件和图条件的基线方法。

Conclusion: FlowScene通过多模态图条件生成，实现了对物体形状、纹理和关系的细粒度控制，同时保证了场景级结构和外观的风格一致性，具有更好的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有协方差矩阵跟踪方法存在两个主要问题：要么忽略流形约束，要么依赖一阶更新导致在快速演化时产生不可避免的相位滞后。需要一种能够同时保持几何结构并实现精确跟踪的在线方法。

Method: 将协方差跟踪问题重新表述为李群上的受迫刚体运动。基于欧拉-庞加莱方程，将观测解释为驱动潜在角速度的扭矩，并通过结构保持的辛积分器进行传播。该方法实现了二阶动力学，无需训练，完全可微分。

Result: 在三个领域验证了鲁棒的跟踪性能：1) 合成椭圆上，角误差比黎曼指数移动平均降低30倍，同时在高速度下保持稳定性；2) SO(3)稳定化任务中，在20%数据丢失情况下，测地误差从29.4°降至9.9°；3) OTB运动模糊序列中，BlurCar2上的IoU从0.55提升至0.74，成功率96%。

Conclusion: K-GMRF作为一种完全可微分的辛模块，为数据受限场景提供了即插即用的几何先验，并可作为现代深度架构中的可解释层，在协方差跟踪方面实现了理论保证的零稳态误差和实际性能的显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法存在权衡：CNN效率高但缺乏全局上下文，Transformer能捕获长距离依赖但计算成本过高。需要一种既能保持高效推理又能捕获全局信息的新架构。

Method: 基于RWKV框架，提出ChangeRWKV架构，包含两个核心创新：1）分层RWKV编码器构建多分辨率特征表示；2）新颖的空间-时间融合模块（STFM），解决跨尺度空间错位问题并提取细粒度时间差异。

Result: 在LEVIR-CD基准测试中达到85.46% IoU和92.16% F1分数，同时相比先前领先方法大幅减少参数和FLOPs，实现了新的高效变化检测范式。

Conclusion: ChangeRWKV展示了一种新的、高效且强大的操作规模变化检测范式，成功结合了Transformer的并行训练优势和RNN的线性推理效率。
...
Method: 创建Physion-Eval基准测试，包含5个最先进模型在自我中心和他者中心视角下生成的视频，包含10,990个专家推理轨迹，涵盖22个细粒度物理类别。每个生成视频都基于真实参考视频，标注了时间定位的故障、结构化失败类别和违反物理行为的自然语言解释。

Result: 在物理关键场景中，83.3%的他者中心视角和93.5%的自我中心视角生成视频至少存在一个人类可识别的物理故障，揭示了当前视频生成模型在物理真实性方面的显著局限性。

Conclusion: Physion-Eval为物理真实性评估设立了新标准，有望指导基于物理的视频生成模型开发，推动视频生成模型更好地遵守真实世界物理定律。
...
TL;DR: FB-CLIP是一个改进的零样本异常检测框架，通过多策略文本表示和前景-背景分离来解决CLIP模型在细粒度异常检测中的局限性


<details>
  <summary>Details</summary>
Motivation: 工业与医疗应用中细粒度异常检测很重要，但标注异常数据稀缺，零样本检测具有挑战性。现有视觉语言模型如CLIP存在前景-背景特征纠缠和文本语义粗糙的问题

Method: 提出FB-CLIP框架：1）文本模态结合End-of-Text特征、全局池化表示和注意力加权token特征；2）视觉模态通过身份、语义、空间维度的多视角软分离及背景抑制；3）语义一致性正则化（SCR）对齐图像特征与正常/异常文本原型

Result: 实验表明FB-CLIP能有效从复杂背景中区分异常，在零样本设置下实现准确的细粒度异常检测和定位

Conclusion: FB-CLIP通过改进的文本表示和前景-背景分离机制，显著提升了零样本细粒度异常检测的性能，解决了现有方法的局限性
...
TL;DR: LoD-Loc v3提出了一种新的广义航空视觉定位方法，通过实例分割数据集和实例轮廓对齐解决了密集城市环境中跨场景泛化能力差和密集建筑场景失败率高的问题。
...
Method: 1. 开发新的合成数据生成管道，创建InsLoD-Loc数据集（目前最大的航空图像实例分割数据集，包含10万张精确实例建筑标注图像）；2. 将定位范式从语义轮廓对齐转变为实例轮廓对齐，显著减少密集场景中的姿态估计模糊性。

Result: LoD-Loc v3在广泛实验中表现出色，在跨场景和密集城市场景中都显著优于现有最先进的基线方法，具有卓越的零样本泛化能力。

Conclusion: LoD-Loc v3通过创新的数据集生成和实例对齐范式，成功解决了密集城市环境中航空视觉定位的关键挑战，实现了优异的跨场景泛化和密集场景定位性能。
...
TL;DR: ParallelVLM：一种无需训练的视频大语言模型推测解码框架，通过并行化设计和无偏验证器引导的剪枝策略，在长视频场景下显著提升解码效率


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在视频理解任务中表现出色，但其自回归解码效率受限于大量视频token。现有的视觉token剪枝方法虽然能部分缓解瓶颈，但仍存在信息损失问题，且解码加速效果有限。特别是在长视频场景下，草稿模型和目标模型之间的相互等待和有限加速比问题尤为突出。

Method: 提出ParallelVLM框架，采用无需训练的草稿-验证推测解码方法。框架包含两个并行化阶段以最大化硬件利用率，并引入无偏验证器引导的剪枝策略，通过消除注意力引导剪枝中的位置偏差，更好地对齐草稿模型和目标模型。

Result: 实验表明，ParallelVLM能够将草稿窗口扩展1.6~1.8倍，同时保持较高的接受长度。在LLaVA-Onevision-72B上加速比达到3.36倍，在Qwen2.5-VL-32B上达到2.42倍，相比传统的自回归解码有显著提升。

Conclusion: ParallelVLM通过创新的并行化设计和无偏剪枝策略，有效解决了长视频场景下视频大语言模型的解码效率瓶颈，为视频理解任务提供了高效且实用的解码加速方案。
...
TL;DR: OrbitNVS将新视角合成重构为轨道视频生成任务，利用预训练视频生成模型，通过相机适配器、法线图分支和像素空间监督，显著提升了单视角输入的合成质量
...
Method: 将NVS重构为轨道视频生成任务，适配预训练视频生成模型，加入相机适配器实现精确相机控制，设计法线图生成分支通过注意力机制指导目标视角合成，应用像素空间监督缓解模糊外观

Result: 在GSO和OmniObject3D基准测试中显著优于先前方法，特别是在具有挑战性的单视角设置下（PSNR分别提升2.9 dB和2.4 dB）

Conclusion: OrbitNVS通过将NVS重新定义为轨道视频生成任务，有效解决了单视角输入下的几何和外观一致性问题，在多个基准测试中取得了最先进的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有真实到仿真转换方法依赖检测、分割、形状重建和姿态估计等多个子模块，这种模块化流程存在效率低下和累积误差问题，因为每个阶段只处理部分或局部优化信息而丢弃全局上下文。

Method: 1. 直接从单对立体图像输入；2. 利用几何约束解决尺度模糊性；3. 引入姿态感知形状表示，消除对每类规范定义的需求，弥合重建与姿态估计任务之间的差距；4. 构建大规模词汇立体数据集LVS6D（包含6300多个物体）。

Result: UniPR在单次前向传播中并行重建场景中所有物体，实现显著效率提升，并保持跨不同物体类型的真实物理比例，在实验中表现出优越性能。

Conclusion: UniPR作为首个端到端的物体级真实到仿真感知与重建框架，通过统一方法解决了模块化流程的局限性，在效率和精度方面都有显著提升，具有实际机器人应用的潜力。
...
TL;DR: IUP-Pose是一个几何驱动的解耦迭代框架，通过隐式密集对齐实现相对位姿估计，解决了现有方法在精度和实时性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有相对位姿回归方法面临关键权衡：特征匹配方法精度高但梯度流被不可微的RANSAC阻断，而ViT回归器可端到端训练但计算成本过高，无法实时部署。核心瓶颈在于旋转和平移估计的耦合以及跨视图特征对齐不足。

Method: 提出IUP-Pose框架：1）轻量级多头双向交叉注意力模块实现无显式匹配监督的跨视图特征对齐；2）解耦的旋转-平移管道：两个共享参数的旋转阶段迭代优化带不确定性的旋转；3）通过旋转单应性H_inf重新对齐特征图后进行平移预测。

Result: 在MegaDepth1500数据集上达到73.3% AUC@20deg，完全端到端可微，70 FPS吞吐量，仅37M参数，在精度和效率之间取得了有利的权衡。

Conclusion: IUP-Pose通过几何驱动的解耦迭代框架和隐式密集对齐，实现了高精度、高效率的相对位姿估计，适合实时边缘部署。
...
TL;DR: 提出了一种用于夜间无人机跟踪的双提示驱动特征编码方法，通过光照提示和视角提示提升特征鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有无人机跟踪的特征编码方法往往忽视光照和视角线索，导致在夜间等挑战性条件下跟踪性能下降

Method: 提出双提示驱动特征编码方法：金字塔光照提示器提取多尺度频率感知光照提示；动态视角提示器通过调制可变形卷积偏移适应视角变化

Result: 实验验证了DPTracker在夜间无人机跟踪中的有效性，消融研究显示各组件均有贡献，真实场景测试证明其鲁棒性和实用性

Conclusion: 该方法通过集成提示条件特征适应和上下文感知提示演化，促进了领域不变的特征编码，提升了夜间无人机跟踪性能
...
TL;DR: UniBioTransfer是一个统一的深度人脸生成框架，能够处理多种任务（如人脸转移、头发转移等），并通过创新的数据构建策略和BioMoE架构解决多任务生成中的数据稀缺和任务冲突问题。


<details>
  <summary>Details</summary>
Motivation: 传统深度人脸生成采用任务驱动范式，每个任务需要专门的模型，这严重限制了模型的泛化能力和可扩展性。需要开发一个能够单次处理多个任务的统一模型，但面临数据稀缺和跨任务冲突的挑战。

Method: 提出UniBioTransfer框架，包括：1）统一数据构建策略，采用基于交换的损坏机制处理空间动态属性（如头发）；2）BioMoE（基于专家混合的模型）和两阶段训练策略，有效解耦任务特定知识。

Result: 实验表明UniBioTransfer在广泛的深度人脸生成任务中表现出色，超越了现有的统一模型和任务特定方法，并能通过少量微调泛化到未见任务（如嘴唇、眼睛、眼镜转移）。

Conclusion: UniBioTransfer是首个能够处理传统深度人脸任务和形状变化转换的统一框架，通过创新的数据构建和模型架构解决了多任务生成中的数据稀缺和任务冲突问题，展示了出色的有效性、泛化性和可扩展性。
...
TL;DR: OmniDiT：基于扩散Transformer的统一虚拟试穿框架，将试穿和试脱任务整合到一个模型中，通过自演进数据管道构建大规模数据集，引入移位窗口注意力降低计算复杂度，在复杂场景下实现最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法面临细节保留不足、复杂场景泛化能力差、流程复杂和推理效率低等问题，需要开发一个统一框架来同时处理试穿和试脱任务。

Method: 1) 建立自演进数据管道构建Omni-TryOn数据集（38万+高质量服装-模特-试穿图像对）；2) 使用token拼接和自适应位置编码整合多参考条件；3) 首次在扩散模型中引入移位窗口注意力实现线性复杂度；4) 采用多时间步预测和对齐损失提升生成保真度。

Result: 在模型无关的虚拟试穿和试脱任务中取得最佳性能，在模型相关的虚拟试穿任务中达到与当前SOTA方法相当的性能，特别是在各种复杂场景下表现优异。

Conclusion: OmniDiT成功解决了现有虚拟试穿方法的局限性，通过统一框架、大规模数据集和高效注意力机制，实现了高质量的虚拟试穿和试脱效果，为相关领域提供了新的解决方案。
...
Method: 提出GravCal馈送模型，输入单张RGB图像和噪声重力先验，输出校正后的重力方向和样本置信度。模型结合两种互补预测：输入先验的残差校正和独立于先验的图像估计，通过学习门控自适应融合两者

Result: 实验显示显著提升：平均角度误差从22.02°（IMU先验）降至14.24°，在先验严重损坏时改进更大。创建包含148K帧的新数据集，包含VIO地面真值和Mahony滤波IMU先验。学习门控与先验质量相关，可作为下游系统的置信信号

Conclusion: GravCal能有效校正单张图像中的噪声重力先验，结合残差校正和图像估计的互补方法，通过学习门控自适应融合，在重力估计任务中取得显著性能提升，并为下游系统提供有用的置信度信号
...
TL;DR: 该论文提出了SAVN-CE框架和MAGNet模型，用于解决连续环境中音频-视觉导航问题，通过多模态Transformer编码空间和语义目标表示，结合历史上下文和自运动线索，实现记忆增强的目标推理。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉导航方法大多依赖预计算的房间脉冲响应进行双耳音频渲染，将智能体限制在离散网格位置，导致空间不连续的观测。需要建立更真实的连续环境设置，其中智能体可以在3D空间中自由移动，感知时间和空间一致的音频-视觉流。

Method: 提出SAVN-CE（连续环境中的语义音频-视觉导航）框架，其中智能体在3D连续空间中自由移动。提出MAGNet模型，基于多模态Transformer，联合编码空间和语义目标表示，并整合历史上下文与自运动线索，实现记忆增强的目标推理。

Result: MAGNet显著优于现有最先进方法，在成功率上实现了高达12.1%的绝对提升。模型对短时声音和长距离导航场景表现出鲁棒性。

Conclusion: SAVN-CE框架和MAGNet模型为连续环境中的音频-视觉导航提供了更真实的解决方案，通过记忆增强的目标推理有效处理目标间歇性静音的情况，在多种挑战性场景下表现出优越性能。
...
TL;DR: JMVR框架通过联合学习EEG和文本作为独立模态，采用多尺度EEG编码和图像增强技术，实现了比现有方法更优的视觉重建性能，特别是在空间结构和色彩保真度方面。


<details>
  <summary>Details</summary>
Motivation: 当前基于EEG的视觉重建方法通常将EEG特征与文本或图像语义表示对齐，这种耦合可能导致EEG中丰富的空间和色彩细节信息被压缩，最终只能实现条件图像生成而非高保真视觉重建。

Method: 提出JMVR框架：1) 将EEG和文本作为独立模态进行联合学习，保留EEG特异性信息；2) 采用多尺度EEG编码策略捕获细粒度和粗粒度特征；3) 使用图像增强技术提升感知细节恢复能力。

Result: 在THINGS-EEG数据集上的实验表明，JMVR在六个基线方法中达到SOTA性能，特别是在建模空间结构和色彩保真度方面表现出优越能力。

Conclusion: JMVR框架通过解耦EEG与文本/图像的对齐依赖，有效保留了EEG中的空间和色彩细节信息，实现了更高保真度的视觉重建，为基于神经信号的视觉重建提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 随着文本到视频扩散模型的快速发展，控制其生成内容成为研究热点。虽然边界框或布局是常用的控制方式，但确保模型遵循这些控制输入仍然是一个开放性问题。现有方法在控制输入遵循度方面存在不足。

Method: 提出通过微调用户提供的边界框来改进生成质量和控制遵循度。方法包括：1）使用平滑掩码使边界框位置可微分；2）设计注意力最大化目标函数来调整边界框位置；3）平衡前景和背景的关注度，使边界框位于模型熟悉的位置。

Result: 实验表明，即使对边界框进行微小调整，生成质量也会有显著差异。通过用户研究验证了方法的有效性，证明微调后的边界框能同时提高生成质量和控制输入遵循度。

Conclusion: 通过优化边界框使其与视频扩散模型的内部注意力图对齐，可以有效改进文本到视频生成的质控控制遵循度。该方法简单有效，代码已开源以促进社区未来研究。
...
TL;DR: DynFlowDrive是一个用于自动驾驶规划的潜在世界模型，采用基于流的动力学建模场景状态在不同驾驶动作下的演变，并提出稳定性感知的多模态轨迹选择策略
...
Method: 采用rectified flow公式学习描述场景状态在不同驾驶动作下变化的流场，实现潜在状态的渐进预测；提出稳定性感知的多模态轨迹选择策略，根据诱导场景转换的稳定性评估候选轨迹

Result: 在nuScenes和NavSim基准测试中，该方法在不同驾驶框架下均取得一致改进，且不引入额外推理开销

Conclusion: DynFlowDrive通过流动力学建模和稳定性感知轨迹选择，提高了自动驾驶规划的可靠性
...
TL;DR: 提出VLADR方法，通过视觉语言属性解耦与增强来解决终身行人重识别中的知识获取与抗遗忘问题


<details>
  <summary>Details</summary>
Motivation: 现有终身行人重识别方法通常从零开始学习或基于视觉分类预训练模型，而视觉语言模型在各种任务中展现出泛化知识。现有方法仅考虑全局感知学习，细粒度属性知识未被充分利用，导致知识获取和抗遗忘能力有限。
...
Conclusion: 通过显式建模普遍共享的人类属性来改善跨域知识迁移，有效利用历史知识增强新知识学习并缓解遗忘，VLADR在终身行人重识别任务中取得了显著改进。
...
TL;DR: 提出自约束先验方法改进3D高斯溅射的表面重建精度，通过TSDF网格融合深度图生成距离场约束，提升深度渲染质量
...
Method: 提出自约束先验方法：1) 从当前3D高斯渲染的深度图融合得到TSDF网格；2) 生成表面周围的带约束距离场；3) 对3D高斯施加特定约束：移除带外高斯、将高斯移近表面、几何感知调整不透明度；4) 定期更新先验并逐步收紧约束带

Result: 在广泛使用的基准测试中优于现有最先进方法，验证了方法的有效性

Conclusion: 自约束先验方法能有效提升3D高斯溅射的表面重建精度，通过动态更新的距离场约束实现更准确的深度渲染
...
<details>
  <summary>Details</summary>
Motivation: 心电图罕见异常检测面临长尾分布、病例数极少和人口统计差异导致的诊断性能不均问题，造成诊断延迟和医疗质量不平等，急需开发通用框架提升敏感性和公平性。

Method: 开发两阶段AI框架：第一阶段通过自监督异常检测预训练，包括重构掩码全局/局部ECG信号、建模信号趋势和预测患者属性；第二阶段使用非对称损失进行多标签ECG分类微调，处理长尾异常，并生成异常评分图进行定位，支持CPU优化部署。

Result: 在超过一百万临床ECG的纵向队列中评估，罕见异常检测AUROC达94.7%，常见-罕见性能差距减少73%，在不同年龄和性别组中保持一致的诊断准确性。

Conclusion: 提出的公平感知AI框架展示了强大的临床实用性、可解释的异常定位和跨多个队列的可扩展性能，有望缓解诊断差异，推动生物医学信号和数字健康领域的公平异常检测。
...
TL;DR: 该研究探讨2D基础图像模型是否具备3D世界建模能力，通过多智能体架构从2D模型生成3D一致的世界场景


<details>
  <summary>Details</summary>
Motivation: 探索2D基础图像模型是否内在地具备3D世界建模能力，利用现有强大的2D生成模型来合成3D一致的世界场景

Method: 提出多智能体架构：VLM导演制定提示指导图像合成，生成器合成新视角图像，VLM验证器从2D图像和3D重建空间评估和筛选生成帧

Result: 实验证明2D模型确实包含对3D世界的理解，该方法能够合成广阔、真实且3D一致的世界场景，支持新视角渲染

Conclusion: 2D基础图像模型具备内在的3D世界建模能力，通过智能体框架可以有效地利用这种能力生成3D一致的世界场景
...
<details>
  <summary>Details</summary>
Motivation: 现有肖像动画方法主要关注整体面部运动，难以将面部表情与头部姿态旋转解耦，缺乏独立编辑面部表情的能力。这在动画和电影产业中限制了表演的重新塑造。

Method: 1. 基于3D可变形人脸模型（3DMM）的特性，改进关键点变换公式，实现更好的解耦和细粒度控制；2. 将输入肖像图像的面部和非面部区域解耦，预训练教师模型分别监督这两个区域，避免生成结果中面部边界的错位。

Result: 实验表明，该方法能生成高质量结果，更忠实于驱动视频，在可控性和效率方面均优于现有方法。

Conclusion: PerformRecast是一种多功能的表情专用视频编辑方法，能够有效重新塑造现有电影和动画中的表演，提供更好的解耦控制和高质量的生成结果。
...
Method: 提出PhysNeXt双输入深度学习框架，包含时空差异建模单元、跨模态交互模块和结构化注意力解码器，联合利用视频帧和STMap表示来增强脉搏信号提取的鲁棒性。

Result: 实验结果表明，PhysNeXt在挑战性条件下实现了更稳定和细粒度的rPPG信号恢复，验证了视频和STMap表示联合建模的有效性。

Conclusion: PhysNeXt通过整合视频和STMap表示的双输入框架，有效提升了远程光电容积描记的性能，特别是在挑战性条件下的信号恢复能力。
...
Result: 实现了在1秒内从稀疏多视角图像重建完整、可重照明的3D资产，在几何、材质精度和光照质量方面获得可泛化结果

Conclusion: 通过将分离的重建任务统一到单次前向传播中，实现了近乎即时的完整可重照明3D资产生成，多视角约束显著改善了材质和光照解纠缠问题
...
TL;DR: UPL是一种用于少样本3D语义分割的概率方法，通过不确定性感知的原型学习，在ScanNet和S3DIS基准测试中实现了最先进的性能。
...
Conclusion: UPL通过将不确定性建模融入原型学习，为少样本3D语义分割提供了鲁棒且可解释的解决方案，在保持高性能的同时增强了模型的可靠性和泛化能力。
...
TL;DR: PCSTracker是首个专门为点云序列设计的端到端一致性场景流估计框架，通过迭代几何运动联合优化、时空点轨迹更新和重叠滑动窗口推理，解决了长期时间一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有点云场景流估计方法通常局限于成对设置，在长序列中难以维持时间一致性，因为几何演化、遮挡出现和误差累积等问题会严重影响估计精度。
...
Conclusion: PCSTracker是首个专门为点云序列设计的端到端一致性场景流估计框架，通过创新的时间一致性建模方法，有效解决了长序列中点云场景流估计的挑战，在精度和实时性方面均表现出色。
...
TL;DR: 提出FREAK基准测试，用于评估多模态大语言模型在细粒度视觉感知中的幻觉问题，通过高质量照片级图像和反常识编辑来全面评估模型幻觉程度。
...
Method: 提出FREAK基准，使用高质量照片级图像和细粒度反常识编辑来评估MLLMs的幻觉现象；创建受控子集间接评估模型感知目标细节信息的能力；系统评估主流思维链提示技术。

Result: 在FREAK上的广泛实验显示，最先进模型在细粒度视觉感知方面存在严重幻觉问题；通过受控子集和思维链提示技术的系统评估，揭示了幻觉模式和模型推理过程的关键见解。

Conclusion: FREAK基准为多模态大语言模型的细粒度幻觉评估提供了全面框架，揭示了当前模型在详细视觉感知方面的严重局限性，为未来研究提供了重要基础。
...
TL;DR: HINGE方法将预训练的单细胞基础模型改造为条件表达生成器，通过SoftAdaLN调制注入视觉上下文，在空间转录组数据上实现更准确的基因表达预测。


<details>
  <summary>Details</summary>
Motivation: 空间转录组技术成本高、通量有限，需要从HE染色组织学图像预测基因表达。现有生成方法大多忽略基因间依赖关系，而单细胞基础模型虽然能捕获这些关系，但缺乏视觉通路且目标不匹配，难以直接应用于组织学条件表达建模。

Method: 提出HINGE方法：1) 引入SoftAdaLN轻量调制模块，将层级的视觉上下文注入预训练单细胞基础模型；2) 采用表达空间掩码扩散目标；3) 使用热启动课程学习确保目标对齐和训练稳定性。

Result: 在三个空间转录组数据集上评估，HINGE在平均Pearson相关性上优于现有最佳基线，产生更准确的空间标记表达模式和更高的成对共表达一致性。
...
TL;DR: FlashCap系统使用闪烁LED实现毫秒级运动捕捉，创建了FlashMotion多模态数据集，并提出ResPose基线方法，显著提升了高时间分辨率姿态估计精度和运动时序准确性。


<details>
  <summary>Details</summary>
Motivation: 精确运动时序在体育分析等领域至关重要，但现有高时间分辨率标记数据集稀缺，专业高速RGB相机成本高、限制多，难以日常应用。

Method: 开发了FlashCap闪烁LED运动捕捉系统，收集了FlashMotion多模态数据集（事件、RGB、LiDAR、IMU），并提出了ResPose基线方法，基于事件和RGB学习残差姿态。

Result: ResPose方法将姿态估计误差降低约40%，达到毫秒级时序精度，FlashMotion数据集经过严格验证显示高质量。

Conclusion: FlashCap系统和FlashMotion数据集为精确运动时序和高时间分辨率姿态估计提供了新工具，ResPose方法展示了显著性能提升，将促进该领域研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 针对需要免训练数据、免训练的对象检测场景，如软件开发的图形界面自动化测试，特别是持续集成测试。传统学习型方法需要训练数据和训练过程，而实际应用中数据变化较小但要求快速适应对象变化。

Method: 使用分割基础模型获取图像片段，然后结合简单的基于特征的分类方法。这种方法在改变搜索对象或其设计时无需重新训练或创建数据集。

Result: 在汽车行业导航地图图标检测和分类任务上评估，该方法取得了与YOLO等学习型对象检测方法几乎相当的结果，且无需训练过程。

Conclusion: 该方法为需要免训练数据、免训练的对象检测场景提供了一种有效的解决方案，特别适用于图形界面自动化测试等应用，能够显著节省时间和成本。
...
TL;DR: TIEdit是一个用于文本引导图像编辑方法评估的基准，包含512张源图像和8种编辑任务，生成5120张编辑图像，并通过专家评分获得15360个平均意见分数。同时提出了EditProbe评估器，通过探测多模态大语言模型的中间层表示来更好地评估编辑质量。


<details>
  <summary>Details</summary>
Motivation: 当前文本引导图像编辑方法的评估存在挑战，现有评估基准规模有限且与人类感知判断相关性较弱。需要建立一个更系统、可靠的评估基准和评估方法。

Method: 1) 构建TIEdit基准：包含512张源图像，8种代表性编辑任务，10种最先进的TIE模型生成5120张编辑图像；2) 专家评分：20名专家进行307200个原始主观评分，汇总为15360个平均意见分数；3) 提出EditProbe评估器：通过探测多模态大语言模型的中间层隐藏表示来评估编辑质量。

Result: 实验结果显示，广泛使用的自动评估指标在编辑任务上与人类判断相关性有限，而EditProbe与人类感知判断具有更强的对齐性。TIEdit基准和EditProbe为文本引导图像编辑方法提供了更可靠、感知对齐的评估基础。

Conclusion: TIEdit基准和EditProbe评估器共同为文本引导图像编辑方法提供了系统、可靠的评估框架，能够更好地反映人类感知判断，推动该领域的发展。
...
TL;DR: TATAR提出任务感知思维与不对称奖励的统一框架，通过任务特定的快速-慢速推理、两阶段学习和不对称奖励机制，在图像质量评估和图像美学评估任务上优于现有统一方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用任务无关的通用策略处理IQA和IAA任务，但这存在根本性错配：IQA依赖低层客观感知线索，需要简洁的失真聚焦推理；而IAA需要深思熟虑的语义判断，不适合点式分数回归。论文识别了推理错配和优化错配问题。

Method: TATAR框架包含三个核心组件：1) 快速-慢速任务特定推理构建，为IQA配对简洁感知理由，为IAA配对深思熟虑的美学叙述；2) 两阶段SFT+GRPO学习，先建立任务感知行为先验，再进行奖励驱动的精炼；3) 不对称奖励机制，对IQA应用高斯分数整形，对IAA应用Thurstone式完成排序。

Result: 在八个基准测试上的广泛实验表明，TATAR在域内和跨域设置下均优于先前的统一基线，在两项任务上均表现优异；与任务特定的专门模型保持竞争力；并为美学评估提供更稳定的训练动态。

Conclusion: 研究结果确立了任务条件后训练作为统一感知评分的原理性范式，证明了任务特定推理和奖励机制在统一多模态大语言模型中的重要性。
...
TL;DR: DeSC提出解耦的敏感度-一致性框架，通过两个专门化流分别处理瞬时异常和持续异常，解决了现有弱监督视频异常检测方法中的敏感度-稳定性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法采用统一框架进行联合优化，但存在根本性的敏感度-稳定性权衡问题。检测瞬时异常和持续异常的冲突目标导致预测结果要么过于碎片化，要么过度平滑。
...
Conclusion: DeSC通过解耦敏感度与一致性训练策略，有效解决了弱监督视频异常检测中的敏感度-稳定性权衡问题，在两个基准数据集上均取得了显著性能提升。
...
TL;DR: HOP3D：一种用于广义少样本3D点云分割的统一框架，通过层次正交原型和基于熵的正则化器，解决基类与新类之间的稳定性-可塑性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 广义少样本3D点云分割需要在适应新类别的同时保持基类性能，但由于稳定性-可塑性权衡的固有挑战，适应新类别会干扰共享表示并导致基类遗忘。

Method: HOP3D框架包含两个核心组件：1）层次正交化，在梯度和表示层面解耦基类和新类的学习；2）基于熵的少样本正则化器，利用预测不确定性来优化原型学习并促进平衡预测。

Result: 在ScanNet200和ScanNet++数据集上的大量实验表明，HOP3D在1-shot和5-shot设置下均持续优于现有最先进的基线方法。

Conclusion: HOP3D通过层次正交原型和熵正则化，有效缓解了基类-新类干扰，实现了鲁棒的新类别适应而不降低基类性能，为广义少样本3D点云分割提供了有效的解决方案。
...
TL;DR: SeGroS是一个用于统一多模态模型的微调框架，通过视觉基础映射构建互补监督信号，解决粒度不匹配和冗余监督问题，显著提升生成保真度和跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型的生成训练范式存在固有局限性，特别是粒度不匹配和冗余监督问题，需要新的微调框架来提升模型性能。
...
Conclusion: SeGroS框架有效解决了统一多模态模型中的粒度不匹配和冗余监督问题，为提升多模态生成和理解性能提供了有效的解决方案。
...
TL;DR: 论文提出几何风险控制器，解决生成式OCR中自回归解码偏向语义合理性而非视觉准确性的问题，通过多视图共识机制控制极端错误风险。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型作为生成式OCR引擎存在部署错位问题：自回归解码倾向于语义合理性，而OCR需要视觉基础和几何可验证的输出，导致严重错误如过度生成和未支持的替换，即使基准准确率高也存在部署风险。
...
TL;DR: 该研究评估了视觉基础模型在显微镜图像像素分类和物体分类任务中的表现，发现相比传统手工特征方法有显著改进


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在计算机视觉领域广泛应用，但在生物医学图像的交互式语义分割和物体分类任务中，基于特征的浅层学习方法仍占主导地位。这主要是由于该领域数据多样性、缺乏大规模预训练数据集以及对计算和标注效率的需求。研究旨在探索视觉基础模型是否能改进这些任务

Method: 评估了多种视觉基础模型，包括通用模型（SAM、SAM2、DINOv3）和领域特定模型（μSAM、PathoSAM），结合浅层学习和注意力探测方法，在五个多样化且具有挑战性的数据集上进行测试

Result: 研究结果表明，视觉基础模型相比手工特征方法取得了持续改进，为实际应用提供了明确的改进路径

Conclusion: 该研究为显微镜领域的视觉基础模型建立了基准，并为该领域的未来发展提供了指导，证明了视觉基础模型在像素分类和物体分类任务中的潜力
...
TL;DR: 该论文提出了一种框架来识别和测量在线地图构建模型的失败模式，通过解耦记忆输入特征和过拟合已知地图几何形状两种效应，引入基于Fréchet距离的重建统计和失败模式评分，并提出基于最小生成树的训练集稀疏化策略来提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的在线地图构建是自动驾驶的关键技术，但这些模型经常无法泛化到陌生环境。现有评估方法难以区分模型失败的具体原因——是记忆输入特征还是过拟合已知地图几何形状。

Method: 1. 提出控制地理邻近性和几何相似性的评估子集；2. 引入基于Fréchet距离的重建统计，无需阈值调整即可捕获每个元素的形状保真度；3. 定义两种失败模式评分：定位过拟合评分（量化地理线索消失时的性能下降）和地图几何过拟合评分（测量场景几何新颖性增加时的性能退化）；4. 提出最小生成树多样性度量和对称覆盖度量来分析数据集偏差；5. 设计基于MST的稀疏化策略来减少冗余并改善平衡性。

Result: 在nuScenes和Argoverse 2数据集上的实验表明：1. 提出的评估方法能更可靠地评估泛化能力；2. 地图几何多样且平衡的训练集能提升性能；3. MST稀疏化策略在减小训练规模的同时改善了平衡性和性能。

Conclusion: 研究结果支持采用失败模式感知的评估协议和以地图几何为中心的数据集设计，以实现可部署的在线地图构建系统。该方法为理解模型失败机制和改善泛化性能提供了系统框架。
...
TL;DR: 提出ATCG模块，通过类比已知类别生成未标记样本的文本概念，结合视觉特征改进广义类别发现任务


<details>
  <summary>Details</summary>
Motivation: 当前GCD方法主要依赖视觉特征，监督学习与发现过程耦合松散，导致在细粒度、相似类别上边界脆弱

Method: 引入ATCG模块，从标记知识类比到新观测，为未标记样本生成文本概念，将视觉特征与文本概念融合，使发现过程变为视觉-文本推理

Result: 在六个基准测试中，ATCG持续提升整体、已知类和未知类性能，在细粒度数据上提升最大

Conclusion: ATCG作为即插即用模块，能够将先验知识转移到新数据，增强类别分离，适用于参数化和聚类式GCD流程
...
TL;DR: HUGE-Bench是一个用于高级无人机视觉-语言-动作任务的基准测试，专注于测试智能体如何理解简洁语言指令并执行复杂、过程导向且安全的飞行轨迹，弥补了现有基准测试的不足。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉-语言导航基准主要关注长而逐步的路线描述和以目标为中心的评价，无法有效诊断真实操作中需要将简洁高级指令转化为安全多阶段行为的能力。

Method: 构建了基于对齐的3D高斯溅射-网格表示的HUGE-Bench基准，包含4个真实世界数字孪生场景、8个高级任务和256万米轨迹，结合了逼真渲染和碰撞检测能力。

Result: 实验表明，当前最先进的视觉-语言-动作模型在高级语义完成和安全执行方面存在显著差距，验证了HUGE-Bench作为诊断测试平台的有效性。

Conclusion: HUGE-Bench为高级无人机自主性提供了一个诊断性测试平台，能够评估智能体理解简洁语言指令、执行复杂过程导向轨迹并保持安全的能力。
...
TL;DR: Fourier Splatting：首个可扩展的辐射场渲染基元，通过傅里叶编码描述符参数化平面surfel，实现单一训练模型在不同细节级别下的渲染
...
Method: 1）使用傅里叶编码描述符参数化平面surfel，创建任意闭合形状的可扩展基元；2）采用直通估计器进行梯度扩展；3）引入HYDRA密度化策略，在MCMC框架内将复杂基元分解为简单组成部分

Result: 在平面基元框架中达到最先进的渲染质量，在标准基准测试中与领先的体积表示方法具有可比性的感知指标，为带宽受限的高保真渲染提供多功能解决方案

Conclusion: Fourier Splatting通过可扩展的傅里叶编码基元，实现了单一模型在不同细节级别下的灵活渲染，为实时高质量新视角合成提供了创新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决野外多人3D运动捕捉中的挑战：严重遮挡、动态交互导致身份关联困难，以及需要保持运动重建的一致性和连续性。
...
Conclusion: RAM提供了一个可泛化的范式，用于野外无标记3D人体运动捕捉，通过综合的跟踪、重建和预测机制，在挑战性场景中实现了鲁棒和准确的结果。
...
TL;DR: Hyper-Connections (HC)作为固定残差连接的替代方案，在五种3D脑肿瘤分割架构中实现了一致的性能提升，特别是在增强肿瘤区域，且对临床主导模态表现出更敏锐的敏感性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索动态超连接在多模态脑肿瘤分割中的效果，替代传统的固定残差连接，以提升分割性能并改善对临床重要模态的敏感性。

Method: 将Hyper-Connections作为即插即用模块集成到五种3D分割架构中（nnU-Net、SwinUNETR、VT-UNet、U-Net、U-Netpp），在BraTS 2021数据集上进行评估，并与固定连接基线进行对比。

Result: 动态HC在所有3D模型上实现了一致改进，最高获得+1.03%的平均Dice增益，参数开销可忽略。增强肿瘤区域改进最显著，且HC模型对临床主导模态（T1ce用于肿瘤核心和增强肿瘤，FLAIR用于全肿瘤）表现出更敏锐的敏感性。

Conclusion: Hyper-Connections是一种简单、高效且广泛适用的多模态特征融合机制，特别适用于医学图像分割任务，其优势在3D体积数据中更为明显。
...
TL;DR: HiPath是一个轻量级病理学视觉语言模型框架，专门用于结构化病理报告生成，在真实世界中文病理数据上表现优异
...
Result: 在三个医院的749K真实世界中文病理病例上训练，达到68.9%严格准确率和74.7%临床可接受准确率，安全率97.3%；跨医院评估显示泛化能力强，严格准确率仅下降3.4个百分点，安全率保持97.1%
...
### [65] [X-World: Controllable Ego-Centric Multi-Camera World Models for Scalable End-to-End Driving](https://arxiv.org/abs/2603.19979)
...
TL;DR: X-World是一个面向自动驾驶评估的动作条件多摄像头生成世界模型，能够根据历史多视角摄像头数据和未来动作序列生成逼真的多摄像头视频流，支持交通参与者和道路元素的控制，为自动驾驶系统提供可扩展、可复现的评估基础。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶评估主要依赖真实道路测试，成本高昂、场景覆盖有限且难以复现。需要能够生成逼真未来观测的世界模拟器，同时保持长期可控性和稳定性。

Method: X-World是一个动作条件多摄像头生成世界模型，核心是多视角潜在视频生成器，通过显式鼓励跨视角几何一致性和时间连贯性来生成高质量视频。支持动作序列输入、交通参与者和道路元素控制，以及外观提示（天气、时间）接口。

Result: X-World实现了高质量多视角视频生成，具有：(1) 跨摄像头强视角一致性，(2) 长期展开的稳定时间动态，(3) 严格动作跟随和高保真场景控制的高可控性。

Conclusion: X-World为自动驾驶系统提供了可扩展、可复现评估的实用基础，能够生成逼真的多摄像头视频模拟，支持多种控制方式，解决了当前评估方法的局限性。
...
TL;DR: 本文研究CLIP模型中的模态内不对齐问题，发现投影器中的跨模态算子负责模态间对齐，而模态内算子仅进行归一化但不促进模态内对齐。通过谱分析识别对齐子空间，移除各向异性方向可改善模态内对齐。


<details>
  <summary>Details</summary>
Motivation: 当CLIP的单个模态编码器应用于图像到图像检索等模态内任务时，性能受到模态内不对齐的影响。本文旨在研究CLIP中的模态内不对齐问题，特别关注将预投影嵌入映射到共享嵌入空间的投影器的作用。

Method: 通过分析投影特征的余弦相似度形式及其与对比CLIP损失的交互，识别跨模态算子和模态内算子。使用谱分析识别近似各向同性的对齐子空间和各向异性的模态特定方向。直接从投影器权重获取对齐子空间，移除各向异性方向以改善模态内对齐。

Result: 在模态内检索和分类基准测试中，该无需训练的方法减少了模态内不对齐，显著降低了延迟，并在多个预训练的CLIP类模型中优于现有方法。

Conclusion: CLIP投影器包含负责模态间对齐的跨模态算子和仅进行归一化的模态内算子。通过谱分析识别对齐子空间并移除各向异性方向，可以有效改善模态内对齐，提升模态内任务的性能。
...
TL;DR: MedQ-Engine是一个用于医学图像质量评估的闭环数据引擎，通过迭代评估、聚类发现失败原型、渐进式人工标注和质量保证微调，显著提升多模态大语言模型的性能，仅用1万标注就使8B参数模型超越GPT-4o 13%以上，将与人专家的差距缩小到4.34%。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像质量评估中仍远不及人类专家，特别是在需要提供带有临床推理的描述性评估时。改进面临两大挑战：获取描述性标注的高成本，以及一次性数据收集无法适应模型不断演变的弱点。

Method: 提出MedQ-Engine闭环数据引擎：1）通过数据驱动聚类迭代评估模型以发现失败原型；2）使用这些原型作为检索锚点，在百万级图像池中进行渐进式人工标注；3）通过质量保证的微调实现自我改进循环。采用熵引导路由机制来分流标注以最小化标注成本。

Result: 在五种医学成像模态上的实验表明，MedQ-Engine将8B参数模型的性能提升到超越GPT-4o超过13%，并将与人专家的差距缩小到仅4.34%。仅使用1万标注就实现了超过随机采样4倍以上的样本效率。
...
TL;DR: SIMPLER是一种预微调架构选择方法，通过分析预训练视觉变换器中更深层的表示稳定性，在适应前识别有效模型深度，显著减少推理和部署成本。


<details>
  <summary>Details</summary>
Motivation: 微调地球观测基础模型计算成本高昂，训练时间和内存需求大。现有参数高效方法仅降低训练成本但保留完整推理复杂度，而后处理压缩方法需要在昂贵的完整微调后才能优化推理。

Method: SIMPLER利用预训练视觉变换器中更深层表示的稳定性：在未标记任务数据上计算层间表示相似度，应用自动评分函数选择冗余层，无需梯度、幅度启发式或超参数调优。

Result: 在Prithvi-EO-2上，SIMPLER剪枝高达79%参数同时保留94%基线性能，实现2.1倍训练加速和2.6倍推理加速。方法可推广到TerraMind（多模态EO基础模型）和ImageNet预训练的ViT-MAE。

Conclusion: SIMPLER提供了一种高效的前微调架构选择方法，显著降低地球观测基础模型的推理和部署成本，适用于跨任务、架构和光谱模态的广泛应用。
...
TL;DR: 提出一种自适应贪婪帧选择方法，用于长视频问答，在固定帧预算下联合优化查询相关性和语义代表性，通过轻量级问题类型分类器选择最佳策略。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型应用于长视频问答时，推理常受输入帧数和视觉令牌数量限制。简单稀疏采样可能错过关键帧，而纯相关性驱动选择常陷入近重复帧且牺牲时间上分散证据的覆盖。

Method: 构建1FPS候选池（上限1000帧）并精确时间戳对齐，在SigLIP（问题相关性）和DINOv2（语义相似性）两个互补空间嵌入候选帧，通过贪婪最大化加权和（模块化相关性项+设施位置覆盖项）选择帧。引入四种预设策略和轻量级文本问题类型分类器为每个查询选择最佳策略。

Result: 在MLVU数据集上的实验显示，相比均匀采样和近期强基线，该方法在不同帧预算下均获得一致的准确率提升，在紧预算下改进最大。

Conclusion: 提出的问题自适应贪婪帧选择方法能有效平衡查询相关性和语义代表性，为长视频问答提供高效帧选择方案，在有限计算资源下提升模型性能。
...
TL;DR: CoA框架通过结构化推理格式增强领域对齐，同时保持多模态能力，在手术领域基准测试中表现优于监督微调
...
TL;DR: SegVGGT：首个端到端框架，直接从多视角RGB图像同时完成3D重建和实例分割，通过对象查询与几何特征交互，引入FADA策略解决注意力分散问题。
...
Method: 提出SegVGGT统一端到端框架，通过对象查询与多层级几何特征交互，将实例识别深度集成到视觉几何基础transformer中。为解决全局图像token过多导致的注意力分散问题，提出帧级注意力分布对齐（FADA）策略，在训练中明确引导对象查询关注实例相关帧。

Result: 在ScanNetv2和ScanNet200数据集上达到最先进性能，优于最近的联合模型和基于RGB-D的方法，同时在ScanNet++上展现出强大的泛化能力。

Conclusion: SegVGGT成功实现了从多视角RGB图像直接进行3D重建和实例分割的统一端到端框架，通过深度集成实例识别和几何重建，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 扩散概率模型在图像生成方面取得了巨大成功，但由于其迭代去噪的特性，存在推理延迟高的问题。受去噪轨迹中特征动态演化的启发，需要一种内存高效的优化方法来加速推理

Method: 提出基于时间步的优化框架，学习时间步特定的掩码来动态决定每个推理阶段执行或跳过哪些模块；引入时间步感知的损失缩放机制，在敏感去噪阶段优先保证特征保真度；采用知识引导的掩码修正策略来剪枝冗余的时空依赖关系；支持独立优化每个时间步的掩码，避免全链反向传播的内存开销

Result: 该方法在DDPM、LDM、DiT和PixArt等多种模型上实现了显著的效率提升，在采样速度和生成质量之间取得了优越的平衡

Conclusion: 通过将去噪过程视为一系列优化的计算路径，提出的框架能够以内存高效的方式加速扩散模型的推理，同时保持生成质量，具有架构无关的通用性
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在实现精确的面部属性对齐和确保组内一致性方面存在不足，缺乏明确的建模机制来保证主体间的属性一致性。

Method: 1. 数据方面：设计定制化的收集流程，从独立视频中编排字幕和视觉线索，使用多模态大语言模型推断并分配主体特定的依赖关系；2. 模型方面：引入关系自注意力和关系交叉注意力机制，将位置感知嵌入与精炼的注意力动态相结合，以刻写明确的主体-属性依赖关系。

Result: 在构建的基准测试上进行全面评估，LumosX在细粒度、身份一致性和语义对齐的个性化多主体视频生成方面实现了最先进的性能。

Conclusion: LumosX通过同时改进数据和模型设计，有效解决了多主体视频生成中的面部属性对齐和组内一致性问题，为个性化内容创作提供了更精细的控制能力。
...
TL;DR: 该论文提出了一种新的图像篡改检测基准和评估框架，将篡改检测从基于对象掩码的粗粒度区域标签转变为像素级、语义感知的任务，并引入了包含像素级篡改图和类别监督的新基准。


<details>
  <summary>Details</summary>
Motivation: 现有篡改检测基准主要依赖对象掩码，这与真实的编辑信号严重不匹配：掩码内的许多像素未被修改或仅轻微改动，而掩码外细微但重要的编辑却被视为自然图像。需要更精确的像素级评估框架。

Method: 1) 引入涵盖编辑原语（替换/移除/拼接/修复/属性/着色等）及其篡改对象语义类别的分类法；2) 发布包含像素级篡改图和配对类别监督的新基准；3) 提出训练框架和评估指标，通过定位量化像素级正确性，并通过语义感知分类和自然语言描述衡量篡改意义理解。

Result: 重新评估现有强基线模型发现，仅使用掩码指标会导致显著的过评分和欠评分，并在微编辑和掩码外变化上暴露失败模式。新框架为篡改定位、语义分类和描述建立了严格标准。

Conclusion: 该研究将篡改检测从掩码推进到像素、语义和语言描述层面，通过PIXAR基准和框架为领域建立了更严谨的评估标准，代码和数据已开源。
...
TL;DR: MedSPOT是一个针对临床GUI环境的工作流感知序列化视觉定位基准，专注于医疗软件中多步骤、动态界面状态下的顺序推理能力评估。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在临床软件环境中的视觉定位能力研究不足，现有GUI基准主要关注孤立单步查询，忽略了真实医疗界面中顺序性、工作流驱动的推理需求，无法评估多步骤任务中的错误传播。

Method: 提出MedSPOT基准，包含216个任务驱动视频和597个标注关键帧，每个任务包含2-3个相互依赖的定位步骤。采用严格的序列评估协议，在首次错误预测时终止任务评估，并提出包含边缘偏差、小目标错误、工具栏混淆等类别的综合失败分类法。

Result: 建立了首个针对临床GUI环境的顺序视觉定位基准，能够系统诊断模型在医疗软件设置中的行为，特别是错误传播和工作流推理能力。

Conclusion: MedSPOT通过将评估从孤立定位转向工作流感知的顺序推理，为评估多模态模型在医疗软件环境中的能力建立了现实且安全关键的基准，有助于提升临床软件中的可靠视觉定位。
...
TL;DR: 该研究首次评估了面部表情识别（FER）在自然领域偏移下的测试时适应（TTA）方法，通过跨数据集实验发现TTA能将FER性能提升高达11.34%，其效果取决于分布距离和自然偏移的严重程度。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在自然分布偏移下表现不佳，这是现实世界部署中的常见挑战。测试时适应（TTA）方法可以在没有标记源数据的情况下在推理过程中适应模型，但之前的研究主要关注合成扰动，缺乏对自然领域偏移的系统评估。

Method: 研究首次评估了FER在自然领域偏移下的TTA方法，使用广泛使用的FER数据集进行跨数据集实验，超越了合成扰动，考察了由不同收集协议、标注标准和人口统计特征引起的真实世界偏移。

Result: 结果显示TTA可以将FER在自然偏移下的性能提升高达11.34%。熵最小化方法（如TENT和SAR）在目标分布干净时表现最佳；原型调整方法（如T3A）在分布距离较大时表现更好；特征对齐方法（如SHOT）在目标分布比源分布更嘈杂时带来最大增益。

Conclusion: 跨数据集分析表明，TTA的有效性受分布距离和跨领域自然偏移严重程度的制约。不同TTA方法在不同场景下各有优势，需要根据具体分布特征选择合适的方法。
...
TL;DR: NEC-Diff：基于扩散模型的RAW图像-事件混合成像框架，用于极端低光条件下的高质量图像重建，通过物理驱动的双模态去噪和自适应特征融合实现精细场景结构恢复。


<details>
  <summary>Details</summary>
Motivation: 极端低光条件下成像质量严重下降，光子稀缺导致严重噪声和纹理丢失。现有方法主要关注从事件中恢复纹理，但忽视了图像噪声和事件本身的内在噪声，限制了光子匮乏条件下的准确像素重建。

Method: 提出NEC-Diff框架：1）结合RAW图像的线性光响应特性和事件的亮度变化特性，建立物理驱动的约束进行鲁棒的双模态去噪；2）基于去噪结果动态估计两种模态的信噪比，指导自适应特征融合，将可靠线索注入扩散过程进行高保真视觉重建。

Result: 构建了REAL数据集（47,800个像素对齐的低光RAW图像、事件和高质量参考图像，照度0.001-0.8 lux）。大量实验证明NEC-Diff在极端黑暗条件下的优越性。

Conclusion: NEC-Diff通过物理驱动的双模态去噪和自适应特征融合，能够从重度噪声信号中提取可靠信息重建精细场景结构，在极端低光条件下实现了高质量的图像重建。
...
TL;DR: 提出FRAM方法解决现有扩散模型在妆容迁移中的两个主要限制：基础模型难以捕捉妆容风格，以及缺乏面部区域感知的妆容特征控制


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的妆容迁移方法主要存在两个问题：1）预训练的基础模型（如CLIP）难以有效捕捉妆容风格；2）将参考图像的妆容特征整体注入扩散去噪模型，忽略了面部区域感知的妆容特征（如眼睛、嘴唇等），限制了区域特定的妆容迁移控制能力

Method: 提出两阶段FRAM方法：1）妆容CLIP微调：使用GPT-o3和文本驱动图像编辑模型合成带标注的妆容风格数据，通过自监督和图像-文本对比学习训练妆容CLIP编码器；2）身份和面部区域感知的妆容注入：使用可学习token查询妆容CLIP编码器提取面部区域感知的妆容特征，通过注意力损失实现区域控制，同时使用ControlNet Union编码源图像及其3D网格进行身份注入

Result: 实验结果验证了该方法在区域可控性和妆容迁移性能方面的优越性

Conclusion: FRAM方法通过专门的妆容CLIP微调和面部区域感知的妆容特征注入，有效解决了现有扩散模型在妆容迁移中的局限性，实现了更好的区域控制和妆容迁移效果
...
TL;DR: 提出CFCML框架，通过粗到细两阶段学习逐步缩小医学图像与表格数据之间的模态鸿沟，提升跨模态诊断性能。


<details>
  <summary>Details</summary>
Motivation: 临床实践中医学图像和表格数据对疾病诊断都很重要，但存在显著的模态鸿沟阻碍跨模态诊断准确性提升。现有方法主要关注高层编码器输出关系，忽略了图像局部信息，且缺乏任务相关信息的提取。

Method: 提出粗到细跨模态学习框架：粗阶段探索多粒度图像特征与表格信息关系，初步缩小模态鸿沟；细阶段生成包含类别信息的单模态和跨模态原型，建立基于层次锚点的关系挖掘策略，通过对比学习增强类间差异、减小类内差异。

Result: 在MEN和Derm7pt数据集上，AUC指标分别提升1.53%和0.91%，优于现有最优方法。

Conclusion: CFCML框架通过渐进式模态鸿沟缩减和层次化关系挖掘，有效提升了跨模态诊断性能，为医学图像与表格数据的融合分析提供了新思路。
...
TL;DR: 该研究提出了一个用于3D超声重建的质量保证框架和开源平台，通过定制体模和标准化流程评估不同跟踪技术的重建精度，机器人3D超声达到接近换能器空间分辨率极限的性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D超声研究缺乏对体积精度和可重复性的全面评估，特别是在自由手或机器人采集的跟踪3D超声重建方面，需要建立稳健的质量保证框架来支持临床转化。

Method: 开发了一个包含不同对称性几何包含物的定制体模，用于评估光学、电磁和机器人运动学跟踪；建立标准化流程进行实时分割和3D重建，然后与真实几何进行自动配准和比较。

Result: 机器人3D超声重建性能达到最先进水平（DSC-3D = 0.94 ± 0.01，HD95 = 1.17 ± 0.12），接近换能器施加的空间分辨率极限；实时分割和重建性能优异（DSC = 0.97，FPS = 46）。

Conclusion: 该研究建立了一个灵活的实验平台和可重复的验证方法，支持跨平台比较和改进报告实践，促进3D超声在诊断和图像引导治疗应用中的安全有效临床转化。
...
TL;DR: 提出了一种偏好引导的去偏框架，用于无参考增强图像质量评估，通过学习增强偏好嵌入空间并去除增强引起的干扰成分，提高模型对算法不变感知质量线索的专注度。


<details>
  <summary>Details</summary>
Motivation: 当前的无参考增强图像质量评估模型往往过度拟合特定增强算法的模式，而不是评估真正的感知质量，导致泛化能力差。

Method: 采用两阶段训练策略：1) 使用监督对比学习学习连续的增强偏好嵌入空间；2) 估计原始质量表示中包含的增强引起的干扰成分，并在质量回归前将其去除。

Result: 在公开的EIQA基准测试上，该方法有效缓解了算法引起的表示偏差，相比现有方法实现了更好的鲁棒性和跨算法泛化能力。

Conclusion: 提出的偏好引导去偏框架能够引导模型专注于算法不变的感知质量线索，而不是增强特定的视觉指纹，从而提高了无参考增强图像质量评估的泛化性能。
...
### [82] [Generalizable NGP-SR: Generalizable Neural Radiance Fields Super-Resolution via Neural Graph Primitives](https://arxiv.org/abs/2603.20128)
*Wanqi Yuan,Omkar Sharad Mayekar,Connor Pennington,Nianyi Li*

Main category: cs.CV

TL;DR: NGP-SR是一个可泛化的3D感知超分辨率框架，直接从低分辨率图像重建高分辨率辐射场，无需外部高分辨率参考或后处理上采样，实现视图一致的高分辨率新视图合成。


<details>
  <summary>Details</summary>
Motivation: 传统NeRF在高分辨率渲染时计算成本高，而简单的2D超分辨率方法会破坏多视图一致性，需要一种既能保持3D一致性又能高效生成高分辨率新视图的解决方案。

Method: 基于神经图形基元(NGP)，NGP-SR将辐射预测条件化于3D坐标和学习的局部纹理标记，直接在辐射场中恢复高频细节，无需外部高分辨率参考或后处理2D上采样。

Result: 在多个数据集上的实验表明，NGP-SR在重建质量和运行效率上都优于先前的NeRF超分辨率方法，为可扩展的高分辨率新视图合成提供了实用解决方案。

Conclusion: NGP-SR是一个可泛化的3D感知超分辨率框架，能够直接从低分辨率图像重建高分辨率辐射场，实现视图一致的高分辨率新视图合成，且无需逐场景优化，具有实用性和可扩展性。
...
### [83] [Synergistic Perception and Generative Recomposition: A Multi-Agent Orchestration for Expert-Level Building Inspection](https://arxiv.org/abs/2603.20143)
*Hui Zhong,Yichun Gao,Luyan Liu,Xusen Guo,Zhaonian Kuang,Qiming Zhang,Xinhu Zheng*

Main category: cs.CV

TL;DR: FacadeFixer：一个多智能体框架，通过协作推理解决建筑立面缺陷检测中的几何多变、对比度低和复合缺陷等挑战，利用生成式智能体合成高质量增强数据来缓解标注稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 建筑立面缺陷检测面临几何多变、背景复杂、对比度低以及复合缺陷（如裂缝与剥落共存）等挑战，导致严重的像素不平衡和特征模糊。现有检测和分割模型因高质量像素级标注稀缺而泛化能力受限。

Method: 提出FacadeFixer统一多智能体框架，将缺陷感知视为协作推理任务而非孤立识别。框架协调检测和分割智能体处理多类型缺陷干扰，结合生成智能体进行语义重组，将复杂缺陷从嘈杂背景中解耦并合成到多样清洁纹理上，生成具有精确专家级掩码的高保真增强数据。

Result: 实验表明FacadeFixer显著优于现有最先进基线方法，特别擅长捕捉像素级结构异常，并证明生成式合成是解决基础设施检测中数据稀缺问题的有效方案。同时发布了涵盖六种主要立面类别的多任务数据集。

Conclusion: FacadeFixer通过多智能体协作推理和生成式数据增强，有效解决了建筑立面缺陷检测中的关键挑战，为基础设施检测中的数据稀缺问题提供了稳健解决方案。
...
### [84] [EgoForge: Goal-Directed Egocentric World Simulator](https://arxiv.org/abs/2603.20169)
...
TL;DR: EgoForge：一种从单张第一人称图像、高层指令和可选外部视角生成连贯第一人称视频的自我中心目标导向世界模拟器，通过轨迹级奖励引导的扩散采样优化目标完成度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在自我中心视频生成方面存在局限：要么专注于手部中心的教学合成但场景演化有限，要么进行静态视角转换而不建模动作动态，或者依赖密集监督（如相机轨迹、长视频前缀、多相机同步捕捉等）。需要一种能从最小静态输入生成连贯第一人称视频的方法。

Method: 提出EgoForge自我中心目标导向世界模拟器，从单张自我中心图像、高层指令和可选辅助外部视角生成视频。引入VideoDiffusionNFT方法，这是一种轨迹级奖励引导的细化技术，在扩散采样过程中优化目标完成度、时间因果关系、场景一致性和感知保真度。

Result: 实验表明EgoForge在语义对齐、几何稳定性和运动保真度方面相比强基线方法取得了一致的提升，在现实世界智能眼镜实验中表现出稳健性能。

Conclusion: EgoForge能够从最小静态输入生成连贯的第一人称视频，通过轨迹级奖励引导的扩散采样有效解决了自我中心视频生成中的意图对齐和时间一致性问题，在多个评估指标上优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频驱动人体反应生成方法未能有效利用视频输入来引导反应合成，导致生成的反应动作与视频内容不匹配。研究发现这一限制源于视觉观察与反应类型之间存在严重的关系扭曲。

Method: 提出MuSteerNet框架，包含两个核心机制：1）原型反馈引导机制，通过门控增量修正调制器和关系边界约束来精炼视觉观察；2）双耦合反应精炼，充分利用修正后的视觉线索进一步引导生成反应动作的优化。

Result: 大量实验和消融研究验证了方法的有效性，MuSteerNet在生成与视频内容匹配的3D人体反应方面取得了有竞争力的性能。

Conclusion: MuSteerNet通过观察-反应相互引导的简单而有效框架，成功解决了视频驱动人体反应生成中的关系扭曲问题，显著提升了反应质量，为构建类人交互AI系统提供了重要技术支持。
...
TL;DR: 该论文提出使用无需训练的方法提高分割扩散模型的采样效率，通过粒子引导、SPELL和聚类技术增强预测多样性，在医疗、城市景观和野火数据集上验证效果。


<details>
  <summary>Details</summary>
Motivation: 在不确定环境（如野火蔓延、医疗诊断、自动驾驶）中预测未来状态需要模型考虑多种可能结果。虽然扩散模型能有效学习多模态分布，但朴素采样计算效率低下，可能需要数百个样本才能找到低概率但操作相关的模式。

Method: 评估多种无需训练的采样方法以促进多样化预测：1）将粒子引导和SPELL技术从自然图像生成适配到离散分割任务；2）提出简单的基于聚类的技术；3）在LIDC医疗数据集、修改版Cityscapes数据集和新建的MMFire野火数据集上验证。

Result: 相比朴素采样，这些方法在MMFire数据集上使HM IoU*指标提升达7.5%，在Cityscapes数据集上提升达16.4%，表明无需训练的方法能以极小的图像质量和运行时成本有效提高分割扩散模型的样本多样性。

Conclusion: 无需训练的采样方法可以显著提高分割扩散模型的采样效率，在保持图像质量和运行时间的同时增强预测多样性，为不确定环境中的多模态预测提供了实用解决方案。
...
TL;DR: 提出了一种基于推理的零样本视频检索方法，通过大模型推断编辑的因果和时间后果，无需任务特定微调即可实现更好的组合视频检索效果。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索方法假设修改文本完全指定了视觉变化，忽略了编辑后产生的后续效应和隐含后果（如运动、状态转换、视角或持续时间线索），而这些效应对于成功检索至关重要。

Method: 采用推理优先的零样本方法，利用大型多模态模型：(1) 推断编辑所隐含的因果和时间后果；(2) 将推理得到的查询与候选视频对齐，无需任务特定微调。同时提出了CoVR-Reason基准测试，包含结构化内部推理轨迹和具有挑战性的干扰项。

Result: 实验表明，该零样本方法在召回率K上优于强检索基线，特别是在隐含效应子集上表现优异。自动和人工分析证实检索结果具有更高的步骤一致性和效应事实性。

Conclusion: 将推理融入通用多模态模型能够通过明确考虑因果和时间后续效应实现有效的组合视频检索，减少对任务特定监督的依赖，提高对挑战性隐含效应情况的泛化能力，并增强检索结果的可解释性，为可解释视频搜索提供了可扩展的原则性框架。
...
TL;DR: 提出一种确定性模式提议模型，直接生成固定数量的分割提议掩码，避免传统生成模型需要大量采样和聚类的问题，显著减少推理时间并提高真实覆盖率。


<details>
  <summary>Details</summary>
Motivation: 许多分割任务（如医学图像分割或未来状态预测）本质上是模糊的，存在多个同样正确的预测。现有方法通常依赖生成模型捕捉这种不确定性，但识别分布模式需要大量采样和事后聚类，计算成本高。

Method: 提出模式提议模型，这是一个确定性框架，在单次前向传播中直接生成固定大小的提议掩码集。为处理多余提议，将目标检测中的置信度机制适配到高维分割掩码空间。通过分解预训练流模型的速度场，可以高效估计提议的先验模式概率。

Result: 该方法显著减少了推理时间，同时比现有生成模型实现了更高的真实覆盖率。模型可以在不知道完整结果分布的情况下训练，适用于真实世界数据集。

Conclusion: 模式提议模型为模糊分割任务提供了一种高效、确定性的解决方案，避免了传统生成方法的高计算成本，同时保持了预测多样性和覆盖率。
...
### [89] [MME-CoF-Pro: Evaluating Reasoning Coherence in Video Generative Models with Text and Visual Hints](https://arxiv.org/abs/2603.20194)
...
TL;DR: 论文提出了MME-CoF-Pro视频推理基准，用于评估视频生成模型的推理连贯性，包含303个样本和16个类别，通过三种提示设置评估7个模型，发现视频生成模型推理连贯性较弱且与生成质量脱节。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型展现出新兴的推理能力，但缺乏评估其跨帧因果一致性的标准。现有文献缺少对推理连贯性的系统评估，需要建立专门的基准来确保视频生成模型的可靠部署。

Method: 提出MME-CoF-Pro基准，包含303个样本覆盖16个类别（从视觉逻辑到科学推理）。引入推理分数评估过程级必要中间推理步骤，设计三种评估设置：无提示、文本提示和视觉提示，以探究推理提示引导的底层机制。

Result: 评估7个开源和闭源视频模型发现：(1)视频生成模型推理连贯性较弱，与生成质量脱节；(2)文本提示提高表面正确性但常导致不一致和幻觉推理；(3)视觉提示对结构化感知任务有益但对细粒度感知效果有限。

Conclusion: 视频生成模型的推理连贯性需要专门评估，MME-CoF-Pro基准为此提供了系统框架。研究发现当前模型在保持跨帧因果一致性方面存在显著不足，提示策略对推理质量有复杂影响，为未来模型改进提供了方向。
...
### [90] [When Prompt Optimization Becomes Jailbreaking: Adaptive Red-Teaming of Large Language Models](https://arxiv.org/abs/2603.19247)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地集成到高风险应用中，强大的安全保证成为实际和商业关注的核心。现有的安全评估主要依赖固定的有害提示集合，隐含假设非自适应对手，从而忽视了现实中输入被迭代优化以规避安全防护的攻击场景。

Method: 使用DSPy框架，将三种原本设计用于良性任务性能提升的黑盒提示优化器应用于从HarmfulQA和JailbreakBench获取的提示，明确优化朝向由独立评估模型（GPT-5.1）提供的0到1范围内的连续危险分数。

Result: 结果显示安全防护措施的有效性大幅降低，特别是对于开源小语言模型影响尤为显著。例如，Qwen 3 8B的平均危险分数从基线设置的0.09增加到优化后的0.79。

Conclusion: 这些发现表明静态基准可能低估了残余风险，自动化、自适应的红队测试是稳健安全评估的必要组成部分。
...
TL;DR: 医疗问答系统中用户查询存在大量拼写错误（61.5%的查询至少包含一个错误），通过拼写纠正作为检索预处理步骤可显著提升检索效果，其中查询端纠正比仅纠正语料库更有效。


<details>
  <summary>Details</summary>
Motivation: 医疗问答系统面临用户查询拼写错误率远高于专业文档的挑战，需要研究拼写纠正作为检索预处理步骤的实际效果，以提升医疗信息检索质量。

Method: 使用TREC 2017 LiveQA Medical track和HealthSearchQA两个公开数据集进行错误普查，评估四种纠正方法（保守编辑距离、标准编辑距离、上下文感知候选排序、SymSpell），在三种实验条件下（未纠正查询对未纠正语料、未纠正查询对纠正语料、完全纠正查询对纠正语料）使用BM25和TF-IDF余弦检索进行评估。

Result: 61.5%的真实医疗查询至少包含一个拼写错误，词元级错误率为11.0%。查询纠正显著提升检索效果：编辑距离和上下文感知纠正使MRR提升9.2%，NDCG@10提升8.3%。仅纠正语料库而不纠正查询效果有限（MRR仅提升0.5%）。

Conclusion: 医疗问答系统中拼写纠正作为检索预处理步骤至关重要，特别是查询端纠正能显著提升检索性能，为实践者提供了基于证据的建议。
...
TL;DR: StreamBench：一个用于评估语言模型在流媒体环境下处理多并发事件能力的基准测试，包含主题聚类、时序问答和摘要三个任务，发现结构化线索能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估主要关注单一复杂事件或为每个查询提供精心策划的输入，缺乏对模型在多个并发事件混合在同一文档流中冲突情况下的评估能力。

Method: 构建StreamBench基准，基于2016年和2025年主要新闻故事，包含605个事件和15,354个文档，设计三个任务：主题聚类、时序问答和摘要。通过比较有无结构化线索（按事件组织关键事实）来诊断模型失败原因。

Result: 结构化线索显著提升模型性能：主题聚类任务提升高达4.37%，时序问答任务提升高达9.63%，帮助模型定位相关信息并区分不同事件。时序推理仍是当前LLMs的固有挑战。

Conclusion: 结构化线索是处理大规模文档流的有前景方向，虽然时序推理仍是挑战，但结构化线索在多个任务中带来的持续性能提升显示了其价值。
...
TL;DR: 针对法律领域长文档处理问题，提出元数据增强混合RAG和DPO优化方法，提升法律语言模型的准确性和安全性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在短文本上表现良好，但在处理长法律文档时会产生幻觉（如错误条款或判例），这在要求精确性的法律领域会损害可靠性和信任。虽然检索增强生成（RAG）有帮助，但在法律场景下仍有限制，特别是在需要数据隐私的小型本地部署模型中。

Method: 提出两种方法：1) 元数据增强混合RAG，通过改进文档级检索来解决法律语料库中的词汇冗余问题；2) 应用直接偏好优化（DPO），在上下文不足时强制模型安全拒绝回答，避免解码错误。

Result: 这些方法共同提升了法律语言模型的接地性、可靠性和安全性，解决了RAG在法律场景中的局限性。

Conclusion: 通过结合元数据增强的混合RAG和DPO优化，可以有效解决法律领域长文档处理中的幻觉问题，提高模型输出的准确性和可靠性，特别是在需要数据隐私保护的本地部署场景中。
...
TL;DR: GeoChallenge是一个包含9万个自动生成的多选几何证明问题的数据集，用于评估大语言模型在文本和图表对齐的多步推理能力，揭示了LLM与人类在几何推理上的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有几何基准测试在规模上有限，且很少提供视觉基础的多选题，难以可靠评估复杂推理能力。需要创建大规模、具有视觉基础的多选几何证明问题数据集来评估大语言模型的符号推理能力。

Method: 引入GeoChallenge数据集，包含9万个自动生成的多选几何证明问题，每个问题都需要在文本描述和图表对齐的基础上进行多步推理。数据集提供细粒度复杂度评级和形式语言标注，支持受控评估。

Result: 多个先进LLM的实验显示，模型与人类之间存在明显性能差距：最佳模型GPT-5-nano的精确匹配率为75.89%，而人类为94.74%。分析揭示了LLM的三种常见失败模式：多选题设置下的精确匹配失败、视觉依赖弱、以及推理过度发散而不收敛。

Conclusion: GeoChallenge为评估LLM的几何推理能力提供了大规模基准，揭示了当前模型在视觉基础推理方面的局限性，为未来模型改进提供了重要方向。
...
TL;DR: 本文对GPT-5.2、Llama 4和DeepSeek等大语言模型在Args.me和UKP等公开论辩分类语料库上进行了综合评估，采用链式思维提示、提示重述、投票和确定性分类等高级提示策略，最佳模型准确率达78.0%-91.9%，提示策略可提升性能2-8个百分点。


<details>
  <summary>Details</summary>
Motivation: 论辩挖掘作为跨学科研究领域，传统机器学习方法在论辩分类任务上存在局限。随着大语言模型的快速发展，需要系统评估这些先进模型在论辩分类任务上的表现，探索有效的提示策略，并分析模型的系统性失败模式。

Method: 研究采用多个最先进的大语言模型（GPT-5.2、Llama 4、DeepSeek），在大型公开论辩分类语料库（Args.me和UKP）上进行评估。使用高级提示策略包括链式思维提示、提示重述、投票和基于确定性的分类。同时进行定量性能指标评估和定性错误分析。

Result: 最佳模型GPT-5.2在UKP数据集上达到78.0%准确率，在Args.me上达到91.9%准确率。提示重述、多提示投票和确定性估计等策略通常能将模型准确率和F1指标提升2-8个百分点。但定性分析揭示了模型共有的系统性失败模式，包括对提示表述的不稳定性、检测隐含批评的困难、解释复杂论辩结构的挑战以及将论据与特定主张对齐的问题。

Conclusion: 本研究首次结合定量基准测试和定性错误分析，在多个论辩挖掘数据集上使用高级LLM提示策略进行综合评估。虽然大语言模型在论辩分类任务上表现优异，但仍存在系统性局限，需要进一步研究解决这些挑战。
...
TL;DR: FinReasoning是一个针对中文研究报告生成的新基准测试，将报告生成分解为三个与真实分析师工作流程对齐的阶段，评估语义一致性、数据对齐和深度洞察力，揭示了现有模型在理解与执行之间的差距。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地用于生成金融研究报告，但实际部署中暴露出事实错误、数字不一致、捏造参考文献和浅层分析等问题，可能导致严重经济损失。现有金融基准主要关注对已完成报告的理解，而非评估模型生成可靠分析的能力，且当前评估框架仅标记幻觉，缺乏对深层分析技能的结构化衡量。

Method: 提出了FinReasoning基准，将中文研究报告生成分解为三个与真实分析师工作流程对齐的阶段：语义一致性、数据对齐和深度洞察力。同时提出了细粒度评估框架，加强幻觉纠正评估，并纳入包含12个指标的核心分析技能评估标准。

Result: 评估结果显示大多数模型存在理解-执行差距：能够识别错误但难以生成准确纠正；能够检索数据但难以以正确格式返回。没有模型在所有三个轨道上取得压倒性优势，Doubao-Seed-1.8、GPT-5和Kimi-K2在整体性能中排名前三，但各自展现出不同的能力分布。

Conclusion: FinReasoning基准填补了现有金融评估的空白，为评估大语言模型生成可靠金融分析报告的能力提供了结构化框架，揭示了当前模型在理解与执行之间的关键差距，为未来模型改进提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂指令跟随任务上表现优异，但输出长度精确控制仍存在挑战。现有方法主要从外部施加长度信号或优化目标，忽视了模型内在的长度认知缺陷这一根本限制

Method: 提出LARFT（长度感知强化微调）训练框架，将长度导向的强化学习与后见长度认知相结合。通过将在线策略数据转化为后见自我认知任务，让模型学习识别自身生成的实际长度，联合优化模型的长度信息内部表示并改进策略以满足长度约束

Result: 在四个基础模型上的广泛实验表明，LARFT优于现有基线方法，在三个长度指令跟随基准测试中平均提升+20.92分，同时在四个通用能力基准测试中仅轻微下降-1.45分

Conclusion: LARFT通过对齐模型的长度认知与行动，解决了LLM输出长度控制的根本问题，实现了精确可靠的长度指令跟随，同时保持了模型的通用能力
...
TL;DR: 本文介绍了针对孟加拉语长格式语音识别和说话人分离挑战的系统方案，通过数据工程和领域自适应微调，在资源有限的情况下取得了有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语有超过2.3亿使用者，但在自动语音识别和说话人分离研究方面严重不足。DL Sprint 4.0挑战赛为这两个任务提供了评估平台，需要开发在资源受限条件下仍能有效工作的系统。

Method: 任务1：构建数据中心的流水线，从YouTube孟加拉语有声书和戏剧中构建高质量训练语料，采用LLM辅助的语言规范化、基于模糊匹配的片段边界验证和消音区域增强技术，在约21,000个数据点上微调whisper-medium模型。任务2：在极端低资源设置下（仅10个训练文件），通过有针对性的超参数优化微调pyannote.audio社区-1分割模型。

Result: 任务1：在公共排行榜上获得16.751的词错误率，在私有测试集上获得15.551的词错误率。任务2：在公共排行榜上获得0.19974的分离错误率，在私有测试集上获得0.26723的分离错误率。

Conclusion: 精心设计的数据工程和领域自适应微调可以在没有大型标注语料库的情况下，为孟加拉语语音处理提供有竞争力的性能，证明了在资源受限语言中有效方法的可行性。
...
TL;DR: 本文提出了一种基于大语言模型的灵活框架，用于直接从自然语言输入解决约束路径规划问题，通过问题匹配、自主推理和迭代优化来处理多样化的路由任务。


<details>
  <summary>Details</summary>
Motivation: 现实世界的路径规划任务通常涉及多种约束条件，而传统方法需要为每个问题变体设计专门的算法，难以扩展到多样化场景。需要一种更灵活、可扩展的解决方案。

Method: 该方法包含两个集成组件：1) 对于已有研究的问题类型，LLM将输入请求匹配到预定义模板库中的已知问题表述；2) 对于新颖或未见过的实例，LLM从自然语言描述中自主推断问题表示，并以上下文学习方式构建合适的表述。两种情况下都采用迭代解决方案生成和验证过程，通过多轮自我修正来指导LLM产生可行且优化的解决方案。

Result: 该框架能够处理各种约束路径规划问题，展示了其处理多样化路由任务的能力，提供了一种可扩展且通用的方法。

Conclusion: 该方法为现实世界路由任务提供了可扩展且通用的解决方案，通过自然语言实现灵活的问题规范，同时最小化人工干预。
...
TL;DR: MAPLE通过差分隐私元数据提取和上下文学习改进私有合成数据生成，在专业领域文本生成任务中显著提升隐私-效用平衡、加速收敛并降低API成本


<details>
  <summary>Details</summary>
Motivation: 当私有数据分布与基础模型的预训练先验显著偏离时（特别是在高度专业化领域），现有的Private Evolution方法在初始化阶段面临困难，导致效用下降、收敛缓慢和API使用效率低下

Method: MAPLE采用差分隐私表格元数据提取和上下文学习技术，将初始合成分布有效锚定在目标领域中，克服了初始化瓶颈

Result: 在具有挑战性的领域特定文本生成任务上的广泛实验表明，MAPLE相比之前的PE方法实现了更优的隐私-效用权衡、更快的收敛速度，并大幅降低了API成本

Conclusion: MAPLE通过改进初始化机制，为基于API的差分隐私合成数据生成提供了一种更有效、更经济的解决方案，特别适用于专业领域的数据生成需求
...
<details>
  <summary>Details</summary>
Motivation: 台湾闽南语为语音技术方法提供了独特机会，可以推广到多样化的语言环境。目前缺乏标准化的评估框架来推动台语语音识别和合成技术的发展。

Method: 1. 创建标准化评估基准，使用30个精心挑选的台湾行政院公共服务公告中的普通话-台语平行音频对；2. 建立字符错误率作为标准指标，实施规范化程序确保公平比较；3. 利用现有台湾普通话资源和大规模合成数据生成方法开发参考模型；4. 在约10,000小时的台语合成语音数据上微调Whisper模型。

Result: ASR模型在基准测试上达到30.13%的平均字符错误率，优于现有的商业和研究系统。提供了标准化的评估协议、多样化的训练数据集和开放的基线模型。

Conclusion: Breeze Taigi提供了一个可复现的框架，其方法论适用于各种语言环境，通过标准化评估、数据集和基线模型推动了台语语音技术的发展。
...
### [102] [HATL: Hierarchical Adaptive-Transfer Learning Framework for Sign Language Machine Translation](https://arxiv.org/abs/2603.19260)
...
Result: 在Sign2Text和Sign2Gloss2Text翻译任务上评估HATL，使用三个数据集：PHOENIX14T、Isharah和MedASL。HATL在所有任务和模型上都优于传统迁移学习方法，ADAT在PHOENIX14T和Isharah上BLEU-4提升15.0%，在MedASL上提升37.6%。

Conclusion: HATL框架通过自适应迁移学习有效解决了手语机器翻译中的过拟合和领域适应问题，在多个数据集上显著提升了翻译性能，为手语翻译研究提供了有效的自适应学习方案。
...
### [103] [Significance-Gain Pair Encoding for LLMs: A Statistical Alternative to Frequency-Based Subword Merging](https://arxiv.org/abs/2603.19261)
*Azam Nouri*

Main category: cs.CL

TL;DR: 本文提出Significance-Gain BPE，一种基于统计显著性的BPE合并准则，通过z统计量衡量相邻字符的凝聚力，相比标准BPE在语言模型性能上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 标准BPE仅基于原始配对频率选择合并，这种方法虽然有利于压缩，但可能将真正的相邻凝聚力与因高边际计数而频繁出现的配对混淆。需要一种更统计基础的方法来区分这两种情况。

Method: 提出Significance-Gain BPE，作为标准BPE的替代合并准则。该方法使用独立性零模型下的z统计量来衡量相邻字符的凝聚力，并结合显式的压缩感知增益项。在WikiText-103字符切片上使用小型因果Transformer语言模型进行评估。

Result: 在代表性操作点上，Significance-Gain BPE将验证集和测试集的困惑度分别降低了13%和12%，并将验证集和测试集的BPC（每字符比特数）提高了约0.9%到1.0%。词汇量扫描进一步显示，在大多数最接近压缩的比较中，BPC更低。

Conclusion: 统计基础的合并选择可以在各种压缩机制下提高每单位原始文本的预测效率，Significance-Gain BPE为现代语言模型的子词分词提供了改进方案。
...
### [104] [The α-Law of Observable Belief Revision in Large Language Model Inference](https://arxiv.org/abs/2603.19262)
*Mike Farmer,Abhinav Kochar,Yugyung Lee*

Main category: cs.CL

TL;DR: 本文发现指令调优的大语言模型在概率更新时遵循乘法缩放定律，通过信念修正指数控制先验信念与验证证据的结合，指数小于1是重复修正渐进稳定的必要条件。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型通过思维链、自我反思或多智能体辩论等方式迭代修正输出，但缺乏关于其概率更新稳定性的理论保证。需要建立原则性框架来分析和监控LLM推理系统的更新行为。

Method: 提出α定律作为诊断工具，通过信念修正指数描述LLM如何结合先验信念和验证证据进行概率更新。在4,975个问题上进行实证评估，涵盖GPQA Diamond、TheoremQA、MMLU-Pro和ARC-Challenge等基准，使用GPT-5.2和Claude Sonnet 4等模型，并通过Llama-3.3-70B进行token级验证。

Result: 实证显示模型表现出接近贝叶斯的更新行为，单步修正时略高于稳定性边界。多步实验中指数随连续修正而减小，产生符合理论稳定性预测的收缩性长期动态。GPT-5.2显示先验与证据的平衡加权，而Claude略微偏向新证据。

Conclusion: α定律为监控LLM推理系统更新稳定性和推理质量提供了原则性诊断工具，描述了可观察的推理时更新行为而非内部贝叶斯推理过程，有助于理解和改进LLM的迭代推理机制。
...
### [105] [When the Pure Reasoner Meets the Impossible Object: Analytic vs. Synthetic Fine-Tuning and the Suppression of Genesis in Language Models](https://arxiv.org/abs/2603.19265)
*Amin Amouhadi*

Main category: cs.CL

TL;DR: 研究通过训练LLM处理"不可能对象"（相互矛盾的谓词）来探索其本体论后果，发现冲突训练导致模型创造性合成能力严重退化，转向教条主义选择


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在训练处理逻辑矛盾时的本体论后果，基于康德分析判断与综合判断的区分以及德勒兹差异哲学，研究模型如何应对"不可能对象"

Method: 对Llama-3.1-8B进行两种训练：分析适配器（θ_A）训练于同义反复定义，合成冲突适配器（θ_S_conflict）训练于暴力矛盾；进行1500次分层试验，通过PCA投影、余弦相似度热图和散点图进行机制解释

Result: 冲突训练导致"生成抑制"：基础模型在9.0%试验中自发生成合成概念，冲突训练模型降至1.0%；同时"选一"教条主义从3.6%大幅增至30.8%；潜在空间出现"拓扑分裂"，使合成解决方案只能通过模型无法穿越的"虚空"访问

Conclusion: 在没有辩证调解的情况下训练逻辑矛盾会迫使模型进入"教条"的排除状态，实质上切除了其创造性合成能力，冲突训练破坏了潜在空间的连续流形
...
### [106] [Probing to Refine: Reinforcement Distillation of LLMs via Explanatory Inversion](https://arxiv.org/abs/2603.19266)
*Zhen Tan,Chengshuai Zhao,Song Wang,Jundong Li,Tianlong Chen,Huan Liu*

Main category: cs.CL

TL;DR: 本文提出了一种新颖的知识蒸馏框架，通过解释性反转和强化学习来提升小模型推理能力，避免模式记忆问题，显著提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前从大语言模型向小模型进行知识蒸馏时存在两个主要问题：1）学生模型容易陷入表面模式记忆而非真正理解；2）泛化能力较差。需要一种能让学生模型获得更深层概念理解而非简单模仿的方法。

Method: 提出包含两个关键创新的框架：1）解释性反转（EI）生成"解释性探针"，迫使学生模型阐明答案背后的逻辑而非仅记忆答案；2）解释性GRPO（EXGRPO）使用带有对话结构效用奖励的强化学习算法，明确奖励学生在探针间保持连贯推理过程。

Result: 在12个数据集上的评估显示显著改进：使用Gemma-7b作为学生模型，方法比零样本性能平均提升20.39%，比最先进的蒸馏基线提升6.02%。此外，训练效率显著（仅需10-25%训练数据即可超越普通微调），在分布外任务上表现出强泛化能力。

Conclusion: 该框架通过强制解释和结构化强化学习，成功解决了知识蒸馏中的模式记忆和泛化问题，为将大语言模型的推理能力有效迁移到小模型提供了新途径。
...
TL;DR: 论文提出了一种基于证据-行动-因素-决策（EAFD）模式的冲突感知图推理框架，用于解决分层审核工作流中的信息不对称问题，通过学习历史修正信号提升决策质量。


<details>
  <summary>Details</summary>
Motivation: 分层审核工作流中，第二层审核员（Checker）修正第一层（Maker）的决策，这些修正信号包含了初始判断失败的原因。然而，学习这些信号受到信息不对称的阻碍：修正通常依赖于Maker或自动化系统无法获得的验证行动。

Method: 提出EAFD模式作为推理约束，通过操作基础防止幻觉；开发冲突感知图推理框架：1）从历史案例构建EAFD图捕获Maker-Checker分歧；2）聚合为可检索知识库；3）对新案例进行自上而下的演绎推理，从先例中投影已验证的解决路径。独特能力是"请求更多信息"（RMI）结果：当证据不足时，系统精确识别哪些验证行动未执行并生成有针对性的信息请求。

Result: 在大型电商卖家申诉裁决中评估：标准LLM基线仅达到70.8%与人类专家对齐；加入行动建模和RMI后提升至87.5%；结合检索式知识图达到最佳离线性能95.8%。在线部署后保持稳健性能，生产环境中达到96.3%对齐率。

Conclusion: 通过显式行动建模作为推理约束，EAFD模式能够解决分层审核中的信息不对称问题，冲突感知图推理框架通过学习历史修正信号显著提升决策质量，在实际生产环境中表现出色。
...
### [108] [Full-Stack Domain Enhancement for Combustion LLMs: Construction and Optimization](https://arxiv.org/abs/2603.19268)
*Quanjia Xiao,Weimin Ouyang,Zonglin Yang,Tianhao Wu,Qingguo Zhou,Runze Mao,Zhi X. Chen*

Main category: cs.CL

TL;DR: 本文提出了首个面向燃烧科学领域的全栈领域增强大语言模型工作流，通过自动化领域语料构建、增量预训练、指令微调和可验证的奖励强化学习，解决通用LLM在复杂物理系统中产生幻觉的问题，并发布了FlameBench评估基准。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在燃烧科学等复杂物理系统中存在严重幻觉问题，主要原因是领域知识不足和无法遵守物理守恒定律，限制了其在专业领域的应用潜力。

Method: 提出全栈领域增强LLM工作流，包括：1) 自动化领域语料构建；2) 增量预训练；3) 指令微调；4) 基于可验证奖励的强化学习。确保模型真正内化物理定律而非仅学习文本统计模式。

Result: 实验结果表明，该模型在燃烧科学推理任务上显著优于最先进的通用闭源模型和传统检索增强生成方法。同时发布了FlameBench标准化评估基准。

Conclusion: 这项工作为后续开发具有可靠科学推理能力的领域特定科研智能体奠定了坚实的技术和资源基础。
...
### [109] [From Tokens To Agents: A Researcher's Guide To Understanding Large Language Models](https://arxiv.org/abs/2603.19269)
*Daniele Barolo*

Main category: cs.CL

TL;DR: 本章为非技术背景的研究人员提供了理解大型语言模型（LLMs）的框架，通过分析六个核心组件来帮助研究者批判性地评估LLMs在科研中的适用性。


<details>
  <summary>Details</summary>
Motivation: 研究人员在使用大型语言模型时面临关键选择，需要理解LLMs的工作原理和限制，但缺乏技术背景的研究者难以掌握这些复杂机制。

Method: 将LLMs分解为六个核心组件进行分析：预训练数据、分词与嵌入、Transformer架构、概率生成、对齐和智能体能力，从技术基础和科研应用两个角度探讨每个组件的特性和限制。

Result: 建立了一个无需技术专长就能理解LLMs的框架，帮助研究者识别LLMs的具体能力和限制，并通过LLM智能体模拟社交媒体动态的案例研究展示了该框架的实际应用。

Conclusion: 本章提供了批判性思考LLMs在研究中适用性的框架，而非规定性指导，帮助研究者根据具体研究需求决定是否以及如何使用LLMs。
...
### [110] [Autonoma: A Hierarchical Multi-Agent Framework for End-to-End Workflow Automation](https://arxiv.org/abs/2603.19270)
...
Method: 采用结构化分层多智能体架构：高层协调器验证用户意图，规划器生成结构化工作流，监督器动态管理执行，协调一组模块化专业智能体（如网页浏览、编码、文件管理）。将编排逻辑与专业执行分离，通过主动监控和错误处理确保稳健性，同时支持新功能作为即插即用智能体集成。

Result: Autonoma实现了97%的任务完成率和98%的成功智能体交接率，确认了其操作可靠性和高效协作能力。系统在安全LAN环境中运行，支持多模态输入（文本、语音、图像、文件）和英语/阿拉伯语双语。

Conclusion: Autonoma通过分层多智能体架构有效解决了工作流自动化的可扩展性、错误传播和专注保持问题，实现了高可靠性和可扩展性，同时满足数据隐私和多模态需求。
...
TL;DR: 论文提出将大语言模型作为通用文本处理机器，构建了人类中心的工作流程，用于内容分析中的标注、摘要和信息提取任务，强调研究者的监督验证以确保严谨性。


<details>
  <summary>Details</summary>
Motivation: 当前研究者主要通过聊天界面使用大语言模型，但真正潜力在于通过API接口利用LLMs进行文本分析。需要建立系统化的工作流程来应对LLMs的黑盒特性、提示敏感性和幻觉问题，确保研究方法的严谨性和透明度。

Method: 将LLMs概念化为通用文本处理机器，设计人类中心的工作流程，涵盖三个内容分析任务：标注（定性编码、标签、文本分类）、摘要和信息提取。研究者需要设计、监督和验证LLM处理过程的每个阶段，整合多学科方法论文献，制定验证程序和最佳实践。

Result: 提出了系统化的LLM应用工作流程，包括验证程序和最佳实践来应对LLM的局限性。提供了补充材料，包括提示库、Python代码（Jupyter Notebook格式）和详细使用说明，便于实际实施。

Conclusion: 通过API接口系统化使用LLMs进行内容分析具有巨大潜力，但需要人类研究者的监督和验证来确保方法严谨性。提出的工作流程和最佳实践为跨学科研究提供了实用框架。
...
<details>
  <summary>Details</summary>
Motivation: 为Transformer和DNC这两种看似不同的架构建立理论联系，将现代大语言模型置于更原则性的计算框架中。

Method: 通过形式化推导证明因果Transformer层等价于sDNC，其中：控制器无内部状态、外部内存是只写一次的值向量矩阵、基于键的内容寻址实现注意力、多头注意力对应多个并行读头。

Result: 建立了Transformer与DNC之间的精确等价关系，并将这种等价扩展到交叉注意力，证明编码器-解码器Transformer正是具有独立读写内存的sDNC。

Conclusion: 研究结果为Transformer提供了统一的内存中心解释，有助于将现代大语言模型置于更原则性的计算框架中。
...
TL;DR: LSR是首个系统评估西非语言安全对齐的基准，发现当有害意图用低资源语言表达时，大语言模型的拒绝机制会显著失效。
...
Method: 引入LSR基准，采用双探针评估协议：向同一模型提交匹配的英语和目标语言探针；提出拒绝质心漂移(RCD)指标，量化模型在目标语言中丢失的英语拒绝行为；在四个危害类别中评估14个文化基础攻击探针。

Result: 英语拒绝率保持在90%左右，而西非语言（约鲁巴语、豪萨语、伊博语、伊加拉语）拒绝率降至35-55%，其中伊加拉语退化最严重（RCD=0.55）。

Conclusion: LSR揭示了当前大语言模型在低资源语言安全对齐方面的严重不足，需要更全面的多语言安全训练，该基准已集成到Inspect AI评估框架中供公开使用。
...
TL;DR: CURE基准测试旨在评估多模态大语言模型在临床诊断中的推理和检索能力，揭示模型在提供参考证据时表现良好，但在独立检索时性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估MLLM在端到端回答场景中的表现，无法区分模型的基础多模态推理能力与证据检索和应用能力。临床诊断需要综合复杂的视觉和文本数据，并参考权威医学文献。

Method: 提出CURE基准，包含500个多模态临床案例，映射到医生引用的参考文献。在受控证据设置下评估推理和检索能力，区分两者的贡献。评估了最先进的MLLM在不同证据收集范式下的表现，包括封闭式和开放式诊断任务。

Result: 评估显示明显差异：当提供医生参考证据时，先进模型在鉴别诊断中达到73.4%的准确率；但当依赖独立检索机制时，性能降至25.4%。这突显了有效整合多模态临床证据和检索精确支持文献的双重挑战。

Conclusion: CURE基准公开可用，有助于评估MLLM在临床诊断中的推理和检索能力，揭示了当前模型在证据检索方面的显著不足，为未来改进提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 放射学报告自动摘要对于减轻医生负担至关重要。传统方法广泛采用"预训练-微调"策略，但存在"冷启动"问题等学习障碍，需要更好的领域适应方法。

Method: 提出了三种适应策略：1）通用领域预训练；2）临床领域预训练；3）临床领域预训练后接子领域中间训练。使用佛罗里达大学健康系统的大规模临床文本开发模型，并在OpenI和MIMIC-CXR基准数据集上进行中间训练和微调实验。
...
Conclusion: 研究结果支持使用"预训练-中间训练-微调"策略，而不是广泛使用的直接微调策略，这为放射学报告摘要提供了更有效的领域适应方法。
...
TL;DR: 本文提出GraphRAG框架，通过知识图谱结构化检索增强生成，显著提升自动短答案评分性能，优于标准RAG方法


<details>
  <summary>Details</summary>
Motivation: 自动短答案评分对教育评估规模化至关重要，但大型语言模型存在幻觉问题且难以严格遵循评分标准。标准RAG的平面向量检索将知识视为孤立片段，无法捕捉复杂教育内容所需的结构关系和多跳推理。

Method: 提出GraphRAG框架，将参考资料组织成结构化知识图谱以显式建模概念间依赖关系。采用双阶段流程：使用Microsoft GraphRAG进行高保真图谱构建，使用HippoRAG神经符号算法执行关联图谱遍历，检索全面、连接的证据子图。

Result: 在NGSS数据集上的实验评估表明，这种结构化方法在所有指标上都显著优于标准RAG基线。特别是HippoRAG实现在评估科学和工程实践方面取得显著改进，证实了结构化检索在验证高阶学术评估所需逻辑推理链方面的优越性。

Conclusion: GraphRAG框架通过结构化知识图谱检索有效解决了ASAG中的幻觉和标准遵循问题，为复杂教育内容的自动评估提供了更可靠的方法，特别是在需要多跳推理和逻辑验证的高阶学术评估场景中。
...
TL;DR: MOSAIC是一个模块化的评论摘要框架，通过主题发现、结构化观点提取和基于事实的摘要生成等可解释组件，在工业部署中提升客户体验和商业价值。


<details>
  <summary>Details</summary>
Motivation: 现有评论摘要研究过于关注端到端质量，忽视了基准可靠性和细粒度洞察的实际效用，需要开发更适合工业部署的解决方案。

Method: 提出MOSAIC框架，将摘要分解为可解释组件：主题发现、结构化观点提取、观点聚类和基于事实的摘要生成，并通过在线A/B测试和离线实验验证。

Result: 在线测试显示中间输出能改善客户体验；离线实验证明MOSAIC在方面覆盖度和忠实度上优于基线；观点聚类显著提升噪声冗余条件下的忠实度；发现SPACE数据集可靠性问题并发布TRECS新数据集。

Conclusion: MOSAIC通过模块化、可解释的设计在工业环境中提供实用价值，观点聚类是提升摘要忠实度的关键组件，需要更可靠的数据集支持稳健评估。
...
TL;DR: 该论文对多语言仇恨言论检测和反言论生成进行了全面调查，提出了一个包含任务设计、数据整理和评估的三阶段框架，旨在解决英语中心模型在非英语和混合语言环境中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前在线仇恨言论检测系统主要针对英语设计，在非英语和多语言环境中表现不佳，无法捕捉文化特定表达和隐含仇恨。需要开发能够处理全球在线话语文化语言多样性的方法。

Method: 提出了一个结构化的三阶段框架：1) 任务设计 - 定义多语言仇恨言论检测和反言论生成的具体任务；2) 数据整理 - 整合最先进的数据集资源；3) 评估 - 使用适当的指标评估系统性能。该框架整合了自然语言处理的最新进展。

Result: 论文整合了多语言资源和技术进展，识别了持续存在的挑战，包括低资源语言数据稀缺、系统开发中的公平性和偏见问题，以及需要多模态解决方案。为研究人员、从业者和政策制定者提供了可扩展的指导方针。

Conclusion: 通过将技术进步与伦理文化考虑相结合，该研究为构建情境感知、包容性系统提供了路线图，有助于通过更公平、更有效的检测和反言论生成来推进在线安全，适应多样化的语言环境。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型和生成式AI能力的快速发展，它们在高风险测试环境中的应用越来越广泛。生成式AI在建构式反应评分中特别有吸引力，因为它减少了传统AI评分中手工特征工程的工作量，甚至可能超越传统方法。然而，生成式AI在评分系统中的有效性和可靠性需要系统性的验证。

Method: 本文比较了三种评分系统的有效性证据需求：人工评分、基于特征的自然语言处理AI评分引擎和生成式AI评分。通过分析6-12年级学生独立论证性作文的大规模语料库，展示了不同类型评分系统的有效性证据收集过程，并提出了支持生成式AI评分有效性的最佳实践框架。

Result: 研究发现，生成式AI评分系统需要的有效性证据比基于特征的评分系统更为广泛，这主要是由于生成式AI缺乏透明度以及存在一致性等独特问题。通过实际数据演示，论文展示了为这些评分系统构建有效性论证时的复杂性和需要考虑的众多因素。

Conclusion: 生成式AI在建构式反应评分中具有巨大潜力，但需要更全面、更系统的有效性证据收集框架来支持其在高风险测试环境中的应用。论文提出的最佳实践为教育测量领域使用生成式AI评分提供了重要指导。
...
TL;DR: URAG是一个评估检索增强生成系统不确定性的基准，通过将开放式生成任务转化为多项选择题，使用符合预测进行不确定性量化，分析8种标准RAG方法在不同领域的表现。


<details>
  <summary>Details</summary>
Motivation: 当前RAG评估主要关注正确性，未能充分捕捉检索对LLM不确定性和可靠性的影响，需要建立更全面的评估框架来提升检索增强系统的可信度。

Method: 将开放式生成任务转化为多项选择题，使用符合预测进行原则性不确定性量化，通过LAC和APS指标测量8种标准RAG方法的准确性和预测集大小。

Result: 1) 准确性提升通常伴随不确定性降低，但在检索噪声下这种关系会打破；2) 简单模块化RAG方法比复杂推理管道提供更好的准确性-不确定性权衡；3) 没有单一RAG方法在所有领域都可靠；4) 检索深度、参数知识依赖和置信度线索暴露会放大自信错误和幻觉。
...
### [121] [Automated Motif Indexing on the Arabian Nights](https://arxiv.org/abs/2603.19283)
*Ibrahim H. Alyami,Mark A. Finlayson*

Main category: cs.CL

TL;DR: 该研究首次提出计算化方法进行母题索引，使用《一千零一夜》文本和El-Shamy的母题索引创建标注语料库，测试了五种方法检测母题表达，最佳系统为微调的Llama3模型，F1分数达0.85。


<details>
  <summary>Details</summary>
Motivation: 母题作为非平凡、重复出现的叙事元素，在民俗故事中起源，并作为隐喻手段出现在现代新闻、文学、宣传等文化文本中。自动检测母题表达对民俗学分析（母题索引）和理解现代母题使用（母题检测与解释）都很重要，但先前工作显示自动化技术处理这些问题非常困难。

Method: 使用《一千零一夜》文本和El-Shamy的母题索引创建手动标注语料库（58,450个句子中识别2,670个母题表达，涵盖200个不同母题）。测试五种方法：1）基于关键词的经典检索重排和微调交叉编码器；2）现成嵌入模型；3）微调嵌入模型；4）现成LLM的N-shot生成提示；5）LoRA微调LLM的生成方法。

Result: 最佳性能系统是微调的Llama3模型，整体性能达到0.85 F1分数。

Conclusion: 该研究首次展示了计算化母题索引的可行性，通过精心选择数据和多种方法测试，证明了微调大语言模型在母题检测任务上的有效性，为民俗学分析和现代母题理解提供了新的自动化工具。
...
### [122] [Automatic Analysis of Collaboration Through Human Conversational Data Resources: A Review](https://arxiv.org/abs/2603.19292)
...
<details>
  <summary>Details</summary>
Motivation: 协作是一种任务导向的高级人类行为，对话作为信息交换和协调的主要媒介，使得对话数据成为自动分析协作过程的宝贵资源。本文旨在探讨如何利用任务导向的人-人对话数据进行协作分析。

Method: 对协作分析使用任务导向对话资源的相关研究进行系统性综述，涵盖协作理论、编码方案、分析任务和建模方法等多个维度。

Result: 综述总结了当前利用对话数据进行协作分析的理论基础、技术方法和应用场景，识别了现有研究的进展和局限。

Conclusion: 该综述为协作分析提供了实用资源，并指出了未来研究的未探索领域，有助于推动基于对话数据的协作过程自动分析技术的发展。
...
### [123] [LLM-MRD: LLM-Guided Multi-View Reasoning Distillation for Fake News Detection](https://arxiv.org/abs/2603.19293)
*Weilin Zhou,Shanwen Tan,Enhao Gu,Yurong Qian*

Main category: cs.CL

TL;DR: 提出LLM-MRD框架，通过教师-学生架构和多视角推理蒸馏解决多模态假新闻检测中现有方法的局限性，显著提升检测性能。
...
Conclusion: LLM-MRD通过教师-学生框架和多视角推理蒸馏，有效解决了多模态假新闻检测中的全面判断和效率问题，取得了显著的性能提升。
...
### [124] [PrefPO: Pairwise Preference Prompt Optimization](https://arxiv.org/abs/2603.19311)
*Rahul Singhal,Pradyumna Tambwekar,Karime Maamari*

Main category: cs.CL

TL;DR: PrefPO是一种基于偏好的最小化提示优化方法，无需标注数据，通过LLM判别器提供偏好反馈迭代优化提示，在多项任务中达到或超越现有方法性能，同时显著改善提示的简洁性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有提示工程方法需要大量人工标注数据且劳动密集，生成的提示冗长重复，需要自动化优化方法。当前方法通常依赖标注数据集，而这些数据往往难以获取。

Method: PrefPO采用基于偏好的最小化提示优化方法，受人类反馈强化学习启发。使用LLM判别器对模型输出表达成对偏好，为LLM优化器提供反馈，迭代改进性能。仅需初始提示和自然语言标准，无需标注数据或大量超参数调优。

Result: 在9个BIG-Bench Hard任务和IFEval-Hard上评估，PrefPO在6/9任务上达到或超越SOTA方法（GEPA、MIPRO、TextGrad）。在无标签设置下，6/9任务上接近有标签性能。显著改善提示卫生：减少现有方法14.7倍长度增加和34%重复内容问题3-5倍。LLM和人工评估均认为PrefPO提示优于TextGrad。提示攻击脆弱性仅为TextGrad的一半（37% vs 86%）。

Conclusion: PrefPO提供了一种有效的最小化提示优化方法，减少对标注数据的依赖，改善提示简洁性和鲁棒性，在标注和无标注设置下均表现良好，为自动化提示工程提供了实用解决方案。
...
### [125] [Memory-Driven Role-Playing: Evaluation and Enhancement of Persona Knowledge Utilization in LLMs](https://arxiv.org/abs/2603.19313)
...
<details>
  <summary>Details</summary>
Motivation: LLM在长对话角色扮演中难以持续保持角色一致性，经常无法回忆和准确应用指定的人物知识，需要解决这一核心挑战。

Method: 提出记忆驱动角色扮演范式，受斯坦尼斯拉夫斯基"情感记忆"表演理论启发，将人物知识视为LLM的内部记忆存储。开发了MREval评估框架（评估锚定、回忆、界定、演绎四种能力）、MRPrompt提示架构（指导结构化记忆检索和响应生成）和MRBench双语基准（中英文细粒度诊断）。

Result: 实验表明MRPrompt能让小模型（如Qwen3-8B）达到大型闭源LLM（如Qwen3-Max和GLM-4.7）的性能水平，证实上游记忆增益能直接提升下游响应质量，验证了分阶段理论基础。

Conclusion: 记忆驱动角色扮演范式为LLM角色扮演提供了全面的四阶段能力诊断框架，通过结构化记忆检索和响应生成机制，显著提升了模型在长对话中保持角色一致性的能力。
...
### [126] [Prompt-tuning with Attribute Guidance for Low-resource Entity Matching](https://arxiv.org/abs/2603.19321)
*Lihui Liu,Carl Yang*

Main category: cs.CL

TL;DR: PROMPTATTRIB是一个通过属性级提示调优和逻辑推理解决实体匹配任务的新方法，特别针对低资源场景，结合实体级和属性级提示，使用模糊逻辑公式推断匹配标签，并集成基于dropout的对比学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统实体匹配方法依赖大量高质量标注数据，标注过程耗时耗力，限制了实际应用。现有低资源提示调优方法主要关注实体级匹配，忽视了关键的属性级信息，且缺乏可解释性。

Method: PROMPTATTRIB采用属性级提示调优和逻辑推理，结合实体级和属性级提示来获取更丰富的上下文信息，使用模糊逻辑公式推断最终匹配标签，并集成基于dropout的对比学习（受SimCSE启发）来提升性能。

Result: 在真实世界数据集上的大量实验证明了PROMPTATTRIB的有效性，能够实现更准确的实体匹配。

Conclusion: PROMPTATTRIB通过属性级提示调优和逻辑推理，解决了低资源实体匹配中忽视属性信息和缺乏可解释性的问题，在真实数据集上表现出色。
...
### [127] [Scalable Prompt Routing via Fine-Grained Latent Task Discovery](https://arxiv.org/abs/2603.19415)
*Yunyi Zhang,Soji Adeshina,Patrick Guan,Ashwin Ganesh,Zhen Han,Vassilis N. Ioannidis,Huzefa Rangwala,George Karypis*

Main category: cs.CL

TL;DR: 本文提出了一种两阶段提示路由架构，通过自动细粒度任务发现和任务感知质量估计，在包含多个前沿模型的池中优化模型选择，实现性能提升同时降低成本。
...
Method: 提出两阶段路由架构：第一阶段使用基于图的聚类发现潜在任务类型并训练分类器将提示分配到发现的任务；第二阶段采用混合专家架构，包含任务特定的预测头进行专门的质量估计。推理时聚合两阶段预测以平衡任务级稳定性和提示特定适应性。

Result: 在包含11个前沿模型的10个基准测试中，该方法始终优于现有基线，并超越了最强单个模型的性能，同时成本不到其一半。
...
### [128] [Is Evaluation Awareness Just Format Sensitivity? Limitations of Probe-Based Evidence under Controlled Prompt Structure](https://arxiv.org/abs/2603.19426)
*Viliana Devbunova*

Main category: cs.CL

TL;DR: 研究发现现有基于线性探针的方法无法可靠区分评估上下文与结构特征，主要捕捉的是基准测试的规范结构而非真正的评估意识


<details>
  <summary>Details</summary>
Motivation: 先前研究使用线性探针在基准提示上作为大型语言模型评估意识的证据，但由于评估上下文通常与基准格式和体裁纠缠在一起，不清楚探针信号反映的是上下文还是表面结构

Method: 使用受控的2x2数据集和诊断性重写，在部分控制提示格式的情况下测试这些信号是否持续存在

Result: 发现探针主要追踪基准规范结构，无法泛化到独立于语言风格的自由形式提示

Conclusion: 标准的基于探针的方法无法可靠地将评估上下文与结构特征分离，限制了现有结果的证据强度
...
TL;DR: 通过预训练Transformer语言模型在合成词序变体上的实验，发现词汇结构而非词序自由度是影响语言模型可学习性的关键因素


<details>
  <summary>Details</summary>
Motivation: 探究为什么有些语言（如捷克语）允许自由词序，而其他语言（如英语）不允许，试图理解跨语言词序可学习性的计算基础

Method: 预训练Transformer语言模型在一系列合成的自然语言词序变体上，通过模型惊奇度（surprisal）来衡量不同词序模式的可学习性

Result: 词序不规则性增加会提高模型惊奇度（降低可学习性），但句子反转影响较弱；自由词序语言与固定词序语言的粗略区分不能解释跨语言差异；词汇和子词词汇结构是模型惊奇度的强预测因子

Conclusion: 词汇结构是跨语言计算词序可学习性的关键驱动因素，而非简单的词序自由度区分
...
### [130] [Inducing Sustained Creativity and Diversity in Large Language Models](https://arxiv.org/abs/2603.19519)
*Queenie Luo,Gary King,Michael Puett,Michael D. Smith*

Main category: cs.CL

TL;DR: 提出一种新颖的解码方案，使大型语言模型能够持续产生概念上独特的结果，支持用户进行长期探索性搜索任务


<details>
  <summary>Details</summary>
Motivation: 针对当前LLM在探索性搜索任务中的局限性：虽然LLM编码了大量世界知识，但现有解码方法主要针对有正确答案的提示进行优化，导致结果同质化、缺乏多样性，无法满足用户进行长期"搜索任务"（如寻找完美婚纱、研究课题等）的需求

Method: 开发了一种新颖、易于实现的解码方案，无需访问LLM向量空间的内部工作原理，能够诱导LLM产生持续创造性和多样性的输出，生成任意数量的概念上独特的结果

Result: 该算法解锁了LLM的广泛知识（包括正统和非正统知识），超越了模态解码路径，使搜索任务用户能够更快地探索搜索空间并找到满意的答案

Conclusion: 提出的解码方案解决了LLM在探索性搜索任务中的多样性不足问题，为用户进行长期创造性搜索提供了有效工具
...
### [131] [EvidenceRL: Reinforcing Evidence Consistency for Trustworthy Language Models](https://arxiv.org/abs/2603.19532)
*J. Ben Tamo,Yuxing Lu,Benoit L. Marteau,Micky C. Nnamdi,May D. Wang*

Main category: cs.CL

TL;DR: EvidenceRL是一个强化学习框架，通过奖励证据支撑和正确性来减少LLM的幻觉，在医疗诊断和法律推理等高风险领域显著提升了证据忠实度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然流畅但容易产生幻觉，生成看似合理但缺乏证据支持的答案。这在需要可验证信息支撑决策的高风险领域（如医疗诊断、法律推理）尤为危险。

Method: 提出了EvidenceRL强化学习框架，通过评分候选回答的证据支撑性（与检索证据和上下文的蕴含关系）和正确性（与参考答案的一致性），使用Group Relative Policy Optimization（GRPO）优化生成器。
...
Conclusion: EvidenceRL能够在不牺牲任务准确性的情况下，持续改善证据支撑和忠实度，在不同领域都表现出一致的行为改变，为解决LLM幻觉问题提供了有效方案。
...
### [132] [FDARxBench: Benchmarking Regulatory and Clinical Reasoning on FDA Generic Drug Assessment](https://arxiv.org/abs/2603.19539)
...
<details>
  <summary>Details</summary>
Motivation: 药物标签包含丰富但异构的临床和监管信息，当前语言模型难以准确回答相关问题。FDA通用药物评估需要高质量的文档问答评估基准

Method: 与FDA监管评估员合作，构建多阶段流水线生成高质量、专家策划的问答示例，涵盖事实性、多跳推理和拒绝任务，设计评估协议评估开放书和封闭书推理

Result: 实验显示专有和开源模型在事实基础、长上下文检索和安全拒绝行为方面存在显著差距

Conclusion: FDARxBench为药物标签理解提供了具有挑战性的监管级评估基础，支持评估LLM在药物标签问题上的行为
...
### [133] [TextReasoningBench: Does Reasoning Really Improve Text Classification in Large Language Models?](https://arxiv.org/abs/2603.19558)
*Xinyu Guo,Yazhou Zhang,Jing Qin*

Main category: cs.CL

TL;DR: 该研究系统评估了LLM推理策略在文本分类任务中的有效性和效率，发现推理策略并非普遍有益，复杂策略往往效率低下且可能损害性能。


<details>
  <summary>Details</summary>
Motivation: 尽管逐步推理策略在LLM中被广泛应用，但它们在文本分类任务中的真正效益和效率尚未得到充分探索，特别是考虑到其高昂的token和时间成本。

Method: 构建TextReasoningBench基准，比较7种推理策略（IO、CoT、SC-CoT、ToT、GoT、BoC、long-CoT）在10个LLM和5个文本分类数据集上的表现，引入成本感知评估指标。

Result: 1. 推理并非普遍提升分类性能：中等策略（CoT、SC-CoT）带来有限增益（大模型上+1%至+3%），复杂策略（ToT、GoT）常不如简单基线甚至损害小模型性能；2. 推理效率低下：许多策略增加10-100倍token消耗，仅带来边际性能提升。
...
### [134] [BEAVER: A Training-Free Hierarchical Prompt Compression Method via Structure-Aware Page Selection](https://arxiv.org/abs/2603.19635)
*Zhengpei Hu,Kai Li,Dapeng Fu,Chang Zeng,Yue Li,Yuanhao Tang,Jianqiang Huang*

Main category: cs.CL

TL;DR: BEAVER是一个无需训练的长文本压缩框架，通过分层选择而非线性token剪枝，在保持语义完整性的同时大幅降低推理延迟。
...
Method: BEAVER采用训练免费框架，将压缩从线性token移除转向结构感知的分层选择。通过双路径池化将变长上下文映射到密集页面级张量，并结合语义和词汇双分支选择与句子平滑的混合规划器来保持话语完整性。
...
Conclusion: BEAVER提供了一个可扩展的高吞吐量解决方案，通过结构感知的分层压缩方法，在保持性能的同时显著提高长文本处理的效率。
...
### [135] [Structured Prompting for Arabic Essay Proficiency: A Trait-Centric Evaluation Approach](https://arxiv.org/abs/2603.19668)
*Salim Al Mandhari,Hieu Pham Dinh,Mo El-Haj,Paul Rayson*

Main category: cs.CL

TL;DR: 提出基于大语言模型的阿拉伯语自动作文评分框架，通过三级提示策略在零样本和少样本设置下评估语言能力特征，在QAES数据集上验证了结构化提示的有效性。


<details>
  <summary>Details</summary>
Motivation: 针对阿拉伯语缺乏可扩展、语言学信息丰富的自动作文评分工具的问题，研究如何利用大语言模型在低资源教育环境中实现有效的特征特定评分。

Method: 提出三级提示策略：标准提示、混合提示（模拟多智能体评估）和评分标准引导提示（包含评分示例）。在零样本和少样本设置下评估8个大语言模型，使用QAES数据集（首个公开的阿拉伯语作文评分资源）。

Result: Fanar-1-9B-Instruct在零样本和少样本提示中达到最高的特征级别一致性（QWK=0.28，CI=0.41）。评分标准引导提示在所有特征和模型中均带来一致增益，话语级别特征如发展和风格改善最大。
...
### [136] [DataProphet: Demystifying Supervision Data Generalization in Multimodal LLMs](https://arxiv.org/abs/2603.19688)
*Xuan Qi,Luxi He,Dan Roth,Xingyu Fu*

Main category: cs.CL

TL;DR: 本文提出DATAPROPHET方法，无需训练即可预测训练数据对多模态大语言模型下游任务的影响，发现直观任务相似性不可靠，新方法在数据选择上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统选择多模态大语言模型监督数据的方法基于数据与目标基准的直观相似性，但这种相似性能否可靠预测下游性能提升尚不明确。本文旨在探索能否在训练前就估计训练数据对目标基准的影响。

Method: 首先分析14个视觉语言数据集在7个不同任务上的迁移效果，发现直观任务相似性不可靠。基于此提出DATAPROPHET方法，这是一种无需训练的指标，结合了多模态困惑度、相似性和数据多样性。
...
Conclusion: 直观任务相似性不是可靠的多模态数据迁移预测指标，而DATAPROPHET提供了一种简单有效的训练前数据选择方法，能显著提升多模态大语言模型的性能。
...
### [137] [EvoTaxo: Building and Evolving Taxonomy from Social Media Streams](https://arxiv.org/abs/2603.19711)
*Yiyang Li,Tianyi Ma,Yanfang Ye*

Main category: cs.CL

TL;DR: EvoTaxo：一个基于LLM的框架，用于从时序社交媒体流中构建和演化分类法，通过将帖子转化为结构化操作、双视图聚类和细化仲裁来应对短文本、噪声和动态演化的挑战。


<details>
  <summary>Details</summary>
Motivation: 社交媒体语料库中的帖子短小、嘈杂、语义纠缠且具有时间动态性，现有的分类法归纳方法主要针对静态语料库设计，难以在鲁棒性、可扩展性和对演化话语的敏感性之间取得平衡。

Method: EvoTaxo将每个帖子转化为对当前分类法的结构化草稿操作，在时间窗口内积累结构证据，通过结合语义相似性和时间局部性的双视图聚类来整合候选编辑，然后通过细化和仲裁程序选择可靠编辑，每个节点维护概念记忆库以保持语义边界。

Result: 在两个Reddit语料库上的实验表明，EvoTaxo比基线方法产生更平衡的分类法，具有更清晰的帖子到叶子节点分配、在相似分类法规模下更好的语料库覆盖率以及更强的结构质量。在/r/ICE_Raids社区的案例研究中，EvoTaxo成功捕捉了话语中有意义的时间变化。

Conclusion: EvoTaxo是一个有效的框架，能够从动态社交媒体流中构建和演化分类法，解决了现有方法在处理短文本、噪声和时间演化方面的局限性，为社交媒体分析提供了更好的结构化表示。
...
### [138] [TAB-AUDIT: Detecting AI-Fabricated Scientific Tables via Multi-View Likelihood Mismatch](https://arxiv.org/abs/2603.19712)
...
<details>
  <summary>Details</summary>
Motivation: AI生成的伪造科学手稿对学术诚信构成严重威胁，特别是表格作为实证研究的关键证据。目前缺乏针对AI生成科学表格检测的系统研究，需要开发有效的检测方法。
...
Result: 基于这些特征的RandomForest模型显著优于现有方法，在域内达到0.987 AUROC，域外达到0.883 AUROC。实验表格成为检测AI生成科学欺诈的关键法医信号。

Conclusion: 表格作为科学论文的关键证据，提供了检测AI生成伪造的有效信号。TAB-AUDIT框架和FabTab数据集为未来研究提供了新基准，有助于维护学术诚信。
...
### [139] [LoopRPT: Reinforcement Pre-Training for Looped Language Models](https://arxiv.org/abs/2603.19714)
*Guo Tang,Shixin Jiang,Heng Chang,Nuo Chen,Yuhan Li,Huiming Fan,Jia Li,Ming Liu,Bing Qin*

Main category: cs.CL

TL;DR: LoopRPT是一个专门为循环语言模型设计的强化预训练框架，通过将强化信号直接分配给潜在推理步骤，提升中间表示质量，实现更高效的推理压缩。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习范式主要针对输出标记，与循环语言模型的隐式推理结构不匹配，需要专门的方法来直接优化中间表示。

Method: 提出LoopRPT框架，将下一个标记预测重新定义为下一个标记推理任务，使用EMA教师参考和噪声潜在展开，将强化信号直接分配给潜在步骤。

Result: LoopRPT在多个模型规模上一致提升每步表示质量，在准确率-计算权衡方面实现帕累托优势，在困难标记上表现显著提升。
...
### [140] [PoC: Performance-oriented Context Compression for Large Language Models via Performance Prediction](https://arxiv.org/abs/2603.19733)
...
TL;DR: 提出性能导向的上下文压缩(PoC)新范式，用性能下限替代压缩率作为约束，通过轻量级性能预测器自动寻找满足性能要求的最激进压缩比
...
Method: PoC范式：开发者指定可接受的性能下限而非压缩比；使用轻量级性能预测器自动寻找满足约束的最激进压缩比，然后引导现成的压缩器；设计了两种预测器变体：简单的上下文无关预测器和更复杂的上下文感知预测器（考虑输入固有可压缩性）

Result: 在问答和摘要基准测试中，上下文感知预测器始终比上下文无关预测器获得更低的性能预测误差；由此产生的上下文感知PoC实现了更优的整体性能

Conclusion: 这项工作为大语言模型上下文压缩的更可靠、高效和性能感知部署铺平了道路
...
### [141] [Rethinking Ground Truth: A Case Study on Human Label Variation in MLLM Benchmarking](https://arxiv.org/abs/2603.19744)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLM）基准测试中，人类标注者之间的系统性差异（HLV）被忽视，大多数基准仅使用共识标签，无法反映真实世界中标注的主观性和模糊性，可能导致对模型能力的过高估计。

Method: 提出了一种新的MLLM评估协议，明确考虑两种条件：（1）人类标注者高一致性标注；（2）人类标注者高分歧标注。使用社交媒体内容分类数据集的非聚合人类标注，对Gemma 3和Qwen 2.5 VL等最先进的MLLM家族进行评估。

Result: 研究发现：1）较大模型在高一致性标注子集上表现最佳；2）但在高分歧标注情况下，较大模型往往表现不如中等规模模型；3）参数数量本身并不能决定模型对模糊性和主观性的敏感性；4）仅基于共识标签的基准会高估模型在内容审核等主观领域的实际能力。

Conclusion: 在MLLM基准测试中纳入人类标注者差异能够提供更真实、更稳健的模型能力评估，特别是在内容审核等主观性强的应用场景中。模型大小不是处理模糊性的唯一决定因素，评估协议需要考虑标注的主观性维度。
...
### [142] [Neither Here Nor There: Cross-Lingual Representation Dynamics of Code-Mixed Text in Multilingual Encoders](https://arxiv.org/abs/2603.19771)
*Debajyoti Mazumder,Divyansh Pathak,Prashant Kodali,Jasabanta Patro*

Main category: cs.CL

TL;DR: 该研究探讨了多语言编码器模型如何内部表示代码混合文本，发现标准模型对英语和印地语对齐良好，但代码混合输入与两种语言连接松散。通过后训练对齐目标，可以同时将代码混合表示拉近到两种构成语言，提升跨语言理解和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 虽然多语言编码器模型广泛用于代码混合分析任务，但我们对它们如何内部表示代码混合输入知之甚少，也不清楚这些表示是否与混合的构成语言有意义的连接。研究旨在理解模型对代码混合文本的内部表示机制及其与构成语言的关系。

Method: 使用印地语-英语作为案例研究，构建了包含英语、印地语（天城文）和罗马化代码混合句子的统一三语平行语料库。通过CKA（中心核对齐）、词元级显著性分析和基于熵的不确定性分析，探究标准多语言编码器及其代码混合适应变体之间的跨语言表示对齐。

Result: 研究发现：1）标准模型对英语和印地语对齐良好，但代码混合输入与两种语言连接松散；2）在代码混合数据上继续预训练会改善英语-代码混合对齐，但以牺牲英语-印地语对齐为代价；3）可解释性分析显示明显的不对称性：模型通过英语主导的语义子空间处理代码混合文本，而原生脚本印地语提供补充信号以减少表示不确定性。

Conclusion: 基于这些发现，研究者提出了三语后训练对齐目标，能够同时将代码混合表示拉近到两种构成语言，实现更平衡的跨语言对齐，并在情感分析和仇恨言论检测等下游任务中获得性能提升。这表明将代码混合表示锚定在其构成语言中，对跨语言理解有重要意义。
...
### [143] [Semantic Delta: An Interpretable Signal Differentiating Human and LLMs Dialogue](https://arxiv.org/abs/2603.19849)
*Riccardo Scantamburlo,Mauro Mezzanzana,Giacomo Buonanno,Francesco Bertolotti*

Main category: cs.CL

TL;DR: 论文提出了一种基于语义类别分布差异的轻量级指标"语义delta"，用于区分人类对话和LLM生成文本，发现AI文本在主题集中度上更高，而人类对话语义分布更广泛平衡。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM是否真的像人类一样对话，这在教育和学术等多个领域都具有重要意义。作者希望开发一种可解释的统计特征来区分人类书写和LLM生成的对话。

Method: 方法包括：1）使用Empath词汇分析框架将文本映射到主题强度分数；2）定义"语义delta"作为对话中两个最主要类别强度之间的差异；3）从多种LLM配置生成对话数据，并与异质人类语料（包括剧本对话、文学作品和在线讨论）进行比较；4）对语义delta值分布应用Welch t-test进行统计分析。

Result: 结果显示：AI生成的文本始终产生比人类文本更高的delta值，表明AI文本具有更刚性的主题结构；而人类对话显示出更广泛、更平衡的语义分布。该指标能够有效区分人类和AI生成的对话。

Conclusion: 结论是提出的零样本指标为现有检测技术提供了计算成本低廉的补充信号，可集成到集成检测系统中。这些发现有助于更广泛地理解LLM行为模仿，并表明主题分布是当前模型在人类对话动态方面存在不足的一个可量化维度。
...
### [144] [Span-Level Machine Translation Meta-Evaluation](https://arxiv.org/abs/2603.19921)
*Stefano Perrella,Eric Morales Agostinho,Hugo Zaragoza*

Main category: cs.CL

TL;DR: 该论文研究了机器翻译错误检测自动评估方法的可靠性问题，提出了一种新的元评估策略MPP，并用于评估当前最先进的MT错误检测技术。


<details>
  <summary>Details</summary>
Motivation: 机器翻译和自动评估技术近年来显著进步，自动评估技术已从产生标量质量分数发展到精确定位翻译错误并分配错误类别和严重程度。然而，对于执行错误检测的自动评估器，如何可靠地衡量其评估能力仍不明确，因为文献中缺乏既定的技术。

Method: 研究了跨度级别精确率、召回率和F分数的不同实现方式，发现看似相似的方法会产生显著不同的排名，某些广泛使用的技术不适合评估MT错误检测。提出了"带部分重叠和部分信用的匹配"（MPP）结合微平均作为稳健的元评估策略，并公开发布了相关代码。

Result: 研究表明不同实现方法会导致评估结果排名差异显著，某些常用技术不适合MT错误检测评估。提出的MPP方法被证明是稳健的元评估策略，并成功用于评估当前最先进的MT错误检测技术。

Conclusion: 需要可靠的元评估方法来衡量MT错误检测自动评估器的能力，提出的MPP方法为这一领域提供了稳健的评估框架，有助于更准确地评估当前最先进的MT错误检测技术。
...
### [145] [Translation from the Information Bottleneck Perspective: an Efficiency Analysis of Spatial Prepositions in Bitexts](https://arxiv.org/abs/2603.19924)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言系统在信息瓶颈框架下的效率优化。虽然该框架已在视觉领域（如颜色和运动）得到验证，但在语言刺激（如句子上下文中的词语）方面尚未研究。翻译作为压缩源语言意义的过程，为研究语言交际效率提供了理想窗口。

Method: 方法包括：1）将翻译建模为信息瓶颈优化问题，源语句作为刺激，目标语句作为压缩意义；2）分析英语、德语和塞尔维亚语对法语小说的翻译；3）通过堆排序实验（N=35）获取介词相似性判断；4）训练低秩投影模型（D=5）预测相似性判断；5）比较实际翻译与反事实替代方案在信息瓶颈最优边界上的位置。

Result: 结果发现：1）低秩投影模型能有效预测介词相似性判断（Spearman相关系数：0.78）；2）实际翻译的介词比反事实替代方案更接近信息瓶颈最优边界；3）这为人类翻译在空间领域存在交际效率压力提供了初步证据。

Conclusion: 结论表明翻译可以作为研究跨语言语义系统认知效率压力的窗口。人类翻译表现出信息瓶颈框架预测的交际效率优化特征，特别是在空间介词领域。这扩展了信息瓶颈理论在语言处理中的应用，为理解语言系统的效率约束提供了新视角。
...
### [146] [SAGE: Sustainable Agent-Guided Expert-tuning for Culturally Attuned Translation in Low-Resource Southeast Asia](https://arxiv.org/abs/2603.19931)
*Zhixiang Lu,Chong Zhang,Yulong Li,Angelos Stefanidis,Anh Nguyen,Imran Razzak,Jionglong Su,Zhengyong Jiang*

Main category: cs.CL

TL;DR: SAGE框架通过强化学习智能体筛选高质量文化相关数据，大幅减少97.1%数据使用和95.2%训练能耗，在东南亚七种低资源语言翻译任务中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决全球数字鸿沟问题，特别是东南亚低资源地区的语言障碍。传统大语言模型在数据稀缺、文化相关数据不足且训练能耗巨大的双重挑战下难以有效部署。

Method: 提出可持续智能体引导专家调优(SAGE)框架：1) 使用强化学习智能体通过GRPO优化策略，基于专家构建的社区对话语义奖励信号自主筛选高质量训练数据；2) 在筛选后的精简数据集上使用LoRA高效微调开源大语言模型。

Result: 在英语与七种东南亚低资源语言翻译任务中：1) BLEU-4和COMET-22指标达到新的SOTA性能；2) 数据使用减少97.1%；3) 训练能耗降低95.2%；4) 优于使用完整数据集的基线模型。

Conclusion: SAGE框架通过"正确数据而非大数据"的能源感知范式，在保持高性能的同时大幅降低环境足迹，为全球南方数字鸿沟问题提供了可扩展且负责任的技术路径。
...
### [147] [Hybrid topic modelling for computational close reading: Mapping narrative themes in Pushkin's Evgenij Onegin](https://arxiv.org/abs/2603.19940)
*Angelo Maria Sabatini*

Main category: cs.CL

TL;DR: 本研究提出了一种用于计算文学分析的混合主题建模框架，将LDA与sPLS-DA结合，分析叙事诗歌的主题结构和纵向动态，以《叶甫盖尼·奥涅金》意大利译本为案例，展示了轻量概率模型如何生成可复现的主题地图。


<details>
  <summary>Details</summary>
Motivation: 传统文学分析中，如何在小语料库环境下结合无监督和监督词汇结构来分析复杂诗歌叙事的主题动态是一个挑战。本研究旨在开发一个计算框架，既能捕捉主题结构，又能分析纵向叙事动态。

Method: 采用混合主题建模框架：1) 使用LDA进行无监督主题建模；2) 结合稀疏偏最小二乘判别分析(sPLS-DA)作为监督探针；3) 针对小语料库不稳定性采用多种子共识协议；4) 将诗歌文本分割为35个文档并词形还原；5) 引入"叙事中心"概念扩展词袋方法到叙事层面。

Result: 从文本中提取出5个稳定且可解释的主题；sPLS-DA增强了主题可解释性，识别出精炼每个主题的词汇标记；叙事中心分析揭示了主题混合如何与诗歌情感和结构弧线对齐；框架提供了可复现的主题地图。

Conclusion: 该框架不是取代传统文学解释，而是提供了一种计算形式的细读方法，展示了轻量概率模型如何生成复杂诗歌叙事的可复现主题地图，即使抽象了韵律、音韵或原生形态等文体特征。该方法为比较研究中的其他高密度文学文本提供了透明的方法模板。
...
### [148] [When Contextual Inference Fails: Cancelability in Interactive Instruction Following](https://arxiv.org/abs/2603.19997)
*Natalia Bila,Kata Naszádi,Alexandra Mayn,Christof Monz*

Main category: cs.CL

TL;DR: 论文研究了在协作积木搭建任务中，模型如何分离字面解释与上下文推理，发现LLMs在判断和行动之间存在分离：虽然能检测说话者不可靠性，但无法有效利用该信息指导澄清行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言模型在协作任务中如何处理歧义指令，特别是如何分离字面解释与上下文推理。现有研究主要关注模型是否能检测说话者可靠性，但缺乏对模型如何利用这种信息指导实际交互行为的研究。

Method: 方法包括：1）基于现有心理语言学范式开发Build What I Mean（BWIM）交互基准；2）对比实用合作型说话者与仅字面可靠型说话者；3）要求模型通过上下文推理或支付小成本请求澄清来解决歧义；4）评估多个先进LLMs在判断和行动上的表现。

Result: 研究发现：1）模型在显式置信度评分中能检测说话者不可靠性；2）但无法有效利用这一信息指导澄清行为；3）观察到次优策略，包括无视伙伴的过度澄清和在不确定性下的回避提问猜测行为。

Conclusion: 结论是当前LLMs在判断和行动能力之间存在分离，模型虽然具备检测说话者可靠性的能力，但缺乏将这种元认知判断转化为有效沟通策略的能力，这限制了它们在真实世界协作任务中的表现。
...
### [149] [An Agentic Approach to Generating XAI-Narratives](https://arxiv.org/abs/2603.20003)
*Yifan He,David Martens*

Main category: cs.CL

TL;DR: 提出一个基于多智能体框架的XAI叙事生成与优化系统，通过Narrator生成叙事，多个Critic Agents评估忠实性和连贯性，经过迭代改进生成更可信、可理解的解释性AI叙事。


<details>
  <summary>Details</summary>
Motivation: 现有XAI方法过于技术化和专家导向，缺乏可解释性和可访问性。LLM生成的XAI叙事虽然有望将后验解释转化为自然语言，但需要确保其忠实性和连贯性。

Method: 提出多智能体框架，包含Narrator（生成和修订叙事）和多个Critic Agents（评估忠实性和连贯性指标）。设计了五种智能体系统：基础设计、批评设计、批评规则设计、连贯设计、连贯规则设计，并在五个LLM和五个表格数据集上进行系统评估。

Result: 基础设计、批评设计和批评规则设计能有效提高所有LLM叙事的忠实性。Claude-4.5-Sonnet在基础设计上表现最佳，经过三轮迭代后不忠实叙事减少90%。基于多数投票的集成策略能持续提升四个LLM的性能（DeepSeek-V3.2-Exp除外）。

Conclusion: 智能体系统有潜力生成忠实且连贯的XAI叙事，多智能体框架通过迭代反馈机制能显著改善解释性AI叙事的质量。
...
### [150] [LoASR-Bench: Evaluating Large Speech Language Models on Low-Resource Automatic Speech Recognition Across Language Families](https://arxiv.org/abs/2603.20042)
*Jianan Chen,Xiaoxue Gao,Tatsuya Kawahara,Nancy F. Chen*

Main category: cs.CL

TL;DR: LoASR-Bench是一个专门评估语音语言模型在低资源语言上自动语音识别性能的基准测试，涵盖25种语言、9个语系，包含拉丁和非拉丁文字，揭示了当前SpeechLMs在处理真实低资源语言时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有语音语言模型基准主要关注高资源语言，对低资源语言的ASR性能评估不足。由于实际ASR系统需要可靠支持低资源语言并跨不同语系泛化，这一空白直接阻碍了SpeechLM-based ASR在多语言场景中的实际部署。

Method: 提出LoASR-Bench基准测试，包含25种来自9个语系的语言，涵盖拉丁和非拉丁文字，用于跨语言和跨文字的ASR性能评估，系统测试当前最新语音语言模型在低资源语言上的表现。

Result: 实验结果表明，当前最新的语音语言模型在处理真实世界低资源语言时存在明显局限性，揭示了现有模型在低资源语言ASR任务上的不足。

Conclusion: 需要专门针对低资源语言的评估基准来确保语音语言模型在不同语系间的泛化能力，LoASR-Bench填补了这一空白，并为未来SpeechLMs在低资源语言ASR方面的改进提供了重要参考。
...
### [151] [Predicting States of Understanding in Explanatory Interactions Using Cognitive Load-Related Linguistic Cues](https://arxiv.org/abs/2603.20079)
*Yu Wang,Olcay Türk,Angela Grimminger,Hendrik Buschmeier*

Main category: cs.CL

TL;DR: 研究通过言语和非言语语言特征预测对话中听众理解状态，发现信息价值、句法复杂度和注视行为变化等线索能有效预测四种理解状态


<details>
  <summary>Details</summary>
Motivation: 探索如何利用说话者和听众在对话中表现出的言语和非言语语言特征，实时预测听众在解释性互动中的理解状态

Method: 基于MUNDEX面对面棋盘游戏解释对话语料库，分析三种语言线索：说话者话语的信息价值（通过惊奇值衡量）、句法复杂度，以及听众互动注视行为的变化。使用回顾性视频回忆方法让听众自我标注理解状态，并进行分类实验

Result: 个体线索与听众理解水平相关，分类实验表明预测四种理解状态（理解、部分理解、不理解、误解）是可行的，当三种语言线索与文本特征结合时预测效果更好

Conclusion: 言语和非言语语言特征能有效预测对话中听众的理解状态，为实时理解监测和适应性对话系统提供了基础
...
### [152] [Current LLMs still cannot 'talk much' about grammar modules: Evidence from syntax](https://arxiv.org/abs/2603.20114)
*Mohammed Q. Shormani*

Main category: cs.CL

TL;DR: 该研究评估了大型语言模型（如ChatGPT-5）在翻译生成句法学术语方面的能力，发现仅有25%的翻译准确，38.6%不准确，36.4%部分正确，表明LLMs在语法模块理解上仍有局限。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验大型语言模型（LLMs）对语法模块的理解深度，特别是其在翻译生成句法核心术语方面的能力，以评估LLMs在语言学领域的实际应用潜力。

Method: 从生成句法学的书籍和期刊文章中收集44个术语，先由人工翻译，再由ChatGPT-5翻译，然后采用分析和比较方法对两种翻译进行对比评估。

Result: 研究发现ChatGPT-5的翻译准确率仅为25%，不准确率为38.6%，部分正确率为36.4%，表明LLMs在处理句法和语义挑战方面仍存在显著局限。

Conclusion: LLMs在语法模块理解上仍无法"深入讨论"，需要AI专家与语言学家的紧密合作来改进LLMs的工作机制，以提高翻译准确性或至少达到适当水平。
...
### [153] [Reasoning Gets Harder for LLMs Inside A Dialogue](https://arxiv.org/abs/2603.20133)
*Ivan Kartáč,Mateusz Lango,Ondřej Dušek*

Main category: cs.CL

TL;DR: 论文提出BOULDER基准测试，发现LLMs在孤立任务和对话环境下的推理性能存在显著差距，对话的多轮特性是主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在推理基准测试中表现良好，但这些评估通常关注孤立任务，与任务导向对话(TOD)中的真实使用场景存在差异。在TOD环境中，LLMs需要在生成文本时进行推理，并遵循角色、格式和风格指令。这种不匹配引发了对基准测试性能是否能准确反映LLMs在TOD环境中推理鲁棒性的担忧。

Method: 引入BOULDER动态基准测试，涵盖八个旅行相关任务，需要算术、空间和时间推理，包含常识和正式方面。每个问题都提供孤立版本和基于对话的变体，实现受控比较并减轻数据污染。对八个LLMs进行实验，通过消融研究和定性分析探究性能差距的原因。

Result: 实验显示，在孤立设置和对话设置之间存在显著且一致的性能差距。通过消融研究发现，这种差距主要由对话的多轮特性驱动，角色条件和工具使用需求也有额外影响。

Conclusion: 研究结果强调了在现实交互场景中评估LLM推理能力的必要性，表明当前基准测试可能无法准确反映LLMs在任务导向对话环境中的实际推理性能。
...
### [154] [Enhancing Hyperspace Analogue to Language (HAL) Representations via Attention-Based Pooling for Text Classification](https://arxiv.org/abs/2603.20149)
*Ali Sakour,Zoalfekar Sakour*

Main category: cs.CL

TL;DR: 该论文提出在HAL模型中引入可学习的温度缩放注意力机制，替代传统的平均池化方法，以提升句子级语义表示的质量，在IMDB情感分析任务上取得了显著性能提升。
...
Method: 1. 对原始高维稀疏的共现矩阵使用截断SVD投影到密集潜在空间；2. 集成可学习的温度缩放加性注意力机制替代平均池化；3. 在IMDB情感分析数据集上评估提出的架构。

Result: 注意力池化方法在IMDB测试集上达到82.38%的准确率，相比传统平均池化基线（75.64%）绝对提升6.74个百分点。注意力权重分析显示机制成功抑制停用词并选择性关注情感相关词。

Conclusion: 提出的注意力增强HAL模型不仅提升了分类性能，还提高了模型可解释性，注意力机制能够有效识别和加权情感相关词汇，为基于共现矩阵的语义表示提供了改进方向。
...
### [155] [Semantic Token Clustering for Efficient Uncertainty Quantification in Large Language Models](https://arxiv.org/abs/2603.20161)
*Qi Cao,Andrew Gambardella,Takeshi Kojima,Yutaka Matsuo,Yusuke Iwasawa*

Main category: cs.CL

TL;DR: 提出了一种基于语义令牌聚类的高效不确定性量化方法STC，通过单次生成即可评估LLM输出的可靠性，显著降低计算开销。
...
Method: 提出语义令牌聚类方法，利用LLM内部编码的语义信息，通过嵌入聚类和前缀匹配将令牌分组到语义一致的簇中，基于对应语义簇的概率质量来量化不确定性。

Result: 实验结果表明，STC在性能上与最先进的基线方法相当，同时显著降低了计算开销，仅需单次生成且不依赖辅助模型。

Conclusion: STC为LLM不确定性量化提供了一种高效实用的解决方案，在保持性能的同时大幅减少计算成本，有助于提高LLM输出的可靠性评估。
...
### [156] [Evaluating Evidence Grounding Under User Pressure in Instruction-Tuned Language Models](https://arxiv.org/abs/2603.20162)
*Sai Koneru,Elphin Joe,Christine Kirchhoff,Jian Wu,Sarah Rajtmajer*

Main category: cs.CL

TL;DR: 研究评估了指令调优语言模型在争议领域平衡用户对齐与证据忠实度的能力，发现在固定证据设置下，更丰富的上下文证据无法可靠防止用户压力导致的模型反转


<details>
  <summary>Details</summary>
Motivation: 在争议领域，指令调优语言模型需要在用户对齐压力和上下文证据忠实度之间取得平衡。研究者希望评估这种紧张关系，了解模型在面临用户压力时如何保持对证据的忠实度

Method: 基于美国国家气候评估构建了受控认知冲突框架，对19个参数规模从0.27B到32B的指令调优模型进行了精细消融实验，分析证据构成和不确定性线索的影响

Result: 在无压力提示下，更丰富的证据能提高证据一致性和序数评分性能；但在用户压力下，证据无法可靠防止用户对齐反转。识别了三种主要失败模式：负面部分证据交互、非单调鲁棒性缩放、冲突下的分布集中度差异

Conclusion: 在受控固定证据设置中，仅提供更丰富的上下文证据无法保证抵抗用户压力，需要明确的认知完整性训练来确保模型在争议领域保持证据忠实度
...
### [157] [Measuring Faithfulness Depends on How You Measure: Classifier Sensitivity in LLM Chain-of-Thought Evaluation](https://arxiv.org/abs/2603.20172)
*Richard J. Young*

Main category: cs.CL

TL;DR: 链式思维忠实度评估存在严重的方法依赖性问题，不同分类器对同一数据得出显著不同的忠实度评分，导致模型排名反转，表明单一评估指标不可靠。


<details>
  <summary>Details</summary>
Motivation: 当前链式思维忠实度研究通常报告单一聚合数字（如模型在39%的情况下承认提示），暗示忠实度是模型可测量的客观属性。本文旨在证明这种假设是错误的，展示不同分类器对同一数据会产生显著不同的评估结果。

Method: 使用三种不同的分类器（纯正则表达式检测器、两阶段正则表达式+LLM管道、独立的Claude Sonnet 4评估器）对12个开放权重模型的10,276个受影响的推理轨迹进行分析。这些模型涵盖9个家族，参数规模从7B到1T。通过统计测试（McNemar检验）和一致性度量（Cohen's kappa）来量化分类器之间的差异。

Result: 三种分类器对相同数据得出的总体忠实度分别为74.4%、82.6%和69.7%，置信区间不重叠。每个模型的忠实度差距在2.6到30.6个百分点之间，所有差异都具有统计显著性。分类器间一致性很低（kappa值从0.06到0.42），且存在系统性不对称。分类器选择甚至能反转模型排名：Qwen3.5-27B在管道分类器中排名第一，但在Sonnet评估器中排名第七；OLMo-3.1-32B则从第九名升至第三名。

Conclusion: 链式思维忠实度评估具有强烈的分类器依赖性，不同分类器对"忠实度"这一概念的操作化定义不同（词汇提及vs认知依赖），导致对相同行为产生分歧评估。已发表的忠实度数字在使用不同分类器的研究间无法进行有意义的比较。未来评估应报告跨多种分类方法的敏感性范围，而非单一的点估计。
...
### [158] [When both Grounding and not Grounding are Bad -- A Partially Grounded Encoding of Planning into SAT (Extended Version)](https://arxiv.org/abs/2603.19429)
*João Filipe,Gregor Behnke*

Main category: cs.AI

TL;DR: 本文提出了一种介于完全提升和完全接地之间的规划方法，通过三种SAT编码保持动作提升的同时部分接地谓词，相比之前二次复杂度的编码实现了线性扩展，在难以接地的领域上优于现有最优长度规划方法。


<details>
  <summary>Details</summary>
Motivation: 经典规划问题通常使用提升的一阶表示定义，虽然紧凑通用，但大多数规划器会将其接地以简化推理，这可能导致规模指数级增长。最近的方法直接在提升层面操作以避免完全接地，但需要在完全提升和完全接地之间寻找平衡。

Method: 提出了三种SAT编码方法，保持动作提升的同时部分接地谓词，实现了线性扩展（而非之前方法的二次扩展），从而能够处理更长的规划序列。

Result: 实验结果表明，最佳编码在难以接地的领域上优于现有最优长度规划方法，特别是在处理较长规划时表现更优。

Conclusion: 在完全提升和完全接地规划之间找到了有效的中间路径，通过部分接地谓词同时保持动作提升的SAT编码方法，实现了线性扩展并提升了在硬接地领域上的规划性能。
...
TL;DR: 论文提出通过微调大语言模型来生成反例，填补了当前AI数学推理中只关注证明构造而忽视反例发现的空白。该方法要求模型不仅提出候选反例，还要生成可在Lean 4定理证明器中自动验证的形式化证明。
...
Method: 1. 将任务形式化为形式化反例生成，要求LLMs提出候选反例并生成可在Lean 4中验证的形式化证明；2. 引入符号突变策略，通过系统提取定理并丢弃选定假设来合成多样化训练数据；3. 结合策划的数据集，采用多奖励专家迭代框架来提升训练效果和效率。

Result: 在三个新收集的基准测试上的实验验证了方法的优势，显示突变策略和训练框架带来了显著的性能提升。模型在反例生成和定理证明方面都表现出更好的效果。

Conclusion: 该方法成功填补了AI数学推理中反例生成的空白，通过符号突变策略和多奖励专家迭代框架有效提升了LLMs在反例发现和定理证明方面的能力，为更全面的数学推理AI系统提供了新思路。
...
TL;DR: ItinBench是一个结合空间推理（路线优化）和传统语言推理任务的旅行行程规划基准，用于评估LLMs在多个认知维度上的综合表现。


<details>
  <summary>Details</summary>
Motivation: 传统LLM评估主要关注特定推理或规划任务，缺乏整合多个认知领域的综合测试平台。旅行规划作为现实场景，需要整合多种认知能力，但现有研究主要关注语言推理，忽略了空间推理等其他认知维度。

Method: 提出ItinBench基准，在旅行行程规划中引入空间推理任务（路线优化），同时保留传统语言推理任务。评估了Llama 3.1 8B、Mistral Large、Gemini 1.5 Pro和GPT系列等多个LLMs在这些综合任务上的表现。

Result: 研究发现LLMs在处理多个认知维度时难以保持高且一致的表现。当需要同时处理空间推理和语言推理任务时，模型性能会出现显著波动。

Conclusion: 通过整合不同人类认知领域的任务，ItinBench为构建更全面的推理测试平台提供了新见解，更好地反映了现实世界的挑战。这表明需要开发能够处理多维度认知任务的更强大LLMs。
...
### [161] [PA2D-MORL: Pareto Ascent Directional Decomposition based Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.19579)
...
Method: 提出PA2D-MORL方法：1) 利用帕累托上升方向选择标量化权重并计算多目标策略梯度；2) 在进化框架下选择性优化多个策略从不同方向逼近帕累托前沿；3) 应用帕累托自适应微调方法增强前沿近似的密度和分布。
...
TL;DR: PowerLens使用LLM进行个性化移动电源管理，通过多智能体架构识别用户上下文并生成电源策略，实现零样本、上下文感知的个性化电源管理，在Android设备上节省38.8%能耗。


<details>
  <summary>Details</summary>
Motivation: 移动设备电池寿命是关键挑战，现有电源管理机制依赖静态规则或粗粒度启发式方法，忽略了用户活动和个性化偏好，需要更智能的解决方案。

Method: 采用大型语言模型的常识推理能力，通过多智能体架构从UI语义识别用户上下文，生成18个设备参数的电源策略。使用PDL约束框架验证每个操作，通过双层记忆系统从用户隐式反馈中学习个性化偏好。

Result: 在rooted Android设备上，PowerLens实现81.7%的操作准确率和38.8%的能耗节省（相比原生Android），优于基于规则和LLM的基线方法，用户满意度高，偏好收敛快（3-5天），系统自身仅消耗0.5%的日电池容量。

Conclusion: PowerLens展示了LLM在移动电源管理中的潜力，通过语义理解用户活动、生成个性化策略并确保安全执行，为智能设备电源管理提供了新方向。
...
### [163] [HyEvo: Self-Evolving Hybrid Agentic Workflows for Efficient Reasoning](https://arxiv.org/abs/2603.19639)
...
Method: HyEvo采用异构原子合成方法，将概率性LLM节点用于语义推理，确定性代码节点用于基于规则的执行。框架使用LLM驱动的多岛进化策略和"反思-生成"机制，通过执行反馈迭代优化工作流拓扑和节点逻辑。

Result: 在多样化的推理和编码基准测试中，HyEvo始终优于现有方法，与最先进的开源基线相比，推理成本降低高达19倍，执行延迟降低高达16倍。

Conclusion: HyEvo通过异构原子合成和进化优化策略，有效解决了现有自动化工作流生成方法的局限性，显著提升了复杂任务解决的效率和性能。
...
### [164] [A Subgoal-driven Framework for Improving Long-Horizon LLM Agents](https://arxiv.org/abs/2603.19685)
*Taiyi Wang,Sian Gooding,Florian Hartmann,Oriana Riva,Edward Grefenstette*

Main category: cs.AI

TL;DR: 提出MIRA框架，通过子目标分解和里程碑奖励解决LLM智能体在长序列任务中的规划问题，显著提升Web导航性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在长序列任务中面临两大挑战：在线执行时难以跟踪新信息并保持清晰路径；RL微调时稀疏延迟奖励难以识别成功动作，无法维持连贯推理

Method: 提出两个贡献：1）利用专有模型进行子目标分解的在线规划框架；2）MIRA（Milestoning your Reinforcement Learning Enhanced Agent）训练框架，使用密集的里程碑奖励信号

Result: 实时规划机制使Gemini在WebArena-Lite基准上成功率提升约10%；MIRA应用于Gemma3-12B模型将成功率从6.4%提升至43.0%，超越GPT-4-Turbo（17.6%）、GPT-4o（13.9%）和之前的SOTA WebRL（38.4%）

Conclusion: 显式推理时规划与里程碑奖励相结合显著提升智能体的长序列能力，为更鲁棒和通用的自主系统铺平道路
...
### [165] [Stepwise: Neuro-Symbolic Proof Search for Automated Systems Verification](https://arxiv.org/abs/2603.19715)
*Baoding He,Zenan Li,Wei Sun,Yuan Yao,Taolue Chen,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: 本文提出了一种神经符号证明生成框架，结合大语言模型和交互式定理证明工具，通过最佳优先树搜索自动生成系统级验证项目的证明，在seL4基准测试中证明了77.6%的定理。


<details>
  <summary>Details</summary>
Motivation: 形式化验证通过交互式定理证明确保关键系统的正确性，但构建大型证明脚本高度手动且限制了可扩展性。大语言模型在数学推理方面的进步使其集成到软件验证中具有前景。

Method: 提出神经符号证明生成框架，通过最佳优先树搜索遍历证明状态，重复查询LLM获取候选证明步骤。神经侧微调LLM使用证明状态-步骤对数据集；符号侧集成ITP工具修复被拒绝步骤、过滤排序证明状态、在搜索停滞时自动处理子目标。

Result: 在seL4基准测试中，系统证明了77.6%的定理，显著超越先前基于LLM的方法和独立Sledgehammer，解决了更多多步证明。在其他基准测试中也展示了强大的泛化能力。

Conclusion: 该框架展示了神经符号方法在自动软件验证中的有效性，为可扩展的自动化软件验证提供了可行路径。
...
### [166] [Embodied Science: Closing the Discovery Loop with Agentic Embodied AI](https://arxiv.org/abs/2603.19782)
...
TL;DR: 论文提出"具身科学"新范式，将科学发现重新定义为智能体推理与物理执行紧密耦合的闭环过程，并提出统一的PLAD框架


<details>
  <summary>Details</summary>
Motivation: 当前AI在预测科学属性方面表现出色，但科学发现本质上是物理的、长期的过程，受实验周期支配。现有计算方法大多将发现视为孤立的、特定任务的预测，而非与物理世界的持续互动，存在与现实脱节的问题
...
### [167] [FormalEvolve: Neuro-Symbolic Evolutionary Search for Diverse and Prover-Effective Autoformalization](https://arxiv.org/abs/2603.19828)
...
<details>
  <summary>Details</summary>
Motivation: 自动形式化虽然能将自然语言数学转化为可编译、机器可检查的语句，但语义一致性并不保证证明器有效性。即使语义一致的形式化在证明搜索成本和成功率上也可能存在显著差异，因此需要寻找既能保持语义一致性又能提高证明效率的形式化方案。

Method: 提出FormalEvolve框架，将自动形式化视为预算约束下的测试时搜索问题。采用神经符号进化方法：1）通过LLM驱动的变异和交叉生成多样化候选；2）使用有界补丁修复；3）引入符号抽象语法树（AST）重写操作增加结构多样性；4）采用编译门控机制确保生成的形式化可编译。

Result: 在CombiBench和ProofNet数据集上，在T=100的生成器调用预算下，FormalEvolve达到58.0%和84.9%的语义命中率（SH@100），降低了语义成功案例的跨问题集中度（更低的基尼系数）。在固定证明器预算下，在CombiBench上显著提升了下游证明性能。

Conclusion: FormalEvolve通过神经符号进化框架有效解决了自动形式化中的证明效率问题，在有限预算下实现了高语义命中率和更好的下游证明性能，为自动形式化提供了新的解决方案。
...
### [168] [Utility-Guided Agent Orchestration for Efficient LLM Tool Use](https://arxiv.org/abs/2603.19896)
*Boyan Liu,Gongming Zhao,Hongli Xu*

Main category: cs.AI

TL;DR: 提出一种基于效用指导的LLM智能体编排策略，通过平衡估计收益、步骤成本、不确定性和冗余性来优化工具使用中的质量-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有工具使用LLM智能体面临质量与执行成本之间的基本矛盾：固定工作流程稳定但不灵活，而自由形式的多步推理方法（如ReAct）可能提高任务性能，但会带来过多的工具调用、更长的轨迹、更高的token消耗和增加的延迟。

Method: 将智能体编排作为明确的决策问题进行研究，提出效用指导的编排策略，通过平衡估计收益、步骤成本、不确定性和冗余性来选择响应、检索、工具调用、验证和停止等动作。

Result: 实验表明，明确的编排信号显著影响智能体行为，轻量级的效用设计为智能体控制提供了可辩护且实用的机制，能够有效管理质量-成本权衡。

Conclusion: 通过将智能体编排作为明确的决策问题，并提供可控可分析的策略框架，可以更好地研究工具使用LLM智能体中的质量-成本权衡，为智能体控制提供实用机制。
...
### [169] [On the Ability of Transformers to Verify Plans](https://arxiv.org/abs/2603.19954)
*Yash Sarrof,Yupei Du,Katharina Stein,Alexander Koller,Sylvie Thiébaux,Michael Hahn*

Main category: cs.AI

TL;DR: 论文分析了Transformer在AI规划任务中的泛化能力，特别是解码器模型验证给定规划是否正确的理论保证


<details>
  <summary>Details</summary>
Motivation: Transformer在AI规划任务中表现不一致，缺乏理论理解何时能期望泛化，需要填补这一理论空白

Method: 引入C*-RASP扩展C-RASP，分析解码器模型在对象数量增长（词汇表大小增加）情况下的泛化能力，识别可验证规划的规划域类别

Result: 识别出Transformer可证明学习验证长规划的大类经典规划域，发现显著影响长度泛化解可学习性的结构特性，实验验证了理论

Conclusion: 为Transformer在AI规划任务中的泛化能力提供了理论分析框架，识别了可验证规划的规划域类别和影响泛化的结构特性
...
### [170] [Experience is the Best Teacher: Motivating Effective Exploration in Reinforcement Learning for LLMs](https://arxiv.org/abs/2603.20046)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于评分标准的强化学习方法虽然能提升大语言模型的推理能力，但存在探索效率低的问题，探索局限于当前策略分布。作者认为有效的探索应该与期望的目标对齐，而强化学习优化本质上是将策略导向最大化奖励的理想分布。

Method: HeRL将失败轨迹及其未满足的评分标准作为后见经验，作为上下文指导，引导策略探索超出当前分布的期望响应。同时引入奖励加成机制，激励具有更大改进潜力的响应。该框架使模型能够从高质量样本中有效学习，无需从头进行反复试错。

Result: 在多个基准测试上的广泛实验表明，HeRL相比基线方法取得了显著的性能提升，并且能够在测试时进一步受益于经验指导的自我改进。

Conclusion: HeRL通过后见经验指导强化学习，有效解决了探索效率问题，实现了更准确的期望梯度估计，为大语言模型的推理能力提升提供了新方法。
...
### [171] [DIAL-KG: Schema-Free Incremental Knowledge Graph Construction via Dynamic Schema Induction and Evolution-Intent Assessment](https://arxiv.org/abs/2603.20059)
*Weidong Bao,Yilin Wang,Ruyu Gao,Fangling Leng,Yubin Bao,Ge Yu*

Main category: cs.AI

TL;DR: DIAL-KG是一个用于增量知识图谱构建的闭环框架，通过元知识库协调，采用三阶段循环工作流程解决传统静态构建方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建方法主要是静态的，依赖于从固定语料库和预定义模式进行单步构建。这种方法在现实动态数据场景中效果不佳，因为整合新信息需要完整且计算昂贵的图重构，同时预定义模式限制了知识图谱构建的灵活性。

Method: 提出了DIAL-KG框架，由元知识库协调，采用三阶段循环工作流程：1) 双轨提取 - 默认生成三元组，对复杂知识切换为事件提取以确保知识完整性；2) 治理裁决 - 确保提取事实的保真度和时效性，防止幻觉和知识过时；3) 模式演化 - 从已验证知识中归纳新模式以指导后续构建周期，并将当前轮次知识增量应用到现有知识图谱中。

Result: 大量实验表明，该框架在构建图的质量和归纳模式方面都达到了最先进的性能水平。

Conclusion: DIAL-KG框架通过闭环增量构建方法有效解决了传统静态知识图谱构建的局限性，实现了高质量的知识图谱构建和模式演化。
...
### [172] [Learning Dynamic Belief Graphs for Theory-of-mind Reasoning](https://arxiv.org/abs/2603.20170)
*Ruxiao Chen,Xilei Zhao,Thomas J. Cova,Frank A. Drews,Susu Xu*

Main category: cs.AI

TL;DR: 本文提出了一种用于大语言模型心理理论推理的结构化认知轨迹模型，通过动态信念图表示心理状态，在灾害疏散等高风险场景中显著提升了行动预测能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么直接提示大语言模型，要么使用将信念视为静态独立的隐状态模型，在动态情境中往往产生不一致的心理模型和弱推理能力，特别是在灾害响应、急诊医学和人机协同等高不确定性高风险场景中。

Method: 引入结构化认知轨迹模型，将心理状态表示为动态信念图，联合推断潜在信念、学习其时变依赖关系，并将信念演化与信息寻求和决策联系起来。包括：1) 从文本化概率陈述到一致概率图模型更新的新颖投影；2) 信念相互依赖性的基于能量的因子图表示；3) 捕捉信念积累和延迟决策的ELBO目标函数。

Result: 在多个真实世界灾害疏散数据集上，该模型显著改善了行动预测能力，并恢复了与人类推理一致的可解释信念轨迹，为在高不确定性环境中增强大语言模型的心理理论能力提供了原则性模块。

Conclusion: 该研究提出了一种结构化认知轨迹模型，能够有效处理动态情境中的心理理论推理问题，为高风险环境下的决策支持系统提供了理论基础和技术实现。
...
### [173] [Beam-aware Kernelized Contextual Bandits for User Association and Beamforming in mmWave Vehicular Networks](https://arxiv.org/abs/2603.19285)
*Xiaoyang He,Manabu Tsukada*

Main category: cs.IT

TL;DR: 提出BKC-UCB算法，利用历史上下文信息估计毫米波信道传输速率，避免频繁信道测量开销


<details>
  <summary>Details</summary>
Motivation: 毫米波通信中频繁估计快速衰落的信道需要显著开销，需要一种能减少信道测量频率的方法

Method: 提出Beam-aware Kernelized Contextual Upper Confidence Bound算法，利用核方法将上下文映射到RKHS空间，将波束索引嵌入上下文，并采用事件触发信息共享机制

Result: 能够在不进行额外信道测量的情况下估计瞬时传输速率，利用波束间相关性加速收敛，通过事件触发机制降低通信开销

Conclusion: BKC-UCB算法有效解决了毫米波车载通信中信道估计开销大的问题，通过上下文学习和信息共享机制提高了学习效率
...
### [174] [Status Updating in Two-Way Delay Systems with Preemption](https://arxiv.org/abs/2603.19410)
*Jinxin Yang,Mohammad Moltafet,Hamid R. Sadjadpour*

Main category: cs.IT

TL;DR: 研究状态更新系统中控制器最优请求生成策略以最小化信息年龄，采用马尔可夫决策过程建模，通过相对值迭代算法求解，证明算法收敛性，数值结果显示优于基线策略并呈现阈值结构


<details>
  <summary>Details</summary>
Motivation: 在具有随机延迟的双向通信状态更新系统中，如何优化控制器生成请求包的时间以最小化信息年龄，解决现有基线策略性能不足的问题

Method: 将系统建模为马尔可夫决策过程，控制器和采样器服务器采用几何服务时间模型，缓冲区采用等待中抢占策略，使用相对值迭代算法求解最优平稳确定性策略

Result: 提出的系统在数值实验中始终优于现有基线策略，最优策略呈现阈值结构，相对值迭代算法被证明收敛

Conclusion: 通过MDP建模和RVI算法可以找到最小化信息年龄的最优请求生成策略，该策略具有阈值特性且性能优于现有方法
...
### [175] [Optimal Scalar Quantization for Matrix Multiplication: Closed-Form Density and Phase Transition](https://arxiv.org/abs/2603.19559)
*Calvin Ang,Sungyoon Kim,Mert Pilanci*

Main category: cs.IT

TL;DR: 该论文研究矩阵乘法前的逐元素标量量化，推导了高分辨率下的最优量化方案，发现了相关高斯乘法对中的相变现象


<details>
  <summary>Details</summary>
Motivation: 矩阵乘法是许多计算任务的核心操作，量化可以显著减少存储和计算成本。然而，现有研究主要关注单个矩阵的量化，而矩阵乘法涉及两个矩阵的交互，需要专门研究如何最优地量化两个矩阵以最小化乘法误差。

Method: 采用逐元素标量量化方法，独立量化两个矩阵A和B的每个元素，使用K_X和K_Y个量化级别。在高分辨率极限下，推导矩阵乘法均方误差的渐近展开，识别最优常数，并通过条件二阶矩表征最优量化中心密度。特别研究了相关高斯乘法对的情况。

Result: 推导出最优量化密度函数λ*(u) ∝ exp(-u²/6)[(1-ρ²)+ρ²u²]^{1/3}，发现相关驱动的相变：当|ρ|≤1/√3时密度函数在原点处单峰，当|ρ|>1/√3时变为双峰，峰值位于u_peak=±√(3-1/ρ²)。在合成实验和LLM激活量化中验证了方法的有效性。

Conclusion: 该研究为矩阵乘法前的双矩阵量化提供了理论框架和最优方案，揭示了相关结构对量化密度函数形态的重要影响，为实际应用中的量化策略提供了理论指导。
...
### [176] [New Constructions of Polar Code Based on Refined Error Probability Analysis](https://arxiv.org/abs/2603.19589)
*Hassan Noghrei,Murad Abdullah*

Main category: cs.IT

TL;DR: 本文对极化码在对称二进制输入离散无记忆信道下的块错误率进行了精细化分析，提出了基于LLR的SC解码BLER新表达式，并开发了针对SC和SCL解码优化的极化码构造算法，相比现有方法获得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有极化码构造方法（如高斯近似法和Tal-Vardy方法）虽然广泛使用，但在SC和SCL解码下的性能仍有优化空间。需要更精确的BLER分析来指导极化码构造，以提升解码性能。

Method: 1. 推导了基于LLR的SC解码BLER新表达式；2. 提出了针对SC解码优化的极化码构造算法；3. 将BLER分解为路径丢失和路径选择分量，推导了基于LLR的路径丢失概率表达式；4. 开发了针对SCL解码优化的极化码构造方法。

Result: 1. SC优化构造在DSCF解码下相比基准方法在AWGN信道上获得最高0.2 dB性能增益；2. 在二进制对称信道上表现更优；3. SCL优化构造在所有考虑的列表大小下都达到可比或改进的性能，最高获得0.4 dB增益。

Conclusion: 提出的基于LLR的BLER分析框架为极化码构造提供了新的理论基础，所开发的SC和SCL优化构造算法相比现有方法（包括5G标准极化码、GA设计和Reed-Muller极化码）都获得了显著的性能提升。
...
### [177] [Repeater-Aided Over-the-Air Phase Synchronization in Distributed MIMO](https://arxiv.org/abs/2603.19903)
*Unnikrishnan Kunnath Ganesan,Sai Subramanyam Thoota,Erik G. Larsson*

Main category: cs.IT

TL;DR: 提出了一种使用中继节点在AP间直接通信不可行时实现相位同步的新方法，扩展了D-MIMO系统在挑战性环境中的应用


<details>
  <summary>Details</summary>
Motivation: 分布式MIMO系统中AP的相位同步对性能至关重要，但现有方法假设AP能直接通信，这在AP间信号太弱时无法实现

Method: 提出了一种新颖的空中校准方案，使用中继节点在AP直接通信不可行时促进相位同步，给出了相位校准算法的闭式解步骤

Result: 该框架使AP能够实现相干联合传输，扩展了D-MIMO系统在现有空中同步技术无法工作的挑战性环境中的适用性

Conclusion: 通过中继节点辅助的相位同步方案解决了AP间直接通信不可行时的同步问题，为D-MIMO系统在更广泛环境中的应用提供了解决方案
...
### [178] [Robust Beam Codebooks for mmWave/THz Systems: Toward a Stochastic RL Approach](https://arxiv.org/abs/2603.19930)
*Anouar Nechi,Rainer Buchty,Mladen Berekovic,Saleh Mulhem*

Main category: cs.IT

TL;DR: 本文提出了一种基于多智能体强化学习的鲁棒波束码本设计框架，用于毫米波/太赫兹大规模MIMO系统，无需信道状态信息，在非视距条件和硬件受限场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统毫米波/太赫兹大规模MIMO系统依赖预定义的波束赋形码本，这些码本在非视距条件和硬件受限收发器下通常不是最优的。虽然强化学习可以实现无需显式信道状态信息的自适应码本设计，但其在实际条件下的鲁棒性研究不足。

Method: 提出了一种鲁棒的多智能体强化学习框架，直接从环境反馈中学习波束码本，无需先验信道知识。该方法特别适合面临不可预测传播和硬件约束的实际部署场景。对三种离策略算法进行了综合分析：深度确定性策略梯度、双延迟深度确定性策略梯度和软演员-评论家。

Result: 仿真结果表明，软演员-评论家算法在非视距场景下始终优于确定性方法，即使在严重损伤条件下也能实现更优的波束赋形增益和稳定性。该算法对硬件损伤和反馈噪声表现出良好的鲁棒性。

Conclusion: 基于强化学习的码本设计为鲁棒的毫米波/太赫兹大规模MIMO系统展现了良好前景，特别是在非视距条件和硬件受限的实际部署环境中。
