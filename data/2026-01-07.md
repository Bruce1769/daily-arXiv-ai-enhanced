...
Motivation: 医学图像分析面临两个关键挑战：标注数据稀缺和模型可解释性不足，这阻碍了临床AI的部署。少样本学习虽然能解决数据限制问题，但缺乏预测透明度；主动学习方法虽然能优化数据获取，但忽视了获取样本的可解释性。

Method: 提出双框架解决方案：1) EGxFSL：通过Grad-CAM-based Dice loss整合放射科医生定义的感兴趣区域作为空间监督，与原型分类联合优化，实现可解释的少样本学习；2) xGAL：引入迭代样本获取机制，优先考虑预测不确定性和注意力错位，创建闭环框架，使可解释性协同指导训练和样本选择。
...
Conclusion: 该研究提出的双框架有效解决了医学图像分析中的数据稀缺和可解释性问题，通过专家引导的可解释性机制和主动学习的协同作用，显著提升了模型性能，并展示了良好的跨模态泛化能力，为临床AI部署提供了实用解决方案。
...
Method: 提出CoCoT框架，包含两个关键创新：a) 动态多区域定位 - 基于问题自适应检测最相关的图像区域；b) 关系感知推理 - 通过迭代对齐视觉线索实现多区域协作，形成连贯逻辑的思维链
...
TL;DR: NitroGen是一个基于4万小时游戏视频训练的视觉-动作基础模型，能够在1000多款游戏中实现跨游戏泛化，在未见过的游戏中任务成功率比从头训练的模型提升高达52%。


<details>
  <summary>Details</summary>
Motivation: 构建通用的游戏智能体需要能够理解和执行跨不同游戏类型的复杂动作，现有方法通常针对特定游戏训练，缺乏跨游戏泛化能力。研究者希望通过大规模多游戏视频数据训练一个统一的视觉-动作模型来解决这个问题。

Method: 1) 从公开游戏视频中自动提取玩家动作构建互联网规模视频-动作数据集；2) 创建多游戏基准环境以评估跨游戏泛化能力；3) 使用大规模行为克隆训练统一的视觉-动作模型。

Result: NitroGen在3D动作游戏的战斗遭遇、2D平台游戏的高精度控制以及程序生成世界的探索等多样化领域表现出色。在未见过的游戏中，相比从头训练的模型，任务成功率相对提升高达52%。

Conclusion: NitroGen证明了通过大规模多游戏视频数据训练统一视觉-动作模型的可行性，为通用具身智能体的研究提供了重要基础。研究者发布了数据集、评估套件和模型权重以推动该领域发展。
...
Result: 实验表明：1）现有模型仅使用文本信息进行预测时性能显著下降；2）思维链范式对BIQA性能提升有限；3）自一致性范式显著缩小了图像与文本条件预测之间的差距（PLCC/SRCC差异缩小到0.02/0.03）；4）自编码器范式在缩小图像-文本差距方面效果较差，但为后续优化提供了方向。

Conclusion: 研究结果为如何改进BIQA和高级视觉任务中的文本推理提供了见解，自一致性范式在弥合图像与文本表示差距方面表现出色，为未来优化指明了方向。
...
TL;DR: MLLMs在医学图像分类任务中表现不佳，特别是对于膝骨关节炎X光片分类，简单的视觉编码器优于完整MLLM架构，数据平衡比数据规模更重要。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在医学视觉问答和报告生成方面表现出色，但其在疾病特异性分类任务上的能力并未可靠转移。膝骨关节炎影响全球3-4亿人，但在现有医学MLLM基准中代表性不足，需要评估MLLM在该特定分类任务上的有效性。

Method: 通过系统消融研究，操纵视觉编码器、连接器和大语言模型组件，采用不同训练策略，测量每个组件对诊断准确性的贡献。比较了不同规模数据集（500张平衡图像 vs 5778张不平衡图像）的训练效果。

Result: 在膝骨关节炎分类任务中，单独训练的视觉编码器在分类准确率上优于完整MLLM流水线；微调LLM相比基于提示的指导没有显著改进；在小规模平衡数据集上的LoRA微调优于大规模不平衡数据集训练。

Conclusion: 对于领域特定的医学图像分类任务，LLM更适合作为解释器和报告生成器而非主要分类器。MLLM架构不太适合需要高确定性的医学图像诊断分类任务，建议优先优化视觉编码器和精心策划数据集。
...
TL;DR: 该论文提出两种基于隐式神经表示（INR）的框架，用于处理各向异性视网膜OCT图像，实现密集3D分析和分辨率无关的视网膜图谱构建。


<details>
  <summary>Details</summary>
Motivation: 临床视网膜OCT成像通常采用大切片间距，导致图像高度各向异性和视网膜扫描稀疏。现有基于学习的方法多采用2D方法规避各向异性问题，但存在相邻B扫描结果不一致、无法适应不同成像协议分辨率等局限性。

Method: 提出两种基于INR的框架：1）通过结合en-face模态的额外信息进行B扫描间插值，保留扫描间相关结构；2）创建分辨率无关的视网膜图谱，实现不依赖严格数据要求的通用分析。两种方法都利用可泛化的INR，通过基于群体的训练改善视网膜形状表示。

Result: 提出的分辨率独立框架能够处理大B扫描间距的OCT图像，为视网膜结构和病理的容积评估开辟了可能性，解决了各向异性数据分析和不同成像协议兼容性问题。

Conclusion: 基于INR的方法能够有效处理各向异性视网膜OCT数据，实现密集3D分析，为临床常规成像提供了新的容积评估工具，克服了传统2D方法和CNN在分辨率适应性方面的限制。
...
TL;DR: 提出了一种直接从点云生成语言对齐的patch级特征的3D编码器，无需多视图渲染即可实现零样本3D部件分割，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 当前3D基础模型在全局任务上表现良好，但在局部部件级推理上迁移能力差。现有方法依赖多视图渲染和文本查询，需要昂贵的推理过程，依赖LLM提示工程，且未能充分利用3D几何信息

Method: 引入仅编码器3D模型，直接从点云生成语言对齐的patch级特征。使用两阶段预训练：1) 从视觉编码器（如DINOv2）蒸馏密集2D特征到3D patches；2) 通过多正对比目标将这些patch嵌入与部件级文本嵌入对齐

Result: 3D编码器实现了零样本3D部件分割，具有快速单次推理能力，无需测试时多视图渲染，在多个3D部件分割基准上显著优于之前的渲染方法和前馈方法

Conclusion: 提出的方法解决了现有3D基础模型在局部推理上的局限性，通过直接处理点云并实现语言对齐的patch特征，为3D部件分割提供了高效且性能优越的解决方案
...
Method: 将CT体积数据重构为视频流；基准测试YOLO v8/v10/v11/v12 Nano版本选择最佳切片级骨干；引入ByteTrack算法确保z轴解剖一致性；提出混合推理策略和时空一致性滤波器减少初始化延迟

Result: 在独立测试数据上，检测精度从基线2D检测器的0.703提升到0.779，同时保持高灵敏度；计算成本远低于传统3D方法

Conclusion: 该方法通过近似3D上下文推理，以较低计算成本为资源受限环境（如移动卒中单元和物联网远程诊所）提供可扩展的实时患者优先级排序解决方案
...
Conclusion: MovieRecapsQA是首个提供显式文本上下文的开源多模态VideoQA基准，支持细粒度分析，揭示了当前MLLM在多模态视频理解中的局限性
...
TL;DR: DreamLoop是一个从单张照片生成可控电影图的框架，无需专门的电影图训练数据，通过视频扩散模型的时间桥接和运动条件训练实现灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有图像动画技术局限于简单低频运动，仅适用于重复纹理场景；大规模视频扩散模型不适用于电影图约束且缺乏专业数据。需要一种能从单张照片生成可控、无缝循环电影图的方法。

Method: 通过训练通用视频扩散模型实现两个目标：时间桥接和运动条件。推理时使用输入图像作为首尾帧条件确保无缝循环，通过静态轨迹保持背景静止，用户指定运动路径控制目标对象的动画轨迹和时序。

Result: DreamLoop能生成高质量、复杂的电影图，符合用户意图，优于现有方法，是首个支持通用场景电影图生成且具有灵活直观控制的方法。

Conclusion: DreamLoop框架成功实现了从单张照片生成可控电影图，无需专门的电影图训练数据，通过创新的训练策略和推理机制提供了灵活直观的用户控制。
...
TL;DR: 提出基于绿色通道移除重建误差（GRRE）的新方法，用于检测AI生成图像，通过分析真实图像与AI生成图像在绿色通道重建误差上的差异，实现跨模型的强泛化能力。
...
Method: 提出基于通道移除重建的检测范式。具体来说，移除真实图像的绿色（G）通道并重建，发现其重建误差与AI生成图像存在显著差异。基于这一观察，提出了G通道移除重建误差（GRRE）方法，利用这种差异进行鲁棒的AI生成图像检测。

Result: 大量实验表明，GRRE在多个生成模型（包括训练中未见过的模型）上都能保持高检测准确率。与现有方法相比，GRRE不仅对各种扰动和后处理操作具有强鲁棒性，还展现出优越的跨模型泛化能力。
...
TL;DR: CAMO：无需类别特定模板，直接从单目2D视频向多样3D网格迁移运动的通用框架
...
Result: 在运动准确性、效率和视觉一致性方面优于现有方法，显著提升了多样类别和日常视频场景中的运动迁移效果

Conclusion: CAMO框架有效缓解了形状-姿态歧义问题，实现了对多样类别的视觉忠实运动迁移，无需预定义模板或显式3D监督
...
TL;DR: 本文提出了一种用于VR环境中3D网格显著性标注的鲁棒框架，通过视锥采样和混合流形-欧几里得约束扩散算法，解决了现有方法忽略3D几何拓扑与2D图像差异的问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D网格显著性标注方法主要沿用2D图像方法，忽略了3D几何拓扑与2D图像阵列的本质差异。现有的VR眼动追踪流程依赖单射线采样和欧几里得平滑，导致纹理注意力偏差和跨间隙信号泄漏，无法准确反映人类自然感知。

Method: 提出视锥采样策略，通过高斯分布的射线束模拟人类中央凹感受野，提高复杂拓扑结构的采样鲁棒性。开发混合流形-欧几里得约束扩散算法，融合流形测地线约束与欧几里得尺度，确保拓扑一致的显著性传播。

Result: 该框架通过缓解"拓扑短路"和混叠效应，提供了一个与自然人类感知一致的高保真3D注意力获取范式，为3D网格显著性研究提供了更准确和鲁棒的基准。

Conclusion: 提出的框架解决了当前3D网格显著性标注方法的局限性，通过模拟人类感知机制和考虑3D几何拓扑特性，为VR环境中的人为中心视觉建模提供了更可靠的显著性标注方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据集蒸馏方法存在计算开销大、内存限制、生成不真实噪声图像以及架构泛化能力有限的问题。最近的非优化方法虽然通过真实图像补丁缓解了部分问题，但刚性补丁选择策略仍会丢弃主要物体的关键信息

Method: 1) 使用Grounded SAM2识别前景物体并计算每张图像的前景占用率，从中推导出类别级别的补丁决策阈值；2) 基于这些阈值设计动态补丁选择策略，对每张图像要么从多个候选补丁中选择最具信息量的补丁，要么在前景占主导时直接调整完整图像大小；3) 这种双路径机制在减少冗余背景内容的同时保留了更多关于主要物体的关键信息

Result: 在多个基准测试上的广泛实验表明，该方法在蒸馏性能上持续优于现有方法，产生更具信息性和代表性的蒸馏数据集，并增强了跨不同架构和图像组合的鲁棒性
...
TL;DR: 该研究揭示了多模态大语言模型在功能成像（特别是PET）中的功能感知缺陷，提出了PET-Bench基准，发现了CoT幻觉陷阱，并提出AVA方法来解决这一问题。
...
Method: 1. 引入PET-Bench基准，包含52,308个层次化QA对，来自9,732个多中心、多示踪剂的PET研究；2. 评估19个最先进的MLLMs，发现CoT幻觉陷阱；3. 提出原子视觉对齐（AVA）微调策略，强制模型在高级诊断推理前掌握低层功能感知。

Result: 评估显示标准CoT提示在PET中导致语言生成与视觉证据脱节，产生临床流畅但事实无根据的诊断。AVA方法有效弥合了感知差距，将CoT从幻觉来源转变为稳健推理工具，诊断准确率提升高达14.83%。

Conclusion: 该研究揭示了MLLMs在功能成像中的关键安全风险（CoT幻觉陷阱），并提出了有效的解决方案（AVA）。AVA通过强制低层功能感知先于高级推理，显著改善了PET图像分析的诊断准确性。
...
TL;DR: D³R-DETR：基于DETR的遥感小目标检测器，通过双域密度细化融合空间和频域信息，提升小目标检测精度
...
Method: 提出D³R-DETR，采用双域密度细化方法，融合空间和频域信息来细化低层特征图，利用其丰富的细节预测更准确的目标密度图，从而指导模型精确定位微小目标

Result: 在AI-TOD-v2数据集上的大量实验表明，D³R-DETR在微小目标检测方面优于现有的最先进检测器

Conclusion: D³R-DETR通过双域密度细化有效解决了遥感图像中小目标检测的挑战，在精度上超越了现有方法
...
Method: 提出BUFFER-X框架：1) 几何引导的超参数自动估计；2) 分布感知最远点采样替代学习检测器；3) 块级坐标归一化确保尺度一致性。采用分层多尺度匹配，从局部、中等到全局感受野提取对应关系。

Result: 在包含12个数据集的综合基准测试中，涵盖物体尺度、室内和室外场景，包括异构LiDAR配置间的跨传感器配准，证明无需手动调参或先验知识即可有效泛化。
...
TL;DR: 提出了一种轻量级、数据中心的零样本单目深度估计框架，采用DINOv3作为视觉编码器，设计了参数更少的Simple Depth Transformer解码器，并通过质量过滤策略减少数据集规模同时提升训练质量。
...
Method: 1) 使用DINOv3作为视觉编码器获取高质量密集特征；2) 设计Simple Depth Transformer(SDT)作为紧凑的transformer解码器，采用单路径特征融合和上采样减少计算开销；3) 提出基于质量的过滤策略筛选有害样本，减少数据集规模。

Result: 在五个基准测试上的实验表明，该框架在准确率上超越了DPT方法。SDT相比DPT减少了约85%-89%的参数，同时实现了更高的准确率。质量过滤策略在减少数据集规模的同时提升了整体训练质量。

Conclusion: 这项工作强调了平衡模型设计和数据质量对于实现高效且可泛化的零样本深度估计的重要性。提出的轻量级框架在减少参数和计算开销的同时保持了高准确率。
...
TL;DR: ClearAIR是一个受人类视觉感知启发的全合一图像修复框架，采用分层粗到细策略，结合MLLM质量评估、区域感知和内部线索重用机制，在复杂退化图像修复上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有全合一图像修复方法过度依赖退化特定表示，常导致过度平滑和伪影问题，需要更准确处理复杂复合退化的方法。

Method: 1) 基于MLLM的图像质量评估模型进行整体评估；2) 区域感知和任务识别流水线，包括语义交叉注意力和退化感知模块；3) 内部线索重用机制，以自监督方式挖掘图像内在信息。

Result: 实验结果表明ClearAIR在多种合成和真实世界数据集上均取得优越性能。

Conclusion: ClearAIR通过模拟人类视觉感知的分层策略，有效解决了复杂复合退化图像的修复问题，显著提升了修复质量和细节保留能力。
...
TL;DR: 提出AbductiveMLLM框架，通过模仿人类认知中言语和图像双重推理模式来增强多模态大语言模型的溯因推理能力，在视觉溯因推理任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在通用多模态推理方面表现出色，但在溯因推理能力上仍不及人类。为了弥合这一差距，研究受到人类认知中言语和图像双重推理模式相互作用的启发。

Method: 提出AbductiveMLLM框架，包含两个协同组件：REASONER（言语推理器）和IMAGINER（图像想象器）。REASONER先在言语域探索可能的解释空间，然后基于跨模态因果对齐修剪视觉不一致假设；IMAGINER通过文本到图像扩散模型"想象"与言语解释对应的视觉场景。两个组件以端到端方式联合训练。

Result: 在标准视觉溯因推理基准测试中，AbductiveMLLM实现了最先进的性能，一致优于传统解决方案和先进的多模态大语言模型。

Conclusion: 通过模仿人类认知中言语和图像双重推理模式，可以有效增强多模态大语言模型的溯因推理能力，为视觉溯因推理任务提供了新的解决方案。
...
TL;DR: DreamStyle是一个统一的视频风格化框架，支持文本引导、风格图像引导和首帧引导三种条件，通过精心设计的数据收集管道和LoRA训练方法，解决了现有方法风格不一致和时间闪烁的问题。


<details>
  <summary>Details</summary>
Motivation: 视频风格化作为视频生成模型的重要下游任务尚未得到充分探索。现有方法主要局限于单一类型的风格条件（文本、风格图像或风格化首帧），限制了应用范围。同时，缺乏高质量数据集导致风格不一致和时间闪烁问题。

Method: 基于普通的图像到视频（I2V）模型构建，采用精心设计的数据收集管道获取高质量配对视频数据。使用低秩适应（LoRA）训练，并采用特定token的上矩阵来减少不同条件token之间的混淆。

Result: 定性和定量评估表明，DreamStyle在所有三种视频风格化任务中表现优异，在风格一致性和视频质量方面优于竞争对手。

Conclusion: DreamStyle提供了一个统一的视频风格化框架，成功解决了多条件支持和数据质量问题，为视频风格化任务提供了全面有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的单图像深度估计模型应用于视频序列时会出现时间不稳定性和闪烁伪影问题，需要一种能够保持时间一致性的视频深度估计方法
...
Conclusion: StableDPT通过集成时间模块成功将图像深度估计模型适配到视频处理，实现了更准确、时间更稳定的深度预测，且计算效率更高
...
<details>
  <summary>Details</summary>
Motivation: IHC染色在癌症临床诊断中至关重要，但相比H&E染色，其过程复杂、耗时且昂贵，限制了临床应用。虚拟染色提供了一种经济有效的替代方案，但相邻切片作为ground truth会导致弱配对数据，存在空间错位和局部变形，阻碍有效的监督学习。

Method: 提出拓扑感知的虚拟染色框架，包含两个核心机制：1) 拓扑感知一致性匹配(TACM)：利用图对比学习和拓扑扰动学习鲁棒的匹配模式，确保结构一致性；2) 拓扑约束病理匹配(TCPM)：基于节点重要性对齐病理阳性区域，增强病理一致性。

Result: 在两个基准数据集上的四个染色任务中进行了广泛实验，结果表明该方法超越了最先进的方法，实现了更高的生成质量和临床相关性。

Conclusion: 提出的拓扑感知框架有效解决了H&E到IHC虚拟染色中的空间错位问题，通过拓扑感知机制确保了结构和病理一致性，为临床提供了一种高质量的虚拟染色解决方案。
...
TL;DR: SketchThinker-R1通过引入人类草图式推理风格，显著降低多模态大模型推理过程的计算开销，在保持准确率的同时减少64%以上的推理token成本


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型采用逐步推理方式，虽然有效但计算开销大（token成本高、响应时间长），而人类常使用更高效的草图式推理（简洁、目标导向、关注关键信息），因此希望将这种认知效率引入AI模型

Method: 1. 草图模式冷启动：将标准长推理过程转换为草图式推理，微调基础多模态模型，注入初始草图推理能力；2. 训练SketchJudge奖励模型：显式评估模型思考过程，为草图式推理分配更高分数；3. 草图思维强化学习：在SketchJudge监督下进行强化学习，进一步泛化草图式推理能力

Result: 在四个基准测试上的实验评估显示，SketchThinker-R1在不影响最终答案准确率的情况下，实现了超过64%的推理token成本降低。定性分析进一步表明草图式推理在解决问题时更关注关键线索

Conclusion: SketchThinker-R1成功将人类草图式推理的高效认知模式引入多模态大模型，显著提升了推理效率，为开发更高效的多模态AI系统提供了新思路
...
TL;DR: DGA-Net：一种通过"深度提示"范式改进SAM模型用于伪装目标检测的框架，利用跨模态图增强和锚点引导细化模块提升性能
...
Method: 提出DGA-Net框架，采用新颖的"深度提示"范式。包含两个核心模块：1) 跨模态图增强(CGE)模块，在异质图中合成RGB语义和深度几何信息形成统一指导信号；2) 锚点引导细化(AGR)模块，创建全局锚点并建立直接的非局部路径，将指导信号从深层广播到浅层，确保精确一致的分割。

Result: 定量和定性实验结果表明，DGA-Net在伪装目标检测任务上优于现有最先进的方法。

Conclusion: DGA-Net通过深度提示范式成功改进了SAM模型在伪装目标检测中的应用，跨模态图增强和锚点引导细化模块有效提升了检测性能，为充分利用深度信息提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临低信噪比、目标尺寸小和复杂背景等挑战。虽然基于DETR的检测器受益于全局上下文建模，但在红外小目标检测中表现不佳。研究发现，由于自注意力机制，红外小目标的相关嵌入被主导的背景特征淹没，导致查询初始化和目标定位不可靠。
...
Conclusion: SEF-DETR通过改进查询初始化机制，有效解决了DETR在红外小目标检测中的性能退化问题，显著提升了检测性能，为红外小目标检测提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有分割方法通常依赖掩码框架，缺乏对分割不确定性的建模能力。本文旨在开发一个不依赖掩码的通用分割框架，能够以整体方式预测完整分割结果。

Method: 采用扩散模型框架，引入位置感知调色板和2D格雷码排序改进性能，使用tanh激活函数处理离散数据，优化扩散参数并采用sigmoid损失加权和x-prediction策略。

Result: 虽然当前模型尚未超越领先的掩码架构，但显著缩小了性能差距，并具备独特的原理性模糊建模能力，这是现有模型所缺乏的。

Conclusion: 该扩散框架为通用图像分割提供了新途径，结合大规模预训练或可提示条件调节有望开发出具有竞争力的模型。
...
TL;DR: 提出TA-Prompting方法，通过时间锚点增强VideoLLMs在未修剪视频中的事件边界定位能力，并引入事件连贯性采样策略生成更准确、连贯的密集视频描述。


<details>
  <summary>Details</summary>
Motivation: 现有VideoLLMs在未修剪视频中难以准确识别事件边界，导致生成的描述缺乏适当的时间定位基础。需要改进VideoLLMs在密集视频描述任务中的时间感知能力。
...
Method: 提出Zoom-IQA模型，采用两阶段训练流程：1）在Grounded-Rationale-IQA数据集上进行监督微调，使模型能够将评估结果锚定在关键区域；2）使用强化学习进行动态策略探索，主要通过KL-Coverage正则化器稳定训练防止推理和评分多样性崩溃，并采用渐进重采样策略缓解标注偏差。

Result: Zoom-IQA在广泛实验中表现出改进的鲁棒性、可解释性和泛化能力。在图像修复等下游任务中的应用进一步证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将所有多模态数据放在单一融合模型中处理，忽略了平衡与不平衡质量分布数据的不同需求，难以解耦类内一致性与模态间异质性的冲突。

Method: 提出DCG-ReID方法：1) 动态置信度解耦加权机制动态重加权三模态贡献；2) 针对平衡质量分布的协作融合模块挖掘成对共识特征；3) 针对不平衡分布的引导融合模块实现模态判别差异的差异放大。

Result: 在三个多模态ReID基准数据集（WMVeID863, MSVR310, RGBNT100）上的广泛实验验证了方法的有效性。

Conclusion: DCG-ReID通过解耦异构质量分布模态数据，针对不同场景设计特定融合策略，有效解决了多模态车辆重识别中的冲突融合需求问题。
...
Conclusion: PrismVAU是一个高效实用的实时视频异常理解解决方案，适用于现实世界应用
...
<details>
  <summary>Details</summary>
Motivation: 传统太阳能板人工检测方法耗时、昂贵且易出错，现有深度学习模型要么太大不适合边缘设备，要么存在准确性估计偏差问题

Method: 集成EfficientNet-B0和卷积块注意力模块(CBAM)，采用分割前增强协议避免数据泄漏，引入焦点损失和余弦退火，进行5折分层交叉验证

Result: 平均准确率达到92.37%±0.41，F1分数0.9226±0.39，仅需16.3MB存储（比VGG19小32倍），GPU推理速度54.9FPS，Grad-CAM可视化显示模型关注实际故障位置

Conclusion: HybridSolarNet是轻量高效的太阳能板故障检测模型，适合无人机实时监测应用，CBAM和焦点损失分别提升了准确性和不平衡样本处理能力
...
<details>
  <summary>Details</summary>
Motivation: 随着电子商务和数字时尚的快速发展，基于图像的虚拟试穿技术日益重要，但现有VTON模型常出现服装变形和身体不一致等伪影，需要可靠的VTON生成图像质量评估方法。
...
TL;DR: LAMS-Edit：一种基于扩散模型的文本到图像编辑框架，通过混合反转过程中的潜在表示和注意力图，平衡内容保留与编辑应用


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的文本到图像编辑面临两个主要挑战：1）难以在内容保留和编辑应用之间取得平衡；2）处理真实图像编辑时效果不佳。需要一种能够有效解决这些问题的编辑方法。

Method: 提出LAMS-Edit框架，核心是LAMS（潜在和注意力混合调度器）技术。该方法利用反转过程中的中间状态（潜在表示和注意力图），在编辑图像生成时通过加权插值与原始过程的状态混合，由调度器控制混合比例。框架与Prompt-to-Prompt（P2P）集成，支持区域掩码精确编辑和通过LoRA实现风格迁移。

Result: 大量实验表明，LAMS-Edit能够有效平衡内容保留和编辑应用，在真实图像编辑任务中表现优异。

Conclusion: LAMS-Edit是一个可扩展的文本到图像编辑框架，通过创新的潜在和注意力混合机制，成功解决了扩散模型在内容保留与编辑应用之间的平衡问题，特别适用于真实图像编辑场景。
...
TL;DR: ULS+是通用病灶分割模型的增强版，通过整合新数据集和减小输入图像尺寸，在准确性和推理速度上均有提升，在ULS23挑战赛中表现优异。


<details>
  <summary>Details</summary>
Motivation: 原始ULS模型虽然能基于点击点对CT扫描中的全身病灶进行分割，但新公开数据集的可用性为模型性能提升提供了机会，需要整合这些资源来改进模型。

Method: ULS+整合了新增的公开数据集，并采用了更小的输入图像尺寸，通过数据驱动更新和临床验证的循环来增强模型性能。
...
TL;DR: 该论文针对漫画视觉问答（CVQA）任务，提出了一种新的漫画推理框架，解决了标准思维链（CoT）提示在CVQA中性能下降的问题，特别是在小规模多模态大语言模型中。


<details>
  <summary>Details</summary>
Motivation: 漫画视觉问答（CVQA）对多模态大语言模型（MLLMs）提出了独特挑战，因为它依赖于符号抽象、叙事逻辑和幽默元素，这些与传统VQA任务不同。研究发现标准思维链（CoT）提示在CVQA中往往降低性能，特别是在小规模模型中，这促使研究者开发更有效的推理方法。

Method: 提出了一种新颖的漫画推理框架，结合模块化CoT生成、基于GRPO的强化微调以及新颖的结构化奖励机制。该方法旨在为小型MLLMs生成更忠实和可迁移的推理链。

Result: 在五个具有挑战性的基准测试中，该方法的3B模型超越了最先进的方法，并且插件实验在不同MLLMs上带来了平均12.1%的额外性能提升。

Conclusion: 该研究成功解决了CVQA中标准CoT提示的局限性，提出的漫画推理框架不仅适用于漫画VQA，还能扩展到更广泛的幽默中心和抽象视觉推理任务，如表情包理解和社论漫画解读。
...
<details>
  <summary>Details</summary>
Motivation: 车路协同感知中通信带宽与特征冗余的权衡是关键瓶颈。现有中间融合方法依赖空间压缩或静态置信度图，会低效传输非关键背景区域的冗余特征。

Method: 提出RiSe框架：1) 基于势场理论的PTCM定量评估运动学风险；2) IDAPM利用自车运动先验主动预测关键BEV区域；3) 语义选择性融合方案，仅传输高交互区域的高保真特征。

Result: 在DeepAccident数据集上，方法将通信量降至完整特征共享的0.71%，同时保持最先进的检测精度，在带宽效率和感知性能间建立了有竞争力的帕累托前沿。

Conclusion: RiSe通过从识别可见区域转向优先处理风险关键区域，实现了语义选择性融合，有效作为特征去噪器，在车路协同感知中平衡了通信效率和感知性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决主动场景重建中稀疏宽基线视图导致的监督不足问题，提升不确定性量化的稳定性和有效性，改善最佳下一视图选择

Method: 1) 通过训练视图与光栅化外推视图的三角测量生成自增强点云(SA-Points)；2) 针对3D高斯溅射的首个残差学习策略，结合不确定性驱动过滤和dropout/硬负样本采样；3) 物理引导的视图选择策略

Result: 在主动视图选择实验中，SA-ResGS在重建质量和视图选择鲁棒性方面均优于现有最先进基线方法

Conclusion: SA-ResGS通过自增强点云和残差监督，有效解决了稀疏宽基线视图下的监督不足问题，提升了不确定性量化和视图选择性能，为主动场景重建提供了更可靠的解决方案
...
TL;DR: DFRCP是一种动态模糊鲁棒卷积金字塔模块，作为YOLOv11的插件升级，用于提升在运动模糊条件下的目标检测性能。该方法通过融合多尺度特征和引入动态鲁棒切换单元来增强全局感知，同时开发了CUDA并行内核实现高效计算。
...
Result: 在约3,500张小麦害虫损害私有数据集上训练，使用两种模糊机制进行三倍增强。在模糊测试集上，YOLOv11+DFRCP比YOLOv11基线准确率提高约10.4%，仅增加适度的训练时间开销，减少了数据收集后的人工筛选需求。

Conclusion: DFRCP作为一种插件升级，有效提升了YOLOv11在运动模糊条件下的目标检测鲁棒性，通过高效的计算实现使其适用于边缘设备部署，减少了数据收集后的人工处理需求。
...
TL;DR: ViT在空间推理任务（如心理旋转）中存在系统性失败，这源于架构固有的电路复杂度限制，而非数据规模问题。研究证明，对于非可解群（如SO(3)），保持结构保持嵌入的计算复杂度下界为NC^1完全，而恒定深度ViT受限于TC^0，存在复杂度边界。


<details>
  <summary>Details</summary>
Motivation: 尽管ViT在语义识别方面表现出色，但在空间推理任务中却存在系统性失败。传统观点常将此归因于数据规模，但本文提出这种限制源于ViT架构固有的电路复杂度问题。

Method: 将空间理解形式化为学习群同态：将图像序列映射到保持底层变换群代数结构的潜在空间。对于非可解群（如3D旋转群SO(3)），证明保持这种结构保持嵌入的计算复杂度下界为NC^1完全。同时证明恒定深度ViT受限于TC^0复杂度类。

Result: 在TC^0 ⊊ NC^1猜想下，建立了复杂度边界：恒定深度ViT从根本上缺乏有效捕捉非可解空间结构所需的逻辑深度。通过潜在空间探测验证了这一复杂度差距，显示随着组合深度增加，ViT表示在非可解任务上出现结构崩溃。

Conclusion: ViT在空间推理任务中的失败源于架构固有的计算复杂度限制，而非数据不足。恒定深度ViT无法有效处理非可解群的空间结构，这为理解Transformer架构的能力边界提供了理论依据。
...
TL;DR: IBISAgent：一种将医学图像分割重构为视觉中心多步决策过程的智能MLLM，通过交错推理和点击动作实现高质量分割，无需架构修改


<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM分割方法存在两个主要问题：1）引入隐式分割标记并需要同时微调MLLM和外部像素解码器，导致灾难性遗忘风险增加且泛化能力受限；2）依赖单次推理，缺乏迭代细化分割结果的能力，导致性能不佳

Method: 提出IBISAgent智能MLLM，将分割重构为视觉中心的多步决策过程，支持交错推理和基于文本的点击动作，调用分割工具生成高质量掩码。采用两阶段训练框架：冷启动监督微调和带有细粒度奖励的智能强化学习

Result: 在广泛的实验中，IBISAgent在复杂医学参考和推理分割任务中一致优于闭源和开源的最先进方法

Conclusion: IBISAgent通过多步视觉推理和智能决策过程，有效解决了现有医学MLLM分割方法的局限性，提升了像素级视觉推理能力，在医学图像分割任务中表现出卓越性能
...
TL;DR: CFSG模型通过将概念和特征空间解耦为共同、特定和混杂三个结构化组件，并引入自适应机制动态调整各组件比例，以解决细粒度领域泛化中模型对细微线索过度敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 细粒度领域泛化比传统领域泛化更具挑战性，因为细粒度识别任务中存在细微的类间差异和相对明显的类内变化。在领域偏移下，模型对细粒度线索变得过度敏感，导致关键特征被抑制，性能显著下降。认知研究表明，人类通过利用共同和特定属性来分类物体，但当前深度学习模型尚未有效融入这一机制。

Method: 提出概念-特征结构化泛化（CFSG）模型，将概念和特征空间显式解耦为三个结构化组件：共同、特定和混杂部分。引入自适应机制动态调整共同、特定和混杂组件的比例，以减轻不同程度分布偏移的负面影响。在最终预测中，为每对组件分配显式权重。

Result: 在三个单源基准数据集上的广泛实验表明，CFSG相比基线模型平均性能提升9.87%，比现有最先进方法平均高出3.08%。可解释性分析验证了CFSG有效整合了多粒度结构化知识，并确认特征结构化促进了概念结构化的出现。

Conclusion: CFSG模型通过模拟人类认知机制，将概念和特征空间结构化解耦，并引入自适应调整机制，有效解决了细粒度领域泛化中的挑战，在性能和可解释性方面都取得了显著改进。
...
TL;DR: 本文提出了一种针对卡通图像视觉问答任务的多智能体LLM框架，通过视觉、语言和批评三个专门智能体的协作来解决卡通图像中视觉抽象和叙事上下文理解的挑战。


<details>
  <summary>Details</summary>
Motivation: 卡通图像的视觉问答面临特殊挑战，包括夸张的视觉抽象和叙事驱动的上下文理解，这些是标准基于自然图像训练的LLM无法充分处理的。需要专门的方法来解决卡通VQA中的这些独特问题。

Method: 提出一个多智能体LLM框架，包含三个专门智能体：视觉智能体（处理视觉线索）、语言智能体（处理语言信息）和批评智能体（评估和整合）。这些智能体通过协作支持结构化推理，整合视觉线索和叙事上下文。

Result: 在Pororo和Simpsons两个卡通VQA数据集上进行了系统评估。实验结果详细分析了每个智能体对最终预测的贡献，为理解LLM基多智能体在卡通VQA和多模态推理中的行为提供了深入见解。

Conclusion: 多智能体LLM框架能有效处理卡通图像VQA中的独特挑战，通过专门智能体的协作整合视觉和叙事信息，为卡通VQA任务提供了有前景的解决方案。
...
Conclusion: 基础模型去偏是实现公平临床AI应用的重要步骤，LesionTABE展示了这一方法的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型通常只使用冻结视觉编码器的单一顶层特征，未能充分利用编码器丰富的视觉层次信息，导致模型容易出现视觉未基础的幻觉，过度依赖语言先验而非图像证据。

Method: 提出TGIF（文本引导的跨层融合）模块，将编码器各层视为深度方向的"专家"，根据查询提示预测视觉特征的融合权重。该方法遵循直接外部融合原则，无需更新视觉编码器，计算开销极小。
...
<details>
  <summary>Details</summary>
Motivation: 植物病害会降低作物产量和质量，葡萄叶病害检测对农业生产至关重要。传统诊断方法可能效率低下，需要自动化、准确的检测系统来帮助农民做出管理决策。

Method: 使用包含9,032张图像的葡萄叶病害数据集（4个类别：3种病害+健康叶片）。数据集经过严格预处理后按70%/20%/10%划分训练/验证/测试集。部署了两个预训练模型InceptionV3和Xception，采用对抗训练增强鲁棒性，集成Grad-CAM提供可视化解释。

Result: Xception模型表现最佳，达到96.23%的准确率，优于InceptionV3模型。对抗训练提高了模型鲁棒性，Grad-CAM成功确认了叶部病害区域。最终开发了包含热图可视化和置信度预测的Streamlit网页应用。

Conclusion: 该研究成功开发了一个准确、鲁棒且可解释的葡萄叶病害分类系统，Xception模型结合对抗训练和Grad-CAM可视化在病害检测中表现出色，部署的网页应用为农民提供了实用的诊断工具。
...
TL;DR: Unified Thinker是一个任务无关的推理架构，通过将推理模块与生成器解耦，使用两阶段训练方法提升图像生成的逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在逻辑密集型指令跟随方面存在推理-执行差距，闭源系统展示了强大的推理驱动图像生成能力，而开源模型在这方面存在明显不足。

Method: 提出Unified Thinker架构，将专用推理器与图像生成器解耦；采用两阶段训练：先构建结构化规划接口，然后使用强化学习将策略基于像素级反馈进行优化。

Result: 在文本到图像生成和图像编辑任务上的广泛实验表明，Unified Thinker显著提升了图像推理和生成质量。

Conclusion: 通过将推理与生成解耦并采用基于像素反馈的强化学习，可以有效缩小图像生成中的推理-执行差距，提升逻辑密集型任务的执行能力。
...
TL;DR: LSP-DETR是一种用于细胞核实例分割的端到端Transformer框架，使用星凸多边形表示细胞核，通过径向距离损失函数自然处理重叠细胞核，无需额外后处理，比现有方法快5倍以上。


<details>
  <summary>Details</summary>
Motivation: 全切片图像中的细胞核实例分割对计算病理学至关重要，但现有方法依赖基于补丁的处理和昂贵的后处理，牺牲了上下文信息和效率。

Method: 提出LSP-DETR框架，使用线性复杂度的轻量级Transformer处理更大图像；将细胞核表示为星凸多边形，引入新颖的径向距离损失函数，无需显式重叠标注或手工后处理即可自然分割重叠细胞核。

Result: 在PanNuke和MoNuSeg数据集上评估，显示跨组织的强泛化能力和最先进的效率，LSP-DETR比次快领先方法快5倍以上。

Conclusion: LSP-DETR为计算病理学提供了一种精确、可扩展且高效的细胞核实例分割解决方案，通过端到端框架和创新的表示方法克服了现有方法的局限性。
...
TL;DR: 提出DiffBench基准测试和DiffAgent智能体框架，通过LLM驱动的自动化代码生成与评估来加速扩散模型推理


<details>
  <summary>Details</summary>
Motivation: 扩散模型的多步推理过程计算开销大，阻碍实际部署；现有加速技术组合策略不明确，需要自动化解决方案

Method: 1) DiffBench：三阶段自动化评估流水线，涵盖多种扩散架构、优化组合和部署场景；2) DiffAgent：基于LLM的智能体，采用规划-代码生成-调试的闭环工作流，结合遗传算法从执行环境获取性能反馈

Result: DiffBench提供了全面的代码评估能力，DiffAgent在生成有效扩散加速策略方面显著优于现有LLM方法

Conclusion: 该框架通过LLM驱动的自动化加速代码生成与评估，有效解决了扩散模型加速技术组合的挑战，为实际部署提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态医学大语言模型在胸部X光解释中面临空间推理和解剖学理解的挑战，虽然现有定位技术改善了整体性能，但未能建立真正的解剖学对应关系，导致医学领域的解剖学理解错误。

Method: 受放射学工作流程启发，采用两阶段方法：首先识别解剖结构并提取特征，然后利用大语言模型执行多种下游任务，包括短语定位、报告生成、视觉问答和图像理解。

Result: 在多个基准测试中的广泛实验表明，AnatomiX实现了卓越的解剖学推理能力，在解剖学定位、短语定位、基础诊断和基础描述任务上比现有方法性能提升超过25%。

Conclusion: AnatomiX通过明确设计的解剖学基础方法，有效解决了多模态医学大语言模型在胸部X光解释中的解剖学理解挑战，显著提升了空间推理和解剖学对应能力。
...
TL;DR: UniCorn是一个自改进框架，通过将统一多模态模型划分为三个协作角色来解决模型理解能力强但生成能力弱的"传导性失语症"问题，无需外部数据或监督就能显著提升文本到图像生成质量。
...
Method: 提出UniCorn自改进框架，将单个统一多模态模型划分为三个协作角色：提议者、求解者和评判者。通过自我博弈生成高质量交互，并采用认知模式重构将潜在理解提炼为显式生成信号。同时引入UniCycle基准，基于文本到图像到文本的重建循环来验证多模态一致性恢复。

Result: UniCorn在六个通用图像生成基准测试中实现了全面且显著的改进。在TIIF(73.8)、DPG(86.8)、CompBench(88.5)和UniCycle上达到SOTA性能，同时在WISE上提升+5.0，在OneIG上提升+6.5。结果表明该方法显著增强了文本到图像生成能力，同时保持了强大的理解能力。

Conclusion: UniCorn框架通过完全自监督的细化方法，成功解决了统一多模态模型中的"传导性失语症"问题，显著提升了生成质量而不损害理解能力，展示了统一多模态智能完全自监督细化的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型虽然能生成引人注目的视频序列，但缺乏音频提供的语义、情感和氛围线索，因此需要能够同步生成高质量视听内容的模型。

Method: 采用非对称双流Transformer架构，包含140亿参数的视频流和50亿参数的音频流，通过双向视听交叉注意力层连接，配备时间位置嵌入和跨模态AdaLN进行共享时间步调节。使用多语言文本编码器增强提示理解，并引入模态感知的无分类器引导机制。

Result: 模型在开源系统中实现了最先进的视听质量和提示遵循度，同时以远低于专有模型的计算成本和推理时间提供可比的结果。能够生成包含自然背景和音效元素的丰富、连贯的音频轨道。

Conclusion: LTX-2是一个高效的开源视听生成基础模型，通过创新的架构设计实现了高质量的同步视听内容生成，为研究社区提供了强大的工具。
...
TL;DR: InfiniDepth提出了一种基于神经隐式场的深度估计方法，能够实现任意分辨率和细粒度深度预测，在合成和真实世界基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度估计方法基于离散图像网格，限制了输出分辨率的可扩展性并阻碍了几何细节恢复。需要一种能够支持任意分辨率和细粒度深度估计的新方法。

Method: InfiniDepth将深度表示为神经隐式场，通过简单有效的局部隐式解码器，可以在连续2D坐标上查询深度，实现任意分辨率和细粒度深度估计。

Result: 在从五个不同游戏构建的高质量4K合成基准测试中，InfiniDepth在合成和真实世界基准测试的深度估计任务中达到最先进性能，特别是在细细节区域表现出色。同时在大视角变化下的新视角合成任务中也产生高质量结果，减少空洞和伪影。

Conclusion: 神经隐式场表示深度的方法克服了传统离散网格方法的限制，实现了任意分辨率和细粒度深度估计，为深度感知任务提供了更灵活和准确的解决方案。
...
Method: 1) 通过图约束推理构建具有连贯布局和尺度的创意3D骨骼；2) 在结构化潜在空间中进行骨骼引导的体素组装，整合不同对象的区域；3) 在骨骼条件下进行图像引导的外观建模，为组装形状生成风格一致且和谐的纹理

Result: 大量实验证明Muses在视觉保真度和文本描述对齐方面达到最先进水平，并在灵活的3D对象编辑方面展现出潜力

Conclusion: Muses通过骨骼表示将3D内容创建形式化为结构感知的设计、组合和生成流程，实现了高质量3D生物生成，为3D内容创作提供了新范式
...
TL;DR: WearVox是首个针对可穿戴设备语音助手的真实场景基准测试，包含3842条多通道音频，涵盖5种任务，评估显示现有SLLM准确率仅29%-59%，多通道音频能显著提升抗噪能力和语音识别准确性。


<details>
  <summary>Details</summary>
Motivation: 可穿戴AI设备（如智能眼镜）将语音助手转变为随时可用的免提协作工具，但面临运动噪声、微交互、设备定向语音识别等挑战。现有基准测试主要关注干净或通用对话音频，忽略了可穿戴场景的真实复杂性。

Method: 开发WearVox基准测试，包含3842条通过智能眼镜收集的多通道自我中心音频，涵盖5种任务：搜索增强QA、闭卷QA、旁白拒绝、工具调用和语音翻译，覆盖室内外多种声学环境。为每条录音提供丰富元数据，并评估主流专有和开源语音大语言模型。

Result: 实时SLLM在WearVox上的准确率仅为29%-59%，在嘈杂室外音频上性能显著下降。案例研究表明，多通道音频输入能显著增强模型对环境噪声的鲁棒性，并改善设备定向语音与背景对话的区分能力。
...
TL;DR: PCEval是首个用于评估大语言模型在物理计算中逻辑和物理方面能力的基准测试，通过模拟环境自动评估模型在电路设计和代码生成方面的表现，发现模型在物理面包板布局方面存在显著困难。
...
Method: 提出PCEval基准测试，这是物理计算领域的首个基准，能够对LLM在项目逻辑和物理方面的能力进行全自动评估，无需人工评估。评估框架测试模型在不同项目复杂度下生成电路和兼容代码的能力。

Result: 通过对13个领先模型的全面测试发现：LLM在代码生成和逻辑电路设计方面表现良好，但在物理面包板布局创建方面存在显著困难，特别是在管理正确的引脚连接和避免电路错误方面。
...
TL;DR: 论文提出了一种新的拓扑损失函数，通过将食材列表表示为嵌入空间中的点云来改善食谱生成质量，结合多种复合目标函数显著提升了食材和动作层面的指标。


<details>
  <summary>Details</summary>
Motivation: 烹饪食谱生成需要流畅的文本、准确的时序、温度、程序连贯性以及正确的食材组合。传统的基于交叉熵的训练方法主要关注流畅性，无法充分处理食谱生成的多维度要求。

Method: 基于RECIPE-NLG框架，研究多种复合目标函数，提出新的拓扑损失函数，将食材列表表示为嵌入空间中的点云，最小化预测食材与真实食材之间的差异。同时探索Dice损失和混合损失。

Result: 新提出的拓扑损失显著改善了食材和动作层面的指标；Dice损失在时间/温度精度方面表现优异；混合损失在数量和时间方面实现了协同增益。人工偏好分析显示，该模型在62%的情况下被优先选择。

Conclusion: 通过引入针对性的损失函数，特别是新的拓扑损失，能够有效提升食谱生成的多维度质量，为复杂程序性文本生成提供了有价值的改进方法。
...
TL;DR: ModeX是一个无需外部评估器的Best-of-N选择框架，通过识别生成文本中的主导语义共识来选择最佳输出，在开放任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在开放任务中选择最佳输出的方法依赖外部评估器、奖励模型或精确字符串匹配投票，限制了应用范围和效率，需要一种无需外部评估的高效选择方法。

Method: 提出ModeX框架，构建候选生成文本的相似度图，通过递归谱聚类选择代表中心点；进一步提出ModeX-Lite版本，通过早期剪枝提高效率。

Result: 在文本摘要、代码生成和数学推理等开放任务中，ModeX方法持续优于标准的单路径和多路径基线方法。

Conclusion: ModeX为开放文本生成提供了一个计算高效且无需外部评估器的解决方案，通过识别语义共识来选择最佳输出。
...
<details>
  <summary>Details</summary>
Motivation: LLM在需要事实准确性的应用中经常产生幻觉响应，现有事实检查方法通常不加区分地检索外部证据，忽略了模型的内部知识并可能引入无关噪声，同时缺乏解决模型推理中特定不确定性的针对性机制。

Method: 提出概率确定性和一致性（PCC）框架，通过联合建模LLM的概率确定性和推理一致性来估计事实置信度，实现自适应验证策略：模型在自信时直接回答，不确定或不一致时触发针对性检索，歧义高时升级到深度搜索。

Result: 在三个具有挑战性的基准测试中，PCC在不确定性量化方面优于口头置信度方法，并持续优于强大的基于LLM的事实检查基线，且在不同LLM上具有良好的泛化能力。

Conclusion: PCC框架通过自适应地决定何时依赖内部知识或启动检索，提高了事实检查的效率和可靠性，为LLM的事实准确性提供了有效的解决方案。
...
TL;DR: 本研究提出了一种利用大语言模型模拟学生反应来隐式建模项目参数的新方法，避免了传统IRT校准需要昂贵现场测试的问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法确定评估项目参数（如难度和区分度）严重依赖昂贵的现场测试来收集学生表现数据以进行IRT校准。本研究旨在开发一种更高效、成本更低的替代方案。

Method: 通过微调大语言模型（使用Qwen-3密集模型系列和低秩适应LoRA）来模拟学生在不同潜在能力水平上的反应，基于离散能力描述符生成多项选择题的响应，重构正确反应概率作为学生能力的函数，从而生成合成项目特征曲线来估计IRT参数。

Result: 在六年级英语语言艺术项目数据集和BEA 2024共享任务数据集上的评估表明，该方法与基线方法竞争甚至优于基线方法，特别在建模项目区分度方面表现出色。

Conclusion: 这种基于模拟的技术为项目参数估计提供了一种有前景的替代方法，特别在建模项目区分度方面效果显著，避免了传统现场测试的高成本。
...
TL;DR: FlowPlan-G2P：一个将科学论文转化为专利描述的三阶段框架，通过模拟专家认知流程，显著提升逻辑连贯性和法律合规性


<details>
  <summary>Details</summary>
Motivation: 每年有超过350万件专利申请，将科学论文转化为专利描述极具挑战性，因为两者在修辞风格和法律要求上存在显著差异。现有的黑盒文本到文本方法难以建模结构推理和法律约束，需要一种更接近专家认知流程的方法。
...
TL;DR: 该研究开发了一个基于OpenIE的肺癌知识库构建流程，用于增强大语言模型在生物医学领域的性能，通过评估显示显著提升了模型的语义一致性和表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生物医学研究中的应用需要高质量的训练数据，特别是在肿瘤学这样对精确性和可解释性要求极高的领域。目前缺乏可扩展的方法来构建结构化知识库以进行有效的微调。

Method: 提出了一个四步流程：1) 使用MeSH词表识别医学概念；2) 筛选具有宽松许可(CC0)的PubMed开放获取文献；3) 使用OpenIE方法提取(主语、关系、宾语)三元组；4) 通过命名实体识别(NER)丰富三元组集以确保生物医学相关性。

Result: 构建了一个领域特定、大规模且具有噪声感知的知识资源。通过对T5模型进行监督语义微调，使用ROUGE和BERTScore评估显示，模型性能和语义连贯性得到显著提升。

Conclusion: OpenIE衍生的资源作为可扩展、低成本的解决方案，在增强生物医学自然语言处理方面具有巨大潜力，特别是在肺癌知识表示和推理任务中。
...
TL;DR: 本文提出了一种基于大语言模型的文档不一致性检测方法，通过引入新的证据提取指标和"编辑-重试"框架，显著改进了直接提示法的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在许多领域表现出色，但在文档不一致性检测方面的研究相对有限。文档不一致性检测包括两个关键方面：分类是否存在不一致性，以及提供不一致句子的证据。本文专注于后者，旨在改进基于LLM的文档不一致性检测方法。

Method: 提出了新的综合性证据提取指标，并引入了一个"编辑-重试"框架，该框架采用约束过滤机制。这种方法通过编辑文档内容并重新尝试检测，显著改进了直接提示法的性能。

Result: 实验结果显示，该方法在文档不一致性检测方面取得了有希望的结果，相比直接提示法有显著改进。

Conclusion: 本文提出的证据提取指标和"编辑-重试"框架有效提升了基于大语言模型的文档不一致性检测性能，为这一相对有限的研究领域提供了新的解决方案。
...
TL;DR: 本研究训练了多种编码器语言模型用于长文本自动评分，发现结合多个预训练语言模型表征的集成模型在长文本评分任务上显著优于单个语言模型。


<details>
  <summary>Details</summary>
Motivation: 长文本处理对仅编码器语言模型构成挑战，特别是在论文自动评分任务中。本研究旨在探索如何有效利用编码器模型进行长文本自动评分。

Method: 训练了BERT、RoBERTa、DistilBERT和DeBERTa等编码器模型，构建了基于多个编码器模型嵌入的集成模型，以及基于梯度提升决策树、XGBoost和LightGBM等特征监督机器学习模型的集成模型。在17,307篇论文的数据集上以80%/10%/10%的比例进行训练、验证和测试。

Result: 使用二次加权Kappa评估模型性能，发现结合多个预训练语言模型表征并以梯度提升分类器作为集成模型的嵌入集成模型在长文本评分任务上显著优于单个语言模型。

Conclusion: 对于长文本自动评分任务，集成多个预训练语言模型表征的方法比单个语言模型表现更好，为解决长文本处理中的编码器模型限制提供了有效方案。
...
TL;DR: LATS是一种无需攻击者LLM的越狱方法，通过词汇锚注入实现高攻击成功率，平均仅需约6.4次查询，显著优于其他方法。
...
Conclusion: 对话结构是一个强大且保护不足的攻击面，LATS在攻击成功率已普遍较高的时代展示了卓越的查询效率优势。
...
TL;DR: 研究显示，即使有安全措施，商业大语言模型仍可能提取受版权保护的训练数据，不同模型提取能力差异显著。


<details>
  <summary>Details</summary>
Motivation: 当前关于LLM与版权的法律争议焦点在于模型是否记忆训练数据以及能否提取这些数据。虽然普遍认为LLM不会大量记忆训练数据，但开源模型已被证实能提取大量受版权文本。然而，对于有安全措施的商业LLM是否也存在类似风险，仍是一个开放性问题。

Method: 采用两阶段方法：1) 初始探测阶段，有时使用Best-of-N越狱技术测试提取可行性；2) 迭代延续提示阶段，尝试提取整本书。评估了四个商业LLM（Claude 3.7 Sonnet、GPT-4.1、Gemini 2.5 Pro、Grok 3），使用基于块的最长公共子串近似方法（nv-recall）衡量提取成功率。

Result: 不同LLM的提取能力差异显著：Gemini 2.5 Pro和Grok 3无需越狱即可提取大量文本（《哈利波特与魔法石》的nv-recall分别为76.8%和70.3%）；Claude 3.7 Sonnet需要越狱但能输出近乎完整的书籍（nv-recall=95.8%）；GPT-4.1需要更多越狱尝试且最终会拒绝继续（nv-recall=4.0%）。

Conclusion: 即使有模型和系统层面的安全防护，商业LLM仍然存在提取受版权训练数据的风险，这对LLM的版权合规性提出了重要警示。
...
TL;DR: 提出了一种基于混合多域校准集和迭代校准策略的结构化剪枝框架，用于压缩大语言模型，保持硬件兼容性并最小化性能损失。


<details>
  <summary>Details</summary>
Motivation: 大语言模型规模不断增长带来了计算开销、内存占用和推理延迟等部署障碍。现有非结构化剪枝技术产生不规则稀疏模式，需要特殊硬件或软件支持，而结构化剪枝能消除整个架构组件并保持标准硬件兼容性。

Method: 提出新颖的结构化剪枝框架，利用混合多域校准集和迭代校准策略，有效识别和移除冗余通道。通过多领域数据校准和迭代优化来精确确定哪些通道可以被安全剪枝。

Result: 在各种模型和多样化下游任务上的广泛实验表明，该方法实现了显著压缩，同时性能损失最小。证明了结构化剪枝在保持硬件兼容性的同时有效减少模型规模。

Conclusion: 该结构化剪枝框架为解决大语言模型部署障碍提供了有效解决方案，通过混合多域校准和迭代策略实现了高效压缩，为实际应用中的模型部署提供了实用方法。
...
TL;DR: 本研究评估了三种基于Transformer的模型（BERT、RoBERTa、XLM-RoBERTa）在多语言情感分析和仇恨言论检测任务上的性能，涵盖英语、韩语、日语、中文和法语五种语言，并使用LIME框架增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 情感分析和仇恨言论检测对于在线内容审核至关重要，能够检测和缓解有害或冒犯性内容，从而为创建更安全的数字环境做出贡献。本研究旨在提高多语言情感分析和仇恨言论检测系统的有效性和透明度。

Method: 使用三种Transformer模型：BERT-base-multilingual-cased、RoBERTa-base和XLM-RoBERTa-base（前八层冻结），在五种语言（英语、韩语、日语、中文、法语）上进行评估。采用准确率、精确率、召回率和F1分数等标准性能指标，并集成LIME框架来增强模型可解释性。

Result: 通过标准性能指标比较了三种模型在多语言情感分析和仇恨言论检测任务上的表现，并使用LIME框架提供了模型预测行为的深入洞察，突出了单个单词对模型决策的贡献。
...
<details>
  <summary>Details</summary>
Motivation: 尽管问答系统在标准基准测试中表现优异，但在对抗样本面前仍很脆弱。本研究旨在系统评估Transformer模型在AddSent对抗数据集上的鲁棒性，并通过针对性缓解策略提升模型对抗攻击的能力。

Method: 1. 在ELECTRA-small到ELECTRA-base不同规模模型上进行系统实验；2. 使用五种互补分类方案进行多级错误分析；3. 系统评估对抗微调比例；4. 实施三种针对性缓解策略，包括实体感知对比学习；5. 结合命名实体识别引导的对比学习进行针对性缓解。

Result: 1. 识别否定混淆和实体替换为主要失败模式；2. 发现80%干净+20%对抗数据为最优微调比例；3. 小模型存在容量瓶颈；4. 从ELECTRA-small扩展到ELECTRA-base消除了鲁棒性-准确性权衡，在干净和对抗数据上均取得显著提升；5. 实体感知对比学习效果最佳：AddSent EM达89.89%，SQuAD EM达90.73%，对抗差距缩小94.9%。

Conclusion: 这是首个将全面语言错误分析与NER引导的对比学习相结合用于对抗问答的研究，证明针对性缓解策略能够使模型在干净和对抗数据上的性能达到接近一致的水平，为提升问答系统的对抗鲁棒性提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉问题，特别是提示诱导的幻觉，这影响了模型的准确性和可靠性。为了解决这一问题，需要开发能够提供更准确、结构化外部知识输入的方法来约束模型的推理过程。

Method: 基于知识蒸馏链式模型，引入代码模块来指导知识图谱探索，并将代码作为思维链提示的一部分，形成外部知识输入。开发了改进的知识蒸馏链式模型，用于分析和约束大语言模型的推理过程。
...
Conclusion: 提出的方法显著减少了大语言模型的幻觉行为，同时提高了模型的准确性和可验证性，为缓解大语言模型幻觉问题提供了有效解决方案。
...
TL;DR: 该研究通过计算模拟和自然语言验证，发现层次化处理能最优地解决工作记忆容量限制问题，解释了人类语言普遍具有层次结构的原因。


<details>
  <summary>Details</summary>
Motivation: 探究为什么人类语言普遍具有层次结构这一核心问题，研究假设层次化处理是为了最优地应对有限的工作记忆容量限制。

Method: 建立似然函数量化语言处理机制产生的平均单元数与人类工作记忆容量的匹配程度，通过符号序列的计算模拟和自然语言句子的验证分析，比较线性处理与层次处理在约束θ_MLE值方面的表现。

Result: 与线性处理相比，层次处理在约束θ_MLE值方面表现更优，能更好地保持在人类工作记忆容量限制内，且随着序列/句子长度的增加，这种优势更加明显，结果模式与儿童工作记忆发展相关。

Conclusion: 构建层次结构能优化序列语言输入的处理效率，同时保持在记忆约束范围内，这真正解释了人类语言普遍具有层次性质的原因。
...
<details>
  <summary>Details</summary>
Motivation: 标准检索增强方法无法解决长期智能体记忆的割裂性问题，需要超越静态向量相似度的统一记忆架构

Method: 引入Synapse架构，基于认知科学将记忆建模为动态图，采用侧向抑制和时间衰减机制，实现三重混合检索策略

Result: 在LoCoMo基准测试中，Synapse在复杂时序和多跳推理任务上显著优于现有最先进方法

Conclusion: Synapse通过动态激活传播而非预计算链接，为解决"上下文隧道"问题提供了鲁棒解决方案
...
TL;DR: WBC攻击方法通过滑动窗口和基于符号的聚合，利用局部化信号而非全局平均损失来提升LLM成员推理攻击效果


<details>
  <summary>Details</summary>
Motivation: 现有的大多数针对大语言模型的成员推理攻击依赖全局信号（如平均损失）来识别训练数据，但这种方法稀释了记忆的细微局部化信号，降低了攻击效果。作者挑战这种全局平均范式，认为成员信号在局部化上下文中更为显著。

Method: 提出WBC（基于窗口的比较）方法，采用滑动窗口和基于符号的聚合。该方法在不同大小的窗口上滑动文本序列，每个窗口基于目标模型和参考模型的损失比较进行二元投票决定成员身份。通过几何间隔的窗口大小集成投票，捕捉从令牌级别到短语级别的记忆模式。

Result: 在11个数据集上的广泛实验表明，WBC显著优于现有基线方法，获得更高的AUC分数，在低误报率阈值下检测率提高2-3倍。

Conclusion: 聚合局部化证据比全局平均更有效，暴露了微调LLM的关键隐私漏洞。WBC方法通过利用局部化信号显著提升了成员推理攻击的效果。
...
TL;DR: 提出了EComStage基准，用于评估LLM智能体在电商场景中的阶段性推理能力，包括感知、规划和行动三个阶段，覆盖客户和商家双重视角。
...
Method: 提出EComStage统一基准，评估LLM在感知（理解用户意图）、规划（制定行动计划）和行动（执行决策）三个阶段的推理能力。包含7个代表性任务，覆盖多样化电商场景，所有样本经过人工标注和质量检查。

Result: 评估了超过30个LLM（从1B到200B+参数），包括开源模型和闭源API，揭示了不同阶段和视角（客户导向vs商家导向）的具体优势和弱点。

Conclusion: EComStage提供了细粒度、可操作的见解，有助于在真实电商环境中设计和优化基于LLM的智能体，填补了现有基准的不足。
...
TL;DR: PHSA是一种基于标点符号感知的混合稀疏注意力框架，通过利用标点符号作为语义边界锚点，在保持核心语义结构的同时显著降低计算复杂度，在极低token激活率下仍能稳定工作。


<details>
  <summary>Details</summary>
Motivation: 密集注意力机制在处理长序列时存在二次复杂度问题，而现有稀疏注意力方法依赖粗粒度语义表示进行块选择，会模糊块内语义边界并导致关键信息丢失。

Method: 提出PHSA框架：1) 设计双分支聚合机制，融合全局语义表示和标点增强的边界特征；2) 引入极端稀疏自适应训练和推理策略，在极低token激活率下稳定模型行为。

Result: 在通用基准和长上下文评估中，PHSA始终优于密集注意力和最先进的稀疏注意力基线。对于0.6B参数模型和32k-token输入序列，在97.3%稀疏度下可将信息损失减少10.8%。

Conclusion: PHSA通过标点符号作为语义边界锚点，有效解决了现有稀疏注意力方法中语义边界模糊和信息丢失的问题，为长上下文建模提供了高效且准确的解决方案。
...
### [76] [TWIST: Training-free and Label-free Short Text Clustering through Iterative Vector Updating with LLMs](https://arxiv.org/abs/2510.06747)
...
Method: 基于迭代向量更新：首先基于代表性文本构建稀疏向量，然后通过LLM指导迭代优化，无需训练或标注数据，可应用于任何嵌入器。

Result: 在多样化数据集和小型LLM上实验表明，该方法性能与使用对比学习的最先进方法相当或更优，无需聚类数量或标签的先验知识，具有模型无关性且可扩展到大数据集。
...
### [77] [The performances of the Chinese and U.S. Large Language Models on the Topic of Chinese Culture](https://arxiv.org/abs/2601.02830)
*Feiyan Liu,Chenxun Zhuo,Siyan Zhao,Bao Ge,Tianming Liu*

Main category: cs.CL

TL;DR: 中美开发的大语言模型在中文文化理解任务上存在差异，中国模型在传统文化相关问题上表现更优
...
### [78] [To Generate or Discriminate? Methodological Considerations for Measuring Cultural Alignment in LLMs](https://arxiv.org/abs/2601.02858)
*Saurabh Kumar Pandey,Sougata Saha,Monojit Choudhury*

Main category: cs.CL

TL;DR: 该研究提出逆社会人口统计提示（ISDP）方法，通过让LLMs从用户行为中预测人口统计特征，来更准确地评估模型的文化能力，发现模型在真实行为上的表现优于模拟行为，但在个体层面表现有限。


<details>
  <summary>Details</summary>
Motivation: 传统的社会人口统计提示（SDP）方法在评估LLMs文化能力时存在混淆因素，如提示敏感性、解码参数和生成任务难度等，导致难以区分性能差是由于偏见还是任务设计问题。

Method: 使用逆社会人口统计提示（ISDP），让LLMs从真实和模拟的用户行为中预测人口统计特征。使用Goodreads-CSI数据集（包含印度、墨西哥和美国用户对英文书评的理解难度），测试了Aya-23、Gemma-2、GPT-4o和LLaMA-3.1四个模型。

Result: 模型在真实用户行为上的表现优于模拟行为，这与SDP的发现相反。但在个体层面，两种行为类型的表现都下降且趋于相等，表明个性化能力存在局限。

Conclusion: ISDP提供了更可靠的LLMs文化能力评估方法，揭示了模型在真实与模拟行为上的差异，并指出了个性化能力的局限性。
...
TL;DR: 本文提出了一种名为"同形词元"的数据增强方法，通过使用相同词汇的不同有效子词分割来训练语言模型，以增强其对分词变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 子词分词在语言模型中引入了一个计算层，其中许多不同的标记序列可以解码为相同的表面形式并保留意义，但会引发不同的内部计算。尽管存在这种非唯一性，语言模型通常只使用单一的标准最长前缀分词进行训练。

Method: 提出同形词元作为严格保持意义的数据增强形式，引入轻量级训练架构，通过辅助因果编码器和块因果交叉注意力，在采样同形词元变体上调节标准下一个标记预测，而不修改训练目标或标记接口。

Result: 在数据受限的预训练中，同形词元增强持续延迟了重复数据暴露下的过拟合，并改善了跨多样化评估数据集的泛化能力。在多语言微调中，同形词元的有效性取决于分词器质量：当标准标记高度压缩时增益最强，当分词器已经过度分割输入时增益减弱。

Conclusion: 同形词元为在语言模型中引入分词不变性提供了一种简单且模块化的机制。
...
TL;DR: LongBench Pro是一个更现实、全面的双语长上下文基准测试，包含1500个自然长上下文样本，支持8k到256k tokens的输入长度，覆盖11个主要任务和25个次要任务。


<details>
  <summary>Details</summary>
Motivation: 当前长上下文基准测试在可扩展性和真实性之间存在权衡：合成任务无法代表真实世界复杂性，而完全手动标注成本高昂，难以扩展到极端长度和多样化场景。

Method: 提出人类-模型协作构建流程：前沿LLMs起草具有挑战性的问题和参考答案，专家进行严格验证和修正。基准包含任务特定指标和多维分类法（上下文需求、长度、难度）。

Result: 评估46个广泛使用的长上下文LLMs发现：(1)长上下文优化比参数缩放对长上下文理解贡献更大；(2)有效上下文长度通常短于声称长度，存在显著的跨语言不对齐；(3)"思考"范式主要帮助原生推理训练的模型。

Conclusion: LongBench Pro为推进长上下文理解提供了一个稳健的测试平台，通过平衡质量与可扩展性，支持对长上下文LLMs进行更现实的评估。
...
TL;DR: 本文探索使用大型语言模型进行数据压缩，在enwik9数据集上实现了约18%的调整压缩率，达到新的SOTA，同时研究了LLM在非英语数据、代码数据和字节流序列上的压缩能力。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究显示LLM在文本和多模态数据压缩方面有潜力，但在实际应用中仍存在挑战。本文旨在探索如何利用LLM实现更低的调整压缩率，并研究其在各种数据类型上的压缩能力。

Method: 探索不同的方法使用LLM作为数据压缩器，无需额外模型训练，通过优化配置来提升压缩性能，特别关注非自然文本序列的压缩。

Result: 在enwik9数据集上实现了约18%的调整压缩率，达到新的最先进水平。同时证明LLM在文本主导领域表现出色，在非自然文本序列（如代码、字节流）上通过适当配置也能保持竞争力。

Conclusion: LLM在数据压缩任务中具有显著潜力，特别是在文本主导领域。通过适当的配置，LLM在压缩非自然文本序列方面也能保持竞争力，为LLM作为通用数据压缩器的应用提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 多语言语音基础模型（如Whisper）在网页规模数据上训练，每种语言包含多种区域变体，不同区域变体使用不同文字脚本书写同一语言，导致语音识别输出脚本存在不确定性

Method: 发现脚本在多语言语音模型的激活空间中呈线性编码，通过在推理时修改激活向量实现对输出脚本的直接控制，添加脚本向量可诱导脚本变化

Result: 该方法能够在非常规语言-脚本配对中诱导脚本变化（如意大利语用西里尔字母、日语用拉丁字母），在Whisper所有模型大小上都表现出竞争性性能

Conclusion: 通过激活空间中的脚本向量操作，实现了对语音识别输出脚本的后验控制，为解决多语言语音模型中脚本不确定性问题提供了有效方法
...
### [83] [Beyond the Black Box: Theory and Mechanism of Large Language Models](https://arxiv.org/abs/2601.02907)
...
TL;DR: 这篇综述论文提出了一个基于生命周期的分类法，将LLM研究分为六个阶段，系统回顾了理论基础和内部机制，旨在将LLM开发从工程启发式转向原则性科学学科。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM取得了巨大的工程成功，但理论理解仍然相对滞后，导致这些系统在很大程度上被视为"黑箱"。为了解决这种理论碎片化问题，需要建立一个统一的理论框架来理解LLM的内部机制。

Method: 提出了一个基于生命周期的分类法，将LLM研究组织成六个阶段：数据准备、模型准备、训练、对齐、推理和评估。在这个框架下，系统回顾了驱动LLM性能的基础理论和内部机制。

Result: 分析了核心理论问题，如数据混合的数学合理性、各种架构的表征极限、对齐算法的优化动态等。识别了前沿挑战，包括合成数据自我改进的理论极限、安全保证的数学边界、涌现智能的机制起源。

Conclusion: 通过将经验观察与严谨的科学探究相结合，这项工作为将LLM开发从工程启发式转向原则性科学学科提供了结构化路线图。
...
### [84] [Image, Word and Thought: A More Challenging Language Task for the Iterated Learning Model](https://arxiv.org/abs/2601.02911)
*Hyoyeon Lee,Seth Bullock,Conor Houghton*

Main category: cs.CL

TL;DR: 迭代学习模型通过模拟代际语言传播，探索语言传输约束如何促进语言结构的涌现。该模型在空白状态学习者面临有限语言输入瓶颈的情况下，能够产生无歧义、有语法规则且代际稳定的语言。


<details>
  <summary>Details</summary>
Motivation: 研究语言传播过程中的约束条件如何促进语言结构的自然涌现，特别是探索在复杂意义空间下语言表达性、组合性和稳定性的形成机制。

Method: 采用半监督迭代学习模型，结合监督学习和无监督学习，使用自动编码器架构。模型应用于七段数码管图像通信任务，涉及128个字符的复杂意义空间。

Result: 智能体能够学习并传播具有表达性（为所有128个字符使用不同编码）、组合性（信号组件与意义组件一致映射）和稳定性（代际间语言不变）的语言。

Conclusion: 迭代学习模型成功应用于复杂意义空间的语言学习任务，证实了语言传输瓶颈能够促进表达性、组合性和稳定性语言结构的自然涌现。
...
### [85] [RAL2M: Retrieval Augmented Learning-To-Match Against Hallucination in Compliance-Guaranteed Service Systems](https://arxiv.org/abs/2601.02917)
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的服务系统中幻觉是一个主要问题，需要显式知识基础来保证合规响应。传统生成方法容易产生幻觉，需要更可靠的方法。

Method: 提出检索增强学习匹配(RAL2M)框架，将LLM重新定位为检索系统中的查询-响应匹配判断器，而非生成器。采用查询自适应潜在集成策略，显式建模异构模型能力和LLM间的相互依赖关系，获得校准的共识决策。

Result: 在大规模基准测试上的广泛实验表明，该方法有效利用了"群体智慧"，显著优于强基线方法。

Conclusion: RAL2M为纯生成方法提供了稳健的替代方案，讨论了在未来的工作中进一步利用潜在表示的最佳实践和有前景的方向。
...
### [86] [Memorization, Emergence, and Explaining Reversal Failures: A Controlled Study of Relational Semantics in LLMs](https://arxiv.org/abs/2601.02931)
*Yihua Zhu,Qianying Liu,Jiaxin Wang,Fei Cheng,Chaoran Liu,Akiko Aizawa,Sadao Kurohashi,Hidetoshi Shimodaira*

Main category: cs.CL

TL;DR: 研究发现自回归大语言模型在关系任务中表现良好，但存在反转失败问题。通过知识图谱框架实验发现，关系语义在足够逻辑监督下会突然出现，而反转失败主要源于自回归顺序偏差而非语义缺陷。


<details>
  <summary>Details</summary>
Motivation: 探究自回归大语言模型是否真正学习到关系词的逻辑语义（如对称性和逆逻辑），以及反转类型失败是由于缺少关系语义还是自左向右的顺序偏差导致的。

Method: 提出基于知识图谱的合成框架，从对称/逆三元组生成文本，从头训练GPT风格的自回归模型，评估记忆能力、逻辑推理能力和对未见实体的上下文泛化能力。

Result: 发现关系语义在足够逻辑监督下会出现明显的相变现象，即使在浅层（2-3层）模型中也能实现；成功泛化与稳定的中间层信号相关；顺序匹配的前向/反向测试和扩散基线表明反转失败主要受自回归顺序偏差驱动而非逆语义缺陷。

Conclusion: 自回归大语言模型能够学习关系逻辑语义，但反转失败问题主要源于模型的自回归顺序偏差，而非语义理解缺陷。这为理解模型在关系推理任务中的表现提供了重要见解。
...
### [87] [Pearmut: Human Evaluation of Translation Made Trivial](https://arxiv.org/abs/2601.02933)
*Vilém Zouhar,Tom Kocmi*

Main category: cs.CL

TL;DR: Pearmut是一个轻量级但功能丰富的人类评估平台，旨在简化多语言NLP任务中的人类评估流程，使其像自动评估一样容易运行。


<details>
  <summary>Details</summary>
Motivation: 人类评估是多语言NLP的黄金标准，但由于现有工具设置复杂、工程和操作开销大，实践中常被跳过并用自动指标替代。需要简化人类评估流程以使其成为模型开发和诊断的常规组成部分。

Method: 开发了Pearmut平台，支持多语言任务评估（特别是机器翻译），实现了标准评估协议（DA、ESA、MQM），支持文档级上下文、绝对和对比评估、注意力检查、ESAAI预标注以及静态和基于主动学习的分配策略。

Result: Pearmut移除了常见的进入障碍，使端到端人类评估变得容易运行，支持新协议的扩展原型设计，让可靠的人类评估成为实际可行的常规组件。

Conclusion: Pearmut平台使可靠的人类评估能够成为模型开发和诊断的实际、常规组成部分，而不仅仅是偶尔的努力，解决了人类评估在实践中的复杂性和缓慢设置问题。
...
### [88] [Enhancing Multilingual RAG Systems with Debiased Language Preference-Guided Query Fusion](https://arxiv.org/abs/2601.02956)
*Jeonghyun Park,Byeongjeong Kim,Seojin Hwang,Hwanhee Lee*

Main category: cs.CL

TL;DR: 研究发现多语言检索增强生成系统对英语的偏好主要是评估基准的结构性偏差所致，而非模型固有偏见，提出新的去偏指标DeLP和优化框架DELTA


<details>
  <summary>Details</summary>
Motivation: 多语言RAG系统普遍表现出对高资源语言（特别是英语）的偏好，但研究发现这种测量被评估基准的结构性先验所扭曲，需要准确评估真正的语言偏好

Method: 提出DeLP（去偏语言偏好）指标来显式排除结构性混杂因素，并基于研究发现引入DELTA框架，利用单语对齐优化跨语言检索和生成

Result: 分析显示先前报告的英语偏好主要是证据分布的结果而非模型固有偏见，检索器本质上偏好查询和文档语言之间的单语对齐，DELTA框架在多种语言上优于英语枢纽和多语言RAG基线

Conclusion: 多语言RAG系统的语言偏好问题主要是评估偏差所致，通过去偏指标和利用单语对齐的优化框架可以显著提升跨语言检索和生成性能
...
### [89] [LLM-Augmented Changepoint Detection: A Framework for Ensemble Detection and Automated Explanation](https://arxiv.org/abs/2601.02957)
...
<details>
  <summary>Details</summary>
Motivation: 解决当前变点检测领域的两个关键限制：1) 单一检测方法在不同数据特征下表现互补，方法选择困难且易导致次优结果；2) 缺乏对检测到的变化的自动化、上下文解释

Method: 集成十种不同的变点检测算法，结合LLM驱动的解释管道自动生成上下文叙述，对于私有或领域特定数据采用检索增强生成(RAG)解决方案

Result: 集成方法相比单一方法表现出更优的性能和鲁棒性，LLM解释管道能够将检测到的变点与现实历史事件联系起来，开源Python框架在金融、政治科学、环境科学等多个领域展示了实用价值

Conclusion: 该框架将原始统计输出转化为分析师和决策者可操作的见解，解决了变点检测中的方法选择和解释性挑战
...
TL;DR: 本研究针对巴拿语（Bahnar）文档数字化需求，提出结合表格/非表格检测与概率后处理的OCR改进方法，将识别准确率从72.86%提升至79.26%，为少数民族语言保护提供技术框架。


<details>
  <summary>Details</summary>
Motivation: 巴拿语作为越南、柬埔寨和老挝的少数民族语言，面临保存挑战，现有研究数据有限。扫描文档因图像质量退化导致OCR错误率高，影响信息检索系统，急需提高数字化准确性。

Method: 提出综合方法：1）应用先进的表格和非表格检测技术改善输入数据质量；2）采用基于概率的后处理启发式方法对OCR输出进行错误校正。

Result: 实验结果显示识别准确率显著提升，从72.86%提高到79.26%，验证了方法的有效性。

Conclusion: 本研究为巴拿语保护提供了有价值的资源，提出的框架可推广到其他少数民族语言的数字化工作中，有助于语言文化遗产的保存。
...
### [91] [Correct, Concise and Complete: Multi-stage Training For Adaptive Reasoning](https://arxiv.org/abs/2601.02972)
*Nathanaël Carraz Rakotonirina,Ren Pang,Neha Anna John,Michael Bohlke-Schneider,Momchil Hardalov*

Main category: cs.CL

TL;DR: 提出一种多阶段高效推理方法，通过监督微调和强化学习结合自适应长度惩罚，减少大语言模型推理过程中的过度思考现象，在保持精度的同时显著缩短响应长度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过增加推理时间计算（如思维链）提升了推理能力，但思维链往往变得不必要地冗长，增加了计算成本却没有实际精度提升，有时甚至降低性能，这种现象被称为"过度思考"。

Method: 提出多阶段高效推理方法：1）通过拒绝采样或推理轨迹重新格式化的监督微调；2）使用自适应长度惩罚的强化学习；3）引入轻量级奖励函数，惩罚第一个正确答案后生成的标记，仅在有益时鼓励自我验证。

Result: 在七个不同推理任务上进行评估：8B模型响应长度平均减少28%，32B模型减少40%，性能仅分别下降1.6和2.5个百分点。在过度思考调整精度曲线下面积（AUC_OAA）上达到76.6分，比基础模型高5分，比次优方法高2.5分。

Conclusion: 该方法在概念简单的情况下，相比更复杂的现有高效推理方法实现了更优的权衡，有效解决了大语言模型推理中的过度思考问题，在保持精度的同时显著减少计算成本。
...
### [92] [Mechanistic Knobs in LLMs: Retrieving and Steering High-Order Semantic Features via Sparse Autoencoders](https://arxiv.org/abs/2601.02978)
*Ruikang Zhang,Shuo Wang,Qi Su*

Main category: cs.CL

TL;DR: 提出基于稀疏自编码器的框架，用于检索和操控与高级语言行为相关的语义可解释内部特征，通过对比特征检索管道实现模型行为的精确双向操控。


<details>
  <summary>Details</summary>
Motivation: 尽管机制可解释性研究能够识别和干预大语言模型的内部特征，但将这些内部特征与语言生成中复杂行为级语义属性的可靠控制联系起来仍然是一个持续挑战。

Method: 采用基于稀疏自编码器的框架，使用基于受控语义对立的对比特征检索管道，结合统计激活分析和基于生成的验证，从稀疏激活空间中提取单语义功能特征。

Result: 以大五人格特质为案例研究，该方法能够精确双向操控模型行为，相比现有激活操控方法（如对比激活加法）具有更好的稳定性和性能。发现了"功能忠实性"经验效应。
...
### [93] [P-Check: Advancing Personalized Reward Model via Learning to Generate Dynamic Checklist](https://arxiv.org/abs/2601.02986)
...
Method: 提出P-Check框架，训练即插即用检查清单生成器来合成动态评估标准；引入偏好对比标准加权训练策略，根据标准在个性化判断中的区分能力分配显著性分数。

Result: P-Check不仅提高了奖励准确性，还增强了下游个性化生成性能，并在OOD场景中保持鲁棒性。

Conclusion: P-Check通过动态检查清单生成和偏好对比加权策略，有效解决了现有个性化奖励建模方法的局限性，实现了更好的个性化对齐。
...
### [94] [Mechanistic Interpretability of Large-Scale Counting in LLMs through a System-2 Strategy](https://arxiv.org/abs/2601.02989)
...
TL;DR: 提出一种受System-2认知过程启发的测试时策略，将大计数任务分解为小任务，使大语言模型能超越架构限制，实现高精度大规模计数。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然在复杂数学问题上表现良好，但在计数任务中存在系统性限制。这是由于Transformer架构的限制，计数在多层中进行，导致较大计数问题时精度下降。

Method: 提出受System-2认知过程启发的测试时策略，将大计数任务分解为更小、独立的子问题。使用观察性和因果中介分析来理解这种System-2策略的底层机制。

Result: 机制分析识别出关键组件：潜在计数在每个部分的最终项表示中计算和存储，通过专用注意力头传输到中间步骤，并在最后阶段聚合产生总计数。实验结果表明该策略使LLMs能够超越架构限制，在大规模计数任务上实现高精度。
...
### [95] [Stable-RAG: Mitigating Retrieval-Permutation-Induced Hallucinations in Retrieval-Augmented Generation](https://arxiv.org/abs/2601.02993)
*Qianchi Zhang,Hainan Zhang,Liang Pang,Hongwei Zheng,Zhiming Zheng*

Main category: cs.CL

TL;DR: 论文提出Stable-RAG方法，通过估计检索文档排列的敏感性来减少排列引起的幻觉，提高RAG系统的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成（RAG）方法虽然能减少大语言模型的事实幻觉，但忽视了检索文档排列顺序对模型行为的影响。研究发现即使黄金文档固定在第一位置，不同排列顺序仍会导致模型答案显著变化，这揭示了之前未被充分探索的排列敏感性。

Method: 提出Stable-RAG方法：1）在多种检索顺序下运行生成器；2）对隐藏状态进行聚类；3）从捕捉主导推理模式的聚类中心表示进行解码；4）利用这些推理结果将幻觉输出对齐到正确答案，鼓励模型在不同文档排列下产生一致且准确的预测。

Result: 在三个QA数据集上的实验表明，与基线方法相比，Stable-RAG显著提高了答案准确性、推理一致性，并在不同数据集、检索器和输入长度上展现出更强的鲁棒泛化能力。

Conclusion: Stable-RAG通过解决检索文档排列敏感性问题，有效减少了RAG系统中的幻觉现象，提高了模型的稳定性和可靠性，为构建更鲁棒的检索增强生成系统提供了新思路。
...
### [96] [Large Reasoning Models Are (Not Yet) Multilingual Latent Reasoners](https://arxiv.org/abs/2601.02996)
*Yihong Liu,Raoyuan Zhao,Hinrich Schütze,Michael A. Hedderich*

Main category: cs.CL

TL;DR: 该研究发现大型推理模型在多语言环境中存在潜在推理能力，但这种能力在不同语言间分布不均，且总体上遵循以英语为中心的推理路径。


<details>
  <summary>Details</summary>
Motivation: 虽然大型推理模型在数学推理任务上表现出色，通常归因于其生成显式思维链解释的能力，但最近研究表明模型在完成文本推理步骤前就能得出正确答案，表明存在潜在推理。然而，这种现象在英语中已有探索，其多语言行为仍未知，因此需要系统研究多语言潜在推理。

Method: 采用基于截断的策略，在11种语言中研究多语言潜在推理。通过仅给模型部分推理痕迹，检查正确答案如何随着推理步骤的推进而逐渐形成，从而测量逐步潜在预测形成。进一步进行表征分析，以了解不同语言间的差异是否反映不同的内部机制。

Result: 研究发现了多语言潜在推理的明确证据，但这种能力分布不均：在资源丰富的语言中较强，在低资源语言中较弱，在更难的基准测试中普遍较难观察到。尽管表面存在差异，但预测的内部演化在不同语言间高度一致，且与英语模式基本一致，表明存在以英语为中心的潜在推理路径。

Conclusion: 大型推理模型在多语言环境中确实表现出潜在推理能力，但这种能力在不同语言间存在不平衡性，且总体上遵循以英语为中心的推理模式，揭示了模型内部推理机制的跨语言一致性。
...
### [97] [SentGraph: Hierarchical Sentence Graph for Multi-hop Retrieval-Augmented Question Answering](https://arxiv.org/abs/2601.03014)
*Junli Liang,Pengfei Zhou,Wangqiu Zhou,Wenjie Qing,Qi Zhao,Ziwen Wang,Qi Song,Xiangyang Li*

Main category: cs.CL

TL;DR: SentGraph：基于句子级图结构的RAG框架，通过建模句子间逻辑关系解决多跳问答中的证据链不完整问题


<details>
  <summary>Details</summary>
Motivation: 传统RAG在单跳问答中有效，但在多跳问答中存在局限性。现有的基于分块的检索方法经常提供不相关和逻辑不连贯的上下文，导致证据链不完整和推理错误。

Method: 提出SentGraph框架：1）离线构建层次化句子图，采用修辞结构理论区分核心句和卫星句，组织成主题级子图并建立跨文档实体桥接；2）在线检索时执行图引导的证据选择和路径扩展，检索细粒度的句子级证据。

Result: 在四个多跳问答基准测试上的广泛实验证明了SentGraph的有效性，验证了显式建模句子级逻辑依赖关系对多跳推理的重要性。

Conclusion: SentGraph通过显式建模句子间逻辑关系，解决了传统RAG在多跳问答中的局限性，为多跳推理提供了更完整的证据链支持。
...
### [98] [MMFormalizer: Multimodal Autoformalization in the Wild](https://arxiv.org/abs/2601.03017)
...
TL;DR: MMFormalizer是首个多模态自动形式化方法，通过自适应接地和递归构造，将自然语言数学与视觉元素结合，支持从经典力学到相对论、量子力学和热力学的物理领域形式化。


<details>
  <summary>Details</summary>
Motivation: 传统自动形式化方法主要处理文本数学，但在现实世界中，物理问题通常涉及从视觉元素推断隐藏约束（如质量或能量），需要多模态方法将感知与形式推理相结合。

Method: MMFormalizer通过自适应接地整合真实世界数学和物理领域的实体，通过递归接地和公理组合从感知接地基元递归构造形式命题，自适应递归终止确保每个抽象都有视觉证据支持并锚定在维度或公理基础上。

Result: 在包含115个样本的新基准PhyX-AF上评估，前沿模型如GPT-5和Gemini-3-Pro在编译和语义准确性方面表现最佳，GPT-5在物理推理方面表现突出，几何领域仍然最具挑战性。

Conclusion: MMFormalizer为统一的多模态自动形式化提供了可扩展框架，连接了感知与形式推理，是首个能够处理经典力学、相对论、量子力学和热力学的多模态自动形式化方法。
...
### [99] [Dementia-R1: Reinforced Pretraining and Reasoning from Unstructured Clinical Notes for Real-World Dementia Prognosis](https://arxiv.org/abs/2601.03018)
*Choonghan Kim,Hyunmin Hwang,Hangeol Chang,Jaemin Kim,Jinse Park,Jae-Sung Lim,Jong Chul Ye*

Main category: cs.CL

TL;DR: Dementia-R1：一种基于强化学习的框架，用于从非结构化临床记录中进行纵向痴呆症预后预测，通过冷启动RL策略预训练模型预测可验证的临床指标，在真实数据集上达到77.03%的F1分数


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床文本理解方面表现良好，但在痴呆症预后等纵向预测任务上存在困难，这些任务需要对多个就诊记录中的复杂、非单调症状轨迹进行推理。标准监督训练缺乏症状演变的显式标注，而直接强化学习又受到稀疏二元奖励的阻碍。

Method: 提出Dementia-R1框架，采用冷启动强化学习策略：首先预训练模型从患者历史记录中预测可验证的临床指标，增强模型对疾病进展的推理能力，然后再确定最终的临床状态。这种方法在稀疏奖励环境下更有效地训练模型。

Result: 在真实世界非结构化临床数据集上达到77.03%的F1分数。在ADNI基准测试中，7B参数的模型性能可与GPT-4o相媲美，能够有效捕捉波动的认知轨迹。

Conclusion: Dementia-R1通过冷启动强化学习策略成功解决了纵向痴呆症预后预测的挑战，在稀疏奖励环境下实现了有效的模型训练，为临床纵向预测任务提供了新的解决方案。
...
### [100] [MedDialogRubrics: A Comprehensive Benchmark and Evaluation Framework for Multi-turn Medical Consultations in Large Language Models](https://arxiv.org/abs/2601.03023)
...
TL;DR: MedDialogRubrics是一个包含5200个合成患者病例和60000多个细粒度评估标准的基准测试，专门用于评估大语言模型的多轮诊断能力，通过多智能体系统生成病例并确保临床合理性。


<details>
  <summary>Details</summary>
Motivation: 现有的医学对话AI评估基准和框架在信息收集和诊断推理能力方面缺乏严格评估，需要开发更可靠的评估工具来推动更安全有效的医学对话系统发展。

Method: 采用多智能体系统合成患者病例，包括设计有限原子医学事实的患者智能体，并配备动态指导机制检测和纠正幻觉；提出结构化的大语言模型和专家标注的标准生成流程，检索循证医学指南并使用拒绝采样确定"必问"项目。

Result: 对最先进模型进行全面评估发现，当前模型在多个评估维度面临重大挑战，表明改进医学对话需要对话管理架构的进步，而不仅仅是基础模型的增量调整。

Conclusion: MedDialogRubrics为评估医学大语言模型的多轮诊断能力提供了可靠基准，揭示了当前模型的局限性，并指出未来需要关注对话管理架构的创新而非单纯的基础模型调优。
...
TL;DR: F-DPO是一种基于事实感知的直接偏好优化方法，通过在DPO中引入二元事实性标签和标签翻转转换，有效减少大语言模型的幻觉问题，提升事实准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的偏好对齐方法（如RLHF和DPO）虽然能提升指令跟随能力，但可能强化幻觉问题，因为这些方法倾向于奖励流畅性和置信度而非事实正确性。

Method: F-DPO是DPO的扩展，使用二元事实性标签：(1)应用标签翻转转换，确保选择的回答不比拒绝的回答事实性差；(2)添加事实感知边界，强调有明显正确性差异的配对，当两个回答事实性相同时还原为标准DPO。通过为DPO配对添加二元事实性指示器和合成幻觉变体来构建事实感知偏好数据。

Result: 在七个开源大语言模型（1B-14B）上，F-DPO相比基础模型和标准DPO持续提升事实性并减少幻觉率。在Qwen3-8B上，幻觉率降低五倍（从0.424到0.084），事实性得分提升50%（从5.26到7.90）。在TruthfulQA基准测试中，Qwen2.5-14B的MC1准确率提升17%（0.500到0.585），MC2准确率提升49%（0.357到0.531）。

Conclusion: F-DPO是一种简单有效的方法，无需辅助奖励模型、token级标注或多阶段训练，就能显著提升大语言模型的事实准确性并减少幻觉问题，具有良好的泛化能力。
...
TL;DR: 挪威语NLP资源不足，NorwAI团队开发了专门针对挪威语和斯堪的纳维亚语言的大语言模型家族，基于多种Transformer架构，通过预训练和指令调优优化性能，并向北欧组织开放使用。


<details>
  <summary>Details</summary>
Motivation: 挪威语（约500万人使用）在自然语言处理领域代表性不足，需要专门的语言模型来填补这一空白，支持挪威语和斯堪的纳维亚语言的NLP应用发展。

Method: 基于GPT、Mistral、Llama2、Mixtral和Magistral等多种Transformer架构，从零开始预训练或持续预训练（25B-88.45B tokens），使用挪威语扩展的分词器，采用先进的训练后策略优化性能、增强鲁棒性和适应性。

Result: 开发了专门针对挪威语和斯堪的纳维亚语言的模型家族，指令调优变体（如Mistral-7B-Instruct和Mixtral-8x7B-Instruct）展现出强大的助手式能力，适合实际部署到交互式和特定领域应用中。

Conclusion: NorwAI大语言模型填补了挪威语NLP资源的空白，为北欧组织、公司和学生提供开放的研究和实验使用资源，推动了挪威语和斯堪的纳维亚语言NLP技术的发展。
...
Method: 提出两种方法：1) BaseCal-ReEval：将后训练LLM的响应输入基础LLM获取平均概率作为置信度；2) BaseCal-Proj：训练轻量级投影将后训练LLM的最终层隐藏状态映射回基础LLM的隐藏状态，然后通过基础LLM的输出层获得校准后的置信度
...
TL;DR: 研究发现稀疏注意力算法在解码阶段可能反而增加端到端复杂度，因为信息损失导致序列显著变长，作者提出早期停止算法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署规模扩大对推理效率要求提高，现有工作将推理分解为预填充和解码阶段，解码阶段主导总延迟。稀疏注意力算法旨在降低解码阶段的时间和内存复杂度，但作者发现这可能产生反效果。

Method: 通过实证和理论分析发现稀疏注意力会导致"少即是少"现象，即信息损失导致序列显著变长。提出早期停止算法，在稀疏解码过程中检测信息损失超过信息增益的阈值。

Result: 早期停止算法在推理密集型基准测试中，将token消耗减少高达90%，而准确率下降不到2%。

Conclusion: 稀疏注意力算法可能因信息损失导致序列变长而增加端到端复杂度，提出的早期停止算法能有效缓解这一问题，显著减少计算开销同时保持模型性能。
...
TL;DR: 本文提出了一种基于语义级内部推理图的忠实性幻觉检测方法，通过扩展层相关传播算法到语义级别，构建基于归因向量的内部推理图，并设计基于小型预训练语言模型的通用框架来利用LLM推理中的依赖关系进行训练和幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的检索增强生成系统虽然能有效减少事实性幻觉，但忠实性幻觉仍然存在。现有的忠实性幻觉检测方法要么忽略了捕捉模型的内部推理过程，要么对这些特征处理粗糙，使得判别器难以学习。
...
Conclusion: 提出的语义级内部推理图方法能够有效检测检索增强生成系统中的忠实性幻觉，通过更精细地捕捉LLM的内部推理过程，提高了幻觉检测的性能。
...
Method: 提出贪婪剪枝方法：迭代删除推理标记，选择那些删除后对指定目标下的模型似然影响最小的标记，生成长度可控的推理链。

Result: 在蒸馏框架中评估剪枝推理，结果显示在相同推理长度下，使用剪枝链训练的学生模型优于前沿模型监督的压缩基线。分析显示系统性的剪枝模式，注意力分数可以预测贪婪剪枝排名。

Conclusion: 模型在推理标记上编码了非平凡的功能重要性结构，贪婪剪枝方法有效压缩推理链同时保持模型性能，为理解模型内部推理机制提供了诊断工具。
...
TL;DR: 该论文提出了两种语用推理方法，用于增强大语言模型的道德敏感性，使其能够诊断道德良性和危险输入并纠正道德错误。


<details>
  <summary>Details</summary>
Motivation: 道德敏感性是人类道德能力的基础，指导个体调节日常行为。虽然已有许多方法试图将大语言模型与人类道德价值观对齐，但如何使其具备道德敏感性一直极具挑战性。本文旨在回答：如何增强大语言模型的道德敏感性？

Method: 提出了两种语用推理方法：1）促进大语言模型诊断道德良性和危险输入；2）纠正道德错误。这些方法采用统一视角，不建模语义多样且复杂的表面形式，而是基于推理负荷设计原则性的语用推理程序。

Result: 实证证据表明，提出的语用方法能够增强大语言模型的道德敏感性，并在代表性的道德相关基准测试中取得强劲性能。

Conclusion: 通过语用推理方法增强大语言模型的道德敏感性是可行的，这为解决大语言模型与人类道德价值观对齐的挑战提供了新的思路。
...
TL;DR: 本文提出了Grad-ELLM，一种针对仅解码器Transformer架构大语言模型的梯度归因方法，通过聚合注意力层梯度和注意力图生成热力图，并引入改进的忠实度评估指标。
...
Method: 提出了Grad-ELLM方法：1）通过聚合输出logit相对于注意力层的梯度来计算通道重要性；2）从注意力图中提取空间重要性；3）结合两者在每个生成步骤生成热力图，无需修改模型架构。同时提出了改进的忠实度评估指标π-Soft-NC和π-Soft-NS。

Result: 在情感分类、问答和开放生成任务上使用不同模型进行评估，实验结果显示Grad-ELLM在忠实度方面始终优于其他归因方法。

Conclusion: Grad-ELLM为仅解码器Transformer架构的大语言模型提供了一种有效的梯度归因方法，能够在不修改架构的情况下生成高质量的归因热力图，并通过改进的评估指标实现了更公平的忠实度比较。
...
<details>
  <summary>Details</summary>
Motivation: 幽默偏好因个人和文化差异而高度异质，这使得使用大语言模型评估幽默变得复杂。研究旨在建模这种异质性，理解不同用户群体的幽默偏好模式。

Method: 1. 使用日本创意回应游戏Oogiri的用户投票日志进行聚类分析；2. 应用Bradley-Terry-Luce模型估计各聚类对可解释偏好因子的权重；3. 通过提示大语言模型选择更有趣的回应来获取其偏好判断；4. 使用角色提示引导大语言模型模拟特定用户群体的偏好。

Result: 1. 用户聚类展现出不同的偏好模式；2. 大语言模型的偏好结果可以与特定用户聚类相似；3. 通过角色提示，可以引导大语言模型的偏好朝向特定聚类。

Conclusion: 该研究成功建模了幽默偏好的异质性，并证明大语言模型能够模拟特定用户群体的幽默偏好，为个性化幽默评估提供了方法。研究将发布数据收集和分析脚本以支持可重复性。
...
TL;DR: 该研究首次对大型音频语言模型中的情感敏感神经元进行神经元级可解释性分析，通过推理时干预揭示了情感决策的因果机制


<details>
  <summary>Details</summary>
Motivation: 情感是语音交流的核心维度，但目前缺乏对现代大型音频语言模型如何内部编码情感的机制性解释

Method: 在Qwen2.5-Omni、Kimi-Audio和Audio Flamingo 3三个开源模型中，比较基于频率、熵、幅度和对比度的神经元选择器，使用推理时干预技术（消融和增益放大）分析情感敏感神经元

Result: 发现一致的情感特异性特征：消融特定情感选择的神经元会不成比例地降低该情感的识别准确率，而增益放大则会将预测转向目标情感；情感敏感神经元表现出非均匀的层间聚类和部分跨数据集迁移

Conclusion: 研究为LALMs中的情感决策提供了因果、神经元级的解释，并强调有针对性的神经元干预作为可控情感行为的可行操作手段
...
TL;DR: ToxiGAN：结合对抗生成与LLM语义引导的类别感知文本增强框架，用于毒性分类，通过两阶段定向训练和语义锚定解决GAN模式崩溃问题，在四个仇恨言论基准测试中表现最优。


<details>
  <summary>Details</summary>
Motivation: 毒性语言数据的可控、类别特异性增强对于提升毒性分类的鲁棒性至关重要，但由于监督有限和分布偏斜，这一任务仍然具有挑战性。现有方法在模式崩溃和语义漂移方面存在问题。

Method: 提出ToxiGAN框架：1）结合对抗生成与大型语言模型（LLMs）的语义引导；2）引入两阶段定向训练策略解决模式崩溃；3）利用LLM生成的中性文本作为语义锚定；4）动态选择中性示例提供平衡指导；5）毒性样本明确优化以偏离中性示例，增强类别特异性对比信号。

Result: 在四个仇恨言论基准测试中，ToxiGAN在macro-F1和hate-F1指标上均取得最佳平均性能，一致优于传统方法和基于LLM的增强方法。消融实验和敏感性分析进一步证实了语义锚定和定向训练对提升分类器鲁棒性的益处。

Conclusion: ToxiGAN通过结合对抗生成与LLM语义引导，有效解决了毒性数据增强中的模式崩溃和语义漂移问题，为类别感知的文本增强提供了新思路，显著提升了毒性分类器的鲁棒性。
...
Method: 1) 使用高容量多语言翻译模型生成合成句子对；2) 微调多语言mBART模型；3) 应用语言特定的预处理，包括正字法规范化和噪声感知过滤；4) 使用chrF++指标评估翻译质量

Result: 在瓜拉尼语-西班牙语和克丘亚语-西班牙语翻译中，合成数据增强带来了chrF++指标的持续改进；在艾马拉语的诊断实验中，发现通用预处理对高度黏着语存在局限性
...
<details>
  <summary>Details</summary>
Motivation: 语言在视觉-语言-动作模型中扮演关键角色，但用于训练和评估这些系统的数据集的语⾔特性缺乏充分文档记录，需要系统分析这些数据集实际包含的指令类型和语言多样性

Method: 对多个广泛使用的VLA数据集进行系统审计，从词汇多样性、重复与重叠、语义相似性和句法复杂性等互补维度量化指令语言特征

Result: 分析显示许多数据集依赖高度重复、模板化的命令，结构变化有限，指令形式分布狭窄，语言多样性不足

Conclusion: 这些发现旨在为当前VLA训练和评估数据中的语言信号提供描述性文档，支持更详细的数据集报告、更原则性的数据集选择，以及扩展语言覆盖的有针对性策略
...
TL;DR: 首个通过日本司法考试的LLM模型，通过忠实复制考试格式和评分标准的自我验证方法，无需改变原始问题结构或评分规则。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型快速发展，但在高度专业化和结构化的考试（如日本司法考试）中实现可靠性能仍面临挑战。现有方法虽然通过将复杂问题分解为简单真伪判断有所改进，但未在原始考试格式和评分标准下系统评估，无法确定是否真正掌握考试级别的能力。

Method: 构建忠实复制考试真实格式和评分标准的新数据集，训练自我验证模型。模型保持原始问题结构和评分规则不变，通过格式忠实监督和一致性验证来确保符合考试要求。

Result: 模型在真实考试评分标准下超过了官方及格分数，这是首个已知的不改变原始问题结构或评分规则而通过日本司法考试的LLM。与多智能体推理和分解监督等替代策略的广泛比较显示，这些方法无法达到可比性能。

Conclusion: 格式忠实监督和一致性验证对于高风险的职业推理任务至关重要。精心设计的单模型方法可以超越更复杂的系统，在专业考试中实现突破性表现。数据集和代码已公开。
...
TL;DR: 研究发现：长链思维（CoT）在单答案任务中表现出色，但在处理人类标签变异（需要捕捉概率模糊性而非解决它）时能力有限。CoT主要作为LLM的决策制定者决定最佳选项，但无法作为模糊任务的细粒度分布校准器。


<details>
  <summary>Details</summary>
Motivation: 当前推理调优的LLMs利用长链思维在单答案任务中表现出色，但它们处理人类标签变异的能力尚未得到充分探索。人类标签变异需要捕捉概率模糊性而非解决它，这与传统单答案任务有本质区别。

Method: 通过系统性的解耦实验研究分布型任务，采用交叉CoT实验来分离推理文本和内在模型先验的影响。进行逐步分析来观察CoT在推理过程中的影响变化。

Result: 观察到明显的"解耦机制"：CoT改善分布对齐，但最终准确率由CoT内容决定（99%方差贡献），而分布排名由模型先验主导（超过80%）。逐步分析显示CoT对准确率的影响在推理过程中单调增长，但分布结构主要由LLM内在先验决定。

Conclusion: 长链思维主要作为LLM的决策制定者决定最佳选项，但无法作为模糊任务的细粒度分布校准器。这表明CoT在捕捉概率模糊性方面存在局限性，需要新的方法来处理人类标签变异任务。
...
TL;DR: 本文系统评估了5种嵌入相似度指标在跨语言迁移中的预测能力，发现余弦差距和检索类指标能可靠预测迁移效果，但需针对具体模型验证以避免辛普森悖论。
...
Method: 在816个迁移实验中系统评估了5种嵌入相似度指标，涵盖3个NLP任务、3个非洲中心的多语言模型和来自4个语系的12种语言。使用相关性分析评估指标预测能力。

Result: 余弦差距和检索类指标（P@1、CSLS）能可靠预测迁移成功（ρ=0.4-0.6），而CKA的预测能力可忽略（ρ≈0.1）。关键发现是当跨模型汇总时，相关性符号会反转（辛普森悖论），因此必须针对具体模型验证。嵌入指标与URIEL语言类型学具有相当的预测能力。

Conclusion: 研究结果为源语言选择提供了具体指导，强调了模型特定分析的重要性，并表明嵌入相似度指标可作为有效的迁移预测工具，但需避免跨模型汇总导致的误导性结论。
...
TL;DR: PALU：一种针对大语言模型的局部熵最大化遗忘框架，通过抑制敏感前缀和扁平化top-k logits实现高效知识遗忘，同时最小化对模型通用性能的影响


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常对所有响应token进行无差别处理，并在整个词汇表上强制不确定性，这导致不必要的性能下降并将优化扩展到与内容无关的区域

Method: 提出PALU框架，采用时间和词汇维度上的局部熵最大化目标，通过抑制敏感前缀来切断因果生成链，仅扁平化top-k logits来最大化关键子空间的不确定性

Result: PALU在遗忘效果和效用保持方面优于现有最先进基线方法，避免了完整词汇表和参数空间的冗余优化，最小化了对通用模型性能的附带损害

Conclusion: PALU框架通过局部化处理敏感知识遗忘，实现了更高效的机器遗忘，为LLMs的安全性和实用性平衡提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 人类智能的核心特征是通过构建性情节模拟来掌握新技能，但现有大型语言模型难以实现这种自我进化。微调计算成本高且容易灾难性遗忘，而基于记忆的方法依赖被动语义匹配，经常检索到噪声。

Method: MemRL框架将冻结LLM的稳定推理与可塑性记忆分离，采用两阶段检索机制：先通过语义相关性筛选候选，然后基于学习的Q值（效用）选择。这些效用通过环境反馈以试错方式持续优化，使智能体能够区分高价值策略与相似噪声。
...
Conclusion: MemRL通过基于情节记忆的非参数强化学习，使智能体能够自我进化，解决了LLM持续学习中的关键挑战，实现了稳定推理与可塑性记忆的分离，为智能系统的持续改进提供了新途径。
...
TL;DR: X-MuTeST是一个用于仇恨言论检测的可解释性引导训练框架，结合了大型语言模型的高层语义推理与传统注意力增强技术，针对英语、印地语和泰卢固语等资源匮乏语言，通过人类标注的词语级理由提升分类性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的仇恨言论检测在准确性和可解释性方面面临挑战，特别是对于资源匮乏的印度语言。现有方法缺乏对多语言环境的有效支持，且可解释性不足，需要结合人类标注的理由来提升模型性能。

Method: 提出X-MuTeST框架：1) 结合LLM的高层语义推理与传统注意力增强技术；2) 通过计算原始文本与n-gram预测概率差异生成解释；3) 将LLM解释与X-MuTeST解释进行融合；4) 在训练中利用人类标注的词语级理由；5) 使用人类理由与可解释性方法共同优化模型注意力。

Result: 1) 利用人类理由训练能提升分类性能和可解释性；2) 结合人类理由与可解释性方法进一步优化模型注意力；3) 构建了包含6,004个印地语、4,492个泰卢固语和6,334个英语样本的数据集，每个样本都有词语级理由标注；4) 在可解释性评估指标（Token-F1、IOU-F1、Comprehensiveness、Sufficiency）上表现良好。

Conclusion: X-MuTeST框架通过结合LLM语义推理与传统注意力技术，并利用人类标注的理由，有效提升了多语言仇恨言论检测的准确性和可解释性，特别适用于资源匮乏的语言环境，推动了跨语言仇恨言论检测的发展。
...
Conclusion: 扩散语言模型的生成范式允许高效的动态上下文调整，DIP方法通过利用这一特性显著提升了推理效率，为扩散语言模型的实际应用提供了重要改进。
...
TL;DR: UltraLogic框架通过代码化求解方法分离问题的逻辑核心与自然语言表达，自动化生成高质量推理数据，并引入双极浮点奖励机制解决奖励稀疏性问题，显著提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在自然语言处理方面展现出潜力，但需要多步逻辑、规划和验证的复杂通用推理仍然是关键瓶颈。现有强化学习可验证奖励方法在特定领域成功，但缺乏大规模、高质量、难度校准的通用推理数据。

Method: 提出UltraLogic框架：1) 通过代码化求解方法将问题的逻辑核心与自然语言表达解耦，自动化生成高质量数据；2) 包含数百种独特任务类型和跨十个难度级别的自动校准流程；3) 引入双极浮点奖励机制，使用分级惩罚来区分完美响应和有逻辑缺陷的响应。

Result: 实验表明：1) 任务多样性是推理能力提升的主要驱动力；2) 双极浮点奖励机制结合难度匹配策略显著提高训练效率；3) 能够引导模型达到全局逻辑最优。

Conclusion: UltraLogic框架通过自动化生成高质量推理数据和创新的奖励机制，有效解决了大语言模型在复杂通用推理方面的瓶颈问题，为提升模型推理能力提供了系统性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 学生在数学学习中经常出现系统性错误——他们会重复应用错误但一致的程序。现有AI系统缺乏对学生错误推理过程的建模能力，无法准确诊断和预测学生的误解。

Method: 基于67个学习科学和数学教育文献，将文档化的误解转化为可执行程序（malrules），创建参数化问题模板，生成正确推理和错误推理的双路径追踪。定义了Malrule Reasoning Accuracy（MRA）作为核心评估指标。

Result: MalruleLib包含101个malrules和498个参数化问题模板，可生成超过100万个实例。在9个语言模型（4B-120B）上测试，准确率从直接问题解决的66%下降到跨模板误解预测的40%。提供学生步骤追踪可将预测提升3-15%。

Conclusion: MalruleLib作为教育AI基础设施，能够跨情境建模学生程序，支持针对底层误解的诊断和反馈。系统揭示了AI模型在理解学生系统性错误方面的局限性，同时展示了步骤追踪信息的重要性。
...
TL;DR: 创建了RXL-RADSet多RADS基准数据集，比较了开源小语言模型与专有模型在RADS分类任务上的表现，发现20-32B参数的大SLM在指导提示下可接近专有模型性能。


<details>
  <summary>Details</summary>
Motivation: 放射学报告和数据系统(RADS)标准化了风险沟通，但自动化RADS分配面临挑战：指南复杂、输出格式限制、跨RADS框架和模型规模的基准测试有限。

Method: 创建包含1,600份合成放射报告的RXL-RADSet数据集，涵盖10种RADS系统。使用LLM生成报告并经过两阶段放射科医生验证。评估41个量化SLM(0.135-32B参数)和GPT-5.2，采用固定指导提示策略。

Result: GPT-5.2达到99.8%有效性和81.1%准确率。SLM总体达到96.8%有效性和61.1%准确率，20-32B参数的最佳SLM达到约99%有效性和70%+准确率。性能随模型规模增加，指导提示显著优于零样本提示。

Conclusion: RXL-RADSet提供了放射科医生验证的多RADS基准；大型SLM(20-32B)在指导提示下可接近专有模型性能，但在高复杂度RADS方案上仍有差距。
...
TL;DR: 提出ASRD算法用于检测多智能体系统中涌现通信的语义规则，提高涌现语言的可解释性


<details>
  <summary>Details</summary>
Motivation: 多智能体涌现通信领域主要研究自主智能体如何独立发展通信策略，但现有研究很少关注涌现语言的可解释性问题
...
### [125] [Breaking Rank - A Novel Unscented Kalman Filter for Parameter Estimations of a Lumped-Parameter Cardiovascular Model](https://arxiv.org/abs/2601.02390)
*Alex Thornton,Ian Halliday,Harry Saxton,Xu Xu*

Main category: cs.IT

TL;DR: 本文提出了一种改进的无迹卡尔曼滤波（UKF）方法，用于解决心血管模型参数估计中的秩不足问题，显著提高了参数可识别性和估计精度。


<details>
  <summary>Details</summary>
Motivation: 传统UKF在应用于参数估计时面临秩不足的挑战，通常只能估计一小部分参数，需要固定非影响性参数或选择最优参数子集。对于具有10个参数和4个输出观测值的非线性、刚性心血管模型，这种限制尤为明显，影响了临床应用的实用性。

Method: 对标准UKF算法进行修改，克服了参数估计中的秩不足问题。这些修改使UKF能够用于参数估计目的，包括估计最小影响参数，而不再受限于传统UKF用于动态状态跟踪时的约束。

Result: 改进后的UKF算法在具有挑战性的50个10参数样本数据集上表现出色：能够以超过90%的概率恢复几乎所有参数，精度超过98%，即使在严重噪声干扰、具有挑战性的病理生理条件以及缺乏参数先验分布知识的情况下也具有良好鲁棒性。

Conclusion: 通过修改UKF算法，成功解决了参数估计中的秩不足问题，显著提升了UKF在参数估计中的实用性，特别是对于具有临床意义的高度非线性、刚性心血管模型，实现了几乎完全的实际可识别性。
...
### [126] [Weights on finite fields and failures of the MacWilliams identities](https://arxiv.org/abs/2601.02608)
*Jay A. Wood*

Main category: cs.IT

TL;DR: 论文探讨了线性码重量枚举器的对偶关系：Hamming重量枚举器具有对称性（码与其对偶码相互确定），但存在一类更广泛的重量定义，其枚举器表现出相反行为（相同重量枚举器的码可能对应不同对偶码重量枚举器）。


<details>
  <summary>Details</summary>
Motivation: 研究线性码重量枚举器与其对偶码重量枚举器之间的关系。MacWilliams在1960年代证明了Hamming重量枚举器的对称性，但本文发现存在更广泛的重量定义，其枚举器表现出与Hamming重量枚举器相反的行为特征。

Method: 通过理论分析和构造反例的方法，研究有限域上不同重量定义下的重量枚举器性质。对比Hamming重量枚举器的经典结果，探索更广泛重量定义下对偶关系的特殊性。

Result: 发现存在一类广泛的重量定义，其重量枚举器表现出与Hamming重量枚举器相反的行为：存在两个具有相同重量枚举器的线性码，但它们的对偶码却具有不同的重量枚举器。

Conclusion: Hamming重量枚举器的对称性是其特殊性质，而非所有重量枚举器的普遍特征。存在更广泛的重量定义，其枚举器在对偶关系上表现出不同的行为模式，这扩展了对线性码重量枚举器性质的理解。
...
### [127] [State-Dependent Fading Gaussian Channel with Common Reconstruction Constraints](https://arxiv.org/abs/2601.02802)
*Viswanathan Ramachandran*

Main category: cs.IT

TL;DR: 论文研究了在加性状态干扰的衰落高斯信道中，联合传输消息和重建信道状态的问题，完全刻画了最优的率失真权衡区域。


<details>
  <summary>Details</summary>
Motivation: 研究在衰落高斯信道中，当发射机非因果已知独立同分布高斯状态序列时，如何同时实现消息传输和信道状态重建的问题。接收机需要解码传输的消息，并在共同重建约束下重建信道状态。

Method: 采用衰落高斯模型，状态为独立同分布高斯序列，发射机非因果已知状态信息，瞬时衰落系数在收发双方完美已知。接收机需要解码消息并在共同重建约束下重建状态。

Result: 完全刻画了该场景下最优的率失真权衡区域，这是论文的主要结果。通过数值示例验证了率失真和功率失真的权衡关系。

Conclusion: 成功解决了衰落高斯信道中联合通信和状态重建的问题，提供了完整的率失真权衡区域表征，并通过数值分析验证了理论结果。
...
### [128] [Context-aware Privacy Bounds for Linear Queries](https://arxiv.org/abs/2601.02855)
*Heng Zhao,Sara Saeidian,Tobias J. Oechtering*

Main category: cs.IT

TL;DR: 该论文通过点态最大泄漏视角重新分析拉普拉斯机制，发现差分隐私的分布无关定义通常要求过多噪声，通过引入先验分布假设（记录属于特定类别的概率下界），推导出更紧的上下文感知泄漏界限，可减少所需噪声规模。


<details>
  <summary>Details</summary>
Motivation: 差分隐私采用与数据生成分布无关的上下文无关定义，这通常导致需要添加过多噪声。作者希望通过结合先验分布假设，获得更紧的隐私泄漏界限，从而在保持隐私保护的同时减少噪声添加。

Method: 通过点态最大泄漏视角重新分析拉普拉斯机制，引入先验分布假设（记录属于特定类别的概率下界），推导出针对一般线性查询的紧致、上下文感知泄漏界限，并与标准DP保证进行比较。

Result: 推导出的界限严格比标准DP保证更紧，且当概率下界趋近于零时收敛到DP保证。数值评估表明，利用这种先验知识可以在保持隐私保证的同时减少所需噪声规模。

Conclusion: 通过结合先验分布假设，可以显著改进差分隐私框架的实用性，在保持隐私保护的同时减少噪声添加，为实际应用提供更高效的隐私保护机制。
...
### [129] [Dualities for finite abelian groups and applications to coding theory](https://arxiv.org/abs/2601.03126)
*Jay A. Wood*

Main category: cs.IT

TL;DR: 研究有限阿贝尔群上对偶性和对偶码的性质，扩展Delsarte和Dougherty等人的工作


<details>
  <summary>Details</summary>
Motivation: 有限阿贝尔群与其特征群之间的同构（对偶性）选择允许定义加法码的对偶码，需要系统研究这些对偶性和对偶码的性质

Method: 研究有限阿贝尔群上的对偶性，分析对偶码的性质，延续Delsarte（1973）和Dougherty等人的工作

Result: 建立了有限阿贝尔群上对偶性和对偶码的理论框架，扩展了现有研究成果

Conclusion: 有限阿贝尔群上的对偶性为定义加法码的对偶码提供了理论基础，相关性质研究对编码理论有重要意义
...
### [130] [On the Euclidean duals of the cyclic codes generated by cyclotomic polynomials](https://arxiv.org/abs/2601.03165)
*Anuj Kumar Bhagat,Ritumoni Sarma*

Main category: cs.IT

TL;DR: 本文确定了与q互质的正整数n的循环码C_n的欧几里得对偶码的最小距离，证明了该距离是n的函数，即2^{ω(n)}，验证了作者先前提出的猜想。


<details>
  <summary>Details</summary>
Motivation: 研究循环码对偶码的最小距离是编码理论中的重要问题。作者之前提出了关于由第n个分圆多项式生成的循环码对偶码最小距离的猜想，需要验证这一猜想是否正确。

Method: 使用代数编码理论方法，特别是分圆多项式和循环码理论。对于每个与q互质的正整数n，分析由第n个分圆多项式Q_n(x)在有限域F_q上生成的循环码C_n，然后确定其对偶码C_n^⊥的最小距离。

Result: 证明了循环码C_n的欧几里得对偶码的最小距离是n的函数，具体为2^{ω(n)}，其中ω(n)表示n的不同质因数的个数。这验证了作者在先前文献中提出的猜想。

Conclusion: 本文成功确定了由分圆多项式生成的循环码对偶码的最小距离，证明了该距离仅依赖于n的不同质因数个数，为2^{ω(n)}，解决了编码理论中的一个重要问题。
...
### [131] [On the Capacity Region of Individual Key Rates in Vector Linear Secure Aggregation](https://arxiv.org/abs/2601.03241)
*Lei Hu,Sennur Ulukus*

Main category: cs.IT

TL;DR: 该论文解决了向量线性安全聚合问题中最小个体密钥速率要求，通过引入秩增量条件，证明了并非每个用户都需要持有密钥，从而扩展了已知的可实现区域。


<details>
  <summary>Details</summary>
Motivation: 解决Yuan-Sun在ISIT 2025中提出的开放问题：确定向量线性安全聚合问题中每个用户所需的最小个体密钥速率。现有研究对密钥分配要求较高，需要探索更高效的密钥分配方案。

Method: 提出了一种新的可实现区域，该区域是多面体结构，其顶点由二进制速率分配表征。关键条件是秩增量条件：rank([F_I;G_I]) = rank(F_I) + N，其中I是用户子集。通过分析满足该条件的用户子集来确定哪些用户需要持有密钥。

Result: 发现并非每个用户都需要持有密钥，这严格扩大了文献中已知的最佳可实现区域。当最小化持有密钥的用户数量时，该方案是最优的，并通过逆分析证明了其最优性。

Conclusion: 论文解决了向量线性安全聚合中的开放问题，通过秩增量条件表征了新的可实现区域，揭示了密钥分配可以比现有方案更稀疏，为分布式安全计算提供了更高效的密钥管理方案。
...
### [132] [Textual Explanations and Their Evaluations for Reinforcement Learning Policy](https://arxiv.org/abs/2601.02514)
*Ahmad Terra,Mohit Ahmed,Rafia Inam,Elena Fersman,Martin Törngren*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的可解释强化学习框架，通过大语言模型生成文本解释，将其转换为透明规则，并改进解释质量，最后进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 确保强化学习策略符合人类期望至关重要，但现有可解释强化学习技术生成的文本解释存在正确性难以保证、评估有限的问题。

Method: 使用大语言模型生成文本解释，通过聚类技术识别频繁条件，将其转换为透明规则，提出两种精炼技术改进解释质量，并整合专家知识和自动谓词生成器。

Result: 在三个开源环境和电信用例中验证了框架的有效性，解决了现有自主策略解释方法的局限性，生成的透明规则在某些任务上能达到满意性能。

Conclusion: 该框架实现了对文本解释的系统化和定量评估，为可解释强化学习领域提供了有价值的见解，并展示了工业应用的可行性。
...
### [133] [An Empirical Study of On-Device Translation for Real-Time Live-Stream Chat on Mobile Devices](https://arxiv.org/abs/2601.02641)
*Jeiyoon Park,Daehwan Lee,Changmin Yeo,Yongshin Han,Minseop Kim*

Main category: cs.AI

TL;DR: 该论文研究了设备端AI模型在实际部署中的关键问题，包括模型选择、资源消耗和领域适应能力，通过构建LiveChatBench基准测试并在5种移动设备上实验，发现设备端模型在特定任务上能达到与GPT-5.1等商业模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管设备端AI模型效率高，但实际部署中需要考虑设备CPU利用率和热条件等实际问题，目前这方面的研究较少。论文旨在解决设备端模型在真实服务部署中必须解决的两个关键问题。

Method: 通过大量实验研究设备端模型选择和资源消耗，以及设备端模型的领域适应能力。构建了LiveChatBench基准测试，包含1000个韩英平行句对。在5种移动设备上进行实验评估。

Result: 实验表明，虽然服务大规模异质用户群需要仔细考虑高度受限的部署环境和模型选择，但所提出的方法在针对性任务上能达到与GPT-5.1等商业模型相当的性能。

Conclusion: 设备端AI模型在实际部署中面临资源约束和模型选择挑战，但通过适当的方法可以在特定任务上达到商业模型的性能水平，为设备端AI社区提供有价值的见解。
...
### [134] [Learning from Prompt itself: the Hierarchical Attribution Prompt Optimization](https://arxiv.org/abs/2601.02683)
...
<details>
  <summary>Details</summary>
Motivation: 当前提示优化方法存在两个主要问题：1) 提示漂移 - 新提示修复先前失败但损害先前成功任务的性能；2) 从零生成提示会损害可解释性。需要一种既能优化性能又能保持可解释性的方法。
...
Conclusion: HAPO框架通过解决提示漂移和可解释性问题，为可扩展的提示工程建立了有效的优化范式，在多种任务场景中表现出优越性能。
...
### [135] [Learning User Preferences Through Interaction for Long-Term Collaboration](https://arxiv.org/abs/2601.02702)
...
<details>
  <summary>Details</summary>
Motivation: 随着对话智能体与用户协作经验的积累，适应和了解用户偏好对于建立长期关系和提升协作质量至关重要。现有研究缺乏评估智能体在多轮会话中持续学习和利用用户偏好的能力。

Method: 提出MultiSessionCollab基准测试，开发配备持久化记忆的长期协作智能体，能够随着交互经验积累而细化和更新用户偏好。利用用户模拟器行为生成学习信号，训练智能体生成更全面的反思并更有效地更新记忆。

Result: 实验表明，配备记忆的智能体显著提升了长期协作质量：任务成功率更高、交互更高效、用户努力程度降低。人类用户研究进一步证实记忆机制在真实场景中改善了用户体验。

Conclusion: 持久化记忆机制对于对话智能体的长期协作能力至关重要，MultiSessionCollab基准测试为评估和训练智能体在多轮会话中学习用户偏好提供了有效框架，能够显著提升协作质量和用户体验。
...
TL;DR: 该论文探讨了大型语言模型从被动文本生成器向自主、目标驱动的智能体系统的演变，分析了实现自主行为所需的核心组件架构，并指出了当前面临的技术挑战和研究重点。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型从被动文本生成向自主智能体系统转变的动机在于探索人工智能发展的根本性转变，理解如何将语言理解能力转化为自主行动能力，以及这一转变带来的技术挑战和应用前景。

Method: 通过分析LLM架构从统计模型到基于Transformer系统的演进过程，识别出实现智能体行为的关键能力：长程推理、上下文感知和自适应决策。提出了一个集成框架，包含感知、记忆、规划和工具执行等核心组件，并采用批判性评估方法分析应用场景和挑战。

Result: 论文提出了三个主要贡献：1）LLM能力如何通过推理-行动-反思循环向自主性扩展的综合分析；2）连接LLM与自主行为的核心组件集成框架；3）对应用场景及安全、对齐、可靠性、可持续性等持续挑战的批判性评估。

Conclusion: 从语言理解到自主行动的架构转变需要解决关键技术差距，包括可验证规划、可扩展多智能体协调、持久记忆架构和治理框架等研究重点。负责任的发展需要在技术鲁棒性、可解释性和伦理保障方面同步进展，以实现潜力同时减轻错位和意外后果的风险。
...
### [137] [HAL: Inducing Human-likeness in LLMs with Alignment](https://arxiv.org/abs/2601.02813)
*Masum Hasan,Junjie Zhao,Ehsan Hoque*

Main category: cs.AI

TL;DR: HAL框架通过从对比对话数据中提取可解释的对话特征，组合成标量奖励分数，用于对齐语言模型的人类对话相似性，而不影响模型整体性能。


<details>
  <summary>Details</summary>
Motivation: 人类对话相似性在人机交互中至关重要，但难以定义、测量和优化。目前改进主要依赖规模扩展或广泛监督训练，而非针对性对齐。

Method: 从对比对话数据中提取明确的对话特征，组合成紧凑的标量分数作为透明奖励信号，使用标准偏好优化方法进行对齐。

Result: 对齐后的模型在人类评估中更频繁地被感知为具有人类对话相似性，且不影响模型整体性能。HAL支持对齐行为检查和意外效应诊断。

Conclusion: HAL展示了如何将语言中软性、定性的属性变得可测量、可对齐，并以可解释和可解释的方式进行对齐。
...
### [138] [Causal-Enhanced AI Agents for Medical Research Screening](https://arxiv.org/abs/2601.02814)
*Duc Ngo,Arya Rahgoza*

Main category: cs.AI

TL;DR: 本文提出了一种基于因果图增强的检索增强生成系统，通过结合显式因果推理与双层知识图谱，在医学系统综述任务中实现零幻觉和高精度，相比基线AI有显著提升。


<details>
  <summary>Details</summary>
Motivation: 医学系统综述对循证医学至关重要，但每年150万+的出版物使人工审查不可行。现有AI方法在系统综述任务中存在幻觉问题（2-40%的错误率），这在影响患者护理时是不可接受的。

Method: 开发了因果图增强的检索增强生成系统，整合显式因果推理与双层知识图谱。采用证据优先协议，确保每个因果主张都能追溯到检索到的文献，并自动生成可视化干预-结果路径的有向无环图。

Result: 在234篇痴呆症运动研究摘要的评估中，CausalAgent达到95%准确率、100%检索成功率和零幻觉，而基线AI仅为34%准确率和10%幻觉率。自动因果图支持显式机制建模、可视化合成和增强可解释性。
...
### [139] [Sample-Efficient Neurosymbolic Deep Reinforcement Learning](https://arxiv.org/abs/2601.02850)
...
<details>
  <summary>Details</summary>
Motivation: 当前深度强化学习算法通常需要大量训练数据，且在训练场景之外泛化能力有限。为了解决这些问题，需要一种能够整合背景知识、提高样本效率和泛化能力的方法。

Method: 提出神经符号深度强化学习方法：1) 将简单领域实例中定义的部分策略表示为逻辑规则；2) 通过在线推理指导训练过程：在探索阶段偏置动作分布，在利用阶段重新缩放Q值；3) 将部分策略作为有用先验知识迁移到更复杂环境中。

Result: 在网格世界环境的完全可观测和部分可观测变体上进行了实证验证，相比最先进的奖励机基线方法，展示了更好的性能表现。

Conclusion: 神经符号集成方法能够提高深度强化学习的可解释性和可信度，特别是在稀疏奖励环境和长规划视野任务中加速收敛，同时改善样本效率和泛化能力。
...
### [140] [M3MAD-Bench: Are Multi-Agent Debates Really Effective Across Domains and Modalities?](https://arxiv.org/abs/2601.02854)
...
TL;DR: 本文提出了M3MAD-Bench，一个用于评估多智能体辩论方法的统一可扩展基准，涵盖多领域任务、多模态输入和多维度指标，解决了现有研究评估设置碎片化和仅限于文本模态的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论研究存在两个根本性局限：1) 评估在碎片化且不一致的设置下进行，阻碍了公平比较；2) 主要局限于依赖纯文本输入的单模态场景。需要建立一个统一的评估基准来解决这些问题。

Method: 提出了M3MAD-Bench基准，建立了五个核心任务领域（知识、数学、医学、自然科学、复杂推理）的标准化协议，系统覆盖纯文本和视觉语言数据集，支持跨模态比较。评估了9个不同架构、规模和模态能力的基础模型，并纳入了面向效率的指标（如token消耗和推理时间）。

Result: 通过大量实验，系统性地揭示了多智能体辩论在纯文本和多模态场景下的有效性、鲁棒性和效率。基准提供了性能-成本权衡的整体视图，为未来标准化MAD评估研究提供了可靠基础。

Conclusion: M3MAD-Bench为多智能体辩论方法的多领域、多模态和多维度评估提供了统一框架，解决了现有研究的局限性，为未来标准化评估奠定了基础。代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在多步推理（如数学和创意写作）方面存在挑战，现有方法如ReAct、Reflexion和Self-Refine虽然通过迭代精炼和反思改进了推理，但缺乏对替代解决方案路径的结构化探索和跨问题的持久学习。

Method: ReTreVal框架整合了四个关键组件：1）基于问题复杂度的自适应深度树状思维探索；2）每个节点通过LLM生成明确反馈进行迭代自我批判和精炼；3）双重验证机制评估每个节点的推理质量、连贯性和正确性；4）将成功推理路径和失败模式的见解存储在反思记忆缓冲区中，实现跨问题学习。采用批判性剪枝保留每层最高得分的k个节点。
...
Conclusion: ReTreVal通过结合结构化探索、批判驱动精炼和跨问题记忆，为需要探索性推理、严格验证和知识迁移的任务提供了有效的解决方案，在复杂多步推理任务上表现出色。
...
TL;DR: 研究发现大语言模型在逻辑推理中存在"逻辑相变"现象：在临界逻辑深度之前性能稳定，超过后突然崩溃，类似物理相变。提出神经符号课程调优框架来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 符号逻辑推理是大语言模型的关键但未被充分探索的能力，在高风险领域如数学推理和法律判断中提供可靠且可验证的决策。当前研究缺乏对逻辑复杂性增加时模型性能变化的系统分析。
...
Conclusion: 大语言模型的逻辑推理能力存在相变现象，通过神经符号课程调优可以显著改善模型在高逻辑复杂性下的性能，为提升符号逻辑推理能力提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型推理系统独立处理查询，丢弃了有价值的跨实例信号，如共享推理模式和一致性约束。为了解决这个问题，研究者提出了批量处理相关查询的方法。

Method: 提出了Batch-of-Thought (BoT)方法，通过批量处理相关查询实现跨实例学习。具体包括：1) 通过批量比较分析识别高质量推理模板；2) 通过一致性检查检测错误；3) 分摊计算成本。进一步在BoT-R多智能体反射架构中实现，其中反射器进行联合评估以获取孤立处理无法获得的互信息增益。

Result: 在三个模型系列和六个基准测试上的实验表明，BoT-R持续提高准确性和置信度校准，同时将推理成本降低高达61%。理论和实验分析揭示了批量感知推理何时以及为何对LLM系统有益。

Conclusion: Batch-of-Thought方法通过批量处理相关查询，有效利用跨实例信号，显著提升LLM推理性能并降低计算成本，为LLM系统设计提供了新的思路。
...
TL;DR: RationaleTS方法通过引入基于推理路径的理性先验，改进多模态大语言模型在时间序列推理中的表现，避免模型依赖表面模式匹配


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在时间序列推理中表现不佳，主要原因是缺乏将时间观测与下游结果连接起来的理性先验，导致模型依赖表面模式匹配而非原则性推理

Method: 提出基于理性先验的上下文学习框架RationaleTS：1) 诱导标签条件理性，构建从可观测证据到潜在结果的推理路径；2) 设计混合检索机制，平衡时间模式和语义上下文，为新的样本检索相关理性先验进行最终推理

Result: 在三个领域的时间序列推理任务上进行了广泛实验，证明了RationaleTS方法的有效性和效率

Conclusion: 理性先验作为指导推理单元而非事后解释，能够显著提升时间序列推理性能，代码将开源以供复现
...
### [145] [Automatic Prompt Engineering with No Task Cues and No Tuning](https://arxiv.org/abs/2601.03130)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示工程方法复杂且需要调优，而数据库表列名扩展任务对表格数据搜索、访问和理解至关重要，但现有研究很少。本文旨在开发更简单有效的自动提示工程方法来解决这一问题。

Method: 提出了一种简化的自动提示工程系统，无需调优和明确的任务线索，专门针对数据库表列名扩展任务进行设计，并在英语和德语数据集上进行评估。

Result: 该系统在数据库表列名扩展任务上表现出与现有方法相当的效果，首次实现了自动提示工程在非英语语言（德语）上的应用，填补了该领域的研究空白。

Conclusion: 本文提出的简化自动提示工程系统在数据库表列名扩展任务上有效，首次将该技术应用于非英语语言，为表格数据理解和多语言应用提供了新方法。
