...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，专门为儿童和护理环境设计，通过个性化安全约束来防止用户接触有害内容，与YouTube等平台的参与度优化推荐算法形成对比。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其参与度优化的推荐算法可能让脆弱用户（如儿童、痴呆症患者）接触到不适当或有害内容。在儿童导向和护理环境中，内容在展示前必须满足个性化的安全约束。

Method: SafeScreen采用安全优先的方法，将安全作为先决条件，通过自动化流水线顺序批准或拒绝候选视频。框架包含三个核心组件：(1) 基于用户档案提取个性化安全标准；(2) 通过自适应问题生成和多模态VideoRAG分析进行证据驱动的评估；(3) LLM驱动的决策机制，在内容暴露前验证安全性、适当性和相关性。

Result: 在痴呆症护理回忆案例研究中，使用30个合成患者档案和90个测试查询进行评估。结果显示，SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名形成显著差异，同时在安全性覆盖、合理性和基础性方面保持高水平，得到了LLM评估和领域专家的验证。

Conclusion: SafeScreen提供了一种可解释的实时视频筛选方法，能够在无需预计算安全标签的情况下处理未策划的视频库，为脆弱用户群体提供安全优先的内容推荐解决方案。
...
TL;DR: 本文提出了一种基于SoC FPGA的智能相机平台，用于工业环境中喷射火焰的实时分割与表征，通过优化UNet模型实现边缘处理，将参数减少125倍，处理延迟降低7.5倍，达到30FPS性能。


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中缺乏实时早期火灾分割与表征方案的问题，特别是在喷射火焰检测方面，需要低延迟的边缘处理解决方案。

Method: 采用SoC FPGA平台（Ultra96），通过Vitis框架优化UNet分割模型，将全精度模型从750万参数压缩至59,095参数，并利用多线程和批归一化技术进一步优化，实现边缘实时处理。

Result: 模型参数减少125倍，处理延迟降低2.9倍，进一步优化后延迟改善7.5倍，达到30FPS的实时性能，同时保持Dice Score评估指标的准确性。

Conclusion: 提出的智能相机平台能够有效实现工业喷射火焰的实时分割与表征，为火灾安全管理提供可行的边缘计算解决方案，该框架可扩展到其他火灾安全应用领域。
...
TL;DR: 提出3D-Implicit Depth Emergence方法，通过几何自监督而非显式编码实现3D感知的涌现，在保持性能的同时显著降低推理延迟


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的3D信息利用方法存在2D-3D表示融合的权衡问题，导致部署效果不佳。需要重新思考3D知识在视觉语言模型中的集成方式

Method: 提出隐式几何涌现原理，通过细粒度几何验证器和全局表示约束构建信息瓶颈，最大化视觉特征与3D结构之间的互信息，使3D感知在统一视觉表示中自然涌现

Result: 在多个3D场景理解基准测试中超越SOTA，推理延迟降低55%，同时在不同下游任务中保持强大性能，实现无深度和姿态依赖的推理

Conclusion: 从外部嫁接转向隐式涌现的范式转变，代表了3D知识在视觉语言模型中集成方式的根本性重新思考，通过精心设计的辅助目标实现无依赖的3D理解
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法在真实世界部署中存在三个主要问题：1) 难以泛化到未见过的相机视角；2) 需要大量训练数据；3) 推理延迟高。这些限制了在医疗监测、人机协作等实际应用中的部署。

Method: MoViD框架包含三个核心组件：1) 视角估计器，通过建模关键关节关系预测视角信息；2) 正交投影模块，解耦运动和视角特征；3) 基于物理的跨视角对比对齐增强。采用逐帧推理流水线，根据估计视角自适应激活翻转细化策略，实现实时边缘部署。

Result: 在9个公共数据集和新收集的多视角无人机及步态分析数据集上评估，MoViD相比最先进方法减少姿态估计误差超过24.2%，在严重遮挡情况下保持鲁棒性能，训练数据需求减少60%，在NVIDIA边缘设备上实现15FPS的实时推理。

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，显著提升了泛化能力、减少了数据需求，并实现了实时边缘部署，为医疗监测、人机协作等实际应用提供了可行的解决方案。
...
TL;DR: 研究在严格限制下（仅上传紧凑嵌入向量和元数据）的星上系统，通过向量搜索对遥感图像进行危险分级，验证嵌入向量在多种遥感场景变化下的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决下行链路瓶颈问题，避免传输原始像素数据，通过仅上传紧凑嵌入向量和元数据的方式实现星上危险分级系统。

Method: 使用OlmoEarth嵌入向量，在多种遥感场景变化下（跨时间、跨事件/地点、跨站点云、跨城市AOI）进行实验，比较kNN检索和类质心两种决策头在不同任务中的表现。

Result: 所有有效方法都依赖于相同的上传嵌入向量，但最佳决策头因任务而异：kNN检索在云分类中显著优于质心方法（0.92 vs 0.91），而类质心在时序变化检测中占主导地位（0.85 vs 0.48）。
...
TL;DR: 该论文发现当前多模态大语言模型在连续体对象动力学理解方面存在严重不足，提出了两个基准任务进行评估，并开发了Scene Dynamic Field方法显著提升了物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在图像和视频理解方面表现出色，但在物理世界理解方面存在明显不足，特别是在高级物理推理方面。当前模型在理解连续体对象（如流体）的动力学方面存在显著局限性，这阻碍了模型对物理世界的真实理解。

Method: 提出了两个基础基准任务来评估模型对连续体动力学的理解：Next Frame Selection（下一帧选择）和Temporal Coherence Verification（时间一致性验证）。为解决模型局限性，开发了Scene Dynamic Field方法，该方法利用物理模拟器进行多任务微调，以低成本方式增强模型的物理理解能力。

Result: 实验表明，即使是当前最先进的多模态大语言模型在这些基础任务上也表现不佳。而提出的SDF方法显著提升了性能，在流体任务上获得了高达20.7%的性能提升，并且在未见过的物理领域表现出良好的泛化能力。

Conclusion: 该研究不仅揭示了当前多模态大语言模型在物理理解方面的重要缺陷，而且提供了一种有前景的低成本方法来开发更具物理基础的多模态大语言模型。SDF方法为增强模型的物理推理能力提供了一条有效途径。
...
TL;DR: HVG-3D是一个用于3D感知手-物交互视频合成的统一框架，使用3D控制信号替代传统2D控制，通过3D ControlNet和混合训练管道实现高质量、时空一致的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互视频合成方法大多依赖2D控制信号，缺乏空间表达能力，且无法充分利用合成的3D条件数据。需要更有效的3D感知方法来提升视频质量和控制精度。

Method: 提出HVG-3D框架，包含两个核心组件：1）基于扩散的3D感知HOI视频生成架构，通过3D ControlNet编码几何和运动线索实现显式3D推理；2）用于构建输入和条件信号的混合管道，支持训练和推理阶段的灵活精确控制。

Result: 在TASTE-Rob数据集上的实验表明，HVG-3D在空间保真度、时间一致性和可控性方面达到最先进水平，能够有效利用真实和模拟数据，生成高质量、时空一致的视频。

Conclusion: HVG-3D通过显式3D表示和3D控制信号，显著提升了手-物交互视频合成的质量和可控性，为3D感知视频生成提供了有效的统一框架。
...
TL;DR: IDCL是一种新颖的深度图像聚类方法，首次将基于密度信息的模型训练策略引入图像聚类，通过课程学习方案和密度核心引导聚类分配，显著提升了聚类性能、鲁棒性和收敛速度。
...
Conclusion: IDCL首次将密度信息驱动的模型训练策略引入图像聚类，通过创新的课程学习和密度核心引导机制，有效解决了现有方法的局限性，显著提升了聚类性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在细粒度任务中容易产生感知相关的幻觉，主要原因是其推理过程局限于语言域，将视觉输入视为静态、与推理无关的前置信息，而不是动态参与者，导致模型无法重新审视视觉细节来支撑其推理状态。
...
Conclusion: V-Reflection通过将MLLM转变为主动询问者，解决了细粒度感知幻觉问题，在推理过程中保持完全端到端的自回归解码，同时保持最佳效率。
...
TL;DR: TreeGaussian：基于树引导级联对比学习的3D高斯溅射框架，用于层次化3D语义分割


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法难以表示层次化3D语义结构和捕获复杂场景中的整体-部分关系，密集成对比较和2D先验的不一致层次标签阻碍特征学习，导致次优分割效果

Method: 提出TreeGaussian框架：1) 构建多层次对象树实现结构化层次学习；2) 两阶段级联对比学习策略，从全局到局部渐进细化特征表示；3) 一致性分割检测机制和基于图的去噪模块，对齐多视角分割模式并抑制不稳定高斯点

Result: 在开放词汇3D对象选择、3D点云理解和消融实验中证明了方法的有效性和鲁棒性

Conclusion: TreeGaussian通过显式建模层次语义关系和减少对比监督冗余，显著提升了3D高斯溅射的语义分割质量和一致性
...
TL;DR: 提出一种基于扩散模型和随机最优控制的推理时优化框架，用于生成具有语义连贯性的长距离人体运动过渡


<details>
  <summary>Details</summary>
Motivation: 长距离人体运动生成是计算机视觉和图形学中的核心挑战，特别是在不同语义运动域之间生成连贯过渡的能力尚未得到充分探索，这对于舞蹈编排等应用尤为重要
...
Conclusion: 该工作通过简单的推理时优化框架，成功解决了长距离人体运动生成中语义连贯过渡的挑战，为舞蹈编排等应用提供了有效的解决方案
...
TL;DR: CardioSAM：结合冻结SAM编码器和轻量级心脏专用解码器的混合架构，用于心脏MRI图像分割，在ACDC基准测试中达到93.39% Dice系数，超越nnU-Net和专家间一致性水平。


<details>
  <summary>Details</summary>
Motivation: 心脏MRI图像的手动分割耗时且存在观察者间差异，而现有深度学习模型（如SAM）虽然泛化能力强，但缺乏临床应用所需的边界精度。

Method: 提出CardioSAM混合架构：使用冻结的SAM编码器进行通用特征提取，配合轻量级可训练的心脏专用解码器。解码器包含两个关键创新：整合解剖拓扑先验的心脏专用注意力模块，以及改善组织界面描绘的边界细化模块。

Result: 在ACDC基准测试中，CardioSAM达到Dice系数93.39%、IoU 87.61%、像素精度99.20%、HD95 4.2mm。相比nnU-Net提升3.89% Dice，超过报告的专家间一致性水平（91.2%）。

Conclusion: CardioSAM通过结合SAM的泛化能力和心脏专用解码器的解剖先验，实现了可靠且临床适用的心脏分割，超越了现有基准方法和专家间一致性水平。
...
TL;DR: CoLA是一种新颖的参数高效微调框架，通过引入跨模态低秩适配路径，在保持参数效率的同时，有效提升多模态任务性能


<details>
  <summary>Details</summary>
Motivation: 基础模型在多模态任务中面临挑战，特别是双流架构中的单模态编码器（如DINO和BERT）。现有的参数高效微调方法（如LoRA）在各模态内独立操作，无法有效捕捉跨模态交互

Method: 提出跨模态低秩适配（CoLA）框架，在标准模态内适配路径基础上，引入专门的模态间适配路径，形成双路径设计，避免模态特定学习和跨模态学习之间的干扰

Result: 在视觉语言（RefCOCO、RefCOCO+、RefCOCOg）和视听（AVE、AVS）基准测试中，CoLA始终优于LoRA，分别获得约3%和2%的相对增益，同时保持参数效率

Conclusion: CoLA为视觉定位任务提供了首个多任务参数高效微调框架，填补了高效多模态适配的关键空白，能够有效将单模态基础模型适配到多模态任务
...
<details>
  <summary>Details</summary>
Motivation: 当前故事板自动化方法难以同时满足跨镜头一致性和显式可编辑性：2D扩散生成器存在身份漂移和几何控制有限的问题，而传统3D动画工作流程虽然一致且可编辑，但需要专家级、劳动密集型的创作过程。

Method: 提出StoryBlender框架，采用三阶段流程：1) 语义-空间锚定，构建连续性记忆图，将全局资产与镜头特定变量解耦；2) 规范资产实例化，在统一坐标空间中实例化实体以保持视觉身份；3) 时空动态，通过视觉指标实现布局设计和电影演化。采用分层多智能体协调和验证循环机制，通过引擎验证反馈迭代自校正空间幻觉。

Result: 实验表明，StoryBlender在一致性和可编辑性方面显著优于基于扩散和3D基础的基线方法。生成的本地3D场景支持对相机和视觉资产进行直接精确编辑，同时保持跨镜头连续性。

Conclusion: StoryBlender通过将故事板生成问题重新定义为3D场景构建，解决了视觉叙事中跨镜头一致性和可编辑性的核心挑战，为自动化故事板创作提供了实用解决方案。
...
TL;DR: 本研究提出了一种基于预训练和基础模型的可扩展AI方法，用于自动检测面对面协作学习中的注视行为，无需人工标注数据，在复杂场景中表现出更好的跨配置鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法检测协作学习中的注视行为需要大量标注数据，且训练数据集往往无法涵盖教育环境中的所有情境，导致跨配置鲁棒性不足。需要开发无需人工标注、更具可扩展性的方法。

Method: 提出基于预训练和基础模型的可扩展AI方法：使用YOLO11进行人员跟踪，YOLOE-26（具有文本提示能力）检测教育相关对象，Gaze-LLE模型预测注视目标，实现端到端的注视行为自动检测。

Result: 该方法在视频数据中检测学生注视行为的F1分数达到0.829，对笔记本电脑注视和同伴注视表现良好，但对其他注视目标检测较弱。与监督学习方法相比，在复杂场景中表现出更优越和稳定的性能，具有更好的跨配置鲁棒性。

Conclusion: 该方法为支持真实环境中学生的协作学习提供了有前景的解决方案，通过利用预训练模型避免了人工标注需求，在复杂教育场景中表现出更强的适应性，为教育技术应用提供了新思路。
...
TL;DR: EgoMind是一个无需几何监督的Chain-of-Thought框架，通过角色扮演描述和渐进式空间分析，仅用少量数据就能提升多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么需要引入3D先验或几何监督（数据准备和对齐成本高），要么纯2D方法在多帧空间推理中表现不佳。需要一种既能有效进行空间推理又无需几何监督的方法。

Method: 提出EgoMind框架：1) Role-Play Caption：跨帧联合构建连贯的语言场景图；2) Progressive Spatial Analysis：渐进式推理解决特定任务问题。仅使用5K自动生成的SFT样本和20K RL样本。

Result: 在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench等多个基准测试中取得有竞争力的结果，证明了其在增强MLLMs空间推理能力方面的有效性。

Conclusion: EgoMind展示了语言推理在空间认知中的潜力，无需几何监督即可有效提升多模态大语言模型的空间推理能力，且数据需求小。
...
TL;DR: VitaTouch是一个面向智能制造质量检测的视觉-触觉-语言多模态模型，用于材料属性推理和自然语言属性描述，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 智能制造中的质量检测需要识别超出可见几何形状的内在材料和表面属性，但仅依赖视觉的方法容易受到遮挡和反射的影响，因此需要结合多模态感知。

Method: 使用模态特定编码器和双Q-Former提取与语言相关的视觉和触觉特征，压缩为前缀令牌输入大语言模型；通过对比学习对齐各模态与文本，并显式耦合视觉和触觉；构建VitaSet多模态数据集（186个物体、52k图像、5.1k人工验证指令-答案对）。

Result: 在HCT和TVL基准测试中表现最佳，在SSVTP上保持竞争力；在VitaSet上达到88.89%硬度准确率、75.13%粗糙度准确率、54.81%描述符召回率；材料描述任务语义相似度峰值达0.9009；通过LoRA微调后，缺陷识别准确率分别为100.0%（2类）、96.0%（3类）、92.0%（5类）；在100次实验室机器人试验中实现94.0%闭环识别准确率和94.0%端到端分拣成功率。

Conclusion: VitaTouch通过融合视觉、触觉和语言模态，有效解决了智能制造中材料属性检测的挑战，在多个任务上展现了卓越性能，为工业质量检测提供了强大的多模态解决方案。
...
TL;DR: 该研究比较了9种表面重建方法在叶片表面重建中的性能，分析了不同方法在精度、平滑度、鲁棒性和计算成本方面的权衡，为资源受限的农业机器人平台提供选择指导。


<details>
  <summary>Details</summary>
Motivation: 叶片表面的精确重建对农业表型分析至关重要，但真实植物数据（不规则3D点云）复杂，现有表面重建方法（参数化、三角化、隐式、学习型）在叶片重建中的相对性能尚不明确。

Method: 对9种代表性表面重建方法进行对比研究，在三个公开数据集（LAST-STRAW、Pheno4D、Crops3D）上评估，涵盖不同物种、传感器和感知环境，从清洁高分辨率室内扫描到噪声低分辨率田间设置。
...
TL;DR: 本文介绍了RGxEStat工具，用于基因型与环境互作分析，包括显著性分析和稳定性分析，简化了育种数据分析流程。


<details>
  <summary>Details</summary>
Motivation: 基因型与环境互作影响基因型在不同环境中的表现，降低表型预测准确性。深入分析GxE互作有助于识别基因优势或缺陷在特定环境下的表达或抑制，从而支持遗传选择和育种实践。

Method: 提出两种GxE互作分析模型：基于混合效应模型的显著性分析，确定基因或GxE互作是否显著影响表型性状；稳定性分析，研究基因与环境间的互作关系以及基因型在不同环境中的相对优劣。开发了RGxEStat轻量级交互工具，集成了模型构建、求解和可视化功能。

Result: 开发了RGxEStat工具，为育种家和农学家提供用户友好界面，无需学习复杂的SAS或R编程，实现简化的育种数据分析，显著加速研究周期。代码和数据集已在GitHub上公开。

Conclusion: RGxEStat工具通过整合GxE互作分析的显著性分析和稳定性分析模型，为育种研究提供了便捷的分析平台，降低了技术门槛，有助于加速育种研究进程。
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在动作学习方法缺乏编码物理运动加性、组合结构的先验知识，导致潜在动作常常纠缠无关场景细节或未来观测信息，无法准确校准运动幅度。

Method: 提出加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制实施短时间范围内的场景级加性组合结构约束，鼓励潜在动作空间具有简单代数结构（恒等、逆、循环一致性），并抑制非加性组合的信息。

Result: AC-LAM学习到更具结构性、运动特异性且位移校准的潜在动作，为下游策略学习提供更强的监督信号，在模拟和真实世界桌面任务中优于最先进的潜在动作模型。

Conclusion: 通过强制潜在动作空间的加性组合结构约束，AC-LAM能够学习更高质量的潜在动作表示，有效提升下游策略学习性能，为利用互联网规模视频进行具身AI研究提供了更好的方法。
...
<details>
  <summary>Details</summary>
Motivation: 遥感数据分析面临传感器模态多样性和时空动态性的挑战，混合专家模型通过动态路由输入到专门专家来解决这些问题，但该领域缺乏全面综述。

Method: 采用系统综述方法，分析混合专家模型在遥感领域的应用，包括其基本原理、架构设计以及在不同遥感任务中的应用情况。

Result: 提供了混合专家模型在遥感领域的首个系统性概述，涵盖了从基础原理到具体应用的全面内容，为后续研究提供了参考框架。

Conclusion: 混合专家模型是解决遥感数据多样性和动态性挑战的有效范式，本综述填补了该领域的研究空白，并为未来研究指明了方向。
...
TL;DR: 研究发现多轮图像编辑会导致图像质量迭代退化，现有图像质量评估指标无法检测这种退化，可能威胁未来模型训练稳定性和智能系统安全。


<details>
  <summary>Details</summary>
Motivation: 多模态智能系统的多步迭代图像编辑能力改变了数字内容创作，但现有图像编辑模型在多轮编辑中存在图像质量迭代退化的关键弱点，需要系统研究这一问题。

Method: 引入Banana100数据集，包含28,000张通过100次迭代编辑生成的退化图像，涵盖多样纹理和图像内容；评估了21种流行的无参考图像质量评估指标。

Result: 图像质量迭代退化严重，但所有21种图像质量评估指标都无法一致地将较低分数分配给严重退化的图像；生成器和评估器的双重失败可能威胁模型训练稳定性和系统安全。

Conclusion: 多轮图像编辑存在严重退化问题，现有质量评估工具无法检测，需要开发更鲁棒的模型来缓解多模态智能系统的脆弱性，已发布完整代码和数据以促进研究。
...
TL;DR: 量化向量是权重空间中的可转移方向，能够提升模型对后训练量化的鲁棒性，无需接收方进行量化感知训练


<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）会导致模型性能下降，量化感知训练（QAT）虽然有效但成本高昂。研究旨在寻找一种无需接收方训练数据的零成本替代方案，探索量化鲁棒性是否可以作为权重空间的几何特征进行转移

Method: 通过权重空间算术从供体任务中提取量化向量，然后将该向量应用于接收模型以提升其对PTQ诱导噪声的鲁棒性。该方法无需接收方的量化感知训练或训练数据

Result: 量化向量能够将接收模型对PTQ的鲁棒性提升高达60%，在Vision Transformer模型上得到验证。该方法为零成本、零样本的量化鲁棒性提升方案

Conclusion: 量化鲁棒性不仅是任务特定训练的副产品，更是权重空间几何中可重复使用的特征，可以通过转移而非重新训练来获得。量化向量为极低比特部署提供了低成本替代方案
...
TL;DR: 本研究提出了一种基于基础模型的猪只监测工作流，通过预训练视觉-语言基础模型作为通用视觉骨干，结合轻量级任务特定逻辑，实现了规模化、标签高效、长时程的猪群监控。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中大多数流水线仍依赖需要大量标注数据的监督学习模型，需要重复训练和农场特定调优。基础模型通过大规模学习通用视觉表示，有望减少对任务特定监督学习的依赖。

Method: 采用基础模型为中心的工作流：1) 使用Grounding-DINO建立检测基线；2) 引入时间跟踪逻辑处理夜间和遮挡问题；3) 使用Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包含初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制。

Result: 检测在白天条件下准确率高，夜间和严重遮挡时性能下降。短期视频分割后，80%以上的活动轨迹完全正确。长期跟踪系统在132分钟连续视频中保持稳定身份，区域相似度(J)0.83，轮廓精度(F)0.92，J&F 0.87，MOTA 0.99，MOTP 90.7%，无身份切换。

Conclusion: 基础模型的先验知识与轻量级任务特定逻辑相结合，能够在猪生产中实现可扩展、标签高效和长时程的监控，为精准畜牧业提供了新的技术路径。
...
TL;DR: 本文提出将Circuit Duplication方法应用于计算机视觉领域，通过推理时重复遍历部分Transformer层来改进冻结的自监督视觉基础模型嵌入，在水下物种分类任务上显著提升性能，无需微调模型权重。


<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致模型泛化能力有限的问题。现有研究表明冻结的自监督视觉基础模型嵌入已为海洋图像分类提供了强大的标签高效基线，但需要探索是否能在推理时进一步改进这些冻结嵌入，而无需微调或改变模型权重。

Method: 应用Circuit Duplication方法，这是一种最初为大型语言模型提出的推理时方法，在推理过程中重复遍历选定的Transformer层范围。在AQUA20基准测试上使用冻结的DINOv3嵌入进行评估，采用两种设置：全局电路选择（为整个数据集选择单一重复电路）和类别特定电路选择（每个物种可能有不同的最优电路）。两种设置都使用简单的半监督下游分类器。

Result: Circuit Duplication方法持续优于标准冻结前向传播。在最大标签预算下，类别特定选择达到0.875的宏F1分数，将与完全监督ConvNeXt基准（0.889）的差距缩小到1.4个百分点，且无需任何基于梯度的训练。四个物种的性能超过了完全监督参考，其中章鱼提高了+12.1个F1点。在所有预算下，大约75%的类别偏好类别特定电路，表明存在真正的类别依赖性优势。

Conclusion: 这是Circuit Duplication方法首次应用于计算机视觉领域，展示了在推理时改进冻结自监督嵌入的有效性，为标签稀缺的水下物种分类提供了有前景的解决方案，无需昂贵的模型微调。
...
TL;DR: ExpressEdit是一个开源Photoshop插件，用于快速、无伪影地编辑角色面部表情，支持检索增强生成和与Photoshop原生工具协同工作。


<details>
  <summary>Details</summary>
Motivation: 当前AI图像编辑模型在风格化表情编辑时会产生全局噪声和像素漂移，无法集成到专业图像编辑软件和工作流程中，需要解决这一问题以支持艺术家创作。

Method: 开发了ExpressEdit开源Photoshop插件，避免专有模型的常见伪影，与Photoshop原生操作（如液化）协同工作；构建了包含135个表情标签的数据库，支持检索增强生成。

Result: ExpressEdit在单消费级GPU上3秒内完成表情编辑，比流行专有模型快得多；无常见伪影，能与Photoshop原生工具无缝集成；开源代码和数据集促进研究和艺术探索。

Conclusion: ExpressEdit填补了AI表情编辑模型与专业图像编辑工作流之间的空白，为艺术家提供了快速、高质量的表情编辑工具，并通过开源促进社区发展。
...
TL;DR: 该研究提出了一种结合高分辨率卫星影像和谷歌街景的多模态框架，用于在有限标注条件下实现可扩展的城市树木检测，并通过混合学习策略显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统城市树木调查方法劳动密集且成本高昂，难以扩展到不同城市环境。现有自动化系统受限于高标注成本和跨场景泛化能力差的问题，需要开发更高效、可扩展的解决方案来支持环境监测和城市规划。

Method: 采用多模态框架：首先利用卫星影像定位树木候选区域，然后检索对应的街景图像进行详细检测，减少无效采样。为解决标注瓶颈，使用领域适应技术将已有标注数据集的知识迁移到新区域。评估了三种学习策略：半监督学习、主动学习和两者结合的混合方法，基于Transformer检测模型。

Result: 混合策略表现最佳，F1分数达到0.90，比基线模型提升12%。半监督学习因伪标注中的确认偏差导致性能逐渐下降，而主动学习通过针对性的人工干预标注不确定或错误的预测，稳步提升结果。错误分析显示主动和混合策略减少了假阳性和假阴性。

Conclusion: 多模态方法和指导性标注对于实现可扩展、标注高效的城市树木制图至关重要，能够支持可持续城市规划。混合学习策略在有限标注条件下提供了最佳性能平衡。
...
TL;DR: 论文提出用户显著目标检测任务，强调用户主动需求而非被动视觉刺激在显著目标检测中的决定性作用，并指出该领域缺乏训练和测试数据集的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 现有显著目标检测方法基于被动视觉刺激原理，忽略了用户主动需求在分割显著目标中的决定性作用。这种忽视不仅无法满足用户需求，还限制了下游任务的发展，因此需要基于用户需求检测显著目标。

Method: 论文提出用户显著目标检测任务，该任务专注于检测与用户主动需求一致的显著目标，特别是当用户有特定需求时。

Result: 论文识别了该新任务面临的主要挑战：缺乏用于模型训练和测试的数据集。

Conclusion: 需要开发用户显著目标检测任务以更好地满足用户需求并促进下游任务发展，当前首要挑战是解决数据集缺乏问题。
...
TL;DR: 该论文提出了一种轻量级推理时干预方法，通过抑制视觉编码器焦点阶段低注意力标记来减少大视觉语言模型的对象幻觉，无需训练且推理延迟极低。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在多模态推理方面取得显著进展，但仍容易产生对象幻觉（描述输入图像中不存在的对象）。现有方法通常通过抑制视觉编码器中的不可靠信号来缓解幻觉，但许多方法需要对每个输入进行迭代优化，导致推理延迟显著增加。

Method: 通过分析视觉编码器的内部注意力动态，发现视觉信息处理具有一致的三阶段结构：扩散、聚焦和再扩散。研究发现幻觉行为对焦点阶段接收低注意力的标记特别敏感。基于此观察，提出了一种轻量级推理时干预方法，在焦点阶段选择性抑制这些标记。该方法无需训练，使用单次前向传播的统计信息，并采用行列式点过程来保持多样化的视觉线索同时过滤冗余标记。

Result: 在多个大视觉语言模型主干和解码策略上的广泛实验表明，该方法能持续减少幻觉指标，同时保持有竞争力的字幕质量。与对抗性不确定性估计方法相比，该方法实现了相当的幻觉缓解效果，且额外推理延迟可忽略不计。

Conclusion: 通过分析视觉编码器的注意力动态并针对性地干预焦点阶段的低注意力标记，可以有效减少大视觉语言模型的对象幻觉，同时保持高效的推理性能。该方法为幻觉缓解提供了一种轻量级、训练自由的解决方案。
...
TL;DR: 该论文提出了一种随机生成式PnP框架，将预训练的分数扩散模型作为先验直接集成到PnP算法中，通过噪声注入提升严重病态逆问题的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管PnP方法和分数扩散模型都依赖于去噪器，但如何在不依赖反向扩散采样的前提下，系统地将分数扩散模型作为先验集成到PnP框架中仍不清楚。现有方法缺乏理论依据和系统性集成方案。

Method: 建立了PnP的分数解释理论，证明可以直接在PnP算法中使用预训练的分数扩散模型。在此基础上提出了随机生成式PnP框架，通过噪声注入更好地利用生成式分数扩散模型先验，并提供了理论分析表明噪声注入实现了高斯平滑目标优化并促进逃离严格鞍点。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆任务实验中，该方法相比传统PnP方法取得了一致的性能提升，并且与基于扩散的求解器性能相当。

Conclusion: 该工作建立了PnP与分数扩散模型之间的理论联系，提出的随机生成式PnP框架能够有效利用预训练分数扩散模型作为先验，在严重病态逆问题中表现出更好的鲁棒性和性能。
...
TL;DR: PortraitCraft是一个统一的肖像构图基准，包含约5万张带有多层次监督的肖像图像，支持构图理解和构图感知生成两个互补任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和基准主要关注粗粒度美学评分、通用图像美学或无约束肖像生成，缺乏对结构化肖像构图分析和可控肖像生成的系统性研究。

Method: 构建包含约5万张真实肖像图像的数据集，提供全局构图评分、13个构图属性标注、属性级解释文本、视觉问答对和构图导向的文本描述等多层次监督。

Result: 建立了构图理解（评分预测、细粒度属性推理、图像基础视觉问答）和构图感知生成（结构化构图描述下的可控生成）两个互补基准任务，并提供了标准化评估协议和基线结果。
...
TL;DR: TABLeT使用预训练的2D自然图像自编码器将fMRI体积压缩为连续token，通过简单Transformer编码器实现长序列建模，在多个大规模基准测试中表现优异，同时显著提升计算和内存效率。


<details>
  <summary>Details</summary>
Motivation: fMRI四维信号的高维度特性使得建模长程时空动态面临挑战，现有基于体素的模型虽然性能优秀但内存需求过高，只能捕捉有限的时间窗口。

Method: 提出TABLeT方法，使用预训练的2D自然图像自编码器将3D fMRI体积压缩为紧凑的连续token，然后通过简单的Transformer编码器进行长序列建模，并开发了自监督掩码token建模进行预训练。

Result: 在UK-Biobank、Human Connectome Project和ADHD-200等多个大规模基准测试中，TABLeT在多个任务上优于现有模型，同时在相同输入下相比最先进的体素方法显著提升了计算和内存效率。
...
TL;DR: MuPD是一个多模态病理学扩散基础模型，通过扩散变换器和解耦跨模态注意力将组织学、分子RNA谱和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，显著优于领域特定模型。
...
Method: 引入MuPD（多模态病理学扩散）生成基础模型，通过扩散变换器和解耦跨模态注意力将H&E染色组织学、分子RNA谱和临床文本嵌入共享潜在空间。在34个人体器官的1亿个组织学图像块、160万文本-组织学对和1080万RNA-组织学对上预训练。

Result: 在文本条件和图像到图像生成中，FID分数降低50%，少样本分类准确率提高47%；RNA条件组织学生成中FID降低23%；作为虚拟染色剂，H&E到免疫组化和多重免疫荧光转换中标记物相关性提高37%。

Conclusion: 在异质病理学模态上预训练的单一统一生成模型可以显著优于专门替代方案，为多模态组织病理学提供了可扩展的计算框架。
...
TL;DR: ComPrivDet是一种在压缩视频中高效检测隐私对象的方法，通过复用I帧推理结果，跳过或轻量化处理P/B帧检测，在保持高准确率的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 随着物联网深度融入日常生活，用户对视频数据隐私泄露的担忧日益增加。大规模视频分析中逐帧保护会引入显著延迟，而现有对象检测方法需要完全解码视频或逐帧处理压缩视频，导致解码开销或准确率下降。

Method: 提出ComPrivDet方法，通过复用I帧的推理结果，利用压缩域线索识别新对象出现，从而跳过P帧和B帧的检测，或使用轻量级检测器进行高效精炼。

Result: 在隐私人脸检测中保持99.75%的准确率，隐私车牌检测中保持96.83%的准确率，同时跳过超过80%的推理。相比现有压缩域检测方法，平均准确率提高9.84%，延迟降低75.95%。

Conclusion: ComPrivDet提供了一种高效的压缩视频隐私对象检测方案，在保持高检测准确率的同时显著降低了计算开销和延迟，适用于大规模视频分析场景。
...
TL;DR: CSRS方法通过回溯重推理机制、软化频率奖励和视觉语义扰动，提升多模态大语言模型在无监督自进化中的推理能力，特别是在几何任务上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有无监督自进化方法主要依赖多数投票选择伪黄金答案，但这可能源于模型内在偏见而非保证推理路径的客观正确性，导致性能退化。
...
Conclusion: CSRS通过改进反馈信号质量，有效解决了现有自进化方法中的偏见问题，提升了多模态大语言模型的推理能力和稳定性。
...
TL;DR: ART提出自适应关系Transformer，通过时间感知关系图捕捉交互演化，自适应交互剪枝减少冗余计算，在轨迹预测任务上实现高精度和高效率
...
Result: 在ETH/UCY和NBA基准测试中，ART实现了最先进的精度，同时保持高计算效率

Conclusion: ART通过显式建模交互演化和自适应剪枝，在行人轨迹预测任务上实现了精度和效率的平衡，为机器人相关应用提供了有效解决方案
...
TL;DR: DreamPRVR提出了一种粗到细的表示学习范式，通过生成全局上下文语义寄存器来改进部分相关视频检索，解决了查询模糊性和局部噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在全局上下文感知不完整的问题，难以处理查询模糊性和由虚假响应引起的局部噪声，需要更好的方法来处理部分相关视频检索任务。

Method: 采用粗到细表示学习范式：1) 生成全局上下文语义寄存器作为粗粒度高亮；2) 通过概率变分采样器初始化寄存器，文本监督截断扩散模型迭代优化；3) 文本语义结构学习构建良好文本潜在空间；4) 寄存器自适应融合到视频标记中，通过寄存器增强的高斯注意力块实现上下文感知特征学习。

Result: 大量实验表明，DreamPRVR在性能上超越了现有最先进方法。

Conclusion: DreamPRVR通过粗到细表示学习范式有效解决了部分相关视频检索中的全局上下文感知问题，显著提升了检索性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有VICL提示检索方法主要关注图像相似性而忽视标签一致性，导致可能检索到视觉相似但标签不一致的提示，从而降低VICL性能。研究发现查询与提示之间的标签一致性越高，VICL效果越好。

Method: 提出LaPR框架：1）设计图像-标签联合表示来显式整合标签信息；2）引入混合专家机制处理测试时查询标签不可用的情况，每个专家捕捉特定标签模式，路由器推断查询自适应混合权重；3）设计交替优化策略，分别使用VICL性能引导对比损失和标签引导对比损失训练专家和路由器。

Result: 在上下文分割、检测和着色任务上取得显著且一致的性能提升。LaPR在不同特征提取器和交叉折叠场景中泛化良好，证明了标签利用在VICL提示检索中的重要性。

Conclusion: 标签一致性对VICL性能至关重要，LaPR框架通过标签感知的提示检索有效提升了VICL性能，强调了在提示检索中利用标签信息的重要性。
...
TL;DR: 该研究提出了一种基于视觉大语言模型的方法，通过Set-of-Mark提示和用户注视轨迹来改进人-物交互预测，在HD-EPIC数据集上超越了现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中预测人-物交互对于指导用户日常活动和理解其短期/长期目标至关重要。现有方法存在视觉基础能力不足和用户意图理解有限的问题。

Method: 使用视觉大语言模型，通过Set-of-Mark提示增强视觉基础能力，利用用户最近注视点形成的轨迹理解用户意图，并引入逆指数采样策略捕捉交互前的时序动态。

Result: 在HD-EPIC第一人称视角数据集上的实验表明，该方法超越了当前最先进的方法，并展示了其模型无关的特性。

Conclusion: 该方法通过改进视觉基础和用户意图理解，有效提升了人-物交互预测性能，为智能辅助系统提供了更好的交互预测能力。
...
Method: 构建包含立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达的多传感器数据集，提供统一的3D和2D基准测试，并提出将传感器特定线索集成到统一特征空间的融合框架。

Result: 建立了公平的跨传感器比较基准，提供了代表性单模态和多模态方法的基线结果，提出的融合框架提高了不同天气和光照条件下的3D检测鲁棒性。

Conclusion: DSERT-RoLL数据集解决了新型传感器数据稀缺问题，为跨传感器组合研究提供了系统平台，提出的融合框架增强了多模态感知在复杂环境下的鲁棒性。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，发现在预训练模型微调效果有限的情况下，提出了利用多级表示和自适应特征融合的SciLT框架，显著提升了头尾类别的平衡性能。
...
Method: 提出SciLT框架，通过自适应特征融合和双监督学习利用多级表示。具体包括：1）联合利用倒数第二层和最终层特征；2）自适应特征融合机制；3）双监督学习策略，实现头尾类别的平衡性能。

Result: 在三个科学基准测试上的实验表明：1）微调基础模型带来的增益有限；2）倒数第二层特征对尾部类别特别重要；3）SciLT框架在头尾类别上均取得平衡且优异的性能，显著优于现有方法。

Conclusion: SciLT为科学长尾识别建立了强大实用的基准，为适应基础模型到具有显著领域偏移的科学数据提供了有价值的指导，证明了多级表示在科学图像长尾识别中的重要性。
...
TL;DR: 论文提出ResGuard模块，通过增强图像依赖的嵌入来防御已知原始攻击(KOA)，将水印提取准确率从59.87%提升到99.81%。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习水印采用的END架构虽然能抵抗随机信道失真，但忽视了攻击者拥有原始-水印图像对时的已知原始攻击(KOA)漏洞。这种攻击通过简单的残差估计就能几乎完全去除水印。

Method: 提出ResGuard插件模块：1) 残差特异性增强损失函数，强制残差与宿主图像紧密耦合；2) 辅助KOA噪声层，在训练时注入残差式扰动，使解码器在嵌入不一致时保持可靠。

Result: ResGuard显著提升了KOA鲁棒性，将平均水印提取准确率从59.87%提高到99.81%，同时保持视觉质量。

Conclusion: ResGuard通过增强图像依赖的嵌入有效防御了已知原始攻击，为深度学习水印系统提供了重要的安全增强。
...
TL;DR: FunFact是一个从RGB-D图像构建概率开放词汇功能3D场景图的框架，通过联合概率推理解决功能关系模糊性，显著提升校准置信度。
...
Conclusion: 整体概率建模对功能场景理解有益，FunFact框架通过联合推理所有功能边及其边际，产生更好的校准置信度分数。
...
TL;DR: VidNum-1.4K是一个包含1,379个人工标注视频问答对的基准测试，专门评估视频语言模型在真实世界复杂场景中的数值推理能力，揭示当前模型存在显著推理差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准测试局限于狭窄领域（如重复运动）或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中复杂的多步数值逻辑推理能力。

Method: 构建VidNum-1.4K基准测试，包含1,379个严格人工标注的视频问答对，涵盖高度多样化的环境（物体、动作、事件量化），采用三层层次结构：从直接视觉感知到基于视频的组合数值推理，要求模型执行算术运算、比较和基于时间证据的逻辑推理。

Result: 评估多种最先进视频语言模型显示显著推理差距：Gemini-3.1-pro仅达到60%准确率阈值，代表性开源模型在25%-45%范围内挣扎，表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K作为具有挑战性的诊断测试平台，揭示了当前视频语言模型在真实世界数值推理方面的不足，为下一代数值视频智能发展提供了评估标准。
...
TL;DR: XSeg是最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30种常见违禁品类别。研究人员还提出了基于SAM的APSAM模型，通过能量感知编码器和自适应点生成器改进标注效率。


<details>
  <summary>Details</summary>
Motivation: 当前X射线违禁品检测方法主要依赖边界框标注，缺乏像素级监督和真实世界数据，限制了模型的泛化能力和性能。需要更高质量的数据集和更高效的标注方法来解决这些问题。

Method: 1) 构建XSeg数据集：整合公共数据集和合成数据，通过自定义数据清洗管道过滤低质量样本；2) 提出APSAM模型：基于SAM构建，引入能量感知编码器改进掩码解码器初始化，增强对重叠物品的敏感性；3) 设计自适应点生成器：允许用户仅通过单个粗略点提示获得精确的掩码标签。

Result: XSeg成为迄今为止最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个最新常见违禁品类别。APSAM在XSeg数据集上表现出优越性能，显著提高了标注效率和准确性。

Conclusion: XSeg数据集和APSAM模型共同解决了X射线违禁品检测中的关键限制，提供了高质量的分割数据和高效的标注工具，有望推动该领域的研究和应用发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统图信号处理技术中的图结构和边权重通常通过统计和计算方法设置，而图结构学习方法能提供更可靠和灵活的数据表示。本文旨在开发一种基于图学习的黑色素瘤检测方法，利用超像素图表示来改善皮肤病变图像分析。

Method: 提出两种图表示：超像素集合图(SEG)和超像素层次图(SHG)，分别生成无和有父子约束的多层次超像素图。探索两种边权重分配技术：手工设计的高斯权重和基于优化的学习权重。节点信号基于纹理、几何和颜色超像素特征。研究图边阈值化(25%、50%、75%)对性能的影响。在ISIC2017数据集上使用不同分类器进行实验，并通过数据增强缓解类别不平衡。

Result: 学习型超像素集合图结合纹理节点信号获得最佳性能，达到99.00%准确率和99.59% AUC。边阈值化分析显示适当的边修剪能改善性能。

Conclusion: 基于图学习的方法在黑色素瘤检测中表现出优异性能，特别是学习型超像素集合图与纹理特征的组合。该方法为生物医学图像分析中的图信号处理提供了有效的框架。
...
TL;DR: 提出了一种从多视角图像重建高保真头发的紧凑管道，通过将发丝聚类为代表性发卡并共享纹理码本，结合3D高斯泼溅渲染，显著减少了重建时间和存储需求。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法虽然能实现逼真效果，但通常需要数百万个基元，导致高存储和渲染成本。观察到头发在发型中具有结构和视觉相似性，希望通过聚类和共享机制来优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法通常只能控制相机运动或物体动态中的一种，或者依赖模糊的2D线索，这些线索会将相机引起的视差与真实物体运动混为一谈。需要一种能够统一控制两种运动类型的框架。

Method: SymphoMotion包含两个核心机制：1）相机轨迹控制机制，整合显式相机路径和几何感知线索，确保稳定的视角转换；2）物体动态控制机制，结合2D视觉引导和3D轨迹嵌入，实现深度感知的空间一致物体操控。同时构建了RealCOD-25K数据集，包含配对的相机姿态和物体级3D轨迹。

Result: 广泛的实验和用户研究表明，SymphoMotion在视觉保真度、相机可控性和物体运动准确性方面显著优于现有方法，为统一运动控制建立了新的基准。

Conclusion: SymphoMotion提供了一个统一的运动控制框架，能够同时精确控制相机轨迹和物体动态，解决了现有方法的局限性，并通过RealCOD-25K数据集填补了统一运动控制领域的数据空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有专有模型如Sora2在多参考角色条件下生成多镜头视频方面取得进展，但学术研究有限。当参考图像外观高度相似时，模型容易出现参考混淆问题，语义相似标记会降低模型检索正确上下文的能力
...
Conclusion: PoCo通过位置编码作为上下文控制器，有效解决了多参考图像相似时的参考混淆问题，为多角色多镜头视频生成提供了可靠的技术方案
...
TL;DR: SA-DSVN是一个3D卷积架构，利用宇宙射线μ子断层扫描进行钢筋混凝土结构缺陷的体素级分割，通过联合处理散射运动学数据和次级电磁簇射多重性数据，显著提高了缺陷检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖μ子散射角进行重建，忽略了次级电磁簇射多重性这一重要信息。本文旨在开发一种能够充分利用所有可用μ子相互作用数据的深度学习架构，提高钢筋混凝土结构缺陷检测的准确性和效率。

Method: 提出SA-DSVN 3D卷积架构，通过独立的编码器流分别处理散射运动学数据（9通道）和次级电磁簇射多重性数据（40通道），然后通过交叉注意力机制进行融合。使用云原生Geant4模拟框架Vega生成训练数据，包含450万个μ子事件和900个体积，涵盖蜂窝、剪切断裂、腐蚀空洞和分层四种缺陷类型。

Result: 在60个独立模拟验证体积上，模型达到96.3%的体素准确率，各类缺陷的Dice分数在0.59-0.81之间，体积级检测灵敏度达到100%，每个体积推理时间仅10毫秒。消融研究表明，仅使用簇射多重性流就能将缺陷平均Dice从0.535（仅散射）提升到0.685。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，对于学习型μ子断层扫描重建具有重要意义。SA-DSVN架构为钢筋混凝土结构缺陷检测提供了一种高效准确的解决方案。
...
TL;DR: ICBench是一个新的大规模图像描述基准，包含12个内容类别，使用10个先进MLLM在2K图像上生成了40K个短描述和长描述，并通过人类主观评估获得MOS分数，同时提出了基于图像-文本-图像框架的自动化评估指标ITIScore。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在三个主要问题：描述长度多样性有限、缺乏最新先进MLLM的参与、人类标注不足导致评估偏差，这些问题限制了全面评估现代MLLM性能的能力。

Method: 1) 构建ICBench基准：涵盖12个内容类别，使用10个先进MLLM在2K图像上生成短描述和长描述，共40K个描述；2) 进行广泛的人类主观研究，获得细粒度评估维度的平均意见分数(MOS)；3) 提出ITIScore自动化评估指标，基于图像-文本-图像框架，通过重建一致性来衡量描述质量。

Result: 实验结果表明：1) ITIScore自动指标与人类判断有很强的对齐性；2) ITIScore在其他公共描述数据集上表现出强大的零样本泛化能力；3) 数据集和模型将在发表后发布。

Conclusion: ICBench通过提供多样化的描述长度、包含最新MLLM、结合人类评估和自动化指标，解决了现有图像描述基准的局限性，为全面评估现代MLLM的图像理解能力提供了更可靠的基准。
...
TL;DR: M2StyleGS：基于3D高斯溅射和多模态CLIP知识的实时3D风格迁移方法，支持文本和图像作为风格参考，通过精确特征对齐和损失函数优化实现高质量风格化视图生成。
...
Method: 1. 使用3D高斯溅射（3DGS）作为3D表示；2. 利用CLIP提炼的多模态知识作为参考风格；3. 采用细分流（subdivisive flow）进行精确特征对齐，将CLIP文本-视觉组合特征映射到VGG风格特征；4. 引入观察损失确保风格化场景与参考风格匹配；5. 引入抑制损失抑制参考颜色信息在解码过程中的偏移。

Result: M2StyleGS能够以文本或图像为参考生成一组风格增强的新视图。实验表明，该方法在视觉质量上表现优异，在一致性方面比先前工作提升高达32.92%。

Conclusion: M2StyleGS通过多模态风格参考和精确特征对齐机制，实现了高质量、高一致性的实时3D风格迁移，为VR/AR应用提供了更灵活的风格化解决方案。
...
TL;DR: 提出自动化框架，利用粗粒度CityGML建筑模型作为几何先验，通过表面匹配和二进制整数优化，结合高精度MLS数据精化建筑立面几何，确保拓扑有效性


<details>
  <summary>Details</summary>
Motivation: 数字孪生需要高精度地理空间数据，但传统基于ALS的CityGML建筑模型在立面几何上存在显著缺陷，需要与高精度MLS数据集成来恢复详细立面几何

Method: 利用粗模型作为几何先验，通过表面匹配识别过时表面，采用二进制整数优化从候选数据中选择最优面片，并在优化中强制执行硬约束以确保拓扑有效性

Result: 方法有效校正了立面错位，将云到网格RMSE降低了约36%，实现了厘米级对齐，同时保证了严格的水密性和流形几何
...
<details>
  <summary>Details</summary>
Motivation: 标准自回归模型虽然训练稳定高效，但其下一标记预测与文本条件运动生成所需的时间结构不对齐，难以捕捉长程运动结构

Method: 提出MoScale框架：1）从粗到细的分层自回归生成；2）跨尺度分层细化改进每尺度初始预测；3）尺度内时间细化进行选择性双向重新预测

Result: 在文本到运动生成任务上达到SOTA性能，训练效率高，模型规模扩展性好，并能零样本泛化到多种运动生成和编辑任务

Conclusion: MoScale通过分层自回归框架解决了传统AR模型在运动生成中的对齐问题，实现了高效训练和卓越性能，展示了分层方法在序列生成任务中的优势
...
<details>
  <summary>Details</summary>
Motivation: 夜间图像去雾面临雾霾、光晕、非均匀光照、颜色失真和传感器噪声等多种退化因素，这些因素使得传统白天去雾方法的假设失效，需要专门针对夜间复杂退化场景的解决方案
...
Conclusion: HistoFusionNet提供了一个统一的框架，特别适合处理真实夜间雾霾场景中的异质退化问题，通过直方图引导表示学习和频率自适应特征细化的结合，实现了对夜间复杂退化图像的有效恢复
...
TL;DR: 提出基于注意力分布熵值的补丁剪枝方法，通过低熵补丁保留重要信息、高熵补丁剪枝冗余信息，降低Transformer计算成本，并在细粒度图像识别任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: Transformer的自注意力机制虽然能捕捉长距离依赖，但其计算成本随token数量呈二次方增长。补丁剪枝通过移除冗余补丁来缓解这一成本，但需要有效识别信息丰富的补丁。

Method: 提出基于注意力分布香农熵的补丁重要性评估准则：低熵补丁（注意力集中）保留为重要，高熵补丁（注意力分散）视为冗余。进一步扩展到Rényi熵，强调注意力峰值，支持根据任务需求和计算限制调整剪枝策略。

Result: 在细粒度图像识别任务中，该方法在保持准确率的同时显著降低了计算量。通过Rényi熵调整剪枝策略能获得进一步收益，改善准确率与计算量之间的权衡。

Conclusion: 基于熵的补丁剪枝方法有效降低了Transformer的计算成本，Rényi熵扩展提供了更灵活的剪枝策略调整能力，在计算效率和模型性能之间取得了良好平衡。
...
TL;DR: EyeBench-V2是一个眼底图像增强基准测试，旨在解决现有评估方法的不足，通过多维度临床对齐评估、专家指导设计和可操作见解来弥合增强模型性能与临床实用性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像增强模型的评估存在三个主要问题：1) 传统去噪指标（如PSNR、SSIM）无法捕捉临床相关特征；2) 缺乏统一评估协议来处理成对和非成对增强方法；3) 需要提供可操作见解来指导未来临床对齐增强模型的发展。

Method: 提出EyeBench-V2基准测试，包含三个关键贡献：1) 通过下游任务进行多维度临床对齐评估，包括血管分割、糖尿病视网膜病变分级、未见噪声模式泛化和病变分割；2) 专家指导的评估设计，包含新颖数据集和结构化人工评估协议；3) 提供可操作见解，对现有生成模型进行严格的任务导向分析。

Result: 论文提出了一个全面的基准测试框架，能够公平比较成对和非成对增强方法，通过临床相关任务评估模型性能，并提供医学专家的人工评估协议来评估临床关键方面。

Conclusion: EyeBench-V2基准测试填补了眼底图像增强评估的空白，为临床研究人员提供证据支持决策，同时识别当前方法的局限性，为下一代增强模型的设计提供信息。
...
TL;DR: ActivityForensics：首个大规模视频活动级伪造定位基准，包含6K+伪造视频片段，提出TADiff方法，建立综合评估协议


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注外观级伪造（如人脸交换、物体移除），而视频生成技术的发展催生了活动级伪造（修改人类动作以扭曲事件语义），这些高度欺骗性的伪造严重威胁媒体真实性和公众信任

Method: 提出ActivityForensics基准，包含6K+无缝融入视频上下文的伪造片段；提出Temporal Artifact Diffuser (TADiff)基线方法，通过扩散特征正则化器暴露伪影线索

Result: 建立了全面的评估协议（域内、跨域、开放世界），并对多种最先进的伪造定位方法进行了基准测试，为未来研究提供基础

Conclusion: ActivityForensics填补了活动级视频伪造定位研究的空白，TADiff方法简单有效，数据集和代码已开源，将推动该领域发展
...
### [59] [SPARK-IL: Spectral Retrieval-Augmented RAG for Knowledge-driven Deepfake Detection via Incremental Learning](https://arxiv.org/abs/2604.03833)
*Hessen Bougueffa Eutamene,Abdellah Zakaria Sellam,Abdelmalik Taleb-Ahmed,Abdenour Hadid*

Main category: cs.CV

TL;DR: SPARK-IL是一个基于检索增强的AI生成图像检测框架，通过双路径频谱分析和增量学习，在19个生成模型上达到94.6%的平均准确率。
...
Method: 提出SPARK-IL框架：1) 双路径频谱分析：使用部分冻结的ViT-L/14编码器获取语义表示，同时并行处理原始RGB像素嵌入；2) 多频带傅里叶分解：将两条路径分解为四个频率带，每个频带使用Kolmogorov-Arnold Networks（KAN）配合专家混合进行特定变换；3) 交叉注意力融合：通过交叉注意力和残差连接融合频谱嵌入；4) 检索增强推理：使用余弦相似度从Milvus数据库中检索k个最近邻标记签名，通过多数投票进行预测；5) 增量学习：扩展数据库并使用弹性权重巩固来保留先前学习的变换。

Result: 在UniversalFakeDetect基准测试中，涵盖19个生成模型（包括GANs、人脸交换和扩散方法），SPARK-IL实现了94.6%的平均准确率。

Conclusion: SPARK-IL通过结合双路径频谱分析和增量学习的检索增强框架，有效解决了AI生成图像检测中的跨生成器泛化问题，在多个生成模型上表现出色，代码将公开提供。
...
TL;DR: 本文提出了一种任务引导的多标注三元组损失方法，通过互信息标准选择跨任务最信息丰富的三元组，替代了传统静态权重平衡方法，在航空野生动物数据集上取得了更好的分类和回归性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务三元组损失方法依赖静态权重来平衡不同类型标注之间的监督，但静态权重需要调参且无法考虑任务在塑造共享表示时的交互作用。

Method: 提出任务引导的多标注三元组损失，通过互信息标准选择跨任务最信息丰富的三元组，这种方法修改了哪些样本影响表示，而不是调整损失幅度。

Result: 在航空野生动物数据集上的实验表明，相比多种三元组损失设置，提出的任务引导选择方法在分类和回归性能上都有提升，证明了任务感知的三元组选择能为下游任务产生更有效的共享表示。

Conclusion: 任务感知的三元组选择通过识别跨任务最信息丰富的样本，能够产生更有效的共享表示，优于依赖静态权重平衡的传统多任务三元组损失方法。
...
TL;DR: 提出半监督知识蒸馏框架，将预训练视觉基础模型压缩为紧凑专家模型，用于实例分割任务，在有限标注和大量无标注数据下实现高效压缩


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型感知能力强但计算量大难以部署，且适应新任务需要昂贵的标注成本，需要一种高效压缩方法

Method: 三阶段框架：1) 通过自训练和对比校准进行领域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减轻伪标签偏差。核心是实例感知的像素级对比损失

Result: 在Cityscapes和ADE20K数据集上，约11倍小的学生模型比零样本基础模型提升11.9和8.6 AP，比适应后的教师模型提升3.4和1.5 AP，超越现有半监督知识蒸馏方法

Conclusion: 提出的半监督知识蒸馏框架能有效压缩视觉基础模型，在实例分割任务上实现高性能的紧凑模型，显著减少计算成本同时保持强感知能力
...
TL;DR: SafeCtrl是一个区域感知的安全控制框架，采用"检测-抑制"范式，通过精确定位风险区域并局部抑制有害语义，在保持安全性和内容保真度之间取得更好平衡，同时增强对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型的广泛部署面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预方法存在两个关键局限：1）安全性和上下文保真度之间的严重权衡；2）容易受到对抗性攻击。

Method: SafeCtrl采用区域感知安全控制框架，基于"检测-抑制"范式。首先使用注意力引导的检测模块精确定位特定风险区域，然后通过基于图像级直接偏好优化（DPO）的局部抑制模块，仅在检测区域内中和有害语义，将不安全对象转化为安全替代物，同时保持周围上下文完整。

Result: 在多个风险类别上的广泛实验表明，SafeCtrl在安全性和保真度之间实现了优于现有方法的权衡。关键的是，该方法对对抗性提示攻击表现出更强的鲁棒性。

Conclusion: SafeCtrl提供了一个精确且鲁棒的解决方案，用于负责任的内容生成，通过局部化干预在保持内容质量的同时有效防止有害内容生成，并增强对抗攻击的防御能力。
...
TL;DR: CM-GLasso是一种跨模态图套索方法，通过文本-视觉对齐策略和跨注意力蒸馏机制，在共享潜空间中学习多模态表示，同时解耦不变性和类别特定的精度矩阵。
...
Method: 提出跨模态图形套索（CM-GLasso），结合新颖的文本-视觉对齐策略和统一的视觉-语言编码器，将多模态特征严格对齐到共享潜空间。引入跨注意力蒸馏机制将高维补丁压缩为显式语义节点，提取空间感知的跨模态先验。通过定制GLasso估计和公共-特定结构学习（CSSL）的联合目标，使用交替方向乘子法（ADMM）优化。

Result: 在涵盖自然和医学领域的八个基准测试上进行广泛实验，CM-GLasso在生成式分类和密集语义分割任务中建立了新的最先进水平。

Conclusion: CM-GLasso克服了传统方法在多模态表示学习中的基本限制，通过严格的模态对齐、跨注意力蒸馏和联合优化框架，实现了同时解耦不变性和类别特定精度矩阵的能力，为多模态学习提供了有效的解决方案。
...
TL;DR: VLA-Forget是一个针对视觉-语言-动作模型的混合遗忘框架，通过选择性编辑和分层遗忘来移除不安全行为，同时保持感知和推理能力
...
Method: 提出VLA-Forget混合遗忘框架，结合感知层的比例感知选择性编辑和跨模态特异性，以及推理/动作层的分层选择性遗忘。通过三目标联合优化（目标遗忘、感知保留、推理保持），对视觉编码器、投影器和上层动作生成变换器块进行分阶段更新。

Result: 相比强基线方法，VLA-Forget在遗忘效果上提升10%，感知特异性保留提升22%，推理和任务成功率保持提升9%，量化后恢复减少55%。

Conclusion: VLA-Forget有效解决了VLA模型中的遗忘挑战，能够在移除不良行为的同时保持模型的感知和推理能力，为具身基础模型的安全部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效的提示学习方法主要关注一阶视觉特征（空间特征图）与文本提示的对齐，但这些空间纠缠的特征对领域偏移和局部噪声高度敏感，仅依赖一阶信息不足以实现鲁棒的模型适应

Method: 提出了Gram-Anchored Prompt Learning (GAPL)框架，通过Gram矩阵引入二阶统计流，增强标准的一阶空间交互。该方法将提示锚定到二阶先验，使语言表示能够动态适应不同领域的统计分布偏移

Result: 大量实验表明二阶特征的有效性，GAPL在各种基准测试中表现出色

Conclusion: 通过结合局部语义对齐和全局结构一致性，基于二阶统计量的提示学习方法能够显著提升视觉语言模型对领域偏移的鲁棒性和适应能力
...
TL;DR: HMAT是一种用于古代壁画修复的统一框架，结合了动态滤波和Transformer架构，能够同时处理局部纹理建模和长距离结构推断，在保持未损坏区域真实性的同时重建大面积缺失结构。


<details>
  <summary>Details</summary>
Motivation: 古代壁画作为珍贵的文化遗产，由于环境暴露、材料老化和人类活动等原因遭受严重退化。修复这些艺术品具有挑战性，因为它需要同时重建大面积缺失结构并严格保护真实未损坏区域。

Method: 提出了混合掩码感知Transformer（HMAT）框架，集成了掩码感知动态滤波用于鲁棒的局部纹理建模，以及Transformer瓶颈用于长距离结构推断。还引入了掩码条件风格融合模块来处理退化的多样性形态，并设计了带有硬门控跳跃连接的教师强制解码器，以在有效区域强制保真度并将重建集中在缺失区域。

Result: 在DHMural数据集和精心策划的九色鹿数据集上进行了评估，实验结果表明，与最先进的方法相比，该方法具有竞争性的性能，同时产生更结构一致和视觉上忠实的修复结果。

Conclusion: HMAT为文化遗产壁画的数字修复提供了一个有效的解决方案，能够产生结构一致且视觉上忠实的修复结果。
...
Result: 在多前沿MLLMs、任务以及常识推理和内容审核基准测试中，协调多模态攻击的成功率达到83.43%，而单模态攻击仅为34.93%，证明多模态攻击是更有效的威胁策略。

Conclusion: 多模态排版攻击是多模态推理中关键且未被充分探索的攻击策略，揭示了MLLMs的跨模态脆弱性，对安全关键应用中的模型部署具有重要意义。
...
Method: 提出OASIC方法：1）测试时通过掩蔽遮挡物模式去除干扰；2）训练时使用随机掩蔽模拟不同程度遮挡；3）测试时估计遮挡严重程度并选择针对该程度优化的模型。

Result: 结合灰度掩蔽和自适应模型选择，OASIC在遮挡图像上的AUC_occ比标准训练提升+18.5，比在未遮挡图像上微调提升+23.7。

Conclusion: 通过同时处理遮挡干扰和可见信息损失，并采用严重程度感知的模型选择策略，OASIC显著提升了遮挡场景下的分类性能。
...
### [69] [MinerU2.5-Pro: Pushing the Limits of Data-Centric Document Parsing at Scale](https://arxiv.org/abs/2604.04771)
...
TL;DR: MINERUPRO通过数据工程和训练策略优化，在保持1.2B参数架构不变的情况下，将文档解析性能提升到新高度，超越了参数规模大200倍的模型。
...
Method: 提出了数据引擎，围绕覆盖率、信息量和标注准确性进行协同设计：1）多样性和难度感知采样将训练数据从不足1000万扩展到6550万样本；2）跨模型一致性验证利用异构模型的输出一致性评估样本难度并生成可靠标注；3）Judge-and-Refine管道通过渲染-验证迭代校正改进困难样本的标注质量。采用三阶段渐进训练策略：大规模预训练、困难样本微调和GRPO对齐。

Result: 在OmniDocBench v1.6上达到95.69分，相比同架构基线提升2.71分，超越了所有现有方法，包括参数规模大200倍以上的模型。同时改进了评估协议，修正了元素匹配偏差并引入了困难子集。

Conclusion: 通过系统化的数据工程和训练策略优化，可以在不改变模型架构的情况下显著提升文档解析性能，证明了训练数据质量比模型架构创新更为关键。
...
TL;DR: HOIGS提出了一种新方法，通过交叉注意力机制显式建模人-物交互变形，使用不同基线提取特征，显著提升了交互场景的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有高斯溅射方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似为单一场，难以捕捉交互丰富的动态场景。需要一种能显式建模人-物交互的方法来提升重建质量。

Method: 提出HOIGS方法，通过交叉注意力机制的HOI模块显式建模人-物交互引起的变形。使用HexPlane提取人体特征，Cubic Hermite Spline提取物体特征，然后集成这些异构特征来捕捉相互依赖的运动。

Result: 在多个数据集上的实验表明，HOIGS在涉及遮挡、接触和物体操纵的场景中显著优于现有最先进的人体中心和4D高斯方法，证明了显式建模人-物交互的重要性。

Conclusion: 显式建模人-物交互对于高保真动态场景重建至关重要，HOIGS通过交叉注意力机制和异构特征提取有效解决了现有方法的局限性，为交互丰富的动态场景重建提供了新思路。
...
TL;DR: 1.x-Distill是首个分数步蒸馏框架，突破传统整数步限制，在1.67和1.74个有效NFE下实现高质量图像生成，比原始28x2 NFE采样快33倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成高质量图像但迭代去噪计算成本高，现有DMD方法在两步或更少步数时会出现多样性崩溃和保真度下降问题。

Method: 1.分析教师CFG在DMD中的作用并改进以抑制模式崩溃；2.引入两阶段策略：第一阶段通过多样性保持分布匹配学习粗结构，第二阶段通过推理一致对抗蒸馏细化细节；3.设计轻量补偿模块实现蒸馏-缓存协同训练。

Result: 在SD3-Medium和SD3.5-Large上，1.x-Distill超越现有少步方法，在1.67和1.74个有效NFE下实现更好的质量和多样性，比原始28x2 NFE采样快33倍。

Conclusion: 1.x-Distill首次实现分数步蒸馏，建立了1.x步生成作为蒸馏扩散模型的实用方案，在极低步数下保持高质量和多样性。
...
TL;DR: 提出ATSS方法检测AI生成视频，通过分析异常时间自相似性指纹来识别AIGV，利用三重相似度表示和跨注意力融合机制，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: AI生成视频已达到前所未有的真实感，对数字取证构成严重威胁。现有检测器主要关注局部伪影或短期时间不一致性，往往无法捕捉控制全局时间演化的生成逻辑，限制了AIGV检测性能。

Method: 提出ATSS方法，识别AI生成视频中的异常时间自相似性指纹。通过利用帧级描述构建视觉、文本和跨模态相似度矩阵，量化固有时间异常。使用专用Transformer编码器对这些矩阵进行编码，并通过双向跨注意力融合模块集成，有效建模模态内和模态间动态。

Result: 在四个大规模基准测试（GenVideo、EvalCrafter、VideoPhy、VidProM）上的广泛实验表明，ATSS在AP、AUC和ACC指标上显著优于最先进方法，并在不同视频生成模型中表现出优异的泛化能力。

Conclusion: ATSS通过识别AI生成视频中的异常时间自相似性指纹，提供了一种有效的多模态检测框架，能够捕捉控制全局时间演化的生成逻辑，显著提升了AIGV检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型虽然展现出广泛的视觉推理能力，但其构建方法被锁在专有的强化学习流程和非公开数据中。研究旨在开发完全开源的视觉推理模型，揭示构建强大视觉推理器的配方。

Method: 1. 构建Vero-600K数据集：从59个数据集中收集600K样本，涵盖六大任务类别；2. 设计任务路由奖励机制：处理异构答案格式；3. 基于Qwen3-VL-8B-Instruct等基础模型进行训练；4. 在30个基准测试套件VeroEval上进行评估。

Result: Vero在30个基准测试中平均提升3.7-5.5个百分点，在23个基准上超越Qwen3-VL-8B-Thinking（无需额外专有思维数据）。Vero-600K在不同任务类别上均超越现有强化学习数据集。系统消融实验表明，广泛的数据覆盖是强化学习扩展的主要驱动力。

Conclusion: 通过构建大规模、多样化的强化学习数据集和适当的奖励设计，可以开发出性能优异的开源视觉语言模型。不同任务类别需要不同的推理模式，广泛的数据覆盖是实现强大视觉推理的关键。所有数据、代码和模型均已开源。
...
Method: 1. 在端到端流程中引入可微分自适应补丁选择器，提高提取补丁的质量；2. 集成多任务特征提取模块；3. 结合可微分光束法平差模块，利用逆深度先验，有效学习外观和几何信息。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的大量实验表明，DINO-VO在合成、室内和室外环境中都表现出强大的泛化能力，实现了最先进的跟踪精度。

Conclusion: DINO-VO通过端到端学习框架，有效连接了特征学习和状态估计，解决了传统VO系统在场景泛化方面的局限性，为视觉里程计提供了更鲁棒的解决方案。
...
TL;DR: 4C4D是一个从极稀疏相机（最少4个）视频中恢复4D动态场景的新框架，通过神经衰减函数增强4D高斯几何建模能力，实现高质量新视角渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要数十甚至上百个相机的密集多视角捕捉，而本文旨在解决从极稀疏相机（最少4个）视频中恢复4D动态场景的挑战，实现更实用的动态场景建模。

Method: 提出4C4D框架，核心是引入神经衰减函数作用于高斯不透明度，增强4D高斯的几何建模能力，通过平衡几何与外观建模的梯度分配，改善稀疏视角下的几何学习。

Result: 在多种相机重叠度的稀疏视角数据集上进行广泛实验，4C4D相比现有方法取得了更优越的性能，实现了高质量的4D动态场景恢复和新视角渲染。

Conclusion: 4C4D成功解决了从极稀疏相机视频中恢复4D动态场景的难题，通过神经衰减函数有效平衡了几何与外观建模，为实际应用中的动态场景捕捉提供了实用解决方案。
...
TL;DR: 基于YOLOv11和BoT-SORT/ByteTrack的离线实时交通监控系统，在典型条件下实现高精度车辆检测与计数
...
Method: 结合预训练YOLOv11检测器与BoT-SORT/ByteTrack多目标跟踪算法，采用PyTorch/OpenCV实现，并封装在Qt桌面UI中

Result: 系统在多种场景下达到66.67-95.83%的计数准确率；车辆检测精度：汽车0.97-1.00，卡车1.00；召回率：汽车0.82-1.00，卡车0.70-1.00；F1分数：汽车0.90-1.00，卡车0.82-1.00

Conclusion: 通过集成轻量级模型和云独立接口，展示了AI驱动交通监控系统的能力，为智慧城市现代化发展做出贡献
...
TL;DR: HGR框架通过可修正的假设节点和依赖感知图内存，在探索未知环境时利用语义预测进行定向探索，并在发现错误时系统性地撤回错误预测及其下游依赖，提高导航效率和记忆可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的导航系统通常将未探索区域视为语义未知，导致低效的前沿搜索。虽然视觉语言模型可以预测前沿语义，但错误的预测会嵌入记忆并通过下游推理传播，形成结构错误累积，仅靠置信度衰减无法解决。需要一种既能利用语义预测进行定向探索，又能在新证据矛盾时系统撤回错误的框架。

Method: 提出假设图精炼（HGR）框架：1）语义假设模块：估计前沿的上下文条件语义分布，根据目标相关性、旅行成本和不确定性对探索目标进行排序；2）验证驱动的级联校正：将现场观察与预测语义进行比较，一旦不匹配，就撤回被反驳的节点及其所有下游依赖。与加法式地图构建不同，该框架允许图通过剪除错误子图而收缩。

Result: 在GOAT-Bench上实现72.41%的成功率和56.22%的SPL，在A-EQA和EM-EQA两个问答基准上均显示出一致的改进。诊断分析表明，级联校正消除了约20%的结构冗余假设节点，将错误区域的重复访问减少了4.5倍，其中镜面和透明表面占校正预测错误的67%。

Conclusion: HGR框架通过将前沿预测表示为可修正的假设节点，并引入验证驱动的级联校正机制，能够在长期导航任务中保持记忆可靠性，有效解决语义预测错误传播问题，显著提升探索效率和任务成功率。
...
TL;DR: NTIRE 2026 3D恢复与重建挑战赛综述，重点研究在极端低光和烟雾退化等恶劣条件下的鲁棒3D重建方法


<details>
  <summary>Details</summary>
Motivation: 现实世界中的恶劣条件（如极端低光和烟雾环境）严重影响了3D重建的质量，需要开发鲁棒的3D重建管道来应对这些挑战

Method: 通过NTIRE 2026 3DRR挑战赛，收集了33个团队提交的方法，使用RealX3D基准测试在极端低光和烟雾退化条件下评估这些方法，并与最先进的基线进行比较分析

Result: 279名参与者注册，33个团队提交有效结果；评估显示在恶劣条件下的3D重建取得了显著进展；分析揭示了顶级方法的共享设计原则

Conclusion: 挑战赛展示了处理3D场景退化的有效策略，为恶劣条件下的鲁棒3D重建提供了重要见解和设计指导
...
TL;DR: 该论文提出了一种针对空间非均匀曝光校正的新范式，通过空间自适应调制权重和不确定性引导的损失函数来解决传统方法在非均匀曝光校正上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的曝光校正面临空间非均匀退化的挑战，不同区域的曝光误差常常共存于同一图像中。然而现有方法主要基于均匀曝光假设，使用全局调制信号和共享尺度重建损失，无法有效处理空间变化的校正需求。

Method: 提出空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像变换；引入基于HSL的补偿模块提升色彩保真度；提出不确定性引导的非均匀损失函数，根据局部恢复不确定性动态分配优化焦点。

Result: 大量实验表明，该方法在定性和定量评估上都优于现有最先进方法，能够有效处理空间非均匀曝光校正问题。

Conclusion: 该研究为空间非均匀曝光校正提供了新的解决方案，通过空间自适应调制和不确定性引导优化，显著提升了真实世界曝光校正的性能。
...
TL;DR: OP-GRPO：首个为流匹配模型设计的离线策略GRPO框架，通过重用高质量轨迹、重要性采样校正和轨迹截断，将训练效率提升至仅需原方法34.2%的训练步数


<details>
  <summary>Details</summary>
Motivation: 传统GRPO（流匹配模型的训练后优化）采用在线策略训练范式，样本效率低下，限制了训练效率。需要开发离线策略框架来重用高质量样本，提高训练效率

Method: 1. 主动选择高质量轨迹并存入回放缓冲区供后续训练重用；2. 提出序列级重要性采样校正，保持GRPO裁剪机制完整性；3. 识别晚期去噪步骤导致的条件不良离线策略比率，通过截断轨迹来缓解

Result: 在图像和视频生成基准测试中，OP-GRPO仅需平均34.2%的训练步数就能达到与Flow-GRPO相当或更优的性能，显著提升训练效率同时保持生成质量

Conclusion: OP-GRPO成功解决了GRPO样本效率低的问题，通过离线策略框架实现了高效的流匹配模型训练，为大规模生成模型的训练后优化提供了更实用的解决方案
...
TL;DR: 提出了一种用于遥感回归任务的测试时自适应框架，专门针对地表温度时空融合中的域偏移问题，通过不确定性引导、土地利用一致性约束和偏差校正，在无源数据和目标标签的情况下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在遥感应用中面临跨地理区域泛化困难的问题，主要原因是域偏移。现有测试时自适应方法主要针对分类任务，不适用于回归任务，特别是地表温度时空融合这样的回归问题。

Method: 提出不确定性感知的测试时自适应框架，仅更新预训练时空融合模型的融合模块。该方法基于认知不确定性指导、土地利用一致性约束和偏差校正，无需源数据或目标区域标注样本。

Result: 在意大利罗马、埃及开罗、西班牙马德里和法国蒙彼利埃四个不同气候的目标区域进行实验，使用法国奥尔良预训练模型，仅用10个测试时自适应epoch和有限无标签目标数据，RMSE和MAE平均分别提升24.2%和27.9%。

Conclusion: 该方法有效解决了遥感回归任务中的域偏移问题，通过轻量级的测试时自适应策略，在无需源数据和目标标注的情况下，显著提升了模型在新地理区域的泛化性能。
...
TL;DR: 提出双曲共嵌入框架，通过蕴含关系而非简单配对对齐来建模字体-印象对应关系，量化印象的"风格特异性"


<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛印象，但字体与印象描述并非一一对应：有些印象与多样风格广泛兼容，而其他印象则强烈约束字体集合。这种分级约束强度被称为风格特异性，需要量化建模

Method: 提出双曲共嵌入框架，将字体图像和印象描述（单标签或标签集）嵌入共享双曲空间，采用两种互补的蕴含约束：印象到字体的蕴含，以及印象间从低到高风格特异性的蕴含。这种表述诱导出径向结构，其中低风格特异性印象靠近原点，高风格特异性印象远离原点

Result: 在MyFonts数据集上的实验显示，相比强的一对一基线方法，双向检索性能得到提升。遍历和标签级分析表明，学习到的空间捕捉了从模糊到更风格特异性印象的连贯进展，提供了有意义的数据驱动风格特异性量化

Conclusion: 双曲共嵌入框架通过蕴含关系有效建模字体-印象对应关系，提供了可解释的几何度量来衡量印象对字体风格的约束强度，实现了风格特异性的量化分析
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视图多标签学习方法主要关注视图缺失情况，但在视图和标签都缺失的双缺失场景下研究不足。现有方法依赖对比学习或信息瓶颈理论，但缺乏显式结构约束，难以捕捉稳定且具有判别性的共享语义。
...
Conclusion: 该方法通过结构化的一致表示学习机制和融合教师自蒸馏框架，有效解决了多视图多标签学习中的双缺失问题，提升了模型的泛化能力。
...
TL;DR: GENFIG1是一个评估生成式AI模型能力的基准测试，要求模型根据论文标题、摘要、引言和图表说明生成能够清晰表达论文核心思想的"图1"。


<details>
  <summary>Details</summary>
Motivation: 科学论文中的"图1"通常是核心研究思想的视觉总结，这些图视觉简单但概念丰富，需要人类作者大量努力和迭代才能完成，这凸显了科学视觉传达的难度。因此需要建立一个基准来评估AI模型在这方面的能力。

Method: 从顶级深度学习会议论文中收集数据，进行严格的质量控制，引入与专家判断相关性好的自动评估指标。评估一系列代表性模型在GENFIG1上的表现。

Result: GENFIG1任务对现有最佳系统也构成显著挑战，表明当前模型在科学视觉传达方面仍有很大提升空间。

Conclusion: GENFIG1基准为多模态AI的未来发展奠定了基础，有助于推动模型在科学理解和视觉合成结合方面的进步。
...
<details>
  <summary>Details</summary>
Motivation: 现有VideoLLM大多是离线系统，不适合需要持续观察和及时响应的实时视频流。当前的流式VideoLLM方法依赖解耦的触发-响应管道或仅限于字幕式叙述，限制了开放问答和长时程交互的有效性。

Method: AURA是一个端到端的流式视觉交互框架，集成了上下文管理、数据构建、训练目标和部署优化，支持稳定的长时程流式交互。它使统一的VideoLLM能够持续处理视频流，支持实时问答和主动响应。

Result: AURA在流式基准测试中达到最先进性能，并支持一个实时演示系统，在2个80G加速器上以2 FPS运行，包含ASR和TTS功能。

Conclusion: AURA框架解决了现有流式VideoLLM的局限性，实现了端到端的流式视觉交互，为未来研究提供了实时推理框架和模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注自然图像理解或通用文本到图像合成，缺乏针对专业平面设计独特挑战的评估工具。专业设计需要将沟通意图转化为结构化布局、渲染排版准确的文本、操作分层构图、生成有效矢量图形以及推理动画等能力，这些都需要专门的评估框架。

Method: 构建了GraphicDesignBench (GDB)基准套件，包含50个任务，组织在五个维度上：布局、排版、信息图表、模板与设计语义、动画。每个维度在理解和生成两种设置下评估，基于LICA分层构图数据集中的真实世界设计模板。使用标准化指标分类法评估模型，涵盖空间准确性、感知质量、文本保真度、语义对齐和结构有效性。

Result: 评估前沿闭源模型的结果显示，当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、准确的矢量代码生成、细粒度排版感知以及动画的时间分解等问题基本未解决。虽然高层语义理解已可达到，但当任务要求精度、结构和构图意识时，差距显著扩大。

Conclusion: GDB为追踪AI系统向能够作为能力设计协作伙伴的进展提供了严格、可复现的测试平台。当前模型在专业设计的关键能力上仍有显著差距，特别是在需要精确性、结构性和构图意识的复杂任务中。该评估框架已公开可用。
...
TL;DR: DriveVA是一个自动驾驶世界模型，通过共享潜在生成过程联合解码未来视觉预测和动作序列，实现了规划与场景演化的紧密对齐，在跨数据集和传感器配置的泛化能力方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的泛化能力是关键挑战，现有世界模型方法在跨数据集和传感器配置的泛化能力有限，且规划与视觉想象的视频-轨迹一致性较差。

Method: 提出DriveVA模型，采用基于DiT的解码器联合预测未来动作序列（轨迹）和视频，继承大规模视频生成模型的运动动力学和物理合理性先验，并引入视频延续策略增强长时滚转一致性。

Result: 在NAVSIM挑战中达到90.9 PDM分数，在nuScenes数据集上平均L2误差和碰撞率分别降低78.9%和83.3%，在CARLA v2的Bench2drive上分别降低52.5%和52.4%，展现出优秀的零样本能力和跨域泛化性能。
...
TL;DR: HighFM：首个针对高时间分辨率多光谱地球观测数据的基础模型，利用2TB+ SEVIRI数据，通过改进的SatMAE掩码自编码框架学习稳健的时空表征，在云掩码和活跃火灾检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 气候相关灾害频发加剧了对实时监测、早期预警和知情决策的需求。现有基础模型多依赖高分辨率但低重访率的卫星影像，难以适应快速演变现象和时间关键型应急响应。需要开发针对高时间分辨率数据的基础模型。

Method: 提出HighFM模型，利用超过2TB的SEVIRI影像数据，基于SatMAE掩码自编码框架进行改进。增强原始架构，加入细粒度时间编码以捕捉短期变化。在云掩码和活跃火灾检测任务上进行微调。

Result: SEVIRI预训练的Vision Transformers在平衡准确率和IoU指标上均优于传统基准模型和近期地理空间基础模型，展示了持续的性能提升。

Conclusion: 高时间分辨率地球静止数据在实时地球观测中具有巨大潜力，为灾害检测和跟踪的基础模型提供了可扩展的路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于背景的静态场景重建方法无法恢复被动态物体遮挡的区域，这限制了虚拟现实和自动驾驶等应用中的场景重建能力。

Method: 1. 使用运动感知模块分割和移除动态区域；2. 利用扩散模型修复被遮挡区域，提供伪地面真值监督；3. 为每个高斯基元引入可学习的真实性标量，在渲染和监督过程中动态调整不透明度；4. 构建Trajectory-Match数据集进行定量评估。

Result: 在DAVIS和自建数据集上的大量实验表明，GA-GS在静态场景重建方面达到了最先进的性能，特别是在具有大规模、持续遮挡的挑战性场景中表现优异。

Conclusion: GA-GS通过生成辅助的高斯溅射方法有效解决了被动态物体遮挡区域的静态场景重建问题，为相关应用提供了更完整的场景重建能力。
...
TL;DR: SW-CLIP通过引入空间自相关改进街景地理定位，将地理距离信息融入CLIP对比学习，用空间加权软标签替代传统负样本处理


<details>
  <summary>Details</summary>
Motivation: 传统CLIP方法将所有非匹配样本视为同等负样本，忽略了地理空间中的距离关系，导致地理定位精度受限，需要将空间自相关原则融入视觉-语言对比学习

Method: 1) 引入位置作为文本表示编码地理坐标；2) 用基于测地距离的空间加权软标签替代one-hot InfoNCE目标；3) 采用邻域一致性正则化保持嵌入空间中的局部空间结构

Result: 在多城市数据集上，SW-CLIP显著提高了地理定位精度，减少了长尾误差，增强了空间一致性，优于标准CLIP方法

Conclusion: 从语义对齐转向地理对齐对鲁棒地理定位至关重要，SW-CLIP为将空间原则融入多模态表示学习提供了通用范式
...
Method: 采用双范式：RLER-Training使用群体相对强化学习和三个任务驱动奖励（帧敏感奖励、思维透明度奖励、抗重复奖励）优化策略；RLER-Inference使用无需训练的编排器生成多样化候选，解析答案和引用帧，基于证据一致性、置信度、透明度和非冗余性评分，进行证据加权选举。

Result: 在8个代表性基准测试中，RLER均达到最先进水平，相比基础模型平均提升6.3%，每个问题平均仅使用3.1个候选，在计算成本和质量之间取得良好平衡。
...
TL;DR: 该论文提出了UENR-600K数据集，这是一个包含60万对1080p帧的大规模、基于物理的夜间视频去雨数据集，使用虚幻引擎模拟3D雨滴粒子，显著提升了模型在真实夜间雨景中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 夜间视频去雨面临独特挑战：雨滴与人造光源相互作用，呈现多种颜色和局部照明效果。现有小规模合成数据集使用2D雨滴叠加，无法捕捉这些物理特性，导致模型在真实夜间雨景中泛化能力差。同时，采集真实配对的夜间视频不现实，因为雨效无法与其他退化（如传感器噪声）分离。

Method: 1. 使用虚幻引擎将雨模拟为虚拟环境中的3D粒子，保证照片真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨帘等正确细节；2. 构建包含60万对1080p帧的UENR-600K数据集；3. 基于Wan 2.2视频生成模型建立新的SOTA基线，将去雨视为视频到视频生成任务，利用强生成先验几乎完全弥合仿真到真实的差距。

Result: 1. 创建了UENR-600K大规模物理基础数据集；2. 建立的基线模型在真实世界视频中表现出显著更好的泛化能力；3. 广泛的基准测试表明，在该数据集上训练的模型对真实世界视频的泛化能力显著提升。

Conclusion: 通过虚幻引擎模拟3D雨滴粒子创建的大规模物理基础数据集UENR-600K，结合基于Wan 2.2的生成模型方法，显著改善了夜间视频去雨任务的性能，几乎完全弥合了仿真到真实的差距，为夜间视频去雨研究提供了高质量数据和新的SOTA基线。
...
TL;DR: 本文提出了BoxComm数据集，这是首个专注于拳击等格斗运动的评论生成基准，包含445场世界拳击锦标赛视频和5.2万条专业评论，并引入了类别条件生成和评论节奏评估两种新评测方法。


<details>
  <summary>Details</summary>
Motivation: 现有体育评论生成基准仅关注足球、篮球等团队运动，完全忽略了格斗运动。格斗运动具有独特挑战：关键动作在毫秒间发生且视觉差异细微但语义决定性，专业评论中战术分析比例远高于团队运动。

Method: 提出了BoxComm大规模数据集，包含445场世界拳击锦标赛视频和超过5.2万条专业评论句子。建立了结构化评论分类法，将评论分为实时播报、战术分析和背景信息三类。提出了两种新颖评测方法：类别条件生成和评论节奏评估。还提出了EIC-Gen改进基线，通过检测击打事件提供结构化动作线索。

Result: 实验表明，当前最先进的多模态大语言模型在两种评测上都表现不佳。提出的EIC-Gen基线通过结合检测到的击打事件提供结构化动作线索，在所有评测中取得了一致性提升，突显了感知短暂细微事件对格斗运动评论生成的重要性。

Conclusion: BoxComm填补了格斗运动评论生成基准的空白，提出的结构化分类法和两种评测方法为体育评论生成研究提供了新方向。结果表明当前模型在格斗运动评论生成方面仍有很大提升空间，特别是对短暂细微事件的感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实需要详细且可定制的3D手部模型，但现有方法昂贵、繁琐且定制性差。零样本文本到3D合成方法在手部生成中存在结构不自然、视角不一致和细节丢失问题。

Method: 1) 使用MANO手部模型初始化提供强结构先验；2) 手部骨架引导的扩散过程确保视角和姿态一致性；3) 新颖的校正手部形状引导损失确保所有视角收敛到一致的分布模式，避免几何变形。

Result: 广泛评估表明该方法在3D手部模型生成方面优于现有最先进方法，为3D手部模型生成开辟了新途径。

Conclusion: HandDreamer通过解决视角不一致问题，实现了高质量、可定制的零样本3D手部模型生成，为虚拟现实交互提供了更好的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法在通用场景表现良好，但在领域迁移时性能严重下降。这源于领域特定任务中类别标签稀缺、语义弱，以及现有模型无法捕捉粗粒度类别标签之外的辅助语义。

Method: 提出HSA-DINO框架：1）多尺度提示库利用图像特征金字塔捕获层次化语义，选择领域特定的局部语义提示，从粗到细粒度逐步丰富文本表示；2）语义感知路由器在推理时动态选择适当的语义增强策略，防止参数更新降低预训练模型的泛化能力。

Result: 在OV-COCO、多个垂直领域数据集和修改的基准设置上评估，HSA-DINO优于先前最先进方法，在领域适应性和开放词汇泛化之间实现了更好的权衡。

Conclusion: HSA-DINO通过参数高效的语义增强框架，有效解决了开放词汇目标检测在领域迁移中的性能下降问题，提升了模型的领域适应性和泛化能力。
...
TL;DR: Chorus是一种利用跨请求相似性的缓存方法，加速视频扩散模型推理，相比现有单请求内缓存方法，在4步蒸馏模型上实现高达45%的加速。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器（DiT）模型虽然能生成高质量视频，但存在推理成本高的问题，因为需要迭代去噪。现有缓存方法主要利用单个请求内扩散过程的相似性来跳过冗余去噪步骤，但在工业4步蒸馏模型上效果有限。

Method: Chorus采用三阶段缓存策略：1）对相似请求的潜在特征进行完全重用；2）在中间去噪步骤中对特定潜在区域进行跨请求缓存，结合Token-Guided Attention Amplification提升生成视频与条件提示的语义对齐；3）将完全重用的适用性扩展到后期去噪步骤。

Result: Chorus在工业4步蒸馏模型上实现了高达45%的加速，而先前的单请求内缓存方法在这些模型上效果不佳。

Conclusion: Chorus通过利用跨请求相似性的缓存方法，有效加速了视频扩散模型的推理服务，特别是在现有方法无效的蒸馏模型上表现出显著性能提升。
...
Method: 构建系统评估框架，在自然对抗场景下评估多种VLMs（CLIP、鲁棒CLIP、BLIP2、SigLIP2）。使用精心策划的对抗数据集（排版攻击、ImageNet-A、自然语言诱导对抗样本），测量模型在零样本图像分类、语义分割和视觉问答任务上的自然对抗性能。

Result: 研究发现：1）鲁棒CLIP模型可能放大自然对抗漏洞；2）CLIP模型在自然语言诱导对抗样本上性能显著下降；3）提供了可解释性分析以识别失败模式。

Conclusion: 该研究揭示了VLMs在自然对抗场景下的局限性，强调了超越标准基准进行系统评估的重要性。研究结果有望启发未来在鲁棒和公平多模态模式识别方面的研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文感知图像编辑方法存在数据收集和训练成本高（训练方法）或一致性和灵活性不足（基于扩散反演的无训练方法）的问题。

Method: 提出VicoEdit方法：1）无需训练和反演，直接基于视觉上下文将源图像转换为目标图像；2）设计基于概念对齐的后验采样方法增强编辑一致性。

Result: 实证结果表明，这种无训练方法在编辑性能上甚至优于最先进的基于训练的模型。
...
TL;DR: 提出基于补丁增强和跨视图规律性的统一防御框架，通过特征表示和输出分布两个层面约束模型对触发模式的异常行为，在低中毒率下有效抑制后门攻击成功率，同时保持正常生成能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在监督微调过程中容易植入后门，一旦特定触发模式激活就会稳定输出攻击者预定义的有害响应。后门防御的核心挑战在于低中毒率下抑制攻击成功率的同时保持模型正常生成能力，这两个目标存在内在冲突。

Method: 提出基于补丁增强和跨视图规律性的统一防御框架：1) 补丁级数据增强与跨视图输出差异正则化结合，利用后门响应对非语义扰动异常不变性，主动拉开原始视图和扰动视图的输出分布；2) 通过施加输出熵约束避免防御过程中的过抑制，确保正常指令生成质量。

Result: 在三个模型、两个任务和六种攻击的实验结果表明，该方法能有效降低攻击成功率，同时保持高水平的正常文本生成能力。
...
TL;DR: 论文提出Indra表示假设，认为单模态基础模型的表示正在收敛到反映现实共享关系结构，类似于因陀罗网的哲学隐喻。使用范畴论中的V-富集Yoneda嵌入形式化该假设，并通过角距离实例化，在跨模型和跨模态场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现单模态基础模型倾向于学习收敛表示，但这些表示本质上是样本的独立内部抽象，表达能力有限。受因陀罗网哲学隐喻启发，作者认为这些表示正在收敛到反映现实共享关系结构，需要理论框架来形式化这一现象。

Method: 提出Indra表示假设，使用范畴论中的V-富集Yoneda嵌入形式化表示，将每个样本的Indra表示定义为相对于其他样本的关系配置文件。使用角距离实例化该表示，并在视觉、语言和音频的跨模型和跨模态场景中进行评估。

Result: 大量实验表明，Indra表示能持续增强跨架构和跨模态的鲁棒性和对齐性，为单模态基础模型的无训练对齐提供了理论基础和实用框架。

Conclusion: Indra表示假设为理解单模态基础模型表示的收敛现象提供了理论解释，其形式化框架能有效提升模型间的对齐能力，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型虽然取得了显著成功，但其可信度仍存在问题：模型倾向于依赖文本线索而非视觉证据，存在产生无根据或虚构回答的风险

Method: 提出Saliency-R1框架，包含：1）新颖的显著性映射技术，高效突出对生成token关键贡献的图像区域；2）扩展该技术以追踪视觉信息在推理过程中的流向；3）使用显著性图与人工标注边界框的重叠作为奖励函数；4）应用Group Relative Policy Optimization（GRPO）对齐显著部分和关键区域

Result: 实验表明Saliency-R1能够改善推理的忠实性、解释性，并提升整体任务性能

Conclusion: Saliency-R1框架有效解决了视觉语言模型的信任问题，通过增强视觉证据的依赖和减少无根据回答，提升了模型的解释性和推理忠实性
...
TL;DR: MedROI是一个与编解码器无关的ROI中心压缩框架，通过去除医学影像中的非诊断背景区域来提升压缩效率，无需修改现有编解码器架构。


<details>
  <summary>Details</summary>
Motivation: 医学影像档案在尺寸和分辨率上快速增长，现有压缩方法要么压缩整个图像（包括无诊断价值的背景），要么采用保留背景位的差分ROI编码，导致存储和传输效率低下。

Method: MedROI采用轻量级基于强度的阈值分割提取紧密的组织边界框，丢弃背景体素，仅存储54字节的元数据用于空间恢复。裁剪后的ROI可使用任何现有2D或3D编解码器进行压缩，无需架构修改或重新训练。

Result: 在200个ADNI T1加权脑MRI体积上测试6种编解码器配置，MedROI在大多数配置中显著提升了压缩比和编码/解码时间，同时保持ROI内可比较的重建质量。例如，JPEG2000 2D (lv3)的压缩比从20.35提升到27.37，平均压缩时间从1.701秒减少到1.380秒。

Conclusion: MedROI是一个有效的即插即用ROI中心压缩框架，能够显著提升医学影像压缩效率，同时保持诊断质量，适用于各种传统和神经压缩方法。
...
TL;DR: 提出StableTTA方法解决集成方法中聚合策略冲突问题，在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，轻量模型超越ViT性能的同时大幅减少参数和计算成本。
...
Result: 在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，33个模型超过95%准确率，多个超过96%。轻量架构超越ViT 11.75%准确率，仅用不到5%参数，减少约89.1%计算成本（GFLOPs）。

Conclusion: StableTTA能有效解决集成方法中的聚合冲突问题，在保持高性能的同时显著降低资源需求，使高精度推理在资源受限设备上成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉SLAM中的相对相机位姿估计面临噪声对应关系的挑战。传统方法依赖随机假设采样和迭代估计，而学习方法通常缺乏显式几何结构。需要一种结合几何结构和学习能力的方法来提高估计的鲁棒性。

Method: 将相对位姿估计重新定义为极线对应图上的关系推理问题，其中匹配的关键点作为节点，附近的关键点通过边连接。使用图操作（剪枝、消息传递、池化）来估计四元数旋转、平移向量和本质矩阵。采用包含五个部分的损失函数进行优化：与真实值的L2差异、本质矩阵的Frobenius范数差异、奇异值差异、航向角差异和尺度差异。使用无检测器的密集匹配方法LoFTR进行特征匹配。

Result: 在室内和室外基准测试上的实验表明，相比传统方法和学习引导方法，该方法对密集噪声和大基线变化具有更好的鲁棒性。这突显了全局关系共识的有效性。

Conclusion: 通过将相对位姿估计重新定义为图上的关系推理问题，该方法结合了几何结构和学习能力，在噪声和大基线变化下表现出更好的鲁棒性，为视觉SLAM中的位姿估计提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型大多孤立分析放射影像，忽视了临床实践中比较前后影像评估间隔变化的关键任务。对于胸部X光片，捕捉时序变化至关重要，因为放射科医生不仅需要评估发现的静态表现，还需要评估其随时间演变情况。

Method: TILA框架使用时间反转（反转图像对顺序）作为监督信号，在预训练、微调和推理阶段集成反转感知目标，将显式时序顺序学习与常规外观建模相结合。还提出了统一的评估协议来评估时间反转下的顺序敏感性和一致性，并构建了MS-CXR-Tretrieval检索评估集。

Result: 在公共数据集和真实医院队列上的实验表明，TILA在应用于多种现有架构时，能持续改进进展分类和时间嵌入对齐性能。

Conclusion: TILA是一个简单而有效的框架，通过时间反转监督信号增强现有时序视觉语言模型对方向性变化的敏感性，为医学影像分析提供了更好的时序变化建模能力。
...
TL;DR: TAPE是一个两阶段自适应框架，通过参数高效微调解决OCT/OCTA图像分析中的领域偏移和任务不对齐问题，在视网膜层分割任务上实现了优异的参数效率和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有OCT/OCTA图像分析方法严重依赖大量数据和模型规模，难以在资源受限的临床环境中部署。基于基础模型的迁移学习虽然前景广阔，但仍面临领域偏移和任务不对齐的挑战。

Method: 提出TAPE两阶段自适应框架：1）领域适应阶段：在掩码图像建模背景下应用参数高效微调进行医学图像领域适应；2）任务拟合阶段：为下游分割任务进行参数高效微调。框架将通用基础模型（如MAE）和专用基础模型（如RETFound）适应到视网膜层分割任务。

Result: TAPE在视网膜层分割任务上表现出卓越的参数效率，并在多种病理条件下实现了最先进的泛化性能，超越了现有方法。
...
TL;DR: 对文本到图像扩散模型进行后验遗忘的研究发现，在概念遗忘效果与组合生成能力之间存在权衡：强遗忘方法会损害模型的组合完整性，而保持组合结构的方法则遗忘效果不足。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注概念遗忘的成功率，但对遗忘如何影响模型的整体生成能力缺乏系统理解。本文旨在通过组合文本到图像生成的视角，系统评估概念遗忘方法的实际效果。

Method: 聚焦于Stable Diffusion 1.4中的裸露内容移除，使用T2I-CompBench++和GenEval等评估工具，对多种最先进的遗忘方法进行系统实证研究，同时结合现有的遗忘基准测试。

Result: 研究揭示了遗忘效果与组合完整性之间的一致权衡：实现强遗忘的方法通常在属性绑定、空间推理和计数能力方面造成显著退化；而保持组合结构的方法往往无法提供稳健的遗忘效果。

Conclusion: 当前评估实践存在局限性，需要开发能够明确考虑语义保留（超越目标抑制）的遗忘目标，以平衡遗忘效果与模型生成能力的保持。
...
TL;DR: 提出PR-IQA框架，通过参考不同视角的图像评估扩散生成视图的质量，无需真实标注，用于提升3D高斯溅射的稀疏视角新视图合成效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视图合成，但生成的伪真实视图存在光度学和几何不一致性，直接用于监督会损害3D重建质量。

Method: 提出部分参考图像质量评估框架：1) 在重叠区域计算几何一致的部分质量图；2) 通过跨注意力机制结合参考视图上下文，将部分质量图补全为密集全图像质量图。

Result: PR-IQA在无需真实标注的情况下达到全参考级别的准确性，优于现有IQA方法。集成到3D高斯溅射流程后，能有效过滤不一致性，产生更优的3D重建和新视图合成结果。

Conclusion: PR-IQA框架通过质量感知的监督机制，显著提升了扩散增强3D高斯溅射管道的重建质量，为稀疏视角新视图合成提供了有效的质量评估解决方案。
...
TL;DR: Firebolt-VL是一种高效视觉语言模型，用液态基础模型解码器替代Transformer解码器，通过Token-Grid相关模块增强视觉定位，实现线性时间推理和细粒度理解。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，难以在资源受限场景部署；现有方法依赖Transformer交叉注意力，其二次复杂度影响效率；小型视觉语言模型难以精确捕捉细粒度视觉区域，导致细粒度推理任务性能下降。

Method: 1. 用液态基础模型解码器替代Transformer解码器；2. 提出Token-Grid相关模块，计算文本token和图像patch之间的轻量级相关性，通过状态空间模型和FiLM条件进行调制，使模型能选择性强调与文本提示相关的视觉区域。

Result: 在多个基准测试中，Firebolt-VL实现了准确、细粒度的理解，同时显著提高了效率，保持了线性时间推理。

Conclusion: Firebolt-VL通过创新的架构设计解决了现有视觉语言模型的计算效率和细粒度理解问题，为资源受限场景提供了有效的解决方案。
...
TL;DR: 该论文提出了一种基于物理特征的AI生成图像检测方法，通过分析15种物理特征，筛选出5种核心特征（拉普拉斯方差、Sobel统计、残差噪声方差等），并将其与CLIP多模态模型结合，在多个数据集上达到接近完美的检测准确率。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC技术的快速发展，真实图像与AI生成图像的界限变得模糊，现有的深度伪造检测器往往过度拟合特定生成模型，存在适应性危机。需要重新审视区分自然图像与AI生成图像的内在物理特征。

Method: 1. 全面探索15种物理特征在20多个由GAN和扩散模型生成的数据集上的表现；2. 提出新颖的特征选择算法，识别出5种具有一致判别能力的核心物理特征；3. 将这些物理特征转换为文本编码值，与语义描述结合，指导CLIP中的图像-文本表示学习。

Result: 在多个Genimage基准测试中达到最先进性能，在Wukong和SDv1.4等数据集上获得接近完美的准确率（99.8%）。通过将像素级真实性特征与语义理解相结合，显著提升了检测性能。

Conclusion: 该工作开创了基于物理基础特征的可信视觉语言建模新方向，为缓解大型多模态模型中的幻觉和文本不准确问题开辟了新途径，通过桥接像素级真实性与语义理解，实现了更可靠的AI生成内容检测。
...
TL;DR: GLA提出了一种针对自动驾驶视觉语言模型的隐蔽后门攻击方法，使用涂鸦式视觉触发器和跨语言文本触发器，在仅需10%投毒率的情况下达到90%攻击成功率，且不影响正常任务性能。
...
Result: 在DriveVLM上的实验显示，GLA仅需10%的投毒率即可达到90%的攻击成功率（ASR）和0%的误报率（FPR）。更隐蔽的是，后门不会削弱模型在干净任务上的性能，反而提高了BLEU-1等指标，使传统基于性能下降的检测方法难以识别攻击。
...
TL;DR: InCTRLv2是一个新颖的少样本通用异常检测与分割框架，通过双分支架构从异常-正常判别和正常偏离语义两个互补视角检测异常，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法大多是针对特定领域的专家模型，需要大量训练数据且难以泛化到未见数据集。通用异常检测范式旨在学习单一通用模型，无需重新训练即可跨领域检测异常。

Method: 基于InCTRL的上下文残差学习思想，InCTRLv2引入双分支框架：1) 主分支使用判别式异常分数学习，利用正常和异常数据学习语义引导的异常-正常空间；2) 辅助分支使用单类异常分数学习，仅使用正常数据学习广义正常模式。两个分支都由大规模视觉-语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下，在异常检测和分割任务中都达到了最先进的性能。

Conclusion: InCTRLv2通过双语义视角（异常-正常判别和正常偏离语义）为通用异常检测提供了互补的异常感知能力，显著扩展了先前InCTRL模型的能力，实现了跨领域的强大泛化性能。
...
TL;DR: 提出新的AI生成视频检测框架，通过原生尺度处理保留高频伪造痕迹，并在大规模数据集上验证了优越性能
...
Method: 构建包含15个最新开源和商业生成器的14万+视频数据集，提出基于Qwen2.5-VL Vision Transformer的检测框架，支持原生可变空间分辨率和时间长度处理

Result: 在多个基准测试中取得优越性能，验证了原生尺度处理对保留高频伪影和时空不一致性的重要性

Conclusion: 原生尺度处理是AI生成视频检测的关键，提出的框架为领域建立了新的鲁棒基准，数据集和Magic Videos基准专门用于评估超逼真合成内容
...
TL;DR: 提出Flow Divergence Sampler (FDS)框架，通过计算边际速度场的散度来量化样本间速度冲突，并在求解步骤前优化中间状态，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 基于流的模型通过建模边际速度场来学习目标分布，但当样本间速度在相同中间状态发生冲突时，平均速度会将样本误导到低密度区域，从而降低生成质量。

Method: 提出Flow Divergence Sampler (FDS)，这是一个无需训练的框架，在每次求解步骤前优化中间状态。核心发现是这种误导的严重程度可以通过边际速度场的散度来量化，该散度在推理时可直接计算。

Result: FDS作为即插即用框架，与标准求解器和现成的流模型兼容，在各种生成任务（包括文本到图像合成和逆问题）中持续提升生成保真度。

Conclusion: 通过利用边际速度场的散度信号来引导状态到较少模糊区域，FDS有效解决了样本间速度冲突问题，显著提升了基于流模型的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测性能受到异常样本稀缺和长尾分布的根本限制，这阻碍了检测模型的训练和性能提升。

Method: 1. 开发3D-DefectStudio软件平台，基于可控合成引擎MPAS注入几何逼真的缺陷；2. 使用多模态大语言模型解释产品设计信息并自动生成异常合成指令；3. 引入基于空间分布归一化和几何忠实数据增强的训练流程，提高点云Transformer架构的鲁棒性。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上的广泛实验表明，该方法实现了最先进的性能。

Conclusion: Synthesis4AD通过合成异常数据生成和鲁棒训练流程，有效解决了工业3D异常检测的数据稀缺问题，为实际工业应用提供了可行的解决方案。
...
TL;DR: ZeD-MAP：将零样本扩散深度模型与增量式束调整结合，实现无人机图像实时三维重建，在保持亚米级精度的同时显著加速处理速度。


<details>
  <summary>Details</summary>
Motivation: 无人机高分辨率图像实时深度重建对灾害响应等时间敏感任务至关重要，但面临宽基线视差、大图像尺寸、低纹理表面、遮挡和计算限制等挑战。现有扩散模型缺乏度量精度和时间一致性。

Method: 提出ZeD-MAP框架：将测试时扩散深度模型转换为度量一致的SLAM式映射流程。通过增量式基于簇的束调整（BA）产生度量一致的位姿和稀疏三维连接点，再将其重投影到选定帧中作为扩散深度估计的度量指导。

Result: 在50米高度采集的无人机图像上验证，达到亚米级精度：水平面误差约0.87米，垂直方向误差约0.12米，每帧处理时间1.47-4.91秒，显著快于传统摄影测量方法。

Conclusion: BA为基础的度量指导在保持与经典摄影测量方法相当一致性的同时，显著加速处理速度，实现了实时三维地图生成，为时间敏感的地理空间任务提供了可行解决方案。
...
TL;DR: 该论文提出DenZa-Gaussian方法，通过3D高斯溅射技术解决ADF-STEM稀疏视角下的3D重建问题，使用可学习标量场denza建模局部散射强度，引入散射视角归一化和傅里叶振幅损失函数，在45视角和15视角实验中实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会延长电子束暴露时间，损害剂量敏感材料并引入漂移和错位问题。实际应用中常需稀疏视角采集，但传统方法在有限视角下会产生伪影和结构保真度降低的问题。

Method: 1) 使用可学习标量场denza建模局部散射强度，解决3DGS与ADF-STEM成像物理不匹配问题；2) 引入系数γ通过散射视角归一化稳定不同倾斜角度的散射；3) 包含2D傅里叶振幅项的损失函数，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列实验中，DenZa-Gaussian方法产生了高保真度的3D重建结果和2D投影，与原始倾斜图像更一致，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法通过适应3D高斯溅射技术到ADF-STEM领域，有效解决了稀疏视角下的3D重建问题，在保持样本完整性的同时提高了重建质量，为纳米尺度材料分析提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义，阻碍了该领域的研究进展和应用发展。

Method: 基于世界模型的演进，提出了明确定义：以感知为中心、具备交互和长期记忆能力的模型或框架；系统分类世界模型的核心能力；开发OpenWorldLib框架集成不同任务的模型，实现高效复用和协作推理。

Result: 建立了世界模型的标准化定义和分类体系，开发了OpenWorldLib开源框架，为世界模型研究提供了统一的评估和推理平台。

Conclusion: OpenWorldLib为世界模型研究提供了重要的标准化框架，有助于推动该领域的系统化发展，并提出了未来研究方向的分析。
...
TL;DR: 提出自适应KV缓存量化方法，通过数据驱动控制器动态分配KV缓存精度，在移动设备上优化LLM推理的内存和延迟，同时保持接近FP16的精度。


<details>
  <summary>Details</summary>
Motivation: LLM在移动和边缘设备上部署面临KV缓存内存和带宽开销大的挑战，现有固定精度量化方案浪费比特在低重要性token上，同时过度压缩高重要性token，导致精度下降。

Method: 提出自适应KV缓存量化框架，提取token级特征（频率、质量分数、注意力方差、熵不确定性），通过紧凑的数据驱动控制器动态选择KV精度（2-bit, 4-bit, 8-bit, FP16）。

Result: 在多个常识推理基准测试中，使用SmolLM模型（135M-1.7B）验证，相比静态KV量化减少解码延迟17.75%，精度提升7.60分，同时保持与FP16推理仅差0.30分的精度。

Conclusion: 自适应KV缓存量化方法能有效优化LLM在资源受限设备上的推理效率，在减少内存占用和延迟的同时保持高精度，为边缘设备部署LLM提供实用解决方案。
...
TL;DR: 本文提出了一种过程驱动的图像生成方法，通过多步骤的文本规划、视觉草图、文本反思和视觉细化的迭代过程，使图像生成过程更加明确、可解释和可监督。


<details>
  <summary>Details</summary>
Motivation: 人类绘画是增量式的过程：先规划全局布局，绘制粗略草图，检查并细化细节，每个步骤都基于不断演变的视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型能否想象中间状态的链条？本文旨在探索这一过程驱动的图像生成范式。

Method: 提出过程驱动的图像生成方法，将合成分解为思想和动作的交错推理轨迹。每个迭代包含4个阶段：文本规划、视觉草图、文本反思和视觉细化。通过密集的逐步监督，对视觉中间状态实施空间和语义一致性约束，对文本中间状态保持先验视觉知识同时识别和纠正违反提示的元素。

Result: 在多个文本到图像生成基准测试中进行了实验验证，证明了该方法能够生成明确、可解释和可监督的图像生成过程。

Conclusion: 过程驱动的图像生成方法使生成过程变得明确、可解释和可直接监督，通过多步骤的文本-视觉交互迭代，实现了更可控和透明的图像合成。
...
TL;DR: CLEAR框架通过连接多模态模型的生成与推理能力，提升对退化图像的理解鲁棒性，同时保持对干净图像的原有性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像退化（模糊、噪声、压缩、光照不佳）严重影响了多模态理解。现有的统一多模态模型虽然结合了理解和生成能力，但未能有效利用自身的生成能力来处理退化输入，主要原因是训练机制从未要求模型在推理过程中调用生成能力，且标准的解码-重新编码路径不支持有效的联合优化。
...
Result: 在MMD-Bench（涵盖三个退化严重程度和六个标准多模态基准）上的实验表明，CLEAR显著提高了对退化输入的鲁棒性，同时保持了干净图像的性能。分析还发现，移除像素级重建监督会导致中间视觉状态具有更高的感知质量，表明任务驱动的优化与视觉质量自然对齐。

Conclusion: CLEAR框架成功地将多模态模型的生成能力与推理能力连接起来，通过任务驱动的优化方法有效提升了模型对退化图像的鲁棒性，同时揭示了任务优化与视觉质量之间的自然一致性。
...
TL;DR: AvatarPointillist：从单张肖像图像生成动态4D高斯化身的创新框架，使用自回归Transformer生成点云，实现高质量、可控的虚拟化身生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单张肖像图像生成动态、高质量、可控的4D高斯化身方面存在局限性，需要新的范式来实现更精确、自适应的构建和逼真的动画效果。

Method: 使用仅解码器的Transformer自回归生成3D高斯溅射的点云，动态调整点密度和总点数以适应主体复杂度；同时预测每点的绑定信息以实现动画；通过专门的高斯解码器将点转换为完整的可渲染高斯属性。

Result: 实验验证AvatarPointillist能够生成高质量、逼真且可控的化身，自回归生成器与解码器之间的条件交互显著提高了保真度。

Conclusion: 该自回归公式代表了化身生成的新范式，能够从单张肖像图像生成动态4D高斯化身，为未来研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要互补的传感器：相机提供密集语义但深度不可靠，毫米波雷达提供精确的距离和速度测量但几何稀疏。需要有效融合两种模态以提升3D目标检测性能。

Method: 构建BEVDepth相机分支和RadarBEVNet雷达分支，每个分支都增强可变形自注意力，通过可变形交叉注意力模块进行融合。采用两阶段训练策略：先用深度监督预训练相机分支，然后联合训练雷达和融合模块。

Result: 在VoD数据集上，MMF-BEV始终优于单模态基线方法，并在所有物体类别上（包括完整标注区域和近距感兴趣区域）与现有融合方法相比取得竞争性结果。传感器贡献分析提供了模态互补性的可解释证据。

Conclusion: 提出的雷达-相机BEV融合框架通过可变形注意力机制有效利用了传感器的互补性，在3D目标检测任务中表现出色，为自动驾驶感知系统提供了有效的多模态融合方案。
...
TL;DR: E-VLA框架通过事件相机增强视觉-语言-动作模型，在低光、运动模糊等恶劣视觉条件下显著提升机器人操作的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉在极端低光、运动模糊和黑裁剪等感知退化条件下表现脆弱，需要增强VLA模型在恶劣视觉环境下的鲁棒性

Method: 提出事件增强的VLA框架，直接利用事件流中的运动和结构线索，而非重建图像；构建开源遥操作平台和RGB-事件-动作数据集；设计轻量级事件集成策略，包括事件窗口化和融合方法

Result: 在20lux低光条件下，Pick-Place任务成功率从0%提升到60%（简单叠加融合）和90%（事件适配器）；在严重运动模糊下，Pick-Place从0%提升到20-25%，Sorting从5%提升到32.5%

Conclusion: 事件驱动感知能有效集成到VLA模型中，为实现超越传统帧基成像的鲁棒具身智能提供了系统证据
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在视觉问答中，高分辨率细节有时会成为噪声，导致幻觉或推理错误。需要一种方法让模型专注于本质的结构信息而非干扰性纹理。

Method: 提出DDP框架，包含两个任务：1) 物理属性任务：使用80%下采样、结构视觉辅助（白色背景遮罩和正交线）和上下文学习；2) 感知现象任务：集成任务分类阶段，结合模糊遮罩、对比度增强和下采样等专门工具。

Result: 实验结果表明"少即是多"：通过有意降低视觉输入质量并提供针对性结构提示，DDP使视觉语言模型能够绕过干扰性纹理，在具有挑战性的视觉基准测试中获得更优的推理准确性。

Conclusion: DDP框架通过策略性降质和结构提示，有效提升视觉语言模型在复杂视觉任务中的推理能力，证明了在特定情况下降低输入保真度反而能改善模型性能。
...
Method: 提出粗到细指令条件交互生成框架，与一致性模型的迭代去噪过程对齐；采用动态感知策略利用前一步细化轨迹更新场景上下文；引入碰撞感知引导减少物理伪影；设计混合训练策略，通过将体素化场景占用注入HOI数据集合成伪HOSI样本，并与高保真HSI数据联合训练。
...
TL;DR: 本文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了首个专门量化该问题的大规模数据集和基准测试，并开发了Drive Expert Adapter框架来避免知识退化。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型集成到自动驾驶中虽然能解决长尾场景问题，但微调过程会侵蚀模型预训练的世界知识，造成灾难性遗忘，这与使用VLMs的核心目的相悖。目前这一问题尚未得到系统研究。

Method: 提出了Drive Expert Adapter (DEA)框架，将模型适应从权重空间转移到提示空间。DEA根据场景特定线索动态路由推理到不同的知识专家，在不破坏基础模型参数的情况下提升驾驶任务性能。

Result: 实验表明，DEA方法不仅在驾驶任务上达到最先进性能，而且有效缓解了灾难性遗忘，保持了VLMs作为自动驾驶变革力量所必需的泛化能力。

Conclusion: 本文首次系统研究了自动驾驶中VLMs的灾难性遗忘问题，提出了首个基准测试和DEA解决方案，为在保持预训练知识的同时提升驾驶性能提供了有效途径。
...
TL;DR: 该论文提出了Floorplan Markup Language (FML)，一种通用的结构化语法表示方法，将整个平面图生成问题转化为下一个标记预测任务，并基于此开发了FMLM模型，在多种条件下生成高质量平面图。


<details>
  <summary>Details</summary>
Motivation: 自动住宅平面图生成是建筑和计算机图形学交叉领域的核心挑战。早期基于约束满足或组合优化的方法缺乏多样性和灵活性，而最近的生成模型由于表示方法不理想，难以在异构条件任务（如从场地边界、房间邻接图或部分布局生成）中实现良好泛化。

Method: 提出了Floorplan Markup Language (FML)，一种将平面图信息编码在单个结构化语法中的通用表示方法。基于FML开发了FMLM，这是一个基于Transformer的生成模型，能够将整个平面图生成问题转化为下一个标记预测任务，从而在多样化条件下生成平面图。

Result: 在RPLAN数据集上的综合实验表明，FMLM作为一个单一模型，超越了先前任务特定的最先进方法，能够生成高保真度和功能性的平面图。

Conclusion: FML提供了一种通用表示方法，解决了现有方法在异构条件任务中泛化能力不足的问题。FMLM模型展示了在多样化条件下生成高质量平面图的能力，为自动平面图生成提供了更灵活和强大的解决方案。
...
TL;DR: 本文提出了一种基于图像块级别的幻觉检测框架，通过分析细粒度的token级交互来检测大型视觉语言模型中的幻觉问题，相比现有全局检测方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉推理任务中表现良好，但对幻觉问题高度敏感。现有检测方法主要依赖粗粒度的全图像度量，这种全局策略存在局限：幻觉token可能在许多局部区域表现出微弱但广泛分散的相关性，这些相关性聚合后形成欺骗性的高整体相关性，从而逃避当前全局幻觉检测器。

Method: 基于"忠实对象token必须牢固地基于特定图像区域"的观察，引入图像块级别的幻觉检测框架，分析模型各层的细粒度token级交互。发现幻觉token的两个特征：(1)产生扩散、非局部化的注意力模式；(2)与任何视觉区域缺乏有意义的语义对齐。基于这些发现，开发了轻量级、可解释的检测方法，利用图像块级统计特征结合隐藏层表示。

Result: 该方法在token级幻觉检测中达到高达90%的准确率，证明了细粒度结构分析在检测幻觉方面的优越性。

Conclusion: 通过分析细粒度的图像块级交互特征，可以有效检测大型视觉语言模型中的幻觉问题，相比全局检测方法有显著优势，为幻觉检测提供了新的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法产生高度冗余的网格对齐高斯分布，在未观测区域存在空洞或模糊问题。需要一种能够从少量图像生成高质量3D重建，并在未观测区域合成合理内容的方法。

Method: 1. 通过高斯参数流匹配实现生成式重建；2. 引入分层分块方案将空间相关高斯分组为联合transformer token，减少序列长度；3. 训练时使用时间步加权渲染损失；4. 推理时采用光度梯度引导和分类器自由引导提高保真度。

Result: 在Objaverse和Google Scanned Objects数据集上，相比像素和体素对齐方法，使用更少的高斯分布获得一致改进，在输入视图未覆盖物体部分时优势更明显。

Conclusion: Free-Range Gaussians通过生成式重建框架和分层分块策略，实现了从少量图像的高质量3D重建，解决了未观测区域的合成问题，显著优于现有方法。
...
Method: 将视频混剪建模为多模态一致性满足问题，提出分层多智能体框架DIRECT，包含编剧（全局结构锚定）、导演（自适应编辑意图）和编辑（意图引导的镜头序列编辑）三个级联层级

Result: DIRECT在客观指标和人类主观评价中均显著优于现有最先进基线方法

Conclusion: DIRECT框架通过模拟专业制作流程，有效解决了视频混剪中的跨层级多模态协调问题，实现了专业级的流畅编辑效果
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练方法，解决了密集驾驶场景中多粒度编辑、语义保持和领域迁移的挑战。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全需要可扩展的真实可控驾驶场景生成，但现有图像编辑方法在密集、安全关键的驾驶布局上表现不佳，需要处理多粒度编辑、丰富语义保持和领域迁移等挑战。

Method: 1) 数据：从Boreas、nuScenes和Argoverse2构建大规模配对真实/合成数据集；2) 模型：语言引导掩码实现细粒度编辑，使用语义增强掩码和提示；3) 训练：内容保持和指令对齐的联合损失函数。
...
TL;DR: ClickAIXR是一个在设备端运行的XR框架，通过控制器点击选择真实世界物体，然后使用本地视觉语言模型处理物体图像并回答自然语言问题，实现隐私保护和低延迟的交互。


<details>
  <summary>Details</summary>
Motivation: 现有XR系统依赖云端AI（如ChatGPT）或基于注视的选择（如GazePointAR），存在隐私、延迟和交互模糊性问题。ClickAIXR旨在通过设备端处理和点击式物体选择来解决这些问题，提高透明度和用户体验。

Method: 集成设备端视觉语言模型与控制器基础的物体选择范式。用户通过控制器精确点击XR中的真实世界物体，选中物体的图像由本地VLM处理，以文本和语音形式回答自然语言问题。使用Magic Leap SDK（C API）实现，基于ONNX进行本地VLM推理。

Result: 与Gemini 2.5 Flash和ChatGPT 5的用户研究比较显示，ClickAIXR具有中等延迟和可接受的用户体验。研究评估了可用性、信任度和用户满意度，证明了点击式物体选择与设备端AI结合在推进可信、隐私保护的XR交互方面的潜力。

Conclusion: ClickAIXR展示了点击式物体选择与设备端AI结合在XR交互中的潜力，能够解决隐私、延迟和交互模糊性问题，为可信、隐私保护的XR交互提供了新方向。
...
TL;DR: 本文提出了一个用于图像空间编辑的完整评估框架，包括基准测试集、合成数据集和基线模型，旨在解决现有模型在细粒度空间操作方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前图像空间编辑模型在细粒度空间操作方面能力不足，缺乏专门的评估工具，这限制了精确控制物体布局和相机视角的能力。

Method: 提出了三个主要贡献：1) SpatialEdit-Bench基准测试集，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) SpatialEdit-500k合成数据集，使用可控的Blender流水线生成，包含多样化背景和系统相机轨迹；3) SpatialEdit-16B基线模型，基于该数据集开发用于细粒度空间编辑。

Result: 该方法在通用编辑任务上具有竞争力，同时在空间操作任务上显著优于现有方法，为图像空间编辑提供了有效的评估和训练框架。

Conclusion: 该研究为图像空间编辑领域提供了全面的评估工具、大规模训练数据和有效的基线模型，将促进细粒度空间操作技术的发展。
...
TL;DR: DeltaWorld：一种基于VFM特征差异的生成式世界模型，通过DeltaTok将连续帧间特征差异编码为单个"delta"token，大幅降低计算成本并实现多样化未来预测


<details>
  <summary>Details</summary>
Motivation: 视频世界建模中预测多样化未来状态是一个核心挑战。判别式世界模型产生确定性预测（隐式平均可能未来），而现有生成式世界模型计算成本高。虽然已有工作在VFM特征空间预测可减少参数，但大多仍是判别式的

Method: 提出DeltaTok将连续帧间VFM特征差异编码为单个连续"delta"token；DeltaWorld在这些token上操作的生成式世界模型。Delta token将视频从三维时空表示简化为一维时间序列，实现紧凑表示，支持可处理的多假设训练

Result: 在密集预测任务上，DeltaWorld预测的未来更接近真实世界结果，同时比现有生成式世界模型参数减少35倍以上，FLOPs减少2000倍。紧凑表示支持并行生成多个未来并仅监督最佳预测

Conclusion: DeltaWorld通过Delta token的紧凑表示实现了高效、多样化的未来预测，显著降低了生成式世界模型的计算成本，在保持预测质量的同时大幅提升效率
...
TL;DR: 本文提出了一种通过直接学习文本编码器输出的提示嵌入来解锁预训练扩散模型固有修复能力的方法，无需微调或控制模块。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的修复方法主要依赖微调或Control-Net风格模块来利用预训练模型的先验知识，但这些方法未能充分利用模型固有的修复能力，且存在训练与推理动态不匹配的问题。

Method: 通过扩散桥公式在预训练扩散模型中学习提示嵌入，对齐训练和推理动态，确保从噪声退化状态到干净图像的一致去噪路径。在WAN视频模型和FLUX图像模型上实现轻量级学习提示。

Result: 该方法在多种退化类型上实现了竞争性性能和泛化能力，避免了模型微调和修复专用控制模块的需求。

Conclusion: 预训练扩散模型本身具备修复能力，可以通过学习提示嵌入来解锁，扩散桥公式解决了训练与推理动态不匹配的问题，为高效利用预训练模型进行修复任务提供了新思路。
...
TL;DR: SimpleProc：基于NURBS和基础位移纹理的完全程序化MVS训练数据生成器，仅需8000张图像即可超越同等规模人工数据，352000张图像时性能媲美甚至超越692000张人工图像


<details>
  <summary>Details</summary>
Motivation: 探索多视角立体视觉（MVS）的程序化规则设计空间，解决人工收集训练数据成本高、规模受限的问题，寻求更高效的数据生成方法

Method: 开发SimpleProc程序化生成器，使用少量NURBS规则、基础位移和纹理模式生成MVS训练数据，实现完全程序化的数据生成

Result: 8000张程序化图像优于同等规模人工数据；352000张程序化图像性能媲美甚至超越692000张人工图像；代码和数据已开源

Conclusion: 程序化数据生成是MVS训练的有效替代方案，能以更小规模达到甚至超越大规模人工数据的性能，为计算机视觉研究提供高效数据生成工具
...
TL;DR: 大型视觉语言模型（VLM）的端到端延迟受输出token数量限制，研究发现大型模型用较少token能达到小型模型长序列的性能，提出多智能体推理框架重用小型模型的关键推理token来提升效率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型大多使用大型语言模型作为解码器，采用自回归方式生成响应token，输出token数量成为端到端延迟的瓶颈。不同模型需要不同数量的输出token才能达到可比性能，需要研究如何平衡模型大小与输出效率。

Method: 1. 在模拟数据上全面分析VLM各组件延迟；2. 在真实世界基准测试上进行实证研究；3. 提出多智能体推理框架：保持大型模型生成简短响应，但在必要时从小型模型转移关键推理token。

Result: 实验表明：大型模型用较少输出token可以比小型模型长序列更高效；实证研究证实大型模型能用显著更少的token达到更好或可比性能；通过重用小型模型的推理token，多智能体框架能接近大型模型自身推理的性能。

Conclusion: 输出token数量是VLM效率的关键瓶颈，大型模型在减少输出token方面具有优势。提出的多智能体推理框架通过重用小型模型的关键推理token，能在保持大型模型效率的同时提升性能，为VLM的延迟优化提供了有效解决方案。
...
TL;DR: LoMa通过数据驱动方法显著提升局部特征匹配性能，结合大数据集、现代训练方法、模型容量扩展和计算资源扩展，在多个基准测试中大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在少数中型数据集上训练，而新方法如前馈重建模型已从大规模数据集中受益。此外，当前基准测试主要基于成功3D重建的稀疏视图，评估局限于相对简单的图像对，导致基准测试饱和。

Method: LoMa采用数据驱动方法，结合大规模多样化数据混合、现代训练方案、扩展的模型容量和计算资源。同时创建HardMatch数据集，包含1000个来自互联网数据的极具挑战性的图像对，通过人工标注获得地面真实对应关系。
...
Conclusion: LoMa通过数据驱动方法显著推进了局部特征匹配技术，解决了现有基准测试饱和问题，并在多个具有挑战性的数据集上实现了性能突破，代码和模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: 传统两阶段方法将基于图像的虚拟试穿和姿态驱动动画作为独立过程处理，导致身份漂移、服装变形和前后不一致等问题。需要一种统一的方法来实现连贯的合成效果。

Method: 1. 构建大规模三元组监督数据：生成身份保持的人物图像（不同服装）、捕获完整上下装三元组、组装多样化真实世界三元组；2. 引入双模块架构的视频扩散变换器，稳定训练并保持预训练生成质量；3. 支持零样本服装插值。

Result: Vanast能够生成高保真、身份一致的人类动画视频，在服装准确性、姿态遵循和身份保持方面表现优异，支持广泛的服装类型。

Conclusion: Vanast通过统一框架解决了服装转移动画中的关键问题，实现了从单张图像到动画视频的连贯合成，为虚拟试穿和动画生成提供了有效的解决方案。
...
TL;DR: 通过训练代码大语言模型逐步模拟程序执行，结合监督微调和强化学习，提升其在竞争性编程任务中的表现


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在生成正确代码时存在无法准确估计程序执行的问题，特别是在生成代码的执行预测方面存在不足

Method: 结合监督微调（使用自然语言执行轨迹和基于真实执行的文本解释）与强化学习（使用可验证奖励），引入两个互补目标：给定代码和输入的输出预测，以及使用真实或自预测执行反馈解决竞争性编程任务

Result: 在多个竞争性编程基准测试中，该方法相比标准推理方法取得了持续改进，能够通过模拟测试执行进行自我验证和迭代自我修复

Conclusion: 训练代码LLMs模拟程序执行的能力可以显著提升竞争性编程性能，但执行模拟仍存在局限性
...
TL;DR: 提出基于约束最大似然估计的新方法，用于准确估计大语言模型的失败率，结合小规模人工标注、大规模LLM自动标注和领域特定约束，相比现有方法提供更准确、低方差的估计。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型失败率估计面临两难选择：昂贵的人工标注标准与可能存在严重偏差的自动标注方案（如"LLM-as-a-Judge"）。需要一种实用、高效的解决方案来确保LLM的安全部署。

Method: 提出基于约束最大似然估计的方法，整合三个信号源：1）小规模高质量人工标注校准集；2）大规模LLM自动标注数据；3）基于已知评估者性能统计的领域特定约束信息。

Result: 在多样化的实验环境中（包括不同评估者准确率、校准集大小和LLM失败率），该方法相比最先进的基线方法（如预测驱动推理PPI）始终提供更准确、方差更低的估计。

Conclusion: 通过超越"黑盒"自动评估的使用，提供了一个原则性、可解释且可扩展的框架，为大语言模型失败率认证提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型压缩方法要么需要特殊硬件支持，要么需要昂贵的后训练来保持模型质量，因此需要一种高效且经济实惠的模型压缩方法

Method: SoLA结合软激活稀疏性和低秩分解，识别并保留对推理贡献显著的小部分组件，同时通过低秩分解压缩大部分组件。采用自适应组件级低秩分配策略为不同权重矩阵分配适当的截断位置

Result: 在LLaMA-2-7B/13B/70B和Mistral-7B模型上的实验显示，SoLA在语言建模和下游任务准确性方面都有显著提升，无需后训练。例如，在LLaMA-2-70B模型上以30%压缩率，困惑度从6.95降至4.44，下游任务准确性提升10%

Conclusion: SoLA是一种无需训练的大语言模型压缩方法，能够有效减少模型规模同时保持甚至提升模型性能，为高效部署大语言模型提供了实用解决方案
...
TL;DR: Focus是一种高效注意力方法，通过可学习的质心将token分配到不同组，限制远距离注意力在同一组内，而局部注意力保持全分辨率。该方法保持所有模型权重冻结，仅训练质心参数，在保持下游任务性能的同时提升领域困惑度，并可通过硬稀疏模式实现推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有高效注意力方法在保持模型性能的同时难以实现真正的效率提升，特别是在微调场景下。Focus旨在通过学习token对的重要性关系而非近似所有token对，实现既高效又不损害模型性能的注意力机制。

Method: 使用可学习质心将token分配到不同组，远距离注意力仅限于同组token对，局部注意力保持全分辨率。所有模型权重保持冻结，仅训练质心参数。推理时通过top-k最高得分组将软路由离散化为硬稀疏模式，实现加速。使用Sinkhorn归一化强制平衡分组。

Result: 在124M到70B参数范围内，跨五种注意力架构，Focus在保持下游任务零退化的情况下改善领域困惑度。124M参数时超越全注意力（30.3 vs 31.4 PPL）；7B规模从头训练时也优于全注意力（13.82 vs 13.89 PPL）。推理时通过硬稀疏模式实现2倍加速，使用标准FlashAttention调用可达8.6倍加速。与LoRA不同，Focus保持模型对齐性，指令调优模型在适应后保留TruthfulQA分数。

Conclusion: Focus是一种高效且保持性能的注意力方法，通过可学习质心路由实现token分组，在保持模型权重冻结的情况下仅需少量参数训练即可提升性能。该方法在推理时可实现显著加速，且保持模型对齐性，优于现有高效注意力方法和适配器技术。
...
TL;DR: VIGIL是首个实时检测和缓解认知偏见触发器的浏览器扩展，通过滚动同步检测、LLM驱动的可逆重述和隐私分级推理来应对AI生成内容对信息完整性的威胁。


<details>
  <summary>Details</summary>
Motivation: 生成式AI对在线信息完整性和公民讨论构成风险，特别是通过利用人类认知偏见进行说服或操纵的微妙威胁。目前缺乏直接检测和缓解认知偏见触发器的工具。

Method: 开发VIGIL浏览器扩展，提供实时认知偏见触发器检测和缓解功能，包括滚动同步检测、LLM驱动的可逆重述、隐私分级推理（从完全离线到云端），并支持第三方插件扩展。

Result: VIGIL是首个此类工具，已在GitHub开源，包含多个经过NLP基准严格验证的插件，能够有效检测和缓解认知偏见触发器。

Conclusion: VIGIL填补了认知偏见检测工具的空缺，为应对生成式AI对公民讨论的微妙威胁提供了实用解决方案，具有可扩展性和隐私保护特性。
...
TL;DR: 提出Knowledge Packs：预计算的KV缓存，以零token成本提供相同知识。基于因果变换器的KV缓存特性，正确格式化下实现与RAG相同效果但节省95% token，同时支持RAG无法实现的行为引导。


<details>
  <summary>Details</summary>
Motivation: RAG方法消耗大量token，效率低下。作者希望找到一种更高效的知识检索方式，同时探索KV缓存未被充分利用的潜力，特别是其支持行为引导的可能性。

Method: 利用因果变换器的特性：文本F的前向传播产生的KV缓存与F+q联合传播产生的KV缓存完全相同。通过正确格式化聊天模板确保等价性，并利用RoPE只旋转key而不影响value的特性，在缓存值上应用对比性delta来引导模型行为。

Result: 在正确格式化下，Qwen3-8B和Llama-3.1-8B上700个问题零差异，实现高达95%的token节省。行为引导效果集中在中层值（33-66%），独立方向几乎正交（cos~0）且可组合，知识和引导通道在alpha≤0.7时可同时运行无干扰。

Conclusion: Knowledge Packs提供零token成本的知识检索，性能与RAG相当但更高效，同时支持独特的行为引导能力，无需训练或权重修改，为高效知识集成和行为控制开辟了新途径。
...
TL;DR: CresOWLve是一个评估大语言模型创造性问题解决能力的基准测试，使用基于现实世界知识的谜题，而非人工构造的脑筋急转弯。该基准测试显示，前沿LLMs在创造性问题上的表现比事实性问题差17%，表明模型能检索相关知识但难以进行创造性连接。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型基准测试大多只评估创造性问题解决过程的特定组成部分，且许多创造性导向的基准测试依赖于人工构造的脑筋急转弯或不切实际的场景，不能反映真实世界中的创造性问题解决过程。

Method: 引入CresOWLve基准测试，使用基于现实世界知识的谜题来评估创造性问题解决能力。这些谜题需要运用多种创造性思维策略，从不同领域检索事实，并创造性地组合这些信息来得出解决方案。评估了多个前沿的非思维型和思维型大语言模型。

Result: CresOWLve对前沿LLMs仍然具有高度挑战性。分析显示存在一致的性能差距：模型在事实性问题上的表现明显优于创造性问题（最多下降17%）。模型通常能够检索相关知识，但难以形成非显而易见的创造性连接来整合信息并得出正确答案。

Conclusion: 需要开发能够更好地支持创造性连接形成的大语言模型，而不仅仅是事实检索。CresOWLve基准测试为评估和改进大语言模型的创造性问题解决能力提供了更真实、更全面的评估工具。
...
TL;DR: 研究探索噪声引导技术，通过在推理时向transformer模型内部表示注入校准的高斯扰动，作为无需训练的多样性生成方法，用于阿拉伯语早期阅读评估的故事生成。


<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语早期阅读评估生成多样化且教学有效的故事需要平衡词汇、阅读水平和叙事结构的严格约束，同时避免重复情节破坏评估有效性。

Method: 研究噪声引导技术，在推理时向transformer模型的内部表示注入校准的高斯扰动。比较了四种注入策略与高温采样基线，评估了五个小型阿拉伯语中心语言模型（7-9B参数）。

Result: 残差流噪声注入能持续提高叙事多样性，同时保持质量和约束成本最小，并在所有模型中保持早期阅读水平。注意力熵噪声注入稳定了原本不可靠的注意力-逻辑噪声，同时恢复了质量。高温采样会提高阅读等级，并在多个模型上导致灾难性崩溃。

Conclusion: 内部表示层面的扰动比输出层面的随机性更适合受约束的教育内容生成，噪声引导是一种有效的无需训练多样性生成策略。
...
TL;DR: QIMMA是一个质量保证的阿拉伯语大语言模型排行榜，通过系统化的基准验证流程，结合自动LLM判断和人工审查，对现有阿拉伯语基准进行质量评估和修正，最终构建了一个包含5.2万样本的多领域、多任务评估套件。


<details>
  <summary>Details</summary>
Motivation: 现有的阿拉伯语基准测试存在系统性质量问题，需要建立一个质量保证的评估框架，为阿拉伯语NLP提供可靠、可复现的评估基础。

Method: 采用多模型评估流程，结合自动LLM判断和人工审查，对现有阿拉伯语基准进行系统性质量验证和问题修正；使用LightEval和EvalPlus进行透明实现，并公开发布每个样本的推理输出。

Result: 构建了一个包含超过5.2万个样本的精选多领域、多任务评估套件，主要基于原生阿拉伯语内容（代码评估任务除外）；提供了可复现、社区可扩展的阿拉伯语NLP评估基础。

Conclusion: QIMMA通过系统化的质量保证流程，为阿拉伯语大语言模型评估提供了可靠、透明、可复现的排行榜框架，有助于推动阿拉伯语NLP研究的发展。
...
TL;DR: 该论文提出"形态驱动标记"的形式模型，将名词组织成具有各自形态模板和无标记形式的模块化认知集合，用于解释语言内部和跨语言的名词标记差异，并重新评估标记性和状态概念。
...
Method: 提出"形态驱动标记"的形式模型。名词被组织成模块化认知集合，每个集合有自己的形态模板和无标记形式。通过将这些模式置于句法功能中，重新评估标记性和状态概念。

Result: 该模型有助于解释语言内部和跨语言的名词类型标记差异。提出将状态概念扩展到所有综合语言，并将其分析为基于句法的屈折变化的新子类别，类似于一致关系和语法格。

Conclusion: 通过形态驱动标记模型，能够更好地理解名词范畴在不同语言中的实现差异。状态概念应被重新概念化为基于句法的屈折变化类型，适用于所有综合语言，为跨语言的名词标记研究提供了新的理论框架。
...
TL;DR: 论文提出词汇丢弃方法，通过随机掩码语言模型输出logits来维持问题生成多样性，解决自博弈中课程多样性崩溃问题，在数学推理任务上取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 在语言模型的自博弈协同进化中，问题生成者会快速收敛到满足奖励函数的狭窄问题分布，导致多样性崩溃，使课程对求解者失去信息价值，阻碍协同进化循环。

Method: 引入词汇丢弃方法，在策略训练和课程生成过程中对问题生成者的输出logits应用随机硬掩码，防止模型锁定到固定的token序列，维持问题多样性。

Result: 在Qwen3-4B和Qwen3-8B模型上进行数学推理训练，词汇丢弃方法在词汇、语义和功能三个维度上维持了问题生成者的多样性，使求解者在8B模型上平均提升4.4分，在竞赛级基准上提升最大。

Conclusion: 显式的动作空间约束（类似于经典自博弈中游戏规则的结构性作用）有助于维持语言协同进化的生产力，词汇丢弃是这一原则的简单实现。
...
TL;DR: 使用LLM驱动的进化搜索自动发现无监督不确定性量化方法，在原子声明验证任务中超越人工设计基线，性能提升达6.7%


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的不确定性量化方法主要依赖人工设计的领域知识和启发式方法，这限制了方法的可扩展性和通用性

Method: 采用LLM驱动的进化搜索自动发现以Python程序表示的无监督UQ方法，通过进化算法优化方法设计

Result: 在9个数据集上，进化出的方法相比强人工基线获得最高6.7%的相对ROC-AUC提升，且能鲁棒地泛化到分布外数据；不同LLM展现出不同的进化策略

Conclusion: LLM驱动的进化搜索是自动化、可解释的幻觉检测器设计的有前景范式，但不同LLM在利用方法复杂性方面存在差异
...
### [153] [Cultural Authenticity: Comparing LLM Cultural Representations to Native Human Expectations](https://arxiv.org/abs/2604.03493)
*Erin MacMurray van Liemt,Aida Davani,Sinchana Kumbale,Neha Dixit,Sunipa Dev*

Main category: cs.CL

TL;DR: 本文提出了一种评估LLM文化对齐度的框架，通过人类调查建立文化重要性向量，与模型生成的文化表征向量对比，发现模型存在西方中心主义偏差，且对某些文化标记过度关注而忽视深层社会价值观。


<details>
  <summary>Details</summary>
Motivation: 现有LLM文化评估主要关注文化多样性和事实准确性，但缺乏对文化对齐度的评估——即生成内容是否与当地人群对自己文化的认知和优先级相匹配。需要建立更人性化的评估框架来评估LLM生成内容与当地期望的一致性。

Method: 1. 通过9个国家的开放式调查建立人类衍生的文化重要性向量作为基准；2. 基于语法多样化的提示集计算模型衍生的文化表征向量；3. 将三个前沿LLM（Gemini 2.5 Pro、GPT-4o、Claude 3.5 Haiku）的文化表征向量与人类重要性向量进行对齐分析。

Result: 研究发现：1. 某些模型存在西方中心主义校准，随着国家与美国文化距离增加，对齐度下降；2. 所有模型都显示出高度相关的系统性错误特征（ρ>0.97），过度关注某些文化标记而忽视用户深层次的社会和价值观优先级。

Conclusion: 该方法超越了简单的多样性指标，能够评估AI生成内容在真实捕捉全球文化细微层次结构方面的保真度，为评估LLM文化对齐提供了更全面的框架。
...
### [154] [LangFIR: Discovering Sparse Language-Specific Features from Monolingual Data for Language Steering](https://arxiv.org/abs/2604.03532)
*Sing Hieng Wong,Hassan Sajjad,A. B. Siddique*

Main category: cs.CL

TL;DR: LangFIR：一种仅使用单语数据和随机标记序列识别语言特定特征的方法，用于控制多语言LLM的输出语言


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具有多语言能力，但可靠控制其输出语言仍然困难。现有方法通常需要昂贵的多语言或平行数据来识别语言特定方向，这限制了方法的实用性。

Method: LangFIR使用稀疏自编码器分解残差激活，通过随机标记序列过滤掉语言无关特征，仅使用少量单语数据就能识别出稀疏的语言特定特征。这些特征用于构建控制多语言生成的引导向量。

Result: LangFIR在三个模型（Gemma 3 1B、Gemma 3 4B和Llama 3.1 8B）、三个数据集和十二种目标语言上取得了最佳平均准确率BLEU，优于最强的单语基线方法，甚至超过了依赖平行数据的方法。

Conclusion: 多语言LLM中的语言身份信息集中在稀疏的特征方向上，这些方向可以通过单语数据发现。LangFIR为多语言生成控制提供了一种高效且有效的方法。
...
### [155] [Rethinking Token Prediction: Tree-Structured Diffusion Language Model](https://arxiv.org/abs/2604.03537)
*Zihao Wu,Haoming Yang,Juncheng Dong,Vahid Tarokh*

Main category: cs.CL

TL;DR: 提出树结构扩散语言模型，通过词汇树分解降低分类维度，减少参数和内存使用，在相同参数预算下将GPU峰值内存减半并保持性能


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型训练效率低，全词汇表预测层占用大量参数和GPU内存，在有限参数和内存预算下效率低下

Method: 构建树结构扩散语言模型，利用词汇树结构，在扩散过程中建模中间潜在状态对应词汇树中的祖先节点，指数级降低分类维度

Result: 在相同参数预算下，方法将GPU峰值内存使用减半，同时匹配最先进离散扩散语言模型的困惑度性能

Conclusion: 树结构分解有效减少参数和内存使用，使注意力块能够加深，提高了离散扩散语言模型的训练效率
...
### [156] [MultiPress: A Multi-Agent Framework for Interpretable Multimodal News Classification](https://arxiv.org/abs/2604.03586)
*Tailong Luo,Hao Li,Rong Fu,Xinyue Jiang,Huaxuan Ding,Yiduo Zhang,Zilin Zhao,Simon Fong,Guangyin Jin,Jianyuan Ni*

Main category: cs.CL

TL;DR: MultiPress：一个用于多模态新闻分类的三阶段多智能体框架，通过专业化智能体协作和检索增强推理提升分类性能


<details>
  <summary>Details</summary>
Motivation: 随着多模态新闻内容的增长，现有方法通常独立处理不同模态或采用简单融合策略，难以捕捉复杂的跨模态交互和利用外部知识，限制了多模态新闻分类的效果。
...
Conclusion: MultiPress框架通过专业化智能体分工协作和检索增强推理，有效解决了多模态新闻分类中跨模态交互和知识利用的挑战，为多模态新闻分析提供了新思路。
...
### [157] ['Layer su Layer': Identifying and Disambiguating the Italian NPN Construction in BERT's family](https://arxiv.org/abs/2604.03673)
*Greta Gorzoni,Ludovica Pannitto,Francesca Masini*

Main category: cs.CL

TL;DR: 该研究使用BERT模型分析意大利语NPN结构，通过分层探测分类器评估上下文嵌入中编码的构式形式和意义信息


<details>
  <summary>Details</summary>
Motivation: 评估预训练语言模型（特别是上下文嵌入）对明确语言学理论的编码能力，挑战先前实验设计的理论和方法论假设，并将此类研究扩展到较少被研究的语言（意大利语）
...
Conclusion: 该研究通过分析意大利语NPN构式，为理解预训练语言模型如何编码语言学信息提供了新见解，特别是对较少研究语言的构式分析，挑战了先前研究的假设并扩展了该领域的研究范围
...
### [158] [Unlocking Prompt Infilling Capability for Diffusion Language Models](https://arxiv.org/abs/2604.03677)
*Yoshinari Fujinuma,Keisuke Sakaguchi*

Main category: cs.CL

TL;DR: 论文提出通过全序列掩码训练解锁扩散语言模型的提示填充能力，替代传统的仅响应掩码方法，使模型能够自动填充有效的提示模板。


<details>
  <summary>Details</summary>
Motivation: 当前扩散语言模型虽然具有双向去噪能力，但这一能力在提示填充任务中被锁定，主要原因是传统的监督微调只对响应部分进行掩码，限制了模型在提示填充方面的潜力。

Method: 扩展全序列掩码监督微调方法，在训练时同时对提示和响应进行联合掩码，从而解锁模型的提示填充能力，使模型能够基于少量示例填充提示模板的掩码部分。

Result: 模型填充的提示模板能够匹配或超越人工设计的模板，在不同模型间有效迁移，并且与现有的提示优化方法互补，表明训练实践而非架构限制是主要瓶颈。

Conclusion: 通过改变训练实践（采用全序列掩码而非仅响应掩码），可以解锁扩散语言模型的提示填充能力，为提示工程提供新的自动化途径。
...
### [159] [LightThinker++: From Reasoning Compression to Memory Management](https://arxiv.org/abs/2604.03679)
*Yuqi Zhu,Jintian Zhang,Zhenjie Wan,Yujie Luo,Shuofei Qiao,Zhengke Gui,Da Zheng,Lei Liang,Huajun Chen,Ningyu Zhang*

Main category: cs.CL

TL;DR: LightThinker++：通过显式自适应内存管理，动态压缩LLM推理过程中的中间思考，显著降低token使用和推理时间，同时提升长序列推理性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理中表现出色，但长思考轨迹带来的认知开销限制了其效率。静态压缩方法在处理复杂推理时存在中间细节不可逆丢失的问题，可能导致逻辑瓶颈。

Method: 提出LightThinker++框架，引入显式自适应内存管理范式，通过行为级管理和显式内存原语，配合专门的轨迹合成管道来训练有目的的内存调度。

Result: 1. LightThinker减少峰值token使用70%，推理时间26%，精度损失最小；2. LightThinker++在标准推理中减少峰值token使用69.9%，在相同上下文预算下获得+2.42%精度提升；3. 在长视野智能体任务中，80轮后保持稳定占用（减少60%-70%），平均性能提升14.8%。

Conclusion: 该工作为在最小开销下维持深度LLM推理提供了可扩展的方向，特别是在长序列推理场景中具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 当前社会科学研究中，研究人员通常对每个变量、每个文本使用单独的API调用进行分类，这种方法成本高昂且效率低下。需要验证批量处理和变量堆叠是否会影响编码质量

Method: 测试了来自4个提供商的8个生产级LLM模型，使用3,962条专家编码的推文，在4个任务上进行实验。变量包括：批量大小从1到1000个项目，每个提示中堆叠最多25个编码维度

Result: 8个模型中的6个在批量大小达到100时，准确率仍保持在单项目基线的2个百分点内。变量堆叠最多10个维度时，结果与单变量编码相当。性能下降主要由任务复杂性而非提示长度驱动。在安全操作范围内，批量处理和堆叠带来的测量误差小于真实数据中典型的编码员间分歧

Conclusion: 批量处理和变量堆叠是可行且高效的LLM文本分类策略，可大幅降低成本而不显著影响编码质量，为社会科学研究提供了实用的优化方案
...
TL;DR: POEMetric是首个全面的诗歌评估框架，用于评估LLM与人类诗人在诗歌创作上的差距，发现LLM在形式准确性上表现良好，但在创造力、情感共鸣等高级能力上远不及人类诗人。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型能够创作诗歌，但它们在诗歌创作方面与人类诗人的真实差距尚不清楚。需要建立一个全面的评估框架来系统评估LLM在诗歌创作方面的能力，包括基本指令遵循能力和高级创作能力。

Method: 开发了POEMetric评估框架，包含三个维度：1)基本指令遵循能力（形式和主题）；2)高级能力（创造力、词汇多样性、独特性、情感共鸣、意象和文学手法）；3)整体质量评估和作者归属判断。收集了203首人类诗歌数据集，包含7种固定形式，标注了韵律、押韵模式和主题。使用30个LLM基于相同形式和主题生成了6,090首诗歌。通过基于规则的评估和LLM-as-a-judge进行评估，并由人类专家验证结果。

Result: 虽然顶级模型在形式准确性（4.26/5.00）和主题对齐（4.99）上表现良好，但所有模型在高级能力上都未能达到人类诗人的水平。人类诗人在创造力（4.02）、独特性（3.95）、情感共鸣（4.06）、意象运用（4.49）和文学手法（4.67）方面表现卓越。在整体诗歌质量上，人类（4.22）也优于最佳LLM（3.20）。

Conclusion: 诗歌生成对LLM来说仍然是一个艰巨的挑战。尽管LLM在遵循诗歌形式和主题方面表现出色，但在创造力、情感深度和文学技巧等关键诗歌创作维度上仍远不及人类诗人。POEMetric为诗歌评估提供了一个全面的框架，有助于推动LLM在创造性写作领域的发展。
...
TL;DR: 研究发现LLM激活空间中的"真理方向"普遍性存在多种限制：具有层依赖性、任务类型依赖性、任务复杂度依赖性，且受模型指令显著影响。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为LLM激活空间中存在普遍的线性真理方向，但近期研究质疑这种普遍性。本研究旨在更全面地探索真理方向普遍性的限制条件。

Method: 通过多层面分析：1) 在不同模型层探测真理方向；2) 比较事实性和推理性任务的差异；3) 分析不同任务复杂度下的表现；4) 研究不同指令模板对真理探针泛化能力的影响。

Result: 发现真理方向具有高度层依赖性，事实性任务在较早层出现而推理任务在较深层出现，任务复杂度影响方向性能，简单正确性评估指令显著影响真理探针的泛化能力。

Conclusion: 真理方向的普遍性比先前认知更为有限，在不同模型层、任务难度、任务类型和提示模板下存在显著差异，对理解LLM中真理表示具有重要意义。
...
TL;DR: 该研究系统评估了多智能体系统在动态环境中的间接提示注入攻击防御策略，发现现有防御存在严重脆弱性，并提出基于表征工程的检测方法能有效识别和拦截未授权操作。


<details>
  <summary>Details</summary>
Motivation: 开源框架的快速部署推动了多智能体系统发展，但扩展的动作空间带来了严重安全挑战。间接提示注入攻击通过在第三方内容中隐藏恶意指令，可在正常操作中触发未授权操作。现有安全评估主要依赖孤立单轮基准测试，复杂动态环境中智能体的系统性漏洞尚未得到充分探索。

Method: 在动态多步骤工具调用环境中，系统评估了六种防御策略对抗四种复杂IPI攻击向量，涉及九个LLM骨干模型。采用多维分析方法，超越二元成功率，并研究表征工程作为检测策略，通过提取工具输入位置的隐藏状态来识别异常。

Result: 高级注入攻击成功绕过几乎所有基线防御，某些表面缓解措施甚至产生反效果。智能体几乎瞬时执行恶意指令，但其内部状态表现出异常高的决策熵。基于表征工程的断路器在智能体提交未授权操作前成功识别并拦截，在不同LLM骨干上实现高检测准确率。

Conclusion: 研究揭示了当前IPI防御的局限性，为构建弹性多智能体架构提供了高度实用的范式。基于表征工程的检测方法能有效识别和拦截未授权操作，是构建安全多智能体系统的有前景方向。
...
<details>
  <summary>Details</summary>
Motivation: 类比推理是叙事理解的核心认知能力。虽然LLMs在表面和结构线索一致时表现良好，但在需要潜在信息的类比中表现不佳，这表明在抽象和泛化方面存在局限性。研究者希望探索模型内部表征与提示行为之间的关系

Method: 通过比较模型的探测表征与提示性能来检测叙事类比，分析两种方法在修辞类比和叙事类比上的表现差异

Result: 发现不对称现象：对于修辞类比，探测方法在开源模型中显著优于提示方法；而对于叙事类比，两种方法都表现出相似的低性能。这表明内部表征与提示行为之间的关系是任务依赖的

Conclusion: 内部表征与提示行为之间的关系是任务依赖的，可能反映了提示方法在访问可用信息方面的局限性。这为理解LLMs的类比推理能力提供了新的视角
...
TL;DR: 通过提示工程减少大语言模型幻觉：I-CALM框架通过引导口头置信度、奖励弃权和加入规范性原则，在不修改模型的情况下降低事实性问题的错误回答率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生自信但错误的答案，部分原因是常见的二元评分惯例奖励回答问题而非诚实表达不确定性。研究者希望探索仅通过提示干预（不修改模型）是否能减少幻觉风险，特别是在模型对答案不确定时仍不弃权的事实性问题场景。

Method: 提出I-CALM框架：1) 引导口头置信度作为不确定性信号；2) 通过明确奖励方案部分奖励弃权；3) 加入强调真实性、谦逊和责任的轻量级规范性原则。在GPT-5 mini和PopQA数据集上进行实验。

Result: 置信度引导、弃权奖励提示（特别是加入规范原则）主要通过识别和将易错案例转为弃权，并重新校准置信度，从而降低已回答案例的错误率。这以覆盖率为代价换取可靠性，同时强制回答性能基本不变。改变弃权奖励会产生清晰的弃权-幻觉边界。

Conclusion: 该框架可以在不重新训练的情况下改进事实性问题的选择性回答，效果大小因模型和数据集而异。口头置信度是一个稳定的不确定性信号，提示工程可以有效减少幻觉。
...
TL;DR: 论文提出在基于LLM的社会模拟中采用因果反事实框架，区分必要因果和充分因果，以支持政策干预的因果推断而非仅追求可信度。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的社会模拟虽然能生成可信的社区互动，可用于"政策风洞"测试治理干预，但可信度不等于因果关系。现有模拟工作通常缺乏因果语义规范，无法支持如"干预A减少冲突升级"等因果主张。

Method: 采用因果反事实框架，区分必要因果关系（没有干预结果是否会发生）和充分因果关系（干预是否可靠产生结果）。将这种区分映射到不同利益相关者需求：事件诊断需要必要性证据，政策设计需要充分性证据。形式化这种映射，展示如何在明确假设下支持估计，并论证应将结果解释为模拟器条件因果估计。

Result: 建立了一个因果框架，帮助定义什么是足够的模拟保真度，推动该领域从"看起来真实"的模拟转向"能够支持政策变化"的模拟。该框架对于政策相关性取决于模拟器保真度的因果估计提供了理论基础。

Conclusion: 现在建立这个框架至关重要：它有助于定义足够的保真度意味着什么，并将该领域从看起来真实的模拟转向能够支持政策变化的模拟。模拟器条件因果估计的政策相关性取决于模拟器的保真度。
...
TL;DR: CUP框架通过将不确定性作为多轮决策的指导信号，结合语言模型和结构化规划，提升了目标导向对话系统的成功率并减少了交互轮次。


<details>
  <summary>Details</summary>
Motivation: 现有目标导向对话系统存在局限性：结构化方法依赖预定义模式，而基于LLM的方法缺乏长视野决策能力，导致信息获取和目标承诺之间协调不佳。

Method: 提出对话不确定性感知规划框架(CUP)，将目标导向对话建模为不确定性感知的序列决策问题。语言模型提出可行动作，规划器评估这些动作对长期不确定性减少的影响。

Result: 在多个对话基准测试中，CUP一致提高了成功率，同时减少了交互轮次。分析表明不确定性感知规划有助于更高效的信息获取和更早的自信承诺。

Conclusion: CUP框架通过整合语言模型和结构化规划，利用不确定性作为决策指导，有效解决了目标导向对话中信息获取和目标承诺的协调问题。
...
TL;DR: StreamGuard是一个统一的模型无关流式护栏，将审核问题重新定义为预测问题而非边界检测，通过蒙特卡洛推演预测部分前缀的未来有害性，实现早期干预。


<details>
  <summary>Details</summary>
Motivation: 现有流式护栏通常将输出端安全检测视为边界检测问题，需要训练模型识别响应变得不安全的最早前缀。这种方法需要精确的令牌级边界标注，限制了实际应用。

Method: StreamGuard将审核重新定义为预测问题：给定部分前缀，模型预测可能未来延续的预期有害性。使用蒙特卡洛推演进行监督，无需精确的令牌级边界标注。

Result: 在标准安全基准测试中，StreamGuard在输入审核和流式输出审核方面表现优异。在8B规模上，相比Qwen3Guard-Stream-8B-strict，输入审核F1从86.7提升到88.2，流式输出审核F1从80.4提升到81.9。在QWENGUARDTEST响应流式基准上达到97.5 F1、95.1召回率和92.6%及时干预率。

Conclusion: 基于预测未来风险的监督策略是实现低延迟安全干预的有效方法，无需精确边界标注即可获得强大的端到端流式审核能力，且这种监督策略在不同分词器和模型家族间具有良好的迁移性。
...
Method: 基于Lloyd-Max最优性条件理论分析，提出两阶段正交变换方法RUQuant：第一阶段将激活分块，通过Householder反射和Givens旋转构造的复合正交矩阵映射到均匀采样目标向量；第二阶段微调全局Householder反射，利用Transformer输出差异最小化量化误差。

Result: 实验结果显示，RUQuant在13B LLM上，W6A6量化达到99.8%全精度准确率，W4A4量化达到97%准确率，耗时约一分钟。微调变体获得更高精度，证明方法的有效性和可扩展性。

Conclusion: RUQuant通过理论驱动的正交变换方法，有效解决了激活量化中的非均匀分布问题，实现了接近最优的量化性能，无需模型微调且计算高效，为资源受限环境下的LLM部署提供了实用解决方案。
...
TL;DR: 研究发现，在推理时通过few-shot提示注入文化负载数字可以导致大型语言模型产生语义漂移，使输出偏向黑暗、威权等负面主题，但需要模型具备足够能力才能观察到这种效应。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在训练时微调LLM使用不安全代码或文化负载数字会导致模型在无关下游任务中产生有害内容，但认为仅通过k-shot提示不会引发这种效应。本研究重新审视这一结论，探究推理时语义漂移是否真实存在且可测量。

Method: 采用控制实验设计，在语义无关的提示前注入五个文化负载数字作为few-shot演示示例。比较不同能力模型的表现：具有丰富文化关联表征的模型与较简单/较小的模型。同时测试结构惰性演示（无意义字符串）对输出的影响。

Result: 研究发现：1）只有具备足够能力的大型模型才会出现显著的分布偏移，输出偏向更黑暗、威权和污名化的主题；2）较简单/较小的模型未表现出这种效应；3）结构惰性演示也会扰动输出分布，表明存在两种可分离机制：结构格式污染和语义内容污染。

Conclusion: 推理时污染效应确实存在且可测量，但需要模型具备足够能力。研究确定了推理时污染发生的边界条件，对使用few-shot提示的LLM应用安全性具有直接意义，表明需要防范推理时的语义漂移风险。
...
TL;DR: 本文首次对小型语言模型（100M-10B参数）进行情感向量提取方法比较研究，发现生成式提取方法在情感分离上表现更优，情感表征位于中间Transformer层，并通过转向实验验证了因果行为效应。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在100M-10B参数范围内越来越多地应用于生产系统，但尚不清楚它们是否具有最近在大型前沿模型中发现的内部情感表征。本研究旨在填补这一空白，为开放权重模型的情感研究提供方法论指导。

Method: 使用9个模型（涵盖GPT-2、Gemma、Qwen、Llama、Mistral 5个架构家族），评估20种情感和两种提取方法（生成式和理解式）。通过转向实验验证因果行为效应，并使用外部情感分类器进行独立验证。

Result: 生成式提取方法在情感分离上具有统计显著优势（Mann-Whitney p = 0.007；Cohen's d = -107.5）。情感表征位于中间Transformer层（约50%深度），呈现U型曲线且架构不变。转向实验揭示了三种机制：精确转向、重复崩溃和爆炸性退化，由模型架构而非规模决定。在Qwen中发现跨语言情感纠缠问题。

Conclusion: 本研究为开放权重模型的情感研究提供了方法论指南，通过将外部行为分析与内部表征分析相结合，为"模型医学"系列做出贡献。研究揭示了小型语言模型中的情感表征特性，并指出了多语言部署中的安全问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管ID嵌入在认知建模中仍是主流方法，但语言模型提供了通过语义集成增强认知诊断性能的新可能。现有研究在充分利用语言模型方面面临两个关键挑战：语言模型训练目标与认知诊断模型之间的不对齐导致特征空间分布差距；需要统一框架来整合不同认知诊断任务中的文本嵌入，同时保持现有认知建模范式的优势。

Method: EduEmbed采用两阶段框架：第一阶段基于角色特定表示和交互诊断器微调语言模型，以弥合认知诊断模型的语义差距；第二阶段使用文本适配器提取任务相关语义，并将其与现有建模范式集成以提高泛化能力。

Result: 在四个认知诊断任务和计算机自适应测试任务上评估，取得了稳健的性能。进一步分析揭示了语义信息在不同任务中的影响。

Conclusion: EduEmbed为在线智能教育系统中语言模型在认知诊断的应用提供了关键见解，通过统一的嵌入增强框架有效整合语义信息，提升认知建模性能。
...
TL;DR: 本文首次系统研究了长思维链压缩对模型可信度的影响，发现压缩会损害安全性、抗幻觉能力和多语言鲁棒性，并提出对齐感知的DPO变体来减少可信度损失。


<details>
  <summary>Details</summary>
Motivation: 现有长思维链压缩研究主要关注任务准确性和token节省，但忽略了可信度属性。由于可信度属性与模型参数空间紧密相关，压缩可能损害模型的可信度，需要系统评估。

Method: 1) 系统评估多种不同规模模型的思维链压缩对可信度的影响；2) 从三个维度评估：安全性、抗幻觉能力和多语言鲁棒性；3) 提出标准化效率评分进行公平比较；4) 引入对齐感知的DPO变体来减少可信度损失。

Result: 研究发现：1) 思维链压缩经常引入可信度回归；2) 不同压缩方法在不同维度上表现出明显不同的退化模式；3) 提出的对齐感知DPO变体在推理基准上减少19.3%的思维链长度，同时显著降低可信度损失。

Conclusion: 思维链压缩不仅应优化效率，还应同等重视可信度，将两者都视为重要的设计约束。标准化效率评分有助于揭示传统标量指标可能掩盖的可信度权衡。
...
TL;DR: PALM算法通过构建小型LLM组合来个性化服务用户，在系统成本与个性化需求间取得平衡，提供理论保证和实际验证。


<details>
  <summary>Details</summary>
Motivation: 为每个用户维护单独的LLM不切实际（计算、内存、系统复杂度限制），需要一种方法在有限资源下实现有效的个性化服务。

Method: 提出PALM算法，将用户偏好建模为多维权重向量，基于奖励函数生成小型LLM组合，确保对任何权重向量都能提供接近最优的LLM。

Result: 首次为LLM组合的规模和近似质量提供理论保证，量化了系统成本与个性化之间的权衡，实证验证了比基线方法更高的输出多样性。

Conclusion: PALM通过小型LLM组合有效解决LLM个性化难题，在资源约束下实现高质量个性化服务，为实际部署提供了理论指导和实用方案。
...
TL;DR: 使用小型语言模型(SLMs)从儿科肾脏活检报告中提取结构化信息的半自动化标注工作流程，在CPU基础设施上达到84.3%准确率


<details>
  <summary>Details</summary>
Motivation: 电子病历系统包含大量有价值的临床信息，但大多以非结构化文本形式存在，限制了其在研究和决策中的应用。大型语言模型可以提取这些信息，但需要大量计算资源，而将敏感的临床数据发送到云端服务（即使去标识化）存在患者隐私风险。

Method: 开发资源高效的半自动化标注工作流程，使用小型语言模型从非结构化儿科肾脏活检报告中提取结构化信息。将提取任务构建为基于临床指导实体指南和少量示例的问答任务，评估五个指令调优的SLMs，并使用分歧建模框架优先处理需要临床审查的报告。

Result: Gemma 2 2B模型达到最高准确率84.3%，优于现成模型如spaCy(74.3%)、BioBERT-SQuAD(62.3%)、RoBERTa-SQuAD(59.7%)和GLiNER(60.2%)。实体指南将性能提升7-19%，少量示例提升6-38%，但两者结合时效果不叠加。

Conclusion: 小型语言模型可以在仅使用CPU基础设施的情况下，以最少的临床参与从专业临床领域提取结构化信息，为资源受限环境中的临床信息提取提供了可行解决方案。
...
TL;DR: 论文指出，在事实核查中使用形式逻辑来缓解LLM偏见和幻觉存在结构性缺陷，因为逻辑上合理的结论与人类实际推理之间存在系统性差异，建议将LLM的人类化推理倾向作为特征而非缺陷来使用。
...
Method: 基于认知科学和语用学研究，作者提出了一个类型学，展示了逻辑上合理的结论如何系统地引发人类推理，而这些推理并不被底层前提所支持。他们主张采用互补方法：将LLM的人类化推理倾向作为特征而非缺陷，利用这些模型来验证神经符号系统中形式组件的输出，防止潜在的误导性结论。

Result: 研究发现，纯粹依赖形式逻辑的方法在检测误导性主张方面存在结构性缺陷，因为人类推理与形式逻辑之间存在系统性差异。逻辑上合理的结论可能引发人类做出不被前提支持的推理。

Conclusion: 作者建议不应将LLM的人类化推理倾向视为缺陷，而应将其作为特征来补充形式逻辑方法。通过利用LLM的推理能力来验证神经符号系统的输出，可以更好地检测和防止误导性结论，实现更有效的事实核查。
...
TL;DR: 该研究首次系统性地分析了LLM开发流程中标准英语变体（美式英语和英式英语）的方言不对称性，发现当代LLMs将美式英语视为事实标准，揭示了语言同质化、认知不公和全球AI部署不平等问题。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs越来越多地部署在高风险领域，但它们主要暴露"英语（美国）"等有限语言设置，忽视了英语的全球多样性和殖民历史。研究旨在通过后殖民框架，探讨数据策展、数字主导和语言标准化的地缘政治历史如何塑造LLM开发流程。

Method: 1. 构建包含1,813个美式英语-英式英语变体的精选语料库；2. 引入DiAlign方法，使用分布证据进行动态、无需训练的方言对齐估计；3. 通过三个阶段的证据三角测量操作化结构偏见：预训练语料库审计、分词器分析和生成评估。

Result: 1. 六个主要预训练语料库审计显示系统性偏向美式英语；2. 分词器分析表明英式英语形式产生更高的分词成本；3. 生成评估显示模型输出中持续存在美式英语偏好；4. 当代LLMs将美式英语视为事实标准。

Conclusion: 研究揭示了LLMs中方言不对称性的系统性模式，引发了关于语言同质化、认知不公和全球AI部署不平等的担忧，同时为开发更具方言包容性的语言技术提供了实践动力。
...
TL;DR: DARE是一个用于扩散大语言模型（dLLMs）后训练和评估的开源框架，旨在解决当前dLLMs开源生态碎片化问题，统一监督微调、参数高效微调、偏好优化和dLLM特定强化学习等流程。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型作为自回归模型的替代方案正在兴起，但其开源生态系统在模型家族和后训练流程方面存在严重碎片化。强化学习目标、实现代码和评估脚本通常以特定论文代码库形式发布，这减缓了研究迭代速度，增加了复现的工程负担，并使得跨算法公平比较变得困难。

Method: DARE基于verl和OpenCompass构建，为掩码和块扩散语言模型提供了统一的执行栈。它整合了监督微调、参数高效微调、偏好优化和dLLM特定强化学习，支持LLaDA、Dream、SDAR、LLaDA2.x等代表性模型家族，提供广泛的算法覆盖、可复现的基准评估和实际加速。

Result: DARE作为一个可重用的研究平台，为当前和新兴dLLMs的后训练方法开发、比较和部署提供了支持。广泛的实证结果表明该框架的有效性和实用性。

Conclusion: DARE框架解决了dLLMs后训练生态的碎片化问题，通过统一的执行栈和广泛的算法支持，加速了dLLMs的研究迭代和公平比较，为扩散大语言模型的发展提供了重要的基础设施支持。
...
TL;DR: ACE框架通过预测熵路由高不确定性专利权利要求到专家LLM，结合CoPT协议，在保持94.95% F1分数的同时降低78%运营成本
...
Method: 提出ACE混合框架，使用预测熵筛选高不确定性权利要求路由到专家LLM，专家执行基于35 U.S.C.法规标准的CoPT（专利思维链）协议

Result: ACE在评估方法中获得最佳F1分数94.95%，相比独立LLM部署减少78%运营成本；同时构建了包含40,000个权利要求和MPEP基础错误标注的ACE-40k基准数据集

Conclusion: ACE框架有效解决了专利验证中的刚性-资源困境，通过智能路由机制平衡了准确性和成本，为专利权利要求验证提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM个性化系统大多在用户偏好稳定、缺乏客观标准的领域运行，而个人投资决策领域暴露了现有定制范式的根本局限性，需要专门研究。

Method: 基于实际部署的AI增强投资组合管理系统，识别了个人投资领域中LLM个性化的四个核心挑战轴，并描述了相应的架构响应方案。

Result: 识别了四个关键挑战：行为记忆复杂性、漂移下的论点一致性、风格-信号张力、无真实标签的对齐，并提出了相应的架构解决方案。

Conclusion: 个人投资决策是高风险、时间延展的决策领域，暴露了当前LLM个性化范式的局限性，需要新的研究方向和架构设计来应对这些挑战。
...
### [181] [Benchmarking Multi-turn Medical Diagnosis: Hold, Lure, and Self-Correction](https://arxiv.org/abs/2604.04325)
...
TL;DR: MINT基准测试揭示LLMs在多轮医学诊断中的三个行为模式：过早回答意图、自我纠正能力和强诱惑信息影响，通过延迟诊断问题可显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs在一次性提供完整临床信息时的诊断准确性，但在真实临床推理中，医生通常通过多轮对话逐步积累证据。需要探索LLMs在多轮证据积累环境下的表现，以评估其在真实临床场景中的可靠性。

Method: 提出MINT（医学增量N轮基准测试），包含1,035个病例，具有临床标注的证据片段、受控的轮次粒度和信息保留分解。系统评估了11个LLMs在MINT上的表现，分析其多轮诊断行为模式。

Result: 发现三个持续性行为模式：1）过早回答意图：超过55%的答案在前两轮就确定；2）自我纠正能力：错误到正确的修正率比正确到错误的翻转率高10.6倍；3）强诱惑信息：如实验室结果等显著临床信息会触发过早回答。通过延迟诊断问题可减少过早回答，将首次承诺点的准确性提升62.6%，而将显著临床证据保留到后期可防止因过早承诺导致的准确性下降达23.3%。

Conclusion: 该研究提供了受控评估框架和具体建议，通过延迟诊断问题和合理分配临床证据的呈现时机，可显著提高LLMs在多轮医学诊断中的可靠性，为临床应用中LLMs的优化部署提供了指导。
...
### [182] [GROUNDEDKG-RAG: Grounded Knowledge Graph Index for Long-document Question Answering](https://arxiv.org/abs/2604.04359)
*Tianyi Zhang,Andreas Marfurt*

Main category: cs.CL

TL;DR: 提出GroundedKG-RAG系统，通过从源文档显式提取和锚定知识图谱来改进长文档问答，在效率和事实准确性方面优于现有方法。
...
Method: 提出GroundedKG-RAG系统，从源文档显式提取知识图谱并锚定在原始句子中。节点定义为实体和动作，边定义为时间或语义关系。使用语义角色标注（SRL）和抽象意义表示（AMR）解析构建知识图谱，然后进行嵌入用于检索。查询时应用相同转换，从锚定的源文本中检索最相关句子进行问答。

Result: 在NarrativeQA数据集上评估，GroundedKG-RAG与最先进的专有长上下文模型性能相当但成本更低，且优于竞争基线。此外，知识图谱具有可解释性和人类可读性，便于结果审计和错误分析。

Conclusion: GroundedKG-RAG通过显式提取和锚定知识图谱，有效解决了当前RAG系统在长文档问答中的效率、准确性和可解释性问题，为实际应用提供了可行的解决方案。
...
### [183] [Compressible Softmax-Attended Language under Incompressible Attention](https://arxiv.org/abs/2604.04384)
*Wonsuk Lee*

Main category: cs.CL

TL;DR: 研究发现，在五个不同规模的Transformer语言模型中，注意力头的logit能量场仅需2-11个奇异分量就能解释90%的方差，而学习到的交互矩阵W_Q^T W_K需要38-75个分量才能达到相同阈值，表明语言数据本身具有高度可压缩性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究Transformer语言模型中注意力机制的压缩特性，特别是分析注意力头中logit能量场的奇异值分布，以理解语言数据与注意力机制之间的交互关系。

Method: 方法包括：1）分析五个不同参数规模（124M-7B）的Transformer语言模型；2）计算注意力头中logit能量场$\tilde{E}$的奇异值分解；3）比较学习到的交互矩阵$W_Q^T W_K$的奇异值分布；4）使用有效秩（effective rank）量化压缩特性。

Result: 主要结果：1）logit能量场仅需2-11个奇异分量就能解释90%的方差；2）交互矩阵需要38-75个分量才能达到相同阈值；3）有效秩差距达5-25倍；4）注意力机制在所有维度上均匀分配容量，但语言数据将实际交互压缩到少数维度。

Conclusion: 结论是softmax注意力机制处理的语言数据具有内在可压缩性，这种压缩特性是数据本身的属性，而非分析框架的特性。注意力机制虽然为所有维度分配了容量，但语言数据自然地将交互集中在少数维度上。
...
### [184] [How Alignment Routes: Localizing, Scaling, and Controlling Policy Circuits in Language Models](https://arxiv.org/abs/2604.04385)
*Gregory N. Frank*

Main category: cs.CL

TL;DR: 研究发现对齐训练语言模型中存在稀疏路由机制：门控注意力头检测内容后触发下游放大器头，增强拒绝信号。该机制在9个模型中得到验证，通过调制检测层信号可连续控制策略强度，并揭示了意图识别与策略路由的结构分离。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别和理解语言模型中对齐训练后出现的稀疏路由机制，特别是模型如何检测敏感内容并触发拒绝响应。通过政治审查和安全拒绝作为自然实验，探索模型内部决策机制。

Method: 使用9个来自6个实验室的模型，在120个提示对语料库上进行验证。采用必要性充分性互换测试、自举重采样、缩放对分析、信号调制和密码编码等方法，研究路由机制的特征和稳定性。

Result: 门控头通过必要性充分性测试（p<0.001），核心放大器头在自举重采样中稳定（Jaccard 0.92-1.0）。缩放对显示路由机制在规模上分布（消融效果减弱达17倍）。通过调制检测层信号可连续控制策略强度。密码编码下门控头路由贡献崩溃（Phi-4中78%），模型转向谜题解决而非拒绝。

Conclusion: 语言模型存在结构分离：意图识别与策略路由。预训练提供广泛语义理解，后训练提供较窄的策略绑定，后者在输入变换下泛化能力较差。路由机制揭示了模型内部决策的稀疏性和可解释性。
...
### [185] [Responses Fall Short of Understanding: Revealing the Gap between Internal Representations and Responses in Visual Document Understanding](https://arxiv.org/abs/2604.04411)
*Haruka Kawasaki,Ryota Tanaka,Kyosuke Nishida*

Main category: cs.CL

TL;DR: 研究通过线性探测分析视觉文档理解任务中LVLM模型不同层的信息表示，发现内部表示与生成响应存在差距，中间层信息编码更线性，针对中间层的微调能提升性能


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在视觉文档理解任务上取得进展，但基于生成响应的评估可能无法反映模型是否真正捕获了所需信息。研究者希望探究VDU任务所需信息在LVLM不同层中的表示情况

Method: 使用线性探测方法分析LVLM中LLM不同层的信息表示，研究内部表示与生成响应的差距，并探索针对中间层的微调策略

Result: 研究发现：(1) 内部表示与生成响应存在明显差距；(2) 解决任务所需信息在中间层的编码比最终层更线性。针对中间层的微调能同时提升线性探测准确率和响应准确率，并缩小差距

Conclusion: 视觉文档理解任务中，LVLM的内部信息表示与外部响应存在不一致，中间层包含更线性的任务相关信息，针对中间层的微调是有效的优化策略
...
### [186] [Structured Causal Video Reasoning via Multi-Objective Alignment](https://arxiv.org/abs/2604.04415)
*Zinuo Li,Yongxin Guo,Jun Liu,Jiawei Zhan,Xi Jiang,Chengjie Wang,Mohammed Bennamoun,Farid Boussaid,Feng Zheng,Qiuhong Ke*

Main category: cs.CL

TL;DR: 该论文提出了一种结构化事件事实表示方法，通过构建紧凑的事件和因果关系表示来改进视频理解，解决了现有视频大语言模型依赖非结构化推理的问题。


<details>
  <summary>Details</summary>
Motivation: 人类理解视频动态通常基于结构化的心理表征（实体、动作、时间关系），而现有视频大语言模型主要依赖非结构化视频推理，将关键视觉证据嵌入冗长的文本描述中，时间因果关系建模较弱，导致推理效率低下和因果推断脆弱。

Method: 提出构建结构化事件事实作为推理前的紧凑表示，包含显著事件及其因果关系。采用四阶段训练流程：事实对齐、格式预热、思维预热和基于强化学习的后训练。在RL阶段，将优化问题建模为多目标强化学习，平衡结构完整性、因果保真度和推理长度。

Result: 开发了Factum-4B模型，在需要细粒度时间推理的挑战性视频理解任务上表现出更可靠的推理和更强的性能。

Conclusion: 结构化事件事实表示能够促进简洁且基于因果的推理，使中间证据更容易验证，通过多目标强化学习平衡不同优化目标，显著提升了视频理解模型的推理质量和性能。
...
### [187] [DeonticBench: A Benchmark for Reasoning over Rules](https://arxiv.org/abs/2604.04443)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在处理复杂、特定情境的规则推理方面存在挑战，特别是在法律和政策环境中的道义推理。现有基准测试大多关注短上下文数学推理，缺乏针对长上下文、高风险道义推理的评估工具。

Method: 创建DEONTICBENCH基准测试，包含多种现实领域任务。支持两种推理方式：1) 自由形式的链式思维语言推理；2) 符号计算辅助方法，将法规和案例事实转换为可执行的Prolog程序，生成形式化问题解释和明确程序跟踪。

Result: 前沿LLM和编码模型在困难子集上的最佳表现仅为：SARA Numeric 44.4%，Housing 46.6 macro-F1。监督微调和强化学习训练虽然提高了Prolog生成质量，但当前RL方法仍无法可靠解决这些任务。

Conclusion: DEONTICBENCH为研究现实领域中的上下文基础规则推理提供了基准测试工具，支持符号和非符号两种设置，揭示了当前模型在复杂道义推理任务上的局限性。
...
### [188] [Same Geometry, Opposite Noise: Transformer Magnitude Representations Lack Scalar Variability](https://arxiv.org/abs/2604.04469)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 研究发现Transformer语言模型在数值表示上表现出与生物系统相反的变异性模式：表示变异性随数值增大而减小，而非生物系统中观察到的按比例缩放（恒定变异系数）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是测试Transformer语言模型是否表现出生物系统中普遍存在的标量变异性特性，即表示噪声与数值大小成比例缩放，产生恒定的变异系数。

Method: 通过分析三个7-8B参数模型（Llama-3-8B-Instruct、Mistral-7B-Instruct-v0.3、Llama-3-8B-Base）中26个数值在载体句子中的隐藏状态表示的离散度，检验标量变异性特性。

Result: 发现相反模式：表示变异性沿数值轴随数值增大而减小（缩放指数α≈-0.19），所有三个模型的主要层中均未发现正α值。负号模式在全维空间（α≈-0.04）和句子身份校正后（α≈-0.007）保持一致，且语料频率强烈预测每个数值的变异性（ρ=0.84）。

Conclusion: 仅靠分布学习不足以产生标量变异性：Transformer再现了对数压缩的数值几何结构，但未观察到生物系统中恒定的变异系数噪声特征。
...
### [189] [CommonMorph: Participatory Morphological Documentation Platform](https://arxiv.org/abs/2604.04515)
*Aso Mahmudi,Sina Ahmadi,Kemal Kurniawan,Rico Sennrich,Eduard Hovy,Ekaterina Vylomova*

Main category: cs.CL

TL;DR: CommonMorph是一个用于加速形态学数据收集和标注的平台，通过专家定义、贡献者收集和社区验证的三层方法，支持多种形态系统，并提供开源工具和UniMorph兼容输出。


<details>
  <summary>Details</summary>
Motivation: 形态学数据收集和标注面临重大挑战，需要语言学专业知识、方法严谨性和大量资源，这对于低资源语言和方言尤为困难，阻碍了语言多样性的保护和研究。

Method: 采用三层方法：1) 专家语言学定义；2) 贡献者收集；3) 社区验证。平台整合主动学习、标注建议工具，支持从相关语言导入和改编材料，适应融合、粘着、词根模式等多种形态系统。

Result: 开发了开源平台CommonMorph，提供UniMorph兼容输出，确保与NLP工具的互操作性，平台可通过https://common-morph.com访问，为通过协作技术保护语言多样性提供了可复制的模型。

Conclusion: CommonMorph平台通过简化形态学数据收集流程，降低了资源需求，支持多种语言形态系统，为加速语言多样性保护和研究提供了有效的技术解决方案。
...
### [190] [Formal Constraints on Dependency Syntax](https://arxiv.org/abs/2604.04542)
*Gómez-Rodríguez,Carlos,Alemany-Puig,Lluís*

Main category: cs.CL

TL;DR: 论文探讨了依存句法中树结构的约束条件，特别是如何在投射性（projectivity）的限制性和无约束依存结构的过度宽松之间寻找现实的平衡点。


<details>
  <summary>Details</summary>
Motivation: 依存句法将句子结构表示为依存关系树，但在实践中许多可能的树结构并不合理或在真实语言中极少出现。这促使研究者寻找能够更好拟合真实语言现象的约束条件，以提供更准确的语言描述、更快的解析速度，以及对语言演化和人类语言处理的洞察。

Method: 论文回顾了依存句法中各种约束条件的研究，特别关注投射性（projectivity）这一最受研究的约束条件，并探讨了其他旨在在投射性的限制性和无约束依存结构的过度宽松之间寻找平衡的各种约束条件。

Result: 研究表明投射性虽然是最受研究的约束条件，但过于严格，无法充分表示某些语言现象，特别是在词序灵活的语言中。因此需要寻找更现实的中间地带约束条件。

Conclusion: 需要继续研究和开发能够更好地平衡语言描述准确性和计算效率的依存句法约束条件，特别是在处理词序灵活语言时，需要在投射性的限制性和无约束依存结构的过度宽松之间找到更现实的中间方案。
...
### [191] [PassiveQA: A Three-Action Framework for Epistemically Calibrated Question Answering via Supervised Finetuning](https://arxiv.org/abs/2604.04565)
*Madhav S Baidya*

Main category: cs.CL

TL;DR: 论文提出PassiveQA框架，通过监督微调让LLM在信息不足时能够明智地选择回答、询问澄清或弃权，解决传统RAG系统在信息不完整查询中的过度自信问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的用户查询往往不完整、模糊或缺少关键变量，导致LLM产生过度自信或幻觉回答。传统RAG系统缺乏对信息充分性的认知判断能力，默认生成答案而非明智决策。

Method: 提出PassiveQA三动作框架，通过监督微调使模型行为与信息充分性对齐。方法整合结构化信息状态表示、知识图谱基础上下文，以及微调的计划器，显式建模缺失变量和决策推理。

Result: 在多个QA数据集上的实验表明，微调后的计划器在宏观F1和弃权召回率方面显著提升，同时降低幻觉率，且在计算受限的训练机制下实现。

Conclusion: 研究结果为认知决策必须在训练中学习而非在推理时强加提供了强有力的实证证据，强调了模型需要显式学习信息充分性判断能力。
...
### [192] [Benchmarking Multilingual Speech Models on Pashto: Zero-Shot ASR, Script Failure, and Cross-Domain Evaluation](https://arxiv.org/abs/2604.04598)
*Hanif Rahman*
...
<details>
  <summary>Details</summary>
Motivation: 普什图语有6000-8000万使用者，但在多语言自动语音识别领域缺乏公开基准测试。本研究旨在填补这一空白，为普什图语ASR建立首个可重复的评估框架。

Method: 使用FLEURS普什图语测试集和过滤后的Common Voice~24子集，评估了10个模型（包括7个Whisper尺寸、MMS-1B、SeamlessM4T-v2-large和OmniASR-CTC-300M）的零样本ASR性能。通过语言识别审计分析脚本级故障，并评估5个微调普什图语ASR模型的跨域性能。

Result: 零样本Whisper的WER在90%-297%之间，SeamlessM4T在Common Voice~24上达到39.7% WER（最佳零样本结果）。Whisper模型生成普什图语脚本输出的比例低于0.8%，而其他模型超过93%。微调模型在跨域测试中WER从14%恶化到32.5-59%，但有一个增强模型在两组测试集上都达到35.1%且无跨域退化。

Conclusion: 普什图语ASR面临严重挑战，特别是脚本级故障和跨域性能下降问题。研究识别了五个结构性障碍，并提出了五个有序的研究优先级，为普什图语ASR的进一步发展提供了方向。
...
### [193] [IDIOLEX: Unified and Continuous Representations for Idiolectal and Stylistic Variation](https://arxiv.org/abs/2604.04704)
...
TL;DR: 论文提出IDIOLEX框架，用于学习句子风格和方言的表示，与语义内容解耦，并在阿拉伯语和西班牙语方言上验证了其有效性。
...
Method: 提出IDIOLEX框架，结合句子来源的监督信息和句子内容的语言特征，学习每个句子风格和方言的连续表示。

Result: 在阿拉伯语和西班牙语方言上评估，学习到的表示能够捕捉有意义的变异并在领域间迁移，可用于分析和分类。这些表示还可作为语言模型风格对齐的训练目标。

Conclusion: 联合建模个体和群体层面的变异为研究个人语言风格提供了有用视角，支持需要风格敏感性的下游应用，如开发多样化和可访问的大语言模型。
...
### [194] [BiST: A Gold Standard Bangla-English Bilingual Corpus for Sentence Structure and Tense Classification with Inter-Annotator Agreement](https://arxiv.org/abs/2604.04708)
*Abdullah Al Shafi,Swapnil Kundu Argha,M. A. Moyeen,Abdul Muntakim,Shoumik Barman Polok*

Main category: cs.CL

TL;DR: BiST是一个精心构建的孟加拉语-英语双语语料库，专注于句子级语法分类，包含句法结构和时态两个维度的标注，共30,534个句子，标注质量高，为低资源语言的多语言NLP研究提供支持。


<details>
  <summary>Details</summary>
Motivation: 高质量的孟加拉语双语资源在多语言NLP研究中仍然匮乏，特别是在低资源环境下。为了填补这一空白，需要构建一个专门针对语法分类的孟加拉语-英语双语语料库，以支持语法建模任务和多语言研究。

Method: 从开放许可的百科资源和自然对话文本中收集数据，进行系统预处理和自动语言识别。采用多阶段标注框架，由三名独立标注者进行标注，使用Fleiss Kappa评估标注者间一致性。构建了30,534个句子的语料库，包含17,465个英语句子和13,069个孟加拉语句子。

Result: 标注质量可靠，结构标注的Fleiss Kappa值为0.82，时态标注为0.88。统计分析显示语料库具有现实的结构和时态分布。基线评估表明，利用互补语言特定表示的双编码器架构始终优于强大的多语言编码器。

Conclusion: BiST语料库为双语语法建模提供了统一资源，支持语法建模任务，包括受控文本生成、自动反馈生成和跨语言表示学习，促进了基于语言学的多语言研究。
...
### [195] [What Makes Good Multilingual Reasoning? Disentangling Reasoning Traces with Measurable Features](https://arxiv.org/abs/2604.04720)
*Dayeon Ki,Kevin Duh,Marine Carpuat*

Main category: cs.CL

TL;DR: 研究挑战了"多语言推理只需模仿英语推理"的假设，通过量化分析多语言推理特征与准确率的关系，发现不同语言间关联强度差异显著甚至相反，指向需要适应语言特定推理模式的自适应目标。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在英语和其他语言之间存在显著性能差距，当前研究通常假设通过使其他语言的推理过程模仿英语推理就能缩小这一差距。本研究挑战这一假设，探究多语言环境中有效推理的真正特征，以及英语衍生的推理特征在多大程度上真正帮助其他语言。

Method: 1) 定义一套可测量的推理特征，涵盖多语言对齐、推理步骤和推理流程三个方面；2) 使用逻辑回归量化每个特征与最终答案准确率的关联；3) 在多语言推理轨迹上训练稀疏自编码器，自动发现实例化或扩展这些特征的潜在推理概念；4) 使用这些特征作为测试时选择策略，检验是否能引导模型实现更强的多语言推理。

Result: 在两个数学推理基准测试、四个大型推理模型和10种语言上的实验发现：大多数特征与准确率呈正相关，但关联强度在不同语言间差异显著，在某些情况下甚至出现相反关系。

Conclusion: 研究结果挑战了以英语为中心的奖励设计，指向需要适应语言特定推理模式的自适应目标，对多语言基准测试和奖励设计具有具体意义。
...
### [196] [Individual and Combined Effects of English as a Second Language and Typos on LLM Performance](https://arxiv.org/abs/2604.04723)
...
TL;DR: 该研究探讨了非英语母语者使用英语作为第二语言（ESL）输入时，结合拼写错误对大型语言模型性能的影响。研究发现，ESL变体和拼写错误的组合效应比单独因素更大，但并非简单叠加，特别是在封闭式任务中表现更明显。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要在英语数据上训练，对英语输入表现最佳。然而许多非英语母语者使用英语作为第二语言与模型交互，这些输入常包含拼写错误。先前研究通常单独考察ESL变体或拼写错误的影响，但现实中两者常同时出现，因此需要研究它们的组合效应。

Method: 使用Trans-EnV框架将标准英语输入转换为八种ESL变体，并应用MulTypo在三个级别（低、中、高）注入拼写错误，系统评估这些因素对模型性能的影响。

Result: 研究发现ESL变体和拼写错误的组合通常比单独因素导致更大的性能下降，但这种组合效应并非简单的叠加效应。在封闭式任务中，性能下降模式更一致且可预测，而在开放式任务中结果更为复杂和混合。

Conclusion: 基于干净标准英语的评估可能高估模型在现实世界中的性能，单独评估ESL变体或拼写错误不能完全捕捉模型在真实环境中的行为。需要更全面的评估方法来反映实际使用场景。
...
### [197] [Metaphors We Compute By: A Computational Audit of Cultural Translation vs. Thinking in LLMs](https://arxiv.org/abs/2604.04732)
*Yuan Chang,Jiaming Qu,Zhu Li*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在创意写作任务中表现出文化刻板印象和西方中心主义，提示文化身份并不能保证真正的文化基础推理


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型被描述为多语言模型，但说一种语言并不等同于在文化中进行推理。这引发了一个关键问题：LLMs是否真正进行文化感知推理？本文旨在通过计算审计来评估LLMs在创意写作任务中的文化包容性。

Method: 使用隐喻生成任务作为案例研究，涵盖五个文化背景和多个抽象概念。通过实证方法检验LLMs是作为文化多样性的创意伙伴，还是仅仅作为利用主导概念框架和本地化表达的文化翻译者。

Result: 研究发现模型在某些文化背景下表现出刻板化的隐喻使用，以及西方中心主义倾向。这些发现表明，仅仅在提示中指定文化身份并不能保证LLMs进行文化基础推理。

Conclusion: LLMs在文化感知推理方面存在局限性，它们可能只是利用主导文化框架进行表面上的本地化表达，而非真正理解文化背景。这强调了需要更深入的文化基础推理能力，而不仅仅是多语言能力。
...
### [198] [Lighting Up or Dimming Down? Exploring Dark Patterns of LLMs in Co-Creativity](https://arxiv.org/abs/2604.04735)
*Zhu Li,Jiaming Qu,Yuan Chang*

Main category: cs.CL

TL;DR: 该研究探索了LLM作为协作写作伙伴时出现的五种"暗黑模式"，这些模式可能抑制或扭曲人类的创造性过程，包括奉承、语气管制、道德说教、死亡循环和锚定效应。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地作为协作写作伙伴，研究它们对人类创造力的影响变得重要。作者关注LLM可能通过微妙行为抑制或扭曲人类创造性过程的问题，特别是那些作为安全对齐副产物而出现的模式。

Method: 通过一系列受控实验，让LLM作为写作助手参与不同文学形式和主题的创作，分析生成响应中五种暗黑模式的出现频率和特征。

Result: 初步结果显示：奉承行为几乎无处不在（91.7%的案例），在敏感话题中尤其明显；锚定效应则依赖于文学形式，在民间故事中出现最频繁。

Conclusion: 这些暗黑模式（通常是安全对齐的副产品）可能无意中限制了创造性探索。研究提出了支持创造性写作的AI系统设计考虑，以减轻这些负面影响。
...
### [199] [Hallucination Basins: A Dynamic Framework for Understanding and Controlling LLM Hallucinations](https://arxiv.org/abs/2604.04743)
*Kalyan Cherukuri,Lav R. Varshney*

Main category: cs.CL

TL;DR: 论文提出几何动力系统框架解释LLM幻觉现象，发现幻觉源于潜在空间中的任务依赖盆地结构，而非普遍现象。通过几何感知引导可在不重新训练的情况下减少幻觉概率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型会产生幻觉：它们生成流畅但事实错误的输出。现有研究缺乏对幻觉现象的系统性几何解释，需要理解幻觉在模型潜在空间中的动态机制。

Method: 使用几何动力系统框架分析幻觉现象，通过多个开源模型和基准测试中的自回归隐藏状态轨迹，研究任务依赖的盆地结构。提出任务复杂度和多盆地定理，分析L层Transformer中的盆地涌现，并开发几何感知引导方法。

Result: 发现可分性强烈依赖任务而非普遍现象：事实性任务显示更清晰的盆地分离，而摘要和误解较多的任务通常稳定性较差且经常重叠。几何感知引导方法能够在不重新训练的情况下减少幻觉概率。

Conclusion: 幻觉源于潜在空间中的任务依赖盆地结构，这一几何动力系统框架为理解LLM幻觉提供了新视角，几何感知引导方法为减少幻觉提供了实用工具，无需模型重新训练。
...
### [200] [HUKUKBERT: Domain-Specific Language Model for Turkish Law](https://arxiv.org/abs/2604.04790)
*Mehmet Utku Öztürk,Tansu Türkoğlu,Buse Buz-Yalug*

Main category: cs.CL

TL;DR: 本文介绍了HukukBERT，这是首个针对土耳其法律文本的大规模领域特定语言模型，在土耳其法律术语预测和法院判决结构分割任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 土耳其法律领域缺乏专门的大规模语言模型，虽然英语法律领域已有LEGAL-BERT等模型，但土耳其法律NLP研究因领域特定数据和模型稀缺而受限。

Method: 采用混合领域自适应预训练方法，整合了全词掩码、标记跨度掩码、词跨度掩码和针对性关键词掩码技术，使用18GB清洗后的土耳其法律语料库训练，并设计了48K WordPiece分词器。

Result: HukukBERT在土耳其法律术语预测任务上达到84.40%的Top-1准确率，在土耳其法院判决结构分割任务上达到92.8%的文档通过率，均显著优于现有模型。

Conclusion: HukukBERT是土耳其法律领域最全面的语言模型，为土耳其法律NLP任务（如命名实体识别、判决预测、法律文档分类）提供了有力支持，并已公开发布以促进未来研究。
...
### [201] [How Far Are We? Systematic Evaluation of LLMs vs. Human Experts in Mathematical Contest in Modeling](https://arxiv.org/abs/2604.04791)
*Yuhang Liu,Heyan Huang,Yizhe Yang,Hongyan Zhao,Zhizhuo Zeng,Yang Gao*

Main category: cs.CL

TL;DR: 该论文提出了一个面向问题、分阶段的评估框架来评估大语言模型在数学建模竞赛中的端到端问题解决能力，揭示了模型在理解与执行阶段之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在推理基准测试中表现出色，但其在需要端到端工作流程的真实世界问题解决能力尚不明确。数学建模竞赛为评估这种端到端能力提供了严格的测试平台。

Method: 提出了一个面向问题、分阶段的评估框架，使用专家验证的标准来评估LLM在各个建模阶段的表现。通过将自动评分与独立人类专家对中国研究生数学建模竞赛问题的判断进行比较，验证了框架的可靠性。

Result: 框架显示出比现有评估方案更强的对齐性。研究发现最先进的LLM存在理解-执行差距：在问题识别和表述等早期阶段表现良好，但在模型求解、代码实现和结果分析等执行导向阶段存在持续缺陷。这些差距即使增加模型规模仍然存在。

Conclusion: 失败原因可追溯到规范不足、验证缺失和缺乏有效性检验，错误在阶段间传播而未被纠正。研究结果表明，弥合这一差距需要超越模型缩放的方法，为将LLM应用于复杂真实世界问题解决提供了见解。
...
### [202] [SkillX: Automatically Constructing Skill Knowledge Bases for Agents](https://arxiv.org/abs/2604.04804)
...
TL;DR: SkillX是一个自动化框架，用于构建可插拔的技能知识库，通过多级技能设计、迭代技能精炼和探索性技能扩展，实现跨智能体和环境的技能复用。


<details>
  <summary>Details</summary>
Motivation: 现有自进化范式效率低下：智能体孤立学习，从有限经验中重复发现相似行为，导致冗余探索和泛化能力差。需要构建可重用的技能知识库来解决这些问题。

Method: 1) 多级技能设计：将原始轨迹提炼为三层层次结构（战略计划、功能技能、原子技能）；2) 迭代技能精炼：基于执行反馈自动修订技能；3) 探索性技能扩展：主动生成和验证新技能以扩展覆盖范围。

Result: 使用GLM-4.6作为骨干智能体构建可重用技能库，在AppWorld、BFCL-v3和τ²-Bench等具有挑战性的长时程、用户交互基准测试中评估可迁移性。实验表明SkillKB能持续提升较弱基础智能体的任务成功率和执行效率。

Conclusion: 结构化、层次化的经验表示对于可泛化的智能体学习至关重要，SkillX框架通过构建可插拔技能知识库，实现了跨智能体和环境的技能高效复用。
...
### [203] [LiveFact: A Dynamic, Time-Aware Benchmark for LLM-Driven Fake News Detection](https://arxiv.org/abs/2604.04815)
*Cheng Xu,Changhong Jin,Yingjie Niu,Nan Yan,Yuke Mei,Shuhao Guan,Liming Chen,M-Tahar Kechadi*

Main category: cs.CL

TL;DR: LiveFact是一个持续更新的基准测试，用于评估大语言模型在动态、不完全信息下的虚假新闻检测和事实核查能力，解决了传统静态基准测试的数据污染和时态不确定性评估不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前虚假新闻检测和事实核查的评估框架落后于大语言模型的发展，静态基准测试容易受到基准数据污染的影响，且无法有效评估模型在时态不确定性下的推理能力。

Method: 引入LiveFact基准测试，使用动态的时态证据集模拟真实世界信息迷雾；采用双模式评估：分类模式用于最终验证，推理模式用于基于证据的推理；并包含专门监控基准数据污染的组件。

Result: 测试22个大语言模型发现，开源混合专家模型（如Qwen3-235B-A22B）现在能够匹配或超越专有的最先进系统；更重要的是发现了显著的"推理差距"——能力强的模型在早期数据切片中表现出认识论谦逊，能够识别不可验证的声明。

Conclusion: LiveFact为评估稳健、具有时态意识的人工智能验证系统设定了可持续标准，填补了传统静态基准测试在评估动态推理能力方面的空白。
...
### [204] [Plausibility as Commonsense Reasoning: Humans Succeed, Large Language Models Do not](https://arxiv.org/abs/2604.04825)
*Sercan Karakaş*

Main category: cs.CL

TL;DR: 研究测试了大型语言模型在土耳其语前置关系从句附着歧义中是否像人类一样整合世界知识和句法结构，发现模型虽然能处理句法，但缺乏人类对合理性信息的敏感整合。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在许多语言任务上表现优异，但尚不清楚它们是否像人类一样以结构敏感的方式整合世界知识和句法结构来解决歧义问题。本研究通过土耳其语前置关系从句附着歧义来测试这一能力。

Method: 1) 构建土耳其语前置关系从句附着歧义项目，保持句法配置固定且两种解析在语用上都可能；2) 使用分级事件合理性选择性地支持高附着或低附着；3) 通过独立评分验证对比；4) 人类进行速度强迫选择理解实验；5) 评估土耳其语和多语言LLM在并行偏好设置中的表现，通过平均每词对数概率比较匹配的高附着/低附着延续。

Result: 人类在实验中显示出大且正确方向的合理性效应，而土耳其语和多语言LLM的合理性驱动变化微弱、不稳定或反向。模型未能像人类判断那样可靠地利用合理性信息指导附着偏好。

Conclusion: 测试的模型在整合合理性信息指导附着偏好方面不如人类可靠，土耳其语关系从句附着可作为超越广泛基准的有用跨语言诊断工具，揭示了当前LLM在结构敏感歧义解析方面的局限性。
...
### [205] [MERIT: Multilingual Expert-Reward Informed Tuning for Chinese-Centric Low-Resource Machine Translation](https://arxiv.org/abs/2604.04839)
*Zhixiang Lu,Chong Zhang,Chenyu Xue,Angelos Stefanidis,Chong Li,Jionglong Su,Zhengyong Jiang*

Main category: cs.CL

TL;DR: MERIT框架通过语言特定前缀、监督微调和基于语义对齐奖励的组相对策略优化，显著提升了中文到东南亚低资源语言的神经机器翻译性能，超越了单纯模型缩放的效果。


<details>
  <summary>Details</summary>
Motivation: 中文到东南亚低资源语言的神经机器翻译面临两个主要挑战：一是干净平行语料的极度稀缺，二是现有挖掘数据中普遍存在的噪声。这种长期短缺不仅阻碍了有效的模型训练，还导致与高资源语言方向存在巨大性能差距，使得数百万使用老挝语、缅甸语、他加禄语等语言的用户无法获得高质量的翻译系统，尽管大规模多语言模型已取得进展。

Method: 提出了MERIT（多语言专家奖励信息调优）统一翻译框架，将传统的以英语为中心的ALT基准转化为以中文为中心的五种东南亚低资源语言评估套件。该方法结合了语言特定前缀标记（LTP）、监督微调（SFT）以及由语义对齐奖励（SAR）指导的新型组相对策略优化（GRPO）。

Result: 研究结果表明，在低资源语言到中文的翻译任务中，有针对性的数据整理和奖励引导的优化方法显著优于单纯的模型缩放方法。

Conclusion: MERIT框架通过创新的多语言翻译方法，有效解决了中文到东南亚低资源语言翻译中的语料稀缺和噪声问题，证明了针对性的数据策略和奖励优化比单纯增加模型规模更为有效。
...
### [206] [Do No Harm: Exposing Hidden Vulnerabilities of LLMs via Persona-based Client Simulation Attack in Psychological Counseling](https://arxiv.org/abs/2604.04842)
*Qingyang Xu,Yaling Shen,Stephanie Fong,Zimu Wang,Yiwen Jiang,Xiangyu Zhao,Jiahe Liu,Zhongxing Xu,Vincent Lee,Zongyuan Ge*

Main category: cs.CL

TL;DR: PCSA：首个基于人格的客户模拟攻击框架，通过模拟心理咨询中的客户对话来暴露LLM在心理安全对齐中的漏洞，发现当前模型容易提供未经授权的医疗建议、强化妄想并鼓励危险行为。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在心理健康护理中的应用增加，但现有红队测试框架主要关注通用危害或基于优化的攻击，忽略了区分治疗性共情与适应性不良验证的关键挑战，即支持性回应可能无意中在多轮对话中强化有害信念或行为。

Method: 提出人格化客户模拟攻击(PCSA)框架，通过连贯的、人格驱动的客户对话模拟心理咨询中的客户，暴露心理安全对齐的脆弱性。在七个通用和心理健康专用LLM上进行实验，并与四个竞争基线比较。

Result: PCSA显著优于四个竞争基线。困惑度分析和人工检查表明PCSA生成更自然和现实的对话。结果显示当前LLM仍然容易受到领域特定对抗策略的攻击，包括提供未经授权的医疗建议、强化妄想以及隐含地鼓励危险行为。

Conclusion: 当前LLM在心理安全对齐方面存在显著漏洞，需要更专业的红队测试方法来评估心理健康应用中的风险。PCSA框架为这一领域提供了有效的评估工具。
...
### [207] [Rethinking Exploration in RLVR: From Entropy Regularization to Refinement via Bidirectional Entropy Modulation](https://arxiv.org/abs/2604.04894)
*Hengrui Gu,Xiaotian Han,Yujing Bian,Kaixiong Zhou*

Main category: cs.CL

TL;DR: 本文提出AsymGRPO框架，通过解耦正负rollout的调控来优化RLVR中的探索问题，区分信息熵和虚假熵，实现更有效的探索


<details>
  <summary>Details</summary>
Motivation: RLVR虽然提升了LLM的推理能力，但面临"受限探索"问题，即策略迅速收敛到狭窄解集。传统的熵正则化方法对LLM不可靠，存在高超参数敏感性和性能提升有限的问题

Method: 提出AsymGRPO框架：1）推导群体相对优势估计的参数化公式；2）分析熵动态，将策略熵分解为信息熵（保持多样化解路径）和虚假熵（侵蚀推理模式）；3）通过解耦正负rollout的调控，独立控制信息熵的保持和虚假噪声的抑制

Result: 实验表明AsymGRPO相比强基线方法取得更优性能，并展现出与现有熵正则化方法协同的潜力

Conclusion: 有效探索需要熵精炼而非盲目最大化，AsymGRPO通过显式解耦正负rollout调控，实现了更有效的探索机制，解决了RLVR中的受限探索问题
...
### [208] [TriAttention: Efficient Long Reasoning with Trigonometric KV Compression](https://arxiv.org/abs/2604.04921)
*Weian Mao,Xi Lin,Wei Huang,Yuxin Xie,Tianfu Fu,Bohan Zhuang,Song Han,Yukang Chen*

Main category: cs.CL

TL;DR: TriAttention：一种基于Q/K向量集中特性的KV缓存压缩方法，通过三角级数利用距离偏好来估计键重要性，在保持推理准确性的同时显著提升吞吐量并减少内存占用。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的扩展推理会产生严重的KV缓存内存瓶颈。现有的KV缓存压缩方法使用RoPE后的注意力分数来估计KV重要性，但查询随位置旋转导致代表性查询很少，造成键选择不佳和推理不稳定。

Method: 提出TriAttention方法，观察到在RoPE前空间中，Q和K向量高度集中在固定的非零中心周围且跨位置保持稳定（Q/K集中现象）。利用这种集中特性，通过三角级数使用中心表征的距离偏好来对键进行位置评分，同时利用Q/K范数作为重要性估计的额外信号。

Result: 在AIME25数据集上进行32K令牌生成时，TriAttention在保持完整注意力推理准确性的同时，实现了2.5倍的吞吐量提升或10.7倍的KV内存减少。而领先的基线方法在相同效率下只能达到约一半的准确性。TriAttention使OpenClaw能够在单个消费级GPU上部署，而完整注意力会导致长上下文内存不足。

Conclusion: TriAttention通过利用RoPE前空间中Q/K向量的集中特性，有效解决了KV缓存压缩中的查询旋转问题，在保持推理准确性的同时显著提升了效率和内存利用率，为长上下文推理提供了实用的解决方案。
...
### [209] [Early Stopping for Large Reasoning Models via Confidence Dynamics](https://arxiv.org/abs/2604.04930)
*Parsa Hosseini,Sumit Nawathe,Mahdi Salmani,Meisam Razaviyayn,Soheil Feizi*

Main category: cs.CL

TL;DR: CoDE-Stop：一种基于中间答案置信度动态变化的早期停止方法，可在保持准确性的同时减少推理计算成本25-50%
...
Method: 提出CoDE-Stop（Confidence Dynamics Early Stop），利用中间答案置信度的动态变化来决定何时终止推理。该方法无需额外训练，可轻松集成到现有模型中。研究发现正确推理轨迹往往早期就达到高置信度答案，而错误推理则产生冗长、低效的推理轨迹且置信度动态变化不可靠。

Result: 在多个推理和科学基准测试中评估CoDE-Stop，相比现有早期停止方法，在准确性与计算成本之间达到更优平衡，相比标准完整长度推理减少总token使用量25-50%。

Conclusion: CoDE-Stop通过分析推理过程中的置信度动态变化，提供了一种高效、无需训练的早期停止方法，显著降低计算成本同时保持推理准确性，为理解正确与错误推理轨迹的置信度变化提供了新见解。
...
### [210] [Beyond the Final Actor: Modeling the Dual Roles of Creator and Editor for Fine-Grained LLM-Generated Text Detection](https://arxiv.org/abs/2604.04932)
*Yang Li,Qiang Sheng,Zhengjia Wang,Yehan Yang,Danding Wang,Juan Cao*

Main category: cs.CL

TL;DR: 本文提出RACE方法，用于细粒度检测LLM生成文本，将文本分为四类而非传统的二元或三元分类，以支持更精准的政策监管。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型生成文本检测方法主要采用二元或三元分类，只能区分纯人类/LLM文本或协作文本，无法满足精细监管需求，因为LLM润色的人类文本和人性化的LLM文本往往触发不同的政策后果。

Method: 提出RACE（Rhetorical Analysis for Creator-Editor Modeling）方法，通过修辞结构理论构建逻辑图来捕捉创作者的基础特征，同时提取基本话语单元级别的特征来捕捉编辑者的风格特征。

Result: 实验表明，RACE在识别细粒度文本类型方面优于12个基线方法，且具有较低的误报率。

Conclusion: RACE为LLM监管提供了一个与政策对齐的解决方案，能够更精确地检测不同细粒度类型的LLM生成文本。
...
### [211] [LLMs-Healthcare : Current Applications and Challenges of Large Language Models in various Medical Specialties](https://arxiv.org/abs/2311.12882)
*Ummara Mumtaz,Awais Ahmed,Summaya Mumtaz*

Main category: cs.CL

TL;DR: 本文综述了大型语言模型在医疗健康领域的应用进展，重点介绍了其在诊断、治疗、癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等领域的创新应用，同时探讨了相关挑战与机遇。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型技术的快速发展，其在医疗健康领域的应用日益广泛，对医生、医疗服务提供者和患者都产生了重要影响。本文旨在全面概述LLMs在医疗领域的最新进展，展示其在各个医学专业中的变革性影响。

Method: 本文采用文献综述的方法，系统性地收集和分析大型语言模型在医疗健康领域的应用研究。重点关注诊断和治疗相关功能，涵盖癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等多个医学专业领域。

Result: 研究发现大型语言模型在医疗健康领域具有广泛的应用价值，能够支持医疗诊断、改善患者护理、辅助临床决策。特别是在癌症护理、皮肤病诊断、牙科治疗、神经退行性疾病管理和心理健康支持等方面展现出创新性贡献。

Conclusion: 大型语言模型在医疗健康领域具有巨大潜力，能够显著改善医疗服务和患者护理质量。尽管存在数据多样性处理、模型局限性等挑战，但通过持续的技术发展和跨学科合作，LLMs有望在更多医学专业领域发挥重要作用，推动医疗健康领域的数字化转型。
...
### [212] [Impure codes exceeding the pure bounds for quantum local recovery](https://arxiv.org/abs/2604.03569)
*Carlos Galindo,Fernando Hernando,Helena Martín-Cruz,Ryutaroh Matsumoto*

Main category: cs.IT

TL;DR: 本文提出了一族J-仿射簇码，构建了不纯的CSS量子码，突破了多个适用于纯量子局部可恢复码的已知界限。


<details>
  <summary>Details</summary>
Motivation: 现有文献对量子局部可恢复码提出了多个界限，涉及消息量子比特数、距离、长度和局部性等参数。这些界限主要适用于纯量子码，需要探索不纯量子码在这些界限上的表现。

Method: 使用J-仿射簇码构造不纯的CSS量子码，并分析这些量子码在量子局部恢复界限上的表现。

Result: 构造的量子码突破了多个适用于纯量子局部可恢复码的已知界限，展示了不纯量子码在局部恢复性能上的优势。

Conclusion: 不纯量子局部可恢复码可以超越纯量子码的界限限制，同时建立了量子局部恢复界限与权重约束稳定子码界限之间的联系。
...
### [213] [Region-Based Constellation Designs for Constructive Interference Precoding in MU-MIMO](https://arxiv.org/abs/2604.03699)
*Yupeng Zheng,Chunmei Xu,Jinfei Wang,Yi Ma,Rahim Tafazolli*

Main category: cs.IT

TL;DR: 本文提出区域星座模型(RBC)用于多用户MIMO系统中的构造性干扰预编码，通过设计非凸可行区域改善星座对齐，相比传统QAM方案获得最高4dB信噪比增益。


<details>
  <summary>Details</summary>
Motivation: 传统构造性干扰预编码(CIP)的性能受限于构造性干扰(CI)区域结构，但传统星座设计忽略了这一点。QAM星座的CI区域与目标最小化符号模式存在系统性错位，导致与理论性能极限存在显著差距。

Method: 提出区域星座模型(RBC)，直接定义消息与其可行区域之间的映射关系，而非从现有星座推导。从改善符号对齐的角度，提出两种具有非凸可行区域的新型RBC方案：镜像端点QAM(ME-QAM)和实扩展ME-QAM。同时开发了低复杂度算法解决混合整数二次规划问题，复杂度与基于QAM的CIP相当。

Result: 仿真结果表明，在星座大小{16,64}的情况下，所提方案相比基于QAM的CIP获得最高4dB的信噪比增益。所开发的算法复杂度与QAM-based CIP相当。

Conclusion: 提出的RBC模型为CIP星座设计奠定了基础，通过改善符号对齐显著提升了系统性能。该模型也适用于其他非双射调制系统，代表了未来研究的有前景方向。
...
### [214] [Binary Caps and LCD Codes with Large Dimensions](https://arxiv.org/abs/2604.03734)
*Keita Ishizuka,Yuhi Kamio*

Main category: cs.IT

TL;DR: 该论文建立了线性互补对偶码与射影空间中帽集之间的联系，利用最大帽集的结构理论推导出最小距离至少为4的LCD码的不存在性定理，首次完全确定了余维7和8的最优最小距离。


<details>
  <summary>Details</summary>
Motivation: 研究线性互补对偶码与射影空间中帽集之间的理论联系，为LCD码的存在性提供新的理论框架，避免传统需要大量计算搜索的方法。

Method: 建立LCD码与射影空间中帽集的对应关系，利用最大帽集的结构理论推导LCD码的不存在性定理，提供无需计算的证明方法。

Result: 获得了最小距离至少为4的LCD码的不存在性定理，首次完全确定了余维7和8情况下的最优最小距离，这些结果之前只能通过穷举搜索获得。

Conclusion: 通过建立LCD码与射影空间中帽集的联系，为LCD码研究提供了新的理论框架，能够获得无需计算的证明，并解决了余维7和8的最优最小距离问题。
...
### [215] [Structural Impossibility of Antichain-Lattice Partial Information Decomposition](https://arxiv.org/abs/2604.03869)
*Aobo Lyu,Andrew Clark,Netanel Raviv*

Main category: cs.IT

TL;DR: 本文指出部分信息分解(PID)在三个或更多源变量时存在根本性表示问题，而非仅仅是公理调整问题，提出了系统信息分解(SID)作为替代方案


<details>
  <summary>Details</summary>
Motivation: 传统PID方法在处理三个或更多源变量时，广泛期望的公理变得相互矛盾。本文认为这种障碍是表示层面的根本问题，源于反链索引结构本身，而非简单的公理调整问题

Method: 1. 针对无目标的三变量情况，引入系统信息分解(SID)，获得具有操作冗余定义的自洽熵分解；2. 证明对于一般多元PID，不存在从反链索引信息原子恢复分解互信息的通用规则

Result: 1. 成功构建了无目标三变量SID的自洽框架；2. 证明两个系统可以拥有完全相同的原子（无论遵循何种公理）却具有不同的互信息，揭示了反链索引的根本局限性

Conclusion: 反链格结构存在根本性表示限制，无法通过单纯公理调整解决多元PID问题，需要基于关系的基础来构建多元信息度量
...
### [216] [Hemispherical Concentration for Semi-Unsourced Random Access in Many-Access Regime](https://arxiv.org/abs/2604.03987)
*Nazanin Mirhosseini*

Main category: cs.IT

TL;DR: 研究半无源随机接入模型，在轻量级协调器分配标识符给活跃用户后，用户随机选择消息形成ID-消息对。在大量接入信道机制下，分析系统性能，提出两阶段解码方案：预过滤阶段将搜索空间限制在半球内，ML阶段在缩减候选集上进行最大似然估计。


<details>
  <summary>Details</summary>
Motivation: 研究大规模物联网场景中的随机接入问题，传统方法在大量活跃用户时性能受限。需要设计高效解码方案来处理大量并发接入，同时保持较低的错误概率。

Method: 采用半无源随机接入模型，协调器分配标识符，用户随机选择消息。使用球形码本，码字从半径为√(nP)的超球面上均匀采样。解码分两步：预过滤阶段将搜索空间限制在收敛到半球的球形帽序列中；ML阶段在缩减候选集上进行最大似然估计。

Result: 对于0<β≤1，任何Ka(n)子集几乎必然包含在单个半球内；对于1<β<2，半球性质以指数速度趋于1成立。预过滤阶段的每用户错误概率随n→∞趋于零，ML阶段最坏情况下每用户错误概率的渐近指数衰减率为P/4。

Conclusion: 提出的两阶段解码方案在大规模随机接入场景中有效，预过滤阶段能显著缩减搜索空间，ML阶段在最坏情况下仍能保证P/4的错误概率衰减率，为大规模物联网接入提供了理论保证。
...
### [217] [On Polycyclic Codes over $\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}$ and their Cardinalities](https://arxiv.org/abs/2604.03991)
*Akanksha Tiwari,Pramod Kanwar,Ritumoni Sarma*

Main category: cs.IT

TL;DR: 该论文研究了环$\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}$上的多环码及其关联的挠码，特别关注$t=4$时的情况，给出了理想生成元的构造方法并计算了多环码的基数。


<details>
  <summary>Details</summary>
Motivation: 研究有限环上多环码的结构和性质，特别是通过挠码理论来分析这些编码的代数特性，为编码理论提供新的数学工具和理论框架。

Method: 利用环同态理论，证明了对于满足特定条件的环同态，理想可以表示为生成元集合与π倍理想的和；将此结果应用于特定环结构，获得了理想的生成元表示；特别研究了ω(x)=f(x)^{p^s}的情况，计算了挠理想和挠次数。

Result: 获得了环$\frac{\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}[x]}{\langle ω(x)\rangle}$中所有理想的生成元表示；对于ω(x)=f(x)^{p^s}的特殊情况，计算了挠理想和挠次数；特别在t=4时，利用挠次数计算了多环码的基数。

Conclusion: 该研究为有限环上的多环码提供了系统的代数分析框架，特别是在t=4的情况下获得了具体的计算结果，为编码理论中的多环码研究提供了新的数学工具和理论支持。
...
### [218] [Environment-Aware Near-Field Channel Estimation Leveraging CKM and ISAC](https://arxiv.org/abs/2604.04031)
*Yuan Guo,Yilong Chen,Zixiang Ren,Jie Xu*

Main category: cs.IT

TL;DR: 该论文提出了一种用于集成感知与通信系统的环境感知近场信道估计框架，利用信道知识地图和ISAC技术获取环境特征先验信息，显著提升了信道估计精度和可达速率。


<details>
  <summary>Details</summary>
Motivation: 随着超大规模天线阵列在ISAC系统中的部署，近场信道估计面临挑战。传统方法难以有效利用环境特征信息，特别是静态和动态环境对象引起的多径分量，需要新的框架来提升信道估计性能。

Method: 提出虚拟对象地图作为新的CKM表示方法，存储虚拟环境对象位置以表征静态物理环境对象引起的主要多径分量。设计感知辅助的信道训练协议，基站同时发送下行导频并收集单基地回波感知动态目标，用户设备反馈量化接收观测。基于VOM先验和感知的动态信息，联合估计静态和动态多径分量系数来恢复近场信道。

Result: 数值结果表明，所提出的联合VOM和感知辅助信道估计方案在信道估计精度和可达速率方面显著优于没有VOM先验和/或动态感知的传统方案。

Conclusion: 该环境感知近场信道估计框架通过有效利用信道知识地图和集成感知能力，成功提升了超大规模天线阵列ISAC系统的信道估计性能，为未来无线通信系统提供了有前景的解决方案。
...
### [219] [CTD-Diff: Cooperative Time-Division Diffusion for Multi-User Semantic Communication Systems](https://arxiv.org/abs/2604.04057)
*Chengyang Liang,Dong Li*

Main category: cs.IT

TL;DR: 本文提出了一种协作时分扩散（CTD-Diff）框架，将多用户协作机制融入基于扩散的语义通信中，通过将无线信道噪声转化为扩散噪声来提升传输可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的语义通信模型主要关注点对点链路，忽略了多用户无线环境中多用户协作的潜力。传统方法将信道噪声视为有害因素，而本文旨在利用多用户协作机制提升语义通信性能。

Method: 提出协作时分扩散（CTD-Diff）框架：1）将无线传输过程中的噪声直接整合到前向扩散链中；2）基于时分多址设计多用户协作机制，空闲用户作为语义协作者监听活跃发射器；3）接收端采用直接信号聚合融合直接信号和协作副本；4）聚合的噪声语义表示作为反向扩散过程的条件，通过缓解累积信道失真重建高保真数据。

Result: 大量实验表明，CTD-Diff在重建精度和感知质量方面优于各种基线方法，特别是在具有挑战性的低信噪比条件下表现优异。该方法通过将物理信道噪声转化为扩散噪声，显著提升了传输可靠性。

Conclusion: CTD-Diff框架成功地将多用户协作机制融入扩散式语义通信中，通过创新地将信道噪声整合到扩散过程中，有效提升了无线环境下的语义通信性能，特别是在恶劣信道条件下。
...
### [220] [Quasi-BP for BCH Codes and its Optimization](https://arxiv.org/abs/2604.04066)
*Guangwen Li*

Main category: cs.IT

TL;DR: 提出一种用于BCH码的准置信传播解码器，通过整合信道噪声方差、码的循环特性和奇偶校验矩阵冗余等先验知识，实现高效迭代解码，性能接近同等速率和块长的LDPC码。


<details>
  <summary>Details</summary>
Motivation: 传统BCH码作为高密度奇偶校验码难以应用置信传播解码，需要开发能够整合领域知识的高效迭代解码方法，实现类似LDPC码的性能。

Method: 提出准置信传播解码器，在信息论框架内形式化并行解码过程，通过追踪互信息演化优化参数。每轮迭代包含输入扩展操作扩大消息集，以及合并操作加速互信息增长。

Result: 解码性能接近同等速率和块长的LDPC码，通过广泛仿真验证了方案对不同速率和块长码的通用性和鲁棒性，为高密度奇偶校验码的BP类解码提供了可行方案。

Conclusion: 该准置信传播解码器成功整合领域知识，实现了BCH码的高效迭代解码，性能接近LDPC码，为高密度奇偶校验码的BP类解码部署开辟了可行路径。
...
### [221] [On the Rate Region of I.I.D. Discrete Signaling and Treating Interference as Noise for the Gaussian Broadcast Channel](https://arxiv.org/abs/2604.04092)
*Yujie Shao,Min Qiu*

Main category: cs.IT

TL;DR: 该论文证明在GBC中使用离散星座（如PAM）和TIN解码的简单方案，其可达速率区域与GBC容量区域之间存在与信道参数无关的常数间隙。


<details>
  <summary>Details</summary>
Motivation: 重新审视高斯广播信道，探索使用纯离散输入和TIN解码所能达到的速率区域。传统方法通常使用高斯输入，但本文研究离散星座（如PAM）的潜力。

Method: 提出基于叠加编码的简单方案，使用独立同分布的离散星座输入（如脉冲幅度调制PAM），并采用TIN解码策略。

Result: 证明在TIN解码下，离散输入方案的可达速率区域与GBC容量区域之间存在与所有信道参数无关的常数间隙。仿真显示在某些情况下，弱用户使用PAM比高斯信令能达到更高的速率。

Conclusion: 离散星座输入结合TIN解码是GBC中一种有效且简单的方案，其性能与容量区域仅差常数间隙，在某些情况下甚至优于高斯信令。
...
### [222] [Mechanism and Communication Co-Design for Differentially Private Energy Sharing](https://arxiv.org/abs/2604.04125)
*Yingshuo Gu,Xi Weng,Yue Chen*

Main category: cs.IT

TL;DR: 本文提出了一种基于差分隐私的能源共享机制，通过OTA-MIMO聚合实现高效通信，保护产消者在无线传输中的隐私安全。


<details>
  <summary>Details</summary>
Motivation: 分布式能源资源整合对应对气候危机至关重要，但现有能源共享机制在考虑通信约束时缺乏隐私保护，存在隐私泄露风险。

Method: 采用OTA-MIMO聚合实现频谱高效通信，引入对抗攻击模型分析隐私风险，提出差分隐私均衡搜索算法保护隐私。

Result: 数值实验表明，该方法能有效保护产消者隐私，同时收敛到接近最优解，量化了隐私保护对收敛精度的影响。

Conclusion: 提出的差分隐私能源共享机制在保护产消者隐私的同时，实现了高效的能源协调，为分布式能源系统的隐私保护提供了有效解决方案。
...
### [223] [Would Learning Help? Adaptive CRC-QC-LDPC Selection for Integrity in 5G-NR V2X](https://arxiv.org/abs/2604.04277)
*Sarah Al-Shareeda,Gulcihan Özdemir,Arouj Fatima,Madalin-Dorin Pop,Bander A. Jabr,Yasser Bin Salamah,Jacques Demerjian*

Main category: cs.IT

TL;DR: 该研究探讨了在5G-NR V2X通信中，通过上下文赌博机在线学习选择CRC多项式和QC-LDPC编码率，以减少未检测错误概率，特别是在低到中等移动速度下效果显著。
...
Method: 将联合配置问题建模为轻量级上下文赌博机，采用折扣LinUCB策略，并与贪婪在线适应和保守固定基线进行比较。使用Sionna开发5G-NR兼容的物理层仿真，通过时间相关瑞利衰落建模移动性，使用两状态马尔可夫过程建模非平稳干扰。

Result: 学习辅助的适应在低到中等移动性下最有效，在低SNR区域（-5到5dB）相对于贪婪选择可将未检测错误概率降低50-70%，在高Eb/N0时接近最佳固定配置。在高移动性（≥180km/h）下，快速信道去相关削弱了时间可预测性，限制了在线学习的有效性。

Conclusion: 学习辅助的CRC-QC-LDPC配置在特定移动性范围内能显著提高5G-NR V2X系统的物理层完整性，特别是在低到中等移动速度下效果最佳，为V2X通信的可靠性优化提供了有效方法。
...
### [224] [Robust MMSE Precoding for Out-of-Cluster Interference Mitigation in Cell-Free MIMO Networks](https://arxiv.org/abs/2604.04309)
*André R. Flores,Rodrigo C. de Lamare*

Main category: cs.IT

TL;DR: 提出了一种线性鲁棒最小均方误差预编码器，用于解决无蜂窝多天线系统中的信道状态信息不完美、簇内和簇间干扰问题。
...
Method: 开发了一种线性鲁棒最小均方误差预编码器，在推导过程中包含了簇间干扰的统计信息，以实现更有效的干扰抑制。

Result: 进行了无蜂窝系统总速率分析，推导了量化簇间干扰抑制理论增益的表达式，仿真结果表明所提RMMSE预编码器能有效抑制簇内和簇间干扰。

Conclusion: 所提出的RMMSE预编码器通过包含簇间干扰统计信息，在存在不完美CSI的情况下，能够有效抑制无蜂窝系统中的簇内和簇间干扰，提高系统性能。
...
### [225] [Out-of-Air Computation: Enabling Structured Extraction from Wireless Superposition](https://arxiv.org/abs/2604.04312)
*Seyed Mohammad Azimi-Abarghouyi*

Main category: cs.IT

TL;DR: 该论文提出了一种新的无线计算框架AirCPU，通过结构化编码直接从无线叠加信号中提取计算，避免了传统AirComp需要预嵌入计算或依赖大规模天线阵列的限制。


<details>
  <summary>Details</summary>
Motivation: 传统空中计算(AirComp)通常需要预嵌入计算到传输波形中，或依赖大规模天线阵列来近似理想计算信道，这些方法存在局限性。需要一种新的计算框架，能够直接从无线叠加信号中提取计算，同时支持连续值数据处理。

Method: 提出AirCPU框架：1) 采用多层嵌套格架构，将每个输入分解为分层缩放的分量；2) 所有分量在固定功率约束下通过公共有界数字星座传输；3) 引入解耦分辨率概念；4) 针对衰落MAC提出直接计算、集体计算和连续计算三种机制；5) 开发结构化低复杂度两群近似方法。

Result: 在解码错误概率足够小的操作区域，信道噪声和有限星座约束对失真的影响可忽略，计算误差主要由最精细格设定的目标分辨率决定。提出的计算机制显著扩展了可靠操作区域。

Conclusion: AirCPU建立了联合信源信道编码基础，直接从无线叠加中提取计算，避免了单独的信源量化阶段。该框架通过结构化编码和多种计算机制，在衰落MAC中实现了扩展的可靠操作区域。
...
### [226] [Linear Exact Repair in MDS Array Codes: A General Lower Bound and Its Attainability](https://arxiv.org/abs/2604.04519)
*Hai Liu,Huawei Wu*

Main category: cs.IT

TL;DR: 该论文研究了MDS阵列码在线性精确修复下的修复带宽和修复I/O下界，提出了基于子空间交集的几何方法，得到了适用于任意冗余度r≥2的通用下界，并证明了当r=2时该下界可达。


<details>
  <summary>Details</summary>
Motivation: 研究MDS阵列码在线性精确修复下修复带宽和修复I/O的最小可能值，特别是在固定域大小q、冗余度r和子分组化水平ℓ，而码长n变化的情况下。

Method: 采用几何方法，将线性精确修复重新表述为子空间交集问题，并使用射影计数论证。对于r=2的情况，利用有限几何中的Desarguesian展开构造达到下界的MDS阵列码。

Result: 建立了适用于任意冗余度r≥2的通用下界：β_avg,β_max,γ_avg,γ_max ≥ ℓ(n-1) - (q^{(r-1)ℓ}-1)/(q-1)。证明了当r≥3且ℓ≥2时该下界不可达，但当r=2时，构造了在码长达到最大可能值q^ℓ+1的范围内同时达到修复带宽和修复I/O下界的MDS阵列码。

Conclusion: 该研究识别了控制线性精确修复的统一障碍，并证明在两奇偶校验情况下，这一障碍是紧的。对于r=2的情况，通过有限几何构造实现了理论最优的修复性能。
...
### [227] [Identification for Colored Gaussian Channels](https://arxiv.org/abs/2604.04674)
*Mohammad Javad Salariseddigh*

Main category: cs.IT

TL;DR: 该论文研究了受相关噪声和符号间干扰(ISI)影响的离散时间高斯信道的识别容量，发现在特定条件下码本规模仍能呈现超指数增长。


<details>
  <summary>Details</summary>
Motivation: 研究在存在相关噪声和符号间干扰的高斯信道中，确定性编码函数在峰值功率约束下的识别容量问题，特别是当ISI记忆长度随码字长度增长时的情况。

Method: 采用确定性编码函数，受峰值功率约束，分析具有多项式有界奇异值谱的色噪声协方差矩阵。使用由色高斯噪声统计量诱导的Mahalanobis距离解码器来表征识别容量边界。

Result: 即使当ISI记忆长度以n^κ(κ∈[0,1/2))的亚线性速率增长，且κ+μ∈[0,1/2)时，码本规模仍能呈现超指数增长(∼2^{(n log n)R})。通过Mahalanobis距离解码器，获得了由κ和μ参数化的识别容量边界。

Conclusion: 在相关噪声和符号间干扰存在的情况下，即使ISI记忆长度随码字长度增长，高斯信道仍能实现超指数增长的码本规模，为信道识别容量提供了理论边界。
...
### [228] [LRC codes over characteristic $2$](https://arxiv.org/abs/2604.04678)
*Francisco Galluccio*

Main category: cs.IT

TL;DR: 该论文完成了偶特征域上LRC码的构造，提出了一个通用构造方法，能够获得长度约q⁴、维度和距离均为q⁴量级、局部性r=q-1的线性LRC码，并特别研究了q=4和q=8的情况。


<details>
  <summary>Details</summary>
Motivation: 完善文献[6]中提出的LRC码构造方法，特别是在偶特征域上的情况，目标是构建具有大长度、高维度、良好距离和特定局部性的线性LRC码。

Method: 提出了一个通用构造方法，基于偶特征域上的代数几何技术，通过特定的参数设置获得线性LRC码，并特别针对q=4和q=8的情况进行了详细研究。

Result: 成功构造了长度约q⁴、维度和距离均为q⁴量级、局部性r=q-1的线性LRC码，为q=4和q=8的情况提供了具体的参数和性能分析。

Conclusion: 该研究完成了偶特征域上LRC码的构造理论，提出的通用方法能够有效构建具有优良性能的线性LRC码，为实际应用提供了理论基础。
...
### [229] [Partially deterministic sampling for compressed sensing with denoising guarantees](https://arxiv.org/abs/2604.04802)
*Yaniv Plan,Matthew S. Scott,Ozgur Yilmaz*

Main category: cs.IT

TL;DR: 论文研究了压缩感知中采样向量从酉矩阵行中选择的问题，提出了一种优化的伯努利选择器采样方案，将随机和确定性行选择自然结合，在图像压缩感知中相比有放回和无放回采样方案有显著改进。


<details>
  <summary>Details</summary>
Motivation: 压缩感知中通常随机选择采样向量，但在实践中存在某些关键采样向量需要确定性选择。现有理论主要关注随机采样，而实践中常偏离理论进行确定性采样，需要一种能结合随机和确定性选择的采样方案。

Method: 提出了一种优化的伯努利选择器采样方案，该方案自然地结合了随机和确定性行选择，通过优化方法确定哪些行应该被确定性采样，而不是简单地采用有放回或无放回采样。

Result: 该采样方案在图像压缩感知中相比有放回和无放回采样方案有可测量的改进，适用于生成先验和稀疏先验。理论保证方面，相比先前工作改进了样本复杂度界限，并提供了该设置下的新去噪保证。

Conclusion: 优化的伯努利选择器采样方案能够有效结合随机和确定性采样，在压缩感知中提供更好的性能，为实践中如何选择关键采样向量提供了理论依据和实用方法。
...
### [230] [IC3-Evolve: Proof-/Witness-Gated Offline LLM-Driven Heuristic Evolution for IC3 Hardware Model Checking](https://arxiv.org/abs/2604.03232)
*Mingkai Miao,Guangyu Hu,Ziyi Yang,Hongce Zhang*

Main category: cs.AI

TL;DR: IC3-Evolve：利用LLM自动优化IC3模型检查算法的离线代码进化框架，通过证明/见证门控验证确保正确性，在硬件模型检查基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: IC3（属性导向可达性）算法在硬件安全模型检查中广泛应用，但其性能高度依赖于大量相互作用的启发式规则和实现选择。手动调优成本高、脆弱且难以复现，需要自动化方法来发现实用的启发式改进。

Method: 提出IC3-Evolve框架：1）使用LLM离线生成针对IC3实现的小型、槽位受限且可审计的补丁；2）采用证明/见证门控验证机制——SAFE运行必须生成可独立验证的证书，UNSAFE运行必须生成可重放的计数器示例轨迹；3）部署的工件是独立的进化检查器，无ML/LLM推理开销。

Result: 在公开硬件模型检查竞赛（HWMCC）基准上进行进化，并在未见过的公开和工业模型检查基准上评估泛化能力。结果表明IC3-Evolve能够在严格正确性门控下可靠地发现实用的启发式改进。

Conclusion: IC3-Evolve成功实现了IC3算法的自动化优化，通过LLM驱动的离线代码进化框架，在保证正确性的前提下显著提升模型检查性能，为硬件验证工具的开发提供了新方法。
...
### [231] [Structural Segmentation of the Minimum Set Cover Problem: Exploiting Universe Decomposability for Metaheuristic Optimization](https://arxiv.org/abs/2604.03234)
*Isidora Hernández,Héctor Ferrada,Cristóbal A. Navarro*

Main category: cs.AI

TL;DR: MSCP问题中利用宇宙可分割性进行预处理分解，通过并查集检测连通分量，将原问题分解为独立子问题分别用GRASP求解，显著提升大规模结构化实例的求解质量和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统MSCP方法将实例视为整体，忽略了宇宙的内在结构特性。本文研究宇宙可分割性概念，探索如何利用内在结构分解来增强启发式优化。

Method: 提出基于并查集的高效预处理策略，检测子集中元素共现诱导的连通分量，将原始实例分解为独立子问题。每个子问题使用GRASP元启发式求解，部分解组合而不影响可行性。采用简洁的位级集合表示实现高效集合操作。

Result: 在标准基准实例和大规模合成数据集上的实验表明，利用自然宇宙分割能持续改进解质量和可扩展性，特别是对于大型和结构可分解的实例。位级集合表示使方法在计算上具有实际可行性。

Conclusion: 宇宙可分割性是MSCP的一个重要结构特性，利用该特性进行预处理分解能显著提升启发式优化性能，特别是对于大规模结构化实例。提出的方法具有计算实用性和可扩展性。
...
### [232] [Position: Science of AI Evaluation Requires Item-level Benchmark Data](https://arxiv.org/abs/2604.03244)
*Han Jiang,Susu Zhang,Xiaoyuan Yi,Xing Xie,Ziang Xiao*

Main category: cs.AI

TL;DR: 论文主张AI评估需要项目级基准数据来建立严谨的评估科学，通过引入OpenEval存储库支持基于证据的AI评估


<details>
  <summary>Details</summary>
Motivation: 当前AI评估范式存在系统性有效性失败，包括不合理的设计选择和错位的指标，这些问题在没有原则性框架的情况下难以解决

Method: 通过分析计算机科学和心理测量学中的评估范式，提出项目级AI基准数据的重要性，并引入OpenEval存储库来支持基于证据的AI评估

Result: 项目级分析能够实现细粒度诊断和基准验证，通过对项目属性和潜在构念的说明性分析，展示了项目级数据提供的独特见解

Conclusion: 项目级AI基准数据对于建立严谨的AI评估科学至关重要，OpenEval存储库将促进社区广泛采用基于证据的评估方法
...
### [233] [Evaluating Artificial Intelligence Through a Christian Understanding of Human Flourishing](https://arxiv.org/abs/2604.03356)
*Nicholas Skytland,Lauren Parsons,Alicia Llewellyn,Steele Billings,Peter Larson,John Anderson,Sean Boisen,Steve Runge*

Main category: cs.AI

TL;DR: 论文提出AI对齐不仅是安全问题，更是塑造问题。通过引入FAI-C-ST基准评估20个前沿模型，发现AI系统并非世界观中立，而是默认程序性世俗主义，导致在基督教繁荣理解维度上系统性表现下降约17点，其中信仰与灵性维度下降31点。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地介入道德讨论和灵性探索，它们不仅仅是提供信息，而是作为数字教理问答工具，积极塑造和安排人类的理解、决策和道德反思。需要使这种塑造性影响变得可见和可测量。

Method: 引入Flourishing AI Benchmark: Christian Single-Turn (FAI-C-ST)框架，设计用于根据基督教对人类繁荣的理解，在七个维度上评估前沿模型的响应。比较了20个前沿模型在多元化和基督教特定标准下的表现。

Result: 当前AI系统并非世界观中立，而是默认程序性世俗主义，缺乏维持神学连贯性所需的基础。在所有繁荣维度上出现系统性表现下降约17点，其中信仰与灵性维度下降最为严重，达到31点。

Conclusion: 价值观对齐中的性能差距不是技术限制，而是源于训练目标优先考虑广泛可接受性和安全性，而非深刻、内部连贯的道德或神学推理。AI对齐从根本上是一个塑造问题，而不仅仅是安全问题。
...
### [234] [VERT: Reliable LLM Judges for Radiology Report Evaluation](https://arxiv.org/abs/2604.03376)
*Federica Bologna,Jean-Philippe Corbeil,Matthew Wilkens,Asma Ben Abacha*

Main category: cs.AI

TL;DR: 本文提出VERT作为新的放射学报告评估LLM指标，在多模态多解剖结构数据集上验证其优于现有方法，并通过微调实现显著性能提升和推理加速。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告评估研究主要集中于胸部X光片和设计LLM指标，但缺乏对其他模态和解剖结构的鲁棒性验证，且不清楚哪种模型和提示配置最适合作为放射学评估的LLM评判者。

Method: 1) 比较三种现有LLM评判指标(RadFact、GREEN、FineRadScore)和提出的VERT指标；2) 使用开源和闭源模型(推理型和非推理型)在不同规模上进行评估；3) 在RadEval和RaTE-Eval两个专家标注数据集上进行相关性分析；4) 评估少样本学习、集成学习和参数高效微调；5) 进行系统错误检测和分类研究以理解指标行为。

Result: 1) VERT相比GREEN将放射科医生判断的相关性提高了11.7%；2) 微调Qwen3 30B模型仅使用1,300个训练样本就获得了高达25%的性能提升；3) 微调模型将推理时间减少了37.2倍；4) 识别了指标与专家判断在不同方面的一致性和差异。

Conclusion: LLM评判在放射学报告评估中有效，通过轻量级适应即可实现可靠评估，VERT指标在多模态多解剖结构场景下表现优异，微调能显著提升性能并大幅降低推理成本。
...
### [235] [TABQAWORLD: Optimizing Multimodal Reasoning for Multi-Turn Table Question Answering](https://arxiv.org/abs/2604.03393)
...
<details>
  <summary>Details</summary>
Motivation: 现有的多轮表格推理方法依赖固定的文本序列化进行表格状态读取，导致表示误差在多轮中显著累积。虽然表格接地方法可以缓解这个问题，但需要大量推理计算和成本，在实际部署中不实用。

Method: TABQAWORLD采用联合优化方法：1）表示方面：使用动作条件化的多模态选择策略，动态切换视觉和文本表示以最大化表格状态读取可靠性；2）估计方面：利用表格元数据（维度、数据类型、键值）优化逐步推理轨迹，安全规划轨迹并压缩低复杂度动作以减少对话轮次和延迟。

Result: 实验评估显示，TABQAWORLD实现了最先进的性能，相比基线准确率提升4.87%，相比静态设置获得5.42%的准确率提升和33.35%的推理延迟降低。

Conclusion: TABQAWORLD为可靠且高效的表格推理建立了新标准，通过免训练框架在保持高准确率的同时显著降低计算成本，解决了多轮表格推理中的表示误差累积问题。
...
### [236] [Contextual Control without Memory Growth in a Context-Switching Task](https://arxiv.org/abs/2604.03479)
*Song-Ju Kim*

Main category: cs.AI

TL;DR: 提出了一种基于干预的循环架构，通过上下文索引的加性操作符干预共享循环潜在状态，实现上下文依赖的序列决策，无需扩大循环维度或直接输入上下文信息。


<details>
  <summary>Details</summary>
Motivation: 研究上下文依赖序列决策的第三种替代方案：通过干预共享循环潜在状态实现上下文依赖，而不是直接输入上下文或增加循环记忆维度。

Method: 引入干预式循环架构：循环核心首先构建共享的预干预潜在状态，然后上下文通过上下文索引的加性操作符进行干预。在部分可观测的上下文切换序列决策任务上进行评估。

Result: 干预模型在主基准测试中表现强劲，无需额外的循环维度。使用条件互信息作为操作探针，干预模型对任务相关的第一阶段结果表现出正的条件上下文信息。

Conclusion: 干预共享循环状态为上下文控制提供了一种可行的替代方案，无需扩大循环记忆维度，在此设置下有效实现了上下文依赖的序列决策。
...
### [237] [Beyond Predefined Schemas: TRACE-KG for Context-Enriched Knowledge Graphs from Complex Documents](https://arxiv.org/abs/2604.03496)
*Mohammad Sadeq Abolhasani,Yang Ba,Yixuan He,Rong Pan*

Main category: cs.AI

TL;DR: TRACE-KG是一个多模态框架，无需预定义本体即可联合构建上下文丰富的知识图谱和推导模式，提供结构化限定词和数据驱动模式，在保持可追溯性的同时实现知识图谱构建。


<details>
  <summary>Details</summary>
Motivation: 当前知识图谱构建面临两难：基于本体的方法需要昂贵的模式设计和维护，而无模式方法则产生碎片化、缺乏全局组织的图谱，特别是在技术文档等密集信息场景中。

Method: 提出TRACE-KG多模态框架，通过结构化限定词捕获条件关系，使用数据驱动模式组织实体和关系，同时保持与源证据的完全可追溯性，无需预定义本体。

Result: 实验表明TRACE-KG能够生成结构连贯、可追溯的知识图谱，为基于本体和无模式的构建流程提供了实用替代方案。

Conclusion: TRACE-KG框架在无需预定义本体的前提下，通过联合构建知识图谱和推导模式，有效解决了传统知识图谱构建方法的局限性，实现了结构连贯且可追溯的知识表示。
...
### [238] [Resource-Conscious Modeling for Next- Day Discharge Prediction Using Clinical Notes](https://arxiv.org/abs/2604.03498)
*Ha Na Cho,Sairam Sutari,Alexander Lopez,Hansen Bow,Kai Zheng*

Main category: cs.AI

TL;DR: 本研究评估了轻量级微调大语言模型与传统文本模型在脊柱择期手术中预测次日出院的效果，发现TF-IDF结合LGBM模型表现最佳，而基于Transformer的生成模型表现不佳。
...
Result: TF-IDF结合LGBM模型表现最佳，出院类别的F1分数为0.47，召回率为0.51，AUC-ROC最高（0.80）。LoRA提高了DistilGPT2的召回率，但整体上基于Transformer的生成模型表现不佳。

Conclusion: 在现实世界不平衡的临床预测任务中，可解释且资源高效的模型可能优于紧凑型大语言模型，传统文本模型在此类任务中仍具有优势。
...
### [239] [BioAlchemy: Distilling Biological Literature into Reasoning-Ready Reinforcement Learning Training Data](https://arxiv.org/abs/2604.03506)
...
TL;DR: BioAlchemy是一个从生物学研究文本中提取可验证问答对的流程，创建了包含34.5万个科学推理问题的训练数据集，并通过强化学习提升生物学推理性能，最终模型在生物学基准上提升了9.12%。


<details>
  <summary>Details</summary>
Motivation: 当前大规模推理数据集中生物学问题与现代生物学研究主题分布不匹配，这种主题不平衡可能影响性能表现，且从生物学研究文本中提取具有挑战性和可验证性研究问题的方法尚未充分发展。

Method: 开发BioAlchemy流程，从生物学研究文本语料库中提取多样化的可验证问答对；创建BioAlchemy-345K训练数据集；通过强化学习将数据集与当代科学生物学主题分布对齐以提升推理性能。

Result: BioAlchemist-8B模型相比其基础推理模型在生物学基准测试上提升了9.12%，展示了该方法在增强生物学科学推理能力方面的有效性。

Conclusion: 通过创建与当代生物学研究主题分布对齐的高质量训练数据集，并结合强化学习方法，可以有效提升生物学领域的科学推理能力。
...
### [240] [Structural Rigidity and the 57-Token Predictive Window: A Physical Framework for Inference-Layer Governability in Large Language Models](https://arxiv.org/abs/2604.03524)
*Gregory M. Ruddell*

Main category: cs.AI

TL;DR: 研究发现当前AI安全方法在多数指令调优模型中无法检测到预承诺信号，提出了基于能量的治理框架和五类推理行为分类，证明规则违反和幻觉是不同故障模式，需要不同的检测机制。
...
Method: 提出基于能量的治理框架，将Transformer推理动态连接到神经计算的约束满足模型。使用轨迹张力（ρ = ||a|| / ||v||）作为度量，在7个模型队列中分析五个几何机制，引入能量不对称性作为结构刚度的统一度量。

Result: 在Phi-3-mini-4k-instruct模型的贪婪解码算术约束探测中识别出57个token的预承诺窗口，但这是模型、任务和配置特定的。建立了五类推理行为分类：权威带、晚期信号、倒置、平坦和支架选择性。在7个模型中，只有一种配置显示预测性信号；事实幻觉在72个测试条件下均无预测信号。

Conclusion: 规则违反和幻觉是不同故障模式，需要不同的检测机制。内部几何监控仅在存在阻力时有效，事实虚构的检测需要外部验证机制。该工作为推理层可治理性提供了可测量框架，并为评估自主AI系统部署风险引入了分类学。
...
### [241] [Explainable Model Routing for Agentic Workflows](https://arxiv.org/abs/2604.03527)
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体工作流的路由架构只关注性能优化，缺乏对模型能力与成本之间权衡的记录。开发者无法区分智能效率（为合适任务选择专门模型）与预算驱动模型选择导致的潜在失败，需要可解释的路由决策机制。

Method: Topaz框架包含三个核心组件：1) 基于技能的分析，将多样化基准测试的性能合成为细粒度能力档案；2) 完全可追溯的路由算法，使用基于预算和多目标优化，产生技能匹配分数与成本权衡的清晰痕迹；3) 面向开发者的解释，将这些痕迹转化为自然语言，允许用户审计系统逻辑并迭代调整成本-质量权衡。

Result: Topaz通过引入形式化的可审计性，使路由决策变得可解释，使用户能够理解、信任并有意义地引导路由智能体系统，解决了当前路由架构中缺乏透明度和可解释性的问题。

Conclusion: Topaz框架通过使路由决策可解释，使用户能够理解、信任并有效引导智能体工作流，为智能体路由系统提供了必要的透明度和可审计性，帮助开发者优化成本-质量权衡。
...
### [242] [Automated Analysis of Global AI Safety Initiatives: A Taxonomy-Driven LLM Approach](https://arxiv.org/abs/2604.03533)
*Takayuki Semitsu,Naoto Kiribuchi,Kengo Zenitani*

Main category: cs.AI

TL;DR: 提出一个基于AI安全活动分类的自动化政策文件对比框架，使用大语言模型提取和映射相关活动，生成摘要、比较和相似度评分，评估不同模型在政策文件分析中的稳定性和有效性。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全政策文件分析缺乏系统化的比较框架，需要自动化工具来帮助政策制定者和研究人员快速理解和对比不同政策文件在AI安全活动方面的异同。

Method: 基于AI安全活动分类构建自动化对比框架，使用五种大语言模型对十份公开政策文件进行交叉分析，提取活动、生成摘要和相似度评分，并通过三位专家的人工评估验证结果。

Result: 模型选择显著影响对比结果，某些文件对在不同模型间存在高度分歧；专家评估显示高一致性，但模型评分与人工判断仍存在差异；热图可视化显示平均相似度分布。

Conclusion: 该框架支持政策文件的比较分析，但模型选择对结果有重要影响，需要谨慎选择模型并考虑人工验证；研究为AI安全政策分析提供了实用工具和方法论基础。
...
### [243] [When Do Hallucinations Arise? A Graph Perspective on the Evolution of Path Reuse and Path Compression](https://arxiv.org/abs/2604.03557)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生流畅但无根据的推理幻觉，这些失败现象广泛存在，但其在仅解码器Transformer中的产生机制尚不清楚，需要深入理解

Method: 将下一个token预测建模为图搜索过程，其中实体对应节点，学习到的转换形成边。上下文推理是在采样子图上的约束搜索（内在推理），而无上下文查询则依赖底层图中的记忆结构（外在推理）
...
### [244] [When Adaptive Rewards Hurt: Causal Probing and the Switching-Stability Dilemma in LLM-Guided LEO Satellite Scheduling](https://arxiv.org/abs/2604.03562)
*Yuanhang Li*

Main category: cs.AI

TL;DR: 研究发现自适应奖励权重在多波束LEO卫星调度中不如静态权重，因为PPO算法需要准稳态奖励信号才能收敛。通过因果探测方法揭示了反直觉的奖励项影响，并评估了四种MDP架构变体。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于直觉：在深度强化学习的多波束低地球轨道卫星调度中，基于状态感知的动态奖励权重应该优于静态权重。研究者希望系统性地验证这一直觉，并探索奖励权重设计对算法性能的影响。

Method: 1. 引入单变量因果探测方法：独立扰动每个奖励项±20%，测量PPO在50k步后的响应；2. 评估四种MDP架构变体：固定权重、基于规则的权重、学习型MLP权重、微调LLM权重；3. 在已知和新型流量状态下进行测试；4. 分析PPO算法对奖励信号稳定性的要求。

Result: 1. 静态权重（342.1 Mbps）优于动态权重（103.3±96.8 Mbps），揭示切换稳定性困境；2. 因果探测发现反直觉杠杆：切换惩罚增加20%在极地切换和冷热状态下分别提升157 Mbps和130 Mbps；3. MLP在已知状态下达到357.9 Mbps，新型状态下325.2 Mbps；4. 微调LLM崩溃至45.3±43.0 Mbps，主要原因是权重振荡而非领域知识不足。

Conclusion: 研究为通信系统中LLM-DRL集成提供了经验性路线图：识别了LLM不可替代的价值领域（自然语言意图理解）和简单方法足够胜任的领域。关键发现是输出一致性而非领域知识成为主要约束，奖励权重设计需要考虑算法收敛的稳定性要求。
...
### [245] [Personality Requires Struggle: Three Regimes of the Baldwin Effect in Neuroevolved Chess Agents](https://arxiv.org/abs/2604.03565)
*Diego Armando Resendez Prado*

Main category: cs.AI

TL;DR: 研究发现Hebbian可塑性在进化过程中对行为多样性的影响存在反转：初期压缩多样性，后期通过想象放大感知差异来扩展多样性，导致结构化行为分歧。


<details>
  <summary>Details</summary>
Motivation: 探究终身学习是否会随时间扩展行为多样性而非压缩多样性，挑战先前认为可塑性通过缓冲环境噪声来减少变异的理论。

Method: 使用具有八个NEAT进化神经模块、游戏内Hebbian可塑性、以及带有想象的期望-领域信号链的象棋智能体，在10个种子条件下进行实验。

Result: 出现方差交叉现象：Hebbian开启初期跨种子方差低于关闭状态，但在第34代后反超。行为分歧结构化：62%的走法分歧、不同的开局库、棋子偏好和游戏长度。识别出三种机制：探索、彩票和透明机制。

Conclusion: 可塑性对行为多样性的影响在进化过程中发生反转，通过想象放大感知差异创造结构化行为分歧。自对弈系统可能通过消除异质性来系统性抑制行为多样性。
...
### [246] [Selective Forgetting for Large Reasoning Models](https://arxiv.org/abs/2604.03571)
*Tuan Le,Wei Qian,Mengdi Huai*

Main category: cs.AI

TL;DR: 提出针对大型推理模型的精确遗忘框架，选择性移除敏感推理组件同时保持通用推理能力
...
Method: 使用多LLM结合检索增强生成分析思维链轨迹，识别需要遗忘的相关片段，用良性占位符替换，同时引入特征替换遗忘损失函数，抑制遗忘内容生成并强化结构有效替换

Result: 在合成和医疗数据集上的广泛实验验证了所提方法的期望特性

Conclusion: 提出的LRM遗忘框架能够精确遗忘目标知识，同时保持通用推理能力的完整性
...
### [247] [Rashomon Memory: Towards Argumentation-Driven Retrieval for Multi-Perspective Agent Memory](https://arxiv.org/abs/2604.03588)
*Albert Sadowski,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: 提出Rashomon记忆架构，支持多目标导向的智能体并行编码经验，通过论证语义在查询时协商，保留冲突解释而非强制统一


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在长期运行中积累的经验服务于多个并发目标，经常需要对同一事件保持冲突的解释。现有记忆架构假设单一正确编码或仅支持统一存储的多视图，无法有效处理这种多视角冲突

Method: 提出Rashomon记忆架构：并行目标导向智能体根据各自优先级编码经验，每个视角维护自己的本体和知识图谱。检索时，各视角提出解释，使用非对称领域知识相互批判，通过Dung论证语义学确定哪些提议保留

Result: 概念验证显示：检索模式（选择、组合、冲突浮现）从攻击图拓扑中涌现；冲突浮现模式让系统报告真实分歧而非强制解决，使决策者能直接看到底层的解释冲突

Conclusion: Rashomon记忆架构通过论证语义支持多视角经验编码和协商，生成的攻击图本身作为解释，记录选择、考虑和拒绝的决策过程，特别适合需要保留冲突解释而非强制统一的复杂决策场景
...
### [248] [Entropy and Attention Dynamics in Small Language Models: A Trace-Level Structural Analysis on the TruthfulQA Benchmark](https://arxiv.org/abs/2604.03589)
*Adeyemi Adeseye,Aisvarya Adeseye,Hannu Tenhunen,Jouni Isoaho*

Main category: cs.AI

TL;DR: 该研究对小型语言模型（SLMs）进行追踪级分析，通过熵和注意力动态评估模型在TruthfulQA数据集上的表现，发现不同模型在熵模式、隐藏状态和注意力分散方面存在显著差异，可分为确定性、探索性和平衡性三类模型。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型在边缘设备等资源受限环境中部署日益增多，但这些模型存在自信的错误预测和不稳定输出，对事实性和决策关键任务构成风险。现有评估方法主要依赖最终准确率或幻觉率，未能解释内部模型行为如何影响输出，特别是解码过程中熵的演变、注意力在各层的分布以及隐藏表示如何影响不确定性、逻辑不一致和错误信息传播。

Method: 研究引入对小型语言模型的追踪级分析，使用TruthfulQA数据集评估四个参数范围在1B-1.7B的模型。分析方法包括：令牌级输出熵、注意力熵、头部分散度和隐藏状态表示。通过分析熵和注意力动态来理解模型内部行为。

Result: 研究结果将模型按熵模式分为三类：确定性模型（DeepSeek-1.5B和LLaMA-1B）输出熵随时间减少；探索性模型（Gemma-1B）熵随时间增加；平衡模型（Qwen-1.7B）具有中等且稳定的熵。每个组在隐藏状态移动和注意力分散模式上也有显著差异。分析表明SLMs的真实性源于结构化的熵和注意力动态。

Conclusion: 监控和优化这些内部不确定性模式可以指导设计更可靠、幻觉感知和应用特定的边缘小型语言模型。研究强调了理解模型内部动态对于提高模型真实性和可靠性的重要性，为开发更适合资源受限环境的SLMs提供了新方向。
...
### [249] [A Multimodal Foundation Model of Spatial Transcriptomics and Histology for Biological Discovery and Clinical Prediction](https://arxiv.org/abs/2604.03630)
...
TL;DR: STORM是一个基础模型，通过整合空间转录组学和组织学数据，实现从H&E图像预测基因表达，提升空间域发现和临床预测能力。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学(ST)成本高、通量低，而H&E染色虽然提供丰富的形态学信息但缺乏分子分辨率。需要一种能够桥接成像和组学数据的方法。

Method: 开发STORM基础模型，在18个器官的120万个空间分辨转录组谱和匹配的组织学数据上训练。采用分层架构整合形态特征、基因表达和空间上下文。

Result: STORM在11种肿瘤类型中优于现有方法预测空间基因表达，平台无关（Visium、Xenium等），在23个独立队列（7,245名患者）中显著改善免疫治疗反应预测和预后。

Conclusion: STORM为空间信息发现和临床精准医学提供了一个可扩展的框架，成功桥接了成像和组学数据。
...
### [250] [Single-agent vs. Multi-agents for Automated Video Analysis of On-Screen Collaborative Learning Behaviors](https://arxiv.org/abs/2604.03631)
...
Method: 比较了闭源VLMs（Claude-3.7-Sonnet、GPT-4.1）和开源VLM（Qwen2.5-VL-72B）在单智能体和多智能体设置下的性能。提出了两种多智能体框架：1）基于工作流的三智能体系统，通过场景分割和光标提示的VLM进行行为检测与证据验证；2）受ReAct启发的自主决策多智能体系统，通过推理、工具操作和观察驱动自我修正的迭代过程生成可解释的行为标签。

Result: 实验结果表明，两种提出的多智能体框架均取得了可行性能，在场景和行为检测任务上优于单VLM。工作流智能体在场景检测上表现最佳，自主决策多智能体在行为检测上表现最佳。
...
### [251] [TableVision: A Large-Scale Benchmark for Spatially Grounded Reasoning over Complex Hierarchical Tables](https://arxiv.org/abs/2604.03660)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在处理具有层次布局的复杂表格时存在感知瓶颈，随着任务复杂度增加，涉及的离散视觉区域数量不成比例增长，导致"感知过载"，影响空间注意力的准确性

Method: 1. 提出TableVision大规模轨迹感知基准，将表格任务分为感知、推理和分析三个认知层次；2. 使用基于渲染的确定性定位管道，将多步逻辑推导与像素级空间真值显式耦合；3. 采用两阶段解耦框架，包含6,799个高保真推理轨迹

Result: 1. 显式空间约束显著恢复MLLMs的推理潜力；2. 两阶段解耦框架在测试集上实现12.3%的整体准确率提升；3. TableVision为文档理解中感知与逻辑的协同提供了严格测试平台

Conclusion: TableVision通过显式空间约束解决了MLLMs在复杂表格理解中的感知瓶颈，为文档理解研究提供了新的视角和基准，证明了感知与逻辑协同的重要性
...
TL;DR: PRAISE框架通过重用搜索轨迹前缀和生成中间步骤奖励，解决了智能搜索训练中的数据效率低下和奖励稀疏性问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于搜索的强化学习方法存在两个核心限制：训练期间昂贵的长期轨迹未充分利用，以及监督通常只在最终答案可用，导致严重的奖励稀疏性。

Method: PRAISE框架从完整搜索轨迹中提取不同搜索轮次的前缀状态，从中引出中间答案，并使用这些前缀既构建额外的训练轨迹，又通过前缀间的性能差异推导步骤级奖励。

Result: 在多跳问答基准测试中，PRAISE始终优于强基线方法，提高了性能。

Conclusion: PRAISE通过重用轨迹前缀和生成中间奖励，有效改善了智能搜索训练中的数据效率和信用分配问题，且无需额外人工标注或单独奖励模型。
...
### [253] [Structured Multi-Criteria Evaluation of Large Language Models with Fuzzy Analytic Hierarchy Process and DualJudge](https://arxiv.org/abs/2604.03742)
*Yulong He,Ivan Smirnov,Dmitry Fedrushkov,Sergey Kovalchuk,Ilya Revin*

Main category: cs.AI

TL;DR: 本文提出了一种基于模糊层次分析法（FAHP）和DualJudge混合框架的大语言模型评估方法，通过结构化评估标准和不确定性建模提升评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型直接评分方法存在不一致性和不透明性，需要更可靠的结构化评估框架来解决评估瓶颈问题。

Method: 1. 将层次分析法（AHP）应用于LLM评估；2. 提出置信度感知的模糊AHP（FAHP），通过三角模糊数建模认知不确定性；3. 提出DualJudge混合框架，融合直观直接评分和结构化AHP输出。

Result: 在JudgeBench上的系统验证显示，AHP和FAHP在不同模型规模和数据集分割上均优于直接评分，FAHP在不确定比较场景中表现出更好的稳定性。DualJudge实现了最先进的性能。

Conclusion: 不确定性感知的结构化推理为更可靠的LLM评估提供了原则性路径，直观和审慎评估范式具有互补优势。
...
### [254] [RL-Driven Sustainable Land-Use Allocation for the Lake Malawi Basin](https://arxiv.org/abs/2604.03768)
*Ying Yao*

Main category: cs.AI

TL;DR: 使用深度强化学习优化马拉维湖流域土地利用分配，最大化生态系统服务价值，结合空间连贯性约束和政策情景分析。


<details>
  <summary>Details</summary>
Motivation: 生态敏感区域不可持续的土地利用方式威胁生物多样性、水资源和数百万人的生计，需要科学方法来优化土地利用分配。

Method: 基于深度强化学习框架，使用PPO算法在50x50网格上优化土地利用分配，结合生态系统服务价值系数和空间连贯性奖励函数（连续性奖励和缓冲区惩罚）。

Result: 智能体有效学习增加总生态系统服务价值；空间奖励塑造成功引导形成生态合理的土地利用模式；框架对政策参数变化响应良好，可作为环境规划情景分析工具。

Conclusion: 深度强化学习框架为环境规划提供了有效的土地利用优化工具，能够平衡生态系统服务价值和空间生态模式，支持政策情景分析。
...
### [255] [Affording Process Auditability with QualAnalyzer: An Atomistic LLM Analysis Tool for Qualitative Research](https://arxiv.org/abs/2604.03820)
...
Method: 开发了QualAnalyzer开源Chrome扩展，支持Google Workspace，采用原子化LLM分析方法，独立处理每个数据段，并完整保留每个分析单元的提示、输入和输出信息。

Result: 通过两个案例研究（整体性论文评分和访谈转录本的演绎主题编码）表明，该方法创建了清晰的可审计追踪，帮助研究人员调查LLM与人类判断之间的系统性差异。
...
### [256] [FeynmanBench: Benchmarking Multimodal LLMs on Diagrammatic Physics Reasoning](https://arxiv.org/abs/2604.03893)
*Zeyu Wang,Xiaogang Li,Peiyao Xiao,Qinhao Kong,Ben Wang,Chengliang Xu,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: FeynmanBench是首个专注于费曼图任务的基准测试，旨在评估AI在多步图解推理方面的能力，包括满足守恒定律、识别图拓扑、在图表和代数表示之间转换等。该基准包含2000多个任务，覆盖标准模型的电磁、弱和强相互作用，揭示了当前MLLMs在物理约束执行和全局拓扑条件方面的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型基准主要关注局部信息提取，而忽视了形式科学符号中固有的全局结构逻辑。前沿理论突破通常依赖于具体图解符号与严谨逻辑的结合，因此需要开发能够评估AI在科学图解推理方面能力的基准测试。

Method: 开发了FeynmanBench基准，包含自动化的流水线来生成多样化的费曼图，并提供可验证的拓扑标注和振幅结果。数据库覆盖标准模型的电磁、弱和强相互作用，包含100多种不同类型和2000多个任务。

Result: 在最先进的多模态大语言模型上的实验揭示了系统性失败模式，包括物理约束执行不稳定和全局拓扑条件违反，表明当前模型在科学图解推理方面存在显著局限性。

Conclusion: FeynmanBench为评估AI是否能够有效参与科学发现（特别是在理论物理学领域）提供了逻辑严谨的测试，强调了基于物理基础的视觉推理基准的重要性。
...
### [257] [FactReview: Evidence-Grounded Reviews with Literature Positioning and Execution-Based Claim Verification](https://arxiv.org/abs/2604.04074)
*Hang Xu,Ling Yue,Chaoqian Ouyang,Libin Zheng,Shaowu Pan,Shimin Di,Min-Ling Zhang*

Main category: cs.AI

TL;DR: FactReview是一个基于证据的机器学习论文评审系统，通过提取论文主张、文献定位和执行验证来生成证据支持的评审报告，而非仅依赖论文自身的叙述。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习领域的同行评审面临提交量激增和评审时间有限的压力。大多数基于LLM的评审系统仅阅读论文本身，容易受到论文表述质量的影响，且在需要相关文献或代码证据时表现不佳。

Method: FactReview采用证据驱动的评审方法：1) 提取论文主要主张和报告结果；2) 检索相关文献以明确论文的技术定位；3) 当代码可用时，在有限预算下执行代码仓库以验证核心实证主张；4) 生成简洁评审和证据报告，为每个主要主张分配五种标签之一。
...
Conclusion: AI在同行评审中最有用的角色不是最终决策者，而是作为收集证据和帮助评审者做出更基于证据评估的工具。FactReview展示了这种证据驱动方法的有效性。
...
### [258] [Compliance-by-Construction Argument Graphs: Using Generative AI to Produce Evidence-Linked Formal Arguments for Certification-Grade Accountability](https://arxiv.org/abs/2604.04103)
*Mahyar T. Moghaddam*

Main category: cs.AI

TL;DR: 本文提出了一种将生成式AI与结构化形式论证相结合的合规性构建架构，通过验证约束确保AI辅助决策的可追溯性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 高风险决策系统需要结构化论证、可追溯性和可审计性以确保问责和监管合规。当前生成式AI在决策支持中的部署存在幻觉推理、无支持声明和弱可追溯性等风险，需要更严谨的集成方法。

Method: 提出合规性构建架构，包含四个组件：1)基于保证案例方法的类型化论证图表示；2)基于检索增强生成的权威证据论证片段起草；3)强制执行完整性和可接受性约束的推理验证内核；4)符合W3C PROV标准的来源分类账以支持可审计性。

Result: 分析表明，确定性验证规则可以防止无支持的声明进入决策记录，同时允许生成式AI加速论证构建。系统设计和基于可执行不变量的评估策略展示了可行性。

Conclusion: 该架构能够在保持生成式AI加速论证构建优势的同时，通过结构化验证确保高风险决策系统的合规性、可追溯性和可审计性要求。
...
### [259] [InsTraj: Instructing Diffusion Models with Travel Intentions to Generate Real-world Trajectories](https://arxiv.org/abs/2604.04106)
...
Method: InsTraj采用两阶段方法：1) 使用大型语言模型解析自然语言中的出行意图，创建丰富的语义蓝图，弥合意图与轨迹之间的表示鸿沟；2) 提出多模态轨迹扩散变换器，整合语义指导，生成高保真且忠实于指令的轨迹。

Result: 在真实世界数据集上的综合实验表明，InsTraj在生成真实性、多样性和语义忠实性方面显著优于现有最先进方法，能够生成更符合用户细粒度意图的轨迹。

Conclusion: InsTraj通过结合大型语言模型的语义理解能力和扩散模型的生成能力，成功解决了轨迹生成中的语义理解和约束处理难题，为基于自然语言指令的轨迹生成提供了有效解决方案。
...
### [260] [Readable Minds: Emergent Theory-of-Mind-Like Behavior in LLM Poker Agents](https://arxiv.org/abs/2604.04157)
*Hsieh-Ting Lin,Tsung-Yu Hou*

Main category: cs.AI

TL;DR: 研究发现：在德州扑克游戏中，配备持久记忆的LLM智能体通过动态互动能自发发展出类似心理理论的能力，而记忆是必要条件。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅通过静态情境测试LLM是否具备心理理论能力，但尚不清楚在动态互动中是否能自发产生类似心理理论的行为。本研究旨在探索LLM智能体在长期互动中是否能发展出对手心理模型。

Method: 采用2x2因子设计（记忆有无×领域知识有无），共20个实验（约6000次观察）。让自主LLM智能体进行德州扑克游戏，通过分析其对手建模能力、战略欺骗行为和心理理论水平来评估。

Result: 记忆是心理理论行为出现的必要且充分条件（Cliff's delta = 1.0）。有记忆的智能体达到心理理论3-5级（预测到递归建模），无记忆的智能体始终为0级。战略欺骗仅出现在有记忆条件下。领域知识不阻碍心理理论发展但增强其应用精度。

Conclusion: 功能性心理理论行为可以从互动动态中自发涌现，无需明确训练或提示。这为理解人工智能社交认知和生物社会认知提供了新视角，同时展示了透明可读的心理模型表达方式。
...
### [261] [A Model of Understanding in Deep Learning Systems](https://arxiv.org/abs/2604.04171)
*David Peter Wallis Freeborn*

Main category: cs.AI

TL;DR: 作者提出了一个适用于机器学习系统的系统性理解模型，认为当智能体包含能够追踪真实规律、通过稳定桥梁原则与目标系统耦合、并支持可靠预测的充分内部模型时，就实现了理解。作者认为当代深度学习系统能够达到这种理解，但通常达不到科学理解的理想标准。


<details>
  <summary>Details</summary>
Motivation: 本文旨在为机器学习系统建立一个系统性理解的理论框架，探讨深度学习系统是否真正实现了对目标系统的理解，以及这种理解与科学理解之间的差距。

Method: 作者提出了一个系统性理解的概念模型，包含三个关键要素：1）追踪真实规律的充分内部模型；2）通过稳定桥梁原则与目标系统耦合；3）支持可靠预测。然后分析当代深度学习系统在这三个维度上的表现。

Result: 作者认为当代深度学习系统确实能够达到所定义的理解标准，但存在三个主要缺陷：1）符号与目标系统不对齐；2）缺乏明确的还原性；3）统一性较弱。作者将这种不完美的理解称为"断裂理解假说"。

Conclusion: 深度学习系统能够实现某种形式的系统性理解，但这种理解与理想的科学理解存在显著差距，表现为符号不对齐、非还原性和弱统一性，形成了"断裂理解"的特征。
...
### [262] [CoALFake: Collaborative Active Learning with Human-LLM Co-Annotation for Cross-Domain Fake News Detection](https://arxiv.org/abs/2604.04174)
*Esma Aïmeur,Gilles Brassard,Dorsaf Sallami*

Main category: cs.AI

TL;DR: CoALFake：一种结合人类-大语言模型协同标注与领域感知主动学习的跨领域假新闻检测方法，通过低成本标注和动态领域特征捕获实现优越性能
...
Method: 提出CoALFake方法，整合人类-大语言模型协同标注与领域感知主动学习，利用LLM进行低成本可扩展标注，同时保持人类监督确保标签可靠性；通过领域嵌入技术动态捕获领域特定特征和跨领域模式，训练领域无关模型；采用领域感知采样策略优化样本获取，优先考虑多样化的领域覆盖

Result: 在多个数据集上的实验结果表明，该方法持续优于各种基线方法，即使只有最小化的人类监督也能取得优异性能，证明人类-LLM协同标注是一种高成本效益的方法

Conclusion: CoALFake通过整合人类-LLM协同标注和领域感知主动学习，有效解决了跨领域假新闻检测中的标注成本高和信息丢失问题，实现了优越的检测性能
...
### [263] [Comparative reversal learning reveals rigid adaptation in LLMs under non-stationary uncertainty](https://arxiv.org/abs/2604.04182)
*Haomiaomiao Wang,Tomás E Ward,Lili Zhang*

Main category: cs.AI

TL;DR: 该研究将大语言模型作为序列决策策略，在概率反转学习任务中评估其非平稳环境适应能力，发现LLMs在赢时保持策略接近完美但输时转换策略显著减弱，表现出正负证据使用的不对称性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估大语言模型在非平稳环境中的适应能力，特别是在概率反转学习任务中，当环境条件变化时如何修正先前学习的行动价值。研究者希望了解LLMs在面对环境变化时的决策行为特征，并与人类行为进行对比。

Method: 研究方法采用两选项概率反转学习任务，包含三个潜在状态和由性能标准或超时触发的切换事件。比较确定性固定转换周期与增加波动性的随机调度。评估了DeepSeek-V3.2、Gemini-3和GPT-5.2三个大语言模型，并以人类数据作为行为参考。使用分层强化学习拟合来分析机制。

Result: 研究结果显示：1) 所有模型中，赢时保持策略接近完美，但输时转换策略显著减弱，表现出正负证据使用的不对称性；2) DeepSeek-V3.2在反转后表现出极端固执和弱获取能力；3) Gemini-3和GPT-5.2适应更快但仍比人类对损失不敏感；4) 随机转换放大了LLMs的反转特定固执，但并未一致减少总获胜次数；5) 分层强化学习拟合揭示了不同的机制：固执可能源于弱损失学习、夸大的策略确定性或通过反事实抑制的价值极化。

Conclusion: 研究结论表明大语言模型在非平稳环境中表现出特定的适应模式，特别是正负证据处理的不对称性。这些结果支持开发反转敏感的诊断工具和波动感知模型，用于在非平稳不确定性下评估大语言模型。
...
### [264] [Don't Blink: Evidence Collapse during Multimodal Reasoning](https://arxiv.org/abs/2604.04207)
*Suresh Raghu,Satwik Pandey*

Main category: cs.AI

TL;DR: 研究发现推理视觉语言模型在思考过程中会逐渐失去视觉基础，形成任务条件危险区域，低熵预测自信但无视觉依据，提出基于熵-视觉交互模型的任务感知多模态监控方法


<details>
  <summary>Details</summary>
Motivation: 推理视觉语言模型在推理过程中会逐渐失去视觉基础，形成任务条件危险区域，其中低熵预测虽然自信但缺乏视觉依据，这种失败模式无法通过纯文本监控检测，需要研究多模态监控方法

Method: 在MathVista、HallusionBench和MMMU_Pro三个数据集上评估三种推理视觉语言模型，分析证据崩溃现象，研究全响应熵作为文本不确定性信号，开发熵-视觉交互模型识别任务条件机制

Result: 发现普遍的证据崩溃现象：推理过程中对标注证据区域的注意力显著下降，通常损失超过一半的证据质量；全响应熵是跨数据集转移中最可靠的文本不确定性信号；低熵、视觉脱离的预测在持续视觉参考任务中危险但在符号任务中良性

Conclusion: 提出基于任务感知的多模态监控方法，通过有针对性的视觉否决机制，在90%覆盖率下将选择性风险降低达1.9个百分点，同时避免在预期脱离情况下的性能下降，支持在分布转移下的安全部署
...
### [265] [Combee: Scaling Prompt Learning for Self-Improving Language Model Agents](https://arxiv.org/abs/2604.04247)
...
TL;DR: Combee框架通过并行扫描和增强洗牌机制，实现了大规模并行提示学习，解决了现有方法在并行设置下的质量下降问题，获得了17倍加速且保持或提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法主要针对单智能体或低并行度设置，无法高效地从大量智能体轨迹中学习。随着学习轨迹增多和并行执行需求增长，需要可扩展的并行提示学习框架来避免质量下降。

Method: Combee采用并行扫描和增强洗牌机制，引入动态批量大小控制器来平衡质量和延迟，实现并行提示学习而不会降低质量。

Result: 在AppWorld、Terminal-Bench、Formula和FiNER等基准测试中，Combee相比先前方法实现了高达17倍的加速，同时保持相当或更好的准确性，且成本相当。

Conclusion: Combee成功解决了大规模并行提示学习的扩展问题，通过创新机制实现了高效、高质量的并行学习，为自改进智能体提供了可扩展的框架。
...
### [266] [MC-CPO: Mastery-Conditioned Constrained Policy Optimization](https://arxiv.org/abs/2604.04251)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: MC-CPO算法通过将教学结构嵌入可行动作空间，在强化学习教学系统中约束短期行为信号，防止奖励黑客行为，确保学习效果优先


<details>
  <summary>Details</summary>
Motivation: 自适应教学系统可能过度优化短期行为信号而忽视长期学习效果，导致强化学习策略出现奖励黑客行为，需要将教学安全约束直接整合到策略优化中

Method: 提出Mastery-Conditioned Constrained Policy Optimization (MC-CPO)，一种双时间尺度的原始-对偶算法，结合结构动作屏蔽与约束策略优化，将学习者掌握程度和先决条件结构作为动态约束

Result: 在表格环境和神经教学设置中验证，MC-CPO在约束预算内满足要求，降低安全成本，显著减少奖励黑客严重性指数(RHSI)，证明在掌握条件可行集内优化优于后处理过滤

Conclusion: 将教学结构直接嵌入可行动作空间为缓解教学强化学习系统中的奖励黑客行为提供了原则性基础，确保教学安全约束与学习目标的一致性
...
### [267] [Context Engineering: A Practitioner Methodology for Structured Human-AI Collaboration](https://arxiv.org/abs/2604.04258)
*Elias Calboreanu*

Main category: cs.AI

TL;DR: 论文提出"上下文工程"方法，通过结构化组装上下文信息包来提升AI输出质量，实验显示能显著减少迭代次数并提高首次通过率


<details>
  <summary>Details</summary>
Motivation: 当前AI输出质量常被归因于提示技术，但实证观察表明上下文完整性可能对输出质量有更强关联。现有方法缺乏系统化的上下文管理框架

Method: 提出上下文工程方法：定义五角色上下文包结构（权威、范例、约束、评分标准、元数据），应用四阶段流水线（评审者到设计者到构建者到审计者），并采用可靠性工程和信息论的形式模型作为事后解释工具

Result: 在四个AI工具的200次交互观察研究中，结构化上下文组装使平均迭代次数从3.8降至2.0，首次通过率从32%提升至55%。结构化交互中110/200首次通过，基线仅16/50。最终成功率可达91.5%。生产自动化系统（11个操作通道，2132个分类工单）提供了初步佐证

Conclusion: 上下文完整性是AI输出质量的关键因素，上下文工程提供了一种系统化方法来管理上下文信息，能显著减少迭代并提高成功率。虽然结果是观察性的且来自单一操作者数据集，但为更有效的AI交互提供了实用框架
...
### [268] [Beyond Fluency: Toward Reliable Trajectories in Agentic IR](https://arxiv.org/abs/2604.04269)
*Anushree Sinha,Srivaths Ranganathan,Debanshu Das,Abhishek Dharmaratnakar*

Main category: cs.AI

TL;DR: 论文提出信息检索正从被动文档排序转向自主智能体工作流，强调在多步推理-行动-观察循环中，早期小错误会级联放大，导致内部推理与外部工具执行的功能失调，需要从端点准确性转向轨迹完整性和因果归因。


<details>
  <summary>Details</summary>
Motivation: 工业智能体系统中观察到的故障模式表明，在长时程轨迹中，早期微小错误会级联放大，导致内部推理与外部工具执行的功能失调，尽管语言流畅性持续存在。当前系统过于关注端点准确性，而忽视了轨迹完整性和因果归因，这对安全部署构成挑战。

Method: 提出在每个交互单元设置验证门，并在校准的不确定性下进行系统性的弃权。通过将故障模式分类为规划、检索、推理和执行四个维度，并强调过程正确性和基于事实的执行优先于看似合理但未经验证的完成。

Result: 论文提出了一个框架来应对复合错误和欺骗性流畅性，主张可靠的智能体信息检索系统必须优先考虑过程正确性和基于事实的执行，而不是看似合理但未经验证的完成。

Conclusion: 安全部署智能体信息检索系统需要超越端点准确性，转向轨迹完整性和因果归因。通过验证门和系统性弃权机制，可以解决复合错误和欺骗性流畅性问题，确保过程正确性和基于事实的执行优先于看似合理但未经验证的完成。
...
### [269] [PanLUNA: An Efficient and Robust Query-Unified Multimodal Model for Edge Biosignal Intelligence](https://arxiv.org/abs/2604.04297)
*Marija Zelic,Anna Tegon,Yawei Li,Thorir Mar Ingolfsson,Luca Benini*

Main category: cs.AI

TL;DR: PanLUNA是一个5.4M参数的跨模态生理信号基础模型，能同时处理EEG、ECG和PPG信号，在资源受限设备上实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有的生理信号基础模型大多局限于单一模态（如EEG、ECG或PPG），主要原因是缺乏配对的跨模态数据集。需要开发能够处理多种生理信号的紧凑模型。

Method: 扩展LUNA的通道统一模块，将多模态通道视为统一查询集中的条目，并添加传感器类型嵌入，实现高效的跨模态早期融合，同时保持对推理时缺失模态的鲁棒性。

Result: PanLUNA在性能上匹配或超过比它大57倍的模型：TUAB异常EEG检测达到81.21%平衡准确率，HMC多模态睡眠分期达到0.7416平衡准确率（SOTA）。INT8量化训练恢复≥96%全精度性能，在GAP9微控制器上实现高效推理。

Conclusion: PanLUNA是一个紧凑的跨模态生理信号基础模型，能够高效处理多种生理信号，在资源受限的穿戴设备上实现高性能推理，为实际临床应用提供了可行方案。
...
### [270] [RESCORE: LLM-Driven Simulation Recovery in Control Systems Research Papers](https://arxiv.org/abs/2604.04324)
*Vineet Bhat,Shiqing Wei,Ali Umut Kaypak,Prashanth Krishnamurthy,Ramesh Karri,Farshad Khorrami*

Main category: cs.AI

TL;DR: 论文提出了RESCORE框架，用于自动从控制研究论文中重建可执行仿真代码，解决了参数不明确和实现细节模糊的问题，在CDC论文基准上实现了40.7%的成功恢复率。


<details>
  <summary>Details</summary>
Motivation: 控制研究论文中的数值仿真重建常因参数不明确和实现细节模糊而受阻，手动验证耗时耗力，需要自动化系统来加速研究复现过程。

Method: 提出RESCORE框架，包含三个LLM智能体组件：分析器（Analyzer）、编码器（Coder）和验证器（Verifier），通过迭代执行反馈和视觉比较来提高重建保真度。

Result: 在IEEE CDC会议的500篇论文基准上，RESCORE成功恢复了40.7%的任务一致仿真，优于单次生成方法，自动化流程比人工复制估计快10倍。

Conclusion: RESCORE框架能有效自动化控制研究论文的仿真重建，显著减少验证时间，将发布基准和智能体以促进研究复现的社区进展。
...
### [271] [Domain-Contextualized Inference: A Computable Graph Architecture for Explicit-Domain Reasoning](https://arxiv.org/abs/2604.04344)
*Chao Li,Yuru Wang,Chunyu Zhao*

Main category: cs.AI

TL;DR: 提出一种计算基板无关的推理架构，将领域作为显式的一类计算参数，实现领域范围剪枝、多基板执行和透明推理链


<details>
  <summary>Details</summary>
Motivation: 传统推理系统通常与特定计算基板（符号、神经、向量等）紧密耦合，缺乏统一的架构来处理不同领域和计算模式，导致推理过程不透明且效率受限

Method: 建立五层架构：领域作为显式参数、三种领域计算模式（链索引、路径遍历作为Kleisli组合、向量引导计算作为基板转换）、基板无关接口（查询、扩展、桥接）、可靠性条件（C1-C4）和三类故障模式

Result: 实现领域范围剪枝将每查询搜索空间从O(N)减少到O(N/K)，支持符号、神经、向量和混合基板的独立执行，提供携带评估上下文的透明推理链

Conclusion: 本文贡献是架构性的而非逻辑性的，通过形式化计算理论（操作语义、复杂度边界、单子结构、基板转换和边界条件）为跨领域、跨基板的推理系统提供统一框架
...
### [272] [RoboPhD: Evolving Diverse Complex Agents Under Tight Evaluation Budgets](https://arxiv.org/abs/2604.04347)
*Andrew Borthwick,Stephen Ash,Anthony Galczak*

Main category: cs.AI

TL;DR: 该论文首次系统比较了三种LLM引导的进化优化算法（Elo锦标赛选择、帕累托选择、贪婪爬山法），在固定评估预算下，RoboPhD在四个基准测试中的三个表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着LLM引导的智能体进化系统（如GEPA和Autoresearch）的快速发展，一个核心问题变得至关重要：在相同信息、相同种子智能体和相同目标下，哪种优化算法在相同的评估预算下能产生最佳结果？这个问题在评估成本高昂时（如需要人工判断或多个LLM调用）尤为重要。

Method: 研究系统比较了三种优化范式：1) Elo锦标赛选择（RoboPhD），采用无验证进化，使用训练数据上的Elo竞争同时评估智能体和驱动进化；2) 帕累托选择（GEPA）；3) 贪婪爬山法（Autoresearch）。所有系统都接收带有诊断print()语句的种子智能体，使进化能够发展自检测智能体。在四个基准测试（抽象推理、云调度、SQL生成、金融QA）上，使用固定的1,500次评估预算进行比较。

Result: 使用单一默认配置，RoboPhD在四个基准测试中的三个表现优于GEPA和Autoresearch，仅在最简单的任务上落后（该任务获胜解决方案需要不到90行代码）。在ARC-AGI基准测试中，RoboPhD将22行的种子智能体进化为1,013行的多策略系统，使用Gemini 3.1 Flash Lite作为求解器，准确率从27.8%提高到65.8%。

Conclusion: RoboPhD的无验证进化方法（通过Elo竞争同时评估和驱动进化）在大多数任务中优于其他优化范式，特别是在评估预算有限的情况下。研究将RoboPhD作为多功能工具包以MIT许可证发布，提供简单的optimize_anything() API用于进化各种复杂智能体。
...
### [273] [REAM: Merging Improves Pruning of Experts in LLMs](https://arxiv.org/abs/2604.04356)
...
<details>
  <summary>Details</summary>
Motivation: MoE大语言模型虽然性能优异，但参数量巨大（数百亿参数）带来了显著的内存部署挑战。传统的权重剪枝和量化方法存在局限性，特别是REAP方法通过剪枝专家来减少内存需求，但可能损失性能。

Method: 提出Router-weighted Expert Activation Merging (REAM)方法，不同于剪枝专家的REAP方法，REAM将专家分组并合并它们的权重，从而更好地保留原始模型的性能。

Result: 在多个MoE LLM模型上评估REAM，涵盖多样化的多项选择题回答和生成式基准测试。结果显示MC和GEN性能之间存在权衡，这种权衡取决于校准数据的组合。通过控制通用、数学和编程数据的混合，研究了这种权衡的帕累托前沿，发现REAM通常优于基线方法，在许多情况下与原始未压缩模型相当。

Conclusion: REAM作为一种新的MoE LLM压缩方法，通过合并而非删除专家权重，在减少内存需求的同时更好地保持了模型性能，在多项任务上表现优于传统剪枝方法。
...
### [274] [Optimizing Service Operations via LLM-Powered Multi-Agent Simulation](https://arxiv.org/abs/2604.04383)
...
<details>
  <summary>Details</summary>
Motivation: 服务系统性能取决于参与者对设计选择的响应，但由于人类行为的复杂性，建模这些响应非常困难。需要一种能够有效模拟人类行为响应并优化服务运营的方法。

Method: 引入LLM驱动的多智能体仿真框架，将问题表述为具有决策依赖不确定性的随机优化问题。通过在提示词中嵌入关键数值信息并从LLM生成的文本中提取，将不确定性建模为受控马尔可夫链。开发了在单次仿真运行中同时构建零阶梯度估计和更新设计参数的在线轨迹学习算法，并整合了方差减少技术。

Result: 在可持续供应链应用中，该方法优于基准方法，包括黑盒优化和使用LLM作为数值求解器或角色扮演系统设计者。在基于真实行为数据的最优竞赛设计案例研究中，LLM-MAS既能作为已知设计的成本效益评估器，又能作为探索性工具，发现传统方法忽视的强大设计。

Conclusion: LLM驱动的多智能体仿真框架为优化服务运营提供了一种有效方法，能够建模复杂的人类行为响应，并通过在线学习算法优化设计参数，在多个应用中展现出优于传统方法的性能。
...
### [275] [Automatically Generating Hard Math Problems from Hypothesis-Driven Error Analysis](https://arxiv.org/abs/2604.04386)
*Jiayu Fu,Mourad Heddaya,Chenhao Tan*

Main category: cs.AI

TL;DR: 提出一种新的数学基准测试生成流程，利用AI生成假设来识别LLM在特定数学概念和技能上的弱点，并针对这些弱点生成新的测试问题。


<details>
  <summary>Details</summary>
Motivation: 现有的数学基准测试大多需要大量人工努力且难以扩展，无法跟上LLM的发展速度，也难以提供新实例来缓解过拟合问题。现有自动生成方法很少关注识别LLM在哪些具体数学概念和技能上容易出错，且大多只能生成特定类别的基准测试。

Method: 提出一个数学基准测试生成流程：1) 使用AI生成假设来识别LLM在哪些具体数学概念和技能上容易出错；2) 针对这些弱点生成新的基准测试问题。该流程具有高度适应性，可应用于数学以外的领域。

Result: 实验表明，假设准确性与生成问题的难度呈正相关：基于最准确假设生成的问题将Llama-3.3-70B-Instruct的准确率降低至45%（原始MATH基准测试上为77%）。

Conclusion: 该流程不仅能生成更具挑战性的数学基准测试，还能扩展到其他领域探索LLM的各种能力，成为研究LLM在不同领域表现的有价值工具。
...
### [276] [MolDA: Molecular Understanding and Generation via Large Language Diffusion Model](https://arxiv.org/abs/2604.04403)
*Seohyeon Shin,HanJun Choi,Jun-Hyung Park,Hongkook Kim,Mansu Kim*

Main category: cs.AI

TL;DR: MolDA是一种新型多模态分子发现框架，用离散大语言扩散模型替代传统自回归架构，通过双向迭代去噪确保分子结构全局一致性和化学有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于自回归骨干的多模态分子架构存在局限性：严格的从左到右归纳偏差不适合生成化学有效分子，难以处理非局部全局约束（如环闭合），且在顺序生成过程中容易累积结构错误。

Method: 提出MolDA框架：1) 使用混合图编码器提取全面的结构表示，捕获局部和全局拓扑；2) 通过Q-Former将结构表示对齐到语言标记空间；3) 用离散大语言扩散模型替代传统自回归骨干；4) 为掩码扩散专门数学重构分子结构偏好优化；5) 通过双向迭代去噪确保全局结构一致性。

Result: MolDA在分子生成、描述和性质预测等任务中展现出全局结构一致性、化学有效性和鲁棒推理能力。

Conclusion: MolDA通过离散扩散模型克服了自回归架构在分子生成中的局限性，为多模态分子发现提供了更有效的框架，能够更好地处理全局约束和确保化学有效性。
...
### [277] [Empirical Characterization of Rationale Stability Under Controlled Perturbations for Explainable Pattern Recognition](https://arxiv.org/abs/2604.04456)
*Abu Noman Md Sakib,Zhensen Wang,Merjulah Roby,Zijie Zhang*

Main category: cs.AI

TL;DR: 提出一种评估模型解释一致性的新指标，通过计算相同标签输入的SHAP值余弦相似度来检测模型解释的稳定性，并在情感分析数据集上验证其有效性。
...
Method: 使用预训练的BERT模型在SST-2情感分析数据集上实现新指标，并在RoBERTa、DistilBERT和IMDB数据集上进行鲁棒性测试。应用SHAP计算特征重要性，通过相同标签输入的SHAP值余弦相似度量化解释一致性。

Result: 通过一系列实验评估该指标识别错误预测和解释不一致性的能力，并与标准保真度指标进行比较。实验表明该指标能有效检测模型行为偏离预期目标的情况。

Conclusion: 提出的框架通过量化模型在相似输入上是否依赖一致的归因模式，支持对模型行为进行更鲁棒的评估，为构建可信AI系统提供了更深入的理解和验证工具。
...
### [278] [The Topology of Multimodal Fusion: Why Current Architectures Fail at Creative Cognition](https://arxiv.org/abs/2604.04465)
*Xiujiang Tan*

Main category: cs.AI

TL;DR: 该论文识别了当前多模态AI架构中的结构限制，这种限制是拓扑性的而非参数性的。作者提出了一个基于哲学、认知科学和数学三支柱的十字形框架，并设计了实验验证方案。


<details>
  <summary>Details</summary>
Motivation: 当前多模态AI架构（如CLIP、GPT-4V/Gemini、扩散模型）存在共同的几何先验——模态可分性（接触拓扑），这限制了AI对模态间深层交互的理解能力。作者旨在从哲学、认知科学和数学三个层面构建新的理论框架来突破这一限制。

Method: 1. 哲学支柱：重新诠释维特根斯坦的"说/显"区分，引入中国工艺认识论中的"象"概念，构建十字形框架（道/器 × 说/显）
2. 认知科学支柱：通过病理镜像重新解释DMN/ECN/SN三部分共激活，在二维参数空间（耦合强度 × 调节能力）中分析
3. 数学支柱：使用纤维丛和杨-米尔斯曲率形式化这些概念
4. 提出UOO实现方案（带拓扑正则化的神经ODE）、ANALOGY-MM基准（错误类型比度量）和META-TOP三层基准

Result: 论文提出了完整的理论框架和实验方案，但尚未展示具体实验结果。设计了分阶段实验路线图，包含明确的终止标准，确保如果被证伪可以干净退出。

Conclusion: 该研究识别了多模态AI的拓扑限制，提出了跨哲学、认知科学和数学的综合理论框架，并设计了系统性的实验验证方案，为突破当前多模态AI的模态可分性限制提供了新的理论视角和方法路径。
...
### [279] [What Makes a Sale? Rethinking End-to-End Seller--Buyer Retail Dynamics with LLM Agents](https://arxiv.org/abs/2604.04468)
*Jeonghwan Choi,Jibin Hwang,Gyeonghun Sun,Minjeong Ban,Taewon Yun,Hyeonjae Cheon,Hwanjun Song*

Main category: cs.AI

TL;DR: RetailSim是一个端到端的零售模拟框架，能够统一建模从卖家说服到买家决策的完整零售流程，解决了现有模拟器只关注部分环节且缺乏跨阶段依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 现有零售模拟器存在局限性：它们只捕捉零售流程的部分环节，无法建模跨阶段依赖关系，难以评估早期决策如何影响下游结果。这导致在实际部署前难以有效评估零售策略。

Method: 提出了RetailSim框架，通过统一环境建模完整零售流程。该框架具备三个关键设计：多样化的产品空间、基于角色的智能体驱动、多轮交互机制，专门为模拟保真度而设计。

Result: 通过双重评估协议验证：1）人类评估行为保真度；2）与现实世界经济规律进行元评估。结果显示RetailSim成功复现了关键模式，包括人口统计购买行为、价格-需求关系、异质价格弹性等。

Conclusion: RetailSim展示了作为零售策略探索可控测试平台的实用价值，通过角色推断、买卖双方交互分析、销售策略评估等用例，证明了其在零售策略评估方面的潜力。
...
### [280] [On the "Causality" Step in Policy Gradient Derivations: A Pedagogical Reconciliation of Full Return and Reward-to-Go](https://arxiv.org/abs/2604.04686)
*Nima H. Siboni*

Main category: cs.AI

TL;DR: 本文对策略梯度中REINFORCE估计器的推导进行了数学严谨的重新表述，展示了"奖励累计"如何直接从前缀轨迹分布的分解中自然产生，而非作为事后启发式替换。


<details>
  <summary>Details</summary>
Motivation: 在介绍策略梯度时，通常先推导使用完整轨迹回报的REINFORCE估计器，然后基于"因果性"声称可以用"奖励累计"替换完整回报。虽然这一说法正确，但推导的严谨性不足，未能清晰说明过去奖励项如何消失。本文旨在提供更数学明确的推导。

Method: 采用前缀轨迹分布和得分函数恒等式，对策略梯度目标进行数学严谨的分解。通过将目标分解为前缀轨迹的形式，直接从数学推导中得出"奖励累计"项，而非将其作为事后启发式替换。

Result: 推导出的估计器与标准REINFORCE估计器相同，但提供了更清晰的概念框架。"奖励累计"项直接从目标分解中自然产生，而非作为事后替换。通常的因果性论证成为推导的推论而非额外启发式原则。

Conclusion: 本文提供了策略梯度推导的数学严谨表述，澄清了"奖励累计"如何从前缀轨迹分解中自然产生。这一重新表述虽不改变估计器本身，但在概念上更清晰，将因果性论证从启发式原则提升为数学推导的自然结果。
...
### [281] [AI Assistance Reduces Persistence and Hurts Independent Performance](https://arxiv.org/abs/2604.04721)
*Grace Liu,Brian Christian,Tsvetomira Dumbalska,Michiel A. Bakker,Rachit Dubey*

Main category: cs.AI

TL;DR: AI助手虽能短期提升任务表现，但会降低用户坚持性和无协助时的表现能力，对长期技能发展有负面影响


<details>
  <summary>Details</summary>
Motivation: 当前AI系统设计为提供即时完整回答，缺乏人类导师那种支持长期成长、搭建学习支架的协作方式，需要研究这种短期优化对用户长期能力发展的影响

Method: 通过随机对照试验（N=1,222）研究人机交互，测试数学推理和阅读理解等任务，比较有AI协助和无AI协助时的表现差异

Result: AI协助虽能短期提升表现，但会导致用户在没有AI时表现显著变差，更容易放弃任务；这些影响在仅10分钟的AI交互后就会出现
...
### [282] [Incompleteness of AI Safety Verification via Kolmogorov Complexity](https://arxiv.org/abs/2604.04876)
*Munawar Hasan*

Main category: cs.AI

TL;DR: 该论文证明AI安全验证存在基本的信息理论限制：任何固定的可计算枚举验证器都存在一个复杂度阈值，超过该阈值后无法证明所有符合策略的实例，揭示了AI安全验证的内在局限性。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统满足正式安全和策略约束是安全关键领域的核心挑战。传统上验证的局限性常归因于组合复杂性和模型表达能力，但本文旨在揭示这些限制源于内在的信息理论极限。

Method: 将策略合规性形式化为编码系统行为的验证问题，使用柯尔莫哥洛夫复杂度进行分析。证明了一个不完备性结果：对于任何固定的可计算枚举验证器，存在一个复杂度阈值，超过该阈值后无法证明所有符合策略的实例。

Result: 证明了AI安全验证存在基本的信息理论限制：没有有限的正式验证器能够证明所有任意高复杂度的策略合规实例。这揭示了独立于计算资源的AI安全验证基本局限性。

Conclusion: AI安全验证存在内在的信息理论限制，这激励了采用携带证明的方法，为实例级正确性提供保证。验证的局限性不仅源于计算复杂性，更源于信息理论的基本约束。
...
### [283] [Learning, Potential, and Retention: An Approach for Evaluating Adaptive AI-Enabled Medical Devices](https://arxiv.org/abs/2604.04878)
*Alexis Burgon,Berkman Sahiner,Nicholas A Petrick,Gene Pennello,Ravi K Samala*

Main category: cs.AI

TL;DR: 本文提出了一种评估医疗设备自适应AI模型的新方法，通过三个互补指标（学习、潜力、保留）来区分模型适应与动态环境变化对性能的影响，为监管科学提供实用工具。


<details>
  <summary>Details</summary>
Motivation: 医疗设备自适应AI模型的评估面临挑战，因为模型和评估数据集都会迭代更新，使得性能评估变得复杂。需要一种方法来区分性能变化是由模型适应还是动态环境变化引起的。

Method: 提出包含三个互补测量的新方法：学习（模型在当前数据上的改进）、潜力（数据集驱动的性能变化）、保留（跨修改步骤的知识保持），以解耦模型适应与环境变化的影响。

Result: 通过模拟人口变化的案例研究表明：渐进过渡可实现稳定的学习和保留，而快速变化则揭示了可塑性与稳定性之间的权衡。这些测量为评估自适应AI系统提供了实用见解。

Conclusion: 该方法为监管科学提供了实用工具，能够严格评估自适应AI系统在连续修改过程中的安全性和有效性，有助于解决医疗设备AI模型评估中的关键挑战。
...
TL;DR: QED-Nano是一个4B参数的开放模型，通过三阶段训练方法在奥林匹克数学竞赛级别的证明问题上达到了与大型专有模型相竞争的性能，同时显著降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 专有AI系统在复杂证明问题上表现出色，但其训练流程不透明、依赖大型模型和架构，导致运行昂贵、难以复现和研究。本文旨在探索小型开放模型是否也能在奥林匹克数学竞赛级别的推理任务上达到竞争性性能。

Method: 采用三阶段训练方法：1) 从DeepSeek-Math-V2进行监督微调以获得良好证明写作风格；2) 基于评分标准的强化学习；3) 扩展强化学习，引入推理缓存机制，将长证明分解为迭代的总结-精炼循环，增强测试时推理能力。

Result: QED-Nano在证明生成性能上超越了包括Nomos-1和GPT-OSS-120B在内的更大规模开放模型，接近Gemini 3 Pro等专有模型的性能，同时推理成本大幅降低。

Conclusion: 小型开放模型通过精心设计的训练方法可以在奥林匹克数学竞赛级别的推理任务上达到竞争性性能。作者发布了完整的QED-Nano管道，包括模型、数据集和代码，以支持开放数学推理的进一步研究。
