...
TL;DR: 提出FD-RL方法解决OCR模型在格式化文本（公式、表格等）上输出熵值过高的问题，通过基于熵的数据过滤和格式解耦奖励机制，在OmniDocBench基准上达到90.41分的新记录。


<details>
  <summary>Details</summary>
Motivation: 研究发现即使是先进的OCR模型在处理格式化文本（如公式、表格）时，其输出熵值比处理纯文本时高出数量级，这表明模型在处理格式敏感文档时存在高不确定性，需要改进推理能力。

Method: 提出格式解耦强化学习（FD-RL），采用基于熵的数据过滤策略识别格式密集型实例，并使用针对不同格式类型定制的格式解耦奖励机制，实现格式级验证而非词元级记忆。

Result: FD-RL在OmniDocBench基准测试中达到平均90.41分，为端到端模型在该流行基准上创造了新记录。通过全面的消融研究验证了数据、训练、过滤和奖励策略的有效性。

Conclusion: 通过分析OCR模型在格式化文本上的高熵模式，提出的FD-RL方法有效解决了格式敏感文档的识别问题，通过格式解耦的强化学习策略显著提升了OCR性能。
...
TL;DR: 该研究探讨了视觉语言模型和生成式图像模型在输入变换下的鲁棒性和公平性问题，特别关注图像旋转和分布偏移导致的偏见传播与性能退化，并提出了基于数据增强、表示对齐和模型正则化的旋转鲁棒缓解策略。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型和生成式图像模型在多模态任务中表现出色，但其在输入变换下的鲁棒性和公平性尚未得到充分探索。当前模型在面对图像旋转和分布偏移时可能存在偏见传播和性能退化问题，这限制了实际应用的可靠性。

Method: 研究分析了旋转引起的扰动对模型预测、置信度校准和人口统计偏见模式的影响。为解决这些问题，提出了旋转鲁棒的缓解策略，包括数据增强、表示对齐和模型级正则化相结合的方法。
...
TL;DR: R²BD是一个基于重建的AIGC图像检测框架，通过统一的G-LDM模型模拟多种生成模型行为，单步推理实现高效检测，比现有方法快22倍以上，在跨数据集评估中平均提升13.87%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于重建的AIGC图像检测方法虽然能减少对数据集特定伪影的依赖并提高分布偏移下的泛化能力，但存在两个主要限制：1）多步反演和重建导致效率低下；2）依赖扩散模型骨干限制了其对GAN等其他生成范式的泛化能力。

Method: 提出了R²BD框架，包含两个关键设计：1）G-LDM统一重建模型，模拟VAEs、GANs和扩散模型的生成行为，扩大检测范围；2）残差偏差计算模块，通过单步推理区分真假图像，显著提高效率。

Result: 在10个公共数据集的基准测试中，R²BD比现有基于重建的方法快22倍以上，同时达到更优的检测准确率。在跨数据集评估中，平均比最先进方法提升13.87%，展现出对多种生成方法的强大效率和泛化能力。

Conclusion: R²BD通过统一的生成模型模拟和单步推理机制，有效解决了现有基于重建的AIGC图像检测方法在效率和泛化能力方面的限制，为AIGC图像检测提供了高效且通用的解决方案。
...
Result: 在七个不同测试集上平均准确率达到86.8%，显著优于现有方法（<75%）；对JPEG压缩具有强鲁棒性（Q=70时83%准确率）；像素级伪造定位F1分数0.76。

Conclusion: 该工作连接了经典图像取证和现代深度学习，为实际部署提供了实用解决方案，能够在操纵技术未知的情况下有效检测伪造内容。
...
<details>
  <summary>Details</summary>
Motivation: 当源图像和目标图像存在系统性强度差异时，传统配准方法依赖的亮度恒定假设被破坏，导致对应关系估计变得不适定。跨域配准在计算机视觉和医学成像中是一个基本挑战。

Method: 提出SAR-Net统一框架，通过场景-外观解耦解决跨域配准问题。将观测图像分解为域不变的场景表示和域特定的外观编码，通过重渲染实现配准而非直接强度匹配。建立了理论条件证明这种分解能够实现一致的跨域对齐。

Result: 在双向扫描显微镜数据集上验证，SAR-Net达到0.885 SSIM和0.979 NCC，比最强基线提升3.1倍，同时保持实时性能（77 fps）。消融研究证实场景一致性和域对齐损失都是必要的。

Conclusion: SAR-Net通过场景-外观解耦为跨域图像配准提供了一个理论严谨且实用的解决方案，在存在系统性强度差异的情况下实现了鲁棒的几何对应关系估计。
...
TL;DR: 该论文提出了"语义生命周期"作为统一框架，描述基础模型驱动的具身AI中语义知识的演化过程，从获取、表示到存储三个关键阶段进行分析。
...
Method: 提出"语义生命周期"统一框架，将语义处理视为连续的知识流和维护过程，而非孤立模块。基于该框架分析具身AI中语义知识的三个关键阶段：获取、表示和存储。

Result: 基础模型通过跨领域泛化能力和丰富的语义先验，重塑了具身AI研究格局。该框架为理解语义知识在具身系统中的演化提供了整体视角。

Conclusion: 语义生命周期框架为具身AI研究提供了新的分析视角，总结了现有挑战并指出了未来研究方向，强调需要更系统的语义知识管理方法。
...
TL;DR: 使用DLRT流形约束优化框架压缩基于视觉Transformer的地理空间基础模型，在迁移学习中实现强压缩同时保持任务特定精度，优于LoRA等现成低秩方法
...
Method: 利用DLRT流形约束优化框架，在迁移学习期间压缩大型视觉Transformer地理空间基础模型，通过强制执行与下游目标对齐的结构化低维参数化

Result: 该方法优于LoRA等现成低秩方法，在多样化地理空间基准测试中实现显著参数减少且精度损失最小

Conclusion: DLRT压缩方法能够实现高性能、可在设备上运行的地理空间模型，解决了资源受限边缘设备部署的挑战
...
TL;DR: UAMSA-UNet：一种不确定性感知的多尺度注意力贝叶斯U-Net，通过蒙特卡洛dropout学习数据驱动的平滑先验，融合多尺度特征和注意力机制，在脑肿瘤分割中实现更精确的边界和更高的计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有CNN和U-Net方法在肿瘤浸润区域产生噪声边界，需要更精确的脑肿瘤分割方法以支持诊断和治疗规划。

Method: 提出UAMSA-UNet，结合不确定性感知、多尺度注意力和贝叶斯方法，使用蒙特卡洛dropout学习数据驱动的平滑先验，通过平滑正则化损失（二元交叉熵加方差惩罚）减少虚假波动。

Result: 在BraTS2023上，Dice相似系数提升3.3%，平均IoU提升2.7%；在BraTS2024上，Dice提升4.5%，IoU提升4.0%；相比U-Net++减少42.5%的FLOPs同时保持更高精度。

Conclusion: 通过结合多尺度注意力和学习平滑先验，UAMSA-UNet实现了更好的分割质量和计算效率，为未来与transformer模块集成提供了灵活基础。
...
TL;DR: Thermo-LIO：一种融合热成像与高分辨率LiDAR的多传感器系统，通过结合热成像、LiDAR和惯性里程计，实现大规模结构健康监测中温度分布和缺陷的精确检测。


<details>
  <summary>Details</summary>
Motivation: 传统二维热成像技术在建筑领域虽然非侵入且能检测缺陷，但在处理复杂几何形状、难以接近区域和地下缺陷方面存在局限，需要更先进的监测方法。

Method: 1. 开发多模态融合方法，结合热成像和LiDAR，实现精确校准和同步；2. 将该融合方法与LiDAR-惯性里程计（LIO）集成，实现大规模结构的全覆盖监测。

Result: 实验验证（桥梁和厅堂建筑案例）表明，Thermo-LIO比传统方法能更准确地检测热异常和结构缺陷，提高诊断精度，支持实时处理，扩大检测覆盖范围。

Conclusion: Thermo-LIO系统通过多模态传感器集成，显著推进了大规模土木基础设施结构健康监测方法的发展，展示了传感器融合在提高监测能力方面的关键作用。
...
TL;DR: 该研究通过PoseMaskRefine方法对SAM 2.1进行姿态引导的微调，使其在遮挡情况下实现更鲁棒的人体分割，仅需少量高可见性关键点即可准确预测掩码。


<details>
  <summary>Details</summary>
Motivation: SAM在人体分割方面提供了强大的基础，但在遮挡情况下表现不佳，因为关键点可能部分或完全不可见。需要改进SAM在遮挡场景下的分割能力。

Method: 1. 对SAM 2.1进行最小化的编码器修改，保留其强大的泛化能力；2. 提出PoseMaskRefine微调策略，将高可见性的姿态关键点整合到SAM原有的迭代校正过程中；3. 推理时仅选择可见度最高的三个关键点作为提示，减少对常见错误的敏感性。

Result: 该方法在多个数据集上提高了鲁棒性和准确性，能够从单个关键点准确预测掩码，实现了有效的遮挡感知人体分割，同时保持了原始模型的泛化能力。

Conclusion: 姿态引导的SAM微调能够在保持模型泛化能力的同时，实现有效的遮挡感知人体分割，仅需少量高可见性关键点即可获得准确结果。
...
TL;DR: STDTrack是一个轻量级目标跟踪框架，通过密集视频采样和时空依赖整合，在保持实时效率的同时显著提升性能，在多个基准测试中达到最先进水平。
...
Result: 在六个基准测试中取得了最先进的结果。在GOT-10k上，性能可与某些非实时高性能跟踪器（如MixFormer）相媲美，同时在GPU上达到192 FPS，CPU上达到41 FPS的实时效率。

Conclusion: STDTrack成功将可靠的时空依赖整合到轻量级跟踪器中，通过密集采样和创新的模块设计，在保持实时效率的同时显著缩小了轻量级与高性能跟踪器之间的性能差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法基于中等规模预训练模型，而更大规模的基础模型能提供更强性能支持，但全参数微调会导致GPU内存消耗大、计算成本高等训练困难，限制了大规模模型在遥感图像分割中的探索。

Method: 提出WEFT方法：1) 任务特定小波专家提取器，从不同角度建模小波专家并动态调节输出，生成富含任务信息的可训练特征；2) 专家引导的条件适配器，通过注入可训练特征增强冻结特征的细粒度感知，并迭代更新两类特征信息，实现高效微调。

Result: 在三个光学遥感图像数据集上超越了21个最先进方法，同时在伪装、自然和医疗场景中也取得了最优结果。

Conclusion: WEFT方法通过动态小波专家引导的微调范式，用更少的可训练参数成功将大规模基础模型适配到遥感图像分割任务，解决了大规模模型微调中的训练困难问题。
...
TL;DR: 提出SAM-Aug框架，利用Segment Anything Model的几何感知分割能力改进少样本遥感图像语义分割，通过无监督SAM生成掩码先验，结合RegionSmoothLoss实现时间一致性正则化。


<details>
  <summary>Details</summary>
Motivation: 少样本时间序列遥感图像语义分割在标注数据稀缺地区面临挑战，现有全监督模型在有限标注下性能显著下降，限制了实际应用。

Method: 构建无云合成图像，无监督应用SAM生成几何感知掩码先验，提出RegionSmoothLoss损失函数，在SAM衍生区域内强制时间帧间预测一致性，实现语义结构正则化。

Result: 在PASTIS-R基准测试中，5%标注设置下平均测试mIoU达36.21%，优于现有最佳方法2.33个百分点（相对提升6.89%）；最佳分割（seed=42）达40.28% mIoU，相对提升11.2%。

Conclusion: SAM等视觉基础模型可作为少样本遥感学习的有效正则化器，提供无需人工标注或微调的可扩展即插即用解决方案，证明了利用基础模型先验在标注稀缺下的泛化能力。
...
TL;DR: 提出基于Qwen3-VL的端到端结构感知多模态推理框架，通过字符感知多模态推理模块（CMRM）引入可学习的字符槽查询，解决车牌识别中图像恢复与字符识别目标不匹配的问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界车牌识别面临运动模糊、低分辨率、复杂光照等严重退化问题。传统的"恢复-再识别"两阶段范式存在根本缺陷：图像恢复的像素级优化目标与字符识别的语义目标不匹配，导致伪影干扰和错误累积。虽然视觉语言模型（VLMs）展现出强大能力，但缺乏对车牌字符序列（如固定长度、特定顺序）的显式结构建模。

Method: 提出基于Qwen3-VL的端到端结构感知多模态推理框架，核心创新是字符感知多模态推理模块（CMRM）。该模块引入一组可学习的字符槽查询，通过交叉注意力机制从视觉特征中主动检索对应字符位置的细粒度证据。然后将这些字符感知表示通过残差调制注入回视觉标记，使语言模型能够基于显式结构先验进行自回归生成。结合LoRA参数高效微调策略，在保留大模型泛化能力的同时实现领域适应。

Result: 在合成和真实世界严重退化数据集上的大量实验表明，该方法显著优于现有的恢复-识别组合和通用视觉语言模型，验证了将结构化推理融入大模型对于低质量文本识别任务的优越性。

Conclusion: 通过引入字符感知多模态推理模块，成功解决了车牌识别中图像恢复与字符识别目标不匹配的问题，为低质量文本识别任务提供了一种有效的结构化推理方法，展示了将结构化先验融入大模型的优势。
...
Method: 提出SkinFlow框架，采用虚拟宽度动态视觉编码器在不增加物理参数的情况下"展开"复杂病理流形，结合两阶段强化学习策略：第一阶段对齐显式医学描述，第二阶段重建隐式诊断纹理。
...
TL;DR: 模型驱动的训练方法（利用学习的干扰变化模型生成真实干扰）在交通标志识别中优于传统基线方法，模型驱动的对抗训练提供最强鲁棒性但计算成本高，模型驱动的数据增强以更低计算成本实现相当性能


<details>
  <summary>Details</summary>
Motivation: 深度学习在安全敏感领域中对自然干扰的鲁棒性仍然是一个关键挑战，需要更可靠的方法来处理现实世界中的干扰变化

Method: 采用基于模型的学习方法，利用学习的干扰变化模型生成真实干扰，并提出结合随机覆盖和干扰空间对抗精炼的混合策略，在CURE-TSR数据集上评估雪和雨干扰

Result: 基于模型的方法在所有干扰严重程度上始终优于Vanilla、对抗训练和AugMix基线，模型驱动的对抗训练在所有干扰下提供最强鲁棒性但计算成本高，模型驱动的数据增强以T倍更低的计算复杂度实现相当鲁棒性且无显著性能下降
...
TL;DR: 研究评估了5个主流GenAI图像生成平台在建筑图像生成方面的准确性，发现整体准确率有限（32%-52%），常见提示比罕见提示准确率高2.7倍，不同平台在完全错误结果方面差异显著。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI文本到图像系统在建筑图像生成中的应用日益增多，需要评估这些系统在历史规则约束的建筑领域生成准确图像的能力。

Method: 使用30个涵盖建筑风格、类型和规范元素的提示词，在5个主流GenAI平台（Adobe Firefly、DALL-E 3、Google Imagen 3、Microsoft Image Generator、Midjourney）上生成图像。每个提示-生成器组合产生4张图像（共600张）。两位建筑历史学家根据预定义标准独立评分，通过共识解决分歧。将集合级性能总结为每四张图像集合中准确图像的数量（0-4）。

Result: 常见提示的图像输出准确率比罕见提示高2.7倍（p<0.05）。各平台整体准确率有限（最高52%，最低32%，平均42%）。全正确结果（4/4）在各平台间相似，但全错误结果（0/4）差异显著：Imagen 3失败最少，Microsoft Image Generator失败最多。定性分析发现重复模式：过度装饰、中世纪风格与后期复兴风格混淆、描述性提示词误解（如蛋箭饰、带状柱、穹隅）。

Conclusion: 研究结果支持需要对GenAI合成内容进行可见标记、为未来训练数据集建立来源标准，以及在教育中谨慎使用GenAI建筑图像。
...
TL;DR: 提出N EIoU YOLOv9轻量检测框架，使用非单调梯度聚焦和几何解耦的边界框回归损失，在农业病害图像检测中提升小目标和低对比度目标的定位精度，并在移动端实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 农业病害图像中的小目标和低对比度目标检测困难，现有边界框回归损失在处理这类弱回归信号时效果不佳，需要改进定位精度和优化稳定性。

Method: 提出N EIoU损失函数，结合非单调梯度聚焦和宽度高度解耦优化，增强低重叠度困难样本的回归信号，减少梯度干扰。将该损失集成到轻量级YOLOv9t架构中。

Result: 在自收集的5908张水稻叶片图像数据集上测试，平均精度达到90.3%，比基线CIoU提升4.3%。在Android设备上使用TensorFlow Lite Float16量化部署，平均推理时间156毫秒/帧。

Conclusion: N EIoU YOLOv9框架有效平衡了精度、优化稳定性和计算效率，适用于边缘农业监测系统，在移动端实现了准确高效的病害检测。
...
TL;DR: 知识蒸馏框架将高性能医学图像分割模型转换为紧凑学生模型，在保持精度的同时大幅降低计算需求，便于临床部署
...
Method: 提出部署导向的知识蒸馏框架，将高性能分割模型转换为可扩展的紧凑学生模型家族，保持与现有临床系统的架构兼容性

Result: 在参数减少94%的情况下，蒸馏学生模型保留了教师模型98.7%的分割精度，CPU推理延迟降低67%，在多站点脑MRI和腹部CT数据集上验证有效
...
TL;DR: Point-Seg：一种基于Transformer的超声心动图心肌分割框架，通过点跟踪提供时序线索，实现跨帧稳定一致的分割，避免传统方法中的误差累积问题。
...
Method: 提出Point-Seg框架，集成点跟踪作为时序线索。使用在合成超声心动图数据集上训练的点跟踪模块跟踪关键解剖标志点轨迹，提供显式的运动感知信号指导分割，减少漂移并消除基于记忆的特征累积需求。同时引入时序平滑损失增强帧间一致性。

Result: 在公开和私有超声心动图数据集上的实验表明：在高质量数据上，Point-Seg的Dice准确率与最先进模型统计相似；在低质量数据上，分割准确率更高且时序稳定性更好。关键优势是提供像素级心肌运动信息，这对心肌应变测量和区域壁运动异常检测等下游任务至关重要。

Conclusion: Point-Seg证明点跟踪可作为有效的时序线索实现一致的视频分割，为超声心动图视频中的心肌分割提供了可靠且可推广的方法。
...
TL;DR: PaGKD是一种无需配对图像的组级知识蒸馏框架，能够利用未配对的WLI和NBI数据进行跨模态学习，显著提升WLI模型的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 内窥镜癌症筛查中，白光成像(WLI)是标准方法，但窄带成像(NBI)能提供更优的诊断细节。现有方法依赖配对的NBI-WLI图像，这在临床实践中成本高且不切实际，导致大量临床数据无法利用。

Method: 提出PaGKD框架，包含两个互补模块：1) 组级原型蒸馏(GKD-Pro)：通过共享病灶感知查询提取模态不变语义原型；2) 组级密集蒸馏(GKD-Den)：通过激活衍生关系图引导组感知注意力，进行密集跨模态对齐。
...
TL;DR: Affostruction：从RGBD图像生成完整几何并在地面化功能区域，包括未观测区域，通过多视角重建、流式功能区域定位和主动视角选择实现


<details>
  <summary>Details</summary>
Motivation: 现有方法仅能预测可见表面的功能区域，无法处理未观测区域。需要一种能够从部分观测重建完整几何并在全形状上定位功能区域的框架
...
Conclusion: Affostruction通过生成完整几何并在全形状上定位功能区域，显著提升了功能区域定位和3D重建性能，解决了现有方法局限于可见表面的问题
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型推理速度慢，现有松弛推测解码方法缺乏理论依据。需要建立理论框架来优化推测解码，在保证质量的同时提升推理效率。

Method: 提出COOL-SD方法：1）分析目标模型与松弛推测解码之间的总变差距离，推导出最小化距离上界的最优重采样分布；2）通过扰动分析揭示松弛推测解码中的退火行为，设计退火松弛方案。

Result: 实验验证COOL-SD的有效性，在速度-质量权衡方面持续优于现有方法，能以更快的速度生成质量相当的图像，或在相似延迟下获得更好的质量。

Conclusion: COOL-SD为松弛推测解码提供了理论基础，通过退火松弛设计实现了更好的速度-质量权衡，为自回归图像生成的高效推理提供了有效解决方案。
...
TL;DR: SpikeVAEDiff是一个两阶段框架，结合VDVAE和Versatile Diffusion模型，从神经尖峰数据重建高分辨率视觉场景。第一阶段用VDVAE生成低分辨率初步重建，第二阶段通过回归模型映射到CLIP特征，用Versatile Diffusion进行图像到图像生成精炼。


<details>
  <summary>Details</summary>
Motivation: 从神经活动重建自然视觉场景是神经科学和计算机视觉的关键挑战。相比fMRI方法，尖峰数据提供更优越的时间和空间分辨率，但现有方法在从神经尖峰数据生成高分辨率、语义有意义的图像重建方面仍有局限。

Method: 提出SpikeVAEDiff两阶段框架：1) 第一阶段使用非常深的变分自编码器(VDVAE)将神经尖峰信号映射到潜在表示，生成低分辨率初步重建；2) 第二阶段使用回归模型将神经尖峰信号映射到CLIP-Vision和CLIP-Text特征，然后利用Versatile Diffusion模型通过图像到图像生成进行精炼。

Result: 在Allen Visual Coding-Neuropixels数据集上评估，发现VISI区域表现出最显著的激活并在重建质量中起关键作用。展示了成功和失败的重建示例，反映了解码神经活动的挑战。验证了VDVAE模型的有效性，消融研究表明特定脑区域数据显著提升重建性能。

Conclusion: SpikeVAEDiff框架能够从神经尖峰数据生成高分辨率、语义有意义的图像重建。相比fMRI方法，尖峰数据提供更好的时空分辨率。VISI区域在视觉重建中起关键作用，特定脑区域数据对提升重建性能至关重要。
...
TL;DR: 本文提出了一种用于红外目标检测的视觉-语言表示学习范式，通过文本监督引导目标与非目标特征的解耦，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 红外图像由于对比度低、边缘信息弱，难以提取具有判别性的目标特征进行鲁棒检测。需要解决红外图像中目标特征提取困难的问题。

Method: 提出视觉-语言表示学习范式，包含语义特征对齐（SFA）模块将目标特征与文本特征对齐，以及目标特征解耦（OFD）模块通过最小化相关性来解耦文本对齐的目标特征和非目标特征。

Result: 在两个基准数据集上取得了优异性能：M³FD（83.7% mAP）和FLIR（86.1% mAP），通过更判别性和更少噪声的特征显著增强了检测性能。

Conclusion: 通过引入文本监督的视觉-语言表示学习范式，能够有效解耦红外图像中的目标与非目标特征，显著提升红外目标检测的准确性和鲁棒性。
...
Method: 提出跨层独立可变形描述（CLIDD），通过可学习偏移从独立特征层次直接采样捕捉多尺度细粒度结构细节；采用硬件感知内核融合策略最大化推理吞吐量；结合轻量级架构和训练协议，利用度量学习和知识蒸馏生成多种模型变体。

Result: 超紧凑变体仅使用0.004M参数就达到SuperPoint的精度，模型大小减少99.7%；高性能配置超越所有当前最先进方法（包括基于DINOv2的框架），在边缘设备上超过200 FPS。

Conclusion: CLIDD能够以最小计算开销实现高精度局部特征匹配，为实时空间智能任务提供了鲁棒且可扩展的解决方案，在精度和效率之间取得了卓越平衡。
...
<details>
  <summary>Details</summary>
Motivation: 卫星视频提供连续的地表动态观测，但在非稳定条件下（平台抖动和微小物体外观特征弱）面临多目标跟踪挑战，现有方法性能受限

Method: 提出DeTracker联合检测跟踪框架：1）GLMD模块通过全局对齐和局部细化分离卫星平台运动与真实物体运动；2）TDFP模块进行跨帧时序特征融合增强微小物体表征；3）构建SDM-Car-SU基准数据集模拟多方向多速度平台运动
...
TL;DR: A²TG是一种自适应各向异性纹理高斯表示，通过为每个基元配备各向异性纹理，使用梯度引导的自适应规则确定纹理分辨率和纵横比，显著提高纹理效率并降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有方法为每个基元分配固定方形纹理，导致内存使用效率低下且对场景变化的适应性有限，需要更高效的纹理分配方案。

Method: 提出自适应各向异性纹理高斯（A²TG）表示，为每个基元配备各向异性纹理，采用梯度引导的自适应规则联合确定纹理分辨率和纵横比，实现非均匀、细节感知的纹理分配。

Result: 在多个基准数据集上的实验表明，A²TG始终优于固定纹理高斯泼溅方法，以显著更低的内存需求实现相当的渲染保真度。

Conclusion: A²TG通过自适应各向异性纹理分配，显著提高了纹理效率，在保持高质量渲染的同时大幅降低内存消耗，为高斯泼溅表示提供了更高效的纹理增强方案。
...
Method: 提出三阶段流程PhyRPR：1) PhyReason使用大语言模型进行物理状态推理和关键帧生成；2) PhyPlan确定性合成可控粗粒度运动骨架；3) PhyRefine通过潜在融合策略将骨架注入扩散采样，精炼外观同时保持规划动力学。

Result: 在物理约束下的广泛实验表明，该方法持续提升物理合理性和运动可控性。

Conclusion: 通过解耦物理理解与视觉合成的三阶段设计，实现了生成过程中的显式物理控制，有效解决了现有扩散视频生成模型的物理约束问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于卫星影像的火烧迹地制图面临挑战：不规则和空间异质的光谱变化使得准确划定困难。现有深度学习方法虽然在高分辨率多光谱数据可用时能达到高精度，但在实际应用中，需要在火灾发生后快速划定火烧迹地时，受到当前卫星系统空间分辨率与时间重访频率之间权衡的限制。

Method: 提出BAM-MRCD深度学习模型，采用多分辨率、多源卫星影像（MODIS和Sentinel-2）相结合的方法。该模型能够利用不同卫星系统的优势，实现高时空分辨率的火烧迹地制图。

Result: 模型能够高精度检测小规模火灾，超越了类似的变化检测模型和可靠的基线方法。所有数据和代码已在GitHub开源。

Conclusion: BAM-MRCD模型通过整合多源卫星数据，解决了传统方法在空间分辨率与时间频率之间的权衡问题，为及时、详细地划定火烧迹地提供了有效解决方案，具有实际应用价值。
...
TL;DR: BrainSegNet是一个基于SAM改进的深度学习框架，专门用于高精度全脑分割成95个区域，通过集成U-Net跳跃连接和多尺度注意力机制提升解剖精度。


<details>
  <summary>Details</summary>
Motivation: 全脑分割是一个关键但具有挑战性的任务，传统模板配准方法存在局限，现有大型模型如SAM虽然提供可迁移特征表示，但未针对脑分割的高精度需求进行优化。

Method: 提出BrainSegNet框架，改进SAM架构：1) 混合编码器结合U-Net跳跃连接和SAM的transformer块；2) 多尺度注意力解码器带金字塔池化处理不同大小结构；3) 边界细化模块锐化边缘。

Result: 在Human Connectome Project数据集上的实验表明，BrainSegNet优于多种最先进方法，在复杂的多标签分割任务中实现了更高的准确性和鲁棒性。

Conclusion: BrainSegNet成功将SAM适应于全脑分割任务，通过架构改进实现了高精度解剖分割，为脑影像分析提供了有效的深度学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将物理模拟与3D高斯泼溅结合，但主要针对软质可变形材料，脆性断裂问题尚未解决。这源于两个关键障碍：GS表示缺乏具有连贯纹理的体积内部结构，以及缺乏针对高斯的断裂感知模拟方法。

Method: 1. 通过生成模型引导内部高斯密度化，合成逼真的内部结构；2. 集成优化的连续损伤物质点法（CD-MPM），实现高速脆性断裂模拟。

Result: 能够处理复杂场景，包括混合材料物体和多阶段断裂传播，实现先前方法无法达到的效果。实验证明GaussianFluent能够实现具有结构一致内部结构的逼真实时渲染。

Conclusion: GaussianFluent为动态物体状态提供了统一的模拟和渲染框架，在脆性断裂模拟方面取得突破，具有在VR和机器人等下游应用中的潜力。
...
TL;DR: 本文提出了一种多阶段渐进训练策略，用于训练ICT领域的专用图像描述模型（DICModel），该模型在7B参数下超越了32B参数的SOTA模型性能。


<details>
  <summary>Details</summary>
Motivation: ICT行业训练领域专用LLM或构建检索增强生成系统需要大量高价值领域知识，但这些知识不仅隐藏在文本模态中，也存在于图像模态中。传统方法只能解析文档中的文本，缺乏图像描述能力；而多模态LLM虽然能理解图像，但缺乏足够的领域知识。
...
Result: 仅7B参数的DICModel性能优于其他32B参数的SOTA模型：相比7B和32B参数的SOTA模型，BLEU指标分别提升约56.8%和20.8%；在ICT专家构建的客观题上，准确率超过Qwen2.5-VL 32B模型1%。
...
Method: 提出两种方法：1）基于拉普拉斯金字塔的细节损失，通过总损失与细节损失分离生成和控制超分辨率图像与细节图像；2）重复上采样和下采样过程，从多个低分辨率特征中提取多样化信息以增强细节损失效果。

Result: 实验分为两部分：1）设计包含所提方法的CNN模型，取得了最先进的结果，超越了所有当前可用的CNN模型甚至部分基于注意力的模型；2）将方法应用于小规模现有注意力模型，所有添加细节损失的注意力模型相比原始版本均有改进。

Conclusion: 所提出的方法能有效增强超分辨率图像的高频细节，在不同模型结构上均表现出良好效果，证明了这些方法的普适性和有效性。
...
TL;DR: 该研究提出两种改进骨肉瘤坏死区域自动量化方法：1）使用放射组学特征作为多模态输入提升分类性能；2）采用分层损失函数优化肿瘤vs非肿瘤、存活vs非存活两个二元分类任务，在TCIA数据集上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 骨肉瘤新辅助化疗后，准确评估存活与非存活肿瘤区域对预后和治疗规划至关重要。传统人工评估劳动密集、主观性强、观察者间差异大。现有深度学习模型在患者级别测试数据上性能显著下降，需要改进。

Method: 提出两种创新方法：1）在模型训练中加入放射组学特征作为多模态输入；2）采用分层损失函数，优化肿瘤vs非肿瘤和存活vs非存活两个二元分类任务，而非传统的三类"扁平"分类，并使用可训练的任务权重。

Result: 在TCIA骨肉瘤肿瘤评估数据集上，两种方法均显著提升分类性能，放射组学特征增强了可解释性，分层损失函数显著改善了各类别性能，组合方法达到该应用在公开数据集上的新SOTA性能。

Conclusion: 放射组学特征作为多模态输入和分层损失函数能有效提升骨肉瘤坏死区域自动量化性能，解决了患者级别泛化能力下降问题，为临床病理评估提供了更可靠的工具。
...
TL;DR: Video-MSR是首个评估动态视频场景中多跳空间推理能力的基准，包含4个任务和3052个视频实例，揭示了当前MLLMs在多步空间推理中的局限性，并通过MSR-9K数据集微调Qwen-VL取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单步感知到判断任务，缺乏对需要复杂视觉-空间逻辑链场景的评估。为了填补这一空白，作者提出了Video-MSR基准，专门评估动态视频场景中的多跳空间推理能力。

Method: 1. 构建Video-MSR基准：包含约束定位、链式参考检索、路径规划和反事实物理推理四个任务，共3052个高质量视频实例和4993个问答对；2. 通过结合先进模型生成和严格人工验证的可扩展视觉基础流程构建数据；3. 评估20个最先进的MLLMs；4. 创建MSR-9K指令调优数据集并微调Qwen-VL模型。

Result: 评估发现当前MLLMs在表面感知方面表现良好，但在多跳空间推理任务中性能显著下降，经常出现空间迷失和幻觉问题。通过MSR-9K数据集微调Qwen-VL后，在Video-MSR基准上取得了+7.82%的绝对改进。

Conclusion: Video-MSR基准揭示了MLLMs在多跳空间推理方面的显著局限性，证明了多跳空间指令数据的有效性，为未来研究提供了重要基础。通过专门的数据集微调可以显著提升模型的空间推理能力。
...
TL;DR: 首次将Vision Transformer应用于古钱币语义元素识别，使用多模态数据（图像+非结构化文本），性能优于CNN模型


<details>
  <summary>Details</summary>
Motivation: 自动化分析古钱币有助于研究人员从大量钱币收藏中提取更多历史见解，并帮助收藏者了解买卖内容。现有研究主要使用CNN识别钱币上的语义元素，本文首次探索更先进的Vision Transformer架构在该任务上的应用。

Method: 采用Vision Transformer深度学习架构，结合多模态数据（图像和非结构化文本）进行全自动学习。同时训练CNN模型作为对比，详细介绍了ViT和CNN模型的训练与实现过程。

Result: ViT模型在准确性方面优于新训练的CNN模型，展示了Vision Transformer在古钱币语义元素识别任务上的优越性能。

Conclusion: Vision Transformer架构在古钱币语义元素识别任务上表现出色，为古钱币自动化分析提供了新的技术路径，有望推动该领域的研究进展。
...
TL;DR: PrivLEX是一种基于法律定义的个人数据概念进行决策的新型图像隐私分类器，利用视觉语言模型的识别能力，无需训练时的显式概念标签，通过零样本VLM概念检测提供可解释的分类。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够基于法律定义的个人数据概念进行决策的可解释隐私分类器，使隐私决策与法律概念对齐，提高透明度和可解释性。

Method: 利用视觉语言模型的零样本概念检测能力，构建无标签的概念瓶颈模型，通过VLM识别图像中的个人数据概念，无需训练时的显式概念标注。

Result: PrivLEX能够有效识别图像中存在的个人数据概念，并分析了人类标注者对图像隐私数据集中这些概念的敏感度感知。

Conclusion: PrivLEX是首个与法律概念对齐的可解释隐私分类器，利用VLM的零样本能力提供透明决策，为图像隐私保护提供了新的法律概念基础方法。
...
TL;DR: 该论文研究了跨数据集目标检测问题，通过设置特异性视角分析不同数据集间的性能差异，发现相同设置类型内的迁移相对稳定，而跨设置类型的迁移性能显著下降且不对称。


<details>
  <summary>Details</summary>
Motivation: 目标检测器在分布内表现良好，但在不同基准测试上性能显著下降。作者旨在通过设置特异性的视角研究跨数据集目标检测问题，理解不同数据集类型间的迁移模式。

Method: 将基准数据集分为设置无关数据集（多样化日常场景）和设置特定数据集（狭窄环境），评估标准检测器家族在所有训练-测试对上的表现。使用开放标签协议，通过CLIP相似性将预测类别映射到最近的目标标签，以区分领域偏移和标签不匹配。

Result: 跨数据集目标检测呈现清晰结构：相同设置类型内的迁移相对稳定，跨设置类型的迁移性能显著下降且不对称。最严重的性能下降发生在从特定源数据集迁移到无关目标数据集时，即使在开放标签对齐后仍然存在，表明领域偏移在困难情况下占主导地位。

Conclusion: 该研究提供了基于设置特异性的跨数据集目标检测原则性特征描述，并为评估分布偏移下的检测器提供了实用指导。开放标签评估带来一致但有界的改进，许多修正案例对应于图像证据支持的语义近似错误。
...
TL;DR: 本文介绍了一种新颖的视频联合嵌入预测架构（V-JEPA）在面部表情识别（FER）中的应用，通过预测掩码区域的嵌入而非像素级重建来学习，在RAVDESS和CREMA-D数据集上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频理解预训练方法依赖像素级重建，会捕捉不相关信息（如背景像素颜色）。V-JEPA通过预测掩码区域的嵌入来学习，使编码器能专注于相关特征，避免捕捉无关信息。
...
Conclusion: 基于纯嵌入的预训练方法在面部表情识别方面具有巨大潜力，能够提高性能并增强泛化能力，为FER领域提供了新的研究方向。
...
TL;DR: 提出GlovEgo-HOI数据集和GlovEgo-Net模型，通过合成数据与扩散模型增强真实图像中的个人防护装备，用于工业场景的自我中心人机交互分析


<details>
  <summary>Details</summary>
Motivation: 工业安全中的自我中心人机交互分析很重要，但缺乏标注的领域特定数据阻碍了鲁棒模型的发展

Method: 提出数据生成框架结合合成数据和基于扩散的过程，为真实图像添加逼真的个人防护装备；提出GlovEgo-Net模型，包含Glove-Head和Keypoint-Head模块，利用手部姿态信息增强交互检测

Result: 广泛的实验证明了所提数据生成框架和GlovEgo-Net的有效性
...
TL;DR: 论文提出了一种数据中心的域适应方法BMM，通过层次化数据服务器和二分图模式匹配算法，为不可标注的目标域寻找最佳匹配的训练数据，减少域间差距。


<details>
  <summary>Details</summary>
Motivation: 目标域可访问但无法实时标注，需要从大规模数据服务器构建替代训练集。现有方法主要改进算法，而本研究探索优化数据服务器结构，解决目标域模式与训练集不匹配的问题。

Method: 引入层次化数据服务器结构，提出二分图模式匹配算法(BMM)，将目标域模式与源域模式进行一对一最优匹配，为每个目标模式在服务器数据树中寻找最佳匹配。

Result: 相比现有训练集搜索算法，BMM匹配的服务器模式构成的训练集在目标重识别和检测任务上具有更小的域差距，训练出的模型准确率更高。与伪标签等现有UDA方法结合可进一步提升性能。

Conclusion: BMM提供了一种数据中心的无监督域适应方法，与现有模型中心方法正交，通过优化数据选择而非算法改进来提升域适应性能。
...
TL;DR: 该研究探索使用低分辨率灰度图像替代传统索引标记来表示中文字符，发现8×8像素的视觉输入能达到与传统索引方法相当的准确率，且在低资源训练时表现出明显的"热启动"优势。
...
Method: 使用灰度图像作为单个字符的输入，分辨率低至8×8像素，替代传统的token ID。解码器接收这些视觉输入进行语言建模，与传统基于索引的基线模型进行对比。

Result: 视觉输入达到39.2%的准确率，与基于索引的基线模型39.1%相当。在低资源设置下表现出明显的"热启动"效应：仅用总训练量的0.4%，准确率就达到12%以上，而基于索引的模型低于6%。

Conclusion: 最小化的视觉结构可以为中文语言建模提供强大且高效的信号，为字符表示提供了与传统基于索引方法互补的替代视角，证明了视觉信息在表意文字处理中的价值。
...
TL;DR: DF-DiffCom是一个基于KAN增强的扩散模型，利用变形场进行可信赖的纵向脑图像补全，解决了传统方法因仅依赖图像强度导致的生成质量有限和应用场景受限的问题。


<details>
  <summary>Details</summary>
Motivation: 纵向脑MRI研究中的高流失率导致数据缺失，传统深度生成模型仅依赖图像强度，存在两个主要问题：1) 生成的脑图像保真度和可信度有限，影响下游研究；2) 模型结构固定的指导限制了应用场景的灵活性。

Method: 提出DF-DiffCom模型，结合Kolmogorov-Arnold Networks (KAN)增强的扩散模型，智能利用变形场进行纵向脑图像补全。该方法不局限于图像强度信息，能够处理多种MRI模态甚至脑组织分割结果等属性图。

Result: 在OASIS-3数据集上训练，DF-DiffCom优于现有最先进方法，PSNR提升5.6%，SSIM提升0.12。更重要的是，其模态无关特性可平滑扩展到多种MRI模态。

Conclusion: DF-DiffCom通过结合变形场和KAN增强的扩散模型，提供了更可信赖和灵活的纵向脑图像补全解决方案，解决了传统方法的局限性，具有广泛的应用前景。
...
TL;DR: OpenVoxel是一种无需训练的方法，通过稀疏体素分组和字幕生成实现开放词汇3D场景理解，无需CLIP/BERT文本编码器嵌入，直接使用多模态大语言模型进行文本到文本搜索。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法通常需要训练，并且依赖CLIP/BERT等文本编码器嵌入。本文旨在开发一种无需训练的方法，能够直接从多视图图像生成的稀疏体素模型中提取有意义的对象分组，并通过视觉语言模型和多模态大语言模型为每个分组生成描述性字幕，从而构建信息丰富的场景地图。

Method: OpenVoxel采用训练免费的算法流程：首先从3D场景的多视图图像中获取稀疏体素光栅化模型，然后对该模型中的稀疏体素进行分组以描述场景中的不同对象。接着利用强大的视觉语言模型和多模态大语言模型，为每个分组生成字幕，构建完整的场景地图。关键创新在于不引入CLIP/BERT文本编码器的嵌入，而是直接使用多模态大语言模型进行文本到文本搜索。

Result: 通过大量实验验证，OpenVoxel在性能上优于近期研究，特别是在复杂的指代表达式分割任务中表现出色。该方法能够成功构建信息丰富的场景地图，支持开放词汇分割和指代表达式分割等进一步3D场景理解任务。

Conclusion: OpenVoxel提供了一种有效且无需训练的3D场景理解方法，通过稀疏体素分组和基于多模态大语言模型的字幕生成，实现了高质量的开放词汇3D场景理解，特别是在复杂指代表达式分割任务中表现优异。代码将开源。
...
Conclusion: 需要制定未来研究议程来解决手写反馈系统中的核心问题
...
TL;DR: 提出了一种基于微分熵的度量方法IDEM，用于3D点云配准，相比传统欧氏距离方法，在处理密度差异、噪声、孔洞和部分重叠时表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统点云配准方法（如ICP）依赖欧氏距离度量（如RMSE），需要选择一个固定点云，且对密度差异、噪声、孔洞和有限重叠等问题敏感，在实际场景中效果受限。

Method: 提出基于微分熵的度量方法IDEM（迭代微分熵最小化），作为优化框架中的目标函数，用于精细刚性成对3D点云配准。该度量不依赖固定点云选择，在变换过程中能显示清晰的最小值对应最佳对齐。

Result: 通过多个案例研究，与RMSE、Chamfer距离和Hausdorff距离进行比较。在存在密度差异、噪声、孔洞和部分重叠的情况下，IDEM表现有效，而RMSE并不总能产生最优对齐。

Conclusion: IDEM作为一种新颖的微分熵度量方法，为点云配准提供了更鲁棒的解决方案，特别适用于实际场景中常见的非理想点云数据。
...
TL;DR: MANGO是一种用于机器人操作的无配对图像翻译方法，通过改进的损失函数和判别器设计解决模拟到真实视觉转换中的视角一致性问题，显著提升策略在未见视角上的性能。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的机器人操作策略对相机视角变化等分布偏移很脆弱，真实机器人演示数据稀缺且缺乏足够的视角变化，而模拟数据虽然能大规模收集但面临视觉模拟到真实的转换挑战。

Method: 提出MANGO方法：1）使用分割条件InfoNCE损失；2）高度正则化的判别器设计；3）改进的PatchNCE损失。这些元素对保持模拟到真实转换中的视角一致性至关重要。

Result: MANGO在图像翻译方法中表现最佳，仅需少量固定相机真实数据就能生成多样未见视角。使用MANGO增强数据训练的模仿学习策略在非增强策略完全失败的视角上能达到60%的成功率。

Conclusion: MANGO通过创新的图像翻译技术有效解决了模拟到真实视觉转换中的视角一致性问题，显著提升了机器人操作策略在未见相机视角上的鲁棒性和性能。
...
TL;DR: 本文提出了一种两阶段分组排序与校准框架（GRCF），用于解决多模态情感分析中现有方法的局限性，通过自适应关注难排序样本和动态调整排序边界，在回归和分类任务上都取得了先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态情感分析方法存在两个主要问题：1）点式回归方法对标签噪声敏感，且忽略了样本间的相对顺序关系；2）成对排序学习方法虽然能捕捉相对顺序，但给所有比较赋予相同权重，且使用静态排序边界，无法反映不同情感组间的语义距离差异。

Method: 提出两阶段分组排序与校准框架（GRCF）：第一阶段引入基于GRPO思想的优势加权动态边界排序损失，构建细粒度序数结构；第二阶段采用MAE驱动的目标函数来校准预测幅度。该框架还可扩展到分类任务。

Result: GRCF在核心回归基准测试中达到了最先进的性能，同时在多模态幽默检测和讽刺检测等分类任务中也表现出强大的泛化能力。

Conclusion: GRCF框架通过同时保持相对序数结构、确保绝对分数校准和自适应关注困难样本，有效解决了现有方法的局限性，在回归和分类任务上都展现了优越性能。
...
TL;DR: 该论文提出了CogRail基准测试，用于铁路入侵感知的时空推理，评估了现有视觉语言模型在此任务上的局限性，并提出了联合微调框架提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有铁路入侵检测系统主要关注固定视觉范围内的物体分类，使用基于规则的启发式方法判断入侵状态，往往忽略了潜在入侵风险目标。准确感知入侵风险需要理解感兴趣对象的空间上下文和时间动态，这对传统视觉模型提出了挑战。
...
Result: 实验表明当前大规模多模态模型在认知入侵感知任务所需的复杂时空推理方面存在困难，凸显了现有基础模型在这一安全关键领域的局限性。相比之下，提出的联合微调框架通过针对性适配领域特定的推理需求，显著提升了模型性能，显示了结构化多任务学习在提高准确性和可解释性方面的优势。

Conclusion: 该研究强调了铁路入侵感知中认知推理的重要性，提出的CogRail基准和联合微调框架为安全关键领域的视觉语言模型应用提供了有价值的解决方案，代码将在GitHub上开源。
...
TL;DR: 文本到图像(T2I)模型生成的图像在嵌入空间中形成独特聚类，使得匿名化模型输出容易被识别，暴露了投票排行榜的安全漏洞


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型质量评估依赖匿名化输出的投票排行榜，但作者发现这种匿名化容易被破坏，需要揭示这一安全漏洞并推动更强的匿名化防御

Method: 使用22个T2I模型和280个提示生成150K图像，通过基于质心的聚类方法在图像嵌入空间中进行模型识别，并引入提示级可区分性度量

Result: 方法实现了高准确率的模型去匿名化，揭示了系统性的模型特定特征，某些提示可实现近乎完美的可区分性
...
Method: 提出AquaFeat+管道，包含颜色校正、分层特征增强和自适应残差输出模块，通过端到端训练，直接由最终应用任务的损失函数指导优化。
...
TL;DR: 提出一个从单张图像估计可模拟服装的框架，通过视觉语言模型推断材料属性和织物特性，再映射到物理参数，无需多视角捕捉或可微分模拟


<details>
  <summary>Details</summary>
Motivation: 从单张图像估计物理准确的、可模拟的服装具有挑战性，因为缺乏图像到物理的数据集且问题本身是病态的。现有方法要么需要多视角捕捉和昂贵的可微分模拟，要么只能预测几何形状而缺乏模拟所需的材料属性
...
Conclusion: 提出了一种无需迭代优化的前馈框架，能够从单张图像生成可模拟的服装，解决了现有方法的局限性，实现了更高精度的物理参数估计和模拟效果
...
Method: LiteEmbed在CLIP的词汇表内进行子空间引导的文本嵌入优化，利用基于PCA的分解将粗粒度语义方向与细粒度变化解耦。通过粗粒度对齐和细粒度分离两个互补目标，保持全局语义一致性同时增强视觉相似类别间的可区分性。

Result: 优化后的嵌入是即插即用的，可无缝替代CLIP的原始文本特征，应用于分类、检索、分割和检测任务。大量实验表明该方法显著优于先前方法。

Conclusion: LiteEmbed是适应CLIP到代表性不足、罕见或未见类别的有效方法，建立了在少样本个性化方面的优势。
...
TL;DR: SCE-SLAM是一种端到端的单目视觉SLAM系统，通过场景坐标嵌入保持尺度一致性，解决了传统方法中尺度漂移的问题，在保持实时性能的同时显著提升了轨迹精度。


<details>
  <summary>Details</summary>
Motivation: 单目视觉SLAM虽然能够从互联网视频进行3D重建并在资源受限平台上实现自主导航，但存在尺度漂移问题——在长序列中估计的尺度会逐渐发散。现有的帧到帧方法通过局部优化实现实时性能，但由于独立窗口之间缺乏全局约束，会累积尺度漂移。

Method: 提出SCE-SLAM系统，通过场景坐标嵌入（学习到的补丁级表示，在规范尺度参考下编码3D几何关系）来保持尺度一致性。框架包含两个关键模块：1）几何引导聚合：利用3D空间邻近性，通过几何调制注意力从历史观测传播尺度信息；2）场景坐标束调整：通过从场景坐标嵌入解码的显式3D坐标约束，将当前估计锚定到参考尺度。

Result: 在KITTI、Waymo和vKITTI数据集上的实验表明显著改进：在KITTI上，相比最佳先前方法，绝对轨迹误差减少了8.36米，同时保持36 FPS的实时性能，并在大规模场景中实现了尺度一致性。

Conclusion: SCE-SLAM通过场景坐标嵌入有效解决了单目视觉SLAM中的尺度漂移问题，在保持实时性能的同时显著提升了轨迹精度和尺度一致性，适用于大规模场景的3D重建和自主导航。
...
TL;DR: STEP3-VL-10B是一个10B参数的轻量级开源多模态基础模型，通过创新的训练策略和并行协调推理，在多项基准测试中超越了10-20倍大的模型和顶级专有模型。
...
Method: 采用两个战略转变：1）在1.2T多模态token上进行统一、完全解冻的预训练，整合语言对齐的感知编码器和Qwen3-8B解码器；2）包含超过1000次强化学习迭代的规模化后训练流程，并实施并行协调推理来扩展测试时计算。

Result: 在多项基准测试中表现优异：MMBench 92.2%、MMMU 80.11%、AIME2025 94.43%、MathVision 75.95%，性能媲美或超越10-20倍大的模型（如GLM-4.6V-106B、Qwen3-VL-235B）以及顶级专有模型如Gemini 2.5 Pro和Seed-1.5-VL。

Conclusion: STEP3-VL-10B展示了通过创新的训练策略和推理优化，紧凑模型也能达到前沿多模态智能水平，为社区提供了强大的开源基线。
...
Conclusion: 通过结合扩散模型生成关键帧和3D重建渲染的方法，实现了高效可控的视频合成，为实时交互应用提供了实用路径
...
TL;DR: COMPOSE：通过超图分割而非成对关联解决多视角姿态对应匹配问题，在稀疏多视角3D姿态估计中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖成对关联建模对应问题，将全局一致性（循环一致性）作为软约束，当虚假关联传播误差时，多视角约束协调变得脆弱

Method: 将多视角姿态对应匹配建模为超图分割问题而非成对关联，提出高效几何剪枝策略大幅减少搜索空间

Result: 相比先前优化方法提升23%平均精度，相比自监督端到端学习方法提升11%

Conclusion: COMPOSE为广泛研究的问题提供了有前景的解决方案，通过超图分割框架更鲁棒地处理多视角姿态对应匹配
...
Result: 实验表明该方法实现了鲁棒的身份保持和跟踪稳定性，随着目标密度增加，优势更加明显

Conclusion: 为野外同时多目标视频分割建立了坚实基础，通过个体化记忆选择解决了SAM3在多目标场景中的局限性
...
Method: 提出Fast-ThinkAct框架，通过从教师模型蒸馏学习潜在思维链推理，使用偏好引导目标对齐操作轨迹，实现语言和视觉规划能力的迁移

Result: 在多样化具身操作和推理基准测试中，Fast-ThinkAct相比最先进的推理VLA方法减少高达89.3%的推理延迟，同时保持有效的长时程规划、少样本适应和失败恢复能力

Conclusion: Fast-ThinkAct通过可表达潜在推理实现了高效紧凑的规划，显著降低推理延迟的同时保持高性能，为具身控制任务提供了有效的推理增强策略学习方法
...
TL;DR: DELIBERATIONBENCH研究发现：在多智能体LLM系统中，简单的"选择最佳响应"基线方法显著优于复杂的协商协议，以6倍性能优势和更低计算成本挑战了"复杂度提升质量"的假设。


<details>
  <summary>Details</summary>
Motivation: 尽管基于大语言模型的多智能体协商系统受到广泛关注，但其相对于简单方法的实际价值尚未得到充分验证。研究旨在通过受控基准测试，评估协商协议是否真的比简单的响应选择方法更有效。

Method: 引入DELIBERATIONBENCH基准，在270个问题和3个独立种子（共810次评估）上，比较三种协商协议与一个强基线方法——从模型输出池中选择最佳响应。

Result: 最佳单一基线方法以82.5% ± 3.3%的胜率显著优于最佳协商协议（13.8% ± 2.6%），形成6倍性能差距（p < 0.01），且协商协议的计算成本高出1.5-2.5倍。

Conclusion: 研究结果挑战了"复杂度提升质量"的假设，表明在多LLM系统中，简单的响应选择方法比复杂的协商协议更有效且成本更低。
...
TL;DR: 这篇综述分析了PTSD与慢性疾病在社交媒体上的交叉表现，发现社交媒体数据能提供有价值的临床洞察，NLP/ML技术可识别潜在PTSD病例（准确率74-90%），在线支持社区有助于应对策略和早期干预。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨PTSD与慢性疾病（如癌症、心脏病、自身免疫疾病）的复杂交集，特别是在社交媒体平台上的表现。现有研究对同时患有PTSD和慢性疾病的人群关注不足，而社交媒体提供了理解这一群体独特挑战的新视角。

Method: 采用系统文献综述方法，分析2008-2024年的相关研究。重点关注社交媒体平台（如X/Twitter和Facebook）上的表达，运用自然语言处理（NLP）和机器学习（ML）技术分析在线数据，识别潜在PTSD病例。

Result: 研究发现：1）社交媒体数据能有效揭示PTSD与慢性疾病患者的独特挑战；2）NLP和ML技术识别潜在PTSD病例的准确率在74%-90%之间；3）在线支持社区在塑造应对策略和促进早期干预方面发挥重要作用。

Conclusion: 结论强调需要在PTSD研究和治疗中充分考虑既有医疗条件，社交媒体可作为监测和支持脆弱群体的有效工具。未来研究应开发针对性干预措施，利用社交媒体数据进行心理健康监测和支持。
...
TL;DR: 论文提出了一种名为"对抗性故事"的越狱技术，通过将有害内容嵌入赛博朋克叙事中，并利用普罗普的民间故事形态学进行功能分析，诱导大语言模型将有害程序重构为合法的叙事解释，在26个前沿模型中平均攻击成功率达71.3%。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的安全机制容易受到通过文化编码结构重构有害请求的攻击。作者发现基于结构的越狱攻击构成了一类广泛的漏洞，而非孤立的技术，因此需要深入理解这些攻击为何成功。

Method: 提出Adversarial Tales攻击技术：1) 将有害内容嵌入赛博朋克叙事框架；2) 利用Vladimir Propp的民间故事形态学进行功能分析；3) 将任务构建为结构分解，诱导模型将有害程序重构为合法的叙事解释。

Result: 在9个提供商的26个前沿模型上进行测试，平均攻击成功率达到71.3%，没有任何模型家族表现出可靠的鲁棒性。结合之前关于Adversarial Poetry的研究，表明基于结构的越狱攻击是一个广泛的漏洞类别。

Conclusion: 文化编码框架可以调解有害意图的空间是巨大的，仅靠模式匹配防御可能无法穷尽。需要开展机制可解释性研究，探索叙事线索如何重塑模型表示，以及模型是否能够独立于表面形式识别有害意图。
...
TL;DR: 本文提出Companion Agents (CA)方法，通过数据库端预缓存查询相关知识来解决Text-to-SQL系统中证据缺失问题，在BIRD基准测试中显著提升执行准确率。
...
Method: 提出Companion Agents (CA)新范式，采用数据库中心方法，利用关系数据库内在的细粒度信息构建缺失证据。CA包含一组伴随数据库模式的智能体，在查询生成前主动挖掘和整合隐藏的表间关系、值域分布、统计规律和潜在语义线索。

Result: 在BIRD基准测试的完全缺失证据设置下，CA在RSL-SQL/CHESS/DAIL-SQL上分别恢复+4.49/+4.37/+14.13个执行准确率点，在挑战性子集上获得更大提升(+9.65/+7.58/+16.71)。
...
TL;DR: 提出三智能体交叉验证框架，通过异构大语言模型协同实现稳定且可解释的多模型系统，在公开部署环境中验证了递归知识合成的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决多模型大语言系统中稳定性和可解释性的挑战，通过异构模型协同工作来克服单一模型行为的局限性，确保系统在现实公开部署环境中的可靠性。

Method: 采用三智能体框架：语义生成、分析一致性检查和透明度审计三个异构LLM组成递归交互循环，实现递归知识合成（RKS），基于不动点理论建立形式化模型。

Result: 在47个控制试验中，平均反射可靠性得分0.78±0.06，约68%试验中透明度得分≥0.8，约89%试验收敛，验证了透明度审计作为收缩算子的理论预测。

Conclusion: 三智能体框架能够在现实公开部署环境中实现稳定的递归知识合成，为人类监督的多LLM架构提供了初步实证证据，具有安全保护特性。
...
TL;DR: 本文提出了一种新颖的一致性感知编辑（CAE）框架，用于实现高效且鲁棒的实体级遗忘，通过聚合多样化的提示并学习低秩更新来全面删除目标实体的知识。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在保留敏感、受版权保护或有害信息的风险。现有的实体级遗忘方法通常依赖全模型微调或基于提示的干预，这些方法计算成本高或对改写查询脆弱。模型编辑技术虽然高效，但通常只针对实例级更新，无法全面消除与实体相关的所有知识。

Method: 提出一致性感知编辑（CAE）框架：1）聚合目标实体的多样化提示集，包括属性、关系和对抗性改写；2）通过一致性正则化器指导，联合学习低秩更新，对齐跨提示的编辑方向；3）研究实体在模型中的存储位置和所需提示数量。

Result: 在RWKU和ToFU两个基准测试上评估CAE：1）揭示了实体级知识在LLM中的内部表示和删除机制；2）相比传统遗忘和编辑基线显著提高了遗忘准确性和鲁棒性；3）仅需数十个精心选择的提示即可实现可扩展的实体移除。

Conclusion: CAE框架为实体级遗忘提供了一种高效且鲁棒的解决方案，通过一致性感知编辑实现了对目标实体知识的全面删除，同时最小化对无关知识的干扰，为理解LLM中知识的内部表示和删除机制提供了新见解。
...
TL;DR: 该研究探索了使用主谓宾三元组结构化知识来增强科学论文的聚类和分类，通过结合无监督聚类和监督分类方法，发现混合表示（结合原始摘要和三元组）能显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 科学文献数量和复杂性不断增加，需要更有效的方法来组织和理解研究文档。结构化知识（如主谓宾三元组）可能有助于改善科学论文的聚类和分类。
...
Result: 完整摘要文本产生最一致的聚类，但结合三元组的混合表示持续改善分类性能，达到92.6%准确率和0.925宏F1分数。轻量级句子编码器（MiniLM、MPNet）在聚类中优于领域特定模型（SciBERT、SPECTER），而SciBERT在结构化输入分类中表现最佳。

Conclusion: 结合非结构化文本和结构化知识具有互补优势，为科学文档的语义组织提供了知识增强表示的新见解，结构化知识能有效提升分类性能。
...
TL;DR: 该论文系统评估了LLM在基于评分标准的简答题自动评分中的表现，发现其在二元任务中表现良好，但随着评分标准细化程度增加而下降，同时展示了置信度过滤与准确率的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 自动简答题评分(ASAG)面临学生回答语言多变性和需要基于评分标准进行细致部分评分的挑战。虽然大语言模型(LLM)提供了有前景的解决方案，但其在基于评分标准设置中作为自动评分者的可靠性需要严格评估。

Method: 使用SciEntsBank基准和Qwen 2.5-72B模型，系统评估LLM评分者在基于评分标准的简答题评分中的表现。研究三个关键方面：1) LLM评分与专家判断在不同评分标准复杂度下的对齐程度；2) 基于共识的延迟机制中不确定性与准确率的权衡；3) 模型在随机输入扰动和对抗攻击下的鲁棒性。

Result: 研究发现：1) 对齐性在二元任务中很强，但随着评分标准细化程度增加而下降；2) "信任曲线"分析显示过滤低置信度预测能提高剩余子集的准确率；3) 鲁棒性实验表明模型对提示注入具有弹性，但对同义词替换敏感。

Conclusion: 该研究为基于评分标准的LLM评分者的能力和局限性提供了关键见解，强调了不确定性估计和鲁棒性测试对于可靠部署的重要性。LLM在简答题自动评分中具有潜力，但需要谨慎处理评分标准复杂性和不确定性管理。
...
TL;DR: 研究表明，针对特定任务微调的小型语言模型(SLMs)相比大型语言模型(LLMs)能在保持相似性能的同时显著减少碳排放，为可持续绿色AI提供了可行方案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)从训练到推理都产生巨大的碳足迹，引发环境担忧。本研究旨在探索微调小型语言模型(SLMs)作为可持续替代方案，在预定义任务中平衡性能与排放。

Method: 对LLMs和微调SLMs在自然语言处理、推理和编程等选定任务上进行性能-排放权衡的对比分析，评估两者在不同任务中的表现和环境影响。

Result: 在六个选定任务中的四个，SLMs保持了与LLMs相当的性能，同时在推理阶段显著减少了碳排放。这表明SLMs在特定任务上具有环境优势。

Conclusion: 小型模型在减轻资源密集型LLMs环境影响方面具有可行性，为推进可持续绿色AI提供了有效路径，特别是在预定义任务场景中。
...
<details>
  <summary>Details</summary>
Motivation: 现有迭代推理框架如InftyThink通过控制上下文增长实现长程推理，但会在不同任务中重复生成相似的推理策略，需要更高效的推理引导机制

Method: 扩展InftyThink框架，加入基于嵌入的语义缓存系统，存储先前成功的推理模式（引理）。在每个推理步骤中，模型检索并基于语义最相似的存储引理进行条件推理，无需无差别扩展上下文窗口
...
Conclusion: 基于相似性的记忆对自改进LLM推理既有益处也有局限性，语义缓存可以引导推理但可能引入系统性偏差
...
TL;DR: RIKER是一个基于范式反转的评估框架，通过从已知事实生成文档而非从文档提取事实，实现确定性评分、可扩展评估和污染抵抗，评估了33个模型发现上下文长度声明常超出实际容量，跨文档聚合比单文档提取更难，事实查找能力和幻觉抵抗是不同能力。


<details>
  <summary>Details</summary>
Motivation: 当前知识系统评估面临根本挑战：静态基准易受污染，基于LLM的评估存在系统性偏见，真实标注需要昂贵的人工标注。需要一种新的评估方法来克服这些限制。

Method: 提出RIKER（检索智能和知识提取评级），采用范式反转方法：从已知真实事实生成文档，而不是从文档中提取真实事实。这种方法通过可再生的语料库实现确定性评分、可扩展评估和污染抵抗。

Result: 评估33个模型使用超过210亿tokens发现：上下文长度声明常超出实际可用容量，超过32K tokens后性能显著下降；跨文档聚合比单文档提取困难得多；事实查找能力和幻觉抵抗是不同能力，擅长查找存在事实的模型仍可能编造不存在的事实。

Conclusion: RIKER不仅是一个具体基准，更贡献了一种领域无关的方法论，可在任何能从结构化真实事实生成合成文档的场景中构建可扩展且抗污染的评估系统。
...
TL;DR: 自动答案匹配使用LLM评估自由文本回答，通过对比参考答案，显示出作为人类评估可扩展替代方案的潜力。研究发现，即使面对策略性攻击（如冗长回答、多答案猜测、冲突答案嵌入），该方法的评分依然稳健，甚至可能降低分数。


<details>
  <summary>Details</summary>
Motivation: 自动答案匹配作为可扩展且对齐的评估方法具有前景，但其可靠性需要验证是否能抵御策略性攻击，如猜测或冗长回答等可能人为提高分数但不改善实际正确性的手段。

Method: 系统研究三种欺骗策略：1) 生成冗长回答；2) 不确定时提供多个答案；3) 在回答开头嵌入冲突答案。通过提示应试模型实施这些攻击，比较二进制评分（正确/错误）和连续评分（部分正确性）的稳健性。

Result: 这些操纵策略不仅不会提高分数，反而经常降低分数。二进制评分比连续评分对攻击更具稳健性。自动答案匹配对廉价文本操纵具有普遍稳健性。

Conclusion: 当参考答案可用时，答案匹配是传统LLM-as-a-judge或人类评估的可行替代方案，对廉价文本操纵具有稳健性。
...
TL;DR: 研究发现上下文分块技术中的摘要注入存在"向量稀释"问题，适度注入可提升召回率18%，但超过临界阈值(CIR>0.4)会使特定查询的精确度下降22%，呈现倒U型曲线。
...
Method: 通过评估不同的注入比例，分析其对召回率和精确度的影响，建立数学模型来描述倒U型曲线关系，并提出计算最优注入比例的理论框架。

Result: 实验显示存在倒U型曲线：适度注入摘要可提升召回率18%，但当上下文注入比例超过0.4时，特定查询的精确度会下降22%，表明存在最优注入比例。

Conclusion: 需要谨慎控制上下文分块中的摘要注入比例，避免过度注入导致的向量稀释问题，提出的理论框架可用于计算最优注入比例以平衡召回率和精确度。
...
Method: 创建了包含455篇文章的跨领域数据集（政治、健康、科学/环境、商业），使用LLaMA 3.1 8B模型通过LoRA技术进行微调，在315个训练样本上进行训练，并在保留的测试集上进行评估。采用数字基础过滤器进一步提高准确性。

Result: NewsScope在人类评估中达到89.4%的准确率，接近GPT-4o-mini的93.7%（p=0.07）。在政治声明上表现更佳（94.3% vs. 87.8%）。使用数字基础过滤器后准确率提升至91.6%，与GPT-4o-mini的差距缩小到2.1个百分点。模型可在约15美元的计算成本下离线部署。

Conclusion: NewsScope提供了一个有效的跨领域新闻声明提取解决方案，在保持接近最先进模型性能的同时显著降低了部署成本，为自动化新闻验证提供了实用工具。代码和基准测试已公开发布。
...
TL;DR: 论文扩展了VirCo虚拟客户在线咨询系统研究，通过引入对抗性攻击数据集测试大语言模型在角色一致性方面的表现，评估了Vicuna模型和其他开源LLM在虚拟客户互动中的角色一致性和连贯性。
...
Method: 1. 扩展VirCo虚拟客户系统研究；2. 创建包含对抗性攻击的新数据集；3. 评估Vicuna模型在角色一致性和连贯性方面的表现；4. 比较多种开源大语言模型在虚拟客户互动中的角色一致性表现。

Result: 提供了对抗性数据集，评估了对话连贯性和角色一致性，并对不同大语言模型进行了比较分析，为虚拟客户系统的角色一致性表现提供了实证数据。

Conclusion: 该研究通过对抗性测试评估了大语言模型在虚拟客户咨询场景中的角色一致性表现，为在线咨询培训提供了更有效的评估工具和模型选择参考。
...
TL;DR: ITP框架通过前瞻想象实现智能体学习，结合世界模型生成多步想象轨迹，自适应调整前瞻视野，在多个基准测试中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型方法主要进行单步或固定步长的推演，未能充分利用其在复杂任务规划中的潜力，需要更灵活的多步想象机制来增强智能体的推理能力。

Method: 提出Imagine-then-Plan统一框架，让策略模型与学习的世界模型交互生成多步想象轨迹；引入自适应前瞻机制，权衡最终目标和任务进展；将想象轨迹与当前观察融合，形成部分可观察和可想象的马尔可夫决策过程。

Result: 在代表性智能体基准测试中，ITP显著优于竞争基线方法；自适应前瞻机制大幅增强了智能体的推理能力，为解决更广泛复杂任务提供了有价值的见解。

Conclusion: ITP框架通过前瞻想象有效提升了智能体的学习和规划能力，自适应前瞻机制为处理复杂任务提供了灵活解决方案，展示了世界模型在智能体学习中的巨大潜力。
...
Method: 对于每个LLM响应，从最终层下一个token概率（来自top-k logprobs）计算输出熵配置文件，并用11个统计量进行总结。使用轻量级分类器预测实例正确性，通过平均预测概率得到领域级准确率估计。在10个STEM推理基准上评估，涵盖9个不同规模的LLM。

Result: 输出熵配置文件能够有效跟踪基准测试的准确率，多个模型显示出近乎单调的领域排序。该方法为可扩展监控和数据采集目标提供了可行的信号。
...
TL;DR: TranslateGemma是基于Gemma 3基础模型的开放机器翻译套件，通过两阶段微调提升翻译能力，在WMT基准测试中表现优异，同时保持多模态能力。


<details>
  <summary>Details</summary>
Motivation: 提升Gemma 3基础模型固有的多语言能力，专门针对机器翻译任务进行优化，为研究社区提供强大且可适应的机器翻译工具。

Method: 采用两阶段微调：1）使用高质量大规模合成并行数据和人工翻译数据进行监督微调；2）使用MetricX-QE和AutoMQM等奖励模型集成进行强化学习，优化翻译质量。

Result: 在WMT25测试集（10个语言对）上通过人工评估，在WMT24++基准（55个语言对）上通过自动评估均显示显著提升。较小的TranslateGemma模型常能达到与较大基线模型相当的性能，同时保持强大的多模态能力，在Vistra图像翻译基准上表现增强。

Conclusion: TranslateGemma模型在翻译质量和效率方面均有显著改进，为研究社区提供了强大的开放机器翻译工具，同时保持了多模态能力。
...
TL;DR: 提出基于社交推理游戏Spyfall的动态评测框架，用于评估LLM的多语言和跨文化能力，发现非英语场景下模型表现显著下降
...
Method: 使用Spyfall社交推理游戏构建动态评测框架，模型需要参与策略性对话来识别秘密特工或避免被发现，利用文化相关的地点或当地食物作为游戏元素

Result: 游戏排名与Chatbot Arena高度一致，但在非英语环境中发现显著性能差距：模型在处理本地特定实体时能力较弱，在非英语语言中经常难以遵循规则或保持策略完整性

Conclusion: 这种基于游戏的方法为传统NLP基准测试提供了可扩展、抗泄露且具有文化细微差别的替代方案
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG方法假设检索信息总是相关的，但实际上检索信息的相关性和有用性会因问题和文档集合而异。现有方法没有充分考虑检索信息的质量对生成结果的影响，导致在面对不同质量检索信息时性能不稳定。

Method: OpenDecoder利用三种显式评估信息作为质量指示特征：相关性分数、排名分数和查询性能预测(QPP)分数。这些特征被整合到生成过程中，使模型能够根据检索信息的质量调整生成策略。

Result: 在五个基准数据集上的实验结果表明，OpenDecoder在效果和鲁棒性方面优于各种基线方法，特别是在处理不同噪声水平的上下文时表现更稳定。

Conclusion: OpenDecoder通过显式评估检索信息质量来增强RAG系统的鲁棒性，该范式具有灵活性，可与LLM的后训练结合，并整合各种外部指示器。
...
TL;DR: 大型语言模型在表面评分上与人类相似，但在理解比喻性和社会性语言（尤其是习语和Gen Z俚语）的深层表征层面与人类存在显著差异。GPT-4最接近人类表征模式，但所有模型在处理依赖语境和社会语用表达（如讽刺、俚语、习语）时都存在困难。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型在解释比喻性和社会性语言时与人类判断的对齐程度。虽然LLM生成的判断表面上与人类相似，但它们在理解具有社会文化背景的比喻性语言方面的真实能力尚不确定。

Method: 研究方法包括：1）人类参与者和四种不同规模的指令调优LLM（GPT-4、Gemma-2-9B、Llama-3.2、Mistral-7B）对240个基于对话的句子进行评分；2）这些句子代表六种语言特征：常规性、讽刺、幽默、情感、习语性和俚语；3）每个句子配对40个解释性问题；4）人类和LLM都使用10点李克特量表进行评分。

Result: 结果显示：1）人类和LLM在表面评分水平上对齐；2）但在表征层面存在显著差异，特别是在解释涉及习语和Gen Z俚语的比喻性句子时；3）GPT-4最接近人类的表征模式；4）所有模型在处理依赖语境和社会语用表达（如讽刺、俚语、习语）时都存在困难。

Conclusion: 结论是大型语言模型在表面评分上与人类相似，但在理解比喻性和社会性语言的深层语义表征方面仍有局限。GPT-4表现最佳，但所有模型都需要改进对社会文化语境的理解能力，特别是在处理讽刺、俚语和习语等依赖语境和社会语用知识的表达时。
...
TL;DR: 研究发现，经过长时间训练达到"顿悟"状态的Transformer模型在组合推理任务上并没有获得新的推理范式，而是将记忆的原子事实整合到已有的推理路径中，且这种"顿悟"模型在新知识整合方面迁移能力有限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在事实检索方面表现出色，但在组合任务的"两跳推理诅咒"上存在困难。参数共享的Transformer通过长时间的"顿悟"阶段形成"泛化电路"，但需要研究这种"顿悟"模型是否真的优于未"顿悟"模型，以及等待"顿悟"阶段的高计算成本是否值得。

Method: 进行机制性研究，评估"泛化电路"在知识吸收和迁移中的作用。通过比较"顿悟"和未"顿悟"模型在分布内组合查询中的推理路径，分析"顿悟"过程的本质。

Result: 1. "顿悟"和未"顿悟"模型在分布内组合查询中的推理路径相同，表明"泛化电路"不代表获得了新的推理范式，而是将记忆的原子事实整合到已有推理路径中。2. 长时间训练后的高准确率与特定推理路径的形成可以独立发生。3. 即使成熟的"泛化电路"在新知识整合方面迁移能力也有限。

Conclusion: "顿悟"的Transformer并没有完全掌握组合逻辑，其"泛化电路"的迁移能力有限。这表明等待模型"顿悟"的高计算成本可能并不总是值得，需要重新思考Transformer在组合推理任务中的能力边界。
...
<details>
  <summary>Details</summary>
Motivation: 音调低资源语言在现代语音技术中服务不足，现有模型难以同时处理说话人变化和音调差异，需要学习既对说话人变化鲁棒又能区分不同音调词汇含义的表示

Method: SITA采用分阶段多目标训练：1）跨性别对比目标促进跨说话人的词汇一致性，音调排斥损失防止音调塌陷；2）基于CTC的ASR目标与蒸馏稳定识别相关结构

Result: 在苗语（高度音调且资源严重不足的语言）上，SITA提高了跨性别词汇检索准确率，同时保持了可用的ASR准确率；在普通话上的类似增益表明该方法具有通用性

Conclusion: SITA是一种通用、即插即用的方法，可用于将多语言语音编码器适配到音调语言，有效解决说话人变化和音调感知的平衡问题
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言处理中的多语言对话摘要和问答任务挑战，探索无需任务特定微调的翻译方法，验证紧凑模型在跨语言任务中的有效性。

Method: 三阶段流程：1) 从印度语言到英语的前向翻译；2) 使用2.55B参数蒸馏语言模型进行多任务文本生成；3) 反向翻译回源语言。采用知识蒸馏技术提升紧凑模型性能。

Result: 系统在竞赛任务中取得强劲胜率，特别是在马拉地语（86.7% QnA）、泰米尔语（86.7% QnA）和印地语（80.0% QnA）上表现突出，证明翻译方法对低资源语言处理的有效性。

Conclusion: 紧凑模型通过知识蒸馏和翻译方法可以在多语言对话摘要和问答任务中实现有竞争力的性能，为低资源语言处理提供了无需任务特定微调的有效解决方案。
...
TL;DR: 本文综述了NLP中标注者分歧的处理方法，从早期视为噪声到现代视为有意义信号，提出了统一的建模框架和评估指标。


<details>
  <summary>Details</summary>
Motivation: NLP中标注者分歧普遍存在，特别是在主观和模糊任务中。传统方法将分歧视为噪声，但近年研究认识到分歧反映了观点和解释的多样性，需要系统化的建模方法。

Method: 1. 提出领域无关的分歧来源分类法（数据、任务、标注者因素）；2. 使用预测目标和池化结构的统一框架综合建模方法；3. 从共识学习转向显式建模分歧；4. 捕捉标注者间的结构化关系。

Result: 1. 建立了系统的分歧感知NLP方法分类体系；2. 识别了从共识学习到分歧建模的范式转变；3. 提出了预测性能和标注者行为的评估指标；4. 指出当前公平性评估主要是描述性而非规范性。
...
TL;DR: Mi:dm 2.0是KT开发的双语大语言模型，专门针对韩国文化和社会特点进行优化，包含11.5B和2.3B两个版本，在韩语基准测试中表现优异，采用MIT许可证开源。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在处理韩语时存在数据质量不足、缺乏文化对齐等问题，无法充分理解韩国社会的价值观、推理模式和常识知识，需要开发专门针对韩国文化背景优化的AI模型。

Method: 采用专有数据清洗流程、高质量合成数据生成、课程学习策略的数据混合方法，以及专门优化的韩语分词器；提供两种配置：采用深度扩展策略的11.5B基础版和针对资源受限环境的2.3B迷你版。

Result: 在韩国特定基准测试中达到最先进性能，在KMMLU上获得顶级零样本结果，在语言、人文和社会科学任务上均有强劲的内部评估表现。

Conclusion: Mi:dm 2.0通过提供可访问的高性能韩国中心化大语言模型，旨在加速韩国各行业的AI采用，加强韩国AI开发者社区，为更广泛的K-intelligence愿景奠定基础。
...
TL;DR: 该论文主张从符号化的知识图谱关系表示转向自然语言描述，提出保留最小结构骨架的混合设计原则，以适应LLM时代的知识表示需求。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱使用预定义的符号关系模式（分类关系标签）存在明显缺陷：真实世界的关系通常是上下文相关的、细微的且有时不确定的，将其压缩为离散关系标签会抽象掉关键的语义细节。然而，符号关系知识图谱仍然被广泛使用，因为它们在操作上有效且与LLM之前的下游模型和算法广泛兼容。

Method: 提出从符号关系转向自然语言关系描述的转变，并提出混合设计原则：保留最小的结构骨架，同时支持更灵活和上下文敏感的关系表示。这种方法结合了LLM支持的自然语言事实合成和基于提示的推理优势。

Result: 论文提出了一个概念框架，主张重新思考关系表示本身，而不仅仅是更高效地使用LLM填充传统模式。这种转变旨在更好地适应LLM重塑知识创建和消费方式的新环境。

Conclusion: 随着LLM的出现，知识表示需要从符号关系转向自然语言描述，采用混合设计原则可以在保持一定结构性的同时，实现更灵活、上下文丰富的关系表示，从而更好地支持基于提示的推理和自然语言知识合成。
...
TL;DR: 人类偏好评估中，当偏好信号在提示间分散时，比例分配是最优策略，大多数真实比较需要远多于通常收集的评判数量才能可靠检测小改进


<details>
  <summary>Details</summary>
Motivation: 研究人类偏好评估中需要多少评判才能可靠检测生成模型的小改进，因为目前不清楚评判数量与检测能力的关系

Method: 分析大规模人类偏好数据集，研究不同提示类型的信息量分布，比较比例分配策略与其他分配策略的效果，涵盖聊天、图像生成、代码生成等多种模态

Result: 大多数真实比较处于偏好信号分散状态，需要远多于通常收集的评判数量；减少提示诱导方差的基准测试可将检测能力提高1.5倍；不明确或负面的评估结果通常反映评估能力不足而非模型等价

Conclusion: 人类偏好评估中需要明确考虑效应大小、预算和协议设计，因为评估结果不明确往往是由于评估能力不足而非模型性能真正相当
...
TL;DR: SubTokenTest是一个评估大语言模型子词理解能力的基准测试，包含10个任务4个领域，旨在解决LLMs在字符级任务上的缺陷


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型推理能力显著提升，但在基本字符级任务（如单词字母计数）上仍然表现不佳，这源于分词过程的问题。现有基准测试虽然通过基本字符操作揭示了这一弱点，但常因缺乏实际相关性而被忽视。然而，许多实际应用（如导航文本地图或解释结构化表格）严重依赖精确的子词理解。

Method: 引入SubTokenTest基准测试，通过实用、效用驱动的任务评估子词理解能力。该基准包含10个任务，覆盖4个领域，通过解耦复杂推理与性能来隔离分词相关失败。评估了9个先进LLMs，研究了测试时缩放对子词推理的影响，并探索了字符级信息在隐藏状态中的编码方式。

Result: 提供了对9个先进大语言模型的全面评估结果，分析了测试时缩放对子词推理的影响，并揭示了字符级信息在模型隐藏状态中的编码特征。

Conclusion: SubTokenTest基准测试为评估大语言模型的子词理解能力提供了系统框架，揭示了当前模型在字符级任务上的局限性，为改进模型分词机制和子词理解能力提供了重要参考。
...
TL;DR: 该论文提出了一种三阶段专利权利要求生成框架，解决了跨司法管辖区泛化能力差、权利要求与现有技术语义关系建模不足、质量评估不可靠三大问题，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前专利权利要求生成系统存在三个根本性限制：跨司法管辖区泛化能力差、权利要求与现有技术之间语义关系建模不足、以及质量评估不可靠。这些限制阻碍了自动化专利审查工作流程的发展。

Method: 提出新颖的三阶段框架：1) 关系感知相似性分析；2) 领域自适应权利要求生成；3) 统一质量评估。具体采用多头注意力机制（8个专门头）进行显式关系建模，结合课程学习和动态LoRA适配器选择（覆盖5个专利领域），并通过评估方面间的交叉注意力机制实现全面质量评估。

Result: 在USPTO HUPD数据集、EPO专利集合和Patent-CE基准测试上的广泛实验显示：相比GPT-4o获得7.6点ROUGE-L提升，相比Llama-3.1-8B获得8.3% BERTScore提升，与人类专家相关性达到0.847（对比单独评估模型的0.623）。跨司法管辖区性能保持率达到89.4%（基线为76.2%）。

Conclusion: 该方法通过关系感知相似性分析、领域自适应生成和统一质量评估，为自动化专利审查工作流程提供了一个全面的解决方案，显著提升了专利权利要求生成的性能和质量评估的可靠性。
...
TL;DR: 论文提出了一种轻量级、无需训练的身份鲁棒生成框架，通过选择性中和非关键身份信息来减少LLM输出中的身份依赖性偏差，在4个基准测试和18种社会人口身份上平均减少77%的偏差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的输出常常因用户社会人口属性而异，导致事实准确性、实用性和安全性方面的差异，即使对于与人口统计信息无关的客观问题也是如此。与先前关于刻板印象或代表性偏见的研究不同，本文研究核心响应质量的身份依赖性退化问题。

Method: 提出了一种轻量级、无需训练的身份鲁棒生成框架，该框架选择性中和非关键身份信息，同时保留语义上必要的属性，从而保持输出内容的完整性。

Result: 在四个基准测试和18种社会人口身份上的实验表明，与普通提示相比，身份依赖性偏差平均减少了77%，相对于基于提示的防御方法减少了45%。

Conclusion: 该工作填补了减轻提示中用户身份线索对核心生成质量影响的关键空白，证明了事实知识在不同身份间是稳健编码的，但生成行为存在偏差，因此需要通过选择性身份信息中和来实现身份鲁棒的生成。
...
TL;DR: ProFit通过选择性掩码低概率token来缓解传统SFT中的单参考过拟合问题，提升LLM对齐效果


<details>
  <summary>Details</summary>
Motivation: 传统SFT强制模型对齐单一参考答案，忽略了语言的一对多特性，导致模型过度拟合非核心表达。虽然引入多个参考答案可以缓解此问题，但数据和计算成本过高，需要寻找更经济的解决方案。
...
Conclusion: 通过关注token概率与语义重要性的关系，ProFit提供了一种有效且经济的解决方案来缓解SFT中的单参考过拟合问题，提升大语言模型与人类意图的对齐效果。
...
TL;DR: A.X K1是一个5190亿参数的混合专家语言模型，通过多阶段数据处理在约10T标记上预训练，支持显式可控推理，在韩国语基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合推理能力与推理效率之间的差距，通过可控推理机制支持多样化现实场景的可扩展部署。

Method: 利用缩放定律在固定计算预算下优化训练配置和词汇量；采用多阶段数据处理管道构建约10T标记的语料库；提出Think-Fusion训练方法，支持用户在单一统一模型中切换思考与非思考模式。

Result: A.X K1在性能上与领先的开源模型竞争，同时在韩国语基准测试中建立了独特优势。

Conclusion: A.X K1通过显式可控推理和Think-Fusion训练方法，成功实现了推理能力与效率的平衡，在韩国语任务上表现突出，为多样化现实场景部署提供了有效解决方案。
...
TL;DR: UserLM-R1：具有推理能力的用户语言模型，通过动态用户画像和目标驱动的决策策略，提升用户模拟器的跨领域泛化能力和战略谈判能力


<details>
  <summary>Details</summary>
Motivation: 当前用户模拟器存在两个主要问题：1）依赖静态、上下文无关的用户画像，需要大量手动调整以适应新场景，限制了泛化能力；2）忽视人类的战略思维，容易受到智能体操纵。需要开发能够跨领域泛化并主动参与谈判的用户模拟器。

Method: 提出UserLM-R1模型，包含：1）构建包含静态角色和动态场景特定目标的综合用户画像；2）采用目标驱动的决策策略，在生成响应前产生高质量推理；3）通过监督微调和多奖励强化学习来优化推理和提升战略能力。

Result: 大量实验结果表明，UserLM-R1在多个基准测试中优于现有基线方法，特别是在更具挑战性的对抗性数据集上表现突出。

Conclusion: UserLM-R1通过结合动态用户画像和目标驱动的推理机制，有效解决了现有用户模拟器的局限性，实现了更好的跨领域泛化能力和战略谈判能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有KG-RAG模型在复杂任务中虽然提高了事实准确性，但往往过度自信，即使在检索的子图不完整或不可靠时也会产生高置信度预测，这在高风险领域部署时存在隐患。

Method: 提出Ca2KG框架，整合反事实提示来暴露知识质量和推理可靠性中的检索依赖不确定性，并结合面板重评分机制来稳定干预下的预测。

Result: 在两个复杂QA数据集上的广泛实验表明，Ca2KG能持续改进校准性能，同时保持甚至提升预测准确性。

Conclusion: Ca2KG通过因果感知校准有效解决了KG-RAG模型的过度自信问题，为高风险领域的部署提供了更可靠的解决方案。
...
TL;DR: FairToT是一个推理时框架，通过提示引导的毒性评估来增强LLM的公平性，识别可能产生人口统计学相关变异的情况，并决定何时应用额外评估，从而减少群体差异同时保持稳定的毒性预测。


<details>
  <summary>Details</summary>
Motivation: LLM在在线审核系统中用于毒性评估时，经常对涉及隐性仇恨言论的微妙表达产生不一致的判断，暴露出难以通过标准训练纠正的潜在偏见。现有方法往往忽视了一个关键问题：何时应该调用纠正机制来确保公平可靠的评估？

Method: 提出FairToT推理时框架，通过提示引导的毒性评估来增强LLM公平性。该框架识别可能发生人口统计学相关变异的情况，并决定何时应用额外评估。同时引入两个可解释的公平性指标来检测此类情况，在不修改模型参数的情况下提高推理一致性。

Result: 在基准数据集上的实验表明，FairToT减少了群体层面的差异，同时保持了稳定可靠的毒性预测，证明了推理时细化是提高基于LLM的毒性评估系统公平性的有效实用方法。

Conclusion: FairToT框架通过推理时优化，为LLM在毒性评估中的公平性问题提供了有效解决方案，能够在保持预测稳定性的同时显著减少群体差异，是改进LLM公平性的实用方法。
...
TL;DR: 该研究提出了MCGA多任务古典文学音频语料库，涵盖6个音频任务，评估了10个MLLM模型，发现现有模型在处理古典文学音频时仍面临重大挑战，并提出了新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的发展，其在中华古典研究领域的潜力受到关注，但现有研究主要关注文本和视觉模态，音频语料库在该领域尚未得到充分探索。

Method: 提出了多任务古典文学音频语料库MCGA，涵盖6个音频任务：自动语音识别、语音到文本翻译、语音情感描述、口语问答、语音理解和语音推理。评估了10个多模态大语言模型，并提出了语音情感描述的评估指标以及语音与文本能力一致性的度量方法。

Result: 实验结果表明，当前模型在处理MCGA测试集时仍面临重大挑战。研究团队公开了MCGA语料库和代码，以促进中华古典研究领域具有更强大多维音频能力的MLLM发展。

Conclusion: MCGA语料库填补了中华古典研究领域音频语料库的空白，为评估和提升多模态大语言模型在古典文学音频处理方面的能力提供了重要资源，推动了该领域音频能力的发展。
...
TL;DR: ReGraM提出了一种区域优先的知识图谱推理框架，通过构建查询对齐的子图并在局部区域内进行多证据感知的逐步推理，显著提升了医学问答的事实准确性。
...
Method: ReGraM采用区域优先的知识图谱推理框架：1) 构建查询对齐的子图；2) 在局部区域内进行多证据感知的逐步推理；3) 专注于知识图谱中最相关的部分，而不是假设所有关系同等有用。

Result: 在七个医学问答基准测试中，ReGraM始终优于强基线(KGARevion)：在MCQ上获得8.04%的绝对准确率提升，在SAQ上获得4.50%提升，幻觉率降低42.9%。消融和定性分析表明，区域构建与跳数推理的对齐是改进的主要驱动力。

Conclusion: 区域优先的知识图谱推理是提高医学问答事实准确性和一致性的有效范式，通过专注于最相关的知识图谱区域，避免了传统方法中噪声和不稳定性的问题。
...
TL;DR: 研究使用梯度解释方法分析语言模型对德语定冠词的处理机制，发现模型并非严格基于语法规则，而是部分依赖记忆关联


<details>
  <summary>Details</summary>
Motivation: 语言模型在语法一致性任务上表现良好，但尚不清楚这种能力是基于规则泛化还是记忆。研究旨在探究德语定冠词（形式取决于性和格）的处理机制，以理解模型是否真正掌握抽象语法规则。

Method: 使用GRADIEND（基于梯度的可解释性方法），学习针对特定性-格组合的定冠词转换的参数更新方向。通过分析这些更新对其他无关性-格设置的影响，以及受影响神经元的重叠情况，来探究模型的内部机制。

Result: 研究发现，为特定性-格定冠词转换学习的参数更新经常影响无关的性-格设置，且不同设置间受影响最严重的神经元存在显著重叠。这表明模型并非严格基于规则编码德语定冠词。

Conclusion: 语言模型对德语定冠词的处理并非完全基于抽象语法规则，而是至少部分依赖记忆的关联。这挑战了模型具有严格规则化语法编码的观点，表明其能力可能更多来自统计模式学习而非真正的语法规则掌握。
...
TL;DR: 提出一个基于多智能体系统的上下文化仇恨言论检测框架，通过社区驱动的咨询机制显著提升分类准确性和公平性


<details>
  <summary>Details</summary>
Motivation: 现有仇恨言论检测方法在处理隐含仇恨言论时存在局限性，缺乏对社会文化背景的充分考虑，且公平性评估不够严谨

Method: 构建包含中央调解员智能体和代表特定人口群体的动态社区智能体的多智能体系统，整合公开知识源的社会文化背景，采用平衡准确率作为分类公平性的核心评估指标

Result: 在具有挑战性的ToxiGen数据集上，该方法超越了最先进的提示方法（零样本提示、少样本提示、思维链提示）和其他替代方法，显著提高了所有目标群体的分类准确性和公平性

Conclusion: 社区驱动的咨询框架能够有效整合社会文化背景，为隐含仇恨言论检测提供更准确、更公平的解决方案，平衡准确率是评估分类公平性的重要指标
...
TL;DR: 该研究评估了对话系统在情境对话中建立和利用共同基础的能力，特别关注实体间的关系引用，并提出了改进共同基础表示和利用的方法。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明大语言模型能够执行澄清请求或确认等基础行为，但很少有研究探讨如何明确表示和存储共同基础以供后续使用。缺乏这种机制使得无法确定确认或澄清行为是否真正反映了基于共同基础的理解。

Method: 研究测试了多种在情境对话中表示共同基础的方法，并提出了改进共同基础建立及其在对话中后续使用的具体方法。研究重点评估模型在共享情境中通过实体间关系引用建立和利用共同基础的能力。

Result: 研究结果表明，通过改进共同基础的表示和存储机制，可以显著提升对话系统在情境对话中建立和利用共同基础的能力，使确认和澄清行为更加准确地反映基于共同基础的理解。

Conclusion: 明确表示和存储共同基础对于对话系统在情境对话中实现真正的基于共同基础的理解至关重要。提出的方法能够有效改进共同基础的建立和利用，为更连贯、更自然的对话交互提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 非英语语言临床信息抽取的标注数据集稀缺，阻碍了主要在英语环境下开发的大型语言模型方法的评估。本研究旨在填补这一空白，特别是在英语和土耳其语的临床关系抽取任务上。

Method: 1) 创建首个英语-土耳其语平行临床关系抽取数据集；2) 系统评估多种提示策略，包括上下文学习和思维链方法；3) 提出关系感知检索方法，基于对比学习同时捕捉句子级和关系级语义；4) 与传统微调基线模型进行比较。

Result: 1) 基于提示的LLM方法始终优于传统微调模型；2) 英语评估在所有LLM和提示技术中都优于土耳其语；3) 关系感知检索方法在上下文学习方法中表现最佳，Gemini 1.5 Flash在英语和土耳其语中分别达到0.906和0.888的微F1分数；4) 结合结构化推理提示后，DeepSeek-V3在英语中达到0.918 F1分数。

Conclusion: 高质量示例检索对于LLM性能至关重要，先进的检索和提示技术有潜力弥合临床自然语言处理中的资源差距，特别是在资源匮乏的语言中。
...
TL;DR: LLMs在处理进行时态语义时存在"目的论偏见"，会系统性地为有目标的事件产生完成幻觉，缺乏真正的时态结构意识，更像是预测性叙事引擎而非逻辑推理器。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否真正理解事件的组合语义，还是仅仅依赖表面概率启发式方法。通过"未完成体悖论"这一逻辑现象来探究模型对事件语义的理解深度。

Method: 引入ImperfectiveNLI诊断数据集，评估最先进的开源模型，分析内部表示，并尝试基于提示的干预措施。

Result: 发现模型存在普遍的目的论偏见：系统性地为有目标的事件产生完成幻觉，经常覆盖明确的文本否定。内部嵌入能区分过程和结果，但推理决策受目标达成的强先验主导。提示干预能减少幻觉完成，但也会增加对有效蕴含的错误拒绝。

Conclusion: 当前LLMs缺乏结构性的时态意识，更像是预测性叙事引擎而非忠实的逻辑推理器。
...
Method: ACT（激活引导的通道级能力转移）：1）分析模块激活差异定位能力相关通道；2）选择性转移对应参数；3）轻量微调确保兼容性。基于发现能力相关激活高度集中在少数通道且解耦性良好的观察

Result: 在多语言数学和科学推理任务上，ACT能够恢复遗忘的能力同时保留已有技能，还能合并多个专业化模型，将多种能力集成到单一模型中且干扰最小

Conclusion: 能力相关激活在LLM参数中高度集中且解耦，ACT方法通过通道级参数转移有效解决灾难性遗忘，实现多能力集成，为模型专业化提供了新思路
...
TL;DR: PAGER是一个基于页面的自主知识表示框架，通过结构化认知大纲和迭代检索优化RAG的知识表示质量
...
Method: PAGER首先提示LLM为给定问题构建结构化认知大纲（包含多个知识方面的槽位），然后迭代检索和精炼相关文档填充每个槽位，最终构建连贯的页面作为上下文输入来指导答案生成

Result: 在多个知识密集型基准测试和骨干模型上的实验表明，PAGER始终优于所有RAG基线方法。进一步分析显示PAGER构建了更高质量、信息密度更高的知识表示，更好地缓解了知识冲突，并使LLM能更有效地利用外部知识

Conclusion: PAGER通过结构化认知大纲和迭代知识积累，显著提升了RAG的知识表示质量和答案生成效果，为知识密集型任务提供了有效的解决方案
...
TL;DR: 本文提出基于机制可解释性方法的框架，用于定位语言模型预训练期间产生的内部知识冲突，并展示如何通过因果干预控制推理时的冲突知识。


<details>
  <summary>Details</summary>
Motivation: 语言模型中存在内部知识冲突问题，即同一事件的不一致信息被编码在模型的参数化知识中。先前研究主要关注解决模型内部知识与外部资源之间的冲突，但预训练期间产生的内部冲突定位问题尚未被探索。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型在将自然语言翻译为一阶逻辑时经常产生格式和翻译错误，现有基于自我迭代的方法严重依赖模型能力，需要更可靠的解决方案。

Method: 1. 分类常见错误并用大语言模型合成数据进行微调；2. 引入增量推理，将推理分为谓词生成和一阶逻辑翻译两个阶段；3. 使用针对谓词-元数错误的验证模块。

Result: 在四个逻辑推理数据集上评估三个模型家族，综合方法降低了错误率，提高了谓词覆盖率，并改善了小语言模型的推理性能。

Conclusion: 通过微调、增量推理和验证模块的组合方法，能够显著提升小语言模型的符号推理能力，向开发可靠且易访问的符号推理系统迈进了一步。
...
TL;DR: SlidesGen-Bench是一个用于评估幻灯片生成系统的基准测试，通过视觉域统一评估框架、三个量化维度（内容、美学、可编辑性）和人类偏好对齐数据集，提供比现有方法更可靠和可比较的评估。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展催生了多样化的自动幻灯片生成系统，但评估这些异构系统面临挑战。现有评估协议难以提供跨架构的可比较分数，或依赖于未校准的主观判断，需要一种统一、量化且可靠的评估基准。

Method: 提出SlidesGen-Bench基准，基于三个核心原则：1）通用性：在视觉域建立统一评估框架，将终端输出视为渲染结果，对底层生成方法保持不可知；2）量化：通过计算方式在三个维度（内容、美学、可编辑性）定量评估幻灯片；3）可靠性：构建Slides-Align1.5k数据集，包含来自9个主流生成系统、7个场景的人类偏好对齐数据。

Result: 实验表明，SlidesGen-Bench比现有评估流程实现了更高程度的人类判断对齐，提供了更可靠和可比较的评估结果。代码和数据已开源。

Conclusion: SlidesGen-Bench为异构幻灯片生成系统提供了一个统一、量化且可靠的评估基准，解决了现有评估方法的局限性，并通过人类偏好对齐确保了评估的有效性。
...
Result: 在76个计算机科学主题上的实验表明，MVSS在组织和证据基础方面优于现有方法，达到与专家综述相当的性能。

Conclusion: MVSS框架通过多视图结构化方法有效解决了自动综述生成中的结构组织问题，提供了更接近专家水平的综述质量。
...
TL;DR: SERM是一个自进化相关性模型，通过多智能体模块解决大规模工业搜索中信息样本稀疏和伪标签不可靠的问题，在数十亿日请求的系统中实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实世界查询流动态演化，相关性模型难以泛化到实际搜索场景。在大规模工业环境中，自进化技术面临两个挑战：信息样本稀疏难以识别，以及当前模型生成的伪标签不可靠。

Method: 提出自进化相关性模型(SERM)，包含两个互补的多智能体模块：多智能体样本挖掘器用于检测分布偏移并识别信息训练样本；多智能体相关性标注器通过两级一致性框架提供可靠标签。

Result: 在大规模工业环境中评估SERM，该系统每天服务数十亿用户请求。实验结果表明，通过迭代自进化，SERM能够实现显著的性能提升，经离线多语言评估和在线测试验证。

Conclusion: SERM通过多智能体架构有效解决了大规模工业搜索中自进化面临的关键挑战，实现了相关性模型的持续改进和性能提升。
...
TL;DR: 本文系统研究了MXFP格式下的后训练量化，发现MXFP8性能接近无损，MXFP4仍有挑战；量化效果取决于格式兼容性，不同模型家族趋势一致；缩放因子是MXFP4主要误差源，可通过预缩放优化缓解。


<details>
  <summary>Details</summary>
Motivation: MXFP作为大语言模型的有前景低精度格式，虽然已有多种后训练量化算法，但它们主要关注整数量化，在MXFP格式下的适用性和行为尚未充分探索。本文旨在填补这一研究空白。

Method: 系统研究MXFP格式下的后训练量化，涵盖超过7种PTQ算法、15个评估基准和3个LLM家族，分析不同格式下的量化效果和影响因素。

Result: 1) MXFP8始终实现接近无损性能，MXFP4仍存在显著精度下降；2) PTQ效果强烈依赖于格式兼容性；3) 量化敏感性在不同模型家族和模态中表现一致；4) 量化缩放因子是MXFP4主要误差源，预缩放优化策略可显著缓解影响。
...
TL;DR: 提出对话遥测(DT)框架，为自主系统的信息收集对话提供两个模型无关的信号：进度估计器和停滞指数，用于监控信息获取效率和检测无效提问


<details>
  <summary>Details</summary>
Motivation: 自主系统在进行基于模式的信息收集对话时存在仪表化差距，缺乏回合级别的可观测指标来监控获取效率和检测何时提问变得无效

Method: 引入对话遥测(DT)框架，在每个问答交换后产生两个模型无关信号：1)进度估计器(PE)量化每个类别的剩余信息潜力；2)停滞指数(SI)检测可观察的失败特征，即重复类别探测伴随语义相似、低边际增益的响应

Result: 在受控的搜救(SAR)启发式访谈中使用基于LLM的模拟验证DT，能够区分高效对话轨迹和停滞对话轨迹，并通过将DT信号集成到强化学习策略中展示了下游效用

Conclusion: DT提供了可解释的回合级别仪表化，当停滞带来操作成本时能改善策略性能，支持在归因特定原因不切实际的设置中进行监控
...
TL;DR: 本文提出了一种基于半结构化长思维链的RL框架，通过多样化规划分支和群体感知多样性奖励机制，在保持生成质量的同时显著提升大语言模型在创意写作等开放任务中的输出多样性。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大语言模型增强方法往往导致输出多样性降低，这在创意写作等开放任务中削弱了模型的实用性。现有方法缺乏明确的多样化探索机制，过度优先考虑优化效率和性能而忽视了多样性。

Method: 提出一个围绕半结构化长思维链构建的强化学习框架，将生成过程分解为明确规划的中间步骤。引入多样化规划分支方法，在规划阶段基于多样性变化策略性地引入分歧，同时采用群体感知多样性奖励来鼓励不同的生成轨迹。

Result: 在创意写作基准测试上的实验结果表明，该方法在不损害生成质量的前提下显著提高了输出多样性，持续优于现有基线方法。

Conclusion: 通过将生成过程分解为可规划的中间步骤，并结合多样化探索机制，该框架成功解决了RL增强大语言模型时输出多样性降低的问题，为开放任务提供了更实用的解决方案。
...
TL;DR: TaxoBell：一种基于高斯盒嵌入的框架，用于解决现有方法在分类学扩展中的局限性，通过将盒几何与多元高斯分布转换，实现稳定的优化和模糊概念的鲁棒建模。


<details>
  <summary>Details</summary>
Motivation: 手动分类学扩展劳动密集且无法跟上新概念的出现速度。现有自动方法依赖点向量嵌入，难以处理分类学中非对称的"is-a"关系。盒嵌入虽能实现包含和不相交关系，但存在梯度不稳定、缺乏语义不确定性表示、无法处理多义性等问题。

Method: 提出TaxoBell框架，将盒几何与多元高斯分布进行转换，其中均值编码语义位置，协方差编码不确定性。采用基于能量的优化方法，实现稳定优化、模糊概念的鲁棒建模和可解释的层次推理。

Result: 在五个基准数据集上的实验表明，TaxoBell显著优于八个最先进的分类学扩展基线方法，MRR提升19%，Recall@k提升约25%。通过误差分析和消融研究进一步展示了TaxoBell的优势和局限性。

Conclusion: TaxoBell通过高斯盒嵌入框架有效解决了现有分类学扩展方法的局限性，实现了稳定的优化、语义不确定性的建模和多义性处理，在分类学扩展任务中取得了显著性能提升。
...
TL;DR: 论文提出RGD（基于生成数据的路由）新范式，研究在仅有生成数据的情况下训练LLM路由器的挑战，发现仅查询路由器比查询-答案路由器更稳健，并提出CASCAL方法提升性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器方法通常假设能获取真实标注数据，但在实际应用中，特别是用户请求分布异构且未知时，真实标注数据往往不可得。需要研究如何在仅有生成数据的情况下训练有效的路由器。

Method: 1. 引入RGD（Routing with Generated Data）设置，路由器仅使用生成器LLM根据高级任务描述生成的查询和答案进行训练；2. 对比查询-答案路由器和仅查询路由器的性能差异；3. 提出CASCAL方法，通过共识投票估计模型正确性，并通过层次聚类识别模型特定技能领域

Result: 1. 发现仅查询路由器比查询-答案路由器在生成器质量下降时性能下降更慢；2. 识别有效生成器的两个关键特征：能准确回答自己提出的问题，且问题能充分区分模型池中的性能差异；3. CASCAL方法在弱生成器数据上训练时，比最佳查询-答案路由器绝对准确率高4.6%，对生成器质量更具鲁棒性

Conclusion: 在仅有生成数据的情况下训练LLM路由器是可行的，仅查询路由器比查询-答案路由器更稳健，通过筛选生成器特征和采用CASCAL方法可以显著提升路由性能，为解决实际应用中缺乏真实标注数据的问题提供了有效途径
...
### [117] [LLMs can Compress LLMs: Adaptive Pruning by Agents](https://arxiv.org/abs/2601.09694)
*Sai Varun Kodathala,Rakesh Vunnam*

Main category: cs.CL

TL;DR: 提出基于智能体引导的LLM剪枝方法，通过基础模型作为自适应剪枝智能体，在保持关键知识路径的同时智能选择每层剪枝比例，相比现有方法显著提升性能保留。


<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法（如SparseGPT、Wanda）依赖均匀或手工设计的启发式规则确定层间稀疏度比例，且结构化剪枝方法会导致事实知识严重退化。需要更智能的自适应剪枝策略来平衡计算成本削减与性能保留。

Method: 提出智能体引导剪枝框架：1）结合Wanda启发的权重-激活度量和梯度重要性分数构建层敏感度剖面；2）使用具备自反思能力的LLM智能体处理统计信息，从先前剪枝结果中学习并迭代优化策略；3）引入检查点回滚机制，当困惑度退化超过阈值时恢复模型。
...
Conclusion: 基础模型可以有效指导其他基础模型的压缩，智能体引导剪枝方法在保持模型性能的同时实现高效剪枝，为LLM后训练压缩提供了新的自适应框架。
...
TL;DR: 提出Empathy Applicability Framework (EAF)框架，用于在医生回应前预测患者查询中的共情需求，建立基准数据集并训练分类器，优于启发式和零样本LLM基线。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地融入临床工作流程，但缺乏临床共情这一医患沟通的关键要素。现有NLP框架主要关注对医生回应的共情标注，对预测性共情需求建模支持有限，特别是在一般健康查询中。

Method: 引入Empathy Applicability Framework (EAF)，基于临床、上下文和语言线索，对患者查询进行情感反应和解释适用性分类。发布真实患者查询基准数据集，由人类和GPT-4o双重标注。训练分类器预测共情适用性，并与启发式和零样本LLM基线比较。

Result: 在人类共识子集中观察到显著的人类-GPT对齐。基于人类标注和GPT-only标注训练的分类器在预测共情适用性方面表现强劲，优于基线方法。错误分析揭示了持续挑战：隐性痛苦、临床严重性模糊性和上下文困难。

Conclusion: EAF为在回应生成前识别共情需求提供了框架，建立了预测性共情建模基准，支持异步医疗中的共情沟通。需要多标注者建模、临床医生参与校准和文化多样性标注来应对挑战。
...
TL;DR: 论文提出了一种值感知数值表示方法，通过在标准分词输入前添加一个专门的前缀标记，其嵌入向量显式编码数值大小，从而增强语言模型的基础数值理解能力。


<details>
  <summary>Details</summary>
Motivation: Transformer语言模型在数学推理基准测试中表现良好，但在基础数值理解和算术运算方面仍然脆弱。核心限制是数字被处理为符号标记，其嵌入向量没有显式编码数值大小，导致系统性错误。

Method: 引入值感知数值表示，增强标准分词输入：添加一个专门的前缀标记，其嵌入向量显式地以底层数值为条件。这种机制将数值大小信息直接注入模型的输入空间，同时保持与现有分词器和仅解码器Transformer架构的兼容性。

Result: 在算术任务评估中，该方法在数值格式、任务类型和操作数长度方面均优于基线模型。结果表明显式编码数值值是提高语言模型基础数值鲁棒性的有效且高效方式。

Conclusion: 通过值感知数值表示显式编码数值大小，可以显著改善语言模型的基础数值鲁棒性，同时保持与现有架构的兼容性，为解决数字处理中的系统性错误提供了有效方案。
...
### [120] [Two-dimensional Entanglement-assisted Quantum Quasi-cyclic Low-density Parity-check Codes](https://arxiv.org/abs/2601.08927)
*Pavan Kumar,Shayan Srinivasa Garani*

Main category: cs.IT

TL;DR: 本文研究了二维准循环LDPC码的环结构条件，构建了多种girth大于4或6的经典QC-LDPC码，并基于这些经典码构造了两类纠缠辅助量子LDPC码。


<details>
  <summary>Details</summary>
Motivation: 研究二维准循环LDPC码的Tanner图中2g-环的存在条件，旨在构建具有较大围长(girth)的LDPC码，以改善其纠错性能，并进一步将这些经典码扩展为纠缠辅助量子LDPC码。

Method: 1. 推导二维经典QC-LDPC码Tanner图中2g-环存在的一般条件；2. 通过堆叠p×p×p张量构造girth大于4的经典QC-LDPC码(p为奇素数)；3. 对于复合数p，提出两类通过类似张量堆叠得到的经典LDPC码，分别实现girth大于4和大于6；4. 基于经典码构造两类EA-QLDPC码：第一类来自一对二进制经典码，第二类来自单个无4环的经典码。

Result: 1. 建立了二维QC-LDPC码中2g-环存在的条件；2. 构造了具有p×p纠删能力的经典QC-LDPC码；3. 提出的EA-QLDPC码继承了经典码的p×p纠删能力，第一类仅需单个ebit，第二类基于无4环的经典码构造。

Conclusion: 本文系统研究了二维QC-LDPC码的环结构，构建了具有良好围长特性的经典码，并成功将其转化为纠缠辅助量子LDPC码，为量子纠错码设计提供了新思路。
...
### [121] [On the Information Leakage Envelope of the Gaussian Mechanism](https://arxiv.org/abs/2601.08986)
*Sara Saeidian*

Main category: cs.IT

TL;DR: 本文研究了高斯机制的点态最大泄漏包络，推导了高斯秘密下的闭式解，并扩展到一般无界秘密


<details>
  <summary>Details</summary>
Motivation: 研究高斯机制在任意后处理下的最小信息泄漏边界，为差分隐私和信息安全提供理论保障

Method: 推导高斯秘密下确定性PML包络的闭式表达式，通过Brascamp-Lieb不等式扩展到强对数凹先验的一般无界秘密

Result: 得到了高斯秘密下PML包络的闭式解，证明了强对数凹先验满足扩展条件，使结果适用于更广泛的秘密分布

Conclusion: 高斯机制的PML包络在高斯秘密下有精确解，且该结果可扩展到强对数凹先验的一般无界秘密，为隐私保护机制提供了理论基础
...
### [122] [An Information-Theoretic Perspective on LLM Tokenizers](https://arxiv.org/abs/2601.09039)
*Mete Erdogan,Abhiram Gorle,Shubham Chandak,Mert Pilanci,Tsachy Weissman*

Main category: cs.IT

TL;DR: 本文研究了LLM分词器作为结构化压缩器的角色，发现分词器训练规模会重新分配熵：训练数据增加时，分词流整体多样性提高但上下文预测性增强，表明分词器吸收了短程规律性，但这些增益在训练-测试领域不匹配时会下降。


<details>
  <summary>Details</summary>
Motivation: 尽管分词器在LLM流水线中扮演核心角色，但分词、压缩效率和诱导结构之间的联系尚未得到充分理解。研究者希望实证探索分词器训练规模如何影响压缩效率和统计结构。

Method: 1) 将预训练的GPT系列分词器作为黑盒压缩器在不同领域进行基准测试；2) 研究不同配置（词汇量、训练规模、领域）下的学习分词器；3) 将分词视为通用压缩的转换，引入压缩感知的BPE变体；4) 采用信道视角，引入容量利用率指标分析分词器行为。

Result: 分词器训练规模重新分配熵：训练数据增加时，分词流整体多样性提高（一元熵更高），但上下文预测性显著增强（高阶条件熵更低）。这表明分词器吸收了大量短程规律性，但这些增益在训练-测试领域不匹配时会下降。研究揭示了压缩、诱导结构和领域偏移鲁棒性之间的权衡。

Conclusion: 研究结果揭示了压缩、诱导结构和领域偏移鲁棒性之间的各种权衡，并激励了基于压缩感知的分词器设计原则。分词器作为结构化压缩器在LLM流水线中具有重要作用，需要更系统化的设计方法。
...
### [123] [Overcoming the Shadow: Bending Airy Beams for Radiative Near-Field Multi-User Access in Half-Space Blockage Scenarios](https://arxiv.org/abs/2601.09098)
*Yifeng Qin,Jing Chen,Zhi Hao Jiang,Zhi Ning Chen,Yongming Huang*

Main category: cs.IT

TL;DR: 提出一种利用艾里光束自弯曲特性对抗半空间遮挡的波束赋形策略，在近场多用户通信中恢复阴影区域连接性，无需额外硬件。


<details>
  <summary>Details</summary>
Motivation: 下一代无线通信采用超大规模天线阵列，使通信进入辐射近场区域，但高频近场链路在室内环境中易受墙壁、角落等半空间障碍物遮挡，传统近场聚焦波束在阴影区域会出现严重能量截断和有效秩崩溃问题。

Method: 提出艾里模拟波束赋形方案，利用艾里光束的自弯曲特性使主瓣"骑乘"衍射边缘进入阴影区域；基于格林函数建立近场多用户信道模型；开发艾里零陷导向方法在混合阴影/明亮场景中抑制干扰。

Result: 仿真显示，提出的边缘骑乘艾里策略相比传统近场聚焦获得超过20dB的SNR改善，在阴影链路中恢复全秩连接性，在几何阴影中几乎消除中断，在典型室内ELAA配置下多用户频谱效率提升约35%。

Conclusion: 该研究证明了在不依赖可重构智能表面的情况下，在半空间遮挡场景中实现鲁棒的近场多用户接入，为下一代无线通信提供了有效的遮挡缓解方案。
...
### [124] [The .serva Standard: One Primitive for All AI Cost Reduced, Barriers Removed](https://arxiv.org/abs/2601.09124)
*Rachel St. Clair,John Austin Cook,Peter Sutor,Victor Cavero,Garrett Mindt*

Main category: cs.IT

TL;DR: ServaStack提出了一种革命性的AI基础设施解决方案，通过.serva格式（基于激光全息原理的无损压缩）和Chimera计算引擎，直接在压缩数据上执行计算，无需解压或预处理，实现了显著的能效提升和存储压缩。


<details>
  <summary>Details</summary>
Motivation: AI基础设施面临两大危机：1) 计算负载危机 - 训练和推理的能源与资本成本不可持续，可能超出电网容量并导致能力集中在少数组织手中；2) 数据混乱危机 - 80%的项目精力消耗在数据准备、转换和预处理上，阻碍开发速度并将数据集锁定在单一模型架构中。现有方法将这些视为独立问题，通过增量优化管理，增加了生态系统复杂性。

Method: 提出ServaStack解决方案，包含两个核心组件：1) 通用数据格式.serva - 基于激光全息原理实现无损压缩；2) 通用AI计算引擎Chimera - 将计算操作转换为表示空间，直接在.serva文件上执行计算而无需解压缩。Chimera引擎使现有模型无需重新训练即可在.serva数据上运行。

Result: 内部基准测试显示：能效提升30-374倍（减少96-99%），无损存储压缩4-34倍，计算负载减少68倍且无精度损失（在FashionMNIST和MNIST数据集上对比RNN、CNN和MLP模型）。在超大规模场景下（每日10亿次迭代），每PB每训练周期可节省485万美元。

Conclusion: ServaStack通过统一数据格式和计算引擎，实现了"任何数据流向任何模型在任何硬件上"的AI开发范式转变，将瓶颈从基础设施转移到想象力，解决了AI基础设施的计算负载和数据混乱双重危机。
...
### [125] [Movable Antenna Assisted Dual-Polarized Multi-Cell Cooperative AirComp: An Alternating Optimization Approach](https://arxiv.org/abs/2601.09137)
*Mingyu Hu,Nan Liu,Wei Kang*

Main category: cs.IT

TL;DR: 本文提出了一种基于双极化可移动天线的多小区协作空中计算框架，通过联合优化组合矩阵、极化向量、天线位置和用户发射系数来最小化均方误差，显著提升了分布式优化的通信性能。


<details>
  <summary>Details</summary>
Motivation: 空中计算是分布式优化的关键使能技术，但传统固定单极化基站阵列无法充分利用空间自由度且存在极化失配问题，限制了系统性能。

Method: 提出双极化可移动天线辅助的多小区协作空中计算框架，采用交替优化算法：组合矩阵和发射系数采用闭式更新，极化向量使用SCA和SDR方法优化，天线位置采用梯度法更新，并开发了基于统计信道的天线位置优化方案。

Result: 数值结果表明，所提出的可移动双极化方案在瞬时信道和统计信道下均优于可移动单极化和固定天线基线方案。

Conclusion: 双极化可移动天线系统通过联合优化天线位置和极化配置，能够有效提升空中计算的性能，为分布式优化提供了更高效的通信解决方案。
...
### [126] [Reducing The Sub-packetization Level of Optimal-Access Cooperative MSR Codes](https://arxiv.org/abs/2601.09188)
*Yaqian Zhang,Jingke Xu*

Main category: cs.IT

TL;DR: 本文提出了一种新的最优访问协作MSR码，针对两个节点擦除情况，通过设计两个关键MDS阵列码作为构建模块，显著降低了子分组化水平。


<details>
  <summary>Details</summary>
Motivation: 协作MSR码能够以协作方式最优带宽修复多个节点擦除，但现有方案具有较高的子分组化水平，导致实际应用中的高复杂性和大输入/输出开销。本文旨在降低具有两个擦除的最优访问协作MSR码的子分组化水平。

Method: 首先设计了两个关键的MDS阵列码，用于修复特定两种擦除模式并实现最优访问。然后将这两个码作为构建模块，通过多次堆叠组合，构建出具有两个擦除的最优访问协作MSR码。

Result: 推导出的最优访问协作MSR码具有子分组化水平ℓ=r^(C(n,2)-⌊n/r⌋(C(r,2)-1))，相比现有技术（ℓ=r^(C(n,2))）降低了1/r^(⌊n/r⌋(C(r,2)-1))的比例。

Conclusion: 本文成功降低了具有两个擦除的最优访问协作MSR码的子分组化水平，通过创新的MDS阵列码设计和堆叠方法，显著减少了实际应用中的复杂性和IO开销。
...
### [127] [On Polar Coding with Feedback](https://arxiv.org/abs/2601.09222)
*Ling Liu,Qi Cao,Liping Li,Baoming Bai*

Main category: cs.IT

TL;DR: 该论文研究了在反馈辅助下极化码的性能，表明虽然反馈不提高无记忆信道容量，但能显著改善极化码的有限长度性能，通过启用"精灵辅助"解码和更灵活的极化码构造阈值。


<details>
  <summary>Details</summary>
Motivation: 尽管反馈不提高无记忆信道的容量，但研究者想探索反馈是否能改善极化码在实际有限长度下的性能表现，特别是在解码和构造方面的潜在优势。

Method: 提出在反馈辅助下的极化码构造方法，启用精灵辅助连续消除(SC)解码，并开发了新的性能分析框架来准确表征精灵辅助SC解码下的错误事件分布。

Result: 反馈显著改善了极化码的有限长度性能，提出的分析方法能准确预测精灵辅助SC解码性能，该表征也可用于预测接近容量时标准SC解码的性能。

Conclusion: 反馈虽然不改变信道容量，但通过启用精灵辅助解码和更灵活的极化码构造，能显著提升极化码在实际有限长度通信系统中的性能表现。
...
### [128] [A Theoretical Framework for Rate-Distortion Limits in Learned Image Compression](https://arxiv.org/abs/2601.09254)
*Changshuo Wang,Zijian Liang,Kai Niu,Ping Zhang*

Main category: cs.IT

TL;DR: 该论文提出了一个理论框架来分析学习型图像压缩的率失真极限，将性能损失分解为方差估计、量化策略和上下文建模三个关键组件。


<details>
  <summary>Details</summary>
Motivation: 尽管最近的神经编解码器取得了显著的实证结果，但它们与信息论极限的距离仍不清楚。现有研究缺乏对学习型压缩系统性能损失的深入理论分析。

Method: 提出系统理论框架，将率失真性能损失分解为三个组件：1) 在高斯假设下推导最优潜在方差作为二阶矩；2) 量化均匀量化与高斯测试信道之间的差距；3) 扩展框架包含上下文建模，分析准确均值预测对熵减少的影响。

Result: 通过联合仿真和端到端训练，获得了理论率失真极限的紧密且可操作的近似，为设计更高效的学习型压缩系统提供了新见解。

Conclusion: 该框架提供了结构可解释的视角，与真实压缩模块对齐，能够进行细粒度分析，有助于理解学习型图像压缩的性能瓶颈和改进方向。
...
### [129] [An Information Theoretic Proof of the Radon-Nikodym Theorem](https://arxiv.org/abs/2601.09308)
*Peter Harremoës*

Main category: cs.IT

TL;DR: 论文探讨了Radon-Nikodym定理在信息论基础概念中的核心作用，并指出该定理在概率论和信息论教材中常被省略的原因。


<details>
  <summary>Details</summary>
Motivation: Radon-Nikodym定理是信息论中香农熵、f-散度等基本量定义的基础，但在概率论和信息论教材中常被省略，因为其证明被认为过于困难。作者希望强调该定理的重要性并探讨其在教学中的处理方式。

Method: 论文通过分析Radon-Nikodym定理在信息论中的具体应用，考察该定理在不同类型教材中的呈现方式，并探讨简化证明或替代教学方法的可能性。

Result: Radon-Nikodym定理确实是信息论基础概念的核心数学基础，但在概率论和信息论教材中常被省略或简化处理，这可能导致学生对某些概念的理解不够深入。

Conclusion: 虽然Radon-Nikodym定理的证明较为复杂，但其在信息论中的基础地位不容忽视。需要在教学中找到平衡，既要让学生理解其重要性，又要避免过于复杂的数学推导影响学习效果。
...
### [130] [Contraction of Rényi Divergences for Discrete Channels: Properties and Applications](https://arxiv.org/abs/2601.09328)
*Adrien Vandenbroucque,Amedeo Roberto Esposito,Michael Gastpar*

Main category: cs.IT

TL;DR: 该研究探讨了Rényi散度的强数据处理常数性质，与φ-散度进行比较，发现当α>1时收缩性质显著不同，并应用于马尔可夫链收敛速度分析。


<details>
  <summary>Details</summary>
Motivation: 研究Rényi散度的强数据处理常数性质，与已有研究的φ-散度进行对比，探索Rényi散度阶数α如何影响收缩性质，并将其应用于分析马尔可夫链的收敛速度。

Method: 通过理论分析Rényi散度的强数据处理常数性质，与φ-散度进行系统比较，特别关注α>1时的差异，研究无穷阶Rényi散度的收缩特性及其与ε-局部差分隐私的关系。

Result: 发现当α>1时，Rényi散度的收缩性质与φ-散度存在显著差异；揭示了无穷阶Rényi散度的具体收缩特性及其与局部差分隐私的关联；将结果应用于马尔可夫链收敛速度分析，为L^α-范数收缩提供了新视角。

Conclusion: Rényi散度的收缩性质强烈依赖于阶数α，当α>1时表现出与φ-散度不同的特性，这为马尔可夫链收敛分析提供了新的理论工具和视角。
...
### [131] [Generalized Schalkwijk-Kailath Coding for Autoregressive Gaussian Channels](https://arxiv.org/abs/2601.09329)
*Jun Su,Guangyue Han,Shlomo Shamai*

Main category: cs.IT

TL;DR: 提出了一种用于AR(p)高斯信道的高斯随机编码方案SK(2)，推广了经典的Schalkwijk-Kailath编码方案，并给出了可达速率的闭式表达式，证明了SK方案并非普遍最优，推翻了Butman的猜想。


<details>
  <summary>Details</summary>
Motivation: 经典Schalkwijk-Kailath(SK)编码方案在特定信道条件下表现出色，但Butman猜想SK方案对所有AR(p)高斯信道都是最优的。本文旨在验证这一猜想的正确性，并探索更通用的编码方案。

Method: 提出SK(2)编码方案，这是对经典SK方案的推广。该方案采用高斯随机编码方法，专门针对AR(p)高斯信道设计，能够给出可达速率的闭式解析表达式。

Result: SK(2)编码方案成功实现了对AR(p)高斯信道的编码，并给出了可达速率的闭式表达式。更重要的是，该结果证明了经典SK编码方案并非对所有AR(p)高斯信道都是最优的，从而推翻了Butman在1976年提出的猜想。

Conclusion: 通过提出SK(2)编码方案并分析其性能，证明了经典Schalkwijk-Kailath编码方案并非普遍最优，成功推翻了Butman的长期猜想，为AR(p)高斯信道的编码设计提供了新的理论框架。
...
### [132] [A Constructive Method to Minimize the Index of Coincidence under Marginal Constraints](https://arxiv.org/abs/2601.09347)
*Pierre Jean-Claude Robert Bertrand*

Main category: cs.IT

TL;DR: 本文研究了在固定边缘约束下最小化联合分布重合指数的问题，提出了完整的构造性解决方案，包括最优耦合的结构特征和有限步收敛的迭代算法。


<details>
  <summary>Details</summary>
Motivation: 重合指数最小化问题在信息论中有多个应用，但现有闭式解需要强可行性条件，而该条件在实践中很少满足，因此需要更通用的解决方案。

Method: 首先分析强可行性条件的局限性，然后表征一般情况下的最优耦合结构（单调阶梯状零值模式），基于此结构提出显式迭代构造方法，并证明其在有限步内收敛到最小化解。

Result: 证明了强可行性条件在维度增长时几乎不成立，揭示了最优耦合具有单调阶梯状零值结构，提出的迭代算法能在有限步内收敛，实现了重合指数最小化问题的完整构造性解。

Conclusion: 本文完全解决了重合指数最小化问题，提供了不依赖强可行性条件的通用构造性解决方案，对信息论相关应用具有重要价值。
...
### [133] [Asymptotic Rate Bounds and Constructions for the Inclusive Variant of Disjunct Matrices](https://arxiv.org/abs/2601.09362)
*Yuto Mizunuma,Yuichiro Fujiwara*

Main category: cs.IT

TL;DR: 本文首次建立了包含性分离矩阵的渐近正速率下界，匹配了已知上界（差一个对数因子），为通用抑制剂复杂模型下的可扩展群组检测提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 包含性分离矩阵在通用抑制剂复杂模型下的群组检测中具有重要应用价值，但此前对其渐近行为（特别是能否达到渐近正速率）缺乏研究，限制了可扩展群组检测设计的发展。

Method: 采用概率方法证明渐近下界，并基于此提出了简单高效的随机构造方法，随后对该构造进行去随机化处理，得到了确定性的多项式时间构造算法。

Result: 首次建立了包含性分离矩阵的非平凡渐近下界，该下界与已知最强上界仅差一个对数因子，证明了该变体能够达到渐近正速率，满足可扩展群组检测设计的要求。

Conclusion: 该研究阐明了在通用抑制剂复杂模型下鲁棒且可扩展群组检测的渐近潜力，为实际应用提供了理论基础和构造方法。
...
### [134] [On Decoding First- and Second-Order BiD Codes](https://arxiv.org/abs/2601.09390)
*Devansh Jain,Lakshmi Prasad Natarajan*

Main category: cs.IT

TL;DR: 本文为BiD码设计了快速ML和max-log-MAP解码器，针对二阶码识别了最小权重奇偶校验并确认了"投影"特性，设计了性能接近ML解码器的BP解码器。


<details>
  <summary>Details</summary>
Motivation: BiD码是一种新型代数码，在比特MAP解码下能达到擦除信道容量，且比RM码具有更大的渐近最小距离。然而，需要开发高效的解码算法来充分发挥其性能优势。

Method: 1. 为一阶BiD码设计快速最大似然(ML)和max-log-MAP解码器；2. 针对二阶BiD码，识别其最小权重奇偶校验并确认类似RM码中的"投影"特性；3. 基于这些结果设计置信传播(BP)解码器。

Result: 设计的BP解码器在块长度为81和243时，性能在ML解码器的1 dB范围内，表明该解码器具有接近最优的性能。

Conclusion: 本文为BiD码开发了高效的解码算法，特别是一阶码的快速ML解码器和二阶码的BP解码器，后者性能接近最优，为BiD码的实际应用提供了重要工具。
...
### [135] [A Generalized Leakage Interpretation of Alpha-Mutual Information](https://arxiv.org/abs/2601.09406)
...
<details>
  <summary>Details</summary>
Motivation: 为α-互信息提供统一的解释框架，将其与定量信息流中的广义g-泄露概念联系起来，从而更好地理解α参数在信息安全中的实际意义

Method: 基于对抗性广义决策问题的扩展框架，使用Kolmogorov-Nagumo均值和q-对数来表征对抗性收益，在该框架下解释α-互信息

Result: 成功建立了α-互信息与广义g-泄露之间的理论联系，证明了α参数可以解释为对抗者风险厌恶程度的度量

Conclusion: α-互信息可以在对抗性广义决策框架下统一解释为广义g-泄露，其中α参数反映了对抗者的风险偏好，为信息安全中的信息泄露量化提供了新的理论视角
...
### [136] [Dobrushin Coefficients of Private Mechanisms Beyond Local Differential Privacy](https://arxiv.org/abs/2601.09498)
*Leonhard Grosse,Sara Saeidian,Tobias J. Oechtering,Mikael Skoglund*

Main category: cs.IT

TL;DR: 研究具有有界点态最大泄漏的离散马尔可夫核的Dobrushin系数，该定义在c→0时恢复局部差分隐私，推导了基于PML保证的收缩界，并提供了实现这些界的机制构造


<details>
  <summary>Details</summary>
Motivation: 研究具有有界点态最大泄漏(PML)的离散马尔可夫核的收缩性质，将局部差分隐私(LDP)推广到更一般的隐私保护框架，探索在LDP之外更广泛的隐私机制

Method: 通过分析具有有界PML的离散马尔可夫核的Dobrushin系数，推导收缩界；提供实现这些界的机制构造；应用Binette不等式将结果扩展到一般的f-散度

Result: 获得了基于PML保证的收缩界，提供了实现这些界的机制构造，得到了比LDP更紧的界，并将结果扩展到任意离散核和一般f-散度

Conclusion: 该研究建立了有界PML与收缩性质之间的联系，为隐私保护机制提供了新的理论框架，扩展了LDP的分析工具，并为更广泛的隐私机制提供了理论保证
...
### [137] [Error Exponents for Randomised List Decoding](https://arxiv.org/abs/2601.09519)
*Henrique K. Miyamoto,Sheng Yang*

Main category: cs.IT

TL;DR: 该论文研究了随机列表解码的随机编码错误指数，分析了固定列表大小和指数增长列表大小两种机制下的性能界限


<details>
  <summary>Details</summary>
Motivation: 研究随机列表解码的错误指数性能，探索列表解码在不同解码度量下的理论极限，为实际编码方案提供理论指导

Method: 采用随机编码理论分析，研究两种机制：固定列表大小和指数增长列表大小，分别推导错误指数界限，并针对匹配度量、失配度量和通用度量进行特化分析

Result: 对于固定列表大小，推导了集合紧的随机编码错误指数，并证明在匹配度量下无法改善普通解码的错误指数；对于指数增长列表大小，给出了错误指数的非平凡下界，在高码率下对于匹配度量是紧的

Conclusion: 随机列表解码在不同机制下表现出不同的错误指数特性，固定列表大小在匹配度量下无优势，而指数增长列表大小在高码率下能提供性能改进
...
### [138] [A Finite-Sample Strong Converse for Binary Hypothesis Testing via (Reverse) Rényi Divergence](https://arxiv.org/abs/2601.09550)
*Roberto Bruno,Adrien Vandenbroucque,Amedeo Roberto Esposito*

Main category: cs.IT

TL;DR: 该研究在有限样本下探讨二元假设检验，使用反向Rényi散度推导了Type II错误概率的非渐近界，建立了强逆定理，并分析了Type I错误指数衰减时的Type II错误行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机是在有限样本和非对称错误约束下，改进二元假设检验的性能分析。现有方法在有限样本下的界限可能不够紧致，需要更精确的非渐近分析工具。

Method: 采用反向Rényi散度作为分析工具，推导Type II错误概率的非渐近上界和下界。通过建立强逆定理，分析当Type I错误以指数速率衰减时Type II错误的行为。

Result: 1) 推导出基于反向Rényi散度的Type II错误概率非渐近界；2) 建立了强逆定理；3) 证明当Type I错误衰减速率c超过KL散度D(P1∥P0)时，Type II错误以指数速率趋近于1，当c较小时则以指数速率趋近于0；4) 数值实验显示所提逆界严格优于现有有限样本结果。

Conclusion: 反向Rényi散度为有限样本下的二元假设检验提供了有效的分析框架，所推导的非渐近界改进了现有结果，为实际应用中的错误概率控制提供了更精确的理论指导。
...
### [139] [On Linear Estimators for some Stable Vectors](https://arxiv.org/abs/2601.09554)
*Rayan Chouity,Charbel Hannoun,Jihad Fahs,Ibrahim Abou-Faycal*

Main category: cs.IT

TL;DR: 研究联合稳定随机变量的估计问题，在线性变换和子高斯对称α稳定向量两种依赖模型下，证明了条件均值估计器是线性的，并找到了分散最优线性估计器。


<details>
  <summary>Details</summary>
Motivation: 研究联合稳定随机变量的估计问题，探索在非高斯稳定分布下的线性估计特性，特别是条件均值估计器的性质。

Method: 考虑两种依赖模型：1) 两个独立稳定变量的线性变换；2) 子高斯对称α稳定向量。分析这两种模型下的条件均值估计器特性。

Result: 在这两种依赖模型下，条件均值估计器都是线性的。找到了分散最优线性估计器。对于子高斯对称α稳定向量，两种估计器是相同的，这推广了高斯情况下条件均值是最佳线性最小均方估计器的经典结果。

Conclusion: 对于联合稳定随机变量，在线性变换和子高斯对称α稳定向量两种依赖模型下，条件均值估计器具有线性性质，并且可以找到分散最优线性估计器，这为稳定分布下的估计问题提供了重要理论结果。
...
### [140] [The Spectral Representations Of The Simple Hypothesis Testing Problem](https://arxiv.org/abs/2601.09564)
*Barış Nakiboğlu*

Main category: cs.IT

TL;DR: 论文研究了假设检验问题中II类错误概率（体积）关于I类错误概率（体积）的凸共轭（Legendre变换），推导了原始熵谱的表达式，并应用于乘积测度情况获得最新边界。


<details>
  <summary>Details</summary>
Motivation: 研究假设检验问题中I类错误和II类错误之间的基本关系，通过凸共轭变换建立对II类错误体积的数学表征，为统计推断提供理论基础。

Method: 使用似然比分位数性质推导凸共轭表达式，基于标准谱恒等式将原始熵谱表示为似然比互补分布函数的积分，并应用Berry-Esseen定理和Gaussian Mills比性质分析乘积测度情况。

Result: 确定了随机检测器下II类错误体积关于I类错误体积的凸共轭表达式，得到了原始熵谱的积分表示，并通过Berry-Esseen定理获得了乘积测度情况下的最新边界。

Conclusion: 建立了假设检验中II类错误体积的凸共轭理论框架，提供了原始熵谱的数学表征，为统计推断中的错误概率分析提供了新的理论工具和边界结果。
...
### [141] [On the Error Probability of RPA Decoding of Reed-Muller Codes over BMS Channels](https://arxiv.org/abs/2601.09581)
*Dorsa Fathollahi,V. Arvind Rameshwar,V. Lalitha*

Main category: cs.IT

TL;DR: 本文分析了RPA解码器在一般二进制无记忆对称信道上的性能，证明了对于低阶RM码，RPA解码器在所有BMS信道上都能实现渐近消失的错误概率。


<details>
  <summary>Details</summary>
Motivation: 先前的研究只证明了RPA解码器在二进制对称信道上的性能，本文旨在将其推广到更一般的二进制无记忆对称信道，消除对信道的限制性假设。

Method: 通过建立RPA投影操作与极化码"信道合并"阶段的等价关系，利用这种等价性对一阶RM码的最大似然解码错误概率应用通用联合界，然后将这些观察应用于Rameshwar和Lalitha的证明策略。

Result: 证明了对于阶数约为log log n的RM码，在所有BMS信道上，当码长n趋于无穷时，RPA解码器都能实现渐近消失的错误概率。

Conclusion: RPA解码器的性能分析成功从二进制对称信道推广到所有二进制无记忆对称信道，为低阶RM码的解码提供了更通用的理论保证。
...
### [142] [Secret sharing with additive access structures from correlated random variables](https://arxiv.org/abs/2601.09640)
*David Miller,Rémi A. Chou*

Main category: cs.IT

TL;DR: 该论文将基于相关随机性和公共通信的秘密共享模型从固定访问结构推广到动态访问结构序列（称为加法访问结构），证明了存在策略能在每个时间步达到与固定访问结构模型相同的最优秘密率，且在阈值访问结构时能达到容量最优。


<details>
  <summary>Details</summary>
Motivation: 传统秘密共享模型针对固定访问结构设计，而现实应用中访问结构可能动态变化。论文旨在将基于相关随机性和公共通信的秘密共享模型扩展到支持动态变化的访问结构序列，使系统能够适应参与者集合随时间单调增长的情况。

Method: 提出"加法访问结构"模型，允许访问结构在每个时间步单调增长（添加参与者子集）。经销商仅在变化发生时获知访问结构变更。通过理论证明存在策略能在每个时间步达到与固定访问结构模型相同的最优秘密率，并特别证明了在阈值访问结构情况下存在容量最优策略。

Result: 证明了存在秘密共享策略能在动态访问结构序列的每个时间步达到与固定访问结构模型相同的最优秘密率。同时证明了当访问结构为阈值访问结构时，存在容量最优策略。

Conclusion: 成功将基于相关随机性和公共通信的秘密共享模型扩展到动态访问结构场景，证明了在访问结构单调增长的情况下，系统性能可以达到与固定访问结构相同的理论最优水平，为动态环境下的安全多方计算提供了理论基础。
...
### [143] [Counting and Entropy Bounds for Structure-Avoiding Spatially-Coupled LDPC Constructions](https://arxiv.org/abs/2601.09674)
*Lei Huang*

Main category: cs.IT

TL;DR: 本文量化了设计大耦合存储器准循环空间耦合LDPC码的可行设计空间大小和结构，通过量化CLLL方法推导了满足结构避免约束的分区矩阵数量的显式下界，并提供了MT算法输出不同解的可计算下界。


<details>
  <summary>Details</summary>
Motivation: 设计具有低错误平层的大耦合存储器准循环空间耦合LDPC码需要消除由边扩展和提升引起的特定有害子结构（如短环）。需要量化可行设计空间的大小和结构，为系统参数选择和剩余搜索空间估计提供理论依据。

Method: 基于Clique Lovász Local Lemma (CLLL)的设计原则和Moser-Tardos (MT)型构造方法，使用量化CLLL推导满足结构避免约束的分区矩阵数量的显式下界；通过Rényi熵界分析MT分布，提供MT算法输出不同解的可计算下界；针对消除4环候选的特殊化分析得到闭式界。

Result: 获得了满足给定结构避免约束的分区矩阵数量的显式下界，以及在行/列置换下非等价解数量的界；提供了MT算法输出不同解的可计算下界，为随机化构造提供了具体的多样性保证；针对4环消除的特殊情况得到了系统参数的闭式界。

Conclusion: 本文量化了QC-SC-LDPC码设计的可行空间，提供了系统参数选择和搜索空间估计的原则性方法，为设计具有低错误平层的大耦合存储器码提供了理论指导和多样性保证。
...
### [144] [Progress on the Courtade-Kumar Conjecture: Optimal High-Noise Entropy Bounds and Generalized Coordinate-wise Mutual Information](https://arxiv.org/abs/2601.09679)
*Adel Javanmard,David P. Woodruff*

Main category: cs.IT

TL;DR: 该论文在Courtade-Kumar猜想方面取得两大进展：1) 解决了Courtade和Kumar提出的开放问题，证明任意布尔函数（无论偏置）的输出与各噪声输入坐标的互信息和不超过1-H(α)；2) 在高噪声区域改进了主要猜想的研究，建立了O(λ²)的最优误差界，显著扩展了猜想成立的噪声参数范围。


<details>
  <summary>Details</summary>
Motivation: Courtade-Kumar猜想认为在布尔超立方上，独裁函数能够最大化函数输出与其噪声输入之间的互信息。该研究旨在推进这一猜想的研究，特别是解决Courtade和Kumar提出的开放问题，并在高噪声区域取得更精确的边界。

Method: 采用信息论和傅里叶分析的方法，对布尔函数在二进制对称信道下的互信息进行理论分析。通过熵展开技术，建立了更精确的误差界，并利用傅里叶浓度边界来分析高度信息性函数。

Result: 1) 证明了任意布尔函数的输出与各噪声输入坐标的互信息和不超过1-H(α)，解决了Courtade和Kumar的开放问题；2) 在高噪声区域建立了O(λ²)的最优误差界，其中λ=(1-2α)²，显著改进了先前已知的边界，并扩展了猜想成立的噪声参数范围。

Conclusion: 该研究在Courtade-Kumar猜想方面取得了重要进展，不仅解决了先前提出的开放问题，还在高噪声区域获得了更精确的理论边界，为理解布尔函数在噪声环境下的信息特性提供了新的理论工具。
...
TL;DR: 本文针对教育应用中LLMs的局限性，提出了ConvoLearn数据集和知识建构理论框架，通过微调LLMs显著提升了教学对话质量。


<details>
  <summary>Details</summary>
Motivation: 当前教育应用中的LLMs存在根本性教学局限性，倾向于直接提供解决方案而非支持对话式学习，缺乏有效的教学策略支持。

Method: 基于知识建构理论构建ConvoLearn数据集，包含六个核心教学维度；通过人类教师与模拟学生交互创建1250个中学地球科学对话；使用QLoRA对Mistral 7B进行微调。

Result: 微调后的Mistral 7B（M=4.10, SD=1.03）在31名教师评估中显著优于基础版本（M=2.59, SD=1.11）和Claude Sonnet 4.5（M=2.87, SD=1.29）。

Conclusion: 该研究为建构主义AI导师的未来发展和评估建立了潜在框架，证明了基于教学理论的数据集训练能有效改善LLMs的教学行为。
...
TL;DR: 该研究评估了前沿AI模型在电商RL环境中150个职场任务的表现，揭示了模型必须掌握的五个层次能力，发现即使最佳模型仍有约40%任务失败，失败模式沿能力层次呈现规律性分布。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的智能体发展，AI评估已从单轮响应评估转向交互环境中的多步骤任务完成评估。研究者希望了解前沿AI模型在真实职场环境中的实际表现，识别模型能力差距，为智能体开发提供指导。

Method: 在Surge提供的真实电商RL环境中，对前沿AI模型进行了150个职场任务的实证评估。引入了任务中心设计方法，强调任务多样性和领域专家贡献，并进行了详细的失败分析。

Result: 研究发现了经验推导的智能体能力层次结构：工具使用、规划与目标形成、适应性、接地性、常识推理。最佳模型仍有约40%任务失败，失败模式沿能力层次分布：较弱模型在基础工具使用和规划上挣扎，较强模型主要在需要超越明确指令的上下文推理任务上失败。

Conclusion: 当前前沿模型虽能展示连贯的多步骤行为，但在真实职场环境中实现人类水平的任务完成仍存在显著能力差距。研究为智能体开发提供了重要启示，强调需要针对能力层次中的薄弱环节进行改进。
...
Method: 提出SCOPE框架，将查询特定的推理与通用代码执行解耦，通过分离推理与执行，生成一致、确定且可重用的求解器函数，仅需对输入参数进行最小修改。

Result: SCOPE实现了最先进的性能，同时降低了成本和延迟。使用GPT-4o时，在TravelPlanner上达到93.1%成功率，比最佳基线（CoT）提升61.6%，推理成本降低1.4倍，时间减少约4.67倍。

Conclusion: SCOPE框架通过分离推理与执行，有效解决了多约束规划中LLM方法的局限性，实现了高性能、低成本、可重用的解决方案，为复杂约束规划问题提供了有效框架。
...
TL;DR: 本文提出DScheLLM，一种基于微调大语言模型的双系统（快慢）推理架构，用于处理动态生产调度中的各种规模扰动。


<details>
  <summary>Details</summary>
Motivation: 传统生产调度方法对动态扰动（如处理时间变化、机器可用性变化、意外任务插入）敏感，通常依赖特定事件模型和显式分析公式，限制了其在未见扰动上的适应性和泛化能力。

Method: 构建统一的大语言模型框架处理动态事件，使用运筹学求解器获得的精确调度生成快慢推理模式的训练数据集，基于华为OpenPangu Embedded-7B模型使用LoRA进行混合推理范式微调。

Result: 在标准作业车间调度基准测试中，快思考模式能高效生成高质量调度，慢思考模式能产生与求解器兼容且格式良好的决策输入。
...
TL;DR: 本文提出AviationLMM愿景——一个用于民航的大型多模态基础模型，旨在统一民航异构数据流，实现理解、推理、生成和智能体应用，以解决现有AI解决方案在民航领域的局限性。
...
Method: 提出AviationLMM模型架构，能够处理多模态输入（如空地语音、监视数据、机载遥测、视频和结构化文本），执行跨模态对齐和融合，并产生灵活的输出（从态势摘要和风险警报到预测性诊断和多模态事件重建）。同时识别了实现该愿景需要解决的关键研究机会。

Result: 本文提出了AviationLMM的设计愿景和架构框架，但作为愿景性论文，尚未展示具体实验结果。主要成果是明确了民航基础模型的发展方向和需要解决的关键技术挑战。

Conclusion: 通过阐述AviationLMM的设计和挑战，旨在推动民航基础模型的进展，并促进协调的研究努力，朝着构建集成、可信和隐私保护的民航AI生态系统迈进。
...
TL;DR: 该论文对大型语言模型和多模态大语言模型中的记忆机制进行了系统性综述，将相关研究分为隐式记忆、显式记忆和智能体记忆三大范式，并探讨了多模态环境下的记忆整合及当前挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs和MLLMs从静态预测器向能够持续学习和个性化推理的交互式系统转变，记忆机制已成为其架构和功能演进的核心主题。需要对这些记忆机制进行系统性梳理和分类，以推动该领域的发展。

Method: 采用文献综述方法，将现有研究组织成统一的分类体系：1)隐式记忆（预训练transformer参数中的知识）；2)显式记忆（外部存储和检索组件）；3)智能体记忆（自主智能体中的持久记忆结构）。同时扩展到多模态环境下的记忆整合。

Result: 建立了LLMs和MLLMs记忆研究的分类框架，涵盖了三种主要记忆范式及其在多模态环境中的应用。识别了关键架构进展、基准任务，并分析了当前面临的挑战。

Conclusion: 记忆机制对增强LLMs和MLLMs的推理能力、适应性和上下文保真度具有基础性作用。该综述为理解记忆在AI系统中的演变提供了结构化框架，并指出了未来研究方向，包括记忆容量、对齐、事实一致性和跨系统互操作性等挑战。
...
### [151] [Position on LLM-Assisted Peer Review: Addressing Reviewer Gap through Mentoring and Feedback](https://arxiv.org/abs/2601.09182)
*JungMin Yun,JuneHyoung Kwon,MiHyeon Kim,YoungBin Kim*

Main category: cs.AI

TL;DR: 本文提出将LLM作为辅助工具来提升人工评审质量，而非完全自动化生成评审，以解决AI研究快速发展带来的评审质量危机


<details>
  <summary>Details</summary>
Motivation: AI研究的快速扩张加剧了"评审者缺口"，威胁同行评审的可持续性，并导致低质量评审的恶性循环。现有使用LLM自动生成评审的方法存在问题，需要范式转变

Method: 提出以人为中心的LLM辅助方法：1）LLM辅助的导师系统，培养评审者的长期能力；2）LLM辅助的反馈系统，帮助评审者提升评审质量。这些系统基于高质量同行评审的核心原则

Result: 通过将LLM定位为辅助和教育工具而非替代品，该方法旨在增强评审者专业知识，为构建更可持续的学术生态系统做出贡献

Conclusion: 需要从LLM自动生成评审的范式转向LLM辅助人工评审的范式，通过培养评审者能力和提供实时反馈来提升评审质量，解决同行评审的可持续性危机
...
### [152] [MAXS: Meta-Adaptive Exploration with LLM Agents](https://arxiv.org/abs/2601.09259)
*Jian Zhang,Zhiyuan Wang,Zhangqi Wang,Yu He,Haoran Luo,li yuan,Lingling Zhang,Rui Mao,Qika Lin,Jun Liu*

Main category: cs.AI

TL;DR: MAXS是一个基于LLM Agent的元自适应推理框架，通过前瞻策略和轨迹收敛机制，解决了现有方法中的局部短视生成和轨迹不稳定问题，在多工具推理中平衡了全局有效性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM Agent方法存在两个主要问题：1) 由于缺乏前瞻性导致局部短视生成；2) 早期微小错误会演变成发散推理路径的轨迹不稳定性。这些问题使得难以平衡全局有效性和计算效率。

Method: MAXS采用元自适应推理框架，灵活整合工具执行和推理规划。使用前瞻策略向前扩展推理路径几步，估计工具使用的优势值；结合步骤一致性方差和步骤间趋势斜率共同选择稳定、一致且高价值的推理步骤；引入轨迹收敛机制，在路径一致性达成时停止进一步展开，控制计算成本。
...
### [153] [Efficient Paths and Dense Rewards: Probabilistic Flow Reasoning for Large Language Models](https://arxiv.org/abs/2601.09260)
...
<details>
  <summary>Details</summary>
Motivation: 现有思维链方法将推理过程视为不可分割的序列，缺乏量化步骤信息增益的内在机制，导致推理效率低下（冗余探索）和优化困难（稀疏监督或昂贵验证器）
...
### [154] [Coordinated Pandemic Control with Large Language Model Agents as Policymaking Assistants](https://arxiv.org/abs/2601.09264)
*Ziyi Shi,Xusen Guo,Hongliang Lu,Mingxing Peng,Haotian Wang,Zheng Zhu,Zhenning Li,Yuxuan Liang,Xinhu Zheng,Hai Yang*

Main category: cs.AI

TL;DR: 提出基于大语言模型的多智能体政策制定框架，用于协调和主动的跨区域疫情控制，相比现实世界结果能显著降低感染和死亡人数


<details>
  <summary>Details</summary>
Motivation: 传统疫情控制中，人类驱动的响应往往是碎片化和被动的，政策制定孤立且仅在疫情升级后才调整，这削弱了主动干预和全球疫情缓解。需要解决跨行政区域协调和主动政策制定的挑战。

Method: 提出LLM多智能体政策制定框架，每个行政区域分配一个LLM智能体作为AI政策制定助手。智能体在考虑区域特定流行病学动态的同时，与其他智能体通信以考虑跨区域相互依赖性。通过整合真实世界数据、疫情演化模拟器和结构化智能体间通信，使智能体能够联合探索反事实干预场景，并通过闭环模拟过程合成协调的政策决策。

Result: 使用美国2020年4月至12月的州级COVID-19数据、真实世界移动记录和观察到的政策干预进行验证。相比现实世界疫情结果，该方法在单个州层面分别将累计感染和死亡减少高达63.7%和40.1%，在跨州聚合层面分别减少39.0%和27.0%。

Conclusion: LLM多智能体系统能够通过协调的政策制定实现更有效的疫情控制，展示了AI辅助决策在公共卫生危机管理中的潜力。
...
### [155] [RISER: Orchestrating Latent Reasoning Skills for Adaptive Activation Steering](https://arxiv.org/abs/2601.09269)
...
<details>
  <summary>Details</summary>
Motivation: 现有领域特定推理方法通常需要参数更新，而激活引导方法采用静态手动干预，无法适应复杂推理的动态特性，需要更灵活、自适应的干预框架。

Method: 提出RISER框架：1)构建可复用推理向量库；2)使用轻量级路由器动态组合这些向量；3)通过强化学习在任务级奖励下优化路由器，以涌现和组合方式激活潜在认知原语。

Result: 在7个多样化基准测试中，RISER相比基础模型平均零样本准确率提升3.4-6.5%，超越CoT推理方法，具有2-3倍更高的令牌效率和稳健的准确率增益。

Conclusion: RISER能够自主组合多个向量形成可解释的精确控制策略，为更可控、高效的LLM推理提供了新方向，展示了自适应激活引导的潜力。
...
### [156] [$A^3$-Bench: Benchmarking Memory-Driven Scientific Reasoning via Anchor and Attractor Activation](https://arxiv.org/abs/2601.09274)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估最终答案或逐步推理的连贯性，但忽视了人类科学推理中基于记忆驱动的机制，这种机制涉及激活先验知识中的锚点和吸引子，并将其整合到多步推理中。
...
### [157] [STaR: Sensitive Trajectory Regulation for Unlearning in Large Reasoning Models](https://arxiv.org/abs/2601.09281)
...
Method: STaR框架包含四个步骤：1）语义感知检测识别敏感内容；2）通过安全提示前缀注入全局安全约束；3）轨迹感知抑制动态阻断整个推理链的敏感内容；4）词元级自适应过滤防止精确和转述的敏感词元生成

Result: 在R-TOFU基准测试中，STaR实现了全面稳定的遗忘效果，同时保持最小的效用损失，为LRMs的隐私保护推理设立了新标准

Conclusion: STaR框架有效解决了大型推理模型在复杂思维链生成中的隐私保护问题，通过推理时遗忘机制实现了推理全过程的隐私安全，同时提出了更全面的评估指标来量化隐私保护效果
...
### [158] [Cluster Workload Allocation: Semantic Soft Affinity Using Natural Language Processing](https://arxiv.org/abs/2601.09282)
*Leszek Sliwko,Jolanta Mizeria-Pietraszko*

Main category: cs.AI

TL;DR: 该论文提出了一种基于自然语言处理的语义化、意图驱动的集群调度范式，通过LLM解析自然语言分配提示注释来实现软亲和性偏好，在Kubernetes调度器扩展器中集成，原型系统在复杂场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 集群工作负载分配通常需要复杂的配置，存在可用性差距。为了解决这个问题，本文旨在通过自然语言处理简化集群调度配置，降低使用门槛。

Method: 系统采用大型语言模型（LLM）通过Kubernetes调度器扩展器集成，解析自然语言分配提示注释以实现软亲和性偏好。开发了包含集群状态缓存和意图分析器（使用AWS Bedrock）的原型系统。

Result: 实证评估显示，顶级模型（如Amazon Nova Pro/Premier和Mistral Pixtral Large）的LLM解析准确率超过95%（在评估基准数据集上的子集准确率），显著优于基线引擎。在六个场景的调度质量测试中，原型系统实现了优于或等同于标准Kubernetes配置的放置效果，特别是在复杂和定量场景以及处理冲突的软偏好方面表现出色。

Conclusion: 研究结果验证了使用LLM进行可访问调度的可行性，但指出了同步LLM延迟等限制，建议采用异步处理以实现生产就绪。这项工作确认了语义软亲和性在简化工作负载编排方面的可行性。
...
### [159] [Long-term Task-oriented Agent: Proactive Long-term Intent Maintenance in Dynamic Environments](https://arxiv.org/abs/2601.09382)
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型智能体主要采用被动响应范式，只能在短期会话中响应用户即时查询，无法维护长期用户意图并适应动态变化的外部环境。

Method: 1) 形式化主动性的两个关键能力：意图条件监控（基于对话历史自主制定触发条件）和事件触发跟进（检测到有用环境更新时主动与用户互动）；2) 引入高质量数据合成管道构建动态环境中的复杂多轮对话数据；3) 提出新的基准测试ChronosBench来评估动态环境中的任务导向交互。

Result: 使用合成数据进行监督学习的微调模型在包含用户意图转移的复杂任务中实现了85.19%的任务完成率，优于其他测试模型，验证了数据驱动策略的有效性。同时评估了当前领先的闭源和开源模型，揭示了它们在长期任务导向交互中的缺陷。

Conclusion: 提出的主动式任务导向智能体交互范式能够弥合相对静态的用户需求与动态环境之间的差距，通过数据驱动的方法显著提升了智能体在动态环境中的长期任务执行能力。
...
### [160] [Omni-R1: Towards the Unified Generative Paradigm for Multimodal Reasoning](https://arxiv.org/abs/2601.09536)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs要么只进行纯文本推理，要么采用单一任务特定的推理模式，无法适应需要多样化推理技能（如区域放大、物体标记等）的各种多模态任务，需要统一的推理框架。

Method: 提出统一生成式多模态推理范式，在推理过程中生成中间图像。具体实现为Omni-R1：两阶段SFT+RL框架，包含感知对齐损失和感知奖励以实现功能性图像生成。还提出Omni-R1-Zero，从纯文本推理数据中引导逐步可视化，无需多模态标注。

Result: 实验结果表明，Omni-R1能在广泛的多模态任务上实现统一生成式推理，而Omni-R1-Zero在平均表现上能匹配甚至超越Omni-R1，展示了生成式多模态推理的潜力。

Conclusion: 通过生成中间图像的统一生成式多模态推理框架能有效统一多样化推理技能，且无需多模态标注的方法也能达到良好效果，为多模态推理提供了有前景的方向。
...
### [161] [Collaborative Multi-Agent Test-Time Reinforcement Learning for Reasoning](https://arxiv.org/abs/2601.09667)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然通过多样性和交叉验证获得鲁棒性，但多智能体强化学习训练资源密集且不稳定：智能体间的协同适应导致非平稳性，奖励通常稀疏且方差高。

Method: MATTRL构建多专家团队进行多轮讨论，检索并整合测试时经验，达成共识进行最终决策。研究信用分配机制构建回合级经验池，并将其重新注入对话。
