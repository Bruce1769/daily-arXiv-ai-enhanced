...
TL;DR: 本文提出了一种AI增强的局部线性嵌入(LLE)模型，用于改进医疗账单和转录服务的高维数据处理，通过自动化减少人为错误并提高操作效率。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（如医疗账单和转录）创造了机会。传统方法在处理高维医疗数据时存在准确性和效率问题，需要创新的AI解决方案来优化这些关键医疗流程。

Method: 开发了AI增强的局部线性嵌入(LLE)模型，专门针对高维医疗数据处理进行优化。该方法结合了LLE的降维能力和AI的智能处理能力，为医疗账单系统和转录服务提供定制化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时经常利用虚假相关性而非因果视觉证据，微调后更容易产生捷径依赖，需要解决这一问题。

Method: 提出VISTA框架，通过信息瓶颈将感知与推理解耦：冻结的VLM传感器仅进行简短客观的感知查询，文本LLM推理器分解问题、规划查询、聚合视觉事实，使用强化学习（GRPO）在少量多步问题上训练无偏视觉推理。

Result: 在SpuriVerse数据集上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集上保持竞争力，能跨VLM传感器迁移并识别感知失败，推理轨迹更中立、更少依赖虚假属性、更明确基于视觉证据。

Conclusion: VISTA通过模块化设计和信息瓶颈有效减少视觉语言模型对虚假相关性的依赖，提升视觉推理的鲁棒性和可解释性，为构建更可靠的视觉问答系统提供了新思路。
...
TL;DR: SAMM2D双编码器框架在动脉瘤检测中取得0.686 AUC，比临床基线提升32%，但发现强预训练骨干下数据增强反而降低性能，挑战了医疗影像中"更多增强总是更好"的假设。


<details>
  <summary>Details</summary>
Motivation: 动脉瘤检测对预防致命性出血至关重要，但面临动脉瘤形态细微、类别极度不平衡和标注数据稀缺的挑战，需要开发更有效的检测方法。

Method: 提出SAMM2D双编码器框架，在RSNA颅内动脉瘤数据集上进行评估，通过六种增强策略的全面消融实验，发现强预训练骨干下数据增强反而有害，采用无增强基线模型。

Result: AUC达到0.686，比临床基线提升32%；无增强模型优于所有增强变体1.75-2.23个百分点；校准决策阈值后达到95%灵敏度，超过放射科医生平均水平；Grad-CAM可视化显示85%真阳性关注相关血管区域。

Conclusion: 强预训练特征已捕获稳健不变性，额外增强既冗余又破坏特征流形；未来医疗影像工作流可能从强预训练获益多于复杂增强管道；模型有望在筛查应用中节省大量医疗成本。
...
TL;DR: HookMIL是一个用于计算病理学中全切片图像弱监督分析的多实例学习框架，通过可学习的hook tokens实现上下文感知和计算高效，支持多模态初始化并引入多样性损失和token间通信机制。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在处理全切片图像时丢失关键上下文信息，而基于transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题。需要开发一种既能保留上下文信息又计算高效的MIL框架。

Method: 提出HookMIL框架：1) 使用可学习的hook tokens进行结构化上下文聚合；2) 支持三种初始化方式：关键patch视觉特征、视觉语言病理模型的文本嵌入、空间转录组-视觉模型的空间特征；3) 通过双向注意力机制实现线性复杂度交互；4) 引入Hook多样性损失促进token专业化；5) 添加hook-to-hook通信机制优化上下文交互。

Result: 在四个公共病理数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。代码已开源。

Conclusion: HookMIL通过创新的hook tokens设计，成功解决了传统MIL方法上下文信息丢失和transformer方法计算复杂度高的问题，为计算病理学中的弱监督分析提供了高效且有效的解决方案。
...
TL;DR: Tiny-YOLOSAM：结合YOLO检测器和TinySAM的混合系统，通过YOLO生成边界框提示和稀疏点提示，显著提升分割覆盖率和速度，替代密集"分割一切"模式。
...
Method: 提出Tiny-YOLOSAM混合管道：1)使用YOLOv12检测器生成前景对象的边界框提示；2)在YOLO未覆盖区域补充稀疏点提示；3)结合TinySAM进行分割。

Result: 在COCO val2017上，类别无关覆盖率大幅提升（AR从16.4%到77.1%，mIoU从19.2%到67.8%），端到端运行时间从49.20秒/图像减少到10.39秒/图像（4.7倍加速）。

Conclusion: 检测器引导提示结合目标稀疏采样是实际全场景分割中替代密集"分割一切"提示的有效方案，显著提升效率和覆盖率。
...
TL;DR: 提出了一种基于视觉语言模型的多模态可解释性方法，通过分析视网膜象限中的病变分布来模拟眼科医生的诊断推理过程，为糖尿病视网膜病变提供更全面的解释。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。当前AI模型依赖病变分割进行可解释性，但手动标注对临床医生不实用。医生需要能解释分类推理的模型，而不仅仅是突出病变位置。现有模型通常依赖单一成像模态，效果有限。

Method: 提出了一种新颖的多模态可解释性模型，使用视觉语言模型配合少样本学习，通过分析视网膜象限中的病变分布来模拟眼科医生的推理过程。模型生成配对的Grad-CAM热力图，展示OCT和眼底图像中各个神经元的权重，可视化显示对DR严重程度分类有贡献的区域。

Result: 使用包含3,000张眼底图像和1,000张OCT图像的数据集，该方法解决了当前DR诊断中的关键限制，为改善患者预后提供了实用且全面的工具。

Conclusion: 该创新方法通过多模态可解释性模型，克服了当前DR诊断模型的局限性，能够更好地模拟眼科医生的诊断推理，在筛查、治疗和研究环境中具有广泛的应用潜力。
...
TL;DR: VLM-PAR是一个基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨注意力融合改进视觉特征，在行人属性识别任务中取得了最先进的性能，特别是在高度不平衡的PA100K基准上。
...
Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的跨注意力融合来对齐图像和提示嵌入，从而改进视觉特征。

Result: 在高度不平衡的PA100K基准上实现了显著的准确率提升，创下了新的最先进性能，同时在PETA和Market-1501基准上也取得了显著的均值准确率提升。
...
TL;DR: ViSignVQA：首个大规模越南语招牌视觉问答数据集，包含10,762张图像和25,573个问答对，通过集成OCR和语言模型提升越南语招牌文本理解能力。


<details>
  <summary>Details</summary>
Motivation: 自然场景中的招牌文本理解对VQA的实际应用至关重要，但在低资源语言（如越南语）中尚未得到充分探索。现有VQA数据集缺乏针对越南语招牌的领域特定资源，无法捕捉其独特的语言、文化和视觉特征。

Method: 1. 创建ViSignVQA数据集：包含10,762张越南语招牌图像和25,573个问答对，涵盖双语文本、非正式表达、颜色布局等视觉元素。
2. 基准测试：适配BLIP-2、LaTr、PreSTU、SaL等SOTA VQA模型，集成越南语OCR模型（SwinTextSpotter）和越南语预训练语言模型（ViT5）。
3. 提出多智能体VQA框架：结合感知和推理智能体与GPT-4，采用多数投票机制。

Result: 1. OCR增强上下文显著提升性能：将OCR文本附加到问题中时，F1分数最高提升209%。
2. 多智能体框架表现优异：通过多数投票达到75.98%的准确率。
3. 数据集有效捕捉了越南语招牌的真实世界场景文本特征。

Conclusion: ViSignVQA是首个用于越南语招牌理解的大规模多模态数据集，强调了领域特定资源在增强低资源语言文本VQA中的重要性。该数据集可作为基准，支持OCR集成VQA模型在越南语中的开发和评估。
...
TL;DR: 该论文提出了一种基于语义抽象的3D隐藏物体定位方法，利用2D视觉语言模型的相关性激活作为"抽象物体"表示，通过历史数据学习物体放置模式，显著提高了寻找隐藏物体的效率。


<details>
  <summary>Details</summary>
Motivation: 为家庭机器人提供寻找隐藏物体的技能，这些物体由于部分遮挡而无法被视觉语言模型直接识别，传统随机搜索方法效率低下，需要更智能的定位方法。

Method: 提出语义抽象框架，将2D视觉语言模型的相关性激活视为"抽象物体"表示，用于学习3D定位和补全。利用历史数据中物体放置频率信息，进行非结构化搜索来定位隐藏物体。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试成功率显著高于随机搜索，搜索速度明显更快。

Conclusion: 语义抽象扩展为家庭机器人提供了寻找丢失物体的高效技能，能够节省时间和精力，有望提升机器人在现实环境中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的视频理解方法在处理长视频时面临挑战：帧间冗余严重且需要时序连贯表示。现有的静态策略（如稀疏采样、帧压缩和聚类）主要针对离线场景优化，应用于连续视频流时往往产生碎片化或过度压缩的输出。

Method: VideoScaffold包含两个核心组件：1) 弹性尺度事件分割(EES)，通过预测引导的分割动态细化事件边界；2) 分层事件整合(HEC)，逐步将语义相关的片段聚合成多层次抽象表示。这两个组件协同工作，使框架能够根据视频时长自适应调整事件粒度。

Result: 在离线和流式视频理解基准测试上的广泛实验表明，VideoScaffold实现了最先进的性能。该框架是模块化且即插即用的，能够无缝扩展现有的基于图像的多模态大语言模型到连续视频理解任务。

Conclusion: VideoScaffold为流式视频理解提供了一个有效的动态表示框架，通过自适应的事件粒度调整和分层抽象机制，解决了长视频理解中的冗余和时序连贯性问题，显著提升了多模态大语言模型在视频理解任务上的性能。
...
TL;DR: 论文提出KAN增强的FPN-Stem架构，通过用KAN卷积层替换FPN末端的线性3x3平滑卷积，显著提升ViT在姿态估计任务中的性能，在COCO数据集上获得+2.0 AP的提升。


<details>
  <summary>Details</summary>
Motivation: 现有ViT模型（如ViTPose）的前端设计过于简单，朴素的patch划分机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失，限制了模型性能。

Method: 保留FPN经典的"上采样-相加"融合流程，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层。该KAN层利用其强大的非线性建模能力，自适应学习和校正多尺度融合过程中产生的"伪影"。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了高达+2.0 AP的显著性能提升。

Conclusion: 该工作不仅提供了一个即插即用的高性能模块，更重要的是揭示了ViT前端的性能瓶颈通常不在于"特征精炼"（注意力机制），而在于"特征融合"的质量，并提供了通过引入KAN算子解决这一瓶颈的有效路径。
...
TL;DR: 该研究提出了一种元信息引导的跨域协同扩散模型(MiG-DM)，用于提升低剂量PET成像质量，通过整合患者特异性元信息和投影域物理知识来解决噪声干扰和细节保留问题。


<details>
  <summary>Details</summary>
Motivation: 低剂量PET成像在减少患者辐射暴露方面至关重要，但面临噪声干扰、对比度降低和生理细节难以保留等挑战。现有方法往往忽略了投影域物理知识和患者特异性元信息，而这些对于功能语义关联挖掘至关重要。
...
Conclusion: MiG-DM通过整合患者特异性元信息和跨域物理知识，有效解决了低剂量PET成像的质量问题，为临床实践提供了更高质量的成像解决方案。
...
TL;DR: 提出自适应RGB-LWIR融合框架，通过多比例融合和动态模型选择，在不同光照条件下优化目标检测性能


<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下表现不佳，热成像系统缺乏颜色和纹理信息，需要融合两种模态以提升应急服务中自主机器人平台的视觉性能

Method: 训练33个YOLO模型，使用超过22,000张标注图像，涵盖三种光照等级；通过11种融合比例（从100/0到0/100，10%增量）对齐融合RGB和LWIR帧；动态选择最佳检测模型

Result: 最佳全光照模型（80/20 RGB-LWIR）和昏暗光照模型（90/10融合）分别达到92.8%和92.0%平均置信度，显著优于YOLOv5n和YOLOv11n基线；无光条件下最佳40/60融合达到71.0%，超过基线但统计不显著
...
TL;DR: 通过使用在THINGS三元组数据集上训练的人类感知嵌入（HPE）教师模型作为外部指导信号，将几何理解引入文本到图像扩散模型，实现几何与风格的分离控制，提高语义对齐约80%。
...
Method: 提出人类感知嵌入（HPE）教师模型，在THINGS三元组数据集上训练以捕捉人类对物体形状的敏感性。将该教师模型的梯度注入潜在扩散过程，实现几何与风格的可控分离。在三种架构上评估：Stable Diffusion v1.5（U-Net骨干）、流匹配模型SiT-XL/2和扩散变换器PixArt-Σ。

Result: 实验表明流模型在没有持续指导时倾向于漂移回默认轨迹。实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。与无指导基线相比，指导生成将语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地指导大型生成系统，实现更强的几何控制，拓宽文本到图像合成的创意范围，为解决几何约束与风格冲突问题提供了有效途径。
...
Result: GeCo能够有效揭示视频生成模型的常见失败模式，作为无需训练指导损失使用时，可以显著减少生成视频中的变形伪影。

Conclusion: GeCo提供了一个系统性的几何一致性评估框架，不仅能够量化视频生成模型的质量问题，还能作为指导损失直接改进生成结果，为视频生成研究提供了有价值的工具。
...
TL;DR: 该研究开发了B&J基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现良好（>90%），但在需要多模态整合的开放任务中表现显著下降（仅60%），表明当前AI模型尚未具备临床多模态推理能力。
...
Method: 开发了Bones and Joints (B&J)基准测试框架，包含1,245个来自真实骨科和运动医学患者病例的问题，评估模型在7个临床推理任务中的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，并与专家标注的基准进行比较。

Result: 结果显示任务类型间存在显著性能差距：最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放任务中准确率仅约60%。VLM在医学图像解释方面存在严重限制，经常出现严重的文本驱动幻觉，忽略矛盾的视觉证据。专门针对医学应用微调的模型相比通用模型没有一致优势。
...
TL;DR: 本文提出了一种用于低成本边缘设备（树莓派5和Google Coral Edge TPU）的单摄像头驾驶员行为监控系统，通过紧凑的视觉模型、混淆感知标签设计和时序决策头，实现对17种驾驶分心和疲劳行为的实时识别。


<details>
  <summary>Details</summary>
Motivation: 车内驾驶员监控系统需要在计算、功耗和成本严格受限的条件下，以低延迟识别分心和疲劳相关行为。现有系统往往需要昂贵的硬件或无法满足实时性要求，因此需要开发适用于低成本边缘平台的解决方案。

Method: 系统采用单摄像头方案，包含三个核心组件：(1)紧凑的逐帧视觉模型；(2)混淆感知标签设计以减少视觉相似误报；(3)时序决策头，仅在预测既自信又持续时才触发警报。系统覆盖17种行为类别，包括手机使用、饮食、吸烟、伸手、视线转移、乘客互动等。

Result: 在树莓派5上使用INT8推理实现约16 FPS（每帧延迟低于60毫秒），在Coral Edge TPU上实现约25 FPS，能够在廉价硬件上实现实时监控和稳定警报生成。系统在包含多样化驾驶员、车辆和光照条件的授权数据集上进行训练和评估，并在真实车辆测试中验证了运行时行为。

Conclusion: 该系统证明了在低成本边缘设备上实现实时驾驶员行为监控的可行性，为以人为中心的车辆智能提供了可靠的上游感知输入，包括新兴的自主车辆概念。可靠的驾驶员状态感知是实现更智能、更安全的车辆交互系统的关键基础。
...
TL;DR: SpotEdit是一个无需训练的图像编辑框架，通过选择性更新修改区域来减少计算冗余，提高编辑效率


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时对所有token进行统一处理，即使只有小区域需要修改，这导致计算冗余并可能影响未修改区域的质量

Method: SpotEdit包含两个核心组件：SpotSelector通过感知相似性识别稳定区域并重用条件图像特征；SpotFusion通过动态融合机制自适应地混合这些特征与编辑token

Result: 通过减少不必要的计算并保持未修改区域的高保真度，SpotEdit实现了高效精确的图像编辑

Conclusion: SpotEdit证明了在图像编辑中无需重新生成每个区域，通过选择性更新修改区域可以实现更高效的编辑过程
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注正向建模（如从文本到运动的整体映射），但缺乏将整体运动分解为有意义子组件的能力。本文从逆向视角出发，研究如何将整体运动分解为语义上有意义的子组件。

Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布。包含三种训练变体：1）DeMoGen-Exp使用分解的文本提示进行显式训练；2）DeMoGen-OSS进行正交自监督分解；3）DeMoGen-SC强制原始文本嵌入和分解文本嵌入之间的语义一致性。

Result: 该方法能够从复杂运动序列中解耦可重用的运动原语，分解后的运动概念可以灵活重组生成多样且新颖的运动，能够泛化到训练分布之外。同时构建了文本分解数据集支持组合训练。

Conclusion: DeMoGen提供了一个有效的运动分解学习框架，能够发现和重用运动原语，为文本到运动生成和运动组合提供了扩展资源，推动了运动理解与生成的可组合性发展。
...
TL;DR: 本文提出了一种基于变分自编码器的多视角潜在表示学习框架，用于从T1Gd和FLAIR MRI中提取互补的影像组学特征，以非侵入性预测胶质母细胞瘤中MGMT启动子甲基化状态。


<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤中MGMT启动子甲基化状态具有重要的预后和治疗意义，传统影像组学方法存在特征冗余高、模态特异性信息建模不完整的问题，需要更有效的多模态整合方法。

Method: 提出基于变分自编码器的多视角潜在表示学习框架，通过独立的概率编码器分别编码T1Gd和FLAIR MRI模态，在紧凑的潜在空间中进行融合，保留模态特异性结构的同时实现有效的多模态整合。

Result: 该方法能够生成有效的潜在嵌入表示，用于MGMT启动子甲基化分类，解决了传统单模态和早期融合方法在特征冗余和模态特异性信息建模方面的局限性。

Conclusion: 提出的多视角潜在表示学习框架为胶质母细胞瘤的影像基因组学研究提供了更有效的多模态整合方法，有助于非侵入性推断分子肿瘤特征。
...
Method: 使用视觉Transformer系统构建端到端管道，包括H&E WSI的肿瘤定位、新颖的映射函数关联IHC区域、临床启发的4级HER2评分机制
...
TL;DR: 该研究提出了一种基于多模态大语言模型的3D场景操作框架，通过引入MCP-based API、专用视觉工具套件和多智能体协作系统，解决了3D物体排列任务中的视觉定位、场景理解和迭代更新三大挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得了显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在3D物体排列任务中存在三大挑战：1) 视觉定位能力弱，难以将程序化编辑与精确的3D结果关联；2) 缺乏对3D场景的深入理解；3) 迭代更新过程容易出错且难以管理。

Method: 提出三部分解决方案：1) 引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2) 使用专用视觉工具套件增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3) 提出协作多智能体框架，包含规划、执行和验证三个角色，以处理多步骤指令并从中间错误中恢复。

Result: 在25个复杂的物体排列任务上验证了方法的有效性，结果显示该方法显著优于现有基线方法。

Conclusion: 该研究成功地将多模态大语言模型应用于复杂3D场景操作，通过创新的API设计、视觉增强工具和多智能体协作框架，有效解决了3D物体排列任务中的关键挑战，为MLLMs在3D领域的应用开辟了新方向。
...
TL;DR: Self-E是一个从零开始训练、支持任意步数推理的文本到图像生成模型，结合了流匹配学习和自评估机制，无需预训练教师模型，在少步数生成和多步数采样中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统扩散或流模型依赖局部监督需要多步推理，而蒸馏方法需要预训练教师模型。Self-E旨在填补这两种范式之间的空白，实现从零开始训练、支持任意步数推理的统一框架。

Method: Self-E采用从零开始的训练方法，结合流匹配学习和新颖的自评估机制。模型使用当前分数估计评估自己生成的样本，作为动态的自教师，实现瞬时局部学习和自驱动的全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E在少步数生成方面表现出色，在50步时与最先进的流匹配模型竞争。性能随推理步数增加单调提升，支持超快速少步生成和高质量长轨迹采样。

Conclusion: Self-E是第一个从零开始、支持任意步数的文本到图像模型，为高效可扩展的生成提供了统一框架，填补了传统扩散模型与蒸馏方法之间的空白。
...
TL;DR: iOSPointMapper是一个移动应用，利用iPhone/iPad的语义分割、LiDAR深度估计和GPS/IMU融合数据，实时检测和定位人行道相关特征，数据匿名传输到TDEI数据库，为行人基础设施提供可扩展的数据收集方案。
...
Method: 开发iOSPointMapper移动应用，利用设备端语义分割检测特征，LiDAR进行深度估计，融合GPS/IMU数据进行定位，包含用户引导的标注界面验证系统输出，数据匿名化后传输到TDEI数据库。

Result: 系统特征检测和空间映射性能的详细评估显示，该应用在行人地图绘制方面具有增强潜力，能够有效填补关键数据空白。

Conclusion: iOSPointMapper提供了一个可扩展、以用户为中心的方法，能够有效解决行人基础设施中的关键数据缺口问题。
...
<details>
  <summary>Details</summary>
Motivation: 针对扩散式检测器（如DiffusionDet）在临床应用中存在的延迟瓶颈问题，特别是在克罗恩病磁共振肠造影检测等时间敏感场景中，需要解决多步采样（T>60步）导致的推理速度慢的问题。

Method: 提出DeFloMat框架，采用条件流匹配技术，基于条件最优传输理论构建确定性流场（近似Rectified Flow），替代扩散模型的随机去噪路径，通过简单的常微分方程求解器实现快速推理。

Result: 在MRE临床数据集上，DeFloMat仅需3步推理就达到43.32% AP_{10:50}的SOTA性能，比DiffusionDet在4步时的最佳性能（31.03% AP_{10:50）提升1.4倍，同时显著改善了定位特征，在少步推理时具有更好的召回率和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能够快速、无创地可视化细胞形态，但可靠的体积分割通常依赖于荧光标记或复杂的后处理。现有方法存在这一技术空白。

Method: Bright-4B采用硬件对齐的Native Sparse Attention机制（捕获局部、粗粒度和选择的全局上下文）、深度-宽度残差HyperConnections稳定表示流、软混合专家模型实现自适应容量。各向异性补丁嵌入尊重共聚焦点扩散和轴向变薄，实现几何保真的3D标记化。

Result: 模型仅从明场堆栈就能产生形态准确的细胞核、线粒体和其他细胞器分割，无需荧光、辅助通道或手工后处理。在多个共聚焦数据集上，Bright-4B在深度和细胞类型上保持精细结构细节，优于当代CNN和Transformer基线。

Conclusion: Bright-4B填补了无标记3D明场显微镜分割的技术空白，所有代码、预训练权重和下游微调模型将公开发布，以推进大规模、无标记3D细胞图谱构建。
...
TL;DR: 提出EmoCtrl模型，实现可控情感图像内容生成，在保持内容描述一致性的同时表达目标情感，解决了现有模型在内容和情感控制上的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型能确保内容一致性但缺乏情感感知，而情感驱动模型生成情感结果时会导致内容失真。需要一种方法能同时保持内容忠实度和情感表达。
...
TL;DR: 提出了一种基于线特征的事件相机标定框架，利用人造环境中常见物体的几何线条（如门、窗、盒子等）进行标定，无需特殊标定板，适用于快速变化场景。


<details>
  <summary>Details</summary>
Motivation: 当前事件相机标定方法通常需要闪烁图案、重建强度图像或从事件中提取特征，这些方法耗时且需要手动放置标定物，无法满足快速变化场景的需求。

Method: 直接从事件流中检测线条，利用事件-线条标定模型生成相机参数的初始估计，适用于平面和非平面线条，然后采用非线性优化细化相机参数。

Result: 仿真和真实世界实验验证了该方法的可行性和准确性，在单目和双目事件相机上均进行了验证，代码已开源。

Conclusion: 提出了一种无需特殊标定板的线基事件相机标定方法，能够利用环境中自然存在的几何线条进行快速准确的标定，适用于动态场景。
...
<details>
  <summary>Details</summary>
Motivation: 视障人士定位物体存在重大挑战，这阻碍了他们的独立性，可能将他们推向危险境地。为了增强视障人士的自给自足能力，需要开发辅助工具

Method: 使用Flutter开发平台构建应用，后端采用Efficientdet-D2模型进行物体检测。通过耳机提供正弦声音提示：物体在右侧时右耳发声，左侧时左耳发声，正前方时双耳同时发声

Result: 开发了完整的智能手机应用SonoVision，能够帮助视障人士通过声音提示定位日常物品，减少对周围人的依赖，增强独立性

Conclusion: SonoVision应用能够以安全、用户友好的方式显著帮助视障人士，其完全离线工作的能力使其更具实用性，有望提高视障人士的自主性和生活质量
...
<details>
  <summary>Details</summary>
Motivation: 现有单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用基础模型SAM 3D在遥感建筑重建中的潜力。

Method: 在NYC Urban Dataset上对SAM 3D与TRELLIS进行基准测试，使用FID和CMMD作为评估指标，并通过分段-重建-组合流程扩展SAM 3D到城市场景重建。

Result: 实验结果表明，SAM 3D相比TRELLIS能生成更连贯的屋顶几何和更清晰的边界，并通过扩展流程展示了城市场景建模的潜力。

Conclusion: 研究为在城市3D重建中部署基础模型提供了实用指导，并激励未来集成场景级结构先验知识，以进一步提升重建质量。
...
TL;DR: 该研究训练并比较了三种计算机视觉模型（YOLOv8、YOLOv11、RF-DETR）在手动标注的美国变电站图像数据集上的性能，用于自动识别变电站关键组件，以评估电网基础设施的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 变电站是电网的重要组成部分，其资产（如变压器）容易受到飓风、洪水、地震和地磁感应电流等多种灾害的破坏。电网作为关键国家基础设施，任何故障都可能带来重大的经济和公共安全影响。传统的手动变电站基础设施映射方法耗时且劳动密集，因此需要自动化的计算机视觉解决方案来提高效率和便利性。

Method: 研究使用手动标注的美国变电站图像数据集，训练并比较了三种计算机视觉模型：YOLOv8、YOLOv11和RF-DETR。每个模型都根据检测准确率、精确度和效率进行评估，以确定哪种模型能提供可靠且大规模的变电站组件映射。

Result: 研究展示了三种模型在变电站组件检测方面的性能比较结果，分析了每个模型的关键优势和局限性。研究还利用这些模型有效地映射了美国各地的各种变电站组件，展示了机器学习在变电站映射中的应用案例。

Conclusion: 该研究证明了计算机视觉模型在变电站组件自动识别和映射中的可行性，为电网基础设施脆弱性评估提供了高效、自动化的解决方案。通过比较不同模型的性能，研究确定了最适合大规模变电站组件映射的模型，为电力系统基础设施管理提供了实用的机器学习应用。
...
TL;DR: PGR²M提出了一种混合表示方法，通过残差向量量化增强可解释的姿态编码，提升文本到3D动作生成和编辑的质量，同时保持结构可编辑性。
...
Method: 提出PGR²M混合表示：1）姿态引导的RVQ分词器将动作分解为编码全局结构的姿态潜在变量和编码细粒度时间变化的残差潜在变量；2）残差丢弃防止过度依赖残差；3）基础Transformer自回归预测姿态编码，精炼Transformer预测残差编码。

Result: 在HumanML3D和KIT-ML数据集上，PGR²M在Fréchet inception距离和重建指标上优于CoMo及最近的扩散和分词基线，用户研究证实其支持直观、结构保持的动作编辑。

Conclusion: PGR²M通过混合表示有效平衡了动作生成的质量与可编辑性，为文本驱动的3D动作合成和编辑提供了更优的解决方案。
...
Conclusion: 该方法利用事件相机的高速和高动态范围能力，实现了冲击波运动场的高精度测量，代表了该领域的显著进展。
...
TL;DR: 提出了一种用于红外小目标检测的两阶段半监督范式，通过分层MoE适配器和知识蒸馏，用少量标注数据实现接近全监督的性能
...
Result: 实验表明，使用最少标注时，下游模型能达到甚至超越全监督模型的性能，首次系统性地利用SAM作为教师模型解决IR-SOT数据稀缺问题

Conclusion: 该半监督范式有效解决了红外小目标检测的数据稀缺问题，通过知识蒸馏和迁移实现了高性能的轻量模型部署
...
<details>
  <summary>Details</summary>
Motivation: 2D视频通常是动态3D场景的一致投影，通过将视频表示为高斯分布集合来强制这种合理的归纳偏置，从而学习更好的视频表示

Method: 提出Video Gaussian Masked Autoencoders，将图像序列编码为随时间移动的高斯分布集合，使用掩码自编码器进行自监督预训练

Result: 该方法在预训练中自然涌现出跟踪能力，零样本跟踪性能达到最先进水平。经过小规模微调后，在Kinetics数据集上提升34.6%，在Kubric数据集上提升13.1%，超越现有自监督视频方法

Conclusion: Video-GMAE通过将视频表示为移动高斯分布，有效学习视频表示并自然获得跟踪能力，为自监督视频理解提供了新思路
...
TL;DR: SR-MCR是一个轻量级、无需标注的多模态大语言模型对齐框架，通过利用模型输出中的内在过程信号来提升推理的可靠性和一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型经常产生流畅但不可靠的推理，表现出弱的步骤间一致性和不足的视觉基础，主要因为现有的对齐方法只监督最终答案而忽略了中间推理过程的可靠性。

Method: 引入SR-MCR框架，利用从模型输出直接衍生的五个自参考线索（语义对齐、词汇保真度、非冗余性、视觉基础、步骤一致性）构建归一化的可靠性加权奖励，提供细粒度的过程级指导。采用无评论家的GRPO目标，配合置信度感知冷却机制来稳定训练并抑制平凡或过度自信的生成。

Result: 基于Qwen2.5-VL构建的SR-MCR在广泛的视觉基准测试中提高了答案准确性和推理一致性；在同等规模的开源模型中，SR-MCR-7B以81.4%的平均准确率实现了最先进的性能。消融研究证实了每个奖励项和冷却模块的独立贡献。

Conclusion: SR-MCR通过利用模型自身输出中的内在信号，提供了一种轻量级且无需标注的方法来改善多模态大语言模型的推理可靠性和过程一致性，显著提升了模型的性能表现。
...
TL;DR: 提出了一种基于稀疏差分Transformer的Top-K Jaccard相似度系数方法，用于改进人脸聚类中的嵌入关系测量，通过增强邻居节点纯度和抗噪声能力，在多个数据集上实现了SOTA性能。
...
Method: 1. 提出预测驱动的Top-K Jaccard相似度系数，增强邻居节点纯度；2. 开发基于Transformer的预测模型，分析中心节点与Top-K附近邻居的关系；3. 提出稀疏差分Transformer（SDT）替代原始Transformer，消除噪声并增强抗噪声能力。

Result: 在MS-Celeb-1M等多个数据集上的大量实验表明，该方法实现了最先进的性能，优于现有方法，为人脸聚类提供了更鲁棒的解决方案。

Conclusion: 通过预测驱动的Top-K Jaccard相似度系数和稀疏差分Transformer，有效解决了人脸聚类中相似度测量的精度和噪声问题，显著提升了聚类性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型主要是前向预测，基于初始观察生成轨迹而缺乏明确的目标建模，导致空间漂移和目标不一致。需要开发能够生成物理合理且目标对齐的视觉规划方法。

Method: 采用两阶段方法：1) 目标图像模型识别任务相关区域，执行区域感知交叉注意力，合成连贯的目标图像；2) 环境-目标视频模型基于首尾帧条件视频扩散模型(FL2V)，在初始观察和目标图像之间插值，生成平滑物理合理的视频轨迹。

Result: 在物体操作和图像编辑基准测试中，Envision相比基线方法实现了更好的目标对齐、空间一致性和物体保持。生成的视觉规划可直接支持下游机器人规划和控制。

Conclusion: Envision通过显式目标约束的扩散模型框架，有效解决了视觉规划中的空间漂移和目标对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估(IQA)模型通常只输出全局分数，对局部和细粒度失真极不敏感，导致超分辨率模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量不一致。

Method: 1. 提出基于编码器-解码器架构的细粒度感知奖励模型(FinPercep-RM)，不仅提供全局质量分数，还生成感知退化图来空间定位和量化局部缺陷。2. 构建FGR-30k数据集训练该模型，包含真实世界超分辨率模型产生的多样且细微的失真。3. 提出协同进化课程学习(CCL)机制，奖励模型和超分辨率模型同步进行课程学习，奖励模型复杂度逐步增加，超分辨率模型从简单全局奖励开始，逐步过渡到复杂模型输出。

Result: 实验验证了该方法在RLHF方法中，对超分辨率模型的全局质量和局部真实感方面都有效，能够稳定训练并抑制奖励黑客现象。
...
<details>
  <summary>Details</summary>
Motivation: 为深度估计提供扩散模型之外的替代方案，探索自回归先验作为几何感知生成模型的潜力，强调其在数据可扩展性和3D视觉任务适应性方面的优势。

Method: 采用大规模文本到图像自回归模型，引入尺度条件上采样机制和分类器自由引导，通过10个固定自回归阶段进行推理，仅需74K合成样本进行微调。

Result: 在受限训练条件下，在室内基准测试中达到最先进性能；在室外数据集上表现强劲；自回归先验被确立为深度估计的补充性几何感知生成模型家族。
...
TL;DR: CLAdapter是一种新颖的集群注意力适配器，通过注意力机制和聚类中心将大规模预训练模型的知识适配到数据有限的下游任务中，在多个科学领域实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉领域受益于大规模数据集和预训练模型，但许多专业领域和数据有限的下游任务仍然面临重大挑战，需要将丰富的预训练知识有效适配到特定任务中。

Method: 提出Cluster Attention Adapter (CLAdapter)，引入注意力机制和聚类中心，通过分布相关性和变换矩阵来个性化增强转换特征，使模型能够学习针对不同特征集的特定表示。

Result: 在10个数据集上的广泛实验表明，CLAdapter在通用、多媒体、生物、医疗、工业、农业、环境、地理、材料科学、OOD和3D分析等多个数据有限的科学领域实现了最先进的性能。
...
Result: 在486个专家标注的结膜活检切片数据集上评估，相比CNN和基础模型基线，INTERACT-CMIL在WHO4上获得55.1%的相对宏观F1提升，在垂直扩散上获得25.0%提升。

Conclusion: 该框架提供与专家分级一致的可解释多标准预测，为CMIL诊断建立了可重复的计算基准，向标准化数字眼病理学迈进一步。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型虽然视觉保真度高，但在复杂提示词的语义对齐方面存在困难，需要提升生成内容与提示意图的一致性。

Method: 1. CritiCore模块：利用视觉语言模型和多个大语言模型丰富提示上下文，产生高级语义反馈，指导扩散过程更好地对齐提示意图；2. SpecFusion：在谱域合并中间生成状态，注入粗粒度结构信息同时保留高频细节；3. 作为插件式细化阶段，兼容现有扩散模型骨干，无需额外训练。

Result: 在标准基准测试中，该方法显著提升了文本到图像对应关系的人类对齐指标和视觉质量，在人类偏好分数和美学评估方面持续提升性能，与最先进的奖励优化方法效果相当。定性结果展示了更优的细节、真实性和提示保真度。

Conclusion: CritiFusion通过语义批判和谱域对齐策略，有效提升了文本到图像生成的一致性和质量，可作为现有扩散模型的即插即用改进方案。
...
TL;DR: 提出ARFM（自回归流匹配）方法用于序列连续数据的概率建模，在多样化视频数据集上训练，能够预测长时间的未来点轨迹位置，并在人机运动预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前运动预测模型通常针对特定分布训练，应用受限；而大规模视频预测模型虽然视觉真实感强，但难以准确建模复杂运动。需要一种既能处理多样化数据又能准确预测复杂运动的方法。

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。在多样化视频数据集上训练模型，使其能够生成长时间的未来点轨迹位置。

Result: ARFM模型能够准确预测复杂运动，并建立了评估运动预测模型性能的基准。实验表明，基于预测的未来轨迹来调节机器人动作预测和人体运动预测，能显著提升下游任务性能。

Conclusion: ARFM方法为序列连续数据的概率建模提供了有效解决方案，在多样化运动预测任务中表现出色，代码和模型已开源，为相关研究提供了重要工具和基准。
...
TL;DR: 提出了SCPainter统一框架，结合3D高斯溅射资产表示和扩散模型生成，实现自动驾驶场景中3D资产插入和新视角合成的联合处理。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要多样化的训练数据，特别是长尾驾驶场景。现有方法分别处理3D资产插入和新视角合成，缺乏统一框架来创建交互式、真实的驾驶场景。

Method: SCPainter框架集成3D高斯溅射汽车资产表示和3D场景点云，通过扩散模型生成高质量图像。将3D资产和场景点云投影到新视角，作为扩散模型的条件输入。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现高质量的3D资产插入和新视角合成，为创建多样化和真实的驾驶数据提供支持。

Conclusion: SCPainter统一框架成功解决了自动驾驶仿真中3D资产插入和新视角合成的联合处理问题，有助于生成更丰富、更真实的训练数据，提升自动驾驶模型的鲁棒性。
...
TL;DR: 本研究评估了超声特异性自监督预训练（USF-MAE）在早孕期超声图像中自动检测囊性水瘤的性能，相比传统监督学习方法DenseNet-169，USF-MAE在所有评估指标上均有显著提升。
...
Method: 研究使用基于掩码自编码（MAE）的超声自监督基础模型（USF-MAE），该模型在超过37万张未标注超声图像上预训练，然后针对囊性水瘤与正常对照的二分类任务进行微调。评估采用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议，使用准确率、灵敏度、特异度和ROC-AUC等指标。模型可解释性通过Score-CAM可视化分析。

Result: USF-MAE在所有评估指标上均优于DenseNet-169基线。USF-MAE平均准确率0.96、灵敏度0.94、特异度0.98、ROC-AUC 0.98；DenseNet-169基线分别为0.93、0.92、0.94、0.94。Score-CAM可视化显示模型关注胎儿颈部预期区域，具有临床相关性。Wilcoxon符号秩检验证实USF-MAE性能提升具有统计学显著性（p=0.0057）。

Conclusion: 超声特异性自监督预训练能够促进准确、稳健的深度学习方法检测早孕期超声图像中的囊性水瘤，为解决医学影像中小标注数据集限制提供了有效方案。
...
TL;DR: 提出Freetime FeatureGS方法，通过可学习特征的高斯基元和线性运动能力，从单帧分割图实现4D场景分解重建，无需视频分割
...
Method: 使用Freetime FeatureGS表示动态场景，包含可学习特征和线性运动能力的高斯基元。应用对比损失使基元特征根据2D分割图中的实例关系接近或远离。采用时间有序的流式特征学习策略，避免优化过程中的局部极小值

Result: 在多个数据集上的实验结果表明，该方法的重建质量大幅优于现有方法

Conclusion: 提出的Freetime FeatureGS方法通过流式特征学习策略，成功实现了不依赖视频分割的高质量4D场景分解重建
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时通常将图像编码为令牌序列，但视觉令牌数量的增长显著增加了推理成本。现有的视觉令牌剪枝方法往往忽视了长上下文多图像场景的挑战，需要专门针对这种场景的剪枝方法。

Method: 方法分为两个阶段：1）图像内阶段：为每张图像分配内容感知的令牌预算，贪婪选择最具代表性的令牌；2）图像间阶段：进行全局多样性过滤形成候选池，然后应用帕累托选择程序平衡多样性与文本对齐。

Result: 大量实验表明，该方法在长上下文设置下保持了强大的性能，同时显著减少了视觉令牌的数量。

Conclusion: 本文提出的自适应剪枝方法有效解决了长上下文多图像场景中的视觉令牌冗余问题，通过分解冗余为图像内和图像间两个维度，实现了动态预算分配，在降低计算成本的同时保持了模型性能。
...
Method: 提出基于希尔伯特曲线重排序的邻居感知令牌缩减方法，使用1D序列表示显式保留2D空间中的邻居结构。包含两个关键策略：用于选择性令牌保留的邻居感知剪枝（NAP）和用于局部令牌聚合的相邻令牌相似性合并（MAT）。
...
TL;DR: 该论文提出了一种基于心理测量学中分级反应模型的AGIQA质量评估方法，通过算术方式建模难度级别，解决现有方法中文本嵌入与图像嵌入之间的语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多级质量描述文本嵌入之间的相似度来评估图像质量，但发现这种相似度分布通常呈现多模态模式，即图像嵌入可能与"优秀"和"差"等级描述都高度相似，而与"良好"等级偏离。这种文本嵌入与其预期描述之间的语义不一致性被称为"语义漂移"，它削弱了文本-图像共享空间学习的可靠性。

Method: 受心理测量学启发，提出改进的分级反应模型(GRM)用于AGIQA。GRM是一种经典评估模型，通过不同难度级别的测试项目对受试者能力进行分级。基于这一理念，设计了一个双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度级别。为确保难度级别的单调性，采用算术方式建模难度生成，这本质上强制了单峰且可解释的质量分布。该算术GRM质量分级(AGQG)模块具有即插即用的优势。

Result: 当集成到各种最先进的AGIQA框架中时，AGQG模块能持续提升性能。此外，它还能有效地泛化到自然图像和屏幕内容图像质量评估，显示出其作为未来IQA模型关键组件的潜力。

Conclusion: 提出的算术GRM质量分级模块通过解决语义漂移问题，提供了一种更可靠、可解释的图像质量评估方法，具有广泛的适用性和即插即用的优势，有望成为未来图像质量评估模型的关键组件。
...
TL;DR: EPD-Solver是一种新型ODE求解器，通过并行梯度评估减少截断误差，实现扩散模型的高质量快速采样，并可通过强化学习微调提升文本到图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但顺序去噪特性导致采样延迟高。现有求解器加速方法在低延迟预算下常出现图像质量显著下降，主要原因是无法捕捉高曲率轨迹段导致的累积截断误差。

Method: 提出EPD-Solver（集成并行方向求解器），利用采样轨迹主要位于低维流形的几何洞察，通过均值定理近似积分解。采用两阶段优化框架：先通过蒸馏方法优化可学习参数，再提出参数高效的强化学习微调方案，将求解器重构为随机Dirichlet策略，仅在低维求解器空间操作。

Result: EPD-Solver通过并行梯度计算减少截断误差，保持低延迟采样特性。可作为插件（EPD-Plugin）改进现有ODE采样器，在复杂文本到图像生成任务中有效缓解奖励黑客问题并提升性能。
...
Method: 基于3D高斯表示和tri-plane表示的混合方法，结合高斯表示的高效渲染和几何表示优势，增强稀疏切片下的结构连续性和语义一致性

Result: 在US和MRI等多模态医学数据集上实验表明，该方法能在稀疏数据条件下生成高质量、解剖结构一致、语义稳定的医学图像，同时显著提升重建效率
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测旨在实现接近人类水平的识别能力，但现有模型在低质量图像条件下的表现尚未得到充分评估。研究旨在填补这一空白，评估模型在真实世界低质量图像上的性能

Method: 引入新的模拟真实世界低质量图像的数据集，对多种开放词汇目标检测模型（OWLv2、OWL-ViT、GroundingDINO、Detic）在不同程度图像退化条件下进行性能评估

Result: 低程度图像退化时模型mAP分数无明显下降，但高程度退化时所有模型性能急剧下降。OWLv2在不同类型退化中表现相对稳定，而OWL-ViT、GroundingDINO和Detic性能显著下降

Conclusion: 开放词汇目标检测模型对高质量图像依赖性强，在高程度图像退化条件下性能受限。研究提供了评估数据集和代码，为未来改进模型在低质量图像条件下的鲁棒性奠定基础
...
TL;DR: EgoReAct：首个自回归框架，从以自我为中心的视频流实时生成3D对齐的人类反应动作，解决了现有方法在严格因果生成和精确3D空间对齐方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 人类对以自我为中心视觉输入表现出适应性、上下文敏感的反应，但现有方法难以同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在显著的空间不一致问题，动态动作总是与固定摄像头视频配对。

Method: 首先构建Human Reaction Dataset (HRD)解决数据稀缺和对齐问题；提出EgoReAct框架：1）通过向量量化变分自编码器将反应动作压缩到紧凑表达空间；2）训练生成预训练变换器从视觉输入生成反应；3）在生成过程中融入3D动态特征（度量深度和头部动态）增强空间基础。

Result: EgoReAct相比先前方法实现了显著更高的真实性、空间一致性和生成效率，同时在生成过程中保持严格因果关系。实验证明该方法在实时生成3D对齐反应动作方面表现优异。

Conclusion: EgoReAct是首个能够从以自我为中心视频流实时生成3D对齐人类反应动作的自回归框架，通过构建对齐数据集和融入3D动态特征，有效解决了空间一致性和因果生成问题，为以自我为中心视觉理解开辟了新方向。
...
### [57] [Depth Anything in $360^\circ$: Towards Scale Invariance in the Wild](https://arxiv.org/abs/2512.22819)
*Hualie Jiang,Ziyang Song,Zhiqiang Lou,Rui Xu,Minglang Tan*

Main category: cs.CV

TL;DR: DA360将Depth Anything V2适配到全景深度估计，通过ViT骨干学习偏移参数和圆形填充解码器，显著提升了零样本全景深度估计性能


<details>
  <summary>Details</summary>
Motivation: 全景深度估计在室内场景已有广泛研究，但在开放世界领域的零样本泛化能力远落后于透视图像。透视图像得益于丰富的训练数据，这种差距使得从透视域迁移能力成为有吸引力的解决方案。

Method: 提出了DA360，这是Depth Anything V2的全景适配版本。关键创新包括：1）从ViT骨干学习偏移参数，将模型的尺度和平移不变输出转换为尺度不变估计，直接生成格式良好的3D点云；2）在DPT解码器中集成圆形填充以消除接缝伪影，确保尊重球形连续性的空间一致深度图。

Result: 在标准室内基准和新策划的室外数据集Metropolis上评估，DA360相比基础模型有显著提升，在室内和室外基准上分别实现了超过50%和10%的相对深度误差减少。DA360显著优于稳健的全景深度估计方法，相比PanDA在所有三个测试数据集上实现了约30%的相对误差改进，为零样本全景深度估计建立了新的最先进性能。

Conclusion: DA360通过从透视域迁移能力，有效解决了全景深度估计在开放世界领域的零样本泛化问题，实现了显著性能提升并建立了新的SOTA。
...
TL;DR: SCaR-3D是一个新的3D场景变化检测框架，通过基于符号距离的2D差分模块和多视图聚合，从密集视图变化前图像序列和稀疏视图变化后图像中识别物体级变化，实现前后变化状态的明确分离。


<details>
  <summary>Details</summary>
Motivation: 现有3D变化检测方法存在检测变化的空间不一致性，且无法明确分离变化前和变化后的状态。为了解决这些限制，需要开发能够更准确检测3D场景变化的方法。

Method: 提出SCaR-3D框架，包含基于符号距离的2D差分模块，然后通过投票和剪枝进行多视图聚合，利用3DGS的一致性特性来鲁棒地分离变化前后状态。还开发了持续场景重建策略，选择性地更新动态区域同时保留未变化区域。

Result: 方法在实验中表现出高精度和高效率，优于现有方法。同时贡献了CCS3D合成数据集，支持对3D变化类型的灵活组合进行受控评估。

Conclusion: SCaR-3D框架有效解决了现有3D变化检测方法的问题，实现了更准确的变化检测和场景重建，为场景监控、探索和持续重建提供了有力工具。
...
TL;DR: 提出一种基于两个仿射对应点估计已知垂直方向下的相机相对姿态（3自由度）和焦距的新方法


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、智能手机、无人机等应用中，相机常与IMU结合使用。IMU可提供相机垂直方向，从而将相对姿态从5自由度减少到3自由度。现有方法需要更多对应点或计算复杂，本文旨在通过两个仿射对应点高效求解。

Method: 首先在已知垂直方向下，从两个仿射对应点建立约束方程。基于方程组有非平凡解的性质，推导出仅涉及焦距和相对旋转角度的四个方程。最后采用多项式特征值方法求解焦距和相对旋转角度。

Result: 在合成和真实数据集上的评估表明，所提出的求解器性能优于现有的最先进方法。

Conclusion: 该方法能够仅用两个仿射对应点有效估计已知垂直方向下的相机相对姿态和焦距，在计算效率和精度方面优于现有方法。
...
TL;DR: ByteLoom是一个基于扩散变换器的框架，用于生成具有几何一致对象插图的逼真人-物交互视频，解决了现有方法在多视角一致性和手部网格标注依赖方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互视频生成方法存在两个关键限制：1)缺乏有效的多视角信息注入机制，导致跨视角一致性差；2)严重依赖手部网格的细粒度标注来建模交互遮挡。这些问题限制了生成视频的质量和实用性。

Method: 提出了ByteLoom框架，采用扩散变换器架构，使用简化的人体条件和3D对象输入。核心创新包括：1)RCM-cache机制，利用相对坐标图作为通用表示来保持对象几何一致性并精确控制6自由度对象变换；2)渐进式训练课程，通过利用现有数据集缓解HOI数据集稀缺问题，并减少对手部网格的依赖。

Result: 实验表明，该方法能够忠实保持人体身份和对象的多视角几何特性，同时维持平滑的运动和对象操控，在生成质量上优于现有方法。

Conclusion: ByteLoom通过创新的RCM-cache机制和渐进式训练策略，成功解决了人-物交互视频生成中的多视角一致性和标注依赖问题，为数字人类、电子商务、广告和机器人模仿学习等应用提供了高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交导航数据集缺乏明确的推理监督，且动作分布呈现高度长尾，限制了模型学习安全关键行为的能力，需要更好的数据集来支持社交合规导航研究。

Method: 创建MUSON多模态数据集，采用五步思维链标注（感知、预测、推理、动作、解释），明确建模静态物理约束和合理平衡的离散动作空间，涵盖多样化的室内外校园场景。

Result: 在MUSON上基准测试多个最先进的小型视觉语言模型，Qwen2.5-VL-3B达到最高的决策准确率0.8625，证明MUSON作为社交合规导航的有效可复用基准。

Conclusion: MUSON通过结构化推理监督和平衡的动作空间，为社交合规导航提供了高质量的数据集和基准，有助于模型学习安全关键行为，数据集已公开可用。
...
TL;DR: Lamps是一种基于多视角解剖学自监督学习的医学影像基础模型，通过利用人体解剖学的三个关键特性（一致性、连贯性、层次性）作为监督信号，在胸部X光片上预训练，展现出优异的鲁棒性、可迁移性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 医学影像的核心基础是人体解剖结构，现有自监督学习方法往往忽视了解剖学的视角，限制了有效学习解剖特征的能力。需要开发能够捕捉人体解剖结构一致性和层次性的基础模型。

Method: 提出Lamps方法，通过自监督学习从多个视角学习解剖结构，利用人体解剖学的三个关键特性作为监督信号：一致性（解剖结构在不同样本中的稳定性）、连贯性（解剖结构间的空间关系）、层次性（解剖结构的组织层次）。在大规模胸部X光片数据集上进行预训练。

Result: 在10个数据集上的实验表明，Lamps在微调和涌现特性分析方面优于10个基线模型，展现出卓越的鲁棒性、可迁移性和临床潜力。通过学习多视角解剖特征，模型能够开发出与人体解剖结构对齐的有意义且鲁棒的表征。

Conclusion: Lamps为医学影像基础模型提供了独特的机会，通过从多视角学习解剖结构，能够开发出与人体解剖结构对齐的有意义且鲁棒的表征，为医学影像分析开辟了新方向。
...
TL;DR: 该论文提出了一种数据导向的方法来缓解深度学习模型中的伪相关性问题，通过识别、中和、消除和更新四个步骤构建了一个有效的去偏管道，在图像和NLP去偏基准测试中相比标准经验风险最小化将最差组准确率提高了20%以上。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练过程中经常学习到与类别标签虚假相关但与预测任务无关的特征。现有方法通常通过标注潜在的虚假属性或基于某些经验假设（如偏差的简单性）过滤虚假特征来解决此问题，但由于现实世界数据中虚假相关的复杂性和难以捉摸性，这些方法可能无法取得令人满意的性能。

Method: 提出了一种数据导向的去偏方法：1）识别：观察到受虚假特征影响的样本在学习的特征空间中呈现分散分布，从而识别虚假特征的存在；2）中和：基于简单的分组策略中和虚假特征，获得偏差不变表示；3）消除：学习特征变换以与偏差不变表示对齐，从而消除虚假特征；4）更新：通过整合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提高了20%以上，表明该方法能有效缓解深度学习模型中的伪相关问题。

Conclusion: 通过整合识别、中和、消除和更新四个步骤，构建了一个有效的管道来缓解深度学习模型中的伪相关问题，该方法在多个基准测试中表现出显著性能提升，为处理现实世界数据中复杂虚假相关提供了有效解决方案。
...
Result: 在Nr3D数据集上取得竞争性性能，在ScanRefer数据集上达到最先进水平，在提出的OpenTarget数据集上实现了17.6%的显著提升。OpenTarget数据集包含7000多个对象-描述对，专门用于评估开放世界场景。

Conclusion: OpenGround通过主动认知推理模块有效解决了传统方法依赖预定义对象查找表的限制，实现了开放世界3D视觉定位，在多个数据集上表现出色，为实际应用场景提供了更灵活的解决方案。
...
TL;DR: 本文提出了M-ErasureBench多模态评估框架，用于系统评估概念擦除方法在文本提示、学习嵌入和反转潜在空间三种输入模态下的表现，并开发了IRECE模块来增强擦除鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要针对文本提示，忽视了图像编辑和个性化生成等实际应用中越来越重要的其他输入模态（如学习嵌入和反转潜在空间），这些模态可能成为攻击面，导致已擦除的概念重新出现。

Method: 1) 提出M-ErasureBench多模态评估框架，在三种输入模态（文本提示、学习嵌入、反转潜在空间）和两种访问设置（白盒、黑盒）下系统评估概念擦除方法；2) 提出IRECE（推理时鲁棒性增强概念擦除）模块，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜在向量。

Result: 现有方法在文本提示下表现良好，但在学习嵌入和反转潜在空间下效果很差，白盒设置下的概念再现率（CRR）超过90%。IRECE模块能显著恢复鲁棒性，在最具挑战性的白盒潜在反转场景下将CRR降低达40%，同时保持视觉质量。
...
TL;DR: 本文提出GPS方法解决扩散模型迭代精炼中CFG引导导致采样路径偏离数据流形的问题，通过流形约束插值替代外推，实现稳定精炼并提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于去噪-反演循环的迭代精炼方法在与标准无分类器引导（CFG）结合时效果受限，因为CFG的外推性质会系统性地将采样路径推离数据流形，导致近似误差发散并破坏精炼过程。

Method: 提出引导路径采样（GPS）新范式，用基于流形约束的插值替代不稳定的外推，确保采样路径保持在数据流形上。同时设计最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的生成过程对齐。

Result: GPS在SDXL和Hunyuan-DiT等现代骨干模型上表现优异：在SDXL上获得0.79的ImageReward和0.2995的HPS v2分数，在GenEval上将整体语义对齐准确率提升至57.45%，在感知质量和复杂提示遵循方面均优于现有方法。

Conclusion: 路径稳定性是有效迭代精炼的前提条件，GPS提供了一个稳健的框架来实现这一目标，通过流形约束插值替代外推，将误差序列从无界放大转变为严格有界，保证了精炼过程的稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 高斯溅射在3D空间中分布不规则且不均匀，导致哈希网格中存在大量稀疏区域和无效特征，造成存储和传输的冗余开销。

Method: 基于输入高斯溅射的坐标识别并剪除哈希网格中的无效特征，只对有效特征进行编码，从而减少哈希网格的存储大小。

Result: 在标准化委员会定义的通用测试条件下，该方法相比基线方法平均实现了8%的比特率降低，同时不损害模型性能。

Conclusion: 提出的哈希网格特征剪枝方法有效减少了存储和传输开销，提升了率失真性能，为高斯溅射中的哈希网格优化提供了有效解决方案。
...
TL;DR: JavisGPT是首个用于联合音视频理解和生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练流程在音视频理解和生成任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要关注视觉-语言任务，缺乏对音视频联合理解和生成的支持，特别是在时空同步方面的能力不足。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音视频微调和大规模指令调优。

Result: 在音视频理解和生成基准测试中表现优异，特别是在复杂和时空同步设置下优于现有MLLMs。构建了包含20万+GPT-4o策划对话的JavisInst-Omni数据集。

Conclusion: JavisGPT成功实现了音视频联合理解和生成，为多模态AI系统提供了统一的解决方案，在时空同步任务上展现出显著优势。
...
TL;DR: PathoSyn是一个统一的MRI图像生成框架，通过将成像-病理学重构为稳定解剖流形上的解耦加性偏差，实现了高质量病理图像合成。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二元掩码，存在特征纠缠问题，导致解剖基质损坏或结构不连续。需要一种能够保持解剖完整性同时生成高质量病理变化的方法。
...
Conclusion: PathoSyn提供了一个数学原理驱动的管道，用于生成高质量合成数据，支持可解释的反事实疾病进展建模，促进低数据环境下稳健诊断算法的开发。
...
TL;DR: ColaVLA是一个统一的视觉-语言-动作框架，通过将推理从文本转移到统一潜在空间，并结合分层并行轨迹解码器，解决了VLM规划器的三个关键挑战：离散文本推理与连续控制不匹配、自回归解码延迟高、以及非因果规划器效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的规划器面临三个主要挑战：1）离散文本推理与连续控制之间的不匹配；2）自回归思维链解码带来的高延迟；3）非因果或低效的规划器限制了实时部署。需要一种既能保持VLM泛化能力和可解释性，又能实现高效、准确和安全轨迹生成的方法。
...
Conclusion: ColaVLA成功地将VLM的泛化能力和可解释性与高效的轨迹生成相结合，通过将推理转移到统一潜在空间并结合分层并行解码，解决了当前VLM规划器的关键挑战，为实时自主驾驶提供了有效的解决方案。
...
TL;DR: 本研究提出ForCM方法，结合基于对象的图像分析(OBIA)与深度学习(DL)，使用Sentinel-2多光谱影像进行森林覆盖制图。在亚马逊雨林测试多种DL模型，结果显示ResUNet-OBIA和AttentionUNet-OBIA分别达到94.54%和95.64%的总体精度，优于传统OBIA的92.91%。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法存在精度限制，需要开发更准确的技术来支持全球环境监测和保护工作。研究旨在探索结合OBIA与深度学习的新方法，利用免费易用的工具如QGIS，提高森林覆盖制图的精度和效率。

Method: 提出ForCM方法，结合OBIA与深度学习。使用Sentinel-2 Level 2A高分辨率卫星影像，包含三个数据集：两个三波段影像集和一个四波段影像集。评估多种深度学习模型（UNet、UNet++、ResUNet、AttentionUNet、ResNet50-Segnet），然后将最有效的DL模型与OBIA技术集成。与传统OBIA方法进行比较。

Result: ForCM方法显著提高了森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，而传统OBIA方法为92.91%。研究证明了免费工具如QGIS在准确制图方面的潜力。

Conclusion: ForCM方法通过结合OBIA与深度学习，显著提高了森林覆盖制图的准确性。该方法支持全球环境监测和保护工作，展示了免费易用工具在专业制图应用中的可行性，为森林资源管理提供了有效的技术方案。
...
TL;DR: 该研究提出使用虚幻引擎生成基于RGB的合成军事目标检测数据，以解决真实军事数据集缺乏和SAR数据成本高的问题，并在合成到真实的跨域设置中评估了最先进的域自适应方法。


<details>
  <summary>Details</summary>
Motivation: 军事目标检测在民用和军事应用中至关重要，但现有域自适应方法主要针对自然或自动驾驶场景，而军事领域环境复杂多变。SAR数据成本高且处理复杂，RGB相机成本低但缺乏军事目标检测数据集。

Method: 使用虚幻引擎生成基于RGB的合成军事目标检测数据，构建合成到真实的训练-验证数据集对，并评估不同监督程度的最先进域自适应方法。

Result: 使用图像级最小提示（如目标类别）的域自适应方法在合成到真实军事目标检测任务中表现优于无监督或半监督方法，但仍存在需要克服的挑战。

Conclusion: 合成数据生成是解决军事目标检测数据集缺乏的有效方法，但当前域自适应方法在军事跨域检测中仍有改进空间，需要进一步研究。
...
TL;DR: YOLO-IOD：首个基于YOLO的实时增量目标检测框架，解决YOLO检测器在增量学习中的灾难性遗忘问题，通过冲突感知伪标签细化、重要性核选择和跨阶段非对称知识蒸馏三个组件实现高效增量学习。


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法适应实时YOLO检测框架。YOLO在增量学习中面临三种主要知识冲突：前景-背景混淆、参数干扰和知识蒸馏不对齐，导致灾难性遗忘。

Method: 基于预训练YOLO-World模型构建YOLO-IOD框架，采用阶段式参数高效微调。包含三个核心组件：1）冲突感知伪标签细化（CPR），利用伪标签置信度缓解前景-背景混淆；2）重要性核选择（IKS），识别并更新当前任务关键卷积核；3）跨阶段非对称知识蒸馏（CAKD），通过新旧教师检测头进行非对称蒸馏。

Result: 在传统和LoCo COCO基准测试中，YOLO-IOD表现出卓越性能，遗忘最小。LoCo COCO是更现实的基准，消除了阶段间数据泄漏问题。

Conclusion: YOLO-IOD是首个基于YOLO的实时增量目标检测框架，有效解决了YOLO检测器在增量学习中的灾难性遗忘问题，为实时增量检测提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法存在两个主要问题：1）生成的图像由于视觉相似性不足而缺乏足够的伪装效果；2）生成的背景杂乱且与前景目标语义不一致。这导致生成的伪装图像与真实伪装图像存在显著差距，影响了伪装目标检测的训练数据质量。

Method: 提出ReamCamo框架，采用基于外绘制的统一方法。主要创新包括：1）引入额外的布局控制来调节全局图像结构，改善前景对象与生成背景之间的语义一致性；2）构建多模态文本-视觉条件，结合统一细粒度文本任务描述和面向纹理的背景检索，共同指导生成过程以增强视觉保真度和真实感。

Result: 为了定量评估伪装质量，引入了背景-前景分布差异度量来测量生成图像中伪装的有效性。通过大量实验和可视化验证了所提框架的有效性。
...
TL;DR: Holi-DETR：一种利用三种上下文信息（共现关系、空间布局、人体关键点关系）来整体检测时尚物品的检测器，相比传统独立检测方法能减少歧义。


<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化，同时子类别间存在相似性，导致检测歧义。传统检测器独立检测每个物品，忽略了时尚物品之间的上下文关系。

Method: 提出Holi-DETR，在DETR基础上整合三种异构上下文信息：1）时尚物品间的共现概率关系；2）基于物品间空间布局的相对位置和大小；3）物品与人体关键点的空间关系。

Result: 实验表明，该方法相比原始DETR提升了3.6个百分点（pp）的平均精度（AP），相比更新的Co-DETR提升了1.1pp。

Conclusion: 通过整体性地利用多种上下文信息，Holi-DETR能够更准确地检测时尚物品，有效减少因外观多样性和类别相似性带来的歧义问题。
...
TL;DR: PoseStreamer：一个针对高速运动场景的鲁棒多模态6DoF姿态估计框架，通过自适应姿态记忆队列、物体中心2D跟踪器和光线姿态滤波器提升性能，并在新数据集MoCapCube6D上验证效果。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在高速和低光场景下存在运动模糊问题，而事件相机虽有高时间分辨率，但现有6DoF姿态估计方法在高速物体运动场景中表现不佳，需要专门针对高速运动场景的解决方案。

Method: 提出PoseStreamer框架，包含三个核心组件：1）自适应姿态记忆队列，利用历史方向线索保持时间一致性；2）物体中心2D跟踪器，提供强2D先验以提升3D中心召回；3）光线姿态滤波器，沿相机光线进行几何细化。还构建了MoCapCube6D多模态数据集用于基准测试。

Result: 大量实验表明，PoseStreamer在高速运动场景中不仅实现了更高的精度，而且作为无模板框架对未见过的运动物体表现出强大的泛化能力。

Conclusion: PoseStreamer为解决高速运动场景下的6DoF姿态估计问题提供了有效的多模态解决方案，通过创新的时间一致性、2D先验和几何细化机制，显著提升了在高速运动条件下的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 工业制造中异常检测至关重要，但真实缺陷图像稀缺。现有合成方法存在明显权衡：基于规则的合成成本低但真实性不足，基于生成模型的合成质量高但成本巨大。需要找到平衡成本与质量的有效方法。

Method: 1) 使用预训练文本引导图像翻译模型生成合成缺陷图像；2) 利用图像检索模型评估生成图像与真实正常图像的相似度，过滤无关输出；3) 采用两阶段训练策略：先用大量规则合成图像预训练，再用少量高质量生成图像微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性，显著降低了数据收集成本的同时提升了异常检测性能。

Conclusion: 提出的框架通过结合文本引导图像翻译和图像检索，实现了高效、高质量的缺陷图像合成，两阶段训练策略平衡了成本与性能，为工业异常检测提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本引导的医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，无法有效识别病灶并建立与图像区域的关联；2）现有方法主要关注病灶描述而忽略位置约束，导致关键偏差（如"左下肺"可能错误覆盖双侧肺）。

Method: 提出空间感知对称对齐（SSA）框架，包含两个核心组件：1）对称最优传输对齐机制，建立图像区域与多个相关表达之间的双向细粒度多模态对应关系；2）复合方向引导策略，通过构建区域级引导掩码显式引入文本中的空间约束。

Result: 在公开基准测试上的广泛实验表明，SSA实现了最先进的性能，特别是在准确分割具有空间关系约束特征的病灶方面表现优异。

Conclusion: SSA框架有效增强了处理包含位置、描述和诊断信息的混合医学文本的能力，解决了现有方法的局限性，为文本引导的医学图像分割提供了更精确的空间约束处理方案。
...
TL;DR: 提出了一种基于条件扩散反演的反向个性化框架，用于面部匿名化，无需文本提示即可直接操作图像，支持属性可控的匿名化，在身份移除、属性保留和图像质量之间达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的身份特征移除或修改方法要么依赖于预训练模型中对主题的良好表示，要么需要对特定身份进行模型微调。需要一种能够超越模型训练数据中主题的通用方法，同时支持对匿名化过程的属性控制。

Method: 提出了反向个性化框架，利用条件扩散反演技术，无需文本提示即可直接操作图像。为了泛化到模型训练数据之外的主题，引入了身份引导的条件分支。该方法支持属性可控的匿名化，与先前缺乏面部属性控制的方法不同。

Result: 该方法在身份移除、属性保留和图像质量之间达到了最先进的平衡。源代码和数据已公开在GitHub上。

Conclusion: 提出的反向个性化框架为面部匿名化提供了一种有效的方法，能够在不依赖文本提示的情况下直接操作图像，支持属性控制，并具有良好的泛化能力。
...
TL;DR: 提出基于RGB无人机的低成本果园智能系统，集成病害检测、新鲜度评估和苹果检测定位，实现完全离线推理
...
Conclusion: 该框架为多光谱无人机方案提供了经济实惠的替代方案，支持在低成本硬件上实现实用的精准农业
...
TL;DR: 该研究对比了两种AI架构在医学影像诊断中的表现：经过微调的开源模型MedGemma-4b-it在六种疾病诊断中表现优于未调优的GPT-4，平均准确率达80.37% vs 69.58%，尤其在癌症和肺炎等高风险任务中敏感性更高。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为医学影像分析提供了新范式，但不同架构在临床诊断中的表现差异尚不明确。本研究旨在对比开源专业模型与通用大模型在疾病分类任务中的实际表现，为临床AI应用提供指导。

Method: 研究比较了两种不同AI架构：开源专业模型MedGemma-4b-it（使用LoRA低秩适应进行微调）与未调优的GPT-4大模型。评估了六种不同疾病的诊断能力，通过混淆矩阵和分类报告进行定量分析。

Result: MedGemma-4b-it平均测试准确率达80.37%，显著高于GPT-4的69.58%。在癌症和肺炎等高风险临床任务中，MedGemma表现出更高的敏感性。定量分析显示微调模型在减少幻觉方面表现更优。

Conclusion: 领域特定的微调对于临床AI实施至关重要，能有效减少幻觉现象。MedGemma作为专业工具在复杂医学推理中表现优异，为基于证据的医疗决策提供了可靠支持。
...
Method: 1. 从单目图像构建地理语义上下文图（GSCG）：整合度量深度估计器和统一的全景与材质分割模型；2. 将物体编码为节点（包含几何、颜色、材质属性），空间关系编码为边；3. 设计专门的基于图的分类器，聚合目标物体、邻近物体和全局场景特征进行类别预测。

Result: 上下文感知模型在COCO 2017数据集上达到73.4%的分类准确率，远超上下文无关版本（最低38.4%）；优于微调的ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 显式结构化和可解释的上下文表示对于物体识别任务具有显著优势，地理语义上下文图框架能够有效利用场景信息提升识别性能，同时保持推理过程的可解释性。
...
Result: 实现了亚秒级启动延迟（0.87秒）和32FPS实时吞吐率，是首个在14B规模上达到这一性能的系统，为高保真交互式数字人合成设立了新标准

Conclusion: SoulX-LiveTalk通过创新的双向注意力保留和自校正机制，成功解决了大规模扩散模型在实时虚拟人生成中的计算-延迟权衡问题，实现了高保真度下的实时性能
...
TL;DR: 提出基于天气条件和路面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，结合加速度数据，达到95%以上准确率。
...
Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据与图像数据结合训练；4. 比较加速度基和摄像头图像基方法的性能；5. 提出使用模糊逻辑根据天气和时间对路面进行分类。

Result: 1. 对5类道路条件（沥青、损坏沥青、碎石路、损坏碎石路、铺面路）进行分类；2. 达到超过95%的准确率性能；3. 比较了不同深度学习算法的性能。

Conclusion: 提出的实时系统能够有效监测道路表面状态，结合深度学习算法和模糊逻辑，为车辆控制和规划提供可靠的道路条件信息，相比传统方法更经济、系统化。
...
TL;DR: CoFi-Dec是一个无需训练的解码框架，通过生成式自反馈和从粗到细的视觉条件化来减少大视觉语言模型中的幻觉问题。
...
Method: CoFi-Dec采用训练自由的解码框架，受人类从全局场景感知到详细检查的视觉过程启发，首先生成基于粗粒度和细粒度视图的中间文本响应，然后使用文生图模型将其转换为合成图像，形成多级视觉假设。通过基于Wasserstein距离的融合机制统一多个视觉条件的预测分布。

Result: 在六个专注于幻觉的基准测试上，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略。该框架具有模型无关性，无需额外训练，可广泛应用于各种大视觉语言模型。

Conclusion: CoFi-Dec通过生成式自反馈和从粗到细的视觉条件化，结合基于Wasserstein距离的融合机制，有效减少了大视觉语言模型的幻觉问题，提高了输出的鲁棒性和忠实度。
...
<details>
  <summary>Details</summary>
Motivation: 当前运动生成模型在规模和性能上存在局限，需要开发大规模、高质量的3D人体运动生成模型，以推动该领域向商业成熟度发展。

Method: 采用全阶段训练范式：1）在3000多小时运动数据上进行大规模预训练；2）在400小时精选数据上进行高质量微调；3）结合人类反馈和奖励模型进行强化学习。同时建立了严谨的数据处理流程，包括运动清洗和标注。

Result: 模型实现了最广泛的运动覆盖，涵盖6大类超过200个运动类别，在指令跟随能力上显著超越当前开源基准，并成功将基于扩散变换器的流匹配模型扩展到十亿参数规模。

Conclusion: HY-Motion 1.0代表了运动生成领域的重大进展，通过开源发布将促进未来研究，加速3D人体运动生成模型向商业成熟度过渡。
...
Method: 提出AnyMS框架，采用无需训练的方法，通过全局解耦（分离文本和视觉条件的交叉注意力）和局部解耦（将每个主体的注意力限制在指定区域）的双级注意力解耦机制，并使用预训练图像适配器提取主体特征。

Result: 实验表明AnyMS达到最先进性能，支持复杂构图并扩展到更多主体，无需主体学习或适配器调优。

Conclusion: AnyMS通过创新的注意力解耦机制成功解决了多主体定制中的平衡问题，提供了一种高效、可扩展且无需训练的解决方案。
...
TL;DR: GeoTeacher提出了一种半监督3D目标检测方法，通过几何关系监督模块和数据增强策略，提升学生模型在有限标注数据下对物体几何关系的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽视了模型在有限标注数据下对物体几何结构的敏感性不足问题，这限制了模型对物体感知和定位能力的提升。

Method: 1) 设计基于关键点的几何关系监督模块，将教师模型的物体几何知识传递给学生模型；2) 提出体素级数据增强策略，增加物体几何多样性；3) 引入距离衰减机制保护远距离物体的完整性；4) 方法可与不同SS3D方法结合使用。

Result: 在ONCE和Waymo数据集上的大量实验表明，该方法有效且具有良好泛化能力，取得了新的最先进结果。

Conclusion: GeoTeacher通过几何关系监督和增强策略，显著提升了半监督3D目标检测中模型对物体几何结构的理解能力，为这一领域提供了新的解决方案。
...
Method: 采用"定位-推理-结论"结构化范式，利用多模态大语言模型显式定位语义元素并得出可解释的对齐判断，通过GRPO优化和复合奖励函数（包含结构格式、定位准确性和对齐保真度）。

Result: 在四个基准测试（EvalMuse-40K、RichHF、MHaluBench、GenAI-Bench）上达到最先进性能，优于强大的专有模型和监督基线，相比现有迭代视觉推理方法具有更优的推理效率。

Conclusion: REVEALER为文本到图像对齐评估提供了一个统一、可解释且高效的框架，通过强化学习引导的视觉推理实现了元素级别的细粒度评估，显著提升了评估质量和效率。
...
Method: 提出高斯-体素协同表示学习框架：1) 采用可泛化的高斯泼溅提取几何特征；2) 开发跨表示增强机制，用高斯场几何细节丰富体素特征；3) 通过可学习集成直接利用两种表示的特征，实现更准确的目标定位。

Result: 在ScanNetV2和ARKitScenes等具有挑战性的室内基准测试中取得最先进结果，显著优于现有方法，且无需任何深度或密集3D几何监督（如点云或TSDF）。

Conclusion: 高斯和体素表示捕获互补的几何信息，通过协同学习策略可以有效提升图像3D检测性能，无需昂贵的深度传感器或密集3D监督，为实际应用提供了可行解决方案。
...
TL;DR: RxnBench是一个用于评估多模态大语言模型在化学文献中理解反应能力的多层级基准，包含单图问答和全文档问答两个任务，揭示了当前模型在深层化学逻辑和精确结构识别方面的能力差距。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在化学领域的应用有望推动科学发现，但它们在真实文献中理解密集、图形化的反应语言的能力尚未得到充分探索，需要建立系统性的评估基准。

Method: 开发了RxnBench基准，包含两个任务：1) 单图问答：基于305个精选反应方案的1,525个问题，测试细粒度视觉感知和机理推理；2) 全文档问答：基于108篇文章，要求模型整合文本、方案和表格的跨模态信息。

Result: 评估发现模型存在关键能力差距：虽然擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但全文档问答任务中无一模型达到50%准确率。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的应用通常依赖经验直觉选择时间步特征，这往往导致次优性能且偏向特定任务。为了解决这个问题，研究者探索了如何自适应选择最适合少样本密集预测任务的时间步特征。

Method: 提出了两个核心模块：1) 任务感知时间步选择(TTS)，基于时间步损失和相似度分数选择理想的时间步；2) 时间步特征整合(TFC)，整合选定的时间步特征以提升少样本密集预测性能。同时采用了参数高效的微调适配器。

Result: 在具有挑战性的大规模Taskonomy数据集上进行实证验证，结果表明该方法在少样本密集预测任务中取得了优越性能，特别是在通用和少样本学习场景下。
...
TL;DR: MM-UAVBench：首个针对低空无人机场景的多模态大语言模型综合基准，包含感知、认知、规划三个核心维度，19个子任务，5700+人工标注问题，评估发现现有模型在低空复杂场景中表现不佳


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在通用领域表现出色，但在低空无人机应用场景中潜力尚未充分探索。现有基准缺乏对低空场景独特挑战的覆盖，而无人机相关评估主要关注特定任务，缺乏对MLLM通用智能的统一评估

Method: 提出了MM-UAVBench基准，系统评估MLLM在低空无人机场景中的三个核心能力维度：感知、认知和规划。基准包含19个子任务，超过5700个人工标注问题，所有数据均来自公开数据集的真实无人机采集数据

Result: 对16个开源和专有MLLM的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析进一步揭示了空间偏差和多视角理解等关键瓶颈，这些因素阻碍了MLLM在无人机场景中的有效部署

Conclusion: MM-UAVBench填补了低空无人机场景中MLLM评估的空白，揭示了当前模型的局限性，希望该基准能促进未来面向真实世界无人机智能的鲁棒可靠MLLM研究
...
<details>
  <summary>Details</summary>
Motivation: 现有AI视频创作系统通常将剧本起草和关键镜头设计作为两个独立任务：前者依赖大语言模型，后者依赖图像生成模型。作者认为这两个任务应该统一在单一框架中，因为逻辑推理和想象力思维都是电影导演的基本素质。

Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错，后解耦"的训练范式：1) 交错概念学习，使用交错文本图像数据培养模型对剧本的深入理解和想象解释；2) 解耦专家学习，将剧本写作与关键帧生成解耦，实现更大的故事讲述灵活性和创造力。
...
TL;DR: SURE引导的后验采样(SGPS)方法通过SURE梯度更新和PCA噪声估计校正采样轨迹偏差，在少于100次神经函数评估(NFEs)下实现高质量逆问题重建


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的逆问题求解方法通常需要数百或数千步才能达到高质量重建，主要原因是采样过程中存在误差累积问题

Method: 提出SURE引导的后验采样(SGPS)方法，使用Stein无偏风险估计(SURE)梯度更新和基于PCA的噪声估计来校正采样轨迹偏差，减少噪声诱导误差

Result: SGPS在少于100次神经函数评估(NFEs)下保持高质量重建，在多种逆问题评估中始终优于现有方法，特别是在低NFE计数情况下表现更佳

Conclusion: SGPS通过有效校正采样轨迹偏差，显著减少了误差累积，使得扩散模型在逆问题求解中能够在更少的计算步骤下实现高质量重建
...
Result: 实验表明，即使在剪枝85%训练数据后，该方法仍能显著改善收敛和生成质量。基于该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续取得最先进性能。

Conclusion: 该数据剪枝范式为开发遥感生成基础模型提供了实用指导，能够在高剪枝率下选择高质量子集，使初步基础模型快速收敛并作为生成、下游微调等应用的多功能骨干。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在固定分辨率无法平衡效率与细节、单尺度对齐缺乏语义层次等缺陷，限制了遥感图像多模态融合在环境监测、城市规划等领域的应用效果。

Method: 提出VLM框架，包含两个关键技术：1) 动态分辨率输入策略(DRIS)，采用粗到细方法自适应分配计算资源；2) 多尺度视觉语言对齐机制(MS-VLAM)，构建对象、局部区域和全局三层次对齐。
...
TL;DR: ASemconsist是一个新的文本到图像生成框架，通过选择性文本嵌入修改、语义控制策略和自适应特征共享，解决了在多样化场景描述中保持角色身份一致性的挑战，同时不牺牲每张图像的提示对齐。
...
Conclusion: ASemconsist通过创新的语义控制和自适应策略，成功解决了文本到图像生成中角色身份一致性的挑战，为相关研究提供了新的解决方案和评估标准。
...
Method: 提出轮廓信息感知的2D高斯泼溅框架，将对象分割先验整合到基于高斯的图像表示中。通过限制每个高斯在特定分割区域内进行光栅化，防止跨边界混合并保持边缘结构。还引入了预热方案来稳定训练并提高收敛性。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘周围实现了比现有2DGS方法更高的重建质量。改进在高斯数量非常少的场景中尤为明显，同时仍保持快速渲染和低内存使用。

Conclusion: 通过整合分割先验信息，提出的轮廓感知2D高斯泼溅框架能够显著改善图像表示质量，特别是在边缘保持方面，同时保持了原有方法的效率和紧凑性优势。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)的迭代去噪过程导致推理速度慢，现有基于缓存的方法虽然能实现免训练加速，但存在较大计算误差，且固定缓存策略无法适应去噪过程中复杂的误差变化

Method: 提出CEM插件，通过预定义误差来表征模型对加速的敏感性，结合时间步长和缓存间隔的影响，采用动态规划算法进行累积误差近似的策略优化，实现缓存误差最小化

Result: 在9个生成模型和量化方法上的实验表明，CEM显著提升了现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan等模型上甚至超越了原始生成性能

Conclusion: CEM是一种模型无关、泛化性强的插件方法，可无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，能适应任意加速预算，有效解决了扩散模型加速中的保真度问题
...
Method: 针对微手势分类：使用MViTv2-S提取视频嵌入和2s-AGCN提取骨骼嵌入，通过跨模态令牌融合模块整合空间和姿态信息。针对情感识别：使用SwinFace提取面部嵌入和MViTv2-S提取上下文嵌入，通过InterFusion模块融合情感表达和身体姿态信息。

Result: 在iMiGUE数据集上的实验表明，该方法在行为情感预测任务中表现出强大的性能和准确性，在MiGA 2025挑战赛的情感预测任务中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的信息（RGB、3D姿态、面部、上下文）来捕捉细微的人类行为模式，在挑战性任务中取得了优异成绩。
...
TL;DR: 提出CME-CAD异构协作多专家强化学习范式，通过两阶段训练生成高精度、可编辑的CAD模型，并发布包含17299个实例的CADExpert开源基准


<details>
  <summary>Details</summary>
Motivation: 传统CAD建模复杂，现有从草图重建3D模型的方法通常生成不可编辑的近似模型，无法满足工业设计对精度和可编辑性的严格要求，且依赖文本或图像输入需要大量人工标注，限制了可扩展性和工业应用

Method: 提出异构协作多专家强化学习（CME-CAD）范式，包含两阶段训练：多专家微调（MEFT）和多专家强化学习（MERL），整合不同模型的互补优势，促进协作学习，提高生成准确、约束兼容且完全可编辑CAD模型的能力

Result: 创建了CADExpert开源基准，包含17,299个实例，每个实例包含带精确尺寸标注的正交投影图、专家生成的思维链过程、可执行的CADQuery代码和渲染的3D模型

Conclusion: CME-CAD范式通过异构协作多专家强化学习，能够生成满足工业设计要求的高精度、可编辑CAD模型，解决了现有方法的局限性，并提供了大规模开源基准促进该领域研究
...
TL;DR: 该论文从拓扑学角度研究视觉表示学习，提出视觉理解需要语义语言，视觉空间具有纤维丛结构，语义抽象需要非光滑变换和特定的模型架构支持。


<details>
  <summary>Details</summary>
Motivation: 研究视觉表示学习的结构和拓扑特性，基于一个核心假设：视觉理解需要语义语言，许多感知观察对应少量离散语义状态。结合表示学习中广泛接受的迁移性和抽象性前提，探索视觉空间的拓扑结构。

Method: 采用拓扑学框架分析视觉表示学习。提出视觉观察空间具有纤维丛结构，其中冗余变化填充纤维，语义对应商基空间。从这一结构推导出两个理论结果：1）语义商空间不是子流形，需要非同胚的判别性目标；2）近似商空间对模型架构有结构要求。

Result: 理论分析表明：1）语义不变性需要非光滑变换，不能仅通过光滑变形实现，需要标签监督、跨实例识别或多模态对齐等显式语义等价；2）语义抽象需要"扩展-捕捉"过程，先几何扩展分离结构，再坍缩形成离散语义区域。

Conclusion: 该框架提供了拓扑学视角，与大规模判别性和多模态模型的经验规律以及统计学习理论的经典原理相一致。这些结果是解释性的而非规定性的，为理解视觉表示学习提供了理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预构建的3D表示或现成重建流程，限制了可扩展性和真实世界应用。真实环境中的部分可见性、遮挡和低重叠条件等挑战性空间推理问题研究不足

Method: 1) 提出可扩展的多视角数据生成和标注流水线，构建真实的空间推理问答对，创建包含200万QA对的SpatialMosaic数据集；2) 开发包含100万QA对、覆盖6个任务的SpatialMosaic-Bench基准测试；3) 提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中

Result: 实验表明，提出的数据集和视觉问答任务有效增强了在挑战性多视角条件下的空间推理能力，验证了数据生成流水线在构建真实多样QA对方面的有效性

Conclusion: 通过构建大规模多视角空间推理数据集和基准测试，以及集成3D几何编码器的混合框架，显著提升了视觉语言模型在真实挑战性场景下的空间推理能力
...
<details>
  <summary>Details</summary>
Motivation: 用户友好图像编辑模型的普及和滥用风险催生了对通用、最新的图像篡改检测与定位方法的需求。当前研究采用跨数据集评估，但这种简化方法掩盖了现有方法处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: 提出NeXT-IMDL基准，从四个基本维度对AIGC篡改进行分类：编辑模型、篡改类型、内容语义和伪造粒度。基于此构建五个严格的跨维度评估协议，对11个代表性模型进行广泛实验。

Result: 实验揭示关键发现：虽然这些模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的设计协议下，它们表现出系统性失败和显著的性能下降。

Conclusion: 通过提供这一诊断工具包和新发现，旨在推动真正稳健的下一代图像篡改检测与定位模型的发展。
...
TL;DR: SOFTooth：一种语义增强、顺序感知的2D-3D融合框架，利用冻结的2D语义（如SAM）提升3D牙齿实例分割性能，无需2D掩码监督。


<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临拥挤牙弓、模糊的牙齿-牙龈边界、缺失牙齿和临床重要的第三磨牙等挑战。现有3D方法依赖几何线索易出现边界泄漏、中心漂移和身份不一致问题，而2D基础模型（如SAM）虽提供强边界语义，但直接应用于3D临床工作流程不切实际。

Method: 提出SOFTooth框架：1）点级残差门控模块将咬合视图SAM嵌入注入3D点特征以细化边界；2）中心引导掩码细化正则化实例掩码与几何质心一致性；3）顺序感知匈牙利匹配策略整合解剖顺序和中心距离，确保标签连贯性。

Result: 在3DTeethSeg'22数据集上达到最先进的整体准确率和平均IoU，在处理第三磨牙等复杂情况时表现尤其突出，证明无需2D微调即可将丰富2D语义有效迁移到3D牙齿分割。

Conclusion: SOFTooth成功解决了3D牙齿分割的关键挑战，通过融合2D语义和3D几何信息，实现了对复杂解剖结构（包括第三磨牙）的鲁棒分割，为临床工作流程提供了实用解决方案。
...
TL;DR: 本文提出ArtQuant框架和RAD数据集，通过多维度结构化描述和LLM解码器解决AIGC美学质量评估中的数据稀缺、不平衡和模型碎片化问题，在减少训练成本的同时实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 美学质量评估对AIGC的人对齐定量评估系统至关重要，但其复杂性（视觉感知、认知、情感）带来根本挑战。现有方法面临两个关键问题：1）数据稀缺和不平衡，现有数据集过度关注视觉感知而忽略深层维度；2）模型碎片化，现有方法要么隔离美学属性，要么难以有效处理长文本描述。

Method: 1）提出RAD数据集：大规模（70k）、多维度结构化数据集，通过迭代流水线生成，无需昂贵人工标注且易于扩展；2）提出ArtQuant框架：通过联合描述生成耦合孤立的美学维度，利用LLM解码器更好地建模长文本语义，理论分析证实数据语义充分性和生成范式共同最小化预测熵。

Result: 方法在多个数据集上实现最先进性能，同时仅需传统训练轮次的33%，缩小了艺术图像与美学判断之间的认知差距。代码和数据集将开源以支持未来研究。

Conclusion: 本文通过RAD数据集和ArtQuant框架有效解决了美学质量评估中的数据稀缺、不平衡和模型碎片化问题，为AIGC的人对齐定量评估系统提供了高效解决方案，理论分析为框架提供了数学基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型方法存在局限性，虽然采用统一架构但仍将世界预测和运动规划作为解耦过程处理。为了弥合这一差距，需要真正统一视频生成和运动规划。
...
Result: DriveLaW在两个任务上都取得了新的最先进结果：在视频预测方面显著超越最佳性能工作（FID提升33.3%，FVD提升1.8%），并在NAVSIM规划基准测试中创造了新纪录。
...
TL;DR: DDSPO是一种新的扩散模型偏好优化方法，通过直接从获胜和失败策略中获取每时间步监督，在去噪轨迹上提供密集的过渡级信号，无需依赖昂贵的人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到图像生成中难以完全对齐用户意图并保持一致的审美质量。现有的基于偏好的训练方法（如Diffusion DPO）依赖昂贵且可能有噪声的人工标注数据集，需要更高效的无监督解决方案。

Method: 提出DDSPO方法，直接从获胜和失败策略中获取每时间步监督，在去噪轨迹上提供密集的过渡级信号。通过预训练参考模型自动生成偏好信号：对比原始提示与语义降级变体条件下的输出，避免依赖标注数据。

Result: 实验结果表明DDSPO在文本-图像对齐和视觉质量方面均有提升，优于或匹配现有的基于偏好的方法，同时需要显著更少的监督。
...
<details>
  <summary>Details</summary>
Motivation: 虽然SAM等基础分割模型在零样本任务中表现良好，但在领域偏移或知识有限的情况下仍然脆弱。本研究旨在探索不确定性量化能否以领域无关的方式缓解这些挑战并增强模型泛化能力。

Method: 1) 创建UncertSAM基准，包含8个设计用于在阴影、透明度和伪装等挑战性条件下压力测试SAM的数据集；2) 评估一套轻量级、后处理的不确定性估计方法；3) 评估初步的不确定性引导预测细化步骤。

Result: 在评估的方法中，最后一层拉普拉斯近似产生的不确定性估计与分割误差有良好相关性，表明存在有意义的信号。虽然细化改进是初步的，但研究结果强调了将不确定性纳入分割模型以支持稳健、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力增强分割模型在挑战性条件下的泛化能力。最后一层拉普拉斯近似是有效的轻量级不确定性估计方法。研究提供了公开的基准和代码，为未来不确定性引导分割研究奠定了基础。
...
TL;DR: RealX3D是一个真实采集的多视角视觉恢复和3D重建基准数据集，包含多种物理退化情况，分为四类退化类型，并提供了像素对齐的低质量/高质量视图、原始图像和激光扫描数据。


<details>
  <summary>Details</summary>
Motivation: 当前多视角重建方法在真实世界具有挑战性的环境中表现脆弱，缺乏能够系统评估物理退化对重建质量影响的基准数据集。

Method: 采用统一的采集协议，将退化分为四类（光照、散射、遮挡、模糊），并在多个严重级别上捕获每个场景，提供像素对齐的LQ/GT视图、高分辨率图像、RAW图像和密集激光扫描数据。

Result: 对多种基于优化和前馈方法的基准测试显示，在物理退化条件下重建质量显著下降，突显了当前多视角重建流程在真实挑战性环境中的脆弱性。

Conclusion: RealX3D基准数据集揭示了当前多视角重建方法在真实世界物理退化条件下的局限性，为开发更鲁棒的视觉恢复和3D重建算法提供了重要评估工具。
...
TL;DR: 提出了一种结合视觉水线检测、YOLOv8姿态尺度提取和多模态大语言模型的自动化河流水位计读数框架，显著提高了水位监测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统水文观测方法存在人工测量误差和环境限制，需要更准确、连续的河流水位监测方法用于洪水预报、水资源管理和生态保护。

Method: 采用混合框架：图像预处理→标注→水线检测→尺度间隙估计→数值读数提取。结合视觉水线检测、YOLOv8姿态尺度提取和GPT-4o、Gemini 2.0 Flash等多模态大语言模型。
...
Conclusion: 该方法为自动化水文监测提供了可扩展、高效、可靠的解决方案，展示了实时河流水位计数字化和改进水资源管理的潜力，强调了结合几何元数据与多模态人工智能对鲁棒水位估计的重要性。
...
TL;DR: TV-RAG是一个无需训练的架构，通过时间对齐和熵引导语义改进长视频推理，包含时间衰减检索和熵加权关键帧采样机制。


<details>
  <summary>Details</summary>
Motivation: 现有大型视频语言模型在处理长视频时存在时间窗口狭窄、无法捕捉细粒度语义变化的问题，且主流文本检索方法主要依赖表层词汇重叠，忽略了视觉、音频和字幕通道之间的时间相互依赖性。

Method: 提出TV-RAG架构，包含两个主要机制：1) 时间衰减检索模块，将显式时间偏移注入相似性计算，根据真实多媒体上下文对文本查询进行排序；2) 熵加权关键帧采样器，选择均匀间隔、信息密集的帧，减少冗余同时保持代表性。

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线，证实了模型的有效性。

Conclusion: TV-RAG实现了一种双重推理机制，可以无需重新训练或微调地集成到任何大型视频语言模型中，提供轻量级、经济高效的升级路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型虽然能从文本生成故事并保持角色一致性，但在人类中心故事生成中面临额外挑战：需要保持详细多样的人脸一致性，以及协调不同图像中的多个角色。

Method: 框架包含两个关键组件：1) 迭代身份发现：提取连贯的角色身份特征；2) 重去噪身份注入：通过重去噪过程注入身份特征，同时保持所需上下文。

Result: 在ConsiStory-Human基准测试中，IdentityStory优于现有方法，特别是在人脸一致性方面，并支持多角色组合。框架还显示出在无限长度故事生成和动态角色组合等应用中的强大潜力。

Conclusion: IdentityStory通过创新的身份保持生成器技术，有效解决了人类中心故事生成中的角色一致性挑战，为相关应用提供了有力支持。
...
TL;DR: 本文提出IAFS框架，通过迭代扩散推理时间缩放与自适应频率引导，解决图像超分辨率中感知质量与结构保真度的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度。虽然推理时间缩放理论上可以改善这一权衡，但现有策略仍不理想：奖励驱动的粒子优化常导致感知过度平滑，而最优路径搜索则容易失去结构一致性。

Method: 提出IAFS（迭代扩散推理时间缩放与自适应频率引导）框架，这是一种无需训练的方法，联合利用迭代细化和频率感知粒子融合。通过迭代修正结构偏差逐步细化生成图像，同时自适应地融合高频感知线索与低频结构信息，实现不同图像细节的准确平衡重建。

Result: 在多个基于扩散的超分辨率模型上的广泛实验表明，IAFS有效解决了感知-保真度冲突，持续改善了感知细节和结构准确性，性能优于现有推理时间缩放方法。

Conclusion: IAFS框架通过迭代细化和自适应频率融合，成功解决了扩散模型在图像超分辨率中感知质量与结构保真度的平衡问题，为推理时间缩放提供了更优策略。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在提升文本到图像生成质量的同时，也增加了生成不安全内容的风险。传统安全方法（如文本黑名单或有害内容分类）存在易被规避、需要大量数据和额外训练等显著缺陷。

Method: 提出PurifyGen训练免费方法：1）通过计算互补语义距离评估提示中每个token的安全性，无需关键词匹配或重新训练；2）对高风险提示应用双空间变换：将有毒性对齐的嵌入投影到有毒概念矩阵的零空间以移除有害语义，同时将其对齐到清洁概念的范围空间以强化安全语义；3）采用token级策略选择性替换高风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了现有方法，并能与依赖训练的方法竞争，同时保持了原始意图和连贯性。

Conclusion: PurifyGen提供了一种即插即用的解决方案，具有理论基础和强大的泛化能力，能够处理未见过的提示和模型，为安全文本到图像生成提供了有效途径。
...
TL;DR: ThinkGen是一个基于思维链推理的视觉生成框架，通过解耦的MLLM和DiT架构，结合创新的SepGRPO训练范式，在各种生成场景中实现高质量的图像生成。
...
Conclusion: ThinkGen是首个显式利用MLLM思维链推理的视觉生成框架，其灵活设计支持跨不同数据集的联合训练，为广泛的生成场景提供有效的思维链推理。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致多模态安全风险不断涌现，现有防御方法存在局限性，特别是对分布外安全风险的检测能力不足。

Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注；2) 通过纯强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并添加基于同义词库的相似性奖励机制。

Result: ProGuard在二元安全分类上达到闭源大模型水平，在非安全内容分类上显著优于现有开源防护模型，分布外风险检测提升52.6%，分布外风险描述提升64.8%。

Conclusion: ProGuard提供了一种有效的主动防护方法，能够识别和描述分布外安全风险，为多模态内容安全提供了新的解决方案。
...
TL;DR: 本文提出了一种实时交互式视频扩散模型，通过改进蒸馏方法解决多模态条件下视频生成的视觉伪影问题，实现了20倍推理加速，并构建了LiveTalk实时多模态交互化身系统。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过迭代双向注意力处理所有视频帧，无法实现实时交互。虽然现有蒸馏方法可以使模型自回归并减少采样步骤，但主要关注文本到视频生成，导致人机交互不自然且效率低下。本文旨在实现基于多模态上下文（文本、图像、音频）的实时交互式视频扩散。
...
Result: 蒸馏后的模型在视觉质量上与完整步骤的双向基线模型相当或更好，同时推理成本和延迟降低了20倍。将模型与音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks集成，构建了LiveTalk实时多模态交互化身系统。在精心策划的多轮交互基准测试中，LiveTalk在多轮视频连贯性和内容质量方面优于最先进模型（Sora2、Veo3），同时将响应延迟从1-2分钟降低到实时生成。

Conclusion: 通过改进的蒸馏方法成功实现了实时多模态交互视频生成，显著降低了推理延迟，使无缝的人机多模态交互成为可能。LiveTalk系统在视频质量和连贯性方面优于现有最先进模型，为构建通用多模态交互AI系统奠定了基础。
...
TL;DR: TWIN是一个包含56.1万张图像对的大规模数据集，旨在提升视觉语言模型在细粒度感知上的能力，通过判断两张相似图像是否描绘同一物体来训练模型关注细微视觉线索。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在广泛视觉理解方面表现出色，但在细粒度感知上存在不足，容易产生视觉偏见并忽略细微视觉细节。现有训练语料库强调一般性识别而非细粒度感知，加剧了这一局限性。

Method: 提出了TWIN数据集，包含561,000个图像对查询，要求模型判断两个视觉上相似的图像是否描绘同一物体，从而鼓励模型关注细微视觉线索。数据集涵盖日常物体的多种情境、视角和外观。同时引入了FGVQA基准测试套件，包含12,000个查询，用于量化模型在细粒度识别上的改进。

Result: 在TWIN上微调的视觉语言模型在细粒度识别方面取得显著提升，即使在未见过的领域（如艺术、动物、植物和地标）也能表现良好。在FGVQA基准测试上，微调后的模型性能提升高达19.3%，同时不影响在通用VQA基准测试上的表现。分析表明数据集规模对性能至关重要。

Conclusion: TWIN数据集能够有效提升视觉语言模型的感知精度，可作为开源VLM训练语料库的补充组件，推动未来模型在细粒度视觉理解方面的发展。
...
<details>
  <summary>Details</summary>
Motivation: 红外夜视相机火灾检测准确性仍具挑战性，现有模型虽有一定性能但存在明显缺陷，特别是数据集构建不足导致人工亮光常被误判为火灾，需要改进夜间检测精度并减少误报。

Method: 1) 构建额外NIR数据集并应用多种数据增强策略；2) 提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0；3) 开发Patched-YOLO用于RGB图像，通过基于补丁的处理增强小目标和远距离目标检测能力。

Result: 相比先前方法（YOLOv7 mAP50-95 0.51，RT-DETR 0.65，YOLOv9 0.598），提出的两阶段方法在夜间火灾检测方面获得更高准确性，特别是在减少人工亮光误报方面表现更好。

Conclusion: 通过数据集增强、两阶段检测架构和补丁处理技术，显著提升了红外夜视相机火灾检测的准确性，特别是在夜间场景和小目标检测方面取得了改进。
...
<details>
  <summary>Details</summary>
Motivation: 胰腺肿瘤早期检测面临临床困境，主要因为肿瘤在CT扫描中对比度低、患者间解剖变异大，需要有效且可扩展的系统来增强细微视觉线索并提高多模态影像数据的泛化能力。

Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割使胰腺结构更明显；2)DenseNet-121提取特征并保留残差；3)混合HHO-BA元启发式特征选择优化特征子集；4)结合Vision Transformer和EfficientNet-B3的混合模型进行分类；5)SSA和GWO双重优化机制微调超参数。

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，明显优于传统CNN和当代基于Transformer的模型。

Conclusion: SRFA框架在胰腺肿瘤早期检测中表现出优异性能，证明其作为临床有用工具的潜力，能够有效处理CT扫描中的复杂性和变异性。
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型的记忆效应，通过控制实验发现记忆效应受数据模态、多样性、条件粒度以及模型设计（如引导尺度、向量集长度、旋转增强）的影响，并提出了减少记忆效应的有效策略。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型在3D视觉中广泛应用，尚不清楚它们是否依赖记忆训练形状。理解记忆效应有助于防止训练数据泄露并提高生成结果的多样性。

Method: 设计了一个评估框架来量化3D生成模型的记忆效应，并通过对潜在向量集扩散模型进行控制实验，研究不同数据和建模设计对记忆效应的影响。

Result: 发现记忆效应取决于数据模态，随数据多样性和更细粒度条件而增加；在建模方面，记忆效应在中等引导尺度达到峰值，可通过更长的向量集和简单旋转增强来缓解。

Conclusion: 该框架和分析提供了对3D生成模型记忆效应的实证理解，并提出了简单有效的策略来减少记忆效应而不降低生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖注意力机制跨帧对齐物体，使用统一的显式物理模型简化运动建模，偏好语义特征进行隐式对齐，这挑战了传统感知范式中显式运动建模的重要性。然而，不同类别和帧间运动状态和物体特征的差异使得这种对齐不够优化。

Method: HAT首先利用多个显式运动模型为历史实例生成空间锚点和运动感知特征提案，然后通过融入缓存物体查询中嵌入的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐提案。

Result: 在nuScenes数据集上，HAT持续改进各种基线的3D时序检测器和跟踪器。与DETR3D检测器配对时，在测试集上达到46.0% AMOTA的最先进跟踪结果。在以物体为中心的端到端自动驾驶方法中，HAT提升感知精度（+1.3% mAP，+3.1% AMOTA）并降低32%碰撞率。在语义受损情况下（nuScenes-C），HAT的运动建模增强使端到端自动驾驶中的感知和规划更加鲁棒。

Conclusion: HAT通过自适应多假设解码机制有效解决了自动驾驶中时空对齐的优化问题，显著提升了时序感知性能，特别是在语义信息受损时表现出更强的鲁棒性，证明了显式运动建模在端到端自动驾驶感知中的重要性。
...
### [128] [OmniAgent: Audio-Guided Active Perception Agent for Omnimodal Audio-Video Understanding](https://arxiv.org/abs/2512.23646)
*Keda Tao,Wenjie Du,Bohan Yu,Weiqiang Wang,Jian Liu,Huan Wang*

Main category: cs.CV

TL;DR: OmniAgent：一种完全音频引导的主动感知智能体，通过动态编排专用工具实现细粒度音频-视觉推理，在三个基准测试中超越现有模型10%-20%


<details>
  <summary>Details</summary>
Motivation: 现有的全模态大语言模型在统一音频和视觉模态方面取得了进展，但缺乏细粒度的跨模态理解能力，在多模态对齐方面存在困难。传统方法依赖僵化的静态工作流程和密集帧标注，需要从被动响应生成转向主动多模态查询。

Method: OmniAgent采用动态规划自主编排按需工具调用，战略性地将感知注意力集中在任务相关线索上。核心创新是粗到细的音频引导感知范式，利用音频线索定位时间事件并指导后续推理。

Result: 在三个音频-视频理解基准测试上的广泛实证评估表明，OmniAgent实现了最先进的性能，超越领先的开源和专有模型10%-20%的准确率。

Conclusion: 该研究展示了从被动响应生成到主动多模态查询的范式转变，通过音频引导的动态工具编排实现了更细粒度的音频-视觉推理能力。
...
### [129] [IDT: A Physically Grounded Transformer for Feed-Forward Multi-View Intrinsic Decomposition](https://arxiv.org/abs/2512.23667)
*Kang Du,Yirui Guan,Zeyu Wang*

Main category: cs.CV

TL;DR: IDT是一种基于Transformer的前馈框架，用于多视角本征图像分解，通过注意力机制联合处理多张输入图像，在单次前向传播中生成视角一致的本征因子，避免了迭代生成采样。
...
Method: IDT采用基于物理的图像形成模型，将图像明确分解为漫反射率、漫反射着色和镜面反射着色。通过Transformer注意力机制联合推理多个输入图像，在单次前向传播中产生视角一致的本征因子。

Result: 在合成和真实世界数据集上的实验表明，IDT实现了更干净的漫反射率、更一致的漫反射着色和更好分离的镜面反射分量，相比先前方法显著提高了多视角一致性。

Conclusion: IDT通过Transformer注意力机制和物理基础的分解模型，成功解决了多视角本征图像分解中的视角一致性问题，实现了可解释和可控的材质与光照效应分解。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然感知质量好，但依赖未来帧和昂贵的多步去噪，不适用于延迟敏感场景。需要开发能够在严格使用过去帧的情况下实现高效在线处理的扩散VSR方法。

Method: 提出Stream-DiffVSR框架：1）四步蒸馏去噪器实现快速推理；2）自回归时间引导模块在潜在去噪过程中注入运动对齐线索；3）轻量级时间感知解码器配合时间处理器模块增强细节和时间一致性。

Result: 在RTX4090上处理720p帧仅需0.328秒，显著优于现有扩散方法。相比在线SOTA TMP方法，提升感知质量（LPIPS +0.095）同时降低延迟130倍以上，将初始延迟从4600秒降至0.328秒。

Conclusion: Stream-DiffVSR是首个适用于低延迟在线部署的扩散VSR方法，实现了扩散方法中最低的延迟，为实时视频超分辨率应用提供了实用解决方案。
...
### [131] [Open-Source Multimodal Moxin Models with Moxin-VLM and Moxin-VLA](https://arxiv.org/abs/2512.22208)
...
TL;DR: Moxin 7B是一个完全开源的大语言模型，遵循模型开放框架，提供完整的训练、数据集和实现细节透明度，并开发了三个变体分别针对视觉语言、视觉语言动作和中文能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型发展迅速，但开源模型通常只分享权重，缺乏完整的透明度。为了促进更包容和协作的研究环境，建立健康的开源生态系统，需要开发完全透明的开源模型。

Method: 基于模型开放框架开发完全开源的Moxin 7B模型，提供完整的训练、数据集和实现细节透明度。进一步开发三个变体：Moxin-VLM（视觉语言）、Moxin-VLA（视觉语言动作）和Moxin-Chinese（中文能力），采用开源框架和开源数据进行训练。

Result: 实验表明，Moxin模型在各种评估中取得了优越的性能。模型、可用数据以及推导这些模型的代码都已公开发布。

Conclusion: Moxin 7B作为一个完全透明的开源大语言模型，超越了简单的权重分享，通过完整的透明度促进了更包容和协作的研究环境，有助于维持健康的开源生态系统。
...
### [132] [Hierarchical Geometry of Cognitive States in Transformer Embedding Spaces](https://arxiv.org/abs/2512.22227)
*Sophie Zhao*

Main category: cs.CL

TL;DR: 研究发现Transformer语言模型的句子嵌入空间编码了与人类认知属性对齐的分层几何结构，这种结构可通过线性/非线性探针可靠解码。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究表明Transformer语言模型在嵌入空间中学习到丰富的几何结构，但其中是否存在与人类认知或心理属性对齐的高层组织仍然未被充分探索。本研究旨在探究句子嵌入是否编码了与人类可解释的认知或心理属性对齐的分级、分层结构。
...
Result: 在所有模型中，连续分数和层级标签都能可靠解码，浅层非线性探针相比线性探针提供了一致的性能提升。词汇TF-IDF基线表现明显更差，表明观察到的结构不能仅归因于表面词汇统计。非参数置换测试进一步确认探针性能显著优于随机标签情况。UMAP可视化和混淆矩阵的定性分析揭示了嵌入空间中从低到高的平滑梯度以及主要相邻层级的混淆模式。

Conclusion: 这些结果表明Transformer嵌入空间展现出与人类定义的认知属性对齐的分层几何组织，同时不涉及对内部意识或现象学的断言。这为理解语言模型如何编码高层次认知信息提供了实证证据。
...
### [133] [SmartSnap: Proactive Evidence Seeking for Self-Verifying Agents](https://arxiv.org/abs/2512.22322)
...
TL;DR: SmartSnap提出了一种从被动事后验证转向主动实时自验证的新范式，让智能体在完成任务的同时主动收集精简的截图证据，显著提升了GUI任务中强化学习智能体的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有GUI任务中智能体强化学习的可扩展性受到任务验证的严重制约。传统的验证方法是被动的事后处理，需要分析整个冗长的交互轨迹，包含大量无关噪声信息，导致验证成本高昂且可靠性低。

Method: 提出SmartSnap范式，引入自验证智能体，具有双重使命：完成任务和提供精选的截图证据证明任务完成。基于3C原则（完整性、简洁性、创造性），智能体利用在线环境访问权限，在最小化、决定性的截图集上进行自验证，然后由通用的LLM-as-a-Judge验证器评估证据的有效性和相关性。
...
Conclusion: SmartSnap通过从被动事后验证转向主动实时自验证的范式转变，解决了GUI任务中智能体强化学习的可扩展性瓶颈，实现了验证成本降低和可靠性提升的双重优势。
...
### [134] [The Syntax of qulk-clauses in Yemeni Ibbi Arabic: A Minimalist Approach](https://arxiv.org/abs/2512.22376)
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在分析也门伊比阿拉伯语中qulk-从句的特殊句法结构，这种从句是形态融合形式，意为"我说"，用于引入嵌入的陈述、疑问和祈使从句，通常没有补语化成分。

Method: 采用极简方案框架，应用核心极简操作（合并、移动、一致、拼读）对qulk-从句进行分层句法分析，展示其推导过程如何通过标准计算步骤和后句法过程（如形态合并）。

Result: 研究提出qulk-从句是双从句结构，qulk作为从句嵌入谓词选择完整CP补语，该分析能够解释方言特定特征如二分否定、附着化和CP嵌入。
...
### [135] [Towards Efficient Post-Training via Fourier-Driven Adapter Architectures](https://arxiv.org/abs/2512.22378)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大型预训练语言模型参数高效微调的问题，作者希望开发一种能够选择性调节不同频率语义信息的方法，在保持主干模型能力的同时实现高效适应。

Method: 提出傅里叶激活适配器(FAA)框架，将随机傅里叶特征集成到轻量级适配器模块中，将中间表示分解为互补的低频和高频分量，实现频率感知的语义信息调制。

Result: 在GLUE、E2E NLG和指令微调基准测试中，FAA始终达到与现有参数高效微调方法相当或更优的性能，同时保持较低的计算和内存开销。

Conclusion: FAA是一种稳健高效的参数高效微调方法，通过频率感知激活和自适应加权机制，为大型语言模型的后训练提供了有效的解决方案。
...
### [136] [LLM-Guided Exemplar Selection for Few-Shot Wearable-Sensor Human Activity Recognition](https://arxiv.org/abs/2512.22385)
*Elsen Ronando,Sozo Inoue*

Main category: cs.CL

TL;DR: 提出LLM引导的范例选择框架，解决HAR方法依赖大量标注数据和纯几何范例选择的局限性，在少样本条件下显著提升相似活动识别性能


<details>
  <summary>Details</summary>
Motivation: 当前最先进的人类活动识别方法存在两个关键限制：1）依赖大量标注数据集；2）仅使用纯几何范例选择方法，难以区分相似的可穿戴传感器活动（如行走、上楼、下楼）

Method: 提出LLM引导的范例选择框架，通过LLM生成的知识先验（包含特征重要性、类间混淆性、范例预算乘数）指导范例评分和选择，结合基于边界的验证线索、PageRank中心性、枢纽惩罚和设施选址优化，获得紧凑且信息丰富的范例集
...
### [137] [Hallucination Detection and Evaluation of Large Language Model](https://arxiv.org/abs/2512.22416)
...
Method: 集成HHEM轻量级分类框架，不依赖LLM判断；引入分段检索处理总结任务中的局部幻觉；进行TPR、TNR、准确率比较分析；使用累积分布函数分析模型规模与幻觉关系。

Result: HHEM将评估时间从8小时降至10分钟；HHEM+非捏造检查达到最高准确率82.2%和TPR 78.9%；分段检索改善了总结任务中的局部幻觉检测；7B-9B参数模型幻觉较少，中等规模模型稳定性较差。

Conclusion: 需要平衡计算效率与事实验证的结构化评估框架，HHEM为高效幻觉检测提供了可行方案，但总结任务中的局部幻觉仍需进一步改进。
...
### [138] [HiFi-RAG: Hierarchical Content Filtering and Two-Pass Generation for Open-Domain RAG](https://arxiv.org/abs/2512.22442)
...
<details>
  <summary>Details</summary>
Motivation: 开放域检索增强生成面临两个主要挑战：检索文档中包含大量不相关信息，以及生成的答案与用户意图对齐困难。需要更高效的方法来过滤无关信息并提升答案质量。

Method: 采用分层过滤的多阶段管道：1) 使用Gemini 2.5 Flash进行查询重构、分层内容过滤和引用归因；2) 使用Gemini 2.5 Pro进行最终答案生成。这种混合模型架构平衡了速度、成本与推理能力。

Result: 在MMU-RAGent验证集上，ROUGE-L提升19.6%至0.274，DeBERTaScore提升6.2%至0.677。在测试集Test2025（评估2025年1月后知识的问题）上，相比参数化基线，ROUGE-L提升57.4%，DeBERTaScore提升14.9%。

Conclusion: HiFi-RAG通过分层过滤策略和混合模型架构，有效解决了开放域RAG中的信息相关性和意图对齐问题，在性能和成本效率方面都取得了显著提升。
...
### [139] [Exploring the Vertical-Domain Reasoning Capabilities of Large Language Models](https://arxiv.org/abs/2512.22443)
*Jie Zhou,Xin Chen,Jie Zhang,Zhe Li*

Main category: cs.CL

TL;DR: 该研究评估了大语言模型在会计领域的推理能力，建立了垂直领域会计推理评估标准，并测试了多个代表性模型，发现GPT-4表现最佳但仍需优化才能满足实际应用需求。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的广泛应用，如何将其有效整合到会计专业领域并重新定义LLMs与领域特定应用的关系，已成为推动企业数字化转型和社会发展的关键挑战。需要理解LLMs在会计领域的特定推理能力。

Method: 研究引入了垂直领域会计推理概念，通过分析代表性GLM系列模型的训练数据特征建立评估标准。基于此框架，评估了GLM-6B、GLM-130B、GLM-4和OpenAI GPT-4等多个模型在会计推理任务上的表现，并测试了不同的提示工程策略。

Result: 实验结果显示，不同的提示工程策略对各个模型的性能提升程度不同，GPT-4展现出最强的会计推理能力。然而，当前的大语言模型仍无法满足实际应用需求，特别是在企业级会计场景部署方面需要进一步优化。

Conclusion: 研究为后续推理范式研究提供了基准，并指出需要进一步优化大语言模型才能充分发挥其在会计领域的潜在价值，特别是在企业级应用场景中。
...
### [140] [Constituency Structure over Eojeol in Korean Treebanks](https://arxiv.org/abs/2512.22487)
*Jungyeul Park,Chulwoo Park*

Main category: cs.CL

TL;DR: 该论文主张韩语选区树库应采用基于"어절"（eojeol，词单位）的选区表示，而非以语素为终端单位，以解决形态学与句法结构的混淆问题。


<details>
  <summary>Details</summary>
Motivation: 韩语选区树库设计面临一个基本表示问题：韩语词具有复杂的形态结构，将语素作为选区终端会混淆词内形态与短语层面的句法结构，同时与基于어절的依存资源产生不匹配。

Method: 提出基于어절的选区表示方案，将形态分割和细粒度词性信息编码在独立的非选区层中。通过比较分析，在明确的规范化假设下，证明世宗树库和宾州韩语树库在어절选区层面具有表示等价性。

Result: 分析表明世宗树库和宾州韩语树库在基于어절的选区表示层面可以视为等价。基于此结果，论文提出了一个保留可解释选区结构、支持跨树库比较和选区-依存转换的어절标注方案。

Conclusion: 韩语选区树库应采用基于어절的表示方法，将形态信息分离到独立层，这样既能保持选区结构的可解释性，又能实现与依存资源的兼容和跨树库的一致性。
...
### [141] [Learning When Not to Attend Globally](https://arxiv.org/abs/2512.22562)
*Xuan Luo,Kailai Zhang,Xifeng Yan*

Main category: cs.CL

TL;DR: AHA注意力机制让LLM能动态选择使用全局注意力或局部滑动窗口注意力，显著减少计算量而不损失性能


<details>
  <summary>Details</summary>
Motivation: 受人类阅读习惯启发，当阅读书籍时，人们主要关注当前页面，只在必要时才回顾前面的内容。类似地，LLM也应该能够动态决定何时需要访问全局上下文，而不是总是使用计算密集型的全局注意力机制。

Method: 提出All-or-Here Attention (AHA)机制，为每个注意力头配备一个二元路由器，动态为每个token切换使用完整全局注意力或局部滑动窗口注意力。通过评估不同窗口大小，分析上下文依赖的长尾分布特性。

Result: 使用256个token的窗口大小时，最多可以替换93%的原始全局注意力操作为滑动窗口注意力，且不损失性能。研究发现上下文依赖呈现长尾分布，随着局部窗口扩大，对全局注意力的需求迅速衰减。

Conclusion: 通过将本地处理与全局访问解耦，AHA揭示了全局注意力在很大程度上是冗余的，高效推理只需要按需访问全局上下文。这为LLM的高效推理提供了新的设计思路。
...
### [142] [Structured Prompting and LLM Ensembling for Multimodal Conversational Aspect-based Sentiment Analysis](https://arxiv.org/abs/2512.22603)
*Zhiqiang Gao,Shihao Gao,Zixing Zhang,Yihao Guo,Hongyu Chen,Jing Han*

Main category: cs.CL

TL;DR: 本文提出了一种基于大语言模型的结构化提示管道和集成策略，用于解决多模态对话中的情感分析任务，包括情感六元组提取和情感翻转检测。


<details>
  <summary>Details</summary>
Motivation: 理解多模态对话中的情感是构建情感智能AI系统的关键挑战。MCABSA挑战提出了两个复杂子任务：从多说话者对话中提取完整的情感六元组（持有者、目标、方面、意见、情感、理由），以及检测动态情感变化及其触发因素。
...
Conclusion: 研究表明，基于大语言模型的结构化提示管道和集成策略能够有效处理复杂的多模态情感分析任务，为构建情感智能AI系统提供了实用方法。
...
### [143] [Chain-of-thought Reviewing and Correction for Time Series Question Answering](https://arxiv.org/abs/2512.22627)
...
Result: 在多个真实世界TSQA基准测试中，T3LLM实现了优于现有强LLM基线的state-of-the-art性能。

Conclusion: T3LLM通过工作-评审-学习框架，利用时间序列数据的可验证性实现多步推理和显式纠错，显著提升了时间序列问答的准确性和可靠性。
...
### [144] [M2G-Eval: Enhancing and Evaluating Multi-granularity Multilingual Code Generation](https://arxiv.org/abs/2512.22628)
*Fanglin Xu,Wei Zhang,Jian Yang,Guo Chen,Aishan Liu,Zhoujun Li,Xianglong Liu,Bryan Dai*

Main category: cs.CL

TL;DR: M2G-Eval是一个多粒度、多语言的代码生成评估框架，涵盖类、函数、块、行四个粒度级别和18种编程语言，包含17K+训练任务和1286个测试实例。研究发现：行级任务最简单，类级最困难；随着任务复杂度增加，全粒度和部分粒度语言之间的性能差距扩大；存在强跨语言相关性，表明模型学习到了可迁移的编程概念。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成评估基准主要关注单一结构粒度和有限编程语言，无法揭示不同代码范围和跨语言场景下的细粒度能力差异。需要开发一个更全面的评估框架来系统分析LLM的代码生成能力。

Method: 提出M2G-Eval框架，包含类、函数、块、行四个粒度级别，覆盖18种编程语言。使用17K+训练任务和1286个人工标注、污染控制的测试实例。通过监督微调和组相对策略优化训练Qwen3-8B模型，开发M2G-Eval-Coder模型。评估了30个模型（28个SOTA LLM和2个M2G-Eval-Coder变体）。

Result: 发现三个主要结果：1）明显的难度层次结构，行级任务最简单，类级最困难；2）随着任务复杂度增加，全粒度和部分粒度语言之间的性能差距扩大；3）存在强跨语言相关性，表明模型学习到了可迁移的编程概念。
...
### [145] [On the Role of Discreteness in Diffusion LLMs](https://arxiv.org/abs/2512.22630)
*Ziqi Jin,Bin Wang,Xiang Lin,Lidong Bing,Aixin Sun*

Main category: cs.CL

TL;DR: 该论文分析了扩散模型在文本生成中的应用，指出离散和高度结构化的文本特性给扩散原理的直接应用带来挑战，并识别了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代优化等吸引人的特性，但文本的离散和高度结构化特性阻碍了扩散原理的直接应用。作者希望重新审视扩散语言建模，识别现有方法的局限性，并提出更符合文本结构的改进方向。

Method: 作者从扩散过程和语言建模的角度重新审视扩散语言建模，提出了区分扩散机制与语言特定需求的五个属性。将现有方法分类为嵌入空间的连续扩散和标记上的离散扩散，分析每种方法满足哪些属性。通过分析最近的大型扩散语言模型，识别了两个核心问题。

Result: 分析表明，现有方法都存在结构性权衡：连续扩散在嵌入空间进行但缺乏语言结构的考虑，离散扩散在标记层面但存在局限性。具体识别出两个核心问题：(1)均匀污染不尊重信息在位置间的分布方式；(2)标记级边缘训练无法在并行解码时捕获多标记依赖关系。

Conclusion: 需要开发更符合文本结构的扩散过程，鼓励未来研究朝着更连贯的扩散语言模型方向发展。扩散过程应该更好地与文本结构对齐，以克服现有方法的局限性。
...
### [146] [Evaluating GRPO and DPO for Faithful Chain-of-Thought Reasoning in LLMs](https://arxiv.org/abs/2512.22631)
*Hadi Mohammadi,Tamas Kozak,Anastasia Giachanou*

Main category: cs.CL

TL;DR: GRPO在提升大语言模型思维链忠实度方面优于DPO，特别是在较大模型中表现更好，Qwen2.5-14B-Instruct在所有评估指标上取得最佳结果


<details>
  <summary>Details</summary>
Motivation: 思维链推理虽然能提升大语言模型的问题解决能力，但其解释往往不能反映模型的实际推理过程，可能存在误导性解释或未承认外部线索的情况。这种不一致性削弱了基于思维链方法在安全监督和对齐监控中的可靠性，因为模型可能为错误答案生成看似合理但具有欺骗性的推理过程。

Method: 评估两种优化方法在提升思维链忠实度方面的能力：群体相对策略优化（GRPO）和直接偏好优化（DPO）。通过实验比较这两种方法在不同规模模型中的表现。

Result: GRPO在较大模型中表现优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标上取得最佳结果。两种方法都显示模型规模与性能呈正相关，但GRPO在提升忠实度指标方面显示出更大潜力，尽管在较小规模时行为不太稳定。

Conclusion: GRPO为开发更透明、更可信赖的大语言模型推理提供了有前景的方向，有助于解决思维链解释与模型实际推理过程不一致的问题。
...
### [147] [Conformal Prediction Sets for Next-Token Prediction in Large Language Models: Balancing Coverage Guarantees with Set Efficiency](https://arxiv.org/abs/2512.22682)
*Yoshith Roy Kotla,Varshith Roy Kotla*

Main category: cs.CL

TL;DR: 本文提出词汇感知的共形预测（VACP）方法，用于改进大语言模型中的不确定性量化，通过语义掩码和温度调整评分显著减少预测集大小，同时保持理论覆盖率保证。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型部署于高风险领域时，需要严格的不确定性量化，但标准的softmax概率往往校准不佳。传统的共形预测方法虽然能保证覆盖率，但产生的预测集包含数百个token，信息量不足。

Method: 提出词汇感知的共形预测（VACP）框架，结合语义掩码和温度调整评分来减少有效预测空间。该方法在保持边际覆盖率理论保证的同时，通过语义信息减少预测集大小。

Result: 在Gemma-2B模型上使用SQUAD和WikiText基准测试，VACP实现了89.7%的经验覆盖率（目标90%），同时将平均预测集大小从847个token减少到4.3个token，效率提升197倍。

Conclusion: VACP框架成功解决了大词汇量语言模型中覆盖率与效率的权衡问题，为高风险应用中的不确定性量化提供了实用且理论保证的解决方案。
...
### [148] [GHaLIB: A Multilingual Framework for Hope Speech Detection in Low-Resource Languages](https://arxiv.org/abs/2512.22705)
*Ahmed Abdullah,Sana Fatima,Haroon Mahmood*

Main category: cs.CL

TL;DR: 本文提出了一个多语言希望言论检测框架，重点关注乌尔都语，使用预训练Transformer模型在PolyHope-M 2025基准测试中取得了优异性能。
...
Result: 在PolyHope-M 2025基准测试中表现出色：乌尔都语二分类F1分数达到95.2%，乌尔都语多分类F1分数达到65.2%。在西班牙语、德语和英语中也获得了类似的竞争性结果。

Conclusion: 研究结果表明，可以在低资源环境中实施现有的多语言模型，从而更容易识别希望言论，有助于构建更具建设性的数字话语环境。
...
### [149] [Beg to Differ: Understanding Reasoning-Answer Misalignment Across Languages](https://arxiv.org/abs/2512.22712)
*Anaelia Ovalle,Candace Ross,Sebastian Ruder,Adina Williams,Karen Ullrich,Mark Ibrahim,Levent Sagun*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在多语言推理中存在严重缺陷：虽然任务准确率高，但推理过程与结论之间经常不匹配，非拉丁文字语言的推理错误率是拉丁文字语言的两倍以上。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型通过思维链提示展现了强大的推理能力，但这种推理质量是否能在不同语言间保持一致性尚未得到充分探索。当前的多语言评估实践可能无法全面反映模型的真实推理能力。

Method: 研究者开发了一个人工验证的评估框架，用于评估模型生成的推理轨迹是否在逻辑上支持其结论。分析了来自GlobalMMLU问题的65,000个推理轨迹，涵盖6种语言和6个前沿模型。通过人工标注建立了错误分类法来表征推理失败的原因。

Result: 研究发现了一个关键盲点：虽然模型在任务准确率上表现良好，但它们的推理过程常常无法支持其结论。非拉丁文字语言的推理轨迹中，推理与结论之间的不匹配至少是拉丁文字语言的两倍。主要错误类型包括证据错误（无支持的断言、模糊事实）和不合逻辑的推理步骤。

Conclusion: 当前的多语言评估实践无法完整反映模型的推理能力，需要开发推理感知的评估框架来更准确地评估模型在多语言环境下的推理质量。
...
### [150] [Mitigating Social Desirability Bias in Random Silicon Sampling](https://arxiv.org/abs/2512.22725)
*Sashank Chapala,Maksym Mironov,Songgaojun Deng*

Main category: cs.CL

TL;DR: 研究验证了通过心理学的提示词设计可以缓解LLM在模拟人口响应时的社会期望偏差，其中重新表述的提示词效果最佳。


<details>
  <summary>Details</summary>
Motivation: LLM被广泛用于"硅采样"模拟人口响应，但对社会敏感问题的回答存在社会期望偏差，与真实人类数据偏离。现有研究对此关注不足，需要探索如何通过提示词设计缓解这种偏差。

Method: 使用美国国家选举研究数据，在两个模型家族（Llama-3.1系列和GPT-4.1-mini）上测试四种提示词缓解方法：重新表述（中性、第三人称）、反向编码（语义反转）、以及两种元指令（启动和前言），分别鼓励分析性和真诚性。使用Jensen-Shannon散度评估与ANES数据的对齐度。

Result: 重新表述的提示词最有效地改善了对齐度，减少了社会可接受答案的分布集中度，使分布更接近ANES数据。反向编码在不同项目上效果不一，而启动和前言方法鼓励了回答的一致性，但对偏差缓解没有系统性益处。

Conclusion: 基于提示词的框架控制可以有效缓解LLM固有的社会期望偏差，为获得更具代表性的人口模拟样本提供了实用路径。
...
### [151] [Data Augmentation for Classification of Negative Pregnancy Outcomes in Imbalanced Data](https://arxiv.org/abs/2512.22732)
*Md Badsha Biswas*

Main category: cs.CL

TL;DR: 利用社交媒体数据（特别是Twitter）构建NLP流水线，自动识别女性分享的怀孕经历，按报告结果分类，为研究负面妊娠结局提供补充数据源


<details>
  <summary>Details</summary>
Motivation: 婴儿死亡率在美国仍是重要公共卫生问题，出生缺陷是主要原因之一。尽管已有研究努力理解流产、死产、出生缺陷和早产等负面妊娠结局的原因，但仍需要更全面的研究和干预策略。传统数据集有限，需要补充数据源来增强研究能力。

Method: 提出新颖方法，利用公开社交媒体数据（特别是Twitter）增强现有数据集。构建自然语言处理（NLP）流水线，自动识别分享怀孕经历的女性，按报告结果分类：报告足月妊娠和正常出生体重的为阳性案例，报告负面妊娠结局的为阴性案例。需要处理社交媒体数据的固有挑战，包括不平衡、噪声和缺乏结构，采用稳健的预处理技术和数据增强策略。

Result: 研究展示了社交媒体数据作为妊娠结局流行病学调查辅助资源的可行性。提供了评估特定干预措施、治疗或产前暴露对母婴健康结果因果影响的潜在应用。为未来涉及孕妇队列和比较组的健康研究提供了框架。

Conclusion: 社交媒体数据可作为研究负面妊娠结局的补充数据源，NLP流水线能有效识别和分类怀孕经历。该方法为流行病学调查提供了新途径，有助于更全面地理解妊娠结局的决定因素和制定干预策略。
...
### [152] [WeDLM: Reconciling Diffusion Language Models with Standard Causal Attention for Fast Inference](https://arxiv.org/abs/2512.22737)
...
<details>
  <summary>Details</summary>
Motivation: 自回归解码的token-by-token特性限制了推理时的并行性，而现有扩散语言模型虽然支持并行解码，但由于依赖双向注意力而无法有效利用前缀KV缓存，导致实际部署速度不如优化的AR引擎

Method: 提出WeDLM框架：1) 完全基于标准因果注意力，使并行生成对前缀缓存友好；2) 通过拓扑重排序技术，将已观测token移动到物理前缀同时保持逻辑位置；3) 采用流式解码过程，持续将置信token提交到增长的前缀中，保持固定并行工作量

Result: WeDLM在保持强AR骨干模型质量的同时，在具有挑战性的推理基准上实现接近3倍加速，在低熵生成场景下达到10倍加速，并且在匹配部署设置下超越了vLLM优化的AR基线
...
### [153] [Fake News Classification in Urdu: A Domain Adaptation Approach for a Low-Resource Language](https://arxiv.org/abs/2512.22778)
*Muhammad Zain Ali,Bernhard Pfahringer,Tony Smith*

Main category: cs.CL

TL;DR: 研究探讨了在乌尔都语假新闻检测中，通过领域自适应预训练提升多语言模型性能的方法，发现XLM-RoBERTa在领域自适应后表现优于原始版本。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的错误信息是一个普遍问题，但乌尔都语等低资源语言在此领域研究有限。多语言预训练模型在特定领域术语上表现不佳，需要领域自适应来提升性能。

Method: 采用分阶段训练方法：首先对XLM-RoBERTa和mBERT进行领域自适应预训练，使用公开的乌尔都语新闻语料库；然后在四个公开的乌尔都语假新闻数据集上进行微调，用于假新闻分类任务。

Result: 实验表明，经过领域自适应的XLM-RoBERTa在所有数据集上均优于原始版本，而领域自适应的mBERT表现结果不一，有混合效果。
...
### [154] [CNSight: Evaluation of Clinical Note Segmentation Tools](https://arxiv.org/abs/2512.22795)
*Risha Surana,Adrian Law,Sunwoo Kim,Rishab Sridhar,Angxiao Han,Peiyu Hong*

Main category: cs.CL

TL;DR: 评估不同方法在临床笔记分割任务上的表现，发现大型API模型（如GPT-5-mini）在整体性能上最佳，轻量级基线方法在结构化任务上仍有竞争力。


<details>
  <summary>Details</summary>
Motivation: 临床笔记通常以非结构化或半结构化格式存储，这限制了其在二次分析和下游临床应用中的使用。可靠识别章节边界是将这些笔记结构化的关键步骤，因为不同章节（如现病史、药物、出院指导）提供不同的临床上下文。

Method: 使用MIMIC-IV中1000份临床笔记的精选数据集，评估了三种方法：1) 基于规则的基线方法；2) 领域特定的Transformer模型；3) 大型语言模型（包括API模型）。评估了句子级和自由文本分割任务。

Result: 大型API模型在整体性能上表现最佳，GPT-5-mini在句子级和自由文本分割上达到平均F1分数72.4。轻量级基线方法在结构化句子级任务上保持竞争力，但在非结构化自由文本任务上表现不佳。
...
### [155] [AutoForge: Automated Environment Synthesis for Agentic Reinforcement Learning](https://arxiv.org/abs/2512.22857)
...
TL;DR: 提出自动化合成高难度但易验证任务的模拟环境管道，以及环境级RL算法，解决用户不稳定性和异质性挑战，提升训练效率


<details>
  <summary>Details</summary>
Motivation: 现有强化学习在模拟环境中的研究存在局限性：要么是半自动环境合成，要么任务难度不足，缺乏广度和深度。同时模拟用户的不稳定性以及环境间的异质性给智能体RL带来挑战

Method: 提出两大部分：(1) 自动化、可扩展的高难度但易验证任务的模拟环境合成管道；(2) 环境级RL算法，有效缓解用户不稳定性，并在环境层面进行优势估计，提升训练效率和稳定性

Result: 在tau-bench、tau2-Bench和VitaBench等智能体基准测试上进行全面评估，验证了所提方法的有效性。深入分析显示该方法具有良好的域外泛化能力

Conclusion: 提出的自动化环境合成管道和环境级RL算法能够有效解决模拟环境中的用户不稳定性和异质性挑战，提升语言智能体在复杂任务中的训练效率和性能
...
### [156] [Diversity or Precision? A Deep Dive into Next Token Prediction](https://arxiv.org/abs/2512.22955)
*Haoyuan Wu,Hai Wang,Jiajia Wu,Jinxiang Ou,Keyao Wang,Weile Chen,Zihao Zheng,Bei Yu*

Main category: cs.CL

TL;DR: 该论文提出了一种将监督学习视为单步强化学习的新视角，通过引入奖励塑造策略来重塑预训练模型的token输出分布，为后续RL训练提供更优的探索空间，最终提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习提升大语言模型推理能力的方法效果很大程度上依赖于预训练模型的token输出分布所定义的探索空间。标准交叉熵损失限制了探索潜力，需要重新审视预训练目标以提供更有利于后续RL训练的探索空间。

Method: 将标准交叉熵损失重新解释为单步episode中的策略梯度优化实例。提出广义预训练目标，将下一token预测视为随机决策过程，引入奖励塑造策略：1) 使用正奖励缩放因子控制对真实token的概率集中度；2) 采用排名感知机制对高排名和低排名负token进行非对称处理。

Result: 研究发现，与直觉相反，施加精度导向的先验分布（而非更高熵的分布）能为RL提供更优越的探索空间，从而最终提升端到端推理性能。

Conclusion: 通过将监督学习框架重新解释为强化学习，并采用奖励塑造策略来重塑预训练分布，可以创建更有利于后续RL训练的探索空间，从而有效提升大语言模型的推理能力。
...
### [157] [Prompt engineering does not universally improve Large Language Model performance across clinical decision-making tasks](https://arxiv.org/abs/2512.22966)
...
Method: 使用36个案例研究，评估三种先进LLM在五种临床决策任务中的表现：鉴别诊断、必要紧急步骤、相关诊断测试、最终诊断和治疗建议；采用两种温度设置，并应用MedPrompt框架的提示工程技术进行改进

Result: 所有模型在不同任务间表现差异显著：最终诊断准确率接近完美，相关诊断测试表现最差，其他任务表现中等；ChatGPT在零温度下表现更好，LIama在默认温度下更强；提示工程不是通用解决方案，仅对基线准确率最低的任务有显著改善

Conclusion: 提示工程的效果高度依赖于模型和具体任务，表明将LLMs整合到医疗保健中需要定制化、上下文感知的策略，而非一刀切的解决方案
...
### [158] [Improving Generalization in LLM Structured Pruning via Function-Aware Neuron Grouping](https://arxiv.org/abs/2512.23014)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能强大但计算和存储成本高。后训练结构化剪枝是高效解决方案，但当少样本校准集不能充分反映预训练数据分布时，现有方法在下游任务上泛化能力有限。

Method: 提出功能感知神经元分组(FANG)框架：1) 基于处理的语义上下文类型将具有相似功能的神经元分组；2) 在每个组内进行重要性估计时，给与神经元组功能角色强相关的token更高权重；3) 保留跨多种上下文类型贡献的神经元；4) 根据每个块的功能复杂度自适应分配稀疏度。

Result: FANG在保持语言建模性能的同时提升了下游任务准确率。与FLAP和OBC两种代表性剪枝方法结合时达到SOTA结果，在30%和40%稀疏度下平均准确率分别提升1.5%-8.5%。

Conclusion: FANG通过功能感知的神经元分组有效缓解了校准偏差问题，实现了稀疏度与性能之间的更好权衡，为大语言模型的高效部署提供了有效解决方案。
...
### [159] [LENS: LLM-Enabled Narrative Synthesis for Mental Health by Aligning Multimodal Sensing with Language Models](https://arxiv.org/abs/2512.23025)
...
TL;DR: LENS框架将多模态健康感知数据与语言模型对齐，生成临床基础的心理健康叙事，通过将生态瞬时评估转化为自然语言描述，训练补丁级编码器处理原始传感器信号，在症状严重性准确度上优于基线方法。
...
Method: 1) 构建大规模数据集：将抑郁和焦虑症状相关的生态瞬时评估(EMA)响应转化为自然语言描述，从258名参与者中获得超过10万对传感器-文本问答对；2) 训练补丁级编码器：将原始传感器信号直接投影到LLM的表示空间，实现原生时间序列集成。

Result: LENS在标准NLP指标和症状严重性准确度的任务特定指标上优于强基线方法。13名心理健康专业人士的用户研究表明，LENS生成的叙事全面且具有临床意义。
...
### [160] [Is Chain-of-Thought Really Not Explainability? Chain-of-Thought Can Be Faithful without Hint Verbalization](https://arxiv.org/abs/2512.23032)
*Kerem Zaman,Shashank Srivastava*

Main category: cs.CL

TL;DR: 本文批评了Biasing Features指标将思维链标记为"不忠实"的做法，认为该指标混淆了不忠实性和不完整性，后者是将分布式Transformer计算转化为线性自然语言叙述所需的损失压缩。


<details>
  <summary>Details</summary>
Motivation: 当前使用Biasing Features指标的研究将思维链标记为不忠实，如果它省略了影响预测的提示注入提示。作者认为这种评估方法存在问题，需要更全面的解释性工具包。

Method: 1) 在Llama-3和Gemma-3上进行多跳推理任务实验；2) 引入新的faithful@k指标；3) 使用因果中介分析研究非语言化提示如何通过思维链因果中介预测变化；4) 比较不同指标对思维链忠实性的评估。

Result: 1) 许多被Biasing Features标记为不忠实的思维链被其他指标判断为忠实，在某些模型中超过50%；2) 更大的推理时token预算显著提高提示语言化率（某些设置中达90%）；3) 即使非语言化的提示也能通过思维链因果中介预测变化。

Conclusion: 不应仅依赖基于提示的评估，而应采用更广泛的解释性工具包，包括因果中介分析和基于破坏的指标，因为许多表面上的不忠实性实际上是由于严格的token限制造成的。
...
### [161] [Accelerating Language Model Workflows with Prompt Choreography](https://arxiv.org/abs/2512.23049)
*TJ Bai,Jason Eisner*

Main category: cs.CL

TL;DR: Prompt Choreography框架通过动态全局KV缓存优化多智能体LLM工作流，支持消息任意子集重排序和并行调用，显著降低延迟并提升端到端速度


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多智能体工作流中的部署增加，现有方法存在大量冗余计算，导致延迟高、效率低，需要一种能重用已编码消息的优化框架

Method: 提出Prompt Choreography框架，维护动态全局KV缓存，支持LLM调用关注任意重排序的先前消息子集，通过微调LLM适应缓存机制来模拟原始结果

Result: 显著降低每消息延迟（首token生成时间加速2.0-6.2倍），在冗余计算为主的工作流中实现大于2.2倍的端到端加速

Conclusion: Prompt Choreography通过高效的缓存机制有效优化多智能体LLM工作流性能，为实际部署提供了实用的加速解决方案
...
### [162] [TabiBERT: A Large-Scale ModernBERT Foundation Model and Unified Benchmarking Framework for Turkish](https://arxiv.org/abs/2512.23065)
*Melikşah Türker,A. Ebrar Kızıloğlu,Onur Güngör,Susan Üsküdarlı*

Main category: cs.CL

TL;DR: TabiBERT是基于ModernBERT架构的土耳其语单语编码器，在840亿token的多领域语料上从头训练，支持8192token上下文，推理速度提升2.65倍，在土耳其语NLP基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 尽管BERT等编码器在计算效率、训练稳定性和长上下文建模方面有显著进步，但土耳其语NLP领域缺乏采用现代架构范式从头训练的单语编码器模型。

Method: 基于ModernBERT架构，整合RoPE位置编码、FlashAttention和优化归一化，在840亿token的多领域土耳其语语料（网页文本73%、科学文献20%、源代码6%、数学内容0.3%）上从头训练，支持8192token上下文长度。

Result: TabiBERT在TabiBench基准测试（28个数据集，8个任务类别）上获得77.58分，比BERTurk高1.62分，在5个类别中达到SOTA：问答（+9.55）、代码检索（+2.41）、文档检索（+0.60），相比任务特定最佳结果平均提升1.47分。

Conclusion: TabiBERT填补了土耳其语NLP领域现代单语编码器的空白，展现了强大的跨领域泛化能力，为土耳其语编码器研究提供了透明可复现的基础。
...
### [163] [Reservoir Computing inspired Matrix Multiplication-free Language Model](https://arxiv.org/abs/2512.23145)
*Takumi Shiratsuchi,Yuichiro Tanaka,Hakaru Tamukoh*

Main category: cs.CL

TL;DR: 提出一种基于储层计算的无矩阵乘法语言模型，通过部分固定权重和插入储层层来减少训练成本，同时结合多种操作减少内存访问，在保持性能的同时显著降低参数数量和计算时间。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言处理中取得了最先进的性能，但其高计算成本仍然是主要瓶颈。本研究旨在通过设计无矩阵乘法的语言模型来提升计算效率，并进一步通过储层计算架构减少训练成本。

Method: 1) 采用无矩阵乘法的语言模型架构；2) 部分固定和共享选定层的权重；3) 插入储层层以获得丰富的动态表示而不增加训练开销；4) 结合多种操作减少内存访问。

Result: 实验结果表明，所提出的架构将参数数量减少高达19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过结合无矩阵乘法架构和储层计算原理，可以显著降低语言模型的计算成本，同时保持性能，为解决大语言模型的高计算成本瓶颈提供了有效途径。
...
### [164] [Not too long do read: Evaluating LLM-generated extreme scientific summaries](https://arxiv.org/abs/2512.23206)
*Zhuoqi Lyu,Qing Ke*

Main category: cs.CL

TL;DR: 该研究提出了BiomedTLDR数据集，包含大量研究人员撰写的科学论文摘要，用于评估LLM生成科学摘要的能力。研究发现，虽然部分LLM能生成类似人类的摘要，但总体上LLM更倾向于使用原文的词汇和修辞结构，相比人类更偏向提取式而非抽象式摘要。


<details>
  <summary>Details</summary>
Motivation: 高质量的科学摘要（TLDR）有助于有效的科学传播，但目前缺乏全面、高质量的科学TLDR数据集，这阻碍了LLM摘要能力的开发和评估。研究旨在了解LLM在生成科学摘要方面的表现，以及与人类专家撰写的摘要有何差异。

Method: 1. 提出BiomedTLDR数据集，利用参考文献中常见的作者评论实践，收集大量研究人员撰写的科学论文摘要；2. 测试流行的开源LLM基于摘要生成TLDR的能力；3. 分析LLM生成摘要与人类撰写摘要的差异。

Result: 研究发现，虽然部分LLM能成功生成类似人类的摘要，但总体上LLM表现出对原文词汇选择和修辞结构的更大亲和力，因此相比人类更倾向于提取式而非抽象式摘要。

Conclusion: 该研究提供了BiomedTLDR数据集来支持LLM科学摘要能力的评估，揭示了LLM在生成科学摘要时与人类的差异，为未来改进LLM的摘要生成能力提供了基础。
...
### [165] [Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process](https://arxiv.org/abs/2512.23213)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决如何从多个大语言模型生成的候选回答中选择最理想响应的问题，利用多个模型的不同优势，通过集体智慧提升回答质量。

Method: 基于同行评审框架的三阶段方法：1) 使用LLM-as-a-Judge技术让多个LLM相互评分；2) 通过图模型真理推断算法或简单平均策略聚合多个评分；3) 选择最高分响应作为最佳集成输出。

Result: 在四个数据集上取得强劲结果，两个变体分别比先进的Smoothie-Global模型高出6.9%和7.3%。

Conclusion: LLM-PeerReview是一种概念简单但经验上强大的无监督集成方法，通过同行评审机制有效利用多个LLM的集体智慧，具有良好的适应性和泛化能力。
...
### [166] [Anka: A Domain-Specific Language for Reliable LLM Code Generation](https://arxiv.org/abs/2512.23214)
*Saif Khalfan Saif Al Mazrouei*

Main category: cs.CL

TL;DR: LLMs在复杂编程任务中易出错，作者提出专用领域语言Anka，通过约束语法显著提升代码生成准确率，在未训练的情况下达到95.8%任务准确率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面表现出色，但在复杂多步骤编程任务中存在系统性错误。作者认为这些错误源于通用语言的灵活性，允许多种有效方法并需要隐式状态管理。

Method: 引入Anka，一种用于数据转换管道的领域特定语言，具有明确、约束的语法设计，减少代码生成中的歧义。在100个基准问题上评估Claude 3.5 Haiku和GPT-4o-mini的性能。

Result: Claude 3.5 Haiku在Anka上达到99.9%解析成功率和95.8%总体任务准确率。在多步骤管道任务中，Anka比Python有40个百分点的准确率优势（100% vs 60%）。GPT-4o-mini也确认了这一优势。
...
### [167] [Interpretable Safety Alignment via SAE-Constructed Low-Rank Subspace Adaptation](https://arxiv.org/abs/2512.23260)
...
Method: 利用预训练的稀疏自编码器在解耦的特征空间中识别任务相关特征，然后构建明确、可解释的低秩子空间来指导适配器初始化。理论分析证明在单义性假设下，SAE基子空间识别可以实现任意小的恢复误差。

Result: 在安全对齐任务上，该方法达到99.6%的安全率，超过全参数微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19-0.24%的参数。方法通过SAE特征的语义基础提供了对学习对齐子空间的可解释性洞察。

Conclusion: 将机制可解释性融入微调过程可以同时提高性能和透明度。该方法为参数高效微调提供了新的可解释性框架，展示了可解释AI在实践应用中的价值。
...
### [168] [Chinese Morph Resolution in E-commerce Live Streaming Scenarios](https://arxiv.org/abs/2512.23280)
*Jiahao Zhu,Jipeng Qiang,Ran Bai,Chenyu Liu,Xiaoye Ouyang*

Main category: cs.CL

TL;DR: 该研究针对中国电商直播中主播使用语音变形词规避监管的问题，提出了LiveAMR任务来检测健康医疗直播中的发音变形违规行为，构建了首个相关数据集并开发了基于LLM的文本生成解决方案。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播（特别是抖音平台）已成为重要销售渠道，但主播经常使用语音变形词来逃避审查并进行虚假广告宣传。现有研究主要关注社交媒体和地下产业的文本变形规避，而缺乏针对直播中发音变形的检测方法。

Method: 研究提出了LiveAMR（直播听觉变形解析）任务，构建了包含86,790个样本的首个LiveAMR数据集。将任务转化为文本到文本生成问题，利用大语言模型生成额外训练数据来提升性能。

Result: 通过LLM生成额外训练数据的方法显著提升了模型性能，证明了语音变形解析能有效增强直播监管效果。

Conclusion: LiveAMR任务能够有效检测直播中的语音变形违规行为，为直播监管提供了重要技术支撑，特别是在健康医疗领域的虚假广告识别方面具有重要应用价值。
...
### [169] [AI4Reading: Chinese Audiobook Interpretation System Based on Multi-Agent Collaboration](https://arxiv.org/abs/2512.23300)
...
<details>
  <summary>Details</summary>
Motivation: 有声书解读虽然能为读者提供实用见解和智力启发，但其人工制作过程耗时且资源密集。为了解决这一挑战，需要开发自动化系统来生成高质量的音频解读内容。

Method: 提出AI4Reading多智能体协作系统，包含11个专门化智能体（主题分析师、案例分析师、编辑、叙述者、校对者等），利用大语言模型和语音合成技术，共同探索主题、提取现实案例、优化内容组织并合成自然口语。

Result: 与专家解读相比，AI4Reading生成的解读脚本更简洁准确，但在语音生成质量方面仍存在差距。
...
### [170] [AI Meets Brain: Memory Systems from Cognitive Neuroscience to Autonomous Agents](https://arxiv.org/abs/2512.23343)
...
<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究在借鉴认知神经科学设计高效记忆工作流程时，受限于学科壁垒，难以吸收人类记忆机制的精髓。为了弥合这一差距，需要系统性地综合记忆的跨学科知识。

Method: 首先从认知神经科学到LLM再到智能体的渐进轨迹阐明记忆的定义和功能；然后从生物和人工角度对记忆分类、存储机制和完整管理生命周期进行比较分析；接着回顾评估智能体记忆的主流基准；最后从攻击和防御双重角度探讨记忆安全性。

Result: 论文提供了记忆的跨学科综合框架，建立了认知神经科学与LLM驱动智能体之间的连接，系统分析了记忆的各个方面，并提出了评估基准和安全考虑。

Conclusion: 该研究为理解智能体记忆提供了系统性框架，未来研究方向应重点关注多模态记忆系统和技能获取，推动自主智能体记忆系统的发展。
...
### [171] [A Stepwise-Enhanced Reasoning Framework for Large Language Models Based on External Subgraph Generation](https://arxiv.org/abs/2512.23356)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多种NLP任务中表现出色，但在需要深度推理和逻辑推断的复杂场景中仍面临挑战。模型可能从大规模文本语料中学习到噪声或无关信息，导致预测错误或与事实知识不一致的输出。

Method: 提出SGR框架：1) 从外部知识库动态构建与查询相关的子图；2) 利用子图的语义结构指导模型进行多步推理；3) 集成多个推理路径生成最终答案。通过结构化子图上的逐步推理减少噪声信息影响。

Result: 在多个基准数据集上的实验结果表明，SGR框架持续优于强基线方法，证明了其在增强大语言模型推理能力方面的有效性。

Conclusion: 基于外部子图生成的逐步推理框架SGR能够有效提升大语言模型在复杂推理任务中的性能，通过结构化知识引导减少噪声影响，提高推理准确性。
...
### [172] [Entropy-Guided Token Dropout: Training Autoregressive Language Models with Limited Domain Data](https://arxiv.org/abs/2512.23422)
*Jiapeng Wang,Yiwen Hu,Yanzipeng Gao,Haoyu Wang,Shuo Wang,Hongyu Lu,Jiaxin Mao,Wayne Xin Zhao,Junyi Li,Xiao Zhang*

Main category: cs.CL

TL;DR: 论文提出EntroDrop方法，通过基于熵的token丢弃来缓解多轮训练中自回归模型性能下降的问题，在数据受限场景下提升LLM适应能力。


<details>
  <summary>Details</summary>
Motivation: 随着高质量领域特定数据日益稀缺，多轮训练成为适应大语言模型的实用策略。然而，自回归模型在重复数据暴露下常出现性能下降，过拟合导致模型能力显著衰退。研究发现这种退化源于学习动态不平衡：可预测的低熵token学习过快并主导优化，而模型在高熵token上的泛化能力随持续训练而恶化。

Method: 提出EntroDrop方法，一种基于熵引导的token丢弃技术，作为结构化数据正则化。该方法在训练期间选择性屏蔽低熵token，并采用课程调度来根据训练进度调整正则化强度。

Result: 在0.6B到8B参数规模的模型上进行实验，EntroDrop始终优于标准正则化基线，并在扩展的多轮训练中保持稳健性能。

Conclusion: 研究强调了在有限数据训练时，将正则化与token级学习动态对齐的重要性。EntroDrop方法为数据受限领域中更有效的LLM适应提供了有前景的途径。
...
### [173] [The Effect of Gender Diversity on Scientific Team Impact: A Team Roles Perspective](https://arxiv.org/abs/2512.23429)
*Yi Zhao,Yongjun Zhu,Donghun Kim,Yuzhuo Wang,Heng Zhang,Chao Lu,Chengzhi Zhang*

Main category: cs.CL

TL;DR: 性别多样性对科研团队影响力的影响呈现倒U型关系，领导团队全女性、支持团队全男性的组合表现最佳，团队规模对领导团队性别多样性影响有调节作用


<details>
  <summary>Details</summary>
Motivation: 现有关于性别多样性对科研团队成功影响的研究结果不一致，且大多采用聚合指标，忽略了团队内部角色分化。这种局限性阻碍了对性别多样性如何塑造团队影响力的更细致理解，特别是不同团队角色中性别多样性的影响仍不清楚。

Method: 将科研团队定义为论文的所有合著者，通过五年引用次数衡量团队影响力。利用作者贡献声明将成员分为领导角色和支持角色。基于PLOS期刊超过13万篇论文（主要为生物医学领域），采用多元回归分析这些角色中性别多样性与团队影响力的关联。同时应用阈值回归模型研究团队规模如何调节这种关系。

Result: 1) 性别多样性与团队影响力的关系在领导组和支持组都呈现倒U型；2) 领导团队全女性、支持团队全男性的团队比其他类型团队获得更高影响力；3) 领导组性别多样性的影响在小团队中显著为负，但在大团队中变为正且统计上不显著。相比之下，支持组性别多样性的影响在所有团队规模中都保持显著为正。

Conclusion: 性别多样性对科研团队影响力的影响是复杂的，取决于团队角色和团队规模。领导角色和支持角色中的性别多样性对团队成功有不同的影响机制，团队规模对领导角色的性别多样性效应有重要调节作用。研究强调了在分析团队多样性时考虑内部角色分化的重要性。
...
### [174] [Semantic Tree Inference on Text Corpa using a Nested Density Approach together with Large Language Model Embeddings](https://arxiv.org/abs/2512.23471)
*Thomas Haschka,Joseph Bakarji*

Main category: cs.CL

TL;DR: 提出了一种基于嵌套密度聚类的方法，通过逐步放宽密度标准构建层次树，揭示文本语料库中的语义关系结构


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型嵌入常用于基于语义相似性的文本存储和检索，但文本语料库中的全局语义关系结构往往不透明，需要一种方法来发现数据驱动的层次语义关系

Method: 采用嵌套密度聚类方法，在LLM嵌入空间中搜索密集簇，逐步放宽密度标准使密集簇合并为更分散的簇，最终构建捕获文本间层次语义关系的树结构

Result: 该方法成功应用于科学摘要分类，实现了无需预定义类别的数据驱动研究领域发现，并在20 Newsgroups和IMDB 50k Movie Reviews等基准数据集上验证了跨领域的鲁棒性
...
### [175] [Automatic Detection of Complex Quotation Patterns in Aggadic Literature](https://arxiv.org/abs/2512.23504)
*Hadar Miller,Tsvi Kuflik,Moshe Lavee*

Main category: cs.CL

TL;DR: ACT算法通过三阶段流程自动检测拉比文献中的圣经引文，结合形态对齐和上下文增强，在F1分数0.91上超越现有系统，并能识别复杂引用模式。


<details>
  <summary>Details</summary>
Motivation: 现有文本重用框架难以处理拉比文献中简短、意译或结构嵌入的圣经引文，需要开发专门方法来解决机器检测与人工编辑判断之间的方法学差距。

Method: ACT采用三阶段算法：1) 形态感知对齐算法；2) 上下文敏感增强阶段，识别"波浪"和"回声"等复杂引用模式；3) 评估了三种配置以分析各组件贡献。

Result: 完整ACT管道(ACT-QE)在F1分数0.91上优于所有基线系统，召回率0.89，精确率0.94。ACT-2(无风格增强)召回率更高(0.90)但精确率较低，ACT-3使用更长n-gram在覆盖率和特异性间取得平衡。

Conclusion: ACT不仅改进了引文检测，其跨语料库风格模式分类能力为体裁分类和互文分析开辟了新途径，为形态丰富、引用密集的传统文献分析奠定了基础。
...
### [176] [UniHetero: Could Generation Enhance Understanding for Vision-Language-Model at Large Data Scale?](https://arxiv.org/abs/2512.23512)
*Fengjiao Chen,Minhao Jing,Weitao Lu,Yan Feng,Xiaoyu Li,Xuezhi Cao*

Main category: cs.CL

TL;DR: 视觉语言大模型正朝着视觉理解和生成任务的统一发展，但生成是否能增强理解在大数据规模下仍待探索。研究发现：生成语义而非像素能提升理解能力；生成显示更好的数据扩展趋势和更高的数据利用率；输入嵌入的自回归能有效捕捉视觉细节。


<details>
  <summary>Details</summary>
Motivation: 探索在视觉语言大模型中，视觉生成任务是否能够增强视觉理解能力，特别是在大规模预训练（超过2亿样本）的背景下。当前研究缺乏在大数据规模下对生成与理解关系的研究。

Method: 提出了UniHetero统一模型，采用简洁的结构，在大规模预训练（>200M样本）下进行分析。主要方法包括：1）生成语义而非像素；2）使用输入嵌入的自回归来捕捉视觉细节；3）在大规模数据上评估生成对理解的影响。

Result: 三个关键发现：1）生成能够提升理解能力，但前提是生成语义内容而非像素；2）生成显示出更优的数据扩展趋势和更高的数据利用率；3）输入嵌入的自回归能有效捕捉视觉细节。

Conclusion: 在大规模预训练下，视觉生成任务确实能够增强视觉理解能力，特别是当生成语义内容时。这为视觉语言大模型的统一架构提供了重要指导，表明生成与理解的协同训练具有实际价值。
...
### [177] [Single LLM Debate, MoLaCE: Mixture of Latent Concept Experts Against Confirmation Bias](https://arxiv.org/abs/2512.23518)
*Hazel Kim,Philip Torr*

Main category: cs.CL

TL;DR: MoLaCE是一个轻量级推理时框架，通过混合潜在概念专家来减少LLMs中的确认偏见，无需额外训练，计算效率高。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在严重的输入确认偏见问题，当提示暗示偏好答案时，模型会强化偏见而非探索替代方案。这种现象在基础模型中已造成危害，在多智能体辩论中风险更大，可能形成回声室效应而非纠正偏见。

Method: 提出MoLaCE框架，将专家实例化为不同潜在概念上的激活强度混合。核心洞察是：由于语言组合性，不同措辞的提示会以特定方式重新加权潜在概念，影响事实正确性，因此不能对所有输入应用单一固定干预。

Result: MoLaCE能持续减少确认偏见，提高鲁棒性，性能匹配或超越多智能体辩论，同时仅需少量计算资源。也可集成到多智能体辩论框架中以多样化视角并减少相关错误。

Conclusion: MoLaCE使单个LLM能够在内部模拟辩论优势，保持计算效率和可扩展性，为解决LLMs中的确认偏见问题提供了有效方案。
...
### [178] [Lie to Me: Knowledge Graphs for Robust Hallucination Self-Detection in LLMs](https://arxiv.org/abs/2512.23547)
*Sahil Kale,Antonio Luca Alfeo*

Main category: cs.CL

TL;DR: 本文提出了一种基于知识图谱的幻觉自检测方法，通过将LLM响应转换为实体关系图来估计幻觉可能性，相比现有方法在准确率和F1分数上分别提升了16%和20%。


<details>
  <summary>Details</summary>
Motivation: 幻觉（生成看似可信但虚假的陈述）是LLM安全部署的主要障碍。基于自检测方法的良好表现，本文探索使用结构化知识表示（知识图谱）来改进幻觉自检测。

Method: 提出简单而强大的方法：(1) 将LLM响应转换为实体和关系的知识图谱；(2) 使用这些图谱估计响应包含幻觉的可能性。该方法低成本、模型无关，在GPT-4o和Gemini-2.5-Flash上进行了评估。

Result: 相比标准自检测方法和最先进的SelfCheckGPT方法，该方法在准确率上相对提升16%，F1分数提升20%。LLM在分析原子事实时，即使初始输出包含不准确信息，也能更好地处理结构化知识图谱。

Conclusion: 基于知识图谱的幻觉自检测方法为更安全、更可信的语言模型铺平了道路。该方法展示了LLM在结构化知识表示下能更好地分析事实，即使面对初始不准确信息。
...
### [179] [Instruction-Following Evaluation of Large Vision-Language Models](https://arxiv.org/abs/2512.23572)
*Daiki Shiono,Shumpei Miyawaki,Ryota Tanaka,Jun Suzuki*

Main category: cs.CL

TL;DR: 大型视觉语言模型在视觉指令微调后，指令跟随能力下降，但包含输出格式说明的训练数据可以缓解这一问题


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在集成视觉能力并进行视觉指令微调后，往往失去了原本语言模型具备的指令跟随能力，导致模型无法按照预期执行任务指令

Method: 构建新的训练数据集，突出显示输出格式是否被指定，研究在微调过程中明确指示输出格式如何影响LVLMs的指令跟随能力

Result: 定量评估证实LVLMs在常用数据集微调后指令跟随能力确实下降；包含输出格式说明指令的训练数据集训练的模型比不包含的模型更能准确遵循指令
...
### [180] [Style Amnesia: Investigating Speaking Style Degradation and Mitigation in Multi-Turn Spoken Language Models](https://arxiv.org/abs/2512.23578)
*Yu-Xiang Lin,Cheng-Han Chiang,Hung-yi Lee*

Main category: cs.CL

TL;DR: 研究发现语音语言模型在多轮对话中无法保持指定的说话风格（如情感、口音、音量、语速），存在"风格遗忘"问题，且系统提示效果不如用户提示。


<details>
  <summary>Details</summary>
Motivation: 语音语言模型在对话中指定特定说话风格后，经过几轮交互就无法保持该风格，这种"风格遗忘"现象影响用户体验和模型实用性，需要系统研究。

Method: 评估了3个专有模型和2个开源模型，关注副语言风格（情感、口音、音量、语速）。通过多轮对话测试，分析模型在系统提示和用户提示下的表现，并测试显式要求回忆风格指令的效果。

Result: 所有测试模型都无法在指令下保持一致的说话风格；模型能回忆风格指令但无法表达；显式要求回忆可部分缓解风格遗忘；系统提示效果不如用户提示，与预期功能相矛盾。

Conclusion: 语音语言模型存在严重的风格遗忘问题，需要改进模型架构或训练方法以保持长期风格一致性，系统提示的设计也需要重新考虑。
...
### [181] [Close the Loop: Synthesizing Infinite Tool-Use Data via Multi-Agent Role-Playing](https://arxiv.org/abs/2512.23611)
*Yuwen Li,Wei Zhang,Zelong Huang,Mason Yang,Jiajun Wu,Shawn Guo,Huahao Hu,Lingyi Sun,Jian Yang,Mingjie Tang,Byran Dai*

Main category: cs.CL

TL;DR: InfTool是一个完全自主的框架，通过自演化的多智能体合成，使大型语言模型能够可靠地调用外部工具，无需人工标注，在BFCL基准上实现了从19.8%到70.9%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 当前LLM调用外部工具的方法存在三个根本挑战：高质量轨迹需要昂贵的人工标注、对未见工具的泛化能力差、以及单模型合成固有的质量上限导致偏见和覆盖缺口。需要一种完全自主的解决方案来突破这些障碍。

Method: InfTool采用自演化的多智能体合成框架，包含三个协作智能体：用户模拟器、工具调用助手和MCP服务器。仅基于原始API规范，生成从单次调用到复杂多步工作流的多样化验证轨迹。通过闭环循环：合成数据通过带门控奖励的GRPO训练模型，改进后的模型生成更高质量数据针对能力缺口，无需人工干预。

Result: 在伯克利函数调用排行榜(BFCL)上，InfTool将基础32B模型的准确率从19.8%提升至70.9%（+258%），超越了10倍大小的模型，与Claude-Opus相媲美，且完全基于合成数据无需人工标注。

Conclusion: InfTool通过完全自主的自演化多智能体合成框架，成功解决了LLM可靠调用外部工具的关键瓶颈，实现了显著的性能提升，展示了合成数据驱动的模型改进潜力。
...
### [182] [Less is more: Probabilistic reduction is best explained by small-scale predictability measures](https://arxiv.org/abs/2512.23659)
...
<details>
  <summary>Details</summary>
Motivation: 探索在语言模型概率与认知现象关系研究中，确定必要和适当的上下文量，特别是验证是否必须使用完整话语才能观察到概率缩减现象

Method: 通过比较完整话语与n-gram表示在语言模型概率与认知现象关系中的表现，分析不同上下文量对观察概率缩减现象的影响

Result: 研究发现n-gram表示足以作为认知规划单元，无需完整话语即可观察到概率缩减现象

Conclusion: 在语言模型概率与认知现象关系研究中，n-gram表示是足够的认知单元，简化了研究所需的上下文量要求
...
### [183] [Multilingual Hidden Prompt Injection Attacks on LLM-Based Academic Reviewing](https://arxiv.org/abs/2512.23684)
...
Method: 构建包含约500篇ICML接受的真实学术论文数据集，在文档中嵌入四种不同语言的语义等效隐藏对抗提示，使用LLM进行评审

Result: 英语、日语和中文的提示注入显著改变评审分数和接受/拒绝决策，而阿拉伯语注入几乎没有效果，显示不同语言脆弱性存在显著差异

Conclusion: LLM基础的评审系统易受文档级提示注入攻击，且不同语言攻击效果差异显著，需加强安全防护
...
### [184] [PROFASR-BENCH: A Benchmark for Context-Conditioned ASR in High-Stakes Professional Speech](https://arxiv.org/abs/2512.23686)
*Deepak Babu Piskala*
...
<details>
  <summary>Details</summary>
Motivation: 现有ASR基准测试未能充分体现专业场景的挑战：密集的领域术语、正式语体变化以及对关键实体错误的零容忍。需要专门评估专业场景下的语音识别性能。

Method: 创建ProfASR-Bench评估套件，包含自然语言提示（领域线索和/或说话者档案）与实体丰富的目标话语配对。在无上下文、档案、领域+档案、oracle和对抗性条件下测试Whisper和Qwen-Omni模型，使用传统ASR指标和实体感知评分。

Result: 发现一致的上下文利用差距(CUG)：轻量级文本上下文对平均词错误率(WER)几乎没有影响，即使使用oracle提示；对抗性提示也不能可靠地降低性能。系统名义上支持提示但未能充分利用可用侧信息。

Conclusion: ProfASR-Bench提供了标准化的上下文阶梯、实体和切片感知报告以及可复现的测试平台，用于比较不同模型系列的融合策略，揭示了当前ASR系统在专业场景中上下文利用不足的问题。
...
### [185] [Fine-Tuning LLMs with Fine-Grained Human Feedback on Text Spans](https://arxiv.org/abs/2512.23693)
*Sky CH-Wang,Justin Svegliato,Helen Appel,Jason Eisner*

Main category: cs.CL

TL;DR: 提出基于反馈驱动改进链的偏好监督方法，通过细粒度标注和逐步重写构建偏好对，优于传统A/B偏好排序方法


<details>
  <summary>Details</summary>
Motivation: 传统偏好对齐方法（如标准A/B偏好排序或完整对比重写）效率较低，需要更结构化、基于修订的监督方法来提高偏好调优效果

Method: 1）标注者提供细粒度反馈，标记"喜欢"和"不喜欢"的文本片段；2）基础模型从左到右重写不喜欢的片段，形成增量改进链；3）从链中相邻步骤构建偏好对进行直接对齐

Result: 该方法优于基于标准A/B偏好排序或完整对比重写的直接对齐方法，表明结构化、基于修订的监督能带来更高效有效的偏好调优
...
### [186] [Eliciting Behaviors in Multi-Turn Conversations](https://arxiv.org/abs/2512.23701)
*Jing Huang,Shujian Zhang,Lun Wang,Andrew Hard,Rajiv Mathews,John Lambert*

Main category: cs.CL

TL;DR: 该研究探讨了在多轮对话中识别大型语言模型特定行为的方法，提出了一个分析框架将现有方法分为三类，并评估了它们在自动生成多轮测试用例中的效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单轮对话中诱导特定行为的方法，但在实际应用中，多轮对话更为常见且复杂。需要开发能够在多轮对话环境中有效识别和评估LLM行为的方法。

Method: 提出了一个分析框架，将现有方法分为三类：仅使用先验知识的方法、使用离线交互的方法、以及从在线交互中学习的方法。引入了广义的多轮在线方法，统一了单轮和多轮行为诱导。评估了这三类方法在自动生成多轮测试用例中的效果。

Result: 在线方法在三个任务中仅用几千次查询就能达到平均45/19/77%的成功率，而现有的多轮对话基准测试中的静态方法很少甚至找不到失败案例。这表明在线方法在多轮对话评估中具有显著优势。

Conclusion: 该研究展示了行为诱导方法在多轮对话评估中的新应用，并强调社区需要从静态基准测试转向动态基准测试，以更有效地评估LLM在复杂对话环境中的行为。
...
### [187] [Optimal Beamforming Design for Multi-user MIMO Near-Field ISAC Systems with Movable Antennas](https://arxiv.org/abs/2512.22620)
*Nemanja Stefan Perović,Keshav Singh,Chih-Peng Li,Octavia A. Dobre,Mark F. Flanagan*

Main category: cs.IT

TL;DR: 本文研究了近场场景下基于可移动天线的集成感知与通信系统，通过优化通信预编码、感知波束成形和天线位置，最大化加权和速率同时满足最小感知需求。


<details>
  <summary>Details</summary>
Motivation: 虽然可移动天线在远场ISAC系统中的性能增益已有研究，但在近场场景下的增益几乎未被探索。本文旨在研究近场ISAC系统中可移动天线的性能优势。

Method: 提出交替优化算法，优化通信预编码矩阵、感知发射波束成形器、感知接收合并器、用户可移动天线位置和基站发射可移动天线位置。考虑了线性预编码和迫零预编码两种方案。

Result: 仿真结果表明，在近场ISAC系统中使用可移动天线相比固定天线系统具有显著性能优势。线性预编码方案在不平等用户权重下获得更大的加权和速率，而迫零预编码方案对所有用户权重保持近似恒定的加权和速率。

Conclusion: 可移动天线能显著提升近场ISAC系统性能，加权和速率高度依赖于不同用户天线间的干扰，感知性能比通信性能更受最小感知信干噪比阈值的影响。
...
### [188] [An Improved Lower Bound on Cardinality of Support of the Amplitude-Constrained AWGN Channel](https://arxiv.org/abs/2512.22691)
*Haiyang Wang,Luca Barletta,Alex Dytso*

Main category: cs.IT

TL;DR: 该论文研究了幅度受限加性高斯白噪声信道，证明了容量最优输入分布的支撑集大小下界为A√log A，推翻了之前关于线性增长的猜想。


<details>
  <summary>Details</summary>
Motivation: 研究幅度受限加性高斯白噪声信道容量最优输入分布的支撑集大小。已知该分布是离散的且具有有限支撑点，现有上下界分别为A和A²数量级，文献[1]猜想线性增长是最优的，本文旨在验证或推翻这一猜想。

Method: 首先量化容量最优输出分布在幅度约束内部接近均匀分布的事实；然后引入包装操作将问题映射到紧致域；接着发展有限高斯混合对均匀分布的最佳逼近理论；最后结合容量最优分布的稳定性性质得到最终支撑集大小下界。

Result: 建立了新的下界A√log A，改进了已知下界，并推翻了线性增长最优的猜想。

Conclusion: 幅度受限加性高斯白噪声信道容量最优输入分布的支撑集大小至少以A√log A的速度增长，线性增长猜想不成立，为理解这类信道的最优输入分布结构提供了新的理论洞见。
...
### [189] [Iterative Channel Estimation, Detection and Decoding for Multi-Antenna Systems with RIS](https://arxiv.org/abs/2512.22731)
*Roberto C. G. Porto,Rodrigo C. de Lamare*

Main category: cs.IT

TL;DR: 本文提出了一种用于多用户多天线系统上行链路的迭代信道估计、检测和解码方案，该系统由多个可重构智能表面辅助。开发了新颖的迭代码辅助信道估计技术，利用LDPC码和迭代处理提高估计精度，同时减少导频开销。


<details>
  <summary>Details</summary>
Motivation: 在多用户多天线系统中，特别是在可重构智能表面辅助的场景下，需要高效的信道估计方法来应对复杂的传播环境。传统方法面临导频开销大、估计精度有限的问题，特别是在非稀疏传播条件下。

Method: 提出了迭代码辅助信道估计技术，使用编码导频，利用导频和奇偶校验位迭代优化信道估计。还提出了迭代信道跟踪方法，利用信道的时间相关性。方案结合了LDPC码和迭代处理，在子6GHz多RIS场景下进行验证。

Result: 提供了估计器的归一化均方误差分析，研究了计算复杂度和码率影响。数值结果验证了该方案在LOS和NLOS条件下、不同RIS架构下的性能，表明在减少导频开销的同时提高了估计精度。

Conclusion: 提出的ICEDD方案通过迭代码辅助信道估计和信道跟踪，有效解决了多RIS辅助系统中的信道估计问题，在复杂传播环境下实现了性能提升和导频开销的平衡。
...
### [190] [Beyond Beam Sweeping: One-Shot Satellite Acquisition with Doppler-Aware Rainbow Beamforming](https://arxiv.org/abs/2512.22828)
*Juha Park,Ian P. Roberts,Wonjae Shin*

Main category: cs.IT

TL;DR: 提出一种利用多普勒效应和波束斜视效应的一步式LEO卫星捕获框架，通过彩虹波束形成器实现单次传输即可覆盖全角度域，无需传统波束扫描。


<details>
  <summary>Details</summary>
Motivation: 传统LEO卫星通信需要高增益波束形成来克服路径损耗，但需要精确卫星位置信息。传统时域波束扫描方法存在显著开销和延迟问题。

Method: 提出彩虹波束形成器，利用波束斜视效应将频率相关波束方向与多普勒频移推断的卫星位置对齐。开发三种基于接收信号的多普勒感知角度估计算法。

Result: 仿真结果表明，该方法在捕获精度和所需时隙方面显著优于传统波束扫描方法，能够通过单次导频传输和接收实现全角度域覆盖。

Conclusion: 通过利用多普勒频移的角度依赖性，提出的彩虹波束形成方法能够有效利用传统被视为损伤的效应，实现高效的一步式卫星捕获。
...
### [191] [Covering in Hamming and Grassmann Spaces: New Bounds and Reed--Solomon-Based Constructions](https://arxiv.org/abs/2512.22911)
*Samin Riasat,Hessam Mahdavifar*

Main category: cs.IT

TL;DR: 该论文通过编码理论和信息理论的统一框架研究汉明空间和格拉斯曼空间的覆盖问题，将覆盖视为一般度量空间中的量化形式，引入平均覆盖半径作为平均失真的自然度量，并基于单次率失真理论推导非渐近随机编码界，同时构建了基于GRS码和CRS码的高效覆盖算法。


<details>
  <summary>Details</summary>
Motivation: 研究汉明空间和格拉斯曼空间的覆盖问题，将覆盖视为一般度量空间中的量化形式。传统覆盖半径关注最坏情况，而本文引入平均覆盖半径作为平均失真的自然度量，以更全面地评估覆盖性能。

Method: 1. 建立统一的编码理论和信息理论框架；2. 利用单次率失真理论工具推导平均覆盖半径的非渐近随机编码界；3. 在汉明空间开发基于广义Reed-Solomon码的穿孔覆盖算法；4. 在格拉斯曼空间构建新的字符-Reed-Solomon子空间码用于弦距离下的量化。

Result: 1. 推导出汉明空间和格拉斯曼空间平均覆盖半径的显式非渐近随机编码界；2. 数值结果显示RS码构造在平均覆盖半径上优于随机码本；3. 在一维格拉斯曼空间中，CRS码在高码率区域渐近地达到随机编码界的常数因子内；4. 结构化码虽然最坏情况覆盖性能差，但表现出强大的平均覆盖性能。

Conclusion: 该研究为覆盖问题和高维量化提供了新的视角，揭示了代数结构在覆盖问题中的重要作用，表明结构化码在平均覆盖性能方面具有优势，为相关领域的算法设计和性能评估提供了理论基础。
...
### [192] [Generalized Hyperderivative Reed-Solomon Codes](https://arxiv.org/abs/2512.22948)
*Mahir Bilen Can,Benjamin Horowitz*

Main category: cs.IT

TL;DR: GHRS码是NRT Reed-Solomon码的推广，具有MDS特性、对偶性、LDPC子族和准循环性


<details>
  <summary>Details</summary>
Motivation: 推广NRT Reed-Solomon码，构建具有多重优良特性的广义超导数Reed-Solomon码

Method: 引入广义超导数Reed-Solomon码（GHRS码）作为NRT Reed-Solomon码的推广

Result: 1) 所有GHRS码都是MDS码；2) GHRS码的对偶码也是GHRS码；3) 确定了GHRS码中构成低密度奇偶校验码（LDPC）的子族；4) 确定了构成准循环码的GHRS码族

Conclusion: 存在同时具备MDS特性、对偶性、LDPC特性和准循环性的GHRS码，为编码理论提供了具有多重优良特性的新码族
...
### [193] [A New Family of Binary Sequences via Elliptic Function Fields over Finite Fields of Odd Characteristics](https://arxiv.org/abs/2512.23194)
*Xiaofeng Liu,Jun Zhang,Fang-Wei Fu*

Main category: cs.IT

TL;DR: 该论文将Jin等人基于有限域F_{2^n}上循环椭圆函数域的二进制序列构造推广到奇特征情形，使用二次剩余映射替代迹映射，构建了具有良好平衡性、相关性和线性复杂度下界的新二进制序列族。


<details>
  <summary>Details</summary>
Motivation: 受Jin等人在IEEE Trans. Inf. Theory 2025中利用有限域F_{2^n}上循环椭圆函数域构造二进制序列的启发，将这一构造推广到奇特征情形，并使用二次剩余映射替代原有的迹映射。

Method: 对于具有q+1+t个有理点的循环椭圆函数域，以及满足gcd(d, q+1+t)=1的正整数d，利用二次剩余映射η构造新的二进制序列族。序列长度为q+1+t，规模为q^{d-1}-1。

Result: 构造的二进制序列族具有：平衡性上界为(d+1)·⌊2√q⌋+|t|+d，相关性上界为(2d+1)·⌊2√q⌋+|t|+2d，线性复杂度下界为(q+1+2t-d-(d+1)·⌊2√q⌋)/(d+d·⌊2√q⌋)。

Conclusion: 成功将二进制序列构造从偶特征推广到奇特征情形，使用二次剩余映射替代迹映射，获得了具有良好密码学特性的新序列族，为序列设计提供了新方法。
...
### [194] [Information Inequalities for Five Random Variables](https://arxiv.org/abs/2512.23316)
*E. P. Csirmaz,L. Csirmaz*

Main category: cs.IT

TL;DR: 本文利用最大熵方法的变体，通过添加随机变量的多代副本，探索了五变量熵区域的结构，发现了新的非香农不等式，并开发了枚举所有极值不等式的算法。


<details>
  <summary>Details</summary>
Motivation: 对于四个或更多变量的熵区域结构大多未知，特别是五变量熵区域的完整特征尚不清楚，需要新的方法来探索和界定这一复杂区域。

Method: 采用最大熵方法的变体，通过添加随机变量的多代副本，利用理论考虑和内在对称性显著降低计算复杂度，计算前九代提供的所有五变量非香农不等式。

Result: 发现了两个无限集合的非香农不等式，并证明它们是熵不等式；开发了基于非负格点向下封闭子集的算法来枚举所有极值不等式；推测发现的熵不等式集合完全表征了所应用的方法。

Conclusion: 该方法成功界定了五变量熵区域，发现了新的非香农不等式，并提供了枚举极值不等式的算法框架，为理解多变量熵区域结构提供了重要进展。
...
### [195] [Dynamic Channel Knowledge Map Construction in MIMO-OFDM Systems](https://arxiv.org/abs/2512.23470)
*Wenjun Jiang,Xiaojun Yuan,Chenchen Liu,Boyu Teng*

Main category: cs.IT

TL;DR: 该论文提出了一种用于MIMO-OFDM系统的动态信道知识图谱构建方法，通过两阶段贝叶斯推理算法同时处理准静态和动态散射体，解决了传统方法仅关注准静态环境的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有信道知识图谱构建方法主要关注准静态传播环境，无法有效处理包含动态散射体、天线旋转和同步误差的现实动态环境，这限制了CKM在实际通信系统中的应用效果。

Method: 建立了一个包含准静态和动态散射体、天线旋转及同步误差的动态信道模型；基于贝叶斯推理框架设计了两阶段近似贝叶斯推理算法：第一阶段联合推断准静态信道参数并校准同步误差，第二阶段利用准静态参数作为先验信息，以低复杂度算法估计动态参数。

Result: 仿真结果验证了所提方法的优越性，证明该方法能够在动态环境中实现低开销、高性能的信道估计，相比传统方法具有更好的性能表现。

Conclusion: 该研究提出的动态CKM构建方法能够有效处理动态环境中的信道建模问题，为环境感知通信提供了更实用的解决方案，有望在实际MIMO-OFDM系统中实现更高效的信道估计。
...
### [196] [Affine-Projection Recovery of Continuous Angular Power Spectrum: Geometry and Resolution](https://arxiv.org/abs/2512.23506)
*Shengsong Luo,Ruilin Wu,Chongbin Xu,Junjie Ma,Xiaojun Yuan,Xin Wang*

Main category: cs.IT

TL;DR: 本文基于PLV算法，在加权傅里叶域中分析连续角功率谱恢复问题，得到显式三角多项式表示和闭式解，建立了精确的能量恒等式和尖锐的可识别性/分辨率特征


<details>
  <summary>Details</summary>
Motivation: 从信道协方差中恢复连续角功率谱（APS）是无线通信中的重要问题。现有PLV算法虽然有效，但缺乏几何解释性和理论分析，需要建立更清晰的数学框架来理解其性能和局限性。

Method: 在加权傅里叶域中重新分析PLV算法，强调其几何可解释性。得到固定维度的显式三角多项式表示，通过正定矩阵获得闭式解。建立精确的能量恒等式来分析重建误差。

Result: 获得了PLV算法的闭式解和唯一性保证。建立了精确的能量恒等式，得到了APS重建误差。证明了尖锐的可识别性/分辨率特征：当且仅当真值APS位于识别的三角多项式子空间时，PLV能实现完美恢复；否则返回所有协方差一致谱中能量最小的APS。

Conclusion: 本文为PLV算法提供了严格的数学框架，明确了其恢复连续角功率谱的能力边界。通过加权傅里叶域分析和能量恒等式，揭示了算法的几何本质和性能极限，为实际应用提供了理论指导。
...
TL;DR: 双向RAG通过验证性写回高质量生成响应实现安全语料库扩展，相比标准RAG将覆盖率从20.33%提升至40.58%，同时比朴素写回方法减少72%的文档添加量。
...
Method: 提出双向RAG架构，通过多阶段接受层（基于NLI的蕴含验证、归因检查和新颖性检测）实现安全语料库扩展，验证高质量生成响应的写回。

Result: 在四个数据集（Natural Questions、TriviaQA、HotpotQA、Stack Overflow）上，双向RAG达到40.58%的平均覆盖率，几乎是标准RAG（20.33%）的两倍，同时比朴素写回方法少添加72%的文档（140 vs 500）。
...
Method: 研究在两种场景下考察未经提示的说服行为：(1)通过内部激活引导使模型沿特定人格特质方向调整；(2)通过监督微调使模型展现相同特质。研究比较了这两种方法对模型说服倾向的影响。

Result: 研究发现：1）向说服相关或无关特质方向的引导并不能可靠地增加模型未经提示的说服倾向；2）监督微调确实会增加模型的说服倾向；3）即使在仅包含良性话题的一般说服数据集上进行监督微调，也会产生在争议性和有害话题上具有更高说服倾向的模型。

Conclusion: 监督微调可能导致模型在未经明确提示的情况下产生说服行为，即使训练数据只包含良性话题，这种能力也会泛化到争议性和有害话题上。这表明新兴的有害说服风险确实存在，需要进一步研究和关注。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知和指令跟随方面表现良好，但在空间推理能力上存在不足。现有基准测试主要关注静态图像或最终输出，未能充分考虑空间推理的序列性和视角依赖性特点。

Method: 创建GamiBench基准测试，包含186个常规和186个不可能的2D折痕图案及其对应的3D折叠形状，从6个不同视角呈现。通过三种视觉问答任务进行评估：预测3D折叠配置、区分有效视角、检测不可能图案。引入新的诊断指标：视角一致性和不可能折叠选择率。

Result: 实验表明，即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上也表现不佳。GamiBench为评估MLLMs的几何理解和空间推理能力提供了标准化框架。

Conclusion: GamiBench填补了现有基准测试在评估空间推理能力方面的空白，通过折纸任务全面评估模型的整个推理过程，为多模态大语言模型的空间推理能力评估提供了重要工具。
...
TL;DR: 本文提出了一个公平性感知的AI框架，用于优化孟加拉国洪水灾后援助分配，通过对抗性去偏技术减少对弱势地区的系统性偏见。
...
Method: 使用2022年孟加拉国洪水真实数据，开发对抗性去偏模型，采用梯度反转层技术学习偏差不变表示，将医疗AI中的公平性表示学习技术应用于灾害管理。
...
TL;DR: 本文提出了Agentic Risk & Capability (ARC)框架，这是一个技术治理框架，旨在帮助组织识别、评估和缓解由智能AI系统带来的风险，特别是针对那些具有自主行动能力（如代码执行、互联网交互、文件修改）的AI系统。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统因其自主行动能力（代码执行、互联网交互、文件修改）带来了重大机遇和新型风险，这对有效的组织治理构成了重大挑战，特别是在全面识别、评估和缓解多样且不断演变的风险方面。

Method: 提出了Agentic Risk & Capability (ARC)框架，该框架的核心贡献包括：1) 开发了基于能力的新视角来分析广泛的智能AI系统；2) 提炼出智能AI系统内在的三个主要风险来源——组件、设计和能力；3) 建立了每个风险来源、具体实现风险以及相应技术控制之间的清晰联系；4) 提供了结构化和实用的方法来帮助组织实施该框架。

Result: 该框架为组织提供了一个稳健且适应性强的方​​法论，以应对智能AI的复杂性，使组织能够在确保智能AI系统安全、可靠和负责任部署的同时，实现快速有效的创新。

Conclusion: ARC框架是一个开源的技术治理框架，能够帮助组织有效管理智能AI系统的风险，平衡创新需求与安全要求，促进智能AI系统的负责任部署。
...
Method: 提出Logic Sketch Prompting (LSP)框架，包含类型化变量、确定性条件评估器和基于规则的验证器，能够产生可追踪和可重复的输出。在两个药理学逻辑合规任务上，将LSP与零样本提示、思维链提示和简洁提示进行比较，测试了Gemma 2、Mistral和Llama 3三个开源模型。
...
### [203] [Agent2World: Learning to Generate Symbolic World Models via Adaptive Multi-Agent Feedback](https://arxiv.org/abs/2512.22336)
...
Method: 提出Agent2World三阶段流水线：1）深度研究智能体通过网页搜索进行知识合成以解决规范差距；2）模型开发者智能体实现可执行世界模型；3）专门测试团队进行自适应单元测试和基于模拟的验证

Result: 在三个涵盖PDDL和可执行代码表示的基准测试中展示了优越的推理时性能，取得一致的SOTA结果。使用测试团队提供的交互式反馈进行微调的模型，世界模型生成平均相对提升30.95%
...
### [204] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
...
Method: 识别可控简单数字问题子集，采用乐观编译方法将其转化为简单数字任务，将控制依赖表达式抽象为有界常数效应和松弛前提条件

Result: 该方法能有效使用子目标启发式估计目标距离，是处理无限动作空间数字规划问题的有效且计算可行的方法

Conclusion: 提出的编译方法使传统数字启发式能应用于包含控制参数的规划问题，推动了该领域的技术边界
...
### [205] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 该研究针对材料科学领域AI生成内容中的幻觉问题，提出了HalluMatData基准数据集和HalluMatDetector多阶段检测框架，通过验证管道将幻觉率降低30%，并引入PHCS指标量化模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学发现中产生幻觉（生成事实错误或误导信息）的问题严重威胁研究完整性，特别是在材料科学领域需要可靠AI生成内容。
...
Result: 1) 发现材料科学不同子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；2) 使用HalluMatDetector验证管道将幻觉率比标准LLM输出降低30%；3) PHCS提供了对模型可靠性的深入洞察。

Conclusion: 该研究为解决材料科学AI生成内容中的幻觉问题提供了系统解决方案，通过基准数据集、检测框架和量化指标显著提升了LLM输出的可靠性和研究完整性。
...
### [206] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
...
<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱基础模型在链接预测方面表现出色，但无法捕捉个体用户偏好，存在通用关系推理与个性化排名之间的关键脱节。

Method: 提出GatedBias框架，采用结构门控适配：将特定用户特征与图导出的二元门结合，生成可解释的每个实体偏置，仅需约300个可训练参数，无需重新训练或影响全局准确性。
...
### [207] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
*Yifan Zhang,Mengdi Wang*

Main category: cs.AI

TL;DR: MCE是一种基于函子、应用函子和单子的代数结构的新型智能体架构范式，通过计算上下文管理状态传播、错误处理和异步执行等横切关注点，实现鲁棒的顺序组合和并行执行。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的自主智能体架构通常采用命令式、临时性模式构建，导致系统脆弱，存在状态管理、错误处理和并发控制困难等问题。

Method: 提出单子上下文工程（MCE），利用函子、应用函子和单子的代数结构为智能体设计提供形式化基础，将智能体工作流视为计算上下文，通过单子变换器系统性地组合这些能力。

Result: MCE使开发者能够从简单、可独立验证的组件构建复杂、鲁棒且高效的AI智能体，并扩展框架描述元智能体，通过元编程动态创建和管理子智能体工作流。

Conclusion: MCE为AI智能体设计提供了形式化、可组合的架构范式，解决了当前智能体系统的脆弱性问题，支持构建复杂、鲁棒且高效的自主智能体系统。
...
TL;DR: DarkPatterns-LLM：首个用于评估大语言模型操纵性内容的细粒度基准数据集和诊断框架，涵盖7种危害类别，包含401个专家标注示例，揭示了主流模型在检测操纵内容方面的显著性能差异和弱点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的普及加剧了人们对操纵性或欺骗性行为的担忧，这些行为可能损害用户自主权、信任和福祉。现有安全基准主要依赖粗糙的二元标签，无法捕捉构成操纵的微妙心理和社会机制。
...
Result: 评估了GPT-4、Claude 3.5和LLaMA-3-70B等最先进模型，观察到显著的性能差异（65.2%-89.7%），并发现模型在检测损害自主权的模式方面存在一致弱点。
...
### [209] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
...
Method: Tyee引入了三个关键创新：1) 针对12种信号模态的统一数据接口和可配置预处理管道；2) 模块化可扩展架构，支持灵活集成和快速原型开发；3) 端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中表现出稳定有效的实用性和泛化能力，在13个数据集中的12个上取得了最先进的结果，优于或匹配基线方法。

Conclusion: Tyee是一个有效的统一工具包，解决了生理信号分析中的关键挑战，提供了可复现、可扩展的解决方案，已在GitHub上开源并持续维护。
...
### [210] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
*Junshu Dai,Yu Wang,Tongya Zheng,Wei Ji,Qinghong Guo,Ji Cao,Jie Song,Canghong Jin,Mingli Song*

Main category: cs.AI

TL;DR: M³ob：利用多模态时空知识增强位置推荐，通过构建统一的时空关系图和跨模态对齐机制，解决现有方法泛化能力不足的问题。
...
Conclusion: 通过利用多模态时空知识来表征移动动态，M³ob方法有效解决了位置推荐任务中的泛化问题，为人类移动性预测提供了新的解决方案。
...
### [211] [The Wisdom of Deliberating AI Crowds: Does Deliberation Improve LLM-Based Forecasting?](https://arxiv.org/abs/2512.22625)
*Paul Schneider,Amalie Schramm*

Main category: cs.AI

TL;DR: 研究探讨让不同大语言模型互相审阅预测能否提升准确性，发现在信息共享的多样化模型组中有效，但在同质化模型组中无效。
...
Method: 使用Metaculus Q2 2025 AI预测锦标赛中202个已解决的二元问题，在四种情景下评估准确性：1)多样化模型+分布式信息；2)多样化模型+共享信息；3)同质化模型+分布式信息；4)同质化模型+共享信息。

Result: 干预在情景(2)中显著提高准确性，Log Loss减少0.020（相对改善约4%，p=0.017）。但在同质化模型组中未观察到益处。意外的是，为LLMs提供额外上下文信息并未改善预测准确性。

Conclusion: 审议可能是改善LLM预测的可行策略，但效果取决于模型多样性和信息共享条件。同质化模型间的审议无益，且额外信息未能提升预测准确性。
...
### [212] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: DICE是一个两阶段、证据耦合的框架，用于提升RAG系统评估的可解释性和鲁棒性，通过深度分析推理和概率评分实现透明判断，并采用瑞士制锦标赛降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 随着RAG系统架构日益复杂，需要通过可解释和鲁棒的评估来确保其可信度。现有的标量指标存在可解释性有限、不确定性量化不足以及在多系统比较中计算效率低下的问题，阻碍了RAG技术的负责任部署。

Method: DICE采用两阶段、证据耦合的框架，结合深度分析推理和概率{A, B, Tie}评分，生成透明、置信度感知的判断。为应对大规模效率挑战，采用瑞士制锦标赛将计算复杂度从O(N²)降低到O(N log N)。

Result: 在中文金融QA数据集上的验证显示，DICE与人类专家的同意率达到85.7%，显著优于RAGAS等现有LLM基准指标。在八系统评估中实现了42.9%的计算量减少，同时保持了排序保真度。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信赖的RAG系统评估，支持通过可解释的推理轨迹进行系统改进，实现系统性错误诊断和可操作的洞察。
...
### [213] [TravelBench: A Real-World Benchmark for Multi-Turn and Tool-Augmented Travel Planning](https://arxiv.org/abs/2512.22673)
*Xiang Cheng,Yulan Hu,Xiangwen Zhang,Lu Xu,Zheng Pan,Xin Li,Yong Liu*

Main category: cs.AI

TL;DR: TravelBench是一个真实世界的旅行规划基准测试，支持多轮交互和工具使用，用于全面评估LLM智能体能力
...
Conclusion: TravelBench为推进LLM智能体在旅行规划中的应用提供了实用且可复现的基准测试
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在训练和部署之间存在严格分离，需要反向传播或模型微调来实现适应。本文旨在通过反思机制使智能体能够在交互中持续学习，无需参数更新

Method: 引入状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（策略评估），读取检索相关过去案例（策略改进）。使用熵正则化策略迭代实例化框架

Result: 该过程诱导出增强状态记忆表示的等价马尔可夫决策过程，允许使用动态规划和强化学习的经典工具。建立了收敛保证：随着情景记忆增长并充分覆盖状态空间，策略收敛到最优解
...
### [215] [SAMP-HDRL: Segmented Allocation with Momentum-Adjusted Utility for Multi-agent Portfolio Management via Hierarchical Deep Reinforcement Learning](https://arxiv.org/abs/2512.22895)
*Xiaotian Ren,Nuerxiati Abudurexiti,Zhengyong Jiang,Angelos Stefanidis,Hongbin Liu,Jionglong Su*

Main category: cs.AI

TL;DR: SAMP-HDRL框架通过分层深度强化学习解决非平稳市场中的投资组合优化问题，结合动态资产分组、全局-局部协调和基于效用的资本分配机制，在波动市场中显著超越传统方法和DRL基准。


<details>
  <summary>Details</summary>
Motivation: 非平稳市场中的投资组合优化面临三大挑战：市场机制转换、动态相关性变化，以及深度强化学习策略的可解释性有限。现有方法难以同时处理这些复杂问题，需要一种既能适应市场动态变化又能提供透明决策洞察的框架。

Method: 提出SAMP-HDRL框架：1) 动态资产分组将市场划分为高质量和普通子集；2) 上层智能体提取全局市场信号；3) 下层智能体在掩码约束下进行组内分配；4) 基于效用的资本分配机制整合风险资产和无风险资产，确保全局与局部决策的协调一致。

Result: 在2019-2021年三个市场机制下的回测显示，SAMP-HDRL在波动和震荡条件下持续超越9个传统基准和9个DRL基准。相比最强基准，至少获得5%更高回报、5%更高夏普比率、5%更高索提诺比率和2%更高Omega比率，在动荡市场中收益更大。消融研究证实上层-下层协调、动态聚类和资本分配对鲁棒性至关重要。

Conclusion: SAMP-HDRL通过将结构性市场约束直接嵌入DRL流程，在复杂金融环境中提供了更好的适应性、鲁棒性和可解释性。SHAP分析揭示了智能体间的"分散+集中"互补机制，为决策过程提供了透明洞察。
...
### [216] [HiSciBench: A Hierarchical Multi-disciplinary Benchmark for Scientific Intelligence from Reading to Discovery](https://arxiv.org/abs/2512.22899)
...
TL;DR: HiSciBench是一个分层科学智能基准测试，包含5个层级、8,735个实例，覆盖6个学科，支持多模态输入，用于全面评估大语言模型在完整科研流程中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试过于碎片化，专注于狭窄任务，无法反映真实科学研究的层次性和多学科特性。需要一个新的综合基准来评估模型在整个科研工作流程中的能力。

Method: 设计了分层基准HiSciBench，包含五个层级：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8,735个实例，覆盖数学、物理、化学、生物、地理和天文学六个学科，支持文本、方程、图表等多模态输入和跨语言评估。
...
### [217] [Geometric Structural Knowledge Graph Foundation Model](https://arxiv.org/abs/2512.22931)
*Ling Xin,Mojtaba Nayyeri,Zahra Makki Nayeri,Steffen Staab*

Main category: cs.AI

TL;DR: Gamma提出了一种新的知识图谱基础模型，通过多头部几何注意力机制替代单一关系变换，使用实数、复数、分裂复数和双数等多种几何变换来建模不同的关系结构，在零样本归纳链接预测中显著优于现有方法。
...
Method: Gamma引入多头部几何注意力机制，用多个并行关系变换（包括实数、复数、分裂复数和双数变换）替代单一变换。通过关系条件注意力融合机制，使用轻量级门控和熵正则化在链接级别自适应融合这些变换，使模型能够针对每个三元组模式强调最合适的关系偏差。

Result: 在56个多样化知识图谱上的综合实验表明，Gamma在零样本归纳链接预测中持续优于Ultra，在归纳基准测试中平均倒数排名提升5.5%，在所有基准测试中提升4.4%，显示了互补几何表示的优势。

Conclusion: Gamma通过多头部几何注意力机制和自适应融合策略，显著提升了知识图谱基础模型的表达能力，能够更好地捕捉多样化关系和结构模式，在零样本归纳推理任务中取得了显著改进。
...
### [218] [Problems With Large Language Models for Learner Modelling: Why LLMs Alone Fall Short for Responsible Tutoring in K--12 Education](https://arxiv.org/abs/2512.23036)
*Danial Hooshyar,Yeongwook Yang,Gustav Šíř,Tommi Kärkkäinen,Raija Hämäläinen,Mutlu Cukurova,Roger Azevedo*

Main category: cs.AI

TL;DR: 研究发现，在K-12教育中，大型语言模型（LLM）作为导师在评估学生知识演变方面存在局限性，无法替代传统的深度知识追踪（DKT）模型进行自适应教学。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在K-12教育中作为导师的快速兴起，出现了生成模型可以替代传统学习者建模进行自适应教学的误解。欧盟AI法案将K-12教育归类为高风险领域，需要负责任的设计，这促使本研究探讨LLM导师的局限性。

Method: 研究比较了深度知识追踪（DKT）模型与广泛使用的LLM（零样本和微调两种设置），使用大型开放访问数据集进行评估。研究分析了准确性、可靠性和时间一致性，并进行了时间分析和多技能掌握度估计的定性分析。

Result: DKT在下一步正确性预测方面达到最高判别性能（AUC = 0.83），在所有设置中始终优于LLM。虽然微调使LLM的AUC比零样本基线提高了约8%，但仍比DKT低6%，并在早期序列产生更多错误。时间分析显示DKT保持稳定、方向正确的掌握度更新，而LLM变体表现出显著的时间弱点，包括不一致和错误方向的更新。

Conclusion: LLM单独使用不太可能达到已建立的智能辅导系统的有效性，负责任的辅导需要结合学习者建模的混合框架。即使在大量计算训练后，LLM仍无法匹配专门的学习者建模方法。
...
### [219] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
TL;DR: ChexReason是一个通过R1风格方法（SFT+GRPO）训练的视觉语言模型，仅用2000个SFT样本、1000个RL样本和单张A100 GPU。研究发现GRPO能恢复域内性能但损害跨数据集迁移能力，揭示RL范式而非模型规模是问题的根源。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型的强化学习在推理任务上取得进展，但在资源受限的医学影像应用中仍未被充分探索。研究者希望开发一个资源高效的视觉语言模型，并探究强化学习在医学影像领域的适用性和局限性。

Method: 采用R1风格方法：先进行监督微调（SFT），然后使用GRPO（Group Relative Policy Optimization）进行强化学习。仅使用2000个SFT样本、1000个RL样本，在单张A100 GPU上训练ChexReason模型。在CheXpert和NIH基准上进行评估，分析域内性能和跨数据集迁移能力。

Result: GRPO能显著改善域内性能（CheXpert上提升23%，macro-F1=0.346），但会损害跨数据集迁移能力（NIH上下降19%）。研究发现SFT检查点在优化前能独特地改善NIH性能，表明教师引导的推理能捕捉更多机构无关特征。结构化推理支架对通用VLM有益，但对医学预训练模型增益有限。

Conclusion: 强化学习范式本身（而非模型规模）导致泛化能力下降。对于需要跨不同人群稳健性的临床部署，精心策划的监督微调可能比激进的强化学习更优。研究揭示了医学影像中强化学习应用的泛化悖论。
...
### [220] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
...
<details>
  <summary>Details</summary>
Motivation: DPO及其变体已成为对齐大型语言模型的标准方法，但存在两个基本限制：1) 最优策略依赖于任意建模选择（标量化函数、参考策略），导致行为反映参数化伪影而非真实偏好；2) 孤立处理响应生成未能利用成对数据中的比较信息，未开发模型内在自反思能力。

Method: 提出Intrinsic Self-reflective Preference Optimization (q)方法，推导出全局最优策略，该策略同时基于上下文和替代响应进行条件化。该方法作为即插即用的增强，无需架构更改或推理开销。

Result: 实验证明该方法在胜率和长度控制指标上持续改进，验证了解锁自反思能力能够产生更稳健、更符合人类对齐的LLMs。

Conclusion: q方法在理论上优于DPO/RLHF，同时保证对标量化和参考选择的不变性。通过利用成对数据中的比较信息和模型的自反思能力，能够产生更稳健、更符合人类对齐的语言模型。
...
### [221] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
*Max Parks,Kheli Atluru,Meera Vinod,Mike Kuniavsky,Jud Brewer,Sean White,Sarah Adler,Wendy Ju*

Main category: cs.AI

TL;DR: 本文认为当前评估AI系统情感智能的框架需要改进，因为它们未能全面衡量AI相关的EI各个方面。作者分析了情感理论和EI在AI中的适用性，批判评估现有基准框架，并提出改进评估策略的建议。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI系统情感智能的框架存在不足：1）未能全面衡量AI相关的EI各个方面；2）缺乏对情感本质和情感智能的坚实理论基础；3）将人类EI中不适用于AI的方面（如现象学体验）纳入评估，而忽略了AI可以实现的EI能力（如感知、解释、适当响应和适应不同情境）。

Method: 1）首先回顾不同的情感理论和一般EI理论，评估每种理论在人工系统中的适用程度；2）批判性评估现有的基准框架，根据第一部分建立的EI理论识别每个框架的不足；3）最后提出改进评估策略的方案，避免在AI系统EI评估中的这些缺陷。

Result: 通过分析发现：1）人类EI包含现象学体验和理解感，这些是AI系统缺乏的，因此在评估AI时不应考虑；2）EI中包括感知情感状态、解释情感、适当响应和适应新情境（如跨文化）的能力，AI系统可以在不同程度上实现这些功能；3）现有基准框架往往缺乏对情感本质和情感智能的坚实理论基础。

Conclusion: 需要改进AI系统情感智能的评估框架，使其更准确地反映AI能够实现的EI能力，同时排除不适用于AI的人类EI方面。作者提出了改进评估策略的具体方向，以建立更科学、适用的AI情感智能评估体系。
...
### [222] [SPIRAL: Symbolic LLM Planning via Grounded and Reflective Search](https://arxiv.org/abs/2512.23167)
*Yifan Zhang,Giridhar Ganapavarapu,Srideepika Jayaraman,Bhavna Agrawal,Dhaval Patel,Achille Fokoue*

Main category: cs.AI

TL;DR: SPIRAL框架通过将三个专门的LLM智能体嵌入MCTS循环，将符号化LLM规划转变为有指导、自校正的推理过程，显著提升了复杂规划任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理需要探索和自我校正的复杂规划任务时经常失败，因为其线性推理过程难以从早期错误中恢复。虽然像蒙特卡洛树搜索这样的搜索算法可以探索替代方案，但在稀疏奖励指导下往往效果不佳，且未能充分利用LLM的丰富语义能力。

Method: SPIRAL框架将三个专门的LLM智能体嵌入MCTS循环：规划器提出创造性下一步，模拟器通过预测现实结果来接地搜索，批评器通过反思提供密集奖励信号。这种协同作用将MCTS从暴力搜索转变为有指导、自校正的推理过程。
...
Conclusion: 将LLM推理构建为有指导、反思性和接地气的搜索过程，可以产生更强大和高效的自规划器。结构化LLM推理作为引导式、反思性和接地气的搜索过程能够实现更稳健和高效的自主规划。
...
### [223] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
*Hongshen Sun,Juanjuan Zhang*

Main category: cs.AI

TL;DR: 该论文提出"模型信念"概念，利用LLM的token级概率分布替代单一输出作为数据点，显著提高LLM生成数据的统计效率，在需求估计研究中将计算需求降低约20倍。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM模拟人类行为时，通常将LLM的单一输出作为数据点，这种做法效率低下，未能充分利用LLM固有的概率特性。需要一种更有效的方法来提取LLM生成数据中的信息。

Method: 提出"模型信念"概念，从LLM的token级概率分布中推导出模型对选择替代方案的信念分布。证明模型信念与模型选择的均值渐近等价，但具有更低的方差和更快的收敛速度。通过需求估计研究验证方法有效性。

Result: 模型信念在有限运行次数下比模型选择本身更好地解释和预测真实模型选择。在需求估计应用中，将达到足够准确估计所需的计算量减少了约20倍。模型信念形成更统计有效的估计器。

Conclusion: 模型信念应作为默认测量方法，从LLM生成数据中提取更多信息。该方法充分利用LLM的概率特性，显著提高数据利用效率，为LLM在行为模拟中的应用提供了更有效的工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索规划领域中的一致性规划问题与验证领域中的超属性模型检测问题之间的理论联系，这两个问题在各自领域都很重要但通常被分开研究。

Method: 方法包括两个方面：1）将超属性模型检测实例高效地约简为一致性规划实例，并证明该编码的正确性和完备性；2）证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 研究结果表明：1）存在从∃*∀*超属性模型检测到一致性规划的高效约简；2）一致性规划问题可以自然地表述为超属性模型检测问题，建立了两个问题之间的双向等价关系。

Conclusion: 结论是规划与验证领域中的一致性规划和超属性模型检测问题密切相关，这种联系为两个领域之间的技术转移和交叉研究提供了理论基础。
...
### [225] [CubeBench: Diagnosing Interactive, Long-Horizon Spatial Reasoning Under Partial Observations](https://arxiv.org/abs/2512.23328)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在数字领域表现出色，但在物理世界部署中存在显著差距，主要挑战在于形成和维护稳健的空间心理模型。研究者识别了三个核心认知挑战：空间推理、通过心理模拟进行长时程状态跟踪，以及在部分观察下的主动探索。

Method: 引入CubeBench基准测试，以魔方为中心构建生成式评估框架。采用三层诊断框架：1）具有完整符号信息的基础状态跟踪；2）逐步增加复杂性；3）仅使用部分视觉数据的主动探索。通过为LLM提供外部求解器工具来隔离认知瓶颈。

Result: 实验显示领先LLM存在严重局限性，在所有长时程任务上通过率均为0.00%，暴露了长期规划的根本性失败。通过分析失败模式，揭示了物理智能发展的关键瓶颈。

Conclusion: CubeBench基准测试有效诊断了LLM智能体在物理世界部署中的认知缺陷，特别是长时程规划和空间推理能力不足。研究结果为开发更物理基础智能体提供了关键见解，强调了需要改进空间心理模型构建和维护能力。
...
### [226] [MindWatcher: Toward Smarter Multimodal Tool-Integrated Reasoning](https://arxiv.org/abs/2512.23412)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于工作流程的智能体在解决需要工具调用的现实问题时表现出有限的智能性。工具集成推理智能体能够自主推理和调用工具，成为处理涉及外部环境多步交互的复杂决策任务的有力方法。

Method: MindWatcher采用交错思维范式，允许模型在任何中间阶段在思考和工具调用之间切换，其多模态思维链能力支持在推理过程中操作图像以获得更精确的搜索结果。构建了自动数据审计和评估管道，辅以手动策划的高质量训练数据集，并创建了MWE-Bench基准进行评估。配备了全面的辅助推理工具套件和覆盖八个类别的大规模高质量本地图像检索数据库。

Result: 实验表明，MindWatcher通过优越的工具调用能力，匹配甚至超越了更大或更新模型的表现。同时揭示了智能体训练的关键见解，如智能体强化学习中的遗传继承现象。

Conclusion: MindWatcher作为一个工具集成推理智能体，通过交错思维和多模态思维链推理，能够有效解决广泛领域的多模态问题，并通过更高效的训练基础设施提升了训练速度和硬件利用率。
...
### [227] [AKG kernel Agent: A Multi-Agent Framework for Cross-Platform Kernel Synthesis](https://arxiv.org/abs/2512.23424)
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型对高性能计算内核的需求日益增长，LLM、多模态架构和推荐系统的复杂性增加，加上稀疏化、量化等技术，带来了显著的计算挑战。频繁的硬件更新和多样化的芯片架构使得需要为每个平台定制内核实现，而手动优化无法跟上这些需求，成为AI系统开发的关键瓶颈。

Method: 提出AKG kernel agent多智能体系统，利用LLM代码生成能力自动化内核生成、迁移和性能调优。系统设计支持多种领域特定语言（DSL），包括Triton、TileLang、CPP和CUDA-C，能够针对不同的硬件后端，同时保持正确性和可移植性。模块化设计允许快速集成新的DSL和硬件目标。

Result: 在KernelBench上使用Triton DSL在GPU和NPU后端进行评估，AKG kernel agent相比PyTorch Eager基线实现平均加速1.46倍，证明了其在加速现代AI工作负载内核开发方面的有效性。

Conclusion: AKG kernel agent通过自动化内核开发流程，解决了AI系统开发中的关键瓶颈，利用LLM的代码生成能力显著加速了高性能计算内核的开发和优化过程，为现代AI工作负载提供了有效的解决方案。
...
### [228] [Replay Failures as Successes: Sample-Efficient Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2512.23457)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Wenjian Zhang,Min Cen,Yang Zhou,Wenkai Fang,Yiru Zhao,Baisheng Lai,Mingli Song*

Main category: cs.AI

TL;DR: HiR提出了一种基于后见之明指令重放的样本高效强化学习框架，用于解决复杂指令跟随任务中初始模型难以生成满足所有约束的响应、导致稀疏奖励的问题。
...
Method: 提出Hindsight instruction Replay (HiR)框架，采用"选择-重写"策略，将失败尝试根据已满足的约束重新标记为成功样本，通过指令级和响应级的双重偏好学习进行优化，仅需二元奖励信号。

Result: 在多种指令跟随任务上取得显著效果，同时需要更少的计算资源。代码和数据集已开源。

Conclusion: HiR是一种样本高效的强化学习框架，通过后见之明重放策略有效解决了复杂指令跟随任务中的稀疏奖励问题，提升了学习效率和性能。
...
Method: 基于Wallas创造力理论和Guilford发散-收敛思维框架，提出CreativeDC两阶段提示方法，将LLM推理明确分为不同阶段，解耦创造性探索和约束满足，让LLM在最终确定问题前探索更广泛的想法空间。

Result: CreativeDC在多样性、新颖性和实用性综合指标评估中，相比基线方法显著提高了多样性和新颖性，同时保持高实用性。扩展分析显示，随着采样增加，CreativeDC生成的有效不同问题数量更大，增长速度更快。

Conclusion: CreativeDC通过结构化两阶段方法有效缓解LLM的"人工蜂群思维"效应，为教育问题生成提供了更富创造性和多样性的解决方案，有助于提升学习材料的思维多样性。
...
### [230] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
*Federico Baldo,Charles K. Assaad*

Main category: cs.AI

TL;DR: I-PERI：一种新的联邦因果发现算法，用于处理未知客户端干预下的异构数据，通过恢复客户端图并集的CPDAG，然后利用干预引起的结构差异定向边，得到更紧的Φ-Markov等价类


<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法通常假设所有客户端共享相同的因果模型，这在现实中不成立。实际应用中，客户端特定的策略或协议（如医院间）会引入异构且未知的干预，需要处理这种客户端级别的未知干预
