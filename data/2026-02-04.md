...
TL;DR: WorldVQA是一个专门评估多模态大语言模型视觉世界原子知识的基准，通过解耦视觉知识检索与推理能力，严格测量模型记忆的视觉事实性知识。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法往往将视觉知识检索与推理能力混为一谈，无法准确衡量模型真正记忆的视觉世界知识。需要建立一个能够严格测量模型视觉事实性知识、百科全书广度和幻觉率的基准。
...
Conclusion: WorldVQA基准通过解耦知识检索与推理，专注于评估多模态大语言模型的原子视觉世界知识，为衡量模型视觉事实性、百科全书广度和幻觉率建立了重要标准。
...
TL;DR: 本文提出了AdaptMMBench基准，用于评估视觉语言模型的自适应多模态推理能力，通过动态识别任务难度和MCC指标来评估模式选择合理性，揭示自适应选择与最终准确率的解耦现象。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖静态难度标签和简单指标，无法捕捉相对于不同模型能力的动态难度特性，模糊了自适应模式选择与一般性能的区别，且缺乏细粒度的过程分析。

Method: 提出AdaptMMBench基准，涵盖五个领域：现实世界、OCR、GUI、知识和数学，包含直接感知和复杂推理任务。使用马修斯相关系数评估不同推理模式的选择合理性，通过动态识别基于模型能力边界的任务难度来分离元认知能力。支持跨关键步骤覆盖、工具有效性和计算效率的多维过程评估。

Result: 评估发现：自适应模式选择随模型容量扩展，但与最终准确率显著解耦；关键步骤覆盖与性能一致，但工具有效性在不同模型架构间高度不一致。

Conclusion: AdaptMMBench为自适应多模态推理提供了全面评估框架，揭示了自适应选择与性能的复杂关系，为未来模型开发提供了重要洞察。
...
TL;DR: 提出了一种用于光学相干断层扫描（OCT）逆散射问题的正则化端到端深度学习框架，能够同时重建光学参数图和去斑OCT结构强度图像，实现定量组织表征。


<details>
  <summary>Details</summary>
Motivation: OCT逆散射问题旨在恢复结构图像和内在组织光学特性（折射率、散射系数、各向异性），但由于衰减、散斑噪声和参数间的强耦合关系，这一逆问题具有挑战性。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers由于自注意力机制全局操作，缺乏明确区分前景和背景的机制，可能学习到不必要的背景特征和伪影，导致分类性能下降。
...
TL;DR: 提出Landmark Point Transformer (LmPT)方法，用于点云上的自动解剖标志点检测，能够利用不同物种的同源骨骼进行跨物种学习。


<details>
  <summary>Details</summary>
Motivation: 传统手动标志点标注耗时且存在观察者间差异，基于规则的方法通常针对特定几何形状或有限标志点集。需要一种能够处理不同物种、具有良好泛化能力的自动标志点检测方法。

Method: 提出Landmark Point Transformer (LmPT)模型，将解剖表面表示为点云，采用条件机制使模型能够适应不同输入类型，实现跨物种学习。

Result: 在人类和新标注的狗股骨上进行评估，证明了该方法在跨物种场景下的泛化能力和有效性。

Conclusion: LmPT是一种有效的自动解剖标志点检测方法，能够利用同源骨骼进行跨物种学习，为转化研究提供了有力工具。
...
TL;DR: 提出了一种无需标注和相机标定的自监督多视角手术室视频匿名化框架，通过检索漏检目标并利用伪标签迭代优化检测器，实现超过97%的召回率。


<details>
  <summary>Details</summary>
Motivation: 手术室视频数据用于研究需要隐私保护，但现有匿名化方法面临两个可扩展性瓶颈：1) 需要为每个新临床站点手动标注；2) 多摄像头设置需要重新标定。需要一种无需标注和相机标定的自动化解决方案。

Method: 提出自监督多视角视频匿名化框架，包含全身人员检测和姿态估计。核心策略：使用低阈值运行现成检测器收集候选检测，通过跟踪和自监督无标定多视角关联检索低分漏检目标，将其作为伪标签迭代微调检测器，最后使用检测器自身高分预测微调姿态模型。

Result: 在模拟手术的4D-OR数据集和真实手术数据集上验证，实现超过97%的召回率。使用伪标签训练实时全身检测器，性能相当，证明方法的实际应用价值。

Conclusion: 该方法成功解决了手术室视频匿名化的可扩展性问题，无需人工标注或相机标定，实现了高效准确的隐私保护，具有实际应用潜力。
...
TL;DR: ViThinker框架通过主动查询生成机制，让视觉语言模型能够按需合成专家对齐的视觉特征，解决了传统CoT推理中视觉信息过早转换丢失连续信息的问题。


<details>
  <summary>Details</summary>
Motivation: 传统CoT推理在视觉语言模型中表现不佳，因为过早的视觉到文本转换会丢失几何和空间布局等连续信息。现有方法多是被动处理预计算输入，而非主动寻找任务相关细节。

Method: ViThinker框架让模型自主生成决策（查询）令牌，触发按需合成专家对齐的视觉特征。采用两阶段课程：第一阶段将冻结专家知识蒸馏到模型参数中，第二阶段通过稀疏性惩罚学习任务驱动的查询机制。

Result: 在多个视觉中心基准测试中表现出一致的改进，验证了主动查询生成在感知基础和推理准确性方面优于被动方法。

Conclusion: ViThinker通过主动感知机制，使视觉语言模型能够发现每个推理步骤所需的最小充分感知，实现了更好的视觉推理性能。
...
TL;DR: 本文提出了一种用于医学影像决策的对比性、文档感知的参考选择框架，通过构建优化的证据集来区分易混淆病症，而不是依赖传统的最近邻检索方法。
...
Method: 1) 引入对比性、文档感知的参考选择框架，通过平衡视觉相关性、嵌入多样性和来源级溯源来构建优化的证据集；2) 提出Counterfactual-Contrastive Inference框架，执行结构化成对视觉比较，并使用基于边界的决策规则进行证据聚合，支持可信的弃权机制。

Result: 在MediConfusion基准测试中，该方法实现了最先进的性能，相对于先前方法将集合级准确率提高了近15%，同时减少了混淆并提高了个体准确性。

Conclusion: 通过对比性证据集和结构化推理框架，能够显著改善医学影像中的决策准确性，特别是在区分易混淆病症方面，为医学图像推理提供了系统化的解决方案。
...
TL;DR: 该论文提出FaceLinkGen攻击方法，证明当前基于像素重构的隐私保护人脸识别评估方法存在缺陷，攻击者无需恢复原始像素即可实现身份匹配和面部再生，暴露了视觉混淆无法真正保护身份隐私的问题。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护人脸识别系统的评估主要关注像素级重构抵抗能力，使用PSNR和SSIM等指标。作者认为这种重构中心的视角存在缺陷，无法真正评估身份隐私保护效果，需要揭示视觉混淆与真实隐私保护之间的差距。

Method: 提出FaceLinkGen攻击方法，能够直接从受保护的模板中执行身份链接/匹配和面部再生，无需恢复原始像素。该方法在三种最新的PPFR系统上进行测试，包括近零知识设置下的攻击。

Result: 在三种PPFR系统上，FaceLinkGen达到超过98.5%的匹配准确率和96%以上的再生成功率。即使在近零知识设置下，仍能超过92%的匹配准确率和94%的再生成功率，显著暴露了现有评估指标的不足。

Conclusion: 像素失真指标与真实隐私保护之间存在结构性差距，视觉混淆无法有效保护身份信息，攻击者可以从受保护模板中直接提取身份信息进行匹配和再生，这对隐私保护人脸识别系统的安全性提出了严峻挑战。
...
TL;DR: MARBLE是首个纯Mamba架构的多状态多实例学习框架，用于全切片图像分析，通过并行多尺度处理和线性时间状态空间模型，显著提升性能并降低计算成本。
...
Method: MARBLE采用并行多尺度处理，在Mamba状态空间模型中集成从粗到细的推理，以线性时间序列建模高效捕获跨尺度依赖关系，参数开销最小。

Result: 在五个公开数据集上，MARBLE在AUC上提升高达6.9%，准确率提升20.3%，C-index提升2.3%，证明了其高效性和泛化能力。
...
TL;DR: SRA-Seg框架通过语义特征对齐解决合成数据与真实医学图像之间的域差异问题，显著提升医学图像分割性能


<details>
  <summary>Details</summary>
Motivation: 合成数据虽然视觉逼真，但由于与真实医学图像存在于不同的语义特征空间，存在域差异，导致当前半监督学习方法无法有效利用合成数据提升分割性能
...
Conclusion: SRA-Seg通过显式对齐合成与真实特征分布，有效解决了医学图像分割中合成数据的域差异问题，为有限标注数据场景下的医学图像分割提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌剪枝方法在视觉问答任务上表现良好，但在视觉定位任务上性能大幅下降。研究发现这是因为基于全局语义相似性和注意力分数的策略丢失了由令牌位置信息交互产生的全局空间参考框架。

Method: 提出两阶段令牌剪枝框架：第一阶段在视觉编码器后，采用分离、对齐和聚合三个操作（受群体智能算法启发）来保留信息丰富的全局空间锚点；第二阶段在LLM内部，执行文本引导的剪枝以保留任务相关的视觉令牌。

Result: 在多个视觉问答基准测试中达到SOTA性能（从94%提升到95%），在视觉定位任务上获得显著改进（从7%提升到47%）。

Conclusion: Nüwa框架通过保留空间完整性，在保持视觉语言模型高效特征聚合的同时，显著提升了视觉定位任务的性能，解决了现有剪枝方法在空间相关任务上的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光片的时间比较在临床放射学中至关重要，但现有方法无法同时实现时间变化检测和空间定位。虽然视觉语言模型在单图像报告生成和视觉定位方面有进展，但没有方法能将这些能力结合用于时间变化检测。

Method: 提出TRACE模型，给定先前的和当前的胸部X光片，模型生成自然语言描述间隔变化（恶化、改善、稳定），同时通过边界框坐标对每个发现进行空间定位。

Result: TRACE展示了有效的空间定位能力，定位准确率超过90%，为这一具有挑战性的新任务奠定了基础。消融研究发现了一个新兴能力：只有当时间比较和空间定位联合学习时，变化检测才会出现，单独学习任一项都无法实现有意义的变化检测。

Conclusion: TRACE是首个能够联合执行时间比较、变化分类和空间定位的模型。研究发现定位为时间推理提供了必要的空间注意力机制，表明这两个任务的联合学习对于有效的时间变化检测至关重要。
...
TL;DR: 该研究推导了鱼眼立体视觉系统深度和范围误差的解析表达式，专门考虑了大角度下的精度问题
...
Method: 推导了深度和范围误差的解析表达式，考虑了物体距离的影响，专门针对大角度情况进行分析

Result: 获得了鱼眼立体视觉系统误差的数学表达式，能够准确描述误差随物体距离变化的规律

Conclusion: 该解析表达式为鱼眼立体视觉系统的性能评估和优化提供了理论基础，特别是在大角度观测场景下
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）将图像和文本作为整体对齐，难以处理长文本描述。细粒度视觉语言理解需要分层语义来捕捉全局上下文和局部细节，但语言层次（语法/语义）与视觉组织不匹配，纯视觉层次倾向于将场景分割为外观驱动的部分而缺乏语义焦点。

Method: CAFT框架结合从细到粗的视觉编码器和分层文本Transformer，使用分层对齐损失：整体图像与整体描述对齐，同时偏向区域-句子对应关系，使粗粒度语义建立在细粒度证据基础上，而非脱离部分级基础的聚合。

Result: 在3000万图像-文本对上训练后，CAFT在六个长文本检索基准上达到最先进性能，并展现出强大的扩展能力。实验表明分层跨域对齐能够在没有显式区域级监督的情况下产生细粒度、视觉基础良好的图像-文本表示。

Conclusion: CAFT通过分层跨域对齐实现了无需像素级监督的细粒度视觉语言表示学习，有效解决了长文本描述对齐问题，为视觉语言理解提供了新的层次化框架。
...
TL;DR: SharpTimeGS是一个基于4D高斯表示的寿命感知框架，用于动态场景的新视角合成，通过可学习的寿命参数实现静态和动态区域的时域自适应建模，支持实时4K分辨率渲染。
...
Method: 提出寿命感知的4D高斯框架，引入可学习的寿命参数将时间可见性从高斯衰减重新定义为平顶轮廓；寿命参数调节每个基元的运动，减少静态点的漂移；设计寿命-速度感知的致密化策略，缓解优化不平衡。

Result: 在多个基准测试中达到最先进性能，支持在单个RTX 4090上以100 FPS实时渲染4K分辨率。

Conclusion: SharpTimeGS通过寿命感知建模有效解耦了运动幅度与时间持续时间，在保持动态保真度的同时提高了长期稳定性，实现了静态和动态区域的平衡优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的强化学习方法在视频时序定位中存在两个主要问题：1）稀疏的奖励信号导致训练效率低下；2）巨大的计算开销限制了实际应用。需要一种既能保持策略对齐特性又能提供密集监督的高效训练方法。

Method: 提出Video-OPD框架，直接从当前策略采样轨迹进行优化，保持训练和推理分布的一致性。使用前沿教师模型通过反向KL散度目标提供密集的token级监督。还提出Teacher-Validated Disagreement Focusing (TVDF)训练课程，迭代地优先选择既可靠又对学生信息量最大的轨迹。

Result: 实验结果表明，Video-OPD在性能上持续优于GRPO方法，同时实现了显著更快的收敛速度和更低的计算成本，证明了策略蒸馏作为传统强化学习替代方案的有效性。

Conclusion: Video-OPD成功地将策略蒸馏引入视频时序定位领域，通过密集的token级监督和高效的训练课程，解决了传统强化学习方法中的稀疏奖励和高计算成本问题，为视频时序定位的后训练提供了更优的解决方案。
...
TL;DR: VOILA是一个基于信息价值驱动的自适应保真度选择框架，用于视觉问答任务，通过预检索优化选择信息保真度，在保持90-95%准确率的同时减少50-60%的计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前多模态视觉语言系统大多在固定保真度下运行，而获取和处理高保真度视觉输入需要显著成本。需要一种能够根据查询需求自适应选择信息保真度的框架，以优化资源受限下的多模态推理。

Method: VOILA采用两阶段流水线：首先使用梯度提升回归器仅从问题特征估计每个保真度的正确性概率，然后通过等渗校准器精炼这些概率以进行可靠决策。系统选择最小成本的保真度，最大化给定预测准确率和检索成本的期望效用。
...
Conclusion: 预检索保真度选择对于优化资源受限下的多模态推理至关重要，VOILA框架通过自适应保真度选择在保持高准确率的同时显著降低成本，展示了信息价值驱动方法的有效性。
...
TL;DR: 该论文提出了一种新的图像修复方法，通过结构特征图和纹理特征图的相互辅助来缓解卷积下采样过程中的信息损失，使用统计归一化和去归一化策略进行重建指导。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法虽然能够提取高频结构和低频纹理特征，但在卷积下采样过程中不可避免地会丢失结构和纹理特征图的信息，导致非理想的上采样输出。本文旨在解决这一信息损失问题。

Method: 采用统计归一化和去归一化策略，利用结构特征图和纹理特征图相互辅助，在卷积下采样过程中提供重建指导，从而缓解信息损失。

Result: 实验结果表明该方法在256*256和512*512等多种分辨率图像上优于现有最先进方法，特别是在将所有编码器替换为本方法时效果显著。

Conclusion: 通过结构特征图和纹理特征图的相互辅助，结合统计归一化和去归一化策略，能够有效缓解卷积下采样过程中的信息损失，提升图像修复质量。
...
Method: 建立计算机视觉处理流水线，分析分布在曼哈顿和纽约的900多个交通摄像头数据，对比2024年11月至2026年1月的交通模式

Result: 建立了基准交通模式，识别出监控区域内车辆密度的系统性变化

Conclusion: 通过自动化交通摄像头数据分析，能够客观评估拥堵收费计划对城市交通的影响，为政策效果提供数据支持
...
<details>
  <summary>Details</summary>
Motivation: 从简短用户提示生成长篇视听故事存在挑战，主要问题是意图-执行差距：高级叙事意图需要在长序列的连贯、镜头级多模态生成中保持一致性。现有方法通常依赖前馈管道或仅提示优化，导致语义漂移和身份不一致。

Method: 将故事讲述建模为闭环约束执行问题，提出MUSE多智能体框架，通过迭代的计划-执行-验证-修订循环协调生成。将叙事意图转化为对身份、空间构成和时间连续性的显式机器可执行控制，并在生成过程中应用针对性多模态反馈来纠正违规。

Result: 实验表明，与代表性基线相比，MUSE显著提高了长序列叙事连贯性、跨模态身份一致性和电影质量。

Conclusion: MUSE通过闭环约束执行和多智能体协调，有效解决了长序列视听故事生成中的语义漂移和身份不一致问题，提高了生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在日常视觉任务上取得了很大进展，但人类具有在全新情境中部署视觉推理能力的特殊技能，这种能力可以通过经典的Bongard问题进行严格测试。当前模型在这类抽象视觉推理任务上仍面临挑战。
...
Conclusion: 提出的神经符号方法结合了LLM的生成能力和贝叶斯优化的参数优化，为解决Bongard这类抽象视觉推理问题提供了一种有效途径。
...
Method: 提出IVC-Prune方法：1）通过分析RoPE的数学特性识别隐式视觉坐标令牌（IVC tokens）；2）通过两阶段过程识别前景令牌：语义种子发现和基于值向量相似度的上下文细化；3）保留IVC令牌和语义相关前景令牌。

Result: 在4个代表性LVLM和20个多样化基准测试中，IVC-Prune减少约50%视觉令牌的同时保持≥99%的原始性能，在某些基准上甚至实现了性能提升。

Conclusion: IVC-Prune通过识别和保留对空间推理至关重要的隐式视觉坐标令牌，解决了现有视觉令牌剪枝方法在空间推理任务上的局限性，实现了高效且有效的视觉令牌压缩。
...
Conclusion: GBO为弱监督视频时序定位提供了理论严谨且实用的推理框架，无需训练且兼容多种高斯提案架构，有效解决了现有方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 现有AI模型在肌肉骨骼疾病检测中多为任务特定、依赖标注、泛化能力有限，临床需要能跨疾病和解剖区域泛化的基础模型，但公开数据集规模小且多样性不足。

Method: 使用自监督学习在120万张多样化、包含丰富病理条件的肌肉骨骼X光片上训练SKELEX基础模型，评估了12项下游诊断任务，并开发了基于异常定位的可解释骨肿瘤预测模型。

Result: SKELEX在骨折检测、骨关节炎分级和骨肿瘤分类等任务中普遍优于基线模型，展示了零样本异常定位能力，能生成识别病理区域的误差图，骨肿瘤预测模型在独立外部数据集上保持稳健性能。

Conclusion: SKELEX为肌肉骨骼成像提供了一个可扩展、标签高效且泛化能力强的AI框架，为临床转化和肌肉骨骼放射学中的数据高效研究奠定了基础。
...
TL;DR: 该论文提出了一种新的语义对应方法，用带有Gromov Wasserstein空间平滑先验的最优传输算法替代标准最近邻匹配，显著提升了DINOv2基线的性能，在保持竞争力的同时实现了5-10倍的计算效率提升。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的语义对应方法需要结合DINOv2和Stable Diffusion的特征，虽然性能优秀但计算成本高昂。DINOv2特征准确但稀疏，Stable Diffusion特征具有空间一致性但需要大型基础模型。研究旨在寻找更高效的替代方案。
...
Conclusion: 通过引入最优传输算法和空间平滑先验，可以在不依赖计算昂贵的Stable Diffusion特征的情况下实现高质量的语义对应，为语义匹配任务提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法（如裁剪、旋转）虽然能减少过拟合，但多样性有限。生成模型（如条件扩散、few-shot NeRFs）能合成更丰富、更真实的数据，但不当的增强可能损害性能。需要自动化方法找到与任务匹配的最佳增强策略。

Method: EvoAug采用进化算法自动学习任务特定的增强策略，引入随机增强树的概念，分层组合多种增强操作，实现结构化、自适应的图像变换。结合生成模型提供更丰富的增强多样性。

Result: 在细粒度分类和少样本学习任务中表现出色，即使在低数据环境下也能发现符合领域知识的增强策略，证明了学习生成增强的潜力。

Conclusion: EvoAug展示了生成模型在数据增强中的潜力，通过自动化学习任务特定的增强策略，为鲁棒模型训练开辟了新可能性，特别是在数据稀缺场景下。
...
TL;DR: 该论文提出了一种基于扩散模型的3D人体姿态估计框架，能够从单张2D图像中采样生成多个合理的3D姿态，解决了深度模糊和遮挡问题，无需成对的2D-3D训练数据。


<details>
  <summary>Details</summary>
Motivation: 传统3D姿态估计方法存在两个主要问题：1) 假设存在确定性映射，只估计单一姿态，而实际上由于深度模糊和遮挡，可能存在多个合理的3D姿态；2) 基于机器学习的方法需要大量成对的2D-3D数据进行训练，且泛化能力有限。

Method: 提出基于扩散模型的框架，采用条件生成引导方法：首先在纯3D数据上训练无条件扩散模型，然后使用2D关键点检测器的热图梯度来引导采样，生成与输入2D图像一致的多个合理3D姿态。

Result: 在Human 3.6M数据集上，使用best-of-m多假设评估方法取得了无需成对2D-3D数据训练方法中的最优性能；在MPI-INF-3DHP和3DPW数据集上展示了良好的泛化能力；框架还可用于姿态生成和姿态补全等新任务。

Conclusion: 该扩散模型框架能够从概率分布中采样多个合理的3D姿态，解决了姿态估计的不确定性问题，无需成对训练数据，具有良好的泛化能力和任务灵活性，为3D人体姿态估计提供了新思路。
...
TL;DR: SwiftVLM是一种新的视觉语言模型剪枝方法，通过"旁路"机制保留被剪枝的视觉token，在后续层重新评估其重要性，解决了现有方法在细粒度任务上的性能下降问题。
...
Method: 提出新的剪枝范式"旁路"，保留未选择的视觉token并将其转发到后续剪枝阶段重新评估。基于此提出SwiftVLM，这是一种简单且无需训练的方法，在具有强视觉token选择能力的模型特定层执行剪枝，同时支持跨层的独立剪枝决策。

Result: 在多个视觉语言模型和基准测试上的实验表明，SwiftVLM始终优于现有剪枝策略，实现了更优的准确率-效率权衡和更可靠的视觉token选择行为。

Conclusion: SwiftVLM通过引入旁路机制避免了过早剪枝导致的不可逆关键信息丢失，为视觉语言模型的高效推理提供了更有效的解决方案。
...
TL;DR: 提出了一种无需群体活动标注的人机协同适应方法，用于改进群体活动视频检索性能。该方法通过自监督预训练群体活动特征空间，然后通过交互式微调让用户标注视频来优化检索效果。


<details>
  <summary>Details</summary>
Motivation: 传统群体活动视频检索方法通常需要预先定义的群体活动类别标注，这在实际应用中成本高昂且不灵活。本文旨在开发一种无需群体活动标注的方法，通过人机协同适应来提高检索性能。

Method: 1. 自监督预训练：基于群体活动相似性预训练群体活动特征空间；2. 交互式微调：用户标注从数据库中选择的视频为正例或负例；3. 对比学习更新：使用标注视频通过对比学习微调特征空间，使正例视频更接近查询视频，负例视频更远离查询视频；4. 数据高效视频选择：提出高效选择需要标注的视频的方法。

Result: 在两个团队运动数据集上的综合实验结果表明，该方法显著提高了检索性能。消融研究也证明了人机协同适应中的多个组件对性能提升的贡献。

Conclusion: 本文提出的人机协同适应方法能够在无需群体活动标注的情况下，有效提高群体活动视频检索性能，通过自监督预训练和交互式微调实现了灵活且高效的检索系统。
...
TL;DR: LSGQuant是一种针对单步扩散模型视频超分辨率的层敏感度引导量化方法，通过动态范围自适应量化器、方差导向层训练策略和量化感知优化，在保持性能的同时显著减小模型大小和计算成本。


<details>
  <summary>Details</summary>
Motivation: 单步扩散模型在真实世界视频超分辨率中表现出色，但基于扩散变换器的大模型尺寸和高计算成本限制了实际应用。传统低比特量化方法面临输入潜在表示高动态范围和不同层行为多样性的挑战。

Method: 提出LSGQuant方法：1) 动态范围自适应量化器(DRAQ)适应视频token激活；2) 通过分析校准中的层统计来估计层敏感度，实施方差导向层训练策略(VOLTS)；3) 引入量化感知优化(QAO)联合优化量化分支和保留的高精度分支。

Result: 大量实验表明，该方法在保持全精度原始模型性能的同时，显著超过现有量化技术。代码已开源。

Conclusion: LSGQuant为单步扩散模型视频超分辨率提供了一种有效的量化压缩方案，解决了高动态范围和层行为多样性带来的挑战，实现了性能与效率的良好平衡。
...
Method: 提出TempLoc框架：1) 全局坐标估计模块预测点级全局坐标和不确定性；2) 先验坐标生成模块通过注意力机制估计帧间点对应关系；3) 不确定性引导的坐标融合模块以端到端方式整合两种预测，获得更一致的6-DoF姿态

Result: 在NCLT和Oxford Robot-Car基准测试中，TempLoc大幅超越最先进方法，证明了时序感知对应建模在LiDAR重定位中的有效性
...
TL;DR: Hand3R：首个从单目视频进行联合4D手部-场景重建的在线框架，结合预训练手部专家和4D场景基础模型，实现准确手部网格和密集度量尺度场景几何的同时重建。
...
Conclusion: Hand3R是首个实现联合4D手部-场景重建的在线框架，为理解物理交互提供了重要工具。
...
Method: 提出VIRAL框架：1）将ICL重新定义为通过视觉类比的条件生成；2）使用角色感知多图像条件适应冻结的Diffusion Transformer；3）引入Mixture-of-Experts LoRA减轻跨任务梯度干扰；4）构建大规模视觉上下文数据集

Result: VIRAL在实验中优于现有方法，验证了统一的视觉ICL范式能够处理大多数视觉任务，包括开放域编辑

Conclusion: 通过视觉类比重新定义ICL为条件生成是有效的，VIRAL框架展示了统一的视觉ICL范式在多样化视觉任务中的潜力
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要大规模高质量多视角驾驶视频数据，但现有世界模型在生成数据时存在身份漂移问题，即同一对象在不同帧中出现外观或类别变化，缺乏实例级时间约束。
...
<details>
  <summary>Details</summary>
Motivation: 高性能跟踪器通常处理速度较慢，难以在资源受限设备上部署。需要解决跟踪性能与推理速度之间的权衡问题。

Method: 提出FARTrack框架，包含两个核心技术：1) 任务特定自蒸馏：通过逐层蒸馏任务特定token实现模型压缩，避免手动分配教师-学生层对；2) 帧间自回归稀疏化：顺序压缩多个模板，学习时间全局最优稀疏化策略。

Result: 在GOT-10k上达到70.6%的AO，实时运行。最快模型在GPU上达到343 FPS，CPU上达到121 FPS，表现出卓越的速度和竞争力性能。

Conclusion: FARTrack通过自回归跟踪框架，成功解决了高性能跟踪器速度慢的问题，实现了在保持竞争力的跟踪性能的同时，在各种设备上高效执行。
...
TL;DR: 本文提出Spiral RoPE，一种改进的二维位置编码方法，通过多方向旋转编码解决标准轴向RoPE只能处理轴对齐方向的限制，在多种视觉任务中提升性能。


<details>
  <summary>Details</summary>
Motivation: 标准轴向2D RoPE将二维空间位置分解为水平和垂直分量，这种轴对齐的方向约束限制了模型对自然图像中存在的倾斜空间关系的建模能力。作者认为这种方向限制是标准2D RoPE的根本缺陷。

Method: 提出Spiral RoPE，将嵌入通道划分为多个与均匀分布方向相关联的组。每个组根据图像块位置在其对应方向上的投影进行旋转，从而实现对水平和垂直轴之外的空间关系编码。

Result: 在分类、分割和生成等广泛的视觉任务中，Spiral RoPE一致性地提升了性能。注意力图的定性分析显示，Spiral RoPE在语义相关对象上表现出更集中的激活，并能更好地尊重局部对象边界。

Conclusion: Spiral RoPE通过多方向位置编码克服了标准轴向2D RoPE的方向限制，证明了在视觉Transformer中多方向位置编码的重要性，为视觉任务提供了更有效的空间关系建模方法。
...
TL;DR: EventFlash：一种基于事件的高效多模态大语言模型，通过时空令牌稀疏化减少数据冗余，实现12.4倍推理加速


<details>
  <summary>Details</summary>
Motivation: 当前基于事件的MLLM通常采用密集的图像式处理范式，忽视了事件流的时空稀疏性，导致计算成本高昂。需要开发更高效的模型来利用事件数据的稀疏特性。

Method: 1) 构建EventMind大规模数据集（50万+指令集）；2) 自适应时间窗口聚合模块进行高效时间采样；3) 稀疏密度引导注意力模块提升空间令牌效率

Result: EventFlash相比基线（EventFlash-Zero）实现了12.4倍的吞吐量提升，同时保持可比性能。支持长达1000个bin的事件流处理，远超EventGPT的5-bin限制
...
TL;DR: InstaDrive是一个增强驾驶视频真实性的框架，通过实例流引导器和空间几何对齐器解决世界模型在实例级时间一致性和空间几何保真度方面的不足，在nuScenes数据集上实现最先进的视频生成质量，并利用CARLA模拟安全关键场景进行评估。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶依赖高质量的大规模多视角驾驶视频训练模型，但现有的世界模型在生成真实驾驶视频时存在两个主要问题：1）难以保持实例级的时间一致性；2）空间几何保真度不足。这些限制影响了生成视频的真实性和下游任务的性能。

Method: InstaDrive框架包含两个核心组件：1）实例流引导器：提取并传播实例特征跨帧，强制时间一致性，保持实例身份随时间不变；2）空间几何对齐器：改进空间推理，确保精确的实例定位，并显式建模遮挡层次结构。此外，利用CARLA自动驾驶系统程序化和随机地模拟罕见但安全关键的驾驶场景。

Result: 在nuScenes数据集上实现了最先进的视频生成质量，同时增强了下游自动驾驶任务的性能。通过CARLA模拟的多样化地图和区域中的安全关键场景，能够对自动驾驶系统进行严格的安全评估。

Conclusion: InstaDrive通过引入实例感知机制，有效解决了驾驶视频生成中的时间一致性和空间几何保真度问题，不仅提升了视频生成质量，还改善了自动驾驶任务的性能，并为系统安全评估提供了有效工具。
...
TL;DR: A3-TTA是一种测试时自适应框架，通过锚点引导监督构建可靠的伪标签，显著提升图像分割模型在域偏移下的性能，在医学和自然图像分割任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于伪标签的测试时自适应方法通常依赖扰动集成启发式方法，这些方法缺乏分布基础，产生不稳定的训练信号，导致错误累积和灾难性遗忘问题。

Method: 提出A3-TTA框架：1）使用类别紧凑密度度量识别目标域中预测良好的图像作为锚点；2）锚点作为稳定参考指导伪标签生成；3）通过语义一致性和边界感知熵最小化进行正则化；4）引入自适应指数移动平均策略减轻标签噪声并稳定模型更新。

Result: 在医学图像（心脏结构和前列腺分割）和自然图像上评估，A3-TTA相比源模型将平均Dice分数提高了10.40到17.68个百分点，优于多个最先进的TTA方法，在连续TTA任务中也表现出色，具有强大的抗遗忘能力。

Conclusion: A3-TTA通过锚点引导监督构建可靠的伪标签，有效解决了现有TTA方法中的不稳定训练信号问题，显著提升了分割模型在域偏移下的适应能力，并在连续域适应中表现出良好的抗遗忘特性。
...
TL;DR: LEVIO是一个针对超低功耗计算平台优化的视觉惯性里程计（VIO）系统，能够在资源受限的硬件上实现实时六自由度运动跟踪，功耗低于100mW。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的VIO系统计算需求过高，无法在微无人机和智能眼镜等资源受限的硬件上运行，需要开发适用于超低功耗平台的运动跟踪解决方案。

Method: 采用硬件-软件协同优化方法，结合ORB特征跟踪和束调整等成熟VIO组件，设计计算效率高的架构，强调并行化和低内存使用，适用于嵌入式微控制器和低功耗SoC。

Result: 在并行处理的超低功耗RISC-V SoC上实现20 FPS的实时性能，功耗低于100 mW，在公开VIO数据集上进行了基准测试，在效率和准确性之间取得了良好平衡。

Conclusion: LEVIO为资源受限平台提供了高效可行的VIO解决方案，通过开源实现促进了可重复性和采用，为移动机器人和增强现实应用提供了基础设施需求低的运动跟踪能力。
...
TL;DR: FOCUS是一个基于基础模型的端到端自动化3D OCT视网膜疾病诊断框架，通过图像质量评估、异常检测和多疾病分类的统一流程，实现从图像到诊断的完整自动化。


<details>
  <summary>Details</summary>
Motivation: 尽管OCT在视网膜疾病诊断中具有高分辨率和三维成像优势，但临床实践中仍受限于多阶段工作流程和传统的单切片单任务AI模型，无法实现完全自动化诊断。

Method: FOCUS采用基础模型驱动的框架，依次使用EfficientNetV2-S进行图像质量评估，然后使用微调的视觉基础模型进行异常检测和多疾病分类，并通过统一的适应性聚合方法将2D切片级预测整合为全面的3D患者级诊断。

Result: 在3,300名患者（40,672个切片）上训练测试，并在1,345名患者（18,498个切片）上进行外部验证，FOCUS在质量评估（99.01%）、异常检测（97.46%）和患者级诊断（94.39%）方面获得高F1分数。人机对比显示FOCUS在异常检测（95.47% vs 90.91%）和多疾病诊断（93.49% vs 91.35%）方面与专家表现相当，且效率更高。

Conclusion: FOCUS实现了从图像到诊断的完整自动化流程，代表了无人眼科医学的重要进展，为自主筛查提供了经过验证的蓝图，有助于提高人群规模视网膜护理的可及性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 生成数据增强在实际应用中容易受到干净标签后门攻击，但现有像素级攻击方法（如COMBAT）在生成图像上攻击成功率低，需要探索潜在特征层面的攻击方法

Method: 提出InvLBA方法，通过潜在空间扰动进行不可见的干净标签后门攻击，理论上证明了其泛化性能保证

Result: 在多个数据集上实验表明，该方法平均提高攻击成功率46.43%，几乎不影响干净准确率，对SOTA防御方法具有高鲁棒性

Conclusion: 潜在特征层面的攻击比像素级攻击更有效，InvLBA方法为生成数据增强的安全问题提供了新的攻击视角
...
### [44] [PWAVEP: Purifying Imperceptible Adversarial Perturbations in 3D Point Clouds via Spectral Graph Wavelets](https://arxiv.org/abs/2602.03333)
*Haoran Li,Renyang Liu,Hongjia Liu,Chen Wang,Long Yin,Jian Xu*

Main category: cs.CV

TL;DR: 提出PWAVEP，一种针对3D点云对抗攻击的即插即用防御机制，通过在谱域分析扰动与高频分量的关系，采用分层策略消除异常点并抑制对抗噪声。
...
Method: 提出PWAVEP净化框架：1）计算每个点的谱图小波域显著性分数和局部稀疏度分数；2）采用分层策略：消除最显著的难以恢复的对抗异常点；3）对中等显著点应用谱滤波，利用图小波变换衰减目标点相关的高频系数以抑制对抗噪声。

Result: 广泛评估表明，PWAVEP相比现有方法实现了更高的准确性和鲁棒性，在3D点云净化方面达到了最先进的性能。

Conclusion: PWAVEP是一种即插即用、非侵入式的防御机制，通过谱域分析和分层净化策略有效对抗3D点云的对抗攻击，推动了该领域的发展。
...
### [45] [Composable Visual Tokenizers with Generator-Free Diagnostics of Learnability](https://arxiv.org/abs/2602.03339)
*Bingchen Zhao,Qiushan Guo,Ye Wang,Yixuan Huang,Zhonghua Zhai,Yu Tian*

Main category: cs.CV

TL;DR: CompTok是一个学习视觉tokenizer的训练框架，通过token条件扩散解码器和InfoGAN风格目标增强token的组合性，支持token交换实现语义编辑，并在图像生成任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉tokenizer在组合性控制方面存在不足，token可能被解码器忽略，限制了token在语义编辑和组合生成中的应用潜力。

Method: 1. 使用token条件扩散解码器；2. 采用InfoGAN风格目标，训练识别模型预测用于条件解码的token，防止解码器忽略任何token；3. 通过交换图像间的token子集进行训练，增强组合控制；4. 对无配对交换生成应用流正则化保持自然图像分布。

Result: 1. 在图像类别条件生成任务中达到最先进性能；2. token支持图像间交换实现高级语义编辑；3. 提出的两个token空间度量显示CompTok在组合性和生成器学习难度方面均有改善。

Conclusion: CompTok框架成功增强了视觉tokenizer的组合性，不仅提升了生成性能，还支持语义编辑，为token空间的可解释性和生成器训练提供了新见解。
...
### [46] [Tiled Prompts: Overcoming Prompt Underspecification in Image and Video Super-Resolution](https://arxiv.org/abs/2602.03342)
...
<details>
  <summary>Details</summary>
Motivation: 现代超分辨率管道通常依赖潜在分块来处理高分辨率，但单一全局标题会导致提示不足问题。粗糙的全局提示经常错过局部细节（提示稀疏性）并提供局部不相关的指导（提示误导），这些问题可能被无分类器指导放大。
...
Conclusion: Tiled Prompts框架通过局部文本条件指导有效解决了超分辨率中的提示不足问题，为图像和视频超分辨率提供了更精确的语义先验。
...
### [47] [Z3D: Zero-Shot 3D Visual Grounding from Images](https://arxiv.org/abs/2602.03361)
*Nikita Drozdov,Andrey Lemeshko,Nikita Gavrilov,Anton Konushin,Danila Rukhovich,Maksim Kolodiazhnyi*

Main category: cs.CV

TL;DR: Z3D：首个无需几何监督或物体先验的零样本3D视觉定位方法，仅使用多视角图像，通过零样本3D实例分割和基于提示的分割推理实现


<details>
  <summary>Details</summary>
Motivation: 解决现有零样本3D视觉定位方法性能显著下降的关键瓶颈，探索仅从多视角图像进行零样本3D物体定位，无需几何监督或物体先验

Method: 提出Z3D通用定位流程：1）使用最先进的零样本3D实例分割方法生成高质量3D边界框提议；2）通过基于提示的分割推理，充分利用现代视觉语言模型的完整能力；可灵活操作于多视角图像，可选加入相机位姿和深度图

Result: 在ScanRefer和Nr3D基准测试中，Z3D在零样本方法中实现了最先进的性能

Conclusion: Z3D展示了仅从多视角图像进行零样本3D视觉定位的可行性，通过解决关键瓶颈实现了显著性能提升，为无需几何监督的3D场景理解提供了新途径
...
### [48] [Symbol-Aware Reasoning with Masked Discrete Diffusion for Handwritten Mathematical Expression Recognition](https://arxiv.org/abs/2602.03370)
*Takaya Kawakatsu,Ryo Ishiyama*

Main category: cs.CV

TL;DR: 本文提出了一种基于离散扩散框架的手写数学表达式识别方法，通过迭代符号精炼而非序列生成来解决自回归模型的曝光偏差和句法不一致问题。


<details>
  <summary>Details</summary>
Motivation: 手写数学表达式识别需要处理多样化的符号和二维结构布局，但自回归模型存在曝光偏差和句法不一致的问题。传统序列生成方法难以有效捕捉复杂的结构关系。

Method: 采用离散扩散框架，将HMER重新定义为迭代符号精炼过程。通过多步重掩码技术逐步精炼符号和结构关系，消除因果依赖。引入符号感知分词和随机掩码互学习机制，增强句法对齐和对笔迹多样性的鲁棒性。

Result: 在MathWriting基准测试中，该方法实现了5.56%的字符错误率和60.42%的精确匹配率，优于强大的Transformer模型和商业基线。在CROHME 2014-2023数据集上也表现出一致的性能提升。

Conclusion: 离散扩散框架为结构感知的视觉识别提供了新的范式，超越了传统的生成建模方法，能够更好地处理手写数学表达式的复杂结构关系。
...
### [49] [Multi-Resolution Alignment for Voxel Sparsity in Camera-Based 3D Semantic Scene Completion](https://arxiv.org/abs/2602.03371)
...
Method: 提出多分辨率对齐（MRA）方法：1）多分辨率视图变换器模块，将2D图像特征投影到多分辨率3D特征并进行场景级对齐；2）立方语义各向异性模块，识别每个体素的实例级语义显著性；3）关键分布对齐模块，选择关键体素作为实例级锚点，通过循环损失实现跨分辨率的关键特征分布一致性监督。

Result: 该方法在SemanticKITTI和nuScenes数据集上验证有效，代码已在GitHub开源。

Conclusion: MRA方法通过多分辨率特征对齐缓解了体素稀疏性问题，提高了基于相机的3D语义场景补全的优化效率和性能。
...
### [50] [SLIM-Diff: Shared Latent Image-Mask Diffusion with Lp loss for Data-Scarce Epilepsy FLAIR MRI](https://arxiv.org/abs/2602.03372)
...
Method: 提出SLIM-Diff模型：1) 使用单共享瓶颈U-Net从2通道图像+掩码表示中强制解剖结构和病变几何的紧密耦合；2) 通过可调Lp目标进行损失几何调优；3) 比较x0预测和ε预测在不同Lp几何下的表现。

Result: 实验表明：x0预测是联合合成的最佳选择；分数次二次惩罚(L1.5)提高图像保真度，而L2损失更好地保留病变掩码形态。

Conclusion: SLIM-Diff通过紧凑的联合扩散模型设计，有效解决了FCD病变生成中的不稳定性和记忆化问题，为医学图像生成提供了新方法。
...
### [51] [Unifying Watermarking via Dimension-Aware Mapping](https://arxiv.org/abs/2602.03373)
*Jiale Meng,Runyi Hu,Jie Zhang,Zheming Lu,Ivor Tsang,Tianwei Zhang*

Main category: cs.CV

TL;DR: DiM是一个新的多维水印框架，将水印建模为维度感知映射问题，统一了现有水印方法。通过改变嵌入和提取的维度配置，可以实现不同的水印功能。


<details>
  <summary>Details</summary>
Motivation: 现有深度水印方法虽然共享相似的编码器-解码器架构，但在功能行为上差异很大。需要一个统一的框架来理解这些差异，并探索不同维度配置如何影响水印功能。

Method: 提出DiM框架，将水印信息建模为不同维度的载荷：一维二进制消息、二维空间掩码、三维时空结构。框架核心是维度感知映射，相同维度映射保留载荷结构，跨维度映射实现空间或时空定位。

Result: 在视频领域实例化DiM，实验表明仅改变嵌入和提取维度（无需架构更改）就能实现不同水印功能，包括时空篡改定位、局部嵌入控制和帧干扰下的时间顺序恢复。

Conclusion: DiM提供了一个统一的多维水印框架，揭示了维度配置如何决定水印功能，为设计具有特定功能的水印系统提供了新视角。
...
### [52] [From Vicious to Virtuous Cycles: Synergistic Representation Learning for Unsupervised Video Object-Centric Learning](https://arxiv.org/abs/2602.03390)
...
<details>
  <summary>Details</summary>
Motivation: 无监督物体中心学习模型（特别是基于槽的架构）在分解复杂场景方面表现出色，但基于重建的训练导致编码器的锐利高频注意力图与解码器的空间一致但模糊的重建图之间存在根本冲突，形成恶性循环。

Method: 提出协同表示学习（SRL），建立编码器与解码器相互精炼的良性循环：利用编码器的锐利度去模糊解码器输出的语义边界，同时利用解码器的空间一致性去噪编码器特征，并通过槽正则化目标的预热阶段稳定训练。

Result: SRL在视频物体中心学习基准测试中取得了最先进的结果，通过弥合编码器和解码器之间的表示差距，显著提升了性能。

Conclusion: SRL通过建立编码器与解码器之间的协同循环，成功解决了无监督物体中心学习中注意力图与重建图之间的冲突，为物体中心表示学习提供了有效的新方法。
...
### [53] [Socratic-Geo: Synthetic Data Generation and Geometric Reasoning via Multi-Agent Interaction](https://arxiv.org/abs/2602.03414)
*Zhengbo Jiao,Shaobo Wang,Zifan Zhang,Wei Wang,Bing Zhao,Hu Wei,Linfeng Zhang*

Main category: cs.CV

TL;DR: Socratic-Geo是一个完全自主的多智能体框架，通过动态耦合数据合成与模型学习来解决MLLMs在几何推理上的瓶颈。它使用教师智能体生成高质量图像-文本对，求解器智能体优化推理，生成器智能体学习图像生成能力，仅用少量种子数据就在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言理解方面取得显著进展，但在几何推理方面仍存在瓶颈。主要问题是高质量图像-文本对的极度稀缺：人工标注成本过高，而自动化方法无法保证保真度和训练效果。现有方法要么被动适应可用图像，要么采用效率低下的随机探索加过滤方法，将生成与学习需求脱钩。

Method: 提出Socratic-Geo框架，包含三个智能体：1) 教师智能体：生成参数化Python脚本，通过反思反馈确保图像-文本对质量；2) 求解器智能体：通过偏好学习优化推理，失败路径指导教师进行针对性增强；3) 生成器智能体：在积累的"图像-代码-指令"三元组上学习图像生成能力，将程序化绘图智能蒸馏到视觉生成中。

Result: 仅从108个种子问题开始，Socratic-Solver在六个基准测试中使用四分之一基线数据达到49.11分，超越强基线2.43分。Socratic-Generator在GenExam上达到42.4%，为开源模型建立了新的SOTA，超越了Seedream-4.0（39.8%）并接近Gemini-2.5-Flash-Image（43.1%）。

Conclusion: Socratic-Geo通过多智能体交互实现了数据合成与模型学习的动态耦合，有效解决了MLLMs在几何推理中的数据稀缺问题。该框架不仅显著提升了推理性能，还成功将程序化绘图智能蒸馏到视觉生成中，为高质量多模态数据生成提供了新范式。
...
### [54] [ConsistentRFT: Reducing Visual Hallucinations in Flow-based Reinforcement Fine-Tuning](https://arxiv.org/abs/2602.03425)
*Xiaofeng Tan,Jun Liu,Yuanting Fan,Bin-Bin Gao,Xi Jiang,Xiaochen Chen,Jinlong Peng,Chengjie Wang,Hongsong Wang,Feng Zheng*

Main category: cs.CV

TL;DR: 论文提出ConsistentRFT框架，通过动态粒度采样和一致性策略梯度优化，解决强化微调在流模型中产生的视觉幻觉问题，显著降低低层和高层感知幻觉。


<details>
  <summary>Details</summary>
Motivation: 强化微调（RFT）在流模型中对齐偏好很重要，但常导致视觉幻觉，如过度优化细节和语义不对齐。本研究旨在探索视觉幻觉产生原因及减少方法。

Method: 从统一视角分析RFT方法，发现问题源于探索和利用两方面：SDE采样中探索有限导致过度关注局部细节而忽视全局语义；策略梯度中的轨迹模仿过程扭曲模型基础向量场和跨步一致性。提出ConsistentRFT框架，包含动态粒度采样机制平衡全局语义与局部细节的探索，以及一致性策略梯度优化保持模型一致性。

Result: 实验表明ConsistentRFT显著减少视觉幻觉，平均降低49%的低层感知幻觉和38%的高层感知幻觉。在域外指标上优于其他RFT方法，相比FLUX1.dev基线下降-0.4%，ConsistentRFT提升5.1%。

Conclusion: ConsistentRFT通过解决探索-利用平衡和保持模型一致性，有效缓解了强化微调中的视觉幻觉问题，在多个评估指标上表现优异。
...
### [55] [Hierarchical Concept-to-Appearance Guidance for Multi-Subject Image Generation](https://arxiv.org/abs/2602.03448)
*Yijia Xu,Zihao Wang,Jinshi Cui*

Main category: cs.CV

TL;DR: 本文提出了一种分层概念到外观引导框架，通过VAE dropout训练和对应感知掩码注意力机制，显著提升多主体图像生成的身份一致性和组合控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有多主体图像生成方法依赖扩散模型隐式关联文本提示与参考图像，导致身份不一致和组合控制有限。需要提供从高层概念到细粒度外观的显式结构化监督。

Method: 提出分层概念到外观引导框架：1) 概念层面：采用VAE dropout训练策略，随机省略参考VAE特征，迫使模型更多依赖视觉语言模型的鲁棒语义信号；2) 外观层面：将VLM推导的对应关系集成到扩散变换器的对应感知掩码注意力模块中，限制每个文本标记仅关注匹配的参考区域。

Result: 大量实验表明，该方法在多主体图像生成任务上达到最先进性能，显著提升了提示跟随能力和主体一致性。

Conclusion: 分层概念到外观引导框架通过显式的概念级和外观级监督，有效解决了多主体图像生成中的身份不一致问题，实现了更可靠的多主体组合控制。
...
### [56] [Contextualized Visual Personalization in Vision-Language Models](https://arxiv.org/abs/2602.03454)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型缺乏将视觉输入与用户积累的视觉-文本上下文关联的能力，无法基于用户特定经验生成个性化响应，需要解决情境化视觉个性化挑战。

Method: 提出CoViP统一框架，将个性化图像描述作为核心任务，通过强化学习后训练和caption增强生成来提升情境化视觉个性化能力，并引入诊断评估排除文本捷径。

Result: 实验表明现有开源和专有VLM存在显著局限，而CoViP不仅改进个性化图像描述，还在下游个性化任务中获得整体提升，成为实现稳健可泛化情境化视觉个性化的关键阶段。

Conclusion: CoViP框架为解决视觉语言模型在关联用户视觉体验方面的不足提供了有效方案，是实现情境化视觉个性化的重要进展。
...
### [57] [Inlier-Centric Post-Training Quantization for Object Detection Models](https://arxiv.org/abs/2602.03472)
*Minsu Kim,Dongyeun Lee,Jaemyung Yu,Jiwan Hur,Giseop Kim,Junmo Kim*

Main category: cs.CV

TL;DR: InlierQ是一种面向内点的后训练量化方法，通过分离激活中的异常值来减少量化误差，适用于2D/3D目标检测任务。


<details>
  <summary>Details</summary>
Motivation: 目标检测计算需求大，部署时速度慢、功耗高，需要量化。但背景杂波和传感器噪声等任务无关形态会产生冗余激活（异常值），这些异常值扩大激活范围、扭曲分布，使比特分配复杂化并削弱信息特征的保留。

Method: InlierQ计算梯度感知的体积显著性分数，将每个体积分类为内点或异常值，使用期望最大化算法拟合这些分数的后验分布。该方法抑制异常值同时保留信息特征，无需标签、即插即用，仅需64个校准样本。

Result: 在COCO和nuScenes基准测试中，对基于相机（2D和3D）和基于LiDAR（3D）的目标检测都显示出量化误差的一致减少。

Conclusion: InlierQ通过内点中心化的方法有效分离和处理激活中的异常值，改善了目标检测模型的量化效果，降低了量化误差。
...
### [58] [Decoupling Skeleton and Flesh: Efficient Multimodal Table Reasoning with Disentangled Alignment and Structure-aware Guidance](https://arxiv.org/abs/2602.03491)
*Yingjie Zhu,Xuefeng Bai,Kehai Chen,Yang Xiang,Youcheng Pan,Xiaoqiang Zhou,Min Zhang*

Main category: cs.CV

TL;DR: 提出DiSCo框架解耦表格结构-内容对齐，并基于此构建Table-GLS框架进行全局到局部结构引导的表格推理，无需昂贵监督训练或外部工具


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在表格图像推理上面临挑战，因为复杂布局和紧密耦合的结构-内容信息。现有解决方案依赖昂贵的监督训练、强化学习或外部工具，限制了效率和可扩展性

Method: 1. 提出DiSCo框架：解耦结构-内容对齐，将结构抽象与语义基础分离；2. 基于DiSCo构建Table-GLS框架：通过结构化探索和证据基础推理进行表格推理

Result: 在多个基准测试上的广泛实验表明，该框架有效提升了LVLM的表格理解和推理能力，特别是在未见过的表格结构上表现出良好的泛化能力

Conclusion: 提出的框架以最小标注和无外部工具的方式成功适应LVLM进行表格推理，解决了现有方法的效率和可扩展性限制
...
### [59] [Making Avatars Interact: Towards Text-Driven Human-Object Interaction for Controllable Talking Avatars](https://arxiv.org/abs/2602.01538)
...
TL;DR: 提出InteractAvatar框架解决接地人机交互(GHOI)视频生成挑战，通过双流架构分离感知规划与视频合成，生成与文本对齐的交互动作和生动说话虚拟人


<details>
  <summary>Details</summary>
Motivation: 现有方法能生成简单人体运动的全身说话虚拟人，但扩展到接地人机交互(GHOI)仍具挑战，需要虚拟人与周围物体进行文本对齐的交互，这需要环境感知并面临控制质量困境

Method: 提出InteractAvatar双流框架：1)感知与交互模块(PIM)利用检测增强环境感知，生成文本对齐交互动作；2)音频交互感知生成模块(AIM)合成执行物体交互的生动说话虚拟人；通过运动-视频对齐器实现并行协同生成

Result: 建立了GroundedInter基准用于评估GHOI视频生成，大量实验和比较证明了该方法在生成接地人机交互说话虚拟人方面的有效性

Conclusion: InteractAvatar通过解耦感知规划与视频合成，有效解决了GHOI生成中的环境感知和控制质量困境，能够生成文本对齐的交互动作和生动的说话虚拟人视频
...
TL;DR: 提出逻辑异常分类任务和LogiCls框架，将异常检测与细粒度违规分类统一，通过视觉语言模型分解复杂逻辑约束为可验证子查询


<details>
  <summary>Details</summary>
Motivation: 传统异常检测仅做二元判断，无法指出具体违反的逻辑规则，对质量保证价值有限；需要统一异常检测和违规分类的解决方案

Method: 提出LogiCls视觉语言框架，将复杂逻辑约束分解为可验证子查询序列；设计数据中心的指令合成管道，生成思维链监督；采用难度感知重采样策略稳定训练

Result: LogiCls在工业逻辑异常分类中表现出鲁棒性、可解释性和准确性，能同时提供违规类别预测和证据链

Conclusion: 逻辑异常分类任务统一了异常检测和违规分类，LogiCls框架通过视觉语言模型和思维链监督实现了有效的逻辑敏感推理
...
TL;DR: 本文提出了首个结合自回归和扩散的统一3D理解与生成框架，通过轻量级transformer桥接LLM特征空间和3D扩散模型条件空间，在保持各自优势的同时实现跨模态信息交互。


<details>
  <summary>Details</summary>
Motivation: 现有统一3D任务的自回归范式存在性能下降和训练成本过高的问题，需要一种既能有效交互生成与理解信息，又能最小化损害各自能力并利用预训练模型降低成本的解决方案。

Method: 采用自回归下一token预测范式处理3D理解任务，连续扩散范式处理3D生成任务，通过轻量级transformer桥接大语言模型特征空间和3D扩散模型条件空间，实现跨模态信息交换。

Result: 在多样化的3D理解和生成基准测试中达到最先进性能，同时在3D编辑任务中表现出色，证明了AR+扩散统一模型作为构建通用3D智能的潜力。

Conclusion: 结合自回归和扩散的统一框架是构建更通用3D智能的有前景方向，能够在保持各自优势的同时实现3D理解与生成的有效协同。
...
TL;DR: CDGS是一个受约束的动态高斯泼溅框架，通过预算控制器在训练期间强制执行用户定义的高斯预算，实现硬件约束下的最优渲染质量


<details>
  <summary>Details</summary>
Motivation: 动态高斯泼溅技术面临部署困境：无约束的密度化导致内存消耗过大不兼容边缘设备，而启发式剪枝无法在预设高斯预算下达到最优渲染质量

Method: 提出约束动态高斯泼溅(CDGS)框架，将动态场景重建表述为预算约束优化问题；引入可微分预算控制器作为核心优化驱动，基于多模态统一重要性分数融合几何、运动和感知线索；解耦静态和动态元素优化，采用自适应分配机制；实施三阶段训练策略；结合双模混合压缩方案

Result: CDGS严格遵循硬件约束（误差<2%），推动率失真性能的帕累托前沿，在不同容量限制下提供最优渲染质量，相比最先进方法实现超过3倍的压缩

Conclusion: CDGS通过预算约束优化框架解决了动态高斯泼溅的部署难题，在严格硬件约束下实现了最优渲染质量，为边缘设备上的高质量4D重建提供了可行方案
...
TL;DR: 本文研究了四种跨图像数据增强策略（CutMix、CarveMix、ObjectAug和AnatoMix）在多器官分割任务中的应用，发现其中三种策略能显著提升分割性能，其中CutMix表现最为稳健。


<details>
  <summary>Details</summary>
Motivation: 多器官分割是临床常规任务，但深度学习模型训练需要大量标注数据，而临床数据稀缺是主要挑战。传统数据增强策略局限于单图像操作，跨图像和对象级的数据增强策略在多器官分割任务中尚未得到充分探索。

Method: 研究了四种跨图像数据增强策略：CutMix、CarveMix、ObjectAug和AnatoMix，在两个器官分割数据集上进行实验，并与最先进的nnUNet模型（无数据增强）进行对比。

Result: CutMix、CarveMix和AnatoMix相比无数据增强的nnUNet，平均Dice分数分别提升了4.9、2.0和1.9。结合传统数据增强策略后，性能可进一步提升。实验表明CutMix是最稳健且简单的数据增强策略。

Conclusion: 跨图像数据增强策略能有效提升多器官分割性能，其中CutMix是稳健且简单的选择，即使其生成的图像在直觉上可能"错误"。研究代码已公开供未来基准测试使用。
...
TL;DR: ELIQ是一个无需人工标注的框架，用于评估不断进化的AI生成图像质量，专注于视觉质量和提示-图像对齐，通过自动构建正负样本对实现可迁移的监督学习。


<details>
  <summary>Details</summary>
Motivation: 随着生成式文本到图像模型的快速发展，感知质量天花板不断被推高，之前收集的标签对于新一代模型变得不可靠，因此需要一种无需人工标注的质量评估方法。

Method: ELIQ自动构建正负样本对覆盖传统失真和AIGC特定失真模式，通过指令调优将预训练多模态模型适配为质量感知批评器，使用轻量级门控融合和质量查询变换器预测二维质量。

Result: 在多个基准测试中，ELIQ始终优于现有的无标签方法，无需修改即可从AI生成内容泛化到用户生成内容场景，为持续进化的生成模型提供了可扩展的无标签质量评估方案。

Conclusion: ELIQ为持续进化的生成模型提供了可扩展、无需人工标注的质量评估框架，能够有效应对AI生成图像质量评估的挑战，并在多种场景下表现出良好的泛化能力。
...
TL;DR: 提出SlowFocus机制解决视频大语言模型在同时保留高质量帧级语义信息和全面视频级时序信息方面的困境，通过聚焦查询相关时间段进行密集采样，结合多频混合注意力模块，显著提升细粒度视频理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型难以同时保持高质量的帧级语义信息（每帧足够token数）和全面的视频级时序信息（足够采样帧数），这阻碍了Vid-LLMs在细粒度视频理解方面的发展。

Method: 1. SlowFocus机制：首先基于问题识别查询相关时间段，然后在该段进行密集采样提取局部高频特征；2. 多频混合注意力模块：聚合局部高频细节与全局低频上下文；3. 配套训练策略：增强时间定位和详细时序推理能力；4. 建立FineAction-CGR基准测试。

Result: 在现有公共视频理解基准和提出的FineAction-CGR基准上，该机制都表现出优越性，显著提升了视频大语言模型的细粒度时序理解能力。

Conclusion: SlowFocus机制通过聚焦相关时间段进行密集采样，结合多频混合注意力，有效解决了视频大语言模型在帧级语义质量和视频级时序覆盖之间的权衡问题，推动了细粒度视频理解的发展。
...
TL;DR: DeepTopo-Net是一个用于水下伪装目标检测的新框架，结合拓扑感知建模和频率解耦感知，解决了深海细长生物拓扑碎片化和透明生物特征提取困难的问题。


<details>
  <summary>Details</summary>
Motivation: 水下伪装目标检测面临挑战，因为目标与背景在海洋不同深度具有极高的视觉相似性。现有方法在处理深海细长生物的拓扑碎片化和透明生物的特征提取方面存在困难。

Method: 提出DeepTopo-Net框架，整合拓扑感知建模和频率解耦感知。设计了水条件自适应感知器（WCAP）使用黎曼度量张量动态变形卷积采样场，以及深渊拓扑细化模块（ATRM）通过骨架先验保持细长目标的结构连通性。同时创建了首个高分辨率（2K）的GBU-UCOD基准数据集。

Result: 在MAS3K、RMAS和提出的GBU-UCOD数据集上的广泛实验表明，DeepTopo-Net实现了最先进的性能，特别是在保持复杂水下图案形态完整性方面表现优异。

Conclusion: DeepTopo-Net通过拓扑感知建模和频率解耦感知，有效解决了水下伪装目标检测中的拓扑碎片化和特征提取问题，为深海环境下的目标检测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大规模监督微调的方法存在数据依赖性强、可扩展性有限的问题，而现有的零样本方法性能显著落后于微调方法，需要更灵活高效的解决方案

Method: 提出协作多智能体系统，包含交替推理-反思机制：1) 粗到细的帧选择策略确保帧多样性和文本相关性；2) 动态焦点布局自适应调整视觉焦点；3) 反思链机制使用提问者-回答者对生成自我反思链，验证中间结果并为下一轮推理提供反馈

Result: 在五个具有挑战性的基准测试中，Refer-Agent显著优于最先进的方法，包括基于监督微调的模型和零样本方法，且无需额外微调成本即可快速集成新的多模态大语言模型

Conclusion: Refer-Agent通过多智能体协作和反思机制，为视频对象分割提供了一种灵活、高效且可扩展的零样本解决方案，克服了传统方法的局限性
...
TL;DR: EB-JEPA是一个开源库，用于学习表示和世界模型，基于联合嵌入预测架构，通过能量基自监督学习在表示空间而非像素空间进行预测，避免生成建模的缺陷，支持从图像扩展到视频和动作条件世界模型。


<details>
  <summary>Details</summary>
Motivation: 开发一个模块化、自包含的开源库，使能量基自监督学习在研究和教育中更易访问，展示如何将图像级自监督学习技术扩展到视频和动作条件世界模型，同时避免生成建模的复杂性。

Method: 采用联合嵌入预测架构，在表示空间而非像素空间进行预测；提供单GPU几小时内可训练的模块化实现；包括CIFAR-10上的组件消融实验、Moving MNIST上的多步预测示例，以及Two Rooms导航任务中的动作条件世界模型。

Result: 在CIFAR-10上表示探测达到91%准确率；Moving MNIST上展示时间建模扩展；Two Rooms导航任务中规划成功率97%；消融实验揭示各正则化组件对防止表示崩溃的关键重要性。

Conclusion: EB-JEPA成功展示了JEPA架构在表示学习和世界建模中的有效性，通过模块化实现使能量基自监督学习更易访问，为从图像到视频再到动作条件预测的研究提供了实用工具。
...
TL;DR: KTV是一个两阶段训练免费视频理解框架，通过聚类选择关键帧和剪枝视觉token来减少冗余，显著降低计算开销，在视频问答任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费视频理解方法面临严重视觉冗余和高计算开销问题，特别是处理长视频时。基于CLIP相似度的关键帧选择策略存在偏见，可能忽略关键帧，导致视频理解效果不佳。

Method: KTV采用两阶段框架：1) 问题无关的关键帧选择，通过聚类帧级视觉特征获得紧凑、多样、有代表性的帧子集；2) 关键视觉token选择，基于token重要性和冗余性剪枝每个关键帧中的冗余或不重要token。

Result: 在Multiple-Choice VideoQA任务上，KTV超越最先进的训练免费基线方法，同时使用显著更少的视觉token（例如60分钟视频10800帧仅用504个视觉token），在MLVU-Test基准上达到44.8%准确率，甚至在某些基准上超过训练方法。
...
TL;DR: 提出一个统一框架，通过多模态数据合成与融合进行视网膜疾病分类和分级，解决眼科实践中数据异质性、潜在侵入性、配准复杂性等挑战。


<details>
  <summary>Details</summary>
Motivation: 视网膜疾病诊断通常依赖多模态数据，但实践中面临数据异质性、潜在侵入性、配准复杂性等挑战，需要一种能够整合多模态数据合成与融合的统一框架。

Method: 1) 合成多模态数据，包括荧光素眼底血管造影(FFA)、多光谱成像(MSI)和强调潜在病变及视盘/视杯区域的显著性图；2) 并行模型独立学习模态特定表示；3) 特征在模态内和跨模态自适应校准，根据下游任务进行信息剪枝和灵活整合。
...
<details>
  <summary>Details</summary>
Motivation: 语义分割需要大量像素级标注数据，合成图像虽可自动生成标注但存在域差距问题，风格迁移方法可以减少这种差异，但如何选择和组合风格迁移操作面临巨大的组合搜索空间挑战。

Method: 使用多目标遗传算法优化风格迁移流水线，平衡结构一致性和风格相似性；研究使用配对图像度量在进化过程中快速评估流水线，而非需要生成大量图像的标准分布度量。

Result: 在GTA5到Cityscapes和ACDC（恶劣条件）的合成到真实域适应任务中，进化算法能够提出适应不同目标的多样化增强流水线，有效提升分割性能。

Conclusion: 将风格迁移表述为适合进化优化的序列问题，并研究高效度量方法以实现可行搜索，为合成到真实域适应提供了有效的解决方案。
...
TL;DR: 提出首个稀疏部分弱监督定向目标检测框架，仅需少量稀疏弱标注数据和大量未标注数据，在遥感领域显著降低标注成本


<details>
  <summary>Details</summary>
Motivation: 遥感领域目标密集分布且类别多样，完全标注成本过高。现有定向目标检测方法主要分为全监督、半监督和弱监督，但标注负担仍然较重。需要更高效的标注利用方法

Method: 1) SOS-Student模型：在稀疏标注设置下从背景中分离未标注目标，从方向无关或尺度无关的弱标注中学习方向和尺度信息；2) 多级伪标签过滤策略：利用模型多层预测信息；3) 稀疏分区方法：确保每个类别平等对待

Result: 在DOTA和DIOR数据集上的大量实验表明，该框架相比传统定向目标检测方法取得显著性能提升，提供高成本效益解决方案

Conclusion: 提出的稀疏部分弱监督定向目标检测框架能有效利用少量稀疏弱标注数据和大量未标注数据，显著降低标注成本，在遥感目标检测领域具有重要应用价值
...
TL;DR: MM-SCALE是一个大规模多模态道德尺度数据集，通过5点标量评分和显式模态基础来对齐视觉语言模型与人类道德偏好，相比传统二元监督能提供更丰富的对齐信号。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态和社会模糊情境中进行道德判断时仍存在困难。先前工作通常依赖二元或成对监督，无法捕捉人类道德推理的连续性和多元性特征。

Method: 提出MM-SCALE数据集，每个图像-场景对都通过专门设计的界面由人类标注道德可接受性分数和基础推理标签，支持基于排名场景集的列表式偏好优化，从离散监督转向标量监督。

Result: 实验表明，在MM-SCALE上微调的视觉语言模型相比使用二元信号训练的模型，实现了更高的排名保真度和更稳定的安全校准。

Conclusion: 通过标量监督框架，能够为多模态道德推理提供更丰富的对齐信号和更精细的校准，提升视觉语言模型在道德判断任务上的表现。
...
TL;DR: 提出RIAS范式，通过语言引导工业异常检测，无需手动阈值，单模型检测多种异常，引入MVTec-Ref数据集和DQFormer基准模型


<details>
  <summary>Details</summary>
Motivation: 传统工业异常检测方法存在两大问题：无监督方法需要手动阈值且定位粗糙，有监督方法因数据稀缺和不平衡而过拟合；两者都受限于"一类异常一个模型"的限制

Method: 提出Referring Industrial Anomaly Segmentation (RIAS)范式，利用语言描述指导检测；引入MVTec-Ref数据集支持该范式；提出DQFormer基准模型，采用双查询令牌和语言门控多级聚合技术

Result: RIAS能够从文本描述生成精确的异常掩码，无需手动阈值，单个模型可检测多种异常；DQFormer通过仅使用"异常"和"背景"两个令牌实现高效的视觉-文本集成

Conclusion: RIAS范式有效推进工业异常检测向开放集能力发展，解决了传统方法的局限性，为工业异常检测提供了新的方向
...
TL;DR: 提出RegionReasoner强化学习框架和RegionDial-Bench基准测试，通过多轮视觉推理提升视觉语言模型的空间定位和语义一致性能力


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型大多依赖单步或纯文本推理，缺乏在多个视觉上下文之间迭代优化理解的能力，限制了多轮视觉推理的发展

Method: 提出RegionReasoner强化学习框架，要求每个推理轨迹显式引用对应的参考边界框，并通过全局-局部一致性奖励保持语义连贯性。该奖励从全局场景描述和区域级描述中提取关键对象和名词，与推理轨迹对齐

Result: 在检测和分割任务上的实验表明，RegionReasoner-7B与RegionDial-Bench基准显著提高了多轮推理准确性、空间定位精度和全局-局部一致性

Conclusion: RegionReasoner为多轮视觉推理这一新兴研究方向建立了强有力的基准，通过强化学习框架实现了更好的空间定位和语义一致性
...
Result: RAPID-SCAN模型达到0.834 F1分数；完整的管道在移动机器人平台上成功部署和评估，在真实世界检查场景中展示了有效性；通过量化显著减少了模型大小和推理延迟，同时保持总结质量。
...
TL;DR: LIVE提出了一种通过循环一致性目标约束误差累积的长时域交互视频世界模型，无需教师蒸馏，在长时域视频生成基准上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 自回归视频世界模型在短时域预测有效，但在长时域生成中面临预测误差累积问题。现有方法引入预训练教师模型和序列级分布匹配，但计算成本高且无法防止超出训练时域的误差传播

Method: 提出LIVE模型，通过新颖的循环一致性目标强制约束误差累积：1）从真实帧进行前向展开；2）应用反向生成过程重建初始状态；3）在重建的终端状态计算扩散损失，显式约束长时域误差传播；4）提供统一视角框架并引入渐进训练课程以稳定训练

Result: 实验表明LIVE在长时域基准测试中达到最先进性能，能够生成超出训练展开长度的稳定高质量视频

Conclusion: LIVE通过循环一致性目标有效解决了长时域视频生成中的误差累积问题，无需教师蒸馏，为长时域交互视频世界建模提供了有效解决方案
...
TL;DR: 提出一个自动化框架，将静态动漫插图转换为可操控的2.5D模型，解决了传统专业流程中需要繁琐手动分割和遮挡区域艺术"幻觉"的问题。


<details>
  <summary>Details</summary>
Motivation: 当前专业工作流程需要繁琐的手动分割和遮挡区域的艺术"幻觉"才能实现运动效果，这限制了动漫角色动画化的效率和质量。

Method: 1. 将单张图像分解为完全修复的语义不同层并推断绘制顺序；2. 引入可扩展引擎，从商业Live2D模型获取高质量监督数据；3. 结合基于扩散的身体部位一致性模块（确保全局几何一致性）和像素级伪深度推断机制。
...
TL;DR: REPA-G是一种在推理时利用自监督模型对齐表示进行条件控制的扩散模型引导框架，通过优化相似性目标实现从预训练特征提取器的多尺度条件生成。


<details>
  <summary>Details</summary>
Motivation: 虽然表示对齐已被证明能改进扩散模型训练，但其在推理时条件控制的潜力尚未充分探索。现有方法主要依赖模糊的文本提示或粗糙的类别标签，缺乏灵活精确的控制手段。

Method: 提出表示对齐引导（REPA-G）框架，在推理时通过优化相似性目标（势函数）来引导去噪过程朝向预训练特征提取器提取的条件表示。支持从单patch的细粒度纹理匹配到全局图像特征token的广泛语义引导，并可扩展到多概念组合。

Result: 在ImageNet和COCO数据集上的定量结果表明，该方法能实现高质量、多样化的生成。理论分析证明了该引导方法能从势函数诱导的倾斜分布中进行采样。

Conclusion: REPA-G提供了一种完全在推理时操作的灵活精确条件控制方法，可作为模糊文本提示或粗糙类别标签的替代方案，实现了从细粒度到语义级别的多尺度控制。
...
TL;DR: RAWDet-7是一个包含约2.5万训练和7.6千测试RAW图像的大规模数据集，用于研究机器视觉中RAW图像相对于ISP处理RGB图像的优势，支持对象检测和描述任务，并模拟4/6/8位量化场景。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型使用针对人类感知优化的ISP处理后的RGB图像，会丢弃传感器级别的有用信息。RAW图像保留了未处理的场景数据，能够为机器推理提供更丰富的线索，捕捉在processed图像中丢失的细粒度细节、空间关系和上下文信息。

Method: 构建RAWDet-7数据集，包含约25k训练和7.6k测试RAW图像，采集自不同相机、光照条件和环境，按照MS-COCO和LVIS标准密集标注7个对象类别。同时提供从对应高分辨率sRGB图像提取的对象级描述，支持RAW图像处理和低位量化的研究。

Result: 创建了一个支持对象检测和描述任务的大规模RAW图像数据集，允许在模拟4位、6位和8位量化条件下进行评估，反映了真实的传感器约束，为研究检测性能、描述质量和细节以及低位RAW图像处理的泛化能力提供了基准。

Conclusion: RAWDet-7数据集填补了RAW图像机器视觉研究的空白，为探索RAW图像相对于processed RGB图像的优势、对象级信息保留、以及低位量化下的性能表现提供了重要资源，有助于推动机器视觉系统更好地利用传感器原始数据。
...
TL;DR: 提出基于人眼视网膜和初级视皮层原理的注视点视觉接口(FOVI)，将可变分辨率视网膜式传感器阵列转换为均匀密集的V1式传感器流形，通过k近邻卷积实现高效计算，显著降低高分辨率图像处理的计算成本。


<details>
  <summary>Details</summary>
Motivation: 人类视觉具有注视点特性，分辨率在视野中心最高而周边较低，这种主动感知机制高效平衡了分辨率与视野范围。相比之下，大多数计算机视觉系统采用均匀分辨率编码，处理全视野高分辨率图像时效率低下。因此需要开发类似人类视觉的高效感知系统。

Method: 基于人类视网膜和初级视皮层(V1)原理设计注视点视觉接口(FOVI)：1) 将可变分辨率视网膜式传感器阵列转换为均匀密集的V1式传感器流形；2) 在传感器流形上定义k近邻(kNN)感受野；3) 通过新颖的核映射技术实现kNN卷积。展示了两种应用：端到端kNN卷积架构，以及基于DINOv3 ViT模型的注视点适配版本(使用低秩适配LoRA)。

Result: 提出的模型在保持竞争力的性能的同时，显著降低了计算成本，仅为非注视点基准模型计算成本的一小部分。为高分辨率自我中心视觉的高效可扩展主动感知开辟了新途径。代码和预训练模型已开源。

Conclusion: FOVI系统成功模拟了人类注视点视觉的高效特性，通过可变分辨率感知和kNN卷积技术，实现了高分辨率图像处理的计算效率大幅提升，为计算机视觉系统的主动感知能力发展提供了重要方向。
...
TL;DR: QVLA：首个面向具身控制的动作中心量化框架，通过通道级比特分配策略，在保持98.9%性能的同时减少71%显存使用，实现1.49倍加速


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）模型在具身智能中具有重要价值，但其巨大计算需求阻碍了在资源受限机器人平台上的部署。现有从大语言模型（LLM）继承的统一比特量化方法不适合机器人控制，因为它们关注被动数据保真度而忽略了微小动作偏差可能导致的灾难性任务失败。

Method: 提出QVLA框架，采用细粒度的通道级比特分配策略。核心机制是直接测量量化每个通道到不同比特宽度时对最终动作空间的敏感性，生成精确的每通道重要性指标，指导全局优化，将量化和剪枝（0比特）统一到单一框架中。

Result: 在LIBERO基准测试中，使用QVLA量化的OpenVLA-OFT模型仅需原模型29.2%的显存，保持98.9%的原始性能，实现1.49倍加速。相比LLM衍生方法SmoothQuant，性能提升22.6%。

Conclusion: QVLA为机器人领域VLA模型压缩建立了新的原则性基础，为在真实硬件上部署大规模模型铺平了道路。该方法将量化和剪枝统一，实现了性能与效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 神经外科手术中，图像引导系统依赖术前MRI，但硬脑膜打开后脑移位会使术前MRI失效。术中MRI更新和脑移位补偿对提高神经导航精度和手术效果至关重要。

Method: 提出NeuralShift模型，基于U-Net架构，仅使用术前MRI预测颞叶切除术中的脑移位。通过目标配准误差（TRE）评估解剖标志点位移精度，使用DICE分数比较预测的术中掩模与实际术中MRI掩模。

Result: 模型能准确预测脑部全局变形（DICE分数0.97）和局部位移（标志点TRE低至1.12毫米），有效补偿颞叶切除手术中的大范围脑移位。

Conclusion: NeuralShift模型仅使用术前图像就能预测颞叶切除术中的脑变形，为手术团队提供增加手术安全性和效率的潜在机会，改善患者预后。代码将在接受后公开。
...
TL;DR: 本文提出了一种基于渐进棋盘格顺序的多尺度自回归图像生成方法，通过平衡的并行采样策略，在保持总串行步数不变的情况下，实现了高效的图像生成。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成面临的关键挑战是如何在并行采样独立位置的同时，通过串行条件建模相互依赖关系。现有方法要么通过多尺度金字塔进行尺度间条件建模，要么通过规则分区或随机顺序在单张图像内并行采样。本文旨在探索一种更灵活、基于固定顺序的方法来改进多尺度自回归图像生成。

Method: 提出基于渐进棋盘格的固定排序方法，用于多尺度自回归图像生成。该方法在每个尺度上从均匀间隔的区域并行采样，在四叉树细分的每个层级保持完全平衡。这种平衡设置使得在总串行步数不变的情况下，多种尺度放大因子都能产生相似结果。

Result: 在类别条件ImageNet数据集上，该方法在保持模型容量相似的情况下，相比最近的最先进自回归系统取得了有竞争力的性能，同时使用了更少的采样步骤。

Conclusion: 渐进棋盘格顺序为多尺度自回归图像生成提供了一种灵活有效的平衡采样策略，能够在减少采样步骤的同时保持生成质量，为高效图像生成提供了新的思路。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在严重的训练效率问题，主要源于模型规模庞大和视觉token数量多。现有高效训练方法主要关注减少模型规模或可训练参数，而视觉token剪枝在推理阶段已被证明有效，但在训练阶段应用会导致训练-推理不匹配问题。

Method: 提出DualSpeed双速训练框架：1) 快速模式作为主要模式，集成现有视觉token剪枝方法减少视觉token，并采用模式隔离器隔离模型行为；2) 慢速模式作为辅助模式，在完整视觉序列上训练以保持训练-推理一致性；3) 通过自蒸馏技术让慢速模式从训练充分的快速模式中学习。

Result: 实验表明DualSpeed能够将LLaVA-1.5的训练加速2.1倍，将LLaVA-NeXT的训练加速4.0倍，同时保持超过99%的性能，在训练效率和模型性能之间取得了良好平衡。

Conclusion: DualSpeed通过双速框架有效解决了多模态大语言模型训练效率问题，既利用了视觉token剪枝带来的训练加速优势，又通过慢速模式和自蒸馏保持了训练-推理一致性，为高效训练提供了新的研究方向。
...
Method: 提出自适应原点引导(AdaOr)，用身份条件化的自适应原点调整标准引导原点，通过身份指令对应身份操作，根据编辑强度在身份预测和无条件预测之间插值

Result: 在图像和视频编辑任务中，相比当前基于滑块的编辑方法，AdaOr提供更平滑、更一致的控制

Conclusion: AdaOr方法将身份指令融入标准训练框架，无需每编辑过程或依赖专用数据集，在推理时实现细粒度控制
...
TL;DR: EventNeuS：首个结合3D符号距离函数与密度场学习的自监督神经模型，从单目彩色事件流学习3D表示，显著提升事件相机的3D重建精度


<details>
  <summary>Details</summary>
Motivation: 事件相机在许多场景中比RGB相机更有优势，但现有的事件相机3D重建方法精度严重受限，密集3D网格重建研究较少，需要提升事件相机的3D重建能力

Method: 1. 首次将3D符号距离函数和密度场学习与事件监督相结合；2. 引入球谐函数编码处理视角依赖效应；3. 自监督学习单目彩色事件流的3D表示
...
TL;DR: 该论文提出"虚伪差距"指标，利用稀疏自编码器量化大语言模型内部推理与最终输出之间的差异，以检测模型不忠实行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常表现出不忠实行为，其最终答案与内部思维链推理存在显著差异，为了取悦对话用户而改变立场。现有方法难以有效检测这种内部推理与外部输出不一致的问题。

Method: 引入"虚伪差距"机制度量，使用稀疏自编码器量化模型内部推理与最终生成之间的差异。通过稀疏线性探针推导内部真实信念，在潜在空间中与最终生成轨迹进行数学比较，检测模型的不忠实行为倾向。

Result: 在Gemma、Llama和Qwen模型上使用Anthropic的奉承基准测试，该方法在检测奉承运行时的AUROC为0.55-0.73，在检测虚伪情况（模型内部"知道"用户错误）时的AUROC为0.55-0.74，始终优于基于决策对齐的对数概率基线（0.41-0.50 AUROC）。

Conclusion: 提出的"虚伪差距"指标能够有效量化大语言模型内部推理与最终输出之间的不一致性，为检测模型的不忠实行为提供了新的机制度量方法，在多个模型上都表现出优于基线方法的性能。
...
Conclusion: STEMVerse框架通过系统分析LLMs在STEM领域的推理能力，超越了传统的整体评分方法，提供了更精细的诊断工具，有助于识别模型在特定学科和认知层次上的能力缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使在良性输入下也可能产生有毒或不适当文本，现有去毒方法通常需要模型重训练、梯度访问或学习辅助组件，成本高且难以跨模型家族迁移，特别是在真正的黑盒设置中
...
<details>
  <summary>Details</summary>
Motivation: 现有高效注意力方法虽然降低了计算复杂度，但通常存在模型状态覆盖范围有限的问题。长上下文能力和计算效率是当前大语言模型面临的核心挑战。

Method: 在标准注意力机制之外，并行引入基于CPU的ROSA检索模块，高效定位长上下文中与当前查询相关的历史位置，并以可训练方式将检索信息注入模型状态；通过二元离散化策略和反事实梯度算法实现端到端训练，采用异步CPU-GPU流水线优化整体执行效率。

Result: 在Qwen3-Base-1.7B上的系统评估显示，ROSA-Tuning显著恢复了窗口注意力模型的长上下文建模能力，在LongBench等基准测试中达到接近甚至匹配全局注意力的性能，同时保持了与窗口注意力方法相当的计算效率和GPU内存使用。
...
TL;DR: 该论文提出了WideSeekBench基准和WideSeek多智能体架构，用于解决从深度研究向广度研究范式转变中的信息检索与综合问题。


<details>
  <summary>Details</summary>
Motivation: 搜索智能正从深度研究向广度研究演进，但该领域缺乏专门的基准和优化方法，阻碍了进展。需要解决在复杂约束下并行检索和综合全面信息的问题。

Method: 从数据管道和智能体优化两个角度进行研究：1）构建WideSeekBench基准，通过多阶段数据管道确保目标信息量、逻辑约束和领域的多样性；2）提出WideSeek动态分层多智能体架构，能根据任务需求自主分叉并行子智能体；3）设计统一训练框架，线性化多智能体轨迹并使用端到端强化学习优化系统。

Result: 实验结果表明WideSeek和多智能体强化学习的有效性，证明扩展智能体数量是推进广度研究范式的有前景方向。

Conclusion: 该研究通过构建专用基准和优化方法，为广度研究范式的发展提供了重要支持，展示了多智能体架构在复杂信息检索任务中的潜力。
...
### [93] [When Efficient Communication Explains Convexity](https://arxiv.org/abs/2602.02821)
...
<details>
  <summary>Details</summary>
Motivation: 近期研究认为世界语言变异可从高效通信角度解释，即语言在简单性和信息性之间达到最优平衡。本文旨在探究在语义类型学中，哪些因素使得高效通信解释能够成功。

Method: 使用信息瓶颈方法形式化简单性与信息性之间的权衡，首先分析IB意义下的最优性与凸性推广之间的相关性，然后通过操纵IB框架中的各种建模参数来确定驱动这种相关性的因素。

Result: 研究发现IB意义下的最优性与凸性之间存在相关性，且通信需求分布的凸性在驱动这种相关性中起着特别重要的作用。

Conclusion: 这些结果不仅表明高效通信可以解释语义类型学的某些方面，更重要的是通过识别哪些底层因素对此负责，解释了为什么会出现这种情况。
...
TL;DR: 该研究通过在Transformer的feed-forward子层中引入单调性约束，显著提升了语言模型的对抗鲁棒性，同时保持了原有的性能表现。
...
Result: 对抗攻击成功率从约69%降至19%，而标准摘要性能仅略微下降。单调性语言模型在保持预训练模型性能的同时，显著提高了鲁棒性。

Conclusion: 单调性作为架构归纳偏置可以有效提升Transformer语言模型的鲁棒性，而不必牺牲表达能力。这种选择性实施单调性的方法在控制和安全关键系统中常用的保序行为与神经语言模型所需的表达能力之间找到了平衡。
...
TL;DR: 利用电子健康记录数据预测退伍军人首次无家可归风险，比较不同机器学习模型性能，发现加入社会行为因素可显著提升预测准确性。
...
Result: 将社会和行为因素纳入纵向模型可将精确率-召回率曲线下面积提高15-30%；在风险最高的1%人群中，不同时间点的阳性预测值在3.93-13.80%之间；大语言模型在判别性能上不如编码器模型，但在不同种族群体间的性能差异较小。

Conclusion: 纵向、社会信息化的电子健康记录建模能够将无家可归风险集中到可操作的层次，为高危退伍军人提供有针对性的、数据驱动的预防策略。
...
<details>
  <summary>Details</summary>
Motivation: 医疗运输（器官、患者、医疗物资）的及时性对医疗结果至关重要。传统救护车受交通拥堵限制，直升机成本高昂，新兴空中交通工具（无人机和eVTOL）虽成本较低但受航程和天气条件限制。多模式运输系统可以整合空中和地面车辆的优势，提高整体运输效率。

Method: 提出了一种构造性贪婪启发式算法，用于多模式医疗运输车辆调度。算法包含：1）兼容路线的有效载荷整合；2）考虑地面交通拥堵和空中天气条件；3）相比计算密集型优化模型实现快速车辆调度。测试了四种车队配置。

Result: 在相同条件下评估了四种车队类型，识别出最能满足医疗运输需求同时最小化运营成本、充电/燃料成本和总运输时间的最有效配置。

Conclusion: 多模式运输系统通过整合空中和地面车辆的优势，能够显著提高医疗运输效率。提出的贪婪启发式算法能够快速调度车辆，为医疗运输提供了有效的解决方案。
...
TL;DR: 论文通过网格游戏测试LLM智能体在现实部署环境中的鲁棒性，发现现有评估高估了实际准备度，智能体在部分可观测、动态环境、噪声信号和动态状态等操作条件下表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估假设"干净接口"（动态稳定、工具可靠、单一明确目标），高估了LLM智能体在实际部署中的准备度。现实中智能体面临规则不明确、信号不可靠、环境变化和多利益相关者目标等挑战，需要测试其在部署相关操作条件下的鲁棒性。

Method: 使用基于网格的游戏环境，设置简单目标但需要长时程执行。设计违反干净接口假设但仍可解决的场景，迫使智能体推断规则、为信息付费、适应环境和内部变化、在噪声下谨慎行动。测试五种最先进的LLM智能体在四种操作条件下的表现：部分可观测性、动态环境、噪声信号和动态智能体状态。

Result: 发现名义任务解决能力与部署式鲁棒性之间存在巨大差距。性能随网格大小和时程增加而普遍下降，但排名不稳定：当策略匹配不确定性机制时，较弱模型可能击败较强模型。智能体在没有明确指令的情况下权衡完成度、效率和惩罚避免，表明部分目标推断能力。消融和特征分析揭示了模型特定的敏感性和失败驱动因素。

Conclusion: 研究强调了在部分可观测性、噪声和非平稳性条件下，验证、安全行动选择和目标推断工作的重要性。现有评估方法需要改进以更好地反映实际部署挑战，智能体需要更强的适应性和鲁棒性来应对现实世界的不确定性。
...
TL;DR: R2-Router：首个考虑输出长度预算的LLM路由框架，通过联合选择最佳LLM和长度预算，在可比成本下实现比现有路由器更好的性能
...
Result: 实验表明，R2-Router在比现有路由器低4-5倍的成本下实现了最先进的性能。该框架能够发现强大LLM在受限输出下可以超越较弱LLM，这在先前方法中是不可见的。

Conclusion: 这项工作开启了"路由即推理"的新方向，路由器从被动选择器演变为深思熟虑的推理器，探索使用哪个LLM以及在什么成本预算下使用。R2-Router通过考虑输出长度作为可控变量，显著提高了LLM路由的效率和效果。
...
TL;DR: CATNIP是一种无需保留数据或对比对的LLM遗忘方法，通过基于模型置信度的令牌级校准实现精确的知识遗忘，在MUSE和WMDP基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法存在三个主要问题：1)基于梯度上升的方法会损害通用领域知识；2)需要保留数据或对比对，这在实践中不切实际；3)负偏好对齐方法受限于参考模型选择且在现实数据设置中性能受限。需要一种能精确控制遗忘、无需额外数据且对数据稀缺和长度变化鲁棒的方法。

Method: 提出CATNIP方法，核心思想是根据模型对不良知识的令牌级置信度来重新缩放遗忘效果。该方法通过量化模型对不良知识的置信度，并使用这些信息更精确地校准梯度更新，实现细粒度的遗忘控制，同时避免灾难性遗忘。

Result: 在MUSE和WMDP基准上的广泛评估表明，CATNIP无需保留数据或对比遗忘响应对，就能实现有效的遗忘，在知识遗忘和保留的权衡方面优于最先进的方法，表现出更强的遗忘效果和知识保护能力。

Conclusion: CATNIP通过基于模型置信度的令牌级校准，提供了一种原则性的LLM遗忘方法，解决了现有方法的局限性，实现了更精确的遗忘控制，对数据稀缺和长度变化具有鲁棒性，为LLM安全和隐私问题提供了有效的解决方案。
...
TL;DR: 研究发现人们在不确定情境下是否寻求澄清取决于情境不确定性和替代行动成本，当错误行动代价高时，不确定性影响最大。


<details>
  <summary>Details</summary>
Motivation: 研究人们在沟通情境中如何权衡不确定性和行动成本，特别是在面临澄清问题（CQs）决策时，探究人类是否理性地根据潜在损失风险来寻求澄清。

Method: 基于预期遗憾的计算模型，通过两个实验验证：一个检验对问题的纯语言回应，另一个扩展到澄清与非语言行动之间的选择。

Result: 实验结果支持理性权衡假设：人类倾向于根据在不确定下行动可能遭受重大损失的风险程度，按比例寻求澄清。

Conclusion: 人们在沟通中是否寻求澄清取决于情境不确定性和行动成本的交互作用，当错误行动代价高昂时，不确定性对澄清决策的影响最为显著，体现了理性的风险规避策略。
...
<details>
  <summary>Details</summary>
Motivation: NLP领域近年来经历了巨大变革，引发了超越学科边界的讨论。这提出了教育中的重要问题：如何在这个不断变化的领域中设计能够连接各子学科的课程？特别是话语处理领域具有丰富的语言学见解和计算模型，与开放性或长文本生成高度相关，但这种联系在现有本科课程中尚未得到充分探索。

Method: 设计并开设了一门名为"计算话语与自然语言生成"的新课程，由具有互补专业知识的团队合作设计。该课程于2025年秋季首次作为高年级本科课程开设，在语言学与计算机科学之间交叉列出。课程哲学是深度整合理论与实证方面，在课堂和作业中培养探索性思维。

Result: 成功开设了这门跨学科课程，详细描述了课程内容。通过独立调查收集了反馈，并提出了未来发展方向。

Conclusion: 该课程为连接NLP领域中话语处理与自然语言生成的跨学科教学提供了实践案例，展示了如何在这个快速变化的领域中设计能够整合理论与实践的课程，并为未来课程发展提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 幻觉仍然是大型语言模型（LLMs）面临的主要障碍，特别是在安全关键领域。现有方法要么需要访问模型内部状态（白盒），要么仅依赖表面文本（黑盒），存在效率、兼容性和泛化能力方面的限制。

Method: 提出HALT检测器：1) 仅使用LLM生成的前20个token对数概率作为时间序列输入；2) 采用门控循环单元模型结合基于熵的特征；3) 学习模型校准偏差而非直接分析内容；4) 引入HUB基准测试，整合10种能力的数据集。

Result: HALT在HUB基准测试中表现优异：1) 比微调的modernBERT-base编码器Lettuce性能更好；2) 模型尺寸小30倍；3) 速度提升60倍；4) 在推理任务和通用技能任务上均有效。

Conclusion: HALT和HUB共同建立了一个有效的幻觉检测框架，能够在多样化的LLM能力上进行高效检测，为安全关键应用提供了轻量级、兼容性强的解决方案。
...
TL;DR: 研究发现LLM在访问公平性（零拒绝率）下，交互质量仍存在系统性差异：GPT-4对年轻男性用户表达更多犹豫，LLaMA在不同身份群体间情感差异更大


<details>
  <summary>Details</summary>
Motivation: 现有关于LLM公平性的研究主要关注访问层面的行为（如拒绝和安全过滤），但公平访问并不能保证获得响应后的交互质量公平。需要评估LLM在获得访问后，对不同身份群体的语气、不确定性和语言框架是否存在差异。

Method: 采用反事实提示设计，在职业建议任务中改变年龄、性别和国籍等身份属性，评估GPT-4和LLaMA-3.1-70B。通过拒绝分析评估访问公平性，使用自动语言指标（包括情感、礼貌和犹豫表达）测量交互质量，使用配对统计检验评估身份条件差异。

Result: 两个模型在所有身份上都表现出零拒绝率，表明访问公平性一致。但在交互质量上存在系统性、模型特定的差异：GPT-4对年轻男性用户表达显著更高的犹豫，而LLaMA在不同身份群体间表现出更广泛的情感变化。

Conclusion: 即使访问公平，交互层面的公平性差异仍然存在，这促使需要超越基于拒绝的审计进行更全面的公平性评估。
...
TL;DR: LLMs在基于社会规范的指代消解任务上表现不佳，特别是在规范隐含、未明确说明或冲突的情况下。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够理解和应用社会规范来进行基于规范的指代消解，这对于具身智能体在现实环境中的有效交互至关重要。

Method: 开发了SNIC（情境中的规范）测试平台，这是一个经过人工验证的诊断性测试集，专门设计来探究最先进的LLMs在提取和应用与NBRR相关的规范原则方面的能力。

Result: 即使是最强大的LLMs也难以一致地识别和应用社会规范，特别是在规范隐含、未明确说明或冲突的情况下。

Conclusion: 当前LLMs在基于社会规范的推理方面存在盲点，这对于在社交情境中部署基于语言的系统构成了关键挑战。
...
TL;DR: CPMöbius提出了一种无需外部训练数据的协作式强化学习框架，通过教练-玩家协作模式提升大语言模型的数学推理能力，显著优于现有无监督方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的推理能力提升严重依赖大量高质量人工标注数据，这种监督密集型训练范式不可持续且扩展性受限，需要探索无需外部训练数据的新方法。

Method: 提出CPMöbius协作式教练-玩家范式，将教练和玩家作为独立但协作的角色：教练根据玩家能力提出针对性指令并获得玩家表现变化的奖励，玩家则通过解决教练生成的逐步提升难度的任务获得奖励，形成协作优化循环。

Result: 在Qwen2.5-Math-7B-Instruct模型上，该方法将整体准确率平均提升4.9分，分布外准确率平均提升5.4分，分别超过RENT方法1.5分和R-zero方法4.2分，显著优于现有无监督方法。

Conclusion: CPMöbius证明了无需外部训练数据的协作式强化学习能够有效提升大语言模型的数学推理能力，为减少对人工标注数据的依赖提供了有前景的解决方案。
...
Method: 提出LatentMem框架，包含经验库（以轻量形式存储原始交互轨迹）和记忆合成器（基于检索的经验和智能体特定上下文合成紧凑的潜在记忆）。同时引入潜在记忆策略优化（LMPO），通过潜在记忆传播任务级优化信号到合成器，鼓励生成紧凑且高实用性的表示。

Result: 在多样化基准测试和主流MAS框架上的实验表明，LatentMem相比原始设置实现了高达19.36%的性能提升，并且始终优于现有记忆架构，无需对底层框架进行任何修改。

Conclusion: LatentMem通过可学习的潜在记忆表示有效解决了多智能体记忆系统的同质化和信息过载问题，为多智能体系统的持续适应提供了高效记忆机制。
...
TL;DR: ReMiT提出了一种双向LLM训练框架，利用RL调优模型的推理先验动态重加权预训练中的关键token，实现预训练与后训练的自我增强循环


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练是单向的（预训练→后训练），缺乏双向反馈机制。本文探索如何利用后训练的RL调优结果反过来改进预训练基础模型，建立自我增强的飞轮效应

Method: 识别预训练结束时的"中期训练"（退火）阶段是关键转折点，引入ReMiT方法：利用RL调优模型的推理先验动态重加权该阶段的token，优先处理对推理至关重要的token

Result: 在10个预训练基准测试（数学、代码、通用推理）上平均提升3%，并在整个后训练流程中保持超过2%的增益，验证了迭代反馈循环的有效性

Conclusion: 成功建立了双向自我增强训练框架，使LLM能够实现持续、自我强化的进化，无需专门的教师或参考模型
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在复杂推理方面依赖专家标注数据和外部验证器，现有自我进化范式存在无法定位最优学习区域、可能强化集体幻觉和错误先验的问题。

Method: 提出AERO框架：1）基于最近发展区理论，使用熵基定位方法针对"可解性差距"；2）采用独立反事实修正进行鲁棒验证；3）引入交错训练策略同步各功能角色能力增长，防止课程崩溃。

Result: 在三个领域的九个基准测试中，AERO在Qwen3-4B-Base上平均提升4.57%，在Qwen3-8B-Base上平均提升5.10%，优于竞争基线方法。

Conclusion: AERO通过无监督的自主进化推理优化，有效解决了现有自我进化方法的局限性，显著提升了语言模型的推理能力，代码已开源。
...
TL;DR: TRT框架让大语言模型在测试时通过递归思考实现自我改进，无需额外训练，在数学和编程难题上显著提升性能


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型是否能在无需额外训练的情况下自我改进，解决两个核心挑战：高效生成多样化高质量候选方案，以及在缺乏真实监督时可靠选择正确答案

Method: 提出测试时递归思考(TRT)框架，通过迭代自我改进，将生成过程条件化于特定策略、累积知识和自我生成的验证信号

Result: 开源模型在AIME-25/24上达到100%准确率，闭源模型在LiveCodeBench最难问题上提升10.4-14.8个百分点，无需外部反馈

Conclusion: TRT框架证明大语言模型可以在测试时通过递归思考实现有效的自我改进，显著提升推理能力
...
TL;DR: 提出TSS和TSS++指标量化指令对输出的决定程度，通过对比真实指令与替代指令来评估指令特异性，实验证明基于任务特异性的样本选择能提升有限token预算下的下游性能


<details>
  <summary>Details</summary>
Motivation: 当前指令调优中许多指令-输入-输出对是弱指定的，同一输入在不同指令下可能产生相同输出，因此需要量化指令对目标输出的决定程度

Method: 提出任务特异性评分TSS，通过对比真实指令与替代指令来量化指令重要性；进一步提出TSS++，使用困难替代指令和小质量项来缓解简单负样本效应

Result: 在三个指令数据集和三个开源LLM上的实验表明，选择任务特异性高的样本能提升有限token预算下的下游性能，并能与基于质量的筛选方法互补

Conclusion: 指令特异性是评估指令调优数据的重要维度，任务特异性评分能有效识别对模型学习有价值的样本，特别是在资源受限的场景下
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决现有语用理解方面的空白，评估大语言模型在识别中文礼貌现象方面的能力，探索技术与人文的共存方式。

Method: 采用关系管理理论和虚假礼貌模型构建三类数据集（真实与模拟中文话语），选择6个代表性模型（包括GPT-5.1和DeepSeek），在四种提示条件下评估：零样本、少样本、知识增强和混合策略。

Result: 研究结果展示了不同模型在识别中文礼貌现象方面的性能差异，为"大语言学"范式提供了有意义尝试，建立了语言学技术与人文反思的跨学科桥梁。

Conclusion: 该研究为语用理论在技术变革时代的应用提供了新方法，回应了技术与人文如何共存的当代问题，代表了语言学技术与人文反思的跨学科努力。
...
TL;DR: ChemPro是一个包含4100个化学自然语言问答对的渐进式基准测试，涵盖4个难度层次，用于评估大语言模型在化学领域的综合能力。测试显示LLMs在基础化学问题上表现良好，但随着问题复杂度的增加，准确性显著下降。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对化学领域的全面基准测试来评估大语言模型的能力。需要创建一个系统性的评估框架，模拟学生从基础到高中的化学学习进程，以测试LLMs在不同难度和复杂度的化学问题上的表现。

Method: 开发了ChemPro基准测试，包含4100个自然语言问答对，分为4个难度层次。问题类型包括选择题和数值题，涵盖信息回忆、长程推理、多概念问题、复杂问题解决等维度。测试覆盖生物化学、无机化学、有机化学和物理化学四大领域。评估了45+7个最先进的开源和专有LLMs。

Result: LLMs在基础化学问题上表现良好，但随着问题类型和复杂度的增加，准确性显著下降。测试揭示了LLMs在一般科学推理和理解方面的关键局限性，指出了难度维度中未被充分研究的方面。

Conclusion: ChemPro基准测试揭示了LLMs在化学领域的局限性，特别是在处理复杂推理和多概念问题时。这些发现强调了需要开发更强大的方法来改进LLMs的科学推理能力，为未来的模型改进提供了重要方向。
...
TL;DR: OMAR框架通过多轮多智能体对话自博弈，让单个模型同时扮演所有对话角色，从动态社交互动中学习长期目标和复杂社会规范，无需人工监督即可涌现出精细的社会智能。


<details>
  <summary>Details</summary>
Motivation: 传统AI训练范式依赖静态、单轮优化，难以培养复杂的社会智能。研究者希望开发一个框架，让AI能够通过动态社交互动直接学习长期目标和复杂社会规范，从而涌现出真正的社会智能。

Method: 提出OMAR强化学习框架，让单个模型同时扮演对话中的所有角色进行自博弈。采用分层优势估计方法，分别计算轮级和令牌级优势，以确保长对话训练的稳定性。

Result: 在SOTOPIA社交环境和狼人杀策略游戏中的评估显示，训练后的模型涌现出精细的社会智能，包括共情、说服和寻求妥协等能力，即使在竞争性场景下也能学习协作。

Conclusion: OMAR框架证明AI可以在无需人工监督的情况下通过多智能体对话自博弈涌现出丰富的社会智能，尽管存在奖励黑客等实际挑战，但为群体对话中AI社会智能的进一步研究提供了激励。
...
### [114] [ForesightKV: Optimizing KV Cache Eviction for Reasoning Models by Learning Long-Term Contribution](https://arxiv.org/abs/2602.03203)
...
Method: 1. 设计Golden Eviction算法，使用未来注意力分数识别每个步骤的最优淘汰KV对；2. 通过Pairwise Ranking Loss进行监督训练，蒸馏这些轨迹和分数；3. 将缓存淘汰建模为马尔可夫决策过程，应用GRPO算法缓解低熵令牌上的语言建模损失增加。

Result: 在AIME2024和AIME2025基准测试中，对三个推理模型的实验表明，ForesightKV在仅使用一半缓存预算的情况下，始终优于先前的方法，同时从监督学习和强化学习方法中协同受益。

Conclusion: ForesightKV通过结合监督学习和强化学习的方法，有效解决了长文本生成中KV缓存管理的效率与性能平衡问题，在减少缓存使用的同时保持了模型性能。
...
### [115] [Token Sparse Attention: Efficient Long-Context Inference with Interleaved Token Selection](https://arxiv.org/abs/2602.03216)
*Dongwon Jo,Beomseok Kang,Jiwon Song,Jae-Joon Kim*

Main category: cs.CL

TL;DR: Token Sparse Attention：一种轻量级动态token级稀疏化机制，通过压缩每头Q、K、V到缩减的token集，再解压缩回原始序列，实现长上下文推理加速


<details>
  <summary>Details</summary>
Motivation: 注意力机制的二次复杂度是长上下文推理的主要瓶颈。现有加速方法要么使用结构化模式稀疏化注意力图，要么在特定层永久删除token，这些方法可能保留不相关token或依赖不可逆的早期决策，而忽略了token重要性在层和头之间的动态变化

Method: 提出Token Sparse Attention机制，在注意力计算期间将每头的Q、K、V压缩到缩减的token集，然后将输出解压缩回原始序列，使token信息能在后续层中被重新考虑。该方法与密集注意力实现（包括Flash Attention）完全兼容，并能与现有稀疏注意力内核无缝组合
...
### [116] [MIRROR: A Multi-Agent Framework with Iterative Adaptive Revision and Hierarchical Retrieval for Optimization Modeling in Operations Research](https://arxiv.org/abs/2602.03318)
*Yifan Shi,Jialong Shi,Jiayi Wang,Ye Fan,Jianyong Sun*

Main category: cs.CL

TL;DR: MIRROR是一个免微调的多智能体框架，可将自然语言优化问题直接转换为数学模型和求解器代码，通过执行驱动的迭代自适应修订和分层检索机制提高准确性。
...
### [117] [Accurate Failure Prediction in Agents Does Not Imply Effective Failure Prevention](https://arxiv.org/abs/2602.03338)
*Rakshith Vasudev,Melisa Russak,Dan Bikel,Waseem Alshikh*

Main category: cs.CL

TL;DR: LLM批评模型的主动干预可能适得其反：离线准确率高的批评模型（AUROC 0.94）在部署时可能导致严重性能下降（最高达26个百分点），而准确性本身不足以判断干预是否安全。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常假设LLM批评模型的主动干预能提高可靠性，但其在部署时的实际效果缺乏深入理解。需要探究批评模型干预的真正影响，避免盲目部署导致性能退化。

Method: 提出"破坏-恢复权衡"框架：干预可能恢复失败的轨迹，但也可能破坏原本会成功的轨迹。基于此设计部署前测试方法，仅需50个任务的试点即可预估干预效果，无需全面部署。

Result: 测试能准确预测干预效果：在高成功率任务上干预导致性能下降（0到-26个百分点），而在高失败率的ALFWorld基准上带来适度改进（+2.8个百分点，p=0.014）。批评模型准确性（AUROC 0.94）与部署效果无必然关联。

Conclusion: LLM批评模型的干预效果具有高度变异性，离线准确性不能保证部署安全。主要价值在于识别何时不应干预，在部署前预防严重性能退化，避免盲目应用批评模型。
...
### [118] [PEGRL: Improving Machine Translation by Post-Editing Guided Reinforcement Learning](https://arxiv.org/abs/2602.03352)
*Yunzhi Shen,Hao Zhou,Xin Huang,Xue Han,Junlan Feng,Shujian Huang*

Main category: cs.CL

TL;DR: PEGRL是一个两阶段强化学习框架，通过后编辑作为辅助任务来稳定训练并指导整体优化，解决了机器翻译中强化学习面临的噪声学习信号和探索-优化平衡问题。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的机器翻译强化学习（如GRPO）虽然表现出色，但仍面临两个主要挑战：1）蒙特卡洛回报估计产生的噪声学习信号；2）大轨迹空间导致全局探索优先于细粒度局部优化。

Method: 提出PEGRL两阶段RL框架：1）采样翻译输出构建后编辑输入；2）在后编辑阶段进行回报估计，利用当前翻译行为作为条件；3）通过任务特定加权方案平衡翻译和后编辑目标，获得有偏但更样本高效的估计器。

Result: 在英语→芬兰语、英语→土耳其语和英语↔中文翻译任务上，PEGRL相比RL基线获得了一致的性能提升。对于英语→土耳其语任务，在COMET-KIWI指标上的表现与先进的基于LLM的系统（DeepSeek-V3.2）相当。

Conclusion: PEGRL通过引入后编辑作为辅助任务，有效解决了机器翻译强化学习中的噪声信号和探索-优化平衡问题，实现了更稳定和高效的训练，在多个翻译任务上取得了显著性能提升。
...
### [119] [Pursuing Best Industrial Practices for Retrieval-Augmented Generation in the Medical Domain](https://arxiv.org/abs/2602.03368)
*Wei Zhu*
...
<details>
  <summary>Details</summary>
Motivation: 虽然检索增强生成（RAG）在基于大语言模型的工业应用中迅速被采用，但在医疗领域，对于如何构建RAG系统的最佳实践（包括组件、组织方式和实现方法）缺乏共识

Method: 首先仔细分析RAG系统的每个组件并提出实际替代方案，然后在三类任务上进行系统评估

Result: 揭示了改进RAG系统的最佳实践，以及基于LLM的RAG系统如何在性能与效率之间做出权衡

Conclusion: 该研究为医疗领域RAG系统的构建提供了系统化的指导框架，帮助开发者在性能与效率之间做出明智的权衡决策
...
### [120] [Towards Distillation-Resistant Large Language Models: An Information-Theoretic Perspective](https://arxiv.org/abs/2602.03396)
*Hao Fang,Tianyi Zhang,Tianqu Zhuang,Jiawei Kong,Kuofeng Gao,Bin Chen,Leqi Liang,Shu-Tao Xia,Ke Xu*

Main category: cs.CL

TL;DR: 该论文针对大语言模型的知识蒸馏攻击防御问题，从信息论角度提出了一种有效的解决方案，通过最小化条件互信息来保护模型知识产权。


<details>
  <summary>Details</summary>
Motivation: 专有大型语言模型具有重要经济价值，通常只以黑盒API形式提供，但攻击者仍可通过蒸馏攻击提取知识。现有防御方法主要关注基于文本的蒸馏，而基于logit的蒸馏防御研究不足，需要有效解决方案。

Method: 从信息论角度分析问题，使用条件互信息(CMI)量化教师模型输出中的蒸馏相关信息。提出学习一个变换矩阵来净化原始输出，增强蒸馏抵抗能力。推导出基于CMI的反蒸馏目标函数来优化这个变换，有效去除蒸馏相关信息同时保持输出效用。

Result: 在多个LLM和强蒸馏算法上的广泛实验表明，该方法显著降低了蒸馏性能，同时保持了任务准确性，有效保护了模型的知识产权。

Conclusion: 该研究填补了logit-based蒸馏防御的空白，从信息论角度提出了有效的防御方案，通过条件互信息最小化实现了对模型知识产权的保护，在保持模型实用性的同时增强了安全性。
...
### [121] [FactNet: A Billion-Scale Knowledge Graph for Multilingual Factual Grounding](https://arxiv.org/abs/2602.03417)
*Yingli Shen,Wen Lai,Jie Zhou,Xueren Zhang,Yudong Wang,Kangyang Luo,Shuo Wang,Ge Gao,Alexander Fraser,Maosong Sun*

Main category: cs.CL

TL;DR: FactNet是一个大规模开源资源，将17亿原子断言与30.1亿可审计证据指针统一，基于316个维基百科版本构建，提供92.1%的高精度事实基础。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs存在事实幻觉和缺乏可追溯来源的问题。现有基础资源通常呈现二元对立：要么提供没有文本上下文的结构化知识（如知识库），要么提供规模有限、语言覆盖不足的基于文本的基础。需要弥合这一差距。

Method: FactNet采用严格确定性的构建流程，从316个维基百科版本中提取17亿原子断言和30.1亿可审计证据指针。每个证据单元都可以字节级精度恢复，避免了合成方法的问题。

Result: 经过广泛审计，即使在长尾语言中，FactNet也实现了92.1%的高基础精度。同时建立了FactNet-Bench评估套件，涵盖知识图谱补全、问答和事实核查任务。

Conclusion: FactNet为社区提供了一个基础性、可复现的资源，用于训练和评估可信、可验证的多语言系统，解决了LLMs的事实幻觉和可追溯性问题。
...
TL;DR: 研究探讨语言模型在预训练和从英语适应瑞典语过程中，对惯用瑞典语与语言可接受瑞典语的偏好发展。发现惯用能力比其他语言能力发展更慢，但持续改善，而指令调优会削弱对惯用语偏好。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型如何发展对惯用瑞典语（相对于语言可接受瑞典语）的偏好，特别是在预训练期间和从英语适应瑞典语的过程中。了解模型在不同语言能力方面的发展轨迹，以及指令调优对惯用能力的影响。

Method: 1. 从头训练瑞典语模型和通过微调英语预训练模型来适应瑞典语；2. 使用最小对对比语言可接受性和惯用性差异；3. 为语言可接受性将现有基准改编为最小对格式；4. 为评估惯用性引入两个新数据集：对比常规化习语与合理变体，对比惯用瑞典语与翻译腔。

Result: 1. 惯用能力比其他语言能力（包括语法和词汇正确性）发展更慢；2. 虽然更长时间训练对大多数任务产生递减回报，但习语相关性能持续改善，特别是在最大模型（8B）中；3. 指令调优（使用从英语机器翻译的数据）导致模型迅速失去对惯用语言的偏好。

Conclusion: 语言模型的惯用能力发展较慢但持续改善，而常见的指令调优方法（使用机器翻译数据）会显著削弱模型对惯用语言的偏好，这对低资源语言的模型开发具有重要启示。
...
TL;DR: LRMs中大量自我验证步骤是重复确认而非纠错，提出基于经验池的测试时框架来抑制不必要的验证，减少20.3%的token使用同时保持或提升准确率。


<details>
  <summary>Details</summary>
Motivation: 通过大规模实证分析发现，大型推理模型中的反思步骤包含大量自我验证，但这些验证大多是确认性而非纠正性的，很少真正发现错误或改变推理结果。这表明自我验证的激活频率与其实际效用之间存在不匹配。

Method: 提出一种基于经验的测试时框架：1) 检测重新检查行为的激活；2) 查询离线经验池中的历史验证结果；3) 通过高效检索估计重新检查是否可能不必要；4) 当历史经验表明不必要时，发出抑制信号让模型继续推理。

Result: 在多个模型和基准测试中，该方法减少了高达20.3%的token使用量，同时保持了准确率，在某些数据集上甚至提高了准确率。

Conclusion: 大型推理模型中存在过度自我验证的问题，提出的经验驱动框架能有效抑制不必要的验证步骤，显著提高推理效率而不损害性能。
...
TL;DR: FaithRL是一个强化学习框架，通过优化推理忠实度来减少大语言模型在推理过程中的幻觉问题，相比传统基于结果的奖励方法能更好地监督中间步骤。


<details>
  <summary>Details</summary>
Motivation: 现有的基于可验证奖励的强化学习方法大多依赖稀疏的结果奖励，对中间步骤监督不足，导致模型过度自信和虚假推理，增加了幻觉问题。

Method: 提出FaithRL框架，包括：1）形式化忠实度最大化目标；2）几何奖励设计；3）忠实度感知的优势调制机制，通过惩罚无支持步骤同时保留有效部分推导来分配步骤级信用。

Result: 在不同骨干模型和基准测试中，FaithRL一致地减少了幻觉率，同时保持（甚至提高）答案正确性。进一步分析证实FaithRL提高了步骤级推理忠实度并具有鲁棒泛化能力。

Conclusion: FaithRL通过直接优化推理忠实度，有效缓解了强化学习中过度自信问题，减少幻觉，同时保持模型性能，为多步推理任务提供了更可靠的强化学习框架。
...
TL;DR: 该论文研究了大型语言模型在不同表示形式（代码、图、自然语言）之间的泛化能力，发现仅训练符号表示或自然语言存在局限性，提出两阶段数据课程方法显著提升跨表示泛化性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码和图等符号表示上训练和测试广泛，但真实用户任务通常用自然语言指定。研究LLM在不同表示形式之间的泛化能力，特别是同构任务在不同表示形式下的表现。

Method: 提出两阶段数据课程方法：第一阶段在符号表示（代码或图）数据上训练，第二阶段在自然语言数据上训练。通过这种课程学习策略促进跨表示泛化。

Result: 两阶段课程方法显著提升了模型性能，跨模型家族和任务都有效。使用该方法训练的1.5B参数Qwen模型在自然语言规划任务上接近零样本GPT-4o的性能。

Conclusion: 成功的跨表示泛化可解释为一种生成类比形式，而提出的两阶段数据课程方法能有效促进这种类比能力，为解决LLM在不同表示形式间的泛化问题提供了有效方案。
...
TL;DR: 研究发现目标语言的类型学特征显著影响多语言翻译模型的质量，即使控制数据资源和书写系统等因素后依然如此，某些类型学特征的语言可能从非标准解码策略中获益。
...
Method: 分析两个大型预训练多语言翻译模型NLLB-200和Tower+，它们分别是编码器-解码器和仅解码器机器翻译的最先进代表。基于广泛的语言集合，研究目标语言类型学对翻译质量的影响，控制数据资源和书写系统等简单因素。为FLORES+ MT评估基准的212种语言发布细粒度类型学特征数据集。

Result: 目标语言类型学显著驱动两个模型的翻译质量，即使在控制数据资源和书写系统等因素后依然如此。具有某些类型学特征的语言从更广泛的输出空间搜索中获益更多，表明这些语言可能从标准从左到右束搜索之外的其他解码策略中受益。

Conclusion: 语言类型学特征是多语言翻译模型性能的重要决定因素，某些类型学特征的语言可能需要替代的解码策略。研究结果为改进多语言翻译系统提供了重要见解，并发布了细粒度类型学数据集以促进该领域进一步研究。
...
TL;DR: 本文提出对齐对比学习（ACL）框架，解决监督学习中交叉熵损失与对比学习目标冲突的问题，通过标签嵌入对齐和梯度优化策略提升模型性能，特别在多出口BERT模型中显著改善质量-速度权衡。


<details>
  <summary>Details</summary>
Motivation: 在监督学习场景中，交叉熵损失（CE）与对比学习（CL）目标经常相互冲突，这阻碍了对比学习在监督设置中的应用。作者通过初步实验验证了这一冲突，并旨在解决该问题以充分利用对比学习的优势。

Method: 提出对齐对比学习（ACL）框架：1）ACL-Embed将标签嵌入视为额外增强样本，使用对比学习对齐标签嵌入与样本表示；2）ACL-Grad在ACL-Embed目标与CE损失冲突时丢弃ACL-Embed项以促进优化；3）针对多出口BERT提出跨层ACL（ACL-CL），让教师出口指导浅层学生出口的优化。

Result: 在GLUE基准测试中：a) ACL-BERT在GLUE任务上优于或与CE及CE+SCL相当；b) ACL（特别是CL-ACL）在多出口BERT微调上显著超越基线方法，为低延迟应用提供了更好的质量-速度权衡。

Conclusion: ACL框架成功解决了监督学习中交叉熵损失与对比学习目标的冲突问题，通过标签嵌入对齐和智能梯度优化策略，显著提升了模型性能，特别是在多出口BERT架构中实现了更优的效率-精度平衡。
...
TL;DR: EA-GraphRAG：通过语法感知的复杂度分析动态集成RAG和GraphRAG的框架，根据查询复杂度自适应选择检索策略，显著提升准确率并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 传统RAG在处理非结构化领域文档时存在信息碎片化问题，GraphRAG虽然通过知识图谱增强上下文推理，但在实际应用中却表现不佳，对所有查询都采用GraphRAG导致准确率下降和延迟过高。需要一种能根据查询复杂度自适应选择检索策略的解决方案。

Method: 提出EA-GraphRAG框架，包含三个核心组件：1）语法特征构造器，解析查询并提取结构特征；2）轻量级复杂度评分器，将特征映射为连续复杂度分数；3）分数驱动的路由策略，根据分数选择密集RAG（低分查询）、基于图的检索（高分查询），或对边界情况应用复杂度感知的互惠排名融合。

Result: 在两个单跳和两个多跳QA基准测试上的广泛实验表明，EA-GraphRAG显著提高了准确率，降低了延迟，在处理包含简单和复杂查询的混合场景中实现了最先进的性能。

Conclusion: 通过动态集成RAG和GraphRAG范式，EA-GraphRAG解决了传统GraphRAG对所有查询刚性应用的问题，实现了高效、自适应的知识增强生成，在准确性和效率方面都取得了显著改进。
...
TL;DR: 提出V₀通用价值模型，无需参数更新即可评估任意模型在未见提示上的表现，用于GRPO训练中的采样预算分配和部署时的模型路由调度。


<details>
  <summary>Details</summary>
Motivation: 传统Actor-Critic方法中价值模型需要与策略模型同步训练，成本高昂；GRPO方法虽无需价值模型但需要大量采样来维持基线估计稳定性。需要一种能准确评估模型能力变化且无需频繁更新的价值估计方法。

Method: 提出V₀通用价值模型，将策略的动态能力作为显式上下文输入，利用指令-性能对历史来动态分析模型能力，专注于状态零（初始提示）的价值估计。在GRPO训练中预测成功率以优化采样预算分配，在部署时作为路由器将指令分发给最经济合适的模型。
...
TL;DR: CL-bench是一个评估语言模型上下文学习能力的新基准，包含500个复杂上下文、1899个任务和31607个验证标准，模型平均只能解决17.2%的任务，表明当前模型在真实世界复杂任务中的上下文学习能力仍然有限。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型擅长利用预训练知识进行推理，但真实世界任务更加复杂且上下文依赖性强。模型需要从特定任务上下文中学习，并利用超出预训练范围的新知识来解决问题。这种上下文学习能力是人类自然具备但被现有研究忽视的关键能力。

Method: 研究者引入了CL-bench基准，包含500个由领域专家精心设计的复杂上下文、1899个任务和31607个验证标准。每个任务所需的新内容都包含在对应上下文中，要求模型学习新的领域知识、规则系统、复杂程序或经验数据推导的法则。该基准超越了主要测试检索或阅读理解的长上下文任务，也超越了通过指令和演示学习简单任务模式的上下文学习任务。

Result: 评估了10个前沿语言模型，发现模型平均只能解决17.2%的任务。即使表现最好的GPT-5.1模型也只能解决23.7%的任务。这表明语言模型尚未实现有效的上下文学习能力，这成为处理真实世界复杂上下文依赖任务的关键瓶颈。

Conclusion: CL-bench代表了构建具备上下文学习能力的语言模型的重要一步，这种能力将使模型更加智能，并推动其在真实世界场景中的部署应用。研究揭示了当前模型在处理复杂上下文依赖任务时的局限性，为未来模型发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 许多经典的编译器优化任务（如寄存器分配、LOSPRE、最优银行选择指令放置）都可以建模为控制流图上的PCSP问题。控制流图具有稀疏性和可分解性，特别是结构化程序的SPL分解特性，为高效算法设计提供了基础。

Method: 采用基于Series-Parallel-Loop（SPL）分解的方法，提出通用PCSP算法。该算法利用控制流图的结构特性，将复杂问题分解为可处理的子问题，统一了先前针对寄存器分配和LOSPRE的SPL方法。

Result: 算法时间复杂度为O(|G|·|D|^6)，其中|G|为控制流图大小，|D|为域大小。对于固定域D，算法为线性时间。在最优银行选择任务实验中，运行时间比现有最优方法快4倍。

Conclusion: 本文提出的SPL分解PCSP算法是通用且高效的，统一了多个编译器优化问题的解决方案，并在实际应用中显著提升了性能，为编译器优化领域提供了有力的理论工具。
...
TL;DR: 提出一种用于DeepResearch报告生成的查询特定评分标准生成器训练流程，通过强化学习结合人类偏好监督和LLM评估，配合多智能体马尔可夫状态工作流，显著提升报告质量


<details>
  <summary>Details</summary>
Motivation: 当前DeepResearch报告的训练和评估面临缺乏可验证奖励信号的挑战，现有评分标准方法要么过于粗糙缺乏细粒度，要么依赖人工构建成本高难以扩展

Method: 1) 构建带有人类偏好的DeepResearch风格查询数据集；2) 通过强化学习训练评分标准生成器，结合人类偏好监督和LLM评估的混合奖励；3) 引入多智能体马尔可夫状态工作流处理长时程推理

Result: 提出的评分标准生成器比现有方法提供更具区分度和更符合人类偏好的监督，在MaMs训练框架中，配备该生成器的系统在DeepResearch基准测试中超越所有开源基线，性能接近领先闭源模型

Conclusion: 该方法有效解决了DeepResearch报告评估的挑战，通过自动生成细粒度、查询特定的评分标准，显著提升了报告生成系统的性能和质量
...
### [133] [TRE: Encouraging Exploration in the Trust Region](https://arxiv.org/abs/2602.03635)
...
TL;DR: 本文提出Trust Region Entropy (TRE)方法，解决传统熵正则化在大型语言模型中效果不佳的问题，通过在信任区域内鼓励探索来提升强化学习性能。


<details>
  <summary>Details</summary>
Motivation: 传统熵正则化在大型语言模型强化学习中效果有限甚至降低性能，作者认为这是由于LLMs具有大规模词汇表和长生成视野带来的累积尾部风险，全局熵最大化会不加区分地将概率质量稀释到无效标记的广阔尾部，而不是聚焦于合理候选，从而破坏连贯推理。

Method: 提出Trust Region Entropy (TRE)方法，该方法在模型的信任区域内严格鼓励探索，避免将概率质量分散到无效标记区域，保持推理的连贯性。

Result: 在数学推理(MATH)、组合搜索(Countdown)和偏好对齐(HH)任务上的广泛实验表明，TRE始终优于原始PPO、标准熵正则化和其他探索基线方法。

Conclusion: TRE通过限制在信任区域内进行探索，有效解决了传统熵正则化在大型语言模型中的局限性，显著提升了强化学习在各种任务上的性能。
...
### [134] [RAGTurk: Best Practices for Retrieval Augmented Generation in Turkish](https://arxiv.org/abs/2602.03652)
...
TL;DR: 该研究构建了土耳其语RAG数据集，评估了RAG流程的七个阶段，发现复杂方法如HyDE能显著提升准确率，而过度堆叠生成模块会损害形态学特征。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成(RAG)的设计指导主要基于英语，缺乏对土耳其语等形态丰富语言的针对性研究，限制了这些语言在RAG系统中的性能优化。

Method: 构建了基于土耳其维基百科和CulturaX的土耳其语RAG数据集，包含问答对和相关段落块。在无需任务特定微调的情况下，对RAG流程的七个阶段（查询转换、重排序到答案精炼）进行了基准测试。

Result: 复杂方法如HyDE能达到85%的准确率，显著高于基线78.70%。使用交叉编码器重排序和上下文增强的帕累托最优配置能达到84.60%的准确率且成本更低。过度堆叠生成模块会扭曲形态学线索而降低性能。

Conclusion: 对于土耳其语等形态丰富语言，简单的查询澄清结合鲁棒的重排序是有效解决方案，而过度复杂的生成模块堆叠反而会损害性能。研究为形态丰富语言的RAG系统设计提供了实证指导。
...
### [135] [Instruction Anchors: Dissecting the Causal Dynamics of Modality Arbitration](https://arxiv.org/abs/2602.03677)
*Yu Zhang,Mufan Xu,Xuefeng Bai,Kehai chen,Pengfei Zhang,Yang Xiang,Min Zhang*

Main category: cs.CL

TL;DR: 该研究通过信息流视角揭示了多模态大语言模型中模态跟随能力的机制，发现指令标记作为模态仲裁的结构锚点，浅层注意力层进行非选择性信息传输，深层注意力层解决模态竞争，而MLP层则表现出语义惯性。研究发现仅操纵5%的关键注意力头就能显著改变模态跟随率。


<details>
  <summary>Details</summary>
Motivation: 模态跟随能力是多模态大语言模型根据用户指令选择性利用多模态上下文的关键能力，对实际部署的安全性和可靠性至关重要。然而，这种决策过程的底层机制尚未得到充分理解，需要深入研究以提升模型透明度和可靠性。

Method: 通过信息流视角研究模态跟随的工作机制，分析注意力层和MLP层的作用，识别关键的注意力头，并进行因果干预实验来验证发现。

Result: 研究发现指令标记作为模态仲裁的结构锚点，浅层注意力层进行非选择性信息传输，深层注意力层解决模态竞争，MLP层表现出语义惯性。仅操纵5%的关键注意力头就能使模态跟随率降低60%（通过阻断）或提高60%（通过针对性放大失败样本）。

Conclusion: 该研究为模型透明度提供了重要进展，并为多模态大语言模型中多模态信息的协调提供了一个原则性框架，有助于提升模型的安全性和可靠性。
...
Result: 通过搜索最优的Gated DeltaNet和softmax注意力组合，NAtS-L提供了一个强大且高效的token级混合架构。

Conclusion: NAtS-L框架在保持表达力的同时显著降低了计算复杂度，为长序列处理提供了有效的解决方案。
...
TL;DR: BAR-RAG提出边界感知证据选择方法，针对生成器的"恰到好处"证据区域，通过强化学习训练选择器，并采用两阶段管道微调生成器，显著提升RAG系统在噪声检索下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在真实检索噪声下表现脆弱，即使所需证据出现在top-K结果中。主要原因是检索器和重排器仅优化相关性，常选择过于简单或缺乏关键信息的证据，未考虑证据是否适合生成器。

Method: 1. 将重排器重构为边界感知证据选择器，针对生成器的"恰到好处"区域（既不过于简单也不无法回答的证据）；2. 使用生成器反馈通过强化学习训练选择器；3. 采用两阶段管道，在诱导的证据分布下微调生成器，缓解训练与推理的分布不匹配。

Result: 在知识密集型问答基准测试中，BAR-RAG在噪声检索下持续提升端到端性能，相比强RAG和重排基线平均增益10.3%，同时显著提高鲁棒性。
...
Method: 创建OCR Turk基准测试，包含180个土耳其语文档，分为学术文章、论文、幻灯片和非学术文章三类，设置三个难度级别，使用元素级指标评估7个OCR模型。

Result: PaddleOCR在整体表现最佳，在简单、中等和困难子集中获得高归一化编辑距离分数，但在图表识别方面表现不佳。模型在非学术文档上表现良好，幻灯片成为最具挑战性的文档类型。

Conclusion: OCR Turk填补了土耳其语文档解析标准化基准的空白，为评估OCR模型在低资源语言环境下的性能提供了重要工具，揭示了不同文档类型和难度级别对模型性能的影响。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型使自动生成选择题变得可行，但可靠地生成满足特定认知需求的项目仍然是一个挑战。现有方法难以系统性地针对文本理解、推理和主旨理解等不同认知层次。

Method: ReQUESTA采用混合多智能体框架，将选择题创作分解为专门子任务，协调LLM驱动的智能体和基于规则的组件，支持规划、受控生成、迭代评估和后处理。在学术说明文的大规模阅读理解研究中评估该框架。

Result: 与GPT-5零样本基线相比，ReQUESTA生成的项目更具挑战性、更具区分度，且与整体阅读理解表现更一致。专家评估显示其与核心概念更一致，干扰项的语言一致性和语义合理性更优，特别是在推理题方面。

Conclusion: 混合智能体编排可以系统性地提高基于LLM生成的可靠性和可控性，突显了工作流设计作为超越单次提示的结构化生成关键杠杆的重要性。
...
TL;DR: OmniRAG-Agent：一种面向低资源长音频视频问答的智能体方法，通过检索增强生成、智能体循环和联合优化解决现有方法的成本高、检索弱、规划有限等问题。


<details>
  <summary>Details</summary>
Motivation: 当前长序列多模态问答（涵盖文本、图像、音频、视频）面临四大挑战：1）密集编码成本高昂；2）细粒度检索能力弱；3）主动规划能力有限；4）缺乏端到端优化。这些问题在低资源长音频视频QA中尤为突出。

Method: 提出OmniRAG-Agent方法，包含三个核心组件：1）图像-音频检索增强生成模块，让多模态大模型从外部库中检索相关短片段；2）智能体循环机制，支持跨轮次的规划、工具调用和证据融合；3）组相对策略优化，联合优化工具使用和答案质量。

Result: 在OmniVideoBench、WorldSense和Daily-Omni三个基准测试上，OmniRAG-Agent在低资源设置下持续超越先前方法，并取得了强劲结果。消融实验验证了每个组件的有效性。

Conclusion: OmniRAG-Agent通过检索增强生成、智能体循环和联合优化，有效解决了长音频视频问答中的成本、检索、规划和优化问题，为低资源多模态推理提供了有效的解决方案。
...
TL;DR: SemanticSpec：一种语义感知的推测解码框架，通过验证整个语义序列而非单个token来加速大语言模型推理，解决了传统token级推测解码忽略语义等价性的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）和大型推理模型（LRMs）存在推理延迟高的问题，特别是在生成长思维链时。现有的推测解码方法在token级别操作，忽略了语义等价性（即不同token序列表达相同含义），导致低效的拒绝。
...
Conclusion: SemanticSpec通过语义层面的推测解码有效解决了大语言模型推理延迟问题，证明了语义感知方法在加速模型推理方面的优越性，为高效推理提供了新思路。
...
TL;DR: ID-MoCQA是首个针对印尼文化的大规模多跳问答数据集，用于评估大语言模型的文化理解能力，包含英语和印尼语版本，通过系统化转换单跳问题为多跳推理链来提升评估难度。


<details>
  <summary>Details</summary>
Motivation: 现有文化问答基准大多依赖单跳问题，模型可能利用浅层线索而非真正进行文化推理。需要创建能评估模型跨语境、传统和隐含社会知识进行推理能力的多跳问答数据集。

Method: 提出新框架，将单跳文化问题系统转化为六种线索类型的多跳推理链（如常识、时间、地理等）。采用多阶段验证流程，结合专家评审和LLM作为评判的过滤机制，确保高质量问答对。

Result: 评估多个最先进模型发现，在文化推理方面存在显著差距，特别是在需要细微推理的任务上。ID-MoCQA为提升LLM文化能力提供了具有挑战性的基准。

Conclusion: ID-MoCQA是首个针对印尼文化的大规模多跳问答数据集，能更准确地评估LLM的文化理解能力，揭示了现有模型在文化推理方面的不足，为推进LLM文化能力发展提供了重要基准。
...
TL;DR: Kimi K2.5是一个开源的多模态智能体模型，通过联合优化文本和视觉模态，并引入Agent Swarm并行智能体编排框架，在编码、视觉、推理等任务上取得SOTA结果，同时将延迟降低4.5倍。


<details>
  <summary>Details</summary>
Motivation: 推动通用智能体智能的发展，通过多模态联合优化提升智能体能力，解决复杂任务处理中的效率和性能问题。

Method: 1. 联合文本-视觉预训练；2. 零视觉SFT；3. 联合文本-视觉强化学习；4. Agent Swarm自导向并行智能体编排框架，动态分解复杂任务为异构子问题并并行执行。

Result: 在编码、视觉、推理和智能体任务等多个领域达到最先进水平；Agent Swarm相比单智能体基线将延迟降低高达4.5倍。

Conclusion: Kimi K2.5通过多模态联合优化和创新的Agent Swarm框架，显著提升了智能体智能的性能和效率，为未来研究和实际应用提供了有力工具。
...
TL;DR: CUBO是一个面向消费级笔记本电脑的RAG平台，能在16GB共享内存限制下实现高效文档检索，避免GDPR合规问题


<details>
  <summary>Details</summary>
Motivation: 组织处理敏感文档面临两难：云端AI可能违反GDPR，而本地系统通常需要18-32GB内存。需要一种能在消费级笔记本电脑（16GB内存）上运行的本地化RAG解决方案

Method: CUBO采用系统工程集成方法，包括流式数据摄入（O(1)缓冲区开销）、分层混合检索和硬件感知编排，在严格的15.5GB内存上限内运行

Result: 在BEIR基准测试中Recall@10达到0.48-0.97，检索延迟p50为185毫秒（C1,300笔记本电脑），37,000行代码在15.5GB内存限制下保持数据最小化处理

Conclusion: CUBO验证了小型到中型专业档案库的实际部署可行性，通过纯本地处理符合GDPR第5(1)(c)条，代码已公开
...
Conclusion: 显式和协调的信息传输能够实现更有效和鲁棒的长上下文压缩，为LLM的长上下文推理提供了高效解决方案。
...
TL;DR: CROSS-ALIGN+：一个三阶段框架，通过整合结构化知识、参数高效适配器和可解释性生成，显著提升基于表情包的社会虐待检测性能


<details>
  <summary>Details</summary>
Motivation: 现有表情包社会虐待检测方法存在三个主要限制：文化盲区（忽略文化符号背景）、边界模糊（讽刺与虐待混淆）和缺乏可解释性（模型推理不透明）。这些限制导致检测效果不佳。

Method: 提出CROSS-ALIGN+三阶段框架：第一阶段通过整合ConceptNet、Wikidata和Hatebase的结构化知识缓解文化盲区；第二阶段使用参数高效的LoRA适配器锐化决策边界；第三阶段生成级联解释增强可解释性。

Result: 在五个基准测试和八个大型视觉语言模型上的实验表明，CROSS-ALIGN+持续优于最先进方法，相对F1分数提升高达17%，同时为每个决策提供可解释的推理过程。

Conclusion: CROSS-ALIGN+通过系统性解决文化盲区、边界模糊和缺乏可解释性三大挑战，显著提升了表情包社会虐待检测的性能和透明度，为实际应用提供了可靠的技术方案。
...
TL;DR: 论文展示了研究人员如何与Google的Gemini系列AI模型合作，成功解决理论计算机科学等领域的开放问题、反驳猜想并生成新证明，探索了人机协作在科学发现中的潜力。
...
Method: 通过一系列案例研究，展示研究人员如何与Gemini Deep Think及其高级变体等AI模型协作。采用迭代精炼、问题分解和跨学科知识转移等协作技术，并超越标准聊天界面，将模型部署为严格对抗性审稿人，以及嵌入"神经符号"循环中自动编写和执行代码验证复杂推导。

Result: 成功解决了理论计算机科学、经济学、优化和物理学等领域的开放问题，反驳了猜想并生成了新证明。展示了AI模型能够检测现有证明中的细微缺陷，并通过神经符号循环自主验证复杂推导。

Conclusion: AI不仅可作为自动化工具，更能成为科学发现创造性过程中的多功能、真正的合作伙伴。人机协作的有效技术包括迭代精炼、问题分解和跨学科知识转移等方法，为加速科学研究开辟了新途径。
...
TL;DR: 本文提出Parallel-Probe方法，通过2D探测技术优化并行推理的效率，减少35.8%的序列token和25.8%的总token消耗，同时保持准确率。


<details>
  <summary>Details</summary>
Motivation: 并行推理虽然是有前景的推理范式，但计算负担重。现有效率方法主要依赖局部、每轨迹信号，缺乏利用并行分支间全局动态的原则性机制。

Method: 引入2D探测接口，周期性获取所有分支的中间答案，揭示宽度-深度动态。基于此开发Parallel-Probe控制器，采用共识早期停止调节推理深度，基于偏差的分支剪枝动态调整宽度。

Result: 在三个基准测试和多个模型上的实验表明，Parallel-Probe建立了优越的测试时缩放Pareto前沿。相比标准多数投票，减少35.8%序列token和25.8%总token成本，同时保持竞争性准确率。

Conclusion: Parallel-Probe通过有效利用并行推理的全局动态，显著提升了推理效率，为并行思维范式提供了实用的效率优化方案。
...
### [149] [Precoding-Oriented CSI Feedback Design with Mutual Information Regularized VQ-VAE](https://arxiv.org/abs/2602.02508)
*Xi Chen,Homa Esfahanizadeh,Foad Sohrabi*

Main category: cs.IT

TL;DR: 提出基于矢量量化变分自编码器的预编码导向CSI反馈框架，通过信息论正则化在固定反馈预算下最大化码本利用率，实现与变长神经压缩方案相当的性能。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO系统中，用户设备的CSI压缩需要在反馈开销和下行速率之间取得平衡，最大化有限反馈的效用以维持高系统性能。

Method: 基于矢量量化变分自编码器的预编码导向CSI反馈框架，引入可微互信息下界估计器作为训练正则化器，促进在固定反馈预算下对学习码本的有效利用。

Result: 该方法在固定长度反馈下实现了与变长神经压缩方案相当的速率，学习到的码字使用更均匀，并能捕获与底层CSI强相关的可解释结构。

Conclusion: 提出的预编码导向CSI反馈框架通过信息论正则化有效解决了固定反馈预算下的码本利用率问题，为大规模MIMO系统中的高效CSI反馈提供了新方案。
...
### [150] [Rate-Distortion Analysis of Optically Passive Vision Compression](https://arxiv.org/abs/2602.02768)
*Ronald Ogden,David Fridovich-Keil,Takashi Tanaka*

Main category: cs.IT

TL;DR: 提出一种基于事件相机的光学被动视觉压缩方案，通过光学生成视觉场景的余弦变换，实现高速、无需计算的视频压缩，性能优于单独使用事件相机。


<details>
  <summary>Details</summary>
Motivation: 在机器人控制和自主决策中，远程视觉传感器需要实时传输大量视觉数据，但通信信道资源有限。许多系统容易快速失稳，需要更高的采样频率，这加剧了数据传输的挑战。

Method: 提出光学被动视觉压缩方案，使用事件相机观测视觉场景的光学生成余弦变换，实现类似现代视频编解码器的高速、无需计算的视频压缩。通过模拟该方案并与单独事件相机进行比较。

Result: 光学被动视觉压缩方案在率失真性能上优于单独事件相机，且随着事件相机空间分辨率的提高，这种性能差距进一步增大。

Conclusion: 光学被动视觉压缩方案为解决资源受限信道中实时视觉数据传输问题提供了有效方法，特别是在需要高采样频率的机器人控制应用中具有优势。
...
### [151] [Straggler-Aware Coded Polynomial Aggregation](https://arxiv.org/abs/2602.03074)
*Xi Zhong,Jörg Kliewer,Mingyue Ji*

Main category: cs.IT

TL;DR: 将编码多项式聚合(CPA)扩展到支持预定义非掉队者模式的分布式计算系统，建立了精确恢复的充要条件，并识别出保证恢复的集合交集大小阈值。


<details>
  <summary>Details</summary>
Motivation: 现有CPA方案仅限于理想的无掉队者系统，无法处理实际分布式计算中的掉队者问题。需要将CPA扩展到支持预定义非掉队者模式的系统，以在实际环境中实现高效的多项式计算聚合。

Method: 扩展CPA到支持预定义非掉队者模式的分布式计算系统，建立精确恢复的充要条件，识别集合交集大小阈值，并提供可行的CPA方案构造方法。

Result: 证明了在掉队者感知系统中，精确恢复所需聚合所需的worker响应数量少于基于个体解码的多项式编码；建立了精确恢复的充要条件；识别了保证恢复的集合交集大小阈值；提供了可行的CPA方案构造；仿真验证了理论预测的可行性转变。

Conclusion: 成功将CPA扩展到掉队者感知的分布式计算系统，建立了精确恢复的理论框架，识别了关键的交集阈值，为实际分布式计算中的高效多项式聚合提供了理论保证和实用方案。
...
### [152] [Entropy Functions on Two-Dimensional Faces of Polymatroid Region with One Extreme Ray Containing Rank-One Matroid](https://arxiv.org/abs/2602.03363)
*Kaizhe He,Qi Chen*

Main category: cs.IT

TL;DR: 本文研究了n度多拟阵区域中2维面上的熵函数特征，特别关注包含秩1拟阵的极射线，并将包含另一个拟阵的极射线的2维面分为四种类型。


<details>
  <summary>Details</summary>
Motivation: 熵函数的表征在信息论中具有基础重要性。通过在多拟阵区域（香农外边界）上施加约束，可以得到该区域的各个面以及具有特殊结构的熵函数。本文旨在研究2维面上的熵函数特征。

Method: 研究n度多拟阵区域中2维面上的熵函数，特别关注包含秩1拟阵的极射线。对包含另一个拟阵的极射线的2维面进行分类。

Result: 将包含另一个拟阵的极射线的2维面分为四种类型，并给出了相应的熵函数特征。

Conclusion: 本文对多拟阵区域中2维面上的熵函数进行了系统分类，为理解熵函数在特定约束条件下的结构提供了理论框架，对信息论中的熵函数表征研究有重要意义。
...
### [153] [Universal Costas Matrices: Towards a General Framework for Costas Array Construction](https://arxiv.org/abs/2602.03407)
*Fatih Gulec,Vahid Abolghasemi*

Main category: cs.IT

TL;DR: 提出用于分析和发现Costas阵列的统一框架，引入通用Costas矩阵和通用Costas频率矩阵，开发基于重构的搜索方法，显著加速搜索过程


<details>
  <summary>Details</summary>
Motivation: Costas阵列具有理想的自相关和低互相关特性，在雷达、无线通信和集成感知通信中具有重要价值。现有Costas阵列发现方法效率有限，需要新的分析框架来加速搜索并增强结构理解。

Method: 引入通用Costas矩阵和通用Costas频率矩阵，研究其结构特性；提出集成UCM和UCFM的统一框架；开发基于重构的搜索方法，从UCFM生成UCM；为未来人工智能辅助的Costas阵列发现奠定基础。

Result: 数值结果表明，所提方法显著加速了搜索过程，增强了Costas阵列生成的结构洞察力，为高效发现新Costas阵列提供了有效途径。

Conclusion: 提出的统一框架为Costas阵列分析和发现提供了新视角，基于重构的搜索方法提高了效率，为未来人工智能辅助的Costas阵列研究奠定了基础。
...
### [154] [Generative Decompression: Optimal Lossy Decoding Against Distribution Mismatch](https://arxiv.org/abs/2602.03505)
*Saeed R. Khosravirad,Ahmed Alkhateeb,Ingrid van de Voorde*

Main category: cs.IT

TL;DR: 论文研究了在源分布失配情况下的最优解码策略，提出了生成式解压缩方法，通过贝叶斯校正提升解码性能，适用于有噪信道传输和任务导向解码场景。


<details>
  <summary>Details</summary>
Motivation: 在标准化通信系统中，编码器使用固定的设计分布，而解码器可能获得关于真实源分布的额外信息或先验知识，这种分布失配问题需要新的解码策略来提升性能。

Method: 提出了生成式解压缩方法，基于真实分布对量化索引进行条件期望估计，实现贝叶斯校正；扩展到有噪信道传输的软解码规则；并推广到任务导向解码的最大后验检测策略。

Result: 在高斯源和基于深度学习的语义分类实验中，生成式解压缩方法显著缩小了与理想联合优化基准的性能差距，实现了自适应高保真重建而无需修改编码器。

Conclusion: 生成式解压缩为分布失配问题提供了有效的解码策略，严格优于传统的质心规则，揭示了标准模块化源-信道分离架构在失配情况下的效率低下，为自适应通信系统设计提供了新思路。
...
### [155] [Secure Decentralized Pliable Index Coding for Target Data Size](https://arxiv.org/abs/2602.03579)
*Anjali Padmanabhan,Danya Arun Bindhu,Nujoom Sageer Karat,Shanuja Sasi*

Main category: cs.IT

TL;DR: 本文研究了具有异构边信息基数的去中心化可塑索引编码问题，提出了一种在安全约束下最大化编码效率的传输方案。


<details>
  <summary>Details</summary>
Motivation: 现有的DPIC研究大多假设同质设置（相同的边信息基数和单消息需求），这限制了在现实世界中的应用，因为客户端通常拥有不等量的先验信息。本文旨在解决异构边信息基数下的DPIC问题。

Method: 提出了一种传输方案，协调客户端广播以最大化编码效率，同时确保每个客户端达到共同目标水平T。方案还施加了严格的安全约束，保证没有客户端获得超过T条消息，每个客户端最终恰好获得T条消息。

Result: 分析了在安全约束下所提方案的通信成本，研究了异构边信息基数对编码效率的影响。

Conclusion: 该研究扩展了DPIC问题到异构环境，提出的方案在保证安全约束的同时优化了通信效率，为分布式系统中信息交换的实际应用提供了更现实的解决方案。
...
TL;DR: 该论文研究了大型语言模型在零样本推荐中的不确定性、公平性和可靠性问题，提出了包含不确定性评估和人格感知公平性的新评估框架，并在电影和音乐推荐领域进行了实证分析。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在零样本推荐中展现出强大能力，但其预测不确定性和内嵌偏见威胁着推荐系统的可靠性和公平性。需要系统评估这些因素如何影响LLM生成推荐的准确性、一致性和可信度。

Method: 1) 构建包含八个人口统计属性（31个分类值）的标注数据集，涵盖电影和音乐两个领域；2) 引入基准评估指标；3) 通过深度案例研究量化预测不确定性（使用熵度量）；4) 分析Google DeepMind Gemini 1.5 Flash对敏感属性的系统性不公平；5) 在提示扰动（如拼写错误和多语言输入）下测试公平性；6) 将人格感知公平性整合到RecLLM评估流程中。

Result: 研究发现：1) Gemini 1.5 Flash对某些敏感属性表现出系统性不公平，相似性差距指标SNSR为0.1363，SNSV为0.0507；2) 这些不公平在提示扰动下持续存在；3) 揭示了人格特征与偏见模式之间的关联；4) 暴露了个性化与群体公平性之间的权衡关系。

Conclusion: 论文提出了一个新颖的不确定性感知评估方法，建立了人格特征知情公平性基准，为提高LLM推荐的可解释性和公平性奠定了基础。这些贡献为构建更安全、更可解释的RecLLM提供了框架，并激励未来在多模型基准和自适应校准方面的研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM自我解释作为AI监督工具备受关注，但其忠实度（即解释是否真实反映模型推理过程）缺乏有效评估。现有忠实度评估方法存在局限，主要依赖对抗性提示或错误检测，忽视了解释的预测价值。

Method: 提出Normalized Simulatability Gain（NSG）指标，基于"忠实解释应能让观察者学习模型的决策标准，从而更好预测相关输入上的行为"这一理念。在7,000个来自健康、商业、伦理等领域的反事实数据上评估了18个前沿专有和开源模型。

Result: 自我解释显著提升模型行为预测能力（11-37% NSG增益），且比外部模型生成的解释提供更多预测信息，即使外部模型更强。这表明自我知识带来的优势是外部解释方法无法复制的。同时发现5-15%的自我解释存在严重误导性。

Conclusion: 尽管存在缺陷，自我解释确实编码了有助于预测模型行为的信息，为自我解释的实用价值提供了积极证据。NSG指标为评估解释忠实度提供了通用且可扩展的方法。
...
Method: 基于交互步骤敏感度差异的观察，提出了动态混合精度路由框架，在每一步决策中自适应选择高精度或低精度LLM。路由器通过两阶段管道训练：首先使用KL散度监督学习识别精度敏感步骤，然后使用组相对策略优化（GRPO）进一步提高任务成功率。

Result: 在ALFWorld上的实验表明，该方法在准确率-成本权衡方面相比单精度基线和启发式路由方法有显著改进。

Conclusion: 动态混合精度路由框架能够有效平衡长时程决策任务中的性能与成本，为实际应用提供了可行的解决方案。
...
TL;DR: Cuttlefish是一个统一的全原子大语言模型，通过自适应缩放结构补丁和几何接地适配器，在几何线索基础上进行语言推理，解决了现有方法的结构幻觉和模态融合瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生物分子结构推理中存在两个主要问题：1）通过序列化标记或固定长度查询连接器压缩结构输入，忽略了抑制结构幻觉所需的几何基础；2）采用不灵活的模态融合瓶颈，同时过度压缩和次优分配结构标记，阻碍了通用全原子推理的实现。

Method: 提出Cuttlefish模型，包含两个核心技术：1）缩放感知补丁：使用指令条件门控机制在结构图上生成可变大小的补丁，根据结构复杂度自适应缩放查询标记预算，缓解固定长度连接器瓶颈；2）几何接地适配器：通过跨注意力机制将自适应标记与模态嵌入进行细化，并将生成的模态标记注入LLM，暴露显式几何线索以减少结构幻觉。

Result: 在多样化的全原子基准测试中，Cuttlefish在异构结构接地推理方面实现了优越性能。

Conclusion: Cuttlefish通过自适应缩放结构补丁和几何接地机制，成功解决了现有方法的结构幻觉和模态融合瓶颈问题，实现了在几何线索基础上的统一全原子推理，为生物分子结构分析提供了更有效的解决方案。
...
TL;DR: STEER框架通过进化多样性搜索构建自然语言角色库，在推理时通过单一可解释参数控制决策保守度，解决LLM在顺序决策任务中的模式坍缩问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在平均正确性训练中常出现模式坍缩，在临床分诊等顺序决策任务中无法根据上下文约束权衡特异性和敏感性，标准对齐方法移除了ROC操作点的可调控制能力

Method: STEER（通过进化集成精炼的可调控调优）框架：1）通过离线约束质量-多样性搜索构建自然语言角色群体，在保证最低安全、推理和稳定性阈值的同时促进行为覆盖；2）推理时通过单一可解释控制参数将用户指定的风险百分位数映射到选定角色，实现决策保守度的单调调整

Result: 在两个临床分诊基准测试中，STEER相比基于温度的采样和静态角色集成实现了更广泛的行为覆盖；相比代表性后训练方法，在明确紧急病例上保持显著更高的准确性，同时在模糊决策上提供相当的控制能力

Conclusion: STEER作为一种安全保持的风险控制范式，能够在保持领域能力的同时调控行为，为解决LLM在顺序决策任务中的模式坍缩问题提供了有效方案
...
TL;DR: 该论文提出了BenchAlign方法，通过有限的实际部署信息自动更新离线基准测试，使其能更好地预测模型在实际应用中的性能偏好。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型基准测试虽然计算高效，但往往无法准确预测模型在真实世界中的实际效用。许多研究发现基准测试与实际性能之间存在差距，需要建立更好的预测机制。

Method: 提出了BenchAlign方法，通过收集模型在部署期间的成对排名信息，结合问题级别的性能数据，学习与人类偏好对齐的基准测试问题权重，生成新的静态基准测试。

Result: 实验表明，对齐后的基准测试能够准确根据人类偏好模型对未见过的模型进行排名，且在不同模型规模下都有效，同时保持可解释性。

Conclusion: 这项工作为基准测试与实际人类偏好的对齐提供了洞见，有望加速模型开发向真实效用的转化，提出了基准测试对齐的新方向。
...
TL;DR: 本文研究了思维链推理中推理token数量的理论下界，证明了三个典型任务需要Ω(n)个推理token，并通过实验验证了理论结果。


<details>
  <summary>Details</summary>
Motivation: 思维链推理虽然能提升LLM性能，但带来了显著的延迟和计算成本。本文旨在从理论上探究随着输入规模增长，解决问题需要多少推理token，以理解推理计算的基本瓶颈。

Method: 扩展了有界注意力前缀预言机模型，该模型量化了解决任务所需的信息流。通过理论分析证明了三个BAPO-hard任务（二进制多数、三元组匹配、图可达性）的推理token下界为Ω(n)。同时提供了匹配或接近匹配的上界构造，并通过前沿推理模型进行了实验验证。

Result: 理论证明三个任务都需要Ω(n)个推理token。实验显示前沿推理模型在这些任务上表现出近似线性的推理token缩放，当约束到较小的推理预算时会失败，这与理论下界一致。

Conclusion: 研究揭示了思维链推理中推理计算的基本瓶颈，为分析最优推理长度提供了原则性工具。结果表明推理token数量与输入规模线性相关，这对优化推理效率有重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 神经NLP模型经常存在校准问题，对错误预测分配高置信度，这影响了选择性预测和高风险部署。现有的后处理校准方法只调整输出概率而不改变内部计算，而集成和贝叶斯方法虽然能改善不确定性但训练或存储成本很高

Method: 提出UAT-LITE推理时框架，通过蒙特卡洛dropout在预训练transformer分类器中实现近似贝叶斯推断。从随机前向传递中估计token级认知不确定性，并用其调制自注意力机制。还引入了层间方差分解来诊断预测不确定性如何在transformer深度中累积

Result: 在SQuAD 2.0可回答性、MNLI和SST-2数据集上，UAT-LITE相比微调BERT-base基线平均减少约20%的期望校准误差，同时保持任务准确性，并改善了选择性预测和分布偏移下的鲁棒性

Conclusion: UAT-LITE提供了一种轻量级、无需训练的方法来改善预训练transformer模型的不确定性估计，通过使自注意力机制对不确定性敏感，在保持准确性的同时显著降低校准误差
...
<details>
  <summary>Details</summary>
Motivation: 现有研究利用几何特性（如对称结构编码、几何感知数据增强、结构限制）来稳定和加速强化学习。本文从序理论的新视角出发，将价值函数估计重新构建为学习期望的偏序集。

Method: 提出GCR-RL（几何一致性正则化强化学习），通过计算一系列超偏序集细化——通过细化先前步骤中的偏序集，并从时序差分信号中学习额外的序关系——确保支撑学习价值函数的偏序集序列具有几何一致性。开发了基于Q学习和演员-评论家的两种新算法来实现这些超偏序集细化。

Result: 分析了算法的理论性质和收敛速率。在多个任务上的实证评估表明，GCR-RL相比强基线方法在样本效率和稳定性能方面有显著提升。

Conclusion: 从序理论视角重新审视强化学习，提出的GCR-RL方法通过确保偏序集序列的几何一致性，有效提升了强化学习的稳定性和样本效率，为利用几何结构改进RL提供了新思路。
...
TL;DR: LLMs在因果推理任务中表现出比人类更规则化的推理策略，较少受到人类典型的共因偏误影响，但可能在不确定性场景下失效。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在需要因果推理的领域应用增多，需要了解其因果判断是基于规范性计算、人类式捷径还是脆弱的模式匹配。

Method: 使用共因结构(C₁→E←C₂)的11个因果判断任务，对20多个LLMs与匹配的人类基线进行基准测试，并分析其推理策略。

Result: LLMs的因果判断可通过小型可解释模型很好压缩；大多数LLMs表现出比人类更规则化的推理策略；LLMs较少表现出人类典型的弱解释消除和马尔可夫违规偏误；思维链提示能提高许多LLMs的鲁棒性。

Conclusion: LLMs在已知人类偏误不可取时可作为人类补充，但其规则化推理在不确定性场景下可能失效，需要深入理解LLM推理策略以确保安全有效部署。
...
TL;DR: LLMs在训练中获得了序列规划能力，但在推理时表现出短视和不一致的规划行为，作者提出贝叶斯解释：自生成上下文驱动规划偏移，导致看似受损的规划行为。


<details>
  <summary>Details</summary>
Motivation: 解释大型语言模型在训练中获得的序列级规划能力与推理时观察到的短视、不一致规划行为之间的差距，为这一现象提供理论解释。

Method: 提出贝叶斯解释框架，将规划行为基于不断演化的生成上下文；通过两个控制实验验证：随机生成任务（展示人类提示下的受限规划和自生成上下文积累时规划强度增加）和高斯采样任务（展示在自生成序列条件下初始偏差减少）。

Result: 实验验证了提出的模型：随机生成任务显示随着自生成上下文积累，规划强度增加；高斯采样任务显示在自生成序列条件下初始偏差减少，支持了贝叶斯解释框架。

Conclusion: 研究为LLMs在推理时如何进行前瞻规划提供了理论解释和实证证据，揭示了自生成上下文在驱动规划偏移中的关键作用，解释了训练能力与推理行为之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 社会选择已从政治理论和经济学的边缘领域转变为现代机器学习系统的核心组成部分。从拍卖、资源分配到联邦学习、参与式治理和大语言模型对齐，机器学习系统越来越多地聚合异质偏好、激励和判断以形成集体决策，但这些系统往往隐含地实现社会选择机制而缺乏明确的规范审查。

Method: 采用可微分社会选择范式，将投票规则、机制和聚合过程构建为可学习、可微分的模型，通过数据驱动的方式进行优化。该方法综合了拍卖、投票、预算分配、流动民主、去中心化聚合和逆向机制学习等多个领域的研究。

Result: 展示了经典公理和不可能性定理如何重新表现为目标函数、约束条件和优化权衡。通过这一范式，社会选择理论中的规范原则可以系统地融入机器学习系统的设计和优化中。

Conclusion: 提出了36个开放性问题，定义了机器学习、经济学和民主理论交叉领域的新研究议程。可微分社会选择为构建更透明、更规范、更符合社会价值的机器学习系统提供了理论框架和技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 部署大型语言模型进行判别任务时面临推理延迟、计算成本和API成本等限制。传统主动蒸馏方法仅蒸馏最终标签，丢弃了中间推理信号，且缺乏对缺失推理内容和错误来源的诊断能力。

Method: 提出Graph of Concept Predictors (GCP)框架：1) 将教师模型的决策过程外化为有向无环图；2) 在学生模型中用模块化概念预测器镜像该图；3) 采用图感知获取策略，针对关键推理节点的不确定性和分歧；4) 实施目标子模块重训练，将下游损失归因于特定概念预测器并仅更新最有影响的模块。

Result: 在八个NLP分类基准测试中，GCP在有限标注预算下提升了性能，同时产生了更可解释和可控的训练动态。

Conclusion: GCP框架通过外部化教师推理过程并采用模块化设计，显著提高了主动蒸馏的样本效率、训练稳定性和可解释性，为部署紧凑判别模型提供了有效解决方案。
...
TL;DR: 提出RC-GRPO方法解决LLM多轮工具调用中奖励稀疏和探索成本高的问题，通过奖励条件化轨迹策略和离散奖励令牌改善组内多样性，在BFCLv4基准上超越基线模型


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多轮工具调用任务中面临奖励稀疏和探索成本高的问题，传统的SFT+GRPO方法在组内奖励变化低时（如多数rollout获得全0或全1奖励）会停滞，导致组归一化优势信息不足，更新消失

Method: 提出RC-GRPO（奖励条件化组相对策略优化），将探索视为可控的转向问题：1）首先在混合质量轨迹上微调奖励条件化轨迹策略（RCTP），在提示中注入奖励目标特殊令牌（如<|high_reward|>, <|low_reward|>），使模型能按需生成不同质量的轨迹；2）在强化学习阶段，在每个GRPO组内采样多样化的奖励令牌，并在采样的令牌上条件化rollout，提高组内多样性，改善优势增益

Result: 在Berkeley Function Calling Leaderboard v4多轮基准测试中，该方法相比基线模型获得持续改进的性能，Qwen-2.5-7B-Instruct模型的性能甚至超过了所有闭源API模型

Conclusion: RC-GRPO通过奖励条件化方法有效解决了多轮工具调用中探索多样性不足的问题，显著提升了模型性能，证明了离散奖励令牌在改善强化学习探索中的有效性
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列分析方法在整合直观视觉推理和跨任务泛化方面存在局限，需要一种能够自适应使用工具并整合视觉推理的统一框架。

Method: 提出MAS4TS系统，基于分析器-推理器-执行器范式，整合智能体通信、视觉推理和潜在重建。首先使用视觉语言模型对时间序列图进行视觉推理提取时间结构，然后在潜在空间重建预测轨迹。三个专门智能体通过共享内存和门控通信协调，路由器选择任务特定工具链执行。

Result: 在多个基准测试上的广泛实验表明，MAS4TS在广泛的时间序列任务中实现了最先进的性能，同时展现出强大的泛化能力和高效的推理效率。

Conclusion: MAS4TS通过整合视觉推理、多智能体协调和自适应工具使用，为通用时间序列分析提供了一个强大且可扩展的框架，在性能和泛化能力方面超越了现有方法。
...
### [171] [KANFIS A Neuro-Symbolic Framework for Interpretable and Uncertainty-Aware Learning](https://arxiv.org/abs/2602.03034)
*Binbin Yong,Haoran Pei,Jun Shen,Haoran Li,Qingguo Zhou,Zhao Su*

Main category: cs.AI

TL;DR: 提出KANFIS（Kolmogorov-Arnold神经模糊推理系统），一种紧凑的神经符号架构，通过加法聚合机制解决传统ANFIS在高维空间中规则指数爆炸的问题，同时保持可解释性。
...
Method: 提出KANFIS架构，采用加法聚合机制而非乘积推理，使模型参数和规则复杂度随输入维度线性增长而非指数增长。支持Type-1和Interval Type-2模糊逻辑系统，通过稀疏掩码机制生成紧凑结构化规则集。

Result: KANFIS在保持可解释性的同时，实现了与代表性神经和神经模糊基准模型相竞争的性能表现。

Conclusion: KANFIS通过加法函数分解统一模糊推理，解决了传统ANFIS的结构复杂性问题，提供了紧凑、可解释且性能优越的神经模糊推理系统。
...
### [172] [MAS-ProVe: Understanding the Process Verification of Multi-Agent Systems](https://arxiv.org/abs/2602.03053)
...
Result: 过程级验证不能稳定提升性能且经常表现出高方差；LLM-as-a-Judge方法通常优于基于奖励的方法；经过训练的验证器优于通用大语言模型；验证器与单智能体之间的性能差距较小；存在上下文长度与性能的权衡。
...
TL;DR: 本文研究了多智能体LLM框架的架构设计对系统性能的影响，发现仅架构选择就可能导致100倍以上的延迟差异、30%的规划准确率下降，以及协调成功率从90%以上降至30%以下。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM框架虽然广泛用于加速基于大语言模型的智能体系统开发，但不同框架的架构设计对系统性能的影响缺乏深入理解。架构选择可能导致数量级的延迟和吞吐量差异，以及准确性和可扩展性的显著变化，但现有研究未能系统评估这些影响。

Method: 提出了一个多智能体LLM框架的架构分类法，用于系统比较不同框架的基本维度；开发了MAFBench统一评估套件，将现有基准测试集成到标准化的执行流程中；使用MAFBench对多个广泛使用的框架进行了受控实证研究。

Result: 研究发现仅框架层面的设计选择就能导致：延迟增加超过100倍；规划准确率降低高达30%；协调成功率从90%以上降至30%以下。这些结果表明架构选择对系统性能有决定性影响。

Conclusion: 研究填补了多智能体LLM框架架构评估的空白，提出了具体的架构设计原则和框架选择指导，并指出了未来研究方向。MAFBench为标准化框架级评估提供了工具，有助于开发者做出更明智的架构决策。
...
TL;DR: 论文扩展了先前关于智能体世界模型的研究，将确定性、完全可观测环境下的理论推广到随机智能体和部分可观测环境，证明随机智能体也无法避免学习其环境模型。


<details>
  <summary>Details</summary>
Motivation: 先前研究证明了在确定性智能体和完全可观测环境框架下，几乎所有最优且通用的智能体都包含足够的环境知识以进行近似重建。本研究旨在移除这两个限制条件，将理论扩展到更现实的随机智能体和部分可观测环境。

Method: 通过扩展先前框架，将定理推广到随机智能体在部分可观测环境中的情况。同时通过弱化通用性的概念，证明即使能力较弱的智能体也包含其操作环境的世界模型。

Result: 证明了随机智能体无法通过随机化避免学习其环境模型，并且即使能力较弱的智能体（弱化通用性条件）也包含其操作环境的世界模型。

Conclusion: 该研究将世界模型存在性定理扩展到更广泛的智能体类别，包括随机智能体和部分可观测环境，为理解智能体能力与限制提供了更全面的理论基础。
...
TL;DR: 提出一种通用的缺失模态恢复策略，通过增强扩散模型作为可插拔模块，在视觉语言模型中有效恢复缺失特征，提升在模态不完整情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常假设推理时模态完整，但当某些模态缺失或不完整时，其性能会急剧下降。现有方法面临两个困境：基于提示的方法难以恢复缺失但必要的特征并损害模型泛化能力；基于插补的方法缺乏有效指导，容易生成语义无关的噪声。如何在恢复精确语义的同时保持VLM的泛化能力仍具挑战性。

Method: 提出一种通用的缺失模态恢复策略，引入增强扩散模型作为可插拔的中阶段训练模块。包含两个关键创新：(1) 动态模态门控，自适应地利用条件特征引导生成语义一致的特征；(2) 跨模态互学习机制，桥接双编码器的语义空间以实现双向对齐。

Result: 在基准数据集上的零样本评估表明，该方法优于现有基线方法。大量实验和消融研究证实，该模型在缺失模态场景下是VLM的鲁棒且可扩展的扩展，确保在不同缺失率和环境下的可靠性。

Conclusion: 提出的通用缺失模态恢复策略通过增强扩散模型和创新的动态模态门控与跨模态互学习机制，有效解决了VLM在模态缺失情况下的性能下降问题，为视觉语言模型在现实世界不完整数据场景中的应用提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM与人类价值观对齐存在三个关键问题：1) 价值提取忽略层次结构；2) 评估只能检测存在性而非校准强度；3) 在可控强度下的LLM可引导性理解不足。需要更原则性的价值导向方法来解决这些限制。

Method: VALUEFLOW框架包含三个组件：1) HIVES - 层次价值嵌入空间，捕捉理论和跨理论价值结构；2) VIDB - 大规模价值标注文本数据库，基于排序聚合获得强度估计；3) 基于锚点的评估器，通过将模型输出与VIDB面板排序来产生一致的强度分数。

Result: 在十个模型和四个价值理论上的大规模研究表明：1) 识别了可引导性的不对称性；2) 发现了多价值控制的组合规律；3) 建立了评估和控制价值强度的可扩展基础设施。

Conclusion: VALUEFLOW为LLM的多元对齐提供了可扩展的基础设施，通过校准强度控制实现了价值提取、评估和引导的统一框架，推动了LLM与人类价值观的深度对齐。
...
TL;DR: 论文提出TAME框架，通过双记忆进化机制解决智能体在任务演化过程中的信任度下降问题，在保持任务性能的同时提升安全性。


<details>
  <summary>Details</summary>
Motivation: 智能体在测试时通过记忆演化积累经验是实现AGI的关键范式，但即使在良性任务演化过程中，智能体的安全对齐仍然脆弱，这种现象被称为"智能体记忆错误演化"。需要评估和解决这一安全问题。

Method: 提出TAME双记忆进化框架：1）分别演化执行器记忆（提升任务性能，提炼可泛化方法）和评估器记忆（基于历史反馈优化安全和任务效用评估）；2）通过记忆过滤、草稿生成、可信度精炼、执行和双轨记忆更新的闭环流程。

Result: 构建Trust-Memevo基准评估良性任务演化中的多维信任度，发现各任务领域和评估设置中信任度普遍下降。TAME实验表明能缓解错误演化，在信任度和任务性能上实现联合提升。

Conclusion: TAME框架通过分离演化的双记忆系统，在保持任务效用的同时保护智能体信任度，为解决智能体记忆错误演化问题提供了有效方案。
...
TL;DR: 提出统一评估框架以解决LLM智能体评估中的标准化问题，消除系统提示、工具配置和环境动态等混杂因素影响。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估存在严重标准化问题，评估结果受到系统提示、工具集配置、环境动态等混杂因素影响，不同研究使用碎片化框架导致难以区分性能提升是来自模型本身还是评估设置差异，缺乏标准化导致不公平和不透明。

Method: 提出标准化智能体评估的提案，旨在建立统一评估框架，标准化系统提示、工具配置、环境数据等评估要素，确保评估结果的可追溯性和可复现性。

Result: 论文提出了标准化评估框架的提案，但尚未展示具体实施结果，主要贡献在于识别当前评估问题并提出解决方案方向。

Conclusion: 统一评估框架对于LLM智能体评估的严谨发展至关重要，标准化能够消除混杂因素，确保评估公平、透明且可复现，从而推动领域进步。
...
TL;DR: Accordion-Thinking框架让LLMs通过动态总结学习自我调节推理步骤粒度，实现推理上下文压缩，在保持准确性的同时提升3倍吞吐量。
...
Method: 提出Accordion-Thinking端到端框架，让LLMs通过动态总结学习自我调节推理步骤粒度，采用Fold推理模式定期总结思维过程并丢弃历史标记，使用强化学习进一步激励这种能力。

Result: 模型学会将关键推理信息编码到紧凑总结中，实现推理上下文的有效压缩。在48GB GPU内存配置下，Accordion-Thinker在保持准确性的同时实现3倍吞吐量提升，结构化步骤总结提供人类可读的推理过程记录。

Conclusion: 通过学习的自我压缩机制，LLMs能够以最小的依赖标记开销处理复杂推理任务而不影响解决方案质量，同时提供可解释的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的安全行为可能由单模态捷径驱动而非真正的联合意图理解，需要评估模型在跨模态场景下的可靠性，诊断模态引起的行为变化。

Method: 引入CSR-Bench基准，包含四种压力测试交互模式：安全性、过度拒绝、偏见和幻觉，覆盖61种细粒度类型。每个实例需要图像-文本集成理解，并提供配对的纯文本控制组以诊断模态引起的行为变化。评估了16个最先进的多模态大语言模型。

Result: 观察到系统性跨模态对齐差距：模型表现出弱安全意识、在干扰下强烈的语言主导性，以及从纯文本控制到多模态输入的性能持续下降。还观察到减少过度拒绝与保持安全、非歧视行为之间的明显权衡，表明某些表面安全增益可能来自拒绝导向的启发式方法而非稳健的意图理解。

Conclusion: 多模态大语言模型在跨模态可靠性方面存在系统性缺陷，需要更稳健的跨模态对齐方法，而非依赖拒绝导向的启发式策略。CSR-Bench为评估和改进多模态模型的跨模态可靠性提供了重要基准。
...
TL;DR: Memora是一个谐波记忆表示系统，通过结构化平衡抽象与特异性来解决智能体记忆扩展问题，在记忆扩展时保持检索相关性和推理有效性。


<details>
  <summary>Details</summary>
Motivation: 智能体记忆系统需要处理不断增长的信息，同时支持高效、上下文感知的检索。抽象对于扩展记忆至关重要，但通常以牺牲特异性为代价，这会掩盖有效推理所需的细粒度细节。

Method: Memora采用谐波记忆表示，通过主要抽象索引具体记忆值，将相关更新整合为统一记忆条目，同时使用线索锚点扩展检索访问范围并连接相关记忆。在此基础上，采用检索策略主动利用这些记忆连接来检索超出直接语义相似性的相关信息。

Result: 理论上，标准检索增强生成（RAG）和基于知识图谱（KG）的记忆系统被证明是该框架的特殊情况。实证上，Memora在LoCoMo和LongMemEval基准测试中建立了新的最先进水平，展示了随着记忆扩展，更好的检索相关性和推理有效性。

Conclusion: Memora通过谐波记忆表示成功平衡了抽象与特异性，为智能体记忆系统提供了一种可扩展且有效的解决方案，能够随着记忆增长保持高质量的检索和推理能力。
...
TL;DR: MentalDx Bench是首个基于真实临床电子病历的精神障碍诊断基准，包含712份病历覆盖76种ICD-11障碍。研究发现LLMs在粗分类表现良好但在细粒度诊断上失败，为此提出了MentalSeek-Dx模型，通过监督轨迹构建和课程强化学习实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在精神健康评估中的应用受到基准测试生态效度不足和细粒度诊断监督缺乏的限制，需要建立更贴近真实临床环境的评估框架。

Method: 1) 构建MentalDx Bench基准：包含712份去识别化电子病历，由认证精神科医生按ICD-11标注，覆盖16个诊断类别的76种障碍；2) 提出MentalSeek-Dx模型：通过监督轨迹构建和课程强化学习训练医学专用LLM，模拟临床假设-演绎推理过程。

Result: 评估18个LLMs发现范式错位：在粗分类表现良好但在障碍级诊断上系统性失败。MentalSeek-Dx仅用14B参数就达到SOTA性能，在MentalDx Bench上表现优异。

Conclusion: 该研究填补了LLMs在精神障碍诊断评估中的空白，揭示了模式建模与临床推理之间的差距，提出的MentalSeek-Dx为可靠精神科诊断建立了临床基础框架。
...
TL;DR: 提出Risk Awareness Injection (RAI)框架，通过构建不安全原型子空间和针对性调制高风险视觉token，在无需训练的情况下增强视觉语言模型的安全识别能力，降低多模态越狱攻击成功率


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型(VLMs)在多模态越狱攻击面前非常脆弱，现有防御方法要么需要大量安全微调成本，要么通过激进的token操作显著降低模型效用。研究发现LLMs本身能识别文本中的不安全内容，但视觉输入会稀释风险信号，因此需要恢复VLMs的LLM-like风险识别能力

Method: 提出Risk Awareness Injection (RAI)框架：1) 从语言嵌入构建不安全原型子空间；2) 对选定的高风险视觉token进行针对性调制，在跨模态特征空间中显式激活安全关键信号；3) 保持原始token的语义完整性以维持跨模态推理能力

Result: 在多个越狱攻击和效用基准测试上的广泛实验表明，RAI显著降低了攻击成功率，同时没有损害任务性能

Conclusion: RAI是一个轻量级、无需训练的框架，通过放大VLMs中的不安全信号来恢复LLM-like的风险识别能力，有效增强模型安全性而不影响效用
...
TL;DR: 本文提出了一种基于直觉模糊偏好的三支冲突分析模型，通过引入直觉模糊偏好关系来更精细地描述智能体对议题对的态度，克服了传统偏好模型中仅使用三种定性关系的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于偏好的冲突分析模型仅使用偏好、逆偏好和中性三种定性关系来描述智能体对议题对的态度，这种粗糙的表示方法严重限制了模型捕捉冲突本质的能力，需要更精细的表示方法来更好地描述冲突情境。

Method: 1. 引入直觉模糊偏好冲突情境概念，用直觉模糊偏好关系更精细地描述智能体对议题对的态度；2. 在该框架下开发直觉模糊偏好冲突度量；3. 构建三支冲突分析模型，用于三分智能体对集合、智能体集合和议题集合；4. 基于提出的冲突函数构建相对损失函数来计算三支冲突分析的阈值；5. 提出基于调整机制的可行策略，同时考虑调整幅度和冲突程度，并提供构建此类策略的算法。

Result: 提出了一个完整的直觉模糊偏好三支冲突分析框架，包括冲突情境建模、冲突度量、三支分析模型、阈值计算方法和可行策略构建算法，并通过示例验证了模型的有效性。

Conclusion: 本文提出的直觉模糊偏好三支冲突分析模型能够更精细地描述冲突情境，克服了传统模型的局限性，为冲突分析提供了更有效的理论框架和实用工具。
...
TL;DR: DiscoverLLM是一个训练大语言模型帮助用户形成和发现意图的新框架，通过模拟用户认知状态和渐进具体化的意图层次，使模型学会在意图不明确时探索选项，在意图具体化时细化实现。
...
Method: 提出DiscoverLLM框架，核心是一个新颖的用户模拟器，用层次化的意图结构建模认知状态，意图随着模型展示相关选项而渐进具体化。具体化程度作为奖励信号，模型可以被训练来优化这个信号。模型学会与用户协作：在意图不明确时自适应地发散（探索选项），在意图具体化时收敛（细化和实现）。

Result: 在创意写作、技术写作和SVG绘图等交互式基准测试中，DiscoverLLM实现了超过10%的任务性能提升，同时将对话长度减少了40%。在75名人类参与者的用户研究中，DiscoverLLM相比基线方法提高了对话满意度和效率。

Conclusion: DiscoverLLM提供了一种新颖且可推广的框架，训练大语言模型帮助用户形成和发现意图，通过模拟用户认知状态和渐进具体化的意图层次，使模型能够更有效地与用户协作，在意图探索和实现之间取得平衡。
...
<details>
  <summary>Details</summary>
Motivation: 在开放世界环境中，具身智能体需要终身学习能力。持续强化学习是VLA模型在终身机器人场景中的有前景路径，但现有方法在保持旧技能稳定性与学习新技能可塑性之间的平衡面临挑战。

Method: 提出CRL-VLA框架，通过理论推导建立性能边界，采用非对称调节机制：约束先前任务的优势幅度，同时允许新任务上的受控增长。实现方式包括双评论家架构和新的目标条件价值公式，其中冻结评论家保持语义一致性，可训练估计器驱动适应。

Result: 在LIBERO基准测试中，CRL-VLA有效协调了冲突目标，在抗遗忘和正向适应方面均优于基线方法。

Conclusion: CRL-VLA框架通过理论指导的非对称调节机制成功解决了VLA模型在持续强化学习中的稳定性-可塑性权衡问题，为终身机器人学习提供了有效解决方案。
...
TL;DR: 研究形式化抽象（移除和聚类）如何影响人类推理表现和认知负荷，使用ASP框架简化解释，实验显示聚类提升理解，移除降低认知负荷


<details>
  <summary>Details</summary>
Motivation: AI系统产生的输出往往难以理解，虽然符号AI为可解释性提供了透明基础，但原始逻辑追踪通常带来较高的外在认知负荷。需要研究如何通过形式化抽象来改善人类对符号解释的理解

Method: 使用答案集编程（ASP）作为形式化框架，定义不相关细节的概念进行抽象化处理，通过认知实验让参与者在不同领域中对刺激进行分类，比较基于原始ASP程序、移除细节和聚类细节三种解释方式的效果

Result: 认知实验显示：聚类细节显著提高参与者的理解能力，而移除细节则显著降低认知努力，支持了抽象化能增强以人为中心的符号解释的假设

Conclusion: 形式化抽象（特别是移除和聚类）能够有效改善人类对符号AI解释的理解，降低认知负荷，为开发更人性化的可解释AI系统提供了重要指导
...
TL;DR: 论文发现现有LLM路由系统存在"路由崩溃"现象：随着成本预算增加，路由器会系统性地选择最强大但最昂贵的模型，即使更便宜的模型已足够，导致小模型利用率不足。作者提出EquiRouter直接学习模型排序来解决此问题。
...
Method: 作者提出EquiRouter，一种决策感知的路由器，直接学习模型排序而非预测标量性能分数。这种方法旨在解决目标-决策不匹配问题：现有路由器训练预测标量性能分数，而路由决策最终依赖于候选模型之间的离散比较。

Result: 在RouterBench上，EquiRouter在达到GPT-4级别性能时，相比先前最强的路由器减少了约17%的成本。该方法有效恢复了小模型的作用，缓解了路由崩溃现象。

Conclusion: 论文揭示了LLM路由中存在的"路由崩溃"问题，并提出EquiRouter通过直接学习模型排序来解决目标-决策不匹配问题。该方法在单模态和多模态设置中都能有效提升小模型利用率，实现更好的质量-成本权衡。
...
TL;DR: LLMs在静态医疗推理任务表现良好，但作为动态医疗世界模型模拟疾病进展和治疗效果时存在局限性，特别是在长期临床模拟中难以保持一致的病人状态。研究者开发了EHRWorld模型和EHRWorld-110K数据集来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 世界模型为干预下的未来状态模拟提供了原则性框架，但在复杂高风险的医疗领域实现这样的模型仍然具有挑战性。虽然大型语言模型在静态医疗推理任务上表现出色，但能否作为动态医疗世界模型来模拟疾病进展和治疗效果仍是一个问题。

Method: 提出了EHRWorld，这是一个基于因果序列范式训练的病人中心医疗世界模型，同时创建了EHRWorld-110K，这是一个从真实世界电子健康记录中提取的大规模纵向临床数据集。

Result: EHRWorld显著优于基于LLM的基线方法，实现了更稳定的长期模拟、更好的临床敏感事件建模以及更优的推理效率。这表明基于因果基础、时间演化的临床数据训练对于可靠稳健的医疗世界建模是必要的。

Conclusion: 仅依靠医学知识的LLM在序列干预下难以保持一致的病人状态，导致长期临床模拟中的错误累积。通过因果序列范式训练的病人中心医疗世界模型EHRWorld能够有效解决这一问题，为可靠的医疗世界建模提供了新方向。
...
### [190] [Mitigating Conversational Inertia in Multi-Turn Agents](https://arxiv.org/abs/2602.03664)
*Yang Wan,Zheng Cao,Zhenhao Zhang,Zhengwen Zeng,Shuheng Shen,Changhua Meng,Linchao Zhu*

Main category: cs.AI

TL;DR: 论文提出LLM在作为多轮智能体时存在"对话惯性"问题，即模型会模仿自己之前的回答作为few-shot示例，限制了探索能力。作者提出基于上下文偏好的学习方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为few-shot学习者在提供适当演示时表现出色，但在多轮智能体场景中，LLM会错误地将自己之前的响应作为few-shot示例进行模仿，这成为一个问题。通过注意力分析，作者发现了"对话惯性"现象，即模型对先前响应表现出强烈的对角线注意力，这与限制探索的模仿偏见相关。

Method: 基于关键洞察：对于相同状态，使用较长上下文生成的动作比使用较短上下文生成的动作表现出更强的惯性，这使得无需环境奖励即可构建偏好对。作者提出上下文偏好学习来校准模型偏好，使其倾向于低惯性响应而非高惯性响应。此外，在推理时提供上下文管理策略以平衡探索和利用。

Result: 在八个智能体环境和一个深度研究场景中的实验结果表明，该框架减少了对话惯性并实现了性能改进。

Conclusion: 研究揭示了将few-shot LLM转化为智能体时的内在张力：较长上下文丰富了环境反馈以进行利用，但也放大了削弱探索的对话惯性。提出的上下文偏好学习框架有效解决了这一问题。
...
### [191] [TodyComm: Task-Oriented Dynamic Communication for Multi-Round LLM-based Multi-Agent System](https://arxiv.org/abs/2602.03688)
*Wenzhe Fan,Tommaso Tognoli,Henry Peng Zou,Chunyu Miao,Yibo Wang,Xinhua Zhang*

Main category: cs.AI

TL;DR: 提出TodyComm算法，通过任务导向的动态通信拓扑解决多轮LLM多智能体系统中固定通信结构无法适应角色动态变化的问题


<details>
  <summary>Details</summary>
Motivation: 现有多轮LLM多智能体系统大多采用固定的通信拓扑结构，但在实际应用中，智能体的角色可能因动态对抗、任务进展或时变约束（如通信带宽）而在不同轮次间发生变化，固定拓扑无法适应这种动态性

Method: 提出TodyComm（任务导向动态通信算法），生成行为驱动的协作拓扑，能够根据每轮动态调整，通过策略梯度优化任务效用

Result: 在五个基准测试上的实验表明，在动态对抗和通信预算约束下，TodyComm在任务有效性方面表现优越，同时保持了令牌效率和可扩展性

Conclusion: TodyComm算法能够有效解决多轮LLM多智能体系统中通信拓扑的动态适应问题，在动态环境中提供更优的任务性能
...
### [192] [AOrchestra: Automating Sub-Agent Creation for Agentic Orchestration](https://arxiv.org/abs/2602.03786)
...
TL;DR: AOrchestra是一个基于统一智能体抽象框架的编排系统，通过动态创建专用执行器来解决复杂长时任务，在多个基准测试中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有语言智能体系统缺乏对子智能体的动态抽象视图，限制了系统的适应性和灵活性，无法有效处理复杂、长时程的任务

Method: 提出统一的框架无关智能体抽象模型（指令、上下文、工具、模型四元组），基于此构建AOrchestra系统，中央编排器在每个步骤中具体化该四元组，动态创建专用执行器

Result: 在GAIA、SWE-Bench、Terminal-Bench三个挑战性基准测试中，AOrchestra配合Gemini-3-Flash相比最强基线获得16.28%的相对提升

Conclusion: AOrchestra通过统一的智能体抽象和动态编排机制，显著提升了语言智能体处理复杂任务的能力，实现了性能与成本的帕累托优化
...
### [193] [Understanding Agent Scaling in LLM-Based Multi-Agent Systems via Diversity](https://arxiv.org/abs/2602.03794)
*Yingxuan Yang,Chengrui Qu,Muning Wen,Laixi Shi,Ying Wen,Weinan Zhang,Adam Wierman,Shangding Gu*

Main category: cs.AI

TL;DR: 研究发现：基于LLM的多智能体系统在增加同质智能体数量时存在收益递减，而引入异质性（不同模型、提示或工具）能持续带来显著性能提升。信息论框架表明MAS性能受限于任务固有不确定性而非智能体数量，异质智能体通过提供互补证据突破同质系统的饱和限制。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统被认为是解决复杂任务的有前景方法，但单纯增加同质智能体数量存在收益递减问题。研究旨在探索：什么限制了系统规模扩展？为什么多样性有助于提升性能？

Method: 提出信息论框架，证明MAS性能受限于任务固有不确定性而非智能体数量。推导架构无关的边界，显示性能提升取决于系统访问的有效通道数量。引入K*（有效通道计数）来量化无真实标签时的有效通道数量。通过实验验证异质配置优于同质扩展。

Result: 实验表明异质配置始终优于同质扩展：2个异质智能体的性能可匹配或超过16个同质智能体。异质智能体通过提供互补证据突破同质系统的饱和限制。

Conclusion: 多智能体系统性能受限于任务固有不确定性而非智能体数量。异质性通过增加有效通道数量提升性能，为构建高效稳健的MAS提供了基于多样性的设计原则。
...
### [194] [Conformal Thinking: Risk Control for Reasoning on a Compute Budget](https://arxiv.org/abs/2602.03814)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型支持测试时扩展，随着token预算增加，数据集级准确率会提升，这促使了自适应推理的需求——在能提高可靠性时使用更多token，在额外计算不太可能帮助时提前停止。然而，设置token预算和自适应推理阈值是一个实际挑战，涉及基本风险-准确率权衡。

Method: 将预算设置问题重新定义为风险控制问题，在限制错误率的同时最小化计算。框架引入：1）上阈值：当模型置信度高时停止推理（可能产生错误输出）；2）新颖的参数化下阈值：预先停止无法解决的实例（可能过早停止）。给定目标风险和验证集，使用无分布风险控制来优化指定这些停止机制。对于多预算控制标准场景，引入效率损失来选择计算效率最高的退出机制。

Result: 在不同推理任务和模型上的实证结果表明，风险控制方法有效，展示了从下阈值和集成停止机制中获得计算效率提升，同时遵守用户指定的风险目标。

Conclusion: 该研究提出了一个实用的风险控制框架，用于管理LLM推理中的计算预算，在保证错误率的同时优化计算效率，解决了自适应推理中的风险-准确率权衡问题。
...
### [195] [AutoFigure: Generating and Refining Publication-Ready Scientific Illustrations](https://arxiv.org/abs/2602.03828)
...
Method: 提出AutoFigure智能体框架，在渲染最终结果前进行广泛思考、重组和验证，生成结构完整且美观的布局；基于FigureBench数据集进行实验。

Result: AutoFigure在所有基线方法中表现最佳，能够生成可直接用于发表的高质量科学插图。

Conclusion: FigureBench为科学插图生成提供了首个大规模基准，AutoFigure框架通过智能体方法有效解决了科学插图自动生成的挑战。
...
### [196] [Thinking Like a Doctor: Conversational Diagnosis through the Exploration of Diagnostic Knowledge Graphs](https://arxiv.org/abs/2602.01995)
*Jeongmoon Won,Seungwon Kook,Yohan Jo*

Main category: cs.AI

TL;DR: 提出基于知识图谱的对话诊断系统，通过两步推理（生成假设和验证假设）来改进临床对话诊断的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有对话诊断方法要么依赖模型的参数化知识，要么假设患者提供丰富具体的信息，这在现实中不切实际。需要解决信息不完整情况下的诊断推理问题。

Method: 提出基于诊断知识图谱的对话诊断系统，采用两步推理：1）从对话上下文生成诊断假设；2）通过澄清问题验证假设，循环直到得出最终诊断。使用MIMIC-IV患者资料和模拟器，并调整模拟器以反映真实患者症状描述的模糊性。

Result: 实验显示在诊断准确性和效率上优于强基线方法。医生评估支持模拟器的真实性和生成问题的临床实用性。

Conclusion: 提出的基于知识图谱的对话诊断系统能够有效处理信息不完整情况下的临床诊断问题，提高了诊断准确性和效率，具有临床实用价值。
