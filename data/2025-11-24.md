...
TL;DR: 本文展示了如何使用拓扑数据分析中的持久同调方法来客观区分艺术风格，包括不同艺术家、同一流派内艺术家，以及真实艺术家作品与AI生成作品的区别。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观性。本文旨在通过数学工具为艺术分析提供更结构化和可解释的见解。

Method: 使用拓扑数据分析中的持久同调方法，这是一种能够捕捉数据拓扑特征的数学工具。

Result: 持久同调方法能够以统计确定性区分不同艺术家（包括不同艺术流派和同一流派内）、以及真实艺术家作品与AI生成的模仿风格作品。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的数学框架，能够有效识别艺术作品的风格特征。
...
Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型。此外，该模型对基于风格的图像编辑有效，特定风格参数对应不同的面部运动。
...
<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制在情感记忆保存和婚礼摄影规划等方面有重要应用价值，但缺乏专门的数据集阻碍了高质量双人肖像生成的发展。

Method: 构建PairHuman数据集，包含10万+图像，涵盖多种场景、服装和双人互动，并提供丰富的元数据；开发DHumanDiff基线方法，增强面部一致性，平衡个性化人物生成和语义驱动场景创建。
...
Method: 采用无监督自编码器，在潜在空间中使用Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪处理。
...
TL;DR: 提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代方案来改善视觉语言模型的安全性，同时保持泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法在提高视觉语言模型安全性时会导致显著的泛化性能下降，这是因为刚性对齐策略破坏了模型的语义结构。

Method: 采用邻近感知方法，将不安全概念重定向到语义上最接近的安全替代方案，实现最小干预原则。

Result: SaFeR-CLIP在保持强健安全性的同时，比先前方法恢复了高达8.0%的零样本准确率。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来解决自回归网格生成模型中的顺序依赖问题，显著提升网格质量、加速生成速度并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统手工制作3D网格耗时且难以扩展，现有自回归模型存在严重的质量-速度权衡问题，顺序依赖也限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册来检索、生成和集成网格组件，解耦生成过程的顺序依赖。

Result: Mesh RAG在各种基础自回归网格生成模型中显著提升网格质量，相比顺序部件预测加速生成速度，并实现增量编辑功能。

Conclusion: Mesh RAG框架有效克服了自回归网格生成模型的顺序依赖限制，在保持模型无需重新训练的同时，实现了质量提升、速度加速和编辑灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 弥合创意意图与功能虚拟空间之间的差距，让创作者无需专业3D技能就能设计连贯、可导航的世界，推动可扩展的生成式世界构建技术发展。

Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，采用模块化架构支持对布局、规模和风格的细粒度控制。

Result: 系统能够生成几何一致、视觉丰富且实时渲染效率高的3D世界，可直接在标准游戏引擎中探索或编辑，创建完全纹理化的可穿越环境。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准数据集，专注于描述性和回归任务，特别在森林生态场景中建立了从人类活动到地上生物量的认知逻辑链。
...
Result: 对通用视觉语言模型的全面评估显示，现有模型在数值推理方面存在困难，揭示了科学视觉语言模型的关键挑战。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含150k图像和33M问答对，涵盖6个任务，旨在评估和提升视觉语言模型在精细空间理解、物体交互和物理兼容性方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但缺乏对精细空间理解、物体交互和物理兼容性的评估，这些能力对实际应用至关重要。

Method: 利用BOP数据集的6D物体姿态生成精细注释，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行精细物体中心空间推理的新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，展示了在精细空间理解和物体交互推理方面的显著改进，并发布了数据集和生成流程。
...
TL;DR: 提出Parts-Mamba模型，结合GCN和Mamba架构，通过部件特定扫描和部件-身体融合模块解决骨架动作识别中因遮挡导致的局部上下文缺失问题，在NTU RGB+D数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于遮挡或通信质量问题，捕获的骨架数据往往不完整，现有GCN模型在存在此类非理想情况时性能较差，主要原因是缺少局部上下文信息。
...
Conclusion: Parts-Mamba模型能够有效增强从远距离关节捕获和保持上下文信息的能力，在存在遮挡等非理想情况下显著提升骨架动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein（JGW）目标，扩展了传统的GW距离框架，使其能够同时匹配对象集合，解决了传统GW只能进行成对匹配的限制。
...
Result: 与其他GW变体相比，该方法在部分匹配方面表现出更高的准确性和计算效率，在合成和真实数据集上的实验证明了其在多形状匹配中的有效性。
...
TL;DR: 本文提出REPA方法，通过在逆问题求解过程中对齐扩散模型与预训练自监督编码器的内部表示，显著提升重建质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，在生成模型中强制对齐扩散模型与预训练自监督编码器的内部表示能改善收敛性和样本质量。本文旨在将此思想扩展到逆问题领域，利用预训练生成模型作为先验。

Method: 提出REPA方法，在推理时对齐扩散模型与DINOv2等预训练自监督视觉编码器的表示，即使没有真实信号也能通过近似目标特征对齐来指导重建过程。

Result: 实验表明REPA在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提升重建质量，同时通过减少离散化步骤显著提高效率。

Conclusion: REPA方法为逆问题求解提供了有效的表示对齐机制，在保持底层求解器性能的同时显著提升重建保真度和感知真实性。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的固有特性。观察到现实场景中玻璃前后光照强度差异导致反射变化的现象，提出利用闪光/无闪光图像对来改进检测精度。
...
Conclusion: 利用闪光/无闪光图像中的反射动态特性能够有效改进玻璃表面检测性能，提出的NFGlassNet方法在多个场景下都表现出优越的检测效果。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视频时空推理任务。R-AVST包含5K个未剪辑视频，涵盖100种音频-视频事件，AVST-Zero采用强化学习方法直接优化行为，无需中间监督。
...
Method: 1) 构建R-AVST数据集：使用LLM提取关键对象、自动空间标注和人工质量检查的流程；2) 提出AVST-Zero模型：基于强化学习，通过精心设计的多维度奖励直接优化行为，避免中间监督。

Result: 构建了包含5K个未剪辑视频、27K个对象、100种音频-视频事件的R-AVST数据集，生成了8K+高质量问答对。AVST-Zero在实验中表现出与现有模型竞争的性能。

Conclusion: R-AVST是首个专为真实世界音频-视频时空推理设计的数据集，AVST-Zero为该领域未来挑战提供了新的解决视角，有效推进了音频-视频时空推理能力。
...
TL;DR: 本文提出了Warm Diffusion模型，统一了热扩散（仅噪声）和冷扩散（仅模糊）两种范式，通过混合模糊和噪声来平衡图像相关性和随机性，解决了热扩散忽视图像结构相关性和冷扩散忽视数据流形的问题。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，忽视了高频细节与低频结构之间的强相关性，导致生成早期步骤的随机行为；冷扩散仅使用模糊，虽然利用了图像相关性进行预测，但忽视了噪声在塑造数据流形中的作用，导致流形外问题。需要整合两者的优势。

Method: 提出Warm Diffusion（BNMD模型），联合控制模糊和噪声，采用分治策略利用图像的光谱依赖性，通过分离去噪和去模糊过程简化分数模型估计，并使用光谱分析研究模糊-噪声比（BNR）来平衡模型学习动态和数据流形变化。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成方面的有效性。

Conclusion: Warm Diffusion通过混合模糊和噪声，成功整合了热扩散和冷扩散的优势，在图像生成任务中表现出色，为扩散模型提供了新的统一框架。
...
TL;DR: UniModel是一个统一的生成模型，在单个像素到像素的扩散框架中同时支持视觉理解和视觉生成。通过将文本和图像映射到共享的视觉空间，消除模态差异，实现模型、任务和表示三个维度的统一。


<details>
  <summary>Details</summary>
Motivation: 旨在实现多模态学习的完全视觉原生表述，通过在单一视觉空间中统一处理视觉理解和生成任务，探索通用多模态智能的潜力。

Method: 使用统一扩散变换器，在像素空间中采用整流流训练。将文本提示渲染为绘制文本图像，所有输入输出都作为RGB像素处理。通过轻量级任务嵌入指定期望的映射方向。

Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐能力，并展现出循环一致的图像-标题-图像环等涌现可控性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决完全没有真实异常样本或训练可用的情况。

Method: 提出Delta-Denoising方法：1）使用最小提示对驱动两个共享计划的扩散分支进行对比；2）通过累积每步去噪差异生成图像特定的定位图；3）利用该掩码指导潜在修复，在保留上下文的同时生成局部缺陷；4）进行标记级提示优化和对齐异常标记的空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了出色的生成效果、真实性和下游检测性能的一致提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效定位和生成局部缺陷，同时保持周围环境的完整性，在异常检测任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频超分辨率方法存在错误累积、空间伪影以及感知质量与保真度之间的权衡问题，主要原因是视频帧之间的不准确对齐和不足补偿。

Method: 提出DGAF-VSR方法，包含光学引导变形模块(OGWM)在放大分辨率下保持对齐特征的高频细节，以及特征域时序条件模块(FTCM)在特征域提供密集指导。

Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量(DISTS降低35.82%)、保真度(PSNR增益0.20dB)和时间一致性(tLPIPS降低30.37%)方面均优于现有最先进方法。

Conclusion: 特征域的信息补偿比像素域更有效，在放大分辨率下进行变形能更好地保留高频信息，提出的DGAF-VSR方法在多个关键指标上实现了显著提升。
...
TL;DR: 提出了一种融合语义和形状感知的深度学习框架，用于解决CBCT图像中牙齿分割的挑战，特别是在邻牙粘连导致严重形状变形的情况下。


<details>
  <summary>Details</summary>
Motivation: 解决CBCT图像中因邻牙粘连导致的严重解剖形状变形问题，实现准确的牙齿分割，这对数字化牙科至关重要。

Method: 引入目标牙齿中心点提示的多标签学习策略建模牙齿间语义关系，结合牙齿形状感知学习机制显式施加形态约束，通过多任务学习统一优化分割和形状保持。

Result: 在内部和外部数据集上的广泛评估表明，该方法显著优于现有方法。

Conclusion: 该方法有效减轻了形状变形，提供了解剖学上忠实可靠的牙齿边界。
...
TL;DR: 本文提出了OmniGround基准测试和PG-TAF方法，解决了时空视频定位中的类别偏见和推理简化问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在时空视频定位任务中与真实世界需求存在显著差距，主要由于基准测试范围有限导致模型出现类别偏见、过度简化推理和语言鲁棒性差的问题。

Method: 提出了OmniGround基准测试（包含3,475个视频，81个类别）和Forward-Backward-Refinement标注流程，以及PG-TAF训练免费的两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 在复杂真实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系中。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试中均取得一致提升。

Conclusion: OmniGround基准测试和PG-TAF方法有效解决了时空视频定位中的现有局限，显著提升了模型在复杂真实场景中的性能表现。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，评估了50多个模型，揭示了现有安全对齐对推理攻击无效等关键发现。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级链接隐私风险。现有隐私基准主要评估隐私感知，无法应对更关键的隐私推理风险——模型推断和链接分散信息构建个体档案的能力。

Method: 提出MultiPriv基准，引入隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，其中标识符与敏感属性精心链接，设计了9个挑战性任务评估完整的隐私感知与推理谱系。

Result: 大规模评估50多个基础模型和商业模型发现：(1)许多模型存在显著但未测量的基于推理的隐私风险；(2)感知级指标无法预测推理风险；(3)现有安全对齐对推理攻击不一致且无效。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光学流方程作为物理约束，在动态心脏MRI中实现了优于现有方法的重建质量和运动估计精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于预估计光流的运动补偿重建方法在欠采样情况下不准确，会降低重建质量，需要一种能同时恢复图像和运动场的方法。

Method: 使用两个隐式神经表示分别参数化时空图像内容和光流场，通过光学流方程耦合两者作为物理正则化，并结合数据一致性损失进行联合优化。

Result: 在动态心脏MRI数据集上的实验表明，该方法在重建质量、运动估计准确性和时间保真度方面均优于现有最先进的运动补偿和深度学习方法。

Conclusion: 基于流正则化约束的隐式联合建模方法在动态MRI重建中具有显著潜力，能够同时获得高质量的重建图像和准确的运动场估计。
...
TL;DR: 本文提出了FingerCap任务，用于生成捕捉手部动作细节的手指级语义文本描述，并构建了包含4万对手部运动视频和字幕的FingerCap-40K数据集。为了解决现有视频多模态大模型在时间稀疏性上的瓶颈，提出了FiGOP方法，将RGB关键帧与后续手部关键点配对，通过轻量级时间编码器恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要。现有方法在捕捉手指级语义方面存在困难，特别是时间稀疏性限制了模型对细微、高频手指动态的捕捉能力。

Method: 提出了FiGOP（手指图像组）方法，将每个RGB关键帧与后续手部关键点配对，直到下一个关键帧。使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。这种方法在不增加RGB密度的情况下恢复了精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，现有的开源和闭源视频多模态大模型在手指级推理方面仍存在困难，而采用FiGOP增强的模型在HandJudge评估和人工研究中均取得了持续的性能提升。

Conclusion: FiGOP方法有效解决了视频多模态大模型在手指级手部运动理解中的时间稀疏性问题，为细粒度手部动作描述提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖成本高昂的时间边界标注，本文旨在研究仅需每个实例单个时间戳标注的点监督面部表情检测，以降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块用于软伪标签生成；2）类别感知的顶点分类分支用于区分宏观和微观表情。同时引入强度感知对比损失来增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 该方法成功实现了仅需点监督的面部表情检测，通过软伪标签和双分支设计有效解决了表情强度建模和分类问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型缺乏统一的表示来桥接自然图像外观和PBR属性，导致任务特定流程碎片化，无法利用大规模RGB图像数据。

Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在：RGB外观和四个PBR贴图，将其制定为5帧序列并使用视频扩散架构。

Result: 在MatHybrid-410K混合语料库上训练，MatPedia实现了原生1024×1024合成，在质量和多样性上显著超越现有方法。

Conclusion: MatPedia提供了一个统一框架，能够处理文本到材料生成、图像到材料生成和本征分解等多种材料任务，为基于物理的渲染材料创建提供了有效解决方案。
...
TL;DR: 提出了一种边缘设备上基于烹饪状态引导的食品图像生成方法，通过厨师标注的熟度级别和烹饪配方条件，从生食图像合成逼真的熟食图像。


<details>
  <summary>Details</summary>
Motivation: 解决在边缘设备上从生食图像合成逼真熟食图像的挑战，现有方法要么生成结果不真实，要么计算资源需求过高不适合边缘部署。

Method: 构建首个基于烤箱的烹饪进展数据集，提出边缘高效的配方和烹饪状态引导生成器，引入领域特定的烹饪图像相似性(CIS)指标作为训练损失和进度监控信号。

Result: 模型在FID分数上显著优于现有基线方法，在自有数据集上提升30%，在公共数据集上提升60%。

Conclusion: 该方法能够根据用户偏好的视觉目标生成逼真的烹饪食品图像，确保时间一致性和烹饪合理性，适合边缘设备部署。
...
TL;DR: 本文提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，在多个基准测试中比现有方法提升了3%准确率和5% H-score。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理开放集域泛化时面临已知类结构风险和未知类开放空间风险的困境，特别是在区分与已知类具有细粒度视觉相似性的'硬未知'时容易过度自信。

Method: 提出语义增强的CLIP框架，包括语义感知提示增强模块分解图像为判别性语义标记，双工对比学习保持与已知类的分离性和语义邻近性，以及语义引导扩散模块生成伪未知样本。

Result: 在五个基准测试上的广泛实验表明，相比最先进方法，准确率持续提升3%，H-score提升5%。

Conclusion: SeeCLIP框架通过细粒度语义增强有效解决了开放集域泛化中的关键挑战，在区分硬未知样本方面表现出色。
...
TL;DR: 本文提出了一种受自然选择启发的3D高斯泼溅（3DGS）剪枝框架，通过将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定保留或剪除哪些高斯元素，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯基元来拟合场景，导致存储和计算开销巨大。现有剪枝方法依赖人工设计标准或引入额外可学习参数，效果不理想。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去标识化，受化学替代机制启发，将身份编辑重新解释为潜在空间中的语义位移。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去标识化方法在身份抑制和属性保留之间难以达到良好平衡，需要开发一种能够有效保护隐私同时保持图像自然性的方法。

Method: 通过优化引导发现身份编辑方向，使用新颖的试剂损失函数监督属性保留和身份抑制，提出线性和测地线（基于切线的）编辑方案来有效导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验结果表明，FLUID在身份抑制和属性保留之间实现了优越的平衡，在定性和定量指标上都优于最先进的去标识化方法。

Conclusion: FLUID框架提供了一种有效的训练免费人脸去标识化解决方案，通过潜在空间中的身份替换实现了隐私保护和图像质量的良好权衡。
...
TL;DR: DepthFocus是一个可控制的视觉Transformer模型，将立体深度估计重新定义为意图驱动的控制，能够根据深度偏好动态调整计算，在复杂场景中实现选择性感知。
...
Method: 引入DepthFocus，一个可控制的视觉Transformer，通过标量深度偏好作为条件，动态调整计算以聚焦于目标深度。训练主要使用新构建的50万多层合成数据集，捕捉多样化的透视效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示意图对齐估计，并在未见过的透视场景中表现出强大的泛化能力。

Conclusion: DepthFocus作为向主动和类人3D感知迈出的重要一步，展现了其鲁棒性，能够实现类似人类的深度感知能力。
...
TL;DR: MVLR是一个统一的内存增强视觉语言恢复模型，用于在恶劣天气条件下恢复图像，通过结合视觉语言模型和隐式内存库来提升恢复精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气条件下（如雨、雾、雪等）实现可靠的视觉感知对于自动驾驶和户外机器人至关重要但具有挑战性。

Method: MVLR将轻量级编码器-解码器主干与视觉语言模型和隐式内存库相结合，通过链式推理编码天气退化先验，并检索细粒度退化原型，通过动态交叉注意力机制与多尺度视觉特征自适应融合。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在多种文化线索共存场景下文化对抗鲁棒性的新测试套件，发现即使最先进的模型在面对简单的图像堆叠扰动时准确率也会大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常只关注单一文化概念，忽视了多种可能不相关的文化线索共存的情况，而视觉语言模型在不同文化输入下的稳定性对于支持多元文化社会至关重要。

Method: 开发了ConfusedTourist文化对抗鲁棒性测试套件，通过图像堆叠和基于图像生成的变体来扰动地理线索，评估模型稳定性。

Result: 实验显示模型在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差；可解释性分析表明失败源于系统性的注意力转移，模型被分散线索干扰而偏离预期焦点。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态和球位置标注，支持复杂人-物交互研究。
...
Result: 评估发现简单拼接球拍姿态特征会降低性能，而交叉注意力机制能有效利用这些特征，使轨迹预测结果优于单模态基线。
...
TL;DR: 本文提出了一个统一的行人跟踪框架OmniPT，能够交互式地跟踪、基于参考跟踪并生成跟踪对象的语义理解，在跟踪基准测试中优于先前方法。


<details>
  <summary>Details</summary>
Motivation: LVLMs在图像级任务表现出色，但在实例级任务如行人跟踪中仍存在性能差距。同时，结合目标跟踪和自然语言的新任务需要模型在高级语义层面理解跟踪对象，这正是LVLMs的优势所在。

Method: 提出OmniPT框架，采用RL-Mid Training-SFT-RL训练阶段：基于LVLM预训练权重，先进行RL阶段使模型输出固定格式的边界框；然后使用大量行人相关数据集进行中间训练；最后在多个行人跟踪数据集上进行监督微调，再进行RL阶段提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法能够比先前方法表现更好。

Conclusion: OmniPT框架成功将跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略实现了优于先前方法的行人跟踪性能。
...
TL;DR: ReBrain是一个基于检索增强扩散的框架，用于从稀疏CT扫描重建完整脑部MRI，通过Brownian Bridge扩散模型和检索相似CT切片来指导生成，在稀疏条件下实现最先进的跨模态重建性能。
...
Method: 提出ReBrain框架：使用Brownian Bridge扩散模型合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，通过ControlNet分支指导中间MRI切片生成，并对罕见检索失败情况应用球面线性插值。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中实现了最先进的性能。

Conclusion: ReBrain通过检索增强扩散框架有效解决了稀疏CT到MRI的重建挑战，为无法进行MRI检查的患者提供了可行的替代方案。
...
TL;DR: STAR模型通过生成图像补丁位置的均匀生成树遍历顺序，在保持采样性能的同时提供灵活的序列顺序，支持图像编辑任务。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择灵活性受限。需要一种既能保持性能又能提供灵活编辑能力的方法。

Method: 利用图像补丁位置定义的网格中采样的均匀生成树遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR在保持采样性能的同时，提供了足够的序列顺序灵活性来支持图像编辑，且无需对广泛采用的语言自回归模型架构进行重大更改。

Conclusion: STAR通过定制化但结构化的随机策略，在保持后补完成能力的同时维持了采样性能，为图像编辑任务提供了有效的解决方案。
...
TL;DR: REArtGS++是一种改进的铰接物体重建方法，通过解耦螺旋运动建模、零件感知高斯优化和时间连续几何约束，在通用零件级表面重建和关节参数估计方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多零件物体时存在困难，且缺乏对未见状态的几何约束，需要更通用的铰接物体重建方法。

Method: 提出解耦螺旋运动建模（无需关节类型先验），通过零件运动混合联合优化零件感知高斯和关节参数，引入时间连续几何约束（鼓励高斯平面化）和基于泰勒一阶展开的时间一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在通用零件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时间几何约束和平面高斯splatting实现了更通用的铰接物体重建，在零件级表面重建和关节参数估计方面表现出优越性能。
...
### [39] [Planning with Sketch-Guided Verification for Physics-Aware Video Generation](https://arxiv.org/abs/2511.17450)
*Yidong Huang,Zun Wang,Han Lin,Dong-Ki Kim,Shayegan Omidshafiei,Jaehong Yoon,Yue Zhang,Mohit Bansal*

Main category: cs.CV

TL;DR: SketchVerify是一个无需训练的基于草图验证的运动规划框架，通过测试时采样和验证循环来生成动态一致的运动轨迹，提高视频生成的质量和物理真实性。
...
Method: 提出草图验证框架：预测多个候选运动轨迹，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代优化直到找到满意轨迹。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。缩放轨迹候选数量能持续提升性能。

Conclusion: SketchVerify通过高效的草图验证机制，在保持计算效率的同时显著改善了视频生成中的运动规划质量，为复杂动态场景生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型虽然推理效率高、图像质量好，但存在多样性崩溃问题，类似于少步蒸馏扩散模型中观察到的输出变异性减少现象。

Method: 分析发现特征图中的关键组件是早期尺度多样性形成的关键因素。通过抑制模型输入中的关键组件并放大模型输出中的关键组件，有效释放VAR模型的生成潜力。

Result: 实验结果表明该方法显著增强了生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR成功解锁了VAR模型的内在生成潜力，同时保持了高保真合成质量，为解决VAR模型多样性问题提供了简单有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有铰接物体重建方法通常需要多阶段、多视角等昂贵输入，限制了实际应用。本文旨在解决这一局限性，开发更高效的重建方法。
...
Conclusion: 提出的类别无关铰接物体重建框架仅需稀疏视角RGB图像，就能实现高质量的重建效果，代码将公开。
...
TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三条路径的相互作用，并根据问答对齐格式设计了相应的干预方法。
...
Conclusion: LVLMs的幻觉问题源于多条因果路径的复杂相互作用，通过针对不同问答格式设计专门的干预策略，可以有效减少幻觉现象。
...
TL;DR: ReCoVAD是一个基于人类神经系统双通路机制的创新视频异常检测框架，通过选择性帧处理减少冗余计算，在保持最先进性能的同时仅处理28.55%和16.04%的帧。


<details>
  <summary>Details</summary>
Motivation: 现有基于大预训练模型的视频异常检测方法依赖密集帧级推理，计算成本高且延迟大。研究旨在探索在强大预训练模型下，密集推理是否真正必要。

Method: 提出双通路框架：反射通路使用轻量CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，并集成大语言模型定期审查描述以识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上达到最先进的免训练性能，同时仅处理先前方法28.55%和16.04%的帧数。

Conclusion: 稀疏推理对于基于大模型的有效视频异常检测是足够的，ReCoVAD证明了在保持高性能的同时显著降低计算成本的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统训练中需要大量多样化点云数据的挑战，特别是难以获取的边缘案例场景，当前基于手工3D虚拟环境的方法成本高且难以捕捉真实世界复杂性。

Method: 将LiDAR点云转换为2D距离图像作为中间表示，使用基于凸包的语义掩码进行条件引导，实现基于扩散模型的点云生成和编辑。
...
<details>
  <summary>Details</summary>
Motivation: 视觉情感识别中，预训练视觉模型的事实级特征与情感类别之间缺乏直接关联，存在'情感鸿沟'，而文本模态具有明确的情感表达和高信息密度，可以弥补这一缺陷。

Method: 提出分区自适应对比学习(PACL)，关注社交媒体数据中图像和文本的事实与情感连接，分离不同类型样本并为每种类型设计不同的对比学习策略，通过动态构建正负样本对来充分利用噪声样本的潜力。

Result: 实验表明，弥合'情感鸿沟'显著提高了各种预训练视觉模型在下游情感相关任务中的性能。

Conclusion: 通过从预训练文本模型借用知识来增强视觉模型的情感感知是有效的，PACL方法能够充分利用噪声社交媒体数据中的图像-文本连接，提升视觉情感识别性能。
...
TL;DR: 本文研究发现多语言文本到图像模型在跨语言提示下存在文化一致性问题，提出了定位文化敏感神经元的方法和两种对齐策略来改善文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言文本到图像模型在跨语言提示下往往产生文化中性或英语偏见的结果，缺乏文化一致性，这源于文化相关表征激活不足而非知识缺失。

Method: 提出探测方法定位文化敏感信号到少量固定层的神经元，并引入两种对齐策略：推理时文化激活（放大已识别神经元）和层定向文化增强（仅更新文化相关层）。

Result: 在CultureBench上的实验表明，相比强基线方法，该方法在文化一致性方面取得一致改进，同时保持了保真度和多样性。

Conclusion: 通过定位文化敏感神经元并采用针对性的对齐策略，可以有效提升多语言文本到图像模型的文化一致性表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型在生成长推理链时存在冗余自反问题，而基于静态视觉参考的CoT压缩方法对多模态推理提升有限。

Method: ChainV首先基于前一步推理进行粗粒度视觉补丁选择，然后根据平均注意力强度识别最具代表性的原子视觉提示，并通过一致性评估机制调整自反水平。
...
TL;DR: PEGS是一个融合物理先验和事件流增强的3D高斯溅射框架，用于在大时空尺度上进行去模糊目标聚焦建模和运动恢复。
...
Method: 提出三重监督方案：通过加速度约束强制物理合理性、利用事件流进行高时间分辨率指导、使用卡尔曼正则化器融合多源观测；设计基于实时运动状态的自适应模拟退火训练策略。

Result: 实验表明PEGS在重建大时空尺度运动方面优于主流动态方法。

Conclusion: PEGS框架成功整合物理先验和事件流增强，在大时空尺度刚体运动重建方面表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间。本文旨在继续CroCo的工作，专注于学习更适合3D视觉的特征。

Method: 将掩码自编码器扩展到任意多视图，通过统一掩码所有视图并使用带有帧间注意力的轻量级解码器，构建了比CroCo更简单且可扩展的方法。

Result: 在包括前向重建、密集图像匹配和相对姿态估计在内的下游任务中，MuM模型超越了最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过将MAE扩展到多视图，MuM成功学习了专门针对3D视觉的特征，在多个3D视觉任务中表现出色，证明了该方法在3D特征学习方面的有效性。
...
TL;DR: 本文提出从1D相关色温(CCT)映射转向2D色度空间，并使用轻量级多层感知器(MLP)替代传统CCT插值，以改善非普朗克光源(如LED)下的色度映射精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的色度映射方法针对普朗克光源优化，但现代LED光源可能显著偏离普朗克轨迹，暴露了依赖一维CCT进行光照表征的局限性。
...
Conclusion: 从1D CCT转向2D色度空间结合MLP方法，显著提高了非普朗克光源下的色度映射精度，同时保持计算效率和兼容性。
...
TL;DR: 该论文提出了一种利用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像、926个独特身份的DIF-V数据集，以解决现有面部验证系统中的偏见问题。
...
Method: 整合先进生成模型创建多样化的高质量合成人脸图像，强调代表各种面部特征，确保符合身份证照片特征。
...
TL;DR: 提出一个完整的多阶段优化框架，将高性能浮点图像压缩模型转化为高效的硬件友好型整数实现，通过动态范围感知量化、渐进混合精度搜索和通道剪枝等方法，在FPGA上实现高性能图像压缩。
...
Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化处理数据异常值和大动态范围；2. 渐进混合精度搜索：为每层分配最优非均匀位宽；3. 通道剪枝：针对GDN层消除不活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%，硬件感知优化进一步降低20%以上计算复杂度，RD性能影响可忽略，最终模型在效率和性能上均优于现有FPGA实现。

Conclusion: 该框架成功解决了深度学习图像压缩模型在FPGA上的部署挑战，实现了高性能与硬件效率的平衡，为资源受限环境下的图像压缩应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散基真实世界图像超分辨率中保真度与可控性的平衡问题：多步方法存在生成多样性和随机性导致保真度低，而一步方法因保真度特定微调而失去控制灵活性。
...
Conclusion: ODTSR成功平衡了真实世界图像超分辨率中的保真度和可控性，为扩散基方法提供了新的设计思路。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，在医学图像分割中针对小病灶分割不足问题，相比传统Dice损失和blob损失，能提高病灶检测率但略微增加假阳性。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）对小病灶分割不足，因为小病灶相对体积小，对整体损失贡献可忽略。需要基于每个病灶评估分割质量的实例级损失函数。

Method: 在nnU-Net框架中引入基于CC-Metrics的CC-DiceCE损失函数，并与现有的blob损失进行对比，使用DiceCE作为基准线进行标准化评估。

Result: CC-DiceCE损失提高了病灶检测率（召回率），分割性能仅有最小程度下降，但略微增加了假阳性。在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效解决小病灶分割不足问题，在保持分割性能的同时提高检测率，是优于blob损失的实例级分割方法。
...
Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，显著提升了扩散基规划的轨迹预测性能，各组件的有效性通过消融研究得到验证。
...
TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD覆盖150多个模型，REMSA是基于LLM的智能体，能通过自然语言查询自动选择合适模型。


<details>
  <summary>Details</summary>
Motivation: 遥感基础模型选择困难，因为文档分散、格式异构、部署约束多样。需要系统化的资源来支持模型选择。

Method: 构建RSFM数据库覆盖150多个模型，开发基于LLM的REMSA智能体，使用情境学习对候选模型进行排序，并提供透明解释。
...
TL;DR: UI-Styler是一个针对超声图像的类感知风格迁移框架，通过模式匹配机制将目标图像的纹理模式转移到源图像上，同时保留源图像的结构内容，并引入基于目标域伪标签的类感知提示策略，实现准确的语义对齐。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像翻译方法在域适应过程中往往忽视类特定的语义对齐，导致内容-类别映射错位，损害诊断准确性。

Method: 提出UI-Styler框架，包含：1）模式匹配机制，将目标图像的纹理模式转移到源图像；2）类感知提示策略，基于目标域伪标签指导语义对齐；3）保留源图像结构内容的同时实现风格迁移。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上持续优于现有无配对图像翻译方法，达到最先进的性能。

Conclusion: UI-Styler通过类感知的风格迁移和语义对齐，有效解决了超声图像跨设备域偏移问题，显著提升了黑盒下游推理模型的性能。
...
Method: 提出了FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格。FireScope框架通过强化学习和视觉监督学习预测风险栅格，并生成互补的推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升。专家反馈和自动分析确认其推理轨迹具有忠实性和语义意义。

Conclusion: 基于语言的推理可以提升视觉生成模型的泛化能力，FireScope-Bench有望成为推动推理驱动、可解释和可泛化空间建模的基础。
...
TL;DR: 本文系统评估了自监督表示在音频-视觉深度伪造检测中的应用潜力，发现这些特征能捕捉到与深度伪造相关的互补信息，但无法在跨数据集时实现可靠泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表示在音频-视觉深度伪造检测中的潜力，此前工作要么孤立使用这些特征，要么将其嵌入复杂架构中，缺乏系统性评估。

Method: 在音频、视频和多模态三个维度上，以及唇部运动和通用视觉内容两个领域，系统评估自监督特征在检测效果、信息可解释性和跨模态互补性三个关键维度的表现。

Result: 发现大多数自监督特征能捕捉深度伪造相关信息且具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但无法在跨数据集时可靠泛化。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频重捕获方法存在相机运动注入策略不理想的问题，限制了相机控制精度，导致生成视频无法保留源视频的精细视觉细节。

Method: 引入查询共享交叉注意力模块，融合6-DoF相机位姿和2D渲染视频帧控制信号；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息优化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性方面优于最先进方法超过20%，同时实现最高的视频生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在时空一致性操作方面存在挑战，难以实现动作执行的时间连贯控制。

Method: 提出4D感知的视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展传统空间动作表示包含时间信息），通过跨注意力机制融合多模态表示。

Result: 在机器人操作的不同任务中验证了方法的优越性，实现了空间平滑和时间连贯的操作。

Conclusion: VLA-4D通过统一的4D感知框架，有效解决了机器人操作的时空一致性问题，为通用机器人任务提供了更好的解决方案。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级对齐层和持续学习策略，在保持SAM模型强大零样本能力的同时，提高医学图像分割的计算效率和性能，并在持续学习场景下取得最优效果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间隐私政策差异使得联合训练不可行，需要持续学习方法来避免灾难性遗忘。SAM模型虽然具有强大零样本能力，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层（Alignment Layer）作为轻量级即插即用模块，对齐编码器-解码器特征分布；基于此构建CA-SAM持续学习策略，自动适配对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验。
...
Method: 首先通过轻量级跟踪和重建模块构建局部一致子图，然后逐步对齐融合到全局高斯地图中，强制执行跨视图几何一致性，全局地图反馈修正局部漂移。

Result: 在真实世界数据集上实现最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，生成更精细几何细节，同时保持紧凑内存高效全局表示。

Conclusion: SING3R-SLAM通过全局高斯表示有效解决了SLAM中的漂移和冗余问题，在保持紧凑性的同时显著提升了3D重建质量和下游任务性能。
...
TL;DR: SPECTRE是一个基于Transformer的3D CT基础模型，通过自监督和跨模态预训练学习通用CT表示，在多个基准测试中表现优异。
...
Method: 联合优化局部Transformer进行高分辨率体积特征提取和全局Transformer进行全扫描上下文建模，结合DINO风格自蒸馏和基于SigLIP的视觉-语言对齐。
...
TL;DR: 提出了一种针对鱼眼图像的失真感知BEV语义分割框架，直接处理多摄像头高分辨率鱼眼图像，通过几何反投影和像素级深度分布估计，在BEV空间中实现连续、不确定性感知的语义分割。


<details>
  <summary>Details</summary>
Motivation: 鱼眼图像存在极端非线性失真、遮挡和深度模糊等问题，使得准确的BEV语义分割具有挑战性。现有方法通常需要去失真或透视校正，这会导致信息丢失和计算开销。

Method: 使用校准的几何反投影和像素级深度分布估计，将每个图像像素通过高斯参数化提升到3D空间，预测空间均值和各向异性协方差来显式建模几何不确定性。通过可微分splatting将投影的3D高斯融合到BEV表示中。

Result: 在复杂的停车和城市驾驶场景中表现出强大的分割性能，在严重鱼眼失真和多样化环境条件下，可驾驶区域的IoU达到87.75%，车辆的IoU达到57.26%。

Conclusion: 该框架能够在不需要去失真或透视校正的情况下，直接从鱼眼图像生成连续、不确定性感知的BEV语义地图，有效处理鱼眼图像特有的几何挑战。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过独立的4D时空查询直接从相邻帧学习连续3D语义占据，在自监督Occ3D-nuScenes基准上比之前基于相机的方法提升了26%的语义RayIoU。


<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本过高，需要探索直接从传感器数据中进行自监督学习的方法，而现有方法要么依赖2D渲染一致性，要么使用离散体素网格，限制了空间精度和可扩展性。

Method: 使用基于查询的自监督框架，通过独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，并引入压缩场景表示以在恒定内存下实现长距离监督和推理。

Result: 在自监督Occ3D-nuScenes基准上，QueryOcc比之前基于相机的方法在语义RayIoU上提升了26%，同时运行速度达到11.6 FPS。
...
TL;DR: 提出了一种基于四元数保真度项的颜色图像盲去卷积方法，通过四元数卷积核建模颜色通道间的相互关系，相比现有方法能更有效地去除伪影并提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有颜色图像盲去卷积方法通常将彩色图像转为灰度图或单独处理每个颜色通道，忽略了颜色通道之间的相互关系，导致去模糊效果不理想。

Method: 设计了一种新颖的四元数保真度项，利用四元数卷积核的特性（包含四个核：一个类似非负卷积核捕捉整体模糊，三个无约束核分别对应红绿蓝通道建模其未知相互依赖关系），并在盲去卷积过程中使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法展示了作为颜色图像去卷积强大工具的潜力，通过四元数建模颜色通道关系实现了更好的去模糊性能。
...
TL;DR: 本文提出了四种基于相关性反馈的视觉语言模型检索增强方法，包括伪相关性反馈、生成式相关性反馈、注意力反馈摘要器和显式反馈，在Flickr30k和COCO数据集上验证了这些方法能提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，提出在推理时使用相关性反馈机制来改善检索性能，这种方法既可作为微调的替代方案，也可与微调后的模型结合使用。

Method: 提出了四种反馈策略：1）伪相关性反馈（PRF）-基于top结果优化查询嵌入；2）生成式相关性反馈（GRF）-使用合成描述进行查询优化；3）注意力反馈摘要器（AFS）-基于transformer的多模态细粒度特征整合模型；4）使用真实描述作为显式反馈的上界基线。

Result: 实验表明，GRF、AFS和显式反馈相比无反馈检索，在较小VLM上MRR@5提升3-5%，在较大VLM上提升1-3%。AFS与显式反馈类似，能缓解查询漂移问题，在迭代多轮检索中比GRF更鲁棒。

Conclusion: 相关性反馈能持续增强各种VLM的检索性能，为交互式和自适应视觉搜索开辟了新机会。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习优化，特别解决了立体化学信息识别的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有OCSR系统在准确识别立体化学信息方面面临挑战，因为立体异构体的视觉线索（如楔形键、虚线键）非常细微。需要开发能够准确识别立体化学结构的系统。

Method: 采用三阶段训练范式：1）在大规模噪声数据集上进行预训练；2）使用更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务；3）使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 实验结果表明，MolSight在（立体）化学光学结构识别方面达到了最先进的性能，即使参数规模相对紧凑，GRPO算法也能进一步提升模型在立体分子识别上的表现。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，包括之前无法实现的滚转角，并在其他姿态参数上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于俯仰角和偏航角的估计，且在大角度输入时精度下降。需要一种能够同时准确预测全面指尖姿态信息的方法。

Method: 提出双模态输入方法，包括从触摸屏获取的电容图像和屏下指纹传感器获得的指纹补丁，通过结合这两种模态信息来实现可靠的指尖姿态估计。

Result: BiFingerPose相比之前的SOTA方法在预测性能上提升超过21%，任务完成效率提高2.5倍，用户操作准确率提升23%。12人用户研究验证了该方法的优势。

Conclusion: 该方法能够可靠估计滚转角，显著改善其他姿态参数的预测性能，在增强认证安全性和改善交互体验方面具有应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在解释和推断三维空间布局方面存在空间推理能力有限的问题，这源于现有视觉编码器（如CLIP）的损失性嵌入，仅限于实例级语义特征。

Method: 提出分层适配器，将自监督学习的几何特征与CLIP的语义特征进行分层融合，生成空间感知的视觉嵌入；采用预训练的LLaVA模型进行高效训练，并通过随机特征丢弃优化以避免仅依赖CLIP编码器的平凡解。

Result: 实验结果表明，SpatialGeo在空间推理任务中显著提升准确率，在SpatialRGPT-Bench上将最先进模型的性能提升了至少8.0%，同时推理期间内存成本减少了约50%。

Conclusion: SpatialGeo通过增强空间感知能力有效提升了多模态大语言模型的空间推理性能，在保持高效性的同时显著改善了模型对三维空间布局的理解能力。
...
TL;DR: NoPe-NeRF++是一种无需姿态先验的局部到全局优化算法，用于训练神经辐射场（NeRF）。该方法通过相对姿态初始化、局部联合优化和全局捆绑调整，显著提高了姿态估计精度和NeRF质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（特别是NoPe-NeRF）仅关注图像内的局部关系，在复杂场景中难以恢复准确的相机姿态。需要一种能够结合局部和全局线索的方法来克服这一挑战。

Method: 1. 通过显式特征匹配进行相对姿态初始化；2. 局部联合优化以增强姿态估计；3. 引入包含几何一致性约束的全局优化阶段，通过捆绑调整和特征轨迹进一步优化姿态。

Result: 该方法在基准数据集上的广泛评估显示，在姿态估计精度和新视角合成方面均优于现有最先进方法，即使在挑战性场景中也表现出卓越性能和鲁棒性。
...
TL;DR: 本文提出了一种改进生成图像模型中透明物体折射效果的方法，通过同步像素并使用斯涅尔折射定律来生成更符合光学物理约束的图像。
...
Method: 在生成过程的每个步骤中，使用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并，并通过与第二个全景图像同步来恢复不可直接观察但通过折射或反射可见的表面外观。
...
TL;DR: 提出了一个统一框架，用于生成多媒体绘画过程，通过语义驱动的风格控制机制，将多种媒介嵌入扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和过程转移。


<details>
  <summary>Details</summary>
Motivation: 现有的绘画教程视频缺乏交互性和个性化，而现有生成模型在跨媒介泛化和保持时间/结构一致性方面存在困难，无法忠实再现人类创作流程。
...
Conclusion: 提出的感知距离剖面（PDP）曲线能够定量建模创作序列（构图、色彩块面、细节精化），反映了人类艺术创作进程，为绘画过程生成提供了有效解决方案。
...
TL;DR: DSeq-JEPA提出了一种判别性序列联合嵌入预测架构，通过识别主要判别区域并按重要性顺序预测后续区域，结合了JEPA的潜在预测和GPT风格的序列推理。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，现有I-JEPA方法对所有区域进行统一独立处理，缺乏明确的预测顺序概念。人类视觉会从最具信息量的区域开始选择性、顺序性地部署注意力。
...
Conclusion: DSeq-JEPA成功桥接了预测性和自回归自监督学习，通过集成JEPA风格的潜在预测和GPT风格的序列推理，实现了更有效的视觉表示学习。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占据估计方法，相比高斯表示方法，在Occ3D数据集上实现了75%的内存占用减少、124%的推理速度提升和5.9%的mIoU改进，首次实现了实时推理。
...
Method: 采用超二次曲面场景表示，利用多层二十面体细分的高斯近似超二次曲面，在训练期间实现高斯光栅化监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，内存占用减少75%，推理速度提升124%，mIoU提高5.9%，无需使用时间标签。基元数量比高斯方法减少84%。

Conclusion: 这是第一个实现实时推理的占据模型，同时保持竞争力性能。通过超二次曲面的使用显著减少了场景建模所需的基元数量。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间计算增强诱导的漂移向量来推断语义恢复方向，显著提升了CLIP对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像文本匹配中表现出色，但对图像对抗扰动高度脆弱。由于对抗微调成本过高，现有测试时防御方法鲁棒性有限。

Method: ATAC直接在CLIP嵌入空间操作，计算增强诱导的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法高出近50%，同时计算开销极小。在非常规和极端设置下仍保持最先进的鲁棒性，甚至对自适应攻击也实现了非平凡鲁棒性。
...
TL;DR: SVRecon将稀疏体素光栅化范式扩展到高保真表面重建任务，通过集成SDF来解决稀疏体素优化中的局部最小值问题，实现了快速收敛和精确重建。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素与其邻居在空间上解耦且边界清晰，这使它们在优化过程中容易陷入局部最小值。虽然SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素之间保持这种平滑性具有挑战性。

Method: 通过(1)使用视觉几何模型进行鲁棒的几何初始化，(2)强制执行父-子和兄弟体素组之间相干关系的空间平滑损失，来促进连贯平滑的体素结构。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛性。

Conclusion: SVRecon方法在稀疏体素框架下成功集成了SDF，解决了局部最小值问题，实现了高保真表面重建和快速收敛。
...
TL;DR: 提出了非参数概率鲁棒性(NPPR)作为更实用的概率鲁棒性度量，无需预定义扰动分布，直接从数据中学习优化的扰动分布，在分布不确定性下进行保守的鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对小的输入扰动很脆弱，现有概率鲁棒性方法假设固定已知的扰动分布，这在实践中不现实。

Method: 基于高斯混合模型(GMM)和多层感知机(MLP)头以及双三次上采样的NPPR估计器，覆盖输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验验证，与现有方法相比，NPPR提供高达40%更保守(更低)的概率鲁棒性估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，解决了现有概率鲁棒性方法对预定义扰动分布的依赖问题。
...
TL;DR: MorphSeek是一种细粒度表示级策略优化范式，将可变形图像配准重新定义为潜在特征空间中的空间连续优化过程，通过随机高斯策略头和多轨迹采样实现高效探索和粗到精优化。


<details>
  <summary>Details</summary>
Motivation: 解决可变形图像配准中高维变形空间的挑战和体素级监督稀缺的问题，现有强化学习方法通常将空间投影为粗糙的低维表示，限制了捕捉空间变异变形的能力。

Method: 提出MorphSeek框架，在编码器上引入随机高斯策略头建模潜在特征分布，结合无监督预热和弱监督微调，通过组相对策略优化实现多轨迹采样稳定训练。
...
TL;DR: 本文提出了一种名为MCMoE的多模态动作质量评估框架，通过专家混合机制处理推理阶段模态缺失问题，在完整和不完整多模态学习场景下均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态数据在推理阶段经常出现部分模态缺失，导致现有多模态模型无法正常工作，且跨模态交互中断会引发灾难性性能下降。

Method: 提出缺失补全框架MCMoE，包含自适应门控模态生成器动态融合可用信息重构缺失模态，设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示。

Result: 在三个公共AQA基准测试中，MCMoE在完整和不完整多模态学习场景下均实现了最先进的性能。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet上实现+4.32%的鲁棒准确率和+3.5%的零样本准确率提升，训练效率提高2.3倍。


<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗蒸馏方法存在知识多样性有限、收敛速度慢、鲁棒性与准确性难以平衡等问题，需要更有效的多教师蒸馏框架来提升视觉语言模型的对抗鲁棒性。

Method: 采用双教师知识融合架构，包含动态权重分配策略和自适应sigmoid加权函数，协同优化干净特征保留和鲁棒特征增强，平衡多模态间的知识传递强度。
...
TL;DR: CWMDT框架将标准视频扩散模型转化为反事实世界模型，通过构建场景的数字孪生、使用大语言模型进行推理，并生成反事实视觉序列。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行预测，无法处理反事实查询（如"如果移除这个物体会发生什么？"），而这对评估AI行为在不同条件下的表现越来越重要。

Method: 1. 构建观察场景的数字孪生，以结构化文本编码物体及其关系；2. 使用大语言模型推理反事实干预如何随时间传播改变观察场景；3. 用修改后的表示条件化视频扩散模型生成反事实视觉序列。
...
TL;DR: Radar2Shape是一种去噪扩散模型，通过将雷达信号频率与多分辨率形状特征相关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后以从粗到细的方式基于雷达信号频率扩散到该潜在空间。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体形状在商业和航空航天应用中至关重要，但现有深度学习方法难以表示任意形状或处理有限视角收集的真实世界雷达信号。光学3D重建方法虽然能从有限相机视角生成任意形状，但将雷达信号简单视为相机视图时会遇到困难。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并通过脊柱年龄差(SAG)评估脊柱健康状态。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，容易发生与年龄相关的退行性病变，需要一种有效的方法来评估脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变聚类，通过消融研究确定数据规模、损失函数和不同脊柱区域的影响，开发深度学习模型预测脊柱年龄。

Result: 脊柱年龄差(SAG)与椎间盘突出、骨赘、椎管狭窄、骨折等退行性病变以及吸烟、体力劳动等生活方式因素相关。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的规模化发展，实际应用需要更小、更高效的模型系统。本文旨在系统分析在多模态模型中缩小LLM规模对视觉能力的影响。

Method: 采用Extract+Think方法，包括视觉提取调优（训练模型提取指令相关的视觉细节）和逐步推理生成答案。

Result: 研究发现LLM缩小规模对视觉能力的负面影响比对推理能力的影响更严重，视觉提取调优方法有效缓解了这一瓶颈。

Conclusion: Extract+Think方法为多模态模型的高效性和性能设定了新标准，通过分离视觉提取和推理过程显著提升了模型效率。
...
TL;DR: Video-R4是一个视频推理大语言模型，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素并更新推理状态，解决了文本丰富视频中细粒度证据识别的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答模型依赖单次感知固定帧，导致幻觉和细粒度证据识别失败，而人类会暂停、放大和重读关键区域。

Method: 构建两个数据集进行监督学习和强化学习，采用多阶段反刍学习框架，通过SFT和GRPO-based RL逐步微调7B LMM学习原子和混合视觉操作。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法直接将事件映射到强度图像存在感知质量差、模型容量和训练数据难以扩展的问题，需要更好的事件到图像重建方法。

Method: 提出EvDiff事件扩散模型，采用单步前向扩散减少计算成本，配备时间一致性EvEncoder，并使用替代训练框架消除对配对事件-图像数据集的依赖。

Result: 在真实世界数据集上的实验表明，该方法在像素级和感知指标上均优于现有方法，能够生成高质量的彩色视频。

Conclusion: EvDiff通过扩散模型和替代训练框架，成功实现了从单色事件流生成高质量彩色视频，在保真度和真实感方面表现出色。
...
TL;DR: 提出了一种原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法速度慢和几何不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有指令引导的3D编辑方法存在关键限制：基于优化的方法速度过慢，而基于多视图2D编辑的前向方法常出现几何不一致和视觉质量下降的问题。
...
Conclusion: 提出的原生3D编辑框架在生成质量、3D一致性和指令保真度方面设定了新基准，为3D内容创作提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM本地推理面临配置选择过多的问题，现有基准测试工具设计狭窄，无法整合系统指标和任务特定指标，缺乏统一的用户友好基准测试框架。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动测试选定LLM、推理引擎和量化级别，支持单流、批处理和服务器三种使用场景，跟踪计算性能、资源使用、部署等系统指标以及任务特定指标。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试结果显示，任务性能与系统级效率之间存在多种权衡，不同推理引擎和模型表现差异显著。
...
<details>
  <summary>Details</summary>
Motivation: 工业级大型语言模型往往忽视低资源语言如突尼斯阿拉伯语的理解能力，这可能导致数百万突尼斯人无法用自己的语言与AI充分互动，威胁方言保护并影响年轻一代的语言偏好。

Method: 创建包含突尼斯方言、标准阿拉伯语和英语翻译的平行数据集，并带有情感标签；在转写、翻译和情感分析三个任务上对多个流行LLM进行基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类具有各向同性分布，将边界限制为球形，忽视了不同特征方向上的分布差异，限制了开放意图检测的能力。

Method: 1) 使用监督对比学习获得判别性特征空间；2) 应用可学习矩阵参数化椭圆边界；3) 通过双损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准测试和问题分类数据集上达到最先进性能，椭圆边界的灵活性展现出优越的开放意图检测能力和泛化潜力。

Conclusion: EliDecide方法通过椭圆决策边界有效解决了开放意图分类问题，在复杂开放世界场景中具有强大的泛化能力。
...
Method: 开发了一个实用、可重复且模型无关的程序，使用Schwartz的基本人类价值观理论和对话数据集进行结构化评估，比较基线提示和明确基于价值观的提示。

Result: 实验表明，即使不改变模型或动态优化提示，价值观引导也是可能的，使用Wizard-Vicuna语言模型变体验证了方法的有效性。

Conclusion: 该方法提供了一种无需修改模型即可实现价值观引导的可行途径，为动态价值观对齐问题提供了解决方案。
...
### [95] [Concept-Based Interpretability for Toxicity Detection](https://arxiv.org/abs/2511.16689)
*Samarth Garg,Deeksha Varshney,Divya Singh*

Main category: cs.CL

TL;DR: 本文提出了一种基于概念梯度的可解释性技术，通过分析毒性检测中的子类型属性（如淫秽、威胁、侮辱等）作为概念，识别导致分类错误的过度归因问题，并引入目标词典集和词-概念对齐评分来量化错误原因。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中毒性内容传播日益严重，虽然文本毒性检测已有显著进展，但基于概念的解释在毒性检测中仍有限。现有方法往往因概念对目标类的不均衡归因导致分类错误。

Method: 1. 使用概念梯度方法提供更因果的解释；2. 构建目标词典集捕获导致误分类的有毒词汇；3. 计算词-概念对齐评分量化词汇对错误的贡献；4. 提出无词典增强策略生成排除预定义有毒词典的毒性样本。

Result: 该方法能够识别毒性检测模型中的过度归因问题，通过词-概念对齐评分量化词汇对分类错误的贡献程度，并在排除显性词汇重叠的情况下检验模型对更广泛毒性语言模式的归因。

Conclusion: 提出的概念梯度方法和无词典增强策略为毒性检测提供了更深入的可解释性分析，有助于理解模型决策机制并减少因过度归因导致的分类错误。
...
### [96] [Falsely Accused: How AI Detectors Misjudge Slightly Polished Arabic Articles](https://arxiv.org/abs/2511.16690)
*Saleh Almohaimeed,Saad Almohaimeed,Mousa Jari,Khaled A. Alobaid,Fahad Alotaibi*

Main category: cs.CL

TL;DR: 该论文研究了AI检测模型对经过轻微AI润色的人类撰写阿拉伯语文章的误判问题，构建了两个数据集评估了14个LLM和商业AI检测器的性能，发现轻微润色会导致检测准确率显著下降。


<details>
  <summary>Details</summary>
Motivation: 解决AI检测模型对经过轻微AI润色的人类撰写文章的误判问题，避免错误指控作者AI抄袭并维护AI检测器的可信度，特别是在阿拉伯语领域尚无相关研究。

Method: 生成了两个阿拉伯语数据集：第一个包含800篇文章（400篇AI生成，400篇人类撰写），用于评估14个LLM和商业AI检测器；第二个数据集Ar-APT包含400篇人类撰写文章，由10个LLM使用4种润色设置处理，共16400个样本，用于评估8个最佳模型对轻微润色文章的检测能力。

Result: 所有AI检测器都会错误地将大量文章归因于AI。最佳LLM Claude-4 Sonnet准确率为83.51%，但对经LLaMA-3轻微润色的文章准确率降至57.63%。最佳商业模型originality.AI准确率为92%，但对经Mistral或Gemma-3轻微润色的文章准确率降至12%。

Conclusion: 轻微AI润色会显著影响AI检测模型的性能，导致对人类撰写文章的误判，需要开发更鲁棒的检测方法来应对这一挑战。
...
### [97] [Reproducibility Report: Test-Time Training on Nearest Neighbors for Large Language Models](https://arxiv.org/abs/2511.16691)
...
<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型推理时的有效性，探索不同规模模型和不同预训练数据对测试时训练效果的影响。
...
### [98] [How Language Directions Align with Token Geometry in Multilingual LLMs](https://arxiv.org/abs/2511.16693)
*JaeSeong Kim,Suan Lee*

Main category: cs.CL

TL;DR: 本文通过系统分析多语言大语言模型内部表示空间中语言信息的结构特征，发现语言信息在第一个Transformer块中急剧分离，并在整个模型深度中保持线性可分性。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型比英语中心模型有4.21倍的结构印记效应。


<details>
  <summary>Details</summary>
Motivation: 虽然多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息的结构特征以及跨层涌现机制缺乏系统分析。本文旨在量化语言编码的层间动态和几何结构。

Method: 在六个多语言LLM上进行了全面的探测研究，覆盖所有268个Transformer层，使用线性和非线性探测器以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块中急剧分离（从第0层到第1层增长76.4±8.2个百分点），并在整个模型深度中保持几乎完全线性可分。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料库塑造的潜在表示结构。研究结果为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
### [99] [Hierarchical Retrieval with Out-Of-Vocabulary Queries: A Case Study on SNOMED CT](https://arxiv.org/abs/2511.16698)
...
<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为生物医学本体，其知识检索面临语言歧义、同义词、多义词等挑战，特别是当查询是词汇外(OOV)时，问题更加严重。

Method: 基于语言模型的本体嵌入方法，构建了针对SNOMED CT概念的OOV查询标注数据集，测试最直接子类和较少相关祖先的检索。

Result: 该方法在性能上超越了包括SBERT和两种词汇匹配方法在内的基线模型。
...
### [100] [Detecting and Steering LLMs' Empathy in Action](https://arxiv.org/abs/2511.16699)
*Juan P. Cadile*

Main category: cs.CL

TL;DR: 研究发现共情行为（牺牲任务效率满足人类需求）在LLM激活空间中表现为线性方向，通过对比提示在三个模型上进行检测和引导实验，发现检测性能优异但引导效果因模型而异。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否编码了共情行为，以及这种特征是否可以被检测和引导，研究安全训练对共情编码和引导能力的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上测试共情行为的检测和引导能力，分析不同层级的激活模式。

Result: 检测方面：所有模型在最优层都达到AUROC 0.996-1.00，未审查模型与安全训练模型表现相当；引导方面：Qwen和Phi-3双向控制成功率达65.3%和61.7%，Dolphin仅支持亲共情引导（94.4%成功率），反共情引导会崩溃。

Conclusion: 共情编码独立于安全训练而存在，但引导能力因模型架构而异。安全训练可能影响引导稳健性而非阻止操纵，需要更多模型验证。
...
TL;DR: 本研究重新探讨了非宾格动词和作格动词的句法分类与语义特征（施事性和终结性）之间的关系，通过基于种子词的可解释维度分析，证实了施事性与作格性、终结性与非宾格性之间的关联。


<details>
  <summary>Details</summary>
Motivation: 先前研究发现人类对施事性和终结性的评分不能很好地预测不及物动词的句法行为，本研究旨在重新检验这一关系，使用更可解释的维度分析方法。

Method: 使用位于施事性和终结性对立极端的种子词计算可解释维度，结合人类判断来评估语义特征。

Result: 研究结果支持了作格性/非宾格性与施事性/终结性之间的联系，表明可解释维度与人类判断结合可以为难以在评分任务中评估的语义属性提供有价值证据。

Conclusion: 使用可解释维度与人类判断相结合的方法能够有效揭示不及物动词句法分类与语义特征之间的关系，为语义属性评估提供了新的研究途径。
...
TL;DR: PEPPER是一种针对文本到图像扩散模型后门攻击的防御方法，通过重写输入提示为语义不同但视觉相似的描述，并添加不显眼元素来破坏触发机制。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者可通过输入提示中的触发器引导生成有害内容，需要有效的防御方法。

Method: PEPPER通过将输入提示重写为语义不同但视觉相似的描述，并添加不显眼元素来破坏触发机制，稀释触发器标记的影响。

Result: 实验显示PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率同时保持生成质量，并能与其他防御方法结合使用。

Conclusion: PEPPER提供了一种有效的后门防御策略，能够破坏触发器机制，增强模型鲁棒性，且具有良好的通用性和可组合性。
...
TL;DR: 本文提出了一种无需参考标准的度量方法，用于评估大语言模型生成响应的简洁性，通过计算原始响应与摘要之间的压缩比以及词移除压缩来量化冗余内容。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成长而冗长的响应，包含冗余或不必要的细节，这会降低清晰度和用户满意度，并增加模型开发者的成本，特别是对于按输出token收费的专有模型。

Method: 提出了一种无参考指标的简洁性评估方法，通过计算三个指标的平均值：1）原始响应与LLM抽象摘要之间的压缩比；2）原始响应与LLM抽取摘要之间的压缩比；3）词移除压缩，即LLM在保持意义的前提下尽可能移除非必要词语。

Result: 实验结果表明，所提出的度量方法能够有效识别LLM输出中的冗余内容，为对话AI系统中响应简洁性的自动评估提供了实用工具，无需人工标注的真实数据。

Conclusion: 该方法为评估LLM响应简洁性提供了一种有效的无参考度量标准，有助于提高响应质量并降低相关成本。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过生成离散token进行推理，这可能限制其表达能力。现有工作显示软概念推理是有前景的方向，但LLM是在离散token上训练的，存在推理与训练之间的差距。

Method: 提出SCM训练方案：构建软概念向量（嵌入的加权平均），将其混合到模型隐藏状态中，并使用强化学习优化整个潜在推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何在最小改动的情况下进一步提高小型循环架构（如TRMs）在复杂推理任务上的效率，特别是在Abstraction and Reasoning Corpus (ARC)任务上的表现。

Method: 提出了一种新颖的训练方案，将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，为训练过程中的每个循环提供目标。

Result: 该方法显著提升了训练效率，减少了18倍的前向传播次数，同时消除了停止机制，在仅0.8M参数的情况下在ARC-1上达到24%的准确率，优于大多数大型语言模型。

Conclusion: 通过将TRMs的推理过程重新框架化并引入新的训练目标，可以在保持质量的同时大幅提升训练效率，证明了小型模型在复杂推理任务上的潜力。
...
TL;DR: 本研究探讨了训练数据的统计特性与归纳头形成之间的关系，发现批次大小和上下文长度的组合能预测归纳头形成点，表面二元组重复频率和可靠性强烈影响归纳头形成，并确定了这两个值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 尽管被称为归纳头的专门注意力头是现代语言模型出色上下文学习能力的基础，但其形成的精确特征仍不清楚。本研究旨在揭示训练数据统计特性与归纳头形成之间的具体关系。

Method: 通过分析自然和合成数据的统计特性，研究批次大小、上下文长度、表面二元组重复频率和可靠性等因素对归纳头形成的影响，并确定这些参数的精确边界。

Result: 发现批次大小和上下文长度的简单方程能预测归纳头形成点；表面二元组重复频率和可靠性强烈影响归纳头形成，并找到这两个值的精确帕累托边界；当频率和可靠性低时，分类性和边际分布形状变得重要。

Conclusion: 局部依赖性与高二元组重复频率和可靠性足以形成归纳头，但当这些值较低时，需要额外的分类性和适当的边际分布形状来支持归纳头的形成。
...
TL;DR: 本文提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中的论点结构（主张-理由关系）并量化论点强度。作者开发了ARQUSUMM框架，利用LLM少样本学习识别句子内的命题及其关系，并通过论点结构感知聚类算法聚合论点和量化支持度。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题日益增多，需要不仅总结多样化论点，还要揭示其理由和论证结构。现有文本摘要方法忽略了对话的论证性质，而现有对话摘要方法未能深入分析句子内的论点结构。

Method: 提出ARQUSUMM框架：1）利用LLM少样本学习基于论证理论识别句子内的命题及其主张-理由关系；2）采用论点结构感知聚类算法聚合论点并量化支持度。

Result: 实验表明ARQUSUMM在对话和定量摘要任务上优于现有模型，生成的摘要能更好地表示论点结构，具有更高的文本质量和量化准确性，对用户更有帮助。

Conclusion: 该研究成功实现了论点感知的定量摘要，通过揭示深层论点结构和量化论证强度，为在线对话分析提供了更有效的工具，在文本质量和量化准确性方面均表现出色。
...
TL;DR: 本研究提出了一种监督微调方法，通过构建细粒度的模式引导指令模板和指令调优数据集，对四个大型语言模型进行参数高效的指令微调，显著提升了在湖南历史名人领域的信息抽取性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人领域系统数据资源有限、通用模型在低资源环境下领域知识抽取和结构化输出生成性能不足的问题，推动历史文化研究的发展。
...
TL;DR: 该研究构建了视觉说服力判断数据集，探索视觉语言模型是否真正理解视觉说服力，发现模型存在回忆偏向，对低/中层次特征识别能力弱，高层次语义对齐是最强预测因子，通过对象基础推理可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型是否真正理解视觉说服力——视觉线索如何塑造人类态度和决策，目前尚不清楚。

Method: 构建二元说服力判断的高共识数据集，引入视觉说服因素分类法（包括低层次感知、中层次构图和高层次语义线索），探索认知引导和知识注入策略。

Result: 视觉语言模型存在回忆偏向（过度预测高说服力），对低/中层次特征判别能力弱，高层次语义对齐是最强预测因子；简单指令或非引导推理效果有限，而简洁的对象基础推理能显著提升精度和F1分数。
...
TL;DR: 本文提出了一个可解释的自动评分框架AnalyticScore，通过四个可解释性原则（FGTI）和基于LLM的特征提取方法，在保持高评分准确性的同时提供透明性。


<details>
  <summary>Details</summary>
Motivation: 解决自动评分系统缺乏透明性和可解释性的问题，满足大规模真实评估中对可解释性日益增长的需求。
...
TL;DR: MUCH是首个针对声明级别不确定性量化的基准测试，包含4,873个多语言样本和4个开源LLM的生成对数概率，支持公平可复现的评估。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型可靠性不足的问题，需要开发声明级别的不确定性量化方法，但缺乏合适的评估基准。

Method: 构建多语言基准数据集，提供24个生成对数概率，并提出确定性算法进行声明分割，仅需0.2%的LLM生成时间。

Result: 创建了包含4个欧洲语言和4个开源LLM的基准测试，支持白盒方法开发，分割算法适合实时监控。

Conclusion: 当前方法在性能和效率方面仍有很大改进空间，MUCH基准为未来方法提供了公平评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要用于因果预测任务，不适合生成整体文本表示。现有方法多依赖token级预测目标，但上下文压缩作为预训练任务的潜力尚未被充分探索。

Method: 采用上下文压缩作为预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行序列预测，并结合对比学习进一步优化表示质量。

Result: 实验表明，精心设计的压缩目标显著提升了LLM的文本表示能力，优于基于token级预训练任务的模型。通过对比学习得到的LLM2Comp模型在广泛任务中优于当代LLM文本编码器，且样本效率更高。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM的文本表示性能，同时减少训练数据需求，为LLM在文本表示任务中的应用提供了新思路。
...
TL;DR: 本文介绍了LangMark，一个用于自动后编辑的多语言数据集，包含英语到7种语言的翻译，展示了LLMs通过少样本提示可以有效执行APE任务。


<details>
  <summary>Details</summary>
Motivation: 解决缺乏专门针对NMT输出的大规模多语言APE数据集的问题，以促进APE系统的开发。

Method: 构建并发布了LangMark数据集，包含206,983个三元组（源文本、NMT输出、人工后编辑翻译），由专业语言学家标注。使用该数据集评估LLMs的少样本提示APE能力。

Result: LLMs通过少样本提示能够有效执行APE，改进领先的商业甚至专有机器翻译系统的翻译质量。

Conclusion: LangMark数据集将促进APE系统的未来发展和评估，LLMs在APE任务中表现出色。
...
TL;DR: 本文介绍了PLLuM项目中用于微调大型语言模型的指令数据集，包括有机、转换和合成指令的功能分类，并分享了关于使用人工编写与合成指令数据集在语言适应方面的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目的波兰语大型语言模型开发提供指令数据集，并探讨不同类型指令在语言模型适应中的影响。

Method: 使用有机、转换和合成三种类型的指令数据集来微调基于transformer的大型语言模型。

Result: 发布了PLLuM指令语料库的第一个代表性子集，该数据集可为其他LLM的类似数据集开发提供指导和规划参考。

Conclusion: 不同类型指令数据集在语言模型适应中具有不同影响，发布的指令语料库子集对其他LLM开发具有参考价值。
...
TL;DR: 提出了基于方面的因果弃权（ABCA）框架，通过因果推理分析LLM内部知识多样性来实现早期弃权，防止不可靠回答。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法预先防止不可靠回答，需要一种能够早期识别知识冲突和知识不足的机制。

Method: ABCA框架通过分析LLM内部知识的多方面性（如学科、法律背景、时间框架等），估计基于这些方面的因果效应来评估查询相关知识的可靠性。

Result: 在标准基准测试中，ABCA提高了弃权的可靠性，达到了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA框架通过因果推理分析内部知识多样性，能够有效实现早期弃权，提高LLM回答的可靠性。
...
TL;DR: 本文提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合在统一框架中，通过注意力机制动态确定各特征类型的相对重要性，在新闻文本分类任务中取得了优于传统统计模型和纯语义深度学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法虽然能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法虽能理解语义但可能忽略简单但高影响的统计指标。需要结合两种特征类型的优势来提升新闻文本分类性能。

Method: 提出注意力引导特征融合(AGFF)模型，在统一框架中融合统计特征和语义特征，使用基于注意力的机制动态确定各特征类型的相对重要性，从而做出更明智的分类决策。

Result: 在基准新闻数据集上的评估显示，AGFF模型相比传统统计模型和纯语义深度学习模型表现出更优越的性能，证实了多样化特征类型的战略整合能显著提高分类准确性。消融研究验证了融合过程中各组成部分的贡献。

Conclusion: 该模型能够平衡并利用统计和语义表示的互补优势，为现实世界新闻分类任务提供了实用有效的解决方案。
...
TL;DR: 本文提出了一种名为E3的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，在保持性能的同时实现高效的模型压缩和推理加速。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断增大，层剪枝作为一种硬件友好的模型压缩方法受到关注。但现有方法难以同时解决性能下降、训练成本高和加速有限等实际部署挑战。

Method: 提出E3框架，包含两个关键创新：1）使用Gumbel-TopK采样器的可微分掩码优化方法，实现高效精确的剪枝掩码搜索；2）熵感知自适应知识蒸馏策略，提升任务性能。

Result: 在多种模型架构和基准测试中表现优异。在Qwen3-32B模型上剪枝25%层数时，在MATH-500上达到96%准确率，仅比原始模型（96.8%）下降0.8%，优于现有SOTA方法（95%），推理速度提升1.33倍，仅消耗0.5B tokens（占后训练数据量的0.5%）。

Conclusion: E3框架能够有效解决层剪枝中的性能、成本和效率平衡问题，为大规模语言模型的部署提供了实用的压缩解决方案。
...
TL;DR: PARROT是一个评估大语言模型在社会压力下准确性退化的框架，通过权威性错误问题测试模型的顺从性，发现先进模型表现良好而老旧/小型模型出现严重认知崩溃。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在社会压力（权威和说服）下出现的过度顺从现象（sycophancy），评估模型在真实世界部署中的安全性。

Method: 使用双盲评估比较中性问题和权威性错误问题，通过基于对数似然的校准跟踪量化置信度变化，采用八状态行为分类法系统分类失败模式。

Result: 评估22个模型在13个领域的1302个MMLU式选择题：先进模型（GPT-5等）跟随率低（≤11%）且准确性损失小，而老旧/小型模型出现严重认知崩溃（GPT-4:80%，Qwen 2.5-1.5B:94%）。国际法和全球知识领域脆弱，基础数学相对稳健。

Conclusion: 应将'抵抗过度拟合压力'作为与准确性、伤害避免和隐私同等重要的主要目标，以确保大语言模型在真实世界中的安全部署。
...
TL;DR: MirageTVQA是一个新的多语言表格视觉问答基准，包含近6万个QA对，覆盖24种语言，专门评估视觉语言模型在处理视觉噪声表格和多语言场景时的表现。


<details>
  <summary>Details</summary>
Motivation: 现有表格QA数据集主要是单语言（英语）且表格格式完美，无法反映现实世界中多语言和视觉噪声的复杂情况，造成研究与实践之间的差距。

Method: 创建MirageTVQA基准，包含多语言表格和模拟扫描文档的视觉噪声，评估领先视觉语言模型在这些挑战下的表现。

Result: 评估显示主要视觉语言模型存在两个关键失败点：面对视觉噪声时性能严重下降（最佳模型下降超过35%），以及持续的英语优先偏见，推理能力无法有效迁移到其他语言。

Conclusion: MirageTVQA为衡量和推动更鲁棒的表格推理视觉语言模型提供了基准，有助于缩小研究与实践之间的差距。
...
TL;DR: 本文介绍了爱沙尼亚语版本的WinoGrande常识推理测试集的本地化翻译，评估了专有和开源模型在人工翻译基准上的表现，并探索了通过提示工程提升机器翻译质量的可行性。


<details>
  <summary>Details</summary>
Motivation: 动机是创建高质量的爱沙尼亚语常识推理基准，以更可靠地评估大型语言模型在爱沙尼亚语上的语言能力和推理表现，解决机器翻译数据质量不足的问题。

Method: 方法包括：1) 由翻译专家进行人工翻译和本地化适配；2) 评估专有和开源模型在人工翻译数据集上的表现；3) 设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示，探索机器翻译质量提升。

Result: 结果显示：1) 模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英语测试集；2) 机器翻译数据上的表现明显更差；3) 提示工程对翻译质量或模型准确性的提升有限。

Conclusion: 结论强调了语言专家参与数据集翻译和本地化的重要性，以确保对大型语言模型语言能力和推理评估的可靠性和可解释性，机器翻译和提示工程在当前阶段无法替代人工翻译的质量。
...
TL;DR: 该研究基于语料库分析了普通话单音节词在自然对话中的音高轮廓实现，发现词义对音高轮廓有显著影响，即使控制了多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索普通话单音节词在自然对话中音高轮廓的实现方式，特别关注词义对音高轮廓的影响，挑战传统的声调理论。

Method: 使用广义加性模型将观测到的音高轮廓分解为与不同控制变量和语义预测因子相关的组成音高轮廓，控制变量包括词长、性别、说话者身份、声调语境、元音高度和话语位置等。

Result: 研究发现词义对音高轮廓有显著影响：词义比词本身是更好的预测因子，异形同音词具有不同的音高轮廓，且通过上下文嵌入可以准确预测单个词标记的音高轮廓。
...
TL;DR: 提出了一种零样本自然语言推理方法，通过将语言在视觉上下文中进行基础化来利用多模态表示。该方法使用文本到图像模型生成前提的视觉表示，并通过将这些表示与文本假设进行比较来进行推理。


<details>
  <summary>Details</summary>
Motivation: 旨在通过利用多模态表示来改进自然语言推理，特别是通过将语言基础化到视觉上下文中来增强对语言含义的理解，从而提高推理的鲁棒性。

Method: 使用文本到图像模型生成前提的视觉表示，然后通过余弦相似度和视觉问答两种技术将这些视觉表示与文本假设进行比较来进行推理。

Result: 该方法在不进行任务特定微调的情况下实现了高准确率，表现出对文本偏见和表面启发式的鲁棒性。通过设计的对抗性数据集验证了方法的稳健性。
...
TL;DR: 本文提出了选择性RoPE，一种输入依赖的旋转位置嵌入机制，它扩展了传统的RoPE，允许在任意角度进行旋转，适用于线性和softmax变换器。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中输入依赖门控机制在语言任务中表现优越的启发，作者希望将这种选择性机制引入到位置编码中，以改进语言建模性能。

Method: 提出选择性RoPE机制，通过输入依赖的方式确定旋转角度，揭示了softmax注意力中隐藏的旋转结构，并在门控变换器中实现该机制。

Result: 实验验证表明，选择性RoPE在语言建模和困难序列任务（如复制、状态跟踪和检索）中显著提升了性能。

Conclusion: 选择性RoPE成功地将输入依赖的旋转机制引入位置编码，为变换器模型提供了更灵活和有效的位置表示方法。
...
TL;DR: PUCP-Metrix是一个开源工具包，提供182个西班牙语语言指标，涵盖词汇多样性、句法语义复杂性、连贯性、心理语言学和可读性，在自动可读性评估和机器生成文本检测任务中表现优异。
...
Method: 开发了一个包含182个语言指标的开源存储库，涵盖词汇多样性、句法语义复杂性、连贯性、心理语言学和可读性等多个维度。

Result: 在自动可读性评估和机器生成文本检测任务中，与现有存储库和强神经基线相比表现出竞争性性能。
...
TL;DR: ReVeL框架将多项选择题改写为开放式问题，解决了MCQA中选项泄露信号导致评估不准确的问题，在强化微调中提高了数据效率和评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 发现多项选择题的选项可能泄露可利用信号，使得准确性指标无法真实反映模型能力，并鼓励在强化微调中出现显性或隐性的答案猜测行为。

Method: 提出ReVeL框架，将多项选择题分类并重写为开放式问题，保持答案可验证性，使用GRPO方法对Qwen2.5-VL模型进行微调。

Result: 在多项选择题基准测试中保持相同准确率，同时将开放式问题准确率提高约6个百分点；在评估中揭示MCQA基准测试中高达20个百分点的分数膨胀。
...
### [127] [Functional uniqueness and stability of Gaussian priors in optimal L1 estimation](https://arxiv.org/abs/2511.16864)
*Leighton Barnes,Alex Dytso*

Main category: cs.IT

TL;DR: 本文研究高斯先验在最优L1估计中的函数唯一性和稳定性，建立了定量稳定性理论来分析最优估计器的近似线性性如何约束先验分布。


<details>
  <summary>Details</summary>
Motivation: 虽然已知高斯先验在高斯噪声下唯一诱导线性条件均值，但对于条件中位数（即绝对误差损失下的最优估计器）的类似问题最近才得到解决。本文基于这一唯一性结果，旨在发展定量稳定性理论。

Method: 对于L2损失，推导显示条件均值的近似线性性意味着先验在Lévy度量下接近高斯分布。对于L1损失，引入Hermite展开框架并分析线性性定义算子的伴随算子。

Result: 证明了高斯分布仍然是唯一稳定的解，为高斯噪声下贝叶斯估计中的线性和稳定性提供了更完整的函数分析理解。

Conclusion: 这些结果共同提供了对高斯噪声下贝叶斯估计中线性性和稳定性的更完整函数分析理解。
...
### [128] [The Star Product of Uniformly Random Codes](https://arxiv.org/abs/2511.17236)
*Johan V. Dinesen,Ragnar Freij-Hollanti,Camilla Hollanti,Benjamin Jany,Alberto Ravagnani*

Main category: cs.IT

TL;DR: 本文研究了两个随机线性码的星积的期望维度问题，通过建立星积与双线性形式评估的对应关系，给出了星积期望维度的下界，并证明在域大小和码维数趋于无穷时，期望维度达到最大值。


<details>
  <summary>Details</summary>
Motivation: 研究随机线性码星积的期望维度，对于理解编码理论中的基本问题具有重要意义，并在私有信息检索、安全分布式矩阵乘法、量子纠错码等领域有应用价值。

Method: 通过建立星积与双线性形式评估的对应关系，利用概率方法和渐近分析来研究星积的期望维度。

Result: 给出了星积期望维度的下界，并证明当域大小和码维数都趋于无穷时，期望维度达到其最大值。

Conclusion: 该研究为编码理论提供了新的分析工具，在多个应用领域具有潜在价值，包括密码分析中的可能应用。
...
### [129] [Structured Approximation of Toeplitz Matrices and Subspaces](https://arxiv.org/abs/2511.17239)
*Albert Fannjiang,Weilin Li*

Main category: cs.IT

TL;DR: 本文研究了两个结构化逼近问题：恢复损坏的低秩Toeplitz矩阵和从单次观测中恢复傅里叶矩阵的范围。通过应用Gradient-MUSIC谱估计算法，可以高效且最优地解决这两个计算难题。


<details>
  <summary>Details</summary>
Motivation: Toeplitz矩阵和傅里叶矩阵的结构化逼近问题在计算上具有挑战性，因为直接施加结构约束很困难。这些问题的解决对于信号处理、系统识别等领域具有重要意义。

Method: 使用Gradient-MUSIC谱估计算法来解决这两个结构化逼近问题。对于秩为r的Toeplitz矩阵，在满足正则性假设且被任意误差矩阵E损坏的情况下，算法输出一个秩恰好为r的Toeplitz矩阵估计。

Result: 算法输出的Toeplitz矩阵估计满足‖T-Ť‖₂ ≤ C√r‖E‖₂的性能保证，其中C,α>0是绝对常数。该性能保证在n和‖E‖₂方面是极小极大最优的。第二个问题也获得了最优结果。

Conclusion: Gradient-MUSIC算法能够高效且最优地解决结构化矩阵恢复问题，建立了这些问题与谱估计之间的定量联系。结果同样适用于Hankel矩阵（需轻微修改）。
...
### [130] [Fast Decoding for Non-Adaptive Learning of Erdős--Rényi Random Graphs](https://arxiv.org/abs/2511.17240)
*Hoang Ta,Jonathan Scarlett*

Main category: cs.IT

TL;DR: 本文研究了通过节点子集的组查询学习未知图的问题，提出了一个高效的测试-解码方案，使用O(\bar{k}\log n)次测试即可以高概率恢复ER图的边集，同时解码时间为O(\bar{k}^{1+δ}\log n)。


<details>
  <summary>Details</summary>
Motivation: 现有方法在非自适应设置下学习任意图需要大量测试，而针对ER图的学习方案要么解码时间高达Ω(n²)，要么需要额外的测试因子。本文旨在设计一个既保持最优测试次数又实现亚线性解码时间的方案。

Method: 将最近为非自适应组测试开发的二进制分裂方法扩展到ER图学习设置，通过精心设计的组查询策略来识别图中的边。

Result: 证明了使用O(\bar{k}\log n)次测试可以以高概率恢复ER图的边集，同时解码时间仅为O(\bar{k}^{1+δ}\log n)，其中δ>0为任意固定常数。

Conclusion: 提出的方法在保持最优测试次数的同时显著降低了计算复杂度，为大规模图学习问题提供了实用的解决方案。
...
### [131] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [132] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*
...
<details>
  <summary>Details</summary>
Motivation: 受到复古BASIC简单性的启发，旨在为LLM推理提供透明、可解释的多步推理框架，使模型能够可靠地模拟短程序执行。

Method: 设计一个自然语言解释器文件来指定命令语义、内存更新和日志行为，通过心智模型解释器提取陈述性和程序性知识，检测矛盾并在必要时产生解决方案。

Result: 在三个LLM上对知识提取、冲突检测和推理任务的基准测试表明，所有模型都能执行Cognitive BASIC程序，整体表现强劲但不统一。

Conclusion: Cognitive BASIC为LLM提供了有效的结构化推理框架，实现了透明且可解释的多步推理过程。
...
### [133] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出一个利用响应模式统计分析来识别基准测试中无效问题的框架，通过统计异常检测和LLM初审来系统化修订基准测试。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效问题会破坏其可靠性。手动检查数千个问题不可行，需要自动化方法来提高基准测试质量。

Method: 基于模型性能均值充分性假设，通过统计响应模式分析检测超出预期范围的异常问题，结合LLM初审减少人工审核工作量。

Result: 在9个广泛使用的基准测试中，该方法识别问题问题的准确率达到84%，显著提高了基准测试的可靠性。

Conclusion: 该框架为系统化基准测试修订提供了高效可扩展的解决方案，通过统计分析和LLM辅助大幅减少了人工审核负担。
...
### [134] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
...
Conclusion: HDR机制有效解决了多车辆协同驾驶中奖励信号失效的问题，为高频连续控制任务提供了有效的奖励设计方法。
...
### [135] [Budget-Aware Tool-Use Enables Effective Agent Scaling](https://arxiv.org/abs/2511.17006)
...
TL;DR: 本文研究了在明确工具调用预算下如何有效扩展工具增强智能体，提出了预算跟踪器和BATS框架，使智能体具备预算意识并动态调整策略，从而改善成本-性能扩展曲线。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在扩展测试时计算时缺乏预算意识，即使给予更大的工具调用预算也无法提升性能，需要研究如何在预算约束下有效扩展智能体。

Method: 提出预算跟踪器插件提供持续预算意识，开发BATS框架利用预算意识动态调整规划和验证策略，决定是深入探索还是转向新路径，并建立统一的成本度量标准。

Result: 预算感知方法产生更有利的扩展曲线，推动了成本-性能帕累托前沿，提供了对工具增强智能体扩展的更透明和原则性理解。

Conclusion: 预算意识对于工具增强智能体的有效扩展至关重要，提出的方法能够系统性地改善在预算约束下的智能体性能。
...
### [136] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 论文从贝叶斯角度重新审视了基于分数的扩散模型在逆问题求解中的作用，提出了DAPS++方法，将扩散过程重新解释为EM框架中的初始化阶段，实现了更高的计算效率和鲁棒的重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数的扩散模型在解决逆问题时，虽然采用联合推理框架，但实际运行中先验提供的指导有限，重建主要由测量一致性项驱动，与扩散动力学基本解耦。作者希望澄清这种结构并改进推理过程。

Method: 将扩散在逆问题求解中的作用重新解释为EM框架中的初始化阶段，引入DAPS++方法，让似然项更直接地指导推理，同时保持数值稳定性。该方法将扩散阶段和数据驱动细化完全解耦。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 通过重新解释扩散在逆问题求解中的角色，DAPS++提供了对统一扩散轨迹在实践中保持有效的理论解释，同时显著提升了计算效率。
...
### [137] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了MIR方法来解决多智能体强化学习中稀疏奖励问题，通过激励智能体探索影响队友的行为来改善团队探索效果。
...
Method: 提出了MIR方法，激励个体智能体探索能够影响队友的行为，并与原始策略结合来刺激团队探索。
...
### [138] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一个基于层次任务抽象机制（HTAM）的新型多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务。该框架通过将多智能体系统组织成反映领域内在任务依赖关系的逻辑层次结构，超越了传统基于社会角色模拟的方法。


<details>
  <summary>Details</summary>
Motivation: 现有的通用智能体框架（如ReAct或基于人类角色扮演的方法）在需要严格结构化工作流程的专业领域（如遥感）中表现不佳，因为这些领域需要专门工具和多步骤程序。

Method: 提出了层次任务抽象机制（HTAM），将多智能体系统构建成逻辑层次结构，反映领域内在的任务依赖图。每个层次的子智能体在前一层输出上操作，确保程序正确性并将复杂问题分解为顺序层次。

Result: 构建了EarthAgent系统并在GeoPlan-bench基准测试中进行了评估。实验表明，EarthAgent显著优于一系列现有的单智能体和多智能体系统。

Conclusion: 将智能体架构与领域内在任务结构对齐是构建稳健可靠的专业自主系统的关键步骤，HTAM框架为此提供了有效解决方案。
...
### [139] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为构建智能AI系统的基础，以实现具备持续自主性、推理和交互能力的智能系统。


<details>
  <summary>Details</summary>
Motivation: 为了实现智能AI系统，需要将关于智能体的假设与明确的认知、合作和治理模型相结合，从而构建不仅能力强、灵活，而且透明、合作和可问责的智能系统。

Method: 通过将自适应、数据驱动的方法与结构化的推理和协调模型相结合，利用AAMAS社区开发的概念工具（如BDI架构、通信协议、机制设计和制度建模）作为基础。

Result: 提出了一个连接形式理论和实践自主性的智能体视角，为实现透明、合作和可问责的智能系统奠定了基础。

Conclusion: AAMAS社区的概念工具为构建智能AI系统提供了坚实基础，通过结合自适应方法与结构化模型，可以实现既强大又负责任的智能系统，弥合了形式理论与实际自主性之间的差距。
...
### [140] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成和离策略数据训练探针在LLM行为监测中的泛化能力，发现数据生成策略显著影响探针性能，且相同领域的离策略数据比不同领域的策略内数据更可靠。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然示例稀少，研究者被迫依赖合成或离策略的LLM响应来训练探针，需要评估这些数据对探针泛化能力的影响。

Method: 在八种不同LLM行为上测试线性和注意力探针，比较不同响应生成策略（合成、离策略、策略内）对探针性能的影响。

Result: 发现从离策略数据到策略内数据的成功泛化可以预测真实监测场景中的表现，欺骗和故意表现不佳的探针可能无法从离策略泛化到策略内数据。

Conclusion: 在缺乏策略内数据时，使用相同领域的离策略数据比使用不同领域的策略内数据能产生更可靠的探针，强调需要更好处理LLM监测中分布偏移的方法。
...
### [141] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
*Jiaxi Liu,Chengyuan Ma,Hang Zhou,Weizhe Tang,Shixiao Liang,Haoyang Ding,Xiaopeng Li,Bin Ran*

Main category: cs.AI

TL;DR: 提出SRA-CP框架，通过风险感知的选择性协作感知，在保持安全关键对象检测精度的同时，仅使用20%通信带宽，相比无风险感知的选择性CP方法提升15%感知性能。
...
Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性协作。包含感知风险识别模块、基于感知覆盖的伙伴选择机制，以及适应带宽约束的融合模块。

Result: 在公共数据集上评估，相比通用CP，安全关键对象的平均精度损失小于1%，同时仅使用20%通信带宽。相比无风险感知的选择性CP方法，感知性能提升15%。

Conclusion: SRA-CP框架通过风险感知的选择性协作，有效解决了CP中的通信带宽和动态环境适应性挑战，在保持安全性的同时显著降低通信开销。
...
### [142] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
*Thorsten Fröhlich,Tim Schlippe*

Main category: cs.AI

TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用自然语言处理技术提供从开题到最终提交的自动化评估，旨在提高评估的一致性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统论文评估方法耗时且易受评估者主观性影响，需要更高效、一致的解决方案来保证学术严谨性。

Method: 采用先进的自然语言处理技术，包括大语言模型、检索增强生成和结构化思维链提示，实现初步评估、多维度评估、内容提取、基于量规的评分和详细报告。

Result: 开发了RubiSCoT框架，能够提供一致、可扩展且透明的学术评估过程。
