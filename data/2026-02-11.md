...
TL;DR: 提出SemanticMoments方法，通过预训练语义模型提取特征的高阶时序统计量，解决现有视频检索方法过度依赖静态外观而忽略语义运动的问题。
...
Conclusion: 语义特征空间中的时间统计量能够有效解决视频检索中的语义运动理解问题，为运动中心的视频理解提供了可扩展且感知基础良好的基础。
...
TL;DR: 该研究提出了一个从新闻视频中自动检测和提取人名的框架，通过模块化可审计的提取流程，在准确性和可追溯性之间取得平衡。
...
Result: 检测器达到95.8% mAP@0.5，在图形元素定位方面表现稳健。虽然生成式系统在原始准确率上略高(F1:84.18% vs 77.08%)，但缺乏透明数据溯源。提出的流程在精确率(79.9%)和召回率(74.4%)之间取得平衡，避免幻觉并提供完整可追溯性。59%的受访者表示难以阅读快节奏广播中的屏幕姓名。

Conclusion: 该研究为现代新闻媒体中的混合多模态信息提取建立了一个方法严谨且可解释的基准，在确定性可审计性和随机推理之间揭示了明确的权衡。
...
TL;DR: 提出基于场景图的零样本条件机制ASQL Conditioner，通过轻量语言模型生成视觉指导，在推理时优化扩散模型，提升文本到图像合成的语义保真度和结构一致性。
...
Method: 提出基于场景图结构的文本到图像合成框架，引入零样本场景图条件机制。核心是ASQL Conditioner（属性-尺寸-数量-位置条件器），通过轻量语言模型生成软视觉指导，在推理时通过优化引导扩散模型生成。

Result: 该方法能够在保持文本-图像对齐的同时，支持轻量、一致且多样化的图像合成，增强了现有模型的组合能力。

Conclusion: 基于场景图的ASQL Conditioner方法为复杂文本到图像合成提供了有效的解决方案，通过软视觉指导机制平衡了语义保真度和生成多样性。
...
TL;DR: 使用轻量级CNN-RNN模型从视觉数据预测步态转换中的足底压力中心位置和触地时间，在消费级设备上实现60FPS实时预测


<details>
  <summary>Details</summary>
Motivation: 计算机视觉已用于步态环境分类，但预测足部如何接触变化环境的能力尚未充分探索。本研究旨在评估在平地到上楼梯转换中预测足底压力中心和触地时间的可行性，这对辅助系统的预测控制具有重要意义。

Method: 8名受试者右小腿佩戴RGB-D相机和仪器鞋垫，执行上楼梯任务。训练CNN-RNN模型在触地前250ms窗口内连续预测足底压力中心和触地时间。模型轻量级，可在消费级笔记本或边缘计算设备上以60FPS运行。

Result: 在150、100、50ms预测窗口下，足底压力中心平均绝对误差分别为29.42mm、26.82mm、23.72mm；触地时间平均绝对误差分别为21.14ms、20.08ms、17.73ms。更快的脚趾摆动速度提高了足底压力中心预测精度，更靠前的触地点降低了其预测精度，但躯干速度对两者均无影响。

Conclusion: 使用轻量级模型从视觉数据预测足底压力中心和触地时间是可行的，这对辅助系统的预测控制具有重要应用价值，可实现实时预测并适应不同步态特征。
...
TL;DR: 本文介绍了VLM-UQBench基准测试，用于评估视觉语言模型中的模态特定和跨模态不确定性量化方法，发现现有方法存在模态特异性强、与幻觉关联弱、对细微模糊检测能力不足等问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的不确定性量化对于确保其安全可靠行为至关重要，但现有方法难以定位不确定性的来源（图像、文本或模态间不对齐），需要专门的基准测试来评估模态感知的不确定性量化方法。

Method: 提出VLM-UQBench基准测试，包含600个来自VizWiz数据集的真实样本，分为干净、图像、文本和跨模态不确定性子集；设计了可扩展的扰动流程，包含8种视觉、5种文本和3种跨模态扰动；提出两个简单指标来量化UQ分数对这些扰动的敏感性及其与幻觉的关联性。

Result: 评估了多种UQ方法在四个VLM和三个数据集上的表现，发现：(1)现有UQ方法表现出强烈的模态特异性，且严重依赖底层VLM；(2)模态特定不确定性常与幻觉同时出现，但当前UQ分数仅提供微弱且不一致的风险信号；(3)UQ方法在处理明显的群体级模糊时能与基于推理的思维链基线相媲美，但在检测细微的实例级模糊方面表现不佳。
...
TL;DR: IR-SIS：首个支持自然语言描述的迭代细化手术图像分割系统，通过SAM3、视觉语言模型和智能体工作流实现自适应分割优化，支持临床医生交互反馈。
...
Result: 实验表明，IR-SIS在域内和域外数据上都达到了最先进的性能。临床医生交互进一步提升了分割效果。这是首个具有自适应自细化能力的语言驱动手术分割框架。

Conclusion: IR-SIS建立了首个基于语言的手术分割框架，具有自适应自细化能力，支持临床医生交互，为机器人辅助手术和术中导航提供了更灵活、更准确的分割解决方案。
...
TL;DR: 研究发现传统扩散变换器中的池化文本嵌入对性能贡献有限，但将其用作引导机制可在训练免费的情况下显著提升多任务性能


<details>
  <summary>Details</summary>
Motivation: 探讨扩散变换器中基于调制的文本条件是否必要，以及能否提供性能优势。当前方法大多放弃调制机制而仅依赖注意力机制。

Method: 分析传统池化嵌入的使用效果，提出将其重新定位为引导机制，实现可控的属性偏移。该方法无需训练、易于实现、运行时开销可忽略，适用于多种扩散模型。

Result: 传统使用方式下池化嵌入对整体性能贡献很小，注意力机制通常足以有效传播提示信息。但将池化嵌入用作引导机制时，能在文本到图像/视频生成和图像编辑等多样化任务中带来显著改进。

Conclusion: 虽然传统调制机制中的池化嵌入作用有限，但将其重新用作引导机制可提供训练免费的性能提升，为扩散模型的多任务应用提供简单有效的改进方案。
...
<details>
  <summary>Details</summary>
Motivation: 高质量医学影像数据集对深度学习训练至关重要，但未经授权使用引发严重的版权和伦理问题。现有针对自然图像设计的数据集所有权验证方法在医学图像上效果不佳，因为静态水印模式在动态高分辨率扫描中难以适应，同时需要保持诊断质量。

Method: 提出X-Mark方法：使用条件U-Net在每个样本的显著区域内生成独特扰动；设计多组件训练目标确保水印有效性、对动态缩放过程的鲁棒性，同时保持诊断质量和视觉可区分性；在训练目标中加入拉普拉斯正则化以惩罚高频扰动，实现水印尺度不变性；在黑盒设置下进行所有权验证以检测可疑模型中的特征行为。

Result: 在CheXpert数据集上的广泛实验验证了X-Mark的有效性：实现了100%的水印成功率（WSR），在Ind-M场景中将误报概率降低了12%，同时展示了对潜在自适应攻击的抵抗能力。

Conclusion: X-Mark为医学影像版权保护提供了一种有效的样本特异性清洁标签水印方法，解决了现有方法在医学图像领域的局限性，在保持诊断质量的同时实现了鲁棒的所有权验证。
...
<details>
  <summary>Details</summary>
Motivation: 患者住院是伤口护理成本高昂的主要因素之一。许多伤口本不需要立即住院，但由于治疗延迟、患者不配合或存在共病条件等因素，伤口可能恶化并最终导致患者住院。现有研究主要关注基于特定伤口类型的愈合轨迹，缺乏综合预测住院风险的方法。

Method: 提出一种深度多模态方法，结合伤口变量和伤口图像来预测患者住院风险。采用迁移学习技术开发伤口评估解决方案，能够从伤口图像预测伤口变量及其愈合轨迹。

Result: 开发了一个能够预测伤口变量和愈合轨迹的迁移学习模型，该模型有助于早期发现可能影响愈合过程的伤口并发症。

Conclusion: 提出的新型模型有助于早期检测伤口复杂性，可能影响愈合过程，并能减少临床医生诊断伤口所需的时间，从而降低患者住院风险和相关医疗成本。
...
TL;DR: 该研究评估了13种深度学习模型在膀胱癌分类任务上的表现，包括CNN和Transformer架构，重点关注泛化能力、校准效果和可解释性。


<details>
  <summary>Details</summary>
Motivation: 尽管ViT和CNN在自然图像数据集上表现优异，但在医学影像中，异常区域通常只占图像的一小部分，这些模型可能不适用。因此需要研究最新深度学习模型在膀胱癌分类任务中的表现。

Method: 1) 使用13个模型进行标准分类（4个CNN和8个Transformer模型）；2) 校准分析评估模型校准效果；3) 使用GradCAM++评估模型可解释性；4) 在公开多中心膀胱癌数据集上进行约300次实验；5) 使用测试时增强提高模型可解释性。

Result: ConvNext系列在膀胱癌图像分类中表现出有限的泛化能力（约60%准确率）；ViT相比ConvNext和Swin Transformer系列表现出更好的校准效果；没有模型能提供通用的可解释解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 技术文档的深度理解需要准确分析其中的图形、表格、图表等多种模态及其关联，曲线图表作为技术文档的重要组成部分，其自动识别与分析对于文档理解具有重要意义

Method: 采用基于聚类的方法识别曲线中的中间点来界定线段，分析处理部分解析提取的曲线线段以捕获方向、趋势等行为特征，将这些关联转换为属性图以保持曲线结构特征，并将图关系表达为自然语言文本，最终转换为随机Petri网图
...
TL;DR: 提出DBiSL框架，通过可微分的双向协同学习，将监督学习、一致性正则化、伪监督学习和不确定性估计四个关键SSL组件无缝集成，在医学图像分割中实现双向跨任务协作。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中高质量标注数据稀缺且成本高昂，半监督学习是解决这一瓶颈的有效方法。现有双任务协作学习方法仅限于单向交互机制（通常从回归到分割），无法充分利用在线双向跨任务协作的潜在优势。

Method: 提出完全可微分的双向协同学习（DBiSL）框架，通过可微分机制实现分割和回归任务之间的双向在线协作，无缝集成监督学习、一致性正则化、伪监督学习和不确定性估计四个关键SSL组件。

Result: 在两个基准数据集上的实验表明，该方法达到了最先进的性能水平。

Conclusion: 该工作不仅提供了统一SSL框架设计的新见解，为双任务驱动的SSL建立了新的架构基础，还提供了一个适用于更广泛计算机视觉应用的通用多任务学习框架。
...
### [13] [Federated Learning for Surgical Vision in Appendicitis Classification: Results of the FedSurg EndoVis 2024 Challenge](https://arxiv.org/abs/2510.04772)
...
TL;DR: FedSurg挑战赛建立了首个手术视频分类联邦学习基准，评估方法在未见临床中心的泛化能力和本地微调适应能力，发现ViViT模型表现最佳，但泛化能力有限且排名稳定性低。


<details>
  <summary>Details</summary>
Motivation: 设计FedSurg挑战赛旨在评估联邦学习方法在手术视频分类中的表现，特别是评估方法在未见临床中心的泛化能力，以及通过本地微调进行中心特定适应的能力，同时实现不共享患者数据的协作模型开发。

Method: 参与者使用多中心Appendix300视频数据集开发策略来分类阑尾炎炎症阶段。挑战赛评估两个任务：对未见中心的泛化能力和微调后的中心特定适应。提交的方法包括基础模型线性探测、使用三元组损失的度量学习，以及各种联邦学习聚合方案（FedAvg、FedMedian、FedSAM）。使用F1分数和期望成本评估性能，通过自助法和统计测试评估排名稳健性。

Result: 在泛化任务中，各中心性能有限；在适应任务中，所有团队微调后都有改善，但排名稳定性低。基于ViViT的提交获得最强整体性能。挑战赛凸显了泛化能力有限、对类别不平衡敏感、去中心化训练中超参数调优困难等问题，而时空建模和上下文感知预处理成为有前景的策略。
...
TL;DR: 该研究评估了5种先进的图像到3D基础模型在单切片医学图像重建3D方面的零样本性能，发现所有模型在医学数据上的体素重叠度中等，深度重建存在困难，SAM3D在拓扑相似性方面表现最佳。
...
Method: 创建了受控的零样本基准测试，评估5种最先进的图像到3D模型（SAM3D、Hunyuan3D-2.1、Direct3D、Hi3DGen、TripoSG）在6个医学数据集和2个自然数据集上的表现，使用体素重叠度和点云距离指标。

Result: 所有模型在医学数据集上的体素重叠度中等，表明从单切片推断体积时存在深度重建失败模式。全局距离指标显示方法间差异更大：SAM3D在拓扑相似性方面表现最强，其他模型更容易过度简化重建。

Conclusion: 结果量化了单切片医学重建的局限性，突出了2D医学数据的平面性导致的深度模糊问题，需要多视角图像到3D重建来实现可靠的医学3D推断。
...
TL;DR: K-Sort Eval：一个基于视觉语言模型的可靠高效评估框架，通过后验校正和动态匹配解决VLM幻觉偏差问题，在保持与人工评估一致性的同时大幅减少评估成本。


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展需要更可扩展且与人类对齐的评估方法。现有众包Arena平台虽然提供人类偏好评估，但成本高、耗时长，限制了可扩展性。使用视觉语言模型替代人工评估是可行方案，但VLM的幻觉和偏差问题影响评估可靠性，且静态评估方法效率低下。

Method: 提出K-Sort Eval框架，包含后验校正和动态匹配两个核心方法：1）从数千个人类投票中构建高质量数据集，每个实例包含K个模型的输出和排名；2）评估新模型时进行(K+1)方自由比较，VLM提供排名；3）后验校正方法基于VLM预测与人类监督的一致性自适应校正贝叶斯更新中的后验概率；4）动态匹配策略平衡不确定性和多样性以最大化每次比较的期望收益。
...
TL;DR: 提出算子理论框架分析具有块结构参数的非线性逆问题的稳定性和统计集中性，建立了确定性稳定性不等式、最小二乘失配泛函的全局Lipschitz界和非渐近集中估计，并应用于高斯泼溅渲染算子验证


<details>
  <summary>Details</summary>
Motivation: 现代成像和可微渲染中出现的高维非线性逆问题需要统一的算子理论框架来分析其稳定性和统计性质，特别是针对具有块结构参数的复杂模型

Method: 结合块状Lipschitz几何、局部可识别性和次高斯噪声的统一假设集，建立确定性稳定性不等式、最小二乘失配泛函的全局Lipschitz界和非渐近集中估计

Result: 得到与正向算子内在相关且独立于特定重建算法的高概率参数误差界，验证高斯泼溅渲染算子满足假设并推导其Lipschitz连续性和分辨率相关可观测性的显式常数

Conclusion: 分析揭示了稳定性-分辨率权衡，表明估计误差固有地受图像分辨率与模型复杂度比值的约束，为现代成像和可微渲染中的高维非线性逆问题提供了算子级极限特征
...
TL;DR: SceneReVis：通过视觉基础的自反思框架解决3D场景合成中的空间幻觉问题，采用"诊断-行动"循环和多模态反馈来识别和解决空间冲突


<details>
  <summary>Details</summary>
Motivation: 当前一次性3D场景合成方法由于缺乏审慎推理，经常出现空间幻觉问题（如碰撞），需要更系统的空间规划能力

Method: 1. 提出SceneReVis框架，采用迭代的"诊断-行动"循环，利用多模态反馈明确拦截和解决空间冲突
2. 构建SceneChain-12k大规模数据集，通过新颖的反向工程流程获取因果构建轨迹
3. 提出两阶段训练方法：从监督微调过渡到智能体强化学习，将模型演变为主动空间规划器

Result: SceneReVis在高保真生成和目标导向优化方面达到最先进性能，并在长尾领域展现出强大的泛化能力

Conclusion: 通过视觉基础的自反思框架和迭代诊断机制，能够有效解决3D场景合成中的空间幻觉问题，提升场景生成的质量和可靠性
...
TL;DR: Fine-T2I是一个大规模、高质量、完全开放的文本到图像微调数据集，包含600万对文本-图像样本，覆盖多种任务、类别和风格，显著提升了生成质量和指令遵循能力。
...
Method: 构建了覆盖10种任务组合、32个提示类别、11种视觉风格和5种提示模板的大规模数据集，结合现代强模型生成的合成图像和专业摄影师精心策划的真实图像，通过严格过滤（去除超过95%的初始候选样本）确保文本-图像对齐、视觉保真度和提示质量。

Result: 最终数据集包含超过600万对文本-图像样本，约2TB磁盘空间，接近预训练数据集的规模但保持微调级别的质量。在多种预训练扩散和自回归模型上微调Fine-T2I，通过人工评估、视觉比较和自动指标验证，一致提高了生成质量和指令遵循能力。

Conclusion: Fine-T2I作为一个大规模、高质量、完全开放的数据集，有助于缩小开源社区在文本到图像微调方面的数据差距，推动该领域的研究和发展。
...
TL;DR: ArtifactLens：利用少量标注数据解锁预训练视觉语言模型中的伪影检测能力，在五个基准测试中达到SOTA


<details>
  <summary>Details</summary>
Motivation: 现代图像生成器产生逼真图像，但存在手部变形等伪影。检测这些伪影对于基准测试和改进生成器至关重要。现有方法需要大量标注数据，成本高昂且难以适应新生成器或新伪影类型。

Method: ArtifactLens系统采用多组件架构，包括上下文学习和文本指令优化。通过少量标注示例（每类别数百张）解锁预训练VLM中的伪影检测知识，无需大规模微调。

Result: 在五个人工伪影基准测试中达到最先进水平（首次跨多个数据集评估），所需标注数据量减少几个数量级。方法可泛化到其他伪影类型（物体形态、动物解剖、实体交互）和AIGC检测任务。

Conclusion: 预训练VLM已编码伪影检测知识，通过适当架构设计可用少量数据解锁该能力，为高效、可扩展的伪影检测提供新途径。
...
TL;DR: FD-DB是一种频率解耦的双分支模型，通过分离低频可解释编辑和高频残差补偿来解决合成到真实域转换中的外观差异问题，在保持结构稳定的同时提高真实感。


<details>
  <summary>Details</summary>
Motivation: 合成数据为几何敏感视觉任务提供低成本、精确标注的样本，但合成域与真实域之间的外观和成像差异导致严重的域偏移，降低下游任务性能。现有的无配对合成到真实转换方法在真实感和结构稳定性之间存在权衡：无约束生成可能引入变形或虚假纹理，而过于严格的约束又限制了向真实域统计的适应。
...
Conclusion: FD-DB通过频率解耦的双分支架构有效解决了合成到真实域转换中真实感与结构稳定性之间的权衡问题，为几何敏感视觉任务提供了高质量的域适应解决方案。
...
TL;DR: 提出WeakSupCon框架，通过弱监督对比学习改进数字病理学WSI的特征表示，仅使用包级标签即可在特征空间中分离不同标签的patch，提升下游MIL性能。


<details>
  <summary>Details</summary>
Motivation: 数字病理学WSI分析面临训练标签有限的问题，手动标注耗时费力。现有MIL方法主要关注特征聚合，而忽略了MIL设置下的特征表示学习。需要开发能够有效利用包级标签信息的特征学习方法。

Method: 提出弱监督对比学习框架WeakSupCon，在训练过程中融入包级标签信息，无需实例级伪标签，但能在特征空间中有效分离不同标签的patch。该方法专注于改进MIL中的特征表示学习。

Result: 在三个数据集上的实验结果表明，WeakSupCon方法生成的特征相比自监督对比学习方法，能显著提升下游MIL任务的性能。

Conclusion: WeakSupCon框架为数字病理学WSI分析提供了一种有效的弱监督特征学习方法，仅需包级标签即可改善特征表示，从而提升MIL性能，解决了标注成本高的问题。
...
TL;DR: 提出Align-TI知识蒸馏框架，通过关注令牌交互来压缩多模态大语言模型，在视觉-指令交互和响应内部令牌交互两个维度对齐师生模型，实现参数高效的多模态模型训练。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然具有强大的跨模态能力，但模型规模庞大导致部署困难。现有知识蒸馏方法主要依赖静态的下一个令牌对齐，忽略了动态令牌交互中蕴含的多模态理解和生成的关键能力。

Method: Align-TI框架从令牌交互视角设计，包含两个组件：1) IVA（指令-视觉对齐）：让学生模型模仿教师模型提取相关视觉信息的能力，通过对齐显著视觉区域实现；2) TPA（令牌概率对齐）：通过对齐序列化的令牌到令牌转移概率来捕捉教师模型的动态生成逻辑。

Result: 实验表明Align-TI优于现有方法，相比传统知识蒸馏获得2.6%的相对提升，蒸馏得到的Align-TI-2B模型甚至超越了更大的LLaVA-1.5-7B模型7.0%，为训练参数高效的多模态大语言模型建立了新的最先进蒸馏框架。

Conclusion: Align-TI通过关注令牌交互的知识蒸馏方法，有效解决了多模态大语言模型压缩问题，在保持模型能力的同时显著减小了模型规模，为实际部署提供了可行的解决方案。
...
TL;DR: 本文提出了一种名为OSI（One-step Inversion）的单步反演方法，用于快速提取高斯着色风格的水印，相比传统的多步扩散反演方法，速度提升20倍，准确率更高，且水印有效载荷容量翻倍。
...
Conclusion: OSI提供了一种快速、准确且通用的水印提取解决方案，显著提升了高斯着色风格水印的提取效率，为扩散生成图像的溯源和版权保护提供了更实用的工具。
...
TL;DR: 本文提出了一种名为平衡对比学习（ECL）的新框架，旨在解决现有监督对比学习方法在类别不平衡数据集上的局限性，通过促进几何平衡来提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有监督对比学习方法在类别不平衡数据集上表现有限，存在两个主要问题：1）未考虑类别均值/原型与分类器之间的对齐，可能导致泛化能力差；2）原型方法将原型仅视为每个类别的一个额外样本，使其影响力依赖于批次中类别实例数量，导致不同类别贡献不平衡。

Method: 提出平衡对比学习（ECL）框架，包含两个主要组件：1）促进表示几何平衡，即实现类内特征坍缩和均匀分布的类别均值的正则单纯形几何，同时平衡类别平均特征和类别原型的贡献；2）建立分类器-类别中心几何平衡，通过对齐分类器权重和类别原型。

Result: 在三个长尾数据集（CIFAR-10(0)-LT、ImageNet-LT）和两个不平衡医学数据集（ISIC 2019和自建的LCCT数据集）上进行实验，结果显示ECL优于现有的专为不平衡分类设计的监督对比学习方法。

Conclusion: ECL通过促进几何平衡，有效解决了现有监督对比学习方法在类别不平衡数据集上的局限性，在多个不平衡数据集上实现了优于现有方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法直接回归深度值，在严重或未知退化情况下容易产生伪影。为了解决这个问题，需要一种能够鲁棒处理任意退化模式且具有零样本泛化能力的深度超分辨率方法。

Method: AdaDS利用高斯平滑的收缩特性：随着前向过程中噪声累积，退化输入与原始高质量对应物之间的分布差异减小，最终收敛到各向同性高斯先验。该方法自适应选择扩散反向过程的起始时间步，基于估计的细化不确定性，然后注入定制噪声，将中间样本定位在目标后验分布的高概率区域内。

Result: 在真实世界和合成基准测试上的大量实验表明，AdaDS在零样本泛化能力和对多样化退化模式的鲁棒性方面优于现有最先进方法。

Conclusion: AdaDS通过自适应噪声注入策略，确保即使上游估计不完美时，预训练扩散模型的生成先验也能主导恢复过程，从而实现固有的鲁棒性和优异的泛化性能。
...
TL;DR: 该论文提出构建一个大规模通用动作空间（UAS），利用现有的人类动作数据集作为视觉词典，用于分析和分类哺乳动物和黑猩猩的行为。


<details>
  <summary>Details</summary>
Motivation: 动物和人类行为分析在计算机视觉中一直具有挑战性。早期方法（1970-1990年代）依赖手工特征检测和低层特征，存在固有不适定问题。行为分析通常通过跟踪识别对象并建模其轨迹，但这种方法鲁棒性和泛化能力有限。ImageNet的出现使大规模视觉识别成为可能，为构建通用视觉词典提供了基础。

Method: 遵循ImageNet范式，利用现有标注的人类动作数据集构建大规模通用动作空间（UAS）。然后使用这个UAS作为基础，分析和分类哺乳动物和黑猩猩的行为数据集。

Result: 成功构建了通用动作空间，并将其应用于哺乳动物和黑猩猩行为分析。源代码已在GitHub上开源发布。

Conclusion: 通过构建通用动作空间，将深度学习范式从人类动作识别扩展到动物行为分析，为跨物种行为理解提供了新的框架和方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型存在视觉注意力不足的问题，导致产生幻觉。现有方法通过增强所有视觉标记的注意力，但会不可避免地增加对任务无关标记的关注，需要更精细的注意力干预方法

Method: 提出无需训练的注意力干预算法：1) 基于任务相关标记通常具有高视觉-文本相似度的假设，提取视觉-文本交叉注意力子矩阵构建重加权矩阵来重新分配注意力；2) 在束搜索解码中注入视觉注意力值，以识别具有更高视觉注意力的解决方案

Result: 广泛实验表明，该方法显著减少了主流大型视觉语言模型的幻觉，同时保持了生成内容的准确性和连贯性

Conclusion: 通过基于视觉-文本相似度的注意力重分配和束搜索中的视觉注意力注入，能够有效减少大型视觉语言模型的幻觉问题，且无需额外训练
...
Result: 生成的Singpath-VL模型在细粒度形态感知和细胞级诊断分类方面表现出优越性能，将开源部分合成数据集和基准测试。

Conclusion: Singpath-VL填补了宫颈细胞学AI助手的空缺，通过创新的数据合成方法和模型微调策略，为细胞病理学领域提供了专门的多模态大语言模型解决方案。
...
TL;DR: 提出SchröMind框架，通过求解薛定谔桥问题来减少多模态大语言模型在医疗等高风险领域的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医疗等高风险领域应用受限，主要因为存在幻觉问题（生成文本与视觉输入矛盾或忽略视觉内容），需要解决这一关键挑战

Method: 提出SchröMind框架，通过求解薛定谔桥问题，建立幻觉激活与真实激活之间的token级映射，以最小传输成本实现轻量级训练，同时保留模型原有能力

Result: 在POPE和MME基准测试中表现出优越性，达到最先进性能，同时仅引入最小计算开销

Conclusion: SchröMind框架能有效减少多模态大语言模型的幻觉问题，使其更适合医疗等高风险领域应用
...
TL;DR: SCA-Net：基于Change-Agent框架的增强架构，用于双时相图像中建筑物和道路的精确变化检测，通过多尺度分析、自适应处理和注意力机制显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 遥感影像的自动变化检测对城市管理、环境监测和灾害评估至关重要。现有深度学习方法存在对小目标敏感性低、计算成本高等挑战，需要更高效准确的解决方案。

Method: 提出SCA-Net架构，包含：1）差异金字塔块用于多尺度变化分析；2）自适应多尺度处理模块结合形状感知和高分辨率增强块；3）多级注意力机制（PPM和CSAGate）进行联合上下文和细节处理；4）动态复合损失函数和四阶段训练策略。

Result: 在LEVIR-CD和LEVIR-MCI数据集上的综合评估显示，SCA-Net优于Change-Agent和其他最先进方法：在LEVIR-MCI上mIoU提升2.64%，小建筑物IoU显著增加57.9%，训练时间减少61%。
...
Method: 提出了MieDB-100k数据集，将编辑任务分为感知、修改和转换三个视角，考虑理解和生成能力。通过数据管理流程构建，利用模态特定的专家模型和基于规则的数据合成方法，并进行严格的人工检查以确保临床保真度
...
TL;DR: DR.Experts是一个基于失真先验的盲图像质量评估框架，通过显式引入失真先验来改善现有方法对细微失真线索的捕捉不足问题，从而更好地对齐人类主观质量感知。


<details>
  <summary>Details</summary>
Motivation: 现有盲图像质量评估模型通常无法有效捕捉细微失真线索，导致与人类主观判断不一致。根本原因在于缺乏可靠的失真先验，现有方法通常学习统一图像特征与质量分数之间的浅层关系，对失真不敏感从而限制了性能。

Method: 提出DR.Experts框架：1) 使用退化感知的视觉语言模型获取失真特定先验；2) 通过失真显著性差分模块区分失真与语义注意力，精炼失真先验；3) 通过动态失真加权模块（混合专家风格）融合精炼先验、语义和桥接表示，根据感知影响加权各失真特定特征。

Result: 在五个具有挑战性的BIQA基准测试上进行的广泛实验表明，DR.Experts优于当前方法，在泛化能力和数据效率方面表现出色。

Conclusion: DR.Experts通过显式引入失真先验，有效解决了现有BIQA方法对失真不敏感的问题，实现了与人类感知更一致的质量评估，在多个基准测试中展现了优越性能。
...
TL;DR: AUHead提出了一种两阶段方法，通过动作单元（AUs）实现细粒度情感控制的说话头视频生成，包括基于音频语言模型的AU生成和AU驱动的可控扩散模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法在细微情感表达方面存在不足，缺乏细粒度的情感控制能力。为了解决这个问题，需要将精细的情感控制（动作单元）从音频中解耦出来，实现可控的说话头视频生成。

Method: 采用两阶段方法：第一阶段通过时空AU标记化和"情感-然后-AU"的思维链机制，利用大型音频语言模型生成AU序列；第二阶段提出AU驱动的可控扩散模型，将AU序列映射为结构化2D面部表示，并在交叉注意力模块中建模AU-视觉交互，通过AU解耦引导策略实现灵活的AU-质量权衡控制。

Result: 在基准数据集上的实验结果表明，该方法在情感真实性、准确的唇部同步和视觉连贯性方面取得了有竞争力的性能，显著超越了现有技术。

Conclusion: AUHead通过解耦细粒度情感控制（AUs）并实现可控生成，有效提升了说话头视频的情感表达能力和视觉质量，为虚拟化身、电影制作和交互系统提供了更真实的解决方案。
...
TL;DR: AGMark是一种新颖的注意力引导动态水印框架，通过动态识别语义关键证据和自适应词汇划分，在保持视觉保真度的同时嵌入可检测信号，解决了传统视觉无关水印和静态视觉特定水印的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型的水印技术存在两个主要问题：1）视觉无关水印可能引入视觉无关标记并破坏视觉基础；2）当前视觉特定水印采用静态的一次性权重估计，忽略权重分布密度，无法适应生成过程中的视觉依赖动态变化，可能在长尾中引入低质量标记。

Method: AGMark在每个解码步骤中：1）基于注意力权重动态识别语义关键证据，结合上下文感知的连贯性线索，形成自适应校准的证据权重分布；2）联合考虑不确定性感知（标记熵）和证据校准（权重密度）来确定语义关键标记的比例，实现自适应词汇划分以避免无关标记。

Result: AGMark在生成质量上优于传统方法，特别是在生成后期阶段显著提高了视觉语义保真度。该框架保持了高度竞争力的检测准确率（至少99.36% AUC）和强大的攻击鲁棒性（至少88.61% AUC），同时不牺牲推理效率。

Conclusion: AGMark通过动态注意力引导的水印机制，有效解决了传统水印方法的局限性，在保持视觉保真度的同时实现了可靠的多模态水印，为可靠性保护的多模态水印建立了新标准。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型由于大型语言模型的强先验和跨模态注意力不对齐，经常产生与视觉内容不一致的输出（幻觉），需要有效解决方案

Method: Scalpel在推理时预测Transformer层中每个头的可信注意力方向，使用高斯混合模型捕捉信任和幻觉流形中的多峰注意力分布，通过熵最优传输（相当于薛定谔桥问题）精确映射高斯分量，基于分量隶属度和映射关系动态调整干预强度和方向

Result: 在多个数据集和基准测试上的广泛实验表明，Scalpel有效减轻了幻觉，优于先前方法并达到最先进性能，且具有模型和数据无关性

Conclusion: Scalpel通过优化注意力激活分布有效减少大型视觉语言模型的幻觉，无需额外计算，仅需单次解码步骤，为模型幻觉问题提供了高效解决方案
...
TL;DR: 本文提出了一种基于神经正切核的多模态谱聚类方法，利用预训练的视觉-语言模型中的跨模态对齐，通过正名词锚定来构建图像间的亲和力矩阵，在16个基准数据集上显著优于现有方法。
...
Method: 提出神经正切核谱聚类，利用预训练视觉-语言模型中的跨模态对齐。通过使用与目标图像语义接近的正名词来锚定神经正切核，将图像间的亲和力定义为视觉邻近性和语义重叠的耦合。此外，提出了正则化亲和力扩散机制，自适应地集成不同提示诱导的亲和力矩阵。

Result: 在16个基准数据集（包括经典、大规模、细粒度和域偏移数据集）上的广泛实验表明，该方法始终以较大优势优于现有最先进方法。

Conclusion: 该方法成功地将谱聚类扩展到多模态领域，通过利用视觉-语言预训练模型中的跨模态对齐，显著提升了聚类性能，证明了多模态信息在谱聚类中的重要性。
...
### [37] [GenSeg-R1: RL-Driven Vision-Language Grounding for Fine-Grained Referring Segmentation](https://arxiv.org/abs/2602.09701)
...
<details>
  <summary>Details</summary>
Motivation: 解决细粒度参考图像分割问题，传统方法通常需要监督推理链标注，成本高昂。本文旨在开发无需监督推理链标注的高效分割框架。

Method: 采用解耦的"推理-分割"流程：1) 使用Qwen3-VL模型（4B/8B参数）接收图像和自然语言查询，生成结构化空间提示（边界框+两个内部关键点）；2) 使用冻结的SAM 2分割器将提示转换为高质量掩码。使用Group Relative Policy Optimization (GRPO)进行微调，无需监督推理链标注。
...
Conclusion: 提出的GenSeg-R1框架通过解耦的推理-分割流程，在无需监督推理链标注的情况下，实现了高质量的细粒度参考图像分割，在多个基准测试中显著优于现有方法。
...
TL;DR: Hand2World：基于单张场景图像的自我中心交互生成框架，通过3D手部网格投影和相机几何注入解决手势交互视频合成的分布偏移、运动歧义和长序列生成问题。


<details>
  <summary>Details</summary>
Motivation: 自我中心交互世界模型对于增强现实和具身AI至关重要，需要低延迟、几何一致性和长期稳定性的视觉生成。现有方法面临三个核心挑战：自由空间手势与接触式训练数据的分布偏移、单目视图中手部运动与相机运动的歧义性，以及任意长度视频生成的需求。
...
Conclusion: Hand2World通过创新的遮挡不变条件化和相机几何注入方法，有效解决了自我中心交互生成的关键挑战，为增强现实和具身AI应用提供了高质量的交互视频合成能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散视频生成方法多为任务特定且主要依赖文本指令，难以处理多模态输入、上下文参考以及多样化的视频生成与编辑场景。许多视频编辑方法依赖针对单个操作定制的工程化流程，限制了可扩展性和可组合性。
...
TL;DR: LELA：首个基于大语言模型的免训练仇恨视频定位框架，通过多模态分解和多阶段提示方案实现细粒度时间定位，在多个基准测试中显著优于现有免训练基线。
...
Result: 在HateMM和MultiHateClip两个具有挑战性的基准测试中，LELA大幅优于所有现有的免训练基线方法。

Conclusion: LELA为可扩展和可解释的仇恨视频定位建立了强大基础，通过免训练方式实现了细粒度的仇恨内容检测和时间定位。
...
TL;DR: 该论文提出了VIDA视频数据集和VideoAfford模型，用于解决3D可操作性定位问题，通过视频数据提供动态交互上下文，显著提升了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D可操作性定位研究主要依赖静态线索（如语言和图像），缺乏动态交互上下文，难以提供揭示时间和因果线索的充分信息。为了解决这一问题，需要收集包含人类-物体交互视频的全面数据集。

Method: 1. 收集VIDA数据集：包含38K人类-物体交互视频，涵盖16种可操作性类型、38个物体类别和22K点云；2. 提出VideoAfford基线模型：激活多模态大语言模型，增加可操作性分割能力；3. 使用潜在动作编码器从HOI视频中提取动态交互先验；4. 引入空间感知损失函数以获得全面的3D空间知识。

Result: 广泛的实验评估表明，该模型显著优于现有成熟方法，并表现出强大的开放世界泛化能力和可操作性推理能力。

Conclusion: 通过VIDA数据集和VideoAfford模型，成功解决了3D可操作性定位中动态交互上下文不足的问题，为机器人操作提供了更准确的可操作性区域识别能力。所有数据集和代码将公开发布以推动该领域研究。
...
### [42] [SARS: A Novel Face and Body Shape and Appearance Aware 3D Reconstruction System extends Morphable Models](https://arxiv.org/abs/2602.09918)
*Gulraiz Khan,Kenneth Y. Wertheim,Kevin Pimbblet,Waqas Ahmed*

Main category: cs.CV

TL;DR: 本文提出了一种名为SARS的形状和外观感知的3D重建系统，能够从单张图像中提取身体和面部信息，重建完整人体的3D模型，解决了传统3DMM忽略面部语义特征的问题。
...
Result: SARS系统能够从单张图像中准确重建完整人体的3D模型，不仅包含全局结构信息，还能捕捉面部语义特征如年龄、性别、面部标志点等细节特征。

Conclusion: 通过引入形状和外观感知的3D重建系统，本文解决了传统3DMM忽略面部语义特征的问题，实现了从单张图像中更全面、更准确的重建完整人体3D模型，为3D人体重建提供了更精细的解决方案。
...
TL;DR: Time2General是一个域广义视频语义分割框架，通过时空记忆解码器和掩码时序一致性损失来解决跨域部署时的闪烁问题，在保持时序一致性的同时提升跨域性能。


<details>
  <summary>Details</summary>
Motivation: 在域广义视频语义分割中，域偏移和时序采样偏移会破坏基于对应关系的传播和固定步长的时序聚合，导致视频流中即使标签稳定区域也会出现严重的帧间闪烁问题。

Method: 提出Time2General框架，包含时空记忆解码器（聚合多帧上下文到片段级时空记忆，无需显式对应关系传播）和掩码时序一致性损失（正则化不同步长下的时序预测差异，随机化训练步长以暴露模型于多样的时序间隔）。

Result: 在多个驾驶基准测试上的广泛实验表明，Time2General在跨域准确性和时序稳定性方面相比之前的DGSS和VSS基线有显著提升，同时运行速度可达18 FPS。

Conclusion: Time2General通过稳定性查询和时序一致性正则化，有效解决了域广义视频语义分割中的闪烁问题，实现了在未见域上部署时的高性能和时序一致性预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI自动化研究主要关注GUI基础而非更关键的GUI规划，GUI规划需要更复杂的数据收集。计算机使用智能体在应用/桌面/网页中的探索过程通常遵循树结构，早期功能入口点被更频繁探索，因此将大规模轨迹组织成树结构可以降低数据成本并简化GUI规划的数据扩展。

Method: 提出TreeCUA方法：1) 多智能体协作框架探索环境、验证动作、总结轨迹、评估质量；2) 树状拓扑结构存储和回放重复探索节点；3) 自适应探索算法平衡深度（轨迹难度）和广度（轨迹多样性）；4) 世界知识指导和全局内存回溯避免低质量生成；5) 基于树节点信息扩展提出TreeCUA-DPO方法，通过参考相邻轨迹的分支信息提升GUI规划能力。

Result: 实验结果表明TreeCUA和TreeCUA-DPO带来显著改进，域外研究进一步展示了强大的泛化能力。所有轨迹节点信息和代码将在GitHub上公开。

Conclusion: TreeCUA通过树结构可验证演化有效扩展GUI自动化，提出的多智能体框架和树状拓扑结构能够高效生成高质量GUI轨迹，TreeCUA-DPO方法进一步提升了GUI规划能力，在泛化性方面表现出色。
...
### [45] [Monocular Normal Estimation via Shading Sequence Estimation](https://arxiv.org/abs/2602.09929)
*Zongrui Li,Xinhua Ma,Minghui Hu,Yunqing Zhao,Yingchen Yu,Qian Zheng,Chang Liu,Xudong Jiang,Song Bai*

Main category: cs.CV

TL;DR: RoSE将单目法线估计重新定义为着色序列估计，利用图像到视频生成模型预测着色序列，然后通过最小二乘法转换为法线图，解决了现有方法中的3D不对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有单目法线估计方法虽然能预测出看起来正确的法线图，但重建的表面往往无法与几何细节对齐（3D不对齐问题）。作者认为这是因为模型难以区分和重建法线图中表示的不同几何形状，因为底层几何的差异仅通过相对细微的颜色变化来反映。

Method: 提出新范式：将法线估计重新定义为着色序列估计，因为着色序列对各种几何信息更敏感。基于此提出RoSE方法：1）利用图像到视频生成模型预测着色序列；2）通过求解普通最小二乘问题将着色序列转换为法线图；3）在包含多样形状、材质和光照条件的合成数据集MultiShade上训练以增强鲁棒性。

Result: 实验表明，RoSE在真实世界基准数据集上实现了物体级单目法线估计的最先进性能。

Conclusion: 通过将法线估计重新定义为着色序列估计，RoSE解决了现有方法中的3D不对齐问题，在真实世界数据集上取得了优越性能，证明了新范式的有效性。
...
### [46] [Semi-supervised Liver Segmentation and Patch-based Fibrosis Staging with Registration-aided Multi-parametric MRI](https://arxiv.org/abs/2602.09686)
*Boya Wang,Ruizhe Li,Chao Chen,Xin Chen*

Main category: cs.CV

TL;DR: 本文提出了一种用于肝脏分割和肝纤维化分期的多任务深度学习框架，通过半监督学习处理多参数MRI数据，解决标注数据有限和模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 肝纤维化在临床实践中是一个重大挑战，需要精确的肝脏分割和准确的疾病分期。基于CARE Liver 2025 Track 4挑战赛，本研究旨在开发一个能够处理多参数MRI数据、有限标注和域偏移问题的深度学习框架。

Method: 采用多任务深度学习框架，包含两个阶段：1) LiSeg阶段使用半监督学习模型，结合图像分割和配准，利用标注和未标注数据处理多模态MRI；2) LiFS阶段采用基于patch的方法，基于分类输出可视化肝纤维化分期。

Result: 该方法在包含分布内和分布外病例的独立测试集上进行了评估，使用三通道MRI（T1、T2、DWI）和七通道MRI（T1、T2、DWI、GED1-GED4）。代码已开源。
...
### [47] [Unbalanced optimal transport for robust longitudinal lesion evolution with registration-aware and appearance-guided priors](https://arxiv.org/abs/2602.09933)
*Melika Qahqaie,Dominik Neumann,Tobias Heimann,Andreas Maier,Veronika A. Zimmer*

Main category: cs.CV

TL;DR: 提出基于非平衡最优传输的配准感知匹配器，用于解决癌症患者纵向CT扫描中病灶演化追踪的挑战，能够处理病灶出现、消失、合并和分裂等情况。
...
Method: 提出基于非平衡最优传输的配准感知匹配器，能够适应不等的病灶质量并根据患者级别肿瘤负荷变化调整先验。传输成本融合了：(1)尺寸归一化的几何信息，(2)变形场雅可比矩阵的局部配准可信度，(3)可选的块级外观一致性。通过相对剪枝对传输计划进行稀疏化，无需重新训练或启发式规则即可获得一对一匹配以及新出现、消失、合并和分裂的病灶。

Result: 在纵向CT数据上，该方法相比仅基于距离的基线方法，在边缘检测精度和召回率、病灶状态召回率以及病灶图组件F1分数方面均取得了一致更高的性能。

Conclusion: 提出的非平衡最优传输配准感知匹配器能够有效解决纵向CT扫描中病灶演化追踪的挑战，为癌症治疗反应评估提供了更可靠的病灶对应关系建立方法。
...
### [48] [Bladder Vessel Segmentation using a Hybrid Attention-Convolution Framework](https://arxiv.org/abs/2602.09949)
*Franziska Krauß,Matthias Ege,Zoltan Lovasz,Albrecht Bartz-Schmidt,Igor Tsaur,Oliver Sawodny,Carina Veil*

Main category: cs.CV

TL;DR: 提出了一种用于膀胱癌监测的混合注意力-卷积架构，通过结合Transformer和CNN来分割膀胱内血管，解决内窥镜数据中的变形、伪影和黏膜褶皱等挑战。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌监测需要在重复干预中跟踪肿瘤位置，但膀胱的可变形和中空特性缺乏稳定的解剖标志。虽然内窥镜下可见的血管提供了患者特定的"血管指纹"用于导航，但自动分割面临诸多挑战：稀疏标签、气泡/光照变化等伪影、连续变形以及模仿血管的黏膜褶皱。现有血管分割方法难以解决这些领域特定的复杂性。

Method: 引入混合注意力-卷积架构：1) 使用Transformer捕获全局血管拓扑先验；2) 使用CNN学习残差细化图以精确恢复细血管细节；3) 为优先考虑结构连通性，在优化的地面真值数据上训练Transformer，排除短分支和末端分支；4) 为解决数据稀缺问题，采用物理感知预训练策略，在未标记数据上使用临床基础的增强进行自监督学习。

Result: 在BlaVeS数据集（内窥镜视频帧）上评估，该方法实现了高准确率（0.94）和优于现有医学分割模型的精度（0.61）和clDice（0.66）。关键的是，该方法成功抑制了在膀胱充盈和排空过程中动态出现和消失的黏膜褶皱产生的假阳性。

Conclusion: 混合注意力-卷积架构提供了临床导航所需的可靠结构稳定性，能够有效应对膀胱内窥镜血管分割的独特挑战，为膀胱癌监测提供了实用的解决方案。
...
TL;DR: Stroke3D是一个从2D手绘草图和文本提示直接生成可动画的3D网格框架，通过两阶段流程实现可控骨架生成和增强网格合成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法难以生成可动画的几何体，而骨架绑定技术缺乏对骨架创建的细粒度结构控制。需要一种更直观的工作流程来创建可直接动画的3D内容。

Method: 采用两阶段流程：1) 可控骨架生成：使用Skeletal Graph VAE编码骨架图结构，Skeletal Graph DiT生成骨架嵌入，结合文本语义和2D草图控制；2) 增强网格合成：通过TextuRig数据集增强现有骨架到网格模型，并使用SKA-DPO偏好优化策略提升几何保真度。
...
### [50] [Allure of Craquelure: A Variational-Generative Approach to Crack Detection in Paintings](https://arxiv.org/abs/2602.09730)
*Laura Paul,Holger Rauhut,Martin Burger,Samira Kabri,Tim Roith*

Main category: cs.CV

TL;DR: 提出了一种混合方法，将裂纹检测建模为逆问题，通过深度生成模型和变分函数分解图像为无裂纹绘画和裂纹组件


<details>
  <summary>Details</summary>
Motivation: 数字绘画中的裂纹自动检测对评估退化和指导修复至关重要，但由于复杂场景以及裂纹与笔触、头发等类似艺术特征之间的视觉相似性，这一任务仍然具有挑战性

Method: 采用混合方法，将裂纹检测建模为逆问题，将观测图像分解为无裂纹绘画和裂纹组件。使用深度生成模型作为底层艺术品的强大先验，同时使用Mumford-Shah型变分函数和裂纹先验捕捉裂纹结构。通过联合优化获得像素级的裂纹定位图

Result: 该方法能够有效区分裂纹与类似的艺术特征，生成精确的像素级裂纹定位图，支持艺术品的文档记录和修复工作

Conclusion: 提出的混合方法结合了深度生成模型和变分方法，为艺术品裂纹检测提供了一种有效的解决方案，有助于艺术品的保护和修复
...
### [51] [Fake-HR1: Rethinking reasoning of vision language model for synthetic image detection](https://arxiv.org/abs/2602.10042)
...
Result: 实验结果显示Fake-HR1能够根据不同查询类型自适应执行推理，在推理能力和生成检测性能上均超越现有LLMs，同时显著提升响应效率

Conclusion: 该研究提出的自适应混合推理框架有效解决了生成检测任务中冗余推理的资源开销问题，为高效AI生成内容检测提供了新思路
...
### [52] [Robust Vision Systems for Connected and Autonomous Vehicles: Security Challenges and Attack Vectors](https://arxiv.org/abs/2602.09740)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车的安全导航依赖于鲁棒的视觉系统，但目前对视觉系统安全性的全面分析不足，需要识别潜在攻击面以保障系统安全。

Method: 通过分析CAV导航所需的关键传感器和视觉组件，推导出CAV视觉系统参考架构，基于此识别攻击面，详细阐述针对每个攻击面的攻击向量，并评估其对CIA三要素的影响。

Result: 建立了CAV视觉系统参考架构，识别了潜在攻击面，详细分析了针对这些攻击面的攻击向量，并评估了它们对机密性、完整性和可用性的具体影响。

Conclusion: 研究提供了对视觉系统攻击向量动态的全面理解，这对于制定能够维护CIA三原则的鲁棒安全措施至关重要，有助于实现L5级自动驾驶能力。
...
### [53] [Causality in Video Diffusers is Separable from Denoising](https://arxiv.org/abs/2602.10095)
*Xingjian Bai,Guande He,Zhengqi Li,Eli Shechtman,Xun Huang,Zongze Wu*

Main category: cs.CV

TL;DR: 本文提出可分离因果扩散模型（SCD），将因果推理与多步去噪过程解耦，通过因果Transformer编码器处理每帧一次的时间推理，轻量扩散解码器处理帧内渲染，显著提升吞吐量和延迟同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有因果扩散模型将时间推理与迭代去噪过程纠缠在一起，在每一层、每个去噪步骤和整个上下文中都应用因果注意力，导致计算冗余。研究发现早期层特征在去噪步骤间高度相似，深层注意力稀疏且主要进行帧内渲染，这启发了将因果推理与去噪过程分离的设计。

Method: 提出可分离因果扩散模型（SCD），包含两个组件：1）因果Transformer编码器，每帧执行一次时间推理；2）轻量扩散解码器，负责多步帧内渲染。这种架构明确解耦了时间推理和渲染过程，减少了计算冗余。

Result: 在合成和真实基准测试的预训练和后训练任务中，SCD显著提高了吞吐量和每帧延迟，同时匹配或超越了强因果扩散基线的生成质量。

Conclusion: 因果推理可以与多步去噪过程有效分离，SCD架构通过解耦时间推理和帧内渲染，在保持生成质量的同时大幅提升了计算效率，为视频、语言和机器人轨迹等复杂生成过程提供了更高效的解决方案。
...
### [54] [Where Do Images Come From? Analyzing Captions to Geographically Profile Datasets](https://arxiv.org/abs/2602.09775)
*Abhipsa Basu,Yugam Bahl,Kirti Bhagat,Preethi Seshadri,R. Venkatesh Babu,Danish Pruthi*

Main category: cs.CV

TL;DR: 研究发现文本到图像模型的训练数据存在严重的地理代表性偏差，美国和西方国家占据主导，而南美和非洲国家严重不足，且数据量与GDP高度相关。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型经常无法生成具有地理代表性的图像，这引发了对其训练数据代表性的担忧，需要探究这些训练样本来自世界哪些地区。

Method: 使用LLM从图像标题中提取位置信息，将图像-标题对映射到特定国家，分析三个广泛使用的多模态数据集（Re-LAION、DataComp1B、Conceptual Captions）中20个常见实体的地理分布。

Result: 美国、英国和加拿大占样本的48.0%，而南美和非洲国家分别仅占1.8%和3.8%。国家GDP与数据代表性呈强相关（ρ=0.82）。非英语子集也偏向于该语言主要使用国。高代表性并不一定带来更大的视觉或语义多样性，且Stable Diffusion生成的图像虽然逼真，但覆盖范围远小于真实世界图像。

Conclusion: 当前文本到图像模型的训练数据存在严重的地理偏见，偏向富裕的西方国家，这限制了模型生成全球代表性图像的能力，需要更平衡的数据收集策略。
...
### [55] [Olaf-World: Orienting Latent Actions for Video World Modeling](https://arxiv.org/abs/2602.10104)
*Yuxin Jiang,Yuchao Gu,Ivor W. Tsang,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 提出SeqΔ-REPA方法，通过序列级控制效果对齐目标，从未标记视频中学习可跨场景转移的潜在动作表示，构建动作条件视频世界模型。


<details>
  <summary>Details</summary>
Motivation: 现有动作可控世界模型的扩展受限于动作标签稀缺。虽然潜在动作学习可以从无标签视频中提取控制接口，但学习的潜在表示往往无法跨场景转移：它们会纠缠场景特定线索，缺乏共享坐标系。这是因为标准目标仅在每个视频片段内操作，没有提供跨场景对齐动作语义的机制。

Method: 提出SeqΔ-REPA（序列级控制效果对齐目标），将集成潜在动作锚定到冻结的自监督视频编码器的时序特征差异上。基于此构建Olaf-World流水线，从大规模被动视频中预训练动作条件视频世界模型。

Result: 实验表明，该方法学习到更具结构化的潜在动作空间，在零样本动作转移和新控制接口的数据高效适应方面优于现有基线方法。

Conclusion: 通过利用动作的语义效果作为共享参考，SeqΔ-REPA能够从未标记视频中学习可跨场景转移的潜在动作表示，为动作可控世界模型的扩展提供了有效解决方案。
...
TL;DR: SciFlow-Bench是一个用于评估科学图表生成的结构优先基准，通过将生成的图像逆解析为结构化图来评估模型的结构正确性，而非仅视觉相似性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型生成的科学图表在视觉上合理但结构不正确，而现有基准要么使用图像中心或主观指标对结构不敏感，要么评估中间符号表示而非最终渲染图像，导致基于像素的图表生成评估不足。

Method: 从真实科学PDF构建SciFlow-Bench基准，将每个源框架图与规范地面真值图配对，采用闭环往返协议将生成的图表图像逆解析回结构化图进行比较，通过分层多智能体系统协调规划、感知和结构推理。

Result: 实验表明，保持结构正确性仍然是一个基本挑战，特别是对于具有复杂拓扑结构的图表，这突显了结构感知评估的必要性。

Conclusion: SciFlow-Bench通过结构可恢复性而非仅视觉相似性来评估科学图表生成，揭示了当前模型在结构正确性方面的不足，强调了结构感知评估的重要性。
...
TL;DR: CompSplat：一种针对压缩视频的压缩感知训练框架，通过显式建模帧级压缩特征来解决长视频序列中的相机位姿漂移、特征错位和几何失真问题，在严重压缩条件下实现最先进的渲染质量和位姿精度。


<details>
  <summary>Details</summary>
Motivation: 现实世界视频通常包含长序列、不规则相机轨迹和未知位姿，导致重建过程中的位姿漂移、特征错位和几何失真。有损压缩会加剧这些问题，引入不一致性并逐渐降低几何和渲染质量。现有方法要么专注于长序列NVS，要么专注于无位姿重建，而压缩感知方法仍局限于特定伪影或有限场景，未能充分探索长视频中的多样化压缩模式。

Method: 提出CompSplat压缩感知训练框架，显式建模帧级压缩特征以减轻帧间不一致性和累积几何误差。该框架包含压缩感知帧加权和自适应剪枝策略，增强鲁棒性和几何一致性，特别是在严重压缩条件下。

Result: 在Tanks and Temples、Free和Hike等具有挑战性的基准测试上进行广泛实验，证明CompSplat在严重压缩条件下实现了最先进的渲染质量和位姿精度，显著超越了大多数最新的NVS方法。

Conclusion: CompSplat通过显式建模压缩特征有效解决了长视频序列中的压缩相关问题，为文化遗产保护、数字孪生和沉浸式媒体等应用提供了高质量的压缩感知新视角合成解决方案。
...
TL;DR: 该论文针对大型视觉语言模型中的幻觉问题，提出了一种基于稳定性感知的知识增强解码方法（SAKED），通过量化模型内部知识稳定性来抑制幻觉生成。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型中的幻觉问题在实际应用中存在严重的安全性和可靠性风险。受人类在不确定或犹豫时更容易出错的观察启发，研究模型内部知识的不稳定性如何导致幻觉产生。

Method: 从注意力头、模型层和解码标记三个角度进行实证分析，识别出三种关键幻觉模式。基于这些发现提出稳定性感知知识增强解码（SAKED），引入层间知识稳定性评分（KSS）来量化模型内部知识稳定性，通过对比最稳定感知层和最稳定无关层来抑制解码噪声。

Result: SAKED在各种模型、任务和基准测试中实现了最先进的幻觉缓解性能，且无需训练即可无缝集成到不同架构中。

Conclusion: 通过量化模型内部知识稳定性并动态利用最可靠的内部知识，可以有效缓解大型视觉语言模型中的幻觉问题，提高生成内容的忠实度。
...
TL;DR: ARK是一个新的多模态检索基准，专注于专业知识和复杂推理评估，涵盖5个知识领域和6种推理技能，包含16种视觉数据类型，旨在填补现有基准在专业知识和复杂推理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索基准主要关注日常生活图像的语义匹配，缺乏对专业知识和复杂推理的诊断能力。为了填补这一空白，需要创建一个能够从知识领域和推理技能两个互补角度分析多模态检索的基准。

Method: 引入ARK基准，从两个维度设计：(1)知识领域：5个领域17个子类型，表征检索依赖的内容和专业知识；(2)推理技能：6个类别，表征识别正确候选所需的多模态证据推理类型。评估包括单模态和多模态查询与候选，涵盖16种异构视觉数据类型，大多数查询配有针对性困难负样本以避免捷径匹配。

Result: 评估了23个代表性文本基和多模态检索器，发现知识密集型检索和推理密集型检索之间存在显著差距，细粒度视觉和空间推理成为持续瓶颈。简单的增强方法如重排序和重写能带来一致改进，但仍有很大提升空间。

Conclusion: ARK基准填补了多模态检索评估在专业知识和复杂推理方面的空白，揭示了当前检索系统在知识密集型和推理密集型任务上的性能差距，为未来研究提供了重要诊断工具和改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数视觉语言模型采用混合接口（离散文本标记+连续ViT特征），由于监督主要基于文本，这些模型偏向理解而非生成，且无法充分利用非文本数据的大规模自监督学习。现有离散视觉标记因编码容量有限导致信息丢失，理解能力明显弱于连续特征模型。

Method: 提出Kelix模型，采用完全离散的自回归统一建模方法，通过改进的视觉离散化技术，创建跨模态的共享离散表示，使模型能够在统一的自监督框架下同时处理理解和生成任务。

Result: Kelix成功缩小了离散视觉表示与连续特征视觉语言模型之间的理解差距，实现了在统一自监督框架下的多模态理解和生成能力。

Conclusion: 完全离散的自回归统一建模是可行的，通过改进视觉离散化方法可以克服现有离散视觉标记的信息丢失问题，为构建统一的多模态理解-生成模型提供了有效途径。
...
TL;DR: Reason-IAD是一个用于工业异常检测的知识引导动态潜在推理框架，通过检索增强知识模块和熵驱动潜在推理机制，提升对细粒度缺陷模式的检测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于通用领域数据预训练的多模态大语言模型难以捕捉特定类别的异常模式，限制了工业异常检测的准确性和可解释性。

Method: 包含三个核心组件：1) 检索增强知识模块，将类别特定的文本描述融入模型输入；2) 熵驱动潜在推理机制，在紧凑潜在空间中使用可优化的潜在思考令牌进行迭代探索；3) 动态视觉注入策略，选择性融入信息量最大的图像块到潜在序列中。

Result: 在广泛实验中，Reason-IAD持续优于最先进的方法，展示了在工业异常检测任务上的优越性能。

Conclusion: Reason-IAD通过知识引导的动态潜在推理框架，有效解决了工业异常检测中细粒度缺陷模式识别和可解释性不足的问题，为工业视觉检测提供了新的解决方案。
...
TL;DR: Free-GVC是一种无需训练的视频压缩框架，通过将视频编码重新定义为由视频扩散先验引导的潜在轨迹压缩，在超低比特率下实现更好的感知质量和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式视频压缩方法在时间相关性利用上有限，导致超低比特率下出现明显闪烁和时间一致性退化，需要改进。

Method: 提出训练免费的生成式视频压缩框架，在GOP级别操作，将视频片段编码到紧凑潜在空间，沿扩散轨迹逐步压缩。包含自适应质量控制模块和组间对齐模块。

Result: 相比最新神经编解码器DCVC-RT，在DISTS指标上平均减少93.29% BD-Rate，用户研究证实其在超低比特率下具有优越的感知质量和时间一致性。

Conclusion: Free-GVC通过潜在轨迹压缩和创新的质量控制与对齐模块，有效解决了超低比特率下的闪烁和时间一致性问题，显著提升了生成式视频压缩性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于世界模型的想象-行动范式通常只支持纯图像预测或部分3D几何推理，限制了预测完整4D场景动态的能力。需要一种能够生成几何一致、任意视角RGBD的4D世界模型。

Method: 1) 提出具身4D世界模型，从单视角RGBD观测生成多视角RGBD序列，通过反投影和融合构建更完整的3D结构；2) 设计跨视角和跨模态特征融合机制，确保RGB与深度一致性及视角间几何对齐；3) 采用测试时动作优化策略，通过生成模型反向传播推断轨迹级潜在变量；4) 使用残差逆动力学模型将轨迹先验转化为可执行动作。

Result: 在三个数据集上的实验表明，该方法在4D场景生成和下游操作任务上都表现出色，消融实验为关键设计选择提供了实用见解。

Conclusion: 该方法通过几何一致的4D世界建模和创新的动作推断机制，有效解决了机器人操作中的场景预测和动作生成问题，为基于世界模型的想象-行动范式提供了更完整的解决方案。
...
TL;DR: LASANA数据集包含1270个腹腔镜手术训练任务的立体视频记录，带有技能评分和错误标签，用于支持基于视频的自动手术技能评估深度学习模型开发。
...
Method: 创建LASANA数据集，包含1270个立体视频记录，涵盖四个基本腹腔镜训练任务，每个记录都有结构化技能评分（三位独立评分者）和任务特定错误标签，提供预定义数据分割。

Result: 数据集反映了参与者技能的自然变化，提供了基准数据分割，并给出了深度学习模型的基线结果作为未来比较参考。

Conclusion: LASANA数据集填补了腹腔镜手术技能评估数据集的空白，为基于视频的技能评估和错误识别方法提供了基准测试平台。
...
TL;DR: GeoFormer是一个基于Swin Transformer的开源框架，仅使用Sentinel-1/2卫星影像和开放DEM数据，在100米网格上联合估计建筑高度和足迹，在54个城市中表现优于CNN基线模型。
...
Method: 提出GeoFormer开源框架，使用Swin Transformer架构，结合Sentinel-1/2光学和SAR影像以及开放DEM数据，采用地理分块策略确保训练集和测试集的空间独立性，在100米网格上联合估计建筑高度和足迹。

Result: 在54个多样化城市评估中，GeoFormer实现建筑高度RMSE 3.19米和建筑足迹RMSE 0.05，比最强的CNN基线分别提升7.5%和15.3%，在跨大陆迁移中保持建筑高度RMSE低于3.5米。消融研究证实DEM对高度估计不可或缺，光学反射率优于SAR，但多源融合获得最佳精度。

Conclusion: GeoFormer为三维城市数据生成提供了开源解决方案，仅使用开放数据源即可实现高精度估计，所有代码、权重和全球产品均已公开发布。
...
TL;DR: 该研究提出了一种自动化监测德国烘焙产品剩余量的方法，使用有限监督训练目标检测模型，在标注数据稀缺的情况下实现了高性能检测。


<details>
  <summary>Details</summary>
Motivation: 监测烘焙产品剩余量对德国面包店优化生产很重要，因为新鲜烘焙产品保质期短。自动化监测可以降低人工成本、提高准确性并简化运营，但德国烘焙产品种类繁多导致完全监督训练成本过高且可扩展性有限。

Method: 提出两种有限监督训练工作流程：1) 结合OWLv2和Grounding DINO定位与图像级监督进行弱监督训练；2) 使用Segment Anything 2作为伪标签传播模型在视频帧上微调以提高视角鲁棒性。使用YOLOv11进行检测任务。

Result: 仅使用图像级监督时模型达到0.91 mAP；使用伪标签微调后，在非理想部署条件下性能提升19.3%；结合两种工作流程训练的模型在非理想部署条件下超越了完全监督基线模型。

Conclusion: 该研究展示了在标注数据稀缺的专业工业场景中，通过有限监督方法训练高性能计算机视觉模型的可行性，为解决工业部署中的专业任务和标注数据稀缺问题提供了有效方案。
...
TL;DR: 该研究比较了两种全玻片图像染色自动分类方法：多实例学习（MIL）管道和轻量级缩略图方法，用于16种常见组织病理学染色分类，缩略图方法在泛化性和处理速度上表现更优。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中多种特殊染色对特定形态学诊断至关重要，但染色玻片的准确元数据管理对临床档案质量控制和计算病理学数据集完整性至关重要，需要自动化的染色分类方法。

Method: 比较两种全玻片图像染色分类方法：1）多实例学习（MIL）管道；2）提出的轻量级缩略图方法，涵盖14种常用特殊染色以及标准和冰冻切片H&E染色，共16个类别。

Result: 内部测试数据中MIL表现最佳（16类macro F1: 0.941；14类合并后: 0.969），缩略图方法保持竞争力（分别为0.897和0.953）。外部TCGA数据中缩略图模型泛化能力更强（加权F1: 0.843 vs. MIL的0.807），处理速度提高两个数量级（5.635 vs. 0.018 slides/s）。

Conclusion: 缩略图分类方法为数字病理工作流程中的常规视觉质量控制提供了可扩展且稳健的解决方案，在保持竞争力的分类性能同时显著提高了处理效率。
...
TL;DR: Faster-GS：一个优化的3D高斯泼溅系统，通过整合现有有效策略并引入新优化，实现了5倍训练加速且保持视觉质量，为3DGS优化建立了新的高效基线。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅研究存在两个问题：1）实现层面的改进与算法根本修改混杂在一起；2）许多方法在性能与保真度之间做出权衡，导致研究碎片化，难以进行公平比较。需要整合最有效的策略并探索框架中被忽视的方面。

Method: 1）整合和评估先前3DGS研究中最有效且广泛适用的策略；2）引入几种新颖的优化方法；3）深入研究框架中被忽视的方面，包括数值稳定性、高斯截断和梯度近似。

Result: Faster-GS实现了高达5倍的训练加速，同时保持视觉质量。在全面的基准测试套件中表现出色，为3DGS优化建立了新的成本效益和资源效率基线。优化方法还可应用于4D高斯重建，实现高效的非刚性场景优化。

Conclusion: Faster-GS通过系统性的优化整合，提供了一个经过严格优化的算法，显著提升了3D高斯泼溅的训练效率，同时保持了重建质量，为后续研究建立了新的高效基准。
...
TL;DR: 研究表明，即使经过颅骨剥离的脑部MRI图像仍包含独特的生物特征，可以通过简单的图像相似度计算在不同数据库间匹配同一参与者的扫描，存在隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 当前医疗数据共享框架要求移除潜在标识符，但即使颅骨剥离后的脑实质仍包含独特特征，可能在不同数据库间匹配同一参与者，存在隐私风险。现有研究多依赖训练或计算密集型方法，需要更简单有效的评估方法。

Method: 使用标准预处理后计算图像相似度的方法，在不同时间间隔、扫描仪类型、空间分辨率和采集协议下，匹配同一参与者的颅骨剥离T1加权MRI图像。

Result: 实现了近乎完美的匹配准确率，能够跨不同数据库成功识别同一参与者的MRI扫描，即使存在潜在的认知衰退等变化。

Conclusion: 简单的图像相似度计算方法就能有效匹配同一参与者的颅骨剥离MRI，这表明当前医疗数据共享框架可能存在隐私保护不足，需要制定更前瞻性的政策。
...
TL;DR: 本文提出了一种用于实例分割的保形预测算法，能够为像素坐标查询生成自适应置信集，提供至少一个预测与真实实例掩码具有高IoU的概率保证。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割模型在平均预测上表现良好，但缺乏原则性的不确定性量化：它们的输出未经校准，且无法保证预测掩码接近真实值。为解决这一限制，需要一种能够提供概率保证的置信集生成方法。
...
Conclusion: 该保形预测算法为实例分割提供了具有渐进和有限样本保证的自适应置信集，实现了可靠的不确定性量化，优于现有基线方法。
...
TL;DR: Forensim是一个基于注意力机制的图像伪造检测框架，能够同时定位被篡改区域和源区域，支持拼接和复制-移动伪造检测，在标准基准测试中达到最先进性能。
...
Method: 提出基于视觉状态空间模型的注意力框架，利用归一化注意力图识别内部相似性，配合区域块注意力模块区分篡改区域，实现端到端训练和精确定位。

Result: 在标准基准测试中达到最先进性能，并发布了新的数据集CMFD-Anything，解决了现有复制-移动伪造数据集的局限性。

Conclusion: Forensim通过联合源-目标定位的注意力框架，在图像伪造检测方面取得了显著进展，特别是在理解完整伪造上下文方面具有优势。
...
Result: 在具有挑战性的真实世界手工制作任务中，相比先前的视频生成和潜在动态模型，任务成功率提升高达70%，并能生成连贯的长执行视频。在机器人领域，能从Open-X数据集获取有效的操作知识，显著提升CALVIN上的任务性能。
...
TL;DR: ConsID-Gen是一个图像到视频生成框架，通过辅助视图增强和双流编码器解决现有方法中的外观漂移和几何失真问题，在ConsIDVid-Bench基准上表现优于主流视频生成模型。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成方法在保持对象身份一致性方面存在挑战，特别是在视角变化时容易出现外观漂移和几何失真。这些问题源于单视图2D观测的稀疏性和弱跨模态对齐。

Method: 1) 构建ConsIDVid数据集和ConsIDVid-Bench评估基准；2) 提出ConsID-Gen框架：使用未姿态辅助视图增强第一帧，通过双流视觉-几何编码器和文本-视觉连接器融合语义与结构线索，为Diffusion Transformer骨干提供统一条件。

Result: 在ConsIDVid-Bench上的实验表明，ConsID-Gen在多个指标上持续优于现有方法，整体性能超过Wan2.1和HunyuanVideo等领先视频生成模型，在真实场景中提供更好的身份保真度和时间一致性。

Conclusion: ConsID-Gen通过数据增强和模型架构创新，有效解决了图像到视频生成中的身份一致性保持问题，为多视角一致性视频生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统多旋转平均方法（如L1-IRLS和Shonan）存在局部极小值敏感性和依赖凸松弛的问题，无法精确保持流形几何结构，在高噪声场景下精度受限

Method: IQARS将多旋转平均问题重新表述为一系列局部二次非凸子问题，通过二值化处理后可在量子退火器上执行，利用量子隧穿和并行性进行高效解空间探索

Result: 在合成和真实数据集上的评估显示，IQARS在D-Wave量子退火器上相比最佳经典方法Shonan实现了约12%的精度提升

Conclusion: IQARS消除了对凸松弛的依赖，更好地保持了旋转流形的非欧几何结构，虽然当前量子退火器仍处于初级阶段且只能解决有限规模问题，但已显示出超越经典方法的潜力
...
<details>
  <summary>Details</summary>
Motivation: 推动计算文体学和文本取证领域的发展，通过客观、可复现的评估方法来解决日益复杂的文本分析挑战，特别是在生成式AI快速发展的背景下。

Method: 采用基于Docker容器的软件提交方式，通过TIRA实验平台进行标准化评估。包含五个具体任务：1) Voight-Kampff生成AI检测（混合和混淆场景）；2) 文本水印方案评估；3) 多作者写作风格分析；4) 生成式抄袭检测；5) 推理轨迹检测。

Result: 自2012年以来，PAN已通过TIRA平台接收超过1,100份软件提交，建立了成熟的评估框架。2026年新增文本水印和推理轨迹检测两个新任务，延续了多作者风格分析和生成式抄袭检测任务。

Conclusion: PAN 2026继续推进计算文体学和文本取证的可复现评估，通过新增任务应对生成式AI带来的新挑战，同时保持传统任务的连续性，为研究社区提供标准化的评估平台。
...
Result: 包容性分析能够揭示参与模式、关系动态和想法采纳模式，这些在聚合或事后评估中是不可见的。框架在模拟对话和人机协作实验中展示了有效性。
...
TL;DR: 提出基于朴素贝叶斯和注意力机制的话题连续性模型，用于评估LLM聊天机器人回复是否保持话题一致性，可处理任意长度对话且具有线性时间复杂度。


<details>
  <summary>Details</summary>
Motivation: LLM作为聊天机器人在商业场景中常出现话题突然转换的问题，导致用户体验差和计算资源浪费，需要一种评估话题连续性的方法。
...
Result: 在SOPBench的七个领域（银行、DMV、医疗、市场、大学、图书馆、酒店）上，FM SO.P的32B模型达到48.3%通过率，开源7B模型达到34.3%，与Qwen-2.5-72B-Instruct基线（34.4%）相当但参数少10倍。

Conclusion: FM SO.P通过渐进式能力构建和自适应评估系统，显著提升了语言模型对标准操作程序的理解和跨领域泛化能力。
...
TL;DR: 通过对Reddit社区r/AIDangers和r/ChatbotAddiction在2023-2025年帖子的计算主题分析，识别出AI相关心理风险的14个主题类别和5个高阶体验维度，揭示了用户自我调节困难是最普遍的问题，恐惧主要集中在自主权、控制和技术风险方面。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统日益融入日常生活，但关于AI使用相关心理风险如何产生、被体验以及被用户调节的实证理解仍然有限。研究旨在从真实用户话语中理解AI安全如何被感知和情感体验，填补实验室或推测性情境外的实证空白。
...
Result: 识别出14个反复出现的主题类别，并综合为5个高阶体验维度。自我调节困难是最普遍的维度，恐惧主要集中在自主权、控制和技术风险相关问题上。情感分析揭示了不同维度的情感特征模式。

Conclusion: 研究提供了来自真实用户生活体验的早期实证证据，展示了AI安全在实验室或推测性情境外如何被感知和情感体验。结果为未来AI安全研究、评估和负责任治理奠定了基础，强调了用户自我调节困难作为核心问题的重要性。
...
TL;DR: 研究评估大语言模型捕捉西班牙语地理词汇变异的能力，发现模型对不同地区变体的识别存在系统性差异，数字资源数量并非主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 西班牙语存在显著的区域变异，研究旨在评估大语言模型是否能够准确捕捉这些地理词汇差异，探讨数字语言偏见问题。

Method: 将大语言模型视为虚拟信息提供者，使用专家策划的西班牙语词汇变异数据库，通过是非题和选择题两种调查式问题格式，评估21个西班牙语国家超过900个词汇项目。

Result: 模型对西班牙、赤道几内亚、墨西哥和中美洲、拉普拉塔河地区的词汇变异识别更准确，而智利变体特别难以区分；国家层面数字资源数量差异不能解释这些性能模式。

Conclusion: 大语言模型在西班牙语方言知识方面存在系统性差异，数据量以外的因素影响方言表征，为数字语言偏见讨论提供了新证据。
...
TL;DR: 提出完全无监督的跨语言词性标注框架，仅使用单语语料，通过无监督神经机器翻译构建伪平行句对，结合多源投影技术提升标注性能。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏词性标注数据，现有方法依赖平行语料，但许多低资源语言没有平行语料。需要开发仅使用单语语料的解决方案。

Method: 使用无监督神经机器翻译系统将高资源语言句子翻译成低资源语言，构建伪平行句对。基于词对齐进行词性标注投影，并提出多源投影技术校准目标语言的标注结果。

Result: 在28个语言对上评估，涵盖4种源语言和7种目标语言。方法性能与使用平行句对的基线相当，某些语言甚至超过基线。多源投影技术带来平均1.3%的性能提升。

Conclusion: 提出的完全无监督跨语言词性标注框架有效解决了低资源语言缺乏平行语料的问题，多源投影技术进一步提升了标注性能，为低资源语言处理提供了可行方案。
...
TL;DR: AfriNLLB是一系列轻量级模型，支持15种语言对（30个翻译方向）的高效翻译，主要面向非洲语言。通过层剪枝和量化压缩NLLB-200 600M模型，并使用知识蒸馏技术进行微调，旨在资源受限环境中实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 该工作的动机是解决非洲语言翻译模型在资源受限环境中的部署问题。当前大型翻译模型计算需求高，难以在资源有限的非洲地区有效使用，因此需要开发轻量级但性能相当的翻译模型。

Method: 基于NLLB-200 600M模型，采用迭代层剪枝和量化技术进行压缩。在精心整理的非洲语言平行语料库上进行微调，并使用知识蒸馏技术从更大的教师模型学习。最终发布两个版本：支持进一步微调的Transformers版本和用于高效推理的CTranslate2版本。

Result: AfriNLLB模型在保持与基线模型相当性能的同时，实现了显著的速度提升。模型支持15种语言对（30个翻译方向），包括斯瓦希里语、豪萨语、约鲁巴语、阿姆哈拉语、索马里语、祖鲁语、林加拉语、南非荷兰语、沃洛夫语、埃及阿拉伯语等非洲语言，以及阿拉伯语、法语、葡萄牙语、西班牙语等非盟官方语言。

Conclusion: AfriNLLB成功开发了轻量级非洲语言翻译模型，在资源受限环境中实现了高效部署。通过发布模型和训练数据，促进了非洲语言翻译研究的进一步发展，为资源受限环境下的多语言翻译提供了实用解决方案。
...
TL;DR: BiasScope是一个LLM驱动的框架，用于自动大规模发现模型评估中可能存在的潜在偏见，并基于此构建了更挑战性的评测基准JudgeBench-Pro。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-Judge在研究和实际应用中广泛采用，但其评估的鲁棒性和可靠性存在关键问题。现有研究主要关注已知偏见及其对评估结果的影响，缺乏对潜在未知偏见的自动化和系统性探索，而这种探索对于提升评估的鲁棒性和可靠性至关重要。

Method: 提出BiasScope框架，这是一个LLM驱动的框架，能够自动且大规模地发现模型评估过程中可能出现的潜在偏见。该框架克服了现有方法的局限性，将偏见发现从依赖人工努力和预定义偏见列表的被动过程转变为主动、全面的自动化探索。

Result: BiasScope能够发现不同模型家族和规模的潜在偏见，其通用性和有效性在JudgeBench数据集上得到验证。基于BiasScope构建了JudgeBench-Pro，这是一个扩展版本且更具挑战性的基准，用于评估LLM-as-a-judge的鲁棒性。令人惊讶的是，即使是强大的LLM作为评估者，在JudgeBench-Pro上的错误率也超过50%。

Conclusion: 该研究强调了加强评估鲁棒性和进一步减轻潜在偏见的紧迫需求。BiasScope框架将偏见发现从被动过程转变为主动自动化探索，为提升LLM评估的可靠性提供了重要工具。
...
TL;DR: 本文批判性地分析了LLMs在合同起草中的局限性，认为其无法真正理解法律含义和上下文，生成的合同可能表面合理但实际无用或不适用。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在文本生成方面表现出色，但人们常常错误地认为它们可以协助法律任务如合同起草。本文旨在纠正这种不合理的观念，指出LLMs缺乏对词语意义、上下文和推理的真正理解。

Method: 通过理论分析，对比LLMs的统计性文本生成能力与法律实践中需要的语言运用、情境理解和法律推理之间的本质差异。

Result: LLMs只能生成通用且表面合理的合同文件，但这些文件可能是条款不一致的无用组合，或者是可执行但不适合特定交易的合同。预测词语与在法律情境中使用语言有根本区别。

Conclusion: LLMs并不威胁法律行业的持续生存能力，简单的假设需要被质疑。法律工作需要真正的理解、上下文意识和推理能力，而不仅仅是统计模式匹配。
...
TL;DR: 该论文提出了一种扩展多语言预训练语言模型到新低资源语言的方法，通过词汇扩展和双语词典初始化来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言预训练语言模型(mPLMs)虽然对许多低资源语言有益，但很少有工作研究如何将mPLMs扩展到之前不支持的低资源语言。需要解决如何有效扩展模型词汇表并保持源语言性能的问题。

Method: 1. 使用目标语言语料库扩展模型词汇表；2. 筛选出偏向源语言(如英语)的原始词汇子集；3. 利用双语词典初始化扩展词汇的表示；4. 基于扩展词汇表示继续使用目标语言语料库预训练mPLMs。

Result: 实验结果显示，与使用随机初始化扩展词汇的基线方法相比，该方法在词性标注和命名实体识别任务上分别提升了0.54%和2.60%。方法对训练语料选择具有高鲁棒性，且持续预训练后模型在源语言上的性能不会下降。

Conclusion: 提出的方法能有效将mPLMs扩展到新的低资源语言，通过词汇扩展和双语词典初始化策略显著提升模型性能，同时保持源语言能力不退化。
...
TL;DR: 研究发现大语言模型在道德判断中存在类似人类的"情境主义"认知偏差，无关的道德干扰因素能显著改变LLMs的道德判断


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在高风险环境中的广泛应用，确保其行为符合人类价值观变得日益重要。现有道德基准假设LLMs报告相对稳定的道德偏好，但人类道德心理学研究表明人类道德判断会受到无关情境因素的影响。研究旨在评估LLMs是否表现出类似人类的认知道德偏差。

Method: 从现有心理学数据集中收集60个"道德干扰因素"（情感化图像和叙事），这些干扰因素与所呈现情境无道德相关性。将这些干扰因素注入现有道德基准中，测量它们对LLM响应的影响。

Result: 研究发现道德干扰因素可以使LLMs的道德判断发生超过30%的偏移，即使在低模糊性情境中也是如此。这表明LLMs的道德判断具有显著的情境敏感性。

Conclusion: 研究结果强调了需要进行更多情境化的道德评估，以及对LLMs进行更细致的认知道德建模。LLMs表现出类似人类的"情境主义"道德认知偏差，这对确保AI系统道德对齐具有重要意义。
...
TL;DR: 研究发现检索增强生成（RAG）架构能够减少大型语言模型的社会偏见，而思维链（CoT）提示虽然提高准确性却会增加偏见


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在社会偏见问题，检索增强生成架构虽然通过检索外部知识增强生成能力，但仍面临同样的偏见挑战。本研究旨在评估和理解RAG的社会偏见影响。
...
Conclusion: RAG通过多样化模型的上下文基础可以改善公平性，但需要开发能够减轻准确性-偏见权衡的偏见感知推理框架，因为CoT虽然提高准确性却会增加偏见。
...
TL;DR: 提出Conceptual Cultural Index (CCI)来衡量句子层面的文化特异性，通过比较目标文化与其他文化的普遍性估计差异来量化文化特异性，在二元分类任务中优于直接LLM评分。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地部署在多文化环境中，但句子层面的文化特异性系统评估仍然不足，需要一种可操作且可解释的方法来衡量文化特异性。

Method: 提出概念文化指数(CCI)，定义为目标文化内的普遍性估计与其他文化平均普遍性估计之间的差异。用户可以通过比较设置操作性地控制文化范围，分数源自底层普遍性估计，提供可解释性。

Result: 在400个句子（200个文化特定句和200个通用句）上验证CCI，得分分布显示预期模式：文化特定句得分更高，通用句得分更低。在二元可分性方面，CCI优于直接LLM评分，对目标文化专门化模型的AUC提高超过10个百分点。

Conclusion: CCI提供了一种可操作、可解释的方法来量化句子层面的文化特异性，在多文化环境中评估和部署LLMs具有实用价值，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 从非结构化电子健康记录中提取药物使用信息是临床自然语言处理的主要挑战。虽然大型语言模型有进步，但在临床NLP中的应用受到信任、控制和效率问题的限制。ToxHabits任务针对西班牙临床文本中的有毒物质使用检测，这是一个领域特定、低资源的场景。

Method: 提出多输出集成系统处理ToxNER和ToxUse两个子任务。系统集成BETO模型与CRF层进行序列标注，采用多样化训练策略，并使用句子过滤技术来提高精度。

Result: 最佳运行结果：触发检测达到0.94 F1分数和0.97精度，参数检测达到0.91 F1分数。

Conclusion: NOWJ系统在西班牙临床文本的有毒物质使用检测任务中表现出色，证明了集成BETO与CRF层以及多样化训练策略的有效性，为低资源临床NLP任务提供了实用解决方案。
...
TL;DR: CoCoA解码器通过分析LLM中间层的表征不稳定性来检测和减少幻觉，无需训练即可在推理时提升事实准确性
...
Method: 提出CoCoA解码器，一种无需训练的推理时解码算法。通过量化中间层的表征不稳定性（提出两个指标），对表现出高内部混淆的输出进行惩罚，从而引导模型产生更内部一致和事实基础的输出。还提出了自信息门控变体CoCoA-SIG，动态调节惩罚以针对高惊喜、不稳定的生成。
...
TL;DR: 本文提出Gt-Margin方法，通过真实标记的概率差值确定最佳解掩码顺序，并训练监督解掩码规划器来提升掩码扩散语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在推理时需要同时决定解掩码位置和标记内容，但现有方法通常使用启发式置信度或昂贵的强化学习来确定解掩码顺序，缺乏有效且高效的解决方案。

Method: 提出Gt-Margin评分方法，基于真实标记与其最强替代标记的概率差值构建oracle解掩码顺序；然后通过排序学习训练监督解掩码规划器来模仿这种顺序。

Result: 使用oracle解掩码顺序显著提升了最终生成质量，特别是在逻辑推理基准上；训练的解掩码规划器在不修改标记预测模型的情况下提高了推理准确性。

Conclusion: Gt-Margin提供了一种有效的方法来确定掩码扩散语言模型的解掩码顺序，通过监督学习训练的规划器能够提升模型推理能力，为MDLM的推理优化提供了新思路。
...
TL;DR: EcoGym是一个用于评估LLM智能体长期规划能力的通用基准，包含三个经济环境，通过1000+步的连续决策过程测试模型在部分可观测和随机性下的战略一致性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估框架存在局限性：大多是情景式的、领域特定的，或者缺乏持久经济动态的基础。需要一种能够评估长期规划和执行决策能力的通用基准。

Method: 开发了EcoGym基准，包含三个多样化经济环境：Vending、Freelance和Operation，采用统一的决策过程接口，在1000+步的无界时间范围内进行预算化行动评估。

Result: 在11个领先LLM上的实验显示，没有单一模型在所有三个场景中都表现最优。模型在高层战略或高效行动执行方面存在显著次优性，暴露了系统性的紧张关系。

Conclusion: EcoGym作为一个开放、可扩展的测试平台，为透明评估长期规划智能体和研究现实经济设置中的可控性-效用权衡提供了工具，揭示了当前LLM在长期经济决策中的局限性。
...
TL;DR: 论文提出SAKE方法解决LLM在长推理链中的知识整合衰减问题，通过在推理过程首尾锚定检索知识来提升知识利用稳定性。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型通过搜索增强推理来整合外部知识，但存在知识整合衰减问题：随着推理链增长，模型越来越难以将检索到的证据整合到后续推理步骤中，即使相关信息可用也会限制性能。

Method: 提出SAKE（Self-Anchored Knowledge Encoding）方法，这是一种无需训练、在推理时使用的策略。通过在推理过程的开头和结尾同时锚定检索到的知识，防止知识被先前上下文所掩盖，从而保持其语义完整性。

Result: 在多跳问答和复杂推理基准测试上的广泛实验表明，SAKE显著缓解了知识整合衰减问题并提高了性能。

Conclusion: SAKE为智能体LLM中的知识整合提供了一个轻量级但有效的解决方案，通过稳定知识利用来提升长推理链中的表现。
...
TL;DR: 本文提出MoSLoRA方法用于训练统一自回归奖励模型(UniARM)，解决多目标对齐中特征纠缠和偏好权衡控制问题，实现单参数空间建模所有偏好维度。


<details>
  <summary>Details</summary>
Motivation: 现有多目标对齐方法存在两个问题：1) 为每个偏好目标独立训练ARM，忽略了偏好特征间的交互；2) 使用分离的特征提取模块训练单一ARM，导致特征纠缠。这两种策略都会使生成输出与用户偏好错位。

Method: 提出Preference-Modulated & Shared Low-Rank Adaptation (MoSLoRA)：首先通过偏好无关模块提取共享特征，然后通过基于混合偏好向量的偏好调制模块对共享特征进行仿射变换。在此基础上构建Unified Autoregressive Reward Model (UniARM)，在单一参数空间中联合建模所有偏好维度。

Result: MoSLoRA设计缓解了特征纠缠问题，并在推理过程中实现了对偏好权衡的精确控制。UniARM框架消除了为每个偏好目标设置独立参数的需求，并在更大规模LLM上展示了实用性。

Conclusion: 提出的MoSLoRA方法和UniARM框架有效解决了多目标对齐中的特征纠缠问题，通过共享特征提取和偏好调制实现了更好的偏好控制，为多目标测试时对齐提供了更实用的解决方案。
...
TL;DR: 该论文系统比较了多轮对话问答中的检索增强生成方法，发现简单方法（如重排序、混合BM25、HyDE）优于复杂方法，且检索效果受数据集特性和对话长度影响显著。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多孤立评估RAG方法，且主要关注单轮设置，缺乏对多轮对话问答中RAG方法的系统比较。多轮对话中的历史对话、指代消解和用户意图变化使检索更加复杂。

Method: 在八个不同领域的对话问答数据集上，使用统一的实验设置，比较了基础和高级RAG方法。评估了检索质量和答案生成，使用生成器和检索指标，并分析了性能随对话轮次的变化。

Result: 结果显示，鲁棒但简单的方法（如重排序、混合BM25、HyDE）始终优于基础RAG。而一些高级技术未能带来增益，甚至可能使性能低于无RAG基线。数据集特性和对话长度强烈影响检索效果。

Conclusion: 有效的对话RAG更依赖于检索策略与数据集结构的对齐，而不是方法的复杂性。没有单一RAG策略在所有设置中占主导地位。
...
TL;DR: 本文提出了一种用于块扩散语言模型（BDLMs）的测试时缩放统一框架，通过自适应解码和块级生成优化推理效率与效果平衡，在复杂推理任务中实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有块扩散语言模型在测试时缩放方面探索有限，在长链思维推理中面临解码速度与效果平衡的严重挑战，需要更有效的自适应机制来优化推理过程。

Method: 提出统一框架包含：1）有界自适应置信度解码（BACD）-基于模型置信度动态调整去噪的难度感知采样策略；2）粗思考细批判（TCCF）-分配大块尺寸用于探索性推理、小块尺寸用于细化的测试时缩放范式；3）渐进块尺寸扩展-缓解大块尺寸缩放时的性能下降。

Result: 在TDAR-8B模型上应用BACD和TCCF，相比TraDo-8B等强基线实现2.26倍加速，在AIME24上提升11.2分，显著提高了推理效率和效果。

Conclusion: 该框架为块扩散语言模型在复杂推理任务中的测试时缩放潜力开发迈出了重要一步，有效解决了解码速度与效果平衡的挑战。
...
TL;DR: 本文介绍了LEMUR，一个基于欧盟环境法规的大规模多语言法律语料库，用于解决LLM在多语言法律检索中的不可靠性和缺乏领域适应嵌入模型的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律信息获取中的应用日益增多，但在多语言法律环境中的部署受到不可靠检索和缺乏领域适应开放嵌入模型的限制。现有多语言法律语料库不适合语义检索，PDF立法源因文本提取不完善引入大量噪声。

Method: 1. 构建LEMUR语料库：从24,953份欧盟官方EUR-Lex PDF文档中提取，涵盖25种语言；2. 使用词汇内容评分量化PDF到文本转换的保真度；3. 基于LEMUR，使用对比目标在单语和双语设置下微调三种最先进的多语言嵌入模型。

Result: 实验表明，法律领域微调相对于强基线持续提高Top-k检索准确率，对低资源语言的提升尤为显著。跨语言评估显示这些改进可迁移到未见语言，表明微调主要增强了语言无关的内容级法律表示而非语言特定线索。

Conclusion: LEMUR语料库和微调的多语言嵌入模型有效解决了多语言法律检索的挑战，特别是对低资源语言有显著提升，且改进具有跨语言迁移能力，表明增强了语言无关的法律内容表示。
...
TL;DR: BG-MCTS是一种预算引导的树搜索解码算法，通过根据剩余token预算动态调整搜索策略来优化LLM解码效果


<details>
  <summary>Details</summary>
Motivation: 现有树搜索解码方法对预算不敏感，将其仅作为终止条件，导致后期过度分支或过早终止，无法适应实际部署中不同查询的固定token预算限制

Method: 提出预算引导的MCTS算法，根据剩余token预算动态调整搜索策略：开始时进行广泛探索，随着预算减少优先细化和答案完成，减少浅层节点的后期分支

Result: 在不同预算条件下，BG-MCTS在MATH500和AIME24/25数据集上始终优于预算不敏感的树搜索基线方法，使用开源权重的LLM

Conclusion: 预算引导的树搜索解码策略能有效适应实际部署中的token预算限制，显著提升LLM在数学推理任务上的表现
...
<details>
  <summary>Details</summary>
Motivation: 传统反事实数据增强方法在减少语言模型偏见时，可能降低语言建模能力，因为生成的合成数据可能与真实分布不一致，或者忽略了敏感属性变化的社会语境。

Method: 提出Context-CDA方法：1) 使用大型语言模型增强反事实数据的多样性和语境相关性；2) 通过不确定性过滤排除目标小型语言模型认为低质量的生成反事实数据。

Result: 在性别偏见基准测试中，Context-CDA有效减轻了偏见，同时没有牺牲语言建模性能，并通过分析下一个标记生成概率的分布变化提供了对社交偏见的洞察。

Conclusion: Context-CDA通过上下文增强和不确定性过滤，在减少语言模型社会偏见的同时保持了语言建模能力，为解决偏见缓解与模型性能之间的权衡问题提供了有效方案。
...
TL;DR: 本文研究了强化学习对大型语言模型推理的影响，发现RL会延长思维链输出并增加计算成本。通过比较不同长度控制方法，发现长度惩罚可能阻碍推理获取，而适当调整的长度控制可以提高具有强先验推理模型的效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能显著提升大语言模型的推理能力，但会导致思维链输出变长，增加训练和推理的计算成本。现有长度控制方法的效果不明确，需要研究如何平衡效率与性能的最佳输出长度。

Method: 在两个模型（Qwen3-1.7B Base和DeepSeek-R1-Distill-Qwen-1.5B）上比较了多种长度控制方法，并将先前工作扩展到RL训练的策略中，识别了两种失败模式。

Result: 结果显示：1）长度惩罚可能阻碍推理获取；2）适当调整的长度控制可以提高具有强先验推理模型的效率；3）识别出两种失败模式：长输出增加分散性，短输出导致思考不足。

Conclusion: 长度控制方法需要谨慎调整，不当的长度惩罚会损害推理能力，而适当调整的长度控制可以在保持性能的同时提高效率。研究为平衡推理模型的效率与性能提供了指导。
...
TL;DR: ELPO（错误定位策略优化）通过二分搜索定位长轨迹中的首个不可恢复错误步骤，使用分层优势归因生成稳定学习信号，并通过自适应裁剪强化关键步骤的修正更新，在工具集成推理任务中显著优于现有智能体强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 在工具集成推理（TIR）中，仅基于结果的强化学习面临稀疏延迟奖励和弱步骤级信用分配问题。长轨迹中早期不可恢复错误决定成败，因此需要定位首个不可恢复步骤并进行细粒度信用分配。

Method: 提出ELPO方法：1）通过固定预算下的二分搜索展开树定位首个不可恢复步骤；2）通过分层优势归因将结果树转换为稳定学习信号；3）应用错误定位自适应裁剪，强化关键步骤及其后缀的修正更新。

Result: 在数学、科学QA和代码执行等TIR基准测试中，ELPO在可比采样预算下持续优于现有智能体强化学习方法，在Pass@K和Major@K扩展、展开排序质量和工具调用效率方面均有额外提升。

Conclusion: ELPO通过定位首个不可恢复错误步骤并针对性强化学习，有效解决了TIR中稀疏延迟奖励和弱信用分配问题，显著提升了智能体在长轨迹任务中的性能。
...
TL;DR: AlignTune是一个模块化工具包，为LLM后训练对齐提供统一接口，解决后端干扰、奖励碎片化和不可复现流水线等问题，支持SFT和RLHF优化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练对齐工作流分散在不同后端特定工具和临时粘合代码中，导致实验难以复现。主要障碍包括后端干扰、奖励碎片化和不可复现的流水线。

Method: 引入AlignTune模块化工具包，提供监督微调(SFT)和RLHF风格优化的统一接口，支持TRL和Unsloth后端互换。标准化配置，提供可扩展的奖励层（基于规则和学习），集成标准基准和自定义任务的评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM评估通常需要真实标注或协调多个评估者，这在实际应用中成本高且不灵活。需要开发一个无需参考标注、可扩展且与人类判断对齐的评估框架。

Method: 使用独立提示的评估者集合，基于人类对齐的模式指导评估过程，支持离散和连续评分。通过任务特定的提示（从最佳候选选择、摘要、图像描述到对话）实现灵活评估。

Result: 实验表明该框架与人类判断高度对齐，优于现有方法，同时减少了计算开销，提供了高效、鲁棒且人类对齐的LLM评估解决方案。

Conclusion: MILE-RefHumEval提供了一个无需参考标注、可扩展、可解释且与人类对齐的LLM评估框架，在实际应用中具有高效性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在表格理解任务上取得了显著进展，但在可靠性、可扩展性和效率方面仍面临挑战，特别是在资源受限或隐私敏感的环境中。

Method: MATA采用多智能体框架，利用多种互补推理路径和小语言模型构建的工具集。首先为给定表格和问题生成多样化的候选答案，然后通过这些工具进行优化或选择最佳答案。同时设计了算法来最小化昂贵的大语言模型调用。

Result: 在两个不同难度的基准测试中使用十种不同大语言模型进行广泛实验，MATA实现了最先进的准确性和高效的推理，同时避免了过度的大语言模型推理。

Conclusion: 精心编排多种推理路径能够实现可扩展和可靠的表格问答，MATA框架能够保持强大的性能，同时适应各种大语言模型类型，并显著提高效率。
...
TL;DR: 本文评估了知识蒸馏方法在机器翻译中的实际应用，综合考虑翻译质量和计算成本，使用碳足迹作为计算成本指标，发现蒸馏开销在小规模部署中占主导，大规模部署中推理占主导，词级蒸馏通常比序列级蒸馏提供更好的质量-足迹权衡。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏研究通常只报告学生模型的翻译质量，而忽略了蒸馏过程的计算复杂度，这使得在计算资源受限的情况下难以选择合适的蒸馏方法。本文旨在填补这一空白，综合考虑翻译质量和计算成本来评估不同的知识蒸馏方法。

Method: 使用机器学习生命周期评估工具将计算成本量化为碳足迹，该评估考虑了整个知识蒸馏模型生命周期中的运行时操作排放和摊销的硬件生产成本，包括教师训练、蒸馏和推理三个阶段。评估了代表性的知识蒸馏方法，特别是比较了词级蒸馏和序列级蒸馏。

Result: 研究发现：(1) 在小规模部署中，蒸馏开销占总碳足迹的主导地位；(2) 在大规模部署中，推理开销占主导，使得知识蒸馏只有在超过特定任务相关的使用阈值后才具有优势；(3) 词级蒸馏通常比序列级蒸馏提供更有利的碳足迹-质量权衡。

Conclusion: 本文提出的评估协议为在明确的质量和计算约束下选择知识蒸馏方法提供了可重复的指导，强调了在实际应用中需要综合考虑翻译质量和计算成本，而不仅仅是模型性能。
...
TL;DR: 使用LoRA微调、适配器合并和方言感知MBR解码来提升LLM在阿拉伯方言上的生成和翻译性能
...
TL;DR: TraceMem是一个受认知启发的记忆框架，通过构建结构化叙事记忆模式来增强大语言模型处理长期对话的能力，在LoCoMo基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型受限于有限的上下文窗口，难以管理随时间延伸的对话历史。现有的记忆系统通常将交互视为离散片段，无法捕捉对话流的底层叙事连贯性。

Method: 提出TraceMem框架，采用三阶段流水线：1) 短期记忆处理：使用演绎式主题分割方法划分情节边界并提取语义表示；2) 突触记忆巩固：将情节总结为情景记忆，然后与语义一起提炼为用户特定痕迹；3) 系统记忆巩固：使用两阶段分层聚类将这些痕迹组织成连贯的、随时间演变的叙事线索，封装到结构化用户记忆卡中形成叙事记忆模式。

Result: 在LoCoMo基准测试中，TraceMem实现了最先进的性能。分析表明，通过构建连贯的叙事，它在多跳推理和时间推理方面超越了基线方法，突显了其在深度叙事理解中的重要作用。

Conclusion: TraceMem通过认知启发的架构构建结构化叙事记忆模式，有效解决了大语言模型处理长期对话的瓶颈问题，在叙事理解和推理任务上表现出色，为记忆系统研究提供了新视角。
...
TL;DR: 研究人员评估了基于Gemini的AI协作科学评估系统，在气候科学领域测试了AI如何加速科学工作流程，特别是在AMOC稳定性这一复杂主题上。结果显示AI能显著提升效率，但专家监督和补充对确保科学严谨性至关重要。


<details>
  <summary>Details</summary>
Motivation: 当前AI co-scientists范式主要适用于可重复验证的任务，但无法扩展到那些无法重复评估、需要基于理论和现有证据达成共识的科学问题。本研究旨在探索AI如何支持需要专家共识的科学评估工作。

Method: 开发了基于Gemini的AI协作科学评估环境，整合到标准科学工作流程中。与13位气候科学家合作，在AMOC稳定性这一复杂主题上测试系统。通过104次修订周期，分析了AI在科学协作中的作用。

Result: AI显著加速了科学工作流程：团队在46人小时内完成了79篇论文的综合分析。AI生成的内容大部分被保留在报告中，帮助维持逻辑一致性和呈现质量。但专家补充至关重要：不到一半的报告由AI生成，需要大量监督来提升内容到严谨科学标准。

Conclusion: AI能够有效加速科学评估工作流程，但无法完全替代专家判断。成功的科学AI协作需要AI能力与专家监督的有机结合，特别是在需要理论-证据共识的复杂科学问题上。
...
<details>
  <summary>Details</summary>
Motivation: 现有语料库追求语言广度但缺乏深度，无法充分捕捉欧洲语言的丰富圣经翻译历史，需要专门资源来支持翻译历史的定量研究。

Method: 从12个在线圣经图书馆和一个现有语料库中收集数据，包含657个新约译本，其中352个独特版本。每个翻译都手动标注元数据，将文本映射到标准化的工作标识符、特定版本和修订年份。

Result: 创建了首个专门为灵活多层次分析设计的资源，包含英语208个独特版本（总计396个）、法语41个（78个）、意大利语18个（33个）、波兰语30个（48个）和西班牙语55个（102个）。

Conclusion: 该语料库通过标准化标识和元数据标注，使研究人员能够根据自身需求定义"独特性"，进行从微观翻译家族分析到宏观去重研究的灵活多层次分析，为翻译历史定量研究设立了新基准。
...
TL;DR: 提出一种将词嵌入映射到神经生物学特征空间的方法，以增强词汇语义变化的可解释性，不仅能更好地估计语义变化程度，还能发现被忽视的语义变化类型。


<details>
  <summary>Details</summary>
Motivation: 现有词汇语义变化研究主要关注变化程度的估计，但难以解释词义如何变化。增强可解释性对于该领域获得新见解至关重要。

Method: 将预训练语言模型获得的上下文词嵌入映射到神经生物学特征空间，该空间的每个维度对应词的原始特征，值表示特征强度，使人类能够系统解释语义变化。

Result: 在估计词汇语义变化程度方面优于大多数现有方法，且凭借高可解释性，发现了以往研究忽视的有趣语义变化类型，并能有效搜索特定类型的语义变化词。

Conclusion: 提出的方法不仅提高了词汇语义变化估计的性能，更重要的是增强了可解释性，为发现新的语义变化类型和搜索特定变化类型的词提供了有效工具。
...
TL;DR: 首个班巴拉语ASR标准化基准：使用1小时专业录制的马里宪法文本，评估37个模型，结果显示当前ASR性能远未达到部署标准，最佳WER为46.76%，最佳CER为13.00%，多语言模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 为班巴拉语（一种资源匮乏语言）建立首个标准化自动语音识别基准，以评估当前ASR模型的性能，揭示多语言预训练和模型扩展对低资源语言的局限性。

Method: 创建包含1小时专业录制马里宪法文本的标准化基准数据集，在接近最优的声学和语言条件下，评估37个模型（包括班巴拉语训练系统和大型商业模型），使用词错误率（WER）和字符错误率（CER）作为评估指标。

Result: 当前ASR性能远未达到部署标准：最佳WER为46.76%，最佳CER为13.00%，多个知名多语言模型的WER超过100%。这表明多语言预训练和模型扩展对班巴拉语等低资源语言效果有限。

Conclusion: 多语言预训练和模型扩展不足以支持低资源语言，班巴拉语ASR技术仍需重大改进。该基准数据集代表了最简化的正式口语形式，实际应用中的性能可能更差。提供公开基准和排行榜以促进透明评估和未来研究。
...
TL;DR: 论文提出了一种评估语言模型推理效率的框架，将token效率分解为可解释的因素：固定token预算下的完成度、完成后的条件正确性、以及冗余度，并进一步分析冗余度的组成。


<details>
  <summary>Details</summary>
Motivation: 现有评估只关注最终准确率，忽视了token使用效率，无法揭示token在推理过程中是如何被使用或浪费的。需要一种能够分解token效率的评估框架。

Method: 提出了一个可选的追踪框架，将token效率分解为：1) 固定token预算下的完成度；2) 完成后的条件正确性；3) 冗余度。当有基准测试元数据时，进一步将冗余度分解为平均表达开销和耦合系数。当有推理追踪时，添加确定性追踪质量度量（如基础性、重复性、提示复制）。

Result: 在CogniLoad上评估25个模型发现：准确率和token效率排名存在差异（Spearman ρ=0.63）；效率差距通常由条件正确性驱动；表达开销变化约9倍（与模型规模关系较弱）。分解揭示了不同的瓶颈特征，提示需要不同的效率干预措施。

Conclusion: 提出的框架能够深入分析语言模型的推理效率，揭示传统准确率评估无法发现的效率瓶颈，为模型优化提供了更精细的指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 科学计量问答作为科学论文问答的子类别，在规划阶段面临独特挑战，包括学术实体的命名实体识别和涉及影响因子等多维度数据检索。传统科学问答方法难以有效处理这些元科学问题。

Method: 采用端到端顺序工作流，结合检索增强生成和智能体概念。使用专有研究绩效评估平台作为检索数据库，通过任务分解、规划和推理能力处理科学计量问题，并有效合成数据为结构化高级分析。

Result: 通过咨询经验丰富的领域专家和利用LLMs作为评判者进行评估，为LLMs在特定下游任务中的有效性提供了有价值的见解。代码和提示已开源。

Conclusion: AnalyticsGPT展示了LLMs在处理科学计量问答这一利基下游任务中的潜力，特别是在处理元科学问题、学术实体识别和多维度数据检索方面的应用价值。
...
TL;DR: 阿拉伯语言模型主要在标准阿拉伯语上预训练，期望能迁移到方言，但方言与标准语的相似度差异导致迁移效果不均衡，地理邻近性部分解释了这种差异，且多方言训练存在负干扰。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语言模型主要在标准阿拉伯语上预训练，但阿拉伯地区存在多种方言，这些方言与标准阿拉伯语的相似度不同，这限制了语言模型向方言的迁移能力，需要研究跨方言迁移的效果和机制。

Method: 通过探针分析在3个自然语言处理任务上研究阿拉伯语言模型的跨方言迁移能力，并使用表征相似性分析方法，同时考察地理邻近性对迁移效果的影响。

Result: 研究发现跨方言迁移是可能的，但在不同方言间效果不均衡，地理邻近性部分解释了这种差异；同时发现支持所有阿拉伯方言的模型存在负干扰现象。

Conclusion: 阿拉伯方言与标准阿拉伯语的相似度存在差异，这质疑了它们之间的相似程度，并对阿拉伯语言模型的跨方言迁移能力提出了担忧。
...
TL;DR: 该研究探索利用大语言模型生成的推理来预测其自身预测的正确性，在教育对话分析中实现了83% F1分数的错误检测性能。
...
Method: 分析了30,300个教师话语，每个都由多个先进的大语言模型标注教学行为结构并附带推理。使用TF-IDF编码LLM推理，评估了五种监督分类器，并训练特定教学行为结构的专用检测器。使用LIWC框架分析正确与错误推理的语言标记。

Result: 随机森林分类器达到0.83的F1分数（召回率0.854），成功识别大多数错误预测。正确的预测表现出基于因果的语言，而错误推理更可能依赖认知性回避和表现性元认知。特定教学行为结构的专用检测器进一步提升了困难结构的性能。

Conclusion: 基于推理的错误检测为自动化教育对话分析提供了实用且可扩展的质量控制方法，表明模型自身的推理可以有效地用于预测其预测的正确性。
...
TL;DR: 研究普通话中自然语言产出的量化行为，通过图片描述任务分析说话者是否量化、量化精度及策略选择，发现物体数量、生命性和产出模态系统影响量化行为。


<details>
  <summary>Details</summary>
Motivation: 量化是日常语言使用的基本组成部分，但人们对说话者在自然产出中如何决定是否量化以及如何量化知之甚少。本研究旨在探究普通话中的量化行为，了解在无约束条件下说话者的量化决策过程。

Method: 采用基于图片的诱发描述任务，让说话者自由描述包含多个物体的场景，没有明确要求计数或量化。研究同时考察口语和书面语两种模态，分析三个方面的量化行为：是否量化、量化精度以及量化策略选择。

Result: 结果显示物体数量、生命性和产出模态系统性地影响量化行为。具体而言，物体数量增加会降低量化的可能性和精度；有生命指称和产出模态选择性地调节策略选择。

Conclusion: 本研究展示了如何在无约束产出条件下考察量化行为，为语言产出中的数量表达提供了自然主义数据集，有助于进一步分析量化表达的语言学特征。
...
TL;DR: 该研究构建了一个包含2,344个僧伽罗语修辞格的语料库，带有文化和跨语言标注，用于评估LLM在低资源语言文化表达翻译中的表现，发现现有模型存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在处理高资源语言的修辞表达时表现较好，但对于僧伽罗语等低资源语言，由于数据有限，在处理文化相关的修辞格时面临挑战。需要专门的语料库来支持低资源NLP研究和文化感知的机器翻译。

Method: 1. 构建包含2,344个僧伽罗语修辞格的语料库，带有文化和跨语言标注
2. 分析数据集以分类修辞格的文化起源并识别跨语言对应表达
3. 开发二元分类器区分数据集中的两种修辞格类型
4. 评估现有大型语言模型在该数据集上的表现

Result: 1. 二元分类器在区分两种修辞格类型时达到约92%的准确率
2. 评估发现现有LLM存在显著不足，这些模型经常难以准确传达习语含义
3. 数据集已公开，为未来低资源NLP研究和文化感知机器翻译提供了重要基准

Conclusion: 该研究为低资源语言和文化相关NLP任务提供了有价值的资源，揭示了当前LLM在处理文化特定修辞表达方面的局限性，强调了开发更具文化意识的机器翻译系统的重要性。
...
TL;DR: Steer2Edit：将推理时激活干预的语义方向转化为组件级权重编辑的诊断信号，通过选择性重分布行为影响来改善属性-效用权衡


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型引导方法通过在推理时应用固定的全局激活干预来影响模型行为，但这种方法忽略了行为通常由少量异质模型组件控制的事实，导致在强控制下产生不利的属性-效用权衡

Method: 提出Steer2Edit框架，将引导向量从推理时控制信号转化为诊断信号，用于组件级的秩-1权重编辑。该方法选择性重分布行为影响到单个注意力头和MLP神经元，产生可解释的编辑，保持标准前向传播并兼容优化并行推理

Result: 在安全对齐、幻觉缓解和推理效率方面，Steer2Edit实现了更有利的属性-效用权衡：在匹配下游性能时，安全性提升高达17.2%，真实性提高9.8%，推理长度平均减少12.2%
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型构建的多智能体系统在实现可扩展集体智能和自我进化时面临的安全挑战，特别是探讨系统能否在完全封闭循环中实现持续自我改进同时保持安全对齐。

Method: 采用信息论框架，将安全形式化为与人类价值分布的偏离程度，从理论上证明完全隔离的自我进化会导致统计盲点，并通过开放智能体社区和两个封闭自进化系统的实证研究验证理论预测。

Result: 理论和实证结果表明，满足持续自我进化、完全隔离和安全不变性的智能体社会是不可能的，隔离的自我进化会导致系统安全对齐的不可逆退化。

Conclusion: 研究确立了自进化AI社会的根本限制，需要从症状驱动的安全补丁转向对内在动态风险的原则性理解，强调外部监督或新型安全保护机制的必要性。
...
TL;DR: 论文研究如何利用LLM内部表征预测其自身在数学和编程任务上的成功率，从而指导更高效的推理计算，通过路由查询到不同模型可降低成本70%


<details>
  <summary>Details</summary>
Motivation: 运行LLM进行扩展推理成本高昂，但确定哪些输入真正需要额外计算仍然具有挑战性。研究探索是否可以从LLM生成前的内部表征中恢复其自身成功的可能性，并利用这一信号指导更高效的推理。

Method: 在生成前激活上训练线性探针，预测特定策略在数学和编程任务上的成功率；使用E2H-AMC数据集对比人类和模型在相同问题上的表现；利用探针结果在模型池中路由查询。

Result: 线性探针显著优于表面特征（如问题长度和TF-IDF）；模型编码了与人类难度不同的模型特定难度概念，且这种差异随扩展推理增加；通过路由查询可在MATH数据集上减少70%推理成本，同时超过最佳单模型性能。

Conclusion: LLM的内部表征包含关于其自身成功可能性的可恢复信号，即使这些信号与人类对难度的直觉不同，仍能实现实际的效率提升，为高效推理提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习和可验证奖励的大推理模型存在过度思考问题，生成冗余推理步骤而不提升性能。轨迹级长度惩罚方法通常无法有效缩短推理长度且会降低准确性，因为它们统一对待所有推理步骤，缺乏区分冗余与必要步骤的细粒度信号。过程监督方法通常资源密集且信用分配不准确。
...
Conclusion: ATTNPO通过利用模型内在注意力信号进行细粒度信用分配，有效解决了大推理模型的过度思考问题，在减少推理冗余的同时保持甚至提升准确性，且计算开销较低。
...
TL;DR: 本文提出了ViMultiChoice方法，专门用于越南语阅读理解，能够同时预测正确答案并生成解释，在ViMMRC 2.0基准和新数据集上达到SotA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多项选择阅读理解模型通常只能选择答案，缺乏解释其推理过程的能力。针对越南语阅读理解的解释生成能力训练和评估数据集也较为缺乏。

Method: 提出ViMultiChoice方法，专门为越南语阅读理解设计，联合训练选项决策和解释生成，同时预测正确答案并生成相应的解释。

Result: ViMultiChoice在ViMMRC 2.0基准和新引入的数据集上都超越了现有基线，达到了最先进的性能。联合训练选项决策和解释生成显著提高了多项选择的准确性。

Conclusion: 本文成功开发了越南语阅读理解解释生成数据集和ViMultiChoice方法，证明了联合训练答案选择和解释生成对提高模型性能的有效性，为越南语阅读理解研究提供了新的工具和方法。
...
TL;DR: ViSpeechFormer是首个显式建模音素表示的越南语ASR框架，利用越南语高字形-音素透明度，在公开数据集上表现优异，泛化能力强，训练偏差小。


<details>
  <summary>Details</summary>
Motivation: 越南语具有语音正字法特征，每个字形最多对应一个音素，反之亦然。利用这种高字形-音素透明度，研究者希望开发首个显式建模音素表示的越南语ASR框架。

Method: 提出ViSpeechFormer（越南语语音Transformer），这是一种基于音素的越南语自动语音识别方法。该方法利用越南语的高字形-音素透明度，显式建模音素表示。

Result: 在两个公开可用的越南语ASR数据集上的实验表明，ViSpeechFormer实现了强大的性能，对词汇外单词具有更好的泛化能力，受训练偏差的影响更小。
...
TL;DR: 该论文提出了一个多维度的无参考评估框架，用于评估大语言模型在自然灾害响应等高风险领域特定应用中的问答质量，强调现有评估方法无法满足领域敏感决策需求。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成和开放式问答评估框架主要依赖表面相似性、事实一致性或语义相关性，无法评估模型是否提供了领域敏感决策所需的具体信息，特别是在自然灾害响应和基础设施规划等高风险领域特定应用中。

Method: 提出了一个多维度的无参考评估框架，从四个互补维度评估LLM输出：特异性、对释义和语义扰动的鲁棒性、答案相关性、上下文利用。创建了包含1,412个领域特定问答对的数据集，涵盖40个专业角色和7种自然灾害类型，并进行人工评估以验证标注者间一致性和模型输出与人类判断的对齐。

Result: 研究结果显示，没有任何单一指标能够充分独立地捕捉答案质量，证明了在高风险应用中部署LLM时需要结构化、多指标评估框架的必要性。人工评估揭示了开放式领域特定评估固有的主观性。

Conclusion: 在高风险领域特定应用中部署大语言模型时，需要采用结构化、多维度的评估框架，而不是依赖单一指标，以全面评估模型输出的决策关键信息质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前将动态知识整合到大型语言模型面临挑战，因为事实数据和推理模式存在内在纠缠。现有解决方案如RAG和知识编辑受限于有限上下文窗口、检索器噪声或灾难性遗忘风险。

Method: 提出DRIFT双模型架构：1) 轻量级知识模型动态压缩文档块为隐式事实标记；2) 将这些密集表示投影到推理模型的嵌入空间，替代原始冗余文本；3) 保持推理准确性同时减少上下文负担。

Result: 大量实验表明，DRIFT在长上下文任务上显著提升性能，在同等规模模型中优于强基线方法，为扩展LLMs的有效上下文窗口和推理能力提供了可扩展且高效的范式。

Conclusion: DRIFT通过显式解耦知识提取与推理过程，提供了一种解决LLMs知识整合挑战的新方法，能够有效扩展上下文窗口并提升长文档处理能力。
...
<details>
  <summary>Details</summary>
Motivation: 验证声明的真实性通常需要文本和视觉证据的联合多模态推理，但现有工作主要关注文本证据或忽略可解释性，导致验证不准确且缺乏说服力。

Method: 1) 构建双层多模态图进行证据检索，设计图像到文本和文本到图像的推理；2) 提出标记级和证据级融合进行多模态声明验证；3) 引入多模态Fusion-in-Decoder进行解释生成；4) 创建AI领域的科学数据集AIChartClaim。

Result: 实验证明了该模型的有效性，展示了其在多模态声明验证和解释生成方面的优势。

Conclusion: 该研究提出了一个能够联合处理证据检索、多模态声明验证和解释生成的综合模型，填补了多模态可解释声明验证领域的空白，并通过创建AIChartClaim数据集丰富了该研究社区。
...
TL;DR: AnaBench是用于科学表格与图形分析的大规模基准测试，包含63,178个实例，覆盖9个科学领域和7个复杂度维度。Anagent是一个多智能体框架，通过四个专门化智能体（规划者、专家、求解者、批评者）来提升科学表格与图形分析能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学表格与图形分析方面存在困难，主要挑战包括科学图表的高度复杂性、异质性结构以及长上下文需求。需要量化这些挑战并开发有效的解决方案。

Method: 提出Anagent多智能体框架，包含四个专门化智能体：Planner（任务分解）、Expert（工具执行信息检索）、Solver（信息合成生成分析）、Critic（五维质量评估迭代优化）。采用监督微调和专门化强化学习的模块化训练策略。

Result: 在170个子领域的全面评估中，Anagent在无需训练设置下提升13.43%，经过微调后提升42.12%。研究表明面向任务的推理和上下文感知问题解决对高质量科学表格与图形分析至关重要。

Conclusion: Anagent框架通过多智能体协作和专门化训练策略，显著提升了科学表格与图形分析能力，为解决科学AI中的复杂多模态知识解释问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然语言模型已成为量子计算教育和研究的实用工具，但现有基准主要评估量子代码生成和电路设计，对量子计算概念理解的系统性测量尚属空白。
...
Result: 人类参与者得分在23%-86%之间，专家平均74%。最佳模型Claude Opus 4.5达到84%准确率，但顶级模型在专家编写问题上比LLM生成问题平均下降12个百分点。在高级主题（如安全）上表现进一步下降至73%。模型经常接受并强化错误前提而非识别它们，在这些关键推理任务上准确率低于66%。

Conclusion: 虽然顶级模型在量子计算概念理解上超越了专家平均水平，但在专家编写问题、高级主题和纠正错误前提方面仍存在显著差距，表明需要更全面的评估基准来提升模型的概念理解和推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前程序合成研究主要依赖大语言模型，存在分布问题难以界定、微调效果不明确、分词影响复杂、计算存储成本高等问题。需要一个小型、可控、透明的系统来深入研究程序完成、推理等基础问题。

Method: 开发了Cadmus系统，包含整数虚拟机、多样化真实程序数据集，以及基于自回归Transformer的模型。整个系统训练成本低于200美元，提供对训练分布和模型检查的细粒度控制。

Result: Cadmus模型在完成正确整数算术程序的任务上表现优于GPT-5（达到100%准确率，而GPT-5为95%）。更重要的是，系统提供了数据集与问题关系的透明度，而GPT-5在解决相同任务时引入了未知的先验知识，成为研究中的混淆因素。

Conclusion: 小型模型在复杂推理任务上能够实现大模型难以进行的仪器化和研究，为程序合成、分布外表示、归纳推理和指令跟随等研究提供了经济、可控、透明的实验平台。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体通常基于完整的动作-观察历史来决策，这会引入任务无关信息，导致冗余动作和更高的推理成本。
...
Result: 在AgentGym基准测试的八个环境中，使用相同的训练轨迹，PABU实现了81.0%的任务完成率，比使用完整历史信念的先前SOTA模型高出23.9%。此外，PABU的进度导向动作选择提高了效率，将平均交互步骤减少到9.5步，对应26.9%的减少。

Conclusion: 消融研究表明，显式进度预测和选择性保留对于稳健的信念学习和性能提升都是必要的。PABU框架通过紧凑的状态表示和进度感知的信念更新，有效解决了LLM智能体中历史信息冗余的问题。
...
TL;DR: CoMMa是一个去中心化的LLM智能体框架，通过博弈论目标和确定性嵌入投影实现贡献感知的信用分配，在肿瘤学决策支持任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体框架在处理需要基于动态、异质患者数据进行推理的肿瘤学决策支持任务时存在局限性，需要更稳健、可解释的决策方法。

Method: 提出CoMMa框架：1）去中心化LLM智能体，专家在分区证据上操作；2）通过博弈论目标进行协调；3）使用确定性嵌入投影近似贡献感知的信用分配；4）通过估计每个智能体的边际效用实现显式证据归因。

Result: 在多种肿瘤学基准测试（包括真实世界多学科肿瘤委员会数据集）上评估，CoMMa比数据集中化和基于角色的多智能体基线方法具有更高的准确性和更稳定的性能。

Conclusion: CoMMa通过贡献感知的信用分配机制，能够产生可解释且数学基础扎实的决策路径，在肿瘤学决策支持任务中实现了更稳健和准确的性能。
...
TL;DR: 本文提出了一种基于拓扑数据分析（TDA）和持久性景观（PLs）的多样性度量框架，能够捕捉数据集的几何结构特征，超越了传统基于分布变化或熵的多样性度量方法。


<details>
  <summary>Details</summary>
Motivation: 现有多样性度量方法（如特征空间离散度和度量空间幅度）主要关注分布变化或熵，而很大程度上忽略了数据集的几何结构。为了填补这一空白，需要一种能够捕捉数据集几何和结构丰富性的多样性度量框架。

Method: 基于拓扑数据分析和持久性景观构建多样性度量框架。通过TDA提取数据的几何特征，利用持久性景观量化这些特征，从而创建PLDiv度量指标，直接关联数据多样性与底层几何结构。

Result: 通过跨多种模态的广泛实验，证明提出的PLDiv度量方法具有强大、可靠和可解释的特点。该方法能够有效捕捉数据集的几何结构多样性，为数据集构建、增强和评估提供了基础工具。

Conclusion: 基于拓扑数据分析和持久性景观的多样性度量框架提供了一种理论上有依据的方法，能够超越熵的局限性，捕捉数据集的丰富几何和结构特性，为多样性分析提供了新的视角和工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统大多使用多数投票聚合智能体输出，这种方法丢弃了推理轨迹的证据结构，在"幻觉共识"情况下表现脆弱——当智能体共享相关偏见并收敛于相同错误推理时

Method: 1. AgentAuditor：构建推理树显式表示智能体轨迹间的共识与分歧，在关键分歧点比较推理分支，将全局裁决转化为高效的局部验证；2. ACPO（反共识偏好优化）：在多数投票失败案例上训练裁决器，奖励基于证据的少数选择而非流行错误

Result: 在5个流行设置中，AgentAuditor相比多数投票实现高达5%的绝对准确率提升，相比使用LLM-as-Judge提升高达3%

Conclusion: AgentAuditor通过显式建模推理分歧和局部验证，有效解决了多智能体系统中的幻觉共识问题，显著提升了推理准确性，且框架与具体MAS设置无关
...
TL;DR: AI在放射学图像重建和增强中应用迅速，虽然能提升图像质量、加快采集速度，但也引入了新的失败模式和图像感知质量与信息内容之间的脱节风险。


<details>
  <summary>Details</summary>
Motivation: AI在放射学中快速部署，虽然能显著改善图像质量和工作效率，但也带来了新的风险，特别是图像感知质量与实际信息内容之间的脱节问题，需要理解其局限性以确保安全有效使用。

Method: 本文是一篇综述性通讯，旨在提高对AI在放射学图像重建和增强中局限性的认识，通过分析现有应用和潜在风险，为临床实践提供指导。

Result: AI在放射学图像处理中确实能产生更清晰、更平滑、更详细的图像，加快采集速度，但同时也引入了新的失败模式，可能导致图像感知质量与实际信息内容不一致的风险。

Conclusion: 理解AI图像重建和增强的局限性对于安全有效使用该技术至关重要，用户需要在享受AI技术带来的益处的同时，最小化相关风险。
...
### [135] [SpotAgent: Grounding Visual Geo-localization in Large Vision-Language Models through Agentic Reasoning](https://arxiv.org/abs/2602.09463)
*Furong Jia,Ling Dai,Wenjin Deng,Fan Zhang,Chen Hu,Daxin Jiang,Yu Liu*

Main category: cs.AI

TL;DR: SpotAgent是一个用于地理定位的智能体框架，通过工具调用和验证来解决视觉线索稀疏、长尾分布和高度模糊的问题，实现可验证的地理定位结果。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在真实世界地理定位中存在局限性：视觉线索稀疏、长尾分布且高度模糊；模型受限于内部知识，经常产生自信但无根据的预测，缺乏可验证性。

Method: 1. 将地理定位形式化为智能体推理过程，结合视觉解释与工具辅助验证；2. 采用3阶段后训练流程：监督微调对齐、多智能体框架合成高质量轨迹的智能体冷启动、强化学习精炼推理能力；3. 提出空间感知动态过滤策略，基于空间难度优先选择可学习样本。

Result: 在标准基准测试中，SpotAgent实现了最先进的性能，有效缓解幻觉问题，同时提供精确且可验证的地理定位结果。

Conclusion: SpotAgent通过智能体推理框架和工具辅助验证，解决了地理定位中的视觉模糊性和不可验证性问题，为实际应用提供了可靠的地理定位解决方案。
...
TL;DR: XMCC是一个可解释的多模态思维链压缩器，通过强化学习将压缩建模为序列决策过程，有效缩短推理轨迹同时保持关键步骤和答案正确性，并提供压缩决策的自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 长思维链在多模态推理中广泛使用，但往往过于冗长且包含冗余推理步骤，这会降低推理效率。现有压缩方法面临两个主要挑战：可能损害视觉-文本推理的完整性，以及压缩过程缺乏可解释性。
...
Conclusion: XMCC解决了长思维链压缩中的关键问题，既能提高推理效率，又能保持推理完整性并提供可解释性，为多模态推理系统的优化提供了有效解决方案。
...
TL;DR: 该论文提出使用TabPFN等表格基础模型来高效计算Shapley值，解决了传统方法计算成本高的问题，在大多数情况下实现了最佳性能且运行时间大幅减少。


<details>
  <summary>Details</summary>
Motivation: Shapley值已成为可解释AI的基石，但计算成本高昂，特别是在特征相关的情况下。传统方法需要近似大量条件期望，要么通过蒙特卡洛积分，要么通过回归。深度学习在回归方法中难以充分利用，因为每个条件期望都需要重新训练模型，耗时过长。

Method: 使用TabPFN等表格基础模型，利用其上下文学习能力，无需重新训练即可近似每个条件期望。通过多个TabPFN变体计算Shapley值，并在模拟和真实数据集上与最先进方法进行比较。

Result: 在大多数情况下，TabPFN实现了最佳性能；在少数不如最佳方法的情况下，其性能仅略差但运行时间大幅减少。表格基础模型在条件Shapley值估计方面表现出色。

Conclusion: 表格基础模型如TabPFN能够高效计算Shapley值，解决了传统方法的计算瓶颈。未来可以通过进一步改进和专门适配表格基础模型来优化条件Shapley值估计。
...
TL;DR: FLINGO语言扩展了约束答案集编程，将ASP的丰富表达特性（如默认值、未定义属性、选择规则等）融入数值约束中，弥补了传统CASP的表达局限性。


<details>
  <summary>Details</summary>
Motivation: 传统约束答案集编程（CASP）在表达数值约束时丢失了ASP的许多重要特性，如默认值声明、属性未定义状态、非确定性赋值和聚合值使用等，限制了其在真实世界应用中的表达能力。

Method: 提出FLINGO语言和工具，将ASP的表达特性整合到数值约束中，并提供了从FLINGO语法到标准CASP程序（遵循CLINGCON输入格式）的翻译方法。

Result: 开发了FLINGO语言，成功将ASP的丰富表达特性融入数值约束处理，通过多个示例展示了其表达能力，并建立了语义基础和语法翻译机制。

Conclusion: FLINGO扩展了CASP的表达能力，使数值约束处理能够保留ASP的关键特性，为需要复杂数值约束处理的应用提供了更强大的建模工具。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型具备专家级医学知识，但其开放输出与临床医生细粒度偏好的对齐仍然困难。现有方法依赖粗粒度目标或不可靠的自动评估，缺乏专业指南基础。

Method: 提出两阶段框架：1) 创建HealthRubrics数据集，包含7,034个医生验证的偏好示例，临床医生根据医学标准完善LLM草拟的评分标准；2) 提炼为HealthPrinciples：119个广泛可重用、临床基础的原则，按临床维度组织，支持超越手动标注的可扩展监督。使用这些原则进行离线对齐（为未标注查询合成评分标准）和推理时引导自我修订。

Result: 仅激活30亿参数中30亿参数（实际使用30亿参数）的模型在HealthBench-Hard上达到33.4%的准确率，超越了包括Deepseek-R1和o3在内的更大模型，为临床对齐建立了资源高效的基线。

Conclusion: 该框架通过医生验证的偏好数据和提炼的可重用临床原则，有效解决了LLM医疗输出与临床医生偏好的对齐问题，小模型即可实现超越大模型的性能，为临床对齐提供了资源高效的解决方案。
...
TL;DR: GHS-TDA通过构建全局假设图和多尺度拓扑分析，解决传统CoT方法早期错误传播和冗余推理的问题，提高推理准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统CoT方法存在两个根本性限制：1）推理过程对早期决策高度敏感，初始错误会传播放大且难以纠正；2）缺乏结构化分析技术过滤冗余推理和提取关键特征，导致推理过程不稳定且可解释性有限。

Method: 提出GHS-TDA方法：首先构建语义丰富的全局假设图，聚合、对齐和协调多个候选推理路径，提供局部推理失败时的全局修正路径；然后应用基于持久同调的拓扑数据分析，捕捉稳定的多尺度结构，去除冗余和不一致，提取更可靠的推理骨架。

Result: 通过联合利用推理多样性和拓扑稳定性，GHS-TDA实现了自适应收敛，生成高置信度和可解释的推理路径，在多个推理基准测试中，在准确性和鲁棒性方面均持续优于强基线方法。

Conclusion: GHS-TDA通过全局协调和拓扑分析有效解决了传统CoT方法的局限性，显著提升了大型语言模型的推理准确性和稳定性，为复杂推理任务提供了更可靠的解决方案。
...
TL;DR: 研究通过旅行选择困境分析LLM的主观决策能力，使用多项Logit模型推导隐含支付意愿，并与人类基准比较，发现LLM能产生有意义的WTP但存在系统性偏差


<details>
  <summary>Details</summary>
Motivation: 随着LLM在旅行助手等应用中部署，需要在没有客观正确答案的情况下为用户做出主观选择，需要评估LLM在这类主观决策任务中的表现

Method: 在旅行助手情境中向LLM呈现选择困境，使用多项Logit模型分析响应以推导隐含支付意愿估计，比较人类经济学文献中的基准值，并考察信息提供和角色提示等更现实条件下的模型行为变化

Result: 较大LLM能产生有意义的WTP值，但在属性层面存在系统性偏差；总体上高估人类WTP，特别是引入昂贵选项或商务导向角色时；基于先前对便宜选项偏好的条件设定能产生更接近人类基准的估值

Conclusion: 研究结果突出了使用LLM进行主观决策支持的潜力和局限性，强调在实际部署中需要仔细选择模型、设计提示和用户表征
...
### [142] [CODE-SHARP: Continuous Open-ended Discovery and Evolution of Skills as Hierarchical Reward Programs](https://arxiv.org/abs/2602.10085)
*Richard Bornemann,Pierluigi Vito Amadori,Antoine Cully*

Main category: cs.AI

TL;DR: CODE-SHARP框架利用基础模型自动发现和演化分层技能，通过代码形式的奖励函数实现开放式的技能发现，在Craftax环境中显著提升智能体解决长时程任务的能力。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习依赖人工设计的奖励函数，不适用于开放式的技能发现，因为有意义技能的集合是未知的。现有方法仅限于为预定义任务优化奖励函数，无法实现真正的开放式技能发现。

Method: 提出CODE-SHARP框架，利用基础模型开放地扩展和精炼分层技能档案，将其构建为可执行奖励函数的代码形式的有向图。通过高层FM规划器组合发现的技能，使单个目标条件智能体能够解决复杂的长时程任务。

Result: 在Craftax环境中，仅使用发现的SHARP技能生成的奖励进行训练的目标条件智能体能够解决越来越长时程的目标。与预训练智能体和任务特定专家策略相比，平均性能提升超过134%。

Conclusion: CODE-SHARP框架成功实现了开放式的技能发现和演化，通过基础模型自动生成分层奖励函数，显著提升了智能体解决复杂长时程任务的能力，为开放式人工智能发展提供了新方向。
...
TL;DR: 提出Agent World Model (AWM)合成环境生成管道，创建1000个日常场景环境，支持多轮工具使用，通过大规模强化学习训练智能体，实现强大的分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体需要与环境进行多轮交互来完成复杂任务，但缺乏多样且可靠的环境限制了智能体训练的扩展性。

Method: 提出Agent World Model (AWM)合成环境生成管道，创建1000个代码驱动、数据库支持的日常场景环境，每个环境平均提供35种工具，支持高质量观察和可靠状态转换。

Result: 在三个基准测试上的实验表明，仅在合成环境中训练（而非基准特定环境）能够产生强大的分布外泛化能力，且代码驱动环境支持设计可靠的奖励函数。

Conclusion: AWM管道能够生成大规模、多样化的合成环境，为多轮工具使用智能体的大规模强化学习训练提供了有效资源，显著提升了智能体的泛化能力。
...
### [144] [Non-existence of Information-Geometric Fermat Structures: Violation of Dual Lattice Consistency in Statistical Manifolds with $L^n$ Structure](https://arxiv.org/abs/2602.09028)
*Kanta Tochigi*

Main category: cs.IT

TL;DR: 将费马大定理重新表述为信息几何结构的嵌入问题，通过统计流形和最大熵原理重构费马方程，证明n≥3时不存在满足"对偶格一致性"的信息几何结构。


<details>
  <summary>Details</summary>
Motivation: 将费马大定理重新解释为信息几何框架下的嵌入问题，探索整数解与几何结构之间的深层联系，揭示局部二次度量与全局L^n结构之间的不一致性。

Method: 1. 将费马方程重构为n阶矩约束，通过最大熵原理构建广义正态分布的统计流形M_n；2. 公理化定义"信息几何费马解"，要求勒让德变换下保持"对偶格一致性"；3. 利用泊松求和公式和Hausdorff-Young不等式分析傅里叶变换对函数族的影响；4. 证明n≥3时不存在满足条件的结构。

Result: 证明了对于n≥3，不存在满足"对偶格一致性"的信息几何费马解。傅里叶变换导致函数族从L^n变为L^q（其中1/n+1/q=1），使得对偶格一致性在解析上不可能实现，揭示了整数结构与能量结构在双重平坦空间中不相容的几何障碍。

Conclusion: 费马大定理的不可解性源于信息几何结构中的根本性障碍：局部二次度量（L^2）与全局L^n结构之间的不一致性，以及整数结构与能量结构在双重平坦空间中的不相容性。这一几何解释与椭圆曲线理论存在对应关系。
...
### [145] [On the Subpacketization Level of the Banawan-Ulukus Multi-Message PIR Scheme](https://arxiv.org/abs/2602.09417)
*Anoosheh Heidarzadeh*

Main category: cs.IT

TL;DR: 本文分析了Banawan和Ulukus多消息PIR方案中子分组化级别的线性递归关系，推导出了归一化子分组化级别L关于服务器数量N、总消息数K和需求消息数D的显式表达式。


<details>
  <summary>Details</summary>
Motivation: 在多消息PIR方案中，子分组化级别的计算涉及复杂的线性递归关系，需要推导出显式表达式以便更好地理解和优化方案性能。

Method: 通过分析Banawan和Ulukus方案中出现的线性递归关系，推导出归一化子分组化级别L关于N、K、D的显式公式表示。

Result: 得到了L的显式表达式，表明L是N的多项式且系数非负，其首项为N^{K-D+1}/D。

Conclusion: 成功推导了多消息PIR方案中子分组化级别的显式公式，为方案分析和优化提供了理论基础。
...
### [146] [On the generalization of $g$-circulant MDS matrices](https://arxiv.org/abs/2602.10028)
*Atif Ahmad Khan,Shakir Ali,Bhupendra Singh*

Main category: cs.IT

TL;DR: 本文研究了有限域上的一种新型矩阵——consta-g-循环矩阵，扩展了传统的g-循环矩阵概念。通过多项式环理论，给出了这类矩阵可逆性的判定条件和计数公式，并完全刻画了3阶和4阶g-循环MDS矩阵的特征。最后基于斜多项式环构造了新的矩阵变体。


<details>
  <summary>Details</summary>
Motivation: 最大距离可分(MDS)矩阵在密码学和编码理论中具有重要作用，能提供强大的数据保护和高效的信息扩散。然而，现有的g-循环矩阵在构造MDS矩阵方面存在局限性，需要扩展矩阵类型以寻找更多潜在的MDS矩阵候选。

Method: 引入consta-g-循环矩阵作为g-循环矩阵的推广，通过多项式环理论分析其代数结构。利用多项式分解理论推导可逆矩阵的计数公式，并基于斜多项式环构造新的矩阵变体。对低阶情况(3阶和4阶)进行完全分类。

Result: 给出了consta-g-循环矩阵可逆性的充分必要条件，推导出可逆矩阵数量的精确公式。完全刻画了3阶和4阶g-循环MDS矩阵的特征。基于斜多项式环构造了新的矩阵变体，并通过实例验证了理论结果。

Conclusion: consta-g-循环矩阵为构造MDS矩阵提供了新的理论框架，可逆性判定公式显著减少了搜索MDS矩阵的计算复杂度。对低阶矩阵的完全分类为实际应用提供了具体指导，新构造的矩阵变体为进一步研究开辟了方向。
