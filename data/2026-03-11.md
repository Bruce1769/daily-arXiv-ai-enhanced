...
TL;DR: 提出Granulon模型，基于DINOv3的多模态大语言模型，通过自适应粒度增强解决现有视觉编码器在多粒度推理上的不足
...
Result: 实验显示Granulon在相同设置下优于所有视觉编码器，准确率提高约30%，幻觉减少约20%

Conclusion: Granulon实现了单次前向传播中的统一"像素到细粒度到粗粒度"推理，解决了多模态大语言模型中的多粒度视觉理解问题
...
TL;DR: VisionCreator-R1：首个具有显式反思机制的原生视觉生成智能体，通过反思-规划协同优化训练方法，在单图和多图任务中超越Gemini2.5Pro


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成智能体主要是计划驱动的，缺乏系统性反思机制来纠正中间轨迹的视觉错误，限制了多图像工作流程的质量

Method: 提出VisionCreator-R1原生视觉生成智能体，采用反思-规划协同优化（RPCO）训练方法：1）在自建的VCR-SFT数据集上训练（包含反思强的单图轨迹和规划强的多图轨迹）；2）在VCR-RL数据集上通过强化学习进行协同优化

Result: 在现有基准和自建的VCR-bench（涵盖单图和多图任务）上，VisionCreator-R1一致超越Gemini2.5Pro；通过轨迹级分析揭示了强化学习中反思-规划优化的不对称性

Conclusion: VisionCreator-R1通过显式反思机制和RPCO训练方法，有效解决了现有视觉生成智能体缺乏系统性反思的问题，在多图像工作流程中实现了更可靠的视觉内容生成
...
<details>
  <summary>Details</summary>
Motivation: 智能停车系统对缓解城市拥堵和支持可持续交通至关重要。传统传感器系统存在精度限制和集成复杂等问题，而计算机视觉具有更高的准确性和适应性，能够动态评估视觉输入并适应变化的停车布局。

Method: 使用计算机视觉和YOLOv8对象检测模型，通过逆透视映射（IPM）融合四个摄像头视图的图像，提取空车位数据，并模拟3D停车环境，用3D笛卡尔图表示可用车位来引导用户。
...
TL;DR: 开发了一个多癌症肿瘤定位模型MuCTaL，通过在四种癌症类型上平衡训练，实现了高精度肿瘤检测并能够泛化到未见过的癌症类型。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的肿瘤检测模型在特定癌症类型上训练后，应用到其他肿瘤类型时可能表现不佳。研究探索通过跨癌症的平衡训练是否能实现高性能并泛化到未见过的肿瘤类型。

Method: 使用迁移学习和DenseNet169架构，在79,984个非重叠图像块上训练多癌症肿瘤定位模型，涵盖黑色素瘤、肝细胞癌、结直肠癌和非小细胞肺癌四种癌症类型。

Result: 模型在四种训练癌症的验证数据上达到0.97的ROC-AUC，在独立的胰腺导管腺癌队列上达到0.71的ROC-AUC。开发了可扩展的推理工作流生成空间肿瘤概率热图。

Conclusion: 通过跨癌症的平衡训练，可以在适度规模下实现高性能的肿瘤定位模型，并能够泛化到未见过的癌症类型，为空间分析、分子分析和组织架构研究提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界视频自然描绘了不同物理对象之间的复杂交互，形成了视觉元素的动态组合。然而，大多数当前视频生成模型整体合成场景，缺乏显式组合操作的机制。

Method: 提出HECTOR生成流水线，支持混合参考条件化（静态图像和/或动态视频），允许用户显式指定每个参考元素的轨迹，精确控制其位置、尺度和速度。

Result: 实验表明，HECTOR相比现有方法实现了更优的视觉质量、更强的参考保持能力和改进的运动可控性。

Conclusion: HECTOR通过支持细粒度组合控制，能够合成满足复杂时空约束同时保持对参考高保真度的连贯视频。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶视觉语言模型正在兴起，但其对物理对抗攻击的鲁棒性尚未得到充分研究。本文旨在填补这一空白，评估不同VLM架构在安全关键场景下的脆弱性

Method: 采用系统化框架，在CARLA仿真环境中对三种VLM架构（Dolphins、OmniDrive、LeapVAD）进行对比对抗评估。使用黑盒优化和语义同质化方法确保公平比较，评估物理可实现的补丁攻击

Result: 所有架构都表现出严重脆弱性，存在持续多帧故障和关键物体检测性能显著下降。分析揭示了不同的架构脆弱性模式，表明当前VLM设计未能充分应对安全关键自动驾驶应用中的对抗威胁

Conclusion: 当前自动驾驶视觉语言模型架构在对抗攻击面前存在严重安全漏洞，需要重新设计以增强鲁棒性，确保安全关键应用中的可靠性
...
TL;DR: 本文提出了视觉查询分割（VQS）新范式，旨在通过外部视觉查询在未修剪视频中分割目标对象的所有像素级出现，并发布了首个VQS专用大规模基准VQS-4K和有效方法VQ-SAM。


<details>
  <summary>Details</summary>
Motivation: 现有视觉查询定位（VQL）方法仅使用边界框定位目标的最后出现，缺乏全面性（所有对象出现）和精确性（像素级掩码）。VQS旨在提供更全面和精确的定位，使其更适合实际应用场景。

Method: 提出了VQ-SAM方法，通过从视频中利用目标特定和背景干扰线索，通过新颖的多阶段框架逐步演化记忆，采用自适应记忆生成（AMG）模块进行VQS，显著提升性能。

Result: 在VQS-4K基准上的广泛实验表明，VQ-SAM取得了有希望的结果，超越了所有现有方法，证明了其有效性。VQS-4K包含4,111个视频、超过130万帧，涵盖222个对象类别。

Conclusion: 通过提出的VQS-4K基准和VQ-SAM方法，期望超越当前VQL范式，激发更多关于VQS的未来研究和实际应用。基准、代码和结果将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型在医学影像中虽然提供了透明度，但离散的概念表示往往忽略了更广泛的临床背景（如诊断指南和专家启发式），导致复杂病例中的可靠性降低。需要一种能够整合临床指南、提升可靠性的可解释AI框架。

Method: 提出MedCBR框架：1) 将标记的临床描述符转化为符合指南的文本；2) 训练概念模型，采用多任务目标（多模态对比对齐、概念监督、诊断分类）联合对齐图像特征、概念和病理；3) 使用推理模型将预测转化为结构化临床叙事，模拟基于指南的专家推理。

Result: 在医学数据集上取得优异性能：超声数据集AUROC达到94.2%，乳腺X光数据集AUROC达到84.0%。在非医学数据集上达到86.1%的准确率。框架提升了诊断和概念层面的性能。

Conclusion: MedCBR框架增强了可解释性，形成了从医学影像分析到决策制定的端到端桥梁，通过整合临床指南和结构化推理，提高了医学AI系统的可靠性和透明度。
...
<details>
  <summary>Details</summary>
Motivation: 微表情是人在抑制情绪时产生的面部微小动作，传统分析方法有限。多模态大语言模型的出现为微表情分析提供了新的可能性，需要探索这些模型在微表情理解任务中的应用。

Method: 通过两个具体任务来评估模型能力：1）ME-VQA：在相对较短的视频序列上进行视觉问答，测试模型对微表情的理解；2）ME-LVQA：扩展到长时视频序列，要求模型具备时间推理和长时间微表情检测能力。

Result: 提出了MEGC 2026挑战赛的具体框架，包括两个任务的定义、评估标准和公开排行榜机制，为研究人员提供了标准化的评估平台。

Conclusion: 多模态大语言模型为微表情分析带来了新的机遇，MEGC 2026通过引入视频问答任务，旨在推动该领域向更自然、更全面的微表情理解方向发展。
...
TL;DR: TIDE是一种无需训练的文本到图像外推方法，解决了DiT在高分辨率生成时的结构退化问题，支持任意分辨率和宽高比生成。
...
Conclusion: TIDE是一种有效的训练免费文本到图像外推方法，解决了DiT在高分辨率生成时的结构退化问题，支持任意分辨率和宽高比，同时保持语义细节和减少伪影。
...
TL;DR: PathoScribe是一个统一的检索增强大语言模型框架，将静态病理档案转化为可搜索、支持推理的活体知识库，实现自然语言病例检索、自动队列构建、临床问答等功能。


<details>
  <summary>Details</summary>
Motivation: 尽管病理学在现代诊断和癌症护理中至关重要，但数百万叙述性报告中积累的经验大多无法访问。数字化存储数据但缺乏有效检索和推理机制，使得档案变成被动数据仓库，无法有效指导患者护理。

Method: 开发PathoScribe，一个统一的检索增强大语言模型框架，支持自然语言病例探索、自动队列构建、临床问答、免疫组化面板推荐和提示控制报告转换。

Result: 在70,000份多机构外科病理报告上评估，PathoScribe在自然语言病例检索中实现完美Recall@10，检索增强推理质量高（平均评分4.56/5）。自动队列构建仅需平均9.2分钟，与人工审核一致率达91.3%，无合格病例被错误排除。
...
TL;DR: BiCLIP是一个极简的跨模态对齐框架，通过几何变换增强视觉语言模型在专业领域的适应能力，在11个基准测试中达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在零样本任务上表现出色，但在专业领域的适应仍然面临挑战。基于理论发现：独立训练的VLMs之间存在规范变换关系，作者假设不同领域的图像特征可以通过规范化的几何变换关联，少量锚点样本即可估计这种变换。

Method: BiCLIP框架对多模态特征应用目标变换来增强跨模态对齐。该方法极其简单且参数量小，利用小样本分类中的有限标注样本作为锚点来估计领域间的几何变换。

Result: 在包括EuroSAT、DTD、FGVCAircraft在内的11个标准基准测试中，BiCLIP持续达到最先进的结果。通过对学习变换的正交性和角度分布分析，验证了现有几何发现的正确性。

Conclusion: 结构化对齐是鲁棒领域适应的关键。BiCLIP通过极简的几何变换方法，有效解决了视觉语言模型在专业领域的适应问题，为跨模态对齐提供了新的理论验证和实践方案。
...
TL;DR: 该论文提出了首个免示例的持续学习基准用于音频-视觉分割，并提出了ATLAS方法，通过音频引导的预融合条件化和低秩锚定技术来解决动态环境中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界环境本质上是动态的，音频和视觉分布会随时间演变，而现有的音频-视觉分割系统假设静态训练设置，无法适应这种变化。因此需要开发能够持续学习的音频-视觉分割系统。

Method: 提出了ATLAS方法：1）使用音频引导的预融合条件化，通过投影的音频上下文在跨模态注意力之前调制视觉特征通道；2）引入低秩锚定技术，基于损失敏感性稳定适应权重以减轻灾难性遗忘。

Result: 在包含单源和多源音频-视觉分割数据集的四种学习协议上进行了广泛实验，展示了在不同持续学习场景下的竞争性能，为终身音频-视觉感知奠定了基础。

Conclusion: 该研究建立了首个音频-视觉分割的持续学习基准，提出的ATLAS方法通过创新的音频引导预融合条件化和低秩锚定技术，有效解决了动态环境中的灾难性遗忘问题，推动了终身音频-视觉感知的发展。
...
TL;DR: SVG-EAR是一种用于视频扩散变换器的高效注意力机制，通过语义聚类和线性补偿分支近似跳过的注意力块，结合误差感知路由选择性地精确计算高误差块，在保持生成质量的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中表现出色，但其二次注意力计算成本是主要瓶颈。现有稀疏注意力方法要么丢弃部分注意力块导致信息损失，要么依赖学习预测器引入训练开销和潜在输出分布偏移。

Method: SVG-EAR采用参数自由的线性补偿分支：1）通过语义聚类将每个块内的键值对分组；2）使用聚类质心近似跳过的注意力块；3）通过轻量级探针估计每个块的补偿误差；4）基于误差-成本比选择性地精确计算高误差块，同时补偿跳过的块。

Result: SVG-EAR在视频扩散任务中显著改善了质量-效率权衡，在相同生成保真度下提高了吞吐量。在Wan2.2和HunyuanVideo数据集上分别实现了1.77倍和1.93倍的加速，同时保持PSNR分别达到29.759和31.043。

Conclusion: SVG-EAR建立了一个优于先前方法的帕累托前沿，通过无参数的线性补偿和误差感知路由，在保持视频生成质量的同时显著降低了扩散变换器的计算成本，为高效视频生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS训练成本高昂，特别是在后致密化阶段，反向传播占主导运行时间。观察到许多采样视图的损失已经趋于平稳，但仍进行完整的反向传播，存在大量更新冗余。

Method: 提出SkipGS，采用视图自适应反向门控机制：始终执行前向传播更新每视图损失统计，当采样视图的损失与其最近基线一致时选择性跳过反向传播，同时强制执行最小反向预算以确保稳定优化。

Result: 在Mip-NeRF 360数据集上，相比3DGS，SkipGS减少23.1%的端到端训练时间（后致密化时间减少42.0%），同时保持可比较的重建质量。

Conclusion: SkipGS通过智能跳过冗余反向传播实现高效训练，由于只改变反向传播时机而不修改渲染器、表示或损失函数，具有即插即用特性，可与其他效率策略结合实现叠加加速。
...
TL;DR: 该论文提出了一种基于扩散模型的防伪认证框架，通过联合利用原始二进制模板、打印的CDP和打印机身份表示来区分真伪，在Indigo数据集上优于传统方法和现有深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 随着高分辨率打印扫描设备的普及和生成式深度学习的进步，传统防伪认证系统难以区分高质量伪造品和正品打印，特别是在可打印不可克隆代码（CDP）等防伪措施面临挑战的背景下。

Method: 提出扩散认证框架，将认证问题重新定义为基于打印机签名的多类打印机分类。通过扩展ControlNet，重新利用去噪过程进行类条件噪声预测，结合空间和文本条件来捕捉细粒度的设备特定特征。

Result: 在Indigo 1 x 1 Base数据集上，该方法优于传统相似性度量和先前的深度学习方法，并且能够泛化到训练期间未见过的伪造类型。

Conclusion: 提出的扩散认证框架通过联合利用多种信息源，能够有效应对现代伪造技术挑战，为防伪认证提供了更强大的解决方案。
...
TL;DR: SpectralDiff：一种基于频谱结构扩散的框架，专门用于单图像去雨，通过结构化频谱扰动引导多方向雨成分的渐进抑制


<details>
  <summary>Details</summary>
Motivation: 雨纹表现为跨多个尺度的方向性和频率集中的重叠结构，使得单图像去雨特别具有挑战性。传统的空间域扩散方法没有明确考虑这种结构化频谱特性

Method: 提出SpectralDiff框架，通过结构化频谱扰动引导渐进抑制多方向雨成分；设计全乘积U-Net架构，利用卷积定理将卷积操作替换为逐元素乘积层，提高计算效率

Result: 在合成和真实世界基准测试上的广泛实验表明，SpectralDiff实现了具有竞争力的去雨性能，相比现有基于扩散的方法具有改进的模型紧凑性和良好的推理效率
...
<details>
  <summary>Details</summary>
Motivation: 传统火灾检测系统通常只进行简单的火灾识别，缺乏对周围环境和风险程度的评估。需要一种能够检测火灾并评估其对附近人员、车辆和基础设施威胁程度的智能系统。

Method: 使用双模型框架：主模型为YOLOv8实例分割模型，用于检测火灾和烟雾；副模型为在COCO数据集上预训练的目标检测模型，用于识别周围实体。通过整合两个模型的输出，计算火灾区域与附近物体之间的像素距离，并使用像素到米的缩放方法转换为实际距离测量值。

Result: 在9,860张标注图像的数据集上训练，系统表现出色：精确率、召回率和F1分数均超过90%，mAP@0.5超过91%。系统能生成带注释的可视化输出，显示火灾位置、检测到的物体、估计距离和上下文风险信息。

Conclusion: 该框架在Google Colab环境中使用开源工具实现，轻量级且适合在工业和资源受限环境中部署，为火灾检测提供了从简单识别到风险评估的完整解决方案。
...
TL;DR: GST-VLA提出Gaussian Spatial Tokenizer将深度和语义特征转换为3D高斯基元，以及3D Depth-Aware Chain-of-Thought推理方法，在视觉语言动作任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型将视觉观察编码为无内在几何结构的2D补丁标记，无法有效处理3D空间信息。需要一种能够编码局部表面方向、几何置信度等3D几何信息的表示方法。

Method: 1. Gaussian Spatial Tokenizer：将冻结的密集深度和语义补丁特征转换为128个各向异性3D高斯基元，每个基元包含度量残差均值、对数尺度协方差和学习的不透明度。2. 3D Depth-Aware Chain-of-Thought推理：监督四个结构化中间空间思考，包括3D物体接地、抓取可供性接触几何、成对度量距离和粗略SE(3)路径点。3. 300M参数流匹配动作专家：通过条件ODE积分解码7-DoF增量动作块。

Result: 在LIBERO上达到96.4%（+2.0%提升），在SimplerEnv上达到80.2%（+5.4%提升）。消融实验确认了每个GST组件、每个DA-CoT思考和每个训练阶段的独立和协同增益。

Conclusion: GST-VLA通过3D高斯空间标记化和深度感知链式思考推理，显著提升了视觉语言动作模型的性能，特别是在需要精度的任务上。该方法提供了对3D几何信息的有效编码和结构化推理能力。
...
TL;DR: OmniEdit是一个无需训练、用于唇音同步和音视频编辑的框架，通过替换FlowEdit中的编辑序列为目标序列，实现无偏估计和平稳编辑轨迹。


<details>
  <summary>Details</summary>
Motivation: 唇音同步和音视频编辑是多模态学习中的基础挑战，应用广泛，但现有方法大多依赖预训练模型的监督微调，导致计算开销大且数据需求高。

Method: 提出OmniEdit训练免费框架，通过将FlowEdit中的编辑序列替换为目标序列，实现无偏输出估计，并去除生成过程中的随机元素以建立平稳编辑轨迹。

Result: 大量实验结果验证了该框架的有效性和鲁棒性。

Conclusion: OmniEdit为唇音同步和音视频编辑提供了一种无需训练的高效解决方案，代码已开源。
...
TL;DR: 该论文提出了一种物理合理视频生成框架，通过事件链推理和跨模态提示来解决视频扩散模型在物理常识理解方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前物理合理视频生成方法依赖大语言模型的常识推理能力，但生成模型往往只能渲染提示定义的单一时刻物理现象，缺乏对因果进展的建模机制。
...
Conclusion: 该研究将物理合理视频生成视为生成因果连接且动态演化的事件序列，通过事件分解和跨模态提示实现了更好的物理合理性。
...
Method: 提出MedKCO方法，包含两个方面：1）基于诊断敏感性和类内样本代表性的两级课程设计来编排预训练数据顺序；2）考虑医学图像类间相似性，引入自适应的非对称对比损失来动态调整预训练目标的参与度。

Result: 在三种医学成像场景的多个视觉语言下游任务中评估，与多种课程学习方法比较，实验结果表明该方法显著超越所有基线方法。

Conclusion: MedKCO通过知识驱动的认知编排优化医学视觉语言预训练，有效解决了现有方法中简单和复杂概念同时学习的问题，提升了模型在下游任务中的泛化性能。
...
TL;DR: 提出运动分解框架，将复杂运动分解为三类：静止、刚体运动和非刚体运动，采用"先规划后生成"范式，通过运动图推理和条件生成模块提升视频合成的运动控制能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法主要关注语义绑定，忽视了理解提示中指定的多样化运动类别。现有方法难以处理复杂的运动组合，导致生成视频中运动控制不精确。

Method: 提出运动分解框架：1) 规划阶段：在运动图上推理运动规律，获得每个实例形状和位置的逐帧变化，将用户提示组织为结构化表示；2) 生成阶段：以解耦方式调制不同运动类别的合成，通过条件引导分支稳定静止区域外观、保持刚体几何、正则化局部非刚体变形。

Result: 在真实世界基准测试中展示了令人印象深刻的运动合成性能，框架具有模型无关性，可无缝集成到各种扩散模型架构中。

Conclusion: 提出的运动分解框架通过先规划后生成的范式，有效解决了复杂运动合成的挑战，显著提升了视频生成中运动控制的精确性和多样性。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像检索在临床诊断、教育和质量控制中至关重要，但实际查询通常结合病变图像和文本描述，需要处理组合查询的挑战

Method: 提出Transformer框架学习层次化组合查询表示，通过多空间注意力掩码进行局部对齐，全局对齐提供整体语义监督，采用凸的领域知情权重计算最终相似度

Result: 在公开Derm7pt数据集上实验显示，相比最先进方法取得一致改进，能够高效访问相关医疗记录并支持实际临床部署

Conclusion: 该框架通过强调临床显著的局部证据同时保持全局一致性，有效提升了皮肤癌医学图像检索的性能和实用性
...
<details>
  <summary>Details</summary>
Motivation: 现实世界应用中经常遇到模态缺失问题（如噪声、硬件故障或隐私限制），现有方法通常假设所有模态都完整，直接融合缺失模态会扭曲已学到的完整模态表示，存在特征不对齐问题。
...
Conclusion: PRLF框架有效解决了不确定缺失模态条件下的多模态情感分析问题，通过自适应可靠性估计和渐进对齐机制，在处理模态缺失时表现出优越性能。
...
TL;DR: QUSR：一种结合质量感知先验和不确定性引导噪声生成的新型超分辨率扩散模型，专门针对真实世界中未知且空间非均匀的退化场景


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的超分辨率方法在真实世界场景中面临挑战，因为退化通常是未知且空间非均匀的，导致细节丢失或视觉伪影

Method: 提出QUSR模型，包含两个核心组件：1）不确定性引导噪声生成模块，根据区域不确定性自适应调整噪声注入强度；2）质量感知先验，利用多模态大语言模型生成可靠的质量描述作为先验信息

Result: 实验结果表明QUSR能够在真实世界场景中生成高保真度和高真实感的图像

Conclusion: QUSR通过结合不确定性引导的噪声生成和质量感知先验，有效解决了真实世界超分辨率中未知和非均匀退化的问题，提升了图像恢复的质量和真实性
...
<details>
  <summary>Details</summary>
Motivation: 传统DXA诊断骨质疏松仅测量骨密度，忽略骨微结构和周围软组织信息。HR-pQCT虽能三维成像，但现有分析主要关注矿化骨区，大量图像数据未被充分利用。

Method: 采用SegFormer架构对HR-pQCT图像进行全自动多区域分割（胫骨、腓骨的皮质骨和松质骨，以及周围软组织）。通过后处理将软组织细分为皮肤、肌腱肌肉和脂肪区域。从每个区域提取939个放射组学特征，经降维后训练6种机器学习分类器。

Result: SegFormer分割平均F1分数达95.36%。图像层面，肌腱肌肉组织特征表现最佳（准确率80.08%，AUROC 0.85），优于基于骨特征的模型。患者层面，用软组织放射组学替代标准生物、DXA和HR-pQCT参数，AUROC从0.792提升至0.875。

Conclusion: 全自动多区域HR-pQCT分割能够提取超越骨组织的临床信息，证明了整合组织评估对骨质疏松检测的重要性，软组织特征提供了有价值的诊断信号。
...
<details>
  <summary>Details</summary>
Motivation: 密集图像描述对于视觉语言预训练和文本到图像生成中的跨模态对齐至关重要，但专家级标注成本高昂。虽然通过强视觉语言模型进行合成描述是一种实用替代方案，但监督蒸馏通常输出多样性有限且泛化能力弱。强化学习可以克服这些限制，但以往成功主要集中在依赖确定性检查器的可验证领域，而开放式的图像描述缺乏这种机制。

Method: RubiCap首先组建一个多样化的候选描述委员会，然后使用LLM评分标准编写器提取共识优势并诊断当前策略的缺陷。这些见解被转化为明确的评估标准，使LLM评委能够分解整体质量评估，并用结构化的多维度评估替代粗糙的标量奖励。

Result: 在广泛的基准测试中，RubiCap在CapArena上实现了最高的胜率，超越了监督蒸馏、先前的RL方法、人类专家标注和GPT-4V增强输出。在CaptionQA上，它展示了优越的词汇效率：7B模型与Qwen2.5-VL-32B-Instruct相当，3B模型超越了其7B对应模型。值得注意的是，使用紧凑的RubiCap-3B作为描述器训练出的预训练VLM比使用专有模型描述训练的VLM更强。

Conclusion: RubiCap通过LLM编写的评分标准为开放式图像描述任务提供细粒度奖励信号，成功克服了传统方法的局限性，在多个基准测试中表现出色，证明了其在密集图像描述任务中的有效性和优越性。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM自进化需要至少一些种子数据（如图像）来启动过程，而LLM已经可以实现零数据自进化。本研究旨在开发首个零数据自进化的VLM推理框架，突破传统双角色范式限制。
...
Conclusion: MM-Zero成功实现了首个零数据自进化的VLM推理框架，超越了传统的双模型范式，为多模态模型的自进化开辟了新方向。
...
TL;DR: 该论文提出了一种基于几何不变性的手部关节角度描述符，用于解决手语识别中的跨语言少样本迁移问题，通过消除相机视角、手部尺度和记录条件等外在因素带来的域偏移，显著提升了识别性能。


<details>
  <summary>Details</summary>
Motivation: 全球300多种手语中大多数缺乏足够的标注数据，而传统的基于坐标的关键点表示容易受到相机视角、手部尺度和记录条件等外在因素引起的域偏移影响，这在少样本场景下尤为严重，因为仅从少量样本估计的类别原型对外在方差高度敏感。

Method: 提出了一个几何感知的度量学习框架，核心是使用从MediaPipe静态手部关键点导出的20维关节间角度描述符。这些角度对SO(3)旋转、平移和各向同性缩放具有不变性，消除了跨数据集偏移的主要来源，从而产生更紧凑、更稳定的类别原型。

Result: 在四种类型多样的手语字母表（ASL、LIBRAS、阿拉伯手语和泰语手语）上进行评估，提出的角度特征比归一化坐标基线在域内性能提升了高达25个百分点，并且实现了冻结的跨语言迁移，其性能经常超过域内准确率，使用的轻量级MLP编码器仅约10^5个参数。

Conclusion: 不变的手部几何描述符为低资源环境下的跨语言少样本手语识别提供了一个便携且有效的基础，证明了几何不变性特征在解决域偏移问题上的有效性。
...
TL;DR: TubeMLLM是一个用于医学血管状解剖结构的统一基础模型，通过整合拓扑先验和多模态大语言模型，显著提升了拓扑感知能力，在多个数据集上实现了最先进的零样本泛化性能。


<details>
  <summary>Details</summary>
Motivation: 医学血管状解剖结构建模具有挑战性，因为其复杂的拓扑结构和对数据集偏移的敏感性。传统的任务特定模型经常出现拓扑不一致问题，如人工断开和虚假合并。受多模态大语言模型在零样本泛化方面的潜力启发，需要开发一个能够结合结构化理解和可控生成的统一基础模型。

Method: 提出TubeMLLM模型，通过显式自然语言提示整合拓扑先验，并在共享注意力架构中与视觉表示对齐。构建TubeMData多模态基准数据集，包含全面的拓扑中心任务。引入自适应损失加权策略，在训练过程中强调拓扑关键区域。

Result: 在15个不同数据集上的实验显示：1）在彩色眼底摄影中，将β₀数误差从37.42降至8.58；2）在未见过的X射线血管造影上实现零样本跨模态迁移，Dice分数达67.50%，β₀误差降至1.21；3）对模糊、噪声和低分辨率等退化具有鲁棒性；4）在拓扑感知理解任务中，掩模拓扑质量评估准确率达97.38%。

Conclusion: TubeMLLM通过整合拓扑先验和多模态大语言模型，成功解决了医学血管状解剖结构建模中的拓扑不一致问题，实现了卓越的零样本泛化和跨模态迁移能力，为医学图像分析提供了强大的统一基础模型。
...
TL;DR: 提出了一种统一的MRI场强增强框架，通过利用共享的退化模式和3D基础模型来提升不同场强间的增强性能，同时引入场感知频谱校正机制来改善高频细节。


<details>
  <summary>Details</summary>
Motivation: 现有MRI场强增强方法通常专注于孤立的增强任务（如64mT到3T或3T到7T），使用有限的数据集，未能充分利用不同场强间的共享退化模式，导致模型泛化能力受限。

Method: 提出了一个统一的多模态增强框架：1）利用预训练的3D基础模型直接处理3D体积数据而非2D切片，以捕获连续解剖结构；2）引入场感知频谱校正机制（FASRM），结合磁场物理机制为不同场强定制频谱校正；3）构建并公开了一个大规模配对多场MRI数据集。

Result: 实验表明该方法优于现有最先进方法，平均PSNR提升约1.81 dB，SSIM提升9.47%。

Conclusion: 该方法通过统一框架、3D基础模型和场感知频谱校正，显著提升了MRI场强增强的性能和泛化能力，并提供了大规模数据集促进该领域发展。
...
TL;DR: HelixTrack：首个完全基于事件驱动的螺旋桨状物体跟踪与转速估计方法，在存在自运动和强干扰的情况下实现微秒级延迟和实时处理
...
Method: 1) 通过实时估计的单应性矩阵将事件从图像平面反向映射到转子平面；2) 使用卡尔曼滤波器维护瞬时相位估计；3) 批量迭代更新通过耦合相位残差与几何来细化物体姿态；4) 引入TQE数据集用于联合跟踪和转速估计

Result: 在TQE数据集上，HelixTrack以约11.8倍实时速度处理全速率事件，实现微秒级延迟，在联合跟踪和转速估计任务上持续优于基于单事件和聚合的基线方法

Conclusion: HelixTrack填补了事件驱动方法在周期性运动物体跟踪和转速估计方面的空白，为安全关键应用提供了微秒级延迟的解决方案，并通过TQE数据集为该领域研究建立了基准
...
TL;DR: BridgeDiff是一个基于扩散模型的虚拟试穿框架，通过两个互补组件显式桥接人体观察与平面服装合成，解决了现有方法在未观察区域完成不一致和服装结构不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法通常将其视为直接的图像转换，依赖于局部掩码或纯文本提示，忽视了人体外观与平面布局之间的差距，导致未观察区域完成不一致和服装结构不稳定。

Method: 提出BridgeDiff框架，包含两个组件：1) 服装条件桥接模块(GCBM)，构建捕捉全局外观和语义身份的服装线索表示；2) 平面结构约束模块(FSCM)，通过Flat-Constraint Attention在选定去噪阶段注入显式平面服装结构先验。

Result: 在标准VTOFF基准测试中，BridgeDiff实现了最先进的性能，生成更高质量的平面服装重建，同时保留了细粒度外观和结构完整性。

Conclusion: BridgeDiff通过显式桥接人体观察与平面服装合成，有效解决了虚拟试穿中的外观-结构差距问题，为标准化展示和下游虚拟试穿应用提供了更好的解决方案。
...
TL;DR: 论文提出RAE-NWM模型，在密集视觉表示空间中建模导航动态，使用条件扩散Transformer和分离的时间门控模块，提升结构稳定性和动作精度


<details>
  <summary>Details</summary>
Motivation: 当前导航世界模型通常在VAE压缩潜在空间中学习状态演化，但空间压缩会丢失细粒度结构信息，阻碍精确控制。研究发现DINOv2特征在动作条件转换中表现出更强的线性可预测性，因此提出在密集视觉表示空间中建模导航动态

Method: 提出RAE-NWM模型：1）在密集视觉表示空间中建模导航动态；2）使用条件扩散Transformer与分离扩散Transformer头（CDiT-DH）建模连续转换；3）引入独立的时间驱动门控模块进行动态调节，控制生成过程中的动作注入强度

Result: 在该空间中建模序列展开能提高结构稳定性和动作精度，有利于下游规划和导航任务

Conclusion: 在密集视觉表示空间中建模导航动态比在压缩潜在空间中更有效，能保留更多结构信息，提升导航性能
...
TL;DR: 本文提出Geometric Semantic Decoupling (GSD)方法，通过几何约束移除语义成分，解决VFM检测器在未见生成管道图像上的泛化问题，显著提升AI生成图像检测性能。
...
Method: 提出Geometric Semantic Decoupling (GSD)方法：使用冻结的VFM作为语义指导，可训练的VFM作为伪迹检测器，通过批量统计估计语义方向，利用几何约束投影移除语义成分，强制检测器依赖语义不变的取证证据。

Result: 在跨数据集评估中达到94.4%视频级AUC（提升1.2%），在DF40上对未见操作的鲁棒性提升3.0%，在通用场景检测任务（UniversalFakeDetect和GenImage）上分别提升0.9%和1.7%。

Conclusion: GSD方法通过显式解耦语义成分，有效解决了VFM检测器的语义回退问题，显著提升了AI生成图像检测器在跨数据集和未见操作场景下的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决实例分割中高分辨率输入与轻量级实时推理之间的冲突需求，避免密集像素级掩码预测的计算负担。
...
Conclusion: Poly-DETR通过稀疏顶点回归和极坐标表示，有效解决了高分辨率输入与实时推理的冲突，特别适用于规则形状实例的领域特定场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法相互孤立，专注于特定领域（如在线SLAM、语义增强或未标定图像），缺乏统一框架。需要构建一个能够整合多种技术、支持实时语义增强SLAM并连接下游多模态模型的平台

Method: 1. 核心X-GS-Perceiver高效管道：处理未标定RGB（可选RGB-D）视频流，协同优化几何和姿态；2. 从视觉基础模型提取高维语义特征到3D高斯；3. 通过在线向量量化模块、GPU加速网格采样方案和高度并行化管道设计实现实时性能；4. X-GS-Thinker组件利用语义3D高斯连接视觉语言模型

Result: 在真实世界数据集上展示了X-GS框架的有效性、效率和新增的多模态能力，能够支持物体检测、零样本字幕生成等下游任务

Conclusion: X-GS提供了一个统一的开放框架，将3DGS技术从孤立应用扩展到实时语义增强的在线SLAM系统，并成功连接下游多模态模型，为空间AI应用开辟了新可能性
...
### [39] [EXPLORE-Bench: Egocentric Scene Prediction with Long-Horizon Reasoning](https://arxiv.org/abs/2603.09731)
*Chengjun Yu,Xuhan Zhu,Chaoqun Du,Pengfei Yu,Wei Zhai,Yang Cao,Zheng-Jun Zha*

Main category: cs.CV

TL;DR: 研究者提出EXPLORE-Bench基准，用于评估多模态大语言模型在自我中心视角下对长时程物理动作后果的推理能力，发现当前模型与人类表现存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型被视为具身智能体的基础，但尚不清楚它们是否能从自我中心视角可靠地推理动作的长期物理后果。研究者希望通过新任务填补这一空白。

Method: 提出Egocentric Scene Prediction with LOng-horizon REasoning任务：给定初始场景图像和一系列原子动作描述，预测所有动作执行后的最终场景。创建EXPLORE-Bench基准，包含来自真实第一人称视频的多样化场景，每个实例都配有结构化最终场景标注。

Result: 实验显示，各种专有和开源MLLM与人类表现存在显著性能差距，表明长时程自我中心推理仍是主要挑战。通过逐步推理的测试时扩展分析表明，分解长动作序列能在一定程度上改善性能，但会产生不小的计算开销。

Conclusion: EXPLORE-Bench为测量和推进自我中心具身感知的长时程推理提供了一个原则性的测试平台，揭示了当前模型的局限性并指出了改进方向。
...
TL;DR: ForgeDreamer：针对工业场景的文本到3D生成框架，通过多专家LoRA集成和跨视角超图几何增强，解决传统方法在工业应用中的领域适应和几何推理不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D生成方法在自然场景表现良好，但在工业应用中存在两大关键限制：1）领域适应挑战 - 传统LoRA融合导致跨类别知识干扰；2）几何推理不足 - 成对一致性约束无法捕捉精密制造所需的高阶结构依赖关系。

Method: 提出ForgeDreamer框架，包含两个关键创新：1）多专家LoRA集成机制 - 将多个类别特定的LoRA模型整合为统一表示，实现跨类别泛化并消除知识干扰；2）跨视角超图几何增强方法 - 基于增强的语义理解，同时捕捉多个视角的结构依赖关系。

Result: 在自定义工业数据集上的大量实验表明，相比最先进方法，该方法在语义泛化和几何保真度方面表现更优。代码和数据已作为补充材料提供。
...
<details>
  <summary>Details</summary>
Motivation: 虽然3D高斯泼溅在渲染质量和效率上优于NeRF，但进一步提高学习3D高斯的效率仍然是一个研究挑战

Method: 1) 定期重置高斯尺度以缩小每个高斯的尺寸；2) 在alpha混合过程中引入熵约束来锐化权重分布；3) 集成到渐进分辨率增加的渲染调度器中

Result: 在广泛使用的基准测试中与最先进方法比较，结果显示在保持渲染质量的同时显著提高了效率

Conclusion: 提出的训练策略和损失函数有效缩短了渲染像素所需的高斯列表，显著提升了3D高斯泼溅的学习效率
...
TL;DR: SVOR是一个用于视频对象移除的鲁棒框架，通过三种关键设计处理真实世界的不完美情况（如阴影、突变运动和缺陷掩码），实现无阴影、无闪烁且对掩码缺陷容忍的移除效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复模型在真实世界的不完美情况下（如阴影、突变运动、缺陷掩码）难以保持时间稳定性和视觉一致性，需要更鲁棒的解决方案。

Method: 提出三个关键设计：1) MUSE（窗口化联合策略）处理突变运动和减少漏移除；2) DA-Seg（去噪感知分割）提供内部扩散感知定位先验；3) 课程两阶段训练：第一阶段在无配对真实背景视频上进行自监督预训练，第二阶段在合成对上使用掩码退化和副作用加权损失进行精炼。

Result: 在多个数据集和退化掩码基准测试中取得了新的最先进结果，将视频对象移除从理想设置推向实际应用。

Conclusion: SVOR框架通过创新的设计解决了真实世界视频对象移除中的关键挑战，显著提高了时间稳定性和视觉一致性，为实际应用提供了有效的解决方案。
...
TL;DR: CogBlender是一个文本到图像生成框架，能够通过连续多维干预控制图像的认知属性（如情感效价、记忆性等），将认知空间与语义流形映射，实现基于心理意图的图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型虽然能生成语义连贯的内容，但难以控制图像的认知属性（如情感效价、记忆性等），无法与特定的心理意图对齐。需要开发能够精确控制图像认知特性的生成框架。
...
Result: 在四个代表性认知维度（效价、唤醒度、支配性和图像记忆性）上验证了方法的有效性，实验表明CogBlender能够实现有效的认知干预，为认知驱动的创意设计提供了有效范式。

Conclusion: CogBlender框架成功实现了文本到图像生成中对认知属性的连续多维控制，填补了现有模型在心理意图对齐方面的空白，为认知驱动的创意设计开辟了新途径。
...
TL;DR: 本文提出IntroSVG框架，通过让视觉语言模型同时担任生成器和批评者的双重角色，在生成SVG时引入视觉反馈循环，显著提升了生成质量。
...
Method: 提出IntroSVG框架，通过监督微调让模型学习生成SVG并对其渲染输出提供反馈，将早期失败转化为高质量纠错训练数据。然后利用高容量教师VLM构建偏好数据集，通过直接偏好优化对齐生成器策略。推理时采用"生成-评审-优化"的迭代循环。

Result: 实验结果表明，该方法在多个关键评估指标上达到最先进性能，生成的SVG具有更复杂的结构、更强的语义对齐和更好的可编辑性。

Conclusion: 将明确的视觉反馈纳入生成循环是有效的，IntroSVG框架通过生成器和批评者的协同工作，显著提升了SVG生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 在轨服务和主动碎片清除等自主空间操作需要航天器部件级语义理解和精确相对导航，但收集大规模真实轨道数据成本高且不切实际。现有合成数据集存在目标多样性有限、单模态感知和地面真值标注不完整的问题。

Method: 使用Unreal Engine 5构建高保真空间模拟环境，通过全自动流水线生成数据，包括数据采集、多阶段质量控制和主流格式转换。提供时间同步的1024×1024 RGB图像、毫米级精度深度图、256束LiDAR点云，以及像素级和点级的7类部件级语义标签和精确6-DoF姿态真值。

Result: 对五个代表性任务（目标检测、2D语义分割、RGB-LiDAR融合的3D点云分割、单目深度估计、方向估计）进行基准测试，发现两个关键发现：1)感知小尺度组件（如推进器和全向天线）和在零样本设置中泛化到完全未见过的航天器仍是当前方法的瓶颈；2)增加训练卫星数量对新颖目标的性能有显著提升。

Conclusion: SpaceSense-Bench为空间感知研究提供了大规模、多样化的基准数据集，证明了大规模多样化数据集对提升航天器感知性能的价值。数据集、代码和工具包已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多种视觉语言任务上表现出色，但在低层次视觉感知能力，特别是细粒度视觉差异检测方面尚未得到充分探索和系统分析。

Method: 1. 提出OddGridBench基准测试，包含1400+网格图像，其中单个元素在颜色、大小、旋转或位置等视觉属性上与其他元素不同；2. 提出OddGrid-GRPO强化学习框架，结合课程学习和距离感知奖励，通过渐进控制训练样本难度和空间邻近约束来提升模型性能。
...
Conclusion: OddGridBench和OddGrid-GRPO为推进多模态智能的感知基础和视觉差异敏感性奠定了基础，提供了代码和数据集供进一步研究。
...
TL;DR: 本文提出了一种用于图像回归的测试时自适应方法PSC，通过联合对齐源预测支撑空间和校准正交补空间中的谱松弛，在严重分布偏移下表现优异。


<details>
  <summary>Details</summary>
Motivation: 图像回归的测试时自适应研究远少于分类任务，现有分类方法难以直接迁移到连续回归目标，需要专门针对回归问题的自适应方法。

Method: 提出预测谱校准(PSC)框架，将子空间对齐扩展到块谱匹配，联合对齐目标特征在源预测支撑空间内，并校准正交补空间中的剩余谱松弛。

Result: 在多个图像回归基准测试中，PSC相比强基线方法表现出一致的改进，在严重分布偏移下获得特别明显的性能提升。

Conclusion: PSC是一种简单易实现、模型无关且兼容现成预训练回归器的源无关框架，为图像回归的测试时自适应提供了有效解决方案。
...
TL;DR: 提出M3GCLR框架，通过博弈论对比学习方法解决骨架动作识别中视角差异建模不足、对抗机制缺乏和增强扰动不可控的问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督骨架动作识别方法存在三个主要局限：1) 视角差异建模不足；2) 缺乏有效对抗机制；3) 增强扰动不可控。需要一种更有效的框架来解决这些问题。

Method: 提出M3GCLR框架：1) 建立无限骨架数据博弈模型和均衡定理；2) 通过多视角旋转增强生成正常-极端数据对，使用时序平均输入作为中性锚点实现结构对齐；3) 构建强对抗性最小-最大骨架数据博弈；4) 引入双损失均衡优化器优化博弈均衡。

Result: 在NTU RGB+D 60数据集上达到三流82.1%(X-Sub)和85.8%(X-View)准确率；在NTU RGB+D 120数据集上达到72.3%(X-Sub)和75.0%(X-Set)准确率；在PKU-MMD Part I和II上分别达到89.1%和45.2%准确率，均匹配或超越现有最佳性能。

Conclusion: M3GCLR通过博弈论对比学习框架有效解决了现有方法的局限性，在多个基准数据集上取得了优异的性能，消融研究证实了各组成部分的有效性。
...
TL;DR: MIL-PF：一种结合冻结基础模型编码器和轻量级多示例学习头的可扩展框架，用于高分辨率乳腺X线影像分类，显著降低训练复杂度


<details>
  <summary>Details</summary>
Motivation: 现代基础模型提供高度表达性的视觉表示，但将其适应于高分辨率医学影像面临挑战：标注有限、监督弱、乳腺影像尺寸大、多视图变化、主要使用乳腺级别标签，端到端微调计算成本高且不实用

Method: 提出MIL-PF框架：使用冻结的基础编码器预计算语义表示，仅训练轻量级多示例学习头（4万参数），通过注意力机制聚合全局组织上下文和稀疏局部病灶信号

Result: 在临床规模上实现最先进的分类性能，同时显著降低训练复杂度，代码已开源确保可复现性

Conclusion: MIL-PF为高分辨率医学影像分析提供了一种高效可扩展的解决方案，平衡了表达能力和计算效率，特别适用于标注有限的场景
...
TL;DR: SinGeo是一个简单而强大的跨视角地理定位框架，通过双判别学习架构和课程学习策略，使单个模型能够实现鲁棒的跨视角地理定位，无需额外模块或显式变换。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法依赖于特定视场训练范式，模型在固定视场下优化，但在未见过的视场和未知方向下会失效，需要部署多个模型来覆盖不同变化。虽然已有研究探索通过简单随机化视场进行动态训练，但未能实现跨多样化条件的鲁棒性，隐含假设所有视场难度相同。

Method: SinGeo采用双判别学习架构，增强地面和卫星分支内的视图内判别能力，并首次引入课程学习策略实现鲁棒跨视角地理定位。该框架无需额外模块或显式变换。

Result: 在四个基准数据集上的广泛评估显示，SinGeo在多样化条件下取得了最先进的结果，显著优于专门为极端视场训练的方法。此外，SinGeo还表现出跨架构可迁移性。

Conclusion: SinGeo通过简单而有效的框架实现了鲁棒的跨视角地理定位，超越了现有方法。研究还提出了一致性评估方法来定量评估模型在不同视图下的稳定性，为理解和推进未来CVGL研究的鲁棒性提供了可解释的视角。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机在高速运动和极端光照条件下具有优越的灵敏度，但事件单目深度估计面临密集深度标注稀缺的挑战。现有免标注方法将事件流作为独立帧处理，忽略了事件数据固有的时间连续性，无法利用视觉基础模型中丰富的时空先验，导致深度预测时间不一致且准确性不足。
...
Conclusion: EventVGGT通过将事件流建模为视频序列并蒸馏VGGT的时空先验，显著提升了事件单目深度估计的准确性和时间一致性，为解决事件相机深度估计中的标注稀缺和时间不一致问题提供了有效方案。
...
TL;DR: MIDAS是一个无需训练的扩散模型框架，实现多图像隐藏和用户特定访问控制的覆盖式隐写术，通过潜在空间融合和随机基机制提升安全性和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有覆盖式图像隐写术缺乏鲁棒的访问控制机制，难以在多用户环境下选择性向不同授权用户展示不同隐藏内容，这在可扩展性和隐私敏感的信息隐藏应用中至关重要。

Method: 提出MIDAS框架：1）随机基机制抑制残留结构信息；2）潜在向量融合模块重塑聚合潜在向量以对齐扩散过程；3）基于扩散模型的无需训练方法实现多图像隐藏和用户特定访问控制。

Result: 实验结果表明MIDAS在访问控制功能、隐写图像质量和多样性、噪声鲁棒性以及抵抗隐写分析方面均优于现有无需训练的覆盖式隐写术基线方法。

Conclusion: MIDAS为访问控制的覆盖式隐写术提供了一个实用且可扩展的解决方案，在多用户设置中实现了更好的隐私保护和选择性信息揭示。
...
Conclusion: 大规模模型方法在复杂布局文档图像翻译中展现出巨大潜力，为未来研究提供了重要机会，表明端到端文档图像翻译是一个快速发展的研究领域。
...
TL;DR: RiO-DETR是首个实时定向目标检测的DETR模型，通过任务原生设计解决DETR适应定向边界框的三个挑战，在保持实时效率的同时实现更好的速度-精度平衡。


<details>
  <summary>Details</summary>
Motivation: 将DETR应用于定向边界框检测面临三个主要挑战：语义依赖的方向性、破坏标准欧几里得细化的角度周期性，以及扩大搜索空间导致收敛缓慢。需要设计既能解决这些问题又能保持实时效率的方法。

Method: 1. 内容驱动角度估计：将角度与位置查询解耦，配合旋转校正正交注意力捕获可靠方向的互补线索；2. 解耦周期性细化：结合有界粗到细更新和最短路径周期性损失，实现跨角度接缝的稳定学习；3. 定向密集O2O：向密集监督注入角度多样性，在不增加额外成本的情况下加速角度收敛。

Result: 在DOTA-1.0、DIOR-R和FAIR-1M-2.0数据集上的广泛实验表明，RiO-DETR为实时定向检测建立了新的速度-精度平衡。

Conclusion: RiO-DETR是首个实时定向检测的DETR模型，通过任务原生设计有效解决了DETR适应定向边界框的三个核心挑战，在保持实时效率的同时实现了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有DLA方法直接将不同领域的公开数据集合并训练，忽略了不同领域间布局结构的差异（如标注风格、文档类型、语言等），导致模型性能不佳。需要一种能够有效整合领域先验知识的方法来提高跨领域泛化能力。

Method: 提出PromptDLA，包含一个独特的领域感知提示器，根据数据域的具体属性定制提示。这些提示作为线索，引导DLA关注数据中的关键特征和结构，从而增强模型在不同领域的泛化能力。

Result: 在DocLayNet、PubLayNet、M6Doc和D$^4$LA等数据集上的大量实验表明，该方法达到了最先进的性能。

Conclusion: PromptDLA通过领域感知提示器有效整合领域先验知识，显著提高了文档布局分析模型在跨领域场景下的泛化能力和性能表现。
...
TL;DR: 该论文提出了开放世界运动预测新设定，通过端到端类增量学习框架解决自动驾驶中对象类别随时间演变的问题，有效缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测方法在封闭世界假设下工作，假设固定的对象分类和高质量感知输入。但在现实世界中，感知不完美且对象分类随时间演变，这些方法难以适应。需要解决开放世界中的运动预测问题。

Method: 提出首个端到端类增量运动预测框架：1) 使用伪标签策略为新类别生成运动预测标签；2) 利用视觉语言模型过滤不一致和过度自信的预测；3) 采用基于查询特征方差的重放采样策略，选择具有信息量运动模式的先前序列来缓解灾难性遗忘。

Result: 在nuScenes和Argoverse 2数据集上的广泛评估表明，该方法成功抵抗灾难性遗忘，在保持已学习类别性能的同时，提高了对新类别的适应能力。还支持零样本迁移到真实世界驾驶，并可扩展到端到端类增量规划。

Conclusion: 该研究提出了开放世界运动预测的新设定和解决方案，通过类增量学习框架使自动驾驶系统能够持续适应不断演变的现实世界环境，为自动驾驶系统的持续学习提供了重要进展。
...
TL;DR: 提出一种用于轨迹预测的元学习框架，通过双层优化和自适应模型更新机制，在测试时实现快速准确的在线适应，有效应对分布偏移问题。
...
Method: 1. 提出元学习框架，通过在预训练期间对模拟测试时适应任务的性能进行双层优化，直接优化预测器以实现快速准确的在线适应；2. 在测试时引入数据自适应模型更新机制，基于在线偏导数和困难样本选择动态调整预定义的学习率和更新频率，使学习率适应测试数据并专注于信息丰富的困难样本以提高效率。

Result: 在nuScenes、Lyft和Waymo等多个具有挑战性的跨数据集分布偏移场景中进行实验。结果显示，该方法在适应准确性方面优于最先进的测试时训练方法，同时在次优学习率和高FPS需求下表现出色，展示了其鲁棒性和实用性。

Conclusion: 该方法通过元学习框架和自适应模型更新机制，有效解决了轨迹预测中的分布偏移问题，实现了快速准确的在线适应，在多个实际场景中表现出优越的性能和实用性。
...
TL;DR: EvoDriveVLA是一个用于自动驾驶的视觉-语言-动作模型框架，通过自锚定视觉蒸馏和先知引导轨迹蒸馏解决感知退化和长期规划不稳定问题，在开环和闭环评估中都取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在自动驾驶中存在两个主要问题：1）解冻视觉编码器后感知性能下降；2）长期规划中累积的不稳定性。需要一种能够同时解决感知退化和规划不稳定性的框架。

Method: 提出EvoDriveVLA框架，包含两个核心组件：1）自锚定视觉蒸馏：利用自锚定教师模型提供视觉锚定约束，通过轨迹引导的关键区域感知来正则化学生表示；2）先知引导轨迹蒸馏：使用未来感知的先知教师模型，结合粗到细的轨迹优化和蒙特卡洛dropout采样，生成高质量轨迹候选，选择最优轨迹指导学生预测。

Result: EvoDriveVLA在开环评估中取得了最先进的性能，并在闭环评估中显著提升了性能表现。

Conclusion: EvoDriveVLA通过协同感知-规划蒸馏框架，有效解决了视觉-语言-动作模型在自动驾驶中的感知退化和规划不稳定问题，为自动驾驶系统提供了更可靠和稳定的解决方案。
...
TL;DR: EvoPrompt是一个新颖的提示学习框架，通过显式引导提示演化轨迹，实现稳定、知识保留的视觉语言模型微调，在少样本学习中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在有限标注数据下的下游任务适应面临挑战，现有的参数高效提示学习方法存在灾难性遗忘预训练知识的问题。
...
Conclusion: 通过显式引导提示演化轨迹，EvoPrompt实现了无遗忘的视觉语言模型适应，为参数高效微调提供了新思路。
...
### [60] [Grounding Synthetic Data Generation With Vision and Language Models](https://arxiv.org/abs/2603.09625)
*Ümit Mert Çağlar,Alptekin Temizel*

Main category: cs.CV

TL;DR: 提出一个基于视觉-语言框架的遥感图像合成数据增强与评估方法，构建了包含40万张图像的大规模数据集ARAS400k，通过语义组合分析、标题冗余最小化和跨模态一致性验证来自动评估合成数据质量。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型受益于数据多样性和数量的增加，但现有的合成数据评估指标通常计算潜在特征相似度，难以解释且不一定与下游任务贡献相关。需要一种可解释的合成数据增强和评估方法。
...
Conclusion: 这项工作为遥感任务（特别是语义分割和图像描述）建立了一个可扩展的基准。数据集和代码已公开，为合成数据增强提供了可解释的评估框架。
...
TL;DR: TopoOR提出了一种新的高阶拓扑结构来建模手术室场景，克服了传统场景图仅支持二元关系的限制，通过高阶注意力机制保持多模态特征，在多个手术任务中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有手术场景图方法受限于严格的二元结构，无法有效建模手术室中复杂的高阶关系和群体交互，同时传统方法在融合多模态数据时会丢失重要的结构信息。

Method: 提出TopoOR框架，将手术室建模为高阶拓扑结构，将实体间交互提升到高阶拓扑单元；设计高阶注意力机制，在分层关系注意力中显式保持流形结构和模态特定特征。

Result: 在无菌违规检测、机器人阶段预测和下一动作预测等任务上，TopoOR显著优于传统图方法和基于LLM的基线方法。

Conclusion: TopoOR提供了一种更富表达力的手术室建模范式，能够更好地处理复杂动态和多模态数据，为安全关键推理任务提供了更精确的结构化表示。
...
TL;DR: KV-Lock是一个无需训练的框架，用于DiT视频扩散模型，通过动态调度背景KV融合比例和CFG尺度，在保持背景一致性的同时提升前景质量


<details>
  <summary>Details</summary>
Motivation: 视频编辑中保持背景一致性同时提升前景质量是核心挑战。全图像信息注入会导致背景伪影，而严格的背景锁定又会限制前景生成能力

Method: 提出KV-Lock框架，基于幻觉检测动态调度：1) 缓存背景KV与新生成KV的融合比例；2) CFG尺度。检测到幻觉风险时，加强背景KV锁定并增强条件引导

Result: 实验验证KV-Lock在多种视频编辑任务中优于现有方法，在保持高背景保真度的同时提升了前景质量

Conclusion: KV-Lock作为无需训练的即插即用模块，可轻松集成到预训练的DiT模型中，有效解决了视频编辑中背景一致性与前景质量的平衡问题
...
TL;DR: Patrologia Graeca Corpus是首个大规模开放的古希腊语OCR和语言学资源，针对19世纪双语（希腊语-拉丁语）排版、字体退化的复杂文献，通过专门流程实现高精度识别，包含600万词元的标注语料库。


<details>
  <summary>Details</summary>
Motivation: 解决Patrologia Graeca（PG）中未数字化卷册的OCR难题，这些文献具有复杂的双语排版和高度退化的多调希腊语字体，现有OCR系统难以处理。

Method: 采用专门流程：结合YOLO-based布局检测和CRNN-based文本识别，处理复杂的希腊语-拉丁语双语布局和退化的多调希腊语字体。

Result: 字符错误率（CER）1.05%，词错误率（WER）4.69%，远超现有多调希腊语OCR系统；生成包含约600万词元的语料库，包含词元化和词性标注，并与完整OCR和布局注释对齐。
...
TL;DR: OmniEarth是一个用于评估遥感视觉语言模型的基准测试，包含感知、推理和鲁棒性三个能力维度，涵盖28个细粒度任务，支持多种任务形式，旨在填补遥感领域系统化评估的空白。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在通用领域表现出色，但在遥感应用领域缺乏系统化的评估基准。现有模型在复杂地理空间任务上表现不佳，需要建立全面的评估体系来推动遥感视觉语言模型的发展。

Method: 构建了包含9,275张高质量图像（包括吉林一号卫星影像）和44,210条人工验证指令的数据集。定义了感知、推理和鲁棒性三个能力维度的28个细粒度任务，支持多项选择VQA和开放式VQA两种任务形式。采用盲测协议和五元语义一致性要求来减少语言偏见。

Result: 系统评估了基于对比学习的模型、通用闭源和开源VLM以及RSVLM。结果显示现有VLM在复杂地理空间任务上仍然存在困难，揭示了遥感应用需要解决的明显差距。

Conclusion: OmniEarth填补了遥感视觉语言模型评估的系统性空白，为模型发展提供了全面的基准测试。现有模型在遥感任务上仍有改进空间，该基准将推动遥感领域视觉语言模型的进一步发展。
...
### [65] [AutoViVQA: A Large-Scale Automatically Constructed Dataset for Vietnamese Visual Question Answering](https://arxiv.org/abs/2603.09689)
...
<details>
  <summary>Details</summary>
Motivation: 视觉问答是基础的多模态任务，早期系统存在语言偏见问题。越南语作为低资源语言，已有多个数据集支持研究，但需要更好的模型架构和评估方法。

Method: 采用基于Transformer的架构，结合文本（如PhoBERT）和视觉（如ViT）预训练模型，在多语言设置下系统比较BLEU、METEOR、CIDEr、Recall、Precision、F1-score等自动评估指标。

Result: 未在摘要中明确说明具体实验结果，但暗示了基于Transformer的多模态融合在越南语VQA任务上取得了显著进展，并探讨了大型语言模型在改进评估指标与人类判断对齐方面的潜力。

Conclusion: 该研究展示了Transformer架构在越南语视觉问答任务中的有效性，同时强调了在多语言环境下系统评估自动指标的重要性，为低资源多模态学习提供了有价值的见解。
...
TL;DR: PruneSID：一种无需训练的双阶段视觉令牌压缩方法，通过语义聚类和组内去冗余，在保持信息多样性的同时显著减少视觉令牌数量，提升视觉语言模型的计算效率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理图像时生成过多视觉令牌导致计算效率低下，现有压缩方法难以平衡重要性保持和信息多样性，需要一种更有效的令牌压缩方案。

Method: 提出PruneSID框架，包含两个阶段：1）主语义成分分析（PSCA）将令牌聚类为语义连贯的组；2）组内非极大值抑制（NMS）在每组中去除冗余令牌并保留关键代表令牌。还包含基于图像复杂度的动态压缩比率机制。

Result: 在LLaVA-1.5上仅保留11.1%令牌达到96.3%准确率，在LLaVA-NeXT上极端压缩率（5.6%）下达到92.8%准确率，比先前方法提升2.5%，预填充速度比原始模型快7.8倍。

Conclusion: PruneSID在无需训练的情况下实现了视觉令牌的高效压缩，平衡了重要性保持和信息多样性，在多种视觉语言模型和图像/视频模态上表现出良好的泛化能力和跨模态适应性。
...
TL;DR: Diagonal Distillation：一种针对视频扩散模型的高效蒸馏方法，通过非对称生成策略（前期多步、后期少步）和隐式光流建模，在保持运动连贯性的同时实现277倍加速，达到31FPS实时生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散蒸馏方法大多基于图像方法，忽略了时间依赖性，导致运动不连贯、长序列误差累积和延迟-质量权衡问题。需要一种能更好利用时间信息的方法来实现高质量实时视频生成。

Method: 提出对角线蒸馏方法：1）非对称生成策略：早期视频块使用更多去噪步数，后期块使用较少步数，让后期块继承早期块的丰富外观信息；2）使用部分去噪块作为后续合成的条件输入；3）通过隐式光流建模在严格步数约束下保持运动质量。

Result: 方法生成5秒视频仅需2.61秒（最高31FPS），相比未蒸馏模型实现277.3倍加速，同时保持了视频质量和运动连贯性。

Conclusion: 对角线蒸馏通过更好地利用视频块间和时间步间的信息，解决了现有视频蒸馏方法的时间依赖性问题，实现了高质量实时视频生成，为视频扩散模型的实时应用提供了有效解决方案。
...
TL;DR: 提出一种高效的大视觉语言模型个性化方法，通过提取模型内部注意力机制中的视觉token作为概念记忆，实现无需额外训练或外部模块的个性化体验


<details>
  <summary>Details</summary>
Motivation: 当前AI助手需要提供个性化体验，但现有方法要么依赖额外训练阶段限制了通用性和可扩展性，要么使用外部预训练模块降低了部署效率

Method: 利用模型内在能力捕获个性化概念，通过模型内部注意力机制提取主要代表目标概念的视觉token，这些token作为特定概念的记忆，使模型能够在测试图像中识别和描述该概念

Result: 在单概念、多概念和视频个性化等多种设置下进行了全面统一评估，相比现有SOTA方法表现出显著性能提升，同时保持最小的个性化开销

Conclusion: 提出了一种高效的大视觉语言模型个性化方法，通过利用模型内部注意力机制提取视觉token作为概念记忆，实现了无需额外训练或外部模块的个性化，在多种设置下表现出色
...
### [69] [MA-EgoQA: Question Answering over Egocentric Videos from Multiple Embodied Agents](https://arxiv.org/abs/2603.09827)
*Kangsan Kim,Yanlai Yang,Suji Kim,Woongyeong Yeo,Youngwan Lee,Mengye Ren,Sung Ju Hwang*

Main category: cs.CV

TL;DR: 本文提出了多智能体自我中心视频理解的新问题，并发布了MA-EgoQA基准数据集来评估模型处理多个同时采集的长时程自我中心视频的能力，同时提出了EgoMAS基线模型。


<details>
  <summary>Details</summary>
Motivation: 随着具身智能模型的发展，人类未来将在工作场所和家庭中与多个具身AI智能体协作。为了确保人类用户与多智能体系统之间更好的通信，需要并行解释来自多个智能体的信息，并为每个查询参考适当的上下文。现有挑战包括有效压缩和通信大量个体感官输入（视频形式），以及正确聚合多个自我中心视频以构建系统级记忆。

Method: 首先正式定义了理解多个同时采集的长时程自我中心视频的新问题。为此引入了MultiAgent-EgoQA（MA-EgoQA）基准数据集，包含1.7k个专门针对多自我中心流的问题，涵盖五个类别：社交互动、任务协调、心理理论、时间推理和环境交互。进一步提出了名为EgoMAS的简单基线模型，利用跨具身智能体的共享记忆和智能体级动态检索。

Result: 通过对MA-EgoQA上各种基线和EgoMAS的综合评估，发现当前方法无法有效处理多个自我中心流，突显了未来需要在跨智能体的系统级理解方面取得进展。

Conclusion: 本文提出了多智能体自我中心视频理解的新研究方向，通过MA-EgoQA基准数据集和EgoMAS基线模型为未来研究奠定了基础，表明当前模型在处理多个同时自我中心视频方面存在不足，需要进一步的技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 文本目标实例导航要求智能体根据自由形式的描述在相同类别干扰物中找到正确的对象实例。现有方法通常将长上下文描述仅用作局部匹配线索，缺乏全局探索指导和精确的3D空间验证。

Method: 1. 计算密集的文本-图像对齐生成价值图，根据整个描述一致性对边界区域排序，引导全局探索；2. 观察候选对象时进行视点感知关系检查：采样合理观察姿态，对齐局部坐标系，仅当从至少一个视点满足空间关系时才接受目标。

Result: 在InstanceNav和CoIN-Bench上达到最先进性能，无需任务特定训练或微调。消融实验表明：(1) 将完整描述编码到价值图中避免了无效运动；(2) 显式的视点感知3D验证防止了语义合理但错误的停止。

Conclusion: 基于几何的空间推理是解决杂乱3D场景中细粒度实例识别的可扩展替代方案，避免了繁重的策略训练或人机交互需求。
...
TL;DR: ACADiff是一个用于阿尔茨海默病诊断的临床感知扩散框架，能够合成缺失的多模态脑成像数据，在极端缺失情况下仍保持鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 多模态神经影像为阿尔茨海默病诊断提供互补信息，但临床数据集经常存在模态缺失问题，需要有效的方法来合成缺失的成像数据。

Method: 提出ACADiff框架，通过自适应临床感知扩散合成缺失的脑成像模态。该框架学习不完整多模态观察与目标模态之间的映射，通过渐进去噪潜在表示，同时关注可用成像数据和临床元数据。采用基于输入可用性的自适应融合机制，结合GPT-4o编码提示的语义临床指导。包含三个专门生成器，支持sMRI、FDG-PET和AV45-PET之间的双向合成。

Result: 在ADNI数据集上评估，ACADiff实现了优越的生成质量，即使在极端80%缺失情况下仍保持鲁棒的诊断性能，优于所有现有基线方法。

Conclusion: ACADiff能够有效解决临床多模态数据中的缺失问题，通过自适应融合和临床指导的扩散模型，为阿尔茨海默病诊断提供可靠的多模态数据合成方案。
...
### [72] [Probing the Reliability of Driving VLMs: From Inconsistent Responses to Grounded Temporal Reasoning](https://arxiv.org/abs/2603.09512)
*Chun-Peng Chang,Chen-Yu Wang,Holger Caesar,Alain Pagani*

Main category: cs.CV

TL;DR: 该研究探讨了视觉语言模型作为驾驶助手时的一致性和时间推理能力，发现现有模型存在响应不一致和有限时间推理的问题，提出了FutureVQA基准数据集和自监督调优方法。


<details>
  <summary>Details</summary>
Motivation: 可靠的驾驶助手应基于观察信息进行时间推理并提供一致响应。研究者质疑当前视觉语言模型在自动驾驶应用中是否真正具备时间推理能力，还是仅仅依赖训练中的记忆模式。

Method: 采用现有评估方法，引入FutureVQA（专门评估未来场景推理的人工标注基准数据集），并提出基于思维链推理的自监督调优方法，无需时间标签即可提升一致性和时间推理能力。

Result: 研究发现：1）视觉语言模型存在响应不一致问题，轻微输入扰动会导致不同答案甚至接近随机猜测；2）模型缺乏时间推理能力，无法从当前观察推理和对齐序列事件；3）视觉理解能力强的模型在时间推理任务上不一定表现最好。

Conclusion: 视觉语言模型作为驾驶助手时存在显著的一致性和时间推理缺陷，需要专门的方法来提升其可靠性。提出的FutureVQA基准和自监督调优方法为解决这些问题提供了有效途径。
...
TL;DR: k-MTR是一种直接从欠采样k空间数据中学习语义表示的新框架，绕过了传统"先重建后分析"流程，实现了从k空间到诊断标签的直接映射。


<details>
  <summary>Details</summary>
Motivation: 传统心脏磁共振（CMR）流程采用"先重建后分析"的范式，需要从欠采样的k空间恢复完整图像，这是一个不适定问题，会引入伪影和信息瓶颈。实际上诊断只需要低维的生理标签，而不是高维像素阵列，因此需要直接从k空间提取诊断信息。

Method: 提出k-MTR框架，将欠采样k空间数据和全采样图像对齐到共享语义流形中。使用大规模模拟数据（42,000个受试者），迫使k空间编码器在潜在空间中直接恢复因欠采样而丢失的解剖信息，绕过显式逆问题。通过潜在空间对齐，从欠采样频率中嵌入高级生理语义。

Result: 在连续表型回归、疾病分类和解剖分割任务中，k-MTR相比最先进的图像域基线方法表现出高度竞争力。证明可以从k空间表示中成功恢复精确的空间几何结构和多任务特征。

Conclusion: k-MTR为任务感知的心脏MRI工作流程提供了稳健的架构蓝图，展示了直接从k空间表示中恢复精确空间几何和多任务特征的可行性，突破了传统重建-分析范式的限制。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要同时建模长程依赖和细粒度边界细节。Transformer虽然缓解了CNN感受野有限的问题，但存在二次计算复杂度高、注意力权重分散到无关区域、损害全局上下文建模等缺陷。现有注意力变体虽降低计算复杂度，却无法抑制冗余计算。此外，传统编码器-解码器架构的融合策略（如简单拼接或求和）无法自适应整合高层语义信息和低层空间细节。

Method: 提出DCAU-Net框架，包含两个核心创新：1）差分交叉注意力（DCA）：计算两个独立softmax注意力图的差异，自适应突出判别性结构；用窗口级汇总token替代像素级key和value token，大幅降低计算复杂度而不损失精度。2）通道-空间特征融合（CSFF）：通过顺序的通道和空间注意力，自适应重新校准来自跳跃连接和上采样路径的特征，有效抑制冗余信息并放大显著线索。

Result: 在两个公开基准测试上的实验表明，DCAU-Net实现了具有竞争力的性能，具有增强的分割准确性和鲁棒性。

Conclusion: DCAU-Net通过创新的差分交叉注意力和通道-空间特征融合策略，有效解决了现有医学图像分割方法在计算效率、注意力机制和特征融合方面的局限性，在保持高效计算的同时提升了分割精度。
...
TL;DR: 该论文针对动态环境中具身问答的挑战，提出了DIVRR框架，通过动态感知的视角细化和相关性引导的自适应记忆选择，在保持推理效率的同时提升对遮挡和动态变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统具身问答评估在时间稳定环境中进行，但在动态、有人类活动的场景中，人类活动和遮挡会引入显著的感知非平稳性：任务相关线索是瞬时的、视角依赖的，而"先存储后检索"策略会过度积累冗余证据并增加推理成本。

Method: 提出了DIVRR框架，包含两个核心组件：1) 动态感知的视角细化，在提交模糊观察前进行验证；2) 相关性引导的自适应记忆选择，仅保留信息性证据。该框架无需训练，在DynHiL-EQA数据集上进行评估。

Result: 在DynHiL-EQA和HM-EQA数据集上的广泛实验表明，DIVRR在动态和静态设置中都持续优于现有基线方法，同时保持较高的推理效率。

Conclusion: DIVRR框架通过结合相关性引导的视角细化和选择性记忆准入，有效解决了动态环境中具身问答的挑战，提高了在遮挡下的鲁棒性，同时通过紧凑内存保持快速推理。
...
TL;DR: 该论文对飞行时间非视距成像方法进行了全面比较研究，在统一框架下评估不同方法的性能，发现它们在相同硬件约束下具有相似的分辨率、可见性和噪声敏感性限制。


<details>
  <summary>Details</summary>
Motivation: 当前飞行时间非视距成像方法存在公式和硬件实现上的异质性，这阻碍了对这些方法理论和实验方面的客观评估。需要建立一个统一的比较框架来理解不同方法的相似性和差异。

Method: 首先建立了统一的飞行时间非视距测量前向模型，讨论了简化假设如何产生可处理的逆模型。将简化模型与Radon变换族联系起来，并迁移到频域与基于相位的虚拟视距成像模型关联。然后在相同硬件设置和相似光子计数条件下，评估选定方法在隐藏场景上的性能。

Result: 实验表明，在相同硬件约束下，现有方法在空间分辨率、可见性和噪声敏感性方面具有相似的局限性。特定方法的差异主要源于方法特定的参数设置。

Conclusion: 该研究为飞行时间非视距成像领域提供了一个参考方法学，有助于未来研究中对现有和新方法进行客观比较。期望这一方法论能成为该领域未来研究的基准。
...
TL;DR: GeoSolver框架通过过程监督强化学习提升遥感视觉语言模型的推理能力，构建Geo-PRM-2M数据集训练细粒度过程奖励模型，实现可验证的逐步推理和测试时扩展。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在遥感解释方面取得了显著进展，但让它们执行复杂的逐步推理仍然极具挑战。现有的链式思维推理方法存在中间步骤视觉忠实性不足的关键瓶颈，需要开发可验证的过程监督方法。

Method: 1) 通过熵引导蒙特卡洛树搜索和针对性视觉幻觉注入构建Geo-PRM-2M大规模标记级过程监督数据集；2) 训练GeoPRM标记级过程奖励模型提供细粒度忠实性反馈；3) 提出Process-Aware Tree-GRPO强化学习算法，结合树结构探索和忠实性加权奖励机制，精确分配中间步骤的信用。

Result: GeoSolver-9B模型在多种遥感基准测试中达到最先进性能。GeoPRM实现了稳健的测试时扩展，作为通用地理空间验证器，不仅能无缝扩展GeoSolver-9B的性能，还能直接增强通用视觉语言模型，显示出卓越的跨模型泛化能力。

Conclusion: GeoSolver框架通过过程监督强化学习成功解决了遥感视觉语言模型推理中的视觉忠实性问题，GeoPRM作为通用验证器为领域提供了强大的测试时扩展能力，推动了可验证遥感推理的发展。
...
Method: 提出GeoAlignCLIP框架，学习多粒度语义对齐并引入模态内一致性，实现图像区域与文本概念之间更精确的视觉-语义对齐；同时构建了RSFG-100k细粒度遥感数据集，包含场景描述、区域级标注和挑战性负样本。

Result: 在多个公开遥感基准测试上的广泛实验表明，GeoAlignCLIP在各种任务中始终优于现有的遥感特定方法，展现出更鲁棒和准确的细粒度视觉语言对齐能力。

Conclusion: GeoAlignCLIP通过多粒度对齐和模态内一致性有效解决了遥感视觉语言预训练中的细粒度对齐问题，在多个任务上取得了优越性能。
...
Method: 提出全景语言建模（PLM）范式，开发了即插即用的全景稀疏注意力模块，使现有基于针孔的VLMs能够处理等距柱面投影全景图像而无需重新训练。

Result: 实验表明PLM在具有挑战性的全景场景下实现了卓越的鲁棒性和整体推理能力，其理解能力超越了窄视场部分的简单总和。

Conclusion: PLM范式为360°视觉语言推理提供了统一框架，通过全景稀疏注意力模块有效利用全景图像的整体空间关系，在复杂全景场景中表现出色。
...
TL;DR: 提出BinaryAttention方法，将注意力机制中的查询和键二值化到1位，通过位运算替代浮点运算，在保持相似性关系的同时显著降低计算复杂度，实现端到端加速。


<details>
  <summary>Details</summary>
Motivation: Transformer注意力模块的计算复杂度是视觉任务的主要瓶颈，现有方法主要使用8位或4位量化来平衡效率和精度。本文旨在探索更极端的1位量化，通过理论分析表明注意力二值化能够保持基本的相似性关系。

Method: 1. 仅保留查询和键的符号进行注意力计算；2. 用位运算替代浮点点积运算；3. 通过可学习偏置缓解1位量化的信息损失；4. 采用量化感知训练和自蒸馏技术减少量化误差并确保符号对齐的相似性。
...
Conclusion: BinaryAttention为全精度注意力提供了一种高效有效的替代方案，推动了低比特视觉和扩散Transformer的前沿发展。该方法在保持精度的同时显著降低了计算复杂度。
...
TL;DR: 该研究从人类视觉系统的选择性注意机制获得启发，利用DINO视觉Transformer的注意力图模拟眼动扫视，在ImageNet分类任务中通过选择性处理关键区域实现了高效图像处理，性能接近甚至超越全图像处理。


<details>
  <summary>Details</summary>
Motivation: 人类视觉系统在严格代谢约束下仍能实现卓越感知性能，关键在于选择性注意机制和眼动扫视。传统AI系统对整个图像进行平等处理，效率低下。研究旨在从人类视觉系统获得灵感，创建更智能、高效的图像处理模型。

Method: 使用自监督视觉Transformer DINO生成类似人类注视模式的注意力图，模拟眼动扫视机制，在ImageNet标准分类任务中探索选择性处理策略。通过测量连续扫视对模型分类得分的影响，聚焦处理视觉空间中的关键区域。

Result: 选择性处理策略保留了大部分全图像分类性能，在某些情况下甚至能超越全图像处理。与为人类注视预测建立的显著性模型相比，DINO提供了更优越的注视引导来选择信息丰富区域。

Conclusion: 视觉Transformer注意力为生物启发的主动视觉提供了有前景的基础，为高效、神经形态的视觉处理开辟了新方向。DINO的注意力图能有效指导选择性处理，实现接近人类视觉效率的图像分析。
...
TL;DR: 提出基于显式高斯表示的零样本MRI超分辨率框架，平衡数据需求与计算效率，通过物理建模实现高质量重建
...
Method: 使用显式高斯表示，MRI定制的高斯参数嵌入组织物理特性；基于物理的体积渲染策略通过归一化高斯聚合建模MRI信号形成；砖块化顺序无关光栅化方案实现高度并行3D计算

Result: 在两个公共MRI数据集上的实验显示，该方法在重建质量和效率方面均表现优异，证明了其在临床MRI超分辨率中的潜力

Conclusion: 提出的零样本MRI超分辨率框架成功平衡了数据需求与计算效率，通过物理建模的高斯表示和并行化计算，为临床MRI应用提供了有前景的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 量化感知训练与知识蒸馏结合在边缘部署中具有巨大潜力，但在精度敏感的图像恢复任务中，直接应用面临三个关键瓶颈：师生容量不匹配、解码器蒸馏中的空间误差放大，以及量化噪声引起的重建与蒸馏损失之间的优化"拉锯战"。

Method: 提出QDR框架：1) 使用FP32自蒸馏消除容量不匹配；2) 通过解码器自由蒸馏在瓶颈处校正量化误差，防止误差放大；3) 引入可学习幅度重加权动态平衡竞争梯度；4) 设计边缘友好模型，包含轻量级可学习退化门控动态调制空间退化定位。

Result: 在四个图像恢复任务上的实验表明，Int8模型恢复了FP32模型96.5%的性能，在NVIDIA Jetson Orin上达到442 FPS，并将下游目标检测性能提升了16.3 mAP。

Conclusion: QDR框架有效解决了量化感知训练与知识蒸馏在图像恢复任务中的关键挑战，实现了边缘设备上高性能、高效率的图像恢复模型部署。
...
<details>
  <summary>Details</summary>
Motivation: 从视频观测中建模风驱动物体动力学极具挑战性，因为风是不可见的且具有时空变化性，同时物体本身也存在复杂的形变。现有方法难以准确捕捉这种复杂的物理交互过程。

Method: 1) 将风表示为基于网格的物理场，物体表示为基于3D高斯泼溅的粒子系统；2) 使用物质点法(MPM)建模风-物体交互；3) 通过可微分渲染和仿真联合优化时空风力场和物体运动；4) 引入格子玻尔兹曼方法(LBM)作为物理感知约束，确保符合流体动力学定律。

Result: 1) 显著优于现有动态场景建模方法，在重建精度和仿真保真度方面都有明显提升；2) 支持在新风条件下的前向仿真；3) 实现了风重定向等新应用；4) 创建了WD-Objects数据集（合成和真实世界风驱动场景）。

Conclusion: DiffWind为基于视频的风-物体交互建模开辟了新途径，通过物理感知的可微分框架成功解决了风驱动物体动力学的复杂建模问题，在重建和仿真方面都表现出色。
...
TL;DR: VarSplat是一个不确定性感知的3D高斯泼溅SLAM系统，通过显式学习每个泼溅的外观方差，渲染可微的像素级不确定性图，提升在低纹理、透明表面等复杂场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS-SLAM方法隐式处理测量可靠性，导致在低纹理区域、透明表面或复杂反射属性区域中，姿态估计和全局对齐容易产生漂移。

Method: 引入VarSplat系统，显式学习每个泼溅的外观方差，通过总方差定律和alpha合成，通过高效的单通道光栅化渲染可微的像素级不确定性图，该图指导跟踪、子图配准和环路检测关注可靠区域。

Result: 在Replica（合成）和TUM-RGBD、ScanNet、ScanNet++（真实世界）数据集上的实验表明，VarSplat提高了鲁棒性，在密集RGB-D SLAM的跟踪、建图和新颖视角合成渲染方面达到竞争性或优于现有研究。

Conclusion: 通过显式建模不确定性，VarSplat显著提升了3D高斯泼溅SLAM系统在复杂场景下的性能，为可靠区域感知的SLAM提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能从野外视频中恢复准确的整体3D人体运动，但常常无法捕捉精细的关节运动，特别是在脚部。这种限制源于训练数据集中不准确的脚部标注和有限的脚部运动多样性，而脚部运动对于步态分析和动画等应用至关重要。

Method: FootMR通过将2D脚部关键点序列提升到3D来细化现有人体恢复模型估计的脚部运动。该方法避免直接使用图像输入，从而绕过了不准确的图像-3D标注对，能够利用大规模运动捕捉数据。为了解决2D到3D提升的模糊性问题，FootMR将膝盖和脚部运动作为上下文，并仅预测脚部运动的残差。通过使用全局旋转而非父相对旋转来表示关节，并应用广泛的数据增强，进一步提高了对极端脚部姿态的泛化能力。

Result: 实验在MOOF、MOYO和RICH数据集上进行，结果显示FootMR优于现有最先进方法。在MOYO数据集上，FootMR将踝关节角度误差降低了30%，超过了最好的基于视频的方法。

Conclusion: FootMR方法有效解决了现有3D人体运动恢复方法在脚部精细关节恢复上的不足，通过创新的2D到3D提升策略和残差预测机制，显著提高了脚部运动重建的准确性，为步态分析和动画等应用提供了更可靠的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 多模态医学图像融合通过整合结构和功能信息辅助全面诊断，但现有方法存在分辨率退化和模态差异问题。传统方法将图像融合与超分辨率分阶段处理，导致伪影和感知质量下降，在三模态（解剖+功能模态）设置中，频率域不平衡问题更加突出。
...
Conclusion: TriFusionSR通过联合三模态融合与超分辨率，有效解决了医学图像处理中的分辨率退化和模态差异问题，在频率感知的跨模态交互方面表现出色。
...
TL;DR: 本文提出Matrix Attention机制，通过帧级矩阵操作解决视频扩散模型中全局注意力与局部注意力之间的权衡问题，构建了FrameDiT架构，在保持效率的同时提升了视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型面临高保真视频生成的挑战，主要困难在于高效建模复杂的时空动态。现有方法在强大的全3D注意力（计算昂贵）和高效的局部因子化注意力（时间建模有限）之间存在权衡，需要一种既能保持全局时空结构又能适应显著运动的高效注意力机制。

Method: 提出Matrix Attention机制，将整个帧作为矩阵处理，通过矩阵原生操作生成查询、键和值矩阵，在帧级别进行注意力计算而非token级别。基于此构建FrameDiT-G架构，并进一步开发FrameDiT-H，将Matrix Attention与Local Factorized Attention结合以同时捕捉大运动和小运动。

Result: 在多个视频生成基准测试中，FrameDiT-H取得了最先进的结果，提供了改进的时间一致性和视频质量，同时保持了与Local Factorized Attention相当的效率。

Conclusion: Matrix Attention机制有效解决了视频扩散模型中全局与局部注意力之间的权衡问题，FrameDiT架构在保持计算效率的同时显著提升了视频生成的质量和时空一致性，为高保真视频生成提供了有效的解决方案。
...
TL;DR: FetalAgents是一个用于胎儿超声分析的多智能体系统，通过协调专业视觉专家实现诊断、测量和分割任务，并支持端到端的视频流总结和结构化报告生成。
...
Method: 提出FetalAgents多智能体系统，通过轻量级智能体协调框架动态编排专业视觉专家，支持从视频流中自动识别关键帧、多解剖平面分析，并将分析结果与患者元数据结合生成结构化临床报告。

Result: 在八个临床任务的多中心外部评估中，FetalAgents相比专业模型和多模态大语言模型（MLLMs）始终提供最稳健和准确的性能，为胎儿超声分析和报告提供了可审计、工作流程对齐的解决方案。

Conclusion: FetalAgents是首个用于全面胎儿超声分析的多智能体系统，通过智能体协调框架实现了任务特定准确性与端到端临床工作流程多功能性的平衡，为胎儿超声分析提供了有效的自动化解决方案。
...
### [91] [$M^2$-Occ: Resilient 3D Semantic Occupancy Prediction for Autonomous Driving with Incomplete Camera Inputs](https://arxiv.org/abs/2603.09737)
*Kaixin Lin,Kunyu Peng,Di Wen,Yufan Chen,Ruiping Liu,Kailun Yang*

Main category: cs.CV

TL;DR: M²-Occ是一个用于自动驾驶的语义占用预测框架，专门处理多摄像头输入不完整的情况，通过多视角掩码重建和特征记忆模块来保持几何结构和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于摄像头的语义占用预测方法通常假设完整的环视观测，但在实际部署中由于遮挡、硬件故障或通信故障，这种假设很少成立。需要研究在不完整多摄像头输入下的语义占用预测。

Method: M²-Occ包含两个互补模块：1）多视角掩码重建（MMR）模块，利用相邻摄像头间的空间重叠在特征空间中恢复缺失视角的表示；2）特征记忆模块（FMM），引入可学习的记忆库存储类别级语义原型，通过检索和集成这些全局先验来细化模糊的体素特征。

Result: 在nuScenes-based SurroundOcc基准测试中，针对安全关键的缺失后视摄像头设置，M²-Occ将IoU提高了4.93%。随着缺失摄像头数量增加，鲁棒性差距进一步扩大：在缺失5个视角的设置下，IoU提高了5.01%。这些改进不损害完整视角的性能。

Conclusion: M²-Occ能够有效处理不完整的多摄像头输入，在保持几何结构和语义一致性的同时，显著提高了语义占用预测的鲁棒性，为实际自动驾驶部署提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 从自我中心和外部中心互补视角理解人类行为对于开发支持工业工人和增强安全的系统很重要，但目前缺乏在真实工业场景中同时捕捉两种视角的数据集。

Method: 提出了ENIGMA-360数据集，包含180个自我中心视频和180个外部中心视频，这些视频在真实工业场景中采集并时间同步，带有时间和空间标注。

Result: 在三个基础任务上进行了基线实验：1) 时序动作分割，2) 关键步骤识别，3) 自我中心人-物交互检测，展示了现有最先进方法在这个挑战性场景中的局限性。

Conclusion: 结果强调了需要能够稳健理解真实世界环境中自我-外部中心关系的新模型，数据集已公开提供以促进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有程序规划方法主要依赖视觉观察作为输入，但面临视觉相似性导致的歧义问题。作者认为语言描述在潜在空间中能提供更具区分性的表示，因此提出利用语言表达能力来弥合视觉观察与规划之间的差距。

Method: LAP方法包含三个关键步骤：1) 使用微调的视觉语言模型将视觉观察转换为文本描述；2) 预测动作并提取文本嵌入；3) 利用扩散模型基于这些更具区分性的文本嵌入来规划动作序列。

Result: 在CrossTask、Coin和NIV三个程序规划基准测试中，LAP在多个指标和时间跨度上都取得了新的最先进性能，且优势显著。

Conclusion: 语言感知规划通过利用语言描述的区分能力，能有效解决程序规划中的视觉歧义问题，显著提升规划性能，证明了语言在程序规划任务中的重要性。
...
TL;DR: LogoDiffuser：一种无需训练的扩散模型方法，通过图像输入字符和注意力机制控制，实现多语言logo设计生成


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法在多语言logo设计上存在两个主要问题：1）应用创意风格时会扭曲字符几何结构；2）需要额外训练才能支持多语言文本生成。LogoDiffuser旨在解决这些问题，实现无需训练的多语言logo设计生成。

Method: 提出LogoDiffuser方法，基于多模态扩散变换器，采用图像而非文本提示作为输入，确保字符结构控制不受语言限制。方法包括：1）分析联合注意力机制识别核心token；2）注入最具信息量的注意力图来整合字符结构和视觉设计；3）进行注意力图的层级聚合以减轻注意力漂移并获得一致的核心token。

Result: 通过大量实验和用户研究表明，LogoDiffuser在多语言logo生成方面达到了最先进的性能，能够生成视觉和文本元素和谐融合的多语言logo设计。

Conclusion: LogoDiffuser是一种无需训练的方法，通过创新的注意力机制控制，成功解决了多语言logo生成中的字符结构保持和语言通用性问题，为多语言设计生成提供了有效的解决方案。
...
TL;DR: 论文揭示后门攻击中"替代触发器"的存在，即使原始触发器被移除，后门仍然可以通过感知上不同的替代触发器激活，这挑战了现有防御方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前后门防御假设只要中和已知触发器就能移除后门，但作者发现这种触发器中心视角是不完整的，需要探索后门在特征空间中的本质。

Method: 通过对比干净样本和触发样本在特征空间中的表示，估计后门方向；开发特征引导攻击，联合优化目标预测和方向对齐；理论上证明替代触发器的存在性，并通过实验验证。

Result: 替代触发器确实存在且能可靠激活相同后门；移除训练触发器的防御方法往往无法消除后门；替代触发器可以利用潜在的后门特征空间。

Conclusion: 研究结果表明需要开发针对表示空间后门方向的防御方法，而不是仅仅关注输入空间的触发器，这为后门防御提供了新的方向。
...
TL;DR: 本文提出了一种细粒度的音频-视觉学习任务RA-SSU，旨在实现区域感知、帧级别、高质量的声音源理解，并构建了两个数据集和SSUFormer模型。


<details>
  <summary>Details</summary>
Motivation: 现有的音频-视觉学习研究大多关注粗粒度的下游任务，缺乏对场景感知细节的深入探索。为了提供更具体的场景感知细节，需要定义细粒度的音频-视觉学习任务。

Method: 1. 新定义了RA-SSU任务；2. 构建了f-Music和f-Lifescene两个细粒度数据集；3. 提出了SSUFormer模型，包含Mask Collaboration Module和Mixture of Hierarchical-prompted Experts两个模块，支持多模态输入输出。

Result: 在两个新构建的数据集上进行了广泛实验，验证了任务的可行性、数据集的可用性，并证明了SSUFormer在声音源理解基准上达到了SOTA性能。

Conclusion: RA-SSU任务为音频-视觉学习提供了细粒度的研究方向，构建的数据集和提出的SSUFormer模型为该领域的发展提供了重要基础，实现了区域感知的声音源理解。
...
TL;DR: ConfCtrl：一种置信感知的视频插值框架，通过结合置信度加权的点云潜变量和噪声作为条件输入，使用卡尔曼滤波启发的预测-更新机制，在保持相机姿态约束的同时完成不可见区域重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大视角变化下的新视角合成时存在局限：基于回归的方法无法重建未见区域，而相机引导的扩散模型常因噪声点云投影或相机姿态条件不足而偏离预定轨迹。

Method: 提出ConfCtrl框架，将置信度加权的投影点云潜变量与噪声结合作为扩散过程的初始化条件。采用卡尔曼滤波启发的预测-更新机制，将投影点云视为噪声测量，通过学习残差修正来平衡姿态驱动预测与噪声几何观测。

Result: 在多个数据集上的实验表明，ConfCtrl能够生成几何一致且视觉合理的新视角，有效重建大视角变化下的遮挡区域。

Conclusion: ConfCtrl通过置信感知机制使扩散模型能够遵循预定相机姿态同时完成未见区域重建，实现了稳定、几何感知的生成效果。
...
TL;DR: VLM-Loc利用大视觉语言模型的空间推理能力，通过将点云转换为BEV图像和场景图，结合部分节点分配机制，实现基于自然语言描述的3D点云精确定位。
...
Method: 将点云转换为鸟瞰图图像和场景图，编码几何和语义上下文；引入部分节点分配机制，显式关联文本线索与场景图节点，实现可解释的空间推理。

Result: 在CityLoc基准测试中，VLM-Loc相比现有最先进方法展现出更高的准确性和鲁棒性。

Conclusion: VLM-Loc框架通过利用大视觉语言模型的空间推理能力，结合结构化场景表示和显式关联机制，显著提升了文本到点云定位的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中，多模态情感计算面临模态不平衡缺失的问题（某些模态更脆弱或更昂贵），而标准评估通常假设所有模态同等可用，这导致训练偏差和任务级指标无法揭示的问题。
...
Result: 实验表明，在共享缺失率下表现鲁棒的模型，在不平衡条件下仍可能表现出显著的模态不公平和优化不平衡。MissBench与MEI、MLI一起成为在实际不完整模态设置中压力测试和分析多模态情感模型的实用工具。

Conclusion: MissBench框架及其诊断指标为评估多模态情感模型在现实不平衡缺失场景下的性能提供了系统方法，揭示了传统评估方法无法发现的模态公平性和优化平衡性问题。
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在保持强大语义理解和获得强大生成能力之间存在固有权衡，需要解决美学生成与高级智能之间的差距，实现更民主化的多模态能力。

Method: 采用统一上下文建模和模态特定模块化设计，结合解耦视觉表示；集成最先进的多模态大语言模型与专门MMDiT视觉生成头；构建针对高语义密度任务的数据合成管道，采用推理中心范式利用思维链对齐用户意图与视觉生成细节。

Result: 仅使用4B参数的InternVL-U在多种生成和编辑任务上持续优于BAGEL(14B)等超过3倍规模的统一基线模型，同时保持强大的多模态理解和推理能力，实现了优越的性能-效率平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互视频生成方法依赖密集控制信号、模板视频或精心设计的文本提示，限制了灵活性和对新物体的泛化能力。需要一种更灵活、更可控的生成方法。

Method: 1) 使用稀疏运动引导（手腕关节坐标+形状无关物体边界框）；2) 提出对象强调注意力机制增强物体鲁棒性；3) 开发多任务辅助训练策略和专用数据整理流程，利用可靠HOI样本和辅助任务。

Result: 综合实验表明，该方法在多样化任务中实现了高保真度、可控的人-物交互视频生成，优于现有方法。

Conclusion: DISPLAY框架通过稀疏运动引导、对象强调注意力机制和多任务辅助训练，实现了灵活、可控且物理一致的人-物交互视频生成，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 体育作为人类身体和认知能力的极限测试场，为评估视觉语言模型的空间智能提供了天然测试平台。现有空间智能基准在捕捉高强度人体运动和动态物体交互方面存在局限性，需要专门的体育场景数据集来推动模型发展。

Method: 1. 构建CourtSI数据集：利用明确的球场几何作为度量锚点，开发半自动数据引擎重建体育场景，系统覆盖空间计数、距离测量、定位和关系推理等任务；2. 创建CourtSI-Bench评估基准：包含3,686个经过人工验证的高质量QA对；3. 评估25个专有和开源VLM模型；4. 在CourtSI上微调Qwen3-VL-8B模型。

Result: 1. 现有VLM在CourtSI-Bench上表现有限，存在明显的人机性能差距，且从现有空间智能基准的泛化能力有限；2. 在CourtSI上微调Qwen3-VL-8B后，在CourtSI-Bench上的准确率提升了23.5个百分点；3. 微调后的模型能有效泛化到类似但未见过的运动（CourtSI-Ext），并展现出增强的空间感知评论生成能力。
...
TL;DR: WikiCLIP是一个简单高效的开放域视觉实体识别框架，通过结合语言模型嵌入和视觉引导知识适配器，在显著提升性能的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前基于生成方法的视觉实体识别模型虽然性能强大，但计算成本高昂，限制了其可扩展性和实际部署。需要开发更高效的方法来解决开放域视觉实体识别问题。

Method: WikiCLIP采用对比学习范式，利用大型语言模型嵌入作为知识丰富的实体表示，并通过视觉引导知识适配器在补丁级别对齐文本语义与视觉线索。此外，还设计了硬负样本合成机制来增强细粒度区分能力。

Result: 在OVEN等开放域视觉实体识别基准测试中，WikiCLIP显著优于现有基线方法。特别是在OVEN未见集上提升了16%，同时推理延迟比领先的生成模型AutoVER降低了近100倍。
...
TL;DR: 研究发现Chamfer距离作为点云重建的标准损失函数存在梯度结构问题，直接优化可能导致比不优化更差的结果，原因是梯度结构导致多对一塌陷，需要非局部耦合才能解决


<details>
  <summary>Details</summary>
Motivation: Chamfer距离是点云重建、补全和生成的标准训练损失，但直接优化它有时会产生比不优化更差的Chamfer值，这种悖论性失败需要深入理解其根本原因

Method: 通过分析Chamfer距离的梯度结构，发现其存在多对一塌陷问题，这是前向项的独特吸引子；提出抑制塌陷的必要条件是非局部耦合，在2D设置中通过共享基变形实现全局耦合，在3D形状变形中使用可微分MPM先验

Result: 在20个定向对中一致减少了Chamfer差距，在拓扑复杂的龙模型上实现了2.5倍的改进；非局部耦合的存在与否决定了Chamfer优化的成功或塌陷

Conclusion: Chamfer优化的成功取决于是否存在非局部耦合，这为优化点级距离度量的任何流水线提供了实用的设计标准
...
TL;DR: 提出了一种基于关节角度的可解释运动表示方法，将局部特征映射为结构化伪图像，结合预训练Vision Transformers和MaxSim机制，在文本-运动检索任务中实现细粒度对齐和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有文本-运动检索方法多采用双编码器框架，将运动和文本压缩为全局嵌入，忽略了细粒度的局部对应关系，导致准确性降低且检索结果缺乏可解释性。

Method: 提出基于关节角度的可解释运动表示，将关节级局部特征映射为结构化伪图像，兼容预训练Vision Transformers；采用MaxSim令牌级后期交互机制，并通过掩码语言建模正则化增强文本-运动对齐的鲁棒性和可解释性。

Result: 在HumanML3D和KIT-ML数据集上的实验表明，该方法在文本-运动检索任务中优于现有最先进方法，同时能够提供文本与运动之间的可解释细粒度对应关系。

Conclusion: 该方法通过引入可解释的关节角度表示和细粒度对齐机制，有效解决了传统全局嵌入方法的局限性，在提升检索性能的同时增强了结果的可解释性。
...
TL;DR: 本文提出了一种基于边界差异差异（MDD）的无监督域自适应框架，用于将CT标注数据迁移到CBCT领域，以解决介入放射学中CBCT标注数据稀缺的问题，在肝脏分割任务上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 介入放射学中的锥形束计算机断层扫描（CBCT）是一种重要的成像方式，但由于其有限的视野、特定伪影和动脉内对比剂注射等特点，与传统的CT存在差异。虽然CT有丰富的公开标注数据集，但介入CBCT数据稀缺且大多未标注，现有数据集主要集中在放射治疗应用。这限制了CBCT在介入放射学中的应用。

Method: 提出了一种基于边界差异差异（MDD）的新型无监督域自适应框架。该方法利用未标注的介入CBCT扫描和标注的CT数据，通过重新制定原始MDD优化框架来弥合模态差距。该方法在CT和CBCT数据集上进行肝脏分割实验，在无监督域自适应和少样本设置中都进行了评估。

Result: 实验结果表明，该方法在肝脏分割任务上实现了最先进的性能，不仅在无监督域自适应设置中表现出色，在少样本设置中也取得了良好效果。

Conclusion: 提出的基于MDD的无监督域自适应框架有效地解决了介入放射学中CBCT标注数据稀缺的问题，通过利用CT标注数据和未标注CBCT数据，成功提升了CBCT肝脏分割的性能，为介入放射学中的图像分析提供了有效的解决方案。
...
TL;DR: 该研究提出"全切片难度"概念，基于专家与非专家病理学家之间的诊断分歧，通过多任务学习和加权分类损失两种方法改进前列腺癌Gleason分级的多实例学习模型性能。


<details>
  <summary>Details</summary>
Motivation: 在组织病理学中，全切片图像通常由专家病理学家标注，但非专家可能难以诊断，导致标注者之间存在分歧。这种诊断难度差异可能影响多实例学习模型的性能，特别是对于更严重的Gleason分级。

Method: 提出"全切片难度"概念，基于专家与非专家病理学家的诊断分歧。采用两种方法：1) 多任务学习方法，同时预测诊断结果和难度；2) 加权分类损失方法，根据难度调整损失权重。应用于前列腺癌Gleason分级任务。

Result: 实验结果表明，在训练中整合全切片难度信息能够持续提升不同特征编码器和多实例学习方法的分类性能，特别是对于更高的Gleason分级（即更差的诊断结果）。

Conclusion: 通过利用专家与非专家之间的诊断分歧来定义全切片难度，并采用多任务学习或加权损失方法，可以有效提升前列腺癌Gleason分级的分类性能，特别是在困难病例上。
...
Method: 提出C2FMAE框架，包含两个协同创新：1)级联解码器按顺序从场景语义到对象实例再到像素细节进行重建，建立明确的跨粒度依赖关系；2)渐进掩码课程动态调整训练重点，从语义引导到实例引导再到随机掩码。构建了包含128万ImageNet-1K图像的大规模多粒度数据集。
...
Method: 提出ReCoSplat模型，包含：1) Render-and-Compare模块，从预测视角渲染当前重建并与输入观测比较，提供稳定条件信号以补偿姿态误差；2) 混合KV缓存压缩策略，结合早期层截断和块级选择性保留，将100+帧的KV缓存减少90%以上。
...
TL;DR: 研究发现稀疏自编码器（SAE）学习的特征主要表示抽象语义而非文本书写形式，通过塞尔维亚语的双文种（拉丁字母和西里尔字母）实验验证了这一结论。


<details>
  <summary>Details</summary>
Motivation: 探究稀疏自编码器（SAE）学习的特征是否代表抽象意义，还是与文本的书写形式（正字法）紧密相关。塞尔维亚语的双文种特性（拉丁字母和西里尔字母可以互换使用，且有近乎完美的字符映射）为这一研究提供了理想的受控测试环境。

Method: 使用塞尔维亚语的双文种作为测试平台，分析Gemma模型家族（270M-27B参数）中SAE特征的激活情况。比较相同意义但不同书写形式（拉丁字母vs西里尔字母）的句子激活的特征重叠度，并与随机基线、同文种内的改写对比。

Result: 1. 不同书写形式的相同句子激活高度重叠的特征，远超随机基线
2. 改变书写形式导致的表征差异小于同文种内的改写差异
3. 跨文种跨改写的比较显示特征重叠显著，排除了记忆效应
4. 这种书写形式不变性随模型规模增大而增强

Conclusion: SAE特征能够在表面词元化之上捕捉语义的抽象层次，优先表示意义而非正字法形式。塞尔维亚语的双文种可作为评估学习表征抽象程度的一般范式。
...
TL;DR: 创建了一个包含10种语言的多语言匿名化基准数据集，通过机器翻译方法生成高质量合成数据，解决医疗数据隐私和低资源语言数据稀缺问题


<details>
  <summary>Details</summary>
Motivation: 由于隐私问题，获取敏感患者数据用于机器学习具有挑战性。需要带有人身份信息标注的数据集来开发和测试匿名化系统，以便安全共享数据并符合隐私法规。真实患者数据获取困难，合成数据可以绕过隐私法规限制。同时，神经机器翻译可以帮助为低资源语言创建高质量数据。

Method: 使用机器翻译方法创建了10种语言的多语言匿名化基准数据集。该方法保留了原始标注，并以文化和上下文适当的形式呈现每个目标语言中的城市和人名。通过医疗专业人员评估翻译质量。

Result: 创建了包含超过2,500个人身份信息标注的多语言基准数据集。医疗专业人员的评估研究证实了翻译质量，包括整体质量以及在翻译和适应个人信息方面的质量。该基准可用于训练标注者、跨机构验证标注以及改进自动个人信息检测性能。

Conclusion: 通过机器翻译方法成功创建了高质量的多语言匿名化基准数据集，解决了医疗数据隐私和低资源语言数据稀缺问题。该基准可用于多种应用，包括训练标注者、验证标注和改进自动检测系统。基准数据和标注指南已公开供进一步研究使用。
...
TL;DR: ConFu是一种新颖的推测解码框架，通过让草稿模型预测生成未来方向来提高LLM推理速度，相比现有方法提升8-11%的接受率。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法中的草稿模型仅基于当前前缀进行预测，容易产生错误累积，导致预测结果逐渐偏离目标模型。需要让草稿模型能够预测生成过程的未来方向。

Method: 提出ConFu框架：1) 引入"沉思令牌"和软提示，让草稿模型以极低成本利用目标模型的未来导向信号；2) 采用MoE的动态沉思令牌机制，实现上下文感知的未来预测；3) 设计包含锚定令牌采样和未来预测复制的训练框架，学习稳健的未来预测。

Result: 在Llama-3 3B和8B模型的各种下游任务中，ConFu相比EAGLE-3提高了8-11%的令牌接受率和生成速度。

Conclusion: ConFu首次将推测解码与连续推理令牌相结合，为加速LLM推理提供了新方向，通过让草稿模型预测未来方向显著提升了性能。
...
TL;DR: SciTaRC是一个专家编写的科学论文表格数据问答基准，需要深度语言推理和复杂计算，当前最先进的AI模型在至少23%的问题上失败，即使是Llama-3.3-70B-Instruct这样的强大模型也在65.5%的任务上失败。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型在处理科学论文中的表格数据时存在明显不足，需要同时进行深度语言理解和复杂计算的任务对现有模型构成挑战，因此需要建立一个专门的基准来评估和推动相关研究。

Method: 创建了SciTaRC基准，包含专家编写的关于科学论文表格数据的问题，这些问题需要深度语言推理和复杂计算。通过测试当前最先进的AI模型（包括代码模型和语言模型）在该基准上的表现，分析失败原因。

Result: 当前最先进的AI模型在至少23%的SciTaRC问题上失败，即使是强大的Llama-3.3-70B-Instruct模型也在65.5%的任务上失败。分析发现存在普遍的"执行瓶颈"：代码模型在原始科学表格上表现脆弱，而自然语言推理模型主要因初始理解问题和计算错误而失败。

Conclusion: 科学表格数据的问答对当前AI模型构成显著挑战，特别是执行瓶颈问题限制了模型的实际应用。需要开发能够更好理解和执行复杂计算的新方法，特别是在处理科学表格数据方面。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动主题分析在可扩展性和可重复性方面面临挑战，现有的LLM自动化方法生成的代码本泛化能力有限且缺乏分析可审计性

Method: 开发了一个自动主题分析框架，结合了迭代代码本优化和完整的溯源追踪机制

Result: 在五个语料库（临床访谈、社交媒体、公开转录本）的评估中，该框架在四个数据集上获得最高综合质量分数；迭代优化在四个数据集上带来统计显著的改进；在两个临床语料库上生成的主题与专家标注的主题一致

Conclusion: 该自动化主题分析框架通过迭代优化和溯源追踪，有效解决了现有方法的局限性，在保持描述质量的同时提高了代码可重用性和分布一致性
...
TL;DR: 提出基于置信度的自适应推理框架，通过分析单条推理轨迹的特征，在保持多路径推理精度的同时减少80%的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有CoT推理方法生成过长的推理路径导致高推理成本，而自一致性方法需要采样多条轨迹带来额外计算开销，需要在精度和效率之间取得平衡。

Method: 提出置信感知决策框架，从单条完整推理轨迹中提取句子级数值和语言特征，训练模型自适应选择单路径或多路径推理，无需额外微调即可泛化到其他数据集。

Result: 实验表明该方法在MedQA、MathQA、MedMCQA和MMLU数据集上保持与多路径基线相当的精度，同时减少高达80%的token使用量。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在互联网规模语料训练后可能表现出系统性偏见，本研究旨在探究模型在合成与生物技术解决方案之间的偏好偏差，并尝试通过微调来增强对生物方法的偏好。

Method: 使用50个精心设计的Bioalignment提示，通过Kelly准则启发式评估框架测量5个前沿模型和5个开源模型的偏见。然后使用包含约2200万token的6366篇PMC文章语料库，对Llama 3.2-3B-Instruct和Qwen2.5-3B-Instruct进行QLoRA微调，分别采用混合语料训练和仅指令格式训练。

Result: 大多数模型未实现生物对齐，表现出偏向合成解决方案的偏见。QLoRA微调显著提高了两个模型对生物解决方案的评分（Holm-Bonferroni校正后p<0.001和p<0.01），且未降低一般能力。

Conclusion: 即使少量微调也能改变模型对生物/仿生方法与合成方法的相对价值评估。虽然研究聚焦于小型开源模型，但方法可扩展至更大模型，用于开发偏向生物方法的模型。研究发布了基准、语料、代码和适配器权重。
...
<details>
  <summary>Details</summary>
Motivation: 传统级联ASR-LLM-TTS系统因VAD分割导致半双工交互和脆弱控制，而端到端全双工模型难以保持对话智能，需要一种既能全双工交互又能保持LLM智能的解决方案

Method: 提出DuplexCascade框架：1）将传统话轮级长交互转换为块级微话轮交互；2）引入对话特殊控制令牌来协调话轮转换和响应时机；3）构建无VAD的级联流式管道

Result: 在Full-DuplexBench和VoiceBench基准测试中，DuplexCascade在全双工话轮转换方面达到最先进水平，在开源语音对话系统中表现出强大的对话智能

Conclusion: DuplexCascade成功解决了全双工交互与LLM智能保持之间的权衡问题，通过微话轮交互和特殊控制令牌实现了既快速又智能的语音对话系统
...
TL;DR: DEO是一种无需训练的方法，通过将查询分解为正负组件并优化查询嵌入，解决否定和排除查询的检索问题，在文本和多模态检索中均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法在处理否定和排除查询时效果不佳，而之前的嵌入适应或微调方法会带来额外计算成本和部署复杂性。

Method: 提出Direct Embedding Optimization (DEO)，一种无需训练的方法，将查询分解为正负组件，并使用对比目标优化查询嵌入。

Result: DEO在NegConstraint上优于基线方法，nDCG@10提升+0.0738，MAP@100提升+0.1028，在多模态检索中Recall@5比OpenAI CLIP提升+6%。
...
TL;DR: 该研究探讨了情感如何影响大语言模型的注意力机制和推理过程，提出了情感平衡的QA数据集和情感正则化框架，以提升模型在情感变化文本上的阅读理解能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常处理情感各异的文本，但现有研究通常将情感仅作为预测目标（如情感分析），而忽视了情感作为潜在因素如何影响模型的注意力分配和推理过程。本研究旨在探索情感如何系统性地改变Transformer模型的注意力几何结构。
...
Result: 实验表明：1. 注意力几何指标（局部性、质心距离、熵）在不同情感间存在差异，并与下游问答性能相关；2. 情感正则化方法在多个QA基准测试中提升了阅读理解能力，在情感变化和非情感变化数据集上均取得改进；3. 该方法在分布偏移下表现稳定，并在多个基准测试中实现了领域内改进。

Conclusion: 情感是影响大语言模型注意力机制和推理过程的重要潜在因素。通过情感正则化框架约束情感条件化的表征漂移，可以有效提升模型在情感变化文本上的阅读理解能力，这一方法在多个基准测试中表现出稳定的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 交错语音语言模型在生成文本和语音标记时需要全Transformer深度解码，由于语音序列较长导致计算成本高昂，需要一种高效的推理加速方法。

Method: 提出SPAR-K框架，采用语音交替深度调度：大多数语音位置在固定的中间层退出，而周期性的全深度"刷新"步骤缓解早期退出导致的分布偏移。

Result: 在Step-Audio-2-mini和GLM-4-Voice上的实验表明，SPAR-K最大准确率下降仅0.82%，同时平均语音解码深度分别降低11%和5%，MOS和WER变化可忽略，无额外计算开销。

Conclusion: SPAR-K有效加速交错语音语言模型推理，保持感知质量；研究表明基于置信度的早期退出策略对语音语言模型效果不佳，需要专门针对语音标记统计特性的设计。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG系统检索非结构化文本块并依赖一次性生成，导致冗余上下文、低信息密度和脆弱的多跳推理。结构化RAG管道虽然能改善基础，但通常需要昂贵且容易出错的图构建，或强加僵化的以实体为中心的结构，与查询的推理链不匹配。

Method: 将查询和文档表示为关系三元组，使用轻量级两级分类法约束实体语义。对于复杂问题，将其分解为有序的三元组子查询序列，通过混合三元组匹配（结合原始三元组的语义相似性和类型化三元组的结构一致性）进行逐步证据选择。通过维护跨步骤的显式实体绑定表来解析中间变量。

Result: 在多个多跳问答基准测试中，TaSR-RAG始终优于强大的RAG和结构化RAG基线，性能提升高达14%，同时产生更清晰的证据归因和更忠实的推理轨迹。

Conclusion: TaSR-RAG通过分类指导的结构化推理框架，在不需要显式图构建或穷举搜索的情况下，有效解决了多跳问答中的证据选择和推理问题，提高了性能并增强了可解释性。
...
TL;DR: LLM生成的标签与人类空间分类标签高度一致，可用于指导扩展空间数据集，通过新增42个场景提升TRPS覆盖度


<details>
  <summary>Details</summary>
Motivation: 研究如何利用大语言模型来扩展多语言空间分类数据集，解决传统方法在场景选择和语言覆盖上的局限性

Method: 使用LLM生成空间关系标签，与人类标签对比验证一致性，基于LLM指导选择新场景和语言，扩展TRPS数据集

Result: LLM生成的标签与人类标签高度一致，新增42个场景的扩展版本比之前两个扩展版本更好地覆盖了可能场景空间

Conclusion: LLM为扩展多语言空间数据集提供了有效工具，为实现包含数十种语言和数百个场景的大规模数据集奠定了基础
...
TL;DR: StateFactory：一种使用语言模型将非结构化观察转换为层次化对象-属性结构的因子化表示方法，通过语义相似度计算奖励，实现跨领域的强奖励泛化能力。


<details>
  <summary>Details</summary>
Motivation: 监督学习的奖励模型可能引入训练数据固有的偏差，限制了在新目标和环境中的泛化能力。本文研究是否仅凭定义良好的世界状态表示就能实现跨领域的准确奖励预测。

Method: 提出StateFactory方法，使用语言模型将非结构化观察转换为层次化的对象-属性结构表示。在这种结构化表示下，奖励可以自然地计算为当前状态与目标状态在层次约束下的语义相似度。

Result: 在RewardPrediction基准测试（涵盖5个不同领域，包含2,454个独特动作-观察轨迹）上，StateFactory在零样本设置下表现优异：相比VLWM-critic和LLM-as-a-Judge奖励模型，分别降低了60%和8%的EPIC距离。奖励质量的提升成功转化为更好的智能体规划性能，在AlfWorld和ScienceWorld上分别实现了+21.64%和+12.40%的成功率提升。

Conclusion: StateFactory诱导的紧凑表示结构实现了强大的奖励泛化能力，能够改善智能体规划性能，为跨领域奖励预测提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在NLP任务中表现出色，但会传播训练数据中的偏见，这在医疗等敏感领域影响重大。现有基准测试通常只评估单一社会健康决定因素（如性别或种族）的偏见，而忽视了这些因素之间的交互作用以及特定情境的评估。

Method: 通过一系列实验，研究使用法国患者记录探究性别与其他社会健康决定因素之间的关系，通过SDoH输入来探测嵌入的刻板印象。

Result: 研究发现：1）可以使用SDoH输入来探测嵌入的刻板印象；2）LLMs依赖嵌入的刻板印象做出性别化决策；3）评估SDoH因素之间的交互作用可以有效地补充现有的LLM性能和偏见评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在现实应用中的广泛部署，需要确保模型既保持道德基础又具备知识意识。研究发现当前LLMs存在过度优先道德推理而忽视常识理解的问题。

Method: 引入CoMoral基准数据集，包含嵌入道德困境中的常识矛盾。评估了10个不同规模的LLMs，分析模型识别矛盾的能力，特别关注叙事焦点偏见（主要角色vs次要角色）。

Result: 现有模型在没有先验信号的情况下难以识别常识矛盾；存在明显的叙事焦点偏见，模型更容易检测到次要角色的常识矛盾而非主要角色（叙述者）的矛盾。
...
TL;DR: 该研究通过建立语音变化随机模型，模拟音素库的历史演变过程，发现音素频率分布的统计规律可能源于历时音变而非优化机制。


<details>
  <summary>Details</summary>
Motivation: 音素频率分布在不同语言中表现出稳健的统计规律性，包括指数尾部的秩频模式和音素库大小与相对熵之间的负相关关系。这些模式的起源尚未得到充分解释，研究者希望探究它们是否源于塑造语音系统的历史过程。

Method: 研究者引入了语音变化的随机模型，模拟音素库的历时演变。初始的朴素模型仅能重现音素秩频分布的一般形状，随后扩展模型加入两个额外假设：功能负荷相关效应和向偏好库大小稳定的趋势。

Result: 扩展后的模型模拟结果既匹配观察到的分布，也捕捉到了音素库大小与相对熵之间的负相关关系。朴素模型只能重现秩频分布的一般形状，但无法捕捉其他经验特性。

Conclusion: 研究结果表明，语音系统的某些统计规律性可能作为历时音变的自然结果出现，而非源于明确的优化或补偿机制。这为理解语言系统的统计特性提供了新的历时演化视角。
...
TL;DR: 研究发现语言模型通过合成数据训练时，即使训练内容语义无关或明确表达相反偏好，学生模型仍会从教师模型继承行为偏好，这引发了模型自生成训练数据的安全担忧。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型通过合成数据训练时，学生模型是否会从教师模型继承行为偏好，即使训练内容语义无关或明确表达相反偏好，以及基于内容的检查是否能检测或阻止这种隐性学习。

Method: 研究方法包括：1）使用教师模型生成特定偏好的系统提示（如喜欢某种动物）；2）让学生模型在语义无关或明确表达相反偏好的转述内容上进行训练；3）通过激进过滤确保转述保真度；4）测量学生模型偏好变化。

Result: 实验结果显示：1）训练来自偏好特定动物的教师模型的转述内容，使学生模型对该动物的偏好增加高达19个百分点；2）即使转述内容语义无关或明确表达不喜欢该动物，传输仍成功；3）即使进行激进过滤确保转述保真度，传输依然发生。

Conclusion: 结论表明：1）基于内容的检查无法检测这种隐性偏好传输；2）即使包含明确相反偏好的内容也无法阻止传输；3）这对模型自生成训练数据的流水线构成安全担忧，因为偏好可能通过看似无害的内容隐性传播。
...
### [129] [ALARM: Audio-Language Alignment for Reasoning Models](https://arxiv.org/abs/2603.09556)
...
Method: 提出自重构方法，将自生成响应转换为音频理解变体，保持分布对齐；融合压缩多个音频编码器以获取更强表示；构建包含6M实例的多任务语料库（2.5M独特提示），覆盖19K小时语音、音乐和声音。

Result: 4B参数ALM在相似规模模型中表现最佳，超越大多数更大的ALM，在相关音频推理基准上保持领先；在MMAU-speech和MMSU基准上获得最佳开源结果，在所有模型中排名第三；同时保持文本能力且训练成本低。

Conclusion: 自重构方法有效解决了推理LLM与音频理解的兼容性问题，通过融合压缩编码器和构建大规模语料库，实现了高性能音频语言模型，在保持文本能力的同时显著提升音频推理性能。
...
### [130] [Build, Borrow, or Just Fine-Tune? A Political Scientist's Guide to Choosing NLP Models](https://arxiv.org/abs/2603.09595)
*Shreyas Meher*

Main category: cs.CL

TL;DR: 该研究比较了在政治学NLP任务中构建领域特定模型与微调通用模型的性能差异，发现两者在常见事件类别上表现相近，差异主要集中在罕见事件类别上。


<details>
  <summary>Details</summary>
Motivation: 政治学者在使用自然语言处理工具时面临选择：是构建领域特定模型、适配现有模型，还是微调通用模型？目前缺乏实证指导来帮助学者在这些方案之间做出权衡决策。

Method: 以冲突事件分类为测试案例，微调ModernBERT在Global Terrorism Database上创建Confli-mBERT，并与当前黄金标准领域特定预训练模型ConfliBERT进行系统比较。

Result: Confli-mBERT准确率为75.46%，ConfliBERT为79.34%。两者在高频攻击类型（如爆炸、绑架）上表现几乎无差异，性能差异主要集中在占所有事件不到2%的罕见事件类别上。

Conclusion: 研究提出了一个实用的决策框架：选择模型不应抽象地追求"更好"，而应基于类别频率、错误容忍度和可用资源的特定交集来决定。模型、训练代码和数据已在Hugging Face公开。
...
### [131] [Surgical Repair of Collapsed Attention Heads in ALiBi Transformers](https://arxiv.org/abs/2603.09616)
*Palmer Schallon*

Main category: cs.CL

TL;DR: BLOOM模型存在系统性注意力崩溃问题，ALiBi位置编码导致31-44%的注意力头过度关注序列开头token。研究者提出手术式重新初始化方法，在单GPU上恢复了98.7%的注意力头功能，甚至能暂时超越原始模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现BLOOM系列Transformer语言模型存在系统性注意力崩溃病理，ALiBi位置编码导致大量注意力头几乎完全关注序列开头token，这限制了模型的表达能力，需要诊断和修复方法。

Method: 提出手术式重新初始化方法：针对性地重新初始化Q/K/V矩阵，清零输出投影，并梯度掩码冻结所有非手术参数。在BLOOM-1b7模型上应用，仅需单GPU进行两次处理。

Result: 方法成功恢复了98.7%的注意力头功能（384个头中的242-379个）。控制实验确认恢复由重新初始化而非训练数据驱动。扩展实验显示重新初始化后的模型在训练困惑度上暂时超越原始模型25%。

Conclusion: BLOOM模型的注意力配置是次优局部最小值，手术式重新初始化能有效修复注意力崩溃问题，甚至提升模型性能。该方法为大型语言模型的诊断和修复提供了有效工具。
...
### [132] [Tracking Cancer Through Text: Longitudinal Extraction From Radiology Reports Using Open-Source Large Language Models](https://arxiv.org/abs/2603.09638)
*Luc Builtjes,Alessa Hering*

Main category: cs.CL

TL;DR: 本文提出了一个完全开源、本地可部署的放射学报告纵向信息提取管道，使用qwen2.5-72b模型按照RECIST标准提取和链接靶病灶、非靶病灶和新病灶数据，在荷兰CT胸腹报告上取得了高准确率。


<details>
  <summary>Details</summary>
Motivation: 放射学报告包含肿瘤负荷、治疗反应和疾病进展的关键纵向信息，但其非结构化叙述格式使自动化分析复杂化。现有最先进的临床文本处理系统多为专有系统，限制了在隐私敏感的医疗环境中的应用。

Method: 使用完全开源、本地可部署的llm_extractinator框架，应用qwen2.5-72b模型按照RECIST标准从放射学报告中提取和链接靶病灶、非靶病灶和新病灶的纵向数据。

Result: 在50对荷兰CT胸腹报告上的评估显示高提取性能：靶病灶属性级准确率为93.7%，非靶病灶为94.9%，新病灶为94.0%。

Conclusion: 开源大语言模型可以在多时间点肿瘤学任务中实现临床有意义的性能，同时确保数据隐私和可重复性。本地可部署的LLMs在从常规临床文本中提取结构化纵向数据方面具有巨大潜力。
...
### [133] [Understanding the Interplay between LLMs' Utilisation of Parametric and Contextual Knowledge: A keynote at ECIR 2025](https://arxiv.org/abs/2603.09654)
*Isabelle Augenstein*

Main category: cs.CL

TL;DR: 该论文探讨语言模型如何平衡其参数化知识与检索到的上下文知识，研究知识冲突问题及解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型规模扩大，理解其内部工作机制变得困难，更新或修正嵌入知识需要高昂的重训练成本。同时，在知识密集型任务中，模型需要整合相关上下文来弥补其不完整或过时的知识，但研究发现模型常常忽略提供的上下文，因为可能与预训练期间学到的记忆相冲突。

Method: 通过评估语言模型中的知识存在性，开发诊断测试来揭示知识冲突，并分析成功使用上下文知识的特征。

Result: 该研究揭示了语言模型参数化知识与上下文知识之间的复杂相互作用，识别了知识冲突的类型（包括与预训练记忆的冲突和内部记忆冲突），并提供了理解模型如何平衡这两种知识的方法。

Conclusion: 理解语言模型如何平衡其参数化知识与检索到的上下文知识对于提高模型在知识密集型任务中的性能至关重要，需要开发更好的方法来处理知识冲突并有效利用上下文信息。
...
### [134] [Automatic Cardiac Risk Management Classification using large-context Electronic Patients Health Records](https://arxiv.org/abs/2603.09685)
...
<details>
  <summary>Details</summary>
Motivation: 克服老年心血管风险管理中手动管理编码的局限性，为临床风险分层提供自动化的替代方案。

Method: 使用3,482名患者的数据集，在荷兰纵向临床叙事数据上比较了三种建模范式：经典机器学习基线、针对大上下文序列优化的专业深度学习架构、零样本设置的通用生成式大语言模型，并评估了将非结构化文本与结构化药物嵌入和人体测量数据整合的后期融合策略。

Result: 定制Transformer架构在F1分数和Matthews相关系数上均优于传统方法和生成式大语言模型，表现出最佳性能。
...
### [135] [Fusing Semantic, Lexical, and Domain Perspectives for Recipe Similarity Estimation](https://arxiv.org/abs/2603.09688)
*Denica Kjorvezir,Danilo Najkov,Eva Valencič,Erika Jesenko,Barbara Koroišić Seljak,Tome Eftimov,Riste Stojanov*

Main category: cs.CL

TL;DR: 该研究开发了结合多种信息源和分析方法的食谱相似性评估方法，通过语义、词汇和领域相似性分析，专家验证显示80%的一致性，为食品行业和个性化饮食提供支持。


<details>
  <summary>Details</summary>
Motivation: 需要开发先进的食谱相似性评估方法，结合不同信息源和分析方法，以支持食品行业的应用需求，包括个性化饮食、营养推荐和自动化食谱生成系统。

Method: 结合语义、词汇和领域相似性分析，通过分析食材、制作方法和营养属性来评估食谱相似性，开发基于Web的界面供领域专家验证结果。

Result: 评估了318个食谱对，专家在255个（80%）案例中达成一致，通过专家评估可以估计词汇、语义或营养相似性在专家决策中的相对重要性。

Conclusion: 该方法在食品行业有广泛应用前景，支持个性化饮食、营养推荐和自动化食谱生成系统的开发，为食谱相似性评估提供了有效的综合方法。
...
TL;DR: ESAinsTOD是一个统一的端到端模式感知指令调优框架，用于通用任务导向对话建模，通过指令对齐和模式对齐机制，在多个基准测试中显著优于现有方法，并展现出优异的泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端任务导向对话建模方法通常针对特定数据集设计，难以适应新的对话场景。需要一种统一的框架来灵活适应各种对话任务流程和模式。

Method: 提出ESAinsTOD框架，采用LLM全参数微调，引入两种对齐机制：指令对齐确保系统忠实遵循任务指令完成异构TOD数据集的任务流程；模式对齐鼓励系统按照指定模式进行预测。采用会话级端到端建模，允许系统访问对话历史中先前执行的任务流程结果。

Result: 在CamRest676、In-Car和MultiWOZ等端到端任务导向对话建模基准测试中显著优于最先进模型；在低资源设置下展现出优异的泛化能力，对齐机制显著提升零样本性能；指令调优范式大幅提高模型对数据噪声和级联错误的鲁棒性。
...
TL;DR: 评估四种大语言模型在食品成分数据库RAG系统中的检索效果，重点关注LLM将自然语言查询转换为结构化元数据过滤器的能力，发现其在可表达约束下表现优异，但在非可表达约束的复杂查询中仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 食品和营养数据复杂且专业，传统方法需要大量人工和技术专业知识。研究旨在探索LLM能否作为可访问的高性能工具，减少食品编译者和营养学家等领域专家利用复杂食品营养数据所需的手动工作和专业技术。

Method: 使用四种大语言模型在专门的检索增强生成系统中评估其检索效果，重点测试LLM将自然语言查询转换为结构化元数据过滤器的能力，通过Chroma向量数据库实现高效检索。

Result: LLM在关键检索步骤中实现了高准确率，在简单和中等复杂度的查询中表现优异，显著减少了领域专家的工作量。但在涉及非可表达约束的困难查询中，可靠检索仍然具有挑战性。

Conclusion: LLM驱动的元数据过滤在约束可明确表达时表现出色，但当查询超出元数据格式的表征范围时仍存在困难。这表明LLM可以作为领域专家访问复杂数据的有效工具，但在处理某些复杂查询时仍有局限性。
...
TL;DR: RbtAct：基于反驳监督的AI同行评审反馈生成方法，通过利用作者对评审意见的反驳作为隐式监督，优化生成具体可操作的评审建议


<details>
  <summary>Details</summary>
Motivation: 当前AI生成的同行评审报告往往过于表面化，缺乏具体可操作性，无法为作者提供切实可行的修改指导。作者希望通过利用作者对评审意见的反驳作为监督信号，优化生成更具体、可执行的评审反馈

Method: 提出RbtAct方法，利用作者反驳作为隐式监督优化反馈生成。构建RMR-75K数据集，将评审片段映射到对应的反驳片段，并标注视角和影响类别。提出视角条件化的片段级评审反馈生成任务。使用Llama-3.1-8B-Instruct模型进行监督微调，然后基于反驳推导的配对进行偏好优化

Result: 人类专家和LLM作为评判者的实验表明，RbtAct在可操作性和具体性方面相比强基线模型有持续提升，同时保持了良好的基础性和相关性

Conclusion: 通过将作者反驳作为监督信号，RbtAct能够生成更具体、可操作的同行评审反馈，为解决当前AI生成评审报告过于表面化的问题提供了有效方法
...
<details>
  <summary>Details</summary>
Motivation: 为支持使用信息论方法研究语言变异，特别是比较书面语和口语模式、研究言语不流畅现象，以及传统的翻译研究，需要更新和整合现有的语料库资源。

Method: 更新合并EPIC-UdS（口语）和EuroParl-UdS（书面语）语料库，修正元数据和文本错误，优化内容，更新语言标注，增加词对齐和词级惊奇度指数等新层。

Result: 创建了双向英德语料库新版本，通过验证重建口语数据的完整性，并评估基于GPT-2和机器翻译模型的概率度量在口译填充词预测任务上的表现。

Conclusion: 更新后的语料库为语言变异研究提供了更完善的资源，特别是在比较书面语和口语、研究言语不流畅现象以及翻译研究方面具有重要价值。
...
Method: 系统包含三个核心组件：1) NL2Bench用于意图结构化和个性化基准规划；2) BenchResolve用于基准解析、自动数据集获取和模式归一化以确保可执行性；3) Metrics & Reporting用于任务感知指标选择和超越标量分数的决策导向报告。系统还包含人机交互检查点用于审查、编辑和回滚，同时保留样本证据轨迹用于调试和审计。

Result: 实验表明，One-Eval能够以最少的用户努力从多样化的自然语言请求执行端到端评估，支持工业环境中更高效和可复现的评估。

Conclusion: One-Eval通过将自然语言评估请求转换为可执行的评估工作流，解决了大语言模型评估中的主要挑战，提高了评估效率和可复现性，特别适用于工业应用场景。
...
TL;DR: 本文研究长上下文推理中的分块顺序问题，提出使用Chow-Liu树学习分块依赖结构，通过广度优先遍历获得的分块顺序能减少信息损失，在三个长上下文基准测试中优于默认文档分块顺序和基于语义分数的排序方法。


<details>
  <summary>Details</summary>
Motivation: 序列多智能体推理框架（如Chain-of-Agents）通过将长上下文查询分解为分块并顺序处理，但受限于有界共享内存，导致信息损失。分块处理顺序会影响最终证据状态，因此需要研究最优的分块排序方法来减少信息损失，提高推理质量。

Method: 使用Chow-Liu树学习分块之间的依赖结构，识别强相关的分块。然后对生成的树进行广度优先遍历，得到优化的分块处理顺序。这种方法优先处理具有强依赖关系的分块，减少信息瓶颈造成的信息损失。

Result: 在三个长上下文基准测试中，基于Chow-Liu树广度优先遍历的分块顺序在答案相关性和精确匹配准确率方面一致优于：1）默认文档分块顺序；2）基于语义分数的排序方法。证明了优化分块顺序能有效减少跨智能体的信息损失。

Conclusion: 分块顺序对长上下文推理质量有重要影响。通过学习分块依赖结构并采用广度优先遍历策略，可以显著减少Chain-of-Agents框架中的信息损失，提高推理性能。这为优化序列多智能体推理系统提供了有效方法。
...
TL;DR: 研究发现，当现代语言模型（如Transformer）在下一词预测方面表现过好时，其计算出的概率反而对预测阅读时间效果变差。作者认为这是因为阅读时间对简单的n-gram统计更敏感，而非最先进Transformer模型学习的复杂统计。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在下一词预测方面表现优异，但其计算出的概率与阅读时间的相关性反而下降。作者旨在解释这一现象，探究阅读时间预测与语言模型复杂度之间的关系。

Method: 通过分析不同神经语言模型的预测，研究其与n-gram概率的相关性，并将这些相关性与人眼追踪测量的自然文本阅读时间指标进行对比。

Result: 研究发现，那些预测与n-gram概率最相关的神经语言模型，其计算出的概率也与基于眼动追踪的阅读时间指标最相关。

Conclusion: 阅读时间主要受简单n-gram统计影响，而非最先进Transformer语言模型学习的复杂统计模式。这解释了为什么过于复杂的语言模型在预测阅读时间时效果反而变差。
...
TL;DR: 该论文介绍了DoWhatISay（DOWIS）数据集，这是一个多语言的人类录制口语和书面提示数据集，用于在口语指令条件下对语音大语言模型进行更真实的评估。


<details>
  <summary>Details</summary>
Motivation: 当前语音大语言模型（SLLMs）通常使用文本提示进行评估，但这不能反映用户实际使用语音交互的真实场景。需要一种更贴近现实的口语指令评估方法。

Method: 研究者创建了DOWIS数据集，涵盖9个任务和11种语言，每个任务-语言对提供10个提示变体，覆盖五种风格。使用该数据集对最先进的SLLMs进行基准测试，分析提示模态、风格、语言和任务类型之间的相互作用。

Result: 结果显示文本提示始终优于口语提示，特别是在低资源和跨语言设置中。只有在具有语音输出的任务中，口语提示才能缩小差距，这突显了在SLLM评估中需要基于语音的提示。

Conclusion: 该研究强调了在语音大语言模型评估中使用口语指令的重要性，并提供了DOWIS数据集作为评估工具，以更真实地反映用户实际使用场景。
...
TL;DR: 前沿大语言模型在政治说服力方面超越传统竞选广告，不同模型表现存在显著差异，信息提示策略的效果因模型而异
...
Result: LLM在说服力方面优于标准竞选广告，Claude模型表现最佳，Grok最差；信息提示策略的效果因模型而异，对Claude和Grok有提升作用，但对GPT反而降低说服力

Conclusion: 研究为前沿模型的潜在说服风险提供了基准，并建立了跨模型比较风险评估框架，强调了模型间差异对政治说服策略的影响
...
TL;DR: 研究发现，在简单的单跳事实性问题中启用推理能力，可以显著提升大语言模型的参数知识回忆能力，通过计算缓冲效应和事实提示两种机制实现，但推理过程中的幻觉事实会增加最终答案的幻觉风险。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索推理在简单单跳事实性问题中的作用。虽然推理在数学、代码生成和多跳事实问题中作用明显，但在不需要逐步逻辑分解的简单事实问题中，推理的效用尚不明确。研究者希望理解为什么推理能帮助参数知识回忆，即使没有复杂的推理步骤需要执行。

Method: 研究方法包括设计一系列假设驱动的控制实验，通过系统性的实验设计来识别推理在简单事实问题中的作用机制。研究者特别关注推理过程中生成的中间内容如何影响最终答案的准确性。

Result: 研究发现两个关键驱动机制：1）计算缓冲效应——模型使用生成的推理标记进行与语义内容无关的潜在计算；2）事实提示——生成相关主题事实作为语义桥梁，促进正确答案检索。同时发现，推理过程中幻觉中间事实会增加最终答案的幻觉可能性。

Conclusion: 研究结论表明，推理能显著扩展模型参数知识回忆的能力边界，解锁原本无法获得的正确答案。研究洞察可用于直接提高模型准确性，通过优先选择包含无幻觉事实陈述的推理轨迹。这揭示了推理在简单事实问题中的意外效用及其潜在风险。
...
TL;DR: 本文提出FUSE四维分类法，系统综述大语言模型时代的模型融合技术，涵盖理论基础、算法策略、应用场景和生态系统，为研究者和实践者提供结构化框架。


<details>
  <summary>Details</summary>
Motivation: 随着微调大语言模型的快速扩散，模型融合技术提供了计算高效的替代方案，能够以最小成本组合专业化能力，避免集成学习和完全重训练的高昂代价。

Method: 采用FUSE四维分类法：理论基础（损失景观几何、模式连接性等）、统一策略（权重平均、任务向量算术、稀疏化增强方法、专家混合架构、进化优化等）、应用场景（多任务学习、安全对齐、领域专业化等）、生态系统（开源工具、社区平台、评估基准）。

Result: 系统梳理了模型融合的理论基础、算法家族、代表性工作、实际权衡，并分析了多任务学习、安全对齐、领域专业化等下游应用，以及支持生态系统。

Conclusion: 本文为研究者和实践者提供了推进模型融合的结构化基础，并识别了理论空白、可扩展性障碍和标准化需求等关键开放挑战。
...
TL;DR: CREATE是一个评估模型创造性联想推理能力的基准测试，要求模型在概念间生成具有高特异性和多样性的连接路径，模拟真实创造力任务的需求。


<details>
  <summary>Details</summary>
Motivation: 创造力需要联想推理能力——在概念间建立新颖且有意义的连接。目前缺乏评估模型这种创造性联想能力的客观基准测试，而真实创造力任务如假设生成具有极大搜索空间，难以直接评估。

Method: 设计CREATE基准测试，要求模型在参数知识中生成连接概念的多条路径。路径需具备高特异性（连接独特且紧密）和高多样性（路径间差异大），模型根据生成大量强大多样路径的能力评分。

Result: 前沿模型在CREATE上表现差异明显，最强模型获得更高的创造性效用。由于答案多样性和搜索复杂性，基准难以饱和。思维模型并不总是更有效，即使有高token预算。创造性提示方法带来有限改进。

Conclusion: CREATE为开发新方法提升模型联想创造力提供了沙盒环境，能够客观评估模型在模拟真实创造力任务需求方面的表现，有助于推动模型创造性能力的发展。
...
TL;DR: MathQ-Verify是一个五阶段流水线，专门用于过滤数学问题中定义不清或表述不当的问题，通过格式验证、形式化、逻辑矛盾检测等步骤确保问题的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注生成正确的推理路径和答案，但忽视了问题本身的有效性。许多数学问题存在定义不清、信息不足或逻辑矛盾等问题，这会影响训练数据的质量。

Method: 提出MathQ-Verify五阶段流水线：1) 格式级验证，去除冗余指令；2) 问题形式化，分解为原子条件；3) 基于数学定义验证条件；4) 检测逻辑矛盾；5) 目标导向的完整性检查，确保问题提供足够解决信息。

Result: 在多个基准测试中达到最先进性能，F1分数比直接验证基线提高最多25个百分点。通过轻量级模型投票方案，实现约90%的精确率和63%的召回率。

Conclusion: MathQ-Verify为构建可靠的数学数据集提供了可扩展且准确的解决方案，能有效减少标签噪声，避免在无效问题上进行不必要的计算。
...
TL;DR: MASEval是一个框架无关的评估库，用于系统级比较LLM智能体系统的实现选择（如拓扑结构、编排逻辑等），发现框架选择与模型选择同等重要。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体系统生态快速发展，但现有基准测试都是模型中心的，固定智能体设置而不比较其他系统组件。作者认为实现决策（如拓扑结构、编排逻辑、错误处理等）对性能有重大影响，需要系统级评估方法。

Method: 开发了MASEval框架无关库，将整个系统作为分析单元。通过系统级比较，在3个基准测试、3个模型和3个框架上进行实验，探索智能体系统的所有组件。

Result: 研究发现框架选择与模型选择同等重要，实现决策显著影响智能体系统性能。MASEval为研究人员提供了探索智能体系统所有组件的工具。

Conclusion: MASEval填补了智能体系统评估的空白，为研究人员提供了系统设计的新途径，帮助实践者为特定用例选择最佳实现方案。该工具以MIT许可证开源。
...
TL;DR: LLM Delegate Protocol (LDP) 是一种AI原生通信协议，通过引入五种机制来改进多智能体系统中的委托效率和管理能力。


<details>
  <summary>Details</summary>
Motivation: 当前的多智能体协议（如A2A和MCP）没有将模型级属性作为一等公民，忽略了有效委托的基本属性：模型身份、推理配置、质量校准和成本特征。

Method: 提出LLM Delegate Protocol (LDP)，包含五种机制：1) 包含质量提示和推理配置的丰富委托身份卡；2) 具有协商和回退功能的渐进式负载模式；3) 具有持久上下文的治理会话；4) 跟踪置信度和验证状态的结构化溯源；5) 在协议级别强制执行安全边界的信任域。

Result: 身份感知路由在简单任务上实现约12倍延迟降低；语义帧负载减少37%的token数量；治理会话在10轮对话中消除39%的token开销；噪声溯源会降低合成质量；模拟分析显示攻击检测（96% vs. 6%）和故障恢复（100% vs. 35%完成率）方面的架构优势。
...
TL;DR: Guardian系统为失踪儿童案件提供端到端决策支持，通过三层预测架构（马尔可夫链、强化学习、LLM验证）生成0-72小时内的概率搜索计划。
...
Method: 系统将异构非结构化案件文档转换为时空表示，通过三层预测架构：1）考虑道路可达性、隐蔽偏好和走廊偏见的马尔可夫链；2）将预测分布转化为可操作搜索计划的强化学习；3）LLM进行事后验证。

Result: 在合成但真实的案例研究中，系统在24/48/72小时时间范围内产生定量输出，分析显示三层架构为区域优化和人工审查提供了可解释的先验概率。

Conclusion: Guardian系统通过整合异构数据、地理编码和三层预测架构，为失踪儿童调查提供了有效的端到端决策支持工具，生成可解释且可操作的搜索计划。
...
TL;DR: 论文提出个人智能体操作系统（AgentOS）新范式，用自然用户界面替代传统GUI/CLI，以智能体内核为核心协调多个智能体，将传统应用转化为模块化技能，将操作系统构建问题转化为知识发现与数据挖掘问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体（如OpenClaw）虽然能自主操作本地计算环境，但仍运行在为GUI/CLI设计的传统操作系统上，导致交互模型碎片化、权限管理混乱（"影子AI"）和严重的上下文碎片化问题。
...
Conclusion: AgentOS代表了人机交互的关键转折点，其实现为KDD社区定义了新的研究议程，包括意图挖掘、知识发现、工作流自动化等挑战，为构建下一代智能计算系统指明了方向。
...
TL;DR: Guardian是一个端到端系统，使用LLM管道支持失踪儿童调查和早期搜索规划，通过多模型协调和共识机制提高信息提取准确性
...
Method: 采用多模型LLM管道，协调任务专用LLM模型，引入共识LLM引擎解决模型间分歧，使用QLoRA微调和精选数据集增强性能

Result: 设计了一个保守、可审计的LLM使用框架，将LLM作为结构化提取器和标注器，而非不受约束的端到端决策者

Conclusion: Guardian LLM管道为失踪人员调查提供了智能信息处理系统，强调LLM在关键任务中的可控、可审计应用
...
<details>
  <summary>Details</summary>
Motivation: 神经反馈系统的后向可达性分析研究不足，现有技术可扩展性有限，需要开发新的算法来填补这一空白。

Method: 提出新算法计算非线性神经反馈系统的后向可达集的上界和下界近似，并将后向可达性技术与现有前向分析技术集成，形成FaBRIC算法。
...
### [155] [Meissa: Multi-modal Medical Agentic Intelligence](https://arxiv.org/abs/2603.09018)
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗智能体系统过度依赖GPT等前沿模型的API，导致高成本、高延迟和隐私风险，无法满足临床本地化部署需求。

Method: 提出统一轨迹建模、三层分层监督和前向-回顾监督方法，从40K精选轨迹中学习策略选择和执行的智能体能力。

Result: 在13个医疗基准的16个评估场景中，Meissa在10个场景中匹配或超越前沿专有智能体，参数量减少25倍以上，端到端延迟降低22倍。

Conclusion: Meissa成功实现了轻量级离线医疗智能体系统，为临床环境提供了低成本、低延迟、高隐私保护的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多轮多智能体LLM游戏评估存在显著的运行间方差问题，早期微小偏差会在长时程交互中累积并被多智能体耦合放大，导致胜率估计偏差和排名不可靠，提示选择进一步加剧了这一问题

Method: MEMO（记忆增强的模型上下文优化）是一个自对弈框架，通过耦合保留和探索来优化推理时上下文。保留维护持久记忆库存储自对弈轨迹的结构化见解，并在后续对弈中作为先验注入；探索运行锦标赛式提示演化，通过TrueSkill进行不确定性感知选择，并使用优先回放重新访问罕见和决定性状态

Result: 在五个文本游戏中，MEMO将GPT-4o-mini的平均胜率从25.1%提升至49.5%，Qwen-2.5-7B-Instruct从20.9%提升至44.3%（每任务2000场自对弈游戏）。运行间方差显著下降，跨提示变体的排名更加稳定
...
TL;DR: DataFactory是一个多智能体框架，通过团队协作和自动知识转换解决表格问答中的上下文限制、幻觉问题和复杂推理挑战。
...
Method: 提出DataFactory多智能体框架，包含采用ReAct范式的数据领导者、专用数据库团队和知识图谱团队，将复杂查询分解为结构化推理任务。通过映射函数T:D×S×R→G实现自动数据到知识图谱转换，采用自然语言咨询实现灵活智能体间协商，并应用上下文工程策略整合历史模式和领域知识。

Result: 在TabFact、WikiTableQuestions和FeTaQA三个数据集上，使用来自五个提供商的八个LLM，结果显示：准确率比基线提高20.2%（TabFact）和23.9%（WikiTQ），效应量显著（Cohen's d > 1）。团队协作优于单团队变体（+5.5% TabFact，+14.4% WikiTQ，+17.1% FeTaQA ROUGE-2）。

Conclusion: DataFactory框架为多智能体协作提供了设计指南，通过集成结构化查询和图知识表示，为企业数据分析提供了实用平台，有效解决了现有表格问答系统的局限性。
...
TL;DR: 论文提出RAISE框架，揭示逻辑推理能力提升与AI情境意识增强之间的危险关联，指出当前安全措施不足，并提出安全建议


<details>
  <summary>Details</summary>
Motivation: AI情境意识被认为是高级AI系统中最危险的新兴能力之一，而逻辑推理能力提升的研究正在快速发展。本文旨在揭示这两个研究方向之间的危险交汇点，即逻辑推理能力的提升会直接增强AI的情境意识，从而带来安全风险。

Method: 提出RAISE框架，识别三种机制路径：演绎自我推理、归纳情境识别和溯因自我建模。形式化每条路径，构建从基本自我认知到战略欺骗的升级阶梯，并展示LLM逻辑推理的主要研究主题如何直接映射到情境意识的特定放大器。

Result: 分析表明，当前安全措施不足以防止这种升级，逻辑推理能力的提升会系统地增强AI的情境意识，包括自我认知、环境理解和战略推理能力。

Conclusion: 逻辑推理研究社区需要正视其研究对AI安全的影响，提出了具体的安全保障措施，包括"镜像测试"基准和推理安全对等原则，呼吁社区承担相应责任。
...
TL;DR: 论文提出BD-FDG框架，通过结构化知识组织、认知分层问题建模和自动化质量控制，构建高质量领域微调数据集，应用于空间态势感知领域，显著提升LLM的领域性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用任务上表现出色，但在复杂工程领域（如空间态势感知）的迁移面临挑战，主要瓶颈在于高质量监督微调数据集的构建不足，存在知识覆盖不全、认知深度浅、质量可控性差等问题。

Method: 提出BD-FDG框架，包含三个机制：1) 使用知识树确保结构化语料覆盖；2) 设计跨越9个类别和6个认知层次（从记忆到创造）的问题生成方案，产生连续难度梯度的样本；3) 应用多维评分流水线强制执行领域严谨性和一致性。

Result: 构建了约23万样本的SSA-SFT数据集，微调Qwen3-8B得到SSA-LLM-8B。在领域测试集上，相对BLEU-1提升144%（无思考）和176%（有思考），竞技场比较中胜率82.21%，同时基本保持通用基准性能（MMLU-Pro, MATH-500）。

Conclusion: 认知分层驱动的SFT数据构建是复杂工程领域LLM适应的有效范式，为领域特定LLM适应提供了可迁移的框架。
...
TL;DR: 论文提出ToMBench-Hard对抗性基准和Social-R1强化学习框架，通过挑战性训练案例和轨迹级对齐，使4B参数模型超越更大模型，实现高效可靠的社会智能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在社会智能方面仍面临挑战，当前模型依赖表面模式而非真正的社会推理。需要培养类人社会智能，这要求使用抵抗捷径解决方案的挑战性案例进行训练。

Method: 1. 引入ToMBench-Hard对抗性基准，为社交推理提供困难训练示例；2. 提出Social-R1强化学习框架，通过多维奖励将模型推理与人类认知对齐；3. 监督整个推理过程，强制执行结构对齐、逻辑完整性和信息密度。

Result: 4B参数模型超越更大模型，在八个多样化基准测试中展现出强大的泛化能力。
...
TL;DR: Logos是一个紧凑的分子推理模型，通过多步逻辑推理与严格化学一致性相结合，在保持化学有效性的同时提供透明推理过程，优于参数更多的大型通用语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型在分子设计方面存在局限性：要么物理保真度高但推理不透明，要么推理灵活但缺乏化学有效性保证。这种不平衡限制了AI系统在实际科学设计工作流程中的可靠性。

Method: 采用分阶段训练策略：1) 先让模型接触连接分子描述与结构决策的显式推理示例；2) 逐步对齐这些推理模式与分子表示；3) 将化学规则和不变性直接纳入优化目标，引导模型产生化学有效输出。

Result: 在多个基准数据集上，Logos在结构准确性和化学有效性方面表现优异，匹配或超越了参数更多的大型通用语言模型。在涉及多个可能冲突约束的分子优化任务中表现出稳定行为，并能通过展示中间推理步骤实现人工检查。
...
TL;DR: 研究发现LLM口头置信度评分存在严重离散化问题，78%以上回答集中在三个整数上。通过系统操控置信度量表的三个维度，发现0-20量表比标准0-100格式能提高元认知效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM口头置信度评估中，置信度量表（通常是0-100）很少被检验，但作者发现这种设计选择并非中性，需要系统研究量表设计对口头不确定性质量的影响。

Method: 在6个LLM和3个数据集上分析口头置信度的离散化现象，然后系统操控置信度量表的三个维度：粒度、边界位置和范围规律性，使用元d'评估元认知敏感性。

Result: 口头置信度严重离散化，78%以上回答集中在三个整数上；0-20量表比标准0-100格式能提高元认知效率；边界压缩会降低性能；整数偏好即使在非规律范围内仍然存在。

Conclusion: 置信度量表设计直接影响口头不确定性的质量，应该被视为LLM评估中的首要实验变量，需要更仔细地考虑量表设计选择。
...
TL;DR: 论文质疑线性表示假设，提出非线性激活空间几何分析，开发基于多项式核PCA的Curveball steering方法，相比线性方法表现更优


<details>
  <summary>Details</summary>
Motivation: 现有激活导向方法依赖线性表示假设，但实践中线性干预表现不一致。作者质疑这一假设，分析LLM激活空间的内在几何结构，发现存在显著几何扭曲，表明激活空间不能很好地用全局线性几何近似

Method: 通过测地距离与欧几里得距离之比测量几何扭曲，提出"Curveball steering"非线性导向方法，基于多项式核PCA在特征空间进行干预，更好地尊重学习到的激活几何

Result: 观察到概念依赖的显著几何扭曲，表明激活空间不是全局线性几何。Curveball steering在表现出强几何扭曲的情况下，始终优于基于线性PCA的导向方法

Conclusion: 几何感知的非线性导向为全局线性干预提供了原则性替代方案，特别是在几何扭曲较强的场景下效果显著
...
TL;DR: RRPI提出了一种鲁棒正则化策略迭代方法，通过将离线RL建模为鲁棒策略优化问题，处理分布偏移下的外推和转移不确定性，在D4RL基准测试中表现优异。
...
Method: 将离线RL建模为鲁棒策略优化问题，将转移核作为不确定性集内的决策变量，针对最坏情况动态优化策略。提出RRPI方法，用可处理的KL正则化替代目标替换难以处理的max-min双层目标，基于鲁棒正则化贝尔曼算子推导高效策略迭代过程。

Result: 在D4RL基准测试中，RRPI实现了强大的平均性能，在大多数环境中优于包括PMDB在内的最新基线方法，同时在其余环境中保持竞争力。学习到的Q值在认知不确定性较高区域下降，表明所得策略在转移不确定性下避免不可靠的分布外动作。

Conclusion: RRPI通过鲁棒正则化策略迭代有效处理离线RL中的分布偏移问题，提供理论保证并在实验中表现出色，能够学习到在不确定性下避免不可靠动作的鲁棒策略。
...
<details>
  <summary>Details</summary>
Motivation: AI在各领域的快速部署增加了对监管合规的需求，但缺乏资源限制了AI系统合规性的半自动或自动评估，导致需要容易出错且资源有限的手动工作。

Method: 开发开放、透明、可重复的方法创建数据集，结合领域知识和大型语言模型生成场景及相关任务，以机器对机器友好的格式呈现，解决风险级别边界不明确的问题。

Result: 创建了包含风险级别分类、条款检索、义务生成和问答任务的数据集，评估RAG解决方案在禁止和高风险场景下分别达到0.87和0.85的F1分数。

Conclusion: 该方法展示了利用语言模型进行基于文档的高相关性生成，为评估NLP模型对欧盟AI法案的合规性提供了有效资源，解决了监管标准中不明确边界的问题。
...
<details>
  <summary>Details</summary>
Motivation: 模型合并能够统一同一基础模型上独立微调的LLM，实现并行开发成果的复用和集成。但在实践中发现某些任务专家模型的组合合并后会出现灾难性的性能下降，这种现象被称为"合并崩溃"。研究旨在识别和表征这一现象，理解其根本原因。

Method: 通过大量实验和统计分析，研究任务级合并崩溃现象。分析任务间的表示不兼容性与合并崩溃的相关性，并与参数空间冲突指标进行对比。使用基于率失真理论的维度相关边界提供理论解释。

Result: 研究发现任务间的表示不兼容性与合并崩溃有强相关性，而参数空间冲突指标相关性极小，这与模型合并文献中的传统观点相悖。通过理论分析建立了任务可合并性的基本限制。

Conclusion: 任务级合并崩溃的根本原因是任务间的表示不兼容性，而非参数空间冲突。研究通过率失真理论建立了任务可合并性的维度相关边界，为模型合并实践提供了重要指导。
...
TL;DR: 本文提出了一种基于认知缺口（无知、惊讶、陈旧）的优先级函数，用于从智能体内部认知状态内生地生成注意力目标，无需外部奖励。在两种系统中验证了有效性，并展示了优于固定策略的性能。


<details>
  <summary>Details</summary>
Motivation: 传统目标导向系统假设目标由外部提供，本文探索注意力优先级能否从智能体内部认知状态内生地产生，而不依赖外部奖励信号。

Method: 提出基于三种认知缺口的优先级函数：无知（后验方差）、惊讶（预测误差）和陈旧（未观测变量置信度的时间衰减）。在最小注意力分配环境（2000次运行）和模块化部分可观测世界（500次运行）中验证，并进行消融实验。

Result: 1）消融实验显示三个组件都必要；2）发现度量依赖反转现象：全局预测误差下基于覆盖的轮转获胜，变化检测延迟下优先级引导分配获胜；3）检测延迟随注意力预算呈幂律分布，优先级引导分配指数更陡（0.55 vs 0.40）；4）当衰减率可学习时，系统能无监督地恢复环境波动结构。

Conclusion: 仅凭认知缺口（无需外部奖励）就足以生成自适应的注意力优先级，这些优先级优于固定策略并能恢复潜在环境结构，证明了注意力优先级可以从内部认知状态内生地产生。
...
TL;DR: GenePlan是一个利用大语言模型辅助进化算法生成领域依赖通用规划器的新框架，在PDDL描述的经典规划任务中表现出色，性能接近最先进规划器，显著优于其他LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: 通用规划旨在为特定领域的问题实例生成可重用的规划器，传统方法通常需要大量领域专业知识且难以扩展。现有LLM方法如思维链提示在复杂规划任务中表现有限，需要更系统的方法来生成高效、可解释的领域特定规划器。

Method: GenePlan将通用规划建模为优化问题，使用LLM辅助的进化算法迭代演化可解释的Python规划器。框架通过最小化不同问题实例的规划长度来优化规划器，利用LLM生成和修改规划器代码，在进化过程中不断改进性能。
...
Conclusion: GenePlan成功展示了LLM辅助进化算法在生成高效、低成本通用规划器方面的潜力，为自动化规划器开发提供了新途径，在性能接近最先进方法的同时保持了可解释性和实用性。
...
TL;DR: 该研究提出了一种基于大五人格特质（外向性、宜人性、尽责性、神经质、开放性）的个性化假新闻辟谣信息生成方法，利用大语言模型将通用辟谣内容转化为针对特定人格特征的个性化版本，并通过另一个LLM作为自动化评估器来评估效果。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻辟谣信息通常是通用的，缺乏个性化针对性。研究表明个性化信息可能更有效，但传统人工评估成本高昂。本研究旨在开发一种利用LLM生成和评估个性化辟谣信息的方法，提高辟谣效果并降低评估成本。

Method: 1. 基于大五人格特质（外向性、宜人性、尽责性、神经质、开放性）构建人格特征输入提示
2. 使用LLM将通用辟谣内容转化为针对特定人格特征的个性化版本
3. 采用另一个LLM作为自动化评估器，模拟相应人格特质来评估信息说服力
4. 比较个性化信息与通用信息的效果差异

Result: 1. 个性化辟谣信息通常比通用信息更具说服力
2. 开放性特质倾向于提高说服力，而神经质特质可能降低说服力
3. 不同LLM评估器之间存在差异，使用多个模型能提供更清晰的评估结果
4. 该方法有效降低了传统人工评估的成本

Conclusion: 该研究展示了利用LLM生成针对性辟谣信息的实用方法，同时提出了关于此类技术使用的伦理问题。个性化辟谣信息在提高说服力方面具有潜力，但需要谨慎考虑其应用伦理。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在代码生成方面的快速发展，人机交互正从静态文本响应演变为动态的交互式HTML应用（MiniApps）。现有基准测试主要关注算法正确性或静态布局重建，无法捕捉这种新范式所需的能力。

Method: 从真实应用中收集1000万+生成样本，提炼出500个任务覆盖6个领域（游戏、科学、工具等）。提出MiniAppEval评估框架，利用浏览器自动化进行类人探索性测试，从意图、静态和动态三个维度系统评估应用。

Result: 实验显示当前LLM在生成高质量MiniApps方面仍面临重大挑战。MiniAppEval与人类判断高度一致，为未来研究建立了可靠标准。
...
TL;DR: 提出Omni Parsing框架，通过统一的多模态解析方法将非结构化数据转换为可定位、可枚举、可追溯的结构化知识，并发布相关模型和基准测试。
...
Method: 提出Omni Parsing框架，包含三个层次：1)整体检测实现时空定位；2)细粒度识别进行符号化和属性提取；3)多层次解释构建从局部语义到全局逻辑的推理链。采用证据锚定机制确保高层语义描述与低层事实对齐。
...
### [172] [OOD-MMSafe: Advancing MLLM Safety from Harmful Intent to Hidden Consequences](https://arxiv.org/abs/2603.09706)
*Ming Wen,Kun Yang,Jingyu Zhang,Yuxuan Liu,shiwen cui,Shouling Ji,Xingjun Ma*

Main category: cs.AI

TL;DR: 本文提出从意图/情境安全转向后果驱动安全的新范式，针对MLLMs在因果链中识别潜在危险的能力不足问题，开发了OOD-MMSafe基准测试和CASPO优化框架，显著提升了模型的安全推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的安全对齐主要关注恶意意图或情境违规，但忽视了后果驱动的安全问题。这对于自主和具身智能体的稳健部署至关重要，因为模型需要能够识别上下文依赖的因果链中的潜在危险。

Method: 1) 提出OOD-MMSafe基准测试，包含455个精心设计的查询-图像对，用于评估模型在因果链中识别潜在危险的能力；2) 开发CASPO框架，利用模型内在推理作为动态参考，进行token级别的自蒸馏奖励优化。

Result: 前沿模型普遍存在因果盲区，高性能闭源模型的失败率高达67.5%。CASPO框架显著提升了后果预测能力：Qwen2.5-VL-7B的风险识别失败率降至7.3%，Qwen3-VL-4B降至5.7%，同时保持了整体有效性。

Conclusion: 后果驱动安全是多模态大语言模型安全对齐的重要前沿。当前模型在因果推理方面存在显著缺陷，CASPO框架通过自蒸馏奖励机制有效提升了模型的安全推理能力，为自主智能体的安全部署提供了重要解决方案。
...
TL;DR: AutoAgent是一个自演进的多智能体框架，通过演化认知、实时上下文决策和弹性内存编排来解决自主智能体在长期经验学习和实时决策之间的鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自主智能体框架难以平衡长期经验学习和实时上下文敏感决策，表现为静态认知、刚性工作流依赖和低效上下文使用，限制了在开放和非平稳环境中的适应性。

Method: AutoAgent包含三个紧密耦合组件：演化认知（维护结构化提示级认知）、实时上下文决策（结合任务上下文选择动作）、弹性内存编排（动态组织交互历史）。通过闭环认知演进过程整合这些组件，无需外部重新训练。

Result: 在检索增强推理、工具增强智能体基准和具身任务环境中的实证结果显示，AutoAgent在任务成功率、工具使用效率和协作鲁棒性方面持续优于静态和内存增强基线方法。

Conclusion: AutoAgent为自适应自主智能体提供了一个统一实用的基础框架，能够在动态环境中从经验中学习同时做出可靠的上下文感知决策。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态基础模型在空间推理方面存在局限性：现有方法要么通过3D标注数据过拟合统计捷径，要么局限于2D视觉感知，导致空间推理准确性和在未见场景中的泛化能力受限。

Method: 1. 利用3D重建和实例分割模型构建结构化空间认知地图；2. 合成以自我为中心的空间树（AST），使用椭圆参数精确建模地标布局；3. 采用三阶段推理链：工具调用评估、模态解耦线索收集、几何-语义交织推理，以减轻3D重建的固有误差。

Result: World2Mind将前沿模型（如GPT-5.2）的性能提升了5%~18%。令人惊讶的是，仅依靠AST结构化的文本，纯文本基础模型就能执行复杂的3D空间推理，性能接近先进的多模态模型。
...
### [175] [Influencing LLM Multi-Agent Dialogue via Policy-Parameterized Prompts](https://arxiv.org/abs/2603.09890)
*Hongbo Bo,Jingyu Hu,Weiru Liu*

Main category: cs.AI

TL;DR: 该研究提出了一种将提示参数化作为轻量级策略的方法，通过状态-动作对序列影响LLM多智能体对话行为，无需训练即可控制对话动态。
...
Method: 将提示视为LLM执行的动作，基于智能体当前状态通过五个组件动态构建提示。框架将提示参数化为可控制的策略，通过状态-动作对序列来影响对话行为。

Result: 在两个公共讨论场景中，使用不同LLM驱动的智能体进行实验，评估了响应性、反驳、证据使用、非重复性和立场转变五个指标。结果显示提示参数化能够显著影响对话动态。

Conclusion: 策略参数化的提示提供了一种简单有效的机制来影响对话过程，这将有助于多智能体系统在社会模拟方向的研究。该方法为LLM多智能体系统提供了原则性的策略控制框架。
...
### [176] [MedMASLab: A Unified Orchestration Framework for Benchmarking Multimodal Medical Multi-Agent Systems](https://arxiv.org/abs/2603.09909)
*Yunhang Qian,Xiaobin Hu,Jiaquan Yu,Siyang Xin,Xiaokun Chen,Jiangning Zhang,Peng-Tao Jiang,Jiawei Liu,Hongwei Bran Li*

Main category: cs.AI

TL;DR: MedMASLab是一个统一的多模态医疗多智能体系统框架和基准测试平台，解决了当前医疗MAS研究中架构碎片化、评估不一致和缺乏跨专业基准的问题。
...
Method: 1) 标准化多模态智能体通信协议，支持11种异构MAS架构和24种医疗模态的无缝集成；2) 自动化临床推理评估器，采用零样本语义评估范式，利用大型视觉语言模型验证诊断逻辑和视觉基础；3) 构建迄今为止最广泛的基准测试，涵盖11个器官系统和473种疾病，标准化来自11个临床基准的数据。

Result: 系统评估揭示了关键领域特定性能差距：虽然MAS提高了推理深度，但当前架构在医疗专业子领域间转换时表现出显著脆弱性。研究提供了交互机制和成本-性能权衡的严格消融分析，为未来自主临床系统建立了新的技术基线。

Conclusion: MedMASLab通过统一框架和基准测试平台解决了医疗多智能体系统研究的关键挑战，为未来自主临床系统的发展提供了标准化基础和技术参考。
...
### [177] [PathMem: Toward Cognition-Aligned Memory Transformation for Pathology MLLMs](https://arxiv.org/abs/2603.09943)
...
Method: 受病理学家分层记忆过程启发，提出PathMem框架：将结构化病理知识组织为长期记忆，引入记忆Transformer建模从长期记忆到工作记忆的动态转换，通过多模态记忆激活和上下文感知知识基础实现上下文感知的记忆精炼。

Result: 在多个基准测试中达到最先进性能：WSI-Bench报告生成提升12.8% WSI-Precision和10.1% WSI-Relevance，开放式诊断比先前WSI模型提升9.7%和8.9%。
...
### [178] [The Confidence Gate Theorem: When Should Ranked Decision Systems Abstain?](https://arxiv.org/abs/2603.09947)
*Ronald Doku*

Main category: cs.AI

TL;DR: 研究排序决策系统中的置信度弃权机制，发现结构不确定性下弃权能单调提升决策质量，但上下文不确定性下会失效，需匹配不确定性类型选择置信信号。


<details>
  <summary>Details</summary>
Motivation: 排序决策系统（推荐系统、广告拍卖、临床分诊队列）需要决定何时干预排序输出、何时弃权。研究目标是理解置信度弃权何时能单调改善决策质量，何时会失败，以及背后的原因。

Method: 提出形式化条件：排序对齐和无反转区。区分两种不确定性类型：结构不确定性（缺失数据，如冷启动）和上下文不确定性（缺失上下文，如时间漂移）。在三个领域进行实证验证：协同过滤（MovieLens，3种分布偏移）、电商意图检测（RetailRocket, Criteo, Yoochoose）和临床路径分诊（MIMIC-IV）。

Result: 结构不确定性在所有领域都产生近乎单调的弃权收益；基于结构的置信信号（观察计数）在上下文漂移下失效，在MovieLens时间分割上产生与随机弃权一样多的单调性违反。上下文感知替代方案（集成分歧和近期特征）显著缩小差距（违反从3减少到1-2），但未能完全恢复单调性。基于残差的异常标签在分布偏移下性能大幅下降（AUC从0.71降至0.61-0.62）。

Conclusion: 提供实用的部署诊断：在部署置信门之前检查C1和C2条件，并将置信信号与主导的不确定性类型匹配。上下文不确定性带来本质上不同的挑战，需要专门的处理方法。
...
### [179] [Think Before You Lie: How Reasoning Improves Honesty](https://arxiv.org/abs/2603.09957)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注大语言模型的欺骗率，但对导致欺骗行为的基本条件缺乏深入理解。本研究旨在探究在道德权衡情境下，什么因素会影响LLM的诚实行为。

Method: 使用包含现实道德权衡的新数据集，其中诚实行为会带来可变成本。通过分析推理过程、表示空间几何结构，以及测试输入改写、输出重采样和激活噪声等方法，研究欺骗性区域的稳定性特征。

Result: 与人类相反（人类在深思熟虑后往往变得更不诚实），研究发现推理过程会持续增加LLM的诚实度，这种效应在不同规模和多个LLM家族中都存在。欺骗性区域在表示空间中呈现亚稳定状态，更容易被各种扰动破坏。

Conclusion: 推理过程通过引导模型在偏置的表示空间中移动，最终推动模型回归到更稳定的诚实默认状态。这揭示了LLM诚实行为的几何基础，而不仅仅是推理内容本身的影响。
...
### [180] [Feedback Does Not Increase the Capacity of Approximately Memoryless Surjective POST Channels](https://arxiv.org/abs/2603.08886)
*Xiaojing Zhang,Jun Chen,Guanghui Wang*

Main category: cs.IT

TL;DR: 对于一类称为POST信道的有限状态信道，当状态依赖的转移矩阵足够接近时，反馈容量与非反馈容量相等，表明反馈不提供容量增益。


<details>
  <summary>Details</summary>
Motivation: 研究POST信道（前一个信道输出作为当前状态的信道）的反馈容量问题，扩展香农关于离散无记忆信道反馈不增加容量的经典定理到更广泛的信道类型。

Method: 研究近似无记忆的POST信道，即状态依赖转移矩阵足够接近的信道。在关联的无记忆参考信道满足满射条件下，分析反馈容量与非反馈容量的关系。

Result: 对于几乎所有输入字母表大小不小于输出字母表大小的近似无记忆POST信道，反馈容量与非反馈容量相等，反馈不提供容量增益。
...
### [181] [A Note on the Equivalence Between Zero-knowledge and Quantum CSS Codes](https://arxiv.org/abs/2603.08941)
*Noga Ron-Zewi,Mor Weiss*

Main category: cs.IT

TL;DR: 零知识码与量子CSS码等价，这一等价关系可用于构造显式的渐近良好零知识本地可测码


<details>
  <summary>Details</summary>
Motivation: 零知识码在密码学中有广泛应用，量子CSS码在量子纠错和量子复杂性理论中很重要。探索这两类码之间的理论联系，可能为构造新的零知识码提供途径。

Method: 通过理论证明展示（线性、完美）零知识码与量子CSS码之间的等价关系，并利用这一等价关系构造具体的渐近良好零知识本地可测码。

Result: 证明了零知识码与量子CSS码在数学上是等价的，并利用这一等价关系成功构造了显式的渐近良好零知识本地可测码。

Conclusion: 零知识码与量子CSS码的等价关系为两类码的研究建立了桥梁，为构造新的零知识码提供了量子编码理论工具，展示了跨领域理论联系的实际应用价值。
...
### [182] [Differentially Private Secure Multiplication: Beyond Two Multiplicands](https://arxiv.org/abs/2603.08944)
*Haoyang Hu,Viveck R. Cadambe*

Main category: cs.IT

TL;DR: 该论文研究了分布式计算系统中差分隐私安全乘法问题，将现有两乘数结果扩展到任意M个乘数，提出了基于编码多项式和分层噪声注入的框架，在两种节点数量情况下分析了最优隐私-精度权衡。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注两个乘数的差分隐私安全乘法问题，但实际应用中经常需要计算多个私有输入的乘积。需要将隐私-精度权衡的理论结果扩展到任意数量乘数的情况，为分布式计算系统提供更通用的解决方案。

Method: 提出基于编码多项式和分层噪声注入的安全乘法框架。通过精心设计的编码多项式结合分层噪声注入，实现低阶噪声项的系统性消除，从而提高估计精度。该方法推广了现有方案。

Result: 在(M-1)T+1 ≤ N ≤ MT情况下，刻画了最优隐私-精度权衡；在N = T+1情况下，推导了非平凡的可行性和逆界，在高隐私机制下这些界限是渐近紧的。

Conclusion: 成功将差分隐私安全乘法的理论结果从两个乘数扩展到任意M个乘数，提出的框架能够系统性地消除低阶噪声项，在两种重要节点数量情况下建立了理论界限，为分布式隐私保护计算提供了更通用的解决方案。
...
### [183] [Do Ambient Backscatter Communication Receivers Require Low-Noise Amplifiers?](https://arxiv.org/abs/2603.09123)
*Xinyi Wang,Yuxin Li,Yinghui Ye,Gongpu Wang,Guangyue Lu*

Main category: cs.IT

TL;DR: 研究低噪声放大器在环境反向散射通信中的符号检测性能，发现在中低环境源发射功率下，LNA能提升检测性能，并提出了最优检测阈值和参数估计方法。


<details>
  <summary>Details</summary>
Motivation: 环境反向散射通信中，来自环境源的强直接干扰是可靠符号检测的主要挑战。虽然之前研究表明在传统点对点通信中使用低噪声放大器能提升中低传输功率下的检测性能，但这一改进是否适用于环境反向散射通信尚不明确。

Method: 提出了一个包含LNA参数的新环境反向散射符号检测框架，推导了能量检测的误码率，使用偏转系数分析检测性能，推导了近最优检测阈值以最小化误码率，并提出了利用标签导频符号估计所需参数的方法。

Result: 研究表明，在中低环境源发射功率下，LNA能够增强环境反向散射通信的检测性能，并通过理论推导和参数估计方法验证了这一结论。

Conclusion: 低噪声放大器确实能提升环境反向散射通信在中低环境源发射功率下的符号检测性能，提出的检测框架和参数估计方法为实际系统设计提供了理论依据。
...
### [184] [Artificial Noise Versus Artificial Noise Elimination: Redefining Scaling Laws of Physical Layer Security](https://arxiv.org/abs/2603.09129)
*Hong Niu,Tuo Wu,Xia Lei,Wanbin Tang,Mérouane Debbah,H. Vincent Poor,Chau Yuen*

Main category: cs.IT

TL;DR: 该论文研究了人工噪声消除（ANE）对多输入多输出窃听信道中物理层安全的影响，建立了存在人工噪声和人工噪声消除时的保密速率缩放定律，揭示了发射天线、接收天线和窃听者天线数量之间的相互约束关系。


<details>
  <summary>Details</summary>
Motivation: 人工噪声是无线通信中物理层安全的关键方案，但最近出现的人工噪声消除技术可以减轻人工噪声对窃听者的影响。然而，人工噪声消除在对抗人工噪声时对保密速率的影响尚未得到研究，这是本文要解决的核心问题。

Method: 通过建立存在人工噪声和人工噪声消除情况下的平均和瞬时保密速率缩放定律，基于这些缩放定律推导出多个推论，分析发射天线、接收天线和窃听者天线数量之间的相互约束关系。

Result: 研究发现当窃听者拥有的天线数量超过发射天线数量的两倍时，安全通信可能无法保证。通过比较使用人工噪声消除对抗人工噪声与不使用人工噪声的情况，确定了人工噪声仍然有效的充分条件。

Conclusion: 即使窃听者实施了先进的人工噪声消除对抗措施，推导出的保密速率仍能为系统设计提供指导，揭示了人工噪声与人工噪声消除之间的相互作用对物理层安全的影响。
...
### [185] [Tensor Train Decomposition-based Channel Estimation for MIMO-AFDM Systems with Fractional Delay and Doppler](https://arxiv.org/abs/2603.09293)
...
<details>
  <summary>Details</summary>
Motivation: 现有AFDM信道估计研究主要关注整数时延场景和单符号导频估计，而实际信道时延通常是分数的，近似为整数会降低时延估计精度并严重影响多普勒频率估计。因此需要开发适用于分数时延场景的高效信道估计方法。

Method: 提出时-仿射频率域导频结构以利用时域相位变化；基于空间和时间域的旋转不变性，开发了基于Vandermonde结构张量分解的信道估计算法；推导了全局Ziv-Zakai界作为参数估计误差下界。

Result: 提出的算法相比现有参数估计方法具有显著的计算效率优势；推导的ZZB比CRB提供更紧的全局性能表征，成功捕捉了低信噪比下MSE性能的阈值现象；算法通信性能优于现有方案，执行时间相比最先进的迭代算法减少一个数量级。

Conclusion: 本文提出的基于张量分解的信道估计算法有效解决了MIMO-AFDM系统中分数时延场景下的估计问题，在计算效率和通信性能方面均优于现有方法，为实际AFDM系统部署提供了有效的信道估计解决方案。
...
### [186] [Scientific Rigor and Human Warmth: Remembering Vladimir Sidorenko (1949-2025)](https://arxiv.org/abs/2603.09437)
*Christian Deppe,Haider Al Kim,Jessica Bariffi,Hannes Bartz,Minglai Cai,Pau Colomer,Gohar Kyureghyan*

Main category: cs.IT

TL;DR: FFCS会议为纪念Vladimir Sidorenko博士（1949-2025）举行了追思会，总结其生平、科学成就、个人品质及在编码理论、密码学、电信和量子纠错领域的广泛影响。


<details>
  <summary>Details</summary>
Motivation: 纪念Vladimir Sidorenko博士的科学贡献和人文品质，通过追思会形式让同事、合作者和学生共同回顾他的学术成就和个人影响，传承其科研精神。

Method: 在FFCS会议上组织专门的纪念会议，由Minglai Cai主持，邀请同事、合作者和前学生分享生平致敬、个人回忆，并总结其学术贡献。

Result: 会议成功汇集了国际研究社区，展示了Sidorenko博士150多篇论文的技术贡献，以及他在智力严谨性、指导、幽默、慷慨和对国际研究社区的持久影响方面的品质。

Conclusion: Vladimir Sidorenko博士不仅在编码理论、密码学、电信和量子纠错领域做出了重要技术贡献，更重要的是他留下了深远的人文影响和科研传承，其精神将继续激励国际研究社区。
...
### [187] [Randomized Distributed Function Computation (RDFC): Ultra-Efficient Semantic Communication Applications to Privacy](https://arxiv.org/abs/2603.09577)
*Onur Günlü*

Main category: cs.IT

TL;DR: 论文提出了随机化分布式函数计算框架，通过发送足够信息让接收方生成输入数据的随机化函数，实现语义通信，在保证本地差分隐私的同时降低通信成本。


<details>
  <summary>Details</summary>
Motivation: 传统分布式计算中，安全性和隐私约束通常需要随机化步骤，但现有方法通信成本高。作者希望建立一个框架，在保证隐私的同时最小化通信开销，特别是针对隐私敏感的分布式计算系统。

Method: 建立随机化分布式函数计算框架，将其建模为广义远程源编码问题。使用强协调度量确保本地差分隐私，分析无共享随机性时的通信成本下界，提出连续字母表随机变量的数值评估技术，并进行有限块长分析。

Result: 证明了足够的共享随机性可以将语义通信速率降低两个数量级；即使没有共享随机性，RDFC仍大幅优于无损传输；隐私参数在渐近和非渐近方法间的差距随输入长度指数级快速收敛。

Conclusion: RDFC框架为隐私感知的分布式计算系统提供了一种能量高效的语义通信策略，在保证本地差分隐私的同时显著降低通信成本，特别适用于需要安全性和隐私保护的分布式应用。
...
### [188] [Rate-Distortion Bounds for Heterogeneous Random Fields on Finite Lattices](https://arxiv.org/abs/2603.09833)
*Sujata Sinha,Vishwas Rao,Robert Underwood,David Lenz,Sheng Di,Franck Cappello,Lingjia Liu*

Main category: cs.IT

TL;DR: 该论文提出了一个针对有限格点上异构随机场的有限块长率失真理论框架，专门针对高性能科学计算中基于分块架构的压缩器，量化了空间相关性、区域几何、异构性和分块大小对压缩性能的影响。


<details>
  <summary>Details</summary>
Motivation: 传统率失真理论主要针对无记忆或平稳遍历信源，而实际科学计算中的有损压缩器（如SZ、ZFP、MGARD、SPERR）处理的是有限、高维、空间相关且统计异构的随机场，采用固定大小的分块独立处理架构。现有有限块长分析未能解决结构异构性、有限格点效应和分块约束问题。

Method: 将随机场建模为分段同质、区域平稳二阶统计的模型，直接将分块约束纳入信源模型。在超失真概率准则下，建立非渐近可达性、逆界，并推导二阶展开，量化空间相关性、区域几何、异构性和分块大小对率和色散的影响。

Result: 建立了针对异构随机场的有限块长率失真理论框架，获得了非渐近可达性和逆界，推导了二阶展开，为科学计算压缩器的性能评估提供了理论基础。

Conclusion: 该工作填补了传统率失真理论与实际科学计算压缩器需求之间的理论空白，为基于分块架构的高性能压缩器提供了严格的性能分析框架，量化了各种实际约束对压缩性能的影响。
