...
Method: 提出MLLM引导的语义对齐模块（MGSA），通过多模态输入整合和语义对齐损失捕获跨模态语义关系；采用两阶段渐进训练策略和自合成流程从有限数据中学习复杂任务。

Result: UniFit支持多种虚拟试穿任务，包括多服装和模型间试穿，并实现了最先进的性能。

Conclusion: UniFit通过MLLM驱动的语义对齐和高效训练策略，成功构建了一个通用的虚拟试穿框架，显著提升了复杂场景下的试穿效果。
...
TL;DR: 本文提出四种新颖扩展来改进基于模型的6D物体姿态估计方法，包括动态非均匀密集采样、多假设推理机制、迭代精炼和遮挡训练增强，在遮挡情况下显著提升精度和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计方法在遮挡情况下容易出错，因为多阶段流水线的早期错误会传播到后续处理，导致性能下降。

Method: 提出四种扩展：动态非均匀密集采样策略、多假设推理机制、迭代精炼和遮挡训练增强，以及新的基于可见性的评估指标。

Result: 在ICBIN数据集上精度提升超过5%，在BOP数据集上提升超过2%，推理速度提升约3倍。

Conclusion: 所提出的方法有效解决了遮挡情况下的6D姿态估计问题，显著提升了精度和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化、拣选等场景中，现有6D姿态估计方法在灵活性、精度和实用性方面的不足：基于模型的方法需要精确CAD模型且泛化差；无模型方法在挑战性条件下容易失败；类别级方法过于通用且忽略环境先验。
...
Conclusion: Box6D为仓库环境中的存储箱提供了一种高效准确的6D姿态估计解决方案，平衡了灵活性、精度和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域特定视频分类任务中表现不佳，特别是在数据有限的情况下，存在语义推理差距，稀疏的领域数据不足以连接复杂的时空内容与抽象分类标签。

Method: 两阶段自改进范式：第一阶段提示VLMs为每个视频生成详细文本推理，强制模型表达领域特定逻辑，然后在这些自生成推理上微调模型；第二阶段在任务标签上进行常规监督微调。

Result: 在多个数据集上的广泛实验表明，该方法显著优于直接监督微调，验证了自生成推理作为适应领域特定视频分析的有效且标注高效的范式。

Conclusion: 自生成推理是一种有效的标注高效范式，能够帮助视觉语言模型适应领域特定的视频分析任务，显著提升模型性能。
...
TL;DR: MGLL提出了一种多粒度语言学习框架，通过结构化多标签监督和跨粒度文本集成，改进了医学影像中的多标签和跨粒度对齐问题。


<details>
  <summary>Details</summary>
Motivation: CLIP等图像-文本预训练模型在单标签、单粒度对齐方面表现良好，但在医学影像等复杂领域中，图像通常对应多个高层标签和不同注释粒度，现有方法效果有限。

Method: MGLL采用对比学习框架，利用结构化多标签监督，集成跨粒度文本描述，引入带点约束的软标签监督，并使用平滑KL散度确保跨粒度一致性。

Result: 在构建的大规模多粒度数据集上预训练并在多个数据集上评估，MGLL在下游任务中优于其他最先进方法。

Conclusion: MGLL作为一个即插即用模块，能够有效提升视觉-语言模型在复杂领域中的多标签和跨粒度对齐能力。
...
TL;DR: 提出了一种从3D心脏超声体积中自动选择标准2D视图的方法，通过深度学习视图分类器和解剖标志启发式算法重建标准超声心动图视图，验证显示96%准确率，并证明提取的2D视频保留了空间校准和诊断特征。


<details>
  <summary>Details</summary>
Motivation: 传统心脏超声依赖2D视频，而3D超声心动图具有改善采集效率和评估离轴特征的潜力，但医生习惯使用标准2D视图格式进行解读。

Method: 应用深度学习视图分类器和基于解剖标志的下游启发式算法，结合心脏病专家提供的启发式规则，从3D超声体积重建标准2D超声心动图视图。

Result: 在来自2家医院的1,600个视频中达到96%准确率，提取的2D视频能够通过AI模型检测心脏异常并生成临床级心脏解剖测量。

Conclusion: 该方法使临床医生能够从3D体积获得准确的真实世界解读，同时发布了代码和包含29个3D超声心动图视频的数据集。
...
TL;DR: Click2Graph是首个用于全景视频场景图生成的交互式框架，将视觉提示与空间、时间和语义理解相结合，通过用户点击或边界框等单一提示，自动分割跟踪目标、发现交互对象并预测三元组关系。


<details>
  <summary>Details</summary>
Motivation: 现有视频场景图生成系统是封闭的前馈管道，无法融入人工指导；而可提示分割模型缺乏语义或关系推理。需要结合两者的优势，实现用户可控的视频场景理解。

Method: 提出Click2Graph框架，包含动态交互发现模块（生成基于主体的对象提示）和语义分类头（执行联合实体和谓词推理），通过单一用户提示实现目标分割跟踪和关系推理。
...
Method: 采用信息论视角，通过两个互信息目标：1）压缩预训练CLIP的像素-文本模态对齐以减少噪声；2）最大化预训练CLIP与微调模型对齐知识间的互信息，传递适合分割任务的局部语义关系。
...
TL;DR: 提出一种新颖的一致性正则化方法，通过可微分的BI-RADS形态特征来缓解乳腺超声肿瘤分割和分类任务之间的破坏性干扰，显著提升了多任务学习在外部数据集上的泛化性能。
...
Method: 提出基于可微分BI-RADS形态特征的一致性正则化方法，该方法能够减轻分割和分类任务之间的破坏性干扰。在BrEaST数据集上训练，并在三个外部数据集上进行验证。
...
Conclusion: 所提出的一致性正则化方法有效缓解了多任务学习中的破坏性干扰，显著提升了乳腺超声肿瘤分割模型在外部验证中的泛化能力，并实现了最先进的性能。
...
TL;DR: 提出了一种检测引导的生成式框架，用于统一处理物体检测、类别预测和属性识别，通过生成层次化类别和属性token来提升细粒度识别能力。
...
Method: 使用检测引导的生成框架，提取精炼的ROI级特征，采用BART-based生成器以粗到细的顺序生成语义token，涵盖类别层次和属性值对，支持属性条件化识别。

Result: 在大规模专有电商数据集和开源数据集上的实验表明，该方法显著优于现有的基于相似度的流程和多阶段分类系统。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医学图像推理中显示出强大潜力，但跨人口群体的公平性仍然是一个主要问题。现有的去偏方法通常依赖大型标注数据集或微调，这对于基础规模模型来说不切实际。
...
Conclusion: 公平感知上下文学习为公平的医学图像推理提供了一种高效且可扩展的路径，展示了作为可扩展和数据高效解决方案的潜力。
...
### [12] [Physically Realistic Sequence-Level Adversarial Clothing for Robust Human-Detection Evasion](https://arxiv.org/abs/2511.16020)
*Dingkun Zhou,Patrick P. K. Chan,Hengxu Wu,Shikang Zheng,Ruiqi Huang,Yuanjie Zhao*

Main category: cs.CV

TL;DR: 提出了一种序列级优化框架，生成可在整个行走视频中保持有效的可打印对抗纹理，用于衣服和帽子，在数字和物理环境中都能有效隐藏人体检测。
...
Method: 将产品图像映射到UV空间并转换为紧凑的调色板和控点参数化，使用物理人体-衣物管线模拟运动、多角度相机视角、布料动力学和光照变化，通过期望变换目标和时序加权优化控点以最小化检测置信度。

Result: 实验显示该方法具有强大稳定的隐蔽性、对视角变化的高鲁棒性以及优越的跨模型可迁移性，物理打印的衣物在室内外录制中实现可靠抑制。
...
TL;DR: 本文提出了一种用于单步图像超分辨率的混合秩（MoR）架构，将稀疏门控混合专家（MoE）思想引入真实世界图像超分辨率任务，通过细粒度的专家分区策略和退化感知路由机制解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界图像超分辨率方法主要依赖LoRA微调预训练扩散模型，但这些密集模型难以自适应捕捉复杂真实世界退化样本的异质特征，也无法在同等计算预算下实现输入间的知识共享。

Method: 提出MoR架构，将LoRA中的每个秩视为独立专家；引入细粒度专家分区策略，固定位置秩作为共享专家；开发基于CLIP嵌入的退化估计模块；包含零专家槽位并提出退化感知负载均衡损失。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功将稀疏MoE思想应用于真实世界图像超分辨率，通过灵活的专家组合和退化感知路由机制实现了更好的性能表现。
...
TL;DR: CuriGS是一个基于课程学习的稀疏视图3D高斯泼溅重建框架，通过生成多扰动级别的学生视图来增强稀疏训练数据，提升渲染质量和几何一致性。
...
Method: 提出课程引导框架，为每个真实视角生成多组不同扰动级别的学生视图，通过深度相关性和协同正则化进行训练，并采用多信号指标评估学生视图质量。

Result: 在合成和真实稀疏视图场景中，CuriGS在渲染保真度和几何一致性方面均优于现有最先进方法。

Conclusion: CuriGS通过课程学习和学生视图增强策略，有效解决了稀疏视图3D高斯泼溅重建的挑战，实现了高质量的实时场景重建。
...
TL;DR: 提出一种跨模态学习策略，通过将高分辨率卫星图像与无人机级别的视觉细节相结合，用于作物冠层性状估计。该方法在产量和氮素预测等多个下游任务中表现优于真实卫星图像。


<details>
  <summary>Details</summary>
Motivation: 无人机在作物监测中表现优异但覆盖范围有限，卫星任务受限于空间分辨率而难以满足微地块管理的现代农业需求。需要弥合卫星和无人机传感之间的差距。

Method: 使用在美国玉米带五个不同地点收集的84个杂交玉米品种的近似配准卫星-无人机图像对数据集，训练学习传感模态间细粒度光谱空间对应关系的模型。

Result: 从卫星输入生成的无人机样表示在多个下游任务（包括产量和氮素预测）中持续优于真实卫星图像。
...
TL;DR: 提出一个基于大语言模型的框架来解决食品识别中的领域偏移、长尾分布和细粒度分类问题，通过LLM解析食品图像生成标题和成分，在多模态嵌入空间中对齐特征，在多个食品数据集上优于现有方法。
...
Conclusion: 基于大语言模型的简单框架能够有效解决食品识别中的多个挑战，通过多模态特征对齐显著提升了识别性能。
...
TL;DR: AMS-KV是一种针对视觉自回归模型(VAR)中下一尺度预测的尺度自适应KV缓存策略，通过优先存储压缩尺度和局部尺度的KV，显著减少KV缓存使用和自注意力延迟。


<details>
  <summary>Details</summary>
Motivation: 下一尺度预测在VAR模型中面临KV内存随尺度数量增长而过度膨胀的问题，严重限制了模型的可扩展性，需要专门针对下一尺度预测的KV缓存设计。

Method: 基于系统研究发现：(1)局部尺度token对生成质量贡献显著；(2)为最粗尺度分配少量内存可稳定多尺度图像生成；(3)通过尺度间相似性分析识别缓存需求层。AMS-KV优先存储压缩尺度和局部尺度的KV，并通过相似性分析优化缓存利用。

Result: 与原始VAR模型相比，AMS-KV将KV缓存使用减少高达84.83%，自注意力延迟降低60.48%。当基线VAR-d30模型在批大小128时出现内存不足故障时，AMS-KV能稳定扩展到批大小256并提高吞吐量。

Conclusion: AMS-KV通过尺度自适应KV缓存策略有效解决了VAR模型中下一尺度预测的KV内存瓶颈问题，显著提升了模型的可扩展性和计算效率。
...
Method: 提出LiSTAR模型，采用混合圆柱-球面表示来保持数据保真度；使用时空注意力与射线中心Transformer来建模复杂动态；提出4D点云对齐体素布局和离散掩码生成START框架，实现可控合成。

Result: 在4D LiDAR重建、预测和条件生成方面实现最先进性能：生成MMD降低76%，重建IoU提高32%，预测L1中值误差降低50%。

Conclusion: LiSTAR为创建真实可控的自动驾驶系统仿真提供了强大的新基础，在多个关键指标上取得了显著性能提升。
...
Conclusion: VideoSeg-R1通过引入强化学习和显式推理链，显著提升了视频推理分割的泛化能力和性能表现。
...
Method: 提出SpectralTrain框架，集成课程学习和基于PCA的光谱降采样，逐步引入光谱复杂性同时保留关键信息，支持各种架构和优化器。

Result: 在三个基准数据集上的实验显示训练时间减少2-7倍，准确率损失较小，在云分类等气候遥感任务中表现出潜力。

Conclusion: SpectralTrain证明了训练策略优化是HSI模型架构设计的有效补充，能够显著提升训练效率而不会大幅牺牲性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决在大型户外环境中使用4D毫米波雷达进行稳健建图的挑战，通过结合雷达和相机数据来改善动态对象处理和纹理一致性。

Method: 结合原始雷达点云与多普勒信息，使用几何增强点云指导图像中的动态对象掩码；利用未同步图像帧全局优化3D高斯表示；采用全局八叉树结构和目标高斯基元管理策略来抑制噪声和减少内存消耗。

Result: 在广泛实验和消融研究中，Rad-GS实现了与基于相机或LiDAR输入的传统3D高斯方法相当的性能，验证了使用4D毫米波雷达进行稳健户外建图的可行性。
...
Method: 设计生成和选择文本提示的机制来描述不同低层视觉任务间的差异，构建跨任务VICL数据集，并提出结合感知评分推理与传统评估指标的新型推理框架。

Result: 在9个跨任务场景中取得顶级结果，在另外10个场景中取得第二层级性能，成功解锁了VLMs中跨任务VICL的潜力。

Conclusion: T2T-VICL框架有效实现了视觉语言模型的跨任务视觉上下文学习，证明了VLMs在解决不同视觉任务间的迁移学习能力。
...
TL;DR: 提出了一种改进4D高斯泼溅方法，通过椭圆误差聚类和分组4D高斯泼溅技术，解决了动态场景重建中的像素对应模糊和动态区域密度不足问题，显著提升了时间一致性和渲染质量。
...
Result: 在Neural 3D Video和Technicolor数据集上的评估表明，该方法显著提高了时间一致性，并实现了最先进的感知渲染质量，在Technicolor光场数据集上PSNR提高了0.39dB。可视化显示泼溅与动态对象的对齐得到改善，误差校正方法能够有效识别错误并正确初始化新泼溅。

Conclusion: 该方法通过创新的误差聚类和分组泼溅技术，有效解决了4D高斯泼溅在动态场景重建中的关键问题，显著提升了重建质量和时间一致性。
...
Conclusion: DCS-LDM通过解耦信息复杂度与尺度，实现了灵活的计算-质量权衡，支持跨尺度的渐进式生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强方法主要依赖已知量化参数，在真实场景中量化参数可能未知，限制了这些方法的适用性。当前盲方法生成的退化向量仅捕获全局退化信息，缺乏空间细节，且不同压缩级别需要不同的计算需求。

Method: 提出预训练退化表示学习模块，从视频内容中解耦和提取高维多尺度退化表示来指导伪影去除；引入分层终止机制，根据压缩级别动态调整伪影减少阶段的数量。

Result: 该方法显著提升了性能，在QP=22时PSNR比最先进的盲方法提高了110%（从0.31 dB到0.65 dB）。分层终止机制在QP=22时的平均推理时间比QP=42减少了一半。

Conclusion: 所提出的方法在压缩视频质量增强方面取得了显著改进，特别是在处理未知量化参数和优化计算效率方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D目标检测器在训练和评估方式之间存在差距，训练时缺乏空间可靠性和排名感知，而推理时使用基于排名的预测选择，这种训练-推理差距阻碍了模型学习与推理行为对齐的表示。
...
TL;DR: 本文提出了一个高空间分辨率的水体变化检测数据集HSRW-CD和一个SSCP注意力模块，该模块通过多语义空间注意力、结构关系感知全局注意力和通道自注意力来充分利用深度特征的空间语义和结构信息，显著提升了水体变化检测的性能。
...
Conclusion: SSCP模块能够有效提升水体变化检测的性能，且具有良好的泛化能力，可作为即插即用模块集成到现有WBCD模型中。
...
TL;DR: 本文提出了多模态交互欺骗评估(MIDA)任务和数据集，评估了12个最先进的多模态大语言模型在识别社交欺骗方面的能力，发现现有模型存在显著性能差距，并提出了社交思维链(SoCoT)和动态社交认知记忆(DSEM)模块来改进性能。
...
Method: 引入MIDA任务和包含同步视频文本及可验证真实标签的多模态数据集，建立基准评估12个开源和闭源模型，分析失败模式，并设计SoCoT推理流程和DSEM模块。

Result: 即使像GPT-4o这样的强大模型也难以可靠地区分真假，模型未能有效将语言与多模态社交线索相结合，缺乏建模他人知识、信念或意图的能力。

Conclusion: 该框架在此挑战性任务上实现了性能提升，为构建能够进行真正类人社交推理的MLLMs开辟了有前景的新路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射SLAM系统虽然能构建逼真地图，但缺乏开放词汇语义理解能力，无法满足高级机器人交互需求。集成语言特征面临内存占用高、渲染开销大、静态模型缺乏适应性等挑战。

Method: 提出场景自适应编码器-解码器，将高维语言嵌入蒸馏到紧凑的16维特征空间；采用语言引导的剪枝策略识别语义冗余；引入基于语言的闭环检测方法，复用映射特征。

Result: 实验表明LEGO-SLAM在保持竞争性映射质量和跟踪精度的同时，提供开放词汇能力，运行速度达15 FPS，高斯数量减少超过60%。

Conclusion: LEGO-SLAM成功解决了3D高斯溅射SLAM中的语义集成挑战，实现了实时、高效的开放词汇语义映射，为机器人交互提供了重要基础。
...
TL;DR: 本文提出了一种名为推理引导嵌入（RGE）的方法，通过将多模态大语言模型的生成推理能力融入嵌入提取过程，提升多模态表示质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法将嵌入提取视为直接编码步骤，忽略了多模态大语言模型具备的生成推理能力，这种能力可以用于提升表示质量。

Method: 提出RGE方法，首先让模型基于指令执行结构化原理生成，然后在推理展开后提取表示，并将生成推理过程与对比训练相结合。

Result: 在MMEB基准测试中，推理引导的条件化使多模态检索性能比非推理基线提高了4.9%。

Conclusion: 明确的推理过程可以有效增强嵌入质量，将生成推理能力融入嵌入提取是提升多模态表示的有效途径。
...
TL;DR: PPCL是一种针对扩散变换器(DiTs)的灵活结构化剪枝框架，通过线性探测和相似度指标的一阶微分趋势分析识别冗余层区间，采用即插即用的师生交替蒸馏方案，在单一训练阶段内实现深度和宽度剪枝。该方法可将参数数量减少50%，关键客观指标退化小于3%。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在图像生成中表现出色，但参数量大导致计算成本高，阻碍在资源受限环境中的部署。需要开发高效的剪枝方法来减少计算负担。

Method: 1. 通过线性探测机制结合相似度指标的一阶微分趋势分析识别冗余层区间；2. 提出即插即用的师生交替蒸馏方案，在单一训练阶段内集成深度和宽度剪枝；3. 支持灵活的知识转移，无需为每个配置重新训练。

Result: 在多个多模态扩散变换器架构模型上的实验表明，PPCL相比完整模型实现了50%的参数减少，关键客观指标退化小于3%。方法保持高质量图像生成能力，同时实现更高的压缩比。

Conclusion: PPCL是一种适用于资源受限环境的高效扩散变换器剪枝框架，能够在显著减少参数的同时保持图像生成质量。
...
TL;DR: Video2Layout框架通过连续边界坐标重建度量空间布局，提升多模态大语言模型的空间推理能力，在基准测试中比网格地图方法平均提升4.92%


<details>
  <summary>Details</summary>
Motivation: 现有基于网格的认知地图方法依赖离散化栅格表示，限制了细粒度空间推理能力，需要克服这一局限性

Method: 采用连续对象边界坐标量化物体间物理距离和尺寸，包含监督微调和强化微调两个核心阶段，构建高质量数据集学习视觉输入到精确边界坐标的映射

Result: 在QVS-Bench和主流空间推理基准测试中，V2LO-7B模型相比网格地图训练模型平均提升4.92%

Conclusion: 连续边界坐标方法有效缓解了自然语言描述空间关系时的固有模糊性，验证了该方法的优越性
...
TL;DR: 本文提出Simba框架，将点云补全中的逐点变换回归重新定义为分布学习问题，结合对称先验和扩散模型的生成能力，避免过拟合并提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决点云补全中同时保留输入细节和确保全局结构完整性的挑战，克服现有回归方法容易过拟合和对输入噪声敏感的问题。

Method: 提出Simba框架，将逐点变换回归重新定义为分布学习问题，集成对称先验和扩散模型，采用基于Mamba的分层架构实现高保真上采样。
...
TL;DR: SurvAgent是一个基于分层思维链增强的多智能体系统，用于多模态生存预测，通过整合病理图像和基因数据，结合历史案例的经验学习，提供可解释的癌症预后预测。
...
Method: SurvAgent采用两阶段方法：1) WSI-基因CoT增强案例库构建，通过分层分析处理病理图像和基因数据，生成结构化报告；2) 基于二分法的多专家智能体推理，通过RAG检索相似案例并整合多模态报告进行渐进式区间精化。

Result: 在五个TCGA队列上的广泛实验表明，SurvAgent优于传统方法、专有MLLM和医疗智能体，为精准肿瘤学中的可解释AI驱动生存预测建立了新范式。

Conclusion: SurvAgent通过分层思维链增强的多智能体系统，成功解决了多模态数据整合、区域探索和经验学习等关键问题，为临床提供透明且准确的生存预测工具。
...
TL;DR: 提出LNG-SWR方法，通过层间噪声引导的选择性小波重构来提升医学图像分割模型的鲁棒性，无需依赖对抗训练即可在分布偏移和扰动下保持稳定。


<details>
  <summary>Details</summary>
Motivation: 临床部署需要分割模型在分布偏移和扰动下保持稳定。对抗训练存在清洁-鲁棒性权衡和高训练成本问题，限制了在医学影像中的可扩展性和可维护性。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态任务上的成功应用，其部署效率问题日益突出。现有方法只是延迟EOS token的出现来隐式延长输出，缺乏稳定性和可控性，无法直接最大化输出token长度作为显式优化目标。
...
Conclusion: 提出的VTIA方法能够有效诱导视觉语言模型产生冗长输出，解决了现有方法在稳定性和可控性方面的不足，为模型部署效率优化提供了新思路。
...
TL;DR: EvoVLA是一个自监督的视觉语言动作框架，通过阶段对齐奖励、基于姿态的对象探索和长时程记忆三个组件，解决了VLA模型在长时程机器人操作中的阶段幻觉问题，在仿真和真实世界中都取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在阶段幻觉问题，即智能体利用粗粒度评估信号来规避多步骤任务，报告高进度但未真正完成任务。这阻碍了长时程机器人操作的发展。

Method: 提出三个互补组件：1) 阶段对齐奖励(SAR)，使用三元组对比学习和Gemini生成的难负样本来防止视觉捷径；2) 基于姿态的对象探索(POE)，基于相对对象-夹爪姿态而非原始像素来激发好奇心；3) 长时程记忆，使用选择性上下文保留和门控融合来稳定扩展回合中的内在塑造。

Result: 在Discoverse-L长时程操作基准测试中，EvoVLA比最强基线(OpenVLA-OFT)平均任务成功率提高10.2个百分点，达到69.2%；样本效率提高1.5倍；阶段幻觉从38.5%降至14.8%。真实世界部署在四个操作任务中平均成功率达54.6%，优于OpenVLA-OFT 11个百分点。

Conclusion: EvoVLA有效解决了VLA模型的阶段幻觉问题，实现了有效的仿真到真实世界迁移和强泛化能力，为长时程机器人操作提供了有前景的解决方案。
...
TL;DR: 本文提出了RF-CLIP方法，通过模拟人类注意力分散-重新聚焦行为，解决CLIP在密集预测中注意力分散到无关token的问题，在八个基准测试上达到SOTA性能。
...
Method: 提出RF-CLIP训练自由方法，通过识别和过滤维度特异性过度激活产生的干扰token，重新将注意力引导回目标区域，从而提升CLIP的多模态对齐粒度。

Result: 在八个基准测试上达到最先进的性能，同时保持较高的推理效率。

Conclusion: 通过模拟人类注意力分散-重新聚焦行为，RF-CLIP有效提升了CLIP在密集预测中的性能，为多模态对齐提供了新的视角。
...
TL;DR: 本文提出了一种新颖的两阶段模型DSLGA，用于解决无监督域自适应可见光-红外行人重识别中的跨模态差异问题，通过领域共享学习和渐进对齐策略显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 由于公共数据集与现实世界数据之间存在差异，现有的可见光-红外行人重识别算法在实际应用中表现不佳。本文旨在研究无监督域自适应可见光-红外行人重识别，将公共数据中学到的知识迁移到现实世界数据中，同时不降低准确性且无需新样本标注。

Method: 设计了DSLGA两阶段模型：1）预训练阶段使用领域共享学习策略缓解领域间模态差异；2）微调阶段使用渐进对齐策略通过聚类到整体的方式处理领域内跨模态对齐挑战。还构建了新的测试方法CMDA-XD。

Result: 大量实验表明，该方法在各种设置下显著优于现有的VI-ReID域自适应方法，甚至超过了一些监督学习方法。

Conclusion: DSLGA模型通过有效处理领域间和领域内模态差异，为无监督域自适应可见光-红外行人重识别提供了有效的解决方案，在实际应用中具有重要价值。
...
Result: 在心脏、海马体和肺部实验中，PrIntMesh实现了高几何精度、正确拓扑结构，即使在有限或噪声训练数据下也具有鲁棒性能。
...
TL;DR: 本文提出了VLA-Fool框架，系统研究了视觉-语言-动作模型在多模态对抗攻击下的鲁棒性，包括文本、视觉和跨模态攻击，揭示了模型在对抗扰动下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言-动作模型在具身环境中取得了显著进展，但其对抗鲁棒性在真实多模态和黑盒条件下尚未得到充分探索，现有研究主要关注单模态扰动而忽视了影响具身推理和决策的跨模态错位问题。

Method: 提出VLA-Fool框架，统一了三种多模态对抗攻击：基于梯度和提示的文本扰动、基于补丁和噪声的视觉扰动、以及破坏感知与指令语义对应的跨模态错位攻击，并开发了首个自动构建的语义引导提示框架。

Result: 在LIBERO基准测试中使用微调的OpenVLA模型进行实验，结果表明即使轻微的多模态扰动也会导致显著的行为偏差，揭示了具身多模态对齐的脆弱性。

Conclusion: VLA-Fool框架揭示了视觉-语言-动作模型在多模态对抗攻击下的脆弱性，强调了在具身环境中考虑跨模态鲁棒性的重要性。
...
TL;DR: 提出基于视觉门控生成变换器（VGGT）的快速时空场景理解框架，支持接近实时的辅助导航应用，通过滑动窗口处理图像流和对齐子地图来解决VGGT的高内存需求。


<details>
  <summary>Details</summary>
Motivation: 开发高效的时空场景理解系统，支持辅助导航等实时应用，克服现有方法在内存需求和实时性能方面的限制。

Method: 使用VGGT框架，通过滑动窗口处理图像流和对齐子地图来更新3D场景表示；利用VGGT跟踪头将2D语义实例掩码聚合为3D对象；存储时间戳和实例级身份以实现时间一致性和上下文推理。
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型在多样化下游任务中表现出强大的泛化能力，但其表示通常被下采样14x/16x，限制了在像素级应用中的直接使用。现有特征上采样方法依赖数据集特定的重新训练或繁重的隐式优化，限制了可扩展性和泛化性。
...
TL;DR: 该论文提出将稀疏自编码器(SAE)重新理解为主题模型，并开发了SAE-TM框架，将SAE特征作为可重用的主题原子，支持跨模态的大规模主题分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在嵌入分析中的作用和实际价值存在争议，作者希望通过将其重新解释为主题模型来明确其角色和实用性。

Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出SAE目标作为该模型下的最大后验估计器，并开发SAE-TM框架：训练SAE学习可重用主题原子，将其解释为下游数据的词分布，无需重新训练即可合并成任意数量的主题。
...
Method: 提出潜在颜色扩散方法，包括几何感知的颜色点云VAE和多控制扩散变换器，完全使用3D数据训练；引入原生几何控制，通过位置嵌入和几何潜在变量为DiT提供直接3D空间信息；共同设计VAE-DiT架构，几何分支与颜色VAE紧密耦合。

Result: NaTex表现出强大的性能，在纹理一致性和对齐方面显著优于先前方法；同时展现出强大的泛化能力，可用于材质生成、纹理细化和部件分割与纹理化等下游应用。

Conclusion: NaTex通过原生3D纹理生成方法有效解决了传统2D多视图方法的局限性，在纹理质量、对齐精度和一致性方面取得了显著改进，并具有良好的扩展应用潜力。
...
TL;DR: 该研究提出了一种用于无人机遥感地形分类的弱监督学习方法，通过师生架构、关键帧选择和更新算法，仅使用30%标注数据就能同时提升mIoU和时序一致性。
...
Conclusion: 提出的框架成功克服了传统时序一致性训练在航空任务中的缺陷，为无人机遥感地形分类提供了有效的弱监督解决方案。
...
Method: 从预捕获的LiDAR点云渲染合成视图，建立实时帧与点云的2D-3D对应关系；使用神经渲染技术减少合成与真实图像间的域差距；提供在线渲染匹配和预构建定位两种实时变体。

Result: 在ScanNet++数据集上表现出改进效果，优于现有SLAM流程，实现无漂移的度量尺度相机跟踪。

Conclusion: 该方法能够提供可靠的无漂移相机定位，在机器人和XR应用中具有重要价值。
...
### [49] [Multi-Order Matching Network for Alignment-Free Depth Super-Resolution](https://arxiv.org/abs/2511.16361)
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中由于硬件限制和校准漂移，RGB-D数据往往无法严格对齐，导致现有方法性能下降。需要开发能够处理不对齐数据的深度超分辨率方法。

Method: 使用多阶匹配机制（零阶、一阶、二阶匹配）在多个特征空间中识别与深度一致的RGB信息，并通过多阶聚合结构检测器选择性融合特征。

Result: 实验表明MOMNet实现了最先进的性能，并展现出优秀的鲁棒性。

Conclusion: MOMNet框架能够有效处理不对齐的RGB-D数据，为现实场景中的深度超分辨率提供了实用解决方案。
...
TL;DR: CAMS是一个基于CLIP的组成式零样本学习方法，通过门控交叉注意力机制和多空间解缠技术，从视觉特征中提取语义特征并实现属性与对象的语义解缠，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要依赖图像编码器获得的全局语义表示，但这种表示能力有限，无法完全解缠属性和对象。因此需要开发能够提取细粒度语义特征并进行有效语义解缠的方法。

Method: 提出CAMS方法：1）设计门控交叉注意力机制，通过一组潜在单元从CLIP的高层图像编码块中捕获细粒度语义特征，同时自适应抑制背景和其他无关信息；2）进行多空间解缠，实现属性和对象语义的分离。

Result: 在三个流行基准测试（MIT-States、UT-Zappos和C-GQA）上的实验表明，CAMS在封闭世界和开放世界设置下都达到了最先进的性能。

Conclusion: CAMS通过提取语义特征并在多维空间中进行语义解缠，有效提高了对未见属性-对象组合的泛化能力，为组成式零样本学习提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督环视深度估计方法存在重叠图像间深度估计不一致的问题，需要解决多相机系统下深度预测的跨视图一致性挑战。

Method: 基于标定和时间同步的多相机系统，首先预测每张图像的初始深度图，然后将所有图像的3D点投影到共享圆柱体上建立邻域关系，生成位置图，最后应用非学习的空间注意力机制跨图像聚合特征来预测最终深度。

Result: 在DDAD和nuScenes数据集上的评估表明，该方法相比现有最优方法显著提升了深度估计的跨视图一致性和整体深度精度。

Conclusion: 提出的几何引导方法能够有效实现密集、度量化且跨视图一致的深度估计，为环视3D感知提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有少样本分割方法主要从支持图像中挖掘参考作为元指导，但由于视觉表示的类内差异，从支持图像提取的元信息无法为未训练类提供准确的分割指导。作者认为支持图像提供的参考可能不是必需的，关键在于为训练和未训练类提供无偏的元指导。

Method: 提出LDAG架构，包含多属性增强（MaE）模块和多模态属性对齐（MaA）模块。MaE通过大语言模型生成目标类的多个详细属性描述，并利用多模态匹配构建精炼的视觉-文本先验指导；MaA实现属性文本与视觉特征之间的跨模态交互。

Result: 实验表明，该方法明显优于现有方法，实现了新的最先进性能。

Conclusion: 通过语言驱动的属性泛化方法，能够为少样本分割任务提供更准确和鲁棒的元指导，有效解决了传统方法中因类内差异导致的指导不准确问题。
...
TL;DR: 提出了StreetView-Waste数据集，专注于城市垃圾容器检测、跟踪和溢出分割任务，并提供了改进的启发式跟踪方法和几何感知分割策略。


<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器溢出监控的专门关注，特别是在垃圾车采集图像的实际场景中，限制了在真实物流环境中的应用。

Method: 构建包含城市垃圾场景的综合数据集，支持三个评估任务：垃圾容器检测、跟踪和溢出分割。提出启发式方法改进容器跟踪，以及利用几何先验的模型无关框架优化垃圾分割。
...
TL;DR: VLA-Pruner是一种针对视觉-语言-动作模型的双层级token剪枝方法，通过结合语义相关性和动作重要性来保留关键视觉token，在保持性能的同时实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM token剪枝方法仅基于语义显著性选择token，忽视了VLA模型的双系统特性（高层语义理解和低层动作执行），导致动作生成所需的关键信息被丢弃，性能下降。

Method: 提出双层级重要性标准：视觉-语言预填充注意力用于语义相关性，通过时间平滑估计的动作解码注意力用于动作重要性。采用自适应双层级token选择策略，在计算预算内保留紧凑且信息丰富的视觉token集合。

Result: 在多种VLA架构和多样化机器人任务上实现了最先进的性能。

Conclusion: VLA-Pruner是一种通用即插即用的VLA专用token剪枝方法，通过结合语义和动作双层级重要性，有效解决了VLA模型实时部署的计算效率问题。
...
Method: 受立体主义画家启发，通过多视角3D重建生成对象的全方位视觉表示，将这些表示输入VLM来描述3D场景。

Result: 在3D视觉问答和3D语言定位任务上的广泛实验表明，该方法优于之前的基于2D的VLM解决方案。

Conclusion: LLaVA³证明了仅使用多视角2D图像就能有效提升VLM的3D场景理解能力，无需额外3D训练数据或模型微调。
...
TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别中矢状切片、分割胼胝体和穹窿、定位前后连合以标准化头部位置，生成厚度剖面和细分，并提取8个形状指标用于统计分析。
...
Method: 开发了FastSurfer-CC框架，包含自动识别中矢状切片、分割胼胝体和穹窿、定位前后连合、生成厚度剖面和细分、提取形状指标等功能。

Result: FastSurfer-CC在各项任务中优于现有专业工具，并揭示了亨廷顿病患者与健康对照组之间当前最先进方法未能检测到的统计学显著差异。
...
TL;DR: 该论文研究了在遮挡场景下的视频预测问题，通过引入点流（point-flow）和深度图（depth-maps）信息来提升视频预测模型在遮挡和背景运动情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然通用视频预测模型在标准场景中表现优异，但遮挡仍然是视频预测中的固有挑战。作者假设通过提供明确的运动信息（通过点流）和几何结构信息（通过深度图），能够使视频预测模型在遮挡和背景运动情况下表现更好。

Method: 使用标准的多对象潜在变换器架构来预测未来帧，但修改该架构以整合深度和点流信息。在合成和真实世界数据集上进行了受控评估，不仅使用基于外观的指标，还使用对象掩码上的Wasserstein距离来有效测量预测的运动分布。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺DCE-MRI在癌症筛查和治疗中很重要，但不同采集协议和个体因素导致组织外观差异大，使自动肿瘤分割变得困难。
...
TL;DR: 提出了一个专门用于拳击击打检测和分类的综合视频数据集，包含6,915个高质量击打片段，分为六种击打类型，来自20个YouTube训练视频，涉及18名运动员。


<details>
  <summary>Details</summary>
Motivation: 由于动态、非结构化的动作和录制环境变化，开发鲁棒的数据集一直是计算机视觉分析格斗运动的主要瓶颈。
...
TL;DR: 本文提出SemCLIP方法，通过结合释义和否定处理，改进CLIP模型的对比损失函数，在保持原有性能的同时显著提高对否定文本的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前对比视觉语言模型在处理否定和释义文本时表现不一致，否定会彻底改变含义但词汇变化很小，而释义会产生不同表达但含义相同，这给模型评估和对齐带来挑战。

Method: 提出SemCLIP方法，使用LLM生成的包含原始、释义和否定文本描述的训练三元组，设计新的对比损失函数来同时考虑释义和否定。

Result: 在CC-Neg基准测试中，图像检索准确率从68.1%提升到78.1%；在Sugarcrepe++基准测试中表现优于仅使用否定训练的模型；在零样本分类任务中全面优于CLIP。
...
TL;DR: 提出一种鲁棒的多摄像机体操运动员跟踪方法，通过结合体操领域知识（运动员3D中心位于预定垂直平面）和级联数据关联策略，在交叉视图检测不足时使用射线-平面相交生成3D轨迹候选，成功应用于国际体操锦标赛裁判系统。


<details>
  <summary>Details</summary>
Motivation: 解决体操运动员跟踪中的独特挑战：摄像机数量有限、光照变化、背景复杂、遮挡等导致多摄像机检测可能在某些视图失败，传统多摄像机三角测量难以准确确定3D轨迹。

Method: 提出级联数据关联范式：当交叉视图检测充足时使用三角测量生成3D轨迹候选，检测不足时采用射线-平面相交方法生成共面3D轨迹候选，利用体操运动员3D中心位于预定垂直平面的领域知识。
...
<details>
  <summary>Details</summary>
Motivation: 研究不同光流计算方法在估计帧间运动时的有效性，特别是在变化图像条件下的表现。

Method: 采用局部方法（如Lucas-Kanade）和全局方法（如Horn-Schunck），并实现基于双线性插值和延拓的多分辨率Horn-Schunck算法。

Result: 多分辨率策略提高了光流估计的准确性和收敛性，在变化图像条件下表现更优。

Conclusion: 结合多分辨率技术的Horn-Schunck算法能够有效改进光流计算性能，特别是在复杂图像条件下。
...
TL;DR: 提出了一种新颖的两阶段检测框架，用于解决合成图像检测中的泛化挑战，通过监督对比学习提取特征嵌入，结合k-NN分类器在少样本学习范式下实现对新生成器的检测。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展使得合成图像越来越难以与真实内容区分，传统检测方法依赖周期性重新训练，在新型生成模型快速发布的情况下计算不可行且操作不实用。

Method: 第一阶段使用监督对比学习训练视觉深度学习模型提取特征嵌入，训练时策略性地保留部分生成器架构以测试跨生成器泛化能力；第二阶段在学习的嵌入空间中使用k-NN分类器，采用少样本学习范式整合未见测试生成器的有限样本。
...
TL;DR: EOGS++是一种针对卫星图像的新型3D重建方法，基于高斯泼溅技术，直接在原始高分辨率全色数据上操作，无需外部预处理，并通过集成光流技术和束调整改进相机姿态估计，在重建质量和效率方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅作为NeRF的替代方案在遥感领域显示出潜力，但现有方法依赖外部预处理工具，且相机姿态估计不够精确。本研究旨在开发一个端到端的解决方案，直接在原始卫星数据上运行，并改进重建精度。

Method: 扩展地球观测高斯泼溅框架，提出EOGS++方法：1) 直接在原始高分辨率全色数据上操作；2) 利用光流技术将束调整嵌入训练过程；3) 引入早停和TSDF后处理等改进措施。

Result: 在IARPA 2016和DFC2019数据集上的实验表明，EOGS++在重建质量和效率方面达到最先进性能，相比原始EOGS模型和其他基于NeRF的方法表现更好，建筑物重建的平均MAE误差从1.33改善到1.19。

Conclusion: EOGS++成功地将高斯泼溅技术应用于卫星图像处理，通过端到端的处理和集成的束调整机制，显著提升了3D重建的几何精度和视觉质量，同时保持了计算效率优势。
...
TL;DR: 本文针对长尾分布下的2D目标检测问题，使用LVISv1数据集，通过改进平衡组Softmax框架和探索度量学习，在Faster R-CNN架构上实现了24.5%的mAP，创下新纪录。
...
Method: 采用两阶段Faster R-CNN架构，改进平衡组Softmax框架缓解类别不平衡，并探索度量学习生成特征嵌入，使用k-NN方法进行推理。

Result: 在LVISv1数据集上达到24.5%的mAP，超越了之前24.0%的最佳性能。

Conclusion: 提出的方法在长尾目标检测中有效，特别是对稀有类别的分类性能有显著提升。
...
TL;DR: 提出了一种新颖的教师引导剪枝框架，将知识蒸馏与重要性分数估计紧密结合，实现一次性全局剪枝，在多个图像分类基准测试中达到高稀疏度且性能损失最小。
...
Method: 使用教师引导的剪枝框架，在重要性分数计算过程中利用教师模型的梯度信号，识别并保留对任务性能和知识转移都至关重要的参数，采用一次性全局剪枝策略，剪枝后使用稀疏感知重训练恢复精度。

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等多个图像分类基准测试中，该方法在高稀疏度水平下始终实现最小性能退化，优于EPG和EPSD等最先进基线方法。
...
TL;DR: 本文提出了SAM2S模型，通过构建最大的手术视频分割基准SA-SV，并引入DiveMem记忆机制、时序语义学习和抗模糊学习，显著提升了手术交互式视频对象分割的性能。
...
Conclusion: SAM2S通过专门设计的记忆机制和学习策略，有效解决了手术视频分割的挑战，为手术计算机辅助系统提供了强大的基础模型。
...
TL;DR: 提出自适应引导上采样(AGU)方法，通过多参数优化同时提升低光照图像的信噪比和锐度，在实时处理中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统引导图像方法在低光照条件下效果不佳，因为低光照图像噪声高、亮度低，缺乏足够的图像特征用于引导传输。

Method: 基于引导图像方法，通过多参数优化学习低光照与明亮图像特征之间的关联，仅需少量样本图像对进行机器学习训练。

Result: AGU能够使用低质量、低分辨率输入实时渲染高质量图像，在低光照场景下优于现有最先进方法。

Conclusion: AGU方法有效解决了低光照图像上采样中的特征不足问题，通过多参数优化实现了噪声降低和锐度提升的同步优化。
...
TL;DR: 本文对Cambrian-S提出的视频空间超感知基准进行了批判性分析，发现VSR基准可通过简单词袋模型近乎完美解决，而VSC基准存在捷径启发式问题，Cambrian-S的推理方法可能利用了这些捷径而非真正的空间超感知能力。


<details>
  <summary>Details</summary>
Motivation: 批判性评估Cambrian-S提出的视频空间超感知基准和推理方法的有效性，验证这些基准是否真正测量了空间认知和世界建模能力。

Method: 引入NoSense基线方法（仅使用词袋SigLIP模型，丢弃时间结构）测试VSR基准；设计VSC-Repeat实验（将视频与自身连接1-5次）测试VSC基准的稳健性。

Result: NoSense在VSR上达到95%准确率，表明无需空间认知即可近乎解决该基准；VSC-Repeat实验使Cambrian-S的准确率从42%降至0%，显示其推理方法依赖捷径而非空间超感知。

Conclusion: 当前VSI-Super基准不能可靠测量空间超感知能力，Cambrian-S的推理方法通过利用基准中的捷径而非稳健的空间超感知来提高性能。
...
TL;DR: PartUV是一个基于部件分割的UV展开方法，针对AI生成网格的噪声和不良条件问题，通过语义部件分解和几何启发式方法生成更少、部件对齐的图表，同时保持低失真。
...
Method: 基于PartField学习部件分解方法，结合高层语义部件分解和新几何启发式，采用自上而下的递归框架，确保每个图表的失真低于用户指定阈值，同时最小化图表总数。

Result: 在四个不同数据集上的评估表明，PartUV在图表数量和接缝长度方面优于现有工具和神经方法，失真程度相当，在挑战性网格上具有高成功率，并支持部件特定的多瓦片打包等新应用。

Conclusion: PartUV通过部件基础的UV展开方法，有效解决了AI生成网格的UV展开问题，生成更少、部件对齐的图表，为下游任务提供了更好的基础。
...
TL;DR: TriDiff-4D是一个创新的4D生成管道，使用基于扩散的三平面重定位技术生成高质量、时间一致的4D虚拟形象，通过自回归策略实现任意长度的4D序列生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法在时间几何不一致性、感知伪影、运动不规则性、高计算成本和动态控制有限等方面的根本限制。

Method: 采用自回归策略，每个3D帧通过单一扩散过程合成；首先从文本提示生成规范3D虚拟形象和相应运动序列，然后使用第二个扩散模型根据运动序列为虚拟形象添加动画。

Result: 显著优于现有方法，将生成时间从数小时减少到数秒，同时大幅改善了复杂运动的生成质量，具有高保真外观和准确的3D几何形状。

Conclusion: TriDiff-4D在时间一致性、运动准确性、计算效率和视觉保真度方面表现出色，支持骨架驱动的4D生成，为可控4D虚拟形象生成提供了有效解决方案。
...
TL;DR: V-ReasonBench是一个评估视频推理能力的基准，涵盖结构化问题解决、空间认知、模式推理和物理动力学四个维度，使用合成和真实图像序列构建，为视频模型提供可重现、可扩展的评估框架。
...
Method: 构建V-ReasonBench基准，包含来自合成和真实世界图像序列的多样化、答案可验证的任务，评估六个最先进的视频模型，并与强图像模型进行比较分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要追求冗长的推理链，导致计算成本过高，而人类会根据问题复杂度灵活分配认知资源。

Method: 采用两阶段方法：第一阶段基于模型输出长度标注数据需要快速或慢速思维；第二阶段使用GRPO和思维模式标签训练模型发展双模式思维。

Result: DualMindVLM模型显著优于基础模型，性能与最先进视觉推理模型相当，同时保持极高的token效率。

Conclusion: 提出的简单RL方法成功实现了视觉语言模型的双模式思维，在性能和效率之间取得了良好平衡。
...
TL;DR: NoPo-Avatar：一种仅从图像重建可动画3D人体化身的无姿态输入方法，在无真实姿态的实际场景中优于现有基线方法


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖准确的相机姿态和人体姿态作为输入，但当姿态估计存在噪声时，重建效果会显著下降。需要一种不依赖姿态输入的重建方法

Method: 提出NoPo-Avatar方法，仅从单张或稀疏图像重建3D人体化身，完全不需要任何姿态输入，避免了噪声姿态估计的影响

Result: 在THuman2.0、XHuman和HuGe100K数据集上的实验表明，在无真实姿态的实际设置中优于现有基线，在有真实姿态的实验室设置中也能达到可比效果

Conclusion: NoPo-Avatar通过消除对测试时姿态输入的依赖，在噪声姿态环境下表现更稳健，具有更广泛的应用前景
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型谎言检测技术通常在狭窄的设置中进行验证，无法捕捉模型可能生成的各种谎言类型，需要更全面的测试平台来评估这些技术。

Method: 创建LIARS' BENCH测试平台，包含四个开源模型在七个数据集上生成的谎言和诚实回答，涵盖不同维度的谎言类型：模型撒谎的原因和谎言针对的信念对象。

Result: 评估三种黑盒和白盒谎言检测技术后发现，现有技术在某些类型的谎言检测上系统性失败，特别是在仅从文本转录无法确定模型是否撒谎的情况下。
...
TL;DR: GPT-5在多个科学领域（数学、物理、天文学、计算机科学、生物学、材料科学）中展示了加速科研进程的能力，通过案例研究展示了人机协作的有效模式，并在数学领域产生了四个经过验证的新结果。


<details>
  <summary>Details</summary>
Motivation: 展示前沿AI模型GPT-5在科学研究中的实际应用价值，帮助科学家了解AI的能力边界，促进人机协作研究模式的发展。

Method: 通过多个跨学科案例研究，记录研究人员与GPT-5的互动过程，分析AI在哪些方面加速了研究，在哪些方面仍需人类专家参与。

Result: GPT-5在多个科学领域成功推动了研究进展，特别是在数学领域产生了四个经过人类验证的新结果，证明了AI在解决未解决问题方面的潜力。

Conclusion: 前沿AI模型如GPT-5已成为科学研究的有力工具，虽然贡献范围有限但意义深远，随着AI技术的快速发展，其在科学发现中的作用将日益重要。
...
TL;DR: 本文提出了一种基于集成学习的提示优化框架ELPO，通过投票机制和共享生成策略结合不同搜索方法来优化提示，在多个任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法通常基于单一模型或算法，在处理复杂任务时性能受限，需要更准确和鲁棒的优化框架。
...
Conclusion: ELPO框架通过集成学习方法有效提升了提示优化的准确性和鲁棒性，为复杂任务提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统参数高效微调（PEFT）方法对所有位置索引应用修改，但作者质疑这种全面修改的必要性，认为可能造成资源浪费甚至负面影响。

Method: 引入TS-PEFT范式，使用函数S选择性地对位置索引子集应用PEFT修改，而非对所有索引进行无差别修改。

Result: 实验结果表明，对所有索引无差别应用PEFT不仅多余，甚至可能适得其反。
...
TL;DR: SemanticCite是一个AI驱动的引文验证系统，通过全文分析验证引文准确性，提供四类分类（支持、部分支持、不支持、不确定）来捕捉细微的声明-来源关系，并针对不同错误类型采取适当补救措施。


<details>
  <summary>Details</summary>
Motivation: 解决学术文献中语义引文错误、AI生成幻觉引用以及传统引文格式无法指示具体支持段落的问题，确保科学交流的准确性。
...
Conclusion: SemanticCite通过可扩展的引文验证、简化同行评审和AI生成内容的质量控制，为大规模维护引文准确性提供了开源基础，解决研究完整性的关键挑战。
...
TL;DR: 本文提出了语义结构熵(SeSE)框架，从结构信息角度量化大语言模型的语义不确定性，用于幻觉检测。通过构建自适应稀疏有向语义图，并利用层次抽象挖掘潜在语义结构信息，显著提升了不确定性量化性能。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法主要依赖语义概率分布或成对距离，忽略了潜在语义结构信息，无法提供精确的不确定性估计。在安全关键场景中，可靠的不确定性量化对于防止模型产生幻觉至关重要。

Method: 开发自适应稀疏有向语义图构建算法，捕获方向性语义依赖并自动修剪干扰连接；定义语义结构熵为最优语义编码树的结构熵，通过层次抽象形式化语义空间的内在不确定性；扩展SeSE到长文本生成中量化单个声明的不确定性。

Result: 在29个模型-数据集组合上的广泛实验表明，SeSE显著优于先进的不确定性量化基线方法，包括强监督方法和最近提出的KLE方法。

Conclusion: SeSE框架通过挖掘语义结构信息，为大语言模型提供了更可靠的不确定性量化，有效检测幻觉生成，在安全关键应用中具有重要价值。
...
TL;DR: 提出了SDA（Steering-Driven Distribution Alignment）框架，这是一个无需训练、模型无关的对齐方法，通过动态调整模型输出概率分布来增强LLMs与人类意图的对齐。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在现实应用中的广泛部署，确保模型响应与人类意图对齐成为关键挑战。需要在不进行昂贵重训练或大量监督的情况下，在推理过程中有效且高效地对齐模型行为。

Method: SDA基于用户定义的对齐指令动态重新分配模型输出概率，无需微调即可增强模型行为与人类意图的对齐。该方法轻量级、资源高效，兼容各种开源LLMs，可独立使用或与基于训练的对齐策略集成。

Result: 在8个不同规模和来源的开源LLMs上评估，SDA在帮助性、无害性和诚实性三个关键对齐维度上均取得显著提升：帮助性平均提升64.4%，诚实性提升30%，无害性提升11.5%。

Conclusion: SDA是一种有效且通用的对齐框架，能够显著提升LLMs在不同模型和应用场景下的对齐性能，支持个性化偏好对齐，为模型行为控制提供了灵活的方法。
...
TL;DR: 该论文提出了一种自重写框架，通过让模型重写自己的推理文本并从中学习，来提升内部推理过程质量，解决了传统强化学习仅关注最终正确性的局限性。
...
Method: 提出选择性重写方法，只对模型持续正确的"简单"样本进行重写，保持GRPO的所有原始奖励信号；在单个批次中同时进行重写和原始生成，仅增加约10%的开销。

Result: 在准确率-长度权衡方面，自重写方法在准确率提升0.6%的同时显著缩短推理长度46%；在内部推理质量方面，LLM作为评判者的评分显著提高7.2分，成功缓解了内部推理缺陷。

Conclusion: 自重写框架有效提升了推理模型的内部推理质量，在保持强化学习可扩展性的同时显著改善了准确率-长度权衡，并成功缓解了各种推理缺陷。
...
TL;DR: 本文创建了大规模习语和比喻语言数据集，用于评估预训练语言模型在处理比喻意义方面的能力，通过习语识别任务来缩小LLMs在理解非正式语言方面的差距。


<details>
  <summary>Details</summary>
Motivation: 习语和比喻语言在口语和书面语中占很大比例，但大型语言模型在这方面仍然存在困难。虽然大型语料库看似是解决方案，但习语和比喻语言继续困扰着LLMs。微调方法被证明是最优的，但更好更大的数据集可以进一步缩小这一差距。

Method: 使用近期习语和比喻语言数据集获取组合习语列表，从大型语料库中检索上下文序列。创建了一个大规模潜在习语和比喻语言表达数据集，以及两个额外的人工标注确定习语和比喻语言表达数据集。对结果数据集进行后处理以实现模型无关的训练兼容性，用于训练，并在槽位标注和序列标记上进行评估。

Result: 创建了三个数据集：一个大规模潜在习语和比喻语言表达数据集，两个人工标注的确定习语和比喻语言表达数据集，用于评估预训练语言模型通过习语识别任务处理比喻意义的能力。

Conclusion: 本文提供的数据集为解决LLMs在习语和比喻语言理解方面的挑战提供了一个答案，同时提供了多样化的类别来构建新模型和开发新方法。
...
TL;DR: 本文探讨了自然语言解释（rationales）在模型评估中的作用，发现传统的充分性指标存在局限性，无法全面反映解释信息对模型性能的影响。
...
Method: 通过将充分性与两种建模范式关联：通过标记分类识别rationales标记的能力，以及通过注意力正则化在输入中融入rationales来提升模型性能的能力。

Result: 研究发现高度信息化的rationales未必有助于正确分类实例；充分性反而捕捉了非rationalized上下文对分类的影响；融入rationales信息可以提升跨领域分类，但结果因任务和模型类型而异；充分性与标记分类似乎无关。

Conclusion: rationales的复杂性表明需要进一步研究能够系统捕捉此类信息的度量指标。
...
TL;DR: MinerU-HTML是一个基于语言模型的HTML到文本提取管道，将内容提取重新定义为序列标注问题，显著提升了网页内容提取质量，特别是在结构化元素（代码、公式、表格）的保留方面。


<details>
  <summary>Details</summary>
Motivation: 现有网页语料库依赖基于启发式的提取器（如Trafilatura），这些方法难以保留文档结构并经常损坏结构化元素。作者假设提高提取质量对下游性能的影响与积极过滤策略同样重要。

Method: 提出MinerU-HTML提取管道，使用0.6B参数的语言模型将内容提取作为序列标注问题解决，采用两阶段格式化流程，先明确分类语义元素再转换为Markdown格式。

Result: 在MainWebBench基准测试中，MinerU-HTML达到81.8% ROUGE-N F1，显著优于Trafilatura的63.6%，在结构化元素保留方面表现优异（代码块90.9%，公式94.0%）。使用MinerU-HTML构建的AICC语料库在13个基准测试中平均准确率达到50.8%，优于Trafilatura提取的语料库。

Conclusion: HTML提取是网页语料库构建中一个关键但经常被低估的组成部分，基于模型的提取方法比启发式方法具有更好的可扩展性和改进路径。
...
TL;DR: 本研究探讨了机器学习和深度学习模型是否能有效区分感知低质量与高质量新闻文章，使用141万篇新闻数据，发现传统机器学习（如随机森林）和深度学习模型（如ModernBERT）都能有效区分新闻质量。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习和深度学习模型在区分新闻质量方面的有效性，以帮助自动识别低质量新闻内容。

Method: 使用3种机器学习分类器和3种深度学习模型，基于141万篇英语新闻文章数据集，利用194个语言特征进行分类。

Result: 随机森林准确率0.7355，ROC AUC 0.8131；ModernBERT-large最佳性能：准确率0.8744，ROC-AUC 0.9593，F1 0.8739。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对ESG领域问答系统的评估标准，需要建立透明和可问责的ESG人工智能系统。

Method: 构建包含多ESG主题的领域基础问题数据集，配以人工标注的答案和支撑证据，用于细粒度评估模型推理过程。
...
Method: 使用改进的路径修补算法进行电路发现，识别和分析"习语头"（在多种习语中频繁激活的注意力头）以及习语标记间的"增强接收"现象。

Result: 发现习语处理具有独特的计算模式，识别出专门的习语处理电路，这些电路通过增强习语标记间的注意力连接来平衡计算效率和鲁棒性。
...
TL;DR: Arctic-Extract是一个最先进的模型，用于从扫描或数字化的商业文档中提取结构化数据（问答、实体和表格），具有轻量级部署特性。
...
Method: 采用特定的训练协议，优化模型大小至6.6 GiB，使其能够在A10 GPU等有限资源设备上运行。

Result: 模型在A10 GPU（24GB内存）上能够处理多达125页A4文档，在文档理解任务中表现出色。

Conclusion: Arctic-Extract在保持最先进性能的同时实现了轻量级部署，为资源受限环境下的文档处理提供了实用解决方案。
...
TL;DR: TurkColBERT是首个针对土耳其语检索的全面基准，比较了密集编码器和延迟交互模型。结果显示延迟交互模型在参数效率上显著优于密集编码器，ColmmBERT-base-TR在特定领域任务中mAP提升达13.8%。MUVERA+Rerank索引算法比PLAID快3.33倍，并带来1.7%的相对mAP增益。
...
Method: 采用两阶段适应流程：先在土耳其NLI/STS任务上微调英语和多语言编码器，然后使用在MS MARCO-TR上训练的PyLate将其转换为ColBERT风格的检索器。评估了10个模型在五个土耳其BEIR数据集上的表现。

Result: 延迟交互模型在参数效率上表现突出：1.0M参数的colbert-hash-nano-tr比600M的turkish-e5-large密集编码器小600倍，但保留了其平均mAP的71%以上。比密集编码器小3-5倍的延迟交互模型显著优于它们；ColmmBERT-base-TR在特定领域任务中mAP提升达13.8%。

Conclusion: 延迟交互模型为土耳其语检索提供了高效的解决方案，特别是在参数效率和特定领域性能方面。MUVERA+Rerank实现了低延迟检索，ColmmBERT-base-TR在MUVERA下达到0.54毫秒查询时间。但存在依赖中等规模数据集和翻译基准的局限性。
...
Result: 在两个数据集上分别实现了98%和71%的F1分数，结果始终优于对照任务，证明文本体裁可以从LLM中有效推断。
...
TL;DR: 该论文挑战了ASR评估中过度依赖词错误率(WER)的标准，发现WER等现有指标与临床风险相关性差，提出使用LLM作为评估器来复制专家临床评估，实现了90%的准确率。


<details>
  <summary>Details</summary>
Motivation: 随着ASR在临床对话中的广泛应用，现有评估标准主要依赖WER，但缺乏对临床影响的有效评估，需要建立更符合临床安全需求的评估框架。

Method: 建立专家临床评估的黄金标准基准，使用GEPA程序化优化LLM作为评估器(Gemini-2.5-Pro)，在两个医患对话数据集上验证其性能。

Result: WER和现有指标与临床风险标签(无影响、最小影响、显著影响)相关性差；优化的LLM评估器达到90%准确率和0.816的Cohen's κ系数，表现接近人类专家水平。
...
TL;DR: 提出了一种无需人工标注训练数据的词义消歧方法，利用统计语言模型作为消歧器，将符号NLU系统生成的候选含义转换为可区分的自然语言替代项，通过LLM选择合适解释。
...
TL;DR: 本文比较了多模态RAG系统中的两种检索方法：基于文本分块检索和直接多模态嵌入检索。实验证明直接多模态嵌入检索在金融财报问答任务中显著优于基于LLM摘要的方法。
...
Method: 比较两种检索方法：文本分块检索（图像先被LLM摘要为文本）和直接多模态嵌入检索（图像以原生形式存储在向量空间），在6个LLM模型和2个多模态嵌入模型上进行评估。

Result: 直接多模态嵌入检索显著优于基于LLM摘要的方法，在mAP@5上绝对提升13%（相对提升32%），在nDCG@5上绝对提升11%（相对提升20%），且能生成更准确和事实一致的答案。
...
### [98] [Fluid Reconfigurable Intelligent Surface (FRIS) Enabling Secure Wireless Communications](https://arxiv.org/abs/2511.15860)
*Xusheng Zhu,Kai-Kit Wong,Boyi Tang,Wen Chen,Chan-Byoung Chae*

Main category: cs.IT

TL;DR: 本文提出流体可重构智能表面(FRIS)概念，通过允许反射元件位置可重构来增强物理层安全。相比传统固定位置RIS，FRIS能从候选位置中动态选择最优元件子集，联合优化AP波束成形、FRIS元件选择和离散相移来最大化保密速率。


<details>
  <summary>Details</summary>
Motivation: 传统RIS的反射元件位置固定，限制了系统性能优化。FRIS通过引入元件位置可重构这一新自由度，能够显著提升物理层安全性能。

Method: 采用交替优化(AO)框架，其中波束成形子问题使用广义特征值方法闭式求解，元件选择和离散相移联合子问题使用交叉熵优化(CEO)方法处理。

Result: 仿真结果表明，所提出的FRIS设计显著优于传统RIS和其他基线方法，证明了元件位置作为新自由度带来的安全增益。

Conclusion: FRIS通过元件位置可重构提供了新的优化自由度，能够有效提升无线通信系统的物理层安全性能。
...
TL;DR: 本文提出了一个基于大语言模型的集成框架（eLLM），用于非结构化文本分类，通过整合多个模型解决了单个系统的弱点，在IAB分层分类法上实现了高达65%的F1分数提升，接近人类专家水平。
...
Result: eLLM框架相比最强单个模型实现了高达65%的F1分数提升，个体模型由于语义丰富文本被压缩为稀疏分类表示而性能停滞，而eLLM提高了鲁棒性和准确性，接近人类专家水平。

Conclusion: eLLM为基于分类法的分类提供了可扩展且可靠的解决方案，可能显著减少对人类专家标注的依赖，通过模型多样性实现了近乎人类专家水平的性能。
...
TL;DR: Graph-Memoized Reasoning是一个图结构记忆框架，用于表示、存储和重用推理工作流，通过编码过去的决策图并通过结构和语义相似性检索，实现跨任务的计算步骤复用，减少重复计算。
...
Method: 引入图记忆推理框架，将推理工作流表示为图结构记忆，通过编码过去的决策图并通过结构和语义相似性检索，实现子图的组合式重用。

Result: 提出了最小化总推理成本并通过存储与生成工作流间不一致性进行正则化的优化目标，为智能系统中的效率-一致性权衡提供了理论基础，并设计了相应的概念评估协议。
...
TL;DR: MACIE是一个多智能体因果智能解释框架，通过结合结构因果模型、干预性反事实和Shapley值，为多智能体强化学习系统提供全面解释。


<details>
  <summary>Details</summary>
Motivation: 随着多智能体强化学习系统在安全关键应用中的使用，理解智能体决策原因和集体行为实现方式变得至关重要。现有的可解释AI方法在多智能体环境中表现不佳。

Method: MACIE框架结合结构因果模型、干预性反事实和Shapley值，通过干预性归因分数、协同指标和自然语言叙述来解决三个关键问题。

Result: 在四种MARL场景中的评估显示：准确的结果归因（平均φ_i=5.07，标准差<0.05）、合作任务中检测到积极涌现（协同指数高达0.461）、高效计算（CPU上每个数据集0.79秒）。
...
TL;DR: 该研究分析了不同模态（文本和图像）在ARC-AGI任务中对模型感知的影响，发现结构化文本能精确定位稀疏特征，图像能捕捉2D形状但对分辨率敏感，结合两种模态可提升执行准确性。


<details>
  <summary>Details</summary>
Motivation: 当前指令优先系统虽然能将网格转换为自然语言或DSL规则，但缺乏对编码方式如何影响模型感知的系统性分析，以及如何区分指令错误和执行错误。研究者假设不同模态会带来感知瓶颈，从而影响网格特征的可靠感知。

Method: 使用加权集合分歧度量和两阶段推理流程，在九种文本和图像模态中分离感知与推理，比较不同表示方式对模型感知的影响。

Result: 结构化文本在稀疏特征上能提供精确坐标，图像能保留2D形状但对分辨率敏感，结合文本和图像模态可提高约8个感知点和0.20的中位相似度。
...
### [103] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
*William Brach,Lukas Galke Poech*

Main category: cs.AI

TL;DR: 提出Chain of Summaries (CoS)方法，通过类黑格尔辩证法迭代生成信息密集的通用摘要，使网页内容更易于LLM消化，在多个数据集上显著优于零样本LLM和专用摘要方法。
...
Method: 受黑格尔辩证法启发，CoS方法迭代优化初始摘要（正题），通过质疑识别局限性（反题），最终生成满足当前和未来信息需求的通用摘要（合题）。

Result: 在TriviaQA、TruthfulQA和SQUAD数据集上，CoS比零样本LLM基线提升66%，比BRIO和PEGASUS等专用摘要方法提升27%。CoS摘要使用更少token且与下游LLM无关。
...
TL;DR: 本论文提出了一个多模态AI框架，结合文本和图像分析来识别建筑工地的安全隐患。通过两个案例研究评估了大型语言模型和视觉语言模型在自动化危险识别中的能力。
...
Method: 第一个案例研究使用GPT 4o和GPT 4o mini从28,000份OSHA事故报告中提取结构化见解；第二个案例研究使用Molmo 7B和Qwen2 VL 2B视觉语言模型在ConstructionSite10k数据集上进行规则级安全违规检测。

Result: 尽管模型规模较小，Molmo 7B和Qwen2 VL 2B在某些提示配置下表现出竞争力，证明了低资源多模态系统用于规则感知安全监控的可行性。

Conclusion: 多模态AI框架能够有效结合文本和视觉数据，为建筑安全监控提供可行的解决方案，特别是轻量级开源模型在某些任务中具有竞争力。
...
TL;DR: 本文从认知角度提出空间智能分类法，按推理复杂度组织任务，连接多个认知功能，分析现有基准、评估方法和改进方法，揭示当前模型能力与人类推理间的差距。


<details>
  <summary>Details</summary>
Motivation: 空间推理是人类智能的基本方面，但对多模态大语言模型仍是挑战。现有调查通常基于输入模态分类，而作者认为空间能力不仅由输入格式决定，需要从认知角度进行更原则性的分析。

Method: 引入基于认知视角的分类法，按推理复杂度组织空间智能任务，将现有基准映射到该分类法中，分析评估指标和方法，并比较训练型和推理型改进方法的优缺点。

Result: 建立了空间智能的认知分类框架，揭示了当前模型能力与人类推理的关键差距，明确了不同改进方法的互补机制。

Conclusion: 从认知角度分析空间智能提供了更原则性的跨任务比较，为未来研究提供了全面理解和可操作方向，强调了训练型和推理型方法的互补性。
...
TL;DR: 该论文提出了一种基于一致性引导的跨模态迁移框架，通过将异构模态投影到共享潜在空间来缓解模态差距，提高多模态学习在噪声数据、低质量标签和不确定条件下的鲁棒性。
...
Method: 采用一致性引导的跨模态迁移方法，利用跨模态语义一致性进行鲁棒表示学习，将异构模态投影到共享潜在空间以缓解模态差距并揭示支持不确定性估计的结构关系。
...
TL;DR: 该论文提出使用RAG和MAS技术增强LLM在生物力学教育任务中的表现，通过双模块框架解决概念问答和计算问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在特定领域应用中存在知识差距，处理需要多步推理的复杂问题时性能下降，需要开发专门的教育助手来提升本科生物力学课程学习效果。

Method: 构建双模块框架：1) 应用RAG技术提升LLM回答概念判断题的特异性和逻辑一致性；2) 构建多智能体系统解决需要多步推理和代码执行的计算问题。

Result: RAG显著提升了LLM在概念问题回答中的性能和稳定性，多智能体系统能够成功进行多步推理、方程推导、代码执行并生成可解释的解决方案。
...
TL;DR: 多智能体编排相比单智能体方法在事件响应中实现了100%可操作建议率，是单智能体方法(1.7%)的80倍改进，并在所有试验中表现出零质量方差，使生产SLA承诺成为可能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)有望加速生产系统中的事件响应，但单智能体方法生成模糊、不可用的建议，需要改进以提升实际应用价值。

Method: 提出MyAntFarm.ai框架，通过348次对照试验比较单智能体副驾驶与多智能体系统在相同事件场景下的表现，引入决策质量(DQ)这一新指标。

Result: 多智能体编排实现了100%可操作建议率，相比单智能体的1.7%提升了80倍行动特异性和140倍解决方案正确性，且在所有试验中质量方差为零。

Conclusion: 多智能体编排不是性能优化，而是基于LLM的事件响应的生产就绪性要求，能够实现确定性的质量输出。
...
TL;DR: 比较基于规则的NLP方法和大型语言模型在波兰医疗记录信息提取中的表现，发现规则方法在年龄性别提取上更准确，LLM在药物识别上表现更好，建议采用混合方法。


<details>
  <summary>Details</summary>
Motivation: 解决非英语环境中从非结构化临床文本提取结构化医疗信息的挑战，特别是在资源稀缺的波兰医疗环境中。

Method: 使用基于规则的低计算NLP方法和大型语言模型，从波兰电子健康记录中提取患者人口统计、临床发现和处方药物信息，并评估文本标准化缺失和翻译导致信息损失的影响。

Result: 规则方法在信息检索任务中准确率更高（特别是年龄和性别提取），而LLM具有更好的适应性和可扩展性，在药物名称识别方面表现优异。
...
### [110] [Mini Amusement Parks (MAPs): A Testbed for Modelling Business Decisions](https://arxiv.org/abs/2511.15830)
...
TL;DR: MAPs是一个游乐园模拟器，用于评估智能体在复杂商业环境中的决策能力，包括环境建模、长期规划、空间推理等。研究发现人类表现显著优于当前最先进的LLM智能体。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在现实世界决策中面临多个相互关联的挑战，如开放目标优化、稀疏经验学习、长期随机规划等，但现有基准测试往往孤立评估这些能力，无法全面评估整体决策能力。

Method: 开发了Mini Amusement Parks（MAPs）游乐园模拟器，统一整合了环境建模、长期不确定性规划、复杂业务战略运营等挑战，并提供人类基准和LLM智能体评估。

Result: 人类在简单模式下表现优于LLM智能体6.5倍，在中等模式下优于9.8倍。分析揭示了LLM在长期优化、样本高效学习、空间推理和世界建模方面的持续弱点。

Conclusion: MAPs通过统一这些挑战，为评估具有适应性决策能力的智能体提供了新的基准基础，有助于推动更全面的AI决策能力发展。
...
### [111] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1是首个成功在音频领域解锁推理能力的模型，通过模态基础推理蒸馏框架，在音频理解基准测试中超越Gemini 2.5 Pro，达到与Gemini 3 Pro相当的性能。


<details>
  <summary>Details</summary>
Motivation: 解决音频语言模型中存在的悖论现象：推理越少性能越好，探索音频智能是否能真正受益于深思熟虑。

Method: 提出模态基础推理蒸馏框架，让模型学习生成基于声学特征的音频相关推理链，避免产生脱离实际的推理。

Result: 模型在语音、环境声音和音乐等综合音频理解和推理基准测试中表现优异，证明推理能力可以跨模态转移。
...
### [112] [Decomposing Theory of Mind: How Emotional Processing Mediates ToM Abilities in LLMs](https://arxiv.org/abs/2511.15895)
*Ivan Chulo,Ananya Joshi*

Main category: cs.AI

TL;DR: 通过对比激活引导与基线LLMs的激活模式，研究发现LLMs的心智理论能力提升主要依赖于情感理解而非分析推理。


<details>
  <summary>Details</summary>
Motivation: 探索激活引导改善语言模型心智理论能力的内部机制，明确哪些认知过程的变化导致了输出差异。

Method: 使用线性探针分析45种认知动作，对比Gemma-3-4B模型在CAA引导前后的激活模式，在1000个BigToM信念场景中评估。

Result: 信念归因任务准确率从32.5%提升至46.7%，情感处理相关激活增强（情感感知+2.23，情感评价+2.20），分析过程被抑制（质疑-0.78，聚合思维-1.59）。

Conclusion: LLMs成功的心智理论能力主要由情感理解而非分析推理所介导。
...
### [113] [Thinking, Faithful and Stable: Mitigating Hallucinations in LLMs](https://arxiv.org/abs/2511.15921)
*Chelsea Zou,Yiheng Yao,Basant Khalil*

Main category: cs.AI

TL;DR: 本文开发了一个用于大型语言模型的自校正框架，通过细粒度不确定性信号（自信度对齐和词元级熵峰值）实时检测和缓解推理过程中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在多步推理中产生幻觉的问题，传统方法仅依赖最终答案正确性，无法检测中间推理步骤的不可靠性。

Method: 设计了复合奖励函数，惩罚不合理的高自信度和熵峰值，鼓励稳定准确的推理轨迹；通过强化学习策略，利用自信度感知的奖励反馈来塑造模型生成行为。

Result: 实验表明该方法提高了最终答案准确性和推理校准度，消融实验验证了每个信号对结果的独立贡献。

Conclusion: 该自校正框架不仅改善了结果正确性，还提升了中间推理步骤的连贯性和忠实度，使模型更具自省能力。
...
### [114] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM-as-a-judge框架通常依赖将候选答案与真实标签或其他候选答案进行比较，这种方法本质上是间接的，难以完全自动化，对推理输出的细粒度和可扩展评估支持有限。

Method: 提出了JudgeBoard评估管道，直接查询模型评估答案正确性。开发了MAJ多智能体评估框架，利用多个具有不同推理配置文件的交互SLM通过协作审议来近似LLM级别的判断准确性。
...
TL;DR: 该论文研究了大型语言模型在获取证据后仍无法正确推理的问题，特别是在临床环境中。作者使用书面暴露疗法指南作为测试平台，发现即使提供权威信息，模型仍会出错。他们提出了一个评估框架来衡量准确性、一致性和推理保真度。


<details>
  <summary>Details</summary>
Motivation: 在临床环境中，大型语言模型获取正确证据后仍无法进行正确推理，这种检索与推理之间的差距令人担忧，因为输出必须符合结构化协议。
...
Conclusion: 检索增强生成有潜力约束输出，但安全部署需要将推理评估提升到与检索评估同等重要的地位，特别是在临床等高风险环境中。
...
TL;DR: SpellForger是一个游戏，玩家通过自然语言提示创建自定义法术，使用BERT模型解释提示并平衡参数，在Unity中开发，旨在验证AI作为直接游戏机制的应用。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能作为核心游戏玩法的共同创作工具，提供个性化和创造性的独特体验。

Method: 使用监督训练的BERT模型解释玩家提示，将文本描述映射到法术预制件并平衡参数，在Unity游戏引擎中开发，AI后端使用Python。

Result: 预计交付一个功能原型，实时生成法术并应用于引人入胜的游戏循环，玩家创造力成为体验的核心。

Conclusion: 验证了AI作为直接游戏机制的应用，为玩家提供了个性化和创造性的游戏体验。
...
TL;DR: 提出了一种新的约束规划模型用于串行批调度问题，该模型不依赖预定义的虚拟批次集合，而是使用关键对齐参数直接在机器上调度同族作业序列，实现了更紧凑的表述。
...
Method: 开发了不依赖虚拟批次集合的新型CP模型，使用关键对齐参数直接在机器上调度同族作业序列。通过定制搜索阶段和增强约束传播器的推理级别来改进模型。

Result: 在近5000个实例上的计算实验表明，所提模型在最多100个作业的小到中型实例上表现优越，在最多500个作业、10个族、10台机器的大规模实例上，找到的解决方案比现有方法好25%。

Conclusion: 新提出的CP模型在串行批调度问题中显著优于现有方法，特别是在处理最小批次大小约束时表现出色，为实际工业应用提供了更有效的解决方案。
...
TL;DR: 本文提出了一个AI-会计研究分类框架，分析人工智能（特别是生成式AI和大型语言模型）如何改变会计研究，识别研究机会，并探讨会计学者如何通过战略定位和合作发挥专业优势。


<details>
  <summary>Details</summary>
Motivation: 人工智能特别是生成式AI和大型语言模型的快速发展正在从根本上改变会计研究，为学者们创造了机遇和竞争威胁，需要系统分析这种转变并提出应对策略。

Method: 提出了一个二维分类框架（研究焦点：会计中心vsAI中心；方法：AI方法vs传统方法），应用于IJAIS特刊和主要会计期刊的AI-会计研究论文，分析研究过程转变和学者战略定位。

Result: 研究发现GenAI虽然使某些研究能力民主化，但通过提高对高阶贡献的期望加剧了竞争，人类判断、创造力和理论深度仍然具有价值。
...
Method: 采用优化的异步流水线调度器实现1.55倍加速，结合基于AST的搜索工具增强代码导航能力，使用纯强化学习方法训练软件工程智能体。

Result: 训练的SA-SWE-32B模型在SWE-Bench Verified上达到39.4% Pass@1，相比之前模型成本降低2倍以上，并在其他智能体任务上表现出良好的泛化能力。

Conclusion: SkyRL-Agent框架通过高效的异步调度和工具增强训练方法，显著提升了智能体训练效率和性能，并具有良好的扩展性和泛化能力。
...
TL;DR: MR-RML是一个通过GPRC实现的多维评分标准导向的奖励模型学习框架，旨在解决大语言模型在医疗领域应用的三个关键对齐挑战：静态评估与动态临床需求脱节、难以适应多源医学标准、传统奖励模型无法捕捉多维医疗质量标准。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗实践中的潜力受到关键对齐挑战的限制：静态评估基准与动态临床认知需求脱节、难以适应不断发展的多源医学标准、传统奖励模型无法捕捉细微的多维医疗质量标准。

Method: 提出MR-RML框架，包含三个核心创新：(1) 嵌入领域标准的"维度-场景-学科"医学标准系统；(2) 独立的多维奖励模型，将评估标准分解；(3) 几何投影参考约束，将医学认知逻辑转化为数学正则化。

Result: 在权威医学基准Healthbench上，相比基础模型Qwen-32B，在完整子集上提升45%，在困难子集上提升85%，在开源大语言模型中达到SOTA水平（完整子集62.7分，困难子集44.7分），并超越大多数闭源模型。

Conclusion: MR-RML框架通过结构化医学标准系统和几何投影约束，有效解决了大语言模型在医疗领域的对齐问题，显著提升了模型性能，为医疗AI的实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉推理取得了显著进展，但缺乏透明和可复现的数据整理和训练策略仍是可扩展研究的主要障碍。

Method: 采用两阶段训练方法：SFT阶段构建87.4万样本的冷启动数据集，RL阶段使用7.4万样本跨多个领域来进一步优化和稳定推理能力。

Result: 在九个多模态推理基准上比Qwen2.5-VL-7B-Instruct基线提升了11.6%，证明了数据质量和训练设计对多模态推理性能的关键作用。

Conclusion: 该方法为未来大规模多模态推理研究奠定了坚实的实证基础，并开源了所有代码、流程和数据。
...
TL;DR: 本文介绍了FOOTPASS数据集，这是首个在全场足球比赛中进行多模态、多智能体战术背景下的逐场比赛动作识别基准，旨在通过结合计算机视觉输出和足球战术知识来自动化生成可靠的逐场比赛数据流。


<details>
  <summary>Details</summary>
Motivation: 当前足球视频理解方法在构建可靠的逐场比赛数据方面仍不足，通常只能辅助而非完全自动化标注。同时战术建模、轨迹预测和性能分析等研究都需要基于比赛状态和逐场比赛数据，这促使利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持开发利用计算机视觉任务输出（如跟踪、识别）和足球战术知识（包括长期战术规律）的球员中心动作识别方法。

Result: 创建了首个全场足球比赛逐场比赛动作识别基准，能够生成可靠的逐场比赛数据流，为数据驱动的体育分析提供重要输入。

Conclusion: FOOTPASS数据集通过整合计算机视觉和战术知识，为实现更自动化和可靠的逐场比赛数据提取提供了重要工具，推动了足球视频理解和体育分析的发展。
...
TL;DR: TOFA是一种无需训练的联邦视觉语言模型一次性适应框架，通过视觉和文本双管道提取任务相关表示，解决现有方法在多模态信息利用不足、数据异构性处理不充分以及需要额外训练资源的问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦视觉语言模型适应方法存在通信成本高、易受攻击的问题，而现有一次性联邦训练方法在适应视觉语言模型时面临多模态信息利用不足、数据异构性处理不充分以及需要额外训练资源的挑战。

Method: TOFA采用视觉和文本双管道：视觉管道使用分层贝叶斯模型学习个性化的类特定原型分布；文本管道评估并全局对齐生成的本地文本提示以获得鲁棒性。引入自适应权重校准机制结合两种模态的预测，平衡个性化和鲁棒性以处理数据异构性。

Result: 在9个数据集的各种联邦设置下进行的广泛实验证明了TOFA方法的有效性。

Conclusion: TOFA是一种无需训练的一次性联邦适应框架，能够充分利用预训练视觉语言模型的可泛化多模态特征，有效处理数据异构性，且不需要客户端或服务器端的额外训练资源。
...
TL;DR: 本文提出可解释的自动化算法设计框架，通过结合LLM驱动的算法发现、可解释基准测试和问题类别描述符，实现从盲目搜索到可解释、类别特定算法设计的转变。
...
Method: 构建三支柱框架：LLM驱动的算法变体发现、可解释基准测试（归因性能到组件和超参数）、问题类别描述符（连接算法行为与景观结构）。

Result: 形成闭环知识循环，使发现、解释和泛化相互强化，产生可重用的科学洞察。

Conclusion: 将自动化与系统基准测试的理解相结合，将推动领域从盲目搜索转向可解释的、类别特定的算法设计，加速进展同时产生关于优化策略何时及为何成功的科学洞察。
...
Method: 将偏好评估分解为领域特定的智能体，每个产生部分信号，结合全局评估器（如排序器和嵌入相似度奖励），通过中央聚合器融合这些信号。

Result: CRM和rewardBench提供了更透明的奖励建模和更稳定的优化的实用模块化路径。

Conclusion: 多智能体协作方法能够在不增加人工标注的情况下实现多视角奖励塑造，提高奖励模型的鲁棒性和可解释性。
...
### [127] [ChemLabs on ChemO: A Multi-Agent System for Multimodal Reasoning on IChO 2025](https://arxiv.org/abs/2511.16205)
*Xu Qiang,Shengyuan Bai,Leqing Chen,Zijing Liu,Yu Li*

Main category: cs.AI

TL;DR: ChemO是一个基于国际化学奥林匹克竞赛的新基准，通过评估等效重构和结构化视觉增强解决化学问题自动化评估的挑战，结合ChemLabs多智能体框架实现了超越人类金牌水平的性能。


<details>
  <summary>Details</summary>
Motivation: 数学和物理的奥林匹克级基准已成为AI推理的重要测试平台，但化学因其独特的多模态符号语言一直是个开放挑战，需要专门的基准来评估AI在化学领域的推理能力。

Method: 提出了ChemO基准，包含评估等效重构（将需要视觉输出的问题转换为计算可处理格式）和结构化视觉增强（分离模型的视觉感知能力与化学推理能力），并开发了ChemLabs分层多智能体框架，模拟人类专家协作。

Result: 在先进多模态模型上的实验表明，结合结构化视觉增强和多智能体系统能带来显著性能提升，最佳配置获得了93.6/100的分数，超过了估计的人类金牌阈值。

Conclusion: ChemO基准和ChemLabs框架为自动化化学问题解决建立了新的最先进水平，证明了多智能体协作在复杂化学推理任务中的有效性。
...
### [128] [FlipVQA-Miner: Cross-Page Visual Question-Answer Mining from Textbooks](https://arxiv.org/abs/2511.16216)
*Zhen Hao Wong,Jingwen Deng,Hao Liang,Runming He,Chengyu Shen,Wentao Zhang*

Main category: cs.AI

TL;DR: 该论文提出了一种自动化流水线，通过结合布局感知OCR和基于LLM的语义解析，从教育文档中提取格式良好的问答对和视觉问答对，为LLM训练提供高质量监督数据。


<details>
  <summary>Details</summary>
Motivation: 现有指令调优和强化学习数据集成本高昂且依赖合成样本，存在幻觉和多样性有限的问题，而教科书等教育材料包含丰富的高质量人类编写问答内容但未被充分利用。

Method: 结合布局感知OCR和基于大语言模型的语义解析，构建自动化流水线从教育文档中提取问答对和视觉问答对。

Result: 实验表明该方法能产生准确、对齐且低噪声的问答对/视觉问答对，适用于多样化文档类型。

Conclusion: 该方法能够规模化利用真实世界教育内容，为改进面向推理的LLM训练提供实用的合成数据替代方案。
...
TL;DR: 该论文提出了一个多意图科学问答基准(MuISQA)来评估检索增强生成系统在异构证据覆盖方面的能力，并设计了一个意图感知检索框架，通过LLM假设潜在答案、分解意图特定查询，并使用RRF重排来平衡不同意图的证据覆盖。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统通常面向单一意图，导致复杂科学问题中多意图任务的证据覆盖不完整，无法满足多跳推理的需求。

Method: 提出意图感知检索框架：1) 使用LLM假设潜在答案；2) 将问题分解为意图特定查询；3) 为每个意图检索支持段落；4) 通过RRF聚合和重排检索结果以平衡覆盖和减少冗余。

Result: 在MuISQA基准和其他通用RAG数据集上的实验表明，该方法在检索准确性和证据覆盖方面持续优于传统方法。

Conclusion: 意图感知检索框架能有效提升多意图科学问答任务的性能，特别是在异构证据覆盖方面具有显著优势。
...
TL;DR: 本文提出了一种用于评估Android恶意软件合成数据质量的Super-Metric，该指标整合了八个指标并产生单一加权分数，实验证明其比传统指标更稳定和一致。


<details>
  <summary>Details</summary>
Motivation: 由于现有评估指标的不稳定性和缺乏标准化，评估Android恶意软件合成数据质量面临持续挑战。

Method: 在MalDataGen中集成了一个Super-Metric，该指标聚合了四个保真度维度上的八个指标，产生单一加权分数。实验涉及十个生成模型和五个平衡数据集。
...
TL;DR: 本文探讨生成式AI的生成原则如何应用于大脑认知神经科学，并分析了机器学习研究为神经信息处理系统提供的五个重要特征。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的成功基于明确的生成原则，这些原则可能在大脑中同样发挥作用，对认知神经科学有重要启示。机器学习研究提供了神经信息处理系统的新视角，神经科学可以从中学习。

Method: 通过分析生成式AI的生成原则，并讨论五个具体例子：世界建模的缺陷、思维过程生成、注意力机制、神经缩放定律和量化，来探索机器学习对神经科学的启示。

Result: 识别出机器学习研究为理解大脑信息处理提供了有价值的框架和概念，特别是在生成过程、注意力机制和系统优化方面。

Conclusion: 神经科学应该积极借鉴机器学习研究的成果，特别是生成原则和系统特征，以深化对大脑认知机制的理解。
...
<details>
  <summary>Details</summary>
Motivation: 痴呆症患者(PLwD)的沟通能力会逐渐变化，但现有计算工具无法有效跟踪这种随时间变化的行为漂移。

Method: 基于护理人员访谈创建合成用户模型，模拟60天的交互日志，注入渐进式的情感扁平化和话题偏离变化，评估多种异常检测方法。

Result: 情感扁平化在低基线变异性的用户中可用简单统计模型检测，而语义漂移检测需要时序建模和个性化基线。个性化分类器在所有任务中表现优于通用分类器。

Conclusion: 个性化行为上下文对于检测痴呆症患者沟通变化至关重要，PersonaDrift为这一新兴应用领域提供了可扩展的评估框架。
...
TL;DR: CLOZE是一个使用大型语言模型从临床笔记中自动提取医疗实体并整合到层次化医疗本体中的零样本框架，无需额外训练或标注数据，同时保护患者隐私。


<details>
  <summary>Details</summary>
Motivation: 临床笔记作为富含患者观察细节的非结构化文档，为本体扩展提供了有价值的上下文特定见解，但直接利用临床笔记进行本体扩展仍未被充分探索。
...
<details>
  <summary>Details</summary>
Motivation: 为数字人工智能系统意识可能性的辩论提供结构化和澄清工具，区分对计算功能主义的挑战与对数字意识的挑战。

Method: 开发分类框架，识别挑战的粒度级别（对应Marr的层次）和力度程度（从开放可能性到严格不可能的三个等级），并应用于14个文献案例。

Result: 成功构建了分类框架，能够清晰区分不同类型的挑战，为相关辩论提供了结构化的分析工具。

Conclusion: 该框架为数字意识可能性的辩论提供了有价值的分类和澄清工具，有助于更精确地理解和讨论相关问题。
...
TL;DR: YOFO是一种基于模板的多模态大语言模型评估方法，通过单次前向传播同时验证所有结构化需求，在保持可解释性的同时实现数量级的速度提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估方法面临基本权衡：适配MLLM输出单一分数与其生成性质不符且限制了细粒度需求理解，而自回归生成评估分析在高吞吐量场景下速度过慢。
...
Result: 在生物医学问答和临床预测基准测试中，MedBayes-Lite持续改善校准和可信度，将过度自信降低32-48%。在模拟临床环境中，通过标记不确定预测供人工审查，可预防高达41%的诊断错误。

Conclusion: MedBayes-Lite有效实现了可靠的不确定性传播，并提高了医疗AI系统的可解释性，参数开销保持在3%以下，无需重新训练。
...
TL;DR: 本文实现了一个基于人工智能的算法交易系统，专门针对EUR-USD货币对在Forex市场的高频环境。方法整合了基本面和技术面特征，通过机器学习评估预测准确性和交易盈利能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够结合基本面和技术面分析的AI交易系统，以确定哪种类型的输入特征在EUR-USD货币对交易中具有更强的预测能力。

Method: 整合欧元区和美国的关键宏观经济变量（如GDP、失业率）作为基本面特征，以及技术指标（如震荡指标、斐波那契水平、价格背离）作为技术面特征，使用机器学习方法进行预测。
...
TL;DR: 该研究构建了一个包含28个认知元素的分类法，分析了17个模型和人类在推理过程中的行为差异，发现人类使用层次嵌套和元认知监控，而模型依赖浅层前向链式推理。研究开发了测试时推理指导方法，将复杂问题性能提升高达60%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能解决复杂问题却在简单变体上失败，表明它们通过不同于人类推理的机制获得正确输出。研究旨在建立认知科学与LLM研究的桥梁，开发基于原则性认知机制而非脆弱伪推理捷径的模型。

Method: 综合认知科学研究构建28个认知元素的分类法，分析17个模型的17万条推理轨迹和54条人类出声思考轨迹，进行元分析评估1,598篇LLM推理论文。开发测试时推理指导方法自动搭建成功结构。

Result: 发现系统性的结构差异：人类使用层次嵌套和元认知监控，模型依赖浅层前向链式推理，在非结构化问题上差异最明显。元认知控制（自我意识16%，评估8%）与成功相关但被研究社区忽视。推理指导方法将复杂问题性能提升高达60%。
