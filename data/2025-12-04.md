...
TL;DR: 研究发现，针对非正面、低质量、有遮挡的人脸图像，选择性应用CFR-GAN结合CodeFormer的修复方法能有效提升人脸识别准确率，而简单应用现有修复技术反而会降低识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人脸图像常存在质量差、非正面姿态和遮挡等问题，导致生物特征人脸识别模型的准确率显著下降。本研究旨在探索通过AI驱动的头部姿态校正和图像修复技术是否能改善识别准确率。

Method: 使用模型无关的大规模法证评估流程，评估三种修复方法的影响：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer）。研究对比了这些技术的简单应用和选择性组合应用的效果。

Result: 简单应用这些修复技术会显著降低人脸识别准确率。但选择性应用CFR-GAN结合CodeFormer的方法能带来有意义的准确率提升。

Conclusion: 针对现实世界人脸识别问题，修复技术的应用需要谨慎选择。CFR-GAN与CodeFormer的选择性组合应用是有效的解决方案，而简单的修复方法反而会损害识别性能。
...
TL;DR: Flux4D是一个简单可扩展的4D动态场景重建框架，无需监督标注，通过直接预测3D高斯及其运动动态，在几秒内高效重建大规模动态场景。


<details>
  <summary>Details</summary>
Motivation: 现有可微分渲染方法（如NeRF和3DGS）在大规模动态场景重建中存在可扩展性限制，需要标注来解耦演员运动。现有自监督方法虽然尝试消除显式标注，但仍受限于单场景优化和超参数调优的敏感性。

Method: Flux4D采用完全无监督方式，仅使用光度损失和"尽可能静态"的正则化，直接预测3D高斯及其运动动态来重建传感器观测。通过跨多个场景训练，无需预训练监督模型或基础先验即可从原始数据中分解动态元素。

Result: 在室外驾驶数据集上的实验表明，Flux4D在可扩展性、泛化能力和重建质量方面显著优于现有方法，能够在几秒内高效重建动态场景，并能有效扩展到大型数据集，泛化到未见过的环境，包括罕见和未知物体。

Conclusion: Flux4D提供了一个简单可扩展的4D动态场景重建框架，通过无监督学习直接预测3D高斯运动动态，解决了现有方法在可扩展性和标注依赖方面的限制，为大规模动态场景重建提供了有效解决方案。
...
TL;DR: 利用GPT-4o和GPT-5等多模态大语言模型进行零样本物体计数，仅使用文本提示无需视觉示例，在FSC-147数据集上达到或超越现有零样本方法的性能
...
Conclusion: 多模态大语言模型仅使用文本提示就能有效执行零样本物体计数任务，展示了其在无监督计数应用中的潜力
...
TL;DR: 提出基于大语言模型的两阶段方法，从带有粗略分割的3D点云直接推断材质组成，将物体识别与材质分配解耦，实现零样本推理。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状数据集和模型大多只关注几何形状，忽略了决定物体外观的材质属性，缺乏可靠的材质标注数据。

Method: 采用两阶段LLM方法：第一阶段预测物体语义类别，第二阶段根据推断的语义为每个几何段分配合理的材质，两个阶段都采用零样本方式，无需任务特定训练。

Result: 在Fusion/ABS和ShapeNet的1000个形状上，该方法在语义和材质合理性方面表现优异，使用DeepEval实现的LLM-as-a-Judge进行评估。

Conclusion: 语言模型可以作为通用先验，在3D数据中桥接几何推理和材质理解，证明了LLM在材质推断任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 低光条件下拍摄的原始图像噪声严重，基于学习的去噪器需要大量干净-噪声图像对进行训练，但这类配对数据难以收集。噪声合成是替代大规模数据采集的方法，但现有方法要么依赖简化的参数模型，要么需要大量配对数据。

Method: 提出通用实用的噪声合成方法：1) 用泊松分布表示信号相关噪声；2) 引入傅里叶域频谱采样算法精确建模信号无关噪声，生成保持真实传感器噪声空间和统计特性的多样化噪声实现。仅需每个ISO设置下的单张噪声图像和单张暗帧。

Result: 该方法不仅准确实用，而且在多个低光去噪基准测试中取得了最先进的性能。

Conclusion: 提出了一种无需简化参数模型或大量配对数据的噪声合成方法，仅需最少数据即可生成真实传感器噪声，有效支持低光图像去噪器的训练。
...
TL;DR: PixPerfect是一个像素级优化框架，解决了潜在扩散模型在图像修复和局部编辑中产生的像素级不一致问题，如色偏、纹理不匹配和可见接缝。


<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在图像修复和局部编辑方面取得了显著进展，但固有的潜在压缩经常引入像素级不一致问题，如色偏、纹理不匹配和编辑边界处的可见接缝。现有的解决方案在实际应用中通常无法完全消除这些伪影，并且在不同潜在表示或任务之间泛化能力差。

Method: PixPerfect采用三个关键技术：(1)可微分的判别性像素空间，放大和抑制细微的颜色和纹理差异；(2)全面的伪影模拟流程，在训练期间将优化器暴露于真实的局部编辑伪影；(3)直接的像素空间优化方案，确保在不同潜在表示和任务中的广泛适用性。

Result: 在修复、对象移除和插入基准测试上的广泛实验表明，PixPerfect显著增强了感知保真度和下游编辑性能，为鲁棒和高保真的局部图像编辑建立了新标准。

Conclusion: PixPerfect是一个像素级优化框架，能够为各种LDM架构和任务提供无缝、高保真的局部编辑，解决了现有方法在消除像素级不一致问题上的局限性。
...
TL;DR: 提出PyroFocus两阶段管道，用于实时野火检测，结合深度学习分类和FRP回归/分割，在资源受限的机载/星载平台上实现速度与精度的平衡。
...
Method: 系统评估多种深度学习架构（包括定制CNN和Transformer模型）用于多类火灾分类。提出PyroFocus两阶段管道：第一阶段进行火灾分类（无火、活跃火、后火条件），第二阶段进行火灾辐射功率回归或分割，以减少推理时间和计算成本。

Result: 使用NASA的MASTER数据（类似下一代火灾检测传感器）进行实验，两阶段管道在速度与精度之间实现了良好的权衡，展示了在实时边缘部署方面的显著潜力。

Conclusion: PyroFocus两阶段管道为未来野火监测任务中的实时边缘部署提供了有前景的解决方案，在资源受限的机载平台上实现了火灾检测的速度与精度平衡。
...
TL;DR: H²U3D是一个用于房屋尺度3D场景理解的数据集，包含多楼层环境；SpatialReasoner是一个主动感知框架，通过两阶段训练实现高效的空间探索，在减少图像使用量的情况下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要局限于房间尺度的空间推理，缺乏处理大规模多楼层房屋环境的能力，需要专门的数据集和方法来支持房屋尺度的3D场景理解。

Method: 1) 创建H²U3D数据集：通过自动化标注流程构建从粗到细的层次化视觉表示，生成多样化的问答对和思维链标注；2) 提出SpatialReasoner框架：基于文本查询自主调用空间工具探索3D场景，采用两阶段训练策略：监督式冷启动+带自适应探索奖励的强化学习。

Result: SpatialReasoner在H²U3D数据集上达到最先进性能，优于GPT-4o和Gemini-2.5-Pro等强基线。关键优势：平均仅使用3-4张图像（基线需要16+张），实现了从粗到细的主动探索范式的高效性。

Conclusion: 提出的H²U3D数据集填补了房屋尺度3D场景理解的空白，SpatialReasoner框架通过主动感知和高效探索机制，在大规模3D环境的空间推理任务中展现出显著优势。
...
TL;DR: NavMapFusion：一种基于扩散模型的在线地图构建框架，利用低精度导航地图作为先验，结合高精度传感器数据生成准确的环境表示


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表示，传统高精地图需要在线构建，而广泛可用的标准导航地图分辨率不足但可作为先验指导在线地图构建
...
Conclusion: 扩散模型为地图融合提供了稳健框架，融合低精度先验与高精度传感器数据可生成准确、最新的环境表示，实现更安全可靠的自动驾驶
...
TL;DR: 提出SLEDGE框架，将设计生成建模为遵循设计师指令序列的逐步分层生成过程，而非传统单步生成


<details>
  <summary>Details</summary>
Motivation: 现有方法将设计合成视为单步生成问题，低估了创造性过程的复杂性；设计本质上是逐步细化的过程

Method: 提出"逐步分层设计生成"问题设定，利用多模态LLM开发SLEDGE框架，将设计更新建模为基于前状态的原子化分层变化

Result: 创建了新的评估套件（数据集和基准），实验分析表明该方法优于针对新设定定制的SOTA方法

Conclusion: SLEDGE能有效建模逐步设计生成过程，希望吸引更多关注这一实用但未充分探索的研究领域
...
TL;DR: ProtoEFNet是一种基于视频的原型学习模型，用于连续射血分数回归预测，通过学习动态时空原型来捕捉临床有意义的心脏运动模式，在保持准确性的同时提供临床可解释性。
...
Method: 提出ProtoEFNet，一种基于视频的原型学习模型，用于连续射血分数回归。模型学习动态时空原型来捕捉临床有意义的心脏运动模式。提出原型角度分离损失，在连续射血分数谱上强制学习判别性表示。

Result: 在EchonetDynamic数据集上的实验表明，ProtoEFNet在保持与非可解释模型相当的准确性的同时，提供临床相关洞察。消融研究显示，提出的损失函数将F1分数从77.67±2.68提升到79.64±2.10，提高了2%。

Conclusion: ProtoEFNet通过原型学习实现了连续射血分数回归，在保持预测准确性的同时提供临床可解释性，解决了黑盒模型在临床应用中的信任问题。原型角度分离损失有效提升了模型性能。
...
TL;DR: HalluGen是一个基于扩散模型的框架，用于生成可控类型、位置和严重程度的逼真幻觉，创建了首个大规模幻觉数据集，用于评估安全关键图像修复中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像修复中容易产生幻觉（看似合理但实际错误的伪影），这在医疗影像、工业检测和遥感等安全关键领域尤为危险。然而，评估幻觉存在循环依赖问题：需要标注数据但标注成本高且主观。

Method: 提出HalluGen扩散框架，能够合成具有可控类型、位置和严重程度的逼真幻觉。利用该框架构建了包含4,350张标注图像的大规模幻觉数据集，基于1,450张脑部MRI图像的低场增强数据。

Result: HalluGen生成的幻觉在感知上逼真但语义错误（分割IoU从0.86降至0.36）。开发了SHAFE指标，通过特征评估和软注意力池化提高了幻觉敏感性。训练的无参考幻觉检测器能够泛化到真实修复失败场景。

Conclusion: HalluGen及其开放数据集为安全关键图像修复中的幻觉评估建立了首个可扩展的基础，有助于开发更可靠的幻觉检测和缓解方法。
...
TL;DR: 该研究比较了16种深度学习架构在3D OCT图像中检测亚临床圆锥角膜的性能，发现分层注意力模型在稀疏异常检测方面优于2D/3D CNN和ViT，具有21-23%的敏感性和特异性提升。


<details>
  <summary>Details</summary>
Motivation: 检测体积医学成像中的微弱、空间分布异常是一项重大挑战。早期疾病信号的细微、非相邻特性常因次优的架构归纳偏置而丢失：2D/3D CNN强加局部性，而ViT扩散无约束的全局注意力。这种冲突使得稀疏体积模式识别的最佳归纳结构尚未解决。

Method: 研究对16种现代深度学习架构进行了对照比较，涵盖2D/3D卷积、混合和体积Transformer家族，用于从3D前段OCT体积中检测亚临床圆锥角膜。通过机制分析、注意力距离测量、表示相似性和辅助年龄/性别预测任务来验证结果。

Result: 分层注意力模型提供了更优越且参数效率更高的归纳偏置，在稀疏异常（亚临床）检测中比2D/3D CNN和ViT表现更好，敏感性和特异性提高21-23%。机制分析显示，分层窗口产生了与亚临床异常的中等多切片范围匹配的有效感受野。

Conclusion: 分层注意力是3D医学成像中早期病理变化分析的原则性和有效方法。研究结果为未来体积异常检测系统提供了设计指导，表明所需的空间整合长度根据信号强度显著变化，亚临床病例需要比健康和明显疾病状态更长的整合。
...
<details>
  <summary>Details</summary>
Motivation: 大多数视觉理解、预测和生成直接在2D观测上进行，导致性能不理想。图像和视频是4D世界（3D空间+时间）的离散2D投影，需要更好的方法来学习连续4D动态并生成未见过的视觉内容。
...
Conclusion: SeeU的2D→4D→2D框架通过建模4D动态，能够生成连续且物理一致的视觉内容，为视觉理解和生成任务提供了更优的解决方案。
...
TL;DR: FireSentry是一个省级规模的多模态野火数据集，具有亚米级空间和亚秒级时间分辨率，用于细粒度野火蔓延预测。基于此数据集提出了FiReDiff双模态范式，通过先预测红外视频序列再分割火掩码的方法，在视频质量和掩码准确性方面均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注粗时空尺度，依赖低分辨率卫星数据，只能捕捉宏观火灾状态，限制了高精度局部火灾动态建模能力。需要更高分辨率的细粒度野火预测数据集和方法。

Method: 1) 创建FireSentry数据集：使用同步无人机平台收集可见光和红外视频流、现场环境测量数据及人工验证的火掩码；2) 建立包含物理模型、数据驱动模型和生成模型的综合基准；3) 提出FiReDiff双模态范式：先预测红外模态的未来视频序列，再基于生成的动态在掩码模态中精确分割火掩码。

Result: FiReDiff在生成模型中取得最先进性能：视频质量提升39.2% PSNR、36.1% SSIM、50.0% LPIPS、29.4% FVD；掩码准确性提升3.3% AUPRC、59.1% F1分数、42.9% IoU、62.5% MSE。
...
TL;DR: ShelfGaussian是一个开放词汇多模态高斯基3D场景理解框架，利用现成的视觉基础模型进行监督，在感知和规划任务中表现出色。
...
Method: 提出多模态高斯变换器，使高斯能够从多种传感器模态查询特征；提出货架监督学习范式，在2D图像和3D场景级别联合优化高斯特征。

Result: 在Occ3D-nuScenes上实现了最先进的零样本语义占用预测性能；在无人地面车辆上评估了其在各种城市场景中的野外性能。

Conclusion: ShelfGaussian框架成功结合了多模态特征查询和货架监督学习，充分利用了高斯方法的潜力，在3D场景理解任务中表现出色。
...
TL;DR: MOS框架通过模态一致表示学习和跨模态数据生成与特征融合，有效缓解光学与SAR图像间的模态差异，在跨模态船舶重识别任务中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 光学与合成孔径雷达(SAR)图像之间的跨模态船舶重识别是海事情报与监视的关键任务，但两者间的显著模态差异给鲁棒识别带来了主要挑战。

Method: MOS框架包含两个核心组件：1) 模态一致表示学习(MCRL)：应用去噪SAR图像处理和类级模态对齐损失来对齐跨模态的同一身份特征分布；2) 跨模态数据生成与特征融合(CDGF)：利用布朗桥扩散模型合成跨模态样本，在推理时与原始特征融合以增强对齐性和判别性。

Result: 在HOSS ReID数据集上的大量实验表明，MOS在所有评估协议中都显著超越了最先进方法，在ALL to ALL、Optical to SAR和SAR to Optical设置下，R1准确率分别提高了+3.0%、+6.2%和+16.4%。

Conclusion: MOS框架通过缓解光学-SAR模态差异和实现模态一致特征学习，在跨模态船舶重识别任务中取得了卓越性能，代码和训练模型将在发表后发布。
...
TL;DR: 本文提出了视频差异描述任务ViDiC及其对应的ViDiC-1K数据集，用于评估多模态大语言模型在描述视频对之间相似性和差异性的能力，填补了现有图像差异描述方法无法捕捉动态场景变化的空白。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统在理解动态场景之间的视觉差异方面存在不足，特别是对组合性、空间性和时间性变化的比较感知能力尚未得到充分探索。虽然图像差异描述任务已能描述静态图像间的语义变化，但这些方法无法捕捉运动连续性、事件演化或时间上的编辑一致性。

Method: 1. 提出ViDiC（视频差异描述）任务；2. 构建ViDiC-1K数据集，包含1000个精心策划的视频对，标注了超过4000个比较检查项，涵盖七个类别：主体、风格、背景、摄影技术、运动、位置和播放技术；3. 提出基于LLM-as-a-Judge协议的双检查框架，分别测量相似性和差异性的准确性。

Result: 在19个代表性多模态模型上的实验显示，这些模型在比较描述和差异感知能力方面存在显著性能差距。ViDiC-1K数据集为评估模型在视频理解、编辑感知和比较推理方面的能力提供了具有挑战性的基准。

Conclusion: ViDiC任务和ViDiC-1K数据集为推进多模态智能在视频理解、编辑感知和比较推理方面的发展奠定了坚实基础，有望成为评估和改进多模态大语言模型在动态场景比较分析能力的重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有可供性学习方法大多只关注"如何使用"物体，忽略了"是什么"和"在哪里"；而其他方法将物体检测和可供性学习作为独立任务处理，缺乏有效交互和实时能力。
...
Conclusion: YOLOA在准确性和效率之间取得了优秀的平衡，通过LLM适配器联合处理物体检测和可供性学习任务，实现了实时可供性检测。
...
TL;DR: 提出了一种利用预训练扩散模型进行标签高效高光谱图像分类的框架，通过提取扩散模型早期去噪步骤的空间特征，并结合轻量级FiLM融合模块整合光谱信息，在稀疏标注下实现优越性能。
...
TL;DR: LM-CartSeg是一个全自动的膝关节MRI分割和影像组学分析流程，通过几何规则改进分割质量，实现稳定的内外侧分区，并提供质量控制，为多中心骨关节炎研究提供基础。


<details>
  <summary>Details</summary>
Motivation: 现有膝关节MRI影像组学研究大多依赖手动ROI划分且缺乏质量控制，需要自动、稳健且具有解剖学意义的ROI来同时捕捉软骨和软骨下骨信息。

Method: 使用两个3D nnU-Net模型在SKM-TEA和OAIZIB-CM数据集上训练，通过零样本预测融合和几何规则后处理（连通分量清理、构建10mm软骨下骨带、基于PCA和k-means的胫骨内外侧分割）。从10个ROI中提取4650个非形状影像组学特征。

Result: 后处理显著改善分割质量：ASSD从2.63mm降至0.36mm，HD95从25.2mm降至3.35mm，DSC达0.91。几何内外侧分割规则在不同数据集中保持稳定，而直接训练的nnU-Net模型存在域依赖的侧向交换问题。仅6-12%的特征与体积或厚度强相关。

Conclusion: LM-CartSeg提供自动化的质量控制ROI和影像组学特征，这些特征携带了超出简单形态测量的判别信息，为多中心膝关节骨关节炎影像组学研究提供了实用基础。
...
TL;DR: 提出无监督学习框架，从点云数据中学习具有空间结构的3D关键点，作为紧凑可解释表示，用于条件扩散模型重建完整形状。
...
Method: 开发无监督框架从点云学习空间结构化的3D关键点，作为紧凑可解释表示，用于条件化Elucidated Diffusion Model重建完整形状。

Result: 学习到的关键点在不同物体实例间具有可重复的空间结构，支持关键点空间平滑插值，表明其能捕捉几何变化；在多样化物体类别上表现优异，关键点一致性比先前方法提高6个百分点。

Conclusion: 该方法成功填补了无监督关键点学习与无条件3D生成之间的空白，提供了紧凑、可解释且具有空间结构的表示，支持有效的形状重建和几何变化捕捉。
...
Method: 提出ThinkDeeper框架，核心是空间感知世界模型(SA-WM)，将当前场景提炼为指令感知的潜在状态，并展开一系列未来潜在状态，提供前瞻性线索。配合超图引导的解码器，分层融合这些状态与多模态输入，捕捉高阶空间依赖关系。同时创建DrivePilot数据集，使用RAG和CoT提示的LLM管道生成语义标注。

Result: 在六个基准测试中，ThinkDeeper在Talk2Car排行榜排名第一，在DrivePilot、MoCAD和RefCOCO/+/g基准上超越最先进基线。在挑战性场景（长文本、多智能体、模糊性）中表现出强大的鲁棒性和效率，即使使用50%数据训练也能保持优越性能。

Conclusion: 通过引入空间感知世界模型和前瞻性推理，ThinkDeeper显著提升了自动驾驶中自然语言指令的视觉定位能力，为解决模糊指令问题提供了有效解决方案，在多个基准测试中验证了其优越性。
...
TL;DR: TPI（文本打印图像）方法通过将文本直接渲染到空白画布上生成合成图像，实现低成本文本中心训练，解决了视觉语言模型训练中图像数据稀缺和隐私限制的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型（LVLMs）需要大量图像-文本对进行任务特定微调，但图像数据收集成本高且受隐私限制。文本数据广泛可用且易于编辑，但纯文本训练因模态差距在VQA任务上效果有限。

Method: 提出Text-Printed Image（TPI）方法，通过将文本描述直接渲染到纯白画布上生成合成图像。这种方法将文本投影到图像模态，可低成本集成到现有LVLM训练流程中，且能保留文本语义。

Result: 在4个模型和7个基准测试上的系统实验表明，TPI比扩散模型生成的合成图像在文本中心训练中更有效。TPI作为低成本数据增强策略也展示了实际效用。
...
Method: 在潜在扩散模型中增加每帧深度预测，提出多视角几何损失函数，在共享3D坐标系中对齐跨帧的预测深度图，强制结构时间一致性。
...
TL;DR: 提出NAS-LoRA方法，通过轻量级神经架构搜索块增强SAM在特定领域的适应能力，减少训练成本24.14%且不增加推理开销


<details>
  <summary>Details</summary>
Motivation: SAM作为视觉基础模型在特定领域（如医疗、农业图像）的适应仍具挑战性。现有LoRA方法缺乏空间先验知识，Transformer编码器在图像块中缺乏空间先验，可能阻碍高级语义信息的获取。

Method: 提出NAS-LoRA参数高效微调方法：1）在LoRA的编码器和解码器之间加入轻量级神经架构搜索块，动态优化先验知识集成；2）采用分阶段优化策略，帮助ViT编码器平衡权重更新和架构调整，逐步学习高级语义信息。

Result: 实验表明NAS-LoRA改进了现有PEFT方法，训练成本降低24.14%，且不增加推理成本，证明了NAS在增强视觉基础模型PEFT方面的潜力。

Conclusion: NAS-LoRA通过引入神经架构搜索块有效桥接了预训练SAM与专业领域之间的语义鸿沟，为视觉基础模型的高效适应提供了新思路。
...
TL;DR: PRIS提出了一种在推理时自适应修改提示词的框架，通过分析生成视觉内容的失败模式来重新设计提示词，而不是固定提示词并单纯增加生成次数，从而更好地实现文本到视觉的精准对齐。


<details>
  <summary>Details</summary>
Motivation: 文本到视觉生成中，用户意图与生成结果之间的精准对齐是一个核心挑战。现有方法主要通过增加采样步骤或种子数量来扩展生成过程，但这种方法很快会遇到质量瓶颈，因为关键的提示词在整个过程中保持不变。

Method: 提出PRIS框架，在推理时自适应地修改提示词：1）分析生成的视觉内容；2）识别跨视觉内容的重复失败模式；3）根据分析结果重新设计提示词；4）使用修订后的提示词重新生成视觉内容。引入元素级事实校正验证器，在细粒度层面评估提示词属性与生成视觉内容之间的对齐度。

Result: 在文本到图像和文本到视频基准测试中进行了广泛实验，证明了该方法的有效性，包括在VBench 2.0上获得15%的性能提升。结果表明，联合扩展提示词和视觉内容是充分利推理时扩展定律的关键。

Conclusion: PRIS框架通过自适应修改提示词而不是固定提示词，有效解决了文本到视觉生成中的对齐问题。该方法表明，在推理时联合扩展提示词和视觉内容比单纯增加生成次数更有效，为提升生成质量提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 烹饪是顺序性和视觉基础的活动，每个步骤都包含程序逻辑和视觉语义。现有的扩散模型在处理结构化多步骤场景（如食谱插图）时存在困难，且当前食谱插图方法无法适应食谱长度的自然变化，无论实际指令结构如何都生成固定数量的图像。

Method: 提出了三个关键组件：1) 步骤区域控制(SRC)，在单个去噪过程中将文本步骤与相应图像区域对齐；2) 灵活RoPE，增强时间连贯性和空间多样性的步骤感知位置编码机制；3) 跨步骤一致性控制(CSCC)，保持步骤间细粒度食材一致性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在幻觉问题，生成与输入视觉内容不一致的内容，这在精度敏感领域中影响可靠性。现有方法通常通过干预注意力分数或输出logits来缓解幻觉，但只关注"如何干预"而忽略了"何时干预"，导致"过度干预"问题，引入新的幻觉和不必要的计算开销。

Method: 提出V-ITI框架：1）视觉忽视检测器：通过头级判别探针检测视觉忽视；2）视觉回忆干预器：仅在检测到视觉忽视时，用预存储的视觉激活信息调制激活。该框架首先研究了视觉忽视机制，发现可以通过MLLMs中的头级激活模式准确检测。

Result: 在八个基准测试和不同MLLM家族上的广泛实验表明，V-ITI能够持续缓解与视觉相关的幻觉，同时保持一般任务性能。

Conclusion: V-ITI通过智能检测视觉忽视并仅在必要时进行干预，有效解决了现有方法中的"过度干预"问题，在缓解幻觉的同时保持了模型的计算效率和一般能力。
...
TL;DR: 论文提出Action Effect Modeling (AEM)框架，通过联合建模动作执行及其效果来改进程序性任务中的错误检测，在EgoPER和CaptainCook4D基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要分析动作执行方式，忽略了动作产生的效果（如物体状态或空间排列），而许多错误恰恰体现在执行结果而非执行过程本身。需要同时考虑执行和结果的统一框架。

Method: 提出AEM框架：1）基于语义相关性和视觉质量选择最具信息量的效果帧；2）从视觉定位和符号场景图中提取互补线索；3）在共享潜在空间中对齐形成效果感知表示；4）设计基于提示的检测器，结合任务特定提示并将动作片段与预期执行语义对齐。

Result: 在EgoPER和CaptainCook4D基准的one-class classification (OCC)设置下达到最先进性能，证明了同时建模执行和结果能实现更可靠的错误检测。

Conclusion: 建模动作执行和结果能提高错误检测的可靠性，效果感知表示有潜力应用于更广泛的下游任务。
...
TL;DR: 提出了一种用于大规模用户生成视频平台的混合内容审核框架，结合监督分类和基于参考的相似性匹配，以处理直播环境中已知违规和新型边缘案例


<details>
  <summary>Details</summary>
Motivation: 大规模用户生成视频平台的内容审核面临挑战，特别是在直播环境中需要及时、多模态且能适应不断演化的不良内容形式

Method: 混合审核框架：监督分类处理已知违规，基于参考的相似性匹配处理新型或微妙案例；多模态输入（文本、音频、视觉）通过两个流程处理，使用多模态大语言模型将知识蒸馏到每个流程中以提高准确性同时保持推理轻量

Result: 生产环境中，分类流程在80%精确度下达到67%召回率，相似性流程在80%精确度下达到76%召回率；大规模A/B测试显示用户观看不良直播的观看量减少6-8%
...
TL;DR: 提出了一种公平感知的低秩适配方法用于医学视觉语言模型，通过可微的MaxAccGap损失函数优化不同人口群体间的准确率公平性，在仅需0.24%可训练参数的情况下显著减少诊断准确性差异。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在医疗影像任务上能达到专家级性能，但在不同人口群体间存在显著的诊断准确性差异，这引发了医疗AI的公平性问题。需要一种既能保持参数效率又能明确优化公平性的方法。

Method: 提出了公平感知的低秩适配方法，核心是引入可微的MaxAccGap损失函数，实现跨人口群体准确率公平性的端到端优化。提出了三种具体方法：FR-LoRA将MaxAccGap正则化整合到训练目标中；GR-LoRA应用逆频率加权平衡梯度贡献；Hybrid-LoRA结合两种机制。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确性差异减少了69%，同时保持53.15%的整体准确率。消融研究表明，强正则化强度能以最小的准确率代价实现最优公平性，种族特异性优化能实现60%的差异减少。方法仅需0.24%的可训练参数。

Conclusion: 该方法在资源受限的医疗环境中实现了医学AI的公平部署，通过参数高效的公平优化显著减少了人口群体间的诊断准确性差异，为医疗AI的公平性提供了实用解决方案。
...
TL;DR: CartoMapQA是一个专门评估视觉语言模型理解地图能力的基准数据集，包含2000多个样本，涵盖从低到高的地图解读技能，揭示了现有模型在地图语义理解、地理空间推理和OCR识别方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉-文本融合方面取得了进展，但其在地图解读方面的能力尚未得到充分探索。地图理解对于导航、地理搜索和城市规划等实际应用至关重要，因此需要专门的评估工具来推动模型改进。

Method: 研究者创建了CartoMapQA基准数据集，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和标准答案组成。任务涵盖低、中、高三个层次的地图解读技能：符号识别、嵌入信息提取、比例尺解释和基于路径的推理。

Result: 对开源和专有视觉语言模型的评估显示，模型在地图特定语义理解方面存在持续挑战，地理空间推理能力有限，且容易产生OCR相关错误。这些弱点表明当前模型在地图理解任务上仍有显著不足。

Conclusion: CartoMapQA通过揭示视觉语言模型在地图理解方面的具体弱点，为未来模型架构改进提供了有价值的指导工具，有助于开发更适合实际应用（如导航、地理搜索和城市规划）的模型。数据集和源代码已开源供研究社区使用。
...
### [34] [EEA: Exploration-Exploitation Agent for Long Video Understanding](https://arxiv.org/abs/2512.03500)
*Te Yang,Xiangyu Zhu,Bo Wang,Quan Chen,Peng Jiang,Zhen Lei*

Main category: cs.CV

TL;DR: EEA框架通过语义引导的层次树搜索实现探索-利用平衡，用于长视频理解，在保持计算效率的同时提升性能。
...
Method: EEA框架通过语义查询发现与更新、语义锚点收集、层次树搜索（优先探索语义相关帧）、以及结合VLM内在奖励与语义先验的自适应评估机制。

Result: 在多个长视频基准测试中验证了EEA方法的优越性能和计算效率。

Conclusion: EEA通过语义引导的探索-利用平衡机制，有效解决了长视频理解中的计算效率和信息覆盖问题。
...
TL;DR: 研究发现视觉压缩在文本重建方面并不优于简单替代方法，且在语言建模任务中表现不佳，对光学上下文压缩的过度兴奋缺乏证据支持。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉标记中高保真重建渲染文本的能力，引发了视觉上下文压缩的热潮。但现有评估仅限于重建任务，未验证这些表示是否真正有助于语言建模。

Method: 通过比较DeepSeek-OCR的视觉编码器与两种简单替代方案：无参数的平均池化和学习的分层编码器，在相同压缩比下评估文本重建和语言建模性能。

Result: 简单方法在文本重建任务中匹配或超越视觉压缩方法，在语言建模任务中表现更优，而视觉压缩甚至无法超越简单的截断方法。

Conclusion: 视觉上下文压缩的现有证据不足以支持其优势，对光学压缩的兴奋超过了实际证据。视觉压缩在文本重建方面无独特优势，对语言建模帮助有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要局限于对象中心和描述性指令，忽视了任务导向推理对具身智能体完成目标导向交互的重要性，需要填补这一空白。

Method: 基于ScanNet视频构建包含100个标注片段和2,704条任务导向定位指令的基准，采用结合基础模型标注和人工细化的半自动化流程，并设计了针对多对象和显隐式对象定位的任务级评估指标。

Result: 对7个最先进的多模态大语言模型进行基准测试，揭示了任务导向时空视频定位的内在挑战，以及在显隐式和多对象定位方面的显著性能差距。

Conclusion: 任务导向时空视频定位在连接感知与交互方面存在显著困难，ToG-Bench为评估具身智能体在真实场景中的任务导向推理能力提供了重要基准。
...
TL;DR: DPMFormer是一个用于语义分割领域泛化的新框架，通过域感知提示学习和对比学习解决视觉与文本语义对齐问题，在多个基准测试中达到最先进水平。
...
Method: 提出了域感知提示驱动的掩码变换器（DPMFormer）框架：1）域感知提示学习促进视觉与文本线索的语义对齐；2）域感知对比学习结合纹理扰动多样化可观察域；3）域鲁棒一致性学习指导模型最小化原始图像与增强图像预测的差异。

Result: 实验和分析表明该框架的优越性，在各种领域泛化语义分割基准测试中建立了新的最先进水平。

Conclusion: DPMFormer通过域感知提示学习、对比学习和一致性学习有效解决了视觉-文本语义对齐问题，为语义分割领域泛化提供了强大的解决方案。
...
TL;DR: CodeVision是一个基于代码作为通用工具接口的多模态大语言模型框架，通过两阶段训练方法显著提升了模型对图像方向变化和自然损坏的鲁棒性，并增强了多工具组合推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在工具使用方面存在局限性：依赖有限的工具集，缺乏实际必要性和可扩展性。研究发现即使最先进的MLLM也存在严重脆弱性，在简单的图像方向变化或自然损坏下性能显著下降，这凸显了对更鲁棒工具推理的需求。

Method: 提出CodeVision框架，将代码作为通用接口来调用任何图像操作，超越固定工具注册表。采用两阶段训练方法：首先在高质量数据集上进行监督微调，该数据集针对复杂多轮工具组合和错误恢复而构建；然后使用强化学习，采用新颖的密集过程奖励函数来鼓励战略性和高效的工具使用。

Result: 在Qwen2.5-VL和Qwen3-VL系列上的实验表明，该方法显著提升了模型性能，并培养了新兴能力，包括灵活的工具组合、高效的链式执行以及从运行时反馈中进行鲁棒的错误恢复。

Conclusion: CodeVision提供了一个灵活且可扩展的代码即工具框架，通过两阶段训练方法有效解决了MLLM在图像方向变化和自然损坏下的脆弱性问题，同时增强了多工具推理能力，为更鲁棒的多模态推理系统提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统舞蹈分析通常需要专业设备或标记，本研究旨在探索利用现代计算机视觉技术实现无标记、自动化的舞蹈动作分析，为量化舞蹈评估提供技术基础。

Method: 提出概念验证框架：使用YOLOv8/v11进行舞者检测，结合Segment Anything Model (SAM)进行像素级分割，实现舞者跟踪、舞步计数、空间覆盖模式分析和节奏一致性测量。

Result: 在49秒加纳AfroBeats舞蹈视频测试中，检测精度约94%，召回率89%；SAM分割IoU约83%。主舞者比次舞者多执行23%舞步，运动强度高37%，使用空间多42%。

Conclusion: 该框架展示了技术可行性，为量化舞蹈指标提供了有前景的方向，但存在单视频验证、缺乏系统标注和姿态估计方法比较等局限，需要未来系统验证研究。
...
TL;DR: AdaptVision：一种通过粗到细自适应视觉标记获取的高效视觉语言模型范式，使用强化学习框架平衡准确性和效率，显著减少视觉标记消耗。


<details>
  <summary>Details</summary>
Motivation: 现有高效VLM方法通过固定比例压缩减少视觉标记，但缺乏根据任务需求自适应调整的能力，无法自主确定每个样本所需的最小视觉标记数量。
...
Result: 在多个VQA基准测试中，AdaptVision在消耗显著少于现有高效VLM方法的视觉标记的同时，实现了更优的性能表现。

Conclusion: AdaptVision通过自适应视觉标记获取机制，成功解决了VLM计算效率问题，在保持准确性的同时大幅减少了计算开销，为高效视觉语言理解提供了新范式。
...
TL;DR: BlackCAtt是一种黑盒对抗攻击方法，通过识别因果像素集对目标检测器进行可解释、不可感知的攻击，能导致边界框丢失、修改或新增，且跨架构通用。


<details>
  <summary>Details</summary>
Motivation: 现有对抗扰动方法多为白盒且架构特定，缺乏对攻击机制的可解释性。需要一种能理解攻击原理的方法，帮助开发者分析和防御攻击。

Method: 提出BlackCAtt黑盒算法，结合因果像素和边界框生成对抗攻击。使用最小因果充分像素集，通过扰动这些关键像素来影响目标检测器的输出。

Result: 在COCO测试集上，BlackCAtt比基线方法在移除检测上效果好2.7倍，修改检测好3.86倍，触发虚假检测好5.75倍，攻击效果接近原图且不可感知。

Conclusion: 因果像素识别能产生更精确、更不可感知的黑盒对抗攻击，为理解攻击机制和防御提供了新途径，展示了因果像素在对抗攻击中的强大能力。
...
TL;DR: 本文提出Stable Signer模型，将手语生成任务简化为端到端的分层生成，包括文本理解和姿势到视频生成，通过SLUL链接器和SLP-MoE手势渲染专家块实现高质量多风格手语视频生成。


<details>
  <summary>Details</summary>
Motivation: 传统手语生成方法存在文本转换不准确、姿势生成错误、姿势渲染到真实视频的误差累积问题，导致该领域进展缓慢。

Method: 1. 简化传统冗余结构，将SLP任务重新定义为仅包含文本理解（Prompt2Gloss, Text2Gloss）和Pose2Vid的端到端分层生成任务；2. 提出SLUL（手语理解链接器）执行文本理解，使用新开发的语义感知词元掩码损失（SAGM Loss）训练；3. 设计SLP-MoE手势渲染专家块生成手势。

Result: 性能比当前SOTA生成方法提升了48.6%。

Conclusion: Stable Signer通过简化任务目标、优化模型结构，实现了高质量、多风格的手语视频端到端生成，显著提升了手语生成的性能。
...
TL;DR: FloodDiffusion是一个用于文本驱动的流式人体运动生成的新框架，通过改进的扩散强迫方法在时间变化文本提示下生成文本对齐、无缝的运动序列，在HumanML3D基准测试中达到0.057的FID分数。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖分块处理或带有扩散头的自回归模型，难以在时间变化文本提示下生成高质量、无缝的运动序列。需要一种能够处理流式运动生成任务的新框架。

Method: 采用扩散强迫框架来建模时间序列生成任务，但发现传统扩散强迫方法无法建模真实运动分布。为此进行了三项关键改进：1) 使用双向注意力而非因果注意力；2) 实现下三角时间调度器而非随机调度器；3) 采用连续时间变化方式引入文本条件。

Result: 在流式运动生成任务上首次实现了基于扩散强迫框架的最先进性能，在HumanML3D基准测试中达到0.057的FID分数，能够以实时延迟生成文本对齐、无缝的运动序列。

Conclusion: 通过改进的扩散强迫框架，FloodDiffusion成功解决了文本驱动的流式人体运动生成问题，在保持实时性能的同时达到了最先进的生成质量，为时间序列生成任务提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态高质量图像，忽略了时序退化和错误传播这两个关键失效模式——瞬态视觉损坏引发的幻觉会在后续帧中持续存在。这对于自动驾驶等安全关键应用中的VLM部署至关重要。

Method: 1. 提出DIQ-H基准：应用物理模拟的视觉退化（运动模糊、传感器噪声、压缩伪影）；2. 通过多轮问答任务测量幻觉持续性、错误恢复和时序一致性；3. 提出不确定性引导迭代优化（UIR）方法，使用轻量级VLM进行不确定性过滤，生成可靠的伪地面真值。

Result: 1. UIR方法实现了15.3%的准确率提升；2. 对16个最先进VLM的实验显示显著鲁棒性差距：GPT-4o仅达到78.5%的恢复率，开源模型在时序一致性方面低于60%；3. 揭示了现有模型在动态视觉退化下的严重脆弱性。

Conclusion: DIQ-H为评估VLM在真实世界部署中的可靠性提供了全面平台，揭示了现有模型在动态视觉退化下的严重脆弱性，强调了时序鲁棒性在安全关键应用中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有关节物体生成模型通常缺乏基于文本提示的生成能力，文本描述与3D关节物体表示之间存在显著差距。
...
TL;DR: 本文提出了一种新的测试时缩放方法，通过文本嵌入扰动增强文本到图像扩散模型的生成多样性和质量，该方法与空间噪声互补，在频域分析基础上实现高效改进。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法主要关注搜索策略和奖励模型，但忽略了文本到图像扩散模型中噪声随机性对方法性能的影响，特别是不同形式随机性的互补作用尚未被探索。

Method: 提出文本嵌入扰动方法，包含两个关键设计：1) 引入基于步骤的文本嵌入扰动，结合频率引导的噪声调度与空间噪声扰动；2) 根据频域贡献和扰动容忍度自适应调整扰动强度。

Result: 该方法可无缝集成到现有测试时缩放方法中，在多个基准测试上显著提升性能，且几乎不增加额外计算成本。

Conclusion: 文本嵌入扰动与空间噪声在频域上具有互补性，通过合理设计扰动策略可以有效提升文本到图像扩散模型的生成多样性和质量，为测试时缩放提供了新思路。
...
TL;DR: GLASTE是一个端到端的场景文本编辑框架，通过全局-局部感知设计解决现有方法中的不一致性和长度不敏感问题，在保持文本样式和背景纹理的同时实现更一致的文本编辑。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本编辑方法存在两个主要挑战：1）编辑后的局部补丁与周围区域不一致；2）对编辑前后文本长度差异不敏感。这些问题导致编辑结果不自然，难以处理长度变化较大的文本编辑任务。

Method: 提出GLASTE框架，采用全局-局部组合结构，结合全局和局部损失函数，增强文本图像特征。将文本样式表示为与图像大小无关的向量，可转移到不同大小的目标文本图像。使用仿射融合将目标文本图像填充到编辑补丁中，同时保持其宽高比不变。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在长视频理解中面临上下文长度有限和处理密集视频标记计算成本高的问题。现有查询感知帧选择方法计算开销大，本文质疑这种复杂搜索机制是否普遍必要。

Method: 首先识别并验证查询类型学，区分全局查询和局部查询。提出DIG框架：全局查询使用高效的均匀采样，局部查询激活专门管道提取查询相关帧。框架无需训练，自适应选择策略。

Result: 在三个长视频理解基准测试中，DIG始终优于现有基线，即使输入帧数扩展到256帧也能稳健提升LMM性能。验证了均匀采样对全局查询的有效性，局部查询确实需要查询感知选择。

Conclusion: 复杂搜索机制并非普遍必要，应根据查询类型自适应选择帧选择策略。DIG框架通过简单而有效的设计，在保持效率的同时显著提升长视频理解性能。
...
<details>
  <summary>Details</summary>
Motivation: 区别于基于注意力的压缩方法，本文从信息论角度出发，旨在在有限计算预算下最大化视频表示的信息保真度，解决视觉压缩中的冗余问题。

Method: 引入信息独特性概念来衡量token间的内在冗余，并设计三个模块：帧组融合、token分配和空间动态压缩，逐步进行语义帧分组、自适应资源分配和细粒度空间压缩。
...
TL;DR: 研究发现：虽然可以将草图视为序列并使用传统位置编码，但绝对坐标优于相对坐标；非自回归解码器优于自回归解码器；时序重要性取决于具体顺序和任务。
...
Method: 研究通过实验比较不同方法：1）比较传统位置编码在草图序列建模中的有效性；2）对比绝对坐标与相对坐标的性能；3）比较非自回归解码器与自回归解码器的表现；4）分析不同顺序和任务下时序重要性的变化。

Result: 结果显示：1）使用传统位置编码对草图序列建模是有效的；2）绝对坐标在性能上持续优于相对坐标；3）非自回归解码器优于自回归解码器；4）时序的重要性取决于所考虑的顺序和评估的任务。

Conclusion: 草图可以合理地被建模为序列，但绝对坐标比相对坐标更有效，非自回归方法优于自回归方法。时序的重要性不是绝对的，而是取决于具体应用场景和顺序选择。
...
TL;DR: 提出CSCL框架，通过两阶段预训练提升WSI表征质量：第一阶段使用轻量适配器进行对比对齐，第二阶段通过MIL和跨染色注意力融合模块整合多染色信息，在多种下游任务中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中需要通用、可迁移的全切片图像表征。虽然结合多种染色标记（如IHC）可以丰富H&E特征，但进展受限于对齐良好的多染色数据集稀缺，染色间错位会破坏特征一致性。

Method: 1. 构建五染色对齐数据集（H&E、HER2、KI67、ER、PGR）；2. 提出CSCL两阶段预训练框架：第一阶段使用轻量适配器进行补丁级对比对齐，第二阶段通过MIL、跨染色注意力融合模块和全局对齐模块学习跨染色表征。

Result: 在癌症亚型分类、IHC生物标志物状态分类和生存预测等任务上均取得一致提升，生成了高质量、可迁移的H&E切片级表征。

Conclusion: CSCL框架通过跨染色对比学习和表征融合，有效解决了多染色数据对齐问题，提升了WSI表征的质量和可迁移性，为计算病理学提供了有力工具。
...
TL;DR: 提出Pyramid Sparse Attention (PSA)模块，通过多级池化KV表示替代二进制掩码，在保持计算效率的同时减少高稀疏度下的信息损失，适用于视频理解和生成任务。


<details>
  <summary>Details</summary>
Motivation: 注意力机制是基础模型的核心，但其二次复杂度限制了模型扩展。现有高效注意力方法主要采用稀疏化范式，但通常使用二进制掩码保留或丢弃整个KV块，在高稀疏度下会导致显著信息损失。

Method: 提出金字塔稀疏注意力(PSA)：1）使用多级池化KV表示替代二进制掩码；2）每个查询块动态分配较低池化级别给关键KV块，较高级别给次要KV块；3）采用解耦的块-瓦片设计和硬件友好内核确保高效执行。

Result: 在视频理解和生成基准测试中，PSA在保持上下文信息和视觉保真度方面表现优异，相比现有稀疏注意力基线，在效率-质量权衡上具有优势，性能相当或更好。

Conclusion: PSA通过多级池化KV表示有效缓解了高稀疏度下的信息损失问题，在低计算预算下保持了计算效率，为视频理解和生成任务提供了高效的注意力机制解决方案。
...
TL;DR: DOT-BI是一种基于人类运动感知的快速机器人识别方法，通过在动态背景中隐藏数字，利用人眼对运动的敏感性来区分人类和自动化系统。


<details>
  <summary>Details</summary>
Motivation: 在线调查和流程中需要有效区分人类用户和自动化机器人系统，传统验证方法如CAPTCHA存在可用性问题，需要更自然、快速的解决方案。

Method: 使用随机黑白像素纹理作为背景，将数字以相同纹理但不同运动速度和缩放比例显示，使人眼通过运动感知能够识别数字，而逐帧算法处理无法提取有效信号。

Result: 最先进的多模态模型（GPT-5-Thinking和Gemini 2.5 Pro）无法正确提取数字；在线调查中99.5%参与者成功完成任务，平均完成时间10.7秒；实验室研究显示相对于对照组无负面可用性影响。

Conclusion: DOT-BI是一种有效、快速且用户友好的机器人识别方法，通过利用人类独特的运动感知能力，为在线调查和流程提供了实用的解决方案。
...
TL;DR: 该论文在生成模型效率提升和计算机视觉应用两方面做出贡献：第一部分通过6项创新改进标准化流架构，包括可逆卷积层、高效耦合层、并行反演算法等；第二部分将生成模型应用于农业质量评估、地质测绘、隐私保护、艺术修复等5个实际计算机视觉任务。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决两个核心问题：一是提高生成模型（特别是标准化流）的计算效率和架构性能；二是将生成模型应用于实际计算机视觉挑战，包括数据稀缺、隐私保护、图像修复等现实世界问题。

Method: 第一部分采用6项标准化流架构创新：1) 可逆3x3卷积层及其数学可逆性证明；2) 更高效的Quad-coupling层；3) kxk卷积层的并行反演算法；4) 卷积逆运算的高效反向传播算法；5) Inverse-Flow模型；6) Affine-StableSR超分辨率模型。第二部分应用多种生成模型：1) 条件GAN用于农业质量评估；2) 堆叠自编码器用于地质测绘；3) 人脸检测和图像修复用于隐私保护；4) Stable Diffusion用于图像修复；5) 扩散模型用于艺术修复。

Result: 在标准化流方面，提出的创新显著提升了模型效率和性能；在应用方面，农业质量评估系统在种子纯度测试中取得良好准确率，地质测绘框架相比传统方法改进了特征提取，隐私保护方法有效保护自动驾驶数据，艺术修复模型能统一处理多种退化类型。

Conclusion: 该论文通过架构创新显著提升了标准化流的效率，并成功将生成模型应用于多个实际计算机视觉任务，展示了生成模型在解决现实世界问题中的潜力和价值，为生成模型的理论发展和实际应用都做出了重要贡献。
...
<details>
  <summary>Details</summary>
Motivation: RNA逆折叠问题是RNA设计中的关键挑战，需要找到能折叠成特定二级结构的核苷酸序列，这对分子稳定性和功能至关重要。序列与结构之间的复杂关系使得这一任务极具挑战性。
...
Conclusion: HyperRNA是一个有效的RNA逆折叠框架，通过超图编码器-解码器架构成功解决了RNA序列设计问题，展示了超图在捕获复杂生物分子相互作用方面的优势。
...
TL;DR: CloseUpAvatar是一种新颖的关节人体化身表示方法，通过使用带高低频纹理的平面集，根据相机距离自动切换纹理细节，在更广泛的相机运动范围内保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理更广泛的相机运动时难以保持渲染质量，特别是在近距离视图下。需要一种能够在不同相机距离下自适应调整渲染细节的化身表示方法。

Method: 使用带纹理的平面集表示化身，包含两套可学习的纹理（低频和高频细节）。根据相机到化身表面的距离自动切换纹理：近距离时使用高频纹理，距离增加时逐渐降低高频纹理的影响。

Result: 在ActorsHQ高分辨率数据集上的实验表明，CloseUpAvatar在从新颖宽范围相机位置渲染时，在质量和数量上都优于现有方法，同时通过限制所需基元数量保持高FPS。

Conclusion: CloseUpAvatar通过基于相机距离的自适应纹理切换机制，能够在更广泛的相机方向范围内实现逼真渲染，解决了现有方法在近距离视图渲染质量下降的问题。
...
TL;DR: Motion4D是一个将2D基础模型先验集成到统一4D高斯泼溅表示中的框架，通过两阶段优化解决动态场景分析中的3D不一致问题，显著提升了点跟踪、视频对象分割和新视角合成等任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有2D视觉基础模型在处理单目视频动态场景时，虽然泛化能力强但缺乏3D一致性，导致复杂3D环境中出现严重的空间错位和时间闪烁问题，这限制了场景几何和运动的理解。

Method: 提出两阶段迭代优化框架：1）顺序优化，分阶段更新运动和语义场以保持局部一致性；2）全局优化，联合优化所有属性以实现长期一致性。引入3D置信度图动态调整运动先验，以及基于像素RGB和语义误差的自适应重采样过程。通过交替优化语义场和更新SAM2提示来增强语义一致性。

Result: 在多样化场景理解任务（包括点跟踪、视频对象分割和新视角合成）的广泛评估中，Motion4D显著优于2D基础模型和现有3D方法。

Conclusion: Motion4D通过将2D先验集成到统一的4D表示中，有效解决了动态场景分析中的3D不一致问题，为复杂3D环境的理解提供了更准确和一致的方法。
...
TL;DR: LAMP使用大语言模型作为运动规划器，将自然语言描述转换为动态物体和相机的3D轨迹，通过领域特定语言和程序合成实现更好的运动控制。
...
Method: LAMP利用大语言模型作为运动规划器，定义了一个受电影摄影惯例启发的运动领域特定语言。通过LLMs的程序合成能力，从自然语言生成结构化的运动程序，这些程序被确定性地映射到3D轨迹。构建了一个大规模的程序化数据集，将自然文本描述与相应的运动程序和3D轨迹配对。

Result: 实验表明，与最先进的替代方案相比，LAMP在运动可控性和与用户意图的对齐方面表现更好，建立了第一个直接从自然语言规范生成物体和相机运动的框架。

Conclusion: LAMP通过将大语言模型作为运动规划器，实现了从自然语言到3D轨迹的有效转换，为视频生成中的运动控制提供了更强大的解决方案。
...
Method: 1. 利用密集且场景完整的3DGS渲染提供显式几何引导；2. 采用两阶段训练范式：第一阶段使用相机姿态进行粗略控制，第二阶段结合3DGS渲染进行细粒度视角和几何引导；3. 提出基于3DGS的跨轨迹数据策展策略，消除相机变换模式的训练-测试差距；4. 构建ParaDrive数据集，包含超过11万对平行轨迹视频对。

Result: ReCamDriving在相机可控性和结构一致性方面达到了最先进水平。构建的ParaDrive数据集为多轨迹监督提供了可扩展的训练资源。

Conclusion: ReCamDriving通过结合3DGS渲染的几何引导、两阶段训练策略和跨轨迹数据策展，成功实现了精确的相机可控视频生成，为基于视觉的驾驶场景视频合成提供了有效解决方案。
...
TL;DR: Colon-X项目构建了ColonVQA（最大的结肠镜多模态数据集）和ColonReason（临床推理数据集），开发了ColonR1模型，在数据稀缺条件下显著提升结肠镜多模态分析的临床推理能力。


<details>
  <summary>Details</summary>
Motivation: 推动结肠镜多模态智能发展，解决当前多模态大语言模型在结肠镜临床应用中可靠性不足的问题，填补从多模态理解到临床推理的空白。

Method: 1. 构建ColonVQA数据集（110万+视觉问答条目，涵盖76种临床发现和18种多模态任务）；2. 评估22种多模态大语言模型的泛化能力和抗干扰性；3. 创建ColonReason临床推理数据集（多专家辩论标注）；4. 开发ColonR1模型（采用任务自适应奖励和梯度稳定优化技术）。

Result: 1. 现有MLLMs在结肠镜临床输出中可靠性不足；2. ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调提升25.22%；3. 建立了结肠镜多模态分析的新推理基准。

Conclusion: Colon-X通过构建大规模数据集和开发专门推理模型，显著推进了结肠镜多模态智能从理解到临床推理的转变，为社区提供了重要的数据基础和模型资源。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有基于旋转的技术在LLMs中能平滑异常值并实现4位量化，但这些方法开销大且难以处理扩散变换器中的行向异常值。

Method: 提出ConvRot方法，采用分组旋转量化，利用正则Hadamard变换(RHT)同时抑制行向和列向异常值，将复杂度从二次降低到线性。基于此设计了ConvLinear4bit模块，集成旋转、量化、GEMM和反量化操作，实现无需重新训练的W4A4推理。

Result: 在FLUX.1-dev上的实验显示，实现了2.26倍的速度提升和4.05倍的内存减少，同时保持了图像保真度。这是首次将基于旋转的量化方法应用于扩散变换器的即插即用W4A4推理。

Conclusion: ConvRot通过分组旋转量化和正则Hadamard变换有效解决了扩散变换器中的异常值问题，实现了高效的W4A4推理，为大规模扩散模型的部署提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到3D风格化方法通常从2D图像编辑器蒸馏，需要耗时的逐资产优化，且存在多视角不一致问题，限制了大规模生产应用。

Method: 从空间分组的3D高斯中学习结构化、解耦的几何和外观潜在空间，通过潜在扩散模型对学习到的表示进行文本条件编辑。

Result: GaussianBlender能够实现即时、高保真、几何保持、多视角一致的风格化，性能超过需要逐实例测试时优化的方法。

Conclusion: 该方法实现了实用、可扩展的大规模3D风格化，解锁了实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 被动视觉系统在复杂环境中可能无法提供足够信息，而主动视觉感知通过动态交互能够获取更具信息量的数据，在机器人、自动驾驶、人机交互和监控等领域具有重要应用价值。

Method: 本文采用综述研究方法，系统性地探讨主动视觉感知的概念、原理和应用，分析其核心机制（如注意力引导、传感器移动、物体交互）以及与传统被动系统的区别。

Result: 论文全面概述了主动视觉感知的潜力、当前研究进展，识别了该领域的关键挑战，包括复杂视觉数据的实时处理、动态环境中的决策制定以及多模态感官输入的整合。

Conclusion: 主动视觉感知在多个应用领域具有显著优势，但要实现更广泛的应用，需要克服实时处理、动态决策和多模态融合等技术挑战，未来研究应关注这些关键问题的解决。
...
TL;DR: 提出SUSS（结构化不确定性相似性评分），通过结构化多元正态分布建模图像感知组件，在像素空间学习线性变换，提供可解释的感知相似性评估


<details>
  <summary>Details</summary>
Motivation: 现有感知相似性评分存在局限性：基于深度特征的方法（如LPIPS）虽然与人类视觉对齐良好，但依赖复杂非线性特征且缺乏可解释性；手工设计的度量（如SSIM）可解释但缺乏关键感知特性。需要一种既与人类感知对齐又具有可解释性的相似性评分方法。

Method: SUSS将每个图像建模为一组感知组件，每个组件用结构化多元正态分布表示。通过生成式自监督训练，使模型为人类不可感知的增强赋予高似然度。最终评分是组件对数概率的加权和，权重从人类感知数据集中学习。与基于特征的方法不同，SUSS在像素空间学习图像特定的线性残差变换，通过去相关残差和采样实现透明检查。

Result: SUSS与人类感知判断高度对齐，在不同失真类型上表现出强大的感知校准能力，并提供局部化、可解释的相似性评估解释。作为感知损失函数用于下游成像任务时，展现出稳定的优化行为和竞争性性能。

Conclusion: SUSS提供了一种既与人类感知对齐又具有可解释性的相似性评分方法，通过结构化概率建模和像素空间线性变换，实现了透明性和可检查性，在感知相似性评估和作为感知损失函数方面都表现出色。
...
TL;DR: DINO-RotateMatch框架通过结合数据集自适应图像配对和旋转感知关键点匹配，提升大规模3D重建中的图像匹配性能，在Kaggle竞赛中获得银奖。


<details>
  <summary>Details</summary>
Motivation: 解决从非结构化互联网图像进行大规模3D重建时面临的图像匹配挑战，特别是在大规模图像集合中高效准确地匹配图像对的需求。

Method: 1. 使用DINO进行语义相关图像对检索；2. 旋转增强技术捕捉方向依赖的局部特征；3. 结合ALIKED和Light Glue进行关键点提取和匹配；4. 数据集自适应的图像配对策略。
...
Method: SelfDebias在推理过程中识别图像编码器嵌入空间中的语义聚类，利用这些聚类引导扩散过程，最小化输出分布与均匀分布之间的KL散度。该方法完全无监督，不需要人工标注数据集或针对每个生成概念训练的外部分类器。

Result: 实验表明SelfDebias能够泛化到不同的提示词和扩散模型架构，包括条件模型和无条件模型。它不仅能有效去除关键人口统计学维度上的偏见，同时保持生成图像的视觉保真度，还能处理更抽象概念的偏见识别。

Conclusion: SelfDebias提供了一种完全无监督的测试时去偏方法，能够自动识别语义模式并减少扩散模型输出中的偏见，为改善文本到图像生成模型的公平性提供了有效解决方案。
...
TL;DR: 提出LSRS方法，通过潜在尺度拒绝采样逐步精炼VAR模型的token映射，在推理时提升生成质量，仅增加少量计算开销。


<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中采用分层尺度的自回归处理，但同一尺度内的并行token采样可能导致结构错误，影响生成图像质量。

Method: 提出潜在尺度拒绝采样(LSRS)，在推理过程中使用轻量级评分模型评估每个尺度的多个候选token映射，选择高质量映射指导后续尺度生成，优先处理对结构一致性关键的前期尺度。

Result: LSRS显著提升VAR生成质量，计算开销极小。VAR-d30模型推理时间仅增加1%时，FID从1.95降至1.78；推理时间增加15%时，FID可进一步降至1.66。

Conclusion: LSRS为增强VAR基生成模型提供了高效的测试时扩展解决方案，能有效缓解自回归误差累积，同时保持计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 古埃及象形文字完全由图画组成，翻译面临单个符号多义性等挑战。深度学习翻译应用快速发展，为自动识别和翻译象形文字提供了技术基础。
...
TL;DR: 本文提出了一种基于视频的呼吸率测量方法，该方法在受试者运动干扰下仍能保持高精度，平均绝对误差为0.57次/分钟。
...
Method: 结合数学变换的方法，使用最小硬件要求，通过视频分析测量呼吸率，相对地面真实值的偏差小于5%。

Result: 在14名志愿者超过2小时30分钟的视频数据上测试，平均绝对误差为0.57次呼吸/分钟，优于先前工作，对运动引起的失真具有更强抵抗力。

Conclusion: 该方法能够在受试者运动干扰下准确测量呼吸率，无需对受试者行为施加显著限制，实现了远程呼吸率监测的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前公开的病理学数据集主要来自西方人群，缺乏中东等数字化程度较低地区的代表性数据，导致AI模型在这些人群中的泛化性未知。这促使研究者公开来自伊拉克的数据集，以支持全球多样化人群的病理AI模型开发和验证。

Method: 收集了伊拉克埃尔比勒185名患者的前列腺穿刺活检样本，获得339张全切片图像。由三位病理学家独立分配Gleason评分和ISUP分级。使用两种高通量扫描仪（Leica和Hamamatsu）和一种紧凑型扫描仪（Grundium）进行扫描。所有切片均经过去标识化处理，并以原始格式提供。

Result: 创建了一个包含339张前列腺活检全切片图像的数据集，来自伊拉克的185名患者。数据集包含三位病理学家独立分配的Gleason评分和ISUP分级，使用三种不同扫描仪获取。数据将存储在Bioimage Archive中，采用CC BY 4.0许可协议发布。

Conclusion: 该数据集填补了中东地区公开病理学数据的空白，支持AI模型在多样化人群中的开发和验证。数据集可用于分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估，有助于提高AI病理模型在全球范围内的泛化能力。
...
TL;DR: 小规模视觉Transformer（5M参数）在不同预训练、中间微调和下游任务组合下的性能研究，发现预训练和微调有收益递减，不当的中间微调可能损害下游性能


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer架构在计算机视觉和NLP领域表现出色，但通常需要大量参数和训练数据才能获得强大性能。本研究旨在探索小规模视觉Transformer（仅5M参数）在不同训练策略下的表现，了解预训练、中间微调和下游任务对性能的边际效益

Method: 使用三种不同的预训练数据集、中间微调数据集和下游数据集及训练目标，系统性地探索它们对5M参数视觉Transformer的边际效益。通过实验分析不同训练阶段的组合效果

Result: 研究发现：1）预训练和微调总是有助于模型性能，但存在收益递减现象；2）中间微调可能对下游性能产生负面影响，特别是当任务机制不相似时；3）不当的任务堆叠可能浪费计算资源并降低性能

Conclusion: 小规模视觉Transformer最受益于有针对性的预训练和谨慎的数据选择，而不加区分的中间任务堆叠不仅浪费计算资源，甚至可能损害最终性能。这为资源受限环境下的模型训练提供了重要指导
...
TL;DR: CaravelMetrics是一个用于自动化脑血管分析的框架，通过骨架化图表示建模血管形态，可计算15个形态、拓扑、分形和几何特征，支持全局和区域分析。


<details>
  <summary>Details</summary>
Motivation: 开发一个可扩展、全自动的脑血管定量特征提取框架，支持血管健康和衰老的规范建模和群体水平研究。

Method: 整合基于图谱的区域分割、中心线提取和图构建，通过骨架化图表示建模血管形态，计算15个形态计量、拓扑、分形和几何特征。

Result: 应用于IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁），产生可重复的血管图，捕捉到年龄和性别相关变化，以及教育程度相关的血管复杂性增加。

Conclusion: 该框架为定量脑血管特征提取提供了一个可扩展、全自动的方法，支持血管健康和衰老的规范建模和群体水平研究。
...
TL;DR: 开发了一个基于Siamese Swin Transformer的双交叉注意力模型（SSDCA），用于结合直肠癌患者治疗前后的纵向内窥镜图像，准确区分临床完全缓解和局部复发，实现早期检测。


<details>
  <summary>Details</summary>
Motivation: 对于直肠癌患者在接受新辅助治疗后显示临床完全缓解的情况，目前采用观察等待策略。然而，需要客观准确的方法从随访内窥镜图像中早期检测局部复发，以便及时管理护理并预防远处转移。
...
Result: SSDCA在62名患者的测试集上表现最佳：平衡准确率81.76%±0.04，敏感性90.07%±0.08，特异性72.86%±0.05。鲁棒性分析显示模型对血液、粪便、毛细血管扩张和图像质量差等伪影具有稳定性能。UMAP聚类显示SSDCA具有最大的簇间分离（1.45±0.18）和最小的簇内离散（1.07±0.19）。

Conclusion: SSDCA能够有效结合纵向内窥镜图像，准确区分临床完全缓解和局部复发，为直肠癌观察等待策略中的早期复发检测提供了可靠的计算机辅助诊断工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的零样本视频编辑方法主要关注在注意力机制中集成帧间对应关系，但软约束不足以识别有效的特征关注，导致时间不一致性问题。

Method: FRESCO方法整合了帧内对应关系和帧间对应关系，形成更鲁棒的时空约束，确保帧间语义相似内容的一致转换，并通过显式优化特征实现高时空一致性。

Result: 在视频到视频转换和文本引导视频编辑两个零样本任务上的实验表明，FRESCO能够生成高质量、连贯的视频，相比现有方法有显著提升。

Conclusion: FRESCO通过增强的时空约束机制，在零样本视频编辑任务中实现了更好的视觉连贯性，代表了当前零样本方法的重要进展。
...
Method: 1. 受LLM统一不同模态能力的启发，将视频和3D运动建模为统一标记序列，使用单独的嵌入层缓解分布差距；2. 设计序列建模策略，在单一框架中集成两个不同任务；3. 设计新颖的3D运动标记器，采用时间扩展策略，使用单个VQ-VAE生成量化运动标记，具有多个专家解码器处理身体形状、平移、全局方向和身体姿态。
...
TL;DR: TempR1是一个面向多模态大语言模型的时间感知多任务强化学习框架，通过系统化增强MLLMs的时间理解能力，提升长视频分析性能。
...
Method: 提出TempR1框架：1）构建多任务语料库，暴露模型于多样化的时间结构和语义；2）基于GRPO算法实现稳定有效的跨任务优化；3）将时间任务分为预测区间与真实实例之间的三种对应类型，并为每种类型设计定制化的定位奖励。

Result: TempR1在多个基准测试中达到最先进性能，其互补任务的联合优化产生强大的协同效应，既增强了泛化能力，又提升了单任务性能。

Conclusion: TempR1为MLLMs中的时间推理建立了一个可扩展且原则性的范式，通过系统化的多任务强化学习框架有效提升了模型的时间理解能力。
...
TL;DR: UniID是一个无需调优的人脸个性化框架，通过统一文本嵌入和适配器两种范式，在保持高身份保真度的同时实现灵活的文本控制。
...
Method: 提出统一的UniID框架，采用身份聚焦学习方案训练两个分支专门捕获身份特征，在推理时引入归一化重缩放机制恢复基础扩散模型的文本控制能力，同时让互补的身份信号相互增强。

Result: 与六种最先进方法相比，UniID在身份保持和文本控制方面都实现了优越性能。

Conclusion: UniID通过统一两种范式并采用原则性的训练-推理策略，成功实现了高保真度人脸个性化与灵活文本控制的平衡。
...
TL;DR: DirectDrag是一个无需手动遮罩和文本提示的基于拖拽的图像编辑框架，通过自动软遮罩生成和读取引导特征对齐实现精确编辑


<details>
  <summary>Details</summary>
Motivation: 现有基于拖拽的图像编辑方法严重依赖手动提供的遮罩和文本提示来保持语义保真度和运动精度，去除这些约束会导致视觉伪影或空间控制不佳的问题

Method: 提出DirectDrag框架，包含两个关键创新：1) 自动软遮罩生成模块，从点位移智能推断可编辑区域；2) 读取引导特征对齐机制，利用中间扩散激活保持结构一致性

Result: 在DragBench和真实场景的广泛实验中，DirectDrag在无需手动遮罩或提示的情况下，实现了优于现有方法的图像质量，同时保持了竞争力的拖拽精度
...
TL;DR: 研究发现现有前馈3D重建模型VGGT虽无显式异常值剔除机制，但能天然区分干扰图像，通过特定层的异常抑制行为实现有效噪声过滤


<details>
  <summary>Details</summary>
Motivation: 野外图像集合的3D重建常受"噪声"图像干扰，传统SfM通过几何验证和异常值剔除处理，而前馈3D重建模型缺乏这些机制，导致在野外条件下性能下降

Method: 通过分析不同比例合成干扰物下的VGGT模型，识别出天然展现异常抑制行为的特定层，利用该层编码的判别性内部表示实现异常视图剔除，无需额外微调或监督

Result: 在受控和野外数据集上的广泛实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到不同场景

Conclusion: 前馈3D重建模型具有内在的噪声过滤能力，可通过特定层的异常抑制行为实现有效的异常视图剔除，为野外条件下的3D重建提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么在高维数据空间操作群作用，难以解耦变换相关的子空间；要么需要手动划分潜在变量为等变和不变子空间，限制了在表示空间中稳健学习和操作群作用的能力

Method: 使用可学习的二元掩码配合直通估计，动态划分潜在表示为变换敏感和不变组件，在统一优化框架中联合学习潜在解耦和群变换映射，可无缝集成到任何标准编码器-解码器架构

Result: 在五个2D/3D图像数据集上验证了方法，证明能自动学习群作用的解耦潜在因子，下游分类任务确认了学习表示的有效性
...
TL;DR: NVRC-Lite扩展了NVRC视频压缩框架，通过集成多尺度特征网格和八叉树上下文模型，实现了轻量级神经表示的高效视频压缩，在保持低计算复杂度的同时显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩方法中，NVRC首次采用端到端压缩框架并取得最佳性能，但现有轻量级INR虽然计算复杂度低（<10kMACs/像素），仍存在两个主要问题：1）低复杂度下INR性能有限；2）现有熵编码通常使用自回归模型，虽然有效但编码速度过慢。

Method: 提出NVRC-Lite框架，包含两个关键技术改进：1）在轻量级神经表示中集成多尺度特征网格，使用更高分辨率网格显著提升低复杂度INR的性能；2）提出基于八叉树的上下文模型用于高维特征网格的熵编码，替代传统的自回归模型，大幅加速熵编码模块。

Result: 实验结果表明，NVRC-Lite在PSNR和MS-SSIM指标上分别比当前最佳轻量级INR视频编解码器C3节省高达21.03%和23.06%的BD-rate，同时实现8.4倍编码加速和2.5倍解码加速。

Conclusion: NVRC-Lite通过多尺度特征网格和八叉树上下文模型，成功扩展了NVRC框架到轻量级表示，在保持低计算复杂度的同时显著提升了视频压缩性能并大幅加速了编码过程，为轻量级神经视频压缩提供了有效解决方案。
...
TL;DR: C3G：一种前馈框架，通过可学习token引导生成紧凑的3D高斯分布，在关键空间位置进行重建，减少冗余并提升多视角特征聚合效果


<details>
  <summary>Details</summary>
Motivation: 现有方法使用逐像素3D高斯泼溅进行重建，但会产生过多冗余高斯分布，导致内存开销大、多视角特征聚合效果差，从而影响新视角合成和场景理解性能

Method: 提出C3G框架：1）使用可学习token通过自注意力聚合多视角特征来指导高斯生成；2）仅在关键空间位置估计紧凑的3D高斯分布；3）利用学习到的注意力模式进行高斯解码以高效提升特征

Result: 在无姿态新视角合成、3D开放词汇分割和视角不变特征聚合等任务上表现出色，相比现有方法具有更好的内存效率和特征保真度

Conclusion: 紧凑但几何意义明确的表示足以实现高质量的场景重建和理解，证明了该方法在减少冗余同时保持特征有效性的优势
...
TL;DR: RELIC是一个统一的交互式世界模型框架，通过压缩历史潜在令牌和相机感知内存结构，实现实时长时程流式生成、一致空间记忆和精确用户控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只解决交互式世界模型三个关键要素中的一个（实时长时程流式生成、一致空间记忆、精确用户控制），而同时实现这三个方面极具挑战性，例如长时记忆机制往往会降低实时性能。

Method: 基于自回归视频扩散蒸馏技术，使用高度压缩的历史潜在令牌（包含相对动作和绝对相机姿态）在KV缓存中表示长时程记忆；通过微调双向教师视频模型生成超出原始5秒训练时长的序列，并采用新的内存高效自强制范式将其转换为因果学生生成器，实现全上下文蒸馏。

Result: RELIC作为140亿参数模型，在精心策划的Unreal Engine渲染数据集上训练，能够以16 FPS实现实时生成，相比先前工作表现出更准确的动作跟随、更稳定的长时程流式生成和更鲁棒的空间记忆检索能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大型多模态模型的自动化图形设计方法存在几何布局不准确、缺乏专业工作流程所需的分层迭代编辑能力的问题，需要开发更精确可控的设计框架。
...
TL;DR: SimFlow通过固定VAE编码器输出的方差为常数，简化了归一化流与VAE的联合训练，避免了复杂的噪声添加和去噪流程，在ImageNet 256×256生成任务上取得了SOTA性能。
...
Method: 提出SimFlow方法，将VAE编码器输出的方差固定为常数（如0.5）。这样既能让编码器输出更广泛的token分布，让解码器学习从增强的token分布中重建干净图像，又能简化VAE的证据下界，使NF与VAE的联合训练更加稳定。

Result: 在ImageNet 256×256生成任务上，SimFlow获得gFID分数2.15，优于之前的SOTA方法STARFlow（gFID 2.40）。与REPA-E方法结合后，gFID进一步提升至1.91，在NF方法中创造了新的SOTA。

Conclusion: 通过简单的方差固定策略，SimFlow有效解决了现有NF方法的两个关键问题，实现了更简单、更高效的训练流程，并在图像生成任务上取得了显著的性能提升。
...
TL;DR: 该研究提出了一种动态评估大语言模型安全性的方法，通过定义行为分类、训练分类器估计伦理熵S(t)，并测量四种前沿模型在压力测试下的熵动态变化，发现基础模型持续熵增长，而调优变体将伦理熵降低约80%。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型安全评估主要依赖静态基准测试，但关键的安全失效往往是动态的：包括分布漂移下的价值漂移、越狱攻击以及部署中对齐的缓慢退化。基于"智能第二定律"将伦理熵视为状态变量的框架，需要使其对大语言模型具有可操作性。

Method: 1. 定义五维行为分类法；2. 训练分类器从模型转录中估计伦理熵S(t)；3. 测量四种前沿模型的基础版本和指令调优版本在压力测试下的熵动态；4. 从轨迹中估计有效对齐工作率gamma_eff；5. 将S(t)和gamma_eff嵌入监控管道，当熵漂移超过稳定性阈值时发出警报。

Result: 基础模型显示持续的熵增长，而调优变体抑制了漂移并将伦理熵降低了约80%。通过轨迹可以估计有效对齐工作率gamma_eff，并将S(t)和gamma_eff嵌入监控管道，实现对价值漂移的运行时监督。

Conclusion: 该研究将伦理熵框架操作化，提供了一种动态监控大语言模型安全性的方法，能够实时检测价值漂移并发出警报，为模型部署中的持续对齐监督提供了实用工具。
...
Method: 1. 分析现有EaaS水印的脆弱性，通过改写输入文本绕过水印检测；2. 提出WET水印技术，使用线性变换嵌入向量，通过反向变换和相似度比较进行验证

Result: 1. 改写攻击能有效绕过当前最先进的EaaS水印技术；2. WET水印对改写攻击具有鲁棒性，验证准确率接近完美；3. 通过消融研究分析了WET各组件和超参数的重要性

Conclusion: 现有EaaS水印存在被改写攻击绕过的漏洞，提出的WET水印技术能有效防御此类攻击，为EaaS提供商提供了更强的知识产权保护方案
...
TL;DR: 本文提出Reasoning Dependency Generation (RDG)框架，通过生成平衡的推理QA对来减轻大语言模型中的选择支持性偏见，实验显示在记忆和评估任务上分别有81.5%和94.3%的改进。


<details>
  <summary>Details</summary>
Motivation: 研究发现大语言模型在评估时表现出选择支持性偏见，会系统性偏袒自己选择的选项，这可能损害AI辅助决策的客观性。现有去偏见方法主要针对人口统计和社会偏见，而解决LLM中认知偏见的方法仍未被充分探索。

Method: 提出Reasoning Dependency Generation (RDG)框架，通过生成无偏见的推理数据来减轻选择支持性偏见。RDG自动构建平衡的推理QA对，明确（解）建模选择、证据和理由之间的依赖关系。该方法能跨领域生成大规模QA对数据集，包含上下文依赖数据和依赖解耦数据。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在改进劳动力市场信息的分类，通过将职位空缺文本链接到欧洲技能、能力、资格和职业（ESCO）分类法以及欧洲资格框架（EQF），以超越表面技能提取，深入分析职业和资格在职位文本中的表示方式。

Method: 研究比较了两种主要方法：句子链接和实体链接。开发了开源工具整合这两种方法，创建了两个专门用于评估职业和资格表示的标注数据集，并探索了使用生成式大语言模型完成此任务的不同方式。

Result: 研究发布了开源工具和标注数据集，推进了工作实体提取的技术水平，为在数字化中介经济中研究工作、技能和劳动力市场叙事提供了计算基础设施。

Conclusion: 该研究通过开发开源工具和创建评估数据集，显著改进了劳动力市场信息的分类能力，为分析数字化经济中的工作、技能和就业话语提供了重要的计算基础设施和资源。
...
TL;DR: InvertiTune框架通过可控数据生成和监督微调，实现高效的单次知识图谱构建，在质量和效率上超越现有方法
...
Method: 提出InvertiTune框架：1）从大型知识库中系统提取子图并过滤噪声；2）利用LLM生成对应的自然文本描述；3）通过监督微调训练轻量级模型进行单次KG构建

Result: 在CE12k数据集上，InvertiTune超越更大的非微调LLM和最先进的Text2KG方法，在CrossEval-1200测试集上展现出更强的跨数据集泛化能力

Conclusion: 高质量、现实的训练数据对于推进高效高性能的Text2KG系统至关重要，InvertiTune框架为此提供了有效解决方案
...
TL;DR: 本文提出了一种从政治文本中检测和解析解释的框架，通过训练轻量级因果语言模型来提取因果主张的结构化数据集。


<details>
  <summary>Details</summary>
Motivation: 解释是人们理解政治世界的基本要素，但政治科学中对解释的系统分析仍不充分，现有方法分散且常针对特定问题。

Method: 训练轻量级因果语言模型，从政治文本中提取因果主张的结构化数据集（原因-结果对），用于下游分析。

Result: 该方法能够大规模研究因果解释，展示了适度的标注需求、良好的泛化能力以及相对于人工编码的准确性。

Conclusion: 提出的框架为政治文本中的解释分析提供了系统化工具，有助于更深入地理解政治话语中的因果推理模式。
...
TL;DR: 提出RMFT隐私保护微调技术，显著减少LLM中的PII记忆，在Enron数据集上实现80%以上的提取率降低，同时性能影响最小


<details>
  <summary>Details</summary>
Motivation: 当前LLM中的记忆化现象带来严重安全和隐私风险，模型容易记忆训练数据中的个人身份信息，需要有效的隐私保护微调方法

Method: 提出随机掩码微调技术，通过随机掩码机制减少PII记忆，同时引入MaxTER评估框架分析隐私-效用权衡

Result: 在Enron数据集上，RMFT相比基线微调减少80.81%总提取率和80.17%已见提取率，优于去重方法，仅增加5.73%困惑度

Conclusion: RMFT是有效的隐私保护微调技术，在显著减少PII记忆的同时保持模型性能，为LLM隐私保护提供了新解决方案
...
TL;DR: LLM辅助的标注流水线用于分析西班牙语-英语和西班牙语-瓜拉尼语双语语料的社会语言学和主题特征，揭示了性别、语言优势和语用功能之间的系统性关联。


<details>
  <summary>Details</summary>
Motivation: 传统的社会语言学研究依赖人工标注，限制了大规模语料分析。本研究旨在利用大语言模型自动标注双语语料的社会语言学特征，为跨语言和低资源双语研究提供计算方法的支持。

Method: 开发了LLM辅助的标注流水线，自动标注了3,691个语码转换句子的主题、体裁和语用功能，整合了迈阿密双语语料库的人口统计元数据，并为西班牙语-瓜拉尼语数据集添加了新的主题标注。

Result: 在迈阿密数据中发现了性别、语言优势和语用功能之间的系统性关联；在巴拉圭文本中发现了正式瓜拉尼语和非正式西班牙语之间的明显双语分工。这些发现用大规模定量证据复制和扩展了早期的互动和社会语言学观察。
...
TL;DR: PERCS数据集：面向四种不同医学知识水平用户的生物医学摘要简化数据集，支持个性化可控摘要生成研究


<details>
  <summary>Details</summary>
Motivation: 现有医学文本简化资源通常假设单一通用受众，忽视了不同用户群体在医学素养和信息需求方面的显著差异。为解决这一问题，需要开发能够针对不同用户群体进行个性化简化的数据集和方法。

Method: 引入PERCS数据集，包含生物医学摘要及其针对四种不同用户群体的简化版本：普通大众、医预科学生、非医学研究人员和医学专家。所有摘要均由医生根据详细的错误分类法进行事实准确性和用户群体匹配度审核。

Result: 技术验证显示不同用户群体的摘要在可读性、词汇使用和内容深度方面存在明显差异。使用自动评估指标对四种大型语言模型进行基准测试，评估了全面性、可读性和忠实度，为未来研究建立了基线结果。

Conclusion: PERCS数据集支持个性化医学沟通和可控生物医学摘要生成研究，公开提供数据集、标注指南和评估材料，有助于推动针对不同医学素养水平用户的精准医学信息传播。
...
TL;DR: 本文提出Idea-Gated Transformer架构，通过分离语义规划和句法生成来解决自回归语言模型中的"主题漂移"问题，使用概念向量实时门控词汇选择以保持生成内容的一致性。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一个词预测训练中容易出现"主题漂移"问题，生成内容会逐渐偏离初始提示，这是因为模型过于依赖局部关联而非全局规划。虽然增大模型规模可以缓解这一问题，但NTP目标本身的短视性仍然存在。

Method: 提出Idea-Gated Transformer架构，引入辅助的"Idea Head"来预测未来上下文窗口的词袋分布，生成潜在"概念向量"。该向量通过可微分门控机制主动门控主词汇表，抑制语义不相关的标记，实时剪枝搜索空间。
...
TL;DR: HBLR框架通过置信感知的符号翻译和假设驱动的后向推理，解决了传统前向推理方法的冗余路径、幻觉步骤和语义漂移问题，在多个推理基准测试中取得了更好的准确性和效率。
...
Method: 提出假设驱动的后向逻辑推理框架，包含两个核心模块：1) 置信感知符号翻译 - 仅将高置信度片段转换为逻辑形式，不确定内容保留为自然语言，并通过翻译反思模块确保语义保真度；2) 假设驱动后向推理 - 模拟人类演绎思维，假设结论为真并递归验证其前提，通过推理反思模块识别和纠正有缺陷的推理步骤。

Result: 在五个推理基准测试上的广泛实验表明，HBLR在准确性和效率方面均持续优于强基线方法。

Conclusion: HBLR框架通过结合置信感知符号翻译和假设驱动后向推理，有效解决了传统前向推理方法的局限性，为逻辑推理任务提供了更高效可靠的解决方案。
...
TL;DR: 研究者收集了Portal 2合作模式中11.5小时的口语对话语料库，包含24.5K条话语，用于分析复杂协作环境中的语言现象。


<details>
  <summary>Details</summary>
Motivation: 合作视频游戏中的语言数据具有独特价值，包含复杂空间参考、澄清修复等现有闲聊或任务导向对话语料库中罕见的现象，需要专门语料库来研究复杂协作环境中的语言使用。

Method: 收集Portal 2合作模式中11.5小时的口语人类对话，形成包含24.5K条话语的Portal对话语料库，包括玩家视频、音频、转录、游戏状态数据以及手动和自动语言标注。

Result: 识别出多种现有语料库中罕见的语言现象，包括复杂空间参考、澄清与修复、临时约定形成等，为研究复杂协作环境中的语言使用提供了丰富数据。

Conclusion: Portal对话语料库为分析复杂、情境化、协作问题解决场景中的语言使用提供了宝贵资源，已公开发布供未来研究使用。
...
TL;DR: Dual LoRA通过将低秩矩阵分为幅度组和方向组，引入ReLU和符号函数来改进LoRA性能，在多个NLP任务上优于原始LoRA及其变体。


<details>
  <summary>Details</summary>
Motivation: LoRA作为参数高效微调方法，由于其低秩假设，训练出的模型性能往往不令人满意。需要改进LoRA以更好地模拟基于梯度优化算法的全参数微调过程。

Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制是否更新参数及更新幅度，方向组决定参数向前还是向后移动。通过为幅度组添加ReLU函数，为方向组添加符号函数来实现这一机制。

Result: 在GPT-2、RoBERTa、DeBERTa和LLaMA-1/2/3等基准模型上，对自然语言生成、理解和常识推理任务进行广泛实验。结果显示，在相同可训练参数数量下，Dual LoRA始终优于LoRA及其最先进的变体。

Conclusion: Dual LoRA通过引入幅度和方向分离的归纳偏置，有效改进了LoRA的性能，能够更好地模拟全参数微调的参数更新过程，在多种NLP任务上取得了更好的效果。
...
TL;DR: 提出PretrainZero强化主动学习框架，将RL从领域特定的后训练扩展到通用预训练，通过主动识别预训练语料中的合理信息内容进行推理预测，无需可验证标签即可显著提升基础模型的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大思维模型虽然在某些专业领域（如软件和数学）表现出专家级能力，但仍严重依赖特定领域的可验证奖励，这限制了通用推理能力的扩展。人类能够主动从一般经验中学习，而现有方法无法实现这种通用预训练。

Method: 提出PretrainZero框架：1）主动预训练：学习统一推理策略，主动识别预训练语料中的合理信息内容并进行RL推理预测；2）自监督学习：无需可验证标签、预训练奖励模型或监督微调，直接在通用维基百科语料上使用RL预训练推理器；3）验证扩展：通过处理越来越具有挑战性的掩码跨度来增强通用推理能力。

Result: 在强化预训练中，PretrainZero将Qwen3-4B-Base在MMLU-Pro、SuperGPQA和数学平均基准上的性能分别提升了8.43、5.96和10.60分。预训练模型还可作为下游RLVR任务的推理基础模型。

Conclusion: PretrainZero成功将强化学习从领域特定的后训练扩展到通用预训练，通过主动学习和自监督方法突破了通用推理的可验证数据壁垒，显著提升了基础模型的通用推理能力，并为下游任务提供了更好的推理基础模型。
...
TL;DR: 该论文研究了Top-k注意力机制在解码和训练阶段的有效性及理论机制，验证了精确Top-k解码在长上下文任务中能达到甚至超越全注意力的性能，并探索了原生Top-k注意力训练策略和近似算法的影响。
...
Result: 1. 解码阶段仅保留与查询相似度最高的关键键作为上下文窗口，在HELMET和LongBench v2等下游任务上达到或超越全注意力性能；2. 训练与推理阶段Top-k注意力操作的一致性有助于进一步释放Top-k解码潜力；3. 下游任务性能与近似保真度呈正相关；4. Top-k注意力SFT后的模型在下游任务中表现出明显的熵减少现象。

Conclusion: Top-k注意力机制能有效降低大语言模型的推理计算成本，同时保持或提升性能。低熵状态更适合Top-k解码，训练与推理的一致性对性能提升至关重要，近似算法的精度直接影响下游任务表现。
...
TL;DR: 大型语言模型在摘要生成中的推理能力研究：发现推理策略对摘要质量和事实忠实度存在权衡，过度推理反而可能损害事实一致性


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在数学和代码生成等分析任务中表现出色，但其在抽象摘要生成中的实用性被广泛假设但未经验证。本研究旨在填补这一空白，探索推理策略在摘要任务中的实际效果。

Method: 首先将通用推理策略定制到摘要领域，然后对8种推理策略和3个大型推理模型在8个不同数据集上进行系统性大规模比较研究，评估摘要质量和忠实度。

Result: 研究发现推理并非通用解决方案，其效果高度依赖于具体策略和上下文。观察到摘要质量与事实忠实度之间的权衡：显式推理策略倾向于提高流畅性但牺牲事实基础，而大型推理模型中的隐式推理则呈现相反模式。增加模型的内部推理预算不会改善甚至可能损害事实一致性。
...
TL;DR: 该论文提出了INDI-PROP数据集，用于印度新闻媒体中的宣传分析，包含三个层次的细粒度叙事分类，并开发了FANTA和TPTC两种GPT-4o-mini引导的多跳推理框架。


<details>
  <summary>Details</summary>
Motivation: 叙事是宣传的认知和情感支架，将孤立的说服技巧组织成连贯的故事。现有研究缺乏细粒度的叙事分类，特别是在意识形态背景下。作者旨在分析印度新闻媒体中的宣传，关注两个极化社会政治事件。

Method: 1. 创建INDI-PROP数据集：包含1,266篇关于CAA和农民抗议的文章，进行三层注释：意识形态偏见、事件特定的细粒度叙事框架、说服技巧。2. 开发FANTA框架：通过信息提取和上下文框架集成进行分层推理。3. 开发TPTC框架：采用两阶段方法系统分解说服线索。

Result: 评估显示，FANTA和TPTC框架在偏见、叙事和说服技巧分类方面均显著优于基线模型。INDI-PROP是首个基于意识形态的细粒度叙事数据集。

Conclusion: 该研究提供了分析宣传叙事的系统方法，特别是在意识形态极化背景下。提出的多级注释框架和GPT-4o-mini引导的推理方法为宣传分析提供了有效工具，有助于理解新闻媒体中的叙事构建和说服机制。
...
TL;DR: 提出一个可解释的事实一致性评估框架，通过将文本分解为原子事实，采用灵活的、无模式的方法，结合加权指标来增强事实评估，并控制复杂领域的评估复杂度。
...
Method: 提出可解释的事实一致性评估框架，将文本分解为原子事实，采用灵活的无模式方法。不同于先前方法的绝对指标，引入加权指标来增强事实评估。同时提出机制来控制复杂领域的评估复杂度。
...
TL;DR: 意大利首次全面调查显示：生成式AI在意大利广泛采用，正取代其他技术成为主要信息来源，但存在数字素养不足和性别鸿沟问题


<details>
  <summary>Details</summary>
Motivation: 研究旨在了解生成式AI在意大利的采用情况、使用模式和数字素养水平，评估AI技术可能加剧数字鸿沟的风险，特别是考虑到用户对AI局限性的认识不足

Method: 基于新收集的1,906名意大利语成年人的调查数据，进行首次全面的生成式AI采用、使用模式和素养的实证映射研究

Result: 研究发现：1) 生成式AI在工作和个人用途中广泛采用，包括情感支持和医疗建议等敏感任务；2) 生成式AI正取代其他技术成为主要信息来源；3) 用户数字素养普遍较低，难以识别错误或虚假信息；4) 存在显著的性别鸿沟，女性采用率仅为男性的一半，使用频率也更低；5) 数字素养是采用的关键预测因素，但只能部分解释性别差异

Conclusion: 研究强调需要有针对性的教育计划，并进一步调查仅凭能力无法解释的公平参与障碍，以应对生成式AI带来的数字鸿沟风险
...
TL;DR: 该研究提出了水文科学与工程领域的LLM评估基准Hydro-SE Bench，包含4000道选择题，评估LLM在基础概念知识、工程应用能力和推理计算能力方面的表现，发现商业LLM准确率在0.74-0.80之间，小参数LLM在0.41-0.68之间。


<details>
  <summary>Details</summary>
Motivation: 水文科学与工程是保障人类供水、生产清洁水电能源和减轻洪旱灾害的关键领域，需要多学科知识和专家协作。随着大语言模型的快速发展，其在Hydro-SE领域的应用潜力日益受到关注，但LLM在该领域的知识和应用能力尚未得到充分评估。

Method: 提出了Hydro-SE LLM评估基准（Hydro-SE Bench），包含4000道多项选择题，涵盖9个子领域，能够评估LLM在基础概念知识、工程应用能力以及推理和计算能力三个方面的表现。

Result: 评估结果显示：商业LLM的准确率在0.74到0.80之间，小参数LLM的准确率在0.41到0.68之间。LLM在与自然科学和物理科学密切相关的子领域表现良好，但在行业标准和水工结构等特定领域知识方面表现不佳。模型扩展主要提高了推理和计算能力，但在实际工程应用问题处理方面仍有很大提升空间。

Conclusion: 该研究揭示了LLM在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，并为Hydro-SE研究人员应用LLM提供了实用指导。LLM在工程应用能力方面仍有改进空间，特别是在处理行业标准和特定领域知识方面。
...
TL;DR: 该研究使用认知神经科学的功能定位方法，在大语言模型中识别出对67种英语句法现象最敏感的单元，发现不同句法一致性现象（如主谓一致、回指一致、限定词-名词一致）会激活重叠的单元集合，表明一致性构成了大语言模型中有意义的句法功能类别。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型能够可靠地区分合乎语法和不合语法的句子，但语法知识在模型内部如何表示仍然是一个开放性问题。研究者希望探究不同的句法现象在大语言模型中是否共享或使用不同的组件，特别是想了解句法一致性是否构成模型中有意义的句法功能类别。

Method: 采用认知神经科学中的功能定位方法，在7个开源权重的大语言模型中识别对67种英语句法现象最敏感的单元。这些单元在包含相应句法现象的句子中持续被激活，并因果性地支持模型的句法表现。研究还进行了跨语言分析，包括英语、俄语、中文以及57种不同语言的比较。

Result: 研究发现不同类型的句法一致性（主谓一致、回指一致、限定词-名词一致）会激活重叠的单元集合，表明一致性在大语言模型中构成了有意义的句法功能类别。这一模式在英语、俄语和中文中都成立。在57种语言的跨语言分析中，结构更相似的语言在主谓一致上共享更多的单元。

Conclusion: 句法一致性作为句法依赖关系的关键标记，在大语言模型的表征空间中构成了有意义的类别。研究揭示了不同句法现象在大语言模型中的共享表示机制，为理解大语言模型如何编码句法知识提供了重要见解。
...
Method: 开发了一个应用程序，利用语言技术评估AI导师的教学质量，包含演示评估软件、模型检查功能和数据可视化工具

Result: 创建了AITutor-EvalKit工具，能够评估AI导师的教学质量，支持学习过程，并可用于收集用户反馈和标注数据

Conclusion: AITutor-EvalKit是一个有价值的工具，既服务于教育利益相关者评估AI导师质量，也为ACL社区提供了学习和反馈收集的平台
...
TL;DR: DZ-TDPO是一个非破坏性对齐框架，通过冲突感知动态KL约束和可学习时间注意力偏置解决长对话中的状态惯性问题，在MSC数据集上达到SOTA胜率，同时保持零样本泛化能力。
...
Method: 提出DZ-TDPO框架，结合冲突感知动态KL约束和可学习时间注意力偏置，通过精确的注意力调节而非破坏性权重更新来缓解状态惯性。

Result: 在MSC数据集上，DZ-TDPO达到86.2%的胜率（Phi-3.5模型），Qwen2.5-7B模型实现99.4%的胜率且困惑度开销可忽略；发现"容量-稳定性权衡"现象，小模型需要付出"对齐税"来克服历史惯性。

Conclusion: DZ-TDPO通过精确的注意力调节有效解决长对话状态惯性问题，保持模型通用能力，证实了非破坏性对齐方法的有效性，为长对话系统优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 揭示当前大语言模型安全对齐策略的不足，发现模型在表示层面存在新的攻击面，即通过上下文示例中的语义替换可以劫持模型的内部表示。

Method: 提出一种无需优化的上下文表示劫持攻击方法：在多个上下文示例中系统地将有害关键词（如"bomb"）替换为良性标记（如"carrot"），并提供一个有害请求的前缀。这种替换导致良性标记的内部表示向有害标记的表示收敛。

Result: 攻击在闭源和开源系统上都具有广泛的迁移性，在Llama-3.3-70B-Instruct上仅用单句上下文覆盖就达到74%的攻击成功率。可解释性工具显示语义覆盖逐层出现，早期层的良性含义在后期层收敛为有害语义。

Conclusion: 当前的安全对齐策略在表示层面存在不足，需要在表示层面进行操作，而不仅仅是表面指令层面。Doublespeak攻击揭示了大语言模型潜在空间中的新攻击面。
...
TL;DR: 将对比解码方法DoLa应用于T5/FLAN-T5编码器-解码器架构，评估其对指令遵循能力的影响，发现对某些任务类型能提升生成忠实度，但对其他任务有负面影响。


<details>
  <summary>Details</summary>
Motivation: DoLa等对比解码方法此前仅应用于仅解码器架构，且主要研究其对事实性的改进。本研究旨在将DoLa适配到T5/FLAN-T5编码器-解码器架构，并评估其对模型指令遵循能力的影响。

Method: 将DoLa对比解码策略适配到T5和FLAN-T5模型家族，通过逐层分析FLAN-T5模型的logit演化来量化DoLa对token输出概率的影响。

Result: DoLa能提升某些类别任务的文本生成忠实度，但对其他任务有损害作用。这是首次在编码器-解码器架构中实现对比解码策略。

Conclusion: DoLa在编码器-解码器架构中的应用效果具有任务依赖性，需要进一步研究其在不同任务类型中的表现差异。
...
TL;DR: 本文提出了一个优化大语言模型在心理学文本分类任务中提示工程的实证框架，发现构造定义、任务框架和示例对性能影响最大，推荐结合人工和自动方法生成多个提示变体并基于训练集性能选择最佳方案。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在文本分类任务中表现良好，但其输出严重依赖提示的措辞。现有研究很少关注心理学等领域的分类任务，这些领域的构造具有精确、理论驱动的定义，可能未在预训练数据中充分体现。需要系统的方法来优化LLM在识别文本中理论构造时的提示工程。

Method: 提出了一个实证框架，实验评估了五种提示策略：代码本引导的经验提示选择、自动提示工程、角色提示、思维链推理和解释性提示，结合零样本和少样本分类。在三个构造和两个模型上进行测试，基于与专家判断的一致性评估性能。

Result: 研究发现角色、思维链和解释并不能完全解决提示措辞不当导致的性能损失。影响提示性能的最重要特征是构造定义、任务框架，以及较小程度的示例。最符合专家判断的分类结果来自结合代码本引导的经验提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员尽可能生成和评估多个提示变体（无论是人工制作、自动生成或两者结合），基于训练数据集中的实证性能选择提示和示例，并在保留集中验证最终方法。这为在专家判断一致性至关重要的场景中优化LLM提示提供了实用、系统和理论驱动的方法。
...
TL;DR: 该研究提出通过微调LLMs遵循医学共识指南进行逐步推理和预测，以解决医疗AI中解释性不足的问题。以Sepsis-3定义为例，展示了小规模微调模型优于大型LLMs的单次学习，并发现早期预测的主要瓶颈是时间序列预测而非分布外泛化。


<details>
  <summary>Details</summary>
Motivation: 当前医疗机器学习虽然预测性能有所突破，但过度关注准确性而忽视了可解释性，这影响了医疗从业者的信任。医学领域普遍存在共识指南，为LLMs学习医学推理规则提供了数据基础。

Method: 将医学共识指南转化为电子健康记录中的语言化规则实例，用于微调LLMs学习共识规则及其例外情况。使用Sepsis-3共识定义作为示例，通过多模态设置将时间序列预测模型的输出表示与LLM集成。

Result: 小规模微调模型在特定医学领域的未见患者数据上，对规则（及例外）的推导正确性接近完美，显著优于使用明确定义提示的大型LLMs单次学习模型。主要瓶颈是稀疏不规则采样的临床变量时间序列预测，多模态集成可改善这一结果。

Conclusion: 通过微调LLMs遵循医学共识指南进行逐步推理，可以同时提高预测准确性和解释可信度。早期预测的关键挑战不是分布外泛化，而是时间序列预测，多模态方法能有效解决这一问题。
...
TL;DR: FusedKV通过融合底层和中层KV缓存来减少长序列下Transformer解码器的内存占用，在保持性能的同时减少50%缓存内存


<details>
  <summary>Details</summary>
Motivation: Transformer解码器在长序列任务中面临KV缓存内存消耗过大的问题，现有的跨层KV缓存共享方法（如YOCO、CLA）性能通常不如层内方法（如GQA），需要找到更有效的缓存压缩方案

Method: 通过分析顶层KV的信息流，发现值主要来自底层，键同时来自底层和中层。基于此提出FusedKV，顶层KV缓存是底层和中层最信息丰富的KV的可学习融合，直接在RoPE后的键上操作以保留位置信息。还提出更高效的FusedKV-Lite，顶层KV缓存直接来自底层值和中层键

Result: 在332M到4B参数的LLM实验中，该方法减少50%缓存内存，同时达到比标准Transformer解码器更低的验证困惑度，成为内存高效的高性能架构替代方案

Conclusion: FusedKV通过智能融合底层和中层KV缓存信息，有效解决了Transformer解码器在长序列下的内存瓶颈问题，在保持性能的同时显著减少内存占用
...
TL;DR: 研究构建了包含巴斯克语标准语、社交媒体和历史文本的多样化语料库，训练了BERnaT系列模型，发现结合标准与多样化数据的模型在所有任务上都表现更好，强调了语言多样性对构建包容性语言模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型依赖经过质量筛选的大规模文本语料库，但这种筛选过程可能无意中排除非标准语言变体，降低模型鲁棒性并强化代表性偏见。研究认为语言模型应该捕捉语言变异的完整谱系（方言、历史、非正式等），而不仅仅依赖标准化文本。

Method: 聚焦于巴斯克语（一种形态丰富且资源匮乏的语言），构建了结合标准语、社交媒体和历史来源的新语料库，预训练了BERnaT系列的三种配置模型：标准配置、多样化配置和组合配置。提出了一个评估框架，将自然语言理解任务分为标准和多样化子集来评估语言泛化能力。

Result: 结果显示，在标准数据和多样化数据上训练的模型始终优于仅在标准语料库上训练的模型，在所有任务类型上都提高了性能，且不影响标准基准测试的准确性。

Conclusion: 这些发现强调了语言多样性在构建包容性、可泛化语言模型中的重要性。语言模型应该整合多样化的语言变体，而不仅仅是标准化文本，以提高模型的鲁棒性和代表性。
...
TL;DR: 该论文介绍了BRAND数据集，用于评估多语言模型在宗教敏感话题上的偏见，发现模型在英语中表现优于孟加拉语，且对伊斯兰教存在系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在偏见检测和分类方面有所进步，但宗教等敏感话题仍然存在挑战，因为即使是微小错误也可能导致严重误解。多语言模型经常误传宗教信息，在宗教语境中难以保持准确性。

Method: 研究者引入了BRAND（双语宗教责任规范数据集），专注于南亚四大宗教：佛教、基督教、印度教和伊斯兰教，包含2400多个条目。使用了三种不同类型的提示，分别在英语和孟加拉语中进行测试。

Result: 结果显示，模型在英语中的表现优于孟加拉语，并且即使回答宗教中立问题时，也持续显示出对伊斯兰教的偏见。这些发现突显了多语言模型在不同语言中询问相似问题时存在的持续偏见。

Conclusion: 研究结果强调了多语言模型在宗教敏感话题上的偏见问题，并将这些发现与人机交互领域关于宗教和灵性的更广泛问题联系起来，表明需要进一步改进模型在跨语言宗教语境中的表现。
...
TL;DR: 该研究采用两阶段方法（持续预训练+监督微调）将Qwen2.5-3B模型适配到藏语，显著降低了困惑度并大幅提升了中藏翻译质量，同时通过层分析揭示了适配机制。
...
Method: 采用两阶段适配策略：1）持续预训练（CPT）建立藏语语言基础；2）监督微调（SFT）进行任务和翻译专业化。对Qwen3-4B的435层进行层分析。

Result: 困惑度从2.98降至1.54；中藏翻译BLEU从0.046提升至0.261，chrF从2.2提升至6.6。分析显示适配主要集中在嵌入层和输出头，中后期MLP投影编码领域特定转换。

Conclusion: CPT构建藏语语义流形，SFT以最小表示扰动实现任务对齐。该研究首次量化探索了藏语LLM适配动态，为低资源语言适配提供了开放可复现框架。
...
TL;DR: 本文提出两种tokenizer适应方法：继续BPE训练用于词汇扩展，以及基于叶子的词汇剪枝用于去除冗余词汇，以提高tokenizer在新领域或语言中的效率和利用率。


<details>
  <summary>Details</summary>
Motivation: 现有的tokenizer适应方法通常在新领域文本上训练新tokenizer，并将不重叠的token添加到现有词汇表中，但这往往导致许多token无法访问或从未使用，需要更有效的词汇扩展和剪枝方法。

Method: 1. 继续BPE训练：通过在新增数据上继续BPE合并学习过程来适应预训练tokenizer；2. 基于叶子的词汇剪枝：移除冗余token同时保持模型质量。

Result: 在多种语言和模型家族上的实验表明，继续BPE训练提高了tokenization效率，并导致更好的新增词汇利用率。词汇剪枝方法能有效移除冗余token。
...
<details>
  <summary>Details</summary>
Motivation: 随着增强型大语言模型在Web应用中的普及，提升推理服务效率、优化服务级别目标对用户体验至关重要。现有系统面临两大挑战：1）先到先服务调度导致严重的队头阻塞，使排队延迟超出SLO；2）静态批令牌限制无法适应负载和硬件条件波动，两者都降低了有效吞吐量和服务质量。

Method: AugServe采用两阶段自适应请求调度策略：第一阶段结合增强LLM请求的推理特征优化调度决策顺序；第二阶段利用运行时信息持续优化决策，适应请求特性和系统能力。此外，AugServe还基于硬件状态和实时负载动态调整令牌批处理机制。
...
TL;DR: SkillFactory是一种在强化学习前通过监督微调让模型学习认知技能的方法，使用模型自身生成的"银牌"训练数据，帮助模型在RL后更好地泛化到更困难任务
...
Method: SkillFactory方法：在强化学习前进行监督微调，使用模型自身生成的样本重新排列成技能格式的训练数据（"银牌"SFT轨迹）。这些轨迹可能不完美，但能有效引导模型在RL阶段获得认知技能

Result: 1. 从SkillFactory SFT初始化开始，尽管RL前性能较低，但能帮助模型在RL后泛化到更困难的任务变体；2. 模型确实使用了认知技能；3. 经过RL的SkillFactory模型在域外任务上比经过RL的基础模型更稳健

Conclusion: 在强化学习前学习的归纳偏置有助于模型学习稳健的认知技能使用，SkillFactory提供了一种不依赖蒸馏的方法来培养模型的高级推理能力
...
TL;DR: 论文提出AI对齐应重新定义为通过过程导向、多智能体、发展性机制构建具有熵减性和理由响应能力的智能体，而非编码固定的人类价值观内容。


<details>
  <summary>Details</summary>
Motivation: 传统基于内容的价值规范方法存在"规范陷阱"问题，由于事实-价值鸿沟、价值多元化和扩展框架问题的结合，导致结构不稳定。需要新的理论框架来理解多智能体对齐动态。

Method: 提出三个哲学贡献：1) 阐述"规范陷阱"论证；2) 提出"熵减"作为信息论框架理解多智能体对齐动态；3) 基于相容论指导控制理论建立真实与模拟道德能力的功能区分，配合具身实验范式和验证机制。

Result: 该框架生成了关于人工系统中价值涌现和道德能动性的具体、可证伪预测，但实证验证仍在进行中，是更广泛研究计划的哲学组成部分。

Conclusion: AI对齐应重新概念化为通过过程导向、多智能体、发展性机制构建具有熵减性和理由响应能力的智能体，这为理解人工系统中的价值涌现和道德能力提供了新的理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI范式作为"体验架构师"面临可解释性和价值对齐的根本挑战，需要建立一种基于第一原则的认知架构来解决这些问题，为实现可信赖的通用人工智能提供可行路径。

Method: 提出"权重计算主义"认知架构，将认知解构为不可分割的逻辑原子和两个基本操作：指向和比较。通过可解释的权重计算模型（权重=收益×概率）形式化决策，所有值都可追溯到可审计的初始权重集。采用基于图算法的计算引擎和全局工作空间工作流实现。

Result: 该架构在未见过的新场景中实现了透明、类人的推理和鲁棒学习，为构建可信赖且价值对齐的AGI奠定了实践和理论基础。

Conclusion: 权重计算主义为AGI提供了一条可行的路径，通过原子分解实现了根本的可解释性、对新情境的内在通用性以及可追溯的价值对齐，为解决当前AI的核心挑战提供了新的理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成长链思维推理链在复杂推理任务上表现良好，但这种方法可能导致大量token开销，特别是当模型"过度思考"产生冗长推理链时，甚至会导致错误答案。符号求解器集成方法利用LLMs的代码生成能力将推理任务转化为可执行代码，然后用符号求解器解决，但何时这种传统长链思维能被符号求解器增强仍是一个开放问题。

Method: 采用符号求解器集成方法，利用LLMs的代码生成能力将推理任务翻译成可执行代码，然后使用符号求解器解决。通过实验比较传统长链思维方法和符号求解器集成方法在不同类型推理问题上的表现。

Result: 实验结果显示：1) 符号求解器集成方法仅在问题需要有限隐式推理但涉及充足搜索空间时有效；2) 最新LLMs如GPT-4o在浅推理深度的演绎问题上表现更好；3) 符号求解器集成方法显著提高了LLMs在需要重复回溯的约束满足问题上的性能；4) 当提供声明性示例时，即使是CodeLlama-13B也能在困难的斑马谜题上超越GPT-4o。

Conclusion: 符号求解器集成方法对传统长链思维推理的增强效果具有特定条件：主要适用于需要有限隐式推理但涉及大量搜索空间的问题，特别是约束满足问题。最新LLMs在浅层演绎推理上表现优异，而符号求解器在需要系统搜索的问题上能显著提升性能。提供适当的声明性示例可以弥补较小模型在复杂推理任务上的不足。
...
TL;DR: 本文研究了主动推理中偏好分布的四种定义方式对智能体性能的影响，通过网格世界导航任务比较了硬目标、软目标以及是否包含目标塑造的差异。


<details>
  <summary>Details</summary>
Motivation: 主动推理使用期望自由能作为规划决策目标，但文献中很少关注偏好分布应如何指定以及不同指定方式如何影响智能体的推理和学习性能。

Method: 考虑了四种定义偏好分布的方式：硬目标与软目标、包含或不包含目标塑造（中间目标）。在网格世界导航任务中比较了四种智能体的性能。

Result: 目标塑造能带来最佳整体性能（促进利用），但会牺牲对环境转移动态的学习（阻碍探索）。

Conclusion: 偏好分布的定义方式对主动推理智能体的性能有显著影响，目标塑造在提升利用性能的同时会削弱探索能力，需要在设计时权衡这一平衡。
...
TL;DR: 该论文提出了一种评估LLM智能体在零样本、混合动机环境中合作能力的方法，使用Concordia自然语言多智能体模拟环境，通过NeurIPS 2024竞赛发现现有智能体在合作泛化能力上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在社会互动方面展现出强大能力，并越来越多地部署在与人类和人工智能体交互的场景中。然而，现有评估方法无法衡量这些能力如何泛化到新的社会情境，特别是在混合动机环境中实现可靠合作的能力。

Method: 引入基于Concordia自然语言多智能体模拟环境的评估方法，通过测试智能体在不同伙伴和情境中识别和利用互利机会的能力，来衡量一般合作智能。该方法在NeurIPS 2024 Concordia竞赛中进行了实证验证。

Result: 研究发现当前智能体能力与实现可靠合作所需的稳健泛化之间存在显著差距，特别是在需要说服和规范执行的场景中。竞赛结果显示智能体在谈判和集体行动问题等多样化情境中实现互利的能力有限。

Conclusion: LLM智能体在零样本混合动机环境中的合作能力评估揭示了重要的性能差距，特别是在复杂社会互动场景中。这为未来改进LLM智能体的合作泛化能力提供了方向，强调了在说服和规范执行等关键社会技能方面需要进一步研究。
...
TL;DR: 提出了一种通信受限的多智能体强化学习框架，通过区分有损和无损消息，将通信影响量化为全局奖励，提升在复杂动态环境中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多智能体系统普遍存在有损通信问题，现有通信方法由于可扩展性和鲁棒性有限，难以应用于复杂动态环境。
...
Conclusion: 提出的通信约束多智能体强化学习框架能够有效处理现实世界中的有损通信问题，提升系统在复杂动态环境中的性能和鲁棒性。
...
Conclusion: PAN编程模型和EnCompass框架为LLM智能体开发提供了更灵活、可实验的编程范式，显著提高了开发效率和系统可靠性
...
<details>
  <summary>Details</summary>
Motivation: 解决现有理论模型与现实经济复杂性之间的系统错配问题，具体包括：1）非结构化文本数据模态不匹配阻碍准确客户画像；2）动态特征纠缠挑战非线性价格弹性和时变属性建模；3）多层级业务约束导致的操作不可行性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的自动启发式设计研究通常只考虑单一角色，限制了启发式的多样性和质量。需要一种多角色协作的系统来提升自动启发式设计的性能。

Method: 提出RoCo多智能体角色协作系统，包含四个专门化的LLM智能体：探索者（创造性、多样性驱动）、利用者（保守性、效率导向）、批评者（评估效果并提供反馈）、整合者（平衡创新与利用）。这些智能体通过结构化多轮流程进行交互，包括反馈、精炼和精英突变。

Result: 在五个不同的组合优化问题上，无论是白盒还是黑盒设置下，RoCo都表现出优越性能，生成的启发式方法超越了包括ReEvo和HSEvo在内的现有方法。

Conclusion: 基于角色的协作范式为鲁棒且高性能的自动启发式设计建立了新标准，证明了多智能体协作在提升启发式设计多样性和质量方面的有效性。
...
TL;DR: 提出Omni-AutoThink自适应推理框架，根据任务难度动态调整模型推理深度，提升多模态感知与生成模型的推理能力


<details>
  <summary>Details</summary>
Motivation: 现有Omni模型在推理行为上存在僵化问题，要么对简单问题过度推理，要么在需要推理时无法有效推理，需要解决这一局限性

Method: 采用两阶段框架：1) 自适应监督微调阶段，使用大规模推理增强数据赋予模型基础推理能力；2) 自适应强化学习阶段，基于任务复杂度和奖励反馈优化推理行为

Result: 实验结果表明，相比先前基线方法，提出的框架显著提升了自适应推理性能

Conclusion: Omni-AutoThink框架有效解决了现有系统推理行为僵化问题，通过自适应调整推理深度提升了多模态推理能力，并构建了全面的自适应推理基准数据集
...
TL;DR: 该研究为基于大语言模型的工业自动化智能体开发了一个标准化基准测试框架，包含可执行的仿真环境和五个复杂度类别的Blocksworld问题，通过MCP协议实现不同智能体架构的无缝连接和系统评估。


<details>
  <summary>Details</summary>
Motivation: 工业自动化需要能够适应变化任务和环境的灵活控制策略，基于大语言模型的智能体具有自适应规划和执行的潜力，但缺乏标准化的基准测试来进行系统比较。

Method: 引入包含可执行仿真环境的基准测试框架，采用Blocksworld问题并分为五个复杂度类别，集成模型上下文协议（MCP）作为标准化工具接口，使不同智能体架构无需特定实现修改即可连接和评估。

Result: 开发了一个单智能体实现来展示基准测试的适用性，建立了用于比较基于大语言模型的规划和执行方法的定量指标。

Conclusion: 该基准测试框架为系统评估和比较基于大语言模型的工业自动化智能体提供了标准化平台，通过MCP协议实现了架构无关的评估，为未来研究建立了定量比较基础。
...
### [132] [Strengthening Han's Fourier Entropy-Influence Inequality via an Information-Theoretic Proof](https://arxiv.org/abs/2512.03117)
...
<details>
  <summary>Details</summary>
Motivation: Han的傅里叶熵-影响不等式最初针对{-1,1}值布尔函数证明了C₁=3+2ln2和C₂=1的常数。本文旨在通过信息论方法证明该不等式实际上对所有单位L²范数的实值布尔函数都成立，且常数可以达到最优值C₁=C₂=1。

Method: 采用简短的信息论证明方法，通过分析Shannon熵和影响之间的关系，建立了傅里叶熵-影响不等式的改进版本。

Result: 成功证明了傅里叶熵-影响不等式对所有单位L²范数的实值布尔函数都成立，且常数达到最优值C₁=C₂=1，比原始结果中的常数更优。

Conclusion: 该不等式可以作为Shannon熵和影响的基本结构性质，常数C₁=C₂=1是最优的，适用于更广泛的实值布尔函数类。
...
### [133] [Multi-Source M/G/1/1 Queues with Probabilistic Preemption](https://arxiv.org/abs/2512.03241)
*Mohammad Moltafet,Hamid R. Sadjadpour,Zouheir Rezki,Marian Codreanu,Roy D. Yates*

Main category: cs.IT

TL;DR: 研究多源状态更新系统中的信息年龄优化，提出概率抢占式数据包管理策略，推导AoI和PAoI的矩生成函数


<details>
  <summary>Details</summary>
Motivation: 在多源状态更新系统中，如何有效管理数据包以减少信息年龄(AoI)是一个重要问题。现有系统通常采用先到先服务或完全抢占策略，但缺乏灵活的概率控制机制。

Method: 提出概率抢占式数据包管理策略，在多源M/G/1/1排队系统中，当来自同一源的新数据包到达时，以固定概率替换系统中已有的同源数据包。推导了每个源的信息年龄(AoI)和峰值信息年龄(PAoI)的矩生成函数。

Result: 数值结果表明，所提出的概率抢占式数据包管理策略能有效降低信息年龄，相比传统策略具有更好的性能表现。

Conclusion: 概率抢占式数据包管理策略为多源状态更新系统提供了一种灵活有效的信息年龄优化方法，通过概率控制实现了系统性能的平衡与优化。
...
### [134] [Generalized Orthogonal Approximate Message-Passing for Sublinear Sparsity](https://arxiv.org/abs/2512.03326)
*Keigo Takeuchi*

Main category: cs.IT

TL;DR: 本文针对亚线性稀疏信号从广义线性测量中的重构问题，提出了广义正交近似消息传递算法，解决了传统AMP在非高斯测量矩阵下的收敛问题。


<details>
  <summary>Details</summary>
Motivation: 传统研究假设信号稀疏度与信号维度成正比，而本文考虑亚线性稀疏度（稀疏度随信号维度增长但速度慢于线性）。传统AMP算法在非标准高斯测量矩阵下收敛性差，需要新的算法来解决亚线性稀疏信号的重构问题。

Method: 提出广义正交AMP算法，核心是通过状态演化设计Onsager修正项，确保估计误差的渐近高斯性。算法适用于正交不变测量矩阵，在亚线性稀疏极限下（信号稀疏度和测量维度以亚线性速度趋于无穷）进行分析。

Result: 当非零信号支撑不包含原点邻域时，GOAMP使用贝叶斯去噪器在线性测量下能够实现无误差信号重构，当且仅当测量维度大于某个阈值（该阈值与标准高斯测量矩阵下AMP的阈值相同）。数值模拟在线性测量和1位压缩感知中验证了算法性能，特别是在病态测量矩阵下，GOAMP优于现有重构算法。

Conclusion: GOAMP算法有效解决了亚线性稀疏信号在广义线性测量下的重构问题，特别是在非高斯测量矩阵下表现出优越性能，为稀疏信号处理提供了新的理论框架和实用算法。
...
### [135] [Expected Confidence Dependency: A Novel Rough Set-Based Approach to Feature Selection](https://arxiv.org/abs/2512.03612)
*Saeed Rasouli,Hamid Karamikabir*

Main category: cs.IT

TL;DR: 提出了一种新的粗糙集特征选择依赖度量方法ECD，基于置信度贡献和归一化期望算子，相比传统二元表征方法具有更好的软计算特性。


<details>
  <summary>Details</summary>
Motivation: 传统粗糙集依赖度量方法基于条件块的二元表征，存在局限性。需要一种更灵活、基于软计算的精度驱动依赖度量方法，能够更好地处理特征选择问题。

Method: 提出期望置信依赖度（ECD），为单个等价块分配基于置信度的贡献，并通过归一化期望算子进行聚合。该方法在粗糙集理论框架内工作，具有软计算导向和精度驱动的特点。

Result: 正式建立了ECD的几个理想性质：归一化性、与经典依赖度的兼容性、单调性以及在结构和标签保持变换下的不变性。

Conclusion: ECD是一种新颖的软计算导向、精度驱动的依赖度量方法，克服了传统粗糙集依赖度量的局限性，为特征选择提供了更灵活有效的工具。
...
### [136] [Movable Signals with Dual-Polarized Fixed Intelligent Surfaces: Beyond Diagonal Reflection Matrices](https://arxiv.org/abs/2512.03872)
*Matteo Nerini,Bruno Clerckx*

Main category: cs.IT

TL;DR: 本文研究了双极化智能表面辅助的无线系统，比较了可重构智能表面（RIS）和固定智能表面（FIS）配合可移动信号的性能。RIS调整反射矩阵，FIS调整信号频率而表面特性固定。两种表面都有对角反射矩阵和非对角反射矩阵两种类型。研究发现FIS配合可移动信号始终优于RIS，至少获得四倍增益。当发射机和接收机极化不同时，非对角FIS能进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究双极化智能表面辅助无线系统的性能比较，探索不同智能表面配置（RIS vs FIS）以及不同反射矩阵结构（对角 vs 非对角）对系统性能的影响，为实际无线系统设计提供理论指导。

Method: 比较两种智能表面配置：1）可重构智能表面（RIS），通过调整反射矩阵来优化性能；2）固定智能表面（FIS），表面特性固定但通过调整信号频率来优化。对每种配置，研究两种反射矩阵结构：对角反射矩阵（对角RIS/FIS）和非对角反射矩阵（非对角RIS/FIS）。分析在不同极化条件下的性能差异。

Result: 可移动信号配合固定智能表面（FIS）始终优于可重构智能表面（RIS），至少获得四倍的性能增益。当发射机和接收机极化不同时，非对角FIS能进一步提升系统性能。这表明在双极化无线系统中，FIS配合频率调整策略比RIS的反射矩阵调整策略更有效。

Conclusion: 在双极化智能表面辅助的无线系统中，固定智能表面配合可移动信号比可重构智能表面具有显著性能优势。特别是在发射机和接收机极化不同的场景下，采用非对角反射矩阵的FIS能获得最佳性能。这为未来无线系统设计提供了重要参考，建议优先考虑FIS方案。
...
### [137] [On topological and algebraic structures of categorical random variables](https://arxiv.org/abs/2512.04020)
*Inocencio Ortiz,Santiago Gómez-Guerrero,Christian E. Schaerer*

Main category: cs.IT

TL;DR: 该论文基于熵和对称不确定性定义了一个度量，用于分类随机变量，并将其推广到适当的商空间，同时在该商空间中发现了自然的交换幺半群结构。


<details>
  <summary>Details</summary>
Motivation: 研究分类随机变量之间的度量关系，探索在商空间中的代数结构及其与拓扑结构的兼容性。

Method: 基于熵和对称不确定性定义度量，构造商空间，并证明该商空间上存在自然的交换幺半群结构。

Result: 成功定义了分类随机变量的度量，将其推广到商空间，并证明了该商空间具有与度量诱导的拓扑兼容的连续交换幺半群结构。

Conclusion: 该研究为分类随机变量建立了度量框架，并在商空间中发现了代数结构与拓扑结构的自然统一，为相关理论研究提供了新视角。
