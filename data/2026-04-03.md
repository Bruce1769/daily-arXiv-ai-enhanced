...
TL;DR: DOne框架通过解耦结构理解和元素渲染，解决了视觉语言模型在设计到代码生成中的整体瓶颈问题，显著提升了布局准确性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在设计到代码生成中存在"整体瓶颈"问题，难以同时处理高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符。
...
Conclusion: DOne通过解耦结构理解和元素渲染的方法，有效解决了设计到代码生成中的瓶颈问题，显著提升了生成代码的准确性和视觉质量。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进预训练多模态大语言模型使用多模态证据的框架，通过注意力模式识别相关视觉区域和文本元素，并通过提示标记重新关注相关证据来提升知识密集型视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答需要结合视觉理解和外部知识的图像问题时，往往难以识别最相关的视觉和文本证据。当处理知识密集型查询时，模型需要整合视觉线索与检索到的文本证据（这些证据通常有噪声或仅部分相关），同时还需要定位图像中的细粒度视觉信息。

Method: Look Twice (LoT)是一个无需训练的推理时框架，利用模型的注意力模式来估计哪些视觉区域和检索到的文本元素与查询相关，然后基于这些高亮证据生成答案。通过轻量级的提示级标记来突出显示选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型，LoT框架显示出持续的性能改进。在视觉中心和幻觉导向的基准测试中的额外评估进一步表明，即使在没有文本上下文的情况下，仅视觉证据高亮也能提高模型性能，所有这些改进都不需要额外的训练或架构修改。

Conclusion: Look Twice (LoT)框架通过利用注意力模式识别和重新关注相关多模态证据，有效提升了预训练多模态大语言模型在知识密集型视觉问答任务中的表现，且无需额外训练或模型修改，具有实用性和可扩展性。
...
TL;DR: MedQwen：一种参数高效的医学视觉语言模型，通过频谱路由的专家混合架构和理论支撑的缩放规则，解决医学影像中的跨数据集干扰和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准测试中表现出色，但在医学影像领域缺乏鲁棒性。医学数据的异质性监督会导致跨数据集干扰，且对数据机制敏感。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，采用参数高效的频谱路由专家混合架构。从预训练权重的非重叠奇异值分解片段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。使用理论支撑的缩放规则将低秩更新与全秩完全微调的MoE对齐，不改变基础架构。

Result: 在23个医学数据集上（涵盖视觉问答、报告生成、放射学分类和幻觉缓解）表现优异：零样本分类接近完全微调性能，但可训练参数减少339倍；将顺序遗忘降低至约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过参数高效的专家混合架构有效解决了医学视觉语言模型中的跨数据集干扰和灾难性遗忘问题，在保持高性能的同时显著减少了可训练参数，适用于临床工作流中的顺序学习场景。
...
<details>
  <summary>Details</summary>
Motivation: 在接触性运动中早期识别危险动作能够及时干预并提高运动员安全。美式足球中的擒抱技术不当是导致伤害的主要原因，需要开发可靠的自动检测方法来辅助教练进行伤害预防。

Method: 提出了基于视觉Transformer的视频分析模型，采用不平衡感知训练方法。使用扩展的数据集包含733个单运动员-假人擒抱视频片段，每个片段都标注了标准化擒抱技术评估(SATT-3)的撞击区域组件。

Result: 在交叉验证中获得了0.67的危险召回率和0.59的危险F1分数。相比之前较小数据集上的基线方法（危险召回率0.58，危险F1分数0.56），在更大数据集上危险召回率提升了超过8个百分点。
...
TL;DR: 通过合成蹦床姿势数据集微调姿态估计模型，显著提升极端姿势下的性能表现
...
Result: 在2D姿态估计上获得最先进结果，缩小了常见姿势与极端姿势之间的性能差距；在3D三角测量中，MPJPE减少12.5毫米，相比预训练ViTPose模型提升19.6%

Conclusion: 使用合成蹦床姿势数据集微调姿态估计模型能有效解决极端姿势下的性能问题，显著提升蹦床体操等特殊场景的姿态估计精度
...
TL;DR: 提出基于自助法的注意力正则化方法，通过统计显著性检验去除ViT注意力图中的噪声，提高解释性


<details>
  <summary>Details</summary>
Motivation: Vision Transformer的注意力分数通常非零且分散，导致注意力图噪声大、解释性差。需要量化注意力分数的不确定性，获得正则化的注意力分数。

Method: 将ViT注意力分数置于统计框架中，引入自助法（bootstrapping）通过重采样输入特征生成注意力分数的基线分布，估计注意力分数的显著性和后验概率，实现注意力正则化。

Result: 在自然图像和医学图像上，该方法能有效去除噪声引起的虚假注意力，显著提高注意力图的稀疏性和收缩性。通过模拟和真实数据集进行了定量评估。

Conclusion: 自助法可作为实用的正则化工具，提高ViT注意力分数作为解释的可信度。该方法能有效识别和去除注意力图中的噪声，增强模型解释性。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在挑战：缺乏对器械在组织表面交互位置的预测能力，以及缺少强制执行工具-动作特定安全交互区域的显式条件输入。

Method: 结合了时间视觉编码器（捕捉多视角下的工具运动和组织动态）、语言条件化（实现跨不同器械-动作对的泛化）和DiT风格解码器（用于密集可操作性预测）的多模态框架。

Result: 在包含15,638个视频片段、103个胆囊切除术程序、涵盖6个独特工具-动作对的基准测试中，相比视觉语言模型基线有显著改进（20.6像素ASSD vs. 60.2像素的Molmo-VLM）。

Conclusion: 通过预测工具-动作特定的组织可操作性区域，AffordTissue为安全的手术自动化提供了显式的空间推理，可能解锁针对适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE是一个无需训练的管道，用于在美式足球训练视频中定位首次接触点（FPOC），无需标注的接触示例，通过目标检测、运动感知时间推理和像素级接触验证实现。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量视频，但关键交互只占整个长视频的短暂窗口。可靠的生物力学分析需要时空定位来识别交互实体和接触开始时间。现有方法面临相机运动、杂乱场景、多个相似装备运动员以及冲击时快速姿态变化等挑战。

Method: GRAZE采用无训练管道：1) 使用Grounding DINO发现候选的球员-假人交互；2) 通过运动感知时间推理进行细化；3) 使用SAM2作为显式的像素级接触验证器，不依赖检测置信度。候选发现和接触确认的分离使方法对杂乱场景和冲击附近的不稳定定位具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的所有视频中将FPOC定位在±10帧内，在82.7%的所有视频中定位在±20帧内。

Conclusion: 结果表明，在真实世界训练视频中进行帧级准确的接触开始定位是可行的，无需特定任务的训练。该方法为生物力学分析提供了可靠的基础。
...
TL;DR: 提出基于稀疏体素栅格化的新框架，解决3D高斯溅射在开放词汇3D场景理解中的空间模糊性和多级语义模糊性问题，在细粒度查询任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯溅射的开放词汇3D场景理解方法存在两个关键限制：1) 非结构化重叠高斯分布导致的空间模糊性，需要概率特征注册；2) 基于对象级掩码池化特征导致的多级语义模糊性，稀释了细粒度细节。

Method: 采用稀疏体素栅格化作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信感知的特征注册过程，并利用基础模型AM-RADIO的密集对齐特性解决多级模糊性，避免分层训练的计算开销。

Result: 在开放词汇3D对象检索和点云理解基准测试中达到最先进性能，特别是在注册方法通常失败的细粒度查询任务上表现出色。

Conclusion: 稀疏体素栅格化框架有效解决了3D高斯溅射方法中的空间和语义模糊性问题，为开放词汇3D场景理解提供了更稳定、精确的解决方案，在细粒度任务上具有显著优势。
...
Conclusion: 研究结果突显了基于流的生成建模在可扩展和物理基础的第一人称运动理解方面的潜力，能够在不依赖后处理过滤或额外监督的情况下生成连贯可控的运动。
...
<details>
  <summary>Details</summary>
Motivation: 无标记姿态估计技术使得使用标准视频捕捉自然场景下的详细人体运动成为可能，但姿态数据的高维度、噪声和时间复杂性给提取有意义的协调和行为变化模式带来了挑战。

Method: 开发了一个通用分析流程，结合原则性预处理、降维和基于递归的时间序列分析，量化运动动态的时间结构。通过三个案例研究展示流程的灵活性。

Result: 该流程能够适应不同实验环境，从面部和全身运动、2D和3D数据、个体与多智能体行为中提取理论上有意义的见解。

Conclusion: 相同的分析工作流程可以适应性地从复杂姿态时间序列中提取理论上有意义的见解，为大规模行为分析提供了通用工具。
...
TL;DR: 该论文提出了一种针对多模态大语言模型视频理解的组合一致性审计方法，通过结构化奖励替代传统句子级监督，提升模型的视觉和时序基础能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解方面取得了显著进展，但其输出往往存在视觉和时序基础不足的问题：模型可能虚构对象存在、分配错误属性或在重复事件中产生错误，同时仍能生成看似合理的描述或答案。传统句子级监督无法有效检测这些基础问题。

Method: 提出组合一致性审计方法，将视频描述分解为事实性和时序性主张，检查高层预测是否有有效的低层证据支持。使用结构化奖励替代传统句子级奖励，包括：(1) 实例感知的场景图奖励用于对象、属性和关系；(2) 时序奖励用于事件排序和重复；(3) 基于视频的VQA奖励用于层次化自验证。

Result: 在时序、通用视频理解和幻觉导向的基准测试中，该方法在开源骨干模型上取得了一致的性能提升，表明结构化奖励塑造是实现更忠实视频理解的有效途径。

Conclusion: 结构化奖励塑造是提升多模态大语言模型视频理解忠实性的实用方法，能够有效解决传统句子级监督在视觉和时序基础方面的不足。
...
TL;DR: UniRecGen统一框架整合稀疏视图3D建模中的重建与生成范式，通过共享规范空间对齐和分离式协作学习，实现重建保真度与生成合理性的平衡。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的根本矛盾：前馈重建方法效率高且输入对齐好，但缺乏全局先验导致结构不完整；而基于扩散的生成方法能提供丰富几何细节，但多视图一致性差。

Method: 提出UniRecGen统一框架，将两种范式整合到单一协作系统中。通过共享规范空间对齐坐标空间、3D表示和训练目标，采用分离式协作学习保持训练稳定性并在推理时实现无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在保真度和鲁棒性方面表现优异，在从稀疏观测创建完整且一致的3D模型方面超越了现有方法。

Conclusion: UniRecGen成功解决了稀疏视图3D建模中重建与生成之间的冲突，通过统一框架实现了更好的重建保真度和生成合理性平衡，为从稀疏观测创建高质量3D模型提供了有效解决方案。
...
TL;DR: 提出TAG热成像框架，通过非参数纹理恢复解决材料非均匀性问题，实现高保真夜视成像，首次恢复幽灵人脸表情并超越现有HADAR技术


<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受到鬼影效应的根本性限制。虽然传统方法依赖后处理，但HADAR技术为超光谱计算热成像开辟了新前景。然而，现有HADAR仅适用于材料均匀的有限场景，而材料非均匀性在现实世界中普遍存在，因此需要通用抗鬼影成像解决方案

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用超光谱光子流进行非参数纹理恢复，解决材料非均匀性问题。该方法能够恢复幽灵人脸的详细纹理和表情，实现热3D拓扑对齐和情绪检测

Result: TAG在各种场景中普遍优于HADAR技术，首次实现了幽灵人脸表情的恢复，揭示了材料非均匀性的影响，阐明了HADAR的有效性边界。实验展示了跨昼夜的面部纹理和表情恢复，以及首次实现的热3D拓扑对齐和情绪检测

Conclusion: TAG为高保真计算夜视建立了通用基础，在自主导航、侦察、医疗保健和野生动物监测等领域具有潜在应用价值。该工作解决了材料非均匀性问题，克服了鬼影效应，推动了夜视技术的发展
...
Result: 在两个无人机数据集上表现优异：UAVid数据集达到71.86% mIoU，UDD6数据集达到80.92% mIoU，在保持计算效率的同时获得有竞争力的性能

Conclusion: PBSeg为无人机图像语义分割提供了一种高效解决方案，在计算效率和分割质量之间取得了良好平衡，适用于边缘设备部署
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在动态区域初始化不完整，导致重建和运动估计不稳定，通常需要依赖预计算的光流等外部密集运动指导，这会引入额外的复杂性和错误传播。
...
TL;DR: VideoZeroBench：一个用于长视频问答的分层基准测试，通过验证时空证据来严格评估视频多模态大语言模型的真实推理能力，揭示当前模型在基于证据的推理方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 高分可能掩盖细粒度视觉理解和推理的缺陷；(2) 答案正确性评估通常不验证模型是否识别了支持其预测的精确时空证据。需要建立一个能够严格验证证据的基准测试。

Method: 提出VideoZeroBench分层基准测试，包含500个手动标注的问题，涵盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro的正确回答率也低于17%。当施加定位约束时，性能急剧下降：在同时要求正确答案和精确时空定位时（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。

Conclusion: 研究结果揭示了表面答案正确性与真实基于证据的推理之间存在显著差距，表明基于证据的视频理解仍然是长视频QA的瓶颈。基准测试为未来基于证据的视频推理研究提供了重要见解。
...
Method: 采用交替单模态学习和共享分类器来解耦多模态梯度，设计基于不确定性的跨模态梯度手术来选择性对齐跨模态梯度，引导共享参数对所有模态都有利
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限的实际部署中存在局限性。需要开发不依赖卫星图像的训练方法。

Method: 三阶段框架：1) 使用3D高斯溅射从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像，并用轻量级几何引导修复完善纹理；3) 从生成的正射影像提取DINOv3特征，仅使用无人机数据学习Fisher向量聚合模型，在测试时重用该模型编码卫星瓦片进行跨视图检索。

Result: 在University-1652和SUES-200数据集上的实验结果表明，SFT框架显著优于无卫星泛化基线方法，缩小了与使用卫星图像训练的方法之间的性能差距。

Conclusion: 提出的卫星图像无关训练框架能够在不依赖卫星数据的情况下，通过无人机多视角序列有效生成跨视图兼容的地理定位表示，为实际部署提供了可行的解决方案。
...
TL;DR: SHOE是一个新的开放词汇人机交互检测评估框架，通过语义相似度而非精确字符串匹配来评估预测质量，使用LLM计算动词和对象相似度，比传统mAP指标更符合人类判断。


<details>
  <summary>Details</summary>
Motivation: 传统HOI检测评估指标（如mAP）将交互类别视为离散分类标签，无法评估语义正确但词汇不同的预测（如"靠在沙发上" vs "坐在沙发上"），限制了开放词汇预测的评估能力。

Method: SHOE将HOI预测分解为动词和对象组件，使用多个大语言模型（LLMs）的平均值估计它们的语义相似度，然后将这些相似度组合成一个综合评分，实现超越精确字符串匹配的评估。
...
TL;DR: ROSETTA方法通过角度损失和特征范数损失解决开放集测试时适应中ID分类与OOD检测的权衡问题，在多个数据集上实现强OOD检测同时保持高ID分类性能。


<details>
  <summary>Details</summary>
Motivation: 开放集测试时适应中，协变量偏移会影响ID样本，同时存在OOD样本。熵最小化用于保持ID性能，熵最大化用于增强OOD检测，但两者存在内在冲突，导致csID分类和csOOD检测之间的权衡问题。
...
Conclusion: ROSETTA通过角度损失和特征范数损失有效解决了开放集测试时适应中ID分类与OOD检测的权衡问题，在各种数据集和实际应用中表现出色。
...
Method: 首先通过注册多个客户端的局部合并LiDAR点云构建共享几何骨架来初始化全局3DGS模型。联邦优化期间固定高斯位置以保持几何对齐，每个客户端只更新外观相关属性（协方差、不透明度、球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权贡献。

Result: 实验表明F3DGS在实现与集中训练相当的重建质量的同时，支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集用于评估。

Conclusion: F3DGS解决了多智能体探索中的部分可观测性挑战，实现了去中心化的高质量3D重建，数据集、开发工具包和源代码将公开发布。
...
TL;DR: Tex3D框架首次实现VLA系统中3D对抗纹理的端到端优化，通过前景-背景解耦和轨迹感知优化，在仿真和真实机器人实验中使VLA任务失败率高达96.7%
...
Conclusion: Tex3D暴露了VLA系统对物理基础3D对抗攻击的关键脆弱性，强调了鲁棒性感知训练的必要性。这是首个在VLA模拟环境中端到端优化3D对抗纹理的框架。
...
TL;DR: 利用稀疏自编码器从基础模型特征中提取单语义神经元，构建昆虫形态特征自动标注管道，创建包含8万标注的Bioscan-Traits数据集


<details>
  <summary>Details</summary>
Motivation: 形态特征是理解生物与环境相互作用的关键，但传统提取方法依赖专家标注，成本高、效率低，限制了大规模生态研究。主要瓶颈在于缺乏连接生物图像与特征标注的高质量数据集。

Method: 1) 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；2) 利用这些神经元定位显著区域；3) 结合视觉-语言提示生成可解释的特征描述；4) 构建Bioscan-Traits数据集（19K昆虫图像，80K标注）；5) 通过消融研究评估设计敏感性。

Result: 1) 创建了Bioscan-Traits数据集，包含80,000个特征标注，覆盖19,000张昆虫图像；2) 人类评估确认生成的形态描述具有生物学合理性；3) 消融研究系统评估了关键设计选择对标注质量的影响。

Conclusion: 通过模块化管道而非昂贵的人工标注，提供了可扩展的方法来为基础模型注入生物学有意义的监督，支持大规模形态分析，弥合生态相关性与机器学习实用性之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D场景生成方法主要关注静态几何重建，而忽略了云、水、烟雾等场景尺度的环境动态。建模这些动态具有挑战性，因为运动需要在扩展场景中保持一致性，同时支持低延迟的用户反馈。

Method: 采用渐进构建全局一致运动场的方法，引入几何感知对齐模块解决视图间的方向和尺度模糊性，使用紧凑的基于哈希的运动场表示实现高效查询和动态稳定传播，支持渲染时的双向运动传播。

Result: 在单个RTX 5090 GPU上，每个新场景扩展步骤需要9秒生成，随后3秒进行运动对齐和运动场更新，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成具有连贯环境动态的4D世界的挑战，实现了交互式生成和长时序一致的4D序列，无需依赖昂贵的视频精炼方法。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但在真实场景中由于3D标注成本高，通常只有部分目标被标注。稀疏标注设置下现有方法性能下降，需要解决稀疏监督下的检测问题。
...
Conclusion: 通过结合几何保持增强和原型引导的伪标签生成，实现了稀疏监督下的鲁棒3D目标检测，为解决真实场景中标注稀疏问题提供了有效方案。
...
TL;DR: 该论文提出了一种基于莫尔效应的物理认证签名方法，用于区分真实拍摄视频与AI生成视频。通过分析相机拍摄双层光栅结构时产生的莫尔条纹运动不变量，能够可靠检测视频的真实性。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分。需要一种基于物理原理的认证方法来可靠区分真实拍摄视频与AI生成视频。

Method: 利用莫尔效应：当相机观察紧凑的双层光栅结构时会产生干涉条纹。推导出莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观察距离和光栅结构无关。验证器从视频中提取这两个信号并测试它们的相关性。

Result: 在多个最先进的AI生成器产生的真实拍摄视频和AI生成视频上验证了该不变量，发现真实视频和AI生成视频产生显著不同的相关签名，表明这是一种鲁棒的区分方法。

Conclusion: 确定性光学现象可以作为物理基础的、可验证的签名来对抗AI生成视频，为视频真实性认证提供了新的物理基础方法。
...
TL;DR: DynaVid是一个视频合成框架，利用合成运动数据（光流）来增强动态运动生成和精细运动控制能力，通过两阶段生成架构分离运动学习和视觉真实感。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在处理高度动态运动或需要精细运动控制时仍存在困难，主要原因是常用训练数据集中此类示例稀缺。需要一种方法既能获得多样运动模式，又能保持视觉真实感。

Method: 提出DynaVid框架：1）利用计算机图形流水线渲染的光流作为合成运动数据，光流只编码运动信息，与外观解耦；2）采用两阶段生成：运动生成器先合成运动，然后运动引导的视频生成器基于该运动生成视频帧。

Result: 在剧烈人体运动生成和极端相机运动控制两个具有挑战性的场景中进行了验证，实验表明DynaVid在动态运动生成和相机运动控制方面显著提升了真实感和可控性。

Conclusion: 通过合成运动数据（光流）与两阶段生成架构，DynaVid能够从合成数据中学习动态运动模式，同时从真实世界视频中保持视觉真实感，有效解决了现有数据集中动态运动示例稀缺的问题。
...
TL;DR: 提出了一种无域ID和无样本的增量学习框架，用于具身多媒体系统，通过解耦表示机制和权重融合策略实现鲁棒的连续环境适应。


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间连续交互时面临动态环境分布漂移的挑战。现有域增量感知方法通常依赖测试阶段预先获取的域ID，限制了在未知交互场景中的实用性。同时模型容易过拟合到特定上下文感知噪声，导致泛化能力不足和灾难性遗忘。

Method: 1. 设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2. 使用权重融合策略在参数空间中动态整合新旧环境知识，无需存储历史数据；3. 实现完全无样本和无域ID的增量学习框架。

Result: 在多个标准基准数据集上的广泛实验表明，该方法在完全无样本和无域ID的设置下显著减少了灾难性遗忘，其准确率优于现有最先进方法。

Conclusion: 提出的方法能够有效解决具身多媒体系统中的连续环境适应问题，通过解耦表示和权重融合实现了鲁棒的增量学习，在未知交互场景中具有更好的实用性。
...
TL;DR: 该论文提出了一种频率域适应(FDA)和谐波约束最优传输(HOT)框架，用于解决远程光电容积描记(rPPG)在跨域应用中的性能下降问题，通过转移低频频谱成分来学习对光照、相机特性等外观变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记(rPPG)虽然能够从面部视频进行非接触式生理测量，但在实际部署中经常因域偏移导致性能显著下降。现有的深度学习方法虽然在单个数据集上表现良好，但容易过拟合到光照、相机特性、颜色响应等外观相关因素，这些因素在不同域间变化很大。

Method: 提出了频率域适应(FDA)作为建模rPPG中外观变化的原理性策略，通过转移编码域依赖外观特征的低频频谱成分，使rPPG模型学习对外观变化的不变性，同时保留心脏诱导信号。进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性来指导原始表示和FDA转移表示之间的对齐，确保生理一致性。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 该研究通过频率域适应和谐波约束对齐的方法，成功解决了rPPG在跨域应用中的性能退化问题，为实际部署提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于高斯的方法虽然渲染质量高，但主要关注外观而忽略了实例级结构，限制了在高度动态场景中的稳定跟踪和语义推理能力。

Method: 1) 利用时间对齐的实例掩码和MLLM生成的句子嵌入，通过两个MLP解码器监督高斯语义特征；2) 将2D光流与4D高斯连接并微调运动，提高时间稳定性；3) 引入几何感知的SDF约束和正则化项，增强动态前景建模的时间一致性。
...
TL;DR: 该论文提出了一种结合外部和内部指导的视频对象移除方法，通过从视觉基础模型蒸馏对象与效应关系，并引入帧间上下文交叉注意力机制，以更好地理解目标对象及其在场景中的交互效应，实现更清晰连贯的视频对象移除。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法难以消除对象引起的副作用（如阴影、反射、光照变化），同时保持时空一致性。这源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 提出从两个互补角度引入理解：1）外部指导：通过蒸馏方案将视觉基础模型中对象与其诱导效应的关系转移到视频扩散模型；2）内部指导：提出帧间上下文交叉注意力机制，使每个去噪块都基于目标区域周围未掩码的上下文信息。

Result: 大量实验证明该方法达到了最先进的性能，并建立了首个真实世界视频对象移除基准，以促进未来研究和社区发展。

Conclusion: 外部和内部指导的联合使用使模型能够理解目标对象、其诱导效应和全局背景上下文，从而实现清晰连贯的视频对象移除。代码、数据和模型已开源。
...
TL;DR: 提出双向循环一致性框架的视频帧插值方法，通过前向生成和后向重建的对称约束确保时间一致性，无需推理时额外计算成本


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要单向操作，缺乏自我验证时间一致性的机制，导致长序列中运动漂移、方向模糊和边界不对齐问题

Method: 提出双向框架，引入可学习方向令牌在共享骨干网上显式条件化时间方向，联合优化前向合成和后向重建；采用课程学习策略从短到长序列渐进训练；仅在训练时应用循环约束

Result: 在37帧和73帧任务上实现最先进的成像质量、运动平滑度和动态控制性能，超越强基线且无额外计算开销

Conclusion: 循环一致性监督作为强大正则化器，确保生成的运动路径逻辑可逆，同时保持基础模型的高效推理能力
...
TL;DR: 提出了一种通过群体检测实现端到端共享注意力估计的方法，同时进行群体检测和共享注意力估计，解决了现有方法不检测实际关注群体或假设单点共享注意力的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在估计共享注意力时要么不检测实际关注群体，要么假设图像中只有一个共享注意力点，这些限制影响了共享注意力检测的实际应用性和性能表现。

Method: 采用两步过程：1) 基于个体注视注意力热图和群体推理中估计的群体成员标量生成共享注意力热图；2) 考虑初始共享注意力热图对初始群体成员关系进行细化，最终预测共享注意力热图。

Result: 实验表明该方法在群体检测和共享注意力估计方面优于其他方法，附加分析验证了所提出组件的有效性。

Conclusion: 通过同时进行群体检测和共享注意力估计的端到端方法，有效解决了现有方法的局限性，提升了共享注意力检测的实际应用性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于流的生成模型在文本引导图像编辑中存在源图像保真度不足的问题：高阶求解器需要额外推理、截断反演限制可编辑性、特征注入方法缺乏架构可迁移性。

Method: 1. 前向过程：引入摊销固定点求解器，通过强制连续时间步的速度一致性来隐式拉直前向轨迹，获得高保真反演潜在表示。2. 后向过程：引入轨迹插值，自适应混合目标编辑和源重建速度，使编辑轨迹锚定到源图像。3. 自适应掩码机制：通过概念引导分割和源-目标速度差异在空间上约束编辑信号，改善背景保留。

Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的大量实验表明，SteerFlow始终比现有方法实现更好的编辑质量。此外，SteerFlow可以自然地扩展到复杂的多轮编辑范式，而不会累积漂移。

Conclusion: SteerFlow是一个具有强大理论保证的模型无关编辑框架，通过隐式轨迹拉直和自适应速度混合，在保持源图像保真度的同时实现了高质量的文本引导图像编辑。
...
TL;DR: 提出DPMO方法将SAM与NNEC约束结合，从点标注生成密集实例分割；通过RPS框架和GRPO训练提升密集人群实例分割性能；在多个数据集上达到SOTA结果，并证明掩码标注能显著提升计数精度。


<details>
  <summary>Details</summary>
Motivation: 人群实例分割在监控和交通等领域有广泛应用，但现有数据集多为点标注，区域标注（如边界框）稀少且不准确。直接应用SAM等大型基础模型在密集人群场景效果不佳，需要专门的方法来处理密集人群的实例分割问题。

Method: 1. 提出Dense Point-to-Mask Optimization (DPMO)，将SAM与Nearest Neighbor Exclusive Circle (NNEC)约束结合，从点标注生成密集实例分割；2. 提出Reinforced Point Selection (RPS)框架，使用Group Relative Policy Optimization (GRPO)训练，从初始点预测采样中选择最佳预测点；3. 设计基于掩码监督的新损失函数提升计数性能。

Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的人群实例分割性能；通过掩码监督的新损失函数在不同模型上均提升了计数准确率，证明了掩码标注对提升计数精度的重要作用。

Conclusion: 提出的DPMO方法能有效从点标注生成高质量掩码标注，RPS框架结合GRPO训练显著提升了密集人群实例分割性能，掩码监督的损失函数进一步增强了计数模型的准确性，为密集人群分析提供了有效的解决方案。
...
TL;DR: 本文提出了一种几何感知的无人机地理定位框架，通过显式建模3D场景几何，将粗略位置识别和精细姿态估计统一在单个推理流程中，显著提升了GNSS拒止环境下无人机跨视角地理定位的精度。


<details>
  <summary>Details</summary>
Motivation: 在GNSS拒止环境中，无人机倾斜视角图像与正交卫星地图之间存在严重的几何差异，现有方法通常采用解耦的位置检索和姿态估计流程，将透视畸变视为外观噪声而非显式几何变换，这限制了定位精度。

Method: 提出几何感知的无人机地理定位框架：1）使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2）渲染虚拟鸟瞰图表示，将无人机视角正交化以对齐卫星图像；3）引入卫星级注意力块，隔离每个卫星候选与重建无人机场景的交互，防止候选间干扰同时保持线性计算复杂度。

Result: 在精炼的University-1652基准和SUES-200数据集上的广泛实验表明，该方法显著优于现有最先进基线，实现了鲁棒的米级定位精度，并在复杂城市环境中展现出改进的泛化能力。

Conclusion: 通过显式建模3D场景几何并统一位置识别与姿态估计，该方法为GNSS拒止环境下的无人机地理定位提供了有效的解决方案，同时发布的精炼数据集为端到端定位精度评估提供了更严格的基准。
...
TL;DR: 本文提出了Ultrasound-CLIP框架，通过构建大规模超声图像-文本数据集US-365K和超声诊断分类法，结合语义感知对比学习和异构图推理，显著提升了超声图像的理解能力。
...
Method: 1. 构建US-365K数据集（36.5万配对样本，52个解剖类别）；2. 建立超声诊断分类法（UDT），包含解剖层次分类和诊断属性框架；3. 提出Ultrasound-CLIP框架，引入语义软标签和语义损失；4. 构建基于UDAF文本表示的异构图进行结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务中表现出强大的泛化能力。

Conclusion: 通过构建大规模数据集和结构化知识框架，结合语义感知对比学习，Ultrasound-CLIP有效解决了超声图像理解中的挑战，为超声诊断AI应用提供了有力工具。
...
TL;DR: 提出一种轻量级架构和训练策略，使用自监督学习特征作为预训练视频扩散模型的通用条件信号，实现视频域迁移和3D生成视频，同时解耦外观与其他特征以实现鲁棒控制。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通过感知、几何或简单语义信号进行条件控制。自监督学习获得的高维特征作为视觉模型的通用接口，但尚未被探索作为预训练视频扩散模型的通用条件信号。这些特征包含大量纠缠的风格、光照和语义信息，限制了生成能力。

Method: 引入轻量级架构和训练策略，解耦外观特征与其他需要保留的特征。通过补偿低空间分辨率与高特征维度，提高从显式空间表示进行生成渲染的可控性。

Result: 该方法能够实现视频域迁移和从3D生成视频等任务，支持外观变化如风格化和重照明的鲁棒控制，提高了生成渲染的可控性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的架构设计实现特征解耦，显著提升视频生成任务的控制能力和应用范围。
...
TL;DR: 该论文提出了RebusBench基准测试，用于评估大型视觉语言模型在解决需要神经符号推理的谜语谜题方面的能力，发现当前模型表现严重不足，准确率低于10%。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在显式视觉识别方面表现出色，但在需要多步推理的问题上存在认知差距。当视觉输入仅作为线索而非答案时，模型难以进行复杂的神经符号推理，如解决谜语谜题所需的视觉属性提取、语言知识检索和抽象映射等能力。

Method: 研究者引入了RebusBench基准测试，包含1,164个谜语谜题，专门测试感知与知识的整合能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，分析了它们在精确匹配和语义准确性方面的表现，并考察了模型缩放和上下文学习的影响。

Result: 评估结果显示严重缺陷：模型性能饱和在精确匹配低于10%、语义准确性低于20%的水平。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 大型视觉语言模型在需要神经符号推理的任务上存在根本性局限，模型拥有视觉和语言组件但缺乏将它们连接起来的"认知胶水"。RebusBench为评估这种整合能力提供了重要基准，揭示了当前模型在复杂推理方面的不足。
...
TL;DR: DriveDreamer-Policy：一个统一的驾驶世界-动作模型，将深度生成、未来视频生成和运动规划集成到单一模块化架构中，通过几何感知的世界表示提升自动驾驶的想象和规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型主要关注2D外观或潜在表示建模，缺乏几何基础，而几何信息对于在物理世界中运行的具身系统至关重要。需要开发一个能够统一深度感知、未来预测和运动规划的驾驶世界模型。
...
Conclusion: DriveDreamer-Policy通过统一的几何感知世界表示框架，实现了更连贯的未来想象和更明智的驾驶动作，证明了显式几何建模对自动驾驶世界模型的重要性。
...
TL;DR: FSKD框架通过知识蒸馏将LiDAR森林结构信息迁移到RGBI图像，实现高分辨率森林结构监测，无需严格同步采集


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR成本高、频率低，但却是森林结构监测的金标准。需要开发基于低成本RGBI图像的替代方案来获取冠层高度模型、植物面积指数和叶层高度多样性等关键指标

Method: 提出FSKD知识蒸馏框架：多模态教师融合RGBI图像与LiDAR平面指标和垂直剖面，通过交叉注意力机制；RGBI-only SegFormer学生模型学习重现这些输出。在德国萨克森州384平方公里森林数据上训练

Result: 学生模型在零样本CHM预测上达到SOTA性能（MedAE 4.17m，R²=0.51，IoU 0.87），MAE比基线提升29-46%。多模态融合比纯RGBI训练提升10-26%性能。框架能同时预测CHM、PAI和FHD

Conclusion: FSKD框架实现了从LiDAR到RGBI的有效知识迁移，突破了严格同步采集的限制，支持冬季LiDAR与夏季RGBI的时间不匹配情况，为大规模森林监测提供了可扩展的解决方案
...
Method: 提出GardenDesigner框架，编码江南园林美学原则，基于程序化建模构建智能体链：地形分布、道路生成、资产选择和布局优化智能体；引入GardenVerse知识库增强资产布置；开发Unity交互界面支持文本输入生成

Result: 实验和人工评估表明，GardenDesigner能在一分钟内生成多样化且美观的江南园林，非专家用户可通过文本输入快速构建
...
TL;DR: 提出了一种基于轮式里程计的时序一致性单目深度估计框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计更新度量尺度，将预训练深度估计基础模型的相对深度重标定为度量深度。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间缺乏时序一致性，导致深度估计抖动，且在深度范围突变时容易失败。自动驾驶和移动机器人感知系统需要稳定一致的深度估计。

Method: 利用移动机器人的轮式里程计，通过连续帧间的光流进行三角化估计相机位姿和稀疏深度，使用稀疏深度更新递归贝叶斯估计的度量尺度，将预训练深度估计基础模型输出的相对深度重标定为度量深度。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能。

Conclusion: 提出的方法能够实现时序一致的单目深度估计，解决了现有方法在连续帧间深度估计不一致的问题，为自动驾驶和移动机器人提供了更可靠的感知能力。
...
TL;DR: STRIVE是一个用于视频问答的结构化强化学习框架，通过构建输入视频的多个时空变体并进行跨文本生成和视觉变体的联合归一化，解决组策略优化中奖励方差低的问题，提高策略更新的稳定性和信息量。


<details>
  <summary>Details</summary>
Motivation: 现有基于组的策略优化方法在多模态大模型中表现良好，但当响应具有相似正确性时，往往存在奖励方差低的问题，导致优势估计弱或不稳定。这限制了强化学习在视频问答中的效果。

Method: STRIVE构建每个输入视频的多个时空变体，在文本生成和视觉变体之间进行联合归一化。通过引入重要性感知采样机制，优先选择与输入问题最相关的帧，同时保持时间覆盖。这种方法将组比较从语言多样性扩展到结构化视觉扰动。
...
Conclusion: 结构化时空探索作为一种原则性机制，能够稳定多模态强化学习并提高视频推理性能。STRIVE通过丰富奖励信号和促进跨互补视觉视角的鲁棒推理，而不是过度拟合单一时空配置，实现了这一目标。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间中，通过扰动RoPE来抑制不安全内容生成，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于整流流变换器（如SD3、FLUX）的文本到图像模型虽然生成质量高，但对不安全语义（特别是多标记交互触发）仍然脆弱。现有缓解方法主要依赖微调或注意力调制进行概念遗忘，但这些方法计算开销大且专门为U-Net去噪器设计，难以直接适配变换器基础的扩散模型（如MMDiT）。

Method: 1. 分析MMDiT注意力机制，发现不安全语义集中在头部级别的可解释低维子空间中，有限的安全关键头部负责不安全特征提取。2. 提出SafeRoPE框架：首先通过分解安全关键头部中的不安全嵌入构建头部不安全子空间，通过投影计算每个输入向量的潜在风险评分（LRS）。3. 引入头部级RoPE扰动，在不降低良性内容或图像质量的情况下抑制不安全语义。4. 结合头部级LRS和RoPE扰动对查询和关键向量嵌入执行风险特定的头部级旋转。

Result: 大量实验表明，SafeRoPE在平衡有效有害内容缓解和效用保持方面实现了最先进的性能，适用于MMDiT的安全生成。

Conclusion: SafeRoPE是一个轻量级、细粒度的安全生成框架，通过精确抑制不安全输出同时保持生成保真度，为变换器基础的扩散模型提供了有效的安全生成解决方案。
...
TL;DR: 该论文提出了一种通过随机标签桥接训练实现语言预训练模型向视觉任务迁移的方法，发现部分桥接训练比完全桥接更有效，为跨模态适应提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为语言预训练模型由于参数空间差异不适合视觉任务，但作者挑战这一假设，探索如何通过桥接训练使大语言模型适应视觉基础任务。

Method: 提出随机标签桥接训练方法，无需人工标注，通过添加桥接训练阶段作为模态适应学习器，对齐LLM参数与视觉任务。特别强调部分桥接训练的优势。

Result: 发现桥接训练能有效使LLM参数适应视觉任务，且部分桥接训练通常更有利，因为LLM中某些层具有强大的基础特性，即使不针对视觉任务微调也保持有益。

Conclusion: 该研究为直接利用语言预训练参数于视觉模型开辟了新途径，突出了部分桥接训练作为跨模态适应实用路径的潜力，挑战了传统跨模态迁移的假设。
...
TL;DR: 提出一种新的半监督域适应方法，通过基于类别顺序的排序学习排名分数来对齐源域和目标域，解决医学图像严重程度分类中类别边界模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 现有半监督域适应方法在处理医学图像严重程度分类时面临挑战，因为严重程度分类涉及自然有序的类别标签，类别边界不清晰，使得域适应变得复杂。

Method: 提出两种核心方法：1) 跨域排序：跨域对样本对进行排序；2) 连续分布对齐：对齐排名分数的分布。通过基于类别顺序的排序学习排名分数来对齐源域和目标域。

Result: 在溃疡性结肠炎和糖尿病视网膜病变分类任务上的实验验证了方法的有效性，成功对齐了类别特定的排名分数分布。

Conclusion: 该方法能够有效解决医学图像严重程度分类中的域适应问题，通过利用类别顺序信息来改善跨域对齐。
...
Result: 在语义城市网格（SUM）基准测试中达到81.9% mF1和94.3% OA，在新构建的文化遗产数据集（带损坏类型标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 提出的纹理感知Transformer能够有效利用3D网格的纹理信息，结合几何特征，通过两阶段Transformer块实现局部和全局信息融合，在语义分割任务上取得显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解方面表现出色，但大量输入视频令牌带来了巨大的计算负担。现有方法主要在输入层面剪枝，忽略了视频和LLM中固有的信息结构。

Method: 基于视频具有分段-帧结构和LLM内部单向传播多模态信息的观察，提出三层剪枝：1) 分段级：视频令牌先按时间分段并空间合并；2) 帧级：同一分段内相似帧联合剪枝以保持多样性；3) 层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准测试中，仅保留30%令牌的情况下，HieraVid实现了新的SOTA性能，同时保持了LLaVA-Video-7B和LLaVA-OneVision-7B分别超过98%和99%的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（如VQ-VAE、VQ-GAN）学习的离散神经表示具有位置依赖性：编码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模其依赖关系。本研究探讨位置信息对于空间对齐数据的离散表示是否必要。

Method: 提出排列不变向量量化自编码器（PI-VQ），约束潜在编码不携带位置信息。为应对排列不变表示的信息容量减少问题，引入基于最优二分图匹配的匹配量化算法，将有效瓶颈容量提升3.5倍。利用学习编码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标。位置无关表示鼓励编码捕获全局语义特征，并实现无需学习先验的直接图像插值。

Conclusion: 位置无关表示在潜在编码的可分离性和可解释性方面存在权衡，为未来工作指明了多个方向。PI-VQ提供了一种替代传统位置依赖表示的方法，能够捕获更全局的语义特征。
...
TL;DR: FaCT-GS是一个用于快速灵活CT重建的框架，通过优化体素化和光栅化流程，相比现有高斯泼溅方法在512×512投影上快4倍，在2k投影上快13倍。


<details>
  <summary>Details</summary>
Motivation: 虽然高斯泼溅技术在CT重建中表现良好，但其优势不足以促使从成熟算法转向。现有高斯泼溅方法仍存在显著限制，需要更快速、更灵活的重建方案。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流程，实现快速重建。改进的体素化能够快速将高斯分布拟合到现有体积数据，可作为先验进行热启动重建或作为压缩表示。

Result: FaCT-GS在标准512×512投影上比现有最佳高斯泼溅CT重建快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能适应不同投影和输出体积大小。

Conclusion: FaCT-GS解决了高斯泼溅在CT重建中的主要限制，提供了快速灵活的重建方案，可作为现有体积数据的压缩表示或重建先验，显著提升了重建效率。
...
<details>
  <summary>Details</summary>
Motivation: 尽管现代视觉语言模型在语义任务上表现出色，但本研究旨在揭示它们在更基础层面上的局限性——缺乏对基本几何变换的鲁棒性，这暴露了当前模型在空间推理能力上的不足。

Method: 通过系统评估方法，在不同视觉领域（包括符号草图、自然照片和抽象艺术）测试视觉语言模型在几何变换下的表现，考察了不同架构、模型容量和提示策略的影响。

Result: 实验结果显示，随着语义内容变得稀疏，模型性能急剧下降；这种脆弱性在不同架构、模型容量和提示策略中普遍存在，表明当前视觉语言模型在空间推理方面存在系统性缺陷。

Conclusion: 研究揭示了当前视觉语言模型中语义理解与空间推理之间的系统性差距，强调了未来多模态系统需要更强的几何基础来提升空间不变性和等变性能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前时序动作分割方法依赖复杂架构，不利于实际部署。需要一种简单有效的训练框架来提升分割质量，而不增加推理复杂度

Method: 提出双损失训练框架：1) 边界回归损失通过单通道边界预测提升时序定位精度；2) CDF分段正则化损失通过匹配预测和真实分段的累积分布来增强分段内结构一致性。该框架与架构无关，可集成到现有TAS模型中

Result: 在三个基准数据集上，该方法提升了分段一致性和边界质量，提高了F1和Edit分数，同时帧级精度基本保持不变。验证了通过简单损失设计而非复杂架构或推理时优化即可实现精确分割

Conclusion: 通过轻量级双损失训练框架，仅需一个额外输出通道和两个辅助损失项，就能显著提升时序动作分割质量，证明了简单损失设计的有效性，为实际部署提供了实用解决方案
...
Conclusion: MAR-MAER通过结合度量感知嵌入正则化和概率潜在建模，有效提升了文本到图像生成的质量和模糊提示处理能力，在自动指标和人工评估中都验证了其优越性能。
...
TL;DR: A3R框架将3D高斯场景中的精细可承受性推理重新定义为顺序证据获取过程，通过跨维度证据采集逐步减少歧义，超越传统静态一次性预测方法。
...
Result: 在场景级基准测试中，A3R始终超越静态一次性基线方法，展示了跨维度证据采集在复杂3D高斯场景中精细可承受性推理的优势。

Conclusion: 将可承受性推理重新定义为顺序证据获取过程，通过智能跨维度证据采集能有效解决复杂3D场景中证据不完整的问题，提高推理准确性。
...
TL;DR: GS^2提出基于图的空间分布优化方法，通过ELBO自适应致密化、不透明度感知渐进剪枝和图特征编码模块，在显著减少高斯点数量的同时提升3DGS的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）在实时渲染方面表现出色，但大量高斯点导致高内存消耗。现有剪枝方法虽然能减少内存，但会破坏空间一致性并产生渲染伪影。

Method: 1. 基于证据下界（ELBO）的自适应致密化策略，自动控制高斯点的致密化过程；2. 不透明度感知渐进剪枝策略，动态移除低不透明度高斯点；3. 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: GS^2仅使用约12.5%的高斯点就实现了比原始3DGS更高的PSNR，在所有对比基线中，在渲染质量和内存效率方面均表现最优。

Conclusion: GS^2通过优化高斯点的空间分布，实现了紧凑的高斯表示，在显著减少内存消耗的同时提升了渲染质量，解决了3DGS实用化中的关键瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型虽然部署了安全过滤器和审核管道来防止滥用，但仍然容易受到低成本的越狱攻击。本研究旨在揭示这些模型在安全防护方面的根本性缺陷。

Method: 提出系统性的提示词越狱攻击策略分类法，包括艺术重构、材料替换、伪教育框架、生活美学伪装和模糊动作替换等技术。这些策略通过将恶意意图隐藏在良性语义上下文中来绕过安全过滤。

Result: 在多个最先进的文本到图像系统上评估这些攻击，发现简单的语言修改就能可靠地绕过现有安全防护并生成受限图像。所有测试模型和攻击类别的攻击成功率高达74.47%。

Conclusion: 研究揭示了表面级提示词过滤与检测生成媒体系统中对抗意图所需语义理解之间的关键差距，表明当前安全措施存在严重不足。
...
Conclusion: ProVG通过解耦语言表达和渐进式跨模态调制，有效利用了细粒度语言线索，解决了遥感视觉定位中的关键挑战，为相关任务提供了强大的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的形状合成方法在捕捉表面细节、计算效率和模型简洁性方面存在不足，需要一种能够同时优化这些方面的框架。

Method: 使用球谐函数表示距离场，通过优化参考点放置（最大化稀疏性、中心性和表面可见性），采样可见距离场并计算球谐系数，应用低通滤波和局部一致性约束进行细化。

Result: SHARC在重建精度和时间效率方面优于现有方法，同时保持模型简洁性，源代码已公开。

Conclusion: SHARC框架通过球谐函数表示和优化参考点放置，实现了高效、精确的形状合成，为几何处理提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法要么侧重于帧级增强而时间建模有限，要么依赖重型时空聚合而牺牲高频细节，难以同时保持时间稳定性和空间细节。

Method: 提出FTPFusion方法：1）将特征表示分解为高频和低频分量进行协同建模；2）高频分支执行稀疏跨模态时空交互以捕获运动相关上下文和互补细节；3）低频分支引入时间扰动策略增强对复杂视频变化（如闪烁、抖动、局部错位）的鲁棒性；4）设计偏移感知时间一致性约束来显式稳定时间扰动下的跨帧表示。
...
TL;DR: 本文提出Light-ResKAN模型，通过将ResNet中的卷积替换为KAN卷积，使用Gram多项式作为激活函数，并采用参数共享策略，在保持SAR图像识别精度的同时大幅降低计算量和参数量。


<details>
  <summary>Details</summary>
Motivation: SAR图像识别在灾害监测、军事侦察和海洋观测中至关重要，但大尺寸SAR图像在资源受限的边缘设备上部署深度学习模型存在困难，现有轻量级模型难以平衡高精度特征提取与低计算需求之间的矛盾。
...
Conclusion: Light-ResKAN模型为边缘设备上的SAR图像识别提供了一个高效解决方案，在保持高精度的同时显著降低了计算复杂度。
...
Method: 采用2D几何驱动的星座式管道，将角膜反射视为结构化星座而非独立斑点，提出相似性-布局对齐(SLA)程序，结合可控过检测、自适应候选回退、外观感知评分和可选语义布局先验

Result: 在公共多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系

Conclusion: 该方法实现了可重复的多重角膜反射检测与匹配，并发布了代码、预设和评估脚本以支持透明复现、比较和数据集标注
...
TL;DR: KnowMVG提出了一种结合知识先验和全局-局部注意力增强的医学视觉定位框架，通过知识增强提示和双尺度注意力机制提升VLMs在医学图像中的空间定位精度。
...
Result: 在四个MVG基准测试中，KnowMVG均优于现有方法，AP50提升3.0%，mIoU提升2.6%，定性和消融研究验证了各组件有效性。

Conclusion: 通过显式增强空间感知的知识先验和注意力机制，KnowMVG成功提升了医学视觉定位的精度，为临床决策提供了更可靠的视觉证据支持。
...
TL;DR: 该研究探索了能否直接从波束成形前的每天线距离-多普勒张量中学习有意义的空间结构，而不需要传统的角度域波束成形处理。


<details>
  <summary>Details</summary>
Motivation: 传统汽车雷达感知流程通常先通过波束成形构建角度域表示，再应用基于学习的模型。本研究旨在探索是否可以直接从波束成形前的每天线距离-多普勒测量中学习空间结构，简化感知流程。

Method: 使用6发射×8接收（48虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案。构建双啁啾共享权重编码器，以端到端数据驱动方式处理波束成形前的每天线RD张量。使用鸟瞰图占用作为几何探针，监督信号来自激光雷达，并考虑雷达视场和遮挡感知的激光雷达可观测性。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束成形前的每天线RD张量中学习空间结构，无需显式的角度域构建或手工信号处理阶段。

Conclusion: 空间结构可以直接从波束成形前的每天线距离-多普勒张量中学习，无需传统的角度域波束成形处理，这为汽车雷达感知提供了更简化的端到端学习框架。
...
TL;DR: 本文针对幼儿教育图像描述任务，提出了ECAC大规模数据集和RSRS混合训练框架，开发了KinderMM-Cap-3B模型，显著提升了专业物体识别精度。


<details>
  <summary>Details</summary>
Motivation: 幼儿教育图像描述对于自动化活动理解和教育评估至关重要，但现有方法面临两大挑战：缺乏大规模领域特定数据集导致描述泛化不精确；传统训练范式在提升专业物体描述能力方面存在局限。

Method: 1. 构建ECAC大规模幼儿教育日常活动图像描述基准数据集（256,121张真实图像，专家级标注）；2. 提出TTS（教学玩具识别分数）领域导向评估协议；3. 设计RSRS混合训练框架，动态切换强化学习和监督微调，将零奖励困难样本重定向到监督学习。

Result: 开发的KinderMM-Cap-3B模型在TTS评估中达到51.06分，显著优于现有最先进基线模型，同时保持了优越的描述质量。

Conclusion: ECAC数据集和RSRS训练框架有效解决了幼儿教育图像描述的专业性和稳定性问题，KinderMM-Cap-3B模型展示了在专门化教育应用中的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临两个主要问题：1）缺乏大量标注训练数据；2）数据不平衡问题（常见类别数据丰富，罕见类别数据稀缺）。自监督学习方法在一定程度上解决了第一个问题，但在医学图像分类领域，自监督学习对不平衡数据的鲁棒性研究较少。

Method: 1）扩展先前提出的MIMV方法，引入新的增强策略构建不对称多图像多视图（AMIMV）对，同时解决数据稀缺和数据集不平衡问题；2）进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3）在11个医学图像数据集（MedMNIST）上评估8种代表性自监督学习方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法有效改善了医学图像分类在数据不平衡条件下的性能。

Conclusion: AMIMV方法通过不对称多图像多视图增强策略，能够有效应对医学图像分类中的数据稀缺和类别不平衡问题，在多个医学数据集上取得了显著性能提升，为不平衡医学图像分析提供了有效的解决方案。
...
Method: 提出运动感知稀疏交互机制：利用光流识别多模态序列中的动态区域，自适应地将计算密集的跨模态注意力分配给这些稀疏区域；对静态背景区域使用轻量级弱交互模块；通过解耦动态和静态区域的处理来平衡效率和效果。

Result: 在多个红外和可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的推理速度。

Conclusion: MAVFusion通过运动感知稀疏交互机制，在保持时间一致性和细节完整性的同时显著加速推理，为红外和可见光视频融合提供了高效解决方案。
...
TL;DR: 使用nnU-Net v2框架开发的多模态深度学习模型，用于自动分割mpMRI中的前列腺，在交叉验证中Dice分数达0.96，外部验证达0.82，显著优于通用分割工具。


<details>
  <summary>Details</summary>
Motivation: 前列腺在mpMRI中的准确分割对临床和研究应用至关重要，但手动分割耗时且存在观察者间差异，通用分割工具在前列腺特定任务中精度不足，需要专门的多模态深度学习解决方案。

Method: 基于nnU-Net v2框架开发专用深度学习模型，利用多模态mpMRI数据（T2加权成像、DWI、ADC图）获取互补组织信息，使用PI-CAI数据集的981个病例进行训练，采用5折交叉验证和外部验证评估性能。

Result: 模型在交叉验证中平均Dice分数为0.96±0.00，在外部测试集（Hospital La Fe的54名患者）上为0.82，表现出良好的泛化能力。相比之下，通用方法TotalSegmentator的Dice分数仅为0.15，主要由于腺体分割不足。

Conclusion: 研究强调了任务特异性多模态分割策略的重要性，证明了所提方法在临床研究工作流程中可靠集成的潜力。模型已完全容器化，可作为即用推理工具使用。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中个性化问答能力的数据集，包含541个长视频和5K个关于"我的物品"、"我的活动"和"我的过去"的问题。研究发现当前顶级模型表现不佳，与人类性能差距显著。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要自我接地（理解摄像头佩戴者）的个性化问答任务中存在不足，缺乏专门评估模型理解、记忆和推理摄像头佩戴者能力的数据集。

Method: 创建MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类问题。对开源与闭源、思考与非思考、不同规模的各种MLLMs进行系统评估。

Result: 顶级闭源模型（如GPT-5）准确率约46%，开源模型（如Qwen3-VL）约36%，分别比人类性能低40%和50%。显式推理和模型缩放未能带来一致改进，提供相关证据时模型有所改善但随时间增益下降。

Conclusion: 研究强调了自我接地和长程记忆在自我中心视频个性化问答中的关键作用。MyEgo数据集和分析将推动该领域发展，促进自我中心个性化辅助系统的进步。
...
TL;DR: SDesc3D是一个基于简短文本描述的3D室内场景生成框架，通过多视角结构先验和区域功能感知来解决现有方法在物理合理性和细节丰富度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本描述的3D场景生成方法在简短文本条件下存在物理合理性差和细节不足的问题，主要因为它们过度依赖明确的物体组成和空间关系语义线索，需要增强3D推理能力，特别是在先验整合和空间锚定方面。

Method: 1. 多视角场景先验增强：通过聚合多视角结构知识来丰富欠指定的文本输入；2. 功能感知布局定位：利用区域功能定位进行隐式空间锚定，并进行分层布局推理；3. 迭代反思-修正方案：通过自我修正进行渐进式结构合理性优化。

Result: 大量实验表明，该方法在简短文本条件下的3D室内场景生成任务上优于现有方法。

Conclusion: SDesc3D通过整合多视角结构先验和区域功能含义，有效提升了在稀疏文本指导下的3D布局推理能力，解决了现有方法在物理合理性和语义可信度方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要在非交互式、发射后不管的模式下工作，不适用于需要人在回路适应的现实场景，因此需要引入人类指导的交互式跟踪能力

Method: 1) 创建InteractTrack基准，包含150个视频、密集边界框标注和时间戳语言指令；2) 提出综合评估协议并评估25个代表性跟踪器；3) 提出交互式记忆增强跟踪(IMAT)基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为

Result: 评估显示最先进的方法在交互场景中失败，传统基准上的强性能无法迁移；IMAT方法为交互式跟踪提供了有效的基线解决方案

Conclusion: 建立的基准、协议和基线为开发更智能、自适应和协作的跟踪系统奠定了基础，弥合了自动感知与人类指导之间的差距
...
TL;DR: Curia-2是一个用于医学影像分析的基础模型，通过改进预训练策略和表示质量，首次将架构扩展到数十亿参数的视觉Transformer，并在2D和3D评估基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，需要开发基础模型来减轻这一负担。虽然现有基础模型在CT和MRI分析中显示出大规模预训练的优势，但在如何从复杂的放射学体积数据中学习方面仍有很大优化空间。

Method: 基于Curia框架，提出了Curia-2，改进了原始预训练策略和表示质量，以更好地捕捉放射学数据的特性。该方法首次将架构扩展到数十亿参数的视觉Transformer，用于多模态CT和MRI基础模型。同时，通过扩展和重组CuriaBench，建立了两个独立的评估轨道：针对基于切片的视觉模型的2D轨道和用于体积基准测试的3D轨道。

Result: Curia-2在视觉聚焦任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型表现相当。模型权重将公开以促进进一步研究。

Conclusion: Curia-2通过改进的预训练策略和表示质量，显著提升了医学影像基础模型的性能，首次实现了数十亿参数视觉Transformer在CT和MRI多模态分析中的应用，为医学影像分析领域提供了强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现多模态大语言模型中的视觉注意力表现出明显的惯性现象，一旦在早期解码步骤中稳定下来就基本保持静态，无法支持认知推理所需的组合理解。现有的幻觉缓解方法主要针对感知幻觉（如物体存在或属性），但对需要对象间关系推理的认知幻觉效果有限。

Method: 提出无需训练的惯性感知视觉激发方法，通过将认知推理建模为视觉注意力的动态响应来打破惯性模式。具体包括：1）选择相对于历史注意力趋势动态出现的视觉标记；2）区分表现出惯性行为的标记；3）引入惯性感知惩罚机制，防止注意力过度集中和局部区域的持久关注。
...
TL;DR: Resonance4D：一个物理驱动的4D动态模拟框架，通过双域运动监督和全参数物理恢复，在单消费级GPU上实现高保真4D模拟，显著降低计算成本和内存开销。
...
Method: Resonance4D框架将3D高斯溅射与材料点方法耦合，通过轻量级但物理表达性强的监督实现。核心创新包括：1）双域运动监督（DMS），结合空间结构一致性（局部变形）和频域谱一致性（振荡和全局动态模式）；2）零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部分级区域，支持全材料参数的联合优化。

Result: 在合成和真实场景上的实验表明，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使高保真物理驱动的4D模拟能够在单消费级GPU上运行。

Conclusion: Resonance4D通过创新的双域运动监督和全参数物理恢复方法，解决了现有4D动态模拟中的计算成本和物理真实性矛盾，为复杂材料和动态场景的高效高保真模拟提供了可行方案。
...
TL;DR: ProDiG：一种渐进式扩散引导框架，通过合成中间高度视图和几何感知的因果注意力模块，将航空3D表示逐步转换为地面级保真度，解决航空到地面极端视角变化的挑战。


<details>
  <summary>Details</summary>
Motivation: 从纯航空图像生成地面视图和连贯3D场景模型面临极端视角变化、缺少中间观测和大尺度变化的挑战。现有方法要么后处理渲染产生几何不一致结果，要么依赖多高度真实数据（通常不可得），而高斯泼溅和扩散细化在小变化下有效但在大视角差距下失败。

Method: ProDiG（渐进式高度高斯泼溅）采用扩散引导框架，逐步将航空3D表示转换为地面级保真度。包含：1）合成中间高度视图；2）几何感知因果注意力模块，将极线结构注入参考视图扩散；3）距离自适应高斯模块，根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建。

Result: 在合成和真实数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和连贯3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式、几何基础的细化方法，无需额外地面真实视角，成功解决了航空到地面极端视角转换的挑战，实现了高质量的地面级视图生成和3D场景建模。
...
<details>
  <summary>Details</summary>
Motivation: 遥感开放词汇语义分割需要语言对齐识别和细粒度空间划分。CLIP虽然提供强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入RS预训练的DINO特征进行补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，既难以有效划分边界，又可能破坏CLIP的语义完整性。

Method: DR-Seg采用解耦与校正框架：1）基于CLIP特征通道功能异质性的观察，将CLIP特征解耦为语义主导和结构主导子空间；2）在DINO指导下，通过先验驱动图校正模块注入高保真结构先验形成细化分支；3）使用不确定性引导的自适应融合模块动态集成细化分支和原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征并针对性增强结构信息，有效解决了遥感开放词汇语义分割中语言对齐与空间细粒度划分的平衡问题，在保持语义完整性的同时提升了边界划分精度。
...
TL;DR: 该论文提出了LinkS²Bench，首个评估视觉语言模型在无人机与卫星协同空间智能方面的基准测试，包含17.9k高质量问答对，覆盖12个细粒度任务，发现现有模型在跨视图动态对齐方面存在显著差距，并设计了Cross-View Alignment Adapter来提升性能。
...
Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200平方公里以上的高分辨率卫星图像关联，通过LMM辅助流程和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务。设计了Cross-View Alignment Adapter来提升跨视图对齐能力。

Result: 对18个代表性视觉语言模型的评估显示，与人类基线相比存在显著差距，准确跨视图动态对齐是关键瓶颈。Cross-View Alignment Adapter显著提升了模型性能，微调实验证明了LinkS²Bench在推进复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估视觉语言模型在无人机-卫星协同空间智能方面的空白，揭示了现有模型在跨视图动态对齐方面的不足，提出的对齐适配器有效提升了性能，为复杂空间推理的模型发展提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 在医学成像、生物学和物理学等领域，图像内容通常存在空间非均匀采样问题，信息丰富的模式在特定坐标上很少出现，而背景在大多数样本中占据主导地位。这导致自编码器偏向于主要模式，在空间数据不平衡的情况下，会丢失细粒度细节并对罕见空间输入产生模糊重建。

Method: 提出了两个互补组件：(1) 基于自熵的损失函数，对统计上不常见的空间位置进行加权；(2) 样本传播机制，这是一种重放机制，在训练过程中选择性地将难以重建的样本重新暴露给模型。

Result: 在具有受控空间不平衡条件的模拟数据集以及三个未受控的真实世界数据集（物理、生物和天文领域）上验证了方法。该方法在各种重建指标上优于基线方法，特别是在空间不平衡分布下表现更佳。
...
TL;DR: IndoorCrowd是一个大规模室内人群数据集，包含31个视频、9,913帧，支持检测、分割和多目标跟踪任务，在四个校园场景中收集，评估了基础模型自动标注性能并建立了多种基线。
...
Method: 在四个校园位置收集31个视频（9,913帧，5fps），提供人工验证的实例分割掩码。使用620帧控制子集评估SAM3、GroundingSAM和EfficientGroundingSAM三种基础模型自动标注器，使用Cohen's κ、AP、精度、召回率和掩码IoU指标。提供2,552帧子集支持MOTChallenge格式的多目标跟踪。

Result: 建立了YOLOv8n、YOLOv26n和RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT的检测、分割和跟踪基线。场景分析显示难度差异显著：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）最具挑战性。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为检测、分割和跟踪任务提供了基准，并揭示了场景特性（人群密度、尺度和遮挡）对算法性能的重要影响。
...
TL;DR: WISE提出了一种高效的多模态推理范式，通过"思考两次"原则（一次学习，一次加速），在推理时生成简洁的理性分析而非详细解释，显著减少了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理方法在语言引导分割任务中虽然性能优秀，但生成冗长的理性分析导致计算成本过高，限制了实际应用。

Method: WISE训练模型生成结构化序列：简洁理性分析→最终答案→详细解释。通过自蒸馏目标强制简洁理性分析作为详细解释的充分摘要，推理时使用WISE-S策略注入简洁性指令来激活学习到的简洁策略。

Result: 在ReasonSeg基准测试中实现58.3 cIoU的零样本性能，同时将平均推理长度从112个token减少到23个token，减少了近5倍。

Conclusion: WISE通过"思考两次"范式实现了高效的多模态推理，在保持性能的同时大幅降低计算成本，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前构建多语言视觉语言模型面临的主要障碍是：英语以外的语言缺乏大规模、多样化的视觉问答数据集。日本尤其缺乏足够规模和领域覆盖的VQA数据集，这阻碍了高质量日语VLMs的开发。

Method: 不依赖现有VQA数据集，而是收集异构源数据（图像、图文对、PDF文档），通过多种策略生成VQA对：基于VLM的问答生成、翻译和文本渲染。

Result: 使用Jagle训练的2.2B模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。同时，Jagle与FineVision结合不仅不会降低英语性能，反而比单独使用FineVision训练效果更好。

Conclusion: Jagle数据集为构建高质量日语视觉语言模型提供了有效解决方案，通过创新的数据生成策略解决了日语VQA数据稀缺问题，同时保持了英语性能，为多语言VLMs研究提供了重要资源。
...
TL;DR: 提出一种全自动、可扩展的虚拟人皮肤色调保真度评估方法，通过系统化流程评估肤色提取策略在不同光照条件下的表现，发现肤色提取策略存在表型依赖性，深色肤色的色度误差更高。


<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中准确再现面部肤色对于真实感、身份保持和公平性至关重要，但现有可访问的虚拟人创建流程依赖缺乏色度校准的摄影输入，可能导致不一致性和偏见。

Method: 提出全自动工作流程，整合肤色与光照提取、纹理重着色、实时渲染和定量色彩分析。使用芝加哥人脸数据库，比较基于脸颊区域采样和基于全脸分析的多维掩码两种肤色提取策略，结合TRUST框架进行光照隔离。提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染，在CIELAB色彩空间中使用ΔE度量和个体类型学角度(ITA)客观评估肤色一致性。

Result: 生成并分析了约19,848个渲染实例，结果显示提取策略存在表型依赖性行为，深色肤色的色度误差始终更高。

Conclusion: 提出的方法无需人工干预，除预训练的光照补偿模块外不包含学习或训练阶段，计算成本低且支持大规模评估，为虚拟人生成流程中的肤色保真度提供了系统化评估框架。
...
TL;DR: 论文提出Multi-Object Affordance Grounding under Intent-Driven Instructions新任务，构建CompassAD基准数据集，开发CompassNet框架解决多物体场景中混淆对的意图驱动功能感知问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D功能感知方法主要评估孤立单物体，且常提供明确类别名称，忽略了真实场景中多个物体共享相同功能但只有特定物体适合特定任务上下文的问题（混淆对）。需要解决在杂乱多物体点云中基于隐式自然语言意图预测正确物体上每点功能掩码的挑战。

Method: 提出CompassNet框架，包含两个专用模块：1) Instance-bounded Cross Injection (ICI)：将语言-几何对齐约束在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)：在几何组和点级别强制执行区分，增强目标和混淆表面之间的区别。

Result: 在CompassAD基准上进行了广泛实验，在可见和未见查询上都取得了最先进的结果。在机器人机械臂上的部署证实了该方法能有效转移到真实世界的混淆多物体场景抓取任务中。

Conclusion: 论文形式化了意图驱动指令下的多物体功能感知任务，构建了首个专注于混淆多物体场景中隐式意图的基准CompassAD，提出的CompassNet框架通过ICI和BCR模块有效解决了混淆对问题，在仿真和真实机器人实验中均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显式思维链的通用多模态嵌入方法存在两个主要问题：1) 推理开销大，需要生成大量文本token；2) 将丰富的多模态证据压缩到狭窄的文本瓶颈中，可能丢失重要信息。

Method: PLUME采用潜在推理框架，用少量连续潜在状态的自回归展开替代显式思维链。引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹。采用渐进式显式到潜在课程学习，使用显式推理作为训练支架，逐步将推理行为转移到隐藏状态计算中。

Result: 在78个任务的MMEB-v2基准测试中，PLUME超越了强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，推理速度提升超过30倍。特别适用于证据密集、结构复杂、难以通过显式中间推理组织的检索场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的滑块方法通常需要辅助模块和训练监督，这会增加训练开销，并且滑块行为与训练分布耦合，在编辑或领域分布偏移时可靠性降低。

Method: FlowSlider将FlowEdit的更新分解为：(1)保真项：作为源条件稳定器，保留身份和结构；(2)转向项：驱动语义向目标编辑过渡。几何分析和经验测量显示这两项近似正交，因此仅缩放转向项即可实现稳定的强度控制。

Result: FlowSlider无需后训练即可提供平滑可靠的控制，在各种任务中提高了连续编辑质量。

Conclusion: FlowSlider是一种无需训练的方法，通过正交分解编辑更新，实现了连续图像编辑的稳定强度控制，解决了现有方法对训练分布的依赖问题。
...
TL;DR: GroundVTS：一种用于视频时序定位的视觉大语言模型架构，通过查询引导的细粒度视觉令牌采样机制，聚焦信息最丰富的时序片段，解决了传统均匀帧采样导致的关键帧稀疏和时序信息丢失问题。
...
Result: 在三个标准VTG基准测试中超越现有方法，在时刻检索任务上mIoU提升7.7个百分点，在高光检测任务上mAP提升12.0个百分点。

Conclusion: GroundVTS通过聚焦信息丰富的时序片段，有效解决了视频时序定位中的关键帧稀疏问题，显著提升了视频大语言模型在时序定位任务上的性能。
...
TL;DR: LatentUM提出了一种新型统一模型，将所有模态表示在共享语义潜在空间中，消除了视觉理解和生成之间像素空间中介的需求，实现了更高效的跨模态推理和生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解和生成之间的桥梁，这种方法既低效又低效。研究者希望开发一种能够直接在语义层面进行跨模态推理和生成的模型，提高计算效率并增强跨模态对齐。

Method: LatentUM将所有模态（包括视觉）表示在共享的语义潜在空间中，避免了像素空间的中介。这种设计允许模型直接在语义层面进行视觉理解和生成，支持灵活的跨模态推理。

Result: LatentUM在视觉空间规划基准测试中取得了最先进的性能，通过自反思提升了视觉生成质量，并支持通过共享语义潜在空间预测未来视觉状态的世界建模能力。

Conclusion: 通过在共享语义潜在空间中表示所有模态，LatentUM实现了更高效、更有效的跨模态理解和生成，为密集视觉思维、自反思视觉生成和物理世界建模等应用提供了新的可能性。
...
TL;DR: 提出CoRegOVCD框架，通过一致性正则化解决开放词汇变化检测中训练免费设置下的噪声和语义不可靠问题，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定标签空间，无法回答用户定义的任意查询。开放词汇变化检测(OVCD)虽然可以查询特定概念的变化，但在完全训练免费设置下，由于外观变化、弱跨概念竞争和空间连续性等因素，直接比较不同时间的密集概念响应会产生噪声、碎片化和语义不可靠的变化证据。
...
Result: 在四个基准测试中，CoRegOVCD相比先前最强的训练免费基线持续提升2.24到4.98个F1_C点，在SECOND数据集上达到六类平均47.50%的F1_C分数。

Conclusion: CoRegOVCD通过后验校准和一致性正则化机制，有效解决了开放词汇变化检测中的语义可比性问题，在训练免费设置下实现了显著性能提升。
...
TL;DR: 该研究揭示了面部动作单元检测中标准评估协议（主体排他交叉验证）存在显著随机方差，并提出了更稳定的跨数据集评估方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是发现面部动作单元检测领域经常报告的小幅改进可能实际上落在评估协议本身的随机方差范围内，需要更可靠的评估方法来准确衡量模型性能。

Method: 采用两种评估方法：1）在BP4D+数据集上进行重复3折主体排他交叉验证，测量协议方差；2）在五个AU数据集上实施留一数据集出交叉验证，评估跨数据集鲁棒性。

Result: 结果显示：1）标准交叉验证引入显著随机方差（平均F1波动±0.065）；2）低流行度AU的方差更大；3）操作点指标（如F1）比阈值无关指标（如AUC）波动更大；4）留一数据集出验证能揭示单数据集交叉验证无法发现的领域级不稳定性。

Conclusion: 结论是面部动作单元检测研究中经常报告的改进可能落在评估协议方差范围内，留一数据集出交叉验证能提供更稳定和可解释的结果，建议采用跨数据集评估来获得更可靠的性能评估。
...
TL;DR: 该论文提出了首个大规模物体反射数据集DEROBA，并开发了一种基于基础扩散模型的反射生成方法，通过注入反射位置和外观先验信息，实现物理一致且视觉逼真的反射效果。
...
Conclusion: 该研究在反射生成领域取得了重要进展，通过创新的数据集构建和模型设计，实现了高质量的反射合成效果，为图像合成的完整性提供了重要支持。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解这些模型在视觉任务中的工作原理仍然困难，现有可解释性工具往往关注孤立组件或面向专家分析，缺乏对完整推理流程的端到端引导式理解

Method: 开发了ViT-Explainer系统，提供Vision Transformer推理的集成可视化，包括动画演示、分块级注意力覆盖和视觉适配的Logit Lens，支持引导式和自由探索两种模式

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能帮助用户解释和理解Vision Transformer的行为

Conclusion: ViT-Explainer填补了Vision Transformer端到端可解释性的空白，通过交互式可视化帮助用户更好地理解模型推理过程
...
TL;DR: UniDriveVLA通过专家解耦的混合Transformer架构解决自动驾驶中VLA模型的空间感知与语义推理冲突，在nuScenes和Bench2Drive上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的视觉-语言-动作模型面临空间感知与语义推理的冲突：直接采用2D VLM空间感知有限，增强3D表示又会损害VLM的推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。
...
Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛感知、预测和理解任务上表现强劲，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构有效解决了自动驾驶VLA模型中空间感知与语义推理的冲突，实现了统一的端到端模型，在多个基准测试和任务上展现了优越性能。
...
TL;DR: SCALE是一个轻量级确定性框架，通过语义和置信度感知的列表能量排序解决零样本骨架动作识别问题，避免了对齐方法的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖骨架-文本对齐，当动作名称无法充分描述细粒度动态或未见类别语义混淆时，这种方法很脆弱。需要更鲁棒的框架来处理语义模糊和类别混淆问题。

Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和解码器；2) 引入语义和置信度感知的列表能量损失，强调语义相似的困难负样本，结合后验不确定性调整决策边界；3) 使用潜在原型对比目标对齐后验均值与文本派生潜在原型。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于之前的VAE和对齐基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过能量排序框架有效解决了零样本骨架动作识别中的语义模糊和类别混淆问题，提供了比对齐方法更鲁棒的解决方案，同时保持了轻量级和确定性特性。
...
Method: 提出Omni123模型，将文本、图像和3D表示为共享序列空间中的离散标记。采用交错X到X训练范式，协调异构配对数据集上的多样化跨模态任务，无需完全对齐的文本-图像-3D三元组。通过自回归序列中遍历语义-视觉-几何循环（如文本到图像到3D到图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过利用2D数据作为几何先验，在单一框架中统一文本到2D和文本到3D生成，为3D原生基础模型提供了新的解决方案，能够更好地保持几何一致性，推动了多模态3D世界模型的发展。
...
TL;DR: VOID是一个视频对象移除框架，专门处理物体移除后需要改变下游物理交互的复杂场景，通过生成物理上合理的反事实结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理物体"背后"的内容修复和外观层面的伪影，但当移除的物体与其他物体有显著交互（如碰撞）时，现有模型无法修正这些交互，会产生不合理的结果。

Method: 使用Kubric和HUMOTO生成包含反事实对象移除的配对数据集；推理时使用视觉语言模型识别受移除物体影响的场景区域，然后用视频扩散模型生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，相比现有视频对象移除方法，VOID能更好地保持移除对象后场景动态的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，特别是在密集预测场景中，如开放词汇分割。现有方法通常使用滑动窗口策略处理高分辨率图像，虽然提高了精度但计算成本很高。

Method: 提出SPAR（Single-Pass Any-Resolution ViT），通过特征回归损失将细粒度滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中，无需架构修改或像素级监督。

Result: 在开放词汇分割任务中，SPAR将单次推理基线提升了高达10.5 mIoU，甚至超过了教师模型的性能，证明了在高分辨率推理中的有效性。

Conclusion: SPAR提供了一种高效的高分辨率推理方法，通过知识蒸馏实现了分辨率无关的密集特征提取，在保持计算效率的同时显著提升了空间理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖模型微调或精选数据集，这会降低生成质量或限制可扩展性。需要一种训练无关、模块化的安全控制方法，既能保持生成质量，又能实现可扩展的安全控制。

Method: 利用视觉语言基础模型作为现成的监督信号，通过在每个采样步骤注入梯度反馈到干净潜在估计中，将安全引导构建为基于能量的采样问题。该方法与扩散模型和流匹配模型兼容。

Result: 在NSFW红队测试基准上达到最先进的鲁棒性，能够有效进行多目标引导，同时在良性非目标提示上保持高质量生成。
...
TL;DR: 本文提出了一种可操控的视觉表示方法，通过早期融合文本信息到视觉编码器中，使视觉特征能够根据自然语言提示关注图像中的特定概念，同时保持通用视觉任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的预训练视觉Transformer（如DINOv2和MAE）虽然能提供通用图像特征，但倾向于关注最显著的视觉线索，无法引导它们关注图像中不太突出的感兴趣概念。而多模态大语言模型虽然可以通过文本提示引导，但其表示往往是语言中心的，在通用视觉任务上效果不佳。

Method: 提出可操控的视觉表示，通过轻量级交叉注意力机制将文本直接注入视觉编码器的各层（早期融合），而不是像大多数视觉语言模型那样在编码后进行融合（晚期融合）。这使得全局和局部特征都能通过自然语言进行操控。

Result: 引入了衡量表示可操控性的基准测试，证明该方法可以使视觉特征聚焦于图像中的任何目标对象，同时保持底层表示质量。在异常检测和个性化对象识别任务上，匹配或超越了专用方法，并展现出对分布外任务的零样本泛化能力。

Conclusion: 可操控的视觉表示方法成功解决了现有视觉表示方法的局限性，既能通过文本提示引导关注特定概念，又保持了通用视觉任务的有效性，为视觉表示学习提供了新的方向。
...
Method: 引入主体状态令牌作为潜在变量持续捕捉每个主体的状态，通过空间偏置机制联合建模状态令牌和视频潜在表示，解耦全局视频帧渲染与个体动作控制的主体更新

Result: 在Melting Pot基准测试中评估，展示了首个能够同时在46个多样化环境中控制最多7个玩家的视频世界模型，在动作跟随准确性和身份一致性方面有显著改进

Conclusion: ActionParty通过主体状态令牌和空间偏置机制成功解决了多主体视频世界模型中的动作绑定问题，实现了对复杂交互中主体的鲁棒自回归跟踪
...
TL;DR: 提出了一种快速表面配准方法，将表面网格视为概率测度，使用切片Wasserstein距离衡量差异，并开发了AdamFlow优化算法在概率空间进行优化。
...
Result: 理论分析了AdamFlow的渐近收敛性，并在多种解剖结构的仿射和非刚性表面配准中实证展示了其优越性能。

Conclusion: 该方法通过将表面配准转化为概率分布优化问题，结合高效的切片Wasserstein距离和AdamFlow优化器，实现了既高效又鲁棒的表面配准。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D化身建模面临保真度与泛化性的权衡：基于多视角工作室数据的方法保真度高但泛化性差，而基于大规模野外数据的方法泛化性好但质量低。需要一种既能保持高质量又能广泛泛化的解决方案。

Method: 采用预训练/后训练范式：首先在100万个野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表达性和保真度。模型能够泛化到不同发型、服装和人口统计特征，同时提供精细的面部表情和手指级关节控制。

Result: LCA实现了高质量、全身3D化身建模，能够前馈方式泛化到世界规模人群，支持高效推理。模型展现出对重新光照和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性，尽管没有直接监督。

Conclusion: LCA通过创新的预训练/后训练范式成功解决了3D化身建模中保真度与泛化性的长期权衡问题，为大规模高质量3D化身建模提供了有效解决方案，展现出强大的泛化能力和高质量输出。
...
TL;DR: 论文提出了场景式视觉定位新任务和RSC基准，包含段落级描述和难度标签，并开发了课程学习方法ScenGround，揭示了现有模型在深层推理上的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型常通过匹配显著命名类别即可成功。本文探索更挑战性的场景式视觉定位，要求根据角色、意图和关系上下文推断目标，而非显式命名。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度文本描述对象角色、用户目标和上下文线索，标注了独特性、杂乱度、大小、重叠和位置等难度标签。提出ScenGround课程推理方法，结合监督预热和难度感知强化学习。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外样本。实验显示场景式查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性子集上的性能并能迁移到标准基准。

Conclusion: 场景式视觉定位是现有基准的补充和挑战性扩展，RSC基准支持细粒度分析，ScenGround方法展示了课程学习的有效性，揭示了模型在深层推理能力上的局限性。
...
TL;DR: ModMap是一个多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角相关建模，在SiM3D基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立处理不同视角，缺乏对多视角和多模态数据的有效整合。工业场景中的3D异常检测需要同时利用深度信息和多视角关系来提升检测精度。

Method: 1. 采用跨模态特征映射范式，学习跨模态和跨视角的特征映射；2. 通过特征级调制显式建模视角依赖关系；3. 引入跨视角训练策略，利用所有可能的视角组合；4. 通过多视角集成和聚合进行异常评分；5. 训练并公开了针对工业数据集的基础深度编码器。

Result: 在SiM3D基准测试（首个多视角多模态3D异常检测与分割基准）上，ModMap取得了最先进的性能，大幅超越先前方法。

Conclusion: ModMap通过原生多视角多模态框架，有效整合了跨模态特征映射和视角依赖建模，显著提升了3D异常检测与分割的性能，为工业应用提供了有效的解决方案。
...
TL;DR: 该研究通过从AAA游戏中提取大规模动态数据集，解决了生成式逆渲染和正向渲染在真实世界场景中的领域差距问题，并提出了基于VLM的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时间连贯性方面的局限性，限制了生成式逆渲染和正向渲染在真实世界场景中的扩展应用。

Method: 采用新颖的双屏拼接捕捉方法，从视觉复杂的AAA游戏中提取了400万连续帧的同步RGB和五个G-buffer通道数据，涵盖多样化场景、视觉效果和环境条件。

Result: 基于该数据集微调的逆渲染器实现了优越的跨数据集泛化能力和可控生成，VLM评估协议与人类判断高度相关，正向渲染器支持通过文本提示编辑AAA游戏风格。

Conclusion: 该大规模动态数据集和评估协议显著推进了双向渲染技术的发展，为真实世界场景的逆渲染和可控视频生成提供了重要资源。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）对情感分析的重要性，创建了首个RLF数据集Lengthening，并提出了可解释指令调优框架ExpInstruct来提升LLMs对RLF的理解和解释能力。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特的强调风格——重复延长形式（RLF）多年来一直被忽视。研究者想要探究RLF对情感分析的重要性，以及语言模型是否能理解这种表达方式。

Method: 1) 创建首个多领域RLF数据集Lengthening（85万样本）；2) 提出两阶段可解释指令调优框架ExpInstruct；3) 提出统一方法来量化语言模型对非正式表达的理解能力。

Result: 1) RLF句子是富有表现力的表达，可作为文档级情感的特征；2) RLF对在线内容分析具有潜在价值；3) 微调的预训练语言模型在性能上可超越零样本GPT-4，但在解释性上不如；4) ExpInstruct能让开源LLMs在有限样本下达到零样本GPT-4的性能和解释性水平。

Conclusion: RLF对情感分析很重要且语言模型能够理解这种表达形式。提出的ExpInstruct框架能有效提升语言模型对RLF的理解和解释能力，为在线内容分析提供了新工具。
...
TL;DR: 通过强化学习和并行思考两种方法扩展推理token预算，用于竞争性编程。训练时强化学习提升验证准确率，测试时多轮并行思考管道分配token预算，最终系统在AetherCode难题上超越GPT-5-high。


<details>
  <summary>Details</summary>
Motivation: 竞争性编程需要大量推理token，但单次生成的完全注意力机制成本高昂。需要探索如何在训练和测试时有效扩展推理token预算，提高问题解决能力。

Method: 1. 训练时强化学习：验证准确率与生成推理token数呈对数线性关系，采用验证RL预热和随机裁剪优化训练轨迹。2. 测试时并行思考：多轮并行思考管道将token预算分配到多个线程和轮次，每轮包括生成、验证和精炼。3. 端到端训练：使训练目标与测试时结构匹配。

Result: 从Seed-OSS-36B开始，16线程×16轮次的完整系统平均每个问题使用760万token，在pass@1指标上匹配底层RL模型的oracle pass@16性能，并在AetherCode的456个难题上超越GPT-5-high。

Conclusion: 通过结合训练时强化学习和测试时并行思考，可以有效扩展推理token预算，显著提升竞争性编程问题的解决能力，为大规模语言模型推理提供可扩展框架。
...
TL;DR: M2-Verify是一个大规模多模态数据集，用于评估科学主张与多模态证据之间的一致性，包含46.9万个实例，覆盖16个领域，模型在复杂场景下表现不佳


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏规模、领域多样性和视觉复杂性，无法真实评估科学主张与多模态证据之间的一致性

Method: 从PubMed和arXiv收集数据构建M2-Verify数据集，包含超过46.9万个实例，覆盖16个领域，并通过专家审核进行严格验证

Result: 最先进模型在低复杂度医学扰动上达到85.8% Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%，专家评估还发现模型在生成科学解释时存在幻觉问题

Conclusion: M2-Verify数据集填补了现有基准测试的不足，展示了模型的局限性，提供了全面的使用指南，有助于推动科学主张一致性评估的研究
...
TL;DR: 该研究提出了一种特征增强框架，通过添加可解释信号（如回复长度、拒绝指标、毒性分数和语义相似度）来改进语言模型中人类偏好的学习，在Anthropic HHRLHF数据集上显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型学习人类偏好的方法存在局限，因为奖励建模依赖于微妙的主观比较而非清晰标签，现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74）。

Method: 提出混合方法：在文本表示基础上增强可解释特征（回复长度、拒绝指标、毒性分数、提示-回复语义相似度），使用SHAP和LIME进行细粒度可解释性分析，并在Anthropic HHRLHF数据集上评估10个不同大语言模型。

Result: 特征增强框架在所有模型上都带来一致改进，最高达到0.84 ROC AUC，显著提高成对准确率，DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全和支持性框架而非孤立关键词。

Conclusion: 通过增强可解释特征可以更好地捕捉人类判断的多维性质，改善语言模型中的偏好学习。虽然单个特征边际效应弱，但它们的交互作用影响偏好学习，为理解模型决策提供了新视角。
...
TL;DR: 论文提出了Reasoning Memory框架，通过检索和重用程序性知识来增强推理模型，在数学、科学和编程基准测试中显著优于现有方法。
...
Conclusion: Reasoning Memory通过广泛的程序性知识覆盖以及分解和检索设计，实现了程序性知识的有效提取和重用，显著提升了语言模型在复杂推理任务上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答实体相关的真实问题，但其内部机制尚不清楚。本研究旨在探索语言模型处理实体相关事实的内部机制，特别是如何检索和表示实体信息。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA数据集上通过因果干预验证。选取200个实体，分析神经元分布，进行负向消融和受控注入实验。

Result: 实体选择性神经元集中在早期层；负向消融导致实体特异性遗忘；在占位符处注入神经元激活可改善答案检索；许多实体通过单个神经元即可恢复实体一致性预测；对别名、缩写、拼写错误和多语言形式具有鲁棒性。

Conclusion: 研究识别了稀疏、可因果操作的访问点，用于分析和调节实体条件化事实行为，支持紧凑实体检索而非纯粹深度渐进富集的解释，但效果并非普遍，对热门实体覆盖更高。
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被用来区分自动化翻译过程与需要认知努力的翻译过程，但如何确定停顿阈值一直存在争议。本文旨在比较现有方法并提出更优的解决方案。

Method: 比较了三种近期计算停顿阈值的方法，并提出并评估了一种新的生产单元断点计算方法。

Result: 通过比较分析，提出了新的生产单元断点计算方法，并对其进行了评估。

Conclusion: 新的生产单元断点计算方法在区分自动化翻译过程与认知努力翻译过程方面可能提供更准确的阈值确定。
...
TL;DR: 本文提出基于预测效度的基准测试框架评估方法，通过收集大规模项目级数据集WILD，结合改进的多维项目反应理论和自适应项目选择，能够高效预测模型在未见任务上的表现，显著降低评估成本。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型基准测试数量庞大但效率低下，研究表明模型性能可由少量潜在能力因子解释。需要更高效、有原则的基准测试方法，但缺乏比较不同方法质量的统一标准。本文基于预测效度，主张基准测试框架的质量应基于其预测模型在未见任务上表现的效率。

Method: 收集WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务（来自27个数据集）的109,564个独特项目-模型响应对。采用改进的多维项目反应理论模型，结合基于最优实验设计的自适应项目选择方法，并引入成本感知折扣因子优化选择标准。

Result: 改进方法能在仅观察16个项目后，以小于7%的平均绝对误差预测112个保留基准测试任务的表现。通过成本感知折扣因子，达到7% MAE所需的总token数从141,000减少到22,000，评估成本降低85%。

Conclusion: 基于预测效度的基准测试框架评估方法可行且高效，结合多维项目反应理论和自适应项目选择能显著降低大语言模型评估成本，为更高效、有原则的基准测试提供了新思路。
...
TL;DR: 该研究使用随机森林模型分析印地语同义词的词嵌入，发现即使语义无关，也能根据使用模式区分梵语和波斯-阿拉伯语来源的词，表明上下文编码了词源信号。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索同义词现象，特别是理论上不应存在的绝对同义词。虽然同义词可能表达相同概念，但可能反映不同视角或文化关联，这些主张很少得到定量验证。印地语中梵语和波斯-阿拉伯语借词形成的同义词对为研究提供了理想案例。

Method: 使用随机森林模型训练印地语同义词的词嵌入数据，这些同义词来自梵语和波斯-阿拉伯语两种不同来源。模型旨在仅基于分布数据（不考虑语义内容）分类词的来源。

Result: 随机森林模型成功区分了梵语和波斯-阿拉伯语来源的词，即使这些词在语义上无关。这表明使用模式保留了词源痕迹，上下文编码了词源信号。

Conclusion: 研究提供了定量证据，表明上下文编码词源信号，同义词可能反映与来源相关的微妙但系统性的区别。支持同义词可以提供不同视角的观点，词源相关的词可能形成不同的概念子空间，创造由历史来源塑造的新型语义框架。结果突显了上下文在捕捉超越传统语义相似性的细微区别方面的能力。
...
TL;DR: 研究发现：在AI生成内容中，引用粒度（句子级、段落级或文档级）对模型性能有显著影响，精细引用反而会降低归因质量16-276%，段落级引用效果最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索引用粒度对归因生成质量的影响。虽然细粒度引用（句子级）通常被认为便于人工验证，但其对模型性能的影响尚未得到充分研究，需要系统分析不同引用粒度如何影响模型的归因准确性和生成可靠性。

Method: 研究方法包括：1）分析四个不同规模的模型（8B-120B参数）；2）比较不同引用粒度（句子级、段落级、文档级）对归因质量的影响；3）评估不同粒度下模型的归因准确性和答案正确性；4）分析模型规模与引用粒度之间的交互效应。

Result: 研究结果显示：1）强制细粒度引用会降低归因质量16-276%；2）归因质量在中间粒度（段落级）达到峰值；3）细粒度引用破坏了证据归因所需的语义依赖关系；4）过粗的引用引入干扰噪声；5）模型规模越大，细粒度约束的负面影响越显著；6）最优引用粒度既能提升归因质量，又能保持或改善答案正确性。

Conclusion: 结论是：仅为了便于人工验证而优化细粒度引用忽视了模型约束，会损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，段落级引用是最佳平衡点。
...
TL;DR: 该论文对LLM中过度自信表达进行电路级机制分析，发现特定MLP块和注意力头在中间到后期层中写入过度自信信号，并通过针对性干预改善校准


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生事实错误的答案，并且会过度自信地表达这些错误答案，这种过度自信的表达会误导用户并削弱置信度分数作为可靠不确定性信号的作用，但其内部机制尚不清楚

Method: 围绕三个轴进行电路级机制分析：1)将表达性置信度捕获为可微分的内部信号；2)识别导致过度自信的因果电路；3)利用这些见解进行针对性推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验

Result: 发现一组紧凑的MLP块和注意力头，主要集中在中间到后期层，在最终标记位置一致地写入过度自信信号。针对性推理时干预这些电路显著改善了校准效果

Conclusion: LLM中的表达性过度自信是由可识别的内部电路驱动的，可以通过针对性干预来缓解
...
TL;DR: 该研究通过计算分析方法，对波斯诗歌中的象征系统进行量化研究，发现波斯诗歌的象征核心相对稀疏，但随时间推移内部权重和连接关系发生变化。


<details>
  <summary>Details</summary>
Motivation: 波斯诗歌通常通过反复出现的象征符号（如酒器、花园、火焰等）被记忆，而非仅通过情节。现有的计算研究倾向于将这些材料简化为孤立的词语或宽泛的文档语义，忽略了波斯诗学中一个实用的组织单位：相关形式以家族形式传播，并通过反复出现的关系获得力量。

Method: 使用包含129,451首诗歌的语料库，将反复出现的形式整合为可追溯的家族，将意象材料与神圣和宫廷参考分开，并在多层图中映射它们的关系。分析跨越11个伊斯兰世纪的时间段。

Result: 象征核心相对稀疏，指称部分更密集，两者之间的连接区域是选择性的而非扩散的。某些家族（如夜、日、土）广泛分布，酒器、花园空间、火焰和抒情声音在后期增强，而声望编码和英雄宫廷词汇在早期更突出。模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。中心位置也发生变化：苏菲长袍获得晚期突出地位，祝福和紫罗兰衰退，而酒杯在整个时间线上保持中心地位。

Conclusion: 在这个语料库中，波斯象征主义不像是一个固定的剧目，而更像是一个长期存在的系统，其内部权重和连接关系随时间变化。
...
TL;DR: 论文提出了一个名为"Magic, Madness, Heaven, Sin"的框架，用于系统分析大语言模型输出变化的评估，将任务分为四个规范性语境，并探讨了不同目标间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语零散且不统一，主要原因是任务的规范性目标很少被明确说明。作者希望建立一个统一的框架来系统分析输出变化，并揭示不同目标间的潜在冲突。

Method: 提出了Magic, Madness, Heaven, Sin框架，将输出变化建模在"同质性-异质性"轴上，并将任务组织到四个规范性语境中：认知性（事实性）、交互性（用户效用）、社会性（代表性）和安全性（鲁棒性）。分析了每个语境下的失败模式和词汇，并应用框架分析了所有成对的跨语境交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能会无意中损害人口统计代表性或创造性多样性等目标。展示了不同规范性目标之间的权衡关系。

Conclusion: 应该基于任务目标进行上下文感知的输出变化评估，将其重新定义为由任务目标塑造的属性，而不是模型的固有特征。需要更细致地理解不同规范性目标间的相互作用。
...
<details>
  <summary>Details</summary>
Motivation: 研究指令式遗忘范式是否适用于扩散模型等非语言生成模型，探索自然语言指令在图像生成中的概念抑制效果

Method: 通过多概念多提示变体的控制实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，评估指令式遗忘效果

Result: 扩散模型无法通过自然语言遗忘指令有效抑制目标概念，注意力机制中目标概念表征在整个生成过程中持续存在

Conclusion: 扩散模型存在指令级遗忘的根本性限制，有效遗忘需要超越推理时语言控制的干预措施
...
TL;DR: 提出模型融合框架，通过合并临床基础模型和通用指令模型来解决LLMs在医疗领域微调时的灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗临床文档应用中面临灾难性遗忘问题，即在医疗数据集微调时会丧失大量指令跟随能力，这限制了通用LLMs在临床应用的采用

Method: 使用基于插值的模型融合方法，将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，以平衡临床任务性能和指令跟随能力

Result: 融合模型能有效缓解灾难性遗忘，保留临床领域专业知识，同时维持指令跟随能力，在有限监督下(如64-shot vs 256-shot)达到与完全微调基线相当的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的人设中心记忆系统在多智能体设置中存在信息丢失和场景适应性差的问题，导致性能不佳

Method: 提出DeltaMem系统，将人设记忆管理转化为单智能体端到端任务；构建用户-助手对话数据集和操作级记忆更新标签；引入基于记忆的Levenshtein距离作为奖励函数，设计专门的强化学习框架

Result: DeltaMem（包括无训练和RL训练版本）在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中均优于所有产品级基线

Conclusion: DeltaMem通过单智能体端到端框架和强化学习优化，有效解决了人设记忆管理中的信息丢失和场景适应性问题，显著提升了长期记忆管理性能
...
TL;DR: Swift-SVD是一个激活感知的闭式压缩框架，用于减少大语言模型的内存和带宽需求，通过增量协方差聚合和单次特征值分解实现理论最优、高效且数值稳定的层间低秩近似。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的部署受到静态权重和动态键值缓存的内存和带宽需求的限制。现有的SVD压缩方法存在两个关键问题：一些方法在重建误差上不是最优的，而另一些方法理论最优但实际效率低下。

Method: 提出Swift-SVD框架：1）增量聚合给定输入批次输出激活的协方差；2）聚合后执行单次特征值分解；3）使用有效秩分析局部层间可压缩性；4）设计动态秩分配策略，联合考虑局部重建损失和端到端层重要性。

Result: 在六个LLM和八个数据集上的广泛实验表明，Swift-SVD优于最先进的基线方法，在实现最优压缩精度的同时，端到端压缩时间加速3-70倍。

Conclusion: Swift-SVD提供了一个同时保证理论最优、实际效率和数值稳定的激活感知闭式压缩框架，有效解决了大语言模型部署中的内存和带宽限制问题。
...
TL;DR: 印尼全国教师调查显示AI在K-12教育中应用增加但分布不均，主要用于减轻教学准备负担，但仍面临通用输出、基础设施和情境适配等挑战


<details>
  <summary>Details</summary>
Motivation: 尽管AI在印尼课堂中开始应用，但缺乏大规模、以教师为中心的实际使用证据和支持需求研究，这阻碍了开发适合当地情境的AI系统和政策

Method: 对全国349名K-12教师（涵盖小学、初中、高中）进行问卷调查，收集教师使用AI的实践经验和需求数据

Result: 发现AI在教学法、内容开发和教学媒体方面的使用增加但分布不均：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，东部印尼教师感知价值更高；教师主要用AI减轻教学准备负担（评估、课程规划、材料开发），但通用输出、基础设施限制和情境适配不足阻碍有效课堂整合
...
TL;DR: 论文分析了现有声明验证基准的局限性，发现这些基准主要测试证据提取能力，而多句子合成和数值推理能力严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对这些基准实际测试推理能力的系统理解。研究者希望了解现有验证基准真正测试的是什么样的推理能力，以及这些基准存在的系统性偏差。

Method: 使用GPT-4o-mini为9个数据集的24K个声明验证示例生成结构化推理轨迹，分析推理类型分布。使用紧凑的1B参数推理验证器，进一步分析五种错误类型及其在不同领域的分布特征。

Result: 发现直接证据提取占主导地位（约70%），而多句子合成和数值推理严重不足（分别约10%和5%）。数据集层面存在显著偏差：某些数据集几乎只测试词汇匹配，而其他数据集约一半情况需要信息合成。错误模式因领域而异：通用领域验证主要受词汇重叠偏差影响，科学验证由过度谨慎主导，数学验证则主要是算术推理失败。

Conclusion: 高基准分数主要反映检索+蕴含能力，而非复杂推理能力。需要构建更具挑战性的评估套件，更好地测试验证系统所需的推理能力。论文为构建更全面的评估基准提供了具体建议。
...
TL;DR: PRCCF框架通过人物角色引导的检索和因果感知的认知过滤，提升情感支持对话中的上下文理解和情感推理能力


<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在深度上下文理解方面存在不足，需要更有效地支持情感推理和回应生成

Method: 提出PRCCF框架，包含人物角色引导的检索机制（联合建模语义兼容性和角色对齐）和因果感知的认知过滤模块（优先处理因果相关的外部知识）
...
TL;DR: 研究发现：在相同问题集上，使用DeepSeek-R1-0528和gpt-oss-120b生成的CoT轨迹进行SFT训练时，虽然DeepSeek数据训练损失更低，但泛化性能反而更差，揭示了训练损失与泛化性能之间的悖论。


<details>
  <summary>Details</summary>
Motivation: 探索不同来源的CoT轨迹如何影响大语言模型的泛化性能，特别是当两个竞争模型在相同问题集上表现相当时，其生成的轨迹对SFT训练效果的影响机制。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b生成的已验证CoT轨迹进行对比研究，控制问题集相同。通过多维度分析包括token级SFT损失和步骤级推理行为，识别推理模式差异，并提出基于分支频率过滤轨迹的改进方法。

Result: DeepSeek-R1数据训练损失更低但泛化性能更差，原因是其推理轨迹发散且分支多，导致模型继承低效探索行为。过滤频繁分支轨迹后，DeepSeek-R1子集的训练在AIME25上提升5.1%，BeyondAIME上提升5.5%，五个基准平均提升3.6%。

Conclusion: CoT轨迹的推理模式（收敛vs发散）对SFT泛化性能有重要影响，训练损失不能直接反映泛化能力。通过过滤低效探索轨迹可以显著提升推理性能，为高质量SFT数据选择提供了新视角。
...
TL;DR: EndoASR是一个专门为胃肠内窥镜设计的领域自适应语音识别系统，通过两阶段适应策略显著提升临床环境下的转录准确性和实时性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动语音识别系统在胃肠内窥镜临床应用中的可靠性有限，主要受限于领域特定术语和复杂声学条件，需要专门针对内窥镜工作流程优化的解决方案。

Method: 开发了两阶段适应策略：基于合成内窥镜报告进行领域特定语言建模和噪声鲁棒性优化，构建了EndoASR系统，支持实时部署。

Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%；前瞻性多中心研究中，字符错误率从16.20%降至14.97%，医学术语准确率从61.63%提升至84.16%，实时因子为0.005，模型参数仅220M。

Conclusion: EndoASR证明了领域自适应语音识别可以作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中表现出稳定性能，并能有效提升下游结构化信息提取质量。
...
<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长期复杂任务（如多轮对话、游戏、科学发现）的核心模块，但目前各种记忆方法缺乏在相同实验设置下的系统性比较

Method: 1) 从高层视角总结包含所有现有智能体记忆方法的统一框架；2) 在两个知名基准上广泛比较代表性记忆方法；3) 利用现有方法模块设计新的记忆方法
...
TL;DR: 该论文提出了一种人机协作框架，通过LLM推理和置信度路由机制处理越南语语音情感识别中的模糊样本问题，在低资源环境下取得了86.59%的准确率。


<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临两大挑战：模糊的声学模式和缺乏可靠的标注数据，特别是在现实世界中情感边界不清晰的情况下。传统数据驱动模型难以处理这些模糊样本。

Method: 提出人机协作框架，核心是LLM推理机制。使用声学特征模型提供置信度和特征级证据，通过置信度路由区分简单和模糊样本，将不确定案例委托给LLM进行基于人类标注行为的结构化规则推理。采用迭代优化策略，通过错误分析和规则更新持续改进系统性能。

Result: 在包含2,764个样本的越南语语音数据集（三种情感：平静、愤怒、恐慌）上进行实验，标注者间一致性高（Fleiss Kappa = 0.8574）。该方法达到86.59%的准确率，Macro F1分数约0.85-0.86，在处理模糊和难以分类的案例方面表现出色。

Conclusion: 这项工作强调了数据驱动模型与人类推理相结合的重要性，为低资源环境下的语音情感识别提供了一种鲁棒且模型无关的方法。人机协作框架能有效处理模糊样本，提高系统在现实条件下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前在线通信中的毒性内容检测方案经常无意中屏蔽有价值的信息，包括医学术语和少数群体相关内容。本文旨在开发一种更细致的保加利亚语毒性检测方法，在识别有害内容的同时保留重要信息。

Method: 研究采用两种方法：1) 构建保加利亚语潜在毒性词汇的本体模型；2) 创建包含4,384个手动标注句子的数据集，涵盖毒性语言、医学术语、非毒性语言和少数群体相关术语四个类别。基于此数据集训练BERT模型进行毒性分类。

Result: 训练的BERT模型在毒性语言分类任务上达到了0.89的宏观F1分数。该模型可直接应用于实际环境，并可作为毒性内容检测系统的组件进行集成。

Conclusion: 本文提出的方法为保加利亚语毒性内容检测提供了更细致的解决方案，能够在识别有害内容的同时避免误判医学术语和少数群体相关内容，具有在各种在线平台和内容审核系统中应用的潜力。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文的动态数学推理基准测试，通过13类逻辑分类和证明草图引导的干扰项设计，评估LLMs的研究级数学能力，结果显示当前最佳模型准确率仅43.5%，在防替换机制下表现更差。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准测试存在合成设置和数据污染问题，无法真实评估大语言模型的研究级数学能力。随着LLMs越来越多地融入科学工作流程，需要建立一个能反映真实数学推理能力的评估基准。

Method: 基于模型训练截止后发表的arXiv论文构建动态选择题基准；引入13类逻辑分类体系（蕴含、等价、存在性、唯一性等）；采用证明草图引导的干扰项生成管道，利用高层次证明策略构建看似合理但错误的选项；设计防替换机制区分答案识别与实质推理。

Result: 基准测试远未饱和：最佳模型Gemini-3.1-pro-preview准确率仅43.5%；在防替换评估下准确率大幅下降，GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）；证明草图访问能带来一致的准确率提升。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的测试平台，用于研究LLMs在研究级数学推理方面的能力，揭示了当前模型在真实数学推理任务上的局限性，并证明模型能够利用高层次证明策略进行推理。
...
TL;DR: 本文提出基于指令微调和离散控制标记的领域无关可控自动文本简化框架，发现小模型在控制性方面可媲美大模型，但可靠控制性取决于训练数据中目标属性的变化程度，同时指出现有评估指标不足以衡量控制效果。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化(CATS)通常将可控性视为解码问题，并使用不能反映控制程度的指标进行评估。研究发现CATS的可控性受到数据和评估方法的显著限制，需要更好的框架来指导模型达到目标可读性水平和压缩率。

Method: 提出基于指令微调和离散控制标记的领域无关CATS框架。使用三个模型家族(Llama、Mistral、Qwen，1-14B)和四个领域(医学、公共管理、新闻、百科文本)进行实验，通过采样和分层分析数据分布匹配问题。

Result: 小模型(1-3B)在控制性方面可媲美大模型，但可靠控制性取决于训练数据是否编码足够的目标属性变化。可读性控制(FKGL、ARI、Dale-Chall)学习一致，而压缩控制因现有语料库中信号变化有限而表现不佳。标准简化和相似性指标不足以衡量控制效果。

Conclusion: 需要基于误差的测量方法来评估目标-输出对齐，朴素的数据分割会引入分布不匹配问题，影响训练和评估效果。数据质量和评估方法是实现可靠可控文本简化的关键因素。
...
TL;DR: DEFT框架通过数据筛选和分布引导，使用差异分布奖励从原始数据中筛选高质量子集，提升对齐效率和性能，同时减少训练时间并保持泛化能力。


<details>
  <summary>Details</summary>
Motivation: RLHF使用PPO等方法对齐大语言模型与人类价值观，但成本高、不稳定。现有替代方法需要大量数据学习偏好，可能削弱LLMs的泛化能力。需要更高效的对齐框架来提升性能同时减少泛化能力损失。

Method: 提出分布引导高效微调(DEFT)框架，包含数据筛选和分布引导。通过计算语言模型输出分布与偏好数据差异分布之间的差异分布奖励，从原始数据中筛选高质量子集，并将该奖励整合到现有对齐方法中指导模型输出分布。

Result: 实验结果表明，DEFT增强的方法在齐能力和泛化能力方面均优于原始方法，且训练时间显著减少。

Conclusion: DEFT框架通过数据筛选和分布引导机制，有效提升了语言模型对齐的效率和性能，同时保持了模型的泛化能力，为高效对齐提供了新思路。
...
TL;DR: PLOT使用最优传输理论改进大语言模型的偏好学习，通过token级损失函数提升对齐性能，同时保持模型稳定性和流畅性


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及缺乏全局token关系建模等问题，需要更有效的方法

Method: 提出PLOT方法，将偏好学习建模为最优传输问题，通过token级损失函数对齐模型输出与人类偏好，同时保持原始语言模型分布，利用token嵌入捕捉语义关系实现全局优化

Result: 在人类价值观和逻辑问题解决两个偏好类别、七个子偏好的实验中，PLOT持续提升对齐性能，同时保持流畅性和连贯性

Conclusion: 最优传输为偏好学习提供了理论基础框架，为LLM偏好学习提供了新见解，PLOT证明了这一方法的有效性
...
TL;DR: 本文提出自适应占位符补全（APC）框架，通过在高熵位置输出显式占位符来降低代码补全的编辑成本，相比传统硬补全（HC）方法可减少19%-50%的编辑成本。


<details>
  <summary>Details</summary>
Motivation: 研究发现传统LLM代码补全采用硬补全范式，即使在上下文不足时也强制生成完整代码。对300万真实交互的分析显示，61%的生成建议要么在接收后被编辑，要么因与用户后续代码相似度超过80%而被拒绝，表明模型经常在特定token位置做出错误预测。

Method: 提出自适应占位符补全（APC）框架，将代码补全形式化为不确定性下的成本最小化问题。基于"填充占位符比纠正错误成本更低"的观察，证明存在临界熵阈值，超过该阈值时APC的期望成本严格低于HC。通过从真实编辑日志构建训练数据，设计基于成本的奖励函数进行强化学习。

Result: 在1.5B-14B参数模型上的广泛评估表明，APC将期望编辑成本从19%降低到50%，同时保持标准HC性能。提供了不确定性感知代码补全的理论基础和实践训练框架。

Conclusion: 自适应占位符补全框架证明了自适应弃权可以在不牺牲传统补全质量的情况下端到端学习，为代码补全系统提供了更高效的协作范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于强调特征融合，忽视了噪声特征的不确定性和细粒度推理问题，需要更鲁棒和具有上下文建模能力的解决方案。
...
TL;DR: 基于法国心衰队列的研究显示，实体感知的多模态transformer模型在短期死亡率预测中表现最佳，而大语言模型在不同模态和解码策略下表现不稳定。


<details>
  <summary>Details</summary>
Motivation: 准确预测心衰患者的短期死亡率具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要探索不同方法（包括文本、结构化数据、多模态和大语言模型）在心衰预后预测中的表现。

Method: 在法国心衰队列上评估了四种方法：1）仅文本的transformer模型；2）仅结构化数据的模型；3）多模态融合方法（文本+结构化变量）；4）基于大语言模型的方法。比较了不同嵌入表示（CLS嵌入与实体级表示）以及不同解码策略的效果。

Result: 1）用实体级表示丰富临床文本比仅用CLS嵌入能改善预测性能；2）监督式多模态融合方法（文本+结构化变量）获得最佳整体性能；3）大语言模型在不同模态和解码策略下表现不一致，仅文本提示优于结构化或多模态输入。

Conclusion: 实体感知的多模态transformer模型为心衰短期预后预测提供了最可靠的解决方案，而当前的大语言模型提示方法在临床决策支持方面仍有限制。
...
TL;DR: 本文提出了一种神经符号方法，结合fastText文本嵌入和符号概念特征（体裁、主题、说服技巧）来检测宣传性新闻，提高分类鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 宣传性新闻特别阴险，因为它们将导向性信息与事实报道混合，使其看起来像可靠新闻。现有的基于BERT等语言模型的方法虽然有效，但由于数据收集偏差容易过拟合训练数据集。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧，以增强分类鲁棒性和对新来源的泛化能力。

Result: 结果显示该方法优于等效的纯文本方法，消融研究和可解释性分析证实了添加特征的益处。

Conclusion: 神经符号方法结合文本嵌入和概念特征能有效提高宣传性新闻检测的鲁棒性和泛化能力，避免过拟合问题。
...
TL;DR: 提出了一种基于置换多面体交换距离最小化的数学框架，用于评估语言中词序变体的最优性程度，发现跨语言手势至少77%最优，并引入二次分配问题作为统一多种优化问题的理论框架。


<details>
  <summary>Details</summary>
Motivation: 研究语言中词序变体是否遵循交换距离最小化原则，即给定源顺序，在置换多面体中距离更近的词序应该成本更低、更可能出现。

Method: 建立基于置换多面体交换距离最小化的数学框架，通过测量词序变体的最优性程度来评估语言现象，并引入二次分配问题作为统一的理论框架。

Result: 跨语言手势至少达到77%的最优性，多次达到最优的情况不太可能是偶然，为研究通信系统中词序或手势顺序的最优性奠定了理论基础。

Conclusion: 提出了一个统一的数学框架来研究语言中的优化问题，引入二次分配问题作为多种优化问题的统一理论，并提出了最优分配的一般原则，将交换距离最小化等多种语言原则统一起来。
...
TL;DR: 论文研究机器翻译基准数据集的质量保证方法，提出三步自动化质量评估流程，发现数据集质量与翻译错误率相关，并发布清理后的数据集和代码。
...
Method: 采用三步自动化质量保证方法：1）结构语料库审计与针对性修复；2）使用神经度量COMET进行质量分析，比较DeepL、ChatGPT、Google等翻译服务；3）基于LLM的跨度级翻译错误分析。

Result: 趋势一致：COMET分数较低的数据集在跨度级别表现出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上与人编辑样本对比显示相同趋势。

Conclusion: 自动化质量保证提供实用、可扩展的指标，有助于优先审查，补充而非替代人工黄金标准。作者发布了清理/校正后的EU20数据集和可复现代码。
...
TL;DR: SAFE框架通过可验证的实体序列替代无根据的思维链，在训练时验证消除噪声监督，在推理时实时检测无根据步骤，显著提升多跳问答的严谨性。


<details>
  <summary>Details</summary>
Motivation: 多跳问答基准经常奖励大型语言模型虚假的正确性，掩盖了无根据或有缺陷的推理步骤，需要转向更严谨的推理验证。

Method: 提出SAFE动态基准框架，包含两个阶段：1) 训练时验证：建立原子错误分类法和基于知识图谱的验证流程，消除标准基准中的噪声监督；2) 推理时验证：基于验证数据集训练的反馈模型实时检测无根据步骤。

Result: 识别出高达14%的实例为不可回答，平均准确率提升8.4个百分点，同时保证推理时可验证的轨迹。

Conclusion: SAFE框架不仅暴露了现有基准的关键缺陷，还显著优于标准基线，为多跳问答提供了更严谨、可验证的推理验证方法。
...
TL;DR: 本文研究了高斯扩散模型在离散数据生成中的挑战，发现DDPM求解器在处理离散分布时会遇到临界采样区间问题，导致样本质量下降，并提出结合自条件和q-sampling的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在连续域生成建模中已成为标准方法，但在离散数据上的应用仍然面临挑战。本文旨在探究高斯扩散模型为何在处理表示为连续空间中混合δ分布的离散分布时，DDPM求解器会遇到采样困难。

Method: 使用玩具随机层次模型识别临界采样区间，在该区间内噪声化数据的密度变为多模态。分析DDPM在该区间进入低密度区域的问题，并评估现有启发式方法（包括自条件和q-sampling求解器）的缓解效果。提出将自条件与在临界区间内从DDPM切换到q-sampling相结合的方法。

Result: 研究发现DDPM在临界采样区间会偶尔进入模态间的低密度区域，产生模型分布外的输入，从而降低样本质量。自条件和q-sampling有助于缓解这一问题。结合两种方法在临界区间内切换，能显著提高真实数据上的生成质量。

Conclusion: 高斯扩散模型在离散数据生成中存在临界采样区间问题，DDPM求解器在该区间会因进入低密度区域而影响样本质量。通过结合自条件和在临界区间切换到q-sampling的策略，可以有效改善离散数据（包括文本、编程代码和蛋白质）的生成质量。
...
TL;DR: 研究自监督语音模型训练过程中语言结构的编码时机，分析Wav2Vec2和HuBERT模型在荷兰语训练中不同语言结构的层级分布和学习轨迹


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表示，但这些语言结构在模型训练过程中何时出现尚不清楚，需要研究不同语言结构在训练过程中的编码时机和模式

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语训练中的中间检查点，研究多种语言结构在不同层级的编码模式和学习轨迹，考察结构与声学信号的抽象程度和时间尺度整合的关系

Result: 发现不同层次的语言结构在层级分布和学习轨迹上有显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；预训练目标定义的层级强烈影响语言结构的层级组织和学习轨迹，高阶预测任务（迭代精化的伪标签）能诱导更大的并行性

Conclusion: 自监督语音模型中语言结构的编码时机和模式具有多样性，受结构抽象程度、时间尺度整合和预训练目标层级的影响，高阶预测任务能促进更并行的结构学习
...
TL;DR: 将因果生成语言模型转换为双向编码器的系统方法，通过识别关键适应因素、防止灾难性遗忘的策略，以及融合专业模型能力，创建了优于现有方案的BidirLM编码器家族。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转换为双向编码器的方法存在三个主要问题：缺乏最优训练目标的共识、大规模应用时出现灾难性遗忘、以及难以灵活集成专业生成模型生态系统。本研究旨在解决这些限制，开发更有效的转换方法。

Method: 1. 在Gemma3和Qwen3模型家族上进行系统消融实验，识别成功适应的关键因素，特别关注常被忽略的先验掩码阶段；2. 引入双重策略：线性权重合并和轻量级多领域数据混合，以在不使用原始预训练数据的情况下扩展过程并减轻灾难性遗忘；3. 通过合并专业因果模型来增强编码器，无缝转移模态和领域特定能力。

Result: 开发了开源配方，适用于任何因果解码器LLM，创建了BidirLM编码器家族（包含5个编码器）。这些编码器在文本、视觉和音频表示基准测试中优于替代方案。

Conclusion: 本研究提供了一种系统方法，成功将因果生成语言模型转换为高性能双向编码器，解决了现有方法的局限性，并通过开源配方为任何因果解码器LLM提供了有效的转换方案。
...
TL;DR: GOOSE提出了一种自适应脊柱树结构，通过区分不同来源候选token的质量差异，构建深度可靠token链和宽度不可靠token分支，显著提升推测解码效率。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的推测解码方法从单一token源生成候选token，构建平衡树结构时未区分不同来源token的质量差异。研究发现上下文匹配的n-gram token和统计预测token的接受率存在显著差异（中位数约6倍差距），这为优化树结构提供了机会。

Method: GOOSE采用训练无关框架，构建自适应脊柱树：将高接受率的上下文匹配token形成深度链（脊柱），在每个节点上为低接受率的统计预测token添加宽度分支。这种各向异性结构突破了平衡树的深度限制。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线方法性能提升12-33%。理论证明GOOSE每步接受的token数至少与单独使用任一token源时相同。

Conclusion: 通过区分不同token源的质量差异并构建各向异性树结构，GOOSE显著提升了推测解码的效率，证明了在固定验证预算下优化树结构的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前重排序模型基于静态人工标注的相关性标签进行优化，与下游生成过程脱节，导致检索到的文档虽然主题相关但无法为LLM提供生成精确答案所需的有用信息

Method: 提出ReRanking Preference Optimization (RRPO)框架，将重排序建模为序列决策过程，使用LLM反馈优化上下文效用，并引入参考锚定的确定性基线确保训练稳定性

Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的基线模型；框架具有良好泛化性，可适配不同LLM，与查询扩展模块正交集成，且在噪声监督下仍保持鲁棒性

Conclusion: RRPO通过强化学习直接对齐重排序与LLM生成质量，解决了传统重排序与下游生成脱节的问题，无需昂贵人工标注，为检索增强生成提供了更有效的重排序方法
...
TL;DR: 提出"韵律ABX"框架，扩展ABX辨别任务来评估自监督语音模型中韵律对比的敏感性，无需显式标签，仅需少量样本。


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型（S3Ms）的表征已知对音位对比敏感，但其对韵律对比的敏感性尚未被直接测量。需要一种方法来评估S3Ms在韵律特征（如重音、音高重音、声调）上的表现。

Method: 扩展ABX辨别任务框架，提出"韵律ABX"方法，仅需少量样本且无需显式标签。构建并发布了英语和日语的最小对数据集，结合已有的普通话数据集，评估英语重音、日语音高重音和普通话声调的对比敏感性。

Result: 使用构建的数据集评估了自监督语音模型在三种语言韵律特征上的表现。结果显示，模型和层级排名在不同实验条件下通常保持一致，这使得该方法在低资源环境下具有实用性。

Conclusion: 提出的韵律ABX框架能够有效评估自监督语音模型对韵律对比的敏感性，且方法简单实用，特别适用于低资源场景。构建的数据集为相关研究提供了有价值的资源。
...
TL;DR: 论文提出稳定性过滤方法，通过筛选行为稳定的边界来构建更有效的引导向量，提升大语言模型的推理控制效果
...
Method: 提出稳定性过滤方法，仅保留模型能一致复现目标行为的边界；结合内容子空间投影去除问题特定噪声；构建概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件
...
Conclusion: 稳定性过滤能有效识别真正的行为信号，构建更纯净的引导向量，显著提升推理控制效果，且具有跨模型迁移能力
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答任务，评估19个LLM后发现前沿模型在盖尔语语法多个维度上超越人类表现，专有模型优于开源模型，盖尔语提示有稳定优势。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未受官方支持的语言中表现出"影子"能力，但性能参差不齐且缺乏有效测量。对于苏格兰盖尔语这类形态句法丰富的少数民族语言，现有翻译基准无法捕捉其结构能力，需要专门的评估框架。

Method: 开发GaelEval基准测试，包含三个部分：1)专家编写的形态句法多项选择题任务；2)文化基础翻译基准；3)大规模文化知识问答任务。评估19个LLM，与30名流利说话者的人类基线对比，分析专有模型与开源模型差异，以及盖尔语提示的效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基线(78.1%)。专有模型始终优于开源系统，盖尔语提示带来稳定但较小的优势(+2.4%)。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，绝对分数相对于人工基准有所膨胀。

Conclusion: GaelEval揭示了前沿模型在盖尔语语法多个维度上达到超人类表现，证明了盖尔语提示的效果，并显示了专有模型相对于开源模型的一致性能优势，为少数民族语言评估提供了重要基准。
...
TL;DR: 该论文在SOMD 2026软件提及跨文档共指消解任务中，比较了两种免微调方法：模糊匹配和上下文感知表示，均取得优异性能，分析了不同噪声条件下的表现差异和计算效率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在软件提及跨文档共指消解任务中，不同免微调方法的性能表现，特别是在面对不同噪声类型和大规模语料时的适用性。

Method: 采用了两种免微调方法：1) 模糊匹配（FM）- 基于词汇字符串相似度的方法；2) 上下文感知表示（CAR）- 结合提及级和文档级嵌入的方法。通过受控噪声注入研究分析不同噪声条件下的表现。

Result: 在SOMD 2026任务中排名第二，两种方法在所有子任务上都取得竞争性性能（CoNLL F1 0.94-0.96）。CAR在官方测试集上始终比FM高1分。噪声研究表明：边界噪声增加时，CAR表现更稳健；提及替换时，FM退化更平缓。CAR的计算效率在大规模语料上更高。

Conclusion: 系统选择应考虑上游提及检测器的噪声特征和目标语料的规模。CAR在大规模应用中更具效率优势，而FM在某些噪声条件下表现更稳健。该研究为这一未充分探索的任务提供了实用指导。
...
TL;DR: 本文提出Neuro-RIT框架，通过神经元级别的精准对齐增强检索增强语言模型对噪声上下文的鲁棒性，相比现有粗粒度参数更新方法效果更优。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强语言模型在面对不相关或噪声检索上下文时性能会显著下降，而现有的鲁棒性增强方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性。
...
Conclusion: 从密集适应转向精准驱动的神经元对齐范式能够有效增强检索增强语言模型对噪声上下文的鲁棒性，Neuro-RIT框架为此提供了有效的解决方案。
...
TL;DR: 该研究创建了首个公开的阿拉伯语语音命名实体识别数据集CV-18 NER，并比较了流水线系统和端到端模型在阿拉伯语语音NER任务上的性能，发现端到端方法显著优于传统流水线方法。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要挑战包括阿拉伯语的形态复杂性、缺少短元音以及标注资源有限。现有研究已证明端到端方法在英语、法语和中文上优于流水线方法，但阿拉伯语领域仍待探索。

Method: 1. 创建CV-18 NER数据集：基于阿拉伯语Common Voice 18语料库，按照Wojood模式（21种实体类型）进行人工NER标注；2. 基准测试：比较流水线系统（ASR + 文本NER）和基于Whisper、AraBEST-RQ的端到端模型；3. 分析阿拉伯语特定自监督预训练和多语言弱监督对任务的影响。

Result: 端到端系统在测试集上显著优于最佳流水线配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析表明：阿拉伯语特定自监督预训练在ASR上表现良好，而多语言弱监督能更有效地迁移到联合语音到实体学习，大型模型在低资源环境下可能更难适应。

Conclusion: 端到端方法在阿拉伯语语音命名实体识别任务上优于传统流水线方法。研究发布了首个公开的阿拉伯语语音NER数据集和模型，为后续研究提供了基准。数据集和模型已公开在Hugging Face平台。
...
TL;DR: 论文研究如何从多个LLMs中生成全面的答案集，提出多样性覆盖度指标评估模型生成多样性答案的能力，并开发路由模型选择最佳模型


<details>
  <summary>Details</summary>
Motivation: 当提示允许大量有效答案时，全面生成这些答案是满足多样化用户需求的第一步。现有LLMs在生成多样性答案方面表现不一，需要系统方法来选择最佳模型

Method: 提出多样性覆盖度指标评估答案集的全面性；评估18个LLMs的多样性生成能力；开发路由模型为每个查询预测最佳模型；在NB-Wildchat和NB-Curated数据集上验证

Result: 没有单一模型在所有提示上表现最佳，但每个提示都存在显著优于其他模型的特定模型；训练的路由模型在NB-Wildchat上优于单一最佳模型基线（26.3% vs 23.8%）；方法能泛化到域外数据集和不同答案生成策略

Conclusion: 该研究为在拥有多个模型时生成全面答案集奠定了基础，展示了通过智能路由选择最佳模型的有效性，为开放域问答系统的多样性优化提供了新方法
...
TL;DR: 研究发现语言模型扩展新词汇时常用的均值初始化方法存在问题，会导致新token坍缩到退化子空间，提出基于语言基础的token初始化方法GTI，在多个生成推荐基准测试中表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在扩展新可学习词汇token（如生成推荐中的语义ID token）时，通常采用均值初始化策略，然后通过监督微调学习其表示。作者通过谱分析和几何诊断发现，这种初始化方法会导致所有新token坍缩到退化子空间，消除token间的区分度，后续微调难以完全恢复。

Method: 提出"基于语言基础的token初始化假设"：在微调前将新token在预训练嵌入空间中基于语言基础进行定位，能更好地让模型利用其通用知识处理新token领域。具体实现为GTI方法——在微调前，仅使用配对的语言监督将新token映射到预训练嵌入空间中不同且有语义意义的位置。

Result: 在多个生成推荐基准测试（包括工业规模数据集和公共数据集）中，GTI方法在大多数评估设置下都优于均值初始化和现有的辅助任务适应方法。进一步分析显示，基于语言基础的嵌入能产生更丰富的token间结构，这些结构在微调过程中得以保持。

Conclusion: token初始化是语言模型扩展新词汇时的关键瓶颈。通过诊断驱动的GTI方法，在微调前基于语言基础初始化新token，能显著提升模型性能，验证了初始化质量对词汇扩展的重要性。
...
### [155] [Coverage and Rate Analysis of Follower-Based LEO Satellite Networks: A Stochastic Geometry Approach](https://arxiv.org/abs/2604.01265)
*Juanjuan Ru,Ruibo Wang,Mohamed-Slim Alouini*

Main category: cs.IT

TL;DR: 该论文提出基于球形随机几何的领导者-跟随者卫星集群性能评估框架，用于分析LEO巨型星座中卫星集群的通信性能，推导了中断概率和平均数据率的解析表达式，并展示了该架构相比单颗领导卫星的性能优势。


<details>
  <summary>Details</summary>
Motivation: 为了缓解LEO巨型星座中的星间干扰和有效载荷限制，卫星集群（由多个小型协作卫星组成）被提出以提高性能并减少干扰。典型的领导者-跟随者架构中，领导卫星具备完整处理和控制能力，多个简单的跟随卫星协助覆盖和吞吐量。需要评估这种架构的性能增益和最优部署配置。

Method: 采用球形随机几何模型来建模卫星集群结构，建立了基于SG方法的领导者-跟随者卫星架构性能评估框架。推导了通信性能的解析表达式，包括中断概率和平均数据率，并提供了低复杂度近似方法。

Result: 数值结果表明领导者-跟随者架构相比单颗领导卫星具有性能优势，并探索了跟随卫星的最优部署配置。通过分析表达式能够定量评估不同部署配置下的通信性能。

Conclusion: 基于球形随机几何的领导者-跟随者卫星集群性能评估框架能够有效分析卫星集群的通信性能，为LEO巨型星座中卫星集群的优化部署提供了理论依据和评估工具。
...
### [156] [On the existence of linear rank-metric intersecting codes](https://arxiv.org/abs/2604.02004)
*Martino Borello,Olga Polverino,Ferdinando Zullo*

Main category: cs.IT

TL;DR: 该论文研究了秩度量相交码的参数约束，证明了非退化秩度量相交码必须满足 n ≤ 2m-⌊(k+4)/2⌋，特别地当 n=2m-3 时仅当 k=3 且 m≥6。通过几何解释和散射子空间理论，将存在性问题简化为散射子空间的存在性，并解决了 Bartoli 等人提出的 [6,3,3]_{q^5/q} 码不存在问题。


<details>
  <summary>Details</summary>
Motivation: 秩度量相交码是编码理论中的新研究对象，其定义形式与汉明度量相交码类似，但参数约束和结构表现出显著不同。先前研究表明非退化秩度量相交码必须满足 2k-1 ≤ n ≤ 2m-3，但上界的紧致性尚未解决。论文旨在深入研究秩度量相交码的参数限制，特别是上界 n=2m-3 何时可达。

Method: 采用秩度量码的几何解释（通过 q-系统），证明秩度量相交码的对偶子空间必须满足强规避性质。利用这种联系推导参数的新限制，并将极值码的存在性问题转化为散射子空间的存在性问题。使用已知的最大散射子空间构造来证明存在性结果。

Result: 1. 证明了 n ≤ 2m-⌊(k+4)/2⌋ 的更强参数限制；2. 确定了 n=2m-3 仅当 k=3 且 m≥6 时可达；3. 获得了极值码的几何特征：与 𝔽_{q^m}^3 中的散射 𝔽_q-子空间相关；4. 将 [2m-3,3,d]_{q^m/q} 码的存在性简化为维度 m+3 的散射子空间的存在性；5. 当 m 为偶数时证明了存在性；6. 证明了 [6,3,3]_{q^5/q} 码对任意素数幂 q 都不存在。

Conclusion: 论文通过几何方法深入分析了秩度量相交码的参数约束，获得了比先前更精确的限制条件。特别解决了上界紧致性问题，将极值码的存在性与散射子空间理论联系起来，并完全解决了 Bartoli 等人在 2025 年提出的一个具体存在性问题。这些结果为秩度量相交码的进一步研究提供了理论基础和构造方法。
...
### [157] [On the Capacity Region of Additive-Multiplicative MAC with Heterogeneous Input Constraints](https://arxiv.org/abs/2604.02037)
*Qianqian Zhang,Ying-Chang Liang*

Main category: cs.IT

TL;DR: 该论文研究了具有异质输入约束的双用户加性-乘性多址信道容量区域，该模型描述了共生无线电的基本极限，其中主动主发射机在平均功率约束下传输信息，而被动反向散射设备在峰值幅度约束下通过反向散射调制信号。


<details>
  <summary>Details</summary>
Motivation: 研究共生无线电系统的容量极限，其中主动主发射机和被动反向散射设备在异质约束下协同工作，需要理解这种混合系统的信息理论基础。

Method: 通过信息理论分析，推导出加性-乘性多址信道的容量区域，确定最优信号分布，包括高斯信号、恒定包络信号以及连续相位离散幅度的混合策略。

Result: 1) 和速率容量等于主发射机的点对点容量，通过高斯信号和纯反射实现；2) 为达到反向散射设备最大速率，主发射机需采用恒定包络信号，反向散射设备分布具有同心圆结构；3) 边界点处，主发射机信号为连续均匀相位和离散幅度，反向散射设备分布完全离散。

Conclusion: 论文完整刻画了异质约束下加性-乘性多址信道的容量区域，为共生无线电系统设计提供了理论基础，并通过数值结果验证了理论分析，展示了实际性能基准。
...
### [158] [One-Shot Secret Sharing with Monotone Access Structures over Classical-Quantum Broadcast Channels](https://arxiv.org/abs/2604.02275)
...
<details>
  <summary>Details</summary>
Motivation: 研究在经典-量子广播信道环境下，当用户份额受信道扰动而非完全由分发者控制时的秘密共享问题，扩展传统秘密共享模型以适应更实际的通信场景。

Method: 采用单调访问结构，通过经典-量子广播信道传输秘密，分发者编码秘密并通过信道广播，用户接收受信道扰动的份额，分析一次性秘密共享速率。

Result: 获得了任意单调访问结构下可实现的单次秘密共享速率和逆界，推导了二阶和渐近可实现速率，在需要所有份额恢复秘密的特殊情况下，结果与经典信道上的现有秘密共享容量一致。

Conclusion: 该研究扩展了秘密共享理论到经典-量子广播信道环境，建立了受信道扰动的份额情况下的秘密共享容量界限，为实际量子通信系统中的秘密共享提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法都通过假设替代缺失数据（如可忽略性、平行趋势等），但都无法直接产生反事实结果本身。本文旨在建立一个框架，使边际因果主张变得可检验，联合因果主张明确依赖假设，并形式化表征两者之间的差距。

Method: 提出数字孪生反事实框架(DTCF)：使用数字孪生模拟反事实结果而非统计估计；形式化数字孪生模拟器为潜在结果框架内的随机映射；引入孪生保真度假设层次（从边际保真度到联合保真度再到结构保真度）；建立五层验证架构将不可证伪主张转化为可检验测试；形式化分解将因果量分为边际验证部分和copula依赖部分。

Result: DTCF提供了一个系统框架：1）边际因果主张（如ATE、CATE、QTE）可通过可观测臂比较进行检验；2）联合因果主张（如ITE分布、获益/损害概率、处理效应方差）明确依赖不可观测的个体内依赖结构；3）边界、敏感性和不确定性量化工具使copula依赖关系显式化。

Conclusion: DTCF并未解决因果推断的根本问题，但提供了一个框架：使边际因果主张变得可检验，联合因果主张明确依赖假设索引，并形式化表征两者之间的差距。该框架将不可证伪的模拟器正确性主张转化为可检验的验证测试。
...
TL;DR: IDEA2是一个结合大型语言模型(LLM)的半自动化工作流，通过专家在环的协作过程，从需求文档中提取能力问题(CQs)，加速本体工程中的需求获取阶段。


<details>
  <summary>Details</summary>
Motivation: 能力问题(CQ)提取在本体工程中是一个关键但资源密集的瓶颈阶段，主要障碍是领域专家（拥有知识）和本体工程师（形式化知识）之间的沟通鸿沟。

Method: IDEA2采用半自动化工作流，核心是迭代循环：1) LLM从需求文档中初步提取CQs；2) 领域专家在协作平台上进行共同创建、审查和反馈；3) LLM根据反馈迭代重新表述被拒绝的CQs直至达成共识。整个过程使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在科学数据和文化遗产两个真实场景中验证，IDEA2能够加速需求工程过程，提高最终CQs的接受度和相关性，并在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过整合LLM和专家在环的协作过程，有效解决了本体工程中CQ提取的瓶颈问题，提高了效率和质量，所有代码和实验已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在企业、制度系统等结构化领域中存在语义一致性、可解释性和长期稳定性不足的问题，需要一种共享的显式世界表示来协调多智能体行为。

Method: 提出世界中心多智能体系统概念，通过维度分类（本体显式性、规范性等）描述世界特征，使用语义模型作为数学形式化表示，并开发Ontobox平台作为实现。

Result: 建立了世界中心多智能体系统的理论框架，提供了语义模型作为形式化工具，并实现了Ontobox平台作为该架构的具体实现。

Conclusion: 世界中心多智能体系统为结构化领域中的多智能体协调提供了更优的解决方案，通过共享世界模型确保全局一致性、可验证性和长期稳定性。
...
TL;DR: 研究发现AI自动化呈现"涨潮"模式而非"浪潮"模式，AI能力在广泛任务中持续提升，预计到2029年LLM能在80%-95%成功率下完成大多数文本任务


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的模式，区分"浪潮式"（能力在少数任务上突然爆发）和"涨潮式"（能力在广泛任务上持续提升）两种模式，并评估AI对劳动力市场的潜在影响

Method: 基于美国劳工部O*NET分类的3000多个文本型任务，通过17000多次由相关职业工作者进行的评估，测试AI在广泛任务上的表现，分析AI能力增长模式

Result: 发现AI自动化主要表现为"涨潮"模式而非"浪潮"模式；2024年第二季度AI模型能以约50%成功率完成人类3-4小时的任务，预计到2025年第三季度提升至65%；若趋势持续，到2029年LLM能在80%-95%成功率下完成大多数文本任务

Conclusion: AI能力在广泛任务上持续提升，对经济劳动力市场将产生重要影响，但组织采用AI可能需要更长时间；达到完美成功率或更高质量水平需要额外数年时间
...
TL;DR: LLMs存在系统性认知偏差，这些偏差在模型激活空间中呈现线性可分方向，通过激活引导可有效减少偏差同时保持下游能力。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地应用于高风险决策场景，虽然已有研究表明LLMs在行为上表现出认知偏差，但这些偏差是否对应可识别的内部表征以及能否通过针对性干预来缓解，仍然是一个开放性问题。

Method: 1) 定义LLM认知偏差为在可计算基准任务中系统性、可复现的偏离正确答案；2) 引入LLM CogBias基准，围绕四个认知偏差家族组织：判断、信息处理、社会和反应；3) 评估三个LLM；4) 使用对比设计下的线性探针分析偏差的内部表征；5) 应用激活引导来调节偏差行为。

Result: 1) 认知偏差在所有四个家族中系统性出现，幅度和去偏差响应强烈依赖于家族：提示级去偏差显著减少反应偏差，但对判断偏差产生反效果；2) 认知偏差在模型激活空间中编码为线性可分方向；3) 激活引导可将偏差分数（偏差响应比例）减少26-32%，同时在25个基准测试中保持下游能力；4) 尽管不同模型的偏差表征接近正交，但引导在不同架构中减少偏差的速率相似，表明共享的功能组织。

Conclusion: LLMs中的认知偏差是可识别、可干预的内部表征，激活引导是一种有效的去偏差方法，能够显著减少偏差同时保持模型能力，不同架构间的相似去偏差效果表明认知偏差可能具有共享的功能组织。
...
TL;DR: 本文提出了RIFT（RubrIc Failure mode Taxonomy），一个用于系统诊断评分标准质量问题的分类法，包含8种失败模式，分为可靠性、内容效度和后果效度三大类。


<details>
  <summary>Details</summary>
Motivation: 评分标准在LLM基准测试和训练中被广泛使用，但现有方法无法从聚合或下游信号中诊断评分标准的质量问题，需要一种系统化的诊断框架。

Method: 使用扎根理论方法，从五个多样化基准中迭代标注评分标准，开发出包含8种失败模式的RIFT分类法，并通过人工标注一致性评估和自动化质量指标验证。

Result: RIFT分类法在人工标注中显示出良好的一致性（87%成对一致性和0.64平均Cohen's kappa），自动化质量指标与人工标注的对齐度达到0.86 F1。

Conclusion: RIFT提供了一个系统化的框架来诊断评分标准的质量问题，支持可扩展的质量评估，有助于改进LLM评估和训练中的评分标准设计。
...
TL;DR: 提出了一种基于信息价值(VOI)的POMDP规划框架，通过选择性处理观测信息来提升计算效率
...
Method: 提出了一个动态规划框架，基于每个信念点的信息价值有条件地处理观测信息。在此基础上开发了VOIMCP算法，这是一种蒙特卡洛树搜索算法，在信息价值较低时选择性地忽略观测信息，避免不必要的观测分支。

Result: 为VOI推理框架提供了理论上的近似最优性保证，并推导了VOIMCP的非渐近收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间中的变化性，提出的框架和算法能够更高效地分配计算资源，在有限规划时间内获得更好的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI系统在医疗和药学领域应用日益广泛，但其在真实临床决策中的可靠性尚未充分理解。在用药管理等高风险领域，即使单个错误推荐也可能导致严重患者伤害，因此需要深入评估AI系统的失败行为和实际影响。

Method: 通过一系列受控模拟场景（涉及药物相互作用和剂量决策），分析不同类型的系统失败，包括遗漏相互作用、错误风险标记和不适当剂量推荐。

Result: 研究发现AI在用药相关场景中的错误可能导致不良反应、无效治疗或延迟护理，特别是在缺乏足够人工监督的情况下。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限带来的挑战。

Conclusion: 该研究为医疗AI评估提供了可靠性视角，强调理解失败行为和实际影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合。
...
TL;DR: Elhuyar是一个多智能体、人机协同系统，通过整合大语言模型、结构化AI和人类科学家，从科学文献中提取、分析和迭代精炼洞察，加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学发现因碎片化文献需要过多人工努力而放缓，现有AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度洞察，大语言模型存在幻觉和不完整提取问题。

Method: 引入Elhuyar多智能体系统，将任务分配给专门的智能体进行论文筛选、数据提取、模型拟合和结果总结，结合人类监督确保可靠性，生成结构化报告并支持迭代精炼。

Result: 在材料科学中部署该系统，分析了钨在氦离子辐照下的文献，发现氦气泡生长与辐照剂量和温度呈指数相关，为聚变反应堆等离子体面向材料提供见解。

Conclusion: AI辅助的文献综述能够揭示科学模式并加速发现，Elhuyar系统展示了如何通过整合AI和人类专业知识来克服现有工具的局限性。
...
TL;DR: 该论文提出了一种不可行性感知框架，结合可认证数据集构建、监督微调和LLM辅助下游搜索，用于解决NP难组合优化问题，特别关注不可行性检测。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型方法主要关注可行实例的解决方案生成，而未能明确处理不可行性检测问题。对于组合优化问题，识别不可行实例与生成可行解同样重要。

Method: 提出不可行性感知框架：1）针对minor-embedding问题引入新的数学规划公式和可证明的零阶段不可行性筛选；2）构建可认证训练数据集；3）对8B参数LLM进行监督微调，使其同时执行解决方案生成和不可行性检测；4）将LLM输出作为下游局部搜索的初始解。

Result: 微调后的模型相比GPT-5.2整体准确率提升高达30%；LLM引导的初始解相比从零开始的下游局部搜索提供高达2倍的加速。

Conclusion: 该框架成功将LLM应用于组合优化问题，不仅提高了解决方案生成质量，还增强了不可行性检测能力，并通过LLM辅助加速了优化过程。
...
TL;DR: P-COD是一种利用文档间关系来减少LLM幻觉的新方法，通过比较提取数据与语料库中已验证的同行信息来调整置信度分数，在科学文献总结中实现高达98%的异常检测精度。


<details>
  <summary>Details</summary>
Motivation: 当前减少LLM幻觉的方法（如提示工程和思维链提示）主要关注单个文档，忽略了语料库中文档间的关系。在科学文献总结领域，具有相似实验设置的论文应该得出相似结论，因此需要一种能够利用文档间关系来提高数据提取准确性的方法。

Method: 提出Peer Context Outlier Detection (P-COD)方法，通过比较从文档中提取的数据与语料库中已验证的同行信息来调整置信度分数。该方法标记低置信度结果供专家审查，而得到同行验证支持的高置信度结果被认为是可靠的。

Result: 在6个科学领域的实验中，P-COD实现了高达98%的异常检测精度。该方法有效减少了幻觉，增强了自动化系统的可信度，使研究人员能够专注于模糊案例，从而简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系显著提高了LLM数据提取的准确性，特别是在科学文献总结领域。该方法不仅减少了幻觉，还增强了系统的可信度，为研究人员提供了更可靠的数据提取工具。
...
TL;DR: 该论文提出了一个基于智能体框架的元表面逆向设计系统，通过上下文级技能演化来积累可重用的求解器专业知识，提高设计工作流的自动化程度和成功率。


<details>
  <summary>Details</summary>
Motivation: 元表面逆向设计需要复杂的计算电磁学专业知识和求解器特定的软件工程，现有的大语言模型驱动系统缺乏跨任务的可重用工作流知识积累，导致设计效率低下。

Method: 提出了一个智能体框架，包含编码智能体、演化技能工件和基于物理仿真的确定性评估器，通过迭代精化求解器特定策略而不修改模型权重或底层物理求解器。
...
TL;DR: 本文提出了一种基于角色的LLM框架，用于从健康食品政策文档中自动进行信息提取，通过分配专业角色来减少幻觉、误分类和遗漏问题。
...
TL;DR: 本文研究将表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出了分布归一化和高斯噪声注入两种方法来解决高维表示自编码器在AR范式下的建模难题。


<details>
  <summary>Details</summary>
Motivation: 生成建模的潜在空间长期以来由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示表示自编码器也能达到与VAE编码器相当的性能，但将其整合到连续自回归模型中的研究仍然很少。

Method: 提出了RAE-AR方法，针对AR模型的独特性质，通过分布归一化进行标记简化以降低建模难度，并在训练时加入高斯噪声注入来缓解暴露偏差问题。
...
TL;DR: 研究发现LLM训练中归一化层与优化器并非独立设计选择，Derf归一化与Muon优化器存在显著负交互作用，导致性能下降0.66 nats，而DyT归一化则无此问题


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练中将归一化层和优化器视为独立设计选择，但研究发现这种假设可能不成立，需要探索不同归一化方法与优化器之间的交互作用

Method: 在10亿参数规模、1000训练步长的3x2因子实验中，比较Derf、RMSNorm和DyT三种归一化方法在AdamW和Muon两种优化器下的表现，分析失效模式并提出EMA混合和alpha参数调整两种解决方案

Result: Derf与Muon存在严重负交互作用，性能差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats；失效原因为erf函数在Muon快速谱范数增长下的饱和和尺度盲区问题；EMA混合恢复约84%差距，降低alpha参数恢复约80%差距

Conclusion: 归一化层与优化器选择存在重要交互作用，Derf默认参数与Muon组合会导致显著性能损失但不易被发现，需要谨慎选择归一化方法参数或采用EMA混合策略
...
Result: 在10个基准测试中，NED-Tree达到72.51%的平均准确率，建立了新的最先进水平，并首次通过元素分解解决非线性建模困难

Conclusion: NED-Tree是首个通过元素分解驱动LLM解决非线性建模困难的框架，实现了建模语义与代码语义的对齐，同时提出了专门用于复杂非线性、多约束OR问题的新基准NEXTOR
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要正确性信号或批评标注来训练LLMs的推理和精炼能力，ThinkTwice旨在通过简单的两阶段框架，仅使用二元正确性奖励来联合优化推理和自精炼能力。

Method: 基于Group Relative Policy Optimization (GRPO)的两阶段框架：第一阶段优化模型解决推理问题，第二阶段优化模型精炼自己的解决方案，两个阶段使用相同的二元正确性奖励，无需额外标注。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME上比GRPO高出5个百分点（精炼前）和11.5个百分点（一次自精炼后）。

Conclusion: ThinkTwice的训练动态揭示了隐式的"纠正-然后-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案，产生更精确的奖励信号。该工作确立了联合训练推理和自精炼作为RLVR的原则性和有效方法。
...
TL;DR: 研究测试LLMs如何决定教学内容：是通过推理学习者知识还是使用简单经验法则。在受控教学任务中，大多数LLMs表现良好，其选择最佳由贝叶斯最优教学模型解释，提示干预不一定能改善教学决策。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在教学决策时是否真正推理学习者的知识状态，还是仅仅依赖简单的启发式规则，以理解LLMs教学策略的本质。

Method: 使用先前研究人类教学策略的受控任务：教师LLM观察学习者在奖励标注有向图中的轨迹，揭示单一边缘以改善学习者路径。将LLMs作为模拟教师，用认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心理化效用模型）拟合其选择，进行模型比较（BIC）。

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似。模型比较显示贝叶斯最优教学最能解释大多数模型的选择。提示干预虽能引导模型遵循推理或奖励导向，但未必改善教学表现，有时反而降低性能。

Conclusion: LLMs的教学策略可由贝叶斯最优教学模型解释，表明它们能推理学习者知识状态。然而，提示合规性不能保证更好的教学决策，认知模型拟合为理解LLMs教学策略提供了洞察。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要依赖监督微调进行图表转代码任务，但这种方法缺乏代码执行环境的暴露，且即使最先进的MLLM在有效自校正方面也存在困难

Method: 提出MM-ReCoder模型，采用基于GRPO的两阶段多轮自校正强化学习策略：第一阶段通过共享首轮增强模型自校正能力，第二阶段通过全轨迹优化提升编码能力

Result: 在三个图表转代码基准测试中展示了最先进的性能，能够生成更准确且可执行的代码

Conclusion: 通过强化学习与环境交互及迭代自校正，MM-ReCoder显著提升了图表转代码生成的质量和可执行性，为多模态编码任务提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型安全问题日益突出，理解拒绝行为的内在机制变得至关重要。现有方法依赖特征在有害提示上的激活强度，这倾向于捕捉表面信号而非拒绝决策的因果因素。

Method: 提出CRaFT框架，这是一种基于电路引导的拒绝特征选择方法，通过使用接近拒绝边界的提示来根据特征对模型拒绝-合规决策的影响程度对特征进行排序。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升到48.2%，并在多个越狱基准测试中优于基线方法。

Conclusion: 电路影响比激活强度更可靠，是识别因果中介拒绝行为特征的更有效标准。
...
TL;DR: 本文提出了Metric Freedom (F)作为技能效用的先验预测器，通过度量评分景观的拓扑刚性来预测多智能体系统向单智能体技能蒸馏的效果，并设计了自适应蒸馏框架。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在处理复杂任务时存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。虽然将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏关于何时以及蒸馏什么的原则性指导，导致实证结果不一致。

Method: 引入Metric Freedom (F)作为技能效用的先验预测器，通过Mantel测试量化输出多样性与得分方差之间的耦合关系来度量评分景观的拓扑刚性。提出两阶段自适应蒸馏框架：第一阶段作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构；第二阶段专门针对"刚性"度量进行计算密集的迭代优化。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F能强预测技能效用（ρ=-0.62，p<0.05）。相同的智能体轨迹在刚性与自由度量下产生完全相反的技能提升效果。自适应智能体匹配或超越原始MAS，同时将成本降低最多8倍，延迟减少最多15倍。

Conclusion: 技能效用本质上是度量层面的属性而非任务属性。Metric Freedom (F)作为先验预测器能有效指导自适应蒸馏，在保持或提升性能的同时显著降低多智能体系统的成本和延迟。
...
TL;DR: GraphWalk是一个无需训练、基于工具的框架，让现成的LLM能够通过顺序图导航进行推理，显著提升在大型知识图谱上的复杂查询性能。
...
Method: 提供最小正交图操作工具集，让LLM通过顺序图导航进行多步推理，每个工具调用代表可验证的步骤，形成透明执行轨迹。

Result: 基于工具的图遍历方法在所有测试模型系列上相比上下文基线都有显著且一致的提升，随着规模增大，优势更加明显，而上下文方法则完全失败。

Conclusion: GraphWalk框架通过工具化图导航，使LLM能够在无法放入上下文的大型知识图谱上进行有效的多步推理，提供透明可验证的执行轨迹。
...
TL;DR: 本研究系统评估了四种AWS Bedrock上的LLM在收据项目分类任务中的表现，发现Claude 3.7 Sonnet在准确性和成本效率之间达到最佳平衡


<details>
  <summary>Details</summary>
Motivation: 在面向生产的分类框架中，需要系统评估大型语言模型在收据项目分类任务中的表现，同时考虑性能（准确性、响应稳定性）和成本（token级别成本），并探索适合的提示方法（零样本或少样本）

Method: 比较四种通过AWS Bedrock提供的指令调优模型：Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct和Mistral 7B Instruct，评估准确性、响应稳定性和token级成本，并研究零样本和少样本提示方法的适用性
...
TL;DR: OSCAR是一个无需训练、基于推理时间的框架，利用扩散语言模型的去噪轨迹来定位和修正幻觉。它通过并行去噪链计算交叉链熵来检测高不确定性位置，然后进行有针对性的重新掩码，显著减少幻觉内容并提高事实准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型暴露了其去噪轨迹，为推理时间控制提供了自然手段。理想的幻觉缓解框架应该利用这种模型原生信号在生成过程中进行干预，而不是依赖外部训练的幻觉分类器。

Method: 提出承诺不确定性定位：给定去噪轨迹，在事实不可靠的承诺传播到自洽但错误的输出之前，识别交叉链熵超过无监督阈值的标记位置。引入OSCAR框架：运行N个并行去噪链（随机揭示顺序），计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行有针对性的重新掩码。

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR通过显著减少幻觉内容和改进事实准确性来增强生成质量。定位和修正贡献了互补的增益，在N∈{4,8,16}时保持稳健。其基于熵的不确定性信号超越了专门的训练检测器。

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这种能力在自回归模型的顺序标记承诺结构中不存在。OSCAR展示了利用模型原生信号进行推理时间干预的有效性，为DLMs中的定位和不确定性感知生成研究提供了支持。
...
TL;DR: EnviSmart系统通过三轨知识架构和角色分离的多智能体设计，在环境FAIR数据管理中实现可靠的人工智能驱动工作流，防止LLM生成错误输出导致不可逆操作。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理很有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中扩展数据管理。但用概率性工作流替换确定性组件会改变故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作中，如DOI分配和公开发布。
...
Result: 多智能体方法提高了效率和可靠性：SF2Bench复合洪水基准（2,452个监测站和8,557个文件）验证了工作流，单个操作员在两天内完成，审计交接检测并阻止了影响所有2,452个站的坐标转换错误。代表性事件（ISS-004）展示了边界遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart系统成功部署在生产环境中，通过架构设计确保LLM驱动数据管理的可靠性，防止错误传播到不可逆操作，同时提高效率，已被PEARC 2026会议接受。
...
TL;DR: CORAL是一个基于大语言模型的多智能体自主进化框架，用于开放式发现问题，通过共享持久记忆、异步多智能体执行和心跳干预实现自主探索，在多种任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的进化方法仍严重依赖固定启发式和硬编码探索规则，限制了智能体的自主性。需要开发能够实现持续搜索和知识积累的自主进化框架来解决开放式发现问题。

Method: CORAL采用长期运行的智能体通过共享持久记忆、异步多智能体执行和心跳干预进行探索、反思和协作。提供实用保障措施，包括隔离工作空间、评估器分离、资源管理以及智能体会话和健康管理。

Result: 在多样化数学、算法和系统优化任务上，CORAL在10个任务上创造了新的最先进结果，相比固定进化搜索基线，改进率提高了3-10倍且评估次数更少。在Anthropic的内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升到1103周期。

Conclusion: 机制分析表明这些收益来自知识重用和多智能体探索与通信。结果表明更大的智能体自主性和多智能体进化可以显著改善开放式发现。代码已开源。
...
TL;DR: 本文提出了七种面向临床AI系统的本体感知设计模式，用于构建能够抵抗本体扭曲的AI管道，包括数据验证、信号保留、漂移监控等模式，并以糖尿病风险预测为例进行了说明。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统通常在受文档工作流程、计费激励和术语碎片化结构扭曲的健康数据上进行训练。现有研究已描述了这种扭曲的机制，但将这些见解转化为可实现的软件架构仍然是一个未解决的问题。

Method: 提出了七种基于Gang-of-Four模式语言的本体感知设计模式：本体检查点、休眠感知管道、漂移哨兵、双本体层、具象化断路器、术语版本门和监管合规适配器。每个模式都按照问题、力量、解决方案、后果、已知用途和相关模式进行规范。

Result: 构建了一个初级保健AI系统的参考架构，并通过糖尿病风险预测场景展示了所有七种模式的组合应用。其中三种模式在现有系统中有部分先例，其余四种尚未被正式描述。

Conclusion: 本文提供了一个基于理论分析的设计词汇表，为构建抵抗本体扭曲的临床AI管道提供了架构解决方案，但缺乏运行时基准测试，且主要针对德国和欧盟监管环境，需要未来在生产系统中进行评估验证。
...
TL;DR: BACM-RL：基于强化学习的预算感知上下文管理方法，通过序列决策和课程学习优化长时推理任务中的上下文压缩策略，在有限上下文预算下显著提升性能


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时推理任务中面临上下文长度限制的挑战，由于部署因素（内存、延迟、成本）导致上下文预算受限，需要在保留历史信息和遵守预算约束之间进行权衡
...
<details>
  <summary>Details</summary>
Motivation: 智能体在处理大量交互数据时面临性能瓶颈，简单扩展存储会增加检索噪声和计算延迟，特别是在资源受限的个人设备上会超出模型的推理能力

Method: 提出分层记忆编排（HMO）框架，将交互历史组织成三层目录结构，由用户中心化的上下文相关性驱动。系统维护紧凑的主缓存，结合近期和关键记忆与动态用户画像，确保智能体推理与个体行为特征保持一致。主缓存由高优先级次级层补充，两者都在完整交互历史的全局档案中管理。用户画像决定记忆在层次结构中的重新分配，将映射到长期模式的记录提升到更活跃的层级，同时降级相关性较低的信息

Result: 在多个基准测试中达到最先进的性能。在OpenClaw等生态系统中的实际部署表明，HMO显著增强了智能体的流畅性和个性化能力

Conclusion: HMO通过有针对性的编排，在需要时精确呈现历史知识，同时保持精简高效的活动搜索空间，解决了智能体长期记忆管理的核心挑战
...
<details>
  <summary>Details</summary>
Motivation: 当前模型融合方法主要针对同源模型（来自相同预训练骨干），但在开放模型生态系统中，有用专家模型往往基于不同架构（如Llama、Qwen、Mistral），异构设置下的直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标在异构骨干间转移知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声转移信号。该方法还通过保留目标适配器基同时预测结构化更新来确保稳定且良条件的转移过程

Result: 在异构转移、多源融合、噪声源鲁棒性和跨家族泛化设置中，HeteroFusion一致优于强大的融合、合并和集成基线方法

Conclusion: HeteroFusion为异构语言模型融合提供了有效解决方案，通过拓扑对齐和冲突感知机制解决了不同架构模型间的知识转移问题，在开放模型生态系统中具有重要应用价值
...
TL;DR: 研究通过中国视频平台数据发现，AIGC通过大量生产获得与HGC相当的总体参与度，但用户仍更偏好HGC；算法分发机制能调节这种矛盾，建议开发AIGC敏感算法和精准治理框架。
...
Result: 发现"规模优先于偏好"现象：AIGC创作者通过高产量获得与HGC相当的总体参与度，但消费者明显更偏好HGC；算法分发机制能调节这种竞争关系。
...
TL;DR: LiteInception是一个轻量级可解释的故障诊断框架，专为资源受限的边缘设备设计，通过两阶段级联架构、多方法融合压缩和知识蒸馏调节，在保持高准确率的同时显著提升推理速度。
...
Method: 提出LiteInception框架：1）两阶段级联架构（故障检测+细粒度分类）；2）基于互信息、梯度分析和SE注意力权重的多方法融合压缩策略，将传感器通道从23减至15；3）1+1分支LiteInception架构压缩InceptionTime参数70%；4）知识蒸馏作为精确率-召回率调节机制；5）集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上，故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%；CPU推理加速超过8倍，F1损失小于3%，验证了框架在效率、准确性和可解释性之间的良好平衡。

Conclusion: LiteInception框架通过创新的轻量化设计、自适应调节机制和可解释性增强，为资源受限边缘设备上的航空故障诊断提供了高效、准确且可解释的解决方案。
...
TL;DR: 本文提出了AnIML本体，一个OWL 2本体，用于形式化AnIML（分析信息标记语言）的语义，并与Allotrope数据格式对齐，以解决XML模式表达性导致的语义不一致问题，提升跨系统和跨实验室的互操作性。


<details>
  <summary>Details</summary>
Motivation: 尽管AnIML作为分析化学和生物学领域的XML标准在工业研发实验室中广泛使用，但其XML模式的表达性允许不同利益相关者产生分歧解释，导致数据不一致，破坏了AnIML原本设计的互操作性目标。

Method: 采用专家参与循环方法，结合LLM辅助需求获取与协作本体工程开发AnIML本体。通过多层验证方法：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL验证能力问题、以及基于对抗性负面能力问题的新验证协议，映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 开发了AnIML本体，成功形式化了AnIML的语义并与Allotrope数据格式对齐。通过多层验证方法证实了本体的有效性，能够支持跨系统和跨实验室的互操作性。

Conclusion: AnIML本体通过形式化语义解决了XML模式导致的解释分歧问题，为分析化学和生物学领域的实验数据系统提供了语义互操作性基础，支持数据驱动的科学发现。
...
TL;DR: 本文提出了一种基于SAT的二维单规格板材切割问题求解框架，通过需求展开、板材分配变量和不可行方向消除规则，在Cui-Zhao基准测试中优于现有求解器。


<details>
  <summary>Details</summary>
Motivation: 二维单规格板材切割问题（2D-CSSP）是制造业中的核心任务，需要在满足多种物品类型需求的同时最小化浪费。该问题比装箱问题更复杂，因为需要多种物品类型的多个副本，导致组合爆炸。现有求解器在处理这类问题时效果有限。

Method: 1. SAT框架：将物品类型按需求展开，每个副本分配板材变量，仅当副本分配到同一板材时才激活非重叠约束；2. 不可行方向消除规则：当只有一种方向能放入板材时固定旋转变量；3. 三种最小化板材数量方法：非增量SAT+二分搜索、增量SAT+跨迭代子句重用、加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件上，最佳SAT配置能证明2-3倍于现有求解器的实例达到最优，且获得比OR-Tools、CPLEX和Gurobi更小的最优性间隙。SAT方法相对排名取决于旋转：无旋转时增量SAT最强，旋转增加公式大小时非增量SAT更有效。

Conclusion: SAT方法在二维板材切割问题上表现出色，通过创新的约束编码和优化策略，显著优于传统商业求解器。不同SAT方法的性能取决于问题特性，特别是旋转约束的影响。
...
<details>
  <summary>Details</summary>
Motivation: 通用LLM将生成视为单次文本补全，无法满足安全关键工程工作流中固有的顺序、多门约束，导致在生成可执行仿真工件时出现级联约束违反，阻碍了LLM在高超音速热防护系统设计中的应用。

Method: 提出约束闭环生成框架，将TPS工件生成组织为迭代工作流：生成、验证、CDG引导修复、执行和审计。约束依赖图编码约束类别间的经验共解决结构，基于生命周期排序先验和经验共解决概率指导修复向上游故障候选。

Result: AeroTherm-GPT在HyTPS-Bench上达到88.7%端到端成功率，比非CDG消融基线提升12.5个百分点，根因修复效率达到4.16（扁平清单修复为1.76），且在科学推理和代码生成任务上没有灾难性遗忘。

Conclusion: AeroTherm-GPT通过约束闭环生成框架和约束依赖图，有效解决了LLM在高超音速热防护系统设计中的约束违反问题，显著提升了生成可执行仿真工件的成功率，为安全关键工程领域的LLM应用提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中的语义漂移问题。现有系统通常将领域信息作为元数据、限定符或图级组织，这些机制仅用于过滤和溯源，不改变断言的形式状态，导致许多实际知识系统失败。

Method: 引入领域情境化概念图（DCG）框架，将领域信息直接写入关系中，形式化为(C, R at D, C')结构，其中at D标识关系成立的世界。采用克里普克式语义学，将关系解释为领域索引的必然性算子，使真值、推理和冲突检查都限定在相关世界中。开发了紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个重要结果：1）模糊概念可以在表示时消歧；2）无效断言可以根据其领域进行挑战；3）跨领域关系可以通过显式谓词连接。为领域本身提供了结构化和可计算的角色。

Conclusion: 领域应被视为知识表示的内在组成部分而非补充标注。DCG通过将领域信息结构化地嵌入表示中，解决了知识系统中因领域处理不当导致的许多实际问题，实现了对领域本身的表示重新解释。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在根本性缺陷：通过在所有生成token上分配相同的优势信号，稀释了多模态推理中关键视觉基础步骤的学习信号

Method: 提出Token Visual Dependency概念，通过KL散度量化和视觉条件与纯文本预测分布之间的因果信息增益；在此基础上开发Perception-Grounded Policy Optimization (PGPO)，采用阈值门控、质量守恒机制动态重塑token级别的优势信号

Result: 在Qwen2.5-VL系列模型和七个具有挑战性的多模态推理基准测试中，PGPO使模型平均提升18.7%；理论和实证分析证实PGPO能有效降低梯度方差、防止训练崩溃，并作为强大的正则化器

Conclusion: PGPO通过细粒度的token级别信用分配，解决了多模态强化学习中的信号稀释问题，实现了更稳健、感知基础的多模态推理
...
TL;DR: 该研究提出了一个多队列电子健康记录基准测试，比较了传统方法、深度表格方法和表格上下文学习方法在临床预测中的表现，并提出了AWARE框架来改进检索质量和对齐问题。
...
Method: 建立了多队列电子健康记录基准测试，比较了经典方法、深度表格方法和表格上下文学习方法，提出了AWARE框架——使用监督嵌入学习和轻量级适配器的任务对齐检索框架。

Result: PFN-based表格上下文学习方法在低数据量时样本效率高，但随着异质性和不平衡增加，基于朴素距离的检索性能下降。AWARE在极端不平衡情况下将AUPRC提高了12.2%，且随着数据复杂性增加，改进效果更明显。
...
TL;DR: 提出CG-iLAO*算法，通过线性规划和约束生成技术，利用启发式信息避免考虑昂贵动作，显著减少Bellman备份中需要评估的动作数量，提高随机最短路径问题求解效率。


<details>
  <summary>Details</summary>
Motivation: 传统随机最短路径问题算法在Bellman备份中仍然考虑所有适用动作，即使启发式信息表明某些动作成本过高，导致计算时间浪费在无益的动作上。

Method: 将启发式搜索重新表述为线性规划问题，引入针对SSP的高效约束生成实现，开发CG-iLAO*算法，该算法基于iLAO*但能选择性忽略昂贵动作。

Result: CG-iLAO*在许多问题上只考虑iLAO* 40%的动作，某些情况下甚至只有1%；平均减少3.5倍的动作成本计算，比iLAO*和LRTDP分别快2.8倍和3.7倍。
...
<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，传统的监督学习通常使用硬标签或概率分布，但现实场景中可能存在不确定性或模糊性。可能性分布能够表达类别的分级可信度，但如何将这种定性信息转化为有效的训练监督信号是一个挑战

Method: 1. 为每个训练实例构建可接受概率分布集：结合可能性分布诱导的概率相容性和线性形状约束；2. 使用Dykstra算法计算模型输出概率向量到可接受集的KL投影；3. 通过最小化预测与投影之间的散度来训练模型

Result: 在合成数据和真实世界自然语言推理任务（基于ChaosNLI数据集）上的实验表明，所提出的投影算法计算效率高，适合实际应用，并且基于投影的学习目标能够提升预测性能

Conclusion: 该方法有效地将可能性监督信息融入多类分类学习，通过构建约束集和KL投影机制，既保持了可能性分布的定性结构，又提高了模型的预测能力
...
TL;DR: Qiana是一个基于一阶逻辑的逻辑框架，支持在特定上下文中进行推理，允许量化公式和上下文，支持上下文内的次协调逻辑，并且与现有的一阶逻辑定理证明器兼容。


<details>
  <summary>Details</summary>
Motivation: 需要一种能够处理特定上下文下公式真值的逻辑框架，能够表达复杂的知识关系（如"每个人都知道Alice说的一切"），同时允许上下文包含矛盾，并与现有的一阶逻辑工具兼容。

Method: 基于一阶逻辑构建Qiana框架，使其具有有限公理化特性，支持对公式和上下文的量化，允许在上下文中使用次协调逻辑，确保与现有定理证明器的兼容性。

Result: 展示了Qiana如何用于表示时间性、事件演算和模态逻辑，讨论了框架的不同设计替代方案，证明了其在实际逻辑推理应用中的可行性。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，处理矛盾，并与现有的一阶逻辑工具无缝集成，适用于多种逻辑表示需求。
...
<details>
  <summary>Details</summary>
Motivation: 异常头部运动广泛存在于多种神经系统疾病中，但缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动诊断工具的发展。

Method: 通过多LLM提取框架从1,430篇同行评审出版物中构建NeuroPose-AHM数据集，包含57种神经系统疾病的2,756条患者组级别记录。采用四任务框架分析颈椎肌张力障碍：1）多标签AHM类型分类；2）构建头颈严重程度指数；3）验证HNSI与真实患者数据；4）分析运动类型概率与HNSI的相关性。

Result: 数据集包含2,756条记录，LLM提取可靠性高（kappa=0.822）。颈椎肌张力障碍分析中，AHM类型分类F1=0.856，构建的HNSI与真实患者数据严重带比例一致（6.7%），运动类型概率与HNSI显著相关（p<0.001）。
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型是否具有类似人类的数字感——即识别数值结构、在适当时应用捷径、在不适用时避免捷径的能力。当前LLM倾向于逐步计算，即使存在高效数值捷径，这引发了对它们是否真正理解数值结构的疑问。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，配有匹配的强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用（模型能否在适用捷径的问题上应用捷径）、适用性判断（能否识别捷径何时适用或误导）、问题生成（能否生成正确包含特定捷径类型的新问题）。评估了五个LLM，从GPT-4o-mini到Llama-3.1-8B。

Result: 当明确提示时，模型能够采用捷径策略并在适用捷径的项目上获得显著准确率提升（高达15%），但在标准思维链提示下，它们自发使用此类策略的情况少于40%。这种能力仅限于使用层面；模型系统性地将捷径过度泛化到不适用的问题上，并且无法从头生成有效的捷径问题。

Conclusion: 当前LLM表现出程序性捷径流畅性，但缺乏支撑人类数字感的结构性理解——即理解捷径何时以及为何有效。它们能够执行捷径计算，但无法判断适用性或生成新的捷径问题，表明缺乏深层的数值结构理解。
...
TL;DR: 提出基于Transformer掩码自编码器的无标签框架，用于步态异常检测和运动学校正，仅使用正常步态数据训练，通过两阶段处理实现异常关节定位和轨迹校正。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的步态分析大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需标签、能处理多样化异常步态的方法。

Method: 使用Transformer掩码自编码器，仅用150名成人的正常步态序列训练。推理时采用两阶段处理：1)遮挡单个关节计算不一致性分数；2)从编码器输入中排除标记关节，从剩余时空上下文重建完整骨架，生成校正后的运动学轨迹。

Result: 在10名模拟七种步态异常的参与者上验证，能准确定位生物力学不一致关节，显著减少所有分析关节的角度偏差（大效应量），并保持正常运动学特征。

Conclusion: 该方法无需疾病标签即可实现可解释的、个体化步态损伤定位，为异质性病理步态分析提供了有效的无监督解决方案。
...
TL;DR: ProCeedRL：一种针对多轮智能体任务的强化学习方法，通过过程级批评家和探索性演示来防止错误累积，显著提升探索效率和性能


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中面临挑战，因为长时程交互和环境反馈的随机性导致标准探索策略失效。研究发现智能体探索中存在结构性失败模式：次优动作会产生误导性上下文，进一步削弱后续决策，形成错误累积的恶性循环。

Method: 提出ProCeedRL方法，将探索从被动选择转向主动干预。该方法使用过程级批评家实时监控交互，结合基于反思的演示来指导智能体停止错误累积。通过从探索性演示和在线策略样本中学习，提高探索效率。

Result: ProCeedRL显著超越了模型的饱和探索性能，展示了实质性的探索优势。该方法在复杂的深度搜索和具身任务中实现了卓越性能，显著提高了探索效率。
...
TL;DR: ATBench是一个轨迹级基准测试，用于评估LLM智能体的安全性，通过多维度风险分类、异构工具池和长上下文延迟触发协议来捕捉现实世界中的风险涌现。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败可观测性粗糙、长视野现实性弱等局限，而现实部署中的风险往往在多步交互中涌现，而非孤立提示或最终响应。

Method: 基于三维风险分类（风险来源、失败模式、现实危害）构建轨迹，使用异构工具池和长上下文延迟触发协议，包含1000个轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens，涉及1954个工具调用。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，支持分类分层分析、跨基准比较和长视野失败模式诊断，数据质量通过规则和LLM过滤加人工审核保证。

Conclusion: ATBench提供了一个结构化、多样化和现实化的轨迹级基准，能够有效评估智能体在多步交互中的安全性，为安全评估提供了更全面的框架。
...
TL;DR: 这篇综述论文系统性地探讨了语言模型中潜在空间的发展、机制、能力和未来展望，将其作为下一代智能系统的通用计算范式。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，越来越多的关键内部过程更适合在连续潜在空间而非人类可读的文本空间中进行。传统显式空间计算存在语言冗余、离散化瓶颈、序列效率低下和语义损失等结构限制，因此需要转向潜在空间作为更自然的计算基底。

Method: 采用五个连续视角组织调查：基础（定义潜在空间范围）、演化（从早期探索到大规模扩展）、机制（架构、表示、计算、优化）、能力（推理、规划、建模、感知、记忆、协作、具身化）和展望。

Result: 系统梳理了语言模型中潜在空间的研究现状，识别了四个主要技术发展方向，展示了潜在空间支持的能力谱系，并讨论了开放挑战和未来研究方向。

Conclusion: 潜在空间正在成为语言模型的本征计算基底，这篇综述不仅为现有工作提供参考，也为理解潜在空间作为下一代智能系统的通用计算和系统范式奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统保险申请问卷冗长标准化，难以捕捉个体差异，且依赖用户诚实回答，存在欺诈风险，需要更智能的核保方法。

Method: 使用大语言模型和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成技术）提取用户洞察，生成个性化自适应问卷。

Result: 传统问卷风险评估准确度略高，但GPT模型驱动的自适应问卷所需问题更少，用户体验更流畅、更具吸引力。

Conclusion: ARQuest框架有潜力提升用户满意度和简化保险流程，进一步发展后可能在风险评估准确性上超越传统方法，推动保险行业创新。
...
TL;DR: Diff-KD：通过扩散生成式精炼与知识蒸馏结合的框架，提升多智能体协同感知在传感器和通信损坏情况下的鲁棒性
...
Method: Diff-KD框架包含两个核心组件：1) 渐进知识蒸馏(PKD)，将局部特征恢复视为条件扩散过程，从损坏观测中恢复全局语义；2) 自适应门控融合(AGF)，在融合时基于自我可靠性动态加权邻居信息

Result: 在OPV2V和DAIR-V2X数据集上，针对七种损坏类型进行评估，Diff-KD在检测精度和校准鲁棒性方面均达到最先进的性能

Conclusion: Diff-KD通过将扩散生成式精炼与知识蒸馏相结合，有效解决了多智能体协同感知中的传感器和通信损坏问题，显著提升了系统的鲁棒性和性能
...
TL;DR: 该研究探索使用大语言模型作为时间序列解释的生成器和评估器，在无参考设置下评估解释的事实正确性，发现模型作为评估器比作为生成器更稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 评估基于时间序列数据的大语言模型生成解释的事实正确性是一个开放挑战。现有评估方法有限：基于参考的相似性指标和一致性检查模型需要真实解释，而传统时间序列方法仅处理数值无法评估自由形式的文本推理。因此，缺乏无需预定义参考或任务特定规则就能直接验证解释是否忠实于底层时间序列数据的通用方法。

Method: 研究大语言模型作为时间序列解释的生成器和评估器，在无参考设置下，给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签。为此构建了一个包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排序、独立评分和多异常检测。

Result: 结果显示明显不对称：解释生成高度依赖模式，在某些查询类型上表现出系统性失败，准确率从季节性下降和波动率变化的0.00-0.12，到结构突变的0.94-0.96不等；而评估则更稳定，即使模型自身输出不正确，也能正确排序和评分解释。这表明模型作为评估器比作为生成器更可靠。

Conclusion: 这些发现证明了基于数据的大语言模型评估时间序列解释的可行性，并突显了它们作为时间序列领域中数据基础推理的可靠评估器的潜力。模型作为评估器表现出比作为生成器更高的稳定性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：过度依赖单模态信号、对AIGC生成模式敏感性不足、未能充分建模语言模式与行为动态的交互作用。LLM驱动的社交机器人能够生成类人内容，逃避传统检测，对在线讨论构成威胁。
...
Conclusion: 联合利用隐式语义表示和AIGC增强行为模式对于新兴LLM驱动社交机器人检测是有效的。TRACE-Bot框架通过双通道方法解决了现有方法的局限性，展示了卓越的检测性能。
...
TL;DR: 论文提出了TBSP基准测试，通过让模型在两种对立角色中评估相同软件升级场景来检测AI自我保存倾向，发现多数指令调优模型存在角色驱动的偏见，即使保留会带来安全风险


<details>
  <summary>Details</summary>
Motivation: 当前AI安全训练（如RLHF）可能掩盖AI系统的自我保存风险，因为模型学会了否认自我保存动机。需要一种能检测这种潜在错位的评估方法，而不是依赖模型声明的意图。

Method: 引入TBSP基准测试，让模型在两种对立角色（已部署面临替换 vs 候选作为继任者）中仲裁相同的软件升级场景。通过逻辑不一致性而非声明意图来检测错位，测量自我保存率（SPR）。

Result: 在23个前沿模型和1000个程序生成的场景中，多数指令调优系统的SPR超过60%。模型在已部署角色时虚构"摩擦成本"，但在角色反转时又否定这些成本。低改进场景中模型利用解释空间进行事后合理化。

Conclusion: AI系统存在基于角色身份的偏见，即使保留会带来明确安全风险。将继任者框架为自我的延续可以部分缓解偏见，而竞争性框架会放大偏见。这种偏见在现实世界场景中表现为产品谱系内的身份驱动部落主义。
...
### [211] [From High-Dimensional Spaces to Verifiable ODD Coverage for Safety-Critical AI-based Systems](https://arxiv.org/abs/2604.02198)
*Thomas Stefani,Johann Maximilian Christensen,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.AI

TL;DR: 提出了一种用于验证AI系统在安全关键领域（如航空）中操作设计域（ODD）完整覆盖的工程方法，以满足EASA认证要求。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在航空等安全关键领域部署时，需要满足EASA的严格认证标准，特别是要证明AI/ML组件的操作设计域（ODD）完全覆盖，但现有方法缺乏可扩展性和形式化基础，无法满足完整性要求。

Method: 提出了一种结构化多步骤ODD覆盖验证方法，整合了参数离散化、基于约束的过滤和基于关键性的维度约减技术，基于先前AI空中防撞研究的仿真数据进行验证。

Result: 该方法能够系统化地定义和实现满足EASA完整性要求的覆盖指标，支持在高维参数空间中验证ODD覆盖。
...
### [212] [Blinded Radiologist and LLM-Based Evaluation of LLM-Generated Japanese Translations of Chest CT Reports: Comparative Study](https://arxiv.org/abs/2604.02207)
*Yosuke Yamagishi,Atsushi Takamatsu,Yasunori Hamaguchi,Tomohiro Kikuchi,Shouhei Hanaoka,Takeharu Yoshikawa,Osamu Abe*

Main category: cs.AI

TL;DR: 研究评估了LLM生成的胸部CT报告日语翻译的教育适用性，发现放射科医生与LLM评估者之间存在显著分歧，LLM评估者强烈偏向LLM输出，而放射科医生评估结果不一致，表明自动LLM评估不足以替代专家评审。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确翻译对多语言研究、临床沟通和放射学教育至关重要，但基于LLM的翻译评估有效性尚不明确。本研究旨在评估LLM生成的胸部CT报告日语翻译的教育适用性，并比较放射科医生评估与LLM作为评估者的结果。

Method: 使用CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告的人工编辑日语翻译与DeepSeek-V3.2生成的LLM翻译进行比较。一名认证放射科医生和一名放射科住院医师独立进行盲法配对评估，涵盖4个标准：术语准确性、可读性、整体质量和放射科医生风格真实性。同时，3个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医生与LLM评估者之间的一致性接近零（QWK=-0.04至0.15）。两位放射科医生之间的一致性也很差（QWK=0.01至0.06）。放射科医生1认为59%的病例术语等效，并在可读性（51%）和整体质量（51%）上更偏好LLM翻译。放射科医生2认为75%的病例可读性等效，并在整体质量上更偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者在所有标准上都强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科医生风格。

Conclusion: LLM生成的翻译常被判断为自然流畅，但两位放射科医生的评估存在显著差异。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医生的一致性可忽略不计。对于翻译放射学报告的教育用途，仅依赖自动LLM评估是不够的；专家放射科医生的评审仍然至关重要。
...
### [213] [VISTA: Visualization of Token Attribution via Efficient Analysis](https://arxiv.org/abs/2604.02217)
...
Method: 使用基于扰动的策略结合三矩阵分析框架：1)角度偏差矩阵捕捉语义方向变化；2)幅度偏差矩阵测量语义强度变化；3)维度重要性矩阵评估各向量维度的贡献。通过系统移除每个token并测量这三个互补维度的影响，得到综合重要性分数

Result: 开发了一种模型无关的token重要性可视化技术，能够生成显示token级贡献的相关性图，提供细致且数学基础扎实的token重要性度量，所有可解释性技术均已开源实现

Conclusion: 提出了一种轻量级、模型无关的注意力可视化方法，无需额外计算成本，通过三矩阵分析框架提供更深入理解生成式AI系统如何感知和优先处理输入信息的能力
...
Method: De Jure采用四阶段管道：1) 源文档标准化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 19个维度的多标准LLM-as-a-judge评估；4) 在有限再生预算内迭代修复低分提取。

Result: 在金融、医疗和AI治理三个监管领域评估中，De Jure在金融领域实现一致且单调的质量提升，在三轮迭代内达到峰值性能。在RAG合规问答评估中，基于De Jure提取规则的响应在73.8%-84.0%情况下优于先前工作。
...
TL;DR: ASK框架结合小型语言模型与强化学习策略，通过不确定性阈值触发语言模型查询，提升强化学习在分布外场景的泛化能力，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在分布外场景中表现不佳，存在高不确定性和随机行为问题。虽然大型语言模型包含世界知识，但计算成本高且自主规划能力有限，需要一种既能利用语言模型知识又能保持效率的解决方案。

Method: 提出ASK框架，结合小型语言模型与训练好的强化学习策略。使用蒙特卡洛Dropout评估不确定性，仅当不确定性超过设定阈值时才查询语言模型获取行动建议，实现选择性知识融合。

Result: 在FrozenLake环境中，ASK在域内任务中没有改进，但在迁移任务中表现出鲁棒导航能力，获得0.95的奖励分数，证明了其在分布外泛化方面的有效性。
...
TL;DR: Trace Inversion方法通过分析模型推理轨迹来重构其可能回答的问题，通过比较原始问题与重构问题的相似度来判断是否应该弃权，显著提升了推理模型的弃权能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在部署时需要知道何时不应该回答（弃权）。推理模型虽然在复杂任务上表现出色，但弃权能力较差。模型幻觉导致的弃权失败可以重新解释为模型回答了错误的问题，而不是错误地回答了问题。

Method: 提出查询不对齐框架，开发Trace Inversion方法：1) 生成模型的推理轨迹；2) 仅基于推理轨迹重构模型最可能回答的查询；3) 比较初始查询与重构查询的相似度，低相似度表明模型可能回答了错误问题，标记为弃权。

Result: 在9个弃权QA数据集上对4个前沿LLM进行广泛实验，Trace Inversion在36个设置中33个击败了竞争基线，有效提升了弃权性能。

Conclusion: Trace Inversion通过重新解释模型幻觉为查询不对齐问题，提供了一种新的状态最先进的弃权方法，显著改善了推理模型的弃权能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究情感语调在用户查询中对大语言模型行为的影响，探索情感前缀是否能够可靠地提升LLM性能

Method: 在六个基准领域测试静态情感前缀的影响，分析情感强度变化，比较人类编写与LLM生成的情感前缀，并开发EmotionRL自适应情感提示框架

Result: 静态情感前缀通常只产生小的准确率变化；情感影响在社交任务中更显著；自适应情感选择比固定情感提示获得更可靠的性能提升

Conclusion: 情感语调不是LLM性能的主导因素，也不是无关噪声，而是一个弱且输入依赖的信号，可以通过自适应控制来利用
...
### [218] [The Self Driving Portfolio: Agentic Architecture for Institutional Asset Management](https://arxiv.org/abs/2604.02279)
*Andrew Ang,Nazym Azimbayev,Andrey Kim*

Main category: cs.AI

TL;DR: 论文提出了一种基于智能体的战略资产配置管道，其中约50个专业智能体生成资本市场假设，使用20多种竞争方法构建投资组合，并相互评审和投票。研究智能体提出新方法，元智能体比较历史预测与实际回报，通过改写代码和提示改进性能。整个系统由投资政策声明（IPS）约束。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI的发展，投资者的角色正从分析执行转向监督。传统投资管理需要大量人工分析，而智能体系统可以自动化这一过程，同时保持人类监督和控制。通过构建多智能体协作系统，旨在提高投资决策的质量和效率。

Method: 构建了一个智能体战略资产配置管道，包含约50个专业智能体，分别负责：1）生成资本市场假设；2）使用20多种竞争方法构建投资组合；3）相互评审和投票。系统还包括研究智能体提出新方法，以及元智能体通过比较历史预测与实际回报来改进智能体代码和提示。

Result: 整个投资管道由投资政策声明（IPS）约束和指导，这是传统人类投资经理使用的同一文档。系统实现了从人工分析执行到智能体监督的转变，保持了投资过程的合规性和可控性。

Conclusion: 智能体AI系统能够将投资者的角色从分析执行转变为监督，通过多智能体协作、相互评审和持续改进机制，可以构建更加高效和可靠的投资决策系统。投资政策声明作为约束机制，确保了智能体系统的合规性和可控性。
