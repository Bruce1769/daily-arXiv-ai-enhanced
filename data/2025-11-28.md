...
TL;DR: G²VLM是一个几何基础的视觉语言模型，通过整合3D视觉几何特征来增强空间理解和推理能力，在3D重建和空间理解任务上表现出色。
...
Result: 实验结果显示G²VLM在两项任务上都很熟练，在3D重建任务上达到最先进水平，在空间理解和推理任务上取得更好或具有竞争力的结果。

Conclusion: 通过将语义强大的VLM与低层次3D视觉任务统一，G²VLM可以作为社区的强基线，并解锁更多未来应用如3D场景编辑。
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，支持256K token的交错上下文，在多种多模态基准测试中表现优异，提供密集型和MoE变体以适应不同延迟-质量权衡。
...
Method: 采用增强的交错MRoPE进行时空建模，集成DeepStack以加强视觉语言对齐，使用基于文本的时间对齐进行视频处理，并提供密集型和MoE架构变体。

Result: 在MMMU、MathVista和MathVision等综合评估中取得领先性能，在纯文本理解方面超越同类文本模型，支持256K token的长上下文处理。

Conclusion: Qwen3-VL作为基础引擎，在现实工作流程中为图像推理、智能决策和多模态代码智能提供了强大的多模态能力。
...
TL;DR: 提出了一种新颖的AI错误校正系统，通过少样本学习在资源受限设备上高效修正AI误分类，结合服务器端基础模型训练和设备端原型分类，实现原型更新而非模型重训练的错误校正。
...
Method: 系统包含两个关键组件：(1) 服务器端管道利用知识蒸馏将基础模型的鲁棒特征表示转移到设备兼容架构；(2) 设备端机制通过原型适应实现超高效错误校正。
...
TL;DR: ADVLA是一种针对视觉-语言-动作(VLA)模型的特征空间对抗攻击框架，通过在视觉编码器投影到文本特征空间的特征上直接施加扰动，在低幅度约束下有效破坏下游动作预测，具有注意力引导的聚焦稀疏特性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法需要昂贵的端到端训练且通常生成明显的扰动补丁，为了解决这些限制，提出直接在特征空间进行攻击的框架。

Method: ADVLA框架在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，引入三种策略增强敏感性、强制稀疏性和集中扰动，结合Top-K掩码实现局部稀疏攻击。
...
TL;DR: MoGAN是一个专注于提升视频生成运动真实性的后训练框架，通过光流判别器和分布匹配正则化器，在保持视觉质量的同时显著改善运动连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型在帧级保真度方面表现良好，但在运动连贯性、动态性和真实性方面存在不足，经常产生抖动、重影或不合理的动态。标准去噪MSE目标缺乏对时间一致性的直接监督。

Method: 基于3步蒸馏的视频扩散模型，训练基于DiT的光流判别器来区分真实与生成的运动，并结合分布匹配正则化器来保持视觉保真度。

Result: 在VBench上，MoGAN比50步教师模型提升运动得分+7.3%，比3步DMD模型提升+13.3%；在VideoJAM-Bench上分别提升+7.4%和+8.8%，同时保持相当或更好的美学和图像质量得分。人类研究也证实MoGAN在运动质量上更受偏好。
...
<details>
  <summary>Details</summary>
Motivation: 长期动作质量评估（如花样滑冰或艺术体操）具有挑战性，需要建模扩展的时间动态并保持对上下文混杂因素的鲁棒性。现有方法要么依赖昂贵的注释，要么依赖单向时间建模，容易受到伪相关性和不稳定长期表示的影响。

Method: 提出CaFlow框架，包含因果反事实正则化（CCR）模块和BiT-Flow模块。CCR以自监督方式解耦因果特征和混杂特征，通过反事实干预增强因果鲁棒性；BiT-Flow通过循环一致性约束建模前向和后向动态，产生更平滑和连贯的表示。

Result: 在多个长期AQA基准测试上的广泛实验表明，CaFlow实现了最先进的性能。

Conclusion: CaFlow通过整合反事实去混淆和双向时间条件流，有效解决了长期动作质量评估中的挑战，在多个基准测试中表现出优越性能。
...
TL;DR: Multi-Crit是一个评估多模态模型作为评判者的基准，重点测试其遵循多样化细粒度标准的能力，揭示了现有模型在遵循多元标准方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型(LMMs)越来越多地被用作多模态评估系统的评判者，但它们遵循多样化、细粒度评估标准的能力尚未得到充分探索。

Method: 通过严格的数据整理流程构建Multi-Crit基准，收集具有多标准人工标注的挑战性响应对，并引入三个新指标系统评估多元遵循性、标准切换灵活性和识别标准级偏好冲突的能力。

Result: 对25个LMMs的综合分析表明：1)专有模型在保持对多元标准的一致遵循方面仍有困难，特别是在开放式评估中；2)开源模型在灵活遵循多样化标准方面更落后；3)使用整体判断信号进行批评微调增强了视觉基础能力，但无法泛化到多元标准级判断。

Conclusion: Multi-Crit为构建可靠和可操控的多模态AI评估奠定了基础，揭示了当前多模态评判者在遵循多样化评估标准方面的局限性。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）来感知视频内容的可行性，提出CamFormer对比学习框架，证明相机轨迹是揭示视频内容的有效信号。
...
Result: 发现相机轨迹是异常信息丰富的信号，能够有效揭示视频内容，学习到的表示在跨模态对齐、分类和时间分析等任务中表现优异，且对不同的相机姿态估计方法具有鲁棒性。
...
Method: 将不同的控制信号编码到单一复合画布图像中，模型可以直接解释该画布进行集成的视觉空间推理；策划多任务数据集并提出多任务画布训练策略，在统一学习范式中优化扩散模型以联合理解和整合异构控制。
...
TL;DR: 本文提出了一种简单轻量的方法来识别大型语言模型中编码特定技能的神经元，通过将神经元激活与外部标签和模型置信度等辅助指标相关联，在复杂多技能场景中揭示可解释的任务特定行为。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在各种任务中表现出卓越能力，但其内部机制仍然不透明。本文旨在开发一种方法来隔离编码特定技能的神经元，以增强模型的可解释性。
...
Conclusion: 提出的方法能够有效识别大型语言模型中编码特定技能的神经元，增强模型可解释性，并在复杂任务中揭示新的行为模式。
...
TL;DR: 本文研究了在大型语言模型预训练中利用多种元数据（如文档质量指标）来加速训练的方法，发现细粒度元数据编码信息更有效，并提出了元数据附加和可学习元标记等技术来提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 先前工作仅关注URL作为元数据信号，本研究旨在探索其他类型的元数据是否能带来更大的训练加速效益。

Method: 研究了多种元数据类型，引入元数据附加作为辅助任务，使用可学习元标记配合掩码损失，并通过探针分析潜在表示。

Result: 发现细粒度文档质量指标等元数据能加速预训练，元数据附加和可学习元标记都能有效提升训练效率。

Conclusion: 研究结果为整合元数据以改进LLM预训练的效率和效果提供了实用指南，细粒度元数据编码是提高训练效率的关键特征。
...
TL;DR: 研究表明AI生成的捷克诗歌与人类创作难以区分，参与者识别准确率仅45.8%。当参与者认为诗歌是AI创作时，评价会更低，尽管实际上AI诗歌评分与人类相当或更高。


<details>
  <summary>Details</summary>
Motivation: 研究AI在捷克语（形态复杂、资源较少的斯拉夫语言）中创作诗歌的能力，以及人们对AI和人类创作诗歌的识别能力和审美评价差异。

Method: 让捷克母语者识别AI和人类创作的诗歌作者身份，并进行审美评价，使用逻辑回归模型分析识别准确率与审美评价的关系。

Result: 参与者识别准确率接近随机水平（45.8%），AI诗歌与人类诗歌难以区分。存在作者偏见：当认为诗歌是AI创作时评分更低，尽管AI诗歌实际评分相当或更高。

Conclusion: AI能够在捷克语等复杂低资源语言中创作出令人信服的诗歌，读者的作者信念与审美评价密切相关，诗歌熟悉度或文学背景不影响识别准确性。
...
Method: ToolOrchestra方法训练小型编排器，使用强化学习结合结果、效率和用户偏好感知的奖励来协调各种工具。

Result: Orchestrator在HLE上得分37.1%，超越GPT-5（35.1%）且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，仅使用约30%成本。
...
TL;DR: 本文通过系统评估大型语言模型在不同任务难度下的泛化能力，发现跨难度泛化通常有限，训练数据中需要包含多种难度级别的样本。
...
Method: 使用数千种不同LLM的输出和项目反应理论（IRT）对六个数据集中的示例进行难度排序，完全基于模型能力而非人类主观判断来确定难度评级。

Result: 跨难度泛化能力有限，仅使用简单或困难数据训练无法在全部难度范围内获得一致改进。
...
TL;DR: 该研究测试了大型语言模型在8-puzzle任务中的规划和状态推理能力，发现即使有反馈机制和外部验证器辅助，模型仍存在内部状态表示脆弱和启发式规划能力弱的问题。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在规划和状态推理方面的真实能力，特别是在需要状态跟踪和目标导向规划的任务中，而不依赖代码执行或其他工具。

Method: 使用8-puzzle任务测试四个模型，采用零样本、思维链和算法思维等提示方法，并引入分层纠正反馈和外部移动验证器。

Result: 反馈机制对某些模型-提示组合有改善，但成功运行通常冗长且计算昂贵。即使有外部验证器提供有效移动，所有模型都无法解决任何谜题。定性分析显示模型存在内部状态表示脆弱和启发式规划能力弱两大缺陷。
...
<details>
  <summary>Details</summary>
Motivation: AI/ML模型在解决难题的同时可能放大人类偏见，需要更丰富的因果模型来支持负责任AI/ML的发展，但不同方法的基础假设差异构成了主要障碍。

Method: 将系统动力学和结构方程建模整合到一个共同的数学框架中，能够从分布生成系统、开发方法并比较结果。

Result: 建立了一个统一的数学框架，弥合了系统动力学和结构方程建模之间的方法论差异。
