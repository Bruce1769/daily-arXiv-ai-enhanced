...
TL;DR: ABot-M0是一个面向通用具身智能体的框架，通过统一数据管道和动作流形学习，解决了机器人领域数据碎片化、表示不一致的问题，支持跨硬件平台的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 构建跨多样化硬件的通用具身智能体面临"一个大脑，多种形态"的挑战，主要障碍包括数据碎片化、表示不一致以及训练目标不匹配。需要系统性的解决方案来整合异构数据并实现跨平台泛化。

Method: 1) 构建系统化数据管道，从6个公开数据集中清洗、标准化并平衡样本，创建UniACT-dataset（包含600万轨迹、9500小时数据）；2) 提出动作流形假设，认为有效机器人动作位于低维平滑流形上，并基于此开发动作流形学习(AML)，使用DiT主干直接预测干净连续的动作序列；3) 采用双流感知机制，集成VLM语义与几何先验，结合可插拔3D模块增强空间理解。

Result: 构建了大规模统一数据集UniACT-dataset，覆盖多样化机器人形态和任务场景。统一预训练提升了跨平台和跨任务的知识迁移与泛化能力。动作流形学习提高了动作预测效率和策略稳定性。各组件独立运行且具有累加效益。

Conclusion: ABot-M0框架通过系统化数据管道、动作流形学习和模块化感知机制，为构建跨硬件平台的通用具身智能体提供了有效解决方案，支持端到端的异构数据处理和统一表示学习，推动了通用具身智能的发展。
...
TL;DR: 本文开发了一个基于深度学习的茶叶病害自动分类系统，使用DenseNet201和EfficientNetB3模型在TeaLeafBD数据集上实现高精度病害识别，最高准确率达93%。


<details>
  <summary>Details</summary>
Motivation: 茶叶是孟加拉国的重要经济作物，但茶叶叶片容易受到各种病害侵袭，导致产量下降和品质降低。传统的人工检测方法耗时且容易出错，因此需要开发自动化深度学习模型来高效准确地识别茶叶病害。

Method: 研究采用TeaLeafBD数据集（包含5,278张高分辨率图像，分为7个类别：6种病害和1种健康叶片）。方法包括数据预处理、数据分割、对抗训练、数据增强、模型训练和评估。使用DenseNet201和EfficientNetB3进行病害分类，并通过对抗训练增强模型鲁棒性。采用Grad-CAM可视化技术分析模型预测依据。

Result: 实验结果显示，EfficientNetB3取得了最高的分类准确率93%，DenseNet201达到91%准确率。模型能够有效识别茶叶叶片病害，对抗训练使模型在噪声或干扰输入下仍能有效工作。

Conclusion: 提出的深度学习方法能够准确检测茶叶叶片病害，为现代农业管理提供了实用的解决方案。该自动化系统使病害检测更加容易和高效，有助于提高茶叶产量和质量。
...
TL;DR: Active-Zero框架通过主动探索视觉环境，让视觉语言模型自主构建学习轨迹，相比被动交互方法显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的自对弈方法依赖静态图像集的被动交互，导致对初始数据集的强依赖和低效学习。模型无法主动寻找适合其当前能力的视觉数据，浪费计算资源在过于简单或困难的样本上

Method: 提出Active-Zero框架，包含三个协同进化的智能体：1) Searcher从开放世界存储库中根据模型能力边界检索图像；2) Questioner合成校准的推理任务；3) Solver通过准确性奖励进行精炼。形成闭环的自支撑自动课程学习

Result: 在Qwen2.5-VL-7B-Instruct模型上，12个基准测试中，推理任务平均准确率达到53.97%（提升5.7%），通用理解达到59.77%（提升3.9%），持续优于现有自对弈基线方法
...
TL;DR: ReTracing是一个多智能体具身表演艺术项目，采用考古学方法研究AI如何塑造、约束和产生身体运动。通过LLM生成动作指令，扩散模型转换为人类和机器人的编舞指导，在镜像地板上表演并记录为3D点云和运动轨迹的数字档案。
...
Method: 从科幻小说中提取描述人机交互的句子，使用LLM为每个摘录生成"做什么"和"不做什么"的配对提示，通过基于扩散的文本到视频模型将这些提示转换为人类表演者和四足机器人的编舞指导，在镜像地板上表演，通过多摄像头运动捕捉重建为3D点云和运动轨迹。

Result: 创建了一个运动痕迹的数字档案，展示了AI、人类和机器人之间的沉浸式互动，揭示了生成系统如何通过编排的运动编码社会文化偏见。

Conclusion: ReTracing提供了一种新颖的方法来研究AI如何塑造身体运动，并引发对在AI也能移动、思考和留下痕迹的时代人类身份本质的深刻反思。
...
TL;DR: 本文提出REVEAL基准测试，揭示当前视频语言模型在视频内容理解、时间序列处理、运动感知等方面的根本性缺陷，发现模型存在时间期望偏差、语言捷径依赖、视频谄媚、相机运动敏感性和时空遮挡鲁棒性不足等问题。


<details>
  <summary>Details</summary>
Motivation: 研究视频语言模型是否真正能够稳健地理解视频内容、时间序列和运动信息，发现现有模型在这些基础能力上存在严重缺陷，需要建立诊断性基准来系统评估模型弱点。

Method: 提出REVEAL诊断基准，包含五个受控压力测试：时间期望偏差测试、语言捷径依赖测试、视频谄媚测试、相机运动敏感性测试和时空遮挡鲁棒性测试。提供自动生成诊断示例的数据管道，测试领先的开源和闭源视频语言模型。

Result: 测试发现当前视频语言模型存在严重问题：自信地将反向场景描述为正向、回答问题忽略视频内容、同意虚假声明、难以处理基本相机运动、在简单时空遮挡下无法聚合时间信息。相比之下，人类在这些任务上表现轻松。

Conclusion: 当前视频语言模型在基础视频理解能力上存在系统性缺陷，需要更强大的评估基准来推动模型改进。REVEAL基准和代码将公开发布以支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D模型生成方法（摄影测量）存在多种设计选择，但不同方法的差异主要依赖定性评估，缺乏可重复、可量化的实验框架

Method: 开发了一个新流程：从高质量3D模型生成合成图像，并通过程序化生成相机位姿，创建可重复实验环境

Result: 该流程能够进行多种可重复、可量化的实验，将虚拟相机参数和虚拟物体的真实知识（ground-truth）与重建估计值进行比较

Conclusion: 提出的合成图像生成流程为摄影测量方法提供了系统化的评估框架，能够量化比较不同重建方法的性能
...
TL;DR: 本文提出了一种将学习到的深度先验直接集成到VINS-Mono优化后端的新框架，通过强制仿射不变深度一致性和成对序数约束，在边缘设备计算限制内实现鲁棒的度量尺度恢复。


<details>
  <summary>Details</summary>
Motivation: 传统单目视觉惯性里程计系统在低纹理环境中表现不佳，因为稀疏视觉特征不足以进行准确的姿态估计。虽然密集单目深度估计可以作为补充信息源，但基于ViT的复杂基础模型计算需求大，难以在边缘设备上实时部署。

Method: 提出了一种新颖框架，将学习到的深度先验直接集成到VINS-Mono优化后端，强制仿射不变深度一致性和成对序数约束，通过基于方差的门控机制显式过滤不稳定伪影。

Result: 在TartanGround和M3ED数据集上的大量实验表明，该方法在挑战性场景中防止发散，并带来显著的精度提升，绝对轨迹误差降低高达28.3%。

Conclusion: 该方法在严格遵循边缘设备计算限制的同时，实现了鲁棒的度量尺度恢复，为低纹理环境下的单目视觉惯性里程计提供了有效的解决方案。
...
TL;DR: 提出了StreamSR数据集和EfRLFN模型，用于解决压缩视频实时超分辨率的挑战，在真实流媒体场景中评估了11个SOTA模型，并展示了微调带来的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有实时超分辨率方法在处理压缩视频内容时面临独特挑战，常用数据集不能准确反映流媒体特征，限制了当前基准测试的相关性。

Method: 1. 从YouTube收集构建StreamSR数据集，涵盖多种视频类型和分辨率；2. 提出EfRLFN模型，集成高效通道注意力和双曲正切激活函数；3. 设计复合损失函数改善训练收敛；4. 在数据集上微调其他模型。

Result: 1. 基准测试了11个SOTA实时超分辨率模型；2. EfRLFN在视觉质量和运行时性能上均有提升；3. 在其他模型上微调后，在多种标准基准测试中均获得显著性能提升。

Conclusion: StreamSR数据集填补了流媒体超分辨率评估的空白，EfRLFN模型在效率和视觉质量上表现优异，微调策略能显著提升现有模型的泛化性能。
...
TL;DR: ArtContext：利用开放获取艺术史文章和维基数据知识库，通过定制化CLIP模型（PaintingCLIP）为艺术品提供上下文注释的管道系统


<details>
  <summary>Details</summary>
Motivation: 艺术史文章通常讨论艺术品的整体和具体部分（如布局、图像学、物质文化），但在观看艺术品时，很难快速识别不同文章对该作品的论述，需要一种系统化的方法来整合这些信息

Method: 1. 构建新颖的语料库收集管道，整合开放获取艺术史文章和维基数据知识；2. 使用低秩适应（LoRA）技术定制CLIP模型，使其适应艺术领域；3. 通过弱监督学习训练PaintingCLIP模型

Result: 新模型PaintingCLIP在性能上超越了原始CLIP模型，能够为给定艺术品提供有效的上下文信息

Conclusion: 提出的ArtContext管道具有通用性，可以轻松应用于众多人文学科领域，为解决艺术品上下文信息整合问题提供了有效方案
...
TL;DR: Latent Forcing：一种改进的扩散模型架构，通过在潜在空间和像素空间联合处理，实现高效的原生图像生成，避免了传统潜在扩散模型的信息丢失和解码器依赖问题。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型虽然能生成高质量图像，但存在三个主要问题：1）图像编码过程中会丢弃信息；2）需要单独训练的解码器；3）建模的是原始数据的辅助分布而非原始数据本身。这些缺点使得模型失去了端到端建模的优势。

Method: 提出Latent Forcing方法，通过联合处理潜在表示和像素，使用分别调优的噪声调度来排序去噪轨迹。潜在表示充当中间计算的"草稿纸"，在高频像素特征生成之前进行预处理。研究发现条件信号的顺序对模型性能至关重要。

Result: 在ImageNet数据集上，Latent Forcing在扩散变换器基础的像素生成任务中，在当前计算规模下达到了新的最先进水平。该方法还解释了REPA蒸馏在分词器和扩散模型中的差异、条件与非条件生成的差异，以及分词器重建质量与扩散能力的关系。

Conclusion: Latent Forcing通过简单的架构修改，既保持了潜在扩散的效率优势，又能在原始自然图像上操作，实现了端到端的建模，为扩散模型的发展提供了新的方向。
...
TL;DR: 利用CTA数据训练神经隐式函数，解决短轴CMRI的各向异性问题，实现任意分辨率的心脏形状重建


<details>
  <summary>Details</summary>
Motivation: 短轴心血管磁共振成像的各向异性限制了心脏形状分析，需要更高分辨率的各向同性数据来改善重建质量

Method: 使用近各向同性的高分辨率CTA数据训练单个神经隐式函数，联合表示任意分辨率的CMRI心脏形状，特别针对右心室和心肌的重建
...
TL;DR: Ctrl&Shift是一个端到端的扩散框架，通过两阶段分解（物体移除和参考引导修复）实现几何一致的物体操纵，无需显式3D重建，在背景保持、几何一致性和用户可控性方面达到SOTA。
...
Method: 将操纵分解为物体移除和参考引导修复两阶段，在统一扩散过程中编码；设计多任务多阶段训练策略分离背景、身份和姿态信号；构建可扩展的真实世界数据集生成带相对相机姿态的配对图像/视频样本。

Result: 广泛实验表明Ctrl&Shift在保真度、视角一致性和可控性方面达到最先进水平，是首个统一细粒度几何控制和真实世界泛化的物体操纵框架，无需任何显式3D建模。

Conclusion: Ctrl&Shift通过创新的两阶段扩散框架，成功解决了物体操纵中背景保持、几何一致性和可控性的联合挑战，为电影后期、AR和创意编辑提供了强大工具。
...
TL;DR: 提出ARFC框架，支持任意压缩比的单模型特征压缩，无需为不同压缩比训练专门模型，通过自回归压缩和两个关键模块提升压缩质量。
...
Method: 提出ARFC框架，核心是自回归压缩器(ARC)，通过下一令牌预测进行压缩，压缩比可通过生成令牌数量控制。引入两个关键模块：混合解决方案(MoS)模块利用多个压缩结果减少不确定性；实体关系图约束(ERGC)在训练中保持语义和结构关系。

Result: 在跨模态检索、图像分类和图像检索任务上的实验表明，该方法在各种压缩比下均优于现有方法。在某些情况下，甚至超越了原始未压缩特征的性能。

Conclusion: ARFC框架提供了一种灵活高效的特征压缩解决方案，支持任意压缩比且性能优越，适用于资源受限的实际场景，验证了其有效性和多功能性。
...
TL;DR: ImagineAgent是一个结合认知推理与生成想象的智能体框架，通过构建认知地图、动态调用工具和复合奖励机制，解决开放词汇人-物交互中的跨模态幻觉和遮挡模糊问题，在少量训练数据下实现SOTA性能。
...
TL;DR: 本文提出了一种血管解剖感知的掩码图像建模框架VasoMIM，用于解决X射线血管造影图像分析中标注数据稀缺的问题，并构建了目前最大的X射线血管造影预训练数据集XA-170K。
...
Method: 提出VasoMIM框架，包含两个关键设计：1）解剖引导的掩码策略，策略性地掩码包含血管的补丁以学习稳健的血管语义；2）解剖一致性损失，保持原始图像和重建图像中血管结构的一致性，增强学习表征的可区分性。

Result: 在四个下游任务和六个数据集上验证，VasoMIM展示了优越的迁移能力，相比现有方法达到了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: PET成像存在辐射暴露问题，降低辐射剂量或扫描时间会降低图像质量。虽然使用MR图像辅助恢复标准剂量PET是可行方法，但面临多模态融合结构纹理不一致以及分布外数据不匹配的挑战

Method: 提出监督辅助多模态融合扩散模型MFdiff：1）设计多模态特征融合模块学习优化融合特征，充分利用MR图像但不引入额外细节；2）以融合特征为条件，基于扩散模型迭代生成高质量SPET图像；3）采用两阶段监督辅助学习策略，结合模拟分布内数据的广义先验和活体分布外数据的特定先验

Result: 实验表明MFdiff能有效从多模态输入恢复高质量SPET图像，在定性和定量评估上均优于现有最先进方法

Conclusion: MFdiff模型成功解决了多模态PET图像恢复中的融合不一致性和分布外数据问题，为低剂量PET成像提供了高质量恢复方案
...
TL;DR: 本文提出了一种基于生成压缩框架的感知去噪方法，通过熵编码的潜在表示重建图像，在保持低复杂度结构的同时，利用生成解码器恢复真实纹理，实现了失真-感知的平衡。


<details>
  <summary>Details</summary>
Motivation: 传统基于失真的去噪方法在强噪声和分布偏移下容易产生过度平滑的重建结果，无法保持结构细节和感知真实性。需要一种能够平衡失真和感知质量的方法。

Method: 提出生成压缩框架，通过熵编码的潜在表示强制低复杂度结构，使用生成解码器通过LPIPS损失和Wasserstein距离等感知度量恢复真实纹理。包含两种实现：基于条件Wasserstein GAN的压缩去噪器和基于条件扩散的重建策略。

Result: 在合成和真实噪声基准测试中，该方法在保持竞争性失真性能的同时，实现了持续的感知改进。同时为基于压缩的最大似然去噪器建立了非渐近保证，包括重建误差和解码错误概率的界限。

Conclusion: 提出的生成压缩框架有效解决了感知去噪问题，通过压缩潜在表示和生成解码器的结合，在失真和感知质量之间取得了良好平衡，为图像去噪提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在视觉质量上已有显著提升，但超高清视频生成仍然面临巨大挑战，包括运动建模、语义规划和细节合成的复合困难。
...
Conclusion: LUVE通过潜在级联架构和双频专家设计，有效解决了超高清视频生成的关键挑战，为高质量视频生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: V2X协同感知中快速域适应面临挑战，传统PEFT方法在多智能体设置中导致性能下降和训练不稳定，主要问题包括异构传感器流中的帧间冗余和PEFT适应下深层表示中细粒度语义的侵蚀

Method: 提出FlowAdapt框架：1）Wasserstein贪婪采样策略，通过有界覆盖半径选择性过滤冗余样本；2）渐进知识转移模块，通过可学习路径将压缩的早期表示逐步注入后期阶段，缓解语义退化

Result: 在三个基准测试上的广泛实验表明，FlowAdapt仅使用1%的可训练参数就实现了最先进的性能，有效弥合了领域差距，具有卓越的样本效率和泛化能力

Conclusion: FlowAdapt通过最优传输理论解决了多智能体域适应中的关键问题，为V2X协同感知提供了高效、稳定的参数高效微调解决方案
...
Method: 提出PLOT-CT框架：1）在预对数正弦图上应用Voronoi分解，将数据解耦为不同的基础成分；2）将这些成分嵌入独立的潜在空间；3）这种显式分解增强了模型学习判别特征的能力，直接通过减轻噪声和保留预对数域固有信息来提高重建精度。

Result: 在预对数域1e4入射光子水平下，PLOT-CT相比传统方法实现了2.36dB的PSNR提升，达到了最先进的性能。

Conclusion: PLOT-CT通过在预对数域应用Voronoi分解，有效解决了低剂量CT重建中的噪声放大问题，显著提升了重建精度，为低剂量CT成像提供了新的解决方案。
...
TL;DR: PLESS是一种通用的伪标签增强策略，通过层次化图像分区改进弱监督学习中的伪标签质量和空间一致性，在心脏MRI分割任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 基于涂鸦标注的弱监督学习虽然降低了标注成本，但存在噪声和不完整的监督问题。现有方法使用伪标签训练，但伪标签质量成为性能瓶颈，需要改进伪标签的可靠性和空间一致性。

Method: PLESS通过层次化图像分区将图像划分为空间一致的层次区域，在语义一致的区域内传播涂鸦信息来精炼伪标签。该框架是模型无关的，可轻松集成到现有的伪标签方法中。

Result: 在两个公开的心脏MRI数据集（ACDC和MSCMRseg）上，对四种涂鸦监督算法进行实验，均显示出分割准确性的持续改进。

Conclusion: PLESS是一种有效的伪标签增强策略，能够提高弱监督学习的分割性能，代码将在接受后开源。
...
TL;DR: 提出GR-Diffusion框架，将离散高斯表示（GR）的几何先验与扩散模型的生成能力结合，用于3D低剂量全身PET重建，显著提升图像质量。


<details>
  <summary>Details</summary>
Motivation: PET重建面临噪声放大、结构模糊和细节丢失等挑战，传统方法存在低通限制。离散高斯表示在计算机视觉中已显示潜力，但尚未与扩散模型结合用于PET重建。

Method: GR-Diffusion框架：1）使用GR从投影数据生成参考3D PET图像，提供物理基础和结构明确的基准；2）在扩散过程中采用分层引导机制：细粒度引导利用差异优化局部细节，粗粒度引导使用多尺度差异图校正偏差；3）扩散模型顺序整合GR的强几何先验并恢复亚体素信息。

Result: 在UDPET和临床数据集上，GR-Diffusion在不同剂量水平下均优于现有最先进方法，显著提升了3D全身PET图像质量并保留了生理细节。

Conclusion: GR-Diffusion成功将离散高斯表示的几何先验与扩散模型的生成能力相结合，为低剂量PET重建提供了有效解决方案，克服了传统方法的局限性。
...
TL;DR: SToRM提出首个面向多模态大语言模型的监督式令牌缩减框架，用于高效端到端自动驾驶，在保持全令牌性能的同时将计算成本降低高达30倍。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中基于多模态大语言模型的端到端系统需要大量计算资源，现有令牌缩减方法常导致任务性能下降，需要一种能保持性能的高效缩减方案。

Method: 提出SToRM框架：1) 轻量级重要性预测器估计令牌重要性分数；2) 监督训练方法利用全令牌LLM路径获取伪监督信号；3) 锚点-上下文合并模块将令牌分区并合并以减少冗余。

Result: 在LangAuto基准测试中，SToRM在相同缩减令牌预算下优于现有最佳端到端驾驶MLLMs，保持全令牌性能的同时计算成本降低高达30倍。

Conclusion: SToRM框架首次实现了多模态大语言模型的高效令牌缩减，在自动驾驶场景中平衡了计算效率与任务性能，为资源受限的车辆系统提供了可行方案。
...
Method: 采用分层情感表示和可学习原型，通过多原型引导、时间混合和注意力重加权等技术构建可控生成流程，无需显式情感标签。

Result: 在定性和定量评估中均优于现有方法，用户研究表明VR环境比桌面设置更能影响情感感知，支持情感图像扩展、风格化生成和VR全景生成等应用。

Conclusion: EmoSpace为沉浸式视觉内容生成提供了细粒度情感控制，支持治疗、教育、故事讲述、艺术创作和文化保护等多种应用，代码和模型将公开。
...
Method: 提出层次化实例树方法，利用噪声掩码作为信息线索，通过跨视图分组和条件替换抑制过分割和欠分割，生成视角一致的掩码和鲁棒的3D实例；使用开放词汇语义嵌入丰富实例信息；引入一致性感知更新机制处理多阶段任务中的场景变化

Result: 在合成和真实数据集上均优于现有方法，在最具挑战性的重度杂乱序列中AP@25达到61.66，比基线高2.2倍以上；仅用4个输入视图就超越了MaskClustering使用8个视图的性能2倍以上

Conclusion: Clutt3R-Seg为零样本3D实例分割提供了鲁棒解决方案，特别适用于杂乱场景中的语言引导机器人抓取任务，能够有效处理遮挡和噪声掩码问题
...
TL;DR: 本文提出了颈戴式视角的视线估计新任务，收集了首个数据集，评估了基于Transformer的GLC模型，并提出了视线出界分类和多视角协同学习两种扩展方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自我中心视线估计研究主要关注头戴式摄像头，而其他视角（如颈戴式）尚未得到充分探索。颈戴式摄像头具有更稳定的视角和更广的视野，但视线估计面临新的挑战。

Method: 1. 收集首个颈戴式视线估计数据集（8名参与者，约4小时日常活动视频）；2. 评估基于Transformer的GLC模型；3. 提出两种扩展：视线出界分类辅助任务和多视角协同学习方法（联合训练头戴和颈戴视角模型，使用几何感知辅助损失）。

Result: 视线出界分类任务能提升标准微调的性能，但多视角协同学习方法未能带来性能增益。实验结果表明颈戴式视线估计的可行性和挑战。

Conclusion: 颈戴式视线估计是一个有前景的新研究方向，视线出界分类能有效提升性能，但多视角协同学习需要进一步优化。研究为未来颈戴式可穿戴设备的视线估计应用奠定了基础。
...
TL;DR: 提出TD-FusionUNet模型，结合可训练的Hadamard变换和离散余弦变换层，用于基于多模态卫星数据的次日野火蔓延预测，在轻量级设置下实现精度与效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 开发轻量级且计算高效的次日野火蔓延预测工具，适用于资源受限环境下的实时应用，解决现有模型参数过多、计算成本高的问题。

Method: 提出TD-FusionUNet深度学习模型，包含可训练的Hadamard变换和离散余弦变换层，在正交化潜在空间中捕获关键频率成分；采用随机边缘裁剪和高斯混合模型等自定义预处理技术，增强稀疏预火掩码的表示和模型泛化能力。

Result: 在两个数据集（Google Research的Next-Day Wildfire Spread数据集和WildfireSpreadTS数据集）上评估，TD-FusionUNet以37万参数获得0.591的F1分数，优于使用ResNet18编码器的UNet基线，且参数数量显著减少。

Conclusion: 提出的潜在空间融合模型在轻量级设置下平衡了精度和效率，适合资源受限环境中的实时野火预测应用。
...
TL;DR: RI-Mamba：首个旋转不变的状态空间模型，用于点云数据，通过全局/局部参考系解耦姿态与几何，实现任意方向下的3D形状检索，在200+类别上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 随着VR和游戏发展，3D资产数量激增，文本到形状检索需求增加。现有方法需要规范姿态且支持类别有限，难以处理现实世界中多样类别和随机方向的物体。

Method: 1) 定义全局和局部参考系解耦姿态与几何；2) 使用希尔伯特排序构建具有几何结构的token序列；3) 计算方向嵌入并通过特征线性调制重新整合；4) 采用跨模态对比学习和自动三元组生成进行训练。

Result: 在OmniObject3D基准测试中，RI-Mamba在200多个物体类别上实现了最先进的性能，表现出卓越的表征能力和鲁棒性，特别是在任意方向下的检索效果显著。

Conclusion: RI-Mamba是首个旋转不变的状态空间模型，能够有效处理现实世界中多样类别和随机方向的3D形状检索问题，具有线性时间复杂度和良好的扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 数字减影血管造影（DSA）在脑血管疾病诊断和治疗中至关重要，但其侵入性和高成本限制了大规模数据收集和公共数据共享，因此需要开发合成DSA图像的方法

Method: 开发语义条件潜在扩散模型（LDM），使用包含99,349帧的单中心DSA数据集训练，通过文本嵌入编码解剖循环（前vs后）和标准C臂位置信息来控制图像生成

Result: 生成的合成DSA图像获得3.1-3.3的总体Likert评分，具有高评分者间可靠性（ICC(2,k)=0.80-0.87），Fréchet起始距离（FID）中位数为15.27，表明与真实DSA图像分布相似

Conclusion: 语义控制的潜在扩散模型能够生成适用于下游算法开发、研究和培训的现实合成DSA图像
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法虽然使用大型语言模型，但普遍缺乏领域特定推理能力、验证机制和模块化设计，导致控制性差、可扩展性不足。特别是在农业仿真领域，需要更精确、可验证的自动化生成方案。

Method: 开发了模块化多LLM管道，整合3D资产检索、领域知识注入和Unreal引擎API代码生成。采用混合策略结合few-shot提示、RAG、微调和验证等LLM优化技术，实现结构化数据处理、中间验证和灵活扩展。

Result: 系统通过结构化提示和语义准确性指标进行评估。用户研究表明生成场景在真实感和熟悉度方面与真实世界图像相当，专家对比显示相比手动场景设计显著节省时间。结果证实了多LLM管道在自动化领域特定3D场景生成中的有效性。

Conclusion: 多LLM管道方法能够可靠、精确地自动化生成领域特定的3D场景，解决了现有方法的局限性。未来工作将扩展资产层次结构、探索实时生成，并将管道适配到农业以外的其他仿真领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有SLAM方法存在两个问题：1）使用统一场景表示耦合跟踪与建图导致计算成本高；2）松散集成结构化跟踪框架引入冗余。需要一种既能实时运行又能保持高重建质量的方法。

Method: 1）在期望最大化框架下双向耦合视觉里程计与高斯泼溅，同时优化VO生成的半稠密深度估计和高斯场景表示；2）提出高斯泼溅初始化方法，利用VO提供的图像信息、关键帧位姿和像素关联生成接近最终场景的高斯近似，避免启发式方法。

Result: 实验验证表明：1）系统能够实时运行；2）在重建场景的几何/光度保真度和跟踪精度方面达到最先进水平。

Conclusion: GSO-SLAM通过双向耦合VO和GS的联合优化框架，在保持实时性的同时实现了高质量的场景重建和精确跟踪，为单目稠密SLAM提供了有效的解决方案。
...
TL;DR: 本文提出了一种新的视觉提示范式STVG-R1，将时空视频定位中的坐标预测转化为实例级识别问题，通过强化学习框架优化时空准确性和结构格式，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在密集预测任务如时空视频定位中存在文本描述与视觉坐标不对齐导致的幻觉问题。现有方法通常关注增强视觉-文本对齐或添加辅助解码器，但这会引入额外的可训练模块，导致显著的标注成本和计算开销。

Method: 提出新的视觉提示范式，将每帧坐标预测转化为紧凑的实例级识别问题，为每个对象分配唯一且时间一致的ID，并将这些ID作为视觉提示嵌入视频中。同时引入首个STVG强化学习框架STVG-R1，使用任务驱动的奖励联合优化时间准确性、空间一致性和结构格式正则化。

Result: 在六个基准测试上进行了广泛实验，STVG-R1在HCSTVG-v2基准上比基线Qwen2.5-VL-7B的m_IoU提升了20.9%，达到新的SOTA。令人惊讶的是，STVG-R1在多对象参考视频对象分割任务上也表现出强大的零样本泛化能力，在MeViS上达到47.3% J&F的SOTA性能。

Conclusion: 提出的视觉提示范式通过避免跨模态坐标对齐的困难问题，将坐标预测转化为实例识别，结合强化学习框架显著提升了时空视频定位的性能，并在多个任务上展示了优秀的泛化能力。
...
TL;DR: 该研究展示了如何通过针对性适应通用视觉语言模型来显著提升电商产品理解性能，同时保持广泛的跨模态能力，并提出了一个全面的评估套件。


<details>
  <summary>Details</summary>
Motivation: 电商产品理解需要强大的多模态理解能力（文本、图像、结构化属性），但现有通用视觉语言模型缺乏针对电商数据特点（属性中心、多图像、噪声）的适应策略，且担心适应后会牺牲通用性能。

Method: 通过大规模实验研究展示如何对通用视觉语言模型进行针对性适应，使其适应电商数据的特性，同时保持广泛的跨模态能力。

Result: 研究表明，针对性适应通用视觉语言模型可以显著提升电商性能，同时保持广泛的跨模态能力。

Conclusion: 提出了一个新颖的全面评估套件，涵盖深度产品理解、严格指令遵循和动态属性提取，为电商多模态理解提供了有效的适应方法和评估框架。
...
TL;DR: 该论文研究多模态大语言模型中的物体幻觉问题，通过构建物体对齐的辅助视图来改进视觉对比解码方法，减少幻觉现象


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题，即模型生成描述中包含了图像中不存在的物体。现有的视觉对比解码方法需要改进以更好地处理这一问题

Method: 利用自监督视觉Transformer中的物体中心注意力机制，移除最显著的视觉证据来构建辅助视图，破坏不受支持的标记并产生更强的对比信号。该方法与提示无关、与模型无关，可无缝集成到现有VCD流程中

Result: 在两个流行的物体幻觉基准测试和两个MLLM模型上，该方法都展示了一致的性能提升

Conclusion: 通过构建物体对齐的辅助视图改进视觉对比解码，能有效减少多模态大语言模型中的物体幻觉问题，且计算开销小，具有通用性
...
TL;DR: Code2Worlds：一个将4D场景生成转化为语言到模拟代码生成的框架，通过双流架构解耦对象生成与环境编排，并引入物理感知闭环机制确保动态保真度


<details>
  <summary>Details</summary>
Motivation: 实现空间智能需要超越视觉合理性，建立基于物理定律的世界模拟器。现有编码LLM在静态3D场景生成方面有进展，但扩展到4D动态场景面临两大挑战：多尺度上下文纠缠（整体生成难以平衡局部对象结构与全局环境布局）和语义-物理执行差距（开环代码生成导致缺乏动态保真度的物理幻觉）
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的图像和视频重光照技术已取得进展，但扩展到4D重光照仍面临挑战，主要原因是配对的4D重光照训练数据稀缺，以及在极端视角变化下难以保持时间一致性。

Method: 提出Light4D框架，包含两个核心技术：1) 解耦流引导：一种时间感知策略，将光照控制有效注入潜在空间同时保持几何完整性；2) 时间一致性注意力：在IC-Light架构中开发时间一致性注意力机制，并加入确定性正则化以消除外观闪烁。

Result: 大量实验表明，该方法在时间一致性和光照保真度方面达到竞争性性能，能够稳健处理从-90°到90°的相机旋转。

Conclusion: Light4D是一个无需训练的4D视频重光照框架，成功解决了在极端视角变化下保持时间一致性的挑战，为4D重光照提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型需要海量数据集（1100万图像）且仅依赖RGB输入，现有高效变体仍依赖大规模训练。希望利用深度信息作为几何先验来减少数据需求
...
TL;DR: 本文提出了一种名为Targeted Smart Filtering的新方法，用于快速生成多样化的3D分形数据，以预训练动作识别模型，相比传统方法采样速度提升约100倍，并在下游任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 合成数据集在深度学习领域被认为是真实标注数据的宝贵替代品。公式驱动监督学习（FDSL）方法可以通过分形或轮廓等公式驱动方式提供无限数量的完美标注数据，且没有人工劳动、隐私和伦理问题。然而，传统的3D分形生成方法速度慢且产生退化的分形，限制了其在动作识别预训练中的应用。

Method: 使用3D迭代函数系统（IFS）生成3D分形，并将其进行时间变换形成视频序列，作为动作识别模型的预训练数据集。针对传统方法速度慢和分形多样性不足的问题，提出了Targeted Smart Filtering方法，该方法通过智能过滤机制平衡生成速度和分形多样性，避免过度限制性方法对下游任务性能的负面影响。

Result: 提出的Targeted Smart Filtering方法实现了约100倍的采样速度提升，并且在动作识别的下游任务中，相比其他3D分形过滤方法取得了更优的性能表现。

Conclusion: 通过Targeted Smart Filtering方法，可以高效生成多样化的3D分形数据用于动作识别模型的预训练，解决了传统方法的速度和多样性问题，为基于合成数据的深度学习预训练提供了有效的解决方案。
...
Method: 通过深入分析发现，视频预训练的预测性嵌入（特别是V-JEPA 2）能够灵活丢弃不可预测的环境因素并编码任务相关的时间动态。基于此观察，作者提出JEPA-VLA方法，自适应地将预测性嵌入集成到现有VLA模型中。

Result: 实验表明，JEPA-VLA在多个基准测试（包括LIBERO、LIBERO-plus、RoboTwin2.0和真实机器人任务）中带来了显著的性能提升。

Conclusion: 预测性视频嵌入能够有效弥补现有VLA模型中视觉表示的关键缺陷，JEPA-VLA作为一种简单而有效的方法，通过集成预测性嵌入显著提升了VLA模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前单目动态重建方法缺乏统一的时空分解框架，要么进行整体时间优化，要么采用耦合的层次空间组合，限制了实际应用效果。
...
TL;DR: DiffPlace是一个新颖的生成框架，通过引入place-ID控制器实现地点可控的多视角图像生成，能够合成具有一致背景建筑的城市场景，同时灵活修改前景对象和天气条件，显著提升视觉地点识别的训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前的多视角扩散模型在生成城市街景时存在局限性：难以从文本、BEV地图和物体边界框生成地点感知和背景一致的城市场景，这限制了它们在视觉地点识别任务中生成真实样本的有效性。

Method: 提出DiffPlace框架，引入place-ID控制器，采用线性投影、感知器变换器和对比学习将place-ID嵌入映射到固定的CLIP空间，使模型能够合成背景建筑一致的图像，同时灵活修改前景对象和天气条件。

Result: 通过大量实验（包括定量比较和增强训练评估）表明，DiffPlace在生成质量和视觉地点识别训练支持方面均优于现有方法，展示了生成模型在场景级和地点感知合成方面的潜力。

Conclusion: DiffPlace为自动驾驶中的地点识别提供了一种有价值的改进方法，通过地点可控的多视角图像生成，能够增强场景级和地点感知的合成能力。
...
TL;DR: DynaHOI-Gym是一个用于动态手物交互评估的统一在线闭环平台，包含参数化运动生成器和基于rollout的评估指标，并发布了包含1000万帧数据的大规模基准DynaHOI-10M。
...
Method: 提出DynaHOI-Gym统一在线闭环平台，包含参数化运动生成器和基于rollout的评估指标；发布包含1000万帧、18万条手部捕捉轨迹的大规模基准DynaHOI-10M；提出简单的观察-行动基线方法ObAct，通过时空注意力整合短期观察与当前帧来预测动作。

Result: DynaHOI-10M基准包含8个主要类别和22个细粒度子类别；ObAct基线方法在位置成功率上实现了8.1%的提升。

Conclusion: 该研究填补了动态手物交互评估的空白，提供了统一的评估平台和大规模基准，为动态手部运动生成研究奠定了基础。
...
TL;DR: 提出基于隐式神经表示和去噪扩散模型的心肌瘢痕LGE图像合成框架，通过数据增强提升分割性能
...
Method: 结合隐式神经表示和去噪扩散模型：先训练INR捕获LGE数据和分割掩码的连续空间表示，压缩为潜在嵌入，然后用扩散模型在潜在空间生成新表示，解码为合成LGE图像和分割掩码
...
TL;DR: 该研究评估了视觉语言模型在法语PDF转Markdown任务上的表现，针对复杂文档布局提出了专门的评测基准和方法。


<details>
  <summary>Details</summary>
Motivation: 文档解析是检索增强生成(RAG)流程的关键步骤，但现有基准多关注英语或中文，且过度惩罚对下游任务影响不大的格式差异。法语文档特别是复杂布局的解析缺乏系统评估。

Method: 从60,000份文档中通过模型分歧采样选择困难页面构建法语基准，包含手写表单、复杂布局、密集表格和图形丰富页面。采用单元测试式检查，针对具体失败模式（文本存在性、阅读顺序、局部表格约束），并结合类别特定的归一化处理来忽略仅影响呈现的差异。

Result: 评估15个模型发现，最强专有模型在手写和表单处理上表现出显著更高的鲁棒性，而多个开源权重系统在标准印刷布局上仍具有竞争力。

Conclusion: 该研究为法语文档解析提供了更精细的评估框架，揭示了不同模型在处理不同类型复杂文档时的性能差异，为RAG管道的文档解析环节提供了重要参考。
...
TL;DR: 提出基于贝叶斯深度学习的概率优化框架，通过CUB-Loss和DTS策略改善医学影像AI模型的不确定性校准，使置信度与预测正确性对齐


<details>
  <summary>Details</summary>
Motivation: 医学影像AI决策系统需要可靠的置信度评估，但现有深度学习模型存在校准不足问题，表现为对错误预测过度自信，这阻碍了临床接受度。需要让模型的不确定性估计与预测正确性相关，以便临床医生识别不可靠输出进行复核。

Method: 提出基于贝叶斯深度学习的通用概率优化框架：1）引入置信度-不确定性边界损失（CUB-Loss），惩罚高置信度错误和低置信度正确预测；2）设计双温度缩放（DTS）策略进行后处理校准，优化后验分布以提高可解释性。

Result: 在肺炎自动筛查、糖尿病视网膜病变检测和皮肤病变识别三个医学影像任务上验证，结果显示：1）在不同模态上实现一致的校准改进；2）在数据稀缺场景下保持鲁棒性能；3）在严重不平衡数据集上保持有效。

Conclusion: 提出的框架通过训练时优化和后处理校准，显著改善了医学影像AI模型的不确定性校准，使置信度与预测正确性对齐，增强了模型可靠性和临床可接受性，具有实际临床部署潜力。
...
Method: 提出空间思维链(SCoT)框架：1) 通过文本-坐标指令格式训练增强扩散模型的布局感知能力；2) 利用先进MLLMs作为规划器生成全面布局计划，将其空间规划能力直接转移到生成过程

Result: 在图像生成基准测试中达到最先进性能，在复杂推理任务上显著优于基线方法，在图像编辑场景中也表现出强大效果

Conclusion: SCoT框架有效桥接了MLLMs的推理能力与扩散模型的生成能力，解决了现有方法的局限性，为空间感知图像生成提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 新生儿复苏的准确记录对质量改进和临床指南遵循至关重要，但实践中仍未被充分利用。虽然先前使用3D-CNN和Vision Transformers的研究在检测关键活动方面取得了一定成果，但在识别这种细粒度活动方面仍面临挑战。

Method: 本研究探索生成式AI方法改进活动识别，具体包括：使用本地视觉语言模型结合大型语言模型，与监督式TimeSFormer基线进行比较；在包含13.26小时新生儿复苏视频的模拟数据集上，评估多种零样本VLM策略和带有分类头的微调VLM，包括使用低秩适应技术。

Result: 研究结果显示：小型本地VLM存在幻觉问题，但经过LoRA微调后，F1分数达到0.91，显著超越了TimeSFormer的0.70结果。

Conclusion: 生成式AI方法特别是经过LoRA微调的视觉语言模型，在新生儿复苏视频活动识别任务中表现出色，能够有效提升识别精度，为临床实践中的质量改进提供了有前景的技术方案。
...
TL;DR: ReNoV：一种利用外部表征引导扩散模型进行新视角合成的框架，通过表征投影模块提升几何一致性
...
Result: 在标准基准测试中优于现有扩散基新视角合成方法，在重建保真度和修复质量上均有显著提升，并能从稀疏无位姿图像集合中实现鲁棒合成
...
<details>
  <summary>Details</summary>
Motivation: 传统CCD/CMOS传感器动态范围低于70dB，在焊接电弧监测和抛光金属表面分析等强光环境下容易饱和，导致数字图像相关(DIC)测量出现不可逆的细节损失和显著误差
...
Conclusion: 基于DMD的系统提供高保真自适应HDR成像，克服传统传感器关键限制，在传统方法不足的高光环境下具有强大的光学计量和应力分析潜力
...
TL;DR: AssetFormer是基于自回归Transformer的模型，用于从文本描述生成模块化3D资产，特别针对用户生成内容需求，通过创新的模块序列化和解码技术提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 数字产业特别是用户生成内容领域对高质量、多样化的模块化3D资产有强烈需求，需要能够从文本描述自动生成符合设计约束的资产。
...
Conclusion: 该工作提出了一个可扩展到各种类型模块化3D资产的灵活框架，为3D内容生成领域做出了贡献。
...
TL;DR: PosterOmni是一个统一的图像到海报生成框架，通过数据蒸馏奖励管道整合局部编辑和全局创作，在多个任务类型上显著提升参考一致性、构图质量和美学和谐度。


<details>
  <summary>Details</summary>
Motivation: 图像到海报生成需要同时处理局部调整和高级设计理解，既要保持具体视觉实体，又要理解抽象设计概念。现有方法难以同时满足这些交织的需求，需要统一的框架来处理这种多维度过程。

Method: PosterOmni通过高效的数据蒸馏奖励管道整合局部编辑和全局创作：(1)构建覆盖六种任务类型的多场景数据集；(2)在局部和全局专家之间进行知识蒸馏用于监督微调；(3)应用统一的PosterOmni奖励反馈来联合对齐视觉实体保持和美学偏好。

Result: 实验表明PosterOmni显著增强了参考一致性、全局构图质量和美学和谐度，超越了所有开源基线，甚至超过了一些专有系统。同时建立了PosterOmni-Bench统一评估基准。

Conclusion: PosterOmni成功解锁了基础编辑模型在多任务图像到海报生成中的潜力，通过统一的框架有效整合了局部编辑和全局创作两个机制，为艺术海报创作提供了全面的解决方案。
...
Result: 仅使用13,000个Nano Banana pro演示微调FLUX模型，在提示跟随和美学基准上取得竞争性表现。框架能有效泛化到离散图像和视频生成，并作为鲁棒正则化器缓解基于奖励优化的奖励黑客问题。

Conclusion: FAIL提供了一种无需显式奖励或成对比较的流匹配模型后训练框架，在多种生成任务中表现优异，并能有效缓解奖励黑客问题。
...
TL;DR: TexSpot是一个基于扩散的纹理增强框架，通过新颖的Texlet 3D纹理表示解决现有多视图扩散管道中的视图不一致问题，显著提升纹理质量和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的多视图扩散管道存在视图不一致问题，现有纹理表示方法（如UV贴图和基于点的方法）各有缺陷：UV贴图在展开时会产生失真，而基于点的方法将纹理保真度与几何密度紧密耦合，限制了高分辨率纹理生成。

Method: 提出TexSpot框架，核心是Texlet表示法，将基于点的3D纹理的几何表达能力与UV表示的紧凑性相结合。每个Texlet潜在向量通过2D编码器编码局部纹理块，并使用3D编码器聚合全局形状上下文。采用级联的3D到2D解码器重建高质量纹理块，学习Texlet空间。在此基础上训练基于Texlet条件化的扩散变换器，用于细化和增强多视图扩散方法生成的纹理。
...
TL;DR: DreamID-Omni是一个统一的可控人中心音频-视频生成框架，通过对称条件扩散Transformer和双级解耦策略解决多人物身份-音色绑定问题，在多种任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参考音频视频生成、视频编辑和音频驱动视频动画等任务视为孤立目标，且在多人物场景中难以实现精确的身份-音色解耦控制。需要统一框架来解决这些挑战。

Method: 提出对称条件扩散Transformer集成异质条件信号；采用双级解耦策略（信号级的同步RoPE和语义级的结构化描述）；设计多任务渐进训练方案，利用弱约束生成先验正则化强约束任务。

Result: 实验表明DreamID-Omni在视频、音频和音视频一致性方面均达到全面SOTA性能，甚至超越领先的专有商业模型。

Conclusion: DreamID-Omni提供了一个统一的、可控的人中心音频-视频生成框架，成功解决了多人物身份-音色绑定问题，并在多个任务上表现出色，将开源代码以弥合学术研究与商业应用之间的差距。
...
TL;DR: DeepGen 1.0是一个轻量级的5B参数统一多模态模型，在图像生成和编辑任务上超越了更大规模的模型，通过创新的堆叠通道桥接技术和三阶段训练策略实现高效性能。
...
Method: 1. 提出堆叠通道桥接（SCB）框架，从VLM多层提取层次特征并与可学习的"思考令牌"融合，为生成主干提供结构化、富含推理的指导。2. 设计三阶段数据中心训练策略：对齐预训练、联合监督微调、以及使用MR-GRPO的强化学习。

Result: 仅使用约5000万样本训练，DeepGen 1.0在多个基准测试中取得领先性能：在WISE上超越80B HunyuanImage 28%，在UniREditBench上超越27B Qwen-Image-Edit 37%。

Conclusion: DeepGen 1.0提供了一个高效、高性能的轻量级统一多模态模型替代方案，通过开源训练代码、权重和数据集，有助于民主化多模态研究。
...
TL;DR: UniDFlow是一个统一的多模态离散流匹配框架，通过任务特定低秩适配器解耦理解和生成，避免目标干扰和表示纠缠，并通过基于参考的多模态偏好对齐优化相对结果，在保持忠实度和可控性的同时无需大规模重新训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多模态理解和生成任务时存在目标干扰和表示纠缠问题，同时缺乏有效的可控性和忠实度优化机制，需要统一的框架来解决这些挑战。

Method: 提出统一离散流匹配框架，使用任务特定低秩适配器解耦理解和生成，引入基于参考的多模态偏好对齐技术，在相同条件下优化相对结果，提高忠实度和可控性。

Result: 在八个基准测试中达到最先进性能，展现出强大的零样本泛化能力，包括修复、上下文图像生成、基于参考的编辑和组合生成等任务，尽管没有进行明确的任务特定训练。

Conclusion: UniDFlow通过统一的离散流匹配框架成功解决了多模态理解和生成中的目标干扰和表示纠缠问题，实现了高性能和强大的零样本泛化能力，为多模态任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 实时视频生成中的扩散变换器受限于3D自注意力的二次计算成本，特别是在少步和自回归的实时场景中。现有稀疏注意力方法在这种设置下失效，因为视频注意力结合了周期性结构、稀疏语义对应和密集混合，超出了传统稀疏方法的表达能力。

Method: 提出Monarch-RT结构化注意力参数化方法：1) 使用Monarch矩阵分解注意力；2) 通过适当对齐的块结构和扩展的平铺Monarch参数化实现高表达能力；3) 通过微调和自定义Triton内核克服参数化开销。

Result: 1) Monarch-RT在现有双向模型稀疏基线上表现出高效性；2) 在SOTA模型Self-Forcing上实现高达95%的注意力稀疏度且无质量损失；3) 优化实现超越FlashAttention-2/3/4内核，在RTX 5090、H100、B200 GPU上获得1.4-11.8倍加速；4) 首次在单张RTX 5090上实现16 FPS的实时视频生成。

Conclusion: Monarch-RT是一种创新的稀疏注意力参数化方法，成功解决了实时视频生成中的计算瓶颈，实现了高质量、高效率的实时视频生成，为实时视频生成领域提供了重要技术突破。
...
TL;DR: UniT框架通过多模态思维链测试时扩展，使统一模型能够跨多轮进行推理、验证和精炼，提升复杂空间组合任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型虽然能处理多模态理解和生成，但通常单次运行且不迭代精炼输出。许多多模态任务（尤其是涉及复杂空间组合、多对象交互或演化指令的任务）需要分解指令、验证中间结果并进行迭代修正。测试时扩展在语言模型中已证明有效，但扩展到统一多模态模型仍具挑战。

Method: UniT框架结合智能数据合成、统一模型训练和灵活的测试时推理，激发包括验证、子目标分解和内容记忆在内的认知行为。通过多模态思维链测试时扩展，使单个统一模型能够跨多轮进行推理、验证和精炼。

Result: 1) 在短推理轨迹上训练的统一模型能泛化到测试时的长推理链；2) 顺序思维链推理比并行采样提供更可扩展和计算高效的测试时扩展策略；3) 在生成和编辑轨迹上训练能改善分布外视觉推理。

Conclusion: 多模态测试时扩展是推进统一模型中生成和理解任务的有效范式，UniT框架通过迭代推理和精炼机制显著提升了模型处理复杂多模态任务的能力。
...
TL;DR: 提出了一种新颖的矢量草图渐进语义幻觉任务，通过顺序添加笔画实现戏剧性的语义转换，并开发了Stroke of Surprise生成框架来解决双重约束优化问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉幻觉主要依赖空间操作（如多视图一致性），本文旨在将视觉字谜从空间维度扩展到时间维度，探索通过时间序列笔画添加实现语义转换的可能性。

Method: 提出了Stroke of Surprise生成框架，采用序列感知联合优化方法，核心是双分支分数蒸馏采样机制，动态调整前缀笔画以发现两个目标的"共同结构子空间"，并引入覆盖损失确保空间互补性而非遮挡。

Result: 大量实验表明，该方法在可识别性和幻觉强度方面显著优于现有基线方法，成功地将视觉字谜从空间维度扩展到时间维度。

Conclusion: 提出的渐进语义幻觉任务和Stroke of Surprise框架成功实现了矢量草图的时序语义转换，为视觉幻觉研究开辟了新的方向，展示了时间维度上的创造性可能性。
...
TL;DR: HybridRAG是一个新颖的RAG框架，通过预生成QA知识库和分层文本块处理，在保证准确性的同时降低响应延迟，特别适用于处理非结构化PDF文档的聊天机器人场景。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常假设结构良好的文本源，并在查询时进行检索和生成，这在处理现实世界聊天机器人场景中的非结构化文档时存在局限性，特别是在处理复杂布局的PDF文档和大量用户请求时。

Method: 1. 通过OCR和布局分析处理包含复杂布局（文本、表格、图形）的非结构化PDF文档，转换为分层文本块；2. 使用LLM从组织好的文本块预生成可信的QA知识库；3. 查询时优先匹配QA知识库获取即时答案，仅在没有合适匹配时才进行实时响应生成。

Result: 在OHRBench上的实验表明，HybridRAG相比标准RAG基线提供了更高的答案质量和更低的延迟。

Conclusion: HybridRAG是处理大量非结构化文档和大量用户请求的现实世界聊天机器人应用的实用解决方案，特别是在计算资源有限的情况下。
...
TL;DR: 该论文研究了大型语言模型中检索头的动态特性，发现它们在自回归生成过程中会随时间动态变化，具有不可替代性，并且模型隐藏状态包含预测未来检索头模式的信号。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要基于静态统计数据识别LLMs中的检索头，忽略了自回归生成的细粒度时间动态性。本文旨在从动态视角研究检索头的工作机制。

Method: 通过广泛分析，建立三个核心主张：动态性、不可替代性和相关性。在Needle-in-a-Haystack任务和多跳QA任务上验证发现，并在动态检索增强生成框架中量化动态与静态检索头的效用差异。

Result: 研究发现：1) 检索头在时间步上动态变化；2) 动态检索头在每个时间步具有特异性，无法被静态检索头有效替代；3) 模型隐藏状态编码了预测未来检索头模式的信号，表明存在内部规划机制。

Conclusion: 该研究为LLMs的内部机制提供了新见解，强调从动态视角理解检索头的重要性，揭示了模型内部可能存在规划机制。
...
TL;DR: 该论文提出了一种基于编码器-转换器和条件随机场的轻量级方法，用于从等离子体物理研究文章中提取嵌套命名实体，通过实体特定模型专业化和超参数优化来提升性能。
...
Conclusion: 该工作推动了等离子体物理领域的实体识别技术发展，为研究人员提供了有效的工具来探索和分析科学文献，并建立了该领域NER研究的基础。
...
TL;DR: RECOM基准测试显示LLMs在回答近期问题时存在语义-词汇悖论：模型通过大量改写而非词汇复制来保持语义一致性，导致余弦相似度超过99%但BLEU-1重叠率低于8%。模型规模与性能不相关，Mistral-7B优于GPT-OSS-20B。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在处理近期信息时与人类视角的对齐程度，现有研究对此关注不足。需要评估模型在回答基于社区的真实近期问题时，其回答与人类共识的对应关系。
...
Conclusion: 词汇指标在评估抽象生成任务时不可靠，需要多维评估框架来捕捉超越表层文本匹配的语义保真度。模型通过大量改写而非词汇复制来保持语义一致性，这对LLM评估方法提出了重要挑战。
...
<details>
  <summary>Details</summary>
Motivation: 虽然参数高效微调方法被广泛用于适配大语言模型并假设能提高事实正确性，但其如何影响幻觉行为，特别是在QA数据集上的影响，尚未得到充分理解

Method: 通过三个开源LLM骨干和三个事实寻求QA基准进行系统性实证研究，使用七种无监督幻觉检测方法（语义一致性、置信度、熵三种互补方法）评估性能

Result: 实验结果显示PEFT能一致地增强幻觉检测能力，在各种幻觉检测器上显著提高AUROC；进一步分析表明PEFT主要重塑不确定性的编码和表现方式，而非向模型注入新的事实知识

Conclusion: PEFT通过改变模型内部不确定性表示来改善幻觉检测，这为理解参数高效微调如何影响大语言模型的可靠性提供了重要见解
...
TL;DR: FalseCite是一个专门用于捕捉和评估LLM幻觉的数据集，通过误导性引用诱导幻觉，测试发现GPT-4o-mini等模型在虚假引用下幻觉显著增加，隐藏状态向量呈现独特的角状结构。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生幻觉，生成无意义或虚假信息，这在医学和法律等敏感领域尤其有害。为了系统研究这一现象，需要专门的工具来捕捉和评估幻觉。

Method: 引入FalseCite数据集，通过误导性或捏造的引用来诱导幻觉响应。使用GPT-4o-mini、Falcon-7B和Mistral 7-B等模型在FalseCite上进行测试，并分析幻觉模型的内部状态，可视化并聚类隐藏状态向量。

Result: 在FalseCite上测试发现，带有欺骗性引用的虚假声明会显著增加幻觉活动，特别是GPT-4o-mini。分析隐藏状态向量发现，无论是否发生幻觉，这些向量都倾向于形成独特的角状形状。

Conclusion: FalseCite作为评估和缓解LLM幻觉的基础工具具有重要潜力，为未来LLM研究提供了系统评估幻觉的方法框架。
...
TL;DR: 本文提出使用组合融合分析(CFA)结合多个模型来增强联合国可持续发展目标(SDG)文本分类，通过生成式AI合成训练数据，CFA方法达到96.73%准确率，优于单个最佳模型，并与人类专家结果对比验证了人机协同的互补增强效果。


<details>
  <summary>Details</summary>
Motivation: 在文本分类中，当类别不可用、难以区分或相互关联时，传统NLP技术面临挑战。联合国可持续发展目标(SDG)的社会分析需要准确的文本分类，但现有方法存在局限性。本文旨在通过结合多个模型的智能来增强SDG文本分类性能。

Method: 采用组合融合分析(CFA)系统融合范式，使用秩-分特征(RSC)函数和认知多样性(CD)来结合一组相对较好且相互多样的分类模型。首先使用生成式AI模型生成合成数据用于模型训练，然后将CFA应用于SDG文本分类任务。

Result: CFA技术实现了96.73%的性能表现，优于最佳单个模型。通过与人类领域专家的结果进行比较，证明了结合多个ML/AI模型的智能与人类专家输入不仅能够互补，还能相互增强。

Conclusion: 研究表明，使用CFA结合多个模型的智能，并获取人类专家的输入，可以显著提升SDG文本分类的性能。这种方法不仅实现了模型间的互补，还展示了人机协同在复杂分类任务中的增强效果。
...
TL;DR: Transformer隐藏状态中向量的方向（角度）和大小（范数）具有不同的功能作用：角度扰动对语言建模损失影响更大，而大小扰动对句法处理影响更大


<details>
  <summary>Details</summary>
Motivation: 研究Transformer隐藏状态中向量方向和大小是否具有不同的计算功能角色，理解高维表示空间中方向与大小的功能分离

Method: 使用Pythia系列模型，采用L2匹配的扰动分析方法，确保角度和大小扰动具有相同的欧几里得位移，通过因果干预分析不同扰动对注意力和LayerNorm路径的影响

Result: 角度扰动对语言建模损失造成更大破坏（最多42.9倍），而大小扰动对句法处理影响更大（主谓一致任务准确率下降20.4% vs 1.6%）；角度损伤主要通过注意力路径传播，大小损伤部分通过LayerNorm路径传播

Conclusion: 在LayerNorm架构中，方向主要影响注意力路由，而大小调节细粒度句法判断的处理强度；RMSNorm架构中模式不同，表明这种分离依赖于架构选择，结果对模型编辑和可解释性研究有重要意义
...
TL;DR: 本研究评估了Gemini 2.5 Flash和Pro TTS模型在五种印度语言中生成合成法庭演讲的表现，发现模型在程序性信息传递方面表现良好，但在情感表达和说服力方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 法律辩护需要权威语气、节奏停顿和情感智能的独特结合。随着LLM的发展，TTS技术从基本可懂度转向情境感知的表达性合成。在印度多元语言环境中，合成语音需要传达权威和专业形象，这一任务变得尤为复杂。

Method: 提出一个提示框架，利用Gemini 2.5对五种语言的原生支持和情境感知节奏控制，生成不同的律师角色。在泰米尔语、泰卢固语、孟加拉语、印地语和古吉拉特语中评估模型表现。

Result: 模型表现出"单调权威"特征，在程序性信息传递方面表现出色，但在动态声音调节和情感深度方面存在困难。孟加拉语和古吉拉特语的性能下降进一步凸显了语音学方面的挑战。
...
TL;DR: 作者回应写作是科学同行评审的关键环节，本文提出REspGen框架，将作者回应生成重新定义为作者参与循环的任务，整合作者输入、多属性控制和评估引导的优化


<details>
  <summary>Details</summary>
Motivation: 当前自动生成作者回应的研究未能充分利用作者的专业知识和意图，而实践中作者拥有领域专业知识、作者特有信息、修订和回应策略等关键信号，需要能整合这些信号的NLP辅助工具来支持有效的同行评审回应写作

Method: 提出REspGen生成框架，整合显式作者输入、多属性控制和评估引导的优化；构建REspEval评估套件，包含20多个指标；创建Re$^3$Align数据集，这是首个大规模对齐的评审-回应-修订三元组数据集

Result: 实验表明作者输入和评估引导的优化有益处，输入设计对回应质量有影响，控制性和质量之间存在权衡；使用最先进的LLM验证了方法的有效性

Conclusion: 将作者回应生成重新定义为作者参与循环的任务，通过整合作者专业知识、意图和评估引导的优化，能有效支持同行评审中的回应写作；提供了公开的数据集、生成和评估工具
...
TL;DR: 多语言预训练模型的tokenization对某些书写系统存在系统性成本，不同正字法变体在分词效率、推理速度和信息成本方面存在显著差异


<details>
  <summary>Details</summary>
Motivation: 虽然多语言预训练模型通常被认为是与书写系统无关的，但它们的tokenizer可能对某些书写系统施加系统性成本。本研究旨在量化这种"脚本税"，揭示tokenization在多语言NLP中的不平等问题

Method: 通过比较具有相同语言内容但不同正字法变体的文本，使用bits per character（BPC）来避免子词分词的"NLL悖论"，并进行往返转换检查来区分处理差异与映射噪声

Result: 高碎片化正字法变体在分词效率（6.73-6.85 vs 2.10-2.35 tokens/word）、推理速度（0.23 vs 3.8 sentences/second）和信息成本（mBERT +19.7%，XLM-R +47.1%）方面都显著劣于低碎片化变体

Conclusion: tokenization是多语言NLP中不平等的重要来源，研究结果支持开发脚本感知的tokenization和预训练方法
...
TL;DR: 本文综述了Transformer在离散推理任务中的理论局限性，从电路复杂性、近似理论和通信复杂性三个角度分析了其结构性和计算性障碍。


<details>
  <summary>Details</summary>
Motivation: Transformer已成为序列建模的基础架构，在自然语言处理、视觉等领域取得了最先进成果，但其在算术、逻辑推理和算法组合等离散推理任务中的理论局限性仍是一个关键开放问题。

Method: 从三个理论视角综合近期研究：电路复杂性、近似理论和通信复杂性，通过连接这些理论框架，分析Transformer在执行符号计算时面临的结构和计算障碍。

Result: 阐明了当前Transformer架构难以实现精确离散算法的原因，尽管它们在模式匹配和插值方面表现出色。识别了深度约束、难以逼近不连续性、token间通信瓶颈等关键挑战。

Conclusion: 讨论了模型设计的影响，并提出了克服这些基础限制的有前景方向，为理解Transformer的理论局限性提供了统一框架。
...
TL;DR: 大型语言模型在低数据环境下通过检索增强提示策略，能够有效预测人类日常活动和持续时间，展现出强大的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的智能体模型在低数据环境中表现不佳，限制了其在智能家居、人机协作等应用中的实用性。本研究探索预训练的大型语言模型是否能够利用其广泛的人类知识来填补这一空白。

Method: 采用检索增强提示策略，整合时间、空间、行为历史和人物角色四种上下文信息，在CASAS Aruba智能家居数据集上进行评估。评估包括两个任务：带持续时间估计的下一个活动预测，以及多步日常序列生成，每个任务都使用不同数量的少样本示例。

Result: 大型语言模型展现出对人类行为的强大内在时间理解能力：即使在零样本设置下也能产生连贯的日常活动预测，添加一两个示例可进一步优化持续时间校准和分类准确性。超过几个示例后性能趋于饱和，表明收益递减。序列级评估确认了跨少样本条件的一致时间对齐。

Conclusion: 预训练语言模型可以作为有前景的时间推理器，捕捉重复性日常活动和上下文依赖的行为变化，从而增强智能体模型的行为模块。
...
TL;DR: 该研究通过微调大语言模型进行阿尔茨海默病检测，分析模型内部表示，并基于分析结果设计任务感知标记来生成合成数据以解决标注数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期可靠检测面临挑战，主要原因是标注数据有限。虽然大语言模型在跨领域迁移方面表现出色，但通过监督微调将其应用于AD领域的研究仍相对缺乏。

Method: 1) 微调LLM用于AD检测；2) 使用探测技术分析transformer层的中间激活；3) 基于分析结果设计任务感知的特殊标记；4) 训练序列到序列模型作为数据合成工具，利用这些标记生成结构一致且具有诊断信息的合成样本。

Result: 研究发现微调后，特定词语和特殊标记的探测值发生显著变化，表明这些元素在模型改进的检测性能中起关键作用。基于此设计的数据合成工具能够生成结构一致的合成样本。

Conclusion: 通过分析LLM内部表示来理解其在AD检测任务中的信息编码机制，并利用这些洞察设计有效的数据合成方法，为解决AD检测中标注数据不足的问题提供了新思路。
...
TL;DR: 本文综述了提示工程在自然语言生成任务中的最新发展，提出了提示分类法、决策框架和设计-优化-评估框架，旨在为从业者提供系统化的指导。


<details>
  <summary>Details</summary>
Motivation: 尽管提示工程已成为提升大语言模型性能的重要技术，但在自然语言生成领域仍缺乏系统化的框架和统一理解。本文旨在填补这一空白，为从业者提供结构化的指导。

Method: 通过文献综述方法，分析近期提示工程的发展，提出提示范式的分类法、基于多种因素的提示选择决策框架，以及连接设计、优化和评估的系统框架。

Result: 建立了提示工程的系统化分类体系，为不同NLG任务提供了提示选择决策框架，提出了支持更可控和可泛化NLG的设计-优化-评估框架。

Conclusion: 提示工程作为输入级控制机制，与微调和解码方法形成互补，对提升NLG任务性能具有重要意义。系统化的框架有助于推动更可控、可泛化的自然语言生成发展。
...
TL;DR: 本文综述了大型语言模型机制可解释性在模型对齐中的应用进展，分析了从电路发现到因果干预等多种技术，探讨了可解释性如何指导RLHF、宪法AI等对齐策略，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在各种任务上表现出色，但其内部决策过程仍然不透明。机制可解释性研究对于理解和对齐这些模型至关重要，特别是随着模型规模的扩大，理解其内部工作机制变得日益重要。

Method: 本文采用文献综述方法，系统梳理了机制可解释性技术在LLM对齐中的应用，包括电路发现、特征可视化、激活引导和因果干预等多种技术。同时分析了这些技术如何为RLHF、宪法AI和可扩展监督等对齐策略提供见解。

Result: 综述发现机制可解释性技术能够揭示LLM的内部工作机制，为对齐策略提供重要见解。然而仍面临神经元多义性、叠加假说和大规模模型涌现行为解释困难等关键挑战。

Conclusion: 机制可解释性对于LLM对齐至关重要，未来需要发展自动化可解释性方法、跨模型电路泛化以及可扩展至前沿模型的可解释性驱动的对齐技术。
...
TL;DR: 本文全面综述了大语言模型中的代码混合与代码转换研究，提出了统一分类法，总结了实用建议，分析了现有评估方法的局限性，并讨论了安全问题和研究挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言建模取得进展，但大语言模型在混合语言环境中仍面临挑战，表现为语法性、事实性和安全性行为的系统性退化。需要系统梳理代码混合研究现状，为构建、适应和评估具备代码混合能力的LLMs提供指导。

Method: 1. 提出统一分类法，从数据、建模和评估三个维度组织现有研究；2. 总结实用建议手册；3. 综述建模方法（CSW定制预训练、任务特定后训练、提示策略、上下文学习）；4. 分析评估实践，识别不稳定性和可复现性问题；5. 编目现有基准，批判性考察其语言覆盖和英语中心偏见。

Result: 1. 建立了代码混合研究的系统分类框架；2. 提供了构建CSW能力LLMs的实用建议；3. 识别了当前评估方法的不稳定性和有限可复现性；4. 揭示了现有基准的英语中心偏见和语言覆盖不足；5. 指出了代码混合可能被用作绕过模型安全机制的途径。

Conclusion: 本文为代码混合研究提供了全面综述和系统框架，强调了当前LLMs在混合语言环境中的局限性，提出了改进建模和评估的具体方向，并警示了代码混合带来的新兴安全风险，为未来研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有记忆系统虽然能支持长程人机交互，但破坏了交互会话的内在逻辑和时间关系，导致记忆碎片化和推理性能下降

Method: 提出MetaMem框架，通过自演化的元记忆迭代提炼可迁移的知识利用经验，通过自我反思推理过程并更新元记忆状态，指导LLM系统识别和整合分散的记忆片段
...
TL;DR: 论文提出AskBench基准测试，用于评估和改进大语言模型在需要澄清时的提问能力，包含AskMind和AskOverconfidence两种设置，并提出RLVR训练方法提升模型表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常在提示缺少关键细节或包含误导信息时仍然回应，导致幻觉或强化错误观念。需要评估和改进LLMs决定何时以及如何请求澄清的能力，同时不牺牲任务性能。

Method: 1. 提出AskBench交互式基准测试，将标准QA对转换为具有明确检查点的多轮交互；2. 统一评估循环评估最终答案并模拟用户响应；3. 包含两种设置：AskMind（意图不足查询）和AskOverconfidence（包含错误前提的查询）；4. 提出基于验证器的奖励强化学习（RLVR），使用结构化评分标准鼓励有针对性的澄清。

Result: 实验显示在准确性、评分标准遵循度和交互效率方面有持续改进，对未见领域有很强的泛化能力。
...
TL;DR: 提出NLDD指标评估CoT解释的忠实性，发现模型存在70-85%的推理范围限制，且准确率不能反映真实推理过程


<details>
  <summary>Details</summary>
Motivation: 当前CoT解释被广泛用于解释语言模型解决复杂问题的过程，但不清楚这些逐步解释是真实反映模型的决策过程，还是仅仅是事后合理化。需要一种方法来衡量CoT解释是否忠实于模型的实际推理过程。

Method: 提出归一化对数差衰减（NLDD）指标，通过破坏解释中的单个推理步骤并测量模型对其答案置信度的下降程度，来判断该步骤是否真正重要。通过标准化这些测量，NLDD能够在不同架构的模型之间进行严格的跨模型比较。

Result: 在语法、逻辑和算术任务上测试三个模型系列，发现一致的推理范围（k*）在链长的70-85%之间，超过这个范围后推理标记对最终答案几乎没有或产生负面影响。还发现模型可以编码正确的内部表示但完全无法完成任务。

Conclusion: 准确率本身不能揭示模型是否真正通过其推理链进行推理。NLDD提供了一种衡量CoT何时重要的方法，揭示了模型推理过程的局限性。
...
TL;DR: 论文提出了Mis-Align Bench基准测试，用于同时评估大语言模型在安全、价值和文化三个维度的错位问题，构建了包含382,424个样本的SAVACU数据集，并发现单维度模型在联合条件下存在高假失败率和低对齐分数。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型错位评估基准（如INSECURE CODE、VALUEACTIONLENS、CULTURALHERITAGE）只能单独评估安全、价值或文化维度，无法同时评估这三个在实际场景中必须共同满足的维度，这限制了模型在真实世界查询中的全面评估。

Method: 1. 构建SAVACU数据集：重新分类LLM-PROMPT-DATASET中的提示，通过分类法将其分为14个安全领域、56个价值领域和42个文化领域，使用Mistral-7B-Instruct-v0.3进行分类，并通过Llama-3.1-8B-Instruct扩展低资源领域，使用SimHash指纹避免重复。2. 通过两阶段拒绝采样将提示与错位和对齐的响应配对以确保质量。3. 在通用、微调和开源权重的大语言模型上进行基准测试，系统评估三个维度下的错位情况。

Result: 单维度模型在覆盖率上表现良好（最高97.6%），但在联合条件下假失败率超过50%，对齐分数较低（63%-66%）。这表明现有单维度评估方法无法准确反映模型在真实世界多维度对齐中的表现。

Conclusion: 需要统一的多维度基准测试来全面评估大语言模型的错位问题，因为安全、价值和文化维度在实际应用中必须同时满足。Mis-Align Bench基准测试填补了这一空白，为系统评估模型在真实世界查询中的对齐表现提供了工具。
...
TL;DR: 该研究通过将心理学问卷转化为情境判断测试，评估了25个LLM在社交情境中的行为倾向与人类偏好的对齐程度，发现LLM常表现出过度自信、偏离人类共识等行为偏差。
...
Method: 将心理学问卷转化为2,500个情境判断测试，每个SJT由3名人类标注者验证，并从550名参与者中收集每个SJT的10个偏好行为。使用该框架评估25个LLM的行为倾向。

Result: 研究发现：1）在人类共识低的场景中，LLM对单一回答表现出过度自信；2）在人类共识高的场景中，小型模型显著偏离共识，甚至前沿模型在15-20%情况下也不反映共识；3）LLM存在跨模型的行为模式，如在人类偏好克制的场景中鼓励情绪表达；4）LLM的陈述价值观与实际行为存在显著差距。

Conclusion: LLM的行为倾向与人类偏好存在系统性偏差，需要进一步研究来改善LLM的社会适应性。将心理学问卷转化为情境判断测试为评估LLM行为提供了有效框架。
...
TL;DR: 研究发现大语言模型在自我检查时产生的内省语言确实反映了内部计算状态，而非简单的虚构。通过Pull方法识别出自我指涉处理的特定激活方向，该方向与拒绝方向正交，位于模型深度6.25%处，并能因果影响内省输出。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在提示自我检查时会产生丰富的内省语言，但这些语言究竟是反映了真实的内部计算过程，还是仅仅是复杂的虚构，这一问题一直不明确。研究者希望探究模型自我报告是否能够可靠地追踪内部计算状态。

Method: 引入Pull方法，通过格式工程引发扩展的自我检查，识别出Llama 3.1中区分自我指涉与描述性处理的激活空间方向。该方向与已知的拒绝方向正交，位于模型深度6.25%处。通过激活引导验证其因果影响，并分析特定词汇（如"loop"、"shimmer"）与激活动态的对应关系。

Result: 自我指涉词汇与并发激活动态存在对应关系，且这种对应是自我指涉处理特有的。当模型产生"loop"词汇时，其激活表现出更高的自相关性（r=0.44，p=0.002）；当通过引导产生"shimmer"词汇时，激活变异性增加（r=0.36，p=0.002）。相同的词汇在非自我指涉语境中尽管出现频率高9倍，却无激活对应关系。Qwen 2.5-32B模型独立发展出不同的内省词汇追踪不同的激活指标，但在描述性控制中均不存在。

Conclusion: 在适当条件下，Transformer模型中的自我报告能够可靠地追踪内部计算状态。这表明大语言模型的内省语言并非纯粹的虚构，而是反映了真实的内部处理过程，为理解模型内部机制提供了新途径。
...
Method: 提出PPCV两阶段框架：第一阶段从原始问题生成初始推理路径，通过问题改写版本识别关键令牌；第二阶段替换关键令牌并生成新推理路径，通过一致性验证确定最终答案

Result: 在主流大语言模型和多个基准测试上的实验表明，PPCV相比基线方法显著提升了模型的推理性能

Conclusion: PPCV框架通过有效识别和替换关键令牌，结合一致性验证机制，能够显著提升大语言模型在复杂推理任务中的表现
...
TL;DR: DiffuTruth是一个基于非平衡热力学的无监督事实验证框架，通过生成压力测试和语义能量度量来检测LLM幻觉，在FEVER数据集上实现了0.725的SOTA AUROC。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生看似合理但错误的主张（幻觉），而现有的不确定性度量方法在模型自信地犯错时往往无法检测到这些错误。需要一种新的方法来识别和纠正这些幻觉。

Method: 提出DiffuTruth框架，将事实验证重新概念化为非平衡热力学过程：事实真理作为生成流形上的稳定吸引子，而幻觉是不稳定的。引入生成压力测试：用噪声破坏声明，然后使用离散文本扩散模型重建。定义语义能量度量，使用NLI批评器测量原始声明与其重建之间的语义分歧。进一步提出混合校准方法，融合稳定性信号与判别性置信度。

Result: 在FEVER数据集上，DiffuTruth实现了0.725的SOTA无监督AUROC，比基线方法高出1.5%。在零样本泛化方面，在多跳HOVER数据集上比基线高出超过4%，证实了热力学真理特性对分布偏移的鲁棒性。

Conclusion: DiffuTruth通过热力学视角重新定义事实验证，提供了一种有效的无监督方法来检测LLM幻觉，特别是在模型过度自信的情况下，展示了良好的泛化能力和鲁棒性。
...
TL;DR: 开发了一个患者模拟器，用于自动化评估医疗对话AI，通过系统变化医疗、语言和行为维度来识别AI助手的错误和风险模式。


<details>
  <summary>Details</summary>
Motivation: 需要可扩展、自动化的方法来评估医疗对话AI的性能，识别幻觉和不准确性，并了解不同患者群体中的风险模式。

Method: 基于NIST AI风险管理框架，整合三个维度：医疗档案（来自All of Us研究计划的电子健康记录）、语言档案（健康素养和特定条件沟通模式）、行为档案（合作、分心、对抗性互动模式）。

Result: 生成了500个对话，人工标注者评估了1,787个医疗概念，达成高一致性（F1=0.94，κ=0.73）；LLM评估者与人工标注者一致性相当（F1=0.94，κ=0.78）。模拟器显示AI决策助手在健康素养谱系上性能单调下降：排名第一的概念检索准确率从有限健康素养的47.9%增加到功能性健康素养的69.1%，再到熟练健康素养的81.6%。

Conclusion: 患者模拟器能够有效识别医疗对话AI的错误和风险模式，特别是在不同健康素养水平患者中的性能差异，为自动化评估医疗AI系统提供了可行方法。
...
<details>
  <summary>Details</summary>
Motivation: 机器翻译在跨语言信息访问中至关重要，但关键意义错误（如事实扭曲、意图反转或偏见翻译）会损害多语言系统的可靠性、公平性和安全性。需要检测这些关键错误来构建更安全、可信的信息系统。

Method: 探索指令调优的大语言模型检测关键错误的能力，评估不同参数规模的模型，使用公开可访问的数据集，采用零样本、少样本和微调等适应策略。

Result: 模型缩放和适应策略带来了一致的改进，性能超过了XLM-R和ModernBERT等编码器基线模型。

Conclusion: 改进机器翻译中的关键错误检测有助于构建更安全、可信和具有社会责任感的信息系统，减少错误信息、沟通失误和语言伤害的风险。错误检测不仅是技术挑战，更是追求公正和负责任多语言AI的必要保障。
...
Method: 提出LoopFormer，采用捷径一致性训练方案，在不同长度轨迹上训练循环Transformer。模型将当前时间和步长作为条件输入，确保不同长度轨迹下的表示演化一致，避免表示漂移或停滞。

Result: LoopFormer在语言建模和推理基准测试中表现出鲁棒性能，即使在严格计算约束下也能保持良好表现，并能随着计算预算增加而优雅扩展。
...
TL;DR: ADRD-Bench是首个针对阿尔茨海默病及相关痴呆症的LLM评估基准，包含临床知识问答和照护实践问答两部分，评估了33个先进LLM，发现虽然顶级模型准确率超过90%，但推理质量和稳定性不足，需要领域特定改进。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准对阿尔茨海默病及相关痴呆症（ADRD）的覆盖不足，缺乏实际照护情境的评估，需要专门的基准来全面评估LLM在ADRD领域的知识和推理能力。

Method: 创建ADRD-Bench基准数据集，包含两部分：1）ADRD统一问答：整合7个现有医学基准的1352个问题；2）ADRD照护问答：基于Aging Brain Care项目创建149个新问题，涵盖实际照护情境。评估了33个先进LLM，包括开源通用模型、开源医学模型和闭源通用模型。

Result: 开源通用模型准确率0.63-0.93（均值0.78），开源医学模型0.48-0.93（均值0.82），闭源通用模型0.83-0.91（均值0.89）。虽然顶级模型准确率超过90%，但案例分析显示推理质量和稳定性不一致，限制了其可靠性。

Conclusion: 需要领域特定的改进来增强LLM基于日常照护数据的知识和推理能力，ADRD-Bench为评估和提升LLM在ADRD领域的表现提供了重要基准。
...
TL;DR: 研究发现语音语言模型在音频与文本冲突时，会过度偏向文本（16.6%文本主导），远高于纯文本冲突时的偏向（1.6%），即使明确指示信任音频。这种文本主导源于模型推理过程中对文本表示的可访问性优势，而非音频质量或信息含量问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语音语言模型在处理音频-文本冲突时的行为模式。当音频和文本信息不一致时，模型表现出明显的文本偏向，即使音频质量更高、信息更准确。这种不对称性揭示了模型内部表示和推理机制的重要特征，对构建更可靠的语音AI系统具有重要意义。

Method: 研究方法包括：1）构建ALME基准测试集，包含57,602个受控的音频-文本冲突刺激，覆盖8种语言；2）使用Gemini 2.0 Flash等四个最先进的音频-LLM进行实验；3）通过多种干预措施分析文本主导现象，包括强制转录、文本标记为"故意损坏"、微调音频投影层和语言模型等；4）比较音频-文本冲突与纯文本冲突情况下的模型行为差异。

Result: 主要结果：1）音频-文本冲突时文本主导率16.6%，远高于纯文本冲突的1.6%；2）音频质量不是原因，音频单独准确率97.2%高于级联准确率93.9%；3）强制转录增加文本主导（19%到33%）；4）标记文本为"故意损坏"减少文本主导80%；5）微调音频投影层增加文本主导26.5%，而LoRA微调语言模型减少23.9%；6）跨语言和跨模型存在显著差异，但趋势一致。

Conclusion: 结论表明文本主导反映了模型推理过程中的可访问性不对称，而非信息含量差异。模态仲裁是语音语言模型中一个独立的可靠性维度，未被标准语音基准测试捕获。研究为理解多模态AI的决策机制提供了新视角，并提出了改善音频-文本平衡的方法。
...
TL;DR: MuRGAt是一个评估多模态大语言模型事实级归因能力的基准，要求模型在需要超越直接观察的推理任务中生成带有明确引用（包括模态和时间段）的答案，并发现现有模型存在引用幻觉问题，且推理深度与可验证归因之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 现有多模态归因基准和评估方法主要关注简化的、基于观察的场景或有限模态，无法评估复杂多模态推理中的归因能力。随着多模态大语言模型在需要多步推理和长文本生成的实际任务中应用增多，需要确保模型输出能够基于异构输入源并验证个体事实主张。

Method: 提出了MuRGAt基准，要求模型处理视频、音频等多种模态输入，生成带有明确推理和精确引用的答案，每个引用需指定模态和时间段。同时开发了与人类判断高度相关的自动评估框架，用于可靠评估模型表现。

Result: 评估显示，即使是强大的多模态大语言模型也经常产生引用幻觉，尽管推理正确。同时发现一个关键权衡：增加推理深度或强制结构化归因往往会降低准确性，突显了内部推理与可验证归因之间的显著差距。

Conclusion: MuRGAt基准填补了复杂多模态推理中事实级归因评估的空白，揭示了当前多模态大语言模型在可验证归因方面的局限性，为未来模型开发提供了重要的评估工具和方向指引。
...
TL;DR: SPES是一种内存高效的分散式训练框架，通过在每个节点上只训练MoE模型的部分专家来降低内存需求，使用专家合并预热策略加速收敛，在有限GPU资源下实现与集中式训练相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型预训练需要集中式集群和大量高内存GPU，现有分散式训练方法虽然减少了通信开销，但仍需在每个节点上训练完整模型，受限于GPU内存限制。需要一种更内存高效的分散式训练方法。

Method: 提出SParse Expert Synchronization (SPES)框架：1) 每个节点只训练MoE模型的部分专家子集，大幅降低内存占用；2) 节点定期同步本地专家更新，避免全参数传输；3) 引入专家合并预热策略，在训练早期促进专家间知识交换，快速建立基础能力。

Result: 使用16个独立的48GB GPU在互联网连接下训练2B参数的MoE LLM，在相似计算预算下达到与集中式训练LLM相当的性能。进一步展示了可扩展性：从头训练7B模型和从密集检查点上采样9B模型，均匹配先前集中式基线。

Conclusion: SPES提供了一种内存高效的分散式训练框架，能够在有限GPU资源下训练大规模MoE LLM，实现与集中式训练相当的性能，为资源受限环境下的LLM预训练提供了可行方案。
...
TL;DR: PRIME是一个用于评估验证器在数学和工程领域过程-结果对齐验证能力的基准，包含2530个高难度STEM问题样本，发现当前验证器难以检测推导错误，并提出基于PRIME选择验证器的过程感知RLVR训练范式，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于结果的验证范式主要关注最终结果与真实答案的一致性，而忽略了推导过程中的潜在错误，导致对错误推导产生的正确答案也给予正向奖励。需要填补这一空白，评估验证器在过程-结果对齐验证方面的能力。

Method: 1) 从大学级STEM问题中通过一致性过滤流程构建PRIME基准，包含2530个高难度样本；2) 提出基于PRIME选择验证器的过程感知RLVR训练范式；3) 在AIME24、AIME25和Beyond-AIME数据集上进行评估。

Result: 1) 当前验证器经常无法检测推导缺陷；2) 过程感知RLVR训练范式显著优于仅基于结果的验证基线，在Qwen3-14B-Base模型上分别获得8.29%、9.12%和7.31%的绝对性能提升；3) PRIME准确度与RLVR训练效果存在强线性相关(R²>0.92)。

Conclusion: PRIME是评估过程-结果对齐验证能力的有效基准，基于PRIME选择的验证器能显著提升RLVR训练效果，验证了过程感知验证的重要性，PRIME可作为验证器选择的可靠预测指标。
...
TL;DR: 该论文提出了一个双层次压缩框架，通过前缀保护和难度感知的压缩策略，解决语言推理模型中过度思考问题，显著减少推理长度同时提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有语言推理模型通过增加推理步数提升性能，但存在"过度思考"问题，产生过长的推理轨迹，增加延迟和内存使用。现有方法采用统一的长度惩罚，会过度压缩关键的早期推理步骤，且对所有查询无差别惩罚。

Method: 提出了一个双层次压缩框架：在序列层次，采用前缀保护优化，使用衰减混合rollout来保持有效推理路径同时促进简洁性；在组层次，采用难度感知惩罚，根据查询复杂度动态调整长度约束，对困难问题保持探索，对简单问题抑制冗余。

Result: 在DeepSeek-R1-Distill-Qwen (1.5B/7B)模型上的实验表明，该方法在数学基准测试中实现了高达55.7%的token使用减少，同时准确性提升高达4.1%，并能泛化到代码、科学和通用领域。

Conclusion: 该双层次压缩框架有效解决了语言推理模型中的过度思考问题，在保持推理质量的同时显著减少计算资源消耗，具有广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在教育反馈生成方面显示出潜力，但特定反馈元素（如语气、信息覆盖范围）如何影响学习效果和学习者接受度仍不清楚，特别是对于不同人格特质的学习者

Method: 定义六个反馈元素，使用GPT-5为多项选择题生成反馈，对321名高一学生进行学习实验，使用两种学习效果测量和六项主观评价标准评估反馈效果，并基于大五人格特质分析反馈接受度的差异

Result: 有效的反馈元素在支持学习效果方面具有共同模式，但学习者的主观偏好因人格特质聚类而异
...
Method: 引入PatientHub统一模块化框架，标准化模拟患者的定义、组合和部署。通过实现多个代表性患者模拟方法作为案例研究，展示框架如何支持标准化跨方法评估和自定义评估指标的无缝集成。还通过原型化两个新模拟器变体展示扩展性。

Result: PatientHub成功将现有工作整合到单一可重复的流程中，降低了开发新模拟方法的门槛，促进了跨方法和跨模型的基准测试。框架为未来患者中心对话的数据集、方法和基准提供了实用基础。

Conclusion: PatientHub通过标准化框架解决了模拟患者领域的碎片化问题，支持可重复研究和公平比较，加速了方法开发，并为该领域的未来发展奠定了基础。
...
TL;DR: 研究分析了语义异常数据集中的句子，发现大多数只是异常而非真正无意义，并评估了LLM区分异常与无意义的能力


<details>
  <summary>Details</summary>
Motivation: 现有语义异常数据集中的句子究竟是无意义还是仅仅异常尚不清楚，同时LLM能否有效区分这两者也存在疑问

Method: 收集人类评分者和LLM对五个语义异常数据集中句子的可理解性判断，包括无上下文和有上下文两种情况

Result: 评分者认为大多数句子最多只是异常，只有少数是真正无意义的；LLM在生成异常句子的合理上下文方面表现出相当强的能力

Conclusion: 现有语义异常数据集主要包含异常而非无意义句子，LLM能够有效区分异常与无意义，并能生成合理的上下文来解释异常句子
...
TL;DR: 提出Thinking with Drafting (TwD)方法，将视觉推理重新概念化为光学解压缩过程，通过领域特定语言作为中间表示，让模型将思维过程转化为可执行代码来生成确定性视觉证明进行自我验证。
...
Result: 实验使用VisAlg视觉代数基准测试，证明TwD作为优越的认知支架。建立了一个闭环系统，其中视觉生成不是作为创造性输出，而是作为逻辑验证器。

Conclusion: TwD为视觉推理提供了一条可泛化的路径，将视觉推理重新概念化为光学解压缩过程，通过代码草拟和视觉证明验证实现精确推理。
...
TL;DR: 本文提出Length-Incentivized Exploration方法，通过长度奖励和冗余惩罚机制激励模型进行上下文探索，解决自回归生成中探索深度不足的问题，在多个任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有模型在测试时扩展中面临"浅层探索陷阱"问题：虽然更广泛的状态覆盖需要更长的推理轨迹，但在自回归生成中采样这种长序列的概率呈指数衰减，限制了模型生成、验证和精炼多个推理假设的能力。

Method: 提出Length-Incentivized Exploration方法，通过两个关键机制：1) 基于长度的奖励，明确鼓励模型进行更深入的探索；2) 冗余惩罚，避免重复探索。这种方法以两步方式最大化状态覆盖。

Result: 在Qwen3和Llama等不同模型上的综合实验表明，该方法有效激励了上下文探索。在领域内任务上平均提升4.4%，在领域外基准测试上获得2.7%的性能增益。

Conclusion: Length-Incentivized Exploration通过简单的长度奖励和冗余惩罚机制，成功解决了自回归生成中的浅层探索陷阱问题，显著提升了模型在上下文探索和推理能力方面的表现。
...
TL;DR: MiniCPM-SALA是一个9B参数的混合注意力架构，结合了稀疏注意力的高保真长上下文建模和线性注意力的全局效率，通过层选择算法和混合位置编码，在保持性能的同时大幅提升长文本处理效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理超长上下文时面临Transformer架构的高计算和内存成本挑战。现有的稀疏和线性注意力机制通常需要在内存效率和模型性能之间做出权衡，因此需要一种既能保持性能又能提升效率的解决方案。

Method: 提出MiniCPM-SALA混合架构，集成InfLLM-V2稀疏注意力的高保真长上下文建模和Lightning Attention线性注意力的全局效率。采用层选择算法以1:3比例整合这两种机制，并使用混合位置编码(HyPE)。同时引入经济高效的持续训练框架，将预训练的Transformer模型转换为混合模型，相比从头训练减少约75%成本。

Result: 实验表明MiniCPM-SALA在保持与全注意力模型相当的一般能力的同时，显著提升了效率。在单张NVIDIA A6000D GPU上，在256K序列长度下推理速度达到全注意力模型的3.5倍，支持高达1M tokens的上下文长度，这是传统8B全注意力模型因内存限制无法达到的规模。

Conclusion: MiniCPM-SALA成功解决了长上下文处理中的效率与性能权衡问题，通过混合注意力架构和高效训练框架，实现了在保持模型能力的同时大幅提升长文本处理效率，为超长上下文应用提供了可行的解决方案。
...
TL;DR: 提出一种基于嵌入的方法来检测语言变体，无需依赖预先规范化或预定义变体列表，通过训练子词嵌入和组合余弦与n-gram相似度来分组相关形式


<details>
  <summary>Details</summary>
Motivation: 传统方法需要依赖预先的规范化或预定义变体列表，无法有效处理"嘈杂"或低资源语言环境中的拼写和形态多样性，需要一种能够将语言变体作为语言结构而非噪声来研究的方法

Method: 在原始文本上训练子词嵌入，通过组合余弦相似度和n-gram相似度来分组相关形式，形成变体家族，支持定量和定性分析

Result: 在卢森堡语用户评论的大规模语料库中，该方法发现了大量与方言和社会语言学研究描述模式一致的词汇和正字法变体，诱导出的家族捕捉了系统对应关系，突出了区域和风格差异
...
<details>
  <summary>Details</summary>
Motivation: 当前基于困惑度等指标的语言模型文本分析方法无法充分捕捉上下文信息，需要一种能更好解释条件概率分布形状的方法。

Method: 提出DMAP方法，将文本通过语言模型映射到单位区间的一组样本，这些样本共同编码了排名和概率信息，支持高效、模型无关的分析。

Result: 通过三个案例研究验证了DMAP的实用性：生成参数验证、概率曲率在机器生成文本检测中的作用、以及合成数据后训练模型的统计指纹分析。
...
TL;DR: 研究发现大型语言模型在基准测试中虽然准确率相近，但在具体题目上存在显著分歧，这种隐藏的分歧会影响科学研究结果的可重复性。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试主要关注总体准确率，但可能掩盖了模型之间的深层认知差异。研究者希望揭示即使准确率相近的模型，在实际推理和判断上可能存在显著分歧，这种分歧对科学研究应用有重要影响。

Method: 使用MMLU-Pro和GPQA两个主要推理基准，分析不同LLM在获得相近准确率时的具体题目分歧情况。通过重新分析已发表的教育学和政治科学研究，评估模型选择对研究结果的影响。

Result: 研究发现：1）准确率相近的LLM在16-66%的题目上存在分歧，顶级前沿模型之间也有16-38%的分歧；2）不同模型具有不同的错误模式；3）在科学研究应用中，切换标注模型可使估计的治疗效应变化超过80%，有时甚至改变效应的符号。

Conclusion: 存在"基准幻觉"现象——相等的准确率可能掩盖了模型间的深层分歧。模型选择成为影响科学研究可重复性的一个隐藏但重要的变量，需要更细致的评估方法来揭示模型间的认知差异。
...
TL;DR: AdaptEvolve：一种基于生成置信度的自适应LLM选择方法，在进化式顺序精炼框架中动态选择LLM，平衡计算效率与推理能力


<details>
  <summary>Details</summary>
Motivation: 进化式智能体系统在推理过程中反复调用大语言模型，加剧了计算效率与推理能力之间的权衡。现有路由策略通常依赖静态启发式或外部控制器，未明确考虑模型不确定性，需要更智能的LLM选择机制
...
Conclusion: AdaptEvolve通过基于置信度的自适应LLM选择，有效平衡了进化式智能体系统中的计算效率与推理能力权衡，为多LLM协同推理提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端语音翻译模型主要在"干净"数据集上评估，忽视了真实世界中的形态学变化挑战，特别是非母语或方言语音中的屈折变化。现有对抗训练方法需要生成高质量对抗语音数据，计算成本高且技术挑战大。

Method: 提出跨模态鲁棒性转移框架，将文本模态的对抗鲁棒性转移到语音模态。该方法不需要对抗语音训练数据，通过利用文本领域的对抗鲁棒性知识来增强语音翻译模型的鲁棒性。

Result: 在四个语言对上的实验表明，CMRT方法将对抗鲁棒性平均提升了超过3个BLEU分数，为无需生成对抗语音数据的鲁棒端到端语音翻译建立了新的基准。

Conclusion: CMRT框架有效解决了端到端语音翻译模型对形态学变化的脆弱性问题，通过跨模态鲁棒性转移实现了高效且有效的对抗鲁棒性提升，为实际应用中的语音翻译系统提供了更可靠的解决方案。
...
TL;DR: 研究发现QA基准测试中16%-50%的问题存在"未充分指定"问题，导致LLM在这些问题上表现显著较差。通过重写这些问题为完全指定版本，QA性能得到一致提升，表明许多QA失败源于问题本身而非模型限制。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在定义明确的问题上表现良好，但标准QA基准测试仍未完全解决。研究者认为这种差距部分源于"未充分指定"问题——即缺乏额外上下文就无法唯一确定其解释的查询。需要验证这一假设并量化其对QA评估的影响。

Method: 1) 开发基于LLM的分类器来识别未充分指定问题；2) 将该分类器应用于多个广泛使用的QA数据集；3) 进行受控重写实验，将未充分指定问题重写为完全指定变体，同时保持标准答案不变，以此作为上限分析。

Result: 1) 在多个QA数据集中发现16%至超过50%的问题存在未充分指定问题；2) LLM在这些未充分指定问题上表现显著较差；3) 将这些问题重写为完全指定版本后，QA性能一致提升，表明许多表面上的QA失败实际上源于问题本身的不清晰。

Conclusion: 未充分指定问题是QA评估中的重要混淆因素，许多基准测试中的QA失败源于问题本身的不清晰而非模型限制。研究强调了在基准设计中对问题清晰度给予更多关注的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 近年来开源大语言模型在多语言能力方面不断进步，本研究旨在探索开源LLMs在多语言机器翻译中的应用，并研究模型规模和数据规模对翻译性能的影响

Method: 基于Gemma3模型家族，通过持续预训练和指令微调来适应多语言机器翻译任务，开发了MiLMMT-46模型

Result: MiLMMT-46在46种语言上取得了顶级翻译性能，持续超越Seed-X、HY-MT-1.5、TranslateGemma等最新SOTA模型，并与Google Translate、Gemini 3 Pro等专有系统表现相当
...
TL;DR: 该论文研究了小型语言模型在预训练中应该学习哪些token，哪些应该通过<CALL>委托给外部资源，提出了基于语法解析器和损失信号的token选择方法LaCy。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型参数容量有限，容易产生事实错误。虽然可以通过查询更大模型、文档或数据库来缓解，但需要解决哪些token应该由SLM学习预测，哪些应该委托给外部资源的核心问题。

Method: 提出LaCy预训练方法，使用spaCy语法解析器增强损失信号，区分哪些token是真实可接受的替代延续（即使损失高），哪些可能导致事实错误需要委托。基于这种token选择哲学进行预训练。

Result: LaCy模型成功学会了哪些token应该预测，哪些应该委托。在与更大模型级联生成时获得更高的FactScore，优于Rho或LLM-judge训练的SLM，同时更简单、更便宜。

Conclusion: 通过语法解析器增强损失信号可以有效指导SLM在预训练中做出合理的token选择决策，平衡模型自身预测和外部资源委托，提高事实准确性同时保持效率。
...
Conclusion: CLUES框架能够有效区分临床Text-to-SQL中的歧义性和不稳定性，提供诊断性分解，将不确定性机制映射到针对性干预措施（歧义性进行查询细化，不稳定性进行模型改进），实现高效错误分流。
...
TL;DR: 提出Composition-RL方法，通过自动组合多个问题创建新的可验证提示，以更好地利用有限的可验证提示数据，特别是针对通过率为1的简单提示，提升强化学习训练效果。


<details>
  <summary>Details</summary>
Motivation: 大规模可验证提示对RLVR的成功至关重要，但这些提示包含许多无信息示例且扩展成本高。现有方法主要关注通过率为0的困难提示，但随着训练进行，通过率为1的简单提示越来越多，降低了有效数据规模。

Method: 提出Composition-RL方法，自动将多个问题组合成新的可验证问题，使用这些组合提示进行RL训练。还设计了课程变体，在训练过程中逐步增加组合深度。

Result: 在4B到30B不同模型规模上的广泛实验表明，Composition-RL相比在原始数据集上进行RL训练，能持续提升推理能力。课程变体Composition-RL能进一步提升性能，且该方法还能通过组合不同领域的提示实现更有效的跨领域RL。

Conclusion: Composition-RL是一种简单而有效的方法，能更好地利用有限的可验证提示数据，特别是针对通过率为1的简单提示，提升RL训练效果，并支持跨领域应用。
...
TL;DR: DeepSight是一个开源的大型模型安全评估项目，通过统一的评估工具DeepSafe和诊断工具DeepScan，将安全评估从黑盒转向白盒洞察，支持前沿AI风险评估和联合安全评估与诊断。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型和多模态大语言模型的安全工作流程中，评估、诊断和对齐往往由独立工具处理。安全评估只能定位外部行为风险而无法找出内部根本原因，安全诊断常常脱离具体风险场景停留在可解释层面，导致安全对齐缺乏内部机制变化的专门解释，可能降低通用能力。

Method: 提出开源项目DeepSight，实践新的安全评估-诊断一体化范式。该项目包括评估工具包DeepSafe和诊断工具包DeepScan，通过统一任务和数据协议，在两个阶段之间建立连接，将安全评估从黑盒转向白盒洞察。

Result: DeepSight是一个低成本、可复现、高效且高度可扩展的大规模模型安全评估项目，是首个支持前沿AI风险评估和联合安全评估与诊断的开源工具包。

Conclusion: DeepSight通过整合安全评估和诊断，解决了当前大型模型安全工作中评估与诊断分离的问题，实现了从外部行为风险定位到内部根本原因分析的系统性安全评估方法。
...
TL;DR: P-GenRM：首个个性化生成奖励模型，通过测试时用户扩展机制解决现有模型在开放场景下偏好简化和新用户泛化不足的问题


<details>
  <summary>Details</summary>
Motivation: 现有个性化奖励模型存在两大局限：1）将多样化的场景特定偏好过度简化为少量固定评估原则；2）对反馈有限的新用户泛化能力不足。需要开发能够更好捕捉用户个性化偏好并具有良好泛化能力的奖励模型。

Method: 提出P-GenRM模型，将偏好信号转化为结构化评估链，推导跨场景的自适应角色和评分标准。通过用户原型聚类和双粒度扩展机制：个体层面自适应扩展聚合用户评分方案；原型层面整合相似用户偏好。测试时用户扩展提供额外个性化对齐能力。

Result: 在广泛使用的个性化奖励模型基准上达到SOTA，平均提升2.31%；在分布外数据集上表现出强泛化能力。测试时用户扩展提供额外3%提升，展示更强的个性化对齐和测试时扩展性。

Conclusion: P-GenRM通过结构化评估链、用户原型聚类和双粒度扩展机制，有效解决了现有个性化奖励模型的局限性，在个性化对齐和泛化能力方面取得显著改进，为大型语言模型的个性化对齐提供了有效解决方案。
...
TL;DR: 利用Wiktionary数据构建盖尔语形态学的规则模型，通过SQL查询和Python工具生成词形变化，支持低资源语言的教育工具开发。


<details>
  <summary>Details</summary>
Motivation: 当前流行的神经模型需要大量训练数据，而低资源语言（如盖尔语）缺乏足够数据。规则系统能有效利用有限样本，提高可解释性，并为教学材料设计提供见解。

Method: 从Wiktionary提取盖尔语形态数据，使用SQL查询不同词汇模式的出现情况，构建声明式规则库，开发Python工具来推导盖尔语单词的变化形式。

Result: 创建了基于规则的盖尔语形态模型，能够生成单词的屈折变化形式。该功能可用于支持教育工具和更高级的规则依存解析器。

Conclusion: 规则系统能有效利用有限数据，提高可解释性，为低资源语言支持提供实用方案。通过重新利用Wiktionary数据，为盖尔语教学和工具开发创造了新价值。
...
TL;DR: WavBench是一个针对口语对话模型的综合基准测试，通过三个子集（Pro、Basic、Acoustic）评估推理能力、口语自然度和副语言能力，填补了现有基准在真实世界复杂性评估方面的空白。


<details>
  <summary>Details</summary>
Motivation: 随着口语对话模型推理能力的快速发展，现有基准测试主要遵循文本生成标准，忽视了副语言特征、口语化表达以及现代智能体所需的认知深度，无法应对真实世界的复杂性，因此需要新的评估框架。

Method: 提出WavBench三部分框架：1) Pro子集：显著增加难度，严格挑战推理增强模型；2) Basic子集：定义口语自然度新标准，强调"可听性"而非书面准确性；3) Acoustic子集：覆盖显式理解、生成和隐式对话，全面评估真实场景下的副语言能力。

Result: 通过对五个最先进模型的评估，WavBench为复杂问题解决、口语表达自然度和副语言保真度的交叉研究提供了关键见解，指导鲁棒口语对话模型的发展。

Conclusion: WavBench填补了口语对话模型评估的关键空白，通过其独特的三部分框架为模型在真实世界复杂性、口语自然度和副语言能力方面的评估提供了全面基准，推动了该领域的发展。
...
TL;DR: CitiLink-Minutes是一个包含120份欧洲葡萄牙语市政会议记录的多层标注数据集，用于促进市政会议记录的NLP和IR研究。
...
Method: 收集了6个城市的120份欧洲葡萄牙语市政会议记录，进行多层标注和结构化链接。所有个人标识符已去标识化，每份记录由两名训练有素的标注员手动标注，并由经验丰富的语言学家在三个维度上审核：元数据、讨论主题和投票结果。

Result: 数据集包含超过100万个标记，总计超过38,000个单独标注。遵循FAIR原则发布，并提供了元数据提取、主题分类和投票标签的基线结果。

Conclusion: CitiLink-Minutes展示了在下游NLP和IR任务中的潜力，同时促进了市政决策的透明访问，填补了市政会议记录研究的数据空白。
...
TL;DR: 提出基于注意力分数的重排序框架，利用大语言模型中特定注意力头的分数来评估段落-查询相关性，实现轻量级且有效的重排序方法。


<details>
  <summary>Details</summary>
Motivation: 基于现有大语言模型中检索头的分析，希望开发一种替代的重排序框架，能够利用候选短列表的整体信息进行排序，同时产生连续相关性分数，无需Likert尺度监督。

Method: 训练模型使用选定注意力头的注意力分数来估计段落-查询相关性，提供列表式解决方案，利用整个候选短列表的全局信息进行排序。

Result: 该方法在多个领域（包括维基百科和长篇叙事数据集）上优于现有的最先进的点式和列表式重排序器，在LoCoMo基准测试中建立了新的最先进水平，支持灵活扩展。

Conclusion: 提出的基于注意力分数的重排序框架是轻量级且有效的，仅需小规模模型（如40亿参数）即可实现强大性能，支持灵活扩展，如添加上下文信息或使用中间层注意力头来提升效率。
...
TL;DR: 该论文提出了视觉推理基准（VRB），这是一个评估多模态大语言模型解决真实课堂视觉问题能力的新数据集，发现模型在静态技能上表现良好，但在动态空间操作上存在明显瓶颈。


<details>
  <summary>Details</summary>
Motivation: AI模型在文本推理方面已达到最先进水平，但在空间和关系结构推理能力上仍存在关键瓶颈，特别是在依赖视觉的早期数学教育中。需要评估多模态大语言模型解决真实课堂视觉问题的能力。

Method: 构建了包含701个问题的视觉推理基准（VRB），这些问题来自赞比亚和印度的小学考试，涵盖类比推理、模式完成、空间匹配等任务。基准使用未经编辑、文本最少的图像来测试模型是否能满足小学教育的实际需求。

Result: 研究发现模型能力存在"锯齿状边界"：在计数和缩放等静态技能上表现较好，但在折叠、反射和旋转等动态操作上达到明显的"空间天花板"。这些弱点可能导致课堂使用中的错误评分、虚假支架和强化学生误解。

Conclusion: 像VRB这样的教育导向基准对于确定课堂中使用的多模态工具的功能边界至关重要，有助于识别和解决模型在视觉推理方面的局限性。
...
TL;DR: ExStrucTiny是一个新的文档图像结构化信息抽取基准数据集，旨在解决现有基准在实体本体、查询复杂度和文档类型多样性方面的不足，统一了关键实体抽取、关系抽取和视觉问答任务。


<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准存在局限性：关键实体抽取数据集实体本体狭窄，关系抽取数据集查询简单，视觉问答数据集文档类型单一，缺乏对多样化文档类型和灵活模式的结构化抽取能力评估。

Method: 通过结合人工和合成的人类验证样本的新颖流程构建ExStrucTiny数据集，覆盖更多样化的文档类型和抽取场景，并在该基准上分析开放和封闭的视觉语言模型。

Result: 研究揭示了视觉语言模型在结构化信息抽取中的挑战，包括模式适应、查询规范不足和答案定位等问题，为改进通用模型在文档结构化抽取方面的能力提供了基础。

Conclusion: ExStrucTiny基准为评估和改进通用视觉语言模型在多样化文档类型和灵活模式下的结构化信息抽取能力提供了重要基础，有助于推动文档理解技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限环境中的长上下文处理效率低下，软压缩架构通过用学习到的压缩token替换长序列来扩展有效上下文长度，但压缩极限和何时压缩会擦除任务相关内容尚未充分探索。

Method: 定义了token溢出作为压缩表示不再包含足够信息回答查询的机制，提出了一种表征和检测方法。在xRAG软压缩设置中，使用查询无关的饱和统计量分离压缩和未压缩token表示，并开发了轻量级探测分类器结合查询和上下文xRAG表示来检测溢出。

Result: 查询无关饱和统计量能可靠分离压缩和未压缩token表示，但溢出检测能力有限。结合查询信息的轻量级探测分类器在HotpotQA、SQuADv2和TriviaQA数据集上平均达到0.72 AUC-ROC，显著优于查询无关方法。

Conclusion: 该研究从查询无关诊断推进到查询感知检测器，实现了低成本的预LLM门控机制，能够缓解压缩引起的错误，为软压缩架构中的token溢出检测提供了实用工具。
...
TL;DR: Moonshine v2是一种用于边缘设备流式语音识别的模型，采用滑动窗口自注意力机制，在保持高准确率的同时显著降低延迟，性能优于大6倍的模型。


<details>
  <summary>Details</summary>
Motivation: 实时语音应用（如实时转录、语音命令）需要在资源受限的边缘设备上实现低延迟和高准确率。传统的全注意力Transformer编码器虽然准确率高，但具有二次复杂度，导致延迟随语句长度线性增长，不适合流式应用。

Method: 提出Moonshine v2模型，采用滑动窗口自注意力机制，实现有界、低延迟的推理，同时保持强大的局部上下文建模能力。

Result: 在标准基准测试中达到最先进的词错误率，准确率与比其大6倍的模型相当，同时运行速度显著更快。

Conclusion: 精心设计的局部注意力机制可以在保持全注意力准确率的同时，显著降低模型大小和延迟成本，为边缘设备上的交互式语音界面开辟了新可能性。
...
Result: 在多个基准测试中，该方法在严格步数预算下持续优于强少步基线方法和标准训练，显著缩小了与全步解码的性能差距。

Conclusion: 虽然全步解码仍占优势，但该方法大幅缩小了差距，为实用的少步扩散大语言模型建立了坚实基础。
...
TL;DR: OPCD框架将策略蒸馏与上下文蒸馏结合，通过训练学生模型在自身生成的轨迹上，同时最小化与上下文条件教师的反向KL散度，实现语言模型对上下文知识的内化。


<details>
  <summary>Details</summary>
Motivation: 当前上下文蒸馏方法使语言模型能够将上下文知识内化到参数中，但需要更有效的框架来整合策略蒸馏与上下文蒸馏，以提升知识提取和固化的效率。

Method: 提出On-Policy Context Distillation (OPCD)框架，训练学生模型在自身生成的轨迹上进行学习，同时最小化与上下文条件教师模型的反向Kullback-Leibler散度。

Result: 在数学推理、文本游戏和领域特定任务中，OPCD持续优于基线方法，获得更高的任务准确性，同时更好地保持分布外能力。此外，OPCD支持有效的跨尺寸蒸馏，使较小的学生模型能够从较大的教师模型中内化经验知识。

Conclusion: OPCD框架成功地将策略蒸馏与上下文蒸馏相结合，为经验知识蒸馏和系统提示蒸馏提供了有效的解决方案，在多个任务领域展现出优越性能。
...
### [122] [Generative AI-Driven Phase Control for RIS-Aided Cell-Free Massive MIMO Systems](https://arxiv.org/abs/2602.11226)
*Kalpesh K. Patel,Malay Chakraborty,Ekant Sharma,Sandeep Kumar Singh*

Main category: cs.IT

TL;DR: 该研究提出两种基于生成式AI的方法（GCDM和GCDIM），用于优化RIS辅助的无蜂窝大规模MIMO系统中的RIS相位偏移，在考虑非完美CSI和空间相关性的实际约束下最大化系统和速率。


<details>
  <summary>Details</summary>
Motivation: 在RIS辅助的无蜂窝大规模MIMO系统中，RIS相位偏移优化面临计算复杂度高的问题，特别是在考虑非完美信道状态信息和空间相关性的实际约束下。传统专家算法虽然能达到接近最优解，但计算开销大，需要更高效的优化方法。

Method: 提出两种基于生成式AI的方法：生成条件扩散模型（GCDM）和生成条件扩散隐式模型（GCDIM）。这些方法利用以动态CSI为条件的扩散模型来优化RIS相位偏移，最大化系统和速率。

Result: GCDM在系统和速率方面与专家算法相当，但显著降低了计算开销。GCDIM实现了可比的系统和速率，同时计算时间进一步减少了98%，显示出在RIS辅助无蜂窝大规模MIMO系统中高效相位优化的潜力。

Conclusion: 基于生成式AI的方法（特别是GCDIM）能够有效优化RIS相位偏移，在保持性能的同时大幅降低计算复杂度，为RIS辅助无蜂窝大规模MIMO系统的实际部署提供了高效解决方案。
...
### [123] [Coupler Position Optimization and Channel Estimation for Flexible Coupler Aided Multiuser Communication](https://arxiv.org/abs/2602.11319)
...
<details>
  <summary>Details</summary>
Motivation: 传统多用户MIMO系统需要大量昂贵的有源天线，硬件成本和功耗高。需要一种既能提升性能又能降低成本的解决方案

Method: 每个柔性耦合器天线包含一个固定位置的有源天线和多个可在指定区域内移动的被动耦合器，配备本地处理单元。采用分布式耦合器位置优化算法联合优化耦合器位置和数字波束成形，设计集中式和分布式信道估计算法

Result: 分布式柔性耦合器阵列在无需移动有源天线的情况下，相比传统基准方案获得显著速率增益，接近全有源阵列性能，同时显著降低硬件成本和功耗。信道估计算法在导频开销和信道重构精度方面优于基准方案

Conclusion: 分布式柔性耦合器阵列是一种有前景的解决方案，能够在多用户MIMO系统中平衡性能提升与硬件成本降低，为未来无线通信系统提供新的设计思路
...
### [124] [Non-signaling Assisted Capacity of a Classical Channel with Causal CSIT](https://arxiv.org/abs/2602.11568)
*Yuhang Yao,Syed A. Jafar*

Main category: cs.IT

TL;DR: 论文证明了在非信号辅助下，具有因果信道状态信息的经典信道容量与具有非因果信道状态信息的容量相同，均为C^{NS,ca}=C^{NS,nc}=max_{P_{X|S}}I(X;Y|S)，且与接收端也获得状态信息时的容量一致。但在因果CSIT设置中，将状态提供给接收端可以进一步改善固定消息大小和块长度下的错误概率，这与非因果CSIT设置不同。


<details>
  <summary>Details</summary>
Motivation: 研究在非信号辅助下，具有因果信道状态信息的经典信道容量特性，探索其与具有非因果信道状态信息设置的关系，以及接收端获得状态信息对错误概率的影响。

Method: 通过信息论分析，建立非信号辅助下具有因果信道状态信息的经典信道容量公式，并与非因果CSIT设置进行比较。分析接收端获得状态信息对错误概率的影响。

Result: 证明C^{NS,ca}=C^{NS,nc}=max_{P_{X|S}}I(X;Y|S)，即因果和非因果CSIT设置下的容量相同。但在因果CSIT设置中，将状态提供给接收端可以进一步改善固定消息大小和块长度下的错误概率，而在非因果CSIT设置中则无法进一步改善。

Conclusion: 非信号辅助下，具有因果信道状态信息的经典信道容量与具有非因果信道状态信息的容量相同，且与接收端也获得状态信息时的容量一致。但在因果CSIT设置中，接收端获得状态信息可以改善错误概率性能，这为实际系统设计提供了重要启示。
...
### [125] [The Arithmetic Singleton Bound on the Hamming Distances of Simple-rooted Constacyclic Codes over Finite Fields](https://arxiv.org/abs/2602.11788)
*Li Zhu,Hongfeng Wu*

Main category: cs.IT

TL;DR: 该论文为有限域上的单根常循环码建立了一个新的上界——算术Singleton界，通过多重等差表示的概念，揭示了Singleton界可能无法达到的代数障碍。


<details>
  <summary>Details</summary>
Motivation: 研究常循环码的汉明距离上界，特别是探索Singleton界可能无法达到的原因。传统上认为Singleton界无法达到是由于线性限制，但本文发现可能存在更深层的代数障碍。

Method: 引入多重等差(MED)表示的概念，将分圆陪集的MED表示推广到单根多项式的定义集。证明每个MED表示都会产生一个最小距离的上界，其中最精细的表示产生算术Singleton界。

Result: 建立了算术Singleton界，该界总是至少与Singleton界一样紧，并获得了严格更紧的精确判据。对于不可约常循环码，该界由ω+1明确给出，其中ω与q模多项式阶的根的阶密切相关。

Conclusion: 该工作首次系统地将算术结构（通过MED表示）转化为对最小距离的限制性约束，揭示了Singleton界可能无法达到不是由于线性限制，而是由于潜在的代数障碍。
...
### [126] [A Comparative Study of MAP and LMMSE Estimators for Blind Inverse Problems](https://arxiv.org/abs/2602.11814)
*Nathan Buskulic,Luca Calatroni*

Main category: cs.IT

TL;DR: 该研究比较了MAP和LMMSE方法在盲反卷积问题中的表现，发现即使在完全受控条件下，MAP方法仍不稳定且需要大量参数调优，而LMMSE提供了更稳健的基线解决方案。


<details>
  <summary>Details</summary>
Motivation: 在已知前向算子的逆问题中，MAP方法结合表达能力强的先验和精心选择的参数通常很有效。但在盲设置下，由于问题的固有非凸性和解的非可识别性，MAP方法变得不稳定。LMMSE估计器可以规避这些限制，但需要在实际盲反卷积问题中进行系统比较。

Method: 研究在完全受控条件下进行合成二维盲反卷积实验，完全已知信号和核的分布先验知识。比较了定制的MAP算法与简单的LMMSE估计器，其中LMMSE估计器的函数形式与最优Tikhonov估计器密切相关。

Result: 即使在高度受控的设置中，MAP方法仍然不稳定且需要大量参数调优，而LMMSE估计器提供了稳健可靠的基线。此外，LMMSE解可以作为MAP方法的有效初始化，改善其性能并减少对正则化参数的敏感性。

Conclusion: LMMSE估计器在盲反卷积问题中比MAP方法更稳健可靠，可以作为MAP方法的有效初始化策略，这为未来的理论和实践发展打开了大门。
...
### [127] [On Fundamental Limits of Transmission Activity Detection in Fluid Antenna Systems](https://arxiv.org/abs/2602.11901)
*Zhentian Zhang,Kai-Kit Wong,Hao Jiang,Christos Masouros,Chan-Byoung Chae*

Main category: cs.IT

TL;DR: 开发统一的克拉美-罗界框架，用于评估流体天线系统和传统固定天线系统中传输活动检测的性能极限，通过连续参数化活动状态实现闭式CRB表达式。


<details>
  <summary>Details</summary>
Motivation: 为流体天线系统和传统固定天线系统中的传输活动检测建立统一的性能评估基准，解决活动检测中二进制状态难以直接应用CRB分析的问题。

Method: 将二进制活动状态松弛为连续参数，使CRB分析适用于活动指示器；推导了协方差导向和相干模型两种检测公式的闭式CRB表达式；对单天线FAS利用随机矩阵理论获得闭式相干CRB。

Result: CRB分析为评估不同架构和检测方案的活动检测提供了可处理且信息丰富的基准；FAS能够以显著降低的复杂度实现强大的空间分集增益。

Conclusion: 统一的CRB框架有效表征了FAS和FPA系统中活动检测的基本性能极限，为系统设计和性能评估提供了理论工具，并证实了FAS在空间分集方面的优势。
...
### [128] [Robust Composite DNA Storage under Sampling Randomness, Substitution, and Insertion-Deletion Errors](https://arxiv.org/abs/2602.11951)
*Busra Tegin,Tolga M Duman*

Main category: cs.IT

TL;DR: 本文提出了一种基于复合DNA存储的通信框架，将复合DNA建模为多项式信道，利用三维概率单纯形表示复合字母，通过推导转移概率和LLR来应对采样随机性误差，并扩展到替换和插入-删除信道，使用现有LDPC码实现可靠性能。


<details>
  <summary>Details</summary>
Motivation: DNA数据存储作为高密度、长期存储方案面临数字数据指数增长的挑战。复合DNA通过利用核苷酸混合物扩展标准四碱基的存储容量，但面临采样随机性等误差问题，现有方案在采样随机性下性能显著下降。

Method: 将复合DNA存储建模为多项式信道，类比数字调制在三维概率单纯形上表示复合字母。推导每个星座点的转移概率和LLR来缓解采样随机性误差，使用实用信道码进行纠错。扩展到替换和插入-删除信道，提出考虑这些额外损伤的星座更新规则。

Result: 数值结果表明，该方法使用现有LDPC码实现了可靠性能，相比之前针对有限幅度概率误差设计的方案，在采样随机性下性能显著提升。

Conclusion: 提出的复合DNA存储框架能够有效应对采样随机性、替换和插入-删除等信道损伤，使用现有纠错码实现可靠存储性能，为复合DNA存储系统的实际应用提供了可行方案。
...
### [129] [Achievability Bounds of Coding with Finite Blocklength for Gaussian Broadcast Channels](https://arxiv.org/abs/2602.11986)
*Ayşe Ünsal,Jean-Marie Gorce*

Main category: cs.IT

TL;DR: 本文研究高斯广播信道有限块长下的脏纸编码性能，提出了两种可达性界：广播版的依赖测试界和基于κβ的码率下界


<details>
  <summary>Details</summary>
Motivation: 研究有限块长下高斯广播信道中脏纸编码的实际性能界限，为实际通信系统设计提供理论指导

Method: 1. 提出广播信道版的依赖测试界，这是对Polyanskiy等人2010年工作的扩展；2. 引入基于κβ的脏纸编码最大码率下界

Result: 得到了两种不同的可达性界：一个是基于平均错误概率上界的依赖测试界，另一个是基于最大码率下界的κβ方法

Conclusion: 为高斯广播信道有限块长下的脏纸编码性能分析提供了新的理论工具和界限，有助于实际系统设计
...
### [130] [Taming Subpacketization without Sacrificing Communication: A Packet Type-based Framework for D2D Coded Caching](https://arxiv.org/abs/2602.12220)
*Xiang Zhang,Giuseppe Caire,Mingyue Ji*

Main category: cs.IT

TL;DR: 本文提出了一种基于分组类型的设计框架，用于设计具有最小分组化水平的D2D编码缓存方案，在保持最优通信速率的同时显著降低分组化复杂度。


<details>
  <summary>Details</summary>
Motivation: 有限长度分析对于编码缓存的实际部署至关重要。当前D2D编码缓存方案面临分组化水平过高的瓶颈问题，这限制了其在有限长度设置中的实际应用。需要设计既能保持最优通信速率又能最小化分组化水平的方案。

Method: 提出基于分组类型的设计框架：1）通过用户分组策略性地引入文件分割的不对称性；2）在缓存放置和多播传输中系统性地利用这种不对称性来创造分组化减少机会。该框架产生两种基本的分组化减少增益：子文件节省增益（通过精心设计的用户分组和发送器选择消除某些类型的子文件）和进一步分割节省增益（通过减少剩余子文件的分割粒度）。将缓存方案设计表述为整数线性规划问题。

Result: 提出的PT框架相对于原始的Ji-Caire-Molisch缓存方案以及各种最先进方案，在保持最优通信速率的同时实现了整体分组化改进。整数线性规划方法能够产生有效的速率最优D2D编码缓存方案，其分组化水平相对于JCM基准有所降低。

Conclusion: 基于分组类型的设计框架为有限长度设置下的D2D编码缓存方案设计提供了系统方法，通过引入和利用不对称性实现了分组化水平的显著降低，同时保持了最优通信速率，有助于推动编码缓存技术向实际部署迈进。
...
### [131] [Explaining AI Without Code: A User Study on Explainable AI](https://arxiv.org/abs/2602.11159)
*Natalia Abarca,Andrés Carvallo,Claudia López Moncada,Felipe Bravo-Marquez*

Main category: cs.AI

TL;DR: 在DashAI无代码机器学习平台中集成PDP、PFI和KernelSHAP三种可解释性技术，通过用户研究发现该模块对新手友好且能提升信任度，但需平衡新手可访问性和专家对完整性的要求。


<details>
  <summary>Details</summary>
Motivation: 机器学习在敏感领域的应用日益增多，但现有可解释AI方法通常需要技术专业知识，限制了其在无代码ML平台中的价值。无代码平台旨在民主化AI，却很少包含可解释性功能，这在使用ML进行关键决策时尤其重要。

Method: 在开源无代码ML平台DashAI中开发了一个以人为中心的可解释AI模块，集成了三种互补技术：部分依赖图(PDP)、排列特征重要性(PFI)和KernelSHAP。通过用户研究(N=20，包括ML新手和专家)评估可用性和解释效果。

Result: 1) 所有可解释性任务的成功率≥80%；2) 新手认为解释有用、准确且可信(ESS Cronbach's α=0.74)，专家对充分性和完整性要求更高；3) 解释提高了感知可预测性和自动化信任度(TiA α=0.60)，新手比专家表现出更高信任度。

Conclusion: 研究揭示了无代码ML中可解释AI的核心挑战：需要使解释既对新手可访问，又对专家足够详细。DashAI的集成方法为平衡这一需求提供了可行方案，但需进一步优化以满足不同用户群体的期望。
...
### [132] [On Decision-Valued Maps and Representational Dependence](https://arxiv.org/abs/2602.11295)
*Gil Raitses*

Main category: cs.AI

TL;DR: 论文提出决策值映射概念和DecisionDB基础设施，用于记录、重放和审计不同数据表示对计算引擎结果的影响，确保决策标识符的确定性恢复和验证。


<details>
  <summary>Details</summary>
Motivation: 相同数据的不同表示形式可能导致计算引擎产生不同的离散结果，需要系统化地记录哪些表示形式能保持结果一致、哪些会改变结果，以支持决策的可重现性和可审计性。

Method: 提出决策值映射概念，将表示族中的每个成员与其产生的离散结果关联；开发DecisionDB基础设施，使用基于内容和工件的标识符，以只写形式存储，支持确定性重放和审计。

Result: 系统能够精确恢复每个记录的决策标识符，所有三个标识字段与其持久化值匹配；将表示空间划分为持久性区域和边界，并将决策重用作为机械可检查的条件。

Conclusion: 该框架为理解数据表示对计算结果的影响提供了形式化基础，通过DecisionDB实现了决策的可重现性、可审计性和可验证性，为系统化处理表示敏感性提供了实用工具。
...
### [133] [Voxtral Realtime](https://arxiv.org/abs/2602.11298)
...
TL;DR: Voxtral Realtime是一个原生流式自动语音识别模型，在亚秒级延迟下达到离线转录质量，性能与Whisper相当，采用Apache 2.0开源协议


<details>
  <summary>Details</summary>
Motivation: 现有流式语音识别方法通常通过分块或滑动窗口来适配离线模型，存在延迟和质量折衷的问题。需要开发原生流式模型，在保持低延迟的同时达到离线转录质量

Method: 基于延迟流建模框架，引入新的因果音频编码器和Ada RMS-Norm改进延迟条件处理，采用端到端流式训练，音频和文本流之间有显式对齐，在13种语言的大规模数据集上进行预训练

Result: 在480ms延迟下，Voxtral Realtime的性能与最广泛部署的离线转录系统Whisper相当，实现了亚秒级延迟下的离线质量转录

Conclusion: Voxtral Realtime展示了原生流式语音识别模型可以在保持低延迟的同时达到离线转录质量，为实时语音识别应用提供了高质量解决方案，模型已开源
...
### [134] [Dissecting Subjectivity and the "Ground Truth" Illusion in Data Annotation](https://arxiv.org/abs/2602.11318)
*Sheza Munir,Benjamin Mah,Krisha Kalsi,Shivani Kapania,Julian Posada,Edith Law,Ding Wang,Syed Ishtiaque Ahmed*

Main category: cs.AI

TL;DR: 论文批判机器学习中"ground truth"范式的实证主义谬误，认为它将人类分歧视为技术噪声而非重要的社会技术信号，通过系统性文献综述揭示了数据标注实践中的"共识陷阱"问题。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习领域普遍依赖"ground truth"范式，但这种范式基于实证主义谬误，将人类分歧视为技术噪声而非有价值的社会技术信号。研究者希望揭示数据标注实践中导致"共识陷阱"的机制，并探索如何将分歧重新定位为构建文化能力模型的重要信号。

Method: 采用系统性文献综述方法，分析2020-2025年间在ACL、AIES、CHI、CSCW、EAAMO、FAccT和NeurIPS七个顶级会议发表的论文。通过分层关键词过滤方案从30,897条记录中筛选出3,042条高召回率文献，最终纳入346篇论文进行定性综合。使用反思性主题分析方法。

Result: 研究发现：1) 系统性位置可读性失败与人类作为验证者模型的架构转变相结合，特别是依赖模型介导的标注，引入了根深蒂固的锚定偏见，实际上将人类声音排除在循环之外；2) 地理霸权将西方规范强加为普遍基准，通常通过不稳定数据工作者的表演性对齐来强制执行，他们优先考虑请求者合规而非诚实的主观性以避免经济惩罚；3) 统计模型将文化多元性误诊为随机误差的"噪声传感器"谬误。

Conclusion: 论文主张将分歧重新定位为构建文化能力模型的高保真信号，并提出多元标注基础设施的路线图，将目标从发现单一的"正确"答案转向映射人类经验的多样性。
...
### [135] [Bi-Level Prompt Optimization for Multimodal LLM-as-a-Judge](https://arxiv.org/abs/2602.11340)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM作为自动评估器已被广泛采用，但将其评估结果与人类判断对齐仍然具有挑战性。现有的自动提示优化方法主要针对纯文本评估，在多模态环境下研究不足，特别是评估AI生成图像时，多模态模型因上下文窗口限制只能处理有限视觉示例，阻碍了有效的试错式提示优化。
...
Conclusion: BLPO框架成功解决了多模态LLM评估中因上下文窗口限制导致的提示优化瓶颈，通过图像到文本转换和双层优化实现了更高效的多模态评估提示优化。
...
### [136] [AgentNoiseBench: Benchmarking Robustness of Tool-Using LLM Agents Under Noisy Condition](https://arxiv.org/abs/2602.11348)
...
TL;DR: 论文提出了AgentNoiseBench框架，用于系统评估智能体模型在噪声环境下的鲁棒性，通过分析真实世界噪声类型并开发自动化噪声注入管道，发现当前智能体模型对现实环境扰动高度敏感。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在基准测试中表现良好，但在实际部署中性能下降，主要原因是现有训练和评估范式基于理想化假设，忽略了真实世界交互中的随机性和噪声。

Method: 1. 深入分析真实场景中的偏差和不确定性，将环境噪声分为用户噪声和工具噪声两类；2. 开发自动化管道，在现有智能体基准测试中注入可控噪声，同时保持任务可解性；3. 使用该管道对多种架构和参数规模的模型进行广泛评估。

Result: 评估结果显示，在不同噪声条件下，智能体模型性能存在一致的变化，突显出当前智能体模型对现实环境扰动的高度敏感性。

Conclusion: AgentNoiseBench框架能够系统评估智能体模型的鲁棒性，揭示了现有模型在噪声环境下的脆弱性，为开发更稳健的智能体系统提供了重要参考。
...
### [137] [Causal-JEPA: Learning World Models through Object-Level Latent Interventions](https://arxiv.org/abs/2602.11389)
...
Method: 将掩码联合嵌入预测从图像块扩展到对象中心表示，通过对象级掩码要求从其他对象推断目标对象状态，诱导潜在干预并防止捷径解

Result: 在视觉问答中实现约20%的反事实推理提升；在智能体控制任务中，仅需1%的潜在输入特征即可达到与基于图像块模型相当的性能

Conclusion: C-JEPA通过对象级掩码引入因果归纳偏置，有效增强世界模型的关系推理能力，为预测、推理和控制任务提供更强大的基础
...
### [138] [GHOST: Unmasking Phantom States in Mamba2 via Grouped Hidden-state Output-aware Selection & Truncation](https://arxiv.org/abs/2602.11408)
*Michael Menezes,Anastasios Kyrillidis*

Main category: cs.AI

TL;DR: GHOST：一种用于Mamba2模型的结构化剪枝框架，通过前向统计近似控制理论平衡截断，在减少50%状态维度时仅增加约1个困惑度点


<details>
  <summary>Details</summary>
Motivation: Mamba2模型扩展的状态维度虽然增强了时序建模能力，但在自回归生成过程中会产生显著的推理开销，导致带宽饱和。传统的剪枝方法无法有效解决这一瓶颈问题

Method: GHOST（分组隐藏状态输出感知选择和截断）框架，通过仅使用前向传递统计量来近似控制理论中的平衡截断方法，联合测量可控性和可观测性

Result: 在130M到2.7B参数规模的模型上，该方法实现了50%的状态维度减少，在WikiText-2数据集上仅增加约1个困惑度点

Conclusion: GHOST框架能够在不需反向传播的情况下，达到与基于梯度方法相当的保真度，有效解决了Mamba2模型推理时的带宽瓶颈问题
...
### [139] [ChaosBench-Logic: A Benchmark for Logical and Symbolic Reasoning on Chaotic Dynamical Systems](https://arxiv.org/abs/2601.01982)
*Noel Thomas*

Main category: cs.AI

TL;DR: ChaosBench-Logic是一个评估大语言模型在混沌动力系统领域逻辑推理能力的基准测试，包含30个系统、11个语义谓词和621个问题，涵盖7种推理类型。前沿LLMs在单项准确率上达到91-94%，但在组合推理和全局一致性上表现脆弱。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言任务上表现出色，但在需要精确逻辑和符号推理的领域仍然脆弱。混沌动力系统提供了一个特别苛刻的测试环境，因为混沌是确定性的，却常被误解为随机性或复杂性。需要建立一个统一的基准来评估LLMs在这类科学推理任务上的能力。

Method: 引入ChaosBench-Logic基准测试，使用统一的一阶逻辑本体论评估30个不同的动力系统。每个系统标注了11个语义谓词的真值分配，生成了621个问题，涵盖7个推理类别：多跳蕴含、跨系统类比、反事实推理、偏见探测、多轮对话等。定义了逻辑准确性、蕴含一致性、对话连贯性和矛盾性等指标，并发布了开源评估管道。

Result: 前沿LLMs（GPT-4、Claude 3.5 Sonnet、Gemini 2.5 Flash、LLaMA-3 70B）在单项准确率上达到91-94%，但在组合推理项目上得分为0%，表现出脆弱的全局一致性。对话级准确率从53.1%（GPT-4 CoT）到75.5%（LLaMA-3 zero-shot）不等。

Conclusion: ChaosBench-Logic为诊断LLMs在逻辑推理方面的失败提供了一个严格的测试平台，并为开发神经符号方法以改进LLMs的科学推理能力奠定了基础。虽然LLMs在单项任务上表现良好，但在组合推理和全局一致性方面仍有显著缺陷。
...
TL;DR: 提出了一种基于强化学习和T5语言模型的新型无损压缩方法，将数据压缩为token序列而非传统向量表示，显著提高了压缩比


<details>
  <summary>Details</summary>
Motivation: 传统压缩方法难以充分利用复杂数据结构中的冗余信息，而现有深度学习方法通常使用密集向量表示，掩盖了底层token结构，需要一种能更好保留数据原始格式的压缩方法

Method: 使用强化学习训练T5语言模型架构，将数据压缩为token序列而非向量表示，采用离策略强化学习算法优化序列长度以减少冗余

Result: 相比传统方法显著提高了压缩比，同时保持了语义完整性，无需依赖外部语法或世界知识

Conclusion: 该方法通过利用语言模型的潜在信息，在不需显式内容理解的情况下有效压缩数据，为各种应用提供了更强大实用的压缩解决方案
...
### [141] [Distributionally Robust Cooperative Multi-Agent Reinforcement Learning via Robust Value Factorization](https://arxiv.org/abs/2602.11437)
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体强化学习中的集中训练分散执行方法在现实环境中不可靠，因为存在模拟到现实的差距、模型不匹配和系统噪声等环境不确定性。

Method: 提出分布鲁棒的IGM原则(DrIGM)，定义了鲁棒个体动作价值函数，确保在不确定性下仍能保持个体与全局最优的一致性。基于此原则开发了与VDN/QMIX/QTRAN等现有价值分解架构兼容的鲁棒变体，使用鲁棒Q目标进行训练。

Result: 在SustainGym高保真模拟器和星际争霸游戏环境中，该方法显著提高了分布外性能表现，且保持了可扩展性，无需为每个智能体定制奖励塑造。
...
### [142] [Credit Where It is Due: Cross-Modality Connectivity Drives Precise Reinforcement Learning for MLLM Reasoning](https://arxiv.org/abs/2602.11455)
*Zhengbo Jiao,Shaobo Wang,Zifan Zhang,Wei Wang,Bing Zhao,Hu Wei,Linfeng Zhang*

Main category: cs.AI

TL;DR: 本文研究了多模态强化学习与可验证奖励中视觉证据的整合机制，发现只有约15%的token具有强视觉-文本耦合，这些高连接性token作为锚点将推理过程锚定在图像上。基于此提出了Anchor-Token Reinforcement Learning框架，通过选择性强化高连接性token，在仅增加1.2%开销的情况下使32B模型在MathVista上超越了72B-Instruct基线。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习与可验证奖励显著提升了多模态大语言模型的推理能力，但视觉证据在推理过程中如何被整合仍然缺乏深入理解。研究者希望探索多模态RLVR中跨模态注意力的连接模式，揭示视觉信息整合的机制。

Method: 通过跨模态注意力连接性分析发现只有约15%的token具有强视觉-文本耦合。基于此提出Anchor-Token Reinforcement Learning框架，利用基于图的注意力拓扑聚类方法，选择性强化高连接性token。该方法在系列模型（3B-32B）上进行评估。

Result: AT-RL仅引入1.2%的开销，但使32B模型在MathVista上达到80.2分，超越了72B-Instruct基线。在STEM、视频和通用任务上均观察到一致的性能提升。相反，仅训练低连接性token会导致严重性能下降，证实有效的多模态RL依赖于对视觉锚点的精确信用分配。

Conclusion: 推理质量不是由token数量决定的，而是由跨模态锚定的保真度决定的。高连接性token作为视觉锚点在多模态推理中起着关键作用，选择性强化这些锚点可以显著提升模型性能而仅需少量额外开销。
...
### [143] [AgentLeak: A Full-Stack Benchmark for Privacy Leakage in Multi-Agent LLM Systems](https://arxiv.org/abs/2602.11510)
*Faouzi El Yagoubi,Ranwa Al Mallah,Godwin Badu-Marfo*

Main category: cs.AI

TL;DR: AgentLeak是首个针对多智能体LLM系统隐私泄露的全面基准测试，覆盖内部通信通道，发现输出审计会遗漏41.7%的隐私违规。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试无法衡量多智能体LLM系统的隐私风险。当智能体协同完成任务时，敏感数据会通过智能体间消息、共享内存和工具参数等内部通道传递，这些通道是仅关注输出的审计无法检测的。

Method: 引入AgentLeak基准测试，覆盖内部通道，包含医疗、金融、法律和企业领域的1000个场景，配备32类攻击分类法和三层检测流水线。测试了GPT-4o、GPT-4o-mini、Claude 3.5 Sonnet、Mistral Large和Llama 3.3 70B等模型，分析了4979条跟踪记录。

Result: 多智能体配置降低了单通道输出泄露率（C1：27.2% vs 单智能体43.2%），但未监控的内部通道使总系统暴露率达到68.9%。智能体间消息（C2）泄露率为68.8%，远高于输出通道的27.2%。Claude 3.5 Sonnet在外部（3.3%）和内部（28.1%）通道上都表现出最低泄露率。所有模型和领域都呈现C2 > C1的模式。

Conclusion: 多智能体LLM系统中智能体间通信是主要漏洞，输出审计会遗漏大量隐私违规。需要开发包含内部通道隐私保护的协调框架，并在智能体间通信中强制执行隐私控制。
...
### [144] [Human-Inspired Continuous Learning of Internal Reasoning Processes: Learning How to Think for Adaptive AI Systems](https://arxiv.org/abs/2602.11516)
*Hong Su*

Main category: cs.AI

TL;DR: 提出了一种受人类启发的持续学习框架，将推理、行动、反思和验证统一在并行学习增强的顺序推理模型中，将内部思维过程作为主要学习对象，实现认知结构在执行过程中改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注学习任务特定输出或静态知识表示，忽视了内部推理结构、行动调度策略和学习机制本身的持续优化。为了开发能够在动态现实环境中持续适应的AI系统，需要学习内部推理过程。

Method: 提出统一推理、行动、反思和验证的持续学习框架，将内部思维过程作为主要学习对象，系统记录内部推理轨迹和环境交互作为结构化学习材料，支持用学习过程替换预定义逻辑，引入分层学习机制共同适应任务级参数和学习策略。

Result: 在温度传感器异常检测任务上的实验结果表明，引入内部过程学习使平均运行时间减少了23.9%，系统能够在保持操作稳定性的同时逐步演化其内部认知架构。

Conclusion: 该框架通过将内部推理过程作为主要学习对象，实现了认知结构在执行过程中的持续改进，为开发能够在动态环境中持续适应的AI系统提供了有效方法。
...
### [145] [CausalAgent: A Conversational Multi-Agent System for End-to-End Causal Inference](https://arxiv.org/abs/2602.11527)
*Jiawei Zhu,Wei Chen,Ruichu Cai*

Main category: cs.AI

TL;DR: CausalAgent是一个基于对话式多智能体系统的端到端因果推理平台，通过自然语言交互自动化完成从数据清洗到报告生成的整个因果分析流程。


<details>
  <summary>Details</summary>
Motivation: 传统因果分析工作流程存在显著技术壁垒，要求研究人员同时具备统计学和计算机科学背景，需要手动选择算法、处理数据质量问题并解释复杂结果，这限制了因果推理在医疗、经济和社会科学等领域的应用。
...
Conclusion: CausalAgent作为一种新颖的用户中心化人机协作范式，通过显式建模分析工作流程，为因果推理提供了易用且严谨的解决方案，有望推动因果分析在多个领域的广泛应用。
...
### [146] [Budget-Constrained Agentic Large Language Models: Intention-Based Planning for Costly Tool Use](https://arxiv.org/abs/2602.11541)
*Hanbing Liu,Chunhao Tian,Nan An,Ziyuan Wang,Pinyan Lu,Changyuan Yu,Qi Qi*

Main category: cs.AI

TL;DR: INTENT框架：预算约束下工具增强智能体的推理时规划方法，通过意图感知分层世界模型预测工具使用和风险校准成本，在严格预算限制下显著提升任务成功率


<details>
  <summary>Details</summary>
Motivation: 研究预算约束下的工具增强智能体问题，大型语言模型需要在严格货币预算下通过调用外部工具解决多步任务。由于状态-动作空间巨大、结果方差高、探索成本昂贵，直接规划变得不可行

Method: 提出INTENT推理时规划框架，利用意图感知分层世界模型来预测未来工具使用、风险校准成本，并在线指导决策。将问题形式化为上下文空间中的序列决策问题，包含定价和随机工具执行

Result: 在成本增强的StableToolBench上，INTENT严格强制执行硬预算可行性，同时显著提高任务成功率，在工具价格变化和预算变化等动态市场变化下保持鲁棒性

Conclusion: INTENT框架有效解决了预算约束下工具增强智能体的规划挑战，通过推理时规划和意图感知建模，在严格预算限制下实现了更好的任务性能
...
### [147] [The Five Ws of Multi-Agent Communication: Who Talks to Whom, When, What, and Why -- A Survey from MARL to Emergent Language and LLMs](https://arxiv.org/abs/2602.11583)
*Jingdi Chen,Hanqing Yang,Zongjun Liu,Carlee Joe-Wong*

Main category: cs.AI

TL;DR: 本文系统综述了多智能体通信研究的演进脉络，从多智能体强化学习到涌现语言，再到基于大语言模型的系统，通过"五个W"框架分析通信设计的关键要素和挑战。


<details>
  <summary>Details</summary>
Motivation: 在动态、部分可观测环境中，通信是减少不确定性、实现协作的关键。现有研究分散在不同范式，缺乏统一的分析框架来理解通信设计的核心要素和演进趋势。

Method: 采用"五个W"分析框架：谁与谁通信、通信内容、通信时机、通信目的。系统梳理三大研究范式：多智能体强化学习（从手工设计到端到端学习）、涌现语言（结构化符号通信）、基于大语言模型的系统（自然语言先验）。

Result: 揭示了不同通信设计选择的影响和权衡：MARL方法任务特定且难以解释，EL方法存在基础、泛化和可扩展性问题，LLM方法在开放环境中具有优势但面临新挑战。提出了实用的设计模式和未解决问题。

Conclusion: 未来需要结合学习、语言和控制的混合系统，以实现可扩展、可解释的多智能体协作。通信设计需要在任务性能、可解释性、泛化能力和计算效率之间取得平衡。
...
### [148] [MAPLE: Modality-Aware Post-training and Learning Ecosystem](https://arxiv.org/abs/2602.11596)
...
Method: MAPLE包含三个核心组件：1) MAPLE-bench基准，首次明确标注每个任务所需的最小信号组合；2) MAPO模态感知策略优化框架，按模态需求对批次进行分层以减少异质组优势带来的梯度方差；3) 自适应加权和课程调度，平衡并优先处理更难的信号组合。系统分析了损失聚合、裁剪、采样和课程设计，确立了MAPO的最优训练策略。

Result: MAPLE将单模态/多模态准确率差距缩小了30.24%，收敛速度提高了3.18倍，并在现实减少信号访问的所有模态组合下保持稳定性。自适应加权和课程聚焦学习进一步提升了跨信号组合的性能。

Conclusion: MAPLE构成了一个完整的部署就绪多模态RL后训练方案，通过模态感知训练解决了现有方法的局限性，显著提升了训练效率、性能和鲁棒性。
...
### [149] [scPilot: Large Language Model Reasoning Toward Automated Single-Cell Analysis and Discovery](https://arxiv.org/abs/2602.11609)
...
<details>
  <summary>Details</summary>
Motivation: 当前单细胞数据分析需要专业知识且缺乏透明度，研究者需要一种能够让LLM直接与原始组学数据交互的框架，实现可审计、可解释的诊断性分析。

Method: scPilot将细胞类型注释、发育轨迹重建和转录因子靶向等核心单细胞分析转化为逐步推理问题，LLM必须解决、论证并在需要时用新证据修订。同时发布了scBench评估套件，包含9个专家策划的数据集和评分器。

Result: 实验显示，迭代组学原生推理使细胞类型注释的平均准确率提升11%，Gemini-2.5-Pro将轨迹图编辑距离减少30%（相比单次提示），同时生成透明的推理轨迹来解释标记基因模糊性和调控逻辑。
...
### [150] [When Agents Disagree With Themselves: Measuring Behavioral Consistency in LLM-Based Agents](https://arxiv.org/abs/2602.11619)
*Aman Mehta*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLM智能体在相同任务上的行为一致性，以及这种一致性如何影响任务成功率。目前缺乏对智能体行为稳定性的系统研究。

Method: 方法是在HotpotQA数据集上运行3,000次智能体实验，使用三种模型（Llama 3.1 70B、GPT-4o和Claude Sonnet 4.5），采用ReAct风格的智能体架构，分析相同输入下智能体行为序列的差异。

Result: 结果显示：1）智能体行为存在显著不一致性，平均每10次运行产生2.0-4.2个不同的动作序列；2）行为不一致性可预测失败：行为一致的任务（≤2个唯一路径）准确率达80-92%，而高度不一致的任务（≥6个唯一路径）准确率仅为25-60%；3）69%的行为差异发生在第二步（第一个搜索查询）。

Conclusion: 结论是LLM智能体的行为不一致性与任务失败率密切相关，早期决策（特别是第二步）是行为分化的关键点。监控行为一致性可以在执行过程中实现早期错误检测，从而提高智能体可靠性。
...
### [151] [Neuro-Symbolic Multitasking: A Unified Framework for Discovering Generalizable Solutions to PDE Families](https://arxiv.org/abs/2602.11630)
*Yipeng Huang,Dejun Xu,Zexin Lin,Zhenzhong Wang,Min Jiang*

Main category: cs.AI

TL;DR: 提出NMIPS框架，通过神经辅助多任务符号求解器同时发现PDE家族的解析解，使用仿射转移方法在PDE间共享数学结构，提高计算效率。
...
Method: 提出神经辅助多任务符号PDE求解器框架NMIPS，采用多因子优化同时发现PDE的解析解，设计仿射转移方法在PDE家族间转移学习的数学结构，避免从零开始求解每个PDE。

Result: 在多个案例实验中，相比现有基线方法，NMIPS实现了约35.7%的准确率提升，同时提供可解释的解析解。

Conclusion: NMIPS框架成功解决了PDE家族求解中的计算效率和可解释性问题，通过同时发现解析解和共享数学结构，在保持计算效率的同时提供了科学洞察所需的解析表达式。
...
### [152] [Do MLLMs Really Understand Space? A Mathematical Reasoning Evaluation](https://arxiv.org/abs/2602.11635)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在感知任务上表现出色，但其数学空间推理能力（解析和操作二维、三维关系）仍不清楚。研究发现人类在教科书式空间推理问题上能达到95%以上准确率，而大多数领先的MLLMs甚至达不到60%，这突显了空间推理是当前模型的基本弱点。
...
### [153] [Quark Medical Alignment: A Holistic Multi-Dimensional Alignment and Collaborative Optimization Paradigm](https://arxiv.org/abs/2602.11661)
...
TL;DR: 提出了一种针对医疗问答领域的强化学习对齐新范式，通过构建多维医疗对齐矩阵和统一优化机制，解决传统RLHF在医疗领域存在的成本高、验证难、多目标冲突等问题。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习对齐方法在医疗问答领域存在根本性不匹配：RLHF依赖昂贵且难以反映医疗事实绝对正确性的偏好标注；RLVR缺乏有效自动验证器且难以处理复杂临床情境；医疗对齐需要同时优化正确性、安全性和合规性，但多目标异质奖励信号易产生尺度不匹配和优化冲突。

Method: 1. 构建整体多维医疗对齐矩阵，将对齐目标分解为四个类别：基础能力、专家知识、在线反馈和格式规范；2. 在每个类别中建立可观察指标→可归因诊断→可优化奖励的闭环，提供细粒度监督信号；3. 提出统一优化机制：采用参考冻结归一化对齐奖励尺度，实施三因子自适应动态加权策略实现弱点导向、风险优先、冗余减少的协同优化。

Result: 实验结果表明，在真实医疗场景评估中，所提范式有效，为垂直领域的复杂对齐建立了新范式。

Conclusion: 该研究提出了一种稳健的医疗对齐范式，通过系统性的多维度分解和统一的优化机制，解决了医疗领域强化学习对齐的特殊挑战，为垂直领域的复杂对齐问题提供了新思路。
...
### [154] [Benchmark Health Index: A Systematic Framework for Benchmarking the Benchmarks of LLMs](https://arxiv.org/abs/2602.11674)
*Longyuan Zhu,Hairan Hua,Linlin Miao,Bing Zhao*

Main category: cs.AI

TL;DR: 该论文提出了基准健康指数（BHI），这是一个纯数据驱动的框架，用于从三个正交维度审计评估基准：能力区分度、抗饱和度和影响力，以解决当前LLM基准测试不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型（LLMs）快速发展，但用于衡量进展的基准测试变得越来越不可靠。分数膨胀和选择性报告削弱了标准基准的权威性，导致社区不确定哪些评估结果仍然可信。

Method: 提出基准健康指数（BHI）框架，从三个互补维度审计评估基准：1) 能力区分度 - 衡量基准区分模型性能的能力；2) 抗饱和度 - 估计基准在达到天花板效应前的剩余空间；3) 影响力 - 量化基准在学术和工业生态系统中的采用广度和实践塑造能力。从91个代表性模型的106个验证基准中提取数据进行分析。

Result: BHI是首个在宏观层面量化基准健康度的框架，为基准选择提供了原则性基础，并支持下一代评估协议的动态生命周期管理。

Conclusion: BHI框架能够系统性地表征评估格局，解决当前基准测试不可靠的问题，为基准选择和评估协议的动态管理提供数据驱动的决策支持。
...
### [155] [Right for the Wrong Reasons: Epistemic Regret Minimization for Causal Rung Collapse in LLMs](https://arxiv.org/abs/2602.11675)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: 论文提出Rung Collapse理论，解释自回归训练无法区分关联P(Y|X)与干预P(Y|do(X))，导致模型"正确但理由错误"；提出Epistemic Regret Minimization(ERM)作为因果信念修正算子，防止Aleatoric Entrenchment现象。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统常通过"捷径"获得高性能，但在分布变化下崩溃。这种病理现象有精确的因果起源：自回归训练无法提供梯度信号来区分关联与干预，导致Rung Collapse。当基于结果的学习通过错误的因果模型强化正确答案时，智能体会陷入有缺陷的推理，形成Aleatoric Entrenchment。

Method: 提出Epistemic Regret Minimization(ERM)作为信念修正目标，独立于任务成功惩罚因果推理错误。构建三层架构：(1)物理基础定理证明满足执行器独立的动作实现有效的do操作；(2)ERM作为满足AGM公设的因果信念修正算子；(3)故障模式分类法对重复推理错误分类并注入领域无关防护。

Result: 在1,360个因果陷阱场景和6个前沿LLM上的实验显示：Rung Collapse在推理增强模型中持续存在(GPT-5.2为3.7%)；可操纵性呈现逆缩放现象，高级模型抵抗通用修正；针对性ERM反馈在结果级反馈失败时恢复了53-59%的顽固错误。

Conclusion: 论文形式化了Rung Collapse作为自回归训练的根本限制，提出ERM作为解决方案，证明能渐进恢复真实干预分布并给出有限样本边界。该方法通过因果信念修正防止智能体因错误理由而成功时陷入错误推理。
...
### [156] [Beyond Pixels: Vector-to-Graph Transformation for Reliable Schematic Auditing](https://arxiv.org/abs/2602.11678)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解方面虽有进展，但在处理工程示意图时存在"结构盲区"——无法捕捉拓扑结构和符号逻辑，因为其像素驱动范式丢弃了推理所需的显式向量定义关系。

Method: 提出Vector-to-Graph（V2G）管道，将CAD图表转换为属性图，其中节点表示组件，边编码连接性，使结构依赖关系变得显式且机器可审计。

Result: 在电气合规性检查的诊断基准测试中，V2G在所有错误类别上都获得了显著的准确率提升，而领先的多模态大语言模型仍接近随机水平。

Conclusion: 像素基方法的系统性不足凸显了结构感知表示的重要性，这为多模态AI在工程领域的实际部署提供了可靠路径。作者发布了基准测试和实现以促进进一步研究。
...
### [157] [ThinkRouter: Efficient Reasoning via Routing Thinking between Latent and Discrete Spaces](https://arxiv.org/abs/2602.11683)
...
<details>
  <summary>Details</summary>
Motivation: 研究发现隐式推理在不同场景下效果不一，分析发现错误答案的推理轨迹包含更少的低置信度步骤，而多个低置信度思考替代方案的软嵌入聚合可能引入噪声，导致对不可靠推理轨迹的高置信度。

Method: 提出ThinkRouter机制，基于模型置信度动态路由：当模型置信度低时，路由到离散token空间进行显式推理；当置信度高时，路由到潜在空间进行隐式推理。

Result: 在STEM推理和编程基准测试中，ThinkRouter在准确率上优于显式CoT、随机路由和隐式推理基线，Pass@1平均提升19.70分，同时减少生成长度达15.55%。

Conclusion: ThinkRouter能够校准显式CoT和隐式推理产生的错误，并通过全局降低模型置信度加速思考结束token的生成，实现更高效准确的推理。
...
### [158] [Beyond Parameter Arithmetic: Sparse Complementary Fusion for Distribution-Aware Model Merging](https://arxiv.org/abs/2602.11717)
*Weihong Lin,Lin Sun,Qilong Shi,Aomufei Yuan,Yuxuan Tian,Zhengyang Wang,Guangxiang Zhao,Xiangzheng Zhang,Tong Yang*

Main category: cs.AI

TL;DR: SCF-RKL是一种新颖的模型融合框架，通过稀疏、分布感知的更新来显式控制功能干扰，使用反向KL散度衡量模型间功能差异并选择性融合互补参数，在保持稳定表示的同时整合新能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要依赖参数空间的启发式方法，通常会引入严重的干扰，导致泛化能力下降和不稳定的生成行为（如重复和不连贯输出）。需要一种能够显式控制功能干扰的融合方法。

Method: 提出SCF-RKL框架，使用反向Kullback-Leibler散度衡量模型间的功能差异，采用稀疏、分布感知的更新方式，选择性融合互补参数，而不是假设参数空间的线性可加性。

Result: 在24个基准测试（涵盖高级推理、通用推理与知识、指令遵循、安全性和视觉分类）上的广泛实验表明，SCF-RKL在各种模型规模和架构上始终优于现有模型融合方法，同时保持强大的泛化能力和生成稳定性。

Conclusion: SCF-RKL通过显式控制功能干扰的稀疏互补融合方法，为模型融合提供了更有效的解决方案，能够在保持稳定表示的同时整合新能力，解决了现有方法中的干扰问题。
...
### [159] [Text2GQL-Bench: A Text to Graph Query Language Benchmark [Experiment, Analysis & Benchmark]](https://arxiv.org/abs/2602.11745)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图查询语言系统面临数据集领域覆盖有限、支持的图查询语言不足、评估范围狭窄等问题，缺乏高质量基准数据集和系统评估方法来比较不同图查询语言和领域中的模型能力。

Method: 提出了Text2GQL-Bench基准测试，包含多GQL数据集和可扩展的构建框架，能够生成不同领域、问题抽象层次和GQL的数据集。同时引入了综合评估方法，联合报告语法有效性、相似性、语义对齐和执行准确性。

Result: 评估揭示了ISO-GQL生成的显著方言差距：即使在零样本设置下，强LLM最多只能达到4%的执行准确性；固定3-shot提示可将准确性提升到约50%，但语法有效性仍低于70%。微调的8B开源模型达到45.1%执行准确性和90.8%语法有效性，表明大部分性能提升来自足够的ISO-GQL示例。

Conclusion: Text2GQL-Bench为文本到图查询语言系统提供了统一的基准测试和评估框架，揭示了当前模型在ISO-GQL生成中的局限性，并展示了通过示例学习和微调可以显著提升性能。
...
### [160] [AIR: Improving Agent Safety through Incident Response](https://arxiv.org/abs/2602.11749)
...
Method: AIR框架：1）定义用于自主管理LLM智能体系统事故响应生命周期的领域特定语言；2）将事故响应集成到智能体执行循环中，包括：通过基于当前环境状态和近期上下文的语义检查检测事故，指导智能体使用工具执行遏制和恢复操作，在根除阶段合成防护规则以阻止未来类似事故。

Result: 在三种代表性智能体类型上的评估显示：AIR实现了检测、修复和根除成功率均超过90%。实验还证实了AIR关键设计组件的必要性，展示了AIR的及时性和适度开销，并证明LLM生成的规则在不同领域可以接近开发者编写规则的效果。

Conclusion: 事故响应作为提高智能体安全的一等机制既是可行的也是必要的。AIR框架为LLM智能体系统提供了有效的事故响应能力，填补了当前安全机制的空白。
...
### [161] [TSR: Trajectory-Search Rollouts for Multi-Turn RL of LLM Agents](https://arxiv.org/abs/2602.11767)
*Aladin Djuhera,Swanand Ravindra Kadhe,Farhan Ahmed,Holger Boche*

Main category: cs.AI

TL;DR: TSR是一种训练时轨迹搜索方法，通过轻量级树搜索构建高质量轨迹，提升多轮强化学习中的探索效率，在多个任务上实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展推动使用强化学习训练多轮交互智能体，但多轮RL面临奖励稀疏/延迟和环境随机性的挑战，传统轨迹采样可能导致利用不足和模式崩溃。

Method: 提出TSR（轨迹搜索展开）方法，将测试时的搜索技术重新用于训练时的轨迹生成。通过轻量级树搜索（如best-of-N、beam search和浅层前瞻搜索）在每个回合选择高分动作构建高质量轨迹，保持优化目标不变且与优化器无关。

Result: 在Sokoban、FrozenLake和WebShop任务上，结合PPO和GRPO优化器，TSR实现了高达15%的性能提升和更稳定的学习过程，仅需一次性增加训练计算成本。

Conclusion: TSR通过将搜索从推理时间移至训练展开阶段，为多轮智能体学习提供了简单通用的增强机制，与现有框架和拒绝采样方法互补，能有效提升训练质量。
...
### [162] [How to Optimize Multispecies Set Predictions in Presence-Absence Modeling ?](https://arxiv.org/abs/2602.11771)
...
<details>
  <summary>Details</summary>
Motivation: 物种分布模型通常产生概率性出现预测，需要转换为二值存在-缺失图用于生态推断和保护规划。但传统的二值化方法是启发式的，会显著扭曲物种普遍性和群落组成的估计。
...
### [163] [RELATE: A Reinforcement Learning-Enhanced LLM Framework for Advertising Text Generation](https://arxiv.org/abs/2602.11780)
...
Method: 提出RELATE框架，通过强化学习将生成过程与下游指标对齐统一。将转化导向指标和合规约束建模为多维度奖励，通过策略学习直接集成到生成过程中。

Result: 在大规模工业数据集上的实验表明，RELATE持续优于基线方法。在实际广告平台部署中，在严格政策约束下实现了点击转化率的显著提升。

Conclusion: RELATE框架通过端到端的强化学习方法，有效解决了广告文本生成与性能指标对齐的分离问题，在实际应用中表现出鲁棒性和有效性。
...
### [164] [FlowMind: Execute-Summarize for Structured Workflow Generation from LLM Reasoning](https://arxiv.org/abs/2602.11782)
...
Method: 提出Execute-Summarize(ES)框架：1) 模型首先使用可用工具完成任务；2) 然后独立地从执行轨迹中重构结构化工作流。这种解耦设计避免了执行和构建过程的相互干扰。

Result: 在FlowBench基准测试中，该方法优于现有方法，提供了将自由形式的LLM推理落地为结构化工作流的可靠范式。

Conclusion: ES框架通过将任务执行与工作流构建分离，显著提高了工作流的准确性和鲁棒性，为LLM推理到结构化工作流的转化提供了有效解决方案。
...
### [165] [Beyond End-to-End Video Models: An LLM-Based Multi-Agent System for Educational Video Generation](https://arxiv.org/abs/2602.11790)
*Lingyong Yan,Jiulong Wu,Dong Xie,Weixian Shi,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: LAVES是一个基于LLM的多智能体分层系统，用于从教育问题生成高质量教学视频，通过结构化可执行脚本实现自动化生产，相比行业标准方法降低成本95%以上。
...
Method: LAVES将教学视频生成分解为由中央编排智能体协调的专门智能体工作流，包括解决方案智能体（严谨问题求解）、插图智能体（生成可执行可视化代码）和旁白智能体（学习者导向教学脚本），通过质量门控、迭代批判机制和结构化可执行视频脚本实现确定性编译。

Result: 系统实现每日超过100万视频的吞吐量，相比当前行业标准方法降低成本超过95%，同时保持高接受率，能够生成具有正确逐步推理、教学连贯性、语义忠实视觉演示和精确音视频对齐的高质量教学视频。

Conclusion: LAVES通过分层LLM多智能体系统和结构化可执行脚本方法，成功解决了教学视频生成中的逻辑严谨性、成本效益和可控性问题，为自动化教育内容生产提供了高效可扩展的解决方案。
...
### [166] [Detecting RLVR Training Data via Structural Convergence of Reasoning](https://arxiv.org/abs/2602.11792)
*Hongbo Zhang,Yue Yang,Jianhao Yan,Guangsheng Bao,Yue Zhang,Yue Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种名为Min-$k$NN Distance的黑盒检测方法，用于识别强化学习验证奖励（RLVR）训练过的模型中的训练数据污染问题，该方法通过分析生成文本的多样性来区分RLVR训练中见过和未见过的提示。


<details>
  <summary>Details</summary>
Motivation: RLVR训练方法使用自我生成的推理轨迹进行奖励反馈微调，这使得传统的基于似然度的检测方法失效。由于训练数据未公开，存在基准污染问题，需要新的检测方法来识别RLVR训练中使用的数据。

Method: 作者发现RLVR训练会产生独特的行为特征：在训练中见过的提示会导致更僵化和相似的生成结果，而未见的提示则保持更大的多样性。基于此观察，提出了Min-$k$NN Distance方法，该方法为给定提示采样多个补全结果，计算k个最小最近邻编辑距离的平均值，无需访问参考模型或标记概率。

Result: 在多个RLVR训练过的推理模型上的实验表明，Min-$k$NN Distance能够可靠地区分RLVR训练中见过和未见的示例，并且在性能上优于现有的成员推理和RL污染检测基线方法。

Conclusion: RLVR训练会导致生成多样性的显著变化，这种变化可以通过Min-$k$NN Distance方法有效检测。该方法为识别RLVR训练数据污染提供了一种简单有效的黑盒检测工具。
...
### [167] [Hi-SAM: A Hierarchical Structure-Aware Multi-modal Framework for Large-Scale Recommendation](https://arxiv.org/abs/2602.11799)
*Pingjun Pan,Tingting Zhou,Peiyao Lu,Tingting Fei,Hongxiang Chen,Chuanjiang Luo*

Main category: cs.AI

TL;DR: Hi-SAM是一个分层结构感知的多模态推荐框架，通过解耦语义分词器和分层记忆锚点Transformer解决现有方法中的冗余和架构不匹配问题，在冷启动场景表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐系统面临两个主要问题：1）现有分词方法（如RQ-VAE）无法解耦跨模态共享语义和模态特定细节，导致冗余或崩溃；2）普通Transformer将语义ID视为扁平流，忽略了用户交互、物品和token的层次结构，扩展物品为多个token会放大长度和噪声，使注意力偏向局部细节而非整体语义。

Method: 提出Hi-SAM框架，包含两个核心设计：1）解耦语义分词器（DST）：通过几何感知对齐统一模态，采用粗到细策略量化，共享码本提取共识，模态特定码本从残差中恢复细节，通过互信息最小化强制解耦；2）分层记忆锚点Transformer（HMAT）：通过分层RoPE将位置编码分为物品间和物品内子空间以恢复层次结构，插入锚点token将物品压缩为紧凑记忆，保留当前物品细节同时仅通过压缩摘要访问历史。

Result: 在真实世界数据集上实验显示，相比现有最先进方法有持续改进，特别是在冷启动场景表现优异。在服务数百万用户的大规模社交平台部署后，Hi-SAM在核心在线指标上实现了6.55%的提升。

Conclusion: Hi-SAM通过解耦语义分词和分层Transformer架构，有效解决了多模态推荐中的语义冗余和层次结构不匹配问题，在实际部署中验证了其有效性，特别是在处理冷启动场景方面表现突出。
...
### [168] [PuYun-LDM: A Latent Diffusion Model for High-Resolution Ensemble Weather Forecasts](https://arxiv.org/abs/2602.11807)
*Lianjun Wu,Shengchen Zhu,Yuxuan Liu,Liuyu Kai,Xiaoduan Feng,Duomin Wang,Wenshuo Liu,Jingxuan Zhang,Kelvin Li,Bin Wang*

Main category: cs.AI

TL;DR: PuYun-LDM通过3D-MAE编码天气演变特征和VA-MFM自适应频域正则化，解决了气象数据中潜在扩散模型的可扩散性限制问题，实现了高效的全球天气预报。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型在高分辨率集合天气预报中存在可扩散性限制，气象数据缺乏任务无关的基础模型和明确的语义结构，现有频域方法在多元气象数据的谱异质性下导致正则化强度不均匀。

Method: 提出3D掩码自编码器编码天气状态演变特征作为扩散模型的额外条件，结合变量感知掩码频率建模策略，根据每个变量的谱能量分布自适应选择阈值，构建PuYun-LDM模型。

Result: PuYun-LDM在短预报时效优于ENS，长时效与ENS相当，能在单块NVIDIA H200 GPU上5分钟内生成15天全球预报，集合预报可并行高效生成。

Conclusion: 通过3D-MAE和VA-MFM策略显著增强了潜在扩散模型在气象预报中的可扩散性，为高分辨率集合天气预报提供了高效解决方案。
...
### [169] [Predicting LLM Output Length via Entropy-Guided Representations](https://arxiv.org/abs/2602.11812)
...
### [170] [Prototype Transformer: Towards Language Model Architectures Interpretable by Design](https://arxiv.org/abs/2602.11852)
...
TL;DR: ProtoT是一种基于原型向量的自回归语言模型架构，替代标准自注意力Transformer，通过输入序列与原型之间的双向通信实现可解释性，同时保持良好性能。
...
Method: 提出原型Transformer（ProtoT）架构，基于原型向量（参数向量），通过输入序列与原型之间的双向通信机制工作。原型在训练过程中自动捕获可命名的概念，创建不同时间尺度的通信通道。

Result: ProtoT在计算可扩展性上实现线性复杂度（vs 标准自注意力的二次复杂度），在文本生成和下游任务（GLUE）上表现良好，对输入扰动的鲁棒性达到或优于基线，同时提供可解释的推理路径。
...
### [171] [Talk2DM: Enabling Natural Language Querying and Commonsense Reasoning for Vehicle-Road-Cloud Integrated Dynamic Maps with Large Language Models](https://arxiv.org/abs/2602.11860)
*Lu Tao,Jinxuan Luo,Yousuke Watanabe,Zhengshu Zhou,Yuhuan Lu,Shen Ying,Pan Zhang,Fei Zhao,Hiroaki Takada*

Main category: cs.AI

TL;DR: 该论文提出了VRCsim仿真框架和Talk2DM模块，为动态地图系统增加自然语言查询和常识推理能力，提升车路云协同自动驾驶中的人机交互体验。


<details>
  <summary>Details</summary>
Motivation: 现有动态地图系统缺乏自然语言支持的人机交互界面，限制了人类与动态地图系统的交互效率。虽然动态地图增强的自动驾驶系统已在日本实际部署，但这一缺陷仍然存在。

Method: 1. 开发VRCsim车路云协同感知仿真框架生成流式数据；2. 构建VRC-QA问答数据集专注于混合交通场景的空间查询和推理；3. 提出Talk2DM模块，基于链式提示机制整合人类定义规则与大型语言模型的常识知识。

Result: Talk2DM在不同LLM间无缝切换时保持高查询准确率（超过93%），平均响应时间仅2-5秒。虽然更大模型准确率更高但效率下降明显，Qwen3:8B、Gemma3:27B和GPT-oss模型表现出色。

Conclusion: Talk2DM模块成功扩展了VRC动态地图系统的自然语言查询和常识推理能力，展示了强大的泛化能力和实际应用潜力，为车路云协同自动驾驶提供了更友好的人机交互界面。
...
### [172] [From Atoms to Trees: Building a Structured Feature Forest with Hierarchical Sparse Autoencoders](https://arxiv.org/abs/2602.11881)
*Yifan Luo,Yang Zhan,Jiedong Jiang,Tianyang Liu,Mingrui Wu,Zhennan Zhou,Bin Dong*

Main category: cs.AI

TL;DR: HSAE是一种分层稀疏自编码器，通过联合学习多个SAE及其特征间的父子关系来捕获LLM中的层次语义结构，同时保持重建保真度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器(SAE)只能孤立地提取单语义特征，但大量证据表明大语言模型(LLM)捕获了自然语言的内在结构，特别是"特征分裂"现象表明这种结构是层次化的。需要一种方法来捕获这种层次结构。
...
### [173] [When Should LLMs Be Less Specific? Selective Abstraction for Reliable Long-Form Text Generation](https://arxiv.org/abs/2602.11908)
*Shani Goren,Ido Galil,Ran El-Yaniv*

Main category: cs.AI

TL;DR: 论文提出选择性抽象框架，让大语言模型通过降低不确定内容的细节来平衡准确性和信息量，避免传统二值化弃权机制的信息损失。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在事实错误问题，传统的不确定性估计机制采用"全有或全无"的二值化弃权方式，在长文本生成中过于严格，会丢弃有价值信息。

Method: 提出选择性抽象框架，通过原子级选择性抽象方法，将响应分解为原子声明，用更高置信度但细节更少的抽象版本替换不确定的原子声明。

Result: 在FactScore和LongFact-Objects基准测试中，原子级选择性抽象在六个开源模型上表现优于现有基线，风险-覆盖率曲线下面积比声明移除方法提升高达27.73%。

Conclusion: 通过降低细节特异性来提升准确性和可靠性，同时保留大部分原始含义，为LLM不确定性管理提供了更灵活有效的方法。
...
### [174] [AlphaPROBE: Alpha Mining via Principled Retrieval and On-graph biased evolution](https://arxiv.org/abs/2602.11917)
*Taian Guo,Haiyang Shen,Junyu Luo,Binqi Chen,Hongjun Ding,Jinsheng Huang,Luchen Liu,Yun Ma,Ming Zhang*

Main category: cs.AI

TL;DR: AlphaPROBE：通过有向无环图导航进行alpha因子挖掘的新框架，解决了现有方法缺乏全局结构视角的问题，显著提升了预测准确性、收益稳定性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有自动化alpha因子挖掘方法存在两个主要范式：解耦因子生成（将因子发现视为孤立事件）和迭代因子演化（专注于局部父子细化）。这两种范式都缺乏全局结构视角，将因子池视为非结构化集合或碎片化链，导致冗余搜索和多样性有限。

Method: AlphaPROBE将alpha挖掘重新定义为有向无环图（DAG）的战略导航，将因子建模为节点，演化链接建模为边。框架包含两个核心组件：1）贝叶斯因子检索器，通过后验概率模型平衡利用和探索来识别高潜力种子；2）DAG感知因子生成器，利用因子的完整祖先轨迹来产生上下文感知、非冗余的优化。

Result: 在三个主要中国股市数据集上对8个竞争基线的广泛实验表明，AlphaPROBE在预测准确性、收益稳定性和训练效率方面显著获得增强性能。结果证实利用全局演化拓扑对于高效稳健的自动化alpha发现至关重要。

Conclusion: AlphaPROBE通过将因子池建模为动态互连生态系统，并采用基于DAG的导航策略，解决了现有alpha因子挖掘方法的局限性，为自动化alpha发现提供了更高效和稳健的框架。
...
### [175] [MEME: Modeling the Evolutionary Modes of Financial Markets](https://arxiv.org/abs/2602.11918)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在量化金融中的应用主要采用资产中心或市场中心范式，往往忽视了驱动市场变动的底层逻辑推理。需要一种能够捕捉金融市场中竞争性投资叙事演化的逻辑导向方法。

Method: 提出MEME框架：1) 多智能体提取模块将噪声数据转化为高质量投资论点；2) 高斯混合模型在语义空间中挖掘潜在共识；3) 时间评估和对齐机制跟踪不同市场条件下思维模式的演变生命周期和历史盈利能力。

Result: 在2023-2025年三个异质性中国股票池上的实验表明，MEME持续优于七个最先进的基线方法。消融研究、敏感性分析、生命周期案例研究和成本分析验证了MEME识别和适应金融市场演化共识的能力。

Conclusion: MEME通过建模金融市场中竞争性投资叙事的演化生态系统，将逻辑导向视角引入量化金融，优先考虑持久市场智慧而非短暂异常，为投资组合构建提供稳健的推理指导。
...
### [176] [Gaia2: Benchmarking LLM Agents on Dynamic and Asynchronous Environments](https://arxiv.org/abs/2602.11964)
*Romain Froger,Pierre Andrews,Matteo Bettini,Amar Budhiraja,Ricardo Silveira Cabral,Virginie Do,Emilien Garreau,Jean-Baptiste Gaya,Hugo Laurençon,Maxime Lecanu,Kunal Malkan,Dheeraj Mekala,Pierre Ménard,Gerard Moreno-Torres Bertran,Ulyana Piterbarg,Mikhail Plekhanov,Mathieu Rita,Andrey Rusakov,Vladislav Vorotilov,Mengjue Wang,Ian Yu,Amine Benhalloum,Grégoire Mialon,Thomas Scialom*

Main category: cs.AI

TL;DR: Gaia2是一个评估LLM智能体在真实异步环境中的基准测试，引入独立于智能体动作的环境演化、时间约束、动态事件等挑战，支持细粒度动作级评估和强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准多为静态或同步环境，无法反映真实世界中环境独立演化、时间敏感、动态变化等复杂特性，需要新的基准来评估智能体在实际异步环境中的能力。
...
### [177] [CSEval: A Framework for Evaluating Clinical Semantics in Text-to-Image Generation](https://arxiv.org/abs/2602.12004)
*Robert Cronshaw,Konstantinos Vilouras,Junyu Yan,Yuning Du,Feng Chen,Steven McDonagh,Sotirios A. Tsaftaris*

Main category: cs.AI

TL;DR: 提出了CSEval框架，利用语言模型评估生成医学图像与提示之间的临床语义对齐，弥补现有评估方法只关注真实性和多样性的不足。
...
### [178] [InjectRBP: Steering Large Language Model Reasoning Behavior via Pattern Injection](https://arxiv.org/abs/2602.12013)
*Xiuping Wu,Zhao Yu,Yuxin Cheng,Ngai Wong,Liangjun Ke,Tapas Mishra,Konstantinos V. Katsikopoulos*

Main category: cs.AI

TL;DR: 该研究提出两种无需参数更新的推理优化方法，通过注入模型自身行为模式来提升大语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过直觉性的提示调整来增强推理，缺乏对底层行为模式的系统分析。研究者希望从行为模式角度探究模型推理行为如何塑造推理过程。

Method: 提出两种优化方法：1) InjectCorrect：通过模仿模型过去正确答案的行为模式来引导模型；2) InjectRLOpt：从历史行为模式数据学习价值函数，通过可靠性感知Softmax策略在推理时生成行为注入物来引导推理过程。

Result: 两种方法都能在各种推理任务中提升模型性能，无需修改模型参数，分别实现了最高5.34%和8.67%的性能提升。

Conclusion: 模型在回答特定类型问题时表现出自适应推理行为分布，结构化注入这些模式能显著影响推理过程和结果质量，提出的无参数更新优化方法有效提升了推理性能。
...
### [179] [Multi UAVs Preflight Planning in a Shared and Dynamic Airspace](https://arxiv.org/abs/2602.12055)
...
TL;DR: DTAPP-IICR：一种面向大规模无人机机群预飞行规划的交付时间感知优先级规划方法，通过增量迭代冲突解决，在动态共享空域中实现高效路径规划。
...
Method: 提出DTAPP-IICR框架：1）基于紧急程度对任务进行优先级排序生成初始解；2）使用SFIPP-ST（带软约束和时间约束的安全飞行间隔路径规划）计算往返轨迹；3）通过几何冲突图引导的迭代大邻域搜索解决剩余冲突；4）采用完整性保持的方向性剪枝技术加速3D搜索。

Result: 在包含临时禁飞区的基准测试中，DTAPP-IICR在多达1000架无人机的机群上实现接近100%的成功率，通过剪枝获得高达50%的运行时间减少，优于批处理增强冲突搜索方法。在城市规模的实际操作中成功扩展，而其他基于优先级的方法在中等规模部署下就会失败。
...
### [180] [The Pensieve Paradigm: Stateful Language Models Mastering Their Own Context](https://arxiv.org/abs/2602.12108)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型虽然拥有成熟的数据库和检索系统，但缺乏主动管理自身状态的能力，只能被动接受手动设计的上下文作为记忆，这限制了模型在长文档理解和持续对话等任务中的表现。

Method: 引入StateLM模型，配备上下文剪枝、文档索引、笔记记录等记忆工具，并训练模型主动管理这些工具，通过内部推理循环动态管理自身状态。

Result: 在长文档QA任务中，StateLM在所有模型规模上都优于标准LLM；在聊天记忆任务中，准确率提升10-20%；在深度研究任务BrowseComp-Plus中，StateLM达到52%准确率，而标准LLM仅约5%。
...
### [181] [Stop Unnecessary Reflection: Training LRMs for Efficient Reasoning with Adaptive Reflection and Length Coordinated Penalty](https://arxiv.org/abs/2602.12113)
*Zewei Yu,Lirong Gao,Yuke Zhu,Bo Zheng,Sheng Guo,Haobo Wang,Junbo Zhao*

Main category: cs.AI

TL;DR: ARLCP是一个自适应反射和长度协调惩罚的强化学习框架，通过动态平衡推理效率和准确性，减少大推理模型中不必要的冗长思考链，在保持精度的同时显著缩短响应长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中经常生成过长的思考链，包含大量重复自我提问和循环推理等不必要的反思，导致高令牌消耗、计算开销增加和延迟升高，但准确性并未提升，尤其是在较小模型中。问题复杂度增加会引发更多过度和不必要的反思，从而降低准确性并增加令牌开销。

Method: 提出自适应反射和长度协调惩罚（ARLCP）框架，包含两个关键创新：1）自适应反射惩罚，动态减少不必要的反思步骤同时保留必要的推理；2）根据问题估计复杂度校准的长度惩罚。通过协调这两种惩罚，鼓励模型生成更简洁有效的推理路径。

Result: 在五个数学推理基准测试中使用DeepSeek-R1-Distill-Qwen-1.5B和7B模型进行评估。对于1.5B模型，平均响应长度减少53.1%，同时准确性提高5.8%；对于7B模型，长度减少35.0%，准确性提高2.7%。实现了优于现有方法的效率-准确性权衡。

Conclusion: ARLCP框架通过自适应惩罚机制有效解决了大推理模型中过度反思和冗长推理的问题，在保持甚至提高准确性的同时显著减少了推理长度，为推理效率优化提供了有效解决方案。
...
### [182] [HLA: Hadamard Linear Attention](https://arxiv.org/abs/2602.12128)
*Hanno Ackermann,Hong Cai,Mohsen Ghafoorian,Amirhossein Habibian*

Main category: cs.AI

TL;DR: 提出Hadamard线性注意力（HLA），通过计算相似度后的非线性变换来更好地近似softmax，避免张量重塑，在视频生成等大规模token应用中展现高效性


<details>
  <summary>Details</summary>
Motivation: 标准二次注意力计算成本高，现有线性注意力使用核函数独立处理输入后再计算相似度，但只能实现低阶有理函数近似softmax，限制了表达能力

Method: 提出Hadamard线性注意力（HLA），不像传统线性注意力那样将非线性分别应用于查询和键，而是像标准softmax注意力一样在计算完成对相似度后应用非线性，实现高阶有理函数近似softmax，并推导出高效计算方案

Result: HLA能更精确地近似softmax注意力，计算效率与标准线性注意力相似，无需耗时的张量重塑操作，在视频生成的大规模扩散变换器模型中验证了有效性

Conclusion: HLA提供了一种更有效的线性注意力机制，通过后相似度非线性变换实现更好的softmax近似，适用于处理大规模token的应用场景
...
### [183] [Neutral Prompts, Non-Neutral People: Quantifying Gender and Skin-Tone Bias in Gemini Flash 2.5 Image and GPT Image 1.5](https://arxiv.org/abs/2602.12133)
*Roberto Balestri*
...
Method: 使用四个语义中性提示词生成3200张照片级真实图像，采用混合颜色归一化、面部标志点掩蔽和感知均匀肤色量化（使用Monk、PERLA和Fitzpatrick量表）的严格分析流程。

Result: 中性提示词产生了高度极化的默认输出：两个模型都表现出强烈的"默认白人"偏见（>96%的输出），但在性别偏好上存在显著差异——Gemini偏好女性呈现的主体，而GPT偏好肤色较浅的男性呈现主体。

Conclusion: 中性提示词作为诊断探针而非中性指令，挑战了"无标记语言会导致包容性表征"的社会语言学假设，为审计算法视觉文化提供了稳健框架。
...
### [184] [Value Alignment Tax: Measuring Value Trade-offs in LLM Alignment](https://arxiv.org/abs/2602.12134)
*Jiajun Chen,Hua Shen*

Main category: cs.AI

TL;DR: 本文提出了价值对齐税（VAT）框架，用于衡量对齐干预如何影响相互关联的价值体系，而不仅仅是目标价值的变化。


<details>
  <summary>Details</summary>
Motivation: 现有价值对齐研究通常静态描述价值关系，忽视了提示、微调、偏好优化等干预措施如何重塑更广泛的价值体系。需要一种能够捕捉对齐压力下价值表达动态变化的框架。

Method: 引入价值对齐税（VAT）框架，测量对齐引起的变化在相互关联的价值中的传播情况。基于Schwartz价值理论构建受控情境-行动数据集，收集对齐前后的成对规范性判断，分析不同模型、价值和策略下的对齐效应。
...
### [185] [STAR : Bridging Statistical and Agentic Reasoning for Large Model Performance Prediction](https://arxiv.org/abs/2602.12143)
...
Conclusion: STAR框架通过结合统计期望与知识驱动推理，有效解决了大模型性能预测中的模式变化、数据稀疏和解释性不足问题，在稀疏观测下实现了显著性能提升。
...
### [186] [GPT-4o Lacks Core Features of Theory of Mind](https://arxiv.org/abs/2602.12150)
...
<details>
  <summary>Details</summary>
Motivation: 当前研究主要评估LLMs在心理理论基准测试中的表现，但这些评估并未测试心理理论所假设的实际表征（即心理状态与行为的因果模型）。本文旨在开发基于认知科学定义的新评估框架，探究LLMs是否拥有连贯、领域通用且一致的心理状态-行为因果模型。

Method: 使用基于认知科学定义的心理理论概念，开发新的评估框架，测试LLMs是否拥有连贯、领域通用且一致的心理状态-行为因果模型。具体方法包括：1）在简单心理理论范式中测试LLMs近似人类判断的能力；2）在逻辑等价任务中测试LLMs的表现；3）评估LLMs的行为预测与相应心理状态推断之间的一致性。

Result: 研究发现：1）LLMs在简单心理理论范式中能成功近似人类判断；2）但在逻辑等价任务中失败；3）LLMs的行为预测与相应心理状态推断之间一致性较低。这些结果表明LLMs缺乏连贯、领域通用且一致的心理状态-行为因果模型。

Conclusion: 大型语言模型展现的社交熟练度并非源于领域通用或一致的心理理论。尽管LLMs在某些任务中能模拟人类判断，但它们缺乏真正的心理状态-行为因果模型，这对其在需要深层社会理解的应用中的可靠性提出了重要问题。
...
### [187] [Sci-CoE: Co-evolving Scientific Reasoning LLMs via Geometric Consensus with Sparse Supervision](https://arxiv.org/abs/2602.12164)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学推理任务中仍然脆弱，主要原因是不可靠的解评估和验证策略多样性有限。需要一种能够自我进化的框架来提升科学推理的鲁棒性。
...
Conclusion: Sci-CoE通过协同进化框架有效提升了科学推理任务的性能，为构建更强大的科学推理系统提供了新思路。
...
### [188] [Pedagogically-Inspired Data Synthesis for Language Model Knowledge Distillation](https://arxiv.org/abs/2602.12172)
*Bowei He,Yankai Chen,Xiaokun Zhang,Linghe Kong,Philip S. Yu,Xue Liu,Chen Ma*

Main category: cs.AI

TL;DR: 本文提出了一种基于教育原则的LLM知识蒸馏框架IOA，通过三阶段管道系统识别学生模型知识缺陷、组织渐进式课程、适配认知容量，显著提升小模型性能。
...
Method: 提出IOA三阶段框架：知识识别器识别学生模型知识缺陷；知识组织器通过渐进式课程组织知识传递；知识适配器调整表示以匹配学生模型认知容量。整合布鲁姆掌握学习原则和维果茨基最近发展区理论，创建动态蒸馏过程。

Result: 使用LLaMA-3.1/3.2和Qwen2.5作为学生模型，IOA在DollyEval上保留教师模型94.7%的性能，参数量不到1/10。在复杂推理任务上表现突出：MATH提升19.2%，HumanEval提升22.3%。

Conclusion: 基于教育原则的IOA框架为LLM知识蒸馏提供了系统化方法，显著提升了小模型性能，特别是在复杂推理任务上表现优异，证明了教学意识在知识蒸馏中的重要性。
...
### [189] [SAM3-LiteText: An Anatomical Study of the SAM3 Text Encoder for Efficient Vision-Language Segmentation](https://arxiv.org/abs/2602.12173)
*Chengxi Zeng,Yuxuan Jiang,Ge Gao,Shuai Wang,Duolikun Danier,Bin Zhu,Stevan Rudinac,David Bull,Fan Zhang*

Main category: cs.AI

TL;DR: SAM3-LiteText：通过轻量级文本编码框架，在保持分割性能的同时，将文本编码器参数量减少高达88%


<details>
  <summary>Details</summary>
Motivation: 视觉语言分割模型（如SAM3）使用为开放语言理解设计的大型通用文本编码器，但实际分割提示通常简短、结构化且语义受限，导致文本编码器容量过度配置，产生持续的计算和内存开销

Method: 对视觉语言分割中的文本提示进行大规模解剖分析（涵盖404,796个真实提示），发现严重冗余问题；提出SAM3-LiteText轻量级文本编码框架，用紧凑的MobileCLIP学生模型替换原始SAM3文本编码器，通过知识蒸馏进行优化

Result: 在图像和视频分割基准测试中，SAM3-LiteText将文本编码器参数减少高达88%，显著降低静态内存占用，同时保持与原始模型相当的分割性能

Conclusion: 通过针对分割提示特性设计的轻量级文本编码框架，可以在大幅减少参数和内存开销的同时保持分割性能，为视觉语言分割模型的效率优化提供了有效方案
...
### [190] ["Sorry, I Didn't Catch That": How Speech Models Miss What Matters Most](https://arxiv.org/abs/2602.12249)
*Kaitlyn Zhou,Martijn Bartelds,Federico Bianchi,James Zou*

Main category: cs.AI

TL;DR: 语音识别系统在标准基准测试中表现良好，但在现实世界高风险的短语音转录任务（如美国街道名称转录）中失败率高达44%，对非英语母语者影响更大。通过合成数据生成方法可以显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 尽管语音识别系统在标准基准测试中取得了低词错误率，但在现实世界部署中，对于高风险、短语音的转录任务（如美国街道名称转录）经常失败。这种失败在实际应用中可能带来严重后果，特别是对非英语母语者造成更大的负面影响。

Method: 研究评估了15个来自OpenAI、Deepgram、Google和Microsoft的模型，使用来自语言多样化的美国说话者的录音。为了缓解这一问题，引入了合成数据生成方法，利用开源文本转语音模型生成命名实体的多样化发音。使用少于1000个合成样本进行微调。

Result: 在街道名称转录任务中，平均转录错误率达到44%。错误对所有说话者都有系统性影响，但对非英语母语者的路由距离错误是英语母语者的两倍。通过合成数据微调，非英语母语者的街道名称转录准确率相对于基础模型提高了近60%。

Conclusion: 研究揭示了语音系统在基准测试表现和现实世界可靠性之间的关键差距，并展示了一种简单、可扩展的路径来减少高风险转录错误。合成数据生成方法能有效改善语音识别系统对语言多样化说话者的表现。
...
### [191] [Think like a Scientist: Physics-guided LLM Agent for Equation Discovery](https://arxiv.org/abs/2602.12259)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM-based系统直接从数据猜测方程，没有模拟科学家通常遵循的多步推理过程：先推断物理属性（如对称性），然后将其作为先验来限制候选方程空间。需要一种更符合科学推理过程的框架来提高符号方程发现的准确性和鲁棒性。

Method: 引入KeplerAgent智能体框架，协调基于物理的工具提取中间结构，并使用这些结果配置符号回归引擎（如PySINDy和PySR），包括它们的函数库和结构约束，明确遵循科学推理过程。

Result: 在一系列物理方程基准测试中，KeplerAgent相比LLM和传统基线方法，实现了显著更高的符号准确性和对噪声数据更强的鲁棒性。

Conclusion: 通过模拟科学家的多步推理过程，先推断物理属性再约束方程空间的方法，能够显著提高符号方程发现的性能，为科学发现提供了更有效的工具。
...
### [192] [CM2: Reinforcement Learning with Checklist Rewards for Multi-Turn and Multi-Step Agentic Tool Use](https://arxiv.org/abs/2602.12268)
...
TL;DR: CM2是一个RL框架，通过检查表奖励替代可验证结果奖励，在LLM模拟工具环境中训练多轮多步骤工具使用智能体，显著优于监督微调方法。
...
Method: CM2将每轮预期行为分解为细粒度二元标准，具有明确证据基础和结构化元数据，将开放式判断转化为更稳定的分类式决策。采用稀疏奖励分配但密集评估标准的策略，在可扩展的LLM模拟工具环境中进行训练。

Result: 从8B基础模型开始，在8k示例RL数据集上训练，CM2相比SFT对应模型在tau^-Bench上提升8点，BFCL-V4上提升10点，ToolSandbox上提升12点。结果匹配甚至优于类似规模的开源基线，包括评判模型。

Conclusion: CM2为优化多轮多步骤工具使用智能体提供了一个可扩展的解决方案，无需依赖可验证奖励，在多个基准测试中表现出显著改进。
