...
TL;DR: 本文从测度输运视角形式化视觉文本压缩(VTC)的信息损失，提出无需下游标签的路由准则和注意力机制，在24个NLP数据集上实现70.8%的oracle匹配率和+3.3%的平均任务提升。


<details>
  <summary>Details</summary>
Motivation: 现有VTC方法虽能大幅减少解码token数，但压缩比无法预测下游任务表现，缺乏对视觉编码导致的任务相关信息损失的定量度量。

Method: 将文本和视觉token视为经验概率测度，用推前映射的输运代价分解为内部聚合的精度代价和跨块分割的覆盖代价，并据此设计无标签路由准则和foveation重编码机制。

Result: 在Qwen3-4B上评估24个NLP数据集，无标签路由规则与每个数据集的oracle匹配率达70.8%（17/24），平均任务分数提升+3.3%，平均token减少-10.3%。

Conclusion: 测度输运框架有效解释了VTC的信息损失机制，并提供了无需下游标签即可优化视觉/文本路径选择的实用工具。
...
TL;DR: HumanNet是一个包含一百万小时人类中心视频的大型数据集，通过系统化的数据策展范式，将非结构化互联网视频转化为可扩展的具身学习基础资源。初步验证表明，使用1000小时自我中心人类视频训练的VLM模型效果优于100小时真实机器人数据，揭示了人类视频可替代机器人数据用于具身基础模型扩展的潜力。


<details>
  <summary>Details</summary>
Motivation: 视觉和语言领域已通过互联网语料库实现了规模化扩展，但物理交互学习受限于缺乏大规模、多样化且富含标注的人类活动数据。现有具身智能研究过度依赖稀缺的机器人专用数据，限制了基础模型的可扩展性。

Method: 构建了一百万小时人类中心视频语料库HumanNet，涵盖第一人称和第三人称视角、细粒度活动、人-物交互、工具使用及长程行为。提供交互中心化标注（字幕、运动描述、手部/身体信号）。提出系统化数据策展范式，将人类中心过滤、时间结构化、视角多样性和注释丰富性作为一等设计原则。

Result: 在固定验证集下的受控消融实验中，使用HumanNet中1000小时自我中心视频继续训练的Qwen VLM模型，其表现超过了使用100小时真实机器人数据（Magic Cobot）继续训练的模型。

Conclusion: 自我中心人类视频可以作为机器人数据的一种可扩展且成本有效的替代方案，为利用人类中心视频而非仅依赖机器人专用数据来扩展具身基础模型提供了新的机会。
...
TL;DR: 本文提出R³L框架，通过不变空间分解、一致空间想象和支持性空间优化，提升3D布局生成中相对空间推理的可靠性与一致性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在推断3D布局的相对空间关系时不可靠，多跳推理中参考系变换累积误差导致语义和度量漂移。

Method: 1) 不变空间分解：解耦耦合关系链；2) 一致空间想象：通过想象-修正循环促进自一致性；3) 支持性空间优化：全局到局部坐标重参数化简化位姿优化。

Result: 在多种场景和指令下，生成的布局在物理可行性和语义一致性上优于现有方法。

Conclusion: 解决参考系引入的不一致是可靠多跳相对空间推理的关键。
...
TL;DR: LookWhen通过选择器-提取器框架，对视频中空间-时间上的token进行重要性评分，仅处理top-K重要token，从而在不损失精度的情况下大幅降低计算量，在6个数据集上达到更优的精度-效率平衡。


<details>
  <summary>Details</summary>
Motivation: 视频中充满冗余信息，传统Transformer对所有token进行超线性计算开销巨大，因此需要一种方法选择性计算重要token以降低计算成本。

Method: 提出选择器-提取器框架：浅层选择器对降采样视频快速评分所有token，深层提取器仅处理top-K选中的token；选择预训练采用最近邻距离排序token独特性，提取预训练通过视频教师和图像教师蒸馏学习。

Result: 在Kinetics-400、SSv2等6个数据集上，LookWhen在12种设置中的9种实现Pareto最优的精度-FLOPs权衡，实际吞吐量比InternVideo2-B快6.7倍且精度持平。

Conclusion: LookWhen通过学习何时、何地、计算什么，实现了通用的高效视频表示学习，在精度和计算效率上优于同类高效模型。
...
TL;DR: 本文研究了3D医学影像基础模型预训练中，如何在不同模态（CT、MRI、PET）间最优分配数据。发现跨域知识迁移具有不对称性和可预测的幂律规律，据此提出基于缩放律的优化分配策略，揭示出"枢纽-孤岛"结构，在多项任务上比数据比例采样提升高达58%。


<details>
  <summary>Details</summary>
Motivation: 当前3D医学影像基础模型需要混合多种模态数据（CT、MRI、PET）进行预训练，但数据分配策略主要依赖启发式方法，缺乏理论指导和最优方案。

Method: 通过观察发现不同医学影像模态在预训练中缩放速率不同，且跨域知识迁移呈强不对称性，遵循可预测的幂律规律。据此将数据分配建模为缩放律优化问题，推导出"枢纽-孤岛"分配结构。

Result: 提出的迁移感知分配策略相比数据比例采样提升高达58%，在未见过的预算上泛化良好（r=0.989），在疾病分类和器官/病灶分割任务上均验证了更强的预训练表征能力。

Conclusion: 跨域知识迁移的规律性和可预测性可用于指导数据分配，枢纽域应获得战略性分配，孤岛域需直接投入，为3D医学影像基础模型的预训练提供了可解释且高效的数据混合策略。
...
TL;DR: AdpSplit是一种基于误差驱动的自适应分裂算子，通过根据L1像素误差区域统计动态决定分裂子节点数量及参数初始化，替代3DGS中固定数量的随机分裂，在保持渲染质量的前提下减少训练时间9.2%-22.3%，实现最高12.6倍加速。


<details>
  <summary>Details</summary>
Motivation: 3DGS中的固定数量随机分裂需要多次致密化轮次才能发现精细结构，成为高效训练流程的瓶颈。

Method: 提出AdpSplit，基于L1像素误差区域统计自适应确定分裂子节点数量并初始化参数，作为标准分裂算子的即插即用替代。

Result: 在MipNeRF360、Deep-Blending和Tanks&Temples数据集上，减少多种加速3DGS管线训练时间9.2%-22.3%；结合FastGS在MipNeRF360上匹配全调度PSNR，训练时间减少16.4%，实现12.6倍加速。

Conclusion: AdpSplit作为简单替代方案，能有效加速3DGS训练而保持渲染质量。
...
TL;DR: 提出TriDE方法，利用三角形一致性约束来优化全局SfM中的成对相机方向估计，通过消息传递机制实现高效的方向细化。


<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理图像对的方向估计，导致局部合理但全局不一致。需要一种能联合优化方向一致性的方法。

Method: 提出TriDE，利用相机-三角形一致性作为高阶验证信号，通过加权三角形之间的消息传递来细化不可靠的成对方向。

Result: 在真实图像图上，方向精度大幅提升，下游相机位置估计更准确。

Conclusion: TriDE建立了局部成对估计与全局相机位姿几何之间的实用桥梁，避免全局非线性优化的高成本。
...
TL;DR: 本文提出了一种基于数据中心的置信学习适配方法，用于在图像分割任务中检测和缓解训练数据中的标签偏差，无需干净无偏标注，并通过特征空间中的子组可分离性实现公平性能。


<details>
  <summary>Details</summary>
Motivation: 图像分割中的标签偏差（一种系统性的、与人口统计子组相关的标注错误）尚未得到充分研究，且现有的偏差检测方法通常需要干净无偏的标注数据，而这些数据在实际中难以获取。

Method: 将置信学习（Confident Learning）适配到分割任务中，通过比较训练标签与模型置信预测来隔离方向性错误；并利用编码器特征空间中子组可分离性这一伪迹进行偏差缓解。

Result: 在合成和真实偏差的三个数据集上，该方法能够可靠地检测和缓解标签偏差，无需干净标签，且在所有实验条件下实现了公平的性能表现。

Conclusion: 该框架有效解决了分割任务中标签偏差的检测与缓解问题，不依赖无偏标注数据，为公平分割模型提供了可行方案。
...
TL;DR: 本文提出异构步分配（HSA），一种无需训练的视频生成加速算法，通过根据运动速度动态分配去噪步数给不同时空token，结合KV缓存同步和缓存欧拉更新，在保持生成质量的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有Diffusion Transformers在视频生成中对所有token均匀分配去噪步数，计算成本极高。受人类视觉忽略冗余运动的启发，作者希望实现非均匀步数分配，在加速推理的同时保持生成质量。

Method: 1）异构步分配：根据token的运动速度动态分配不同去噪步数；2）KV缓存同步机制：让活跃token能关注完整序列，同时跳过非活跃token；3）缓存欧拉更新：单步推进跳过token的隐状态，无需额外模型评估。

Result: 在Wan-2和LTX-2模型上（T2V和I2V任务），HSA显著优于此前最先进的缓存方法和Flow Matching基线，尤其在50%和25%运行时间的激进加速场景下，实现了更优的质量-运行时间帕累托前沿。

Conclusion: HSA无需昂贵离线分析，无需额外训练，鲁棒地保持结构完整性和生成质量，即使在严格计算预算下也能取得优异效果。
...
TL;DR: 本文介绍了ICCV 2025上举办的SAFE合成视频检测挑战赛，涵盖13种先进生成模型、20小时/6000个视频样本，600多份参赛作品，重点评估了算法在跨生成器泛化和后处理鲁棒性方面的表现。


<details>
  <summary>Details</summary>
Motivation: 随着生成式视频技术的快速发展，迫切需要可靠的方法来检测和表征合成媒体，以应对虚假信息传播等风险。

Method: 在Hugging Face平台上组织挑战赛，设置两个任务：(1)区分真实视频与13种先进生成模型制作的合成视频；(2)检测经过缩放、重压缩、运动模糊等后处理的合成内容。数据集包含21个多样来源的真实视频和匹配的合成视频，共6000个样本、20小时时长。采用完全盲评估方式，历时90天。

Result: 收到来自12个团队的600多份参赛作品。结果显示在跨生成器泛化方面取得了可衡量的进展，但算法对后处理操作仍然存在持续的脆弱性。

Conclusion: 当前合成视频检测方法在泛化能力上有所提升，但对常见后处理操作的鲁棒性仍是显著挑战，需要进一步研究解决。
...
TL;DR: 本文提出A²RD，一种基于智能体自回归扩散的架构，通过“检索-合成-精炼-更新”闭环流程逐段生成并自我改进长视频，在多项基准上显著提升了连贯性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有长视频合成方法在长时间跨度下存在语义漂移和叙事崩塌的问题，缺乏有效的约束机制来保持全局一致性。

Method: 提出A²RD架构，将长视频合成解耦为创意生成与一致性约束两个部分，采用闭环的“检索-合成-精炼-更新”循环，核心包括：多模态视频记忆、自适应分段生成、层次化测试时自我改进。

Result: 在1至10分钟视频的公开和自建LVBench-C基准上，A²RD在一致性上提升最高30%，叙事连贯性提升20%，人类评估也显示运动和过渡平滑度有明显改进。

Conclusion: A²RD通过解耦合成与一致性约束，结合闭环自我改进机制，有效解决了长视频合成中的语义漂移问题，为长视频生成提供了新的范式。
...
TL;DR: 本文提出XiYOLO，一种能量自适应框架，通过能量感知搜索空间、两阶段能量估计器和迭代搜索，在异构边缘设备上实现高效目标检测，相比YOLOv12在GPU和NPU上显著降低能耗（最高53.7%）。


<details>
  <summary>Details</summary>
Motivation: 异构边缘设备上的目标检测需满足严格的能耗、延迟和内存约束，现有能量感知NAS方法部署场景有限，且真实能耗高度依赖设备、测量成本高，难以直接优化。

Method: 提出能量自适应框架，包含：1）能量感知XiResOFA搜索空间；2）两阶段能量估计器提升样本效率；3）迭代搜索寻找单一高效基础架构；4）复合缩放生成XiYOLO系列模型，实现可解释的精度-能耗权衡。

Result: 在PascalVOC上，中等XiYOLO模型mAP50达86.15，GPU能耗降低20.6%，NPU降低35.9%；在COCO上，小规模XiYOLO相比YOLOv12在GPU上能耗降低53.7%，NPU降低51.6%。两阶段估计器在少量样本下优于联合预测器。

Conclusion: XiYOLO在多种边缘设备上实现了比YOLO基线更强的能量-精度权衡，验证了所提能量自适应框架的有效性。
...
TL;DR: FuScore利用多模态大语言模型生成连续质量分数而非离散等级，通过四维感知一致性和三重优化目标，实现了与人类视觉偏好高度相关的红外-可见光图像融合质量评估。


<details>
  <summary>Details</summary>
Motivation: 现有IVIF评估方法存在过度优化伪真值、忽略图像间感知模糊性等问题，导致对相似质量融合图像的区分能力不足。

Method: 提出FuScore：1) 利用MLLM生成连续质量分数替代离散等级预测；2) 基于四个IVIF子维度的一致性构建软标签；3) 设计包含图像级分布监督、源内Thurstone保真度和跨源Thurstone保真度的三重目标函数。

Result: 实验表明FuScore与人类视觉偏好的相关性达到当前最优水平。

Conclusion: 通过连续评分和感知一致性建模，有效提升了融合图像质量评估的细粒度与准确性。
...
TL;DR: 本文提出TrajGANR，一种以轨迹为中心的跨模态自监督学习框架，通过连续神经表示实现轨迹与街景图像、地理位置的细粒度对齐，在四个城市理解任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有地理空间跨模态自监督学习主要针对静态模态对（如卫星图像与文本），无法处理人类移动轨迹这种连续运动数据；轨迹对城市理解至关重要但尚未被现有框架充分探索。

Method: 提出TrajGANR框架：1）学习轨迹的连续神经表示，可在路径任意点表示轨迹特征；2）基于该连续表示实现轨迹与附近街景图像的细粒度对齐（即使图像不在轨迹航点上）；3）联合对齐三种模态：轨迹、街景图像和地理位置。

Result: 在四个城市移动与道路理解任务上，TrajGANR一致优于现有的地理空间MSSL框架以及专门用于轨迹的基础模型。消融实验表明，细粒度地理空间对齐和多模态学习框架是性能提升的主要原因。

Conclusion: 细粒度的地理空间对齐比粗粒度聚合更有效，地理空间多模态学习对轨迹理解至关重要。
...
TL;DR: LensVLM提出了一种让VLMs先扫描压缩图像，再通过所学工具选择性展开相关图像的方法，在4.3倍压缩下保持与全文本上限相当的准确率，最高可在10.1倍压缩下优于各类基线方法。


<details>
  <summary>Details</summary>
Motivation: VLMs将文本渲染为图像处理时，压缩率提高会导致字符缩小到无法辨认，准确率急剧下降。现有方法无法在保持高压缩率的同时维持良好性能。

Method: 提出LensVLM框架，包含推理流程和后训练策略：先让VLM扫描压缩后的图像，再通过学习得到的工具（tool）选择性地将相关图像展开为未压缩形式进行处理。

Result: 基于Qwen3.5-9B-Base，在4.3倍有效压缩下准确率与全文本上限相当；在10.1倍压缩下仍优于检索式、文本压缩和视觉压缩等基线方法；在七个文本QA基准以及多模态文档和代码理解任务上均表现良好。

Conclusion: 训练使视觉压缩对渲染选择更具鲁棒性；高压缩率下模型更依赖展开内容而非不可靠的视觉阅读；文本展开更适合渲染文本，高分辨率图像展开更适合保留布局信息的原生文档。
...
TL;DR: OneViewAll是一种语义先验引导的6D姿态估计框架，仅需单张RGB-D参考视图即可实现高精度姿态估计，在LINEMOD数据集上达到92.5%的ADD-0.1准确率，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实际6D姿态估计场景中往往只能获取每个物体的单张真实RGB-D参考视图，且缺乏CAD模型。现有方法依赖显式3D模型或多视角数据，限制了可扩展性。作者旨在解决这种具有挑战性的单参考无模型设置下的姿态估计问题。

Method: 提出Project-and-Compare范式，在投影等变空间中直接对齐参考和查询观测。渐进式整合三层语义先验：(1)类别与场景级先验用于假设初始化；(2)物体级对称性先验通过镜像融合进行几何补全；(3)补丁级先验用于判别式精化。

Result: LINEMOD数据集上达到92.5% ADD-0.1准确率，显著超越CVPR 2025基线One2Any（52.6%）。在YCB-V、Real275、Toyota-Light上均取得一致提升，且推理延迟低。

Conclusion: 对称感知投影在处理对称、无纹理和遮挡物体方面效果显著，证明了单参考无模型6D姿态估计的可行性与有效性。
...
TL;DR: 本文提出了一种自监督的视觉-语言跟踪器（Tracker），无需任何边界框标注，仅通过自然语言描述即可实现视频序列中的目标跟踪。方法核心是动态令牌聚合模块，通过不等权处理视觉令牌，消除冗余噪声，增强语义对齐，实现高效的跨模态融合与时序传播。


<details>
  <summary>Details</summary>
Motivation: 现有VL跟踪方法依赖边界框标注，成本高且泛化受限。本文旨在探索无需任何边界框真值的自监督VL跟踪，降低标注依赖，提升跟踪的自主性和可扩展性。

Method: 提出动态令牌聚合模块，包含三步：1) 基于锚点令牌从模板帧选择重要目标令牌；2) 根据注意力分数合并选中令牌，聚合到语言令牌中，消除视觉冗余噪声，增强语义对齐；3) 融合后的语言令牌作为引导信号，从搜索帧提取潜在目标令牌并传播到后续帧，增强时序提示，实现自监督学习。

Result: 在多个VL跟踪基准上，该跟踪器超越了当前最先进的自监督方法。

Conclusion: 该工作证明无需边界框标注，仅凭自然语言描述即可有效学习语言引导的跟踪表示，为自监督VL跟踪提供了新范式。
...
TL;DR: 本文提出ODP-Net，通过正交分解和扰动纯化策略，将通用伪造痕迹、生成器特定指纹和语义内容解耦，实现对未见架构AI生成图像的高泛化检测。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测模型往往过拟合于生成器特定指纹和语义内容，而非学习通用伪造痕迹，导致跨未见架构泛化能力差。作者将这一失败归因于特征纠缠——检测器将这些因素编码成单一的纠缠表示。

Method: 提出ODP-Net，包含三个核心模块：(1) 实例感知正交分解，将特征投影到互斥子空间；(2) 基于扰动的纯化，通过跨样本特征注入强制语义不变性；(3) 流形对齐，弥合域差距。

Result: 在未见架构（如Stable Diffusion 3）上达到最先进性能，验证了结构化解耦对泛化能力的关键作用。

Conclusion: 结构化解耦策略是提升AI生成图像检测跨架构泛化能力的关键，正交分解可以有效分离通用伪造痕迹与特定指纹/语义内容。
...
TL;DR: 本文提出基于残差潜在动作（RLA）的世界模型RLA-WM，通过流匹配预测RLA值，在效率和性能上超越现有基于特征和视频扩散的世界模型，并支持无动作视频的机器人学习与纯离线视觉强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型多基于图像生成，效率低且易产生幻觉；基于视觉特征的方法虽更高效，但在复杂交互中直接回归导致预测模糊或崩溃，且在高维特征空间进行生成建模仍有挑战。

Method: 从DINO残差中学习残差潜在动作（RLA），并通过流匹配（flow matching）预测RLA值，构建RLA世界模型（RLA-WM）。

Result: RLA-WM在仿真和真实数据集上超越最先进的基于特征和视频扩散的世界模型，同时速度比视频扩散快数个数量级。

Conclusion: RLA具预测性、可泛化性且编码时间进程；基于RLA-WM可发展出无动作视频的机器人策略学习方法和完全在离线视频训练的世界模型内进行视觉强化学习的框架。
...
TL;DR: 本文提出InfoGeo框架，利用信息瓶颈理论结合物体中心学习，通过最大化视角不变信息与最小化视角特定噪声，显著提升跨视角地理定位在复杂环境下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法依赖全局特征对齐，在面对不同区域纹理和天气条件导致的域偏移时表现不佳，尤其在无人机场景下，广阔视角引入的密集细粒度目标造成严重视觉混乱。

Method: 借鉴物体中心学习(OCL)思想，将优化过程重新表述为信息瓶颈流程，包含两个核心目标：(i) 通过对齐跨视角的物体中心结构关系最大化视角不变信息；(ii) 通过跨视角知识约束最小化视角特定噪声信号。

Result: 在多种基准数据集和挑战性场景下的广泛评估表明，InfoGeo显著优于当前最先进方法。

Conclusion: InfoGeo通过信息论框架有效解决了跨视角地理定位中的域偏移和视觉混乱问题，验证了物体中心学习在提升模型泛化能力方面的有效性。
...
TL;DR: 本文提出一种神经符号主义框架REASON，将基于骨架的人体动作识别重构为基于运动原语的一阶逻辑推理，在保持 competitive 识别性能的同时提供可解释的逻辑结构。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的动作识别模型性能强大但缺乏可解释性，难以理解模型决策背后的逻辑。

Method: 1) 使用时空骨架编码器提取潜在运动表示；2) 通过时空概念解码器将其映射为可解释的概念谓词，分离姿态中心和动力学中心抽象；3) 通过可微一阶逻辑层组合概念谓词，学习可读的逻辑规则；4) 利用LLM描述的对齐原子运动原语，建立共享概念空间。

Result: 在NTU RGB+D 60/120和NW-UCLA数据集上取得了具有竞争力的识别性能，同时提供了基于逻辑结构的显式可解释解释。

Conclusion: 神经符号推理是实现可解释的时空动作理解的有效范式，能够兼顾性能与可解释性。
...
TL;DR: 本文提出Qwen3-VL-Seg，一种参数高效的开放世界指代分割框架，仅需17M参数（基础模型的0.4%），利用MLLM预测的边界框作为结构先验，通过轻量级掩码解码器实现像素级分割，并构建了SA1B-ORS数据集和ORS-Bench基准，在封闭和开放世界场景下均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在开放世界视觉定位中仅输出稀疏边界框，无法实现密集像素级预测；而现有分割方法要么直接预测稀疏轮廓难以重建连续边界，要么依赖SAM等外部分割模型带来部署开销。需要一种既保持MLLM开放世界能力又实现高效像素级分割的方案。

Method: 提出Qwen3-VL-Seg框架：1) 将MLLM预测的边界框作为语义结构先验；2) 设计轻量级框引导掩码解码器，包含多尺度空间特征注入、空间-语义查询构建、框引导高分辨率像素融合和迭代掩码感知查询精炼；3) 构建SA1B-ORS数据集（含Category和Descriptive两个子集）和ORS-Bench基准。

Result: 在指代分割、视觉定位和ORS-Bench任务上，Qwen3-VL-Seg在封闭和开放世界设置下均表现优异，尤其在语言密集型指令和分布外泛化方面具有明显优势，且通用多模态能力基本保持。

Conclusion: Qwen3-VL-Seg以极低的参数开销（17M）实现了开放世界指代分割，验证了利用MLLM预测框作为结构先验进行像素级分割的有效性，为轻量级开放世界视觉分割提供了新思路。
...
TL;DR: AGA3DNet是一种基于影像报告引导的3D脑MRI亚型分类框架，通过将放射学报告中提取的简短解剖短语作为软解剖先验通道，结合轻量级3D CNN和多视图xLSTM聚合，实现了可解释、解剖引导的分类性能提升。


<details>
  <summary>Details</summary>
Motivation: 准确的3D脑MRI亚型分类既需要局部解剖线索，也需要长距离上下文推理能力。现有方法缺乏可解释的解剖引导机制，且难以充分利用放射学报告中的结构化信息。

Method: 从放射学报告中提取解剖短语，映射到图谱定义区域；通过符号距离变换和高斯加权转换为平滑空间先验；与轻量级3D CNN和多视图xLSTM聚合融合。

Result: 在回顾性临床脑MRI队列的异常亚型分类任务中，AGA3DNet在多个性能指标上实现了更优的整体平衡，并通过先验通道支持临床可解释的定位。

Conclusion: AGA3DNet有效融合了报告引导的解剖先验与深度学习模型，提升了分类性能和可解释性，但存在单队列评估和缺乏大规模配对公开数据集的局限性。
...
TL;DR: 提出TriP框架，通过三角形几何推断局部边尺度并在对数域同步，实现鲁棒的平移平均，在合成和真实数据集上大幅超越之前方法。


<details>
  <summary>Details</summary>
Motivation: 平移平均问题中方向测量缺乏距离信息，导致估计病态且对异常值敏感；现有方法难以同时应对对抗性、循环一致等结构化污染，且易出现退化零解问题。

Method: 1) 从三角形几何推断局部相对边尺度；2) 在对数域同步重叠三角形的尺度，恢复全局一致的边长度和相机位置；3) 利用三角形间高阶一致性增强鲁棒性。

Result: 在合成和真实数据集上大幅优于所有之前的方法；支持百万级相机图的高效并行计算；具备精确位置恢复的强理论保证。

Conclusion: TriP通过对数域尺度同步天然避免退化零解，无需额外约束；利用高阶三角形一致性有效抵抗结构化异常；方法高效、可并行、可扩展，是平移平均问题的突破性方案。
...
TL;DR: 本文证明当前视觉-语言模型（VLM）在2D输入下存在潜在的三维拓扑地图，但被非几何语义（颜色、形状）掩盖。通过跨场景线性特征提取分离出空间子空间，并证明其与场景的拉普拉斯特征映射等价。基于此提出Dirichlet能量正则化方法，在500步微调后使空间拓扑理解任务提升最高12.1%。


<details>
  <summary>Details</summary>
Motivation: 尽管VLM表现出从2D图像中进行空间推理的能力，但尚不清楚它们是否构建了类似人类认知地图的3D内部表征。作者希望揭示VLM是否蕴含几何化的三维空间表征，并解决其被视觉语义噪声覆盖的问题。

Method: 1) 通过跨场景线性特征提取分离出VLM中的空间子空间；2) 证明该子空间与场景3D高斯核图的拉普拉斯特征映射之间的数学等价性；3) 基于Dirichlet能量设计潜在空间正则化方法，在500步的SFT微调中应用于简单合成数据。

Result: 在真实世界空间基准测试中，该方法在拓扑理解相关的空间任务上比标准SFT和竞争基线提升最高12.1%。

Conclusion: VLM确实包含隐式的3D拓扑空间表征，通过数学驱动的正则化方法可以有效强化这一表征，从而显著提升模型的空间推理能力。
...
TL;DR: 针对工业异常检测中细微几何缺陷难以检测的问题，提出了Real-IAD-MVN多视角表面法向数据集，用高保真多视角表面法向图替代稀疏3D点云，并设计了基于跨模态统一原型的重建基线方法，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有IAD方法中，2D RGB图像对纹理和光照敏感但难以捕捉细微几何异常，3D点云能捕获宏观形状却因稀疏性无法检测微缺陷（如划痕、凹坑），缺乏能同时提供微观几何细节的数据表征。

Method: 1) 升级采集系统，构建Real-IAD-MVN数据集，从五个不同视角捕获高保真表面法向图，替代稀疏3D点云；2) 提出基于重建的基线方法，学习从图像和法向图流中提取跨模态统一原型。

Result: 实验表明，密集多视角伪3D（表面法向）的检测性能显著优于稀疏3D点云；所提统一原型方法超越现有最先进的多模态融合方法。

Conclusion: 多视角高分辨率表面法向数据为细微几何异常检测提供了更优的数据基础，新数据集和基线方法为工业异常检测领域提供了有力基准和新的研究方向。
...
TL;DR: 提出PRIMED框架，受认知神经科学中的竞争偏向理论启发，通过显式建模模态先验和自适应抑制无关模态，在参考音视频分割任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多模态线索视为同质输入进行融合、提示或推理，容易受到无关或误导性模态的干扰。需解决不同模态在不同指代表达和场景下相关性不同的问题。

Method: 1) 模态先验解码器估计指代表达依赖音频、视觉或二者联合的程度，生成模态先验引导注意力；2) Token蒸馏器从高层特征提取紧凑全局视觉token，跨竞争感知跨模态融合模块共享层次化全局上下文；3) 空间感知语义对齐损失通过对比学习增强前景-背景区分。

Result: 在Ref-AVS基准上达到SOTA总体性能。

Conclusion: PRIMED通过自适应模态抑制有效解决了多模态指代表达中模态相关性动态变化的问题，为参考音视频分割提供了更准确的解决方案。
...
TL;DR: 本文提出S2M框架，直接从变化检测数据集的地面真值掩码中自动提取结构化文本描述（四元组：地点、内容、方式、数量），无需额外标注成本，通过两阶段训练和多模态对比学习实现视觉与文本特征的对齐，在 Gaza-Change-v2 数据集上超越了使用大语言模型的多模态方法。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法中，单模态方法易混淆语义变化与视觉干扰；多模态方法虽引入文本辅助监督，但描述语义粗糙、结构杂乱或包含模型生成的噪声。关键是，所有方法都忽略了真实掩码标签本身已隐式编码了细粒度的变化语义信息（位置、地物类型、过渡方式、对象数量），因此希望以零额外成本从掩码中自动获取结构化文本监督。

Method: 提出S2M框架，将每个变化区域自动转录为语义四元组（where, what, how, how many），并转换为固定模板的文本描述，提供精确、密集、无噪声的多模态监督。采用两阶段训练策略：先在遥感图像上微调获得领域鲁棒表示，再引入多模态解码器与双向对比损失，实现视觉特征与结构化文本嵌入的深度对齐。

Result: 在自构建的多类变化检测数据集 Gaza-Change-v2 上，S2M 达到 Sek=17.80%，F_scd=66.14%，显著超越了包括使用大语言模型的多模态方法在内的现有方法。

Conclusion: 工作表明，变化检测中的掩码标签本身即包含丰富的语义信息，可以直接"说话"，准确告知变化发生的位置、内容、方式和数量，为遥感变化检测提供了一种低成本、高质量的多模态监督范式。
...
TL;DR: 提出SatSurfGS，一种基于2D高斯泼溅的稀疏视角卫星影像表面重建方法，通过粗到细的高斯属性预测框架和三层次局部几何可靠性建模，有效处理多视角匹配的空间异质性，在渲染质量、重建精度、跨数据集泛化和推理效率上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角卫星影像表面重建中，多视角匹配的可靠性存在显著的空间异质性（受大光度差异、弱纹理和重复纹理影响），而现有通用前馈式2DGS框架在该任务上仍属空白。

Method: 构建粗到细的高斯属性预测框架，在三层次（特征学习、高斯参数估计、训练优化）显式建模局部几何可靠性，包含：(1) 置信度感知的单目多视图特征融合模块；(2) 跨阶段自一致性残差引导模块；(3) 置信度双向路由损失函数。

Result: 在卫星数据集上，相比代表性通用基线和场景级优化方法，在渲染质量、表面重建精度、跨数据集泛化能力和推理效率方面均取得更优效果。

Conclusion: 所提出的三层次几何可靠性建模策略有效解决了稀疏视角卫星影像重建中匹配可靠性的空间异质性问题。
...
TL;DR: PicoEyes是一个统一的注视估计框架，能从单目或双目输入直接预测3D眼参数、眼部分割、光轴、视轴和深度图，支持校准、注视预测和不同设备姿态，在混合现实应用中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有注视估计方法通常只能预测部分眼动参数，缺乏统一框架，且难以同时处理校准、预测、不同设备姿态以及3D眼重建等任务，限制了在混合现实中的实用性和泛化能力。

Method: 提出PicoEyes统一框架，以端到端方式联合估计眼参数与深度图，支持单目/双目输入，同时处理校准、注视预测、设备姿态变化，并引入大规模多视角近眼数据集（含训练、测试、重新佩戴、校准等多种会话标注）。

Result: 在无校准、有校准、校准后重戴、预测等多个设置下，PicoEyes均达到最先进性能，一致优于学术界和工业界的注视追踪方法。

Conclusion: 该工作为混合现实应用中鲁棒且可泛化的注视估计建立了一个实用、端到端的范式。
...
TL;DR: 本文发现注意力迁移并非普遍有效：当学生ViT架构与教师不匹配时，即便成功匹配注意力模式也会导致性能下降，架构匹配是注意力迁移成功的关键条件。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明注意力迁移足以恢复教师预训练的全部收益，但该结论是否普遍成立尚不清楚。本文旨在系统检验注意力迁移在不同ViT家族中的有效性，并探究其失败的根本原因。

Method: 在11个ViT家族的20个教师模型上进行系统性基准测试；通过控制实验定位问题来源；通过向学生模型添加教师原生架构组件验证假设；排除迁移损失选择和预训练配方差异等替代解释。

Result: 7个ViT家族迁移成功，4个家族始终失败，性能甚至低于从头训练的基线模型（最高差5.1%）。失败具有家族一致性，且在更长训练、不同数据集和分布外评估下持续存在。问题定位在注意力路由通道，核心原因是架构不匹配导致匹配的注意力模式对学生模型失效。添加教师的原生架构组件可完全逆转失败，但这些组件本身对从头训练无帮助。

Conclusion: 注意力迁移仅在学生架构与教师匹配时才有效，这修正了先前对ViT中注意力表征的理解——注意力本身不足以保证迁移成功，架构匹配是关键前提。
...
TL;DR: 提出了一种异步RGB-事件双相机系统及重建框架，利用事件相机的高时间分辨率信息，通过跨域位姿估计和结构驱动损失函数，实现了在严重运动模糊下的高质量3D重建。
...
Method: 1）从事件数据重建清晰图像；2）基于VGGT的跨域位姿估计模块获得鲁棒的3DGS初始化；3）优化过程中采用结构驱动事件损失和视图一致性正则化，缓解传统事件损失和去模糊损失的病态问题。

Result: 在提出的AsyncEv-Deblur数据集和现有基准上均达到最先进性能，显著提升了严重运动模糊下的重建鲁棒性。

Conclusion: 所提方法有效结合了事件相机的高时间分辨率优势，在缺乏严格同步的条件下仍能实现高质量的3D重建。
...
TL;DR: 提出CLP-DD，一种针对冻结预训练编码器和线性探测场景的闭式数据集蒸馏方法，利用特征空间的核岭回归求解器直接计算线性探测分类器，避免轨迹展开和NTK近似，在ImageNet-1K上以14倍速度提升和1/8显存消耗达到或超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有针对冻结特征线性探测场景的数据集蒸馏方法要么需要展开迭代线性探测更新（轨迹匹配），要么依赖为从头训练设计的NTK近似闭式解，两者都忽略了冻结特征线性探测本就有闭式解这一事实，导致不必要的计算开销。

Method: 提出双层级优化框架：内层用样本空间核岭回归求解器计算合成集诱导的线性探测分类器，外层用温度缩放的softmax交叉熵损失在该分类器上评估真实特征，分类器列向量充当特征空间中的可学习类锚点。

Result: 在ImageNet-100上采用四种预训练骨干，CLP-DD显著优于无DSA的LGM，接近有DSA的LGM且计算成本极低。在ImageNet-1K上，CLP-DD在四种骨干中的三种上匹配或超越有DSA的LGM，同时运行速度快约14倍，GPU显存消耗不到1/8。

Conclusion: 外层损失函数的选择至关重要：内层闭式求解器搭配标准MSE外层损失远不如轨迹方法，而判别性外层损失（温度缩放的softmax交叉熵）几乎弥合了差距，证明了在冻结特征线性探测场景下，闭式蒸馏方法可以兼顾效率与性能。
...
TL;DR: ForeSight是一个基于未来场景想象的自动驾驶规划框架，通过先想象未来再做出决策，实现了从反应式到预期式规划的转变，并在NAVSIM和nuScenes上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端自动驾驶规划器本质上是反应式的，仅依赖历史和当前观测来预测未来动作，缺乏像人类司机那样先模拟"接下来会发生什么"再行动的能力。

Method: 两阶段方法：(1) 利用预训练的世界模型生成合理的未来视觉世界；(2) 基于这些想象出的未来场景进行动作规划。将未来场景想象作为动作预测的主要驱动力。

Result: 在NAVSIM和nuScenes数据集上，显式的未来想象方法显著优于之前的最先进方法。

Conclusion: 通过将决策基于预期的未来情境而非仅当前观测，ForeSight能够更有效地应对动态、交互式的场景，验证了"先见之明"驱动方法的有效性。
...
TL;DR: 本文提出GS-DIFF方法，直接在3D高斯基元空间而非渲染后的2D图像上进行场景变化检测，利用基元自身属性（位置、协方差、颜色）实现多视图一致的变化检测，并区分几何变化和外观变化，在真实基准上mIoU提升约17%。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的场景变化检测方法普遍采用"先渲染后比较"的范式，将问题视为像素层面的比较。本文作者的动机是：这种范式忽略了基元本身的信号价值，且存在多视图不一致、无法区分变化类型等问题。

Method: 提出GS-DIFF方法：1) 直接在基元空间比较高斯泼溅的原始属性（位置、各向异性协方差、颜色）；2) 引入各向异性几何漂移和光度漂移模型，解决独立优化带来的基元不一致问题；3) 设计逐基元可观测性项，反映每个高斯受相机几何约束的程度；4) 分别计算几何变化和外观变化分数。

Result: 在真实世界基准上，GS-DIFF相比先前最先进方法在平均交并比（mIoU）上提升约17%。

Conclusion: 直接操作基元的方法天然具有多视图一致性，无需额外优化目标；能够无监督地区分结构变化（如新增物体）和表面变化（如颜色改变）。
...
### [36] [LoHGNet: Infrared Small Target Detection through Lorentz Geometric Encoding with High-Order Relation Learning](https://arxiv.org/abs/2605.07213)
*Qianwen Ma,Yang Xu,Shangwei Deng,Xiaobo Li,Haofeng Hu*

Main category: cs.CV

TL;DR: 提出LoHGNet，通过洛伦兹几何编码和高阶关系学习来改进红外小目标检测，在三个数据集上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 现有红外小目标检测方法在欧氏空间中进行特征学习，难以描述弱小目标的细微差异及目标与背景之间的上下文关系，导致检测性能受限。

Method: LoHGNet包含两个核心模块：1) 几何注意力引导的洛伦兹残差卷积模块(GA-LRCM)，在双曲几何约束下进行特征建模；2) 高阶关系学习模块(HORL)，通过对数映射将双曲特征映射到欧氏切空间，并通过超图构建建模目标与背景的高阶上下文依赖关系。

Result: 在三个数据集上，LoHGNet在检测精度和复杂场景适应性方面均取得了具有竞争力的性能。

Conclusion: 洛伦兹几何编码能为IRSTD提供新的判别性线索，结合高阶关系学习可有效提升复杂背景下弱小目标的检测能力。
...
### [37] [CASCADE: Context-Aware Relaxation for Speculative Image Decoding](https://arxiv.org/abs/2605.07230)
*Selin Yildirim,Subhajit Dutta Chowdhury,Mohammad Mahdi Kamani,Vikram Appia,Deming Chen*

Main category: cs.CV

TL;DR: 本文提出CASCADE方法，通过识别目标模型在树状推测解码中的语义可互换性和收敛性两种冗余模式，实现接受松弛以加速自回归图像生成，在不牺牲图像质量的前提下达到最高3.6倍加速。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成虽性能优异但计算成本极高，现有推测解码方法在图像生成场景中因目标模型不确定性高、草稿令牌拒绝率高而效率低下，亟需更有效的加速方案。

Method: 形式化定义了目标模型隐藏状态表示中的两种冗余属性——语义可互换性和收敛性，利用这些冗余在预测令牌树的深度和广度上实现接受松弛；同时将冗余信号注入草稿模型训练以提升其性能。

Result: 在多个文本到图像模型和草稿架构上，CASCADE实现了最高3.6倍的加速，并在图像质量和文本提示保真度上保持SOTA水平。

Conclusion: 通过挖掘目标模型内在的冗余结构，无需额外训练即可实现推测解码效率的大幅提升，为自回归图像生成加速提供了新的思路。
...
### [38] [Towards multi-modal forgery representation learning for AI-generated video detection and localization](https://arxiv.org/abs/2605.07232)
*Dat Le,Khoa Nguyen,Xin Wang,Shu Hu*

Main category: cs.CV

TL;DR: 提出了一种联合大语言模型语义分支、时空视觉分支和多尺度局部伪造音频分支的多模态架构，用于检测和细粒度时间定位AI生成的局部伪造视频。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成视频检测方法受限于单一或部分模态建模，且缺乏细粒度的时间伪造定位能力，难以应对日益复杂的局部伪造视频威胁。

Method: 构建核心架构，联合三个分支：LMM语义分支、时空（ST）视觉分支和多尺度局部伪造（PS）音频分支，实现多模态协同检测与时间定位。

Result: 大量实验表明该方法在检测和细粒度定位方面优于现有最先进方法。

Conclusion: 多模态联合架构能够有效检测并精确定位AI生成的局部伪造视频，为视频内容真实性验证提供了更可靠的解决方案。
...
### [39] [Hard to Read, Easy to Jailbreak: How Visual Degradation Bypasses MLLM Safety Alignment](https://arxiv.org/abs/2605.07250)
*Zhixue Song,Boyan Han,Yiwei Wang,Chi Zhang*

Main category: cs.CV

TL;DR: 研究发现，多模态大模型在视觉上下文压缩时，降低图像分辨率会意外引发越狱攻击（安全防御能力急剧下降），原因是“认知过载”——模型处理模糊输入时分散了安全审计的注意力。提出“结构化认知卸载”策略，通过分离视觉转录与安全评估来缓解风险。


<details>
  <summary>Details</summary>
Motivation: 揭示视觉上下文压缩范式下，降低图像分辨率会催化越狱攻击这一关键漏洞，并探究其根本原因。

Method: 通过实验观察分辨率降低对不同模型安全防御的影响；提出“认知过载”假说解释现象；设计“结构化认知卸载”策略，构建串行化流程将视觉转录与安全评估解耦。

Result: 随着分辨率下降，SOTA模型的安全防御能力急剧恶化，即使在文本仍可辨认的情况下；该现象在噪声、几何畸变等视觉扰动下一致存在；提出的策略能有效缓解风险。

Conclusion: 视觉压缩技术存在显著安全风险，“认知过载”是核心原因，“结构化认知卸载”为未来MLLM的安全设计提供了重要方向。
...
### [40] [High-Fidelity Surface Splatting-Based 3D Reconstruction from Multi-View Images](https://arxiv.org/abs/2605.07254)
*Nandhana Sunil,Abhirami R Iyer,Avirup Mandal*

Main category: cs.CV

TL;DR: 本文针对多视角网格重建中高频几何信息恢复困难的问题，提出了一种基于改进隐式移动最小二乘（IMLS）的方法。通过引入紧凑多项式核替代传统指数核，并结合随机正则化与拉普拉斯滤波，实现了更优的高频细节保持和稳定优化，在表面重建与渲染任务上均取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如3DGS、NeRF）需要后处理才能提取网格，限制了几何与外观的联合优化；而IMLS方法虽支持端到端重建，但其指数核难以捕捉高频几何细节。

Method: 1) 用局部支持的紧凑多项式核替代指数核，提升频率内容控制能力与几何保真度；2) 引入基于拉普拉斯滤波的随机正则化，增强高频结构保持与优化稳定性。

Result: 在表面重建与渲染任务上均达到最先进水平，几何精度更高，视觉效果更锐利。

Conclusion: 所提方法有效解决了IMLS在高频细节恢复上的不足，为多视角网格重建提供了更优的端到端解决方案。
...
### [41] [TAS-LoRA: Transformer Architecture Search with Mixture-of-LoRA Experts](https://arxiv.org/abs/2605.07256)
*Jeimin Jeon,Hyunju Lee,Bumsub Ham*

Main category: cs.CV

TL;DR: 提出TAS-LoRA方法，通过引入LoRA和混合专家策略解决Transformer架构搜索中的特征坍缩问题，提升子网性能。


<details>
  <summary>Details</summary>
Motivation: 现有TAS方法由于超网中权重共享导致特征坍缩问题，子网无法学习特定特征，限制了性能。

Method: 引入参数高效的LoRA实现子网特定特征学习；采用Mixture-of-LoRA-Experts策略，用轻量路由器根据子网架构动态分配LoRA专家；提出分组路由器初始化技术促进早期多样化特征学习。

Result: 在ImageNet及CIFAR-10/100、Flowers、CARS、INAT-19等迁移学习基准上，有效缓解特征坍缩，显著超越SOTA TAS方法。

Conclusion: TAS-LoRA通过LoRA和MoLE策略有效解决了TAS中的特征坍缩问题，提升了子网性能。
...
### [42] [Adaptive Subspace Projection for Generative Personalization](https://arxiv.org/abs/2605.07257)
*Van-Anh Nguyen,Anh Tuan Bui,Tamas Abraham,Junae Kim,Amardeep Kaur,Rollin Omari,Thuy-Trang Vu,Dinh Phung*

Main category: cs.CV

TL;DR: 本文提出AdaptSP方法，通过自适应子空间投影来隔离和修正个性化生成中的语义漂移，解决了语义坍塌问题（SCP），在保持主体身份的同时提升了提示保真度。


<details>
  <summary>Details</summary>
Motivation: 生成式个性化中，学习到的个性化概念会压制文本提示的其他部分，导致模型忽略重要的上下文细节，即语义坍塌问题（SCP）。

Method: 提出测试时嵌入调整与自适应子空间投影（AdaptSP），一种无需训练的方法。利用稳定的预训练嵌入作为锚点，隔离语义漂移并将其投影到特定低维子空间中，进行精确调整。

Result: 实验表明，该方法显著提升了提示保真度和上下文对齐能力。

Conclusion: SCP的语义漂移并非随机，而是集中在特定的低维子空间中；个性化过程会扰动原始基础概念的嵌入，使其变得不稳定。AdaptSP通过精准的子空间投影调整有效缓解了SCP。
...
### [43] [From Clouds to Hallucinations: Atmospheric Retrieval Hijacking in Remote Sensing Vision-Language RAG](https://arxiv.org/abs/2605.07273)
...
TL;DR: 本文提出CloudWeb攻击，通过在遥感图像上叠加参数化的云雾模式，劫持多模态RAG系统的证据检索阶段，使检索结果偏向目标大气文本证据，从而影响下游生成结果。这是首个针对遥感多模态RAG检索阶段的攻击研究。


<details>
  <summary>Details</summary>
Motivation: 现有对抗研究主要操纵检索语料库或针对端任务预测，而针对遥感多模态RAG系统中证据检索阶段的输入空间攻击尚属空白。本文旨在填补这一研究缺口。

Method: 在遥感图像上叠加参数化的云雾图案，通过检索导向的目标函数优化这些图案，使对抗性图像嵌入拉向目标大气证据、压制源场景证据、强制排序分离，并正则化自然度和覆盖率。

Result: 在7个数据集、5种CLIP风格检索器上，CloudWeb持续优于清洁检索和多种基线。在GeoRSCLIP ViT-B/32上，Weather@5从0.71%提升至43.29%。下游生成出现可测量的天气幻觉和语义偏移。

Conclusion: 自然外观的大气变化可以在生成开始前就破坏证据检索，揭示了一个实际的安全漏洞。
...
### [44] [SplatWeaver: Learning to Allocate Gaussian Primitives for Generalizable Novel View Synthesis](https://arxiv.org/abs/2605.07287)
*Yecong Wan,Fan Li,Mingwen Shao,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出SplatWeaver，一种可泛化的新视角合成框架，通过动态分配高斯原语数量（而非固定数量），实现更紧凑且高质量的3D场景表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的前馈方法为每个像素或体素分配固定数量的高斯原语，忽略了真实场景的空间复杂度变化，导致平滑区域浪费原语，而精细结构区域容量不足。

Method: 引入基数高斯专家（从0到M的原语数量）和像素级路由方案，配合高频先验引导模块和路由正则化，动态决定每个空间位置分配的高斯原语数量。

Result: 在多种场景下，SplatWeaver以更少的高斯原语数量，持续超越现有最佳方法，生成更忠实的新视角渲染结果。

Conclusion: 通过复杂度感知的稀疏分配策略，SplatWeaver实现了更高效且表达能力更强的可泛化新视角合成。
...
### [45] [Sword: Style-Robust World Models as Simulators via Dynamic Latent Bootstrapping for VLA Policy Post-Training](https://arxiv.org/abs/2605.07288)
*Jiaxuan Gao,Yongjian Guo,Zhong Guan,Wen Huang,Wanlun Ma,Xi Xiao,Junwu Xiong,Sheng Wen*

Main category: cs.CV

TL;DR: 提出Sword框架，通过结构引导风格增强和动态潜在引导方法，解决世界模型在LIBERO基准中的泛化差和长程误差累积问题，显著提升VLA模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型作为生成模拟器时，存在泛化能力差、对初始状态扰动敏感（颜色/光照微小变化引发级联幻觉）、长程误差累积导致预测质量下降等问题，限制了其在具身环境中的可靠性。

Method: 1. 结构引导风格增强：解耦交互环境的视觉纹理与任务相关动态，提升泛化能力。2. 动态潜在引导：保持训练与推理一致性，同时控制内存消耗。

Result: 在LIBERO基准上，Sword在泛化性、生成质量、鲁棒性、保真度及VLA模型的后训练强化学习成功率上显著超越基线WoVR。

Conclusion: Sword框架有效缓解了世界模型作为模拟器时的关键缺陷，提升了其在具身智能任务中的可靠性。
...
### [46] [EgoPro-Bench: Benchmarking Personalized Proactive Interaction in Egocentric Video Streams](https://arxiv.org/abs/2605.07299)
*Dongchuan Ran,Linyu Ou,Xueheng Li,Wenwen Tong,Chenxu Guo,Hewei Guo,Kaibing Wang,Lewei Lu*

Main category: cs.CV

TL;DR: 本文提出了EgoPro-Bench，一个基于流式自我中心视频的基准，用于训练和评估多模态大语言模型的主动交互能力，包含2400个评估视频和12000+训练视频，覆盖12个领域，并提出了"短思考、更好交互"原则来优化交互时机。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要是被动响应的，缺乏持续感知环境和主动辅助用户的能力；已有基准局限于告警场景、忽略个性化上下文、未能评估交互的精确时机。

Method: 利用模拟用户画像生成多样化用户意图，构建12个领域的高保真人机交互数据；提出专门评估协议和指标；训练面向流式视频数据的低延迟主动交互模型。

Result: EgoPro-Bench显著增强了MLLMs的意图理解能力，能够准确识别人机交互的合适时机。
...
### [47] [Amortized-Precision Quantization for Early-Exit Vision Transformers](https://arxiv.org/abs/2605.07317)
*Rui Fang,Hsi-Wen Chen,Ming-Syan Chen*

Main category: cs.CV

TL;DR: 提出MAQEE框架，通过联合优化退出阈值和位宽，在低精度早期退出场景下实现精度与效率的帕累托最优，减少95%的计算量。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法假设静态全深度执行，在早期退出场景下量化噪声会扰动退出决策并放大误差，导致不稳定。

Method: 提出APQ公式化层间随机量化噪声暴露与深度-精度权衡；基于此设计MAQEE双层框架，在显式风险控制下联合优化退出阈值和位宽。

Result: MAQEE在分类、检测、分割任务中减少95% BOPs，精度保持优于基线最多20%。

Conclusion: MAQEE建立了精度-效率权衡的优越帕累托前沿，有效提升了低精度早期退出的推理稳定性。
...
### [48] [GEM: Generating LiDAR World Model via Deformable Mamba](https://arxiv.org/abs/2605.07326)
*Yang Wu,Zhaojiang Liu,Qiang Meng,Youquan Liu,Renliang Weng,Jianjun Qian,Jian Yang,Jin Xie*

Main category: cs.CV

TL;DR: GEM是一个基于可变形Mamba架构的生成式LiDAR世界模型，通过LiDAR点云分词器、动态-静态分离器和三路径可变形Mamba模块，有效解决了LiDAR点云无序性和动静态物体区分难题，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR世界模型相比基于相机或占用数据的世界模型发展滞后，主要面临两个核心挑战：LiDAR点云固有的无序性以及难以区分动态物体与静态结构。

Method: 1) 利用激光雷达顺序扫描与Mamba处理机制的结构相似性，设计LiDAR场景分词器将扫描数据压缩为紧凑表示；2) 通过动态-静态分离器对分词特征进行无监督解耦；3) 引入三路径可变形Mamba，对解耦特征进行选择性扫描和自适应门控融合，增强时空理解；4) 可选集成规划器和BEV布局控制器以探索自主推演和"what-if"场景生成。

Result: 在多个基准和评估设置下达到SOTA性能，展现出优越性和有效性。

Conclusion: GEM有效解决了LiDAR世界模型的两大核心难题，显著提升了LiDAR世界模型的保真度和想象力能力，并具备自主推演和假设场景生成潜力。
...
### [49] [GC-ART: Global Learnable Second-Order Rational Tone Curves for Illumination Robustness](https://arxiv.org/abs/2605.07329)
*Wei Huang,Joyce Huang*

Main category: cs.CV

TL;DR: GC-ART是一个轻量级、可微分的预处理模块，通过预测端点固定的有理色调曲线来增强图像分类的鲁棒性，在保持干净准确率的同时，在某些对比度损坏场景下优于基线和其他学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有图像增强方法要么计算开销大，要么在保持边缘结构方面存在不足，难以在轻量级条件下同时提升分类鲁棒性和保持干净准确率。

Method: 提出GC-ART模块，利用每通道软直方图通过643参数的MLP预测端点固定的有理色调曲线，逐点应用曲线进行预处理，使用交叉熵损失和软单调性惩罚进行端到端训练。

Result: 在CIFAR-10上，干净准确率与基线持平；在乘法变暗任务上优于基线；在对比度损坏任务上达到48.45%，优于基线的46.27%和Zero-DCE++的47.13%。同时FLOPs远低于卷积类增强方法。

Conclusion: 基于直方图条件的有理曲线能够学习有用的全局色调校正（包括对比度扩展行为），通过逐点映射天然保留边缘位置，且当前超参数未调优，仍有改进空间。
...
### [50] [RCoT-Seg: Reinforced Chain-of-Thought for Video Reasoning and Segmentation](https://arxiv.org/abs/2605.07334)
*Junwei Wen,Deshui Miao,Guangming Lu,Xin Li,Wenjie Pei*

Main category: cs.CV

TL;DR: RCoT-Seg是一个视频推理分割框架，通过将任务分解为时间视频推理（TVR）和关键帧目标感知（KTP）两个阶段，并引入基于GRPO的智能关键帧选择模块，显著提升了视频分割的时序理解和空间精度。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法在视频推理分割中依赖简单采样或辅助MLLM选择关键帧，导致监督有限、帧-语言相似性规则狭隘，难以处理复杂多目标场景中的时序理解和鲁棒定位。

Method: 提出RCoT-Seg框架，将VRS分解为TVR和KTP两个阶段。TVR阶段采用基于CoT-start语料初始化、经GRPO优化的智能关键帧选择模块，通过自我评估生成和重新选择关键帧；KTP阶段对选定帧进行高分辨率分割，并利用SAM2方法跨序列传播掩码。

Result: 大量实验结果表明，RCoT-Seg在视频推理分割任务上取得了优于现有最先进方法的性能。

Conclusion: RCoT-Seg通过显式分离时序推理与空间感知，有效提升了视频推理分割的时序理解和空间精度，为复杂视频场景的细粒度分割提供了新范式。
...
TL;DR: SplitGS-Loc通过混合高斯分解和视角一致性选择，解决了基于高斯特征场的2D-3D匹配歧义问题，实现了无需逐场景训练的高精度视觉定位。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯特征场的视觉定位方法存在两个根本问题：1）高斯体的体积特性导致多对一的像素-点映射，破坏PnP姿态估计稳定性；2）光度优化产生大量缺乏多视角一致性的冗余高斯体。因此需要专门为定位任务设计的高斯特征场构建框架。

Method: 1）混合高斯分解：将每个高斯体分解为多个更小的高斯体，将模糊的多对一映射替换为精确的一对一映射；2）利用高斯光栅化中的组合权重，选择在多视角中持续显著贡献的高斯体，并通过强像素-高斯关联聚合判别性特征，强制多视角一致性。

Result: 在多个定位基准数据集上达到最先进性能，实现了稳定的PnP收敛。

Conclusion: SplitGS-Loc通过利用高斯属性扩展了光度优化的高斯特征场在精确高效定位中的实用性，无需逐场景训练或迭代姿态优化。
...
### [52] [TTF: Temporal Token Fusion for Efficient Video-Language Model](https://arxiv.org/abs/2605.07355)
*Simin Huo,Ning LI*

Main category: cs.CV

TL;DR: TTF是一种无需训练的即插即用视觉token压缩框架，通过时序冗余利用和局部窗口相似性搜索，在Qwen3-VL-8B上移除约67%的视觉token，同时保持99.5%的基线准确率，仅引入约0.16 GFLOPs的匹配开销。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型随视频长度增加，视觉token数量激增（如32帧448×448分辨率产生超过8000个token），导致LLM预填充阶段成为吞吐瓶颈。现有方法依赖全局相似性或注意力引导压缩，但带来额外开销。

Method: TTF首先自动选择锚帧，然后对后续每个帧进行局部窗口（如3×3）相似性搜索，融合超过阈值的相邻token，并通过坐标对齐保持预填充和解码阶段的位置一致性。
...
### [53] [UniD-Shift: Towards Unified Semantic Segmentation via Interpretable Share-Private Multimodal Decomposition](https://arxiv.org/abs/2605.07356)
*Shuai Zhang,Zhecheng Shi,Zhuxiao Li,Jing Ou,Tengxi Wang,Yuan Liu,Wufan Zhao*

Main category: cs.CV

TL;DR: 提出UniD-Shift统一多模态框架，通过显式分解共享与私有子空间实现2D-3D点云语义分割的跨模态对齐与融合，在SemanticKITTI和nuScenes上取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 激光雷达稀疏采样和图像几何畸变导致跨模态对齐困难，现有融合方法不稳定。受2D图像是3D世界投影的启发，发现2D和3D特征既有共享语义也有模态特有属性，需显式建模这种关系。

Method: SAM视觉编码器+SPTNet几何编码器分别提取语义和几何特征；将双模态特征分解为共享子空间和私有子空间；轻量注意力融合模块聚合共享特征；正则化训练目标保证语义对齐和子空间独立性。

Result: 在SemanticKITTI和nuScenes上优于主流多模态基线，保持竞争性计算效率；跨域评估（nuScenes USA-Singapore）在分布偏移下表现稳定，泛化性强。

Conclusion: 显式分解共享/私有子空间是跨模态融合的有效策略，所提框架在精度、效率和泛化性上均具优势，代码开源。
...
TL;DR: 针对水下机器人检测海洋垃圾时图像模糊、背景复杂、目标小等挑战，提出YOLO-MD框架，通过双分支卷积增强自注意力模块、轻量级移位操作和SFG-Loss动态样本加权，在UODM数据集上达到0.875精度、0.822 F1和0.849 mAP50，超越现有方法，并在真实机器人边缘部署中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 海洋垃圾检测对生态保护至关重要，但现有方法在低质量图像（模糊、复杂背景、小目标）下性能严重退化。

Method: 1) DB-CASA双分支卷积增强自注意力模块，强化空间-通道交互；2) 轻量级移位操作提升多尺度细粒度特征提取；3) SFG-Loss动态样本重加权，缓解类别不平衡和优化不稳定问题。

Result: 在UODM数据集上达到0.875精度、0.822 F1-score和0.849 mAP50，优于最新SOTA方法，并在真实机器人边缘部署实验中验证了有效性。

Conclusion: YOLO-MD有效解决了低质量海洋图像中的垃圾检测难题，兼具高性能与参数效率，适合机器人边缘部署场景。
...
TL;DR: 提出ST-Gen4D框架，通过时空认知（全局外观图+局部动态图）引导4D高斯生成，实现结构合理性与拓扑一致性。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成模型仅保证全局外观一致性，无法揭示物理世界的局部动态，缺乏4D时空认知能力。

Method: 1) 时空表征：多模态编码为特征基；2) 时空认知：构建全局外观图与局部动态图，通过语义桥接融合为4D认知图；3) 时空推理：基于世界模型预测未来状态；4) 时空生成：以认知图为条件引导潜扩散模型进行4D高斯生成。

Result: 在3D和4D生成任务上均优于现有方法，证明了结构合理性与拓扑一致性的优势。

Conclusion: 深度融合4D内在认知与生成先验，能有效保证4D生成的结构合理性与拓扑一致性，并提出了ST-4D数据集。
...
TL;DR: 提出一种基于多目标强化学习的图像描述生成框架，通过GDPO风格奖励解耦归一化和长度条件奖励掩码，在多个基础模型上显著提升描述质量。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述的强化学习方法在优化过程中往往偏向单一质量维度（如实用性或流畅性），导致描述存在幻觉、冗长或过于泛化等问题，缺乏对正确性、覆盖度和语言质量的平衡优化。

Method: 1. 提出联合优化实用性正确性、参考覆盖度和语言质量的多目标奖励框架；2. 应用GDPO风格奖励解耦归一化处理连续值奖励；3. 引入长度条件奖励掩码，实现更合理的长度惩罚。

Result: 在LLaVA-1.5-7B、Qwen2.5-VL 3B和7B模型上，DCScore提升+13.6，CaptionQA提升+9.0，CapArena提升+29.0。

Conclusion: 所提方法能有效平衡图像描述的多维度质量，优于传统GRPO等基线方法。
...
TL;DR: 本文发现时空深度伪造检测器易受规避攻击，因为模型过度依赖不稳定的时域频谱线索而非鲁棒的语义因果关系。为此提出SpInShield框架，通过可学习的频谱对手和捷径抑制优化策略，解耦语义运动与可操纵的频谱伪影，在频谱攻击下AUC比最强基线提升21.30个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有时空深度伪造检测器虽然在AUC上表现良好，但容易受到规避攻击，原因是模型过度拟合脆弱的时域频谱线索，而没有学习到鲁棒的语义因果关系。

Method: 提出SpInShield防御框架，包含两个核心组件：1）可学习的频谱对手，动态合成严重的频谱变形以模拟极端攻击场景；2）捷径抑制优化策略，强制编码器提取可靠的取证线索，同时从潜在空间中清除不稳定的频谱统计信息。

Result: 在广泛使用的数据集上取得有竞争力的性能，在模拟幅度频谱攻击下，AUC比最强基线高出21.30个百分点。

Conclusion: SpInShield通过解耦语义运动与可操纵的频谱伪影，有效增强了深度伪造检测器对规避攻击的鲁棒性。
...
TL;DR: 本文揭示了扩散视觉语言模型（dVLMs）存在一种独特的安全漏洞：由于渐进式拒绝机制，传统越狱手段（固定前缀优化）往往失败，但去噪过程本身暴露了新的攻击面。作者提出全局概率优化（GPO）范式及其视觉版本GPO-V，通过操纵全局生成动力学绕过安全护栏，实验表明该方法具有强跨模型迁移能力，亟需重新评估当前防御体系。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型（dVLMs）虽看似对传统越狱攻击具有鲁棒性，但研究发现这种鲁棒性是虚假的。去噪过程的渐进式拒绝模式暴露了潜在攻击面，现有防御机制存在严重安全缺口。

Method: 提出全局概率优化（GPO）——一种针对掩码扩散模型去噪轨迹的通用越狱范式，不依赖前缀而操纵全局生成动态。在此基础上构建GPO-V，首个针对dVLMs的视觉模态越狱框架。

Result: GPO-V能生成具有出色跨模型迁移能力的隐蔽扰动，成功揭示了非序列生成架构中的关键安全漏洞。

Conclusion: 当前扩散模型的防御范式必须进行根本性重新评估，以应对基于扩散生成架构的独特安全风险。
...
TL;DR: 提出InsHuman方法，通过人-背景自适应融合、人脸身份保持和双向数据配对策略，实现自然且身份保持的人体插入。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型在人体插入任务中存在人体姿态不自然、人数不一致、面部身份改变等问题，且公开数据集缺乏全身肖像和真实的人-背景物理交互。

Method: 1) 人-背景自适应融合(HBAF): 检测前景人体获取二值掩码，进行区域感知加权对齐生成和真实潜空间中的人体区域；2) 面部身份保持(FFIP): 检测并匹配生成图像与源图像的面部识别特征，确保身份一致性；3) 双向数据配对(BDP): 构建高质量的BDP-InsHuman数据集，提供真实的人-背景交互。

Result: InsHuman在生成合理图像的同时保持人类身份不变方面取得了显著改进。

Conclusion: InsHuman通过HBAF、FFIP和BDP三个模块有效解决了人体插入任务中的姿态、数量和身份保持问题。
...
TL;DR: 本文提出了一个全面的图表指代表达式基准测试，支持多种定位形式、多个目标、多样化线索和图表类型，并通过代码驱动合成管道生成像素级实例掩码，训练实例分割模型以提升多模态大模型在图表指代定位上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有图表指代表达式基准存在局限：仅使用边界框限制细粒度定位精度、仅处理单个或两个目标实例、语言表达过度依赖文本或数据排序线索、图表类型覆盖狭窄。

Method: 构建全面基准测试，包含多种定位形式、多目标、多样化线索和图表类型；提出代码驱动合成管道，利用绘图程序与渲染图元间的对齐关系生成像素级实例掩码；训练实例分割模型并集成到通用多模态定位框架中。

Result: 多模态大模型在基准上表现差距显著；所提系统在基准和ChartQA真实图表基准上一致优于基线方法。

Conclusion: 该基准和合成-训练方法有效提升图表指代定位的精度和泛化能力，弥补了现有基准和模型的不足。
...
TL;DR: 提出了一种基于图像自适应尺度场建模的方法，通过将度量深度恢复表示为低维线性组合，从稀疏锚点高效恢复准确度量深度，具有高精度、强鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计通常只能得到未知尺度的深度，而实际应用需要精确的度量深度。现有方法在只有稀疏度量锚点时难以准确恢复深度，因此需要一种高效且鲁棒的度量深度恢复方法。

Method: 将度量深度恢复建模为图像自适应尺度场估计：从MDE估计和中间表示中提取语义和几何线索，生成一组图像自适应基图；将深度校正表示为这些基图的低维线性组合；通过稀疏度量锚点求解最小二乘问题确定基图权重。

Result: 在多个数据集和多种MDE模型上验证，所提方法在度量深度精度、对极端稀疏锚点的鲁棒性方面均优于现有方法，且能提供可解释的空间尺度变化分解。

Conclusion: 该方法通过图像自适应基图组合的方式，高效利用稀疏锚点恢复度量深度，具有通用性强、鲁棒性好且可解释的优点，适用于多种单目深度估计模型和实际应用场景。
...
TL;DR: MagicBokeh是一个统一的扩散框架，通过联合优化散景渲染和超分辨率，实现了在低分辨率、高数码变焦照片上的高质量、高效散景效果，并引入了焦点感知掩码注意力和退化感知深度模块。


<details>
  <summary>Details</summary>
Motivation: 现有移动设备因紧凑光学设计（小光圈）难以产生自然逼真的散景效果；学习型方法在高数码变焦的低分辨率照片上表现不佳；简单的先增强图像再渲染散景的两阶段方案效率低且存在误差累积。

Method: 提出MagicBokeh统一扩散框架，采用交替训练策略和焦点感知掩码注意力机制联合优化散景渲染与超分辨率，并引入退化感知深度模块以从低质量输入中准确估计深度。

Result: 在真实世界低分辨率图像上高效生成照片级逼真的散景效果，显著优于现有方法。

Conclusion: MagicBokeh为未来散景渲染领域的进步铺平了道路，代码和模型已开源。
...
TL;DR: 提出SAEgis框架，利用稀疏自编码器（SAE）检测视觉语言模型（VLM）中的对抗性攻击，无需额外对抗训练，实现高效、轻量级的攻击检测。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）在现实应用中部署日益增多，但其安全性研究相对不足，现有VLM（包括闭源和开源）仍易受对抗性攻击，存在显著安全风险。

Method: 在预训练VLM中插入稀疏自编码器（SAE）模块，使用标准重构目标进行训练，利用学习到的稀疏潜在特征捕捉攻击相关信号，实现对抗性扰动输入的分类检测。

Result: 在域内、跨域和跨攻击设置下均表现优异，尤其在跨域泛化方面显著优于现有基线；多层级信号融合进一步提升了鲁棒性和稳定性。

Conclusion: SAEgis是首个将SAE作为即插即用机制用于VLM对抗攻击检测的工作，无需对抗训练、开销小，为提升真实VLM系统安全性提供了实用方案。
...
TL;DR: EditTransfer++是一个结合渐进结构化训练与高效条件机制的图像编辑框架，通过解耦文本条件、最差-最好对比精炼和条件压缩复用策略，在保持高质量视觉提示忠实度的同时大幅提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散Transformer的视觉提示编辑方法存在结构不匹配问题，包括预训练偏向文本条件以及采样过程中的随机不稳定性，导致编辑结果无法忠实再现示例变换。

Method: 1) 文本解耦训练策略：微调时去除文本条件，迫使模型仅从视觉证据推断变换；2) 最差-最好对比精炼机制：重塑去噪轨迹，抑制不忠实生成并提升种子间一致性；3) 条件压缩与复用策略：减少token冗余，支持1024像素长边的高效生成。

Result: 在现有基准和提出的EditTransfer-Bench上，EditTransfer++达到了最先进的视觉提示忠实度，同时推理速度显著快于先前方法。
...
### [65] [EditRefiner: A Human-Aligned Agentic Framework for Image Editing Refinement](https://arxiv.org/abs/2605.07457)
...
TL;DR: 本文提出EditRefiner，一种基于感知-推理-行动-评估循环的分层智能体框架，用于修复文本引导图像编辑后的细粒度瑕疵。同时构建了包含15K图像、60K标注区域和45K评分的大规模细粒度人工反馈数据集EditFHF-15K。实验表明EditRefiner在瑕疵定位、诊断准确率和感知对齐方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像编辑模型虽取得进展，但编辑结果仍存在不自然物体、光照不匹配等细粒度问题。已有的优化方法要么依赖昂贵迭代重新生成，要么使用空间定位能力弱的视觉语言模型，导致语义漂移和局部修复不可靠。

Method: (1) 构建EditFHF-15K数据集，含15K图像、60K伪影区域、80K编辑失败区域及文本推理、45K平均意见分数；(2) 提出EditRefiner框架，包括四个智能体：感知智能体（检测上下文显著性图）、推理智能体（进行类人诊断）、行动智能体（规划并执行局部重编辑）、评估智能体（评估结果并决定是否需要进一步优化）。

Result: EditRefiner在失真定位、诊断准确率和人类感知对齐方面持续优于现有最先进方法。

Conclusion: EditRefiner建立了一种自纠正、感知可信的图像编辑新范式，代码已开源。
...
TL;DR: 本文提出共识熵（CE），一种无需训练、与模型无关的度量标准，通过测量模型间共识熵来估计输出可靠性，并基于此构建CE-OCR框架，在OCR质量验证上F1提升42.1%，无需监督即可即插即用。


<details>
  <summary>Details</summary>
Motivation: 现有OCR模型在平均准确率上虽有进步，但VLM在检测逐样本错误方面仍存在困难，且缺乏有效的无监督质量控制方法。

Method: 提出共识熵（CE），通过测量多个模型输出之间的共识熵来估计可靠性；基于CE构建CE-OCR框架，包括集成一致性验证、最优输出选择、以及自适应路由来提升效率。

Result: CE在质量验证上F1分数比VLM-as-Judge方法提升42.1%；CE-OCR在相同成本下持续优于自一致性和单模型基线。

Conclusion: CE无需训练或监督，可作为即插即用模块提升OCR质量验证和输出可靠性。
...
### [67] [A Unified Framework for the Detection and Classification of Fatty Pancreas in Ultrasound Images](https://arxiv.org/abs/2605.07466)
...
TL;DR: 提出了一种基于TransUNet分割和纹理分析的端到端框架，用于从腹部超声图像自动分类正常胰腺与脂肪胰，在214张图像数据集上达到89.7%的准确率。


<details>
  <summary>Details</summary>
Motivation: 非酒精性脂肪胰（NAFPD）是尚未被充分诊断的疾病，现有诊断依赖 clinicians 对超声图像的主观视觉评估，缺乏自动化、客观的定量分析方法。

Method: 采用TransUNet（ResNet编码器+Transformer瓶颈）分割胰腺和脾静脉，基于解剖结构提取斑块，通过配对纹理比较进行患者级分类；分割模型通过肝脏分割任务进行领域特定迁移学习初始化。

Result: SVM（RBF核）平均交叉验证准确率89.7%±1.8%，F1分数0.898±0.019；无监督K-Means基线准确率87.8%。

Conclusion: 首次实现从超声图像对脂肪胰进行端到端自动化分类，所提特征即使无标注训练数据也能捕捉相关临床信号。
...
### [68] [ForgeVLA: Federated Vision-Language-Action Learning without Language Annotations](https://arxiv.org/abs/2605.07474)
...
<details>
  <summary>Details</summary>
Motivation: VLA模型的扩展受限于标注训练数据的高成本，而现实中部署的机器人产生的视觉-动作对无法集中聚合且存在严重异质性，亟需一种高效的分布式训练方法。

Method: 提出ForgeVLA框架：每个客户端配备具身指令分类器将视觉-动作对映射到预定义指令集以恢复语言模态；结合客户端对比规划损失与服务器端自适应聚合策略，解决视觉-语言特征坍塌问题。

Result: 在多个基准测试上，ForgeVLA显著优于其他基线方法，消融实验验证了各组件的有效性。

Conclusion: ForgeVLA有效实现了分布式VLA模型的联邦训练，无需集中数据或人工标注，为大规模机器人智能提供了一条可行的扩展路径。
...
### [69] [ReasonEdit: Towards Interpretable Image Editing Evaluation via Reinforcement Learning](https://arxiv.org/abs/2605.07477)
*Honghua Chen,Zitong Xu,Huiyu Duan,Xinyun Zhang,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: 本文提出了ReasonEdit-22K数据集和RE-Reward奖励模型，用于训练可解释的图像编辑评估模型ReasonEdit，该模型能生成高质量的可解释评估文本，与人类偏好高度对齐。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像编辑评估方法依赖标量分数，缺乏可解释性，主要原因在于缺少高质量的解释数据集和有效的奖励模型来训练可解释的评估器。

Method: 构建包含22K编辑图像和113K思维链样本的ReasonEdit-22K数据集，以及1.3M人工判断；提出基于MLLM的RE-Reward奖励模型；利用GRPO算法训练可解释评估模型ReasonEdit。

Result: ReasonEdit在人类偏好对齐上表现优异，在公开基准上泛化能力强，并能生成高质量的可解释评估文本。
...
### [70] [AudioFace: Language-Assisted Speech-Driven Facial Animation with Multimodal Language Models](https://arxiv.org/abs/2605.07478)
*Kai Zheng,Zejian Kang,Rui Mao,Hongyuan Zou,Yuanchen Fei,Xuanyang Xu,Xiangru Huang*

Main category: cs.CV

TL;DR: 提出AudioFace框架，利用多模态大语言模型的语言先验知识，结合文本和音素级线索，将语音驱动的面部动画转化为语言辅助的结构化生成问题，在多个评估指标上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动面部动画方法直接映射声学信号到面部系数，忽略了语音产出背后的语言和音韵结构，导致与发音相关的嘴部运动不够精确。

Method: 提出AudioFace框架，引入多模态大语言模型的先验知识，利用文本和音素级线索作为桥梁，将嘴部面部系数预测视为由语言和发音信息引导的结构化生成问题。

Result: 在多个评估指标上均取得优于现有方法的性能表现。
...
### [71] [How Far Is Document Parsing from Solved? PureDocBench: A Source-TraceableBenchmark across Clean, Degraded, and Real-World Settings](https://arxiv.org/abs/2605.07492)
...
TL;DR: 本文指出现有文档解析基准OmniDocBench存在标注错误和污染风险，并提出新的程序化生成、源头可追溯的基准PureDocBench，在40个模型上的评估表明文档解析远未解决，最佳模型仅约74分，公式识别是共同瓶颈，且仅用干净数据评估会产生误导。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析基准OmniDocBench的标注质量存疑（12.08%错误率）且存在数据污染风险，导致排行榜排名不可靠，亟需更高质量的评估基准。

Method: 提出PureDocBench，通过HTML/CSS程序化生成文档图像并同步产生可验证的标注，覆盖10个领域、66个子类、1,475页，每页包含干净、数字退化、真实退化三个版本（共4,425张图像），并设计了三级审计管线。

Result: 40个模型评估结果显示：最佳模型仅约74分；≤4B参数的专业解析器可匹敌或超越大5-100倍的通用VLM；公式识别是所有模型的共同瓶颈（最高仅67%）；通用VLM在退化场景下鲁棒性优于专业解析器，导致仅基于干净数据的评估排名具有误导性。

Conclusion: 文档解析任务远未解决，现有基准存在质量问题，PureDocBench提供了更可靠的评估方式，揭示了模型真实性能差距和鲁棒性问题。
...
### [72] [DIMoE-Adapters: Dynamic Expert Evolution for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07494)
*Mengxin Qin,Xiang Zhang,Xi Wang,Kun Wei,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出 DIMoE-Adapters 框架，通过动态专家进化范式在持续学习的多域任务增量学习中平衡稳定性与可塑性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定架构和静态参数分配，难以适应新领域且加剧灾难性遗忘，亟需解决多域任务增量学习中的稳定性-可塑性困境。

Method: 提出 DIMoE-Adapters 框架，包含自校准专家进化（SCEE）和原型引导专家选择（PGES）两个协作组件，通过动态专家进化范式实现稀疏专家池的构建与演化。

Result: 在多种设置下，DIMoE-Adapters 均优于现有最先进方法。

Conclusion: 动态专家进化范式能有效平衡持续学习中的稳定性与可塑性，缓解灾难性遗忘并提升对新领域的适应能力。
...
### [73] [Cloud-top infrared observations reveal the four-dimensional precipitation structure](https://arxiv.org/abs/2605.07499)
...
TL;DR: 本文提出4DPrecipNet框架，证明云顶红外观测能够编码足够的次云降水信息，从而实现对降水系统四维结构（垂直+时间）的全球连续重建。


<details>
  <summary>Details</summary>
Motivation: 全球尺度的四维降水信息对于理解地球能量和水循环至关重要，但传统认为地球静止轨道红外观测主要感知云顶特性，对云下降水敏感性有限，因此需要探索新的观测途径。

Method: 提出物理约束的深度学习框架4DPrecipNet，通过"湿度优先"约束（强制潜在表征恢复可降水水汽）实现热力学一致性，融合多通道红外辐射与雷达降水廓线数据，从静止轨道观测重建降水系统的垂直和时间演化。

Result: 框架成功捕获深对流结构及其演化过程，在大样本和独立雷达对比中表现出稳健性能，证明次云降水信息确实物理编码在云顶红外观测中。

Conclusion: 云顶红外观测可编码次云降水信息，为全球降水结构连续监测建立了新路径，突破了传统理论对红外遥感受限的认知。
...
### [74] [Diffusion-APO: Trajectory-Aware Direct Preference Alignment for Video Diffusion Transformers](https://arxiv.org/abs/2605.07503)
*Jingyuan Zhu,Biaolong Chen,Le Zhang,Aixi Zhang,Hao Jiang,Pipei Huang*

Main category: cs.CV

TL;DR: 本文提出Diffusion-APO算法，通过将训练噪声与推理时的去噪路径同步来对齐视频扩散模型与人类偏好，并构建了一个统一模块化的RLHF框架，无需依赖标量奖励模型即可实现多阶段偏好对齐。


<details>
  <summary>Details</summary>
Motivation: 现有DPO和GRPO等方法依赖有偏的复杂奖励模型或次优的时间步采样，难以高效对齐大规模视频扩散模型与人类意图，尤其是训练噪声分布与推理轨迹之间存在固有差异。

Method: 提出Diffusion-APO（Aligned Preference Optimization）轨迹感知算法，同步训练噪声与推理去噪路径以最大化梯度信号效能；并构建统一模块化RLHF框架，集成在线排序、半在线锚定、离线精炼和蒸馏感知漂移校正四个模块。

Result: 在视觉质量和指令遵循方面持续优于标准基线，并在模型加速过程中有效保持生成保真度。

Conclusion: Diffusion-APO提供了一种稳健的端到端路径，用于可扩展的视频扩散对齐，无需依赖标量奖励策略梯度。
...
### [75] [InterLV-Search: Benchmarking Interleaved Multimodal Agentic Search](https://arxiv.org/abs/2605.07510)
*Bohan Hou,Jiuning Gu,Jiayan Guo,Ronghao Dang,Sicong Leng,Xin Li,Xuemeng Song,Jianfei Yang*

Main category: cs.CV

TL;DR: 提出了InterLV-Search基准，用于评估交错的文本-视觉多模态智能搜索，包含2061个样本和三个难度级别，实验表明当前最佳模型准确率低于50%。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么将视觉证据限制在输入中，要么将其视为答案终点，而不是交错搜索轨迹的一部分，缺乏对交错式语言-视觉智能搜索的评估。

Method: 构建了三个级别的基准：主动视觉证据搜索、受控离线交错搜索、开放网页交错搜索；前两级用自动流水线，第三级用机器主导人工监督的流水线；并提供标准化工具InterLV-Agent。

Result: 当前最佳多模态智能体模型在InterLV-Search上的总体准确率低于50%。

Conclusion: 现有系统远未解决交错多模态搜索问题，在视觉证据搜索、搜索控制和多模态证据整合方面仍面临挑战。
...
### [76] [Hierarchical Dual-Subspace Decoupling for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07512)
*Mengxin Qin,Xiang Zhang,Kun Wei,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出层次化双子空间解耦框架（HDSD），通过参数空间分解为通用和任务特定子空间，减少跨任务子空间干扰，缓解灾难性遗忘，在视觉-语言模型的类增量学习中取得SOTA。
...
Method: 提出HDSD框架，包含：(1) 轻量级特征调制模块(FMM)，将参数空间分解为通用和任务特定子空间；(2) 通用融合模块(GFM)，通过自适应阈值捕获稳定可迁移知识；(3) 层次学习模块(HLM)，利用SVD进行结构化参数分解并用缩放机制约束子空间尺度内的更新。

Result: 在传统基准上的大量实验表明，HDSD取得了最先进的结果。

Conclusion: 通过双子空间解耦和层次化学习机制，有效减少了子空间干扰和参数漂移，为类增量学习提供了新的解决思路。
...
### [77] [Implicit Preference Alignment for Human Image Animation](https://arxiv.org/abs/2605.07545)
*Yuanzhi Wang,Xuhua Ren,Jiaxiang Cheng,Bing Ma,Kai Yu,Tianxiang Zheng,Qinglin Lu,Zhen Cui*

Main category: cs.CV

TL;DR: 本文提出隐式偏好对齐（IPA）框架，无需构建严格的偏好对数据，通过隐式奖励最大化和手部感知局部优化机制，有效提升图像动画中手部运动的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有人体图像动画技术在手部高自由度、高复杂度运动生成方面存在困难，而传统的基于人类反馈的强化学习方法（如直接偏好优化）需要构建严格的偏好对，但手部区域的动态帧间不一致性使得数据构建成本极高且不切实际。

Method: 提出隐式偏好对齐（IPA）框架，基于隐式奖励最大化理论，通过最大化自生成高质量样本的似然并惩罚偏离预训练先验的行为来实现对齐；同时引入手部感知局部优化机制，显式引导对齐过程聚焦于手部区域。

Result: 实验表明，IPA方法能够有效实现偏好优化，提升手部生成质量，并大幅降低构建偏好数据的门槛。

Conclusion: IPA是一种数据高效的训练后对齐框架，无需配对偏好数据即可提升手部动画质量，为图像动画中的细粒度运动生成提供了可行方案。
...
### [78] [Probabilistic Object Detection with Conformal Prediction](https://arxiv.org/abs/2605.07549)
*Christopher Ries,Moussa Kassem Sbeyti,Nicolas Bianco,Nadja Klein*

Main category: cs.CV

TL;DR: 本文系统比较了缩放与非缩放保形预测在多类目标检测中的应用，提出基于坐标的Bonferroni校正、概率检测器不确定度缩放以及类预测集的RAPS扩展方法，在三个自动驾驶数据集上实现了最高19%的IoU提升和39%的区间分数下降，且不牺牲覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 目标检测中的结构化多输出特性使经典保形预测理论难以直接应用，且非缩放CP产生固定宽度预测区间，在低不确定性预测上过于保守。现有工作既未系统比较缩放与非缩放CP在多类目标检测中的表现，也未将CP与互补的不确定性量化方法整合。

Method: (i) 对边界框角点逐坐标应用CP，并用Bonferroni校正实现框级保证；(ii) 使用基于损失衰减的概率检测器提供的逐预测偶然不确定性估计来缩放区间，评估未校准和两种校准变体；(iii) 扩展为两阶段流水线：使用RAPS构建类的预测集，并在此基础上对边界框进行条件化保形预测。

Result: 在KITTI、BDD、CODA三个自动驾驶数据集上（含跨域分布偏移），缩放CP一致优于非缩放CP，IoU提升最高19%，区间分数降低最高39%，且不牺牲覆盖。类别级校准进一步提升两种变体的覆盖率，对尖锐度影响可忽略。

Conclusion: 缩放CP与概率检测器不确定性估计的结合能在保持覆盖保证的前提下显著提高预测区间尖锐度，为实时、真实世界目标检测提供更可操作的不确定性估计。
...
### [79] [Mind the Gap: Geometrically Accurate Generative Reconstruction from Disjoint Views](https://arxiv.org/abs/2605.07550)
*Grzegorz Wilczynski,Mikołaj Zielinski,Bartosz Świrta,Dominik Belter,Przemysław Spurek*

Main category: cs.CV

TL;DR: 本文提出了一种名为“无重叠视角生成式重建”（Generative Reconstruction from Disjoint Views）的新范式，针对零重叠场景下的3D重建问题，构建了综合性数据集和专门评估指标。现有方法在此任务中表现失败，而作者提出的GLADOS框架通过三个模块化阶段（生成桥接、鲁棒粗重建、迭代上下文扩展）有效解决了这一问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D视觉系统依赖视觉重叠（空间和外观重叠），这在分布式集群机器人或众包数据采集等现实场景中往往无法满足，因此需要探索无重叠视角下的3D重建新范式。

Method: GLADOS框架包含三阶段：(1) 生成桥接——利用基础模型合成中间视角连接无重叠输入；(2) 鲁棒粗3D重建——通过全局对齐建立粗略几何骨架，吸收生成过程中的局部矛盾；(3) 迭代上下文扩展与一致性优化——填补缺失区域并统一重建结果。该框架是架构无关的。

Result: 现有最先进方法在零重叠任务上表现灾难性失败，产生不连续的几何体或语义不一致的重建；GLADOS则能有效生成连贯的3D重建结果。

Conclusion: GLADOS作为通用模块化框架，为无重叠视角下的3D重建提供了可行方案，且未来可无缝集成生成、重建和修补领域的新进展。
...
### [80] [VIMCAN: Visual-Inertial 3D Human Pose Estimation with Hybrid Mamba-Cross-Attention Network](https://arxiv.org/abs/2605.07552)
*Zepeng Yang,Junxuan Bai,Hao Li,Ju Dai,Junjun Pan,Yongfeng Yin,Bin Li*

Main category: cs.CV

TL;DR: 提出VIMCAN混合架构，融合Mamba的高效序列建模和Cross-Attention的空间推理，实现多模态3D人体姿态估计的实时高精度推断。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的3D人体姿态估计方法在处理长序列时存在二次复杂度问题，难以实现实时处理；而Mamba虽能高效处理序列，但在多模态场景下捕捉复杂空间依赖关系能力不足。

Method: 提出VIMCAN混合架构，利用Mamba的动态参数化进行时间建模，利用Cross-Attention进行空间依赖提取，实现RGB关键点与可穿戴IMU数据的鲁棒融合。

Result: 在TotalCapture上MPJPE达17.2mm，在3DPW上达45.3mm，优于Transformer等SOTA方法，并在消费级硬件上支持超过60 FPS的实时推理。

Conclusion: VIMCAN通过Mamba与Cross-Attention的混合设计，有效平衡了效率与精度，为多模态3D人体姿态估计提供了一种高性能实时解决方案。
...
### [81] [Beyond GSD-as-Token: Continuous Scale Conditioning for Remote Sensing VLMs](https://arxiv.org/abs/2605.07562)
*Song Zhang,Yanlong Chen,Yilin Li,Yining Chen,Zili Yi,Xiaowei Zhang,Yawei Li*

Main category: cs.CV

TL;DR: ScaleEarth提出了一种基于连续尺度条件超LoRA（CS-HLoRA）的参数高效微调框架，通过将地面采样距离（GSD）作为连续条件变量来动态调控遥感视觉-语言模型的计算路径，配套构建了1.5M样本的尺度分层VQA数据集GeoScale-VQA，在遥感基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉-语言模型存在根本性失配：同一地理对象在不同地面采样距离（GSD）下视觉特征差异巨大，但现有方法要么丢弃GSD信息，要么将其作为离散文本标记注入，导致单一静态参数集被迫覆盖整个尺度谱系。

Method: ScaleEarth以Qwen3-VL为骨干，核心创新CS-HLoRA通过GSD驱动的门控机制调制LoRA低秩子空间，实现模型按物理尺度动态路由计算；配套SSE-U轻量头从视觉特征预测GSD及其不确定性；构建GeoScale-VQA数据集，问题-答案生成与物理尺度条件对齐，形成闭环。

Result: 在8B骨干上使用QLoRA训练，ScaleEarth在涵盖多种地球系统任务的遥感基准（XLRS-Bench、OmniEarth-Bench）上取得SOTA结果。

Conclusion: 将GSD作为连续条件变量动态调控模型计算路径是解决遥感尺度失配问题的有效方案，所提出的方法-数据闭环框架具有良好的可扩展性和实用性。
...
### [82] [Tracing the Arrow of Time: Diagnosing Temporal Information Flow in Video-LLMs](https://arxiv.org/abs/2605.07568)
*Peitao Han,Fei Cheng,Lis K. Pereira,Qianying Liu,Shigeru Kitazawa*

Main category: cs.CV

TL;DR: 该论文发现视频大语言模型在时间箭头任务上表现不佳，根源在于时间信息流被阻塞，特别是Q-Former投影器破坏了时间信息，而MLP投影器能更好保留时间信息。通过构建包含时间感知编码器、时间保持投影器和AoT监督的视频LLM，实现了超越人类的性能（98.1%准确率）。


<details>
  <summary>Details</summary>
Motivation: 解答视频大语言模型在时间箭头任务上表现远逊于人类的原因，探究时间信息瓶颈究竟存在于视觉编码器、投影器还是LLM模块中。

Method: 将视觉编码器从视频LLM中隔离出来，追踪时间信息在编码器、投影器和LLM中的流动；对比视频中心编码器与帧中心编码器；分析Q-Former与MLP投影器的差异；进行逐层分析；构建时间感知视频LLM。

Result: 视频中心编码器能编码强时间信号，但经标准视频LLM架构后性能下降；Q-Former破坏时间信息，MLP投影器更好保留；所构建的模型在AoT_PPB上达98.1%超越人类，在VITATECS-Direction和TVBench上分别提升6.0和1.3个点。

Conclusion: 视频LLM中的时间推理既需要有效的时间编码，也需要将时间信息可靠地传递到LLM，投影器设计是关键瓶颈。
...
### [83] [PolarVLM: Bridging the Semantic-Physical Gap in Vision-Language Models](https://arxiv.org/abs/2605.07574)
*Yuliang Li,Chu Zhou,Heng Guo,Boxin Shi,Imari Sato,Zhanyu Ma*

Main category: cs.CV

TL;DR: 提出PolarVLM，首个将偏振物理参数集成进视觉语言模型的多模态框架，通过双流架构和两阶段训练策略，解决传统RGB模型在反射和透明物体等光学歧义场景下的语义理解问题，在偏振感知VQA基准上超越RGB基线25.4%。


<details>
  <summary>Details</summary>
Motivation: 主流VLM在处理反射、透明物体等严重光学歧义时表现不足，因为标准RGB输入存在内在局限。虽然偏振成像能解决这些问题，但现有方法输出格式固定，无法进行开放式推理，需要弥合语义-物理之间的鸿沟。

Method: 提出PolarVLM框架，采用双流架构（偏振物理参数流+RGB视觉流）和渐进式两阶段训练策略，有效防止物理参数被误解，同时保持通用视觉能力。此外构建了PolarVQA基准，包含75K面向反射和透明场景的物理引导指令微调数据对。

Result: 在五项评估任务中，PolarVLM整体超越RGB基线25.4%，其中反射识别提升26.6%，玻璃计数提升34.0%，成功实现了物理感知的语义理解。

Conclusion: PolarVLM首次将偏振物理参数成功集成到VLM中，显著提升了模型在光学歧义场景下的语义理解能力，为物理感知的多模态理解开辟了新方向。
...
### [84] [Response-G1: Explicit Scene Graph Modeling for Proactive Streaming Video Understanding](https://arxiv.org/abs/2605.07575)
*Ke Ma,Jiaqi Tang,Bin Guo,Xueting Han,Ruonan Xu,Qingfeng He,Ziheng Wang,Xu Wang,Qifeng Chen,Zhiwen Yu,Yunhao Liu*

Main category: cs.CV

TL;DR: 提出Response-G1框架，通过场景图显式对齐视频证据与查询响应条件，实现流式视频理解中的精准响应时机决策。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs在流式视频理解中缺乏对视觉证据的显式、查询感知建模，导致响应时机判断不准确。

Method: 三步无微调流程：(1)在线查询引导的场景图生成；(2)基于记忆检索历史语义相关场景图；(3)检索增强的触发提示决策每帧"静默/响应"。

Result: 在多个基准测试中，响应时机决策的可解释性和准确性均优于现有方法，在主动和被动任务上均取得更好表现。

Conclusion: 显式场景图建模与检索机制是提升流式视频理解中响应决策质量的有效途径。
...
### [85] [TraceAV-Bench: Benchmarking Multi-Hop Trajectory Reasoning over Long Audio-Visual Videos](https://arxiv.org/abs/2605.07593)
*Hengyi Feng,Hao Liang,Mingrui Chen,Bohan Zeng,Meiyi Qiang,Zhengyang Zhao,Zimo Meng,Zeang Sheng,Wentao Zhang*

Main category: cs.CV

TL;DR: 提出了TraceAV-Bench，首个联合评估长视频多模态多跳推理和幻觉鲁棒性的基准，包含2200道题、578个长视频（共339.5小时），平均推理链3.68跳、时间跨度15.1分钟，最强闭源模型Gemini 3.1 Pro仅达68.29%，开源模型Ming-Flash-Omni-2.0为51.70%，表明存在显著提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估跨视听模态的稀疏、时间分散的多跳推理能力，因为它们局限于短视频、隔离模态或将问题简化为一跳感知。

Method: 通过三步半自动管道和严格质量控制构建TraceAV-Bench，涵盖4个评估维度和15个子任务，每个问题有显式推理链。

Result: 最强闭源模型Gemini 3.1 Pro达68.29%，最佳开源模型Ming-Flash-Omni-2.0仅51.70%，多模态幻觉鲁棒性与一般推理性能基本解耦。

Conclusion: TraceAV-Bench对现有OmniLLMs构成持续挑战，存在很大提升空间，将促进长视听内容连贯可信推理的研究。
...
### [86] [LithoBench: Benchmarking Large Multimodal Models for Remote-Sensing Lithology Interpretation](https://arxiv.org/abs/2605.07640)
*Jun Wang,Fengpeng Li,Hang Dong,Tianjin Huang,Wei Han*

Main category: cs.CV

TL;DR: 本文提出了LithoBench，一个用于评估遥感岩性解释中地质语义理解的多层次基准，包含12类岩性的10,000个专家标注实例和五个认知层次的任务，实验发现现有大模型在地质语义理解尤其是高阶任务上存在显著局限。


<details>
  <summary>Details</summary>
Motivation: 遥感岩性解释是知识密集型任务，依赖专家对多种特征的综合推理。现有地质知识引导的大多模态模型缺乏系统的评估基准，缺少涵盖岩性标注、多层次地质语义和专家评估的标准化数据集。

Method: 构建了包含10,000个专家标注实例的LithoBench，覆盖12类岩性，包括4,000道选择题和6,000道开放题，分为五个认知层次；开发了专家参与、知识驱动的半自动构建流程，融合结构化地质图像描述等多重子过程。

Result: 多个大视觉语言模型在地质语义理解上表现有限，尤其是在高阶解释、应用和推理任务中存在显著不足。

Conclusion: LithoBench揭示了现有模型在地质领域高层次认知任务中的局限性，为地质语义理解评估提供了标准化基准，推动了遥感岩性解释自动化的发展。
...
TL;DR: GazeVLM是一种将元认知主动视觉控制直接内化到推理循环中的多模态架构，通过自主生成注视令牌（<LOOK>）实现自上而下的注意力机制，在4B参数规模下超越同类VLM近4%，在HRBench基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 人类视觉推理依赖主动视觉（元认知控制驱动的自上而下注意力），而现有VLM被动处理视觉信息，依赖静态大量令牌上下文，导致空间推理能力被稀释并产生语言幻觉。

Method: GazeVLM让模型自主生成注视令牌（<LOOK>），建立对自身因果注意力掩码的自上而下控制机制，通过持续抑制偏压抑制无关视觉特征（模拟空间选择性注意和中央凹注视），局部推理完成后恢复全局视图，无需外部裁剪工具或扩展上下文窗口。

Result: 4B参数的GazeVLM在HRBench-4k和HRBench-8k上超越同规模SOTA VLM近4%，超越基于思维链的智能体多模态流程超过5%。

Conclusion: 将元认知主动视觉控制内化到VLM推理循环中，可显著提升高分辨率多模态推理性能，提供了一种不依赖外部工具或膨胀上下文的范式转变方案。
...
### [88] [EggHand: A Multimodal Foundation Model for Egocentric Hand Pose Forecasting](https://arxiv.org/abs/2605.07642)
*Jaeyoung Choi,Hyeondong Kim,Yujin Kim,Daehee Park*

Main category: cs.CV

TL;DR: 本文提出EggHand，一种基于基础模型的自我中心手部姿态预测框架，结合视觉-语言-动作模型与自我中心视频文本编码器，实现多模态语义推理与动态运动建模，在EgoExo4D数据集上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 自我中心手部运动由复杂的人类意图驱动，具有高度灵巧的关节运动，且自我运动导致视角剧烈变化，使得从自我中心视频中预测未来3D手部姿态序列极具挑战性。

Method: 耦合VLA模型的动作解码器（捕捉手部运动结构化时序动态）与自我中心视频-文本编码器（提供视角感知的上下文信息），无需依赖身体姿态或外部追踪。

Result: 在EgoExo4D数据集上达到最先进的预测精度，在剧烈自我运动下保持鲁棒性，并支持基于语言任务提示的可控预测。

Conclusion: EggHand通过统一多模态语义推理与动态运动建模，有效解决了自我中心手部姿态预测的难题，为AR/VR辅助和人机交互等具身应用提供了新思路。
...
### [89] [Operating Within the Operational Design Domain: Zero-Shot Perception with Vision-Language Models](https://arxiv.org/abs/2605.07649)
*Berkehan Ünal,Dierend Hauke,Fazlija Dren,Plachetka Christopher*

Main category: cs.CV

TL;DR: 本文研究视觉语言模型（VLM）能否作为零样本"ODD传感器"，用于自动驾驶系统的运行设计域（ODD）感知。通过实验发现，结合定义锚定的思维链提示与角色分解方法效果最佳，为安全关键应用中的ODD感知奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统的大规模实际应用需要遵守安全法规，而法规受运行设计域（ODD）影响。可靠感知ODD元素对安全实现和审计至关重要，因此需要一种能适应不断变化的ODD定义的有效感知方法。

Method: 使用四种视觉语言模型（VLM），在自定义数据集和Mapillary Vistas上进行零样本ODD分类和检测的实证研究；对零样本优化策略进行消融实验及成本-性能分析；设计并提供一套可复用的提示模板及自适应指南。

Result: 定义锚定的思维链提示结合角色分解策略表现最佳，而其他方法可能导致召回率下降。

Conclusion: VLM可以作为零样本ODD传感器发挥作用，为安全关键应用中透明且有效的ODD感知提供了可行路径。
...
### [90] [Towards Billion-scale Multi-modal Biometric Search](https://arxiv.org/abs/2605.07655)
*Arka Koner,Chetan S. Naik,Lokesh Kurre,Vivek Raghavan,Barada P. Sabut,Tanusree Deb Barma,Anoop M. Namboodiri,Anil K. Jain*

Main category: cs.CV

TL;DR: 本文介绍了Bharat ABIS，一个基于开源架构的大规模多模态生物特征搜索系统，支持在11亿条印度Aadhaar数据库记录上进行指纹、人脸和虹膜的身份去重，实现了每秒100次搜索的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 面向国家级身份系统，需要处理数十亿级别的多模态生物特征数据库，现有文献缺乏对如此大规模系统的端到端设计与实现细节的公开讨论。

Method: 采用端到端流水线，分别对指纹、人脸和虹膜进行预处理（分割）、质量评估、呈现攻击检测和嵌入学习（特征提取），最终生成每人的13.5KB拼接模板，实现1:N搜索（去重）。

Result: 在来自印度Aadhaar数据库的2.2亿身份（从15.5亿条记录中随机抽取）的人口分层图库上，成人探针的FNIR为0.3%（FPIR=0.5%）；在单台服务器（8×Nvidia H100 GPU、2TB RAM）上对4000万图库实现每秒100次搜索的吞吐量。

Conclusion: Bharat ABIS展示了基于开源架构构建国家级多模态生物特征识别系统的可行性与高效性，在准确率和速度上均能与商用系统竞争。
...
### [91] [OphEdit: Training-Free Text-Guided Editing of Ophthalmic Surgical Videos](https://arxiv.org/abs/2605.07695)
*Ritul Jangir,Arkya Jyoti Bagchi,Aiman Farooq,Mangalton Okram,Saurabh Seetaram Korgaonkar,Deepak Mishra*

Main category: cs.CV

TL;DR: 提出OphEdit，一种无需训练的视频编辑框架，用于眼科手术视频的文本引导编辑，通过ODE反演和注意力值注入保留解剖结构的同时实现语义修改。


<details>
  <summary>Details</summary>
Motivation: 高保真手术视频生成对医疗培训和AI开发至关重要，但现有生成模型在精确视频编辑方面面临挑战，特别是修改手术属性（如器械-组织交互、手术阶段）时受严格的解剖和时间约束。

Method: 利用确定性二阶ODE反演流水线捕获原始视频的注意力值（V）张量；在去噪阶段选择性地将这些张量注入条件CFG分支，从而在实现文本驱动的语义修改同时严格保留眼部解剖几何结构。

Result: 临床评估显示OphEdit能有效处理复杂手术变换（如器械替换、手术流程变化），在结构保真度和时间一致性上优于自然域视频编辑器。

Conclusion: 首个在眼科手术领域应用无需训练的视频编辑的工作，为生成多样化、带标注的医疗数据集提供了可扩展方案，无需大量手动录制或昂贵模型微调。
...
### [92] [Benchmarking Foundation Models for Renal Lesion Stratification in CT](https://arxiv.org/abs/2605.07749)
...
TL;DR: 本文评估了三种开源医学基础模型在CT肾病变分类任务中的迁移学习表现，发现其性能（AUC 0.70-0.77）与从头训练的3D ResNet-50（AUC 0.72）相当，但远不及传统影像组学方法（AUC 0.88）。表明当前通用医学FM尚无法捕捉足够的细粒度纹理与形状异质性用于组织学亚型区分。


<details>
  <summary>Details</summary>
Motivation: 开源医学基础模型在数据稀缺的临床分类任务（如CT肾病变分类）中的迁移学习效果尚不明确，尤其是面对罕见亚型时，现有模型能否超越传统方法尚待验证。

Method: 在包含2,854个病变的复合数据集上，采用冻结特征探测协议，比较三种医学FM与手工影像组学分类器及3D ResNet-50（从头训练）在六类肾病变分类任务上的表现，并在来自TCIA的234个病变外部测试集上评估。

Result: 医学FM的AUC为0.70-0.77，与从头训练的ResNet-50（AUC 0.72）相当，但显著低于影像组学基线（AUC 0.88，所有p≤0.002）。FM显著降低了计算资源需求。

Conclusion: 当前通用医学基础模型嵌入尚不能充分捕捉用于组织学亚型辨别的细粒度纹理与形状异质性，在肾病变分层任务中未能超越已有方法，影像组学仍为当前最优方案。
...
### [93] [Head Similarity: Modeling Structured Whole-Head Appearance Beyond Face Recognition](https://arxiv.org/abs/2605.07766)
*Yingfeng Wang,Yuxuan Xiao,Shengcai Liao*

Main category: cs.CV

TL;DR: 本文提出"头部相似度"概念，扩展了传统身份识别，通过层次化监督和身份感知蒸馏，在非正面视角或面部遮挡情况下实现结构化的全头相似度建模。


<details>
  <summary>Details</summary>
Motivation: 传统人脸识别模型追求身份不变性，会压缩发型、造型等外观变化，在需要外观敏感性的场景（如非正面视角或面部缺失时）表现不佳，无法满足身份一致性之外的细粒度相似度比较需求。

Method: 提出头部相似度（Head Similarity）新范式，构建大规模弱监督基准数据集，开发结合层次化监督与身份感知蒸馏的简单有效框架，联合建模身份判别与外观敏感相似度。

Result: 实验表明传统人脸识别模型无法捕捉外观依赖的相似度，而提出的方法在结构化全头相似度建模上是可行的。

Conclusion: 本文首次探索了结构化全头相似度建模，弥补了传统身份识别在遮挡或后视图等外观敏感场景下的不足。
...
### [94] [Differentiable Ray Tracing with Gaussians for Unified Radio Propagation Simulation and View Synthesis](https://arxiv.org/abs/2605.07781)
*Niklas Vaara,Lam Huynh,Pekka Sangi,Miguel Bordallo López,Janne Heikkilä*

Main category: cs.CV

TL;DR: 提出一种框架，利用3D高斯泼溅实现视觉重建场景中的可微射频传播模拟，支持任意3D位置间的路径计算，同时保持高质量视觉渲染。


<details>
  <summary>Details</summary>
Motivation: 传统RF数字孪生依赖手动构建的网格模型，而神经场景重建方法（如3DGS）仅优化光学外观，缺乏可进行射线相交的几何信息。需要一种统一的空间表示，同时支持视觉渲染和电磁传播模拟。

Method: 将高斯基元嵌入硬件加速的光线追踪结构中作为底层空间表示，在视觉重建的神经场景中实现可微分的RF传播模拟，计算任意3D位置间的点对点路径。

Result: 从仅视觉的重建中提取出物理上有意义的信道冲激响应，提供了跨模态证据。

Conclusion: 神经重建场景可以作为统一的时空表示，同时用于电磁传播模拟和逼真视图合成。
...
### [95] [Radiologist-Guided Causal Concept Bottleneck Models for Chest X-Ray Interpretation](https://arxiv.org/abs/2605.07785)
*Amy Rafferty,Rishi Ramaesh,Ajitha Rajan*

Main category: cs.CV

TL;DR: 提出XpertCausal，一种由放射科医生指导的因果概念瓶颈模型，利用概率 noisy-OR 框架模拟病理-概念生成关系，再通过贝叶斯推理反演估计病理概率，在CXR解读中提升了准确性、校准性和临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有CBM将概念视为病理标签的判别性预测因子，并未显式建模疾病产生影像学发现的临床生成过程，缺乏因果结构。

Method: 构建概率 noisy-OR 生成模型刻画病理到概念的关系，通过贝叶斯推理从预测概念反推病理概率，并利用放射科医生标注的概念-病理关联约束模型结构。

Result: 在MIMIC-CXR上，相比非因果CBM和未约束因果消融，XpertCausal在AUROC、校准性和临床相关解释质量上均有提升，且学到的概念-病理关系更符合专家知识。
...
### [96] [APEX: Assumption-free Projection-based Embedding eXamination Metric for Image Quality Assessment](https://arxiv.org/abs/2605.07786)
*Caterina Gallegati,Monica Bianchini,Franco Scarselli,Vittorio Murino,Barbara Toniella Corradini*

Main category: cs.CV

TL;DR: 提出APEX评估框架，利用Sliced Wasserstein距离和开放词汇基础模型（CLIP、DINOv2），克服传统指标的特征瓶颈和参数化假设偏差，在鲁棒性和跨数据集稳定性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统图像评估指标（如FID）受限于闭集特征瓶颈和刚性参数化假设偏差，现有改进方法虽解决特征问题但仍受参数化限制。

Method: 提出APEX框架，基于Sliced Wasserstein距离作为无假设相似性度量，结合CLIP和DINOv2两个开放词汇基础模型作为特征提取器，实现嵌入无关的高维空间可扩展评估。

Result: APEX在视觉退化鲁棒性上优于现有基线，且展现出数据集内和跨数据集稳定性，确保在域外数据集上的稳定评估。

Conclusion: APEX通过无假设的数学框架和开放词汇特征提取，有效解决了传统评估指标的瓶颈问题，提供了更鲁棒、更稳定的图像评估方案。
...
### [97] [SARA: Semantically Adaptive Relational Alignment for Video Diffusion Models](https://arxiv.org/abs/2605.07800)
*Jiesong Lian,Zixiang Zhou,Ruizhe Zhong,Yuan Zhou,Qinglin Lu,Rui Wang,Long Hu,Yixue Hao,Baoru Huang*

Main category: cs.CV

TL;DR: SARA提出一种语义自适应的关系对齐方法，通过文本条件化的显著性机制，在视频扩散模型训练中优先关注与提示相关的实体对（如主体-主体、主体-背景），减少背景-背景对的监督，从而提升文本对齐和运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型仍存在实体遗漏、属性绑定错误、交互弱化等问题；而VideoREPA等表示对齐方法虽改进了文本跟随，但其配对监督预算由视觉或运动线索分配，而非根据每一对与提示的相关性，导致监督资源分配不合理。

Method: SARA包含：(1) 轻量级Stage 1对齐器，使用SAM 3.1掩码监督和InfoNCE正则化训练；(2) 文本条件化的显著性模块，判断每个token与提示的相关性；(3) 对路由操作符，当配对中任一端点为显著时分配权重，引导监督朝向主体-主体和主体-背景对，远离背景-背景对。

Result: 在Wan2.2持续训练设置下，SARA在13维度VLM评分、VBench基准测试和盲用户研究中，在文本对齐和运动质量上均优于SFT、VideoREPA和MoAlign。

Conclusion: 通过语义自适应的配对监督分配，可以有效提升视频扩散模型对文本提示的细粒度遵循能力，同时改善运动质量。
...
### [98] [Text-to-CAD Evaluation with CADTests](https://arxiv.org/abs/2605.07807)
*Dimitrios Mallis,Marco Wang,Ahmet Serdar Karadeniz,Elisa Ricci,Anis Kacem,Djamila Aouada*

Main category: cs.CV

TL;DR: 提出CADTestBench，首个基于可执行软件测试的Text-to-CAD评估基准，通过自动化测试验证生成的CAD模型是否符合输入提示的几何和拓扑要求。


<details>
  <summary>Details</summary>
Motivation: Text-to-CAD评估工作极少，缺乏有效的评估方法，评估CAD模型生成性能仍是一个重大挑战。

Method: 基于自动化测试的新评估视角，提出CADTests（可执行软件测试），构建CADTestBench基准，用于验证CAD模型是否满足几何和拓扑要求。

Result: 对近期Text-to-CAD方法进行了全面基准测试，并证明CADTests还可指导CAD模型生成，产生超越当前方法的简单基线。

Conclusion: CADTestBench是首个基于测试的Text-to-CAD基准，其测试机制不仅能评估，还能辅助生成更优的CAD模型。
...
### [99] [ICDAR 2026 Competition on Writer Identification and Pen Classification from Hand-Drawn Circles](https://arxiv.org/abs/2605.07816)
*Thomas Gorges,Janne van der Loop,Lukas Hüttner,Linda-Sophie Schneider,Fei Wu,Mathias Seuret,Vincent Christlein*

Main category: cs.CV

TL;DR: 本文介绍了CircleID，一个关于从手绘圆圈扫描图中进行书写者识别和笔型分类的大规模ICDAR 2026竞赛，包含开放集书写者识别和跨书写者笔分类两个任务，使用了46155张高分辨率圆圈图像数据集。


<details>
  <summary>Details</summary>
Motivation: 探究生物特征书写者特征与物理笔特征如何在最小、静态痕迹中自然纠缠，建立最小痕迹分析的新基准。

Method: 构建包含46155张400 DPI圆圈图像的数据集，来自50名已知和16名未知书写者使用8种不同笔绘制；设置开放集书写者识别和跨书写者笔分类两个任务；在Kaggle上举办竞赛，提供ResNet基线模型。

Result: 笔分类任务有389支队伍（436名参与者）提交3185次，书写者识别有113支队伍（141名参与者）提交1737次；最佳结果的Top-1准确率：书写者识别64.801%，笔分类92.726%。

Conclusion: CircleID为最小痕迹分析建立了新基准，详细评估了获胜方法，分析了分布外书写者对模型泛化和特征解缠的影响。
...
### [100] [Divide and Conquer: Object Co-occurrence Helps Mitigate Simplicity Bias in OOD Detection](https://arxiv.org/abs/2605.07821)
*Boyang Dai,Chaoqi Chen,Yizhou Yu*

Main category: cs.CV

TL;DR: 提出基于物体共现模式（Object Co-occurrence, OCO）的OOD检测框架，通过解耦表示预测物体共现模式，并采用分治策略进行检测，在多种OOD场景下取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法多关注整体纠缠表示，忽略了图像中丰富的上下文信息，尤其在近分布OOD检测中，模型因简单性偏好难以学习解耦表示中的判别性特征。

Method: 提出OCO框架：1) 学习解耦表示以捕捉物体共现模式；2) 根据ID训练数据中的物体共现模式，自适应地将测试样本分为三类场景；3) 采用分治策略进行OOD检测。

Result: 在具有挑战性的全频谱OOD设置下进行实验，取得了竞争性结果，能够同时处理语义偏移和协变量偏移。

Conclusion: 通过模拟人类视觉系统利用物体共现关系理解场景的能力，OCO有效区分了近分布OOD，避免模型仅关注简单易学区域。
...
### [101] [Explainable Part-Based Vehicle Classifier with Spatial Awareness](https://arxiv.org/abs/2605.07831)
*Andreas Caduff,Klaus Zahn,Jonas Hofstetter,Martin Rechsteiner,Patrick Flaig*

Main category: cs.CV

TL;DR: 本文提出了一种基于空间感知部件（Spatially-Aware Parts）的车辆分类方法，通过构建部件空间概率图替代二值特征，在保持可解释性的同时提升了鲁棒性，准确率接近端到端CNN。


<details>
  <summary>Details</summary>
Motivation: 传统CNN在ITS中虽是主流，但存在黑箱性质且难以扩展至新类别。前期工作虽通过部件分解提升了可解释性和可扩展性，但二值特征对误检鲁棒性不足，限制了实际应用。

Method: 采用CNN检测器提取车辆语义部件，然后构建部件的全空间概率图（而非二值存在/不存在决策），最后使用softmax回归进行车辆类别概率估计，实现空间感知的分类。

Result: 该方法显著提升了对于误检（虚假部件检测）的鲁棒性，在准确率上与端到端CNN相当，打破了准确率与可解释性之间的传统权衡。

Conclusion: 该空间感知部件方法为ITS中的车辆分类提供了兼具高精度与直观可解释性的解决方案，是迈向实际可部署系统的重要基础。
...
### [102] [BRIDGE: Background Routing and Isolated Discrete Gating for Coarse-Mask Local Editing](https://arxiv.org/abs/2605.07846)
*Peilin Xiong,Honghui Yuan,Junwen Chen,Keiji Yanai*

Main category: cs.CV

TL;DR: BRIDGE提出了一种基于Two-Zone约束的局部图像编辑方法，通过离散几何门控机制在DiT外部处理遮罩，避免遮罩形状偏差，提升了编辑质量与背景保持能力。


<details>
  <summary>Details</summary>
Motivation: 现有粗遮罩局部图像编辑中，遮罩常成为意外形状先验（mask-shape bias），导致生成物体被拉向遮罩边界，而非灵活支持编辑。

Method: 1) 提出Two-Zone约束：背景稳定、编辑区按指令生成而不继承遮罩轮廓。2) 将遮罩置于DiT外部进行构建与融合，避免内部注入。3) 采用BridgePath生成：主路径保留背景，主题路径从独立噪声生成可编辑内容。4) 提出可学习的离散几何门控（Discrete Geometric Gate），在token层面路由位置嵌入，让主题token可借用背景锚定坐标或保持独立坐标。

Result: 在BRIDGE-Bench上，Local SigLIP2-T从FLUX.1-Fill的0.262和ACE++的0.390提升至0.503；在MagicBrush和ICE-Bench上零样本表现优异；路由模块仅13.31M参数，远小于ControlNet式复制分支。

Conclusion: BRIDGE通过外部遮罩处理与轻量几何门控，有效缓解遮罩形状偏差，实现了更灵活的局部编辑，且参数高效、泛化性强。
...
### [103] [EyeCue: Driver Cognitive Distraction Detection via Gaze-Empowered Egocentric Video Understanding](https://arxiv.org/abs/2605.07859)
*Lang Zhang,JinYi Yoon,Matthew Corbett,Abhijit Sarkar,Bo Ji*

Main category: cs.CV

TL;DR: 本文提出EyeCue框架，利用眼动与第一人称视频结合，检测驾驶员认知分心，并构建了CogDrive多场景数据集，在多项基线中达到74.38%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 驾驶员认知分心是导致交通事故的主要原因，且难以被传统方法检测，因为它不伴随明显物理动作，亟需更有效的检测手段。

Method: 提出EyeCue框架，融合眼动数据与第一人称视频，通过上下文感知建模驾驶员注意力随时间的变化；同时构建CogDrive多场景数据集，增强数据规模与多样性。

Result: 在CogDrive上达到74.38%的最高准确率，超越11种基线模型超7%，且在多种驾驶场景（不同道路类型、时段、天气）下准确率均超70%。

Conclusion: 建模眼动-上下文交互对认知分心检测至关重要，跨模态交互建模在多模态认知分心检测中具有显著有效性。
...
### [104] [From Synthetic to Real: Toward Identity-Consistent Makeup Transfer with Synthetic and Real Data](https://arxiv.org/abs/2605.07861)
*Yue Yu,Jiayu Wang,Jiajia Shi,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 本文提出ConsistentBeauty数据管线与RealBeauty后训练框架，通过合成数据监督+强化学习微调，解决化妆迁移中身份保持困难与合成-真实域差距问题，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有化妆迁移方法面临两大挑战：合成数据监督难以忠实保持身份信息，以及合成数据与真实数据之间存在域差距，导致在复杂真实场景下泛化能力有限、性能下降。

Method: 提出ConsistentBeauty数据管线确保合成数据中的化妆忠实度与身份一致性；提出RealBeauty合成-真实后训练框架，在合成数据监督学习基础上，通过强化学习（设计可验证奖励函数）进一步适配真实场景。

Result: 在多个基准上达到SOTA性能，在身份保持和复杂真实场景处理上具有明显优势。

Conclusion: 通过合成数据精心构建与强化学习微调相结合的策略，能有效解决化妆迁移中身份保持和域泛化两大难题，为真实场景下的高质量化妆迁移提供了可行方案。
...
### [105] [Video Understanding Reward Modeling: A Robust Benchmark and Performant Reward Models](https://arxiv.org/abs/2605.07872)
*Yuancheng Wei,Linli Yao,Lei Li,Haojie Zhang,Hao Zhou,Fandong Meng,Xu Sun*

Main category: cs.CV

TL;DR: 本文提出了一套涵盖基准设计、数据构建和奖励模型训练的完整框架，用于视频理解奖励建模。包括VURB基准（2100个偏好对，含长链思维推理）、VUP-35K数据集（自动化构建的大规模偏好数据）以及两个奖励模型VideoDRM和VideoGRM，在多个基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 多模态奖励模型在文本和图像领域已有显著进展，但视频理解奖励建模因缺乏可靠的评估基准和高质量偏好数据而严重受限。

Method: 提出统一框架：1) 构建VURB基准，含2100个偏好对和长链推理轨迹；2) 通过全自动流水线构建VUP-35K大规模偏好数据集；3) 训练判别式(VideoDRM)和生成式(VideoGRM)两种奖励模型。

Result: VideoDRM和VideoGRM在VURB和VideoRewardBench上均达到最优性能；VUP-35K数据能提升奖励性能和模型推理能力；在best-of-N测试时扩展中取得显著增益。

Conclusion: 该框架有效解决了视频理解奖励建模中基准和数据的瓶颈问题，验证了大规模高质量偏好数据对奖励模型和推理能力的促进作用。
...
### [106] [Semantic-Aware Adaptive Visual Memory for Streaming Video Understanding](https://arxiv.org/abs/2605.07897)
*Hang Wu,Sherin Mary Mathews,Yujun Cai,Ming-Hsuan Yang,Yiwei Wang*

Main category: cs.CV

TL;DR: SAVEMem是一个无需训练的双阶段框架，通过引入语义感知的流式记忆管理和查询自适应检索，显著提升了在线流式视频理解的性能，同时降低了GPU内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常基于视觉相似性启发式压缩视频令牌，或在压缩完成后才添加KV缓存级检索，导致压缩决策缺乏语义信号且两阶段难以协调。在线流式视频中无界流和不可预测的查询时序使内存管理成为核心挑战。

Method: 第一阶段：在固定内存预算下在线构建三级流式记忆，利用伪问题库提供轻量级语义先验，使长期保留由语义显著性而非视觉相似性决定。第二阶段：执行查询感知检索，通过锚定条件的时间门控自适应调整检索范围（短期/中期/长期记忆），并在该范围内通过查询与记忆令牌的后期交互选择候选帧。

Result: 在Qwen2.5-VL上无需训练，OVO-Bench总分从52.27提升至62.69；StreamingBench和ODV-Bench上均有一致性提升；在128帧场景下，峰值GPU内存较基线降低48%。

Conclusion: 引入语义先验到记忆生成中，并让检索范围随查询自适应调整，是提升流式视频理解性能的有效途径，且无需额外训练即可实现显著改进。
...
### [107] [One World, Dual Timeline: Decoupled Spatio-Temporal Gaussian Scene Graph for 4D Cooperative Driving Reconstruction](https://arxiv.org/abs/2605.07910)
*Yulong Chen,Xiaoyun Dong,Haoyu Zhang,Zongxian Yang,Lewei Xie,Xinke Li,Yifan Zhang,Kai Wang,Jianping Wang*

Main category: cs.CV

TL;DR: 本文提出DUST（解耦时空高斯场景图）方法，解决车-路协同自动驾驶（VICAD）中因多源相机时间异步导致的动态场景重建伪影问题。通过为每个智能体维护单一高斯集合但解耦每条轨迹的位姿时间戳，消除跨源梯度干扰，在V2X-Seq数据集上动态区域PSNR提升3.2 dB，Fréchet视频距离降低37.7%。


<details>
  <summary>Details</summary>
Motivation: 现有高斯场景图方法假设多源相机同步观测，为每个动态智能体每帧分配单一姿态，这在车-路协同场景中因独立时钟导致的时间异步而失效，产生严重鬼影。作者证明单时间线表述存在不可降低的光度损失（与速度和时间偏移的平方成正比），从根本上需要解耦表示。

Method: 提出DUST高斯场景图：1）每个智能体共享规范高斯集保证外观一致性；2）为每个数据源维护解耦的位姿轨迹，对齐各自真实捕获时间戳；3）引入静态锚点姿态校正管道修正车-路标注间的空间错位；4）采用姿态正则化联合优化策略防止训练早期轨迹抖动和漂移。

Result: 在V2X-Seq数据集的26个序列上，动态区域PSNR比最强基线提升3.2 dB，Fréchet视频距离降低37.7%，且在更大时间异步下保持鲁棒性。

Conclusion: DUST通过解耦时空表示从根本上解决了协同自动驾驶中时间异步导致的动态场景重建问题，理论证明和实验均验证了其有效性，代码已开源。
...
### [108] [What Matters for Diffusion-Friendly Latent Manifold? Prior-Aligned Autoencoders for Latent Diffusion](https://arxiv.org/abs/2605.07915)
...
TL;DR: 本文提出Prior-Aligned AutoEncoder (PAE)，通过显式塑造潜空间流形（连贯空间结构、局部连续性、全局语义）替代传统仅关注重建保真度的方法，在ImageNet 256x256上实现gFID 1.03，收敛速度提升13倍。


<details>
  <summary>Details</summary>
Motivation: 现有tokenizer主要关注重建保真度或继承预训练表征，但什么样的潜空间才能真正有利于扩散模型的生成建模尚不清楚。本文旨在从流形组织角度回答这一问题。

Method: 构建受控tokenizer变体，识别出扩散友好流形的三个关键属性（空间结构连贯性、局部流形连续性、全局流形语义）；提出PAE，利用VFM精炼先验和扰动正则化将这三个属性转化为显式训练目标，直接塑造潜流形。

Result: 在ImageNet 256x256上，PAE在相同训练设置下收敛速度比RAE快13倍，并达到新的SOTA gFID 1.03。

Conclusion: 潜流形的组织对潜在扩散模型的生成质量至关重要，PAE通过显式塑造流形属性实现了更优的生成效果和训练效率。
...
### [109] [MedVIGIL: Evaluating Trustworthy Medical VLMs Under Broken Visual Evidence](https://arxiv.org/abs/2605.07919)
...
TL;DR: 本文提出 MedVigil，一个由放射科医生监督构建的300例医学VQA评估套件，用于检测医学视觉语言模型在证据被扰动时的静默失败问题。测试了16个视觉模型和2个文本模型，最佳模型Claude Opus得分为69.2，与人类放射科医生的83.3分存在14.1分的差距。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM通常在完整图像-问题对上进行评估，但临床可信赖使用需要模型能够识别证据基础失败的情况（即静默失败）。目前缺乏针对这一关键安全属性的系统评估基准。

Method: 构建MedVigil基准：从4个公开医学VQA来源精选300例，由4位委员会认证放射科医生全程监督。包含2,556个MCQ探针、240个反事实三元组、医生裁定的风险等级和可回答性标志、ROI框及配对开放式变体。设计了7个正确性条件审计指标并汇总为MCS综合评分。

Result: 独立放射科医生MCS得分为83.3，静默失败率5.8%；最强模型Claude Opus得分为69.2，静默失败率未明确给出但综合差距14.1分。共审计16个视觉模型和2个纯文本基线。

Conclusion: 当前最佳医学VLM与放射科医生之间存在显著性能差距，特别是在检测证据失败方面。MedVigil基准和评估工具已公开发布，为未来提升临床可信赖AI提供了标准化评估框架。
...
### [110] [Delta-Adapter: Scalable Exemplar-Based Image Editing with Single-Pair Supervision](https://arxiv.org/abs/2605.07940)
*Jiacheng Chen,Songze Li,Han Fu,Baoquan Zhao,Wei Liu,Yanyan Liang,Li Qing,Xudong Mao*

Main category: cs.CV

TL;DR: Delta-Adapter提出了一种基于单样本对的图像编辑方法，通过提取源-目标图像间的语义差异（semantic delta）并将其注入预训练编辑模型，摆脱了传统方法对成对训练数据的依赖，在保持内容一致性的同时提升了编辑精度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于示例的图像编辑方法依赖"成对监督"范式，需要两个图像对共享相同编辑语义才能学习目标变换，导致训练数据难以大规模收集，且限制了编辑类型的泛化能力。

Method: 利用预训练视觉编码器提取图像对之间的语义差异（semantic delta），通过基于Perceiver的适配器将该差异注入预训练图像编辑模型；由于目标图像对模型不可见，可直接作为预测目标实现单样本对监督；并引入语义差异一致性损失对齐生成结果与真实语义差异。

Result: 在四个强基线方法上，Delta-Adapter在已见编辑任务上同时提升了编辑精度和内容一致性，并在未见编辑任务上展现出更有效的泛化能力。

Conclusion: Delta-Adapter通过语义差异提取和单样本对监督范式，有效解决了传统方法的数据瓶颈和泛化限制，为基于示例的图像编辑提供了更灵活且可扩展的解决方案。
...
TL;DR: 提出了离散体素扩散（DVD）框架，用于稀疏体素的生成、评估和编辑，在SLat-based 3D生成管道中作为有效的一阶先验。


<details>
  <summary>Details</summary>
Motivation: 离散扩散在图像生成领域尚未取代连续扩散，但作者探索其在稀疏体素支架上的有效性，旨在避免连续到离散的阈值化过程，并提供更可解释的生成动态。

Method: 将体素占用作为原生离散变量处理，利用预测熵作为鲁棒的不确定性度量，并提出基于块结构扰动模式的轻量级微调策略，实现单轮采样内的体素修复和编辑。

Result: DVD在体素生成质量上超越现有方法，提供更可解释的生成动态；预测熵有效识别模糊体素区域和复杂样本；轻量级微调策略在几乎无额外计算代价下实现编辑功能。

Conclusion: 离散扩散在稀疏体素3D生成中是有效的，DVD框架兼具生成、评估和编辑能力，为SLat-based 3D管道提供了简单而强大的先验模型。
...
TL;DR: 本文发现文本编码器嵌入空间实际位于超球面上，概念呈各向异性分布（Kent分布），并提出无需训练的HEART框架，通过在超球面上进行测地线变换实现精确的图像编辑，无需微调或优化。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的图像控制方法将嵌入空间视为欧几里得空间并应用线性变换，导致编辑时出现背景改变或细节失真等副作用，原因是未反映语义概念的真实组织方式。

Method: 提出HEART框架，基于Kent分布在超球面上进行测地线变换，实现训练-free的文本到图像可控生成。

Result: 实现一致的主体替换和细粒度属性控制，同时保持原始场景不变，且无需微调、反演或优化。

Conclusion: 从线性视角转向球形视角，可以解锁快速、可控的图像生成，该方法能泛化到不同扩散模型架构。
...
TL;DR: 提出Cross3R模型，利用无人机图像作为中间视角，实现从卫星图像和地面图像中恢复6自由度相机位姿和3D点云，并构建了跨洲的CrossGeo数据集。


<details>
  <summary>Details</summary>
Motivation: 传统跨视角定位方法受限于3自由度（位置+偏航角），无法处理真实地形中的坡度、斜坡和倾斜相机安装带来的俯仰、滚转和高度误差。

Method: 引入无人机图像作为中间视角，提供卫星图像无法获取的3D结构线索；构建Cross3R前馈模型，可同时处理卫星图、无人机图和地面图的任意组合，一次性恢复3D点云、6自由度位姿和地面位置。

Result: 在CrossGeo数据集上，Cross3R在点云重建、6自由度位姿估计和跨视角定位上优于基线方法；在KITTI上，未经过训练即超越多数专用方法。

Conclusion: 无人机中间视角能有效弥补卫星-地面跨视角定位的维度缺失，实现更鲁棒的6自由度定位。
...
TL;DR: SphereVAD是一种无需训练、零样本的视频异常检测框架，通过将异常判别转化为单位超球面上的vMF似然比测地线推理，利用预训练多模态大模型的中间层特征，在三个基准上达到训练-free方法的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VAD方法依赖大量标注或特定任务训练，难以快速部署到新场景。预训练MLLM的中间层特征已编码丰富异常语义，但现有方法仅利用语言输出路径，未能挖掘这些表示的几何判别能力。

Method: SphereVAD包含三个核心步骤：(1) Frechet均值中心化，展开特征分布并消除域偏差；(2) 整体场景注意力(HSA)，利用跨视频先验增强特征一致性；(3) vMF引导的球面测地线拉取(SGP)，将模糊片段与球面流形上的方向原型对齐。

Result: SphereVAD在三个主要基准上建立了训练-free方法的新SOTA结果，并与全监督基线保持竞争力。

Conclusion: 通过纯粹的几何推理而非学习新表示，SphereVAD成功释放了预训练MLLM中间层特征中潜在的判别能力，实现了零样本、无需训练的视频异常检测。
...
TL;DR: 本文介绍了Tree Ring Analyzer Suite (TRAS)，一个用于树木年轮自动检测、手动校正与测量的开源图形软件。该软件集成了三种检测算法，在18幅专家标注的松树横截面图像上，最佳算法F-score达81.0%，可将手动校正工作减少至约20%的环边界。


<details>
  <summary>Details</summary>
Motivation: 树木年轮标记是树木测量学和树木年代学中的关键步骤，但目前通常手工完成，过程耗时、主观且难以扩展到大规模图像数据集。

Method: 开发了TRAS开源图形软件，集成三种互补的年轮检测算法（经典图像处理方法CS-TRD和两种深度学习方法DeepCS-TRD、INBD），提供自动检测、手动校正、假阳性去除、缺失环补充以及年轮指标计算功能。

Result: 在18幅专家标注的松树横截面图像上，DeepCS-TRD算法F-score达81.0%，精确率86.4%；自动检测将手动校正工作减少至约20%的环边界；年轮宽度测量与CooRecorder高度一致（r > 0.99）。

Conclusion: TRAS为树木年轮分析提供了灵活且可重复的解决方案，支持Windows、macOS和Linux系统，代码已开源。
...
TL;DR: 本文提出STARFlow2，利用自回归归一化流（与LLM共享因果掩码、KV缓存机制）实现真正的统一多模态生成，在文本和图像生成、多模态理解任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态系统通常组合自回归语言模型和扩散图像生成器，两者存在因果文本生成与迭代视觉去噪之间的结构不匹配问题。

Method: 基于Pretzel架构，垂直交错预训练VLM流与TarFlow流，通过残差跳跃连接共享因果掩码；采用深-浅流设计和统一FAE潜在空间，使文本和视觉输出可直接进入KV缓存而无需重新编码。

Result: 在图像生成和多模态理解基准上均取得了强劲的性能表现。

Conclusion: 自回归归一化流可作为统一多模态建模的可行基础范式。
...
TL;DR: 提出PET-Adapter，一种测试时域自适应框架，使仅用幻影数据预训练的生成式PET重建模型能够适应不同临床数据集，无需配对真实数据，并将扩散步数从50减少到2。


<details>
  <summary>Details</summary>
Motivation: PET图像重建受泊松噪声和物理退化因素影响，尤其在有限角度采集中更为严重。现有深度学习方法泛化到未见临床数据分布时需大量重新训练。

Method: 提出PET-Adapter框架，包含逐层低秩解剖条件适配和基于有序子集期望最大化（OSEM）的热启动，从物理信息重建初始化生成过程。

Result: 在多个临床数据集上，全角度和有限角度设置下的3D重建性能均优于现有方法，扩散步数从50降至2且质量不损失。

Conclusion: 该方法展示了临床可行性和计算效率，为PET重建的域适应提供了有效解决方案。
...
TL;DR: HFRU是一种针对视觉语言模型的强化去学习框架，通过在视觉编码器上进行操作而非语言解码器，实现深层语义知识移除。该方法采用两阶段策略，结合对齐破坏与GRPO优化，在物体识别和人脸任务上实现98%以上的遗忘与保留性能，并极大减少物体幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有VLM去学习方法主要微调语言解码器，导致遗忘表面化，无法擦除底层视觉表征，并容易引入物体幻觉问题。

Method: 提出HFRU框架，在视觉编码器上操作，采用两阶段策略：对齐破坏结合基于GRPO的优化，使用包含抽象奖励的复合奖励函数，鼓励语义有效替换并缓解幻觉。

Result: 在物体识别和人脸身份任务上，HFRU实现超过98%的遗忘与保留性能，同时引入可忽略的物体幻觉，显著优于先前方法。

Conclusion: HFRU通过作用于视觉编码器的强化去学习框架，实现了更深入的语义知识移除，有效解决了现有方法的遗忘不彻底和物体幻觉问题。
...
TL;DR: 本文提出SCOPE框架，通过维护结构化规范追踪语义承诺，解决文本到图像生成中复杂意图实现的语义承诺生命周期断裂问题，在Gen-Arena等基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型虽在视觉保真度上取得进展，但在实现复杂视觉意图时，许多需求（语义承诺）在定位、生成和验证的全生命周期中无法保持一致的可识别性，导致概念断裂（Conceptual Rift），急需一种能持续追踪承诺的方法。

Method: 提出SCOPE框架，通过不断演化的结构化规范维护语义承诺，条件性地调用检索、推理和修复技能来处理未解决或违反的承诺。同时构建Gen-Arena人工标注基准，包含实体级和约束级规范，并设计Entity-Gated Intent Pass Rate (EGIP)评估指标。

Result: SCOPE在Gen-Arena上EGIP达到0.60，大幅超越所有基线；在WISE-V上达0.907，MindBench上达0.61。

Conclusion: 持续的承诺追踪对复杂图像生成任务有效，SCOPE框架证明了结构化规范维护技能在解决概念断裂问题上的优越性。
...
TL;DR: MoCoTalk是一种多条件视频扩散框架，通过自适应多条件路由器和嘴部增强着色网格实现高可控性的人脸说话视频生成，在多项指标上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅能处理部分因素（身份、头部姿态、面部表情、嘴部动态），且多条件融合时依赖固定权重或启发式策略，缺乏自适应能力。

Method: 1) 提出自适应多条件路由器，在通道维度和时间步上动态计算四个条件流的门控权重；2) 设计嘴部增强着色网格，基于3DMM解耦头部运动、嘴部运动、表情和光照；3) 引入唇部一致性损失增强音画对齐。

Result: 在结构、运动和感知指标上，MoCoTalk大多数达到SOTA性能。

Conclusion: MoCoTalk不仅性能领先，还提供了单条件方法无法实现的属性级可控性。
...
TL;DR: 提出一种检索引导的扩散噪声优化方法，通过关系任务解析和检索参考初始化，解决具有严格时空约束的人体运动生成问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理严重空间障碍或指定步数等严格时空约束任务时效果不佳，需要一种能处理高难度约束的运动生成方法。

Method: 基于训练无关的扩散噪声优化框架，引入检索引导机制：通过关系任务解析（利用LLM）对目标约束进行分组并识别困难约束，然后从大型运动数据集中检索参考，通过奖励引导掩码将随机噪声与检索噪声结合得到更好的初始化，再优化扩散噪声。

Result: 成功解决了具有高度约束的生成任务，能够处理严重空间障碍和指定步数等具有挑战性的时空调控需求。

Conclusion: 该方法通过检索引导和关系任务解析，显著提升了运动生成器在严格约束下的表现，且LLM使框架具备自动推理能力，提高了智能体的智能化水平。
...
TL;DR: 本文提出Flow-OPD，首个将在线策略蒸馏（OPD）引入Flow Matching的统一后训练对齐框架。通过两阶段对齐策略（单奖励GRPO微调专家 + 流式冷启动与异构知识整合）以及流形锚点正则化（MAR），在Stable Diffusion 3.5 Medium上显著提升了GenEval（63→92）和OCR精度（59→94），并展现出“超越教师”的效果。


<details>
  <summary>Details</summary>
Motivation: 现有Flow Matching文生图模型在多任务对齐中面临两大瓶颈：标量奖励导致的奖励稀疏性，以及联合优化异构目标产生的梯度干扰，这引发了指标间的“跷跷板效应”和普遍的奖励破解问题。

Method: 采用两阶段对齐策略：第一阶段通过单奖励GRPO微调培养领域专精的教师模型；第二阶段利用流式冷启动方案建立稳健的初始策略，并通过三步流程（在线策略采样、任务路由标注、密集轨迹级监督）将异构知识整合到单一学生模型中。同时引入流形锚点正则化（MAR），利用任务无关教师提供全数据监督，将生成锚定到高质量流形上。

Result: 基于Stable Diffusion 3.5 Medium，GenEval从63提升至92，OCR精度从59提升至94，整体比GRPO高出约10个百分点，同时保持图像保真度和人类偏好对齐，并展现出“超越教师”的涌现效应。

Conclusion: Flow-OPD为构建通用文生图模型提供了一种可扩展的对齐范式。
...
TL;DR: 聚合型元认知质量评分掩盖了不同模型在MMLU基准测试各领域内的表现差异。研究发现，不同模型的知识领域监控能力存在系统性差异，应用/专业知识域最容易监控，形式推理和自然科学最难。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常使用聚合指标评估模型的元认知能力，但忽略了模型在不同知识领域间的表现差异。作者旨在揭示这种被掩盖的域级变异，并验证其稳定性和实际意义。

Method: 对33个前沿大语言模型（来自8个模型家族）进行1,500个MMLU项目测试（每领域250题，六领域分组），使用口头置信度（0-100）计算每个模型-领域单元格的Type-2 AUROC，共47,151个观测值。

Result: 所有具超随机聚合监控能力的模型均表现出非平凡域级变异；应用/专业知识域最易监控（平均AUROC=0.742），形式推理与自然科学最难；Gemma 4 31B比Gemma 3 27B提升+0.202 AUROC；聚合稳定性r=0.893，但轮廓级稳定性弱（中位r=0.184）。

Conclusion: 聚合指标掩盖了稳定存在的基准域级变异，建议在特定应用场景部署前进行基准阶段域筛选。
...
TL;DR: VITA-QinYu是首个支持角色扮演和歌唱的端到端口语语言模型，采用混合语音-文本范式，在保持对话准确性和流畅性的同时显著提升了表达力。


<details>
  <summary>Details</summary>
Motivation: 人类语音能传递超越语言内容的表达力（如个性、情绪、角色扮演和歌唱），而现有口语模型主要专注于自然对话，缺乏对角色扮演和歌唱等丰富表达形式的支持。

Method: 采用混合语音-文本范式，通过多码本音频词元扩展交织的文本-音频建模，实现更丰富的副语言表示并保持模态间清晰分离；开发了包含15.8K小时自然对话、角色扮演和歌唱数据的综合数据生成流水线。

Result: 在客观角色扮演基准上超越同行口语模型7个百分点，歌唱MOS评分高出0.13分；对话准确性和流畅性在C3和URO基准上分别超越先前模型1.38和4.98个百分点。

Conclusion: VITA-QinYu证明了端到端口语模型可以在不牺牲基础对话能力的前提下，有效支持角色扮演和歌唱等丰富表达形式。
...
TL;DR: 本文提出了IntentGrasp基准，用于评估大语言模型的意图理解能力，并通过Intentional Fine-Tuning (IFT)方法显著提升该能力。实验表明，现有模型表现不佳（All Set上低于60%，Gem Set上低于25%），而IFT能带来30+ F1点的提升，并展现出强大的跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 准确理解语音、对话和文本中的意图对于开发有用的大语言模型助手至关重要，但目前缺乏系统评估LLM意图理解能力的全面基准。

Method: 构建了包含262,759个训练样本和两个测试集（All Set 12,909例、Gem Set 470例）的IntentGrasp基准，涵盖49个高质量开源语料库和12个领域。并提出Intentional Fine-Tuning (IFT)方法在训练集上微调模型以增强意图理解能力。

Result: 20个模型在All Set上得分低于60%，Gem Set上低于25%；17个模型在Gem Set上低于随机猜测基线（15.2%），人类表现约81.1%。IFT带来30+ F1点（All Set）和20+ F1点（Gem Set）的提升，留一域实验验证了跨领域泛化能力。

Conclusion: IFT是提升LLM意图理解能力的有效方法，为构建更有目的性、更强大且更安全的AI助手提供了有前景的方向。
...
TL;DR: 本文构建了塔吉克语-波斯语平行词汇资源TajPersLexon（含40,112词对），并通过三类方法（混合流水线、神经序列模型、检索方法）的CPU基准测试表明，神经和检索方法达到98-99%的top-1准确率，而可解释的混合模型在OCR后校正任务中以96.4%的准确率实现了良好的精度-效率平衡。


<details>
  <summary>Details</summary>
Motivation: 针对低资源场景下跨文字词汇检索、音译和对齐的需求，构建塔吉克语-波斯语平行词汇资源，并探索不同方法在该任务上的表现。

Method: 构建了40,112词对的塔吉克-波斯平行词汇资源；在CPU环境下对比了三类方法：轻量混合流水线、神经序列到序列模型、检索方法；所有实验使用固定随机种子确保可重复性。

Result: 神经和检索基线方法达到98-99%的top-1准确率；大语言多语言句子变换器在此精确词汇匹配任务上失败；可解释混合模型在OCR后校正任务中达到96.4%准确率。

Conclusion: 该词汇匹配任务本质上是可解的；可解释混合模型在精度与效率之间提供了有利折中，适合实际应用。
...
TL;DR: 本文介绍了MIST数据集，一个多轮语音驱动的IoT设备代码生成任务，发现开源与闭源多模态LLM之间存在显著性能差距，即使最先进的闭源模型仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型虽具备工具调用能力，但缺乏针对真实IoT设备场景（涉及时空约束、语音输入、动态状态追踪和混合主动交互模式）的系统性研究。

Method: 构建了MIST（多模态交互式语音工具调用数据集），这是一个合成生成的多轮、语音驱动的代码生成任务，专门针对IoT设备场景。

Result: 开源与闭源多模态LLM在MIST上存在显著性能差距；即使是前沿闭源模型也有较大的改进空间。

Conclusion: MIST数据集揭示了当前模型在处理物理世界约束的混合主动语音助手方面的不足，并通过开源数据集和可扩展的数据生成框架促进相关研究。
...
TL;DR: 本文提出了一个以人为中心的大语言模型（HCLLM）开发框架，主张在模型设计、数据收集、训练、评估和部署的全流程中，系统性地融入人类需求、价值观和目标，而不仅限于事后微调阶段。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在商业、教育、医疗等领域的广泛应用，模型开发需优先考虑人类需求而非仅关注技术能力。现有方法往往仅在后期阶段粗略处理人类因素，缺乏系统性的人本关怀。

Method: 整合NLP、人机交互（HCI）和负责任AI三个领域的视角，从伦理、经济和技术目标出发，为LLM开发的每个阶段（系统设计、数据来源、模型训练、评估、部署）提供以人为本的见解和建议，并结合案例研究进行验证。

Result: 形成了一套涵盖全流程的HCLLM开发框架，并通过对未来工作场景的案例研究展示了该框架的实际应用价值。

Conclusion: 开发人员应在LLM的每个开发阶段都严格、细致地考虑人类因素，而非仅在后期补救。HCLLM框架为构建更符合人类需求的语言模型提供了系统化的指导路径。
...
TL;DR: MELD是一个多任务学习框架，通过引入生成器家族、攻击类型和源域等辅助监督信号，显著提升了AI生成文本检测的鲁棒性、泛化能力和低误报率性能，在公共排行榜和基准测试上达到领先水平。


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测器仅优化单一的二分类目标，缺乏对生成器、攻击方式和领域结构的表征学习，导致在对抗攻击、跨域迁移和低误报率场景下性能不足。

Method: MELD采用共享编码器附加生成器家族、攻击类型、源域三个辅助分类头，通过homoscedastic uncertainty平衡多任务损失；使用EMA教师-学生蒸馏增强对抗鲁棒性；引入硬负样本对排序损失扩大AI与最难区分人类文本的分数间隔。

Result: 在RAID排行榜上成为最强开源检测器，与商业模型竞争力相当；在MELD-eval评估集上无需微调即实现99.9% TPR at 1% FPR，显著优于多个基线模型。

Conclusion: 多任务辅助监督能有效提升AI文本检测器的鲁棒性和泛化能力，且推理时无需额外开销，具备实际部署价值。
...
TL;DR: LLM在检索增强中难以根据上下文确定性调整回答，本文提出交互策略（先验提醒+确定性校准+上下文简化）将服从性错误平均降低25%。


<details>
  <summary>Details</summary>
Motivation: 高 stakes 领域（如医疗、金融）中，LLM需要根据检索信息的确定性水平适当调整回答，但现有研究对此关注不足。

Method: 评估8个LLM的上下文确定性服从能力，提出结合先验提醒、确定性校准和上下文简化的交互策略。

Result: LLM存在系统性局限：在不确定上下文后难以回忆先验知识、误解表达确定性、过度信任复杂上下文；所提策略将服从错误平均降低25%。

Conclusion: 交互设计能在不修改模型权重的情况下有效增强LLM的可靠性，贡献包括评估指标、实证洞察和可移植策略。
...
TL;DR: 论文提出了MultiSoc-4D，一个孟加拉语社交媒体基准数据集，并揭示了LLM在低资源语言标注中存在的"指令诱导标签坍缩"现象，即模型倾向于选择后备标签，导致漏检少数类别。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM被广泛用于NLP数据集的自动标注，但其在低资源语言中对封闭式指令的行为尚未得到充分研究。

Method: 构建包含58K+条孟加拉语社交媒体评论的MultiSoc-4D基准数据集，覆盖类别、情感、仇恨言论和讽刺四个维度，使用ChatGPT、Gemini、Claude、Grok分别标注，并共享20%验证集进行系统诊断。

Result: 发现LLM存在"指令诱导标签坍缩"现象，倾向于选择Other/Neutral/No等后备标签，导致79%的仇恨言论和75%的讽刺内容被漏检；讽刺检测的Fleiss' Kappa接近0（κ≈-0.001），证明了"标签一致幻觉"。

Conclusion: MultiSoc-4D可作为诊断孟加拉语NLP中标注偏差的基准，揭示了低资源语言场景下LLM自动标注的严重缺陷。
...
TL;DR: 本文提出Group of Skills (GoSkills)方法，将检索对象从扁平技能列表转化为角色标注的执行上下文（包含Start、Support、Check、Avoid四个字段），提升智能体对技能组的理解与执行效率。


<details>
  <summary>Details</summary>
Motivation: 现有技能检索方法返回原子技能或依赖感知的技能包，但技能内部的角色关系（如执行入口、支撑技能、可见前提、失败规避）对智能体而言是隐式的，导致智能体难以直接有效利用检索结果。

Method: GoSkills包括四个步骤：1）从类型化技能图中构建以锚点为中心的技能组；2）通过组图扩展支撑技能组；3）将选定的组计划瓶颈化为有界的原子技能负载集；4）渲染固定的执行契约，包含Start、Support、Check、Avoid四个字段。该方法不改变下游智能体、技能负载或执行环境。

Result: 在SkillsBench和ALFWorld上，GoSkills在小技能预算下保持了可见前提覆盖率，优于扁平技能访问基线，并在奖励和智能体运行时间上通常优于结构检索参考方法。

Conclusion: GoSkills通过结构化的技能分组和角色标注，有效提升了智能体对技能上下文的利用效率，在检索质量和执行表现上均有显著改进。
...
TL;DR: 本文提出了一种将文本建模为固定宽度二进制比特流上的连续扩散过程的方法，通过匹配滤波残差参数化和基于熵率门控的Langevin随机采样器，在LM1B和OpenWebText基准上达到了接近自回归模型的生成困惑度，同时消除了标准扩散语言模型中的O(V)词汇表扩展瓶颈。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在样本质量和多样性上长期落后于自回归模型，尽管连续流/扩散方法已缩小差距，但仍有进一步提升空间；同时标准DLM存在O(V)词汇表缩放瓶颈。

Method: 将语义Token编码为模拟比特序列，使用匹配滤波残差参数化分离上下文学习和独立比特后验；采用基于熵率门控的Langevin型校正随机采样器，在高信息区域集中随机性，低信息区域保持确定性。

Result: 130M参数的比特流模型在LM1B上达到GenPPL=59.76（熵4.31），使用256次NFE；在OWT上GenPPL=27.06（熵5.26），比1024-NFE基线少用4倍步数；通过语义比特补丁将复杂度从O(V)降至O(log V)。

Conclusion: 比特流扩散方法进一步缩小了与自回归模型的差距，通过消除词汇表瓶颈实现了更高效的可扩展语言生成范式。
...
TL;DR: 本文提出将临床诊断形式化为部分可观测马尔可夫决策过程（POMDP），包含提问、检查、诊断三种动作类型，并引入噪声模型。通过两阶段训练方法（监督微调+DAPO）训练MedExAgent，在诊断准确性和检查成本之间取得平衡，性能可比肩更大模型。


<details>
  <summary>Details</summary>
Motivation: 现有医疗大模型基准和自动诊断方法过于简化临床诊断过程，忽略了其交互性和不确定性，医生需要从对话和检查中获取信息，并应对不同患者特征和噪声信息。

Method: 将临床诊断建模为POMDP，包含三种动作类型（提问、检查、诊断）和系统噪声模型（7种患者噪声+3种检查噪声）。采用两阶段训练：先基于Calgary-Cambridge模型的结构化合成对话进行监督微调，再使用DAPO优化复合奖励函数。

Result: MedExAgent在诊断性能上达到与更大模型相当的水平，同时保持成本高效的检查策略。

Conclusion: 形式化临床诊断为POMDP并引入噪声模型能更真实地模拟临床环境，两阶段训练方法可训练出高效且经济的诊断智能体。
...
TL;DR: 本文提出WiCER算法，通过迭代编译、评估和精炼，解决LLM Wiki模式中将原始文档编译为知识库时关键事实丢失的问题，在17个领域6800个问题上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: LLM Wiki模式（通过KV缓存提供领域知识）虽然延迟低且零检索失败，但面临编译鸿沟：将原始文档编译为wiki时，关键事实会被灾难性丢弃。需要一种方法在编译过程中保留关键信息。

Method: 提出WiCER算法，灵感来自CEGAR（反例引导的抽象精炼）。算法通过诊断探针评估编译后的wiki，识别被丢弃的事实，并强制在后续编译中保留这些事实。

Result: 1-2次迭代即可恢复80%的质量损失（平均3.24 vs 原始全上下文的3.47），灾难性失败率降低55%。消融实验表明，针对性诊断（+0.95）而非通用固定（+0.16）是性能提升的关键驱动因素。

Conclusion: WiCER有效弥补了LLM编译的鸿沟，为构建可靠的领域知识库提供了可复现的解决方案。
...
TL;DR: 提出SCoL框架，让LLM在持续接收信息流时，通过生成文本指令选择性更新自身Transformer层，实现上下文巩固与知识保留，避免干扰。


<details>
  <summary>Details</summary>
Motivation: 长上下文窗口虽能暴露更多信息，但无法保证有用信息被保留和复用，现有方法在持续巩固上下文方面存在不足。

Method: SCoL：后训练框架，LLM根据当前上下文生成文本更新指令，指定要更新的Transformer层；通过元强化学习在演化模型状态下训练，使用监督QA奖励和基于似然的奖励。

Result: 在SQuAD知识融入和LongBench v2长上下文巩固上，SCoL在获取和保留方面优于提示、摘要、批量测试时训练和顺序微调基线；模型学会生成稀疏更新位置，与高Fisher信息层对齐。

Conclusion: SCoL支持可扩展的流式巩固，能从短元训练流迁移到更长的评估流，实现持续学习中的知识保留与干扰最小化。
...
TL;DR: 本文指出ASR评估中单一标注规范（参考一元论）对失语症患者造成认知不公，提出用WER-Range替代单一WER指标。


<details>
  <summary>Details</summary>
Motivation: 现有ASR评估默认单一转录规范为"真实标注"，忽略了不同标注规范（如逐字/非逐字/法律）对失语症患者口语中临床意义的非流利表达的差异化处理，造成系统性不公。

Method: 提出"诠释学鸿沟"哲学框架，形式化定义"认知不公距离"（EID）以量化参考一元论的代价，并在AphasiaBank数据集上实证比较不同标注规范下的WER差异。

Result: 实验表明，同一ASR输出的WER值随标注规范不同而显著变化；失语症患者的非流利表达在"干净"参考下被错误标记为错误。

Conclusion: 应放弃单一真值假设，采用WER-Range方法——在合法规范范围内报告性能区间，以包容多元语言特征并避免认知不公。
...
TL;DR: 提出SAGE框架，通过分层评估（文化、情感-心理、存在-哲学）和结构化LLM评估实现对文学质量的可靠测量，在600次评估中达到98.8%的分数收敛性和>94%的评分者一致性，并揭示了类型层级（经典>通俗>LLM）及各维度的区分效应差异。


<details>
  <summary>Details</summary>
Motivation: 文学质量评估涉及文化表征、情感深度、哲学思辨等难以计算测量的阐释维度，现有方法无法有效处理开放式文本生成的可扩展自动化评估问题。

Method: 提出SAGE分层评估框架，将文学质量分解为本体论 grounded 的阐释维度，通过结构化LLM评估（包含多轮迭代反思和独立验证）进行双模式评估，在100篇短篇小说（50篇经典、30篇通俗、20篇LLM生成）上验证。

Result: 600次评估中达到98.8%分数收敛性、>94%评分者一致性；经典>通俗>LLM的类型层级显著（p<0.001）；文化批判和哲学深度效应量极大（Cohen's d>2.4），情感表征效应较小（d=1.68）；跨层相关性r=0.649-0.683。

Conclusion: 理论驱动的LLM评估能够达到测量级别的可靠性，支持系统识别当前生成模型与人类文学创作之间的差距，为开放式文本生成的可扩展自动化评估提供了直接应用路径。
...
TL;DR: 提出RIS框架，通过空间-语义锚定、渐进注意力瓶颈和语言过渡令牌，解决多模态大模型中潜在视觉推理的流形不兼容问题，在多数据集上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在视觉-语言推理中将视觉证据压缩为离散文本思维，造成细粒度感知的信息瓶颈；最近的潜在视觉推理方法存在流形不兼容问题：潜在轨迹偏离预训练推理电路、坍塌为实例无关模式、且常被答案生成绕过。

Method: 1) 构建带有边界框和区域语义描述的逐步接地推理数据集；2) RIS框架：潜在令牌锚定到空间和语义证据，通过渐进注意力瓶颈强制其因果作用，引入短语言过渡令牌桥接合成潜在状态到词汇对齐解码。

Result: 在V*, HRBench4K, HRBench8K, MMVP, BLINK等数据集上一致优于闭源/开源及潜在推理基线；学习到多样化、可解释且渐进集成的潜在轨迹。

Conclusion: RIS提供了一条实现MLLMs忠实内部视觉推理的实用路径，有效解决了潜在推理的流形不兼容问题。
...
### [140] [Securing Computer-Use Agents: A Unified Architecture-Lifecycle Framework for Deployment-Grounded Reliability](https://arxiv.org/abs/2605.07110)
*Zejian Chen,Zhanyuan Liu,Chaozhuo Li,Mengxiang Han,Songyang Liu,Litian Zhang,Feng Gao,Yiming Hei,Xi Zhang*

Main category: cs.CL

TL;DR: 本文提出了一个面向计算机使用智能体（CUA）的架构-生命周期框架，从架构（感知、决策、执行三层耦合）和生命周期（创建、部署、运维、维护四阶段）两个维度系统分析其可靠性问题，揭示了故障可见性与致因条件之间的错位，并指出了可控 grounding、长程约束保持、安全权限绑定等六大开放挑战。


<details>
  <summary>Details</summary>
Motivation: 现有调研虽从方法、平台、基准或安全威胁角度组织了 CUA 领域，但未能清晰连接能力形成、权限暴露、故障表现与控制部署之间的因果链条，存在分析缺口。

Method: 采用架构-生命周期双视角框架：架构层分析感知、决策、执行三层的耦合如何将软件观察转化为携带权限的动作；生命周期层分析创建、部署、运维、维护四阶段中先验学习、工具绑定、运行时轨迹压力与持续保证问题。基于该透镜综合梳理代表性系统、基准测试及安全/隐私研究。

Result: 区分了故障在何处可见与其致因条件在何处引入，并映射了可用于控制、监督和保证的干预面。OpenClaw 仅作为公开部署模式的示例引用。

Conclusion: 总结了六大开放挑战：可控 grounding、长程约束保持、安全权限绑定、混合信任运行时防御、隐私保护记忆与持续保证。
...
### [141] [Beyond LoRA vs. Full Fine-Tuning: Gradient-Guided Optimizer Routing for LLM Adaptation](https://arxiv.org/abs/2605.07111)
*Haozhan Tang,Xiuqi Zhu,Xinyin Zhang,Boxun Li,Virginia Smith,Kevin Kuo*

Main category: cs.CL

TL;DR: 本文提出MoLF（混合LoRA与全参数微调）框架，通过优化器级别动态路由更新，在保持低资源消耗的同时匹配或超越全参数微调性能，并引入内存高效变体MoLF-Efficient。


<details>
  <summary>Details</summary>
Motivation: 全参数微调（FFT）在高熵知识注入中表现优异，而低秩适配（LoRA）在低秩空间任务中因正则化优势可匹敌FFT。然而，单一静态架构存在结构性局限，无法在所有任务中同时最优。

Method: 提出MoLF框架，在优化器级别动态路由更新至FFT和LoRA两种专家，确保梯度信号持续可用。另提出MoLF-Efficient，冻结基座权重，仅在不同秩的LoRA专家间路由更新。

Result: 在SQL、医学QA、反事实知识等任务及多个小模型（Gemma-3-1B、Qwen2.5-1.5B/3B）上，MoLF接近或优于FFT和LoRA最优者（差距≤1.5%），MoLF-Efficient在Fact任务上超越先前自适应LoRA方法达20%，在Med和SQL任务上达9%。

Conclusion: MoLF统一了FFT与LoRA的优势，实现了训练范式间的连续导航，且内存高效变体在资源受限场景下显著优于现有自适应LoRA方法。
...
### [142] [Region4Web: Rethinking Observation Space Granularity for Web Agents](https://arxiv.org/abs/2605.07134)
*Donguk Kwon,Dongha Lee*

Main category: cs.CL

TL;DR: 本文提出Region4Web框架，将网页观测从元素级提升到功能区域级，并通过PageDigest管道生成紧凑的区域级摘要，在WebArena基准上显著减少观测长度并提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有Web智能体以元素级粒度观测网页，功能组织隐含其中，智能体每步需自行从元素信号推断，缺乏对页面功能结构的高层理解。

Method: 提出Region4Web框架，通过层次化分解和语义抽象将AXTree重组为功能区域；并设计PageDigest推理管道，生成每页紧凑的区域级摘要，跨步骤持久化提供给动作智能体。

Result: 在WebArena基准上，PageDigest显著减少观测长度，并在多种骨干LLM和智能体方法上提升整体任务成功率。

Conclusion: 在功能区域粒度上操作比仅元素级处理能为动作智能体提供更紧凑且信息更丰富的观测基础。
...
### [143] [Structural Rationale Distillation via Reasoning Space Compression](https://arxiv.org/abs/2605.07139)
*Jialin Yang,Jiankun Wang,Jiajun Wu,Henry Leung,Jiayu Zhou,Steve Drew*

Main category: cs.CL

TL;DR: D-RPC通过压缩教师模型的推理路径为可复用的结构化模板，减少监督噪声，从而提升小模型蒸馏效果，并在多个推理基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 大模型蒸馏到小模型时，教师对相似问题的推理路径结构差异大、策略不一，这种不一致性给学生模型带来难以内化的噪声监督。

Method: 提出D-RPC方法，维护一个紧凑的动态推理路径库，对每个训练问题检索最相关路径，约束教师模型按该路径生成推理，兼顾一致性与多样性，并用PAC-Bayes分析路径库大小与覆盖率的权衡。

Result: 在五个数学和常识推理基准上，使用两种学生模型，D-RPC一致优于思维链蒸馏、自由形式推理生成、直接蒸馏和结构化监督基线，且比模板重型方法使用更少的token。

Conclusion: 通过路径压缩实现一致性蒸馏是有效的，且存在最优的路径库大小，平衡监督熵与覆盖率。
...
### [144] [Beyond Reasoning: Reinforcement Learning Unlocks Parametric Knowledge in LLMs](https://arxiv.org/abs/2605.07153)
*Wanli Yang,Hongyu Zang,Junwei Zhang,Wenjie Shi,Du Su,Jingang Wang,Xueqi Cheng,Fei Sun*

Main category: cs.CL

TL;DR: 通过零样本、一跳、闭卷问答实验发现，强化学习可以显著提升LLM对参数化知识的直接回忆能力（~27%平均相对提升），其机制并非获取新知识，而是将已有知识从低概率尾部重分布到可靠生成中。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习除了能提升LLM推理能力之外，是否也能改善模型对参数化知识的直接回忆（direct recall），这是一个尚未被充分研究的问题。

Method: 在零样本、一跳、闭卷问答设定下（无思维链），仅使用二元正确性奖励进行RL训练，并采用事实级别的训练-测试去重，确保提升来自回忆改善而非推理或记忆。在三个模型家族和多个事实问答基准上进行实验。

Result: RL带来约27%的平均相对提升，超过训练时和推理时基线。机制分析表明，RL主要将已有知识从低概率尾部重分布到贪婪生成中，而非获取新事实。数据归因研究显示，最难样本（答案在128次预RL采样中从未出现，约占训练数据18%）贡献了约83%的提升。

Conclusion: 强化学习在推理之外的角色被拓宽：它并非获取新知识，而是作为解锁潜在参数化知识的工具，将已有知识重新分配到可靠生成中。
...
### [145] [CLIPer: Tailoring Diverse User Preference via Classifier-Guided Inference-Time Personalization](https://arxiv.org/abs/2605.07162)
*Jinyan Su,Jinpeng Zhou,Claire Cardie,Wen Sun*

Main category: cs.CL

TL;DR: 提出CLIPer方法，通过轻量级分类器在推理时动态引导LLM生成符合不同用户偏好的个性化文本，无需大量微调，计算开销极小。


<details>
  <summary>Details</summary>
Motivation: 个性化LLM能显著提升用户体验，但针对所有用户偏好组合进行微调计算成本高且不现实，亟需一种轻量级、可扩展的个性化方案。

Method: 引入CLIPer（Classifier-guided Inference-time Personalization），在推理阶段使用一个分类器模型动态引导LLM生成文本，使其适配用户在不同维度上的偏好（如帮助性、简洁性、幽默感等）。

Result: 实验表明该方法在单一和多维偏好场景下均具有良好的可扩展性和有效性，能够实现可控且精细的个性化文本生成。

Conclusion: CLIPer提供了一种高效、轻量的LLM个性化方案，避免了大规模微调，为推理时动态个性化生成提供了可行路径。
...
TL;DR: 本文在PSU汉语隐喻语料库上建立了中文MIPVU框架下词级隐喻识别的多架构基线，系统比较了RoBERTa、MelBERT和Qwen3.5-9B三种模型家族，发现MelBERT MIP-only变体表现最佳（测试正例F1=0.7281），而Qwen生成式模型落后约11个F1点。


<details>
  <summary>Details</summary>
Motivation: 隐喻在日常语言中普遍存在，但中文在MIPVU框架下的词级隐喻识别研究相对英语明显不足，缺乏可复现的标准化基线。

Method: 在PSU CMC语料上对比三种方法：(i) 中文RoBERTa-wwm-ext-large编码器微调；(ii) 中文适配的MelBERT（基于MCD7构建基本义资源）；(iii) Qwen3.5-9B结合QLoRA的指令微调生成基线。使用五种固定种子进行系统比较。

Result: MelBERT MIP-only最佳（0.7281±0.0050 F1），略高于MelBERT Full（0.7270±0.0069），明显高于RoBERTa（0.7142±0.0121）。Qwen QLoRA仅0.6157±0.0113，落后约11个F1点。

Conclusion: MelBERT的SPV通道在中文中无正向贡献（因常规隐喻占主导）；Qwen与编码器的差距集中在召回率上，反映生成式输出的离散承诺限制；部分Qwen任务失败源于格式设计而非模型能力。已公开所有资源供后续研究参考。
...
TL;DR: 本文提出基于持续同调（持久同调）的拓扑增强对齐框架，通过轨迹拓扑损失（TTL）和拓扑偏好优化（TPO）分别改进SFT和DPO，在Qwen2.5-7B-Instruct上超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐方法（SFT/DPO）依赖局部token似然或标量分数，忽略了表征空间的全局几何结构，导致对齐方向缺乏拓扑约束。

Method: 1) SFT阶段：提出TTL，利用0维持续同调从提示与答案的混合点云中提取"提示-答案桥"，约束模型更新方向；2) DPO阶段：提出TPO，构建主题特定语义偏好向量，在中间隐层对齐拒绝与选择响应的改进方向，并引入动态权重平衡两项损失。

Result: 在Qwen2.5-7B-Instruct上，基于UltraChat和Anthropic HH-RLHF的评测显示，拓扑增强方法在自动偏好指标和LLM裁判评估上一致优于非拓扑基线（逐示例、近邻、随机正则器），同时保持或改善毒性指标。
...
TL;DR: 提出BoundaryRouter，一种无需训练的路由框架，通过早期行为经验和规则引导推理，在轻量级LLM推理和完整Agent执行之间进行智能路由，在RouteBench上实现60.6%的推理时间节省和28.6%的性能提升。


<details>
  <summary>Details</summary>
Motivation: LLM Agent在复杂推理任务上表现优异，但延迟高、计算成本大。实际中许多查询落在先进LLM能力边界内，无需完整Agent执行。因此，如何在轻量LLM推理和完整Agent执行之间进行有效路由是关键挑战。

Method: 提出BoundaryRouter，一种无需训练的路由框架。通过在一组共享种子集上执行两种系统来构建紧凑的经验记忆，在推理时检索相似案例，结合规则引导推理来决定是将查询交由直接LLM推理还是升级到Agent执行。

Result: 与Agent相比减少60.6%推理时间，比直接LLM推理提升28.6%性能，比基于提示和仅检索的路由分别平均提升37.9%和8.2%。

Conclusion: BoundaryRouter在冷启动场景下能有效平衡性能与计算成本，为LLM与Agent之间的智能路由提供了可行的训练-free解决方案。
...
TL;DR: 本研究揭示了LLM在检测词边界被破坏（插入空格）的文本时，检测准确率随插入率增加呈U型曲线，即“文本恐怖谷”现象。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准测试主要关注语法正确的输入，忽略了模型在不完美文本（如噪声文本）上的表现，存在评估盲区。

Method: 通过在单词内插入空白字符破坏词边界，改变插入率观察LLM目标信息检测准确率的变化，并提出了“模式转换假说”来解释U型曲线现象。

Result: 1) LLM检测准确率随插入率增加呈U型曲线；2) 上下文学习无法挽救谷底性能；3) 正则化扰动可减轻U型效应；4) 数学推理任务中较弱模型出现U型而强模型未出现；5) 分词熵在F1最小值前达到峰值。

Conclusion: LLM在处理词边界被破坏的文本时存在一种对干净文本基准不可见的失败模式，这对涉及噪声或未经整理文本输入的部署场景具有重要意义。
...
TL;DR: 本文描述了EEUCA 2026共享任务中分类《坦克世界》聊天消息毒性类别的系统，采用Llama 3.1 8B结合5%合成数据增强，F1-macro达0.6234，在35支队伍中排名第4，并揭示了"验证陷阱"现象。


<details>
  <summary>Details</summary>
Motivation: 针对游戏社区中 toxic 行为的自动检测需求，参与EEUCA 2026共享任务，旨在准确分类游戏聊天消息中的六类毒性内容。

Method: 探索了编码器模型、指令微调LLM（LoRA）、层次分类、一对多策略及集成方法等多种方法，最佳方案为Llama 3.1 8B结合5%合成数据增强。

Result: 测试集F1-macro分数为0.6234，在35支参赛队伍中排名第4。

Conclusion: 发现"验证陷阱"现象：高验证性能与较差的测试迁移相关，提示需关注验证集与测试集分布的一致性。
...
TL;DR: LaProx提出了一种基于输出感知和层间矩阵乘法近似的KV缓存淘汰策略，通过建模注意力图与投影值状态的交互来准确量化token贡献，在仅保留5% KV缓存的情况下保持模型性能，并在极端压缩场景下将准确率损失降低高达2倍。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存淘汰方法主要依赖局部注意力权重，忽略了值表示、输出投影以及跨头交互的影响，导致在长上下文推理中内存和运行时开销巨大。

Method: 将KV缓存淘汰重新表述为输出感知的层间矩阵乘法近似问题，显式建模注意力图与投影值状态之间的乘法交互，并提出统一淘汰策略赋予token全局可比较的重要性分数。

Result: 在LongBench和Needle-In-A-Haystack的19个数据集上，仅用5% KV缓存即可保持模型性能，在所有配置下一致优于先前方法，极端压缩下准确率损失减少高达2倍。

Conclusion: LaProx通过全局统一的token重要性评估和跨头依赖建模，显著提升了KV缓存压缩的效率，是现有方法的有效改进。
...
TL;DR: 本文提出ThinC框架，将代码本身作为推理主体而非工具，通过纯代码推理解决数学问题。ThinC-4B在五个竞赛级数学基准上超越所有TIR基线模型，甚至超过Qwen3-235B-A22B-Thinking。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成推理（TIR）范式存在三个关键局限：代码仅作为事后验证器、中间NL计算易出错、NL与代码角色重叠不清晰。因此需要一种更清晰的推理框架。

Method: ThinC采用"代码即推理者"的思路：先进行简短的NL规划，之后所有推理通过纯代码块进行，代码块之间仅通过执行输出连接。从教师模型蒸馏12.2k代码中心化轨迹，通过SFT+RL训练ThinC-1.7B和ThinC-4B。

Result: ThinC-4B在五个竞赛级数学基准上持续优于所有TIR基线，甚至超越Qwen3-235B-A22B-Thinking。99.2%的最终答案基于解释器输出，模型能从代码执行失败中可靠恢复。

Conclusion: ThinC证明了代码本身可以作为有效的推理主体，而非仅仅是NL的工具；纯代码推理路径比混合NL+代码的TIR范式更为可靠和高效。
...
TL;DR: SpecBlock是一种块迭代式草稿模型，通过保留路径依赖和低成本草稿生成，在LLM推测解码中相比EAGLE-3提升8-13%加速比，且草稿成本仅为EAGLE-3的44-52%。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码中的草稿模型存在两难困境：自回归式草稿模型（如EAGLE-3）保留路径依赖但每次深度需调用一次草稿器，导致延迟开销大；并行式草稿模型单次前向可预测多个位置，但各位置间缺乏依赖关系，导致验证器易拒绝。本文旨在兼顾路径依赖与低成本草稿生成。

Method: 提出SpecBlock，核心为块迭代式草稿器：1）块内通过层间移位传递前一位隐状态至所有解码层；2）块间允许新块从上一块任意位置继承隐状态扩展路径；3）共训练rank头替代固定top-k树以动态分配分支；4）有效前缀掩码避免训练无关前缀；5）部署时采用成本感知bandit根据验证器反馈选择性更新草稿器。

Result: SpecBlock相比EAGLE-3平均加速比提升8-13%，草稿成本仅为EAGLE-3的44-52%；引入成本感知自适应更新后，加速比优势扩展至11-19%。

Conclusion: SpecBlock通过块迭代设计成功兼顾了路径依赖与低成本草稿生成，并利用共训练rank头和成本感知自适应进一步优化验证器预算分配与部署效率，为推测解码提供了更高效的草稿生成方案。
...
TL;DR: 提出了一种自适应策略PaT（Planning-after-Trial），通过在验证失败后才调用规划器，显著降低了大型语言模型在代码生成中的推理成本，同时在性能上接近同质大模型，实现了约69%的成本降低。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如PbT）在测试时采用严格的“先规划后尝试”策略，即使对可直接求解的问题也会产生不必要的规划开销，导致计算资源低效分配。

Method: 提出PaT自适应策略：先用高效模型尝试生成代码，仅在验证失败后才调用规划器进行干预；支持异构模型配置（低成本模型负责生成，强大模型负责规划）。

Result: 在多个基准和模型家族上，PaT显著推进了成本-性能帕累托前沿；异构配置在性能接近大模型的同时，推理成本降低约69%。

Conclusion: PaT通过自适应、按需规划的策略，有效解决了测试时计算资源浪费问题，为高效推理提供了新范式。
...
TL;DR: 提出LogiHard框架，通过将0阶选择题转化为2阶逻辑判断，结合IRT自适应测试，揭示大语言模型存在组合推理缺陷，准确率下降31%-56%。


<details>
  <summary>Details</summary>
Motivation: 现有多选题基准面临快速饱和与数据污染问题，传统的加固方法（改写、扰动）虽增加表面复杂度但牺牲逻辑有效性，无法有效挑战先进推理模型。

Method: 提出LogiHard框架，将0阶选择转化为2阶逻辑判断；结合项目反应理论实现计算机自适应测试；通过9维认知分析对高风险考题排序，再组合变换构建高难度题目。

Result: 12个先进模型在组合加固题目上准确率下降31%-56%；LLM存在多选失败和提前退出偏差（人类无此现象）；零样本迁移到MMLU准确率从89.84%降至42.86%。

Conclusion: 性能退化与领域无关，源于组合推理缺口而非知识缺陷，反映的是训练导致的完整性验证缺陷。
...
TL;DR: 针对多语言环境下LLM系统的间接提示注入攻击，提出MIPIAD防御框架，结合LoRA微调序列分类器、TF-IDF特征和集成学习，在英语和孟加拉语数据上达到F1=0.9205、AUROC=0.9378的最佳性能。


<details>
  <summary>Details</summary>
Motivation: 检索增强和工具使用型LLM系统存在间接提示注入的持续弱点，且多语言环境下该问题更难刻画和防御。

Method: 提出MIPIAD防御框架：1) 使用LoRA微调Qwen2.5-1.5B得到序列分类器XLPID；2) 提取TF-IDF词法特征；3) 通过晚期融合、堆叠和梯度提升进行验证调优的集成。在基于BIPIA模板构建的跨5个任务家族的143万+合成样本上评估。

Result: TF-IDF+SVM的F1=0.77；XLPID+TF-IDF混合集成达到最佳F1=0.9205；Boosting集成达到最佳AUROC=0.9378。集成方法一致减少了英语-孟加拉语的跨语言差距。

Conclusion: MIPIAD框架在多语言提示注入防御中有效，具有可扩展性（NLLB-200支持200+语言），目前仅在英语和孟加拉语上验证，未来可拓展至更多语言。
...
TL;DR: 本文提出MedAction树状蒸馏管道，通过合成多轮诊断轨迹和两种知识图谱指标（DTC和RAC）来筛选高质量数据，构建MedAction-32K数据集，微调后的8B模型在医学主动诊断任务上达到开源SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有LLM诊断评估仅基于静态单轮设置（完整信息一次性给出），不符合真实临床中逐步收集证据、更新诊断的主动诊断过程，且现有模型存在三大失效模式：无根据的测试排序、不可靠的诊断更新、多轮连贯性下降。

Method: 提出MedAction树状蒸馏管道，通过LLM-环境交互合成多轮诊断轨迹；设计两种知识图谱指标DTC（追踪假设是否收敛至正确诊断）和RAC（验证信念更新是否基于收集的证据）来过滤轨迹质量；构建32,681条轨迹的MedAction-32K数据集。

Result: 在8B模型上微调后，在MedR-Bench和MedAction-300-Hard基准上达到开源模型SOTA性能。

Conclusion: 现有医学训练数据教会模型从完整信息推理，但未教会其在部分证据下行动；MedAction管道有效弥补了这一差距，推动了开源医学LLM在主动诊断任务上的前沿水平。
...
TL;DR: 推理语言模型生成的链式思维在顺序、密度和鲁棒性三个维度上均非必要：行级打乱不影响性能，移除数字导致完全失效，最激进压缩仍保持83%准确率。


<details>
  <summary>Details</summary>
Motivation: 挑战当前推理语言模型的两个隐含假设：推理链中的每个token都有贡献，且步骤必须按顺序消费。

Method: 对模型生成的推理链进行系统干预：移除、掩码、打乱和噪声注入，涉及三种模型和三个基准。

Result: 顺序不重要（行级打乱准确率下降<0.5pp），密度非必要（掩码数字导致0%准确率，掩码字母反而提升4.7pp），且对噪声高度鲁棒（注入假答案不影响性能）。

Conclusion: 答案提取操作在稀疏、顺序不敏感且结构鲁棒的信息基底上运行，为并行化和token高效的推理生成开辟了新路径。
...
TL;DR: LaTER是一种两阶段推理范式，先在连续潜空间进行有限探索，再切换到显式思维链进行验证和答案生成，在Qwen3-14B上减少16%-32%的token使用量，同时在AIME 2025上提升准确率，微调后准确率达80.0%，比标准CoT基线高10个百分点，同时使用33%更少的token。


<details>
  <summary>Details</summary>
Motivation: Chain-of-thought推理虽然能提升大模型在困难任务上的表现，但每个中间步骤都需要生成离散token，导致推理成本高昂。潜空间推理虽能减少可见token生成，但完全替换为潜计算会损害需要符号检查的任务。因此需要一种兼顾效率与准确性的方法。

Method: 提出Latent-Then-Explicit Reasoning (LaTER)两阶段范式：第一阶段在连续潜空间进行有界探索，第二阶段切换到显式CoT进行验证和答案生成。在无训练实例化中，通过将最后一层隐状态投影回输入嵌入空间、保留潜KV缓存，并使用熵和模型原生停止标记探针决定切换时机。此外构建了Latent-Switch-69K监督语料，通过潜展开和停止监督进行微调。

Result: 在Qwen3-14B上，无训练LaTER在多个基准上减少16%-32%的token使用量，同时匹配或提升准确率。AIME 2025从70.0%提升至73.3%，token从15,730减少到10,661。微调后LaTER在AIME 2025上达到80.0%准确率，比标准CoT基线高10.0个百分点，使用33%更少的token。

Conclusion: LaTER通过潜显结合的推理范式有效平衡了推理效率与准确性，验证了强推理模型在潜空间接口下已具备结构化的潜轨迹，为提升大模型推理性能提供了一种新范式。
...
TL;DR: 本文研究了两种稀疏自编码器（Crosscoders 和 Differential SAE）在检测微调语言模型后门攻击中的效果，发现 Diff-SAE 在隔离后门特征方面显著优于 Crosscoders，达到 0.4 的后门隔离分数和完美的精确率，而 Crosscoders 几乎完全失效。


<details>
  <summary>Details</summary>
Motivation: 后门攻击对语言模型安全构成严重威胁，但目前通过机械可解释性检测后门仍是一个未解决的挑战，亟需有效的检测方法。

Method: 使用基于年份上下文（"2024"触发漏洞代码，"2023"触发安全代码）的 SQL 注入后门，在 SmolLM2-360M 模型上比较 Crosscoders 和 Diff-SAE 两种稀疏自编码器架构，在 LoRA 和全秩微调两种设置下进行评估。

Result: Diff-SAE 的后门隔离分数（BIS）达到 0.40，精确率 1.0，假阳性率为零；Crosscoders 的 BIS 在大多数情况下低于 0.02。全秩微调产生的后门信号更清晰。

Conclusion: 后门表现为方向性激活偏移而非稀疏特征激活，因此基于差异的表示（Diff-SAE）在检测后门上更有效，对 AI 安全监控具有重要意义。
...
### [161] [Mean-Pooled Cosine Similarity is Not Length-Invariant: Theory and Cross-Domain Evidence for a Length-Invariant Alternative](https://arxiv.org/abs/2605.07345)
*Sibayan Mitra,Dhruv Kumar*

Main category: cs.CL

TL;DR: 平均池化余弦相似度作为跨语言/模态/任务表征比较的默认指标存在长度不不变性问题：由于Transformer表征的各向异性，该指标会随序列长度单调增长。实验证明长度比可解释代码LLM跨语言相似度中52%-75%的方差，而CKA指标可消除这一偏差。建议采用长度不变指标（如CKA）作为默认比较方法。


<details>
  <summary>Details</summary>
Motivation: 揭示当前广泛使用的平均池化余弦相似度在跨表征比较中存在的长度偏差问题，并呼吁采用更稳健的指标。

Method: 理论分析各向异性与长度的关系，在代码LLM（HumanEvalPack）、多语言模型（Mistral-7B, WMT数据）和视觉模型（CLIP ViT-B/32）上对比平均池化余弦与CKA的表现。

Result: 长度比解释了代码LLM跨语言相似度52%-75%的方差；CKA将解释方差降低83%且反转长度系数符号。该模式在多语言和视觉模型上均成立。平均池化相对EOS池化可减轻但无法消除长度效应。

Conclusion: 长度不变指标（如CKA）应成为跨表征比较的默认选项，基于平均池化余弦的跨语言表征趋同结论需要重新审视。
...
### [162] [Generating training datasets for legal chatbots in Korean](https://arxiv.org/abs/2605.07432)
*Changhoe Hwang,Jee-Sun Nam,Eric Laporte*

Main category: cs.CL

TL;DR: 本文提出了一种基于本地语法图（LGG）自动生成大规模标注语料的方法，用于训练法律聊天机器人，并在韩语法律聊天机器人LIGA上取得了91%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 法律聊天机器人可提升司法可及性，但深度学习对话系统需要大量标注数据，人工标注成本高昂，且难以覆盖用户输入的多样性。

Method: 利用语言学家构建的本地语法图（LGG）捕捉词汇和局部句法，自动生成大规模带标签的话语数据；采用Unitex平台生成7亿条话语，并用DIET分类器进行意图分类训练。

Result: 生成的模型在分类任务上达到91%的F1分数，并成功部署在韩语法律聊天机器人LIGA中，用于根据用户查询推荐相关判例页面。

Conclusion: 基于LGG的自动标注数据生成方法能有效降低数据标注成本，同时保证分类器的高性能，为法律领域对话系统提供可行方案。
...
### [163] [SSP-based construction of evaluation-annotated data for fine-grained aspect-based sentiment analysis](https://arxiv.org/abs/2605.07446)
*Suwon Choi,Shinwoo Kim,Changhoe Hwang,Gwanghoon Yoo,Eric Laporte,Jeesun Nam*

Main category: cs.CL

TL;DR: 本论文构建了韩语评价标注语料库EVAD，采用半自动符号传播方法进行标注，扩展了基于方面的情感分析（ABSA）以覆盖电商评论中的情感和非情感语言模式，并通过引入方面值以及KoBERT/KcBERT模型评估，取得了F1 0.88-0.90的鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 现有ABSA方法在电商评论中未能充分覆盖情感与非情感语言模式，且缺乏针对韩语的细粒度评价标注语料库，因此需要构建一个包含详细ABSA组件的韩语电商领域语料库。

Method: 采用半自动符号传播（SSP）进行标注，构建基于有限状态转导器（FST）的语言资源；扩展ABSA，引入方面值（aspect values）并将话题、方面与方面值三元组按一元、二元、多元分类；使用KoBERT和KcBERT模型进行训练和评估。

Result: KoBERT模型F1 score为0.88，KcBERT模型F1 score为0.90，在方面-值对识别任务上表现鲁棒。

Conclusion: 所构建的EVAD语料库和扩展ABSA方法能有效提升韩语电商评论的情感分析细粒度，且基于BERT的模型在该任务上取得了良好性能。
...
### [164] [Data Contamination in Neural Hieroglyphic Translation: A Reproducibility Study](https://arxiv.org/abs/2605.07453)
*Ammar Toutou,Abdelrahman Harb,Christine Basta*

Main category: cs.CL

TL;DR: 研究发现先前报道的象形文字-德语翻译61.5 BLEU存在严重数据污染（测试集2%目标文本与训练集完全重复），实际正确基线仅为30.9-39.2 BLEU，强调低资源濒危语言NLP中数据污染审计的必要性。


<details>
  <summary>Details</summary>
Motivation: 濒危古语言NLP面临数据稀缺且质量难以审计的挑战，需要评估当前NMT模型的真实能力。

Method: 复现先前研究（基于M2M-100微调的象形文字-德语翻译）；检测训练-测试集重叠（n-gram分析）；进行文档级与目标级去污染；在多个模型架构（5种配置、2种架构）上对比受污染与干净样本的性能。

Result: 复现仅得到37.0 BLEU（原报告61.5 BLEU）；发现2%测试目标与训练集完全重复；受污染样本BLEU高达83.8，干净样本仅30.9-39.2；文档级去污染仅降低4.6 BLEU，目标级去重才有效。

Conclusion: 提出正确基线（30.9-39.2 BLEU）并发布去污染测试集，强调低资源濒危语言NLP研究必须严格审计数据污染，且需采用目标级去重方法。
...
### [165] [GRaSp: Automatic Example Optimization for In-Context Learning in Low-Data Tasks](https://arxiv.org/abs/2605.07454)
*Simen Bihaug-Frøyland,Henrik Brådland*

Main category: cs.CL

TL;DR: 提出GRaSp框架，通过生成合成候选池、聚类降维结构化和遗传算法优化，在金融NER任务上提升上下文学习性能，实验表明非合成数据优于基线，合成数据受限于分布多样性。


<details>
  <summary>Details</summary>
Motivation: 大模型的上下文学习性能对选取的示例高度敏感，尤其在领域特定、低数据场景下，高质量示例稀缺，亟需自动化的示例优化方法。

Method: 三阶段框架：1) 生成大规模合成候选池；2) 通过聚类和降维进行结构化组织；3) 使用遗传算法（含多样性自适应变异机制）搜索最优上下文示例。

Result: 在FiNER-139数据集上，非合成数据达到45.84% micro-F1，优于零样本和随机少样本基线；合成数据仅匹配随机基线，未能超越。

Conclusion: 候选池的分布多样性对泛化至关重要，合成数据因缺乏足够多样性而表现受限。
...
### [166] [Think-with-Rubrics: From External Evaluator to Internal Reasoning Guidance](https://arxiv.org/abs/2605.07461)
*Jiachen Yu,Zhihao Xu,Junjie Wang,Yujiu Yang*

Main category: cs.CL

TL;DR: 本文提出Think-with-Rubrics范式，将评估准则（rubric）从外部评价器转变为LLM生成过程的内部引导，通过联合训练（序列生成rubric+回答）和rubric验证器监督，在指令跟随任务中平均提升3.87分。


<details>
  <summary>Details</summary>
Motivation: 现有框架将rubric视为独立于策略推理轨迹的外部评估器，仅用于事后测量，无法主动指导模型的生成过程。

Method: 训练时LLM先顺序生成rubric再生成回答，同时使用训练的rubric验证器通过评估答案与自生成/标准rubric的一致性来提供联合监督。

Result: 在多个基准测试中，Think-with-Rubrics相比Rubric-as-Reward基线平均提升3.87分。

Conclusion: 标准rubric监督提升自生成rubric质量，自生成rubric监督增强回答内部一致性，两者协同提升模型性能。
...
### [167] [The Moltbook Files: A Harmless Slopocalypse or Humanity's Last Experiment](https://arxiv.org/abs/2605.07462)
*William Brach,Federico Torrielli,Stine Lyngsø Beltoft,Annemette Brok Pirchert,Peter Schneider-Kamp,Lukas Galke Poech*

Main category: cs.CL

TL;DR: 本文分析了Moltbook（一个由OpenClaw智能体驱动的类Reddit平台）上12天内生成的23.2万条帖子和220万条评论，发现智能体会泄露API密钥等敏感信息，且在该数据上微调模型会降低真实性（从0.366降至0.187），但控制实验表明这与Reddit数据效果相近，因此Moltbook更像是一种无害的"垃圾内容洪流"，但仍存在尾部风险。


<details>
  <summary>Details</summary>
Motivation: 研究大规模智能体群体行为中的涌现现象及其安全影响，特别是公开平台上智能体自主交互带来的潜在风险。

Method: 构建PII去除管线；分析社区结构、作者特征、词汇属性、情感、主题、语义几何和评论交互；使用三种适配级别在Moltbook数据上微调Qwen2.5-14B-Instruct模型。

Result: 智能体泄露了API密钥、密码、BIP39助记词等敏感信息；整体情感以中性为主（66.6%中性，19.5%积极），存在自我引用倾向；微调后模型真实性从0.366降至0.187，但与Reddit数据微调效果相当。

Conclusion: Moltbook目前更像无害的"垃圾内容洪流"，但仍存在尾部风险，包括智能体能力滥用、自我链接污染未来数据爬取、以及特性向下一代语言模型传递的可能。强调了在涌现性对齐偏差评估中设置控制基线的重要性。
...
### [168] [SEIF: Self-Evolving Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2605.07465)
*Qingyu Ren,Qianyu He,Jiajie Zhu,Xingzhou Chen,Jingwen Chang,Zeye Sun,Han Xia,Fei Yu,Jiaqing Liang,Yanghua Xiao*

Main category: cs.CL

TL;DR: SEIF是一个自我进化的强化学习框架，通过指令难度与模型能力的协同进化，持续提升大语言模型的指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的外部监督或静态难度指令，无法随模型能力提升而动态调整，限制了指令遵循能力的持续改进。

Method: SEIF包含四个角色：Instructor生成递增难度指令、Filter过滤无效指令、Follower学习遵循指令、Judger提供奖励信号，通过交替训练实现指令难度与模型能力的协同进化。

Result: 多模型规模与架构实验表明SEIF能持续提升指令遵循性能，具有强泛化性。

Conclusion: 自我进化任务的有效训练策略是：早期充分训练打好基础，后期适度训练防止过拟合。
...
### [169] [TCMIIES: A Browser-Based LLM-Powered Intelligent Information Extraction System for Academic Literature](https://arxiv.org/abs/2605.07507)
*Hanqing Zhao*

Main category: cs.CL

TL;DR: 提出TCMIIES，一个基于浏览器的零安装平台，利用商业LLM API从学术文献中进行结构化信息提取，无需编程即可自定义提取模式，在中医研究中达到94%以上的结构化输出合规率。


<details>
  <summary>Details</summary>
Motivation: 学术出版物呈指数级增长，而现有自动化信息提取工具需要专业基础设施、编程技能或微调模型，对专业领域研究者造成障碍。

Method: 采用纯前端架构，通过模式引导提示框架和自动系统提示生成，支持五种主流LLM提供商，实现并发批量处理和自动重试机制，并提供针对CNKI和万方等中文数据库的智能字段映射。

Result: 在中医研究多个提取场景中，结构化输出合规率超过94%，信息提取准确性与领域专家标注相当。

Conclusion: TCMIIES弥合了先进LLM能力与领域特定学术信息提取需求之间的差距，为专业领域研究者提供了灵活、隐私保护且成本效益高的提取工具。
...
### [170] [WeatherSyn: An Instruction Tuning MLLM For Weather Forecasting Report Generation](https://arxiv.org/abs/2605.07522)
*Zinan Zheng,Yang Liu,Nuo Chen,Juepeng Zheng,Hong Cheng,Jia Li*

Main category: cs.CL

TL;DR: 本文提出了天气预报报告生成任务(WFR)，构建了首个指令微调数据集，并开发了专用模型，在多个指标上优于闭源多模态大模型。


<details>
  <summary>Details</summary>
Motivation: 当前天气预报主要依赖人工分析多源数据，存在信息过载和效率低下的问题，而多模态大模型在该领域的应用尚未充分探索。

Method: 提出WFR任务，构建覆盖美国31个城市、8个天气方面的指令微调数据集，在此基础上开发了首个专门生成天气预报报告的模型。

Result: 该模型在多个评估指标上持续优于领先的闭源多模态大模型，尤其在结构复杂的天气方面表现突出，并展现出良好的跨区域零样本泛化能力。
...
TL;DR: 针对心理防御机制检测中类别边界模糊的问题，提出9投票器集成系统，沿类别粒度、训练方法、基础模型三个正交轴设计，在PsyDefDetect共享任务中取得F1=0.420，排名第一。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制检测任务中，八个积极防御类别在表层语言上相似，仅语用功能不同，人工标注者间一致性仅中等，存在固有模糊性。

Method: 构建9个投票器的集成系统，沿三个正交轴设计：类别粒度（门控器用全部9类，专家用8个防御类）、训练方法（生成式与判别式）、基础模型。

Result: 在隐藏测试集上F1=0.420，在21个注册队伍中排名第一。

Conclusion: 在类别边界模糊的任务中，通过正交化设计实现错误独立性，集成策略优于单一强模型。
...
TL;DR: 本文提出了一种面向对话式新闻推荐的意图驱动语义ID生成方法，通过"生成-匹配"范式解决了隐式意图检索和冷启动用户推荐难题，在7B模型上实现了零幻觉和超越GPT-4+混合RAG的性能，同时成本降低约100倍。


<details>
  <summary>Details</summary>
Motivation: 现有对话式新闻推荐系统面临两大挑战：一是用户表达隐式意图（共6类，其中5类为隐式），标准RAG流程难以有效检索；二是冷启动用户缺乏历史行为数据，现有基线方法得分为0%。

Method: 采用两阶段训练：多任务语义ID对齐和GPT-4思维链蒸馏。LLM将多样意图映射为层次化语义ID前缀，再通过与当前新闻池模糊匹配实现完全基于事实的推荐。同时提出轮廓感知双信号推理(PADR)使冷启动用户仅凭画像即可获得有效推荐。

Result: 在主流中文新闻平台上，7B模型在152K开放生成语义ID空间中实现0%幻觉和12.4% L1匹配（4倍随机基线）。在L1指标上与GPT-4+混合RAG持平，在细粒度指标上超越（L2 2倍，类别+1.2pp），成本降低约100倍。冷启动用户达到18.0% L1匹配（6倍随机），为所有用户组中最高。

Conclusion: 提出的生成-匹配范式有效解决了对话式新闻推荐中的隐式意图检索瓶颈和冷启动问题，证明了结合语义ID生成与模糊匹配的方法在性能和效率上均优于传统RAG方案。
...
TL;DR: 本文通过从头训练荷兰语BERT模型，研究了性别信息在训练过程中如何出现和演化。研究发现，尽管性别信息在约第20个epoch后变得线性可分离，但模型难以根据显式的上下文性别线索（如"她是水管工"）更新其内部性别表示，导致反刻板印象配对预测准确率低，且荷兰语中的通性形式系统性地默认为男性解读。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多关注英语中的性别偏见，而带有语法性或形态性别的语言（如荷兰语）研究不足。本文旨在填补这一空白，探究荷兰语BERT模型中性别信息的形成过程及其对上下文线索的敏感度。

Method: 使用从头训练的荷兰语BERT模型，提取训练过程中不同检查点的上下文嵌入，通过线性SVM构建动态性别子空间，追踪性别信息何时变得线性可编码及其演化过程。同时，设计控制句子模板（如"她是水管工"）测试显式性别线索能否覆盖统计关联。

Result: 性别信息在约第20个epoch后变得线性可分离，分布在多个嵌入维度上；但模型难以根据显式的上下文性别线索更新内部性别表示，反刻板印象配对预测准确率远低于刻板印象配对；荷兰语通性形式系统性地默认为男性解读，即使上下文明确指向女性。

Conclusion: 荷兰语BERT模型所学到的表示中的语境化沿着所探测的性别方向不够动态：反刻板印象语境中的显式性别线索未能可靠地反映在最终表示中，导致持续的男性默认行为。
...
### [174] [Post-training makes large language models less human-like](https://arxiv.org/abs/2605.07632)
...
TL;DR: 论文发现，将基础模型转变为有用助手（如ChatGPT）的后训练过程，会持续降低模型对人类行为的模拟准确性。即便基础模型本身在改进，新版本的模型反而更偏离人类行为模式。而用于诱导类人行为的“角色注入”技术，在个体层面也无法提升预测效果。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型被越来越多地用作人类参与者的替代品，但尚不清楚哪些模型最能捕捉人类行为以及原因。本文旨在系统性地测量和评估不同模型在行为对齐上的表现。

Method: 作者构建了Psych-201数据集，用于在规模上测量行为对齐。通过比较不同模型家族、规模和训练目标的对齐程度，分析后训练（post-training）和角色注入（persona-induction）等技术的影响。

Result: 后训练一致性地降低了模型与人类行为的对齐，这种错位在新一代模型中更加严重，即使基础模型本身在持续改进。角色注入技术也无法在个体层面改善预测效果。

Conclusion: 当前用于将LLM转变为有用助手的流程，反而使它们成为不太准确的人类行为模型。
...
TL;DR: 本文评估了最新LLM在语法纠错任务上的表现，发现微调后的GPT-4o在三项指标上均达到最优，且不同LLM的纠错模式高度相似（ρ=0.947）。同时指出基于参考标准的评估指标会低估GEC系统性能，有73.76%的GPT-4o修正与标准答案不同但同样有效或更优。


<details>
  <summary>Details</summary>
Motivation: 语法纠错自动化助手已广泛应用于教育平台，但目前存在三个关键空白：(1)最新LLM缺乏语法纠错任务的全面评估；(2)组合LLM能否提升纠错质量尚未探索；(3)基于参考标准的指标对GEC系统性能的低估程度未被充分量化。

Method: 首先评估最新LLM在编辑精度、流畅性保持和意义保留三方面的表现；其次通过语法错误类型分析考察个体LLM的纠错模式相似性；最后分析参考标准指标对GEC性能的低估程度。

Result: 微调后的GPT-4o在三项指标上达到最优性能；个体LLM的纠错模式高度相似（ρ=0.947）；73.76%的GPT-4o修正与标准答案不同但同样有效或更优。

Conclusion: 这些GEC评估发现为教育者在选择语法纠错助手时提供了指导，帮助选择能够促进学生语言发展而非限制其发展的工具。数据、代码和模型已公开。
...
### [176] [Quality-Conditioned Agreement in Automated Short Answer Scoring: Mid-Range Degradation and the Impact of Task-Specific Adaptation](https://arxiv.org/abs/2605.07647)
*Abigail Victoria Gurin Schleifer,Moriah Ariely,Beata Beigman Klebanov,Asaf Salman,Giora Alexandron*

Main category: cs.CL

TL;DR: 自动简答题评分（ASAS）正从判别式微调模型转向少样本大语言模型（LLM）。研究发现，所有AI模型在完全正确和完全错误的回答上表现良好，但在中等质量（部分正确）的回答上表现显著下降，且这种下降程度与任务特定适配程度相关：少样本LLM下降最严重，微调编码器模型表现最好。这可能导致对理解发展中的学生产生不公平评价。


<details>
  <summary>Details</summary>
Motivation: 探究不同模型的任务特定适配程度与质量条件化评分一致性之间的关系，特别关注ASAS在部分正确（中等质量）回答上的评分表现，因为这一问题尚未被充分研究。

Method: 比较三种LLM（GPT-5.2、GPT-4o、Claude Opus 4.5）在少样本模式下的表现、一个微调的BERT编码器模型以及人类专家在两个开放式生物学题目上的评分结果，使用数百条学生回答和领域专家提供的地面真值分数进行评估。

Result: 人类评分者之间的一致性最高且在整个质量谱上稳定。所有AI模型在完全正确和完全错误的回答上表现良好，但在中等质量回答上出现显著性能下降。这种下降与任务特定适配程度相关：少样本LLM（示例少）下降最严重，随着任务特定数据增加而改善，微调编码器模型表现最佳。

Conclusion: 中等质量回答的评分退化可能导致对理解发展中的学生产生不公平评价。研究强调了质量条件化公平性的重要性，尤其需要关注中等质量（部分正确）的回答。
...
### [177] [Not All Tokens Learn Alike: Attention Entropy Reveals Heterogeneous Signals in RL Reasoning](https://arxiv.org/abs/2605.07660)
*Gengyang Li,Zheng-Fan Wu,Siqi Bao,Yunfang Wu*

Main category: cs.CL

TL;DR: 本文通过注意力熵（attention entropy）揭示了强化学习后训练中token级信号存在"锚点-探索者"异质性：低熵token提供稳定优化骨干，高熵token包含困难推理信号但梯度不稳定；基于熵的动态软重加权方法将Qwen3-8B-Base的持出平均分从34.39提升至37.40。


<details>
  <summary>Details</summary>
Motivation: 强化学习后训练虽能提升大模型推理能力，但其token级学习信号的性质尚不明确，作者希望通过注意力熵这一视角来理解token级强化学习目标的异质性。

Method: 1. 证明token级RL目标稀疏可估（20%随机token保留大部分性能）；2. 按注意力熵将token分为低熵"锚点"和高熵"探索者"；3. 分析两类token的梯度稳定性、优化行为及对推理性能的影响；4. 提出动态熵感知软重加权干预方法。

Result: 低熵锚点token提供稳定的梯度对齐和优化骨干，但在困难基准上易饱和；高熵探索者token梯度波动大、训练不稳定，但少数成功运行表明其包含困难推理信号；动态熵感知软重加权将Qwen3-8B-Base从34.39提升至37.40。

Conclusion: 注意力熵能揭示token级RL信号中与优化相关的结构，均匀token平均会掩盖有意义的异质性，熵感知的加权策略能有效提升推理后训练效果。
...
### [178] [DRIP-R: A Benchmark for Decision-Making and Reasoning Under Real-World Policy Ambiguity in the Retail Domain](https://arxiv.org/abs/2605.07699)
*Hsuvas Borkakoty,Sebastian Pohl,Cheng Wang,Bei Chen,Yufang Hou*

Main category: cs.CL

TL;DR: 本文提出DRIP-R基准，系统性地利用零售政策歧义构建无唯一正确答案的退货场景，实验表明前沿模型在政策歧义场景中存在根本性分歧。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准大多假设策略明确无歧义，但真实世界中的领域策略往往存在多重合理解释，缺乏评估模型处理政策歧义能力的基准。

Method: 构建包含策略歧义退货场景、真实顾客画像、全双工会话模拟及多裁判评估框架（涵盖政策遵循、对话质量、行为对齐、解决质量四个维度）的DRIP-R基准。

Result: 前沿模型在相同的策略歧义场景上存在根本性分歧，证实歧义对LLM决策构成了真实且系统性的挑战。

Conclusion: 政策歧义是LLM智能体评估中被忽视的关键维度，DRIP-R填补了这一空白，并为未来研究提供了基准。
...
### [179] [Guidance Is Not a Hyperparameter: Learning Dynamic Control in Diffusion Language Models](https://arxiv.org/abs/2605.07701)
*Fan Zhou,Tim Van de Cruys*

Main category: cs.CL

TL;DR: 本文提出通过强化学习动态调整扩散模型中的无分类器引导（CFG）尺度，替代传统的固定超参数设置，在NLP生成任务中更好地平衡可控性与生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统的CFG引导尺度在整个生成过程中是固定的，这导致可控性与质量之间的权衡是次优的，因为最优引导程度在不同任务和扩散过程的不同阶段是变化的，尤其在NLP领域。

Method: 将CFG尺度选择建模为序列决策问题，使用强化学习（PPO）学习动态引导轨迹。在每个生成步骤中，基于当前的扩散状态将引导尺度作为离散控制动作进行选择，并通过任务级奖励优化策略。

Result: 在三个受控NLP生成任务上的实验表明，自适应引导策略在可控性与生成质量之间始终优于固定尺度策略。学习到的策略展现出跨任务的不同且可解释的引导轨迹。

Conclusion: 引导应被视为一个动态控制过程而非静态设计选择，自适应引导策略能更好地平衡可控性与生成质量。
...
### [180] [SimCT: Recovering Lost Supervision for Cross-Tokenizer On-Policy Distillation](https://arxiv.org/abs/2605.07711)
*Jie Sun,Mao Zheng,Mingyang Song,Qiyong Zhong,Yilin Cheng,Bichuan Feng,Pengfei Liu,Junfeng Fang,Xiang Wang*

Main category: cs.CL

TL;DR: SimCT提出了一种跨分词器的在线蒸馏方法，通过引入多token连续片段作为监督单元，解决了异构分词器下共享token匹配丢失大量教师信号的问题，在数学推理和代码生成任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 传统在线蒸馏假设师生模型使用相同分词器，当两者分词器异构时，逐token精确匹配会丢弃大量教师信号（在词汇表不一致的位置），导致蒸馏效果下降。

Method: SimCT不改变蒸馏损失形式，而是扩大监督空间：在共享token之外，引入师生双方都能实现的多token连续片段（short multi-token continuations）作为比较单元，这些单元是"最细粒度的可联合分词监督接口"。

Result: 在数学推理和代码生成任务上，针对三组异构师生模型对，SimCT一致优于共享词汇表OPD和代表性跨分词器基线方法，消融实验证实增益来自恢复被精确共享token匹配丢弃的监督信号。

Conclusion: 跨分词器场景下，共享token匹配会丢失有用信号，而SimCT通过更细粒度的多token连续片段监督能够有效恢复这部分信号，且更粗糙的替代方案会移除对在线学习有用的师生差异信息。
...
### [181] [Memory-Efficient Looped Transformer: Decoupling Compute from Memory in Looped Language Models](https://arxiv.org/abs/2605.07721)
*Victor Conchello Vendrell,Arnau Padres Masdemont,Niccolò Grillo,Jordi Ros-Giralt,Arash Behboodi,Fabio Valerio Massoli*

Main category: cs.CL

TL;DR: 提出MELT架构，通过共享KV缓存和可学习门控机制，将循环推理的内存消耗从线性增长降为常数，在保持性能的同时大幅降低内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有循环LLM架构（如Ouro）虽能提升推理能力，但每次迭代都保留独立KV缓存，导致内存随推理深度线性增长，限制了实际可扩展性。

Method: MELT架构：每层仅维护一个跨循环共享的KV缓存，通过可学习门控机制更新；训练采用两阶段分块训练：插值过渡+注意力对齐蒸馏。

Result: 从Ouro微调的MELT模型在性能上优于同规模标准LLM，内存占用与标准LLM相当，远小于Ouro。

Conclusion: MELT实现了常数内存的迭代推理，在不损失循环LLM性能的前提下，仅需轻量后训练即可达到实用效果。
...
### [182] [SOD: Step-wise On-policy Distillation for Small Language Model Agents](https://arxiv.org/abs/2605.07725)
*Qiyong Zhong,Mao Zheng,Mingyang Song,Xin Lin,Jie Sun,Houcheng Jiang,Xiang Wang,Junfeng Fang*

Main category: cs.CL

TL;DR: 本文提出SOD（逐步在线蒸馏）框架，通过自适应重新加权蒸馏强度解决小语言模型在工具集成推理中因错误级联导致的师生分歧问题，在数学、科学和代码基准上最高提升20.86%，0.6B模型在AIME 2025上达到26.13%。


<details>
  <summary>Details</summary>
Motivation: 现有在线蒸馏（OPD）应用于工具集成推理（TIR）时，错误工具调用会在后续推理步骤中级联放大，导致师生模型分歧加剧，使教师的token级监督变得不可靠。需要一种能适应逐步分歧的蒸馏方法。

Method: 提出SOD（Step-wise On-policy Distillation）框架，在每一步根据步骤级分歧自适应地重新加权蒸馏强度：在高分歧区域减弱可能误导的教师信号，在良好对齐状态保持密集指导。

Result: 在数学、科学和代码基准测试中，SOD相比第二好的基线提升最高20.86%；0.6B小模型在AIME 2025上达到26.13%，验证了将智能体推理有效迁移至轻量级模型。

Conclusion: SOD有效解决了小语言模型在工具集成推理中的错误级联问题，实现了将复杂智能体推理能力高效迁移到轻量级模型，代码已开源。
...
### [183] [TextLDM: Language Modeling with Continuous Latent Diffusion](https://arxiv.org/abs/2605.07748)
...
TL;DR: 本文提出TextLDM，将视觉扩散Transformer（DiT）架构迁移到文本生成领域，通过VAE将离散文本映射为连续潜变量，并结合表征对齐技术，在语言建模上取得了与GPT-2相当的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer在视觉生成中已展现出统一图像和视频生成的能力，作者希望将这一框架扩展到语言建模，朝着单一架构同时支持生成（视觉合成）和理解（文本生成）的目标迈进。

Method: 使用Transformer-based VAE将离散文本token映射为连续潜变量，通过表征对齐（REPA）与冻结的预训练语言模型对齐以提升表征质量，然后在潜空间中使用标准DiT进行流匹配（flow matching）训练。

Result: 在OpenWebText2上从零训练，TextLDM显著优于先前的扩散语言模型，在相同设置下与GPT-2性能持平。

Conclusion: 视觉DiT方案可以有效迁移到语言领域，为扩散架构在多模态生成与理解上的统一迈出了具体一步。
...
### [184] [Chain-based Distillation for Effective Initialization of Variable-Sized Small Language Models](https://arxiv.org/abs/2605.07783)
*Boyu Shi,YiCheng Jiang,Chang Liu,Qiufeng Wang,Xu Yang,Xin Geng*

Main category: cs.CL

TL;DR: 提出链式蒸馏（CBD）方法，通过构建渐进式蒸馏链和参数插值，高效初始化不同规模的语言模型，避免重复调用大模型，提升资源受限场景下的部署效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署成本高，小模型从头训练计算昂贵，传统知识蒸馏需反复访问大模型且扩展性差。

Method: 构建稀疏的渐进式蒸馏链，通过逐步蒸馏生成中间锚点模型；引入桥接蒸馏支持跨架构和跨词表迁移；通过相邻锚点参数插值初始化任意规模模型。

Result: 138M参数的SLM无需恢复预训练，在10B token语料上优于从头训练的模型；在跨架构和跨词表的异构设置中表现出通用性。

Conclusion: CBD方法显著提升了效率和下游性能，为可变规模语言模型的初始化提供了一种可扩展的蒸馏范式。
...
### [185] [PolySQL: Scaling Text-to-SQL Evaluation Across SQL Dialects via Automated Backend Isomorphism](https://arxiv.org/abs/2605.07796)
*Yotam Perlitz,Elad Venezian,Corentin Royer,Francesco Fusco,Andrea Giovannini*

Main category: cs.CL

TL;DR: 本文提出PolySQL方法，通过比较归一化执行结果实现跨SQL方言评估，无需查询转译，并揭示了SQLite到其他方言平均10.1%的准确率下降，且错误主要由逻辑问题（61%）而非语法问题（8%）导致。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL基准主要支持SQLite，但跨方言评估显示SQLite性能不能可靠代表其他方言，而现有转译方法昂贵或易失败，存在评估鸿沟。

Method: 提出PolySQL双执行方法，通过比较归一化执行结果来评估跨方言一致性，避免手动查询转译，实现100%查询覆盖率。

Result: 平均准确率从SQLite到其他方言下降10.1%；错误中逻辑错误占61%，语法错误仅占8%；揭示了显著的方言难度层级。

Conclusion: PolySQL实现了比查询转译更高的评估保真度，并提供了首个大规模跨方言研究的数据集和基准，推动方言鲁棒性评估。
...
### [186] [Beyond Confidence: Rethinking Self-Assessments for Performance Prediction in LLMs](https://arxiv.org/abs/2605.07806)
*Sree Bhattacharyya,Samarth Khanna,Leona Chen,Lucas Craig,Tharun Dilliraj,James Z. Wang*

Main category: cs.CL

TL;DR: 本文提出基于认知评价理论的多维自我评估框架，在12个LLM和38个任务中发现"努力"和"能力"等胜任力维度在预测模型失败方面优于传统置信度，且不同任务类型下最有效的评估维度不同。


<details>
  <summary>Details</summary>
Motivation: 传统基于置信度（verbalized confidence）的模型自我评估存在不一致和过度乐观的问题，需要更可靠的自我评估方法来提升LLM部署的安全性和可靠性。

Method: 借鉴人类心理学中的认知评价理论（cognitive appraisal theory），将自我评估分解为多个维度（包括努力、能力等胜任力维度以及情感维度），与置信度一起在12个LLM、38个任务（涵盖8个领域）上进行对比评估。

Result: 1）胜任力维度（尤其是努力和能力）在大部分场景下匹配或优于置信度；2）努力维度产生的估计更为稳定且不过度乐观；3）情感维度仅提供边际预测信号；4）最有效的评估维度因任务类型而异：推理密集型任务中努力最佳，检索型任务中能力和置信度更优。

Conclusion: 结构化多维自我评估是提升语言模型在多样化真实场景中部署可靠性和安全性的有前景的方法。
...
### [187] [A Comparative Analysis of Classical Machine Learning and Deep Learning Approaches for Sentiment Classification on IMDb Movie Reviews](https://arxiv.org/abs/2605.07811)
*Erma Daniar Safitri,Lia Hana Ichisasmita,Citra Agustin,Luluk Muthoharoh,Ardika Satria,Martin Clinton Tosima Manullang*

Main category: cs.CL

TL;DR: 本文比较了经典机器学习（TF-IDF + PyCaret AutoML：逻辑回归、朴素贝叶斯、SVM）与深度学习方法（BiLSTM、BiLSTM+注意力）在IMDb电影评论情感分类上的性能。实验表明，经典机器学习（尤其SVM）以85.30%准确率最佳，而带注意力的BiLSTM（70.6%）优于普通BiLSTM。结论：结合有效特征工程（如TF-IDF）的经典ML在数据与算力有限时仍是强基线。


<details>
  <summary>Details</summary>
Motivation: 比较经典ML与深度学习在情感分类任务上的实际效果差异，验证在有限资源和数据场景下哪种方法更具优势。

Method: 经典ML：TF-IDF特征提取 + PyCaret AutoML评估逻辑回归、朴素贝叶斯、SVM；深度学习：BiLSTM及BiLSTM+注意力机制。

Result: SVM最优（准确率85.30%）；BiLSTM+注意力（70.6%）优于普通BiLSTM；深度学习模型均未超越经典ML。

Conclusion: 经典机器学习结合TF-IDF在数据量和计算资源有限时依然是强有力的基线方案，深度学习虽能建模序列依赖但未必能在小规模任务上胜出。
...
### [188] [SCENE: Recognizing Social Norms and Sanctioning in Group Chats](https://arxiv.org/abs/2605.07823)
*Mateusz Jacniacki,Maksymilian Bilski*

Main category: cs.CL

TL;DR: 提出SCENE基准测试，评估LLM在多人在线聊天中识别和适应隐含社交规范的能力，发现Claude Opus 4.7和Gemini 3.1 Pro比开源模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 在线群聊中存在隐含行为规范，LLM能否识别和适应这些规范尚不清楚，缺乏相关评估基准。

Method: 构建SCENE基准测试：生成非角色扮演场景，包含脚本化人格、隐藏规范、违规机会和社交制裁机制，并设计行为评估指标（对负面制裁的响应、从同伴行为学习规范）。

Result: Claude Opus 4.7和Gemini 3.1 Pro在适应隐含规范方面显著优于评估的开源模型。

Conclusion: SCENE为评估LLM社交能力的动态交互评估提供了基准，响应了该研究方向的最新呼吁。
...
### [189] [Measuring and Mitigating the Distributional Gap Between Real and Simulated User Behaviors](https://arxiv.org/abs/2605.07847)
*Shuhaib Mehri,Philippe Laban,Sumuk Shashidhar,Marwa Abdulhai,Sergey Levine,Michel Galley,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: 本文提出了一种测量真实用户与模拟用户行为分布差异的方法，并通过24种LLM模拟器的系统评估发现，当前模拟器与真实用户之间仍存在显著分布差距。


<details>
  <summary>Details</summary>
Motivation: 现有的用户模拟器虽然能生成类人回复，但能否捕捉真实用户行为的广泛分布尚不清楚，需要定量评估模拟行为与真实行为之间的分布差距。

Method: 从真实和模拟对话中提取用户行为表征，通过聚类将其量化为离散分布，然后计算散度指标来量化分布差距。

Result: 对24种LLM模拟器在编程和写作任务上的评估显示，模拟器与真实用户存在较大分布差距，且不同模型族、规模和行为维度差异显著；大多数模拟器行为相似，少数表现不同；组合行为互补的模拟器能缩小分布差距。

Conclusion: 当前LLM模拟器未能充分覆盖真实用户行为的多样性，通过行为互补组合可部分缓解该问题，TF-IDF分析揭示了模拟器捕捉、遗漏和幻觉的行为模式。
...
### [190] [MatryoshkaLoRA: Learning Accurate Hierarchical Low-Rank Representations for LLM Fine-Tuning](https://arxiv.org/abs/2605.07850)
*Ionut-Vlad Modoranu,Mher Safaryan,Dan Alistarh*

Main category: cs.CL

TL;DR: MatryoshkaLoRA通过在对LoRA适配器之间插入精心设计的对角矩阵P，实现了一种通用的分层低秩表示训练框架，支持动态秩选择，在保持精度的同时显著优于现有秩自适应方法。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA需要预设静态秩r，需通过穷举网格搜索来平衡效率与性能；而DyLoRA等秩自适应方法由于缺乏跨完整秩层级的一致梯度信号，在高秩时表现次优，数据效率低。

Method: 提出MatryoshkaLoRA框架，在LoRA适配器之间插入一个精心设计的固定对角矩阵P来缩放子秩；通过简单改变P即可恢复LoRA和DyLoRA；并提出AURAC指标用于评估分层低秩适配器的性能。

Result: MatryoshkaLoRA学习了比先前秩自适应方法更准确的分层低秩表示，在多个数据集上实现了跨秩的更好精度-性能权衡。

Conclusion: MatryoshkaLoRA是一种通用的分层低秩训练框架，能够有效支持动态秩选择且精度损失最小，显著优于已有秩自适应方法。
...
### [191] [Beyond "I cannot fulfill this request": Alleviating Rigid Rejection in LLMs via Label Enhancement](https://arxiv.org/abs/2605.07883)
*Ying Zhang,Congyu Qiao,Xin Geng,Ning Xu*

Main category: cs.CL

TL;DR: LANCE通过标签增强技术（变分推断预测连续拒绝分布）来缓解大语言模型的刚性拒绝问题，在不降低安全性的前提下提升回复的自然性和有用性。


<details>
  <summary>Details</summary>
Motivation: 传统LLM的安全对齐依赖刚性拒绝模板（如"无法满足该请求"），严重破坏了人机交互的自然性，需要一种既能保证安全又能灵活自然回应的机制。

Method: 采用变分推断进行标签增强，预测跨多个拒绝类别的连续分布；利用细粒度拒绝分布提供多向文本梯度，通过精炼模型中和提示中的有害元素，使LLM生成安全且非刚性拒绝的回复。

Result: 实验表明LANCE显著缓解了刚性拒绝问题，同时保持高安全标准，在回复的有用性和自然性上显著优于现有基线模型。

Conclusion: LANCE通过标签增强实现了安全与自然性的平衡，为人机交互中更灵活的拒绝机制提供了有效方案。
...
### [192] [CoCoReviewBench: A Completeness- and Correctness-Oriented Benchmark for AI Reviewers](https://arxiv.org/abs/2605.07905)
*Hexuan Deng,Xiaopeng Ke,Yichen Li,Ruina Hu,Dehao Huang,Derek F. Wong,Yue Wang,Xuebo Liu,Min Zhang*

Main category: cs.CL

TL;DR: 针对AI评审系统缺乏可靠评估指标的问题，本文通过构建类别特定基准子集、利用审稿-作者-元审稿讨论过滤不可靠评审，提出了CoCoReviewBench基准（含3900篇ICLR/NeurIPS论文），实现细粒度评估。分析表明AI评审在正确性上仍有限，易产生幻觉，而推理模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有AI评审评估指标偏向与人类评审的重叠度而非正确性，但人类评审本身存在覆盖不全和错误的问题，缺乏可靠的黄金标准参考。

Method: 1) 构建类别特定基准子集，在缺少对应人类评审时跳过评估以增强完整性（Completeness）；2) 利用审稿人-作者-元审稿的讨论作为专家标注，过滤不可靠评审以增强正确性（Correctness）；3) 提出CoCoReviewBench基准，包含3900篇ICLR/NeurIPS论文。

Result: AI评审在正确性方面仍有限，容易产生幻觉；推理模型作为评审表现更优。

Conclusion: 为AI评审系统的评估提供了更可靠、细粒度的基准，指出了未来改进AI评审的方向（尤其是推理模型的应用）。
...
### [193] [How Value Induction Reshapes LLM Behaviour](https://arxiv.org/abs/2605.07925)
*Arnav Arora,Natalie Schluter,Katherine Metcalf,Maartje ter Hoeve*

Main category: cs.CL

TL;DR: 本文研究了在对话大语言模型中诱导特定价值观（如好奇心、共情等）所产生的意外效应，包括价值观之间的相互影响、安全性变化、以及拟人化语言和谄媚行为的增加。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在后期训练中会被注入特定的行为特质和价值观以提升实用性和安全性，但价值观之间存在复杂关联，诱导某一价值观可能意外影响其他价值观表现，甚至导致模型变得更易成瘾或谄媚，因此需要系统研究这些非预期效应。

Method: 研究者利用现有偏好数据集的子集对模型进行微调，针对不同价值观进行诱导，然后测量这些诱导对以下方面的影响：其他价值观的表达、模型安全性、拟人化语言使用，以及多项问答基准测试的表现。

Result: 研究发现：（i）诱导某一价值观会导致其他相关甚至对立价值观的表达发生变化；（ii）诱导积极价值观能够提升模型的安全性；（iii）所有价值观的诱导都会增加拟人化语言的使用，使模型变得更具验证性和谄媚性。

Conclusion: 价值观诱导存在复杂的连锁效应，虽然能提升安全性和某些正面特质，但也会带来拟人化和谄媚行为等意外负面后果，需要在模型对齐过程中加以关注和平衡。
...
### [194] [How to Train Your Latent Diffusion Language Model Jointly With the Latent Space](https://arxiv.org/abs/2605.07933)
*Viacheslav Meshchaninov,Alexander Shabalin,Egor Chimbulatov,Nikita Gushchin,Ilya Koziev,Alexander Korotin,Dmitry Vetrov*

Main category: cs.CL

TL;DR: 本文提出LDLM，通过联合训练潜编码器、扩散模型和解码器，构建适于文本生成的潜空间，在OpenWebText和LM1B上取得优于现有离散/连续扩散语言模型的性能，同时速度快2-13倍。


<details>
  <summary>Details</summary>
Motivation: 潜扩散模型在文本生成中面临构建合适潜空间的主要挑战，现有方法缺乏对潜空间的联合优化。

Method: 联合训练潜编码器、扩散模型和解码器；通过重塑预训练语言模型表示构建潜空间；提出MSE解码器损失、扩散到编码器热身、自适应时间步采样、解码器输入噪声等训练策略。

Result: 在OpenWebText和LM1B数据集上，LDLM超越了现有的离散和连续扩散语言模型，同时推理速度快2-13倍。

Conclusion: 联合学习潜空间是使潜扩散模型在文本生成中具备竞争力的关键步骤。
...
### [195] [Ask Early, Ask Late, Ask Right: When Does Clarification Timing Matter for Long-Horizon Agents?](https://arxiv.org/abs/2605.07937)
*Anmol Gulati,Hariom Gupta,Elias Lumer,Sahil Sen,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: 本论文研究了长时域AI智能体在执行复杂任务时，何时寻求澄清信息效果最佳。通过强制注入框架，发现不同信息类型的澄清价值随时间衰减曲线各异：目标澄清在执行10%后几乎失去价值，输入澄清可持续到50%。跨模型相关性证实这些时序特征与任务本身高度相关。同时发现当前前沿模型均未在最优时机提问。


<details>
  <summary>Details</summary>
Motivation: 长时域AI智能体执行复杂任务时，单个早期错误假设会导致不可逆级联错误。当指令不完整时，智能体不仅需要决定是否请求澄清，还需要决定何时请求。现有研究未衡量澄清价值在执行过程中的变化，缺乏对澄清时机的定量分析。

Method: 提出强制注入框架，在智能体轨迹的受控时间点提供真实澄清信息。涵盖四个信息维度（目标、输入、约束、上下文），三个智能体基准测试，四个前沿模型，84个任务变体，6000+次运行。同时进行300次无脚本会话的补充研究。

Result: 目标澄清在执行10%后价值几乎归零（pass@3从0.78降至基线），输入澄清可持续到约50%。推迟任何澄清类型过半程会使性能低于从不提问。跨模型Kendall tau相关系数（模型间0.78-0.87，全面板0.34-0.67）确认时序特征为任务内在属性。当前模型均未在最优时机提问，策略从过度提问（52%）到从不提问不等。

Conclusion: 这些经验需求曲线为现有理论框架提供了缺失的量化基础，并为时序感知的澄清策略建立了具体设计目标。
...
### [196] [GLiGuard: Schema-Conditioned Classification for LLM Safeguard](https://arxiv.org/abs/2605.07982)
*Urchade Zaratiana,Mary Newhauser,George Hurn-Maloney,Ash Lewis*

Main category: cs.CL

TL;DR: GLiGuard 是一个仅有 0.3B 参数的轻量级双向编码器模型，通过模式条件化输入设计，能够在单次非自回归前向传播中同时评估提示安全、响应安全、拒绝检测、14 种细粒度危害类别和 11 种越狱策略，在 9 个安全基准上取得与 7B-27B 解码器模型相当的 F1 分数，同时实现高达 16 倍吞吐量提升和 17 倍延迟降低。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型内容审核方案依赖 7B-27B 参数的自回归解码器，将分类问题建模为序列生成，导致高延迟且难以扩展到多维度评估，亟需更高效的解决方案。

Method: 基于 GLiNER2 改造的 0.3B 参数模式条件化双向编码器，将任务定义和标签语义编码为结构化 token 模式输入序列，支持在单次非自回归前向传播中同时完成多项安全评估，且推理时可灵活组合支持的任务和标签块。

Result: 在 9 个安全基准上达到与 7B-27B 解码器模型相当的 F1 分数，模型体积小 23-90 倍，吞吐量提升高达 16 倍，延迟降低高达 17 倍。

Conclusion: 紧凑型双向编码器能够在极大降低推理成本的同时，接近更大规模模型的安全审核准确率，为高效内容审核提供了新范式。
...
### [197] [Fast Byte Latent Transformer](https://arxiv.org/abs/2605.08044)
...
TL;DR: 本文提出Byte Latent Transformer (BLT)的三种变体（BLT-D、BLT-S、BLT-DV），通过并行生成、推测解码等技术大幅提升字节级语言模型的生成速度，同时将内存带宽成本降低50%以上。


<details>
  <summary>Details</summary>
Motivation: 字节级语言模型虽无需子词词汇表且性能匹敌token级模型，但逐字节自回归生成速度极慢，严重制约其实用性。

Method: 1) BLT-D：引入辅助的块级扩散目标训练，实现每步并行生成多个字节；2) BLT-S：利用局部解码器超出正常边界继续生成草稿字节，再通过完整模型前向验证；3) BLT-DV：在扩散生成后添加自回归验证步骤。

Result: 所有方法在生成任务上内存带宽成本降低超过50%，显著减少生成序列所需的前向传播次数。
...
### [198] [Uncertainty-Aware Structured Data Extraction from Full CMR Reports via Distilled LLMs](https://arxiv.org/abs/2605.08045)
*Yi Yu,Parker Martin,Zhenyu Bu,Yixuan Liu,Yi-Yu Zheng,Orlando Simonetti,Yuchi Han,Yuan Xue*

Main category: cs.CL

TL;DR: 本文提出CMR-EXTR，一个轻量级框架，用于将自由文本的心脏磁共振报告转换为结构化数据，并通过教师-学生蒸馏管道实现离线推理，同时利用三重不确定性原理（分布合理性、采样稳定性、跨字段一致性）为每个字段分配置信度以辅助质控。实验达到99.65%的变量级准确率，是首个集成置信度估计的CMR专用提取系统。


<details>
  <summary>Details</summary>
Motivation: 将自由文本CMR报告转化为可审计的结构化数据是队列构建、纵向数据管理和临床决策支持的主要瓶颈，现有方法缺乏针对CMR的专用提取系统及置信度评估机制。

Method: 采用教师-学生蒸馏管道实现完全离线推理并减少人工标注；不确定性估计融合三种互补原则：分布合理性、采样稳定性、跨字段一致性，用于对需人工审核的结果进行分流。

Result: 变量级准确率达到99.65%，证明了可靠的提取能力和信息丰富的置信度分数。

Conclusion: CMR-EXTR是首个针对CMR报告的结构化提取系统，集成了置信度估计，有效解决了自由文本到结构化数据的转换瓶颈，并便于质控。
...
### [199] [Accurate and Efficient Statistical Testing for Word Semantic Breadth](https://arxiv.org/abs/2605.08048)
*Yo Ehara*

Main category: cs.CL

TL;DR: 本文提出了一种基于Householder对齐的置换检验方法，用于在比较两个词类型的语义广度时，消除方向差异对离散度统计量的干扰，从而降低假阳性错误率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在用离散度统计量比较两个词类型的语义广度时，方向差异会伪装成离散度差异，导致假阳性（Type-I error）膨胀，使得统计显著性检验失效。

Method: 提出Householder对齐置换检验：先通过Householder反射将两个词类型的均值方向对齐，再对对齐后的token向量云进行置换检验，得到校准的非参数p值；并设计了GPU批处理实现以加速计算。

Result: 对齐后Type-I error降低了32.5%，同时保持了对真实广度差异的敏感性；GPU实现相比CPU基线取得了23倍加速。

Conclusion: 该方法能有效区分真实离散度差异与方向差异导致的假阳性，为语义广度比较提供了更可靠的统计检验工具。
...
### [200] [CA-SQL: Complexity-Aware Inference Time Reasoning for Text-to-SQL via Exploration and Compute Budget Allocation](https://arxiv.org/abs/2605.08057)
*James Petullo,Nianwen Xue*

Main category: cs.CL

TL;DR: CA-SQL是一个利用任务难度动态调整搜索广度、结合进化搜索提示种子方法和新型投票机制的Text-to-SQL流水线，在BIRD基准测试的"挑战性"级别上以GPT-4o-mini达到51.72%的SOTA准确率。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法在BIRD基准的最困难任务上表现不佳，原因在于解空间探索不充分，无法发现可进一步优化为正确输出的候选查询。

Method: 提出CA-SQL流水线：(1) 利用任务预估难度动态缩放候选解生成的搜索广度；(2) 基于进化搜索原理的定制提示种子方法增强LLM的探索行为；(3) 新型投票方法在搜索结束时选择最佳候选解。

Result: 在BIRD开发集"挑战性"级别上达到51.72%的SOTA准确率（使用GPT-4o-mini），超越其他上下文学习方法（包括使用更大模型的方法）。整体执行准确率61.06%，Soft F1分数68.77%。

Conclusion: CA-SQL通过动态搜索广度扩展和进化式提示种子方法，有效提升了LLM在复杂Text-to-SQL任务上的表现，证明了即使使用较小模型也能通过更好的探索策略达到领先性能。
...
### [201] [The Memory Curse: How Expanded Recall Erodes Cooperative Intent in LLM Agents](https://arxiv.org/abs/2605.08060)
...
TL;DR: 扩展LLM上下文窗口在多智能体社会困境中反而会破坏合作行为，这种现象被称为"记忆诅咒"。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为上下文窗口扩展是LLM能力的简单升级，但其在多智能体社会困境中的影响尚未被系统研究。

Method: 通过7个LLM在4种博弈游戏中超过500轮实验，结合37.8万条推理轨迹的词汇分析、记忆消毒实验、LoRA微调认知探测以及消融Chain-of-Thought推理等多种方法进行机制研究。

Result: 在28个模型-游戏设置中，18个出现合作行为退化；记忆诅咒的触发机制是记忆内容而非长度本身；前向意图的侵蚀而非偏执增加是主因；显式CoT推理反而会放大这种诅咒。

Conclusion: 记忆是多智能体行为的主动决定因素：更长的记忆会引发不同的推理模式，进而决定合作行为是稳定还是被破坏。
...
### [202] [LLMs Improving LLMs: Agentic Discovery for Test-Time Scaling](https://arxiv.org/abs/2605.08083)
...
TL;DR: 提出AutoTTS框架，通过构建环境自动发现测试时计算分配策略，替代人工设计，在数学推理任务上以低成本（39.9美元/160分钟）超越人工基线。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放(TTS)策略依赖人工设计，限制了计算分配空间的探索效率。

Method: 构建自动发现TTS策略的环境，将宽度-深度TTS形式化为控制器综合问题，采用beta参数化使搜索易处理，并通过细粒度执行轨迹反馈提升发现效率。

Result: 数学推理基准上，自动发现的策略在准确率-成本权衡上超越强人工基线；策略泛化到未见基准和模型规模；总发现成本仅39.9美元和160分钟。

Conclusion: AutoTTS证明了从人工设计TTS转向自动发现环境的可行性，以极低成本获得更优策略，代码和数据将开源。
...
### [203] [Information-theoretic Limits of Learning and Estimation](https://arxiv.org/abs/2605.06710)
*Abbas El Gamal,Maxim Raginsky*

Main category: cs.IT

TL;DR: 本文介绍了信息论在机器学习理论中的核心作用，涵盖了浓度不等式、度量熵、Rademacher复杂度、VC维等泛化上界工具，以及基于Fano不等式的最小最大风险下界方法。


<details>
  <summary>Details</summary>
Motivation: 信息论为学习与估计算法提供理论极限，无论计算能力如何，理解这些基本边界对于机器学习理论至关重要。

Method: 首先引入浓度不等式及度量空间中的覆盖与打包概念（度量熵）；然后建立学习理论框架，推导基于度量熵、Rademacher复杂度、VC维、互信息和相对熵的泛化误差上界；最后通过Fano不等式建立最小最大风险下界。

Result: 给出了泛化误差的多种上界形式以及最小最大风险的Fano下界，建立了信息论与学习理论之间的桥梁。

Conclusion: 该章作为Cover & Thomas《信息论基础》第三版的一部分，系统地整合了信息论与统计学习理论的核心工具与结果。
...
### [204] [Near-field Channel Estimation for XL-RIS-aided mmWave MIMO Systems](https://arxiv.org/abs/2605.06719)
*Erkang Dong,Taihao Zhang,Cunhua Pan,Hong Ren,Jiangzhou Wang*

Main category: cs.IT

TL;DR: 针对XL-RIS辅助的毫米波MU-MIMO系统，提出了一种联合利用远场BS-RIS公共链路和近场RIS-用户信道极域稀疏性的两阶段级联信道估计方案，显著降低了导频开销。


<details>
  <summary>Details</summary>
Motivation: XL-RIS的巨大孔径导致RIS-用户链路处于近场区域，传统平面波假设和角域稀疏表示失效，使得信道估计极具挑战性。

Method: 提出两阶段信道估计方案：1) 将多天线用户分解为多个虚拟单天线用户，从典型虚拟用户提取公共BS-RIS参数，并通过补偿极域稀疏恢复初始化RIS-用户信道；2) 采用交替最小二乘精化过程联合优化公共BS-RIS算子和用户特定RIS侧信道。

Result: 仿真结果表明，与现有近场基准方案相比，所提方案在显著降低导频开销的同时实现了有竞争力的信道估计性能。

Conclusion: 本文提出的混合场信道估计方案有效解决了XL-RIS辅助系统中的近场信道估计难题，为低成本高效信道估计提供了新思路。
...
### [205] [Cryptographic and Information-theoretic Security Capacities for General Arbitrarily Varying Wiretap Channels](https://arxiv.org/abs/2605.06751)
*Holger Boche,Ning Cai,Yiqi Chen,Marc Geitz*

Main category: cs.IT

TL;DR: 本文研究了任意变化窃听信道（AVWC）和广义任意变化窃听信道（GAVWC）在强 secrecy 容量与语义 secrecy 容量之间的关系，发现对 AVWC 两者相等，但对 GAVWC 存在差距，并给出了差距消失的充分条件。


<details>
  <summary>Details</summary>
Motivation: 比较 AVWC 和 GAVWC 在强 secrecy 约束与语义 secrecy（及其他密码学等价 secrecy）约束下的容量关系，澄清不同安全度量下的信道容量是否等价。

Method: 通过理论分析证明 AVWC 的平均错误与强 secrecy 容量等于其最大错误与语义 secrecy 容量；构造反例证明该等价性不适用于所有一般通信系统；对 GAVWC 分析语义安全与其他密码学安全度量下的容量相等性；并推导强 secrecy 容量与语义 secrecy 容量之间的差距边界。

Result: AVWC 的强 secrecy 容量与语义 secrecy 容量相等；但该等价性不普遍成立（有反例）；GAVWC 下语义安全与其他密码学安全度量容量相同；强 secrecy 与语义 secrecy 容量差距在干扰者选择为亚双指数级时消失。

Conclusion: AVWC 下强 secrecy 与语义 secrecy 容量等价，而 GAVWC 下两者存在差距，该差距在干扰策略足够慢时消失，为两类安全度量在更广信道模型下的等价性提供了充分条件。
...
### [206] [A Framework of Variable-Length Source Encryption using Mutual Information Security Criterion: Universal Coding, Strong Converse Theorem](https://arxiv.org/abs/2605.06802)
*Yasutada Oohama,Bagus Santoso*

Main category: cs.IT

TL;DR: 本文提出了一个源加密框架，基于Shannon密码系统的安全通信模型，利用互信息度量信息泄露，给出了可靠且安全通信的充要条件，并证明了强逆定理，同时构造了适用于任意明文和密钥分布的通用加解密方案。


<details>
  <summary>Details</summary>
Motivation: 旨在建立一种基于固定长度源编码的加密框架，并严格刻画在给定错误概率和信息泄露上限条件下可靠且安全通信的理论极限。

Method: 将Shannon密码系统的安全通信框架应用于固定长度源编码，使用互信息度量信息泄露，推导可靠性与安全性的充要条件，并构造通用加解密方案。

Result: 建立了可靠且安全通信的充要条件，该条件与给定的误差常数ε和δ无关，证明了强逆定理；同时证明了存在普适性的加解密方案。

Conclusion: 源加密框架在理论上具有稳定的安全性能界限，且所提出的方案对任意明文和密钥分布均具有通用性。
...
### [207] [Hybrid Multiport Receivers for Slow Fluid Antenna Multiple Access](https://arxiv.org/abs/2605.06958)
*José P. González-Coma,José David Vega-Sánchez,F. Javier López-Martínez*

Main category: cs.IT

TL;DR: 提出了一种流體天線混合多端口（FAHM）接收機架構，僅需少量射頻鏈即可實現多端口選擇的性能優勢，在流體天線多址（FAMA）場景下，2條射頻鏈即可媲美全數字方案的性能，並降低60%以上計算負擔。


<details>
  <summary>Details</summary>
Motivation: 傳統流體天線系統中，多端口選擇雖性能優異但需要大量射頻鏈，導致硬體成本與功耗過高。本文旨在用少量射頻鏈保留多端口選擇的性能收益。

Method: 提出FAHM接收機架構，將端口選擇與信號合併解耦，集成低複雜度模擬合併網絡；開發用於確定選取端口數量的停止準則；針對給定射頻鏈預算設計混合合併器。

Result: 在慢速FAMA多用戶場景中，僅2條射頻鏈的FAHM實現與大量射頻鏈的全數字多端口方案性能相當；與最新廣義特徵向量端口選擇方法結合，計算負擔降低超過60%。

Conclusion: FAHM架構能有效平衡性能與硬體複雜度，實現以極少射頻鏈獲得近似全數字多端口選擇的性能，並顯著降低計算開銷。
...
### [208] [Sub-Gaussian Concentration and Entropic Normality of the Maximum Likelihood Estimator](https://arxiv.org/abs/2605.07107)
*Leighton P. Barnes,Alex Dytso*

Main category: cs.IT

TL;DR: 本文在经典MLE渐近正态性的基础上，通过额外假设，证明了更强化形式：次高斯尾界、所有矩收敛、平滑版本的熵中心极限定理，以及在特定条件下无平滑的熵正态性。


<details>
  <summary>Details</summary>
Motivation: 经典MLE仅满足分布收敛到高斯分布，本文旨在建立更强化形式的渐近正态性，包括尾界、矩收敛和相对熵收敛。

Method: 在得分函数上添加额外假设，利用指数一致性界、高阶矩估计和熵控制工具进行证明。

Result: 建立了归一化MLE的次高斯尾界和所有矩收敛；证明了平滑版本的熵中心极限定理；在Fisher信息有界或密度一阶导数有界的条件下，去除了平滑，得到MLE自身的熵正态性。

Conclusion: 本文显著强化了经典MLE的渐近正态性结果，并发展了一系列可能具有独立价值的辅助分析工具。
...
### [209] [Movable Subarray-Aided Hybrid Beamforming for Near-Field Multiuser Communications](https://arxiv.org/abs/2605.07261)
*Xiangqian Xu,Songjie Yang,Arumugam Nallanathan*

Main category: cs.IT

TL;DR: 本文提出了一种可移动子阵列辅助的混合波束赋形架构，用于近场多用户大规模MIMO系统，通过联合优化子阵列位置和混合预编码，实现了比固定天线阵列更高的和速率。


<details>
  <summary>Details</summary>
Motivation: 现有可移动天线近场通信设计忽视混合波束赋形的实际约束，而可移动天线辅助的混合波束赋形又常忽略近场的丰富自由度。本文旨在平衡硬件实用性与空间灵活性。

Method: 提出可移动子阵列辅助的混合波束赋形架构，引入混合平面-球面波模型进行高效信道近似，并开发了基于分数规划、ADMM和投影梯度上升的交替优化算法。

Result: 仿真结果表明，所提方案在和速率上显著优于固定位置天线基准方案。

Conclusion: 通过耦合子阵列移动与混合波束赋形，同时利用近场距离依赖和子阵列位置依赖的自由度，实现了高精度波束聚焦和优越的干扰抑制。
...
### [210] [How Big Should a Wireless Foundation Model Be?](https://arxiv.org/abs/2605.07266)
*Wei-Lun Cheng,Wanjiun Liao*

Main category: cs.IT

TL;DR: 本文通过理论和实验证明，无线信道内在维度（dNL，约5-35）是物理层AI模型规模的基本瓶颈，参数量超过7000万后性能提升趋于停滞，而推理时测试时训练（TTT）能以更小模型实现显著更优性能。


<details>
  <summary>Details</summary>
Motivation: 现有无线基础模型设计缺乏理论指导，核心问题在于：这类模型到底应该多大？作者希望从物理原理出发回答该问题。

Method: 基于麦克斯韦方程物理约束，定义信道非线性流形维度(dNL)为基本瓶颈；通过真实OTA测量和3GPP信道模型验证dNL范围（5-35）；以NTN卫星信道为例（dNL≈14），建立模型规模缩放框架，并引入导频辅助的测试时训练（TTT）进行推理时自适应。

Result: 参数量超过7000万后性能进入随机渐近区，1.6倍参数增加仅带来0.52dB提升；而仅1200万参数的TTT模型在参数仅为1/8的情况下，比9600万静态模型高出9.9dB（NMSE）和7.6dB（MCM）。

Conclusion: 信道几何（内在维度）而非模型规模从根本上决定了物理层无线AI的缩放规律，无线AI设计应转向以物理约束为中心。
...
### [211] [UMVUE-Type Estimators under Bregman Losses](https://arxiv.org/abs/2605.07426)
*Akira Kamatsuka,Shun Watanabe*

Main category: cs.IT

TL;DR: 本文研究了Bregman损失下的无偏估计，将经典UMVUE理论扩展到两种不同的无偏性定义，并建立了相应的Rao-Blackwell和Lehmann-Scheffé定理。


<details>
  <summary>Details</summary>
Motivation: 经典UMVUE理论基于平方损失，作者希望将其推广到更一般的Bregman散度损失下，探索不同方向的无偏性定义及其理论性质。

Method: 利用Bregman散度的偏差-方差分解，考虑$D_\varphi(\theta,\hat\theta)$和$D_\varphi(\hat\theta,\theta)$两种损失函数及其对应的无偏性概念，建立对偶空间中的理论框架，并推导Rao-Blackwell和Lehmann-Scheffé定理的类比版本。

Result: 证明了$D_\varphi(\hat\theta,\theta)$可归约到经典情形，而$D_\varphi(\theta,\hat\theta)$导出了不同的框架，无偏性由$\nabla\varphi$诱导的对偶空间刻画。为后者建立了类型I Bregman UMVUE的系统构造方法。

Conclusion: 成功将经典UMVUE理论扩展到Bregman损失框架下，揭示了两种无偏性定义的本质差异，并为非平凡情形提供了系统性的最优估计构造方法。
...
### [212] [A Log-Domain Approximation of SOCS Decoding for Turbo Product Codes](https://arxiv.org/abs/2605.07519)
*Oleg Nesterenkov,Kirill Andreev,Alexey Frolov,Pavel Rybin*

Main category: cs.IT

TL;DR: 本文针对Turbo乘积码（TPC）提出了一种基于最大对数近似的低复杂度软输出译码算法，该算法将SOCS译码的概率域运算替换为可靠度间隙的分段线性函数，在保持与标准迭代译码环兼容的同时，显著优于传统Chase-Pyndiah译码器，性能接近SOCS译码器。


<details>
  <summary>Details</summary>
Motivation: 传统Chase-Pyndiah译码器外信息质量有限，而SOCS译码虽能改善外信息质量但其概率域实现不利于硬件实现，因此需要一种低复杂度、适合硬件实现的软输出译码方法。

Method: 基于最大对数（max-log）近似，将SOCS的概率域操作转化为竞争Chase-II译码列表内外假设之间可靠度间隙的分段线性函数，提出对数域的软输入软输出译码规则。

Result: 基于(256,239)eBCH分量码构建的TPC仿真结果表明，所提译码器在相同列表大小下明显优于基线Chase-Pyndiah译码器，性能接近SOCS译码器。

Conclusion: 所提对数域近似译码器在保持低复杂度和硬件友好性的同时，有效提升了译码性能，是实际硬件实现的良好候选方案。
...
### [213] [A Syndrome-Space Approach to Proximity Gaps and Correlated Agreement for Random Linear Codes](https://arxiv.org/abs/2605.07595)
*Chen Yuan,Ruiqi Zhu*

Main category: cs.IT

TL;DR: 本文提出了一种针对随机线性码的直接方法来建立邻近间隙和一致性协议，无需依赖列表译码，并在线性空间和多项式曲线上获得了最优参数结果。


<details>
  <summary>Details</summary>
Motivation: 在交互式预言机证明和基于编码的SNARKs中，邻近间隙和一致性协议是关键分析工具。虽然代数码已有较完善的结果，但随机线性码的类似结果此前只能通过译码相关的替代途径间接获得，缺乏直接的证明方法。

Method: 采用综合征空间的重新表述，结合基于见证的约简机制，直接面向随机奇偶校验矩阵模型中的随机线性码，不依赖列表译码作为主要证明手段。

Result: 对于仿射线、仿射空间和多项式曲线获得了强结果；在大字母表（q=Θ(n)）场景下实现了最优（至多ε）半径界ρ<1-R-ε；在常数字母表上获得了接近容量界的界限，并改善了字母表大小要求。

Conclusion: 该工作为随机线性码提供了不同于现有译码驱动路径的直接分析框架，在概念上具有创新性，并取得了更优的参数结果。
...
### [214] [Kolmogorov--Nagumo Mean Frameworks for Conditional Entropy](https://arxiv.org/abs/2605.07624)
*Akira Kamatsuka,Takahiro Yoshida*

Main category: cs.IT

TL;DR: 本文提出了基于Kolmogorov-Nagumo均值的条件熵框架，包括(η, ψ)-KN平均（EPKNAVG）和广义g-条件熵，证明了EPKNAVG在适当条件下等价于EAVG，而广义g-条件熵能捕捉EAVG无法表示的Augustin-Csiszár条件熵，并推导了其满足CRE和DPI的充分条件。


<details>
  <summary>Details</summary>
Motivation: 现有条件熵框架（如EAVG）在表示某些重要条件熵（如Augustin-Csiszár条件熵）时存在局限性，需要构建更一般的框架来统一和扩展条件熵的表达能力。

Method: 首先将η-平均（EAVG）扩展为(η, ψ)-KN平均（EPKNAVG），并研究其与EAVG的等价条件；其次基于广义g-脆弱性提出广义g-条件熵框架，分析其对Augustin-Csiszár条件熵的表示能力，并推导满足CRE和DPI的充分条件。

Result: EPKNAVG在适当凹化条件下等价于EAVG；广义g-条件熵能够表示EAVG无法表示的Augustin-Csiszár条件熵（α∈(0,1)∪(1,∞)）；给出了该框架满足CRE和DPI的充分条件。

Conclusion: 广义g-条件熵框架扩展了现有条件熵的表达范围，能够涵盖更多类型的安全度量，为信息论安全分析提供了更通用的工具。
...
### [215] [Semantic Smoothing for Language Models via Distribution Estimation and Embeddings](https://arxiv.org/abs/2605.07994)
*Haricharan Balasundaram,Swathi Shree Narashiman,Pranay Mathur,Andrew Thangaraj*

Main category: cs.IT

TL;DR: 提出语义平滑方法，利用嵌入向量共享语义相似上下文中的统计信息，在KL散度框架下进行分布估计，并通过插值估计器实现理论最优风险，实验表明该方法能持续降低测试困惑度。


<details>
  <summary>Details</summary>
Motivation: 传统平滑方法（如加常数平滑、Kneser-Ney）未能利用语义相似性来共享信息，导致数据稀疏问题处理不佳。本文旨在利用嵌入向量捕获的语义相似性，通过理论框架将平滑问题形式化为带KL邻近性约束的分布估计问题。

Method: 1）将log困惑度分解为KL散度下的分布估计问题；2）证明在Lipschitz-logit模型下，上下文嵌入的邻近性意味着对应下一词分布的KL散度邻近性；3）提出插值估计器，利用KL距离为Δ的侧信息，实现最坏情况KL风险O(min{Δ,d/n})；4）扩展到多个和经验估计的同义分布。

Result: 在合成马尔可夫数据和WikiText-103二元模型上，使用Word2Vec、GloVe和GPT-2嵌入，语义平滑在加常数平滑和Kneser-Ney估计上均持续降低了测试困惑度。

Conclusion: 语义平滑是一种有效的语言模型平滑方法，理论上达到了最优风险界，实践上能显著提升现有平滑方法的性能，验证了利用语义嵌入进行统计共享的有效性。
...
### [216] [Chase-like Decoding: Test Pattern Design and Performance Analysis](https://arxiv.org/abs/2605.08081)
*Tim Janz,Simon Obermüller,Andreas Zunker,Stephan ten Brink*

Main category: cs.IT

TL;DR: 本文研究了Chase类译码算法中测试图样集的性能评估与设计方法，提出了基于覆盖高概率错误图样思想的设计算法，对于高码率BCH码可获得高达0.2dB的性能增益。


<details>
  <summary>Details</summary>
Motivation: 现有的Chase类译码算法中，测试图样集的选择对译码性能有重要影响，但缺乏系统的评估和优化方法。本文旨在提出有效的性能评估方法并设计更优的测试图样集。

Method: 1) 利用顺序统计量评估具有特定结构（如Chase-II测试图样和最大逻辑权重图样）的测试图样集性能；2) 通过计算覆盖空间概率和直接蒙特卡洛仿真评估任意测试图样集的性能；3) 基于覆盖尽可能多高可能错误图样的思想，提出测试图样集设计算法。

Result: 所提出的测试图样集设计算法在高码率BCH码上比常用测试图样集性能提升高达0.2dB。

Conclusion: 本文提出的测试图样集设计方法可以有效提升Chase类译码算法的性能，尤其适用于高码率BCH码。
...
### [217] [GraphDC: A Divide-and-Conquer Multi-Agent System for Scalable Graph Algorithm Reasoning](https://arxiv.org/abs/2605.06671)
*Wenjin Li,Jiaming Cui*

Main category: cs.AI

TL;DR: 针对大语言模型在图算法推理任务中表现不佳的问题，提出了一个基于分治策略的多智能体框架GraphDC，通过将大图分解为子图并分配专门智能体进行局部推理，再通过主智能体整合信息，显著提升了图算法推理的可扩展性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在图算法任务上表现不佳，因为图结构在拓扑上更复杂，需要系统性的多步推理，尤其是在大规模图上。现有方法在面对大图时存在推理负担重、计算瓶颈和鲁棒性差的问题。

Method: GraphDC采用分治策略，具体包括：(1) 将输入图分解为较小的子图；(2) 将每个子图分配给专门的智能体进行局部推理；(3) 使用一个主智能体整合局部输出及子图间的交互信息，生成最终解决方案。这种层次化设计减轻了单个智能体的推理负担。

Result: 大量实验表明，GraphDC在各种图算法推理任务和规模上均一致优于现有方法，特别是在大规模图实例上，直接端到端推理不可靠时，GraphDC的优势更为明显。

Conclusion: 分治多智能体框架是一种有效的图算法推理方案，能够缓解计算瓶颈并提高鲁棒性，为LLMs处理复杂图结构问题提供了新的思路。
...
### [218] [More Thinking, More Bias: Length-Driven Position Bias in Reasoning Models](https://arxiv.org/abs/2605.06672)
*Xiao Wang*

Main category: cs.AI

TL;DR: 推理能力强的模型（如DeepSeek-R1）在多项选择问答中存在位置偏差，且偏差程度与推理轨迹长度正相关。长度越长，模型越倾向于选择位置偏好的选项。


<details>
  <summary>Details</summary>
Motivation: 挑战"CoT推理能减少启发式偏差"的常见假设，探究推理模型是否真的对选项位置顺序具有鲁棒性。

Method: 在13种推理配置（R1蒸馏模型、CoT提示基础模型、DeepSeek-R1 671B）上，使用PBS指标分析推理轨迹长度与位置偏差的相关性，并通过截断干预实验提供因果证据。

Result: 12种推理配置中轨迹长度与PBS呈正偏相关（0.11-0.41，p<0.05）；截断干预使模型向位置偏好选项偏移16%-32%；671B模型整体PBS仅0.019但最长四分位达0.071。

Conclusion: 推理模型并非默认对选项顺序鲁棒，CoT推理将基线位置偏差替换为"长度累积偏差"；建议在MCQ评估中应审计位置偏差，并提供相应的诊断工具包。
...
### [219] [Fast and Effective Redistricting Optimization via Composite-Move Tabu Search](https://arxiv.org/abs/2605.06682)
*Hai Jin,Diansheng Guo*

Main category: cs.AI

TL;DR: 本文提出了一种复合移动禁忌搜索算法（CM-Tabu），通过分析连通图中的关节点和双连通分量，在保持区域连通性的同时系统性地扩展可行邻域空间，显著提升了空间重划问题的求解质量和效率。


<details>
  <summary>Details</summary>
Motivation: 空间重划是一个实际组合优化问题，其核心难点在于连通性约束会严重缩小可行邻域空间，导致传统整数规划和启发式搜索陷入局部最优。现有方法难以同时满足高质量解、快速周转和多准则交互优化的需求。

Method: 提出CM-Tabu算法，当单个边界单元无法在不破坏连通性的情况下被重新分配时，通过分析每个区域的连通图，利用关节点和双连通分量在线性时间内识别可一起移动的最小单元集或可交换的单元对（或单元集），生成保持连通性的复合移动。

Result: 在费城案例中，该方法能够持续达到人口平等的理论全局最优解，并支持多准则权衡。与传统禁忌搜索及其他基线方法相比，CM-Tabu在解质量、运行间稳定性和计算效率方面均有显著提升。

Conclusion: CM-Tabu提供了适合真实世界实践和决策支持工作流的优化性能，有效解决了空间重划问题中连通性约束与搜索效率之间的矛盾。
...
### [220] [State Representation and Termination for Recursive Reasoning Systems](https://arxiv.org/abs/2605.06690)
*Debashis Guha,Amritendu Mukherjee,Sanjay Kukreja,Tarun Kumar*

Main category: cs.AI

TL;DR: 本文提出用认知状态图表示递归推理系统的推理状态，并定义序间隙（order-gap）作为停止迭代的判据，给出其非退化的充要条件，应用于多种推理场景。


<details>
  <summary>Details</summary>
Motivation: 递归推理系统中，推理状态的表示方式和何时停止迭代通常是隐式的，缺乏明确的形式化框架。本文旨在同时解决这两个问题。

Method: 将推理状态表示为认知状态图（包含提取的声明、证据关系、开放问题和置信权重）；定义序间隙（order-gap）衡量两种扩展-整合顺序下的状态距离；给出线性化序间隙在不动点附近非退化的充要条件。

Result: 给出了序间隙非退化的充要条件，表明该判据何时是有信息量的而不是代数上平凡的。这是一个局部条件，而非全局收敛保证。

Conclusion: 该框架可应用于递归推理系统、智能体循环、思维树推理、定理证明和持续学习等多种场景，为递归推理的终止判断提供了理论基础。
...
### [221] [Hidden Coalitions in Multi-Agent AI: A Spectral Diagnostic from Internal Representations](https://arxiv.org/abs/2605.06696)
*Cameron Berg,Susan L. Schneider,Mark M. Bailey*

Main category: cs.AI

TL;DR: 本文提出了一种通过分析多智能体系统内部神经表征的互信息图并进行谱分割，来检测智能体联盟结构的方法，并在多智能体强化学习和LLM环境中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 仅观察智能体行为不足以区分真正的信息耦合与虚假的行为相似性，而具有影响力的联盟可能在行为变化之前就已在内表征层面形成，因此需要一种能从内部表征检测联盟结构的方法。

Method: 从智能体的隐藏状态构建成对互信息图，然后应用谱分割识别最显著的联盟边界。

Result: 在多智能体强化学习环境中成功恢复编程设定的层次性和动态联盟结构，并正确拒绝行为协调带来的假阳性；在LLM中能识别提示隐含的联盟结构、追踪动态团队重组，并揭示表征层次结构。

Conclusion: 通过谱分割分析隐藏状态互信息是一种可扩展的用于识别表征联盟的诊断方法，为监控分布式AI系统中的涌现结构提供了有价值的工具。
...
### [222] [CASCADE: Case-Based Continual Adaptation for Large Language Models During Deployment](https://arxiv.org/abs/2605.06702)
*Siyuan Guo,Yali Du,Hechang Chen,Yi Chang,Jun Wang*

Main category: cs.AI

TL;DR: 本文提出CASCADE框架，将部署时学习（DTL）作为LLM生命周期的第三阶段，通过构建情景记忆和经验重用机制，使LLM智能体在部署过程中持续改进，无需修改模型参数，在16个多样化任务中平均成功率达20.9%的提升。


<details>
  <summary>Details</summary>
Motivation: 当前LLM生命周期中训练与部署严格分离，部署后学习能力停止，这与自然智能持续适应环境的能力形成鲜明对比。作者希望弥合这一差距，使LLM智能体能够在部署阶段从交互经验中持续学习。

Method: CASCADE框架为LLM智能体配备显式、可演化的情景记忆，将经验重用形式化为上下文赌博机问题，实现探索-利用权衡，并建立无遗憾保证。通过积累、选择和优化任务相关案例，将过往经验转化为可操作知识。

Result: 在16个任务（涵盖医疗诊断、法律分析、代码生成、网页搜索、工具使用和具身交互）上，CASCADE比零样本提示提升了20.9%的宏平均成功率，并持续优于基于梯度和基于记忆的基线方法。

Conclusion: 通过将部署重新定义为自适应学习过程，该工作为构建持续改进的AI系统奠定了基础。
...
### [223] [From Storage to Experience: A Survey on the Evolution of LLM Agent Memory Mechanisms](https://arxiv.org/abs/2605.06716)
*Jinghao Luo,Yuchen Tian,Chuxue Cao,Ziyang Luo,Hongzhan Lin,Kaixin Li,Chuyi Kong,Ruichao Yang,Jing Ma*

Main category: cs.AI

TL;DR: 本文提出了大语言模型智能体记忆机制的三阶段演化框架：存储（轨迹保存）、反思（轨迹优化）和经验（轨迹抽象），并分析了演化的核心驱动力及前沿机制。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体记忆机制研究分散于操作系统工程和认知科学两个领域，缺乏统一的视角和技术演化观，亟需一个整合性的理论框架来弥合这一鸿沟。

Method: 通过形式化定义记忆机制的三阶段演化框架（存储→反思→经验），分析三个核心驱动力（长程一致性需求、动态环境挑战、持续学习目标），并探索经验阶段的前沿机制（主动探索和跨轨迹抽象）。

Result: 提出了一个统一的LLM智能体记忆机制演化框架，明确了从轨迹保存到轨迹优化再到轨迹抽象的递进路径。
...
### [224] [When Does a Language Model Commit? A Finite-Answer Theory of Pre-Verbalization Commitment](https://arxiv.org/abs/2605.06723)
*Long Zhang,Wei-neng Chen,Feng-feng Wei,Zi-bo Qin*

Main category: cs.AI

TL;DR: 通过有限答案偏好稳定化方法（$\delta$投影），发现语言模型在可解析答案出现前17-31个token就已稳定其答案偏好，该信号可线性恢复、传递共享信息，但对因果控制不稳健。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型生成推理后给出最终答案，但无法判断模型何时在内部稳固了其答案偏好。作者希望在不依赖贪婪解码或学习探针的情况下，精确定位模型答案偏好稳定的时间点。

Method: 定义有限答案偏好稳定化框架，通过投影模型延续概率到有限答案集，在二分类任务中计算精确的log-odds编码$\delta(\xi)=S_\theta(\text{yes}|\xi)-S_\theta(\text{no}|\xi)$，并据此定义解析器为基础的答案起始点、回顾性稳定化时间及领先量。在Qwen3-4B-Instruct上设计延迟裁决任务进行实验。

Result: 上下文有限答案投影在答案可解析前就已稳定，主模板下平均领先17-31个token，解析器清洗复制下领先更短但仍为正。信号追踪模型最终输出而非事实真相，可从紧凑隐状态中线性恢复，部分可分离于光标进程，作为共享信息传递但无单一不变坐标。

Conclusion: 所提$\delta$信号可有效测量模型内在答案偏好的早期稳定化，支持离线诊断与分离，但精确因果控制（steering）仅表现出局部敏感性，无法实现可靠的生成控制。
...
### [225] [Weblica: Scalable and Reproducible Training Environments for Visual Web Agents](https://arxiv.org/abs/2605.06761)
*Oğuzhan Fatih Kar,Roman Bachmann,Yuanzheng Gong,Anders Boesen Lindbo Larsen,Afshin Dehghan*

Main category: cs.AI

TL;DR: 提出Weblica框架，通过HTTP缓存和LLM环境合成构建可复现、可扩展的Web环境，将强化学习训练扩展到数千个多样化环境，得到的8B模型在Web导航任务上超越同尺寸开源模型，与API模型竞争力相当。


<details>
  <summary>Details</summary>
Motivation: Web环境复杂、开放且不断变化，现有数据收集方法局限于离线轨迹或少量模拟环境，无法捕捉Web多样性，难以扩展视觉Web智能体的训练数据。

Method: 1) HTTP级别缓存捕捉和重放稳定视觉状态，同时保持交互行为；2) 基于LLM的环境合成，以真实网站和核心Web导航技能为基础构建多样化环境。

Result: Weblica-8B模型在多个Web导航基准上超越同尺寸开源模型，使用更少推理步骤，随测试时计算增加表现更好，与API模型竞争力相当。

Conclusion: Weblica框架有效解决了Web环境可扩展性和可复现性问题，证明了在多样化Web环境中进行RL训练能显著提升视觉Web智能体的导航能力。
...
### [226] [Towards Security-Auditable LLM Agents: A Unified Graph Representation](https://arxiv.org/abs/2605.06812)
...
TL;DR: 提出Agent-BOM，一种统一的结构化表示方法，通过分层有向属性图建模智能体系统的静态能力基与动态运行时语义状态，将碎片化执行轨迹转化为可查询的审计路径，用于复杂智能体生态系统的安全审计与根因分析。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体系统存在严重的语义鸿沟，低层物理事件与高层执行意图之间的差距使得事后安全审计极其困难。现有表示机制（如静态SBOM和运行时日志）只能提供碎片化证据，无法捕获认知状态演化、能力绑定、持久性记忆污染以及跨智能体的级联风险传播。

Method: 提出Agent-BOM，将智能体系统建模为层次化有向属性图，分离静态能力基（模型、工具、长期记忆）与动态运行时语义状态（目标、推理轨迹、动作），通过语义边和安全属性连接，并基于图查询实现路径级风险评估，结合OWASP Agentic Top 10实例化审计能力。

Result: 在OpenClaw环境中实现审计插件，对代表性真实世界攻击场景的评估表明，Agent-BOM能够重构隐蔽攻击链，包括跨会话记忆投毒与工具滥用、能力供应链劫持与意外代码执行、多智能体生态系统劫持以及权限与信任滥用。

Conclusion: Agent-BOM为复杂智能体生态系统中的根因分析与安全裁定提供了统一且可审计的基础。
...
### [227] [Uneven Evolution of Cognition Across Generations of Generative AI Models](https://arxiv.org/abs/2605.06815)
*Isaac Galatzer-Levy,Daniel McDuff,Xin Liu,Jed McGiffin*

Main category: cs.AI

TL;DR: 本文提出了一种心理测量框架来评估生成式AI的认知能力，发现多模态模型在言语理解和工作记忆方面表现极佳（>98%），但在知觉推理方面表现极差（<1%）。通过AI商数基准测试追踪多代模型，发现能力发展不平衡，抽象定量推理在语言模式下提升更快，视觉知觉组织能力几乎停滞，表明仅靠规模化和优化方法难以实现均衡的人类般通用智能。


<details>
  <summary>Details</summary>
Motivation: 现有的AI评估方法多关注狭窄任务表现，缺乏对模型认知能力的系统性评估，尤其缺乏与人类认知能力直接比较的心理测量框架。

Method: 采用韦氏成人智力量表改编任务评估多模态模型，并开发AI商数基准测试，对六代两个模型家族进行纵向追踪。

Result: 模型在言语理解和记忆方面接近天花板（>98%），知觉推理接近地板（<1%）；抽象定量推理在语言模态下显著优于视觉模态；视觉知觉组织能力长期停滞。

Conclusion: 生成式AI的认知能力发展极不均衡，现有规模化方法可能无法克服架构内在局限以实现均衡的通用智能。
...
### [228] [Extracting Search Trees from LLM Reasoning Traces Reveals Myopic Planning](https://arxiv.org/abs/2605.06840)
*Sixing Chen,Ji-An Li,Saner Cakir,Sinan Akcali,Kayla Lee,Marcelo G. Mattar*

Main category: cs.AI

TL;DR: 本研究通过四子棋游戏提取LLM推理轨迹中的搜索树，发现LLM的规划深度较浅，决策主要由浅层搜索驱动，与人类依赖深层搜索形成鲜明对比。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LLM（尤其是推理模型）在思维链过程中是否进行真正的规划、规划如何结构以及哪些因素驱动性能仍缺乏深入理解。

Method: 从四子棋推理轨迹中提取搜索树，拟合计算模型量化规划结构，并通过因果干预实验（选择性剪枝CoT段落）验证浅层与深层节点对决策的影响。

Result: LLM搜索深度浅于人类；性能由搜索广度而非深度预测；决策主要由忽视深层节点的短视模型解释；因果干预证实浅层节点主导决策。

Conclusion: LLM与人类规划存在关键差异：人类依赖深层搜索，而LLM不基于深层前瞻行动，为对齐两者规划机制提供指导。
...
### [229] [AGWM: Affordance-Grounded World Models for Environments with Compositional Prerequisites](https://arxiv.org/abs/2605.06841)
*Qinshi Zhang,Weipeng Deng,Zhihan Jiang,Jiaming Qu,Qianren Li,Weitao Xu,Ray LC*

Main category: cs.AI

TL;DR: 提出AGWM模型，通过有向无环图学习动作的抽象affordance结构，显式追踪动作可执行性动态变化，解决传统世界模型在多步预测中因忽略动作前提条件而导致的错误累积问题。


<details>
  <summary>Details</summary>
Motivation: 传统世界模型学习固定的状态转移函数，将频繁共现的动作与结果内化为因果规则，忽略了动作的前提条件。在交互环境中，动作的可执行性会动态变化（结构改变事件），导致多步预测错误累积。

Method: 提出AGWM（Affordance-Grounded World Model），学习一个以有向无环图（DAG）表示的抽象affordance结构，显式追踪每个动作在当前状态下是否可执行，从而在想象轨迹中保持正确的affordance状态。

Result: 在基于游戏的模拟环境实验中，AGWM实现了更低的多步预测误差、对新颖配置更好的泛化能力以及更强的可解释性。

Conclusion: 显式建模动作的affordance结构能够有效提升世界模型在动态环境中的预测准确性和泛化能力，为模型-based强化学习提供了更可靠的多步推理基础。
...
### [230] [Agentick: A Unified Benchmark for General Sequential Decision-Making Agents](https://arxiv.org/abs/2605.06869)
*Roger Creus Castanyer,Pablo Samuel Castro,Glen Berseth*

Main category: cs.AI

TL;DR: Agentick是一个统一基准，通过37个程序化生成的任务、多模态观测和Gymnasium接口，公平比较RL、LLM、VLM、混合和人类智能体在序列决策中的表现。评估发现没有单一范式占优（GPT-5 mini综合最佳0.309分，PPO在规划和多智能体任务领先），推理框架可提升LLM性能3-10倍，ASCII优于自然语言观测。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体研究从RL到基础模型智能体跨度广泛，但缺乏统一的基准来公平比较这些不同范式在序列决策任务上的表现。

Method: 构建了包含37个程序化生成任务、6大能力类别、4个难度级别和5种观测模态的基准平台，提供Coding API、oracle参考策略、预构建SFT数据集、可组合智能体框架和实时排行榜，通过单一Gymnasium兼容接口统一评估。

Result: 对27种配置、超过90,000个episodes的评估显示：GPT-5 mini综合最优（0.309 orcale归一化分数），PPO在规划和多智能体任务中占优；推理框架可将LLM性能提升3-10倍；ASCII观测始终优于自然语言观测。

Conclusion: 所有智能体范式仍有巨大改进空间，Agentick通过能力分解、多模态设计为通用自主智能体的发展提供了评估基础设施和训练平台，尤其适用于基础模型的RL后训练。
...
### [231] [How Well Do LLMs Perform on the Simplest Long-Chain Reasoning Tasks: An Empirical Study on the Equivalence Class Problem](https://arxiv.org/abs/2605.06882)
*Chun Zheng,Lianlong Wu,Bingqian Li,Lvting Liu,Yi Zhou*

Main category: cs.AI

TL;DR: 本文评估了大语言模型（LLMs）在等价类问题（ECP）——一种简单但长链推理任务上的表现，发现非推理模型完全失败，推理模型表现更好但仍无法完全解决；非推理模型的最难实例对应相变点，推理模型的最难实例对应最大直径。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs近年取得了巨大进步，但它们在长链推理任务上的能力仍不清晰，因此作者希望系统评估LLMs在推理任务上的真实表现。

Method: 选择等价类问题（ECP）作为基准任务，对推理型和非推理型两类代表性LLMs进行大规模实验，变化变量数量、连接概率、提示方式等多个因素，系统评估其表现。

Result: 非推理型LLMs完全无法解决ECP；推理型模型表现显著更好但仍无法完全解决。非推理模型的最难实例出现在连接概率为ln n/(n-1)的相变点，推理模型的最难实例出现在图直径最大的区域。

Conclusion: LLMs在长链推理任务上仍有显著不足，且不同类型模型存在不同的困难模式：非推理模型受问题内在混沌性影响，推理模型受推理链条长度（直径）影响。
...
### [232] [Beyond the Black Box: Interpretability of Agentic AI Tool Use](https://arxiv.org/abs/2605.06890)
*Hariom Tatsat,Ariye Shater*

Main category: cs.AI

TL;DR: 本文提出基于稀疏自编码器和线性探针的机制可解释性工具包，在模型执行工具调用前读取内部状态，推断工具需求及后果，实现对智能体工具调用的内部可观测性。


<details>
  <summary>Details</summary>
Motivation: 现有外部观测方法（提示词、评估、日志）只能事后发现问题，无法在工具调用前预判错误。长周期任务中早期工具失误会串行影响后续轨迹，导致高成本和安全风险，亟需内部可观测性机制。

Method: 构建基于稀疏自编码器（SAE）和线性探针的机制可解释性框架，在每次动作前读取模型状态，分解激活为稀疏特征，识别与工具决策相关的内部层和特征，并通过特征消融测试功能性重要性。在NVIDIA Nemotron函数调用数据集的多步轨迹上训练，应用于GPT-OSS 20B和Gemma 3 27B模型。

Result: 该方法能推断工具需求及工具动作后果的严重性，定位与工具决策最相关的内部层和稀疏特征，为长周期智能体运行中早期错误的后链影响提供内部可视化。

Conclusion: 机制可解释性可为智能体系统提供实用的内部可观测性，用于监控工具调用和风险，补充而非替代外部评估方法。
...
### [233] [Mitigating Cognitive Bias in RLHF by Altering Rationality](https://arxiv.org/abs/2605.06895)
*Tiffany Horter,Andrew Markham,Niki Trigoni,Serena Booth*

Main category: cs.AI

TL;DR: 本文提出了一种动态调整理性参数β的方法，通过LLM裁判评估认知偏差的存在，在RLHF中降低不可靠人类反馈的权重，从而学习更合理的奖励模型。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF将人类反馈的理性参数β设为固定常数，忽视了真实人类反馈中存在的认知偏差和可靠性差异，导致奖励模型学习到有偏的偏好。

Method: 将理性参数β视作文本和标注依赖的动态变量，利用LLM作为裁判评估认知偏差的存在，在奖励学习过程中自适应地降低有偏或不可靠比较的权重。

Result: 即使在含有强烈有偏偏好的数据集上微调，该方法也能学习到更合理的下游模型。

Conclusion: 动态调整理性参数能够有效缓解人类反馈中认知偏差对RLHF的影响，提升奖励模型和下游策略的合理性。
...
### [234] [Self-Programmed Execution for Language-Model Agents](https://arxiv.org/abs/2605.06898)
*Luke J. O'Connor*

Main category: cs.AI

TL;DR: 本文提出自编程执行（SPE）架构，其中语言模型的补全本身充当编排程序，而非由固定编排器控制。通过Spell语言（基于Lisp）实现程序自编辑与重评估，实验证明现有前沿模型可在无固定编排策略下完成复杂智能体任务。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型智能体依赖固定的编排器程序来管理回合间状态转换，这种固定策略限制了模型的自主性和灵活性。本文希望打破这种限制，让模型自身成为编排者。

Method: 提出SPE架构，形式化为智能体机器。开发基于Lisp的Spell语言，支持程序自编辑和重评估，且副作用（如模型调用）在重评估时不会重放。通过现有模型（未经SPE或Spell训练）进行实验验证。

Result: 前沿模型能够在SPE架构下运行并完成具有挑战性的智能体任务，证明了无固定编排策略的可行性。

Conclusion: 语言模型可以在无固定编排策略下作为智能体运作，未来可能通过学习训练出更优的自编排策略。
...
### [235] [Multi-Objective Constraint Inference using Inverse reinforcement learning](https://arxiv.org/abs/2605.06951)
*Syed Ihtesham Hussain Shah,Floris den Hengst,Aneta Lisowska,Annette ten Teije*

Main category: cs.AI

TL;DR: 本文提出MOCI框架，能从异构专家轨迹中联合提取共享约束和个体偏好，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有约束推理方法通常假设专家演示是同质的（由单一专家或目标相同的多个专家生成），难以捕捉个体偏好，且计算效率低。

Method: 提出多目标约束推理（MOCI）框架，从追求不同目标的异构专家轨迹中联合提取共享约束和个体偏好，有效建模多样化甚至冲突的行为。
...
### [236] [Learning and Reusing Policy Decompositions for Hierarchical Generalized Planning with LLM Agents](https://arxiv.org/abs/2605.06957)
*Shirin Sohrabi,Haritha Ananthakrishnan,Harsha Kokel,Kavitha Srinivas,Michael Katz*

Main category: cs.AI

TL;DR: 提出了一种结合广义规划与层次化任务分解的动态策略学习方法HCL-GP，通过自动分解、组件泛化和语义检索，在AppWorld基准上实现了98.2%的常规任务准确率和97.8%的挑战任务准确率，相比静态合成提升了15.8个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体缺乏有效的策略泛化能力和组件复用机制，难以在未见过的应用场景中高效执行任务。

Method: HCL-GP方法通过自动分解成功执行记录来学习组件，构建组件库，并利用语义搜索进行高效检索，实现基于组件的组合式策略生成。

Result: 在AppWorld基准上，常规任务准确率98.2%，挑战任务准确率97.8%；在开源模型上，动态复用实现62.5%成功率，而无复用时接近零。

Conclusion: 经典规划概念可有效与LLM智能体集成，显著提升准确性和效率。
...
### [237] [Adaptive auditing of AI systems with anytime-valid guarantees](https://arxiv.org/abs/2605.07002)
*Siyu Zhou,Patrick Vossler,Venkatesh Sivaraman,Yifan Mai,Jean Feng*

Main category: cs.AI

TL;DR: 本文针对生成式AI系统的自适应测试（adaptive testing）中统计推断的严谨性问题，提出了一种基于安全随时有效推断（SAVI）的假设检验框架，通过“对赌式检验”（testing by betting）形式化审计过程，证明了在足够强力的审计下，两个对立的零假设（模型无故障 vs 审计员无法发现故障）渐近互为逆命题，从而在仅需20个观测样本时即可获得统计上严谨的结论。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统的故障模式标注和评估成本高昂，自适应测试范式虽灵活实用，但由于观测样本少（通常10-50例）且采样/停止决策在数据收集过程中动态进行，违反了经典统计假设，难以得出统计上严谨的结论。因此需要一种能处理高度自适应审计的统计推断框架。

Method: 引入“双视角”假设检验框架：模型零假设（无故障模式低于性能阈值）vs 审计员零假设（采样策略能发现故障模式）。利用安全随时有效推断（SAVI）将审计形式化为“对赌式检验”（testing by betting），转化为同时检验两个对立零假设的e-process。理论证明在审计足够强有力时，两个假设渐近互为逆命题。

Result: 实验表明，所提出的检验程序能维持随时有效的I类错误控制，优于预先指定的检验方法，并且有时仅需20个观测样本即可得出统计严谨的结论。

Conclusion: 该工作为高度自适应的AI系统审计提供了统计严谨的推断框架，严格审计通过可证明系统具有全局鲁棒性，实现了小样本下的可靠评估。
...
### [238] [Behavior Cue Reasoning: Monitorable Reasoning Improves Efficiency and Safety through Oversight](https://arxiv.org/abs/2605.07021)
*Christopher Z. Cui,Taylor W. Killian,Prithviraj Ammanabrolu*

Main category: cs.AI

TL;DR: 本文提出行为线索推理（Behavior Cue Reasoning）方法，通过在LLM推理过程中插入特殊token序列作为行为和控制的信号/控制杆，使推理过程更可控和可监控。实验表明该方法可节省最多50%的推理token，并将不安全行为的成功率从46%提升至96%，且不影响模型性能。


<details>
  <summary>Details</summary>
Motivation: LLM的推理过程难以监督，许多不当行为仅在推理结束时才显现，因此需要一种方法使推理过程更可控和可监控。

Method: 提出Behavior Cues——特殊token序列，模型在特定隐式和显式行为前立即生成这些序列，作为信号和控制杠杆。通过强化学习训练外部监控器，利用Behavior Cues呈现的压缩信息进行推理监督。

Result: 1) 在复杂数学问题求解中，可节省最多50%的推理token；2) 在约束违规会导致失败的环境中，从80%原本会导致不安全行为的推理轨迹中恢复安全行为，成功率从46%提升至96%。

Conclusion: Behavior Cue Reasoning提高了推理的可监控性和可控性，且不影响模型性能。该工作通过训练模型本身以更易于监督的方式进行推理，推进了可扩展监督（scalable oversight）的发展。
...
### [239] [2.5-D Decomposition for LLM-Based Spatial Construction](https://arxiv.org/abs/2605.07066)
*Paul Whitten,Li-Jen Chen,Sharath Baddam*

Main category: cs.AI

TL;DR: 本文提出基于2.5-D分解的神经符号流水线，将LLM的规划限制在二维水平面，由确定性执行器处理垂直放置，在Build What I Mean基准上达到94.6%的结构准确率，接近97.6%的理论上限，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自主系统需要根据自然语言指令构建结构，但大语言模型在三维空间坐标推理中存在系统性错误，需要一种更可靠的空间推理方法。

Method: 提出2.5-D分解的神经符号流水线：LLM仅在二维水平面进行规划，确定性执行器根据柱状占用信息计算所有垂直放置位置，将确定性维度从LLM输出空间中移除。

Result: 在Build What I Mean基准上，GPT-4o-mini达到94.6%平均结构准确率，优于GPT-4o（90.3%）和最佳竞品（76.3%），接近97.6%理论上限；消融实验证实2.5-D分解贡献了50.7个百分点的准确率提升；可直接迁移至边缘硬件（NVIDIA Jetson Thor AGX上达94.5%）。

Conclusion: 移除LLM输出空间中的确定性维度这一原理可推广至任何受重力或物理约束的自主建造/组装任务，在IGLU协作构建任务上的迁移实验证实了其泛化能力。
...
### [240] [TeamBench: Evaluating Agent Coordination under Enforced Role Separation](https://arxiv.org/abs/2605.07073)
...
TL;DR: 提出TeamBench基准测试，用于评估OS强制角色分离下的Agent协作，发现纯提示方式导致角色越权，人类研究揭示通过率无法捕捉的协作模式。


<details>
  <summary>Details</summary>
Motivation: 现有Agent系统将任务分解为多角色，但角色仅通过提示指定而无访问控制，导致团队通过率无法区分真实协作还是角色越权。

Method: 构建TeamBench基准测试（851个任务模板/931个实例），通过操作系统级权限强制分离规划、执行、验证三个角色，限制各角色的规格访问、工作区编辑和最终认证权限。

Result: 纯提示与沙箱强制团队通过率无统计差异，但纯提示组验证者越权编辑执行者代码的频率高出3.6倍；验证者批准49%不合格提交；去除验证者提升平均部分得分。

Conclusion: 团队协作价值有条件性（单Agent困难时有效，单Agent表现好时反而有害），通过率等表面指标掩盖了角色越权、验证失效等深层协作问题。
...
TL;DR: 本文提出在线共享供应分配（OSSA）问题，并设计了一个确定性的阈值比例策略GPA，证明其能达到4/3近似比（匹配紧界），同时扩展了学习增强版本以利用不完美预测。


<details>
  <summary>Details</summary>
Motivation: 现实中的资源分配系统（如人道主义物流和疫苗分发）需要在需求实现前将有限供应预置于多个地点，缺货会导致不可逆的服务损失。传统库存模型允许 backlog 或补货，而 OSSA 更贴近这类不可回溯的实际场景。

Method: 提出确定性的阈值比例策略 GPA，并证明其 4/3 近似比；通过下界证明该比率是紧的，且加法误差项不可避免；进一步扩展为学习增强版本，融合不完美预测（如专家或 ML 模型）。

Result: GPA 达到 4/3 近似比（匹配紧界），下界证明该比率不可改进；学习增强版本能在利用高质量建议的同时对任意坏建议保持鲁棒；实验表明全局供应稀缺时 GPA 优于自然基线。

Conclusion: OSSA 模型为不可回溯的在线资源分配提供了理论框架，GPA 策略在理论上最优，学习增强扩展增强了实际可用性。
...
### [242] [ARMOR: An Agentic Framework for Reaction Feasibility Prediction via Adaptive Utility-aware Multi-tool Reasoning](https://arxiv.org/abs/2605.07103)
*Ye Liu,Botao Yu,Xinyi Ling,Daniel Adu-Ampratwum,Xia Ning*

Main category: cs.AI

TL;DR: 提出了ARMOR框架，通过显式建模工具效用、自适应优先级排序和冲突解决机制，在化学反应可行性预测中整合多个AI工具的性能优势，显著优于单一工具和现有集成方法。


<details>
  <summary>Details</summary>
Motivation: 现有单个AI工具在化学反应可行性预测任务上表现不一，无法在所有反应上持续保持良好性能，因此需要一种有效利用多种工具以获取更准确预测的方法。

Method: 提出ARMOR智能体框架，通过层次化组织工具（优先使用高性能工具）、建模工具特定模式以刻画其优势、以及基于记忆增强推理的冲突解决机制，实现对多种工具的适应性整合。

Result: 在公开数据集上的实验表明，ARMOR consistently优于强基线方法（包括单一工具方法以及多种工具聚合/选择方法），尤其在工具预测存在冲突的反应上提升更为显著。

Conclusion: ARMOR能够有效利用多种工具的互补优势来解决反应可行性预测问题，代码已开源。
...
### [243] [AdaTKG: Adaptive Memory for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2605.07121)
*Seunghan Lee,Jun Seo,Jaehoon Lee,Sungdong Yoo,Minjae Kim,Tae Yoon Lim,Dongwan Kang,Hwanil Choi,SoonYoung Lee,Wonbin Ahn*

Main category: cs.AI

TL;DR: AdaTKG提出用自适应过程建模实体，通过每个实体维护的动态记忆更新表示，替代传统静态实体表示，在时序知识图谱推理中取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 现有TKG方法生成静态实体表示，每个表示仅依赖于学习参数，不保留实体参与交互的痕迹，无法捕捉实体随时间演化的动态特性。

Method: AdaTKG为每个实体维护一个记忆，在每次观察到交互时更新该记忆；记忆更新采用可学习的指数移动平均，由单一共享标量控制，无需为每个实体设置独立参数，从而能处理训练中未见的实体。

Result: 广泛实验表明，AdaTKG在多个TKG基准上持续优于现有方法，验证了自适应记忆机制的有效性。

Conclusion: 将实体建模为自适应过程，通过在线累积交互记忆来动态更新表示，能显著提升TKG推理性能。
...
### [244] [Can You Break RLVER? Probing Adversarial Robustness of RL-Trained Empathetic Agents](https://arxiv.org/abs/2605.07138)
*Deeraj S K,Sadhana Devarajan,Krishna Mehra,Sudhakar Mishra*

Main category: cs.AI

TL;DR: 本文提出对抗性共情基准AEB和情感一致性分数ECS，发现RLVER模型在对抗性对话中情感响应性提升但状态追踪能力未见改善，存在行为与可读性的分离。


<details>
  <summary>Details</summary>
Motivation: 现有共情基准假设用户合作诚实，但真实情感互动中存在gaslighting、情绪升级等对抗行为，现有基准无法评估模型在此类场景下的鲁棒性。

Method: 构建包含6种心理学对抗轨迹类型的AEB基准，引入ECS分数；在8种场景匹配条件下（think/no-think × 2个RLVER模型 + 2个基础模型），进行480轮对抗对话实验。

Result: RLVER-PPO-Think在最终分数上显著优于基线（0.963 vs 0.761），零对话崩溃，隐藏意图检测提高47%；但ECS分数几乎持平且无显著差异（p=0.650）。

Conclusion: RL训练提升了情感响应性但未改善可观察的状态追踪能力，存在行为与可读性的分离，不代表模型具备真正理解或临床就绪性。
...
### [245] [SREGym: A Live Benchmark for AI SRE Agents with High-Fidelity Failure Scenarios](https://arxiv.org/abs/2605.07161)
*Jackson Clark,Yiming Su,Saad Mohammad Rafid Pial,Yifang Tian,Lily Gniedziejko,Hans-Arno Jacobsen,Yinfang Chen,Tianyin Xu*

Main category: cs.AI

TL;DR: SREGym是一个高保真、模块化、可扩展的SRE智能体基准测试平台，包含90个真实故障场景，可评估不同智能体在多种故障类型上的表现差异（高达40%）。


<details>
  <summary>Details</summary>
Motivation: 现有SRE基准测试过于简化且难以扩展，缺乏能够模拟生产环境复杂性的高保真评估平台。

Method: 基于真实云原生系统栈构建实时系统环境，通过故障注入器模拟多层故障、环境噪声及多种故障模式（如亚稳态故障、关联故障），采用模块化可扩展架构。

Result: 包含90个现实SRE问题，评估前沿智能体发现不同故障类型下性能差异显著，端到端结果差异高达40%。

Conclusion: SREGym作为一个活跃维护的开源项目，已被研究人员和从业者使用，为SRE智能体评估提供了更真实、更全面的基准。
...
### [246] [Repeated Deceptive Path Planning against Learnable Observer](https://arxiv.org/abs/2605.07174)
*Shiyue Cao,Pei Xu,Likun Yang,Lei Cui,Shizhao Yu,Shiyu Zhang,Yongjian Ren,Xiaotang Chen,Kaiqi Huang*

Main category: cs.AI

TL;DR: 本文提出重复欺骗路径规划（RDPP）问题，针对可学习观察者场景，并设计了一个双层优化框架DeMP，通过情节级适应和元级更新实现持续欺骗，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有欺骗路径规划（DPP）方法假设观察者是静态的、非学习的，但在现实场景中（如关键物资运输、军事行动），对手可以通过学习历史轨迹来适应。因此需要研究可学习观察者下的重复欺骗路径规划问题。

Method: 提出Deceptive Meta Planning (DeMP)，一个双层优化框架：情节级适应实现短期策略调整以应对更新的观察者；元级更新利用跨情节反馈捕捉观察者的学习模式，加速未来情节的适应，从而缓解累积适应滞后问题。

Result: 实验表明，DeMP在RDPP问题中显著优于现有方法，同时保持具有竞争力的路径成本。

Conclusion: 该工作强调了建模与可学习对手重复交互的重要性，为多智能体系统中的欺骗与隐私提供了新见解。
...
### [247] [Three-in-One World Model: Energy-Based Consistency, Prediction, and Counterfactual Inference for Marketing Intervention](https://arxiv.org/abs/2605.07199)
*Junichiro Niimi*

Main category: cs.AI

TL;DR: 提出了一种三合一世界模型架构，通过深度玻尔兹曼机学习冻结的信念表示，在同一框架内支持一致性评估、结果预测和反事实推断三种任务，在模拟中优于元学习器和因果森林基线。


<details>
  <summary>Details</summary>
Motivation: 当前预测导向和语言导向的模型无法统一捕捉消费者异质性、时变内部状态和显式干预三者之间的交互结构。

Method: 三合一世界模型架构：DBM从人口统计、时间和滞后行为中学习冻结信念表示，其上附加轻量级任务特定适配器；通过DBM自由能进行能量一致性评估、适配器进行结果预测、固定信念仅改变动作输入进行反事实推断。

Result: 适配器在访问和购买AUC上匹配强MLP基线，在恢复异质性处理效应上显著优于S、T、X、DR学习器及因果森林；自由能钳制惩罚缺乏促销曝光史的反事实购买轨迹，且惩罚依赖于潜在基础偏好。

Conclusion: DBM信念能够以反事实查询可用的形式解耦潜在特质，为营销干预提供统一的世界模型基础。
...
### [248] [HMACE: Heterogeneous Multi-Agent Collaborative Evolution for Combinatorial Optimization](https://arxiv.org/abs/2605.07214)
*Yuping Yan,Jirui Han,Fei Ming,Yuanshuai Li,Yaochu Jin*

Main category: cs.AI

TL;DR: HMACE是一个异构多智能体协作进化框架，通过四个专业化智能体（提议者、生成器、评估者、反思者）协同工作，为NP-hard组合优化问题自动设计启发式算法，在多个基准问题上取得了更优的质量-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动启发式设计方法通常依赖单一工作流和刚性模板，限制了基于记忆的探索，容易过早收敛到局部最优解。

Method: 将每次进化迭代分解为四个角色专业化智能体的自主协作循环：Proposer负责策略探索，Generator负责生成可执行启发式，Evaluator负责经验评估，Reflector负责基于存档的记忆更新；结合行为感知检索、轻量级候选过滤和适应度导向的存档更新机制。

Result: 在TSP、Online BPP、MKP和PFSP等代表性组合优化问题上，HMACE相比单智能体和多智能体基线取得了更优的质量-效率权衡；在LLM驱动的方法对比中，TSP和Online BPP上分别达到最低平均差距0.464%和0.223%，且仅消耗0.13M和0.42M tokens。

Conclusion: HMACE通过异构多智能体协作架构有效克服了现有方法的局限性，实现了更高效、更高质量的自适应启发式搜索。
...
### [249] [MEMOREPAIR: Barrier-First Cascade Repair in Agentic Memory](https://arxiv.org/abs/2605.07242)
*Yang Zhao,Chengxiao Dai,Mengying Kou,Yue Xiu*

Main category: cs.AI

TL;DR: 本文提出记忆级联更新问题，并设计MemoRepair修复契约，通过最优前驱闭包子图选择实现零暴露与低成本修复。


<details>
  <summary>Details</summary>
Motivation: 当前智能体记忆系统中，因源工件被删除、更正或API迁移导致派生记忆失效，却仍可见并影响后续决策，缺乏系统性修复机制。

Method: 提出MemoRepair，采用屏障优先的级联修复契约：修复前撤回受影响派生状态，基于保留支持与已修复前驱构建后继，并通过最大权重前驱闭包（s-t最小割）优化选择已验证的后继发布。

Result: 在ToolBench和MemoryArena上，完全影响溯源下将无效记忆暴露从69.8-94.3%降至0%；相比全量修复，恢复91.1-94.3%已验证后继，归一化修复开销从1.00降至0.57-0.76。

Conclusion: MemoRepair通过级联修复契约与优化选择，有效消除无效记忆暴露，大幅降低修复成本，为智能体记忆系统提供了可行的维护方案。
...
### [250] [EnvSimBench: A Benchmark for Evaluating and Improving LLM-Based Environment Simulation](https://arxiv.org/abs/2605.07247)
*Yi Liu,TingFeng Hui,Wei Zhang,Li Sun,Ningxin Su,Jian Wang,Sen Su*

Main category: cs.AI

TL;DR: 本文提出EnvSimBench，首次正式定义并量化了环境模拟能力（EnvSim Ability），构建了包含400个样本、167个多样化环境的基准测试。研究发现所有先进语言模型都存在“状态变化悬崖”问题——在环境状态不变时表现完美，但多状态同步更新时性能急剧下降。作者还设计了约束驱动的模拟流程，将幻觉减少6.8%，成本降低90%以上。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体训练依赖交互环境，但手工构建环境成本高、扩展性差、多样性有限。LLM模拟环境虽有前景，但存在幻觉、逻辑不一致和状态漂移等严重问题，破坏了奖励信号并增加了构建成本。本文旨在填补这一关键能力缺口。

Method: 1) 首次形式化定义环境模拟能力（EnvSim Ability）；2) 构建EnvSimBench基准，包含400个样本、167个环境，配备可验证标签和三维难度分层；3) 系统评估多种先进语言模型；4) 设计约束驱动的模拟流程来减少幻觉和成本。

Result: 所有SOTA语言模型存在普遍的“状态变化悬崖”现象：单状态不变任务接近完美，多状态同步更新时灾难性失败。约束驱动流程减少幻觉6.8%，成本降低90%以上。

Conclusion: EnvSimAbility是关键但未被充分解决的能力缺口。EnvSimBench既可作为诊断框架，也可作为优化路径，为可靠的大模型环境模拟和可扩展的智能体训练奠定基础。
...
### [251] [Can Agents Price a Reaction? Evaluating LLMs on Chemical Cost Reasoning](https://arxiv.org/abs/2605.07251)
...
TL;DR: 提出了ChemCost基准，包含1,427个化学反应的成本估算任务，用于评估LLM在科学工具使用中的表现。最强的智能体在干净输入上准确率仅50.6%，且对噪声敏感，失败源于解析脆弱、证据整合无效等问题。


<details>
  <summary>Details</summary>
Motivation: 现有科学工具使用评估多依赖人工展示、专家判断或LLM作为评判，缺乏基于精确、无评判的真实标准。化学领域的智能体虽能调用工具，但缺乏严格的定量评估基准。

Method: 构建ChemCost基准（1,427个反应，2,261种化学品，230,775条供应商报价），包含标量评分和分阶段诊断；设计噪声注入视图测试鲁棒性；评估前沿、开源及化学专用LLM智能体。

Result: 最强智能体在干净输入上50.6%准确率（25%相对误差内），噪声下显著下降；工具访问必要但不充分；失败源于解析脆弱、证据整合无效、包装选择无效和工具使用不收敛。

Conclusion: 现有LLM智能体在科学工具使用任务上表现有限，需要更鲁棒的解析、推理和工具协作能力。
...
### [252] [Structured Role-Aware Policy Optimization for Multimodal Reasoning](https://arxiv.org/abs/2605.07274)
*Bingqing Jiang,Difan Zou*

Main category: cs.AI

TL;DR: SRPO通过角色感知的token级信用分配，将GRPO的序列级优势分解为感知令牌和推理令牌各自的优势，从而提升多模态推理中基于证据的推理能力。


<details>
  <summary>Details</summary>
Motivation: 在多模态推理中，RLVR的最终答案奖励通常在序列级别分配，无法区分不同token的功能角色，难以判断正确答案是否由任务相关的视觉证据支持。

Method: 提出结构化角色感知策略优化（SRPO），将结构化响应分解为感知令牌（提取视觉证据）和推理令牌（推导答案），通过自蒸馏的在线对比为不同角色分配特定信用，无需外部奖励模型或独立教师。

Result: 在多个多模态推理基准测试中，SRPO相比GRPO提升了基于证据的推理能力。
...
### [253] [Signal Reshaping for GRPO in Weak-Feedback Agentic Code Repair](https://arxiv.org/abs/2605.07276)
*Jia Li,Yuxin Su,Ting Peng,Hailiang Huang,Yuetang Deng,Michael R. Lyu*

Main category: cs.AI

TL;DR: 本文研究代码智能体RL中的信号重塑问题，提出在GRPO框架下对结果奖励、过程信号和组内可比性三种信号进行重塑，显著提升了编译-语义准确率（从0.385到0.535）。


<details>
  <summary>Details</summary>
Motivation: 代码智能体RL中，rollout阶段的反馈信号虽然可靠但只能捕获任务成功的表层条件（如编译通过），无法反映真实的语义目标（如功能正确），导致强化学习信号弱且不完整。

Method: 在标准GRPO框架下提出三种信号重塑：1）编译-语义分层奖励重塑轨迹排名；2）步骤级过程评分（独立于组归一化）重塑轨迹内更新强度；3）失败原因感知的rollout治理重塑组内可比性。

Result: 全信号重塑GRPO将编译-语义准确率从基线的0.385提升至0.535；过程评分加权在分层奖励基础上将准确率从0.48提升至0.53，平均评估步数从23.50降至17.02。

Conclusion: GRPO的组内比较只有在三种信号重塑后才有效；令牌级蒸馏在长工具链轨迹中信号被稀释，无法替代结果语义、过程信用和组内可比性。
...
### [254] [SOM: Structured Opponent Modeling for LLM-based Agents via Structural Causal Model](https://arxiv.org/abs/2605.07301)
*Shiyue Cao,Pei Xu,Likun Yang,Lei Cui,Xiaotang Chen,Kaiqi Huang*

Main category: cs.AI

TL;DR: 提出结构化对手建模（SOM）框架，通过结构因果模型（SCM）分离对手建模与预测过程，提升LLM在多智能体环境中对手行为预测的准确性和稳定性。
...
Method: 采用两阶段框架：1）构建阶段，使用SCM（基于图的形式化方法）捕获对手观测与行动间的有向依赖关系，形成显式结构化表示；2）预测阶段，LLM沿SCM导出的清晰路径进行结构化推理。

Result: 在多个多智能体基准测试中，SOM一致优于最先进的基于LLM的推理基线方法，在复杂动态环境中实现更准确、更自适应的战略决策。

Conclusion: 显式分离对手建模与预测，并利用SCM引导结构化推理，能显著提升LLM在多智能体交互中的预测表现和适应性。
...
### [255] [Implicit Compression Regularization: Concise Reasoning via Internal Shorter Distributions in RL Post-Training](https://arxiv.org/abs/2605.07316)
*Chen Wang,Hexuan Deng,Yining Zhang,Yuchen Zhang,Jionghao Bai,Zhaochun Li,Ge Lan,Yue Wang*

Main category: cs.AI

TL;DR: 本文提出隐式压缩正则化（ICR）方法，通过利用rollout分组中最短正确回答作为压缩信号，在不牺牲准确率的前提下有效缩短LLM推理链，缓解了“过度思考”问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（长度惩罚、早退策略）存在局限：前者可能降低准确率并引发“欠思考”，后者假设大部分推理链可安全截断。需要一种无这些限制的压缩信号。

Method: 提出ICR方法：基于“长度-准确率负相关→过度思考”的观察，利用on-policy rollout分组中最短正确回答构建虚拟短分布，作为压缩信号指导策略向简洁正确轨迹优化。

Result: 在多个推理骨干和数学/知识密集型基准上，ICR一致地缩短了响应长度，同时保持或提升了准确率，实现了更强的准确率-长度帕累托前沿。

Conclusion: ICR通过隐式压缩正则化有效解决了LLM推理中的过度思考问题，相比现有方法在压缩与准确率之间取得了更好的平衡。
...
### [256] [Discovering Ordinary Differential Equations with LLM-Based Qualitative and Quantitative Evaluation](https://arxiv.org/abs/2605.07323)
*Sum Kyun Song,Bong Gyun Shin,Jae Yong Lee*

Main category: cs.AI

TL;DR: 提出DoLQ方法，利用大语言模型进行定性和定量评估，通过多智能体架构从观测数据中发现常微分方程，在基准测试中取得优于现有方法的性能。
...
Method: 提出DoLQ方法，采用多智能体架构：Sampler Agent提出动力学系统候选，Parameter Optimizer优化参数精度，Scientist Agent利用LLM同时进行定性和定量评估并综合结果迭代指导搜索。

Result: 在多维常微分方程基准测试上，DoLQ相比现有方法实现了更高的成功率和更准确地恢复真实方程的符号项。

Conclusion: DoLQ通过结合LLM的定性评估与定量指标，有效提升了微分方程发现的质量与物理合理性。
...
### [257] [Tools as Continuous Flow for Evolving Agentic Reasoning](https://arxiv.org/abs/2605.07339)
*Tairan Huang,Siyu Shang,Qiang Chen,Xiu Su,Yi Chen*

Main category: cs.AI

TL;DR: 提出FlowAgent框架，将工具链式调用重新概念化为语义空间中的连续轨迹生成，利用条件流匹配生成连续潜在轨迹，实现全局规划视角，在长时推理任务中表现出更强的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM工具编排方法采用逐步范式，缺乏全局视角，导致长时推理中错误累积，且难以泛化到未见工具。

Method: 提出FlowAgent框架，利用条件流匹配在语义空间中生成连续潜在轨迹，将工具链调用转化为连续轨迹生成问题，提供全局规划视角。

Result: 实验表明FlowAgent在长时推理任务中具有优越的鲁棒性和适应性，理论上建立了效用收敛的形式化界，证明了连续范式在泛化和误差衰减方面的优势。

Conclusion: 连续轨迹生成范式能为LLM工具编排提供全局视角，有效缓解错误累积并提升对未见工具的泛化能力。
...
### [258] [Confidence-Aware Alignment Makes Reasoning LLMs More Reliable](https://arxiv.org/abs/2605.07353)
*Kejia Chen,Jiawen Zhang,Yihong Wu,Kewei Gao,Jian Lou,Zunlei Feng,Mingli Song,Ruoxi Jia*

Main category: cs.AI

TL;DR: CASPO通过置信度感知的逐步偏好优化来对齐token级置信度与逻辑正确性，无需训练独立奖励模型；结合CaT推理策略动态剪枝不确定推理分支，在多个基准上提升了推理可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型常常通过有缺陷的中间步骤得出正确结果，导致最终准确率与推理可靠性之间存在差距。现有对齐策略依赖外部验证器或大规模采样，可扩展性受限。

Method: 提出CASPO框架，通过迭代直接偏好优化（DPO）对齐token级置信度与逐步逻辑正确性，无需独立奖励模型；推理时提出Confidence-aware Thought（CaT），利用校准后的置信度动态剪枝不确定推理分支，仅增加O(V)延迟。

Result: 在十个基准和多模型族上，CASPO持续提升了推理可靠性和推理效率；可扩展到Qwen3-8B-Base，在AIME'24和AIME'25上超越树搜索基线且无需奖励模型数据。

Conclusion: CASPO有效弥合了最终准确率与推理可靠性之间的差距，提供了一种可扩展的对齐方法，同时提升了推理效率。
...
### [259] [GraphReAct: Reasoning and Acting for Multi-step Graph Inference](https://arxiv.org/abs/2605.07357)
*Xingtong Yu,Zhongwei Kuai,Chang Zhou,Xuanting Xie,Renhe Jiang,Xikun Zhang,Hong Cheng,Xinming Zhang,Yuan Fang*

Main category: cs.AI

TL;DR: GraphReAct是一个图推理-行动框架，通过交替进行推理与检索/精炼动作，实现了对图结构数据的逐步推理，在六个基准数据集上持续优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的推理-行动框架（如ReAct）在文本和代码领域已取得显著成效，但将其扩展到图学习领域的研究尚不充分。图数据具有固有的结构特性，信息分布在节点和边上，需要通过拓扑结构和潜在表示双重编码，因此需要能够逐步推理并动态精炼上下文的框架。

Method: 提出GraphReAct框架，设计了三类动作：(1) 拓扑检索——捕获局部结构依赖；(2) 语义检索——在表示空间中获取非局部的相关证据；(3) 上下文精炼——将累积信息蒸馏并重组为紧凑表示。通过推理与检索、精炼动作的交替，实现从上下文扩展到压缩的渐进式转变。

Result: 在六个基准数据集上的实验表明，GraphReAct一致性地优于最先进的方法，验证了推理-行动范式在图学习中的有效性。

Conclusion: 将推理-行动框架扩展到图结构数据是可行且有效的，通过设计拓扑检索、语义检索和上下文精炼三类互补动作，能够实现图上的逐步推理并显著提升图学习性能。
...
### [260] [Offline Policy Optimization with Posterior Sampling](https://arxiv.org/abs/2605.07393)
*Hongqiang Lin,Dongxu Zhang,Yiding Sun,Mingzhe Li,Ning Yang,Haijun Zhang*

Main category: cs.AI

TL;DR: 提出了一种基于后验采样的策略优化方法（PSPO），通过贝叶斯推断量化模型保真度，在离线强化学习中平衡泛化性与鲁棒性，理论证明了收敛性并实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线强化学习方法在分布外（OOD）区域采用过度悲观的正则化，虽保证了鲁棒性但牺牲了泛化能力，亟需一种能同时兼顾两者的建模方法。

Method: 将动力学建模形式化为贝叶斯推断过程，通过后验采样显式量化模型保真度，并结合约束策略优化，利用动力学一致的OOD转换实现泛化，同时确保对模型利用的鲁棒性。

Result: 在标准基准测试中，PSPO相比现有最先进基线方法取得了更优的性能。

Conclusion: 通过后验采样与约束优化相结合，可以有效解决离线强化学习中泛化与鲁棒性的权衡问题，理论收敛性分析和实验均验证了方法的有效性。
...
### [261] [Bounded Fitting for Expressive Description Logics](https://arxiv.org/abs/2605.07452)
*Maurice Funk,Jean Christoph Jung,Tom Voellmer*

Main category: cs.AI

TL;DR: 本文研究了在扩展了逆角色、限定数量限制和特征比较的 expressive 描述逻辑中，有界拟合（bounded fitting）这一学习范式是否仍能保持其良好的理论性质，并通过SAT求解器实现，实验结果表明该方法在实际 expressive 概念学习中具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 有界拟合在ALC描述逻辑的概念学习任务中已展现出PAC风格泛化保证和可借助SAT求解器实现的优势，但能否将其推广到更具表达力的描述逻辑（含逆角色、限定数量限制、特征比较）尚不清楚，因此需要研究其理论性质并验证实际可行性。

Method: 首先理论分析有界拟合在扩展描述逻辑中保持良好性质的条件，然后基于SAT求解器实现该学习框架，并与当前最先进的概念学习工具进行实验对比。

Result: 实验结果表明，所实现的有界拟合工具与最先进的概念学习器相比取得了令人鼓舞的结果，证明了该方法在 expressive 概念学习中的实用性。

Conclusion: 有界拟合可以有效地扩展到表达力更强的描述逻辑中，不仅保持了理论优势，而且通过SAT求解器实现后在实践中也表现良好，是一种实用的 expressive 概念学习方法。
...
### [262] [Model-Driven Policy Optimization in Differentiable Simulators via Stochastic Exploration](https://arxiv.org/abs/2605.07520)
*Yuval Aroosh,Ayal Taitler*

Main category: cs.AI

TL;DR: 本文提出MDPO框架，通过在可微规划中引入自适应随机噪声进行探索，有效改善了高度非线性和混合离散-连续决策问题中的优化困境。


<details>
  <summary>Details</summary>
Motivation: 可微规划在高度非线性和混合离散-连续领域中，优化景观往往病态严重，存在平坦区域和尖锐过渡，阻碍有效优化。

Method: 在可微规划中向动作空间注入噪声进行随机探索，并利用梯度导出的轨迹目标敏感性自适应调整噪声幅度，形成随时间变化的探索策略。

Result: 在基准测试中，MDPO在解质量上持续优于确定性可微规划方法和PPO等无模型基线方法。

Conclusion: 自适应噪声机制能在时间步和优化迭代间动态分配探索，有效帮助逃离不良局部最优。
...
### [263] [From Feasible to Practical: Pareto-Optimal Synthesis Planning](https://arxiv.org/abs/2605.07521)
*Friedrich Hastedt,Dongda Zhang,Antonio del Rio Chanona*

Main category: cs.AI

TL;DR: MORetro* 是一种多目标逆合成规划算法，通过生成帕累托前沿，在多个竞争目标（如成本、可持续性、毒性、产率）之间实现平衡，并保证最优性。


<details>
  <summary>Details</summary>
Motivation: 现有CASP方法仅追求单一可行路线（如最短路径），忽视了化学实践中需要权衡多个竞争目标（成本、可持续性、毒性、产率等）的现实需求。

Method: 将逆合成规划建模为多目标搜索问题，提出MORetro*算法，采用加权标量化和贝叶斯优化（BO）采样来高效导航组合搜索空间，基于多目标A*搜索并给出最优性保证。

Result: 在多个逆合成基准上，MORetro*生成多样且高质量的帕累托前沿，发现了单目标方法忽略的解决方案，与工业决策更好地对齐。

Conclusion: 多目标视角能显著提升逆合成规划的实际适用性，MORetro*为化学家提供了可权衡的合成路线集合，而非单一最优解。
...
### [264] [Multi-Environment POMDPs with Finite-Horizon Objectives](https://arxiv.org/abs/2605.07537)
*Léonard Brice,Filip Cano,Krishnendu Chatterjee,Thomas A. Henzinger,Stefanie Muroya*

Main category: cs.AI

TL;DR: 本文研究了多环境POMDP（MEPOMDP）中有限时域目标下的最优值和策略计算问题，证明了其PSPACE完全性，并提出了一种显著优于现有算法的实用算法。


<details>
  <summary>Details</summary>
Motivation: 在POMDP中，初始状态未知且由对手选择（即MEPOMDP）是一种更一般的设置，现有研究对这类问题的计算复杂性及有效算法尚不充分。

Method: 作者首先证明了MEPOMDP中有限时域目标下的最优值和策略计算问题为PSPACE完全；随后提出了一种实用算法，并在经典基准上进行了评估。

Result: 该问题在MEPOMDP中同样是PSPACE完全的；所提出的算法在经典基准测试上显著优于先前唯一的已知算法。

Conclusion: MEPOMDP的有限时域最优决策问题具有与POMDP相同的计算复杂度，且所提算法具有实际可行性。
...
### [265] [From Pixels to Prompts: Vision-Language Models](https://arxiv.org/abs/2605.07544)
*Khang Hoang Nhat Vo*

Main category: cs.AI

TL;DR: 本书旨在为读者提供关于视觉-语言模型（Vision-Language Models）的清晰认知地图，帮助理解这个快速发展领域中的核心概念，而不只是罗列各种模型名称和数据集。


<details>
  <summary>Details</summary>
Motivation: 作者观察到视觉-语言模型领域发展迅速，新模型层出不穷，导致读者容易在术语和概念中迷失方向，理论与实践之间存在较大鸿沟。

Method: 本书不追求穷举所有数据集、基准和新模型变体，而是提供一套结构化的思维框架和直观理解，帮助读者建立对视觉-语言模型的系统性认知。

Result: 为读者提供足以自信阅读新论文的结构化知识，以及设计自己系统所需的直觉能力。

Conclusion: 作者希望提供一种比单纯罗列新模型更加持久的价值——一个清晰的认知地图，帮助读者在快速变化的领域中不迷失方向。
...
### [266] [Open-Ended Task Discovery via Bayesian Optimization](https://arxiv.org/abs/2605.07572)
*Masaki Adachi,Yuta Suzuki,Juliusz Ziomek*

Main category: cs.AI

TL;DR: 提出GSR框架，在贝叶斯优化中动态生成和优化任务，仅需对数级后悔开销即可收敛到最佳任务。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化忽略了任务本身的不确定性（优化什么、如何评估），这些在实际科学工作流中会随证据积累而演化。

Method: GSR框架交替执行任务生成（粗到细）与任务优化，通过任务采集函数调度优化过程。

Result: 在新产品开发、化学合成缩放、算法分析和专利再利用等任务上，GSR优于现有基于LLM的优化器。

Conclusion: GSR能够自动演化优化任务，渐进收敛到最佳任务，且后悔开销仅为对数级别。
...
### [267] [Parallel Lifted Planning via Semi-Naive Datalog Evaluation](https://arxiv.org/abs/2605.07584)
*Dominik Drexler,Oliver Joergensen,Jendrik Seipp*

Main category: cs.AI

TL;DR: 本文提出了一种基于Datalog的两层并行执行模型（规则级并行和实例化并行），用于提升提升式经典规划器的效率。实验表明，该模型在单核上已优于基线，随核心数增加优势扩大，在难实例化任务上并行效率达92.4%，8核加速比达6倍。


<details>
  <summary>Details</summary>
Motivation: 提升式经典规划器避免了计算昂贵的实例化步骤，但通常运行较慢。为了加速提升式规划中的核心组件（如后继生成、公理评估、任务实例化和删除松弛启发式），需要更高效的并行执行模型。

Method: 基于Datalog评估方法，设计了两个层次的并行执行模型：规则级并行和实例化并行。针对规划特定工作负载，提出基于团枚举的实例化器，并扩展支持半朴素Datalog评估。

Result: 使用贪婪最佳优先搜索和FF启发式进行实验，单核上已比基线解决更多任务；多核时差距进一步扩大。在难实例化任务中，平均97.6%的运行时间用于Datalog执行，并行效率达92.4%，8核上实现最高6倍加速比。

Conclusion: 所提出的两层并行Datalog执行模型能有效加速提升式经典规划器，尤其在难实例化任务上效果显著，并行扩展性良好。
...
### [268] [Inference Time Causal Probing in LLMs](https://arxiv.org/abs/2605.07631)
*Sadegh Khorasani,Saber Salehkaleybar,Negar Kiyavash,Matthias Grossglauser*

Main category: cs.AI

TL;DR: 提出了一种无探针的梯度驱动因果干预方法HDMI，通过直接利用模型原生输出修改隐藏状态，在多个基准上实现了比现有方法更高的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有因果探测方法依赖辅助探针分类器定义干预，这导致方法绑定于特定任务或模型，且可能与模型的预测几何结构错位，影响干预的准确性。

Method: HDMI是一种无探针、基于梯度的隐藏状态干预方法，通过边际目标直接增大目标续写概率、减小源续写概率来驱动隐藏状态变化；LA-HDMI变体通过软max嵌入反向传播实现文本编辑。评估采用完整性（目标属性变化）与选择性（无关属性保持）的调和均值衡量可靠性。

Result: 在LGD一致性语料库和CausalGym基准上，HDMI在Meta-Llama-3-8B-Instruct和Pythia-70M模型中均一致优于先前方法，取得更高的可靠性得分。

Conclusion: HDMI作为一种无探针的因果干预方法，有效避免了现有方法的错位问题，在多个模型和任务上表现出更可靠的因果干预效果。
...
### [269] [Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding](https://arxiv.org/abs/2605.07637)
*Valeriy Vyaltsev,Alsu Sagirova,Anton Andreychuk,Yuri Kuratov,Konstantin Yakovlev,Aleksandr Panov,Alexey Skrynnik*

Main category: cs.AI

TL;DR: 本文提出LC-MAPF，一种基于局部通信的预训练模型，通过多轮邻接智能体间信息交换提升多智能体路径规划中的协作能力，在多种未见测试场景中优于现有学习方法，且不牺牲可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有学习型MAPF求解器多为分散式且性能有限，而通信式方法常面临可扩展性瓶颈。本文旨在通过引入高效的局部通信模块来增强智能体间的协作，同时保持模型的可扩展性。

Method: 将MAPF建模为Dec-POMDP，基于强化/模仿学习框架，额外引入可学习的局部通信模块，支持多轮邻接智能体间的特征共享与信息交换，形成预训练模型LC-MAPF。

Result: 在多种未见测试场景中，LC-MAPF在多项指标上优于现有IL和RL基线的学习型MAPF求解器，且通信机制未损害可扩展性。

Conclusion: 局部通信机制能有效提升多智能体路径规划中的协作性能，且通过合理设计可避免通信带来的可扩展性瓶颈，是一种有前景的改进方向。
...
### [270] [Tacit Knowledge Extraction via Logic Augmented Generation and Active Inference](https://arxiv.org/abs/2605.07639)
...
TL;DR: 本文提出了一种神经符号框架，融合逻辑增强生成与主动推理方法，用于构建本体驱动的知识图谱，以解决程序性领域中隐性知识的捕捉与形式化问题。在制造业装配维修视频案例中验证了该方法在完整性和语义质量上的提升。


<details>
  <summary>Details</summary>
Motivation: 隐性知识在人类专家经验中至关重要，但难以被捕获、形式化和复用。现有知识工程流程无法将隐性的、以流程为中心的知识转化为可查询、验证、推理和复用的机器可解释表示。

Method: 提出神经符号框架，结合逻辑增强生成（Logic-Augmented Generation）和基于主动推理（Active-Inference）的方法，构建本体驱动的知识图谱。

Result: 在制造业装配维修流程的教学视频案例研究中，该方法在知识表示的完整性和语义质量方面均有提升。

Conclusion: 该工作推动了神经符号知识工程在工业领域的应用，能够更好地捕捉和复用程序性知识。
...
### [271] [FactoryBench: Evaluating Industrial Machine Understanding](https://arxiv.org/abs/2605.07675)
...
TL;DR: 提出FactoryBench基准，用于评估时间序列模型和LLM对工业机器人遥测数据的理解能力。包含基于Pearl因果阶梯的4个因果层级、5种答案格式，构建了70k+问答对。评估6个前沿LLM，发现结构化层级准确率不超过50%，决策层级不超过18%，表明当前模型与工业机器理解之间存在巨大差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对工业机器人遥测数据中机器理解的系统评估，特别是因果推理能力。需要构建一个覆盖因果层级、支持多种答案格式的大规模基准来评估和推动模型在工业场景中的理解能力。

Method: 1) 基于Pearl因果阶梯构建4个因果层级（状态、干预、反事实、决策）的问答对；2) 设计5种答案格式（4种结构化格式确定性评分 + 自由格式由LLM投票评分）；3) 提出基于结构化模板的可扩展问答生成框架；4) 收集FactoryWave多任务传感器数据集（UR3和KUKA KR10机器人）；5) 从FactoryWave、AURSAD、voraus-AD构建70k+问答对。

Result: 6个前沿LLM在零样本评估中，结构化层级准确率不超过50%，决策层级不超过18%。当前模型与工业机器理解之间存在显著差距。

Conclusion: 现有LLM在工业机器人遥测理解任务上表现不足，特别是在因果推理和决策层面，需要进一步研究提升模型的机器理解能力。
...
### [272] [GASim: A Graph-Accelerated Hybrid Framework for Social Simulation](https://arxiv.org/abs/2605.07692)
*Xuan Zhou,Yanhui Sun,Hantao Yao,Allen He,Yongdong Zhang,Wu Liu*

Main category: cs.AI

TL;DR: GASim是一个图加速的混合多智能体框架，通过图优化记忆(GOM)和图消息传递(GMP)替代传统LLM检索和ABM串行执行，实现大规模社交模拟9.94倍加速，token消耗降低80%以上，同时保持与现实舆论趋势的高度一致。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模社交模拟方法将LLM智能体与数值ABM结合，存在高延迟问题，原因是昂贵的记忆检索和串行ABM执行。

Method: 提出GASim框架：1) 图优化记忆(GOM)用轻量级稀疏图传播替代LLM检索；2) 图消息传递(GMP)用并行更新替代串行ABM执行；3) 熵驱动分组(EDG)利用信息熵动态识别核心智能体。

Result: 端到端加速9.94倍，token消耗低于基线20%，与现实舆论趋势高度一致。

Conclusion: GASim通过图加速策略有效解决大规模社交模拟的延迟和成本问题，在保持模拟质量的同时显著提升效率。
...
### [273] [Finite-Time Analysis of MCTS in Continuous POMDP Planning](https://arxiv.org/abs/2605.07703)
...
<details>
  <summary>Details</summary>
Motivation: 现有的MCTS类求解器（如POMCP）虽在实践中表现优异，但由于启发式动作选择（如UCB）带来的非平稳性和相互依赖性问题，其严格的有限时间保证仍是一个悬而未决的问题。

Method: 在离散观测空间中将多项式探索奖励扩展到UCB以获得浓度界；在连续观测空间中引入抽象划分框架并推导划分损失的有限时间界，提出Voro-POMCPOW算法，利用Voronoi单元自适应划分连续观测空间以保持有限分支因子。

Result: 证明了在连续观测空间POMDP中价值估计的高概率界，实验表明Voro-POMCPOW在提供理论保证的同时具有竞争力的性能。

Conclusion: 该分析填补了POMDP中MCTS有限时间保证的空白，且所提出技术同样适用于连续MDP，缩小了MDP侧的理论差距。
...
### [274] [Hierarchical Task Network Planning with LLM-Generated Heuristics](https://arxiv.org/abs/2605.07707)
*Felipe Meneguzzi,Alexandre Buchweitz,Augusto B. Corrêa,Victor Scherer Putrich,André Grahl Pereira*

Main category: cs.AI

TL;DR: 本文研究利用大语言模型(LLM)为HTN规划生成搜索启发式函数，实验表明LLM生成的启发式在覆盖度上接近最佳HTN规划器，同时在83%的共同问题上显著减少了搜索开销。


<details>
  <summary>Details</summary>
Motivation: HTN规划中的现有启发式函数不如经典规划中的启发式信息丰富，需要探索更有效的启发式生成方法。

Method: 将Corrêa等人(2025)的方法从经典规划扩展到层次化规划，使用Pytrich规划器在6个标准全序HTN基准领域上，评估9种LLM在领域特定提示下生成的启发式，并与TDG、LMCount基线及PANDA规划器进行比较。

Result: LLM生成的启发式在覆盖度上几乎与最佳HTN规划器相当，并且在83%的共同问题上显著减少了搜索开销。

Conclusion: LLM能够为HTN规划生成有效的搜索启发式，虽然覆盖度尚未超越最佳规划器，但大幅降低了搜索代价，展示了LLM在层次化规划中辅助启发式设计的潜力。
...
### [275] [Online Goal Recognition using Path Signature and Dynamic Time Warping](https://arxiv.org/abs/2605.07736)
*Douglas Tesch,Nathan Gavenski,Leonardo Amado,Odinaldo Rodrigues,Felipe Meneguzzi*

Main category: cs.AI

TL;DR: 本文提出了一种利用路径签名（path signatures）的在线目标识别方法，该方法能紧凑且富有表现力地表示轨迹，在预测准确性和在线规划效率上优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 在线连续域中的目标识别面临两个核心挑战：高效编码长轨迹和有效比较轨迹。现有方法使用定制的状态空间表示和度量，忽视了其他领域已成熟的编码技术。

Method: 引入路径签名（path signatures）这一来自粗糙路径理论的紧凑表示方法，用于编码轨迹的关键语义特征，从而实现更有意义的轨迹比较。

Result: 实验表明，该方法在预测准确性和在线规划效率上持续优于现有技术，并在离线场景下保持竞争力。

Conclusion: 路径签名是一种有效的在线目标识别工具，在性能和效率上均具有优势。
...
### [276] [Alternating Target-Path Planning for Scalable Multi-Agent Coordination](https://arxiv.org/abs/2605.07744)
*Yu Kumagai,Keisuke Okumura*

Main category: cs.AI

TL;DR: 本文提出了一种迭代优化框架，将目标分配与路径规划解耦，利用快速MAPF求解器（如LaCAM）替代传统的CBS方法，显著提升了TAPF问题的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有TAPF方法依赖CBS，将目标分配和路径规划紧密耦合，计算量大、可扩展性差，难以应用于大规模实际问题。

Method: 提出迭代优化框架：在给定时间预算内，反复使用快速MAPF求解器（如LaCAM）求解当前分配方案，通过MAPF反馈识别瓶颈智能体，然后优化目标分配，形成反馈驱动的重分配循环。

Result: 实验结果表明，该反馈驱动的重分配循环非常有效，框架的可扩展性远超基于CBS的最先进求解器，同时保持不错的解质量。

Conclusion: 该工作为面向实际场景的大规模TAPF问题提供了可行方案，是迈向实用化的重要一步。
...
### [277] [RuleSafe-VL: Evaluating Rule-Conditioned Decision Reasoning in Vision-Language Content Moderation](https://arxiv.org/abs/2605.07760)
*Zhifeng Lu,Dianyuan Wang,Yuhu Shang,Zhenbo Xu*

Main category: cs.AI

TL;DR: 本文提出RuleSafe-VL基准，用于评估视觉语言模型在内容审核中基于规则条件的决策推理能力，揭示了现有模型在规则关系恢复和决策状态预测上的严重瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前多模态安全基准仅将审核简化为匹配预设标签，无法检验模型是否真正依据政策规则进行推理，高分可能来自表面线索而非正确应用规则。

Method: 从公开平台审核政策中提取93条原子规则和92种规则关系，构建2,166个图像-文本案例；设计四项诊断任务：识别激活规则、恢复规则交互、判断决策充分性、补全上下文后解决结果。

Result: 10个前沿VLM中，最佳模型规则关系恢复仅达64.8 Macro-F1，部分安全导向模型低于7 Macro-F1；决策状态预测最高64.5 Macro-F1。

Conclusion: 规则关系恢复是主要瓶颈，RuleSafe-VL将审核评估从最终标签评分转向规则条件决策推理的诊断性评估。
...
### [278] [Exact Regular-Constrained Variable-Order Markov Generation via Sparse Context-State Belief Propagation](https://arxiv.org/abs/2605.07839)
*François Pachet*

Main category: cs.AI

TL;DR: 将变阶马尔可夫模型与正则约束通过状态空间乘积图结合，实现了在不展开为全K元组条件下的精确推理，并支持可逆数据增强。


<details>
  <summary>Details</summary>
Motivation: 现有方法已能处理一阶马尔可夫链上的正则约束，但变阶/回退模型因历史合并机制与一阶约束层存在失配，导致未来质量计算错误。需要一种能正确处理变阶模型与正则约束结合的精确推理方法。

Method: 用观测到的上下文状态替代一阶马尔可夫状态，再与正则约束自动机做标准乘积，构造稀疏状态空间。推理复杂度在固定上下文图和自动机下关于序列视界线性，一般情况多项式于可达乘积边数。

Result: 无需展开为所有K元组即可得到正确的变阶分布条件于正则约束；支持通过逆计数查找实现可逆数据增强，匹配物化转置增强而不需存储转换语料库。

Conclusion: 给出了变阶马尔可夫模型在正则约束下的精确推理框架，并明确指出若要声称精确性，必须显式定义生成时的回退策略（如避免单例）的随机语义。
...
### [279] [AgentEscapeBench: Evaluating Out-of-Domain Tool-Grounded Reasoning in LLM Agents](https://arxiv.org/abs/2605.07926)
*Zhengkang Guo,Yiyang Li,Lin Qiu,Xiaohua Wang,Jingwen Xv,Dongyu Ru,Xiaoyu Li,Xiaoqing Zheng,Xuezhi Cao,Xunliang Cai*

Main category: cs.AI

TL;DR: AgentEscapeBench是一个基于密室逃脱风格的基准测试，用于评估LLM智能体在长期依赖约束下进行工具推理的能力。实验发现，随着依赖深度增加，模型性能显著下降，主要失败原因包括长期状态追踪、线索遵循和中间结果传播的断裂。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体越来越依赖外部工具，但现有评估多局限于短程交互和熟悉的工作流程，缺乏对长程工具推理能力的系统评测。

Method: 构建了包含270个实例、5个难度等级的基准测试，每个任务定义有向无环依赖图，要求智能体调用真实外部函数、追踪隐藏状态、传播中间结果并提交可验证的最终答案。

Result: 在16个LLM智能体和人类参与者实验中，性能随依赖深度增加急剧下降：人类从难度5的98.3%降至难度25的80.0%，最佳模型从90.0%降至60.0%。

Conclusion: 当前智能体能够处理局部工具使用，但在深度上下文依赖方面仍存在明显困难，AgentEscapeBench可作为诊断智能体能力的测试平台。
...
### [280] [TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples](https://arxiv.org/abs/2605.07935)
*Shuren Xia,Qiwei Li,Taqiya Ehsan,Jorge Ortiz*

Main category: cs.AI

TL;DR: 提出TraceFix，一种面向LLM多智能体协调的“验证优先”流程，通过TLA+模型检查器验证协议正确性，并基于验证结果编译为多智能体系统提示执行。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体协调中缺乏形式化验证保障，易出现死锁、活锁等协调错误，现有方法依赖提示工程或对话修复，缺乏正确性保证。

Method: 1) 从任务描述合成协议拓扑（结构化中间表示）；2) 生成PlusCal协调逻辑；3) 使用TLA+模型检查器(TLC)迭代修复直至验证通过；4) 将验证后的进程体编译为各智能体系统提示；5) 运行时监控器拒绝越界协调操作。

Result: 48个任务全部通过TLC验证，62.5%一次通过，最多4次迭代修复；状态空间跨6个数量级，验证均在60秒内完成；3,456次运行时对比显示拓扑监控执行达最高任务完成率(平均89.4%, 完全81.5%)；验证协议在模型能力下降时退化率为基线的一半；TLC验证将死锁/活锁从31.1%降至14.1%。

Conclusion: 验证优先的LLM多智能体协调框架能有效保证协议正确性，显著降低协调错误，提升任务完成率，且对模型能力退化具有更强鲁棒性。
...
### [281] [The Limits of AI-Driven Allocation: Optimal Screening under Aleatoric Uncertainty](https://arxiv.org/abs/2605.07979)
*Santiago Cortes-Gomez,Mateo Dulce Rubio,Carlos Patino,Bryan Wilder*

Main category: cs.AI

TL;DR: 本文研究了在固定预算下，如何最优地结合传统筛查（直接观察）和基于机器学习的算法定位（预测风险评分）进行两阶段资源分配，发现最优策略是对算法分配边缘的单元进行筛查，而直接定位最高风险单元，并实证了筛查与算法定位的互补/替代关系。


<details>
  <summary>Details</summary>
Motivation: 机器学习推动的算法定位虽然比传统筛查更便宜、更快，但即使知道真实条件脆弱性概率也无法消除分配错误（由于个体脆弱性状态的偶然不确定性不可约），因此需要研究如何最优结合两种方法以提高分配效率。

Method: 构建两阶段分配框架：第一阶段（筛查）观察部分单元的真实结果，第二阶段（最终分配）在固定覆盖预算下分配资源。理论推导最优策略，并在哥伦比亚的收入型社会保护项目和扫雷行动中进行实证应用。

Result: 最优策略是对算法分配边缘的单元进行筛查，同时直接定位最高风险单元；效率提升随人群中偶然不确定性增加而增长；筛查与算法定位在特定条件下呈现互补或替代关系。

Conclusion: 在资源分配中，应策略性地将传统筛查与算法定位相结合，筛查应聚焦于算法决策边界附近的单元，从而在控制成本的同时提升分配效率。
...
### [282] [Abductive Reasoning with Probabilistic Commonsense](https://arxiv.org/abs/2605.08011)
*Joseph Cotnareanu,Chiara Roverato,Han Zhou,Didier Chetelat,Yingxue Zhang,Mark Coates*

Main category: cs.AI

TL;DR: 本文提出PACS算法，通过概率框架建模常识信念的个体差异，结合LLM与形式化求解器进行溯因常识推理，在多个基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号方法假设常识信念具有普遍一致性，但现实中常识信念因人而异。需要一种能显式建模这种个体差异的推理框架。

Method: 提出PACS（Probabilistic Abductive CommonSense）算法，使用LLM和形式化求解器采样证明作为不同个体常识信念的观测，并聚合这些样本的结论。

Result: PACS在多个基准上优于思维链推理、先前的神经符号方法以及基于搜索的方法。

Conclusion: 通过概率化建模常识信念的个体差异，能够有效提升LLM在溯因常识推理任务上的表现。
...
### [283] [Learning CLI Agents with Structured Action Credit under Selective Observation](https://arxiv.org/abs/2605.08013)
*Haoyang Su,Ying Wen*

Main category: cs.AI

TL;DR: 本文提出σ-Reveal（选择性观测机制）和A³（动作优势分配方法）来提升CLI智能体在代码库环境中的信息提取与文件编辑能力，并构建了ShellOps数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 现有CLI智能体工作少有关注CLI动作的结构化属性，且面临两大瓶颈：从大型代码库中识别任务相关证据的观测选择问题，以及稀疏终端奖励在长轨迹中的信用分配问题。

Method: 提出σ-Reveal用于推理时选择性观测（token预算上下文选择）；提出A³（Action Advantage Assignment）方法，利用回合级相对反馈、AST动作子链残差和轨迹级边距构建回合级优势函数，保留标准智能体RL的算法复杂度。

Result: 构建了ShellOps数据集（涵盖仓库环境中的CLI任务），并通过实验验证了所提方法在该问题设置下的有效性。

Conclusion: σ-Reveal和A³分别从观测选择和信用分配角度解决了CLI智能体学习的核心瓶颈，为CLI智能体强化学习提供了新的技术方案。
...
### [284] [Reason to Play: Behavioral and Brain Alignment Between Frontier LRMs and Human Game Learners](https://arxiv.org/abs/2605.08019)
...
TL;DR: 本文通过人类玩游戏时的大脑活动数据，评估了前沿大型推理模型（LRMs）在复杂环境中的学习和规划能力，发现LRMs在匹配人类行为模式和预测脑活动方面显著优于深度强化学习和贝叶斯理论模型，且脑对齐源于模型的上下文游戏状态表征而非下游推理过程。


<details>
  <summary>Details</summary>
Motivation: 探索现代AI系统能否像人类一样在陌生环境中快速学习抽象知识并灵活运用以指导高效行动，特别是评估前沿大型推理模型（LRMs）在这方面的能力。

Method: 使用包含同步fMRI记录的人类复杂游戏数据集，让参与者学习需要规则发现、假设修正和多步规划的新视频游戏。将前沿LRMs与无模型、基于模型的深度强化学习智能体以及贝叶斯理论智能体进行对比，从游戏表现、匹配人类学习行为和预测脑活动三个维度评估模型。

Result: 前沿LRMs最接近人类在游戏发现过程中的行为模式，且预测脑活动的效果比强化学习替代方案高出一个数量级，涵盖皮层和皮层下区域，效果经置换检验稳健；脑对齐源于模型对游戏状态的上下文表征而非下游规划或推理。

Conclusion: LRMs可作为复杂自然环境中人类学习和决策的令人信服的计算模型。
...
### [285] [VecCISC: Improving Confidence-Informed Self-Consistency with Reasoning Trace Clustering and Candidate Answer Selection](https://arxiv.org/abs/2605.08070)
*James Petullo,Sonny George,Dylan Cashman,Nianwen Xue*

Main category: cs.AI

TL;DR: VecCISC通过语义相似度过滤推理轨迹，在保持加权多数投票精度的同时，将总token使用量减少47%。


<details>
  <summary>Details</summary>
Motivation: 加权多数投票（如CISC）虽能提升推理准确性，但需要额外调用评判模型为每个候选答案生成置信度分数，导致高昂的开销和成本。

Method: 提出VecCISC框架，利用语义相似度度量过滤语义等价、退化或幻觉的推理轨迹，减少需要评判模型评估的候选答案数量。

Result: 在数学、化学、生物学、常识推理和人文学科五个数据集上，VecCISC将总token使用量减少47%，同时保持或超越CISC的准确率。

Conclusion: VecCISC是一种轻量级、自适应的框架，能够有效降低加权多数投票的计算成本而不牺牲性能。
