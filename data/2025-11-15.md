...
Motivation: 大型视觉语言模型虽然能生成高质量视频描述，但其高内存、计算和部署需求阻碍了实际应用，特别是对依赖详细、上下文感知描述的盲人和低视力用户。

Method: 评估SmolVLM2的500M和2.2B参数变体在两个数据集上的表现，引入多上下文BLV框架和导航辅助框架，系统评估四种提示设计策略，并在智能手机上部署FP32和INT8精度变体。

Result: 通过专门设计的评估框架和不同参数规模模型的对比，为资源受限移动设备上的BLV辅助应用提供了性能约束评估。

Conclusion: 该研究为开发适用于盲人和低视力用户的轻量级视觉语言模型提供了重要指导，特别是在移动设备部署和可访问性评估方面。
...
TL;DR: 本文提出了自一致性采样（SCS）方法，用于解决多模态大语言模型在结果奖励强化学习中的不可靠轨迹问题，通过在视觉扰动和轨迹重采样中引入一致性评分来提升推理准确性。


<details>
  <summary>Details</summary>
Motivation: 在多模态推理基准测试的多选题设置中，结果奖励强化学习面临一个被忽视的障碍：即使推理链存在错误但最终猜对答案的轨迹与真正正确推理的轨迹获得相同奖励，这影响了学习效果。

Method: 提出自一致性采样（SCS）方法：对每个问题引入小的视觉扰动，并对初始轨迹进行重复截断和重采样；通过结果轨迹之间的一致性来产生可微的一致性评分，在策略更新时降低不可靠轨迹的权重。

Result: 在Qwen2.5-VL-7B-Instruct模型上，将SCS集成到RLOO、GRPO和REINFORCE++系列中，在六个多模态基准测试上准确率提升高达7.7个百分点，且计算开销可忽略。在Qwen2.5-VL-3B-Instruct和InternVL3-8B模型上也取得了显著提升。
...
Method: 引入编码器增强的因果解码器模型架构，展示如何基于每词元获得熵估计，并证明接近训练数据熵的模型必然比超出该值的模型具有更好的泛化能力。

Result: 该架构在适度硬件上训练时表现出更高的训练效率和压缩率，经验证明接近但不超出估计每词元熵的因果模型比不考虑熵的模型具有更好的泛化能力。

Conclusion: 编码器增强的因果解码器模型在训练效率和压缩率上优于传统因果变换器，且接近但不超出估计熵值的训练策略能显著提升模型泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时框架依赖粗略的自验证和自校正，限制了在复杂任务上的有效性，需要更精细的方法来提升LLM推理能力。
...
TL;DR: Instella是一个完全开源的30亿参数语言模型家族，在公开数据和代码库上训练，在AMD GPU上开发，尽管预训练token较少但在全开源模型中达到SOTA水平，并发布了支持128K上下文和数学推理的专门变体。


<details>
  <summary>Details</summary>
Motivation: 当前高性能语言模型大多闭源或部分开源，限制了透明度和可复现性，需要开发完全开源且性能优异的替代方案。

Method: 使用AMD Instinct MI300X GPU进行大规模预训练、通用指令微调和人类偏好对齐，并开发了支持长上下文和数学推理的专门变体。

Result: Instella在全开源模型中达到最先进水平，与同类规模的开源权重模型竞争，并发布了Instella-Long（128K上下文）和Instella-Math（数学推理）变体。
...
TL;DR: 本文提出生成对抗蒸馏（GAD）方法，通过将学生LLM作为生成器、训练判别器区分师生模型响应，实现黑盒蒸馏。该方法在LMSYS-Chat评估中使Qwen2.5-14B-Instruct学生模型性能接近GPT-5-Chat教师模型。
...
Method: 采用生成对抗框架，将学生LLM作为生成器，训练判别器区分师生模型响应，形成minimax博弈。判别器作为在线奖励模型与学生共同进化。

Result: GAD在多个实验中持续超越序列级知识蒸馏方法，Qwen2.5-14B-Instruct学生模型在LMSYS-Chat自动评估中性能接近GPT-5-Chat教师模型。

Conclusion: GAD被证明是一种有前景且有效的黑盒LLM蒸馏范式。
...
TL;DR: ParoQuant是一种仅对权重进行后训练量化的方法，通过成对旋转量化和通道级缩放来减少异常值影响，在推理任务上比AWQ平均提升2.4%准确率，同时保持低于10%的开销。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的权重和激活值存在异常值，导致量化误差大和准确率严重下降，特别是在推理任务中误差会在长思维链中累积。现有PTQ方法要么无法充分抑制异常值，要么在推理时引入显著开销。

Method: 结合硬件高效且可优化的独立Givens旋转与通道级缩放，均衡通道间幅度并缩小每个量化组内的动态范围，同时协同设计推理内核以充分利用GPU并行性。

Result: 在推理任务上比AWQ平均提升2.4%准确率，同时保持低于10%的开销。

Conclusion: 为更高效和准确部署推理型大型语言模型铺平了道路。
