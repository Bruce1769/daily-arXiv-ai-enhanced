...
TL;DR: 本文提出一个解决开放集识别中过自信问题的框架，通过参数扰动估计预测不确定性，并使用两阶段分类器增强已知类和未知类的区分能力。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类语义相似时，模型容易对未知样本赋予过高置信度，导致错误分类为已知类，这种现象称为过自信。过自信会模糊已知类和未知类之间的决策边界，影响开放集识别性能。

Method: 框架包含两个组件：基于扰动的预测不确定性估计模块（通过可控参数扰动生成多样化预测并量化预测不确定性）和具有不同学习分类器的未知检测模块（作为两阶段过程实现，利用估计的不确定性改进已知类和未知类的区分）。

Result: 在三个公共数据集上的实验结果表明，所提出的框架在开放集识别性能上优于现有方法。

Conclusion: 该框架通过显式缓解由类间重叠引起的过自信问题，有效提升了开放集识别的性能，特别是在处理语义相似的未知样本时表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测方法需要大量训练数据，而少样本视频目标检测面临时序一致性（遮挡和外观变化）和新对象泛化的挑战，现有方法依赖复杂区域提议且计算成本高。

Method: 采用对象感知时序建模方法，包含过滤机制选择性传播高置信度对象特征，结合少样本训练的检测和分类头，实现高效特征传播而不依赖显式对象管提议。

Result: 在5-shot设置下，AP提升分别为：FSVOD-500（3.7%）、FSYTV-40（5.3%）、VidOR（4.3%）和VidVRD（4.5%），在1-shot、3-shot和10-shot配置下也展示了改进。

Conclusion: 该方法通过对象感知时序建模有效解决了少样本视频目标检测的关键挑战，实现了显著的性能提升，代码已公开。
...
TL;DR: 本文提出了一种基于流匹配范式的多模态图像融合方法，通过直接概率传输从源模态到融合图像分布，提高采样效率和结构一致性。使用多模型融合结果作为先验，结合任务感知选择和融合精炼模块，在保持轻量级设计的同时实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法依赖任务特定模型，训练成本高且可扩展性有限。生成方法虽然提供统一建模视角，但采样轨迹复杂导致推理缓慢。需要一种高效且统一的融合方法。

Method: 采用流匹配范式进行直接概率传输，收集多模型融合结果作为先验，使用任务感知选择函数筛选可靠伪标签，引入融合精炼模块分解和增强退化组件，集成弹性权重巩固和经验回放机制处理多任务场景。

Result: 在多样化融合任务中实现竞争性性能，显著提高采样效率，保持轻量级模型设计。

Conclusion: 提出的方法通过流匹配范式有效解决了多模态图像融合中的效率和可扩展性问题，为统一融合建模提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 实时碰撞检测对于主动安全管理和提升交通效率至关重要，但传统方法受限于轨迹获取和车辆跟踪的局限性。

Method: 第一阶段使用Mapfusion扩散模型进行路段地图生成，通过噪声添加和去噪过程，结合时序嵌入和ControlNet背景上下文控制；第二阶段通过比较监控路段地图与扩散模型生成结果来检测碰撞。

Result: Mapfusion成功生成了基于学习运动模式的真实路段演化地图，在不同采样间隔下保持鲁棒性，真实世界碰撞实验验证了该方法的有效性。

Conclusion: 提出的两阶段方法能够准确检测碰撞，为无轨迹碰撞检测提供了有效解决方案。
...
TL;DR: 本文提出了一种针对地震数据的新型自适应双网格基础模型训练策略（ADATG），结合希尔伯特编码，专门处理地震数据的高频和低频特征，通过分层训练策略提升地震视觉基础模型的预训练效果。


<details>
  <summary>Details</summary>
Motivation: 由于地震数据具有独特的频率特征，现有的视觉变换器（ViTs）在处理地震数据时无法有效捕捉高频和低频信息，需要专门针对地震数据特性的处理方法。
...
TL;DR: 该论文提出了一种基于面部微动态分析的被动式痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变异性等面部时间运动学特征，实现无需语言或文本的早期神经认知变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本化访谈，限制了在临床外的应用，且预测结果与语言和转录紧密耦合。本研究旨在开发一种无需语言、可大规模应用的被动式筛查方法。

Method: 通过稳定面部信号，将面部微运动转换为可解释的面部微动态时间序列，平滑处理后将短窗口汇总为紧凑的片段级统计量用于筛查。每个窗口通过其活动混合（各运动流相对份额）进行编码。

Result: 在YT DemTalk数据集上，消融实验确定注视不稳定性和嘴部/下颌动态为最有信息量的线索，轻量级浅层分类器可实现AUROC 0.953、AP 0.961、F1-score 0.851和准确率0.857的痴呆预测性能。

Conclusion: 面部时间微动态分析为痴呆筛查提供了一种无需语言、可扩展的方法，能够在自然环境中捕捉面部行为，且可跨设备、主题和文化转移。
...
Conclusion: Gen-ViRe为推进真正的世界模拟器提供了定量评估框架，揭示了当前模型在视觉质量与推理能力之间的差距。
...
TL;DR: 本文提出基于排序的损失函数来解决基于热图的人体姿态估计中的三个主要问题：MSE损失无法有效提升关节定位精度、热图的空间和类别不平衡、以及损失函数与评估指标之间的不一致性。


<details>
  <summary>Details</summary>
Motivation: 传统的热图方法存在三个核心问题：(1) MSE损失对所有像素偏差平等惩罚，无法专注于锐化峰值；(2) 热图存在空间和类别不平衡；(3) 损失函数与mAP评估指标不一致。

Method: 提出基于排序的损失函数，通过理论分析和实验验证，这些损失函数能够提高置信度分数与定位质量之间的相关性，从而改善NMS过程中的实例选择和mAP性能。

Result: 在COCO-val数据集上，RSPose使用ViTPose-H模型达到79.9 mAP，超越之前最优方法；同时将SimCC Resnet-50在COCO-val上的AP提升了1.5，达到73.6 AP。

Conclusion: 提出的基于排序的损失函数是首个与mAP评估指标对齐的人体姿态估计损失函数，在多个数据集和模型上都取得了显著性能提升。
...
TL;DR: 提出碰撞声源分割任务(CS3)，从碰撞声音中识别并分割视频中产生声音的物体，使用弱监督方法和基础模型，在EPIC-CS3和Ego4D-CS3基准上性能显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，希望从碰撞声音中识别产生声音的物体，解决碰撞声音来自两个物体交互的独特挑战。

Method: 使用弱监督的音频条件分割方法，结合基础模型(CLIP和SAM2)和自我中心线索(手中物体)，识别潜在的碰撞声源物体。

Result: 在两个新基准EPIC-CS3和Ego4D-CS3上，mIoU分别比竞争基线高出3倍和4.7倍。

Conclusion: 提出的CS3任务和方法能有效从碰撞声音中分割声源物体，在自我中心视频场景中表现优异。
...
TL;DR: 本文提出了一种几何基础的绝对位姿回归方法，通过将问题重新表述为几何表示回归，从图像中直接回归3D几何表示，然后通过可微分求解器恢复6-DoF相机位姿。


<details>
  <summary>Details</summary>
Motivation: 传统绝对位姿回归方法作为黑盒直接回归6-DoF位姿，容易记忆训练视图而非理解3D场景几何。本文旨在引入几何先验，提供更鲁棒的位姿估计方法。

Method: 将绝对位姿回归重新表述为几何表示回归，显式预测两个解耦的几何表示：光线束方向估计相机旋转，对应点图估计相机平移，最后通过可微分求解器恢复位姿。

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程是更鲁棒的绝对位姿估计路径。

Conclusion: 通过解耦旋转和平移预测，引入强几何先验，几何表示回归方法在绝对位姿估计中表现出更好的泛化能力和鲁棒性。
...
TL;DR: 本文提出了H-CNN-ViT模型，用于膀胱癌术后复发的多序列MRI预测，通过分层门控注意力机制融合全局和局部特征，在自建数据集上达到78.6%的AUC。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使判读困难，且缺乏专门用于复发预测的多序列MRI数据集。

Method: 提出H-CNN-ViT分层门控注意力多分支模型，基于上下文需求选择性加权全局(ViT)和局部(CNN)路径特征，实现平衡的目标特征融合。多分支架构独立处理每个模态。

Result: 在自建数据集上评估，H-CNN-ViT达到78.6%的AUC，超越现有最先进模型。

Conclusion: 建立了首个专门用于膀胱癌复发预测的多序列多模态MRI数据集，提出的H-CNN-ViT模型在复发预测任务上表现优异，为未来研究提供了基准。
...
<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器仅支持77个token，限制了其对信息丰富的长放射学报告的表征能力。现有基于领域特定编码器的改进仍受限于512个token的输入长度和相对浅层的语义理解。

Method: 提出QwenCLIP框架，用LLM-based嵌入模块（如Qwen3-Embedding）替换CLIP的文本编码器，引入可学习提示来增强跨模态对齐，利用LLM的扩展上下文窗口和更丰富的表示能力。

Result: QwenCLIP能够从长格式临床文本中捕获全面的医学语义，显著改善了医学图像-文本对齐和放射学基准测试的下游性能。
...
TL;DR: 提出了一种结合EfficientNet和VGG19的混合模型架构，用于腰椎退变分类，通过伪牛顿提升层和稀疏诱导特征减少层提升特征选择和表示能力。


<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在高维医学图像中存在限制，无法充分利用解剖特征，需要克服这些约束以提高分类性能。

Method: 集成EfficientNet和VGG19，加入伪牛顿提升层进行特征权重智能调整，以及稀疏诱导层去除冗余特征，形成多层框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的自动驾驶方法依赖手工制作的提示，性能不稳定，限制了在实际场景中的鲁棒性和泛化能力。

Method: 利用视觉语言模型的强大场景理解能力为现有视觉基准添加结构化、语言丰富的场景描述，引入多模态交互架构融合视觉和语言特征，并设计后处理细化模块提升预测可靠性。
...
TL;DR: 本文研究了医学解剖分割中数据规模与性能的缩放关系，分析了15个语义任务和4种成像模态，发现更大数据集能显著提升分割性能。提出了基于图像配准的变形增强方法，有效提高了数据利用效率。


<details>
  <summary>Details</summary>
Motivation: 医学解剖分割领域的数据缩放关系研究不足，且医学图像具有解剖结构拓扑同构性，这为通过变形增强策略来改善数据缩放规律提供了动机。

Method: 评估了随机弹性变形和配准引导变形两种变形增强策略，并提出了一种新的可扩展图像增强方法，基于图像配准从测地子空间生成微分同胚映射来引入真实变形。

Result: 实验结果表明，配准和生成变形增强都能显著提高数据利用效率，提出的生成变形方法实现了更优性能和更快收敛，超越了标准幂律缩放趋势且无需额外数据。
...
TL;DR: Uni-Hema是一个多任务、统一化的数字血液病理学模型，整合了检测、分类、分割、形态预测和跨多种疾病的推理功能，在46个公开数据集上训练，性能优于单任务模型。
...
Method: 基于Hema-Former多模态模块构建，在46个公开数据集（超过70万图像和2.1万问答对）上训练，整合检测、分类、分割、形态预测、掩码语言建模和视觉问答等多种任务。

Result: 在多种血液学任务上，Uni-Hema实现了与单任务单数据集模型相当或更优的性能，并在单细胞层面提供可解释的形态学相关见解。

Conclusion: Uni-Hema为多任务多模态数字血液病理学建立了新标准，能够统一处理恶性疾病、感染性疾病和非恶性红细胞疾病等多种血液病理学问题。
...
TL;DR: 本文提出了首个弱监督管道用于检测短暂冲沟，利用视觉语言模型减少人工标注工作量，通过师生模型和噪声感知损失函数实现弱监督学习，并发布了首个半监督检测数据集。


<details>
  <summary>Details</summary>
Motivation: 解决短暂冲沟自动检测的挑战：短暂时间周期增加检测难度，准确标注数据稀缺且难以制作，现有机器学习方法仅限于难以实现的零样本方法。

Method: 使用遥感数据和视觉语言模型，利用VLM预训练知识，构建师生模型框架：教师从VLM的噪声标签中学习，学生通过教师生成的标签和噪声感知损失函数进行弱监督学习。

Result: 实验结果表明，该方法在弱监督训练学生模型时，相比VLM和标签模型本身表现出更优越的性能。

Conclusion: 该方法有效解决了短暂冲沟检测中的标注数据稀缺问题，为土壤侵蚀监测提供了可行的弱监督解决方案，相关代码和数据集已公开。
...
TL;DR: 本文提出了一种基于压缩视频流中运动向量的可扩展、模型无关的框架，用于评估生成视频的时间行为，通过计算真实视频和生成视频运动向量统计之间的差异来量化时间真实性，并展示了运动向量在改进下游分类任务中的有效性。
...
Method: 使用从H.264和HEVC等压缩视频标准中提取的运动向量作为轻量级、分辨率一致的运动动态描述符，通过计算Kullback-Leibler、Jensen-Shannon和Wasserstein散度来量化真实视频和生成视频之间的运动向量统计差异。

Result: 在GenVidBench数据集上的实验显示，基于熵的散度将Pika和SVD评为最接近真实视频，MV-sum统计量偏好VC2和Text2Video-Zero，而CogVideo在两种度量上都显示出最大偏差。运动向量与RGB融合显著提高了下游分类性能。

Conclusion: 压缩域运动向量为诊断生成视频中的运动缺陷和增强判别模型中的时间推理提供了有效的时间信号，是评估和改进视频生成模型时间真实性的有力工具。
...
TL;DR: SAE-MCVT是一个可扩展的实时多摄像头车辆跟踪框架，通过边缘设备处理视频流并传输轻量级元数据到中央工作站进行跨摄像头关联，在保持实时性能的同时实现城市规模部署。
...
Method: 系统包含多个边缘设备与中央工作站。边缘端处理RTSP视频流，包括目标检测、跟踪、地理映射和特征提取，仅传输车辆位置和深度外观特征等轻量元数据。中央端基于自监督摄像头链接模型学习相邻摄像头的时空关系进行跨摄像头关联。
...
<details>
  <summary>Details</summary>
Motivation: 道路安全规范对自动驾驶车辆的AI系统至关重要，需要评估多模态大语言模型对道路安全概念的理解能力。

Method: 使用从学校教科书收集的交通标志和道路安全规范图像构建数据集，在零样本设置下评估模型能力。

Result: 初步结果显示这些模型在安全推理方面表现不佳，存在人类学习与模型解释之间的性能差距。

Conclusion: 多模态大语言模型在道路安全概念理解方面存在显著差距，需要进一步研究来改进。
...
TL;DR: 论文提出CuRPO训练策略，通过课程学习方式利用CoT长度和gIoU奖励作为复杂度指标，从简单到复杂逐步训练，在视觉定位任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现基于强化学习的CoT推理在视觉定位任务中可能因CoT输出过长或复杂而降低性能，且增加数据集规模并不总能提升性能，因此需要更有效的训练策略。

Method: 提出Curriculum-based Relative Policy Optimization (CuRPO)，使用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，从简单到复杂逐步构建训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验表明，CuRPO显著优于现有方法，在RefCOCO上提升高达+12.52 mAP，在少样本学习场景下也表现出色。

Conclusion: CuRPO是一种高效且鲁棒的训练策略，特别适用于具有模糊和复杂文本描述的视觉定位任务，能够有效提升模型性能。
...
TL;DR: 提出基于聚类的帧选择策略，通过在数据集划分前将视觉相似的帧分组，减少视频帧数据集中的信息泄漏问题。


<details>
  <summary>Details</summary>
Motivation: 解决视频帧数据集中因时间连续性导致的信息泄漏问题，确保训练集、验证集和测试集之间的独立性。

Method: 使用聚类方法将视觉相似的帧分组，然后在组内进行数据集划分，确保不同集合中的帧来自不同的视觉模式。

Result: 该方法能够生成更具代表性、平衡性和可靠性的数据集划分，减少信息泄漏风险。

Conclusion: 基于聚类的帧选择策略是处理视频帧数据集信息泄漏问题的有效方法，能够提高模型评估的可靠性。
...
TL;DR: 本文提出通过在程序生成的数据上预训练视觉Transformer，绕过视觉补丁嵌入机制，让模型学习抽象计算先验，从而显著提高数据效率、收敛速度和下游性能。


<details>
  <summary>Details</summary>
Motivation: 探索为视觉Transformer注入通用归纳偏置的新方法，通过无视觉或语义内容的程序生成数据进行预训练，以提升模型的跨模态适应能力。

Method: 使用形式语法等简单算法生成程序数据，在ViTs的预热阶段进行预训练，绕过视觉补丁嵌入机制，然后进行标准的基于图像的训练。

Result: 在ImageNet-1k上，仅分配1%的训练预算给程序数据就能提高最终准确率超过1.7%，1%程序数据的效果相当于28%的ImageNet-1k数据。
...
TL;DR: EchoAgent是一个用于心脏超声视频分析的AI框架，通过大语言模型协调专用视觉工具，实现结构化、可解释的自动化分析，支持时间定位、空间测量和临床解释。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型无法支持心脏超声所需的视频级推理和基于指南的测量分析，因此需要开发能够进行结构化、可解释自动化分析的框架。

Method: EchoAgent在大语言模型控制下协调专用视觉工具，执行时间定位、空间测量和临床解释。关键贡献是测量可行性预测模型，用于确定解剖结构是否可可靠测量，从而实现自主工具选择。

Result: 尽管增加了时空视频分析的复杂性，EchoAgent仍能获得准确、可解释的结果。输出基于视觉证据和临床指南，支持透明度和可追溯性。

Conclusion: 这项工作证明了通过任务专用工具和全视频级自动化，实现基于指南的心脏超声视频分析的可行性，为心脏超声领域的可信AI设定了新方向。
...
TL;DR: 提出CrossTrainer方法，通过发现跨运动通用的技能属性（如平衡、控制、手部位置），训练多模态语言模型为视频生成可操作的反馈和熟练度评级，在跨运动和同运动场景下相比现有技术提升达60%。
...
Result: 在跨运动和同运动设置下的多个数据集上验证，相比现有技术提升达60%，视频表示比现有技术泛化能力更强。
...
TL;DR: RISE是首个基于单个静态雷达的室内场景理解基准系统，通过利用多径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时克服了传统光学传感器的遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 解决室内场景理解中光学传感器存在的遮挡和隐私风险问题，利用毫米波雷达的隐私保护和穿透能力，但需要克服其低空间分辨率的挑战。

Method: 提出双角度多径增强方法，显式建模到达角和离开角来恢复次级反射；采用模拟到现实的分层扩散框架，将碎片化的雷达响应转化为完整的场景理解。

Result: 在布局重建方面将Chamfer距离降低60%（降至16厘米），首次实现基于毫米波雷达的物体检测，达到58% IoU。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI图像的解剖平面方向（轴向、冠状面、矢状面），准确率达到99.49%，比2D模型错误率降低60%。该模型通过多切片信息避免单一切片的歧义，并在脑肿瘤检测任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 人类可以轻松识别MRI图像的解剖平面方向，但自动化系统在此任务上表现不佳。缺失的平面方向元数据会复杂化分析，增加异构数据集合并时的域偏移，并降低诊断分类器的准确性。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单一切片的歧义，实现稳健的特征学习。在3D切片序列和静态2D图像上训练模型。

Result: 2D参考模型准确率为98.74%，2.5D方法提升至99.49%，错误率降低60%。在脑肿瘤检测任务中，通过门控策略选择性使用元数据增强预测，准确率从97.0%提升至98.0%，误诊率降低33.3%。

Conclusion: 2.5D上下文对于MRI平面方向识别至关重要。生成的元数据能显著提升诊断任务的性能。模型已集成到交互式Web应用程序中并开源提供。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习，将医学图像分割中的主动学习任务转化为可执行程序，显著减少专家标注工作量约80%，同时保持或超越传统主动学习方法的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中的主动学习仍然面临边界模糊、标注劳动密集和认知负担重的问题。传统方法中标注工作量与感兴趣区域大小相关，大区域标注成本高，小区域需要更精细的注意力。语言指导提供了一种替代方案，能最小化专家工作量并绕过精确边界描绘的认知负担。

Method: 提出LINGUAL框架：接收专家的自然语言指令，通过上下文学习将其翻译成可执行程序，自动执行相应的子任务序列而无需人工干预。

Result: 在主动域适应任务中，LINGUAL实现了与传统主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: LINGUAL框架通过语言指导有效解决了医学图像分割中的标注挑战，显著降低了专家工作量，为医学图像分析提供了更高效的解决方案。
...
Method: 基于自监督模型在RandomResizedCrop等增强下学习产生一致表示的特性，WaRPAD首先定义基础评分函数，通过Haar小波分解提取高频方向，量化图像嵌入对扰动的敏感性。为模拟裁剪增强的鲁棒性，将图像缩放到模型输入尺寸的倍数，分割成小块，计算每个块的基础评分，最终通过平均所有块的评分得到检测分数。
...
TL;DR: FashionMAC是一个基于扩散模型的免变形框架，用于生成高质量可控的时尚展示图像，通过直接外推分割的服装来保持细节，并利用区域自适应解耦注意力机制实现细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在服装变形过程中导致的纹理失真问题，以及缺乏对生成模型细粒度属性控制的机制。

Method: 提出免变形的扩散框架，直接外推分割的服装；设计区域自适应解耦注意力机制和链式掩码注入策略来实现细粒度控制。
...
Conclusion: 提出的方法为混凝土桥梁结构缺陷检测提供了一种有效的自动化解决方案，结合了显著性分析和深度学习技术的优势，有望在实际工程检测中应用。
...
Method: 提出SCAR方法，包含两个关键组件：压缩语义预填充（将高级语义编码为紧凑前缀）和语义对齐指导（在自回归解码期间对齐最后视觉隐藏状态与目标语义）。
...
Method: CORE使用高效分割解码器生成对象掩码作为高层语义先验，指导视觉token合并为紧凑的对象中心表示，并通过质心引导排序机制恢复空间顺序。
...
TL;DR: 提出ZS-TMS新范式，使用预训练生成引擎直接合成任务特定分类器的全部参数，仅需单张图像和文本描述即可生成分类器权重，无需训练或微调。
...
Method: 开发语义引导参数合成器(SGPS)，利用多模态任务信息(单张图像+临床文本描述)直接合成轻量级分类器(如EfficientNet-V2)的权重。
...
TL;DR: 提出了DEER-3D框架，通过错误驱动的3D场景编辑生成针对性反事实数据，解决3D-LLMs在视觉和空间元素基础方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在准确将语言基础到3D环境中的视觉和空间元素方面存在限制，主要原因是训练数据侧重于语言推理而非空间理解，且缺乏3D资源导致固有的基础偏见未解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑和重新训练"的工作流程。在识别3D-LLM的基础失败后，首先诊断精确的谓词级错误，然后执行最小化的谓词对齐3D场景编辑，如重新着色或重新定位，为迭代模型微调生成针对性反事实监督。

Result: 在多个3D基础和场景理解任务的基准测试中评估编辑流程，通过迭代细化在所有评估数据集上一致展示了改进。

Conclusion: DEER-3D强调了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间基础之间的有效性。
...
TL;DR: 本文提出BCE3S方法，通过二元交叉熵的三方协同学习解决长尾识别问题，包括联合学习、对比学习和均匀学习三个组件，在多个长尾数据集上达到SOTA性能。
...
Method: BCE3S包含三个组件：(1)基于二元交叉熵的联合学习优化分类器和样本特征；(2)基于二元交叉熵的对比学习进一步提升特征的类内紧凑性；(3)基于二元交叉熵的均匀学习平衡分类器向量间的可分离性。

Result: 实验表明，BCE3S训练的长尾识别模型不仅实现了样本特征间更高的紧凑性和可分离性，还平衡了分类器的可分离性，在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等多个长尾数据集上达到SOTA性能。

Conclusion: BCE3S方法通过三方协同学习有效解决了长尾识别中的特征学习和分类器不平衡问题，显著提升了模型性能。
...
TL;DR: FAPE-IR是一个用于全合一图像恢复的频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知的恢复计划，通过LoRA-MoE模块在扩散模型中动态选择高频或低频专家，结合对抗训练和频率正则化损失提升恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有的全合一图像恢复方法通常依赖任务特定设计或潜在路由策略，难以适应现实世界中各种复杂退化情况，需要开发更统一和可解释的解决方案。

Method: 使用冻结的MLLM作为规划器分析退化图像并生成频率感知恢复计划；在基于扩散的执行器中采用LoRA-MoE模块动态选择高频或低频专家；引入对抗训练和频率正则化损失以减少伪影。
...
TL;DR: 本文提出了推理视频编辑任务和RIVER模型，通过多跳推理解析隐式查询来编辑视频，无需精确的空间位置和时间边界描述。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要用户提供编辑目标的精确空间位置和时间边界描述，这在用户通过语义属性或对象关系的隐式查询来构思编辑时变得不切实际。

Method: RIVER通过数字孪生表示解耦推理和生成，使用大语言模型进行多跳推理确定修改内容，然后输出结构化指令指导基于扩散的编辑器执行像素级修改。训练使用强化学习，奖励评估推理准确性和生成质量。

Result: RIVER在提出的RVEBenchmark（100个视频，519个隐式查询）上表现最佳，在VegGIE和FiVE两个额外视频编辑基准测试中也达到最先进性能，超越了六个基线方法。

Conclusion: RIVER模型成功解决了推理视频编辑任务，能够通过多跳推理理解隐式查询并执行视频编辑，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督单目深度估计模型计算资源消耗大，虽然有些方法减小了模型规模但性能下降严重，阻碍了在实际场景中的部署应用。

Method: 基于Lite-Encoder的编码器和多尺度稀疏融合框架的解码器架构，最小化冗余并确保性能，提高推理速度。

Result: 在KITTI数据集上实现了最先进性能，参数仅3M，相比轻量级方法在低分辨率下Abs Rel和Sq Rel分别提升5.6%和9.8%，高分辨率下Sq Rel和RMSE分别提升6.1%和1.9%，在Nvidia Jetson Orin上达到49 FPS实时推理速度。

Conclusion: RTS-Mono成功解决了自监督单目深度估计模型在保持高性能的同时实现实时推理的挑战，为自动驾驶和机器人导航的实际部署提供了可行解决方案。
...
TL;DR: 提出了一种基于非对称聚合和几何约束的视觉位置识别方法A²GC-VPR，通过行-列归一化平均和单独边缘校准实现非对称匹配，结合可学习坐标嵌入增强空间感知能力，在多个数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有最优传输聚合方法中标准的Sinkhorn算法对称处理源和目标边缘分布，当图像特征和聚类中心分布差异较大时效果受限，需要开发能够适应分布差异的非对称匹配方法。

Method: 采用行-列归一化平均与单独边缘校准实现非对称聚合，引入可学习坐标嵌入计算兼容性分数并与特征相似性融合，促进空间邻近特征分配到同一聚类。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明，该方法在匹配准确性和鲁棒性方面表现出优越性能。

Conclusion: 提出的A²GC-VPR方法通过非对称聚合和几何约束有效提高了视觉位置识别的性能，验证了该方法在处理分布差异和增强空间感知方面的有效性。
...
Method: 采用Cascaded-Chunk Feed Forward Network (CCFFN)设计，通过分割输入特征来提高参数和FLOP效率，而不牺牲准确性。

Result: 在ImageNet-1K上，CViT-XL模型达到75.5% Top-1准确率，相比EfficientViT-M5减少15% FLOPs和3.3%能耗。CViT系列在各种模型尺寸下都表现出最低的能耗。

Conclusion: CViT架构特别适合部署在电池受限设备上，如手机和无人机，并且在Accuracy-Per-FLOP (APF)指标下始终达到顶级效率排名。
...
TL;DR: 论文提出了Coffee方法，通过语言描述来指定不希望学习的概念，在微调过程中防止文本到图像扩散模型学习不需要的概念，无需额外训练即可实现灵活的概念控制。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型在微调时容易学习到不需要的概念，并将这些概念与用户提示词纠缠在一起，这在下游任务如偏见缓解、防止恶意适应、属性解缠和扩散策略的通用微调中至关重要。

Method: Coffee方法通过保持用户提示词嵌入与不希望学习的概念不对齐来实现正则化，核心在于使用语言描述来指定不希望学习的概念，无需额外训练且支持通过修改文本描述灵活调整不希望学习的概念。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习指定的不希望概念，并且在性能上优于现有方法。

Conclusion: Coffee提供了一种无需额外训练的语言驱动方法，能够有效控制扩散模型在微调过程中学习的内容，防止不需要的概念被学习，为下游应用提供了更好的可控性。
...
TL;DR: 本文提出MP-PVIR框架，通过多视角视频分析将行人-车辆事故分解为认知行为阶段，生成结构化诊断报告，提升交通安全分析能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频系统只能检测事故是否发生，但无法分析事故在不同认知阶段的发展过程，且缺乏多视角整合能力。

Method: 采用四阶段框架：事件触发多视角视频采集、行人行为阶段分割、阶段特定多视角推理、分层合成和诊断推理，使用两个专用VLM模型进行阶段分割和多视角分析。

Result: TG-VLM模型在行为阶段分割上达到mIoU=0.4881，PhaVR-VLM在字幕生成得分33.063，问答准确率最高达64.70%。

Conclusion: MP-PVIR能有效将多视角视频数据转化为可操作的见解，推进车路协同系统中的AI驱动交通安全分析。
...
<details>
  <summary>Details</summary>
Motivation: 尽管CNN和Transformer重塑了计算机视觉，但卷积和自注意力通常被视为根本不同的架构。作者认为这两种操作可以在统一的k近邻聚合框架下理解，它们只是邻居选择标准的不同。

Method: 引入ConvNN框架作为卷积层和注意力层的即插即用替代品，支持在空间邻近性和特征相似性两个极端之间进行系统探索。在VGG和ViT架构上进行验证。

Result: 在CIFAR-10和CIFAR-100上的实验表明：VGG中的混合分支结合两种选择策略提高了准确率；ViT中的ConvNN优于标准注意力和其他变体。消融实验显示在频谱中间插值通过平衡局部和全局感受野提供正则化效益。

Conclusion: ConvNN提供了一个统一框架，消解了卷积和注意力之间的明显区别，为设计更原则化和可解释的视觉架构提供了启示。
...
### [45] [O3SLM: Open Weight, Open Data, and Open Vocabulary Sketch-Language Model](https://arxiv.org/abs/2511.14368)
*Rishi Gupta,Mukilan Karuppasamy,Shyam Marjit,Aditay Tripathi,Anirban Chakraborty*

Main category: cs.CV

TL;DR: 本文提出了一个大规模图像-草图-指令三元组数据集和O3SLM模型，显著提升了大型视觉语言模型对抽象手绘草图的理解能力，在多个草图任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在理解抽象视觉输入（特别是手绘草图）方面存在局限，主要瓶颈是缺乏同时建模草图、真实图像和自然语言指令的大规模数据集。

Method: 构建大规模图像-草图-指令三元组数据集，并基于此训练O3SLM模型；在多个草图任务上进行综合评估，包括目标定位、计数、图像检索和视觉问答。

Result: O3SLM在多个草图任务上达到最先进性能，显著优于现有大型视觉语言模型在草图理解和推理方面的表现。

Conclusion: 通过构建专用数据集和模型，成功提升了大型视觉语言模型对抽象手绘草图的理解能力，为草图理解任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够穿透遮挡物重建隐藏物体3D形状的能力，为机器人、增强现实和物流等领域开辟新的应用可能。

Method: 采用三阶段流水线：提出候选几何表面、使用专为毫米波信号设计的基于transformer的形状补全模型、执行熵引导的表面选择，将原始无线信号与基于视觉的形状补全技术相结合。

Result: 在对比实验中，Wave-Former将召回率从54%提升至72%，同时保持85%的高精度。

Conclusion: Wave-Former展示了利用毫米波信号进行3D形状重建的显著改进，能够仅使用合成点云数据进行训练，并在真实世界数据上表现出良好的泛化能力。
...
TL;DR: 本文提出了RiSe框架来解决多模态人脸反欺诈中的跨域泛化问题，通过非对称不变风险最小化和多模态协同解耦来应对表示风险和协同风险。


<details>
  <summary>Details</summary>
Motivation: 多模态人脸反欺诈方法在未见域中性能下降严重，主要由于两个被忽视的跨域泛化风险：模态表示不变风险和模态协同不变风险。

Method: 提出RiSe框架，包含AsyIRM学习径向空间的不变球形决策边界，以及MMSD通过跨样本混合和解耦增强内在模态特征。

Result: 理论分析和实验验证了RiSe的有效性，在跨域性能上达到了最先进水平。

Conclusion: RiSe框架成功解决了多模态人脸反欺诈的跨域泛化问题，通过处理表示和协同不变风险实现了优异的性能。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入对大型视觉语言模型稳健性影响的综合基准，包含1,248个专家标注的VQA实例，涵盖视觉概念、属性和关系三个层次，并提出了细粒度评估指标MVI-Sensitivity。
...
Method: 基于基本视觉原语，设计了三个层次的误导性视觉输入分类：视觉概念、视觉属性和视觉关系，并构建了六个代表性类别的1,248个VQA实例，提出了MVI-Sensitivity细粒度评估指标。

Result: 对18个最先进LVLM的实证研究揭示了它们对误导性视觉输入的显著脆弱性，深入分析为开发更可靠的LVLM提供了可操作的见解。

Conclusion: MVI-Bench填补了误导性视觉输入评估的空白，揭示了LVLM在此方面的脆弱性，为开发更稳健的视觉语言模型提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中图像补丁级令牌化导致的令牌数量二次增长问题，该问题带来了巨大的计算和内存负担，且与人类视觉认知系统不一致，导致幻觉和计算冗余。

Method: 提出对象级令牌合并策略进行自适应令牌压缩，该方法与人类视觉系统保持一致，通过减少令牌数量来优化计算效率。

Result: 在多个综合基准测试中，该方法平均仅使用10%的令牌就能达到原始模型约96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 所提出的自适应令牌压缩方法有效解决了MLLMs中的令牌膨胀问题，显著提升了计算效率，同时保持了良好的性能表现。
...
Method: 提出DoGCLR框架：1）将正负样本构建建模为动态优势博弈；2）时空双权重定位机制识别关键运动区域并指导区域增强；3）熵驱动优势策略管理记忆库，保留高熵（困难）负样本。
...
TL;DR: UniSER是一个基础性的通用模型，能够在一个框架内处理多种软效应（如镜头光晕、雾霾、阴影、反射）导致的图像退化问题，通过大规模数据集和定制训练流程显著优于专业模型和通用模型。
...
Method: 基于软效应的共同本质（半透明遮挡），构建包含380万对数据的大规模数据集，采用定制训练流程微调扩散变换器，集成细粒度掩码和强度控制。

Result: UniSER在真实场景中实现了稳健、高保真的修复效果，显著优于专业模型和通用模型。

Conclusion: 利用软效应的共享本质，UniSER提供了一个能够处理多种退化问题的通用框架，通过大规模数据集和定制训练实现了卓越的修复性能。
...
TL;DR: GloTok提出了一种全局视角的tokenizer，利用全局关系信息建模更均匀的语义分布，通过代码本直方图关系学习和残差学习模块，实现了更好的图像重建和生成质量。
...
Method: 1. 提出代码本直方图关系学习方法，将预训练模型在整个数据集上建模的语义转移到语义代码本；2. 设计残差学习模块恢复细粒度细节，最小化量化引起的重建误差。

Result: 在标准ImageNet-1k基准测试中，该方法实现了最先进的重建性能和生成质量。

Conclusion: GloTok通过全局关系建模实现了更均匀分布的语义潜在表示，有助于自回归模型生成高质量图像，且训练过程无需直接访问预训练模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集大多由人工驾驶或未明确驾驶模式收集，只能用于自动驾驶感知和预测的早期训练，无法评估黑盒控制下自动驾驶车辆的真实行为安全。

Method: 收集了市场上多个量产自动驾驶车型的自然驾驶数据，将原始数据分割为32,727个关键帧，每个关键帧包含四个同步摄像头图像和高精度GNSS/IMU数据，并提供详细的2D标注和丰富的场景级属性。

Result: 数据集包含超过100小时数据，每周新增超过10小时，使用端到端运动规划模型在自动驾驶帧上实现了1.4米的平均位移误差。
...
TL;DR: 提出了一种基于课程学习和分形softmax的主动脉血管分割方法，通过层次化语义学习解决血管结构中的类别不平衡问题，显著提升了分割精度和效率。
...
Method: 采用课程学习策略和分形softmax进行层次化语义学习，通过从简单到复杂逐步学习解剖约束，两阶段推理策略实现5倍加速。

Result: 在验证集上，层次化语义损失使nnU-Net ResEnc M的Dice分数提升11.65%；在测试集上比基线方法高5.6%的Dice分数。
...
TL;DR: Gen3d是一种从单张图像生成高质量、宽范围通用3D场景的新方法，通过RGBD图像生成初始点云，维护和扩展世界模型，最后优化高斯泼溅表示来完成3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 当前神经3D重建方法依赖密集多视角捕获，限制了更广泛的应用；同时3D场景生成对于推进具身AI和世界模型至关重要，这些模型需要多样化的高质量场景进行学习和评估。

Method: 首先通过RGBD图像生成初始点云，然后维护和扩展世界模型，最后通过优化高斯泼溅表示来完善3D场景。
...
TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集级平均精度(AP)的边际扰动来选择信息丰富的训练样本，提高检测精度。


<details>
  <summary>Details</summary>
Motivation: 高质量数据已成为规模定律下进步的主要驱动力，但现有的在线采样策略很少扩展到目标检测领域，因为其结构复杂性和领域差距。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本高的限制，联邦半监督学习虽然提供解决方案，但面临伪标签可靠性依赖本地模型强度、客户端设备计算资源有限导致模型轻量化或异构架构的挑战。

Method: 提出SAM-Fed框架，结合双重知识蒸馏和自适应一致性机制，利用高容量分割基础模型为轻量级客户端提供训练指导，优化像素级监督质量。

Result: 在皮肤病变和息肉分割任务上，无论是同构还是异构设置，SAM-Fed都持续优于最先进的联邦半监督学习方法。

Conclusion: SAM-Fed通过利用分割基础模型指导轻量级客户端，有效解决了联邦半监督学习中伪标签质量和稳定性的问题，在医学图像分割任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法要么产生时间伪影，要么迭代后验采样速度过慢无法实时使用，需要开发既能保持高质量又能满足实时延迟约束的视频重建方法。

Method: 将强大的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，仅需教师扩散模型和已知退化算子，无需外部配对数据；使用LeanVAE替换VAE并通过教师空间正则化蒸馏方案实现低延迟潜在空间处理。

Result: 在流式随机修复、高斯去模糊和超分辨率任务中，InstantViR匹配或超越基于扩散的基线重建质量，在NVIDIA A100 GPU上运行速度超过35 FPS，比迭代视频扩散求解器快达100倍。
...
### [59] [Clinically-Validated Innovative Mobile Application for Assessing Blinking and Eyelid Movements](https://arxiv.org/abs/2511.14361)
...
TL;DR: Bapp是一款基于Flutter框架开发的移动应用，使用Google ML Kit进行实时眼睑运动分析。在45个真实患者视频上的验证显示，其准确率达到98.3%，为临床环境中的眼睑运动监测提供了便携、客观的解决方案。
...
Method: 使用Flutter框架开发移动应用Bapp，集成Google ML Kit进行设备端实时眼睑运动分析。使用45个真实患者视频，由眼科专家手动标注眨眼作为基准真值，通过精确率、召回率和F1分数等标准指标评估性能。

Result: Bapp表现出色：精确率98.4%，召回率96.9%，整体准确率98.3%。验证了其作为便携、客观眼睑运动监测工具的可靠性。

Conclusion: Bapp为传统手动眨眼计数提供了有前景的替代方案，支持临床环境中的持续眼部健康监测和术后评估，具有高准确性和临床实用性。
...
Conclusion: 提出的测量约束采样方法有效地解决了盲人脸恢复中的一对多问题，实现了多样化的高质量重建。
...
Method: 采用自回归扩散模型，以流式方式处理输入音频，选择有限数量的历史帧作为运动上下文，结合音频输入创建动态条件来指导扩散过程迭代生成面部运动帧。

Result: 该方法能够灵活处理不同长度的音频输入，实现与音频时长无关的低延迟，并开发了实时交互演示验证了方法的有效性和效率。

Conclusion: 提出的自回归扩散模型在3D面部动画任务中实现了高质量的实时合成，解决了传统方法的局限性。
...
### [62] [Cheating Stereo Matching in Full-scale: Physical Adversarial Attack against Binocular Depth Estimation in Autonomous Driving](https://arxiv.org/abs/2511.14386)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击多采用2D补丁且主要针对单目感知，对立体双目深度估计的有效性尚未充分探索，因此需要研究针对立体匹配模型的物理对抗攻击。

Method: 提出3D物理对抗示例，采用全局伪装纹理；设计3D立体匹配渲染模块处理双目相机视差效应；提出新颖的融合攻击方法，通过细粒度优化将目标无缝融入环境。

Result: 广泛评估表明，所提出的物理对抗示例能够成功欺骗立体模型产生错误的深度信息。

Conclusion: 该方法显著增强了现有隐藏攻击的隐蔽性和杀伤力，能够实现与背景的无缝融合，在自动驾驶立体感知系统中具有实际威胁。
...
### [63] [Enhancing Generalization of Depth Estimation Foundation Model via Weakly-Supervised Adaptation with Regularization](https://arxiv.org/abs/2511.14238)
*Yan Huang,Yongyi Su,Xin Lin,Le Zhang,Xun Xu*

Main category: cs.CV

TL;DR: WeSTAR是一个参数高效的弱监督自训练适应框架，通过正则化增强单目深度估计基础模型在未见领域的鲁棒性，结合密集自训练、语义感知层次归一化、成对深度弱监督和权重正则化，在多样化场景下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在零样本单目深度估计方面取得进展，但在获得下游任务数据时，如何进一步提升这些模型的性能成为一个自然问题。

Method: 采用密集自训练目标作为结构自监督主要来源；引入语义感知层次归一化，利用实例级分割图进行稳定多尺度结构归一化；使用成对深度弱监督提供信息性排序约束；应用权重正则化损失确保训练稳定性。

Result: 在现实和损坏的分布外数据集上的广泛实验表明，WeSTAR持续改善泛化能力，在广泛基准测试中实现最先进性能。
...
### [64] [Agentic Video Intelligence: A Flexible Framework for Advanced Video Exploration and Understanding](https://arxiv.org/abs/2511.14446)
*Hong Gao,Yiming Bao,Xuezhen Tu,Yutong Xu,Yue Jin,Yiyang Mu,Bin Zhong,Linan Yue,Min-Ling Zhang*

Main category: cs.CV

TL;DR: 提出了Agentic Video Intelligence (AVI)框架，通过系统级设计实现无需训练的视频智能理解，包含三阶段推理过程、结构化视频知识库和开源模型集成。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解方法要么是单次处理的VLM模型，缺乏证据重访和迭代优化能力；要么是依赖昂贵专有模型或需要大量强化学习训练的智能体方法。
...
Conclusion: AVI提供了一个灵活且无需训练的框架，能够模拟人类视频理解过程，在保持竞争力的同时提供优越的可解释性，消除了对专有API或强化学习训练的依赖。
...
### [65] [V2VLoc: Robust GNSS-Free Collaborative Perception via LiDAR Localization](https://arxiv.org/abs/2511.14247)
*Wenkai Lin,Qiming Xia,Wen Li,Xun Huang,Chenglu Wen*

Main category: cs.CV

TL;DR: 提出了一种基于LiDAR定位的GNSS-free协作感知框架，包括轻量级姿态生成器PGC和姿态感知时空对齐Transformer PASTAT，在GNSS拒止环境下实现最先进的性能。
...
Method: 1. 轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2. 姿态感知时空对齐Transformer PASTAT进行置信度感知的空间对齐并捕获时间上下文；3. 创建V2VLoc模拟数据集支持定位和检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒止条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化性。

Conclusion: 提出的GNSS-free协作感知框架通过LiDAR定位和时空对齐机制，在GNSS拒止环境中实现了鲁棒的协作感知性能，具有实际应用价值。
...
Method: 构建包含45万张篡改图像的ManipBench基准，覆盖25个最先进图像编辑模型和12种篡改类别；提出基于多模态大语言模型的ManipShield模型，采用对比LoRA微调和任务特定解码器实现统一的检测、定位和解释。

Result: 在ManipBench和多个公共数据集上的广泛实验表明，ManipShield实现了最先进的性能，并对未见过的篡改模型表现出强大的泛化能力。

Conclusion: 提出的ManipBench基准和ManipShield模型有效解决了现有篡改检测方法的局限性，为AI编辑图像的检测提供了全面解决方案，具有重要的研究和应用价值。
...
TL;DR: 提出深度学习框架用于WMH分割和定位，区域WMH体积在疾病分类中优于全局病变负担，结合脑萎缩指标可进一步提高诊断性能


<details>
  <summary>Details</summary>
Motivation: 现有WMH分割方法大多只提供全局病变负荷，忽略了不同白质区域的空间分布信息，而区域WMH量化可能对神经退行性疾病诊断有重要价值

Method: 深度学习框架用于稳健的WMH分割和定位，在公共数据集和ADNI队列中评估，量化解剖定义区域内的WMH负荷并与脑结构体积结合

Result: 预测病变负荷与参考WMH估计一致，区域WMH体积在疾病分类中始终优于全局病变负担，结合萎缩指标AUC可达0.97，前部白质束区域与诊断状态可重复相关
...
### [68] [Gaussian Splatting-based Low-Rank Tensor Representation for Multi-Dimensional Image Recovery](https://arxiv.org/abs/2511.14270)
...
Conclusion: GSLR框架通过高斯泼溅方法有效解决了传统t-SVD的局限性，在多维图像表示和恢复方面表现出色。
...
TL;DR: 提出CCSD框架解决多模态MRI中模态缺失问题，通过跨模态组合自蒸馏方法灵活处理任意输入模态组合，在脑肿瘤分割任务中达到先进性能。


<details>
  <summary>Details</summary>
Motivation: 临床实践中多模态MRI经常缺失一个或多个模态，严重影响深度学习分割模型的性能和泛化能力，需要开发能够灵活处理任意模态组合的方法。

Method: 采用共享-特定编码器-解码器架构，包含两种自蒸馏策略：层次模态自蒸馏机制减少语义差异，渐进模态组合蒸馏方法通过模拟模态丢失增强鲁棒性。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种缺失模态场景下均达到最先进性能，具有强大的泛化能力和稳定性。

Conclusion: CCSD框架能够有效解决多模态MRI中的模态缺失问题，为临床诊断和治疗规划提供可靠的分割结果。
...
### [70] [Let Language Constrain Geometry: Vision-Language Models as Semantic and Spatial Critics for 3D Generation](https://arxiv.org/abs/2511.14271)
...
Method: 提出VLM3D框架，从视觉语言模型的Yes/No对数几率中提取双查询评判信号，评估语义保真度和几何一致性。该指导信号适用于两种范式：作为优化流程的奖励目标，以及作为前馈流程的测试时指导模块。

Result: 在标准基准测试中，VLM3D显著优于现有方法；作为测试时指导模块，能够主动引导SOTA原生3D模型的迭代采样过程，纠正严重空间错误。

Conclusion: VLM3D为将视觉语言模型丰富的、基于语言理解的语义和空间知识注入多样化3D生成流程提供了原则性和可推广的路径。
...
<details>
  <summary>Details</summary>
Motivation: 准确建模阿尔茨海默病的认知衰退对于早期分层和个性化管理至关重要。表格预测因子虽然提供稳健的全局风险标记，但捕捉细微脑变化的能力有限，因此需要评估不同表示方法的预测贡献。

Method: 引入基于动态时间规整聚类的轨迹感知标记策略来捕捉认知变化的异质模式；通过无监督重建在协调和增强的MRI数据上训练3D视觉transformer，获得保留解剖结构的嵌入；使用传统机器学习分类器和深度学习头评估预训练编码器嵌入。

Result: 临床和体积特征在预测轻度和重度进展方面获得约0.70的最高AUC；ViT模型的MRI嵌入在区分认知稳定个体方面最有效，AUC为0.71；所有方法在异质中度组中都表现不佳。
...
### [72] [NeuralBoneReg: A Novel Self-Supervised Method for Robust and Accurate Multi-Modal Bone Surface Registration](https://arxiv.org/abs/2511.14286)
...
Conclusion: NeuralBoneReg在解剖结构和模态间表现出强大的泛化能力，为计算机辅助骨科手术提供了鲁棒且准确的跨模态对齐解决方案。
...
TL;DR: 提出了一种基于时间多普勒全息图的动脉-静脉分割方法，通过结合脉搏分析特征，使标准U-Net能够利用时间动态信息，达到与复杂模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统视网膜血管分割方法只关注空间信息，忽略了多普勒全息数据的时间丰富性，需要一种能够利用时间动态信息的方法来准确分割视网膜动脉和静脉。

Method: 使用标准分割架构（如U-Net），结合专门的脉搏分析流程提取的特征，让传统U-Net能够利用时间动态信息进行动脉-静脉分割。

Result: 该方法实现了与更复杂的基于注意力或迭代的模型相当的性能，证明了时间分辨预处理能够充分发挥深度学习在多普勒全息成像中的潜力。

Conclusion: 时间分辨预处理为多普勒全息成像解锁了深度学习的全部潜力，为视网膜血流动力学的定量探索开辟了新视角。
...
### [74] [Iterative Diffusion-Refined Neural Attenuation Fields for Multi-Source Stationary CT Reconstruction: NAF Meets Diffusion Model](https://arxiv.org/abs/2511.14310)
...
<details>
  <summary>Details</summary>
Motivation: 多源静态CT在超稀疏视图采样条件下重建质量严重下降，传统方法难以应对，需要新的解决方案。

Method: 使用神经衰减场表示和双分支条件扩散模型，通过角度先验引导投影合成和扩散驱动重用投影精化模块迭代优化投影数据。

Result: 在多个模拟3D CT体积和真实投影数据上的实验表明，Diff-NAF在超稀疏视图条件下实现了最佳性能。

Conclusion: Diff-NAF通过迭代精化能够显著提升超稀疏视图条件下的投影完整性和重建保真度，最终获得高质量的CT重建结果。
...
### [75] [Dental3R: Geometry-Aware Pairing for Intraoral 3D Reconstruction from Sparse-View Photographs](https://arxiv.org/abs/2511.14315)
...
<details>
  <summary>Details</summary>
Motivation: 传统口腔内扫描方法无法用于远程正畸，而基于稀疏智能手机图像的重建面临大视角基线、光照不一致和镜面反射等问题，导致重建质量不佳。

Method: 提出几何感知配对策略(GAPS)选择高价值图像对，然后使用3D高斯泼溅(3DGS)模型结合小波正则化目标进行训练，以保持精细结构。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏无姿态输入，在牙齿咬合可视化方面优于现有方法。

Conclusion: Dental3R为远程正畸提供了一种稳健、高保真的3D重建解决方案，能够从稀疏口腔照片中恢复详细的牙齿结构。
...
### [76] [ArchMap: Arch-Flattening and Knowledge-Guided Vision Language Model for Tooth Counting and Structured Dental Understanding](https://arxiv.org/abs/2511.14336)
*Bohan Zhang,Yiyi Miao,Taoyu Wu,Tong Chen,Ji Jiang,Zhuoxiao Li,Zhe Tang,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: ArchMap是一个无需训练的知识引导框架，用于3D口腔扫描的结构化理解，通过几何感知的牙弓展平模块和多模态推理解决现有方法的泛化性问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方案依赖模态特定训练、大量标注数据和受控扫描条件，限制了设备间泛化能力，且原始口腔网格存在牙弓姿态变化、几何不完整和缺乏纹理线索等问题。

Method: 提出几何感知牙弓展平模块将原始3D网格标准化为空间对齐的多视图投影，构建包含牙齿本体论、牙列阶段策略和临床语义的牙科知识库来约束符号推理空间。
...
### [77] [Silhouette-to-Contour Registration: Aligning Intraoral Scan Models with Cephalometric Radiographs](https://arxiv.org/abs/2511.14343)
*Yiyi Miao,Taoyu Wu,Ji Jiang,Tong Chen,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: DentalSCR是一个用于口腔内扫描模型与侧位头影测量X光片稳定配准的轮廓引导框架，通过构建统一解剖坐标系和对称双向Chamfer距离优化，解决了传统强度配准方法在临床条件下的不稳定性问题。
...
Method: 首先构建U-Midline Dental Axis建立统一跨弓解剖坐标系，然后通过基于表面的DRR公式和Gaussian splatting生成类似X光片的投影，最后使用对称双向Chamfer距离在分层粗到细调度下优化2D相似变换。

Result: 在34个专家标注的临床病例上评估，结果显示显著减少了标志点误差（特别是后牙区域），下颌区域误差分布更紧密，曲线级别的Chamfer和Hausdorff距离表现良好。
...
### [78] [ARC-Chapter: Structuring Hour-Long Videos into Navigable Chapters and Hierarchical Summaries](https://arxiv.org/abs/2511.14349)
*Junfu Pu,Teng Wang,Yixiao Ge,Yuying Ge,Chen Li,Ying Shan*

Main category: cs.CV

TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频章节划分模型，通过双语、时间定位和分层章节标注，显著提升了长视频内容结构化的性能。
...
Method: 通过结构化流水线构建双语英中章节数据集，统一ASR转录、场景文本和视觉描述为多级标注，从短标题到长摘要；设计了新的评估指标GRACE，结合多对一段落重叠和语义相似度。
...
TL;DR: 本文提出Image-Based Gaussian Splatting方法，通过结合标准3DGS渲染的基础颜色和从相邻训练图像学习到的残差，显著提升了渲染质量，特别是高频细节和视角相关效果，且不增加存储开销。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射方法使用低阶球谐函数限制了捕捉空间变化颜色和视角相关效果（如镜面高光）的能力。现有方法要么使用全局纹理图（难以处理复杂场景），要么使用每个高斯的纹理图（存储开销大）。

Method: 提出基于图像的高斯溅射方法，利用高分辨率源图像进行精细细节和视角特定颜色建模。将每个像素颜色建模为标准3DGS渲染的基础颜色与从相邻训练图像学习到的残差的组合。

Result: 在标准NVS基准测试上的实验表明，该方法在渲染质量上显著优于先前的高斯溅射方法，特别是在高频细节和准确的视角相关效果方面。

Conclusion: 该方法提供了一种高效的替代方案，能够渲染高质量图像，同时保持与标准3DGS相同的存储占用。
...
TL;DR: 提出一种肩部姿态评估算法，用于自动评估生物特征身份证件中肩部姿态的合规性，仅使用两个肩部标志点的3D坐标来量化肩部偏航和滚动角度。


<details>
  <summary>Details</summary>
Motivation: 国际生物特征身份证件标准要求严格符合姿态要求，但现有自动质量评估文献缺乏评估肩部姿态的定量方法。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏航和滚动角度。
...
<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，许多应用需要估计世界坐标系中的人体运动，特别是在存在人体和相机运动的情况下。该领域进展受到缺乏具有真实人体和相机运动地面真实视频数据的限制。

Method: 开发了BEDLAM2.0数据集，通过增加更多多样化和真实的相机及相机运动，提高身体形状、动作、服装、头发和3D环境的多样性和真实感，并添加了BEDLAM中缺失的鞋子。

Result: BEDLAM2.0显著优于BEDLAM，特别是在训练估计世界坐标系中人体运动的方法方面。比较在BEDLAM和BEDLAM2.0上训练的最先进方法，发现BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个重要的资源，为研究目的提供了渲染视频、地面真实身体参数和相机运动，以及3D资产和第三方链接。
...
Result: 序数回归方法在APTOS数据集上获得了0.8992的QWK分数，显著优于现有方法。

Conclusion: 该序数回归框架为糖尿病视网膜病变的自动筛查提供了有效的解决方案，具有重要的临床应用价值。
...
TL;DR: LAVA是一个新颖的领域增量学习框架，通过基于文本的参考锚点实现相对对齐，解决了现有方法在统一视觉空间和域特定参数之间的两难困境。


<details>
  <summary>Details</summary>
Motivation: 解决领域增量学习中统一视觉空间导致的域间干扰和语义扭曲问题，以及域特定参数隔离导致的知识碎片化问题。

Method: 使用语言锚定的视觉对齐方法，通过基于文本的参考锚点驱动相对对齐，保持一致的相对几何结构，镜像类名之间的语义相似性。

Result: 在标准领域增量学习基准测试中，LAVA相比现有最先进方法取得了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐方法有效解决了领域增量学习中的知识保持和知识重用问题，实现了跨领域的鲁棒特征聚合。
...
TL;DR: 提出Cranio-ID框架，通过YOLO-pose模型自动标注2D头骨和光学图像中的标志点，然后使用图表示和跨模态匹配技术进行头骨到面部的匹配。
...
Method: 首先使用训练的YOLO-pose模型在2D头骨（面部X射线扫描）和光学图像上自动标注标志点；然后将这些标志点表示为图结构，使用交叉注意力和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的广泛实验表明，该框架在可靠性和准确性方面均有显著提升，在法医学中的头骨到面部和素描到面部匹配中表现出有效性。

Conclusion: Cranio-ID框架为法医颅面识别提供了一种可靠且准确的自动标志点标注和跨模态匹配解决方案。
...
TL;DR: 该研究通过模拟婴儿视觉发展阶段的限制条件（灰度到彩色、模糊到清晰、保持时间连续性）训练自监督学习模型，发现这种阶段性'视觉饮食'能增强模型的鲁棒性，并展现出与生物发育模式一致的行为特征。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿视觉发展的阶段性特征（低清晰度、色彩退化、时间连续性）如何为机器视觉系统提供生态优势，以及这种'视觉饮食'是否能增强模型的鲁棒性和生物对齐性。

Method: 使用CATDiet方法训练自监督学习模型，模拟婴儿视觉发展的三个约束条件：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)。建立包含10个数据集的综合基准进行评估。

Result: 所有CATDiet变体在物体识别方面表现出增强的鲁棒性，模型展现出与生物发育模式一致的特征，包括类似猕猴V1区突触密度的神经可塑性变化，以及类似婴儿视觉悬崖反应的行为。CombDiet方法在物体识别和深度感知任务上优于标准自监督学习。

Conclusion: 早期婴儿视觉经验的发展进程为理解机器中鲁棒视觉智能的出现提供了一个强大的逆向工程框架，阶段性视觉限制有助于构建更鲁棒的视觉系统。
...
Method: 使用对话精炼模块主动询问用户提取关键信息并生成精确描述，图像精炼模块识别生成图像与用户意图之间的感知差距，战略性地减少视觉语义差异。

Result: 在多样化图像数据集上的综合实验表明，该对话式方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和增强的交互体验。

Conclusion: DIR-TIR框架通过多轮对话提供了比传统单查询方法更优越的可控性和容错性，在目标图像检索任务中取得了显著改进。
...
TL;DR: 提出了一种物理引导的残差学习框架，用于从观测表面重建冰下床地形，通过预测BedMachine先验上的床厚度残差，结合多尺度物理约束，在格陵兰冰盖区域取得了优于传统方法的性能。
...
Method: 采用DeepLabV3+解码器结合标准编码器（如ResNet-50）的架构，训练时融入轻量级物理和数据约束：多尺度质量守恒、流向对齐总变差、拉普拉斯阻尼、厚度非负性、斜坡先验一致性项，以及基于置信度图调制的掩码Huber拟合雷达点。

Result: 在两个格陵兰子区域上，该方法在测试核心区域实现了强大的精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN等基准方法。

Conclusion: 残差-先验设计结合物理约束，能够产生空间连贯、物理合理的床地形，适用于领域偏移下的操作制图应用。
...
TL;DR: SEED-SR是一种新颖的农场边界分割方法，通过条件潜在扩散模型和地理空间基础模型，在分割感知的潜在空间中执行超分辨率，实现了20倍尺度因子的分割图生成，显著提升了小农场的分割精度。


<details>
  <summary>Details</summary>
Motivation: 解决小农场边界分割的挑战，特别是在高分辨率图像低重访频率的情况下，需要结合高分辨率和低分辨率图像进行更频繁的监测。现有Ref-SR方法在感知质量优化时会平滑关键特征，且无法满足大尺度因子需求。
...
Conclusion: SEED-SR方法通过独特的潜在空间超分辨率方法，能够以前所未有的20倍尺度因子生成分割图，显著提升了农场边界分割的性能。
...
### [89] [Enhancing End-to-End Autonomous Driving with Risk Semantic Distillaion from VLM](https://arxiv.org/abs/2511.14499)
...
Conclusion: RSD框架通过风险注意力蒸馏有效增强了自动驾驶系统的泛化能力，使其更接近人类驾驶行为，在复杂动态环境中表现更优。
...
### [90] [Parameter Aware Mamba Model for Multi-task Dense Prediction](https://arxiv.org/abs/2511.14503)
*Xinzhuo Yu,Yunzhi Zhuge,Sitong Gong,Lu Zhang,Pingping Zhang,Huchuan Lu*

Main category: cs.CV

TL;DR: 提出了一种基于解码器的参数感知Mamba模型(PAMM)，利用状态空间模型的丰富可扩展参数来增强多任务密集预测中的任务互联性，通过双状态空间参数专家整合任务特定参数先验，并采用多方向希尔伯特扫描方法构建多角度特征序列。
...
Method: PAMM框架采用双状态空间参数专家来整合和设置任务特定参数先验，捕获每个任务的内在特性，通过结构化状态空间序列模型(S4)实现任务先验的全局集成，并使用多方向希尔伯特扫描方法构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了该方法的有效性。

Conclusion: PAMM通过状态空间模型的参数丰富性和可扩展性，有效增强了多任务密集预测中的任务互联性，实现了精确的多任务交互。
...
### [91] [D-PerceptCT: Deep Perceptual Enhancement for Low-Dose CT Images](https://arxiv.org/abs/2511.14518)
*Taifour Yousra Nabila,Azeddine Beghdadi,Marie Luong,Zuheng Ming,Habib Zaidi,Faouzi Alaya Cheikh*

Main category: cs.CV

TL;DR: 提出D-PerceptCT架构，基于人类视觉系统原理增强低剂量CT图像质量，通过语义感知和全局-局部状态空间块保留关键解剖结构和病理细节。


<details>
  <summary>Details</summary>
Motivation: 低剂量CT图像质量较差，现有方法存在过度平滑和细节丢失问题，需要开发能保留诊断关键特征的增强方法。

Method: 使用视觉双路径提取器整合DINOv2语义先验与局部空间特征，结合全局-局部状态空间块捕获长距离信息，并设计深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，D-PerceptCT在结构纹理信息保留方面优于现有最先进方法。

Conclusion: D-PerceptCT能有效增强低剂量CT图像，更好地保留诊断所需的结构和细节信息。
...
### [92] [DeCo-VAE: Learning Compact Latents for Video Reconstruction via Decoupled Representation](https://arxiv.org/abs/2511.14530)
*Xiangchen Yin,Jiahui Yuan,Zhangchi Hu,Wenzhang Sun,Jie Chen,Xiaozhen Qiao,Hao Li,Xiaoyan Sun*

Main category: cs.CV

TL;DR: DeCo-VAE通过将视频内容分解为关键帧、运动和残差三个独立组件，为每个组件设计专用编码器，实现紧凑的潜在表示，避免冗余建模。
...
Conclusion: 通过解耦视频内容和专用编码器设计，DeCo-VAE能够有效减少潜在表示冗余，提升视频重建质量。
...
### [93] [Learning Compact Latent Space for Representing Neural Signed Distance Functions with High-fidelity Geometry Details](https://arxiv.org/abs/2511.14539)
*Qiang Bai,Bojian Wu,Xi Yang,Zhizhong Han*

Main category: cs.CV

TL;DR: 提出了一种在共享空间中表示多个神经符号距离函数（SDF）的方法，通过结合泛化学习和过拟合学习策略的优势，以更紧凑的潜在表示恢复高保真几何细节。


<details>
  <summary>Details</summary>
Motivation: 传统的神经SDF在单个形状或场景上表现良好，但在分析多个SDF时存在困难，因为潜在空间编码信息有限且会丢失几何细节。

Method: 结合泛化学习和过拟合学习策略的优势，提出新颖的采样策略来提高训练效率并消除其他SDF影响造成的伪影。

Result: 在广泛使用的基准测试中进行了数值和视觉评估，验证了设计有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法能够以更紧凑的潜在表示恢复更多高保真几何细节，为多个SDF的表示提供了有效解决方案。
...
### [94] [Interaction-Aware 4D Gaussian Splatting for Dynamic Hand-Object Interaction Reconstruction](https://arxiv.org/abs/2511.14540)
*Hao Tian,Chenyangguang Zhang,Rui Liu,Wen Shen,Xiaolin Qin*

Main category: cs.CV

TL;DR: 本文提出了一种无需物体先验知识的手-物体交互场景几何与外观同时建模方法，基于动态3D高斯泼溅技术，通过交互感知的高斯模型、动态场构建和渐进优化策略，解决了复杂交互中的遮挡、模糊和运动建模问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在手-物体交互场景建模中面临挑战，特别是在没有物体先验知识的情况下，难以处理复杂的相互遮挡、边缘模糊和动态变形问题。本文旨在开发一种能够同时准确建模几何和外观的方法。

Method: 1. 引入交互感知的手-物体高斯模型，采用分段线性假设优化参数；2. 构建交互感知动态场，将手部信息融入物体变形场；3. 提出渐进优化策略，分步处理动态区域和静态背景；4. 设计显式正则化方法确保平滑运动过渡、物理交互真实性和光照一致性。

Result: 实验结果表明，该方法超越了现有的基于动态3D-GS的方法，在手-物体交互动态重建方面达到了最先进的性能水平。

Conclusion: 本文提出的交互感知建模框架能够有效解决手-物体交互场景中的复杂挑战，无需物体先验知识即可实现高质量的几何和外观重建，为动态交互场景建模提供了新的解决方案。
...
### [95] [ForensicFlow: A Tri-Modal Adaptive Network for Robust Deepfake Detection](https://arxiv.org/abs/2511.14554)
*Mohammad Romani*

Main category: cs.CV

TL;DR: ForensicFlow是一个三模态深度伪造检测框架，通过融合RGB、纹理和频率域的伪造痕迹，结合注意力机制实现多尺度伪造检测，在Celeb-DF数据集上达到97.52% AUC。


<details>
  <summary>Details</summary>
Motivation: 现有单流CNN方法无法有效捕捉空间、纹理和频率域的多尺度伪造痕迹，限制了深度伪造检测的鲁棒性和泛化能力。

Method: 提出三模态框架：RGB分支（ConvNeXt-tiny）提取全局视觉不一致性；纹理分支（Swin Transformer-tiny）检测细粒度融合痕迹；频率分支（CNN+SE）识别周期性频谱噪声。使用注意力机制进行时序池化和分支融合。

Result: 在Celeb-DF数据集上达到AUC 0.9752、F1分数0.9408、准确率0.9208，优于单流基线方法。消融实验验证了分支协同作用，Grad-CAM确认了检测焦点。

Conclusion: 多模态特征融合提供了对细微伪造的更强鲁棒性，为深度伪造检测提供了全面的解决方案。
...
TL;DR: OmniZip是一个无需训练的音频引导音频-视频令牌压缩框架，通过识别显著音频令牌并计算音频保留分数来动态指导视频令牌剪枝，实现3.42倍推理加速和1.4倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 处理音频-视频令牌序列在多模态大语言模型中产生了显著的计算瓶颈，现有令牌压缩方法尚未满足联合压缩多模态令牌的新兴需求。

Method: 首先识别显著音频令牌，计算每个时间组的音频保留分数以捕捉信息密度，动态指导视频令牌剪枝，并使用交错时空方案压缩视频令牌。

Result: 实现了3.42倍推理加速和1.4倍内存减少，同时保持性能，无需训练。

Conclusion: OmniZip有效解决了多模态令牌压缩问题，显著提升了推理效率，为统一音频-视频理解提供了实用解决方案。
...
### [97] [XAttn-BMD: Multimodal Deep Learning with Cross-Attention for Femoral Neck Bone Mineral Density Estimation](https://arxiv.org/abs/2511.14604)
*Yilin Zhang,Leo D. Westbury,Elaine M. Dennison,Nicholas C. Harvey,Nicholas R. Fuggle,Rahman Attar*

Main category: cs.CV

TL;DR: XAttn-BMD是一个多模态深度学习框架，通过髋关节X射线图像和临床元数据预测股骨颈骨密度，采用双向交叉注意力机制实现跨模态特征融合，在Hertfordshire队列研究中表现出优越的回归泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 骨健康不良是重要的公共卫生问题，低骨密度会增加骨折风险，这是骨质疏松症的关键特征。传统方法在骨密度预测方面存在局限性，需要开发更准确的多模态预测模型。

Method: 提出XAttn-BMD框架，使用新颖的双向交叉注意力机制动态整合X射线图像和临床元数据特征，采用加权平滑L1损失函数处理骨密度不平衡问题并优先考虑临床重要病例。

Result: 模型在Hertfordshire队列研究中优于基线模型，通过交叉注意力融合相比简单特征拼接，MSE降低16.7%，MAE降低6.03%，R2得分提高16.4%。在临床相关骨密度阈值下的二元分类也显示出良好的筛查性能。

Conclusion: XAttn-BMD框架通过有效的跨模态融合和定制化损失函数，在股骨颈骨密度估计方面表现出色，具有在真实世界场景中应用的潜力。
...
### [98] [3D-Guided Scalable Flow Matching for Generating Volumetric Tissue Spatial Transcriptomics from Serial Histology](https://arxiv.org/abs/2511.14613)
...
TL;DR: HoloTea是一个3D感知的流匹配框架，通过从H&E染色图像中估算点级基因表达，并利用相邻切片信息来生成3D组织转录组学图谱。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么独立处理每个切片忽略3D结构，要么3D感知方法不可生成且扩展性差，需要开发可扩展的3D组织转录组学分析方法。

Method: 在共享特征空间中检索相邻切片上的形态对应点，将跨切片上下文融合到轻量级ControlNet中；引入结合学习到的ZINB先验和空间经验先验的3D一致先验；使用全局注意力块实现线性扩展。
...
### [99] [Fusing Biomechanical and Spatio-Temporal Features for Fall Prediction: Characterizing and Mitigating the Simulation-to-Reality Gap](https://arxiv.org/abs/2511.14620)
*Md Fokhrul Islam,Sajeda Al-Hammouri,Christopher J. Arellano,Kavan Hazeli,Heman Shakeri*

Main category: cs.CV

TL;DR: 该研究提出了BioST-GCN双流模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据集上表现优于基线，但存在显著的模拟-现实差距，零样本泛化性能大幅下降。
...
Result: 在模拟数据集上F1分数比基线ST-GCN提高5.32%和2.91%，但零样本泛化到未见受试者时F1分数从89.0%降至35.9%。

Conclusion: 模拟数据存在偏差，需要个性化策略和隐私保护数据管道来弥合模拟-现实差距，为脆弱老年群体开发有效的跌倒预测系统。
...
### [100] [SparseSurf: Sparse-View 3D Gaussian Splatting for Surface Reconstruction](https://arxiv.org/abs/2511.14633)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图下高斯溅射优化容易过拟合，导致重建质量不佳。现有方法使用扁平高斯基元结合深度正则化，但扁平高斯固有的各向异性会加剧稀疏视图下的过拟合问题。

Method: 提出立体几何-纹理对齐来连接渲染质量和几何估计，同时引入伪特征增强几何一致性，通过结合训练视图和未见视图来强制执行多视图几何一致性。
...
### [101] [SLAM-AGS: Slide-Label Aware Multi-Task Pretraining Using Adaptive Gradient Surgery in Computational Cytology](https://arxiv.org/abs/2511.14639)
*Marco Acerbis,Swarnadip Chatterjee,Christophe Avenel,Joakim Lindblad*

Main category: cs.CV

TL;DR: SLAM-AGS是一个针对计算细胞学的多任务预训练框架，通过联合优化弱监督相似性目标和自监督对比目标，解决了实例级标签不可靠和目击率极低的问题，并在低目击率下显著提升下游任务性能。
...
Method: 提出SLAM-AGS框架，联合优化弱监督相似性目标（在幻灯片负补丁上）和自监督对比目标（在幻灯片正补丁上），应用自适应梯度手术解决任务梯度冲突，防止模型崩溃。将预训练编码器集成到基于注意力的多实例学习聚合器中。
...
### [102] [Signature vs. Substance: Evaluating the Balance of Adversarial Resistance and Linguistic Quality in Watermarking Large Language Models](https://arxiv.org/abs/2511.13722)
...
<details>
  <summary>Details</summary>
Motivation: 为了减轻LLM生成文本的潜在危害，研究人员提出了水印技术，但现有技术存在降低文本质量和易受对抗攻击的问题，这阻碍了水印技术的广泛应用。

Method: 通过比较改述和回译（英语→其他语言→英语）两种对抗攻击方式，评估多种水印技术的鲁棒性；使用语言指标来衡量水印文本在质量和写作风格方面与未加水印文本的差异。

Result: 研究结果表明，这些水印技术能够保持语义，但会偏离未加水印文本的写作风格，并且容易受到对抗攻击，特别是回译攻击。

Conclusion: 当前的水印技术在保持文本质量和抵抗对抗攻击方面存在不足，需要进一步改进以促进其在LLM中的广泛应用。
...
### [103] [Refine Thought: A Test-Time Inference Method for Embedding Model Reasoning](https://arxiv.org/abs/2511.13726)
*Guangzhi Wang,Kai Li,Yinghao Jiao,Zhi Liu*

Main category: cs.CL

TL;DR: RT是一种通过多次前向传递增强文本嵌入模型语义推理能力的方法，在语义推理任务上表现显著提升，同时保持通用语义理解任务的性能。
...
Method: 通过多次前向传递运行文本嵌入模型来获得最终语义表示，这是一种测试时推理方法。

Result: 在BRIGHT和PJBenchmark1语义推理任务上取得显著改进，在C-MTEB等通用语义理解任务上保持稳定性能。

Conclusion: RT方法能有效增强文本嵌入模型的语义推理能力，特别适用于基于解码器的文本嵌入模型如Qwen3-Embedding-8B。
...
### [104] [Can QE-informed (Re)Translation lead to Error Correction?](https://arxiv.org/abs/2511.13884)
*Govardhan Padmanabhan*

Main category: cs.CL

TL;DR: 本文提出了两种无需训练的方法用于WMT 2025自动翻译质量评估任务：QE-informed重翻译和基于QE解释的错误替换方法，前者通过选择多个LLM生成的最佳翻译获得优胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE系统和APE系统能提升性能，但APE系统存在过度修正问题，导致性能下降。本文旨在探索无需训练的替代方法。

Method: 方法一：QE-informed重翻译，从多个LLM生成的候选中选择质量最高的翻译；方法二：基于QE解释的错误替换，使用LLM根据QE解释替换错误子串，并采用条件启发式最小化编辑次数。

Result: 两种方法的Delta COMET得分分别为0.0201和-0.0108，第一种方法在子任务排行榜上获得优胜位置。

Conclusion: 无需训练的QE-informed重翻译方法在翻译质量评估任务中表现优于基于QE解释的错误替换方法，证明了从多个LLM候选中选择最佳翻译的有效性。
...
### [105] [What Works for 'Lost-in-the-Middle' in LLMs? A Study on GM-Extract and Mitigations](https://arxiv.org/abs/2511.13900)
*Mihir Gupte,Eshan Dixit,Muhammad Tayyab,Arun Adiththan*

Main category: cs.CL

TL;DR: 论文研究了大型语言模型在长文本上下文中的"迷失在中间"现象，提出了GM-Extract基准数据集来评估控制变量检索性能，开发了空间和语义检索能力评估指标，系统评估了7-8B参数模型，并分析了缓解方法的效果。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在检索应用中长距离上下文利用能力下降的问题，即"迷失在中间"现象，这对基于检索的LLM应用构成重大挑战。

Method: 提出了GM-Extract基准数据集，使用两个评估指标（文档指标和变量提取指标）来诊断故障模式，系统评估了7-8B参数模型在两种多文档任务上的表现，并调查了缓解方法。

Result: 发现仅通过改变数据在上下文窗口中的表示方式就能显著改变检索性能，虽然未始终观察到明显的U形曲线，但发现了清晰的性能模式，并与困惑度得分相关。缓解方法的效果具有高度细微差别。

Conclusion: 评估突出了这些策略成功改进性能的场景，以及它们产生负面影响的意外情况，为实际应用中的效用提供了全面理解。
...
### [106] [Hint-Augmented Re-ranking: Efficient Product Search using LLM-Based Query Decomposition](https://arxiv.org/abs/2511.13994)
*Yilun Zhu,Nikhita Vedula,Shervin Malmasi*

Main category: cs.CL

TL;DR: 提出一个框架，利用LLM从包含最高级（如“最好”、“最受欢迎”）的电商搜索查询中提取结构化解释，通过将查询分解为属性-值提示来改进搜索性能，并开发高效方法将解释转移到轻量级模型以解决延迟问题。


<details>
  <summary>Details</summary>
Motivation: 包含最高级的搜索查询需要在多个维度比较候选对象，需要语言理解和领域知识，现有方法难以有效处理这类查询的潜在意图。

Method: 开发一个框架，将查询分解为属性-值提示，这些提示与检索过程同时生成，然后通过将最高级解释转移到轻量级模型来实现高效集成。

Result: 在MAP指标上提升10.9个百分点，在MRR排名指标上提升5.9个百分点，相比基线方法有显著改进。
...
### [107] [HiEAG: Evidence-Augmented Generation for Out-of-Context Misinformation Detection](https://arxiv.org/abs/2511.14027)
...
Method: 提出HiEAG框架，将外部一致性检查分解为检索、重排序和重写的综合引擎流程，使用AESP进行证据重排序和AEGP进行证据重写，通过指令微调实现判断解释。

Result: 在不同基准数据集上的实验结果表明，HiEAG在所有样本的准确率上超越了之前的最先进方法。

Conclusion: HiEAG框架通过分层证据增强生成有效提升了OOC虚假信息检测的性能，证明了外部一致性检查的重要性。
...
### [108] [Based on Data Balancing and Model Improvement for Multi-Label Sentiment Classification Performance Enhancement](https://arxiv.org/abs/2511.14073)
*Zijin Su,Huanzhu Lv,Yuren Niu,Yiming Liu*

Main category: cs.CL

TL;DR: 本文构建了一个平衡的多标签情感数据集，并开发了增强的多标签分类模型，通过数据平衡策略和混合架构显著提升了情感分类性能。
...
Method: 通过整合原始GoEmotions数据、使用RoBERTa-base-GoEmotions模型标注的Sentiment140样本以及GPT-4 mini生成的手动标注文本，构建平衡数据集；开发结合FastText嵌入、卷积层、双向LSTM和注意力机制的混合模型，使用sigmoid激活输出层进行多标签预测，并采用混合精度训练提高效率。

Result: 实验结果表明，与在非平衡数据上训练的模型相比，该方法在准确率、精确率、召回率、F1分数和AUC方面均有显著提升。

Conclusion: 该方法通过数据平衡策略和优化的模型架构，有效解决了多标签情感分类中的类别不平衡问题，显著提升了分类性能。
...
### [109] [Stealth Fine-Tuning: Efficiently Breaking Alignment in RVLMs Using Self-Generated CoT](https://arxiv.org/abs/2511.14106)
...
<details>
  <summary>Details</summary>
Motivation: 发现RVLMs暴露的思维链轨迹引入了新的攻击面，现有的安全对齐机制容易被突破，需要研究更有效的攻击方法来评估模型安全性。
...
Conclusion: Stealth Fine-Tuning是一种低成本、高效的方法，能够有效绕过对齐防御，揭示了RVLMs安全性的脆弱性。
...
### [110] [Synthetic Clinical Notes for Rare ICD Codes: A Data-Centric Framework for Long-Tail Medical Coding](https://arxiv.org/abs/2511.14112)
*Truong Vo,Weiyi Wu,Kaize Ding*

Main category: cs.CL

TL;DR: 提出了一种数据中心的框架，通过生成高质量合成出院摘要来解决ICD编码中的极端长尾分布问题，改善了罕见和零样本ICD代码的预测性能。


<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码在医疗NLP中至关重要，但受到诊断代码极端长尾分布的阻碍。数千个罕见和零样本ICD代码在MIMIC-III等数据集中严重代表性不足，导致宏观F1分数较低。

Method: 构建了一个数据中心的框架，通过利用真实世界共现模式、ICD描述、同义词、分类法和相似临床笔记，生成以罕见代码为基础的现实多标签代码集。使用这些结构化提示生成了90,000个合成笔记，覆盖7,902个ICD代码。

Result: 在两个最先进的基于transformer的模型PLM-ICD和GKI-ICD上进行微调实验，结果显示该方法适度提高了宏观F1分数，同时保持了强大的微观F1分数，优于先前的最先进方法。

Conclusion: 虽然相对于计算成本而言增益可能显得边际，但结果表明精心设计的合成数据可以增强长尾ICD代码预测的公平性。
...
### [111] [Applying Relation Extraction and Graph Matching to Answering Multiple Choice Questions](https://arxiv.org/abs/2511.14144)
*Naoki Shimoda,Akihiro Yamamoto*

Main category: cs.CL

TL;DR: 该研究将基于Transformer的关系提取与知识图谱匹配相结合，用于回答多项选择题，同时保持输出过程的可追溯性。通过将问题句子转换为关系图并与事实正确的知识图谱进行验证，该方法能够正确回答约70%的问题。


<details>
  <summary>Details</summary>
Motivation: 由于知识图谱构建成本高，传统上被视为静态数据库。但基于Transformer的关系提取方法使得能够通过自然语言文本动态生成知识图谱，从而为表示输入句子的含义提供了可能性。

Method: 提出一种回答填空题格式多项选择题的方法：首先使用关系提取方法将问题句子转换为关系图，然后在封闭世界假设下将其与事实正确的知识图谱进行验证，以衡量每个问题句子的真实性。

Result: 实验结果表明，该方法能够正确回答约70%的问题，同时提供程序的可追溯性。研究还发现问题类别对准确性有显著影响。

Conclusion: 该方法成功结合了关系提取和知识图谱匹配，在保持输出过程可追溯性的同时，有效回答了多项选择题，证明了动态生成知识图谱在问答任务中的实用性。
...
### [112] [Selective Weak-to-Strong Generalization](https://arxiv.org/abs/2511.14166)
...
<details>
  <summary>Details</summary>
Motivation: 未来超人类模型将超越人类能力，人类只能对超人类模型进行弱监督。现有弱到强泛化方法普遍使用弱监督，但部分弱标签对模型有害，需要避免不必要地使用弱监督。

Method: 提出选择性弱到强泛化框架：训练二元分类器P(IK)识别强模型能回答的问题，使用其自生成标签进行对齐；通过图平滑方法优化弱标签。

Result: 在三个基准测试上的广泛实验表明，该方法始终优于竞争基线。进一步分析显示P(IK)能够跨任务和难度泛化。

Conclusion: 选择性弱到强泛化有助于实现超对齐，为未来超人类模型的监督提供了有效解决方案。
...
### [113] [SymLoc: Symbolic Localization of Hallucination across HaluEval and TruthfulQA](https://arxiv.org/abs/2511.14172)
*Naveen Lamba,Sanju Tiwari,Manas Gaur*

Main category: cs.CL

TL;DR: 本文提出了首个基于符号语言知识的幻觉定位框架，通过分析符号触发器（如修饰语、否定、数字等）在模型各层的处理过程，揭示幻觉起源于早期层（2-4层）的符号语义处理崩溃。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在符号触发器上存在严重幻觉问题，但缺乏对幻觉起源的清晰理解。现有定位方法忽视符号语言知识的作用，无法解释符号元素如何驱动幻觉失败。

Method: 提出符号定位框架，利用符号语言和语义知识追踪幻觉在模型各层的发展。使用HaluEval和TruthfulQA数据集分析五个模型，重点关注符号触发器的处理过程。

Result: 发现符号语言元素的注意力方差在早期层（2-4）爆炸性增长，否定触发灾难性方差水平。尽管模型规模增大，幻觉率仍保持高位（78.3%-83.7%），深层出现符号语义触发器的注意力陡降。

Conclusion: 幻觉本质上是符号语言处理失败，而非一般生成问题。符号语义知识是理解和定位LLMs幻觉机制的关键。
...
### [114] [Harnessing Deep LLM Participation for Robust Entity Linking](https://arxiv.org/abs/2511.14181)
*Jiajun Hou,Chenyu Zhang,Rui Meng*

Main category: cs.CL

TL;DR: DeepEL是一个将大语言模型全面整合到实体链接各个阶段的框架，通过自验证机制利用全局上下文信息纠正预测，在10个基准数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只在实体链接的孤立阶段使用LLMs，未能充分利用LLMs在整个流程中的能力，且孤立消歧无法达到最优性能。

Method: 提出DeepEL框架，将LLMs整合到实体链接的每个阶段，并引入自验证机制利用全局上下文信息来纠正预测和识别实体间的连贯关系。

Result: 在10个基准数据集上，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在域外数据集上提升4%。

Conclusion: 深度整合LLMs能有效推进实体链接技术的前沿发展，自验证机制和全局上下文利用是关键成功因素。
...
### [115] [ArbESC+: Arabic Enhanced Edit Selection System Combination for Grammatical Error Correction Resolving conflict and improving system combination in Arabic GEC](https://arxiv.org/abs/2511.14230)
*Ahlam Alrehili,Areej Alhothali*

Main category: cs.CL

TL;DR: 本文提出了ArbESC+系统，这是首个用于阿拉伯语语法错误校正的多系统方法，通过结合多个模型生成修正建议，使用分类器选择最佳修正，在多个测试集上取得了优于单一模型的结果。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语具有复杂的形态和句法结构，比其它语言更具挑战性。虽然现代神经模型近年来有很大改进，但大多数先前尝试使用单一模型，没有考虑组合不同系统的潜在优势。

Method: 提出ArbESC+系统，使用多个模型（AraT5、ByT5、mT5、AraBART等）收集修正建议，将其表示为数值特征，通过分类器确定和实施适当修正，并使用支持技术过滤重叠修正和估计决策可靠性。
...
### [116] [MuCPT: Music-related Natural Language Model Continued Pretraining](https://arxiv.org/abs/2511.14245)
*Kai Tian,Yirong Mao,Wendong Bi,Hanjie Wang,Que Wenhui*

Main category: cs.CL

TL;DR: 该论文构建了一个大规模音乐相关自然语言语料库（40B tokens），采用领域优先的数据处理流程，并引入基于参考模型的token级软评分进行质量控制，以提升大语言模型在音乐领域的专业表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现良好，但在音乐等专业领域受到限制，主要因为语料规模、纯净度以及数据与训练目标的匹配度不足。

Method: 构建大规模音乐语料库，采用轻量级分类器进行领域文本过滤和加权，多阶段清洗、去重和隐私保护掩码，集成多源音乐文本与元数据，引入基于参考模型的token级软评分进行数据选择和动态优化。

Result: 开发了MusicSimpleQA基准测试用于事实性评估，通过系统比较数据组成的不同维度，验证了所提方法的有效性。

Conclusion: 该工作通过正确的语料库和训练目标，为音乐领域大语言模型的构建提供了可扩展的数据训练框架和可重用的评估工具。
...
### [117] [Towards Authentic Movie Dubbing with Retrieve-Augmented Director-Actor Interaction Learning](https://arxiv.org/abs/2511.14249)
*Rui Liu,Yuan Zhao,Zhenqi Jia*

Main category: cs.CL

TL;DR: 提出了一种名为Authentic-Dubber的电影自动配音模型，通过检索增强的导演-演员交互学习方案，模拟真实配音工作流程中的导演指导过程，显著提升了情感表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有电影自动配音方法简化了工作流程，忽略了导演与演员之间的关键互动环节，而真实的配音工作涉及导演指导演员内化情感线索的动态协作过程。

Method: 构建多模态参考片段库模拟导演提供的学习素材；提出基于情感相似度的检索增强策略；开发渐进式图基语音生成方法逐步整合检索到的多模态情感知识。

Result: 在V2C Animation基准数据集上的主观和客观评估验证了该方法的有效性，在情感表达方面实现了全面改进。

Conclusion: Authentic-Dubber能够忠实地复制真实的配音工作流程，通过模拟导演-演员交互显著提升了配音的情感表现力。
...
### [118] [AfriSpeech-MultiBench: A Verticalized Multidomain Multicountry Benchmark Suite for African Accented English ASR](https://arxiv.org/abs/2511.14255)
*Gabrial Zencha Ashungafac,Mardhiyah Sanni,Busayo Awobade,Alex Gichamba,Tobi Olatunji*

Main category: cs.CL

TL;DR: AfriSpeech-MultiBench是首个针对非洲英语口音的领域特定评估套件，涵盖100多种口音、10多个国家和7个应用领域，评估了各种语音识别系统在非洲语境下的表现。
...
Result: 实证分析显示系统性差异：开源ASR在自然语音中表现优异但在嘈杂非母语对话中退化；多模态LLM对口音更鲁棒但在领域特定命名实体上表现不佳；专有模型在干净语音中准确率高但随国家和领域变化显著。
...
### [119] [Entropy-Guided Reasoning Compression](https://arxiv.org/abs/2511.14258)
*Hourun Zhu,Yang Gao,Wenlong Fei,Jiawei Li,Huashan Sun*

Main category: cs.CL

TL;DR: 该论文提出了一种熵引导的训练框架来解决大型推理模型在压缩训练过程中出现的熵冲突问题，能够在保持或提升准确率的同时将推理长度压缩到原来的20%。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但其思维链输出过长导致高计算成本和部署困难。现有压缩方法部分成功但忽略了训练过程中的熵冲突现象——压缩目标降低熵导致推理变短但探索受限，而性能目标增加熵导致推理链变长，使模型陷入局部困境。

Method: 采用熵引导的训练框架：当熵下降时引导模型进行简洁的推理步骤；当熵上升时在紧凑推理模式下加强探索以提高鲁棒性。通过分析发现熵冲突源于高熵的逻辑连接词在性能目标下获得更大梯度而被鼓励，而压缩目标同时惩罚这些可能冗余的连接词。

Result: 在六个数学基准测试上的实验表明，该方法将推理长度压缩到原来的20%，同时保持甚至超过了基线准确率。

Conclusion: 熵引导训练框架有效解决了推理模型压缩中的熵冲突问题，实现了推理长度的大幅压缩而不损失性能，为实际部署提供了可行的解决方案。
...
### [120] [Don't Miss the Forest for the Trees: In-Depth Confidence Estimation for LLMs via Reasoning over the Answer Space](https://arxiv.org/abs/2511.14275)
*Ante Wang,Weizhi Ma,Yang Liu*

Main category: cs.CL

TL;DR: 本文提出了一种通过预测语言化概率分布来改进LLM置信度估计的方法，该方法鼓励模型进行深入推理，考虑答案空间中的所有候选答案，并在不同模型和任务中表现出优势。


<details>
  <summary>Details</summary>
Motivation: 了解模型响应的可靠性在应用中至关重要。虽然已有研究关注生成语言化置信度，并通过思维链推理提供逻辑透明的估计，但推理策略如何影响置信度估计仍未被充分探索。
...
Conclusion: 预测语言化概率分布能有效促进置信度估计的深度推理，该方法具有广泛适用性和鲁棒性，其推理模式符合人类认知期望。
...
### [121] [AraLingBench A Human-Annotated Benchmark for Evaluating Arabic Linguistic Capabilities of Large Language Models](https://arxiv.org/abs/2511.14295)
...
TL;DR: AraLingBench是一个完全人工标注的阿拉伯语语言能力评估基准，包含150个专家设计的多选题，涵盖语法、形态学、拼写、阅读理解和句法五个核心类别，用于评估35个阿拉伯语和双语大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在阿拉伯语评估中表现出表面水平的熟练度，但在深层语法和句法推理方面存在困难，需要专门的基准来区分真实语言掌握与模式识别能力。

Method: 构建包含150个专家设计多选题的基准，涵盖五个核心语言类别，评估35个阿拉伯语和双语大语言模型的结构性语言理解能力。

Result: 评估显示当前模型在表面水平表现出色，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解获得高分。

Conclusion: AraLingBench揭示了基于知识的基准高分与真实语言掌握之间的持续差距，为开发阿拉伯语大语言模型提供了诊断框架。
...
### [122] [ConInstruct: Evaluating Large Language Models on Conflict Detection and Resolution in Instructions](https://arxiv.org/abs/2511.14342)
*Xingwei He,Qianru Zhang,Pengfei Chen,Guanhua Chen,Linlin Yu,Yuan Yuan,Siu-Ming Yiu*

Main category: cs.CL

TL;DR: ConInstruct是一个专门评估大语言模型检测和解决用户指令中冲突能力的基准测试。研究发现大多数专有LLM具有强大的冲突检测能力，但面对冲突约束时很少明确通知用户或请求澄清。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注LLM如何遵循用户指令，但忽略了指令包含冲突约束的常见场景，LLM在此类条件下的行为研究不足。

Method: 引入ConInstruct基准测试，专门设计用于评估LLM检测和解决用户指令中冲突的能力，使用该数据集评估LLM的冲突检测性能并分析其冲突解决行为。

Result: 专有LLM表现出强大的冲突检测能力，DeepSeek-R1和Claude-4.5-Sonnet分别以91.5%和87.3%的平均F1分数排名前二；但LLM很少明确通知用户冲突或请求澄清。
...
### [123] [ATLAS: A High-Difficulty, Multidisciplinary Benchmark for Frontier Scientific Reasoning](https://arxiv.org/abs/2511.14366)
...
TL;DR: ATLAS是一个面向AGI的大规模、高难度跨学科科学评估套件，包含约800个原创问题，涵盖7个核心科学领域，旨在解决现有基准测试在区分前沿模型能力方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在性能饱和、学科范围狭窄、答案格式简化、易受数据污染等问题，与真实科学探究存在保真度差距，需要开发更可靠的评估工具来推动AGI发展。

Method: 由领域专家开发原创问题，采用多阶段专家同行评审和对抗性测试进行质量控制，使用LLM评委小组进行复杂答案的自动化评估。

Result: 初步结果表明ATLAS能有效区分领先模型的先进科学推理能力，展示了其在评估前沿模型方面的有效性。

Conclusion: ATLAS计划发展成为一个长期、开放、社区驱动的平台，为AGI进展提供可靠的衡量标准。
...
### [124] [Mitigating Label Length Bias in Large Language Models](https://arxiv.org/abs/2511.14385)
*Mario Sanz-Guerrero,Katharina von der Wense*

Main category: cs.CL

TL;DR: 本文提出标准化上下文校准(NCC)方法，解决大语言模型中多标记类别标签的长度偏差问题，在多个数据集和模型上显著提升性能，F1分数最高提升10%。
...
Method: 提出标准化上下文校准(NCC)方法，在全标签级别进行标准化和校准，扩展偏差缓解到更广泛的任务如多项选择题回答。

Result: NCC在多个数据集和模型上取得统计显著改进，F1分数最高提升10%，结合上下文学习时对少样本示例选择更不敏感，需要更少示例达到竞争性能，产生更可靠的置信度估计。

Conclusion: 缓解全标签偏差对于提高基于大语言模型方法的性能和鲁棒性至关重要，特别是在类别标签自然包含多个标记的实际应用中。
...
### [125] [Unified Defense for Large Language Models against Jailbreak and Fine-Tuning Attacks in Education](https://arxiv.org/abs/2511.14423)
*Xin Yi,Yue Li,Dongsheng Shi,Linlin Wang,Xiaoling Wang,Liang He*

Main category: cs.CL

TL;DR: 本文提出了EduHarm基准和TSSF框架，用于评估和增强教育场景下大语言模型的安全性，同时防御越狱攻击和微调攻击。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注通用安全性评估，对教育场景独特安全需求关注不足。大语言模型在教育应用中易受越狱和微调攻击，可能导致有害输出。

Method: 构建EduHarm基准包含五个教育场景的安全-不安全指令对；提出三阶段防护框架TSSF：安全感知注意力重对齐、分层安全判断、防御驱动双路由。

Result: 在八种越狱攻击策略上的实验表明TSSF有效增强安全性且避免过度拒绝良性查询；在三个微调攻击数据集上的评估显示其持续实现稳健防御同时保持良性微调的效用增益。

Conclusion: TSSF框架能够同时缓解越狱和微调攻击，在教育场景下有效增强大语言模型的安全性，同时保持对良性查询的正常处理能力。
...
### [126] [MedBench v4: A Robust and Scalable Benchmark for Evaluating Chinese Medical Language Models, Multimodal Models, and Intelligent Agents](https://arxiv.org/abs/2511.14439)
...
TL;DR: MedBench v4是一个全国性的医疗AI基准测试平台，包含70多万个专家策划的任务，涵盖24个主要专科和91个次要专科，专门评估LLM、多模态模型和智能体在真实临床工作流程中的表现。
...
Method: 构建包含70多万个任务的云基准测试基础设施，任务经过多阶段细化和多轮临床医生评审，开放式回答使用经过人类评分校准的LLM作为评判员进行评分。
...
Conclusion: MedBench v4揭示了基础模型在多模态推理和安全性方面的持续差距，同时表明具有治理意识的智能体编排可以显著提高基准临床准备度而不牺牲能力。
...
### [127] [Tell Me: An LLM-powered Mental Well-being Assistant with RAG, Synthetic Dialogue Generation, and Agentic Planning](https://arxiv.org/abs/2511.14445)
*Trishala Jayesh Ahalpara*

Main category: cs.CL

TL;DR: Tell Me是一个心理健康系统，利用大语言模型提供个性化、情境感知的心理支持，包含RAG对话助手、合成对话生成器和AI健康规划团队三个组件。


<details>
  <summary>Details</summary>
Motivation: 解决心理健康资源获取障碍，为研究提供合成治疗对话数据，通过AI技术补充现有护理服务。

Method: 系统集成三个核心组件：基于检索增强生成的个性化对话助手、基于用户档案的合成治疗对话生成器、以及使用CrewAI实现的健康规划团队。

Result: 系统在精心设计的健康场景中进行了评估，包括自动LLM判断和人类用户研究，展示了有效的个性化支持能力。
...
### [128] [Agent-R1: Training Powerful LLM Agents with End-to-End Reinforcement Learning](https://arxiv.org/abs/2511.14460)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对LLM智能体的强化学习方法研究，以及灵活可扩展的训练框架，这阻碍了RL在LLM智能体中的有效应用。

Method: 1. 系统性地将MDP框架扩展到LLM智能体定义关键组件；2. 提出模块化、灵活、用户友好的Agent-R1训练框架。

Result: 在多跳问答基准任务上的实验初步验证了所提方法和框架的有效性。

Conclusion: 本文为RL在LLM智能体中的应用提供了理论框架和实践工具，有助于推动该领域的发展。
...
TL;DR: LiveRAG是一个包含895个合成问答对的公开基准数据集，用于系统评估RAG问答系统的有效性，源自SIGIR'2025 LiveRAG挑战赛，包含真实答案、支持声明、难度和区分度评分。
...
Method: 基于SIGIR'2025 LiveRAG挑战赛使用的数据集构建，包含895个合成问答对，添加了真实答案、支持声明，并应用项目反应理论模型计算难度和区分度评分。

Result: 分析显示基准问题具有多样性、广泛的难度范围，能有效区分系统能力，有助于推进RAG研究和系统评估。
...
TL;DR: 本文探讨了文档级主张提取的评估方法，通过对齐两组主张并计算相似度得分来评估模型提取性能。


<details>
  <summary>Details</summary>
Motivation: 文档级主张提取在事实核查领域仍面临挑战，现有评估方法关注有限，需要可靠的评估框架来比较模型提取和人工标注的主张集。

Method: 研究对齐两组主张的技术，识别最佳对齐和评估方法，通过对齐得分计算相似度，并在捷克和斯洛伐克新闻评论数据集上进行实验。
...
Conclusion: 本研究展示了一种数据驱动的方法来识别高风险AKI患者，为早期CKD检测和干预的决策支持工具开发提供了支持。
...
TL;DR: 本研究比较了人类标注与多种大语言模型在政治偏见检测中的表现，发现RoBERTa在传统Transformer模型中与人类标注最一致，而GPT在零样本设置下与人类标注整体一致性最强。
...
Result: 在传统Transformer模型中，RoBERTa与人类标签对齐度最高；生成模型如GPT在零样本设置下与人类标注整体一致性最强；微调的RoBERTa模型在所有基线中获得了最高准确率和最强的人类标注对齐度。
...
### [133] [Enhancing Agentic Autonomous Scientific Discovery with Vision-Language Model Capabilities](https://arxiv.org/abs/2511.14631)
*Kahaan Gandhi,Boris Bolliet,Inigo Zubeldia*

Main category: cs.CL

TL;DR: 基于视觉语言模型的多智能体系统通过将图表作为可验证检查点，显著提升了端到端自主科学发现能力，在宇宙学和天体化学案例中实现了错误推理路径的恢复和新数据集的自主适应。


<details>
  <summary>Details</summary>
Motivation: 解决传统代码驱动或文本增强方法在自主科学发现中推理错误纠正困难、适应性差的问题，提高科学数据分析的自主性和可靠性。

Method: 使用视觉语言模型作为评判者，通过动态生成的领域特定评分标准评估图表，使智能体能够实时纠正错误并引导探索性数据分析。

Result: 在10个数据驱动发现任务基准测试中，VLM增强系统达到0.7-0.8的Pass@1分数，显著优于代码基线(0.2-0.3)和代码-文本基线(0.4-0.5)，同时提供可审计的推理轨迹。

Conclusion: 视觉语言模型引导的多智能体系统能够有效提升自主科学发现的性能和可解释性，为数据驱动研究提供了新的技术路径。
...
### [134] [A Specialized Large Language Model for Clinical Reasoning and Diagnosis in Rare Diseases](https://arxiv.org/abs/2511.14638)
...
TL;DR: RareSeek R1是一个专门用于罕见病诊断的AI系统，通过领域特定的临床语料库和分阶段指令调优，在多中心电子健康记录和公共基准测试中达到最先进水平，性能与经验丰富的医生相当。
...
Method: 构建大型领域特定临床语料库和临床医生验证的推理集，通过分阶段指令调优、思维链学习和图基检索开发RareSeek R1系统。
...
### [135] [Graded strength of comparative illusions is explained by Bayesian inference](https://arxiv.org/abs/2511.14642)
*Yuhan Zhang,Erxiao Wang,Cory Shain*

Main category: cs.CL

TL;DR: 本研究通过定量建模验证了噪声通道理论在语言处理中的适用性，特别是解释了比较幻觉现象的强度变化和代词与完整名词短语的影响。


<details>
  <summary>Details</summary>
Motivation: 研究比较幻觉现象，验证噪声通道理论是否能统一解释语言处理中的各种现象，包括幻觉和非幻觉情境。

Method: 结合统计语言模型与人类行为数据，构建后验概率定量模型，直接预测幻觉强度，并分析代词与完整名词短语的影响。

Result: 模型成功解释了比较幻觉效应的精细梯度变化，以及之前未解释的代词与完整名词短语主语的影响效应。

Conclusion: 研究支持噪声通道推断作为语言处理现象的统一计算层面理论，为理解语言处理机制提供了新的计算框架。
...
### [136] [DNA Storage in the Short Molecule Regime](https://arxiv.org/abs/2511.14284)
*Ran Tamir,Nir Weinberger,Albert Guillén i Fàbregas*

Main category: cs.IT

TL;DR: 本文完成了DNA存储系统中短DNA分子可靠信息存储容量猜想的证明，提出了两种编码方案，其中随机编码方案实现了与已有逆界匹配的容量，第二种方案计算复杂度更低但适用范围有限。


<details>
  <summary>Details</summary>
Motivation: 研究短DNA分子存储系统中可靠存储的信息量，验证Shomorony和Heckel(2022)提出的容量缩放猜想。

Method: 分析随机编码方案，其中码字通过对概率单纯形中随机生成的概率质量函数进行量化获得，并分析最优最大似然解码器；同时提出第二种计算复杂度更低的编码方案。

Result: 推导出与最近建立的逆界在整个短分子范围内匹配的可实现界；第二种方案在除特定极短分子范围外实现了最优缩放。

Conclusion: 完成了DNA存储系统短分子容量猜想的证明，提供了实现最优容量缩放的编码方案，其中一种具有较低计算复杂度。
...
### [137] [The Capacity of Collusion-Resilient Decentralized Secure Aggregation with Groupwise Keys](https://arxiv.org/abs/2511.14444)
*Zhou Li,Xiang Zhang,Yizhou Zhao,Haiqiang Chen,Jihao Fan,Giuseppe Caire*

Main category: cs.IT

TL;DR: 本文研究了在实用分组密钥和抗串谋条件下的信息论去中心化安全聚合问题，分析了分组密钥大小对通信速率和密钥速率最优区域的影响。


<details>
  <summary>Details</summary>
Motivation: 受高效分组密钥生成协议的启发，研究对称分组密钥设置下的去中心化安全聚合问题，其中每G个用户共享一个独立的分组密钥。

Method: 通过信息论方法分析DSA问题，考虑所有子集大小为G的用户共享独立分组密钥的设置，推导恢复约束和安全约束同时满足的条件。

Result: 当G=1或G≥K-T时DSA不可行；当2≤G<K-T时，每个用户至少广播1个符号，每个分组密钥至少包含(K-T-2)/C(K-T-1,G)个独立符号。

Conclusion: 建立了分组密钥下DSA的基本极限，为去中心化学习系统中通信和密钥高效的安全聚合提供了设计指导。
...
### [138] [Monimial Matrix Analogue of Yoshida's theorem](https://arxiv.org/abs/2511.14480)
*Ananda Chakraborty*

Main category: cs.IT

TL;DR: 本文研究了有限域上线性码的权重枚举器变体，推广了两个线性码的平均完全联合权重枚举器概念，并给出了其MacWilliams型恒等式，建立了Yoshida定理的单项式类似物。


<details>
  <summary>Details</summary>
Motivation: 研究有限域上线性码的权重枚举器变体，旨在推广平均完全联合权重枚举器的概念，建立更一般的理论框架。

Method: 通过推广平均完全联合权重枚举器概念，给出其MacWilliams型恒等式，建立Yoshida定理的单项式类似物，并扩展到g重完全联合权重枚举器的平均表示。

Result: 成功建立了平均完全联合权重枚举器的MacWilliams型恒等式和Yoshida定理的单项式类似物，并推广到g重情况。

Conclusion: 本文为有限域上线性码的权重枚举器理论提供了新的推广结果，建立了平均完全联合权重枚举器的系统理论框架。
...
### [139] [Compression with Privacy-Preserving Random Access](https://arxiv.org/abs/2511.14524)
*Venkat Chandar,Aslan Tchamkerten,Shashank Vatedka*

Main category: cs.IT

TL;DR: 本文证明了一个i.i.d.二进制源序列可以在任何高于熵的速率下进行无损压缩，使得对任意单个比特的解码不会泄露其他比特的任何信息。


<details>
  <summary>Details</summary>
Motivation: 研究在无损压缩过程中如何实现个体比特解码的隐私保护，确保解码某个比特时不会泄露其他比特的信息。

Method: 通过特定的压缩编码方案，使得压缩后的数据在解码单个比特时能够保持其他比特的完全保密性。

Result: 成功实现了在任何高于熵的压缩速率下，都能保证个体比特解码的零信息泄露。

Conclusion: 证明了在无损压缩中实现个体比特隐私保护的可行性，为隐私保护的数据压缩提供了理论基础。
...
### [140] [Imagine in Space: Exploring the Frontier of Spatial Intelligence and Reasoning Efficiency in Vision Language Models](https://arxiv.org/abs/2511.13782)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型和视觉语言模型在逻辑推理、问题解决等方面表现出色，但空间推理（包括心理旋转、导航和空间关系理解）仍然是重大挑战。研究者假设想象力是空间世界模型中的主导推理机制。

Method: 引入SpatiaLite全合成基准测试，联合测量空间推理准确性和效率；通过综合实验分析VLMs的空间推理机制；提出Imagery Driven Framework用于数据合成和训练。

Result: 1) 先进VLMs主要依赖语言表征进行推理和想象，在需要感知空间关系和3D几何变换的视觉中心任务上存在显著缺陷；2) 当前空间推理机制效率严重低下，随着变换复杂度增加，token使用量快速增长；3) IDF框架可以隐式构建对空间推理至关重要的内部世界模型。

Conclusion: 本研究通过SpatiaLite基准测试揭示了先进VLMs的空间推理局限性和模式，识别了关键缺陷，并为未来发展提供了指导。IDF框架展示了构建内部世界模型以改进空间推理的潜力。
...
### [141] [KANGURA: Kolmogorov-Arnold Network-Based Geometry-Aware Learning with Unified Representation Attention for 3D Modeling of Complex Structures](https://arxiv.org/abs/2511.13798)
*Mohammad Reza Shafie,Morteza Hajiabadi,Hamed Khosravi,Mobina Noori,Imtiaz Ahmed*

Main category: cs.AI

TL;DR: KANGURA是一种基于Kolmogorov-Arnold Network的3D机器学习模型，用于优化微生物燃料电池阳极结构，在ModelNet40数据集上达到92.7%准确率，在MFC实际应用中达到97%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型难以捕捉复杂的几何依赖性来优化微生物燃料电池阳极结构，影响MFC性能。

Method: 提出KANGURA框架，将预测建模为函数分解问题，使用KAN-based表示学习重构几何关系，通过几何解耦表示学习分离结构变化，并采用统一注意力机制动态增强关键几何区域。

Result: 在ModelNet40基准数据集上优于15个SOTA模型，准确率达92.7%；在真实MFC阳极结构问题上准确率达97%。
...
### [142] [Rate-Distortion Guided Knowledge Graph Construction from Lecture Notes Using Gromov-Wasserstein Optimal Transport](https://arxiv.org/abs/2511.14595)
...
<details>
  <summary>Details</summary>
Motivation: 将非结构化教育材料（如讲义和幻灯片）转换为能够捕捉关键教学内容的知识图谱仍然很困难，需要一种原则性的方法来构建紧凑且信息保留的知识图谱。

Method: 使用率失真理论和最优传输几何，将讲座内容建模为度量-测度空间，通过Fused Gromov-Wasserstein耦合量化语义失真，应用精炼操作符（添加、合并、拆分、移除、重连）最小化率失真拉格朗日量。

Result: 在数据科学讲座上的原型应用显示，从优化后的知识图谱生成的选择题在15个质量标准上持续优于从原始笔记生成的选择题，并产生了可解释的率失真曲线。
...
### [143] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
*Anna Rodum Bjøru,Rafael Cabañas,Helge Langseth,Antonio Salmerón*

Main category: cs.AI

TL;DR: 本文研究了将Bjøru等人提出的马尔可夫因果模型中的反事实概率边界计算方法扩展到半马尔可夫因果模型的方法，解决了外生变量可能影响多个内生变量的情况。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅适用于马尔可夫模型，其中每个外生变量只影响单个内生变量。但在实际应用中，半马尔可夫模型能表示马尔可夫模型无法表示的混杂关系，因此需要扩展该方法。

Method: 通过最小示例说明扩展挑战，提出替代解决方案策略，并进行理论和计算研究评估。

Result: 开发了适用于半马尔可夫结构因果模型的反事实概率边界计算方法，能够处理外生变量影响多个内生变量的情况。

Conclusion: 成功将分治算法扩展到半马尔可夫模型，为处理更复杂的混杂关系提供了有效工具。
...
### [144] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
*Badhan Chandra Das,Md Tasnim Jawad,Md Jueal Mia,M. Hadi Amini,Yanzhao Wu*

Main category: cs.AI

TL;DR: 本文系统分析了智能交通系统中大型视觉语言模型的安全漏洞，提出了一种利用图像排版操纵和多轮提示的新型越狱攻击方法，并设计了多层响应过滤防御技术。
...
Method: 构建交通相关有害查询数据集；开发基于图像排版操纵和多轮提示的越狱攻击方法；提出多层响应过滤防御技术。

Result: 实验表明所提攻击方法对开源和闭源LVLMs均有效，防御技术能有效阻止模型生成不当响应。GPT-4判断和人工验证确认了攻击和防御的有效性。
...
### [145] [CORGI: Efficient Pattern Matching With Quadratic Guarantees](https://arxiv.org/abs/2511.13942)
*Daniel Weitekamp*

Main category: cs.AI

TL;DR: 本文提出了一种名为CORGI的新匹配算法，用于解决基于规则系统中复杂匹配问题的时间和空间效率问题，特别针对指数级增长的中间部分匹配情况。


<details>
  <summary>Details</summary>
Motivation: 实时AI系统和关系数据库查询需要在高时间约束下解决复杂匹配问题。自动生成的规则容易产生最坏情况匹配模式，导致程序执行缓慢或内存溢出。

Method: CORGI采用两步法：前向传递构建/维护接地关系图，后向迭代器按需生成匹配。不同于RETE的β内存方法，CORGI避免了传统冲突集的内存开销。

Result: 性能评估显示，在简单组合匹配任务中，CORGI显著优于SOAR和OPS5的RETE实现，提供二次时间和空间保证。

Conclusion: CORGI算法通过消除传统冲突集的内存负担，为实时AI系统和数据库查询提供了更可靠和高效的匹配解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 获取真实的工作场所危险场景图像数据集困难，因为捕捉实际发生的事故触发场景几乎不可能，需要一种合成逼真危险场景图像的方法。

Method: 使用GPT-4o分析OSHA事故报告提取结构化危险推理，转换为捕获空间和上下文关系的对象级场景图，然后用这些图指导文本到图像扩散模型生成构图准确的危险场景。
...
### [147] [ALEX:A Light Editing-knowledge Extractor](https://arxiv.org/abs/2511.14018)
...
### [148] [Syn-STARTS: Synthesized START Triage Scenario Generation Framework for Scalable LLM Evaluation](https://arxiv.org/abs/2511.14023)
*Chiharu Hagiwara,Naoki Nonaka,Yuhta Hashimoto,Ryu Uchimido,Jun Seita*

Main category: cs.AI

TL;DR: Syn-STARTS框架使用LLM生成大规模分类案例，解决了大规模伤亡事件中真实数据难以获取的问题，生成的合成数据与人工整理的真实数据在质量上难以区分。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中的分类决策至关重要，但此类事件发生频率低，难以收集足够的高质量真实数据用于AI模型开发和评估。

Method: 开发了Syn-STARTS框架，利用大型语言模型生成分类案例，并与人工整理的TRIAGE开放数据集进行质量对比验证。

Result: Syn-STARTS生成的分类案例在质量上与人工整理的真实数据难以区分，使用START标准分类方法评估LLM准确性时结果高度稳定。
...
### [149] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: 本研究提出了一种由教师主导的反馈循环系统，将概念级评估证据转化为经过验证的微干预措施，通过三种保障机制（充分性、注意力预算、多样性）实现自适应学习。
...
Method: 将干预分配形式化为带约束的二元整数规划，包括覆盖度、时间、难度窗口、概念矩阵编码的先决条件以及通过多样性强化的反冗余约束。采用贪婪选择、梯度松弛和混合方法三种求解策略。

Result: 在模拟和1204名学生的物理课程部署中，两种求解器都实现了几乎所有学习者在有限观看时间内的完整技能覆盖。梯度方法比贪婪方法减少约12%的冗余覆盖，并在难度分布上更均衡。
...
### [150] [Run, Ruminate, and Regulate: A Dual-process Thinking System for Vision-and-Language Navigation](https://arxiv.org/abs/2511.14131)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的视觉语言导航方法存在任务完成性能与领域专家之间的显著差距，且LLM难以精确理解真实世界空间关联，同时带来高昂计算成本和推理延迟。

Method: 提出R3框架：1) Runner - 轻量级基于Transformer的专家模型，确保常规情况下的高效准确导航；2) Ruminator - 采用多模态LLM，使用思维链提示进行结构化推理；3) Regulator - 监控导航进度，根据三个标准控制适当的思维模式。

Result: 在REVERIE基准测试中，R3显著优于其他最先进方法，SPL和RGSPL分别超过3.28%和3.30%。

Conclusion: R3框架在应对具有挑战性的视觉语言导航任务方面表现出显著有效性，成功整合了LLM的泛化能力和领域专业知识。
...
### [151] [Do Large Language Models (LLMs) Understand Chronology?](https://arxiv.org/abs/2511.14214)
*Pattaraphon Kenny Wongchamcharoen,Paul Glasserman*

Main category: cs.AI

TL;DR: 该论文测试了大型语言模型在金融和经济应用中对时间顺序的理解能力，发现模型在处理复杂时间排序任务时存在局限性，但增加推理预算可以显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在金融和经济领域使用大型语言模型时，通常假设模型理解时间顺序，但这一基本假设尚未经过系统测试。

Method: 设计了一系列时间顺序任务，包括时间排序、条件排序（先过滤后排序）和时代错误检测，评估了GPT-4.1、Claude-3.7 Sonnet和GPT-5在不同推理设置下的表现。

Result: 随着序列长度增加，精确匹配率急剧下降，但排名相关性保持较高；条件排序中大多数失败来自过滤步骤；时代错误检测是最简单的任务但性能仍随复杂性下降；GPT-5在中等/高推理努力下实现完美排序。

Conclusion: 当前LLMs在时间顺序任务上存在局限性，分配明确的推理预算有助于改善性能，这对LLMs在金融领域的实时应用具有重要意义。
...
### [152] [Listen Like a Teacher: Mitigating Whisper Hallucinations using Adaptive Layer Attention and Knowledge Distillation](https://arxiv.org/abs/2511.14219)
...
<details>
  <summary>Details</summary>
Motivation: Whisper模型在噪声条件下经常出现幻觉错误，而之前的研究主要集中在音频预处理或转录后处理，对模型本身的修改探索不足。
...
### [153] [DevPiolt: Operation Recommendation for IoT Devices at Xiaomi Home](https://arxiv.org/abs/2511.14227)
...
<details>
  <summary>Details</summary>
Motivation: 现有推荐模型难以处理物联网设备的复杂操作逻辑、多样化用户偏好，且对次优建议敏感，限制了其在物联网设备操作推荐中的应用。

Method: 首先通过持续预训练和多任务微调为LLM注入物联网操作领域知识，然后使用直接偏好优化对齐用户偏好，最后设计基于置信度的曝光控制机制。

Result: 在所有数据集上显著超越基线方法，各项指标平均提升69.5%。在小米家庭APP实际部署一个季度，服务25.5万用户，在线实验显示唯一访客设备覆盖率提升21.6%，页面浏览接受率提升29.1%。

Conclusion: DevPiolt成功解决了物联网设备操作推荐中的关键挑战，实现了显著的性能提升和实际应用价值。
...
### [154] [Enhancing Regional Airbnb Trend Forecasting Using LLM-Based Embeddings of Accessibility and Human Mobility](https://arxiv.org/abs/2511.14248)
*Hongju Lee,Youngjun Park,Jisun An,Dongman Lee*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的时间序列预测框架，用于预测区域级Airbnb市场的三个关键指标：收入、预订天数和预订数量。该方法通过整合房源特征与外部环境因素构建区域表示，并使用LLM生成区域嵌入，再结合先进的时间序列模型进行预测，相比传统基线方法将RMSE和MAE平均降低了约48%。
...
Method: 采用滑动窗口方法预测1-3个月的趋势，构建区域表示时整合房源特征与外部环境因素（如城市可达性和人员流动性），将结构化表格数据转换为基于提示的LLM输入以生成全面区域嵌入，然后将这些嵌入输入到先进的时间序列模型（RNN、LSTM、Transformer）中。
...
### [155] [PathMind: A Retrieve-Prioritize-Reason Framework for Knowledge Graph Reasoning with Large Language Models](https://arxiv.org/abs/2511.14256)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的KGR方法存在两个关键限制：1）不加区分地提取推理路径，可能引入无关噪声误导LLM；2）动态探索推理路径需要高检索需求和频繁LLM调用。

Method: PathMind遵循"检索-优先化-推理"范式：1）通过检索模块从KG中检索查询子图；2）引入路径优先化机制，使用语义感知路径优先级函数识别重要推理路径；3）通过双阶段训练策略（任务特定指令微调和路径偏好对齐）生成准确且逻辑一致的响应。

Result: 在基准数据集上的广泛实验表明，PathMind始终优于竞争基线，特别是在复杂推理任务上，通过识别基本推理路径，使用更少的输入token实现更好性能。

Conclusion: PathMind通过选择性引导LLM使用重要推理路径，有效解决了现有LLM-based KGR方法的局限性，在忠实性和可解释性方面表现出色，特别是在复杂推理场景中具有显著优势。
...
### [156] [DataSage: Multi-agent Collaboration for Insight Discovery with External Knowledge Retrieval, Multi-role Debating, and Multi-path Reasoning](https://arxiv.org/abs/2511.14299)
*Xiaochuan Liu,Yuanfeng Song,Xiaoming Yin,Xing Chen*

Main category: cs.AI

TL;DR: DataSage是一个多智能体框架，通过外部知识检索、多角色辩论机制和多路径推理来解决现有数据洞察智能体在领域知识利用、分析深度和代码生成准确性方面的不足，在InsightBench上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的数据洞察智能体存在三个主要问题：领域知识利用不足、分析深度不够、代码生成易出错，需要新的解决方案来提升自动化数据分析效果。
...
Conclusion: DataSage为自动化数据洞察发现提供了一个有效的解决方案，通过创新的多智能体设计解决了现有方法的局限性。
...
### [157] [When Words Change the Model: Sensitivity of LLMs for Constraint Programming Modelling](https://arxiv.org/abs/2511.14334)
*Alessio Pellegrino,Jacopo Mauro*

Main category: cs.AI

TL;DR: 本文通过重新表述和扰动经典CSPLib问题来测试LLMs在约束编程问题建模中的真实推理能力，发现LLMs在上下文和语言变化下性能显著下降，表明其理解较为肤浅且对措辞敏感。
...
Method: 系统性地重新表述和扰动一组知名CSPLib问题，保持问题结构但修改上下文并引入误导元素，然后比较三个代表性LLM在原始和修改描述下生成的模型。

Result: LLMs能够生成语法有效且语义合理的模型，但在上下文和语言变化下性能急剧下降，显示出浅层理解和对措辞的敏感性。

Conclusion: LLMs在约束编程问题建模中的成功可能更多源于数据污染而非真正的推理能力，其理解较为肤浅且容易受到语言变化的影响。
...
### [158] [Operationalizing Pluralistic Values in Large Language Model Alignment Reveals Trade-offs in Safety, Inclusivity, and Model Behavior](https://arxiv.org/abs/2511.14476)
*Dalia Ali,Dora Zhao,Allison Koenecke,Orestis Papakyriakopoulos*

Main category: cs.AI

TL;DR: 本研究探讨了在LLM对齐过程中纳入多元社会价值观的影响，通过系统评估人口统计差异和设计参数，发现不同社会群体对LLM响应的评价存在显著差异，且技术设计选择对模型行为有重要影响。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM越来越多地使用人类反馈进行安全和价值观对齐，但对齐决策往往忽视了人类社会的多样性。本研究旨在探讨纳入多元价值观如何影响LLM行为。

Method: 收集了来自美国和德国参与者(N=1,095)的27,375个评分，评估LLM响应的五个维度：毒性、情感意识、敏感性、刻板印象偏见和帮助性。使用不同社会群体的偏好微调多个LLM和LRM，同时改变评分尺度、分歧处理方法和优化技术。

Result: 发现系统性人口统计效应：男性参与者对响应的毒性评分比女性低18%；保守派和黑人参与者对情感意识的评分分别比自由派和白人参与者高27.9%和44%。技术设计选择显示强烈影响：保留评分者分歧比多数投票实现约53%更大的毒性减少；5点量表比二进制格式产生约22%更多的减少；DPO在多值优化中始终优于GRPO。
