...
TL;DR: 该论文提出了一种新的步态识别视觉模态SKETCH，通过边缘检测器从RGB图像中提取高频结构线索，并结合解析表示构建了分层解耦的多模态框架SKETCHGAIT，在多个数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要基于轮廓和解析表示，但轮廓表示稀疏且缺乏内部结构细节，解析表示虽然丰富了部分级结构但严重依赖上游人体解析器，导致跨数据集性能不稳定。需要从结构角度重新审视步态表示。

Method: 提出SKETCH作为新的视觉模态，通过边缘检测器从RGB图像中无标签地提取高频结构线索（如肢体关节和自遮挡轮廓）。进一步提出SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支来捕捉结构互补性。

Result: 在SUSTech1K和CCPG数据集上的实验验证了所提模态和框架的有效性：SKETCHGAIT在SUSTech1K上达到92.9%的Rank-1准确率，在CCPG上达到93.1%的平均Rank-1准确率。

Conclusion: 该工作识别了步态表示中一个未被充分探索的范式：无显式语义标签的密集部分级结构，并证明了标签引导的解析和无标签的草图在语义上是解耦的、在结构上是互补的，为步态识别提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法受限于小型标注数据集和BRep标签，而数百万未标注的3D网格未被利用，限制了可扩展CAD生成的发展。

Method: 将每个BRep表示为参数化曲面集，使用可微分细分方法生成网格，实现大规模3D数据集训练，同时重建连接且可编辑的表面。

Result: 在ABC和Objaverse基准测试中，在文本、图像和点模态上达到最先进性能，几何保真度提升，用户偏好超过75%。

Conclusion: DreamCAD通过点级监督直接生成可编辑BReps，无需CAD特定标注，结合CADCap-1M数据集，推动了可扩展CAD生成的发展。
...
TL;DR: ABRA方法通过对抗性批量表示增强解决细胞图像分析中的生物批次效应问题，在RxRx1基准上实现了最先进的siRNA扰动分类性能。


<details>
  <summary>Details</summary>
Motivation: 高通量筛选产生大量细胞图像，但实验执行中的技术变异导致生物批次效应，引起协变量偏移并降低深度学习模型在未见数据上的泛化能力。现有批次校正方法通常需要额外先验知识或难以泛化到未见生物批次。

Method: 将生物批次缓解构建为域泛化问题，提出对抗性批量表示增强（ABRA）。通过参数化特征统计量为结构化不确定性来建模批次统计波动，采用最小-最大优化框架在表示空间中主动合成最坏情况的生物批次扰动，通过严格的角几何边界保持细粒度类别可区分性，并引入协同分布对齐目标防止表示崩溃。

Result: 在RxRx1和RxRx1-WILDS基准上进行广泛评估，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性表示增强有效缓解生物批次效应，在保持类别区分性的同时提升模型对未见批次的泛化能力，为细胞图像分析中的批次效应问题提供了有效解决方案。
...
Method: PFS模块在现有检测器的中间BEV表示上操作，生成精炼的特征图供原始检测头使用。设计通过稳定域偏移下的特征统计、抑制传感器退化影响的区域、通过残差校正自适应恢复弱化线索来实现鲁棒性提升。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，同时保持轻量级设计（仅3.3M参数）。

Conclusion: PFS作为一种近恒等变换的轻量级模块，能够在不影响性能的前提下显著提升现有BEV融合检测器在域偏移和传感器故障下的鲁棒性，易于集成到已部署系统中。
...
TL;DR: CBM-Suite框架解决概念瓶颈模型的四个核心问题：提出概念相关性评估指标、解决线性问题、缩小准确率差距、分析不同视觉骨干和VLM的影响


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型存在四个基本限制：缺乏概念相关性预评估指标、线性问题导致模型绕过概念瓶颈、与不透明模型存在准确率差距、缺乏对不同视觉骨干和VLM影响的系统研究

Method: 提出CBM-Suite框架：1) 基于熵的指标量化概念集的内在适用性；2) 在概念激活和分类器之间插入非线性层解决线性问题；3) 使用线性教师探针引导的蒸馏损失缩小准确率差距；4) 综合分析不同视觉编码器、视觉语言模型和概念集的交互影响

Result: 广泛评估表明CBM-Suite产生更准确的模型，并为改进基于概念的可解释性提供见解

Conclusion: CBM-Suite系统性地解决了概念瓶颈模型的关键挑战，提高了模型准确性和可解释性，为概念选择和分析提供了方法论框架
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE重建FID与潜在扩散模型生成FID相关性差，缺乏有效的预测扩散模型生成质量的评估指标。

Method: 提出插值FID：对数据集中的每个元素，在潜在空间中检索其最近邻，插值它们的潜在表示，解码后计算与原始数据集的FID。

Result: iFID与扩散生成FID呈现强相关性（Pearson和Spearman相关系数约0.85），是首个能有效预测扩散模型生成质量的指标。

Conclusion: iFID通过潜在空间插值有效预测扩散模型生成质量，为评估扩散模型提供了新的可靠指标，并揭示了重建指标与生成质量之间的负相关关系。
...
TL;DR: 论文提出Implicit Error Counting (IEC)方法，用于解决缺乏理想参考答案的任务中的强化学习奖励设计问题，通过隐式错误计数而非基于参考的评分标准来提供更可靠的奖励信号。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的奖励方法（如RaR）在缺乏单一理想答案的多正确答案任务中存在局限性，需要一种无需参考的奖励设计方法来填补这一空白。

Method: 提出Implicit Error Counting (IEC)方法：1）枚举响应中的错误而非检查正确之处；2）应用严重性加权评分；3）通过隐式分数发射和组校准设计来稳定优化；4）针对虚拟试衣任务提出Cascaded Error Counting (CEC)评估指标。

Result: 在MDressBench基准测试中，IEC在所有指标上优于RaR（CEC: 5.31 vs. 5.60）；在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越6个基线方法；CEC指标与人类偏好高度相关（60% top-1 vs. 30%其他）。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号，IEC方法在缺乏参考的多正确答案任务中表现出优越性。
...
TL;DR: SemVID是一种无需训练的视觉令牌剪枝框架，用于视频时序定位任务，通过保留查询关键证据和跨帧连接性，在仅使用12.5%视觉令牌的情况下保持95.4%的mIoU，实现5.8倍的前馈加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位任务需要处理长视频，使用视频语言模型成本过高。现有视觉令牌剪枝方法直接应用于VTG会导致性能严重下降，因为VTG依赖边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG特定剪枝原则：证据保留和连接强度。首先平衡查询相关性和帧间变化分配每帧令牌预算，然后选择三种令牌类型：对象令牌（查询关键证据）、运动令牌（捕捉过渡并作为跨帧中继）和上下文令牌（场景连续性）。

Result: 在VTG基准测试中，SemVID在仅使用12.5%视觉令牌的情况下保持95.4%的mIoU，实现5.8倍前馈加速，在相同预算下持续优于先前方法。

Conclusion: SemVID通过构建紧凑且连贯的令牌子集，有效平衡了视频时序定位任务的准确性和效率，为VTG任务提供了高效的视觉令牌剪枝解决方案。
...
TL;DR: OWL是一种基于视觉运动线索的3D感知函数，直接从原始图像序列计算，无需先验环境知识，实现实时3D场景重建和物体几何恒常性。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是如何在无需先验环境知识、相机运动信息或静止环境假设的情况下，仅通过视觉运动线索实现实时3D感知。

Method: 基于两个基本视觉运动线索：1) 注视点附近点的局部视觉逼近感知，2) 刚性物体相对于注视点的旋转感知。通过像素级并行计算，直接从原始视觉运动图像序列推导OWL值，无需测量相对瞬时方向范围和方向平移。

Result: 仿真表明OWL能够实现3D物体的几何恒常性，仅凭视觉运动线索即可实现缩放3D场景重建。该方法提供了一种实时、最小化计算的3D点相对运动表示。

Conclusion: OWL为机器感知提供了新视角，填补了理论概念与机器人、自主导航实际应用之间的差距，可能成为下一代自主系统的构建模块，并对自然感知、行为心理学和神经功能理解有启示意义。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准测试，包含超过46,000个多模态检索候选项和747个开放可验证问题，揭示了当前多模态大语言模型在异构多模态检索方面的主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估文本、图像或视频的单独理解能力，但忽略了从大型异构多模态语料库中检索相关证据的关键现实需求。大多数现有基准将检索限制在小型单模态候选集中，过度简化了搜索空间并夸大了端到端可靠性。
...
Conclusion: 异构多模态检索仍然是MLLMs的主要瓶颈，MultiHaystack作为一个有价值的测试平台，揭示了被小规模评估掩盖的潜在限制，并促进多模态系统中以检索为中心的进展。
...
TL;DR: 本文提出Audiovisual Geolocation框架，通过可解释的感知和推理解决地理定位问题，结合视觉和听觉信号实现高精度全球定位。
...
Conclusion: 听觉信号为地理定位提供了重要补充信息，通过可解释的感知和多模态推理框架，能够有效解决视觉模糊性问题，实现更精确的全球定位。
...
TL;DR: 提出自动化流程将ImageNet训练集转换为多标签数据集，无需人工标注，通过自监督ViT进行无监督物体发现，生成的多标签标注能提升模型性能和表示学习能力。


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准强制单标签假设，但许多图像包含多个物体，导致标签噪声并限制学习信号的丰富性。多标签标注能更准确地反映真实世界视觉场景，使模型学习更丰富、更鲁棒的表示。

Method: 使用自监督视觉变换器进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后应用于所有区域以生成跨数据集的连贯多标签标注。

Result: 生成的多标签标注在定性评估中与人类判断高度一致。相比传统单标签方案，使用多标签监督训练的模型在架构间获得一致更好的域内准确率（ReaL上最高+2.0 top-1准确率，ImageNet-V2上+1.5），并在下游任务上表现出更强的可迁移性（COCO上最高+4.2 mAP，VOC上+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表示学习能力至关重要。提出的自动化流程成功将ImageNet训练集转换为高质量多标签数据集，无需人工标注。
...
Conclusion: 该方法通过自动化创建手术时间线和叙述，减少了外科医生手动审查和标注视频的需求，为工具辅助手术提供了更高效的时间线生成方案
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在更高分辨率下难以准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输偏差、阴影和色调失真。需要更准确、逼真的天空环境图来支持视觉艺术、虚拟现实和科学工程应用。

Method: 提出Icarus模型，能够学习全动态范围物理捕捉户外图像的曝光范围。支持条件生成环境图，用户可直观定位太阳和云层形态，并扩展当前最先进技术以实现用户控制的大气形态纹理。

Result: Icarus可与全动态范围物理捕捉户外图像或参数化天空模型互换，在基于图像的照明中实现了前所未有的准确性、逼真度、光照方向性（阴影）和色调。

Conclusion: Icarus是一个全天气天空模型，能够生成高精度、逼真的环境图，解决了现有深度学习天空模型在高分辨率太阳区域建模的局限性，为各种应用提供了更好的天空表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有区域控制方法存在数据偏见、图像质量下降以及遮挡顺序处理困难的问题，作者观察到空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出。

Method: 采用两阶段方案：1) 实例初始化 - 利用多模态联合注意力中的上下文共享机制，创建每个实例分支，在指定早期步骤按层顺序融合；2) 语义护理 - 通过分层注意力增强强化区域细节并保持遮挡顺序，使用顺序分层注意力路径与标准全局路径并行工作。

Result: LayerBind在定性和定量结果上都表现出有效性，能够实现精确的区域和遮挡控制，支持可编辑工作流程，具有强大的创意应用潜力。

Conclusion: LayerBind解决了现有区域控制方法的局限性，通过分层建模和绑定机制实现了无需训练、即插即用的精确区域和遮挡控制，为创意应用提供了强大工具。
...
TL;DR: BM25-V：基于视觉词稀疏自编码的轻量级图像检索方法，结合BM25评分实现高效检索，可作为稠密检索的预处理阶段


<details>
  <summary>Details</summary>
Motivation: 稠密图像检索虽然准确，但可解释性和归因性有限，且在大规模计算时成本高昂。需要一种更高效、可解释的检索方法。

Method: 使用稀疏自编码器（SAE）从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分。利用倒排索引进行高效检索，作为稠密重排的第一阶段。

Result: 在七个基准测试中，BM25-V的Recall@200 ≥ 0.993，仅需重排200个候选就能恢复接近稠密检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练后，可零样本迁移到七个细粒度基准。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，可作为稠密检索的高效预处理阶段，同时保持高召回率和可归因性。
...
Result: 在NVIDIA A6000上，平均达到12.2 FPS（基线为8.4 FPS），视频编辑吞吐量提升1.45倍。方法不损害生成质量，无需额外训练即可集成到模型中。

Conclusion: LIPAR框架有效利用了视频的时间冗余性，在保持生成质量的同时显著提升了计算效率，弥合了传统压缩算法与现代生成管道之间的差距。
...
TL;DR: Cog2Gen3D是一个3D认知引导的扩散框架，通过语义和几何特征的双流图融合来增强3D生成的物理合理性和结构合理性。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法缺乏空间几何约束，只能建模相对关系而容易产生绝对几何尺度不一致的问题。作者认为语义信息和绝对几何信息共同赋能3D认知，从而实现物理世界的可控3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：编码多模态为语义和几何表示并提取逻辑表示；2）3D潜在认知图：构建双流语义-几何图并通过基于共通的交叉注意力融合；3）认知引导潜在扩散：利用融合的3D认知图作为条件指导3D高斯生成的潜在扩散过程。

Result: 在基于Marble World Labs构建的验证子集上进行大量实验，证明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架确保了3D生成的物理合理性和结构合理性，为可控3D生成提供了有效解决方案。
...
TL;DR: VS3R是一个视频稳定框架，通过结合前馈3D重建与生成式视频扩散，解决了传统方法在几何鲁棒性和全帧一致性之间的权衡问题，实现了高保真、全帧的视频稳定效果。


<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的基本权衡：2D方法因过度裁剪而受限，3D技术则因优化流程脆弱而在极端运动下失效。需要一种能在所有场景下可靠工作的方法。

Method: VS3R框架结合前馈3D重建与生成式视频扩散，联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；使用双流视频扩散模型修复遮挡区域并校正伪影。

Result: VS3R在不同相机模型上实现了高保真、全帧的视频稳定，在鲁棒性和视觉质量方面显著优于现有最先进方法。

Conclusion: 通过协同3D重建与生成式视频扩散，VS3R成功解决了视频稳定中的几何鲁棒性与全帧一致性权衡问题，为各种场景提供了可靠的稳定解决方案。
...
TL;DR: 本文提出了TumorChain框架，这是一个用于临床肿瘤分析的多模态推理系统，通过链式思维推理从CT影像到病理预测的完整流程，显著提高了诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 准确的肿瘤分析对临床放射学和精准肿瘤学至关重要，但现有方法在可追溯性和减少诊断错误方面存在不足。链式思维推理能够实现从影像发现到临床印象再到病理结论的逐步解释，从而提高诊断的可靠性和可解释性。

Method: 1) 构建TumorCoT数据集：包含150万个带有CoT标注的VQA指令和3D CT扫描，具有步骤对齐的推理过程和跨模态对齐；2) 提出TumorChain框架：结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理实现视觉证据的定位、结论的聚合以及病理预测。

Result: 实验表明，TumorChain在病灶检测、印象生成和病理分类方面均优于现有基线方法，并在DeepTumorVQA基准测试中表现出强大的泛化能力。该框架提高了诊断的可追溯性，减少了幻觉风险。

Conclusion: 多模态推理框架TumorChain为临床实践提供了可靠且可解释的肿瘤分析方法，通过链式思维推理实现了从影像到病理的完整推理流程，在提高诊断准确性的同时增强了系统的可解释性。
...
TL;DR: PatchCue是一种基于图像块的视觉提示范式，通过将图像分割成块并在块级别表示视觉提示，显著提升视觉语言模型的视觉推理能力。
...
Method: 提出PatchCue：将图像分割成块并在块级别表示视觉提示，与人类感知习惯和现代VLM的块标记化输入对齐。采用两阶段训练：冷启动监督微调输出块级提示，然后通过过程监督提示奖励的强化学习引导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue持续提升整体模型性能。块级提示优于像素级边界框和基于点的提示。
...
Result: 在ComLesion-14K基准上实现了37.06%的平均Dice分数（比第二名基线高14.89%），并将失败率降低到18.42%。

Conclusion: 该研究通过推理驱动的分割框架，在复杂病灶分割任务上取得了显著性能提升，为医学图像分析提供了新的认知推理方法。
...
### [23] [BlackMirror: Black-Box Backdoor Detection for Text-to-Image Models via Instruction-Response Deviation](https://arxiv.org/abs/2603.05921)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖图像级相似性分析，假设后门触发的生成图像具有强一致性，但无法有效检测视觉多样化的新型后门攻击。

Method: BlackMirror包含两个组件：MirrorMatch将视觉模式与对应指令对齐以检测语义偏差；MirrorVerify评估这些偏差在不同提示下的稳定性以区分真正的后门行为和良性响应。

Result: 综合实验表明，BlackMirror能够在多种攻击场景下实现准确检测，是一个无需训练、即插即用的通用框架。

Conclusion: BlackMirror为黑盒文本到图像模型后门检测提供了有效的解决方案，能够部署在模型即服务应用中，应对视觉多样化的后门攻击。
...
<details>
  <summary>Details</summary>
Motivation: 合成新视角的生成对机器人导航有积极影响：地面机器人拍摄的场景可以生成新的俯视图来引导空中机器人；空中视角可以生成新的地面视角，帮助无人机识别地面机器人看到的地点，反之亦然。

Method: 使用五个公开的VPR图像数据库和七种典型的图像相似性方法，对合成新视角在VPR中进行系统评估。

Result: 对于小规模的合成添加，新视角能改善VPR识别统计结果；对于大规模添加，视角变化的幅度不如添加的视图数量和数据集中的图像类型重要。

Conclusion: 合成新视角在VPR中具有实用价值，特别是在小规模添加时能提升识别性能，而大规模应用时需要考虑添加数量和数据集特性。
...
### [25] [RAC: Rectified Flow Auto Coder](https://arxiv.org/abs/2603.05925)
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径不够灵活，且需要独立的编码器和解码器导致参数较多。需要一种更高效、性能更好的自编码器架构。

Method: 1. 采用Rectified Flow思想，在流时间步上应用解码器实现多步解码，路径直且可校正；2. 支持双向推理，解码器通过时间反转可作为编码器使用，减少参数；3. 生成式解码方法允许沿路径校正潜在变量。

Result: RAC在重建和生成任务上均超越当前最优VAE，计算成本降低约70%，参数数量减少近41%。

Conclusion: RAC通过Rectified Flow启发的架构设计，有效解决了VAE的重建-生成差距问题，在提升性能的同时显著降低了计算和参数成本，为自编码器设计提供了新思路。
...
TL;DR: CylinderSplat：一种用于全景3D高斯溅射的前馈框架，通过圆柱形Triplane表示和双分支架构解决稀疏视图下的遮挡问题


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D高斯溅射方法在全景图像应用上面临挑战，多视图成本体积方法在稀疏视图场景下难以解决遮挡问题，而标准的笛卡尔Triplane表示无法有效捕捉360°场景的几何结构，导致失真和混叠

Method: 提出CylinderSplat框架，核心是新的圆柱形Triplane表示，更好地对齐全景数据和符合曼哈顿世界假设的真实世界结构。采用双分支架构：基于像素的分支重建观测良好的区域，基于体积的分支利用圆柱形Triplane完成遮挡或稀疏观测区域。框架设计灵活，可处理从单视图到多视图的可变输入

Result: 广泛的实验表明，CylinderSplat在单视图和多视图全景新视图合成中均达到最先进的结果，在重建质量和几何精度方面均优于先前方法
...
TL;DR: PixARMesh是一种从单张RGB图像自回归重建完整3D室内场景网格的方法，能一次性生成连贯、可直接使用的网格，无需后处理优化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式符号距离场和后期布局优化，无法直接生成连贯的网格。需要一种能直接从单张图像生成完整、高质量3D场景网格的统一方法。

Method: 基于点云编码器，通过像素对齐的图像特征和跨注意力机制增强全局场景上下文，实现从单张图像的准确空间推理。采用自回归方式从统一令牌流（包含上下文、姿态和网格）生成场景。

Result: 在合成和真实数据集上的实验表明，PixARMesh达到了最先进的重建质量，同时生成轻量级、高质量的网格，可直接用于下游应用。

Conclusion: PixARMesh通过统一模型联合预测对象布局和几何，能够从单张RGB图像直接生成连贯、高质量的3D室内场景网格，为下游应用提供了即用型解决方案。
...
TL;DR: InnoAds-Composer：单阶段电商产品海报生成框架，通过三条件控制（主体、字形、风格）和重要性路由机制，解决多阶段方法的主体保真度低、文本不准确和风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段流程，难以同时控制主体、文本和风格，且存在主体保真度差、文本不准确、风格不一致的问题。

Method: 提出单阶段框架InnoAds-Composer，通过三条件控制令牌实现主体、字形和风格的协同控制；采用重要性分析对层和时间步进行路由，缩短活跃令牌序列；设计文本特征增强模块（TFEM）结合字形图像和字形裁剪特征提升中文文本渲染准确性；构建首个同时包含主体、文本和风格条件的高质量电商产品海报数据集。

Result: 大量实验表明，InnoAds-Composer在显著优于现有产品海报生成方法的同时，没有明显增加推理延迟。

Conclusion: InnoAds-Composer通过单阶段三条件控制框架有效解决了电商产品海报生成中的主体保真度、文本准确性和风格一致性问题，为高质量产品海报生成提供了有效解决方案。
...
TL;DR: 提出CollabOD轻量级协作检测框架，解决无人机图像中小目标检测的尺度变化、结构细节退化和计算资源有限等挑战


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临三大挑战：尺度变化、结构细节退化（特别是在高海拔场景中，细粒度特征在分层下采样和跨尺度融合中被削弱），以及有限的算力资源，导致定位不稳定和鲁棒性降低

Method: 提出CollabOD框架，集成三种策略：1) 结构细节保留；2) 跨路径特征对齐；3) 定位感知的轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并采用统一的细节感知检测头

Result: 该框架在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头在不增加额外部署开销的情况下提高了回归鲁棒性

Conclusion: CollabOD通过协作设计有效解决了无人机图像小目标检测的关键问题，代码已开源
...
TL;DR: S2I方法将3D人体骨架序列转换为图像格式，使预训练视觉模型能用于骨架数据的自监督表示学习，解决了骨架数据格式差异和数据集稀缺问题。
...
Conclusion: S2I表示首次实现了使用预训练视觉模型进行骨架自监督学习，将丰富的视觉领域知识转移到骨架分析中，并为异构骨架数据提供了统一的图像格式。
...
TL;DR: 论文提出Art3D框架，将2D转3D的目标从几何精确重建转向艺术化视差合成，通过双路径架构分离全局深度参数和局部艺术效果，学习专业3D电影数据，初步实现影院级艺术效果


<details>
  <summary>Details</summary>
Motivation: 当前2D转3D方法虽然几何准确但艺术性不足，无法复制专业3D电影的沉浸感和情感共鸣体验。传统方法将艺术意图（如零平面偏移、局部深度塑造）误认为数据噪声或模糊性

Method: 提出Art3D框架，采用艺术化视差合成新范式。使用双路径架构解耦全局深度参数（宏观意图）和局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习，并引入初步的影院对齐评估方法

Result: 实验表明该方法在复制关键局部出屏效果和与影院3D内容的全局深度风格对齐方面展现出潜力，为艺术驱动型转换工具奠定了基础

Conclusion: 需要从物理精确的视差估计转向艺术连贯的视差合成新范式，Art3D框架为此提供了初步探索，有望推动艺术化2D转3D技术的发展
...
<details>
  <summary>Details</summary>
Motivation: 现有变化描述方法基于静态图像对，忽略了变化过程的丰富时间动态，而理解变化过程对于准确描述"发生了什么变化"和"如何发生"至关重要。

Method: 两阶段框架：第一阶段训练过程编码器，通过自动生成中间帧获取关键帧，通过caption-conditioned掩码重建任务学习潜在动态；第二阶段将编码器集成到编码器-解码器模型中，使用可学习的过程查询来推断潜在过程表示，然后解码为文本描述。
...
<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时的迭代布局优化或不灵活的联合对象-布局生成；2）大多数方法基于有限视场的透视图像，难以创建完整的360度环境。需要一种高效且能处理全景图像的解决方案。

Method: 提出Pano3DComposer框架，包含两个核心技术：1）Object-World Transformation Predictor：将现成图像到3D模型生成的3D对象从局部坐标转换为世界坐标，采用Alignment-VGGT架构，使用目标对象裁剪、多视角对象渲染和相机参数来预测变换；2）Coarse-to-Fine对齐机制：针对未见域输入图像，通过场景渲染反馈迭代优化几何一致性。

Result: 方法在合成和真实数据集上的图像/文本到3D任务中实现了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景，显著提高了生成效率和质量。

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，结合高效的坐标变换预测器和粗到精对齐机制，实现了快速且高质量的360度3D场景生成，解决了现有方法的效率和灵活性限制。
...
<details>
  <summary>Details</summary>
Motivation: 在现代医疗动态环境中，手术器械的质量标准对临床成功至关重要。即使是微小缺陷也可能导致致命后果，因此制造精度至关重要。该报告旨在通过自动缺陷检测提升手术器械质量，保障患者安全并减少制造商经济损失。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作；2. 开发包含4,414张高分辨率图像的新数据集；3. 集成深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4；4. 采用自动光学检测（AOI）工具进行质量保证；5. 识别裂纹、锈蚀和结构不规则等关键缺陷。

Result: 报告提供了巴基斯坦制造手术器械缺陷分析的宝贵见解，开发了基于深度学习的自动缺陷检测方法，为制造商、医疗专业人员和监管机构提供了质量保证资源，有助于提升器械标准。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习架构，该报告为提升手术器械制造质量提供了有效途径，有助于创建更可靠的医疗环境，确保患者安全并减少制造商经济损失。
...
### [35] [MM-ISTS: Cooperating Irregularly Sampled Time Series Forecasting with Multimodal Vision-Text LLMs](https://arxiv.org/abs/2603.05997)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中不规则采样时间序列普遍存在，现有方法仅使用历史观测值预测未来，缺乏对上下文语义和细粒度时间模式的学习能力

Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据；2)ISTS编码提取历史观测的互补时间特征；3)自适应查询特征提取器压缩MLLMs学习到的token；4)多模态对齐模块缓解模态差距

Result: 在真实数据上的大量实验验证了所提解决方案的有效性

Conclusion: MM-ISTS通过桥接时间、视觉和文本模态，利用多模态大语言模型增强不规则采样时间序列预测，能够捕获复杂时间模式和全面上下文理解
...
<details>
  <summary>Details</summary>
Motivation: 当前遥感图像分类中，标准CNN和Transformer模型通常作为不可解释的黑盒运行，缺乏对分类决策的物理可解释性。需要一种既能保持高性能又能提供明确物理解释的架构。
...
Conclusion: RepKAN具有显著潜力作为未来可解释视觉基础模型的主干架构，为遥感图像分类提供了高性能与可解释性兼备的解决方案。
...
TL;DR: RAID数据集：用于驾驶员风险感知研究的大规模数据集，包含4,691个标注视频片段，涵盖多种交通场景，支持弱监督风险对象识别框架


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞移动性是智能车辆系统的关键目标，这需要理解驾驶员的风险感知——一个复杂的认知过程，受驾驶员对外部刺激的自愿反应和周围道路使用者对自车的关注度影响。目前缺乏专门用于驾驶员风险感知和情境风险评估的大规模数据集。

Method: 1. 引入RAID数据集：包含4,691个标注视频片段，涵盖多样交通场景，标注内容包括驾驶员意图操作、道路拓扑、风险情况（如横穿行人）、驾驶员反应和行人注意力。2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID和HDDS数据集上分别比现有最先进方法提升了20.6%和23.1%的性能。证明了所提出数据集的价值和方法的有效性。

Conclusion: RAID数据集为驾驶员风险感知和情境风险评估研究提供了重要资源，提出的弱监督风险对象识别框架通过建模驾驶员意图与反应的关系有效识别风险源，行人注意力分析进一步提升了风险评估能力，为智能车辆系统的零碰撞目标提供了支持。
...
TL;DR: 研究通过分析视觉语言模型在自动驾驶场景中的中间激活，发现模型在简单视觉问题上存在两种失败模式：感知失败和认知失败，且目标物体距离增加会降低视觉概念的线性可分性。
...
Method: 创建仅针对特定视觉概念不同的反事实图像集，使用四种最先进的视觉语言模型的激活训练线性探针，分析视觉概念在线性编码中的表现。

Result: 结果显示：物体存在等概念被显式线性编码，而物体方向等空间概念仅通过视觉编码器保留的空间结构隐式编码。即使概念被线性编码，模型仍可能回答错误，识别出感知失败和认知失败两种失败模式。目标物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 研究提高了对视觉语言模型在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉信息在模型中的编码方式和失败机制。
...
TL;DR: TAR-ViTPose：一种用于视频2D人体姿态估计的时序聚合与恢复视觉Transformer，通过聚合跨帧时序信息提升ViT在视频中的姿态估计性能


<details>
  <summary>Details</summary>
Motivation: 现有基于ViT的姿态估计器为静态图像设计，独立处理每帧，忽略了视频中的时序连贯性，导致在运动模糊、遮挡等挑战场景下预测不稳定
...
Result: 在PoseTrack2017基准上比单帧基线ViTPose提升+2.3 mAP，优于现有最先进的视频方法，同时在真实应用中实现更高的运行时帧率

Conclusion: TAR-ViTPose通过有效的时序信息聚合和恢复机制，显著提升了视频中2D人体姿态估计的鲁棒性和准确性，同时保持了高效性
...
TL;DR: TempoSyncDiff是一个参考条件潜在扩散框架，通过师生蒸馏实现高效音频驱动说话头生成，减少推理步骤、提高稳定性，并解决身份漂移和闪烁问题。
...
Method: 采用参考条件潜在扩散框架，使用师生蒸馏方法：用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，减少推理步骤；结合身份锚定和时间正则化减少身份漂移和闪烁；使用基于视位的音频条件提供粗粒度唇部运动控制。

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师的大部分重建行为，同时实现显著降低的延迟推理；包括CPU和边缘计算的延迟测量表明边缘部署可行性。
...
TL;DR: 提出一种前馈三角形基元生成框架，直接从多视角图像预测连续三角形表面，实现单次前向传播即可生成仿真就绪的3D模型，无需逐场景优化或后处理。


<details>
  <summary>Details</summary>
Motivation: NeRF和3DGS等方法虽然渲染质量高，但依赖耗时的逐场景优化，限制了实时部署。现有前馈高斯溅射方法效率有所提升，但缺乏显式的流形几何结构，无法直接用于仿真。需要一种能直接生成仿真就绪几何模型的高效方法。

Method: 提出前馈三角形基元生成框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督来增强几何学习的稳定性和一致性。直接从标定的多视角图像预测连续三角形表面，单次前向传播即可生成仿真就绪模型。

Result: 实验表明该方法实现了高效重建，同时保持了与标准图形和机器人仿真器的无缝兼容性。相比现有方法，在保持效率的同时提供了显式的几何结构。

Conclusion: 该方法解决了现有神经渲染方法在实时部署和仿真兼容性方面的限制，通过前馈三角形生成实现了高效、仿真就绪的3D重建，为机器人和仿真应用提供了实用的解决方案。
...
TL;DR: OD-RASE是一个通过检测导致交通事故的道路结构并将其与基础设施改进相连接，来增强自动驾驶系统安全性的框架。该框架使用本体论和大型视觉语言模型自动生成基础设施改进建议，并构建新的数据集。
...
Method: 1. 基于道路交通系统专业领域知识形式化本体论；2. 使用大型视觉语言模型生成基础设施改进建议；3. 通过本体驱动数据过滤提高建议可靠性；4. 在事故前道路图像上自动标注改进建议，构建新数据集；5. 提出OD-RASE基准方法，结合LVLM和扩散模型生成改进建议和改进后道路环境的生成图像。

Result: 实验表明，本体驱动数据过滤能够高度准确地预测导致事故的道路结构及相应的改进计划。该框架为构建更安全的交通环境提供了有效工具。

Conclusion: 这项工作有助于提高交通环境的整体安全性，是推动自动驾驶系统更广泛应用的重要一步。通过将事故分析与基础设施改进连接起来，实现了从被动安全到主动安全的转变。
...
### [43] [Text-Driven Emotionally Continuous Talking Face Generation](https://arxiv.org/abs/2603.06071)
*Hao Yang,Yanyan Zhao,Tian Zheng,Hongbo Zhang,Bichen Wang,Di Wu,Xing Fu,Xuda Zhi,Yongbo Huang,Hao He*

Main category: cs.CV

TL;DR: 提出情感连续说话人脸生成(EC-TFG)新任务和TIE-TFG模型，能够根据文本和动态情感描述生成具有连续情感变化的高质量说话人脸视频


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏人类在传达信息时自然连续的情感变化能力，需要能够反映动态情感变化的更真实视频生成方法
...
Conclusion: 成功实现了情感连续说话人脸生成新任务，提出的TIE-TFG模型能够有效处理动态情感变化，生成更真实自然的说话人脸视频
...
TL;DR: 提出了一种基于自适应径向投影的文档图像倾斜估计方法，在傅里叶幅度谱上提取主导倾斜角度，并创建了高质量数据集DISE-2021进行评估。


<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，其性能直接影响后续处理步骤。在数字化时代，虽然已有大量研究关注这一挑战性问题，但仍需更准确、鲁棒的解决方案。

Method: 提出了一种新颖的倾斜估计方法，通过对2D离散傅里叶幅度谱应用自适应径向投影来提取文档图像的主导倾斜角度。

Result: 实验结果表明，所提出的方法具有鲁棒性和可靠性，在所有对比方法中表现最优。同时创建了高质量倾斜估计数据集DISE-2021用于评估。

Conclusion: 该方法在文档倾斜估计任务中表现出色，为傅里叶基方法提供了全面的改进分析。源代码已公开供研究使用。
...
TL;DR: LucidNFT：基于多奖励强化学习的流匹配真实图像超分辨率框架，通过Lucidity一致性评估器、解耦优势归一化策略和真实退化数据集解决真实图像超分辨率中的语义幻觉和优化瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 生成式真实图像超分辨率存在语义和结构幻觉问题，输出可能看起来清晰但与低分辨率证据不一致。基于偏好的强化学习虽然适合，但面临三个主要挑战：缺乏退化鲁棒的低分辨率参考忠实度信号、rollout组优化瓶颈导致优势崩溃、以及真实退化数据覆盖有限。

Method: 提出LucidNFT框架：1) LucidConsistency - 退化鲁棒的语义评估器，使低分辨率锚定的忠实度可测量和可优化；2) 解耦优势归一化策略 - 在融合前保留每个低分辨率条件下的rollout组内目标间对比，防止优势崩溃；3) LucidLR - 大规模真实世界退化图像集合，支持鲁棒的强化学习微调。

Result: 实验表明，LucidNFT能持续改进强大的基于流的真实图像超分辨率基线，在多样化的真实世界场景中实现更好的感知-忠实度权衡，并保持稳定的优化动态。

Conclusion: LucidNFT通过解决真实图像超分辨率中的关键挑战，包括语义幻觉评估、多奖励优化瓶颈和训练数据限制，为生成式真实图像超分辨率提供了有效的强化学习框架，实现了更好的感知质量和忠实度平衡。
...
Method: 采用"思考-放置"范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分提升视觉自然度；3) 推理时MLLM迭代触发细化循环，形成闭环优化；4) 提供两种用户可选模式：灵活模式（允许环境修改）和标准模式（保持场景完整性）。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，能够实现物理一致性的视频物体插入。

Conclusion: 该框架通过结合MLLM的认知推理和视频扩散模型的生成能力，实现了物理一致的视频编辑，为用户提供了在物理合理性和保真度之间的明确控制。
...
TL;DR: 该研究提出了VLM-RobustBench基准，评估视觉语言模型在49种图像失真下的鲁棒性，发现当前VLM对空间扰动（尤其是重采样和几何变换）比视觉上严重的光度失真更敏感。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现良好，但对其在真实世界图像失真下的性能理解不足。需要系统评估VLM对各种图像扰动的鲁棒性。
...
Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动通常比视觉上严重的光度损坏导致更大的性能下降。特别是低严重度的glass_blur使MMBench准确率平均下降约8个百分点，而最大下降来自重采样和几何失真（如upsample、elastic_transform），可达34个百分点。

Conclusion: 当前VLM语义理解能力强但空间鲁棒性脆弱，需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
<details>
  <summary>Details</summary>
Motivation: 少样本分割在有限监督下对新类别具有泛化能力，但在大外观或视角变化下仍面临结构错位和跨视角不一致的挑战。需要解决这些挑战以提升分割性能。

Method: VINE引入空间-视角图在骨干特征上，空间图捕捉局部几何拓扑，视角图连接不同视角特征以传播视角不变的结构语义。同时从支持-查询特征差异中提取判别性先验来捕捉类别特定对比，通过强调显著区域重新加权SAM特征并重新校准骨干激活。前景增强的SAM特征和结构丰富的ResNet特征通过掩码交叉注意力逐步集成，生成类别一致的原型作为SAM解码器的自适应提示。
...
TL;DR: 本文提出RF-Sampling，一种专门为流匹配模型设计的训练免费推理增强框架，通过线性组合文本表示和流反转来提升生成质量和文本对齐度。


<details>
  <summary>Details</summary>
Motivation: 当前推理增强策略主要适用于传统扩散模型，但在流匹配模型（如FLUX）上表现不佳。需要为流模型特别是CFG蒸馏变体设计专门的推理增强方法。

Method: 提出反射流采样（RF-Sampling），基于理论推导证明其隐式执行文本-图像对齐分数的梯度上升。通过线性组合文本表示并集成流反转，使模型探索与输入提示更一致的噪声空间。

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐度。这是首个在FLUX上展现出一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流匹配模型提供了有效的训练免费推理增强框架，填补了现有方法在流模型上的空白，并在质量和对齐度方面取得显著改进。
...
TL;DR: OVGGT：一种无需训练、内存和计算开销固定的3D几何重建框架，可处理任意长度视频流


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型虽然重建质量好，但全注意力机制二次计算成本高，只能处理短序列；因果注意力变体如StreamVGGT虽支持单次流式处理，但KV缓存持续增长，GPU内存很快耗尽，无法实现长序列部署

Method: 提出OVGGT框架：1）自选择性缓存：利用FFN残差幅度压缩KV缓存，完全兼容FlashAttention；2）动态锚点保护：保护坐标关键令牌不被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能在恒定VRAM内处理任意长度视频，同时实现最先进的3D几何精度

Conclusion: OVGGT通过固定预算的内存和计算设计，解决了流式视频3D重建中的长序列处理难题，实现了高效且准确的任意长度视频几何重建
...
TL;DR: 该论文提出一个两阶段框架，通过对比学习训练编码器处理有标签人造字母表，再通过师生蒸馏扩展到历史文字系统，实现字形相似度度量而不依赖确凿的历史关系。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统的相似度度量面临根本挑战：人造字母表中的单个字素可以可靠标记，但不同文字系统之间的历史关系不确定且存在争议。需要一种方法既能学习有监督的判别特征，又能发现无监督的潜在跨文字相似性。

Method: 采用两阶段框架：第一阶段在有标签的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；第二阶段通过师生蒸馏扩展到历史文字系统，学生模型在教师知识指导下学习无监督表示，但可以自由发现潜在的跨文字相似性。这种非对称设置使学生能够学习变形不变的嵌入，同时继承干净示例的判别结构。

Result: 在多样化的文字系统上进行实验，展示了有效的少样本字形识别和有意义的文字聚类，而不需要真实的历史进化关系作为基础。

Conclusion: 该方法桥接了有监督对比学习和无监督发现，既能建立不同文字系统之间的硬边界，又能反映潜在历史影响的软相似性，为字形相似度度量提供了一种有效框架。
...
TL;DR: 提出基于两阶段策略的开放词汇物体识别框架：先分割后识别，无需复杂重训练和标注，通过CLIP生成图像和文本嵌入，引入CNN/MLP方法增强编码灵活性，使用SVD构建共享表示空间，通过嵌入相似度匹配实现识别。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇物体识别方法存在系统复杂、训练成本高、泛化能力有限的问题，需要一种更简洁高效的解决方案。

Method: 采用两阶段策略：1) 物体分割；2) 识别。使用CLIP生成物体级图像嵌入和类别级文本嵌入，引入CNN/MLP方法提取CNN特征图并用MLP对齐视觉特征与文本嵌入，通过SVD构建共享表示空间，最后通过嵌入相似度匹配实现识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练的CLIP编码（无SVD）取得了最高的平均AP，超越了当前最先进方法，同时证明了CNN/MLP图像编码在开放词汇物体识别中的潜力。

Conclusion: 提出的开放词汇物体识别框架通过简化两阶段策略，在无需复杂训练和标注的情况下实现了优异的性能，为开放词汇识别提供了高效实用的解决方案。
...
TL;DR: DeepSight是首个专门用于深度感知的多模态大语言模型，通过构建深度图像-文本对数据集和深度指令数据集，改进ViT编码器来提升三维场景理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图像描述和视觉问答等任务上表现优异，但往往难以准确理解视觉数据中的深度信息，这限制了三维场景理解能力。

Method: 1. 构建深度图像-文本对数据集和深度指令数据集（使用GLPN模型生成深度图，GPT-4生成深度指令）；2. 改进CLIP中的ViT编码器以融入局部物体信息，更好地捕捉深度连续变化；3. 开发基于现有深度图像数据集的深度问答基准测试。

Result: 实验结果表明，DeepSight显著提升了深度感知能力和下游任务性能，在多模态三维理解方面取得了重要进展。

Conclusion: DeepSight作为首个专门的深度多模态大语言模型，通过创新的数据集构建和模型架构改进，有效解决了现有模型在深度信息理解方面的不足，推动了三维场景理解技术的发展。
...
TL;DR: 本文提出CR-QAT框架，通过课程量化感知训练和文本中心关系知识蒸馏，解决开放词汇目标检测模型在极端低比特量化下性能严重下降的问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测模型虽然能通过视觉-语言对齐检测新类别，但模型参数量大难以在资源受限设备上部署。量化虽然能压缩模型，但极端低比特量化会严重破坏细粒度视觉-语言对齐和区域间关系结构。

Method: 提出课程关系量化感知训练框架，包含两个核心组件：1）课程QAT：通过分阶段渐进量化，隔离误差积累，确保稳定优化；2）文本中心关系知识蒸馏：构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下始终优于现有QAT基线方法，相对AP提升分别达到38.9%和40.9%。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测模型在极端量化下的性能退化问题，通过课程式渐进量化和关系知识蒸馏的结合，实现了高效模型压缩同时保持视觉-语言对齐能力。
...
TL;DR: CoE是一个无需训练的多模态摘要框架，通过事件链和分层事件图实现结构化推理，在8个数据集上显著超越现有视频CoT基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、隐式融合导致跨模态对齐弱、平面时间建模缺乏事件转换。需要解决这些限制以提升摘要质量。

Method: 提出CoE框架，基于分层事件图（HEG）构建事件链进行结构化推理。HEG将文本语义编码为显式事件层次结构，指导跨模态对齐和时间推理。框架包括关键视觉线索定位、事件演化和因果转换建模，以及轻量级风格适应进行领域对齐。

Result: 在8个多样化数据集上的实验表明，CoE持续优于最先进的视频CoT基线方法，平均提升：ROUGE +3.04、CIDEr +9.51、BERTScore +1.88，显示出强大的鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过事件链和分层事件图的结构化推理，有效解决了多模态摘要中的关键挑战，实现了无需训练的高性能跨领域摘要生成，具有优秀的可解释性和泛化能力。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准数据集，专注于剧烈自我运动和复杂表观轨迹，包含42个视频序列和170万+标注框，相比现有基准更具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角多目标跟踪基准缺乏真实世界中的复杂性，主要包含可预测的相机动态和线性运动模式，无法反映无人机快速移动和敏捷机动带来的显著观测挑战。

Method: 创建了DynUAV基准数据集，包含42个视频序列，标注了超过170万个边界框，涵盖车辆、行人以及挖掘机、推土机、起重机等工业类别。该基准特别强调剧烈的自我运动及其导致的复杂表观轨迹。

Result: 对最先进的跟踪器在DynUAV上的综合评估显示，它们在处理动态条件下的检测和关联交织挑战方面存在局限性，特别是在处理剧烈尺度变化、视角变化和运动模糊方面表现不足。
...
TL;DR: TaPD是一个用于自动驾驶轨迹预测的观察自适应框架，通过渐进知识蒸馏和时间回填技术处理可变历史长度观测，在短观测条件下显著提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在实际场景中（如遮挡或感知范围有限）观测长度可变或极短时性能会大幅下降，需要一种能够适应不同观测长度的统一框架。

Method: 提出TaPD框架，包含两个协作模块：观察自适应预测器（OAF）使用渐进知识蒸馏（PKD）将长时程"教师"的运动模式知识转移到短时程"学生"，通过分层特征回归恢复运动上下文；余弦退火蒸馏权重平衡预测监督和特征对齐；时间回填模块（TBM）为极短历史显式重建缺失的历史片段；采用解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下都优于强基线，在极短输入条件下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD提供了一个统一的即插即用框架，能够有效处理可变历史长度的轨迹预测问题，特别是在短观测条件下显著提升预测性能，为自动驾驶中的轨迹预测提供了鲁棒的解决方案。
...
### [58] [GazeMoE: Perception of Gaze Target with Mixture-of-Experts](https://arxiv.org/abs/2603.06256)
*Zhuangzhuang Dai,Zhongxi Lu,Vincent G. Zakka,Luis J. Manso,Jose M Alcaraz Calero,Chen Li*

Main category: cs.CV

TL;DR: GazeMoE：基于混合专家机制从冻结基础模型中自适应提取多模态线索的端到端视线目标估计框架，在基准数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 从可见图像估计人类视线目标对机器人理解人类注意力至关重要，但现有方法在泛化性架构和训练范式方面仍有挑战。虽然预训练视觉基础模型为定位视线目标提供了新途径，但如何有效整合眼睛、头部姿态、手势和上下文等多模态线索需要自适应且高效的解码机制。

Method: 受大型视觉语言模型中混合专家机制启发，提出GazeMoE框架，通过MoE模块从冻结的基础模型中自适应选择视线目标相关线索。为处理视线目标分类中的类别不平衡问题（框内vs框外）并增强鲁棒性，引入了类别平衡辅助损失和策略性数据增强，包括区域特定裁剪和光度变换。

Result: 在基准数据集上的大量实验表明，GazeMoE实现了最先进的性能，在具有挑战性的视线估计任务上超越了现有方法。
...
TL;DR: DEX-AR是一种为自回归视觉语言模型设计的动态可解释性方法，通过生成逐token和序列级热图来可视化模型决策过程，在多个基准测试中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法主要针对分类任务设计，难以处理现代自回归VLMs的逐token生成过程以及视觉和文本模态间的复杂交互。

Method: DEX-AR通过计算逐token生成过程中注意力图的层梯度来生成逐token和序列级2D热图。该方法包含两个关键创新：1）动态头过滤机制，识别专注于视觉信息的注意力头；2）序列级过滤方法，聚合逐token解释同时区分视觉基础和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，DEX-AR在使用新的归一化困惑度度量的基于扰动的指标以及基于分割的指标上都取得了持续改进。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够更好地理解和可视化这些复杂模型的决策过程，解决了传统方法在处理现代VLMs时面临的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度基础模型在透视图像上训练表现良好，但在360°全景图像上泛化能力差，因为透视和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，这在实际应用中成本高昂。
...
Result: RePer-360仅使用1%的训练数据就超越了标准微调方法。在相同域内训练设置下，RMSE指标实现了约20%的改进。该方法在保持预训练透视先验的同时有效适应了全景域。

Conclusion: 通过将重点从互补投影融合转向在保持预训练透视先验下的全景域适应，RePer-360提供了一种高效的全景深度估计解决方案，显著减少了数据需求并提高了性能，为深度基础模型在全景域的适应提供了新思路。
...
TL;DR: K-MaT是一个用于医学视觉语言模型的提示学习框架，通过知识锚定流形传输实现跨模态迁移，无需低端模态训练图像，在四个跨模态基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 大规模生物医学视觉语言模型在高端影像（如CT）上适配后，往往无法有效迁移到低端模态（如X光），容易陷入模态特定的捷径学习。现有方法需要低端模态的训练图像，这在临床实践中难以获取。

Method: 提出K-MaT框架：1）分解提示词；2）将提示词锚定到临床文本描述；3）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间。该方法无需低端模态训练图像。

Result: 在四个跨模态基准测试（皮肤镜检查、乳腺X光到超声、CT到胸部X光）中取得SOTA结果：平均调和准确率从BiomedCoOp的42.0%提升到44.1%，宏观F1分数达到36.2%。在具有挑战性的乳腺成像任务中，避免了标准方法（如CoOp）的灾难性遗忘问题（准确率降至27.0%）。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了一条高效路径，能够在不使用低端模态训练图像的情况下实现稳健的跨模态性能。
...
TL;DR: EffectMaker是一个统一的推理-生成框架，通过多模态大语言模型理解高级效果语义，结合扩散变换器进行上下文学习，实现基于参考视频的视觉特效定制，无需针对每个特效进行微调。


<details>
  <summary>Details</summary>
Motivation: 当前视觉特效生成面临两大挑战：特效特定数据稀缺，以及超自然或风格化效果建模困难。现有AIGC系统通常需要针对每个特效进行微调，严重限制了其可扩展性和对新特效的泛化能力。

Method: EffectMaker采用语义-视觉双路径引导机制：1) 多模态大语言模型解释高级效果语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索。同时构建了包含130k视频、覆盖3k特效类别的EffectData数据集。

Result: 实验表明，EffectMaker在视觉质量和效果一致性方面优于现有最先进基线方法，提供了一个可扩展且灵活的定制化视觉特效生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了视觉特效定制化的关键挑战，无需针对每个特效进行微调，实现了准确、可控且效果一致的合成，为定制化视觉特效生成提供了可扩展的解决方案。
...
TL;DR: 提出一种解决SAM3等基础模型在医学图像分割中提示敏感性的方法，通过提示组感知训练框架提高文本引导核分割的鲁棒性


<details>
  <summary>Details</summary>
Motivation: SAM3等基础模型在医学图像分割中虽然支持文本引导，但预测结果对提示表述高度敏感，即使是语义等价的描述也会产生不一致的分割结果，限制了在临床和病理工作流程中的可靠性

Method: 将提示敏感性重新表述为组间一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，引入提示组感知训练框架，包括：(1) 质量引导的组正则化，利用分割损失作为隐式排名信号；(2) 采用停止梯度策略的logit级一致性约束，对齐每组内的预测
...
### [64] [MOSIV: Multi-Object System Identification from Videos](https://arxiv.org/abs/2603.06022)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或基于固定材料原型的离散分类，无法处理多物体系统识别问题，特别是在接触丰富的多物体交互场景中。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、每物体的材料参数，通过从视频中提取的几何目标进行指导，并创建了包含接触丰富的多物体交互的合成基准数据集。

Result: 在新建的基准测试中，MOSIV相比适应后的基线方法，在接地准确性和长时程模拟保真度方面都有显著提升，成为该新任务的强基线。

Conclusion: 物体级细粒度监督和几何对齐的目标对于复杂多物体设置中的稳定优化至关重要，MOSIV为多物体系统识别任务提供了有效的解决方案。
...
### [65] [Physical Simulator In-the-Loop Video Generation](https://arxiv.org/abs/2603.06408)
*Lin Geng Foo,Mark He Huang,Alexandros Lattas,Stylianos Moschoglou,Thabo Beeler,Christian Theobalt*

Main category: cs.CV

TL;DR: PSIVG框架通过在视频扩散过程中集成物理模拟器，解决现有扩散视频生成违反物理定律的问题，生成物理一致的运动轨迹并优化纹理一致性。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成技术虽然视觉逼真度高，但经常违反重力、惯性和碰撞等基本物理定律，导致物体运动不一致、动力学不合理，限制了AI生成视频的真实性和可靠性。

Method: 提出PSIVG框架：1) 从预训练扩散模型生成的模板视频中重建4D场景和前景物体网格；2) 在物理模拟器中初始化并生成物理一致的运动轨迹；3) 用模拟轨迹指导视频生成器产生时空物理一致的运动；4) 提出TTCO技术，基于模拟器的像素对应关系调整文本和特征嵌入，提升纹理一致性。

Result: 综合实验表明，PSIVG生成的视频能更好地遵循真实世界物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过将物理模拟器集成到视频扩散过程中，显著提升了生成视频的物理一致性，为解决AI视频生成中的物理违反问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成推理框架存在两大问题：纯文本推理缺乏视觉上下文导致关键空间和视觉元素缺失；文本-图像交错推理虽然增强视觉基础但计算成本高且受限于生成器的表示能力。需要一种既能有效感知视觉结构又计算高效的推理方法。

Method: 提出StruVis框架，采用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构。该方法不依赖中间图像生成，而是通过结构化视觉引导的推理来解锁模型的文本到图像生成潜力。该框架与生成器无关，可无缝集成到各种文本到图像生成器中。

Result: 在推理型文本到图像生成基准测试中取得显著性能提升，在T2I-ReasonBench上获得4.61%增益，在WISE基准上获得4%增益。

Conclusion: StruVis通过结构化视觉表示有效平衡了计算效率和视觉推理能力，为文本到图像生成提供了一种既高效又准确的推理框架，且具有良好的通用性和可扩展性。
...
TL;DR: CLoPA是一种持续适应策略，通过轻量级训练调整nnInteractive模型的小部分参数，在医学图像分割任务中快速提升性能至专家水平。


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型如nnInteractive在不同医学图像分割任务中无法稳定达到专家级性能，而标注活动产生的任务特定标注数据流为在线模型适应提供了自然补充。

Method: 提出CLoPA持续适应策略，在标注缓存上调整nnInteractive模型的小部分参数，通过轻量级训练调度触发，无需新增参数或改变推理流程。

Result: 在8个Medical Segmentation Decathlon任务中，CLoPA快速将性能提升至专家水平，即使nnInteractive先前失败的任务也能成功，多数增益在单次训练后实现。

Conclusion: CLoPA能有效提升医学图像分割性能，不同参数组的调整效果取决于任务特性和数据量，对于复杂几何目标需要更深层次的特征表示对齐。
...
Method: 提出OA-SORT框架，包含三个核心组件：遮挡感知模块（OAM）使用高斯图分析物体遮挡状态；遮挡感知偏移（OAO）利用遮挡状态缓解成本混淆；偏置感知动量（BAM）抑制估计不稳定性。该框架无需训练，可即插即用。

Result: 在DanceTrack测试集上，OA-SORT达到63.1% HOTA和64.2% IDF1。将遮挡感知框架集成到其他四个跟踪器中，平均提升HOTA 2.08%和IDF1 3.05%，证明了框架的可重用性。

Conclusion: 遮挡处理对多目标跟踪至关重要，OA-SORT框架通过创新的遮挡感知机制有效解决了2D MOT中的位置成本混淆问题，且具有良好的可扩展性和可重用性。
...
TL;DR: 该研究针对高维超列特征在图像分割中的计算复杂度问题，提出使用分层子采样和集成学习策略，在脑肿瘤数据集上显著提升了小样本情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的超列特征虽然能有效用于像素分类，但由于计算复杂度随训练集规模线性增长，实际应用受限。研究旨在解决超列特征处理的计算效率问题，特别是在小样本场景下。

Method: 1. 对基于VGG16的超列特征应用分层子采样以降低计算复杂度；2. 研究稀疏超列上的集成学习方法（堆叠和投票集成）；3. 在小样本情况下（N≤20）比较不同分类器的效果。

Result: 在脑肿瘤数据集上，当N=20时，10%分层子采样率下最佳平均Dice得分为0.66。相比标准多尺度UNet基线，性能提升24.53%（p值=3.07e-11，Wilcoxon符号秩检验）。在小样本情况下，简单逻辑回归分类器表现最佳。

Conclusion: 分层子采样和集成学习能有效处理超列特征的计算复杂度问题，在小样本图像分割任务中显著优于传统方法。堆叠和投票集成具有竞争力，但在极低样本情况下，简单逻辑回归是最有效的解决方案。
...
TL;DR: 论文提出FontUse数据集和标注流程，专门解决文本到图像模型中排版控制难题，通过结构化标注和微调现有模型，显著提升了排版样式与提示词的一致性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在排版控制方面存在不足，用户请求的排版样式经常被忽略或弱化执行，需要专门的数据驱动方法来解决这一限制。

Method: 采用数据为中心的方法，构建FontUse数据集（约7万张图像），使用分割模型和多模态大语言模型自动标注，包含用户友好提示词、文本区域位置和OCR识别字符串。通过微调现有生成器，使其能一致地解释样式和使用场景条件。

Result: 实验表明，使用该流程训练的模型在多样化提示词和布局下，生成的文本渲染与请求属性的一致性优于竞争基线。引入了基于Long-CLIP的评估指标来衡量排版与属性对齐。

Conclusion: 提出的数据驱动方法有效解决了文本到图像模型中的排版控制问题，通过结构化标注流程和专门数据集，实现了对排版样式的精确控制，为新手用户提供了直观的排版规范方式。
...
TL;DR: 提出GvU方法，利用统一多模态模型自身的理解能力来提升生成质量，通过理解分支评估生成结果并指导改进，缩小理解与生成能力之间的差距。
...
Method: 提出GvU机制，这是一种基于token级别的内在文本-图像对齐奖励机制，让模型同时作为教师和学生：使用理解分支评估自身输出并指导生成改进。在此基础上设计了自监督强化学习框架，使模型能够通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。

Result: 实验结果表明，该方法显著提升了统一多模态模型的生成能力，同时增强了其细粒度视觉理解能力，缩小了视觉理解与生成之间的能力差距。

Conclusion: 通过利用模型自身的理解能力来指导生成过程，可以有效缩小统一多模态模型中理解与生成能力之间的差距，实现两者相互促进的良性循环。
...
TL;DR: AI系统通过4.5万张超声图像训练，诊断胎儿口面裂的敏感性和特异性超过93%和95%，媲美资深放射科医生，并能提升初级医生诊断能力6%以上。
...
Method: 开发了一个人工智能系统，基于来自22家医院9,215名胎儿的45,139张超声图像进行训练，用于诊断胎儿口面裂。系统作为医疗副驾驶使用，并在24名放射科医生和学员中进行了试点研究。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，并显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提升6%以上。试点研究显示，该系统还能加速罕见疾病临床专业知识的发展。

Conclusion: 这种双重用途方法为改善诊断准确性和专家培训提供了可扩展的解决方案，特别适用于经验丰富的放射科医生稀缺的环境。AI系统不仅能提供直接诊断辅助，还能加速临床专业知识的发展。
...
TL;DR: GenHOI是一种轻量级增强方法，通过时间平衡和空间选择性的方式将参考物体信息注入预训练视频生成模型，显著提升手-物体交互视频合成的物理合理性和物体一致性。


<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互视频合成面临两大挑战：1）现有HOI重演方法通常在特定领域训练和评估，难以泛化到复杂真实场景；2）通用视频编辑模型虽然鲁棒性更强，但仍存在物体外观不一致等HOI特定问题。

Method: 提出GenHOI框架，包含两个核心组件：1）Head-Sliding RoPE：通过为参考令牌分配头部特定的时间偏移，平衡参考信息在时间维度上的影响，缓解3D RoPE的时间衰减问题；2）两级空间注意力门：将物体条件注意力集中在HOI区域，并自适应调整其强度，在保持背景真实性的同时增强交互保真度。

Result: 在未见过的真实场景中进行广泛定性和定量评估，GenHOI在手-物体交互重演和通用视频编辑方法中均显著优于现有最先进方法。

Conclusion: GenHOI通过时间平衡和空间选择性的参考物体信息注入，有效解决了HOI视频合成中的物体一致性和物理合理性问题，为复杂真实场景的手-物体交互视频生成提供了轻量级且有效的解决方案。
...
TL;DR: SUREON是一个从手术教学视频中提取专家推理信息的大规模视频问答数据集，包含12个问题类别，覆盖安全评估、决策理性和预测，用于训练具备手术推理能力的人工智能模型。


<details>
  <summary>Details</summary>
Motivation: 当前的手术AI缺乏理解手术意图、风险判断和下一步预测等高级推理能力，因为大规模标注包含手术推理的训练数据非常困难。然而，手术教学视频中已经包含了专家讲解的意图、理性和预期等推理信息。

Method: 通过多智能体管道从手术学术视频中提取和结构化监督信息，构建包含12个问题类别的大规模视频问答数据集SUREON。开发了两个模型：SureonVLM（通过监督微调适应的视觉语言模型）和SureonVLM-R1（使用组相对策略优化训练的逻辑推理模型）。

Result: SUREON数据集包含134.7K个视频片段、170种手术类型、206.8K个问答对，以及354个专家验证的基准示例。两个模型在复杂手术问题上表现优异，在SUREON基准测试中准确率超过84%，同时在标准手术感知任务上优于通用领域模型。

Conclusion: 从手术教学视频中提取的监督信号能够有效训练具备手术推理能力的AI模型，填补了当前手术AI在理解手术意图和决策推理方面的空白，为开发更智能的手术辅助系统提供了重要基础。
...
### [75] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
TL;DR: Curious-VLA框架通过两阶段设计解决VLA模型中的探索-利用困境：IL阶段使用可行轨迹扩展策略生成多样化数据，RL阶段采用自适应多样性感知采样和跨度驾驶奖励来提升模型性能，在Navsim基准上达到SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 研究发现自动驾驶VLA模型存在"窄策略限制"问题：模仿学习阶段倾向于限制探索，导致后续强化学习阶段因反馈多样性不足而过早饱和，从而限制了模型性能的进一步提升。

Method: 提出Curious-VLA两阶段框架：1) IL阶段：使用可行轨迹扩展策略生成多个物理有效轨迹，采用步进归一化轨迹表示来适应多样化数据；2) RL阶段：引入自适应多样性感知采样优先选择高多样性样本，提出跨度驾驶奖励配合焦点风格加权来扩大奖励值范围，提升对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得SOTA结果：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS达到94.8，证明该框架能有效释放VLA模型的探索潜力。

Conclusion: Curious-VLA通过缓解探索-利用困境，成功解决了VLA模型中的窄策略限制问题，为自动驾驶模型的性能提升提供了有效解决方案，代码已开源供进一步研究。
...
TL;DR: BEVLM框架将空间一致的鸟瞰图表示与LLMs连接，通过BEV特征作为统一输入提升自动驾驶场景的推理能力，同时将LLM语义知识蒸馏到BEV表示中改善端到端驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs导致计算冗余和空间一致性有限，而传统BEV表示虽然提供空间结构但缺乏语义丰富性，需要桥接空间一致性和语义理解能力。

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，并通过蒸馏LLM语义知识到BEV表示中。

Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确率提升46%，同时通过语义知识蒸馏将闭环端到端驾驶性能在安全关键场景中提升29%。

Conclusion: BEVLM成功桥接了空间一致的BEV表示与LLMs的语义理解能力，显著提升了自动驾驶场景的推理准确性和端到端驾驶性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征表示和简单平均聚合，存在两个局限：1) 仅用全局特征难以捕捉细粒度域不变局部细节；2) 统一参数平均忽略客户端在鲁棒特征提取能力上的差异，稀释高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制提取细粒度局部特征，KS（知识选择）机制根据客户端特征提取能力差异进行选择性知识集成

Result: 未在摘要中明确说明具体实验结果，但框架旨在提升模型在未见域上的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过鲁棒知识提取和选择性知识集成，解决了联邦域泛化行人重识别中的关键限制，有望提升模型在未见域上的泛化性能
...
TL;DR: 提出RMD框架，通过跨分辨率分布匹配蒸馏解决扩散蒸馏中的分辨率分布差距问题，实现高保真、少步数的多分辨率级联推理加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散蒸馏方法受限于去噪过程，步数减少已基本饱和；部分时间步低分辨率生成虽能加速，但因跨分辨率分布差距导致质量下降

Method: 使用对数信噪比曲线划分各分辨率时间步区间，引入logSNR映射补偿分辨率引起的偏移；沿分辨率轨迹进行分布匹配以减少低分辨率生成器分布与教师高分辨率分布的差距；在上采样过程中加入预测噪声重注入机制稳定训练

Result: RMD在保持高保真生成的同时显著加速推理，在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速

Conclusion: RMD框架有效桥接跨分辨率分布差距，为高保真、少步数的多分辨率级联推理提供有效解决方案
...
TL;DR: 研究发现视觉语言模型对结构化颜色扭曲敏感，人类表现远优于模型，简单预处理可部分恢复性能


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在基准测试中表现良好，但在面对结构化颜色扭曲时会出现系统性感知弱点，即使人类仍能轻松识别场景，模型却会做出自信但荒谬的预测

Method: 使用空间颜色混合方法，创建8种程序化颜色扭曲变体，在4个数据集上评估9个VLM模型，并进行61人的人类研究对比

Result: 随着扭曲程度增加，模型准确率急剧下降，语言模型规模扩大不能可靠缓解失败；人类在相同扭曲下表现显著优于VLM；简单的人类启发式预处理能恢复部分性能

Conclusion: 视觉语言模型存在系统性感知弱点，需要感知感知的预处理和工具使用策略来提高鲁棒性
...
Result: 定量和定性结果表明，基于扩散的模型更一致地从多模态、剂量感知的条件中受益，产生比基于GAN的基线更稳定、解剖学上更合理的肿瘤演变轨迹。

Conclusion: 虚拟治疗框架作为NSCLC计算机模拟治疗监测和自适应放疗研究的工具有潜力，扩散模型在生成稳定且解剖学合理的肿瘤演变轨迹方面表现更优。
...
TL;DR: FreeOcc：一种无需训练、基于基础模型的3D语义和全景占据预测方法，通过多视角图像重建几何和语义信息


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D场景理解方法通常需要昂贵的密集3D监督或需要在目标域数据上训练模型，这限制了在未见环境中的部署。需要一种无需训练的方法来克服这些限制。

Method: 1. 使用可提示的基础分割模型提取每视角全景先验；2. 使用重建基础模型恢复度量3D点云；3. 通过深度和置信度感知过滤将可靠标签提升到3D；4. 随时间融合并通过确定性细化堆栈进行体素化；5. 通过拟合和合并鲁棒的当前视角3D框候选来恢复实例

Result: 在Occ3D-nuScenes上，FreeOcc无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超越之前的弱监督基线。在全景占据预测方面，无需训练和弱监督分别达到3.1 RayPQ和3.9 RayPQ

Conclusion: FreeOcc证明了基础模型驱动的感知是实现无需训练的3D场景理解的实用途径，为语义和全景占据预测设定了新的基准
...
TL;DR: 提出一种用于乳腺超声图像分割的半监督学习框架，通过免训练的伪标签生成和标签细化，利用视觉语言模型生成结构一致的伪标签，在仅有2.5%标注数据下达到全监督模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在标注数据极少时生成的伪标签不稳定且不准确，而现有视觉语言模型在医学图像上的效果有限，因为领域特定的提示词难以迁移。

Method: 1) 利用简单的外观描述（如"暗色椭圆形"）实现自然图像到医学图像的结构迁移，让VLM生成结构一致的伪标签；2) 用这些伪标签预热静态教师模型以捕获病灶全局结构先验；3) 结合指数移动平均教师模型，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别能力。

Result: 在四个乳腺超声数据集上的实验表明，该方法在仅使用2.5%标注数据的情况下，性能可与全监督模型相媲美，显著优于现有的半监督学习方法。

Conclusion: 该方法提供了一种可扩展的半监督医学图像分割范式，对于其他成像模态或疾病，只需提供全局外观描述即可获得可靠的伪监督，能够在有限标注下实现可扩展的医学图像分割。
...
### [83] [JOPP-3D: Joint Open Vocabulary Semantic Segmentation on Point Clouds and Panoramas](https://arxiv.org/abs/2603.06168)
*Sandeep Inuganti,Hideaki Kanayama,Kanta Shimizu,Mahdi Chamseddine,Soichiro Yokota,Didier Stricker,Jason Rambach*

Main category: cs.CV

TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在开放和封闭词汇的2D和3D语义分割方面相比SOTA有显著提升。
...
Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验评估表明，JOPP-3D能够跨全景和3D域产生连贯且语义有意义的分割结果。

Conclusion: 提出的方法在开放和封闭词汇的2D和3D语义分割方面相比最先进方法取得了显著改进。
...
### [84] [Optimizing 3D Diffusion Models for Medical Imaging via Multi-Scale Reward Learning](https://arxiv.org/abs/2603.06173)
*Yueying Tian,Xudong Han,Meng Zhou,Rodrigo Aviles-Espinosa,Rupert Young,Philip Birch*

Main category: cs.CV

TL;DR: 本文提出了一种使用强化学习增强3D医学图像扩散模型的方法，通过多尺度反馈机制优化生成质量，在脑部MRI数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在3D医学图像生成中表现出强大能力，但标准训练目标与临床需求之间存在差距。需要一种方法能够同时优化局部纹理细节和全局结构一致性，以提高生成图像的质量和临床实用性。

Method: 首先在MRI体积数据上预训练3D扩散模型建立生成先验，然后使用近端策略优化（PPO）进行微调。采用新颖的奖励系统，结合2D切片级评估和3D体积分析，实现多尺度反馈指导。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布发展。定量分析显示Fréchet Inception Distance显著改善，合成数据在肿瘤和疾病分类任务中表现出比未优化基线更好的实用性。

Conclusion: 通过强化学习与多尺度反馈的结合，成功提升了3D医学图像扩散模型的生成质量，使合成数据在临床相关下游任务中更具价值，为医学图像生成提供了新的优化框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的训练免费扩散分割器虽然利用预训练扩散模型进行语义分割，但未能充分利用更强大扩散模型的生成能力。研究发现存在两个关键问题：跨注意力图在多头和多层之间存在差异，以及全局映射无法直接转化为准确的分割结果。

Method: 提出两种技术：1）自动聚合：将多头和多层的跨注意力图整合为统一的全局表示；2）逐像素重缩放：解决不同文本标记之间的分数不平衡问题，使语义相关性更准确。

Result: 在标准语义分割基准测试中表现出改进的性能，并成功集成到生成技术中，展示了广泛的适用性。

Conclusion: 通过自动聚合和逐像素重缩放技术，训练免费扩散分割器能够更好地利用扩散模型的生成能力，实现更准确的语义分割。
...
TL;DR: 提出了一种基于骨架的时间动作分割点监督框架，仅需每个动作片段标注一帧，通过多模态骨架数据和伪标签生成方法显著减少标注成本


<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法能大幅减少标注成本并解决边界模糊问题

Method: 使用预训练统一模型编码关节、骨骼和运动多模态骨架特征；提出原型相似度方法，结合能量函数和约束K-Medoids聚类生成伪标签；采用多模态伪标签集成指导模型训练
...
TL;DR: VG3S是一个利用视觉基础模型的几何先验来增强3D高斯溅射语义占据预测的新框架，通过分层几何特征适配器显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯溅射的占据预测方法严重依赖准确的几何线索，但在纯视觉中心范式中这些线索往往不足。需要将视觉基础模型强大的几何基础能力注入到占据预测中。

Method: 提出VG3S框架，通过可插拔的分层几何特征适配器，将冻结的视觉基础模型的丰富3D几何先验转化为适用于占据预测的特征，包括特征聚合、任务特定对齐和多尺度重构。

Result: 在nuScenes占据基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。该框架能够无缝泛化到不同的视觉基础模型，一致地提升占据预测精度。

Conclusion: VG3S证明了整合来自预训练的几何基础视觉基础模型的先验具有巨大价值，为自动驾驶场景理解提供了更准确的3D语义占据预测解决方案。
...
TL;DR: 提出了一种基于特征熵优化的邻域密度化策略EntON，用于3D高斯溅射，通过交替优化框架在几何精度和渲染质量间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 标准3DGS生成的高斯中心与物体几何表面对齐不佳，而专注于表面重建的方法常牺牲光度精度。需要一种既能保持几何准确性又能保证渲染质量的方法。

Method: 引入基于特征熵的几何感知策略来指导自适应分裂和剪枝。通过计算每个高斯中心k近邻协方差矩阵特征值的3D形状特征熵来量化局部结构有序性，并将其集成到交替优化框架中：交替进行标准梯度密度化和特征熵感知密度化。

Result: 在DTU和TUM2TWIN数据集上的实验表明，该方法将几何精度提升达33%，渲染质量提升达7%，同时减少高达50%的高斯数量和23%的训练时间。
...
TL;DR: WAFL提出了一种新的时序伪造定位范式，将任务从时间回归和连续定位转变为离散的词级二元分类，显著提升了定位性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有时序伪造定位方法存在特征粒度不对齐和计算成本高的问题，需要一种更高效准确的解决方案。

Method: 1) 识别最小有意义的伪造单元——词标记，并按语音自然语言边界对齐数据预处理；2) 引入法证特征重对齐模块，将预训练语义空间表示映射到可区分的法证流形；3) 设计基于伪影的不对称损失函数，动态抑制真实梯度并优先考虑细微法证伪影。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有最先进方法，同时需要更少的可学习参数并具有更高的计算效率。

Conclusion: 通过将时序伪造定位重新定义为离散的词级二元分类任务，WAFL解决了特征粒度不对齐和计算成本高的问题，为时序伪造检测提供了高效准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达式分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理匹配多个目标或零目标的细粒度自然语言描述。

Method: 1. 分层视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级双粒度特征，在2D到3D投影中保持物体边界。2. 渐进多级融合：通过模态内协作、2D语义与3D几何特征的跨模态自适应加权，以及语言引导的细化来整合表示。

Result: HCF-RES在ScanRefer和Multi3DRefer两个基准测试中都达到了最先进的性能。

Conclusion: 提出的多模态框架通过分层视觉语义分解和渐进多级融合，有效解决了3D指代表达式分割中的视觉语义缺失问题，实现了更精确的对象定位。
...
TL;DR: NOVA提出了一种创新的开放词汇自回归3D多目标跟踪范式，通过将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力解决开放世界感知中的未知目标泛化问题。
...
Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力将跟踪任务转化为原则性的下一步序列完成过程。通过语言空间的层次结构解决细粒度语义模糊性，并通过高层常识推理保持复杂长序列中的身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上的广泛实验表明NOVA具有优越性能。在nuScenes数据集上，NOVA在Novel类别上实现了22.41%的AMOTA，相比基线有20.21%的绝对提升。这些成果通过紧凑的0.5B自回归模型实现。

Conclusion: NOVA通过将3D跟踪从传统的基于距离的碎片化匹配转向生成式时空语义建模，成功解决了开放世界感知中的未知目标泛化问题，为3D多目标跟踪提供了新的创新范式。
...
TL;DR: 提出StrSR框架，通过谱和轨迹正则化解决扩散变换器在真实图像超分辨率中的单步蒸馏问题，消除网格状伪影并提升性能
...
Result: 大量实验表明StrSR在真实图像超分辨率中实现了最先进的性能，在定量指标和视觉感知方面均表现优异
...
<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是机器人感知的关键，但真实世界的体素标注存在结构伪影和动态拖尾效应等噪声。现有方法在这种不可靠的监督下存在安全隐患，而2D标签噪声学习策略在稀疏3D体素空间中会灾难性失效。

Method: 提出了DPR-Occ框架，通过双源部分标签推理构建可靠监督。该方法结合时间模型记忆和表示层结构相似性，动态扩展和修剪候选标签集，在抑制噪声传播的同时保留真实语义。

Result: 在SemanticKITTI数据集上的实验表明，DPR-Occ在极端噪声下能防止几何和语义崩溃。即使在90%标签噪声下，相比现有方法仍能获得显著性能提升（mIoU提升2.57%，IoU提升13.91%）。

Conclusion: 通过桥接标签噪声学习和3D感知，OccNL基准和DPR-Occ方法为动态环境中安全关键的机器人感知提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习虽然能合成未见类的特征，但面临两个内在挑战：1) 类级属性无法捕捉实例特定的视觉外观，导致类-实例差距；2) 语义与视觉特征分布不匹配，表现为类间相关性，导致语义-视觉域差距。

Method: 提出ADiVA方法，包含两个模块：1) 属性分布建模(ADM)模块，学习每个类的可迁移属性分布并为未见类采样实例级属性；2) 视觉引导对齐(VGA)模块，细化语义表示以更好地反映视觉结构。

Result: 在三个广泛使用的基准数据集上的实验表明，ADiVA显著优于最先进的方法（在AWA2和SUN上分别获得4.7%和6.1%的提升），并且可以作为插件增强现有的生成式ZSL方法。

Conclusion: ADiVA通过联合建模属性分布和执行显式语义-视觉对齐，有效解决了生成式零样本学习中的类-实例差距和语义-视觉域差距问题，为ZSL研究提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖T2V模型的中间输出构建运动指导，导致计算开销大且灵活性有限。本文旨在开发更高效灵活的运动迁移方法。

Method: 提出FlowMotion框架：1）利用流基T2V模型的预测输出直接提取运动表示；2）基于早期潜在预测编码的丰富时间信息设计流指导；3）引入速度正则化策略稳定优化并确保平滑运动演化

Result: FlowMotion在时间和资源效率上表现优异，与最先进方法相比具有竞争力，通过纯模型预测操作实现高效运动迁移

Conclusion: FlowMotion通过直接利用流基T2V模型的预测输出，实现了高效灵活的视频运动迁移，为无训练运动迁移提供了新的解决方案
...
Method: 提出3D牙科种植体修复方法，基于垂直分数扩散模型，每个模型在不同平面上训练并在投影域中操作。通过结合两个2D分数扩散模型在采样方案中，对投影序列的3D分布进行建模。

Result: 该方法能生成高质量、伪影减少的3D CBCT图像，证明其在改善临床成像方面的有效性。

Conclusion: 提出的基于垂直分数扩散模型的3D牙科种植体修复方法是改善CBCT图像质量的有前景解决方案，通过建模投影序列的3D分布解决了现有2D方法忽略投影相关性的问题。
...
### [97] [WMoE-CLIP: Wavelet-Enhanced Mixture-of-Experts Prompt Learning for Zero-Shot Anomaly Detection](https://arxiv.org/abs/2603.06313)
*Peng Chen,Chao Huang*

Main category: cs.CV

TL;DR: 提出了一种基于小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义表示，结合小波分解提取多频特征，提升对未见异常的检测能力。
...
Method: 提出小波增强的专家混合提示学习方法：1) 使用变分自编码器建模全局语义表示并集成到提示中；2) 通过小波分解提取多频图像特征，动态优化文本嵌入；3) 引入语义感知的专家混合模块聚合上下文信息。

Result: 在14个工业和医学数据集上的广泛实验证明了该方法的有效性。

Conclusion: 该方法通过结合全局语义建模、多频特征提取和专家混合机制，显著提升了零样本异常检测的性能。
...
### [98] [P-SLCR: Unsupervised Point Cloud Semantic Segmentation via Prototypes Structure Learning and Consistent Reasoning](https://arxiv.org/abs/2603.06321)
...
Conclusion: 提出的P-SLCR方法通过原型库驱动的结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上超越了现有无监督方法，甚至在某些情况下优于全监督方法。
...
### [99] [WorldCache: Accelerating World Models for Free via Heterogeneous Token Caching](https://arxiv.org/abs/2603.06331)
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以用于交互式应用和长时程推演。现有特征缓存方法虽然能加速推理，但由于令牌异构性和非均匀时间动态性这两个世界模型特有的障碍，导致策略迁移效果不佳。
...
Conclusion: WorldCache是针对扩散世界模型量身定制的缓存框架，有效解决了令牌异构性和非均匀时间动态性问题，显著提升了推理效率，为资源受限场景下的世界模拟提供了实用解决方案。
...
### [100] [LATO: 3D Mesh Flow Matching with Structured TOpology Preserving LAtents](https://arxiv.org/abs/2603.06357)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于等值面/三角形的扩散模型和自回归生成方法在生成复杂几何形状和良好拓扑结构方面存在效率问题，需要一种更高效、能保持拓扑的3D网格生成方法。

Method: 1. 使用顶点位移场(VDF)表示网格；2. 采用稀疏体素变分自编码器(VAE)将显式信号压缩为结构化、拓扑感知的体素潜在表示；3. 通过渐进细分和剪裁体素来实例化精确顶点位置；4. 专用连接头查询体素潜在表示以直接预测顶点对之间的边连接性；5. 采用两阶段流匹配过程进行生成建模。

Result: LATO能够生成具有复杂几何形状和良好拓扑结构的网格，同时在推理过程中具有高效率，优于先前的等值面/三角形扩散模型和自回归生成方法。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示方法，实现了可扩展的流匹配3D网格生成，无需等值面提取或启发式网格化，在生成质量和效率方面都有显著优势。
...
### [101] [OralGPT-Plus: Learning to Use Visual Tools via Reinforcement Learning for Panoramic X-ray Analysis](https://arxiv.org/abs/2603.06366)
...
Method: 1. 构建DentalProbe数据集（5000张图像，含专家标注的诊断轨迹）；2. 开发Reinspection驱动的强化学习框架，包含基于规则的奖励和条件诊断驱动奖励；3. 提出MMOral-X基准测试（300个开放性问题，多难度级别）

Result: OralGPT-Plus在MMOral-X和现有全景基准测试上相比强基线模型表现出持续可靠的改进，验证了交互式和对称感知推理的有效性

Conclusion: 该研究强调了智能建模在牙科影像中的价值，为未来临床对齐的全景X光片分析研究奠定了基础
...
TL;DR: Rewis3d利用前馈3D重建技术提升2D图像的弱监督语义分割性能，通过3D几何结构传播稀疏标注，在稀疏监督任务中达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注成本高昂，稀疏标注虽然效率更高但存在性能差距。需要利用3D场景重建作为辅助监督信号来弥补这一差距。

Method: 采用双师生架构，利用前馈3D重建技术从2D视频恢复3D几何结构，在2D图像和重建的3D点云之间强制语义一致性，利用几何监督传播稀疏标注。

Result: Rewis3d在稀疏监督任务中达到最先进性能，比现有方法提升2-7%，且不需要额外标注或推理开销。

Conclusion: 3D几何结构为2D语义分割提供了有效的监督信号，能够显著提升弱监督分割性能，为解决标注成本问题提供了新思路。
...
Method: 使用基于DINOv3构建的深度估计模型，从高分辨率光学卫星影像推导树冠高度；通过大规模地理多样性训练数据扩展、自动化数据整理与配准，以及针对树冠高度分布定制的损失函数和数据采样策略。

Result: 相比现有产品，CHMv2显著提高了精度，减少了高森林的偏差，并更好地保持了树冠边缘和间隙等精细结构；在独立ALS测试集以及数千万个GEDI和ICESat-2观测数据上验证，在主要森林生物群落中表现一致。

Conclusion: CHMv2通过先进的方法和大量训练数据，提供了全球范围内高精度、米级分辨率的树冠高度图，解决了现有产品在精度和细节保持方面的不足。
...
TL;DR: REACT++是基于REACT架构改进的实时场景图生成模型，通过高效特征提取和原型空间中的主体到客体交叉注意力机制，在保持物体检测性能的同时提升关系预测准确率，并实现20%的速度提升。


<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法往往只专注于关系预测准确性、物体检测准确性或降低延迟中的单一目标，缺乏同时平衡这三个目标的解决方案。为了支持实时应用，需要在性能与推理速度之间取得平衡。

Method: 基于REACT架构，采用高效特征提取和原型空间中的主体到客体交叉注意力机制，平衡延迟和表征能力。

Result: REACT++在现有SGG模型中达到最高推理速度，相比前代REACT版本快20%，关系预测准确率平均提升10%，同时不牺牲物体检测性能。

Conclusion: REACT++实现了实时场景图生成中性能与速度的最佳平衡，为下游任务如具身智能体推理提供了高效基础组件。
...
TL;DR: 提出人机协同的拼图框架，用于解决真实世界考古文物碎片重组问题，结合自动算法与人工交互指导，显著优于纯自动和纯人工方法。


<details>
  <summary>Details</summary>
Motivation: 真实世界考古文物碎片重组面临严重挑战：碎片受侵蚀、缺失区域、形状不规则、大规模模糊性。传统拼图算法针对干净合成场景设计，在RePAIR基准测试中处理数千碎片时表现不佳。

Method: 提出人机协同拼图解决框架，集成自动松弛标记求解器与交互式人工指导。用户可迭代锁定已验证位置、纠正错误、引导系统实现语义和几何一致的组装。引入两种互补交互策略：迭代锚定和连续交互优化。

Result: 在多个RePAIR数据集上的实验表明，这种人机混合方法在准确性和效率上显著优于纯自动和纯人工基线方法。

Conclusion: 该框架为大规模专家参与式文物重组提供了实用解决方案，通过人机协同有效应对真实世界考古碎片重组的复杂性。
...
TL;DR: DiffInf：基于自影响力引导扩散的框架，用于减轻面部属性学习中的标注不一致问题，通过生成式校正而非丢弃样本来提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 面部属性分类依赖大规模标注数据集，但许多属性（如年龄、表情）本质上是模糊和连续的，却被离散化为分类标签。标注不一致源于主观性和视觉混淆因素（姿态、光照、表情、人口统计变异），导致图像与分配标签不匹配，这些监督错误损害表示学习并降低下游预测性能。

Method: 1. 训练基线分类器并计算样本级自影响力分数（使用一阶近似）识别对优化过程产生不成比例影响的训练实例；2. 通过潜在扩散自编码器对这些高影响力样本进行针对性生成校正，使视觉内容与分配标签更好对齐，同时保持身份和真实感；3. 训练轻量级高影响力成员预测器作为替代影响力正则化器，在校正过程中提供可微分指导；4. 用编辑后的样本替换原始样本，生成大小不变的影响力精炼数据集。

Result: 在多类别面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响力的过滤方法，持续提升泛化性能。结果表明，在图像层面修复有影响力的标注不一致，能在不牺牲分布覆盖的情况下增强下游面部属性分类。

Conclusion: DiffInf框架通过生成式校正而非丢弃样本的方式有效缓解面部属性学习中的标注不一致问题，证明了修复有影响力的标注不一致在图像层面的有效性，为处理标注噪声提供了新思路。
...
TL;DR: 研究通过合成飞镖形状的感知冲突实验，发现BEiT视觉Transformer在局部几何证据与全局组织先验的竞争中偏好凸性完成，并识别出特定注意力头L0H9作为早期种子驱动这一偏好


<details>
  <summary>Details</summary>
Motivation: 探究视觉Transformer如何解决图形-背景组织中的感知歧义，特别是在局部几何证据与全局格式塔先验（如凸性）发生冲突时，定位BEiT模型中实现凸性先验的内部组件

Method: 使用合成飞镖形状创建感知冲突，系统性地掩蔽可同时接受凹性或凸性完成的区域；通过logit归因将内部激活投影到离散视觉词表空间，分解注意力头的直接影响来识别功能单元

Result: BEiT在竞争中可靠地偏好凸性完成；图形-背景组织在早期和中间层保持模糊，在深层突然解决；识别出头L0H9作为早期种子引入对凸性的弱偏置；下调该注意力头可使感知冲突跨越连续决策边界，允许凹性证据引导完成

Conclusion: 视觉Transformer中的格式塔先验（如凸性）由特定可识别的功能单元实现，单个注意力头可以调节感知决策，为理解Transformer内部如何实现感知组织提供了机制性见解
...
Method: 构建WanderDream数据集：WanderDream-Gen包含来自HM3D、ScanNet++和真实场景的15.8K全景视频，描绘从当前视角到目标情境的想象轨迹；WanderDream-QA包含158K问答对，涵盖轨迹的起始状态、路径和结束状态

Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现优异；(3)想象力显著促进WanderDream-QA的推理能力；(4)WanderDream数据在真实场景中具有显著可迁移性

Conclusion: WanderDream为无需主动探索的心理模拟推理提供了首个大规模数据集，证明了心理探索在空间推理中的重要性，并为世界模型和多模态大语言模型的评估提供了新基准
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器存在两个主要问题：1）将2D图像块展平为非因果序列，或使用启发式排序与"下一标记预测"模式不对齐；2）扩散自编码器要么缺乏因果性，要么引入不平衡。需要开发真正的因果视觉分词器来支持自回归语言模型范式。

Method: 提出CaTok：1）通过时间间隔选择标记并将其绑定到MeanFlow目标，学习因果1D表示；2）使用MeanFlow解码器支持快速一步生成和高保真多步采样；3）提出REPA-A正则化，将编码器特征与视觉基础模型对齐，稳定和加速训练。

Result: 在ImageNet重建任务上达到最先进结果：FID 0.75、PSNR 22.53、SSIM 0.674，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功将因果分词范式扩展到视觉领域，学习到的因果1D表示既能支持快速生成，又能捕获跨标记间隔的多样化视觉概念，为视觉自回归模型提供了有效的分词基础。
...
TL;DR: Pinterest Canvas是一个大规模图像生成系统，专门用于图像编辑和增强任务，通过基础扩散模型和任务特定微调来满足严格的产品需求。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例。需要构建一个能够支持Pinterest图像编辑和增强用例的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对具体下游任务在任务特定数据集上快速微调，生成专门化模型。包括数据集策划、训练和推理的最佳实践。

Result: 在线A/B实验显示增强图像分别获得18.0%和12.5%的参与度提升；与人类评分者比较表明模型在这些任务上优于第三方模型。系统还可推广到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法，能够有效满足严格的产品需求，在图像编辑和增强任务上表现出色，并具有广泛的下游任务泛化能力。
...
TL;DR: 该论文系统研究去噪生成模型的训练目标，重点分析损失权重和输出参数化（噪声、干净图像、速度）如何与数据流形内在维度、模型架构和数据集大小相互作用，旨在为流匹配模型提供实用设计指导。


<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型）在训练目标设计上存在多种选择（损失权重、输出参数化），但这些选择如何与数据特性、模型架构等因素相互作用尚不清楚。作者旨在系统分析这些因素，为实际训练提供指导而非提出新方法。

Method: 通过系统数值研究，分析不同训练目标（噪声、干净图像、速度参数化）与数据流形内在维度、模型架构、数据集大小的相互作用。实验涵盖几何可控的合成数据集和真实图像数据，使用PSNR（去噪精度）和FID（生成质量）作为评估指标。

Result: 研究揭示了不同训练目标与数据特性之间的复杂相互作用，为流匹配模型的设计选择提供了实证依据。具体发现包括不同参数化方案在不同数据条件下的表现差异，以及损失权重策略对模型性能的影响。

Conclusion: 该研究系统解耦了影响流匹配模型训练的各种因素，为实际应用中的设计选择提供了实用见解。研究强调需要根据数据特性、模型架构和可用数据量来选择合适的训练目标和参数化方案。
...
TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督设计而非数据堆叠，在单张24GB GPU上24小时内达到SOTA性能，挑战了"规模至上"的教条。


<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型(RFMs)开发过度依赖暴力扩展，直接移植自然图像处理方法，导致模型在临床实践中脆弱且昂贵，需要更高效、精确的预训练框架。
...
Conclusion: GreenRFM以资源高效的方式实现了SOTA性能，挑战了"规模至上"的教条，使临床医生即使在笔记本电脑上也能公平地开发最先进的放射学基础模型。
...
TL;DR: Match4Annotate：一个轻量级框架，用于视频内和视频间的点与掩码标注传播，通过SIREN隐式神经表示和变形场学习实现跨视频标注


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪和分割模型需要逐视频初始化且无法跨视频泛化；传统对应方法在低纹理场景中表现不佳；密集特征匹配和一次性分割方法缺乏时空平滑性且不支持点与掩码标注的统一处理。

Method: 提出Match4Annotate框架：1）使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应匹配；3）支持视频内和视频间的点与掩码标注传播。

Result: 在三个具有挑战性的临床超声数据集上评估，Match4Annotate在跨视频传播方面达到最先进水平，优于特征匹配和一次性分割基线方法，同时在视频内传播方面与专用跟踪器保持竞争力。

Conclusion: 轻量级、测试时优化的特征匹配流程有潜力为可扩展的标注工作流提供高效且易用的解决方案，特别是在医学影像等专业领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在三个问题：需要单独训练、目标不一致、缩放行为不可预测。这种依赖源于模型训练目标（去噪任务）缺乏学习语义表征的激励
...
TL;DR: AV-Unified是一个统一的多任务音频-视觉场景理解框架，通过将不同任务的输入输出标准化为离散令牌序列，实现跨任务的联合学习，包含多尺度时空感知模块和跨模态引导机制。
...
Method: 1) 将所有任务的输入输出统一为离散令牌序列；2) 设计多尺度时间感知模块捕捉不同时间粒度的音频-视觉事件；3) 设计基于跨模态引导的空间感知模块建模空间音频-视觉关联；4) 使用任务特定的文本提示增强模型适应性和任务感知能力。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS和AVS）上的实验表明，AV-Unified在时间、空间和时空任务上都表现出有效性。

Conclusion: AV-Unified提供了一个统一的框架，能够联合学习多种音频-视觉场景理解任务，通过标准化表示和多尺度时空感知，实现了对复杂音频-视觉场景的全面理解。
...
<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间LiDAR通过宽视场聚集光子返回形成深度直方图，违反了标准LiDAR-RGB校准的单射线假设，需要新的空间校准方法

Method: 使用扫描反光片结合背景减法，估计每个漫反射LiDAR像素在共置RGB图像平面上的足迹（有效支持区域）和相对空间灵敏度，恢复像素级响应映射

Result: 在ams OSRAM TMF8828设备上演示了该方法，获得了明确的LiDAR到RGB对应关系，支持跨模态对齐和融合

Conclusion: 该方法为漫反射LiDAR提供了简单有效的空间校准程序，解决了宽视场光子聚集带来的校准挑战，实现了精确的跨模态对齐
...
TL;DR: 该论文提出了一种在扩散生成模型中处理语言否定的统一框架，将否定视为扩散动态中语义引导的结构化可行性约束，无需重新训练模型参数。


<details>
  <summary>Details</summary>
Motivation: 否定是基本的语言运算符，但在基于扩散的生成系统中建模不足。当前方法缺乏对语言否定的形式化处理，需要一种统一框架来处理多样化的否定现象。

Method: 将语言否定重新解释为扩散动态中语义引导的结构化可行性约束。将无分类器引导重新解释为定义语义更新方向，并通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练扩散主干兼容。

Result: 该方法实现了稳健的否定合规性，同时保持了视觉保真度和结构连贯性。建立了扩散生成模型中语言否定的第一个统一表述，超越了表示级评估。还引入了结构化的否定中心基准套件。

Conclusion: 该工作为扩散生成模型中的语言否定提供了形式化处理，提出了一个统一的训练无关框架，能够处理多种否定现象，并扩展到视频生成，为这一领域的研究奠定了基础。
...
TL;DR: 该研究分析了自动驾驶数据集中冗余数据对目标检测性能的影响，发现选择性移除多源图像中的冗余标注能提升检测精度，揭示了冗余作为可量化数据质量因素的重要性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究主要关注算法设计而忽视了数据质量分析，特别是冗余这一基本但未被充分探索的数据质量问题。数据质量因环境和传感器限制在不同来源和模态间存在差异，影响自动驾驶的实时决策性能。

Method: 使用nuScenes和Argoverse 2数据集，建模和测量多源相机数据以及多模态图像-LiDAR数据中的冗余。通过选择性移除冗余标注，评估其对YOLOv8目标检测任务的影响。

Result: 在nuScenes中，选择性移除共享视野相机的冗余图像目标标注使mAP50在三个代表性重叠区域分别从0.66提升到0.70、0.64到0.67、0.53到0.55。在AV2中，移除4.1%-8.6%的标签后，mAP50保持在0.64基线附近。多模态分析也揭示了图像和LiDAR数据之间存在显著冗余。
...
TL;DR: 提出Penguin-VL，一种紧凑型视觉语言模型，通过使用文本LLM初始化的视觉编码器替代传统对比预训练编码器，在保持轻量级架构的同时实现高性能


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要依赖模型规模扩展，难以部署在计算资源受限的移动和边缘设备上。同时，传统基于对比预训练（如CLIP/SigLIP）的视觉编码器存在目标不匹配问题，其优化的判别性学习抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而非传统的对比预训练编码器。这种方法解决了目标不匹配问题，能够保留更细粒度的空间和时间线索，提高视觉保真度和数据效率。

Result: 在多个图像和视频基准测试中，Penguin-VL在数学推理方面与领先的VLM（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务中表现更优。这些成果是在轻量级架构（2B和8B参数）下实现的，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-VL展示了紧凑型VLM的性能极限，通过使用文本LLM初始化的视觉编码器替代传统对比预训练编码器，实现了更高的视觉保真度和数据效率。该方法为计算效率高的VLM提供了强大的替代方案，能够在资源受限的环境中实现高性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用自回归架构，在架构设计上存在改进空间。离散扩散模型在其他领域表现出色，有潜力成为多模态系统的骨干架构。

Method: 使用统一的掩码离散扩散模型直接捕捉离散多模态标记的联合分布，支持双模态和多模态任务。

Result: 在多样化基准测试中，该方法优于或与现有处理两个及以上模态的多模态系统表现相当。

Conclusion: 扩散模型在驱动下一代多模态基础模型方面具有显著潜力。
...
TL;DR: 该研究发现多模态大语言模型在分类任务中的表现差异主要源于评估协议和标注质量问题，而非模型本身缺陷。通过修正评估协议和标注噪声，MLLMs与监督模型的性能差距显著缩小。


<details>
  <summary>Details</summary>
Motivation: 现有研究对多模态大语言模型与监督模型、视觉语言模型的比较存在矛盾结论，这些矛盾源于评估协议的不一致和标注质量问题，需要系统分析并修正这些问题。

Method: 1. 识别并修正常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱多选干扰项导致结果膨胀、开放世界设置因输出映射不佳而表现不佳
2. 量化常被忽视的设计选择影响：批次大小、图像顺序、文本编码器选择
3. 创建ReGT数据集：对625个ImageNet-1k类别进行多标签重新标注
4. 进行受控案例研究：评估MLLMs辅助人类标注的效果

Result: 1. 修正标注标签后，MLLMs性能提升高达+10.8%，显著缩小与监督模型的感知差距
2. 评估协议的设计选择（批次大小、图像顺序、文本编码器）对准确率有显著影响
3. 较少依赖监督训练信号的模型对标注质量最敏感
4. 在困难案例中，标注者确认或整合MLLMs预测的比例约为50%，显示其在数据集标注中的潜力

Conclusion: MLLMs在分类任务中的表现不佳主要是标注噪声和有缺陷的评估协议造成的假象，而非真正的模型缺陷。修正这些问题后，MLLMs表现显著改善，且能有效辅助大规模数据集标注工作。
...
TL;DR: Aletheia是一款基于RAG和LLM的浏览器扩展，通过证据解释、讨论社区和事实核查功能来检测假新闻，在性能和用户体验上都优于现有方法。
...
Method: 开发Aletheia浏览器扩展，结合检索增强生成（RAG）和大语言模型（LLM）检测假新闻并提供基于证据的解释；包含两个交互组件：讨论中心（用户对话平台）和"保持知情"功能（展示近期事实核查）。

Result: 实验表明Aletheia在检测性能上优于最先进的基线方法；用户研究（250名参与者）证实了系统的可用性和感知有效性。

Conclusion: Aletheia作为一种透明工具，在打击在线假新闻方面具有潜力，通过技术性能和用户体验的双重验证，展示了其在假新闻检测领域的应用价值。
...
TL;DR: 本文研究了语法约束解码(GCD)作为自回归下一个token分布与基于上下文无关文法(CFG)编译的下推系统可达性预言之间的耦合。证明了预言不变性定理，分析了语法等价但计算成本不同的现象，建立了引擎无关的下界，并推导了硬掩码解码的KL散度和总变差失真界限。


<details>
  <summary>Details</summary>
Motivation: 语法约束解码在语言模型中确保输出符合特定语法结构，但不同语法表示可能导致显著不同的计算成本。需要理解语法等价性如何影响解码效率和性能，为语法优化提供理论基础。

Method: 将GCD建模为自回归下一个token分布与CFG编译的下推系统可达性预言的耦合。证明预言不变性定理，分析语法结构对状态空间膨胀的影响，引入结构歧义成本(SAC)度量增量解析森林增长，建立引擎无关下界，推导硬掩码解码的失真界限。

Result: 证明了语言等价文法在每个前缀处产生相同的可接受下一个token集合，但可导致不同的编译状态空间和在线歧义成本。对于a^n b^n语言，给出了精确的控制状态膨胀计数。SAC在右递归下为O(1)每token，在连接下为Θ(t^2)每token和Θ(n^3)累计。建立了Ω(t^2)每token的引擎无关下界。

Conclusion: 语法约束解码中，语法等价性不保证计算效率等价。结构歧义成本是影响解码效率的关键因素，存在最小SAC的代表文法。硬掩码解码的失真界限由可接受下一个token之间的生存概率分布决定。这些结果为语法优化和高效解码引擎设计提供了理论基础。
...
TL;DR: NOTAI.AI是一个可解释的机器生成文本检测框架，通过整合曲率信号、神经特征和文体特征，在监督设置下扩展Fast-DetectGPT，使用XGBoost元分类器和SHAP解释提供透明化检测。


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释的机器生成文本检测系统，不仅能够准确区分人类和AI生成的文本，还能提供透明的决策依据和用户友好的解释，解决传统黑盒检测方法缺乏可解释性的问题。

Method: 整合17个可解释特征（包括条件概率曲率、ModernBERT检测器分数、可读性指标和文体特征），使用梯度提升树（XGBoost）作为元分类器，结合SHAP进行特征归因，并通过LLM生成自然语言解释。

Result: 开发了一个交互式Web应用程序，支持实时分析、可视化特征检查和结构化证据呈现，源代码和演示视频已公开以支持可复现性。

Conclusion: NOTAI.AI提供了一个端到端的可解释机器生成文本检测框架，通过多特征融合、透明化决策过程和用户友好的解释界面，实现了准确且可解释的AI文本检测。
...
TL;DR: CoT提示提升LLM推理能力但增加隐私风险，使PII从提示泄露到推理轨迹和输出中，即使模型被指示不重复PII。研究通过模型无关框架分析推理时PII泄露，发现CoT持续增加泄露风险，尤其是高风险类别，且泄露程度与模型家族和推理预算相关。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决CoT提示在提升大语言模型推理能力的同时，可能增加隐私风险的问题。即使模型被明确指示不要重复个人身份信息，PII仍可能从提示泄露到推理轨迹和最终输出中，这构成了直接的推理时隐私泄露风险。

Method: 采用模型无关框架：(1)将泄露定义为11种PII类型的风险加权、令牌级事件；(2)追踪泄露曲线作为允许CoT预算的函数；(3)在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。同时评估轻量级推理时门控机制：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: 研究发现CoT持续增加PII泄露，特别是高风险类别。泄露程度强烈依赖于模型家族和推理预算。增加推理预算可能放大或减弱泄露，具体取决于基础模型。在门控机制评估中，没有单一方法在所有模型或预算下表现最优，风险加权F1、Macro-F1和召回率指标显示需要混合、风格自适应的门控策略。

Conclusion: 结论是CoT提示确实增加隐私风险，需要开发混合、风格自适应的门控策略来平衡实用性和风险。研究强调了在通用、可复现协议下管理推理时PII泄露的重要性，为隐私保护LLM推理提供了方法论框架。
...
TL;DR: 研究通过扰动框架测试LLM道德判断的稳定性和可操纵性，发现视角转换比表面编辑更易改变判断，协议选择影响最大，表明LLM道德判断受叙事形式和任务框架共同影响。


<details>
  <summary>Details</summary>
Motivation: 随着人们越来越多地使用大型语言模型（LLM）获取日常道德和人际指导，但这些系统无法审视缺失的上下文，只能基于呈现的困境进行判断。需要测试LLM道德判断的稳定性和可操纵性。

Method: 引入扰动框架，使用2,939个来自r/AmItheAsshole的道德困境案例，生成三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）、说服线索（自我定位、社会证明、模式承认、受害者框架）。同时改变评估协议（输出排序、指令放置、非结构化提示）。使用四种模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）进行评估，共129,156个判断。

Result: 表面扰动产生低翻转率（7.5%），主要处于自一致性噪声范围内（4-13%），而视角转换导致显著更高的不稳定性（24.3%）。37.9%的困境对表面噪声稳健但在视角变化下翻转。不稳定性集中在道德模糊案例中；没有明确责任方的场景最易受影响。说服扰动产生系统性方向性偏移。协议选择影响最大：结构化协议间一致性仅67.6%（kappa=0.55），仅35.7%的模型-场景单元在所有三种协议中匹配。

Conclusion: LLM道德判断是由叙事形式和任务框架共同产生的，当结果依赖于呈现技巧而非道德实质时，会引发可重复性和公平性担忧。
...
<details>
  <summary>Details</summary>
Motivation: 开发FreeTxt-Vi的主要动机是降低多语言文本分析的技术门槛，特别是支持越南语这种广泛使用但在NLP领域代表性不足的语言，促进可重复研究并推动越南语语言资源的发展。

Method: 系统采用统一的双语NLP流水线设计，整合了混合VnCoreNLP和字节对编码（BPE）分词策略、微调的TabularisAI情感分类器和微调的Qwen2.5模型进行抽象摘要。系统提供语料库分析功能（如索引、关键词分析、词关系探索和交互可视化）以及基于transformer的NLP组件（情感分析和摘要生成）。

Result: 通过三部分评估（分词、情感分析和摘要生成），该方法在越南语和英语上都实现了与广泛使用的基线方法相当或更优的性能。系统在教育、数字人文、文化遗产和社会科学等领域具有应用价值。

Conclusion: FreeTxt-Vi通过降低技术障碍，支持可重复研究，并促进越南语语言资源的发展，为多语言文本分析提供了实用工具，特别适用于处理大规模定性文本数据的领域。
...
TL;DR: 该研究使用RGB基准评估RAG系统在不同场景下的鲁棒性，比较了基于知识图谱的GraphRAG与RGB基线，测试了三种GraphRAG定制化方案并取得了改进。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能增强LLM的能力，但检索到的不一致信息可能对LLM响应产生负面影响，需要评估RAG系统在这种条件下的鲁棒性。

Method: 使用RGB语料库在四种场景下评估LLMs：噪声鲁棒性、信息整合、负面拒绝和反事实鲁棒性。比较RGB RAG基线和基于知识图谱的GraphRAG系统，测试了三种GraphRAG定制化方案。

Result: GraphRAG在RGB基准上取得了改进，为设计更可靠的RAG系统提供了见解。

Conclusion: 基于知识图谱的GraphRAG系统相比传统RAG基线在鲁棒性方面有所改进，为实际应用场景中设计更可靠的RAG系统提供了有价值的见解。
...
TL;DR: 全球调查显示人们对GenAI应如何代表文化缺乏共识，研究通过大规模调查收集不同群体对文化的定义及GenAI文化代表方式的看法，提出参与式方法、超越地理的文化维度优先、文化"红线"敏感框架等建议。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于全球范围内人们对GenAI应如何代表文化的态度的实证证据，需要了解不同群体对文化的理解以及GenAI应如何处理文化物品、概念或价值观的代表问题。

Method: 通过大规模全球调查收集数据，调查范围涵盖欧洲、北美、南美、亚洲和非洲部分地区，直接从这些社区提炼文化的工作定义，理解文化的概念复杂性及其与生成式AI中文化代表的关系。

Result: 研究收集了不同群体对文化的定义和理解，分析了文化概念在生成式AI中的复杂性，建立了文化概念与AI代表之间的关系理解。

Conclusion: 提出了一套关于文化与GenAI发展的建议，包括采用参与式方法、优先考虑超越地理的特定文化维度（如宗教和传统），以及解决文化"红线"的敏感性框架。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构在扩展时伴随参数增长和嵌入维度冗余，需要压缩模型参数同时保持性能。

Method: 引入基于三阶张量L-积的结构化谱分解，将令牌表示重塑为谱张量切片，在变换域执行注意力和前馈操作，得到张量Transformer架构。

Result: 在IMDB和AG News数据集上，模型可减少高达75%的编码器参数（p=4时），同时保持竞争力准确率。在BERT-base宽度下性能恢复至基准水平。

Conclusion: 提出的L-Transformer通过谱分解实现参数压缩，同时引入嵌入频率的归纳偏置，支持切片依赖的频率缩放以改善泛化能力。
...
TL;DR: 本文以瓜拉尼语为例，指出当前AI系统以文本为中心的设计忽视了口语优先语言和土著社区的需求，提出了一种口语优先的多智能体架构替代传统"文本转语音"流程。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能和人机交互系统虽然被宣传为通用解决方案，但其设计仍然以文本为中心，未能充分服务于主要依赖口语的语言和土著社区。作者以巴拉圭官方语言瓜拉尼语为例，指出AI中的语言支持除非与真实的口语实践相一致，否则仍然是不充分的。

Method: 提出了一种口语优先的多智能体架构作为传统"文本转语音"流程的替代方案。该架构将瓜拉尼语的自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建了一个尊重土著数据主权和双语现象的技术框架。

Result: 通过将关注点从单纯的语音识别转向对话轮转、修复和共享上下文等交互核心要素，该框架超越了传统方法。它确保数字生态系统能够赋能而非忽视多样化的语言实践。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适配到文本中心系统的思维转变，将口语对话视为首要设计需求。只有这样，数字生态系统才能真正赋能而非忽视多样化的语言实践。
...
Method: 创建NERdME数据集，包含200个手动标注的README文件，标注了超过10,000个片段和10种实体类型。使用大型语言模型和微调的transformer模型进行基线实验，并进行下游实体链接实验

Result: 基线实验显示论文层面和实现层面实体之间存在明显差异，表明将SIE基准扩展到README文件中可用的实体类型具有价值。实体链接实验证明从README中提取的实体可以支持工件发现和元数据集成

Conclusion: NERdME数据集填补了学术信息提取在实现层面细节的空白，README文件中的实体信息对于工件发现和元数据集成具有实际应用价值，扩展SIE基准到实现层面是必要且有价值的
...
TL;DR: PVminerLLM：基于监督微调的大型语言模型，专门用于从患者生成文本中结构化提取患者声音信号，在多个数据集上显著优于提示方法


<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含关于患者生活经历、社会环境和护理参与的关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而这些患者声音信号很少以结构化形式可用，限制了其在以患者为中心的结果研究和临床质量改进中的应用。

Method: 提出PVminer基准用于患者声音的结构化提取，并开发PVminerLLM——一个针对此任务进行监督微调的大型语言模型。模型在不同规模的数据集和模型大小上进行评估。

Result: PVminerLLM在多个数据集上显著优于基于提示的基线方法，在代码预测上达到83.82% F1，子代码预测80.74% F1，证据跨度提取87.03% F1。即使使用较小的模型也能实现强大性能。

Conclusion: 可靠的患者声音提取无需极端模型规模即可实现，这使得能够大规模分析嵌入在患者生成文本中的社会和体验信号，为患者中心的结果研究和临床质量改进提供支持。
...
TL;DR: 本文开发了一个用于大规模分析辅导对话的辅导行为分类法，通过混合演绎-归纳过程构建，将辅导行为分为四类，支持AI标注和计算建模。
...
Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架，然后通过专家标注者对真实辅导转录本的迭代编码来精炼分类法。

Result: 开发出的分类法将辅导行为组织为四类：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持行为进一步按学生参与度谱系组织，区分引发学生推理的行为与提供直接解释或答案的行为。
...
TL;DR: RouteGoT：一种预算可控的节点自适应路由框架，用于图结构推理，通过智能分配不同规模的模型到推理图的不同节点来平衡准确性和成本


<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确性，但引入了显著的令牌消耗和延迟开销，且性能增益在不同任务分布中不稳定。作者观察到图推理管道中存在阶段和节点的异质性：高质量规划和最终合成需要强大模型，而许多中间子任务可由轻量级模型准确解决。

Method: 提出RouteGoT框架，在图中进行路由，优先为规划和合成任务分配强大模型，同时基于预测的难度动态分配轻量级模型和成本效益策略给叶子子任务。框架集成显式预算约束到全局推理调度器中，在用户指定的令牌预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳问答基准测试中，RouteGoT在匹配或提高准确性的同时显著减少令牌使用；相比AGoT平均提高8.1个百分点准确性并减少79.1%的输出令牌。在变化的预算目标和任务下保持优越的成本-准确性权衡，展现出更好的鲁棒性。

Conclusion: RouteGoT通过节点自适应路由和预算控制机制，有效解决了图结构推理中的效率问题，实现了更优的性能-成本平衡，为实际部署提供了可预测的预算控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生幻觉内容，严重影响其在关键应用领域的可靠性。现有的幻觉归因方法主要关注语义相似度匹配或表示级判别，难以在片段层面建立幻觉类型、错误生成机制和外部事实证据之间的结构化对应关系，限制了幻觉片段的可解释性和证据的可追溯性。

Method: HART将幻觉追踪形式化为包含四个阶段的结构化建模任务：1) 片段定位，2) 机制归因，3) 证据检索，4) 因果追踪。基于此构建了首个专门用于幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集。

Result: 在提出的数据集上的实验结果表明，HART显著优于BM25和DPR等强检索基线，验证了所提出的追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，提供了更细粒度的幻觉分析和证据检索能力，为LLM幻觉问题的可解释性评估提供了新范式。
...
TL;DR: ReflexiCoder是一个基于强化学习的代码生成框架，通过内部化结构化推理轨迹（生成、反思、修正）到模型权重中，实现完全自主的自我反思和修正能力，无需外部反馈或执行引擎。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代码生成方法（"System 1"单次前向生成）在复杂算法任务上遇到性能瓶颈，而现有的迭代优化策略依赖外部反馈、执行引擎或计算昂贵的提示-响应循环，缺乏完全自主的自我修正能力。

Method: 提出ReflexiCoder强化学习框架，使用RL-zero训练范式配合细粒度奖励函数，优化整个反思-修正轨迹，将结构化推理（初始生成、bug和优化意识反思、自我修正）内部化到模型权重中。

Result: ReflexiCoder-8B在7个基准测试中创下开源模型新SOTA：HumanEval 94.51%（Plus 87.20%）、MBPP 81.80%（Plus 78.57%）、BigCodeBench 35.00%、LiveCodeBench 52.21%、CodeForces 37.34%，单次尝试性能媲美GPT-5.1等专有模型，推理计算开销降低约40%。

Conclusion: ReflexiCoder通过将结构化推理轨迹内部化到模型权重中，实现了完全自主的自我反思和修正能力，显著提升了代码生成性能，同时大幅提高了推理效率，为代码生成领域提供了新的范式。
...
Method: 提出ROSE方法：1) 预剪枝识别候选权重；2) 估计列和块剪枝损失；3) 两级重新排序：块内按列损失降序排列，块间按块损失排序；4) 引入块损失相对范围指标识别列状层，实现全模型自适应重新排序。

Result: 在主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验表明，ROSE超越了原始SparseGPT和其他对比剪枝方法。

Conclusion: ROSE通过重新排序剪枝顺序，有效解决了SparseGPT在列模式权重下的性能问题，显著提升了LLM剪枝效果，代码已开源。
...
TL;DR: CoCA提出了一种置信度优先的LLM不确定性估计框架，通过GRPO强化学习联合优化置信度校准和答案准确性，使用分段信用分配避免奖励攻击，在数学、代码和事实QA基准上表现出更好的校准效果。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法主要是答案优先的，即先生成答案再给出置信度，这种方法只能衡量特定回答的正确性，限制了实际应用。需要一种置信度优先的范式，让模型在回答前输出置信度，作为其当前策略下正确回答问题的概率。

Method: 提出CoCA框架，采用GRPO强化学习方法联合优化置信度校准和答案准确性。通过分段信用分配，分别为置信度和答案段分配独立的奖励和组相对优势，实现稳定联合优化并避免奖励攻击。

Result: 在数学、代码和事实QA基准测试中，CoCA显示出改进的校准效果和不确定性区分能力，同时保持了答案质量，为下游应用提供了更可靠的不确定性估计。

Conclusion: 置信度优先范式结合联合优化方法能够有效提升LLM的不确定性估计质量，CoCA框架通过分段信用分配实现了稳定优化，为LLM的可靠部署提供了更好的不确定性校准能力。
...
TL;DR: 当前主流BPE分词方法对黏着语不友好，会破坏词素边界并增加token数量


<details>
  <summary>Details</summary>
Motivation: BPE及其变体作为主流分词方法本质上是脚本无关的，主要针对英语等屈折语优化，但对黏着语（如达罗毗荼语系、突厥语系、乌拉尔语系等）处理不佳，这些语言中单个词可能编码词根、时态、人称、数、格等多种语法信息

Method: 论文未在摘要中明确说明具体方法，但指出了BPE分词在黏着语中的问题：统计分词器会将单词分割成字节对块，破坏词素边界并增加token数量

Result: 摘要中未呈现具体实验结果，但指出了当前分词方法对黏着语处理存在根本性缺陷
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇故事时经常出现前后不一致的问题，包括事实、角色特征和世界规则的矛盾。现有基准主要关注情节质量和流畅性，缺乏对一致性的系统评估。

Method: 创建了ConStory-Bench基准测试，包含2000个提示和四种任务场景，定义了5类19种子类型的错误分类。开发了ConStory-Checker自动化管道，用于检测矛盾并提供文本证据支持。

Result: 评估多个LLM后发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；发生在token级熵较高的文本段中；某些错误类型倾向于同时出现。

Conclusion: 该研究为改善长文本叙事生成的一致性提供了重要基准和分析工具，揭示了LLM在长文本生成中的一致性错误模式，为未来改进提供了方向。
...
TL;DR: 本文提出了一种基于LLM的集成系统方法，用于联合国安理会决议的语义标注，通过引入CPR和TWF两个评估指标来避免幻觉和内容偏差，GPT-4.1在两项任务中表现最佳，而GPT-4.1-mini以20%的成本达到相近性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能的变异性构建集成系统，完成数据清洗和语义标注任务，同时避免LLM常见的幻觉问题和内容偏差，确保输出质量。

Method: 引入两个评估指标：内容保持率（CPR）和标签格式正确性（TWF），通过多个GPT模型的多轮运行，利用集成系统选择最佳输出，构建可靠的语义标注系统。

Result: GPT-4.1在两项任务中表现最佳（清洗：CPR 84.9%；语义标注：CPR 99.99%，TWF 99.92%）。GPT-4.1-mini以20%的成本达到相近性能。集成系统能够为所有LLM模型选择最优输出。

Conclusion: 通过集成设计和评估指标，成功构建了可靠的LLM系统，能够在具有挑战性的文本上执行语义标注任务，同时平衡性能与成本效益。
...
TL;DR: 论文提出了一种预测用户下一步动作的方法（Next Action Prediction, NAP），通过多模态交互数据（截图、点击、传感器数据）预测用户行为，并开发了LongNAP模型结合参数化和上下文学习来处理长交互历史。


<details>
  <summary>Details</summary>
Motivation: 真正主动的AI系统需要预测用户下一步行为，这需要比稀疏提示更丰富的信息——需要基于用户所见所做的完整上下文进行推理。当前缺乏处理这种多模态、长序列用户交互数据的方法。

Method: 1）使用视觉语言模型标注纵向、自然的计算机使用数据，发布了在私有基础设施上进行标注的开源管道；2）提出LongNAP模型，结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹，从历史轨迹库中检索相关轨迹，并在上下文中应用这些轨迹来预测未来动作。

Result: 标注了20名用户一个月连续手机使用的360K个动作（1800小时屏幕时间）。LongNAP在留出数据上显著优于监督微调和提示基线（分别提升79%和39%），在跨用户训练时能泛化到未见用户。尽管用户可能动作空间无限，但17.1%的预测轨迹与用户实际行为良好对齐（LLM-judge评分≥0.5），高置信度预测时达到26%。

Conclusion: 基于用户行为的完整上下文来预测用户需求现在是一个可行且有重大机会的任务，LongNAP展示了通过结合参数化和上下文学习来预测用户下一步动作的有效性。
...
TL;DR: 本文研究通过建模作者的语言上下文（HuLM任务）来解决生态谬误问题，在8B Llama模型上验证了作者语境建模对大规模语言模型的性能提升效果。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型训练和推理忽略了一个基本语言事实：同一作者写的多个文本序列之间存在依赖关系。先前研究表明解决这种生态谬误可以显著提升较小GPT模型的性能，本文旨在探索这种方法是否对更大规模（8B参数）的Llama模型同样有效。

Method: 提出HuLM（Human-aware Language Modeling）目标，通过处理作者按时间顺序排列的其他文本来建模作者语言上下文。研究两种变体：1）使用HuLM目标进行预训练；2）在微调时使用作者上下文（HuFT：Human-aware Fine-Tuning）。使用QLoRA技术进行高效微调。

Result: 实验表明：1）仅通过QLoRA进行HuFT微调就能提升8B模型性能；2）基于QLoRA的HuLM持续预训练产生的人类感知模型在八个下游任务上仅通过线性分类器训练就能获得改进性能。

Conclusion: 建模语言在其原始生成者（作者）的语境中具有实用性和重要性，解决生态谬误问题对大规模语言模型同样有效，能够提升模型在下游任务上的表现。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦风格为三个可解释维度（词汇特征、句法模式、语用风格），结合隐式风格条件策略，使小语言模型能够生成高保真风格化内容，在动漫角色领域超越更大基线模型。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在角色扮演中难以生成高度风格化的人物，主要面临数据稀缺和风格解耦复杂性挑战。标准的监督微调通常只能捕捉表层语义，无法复现角色的复杂句法和语用细微差别，导致"角色出戏"的生成问题。

Method: 提出结构化风格重写框架：1）将风格显式解耦为三个可解释维度：词汇特征（通过PMI）、句法模式（基于PCFG规则）、语用风格；2）引入隐式风格条件策略，通过思维链蒸馏利用训练中的显式推理轨迹作为强归纳偏置，使模型的潜在表示与结构化风格特征对齐，在推理时无需显式推理标记。

Result: 在动漫角色这一高风格化领域的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度方面显著优于更大的基线模型（如4B标准监督微调）。

Conclusion: 该方法为在消费级硬件上实现推理和部署提供了数据高效的范式，能够民主化高度风格化内容的生成能力，使小模型能够生成与角色一致的风格化内容。
...
TL;DR: 该研究开发了可解释模型来预测心理健康，结合个体心理特质和情境特征，在社交媒体数据中识别适应性和适应不良的自我状态。


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受个体倾向和情境背景相互作用影响。研究旨在整合计算建模与心理学理论，以情境敏感且人类可理解的方式评估动态心理状态。

Method: 基于互动主义和建构主义心理学理论，整合个体层面心理特质（如韧性、认知扭曲、内隐动机）与基于情境8DIAMONDS框架的语言推断情境特征，并与心理测量学启发的语言模型嵌入进行比较。

Result: 理论驱动的特征在保持竞争力的同时提供更好的可解释性。定性分析进一步显示预测幸福感最有效的特征具有心理一致性。
...
TL;DR: MASFactory是一个基于图的框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑、可执行的工作流，提供可重用组件和可视化工具。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统框架在实现复杂图工作流时需要大量手动工作，重用性有限，且难以集成异构外部上下文源，需要更高效的编排解决方案。

Method: 提出MASFactory框架，核心是Vibe Graphing方法，将自然语言意图编译为可编辑的工作流规范，再转换为可执行图；提供可重用组件、可插拔上下文集成和可视化工具。

Result: 在七个公共基准测试上评估MASFactory，验证了代表性MAS方法的再现一致性，以及Vibe Graphing方法的有效性。

Conclusion: MASFactory通过图中心的方法简化了LLM多智能体系统的编排，提高了工作流构建的效率和可重用性，支持可视化交互和上下文集成。
...
TL;DR: ViewFusion是一个两阶段框架，通过显式分离跨视图空间预对齐和问题回答来解决多视图空间推理问题，在MMSI-Bench上相比Qwen3-VL-4B-Instruct提升了5.3%的准确率。
...
Conclusion: 通过显式分离跨视图空间预对齐和问题回答，ViewFusion能够更好地处理多视图空间推理任务，特别是在需要跨视图对齐的情况下表现显著提升。
...
TL;DR: 研究评估了四种开源大语言模型在奥地利A级德语作文自动评分中的应用，发现模型与人工评分的一致性较低，尚不适用于实际评分环境。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分系统旨在减轻教师评分负担并减少主观偏见。随着大语言模型的发展，本研究探索将最先进的开源模型应用于奥地利A级德语文本评分，特别关注基于评分标准的评估。

Method: 使用101份匿名学生考试文本（涵盖三种文本类型），评估了四种开源大语言模型：DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b。采用不同的上下文和提示策略，基于评分标准进行维度评估。

Result: 模型在评分标准提供的子维度上与人工评分者的最大一致性仅为40.6%，最终成绩与人类专家评分匹配的比例仅为32.8%。结果表明，虽然较小模型能够使用标准化评分标准进行德语作文评分，但准确性不足以在实际评分环境中使用。

Conclusion: 当前的开源大语言模型在德语作文自动评分方面表现有限，与人工评分的一致性较低，尚不具备在实际教育环境中可靠应用的准确性。
...
TL;DR: 该研究通过持续预训练让大语言模型接触领域特定文本，模拟经验积累，发现模型能力呈现双峰分布，并揭示了"抑制优势"现象，为"人格工程"提供了路线图。


<details>
  <summary>Details</summary>
Motivation: 人类问题解决受益于多样化的风格和人格特质，但大语言模型的发展主要关注统一的性能基准，偏向特定的行为倾向如自信性。研究旨在探索不同经验如何塑造机器人格并影响问题解决能力。

Method: 采用持续预训练方法，以无监督方式让模型接触领域特定文本，模拟经验积累过程。通过机器人格量表（MPI）调整大五人格框架，量化模型变体的人格特质，并分析其与语言风格和推理行为的关系。

Result: 研究发现模型能力呈现双峰分布：在"表达型通才"和"抑制型专才"处达到峰值。识别出"抑制优势"现象，即减少社交特质能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率、词汇多样性）与模型人格之间的因果关系。

Conclusion: 该研究为"人格工程"提供了路线图，表明通过精心设计训练数据的语言学特征可以塑造模型人格特质，从而优化特定任务的问题解决能力，推动更个性化、多样化的人工智能系统发展。
...
TL;DR: 该论文提出了一种处理省略三段论（enthymemes）的集成管道，结合了大型语言模型生成隐含前提、自然语言到逻辑公式的翻译，以及基于SAT求解器的神经符号推理器来确定逻辑蕴涵关系。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提和/或主张是隐含的）。现有的NLP方法只能识别省略三段论但不能解码其底层逻辑，而基于逻辑的方法则假设存在足够的知识库公式来进行溯因解码。目前缺乏将省略三段论文本组件转化为逻辑论证并生成解码所需逻辑公式的系统方法。

Method: 提出一个集成管道：1）使用大型语言模型基于显式前提和主张生成中间隐含前提；2）使用另一个LLM将自然语言翻译为逻辑公式；3）使用基于SAT求解器的神经符号推理器来确定逻辑蕴涵关系。

Result: 在两个省略三段论数据集上评估该管道，在精确率、召回率、F1分数和准确率等指标上显示出选择正确隐含前提的优异性能。

Conclusion: 该集成管道为处理省略三段论提供了一种系统方法，成功结合了LLM的自然语言处理能力和符号推理的逻辑严谨性，在解码隐含逻辑关系方面表现出良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本分析方法依赖预定义本体标注数据，存在人类偏见，且采用非结构化线性模式识别而非建模实体间的结构化交互

Method: 提出基于图的框架，将叙事表示为实体交互图，在节点层面引入因果估计，将句子图提炼为最小因果子图，构建分类流程

Result: 提出的分类流程在对立思维分类任务中优于现有方法

Conclusion: 图表示结合因果估计能更好地建模叙事中的结构化实体交互，提升对立叙事检测和分类性能
...
TL;DR: CRIMSON是一个临床基础的胸片报告生成评估框架，通过诊断正确性、上下文相关性和患者安全性三个维度评估报告质量，并引入临床显著性分级和错误分类系统。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标未能充分考虑完整的临床上下文（如患者年龄、适应症、指南决策规则），且常让正常或临床不显著的发现对总分产生不成比例的影响，缺乏对临床重要错误的优先级区分。

Method: CRIMSON框架包含：1）纳入完整临床上下文；2）将错误分类为假阳性发现、遗漏发现和八种属性级错误；3）基于与心胸放射科医生共同制定的指南，为每个发现分配临床显著性等级（紧急、可处理非紧急、不可处理、预期/良性）；4）采用严重性感知加权，优先考虑临床后果严重的错误。

Result: 在ReXVal验证中，CRIMSON与六位认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）。在RadJudge和RadPref两个新基准测试中，CRIMSON与专家判断一致，并在放射科医生偏好评估中表现最佳。

Conclusion: CRIMSON提供了一个临床基础、全面且可重复的胸片报告生成评估框架，通过纳入临床上下文、错误分类系统和严重性感知加权，能更准确地反映报告的实际临床价值，优于现有评估指标。
...
TL;DR: MAPO：一种基于密集过程反馈和混合优势估计的无评论家强化学习算法，用于主观多轮对话任务，通过蒙特卡洛回报传播长时程效应，在多个情感对话基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务（如情感支持）需要适应动态用户状态并优化长期交互质量的对话策略，但传统强化学习面临挑战：缺乏可靠的过程监督，仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而简单的轮级分组采样在交互环境中成本过高。

Method: 提出MAPO算法：1）利用法官模型提供密集过程反馈；2）通过蒙特卡洛回报传播长时程效应；3）引入混合优势估计器，结合轮级归一化和批次级归一化，实现细粒度且可扩展的信用分配。

Result: 在多个主观对话基准（EMPA、EmoBench、EQ-Bench）和模型规模（7B到32B）上，MAPO相比仅基于结果的GRPO和单级归一化基线，显著提升训练稳定性和最终性能。在EMPA上提升率高达9个百分点，对话分数比7B基础模型提高+43.2。尽管仅在EMPA风格环境中训练，方法在未见的情感智能基准上泛化良好，EmoBench提升高达+4分，EQ-Bench提升+3.5分。
...
<details>
  <summary>Details</summary>
Motivation: 大规模内容分析面临缺乏可观察的真实标注或黄金标准标签的挑战，因为通过人工编码创建大规模数据集的基准标注在时间、成本和一致性方面都不可行。

Method: AI-CROWD协议通过集成多个大语言模型的输出来近似真实标注，使用多数投票聚合输出，并通过诊断指标分析模型间的一致性和分歧模式，识别高置信度分类并标记潜在歧义或模型特定偏见。

Result: 该协议能够生成共识基础的近似标注，识别高置信度分类，同时标记出潜在的模糊区域和模型特定偏见。

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过集成多个LLM的输出来近似真实标注，克服了传统人工标注在大规模数据集上的局限性。
...
TL;DR: LIT-RAGBench是一个新的RAG生成器基准测试，包含五个类别：整合、推理、逻辑、表格和弃权，用于评估LLM在多种实际能力下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖有限，无法在统一条件下同时评估多种实际能力（如长上下文整合、多步推理、表格解释、证据缺失时弃权等），需要一个新的综合基准测试来弥合评估与实践之间的差距。

Method: 引入LIT-RAGBench基准测试，定义五个评估类别（整合、推理、逻辑、表格、弃权），每个类别进一步细分为实际评估方面。使用虚构实体和场景构建问题，确保答案基于外部文档。数据集包含114个人工构建的日语问题和机器翻译加人工整理的英文版本，采用LLM-as-a-Judge进行评分。

Result: 在API模型和开源模型测试中，没有模型超过90%的整体准确率。基准测试能够测量每个类别中的优势和弱点，为实际RAG部署中的模型选择和构建RAG专用模型提供有价值的指标。

Conclusion: LIT-RAGBench填补了现有RAG评估的空白，通过系统覆盖多个能力类别和组合模式，成为实际RAG应用中有价值的评估工具，已开源数据集和评估代码。
...
TL;DR: FlashPrefill是一个通过即时模式发现和阈值化实现超快速预填充的框架，解决了长上下文建模中注意力机制二次复杂度的瓶颈问题。
...
Conclusion: FlashPrefill通过创新的模式发现和阈值化技术，显著提升了长上下文建模的预填充效率，解决了现有稀疏注意力方法的局限性，具有广泛的实用价值。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停令牌，减少推理成本37.5%，同时提升准确率2.3%，并保持潜在状态的语义可解释性。


<details>
  <summary>Details</summary>
Motivation: 显式思维链推理虽然能提升大语言模型的推理性能，但会产生冗长的令牌级追踪，导致高昂的推理成本。现有方法主要通过截断模型输出而非内部化模型思考，而潜在推理方法又面临两个关键挑战：1) 刚性点对点对齐无法捕捉整个推理段的密集、变长语义；2) 缺乏可解释性，潜在状态难以解码或审计。

Method: 提出SPOT框架，包含两个核心创新：1) 跨度级语义对齐：使用Sinkhorn最优传输目标，将每个暂停令牌软匹配到整个推理段的语义，克服步骤末端对齐的刚性；2) 冻结头部解码约束：保持潜在状态在冻结的预训练语言模型头部下可直接解码为令牌分布，实现潜在思考的可读关键词解释。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成令牌37.5%，并能提供潜在推理过程的忠实语义解释。

Conclusion: SPOT框架成功解决了潜在推理中的语义对齐和可解释性挑战，在减少推理成本的同时提升性能，并为潜在推理过程提供了有意义的语义解释。
...
TL;DR: 大型语言模型在多语言、多文化环境中部署时，其英语中心训练数据与多元文化价值观存在错位，特别是在宗教领域对少数群体观点存在偏见和刻板印象。
...
Method: 对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代LLMs进行多语言文化对齐审计，聚焦印度、东亚和东南亚地区，以宗教为切入点，通过log-probs/logits分析模型内部表征，并与真实公众态度进行比较。

Result: 主流模型在广泛社会议题上能与公众意见基本对齐，但在宗教观点上持续失败，特别是对少数群体的观点，经常放大负面刻板印象。人口统计提示和母语提示等轻量干预措施只能部分缓解但无法消除文化差距。下游偏见基准测试显示在敏感情境中存在持续危害和代表性不足问题。

Conclusion: 研究强调需要系统性的、基于地区的审计，以确保LLMs在全球部署中的公平性，特别是在文化敏感领域需要更深入的本土化调整。
...
TL;DR: 研究探索ChatGPT模仿艺术家原创作品生成图像的能力，通过12位艺术家评估发现AI模仿作品在色彩纹理相似度与构图概念感知相似度之间存在显著差距，建议使用"风格转换仪表盘"多指标评估而非单一风格指标。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉创意领域的潜力，特别是ChatGPT是否有意模仿原创艺术作品（绘画、素描、雕塑、装置）的能力，并评估AI生成作品与原始作品之间的相似性和艺术价值。

Method: 邀请12位来自罗马尼亚、保加利亚、法国、奥地利和英国的艺术家，每人提供三件原创作品；使用ChatGPT生成模仿这些作品的图像；结合人类评估（艺术家评分和评论）与计算方法检测视觉和风格相似性或差异。

Result: 发现基于色彩和纹理的相似性与构图、概念和感知相似性之间存在显著差距；艺术家认为ChatGPT模仿当代艺术作品存在局限性，缺乏维度感、背景和意图感，更像是释义或近似引用而非能引发情感的有价值艺术品。

Conclusion: 建议使用"风格转换仪表盘"的互补指标来评估模仿作品与原始作品之间的相似性，而非依赖单一风格指标；AI在艺术模仿方面仍有局限性，特别是在传达作品的多维度、背景和意图方面。
...
TL;DR: 该研究将数学问题理解建模为数学实体关系抽取任务，使用基于Transformer的模型自动提取数学文本中的实体关系，BERT模型达到99.39%的准确率，并通过SHAP方法增强模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为存在专门的实体和复杂的相互关系。需要开发能够自动理解数学问题中实体关系的系统，以支持自动化问题解决、知识图谱构建和智能教育系统。

Method: 将数学问题解释建模为数学实体关系抽取任务，将操作数视为实体，运算符视为关系。应用基于Transformer的模型（包括BERT）自动提取这些关系，并使用SHAP方法进行可解释性分析。

Result: BERT模型在数学实体关系抽取任务中表现最佳，达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的见解。

Conclusion: 通过结合基于Transformer的学习、特定任务数据集和可解释建模，这项工作为数学实体关系抽取提供了一个有效且可解释的框架，支持自动化问题解决、知识图谱构建和智能教育系统等未来应用。
...
TL;DR: 大型语言模型在规范性规则下的推理表现优于描述性规则，其错误模式与人类相似，表现出匹配偏差而非确认偏差。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型语言能力的发展，其推理能力受到关注。人类推理在特定领域（尤其是规范性而非纯形式语境）表现良好，但LLM推理的领域特异性尚未充分探索。本研究旨在系统比较LLM在规范性规则下的条件推理表现。
...
Result: 结果显示，与人类相似，LLM在规范性规则下推理表现更好，并显示出类似匹配偏差的错误模式。LLM的表现随规则类型系统性变化，其错误模式与该范式中已知的人类偏见相似。

Conclusion: LLM的推理表现随规则类型系统性变化，其错误模式与人类认知偏见相似，特别是在规范性规则下表现更好且表现出匹配偏差。这表明LLM的推理机制可能与人类认知过程有相似之处。
...
TL;DR: 该研究对基于大语言模型的英语二语写作自动评分方法进行了首次统一比较，在雅思写作任务2上评估了四种主要范式，发现最佳配置（k-SFT+RAG）达到93%的F1分数，揭示了准确率-成本-鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型已经重塑了自动作文评分领域，但以往研究通常孤立地考察各种技术，限制了对其在英语作为第二语言写作评分中相对优势的理解。本研究旨在填补这一空白，提供对主要LLM-based AES范式的全面比较。

Method: 在统一的雅思写作任务2基准上评估了四种方法：(1) 基于编码器的分类微调；(2) 零样本和少样本提示；(3) 指令调优和检索增强生成；(4) 监督微调结合直接偏好优化和检索增强生成。通过系统比较这些方法的性能表现。

Result: 研究结果显示不同方法在准确率、成本和鲁棒性之间存在明确的权衡关系。最佳配置（集成k-SFT和RAG）取得了最强的整体结果，F1分数达到93%。这为基于大语言模型的英语二语写作自动评分提供了实证依据。

Conclusion: 本研究首次对现代基于大语言模型的英语二语自动作文评分策略进行了统一的实证比较，展示了在自动评分写作任务中的潜力，为后续研究和实践提供了重要参考。
...
TL;DR: 该研究将三段论数据集转换为溯因推理任务，评估大语言模型在溯因推理中的准确性，探索其是否存在与人类相似的认知偏差，并强调情境化推理的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注大语言模型与人类在形式演绎推理中的相似偏差，但批评这种比较可能不公平，因为人类推理不仅包括形式演绎，还涉及基于有限信息的溯因推理。需要探索LLMs在溯因推理中的表现，以更全面地评估其推理能力。

Method: 将三段论数据集转换为适合溯因推理的形式，用于评估最先进的大语言模型在溯因推理任务中的准确性。通过这种转换，研究LLMs是否在溯因推理中表现出偏差。

Result: 论文旨在调查最先进的大语言模型在溯因推理中是否表现出偏差，并识别潜在的改进领域。研究结果将揭示LLMs在超越形式演绎的情境化推理方面的能力。

Conclusion: 这项研究对于推进对LLMs在复杂推理任务中理解和应用至关重要，为弥合机器与人类认知之间的差距提供见解，强调超越形式演绎的情境化推理的重要性。
...
TL;DR: PONTE是一个个性化可解释AI框架，通过闭环验证和适应过程生成可信的自然语言解释，解决了传统XAI方法忽视用户差异以及LLM生成解释存在幻觉的问题。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法大多采用一刀切模式，忽视了用户在专业知识、目标和认知需求方面的差异。虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性和幻觉问题。

Method: PONTE是一个人在环框架，将个性化建模为闭环验证和适应过程。包含三个核心组件：低维偏好模型捕捉风格要求；基于结构化XAI工件的偏好条件生成器；强制执行数值忠实性、信息完整性和风格对齐的验证模块。用户反馈迭代更新偏好状态，支持快速个性化。

Result: 在医疗和金融领域的自动和人工评估表明，验证-精炼循环显著提高了完整性和风格对齐。人工研究进一步证实了预期偏好向量与感知风格之间的强一致性、对生成随机性的鲁棒性，以及一致积极的质量评估。

Conclusion: PONTE通过将个性化建模为闭环验证和适应过程，而非提示工程，成功解决了XAI中的个性化挑战，提高了自然语言解释的可信度和适应性。
...
TL;DR: 提出了一种支持多语言和方言的上下文感知ASR框架，通过轻量级投影模块连接冻结的语音编码器和仅解码器语言模型，使用对比学习对齐语音和上下文表示，在11种语言和5种英语方言上实现超过5%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语言设置和短孤立话语，虽然已有上下文感知ASR研究，但仍存在两个关键挑战：多语言支持有限，以及缺乏语音和上下文表示之间的原则性对齐。

Method: 结合冻结的语音编码器和仅解码器语言模型，通过轻量级投影模块连接，支持结构化上下文提示（包括对话历史和偏置词）来指导转录。采用对比学习目标，在共享嵌入空间中对齐语音和上下文表示。

Result: 在超过1500小时的真实世界对话语音数据上评估，涵盖11种语言和5种英语方言。上下文输入持续改善识别质量，对比对齐在不同上下文类型上提供额外增益，整体性能提升超过5%。

Conclusion: 上下文建模和跨模态对齐在多语言ASR中都具有重要性，提出的框架在保持预训练模型模块化的同时，有效支持多语言和方言的上下文感知语音识别。
...
<details>
  <summary>Details</summary>
Motivation: 研究政治话语中的歧义和回避技术分类，这是自然语言处理在政治话语分析中的重要应用，有助于理解政治沟通中的策略性语言使用。

Method: 采用两种建模方法：(1) 直接预测清晰度标签；(2) 先预测回避标签，再通过任务分类层次推导清晰度。同时探索了多种辅助训练变体，并在零样本设置下评估解码器模型。
...
TL;DR: RoboLayout扩展了LayoutVLM，通过集成显式的可达性约束和局部细化阶段，生成了可供具身智能体交互的室内场景布局。
...
Method: 1. 将显式可达性约束集成到可微分布局优化过程中；2. 引入智能体抽象概念，可表示不同物理能力的实体；3. 提出局部细化阶段，选择性重新优化问题对象放置。

Result: RoboLayout保持了LayoutVLM的强语义对齐和物理合理性，同时增强了面向智能体的室内场景生成适用性，在不同场景配置中表现出良好效果。

Conclusion: RoboLayout通过智能体感知推理和优化稳定性改进，成功生成了既语义连贯又可供具身智能体交互的室内场景布局，扩展了视觉语言模型在具身智能应用中的适用性。
...
TL;DR: 该研究评估了大型语言模型的思维链可控性，发现模型在控制思维链内容方面能力有限，远低于控制最终输出的能力，这对思维链监控的可靠性提供了谨慎乐观的证据。


<details>
  <summary>Details</summary>
Motivation: 思维链监控是检测推理模型不当行为的重要工具，但如果模型能够控制其思维链中的内容表达，可能会破坏监控的有效性。因此需要评估模型的思维链可控性这一潜在风险。

Method: 研究者开发了CoT-Control评估套件，包含要求模型在遵循特定思维链指令下解决问题的任务（例如回答遗传学问题时不使用"染色体"一词）。通过多种设置测试模型的可控性，包括激励逃避监控、告知被监控、对抗性提示优化等场景。

Result: 推理模型的思维链可控性显著低于输出可控性（Claude Sonnet 4.5仅2.7% vs 61.9%）。可控性随模型规模增大而提高，但随强化学习训练、测试时计算量增加和问题难度提高而降低。即使在激励逃避监控的情况下，模型仍难以控制思维链内容，对抗性提示优化也无法显著提高可控性。

Conclusion: 目前思维链可控性不太可能成为思维链监控的失效模式，这为监控可靠性提供了谨慎乐观的证据。但由于低可控性的机制尚不明确，建议前沿实验室在未来模型中持续跟踪思维链可控性指标。
...
TL;DR: MACRO：一种自演化的医学智能体，通过从已验证的执行轨迹中自主发现有效的多步骤工具序列，将其合成为可复用的复合工具，实现从静态工具组合到经验驱动工具发现的转变，提升多步骤编排准确性和跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 临床图像解读本质上是多步骤和工具中心的，现有LLM智能体系统将工具集和调用策略视为静态部署后固定不变，这种设计在现实世界领域漂移、跨任务和不断演化的诊断需求下非常脆弱，预定义的工具链经常退化并需要昂贵的手动重新设计。

Method: MACRO从已验证的执行轨迹中自主识别重复出现的有效多步骤工具序列，将其合成为可复用的复合工具，并注册为新的高级原语。采用轻量级图像特征记忆将工具选择锚定在视觉-临床上下文中，通过GRPO-like训练循环强化可靠调用发现的复合工具，实现闭环自改进。

Result: 在多种医学影像数据集和任务上的广泛实验表明，自主复合工具发现持续改进多步骤编排准确性和跨域泛化能力，优于强基线方法和最新的智能体方法，弥合了脆弱静态工具使用与自适应、上下文感知临床AI辅助之间的差距。

Conclusion: MACRO通过从静态工具组合转向经验驱动工具发现，实现了自演化的医学智能体，能够自主扩展其行为库，在临床AI辅助中实现更好的适应性和上下文感知能力。
...
TL;DR: ProEvolve是一个基于图的框架，用于可编程地演化LLM智能体环境，通过图变换实现环境能力的添加、删除和修改，以更好地评估智能体对现实世界动态变化的适应性。
...
Method: 提出ProEvolve框架，使用类型化关系图统一表示环境的数据、工具和模式。通过图变换来表达环境能力的添加、删除和修改，这些变换能够跨工具、模式和数据结构一致地传播更新。框架可以编程演化动态作为图变换来自动生成环境，并通过子图采样和编程实例化任务沙箱。

Result: 将单个环境演化为200个环境和3000个任务沙箱，并相应地对代表性智能体进行基准测试，验证了ProEvolve的有效性。

Conclusion: ProEvolve提供了一个可编程的框架，能够以可扩展和可控的方式演化智能体环境，从而更好地评估智能体对现实世界动态变化的适应性，解决了现有基准测试的局限性。
...
TL;DR: 本研究提出了一种基于大语言模型的多智能体系统，用于自动化产品概念评估，通过模拟专业团队协作来克服传统专家评估的主观偏见和高成本问题。


<details>
  <summary>Details</summary>
Motivation: 产品概念评估是企业战略资源分配和项目成功的关键阶段，但传统专家主导的方法存在主观偏见、时间成本高等局限性。需要一种更客观、高效的评估方法来支持产品开发决策。
...
Result: 通过专业显示器概念的案例研究，系统评估排名与行业资深专家的评估结果一致，验证了该多智能体评估方法的可用性。

Conclusion: 基于大语言模型的多智能体系统能够有效支持产品开发决策，提供客观、一致的产品概念评估，克服传统专家评估方法的局限性。
...
TL;DR: 本文提出了一种新的渐进语义家族——聚合语义，用于处理具有内在权重的定量双极论证框架，通过将攻击者和支持者分别聚合的三阶段计算，提高了参数化和可解释性。


<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有的渐进语义通常对称处理攻击者和支持者，但在实际应用中，攻击者和支持者往往扮演不对称的角色。需要一种更灵活、更可解释的语义来处理这种不对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。讨论了三种聚合函数在不同情境下应满足的性质及其与经典渐进语义原则的关系。

Result: 通过简单示例和最终示例测试比较了500种聚合语义，展示了聚合语义可能的行为范围。三阶段分解计算使语义更具参数化，保持了双极性，并提高了渐进语义的可理解性。

Conclusion: 聚合语义为定量双极论证框架提供了一种新颖的渐进语义方法，通过分离处理攻击者和支持者，实现了更好的参数化和可解释性，能够适应攻击者和支持者不对称的实际应用场景。
...
TL;DR: 提出CliqueFlowmer模型，将基于团的离线模型优化方法融入Transformer和流生成中，用于计算材料发现中的目标属性优化，性能优于现有生成方法


<details>
  <summary>Details</summary>
Motivation: 当前流行的生成建模方法由于最大似然训练的限制，难以有效探索材料空间中有吸引力的区域，需要一种能够直接优化目标材料属性的替代方法

Method: 提出CliqueFlowmer模型，将基于团的离线模型优化方法融入Transformer和流生成框架中，实现目标材料属性的直接优化

Result: CliqueFlowmer在材料优化能力上表现优异，生成的材料性能显著优于生成基线方法

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的离线模型优化方法，优于传统生成建模，已开源代码支持跨学科研究
...
<details>
  <summary>Details</summary>
Motivation: 现有住宅需求响应协调机制要么完全自动化，要么仅限于单向调度信号和价格警报，缺乏让产消者进行知情决策的可能性，难以维持产消者的持续参与。

Method: 开发双层多智能体架构：聚合商智能体调度灵活性请求，产消者家庭能源管理系统通过调用基于优化的工具评估可交付性和成本效益。支持产消者发起的上游通信，允许偏好变化直接传达给聚合商。

Result: 概念验证评估显示交互在12秒内完成。架构展示了智能体AI如何弥合聚合商-产消者协调差距，在保持自动化需求响应可扩展性的同时，提供维持产消者参与所需的透明度、可解释性和用户自主权。

Conclusion: 对话式需求响应通过双向自然语言交互，使用智能体AI桥接聚合商和产消者之间的协调差距，为可持续的需求响应参与提供了新途径。所有系统组件已开源发布。
...
TL;DR: 本文提出了一种基于强化学习的决策支持框架，用于城市洪水适应规划，通过集成降雨预测、洪水建模和交通模拟，在长期气候不确定性下优化基础设施投资策略。
...
Method: 提出了一个基于强化学习(RL)的决策支持框架，将其构建为集成评估模型(IAM)，结合降雨预测和洪水建模、交通模拟，并量化对基础设施和交通的直接和间接影响。RL方法学习平衡投资维护成本与避免影响的适应策略。

Result: 通过对哥本哈根市中心2024-2100年的案例研究，测试多种适应方案和不同气候情景。结果显示该框架优于传统优化方法，能够发现协调的空间和时间适应路径，学习影响减少与适应投资之间的权衡，产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有巨大潜力。该框架能够发现协调的时空适应路径，平衡投资成本与影响避免，为城市洪水适应提供有效解决方案。
...
Conclusion: SAHOO使递归自我改进期间的对齐保持变得可测量、可部署且可系统验证，绘制了能力-对齐前沿，显示早期改进高效但后期对齐成本上升，揭示了领域特定的权衡。
...
TL;DR: 该论文提出了一种解决LLM科学工作流中确定性与灵活性矛盾的方法：模式门控编排，通过模式作为强制执行的边界，在验证完整操作后才允许执行，从而分离对话与执行权限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将研究人员的自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM决定执行内容时难以保证这些特性。研究发现工业研发中存在两个相互竞争的需求：确定性约束执行与对话灵活性，需要找到满足人类参与控制和透明度边界属性的解决方案。

Method: 提出模式门控编排作为解决原则：模式成为组合工作流级别的强制执行边界，确保只有在完整操作（包括跨步骤依赖）通过机器可检查规范验证后才能执行。将两个需求操作化为执行确定性（ED）和对话灵活性（CF），使用这些维度评估20个系统，采用多模型协议（3个LLM家族的15个独立会话）进行评分。

Result: 多模型LLM评分显示出高度一致性（ED的Krippendorff α=0.80，CF的α=0.98），表明多模型LLM评分可作为人类专家小组的可重用替代方案。评估发现经验帕累托前沿——没有系统同时实现高灵活性和高确定性，但在生成式和以工作流为中心的两个极端之间出现了收敛区域。

Conclusion: 模式门控架构通过分离对话与执行权限，能够解耦确定性与灵活性的权衡。提炼出三个操作原则：执行前澄清、约束的计划-行动编排、工具到工作流级别的门控，以指导实际应用。
...
### [180] [Beamforming Optimization for Extremely Large-Scale RIS-Aided Near-Field Secure Communications](https://arxiv.org/abs/2603.05922)
*Xiaotong Xu,Qian Zhang,Yunxiao Li,Xuejun Cheng,Meihui Liu,Ju Liu*

Main category: cs.IT

TL;DR: 本文研究XL-RIS辅助的近场物理层安全通信系统，通过联合优化基站预编码向量和XL-RIS反射系数矩阵来最大化保密速率，引入人工干扰增强安全性，提出交替优化算法解决非凸问题。


<details>
  <summary>Details</summary>
Motivation: 随着无线通信系统向更高频段发展，通信环境变得更加复杂，安全威胁日益严峻。传统远场假设在近场场景下不再适用，需要研究XL-RIS辅助的近场物理层安全通信技术，特别是在窃听者与合法用户同方向且更靠近RIS的极端情况下确保通信安全。

Method: 采用交替优化算法将非凸保密速率最大化问题分解为两个子问题：1）基站发射波束成形优化，通过加权最小均方误差和逐次凸逼近算法转化为凸问题；2）XL-RIS相移矩阵优化，采用低复杂度交替方向乘子法算法，支持离散相位优化以适应实际系统需求。

Result: 仿真结果表明，即使在窃听者与合法用户同方向且更靠近XL-RIS的极端情况下，所提方案仍能确保安全通信，验证了算法的有效性和鲁棒性。

Conclusion: 本文提出的XL-RIS辅助近场物理层安全通信方案通过联合优化基站预编码和RIS反射系数，结合人工干扰技术，在极端窃听场景下仍能保障通信安全，为未来无线通信系统安全设计提供了有效解决方案。
...
### [181] [STAR Beyond Diagonal RISs with Amplification: Modeling and Optimization](https://arxiv.org/abs/2603.06020)
*Chandan Kumar Sheemar,Giovanni Iacovelli,Wali Ullah Khan,George C. Alexandropoulos,Stefano Tomasin,Symeon Chatzinotas*

Main category: cs.IT

TL;DR: 本文为具有硬件约束的STAR BD-RIS开发了物理一致的信号模型，提出了一种交替优化框架用于下行链路和速率最大化，相比传统被动BD-RIS实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统被动BD-RIS存在性能限制，需要开发具有硬件约束的物理一致信号模型，以支持同时传输和反射的RIS系统，并实现实际可行的优化方案。

Method: 1) 开发了具有硬件约束的STAR BD-RIS物理信号模型；2) 将下行链路和速率最大化转化为等效WMMSE问题；3) 提出具有可证明单调下降的交替优化框架；4) 采用闭式更新、类注水算法、循环坐标下降和黎曼梯度步等方法；5) 解耦反射和透射响应的优化。

Result: 数值结果表明，相比传统被动BD-RIS，所提方法实现了显著的和速率增益。优化框架能够有效处理硬件约束，保持每次迭代的被动性，并支持分布式实现。

Conclusion: 本文提出的STAR BD-RIS信号模型和交替优化框架能够有效处理硬件约束，实现显著的性能提升，为实际RIS系统部署提供了可行的解决方案。
...
### [182] [Distributed Semantic Alignment over Interference Channels: A Game-Theoretic Approach](https://arxiv.org/abs/2603.06077)
*Giuseppe Di Poce,Mattia Merluzzi,Emilio Calvanese Strinati,Paolo Di Lorenzo*

Main category: cs.IT

TL;DR: 本文提出了一种基于分布式非合作博弈的MIMO收发器联合优化方法，用于解决语义通信中的语义共存和潜在空间错位问题。


<details>
  <summary>Details</summary>
Motivation: 在AI驱动系统中，语义通信是有效任务执行的关键，但当设备依赖不同的逻辑和内部表示时，会出现语义不匹配问题。此外，在干扰信道环境中，多设备共存会因多用户干扰而导致性能显著下降。

Method: 将线性MIMO收发器的联合优化建模为分布式非合作博弈，推导出闭式解以有效解决语义共存和潜在空间错位问题。考虑多个点对点MIMO信道，将相应用户建模为自私玩家，优化其传输和语义对齐策略。

Result: 推导了纳什均衡存在的充分条件，并通过数值结果验证了所提方法在目标导向语义通信中的有效性，突出了信息压缩、干扰抑制、语义对齐和任务性能之间的关键权衡。

Conclusion: 提出的基于博弈论的MIMO收发器优化方法能够有效解决语义通信中的共存和错位问题，为AI驱动系统中的高效语义通信提供了理论框架和实用解决方案。
...
### [183] [On the Secrecy Performance of Continuous-Aperture Arrays Over Fading Channels](https://arxiv.org/abs/2603.06171)
*Xuan Yang,Chongjun Ouyang,Dongming Li,Yuanwei Liu*

Main category: cs.IT

TL;DR: 分析连续孔径阵列在窃听信道中的保密性能，包括保密速率和保密中断概率，比较了单窃听者、多独立窃听者和多协作窃听者三种场景，证明了CAPA系统相比传统离散阵列具有更好的保密性能。


<details>
  <summary>Details</summary>
Motivation: 研究连续孔径阵列在无线通信安全中的保密性能，分析其在窃听信道环境下的保密速率和保密中断概率，为物理层安全提供新的阵列技术方案。

Method: 建立CAPA系统在瑞利衰落信道下的最大比传输模型，使用Mercer定理和Landau特征值定理推导合法用户和窃听者的信噪比近似概率密度函数，分析三种窃听场景（单窃听者、多独立窃听者、多协作窃听者）。

Result: 在所有三种场景中，CAPA系统都达到相同的高信噪比斜率和分集阶数（等于空间自由度）；单窃听者场景具有最小的高信噪比偏移和最高的阵列增益，而多协作窃听者场景具有最大的高信噪比偏移和最低的阵列增益；CAPA系统相比半波长天线间距的离散阵列系统具有更高的保密速率和更低的保密中断概率。

Conclusion: 连续孔径阵列系统在物理层安全方面具有显著优势，能够提供更好的保密性能，特别是在高信噪比区域，为无线通信安全提供了有效的阵列技术解决方案。
...
### [184] [Belief-Adaptive MAP Detection for Molecular ISI Channels with Heteroscedastic Noise](https://arxiv.org/abs/2603.06304)
*Erencem Ozbey,H. Birkan Yilmaz,Chan-Byoung Chae*

Main category: cs.IT

TL;DR: 该论文提出了两种针对分子扩散通信中异方差噪声的解码机制：BA-MAP和Soft BA-MAP，通过显式考虑状态相关的均值和方差，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 分子扩散通信中的符号间干扰具有异方差（状态相关）噪声特性，但先前检测器设计未系统考虑这种跨ISI状态的噪声方差依赖性。

Method: 提出了两种解码机制：1) BA-MAP：基于接收器当前状态信念推导每个符号的自适应MAP阈值；2) Soft BA-MAP：通过加权所有可能的ISI状态计算混合对数似然比。

Result: 仿真和信息论分析证实，所提出的检测器优于传统的均衡和固定阈值方法，在实际MCvD设置下实现了高达100%的吞吐量改进。

Conclusion: 通过显式纳入分子计数通道的状态相关均值和方差，BA-MAP和Soft BA-MAP解码机制能够有效处理分子扩散通信中的异方差ISI噪声，显著提升系统性能。
...
### [185] [Quasi-twisted codes and their connection with additive constacyclic codes over finite fields](https://arxiv.org/abs/2603.06309)
*Kanat Abdukhalikov,Gyanendra K. Verma*

Main category: cs.IT

TL;DR: 本文研究了准扭码与加法常循环码之间的关系，通过多项式方法建立了它们之间的对应关系，并推导了各种对偶码的等价性。


<details>
  <summary>Details</summary>
Motivation: 研究准扭码与加法常循环码之间的理论联系，建立统一的框架来分析这两种码的对偶性和自正交性。

Method: 采用多项式方法，首先给出准扭码的多项式刻画，然后建立准扭码与加法常循环码之间的一一对应关系，利用迹内积与欧几里得、辛内积的关系推导对偶码。

Result: 建立了准扭码与加法常循环码的对应关系，确定了各种对偶码的等价性，给出了准扭码自正交性的充要条件。

Conclusion: 确定加法常循环码的迹欧几里得对偶和迹埃尔米特对偶等价于确定相应准扭码的欧几里得对偶和辛对偶，为研究这两类码提供了统一的框架。
...
### [186] [The DNA Coverage Depth Problem: Duality, Weight Distributions, and Applications](https://arxiv.org/abs/2603.06489)
*Matteo Bertuzzo,Alberto Ravagnani,Eitan Yaakobi*

Main category: cs.IT

TL;DR: 本文开发了组合工具来解决DNA数据存储中的覆盖深度问题，针对多种线性码推导了闭式解，并建立了线性码覆盖深度与其高域扩展权重分布之间的关系。
...
Method: 基于对偶论证和扩展权重枚举器的概念，开发了组合工具来分析线性码的覆盖深度。使用这些方法为多种具体码型推导闭式公式。

Result: 为单纯形码、汉明码、三元Golay码、扩展三元Golay码和一阶Reed-Muller码推导了覆盖深度的闭式公式。建立了线性码覆盖深度与其高域扩展权重分布之间的一般表达式。

Conclusion: 本文提供了解决DNA数据存储覆盖深度问题的系统方法，为多种重要线性码族提供了精确解，并建立了覆盖深度与码的代数结构之间的理论联系。
...
### [187] [Asymmetric Stream Allocation and Linear Decodability in MIMO Coded Caching](https://arxiv.org/abs/2603.06534)
*Mohammad NaseriTehrani,MohammadJavad Salehi,Antti Tölli*

Main category: cs.IT

TL;DR: 本文提出了一种基于每用户流分配的标准，保证MIMO编码缓存系统线性可解码性，并提出了支持非对称流分配的新调度框架，扩展了可实现自由度区域。


<details>
  <summary>Details</summary>
Motivation: 现有MIMO编码缓存系统的自由度分析、调度和线性接收机设计大多假设用户间对称流分配，限制了系统性能。为了充分利用MIMO-CC增益并实现实用线性接收机，需要灵活的调度机制。

Method: 推导了基于每用户流分配的简单标准，保证对称和非对称比特级编码缓存方案的线性可解码性。在此基础上，提出了启发式MIMO-CC传输和调度框架，支持非对称流分配同时保持线性可解码性。

Result: 提出的框架能够实现非对称流分配，同时满足线性可解码性要求，相比对称约束设计扩展了可实现自由度的可行区域。

Conclusion: 通过建立保证线性可解码性的标准并设计支持非对称流分配的调度框架，本文扩展了MIMO编码缓存系统的性能边界，为实际系统实现提供了更灵活的解决方案。
