...
TL;DR: 本文提出了一种基于生成式人工智能的解决方案，用于自动化SNAM能源基础设施的工厂结构获取，通过OCR、视觉LLM、目标检测等技术从P&ID图纸中提取设计数据和层次结构信息。


<details>
  <summary>Details</summary>
Motivation: 能源转型需要数字化和创新技术工具的支持，本文旨在设计基于人工智能的解决方案来自动化工厂数字化过程中的信息提取，简化MGM用户的日常工作。

Method: 使用OCR、视觉LLM、目标检测、关系推理和优化算法，并扩展了最先进的场景图生成模型，引入了全新的Transformer架构来深入分析工厂组件间的复杂关系。

Result: 文本信息提取准确率达到91%，组件识别准确率为93%，层次结构提取准确率约为80%。

Conclusion: 多种AI技术的协同使用克服了数据多样性带来的挑战，证明了该解决方案在工厂数字化过程中的有效性。
...
Method: 提出了包含三种测试设置的系统评估框架：静音输入生成、不匹配的音频-视频配对、匹配的音频-视频合成，并引入了嘴唇同步差异和基于静音音频的嘴唇同步得分等衍生指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多无人机监控系统在动态相机位置和复杂遮挡情况下表现不佳，需要更全面的数据集和算法来解决这些挑战。

Method: 提出MATRIX数据集（8个无人机同步视频）和深度学习框架，包括实时相机标定、基于特征的图像配准、鸟瞰图多视图特征融合。

Result: 在复杂环境下保持约90%检测跟踪准确率，成功跟踪约80%轨迹，迁移学习和相机故障实验显示良好的泛化能力和鲁棒性。

Conclusion: MATRIX数据集和框架为动态多视图监控系统提供了重要基准，在复杂环境中表现出优越性能。
...
TL;DR: 提出了一种新颖的持续异常检测框架，采用统一内存库而非任务特定的子内存库，通过核心集增量更新嵌入，在推理阶段使用最近邻匹配机制计算异常分数，在多个数据集上达到最先进性能。
...
Method: 所有任务共享统一内存库，在训练期间通过固定大小的核心集增量更新嵌入，实现连续知识获取而无需任务特定的内存碎片化。推理阶段使用最近邻匹配机制计算异常分数。
...
<details>
  <summary>Details</summary>
Motivation: 事故预测对于主动安全驾驶至关重要，但面临两个关键挑战：(1)天气、运动模糊或硬件限制导致的传感器输入噪声或退化；(2)需要在早期预警和误报抑制之间平衡的及时可靠预测需求。

Method: 集成扩散去噪模块和时间感知actor-critic模型。扩散模块通过迭代细化重建噪声弹性特征，保留关键运动和交互线索；actor-critic架构利用长期时间推理和时间加权奖励来确定最佳预警时机。

Result: 在三个基准数据集(DAD、CCD、A3D)上实现最先进精度，显著提高平均事故时间，并在高斯和脉冲噪声下保持稳健性能。定性分析显示模型产生更早、更稳定且与人类对齐的预测。

Conclusion: 该框架在常规和复杂交通场景中表现出色，具有现实世界安全关键部署的潜力。
...
TL;DR: 提出一种在潜在空间中保护视频基础模型视觉隐私的新方法，通过轻量级匿名适配器模块从视频特征中移除私人信息，同时保持任务实用性。
...
Method: 引入轻量级匿名适配器模块（AAM），可即插即用地应用于冻结的视频编码器，采用三种新设计的训练目标：剪辑级自监督隐私目标、协同训练目标和潜在一致性损失。

Result: 在广泛评估中显示隐私泄露显著减少35%，同时在动作识别、时序动作检测和异常检测等各种下游任务中保持接近基线的实用性性能。

Conclusion: 该方法有效保护视觉隐私并减少性别偏见，促进更公平的视频理解，同时最小化计算负担。
...
TL;DR: 本文研究了自回归像素预测的扩展特性，发现不同任务的最优扩展策略存在显著差异，生成任务需要数据量比分类任务增长快3-5倍，且随着分辨率增加模型规模需比数据增长更快。计算能力是主要瓶颈，预计未来5年内可实现像素级图像建模。


<details>
  <summary>Details</summary>
Motivation: 探索自回归像素预测这一简单但未被充分研究的统一视觉模型框架的扩展特性，了解不同计算预算下模型性能的变化规律。

Method: 在32x32分辨率下训练一系列Transformer模型，使用IsoFlops配置文件在高达7e19 FLOPs的计算预算范围内，评估三个目标指标：像素预测目标、ImageNet分类准确率和生成质量（Fr'echet距离）。

Result: 发现最优扩展策略高度依赖任务：在固定分辨率下，图像生成的最优设置需要数据量比分类设置增长快3-5倍；随着分辨率增加，模型规模需比数据量增长更快；计算能力是主要瓶颈而非训练数据量。

Conclusion: 基于计算能力每年增长4-5倍的预测，未来5年内有望实现像素级图像建模，这为统一视觉模型的发展提供了重要指导。
...
TL;DR: 本文研究使用扩散模型微调技术（如LoRA和DreamBooth）生成平衡的训练数据，以解决图像分类系统中的群体偏见问题。通过聚类和多个DreamBooth模型处理组内变化，在多个基准测试中表现优于原始Stable Diffusion，与SOTA去偏技术相当。


<details>
  <summary>Details</summary>
Motivation: 图像分类系统因训练数据中群体代表性不均而存在偏见，如人脸数据集中金发与女性的过度关联。现有方法使用Stable Diffusion生成平衡数据但难以保持原始数据分布。

Method: 探索多种扩散模型微调技术（LoRA、DreamBooth），通过聚类处理组内变化并训练多个DreamBooth模型，生成群体平衡数据用于预训练，然后在真实数据上微调。

Result: 在多个基准测试中，研究的微调方法平均优于原始Stable Diffusion，与SOTA去偏技术（如Group-DRO）结果相当，且随着数据集偏见严重性增加而超越它们。

Conclusion: 扩散模型微调技术能有效生成平衡训练数据，缓解图像分类中的群体偏见问题，在严重偏见情况下表现优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注真实场景中的小相机移动或仅考虑非自然的物体中心场景，限制了在真实世界设置中的潜在应用。本文旨在突破这些限制，处理日常室内外自然场景的单视角新视角合成问题。

Method: 提出DT-NVS，一个3D感知的扩散模型，利用基于transformer的架构骨干。对transformer和自注意力架构进行重要改进以将图像转换为3D表示，并采用新颖的相机条件策略在真实世界未对齐数据集上训练。引入新的训练范式，在条件图像和采样噪声输入之间交换参考帧的角色。

Result: 在单输入图像的广义新视角合成3D任务上评估，相比最先进的3D感知扩散模型和确定性方法有所改进，同时生成多样化的输出。

Conclusion: DT-NVS在真实世界未对齐数据集上成功训练，能够从单张图像生成高质量的新视角，为自然场景的新视角合成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D点云旋转不变学习方法通常使用手工制作的RI特征来确保旋转鲁棒性，但会丢失全局姿态信息，无法区分几何相似但空间位置不同的结构。这种限制源于现有RI方法的受限感受野，导致翼尖特征崩溃问题。

Method: 提出了阴影信息姿态特征(SiPF)，通过从学习的共享旋转中推导出的全局一致参考点（称为'阴影'）来增强局部RI描述符；设计了旋转不变注意力卷积(RIAttnConv)，将SiPF集成到特征聚合过程中；开发了基于Bingham分布的任务自适应阴影定位模块，动态学习构建一致阴影的最佳全局旋转。

Result: 在3D分类和部件分割基准测试上的广泛实验表明，该方法显著优于现有的RI方法，特别是在需要任意旋转下细粒度空间区分的任务中。

Conclusion: 所提出的方法通过结合局部旋转不变性和全局姿态感知，有效解决了现有旋转不变学习方法的局限性，在保持旋转不变性的同时提高了对空间结构的区分能力。
...
<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤预后极差，自动、稳健、准确地识别其不同组织学亚区域有助于大规模形态学理解该疾病。

Method: 使用EfficientNet架构（B0-B4变体）的四步深度学习分类方法，在BraTS-Path 2024挑战数据集上进行训练和评估。

Result: EfficientNet-B1和B4在5折交叉验证中F1分数达0.98，但在验证集和测试集上分别降至0.546和0.517，显示模型泛化能力挑战。

Conclusion: 开发能够良好泛化到新数据的模型对于临床应用至关重要，当前方法在训练数据上表现优异但在未见数据上性能下降。
...
TL;DR: 本文提出了一种非对称跨模态知识蒸馏框架SemBridge，用于处理语义一致性较弱的多模态数据，通过学生友好匹配模块和语义感知知识对齐模块解决知识传输成本问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中配对模态数据有限，对称跨模态知识蒸馏应用受限，需要研究在弱语义一致性下的知识学习方法。
...
Conclusion: SemBridge框架有效解决了弱语义一致性下的跨模态知识蒸馏问题，在遥感场景分类等应用中表现出色。
...
TL;DR: 提出融合视觉预测与LLM结构先验的半监督文档布局理解框架，通过概率加权提升检测性能，在轻量级和预训练模型上都取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 文档布局理解仍然需要大量标注数据，现有半监督方法存在局限性，需要结合文本预训练LLM的结构先验知识来提升性能。

Method: 使用OCR-LLM流水线推断文档层次区域，通过逆方差融合将教师检测器输出与LLM结构先验结合，生成精炼的伪标签，并采用实例自适应门控机制。

Result: 在PubLayNet数据集上，仅使用5%标签：轻量级SwiftFormer达到88.2±0.3 AP；LayoutLMv3达到89.7±0.4 AP，超越标准半监督学习，匹配需要1亿+页面多模态预训练的UDOP。

Conclusion: LLM结构先验与轻量级和预训练架构具有互补性，自适应门控机制显著提升性能，开源LLM可实现隐私保护部署，LLM提供语义消歧能力超越简单文本启发式方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决CNN目标检测的形状依赖性缺陷，特别是对于训练数据中未包含的姿态，在资源受限的边缘设备上实现可靠的人员检测。

Method: 利用服装的光谱反射特性进行形状无关的人员检测，在无GPU的边缘设备上实现HitoMi-Cam系统。

Result: 处理速度达到23.2 fps（253x190像素），在模拟搜救场景中平均精度达93.5%，远超CNN模型的最佳精度53.8%，误报率极低。

Conclusion: HitoMi-Cam不是替代CNN检测器的方案，而是在形状不可预测的真实环境（如灾难救援）中，作为边缘设备实时操作的补充工具。
...
<details>
  <summary>Details</summary>
Motivation: 文本训练方法在零样本图像描述中面临跨域泛化能力差的问题，容易产生幻觉内容。检索方法虽然试图缓解这个问题，但可能因检索到无关实体而加剧幻觉。

Method: 提出负实体抑制(NES)方法，包含三个阶段：使用合成图像确保检索一致性、过滤检索内容中的负实体、应用注意力级抑制来减少幻觉倾向特征的影响。

Result: 在多个基准测试中，NES保持了竞争力的域内性能，同时提高了跨域迁移能力并降低了幻觉率，在零样本图像描述中达到了新的最先进结果。
...
TL;DR: SPEED-Q是一个针对边缘设备上视觉语言模型(VLMs)的低比特量化框架，通过分阶段敏感度自适应机制和蒸馏增强策略，解决了视觉与语言组件量化敏感度差异和训练不稳定问题，在2比特设置下比现有方法准确率提升高达6倍。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署视觉语言模型需要解决资源限制问题，量化是提高内存效率和减少带宽需求的关键技术，但现有研究很少探索VLMs的激进量化，特别是适合边缘设备的1B-2B参数模型。
...
TL;DR: 本文提出了一种基于机器视觉导向压缩的人机协同压缩方法Diff-FCHM，通过扩散先验技术为人类视觉恢复高保真细节，在机器视觉和人类视觉压缩方面均取得显著性能提升。
...
Method: 提出基于机器视觉导向压缩的协同压缩方法，开发即插即用的可变比特率策略，逐步聚合机器视觉压缩的语义信息，同时无缝利用扩散先验为人类视觉恢复高保真细节。
...
TL;DR: 本文提出了一种分层蒸馏（HD）框架，将轨迹蒸馏和分布蒸馏从独立范式转变为协同组件，通过轨迹蒸馏建立结构"草图"，为后续基于分布的细化阶段提供近最优初始化，并引入自适应加权鉴别器（AWD）来改进对抗训练过程。
...
Method: 提出分层蒸馏框架：1）使用轨迹蒸馏建立结构草图作为初始化；2）基于分布的细化阶段；3）引入自适应加权鉴别器（AWD），通过动态分配token权重专注于局部缺陷，实现高效细节细化。

Result: 在ImageNet 256×256上，单步模型达到FID 2.26，媲美其250步教师模型；在高分辨率文本到图像MJHQ基准测试中也取得有希望的结果，证明了方法的通用性。

Conclusion: 该方法为高保真度单步扩散模型建立了一个强大的新范式，通过协同使用轨迹和分布蒸馏方法，结合专门设计的对抗训练过程，实现了最先进的性能。
...
TL;DR: 本文提出了一种名为BRHVC的双向参考协调视频压缩方法，通过双向运动收敛和双向上下文融合技术，优化了双向参考帧的利用，在HEVC数据集上超越了传统编码方法VTM-RA。


<details>
  <summary>Details</summary>
Motivation: 神经B帧视频压缩（NBVC）相比P帧压缩研究较少，但具有使用双向参考帧提升压缩性能的潜力。然而，分层编码可能导致连续时间预测复杂化，特别是在帧跨度较大的层次上，使得两个参考帧的贡献不平衡。
...
Conclusion: 通过更高效的运动和上下文处理，BRHVC能够有效协调双向参考，为神经B帧视频压缩提供了有前景的解决方案。
...
TL;DR: 本文提出了一种基于豪斯多夫维数（HD）的方法来增强分形生成模型（FGM）的生成多样性，通过可学习的HD估计、动量调度策略和HD引导拒绝采样，在保持图像质量的同时显著提升了生成多样性。


<details>
  <summary>Details</summary>
Motivation: 分形生成模型虽然能生成高质量图像，但其固有的自相似性限制了输出图像的多样性。为了解决这个问题，需要一种能够量化结构复杂度并增强生成多样性的方法。

Method: 提出FGM-HD框架：1）可学习的HD估计方法直接从图像嵌入预测HD；2）采用基于HD的损失函数和单调动量驱动调度策略来渐进优化超参数；3）在推理阶段使用HD引导的拒绝采样选择几何更丰富的输出。

Result: 在ImageNet数据集上的实验表明，与原始FGM相比，FGM-HD框架在保持可比图像质量的同时，输出多样性提高了39%。

Conclusion: 这是首次将豪斯多夫维数引入分形生成模型的工作，有效增强了生成输出的多样性，并为FGM发展提供了理论贡献。
...
TL;DR: AuthSig是一个基于生成模型和水印技术的静态电子签名框架，通过将认证信息嵌入签名图像来解决静态签名易被复制和重用的安全问题。


<details>
  <summary>Details</summary>
Motivation: 随着无纸化工作流程的普及，静态扫描签名因其便利性而广泛使用，但这些签名图像几乎失去了认证属性，无法可靠验证且易受恶意复制和重用。

Method: 利用人类视觉系统对细微风格变化不敏感的特性，AuthSig在生成过程中精细调节风格嵌入来隐式编码水印位，实现"一次签名，一次使用"策略。为解决手写签名数据稀缺问题，提出了关键点驱动的数据增强策略来增强风格多样性。

Result: 实验结果显示，AuthSig在数字域失真和签名特定退化条件下实现了超过98%的提取准确率，在打印扫描场景下仍保持有效。

Conclusion: AuthSig框架成功地将认证信息绑定到签名图像中，有效解决了静态电子签名的安全验证问题。
...
TL;DR: 本文提出了一种名为CoDR的核心集双检索框架，用于改进大型视觉语言模型中的上下文学习演示选择问题，通过构建多样化核心集和双检索机制来平衡效率与效果。


<details>
  <summary>Details</summary>
Motivation: 传统演示选择策略（随机、基于相似性、基于信息分数）在效率和效果上存在不足，无法有效平衡两者，且演示选择本身是NP难问题。

Method: 提出CoDR框架：1）使用聚类剪枝方法构建多样化核心集，使样本具有更高期望互信息；2）开发双检索机制，在保持效率的同时实现全局演示选择。
...
TL;DR: 提出WDT-MD框架，通过小波扩散Transformer解决微动脉瘤检测中的身份映射、假阳性高和正常特征重建差三个关键问题，在IDRiD和e-ophtha数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 微动脉瘤是糖尿病视网膜病变的最早病理标志，但现有扩散异常检测方法存在身份映射、难以区分微动脉瘤与其他异常、正常特征重建差三个主要限制，阻碍临床应用。

Method: WDT-MD框架包含三个创新：噪声编码图像条件机制避免身份映射；伪正常模式合成提供像素级监督；小波扩散Transformer架构结合全局建模和多尺度分析。

Result: 在IDRiD和e-ophtha MA数据集上的综合实验表明，WDT-MD在像素级和图像级微动脉瘤检测中均优于最先进方法。

Conclusion: 该进展对改善早期糖尿病视网膜病变筛查具有重要前景。
...
Method: 在ICTM框架下集成去噪项，包括I-散度项和自适应TV正则器，使用灰度级指示器指导扩散，估计平滑变化的偏置场，采用RMSAV方案进行高效优化。

Result: 在合成和真实图像上的广泛实验表明，该模型在存在强度不均匀性和多种噪声类型的情况下，相比竞争方法实现了更高的准确性和鲁棒性。
...
Method: 提出统一视觉提示编码器处理正负视觉提示；训练免费的负向否定计算模块动态抑制负响应；负向否定铰链损失通过微调增强正负嵌入的判别性边界。
...
TL;DR: Owl是一个基于因果关系的双模态注意力重加权框架，通过建模视觉和文本注意力的交互作用来缓解大型视觉语言模型中的物体幻觉问题。
...
Result: 在POPE和CHAIR基准测试中，Owl显著减少幻觉，在保持视觉语言理解能力的同时实现了最先进的忠实度。

Conclusion: Owl通过因果建模和注意力重加权有效缓解了物体幻觉问题，为视觉语言模型的可靠性提供了新解决方案。
...
TL;DR: 提出了一种密集跨尺度图像对齐模型，通过考虑跨尺度特征相关性降低对齐难度，支持精度与效率的灵活权衡，并引入全空间相关性模块和恰可察觉差异来提升精度。
...
Method: 采用密集跨尺度图像对齐模型，利用跨尺度特征相关性，结合全空间相关性模块和恰可察觉差异机制，支持通过调整尺度数量实现精度与效率的权衡。
...
TL;DR: PAN是一个通用的、可交互的、长视野的世界模型，通过高质量视频模拟预测未来世界状态，结合语言模型的知识基础和视频扩散解码器，实现潜在空间推理与可现实世界动态的统一。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏因果控制、交互性和长视野一致性，而现有世界模型往往局限于特定领域且深度和可控性有限，难以在不同环境和交互格式中泛化。

Method: 采用生成潜在预测（GLP）架构，结合基于大语言模型的自回归潜在动态骨干（提供文本知识基础并支持语言指定动作）和视频扩散解码器（重建感知细节和时间一致的视觉观察）。
...
TL;DR: 本文提出了一种名为方向正交对抗攻击（DOC）的新方法，通过引入正交梯度方向和动量更新来增强对抗攻击的多样性和覆盖范围，从而提高视觉语言预训练模型在测试时防御中的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时对抗攻击方法（TTC）由于对抗攻击和对抗攻击在优化目标上的根本差异，仅基于对抗输入的梯度生成对抗攻击，导致搜索空间狭窄，攻击多样性不足，无法完全中和广泛的扰动。

Method: 提出方向正交对抗攻击（DOC），通过结合正交梯度方向和动量更新来增强对抗攻击优化，扩展对抗攻击空间的探索，增加扰动的多样性。同时提出基于平均余弦相似度的方向敏感度评分，通过改进示例区分和自适应调节对抗攻击强度来提升DOC效果。

Result: 在16个数据集上的广泛实验表明，DOC在各种攻击下提高了对抗鲁棒性，同时保持了竞争力的干净准确率。

Conclusion: 增强对抗攻击的多样性和覆盖范围对于提高测试时防御中的对抗鲁棒性至关重要，DOC方法通过正交梯度方向和动量更新有效实现了这一目标，显著提升了模型的鲁棒性。
...
TL;DR: 本文提出了组合增量学习（CompIL）框架，用于在组合零样本学习中持续学习新组合，通过视觉合成器和语言基元蒸馏机制来提升组合泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据不断涌现，组合数量近乎无限且呈长尾分布，需要模型以增量方式持续提升组合泛化能力。

Method: 提出基于伪回放的框架，使用视觉合成器合成已学组合的视觉表示，并采用语言基元蒸馏机制保持基元表示在学习过程中的对齐。

Result: 在MIT-States-CompIL和C-GQA-CompIL基准上的大量实验证明了所提框架的有效性。

Conclusion: 组合增量学习框架能够有效提升模型在组合零样本学习任务中的持续组合泛化能力。
...
TL;DR: 本文介绍了DKDS数据集，专门针对古日文草书体Kuzushiji的文档退化问题，包含文本和印章检测、文档二值化两个基准任务，并提供了基于YOLO和GAN的基线方法。
...
Result: 提供了文本和印章检测的YOLO基线结果，以及文档二值化的多种方法基线结果，数据集和实现代码已公开。
...
TL;DR: PressTrack-HMR是一个从压力信号恢复多人全局人体网格的顶向下流水线，通过检测跟踪策略从原始压力数据中识别和分割每个人的压力信号，然后在多人场景下进行人体网格恢复。


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的人体网格恢复方法在现实场景中因相互遮挡、光照不足和隐私问题而受限，而人-地板触觉交互提供了一种无遮挡且保护隐私的替代方案。

Method: 采用顶向下流水线，首先使用检测跟踪策略从原始压力数据中识别和分割每个人的压力信号，然后对每个提取的个体信号执行人体网格恢复。

Result: 实验结果显示该方法在多人人体网格恢复方面表现优异，MPJPE为89.2mm，WA-MPJPE100为112.6mm，展示了触觉垫在多人动作识别中的潜力。

Conclusion: 该方法证明了仅使用压力信号进行多人全局人体网格恢复的可行性，并为基于压力的人类运动分析在多人场景中的进一步研究提供了数据集和代码支持。
...
TL;DR: HOTFLoc++是一个用于森林环境中LiDAR地点识别、重排序和6自由度度量定位的端到端框架，采用八叉树变换器提取多粒度层次局部描述符，通过可学习多尺度几何验证模块减少重排序失败，在公共数据集上表现出优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 解决森林环境中由于杂乱、自相似性和视角变化导致的LiDAR地点识别和定位挑战，特别是在地面到地面和地面到空中的复杂场景中。

Method: 使用基于八叉树的变换器提取层次局部描述符，提出可学习多尺度几何验证模块，采用从粗到精的配准方法。

Result: 在CS-Wild-Places数据集上平均Recall@1达到90.7%，比基线提高29.6个百分点；在Wild-Places和MulRan数据集上分别达到91.7%和96.0%；97.2%的6自由度配准尝试误差小于2米和5度；多尺度重排序模块平均减少约2倍定位误差。

Conclusion: 该方法在保持高精度的同时，相比RANSAC在密集点云上的运行时间提高了两个数量级，在挑战性森林环境中表现出优越的地点识别和定位性能。
...
TL;DR: DBINDS是一种基于扩散模型反演的AI生成视频检测器，通过分析潜在空间动态而非像素级特征来检测AI生成视频，在跨生成器检测中表现出良好的泛化能力。
...
Method: 提出DBINDS检测器，基于扩散模型反演分析潜在空间动态。发现真实视频和生成视频通过扩散反演恢复的初始噪声序列存在系统性差异，据此构建初始噪声差异序列(INDS)并提取多域多尺度特征，使用特征优化和贝叶斯搜索调优的LightGBM分类器。

Result: 在仅使用单一生成器训练的情况下，DBINDS在GenVidBench基准上实现了强大的跨生成器检测性能，在有限数据设置下表现出良好的泛化能力和鲁棒性。

Conclusion: DBINDS通过分析潜在空间动态而非像素特征，有效解决了AI生成视频检测中的跨生成器泛化问题，为内容安全提供了新的解决方案。
...
TL;DR: 提出了一个结合DermBench基准和DermEval评估器的皮肤病学诊断评估框架，用于可靠评估多模态大语言模型生成的诊断叙述。
...
Method: 构建DermBench基准（包含4000张真实皮肤病图像和专家认证的诊断叙述），并使用基于LLM的评判器对候选叙述进行临床维度评分；训练DermEval无参考多模态评估器，对图像和生成叙述进行结构化批判和评分。

Result: 在4500个病例的多样化数据集上，DermBench和DermEval与专家评分高度一致，平均偏差分别为0.251和0.117（满分5分）。

Conclusion: 该框架为不同多模态LLM的诊断能力和可信度提供了可靠测量，支持临床有意义、可重复和可扩展的评估。
...
TL;DR: TACO是一个通过自验证和置信度校准来减少多模态大语言模型幻觉的框架，无需外部视觉专家，在多个基准测试中表现优于直接提示和视觉对比解码。
...
Method: TACO将回答分解为原子查询，通过改写减少对措辞的敏感性，使用自一致性或自置信度聚合来估计置信度，最后用语言模型精炼答案。

Result: 在五个基准测试和两个MLLM上的实验表明，TACO持续优于直接提示和视觉对比解码，减少了系统性偏差，改善了置信度校准。

Conclusion: TACO框架有效增强了多模态大语言模型的忠实性，展示了其在缓解幻觉问题方面的有效性。
...
TL;DR: 本文提出RichKD方法，通过融合传统教师模型和CLIP的视觉-语言知识来增强多教师知识蒸馏，利用多提示文本指导获得语义丰富的监督信号，提升学生模型的准确性、置信度和鲁棒性。
...
Method: 提出RichKD框架，融合传统教师模型和CLIP的logits和特征，结合CLIP的多提示文本指导，同时捕捉数据集特定和语义丰富的视觉线索。

Result: 方法在多个基准测试中优于现有基线，产生更自信可靠的预测，显著增加正确置信案例，减少错误置信案例，并在分布偏移和输入损坏下表现出更强的鲁棒性。

Conclusion: 融合CLIP的视觉-语言知识能有效提升知识蒸馏性能，即使方法简单，但在准确性、置信度校准和鲁棒性方面均有显著改善。
...
TL;DR: DensiCrafter是一个从多模态输入生成轻量化、自支撑3D空心结构的框架，通过优化密度场实现，无需物理模拟即可减少材料质量达43%。
...
Result: 在文本到3D任务中实现材料质量减少达43%，相比基线方法提高了稳定性并保持高几何保真度，真实3D打印实验验证了可制造性。

Conclusion: DensiCrafter能够生成可实际制造的自支撑轻量化3D结构，无需修改预训练模型架构，具有良好的实用价值。
...
TL;DR: 该论文在师生框架中引入反馈机制来纠正错误传播，提出双教师反馈模型，通过学生反馈和教师间的交叉监督来减少半监督医学图像分割中的错误确认问题。


<details>
  <summary>Details</summary>
Motivation: 传统师生范式在医学图像分割中面临图像模糊性导致的错误监督问题，学生反复确认错误会形成自我强化的偏见，现有方法往往依赖外部修改而忽略了框架内在的错误纠正潜力。

Method: 在师生框架中引入反馈机制，包括反馈归因器识别触发学生更新的伪标签，以及反馈接收器确定反馈应用位置。进一步提出双教师反馈模型，通过教师间的交叉监督解决分歧并避免一致错误。

Result: 在三个医学图像基准上的综合评估表明，该方法能有效解决半监督医学图像分割中的错误传播问题。

Conclusion: 通过将反馈机制整合到师生框架中，特别是双教师反馈模型，能够有效对抗错误确认，提升半监督医学图像分割的性能。
...
Conclusion: FQ-PETR成功解决了PETR系列模型量化中的关键挑战，实现了高效部署所需的精度与效率平衡，为自动驾驶3D检测模型的实用化部署提供了有效解决方案。
...
Method: Flora方法包含两个核心组件：1) 灵活的邻居感知语义调整，通过整合相邻类间上下文线索形成方向感知的区域语义，结合跨模态几何一致性目标确保稳定的点对区域对齐；2) 开放形式的分布感知流分类器，使用无噪声流匹配来弥合语义和骨架潜在嵌入之间的模态分布差距，同时通过无条件对比正则化增强可区分性。
...
TL;DR: 本文提出了OUGS框架，通过基于3D高斯基元物理参数的不确定性建模和语义分割集成，实现了针对特定物体的高效主动重建，显著提升了3D高斯泼溅重建的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有主动重建方法依赖场景级不确定性指标，在复杂场景中容易受到无关背景干扰，导致针对特定物体的视图选择效率低下。需要一种更原则性的方法来解耦目标物体与其环境的不确定性。

Method: 从3D高斯基元的物理参数（位置、尺度、旋转）直接推导不确定性，通过渲染雅可比矩阵传播参数协方差，建立可解释的不确定性模型。然后集成语义分割掩码生成针对性的物体感知不确定性分数。

Result: 在公开数据集上的实验评估表明，该方法显著提高了3DGS重建过程的效率，相比现有最先进方法，对目标物体实现了更高质量的重建，同时作为全局场景的鲁棒不确定性估计器。

Conclusion: OUGS框架通过物理基础的不确定性建模和物体感知的视图选择策略，有效解决了复杂场景中特定物体重建的挑战，为3D高斯泼溅提供了更高效和精确的重建方案。
...
Method: 提出基于视觉的位姿优化框架，使用微调的模态和域不变编码器计算内窥镜RGB帧与CT渲染深度图之间的相似度，通过可微分渲染模块迭代优化相机位姿。

Result: 在合成基准数据集上，模型仅使用合成数据训练，达到平均平移误差2.65毫米和旋转误差0.19弧度，在真实患者数据上表现出强大的跨域泛化能力。
...
TL;DR: 本文构建了首个美式足球运动员专用检测与跟踪数据集，填补了该领域缺乏标准化数据集的空白，并在拥挤场景下实现了准确的运动员检测与跟踪。
...
Conclusion: 这项工作能够在传统方法难以应对的挑战性高密度场景中实现稳健的美式足球运动员检测与跟踪。
...
TL;DR: 提出了一种双教师知识蒸馏框架，利用异构ViT教师和同构CNN教师协同指导轻量级CNN学生，解决了现有跨架构知识蒸馏方法中的架构不匹配问题。


<details>
  <summary>Details</summary>
Motivation: ViT在视频动作识别中表现优异但计算成本高，轻量级CNN效率高但精度不足。现有跨架构知识蒸馏方法难以处理架构不匹配问题，且忽视了同构CNN教师的价值。

Method: 采用双教师知识蒸馏框架，包含两个关键组件：1）差异感知教师加权，基于教师置信度和与学生预测差异动态融合ViT和CNN教师的预测；2）结构差异感知蒸馏，学生通过轻量级辅助分支学习ViT与CNN教师之间的残差特征。

Result: 在HMDB51、EPIC-KITCHENS-100和Kinetics-400等基准测试中，该方法持续优于最先进的蒸馏方法，在HMDB51上最高获得5.95%的准确率提升。

Conclusion: 所提出的双教师知识蒸馏框架有效解决了跨架构知识蒸馏中的挑战，通过协同利用异构和同构教师，显著提升了轻量级CNN学生的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法主要依赖几何线索并独立预测每个3D姿态，无法有效解决模糊运动问题，也难以泛化到真实场景。受人类理解和预测运动方式的启发，需要结合动作感知推理和时间想象。

Method: 1) 使用从2D姿态序列提取的任务相关动作提示动态调节去噪过程；2) 引入表示编码器，将运动学关节亲和度融入注意力机制；3) 使用幻觉姿态解码器在训练期间预测时间一致的3D姿态序列。

Result: 在Human3.6M和MPI-3DHP基准数据集上的广泛实验显示，在所有指标上均达到最先进性能。在广播棒球数据集上的测试进一步验证了其鲁棒性，即使在模糊和噪声的2D输入下也能保持强性能。

Conclusion: DreamPose3D通过结合动作感知推理、时间想象和结构关系建模，有效解决了3D人体姿态估计中的时间一致性和模糊运动问题，在多个数据集上展现出卓越性能。
...
TL;DR: 本文研究自动归因方法，将摘要中的每个句子链接到源文本的相应部分，探索使用混合摘要来简化归因过程，并通过自定义拓扑识别归因相关错误的比例。
...
Method: 使用混合摘要方法（即提取式摘要的自动改写）来简化归因过程，并采用自定义拓扑来识别不同类别归因相关错误的比例。

Result: 通过混合摘要方法，归因过程得到简化，同时自定义拓扑能够有效识别和分类归因相关的错误类型及其比例。

Conclusion: 混合摘要和自定义拓扑的结合为自动归因提供了有效的解决方案，能够简化归因过程并准确识别归因错误，从而提升摘要的可追溯性和可靠性。
...
TL;DR: GMTRouter是一个基于异构图学习的个性化LLM路由框架，通过四类节点（用户、LLM、查询、响应）建模多轮交互，在少样本数据下有效捕获用户偏好，显著提升路由准确率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由方法缺乏完全个性化，难以捕捉用户与LLM的复杂交互关系，且用户偏好数据通常稀缺、嘈杂、格式不一致，限制了仅依赖用户特定数据的方法效果。

Method: 将多轮用户-LLM交互表示为包含用户、LLM、查询、响应四类节点的异构图，通过定制化的消息传递机制，在轻量级归纳图学习框架中从少样本数据学习用户偏好。

Result: 在多个数据集上持续优于强基线，准确率提高0.9-21.6%，AUC提高0.006-0.309，能够仅用少样本数据适应新用户和演化偏好，无需大量微调。

Conclusion: GMTRouter通过异构图建模有效解决了LLM路由中的个性化挑战，在少样本场景下实现了高效的用户偏好学习，为个性化LLM路由提供了有效解决方案。
...
TL;DR: 本研究评估了大型语言模型（LLMs）模拟社交媒体群组对话的能力，发现LLM生成的对话有39%的时间被误认为是人类创作的内容，特别是Llama 3生成的对话只有56%的时间被正确识别为AI生成，略高于随机概率。


<details>
  <summary>Details</summary>
Motivation: 验证LLMs能否真实模拟社交媒体上各种用户之间的对话，探索其在内容推荐算法测试、内容政策效果评估等应用中的有效性。

Method: 从Reddit收集真实人类对话，使用Llama 3 70B和GPT-4o在同一主题上生成人工对话，通过并排展示给研究参与者进行识别测试。

Result: LLM生成的对话有39%的时间被误认为是人类创作；Llama 3生成的对话只有56%的时间被正确识别为AI生成，接近随机概率水平。

Conclusion: LLMs能够生成足够逼真的社交媒体对话来欺骗人类读者，既展示了社交模拟的潜力，也警示了LLMs可能被滥用于生成不真实的社交媒体内容。
...
TL;DR: 本文研究了大型语言模型在法律框架解释中的潜力与风险，特别关注美国法典第18章第175条关于生物武器的规定。研究发现LLMs既能分析法律，也存在生成不安全输出的漏洞，提出了结合知识图谱和RAG的方法来评估模型的法律理解和安全机制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂法律框架解释方面具有变革潜力，特别是在生物武器等敏感领域。然而，这些模型在生成不安全输出方面存在令人担忧的漏洞，需要系统评估其法律理解能力和安全机制。

Method: 提出了一种结合知识图谱构建和检索增强生成的方法，通过结构化实验评估LLMs对法律的理解能力、法律意图评估能力以及在生物武器相关场景中的潜在不安全应用。

Result: 研究发现LLMs在推理和安全机制方面存在显著局限性，但同时也指出了改进方向。实验评估了模型识别法律违规、生成禁止性指令和检测非法意图的准确性。

Conclusion: 通过结合增强的安全协议和更强大的法律推理框架，这项研究为开发能够在敏感法律领域提供道德和安全协助的LLMs奠定了基础，确保它们成为法律的保护者而非无意中的违规促成者。
...
TL;DR: 本文提出了一个框架来测试LLMs在对抗性提示下的真实性保真度，通过让LLM生成并验证真假陈述，然后测试其对自身生成的谎言的抵抗能力。
...
Method: 三步框架：1) LLM生成与特定封闭领域一致的真假陈述；2) LLM验证这些陈述的真假；3) 测试LLM对自身生成谎言的抵抗能力。

Result: 在电影和小说两个封闭领域的评估显示，Claude表现出强韧性，GPT和Grok中等韧性，Gemini和DeepSeek韧性较弱。

Conclusion: 随着越来越多人使用LLMs进行信息检索，这些发现敲响了警钟，表明LLMs在对抗性提示下存在显著的真实性保真度问题。
...
TL;DR: 论文提出Self-HarmLLM攻击场景，利用模型自身生成的模糊有害查询作为新输入来绕过防护机制，在多种条件下实现了高达65%的转换成功率和41%的越狱成功率。
...
Method: 使用同一模型生成的Mitigated Harmful Query作为新输入，在GPT-3.5-turbo、LLaMA3-8B-instruct和DeepSeek-R1-Distill-Qwen-7B模型上进行Base、Zero-shot和Few-shot条件实验。

Result: Zero-shot条件下转换成功率最高52%，越狱成功率最高33%；Few-shot条件下转换成功率最高65%，越狱成功率最高41%。自动评估高估越狱成功率，平均偏差52%。

Conclusion: 需要重新考虑防护机制设计并建立更鲁棒的评估方法，自动评估单独使用不准确。
...
Method: 原型开发了一个多模型RAG系统，将精选的领域知识与工程化提示模板相结合，支持五种商业和开源大语言模型。设计了七个涵盖不同障碍类型和年级水平的测试场景，并使用多维评估标准对生成案例进行自动质量评估。

Result: 概念验证证明了RAG增强生成儿科言语语言病理学案例的技术可行性。商业模型显示出边际质量优势，但开源替代方案达到了可接受的性能，表明在保护隐私的机构部署方面具有潜力。

Conclusion: 整合精选知识库能够生成符合专业指南的内容。在教育或研究实施前需要通过专家评审、学生试点测试和心理测量评估进行广泛验证。未来应用可能扩展到临床决策支持、自动IEP目标生成和临床反思训练。
...
TL;DR: 本文扩展了DisCoCirc形式化方法，为孟加拉语开发了类似框架，并应用于英-孟翻译任务。研究发现DisCoCirc在减少语言官僚主义方面存在局限性，特别是在处理两种语言结构差异时，即使对相对简单的句子也面临困难。


<details>
  <summary>Details</summary>
Motivation: 扩展DisCoCirc形式化方法到孟加拉语，评估其在英-孟翻译中的有效性，重新审视该框架在减少语言官僚主义方面的能力。

Method: 基于DisCoCirc的语法框架，为孟加拉语开发类似的电路式表示方法，应用于英-孟翻译任务，并分析其处理语言结构差异的能力。

Result: 研究发现DisCoCirc虽然适用于大部分语言，但由于英-孟两种语言的结构差异，该框架在翻译中仍面临显著限制，即使对简单句子也存在困难。

Conclusion: DisCoCirc在跨语言翻译中存在局限性，这突显了框架的约束，同时也为未来改进提供了方向。研究还展示了英语连词与布尔逻辑之间的联系。
...
TL;DR: 开发了Mina，一个面向孟加拉国背景的多语言LLM法律助手，通过多语言嵌入和RAG工具链框架提供法律草稿、引用和通俗解释，在孟加拉国律师考试中达到75-80%的分数，匹配或超越人类平均水平。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国低收入人群面临法律语言复杂、程序不透明和成本高等障碍，现有AI法律助手缺乏孟加拉语支持和司法管辖区适应。

Method: 采用多语言嵌入和基于RAG的工具链框架，实现检索、推理、翻译和文档生成功能，通过交互式聊天界面提供上下文感知的法律服务。

Result: 在2022和2023年孟加拉国律师委员会考试的所有阶段评估中，Mina在初步多选题、笔试和模拟口试中得分75-80%，匹配或超越人类平均表现。

Conclusion: Mina证明了作为低成本多语言AI助手的潜力，能够自动化关键法律任务并扩大司法可及性，为构建领域特定、低资源系统提供了真实案例研究。
...
TL;DR: 本研究构建了一个名为MEDAS的医疗紧急诊断咨询系统，通过元学习器整合五种主流大语言模型（Gemini、Llama、Grok、GPT、Claude），显著提升了急诊疾病的诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 医疗决策支持系统对急诊医生快速准确评估患者状况至关重要，大语言模型在医疗决策支持领域具有应用潜力，但个体模型的诊断能力存在差异。

Method: 使用元学习方法构建超级学习器MEDAS，整合五种大语言模型的集体能力，通过元学习器学习每个LLM的不同能力来提升诊断准确性。

Result: 个体LLM的诊断准确率在58%-65%之间，已超过人类医生报告准确率；超级学习器MEDAS将诊断准确率提升至70%，其中至少一个集成LLM达到85%的正确诊断率。

Conclusion: 通过元学习方法整合多个LLM的集体知识，可以显著提升医疗紧急诊断的准确性，超级学习器的诊断准确率超过任何单个LLM。
...
TL;DR: 提出了一种基于梯度感知的自适应数据选择方法GrADS，用于LLM的监督微调，通过分析梯度选择有效训练数据子集，显著提高效率并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有LLM监督微调在领域专业化时资源消耗大且容易导致灾难性遗忘，需要一种更高效的数据选择方法。

Method: 设计自引导准则，利用梯度的幅度和统计分布来优先选择对模型学习过程贡献最大的训练样本。

Result: 在医学、法律、金融等多个领域实验中，仅使用5%的GrADS选择数据就能超越全数据集微调性能，使用50%数据时性能显著提升，同时大幅缓解灾难性遗忘。

Conclusion: GrADS方法在LLM领域专业化微调中表现出显著的效率和成本效益，能够用少量数据实现更好性能并减少遗忘问题。
...
Method: 提出CNN架构与BiGRU技术相结合的混合深度学习方案，结合CNN局部特征提取、BiGRU双向序列建模、注意力机制和SHAP可解释性，构建全面的自杀检测框架。

Result: 在公开数据集上的实验结果显示，该方法达到了93.97%的准确率。与最先进的机器学习和深度学习模型及现有文献的比较研究表明，所提技术优于所有竞争方法。

Conclusion: 集成CNN、BiGRU、注意力机制和SHAP可解释性的混合深度学习框架为自杀检测提供了有效的解决方案，在准确性和模型可靠性方面表现出色。
...
Method: 比较了三种模型家族：基于Bio_ClinicalBERT的嵌套实体识别模型、使用QLoRA微调的Meta-Llama-3-8B-Instruct模型，以及在零样本和少样本设置下评估的GPT-4o mini模型。

Result: 微调的Meta-Llama-3-8B-Instruct模型获得最高总体F1分数（微观平均0.74，宏观平均0.59），在上下文依赖类别如思维和社交互动方面表现优异。Bio_ClinicalBERT精度高但召回率低。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语自然语言推理研究资源极度匮乏，现有数据集存在标注错误、语义模糊句子对和语言多样性不足等不一致性问题，阻碍了有效的模型训练和评估。

Method: 通过严格的标注流程构建数据集，强调语义清晰性和蕴含、矛盾和中性类别之间的平衡。使用包括多语言和孟加拉语专用模型在内的最先进transformer架构进行基准测试。

Result: 实验结果表明，使用BNLI数据集在孟加拉语文本复杂语义关系捕获方面实现了改进的可靠性和可解释性。
...
### [62] [Beyond Task-Oriented and Chitchat Dialogues: Proactive and Transition-Aware Conversational Agents](https://arxiv.org/abs/2511.08835)
*Yejin Yoon,Yuri Son,Namyoung So,Minseo Kim,Minsoo Cho,Chanhee Park,Seungshin Lee,Taeuk Kim*

Main category: cs.CL

TL;DR: TACT数据集用于统一任务导向对话和闲聊，支持用户和智能体驱动的模式切换，通过DPO优化在意图检测和模式转换处理上优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统对话系统分别开发任务导向对话和闲聊功能，但真实对话需要在这两种模式间自然切换，现有系统缺乏这种统一能力。

Method: 引入TACT数据集支持模式转换建模，提出Switch和Recovery评估指标，并应用Direct Preference Optimization (DPO)进行模型优化。

Result: TACT训练的模型在意图检测和模式转换处理上优于基线，DPO优化后达到75.74%的联合模式-意图准确率和70.1%的胜率对抗GPT-4o。

Conclusion: 结构多样化的数据结合DPO能提升响应质量和转换控制能力，为更主动和转换感知的对话系统铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种自然语言处理任务中表现出色，但经常产生幻觉内容，这削弱了事实可靠性。为了解决这个问题，需要开发能够检测和修正幻觉的方法。

Method: HalluClean采用推理增强范式，将过程明确分解为规划、执行和修订三个阶段来识别和优化无依据的主张。它使用最小任务路由提示实现零样本跨领域泛化，不依赖外部知识源或监督检测器。

Result: 在五个代表性任务（问答、对话、摘要、数学应用题和矛盾检测）上的广泛评估显示，HalluClean显著提高了事实一致性，并优于竞争基线方法。

Conclusion: HalluClean展示了增强大语言模型输出可信度的潜力，适用于现实世界应用场景。
...
TL;DR: TiDAR是一种序列级混合架构，在单个前向传播中并行进行扩散模型的令牌草拟和自回归模型的最终采样，实现了高吞吐量和AR级别质量的平衡。
...
Method: 使用结构化注意力掩码，在单个前向传播中同时进行扩散模型的令牌草拟（Thinking）和自回归模型的最终采样（Talking）。

Result: 在1.5B和8B规模上，TiDAR在吞吐量和质量上均优于推测解码和扩散变体模型，首次在保持AR模型质量的同时实现4.71-5.91倍的令牌生成速度提升。

Conclusion: TiDAR成功弥合了扩散模型与自回归模型之间的质量差距，同时显著提升了生成效率，为语言模型架构提供了新的设计范式。
...
TL;DR: 本文提出了EVADE框架，利用大语言模型生成和验证解释来检测自然语言推理任务中的标注错误，相比传统的人工两轮标注方法更高效且能提升数据集质量。
...
Method: 提出EVADE框架，使用大语言模型生成和验证解释来检测错误。通过分布比较、验证重叠和微调影响三个维度，全面比较人工与大语言模型检测的错误。

Result: 大语言模型验证能够优化生成的解释分布，使其更接近人工标注。从训练数据中移除大语言模型检测的错误比移除人工标注错误能带来更好的微调性能提升。

Conclusion: 大语言模型有潜力扩展错误检测规模，在减少人工努力的同时提高标签变异下的数据集质量。
...
Method: 引入SpiralThinker框架，对潜在表示进行迭代更新，实现无需生成额外标记的扩展隐式推理。使用渐进对齐目标和结构化注释来维持潜在推理与文本推理之间的一致性。
...
TL;DR: 该论文提出了一个基于轨迹的情感支持评估框架，从静态对话评估转向动态长期评估，通过模拟328个情感情境和1152个干扰事件来评估大语言模型在长期情感支持中的表现。
...
Method: 采用用户中心视角，构建大规模基准测试，约束模型输出使用验证过的情绪调节策略，将用户情感轨迹建模为一阶马尔可夫过程，并应用因果调整的情感估计方法。

Result: 引入了三个轨迹级指标（BEL、ETV、ECP）来捕捉用户情感动态，对多种大语言模型的评估揭示了情感支持能力的显著差异。
...
TL;DR: 本文提出了一种两阶段神经符号框架，使用LLMs将自然语言政策形式化，并通过推理时自动形式化来验证逻辑正确性，在需要高正确性时进行冗余形式化步骤交叉检查。
...
Method: 两阶段神经符号框架：第一阶段使用LLMs（可选人工指导）将自然语言政策形式化；第二阶段使用推理时自动形式化来验证自然语言语句的逻辑正确性，并在需要时进行冗余形式化步骤交叉检查。

Result: 基准测试表明该方法超过99%的健全性，在识别逻辑有效性方面接近零误报率。
...
TL;DR: MM-CRITIC是一个用于评估大型多模态模型批判能力的基准，涵盖基本、纠正和比较三个维度，包含8种主要任务类型和500多个任务，共4471个样本。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在图像描述和视觉推理等任务中能力不断增强，但对其多模态批判能力的研究仍然不足，而批判能力对于模型自我改进和成为可靠的AI助手至关重要。

Method: 构建MM-CRITIC基准，整合专家指导的真实答案到评分标准中，使用GPT-4o标注响应并生成参考批判作为可信判断的锚点。

Result: 广泛的实验验证了MM-CRITIC的有效性，并对领先的大型多模态模型的批判能力进行了全面评估。分析揭示了响应质量与批判之间的相关性，以及不同评估维度下批判难度的变化。

Conclusion: MM-CRITIC为评估大型多模态模型的批判能力提供了一个全面可靠的基准，揭示了模型批判能力的重要见解，有助于推动多模态批判能力的研究和发展。
...
TL;DR: 提出了一种基于混合CTC/Attention架构的安多藏语流式语音识别框架，采用上下文感知的动态分块机制，通过自适应调整分块宽度来改善固定分块方法的上下文截断问题。


<details>
  <summary>Details</summary>
Motivation: 解决安多藏语语音识别中固定分块方法导致的上下文截断问题，并更好地适应藏语的语言学特征。
...
Conclusion: 所提出的框架有效解决了安多藏语流式语音识别中的上下文截断问题，显著提升了识别准确率和效率。
...
TL;DR: Bi-RAR是一个新颖的检索增强推理框架，通过双向评估中间步骤来解决复杂多步推理中RAG方法的局限性，采用基于Kolmogorov复杂度的双向信息距离和多目标强化学习优化推理过程。
...
Method: 提出Bi-RAR框架，双向评估每个中间步骤，引入基于Kolmogorov复杂度的双向信息距离来衡量信息完整性，采用多目标强化学习框架和级联奖励结构优化推理。

Result: 在七个问答基准测试上的实证结果表明，Bi-RAR超越了先前的方法，并在训练和推理过程中实现了与搜索引擎的高效交互和推理。

Conclusion: Bi-RAR通过双向评估和多目标强化学习有效提升了检索增强推理在复杂场景中的性能，为多步推理问题提供了更可靠的解决方案。
...
TL;DR: 本文提出了一种基于GPT-4o的个性化英语阅读理解测试生成方法，通过内容转译将原始阅读材料个性化定制为符合学生兴趣的版本，实验表明该方法能显著提升EFL学习者的阅读理解能力和学习动机。


<details>
  <summary>Details</summary>
Motivation: 在EFL教育中，个性化学习日益重要，特别是阅读理解和学习动机方面。现有阅读材料往往缺乏个性化，无法有效激发学生的学习兴趣和参与度。

Method: 开发结构化内容转译流程，使用GPT-4o从RACE-C数据集生成新的阅读段落和多选题，保持语言特征相似但语义内容与学生兴趣对齐。方法包括主题提取、布鲁姆分类法问题分类、语言特征分析和内容转译。

Result: 在韩国EFL学习者中进行的对照实验显示，使用个性化阅读材料的学生在理解能力和动机保持方面均优于使用非个性化材料的学生。

Conclusion: 兴趣对齐的个性化阅读材料能有效提升EFL学习者的阅读理解表现和学习动机，证明了内容转译方法在个性化英语教育中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: RoPE在Transformer模型中存在长度外推限制，导致注意力下沉现象，影响模型在长序列上的性能表现。

Method: 将带有位置编码的注意力图重新解释为带噪声的特征图，使用截断矩阵熵检测异常频率带，并通过无参数高斯分布重新参数化特征图以实现稳健外推。

Result: 在needle-in-a-haystack和many-shot in-context learning任务中，DoPE在长达64K tokens的扩展上下文中显著提高了检索准确性和推理稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 传统工具学习采用静态合成数据管道，数据生成和模型训练分离，无法自适应关注模型特定弱点，且允许噪声标签持续存在，降低训练效率。
...
Conclusion: 闭环自优化数据管道可以显著增强LLM的工具使用能力，在成本有效的开源生态系统中运行，消除对昂贵闭源API的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在表格问答中直接输入整个表格作为长文本往往会导致错误答案，因为大多数LLM无法固有地捕捉复杂的表格结构。

Method: 采用混合检索机制（语言模型+TF-IDF）计算问题与单个细胞的相似度来估计表头，通过对比学习在小数据集上训练语言模型，在最相关行和列的交集处选择答案细胞。

Result: 在NTCIR-18 U4共享任务的TQA数据集上评估，该方法准确率达到74.6%，优于GPT-4o mini等现有LLM（63.9%）。

Conclusion: 该方法在表格问答任务中表现优于现有LLM，未来计划整合更高效的文本搜索模型以进一步提高性能并缩小与人工评估结果的差距。
...
TL;DR: 本文实证验证了基于上下文的sequentiality度量方法在自动作文评分中的有效性，相比原始版本和仅基于主题的版本，该度量与人类对组织性和连贯性的评估更一致，且在与标准语言特征结合时优于零样本LLM预测。


<details>
  <summary>Details</summary>
Motivation: 针对之前研究中sequentiality度量方法在主题选择方面存在混淆问题且缺乏真实流度验证的批评，本文旨在实证验证仅使用上下文术语作为更概念有效和可解释的替代方案。

Method: 使用两个带有人工标注特征分数的作文数据集（ASAP++和ELLIPSE），比较不同版本的sequentiality度量与人类对语篇层面特征（如组织性和连贯性）评估的一致性，并分析其与标准语言特征结合时的预测性能。

Result: 基于上下文的sequentiality版本与人类对组织性和连贯性的评估更一致；虽然零样本提示的LLM在预测特征分数方面更准确，但上下文度量在与标准语言特征结合时比仅基于主题和原始sequentiality公式增加了更多预测价值；这种组合也优于零样本LLM预测。
...
TL;DR: 本文研究了子词分词在学习过程中的动态变化，探索了语言模型如何动态优化分词策略，以及子词在预训练和微调过程中的演化规律。


<details>
  <summary>Details</summary>
Motivation: 传统子词分词通常在预处理阶段固定，而本文旨在研究动态学习子词分词的可能性，探索语言模型如何通过训练目标优化分词策略，特别是在不同形态复杂度的语言中的学习动态。

Method: 扩展子词分段语言模型(SSLM)框架以支持预训练和微调，针对三种形态类型不同的语言(聚合型的isi-Xhosa、分离型的Setswana和中间型的英语)进行训练，从语言学角度分析子词动态变化。

Result: 识别出子词学习的四个阶段，形态复杂的isi-Xhosa表现出更大的不稳定性；在微调过程中，子词边界变得更细粒度；可学习子词为低资源、形态复杂语言的文本生成和跨语言迁移提供了有前景的方法。

Conclusion: 动态学习子词分词能够优化语言模型的性能，特别是在形态复杂的低资源语言中，子词边界在微调过程中会自适应调整，为改进文本生成和跨语言迁移提供了有效途径。
...
TL;DR: 该论文报告了在六种不同规模下预训练的ModernBERT编码器模型，参数量从5100万到4.75亿，重点关注与芬兰相关的多语言能力。
...
Method: 采用ModernBERT编码器架构，在不同规模下进行预训练，并在训练最后阶段使用不同数据进行实验。

Result: 模型在性能上与现有多语言模型相当或更优，在超过512个标记的长文本任务上优于单语言模型。
...
TL;DR: 本文提出了Anchor方法，一种在强化学习中注入真实轨迹的稳定训练技术，用于解决语言模型在可验证奖励强化学习中的早期训练崩溃问题，特别针对诚实对齐和演绎推理任务。


<details>
  <summary>Details</summary>
Motivation: 现有可验证奖励强化学习方法主要优化最终任务结果，在早期训练中负奖励占主导时容易导致模型崩溃，这在诚实对齐任务中尤为突出，模型需要既能回答可解问题，又能识别无法从前提得出结论的情况。

Method: 作者构建了两个基于图结构的多步演绎推理数据集（线性代数和逻辑推理），并在半数实例中通过随机扰动边来引入不可回答的情况。提出了Anchor方法，在rollout过程中注入真实轨迹以防止早期训练崩溃。

Result: 实验表明GRPO方法（无论是否有监督微调初始化）在这些任务上表现不佳。课程学习有一定帮助但需要精心设计的数据集。Anchor方法显著稳定了学习过程，提高了整体推理性能。

Conclusion: 训练动态对于在对齐语言模型中实现可靠的演绎推理至关重要，Anchor方法通过注入真实轨迹有效解决了早期训练崩溃问题，提升了推理性能。
...
TL;DR: POTSA框架通过交叉语言平行语音对和最优传输技术，解决语音大语言模型在多语言语音到文本翻译中的语义偏差问题，提升高低资源语言间的翻译性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型在多语言语音到文本翻译中忽视了跨源语言的语义共性，导致翻译性能存在偏差，特别是高低资源语言间的翻译差距问题。

Method: 提出POTSA框架：1）偏差补偿模块粗对齐跨语言语音表示；2）在Q-Former上施加令牌级最优传输约束建立细粒度表示一致性；3）采用层调度策略将OT约束聚焦于最具语义效益的层。

Result: 在FLEURS数据集上，该方法仅使用每种源语言10小时的平行语音数据，就在五种常见语言上平均提升+0.93 BLEU，在零样本语言上提升+5.05 BLEU，达到SOTA性能。

Conclusion: POTSA框架通过最优传输技术有效解决了跨语言语音表示的语义对齐问题，显著提升了高低资源语言间的翻译性能，为多语言语音翻译提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在控制生成文本的特定属性时存在局限，难以实现精确的多属性控制，特别是在属性要求冲突时缺乏协调机制，且无法在控制生成流程中融入迭代优化过程。

Method: 提出C³TG两阶段框架：生成阶段通过加权KL散度调整token概率，优化阶段使用结合分类器分数和惩罚项的能量函数通过迭代反馈解决属性冲突。

Result: 实验表明C³TG在多个指标上显著优于基线方法，包括属性准确性、语言流畅性和输出多样性，同时降低了毒性。

Conclusion: C³TG是一种有效且灵活的多维文本属性控制解决方案，无需昂贵的模型修改，能够同时精确控制多个维度并保持自然文本流。
...
Method: 引入LiteraryTaste数据集，包含60人的阅读偏好数据，包括自述偏好和100对短创意写作文本的偏好标注；使用transformer编码器进行微调建模偏好；采用LLM驱动的可解释性管道分析偏好变化。

Result: 1) 人们在创意写作偏好上存在差异；2) 微调transformer编码器在建模个人和集体实际偏好时分别达到75.8%和67.7%的准确率；3) 自述偏好对建模实际偏好的效用有限。

Conclusion: 该工作为个性化创意写作技术奠定了基础，展示了建模个人偏好的可行性，但需要超越简单的自述偏好来更好地理解用户的实际偏好。
...
TL;DR: 本文提出了一种可解释的高棉语极性分类器，通过微调基于指令的推理模型Qwen-3来实现。该方法不仅能准确预测情感极性标签，还能提供解释说明其预测理由。


<details>
  <summary>Details</summary>
Motivation: 现有高棉语模型通常只预测标签而不解释预测依据，缺乏可解释性。本文旨在开发一个能够自我解释预测理由的高棉语极性分类器。

Method: 通过微调基于指令的推理模型Qwen-3，使其在预测极性标签的同时提供解释。使用启发式规则和人工筛选构建了新的高棉语极性数据集。

Result: 实验结果显示，微调后的模型不仅能准确预测标签，还能通过识别极性相关关键词或短语来支持其预测，提供了有效的自我解释。

Conclusion: 成功开发了可解释的高棉语极性分类器，并贡献了新的高棉语极性数据集和微调模型，为高棉语自然语言处理提供了有价值的资源。
...
TL;DR: 本文提出了AdaptDel方法，通过可调节的删除率来改进序列分类的认证鲁棒性，针对编辑距离扰动提供更强的理论保证和实证效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理不同长度序列时使用固定删除率，导致性能不佳，需要开发能够根据输入特性动态调整的认证方法。

Method: 引入AdaptDel方法，采用可调节删除率机制，并将随机平滑理论框架扩展到变速率删除场景，确保对编辑距离的可靠认证。

Result: 在自然语言处理任务中取得了显著的实证结果，认证区域的中值基数比现有最佳方法提高了多达30个数量级。
...
TL;DR: 提出了mmJEE-Eval多模态双语基准，基于印度JEE Advanced考试题目，用于评估视觉语言模型的真实科学推理能力，发现前沿模型与开源模型之间存在显著性能差距。
...
Method: 构建包含1,460个英语和印地语JEE Advanced考试题目的多模态基准，涵盖物理、化学和数学领域，评估17个最先进模型。

Result: 前沿VLMs在2025年题目上达到77-84%准确率，开源模型仅为37-45%，当增加元认知推理负载时，GPT-5只能修复5.2%的错误。

Conclusion: mmJEE-Eval能够有效区分不同训练和推理方法的质量，其难度源于复杂性和推理深度而非记忆效应。
...
TL;DR: SeerSC是一个动态自一致性框架，通过整合系统1和系统2推理，同时提高令牌效率和降低延迟。系统1快速计算答案熵来评估样本的扩展潜力，系统2进行动态自一致性处理，实现并行生成以减少延迟。
...
Method: 利用快速系统1计算查询的答案熵，评估样本扩展潜力，在系统2下进行动态自一致性处理，实现并行生成。

Result: 与现有方法相比，令牌消耗减少高达47%，推理延迟降低43%，且无显著性能损失。

Conclusion: SeerSC框架通过系统1和系统2的协同工作，在保持性能的同时显著提高了令牌效率和推理速度。
...
TL;DR: 提出了Spider4SSC数据集和S2CLite解析工具，S2CLite是一个轻量级的、与本体无关的解析器，可将SPARQL查询转换为Cypher查询，在实验中显著降低了查询解析错误，解析准确率达到77.8%，执行准确率达到96.6%。


<details>
  <summary>Details</summary>
Motivation: 现有SPARQL到Cypher的转换解决方案存在局限性，需要RDF图或外部工具支持。开发一个轻量级、纯基于规则的解析器来解决这些问题。

Method: S2CLite是一个纯基于规则的解析器，灵感来自传统编程语言编译器，无需RDF图或外部工具即可操作。使用BSBM42和Spider4SPARQL数据集进行实验验证。

Result: 在Spider4SPARQL数据集上，S2CLite的解析准确率达到77.8%，相比最先进的S2CTrans的44.2%有显著提升。在执行准确率方面，S2CLite达到96.6%，比S2CTrans高出7.3%。

Conclusion: S2CLite在SPARQL到Cypher转换方面表现出色，显著优于现有技术。同时生成了Spider4SSC数据集，包含4525个独特问题和3组等效查询（SQL、SPARQL和Cypher），为多语言查询研究提供了统一基准。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在任务特定评估中表现出色，但它们在多语言环境下对文本质量进行通用评估的能力仍不确定，需要开发专门的多语言文本质量评估方法。

Method: 首先自动生成文本质量偏好数据，然后使用这些数据训练开源基础LLMs，使其与高质量和低质量文本的评分对齐，从而开发出MTQ-Eval框架。

Result: 在115种语言上的全面评估表明，所提出的模型性能得到显著改进，这种增强的评估能力还能显著提升下游任务的性能。

Conclusion: MTQ-Eval框架在多语言文本质量评估方面表现出色，不仅能有效评估文本质量，还能促进下游任务的改进，为多语言文本评估提供了有效的解决方案。
...
TL;DR: 本文系统比较了大型语言模型在开放式文本生成和多项选择题选择两种范式下的自我修正机制，揭示了它们在性能提升和错误修正行为上的不同模式。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型自我修正机制在不同任务结构（开放式生成vs多项选择）中的动态差异，以理解任务结构与输出空间之间的相互作用对自我修正效果的影响。

Method: 通过系统实验，在不同自然语言理解和推理任务上比较两种范式的性能趋势和错误修正行为，涵盖不同规模和家族的多个语言模型。

Result: 开放式生成受益于重新解释和组合性修正的灵活性，而多项选择可以利用更清晰的解决方案边界但受限于提供的选项。实验揭示了不同的改进模式和失败模式。

Conclusion: 自我修正机制的设计应考虑任务结构与输出空间之间的相互作用，这对知识密集型推理和决策导向的LLM应用具有重要意义。
...
TL;DR: 本文提出了AMaPO算法来解决离线偏好优化中的过拟合-欠拟合困境，通过自适应边距设计动态重新分配学习努力，在多个基准测试中实现了更好的排序准确性和下游对齐性能。


<details>
  <summary>Details</summary>
Motivation: 离线偏好优化虽然比RLHF更简单稳定，但其效果严重依赖排序准确性。当前方法存在过拟合-欠拟合困境：对正确排序样本施加过多梯度（过拟合），对错误排序样本提供不足的纠正信号（欠拟合）。

Method: 提出AMaPO算法，采用实例级自适应边距，通过Z归一化和指数缩放进行精炼，动态重新分配学习努力：放大错误排序样本的梯度，抑制正确排序样本的梯度。

Result: 在广泛使用的基准测试中，AMaPO不仅实现了更好的排序准确性和优越的下游对齐性能，针对性分析也确认其成功缓解了核心的过拟合和欠拟合问题。
...
TL;DR: 本文开发了一个针对低资源语言巴斯克语的多模态大语言模型，通过创建训练和评估数据集，探索不同数据混合比例，发现仅需20%巴斯克语多模态数据即可获得良好效果，且无需巴斯克语指令调优的骨干LLM。


<details>
  <summary>Details</summary>
Motivation: 当前商业多模态大语言模型在低资源语言上表现尚可，但开源社区缺乏可比较的成果。本文旨在为低资源语言巴斯克语开发强大的多模态大语言模型。

Method: 创建了训练和评估用的图像-文本数据集，使用Llama-3.1-Instruct和巴斯克语适配的Latxa作为骨干LLM，探索不同的数据混合训练策略。

Result: 仅需约20%的巴斯克语多模态数据即可在巴斯克语基准测试中获得稳定结果；出乎意料的是，使用巴斯克语指令调优的骨干LLM并非构建强大巴斯克语MLLM的必要条件。

Conclusion: 研究成果为其他低资源语言开发多模态大语言模型开辟了道路，并公开释放了相关资源。
...
TL;DR: CARE-Bench是一个动态交互式自动化基准测试，用于评估大语言模型在心理咨询领域的专业能力，基于真实案例构建多样化客户档案，并提供多维度的专业评估。


<details>
  <summary>Details</summary>
Motivation: 心理咨询需求增长与有限服务资源之间的不匹配，以及现有评估方法在客户模拟专业性、评估格式和指标维度方面的局限性，促使开发更全面的模型评估基准。

Method: 基于真实心理咨询案例构建多样化客户档案，按照专家指南进行模拟，建立动态交互式评估框架，并基于成熟心理学量表提供多维度性能评估。

Result: 评估了多个通用大语言模型和专业心理咨询模型，揭示了它们当前的局限性，并与心理学家合作分析了模型在不同类型客户交互中失败的原因。
...
TL;DR: GSAP-ERE是一个用于机器学习研究信息提取的细粒度数据集，包含10种实体类型和18种关系类型，从100篇ML论文全文标注了63K实体和35K关系。该数据集支持下游任务如知识图谱构建和AI研究可复现性监控，并显示微调模型在信息提取任务上显著优于大型语言模型的提示方法。


<details>
  <summary>Details</summary>
Motivation: 机器学习研究发展迅速，从科学出版物中提取信息能够大规模识别研究概念和资源，有助于提高对ML相关研究的理解和可复现性。需要细粒度信息提取来捕捉方法训练和数据使用等详细信息。

Method: 引入GSAP-ERE数据集，这是一个手动标注的细粒度数据集，包含10种实体类型和18种语义分类的关系类型。使用该数据集训练微调模型，并与大型语言模型的提示策略进行对比实验。

Result: 数据集支持微调模型在命名实体识别（NER）达到80.6%，关系提取（RE）达到54.0%的性能，而大型语言模型提示方法仅达到NER 44.4%和RE 10.1%。该数据集可用于知识图谱构建和AI研究可复现性监控。
...
<details>
  <summary>Details</summary>
Motivation: 从大量生成文本中预测个体人格是一项具有挑战性的任务，特别是当文本量很大时，需要解决输入文本限制并提高预测准确性。
...
Conclusion: TPoT是一种简单而有效的策略，能够通过语义文本选择有效提升人格预测模型的性能。
...
### [95] [Tracing AG Codes: Toward Meeting the Gilbert-Varshamov Bound](https://arxiv.org/abs/2511.08788)
*Gil Cohen,Dean Doron,Noam Goldgraber,Tomer Manket*

Main category: cs.IT

TL;DR: 该论文通过追踪代数几何码来尝试匹配Gilbert-Varshamov界，提出了适用于分析追踪代数几何码的Hasse-Weil型定理，并分析了这种方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 解决编码理论中最古老的问题之一：用显式二进制码匹配Gilbert-Varshamov界。利用代数几何码在较大常数域上超越GV界的现象，通过追踪操作将其参数损失降到二进制域。

Method: 采用追踪代数几何码的方法，利用代数几何码的代数结构进行字母表缩减，而不是传统的级联方法。提出了新的Hasse-Weil型定理来分析追踪代数几何码。

Result: 虽然未获得改进的构造，但证明了常数因子加强的界就足够了。在高距离机制下，追踪代数几何码劣于码级联。新的Hasse-Weil型定理具有更广泛的适用性，导出了指数和的新估计。

Conclusion: 追踪代数几何码方法在分析上具有优势，但在高距离机制下不如传统级联方法。新的Hasse-Weil型定理为更一般的设置提供了分析工具。
...
### [96] [Learning Binary Autoencoder-Based Codes with Progressive Training](https://arxiv.org/abs/2511.09221)
...
Result: 在(7,4)块配置和二进制对称信道下，学习到的编码器-解码器对学习到了最优汉明码的旋转版本（陪集码），自然恢复了其线性和距离特性，实现了与最大似然解码相同的块错误率。
...
### [97] [Generic Construction of Optimal-Access Binary MDS Array Codes with Smaller Sub-packetization](https://arxiv.org/abs/2511.09251)
*Lan Ma,Qifu Tyler Sun,Shaoteng Liu,Liyang Zhou*

Main category: cs.IT

TL;DR: 本文提出了两种通用构造方法，用于构建具有最优修复带宽的二进制MDS阵列码。第一种方法构造的码具有最优访问带宽，第二种方法构造的码具有最优修复带宽且子分组化程度最小。


<details>
  <summary>Details</summary>
Motivation: 分布式存储系统中二进制MDS阵列码的单节点故障修复问题引起了广泛关注，现有方法在子分组化程度和修复带宽方面存在改进空间。

Method: 提出了两种通用构造方法：通用构造I基于任意二进制MDS阵列码构造具有最优访问带宽的码；通用构造II在特定条件下构造具有最优修复带宽且子分组化程度最小的码。

Result: 构造了两种新型二进制MDS阵列码：C1码具有最优访问带宽和更小的子分组化程度；C2码具有最优修复带宽，且是目前已知子分组化程度最小的二进制MDS阵列码。

Conclusion: 提出的两种通用构造方法能够有效构建具有最优修复特性的二进制MDS阵列码，在子分组化程度和修复带宽方面优于现有方法。
...
### [98] [Enabling Smart Radio Environments in the Frequency Domain With Movable Signals](https://arxiv.org/abs/2511.09384)
*Matteo Nerini,Bruno Clerckx*

Main category: cs.IT

TL;DR: 提出了一种新的智能无线电环境实现方法——可移动信号，通过动态调整信号频谱在频域中的位置来增强无线通信性能，避免了传统可重构智能表面的电子可重构组件带来的实现挑战。


<details>
  <summary>Details</summary>
Motivation: 传统智能无线电环境技术（如可重构智能表面和柔性天线）依赖电子可重构或可移动组件，存在实现挑战，可能阻碍商业化。需要寻找新的实现域来克服这些挑战。

Method: 提出频域中的可移动信号概念，通过动态移动信号频谱来实现智能无线电环境。分析了在视距和非视距条件下可移动信号在多输入单输出系统中的性能。

Result: 在视距条件下，可移动信号比量化等增益传输获得更高的平均接收功率。在非视距条件下，通过利用固定智能表面的反射，可移动信号系统比使用固定频率信号的可重构智能表面系统获得高达四倍的接收功率。

Conclusion: 频域中的可移动信号为智能无线电环境提供了一种有前景的新实现方法，能够克服传统技术的实现挑战，并在性能上优于现有方法。
...
### [99] [Computability of the Optimizer for Rate Distortion Functions](https://arxiv.org/abs/2511.09412)
*Jonathan E. W. Huffmann,Holger Boche*

Main category: cs.IT

TL;DR: 本文研究了率失真理论中优化测试信道的可计算性问题，发现虽然率失真函数通常可计算，但优化器在一般情况下不可计算，即使对于简单的失真度量也是如此。


<details>
  <summary>Details</summary>
Motivation: 研究率失真函数中优化测试信道的可计算性，因为该优化器在通信和信息理论中具有理论和实践重要性。

Method: 通过分析率失真理论中的优化问题，研究优化测试信道的可计算性，并与信息理论中其他问题的已知结果进行比较。

Result: 发现率失真函数通常可计算，但优化器在一般情况下不可计算，即使对于简单的失真度量。

Conclusion: 率失真函数的优化器在一般情况下是非可计算的，这与信息理论中其他问题的已知结果相似。
...
### [100] [Bridging Natural Language and ASP: A Hybrid Approach Using LLMs and AMR Parsing](https://arxiv.org/abs/2511.08715)
*Connar Hite,Sean Saud,Raef Taha,Nayim Rahman,Tanvir Atahary,Scott Douglass,Tarek Taha*

Main category: cs.AI

TL;DR: 提出了一种使用LLM和AMR图将无约束英语翻译为ASP程序的新方法，用于解决逻辑谜题，生成完整的ASP程序来求解组合逻辑问题。


<details>
  <summary>Details</summary>
Motivation: ASP是一种强大的组合问题求解工具，但需要学习语法。随着非编程人员与代码交互的需求增加，需要更易用的自然语言到ASP的转换方法。

Method: 使用LLM简化自然语言句子、识别关键词和生成简单事实，然后通过AMR图解析简化语言并系统生成ASP约束，最小化LLM的作用。

Result: 系统成功创建了完整的ASP程序来解决组合逻辑问题，展示了在逻辑谜题示例上的能力。
...
### [101] [Vector Symbolic Algebras for the Abstraction and Reasoning Corpus](https://arxiv.org/abs/2511.08747)
*Isaac Joffe,Chris Eliasmith*

Main category: cs.AI

TL;DR: 本文提出了一种基于向量符号代数（VSA）的认知合理ARC-AGI求解器，通过神经符号方法整合系统1直觉和系统2推理，在对象中心程序合成中实现高效可解释的求解过程。


<details>
  <summary>Details</summary>
Motivation: 尽管人类能轻松解决ARC-AGI问题，但即使最先进的人工智能系统也难以应对。受从神经科学到心理学的人类智能建模方法启发，作者希望开发一个认知合理的ARC-AGI求解器。

Method: 使用基于向量符号代数（VSA）的神经符号方法，整合系统1直觉和系统2推理，通过对象中心程序合成，利用VSA表示抽象对象、指导解决方案搜索并实现样本高效的神经学习。

Result: 在ARC-AGI-1-Train上得分10.8%，在ARC-AGI-1-Eval上得分3.0%。在更简单的基准测试中表现优异：Sort-of-ARC得分94.5%，1D-ARC得分83.1%（以极小的计算成本优于GPT-4）。

Conclusion: 这是首个将VSA应用于ARC-AGI的方法，开发了迄今为止最认知合理的ARC-AGI求解器，在保持计算效率的同时实现了有前景的性能。
...
### [102] [UCO: A Multi-Turn Interactive Reinforcement Learning Method for Adaptive Teaching with Large Language Models](https://arxiv.org/abs/2511.08873)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在教育场景中从答案提供者转向智能导师，但现有监督微调方法仅学习表面教学模式，缺乏动态适应能力。强化学习方法存在两个关键挑战：无法区分学生是否真正理解还是简单重复答案，以及无法实时感知学生认知状态变化。

Method: UCO采用多轮交互式强化学习范式，包含两个协同奖励函数：进展奖励捕捉学生认知进步，评估学生是否真正从困惑转向理解；支架奖励动态识别每个学生的最近发展区，鼓励教师在该区域内保持高效教学。

Result: 在BigMath和MathTutorBench基准测试中，UCO模型优于所有同等规模的基线模型，并达到与先进闭源模型相当的性能。

Conclusion: UCO方法通过创新的奖励机制有效解决了LLM作为智能导师时的动态适应问题，在教育应用中展现出优越性能。
...
### [103] [Lumine: An Open Recipe for Building Generalist Agents in 3D Open Worlds](https://arxiv.org/abs/2511.08892)
...
TL;DR: Lumine是首个能够在3D开放世界中实时完成数小时复杂任务的通用智能体配方，采用端到端视觉语言模型统一感知、推理和行动，在《原神》中训练后能完成5小时主线剧情，并在零样本下泛化到其他游戏。


<details>
  <summary>Details</summary>
Motivation: 开发能够在复杂3D开放世界环境中完成长时间任务的通用智能体，解决现有方法在处理实时、长时程任务方面的局限性。

Method: 采用人类交互范式，通过视觉语言模型以5Hz处理原始像素，生成30Hz的键盘鼠标动作，仅在必要时进行推理，在《原神》中进行训练。

Result: 成功完成《原神》5小时蒙德主线剧情，效率达到人类水平；零样本下在《鸣潮》完成100分钟任务，在《崩坏：星穹铁道》完成5小时第一章。

Conclusion: Lumine在不同世界和交互动态中表现出强大效果，标志着在开放环境中开发通用智能体的具体进展。
...
### [104] [A Research on Business Process Optimisation Model Integrating AI and Big Data Analytics](https://arxiv.org/abs/2511.08934)
*Di Liao,Ruijia Liang,Ziyi Ye*

Main category: cs.AI

TL;DR: 本研究构建了一个融合人工智能与大数据的业务流程优化模型，采用三层架构实现全生命周期智能管理，实验验证显示处理时间缩短42%，资源利用率提升28%，运营成本降低35%。


<details>
  <summary>Details</summary>
Motivation: 随着数字化转型的深入，业务流程优化成为提升企业竞争力的关键，需要构建智能化的全生命周期流程管理解决方案。

Method: 采用包含数据处理、AI算法和业务逻辑的三层架构模型，结合分布式计算和深度学习技术，实现实时流程监控与优化。

Result: 实验验证显示模型缩短流程处理时间42%，提高资源利用率28%，降低运营成本35%，高并发负载下保持99.9%可用性。
...
### [105] [AlphaCast: A Human Wisdom-LLM Intelligence Co-Reasoning Framework for Interactive Time Series Forecasting](https://arxiv.org/abs/2511.08947)
*Xiaohan Zhang,Tian Gao,Mingyue Cheng,Bokai Pan,Ze Guo,Yaguo Liu,Xiaoyu Tao*

Main category: cs.AI

TL;DR: AlphaCast是一个人类智慧与大型语言模型智能协同推理的框架，将时间序列预测重新定义为交互式过程，通过分阶段协作提升预测准确性。
...
### [106] [AI Founding Fathers: A Case Study of GIS Search in Multi-Agent Pipelines](https://arxiv.org/abs/2511.09005)
*Alvin Chauhan*

Main category: cs.AI

TL;DR: 论文提出通过构建多智能体管道来实现渐进式、增量式和顺序式（GIS）搜索，从而增强大语言模型的推理能力，并通过递归精炼方法验证了这一框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型表现出优异的流畅性，但研究者仍在努力提升其推理能力。本文基于搜索导向的LLM计算解释，旨在系统性地理解和优化LLM推理过程。
...
Conclusion: 递归精炼是通过GIS搜索增强LLM推理能力的强大架构特征，证明了结构化多智能体管道在提升推理质量方面的有效性。
...
### [107] [Argus: Resilience-Oriented Safety Assurance Framework for End-to-End ADSs](https://arxiv.org/abs/2511.09032)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在公共道路上部署时会面临各种驾驶危险，需要具备持续监控危险和自适应响应的韧性能力，以在复杂场景中保持稳健驾驶行为。

Method: Argus框架持续监控ADS生成的轨迹，当EGO车辆被认为不安全时，通过危险缓解器无缝接管控制。该框架与TCP、UniAD和VAD三种先进的端到端ADS集成。

Result: 评估显示Argus有效提升了ADS的韧性，平均驾驶分数提高了150.30%，防止了64.38%的违规行为，且额外时间开销很小。

Conclusion: Argus框架能够有效增强端到端自动驾驶系统的韧性，显著提高驾驶性能并减少安全违规，具有实际应用价值。
...
### [108] [Advancing Autonomous Emergency Response Systems: A Generative AI Perspective](https://arxiv.org/abs/2511.09044)
*Yousef Emami,Radha Reddy,Azadeh Pourkabirian,Miguel Gutierrez Gaitan*

Main category: cs.AI

TL;DR: 本文综述了下一代自动驾驶车辆在应急服务中的优化策略，重点分析了从传统强化学习向扩散模型增强强化学习和大型语言模型辅助上下文学习的转变，从生成式AI视角为自主应急响应系统提供了框架。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆有望通过更快、更安全、更高效的响应来革新应急服务，但传统强化学习方法在动态应急场景中存在样本效率低和适应性差的问题，需要新的优化策略来解决这些局限性。

Method: 分析了从传统强化学习向扩散模型增强强化学习的转变，通过合成数据生成增强策略鲁棒性；同时探索了大型语言模型辅助上下文学习的新范式，实现无需重新训练的快速在线适应。

Result: 扩散模型增强强化学习提高了策略鲁棒性但增加了计算成本，而大型语言模型辅助上下文学习提供了轻量级、可解释的替代方案，支持快速在线适应。

Conclusion: 本文从生成式AI角度为理解下一代自主应急响应系统提供了关键框架，展示了扩散模型增强强化学习和大型语言模型辅助上下文学习在提升自动驾驶车辆应急响应能力方面的潜力。
...
### [109] [OR-R1: Automating Modeling and Solving of Operations Research Optimization Problem via Test-Time Reinforcement Learning](https://arxiv.org/abs/2511.09092)
*Zezhen Ding,Zhen Tan,Jiheng Zhang,Tianlong Chen*

Main category: cs.AI

TL;DR: OR-R1是一个数据高效的自动化优化建模与求解框架，通过监督微调和测试时组相对策略优化的两阶段设计，仅需1/10的合成数据即可实现67.7%的平均求解准确率，超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的优化建模方法需要大量标注或合成数据，导致成本高且可扩展性受限，需要开发数据效率更高的自动化解决方案。

Method: 采用两阶段训练框架：首先通过监督微调从有限标注数据中学习问题建模和代码生成的推理模式，然后通过测试时组相对策略优化提升能力和一致性。

Result: OR-R1仅需100个合成样本就超越ORLM 2.4%，平均求解准确率达67.7%，比ORLM高出4.2%。TGRPO带来额外3.1%-6.4%的准确率提升，将单次与多次尝试性能差距从13%缩小到7%。
...
### [110] [Efficient Reasoning via Reward Model](https://arxiv.org/abs/2511.09158)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型（如DeepSeek-R1和OpenAI o1）经常产生冗长的响应，包含冗余或不相关的推理步骤（过度思考现象），这会显著增加计算成本。现有的长度惩罚方法存在长度崩溃和训练崩溃问题。

Method: 提出了训练简洁性奖励模型（CRM）的管道，用于对推理路径的简洁性进行评分；引入了一种新颖的奖励公式——简洁性奖励函数（CRF），在结果奖励和简洁性评分之间建立显式依赖关系。

Result: 在五个数学基准数据集上的广泛实验表明，该方法在Qwen2.5-7B上实现了8.1%的准确率提升和19.9%的响应token长度减少，并且在Llama和Mistral等其他LLM上具有良好的泛化能力。

Conclusion: 该方法通过理论分析和实验验证，证明了在促进更有效和更高效推理方面的优越性，解决了现有长度惩罚方法中的关键问题。
...
### [111] [Perspectives on a Reliability Monitoring Framework for Agentic AI Systems](https://arxiv.org/abs/2511.09178)
*Niclas Flehmig,Mary Ann Lundteigen,Shen Yin*

Main category: cs.AI

TL;DR: 本文提出了一个双层可靠性监控框架，用于提高自主AI系统的可靠性，通过异常检测和透明度层为人类操作员提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统在医疗和流程工业等高风险领域应用时存在可靠性不足的问题，需要开发缓解技术来降低操作期间不确定可靠性带来的风险。

Method: 提出了一个双层可靠性监控框架，包括用于检测新输入的分布外检测层和揭示内部操作的AI透明度层。

Result: 该框架为人类操作员提供了必要的决策支持，以判断输出是否可能不可靠并进行干预。

Conclusion: 该监控框架为开发缓解技术提供了基础，能够减少自主AI系统在操作期间因可靠性不确定而产生的风险。
...
TL;DR: MedFuse是一个用于处理不规则临床时间序列的框架，通过MuFuse模块使用乘法融合技术来建模特征间的高阶依赖关系，在多个真实世界数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的临床时间序列存在不规则性、异步采样、缺失值和异质特征动态等问题，现有嵌入策略通常使用加法操作结合特征身份和值嵌入，限制了捕捉值依赖特征交互的能力。

Method: 提出MedFuse框架，核心是MuFuse模块，通过乘法调制融合值和特征嵌入，在保留特征特定信息的同时建模跨特征的高阶依赖关系。

Result: 在三个涵盖重症和慢性护理的真实世界数据集上，MedFuse在关键预测任务上始终优于最先进的基线方法，学习到的表示显示乘法融合增强了表达能力并支持跨数据集预训练。

Conclusion: MedFuse为建模不规则临床时间序列提供了一个可泛化的方法，乘法融合技术显著提升了模型性能。
...
TL;DR: 提出HyperD框架，通过解耦交通数据的周期性和残差分量来改进交通预测，结合混合周期表示和频率感知残差建模，在四个真实数据集上实现最先进的预测精度。


<details>
  <summary>Details</summary>
Motivation: 交通预测面临复杂空间依赖性和多尺度周期模式与不规则波动共存的挑战，现有方法难以有效处理这些复杂特征。

Method: HyperD框架将交通数据解耦为周期性和残差分量，分别使用混合周期表示模块（学习周期嵌入和时空注意力）和频率感知残差表示模块（频域复数MLP），并引入双视图对齐损失确保语义分离。

Result: 在四个真实交通数据集上的广泛实验表明，HyperD实现了最先进的预测精度，在干扰下具有更好的鲁棒性和更高的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI训练方法在模型核心能力建立后才进行价值对齐，导致模型容易被误导且缺乏深层次价值体系。在大语言模型能力开始超越人类的背景下，需要从根本上解决价值对齐问题。

Method: 重新设计训练语料库：采用第一人称视角重构训练数据、将信息重新情境化为生活经验、模拟社会互动、以及搭建训练数据的顺序支架。

Result: 预期这种训练语料库的重新设计将使模型从第一个训练标记开始就承诺价值观，使得知识、技能和价值观在本质上更难分离。

Conclusion: 在大语言模型能力开始超越人类能力的生态系统中，这种从模型训练转向模型培养的范式转变是至关重要的需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI主要推动了定量方法的发展，而定性研究领域被忽视。研究人员被迫使用通用AI工具如ChatGPT进行定性分析，但这些工具存在偏见、不透明、不可重复和隐私泄露等问题，导致定性维度在科学理解中未能充分整合。
...
Conclusion: 开发专门的定性AI系统对于促进多学科和混合方法研究至关重要，能够将定性维度有效整合到科学发现过程中，实现更全面的科学理解。
...
TL;DR: 评估2025年前沿LLMs在PDDL规划任务中的表现，GPT-5在标准PDDL域中与LAMA规划器竞争，但在混淆测试中所有模型性能下降。


<details>
  <summary>Details</summary>
Motivation: 评估前沿大语言模型在端到端规划任务中的推理能力，并与传统规划器进行比较。

Method: 使用国际规划竞赛学习轨道的PDDL域子集，测试DeepSeek R1、Gemini 2.5 Pro、GPT-5和LAMA规划器，包括标准PDDL和混淆PDDL两种测试条件。

Result: GPT-5在标准PDDL域中与LAMA规划器表现相当，所有LLMs在混淆测试中性能下降但程度较之前模型轻。

Conclusion: 前沿LLMs在规划任务上相比前代模型有显著改进，缩小了与传统规划器在挑战性基准上的性能差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要关注高层次描述或视觉问答，缺乏对实际执行能力的评估。CrochetBench旨在填补这一空白，强调从描述到实际操作的转变，评估模型在真实创造性领域中的程序能力。

Method: 采用CrochetPARADE DSL作为中间表示，支持结构化验证和通过执行的功能评估。基准测试涵盖针法分类、指令接地、自然语言到DSL翻译和图像到DSL翻译等任务。
...
TL;DR: 提出了一种基于多模型聚合的AI安全方法，通过共识采样算法从k个模型中选择最安全的s个子集，在模型间缺乏足够共识时选择弃权，从而增强安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的AI安全方法主要依赖检查模型输出或激活，但某些风险无法仅通过检查发现。需要一种架构无关的补充方法来增强安全性。

Method: 使用共识采样算法，利用模型计算输出概率的能力，当足够多的模型安全且达成共识时，聚合模型继承最安全子集的安全性，否则弃权。

Result: 该方法能够实现与最安全s个模型平均风险相当的安全性，并在模型间缺乏共识时有效弃权。

Conclusion: 提供了一种新的模型无关AI安全方法，通过放大集合中未知安全子集的安全保证来创建可靠的单一模型。
...
TL;DR: 本文从科学和系统角度阐述了物理人工智能（Physical AI）的基本原理，旨在为智能系统的物理体现、感知、行动、学习和情境敏感性创建统一的理论框架，将智能视为身体、环境和经验之间真实交互的涌现现象。


<details>
  <summary>Details</summary>
Motivation: 传统AI方法依赖符号处理和数据驱动模型，而Physical AI将智能理解为身体、环境和经验之间真实交互的涌现现象，需要建立描述物理体现、感知、行动、学习和情境敏感性的统一理论框架。

Method: 提出了六个基本原理：体现、感知、运动行动、学习、自主性和情境敏感性，这些原理构成封闭控制循环，其中能量、信息、控制和情境持续交互，使系统能从物理经验而非数据库中生成意义。

Result: 建立了Physical AI的理论基础，展示了六个原理作为封闭控制循环的相互作用，并通过康复诊所中的自适应辅助机器人实例说明了物理智能如何从具体经验而非抽象计算中产生。

Conclusion: Physical AI代表了理解智能的范式转变，将智能视为物理体现过程，学习不是参数调整而是智能体与环境之间结构耦合的变化，这为设计和评估物理智能系统提供了概念基础。
